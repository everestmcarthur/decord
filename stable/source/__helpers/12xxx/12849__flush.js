...
// Function ID: 12850
// Name: _flush
// Dependencies: [5, 12828, 12850, 12829, 12801, 12812, 12815, 12808, 12838, 12802, 12824]
  ...
  if (c1 === 2) {
    ...
    try {
      ...
      if (arg0 === 1) {
        ...
        const client = require("module_12828").getClient();
        if (client) {
          client.flush(tmp13);
        } else {
        ...
        c1 = 3;
        const obj3 = require("module_12828");
        tmp13 = closure_0;
      }
    } catch (tmp8) {
      c1 = tmp;
...
function endSession() {
  ...
  if (tmp3) {
    tmp(12824).closeSession(tmp3);
    const tmpResult = tmp(12824);
  }
  const isolationScope1 = _mod12828.getIsolationScope();
  const tmpResult4 = _mod12828;
  const currentScope1 = _mod12828.getCurrentScope();
  const tmpResult5 = _mod12828;
...
export const captureException = function captureException(arg0, arg1) {
  const currentScope = _mod12828.getCurrentScope();
  return currentScope.captureException(arg0, _mod12850.parseEventHintOrCaptureContext(arg1));
};
...
export const startSession = function startSession(arg0) {
  ...
  if (undefined === environment) {
    environment = tmp(12838).DEFAULT_ENVIRONMENT;
  }
  ...
  if (tmp10) {
    tmp(12824).updateSession(session1, { status: "exited" });
...
export const withMonitor = function withMonitor(monitorSlug, arg1, arg2) {
  ...
  function finishCheckIn(status) {
    const obj = { monitorSlug, status, checkInId: captureCheckInResult, duration: _mod12815.timestampInSeconds() - closure_3 };
    const currentScope = _mod12828.getCurrentScope();
    const client = _mod12828.getClient();
    if (client) {
      if (client.captureCheckIn) {
  ...
  let currentScope = require("module_12828").getCurrentScope();
  let obj = { monitorSlug, status: "in_progress" };
  let obj2 = require("module_12828");
  let client = require("module_12828").getClient();
  if (client) {
    if (client.captureCheckIn) {
    ...
    closure_3 = tmp(12815).timestampInSeconds();
    const tmpResult = tmp(12815);
    return tmp(12828).withIsolationScope(() => {
      try {
  ...
  let obj3 = require("module_12828");
  captureCheckInResult = require("module_12812").uuid4();
};
