...
// Function ID: 9148
// Name: _asyncLoop
// Dependencies: [41, 42, 5, 9148, 9149]
...
export const concatBytes = function concatBytes() {
  ...
  if (0 < items.length) {
    do {
      let arr2 = items[num];
      let bytesResult = Hash(9148).bytes(arr2);
...
export const randomBytes = function randomBytes(result) {
  ...
  if (Hash(9149).crypto) {
    if (typeof tmp(9149).crypto.getRandomValues === "function") {
      const _crypto2 = tmp(9149).crypto;
      const _Uint8Array = Uint8Array;
  ...
  if (Hash(9149).crypto) {
    if (typeof tmp(9149).crypto.randomBytes === "function") {
      const _crypto = tmp(9149).crypto;
