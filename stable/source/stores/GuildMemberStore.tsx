...
function computeDerivedMemberState(unsafeMutableRoles, roles) {
  ...
  let tmp4;
  if (0 === roles.length) {
    return { colorString: null, colorStrings: null, colorRoleId: "ao", hoistRoleId: "no", iconRoleId: "__initData", highestRoleId: "a" };
  } else {
    ...
    if (null == guild) {
      ...
      if (null != tmp13[user.id].communicationDisabledUntil) {
        if (tmp32Result.isMemberCommunicationDisabled(tmp15)) {
          ...
          if (result) {
            result = tmp32(4188).isMemberCommunicationDisabled(tmp15);
            const tmp32Result2 = tmp32(4188);
          }
          if (result) {
      ...
      const item = members.forEach((user) => {
        ...
        if (null != dependencyMap[user.user.id].communicationDisabledUntil) {
          if (tmp5Result5.isMemberCommunicationDisabled(tmp9)) {
            ...
            if (result) {
              result = tmp5(4188).isMemberCommunicationDisabled(tmp9);
              const tmp5Result6 = tmp5(4188);
            }
            if (result) {
      ...
      if (null == guild) {
        ...
        if (null != tmp[guildMember.user.id].communicationDisabledUntil) {
          if (tmp18Result.isMemberCommunicationDisabled(tmp22)) {
            ...
            if (result) {
              result = tmp18(4188).isMemberCommunicationDisabled(tmp22);
              const tmp18Result2 = tmp18(4188);
            }
            if (result) {
