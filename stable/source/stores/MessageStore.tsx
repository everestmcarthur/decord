...
const messageStore = new MessageStore(DispatcherDefault, {
  ...
  MESSAGE_CREATE: function handleIncomingMessage(isPushNotification) {
    ...
    if (isPushNotification.isPushNotification) {
      if (tmp3Result.isIOSPushNotificationRawPayloadFixExperimentEnabled()) {
        ...
        tmp(5272).commit(orCreate.receivePushNotification(message, isConnectedResult));
      ...
      if (ready) {
        ...
        if (tmp6) {
          ...
          tmp(5272).commit(receiveMessageResult);
          receiveMediaMentionMessage(message);
          const tmpResult2 = tmp(5272);
        }
        ready = tmp6;
      }
      return ready;
      ...
      if (orCreate.has(id)) {
        ...
        if (null != message_id) {
          if ("content" in message) {
            const orCreate1 = tmp(5272).getOrCreate(message.media_mention.attachment_id);
        ...
        if (orCreate.revealedMessageId !== id) {
          ...
          if (null != value) {
            ...
            if (null != attachment_id) {
              value2 = tmp(5272).get(attachment_id);
              if (null != value2) {
  ...
  MESSAGE_DELETE_BULK: function handleMessageDeleteBulk(ids) {
    ids = ids.ids;
    let mutation;
    const orCreate = mutation(5272).getOrCreate(ids.channelId);
    if (null == orCreate) {
      return false;
    } else {
