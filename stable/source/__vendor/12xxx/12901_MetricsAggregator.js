...
const entry = {
  ...
  value: function add(metricType, arg1, diff, none, tags) {
    ...
    const bucketKey = MetricsAggregator(12902).getBucketKey(metricType, sanitizeMetricKeyResult, sanitizeUnitResult, sanitizeTagsResult);
    const _buckets = this._buckets;
    value = _buckets.get(bucketKey);
    let num = 0;
    if (value) {
      num = 0;
      if (metricType === tmp5(12898).SET_METRIC_TYPE) {
    ...
    if (value) {
      ...
      const tmp15 = new tmp5(12903).METRIC_MAP[metricType](diff);
      obj7.metric = tmp15;
    ...
    const result1 = MetricsAggregator(12806).updateMetricSummaryOnActiveSpan(metricType, sanitizeMetricKeyResult, diff, sanitizeUnitResult, obj, bucketKey);
