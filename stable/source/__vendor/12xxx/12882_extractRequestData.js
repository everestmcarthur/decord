...
// Dependencies: [12883, 12884, 12885, 12808, 12846, 12810, 12800, 12801, 12807]
...
// Module 12882 (extractRequestData)
...
function extractRequestData(headers, arg1) {
  ...
  let item = include.forEach((item) => {
    if ("headers" === item) {
      ...
      if (!obj5.includes("ip")) {
        const ipHeaderNames = _mod12884.ipHeaderNames;
      ...
      if (!cookies) {
        ...
        if (cookie) {
          cookie = _mod12885.parseCookie(tmp21.cookie);
        }
        cookies = cookie;
      }
      if (!cookies) {
...
function headersToDict(arg0) {
  ...
  try {
    ...
    if (_mod12800.DEBUG_BUILD) {
      const logger = tmp4(12801).logger;
      logger.warn("Sentry failed extracting headers from a request object. If you see this, please file an issue.");
    }
    tmp4 = require;
  }
...
export const addNormalizedRequestDataToEvent = function addNormalizedRequestDataToEvent(request, normalizedRequest, user, include) {
  ...
  if (obj.request) {
    ...
    if (obj2.includes("cookies")) {
      ...
      if (!cookies) {
        ...
        if (obj4.cookie) {
          parseCookieResult = _mod12885.parseCookie(obj4.cookie);
        }
  ...
  if (obj.ip) {
    ...
    if (ipAddress) {
      ipAddress = _mod12884.getClientIPAddress(normalizedRequest.headers);
    }
    if (!ipAddress) {
...
export const addRequestDataToEvent = function addRequestDataToEvent(request, request, include) {
  ...
  if (obj.ip) {
    ...
    if (ip) {
      ip = _mod12884.getClientIPAddress(request.headers);
    }
    if (!ip) {
      ip = request.ip;
    }
...
export const httpRequestToRequestData = function httpRequestToRequestData(headers) {
  ...
  return _mod12807.dropUndefinedKeys(request);
};
