...
export const UnicodeExtensionComponents = function UnicodeExtensionComponents(str) {
  _mod14244.invariant(str === str.toLowerCase(), "Expected extension to be lowercase");
  let num = 3;
  _mod14244.invariant("-u-" === str.slice(0, 3), "Expected extension to be a Unicode locale extension");
  const attributes = [];
  const keywords = [];
  if (3 < str.length) {
    while (true) {
      ...
      let tmp8 = require;
      let invariantResult2 = _mod14244.invariant(tmp6 >= 2, "Expected a subtag to have at least 2 characters");
      if (undefined === iter) {
        if (2 !== tmp6) {
          let tmp13 = iter;
