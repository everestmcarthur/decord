...
// Dependencies: [1162, 14208, 14230, 14209, 14223, 14225]
// Exports: PartitionNumberPattern

// Module 14235 (PartitionNumberPattern)
import digitsToString2 from "digitsToString" /* 14208 */;
...
let e = e_mod;
const module_14230 = e.__importDefault(_mod14230);
...
export const PartitionNumberPattern = function PartitionNumberPattern(internalSlots, isNaN) {
  ...
  if (isNaN.isNaN()) {
    ...
    if (!isNaN.isZero()) {
      _mod14209.invariant(isNaN.isFinite(), "Input must be a mathematical value");
  ...
  if ("never" !== signDisplay) {
    if ("auto" === signDisplay) {
      ...
      _mod14209.invariant("negative" === signDisplay, "signDisplay must be \"negative\"");
      let num8 = 0;
      if (roundedNumber.isNegative()) {
        num8 = 0;
        if (!roundedNumber.isZero()) {
  ...
  return module_14230.default({ roundedNumber, formattedString, exponent: num, magnitude: num2, sign: num7 }, internalSlots.dataLocaleData, pl, internalSlots);
};
