...
let items = [
  entry,
  {
    ...
    value: function write(arg0, tag, writeUnknownFields) {
      ...
      if (false !== onWrite) {
        if (true === onWrite) {
          onWrite = ReflectionBinaryWriter(1192).UnknownFieldHandler.onWrite;
        }
  ...
  },
  {
    ...
    value: function message(tag, arg1, internalBinaryWrite, arg3, arg4) {
      if (undefined !== arg4) {
        ...
        const joined = tag.join();
        const tagResult = tag.tag(arg3, ReflectionBinaryWriter(1192).WireType.LengthDelimited);
      }
    }
  },
  {
  ...
  },
  {
    ...
    value: function packed(tag, arg1, no, arg3) {
      let length;
      if (arg3.length) {
        const obj = ReflectionBinaryWriter(1197);
        const self = this;
        obj.assert(arg1 !== ReflectionBinaryWriter(1201).ScalarType.BYTES && arg1 !== ReflectionBinaryWriter(1201).ScalarType.STRING);
        tag.tag(no, ReflectionBinaryWriter(1192).WireType.LengthDelimited);
        tag.fork();
        let num2 = 0;
        if (0 < arg3.length) {
          do {
        ...
        const joined = tag.join();
        const tmp4 = arg1 !== ReflectionBinaryWriter(1201).ScalarType.BYTES && arg1 !== ReflectionBinaryWriter(1201).ScalarType.STRING;
      }
    }
  },
  {
    ...
    value: function scalarInfo(arg0, byteLength) {
      const Varint = ReflectionBinaryWriter(1192).WireType.Varint;
