...
// Dependencies: [6866, 6867, 6879, 6881, 6882, 6854, 6884, 6895, 6896, 6897]

// Module 6878 (RecyclerViewManager)
import _modDef6867 from "module_6867" /* 6867 */;
import ViewabilityManagerDefault from "ViewabilityManager" /* 6882 */;
  ...
  },
  {
    ...
    value: function getLayoutManagerClass() {
      ...
      if (self.numColumns > 1) {
        if (self.propsRef.horizontal) {
          const _Error = Error;
          const error1 = new Error(RecyclerViewManager(6854).ErrorMessages.numColumnsAndHorizontalIncompatible);
          throw error1;
        }
      }
      if (self.propsRef.masonry) {
        let RVLinearLayoutManagerImpl = RecyclerViewManager(6884).RVMasonryLayoutManagerImpl;
      } else {
        if (self.numColumns > 1) {
          if (!self.propsRef.horizontal) {
            RVLinearLayoutManagerImpl = RecyclerViewManager(6895).RVGridLayoutManagerImpl;
          }
        }
        RVLinearLayoutManagerImpl = RecyclerViewManager(6896).RVLinearLayoutManagerImpl;
      }
