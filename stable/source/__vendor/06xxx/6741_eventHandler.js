...
// Function ID: 6742
// Name: eventHandler
// Dependencies: [6723, 6661, 6676, 6663, 6725]
// Exports: eventHandler

// Module 6741 (eventHandler)
import _mod6661 from "module_6661" /* 6661 */;
import TouchEventType from "TouchEventType" /* 6663 */;
...
function handleStateChangeEvent(result, arg1, lastUpdateEvent, fn) {
  ...
  if (oldState === _mod6661.State.UNDETERMINED) {
    if (state === tmp(6661).State.BEGAN) {
      tmp(6723).runCallback(tmp(6676).CALLBACK_TYPE.BEGAN, arg1, result);
  ...
  if (oldState === _mod6661.State.BEGAN) {
    if (state === tmp(6661).State.ACTIVE) {
      ...
      tmp(6723).runCallback(tmp(6676).CALLBACK_TYPE.START, arg1, result);
  ...
  if (oldState !== state) {
    ...
    obj2.canceled = state === tmp(6661).State.FAILED || state === tmp(6661).State.CANCELLED;
    if (oldState === tmp(6661).State.ACTIVE) {
      if (fn != null) {
        fn(obj2);
      }
      tmp(6723).runCallback(tmp(6676).CALLBACK_TYPE.END, arg1, obj2);
      const tmpResult5 = tmp(6723);
    }
    const tmp7 = state === tmp(6661).State.FAILED || state === tmp(6661).State.CANCELLED;
    tmp(6723).runCallback(tmp(6676).CALLBACK_TYPE.FINALIZE, arg1, obj2);
    if (lastUpdateEvent) {
      lastUpdateEvent.lastUpdateEvent = undefined;
    }
    const tmpResult6 = tmp(6723);
  }
}
handleStateChangeEvent.__closure = { flattenAndFilterEvent: fn(6723).flattenAndFilterEvent, State: fn(6661).State, runCallback: fn(6723).runCallback, CALLBACK_TYPE: fn(6676).CALLBACK_TYPE };
...
let obj = { flattenAndFilterEvent: fn(6723).flattenAndFilterEvent, State: fn(6661).State, runCallback: fn(6723).runCallback, CALLBACK_TYPE: fn(6676).CALLBACK_TYPE };
handleUpdateEvent.__closure = { flattenAndFilterEvent: fn(6723).flattenAndFilterEvent, runCallback: fn(6723).runCallback, CALLBACK_TYPE: fn(6676).CALLBACK_TYPE };
...
function handleTouchEvent(eventType, arg1) {
  if (eventType.eventType !== TouchEventType.TouchEventType.UNDETERMINED) {
    const tmpResult = tmp(6723);
    tmpResult.runCallback(tmp(6723).touchEventTypeToCallbackType(eventType.eventType), arg1, eventType);
    const tmpResult2 = tmp(6723);
  }
}
let obj2 = { flattenAndFilterEvent: fn(6723).flattenAndFilterEvent, runCallback: fn(6723).runCallback, CALLBACK_TYPE: fn(6676).CALLBACK_TYPE };
handleTouchEvent.__closure = { TouchEventType: fn(6663).TouchEventType, runCallback: fn(6723).runCallback, touchEventTypeToCallbackType: fn(6723).touchEventTypeToCallbackType };
      ...
      if (tmpResult6.isTouchEvent(result)) {
        if (typeof handleTouchEvent === "function") {
          if (result.eventType !== tmp(6663).TouchEventType.UNDETERMINED) {
            const tmpResult7 = tmp(6723);
            tmpResult7.runCallback(tmp(6723).touchEventTypeToCallbackType(result.eventType), arg2, result);
            const tmpResult8 = tmp(6723);
          }
        } else {
...
const obj3 = { TouchEventType: fn(6663).TouchEventType, runCallback: fn(6723).runCallback, touchEventTypeToCallbackType: fn(6723).touchEventTypeToCallbackType };
eventHandler.__closure = { maybeExtractNativeEvent: fn(6723).maybeExtractNativeEvent, isEventForHandlerWithTag: fn(6723).isEventForHandlerWithTag, isStateChangeEvent: fn(6725).isStateChangeEvent, handleStateChangeEvent, isTouchEvent: fn(6725).isTouchEvent, handleTouchEvent, handleUpdateEvent };
