...
// Dependencies: [32, 19, 17, 21, 6673, 6733, 6669, 6728, 6670, 6660, 6698, 6729, 6730, 6722, 6731]
...
export const InterceptingGestureDetector = function InterceptingGestureDetector(gesture) {
  ...
  gesture(6673).useEnsureGestureHandlerRootView();
  ...
  if (ReanimatedNativeDetector) {
    ...
    const result = tmp(6729).ensureNativeDetectorComponent(ReanimatedNativeDetector);
    const tmpResult = tmp(6729);
    const gestureRelationsUpdater = tmp(6730).useGestureRelationsUpdater(gesture);
    ...
    const detectorAttachmentGuard = tmp(6731).useDetectorAttachmentGuard(tmp8Result5);
    ...
    obj4.style = tmp(6669).nativeDetectorStyles.detector;
    ...
    return jsx(tmp(6733).InterceptingDetectorContext, obj3);
  } else {
    const _Error = Error;
