...
// Function ID: 5223
// Name: findOffsets
// Dependencies: [5223, 5216, 5213]
// Exports: findOffsets

// Module 5222 (findOffsets)
import _mod5213 from "module_5213" /* 5213 */;
import _modDef5216 from "module_5216" /* 5216 */;
import get64BitValue from "get64BitValue" /* 5223 */;
...
function parseBox(getUint32, sum) {
  ...
  if (length >= 8) {
    ...
    if (1718909296 === uint321) {
      const obj4 = { type: "ftyp", majorBrand: _mod5213.getStringFromDataView(getUint32, contentOffset, 4), length };
      return obj4;
      ...
      if (1835365473 === uint321) {
        ...
        if (!tmp3) {
          ...
          const sum8 = sum7 + 2;
          obj17.itemName = _mod5213.getNullTerminatedStringFromDataView(getUint32, sum8);
        ...
        if (uint8 >= 2) {
          ...
          const sum12 = sum11 + 4;
          obj17.itemName = _mod5213.getNullTerminatedStringFromDataView(getUint32, sum12);
          const sum13 = sum12 + (obj17.itemName.length + 1);
          if (obj17.itemType === c4) {
            obj17.contentType = tmp13(5213).getNullTerminatedStringFromDataView(getUint32, sum13);
            const sum14 = sum13 + (obj17.contentType.length + 1);
            parseItemLocationBoxResult = obj17;
            if (sum + length > sum14) {
              obj17.contentEncoding = tmp13(5213).getNullTerminatedStringFromDataView(getUint32, sum14);
              const sum15 = obj17.contentEncoding.length + 1;
              parseItemLocationBoxResult = obj17;
              const tmp13Result3 = tmp13(5213);
            }
            const tmp13Result = tmp13(5213);
          } else {
            parseItemLocationBoxResult = obj17;
            if (obj17.itemType === c5) {
              obj17.itemUri = tmp13(5213).getNullTerminatedStringFromDataView(getUint32, sum13);
              const sum16 = obj17.itemUri.length + 1;
              parseItemLocationBoxResult = obj17;
              const tmp13Result4 = tmp13(5213);
            }
          }
        }
      } else {
