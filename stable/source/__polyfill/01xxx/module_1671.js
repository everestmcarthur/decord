...
import startWebLayoutAnimation from "startWebLayoutAnimation" /* 1690 */;
import NativeEventsManager from "NativeEventsManager" /* 1743 */;
...
export const createAnimatedComponent = function createAnimatedComponent(name, arg1) {
  ...
  const entry = {
    ...
    value: function componentDidMount() {
      ...
      if (!closure_16) {
        _willUnmount = tmp25(1639).isFabric();
        const tmp25Result = tmp25(1639);
      }
      if (_willUnmount) {
      ...
      if (_willUnmount) {
        const result5 = tmp25(1680).unmarkNodeAsRemovable(componentViewTag);
        const tmp25Result2 = tmp25(1680);
      }
      self._isFirstRender = false;
    }
  };
  let items = [
          ...
          if (self._componentDOMRef) {
            if (exiting) {
              if (tmp4Result.getReducedMotionFromConfig(exiting)) {
                ...
                const result2 = tmp4(1723).addHTMLMutationObserver();
                const tmp4Result8 = tmp4(1723);
        ...
        if (exiting) {
          if (!module_1639) {
            if (!tmp4Result12.isFabric()) {
              ...
              const tmp4Result14 = tmp4(1676);
            }
            tmp4Result12 = tmp4(1639);
          }
        }
      }
    },
        ...
        if (undefined !== this._viewInfo) {
          ...
          if (closure_1_16) {
            ...
            if (findHostInstanceResult) {
              const viewInfo = tmp(1749).getViewInfo(findHostInstanceResult);
              ({ viewTag, viewName, viewConfig } = viewInfo);
              const tmpResult = tmp(1749);
    ...
    },
    {
      ...
      value: function componentDidUpdate(layout, arg1, arg2) {
        ...
        if (_componentDOMRef) {
          tmp(1690).saveSnapshot(self._componentDOMRef);
          const tmpResult = tmp(1690);
        }
        if (module_1639) {
          if (arg2) {
