// Module ID: 9153
// Function ID: 9154
// Dependencies: [41, 42, 93, 95, 98, 9147, 9148]
// Exports: Chi, Maj
...
const entry = {
  ...
  value: function update(B) {
    let tmp8;
    const self = this;
    HashMD(9148).exists(this);
    ({ buffer, blockLen, view } = this);
    const toBytesResult = HashMD(9147).toBytes(B);
    let num = 0;
...
let items = [
  entry,
  {
    ...
    value: function digestInto(content) {
      const self = this;
      HashMD(9148).exists(this);
      HashMD(9148).output(content, this);
      this.finished = true;
      ...
      self.process(view, 0);
      const view1 = HashMD(9147).createView(content);
