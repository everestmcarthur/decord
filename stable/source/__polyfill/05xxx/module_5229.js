...
export default {
  read(dataView, sum) {
    ...
    let tmp4;
    if (8 <= shortAt) {
      const byteAt = tmp(5230).getByteAt(dataView, sum + 7);
    ...
    let tmp6;
    if (3 <= shortAt) {
      const byteAt1 = tmp(5230).getByteAt(dataView, sum + 2);
      const obj3 = { value: byteAt1, description: "" + byteAt1 };
      tmp6 = obj3;
      const tmpResult4 = tmp(5230);
    }
