// Module ID: 4916
// Function ID: 4917
// Dependencies: [109, 19, 17, 21, 4917, 4918, 4919, 4915, 4920, 4921, 4922, 4924, 4925, 4926]
...
const forwardRefResult = noop.forwardRef(function InnerScreen(activityState, ref) {
  ...
  const previous = _mod4919.usePrevious(activityState.activityState);
  ...
  if (undefined === enabled) {
    enabled = tmp2(4915).screensEnabled();
    const tmp2Result = tmp2(4915);
  }
  let freezeOnBlur = activityState.freezeOnBlur;
  if (undefined === freezeOnBlur) {
    freezeOnBlur = tmp2(4915).freezeEnabled();
    const tmp2Result6 = tmp2(4915);
  }
  ...
  if (enabled) {
    if (tmp2(4915).isNativePlatformSupported) {
      ...
      const sheetInitialDetentIndex1 = tmp2(4920).resolveSheetInitialDetentIndex(num3, sheetAllowedDetents1.length - 1);
      ...
      _modDef4921(false, "[RNScreens] Using featureFlags `ios26AllowInteractionsDuringTransition` with `iosPreventReattachmentOfDismissedScreens` disabled is discouraged and will result in visual bugs on screen transitions. See flags description for details.");
      if (isNativeStack) {
      ...
      obj4.fullScreenSwipeEnabled = tmp2(4924).parseBooleanToOptionalBooleanNativeProp(fullScreenSwipeEnabled);
      let num7;
      ...
      if (isNativeStack) {
        ...
        tmp33Result = tmp33(tmp31(4926).Provider, obj7);
      }
