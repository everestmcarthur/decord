...
import COUNTER_METRIC_TYPE from "COUNTER_METRIC_TYPE" /* 12898 */;
...
function addToMetricsAggregator(arg0, SET_METRIC_TYPE, arg2, arg3, arg4) {
  ...
  if (client) {
    ...
    let rootSpan;
    if (activeSpan) {
      rootSpan = tmp3(12806).getRootSpan(activeSpan);
      const tmp3Result = tmp3(12806);
    }
    let description = rootSpan;
    if (rootSpan) {
      description = tmp3(12806).spanToJSON(rootSpan).description;
    ...
    const globalSingleton = _mod12802.getGlobalSingleton("globalMetricsAggregators", () => {
...
export const metrics = {
  ...
  timing(arg0, name, fn) {
    ...
    if (typeof fn === "function") {
      let timestampInSecondsResult = require("module_12815").timestampInSeconds();
      ...
      return require("module_12835").startSpanManual(obj3, (arg0) => {
        closure_0 = arg0;
        return closure_0(name[10]).handleCallbackErrors(() => fn(), () => {

        }, () => {
          const timestampInSecondsResult = _mod12815.timestampInSeconds();
