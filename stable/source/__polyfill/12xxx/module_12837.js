// Module ID: 12837
// Function ID: 12838
// Dependencies: [12807, 12838, 12828, 12806, 12814, 12816, 12833]
...
const _mod12838 = tmp3(12838);
require = arg1;
const dependencyMap = arg6;
function getDynamicSamplingContextFromSpan(spanContext) {
  const client = _mod12828.getClient();
  if (client) {
    const rootSpan = tmp(12806).getRootSpan(spanContext);
    if (rootSpan[_frozenDsc]) {
      ...
      let result = value;
      if (value) {
        result = tmp(12814).baggageHeaderToDynamicSamplingContext(value);
        const tmpResult6 = tmp(12814);
      }
      if (result) {
        return result;
      } else {
        ...
        if (!DEFAULT_ENVIRONMENT) {
          DEFAULT_ENVIRONMENT = tmp(12838).DEFAULT_ENVIRONMENT;
        }
        ...
        const dropUndefinedKeysResult = tmp(12807).dropUndefinedKeys(obj2);
        client.emit("createDsc", dropUndefinedKeysResult);
        const tmpResult7 = tmp(12807);
        const spanToJSONResult = tmp(12806).spanToJSON(rootSpan);
        const tmp13 = spanToJSONResult.data || {};
        const tmp14 = tmp13[tmp(undefined, 12816).SEMANTIC_ATTRIBUTE_SENTRY_SAMPLE_RATE];
        if (null != tmp14) {
        ...
        tmp17 = "url" !== tmp13[tmp(undefined, 12816).SEMANTIC_ATTRIBUTE_SENTRY_SOURCE] && description;
        if (tmpResult9.hasTracingEnabled()) {
          const _String = String;
          dropUndefinedKeysResult.sampled = String(tmp(12806).spanIsSampled(rootSpan));
...
export const freezeDscOnSpan = function freezeDscOnSpan(arg0, arg1) {
  const result = _mod12807.addNonEnumerableProperty(arg0, _frozenDsc, arg1);
};
export const getDynamicSamplingContextFromClient = function getDynamicSamplingContextFromClient(trace_id, getOptions) {
  ...
  const dropUndefinedKeysResult = _mod12807.dropUndefinedKeys({ environment: DEFAULT_ENVIRONMENT, release: options.release, public_key: tmp2.publicKey, trace_id });
...
export const getDynamicSamplingContextFromScope = function getDynamicSamplingContextFromScope(getOptions, getPropagationContext) {
  ...
  if (!dsc) {
    ...
    let DEFAULT_ENVIRONMENT = options.environment;
    if (!DEFAULT_ENVIRONMENT) {
      DEFAULT_ENVIRONMENT = tmp5(12838).DEFAULT_ENVIRONMENT;
    }
    ...
    const dropUndefinedKeysResult = _mod12807.dropUndefinedKeys(obj2);
    getOptions.emit("createDsc", dropUndefinedKeysResult);
