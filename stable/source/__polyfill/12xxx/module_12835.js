// Module ID: 12835
// Function ID: 12836
// Dependencies: [32, 12828, 12834, 12823, 12836, 12806, 12818, 12819, 12820, 12813, 12811, 12829, 12801, 12833, 12837, 12839, 12830, 12840, 12842, 12816]
...
function createChildOrRootSpan(forceTransaction) {
  ...
  if (obj.hasTracingEnabled()) {
    const isolationScope = tmp(12828).getIsolationScope();
    if (parentSpan) {
      if (!forceTransaction.forceTransaction) {
        ...
        if (!tmp9) {
          spanIsSampledResult = tmp(12806).spanIsSampled(parentSpan);
        ...
        if (spanIsSampledResult) {
          ...
          let sentrySpan = new tmp(12842).SentrySpan(obj2);
        } else {
          const obj3 = { traceId };
          sentrySpan = new tmp(12834).SentryNonRecordingSpan(obj3);
        }
        const spanContextResult = parentSpan.spanContext();
        tmp(12806).addChildSpanToSpan(parentSpan, sentrySpan);
        const tmpResult12 = tmp(12806);
        const client = tmp(12828).getClient();
        if (client) {
        ...
        const tmpResult13 = tmp(12828);
        tmp(12806).addChildSpanToSpan(parentSpan, sentrySpan);
        const tmpResult14 = tmp(12806);
      }
      tmp(12839).logSpanStart(sentrySpan);
      const tmpResult15 = tmp(12839);
      const result = tmp(12830).setCapturedScopesOnSpan(sentrySpan, scope, isolationScope);
      return sentrySpan;
    }
    if (parentSpan) {
      const dynamicSamplingContextFromSpan = tmp(12837).getDynamicSamplingContextFromSpan(parentSpan);
      const tmpResult17 = tmp(12837);
      ...
      const tmpResult18 = tmp(12806);
      const merged1 = Object.assign(spanArguments);
      const tmp42 = _startRootSpan(obj4, scope, tmp(12806).spanIsSampled(parentSpan));
      const spanIsSampledResult1 = tmp(12806).spanIsSampled(parentSpan);
      tmp(12837).freezeDscOnSpan(tmp42, dynamicSamplingContextFromSpan);
      sentrySpan = tmp42;
      const tmpResult19 = tmp(12837);
    } else {
      const obj5 = {};
      ...
      sentrySpan = tmp33;
      if (dsc) {
        tmp(12837).freezeDscOnSpan(tmp33, dsc);
        sentrySpan = tmp33;
        const tmpResult20 = tmp(12837);
      }
    }
    const tmpResult = tmp(12828);
  } else {
    const sentryNonRecordingSpan = new tmp(12834).SentryNonRecordingSpan();
...
function _startRootSpan(name, arg1, parentSampled) {
  ...
  if (arg1.getScopeData().sdkProcessingMetadata[__SENTRY_SUPPRESS_TRACING__]) {
    ...
    sampleSpanResult = tmp(12840).sampleSpan(options, obj2);
    const tmpResult = tmp(12840);
  }
  ...
  obj4.attributes = { [_mod12816.SEMANTIC_ATTRIBUTE_SENTRY_SOURCE]: "custom" };
  obj4.sampled = tmp6;
  const sentrySpan = new tmp(12842).SentrySpan(obj4);
  if (undefined !== tmp7) {
    const attr = sentrySpan.setAttribute(tmp(12816).SEMANTIC_ATTRIBUTE_SENTRY_SAMPLE_RATE, tmp7);
  }
  if (client) {
    client.emit("spanStart", sentrySpan);
  }
...
export const startInactiveSpan = function startInactiveSpan(experimental) {
  ...
  let mainCarrier = require("module_12819").getMainCarrier();
    ...
    return fn(() => {
      ...
      if (_getSpanForScopeResult) {
        ...
        if (options.parentSpanIsAlwaysRootSpan) {
          rootSpan = tmp(12806).getRootSpan(_getSpanForScopeResult);
      ...
      if (experimental.onlyIfParent) {
        if (!tmp5) {
          let sentryNonRecordingSpan = new tmp(12834).SentryNonRecordingSpan();
        }
        return sentryNonRecordingSpan;
      }
...
export const startSpan = function startSpan(experimental, arg1) {
  ...
  if (asyncContextStrategy.startSpan) {
    ...
    if (experimental.startTime) {
      let obj4 = {};
      const merged1 = Object.assign(obj3);
      obj4.startTimestamp = tmp3(12806).spanTimeInputToSeconds(experimental.startTime);
      delete tmp[tmp2];
      tmp10 = obj4;
      const tmp3Result = tmp3(12806);
    }
    obj4 = tmp10;
    ({ forceTransaction: __SENTRY_SUPPRESS_TRACING__, parentSpan: createChildOrRootSpan } = experimental);
    return tmp3(12828).withScope(experimental.scope, () => {
      ...
      return undefined !== createChildOrRootSpan ? ((arg0) => {
        ...
        const currentScope = sentryNonRecordingSpan(12828).getCurrentScope();
        const obj = sentryNonRecordingSpan(12828);
