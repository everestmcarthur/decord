// Module ID: 12839
// Function ID: 12840
// Dependencies: [12829, 12806, 12801]
...
export const logSpanEnd = function logSpanEnd(spanContext) {
  if (_mod12829.DEBUG_BUILD) {
    const spanToJSONResult = tmp(12806).spanToJSON(spanContext);
...
export const logSpanStart = function logSpanStart(spanContext) {
  if (_mod12829.DEBUG_BUILD) {
    ...
    const spanIsSampledResult = tmp(12806).spanIsSampled(spanContext);
    ...
    if (rootSpan !== spanContext) {
      const tmpResult6 = tmp(12806);
      ({ op: op2, description: description2 } = tmp(12806).spanToJSON(rootSpan));
      ...
      const spanToJSONResult1 = tmp(12806).spanToJSON(rootSpan);
    }
    const logger = tmp(12801).logger;
    const _HermesInternal9 = HermesInternal;
    logger.log("" + combined + "\n  " + items.join("\n  "));
    const tmpResult5 = tmp(12806);
  }
};
