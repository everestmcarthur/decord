// Module ID: 10608
// Function ID: 10609
// Dependencies: [41, 42, 93, 95, 98, 10437, 10606, 10438, 10444]
...
const regExp = new RegExp("(" + repeatedTimeunitPattern.matchAnyPattern(_mod10606.MONTH_DICTIONARY) + ")(?:-|/|\\s*,?\\s*)(" + _mod10606.ORDINAL_NUMBER_PATTERN + ")(?!\\s*(?:am|pm))\\s*(?:(?:al|\\-|\\alle|\\del|\\s)\\s*(" + _mod10606.ORDINAL_NUMBER_PATTERN + ")\\s*)?(?:(?:-|/|\\s*,?\\s*)(" + _mod10606.YEAR_PATTERN + "))?(?=\\W|$)(?!\\:\\d)", "i");
...
const items = [
  entry,
  {
    ...
    value: function innerExtract(createParsingComponents, index) {
      const tmp3 = ENMonthNameMiddleEndianParser(10606).MONTH_DICTIONARY[index[1].toLowerCase(index[1])];
      const result = ENMonthNameMiddleEndianParser(10606).parseOrdinalNumberPattern(index[2]);
      if (result > 31) {
        return null;
      } else {
        ...
        if (index[4]) {
          parsingComponents.assign("year", tmp(10606).parseYear(index[4]));
        } else {
          parsingComponents.imply("year", tmp(10438).findYearClosestToRef(createParsingComponents.refDate, result, tmp3));
        }
        if (index[3]) {
          const result1 = tmp(10606).parseOrdinalNumberPattern(index[3]);
