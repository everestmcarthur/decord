...
// Module 10433
import ENDefaultConfiguration2 from "ENDefaultConfiguration" /* 10434 */;
...
const entry1 = {
  ...
  value: function executeParser(debug, pattern) {
    ...
    if (match) {
      ...
      if (extractResult instanceof ParsingContext(10440).ParsingResult) {
...
const entry2 = {
  ...
  value: function createParsingComponents(date) {
    ...
    if (!(date instanceof ParsingContext(10440).ParsingComponents)) {
      const self = this;
      parsingComponents = new ParsingContext(10440).ParsingComponents(this.reference, date);
    }
...
const items2 = [
  entry2,
  {
    ...
    value: function createParsingResult(sum, match, extractResult, date) {
      ...
      return new ParsingContext(10440).ParsingResult(self.reference, sum, substr, parsingComponents, parsingComponents1);
    }
  },
  {
    key: "debug",
