  ...
  value: function extractTimeComponent(assign, arg1) {
    ...
    if (num < 60) {
      if (parsed <= 24) {
        let PM1 = null;
        if (parsed >= 12) {
          PM1 = DESpecificTimeExpressionParser(10442).Meridiem.PM;
        }
        let tmp5 = PM1;
        let tmp6 = parsed;
        if (null != arg1[5]) {
          if (parsed > 12) {
            ...
            if (str8.match(/morgen|vormittag/)) {
              ...
              tmp9 = num2;
              PM1 = DESpecificTimeExpressionParser(10442).Meridiem.AM;
            }
            let tmp10 = tmp9;
            ...
            if (str8.match(/nacht/)) {
              if (12 == tmp10) {
                let PM = DESpecificTimeExpressionParser(10442).Meridiem.AM;
              } else if (tmp10 < 6) {
                PM = DESpecificTimeExpressionParser(10442).Meridiem.AM;
              } else {
                PM = DESpecificTimeExpressionParser(10442).Meridiem.PM;
                const num4 = tmp10 + 12;
              }
            }
          }
        ...
        if (null !== tmp5) {
          ...
          assign.imply("meridiem", DESpecificTimeExpressionParser(10442).Meridiem.AM);
        } else {
          assign.imply("meridiem", DESpecificTimeExpressionParser(10442).Meridiem.PM);
        }
        if (null != arg1[4]) {
