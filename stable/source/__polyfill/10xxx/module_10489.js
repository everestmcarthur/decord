// Module ID: 10489
// Function ID: 10490
// Dependencies: [41, 42, 93, 95, 98, 10437, 10483, 10438, 10444]
...
const regExp = new RegExp("(?:am\\s*?)?(?:den\\s*?)?([0-9]{1,2})\\.(?:\\s*(?:bis(?:\\s*(?:am|zum))?|\\-|\\\u2013|\\s)\\s*([0-9]{1,2})\\.?)?\\s*(" + repeatedTimeunitPattern.matchAnyPattern(_mod10483.MONTH_DICTIONARY) + ")(?:(?:-|/|,?\\s*)(" + _mod10483.YEAR_PATTERN + "(?![^\\s]\\d)))?(?=\\W|$)", "i");
...
const items = [
  entry,
  {
    ...
    value: function innerExtract(createParsingResult, index) {
      ...
      const tmp4 = DEMonthNameLittleEndianParser(10483).MONTH_DICTIONARY[index[3].toLowerCase(index[3])];
      ...
      if (parsed > 31) {
        ...
        if (index[4]) {
          ...
          start2.assign("year", tmp2(10483).parseYear(index[4]));
        } else {
