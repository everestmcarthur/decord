...
// Function ID: 7725
// Name: QuestTaskUtils
// Dependencies: [7703, 5452, 12, 1090, 7699, 1369, 2]
...
export const getQuestTaskDetails = function getQuestTaskDetails(value, DESKTOP) {
  if (arr.some((item) => null != value.config.taskConfigV2.tasks[item])) {
    ...
    if (closure_133_0.some((item) => null != config.config.taskConfigV2.tasks[item])) {
      ...
      if (closure_136_0.some((item) => null != config.config.taskConfigV2.tasks[item])) {
        ...
        if (tmp8) {
          const obj4 = { quest: value, taskType: tmp(5452).FirstPartyQuestTaskTypes.PLAY_ON_DESKTOP };
          ...
          const obj5 = { quest: value, taskType: tmp(5452).FirstPartyQuestTaskTypes.PLAY_ACTIVITY };
          tmp13Result = _getTaskDetailsForType(obj5);
        } else {
          const obj6 = { quest: value, taskType: tmp(5452).FirstPartyQuestTaskTypes.STREAM_ON_DESKTOP };
  ...
  if (null != tmp13Result) {
    ...
    if (null == tmp26) {
      const obj8 = { progressSeconds: 0, targetSeconds: 1, targetMinutes: 1, percentComplete: 0, taskType: tmp(5452).FirstPartyQuestTaskTypes.STREAM_ON_DESKTOP };
      tmp26 = obj8;
    }
    return tmp26;
  }
