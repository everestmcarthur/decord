...
// Dependencies: [32, 19, 17, 7703, 5444, 21, 4560, 576, 7709, 11281, 4259, 1114, 5597, 11488, 11501, 5447, 15094, 504, 11783, 7724, 7150, 15124, 15125, 11779, 11491, 7728, 11197, 11198, 11202, 5451, 15163, 15165, 15166, 7722, 8594, 4556, 2]
// Exports: default
...
function useEnrolledQuestContentProps(quest) {
  ...
  _slicedToArray = quest(11491).useQuestImpressionId();
  let obj2 = quest(11491);
  ...
  const tmpResult = quest(11488);
  const questTaskDetails1 = quest(11488).useQuestTaskDetails(quest);
  const tmpResult7 = quest(11488);
  let isQuestProgressing1 = quest(11488).useIsQuestProgressing(quest);
  const tmpResult8 = quest(11488);
  const tmp5Result = _slicedToArray(quest(11488).useTaskPlatformScreen(quest, questTaskDetails1), 3);
  ...
  obj7.onNext = onNext;
  items5[1] = obj7;
  const tmpResult11 = quest(15094);
  closure_130_0 = quest;
  [tmp32, tmp33] = items5;
  const tmp5Result6 = _slicedToArray(items5, 2);
  const questLogger = quest(7709).getQuestLogger({ quest, location: constants.QUEST_HOME_MOBILE });
  ...
  const obj10 = {
    ...
    start: noop.useCallback(() => {
      ...
      result.then((errorHints) => dependencyMap(errorHints.errorHints)).catch((error) => {
        ...
        obj2.icon = sourceQuestContent(5597);
        sourceQuestContent(4259).open(obj2);
      }).finally(() => impressionId(false));
    }, items6)
  };
  const tmp40 = null == completedAt1 && !isQuestProgressing && tmp6[0] === quest(5447).TaskPlatformScreen.CONSOLE && !hasWatchVideoOnMobileTasks;
  return {
    quest,
    defibrillator: obj10,
    step: tmp32,
    ...
    handleTaskSelect(arg0) {
      ...
      if (tmp4Result.shouldMigrateToAdAnalyticsInterface(tmp4(11197).AdAnalyticsInterfaceExperimentStep.STEP_2_CLICKED_INTERNAL, "quest_bottom_sheet")) {
        const obj = { type: tmp4(11202).AdUserActionType.CLICK_INTERNAL, adCreativeType: tmp4(5451).AdCreativeType.QUEST, adCreativeId: quest.id, questContentCTA: DESELECT_PLATFORM, surfaceId: tmp4(5447).QuestContent.QUEST_BOTTOM_SHEET, sourceQuestContent, impressionId };
        tmp4(11198).captureAdUserAction(obj);
        const tmp4Result2 = tmp4(11198);
      } else {
        const obj2 = { questId: quest.id, questContent: tmp4(5447).QuestContent.QUEST_BOTTOM_SHEET, questContentCTA: DESELECT_PLATFORM, sourceQuestContent };
        closure_2(obj2);
      }
      closure_4(arg0);
    }
...
function MicrophoneUnit(arg0) {
  ...
  if (num > 0) {
    ...
    const items1 = [closure_1_10(tmp2(8594).WarningIcon, obj4), ];
    ...
    items1[1] = closure_1_10(tmp2(4556).Text, obj6);
