...
// Dependencies: [19, 17, 1074, 21, 4560, 576, 4527, 7382, 9972, 4255, 2024, 4975, 1114, 1611, 7154, 4271, 4556, 11298, 3353, 7150, 15167, 2]
// Exports: default

// Module 15170 (QuestOrbMultiplierPerkInfoActionSheet)
...
function Footer(eligibleToReceivePremiumRewards) {
  ...
  if (eligibleToReceivePremiumRewards.eligibleToReceivePremiumRewards) {
    ...
    items[1] = tmp9(tmp10(4975).Button, obj3);
    obj.children = items;
    let tmp11 = obj;
  } else {
    ...
    items1[1] = tmp9(tmp10(4975).Button, obj5);
    obj.children = items1;
    tmp11 = obj;
  }
...
export default function QuestOrbMultiplierPerkInfoActionSheet(multiplier) {
  ...
  const result = multiplier(11298).shouldReceiveQuestOrbMultiplier(orbMultiplierEligibility);
  ...
  const memo1 = noop.useMemo(() => {
    if (orbMultiplierEligibility === QuestOrbMultiplierUtils.QuestOrbMultiplierEligibilityType.CREPE) {
      ...
      let formatResult = intl2.format(_modDef3353.UkrcSH, obj2);
    } else {
      const intl = tmp(1114).intl;
  ...
  let obj3 = { visible: orbMultiplierEligibility === multiplier(11298).QuestOrbMultiplierEligibilityType.NITRO || orbMultiplierEligibility === multiplier(11298).QuestOrbMultiplierEligibilityType.UPSELL, children: null };
  const tmp4 = orbMultiplierEligibility === multiplier(11298).QuestOrbMultiplierEligibilityType.NITRO || orbMultiplierEligibility === multiplier(11298).QuestOrbMultiplierEligibilityType.UPSELL;
  obj3.children = closure_8(SheetContent, { title: memo, body: memo1, eligibleToReceivePremiumRewards: result });
  obj2.children = closure_8(orbMultiplierEligibility(15167), obj3);
  return closure_8(multiplier(7150).BottomSheet, obj2);
};
