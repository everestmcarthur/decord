...
// Dependencies: [32, 19, 17, 1076, 21, 8770, 576, 4560, 8816, 38, 1889, 8817, 8818, 8172, 8803, 5587, 8851, 1077, 8852, 8805, 8828, 8830, 8771, 7553, 4409, 4262, 2]

// Module 15090 (QuestHomeOrbShopRewardCardAssetTile)
...
import useDefaultVariantIndex from "useDefaultVariantIndex" /* 8771 */;
import AvatarDecorationSampleV2 from "AvatarDecorationSampleV2" /* 8816 */;
import _modDef8817 from "module_8817" /* 8817 */;
import CutoutableAvatarDecorationDefault from "CutoutableAvatarDecoration" /* 8818 */;
...
function CompactAvatarDecorationPreview(arg0) {
  ...
  const items = [React6(hasOwnProperty, { style: tmp.avatar, resizeMode: "contain", source: _modDef8817, accessible: false }), ];
  const obj2 = { style: tmp.avatar, resizeMode: "contain", source: _modDef8817, accessible: false };
...
function ProductPreviewInner(cardHeight) {
  ...
  let tmp = closure_16();
  const shopProductItems = cardWidth(8172).useShopProductItems(product);
  ({ firstProfileEffect, firstAvatarDecoration, firstNameplate } = shopProductItems);
  const tmp5 = cardWidth < cardWidth(8770).COLLECTIBLES_SHOP_CARD_WIDTH || cardHeight < cardWidth(8770).COLLECTIBLES_SHOP_CARD_HEIGHT;
  dependencyMap = tmp5;
  ...
  if (product.type === cardWidth(1889).CollectiblesItemType.BUNDLE) {
    ...
    return closure_8(cardHeight(8803), obj2);
  } else if (product.skuId === EXTERNAL_PRODUCT_SKU_IDS.ORB_PROFILE_BADGE) {
    const obj3 = { source: null, style: null };
    const obj4 = { uri: cardHeight(8851) };
    obj3.source = obj4;
    obj3.style = tmp.externalProductImage;
    return closure_8(cardHeight(5587), obj3);
  } else {
    const ALL = tmp2(1077).FractionalPremiumSKUsSets.ALL;
    if (ALL.has(product.skuId)) {
      size = { skuId: product.skuId, width: tmp2(8852).FRACTIONAL_NITRO_COIN_SIZE.CARD, height: tmp2(8852).FRACTIONAL_NITRO_COIN_SIZE.CARD };
      return closure_8(tmp2(8852).FractionalNitroCoinIllustration, size);
    } else {
      ...
      if (tmp2(1889).CollectiblesItemType.AVATAR_DECORATION === type) {
        if (flag) {
          ...
          tmp24Result = tmp24(cardHeight(8816), obj6);
        }
        return tmp24Result;
        ...
        obj7.children = closure_8(cardHeight(8805), obj8);
        ...
        if (flag) {
          ...
          if (prop == null) {
            prop = tmp2(8770).COLLECTIBLES_SHOP_CARD_WIDTH - PX_32;
          }
          ...
          obj9.children = tmp14(cardHeight(8828), obj10);
        ...
        return closure_8(cardHeight(8830), obj11);
      } else {
        return null;
      }
    }
...
let c10 = 100;
const diff = fn(8770).COLLECTIBLES_SHOP_CARD_HEIGHT - 2 * nativeDefault.space.PX_16;
