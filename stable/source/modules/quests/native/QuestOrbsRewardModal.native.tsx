...
// Dependencies: [32, 5, 19, 17, 4552, 1371, 1895, 5444, 21, 4763, 11784, 1896, 4560, 576, 8843, 5631, 5680, 1114, 5587, 8814, 1116, 11785, 8308, 11786, 11787, 11788, 11789, 504, 8856, 1093, 11295, 11790, 11772, 5447, 7123, 4556, 4975, 2]
...
import APNGPlayer from "APNGPlayer" /* 8814 */;
...
function AnimatedOrbContainer(isAppActive) {
  ...
  const items1 = [tmp13, , ];
  const obj3 = { uri: _modDef11788, style: null, onLoad: null, animate: null };
  ...
  if (tmp15Result) {
    const obj5 = { uri: _modDef11789, style: null, onLoad: null, animate: null };
...
export default function QuestOrbsRewardModal(quest) {
  ...
  let obj = num(504);
  num = num(8856).useFetchVirtualCurrencyBalance().balance;
  let obj2 = num(8856);
  ...
  if (!tmp6) {
    ...
    let items6 = [tmp17(tmp2(4556).Heading, obj14), ];
    ...
    items6[1] = tmp17(tmp2(4556).Text, obj16);
    ...
    tmp15(tmp2(7123).SafeAreaPaddingView, rect);
  }
};
