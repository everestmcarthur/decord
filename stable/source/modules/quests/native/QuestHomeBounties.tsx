...
// Dependencies: [32, 19, 17, 7702, 5444, 1954, 21, 576, 15071, 4560, 11779, 1114, 5692, 4556, 15068, 1943, 7388, 573, 15072, 4528, 7718, 7728, 5447, 15005, 7038, 11197, 11198, 11202, 5451, 15081, 11783, 8880, 11488, 504, 15082, 15087, 2]

// Module 15070 (QuestHomeBounties)
...
import BountyCard from "BountyCard" /* 15071 */;
import BountiesCtaHeaderDefault from "BountiesCtaHeader" /* 15082 */;
import QuestHomeOrbShopCarouselDefault from "QuestHomeOrbShopCarousel" /* 15087 */;
...
function QuestHomeBountiesCarousel(bounties) {
  ...
  const callback5 = first1.useCallback((nativeEvent) => {
    ...
    if (rounded !== first1) {
      ...
      if (rounded > tmp3) {
        let LEFT = tmp6(7728).HorizontalScrollingDirection.RIGHT;
      } else {
        LEFT = tmp6(7728).HorizontalScrollingDirection.LEFT;
      }
      result.scrollingDirection = LEFT;
      result.carouselPosition = rounded;
      QUEST_HOME_MOBILE_CAROUSEL = tmp6(5447).QuestContent.QUEST_HOME_MOBILE_CAROUSEL;
  ...
  const callback7 = first1.useCallback((bountyId, questContentPosition) => {
    if (obj.shouldMigrateToAdAnalyticsInterface(AdAnalyticsInterfaceExperiment.AdAnalyticsInterfaceExperimentStep.STEP_2_CLICKED_INTERNAL, "quest_home_bounties")) {
      const obj2 = { type: tmp(11202).AdUserActionType.CLICK_INTERNAL, adCreativeType: tmp(5451).AdCreativeType.BOUNTY, adCreativeId: bountyId.id, questContentCTA: tmp(7728).QuestContentCTA.START_BOUNTY, surfaceId: tmp(5447).QuestContent.QUEST_HOME_MOBILE_CAROUSEL, sourceQuestContent: tmp(5447).QuestContent.QUEST_HOME_MOBILE_CAROUSEL, questContentPosition };
      tmp(11198).captureAdUserAction(obj2);
      const tmpResult = tmp(11198);
    } else {
      const obj3 = { adContentId: bountyId.id, adCreativeType: tmp(5451).AdCreativeType.BOUNTY, questContent: tmp(5447).QuestContent.QUEST_HOME_MOBILE_CAROUSEL, questContentCTA: tmp(7728).QuestContentCTA.START_BOUNTY, sourceQuestContent: tmp(5447).QuestContent.QUEST_HOME_MOBILE_CAROUSEL, questContentPosition };
      const result = tmp(7718).trackAdContentClicked(obj3);
...
export default noop.memo(function QuestHomeBounties(shopCarouselConfig) {
  ...
  const questHomeBounties = first(11488).useQuestHomeBounties().questHomeBounties;
  ...
  const obj = first(11488);
  const tmp4 = _slicedToArray(first(7388).useSelectedDismissibleContent(items1), 2);
  first = tmp4[0];
  importDefault = tmp6;
