...
// Function ID: 11773
// Name: QuestUtils
// Dependencies: [5, 19, 11773, 5444, 1074, 21, 4527, 11774, 1896, 11777, 11295, 11784, 11791, 7709, 11281, 5447, 4259, 1114, 5597, 5621, 11489, 7722, 9514, 4418, 7382, 1115, 4255, 576, 11792, 9229, 4906, 4910, 1484, 2]
...
function openRewardClaimBottomSheet(arg0) {
  ...
  return ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(11777, dependencyMap.paths), timestampProducer, { questId, questContent, questContentPosition, sourceQuestContent });
}
function viewReward(quest) {
  ...
  if (obj.hasQuestRewardCode(quest.config)) {
    ...
    ActionSheetActionCreatorsDefault.openLazy(tmp(1896)(11777, dependencyMap.paths), timestampProducer, obj2);
  } else {
    if (tmpResult.hasVirtualCurrencyReward(quest.config)) {
      const obj3 = { quest };
      const result = tmp(11784).openQuestOrbsRewardModal(obj3);
      const tmpResult3 = tmp(11784);
    } else {
      const obj4 = { quest, product, onSuccess };
      const result1 = tmp(11791).openQuestCollectibleRewardModal(obj4);
...
export const openRewardDetailsBottomSheet = function openRewardDetailsBottomSheet(questId) {
  return ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(11774, dependencyMap.paths), React5, { questId: questId.questId });
};
export { viewReward };
  ...
  if (obj.canLaunchActivity(quest)) {
    ...
    if (features.includes(constants.MOBILE_ACTIVITY_QUEST)) {
      ...
      if (features2.includes(tmp3.CLOUD_GAMING_ACTIVITY)) {
        ...
        let tmp5Result = tmp5(tmp(11792).CloudIcon, obj3);
      } else {
        const obj4 = { size: "sm", style: obj2, color: nativeDefault.colors.WHITE };
        tmp5Result = tmp5(tmp(9229).GameControllerIcon, obj4);
      }
      return tmp5Result;
    }
    tmp3 = constants;
