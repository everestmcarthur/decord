...
export default function BountiesAutoScrollOverlayNuxGradient(active) {
  ...
  const sharedValue = active(4296).useSharedValue(0);
  ...
  const animatedStyle = active(4296).useAnimatedStyle(fn);
  ...
  const obj2 = active(4296);
  const items2 = ["" + BRAND_500 + "80", "" + BRAND_500 + "00"];
  obj4.colors = items2;
  obj4.locations = [0, 1];
  obj4.start = { x: 0.5, y: 1 };
  obj4.end = { x: 0.5, y: 0 };
  const items3 = [closure_5(sharedValue(4987), obj4), closure_5(sharedValue(4987), { style: StyleSheet.absoluteFillObject, colors: ["rgba(0,0,0,0.4)", "rgba(0,0,0,0)"], locations: [0, 0.5], start: { x: 0.5, y: 1 }, end: { x: 0.5, y: 0 } })];
  obj3.children = items3;
  return closure_6(sharedValue(4296).View, obj3);
};
