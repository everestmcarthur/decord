...
// Dependencies: [5, 19, 17, 9516, 7703, 5444, 9511, 21, 4560, 576, 504, 11783, 5447, 8098, 8679, 1114, 5126, 7168, 7724, 9658, 11488, 11780, 15125, 9513, 4527, 11772, 8362, 11300, 7150, 5587, 4987, 1093, 11775, 9060, 15154, 4973, 4556, 4975, 15121, 2]
// Exports: default

// Module 17037 (QuestProgressBottomSheet)
...
const FramesActionCreatorsDefault = tmp(9513);
require = fn;
...
function QuestProgressBottomSheet(quest) {
  quest = quest.quest;
  claim = undefined;
  const isScreenLandscape = quest(5126).useIsScreenLandscape();
  const tmp4 = closure_13(isScreenLandscape);
  let obj = quest(5126);
  let obj2 = quest(7168);
  const getOrFetchApplication = obj2.useGetOrFetchApplication(quest(7724).getActivityApplicationId(quest));
  let id;
  let obj3 = quest(7724);
  if (getOrFetchApplication != null) {
    id = getOrFetchApplication.id;
  }
  const url = claim(9658)({ applicationId: id, size: 600, names: ["embedded_cover"] }).url;
  const tmp7 = claim(9658);
  const questTaskDetails = quest(11488).useQuestTaskDetails(quest);
  const intl = tmp(1114).intl;
  let obj4 = { questName: quest.config.messages.questName };
  let tmpResult = quest(11488);
  const formatToPlainStringResult = intl.formatToPlainString(quest(1114).t.EAYZAr, { questName: quest.config.messages.questName });
  const tmpResult4 = quest(11780);
  const userStatus = quest.userStatus;
  let completedAt;
  const questsInstructionsToWinReward = tmpResult4.useQuestsInstructionsToWinReward({ quest, taskDetails: questTaskDetails, location: QuestsExperimentLocations.QUEST_ACTIVITY_BOTTOM_SHEET, sourceQuestContent: quest(5447).QuestContent.RUNNING_ACTIVITY });
  if (userStatus != null) {
  ...
  let obj5 = { quest, taskDetails: questTaskDetails, location: QuestsExperimentLocations.QUEST_ACTIVITY_BOTTOM_SHEET, sourceQuestContent: quest(5447).QuestContent.RUNNING_ACTIVITY };
  const tmp11 = QuestsExperimentLocations;
  const isQuestAccessSuspended = quest(11488).useIsQuestAccessSuspended();
  const tmpResult5 = quest(11488);
  const tmpResult6 = quest(15125);
  const questRewardClaimHandler = tmpResult6.useQuestRewardClaimHandler({ quest, questContent: quest(5447).QuestContent.RUNNING_ACTIVITY, sourceQuestContent: quest(5447).QuestContent.RUNNING_ACTIVITY });
  ...
  if (tmp24) {
    ...
    tmp24 = closure_11(tmp6(5587), obj8);
  }
  const items3 = [tmp24, , , ];
  const obj10 = { style: tmp4.heroGradient, start: null, end: null, colors: null };
  const obj6 = { quest, questContent: quest(5447).QuestContent.RUNNING_ACTIVITY, sourceQuestContent: quest(5447).QuestContent.RUNNING_ACTIVITY };
  ...
  size.width = num;
  obj11.children = closure_11(claim(11775), size);
  items3[2] = closure_11(closure_5, obj11);
  const obj12 = { style: tmp4.contextMenuContainer, children: null };
  const obj13 = { icon: claim(9060), onPress: callback2, variant: "secondary-overlay", size: "sm", accessibilityLabel: null };
  const intl2 = tmp(1114).intl;
  obj13.accessibilityLabel = intl2.string(quest(1114).t.RDE0Sc);
  const items5 = [closure_11(quest(8098).IconButton, obj13), ];
  const obj14 = { quest, showShareLink: true, location: tmp11.QUEST_ACTIVITY_BOTTOM_SHEET, sourceQuestContent: null, children: null };
  const tmp6Result4 = claim(11775);
  obj14.sourceQuestContent = quest(5447).QuestContent.RUNNING_ACTIVITY;
  obj14.children = contextMenuButton;
  items5[1] = closure_11(claim(15154), obj14);
  ...
  const items7 = [closure_11(quest(4556).Text, { variant: str, color: "mobile-text-heading-primary", children: formatToPlainStringResult }), ];
  ...
  items7[1] = closure_11(quest(4556).Text, obj17);
  obj16.children = items7;
  const items8 = [closure_12(quest(4973).Stack, obj16), ];
  ...
  items9[1] = closure_11(quest(4975).Button, obj21);
  obj18.children = items9;
  items8[1] = closure_12(quest(4973).Stack, obj18);
  obj15.children = items8;
  items6[1] = closure_12(closure_5, obj15);
  obj20.children = items6;
  return closure_12(quest(7150).BottomSheet, obj20);
}
...
export default function QuestProgressBottomSheetConnected(questId) {
  ...
  if (null != stateFromStores) {
    ...
    tmp4 = closure_11(tmp(11783).QuestContentImpressionTrackerNative, obj2);
  }
  return tmp4;
};
