...
// Dependencies: [32, 19, 17, 4552, 4224, 13417, 1074, 1954, 1373, 21, 672, 576, 4560, 1483, 13418, 5123, 5628, 4556, 1114, 1242, 7380, 13419, 8057, 13420, 13356, 10747, 13441, 13442, 13402, 13443, 13447, 13463, 13367, 13414, 13415, 7385, 1611, 13354, 504, 7393, 8066, 13464, 9798, 4296, 4974, 4411, 13465, 4380, 1943, 8056, 1945, 5587, 13466, 13467, 12620, 9799, 4987, 1115, 4964, 2]
// Exports: default

// Module 13416 (PremiumNitroHome)
...
import PremiumPerkCardDefault from "PremiumPerkCard" /* 13420 */;
...
function NitroHomeHeader(arg0) {
  ...
  const mobileNitroManageSubscriptionsSettingsExperiment = onClose(13418).useMobileNitroManageSubscriptionsSettingsExperiment({ location: "NitroHomeHeader" });
  let hasActiveTrial;
  ...
  const obj2 = onClose(13418);
  const tmp8 = !hasActiveTrial;
  const items1 = [closure_17(onClose(5123).PressableOpacity, { style: tmp.backButtonWrapper, onPress: callback, children: closure_17(onClose(5628).ArrowLargeLeftIcon, { size: "md", color: "white" }) }), , ];
  ...
  items1[1] = closure_17(onClose(4556).Text, obj6);
  if (mobileNitroManageSubscriptionsSettingsExperiment) {
    ...
    const obj8 = {
      ...
      children: tmp11(tmp2(7380).SettingsIcon, { size: "md", color: "white" })
    };
    tmp11Result = tmp11(tmp2(5123).PressableOpacity, obj8);
  }
  ...
  if (tmp11Result2) {
    const obj9 = { style: tmp.pillParent, children: tmp11(tmp2(13419).PremiumReferralTrialPill, { hasExtraMargin: true }) };
...
function NewTab(arg0) {
  ...
  if (tmp12) {
    ...
    const obj7 = { width: tmp2(13420).PERK_CARD_SIZES[tmp2(undefined, 13420).PerkCardVariant.WIDE].width };
    obj6.style = obj7;
    ...
    tmp12 = closure_17(tmp8(13402), obj6);
    const tmp8Result = tmp8(13402);
  }
  const items1 = [tmp12, closure_17(require("TieredTenureBadgePerkCard").TieredTenureBadgePerkCard, {}), , , , , ];
  let tmp15Result = null;
  if (isEligibleSenderForReferralProgram) {
    const obj8 = { onLayout: useScrollToSectionDefault(ref, scrollToSectionId).createSectionLayoutHandler(NitroHomeSectionId.REFERRAL_PROGRAM), children: tmp15(tmp2(13447).ReferralProgramPerkCard, {}) };
...
function PerksTab(extraBottomHeight) {
  ...
  if (fractionalState === FractionalPremiumStates.FP_ONLY) {
    ...
    items4[2] = tmp8(tmp9(13463), obj5);
    const obj6 = { title: null, perks: null, onItemChange: null };
    const tmp9Result3 = tmp9(13463);
...
export default function PremiumNitroHome(onClose) {
  ...
  const commonTriggerPoint = bottom(13414).useCommonTriggerPoint(bottom(13415).OpenNitroTriggerPoint);
  let obj = bottom(13414);
  const giftCardMobileConsumptionHalfsheet = bottom(7385).useGiftCardMobileConsumptionHalfsheet();
  const tmp5 = closure_24();
  bottom = navigation(1611)().bottom;
  let obj2 = bottom(7385);
  const youBarSettingsCustomHeaderPaddingTop = bottom(13354).useYouBarSettingsCustomHeaderPaddingTop();
  let obj3 = bottom(13354);
  ...
  const isInReverseTrial = bottom(8066).useIsInReverseTrial();
  const obj8 = bottom(8066);
  const maybeFetchTieredTenureBadgeData = bottom(13464).useMaybeFetchTieredTenureBadgeData();
  const field = PremiumNitroNavigationStore.useField("scrollToSectionId");
  const effect = stateFromStores.useEffect(() => () => {
    closure_1_9.resetState();
  }, []);
  const obj9 = bottom(13464);
  ...
  const segmentedControlState = bottom(9798).useSegmentedControlState(obj11);
  const obj10 = bottom(9798);
  ...
  sharedValue = bottom(4296).useSharedValue(num);
  const obj16 = bottom(4296);
  class Q {
    constructor() {
  ...
  const animatedStyle = bottom(4296).useAnimatedStyle(Q);
  ...
  let tmpResult = bottom(4296);
  const theme = bottom(4411).useThemeContext().theme;
  let tmpResult6 = bottom(4411);
  const isThemeDarkResult = bottom(4411).isThemeDark(theme);
  const tmpResult7 = bottom(4411);
  const tmp6Result = navigation(13465);
  const tmp6ResultResult = navigation(13465)(tmp21.endsAt, bottom(13465).CountDownMessageTypes.ENDS_IN);
  isEligibleSenderForReferralProgram = bottom(8057).useIsEligibleSenderForReferralProgram();
  ...
  const effect2 = obj5.useEffect(() => {
    if (!obj.UNSAFE_isDismissibleContentDismissed(dismissible_content.DismissibleContent.MOBILE_NITRO_HOME_SETTINGS_BADGE)) {
      const result = tmp(4380).UNSAFE_markDismissibleContentAsDismissed(tmp(1943).DismissibleContent.MOBILE_NITRO_HOME_SETTINGS_BADGE);
    ...
    if (!tmpResult4.UNSAFE_isDismissibleContentDismissed(dismissible_content.DismissibleContent.REFERRAL_TRIAL_MOBILE_NITRO_HOME_BADGE)) {
      const result1 = tmp(4380).UNSAFE_markDismissibleContentAsDismissed(tmp(1943).DismissibleContent.REFERRAL_TRIAL_MOBILE_NITRO_HOME_BADGE);
      const tmpResult5 = tmp(4380);
    }
    if (isEligibleSenderForReferralProgram) {
      const result2 = tmp(8056).markReferralProgramEntrypointBadgeAcknowledged();
      const tmpResult6 = tmp(8056);
    }
  }, items5);
  ...
  const tmpResult9 = bottom(13356);
  obj18.source = navigation(13466);
  const items8 = [closure_17(navigation(5587), obj18), closure_17(NitroHomeHeader, { onClose: onClose.onClose, subscription: stateFromStores1 }), , , ];
  ...
  if (tmp26Result) {
    ...
    tmp26Result = tmp26(tmp6(13467), obj19);
  }
  items8[2] = tmp26Result;
  const obj20 = { style: tmp5.tabContent, children: null };
  const items9 = [closure_17(bottom(12620).SegmentedControlPages, { state: segmentedControlState }), ];
  if (tmp26Result4) {
  ...
  const tmp6Result3 = navigation(5587);
  const items11 = [closure_17(c5, { style: tmp5.segmentedControlActual, onLayout: callback, children: closure_17(bottom(9799).SegmentedControl, { state: segmentedControlState, variant: "experimental_Small" }) }), , ];
  ...
  if (!isThemeDarkResult) {
    ...
    items12[1] = obj25;
    obj24.style = items12;
    tmp26Result5 = tmp26(tmp6(4987), obj24);
  }
  items11[1] = tmp26Result5;
  const obj23 = { style: tmp5.segmentedControlActual, onLayout: callback, children: closure_17(bottom(9799).SegmentedControl, { state: segmentedControlState, variant: "experimental_Small" }) };
  if (tmpResult10.isAndroid()) {
    ...
    tmp26Result6 = tmp26(tmp6(4964), obj28);
    const tmp6Result4 = tmp6(4964);
  }
  items11[2] = tmp26Result6;
  obj22.children = items11;
  items8[4] = closure_18(navigation(4296).View, obj22);
