...
// Dependencies: [32, 19, 17, 4552, 4223, 4224, 1074, 1373, 1085, 21, 4560, 576, 5524, 9370, 4218, 7234, 4556, 1114, 8050, 3063, 1379, 38, 7447, 8061, 8059, 7393, 7446, 7162, 504, 9376, 1608, 7409, 9385, 7438, 8839, 9387, 9388, 9390, 9397, 4975, 8668, 7422, 2]
// Exports: default

// Module 9369 (PremiumFeaturesCard)
...
import usePremiumPlanPriceDefault from "usePremiumPlanPrice" /* 9370 */;
import usePremiumFeaturesDefault from "usePremiumFeatures" /* 9376 */;
import PremiumFeaturesWumpusDefault from "PremiumFeaturesWumpus" /* 9390 */;
import PremiumFeatureListDefault from "PremiumFeatureList" /* 9397 */;
...
function PriceText(fractionalPremiumInfo) {
  ...
  if (null != discountedPriceString) {
    if (null != discountOffer) {
      ...
      return closure_1_19(tmp7(4556).Text, obj3);
    }
  }
    ...
    if (null != premiumSubscription) {
      if (premiumSubscription.planIdFromItems === PREMIUM_YEAR_TIER_2.PREMIUM_YEAR_TIER_2) {
        ...
        if (null != premiumSubscription) {
          ...
          if (tmp21) {
            tmp21 = tmp7(4218).getPremiumType(planIdFromItems) === premiumType;
            const tmp7Result = tmp7(4218);
          }
          flag = tmp21;
        }
        if (flag) {
          ...
          if (!hasActiveTrial) {
            ...
            const obj5 = { percent: percentage, regularPrice: formatRateResult, renewalDate: tmp7(4218).getExpectedRenewalDate(premiumSubscription, fractionalPremiumInfo.fractionalPremiumInfo) };
            let formatResult = intl2.format(tmp7(1114).t.z2oQtA, obj5);
            const tmp7Result5 = tmp7(4218);
          }
          const obj6 = { variant: "text-md/normal", color: "text-overlay-light", style: tmp.discountPriceText, children: formatResult };
          return closure_1_19(tmp7(4556).Text, obj6);
        }
      }
      if (premiumSubscription.hasAnyPremiumGroup) {
        ...
        if (null != prop) {
          let priceString1 = tmp7(8050).getPriceString(premiumSubscription);
          ...
          formatResult = intl6.format(_modDef3063.FwjZzr, obj7);
          const tmp7Result6 = tmp7(8050);
        }
      }
      if (activeDiscountInfo.discountId === closure_1_11) {
        ...
        if (source === tmp7(9370).PremiumPlanPriceSource.API) {
          ...
          const obj8 = { numMonths: duration2, discountedPrice: tmp7(7234).formatPrice(rounded, tmp4.currency), billingPeriod: null, fullPrice: null };
  ...
  if (null != subscriptionTrial) {
    if (premiumType === map1[subscriptionTrial.skuId]) {
      ...
      obj12.trialPeriod = tmp7(4218).formatIntervalDuration(obj11);
      let priceString2;
      if (tmp4 != null) {
      ...
      let tmp11Result = closure_1_19(tmp7(4556).Text, obj10);
      const tmp7Result8 = tmp7(4218);
    }
    return tmp11Result;
  }
...
export default function PremiumFeaturesCard(premiumType) {
  ...
  const premiumTrialOffer = premiumType(7447).usePremiumTrialOffer();
  const obj = premiumType(7447);
  const premiumDiscountOffer = premiumType(8061).usePremiumDiscountOffer();
  const obj2 = premiumType(8061);
  const activeDiscountInfo = premiumType(8059).useActiveDiscountInfo();
  const obj3 = premiumType(8059);
  const tmp12 = useFractionalPremiumInfoDefault();
  let subscriptionTrial;
  const premiumTrialOfferPremiumType = premiumType(7446).usePremiumTrialOfferPremiumType();
  if (premiumTrialOffer != null) {
    subscriptionTrial = premiumTrialOffer.subscriptionTrial;
  }
  const obj4 = premiumType(7446);
  let interval;
  if (subscriptionTrial != null) {
    interval = subscriptionTrial.interval;
  }
  ...
  const result = premiumType(4218).formatIntervalDuration(obj5);
  ...
  const tmp8Result = premiumType(4218);
  ...
  const tmp8Result12 = premiumType(7409);
  const items3 = [premiumBundleWithPredicate];
  const discountedPriceString = premiumType(9385).useDiscountedPremiumProductInfo(premiumDiscountOffer, items3).discountedPriceString;
  let tmp32 = tmp24;
  ...
  const items4 = [closure_19(premiumType(7438).PremiumPill, { style: tmp3.pill, discountOffer: premiumDiscountOffer, isActiveDiscount: null != activeDiscountInfo, shouldShowDiscountUpsell: null != premiumDiscountOffer && null != discountedPriceString, premiumType, trialOffer: premiumTrialOffer }), ];
  ...
  const tmp8Result13 = premiumType(9385);
  if (flag3) {
    let tmp37Result = tmp37(tmp4(9387), { width: 185, height: 20, alwaysWhite: true });
  } else {
    const obj11 = { premiumType, style: tmp3.logo };
    tmp37Result = tmp37(tmp4(9388), obj11);
  }
  ...
  if (hideButton) {
    ...
    if (tmp37Result6) {
      ...
      obj14.children = tmp37(tmp8(4556).Text, obj15);
    ...
    if (tmp32) {
      ...
      obj17.children = tmp37(tmp8(4556).Text, obj18);
      let obj19 = obj17;
    } else {
      ...
      if (tmp37Result6) {
        ...
        if (null != premiumDiscountOffer) {
          ...
          const tmp37Result7 = tmp37(tmp8(8668).NitroWheelIcon, obj21);
        }
