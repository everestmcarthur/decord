...
// Dependencies: [19, 17, 21, 4560, 9244, 8340, 1114, 4556, 4520, 576, 9246, 14941, 2]
// Exports: default
  ...
  return closure_5(c3, {
    ...
    children: (() => {
      ...
      children[2] = items.map((children, index) => {
        ...
        items = [, ];
        items[0] = closure_2_5(items(4520).CircleCheckIcon, { style: items.permissionIcon, size: "xs", color: c1(576).colors.STATUS_POSITIVE });
        items[1] = closure_2_5(items(4556).Text, { style: items.permissionText, variant: "text-sm/normal", color: "text-default", children });
        obj.children = items;
      ...
      if (_undefined3 != null) {
        mapped = _undefined3.map((disclosure, index) => {
          ...
          if (null != textForDisclosure) {
            ...
            items = [hasOwnProperty(tmp(14941).DisclosureIcon, obj3), ];
            const obj4 = { style: items.permissionText, variant: "text-sm/normal", children: textForDisclosure };
            items[1] = hasOwnProperty(tmp(4556).Text, obj4);
            obj2.children = items;
