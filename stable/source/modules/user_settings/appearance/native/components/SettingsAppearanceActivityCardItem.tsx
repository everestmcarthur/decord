...
// Dependencies: [19, 17, 2025, 15297, 21, 4296, 1178, 4560, 576, 563, 8819, 5587, 4556, 1880, 15298, 15299, 15300, 2]
// Exports: default

// Module 15296 (SettingsAppearanceActivityCardItem)
  ...
  if (null != avatars) {
    mapped = substr.map((source, index) => {
      ...
      if (index !== diff) {
        ...
        const point = { shape: tmp3(8819).CutoutShape.Circle, x: tmp3(1178).AVATAR_SIZE_MAP[tmp3(undefined, 1178).AvatarSizes.XSMALL_20] - 4 - 2, y: -2, size: tmp3(1178).AVATAR_SIZE_MAP[tmp3(undefined, 1178).AvatarSizes.XSMALL_20] + 4 };
        const items = [point];
        obj3.cutouts = items;
        obj3.children = tmp5;
