...
// Dependencies: [32, 19, 4476, 4477, 7894, 4481, 7892, 504, 2]
  ...
  function _loop(type) {
    ...
    obj.variants = buckets.map((item, index) => {
      ...
      if (typeof type.description === "object") {
        ...
        experimentBucketName = closure_1(7892).getExperimentBucketName(item);
        const obj3 = closure_1(7892);
      }
      obj.label = experimentBucketName;
      obj.shortLabel = closure_1(7892).getExperimentBucketName(item);
      if (item === constants.CONTROL) {
        let TREATMENT = obj(7894).Variation_Type.CONTROL;
      } else if (item === tmp4.NOT_ELIGIBLE) {
        TREATMENT = obj(7894).Variation_Type.UNSPECIFIED;
      } else {
        TREATMENT = obj(7894).Variation_Type.TREATMENT;
      }
      obj.type = TREATMENT;
      return obj;
    });
