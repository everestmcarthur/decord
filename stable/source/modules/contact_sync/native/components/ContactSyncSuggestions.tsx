...
// Dependencies: [32, 19, 17, 1074, 12703, 21, 4560, 576, 5682, 4982, 4556, 4404, 1396, 5604, 1178, 1114, 4262, 4409, 11, 8599, 8880, 4987, 1093, 4975, 1242, 2]
// Exports: default

// Module 12702 (ContactSyncSuggestions)
...
function SuggestionRow(suggestion) {
  ...
  if (tmp8Result) {
    ...
    tmp8Result = tmp8(tmp3(4556).Text, obj5);
  }
  items[1] = tmp8Result;
...
let obj2 = { container: { flex: 1, backgroundColor: nativeDefault.colors.BACKGROUND_BASE_LOWER, paddingTop: fn(5682).NAV_BAR_HEIGHT + 32, justifyContent: "center" }, header: null, title: null, subtitle: null, list: null, divider: null, linearGradient: null, redesignButton: null, sectionHeader: null };
let obj3 = { flex: 1, backgroundColor: nativeDefault.colors.BACKGROUND_BASE_LOWER, paddingTop: fn(5682).NAV_BAR_HEIGHT + 32, justifyContent: "center" };
...
export default function ContactSyncSuggestions(friendSuggestions) {
  ...
  const token = friendSuggestions(4262).useToken(onSubmit(576).colors.BACKGROUND_BASE_LOW);
  let obj = friendSuggestions(4262);
  let items = [friendSuggestions(4409).hexOpacityToRgba(token, 0), ];
  let obj2 = friendSuggestions(4409);
  items[1] = friendSuggestions(4409).hexOpacityToRgba(token, 100);
  const tmp4 = first(noop.useState(reduced), 2);
  first = tmp4[0];
  noop = tmp4[1];
  let obj3 = friendSuggestions(4409);
  const fontScale = friendSuggestions(4982).useFontScale();
  ...
  let obj4 = friendSuggestions(4982);
  ...
  const items5 = [closure_8(friendSuggestions(8880).FlashList, obj7), , ];
  ...
  obj9.colors = items;
  items5[1] = closure_8(onSubmit(4987), obj9);
  ...
  obj10.children = closure_8(friendSuggestions(4975).Button, obj11);
