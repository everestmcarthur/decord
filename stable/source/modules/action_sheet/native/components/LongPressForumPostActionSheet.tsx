...
// Dependencies: [19, 4200, 4201, 7306, 502, 1979, 4575, 1074, 1964, 21, 10227, 10248, 1114, 10249, 10251, 6970, 7110, 4501, 7765, 9782, 4523, 4513, 10253, 5095, 10255, 10256, 11, 7182, 7380, 8631, 11255, 4527, 11253, 1896, 4503, 11257, 9065, 10149, 10966, 11317, 10958, 4904, 4518, 7456, 1178, 10634, 7190, 4258, 504, 7304, 7269, 7886, 1935, 4713, 10979, 5584, 7198, 1608, 11003, 7200, 2]
// Exports: default

// Module 10226 (LongPressForumPostActionSheet)
...
import buildFavoritesSectionButtonsDefault from "buildFavoritesSectionButtons" /* 10227 */;
...
import useFavoritesGuildChannelActionsDefault from "useFavoritesGuildChannelActions" /* 10979 */;
...
export default function ForumPostLongPressActionSheet(thread) {
  ...
  const canMarkChannelUnread = thread(10251).useCanMarkChannelUnread(thread);
  const obj5 = thread(10251);
  ...
  const firstMessage = thread(7304).useFirstForumPostMessage(thread).firstMessage;
  const obj7 = thread(7304);
  const isThreadModerator = thread(7269).useIsThreadModerator(parentChannel);
  const obj8 = thread(7269);
  const canManageThread = thread(7269).useCanManageThread(thread);
  const obj9 = thread(7269);
  const canUnarchiveThread = thread(7269).useCanUnarchiveThread(thread);
  const obj10 = thread(7269);
  const existingPin = thread(7886).useExistingPin(thread);
  const obj11 = thread(7886);
  ...
  if (null != stateFromStores) {
    const obj14 = { guild: stateFromStores, size: tmp(5584).GuildIconSizes.LARGE };
  ...
  const MarkChannelUnreadExperiment = tmp(10248).MarkChannelUnreadExperiment;
  if (MarkChannelUnreadExperiment.getConfig({ location: "forum_post_action_sheet" }).enabled) {
    if (!stateFromStores3) {
      if (canMarkChannelUnread) {
        ...
        obj18.IconComponent = tmp(10249).ChatMarkUnreadIcon;
      ...
      if (!stateFromStores4) {
        ...
        if (stateFromStores1) {
          ...
          obj20.IconComponent = tmp(4501).UserMinusIcon;
          ...
          obj20.IconComponent = tmp(9782).BellIcon;
          obj20.onPress = function onPress() {
      ...
      if (archived) {
        if (canUnarchiveThread) {
          ...
          obj21.IconComponent = tmp(4523).ClockIcon;
        ...
        obj22.IconComponent = tmp(4513).XLargeIcon;
      ...
      if (canManageThread) {
        ...
        if (locked) {
          ...
          obj23.IconComponent = tmp(10253).LockUnlockedIcon;
          ...
          obj23.IconComponent = tmp(5095).LockIcon;
          obj23.onPress = function onPress() {
      ...
      if (tmp38) {
        ...
        obj24.IconComponent = tmp(10255).PencilIcon;
      ...
      if (canManageThread) {
        ...
        obj25.IconComponent = tmp(7380).SettingsIcon;
        ...
        if (parentChannel.availableTags.length > 0) {
          ...
          obj26.IconComponent = tmp(11255).TagsIcon;
          obj26.onPress = function onPress() {
            ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(11253, dependencyMap.paths), "ForumPostTagsActionSheet", { thread, parentChannel, canManageThread });
          };
          buttons7.push(obj26);
        }
      }
      ...
      obj27.IconComponent = tmp(4503).LinkIcon;
      ...
      if (stateFromStores4) {
        ...
        if (isThreadModerator) {
          ...
          if (hasFlagResult) {
            ...
            obj29.IconComponent = tmp(10958).PinIcon;
        ...
        if (!isThreadModerator) {
          if (!tmp24) {
            ...
            if (setting) {
              ...
              obj31.IconComponent = tmp(10634).IdIcon;
            ...
            obj32.header = tmp20(tmp(11003).ActionSheetIconHeader, obj33);
            ...
            return tmp20(tmp(7198).ActionSheet, obj32);
          }
        }
        if (tmp24) {
        ...
        if (stateFromStores2) {
          ...
          obj36.IconComponent = tmp(9782).BellIcon;
          ...
          obj36.IconComponent = tmp(9065).BellSlashIcon;
          obj36.onPress = function onPress() {
            ...
            obj.openLazy(asyncRequireImpl(10149, dependencyMap.paths), combined, obj2);
          };
          push3(obj36);
        }
        ...
        obj37.IconComponent = tmp(10966).ChannelNotificationIcon;
  ...
  obj38.IconComponent = tmp(6970).EyeIcon;
