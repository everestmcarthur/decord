...
function RemoteAuthLogin(arg0) {
  ...
  const items = [closure_9(closure_6, { source: _modDef13868, style: tmp.mainImage }), , , ];
...
function RemoteAuthLoginSucceeded() {
  ...
  const items = [React7(timestampProducer, { source: _modDef13869, style: tmp.mainImage }), , , ];
...
function RemoteAuthNotFound() {
  const tmp = closure_12();
  const obj = { children: null };
  const items = [React7(timestampProducer, { source: _modDef13870, style: tmp.mainImage }), , , ];
  const obj3 = { variant: "heading-xl/extrabold", children: null };
  const intl = util.intl;
...
export default function RemoteAuth(arg0) {
  const tmp = closure_12();
  const obj = { source: _modDef13867, imageStyle: null, style: null, children: null };
  ({ imageStyle: obj.imageStyle, background: obj.style } = tmp);
  const obj2 = { style: null, source: _modDef13865 };
