...
import NativeAuthenticationModuleDefault from "NativeAuthenticationModule" /* 15996 */;
...
export const CompanionRemoteAuth = function CompanionRemoteAuth() {
  ...
  let obj2 = noop;
  const state = navigation(15992).useAuthWebsocket(callback, true).state;
  ...
  if (navigation(15991).RemoteAuthStep.INITIALIZING !== step) {
    if (tmp2(15991).RemoteAuthStep.PENDING_REMOTE_INIT !== step) {
      if (tmp2(15991).RemoteAuthStep.PENDING_TICKET === step) {
        ...
        const obj10 = { username: tmp5(4404).getUserTag(user) };
        obj9.children = intl3.format(tmp2(1114).t.Cbl5JK, obj10);
        items2[2] = tmp11(tmp2(4556).Text, obj9);
        ...
        obj11.children = tmp11(tmp2(4975).Button, obj12);
        items2[3] = tmp11(tmp13, obj11);
        obj6.children = items2;
        let tmp11Result = closure_9(closure_8, obj6);
        const tmp5Result2 = tmp5(4404);
  ...
  items3[1] = closure_7(navigation(4556).Text, obj14);
  ...
  obj15.children = closure_7(navigation(4975).Button, obj16);
