...
// Dependencies: [7595, 502, 7280, 1957, 4781, 2011, 4381, 7609, 1074, 1084, 7600, 7292, 7295, 7297, 573, 7610, 7602, 7607, 7611, 7612, 11, 4783, 4669, 7118, 2]

// Module 7608 (ExplicitMediaManager)
import SnowflakeUtilsDefault from "SnowflakeUtils" /* 11 */;
import flattenDefault from "flatten" /* 4669 */;
...
import uniqWithDefault from "uniqWith" /* 7612 */;
    ...
    if (null != message1.channel_id) {
      ...
      if (null != dependencyMap[combined]) {
        ...
        if (UPDATE === ExplicitMediaRedactionUtils.TimeoutCancelSource.UPDATE) {
          ...
          const result = tmp5(7600).trackExplicitMediaScanComplete(obj);
          const tmp5Result = tmp5(7600);
        }
...
function handleUnscannedMessages(found2, isMessageUpdate) {
  ...
  const item = found1.forEach((channel_id) => {
    ...
    if (null == closure_14[combined]) {
      let obj2 = { name: closure_0(7607).MetricEvents.EXPLICIT_MEDIA_SCAN_CLIENT_TIMEOUT_CREATE };
      closure_1(7602).increment(obj2);
      const obj3 = { setAt: null, timeout: null };
      const _Date = Date;
      obj3.setAt = Date.now();
      const _setTimeout = setTimeout;
      obj3.timeout = setTimeout(() => {
        if (maybeCancelTimeout(closure_0, found1(7600).TimeoutCancelSource.TIMEOUT)) {
          message = message.getMessage(tmp.channel_id, tmp.id);
          if (null != message) {
            const unscannedMediaIds = tmp2(7292).getUnscannedMediaIds(message);
            ({ attachmentIds, embedIds } = unscannedMediaIds);
            const tmp2Result = tmp2(7292);
            ...
            const result = tmp2(7600).trackScanningTimedOut(obj);
            const tmp2Result2 = tmp2(7600);
          }
...
function processMessagesFromAction(firstMessages, isMessageUpdate) {
  ...
  const arr4 = obj2(7612)(tmp2, (id, id2) => id.id === id2.id && id.channel_id === id2.channel_id);
  ...
  const item = arr4.forEach((channel_id) => {
    ...
    if (attachments1 != null) {
      num3 = attachments1.filter((media) => {
        obj = closure_1_0(7292);
        return obj.isMediaFlaggedForHarmType(closure_1_0(7295).ContentHarmType.EXPLICIT, { type: closure_1_0(7297).ObscuredMediaTypes.Attachment, media });
      }).length;
    }
    if (num3 == null) {
      num3 = 0;
    ...
    if (embeds1 != null) {
      num4 = embeds1.filter((media) => {
        obj = closure_1_0(7292);
        return obj.isMediaFlaggedForHarmType(closure_1_0(7295).ContentHarmType.EXPLICIT, { type: closure_1_0(7297).ObscuredMediaTypes.Embed, media });
      }).length;
    }
    if (num4 == null) {
      num4 = 0;
    ...
    if (attachments2 != null) {
      num5 = attachments2.filter((media) => {
        obj = closure_1_0(7292);
        return obj.isMediaFlaggedForHarmType(closure_1_0(7295).ContentHarmType.GORE, { type: closure_1_0(7297).ObscuredMediaTypes.Attachment, media });
      }).length;
    }
    if (num5 == null) {
      num5 = 0;
    ...
    if (embeds2 != null) {
      num6 = embeds2.filter((media) => {
        obj = closure_1_0(7292);
        return obj.isMediaFlaggedForHarmType(closure_1_0(7295).ContentHarmType.GORE, { type: closure_1_0(7297).ObscuredMediaTypes.Embed, media });
      }).length;
    }
    if (num6 == null) {
      num6 = 0;
    ...
    if (attachments3 != null) {
      num7 = attachments3.filter((media) => {
        obj = closure_1_0(7292);
        return obj.isMediaFlaggedForHarmType(closure_1_0(7295).ContentHarmType.SELF_HARM, { type: closure_1_0(7297).ObscuredMediaTypes.Attachment, media });
      }).length;
    }
    if (num7 == null) {
      num7 = 0;
    ...
    if (embeds3 != null) {
      num8 = embeds3.filter((media) => {
        obj = closure_1_0(7292);
        return obj.isMediaFlaggedForHarmType(closure_1_0(7295).ContentHarmType.SELF_HARM, { type: closure_1_0(7297).ObscuredMediaTypes.Embed, media });
      }).length;
    }
    if (num8 == null) {
      num8 = 0;
      ...
      if (obj6.getChannelIdAndAuthorIdFromMessage(message).authorId !== AuthenticationStore.getId()) {
        ...
        if (!tmp20Result6.hasUnscannedMedia(message)) {
          ...
          if (!hasUnscannedMediaResult) {
            const tmp20Result7 = tmp20(7292);
            hasUnscannedMediaResult = tmp20Result7.hasUnscannedMedia(tmp20(4783).updateMessageRecord(message1, message));
            const tmp20Result8 = tmp20(4783);
          }
          if (!hasUnscannedMediaResult) {
            maybeCancelTimeout(message1, tmp20(7600).TimeoutCancelSource.UPDATE);
          }
        }
...
function maybeScanMessagesForChannelId(channelId) {
  ...
  if (tmp) {
    ...
    const arr5 = obj2(7612)(tmp3, (id, id2) => id.id === id2.id && id.channel_id === id2.channel_id);
