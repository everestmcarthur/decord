  ...
  return shouldUseMobileWebRedirectCheckout.useCallback(asyncGeneratorStep(async (arg0, value) => {
    if (c3 === 2) {
      ...
      try {
            ...
            if (null != tmp2) {
              ...
              if (closure_2 > 0) {
                if (!handleMobileWebRedirectCheckout.hasFetched) {
                  const items = [tmp2(7419).init(), ];
                  let obj2 = tmp2(7419);
                  items[1] = tmp3(4458).fetchGuildBoostSlots();
                  dependencyMap = 1;
                  c3 = 1;
        ...
        closure_128_0 = PERK;
        availableGuildBoostSlots = tmp3(4454).getAvailableGuildBoostSlots(handleMobileWebRedirectCheckout.boostSlots);
        if (availableGuildBoostSlots.length >= closure_129_2) {
          ...
          const result = tmp3(7403).launchGuildBoostFlowOrAlert(obj10);
          const obj6 = tmp3(7403);
        }
        const obj5 = tmp3(4454);
      } catch (tmp50) {
        c3 = tmp;
        throw tmp50;
      }
