...
export default function TieredTenureBadgeCoachmark(arg0) {
  ...
  if (null != tieredTenureBadge) {
    tieredTenureBadgeData = tmp(7634).getTieredTenureBadgeData(tieredTenureBadge);
  ...
  const tmp5 = _slicedToArray(tieredTenureBadgeData(7388).useSelectedDismissibleContent(items1), 2);
  first = tmp5[0];
  dependencyMap = tmp7;
  ...
  const memo = noop.useMemo(() => {
    ...
    obj.onButtonPress = function onButtonPress() {
      ...
      tieredTenureBadgeData(7382).openUserSettings({ screen: constants.PREMIUM });
    };
    const intl3 = util.intl;
    obj.buttonLabel = intl3.string(util.t.RzWDqY);
    return obj;
  }, items2);
  const tmpResult3 = tieredTenureBadgeData(7388);
