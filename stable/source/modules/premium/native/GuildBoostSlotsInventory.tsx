...
// Module 13505 (GuildBoostSlotsInventory)
...
const SubscriptionPlaceholderPattern = tmp2(13507);
require = fn;
...
function GuildBoostSlot(guildBoostSlot) {
  ...
  if (null == guildBoostSlot.guild) {
    ...
    if (!guildBoostSlot.isOnCooldown()) {
      ...
      obj5.children = tmp6(guildBoostSlot(4556).Text, obj6);
      tmp6Result2 = tmp6(guildBoostSlot(5123).PressableOpacity, obj5);
    }
...
function UnusedGuildBoostSlots(unusedSlots) {
  ...
  if (found1.length > 0) {
    ...
    obj4.children = closure_11(found(4556).Text, obj5);
    obj2.action = closure_11(found(5123).PressableOpacity, obj4);
...
function BoostedGuildInfo(guild) {
  ...
  if (null != guild) {
    ...
    const obj7 = { source: _modDef13506, style: tmp.guildInfoRowIcon };
  ...
  if (0 !== found.length) {
    ...
    if (null != stateFromStores) {
      ...
      if (found.length > 0) {
        ...
        const items3 = [closure_11(tmp3(4556).Text, obj7), found.map((guildId) => closure_2_11(BoostedGuild, { guildId, guildBoostSlots: valueResult[guildId] }, guildId))];
        obj6.children = items3;
