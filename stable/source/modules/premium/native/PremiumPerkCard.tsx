...
export default function PremiumPerkCard(variant) {
  ...
  if (null != imageSrc) {
    if (null != imageOverlayText) {
      ...
      imageSrc = React7(tmp6(4556).Text, obj5);
    ...
    if (null != description) {
      ...
      let tmp21 = React7(tmp6(4556).Text, obj7);
    } else {
      tmp21 = null;
    ...
    items4[2] = React7(tmp6(4556).Text, obj11);
    ...
    if (tmp29Result) {
      ...
      obj13.children = tmp29(tmp6(4975).Button, obj14);
...
export const usePremiumPerkCard = function usePremiumPerkCard() {
  subscriptionPlansLoaded = subscriptionPlansLoaded(13421).useSubscriptionPlansLoaded();
  let obj = subscriptionPlansLoaded(13421);
  const maxFileSizeForPremiumType = subscriptionPlansLoaded(4218).getMaxFileSizeForPremiumType(PremiumTypes.TIER_2);
  const callback = noop.useCallback(() => {
    const obj2 = { screen: constants.COLLECTIBLES_SHOP, params: null };
    const obj = subscriptionPlansLoaded(7382);
    obj2.params = { analyticsSource: AnalyticsLocationDefault.PREMIUM_MARKETING_PERK_CARD };
    obj.openUserSettings(obj2);
  }, []);
  const callback1 = noop.useCallback(() => {
    subscriptionPlansLoaded(7382).openUserSettings({ screen: constants.PROFILE_CUSTOMIZATION });
  }, []);
  const items = [subscriptionPlansLoaded];
  const callback2 = noop.useCallback(() => {
    subscriptionPlansLoaded(7382).openUserSettings({ screen: constants.APPEARANCE_THEME_PICKER });
  }, []);
  ...
  const callback4 = noop.useCallback(() => {
    subscriptionPlansLoaded(7382).openUserSettings({ screen: constants.APP_ICONS });
  }, []);
  ...
  let obj2 = subscriptionPlansLoaded(4218);
  obj20.imageSrc = { uri: _modDef13438 };
  ...
  obj20.bodyComponent = closure_9(subscriptionPlansLoaded(4556).Text, obj22);
