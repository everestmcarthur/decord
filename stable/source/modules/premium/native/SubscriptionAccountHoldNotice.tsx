...
export default function SubscriptionAccountHoldNotice(subscription) {
  ...
  if (subscription.status === SubscriptionStatusTypes.ACCOUNT_HOLD) {
    ...
    const obj3 = { size: subscription(1178).IconSizes.MEDIUM, style: tmp.icon, source: _modDef12773 };
    ...
    const obj5 = { endDate: subscription.currentPeriodEnd, planDescription: subscription(4218).getDisplayName(subscription.planId) };
    obj4.children = intl.format(subscription(1114).t["7I21Iz"], obj5);
    items[1] = closure_6(subscription(4556).Text, obj4);
    obj2.children = items;
    ...
    items1[1] = closure_6(subscription(4975).Button, obj7);
    obj.children = items1;
