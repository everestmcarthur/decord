...
// Dependencies: [109, 5, 32, 19, 17, 7424, 2025, 7237, 7421, 1373, 1074, 1182, 4542, 1085, 21, 4560, 576, 4218, 4556, 1114, 38, 7431, 1880, 504, 13303, 7234, 4975, 4962, 4971, 10708, 7417, 4495, 7447, 8061, 9385, 8171, 7162, 7182, 5598, 10668, 7406, 4992, 10813, 4527, 1242, 7405, 1608, 4905, 10709, 4884, 7409, 4411, 10727, 10728, 10729, 10730, 10731, 10732, 1115, 2024, 7150, 11440, 7154, 5587, 8608, 13304, 1178, 9371, 10712, 10809, 2]
// Exports: default, getItemsByPremiumTypePredicate

// Module 7423 (PremiumPlanSelectionActionSheet)
...
function PlanOption(premiumItem) {
  ...
  let checkoutPlanPriceString = premiumItem(13303).useCheckoutPlanPriceString(premiumItem.productId, stateFromStores);
  const obj2 = premiumItem(13303);
  ...
  const checkoutPlanDiscountPrices = premiumItem(13303).useCheckoutPlanDiscountPrices(premiumItem.productId, obj4);
  const obj3 = premiumItem(13303);
  ({ orderRequired, orderRecord } = useNativeCheckoutStore((orderRequired) => ({ orderRequired: orderRequired.orderRequired, orderRecord: orderRequired.orderRecord })));
  ...
  if (tmp11) {
    ...
    tmp11 = skuId === first(4218).getSkuIdForPremiumType(premiumTier);
    const obj5 = first(4218);
  }
  let tmp14 = tmp11;
  if (tmp14) {
  ...
  const tierDisplayNameByPlanId = premiumItem(4218).getTierDisplayNameByPlanId(premiumItem.basePlanId);
  const tmp4Result = premiumItem(4218);
  const intervalString = first(4218).getIntervalString(premiumItem.interval, false);
  let tmp20 = tmp14;
  if (!tmp14) {
  ...
  if (null != checkoutPlanDiscountPrices) {
    formatRateResult = tmp4(7234).formatRate(checkoutPlanDiscountPrices.discountedPrice, tmp26.interval, tmp26.intervalCount);
    const tmp4Result5 = tmp4(7234);
  }
  if (tmp14) {
    const intl = tmp4(1114).intl;
    const obj6 = { price: tmp4(7234).formatPrice(0, USD, { minimumFractionDigits: 0, maximumFractionDigits: 0 }) };
    let formatToPlainStringResult = intl.formatToPlainString(tmp4(1114).t.hXcaLT, obj6);
    const tmp4Result6 = tmp4(7234);
  } else {
  ...
  const tmp4Result7 = premiumItem(7234);
  if (first > 0) {
  ...
  const items3 = [closure_35(premiumItem(4556).Text, obj11), ];
  ...
  items4[1] = closure_35(premiumItem(4556).Text, obj17);
  ...
  if (!tmp14) {
    ...
    if (null != formatRateResult) {
      ...
      if (null != discountOffer) {
        ...
        if (null == premiumItem.selectedPremiumType) {
          ...
          tmp33Result4 = tmp33(tmp4(4556).Text, obj19);
        }
      }
    }
    ...
    if (tmp33Result5) {
      ...
      tmp33Result5 = tmp33(tmp4(4556).Text, obj21);
    }
...
export default function PremiumPlanSelectionActionSheetWithOrderCTX(predicate) {
  ...
  if (obj.isIOS()) {
    ...
    const premiumTrialOffer = tmp(7447).usePremiumTrialOffer();
    ...
    const isEligibleForBogoOffer = tmp(10712).useIsEligibleForBogoOffer();
    if (null == fn3) {
      let premiumBundlesWithPredicate = tmp(7409).getPremiumBundlesWithPredicate(fn);
      const tmpResult7 = tmp(7409);
    } else {
    ...
    if (null != found) {
      ...
      const tmpResult9 = tmp(7409);
    } else {
      const obj5 = { subscriptionPlanId: __initData.PREMIUM_YEAR_TIER_2, skuId: tmp(4218).castPremiumSubscriptionAsSkuId(TIER_22.TIER_2), quantity: 1 };
      mapped = [obj5];
      const tmpResult10 = tmp(4218);
    }
    const obj6 = {
