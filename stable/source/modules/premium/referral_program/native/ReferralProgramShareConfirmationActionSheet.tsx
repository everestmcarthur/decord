...
// Module 13456 (ReferralProgramShareConfirmationActionSheet)
...
function SharedUser(user) {
  ...
  const items1 = [closure_5(user(1178).Avatar, { style: items, size: user(1178).AvatarSizes.REFRESH_MEDIUM_32, user, guildId: "Array" }), , ];
  if (tmp6) {
    const obj4 = { children: null };
    const obj5 = { variant: "text-md/medium", color: "text-muted", style: tmp.recipientDisplayName, children: name };
    const items2 = [tmp9(tmp5(4556).Text, obj5), ];
    ...
    items2[1] = tmp9(tmp5(4556).Text, obj6);
    ...
    tmp9Result = tmp9(tmp5(4556).Text, obj8);
  }
  items1[1] = tmp9Result;
  const obj9 = { variant: "secondary", size: "sm", text: null, icon: null, onPress: null };
  const intl2 = tmp5(1114).intl;
  obj9.text = intl2.string(user(1114).t["g33r/P"]);
  const obj3 = { style: items, size: user(1178).AvatarSizes.REFRESH_MEDIUM_32, user, guildId: "Array" };
  obj9.icon = closure_5(user(5071).ChatIcon, { size: "xs", color: nativeDefault.colors.CONTROL_SECONDARY_TEXT_DEFAULT });
  obj9.onPress = function onPress() {
    ActionSheetActionCreatorsDefault.hideActionSheet();
    ChannelActionCreatorsDefault.openPrivateChannel({ recipientIds: user.id });
  };
  items1[2] = closure_5(user(4975).Button, obj9);
...
export default function ReferralProgramShareConfirmationActionSheet(trialCreationResult) {
  ...
  const obj3 = { startExpanded: true, contentStyles: tmp.content, header: closure_5(tmp5(7149).BottomSheetTitleHeader, { title: null }), children: null };
  const obj4 = { children: null };
  const formatResult = intl3.format(tmp5(1114).t.AwGSWl, obj);
  const items = [closure_5(View, { style: tmp.headerAsset, children: closure_5(tmp5(5692).FistBumpSpotIllustration, {}) }), closure_5(tmp5(4556).Text, { variant: "heading-lg/bold", color: "mobile-text-heading-primary", style: tmp.header, children: stringResult }), closure_5(tmp5(4556).Text, { variant: "text-md/medium", color: "text-default", style: tmp.subheader, children: formatResult }), ];
  const obj8 = { style: tmp.recipientContainer, children: null };
  const obj5 = { style: tmp.headerAsset, children: closure_5(tmp5(5692).FistBumpSpotIllustration, {}) };
  ...
  obj4.children = items;
  obj3.children = closure_7(tmp5(4973).Stack, obj4);
  return closure_5(tmp5(7150).BottomSheet, obj3);
};
