...
function updateIOSExperiments() {
  if (obj.isIOS()) {
    ...
    if (NSUserDefaultsBridge != null) {
      const setShouldEnableYYTextReplacement = NSUserDefaultsBridge.setShouldEnableYYTextReplacement;
      if (setShouldEnableYYTextReplacement != null) {
        const result = setShouldEnableYYTextReplacement(tmp(17821).shouldEnableYYTextReplacement({ location: "NativeExperimentBridgeManager" }));
        const tmpResult = tmp(17821);
      }
    }
  }
  ...
  if (NSUserDefaultsBridge2 != null) {
    ...
    if (setShouldFixPushNotificationRawPayload != null) {
      const result1 = setShouldFixPushNotificationRawPayload(tmp(5275).isIOSPushNotificationRawPayloadFixExperimentEnabled());
      const tmpResult2 = tmp(5275);
    }
  }
  obj = PlatformUtils;
