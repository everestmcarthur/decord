...
// Dependencies: [1074, 1982, 1242, 10299, 4527, 13911, 1896, 13969, 13975, 2]
// Exports: default
...
export default function openGuildActionSheet(id) {
  ...
  if (!obj.isFavoritesGuildId(id.id)) {
    ...
    if (tmpResult.shouldNSFWGateGuild(id.id)) {
      const obj4 = { guild: id };
      tmp3(4527).openLazy(tmp(1896)(13911, tmp2.paths), "NsfwGateGuildSettingsActionSheet", obj4);
      const tmp3Result = tmp3(4527);
    } else {
      const features = id.features;
      const hasItem = features.has(constants2.HUB);
      const openLazy = tmp3(4527).openLazy;
      const tmpResult2 = tmp(1896);
      if (hasItem) {
        const _HermesInternal2 = HermesInternal;
        const obj5 = { guild: id, expanded: flag };
        openLazy(tmpResult2(13969, tmp2.paths), "GuildActionSheet:" + id.id, obj5);
        const tmpResult1Result = tmpResult2(13969, tmp2.paths);
      } else {
        const _HermesInternal = HermesInternal;
        const obj6 = { guild: id, expanded: flag };
        openLazy(tmpResult2(13975, tmp2.paths), "GuildActionSheet:" + id.id, obj6);
        const tmpResult1Result1 = tmpResult2(13975, tmp2.paths);
      }
