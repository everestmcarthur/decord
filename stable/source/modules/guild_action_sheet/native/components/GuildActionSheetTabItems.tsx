...
// Dependencies: [19, 1957, 2012, 2011, 1074, 21, 13964, 4469, 504, 9829, 9826, 5433, 8098, 1114, 9381, 576, 4740, 4527, 5434, 10036, 7950, 7119, 7381, 9069, 2]
// Exports: default
...
export default function GuildActionSheetTabItems(guild) {
  guild = guild.guild;
  let stateFromStores;
  let canAccessSettings = guild(13964).useGuildActionSheetPermissions(guild).canAccessSettings;
  ...
  let obj2 = guild(504);
  let shouldRenderInviteResult = guild(9829).shouldRenderInvite(stateFromStores, guild);
  ...
  obj6.icon = closure_12(guild(9381).BoostGemIcon, { color: stateFromStores(576).unsafe_rawColors.GUILD_BOOSTING_PINK });
  ...
  const items2 = [closure_12(guild(8098).IconButton, obj6), , , ];
  if (shouldRenderInviteResult) {
    ...
    obj8.icon = tmp3(10036);
    obj8.onPress = function onPress() {
      ActionSheetActionCreatorsDefault.hideActionSheet();
      closure_2();
    };
    shouldRenderInviteResult = tmp7(tmp(8098).IconButton, obj8);
  }
  ...
  obj9.icon = stateFromStores(7950);
  obj9.onPress = function onPress() {
    ActionSheetActionCreatorsDefault.hideActionSheet();
    NotificationSettingsModalActionCreatorsDefault.open(guild.id);
  };
  items2[2] = closure_12(guild(8098).IconButton, obj9);
  if (canAccessSettings) {
    ...
    obj10.icon = tmp3(7381);
    obj10.onPress = function onPress() {
      ActionSheetActionCreatorsDefault.hideActionSheet();
      GuildSettingsActionCreatorsDefault.open(guild.id);
    };
    canAccessSettings = tmp7(tmp(8098).IconButton, obj10);
  }
  items2[3] = canAccessSettings;
  obj4.children = items2;
  return tmp6(guild(5433).ButtonGroup, obj4);
};
