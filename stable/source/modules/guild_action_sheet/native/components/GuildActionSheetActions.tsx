...
// Dependencies: [19, 17, 7532, 1221, 1975, 7636, 2015, 4575, 4741, 1371, 1074, 7097, 7945, 4742, 21, 4560, 576, 504, 7200, 1114, 10969, 7335, 4380, 1943, 7528, 4527, 4763, 11554, 1896, 12295, 4712, 7162, 7182, 9213, 1178, 9809, 13914, 1187, 1940, 9073, 13915, 8166, 9190, 4556, 1935, 6995, 7119, 10152, 7114, 13963, 10149, 13964, 9679, 5407, 9743, 9704, 11574, 13965, 4906, 38, 13912, 10104, 10103, 7535, 11560, 13966, 8635, 8018, 11834, 7189, 7190, 4258, 1396, 2]
...
// Module 13913 (GuildActionSheetActions)
...
import useOpenProfileSettingsDefault from "useOpenProfileSettings" /* 9213 */;
...
import useIsServerThemeAvailableForGuildDefault from "useIsServerThemeAvailableForGuild" /* 13914 */;
import markGuildsAsReadDefault from "markGuildsAsRead" /* 13963 */;
import GuildAntiRaidModalActionCreators from "GuildAntiRaidModalActionCreators" /* 13966 */;
...
const DiscordTagDefault = tmp2(9809);
require = fn;
function HideMutedChannelsOption(guild) {
  ...
  return closure_21(guild(7200).ActionSheetSwitchRow, obj2);
}
function BrowseChannelsOption(guild) {
  ...
  const callback = noop.useCallback(() => {
    ...
    ModalActionCreatorsDefault.pushLazy(asyncRequireImpl(11554, dependencyMap.paths), { guildId: guild.id }, closure_18);
  }, items2);
  if (!features.has(constants2.COMMUNITY)) {
    ...
    tmp8Result2 = tmp8(tmp3(12295).NewBadge, {});
  }
}
...
function GuildThemePreferenceOption(guild) {
  ...
  if (tmp2) {
    ...
    tmp6 = closure_21(tmp3(7200).ActionSheetSwitchRow, obj2);
  }
  return tmp6;
}
function ServerTagOption(guild) {
  ...
  const tmp = closure_23();
  [][0] = guild.id;
  const result = guild(9073).canViewMobileServerTag(guild.id);
  if (result) {
    ...
    const guildTagBadgeUrl = tmp2(8166).getGuildTagBadgeUrl(guild.id, badge, GuildTagBadgeSize.SIZE_16);
    ...
    obj4.children = closure_21(tmp2(9190).BaseGuildTagChiplet, obj5);
    ...
    items[1] = closure_21(tmp2(4556).Text, obj6);
    obj2.children = items;
    obj3.label = closure_22(View, obj2);
    obj3.onPress = tmp5;
    return closure_21(tmp2(7200).ActionSheetRow, obj3);
  } else {
    return null;
  }
  let obj = guild(9073);
    ...
    t = {
      ...
      onPress() {
          if (stateFromStores1) {
            ...
            obj2.openLazy(asyncRequireImpl(10149, dependencyMap.paths), "muteSettings" + guild.id, obj3);
            const tmp5 = asyncRequireImpl(10149, dependencyMap.paths);
          }
        }
    };
...
export const GuildActionSheetPrimaryActions = function GuildActionSheetPrimaryActions(guild) {
  ...
  if (obj.useGuildActionSheetPermissions(guild).canManageChannels) {
    ...
    items1.push(closure_21(tmp(7200).ActionSheetRow, obj3));
    ...
    items1.push(closure_21(tmp(7200).ActionSheetRow, obj4));
  }
  if (tmp3) {
    ...
    items1.push(closure_21(tmp(7200).ActionSheetRow, obj5));
  }
  if (stateFromStores) {
...
export const GuildActionSheetDirectoryActions = function GuildActionSheetDirectoryActions(guild) {
  ...
  if (tmp4) {
    ...
    if (0 !== items.length) {
      ...
      tmp5Result = tmp5(tmp9(7200).ActionSheetRow.Group, obj2);
    }
    return tmp5Result;
  } else {
    const features = guild.features;
    ...
    t = items.push(tmp5(tmp9(7200).ActionSheetRow, obj3));
  }
};
export const GuildActionSheetSecondaryActions = function GuildActionSheetSecondaryActions(guild) {
  ...
  const canReportRaid = guild(10104).useCanReportRaid(guild);
  let obj = guild(10104);
  const guildIncidentsState = guild(10103).useGuildIncidentsState(guild.id);
  const items = [];
  ({ shouldShowIncidentActions, isUnderLockdown } = guildIncidentsState);
  let obj2 = guild(10103);
  const optInEnabledForGuild = guild(7535).useOptInEnabledForGuild(guild.id);
  items.push(closure_21(ChangeIdentityOption, { guild, user: currentUser }));
  ...
  if (features.has(constants2.COMMUNITY)) {
    ...
    items.push(tmp8(tmp3(7200).ActionSheetSwitchRow, obj4));
  }
  items.push(closure_21(HideMutedChannelsOption, { guild }));
  items.push(closure_21(RestrictedGuildPrivacyOption, { guild }));
  const obj3 = guild(7535);
  ...
  if (canReportRaid) {
    ...
    items.push(tmp8(tmp3(7200).ActionSheetRow, obj5));
  }
  if (!tmp2) {
    const obj6 = { label: null, onPress: null };
    ...
    items.push(tmp8(tmp3(7200).ActionSheetRow, obj6));
  }
  if (!shouldShowIncidentActions) {
    if (tmp2) {
      ...
      if (0 !== items.length) {
        ...
        tmp8Result = tmp8(tmp3(7200).ActionSheetRow.Group, obj7);
      }
      return tmp8Result;
    } else {
      ...
      t2 = items.push(tmp8(tmp3(7200).ActionSheetRow, obj8));
    }
  } else {
    ...
    const obj9 = {
      ...
      onPress() {
          ...
          ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(11834, dependencyMap.paths), "GuildIncidentActionsActionSheet", { guild, analyticsData: obj2 });
        }
    };
    t = items.push(tmp8(tmp3(7200).ActionSheetRow, obj9));
  }
};
  ...
  if (setting) {
    ...
    if (tmp5) {
      if (null != guild.icon) {
        ...
        items.push(tmp7(tmp(7200).ActionSheetRow, obj2));
      }
      if (null != guild.banner) {
        ...
        items.push(tmp7(tmp(7200).ActionSheetRow, obj3));
      }
    }
    ...
    return closure_21(tmp(7200).ActionSheetRow.Group, obj4);
  } else {
    return null;
  }
};
