...
// Function ID: 8135
// Name: useHeroColors
// Dependencies: [32, 19, 4552, 1183, 8133, 7552, 8135, 1091, 672, 504, 2]
// Exports: default, getHeroColors

// Module 8134 (useHeroColors)
...
export default function useHeroColors(pendingAvatarSrc) {
  ...
  let obj2 = first(504);
  const fallbackHeroColor = first(8135).getFallbackHeroColor(stateFromStores1, stateFromStores);
  let obj3 = first(8135);
  let tmp4 = _slicedToArray(first(8133).useAvatarColors(pendingAvatarSrc, fallbackHeroColor), 2);
  first = tmp4[0];
  closure_1 = tmp6;
...
export const getHeroColors = function getHeroColors(game_name) {
  const fallbackHeroColor = num(8135).getFallbackHeroColor(ThemeStore.theme, AccessibilityStore.saturation);
  num = 1;
  if (AccessibilityStore.desaturateUserColors) {
    num = tmp.saturation;
  }
  const useColorStore = tmp2(8133).useColorStore;
