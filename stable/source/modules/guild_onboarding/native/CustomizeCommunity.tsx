...
function EmptyCustomizeCommunity(setTab) {
  ...
  const obj = setTab(4269);
  const tmp4 = importDefault;
  const isThemeDarkResult = setTab(4269).isThemeDark(useThemeDefault());
  ...
  const items1 = [closure_16(closure_5, { style: tmp.emptyContainerImage, source: tmp4(isThemeDarkResult ? 11556 : 11557) }), , ];
  ...
  items1[2] = closure_16(setTab(4556).Text, obj6);
...
function DropdownPrompt(guildId) {
  ...
  const callback = stateFromStoresArray.useCallback(() => {
    ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(7135, dependencyMap.paths), "DropdownOptions", {
      guildId,
      promptId: _prompt.id,
      onSelect(arg0, arg1) {
        let flag = arg1;
...
export default function CustomizeCommunity(guildId) {
  ...
  const effect1 = noop.useEffect(() => null != guildId ? (() => {
    guildId(7110).ackGuildFeature(closure_1_0, constants.GUILD_ONBOARDING_QUESTION, GuildOnboardingPromptsStore.ackIdForGuild(closure_1_0));
  ...
  if (0 === onboardingPromptsRaw.length) {
    ...
    if (tmp24Result) {
      ...
      const items5 = [closure_16(tmp4(4556).Text, obj9), newOnboardingPrompts.map(callback), onboardingPromptsWithNewAnswers.map(callback), ];
