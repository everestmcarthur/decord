...
// Function ID: 8337
// Name: ICYMIStore
// Dependencies: [32, 8337, 4476, 7526, 502, 1957, 8346, 1979, 4781, 4199, 4575, 4209, 4741, 8348, 8350, 1074, 8359, 1090, 8351, 8349, 8131, 8136, 8342, 8360, 7341, 4783, 504, 573, 2]

// Module 8336 (ICYMIStore)
import initializeDefault from "initialize" /* 504 */;
import DispatcherDefault from "Dispatcher" /* 573 */;
import DurationsDefault from "Durations" /* 1090 */;
import utils from "utils" /* 8136 */;
import ICYMITypes from "ICYMITypes" /* 8349 */;
import ICYMIUtils from "ICYMIUtils" /* 8351 */;
...
import ICYMIFiltersStore from "ICYMIFiltersStore" /* 8348 */;
import ICYMIUnreadStateStore from "ICYMIUnreadStateStore" /* 8350 */;
  ...
  if (null != arg0) {
    ...
    let item = dehydratedItems.forEach((id) => {
      ...
      if (tmp4) {
        let tmp7 = tmp;
        if (!tmp) {
          tmp7 = !tmp2(8351).isItemUnreadInChannel(id.data.channel_id, id.data.message_id);
          const tmp2Result = tmp2(8351);
        }
        tmp6 = tmp7;
      }
      if (tmp6) {
        ...
        if (id.type === tmp2(8349).ICYMIItemTypes.MESSAGE) {
    ...
    if (!ACTIVITY.has(content.content.id)) {
      if (!tmpResult.isEntryExpired(content.content)) {
        ...
        if (null == dependencyMap[content.content.id]) {
          const obj2 = { id: content.content.id, type: tmp(8349).ICYMIItemTypes.ACTIVITY, score: 15, activity: content.content };
          dependencyMap[content.content.id] = obj2;
        }
        const obj3 = { id: content.content.id, type: tmp(8349).ICYMIItemTypes.ACTIVITY, score: 15, data: null };
  ...
  while (iter !== undefined) {
    ...
    if (nextResult.type !== ICYMITypes.ICYMIItemTypes.RECOMMENDED_GUILDS) {
      if (!set.has(tmp2.id)) {
        ...
        if (tmp7) {
          let tmp9 = tmp2.type !== tmp3(8349).ICYMIItemTypes.MESSAGE;
          if (!tmp9) {
            let tmp3Result = tmp3(8351);
...
const ContentInventoryFeedKey = fn(8359).ContentInventoryFeedKey;
...
obj = {
  ...
  LOAD_ICYMI_DEHYDRATED: function handleLoadDehydrated(items) {
    ...
    set1 = new Set(set1(8349).SUPPORTED_ITEM_TYPES);
    ...
    set = new Set(set1(8349).SUPPORTED_ITEM_TYPES);
    [arr9, arr10] = items5;
    ...
    if (c41) {
      if (0 !== c38) {
        if (!isInitialLoad) {
          ...
          const tmp8 = arr11.length > tmp(8349).MIN_ITEMS_FOR_NEW_PILL;
          if (!isReloading) {
            hasNewContent = tmp8;
          }
          if (tmp8) {
            ...
            tmp(8351).hydrateItems(items6, 0, tmp(8349).ICYMI_PAGE_SIZE);
            if (arr9.length + arr10.length === 0) {
              c54 = true;
            }
            const tmpResult = tmp(8351);
          }
          const ICYMIAnalytics = tmp(8360).ICYMIAnalytics;
