...
// Function ID: 16490
// Name: useICYMIItems
// Dependencies: [19, 8336, 8349, 504, 8352, 2]
// Exports: default
...
function createItem(id, type, unread) {
  ...
  if (ICYMITypes.ICYMIItemTypes.MESSAGE === type) {
    ...
    if (tmp(8349).ICYMIItemTypes.ACTIVITY !== type) {
      if (tmp(8349).ICYMIItemTypes.CUSTOM_STATUS !== type) {
        if (tmp(8349).ICYMIItemTypes.GUILD_EVENT === type) {
          ...
          obj6.unread = unread;
          return obj6;
        } else if (tmp(8349).ICYMIItemTypes.RECOMMENDED_GUILDS === type) {
    ...
    if (0 < stateFromStores2) {
      while (true) {
        ...
        if (!stateFromStores3[tmp6.id]) {
          ...
          let tmp10 = null == tmp9;
          if (tmp10) {
            tmp10 = tmp6.type === stateFromStores1(8349).ICYMIItemTypes.MESSAGE;
          }
          if (tmp10) {
    ...
    if (num < stateFromStores2) {
      while (true) {
        ...
        if (!stateFromStores3[tmp19.id]) {
          ...
          if (tmp23) {
            tmp23 = tmp19.type === stateFromStores1(8349).ICYMIItemTypes.MESSAGE;
          }
          if (tmp23) {
