...
// Function ID: 13013
// Name: Badges
// Dependencies: [19, 17, 2025, 21, 4560, 576, 13010, 8136, 4556, 504, 9229, 11610, 13013, 1114, 13015, 13017, 9204, 10185, 8723, 1090, 2]
...
const GameControllerIcon = tmp2(9229);
require = fn;
...
function ContentTimestamp(entry) {
  ...
  if (isEntryActiveResult) {
    ...
    const obj3 = { variant: "text-sm/medium", style: tmp.text, children: tmp2(8136).formatEndedTimestamp(entry, tmp6) };
    tmp7Result = tmp7(tmp2(4556).Text, obj3);
    const tmp2Result = tmp2(8136);
  }
  return tmp7Result;
}
...
export const MarathonBadge = function MarathonBadge(entry) {
  ...
  if (obj.isEntryMarathon(entry)) {
    const tmpResult = tmp(8136);
    const colors = nativeDefault.colors;
    const isEntryActiveResult = tmp(8136).isEntryActive(entry);
    const tmp6 = tmp(8136).isEntryActive(entry) ? colors.STATUS_POSITIVE : colors.CONTENT_INVENTORY_OVERLAY_TEXT_SECONDARY;
    const marathonDescription = tmp(8136).getMarathonDescription(entry);
    const text = marathonDescription.text;
    let tmp9 = null;
    if (null != text) {
      const obj2 = { Icon: tmp(11610).TimerIcon, iconColor: tmp6, text, accessibilityLabel: tmp8 };
...
export const NewGameBadge = function NewGameBadge(entry) {
  ...
  if (obj.isEntryNew(entry.entry)) {
    const obj2 = { Icon: tmp(13013).NewUserIcon, text: null, iconColor: null };
...
export const StreakBadge = function StreakBadge(entry) {
  ...
  if (null != streakCount) {
    tmp4 = null;
    if (streakCount >= 2) {
      const obj2 = { Icon: tmp(13015).FlashIcon, text: null, iconColor: null, accessibilityLabel: null };
      const intl = tmp(1114).intl;
...
export const TrendingBadge = function TrendingBadge(entry) {
  ...
  if (null != trendingType) {
    tmp4 = null;
    if (trendingType !== tmp(13017).TrendingType.TRENDING_TYPE_UNSPECIFIED) {
      const obj2 = { Icon: tmp(9204).FireIcon, text: null, iconColor: null };
      const intl = tmp(1114).intl;
...
export const ResurrectedBadge = function ResurrectedBadge(entry) {
  ...
  if (null != obj.getResurrectedEntryLastPlayTime(entry.entry)) {
    const obj2 = { Icon: tmp(10185).RetryIcon, text: null, iconColor: null };
    const intl = tmp(1114).intl;
...
export const TopGameBadge = function TopGameBadge(entry) {
  ...
  if (null == entryDuration) {
    ...
    const obj2 = { Icon: tmp(8723).TrophyIcon, text: null, iconColor: null };
