...
// Dependencies: [19, 17, 7526, 1957, 1979, 21, 16455, 576, 9674, 1114, 9671, 4713, 9711, 9775, 8352, 9795, 7342, 9786, 16496, 11, 4556, 9777, 5089, 1178, 504, 2]
// Exports: default
...
function ICYMIGuildEventRow(event) {
  ...
  const tmp4 = guild(9674)(event, null);
  ...
  const locationFromEvent = event(9711).getLocationFromEvent(event);
  let obj = event(9711);
  const eventLocationIconSource = event(9775).getEventLocationIconSource(event, channel, true);
  let obj2 = event(9775);
  const eventLocationIconComponent = event(9775).getEventLocationIconComponent(event, channel, true);
  let obj3 = event(9775);
  const items1 = [event];
  const nextRecurrenceIdInEvent = event(9671).getNextRecurrenceIdInEvent(event);
  ...
  const tmp2ResultResult = guild(9786)(guild_id, id, nextRecurrenceIdInEvent);
  ...
  obj7.children = closure_11(event(4556).Text, {
    ...
    children: noop.useMemo(() => {
      if (closure_2) {
        ...
        eventTimeData = tmp(9671).getEventTimeData(c3);
  ...
  items3[1] = closure_11(event(4556).Text, { style: title, variant: "text-lg/semibold", children: event.name });
  ...
  if (tmp19Result) {
    ...
    obj9.children = tmp8(9777).guildEventDetailsParser(event.description, true, obj10);
    tmp19Result = tmp19(tmp8(4556).Text, obj9);
    const tmp8Result = tmp8(9777);
  }
  ...
  items4[1] = closure_11(event(4556).Text, obj15);
  ...
  if (tmp7 == null) {
    ...
    if (null != locationFromEvent) {
      result = tmp8(9777).guildEventLocationParser(locationFromEvent, true);
      const tmp8Result2 = tmp8(9777);
    }
    tmp27 = result;
  }
  items6[1] = closure_11(event(4556).Text, { lineClamp: 2, variant: "text-xs/normal", color: "text-muted", children: tmp27 });
