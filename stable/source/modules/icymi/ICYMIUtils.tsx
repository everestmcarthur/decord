...
// Function ID: 8352
// Name: ICYMIUtils
// Dependencies: [5, 7526, 1961, 1957, 1979, 4781, 4575, 8336, 8350, 1074, 8349, 8352, 4783, 11, 504, 8353, 8357, 8131, 8358, 7898, 1114, 7110, 1896, 1074, 2]
...
import ContentInventoryAuthorType from "ContentInventoryAuthorType" /* 8357 */;
import ICYMIItemTypes from "ICYMIItemTypes" /* 8358 */;
      ...
      if (0 === c4) {
        if (arg0 === 1) {
          ...
          if (0 !== substr.length) {
            ...
            const found1 = found.filter((type) => type.type === dependencyMap(8349).ICYMIItemTypes.MESSAGE);
            ...
            const mapped1 = found.map((type) => {
              if (type.type === dependencyMap(8349).ICYMIItemTypes.MESSAGE) {
            ...
            const found3 = found.filter((type) => type.type === dependencyMap(8349).ICYMIItemTypes.ACTIVITY);
  ...
  if (channel.type === constants.GUILD_ANNOUNCEMENT) {
    ...
    if (message.attachments.length > 0) {
      ...
      if (obj.isValidImageAttachment(message.attachments[0])) {
        ContentType = tmp6(8349).ContentType;
        let IMAGE = ContentType.IMAGE;
      } else {
        const result = tmp6(7898).isValidVideoAttachment(message.attachments[0]);
        const ContentType2 = tmp6(8349).ContentType;
        IMAGE = result ? ContentType2.VIDEO : ContentType2.FILE;
        const tmp6Result = tmp6(7898);
      }
...
export const contentTypeToText = function contentTypeToText(arg0) {
  ...
  if (ICYMITypes.ContentType.POPULAR_MESSAGE === arg0) {
  ...
  } else if (tmp(8349).ContentType.IMAGE === arg0) {
    const intl9 = tmp(1114).intl;
    return intl9.string(tmp(1114).t.gmOWAo);
  } else if (tmp(8349).ContentType.VIDEO === arg0) {
    const intl8 = tmp(1114).intl;
    return intl8.string(tmp(1114).t.swhcPM);
  } else if (tmp(8349).ContentType.LINK === arg0) {
    const intl7 = tmp(1114).intl;
    return intl7.string(tmp(1114).t.oj5yvD);
  } else if (tmp(8349).ContentType.THREAD === arg0) {
    const intl6 = tmp(1114).intl;
    return intl6.string(tmp(1114).t.DwLrLK);
  } else if (tmp(8349).ContentType.FORUM_POST === arg0) {
    const intl5 = tmp(1114).intl;
    return intl5.string(tmp(1114).t["Q9/6BS"]);
  } else if (tmp(8349).ContentType.CHANGED_STATUS === arg0) {
    const intl4 = tmp(1114).intl;
    return intl4.string(tmp(1114).t.TGrUmi);
  } else if (tmp(8349).ContentType.INTERESTING === arg0) {
    const intl3 = tmp(1114).intl;
    return intl3.string(tmp(1114).t["TahE/i"]);
  } else if (tmp(8349).ContentType.ANNOUNCEMENT === arg0) {
    ...
    return stringResult;
  } else if (tmp(8349).ContentType.FILE === arg0) {
