...
// Function ID: 4772
// Name: AgeGateUtils
// Dependencies: [1975, 1957, 4772, 1979, 1371, 1098, 1074, 1114, 11, 4773, 5423, 5424, 504, 8414, 2024, 7211, 7329, 2]
...
function shouldShowAgeGateForVoiceChannel(channelId) {
  ...
  if (tmp4) {
    ...
    let flag = false;
    if (null != currentUser) {
      const result1 = tmp(4773).shouldShowTiggerPawtect();
      const tmpResult = tmp(4773);
      const tmpResult2 = tmp(5423);
      const tmp9 = tmp(5423).isFeatureAgeGated(tmp(5424).AgeGatedFeature.AGE_GATED_SPACES) && result1;
      flag = true !== currentUser.nsfwAllowed || tmp(5423).isFeatureAgeGated(tmp(5424).AgeGatedFeature.AGE_GATED_SPACES) && result1;
      const tmp10 = true !== currentUser.nsfwAllowed || tmp(5423).isFeatureAgeGated(tmp(5424).AgeGatedFeature.AGE_GATED_SPACES) && result1;
    }
    let tmp12 = !flag;
    if (flag) {
...
export const useAgeGateVerifyContentForGuild = function useAgeGateVerifyContentForGuild(stateFromStores) {
  ...
  if (shouldShowTiggerPawtect) {
    ...
    if (!isAgeVerified) {
      ...
      obj5.modalType = tmp(8414).NsfwSpaceWarningModalType.NSFW_CHANNEL_AGE_VERIFY;
    }
    ...
    NSFW_CHANNEL_AGE_VERIFY = tmp(8414).NsfwSpaceWarningModalType.NSFW_CHANNEL_AGE_VERIFY;
    ...
    NSFW_CHANNEL_UNDERAGE = tmp(8414).NsfwSpaceWarningModalType.NSFW_CHANNEL_UNDERAGE;
    ...
    obj5.modalType = tmp(8414).NsfwSpaceWarningModalType.NSFW_CHANNEL_VERIFIED;
    return obj5;
  }
};
