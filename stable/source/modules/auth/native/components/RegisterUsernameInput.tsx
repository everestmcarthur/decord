...
function UsernameStatusMessage(arg0) {
  ...
  if (type === UniqueUsernamesTypes.NameValidationState.ERROR) {
    ...
    const items = [React7(tmp3(6610).CircleErrorIcon, obj3), ];
    ...
    items[1] = React7(tmp3(4556).Text, obj4);
    obj2.children = items;
    let tmp6 = closure_1_10(tmp3(4973).Stack, obj2);
  } else {
    if (isUsernameFocused) {
      ...
      if (type1 === tmp3(14709).NameValidationState.AVAILABLE) {
        ...
        obj5.children = React7(tmp3(4556).Text, obj6);
        tmp6 = React7(tmp3(4556).Text, obj5);
      }
    }
    tmp6 = null;
    if (isUsernameFocused) {
...
obj3.layout = LinearTransition.easing(Easing.inOut(fn(4296).Easing.quad)).duration(300);
const size = fn(2);
...
export const RegisterUsernameInput = function RegisterUsernameInput(setUsername) {
  ...
  const items3 = [closure_9(setUsername(6606).TextInput, obj3), closure_9(UsernameStatusMessage, { usernameStatus, isUsernameFocused: tmp5[0] })];
