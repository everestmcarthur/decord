...
// Dependencies: [5, 32, 19, 17, 6595, 4544, 8745, 15949, 15950, 1074, 21, 4560, 576, 4296, 6944, 15946, 504, 15972, 15974, 15957, 1114, 15965, 15948, 15964, 6972, 15975, 15976, 4975, 6941, 5578, 2]
// Exports: default
...
import useAuthFlowBackHandlerDefault from "useAuthFlowBackHandler" /* 15965 */;
import asyncGeneratorStep from "asyncGeneratorStep" /* 5 */;
import _slicedToArray from "module_32" /* 32 */;
import noop from "module_19" /* 19 */;
import ConsentStore from "ConsentStore" /* 6595 */;
import InviteStore from "InviteStore" /* 4544 */;
import DisplayedInviteStore from "DisplayedInviteStore" /* 8745 */;
...
obj2.layout = LinearTransition.easing(Easing.inOut(fn(4296).Easing.quad)).duration(300);
const size = fn(2);
...
export default function RegisterAccountInformation() {
  ...
  const passwordRegistrationStep = context(15972).usePasswordRegistrationStep();
  ({ password, validatePassword: _slicedToArray, setPassword, passwordScore, preventSubmitPassword } = passwordRegistrationStep);
  let obj4 = context(15972);
  const usernameRegistrationStep = context(15974).useUsernameRegistrationStep(AuthStates.REGISTER_ACCOUNT_INFORMATION);
  ...
  const result = context(15957).hasAllRegistrationFieldsCompleted(obj6, { isConsentRequired: true === stateFromStores1 });
  ConsentStore = result;
  ...
  const tmp5Result3 = context(15957);
  ...
  const tmp5Result4 = context(15948);
  ...
  const items4 = [closure_17(closure_6, { style: tmp4.container, children: closure_17(context(15975).RegisterUsernameInput, { username, setUsername, onSubmitEditing: callback, usernameStatus, submitBehavior: "submit", autoFocus: true }) }), , ];
  ...
  obj11.children = closure_17(context(15976).RegisterPasswordInput, obj12);
  ...
  const items5 = [closure_17(context(4975).Button, obj14), ];
  let tmp27Result = null;
  if (null != tmp11.message) {
    tmp27Result = null;
    if ("" !== tmp11.message) {
      const obj15 = { style: tmp4.errors, children: tmp11.message };
      tmp27Result = tmp27(tmp(6941), obj15);
    }
  }
  ...
  if (!tmp3) {
    ...
    tmp27Result4 = tmp27(tmp(5578), obj16);
  }
  return tmp27Result4;
};
