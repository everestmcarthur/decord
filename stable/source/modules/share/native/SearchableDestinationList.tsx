...
// Dependencies: [32, 19, 17, 1074, 10865, 21, 4560, 576, 10990, 7038, 10986, 9841, 7661, 1369, 7049, 10124, 10998, 10871, 10999, 1114, 5125, 7050, 2]
// Exports: default
...
export default function SearchableDestinationList(getRowIsUnavailable) {
  ...
  const callback5 = obj.useCallback((arg0, arg1) => {
    ...
    if (type !== sortByMatchScore.AutocompleterResultTypes.HEADER) {
      const tmp2Result = tmp2(10986);
      const destinationKeyResult = tmp2Result.destinationKey(tmp2(10986).getDestinationIdFromResult(tmp));
      let tmp6;
      ...
      if (tmp2(9841).AutocompleterResultTypes.USER === type) {
        const element = { type: "user", props: null };
        const obj3 = {};
        const merged1 = Object.assign(obj);
        obj3.user = record;
        obj3.type = tmp2(7661).getRelationshipType(record.id);
        obj3.onPress = callback3;
        element.props = obj3;
        return element;
      } else if (tmp2(9841).AutocompleterResultTypes.GROUP_DM === type) {
        ...
        if (tmp2(9841).AutocompleterResultTypes.TEXT_CHANNEL !== type) {
          if (tmp2(9841).AutocompleterResultTypes.VOICE_CHANNEL !== type) {
