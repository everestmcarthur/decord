...
// Dependencies: [4199, 1074, 7292, 7297, 7296, 7898, 563, 1935, 11531, 1114, 2]
...
function getForumPostShouldObscure(media, arg1, enabledHarmTypesBitmaskForChannelType) {
  if (null == media) {
    ...
    if (ForumPostMediaUtils.ForumPostMediaTypes.EMBED === type) {
      const obj2 = { type: tmp4(7297).ObscuredMediaTypes.Embed, media };
      let tmp = obj2;
    } else if (tmp4(7898).ForumPostMediaTypes.ATTACHMENT === type) {
      const obj = { type: tmp4(7297).ObscuredMediaTypes.Attachment, media };
      tmp = obj;
    } else {
      tmp = null;
      if (tmp4(7898).ForumPostMediaTypes.COMPONENT === type) {
        const obj3 = { type: tmp4(7297).ObscuredMediaTypes.GenericMedia, media: media.srcUnfurledMediaItem };
        tmp = obj3;
      }
    }
    if (null == tmp) {
      ...
      const mediaObscuredReasonFromBitmask = tmp4(7292).getMediaObscuredReasonFromBitmask(tmp, enabledHarmTypesBitmaskForChannelType);
      tmp4(7292);
...
export const getObscureReasonForEmbed = function getObscureReasonForEmbed(embed, message, flag2, enabledContentHarmTypeFlags) {
  ...
  if (!bot) {
    const obj3 = { type: tmp(7297).ObscuredMediaTypes.Embed, media: embed };
    isMediaScanPendingResult = tmp(7292).isMediaScanPending(obj3, enabledContentHarmTypeFlags);
...
export const getObscureReasonForUnfurledMediaItem = function getObscureReasonForUnfurledMediaItem(unfurledMediaItem, enabledHarmTypesBitmaskForChannelAndAuthorId, arg2, isBot) {
  ...
  if (!flag2) {
    const obj3 = { type: tmp(7297).ObscuredMediaTypes.GenericMedia, media: unfurledMediaItem };
    isMediaScanPendingResult = tmp(7292).isMediaScanPending(obj3, enabledHarmTypesBitmaskForChannelAndAuthorId);
    const tmpResult = tmp(7292);
  }
