...
// Dependencies: [7595, 4209, 1371, 1074, 8268, 11549, 1096, 8264, 4255, 4792, 8262, 4713, 5028, 2]
...
export const handleMessagesTapImage = function handleMessagesTapImage(tapImageData) {
  ...
  if (null != portal) {
    embedId(8268).markPortalAlive(portal);
    const obj = embedId(8268);
  }
  if (true === allowWithinModal) {
    ...
    if (type1 !== embedId(1096).MessageReferenceTypes.FORWARD) {
      ...
      const found = attachments.filter((item) => !embedId(8264).isThumbnailAttachment(item));
      if ("attachment" === type) {
        if (index < found.length) {
          const tmp10Result = tmp10(8264);
      ...
      if ("embed" === type) {
        if (null != embedIndex) {
          ...
          if (importDefault < result.length) {
            tmp10(8264).setMediaSourcePortal(result[importDefault], portal);
            found2 = result;
            tmp25 = tmp10;
            const tmp10Result8 = tmp10(8264);
          }
        }
        if (-1 !== importDefault) {
          ...
          if (showContextName) {
            ...
            const tmp25Result4 = tmp25(5028);
          }
          obj3.contextIcon = channelIcon;
          tmp25(8262).openMediaModal(obj3);
          const tmp25Result = tmp25(8262);
        }
      }
      if ("component" === type) {
        if (null == componentId) {
          ...
          if (null != embedId) {
            ...
            if (null != components) {
              if (0 !== components.length) {
                const tmp10Result9 = tmp10(8264);
                const result1 = tmp10Result9.extractMediaSourcesFromComponent(tmp7, components, messageChannel.guild_id, tmp10(4792).asComponentId(componentId), componentMediaIndex);
                if (null != result1) {
                  ({ sources, initialIndex: closure_1 } = result1);
                  tmp10(8264).setMediaSourcePortal(sources[importDefault], portal);
                  found2 = sources;
                  tmp25 = tmp10;
                  const tmp10Result11 = tmp10(8264);
                }
                const tmp10Result10 = tmp10(4792);
              }
            }
          }
        ...
        const result2 = tmp10(8264).extractMediaSourcesFromMessage(tmp7, tmp12, messageChannel.guild_id);
        let num2 = 0;
        found2 = result2;
        tmp25 = tmp10;
        if (0 < result2.length) {
          ...
          if (flattenSourceResult.noCarousel) {
            ...
            importDefault = 0;
            tmp20(8264).setMediaSourcePortal(items[0], portal);
            found2 = items;
            tmp25 = tmp20;
            const tmp20Result = tmp20(8264);
          } else {
            importDefault = num2;
            tmp20(8264).setMediaSourcePortal(result2[num2], portal);
            ...
            tmp25 = tmp20;
            const tmp20Result2 = tmp20(8264);
          }
        }
        const tmp10Result12 = tmp10(8264);
      }
    } else {
