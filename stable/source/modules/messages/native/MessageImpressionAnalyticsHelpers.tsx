      ...
      if (state.state !== tmp.BANNED) {
        if (state.state !== tmp.ERROR) {
          if (obj5.getInviteType(state) !== InviteTypes.GUILD) {
            ...
            const guildInviteExtendedType = tmp7(7735).getGuildInviteExtendedType(state);
            if (guildInviteExtendedType !== tmp7(7735).GuildInviteExtendedType.VOICE_CHANNEL) {
              ...
              if (null != id) {
                ...
                let enabled = tmp7(11311).getVoiceChannelListInviteExperiment(obj).enabled;
                if (enabled) {
                  enabled = tmp7(11312).canShowVoiceChannelListInviteEmbed(state);
                  const tmp7Result4 = tmp7(11312);
                }
                const obj2 = { treatmentRendered: enabled };
                tmp4 = obj2;
                const tmp7Result3 = tmp7(11311);
              }
              return tmp4;
            }
            const tmp7Result = tmp7(7735);
          }
          ...
          if (nextResult.type === CodedLink.CodedLinkType.INVITE) {
            ...
            if (null != invite) {
              ...
              if (null != tmp37) {
                ...
                obj.type = tmp7(11310).MessageViewTrackingType.VOICE_INVITE_EMBED;
