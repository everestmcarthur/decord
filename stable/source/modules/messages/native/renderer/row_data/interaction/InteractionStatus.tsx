...
export const createInteractionStatus = function createInteractionStatus(message, interaction) {
  ...
  if (InteractionUtils.InteractionStatusViewState.SENDING === interactionStatusViewState) {
    ...
    return obj2;
  } else if (tmp(8117).InteractionStatusViewState.CREATED === interactionStatusViewState) {
    ...
    return obj3;
  } else if (tmp(8117).InteractionStatusViewState.TIMED_OUT === interactionStatusViewState) {
    const obj5 = { text: null, state: null };
    const intl2 = tmp(1114).intl;
    obj5.text = intl2.string(tmp(1114).t.h8hzPd);
    obj5.state = constants.FAILED;
    return obj5;
  } else if (tmp(8117).InteractionStatusViewState.FAILED === interactionStatusViewState) {
    ...
    return obj6;
  } else if (tmp(8117).InteractionStatusViewState.EPHEMERAL_SUCCESS === interactionStatusViewState) {
