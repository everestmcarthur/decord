...
// Dependencies: [17, 1385, 1957, 1371, 1074, 1399, 1396, 4793, 11624, 576, 7521, 1894, 7962, 7964, 9552, 1114, 9295, 2]
...
import createDisplayNameStylesMobile from "createDisplayNameStylesMobile" /* 7964 */;
...
export const createExecutedCommand = function createExecutedCommand(message, channel, roleStyle, forcedTheme, defaultUsernameColor) {
  ...
  if (null != message.interaction) {
    ...
    if (null == user2) {
      ...
      if (result1) {
        processColorStringsResult = tmp16(7962).processColorStrings(userAuthor.colorStrings);
        const tmp16Result13 = tmp16(7962);
      }
      let processColorStringsResult1 = null;
      if (result2) {
        processColorStringsResult1 = tmp16(7962).processColorStrings(userAuthor1.colorStrings);
        const tmp16Result14 = tmp16(7962);
      }
      ...
      if (null != tmp24) {
        ...
        const displayNameFontIdForMobileUser1 = tmp16(7964).getDisplayNameFontIdForMobileUser(user3, guildId);
        const tmp16Result16 = tmp16(7964);
      }
      ...
      if (tmp) {
        if (!result) {
          const result3 = tmp16(9552).isActivitiesInTextEnabled(channel);
          ...
          const tmp16Result18 = tmp16(9552);
        }
      ...
      if (result) {
        result4 = tmp16(9295).formatPrimaryEntryPointCommandName(displayName);
        const tmp16Result19 = tmp16(9295);
      }
      ...
      if (null != tmp24) {
        ...
        obj17.targetUsername = tmp16(4793).getUserAuthor(tmp24, channel).nick;
        let id3;
        if (tmp24 != null) {
          id3 = tmp24.id;
        }
        ...
        const tmp16Result20 = tmp16(4793);
      }
