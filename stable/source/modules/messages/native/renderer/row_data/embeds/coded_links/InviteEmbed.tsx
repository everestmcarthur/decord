        ...
        if (invite.state !== tmp5.BANNED) {
          if (invite.state === tmp5.ERROR) {
            ...
            if (InviteTypes.GROUP_DM === inviteType) {
              return tmp29(13215).createGroupDMInvite(invite, tmp28, theme);
            } else if (tmp32.FRIEND === inviteType) {
              const tmp29Result10 = tmp29(13216);
              ...
              if (tmp29(7735).GuildInviteExtendedType.EVENT === guildInviteExtendedType) {
                ...
                if (null != id1) {
                  ...
                  if (tmp29Result15.getVoiceChannelListInviteExperiment(obj3).enabled) {
                    const voiceChannelListInviteEmbed = tmp29(11312).createVoiceChannelListInviteEmbed(invite, theme);
                    if (null != voiceChannelListInviteEmbed) {
                      return voiceChannelListInviteEmbed;
                    }
                    const tmp29Result16 = tmp29(11312);
                  }
                  tmp29Result15 = tmp29(11311);
                }
                return tmp29(13213).createGuildInvite(invite, tmp28, theme);
              } else {
                return tmp29(13213).createGuildInvite(invite, tmp28, theme);
              }
              const tmp29Result11 = tmp29(7735);
            }
          }
        }
      }
