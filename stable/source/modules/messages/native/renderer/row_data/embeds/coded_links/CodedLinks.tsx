...
// Function ID: 13211
// Name: CodedLinks
// Dependencies: [32, 4200, 1979, 1371, 7690, 11944, 4548, 13211, 13212, 13222, 13224, 13217, 13226, 13219, 11484, 11812, 11536, 13227, 1369, 2]
        ...
        if (obj.isApplicationCodedLink(type)) {
          if (null == closure_1) {
            ...
            if (tmpResult.isApplicationCodedLinkMobileSupported(type)) {
              const applicationCodedLinkData = tmp(7690).getApplicationCodedLinkData(type, code, url);
              if (null == applicationCodedLinkData) {
                return null;
              } else {
                const obj2 = { appId: applicationCodedLinkData.applicationId, channel: tmp34, message, theme };
                const appLinkGateResult = tmp(11944).getAppLinkGateResult(obj2);
                if ("unavailable" === appLinkGateResult.state) {
                  ...
                  if (tmp(4548).CodedLinkType.ACTIVITY_BOOKMARK === type2) {
                    ...
                    return tmp(13211).createActivityMessageEmbed(obj3);
                  } else {
                    ...
                    return tmp(11944).createAppMessageEmbed(obj4);
                  }
                }
                const tmpResult19 = tmp(11944);
              }
              const tmpResult18 = tmp(7690);
            } else {
              return null;
            }
            tmpResult = tmp(7690);
          ...
          return tmp(13212).createInviteEmbed(message, code, theme);
        } else if (tmp(4548).CodedLinkType.TEMPLATE === type) {
          return tmp(13222).createGuildTemplateEmbed(code, theme);
        } else if (tmp(4548).CodedLinkType.BUILD_OVERRIDE === type) {
          return tmp(13224).createBuildOverrideEmbed(code, theme);
          ...
          if (isStaffResult) {
            buildOverrideEmbed = tmp(13224).createBuildOverrideEmbed(code, theme);
            const tmpResult25 = tmp(13224);
          }
          return buildOverrideEmbed;
        } else if (tmp(4548).CodedLinkType.EVENT === type) {
          return tmp(13217).createGuildScheduledEventLinkEmbed(code, theme);
        } else if (tmp(4548).CodedLinkType.CHANNEL_LINK === type) {
          return tmp(13226).createVoiceChannelLinkEmbed(code, theme);
        } else if (tmp(4548).CodedLinkType.EMBEDDED_ACTIVITY_INVITE === type) {
          const obj5 = { theme, inviteCode: code };
          return tmp(13219).createEmbeddedActivityInviteEmbed(obj5);
        } else if (tmp(4548).CodedLinkType.EXPERIMENT === type) {
          let experimentEmbed = null;
          if (tmpResult29.canSeeExperimentEmbeds()) {
            experimentEmbed = tmp(11812).createExperimentEmbed(url, theme);
            const tmpResult30 = tmp(11812);
          }
          ...
          if (tmp(4548).CodedLinkType.GUILD_PRODUCT !== type) {
            if (tmp(4548).CodedLinkType.SERVER_SHOP !== type) {
              if (tmp(4548).CodedLinkType.SOCIAL_LAYER_STOREFRONT !== type) {
                if (tmp(4548).CodedLinkType.SOCIAL_LAYER_STOREFRONT_APP !== type) {
                  if (tmp(4548).CodedLinkType.QUESTS_EMBED === type) {
                    ...
                    return tmp(13227).createQuestsEmbed(obj6);
                  } else {
              ...
              if (type === tmp(4548).CodedLinkType.SOCIAL_LAYER_STOREFRONT_APP) {
                ...
                obj7.theme = theme;
                return tmp(11536).createSocialLayerStorefrontProductDetailsEmbed(obj7);
              } else {
                ...
                obj9.theme = theme;
                return tmp(11536).createSocialLayerStorefrontProductDetailsEmbed(obj9);
              }
            }
          }
          return null;
