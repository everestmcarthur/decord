...
// Dependencies: [1374, 7961, 7963, 1114, 1396, 4213, 8043, 7965, 2]
// Exports: createPollResultSystemMessage

// Module 8042 (PollResultSystemMessage)
import EmojiConstants from "EmojiConstants" /* 1374 */;
import useAuthorWithProcessedColor from "useAuthorWithProcessedColor" /* 7961 */;
import parsePollResultSystemMessageEmbedDefault from "parsePollResultSystemMessageEmbed" /* 8043 */;
...
export const createPollResultSystemMessage = function createPollResultSystemMessage(message) {
  ...
  if (null == tmp3) {
    ...
    obj2.usernameOnClick = tmp(7963)(obj4);
    ...
    if (0 === tmp3.totalVotes) {
      ...
      let formatToPartsResult = intl3.formatToParts(tmp21(1114).t["9dPxsm"], obj7);
    } else {
      const _Math = Math;
      ...
      if (null == tmp3.victorAnswerId) {
        ...
        formatToPartsResult = intl2.formatToParts(tmp21(1114).t.dqftZ2, obj8);
      } else {
        const items = [];
        ...
        if (null == id) {
          ...
          formatToPartsResult = intl.formatToParts(tmp21(1114).t.zFwIxC, obj10);
        } else {
          if (null != id.id) {
            ...
            obj12.src = tmp(1396).getEmojiURL(obj13);
            const tmpResult = tmp(1396);
            const obj15 = { id: null, animated: false, size: null };
            id = id.id;
            obj15.id = id;
            obj15.size = EMOJI_URL_BASE_SIZE;
            obj12.frozenSrc = tmp(1396).getEmojiURL(obj15);
            items.push(obj12);
            const tmpResult3 = tmp(1396);
          } else {
            const obj = { type: "emoji", content: tmp(4213).convertSurrogateToName(id.name, false), surrogate: id.name };
            items.push(obj);
            const tmpResult4 = tmp(4213);
          }
    ...
    const obj16 = {};
    const merged3 = Object.assign(tmp(7965)(message));
