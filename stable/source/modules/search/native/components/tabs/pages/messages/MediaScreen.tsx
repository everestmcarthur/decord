...
// Function ID: 16696
// Name: MediaScreen
// Dependencies: [19, 7330, 1957, 7281, 12343, 7879, 12357, 1074, 21, 16688, 16648, 16696, 504, 16697, 12342, 4417, 12344, 8263, 16645, 14824, 8412, 8414, 16698, 16702, 16652, 2]

// Module 16695 (MediaScreen)
...
export default noop.memo(function MediaScreen(searchContext) {
  ...
  const searchMessages = searchContext(16696).useSearchMessages(searchContext, tab);
  let obj2 = searchContext(16696);
  ...
  const searchMessagesLoadingState = searchContext(16697).useSearchMessagesLoadingState({ searchContext, tab, placeholderHeight: tmp2, numColumns });
    ...
    const nextMessages = SearchPlatformUtilsDefault.fetchNextMessages(searchContext, tab, () => {
      if (obj.isModalOpen(MEDIA_MODAL_KEY)) {
        ...
        if (null != messages) {
          ...
          const obj2 = { sources: items };
          const result = tmp(tmp2[17]).setMediaViewerSources(obj2);
  ...
  const onPressMediaItem = searchContext(16645).useOnPressMediaItem({ searchContext, allMediaResults: memo, onEndReached: callback, onEndReachedThreshold: 500 });
  ...
  const callback1 = searchMessages.useCallback((media, index) => {
    ...
    if (obj.shouldAgeVerifyForSearchMedia(media, found)) {
      const obj2 = { entryPoint: tmp2(8414).AgeVerificationModalEntryPoint.SEARCH_MEDIA_PREVIEW };
      ...
      obj5.index = index;
      const result1 = tmp2(16698).trackMessageItemPress(obj5);
  ...
  obj7.ItemSeparatorComponent = searchContext(16652).MediaVerticalSeparator;
