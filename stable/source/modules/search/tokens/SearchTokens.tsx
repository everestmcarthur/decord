...
// Function ID: 12346
// Name: SearchTokens
// Dependencies: [32, 1957, 2012, 4209, 2011, 4405, 1371, 12346, 12347, 1074, 4153, 12, 1114, 4713, 4404, 12344, 12348, 5517, 5442, 9841, 12349, 12350, 2]
...
function dateValidator(getFullMatch, arg1) {
  ...
  if (null != tmp2) {
    ...
    if (set.has(formatted)) {
      ...
      const _Set = Set;
      const tmp32Result = tmp32(4153);
      const set1 = new Set(tmp32(4153).weekdays().map((item) => item.toLowerCase()));
      ...
      const weekdaysResult = tmp32(4153).weekdays();
    }
...
function generateDateAutocompletions() {
  ...
  const arraySpreadResult = HermesBuiltin.arraySpread(Array.from(new Set(_modDef4153.weekdays().map((item) => item.toLowerCase()))), tmp2);
...
function getChannelAutocompletions(arg0) {
  ...
  if (str.startsWith("\"")) {
    ...
    if (obj3.isGuildLikeSearchContext(searchContext)) {
      ...
      obj.boosters = tmp(5442).getBoosterMap(tmp(9841).AutocompleterResultTypes.TEXT_CHANNEL);
      const tmpResult = tmp(5442);
      ...
      obj2.boosters = tmp(5442).getBoosterMap(tmp(9841).AutocompleterResultTypes.VOICE_CHANNEL);
      ...
      if (searchContext.type === constants.DMS) {
        if (!StreamerModeStore.hidePersonalInformation) {
          ...
          obj5.boosters = tmp(5442).getBoosterMap(tmp(9841).AutocompleterResultTypes.GROUP_DM);
          const tmpResult5 = tmp(5442);
          ...
          obj6.boosters = tmp(5442).getBoosterMap(tmp(9841).AutocompleterResultTypes.USER);
          const tmpResult6 = tmp(5442);
          const queryDMChannelsResult = obj8.queryDMChannels(obj6);
          const sorted = _modDef12(queryGroupDMsResult.concat(queryDMChannelsResult)).sort(tmp(9841).sortByMatchScore);
...
export const isValidFilterAnswerForSubmit = function isValidFilterAnswerForSubmit(searchTokenType, trimmed) {
  if (null == closure_37[searchTokenType]) {
    ...
    if (SearchTokenTypes.ANSWER_HAS === tmp) {
      return tmp7(12348).validateForMapWithNegation("has", getHasMap(), token);
      ...
      return tmp7(12348).validateForMapWithNegation("author_type", obj, token);
