...
// Dependencies: [1957, 2021, 1979, 2011, 4405, 1371, 1074, 5515, 12344, 9845, 4404, 4679, 5519, 5442, 12345, 504, 573, 2]
...
import UserSearchManagerDefault from "UserSearchManager" /* 9845 */;
import SearchUtils from "SearchUtils" /* 12344 */;
import SearchTokensDefault from "SearchTokens" /* 12345 */;
  ...
  if (constants.FILTER === type) {
    ...
    if (null != filter) {
      ...
      if (obj.isGuildLikeSearchContext(searchContext)) {
        ...
        if (tmp8) {
          if (0 !== str.length) {
            const searchContextId = tmp6(12344).getSearchContextId(searchContext);
            value = map1.get(searchContextId);
            if (value == null) {
...
const searchAutocompleteStoreClass = new SearchAutocompleteStoreClass(DispatcherDefault, {
  ...
  SEARCH_AUTOCOMPLETE_QUERY_UPDATE: function handleSearchAutocompleteQueryUpdate(arg0) {
    ...
    if (null != value) {
      ...
      const searchContextId1 = tmp5(12344).getSearchContextId(searchContext);
      let value4 = map1.get(searchContextId1);
      if (value4 == null) {
        const obj6 = { results: [], context: tmp(9845).getUserSearchContext(handleUserSearchResults.bind(null, searchContext)) };
        value4 = obj6;
    ...
    if (tmp9) {
      const searchContextId2 = tmp5(12344).getSearchContextId(searchContext);
      let value5 = map1.get(searchContextId2);
      if (value5 == null) {
        const obj9 = { results: [], context: tmp(9845).getUserSearchContext(handleUserSearchResults.bind(null, searchContext)) };
        value5 = obj9;
        const tmpResult3 = tmp(9845);
      }
      ...
      if (null != trimmed) {
        if (trimmed.length > 0) {
          const guildIdFromSearchContext = tmp5(12344).getGuildIdFromSearchContext(searchContext);
          if (null != guildIdFromSearchContext) {
            const members = tmp(5519).requestMembers(guildIdFromSearchContext, trimmed, c17);
            const tmpResult4 = tmp(5519);
          }
          ...
          obj10.filters = obj11;
          const tmp5Result5 = tmp5(12344);
          obj10.boosters = tmp5(5442).getBoosterMap(AutocompleterResultTypes.USER);
