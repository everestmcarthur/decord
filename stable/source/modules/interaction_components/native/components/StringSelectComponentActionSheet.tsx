...
// Dependencies: [32, 19, 21, 4560, 576, 8120, 1894, 4527, 11827, 7130, 4556, 1114, 2]
// Exports: default
...
export default function StringSelectComponentActionSheet(selectionActionComponent) {
  ...
  let tmp = callback();
  dependencyMap = tmp;
  let obj = selectionActionComponent(8120);
  let tmp3 = first(noop.useState(new Set(selectionActionComponent(8120).getInitialStringSelectOptions(selectionActionComponent, containerId))), 2);
  first = tmp3[0];
  noop = tmp3[1];
  ...
  let set = new Set(selectionActionComponent(8120).getInitialStringSelectOptions(selectionActionComponent, containerId));
  const tmp9 = memo;
  ...
  return tmp9(onSubmit(11827), obj2);
};
