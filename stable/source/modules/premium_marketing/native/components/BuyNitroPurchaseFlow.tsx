...
// Dependencies: [5, 19, 7424, 7421, 1074, 1085, 21, 10708, 7406, 4218, 10668, 10813, 13395, 7405, 4905, 1114, 7409, 1242, 10709, 4884, 9371, 1115, 10809, 2]
// Exports: default

// Module 13394 (BuyNitroPurchaseFlow)
  ...
  const effect = applicationId.useEffect(() => {
    analyticsLoadId = function _runPurchase() {
      ...
      const tmp = asyncGeneratorStep(async () => {
        ...
        if (closure_1_14) {
          ...
          if (id != null) {
            ...
            if (checkoutContextRecord != null) {
              const availablePlanForItems = checkoutContextRecord.getAvailablePlanForItems(sku_id(7409).getSubscriptionItemsForProduct(_null.productId));
        ...
        basePurchaseFlowAnalyticsFields(1242).track(constants.PAYMENT_FLOW_STEP, sku_id(10668).getPaymentFlowStepAnalyticsFields(basePurchaseFlowAnalyticsFields, { from_step: sku_id(10668).PaymentFlowStep.PLAN_SELECT, to_step: sku_id(10668).PaymentFlowStep.EXTERNAL_PAYMENT, subscription_plan_gateway_plan_id: _null.productId, sku_id }));
        ...
        if (1 === tmp7) {
          ...
          if (closure_3 instanceof basePurchaseFlowAnalyticsFields(10709)) {
            const subscriptions = sku_id(4884).fetchSubscriptions();
            sku_id(4884);
            ...
            basePurchaseFlowAnalyticsFields(4905).show(obj14);
            basePurchaseFlowAnalyticsFields(4905);
          }
          closure_1_7();
          c5 = 3;
        } else if (arg0 === 1) {
...
export default function BuyNitroPurchaseFlow(item) {
  ...
  const NitroACOMSubscriptionExperiment = item(9371).NitroACOMSubscriptionExperiment;
  if (obj.isIOS()) {
