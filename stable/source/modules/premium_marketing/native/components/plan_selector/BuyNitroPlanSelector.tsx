...
// Dependencies: [19, 17, 7237, 4542, 1085, 21, 4560, 576, 5441, 504, 13360, 13408, 13409, 13396, 1114, 13410, 5685, 5688, 13304, 7234, 1115, 2]
// Exports: default

// Module 13407 (BuyNitroPlanSelector)
...
obj2.hrkWarning = { borderRadius: nativeDefault.modules.mobile.TABLE_ROW_BORDER_RADIUS, backgroundColor: fn(5441).DARK_PRIMARY_630_LIGHT_PRIMARY_230, marginTop: nativeDefault.space.PX_8, overflow: "hidden" };
...
export default function BuyNitroPlanSelector(planSelection) {
  ...
  const isBuyNitroPurchaseBlocked = planSelection(13360).useIsBuyNitroPurchaseBlocked();
  const obj2 = planSelection(13360);
  closure_4 = planSelection(13408).useBuyNitroPlanLabelRenderer();
  const obj3 = planSelection(13408);
  closure_5 = planSelection(13409).useBuyNitroPlanSubLabelRenderer();
  const obj5 = { style: tmp.header, children: null };
  const obj6 = { title: null };
  const obj4 = planSelection(13409);
  const tmp6 = closure_8;
  const tmp9 = selection;
  const intl = planSelection(1114).intl;
  obj6.title = intl.string(planSelection(1114).t.u95Dt4);
  const items2 = [closure_7(selection(13396), obj6), closure_7(selection(13410), { selectedTier: selection.tier, setSelectedTier: selection.setTier, disabled: isBuyNitroPurchaseBlocked })];
  ...
  if (tmp8Result) {
    ...
    tmp8Result = tmp8(tmp2(5685).TableRadioGroup, obj9);
  }
