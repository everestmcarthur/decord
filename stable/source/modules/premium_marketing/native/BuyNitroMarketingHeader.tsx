...
// Dependencies: [19, 17, 7432, 21, 576, 4560, 4296, 4563, 4561, 4495, 13400, 1115, 9487, 13398, 1093, 5587, 7434, 9396, 4987, 1114, 13401, 4556, 8098, 12290, 2]
// Exports: default
...
let closure_11 = createStyles.createStyles((arg0, arg1, arg2) => {
  ...
  const rect = { position: "absolute", top: -tmp(576).space.PX_16, right: "call", left: "useStateFromStores", alignSelf: 0. };
  obj2.clouds = rect;
...
export default function BuyNitroMarketingHeader(arg0) {
  ...
  const effect = noop.useEffect(() => {
    ...
    if (!accessibilityPreferencesSharedValue.get().reduceMotion) {
      ...
      const result = sharedValue.set(tmpResult.withRepeat(tmp(4561).withTiming(PX_8, { duration: 3400 }), -1, true));
      const tmpResult6 = tmp(4561);
      const tmpResult7 = tmp(4296);
      const result1 = sharedValue1.set(tmpResult7.withRepeat(tmp(4561).withTiming(-PX_12, { duration: 2600 }), -1, true));
      const tmpResult8 = tmp(4561);
      const tmpResult9 = tmp(4296);
      const result2 = sharedValue2.set(tmpResult9.withRepeat(tmp(4561).withTiming(0.6, { duration: 2800 }), -1, true));
      const tmpResult10 = tmp(4561);
    }
  }, items);
