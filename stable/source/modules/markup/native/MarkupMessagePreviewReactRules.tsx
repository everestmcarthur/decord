...
// Dependencies: [19, 10123, 21, 4555, 8097, 10126, 6620, 4996, 4998, 10131, 5587, 1116, 10132, 10133, 1178, 10134, 2]
// Exports: default
...
function createMessagePreviewReactRules(customEmojiSize) {
  ...
  obj[num(4996).AST_KEY.SPOILER] = {
    react(node, output, muted) {
      ...
      obj.children = num(8097).smartOutput(node, output, muted);
  ...
  obj[num(4996).AST_KEY.STATIC_ROUTE_LINK] = {
    react(channelId, output, state) {
      ...
      if (obj.isStaticRouteIconType(channelId.channelId)) {
        smartOutputResult = num(8097).smartOutput(channelId, output, state);
        const tmpResult = num(8097);
      }
  ...
  obj[num(4996).AST_KEY.INLINE_CODE] = obj13;
  const obj14 = {};
  ...
  obj17.react = function react(node, output, key) {
    const obj = { children: null };
    const items = ["\u{1F4CE} ", num(8097).smartOutput(node, output, key)];
    obj.children = items;
    return closure_1_5(num(1178).LegacyText, obj, key.key);
  };
  obj[num(4996).AST_KEY.ATTACHMENT_LINK] = obj17;
  const obj18 = {};
  const merged15 = Object.assign(MarkupRulesDefault.RULES[num(undefined, 4996).AST_KEY.SOUNDBOARD]);
  obj18.react = defaultReactFn;
  obj[num(4996).AST_KEY.SOUNDBOARD] = obj18;
  obj[num(4996).AST_KEY.GUILD] = { react: defaultReactFn };
  const obj19 = { react: num(10133).createInlineChannelReact("\u{1F4AC}") };
  obj[num(4996).AST_KEY.CHANNEL] = obj19;
  const obj21 = {};
  ...
  obj21.react = function react(node, output, key) {
    const obj = { children: null };
    const items = ["/", num(8097).smartOutput(node, output, key)];
    obj.children = items;
    return closure_1_5(num(1178).LegacyText, obj, key.key);
  };
  obj[num(4996).AST_KEY.COMMAND_MENTION] = obj21;
  const obj22 = {};
  ...
  obj[num(4996).AST_KEY.TIMESTAMP] = obj22;
  const obj23 = {};
  ...
  obj23.react = function react(arg0, output, state) {
    ...
    let smartOutputResult = null;
    if (null != first1) {
      smartOutputResult = num(8097).smartOutput(first1, output, state);
      const obj = num(8097);
    }
    return smartOutputResult;
  };
  obj[num(4996).AST_KEY.LIST] = obj23;
