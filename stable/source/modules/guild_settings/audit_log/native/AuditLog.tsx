...
// Function ID: 17559
// Name: AuditLog
// Dependencies: [19, 17, 1183, 1385, 4209, 1371, 1074, 21, 4560, 576, 1399, 1396, 5283, 4411, 7168, 4556, 1115, 4271, 17541, 1114, 4404, 1178, 1971, 4713, 4705, 4153, 1369, 1091, 10632, 7130, 5607, 17550, 5123, 4527, 8180, 14605, 504, 2]
          ...
          if (log.targetType === constants.GUILD) {
            if (typeof target === "object") {
              if (obj6.isGuildRecord(tmp5)) {
                ...
                tmp8Result = closure_2_14(tmp15(4556).Text, obj5, `target${tmp5.id}`);
              }
  ...
  if (log.action !== constants2.AUTO_MODERATION_BLOCK_MESSAGE) {
    if (log.action !== tmp21.AUTO_MODERATION_FLAG_TO_CHANNEL) {
      if (log.action !== tmp21.AUTO_MODERATION_USER_COMMUNICATION_DISABLED) {
        if (log.action !== tmp21.AUTO_MODERATION_QUARANTINE_USER) {
          if (null != log.options.integration_type) {
            ...
            if (null != value) {
              ...
              const tmp9Result = tmp9(4411);
              const tmp25 = tmp9(4411).isThemeDark(theme) ? icon.darkPNG : icon.lightPNG;
        ...
        items2[1] = tmp17(tmp10(5123).PressableOpacity, obj7);
        ...
        items2[2] = tmp15(tmp10(5123).PressableOpacity, obj9);
        ...
        return tmp15(tmp10(5607).Card, obj4);
      }
    }
  }
