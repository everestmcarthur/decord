...
function Header(arg0) {
  ...
  if (null != application2) {
    ...
    const obj4 = { source: tmp3Result.makeSource(tmp3(4411).isThemeDark(tmp2) ? icon.darkPNG : icon.lightPNG), disableColor: true };
    tmp8Result = tmp8(tmp3(1178).Icon, obj4);
    const tmp3Result2 = tmp3(4411);
  }
...
function NumericalConfigRule(existingPendingConfiguration) {
  ...
  const realizedOperatorForResult = metadataField(17640).realizedOperatorFor(existingPendingConfiguration.operator);
  ...
  const tmpResult = metadataField(17640);
  str1 = metadataField(17640).displayedValueFor(value, realizedOperatorForResult).toString();
  let mapped = noop;
  ...
  if (undefined !== fieldTextHook) {
    ...
    if (undefined !== fieldText) {
      ...
      items1[1] = onInputValueChange(tmp(4556).Text, obj7);
    ...
    return onInputValueChange(tmp(7201).TableSwitchRow, obj8, metadataField);
  }
}
