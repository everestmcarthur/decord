...
// Dependencies: [5, 32, 19, 17, 1371, 11851, 1074, 21, 4560, 576, 4262, 1483, 504, 38, 9069, 11849, 4258, 4712, 5584, 4556, 11853, 1178, 1114, 4404, 5687, 5604, 4975, 2]
// Exports: default
...
export default function TransferOwnership(guild) {
          ...
          if (arg0 === 1) {
            ...
            if (!mfaEnabled.mfaEnabled) {
              if (null != tmp27.email) {
                v1 = 1;
                dependencyMap = 1;
                const obj6 = { value: v1(9069).sendTransferOwnershipPincode(user.id), done: false };
                return obj6;
              }
            }
            let MFA = null;
            ...
            const obj4 = v1(9069);
            v1(9069).transferOwnership(user.id, id.id, MFA).then(() => {
              v1(11849).close();
              const obj = v1(11849);
              v1(9069).close();
              const obj2 = v1(9069);
            ...
            dependencyMap = 3;
            const transferOwnershipResult = v1(9069).transferOwnership(user.id, id.id, MFA);
          }
        } else if (arg0 === 1) {
          dependencyMap = 3;
          throw value;
  ...
  const token = guild(4262).useToken(toUser(576).modules.mobile.TABLE_ROW_PADDING);
  let tmp5 = closure_16();
  let obj = guild(4262);
  ...
  nickname = toUser(4712).getNickname(guild.id, undefined, toUser);
  ...
  const items1 = [closure_13(c7, { source: toUser(11853), style: tmp5.arrow }), , , ];
  const obj10 = { style: tmp5.avatarsWrapper, children: null };
  const obj9 = { source: toUser(11853), style: tmp5.arrow };
  ...
  if (null == nickname) {
    if (!hasAvatarForGuildResult) {
      ...
      const obj15 = { GuildHook: GuildWithSmallIcon, user: tmp3(4404).getUserTag(toUser) };
    ...
    const obj19 = { username: tmp3(4404).getUserTag(toUser) };
    obj18.label = intl3.format(tmp(1114).t.xm6ACJ, obj19);
    obj18.onPress = function handleConfirmToggle(arg0) {
      _undefined(arg0);
    };
    obj17.children = tmp11(tmp(5604).TableCheckboxRow, obj18);
    items3[1] = tmp11(tmp(5687).TableRowGroup, obj17);
    const obj20 = {
    ...
    obj20.disabled = !tmp8;
    items3[2] = tmp11(tmp(4975).Button, obj20);
  ...
  obj21.user = toUser(4404).getUserTag(toUser);
