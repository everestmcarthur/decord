...
// Dependencies: [19, 17, 1074, 1373, 21, 4560, 576, 8852, 5587, 13162, 4495, 7588, 4411, 10729, 10730, 4556, 1114, 2024, 4255, 7150, 7431, 5123, 11110, 4975, 4527, 7154, 2]
// Exports: default
...
import FractionalNitroCoinIllustration from "FractionalNitroCoinIllustration" /* 8852 */;
...
function NitroAcquiredHeader(skuId) {
  ...
  const obj2 = { source: _modDef13162 };
...
let size = { width: fn(8852).FRACTIONAL_NITRO_COIN_SIZE.COLLECTED_SHEET, height: fn(8852).FRACTIONAL_NITRO_COIN_SIZE.COLLECTED_SHEET, position: "absolute", top: "50%", left: "50%", transform: null };
let obj5 = { flex: 1, gap: nativeDefault.space.PX_16, alignSelf: "stretch" };
let items = [{ translateX: -fn(8852).FRACTIONAL_NITRO_COIN_SIZE.COLLECTED_SHEET / 2 }, ];
const obj6 = { translateX: -fn(8852).FRACTIONAL_NITRO_COIN_SIZE.COLLECTED_SHEET / 2 };
items[1] = { translateY: -fn(8852).FRACTIONAL_NITRO_COIN_SIZE.COLLECTED_SHEET / 2 };
...
export default function FractionalNitroCollectedActionSheet(skuId) {
  ...
  noop = tmp4;
  const fetchFractionalPremiumInfo = skuId(7588).useFetchFractionalPremiumInfo();
  ...
  const memo = noop.useMemo(() => {
    if (consumed) {
      ...
      if (obj2.isThemeDark(closure_3)) {
        let tmp8Result = tmp8(10729);
      } else {
        tmp8Result = tmp8(10730);
      }
  ...
  if (consumed) {
    ...
    let tmp12Result = tmp12(tmp2(7431), obj2);
    let tmp15 = tmp12;
  } else {
    let obj3 = { skuId };
  ...
  obj5.children = tmp15(skuId(11110).CircleQuestionIcon, { style: tmp.questionIcon, color: consumed(576).colors.WHITE });
  items2[1] = tmp15(skuId(5123).PressableOpacity, obj5);
  ...
  if (fetchFractionalPremiumInfo.isLoading) {
    ...
    items5[1] = tmp15(tmp5(4975).Button, obj11);
  ...
  return closure_10(skuId(7150).BottomSheet, obj12);
};
