...
// Function ID: 15820
// Name: HeroBlock
// Dependencies: [19, 17, 7542, 1076, 1074, 21, 8770, 4560, 576, 15088, 15820, 1483, 8773, 504, 11489, 4495, 15821, 4262, 4409, 15075, 15077, 7162, 7182, 15822, 1242, 4411, 15824, 15825, 15826, 4987, 4556, 4975, 1114, 11772, 5449, 5123, 7209, 7156, 15827, 8880, 2]
// Exports: default

// Module 15819 (HeroBlock)
...
import FeaturedFirstCardCoachmarkAnchorDefault from "FeaturedFirstCardCoachmarkAnchor" /* 15822 */;
...
const result = 0.75 * fn(8770).COLLECTIBLES_SHOP_CARD_WIDTH;
...
export default function _default(heroBlock) {
  ...
  closure_5 = undefined;
  closure_6 = undefined;
  let obj = heroBlock(15820);
  dependencyMap = heroBlock(1483).useNavigation();
  let obj2 = heroBlock(1483);
  noop = heroBlock(8773).useCollectiblesAnalyticsContext();
  ...
  let isEligibleForQuests = heroBlock(11489).getIsEligibleForQuests();
  const tmpResult9 = heroBlock(11489);
  const tmp7 = preferVCPrice(4495)();
  const tmp8 = closure_14();
  const tmpResult10 = heroBlock(15821);
  const token = heroBlock(4262).useToken(preferVCPrice(576).colors.BACKGROUND_BASE_LOW);
  const tmpResult11 = heroBlock(4262);
  const tmpResult12 = heroBlock(4409);
  const tmpResult13 = heroBlock(4409);
  const hexToRgbaStringResult = tmpResult12.hexToRgbaString(heroBlock(4409).hexWithOpacity(token, 0));
  const token1 = heroBlock(4262).useToken(preferVCPrice(576).colors.BACKGROUND_BASE_LOWEST);
  const tmp12 = preferVCPrice(15075)();
  closure_5 = tmp12;
  const items1 = [heroBlock.rankedSkuIds, tmp12];
  const memo = noop.useMemo(() => closure_5(heroBlock.rankedSkuIds), items1);
  const tmpResult14 = heroBlock(4262);
  const filteredAndSortedProducts = heroBlock(15077).useFilteredAndSortedProducts({ products: memo, bypassAndroidUnsyncedFilter: tmp4 });
  closure_6 = tmp14;
  const tmpResult15 = heroBlock(15077);
  let unpublishedAt;
  ...
  if (undefined === stateFromStores) {
    ...
    if (!tmp4) {
      const obj4 = { value: tmp15(preferVCPrice(7182).COLLECTIBLES_SHOP_HERO).analyticsLocations, children: null };
      ...
      if (tmp24Result) {
        ...
        if (tmp4) {
          ...
          tmp22Result = tmp22(tmp6(4987), obj8);
        }
        ...
        items3[2] = tmp22(tmp6(4987), obj12);
      ...
      if (tmp4) {
        ...
        if (tmp22Result6) {
          ...
          tmp22Result6 = tmp22(tmp(4556).Text, obj15);
        }
        ...
        if (tmp22Result7) {
          ...
          tmp22Result7 = tmp22(tmp(4556).Text, obj16);
        }
        ...
        if (isEligibleForQuests) {
          ...
          isEligibleForQuests = tmp22(tmp(4975).Button, obj19);
        }
        ...
        const obj28 = { style: tmp8.heroViewAllIcon, children: tmp22(tmp(7209).ChevronSmallRightIcon, { size: "sm", color: "white" }) };
        items9[1] = tmp22(tmp25, obj28);
        obj24.children = items9;
        obj20.children = tmp24(tmp25, obj24);
        tmp24Result2 = tmp22(tmp(5123).PressableOpacity, obj20, stateFromStores.storeListingId);
      }
      ...
      if (tmp4) {
        ...
        let tmp22Result9 = tmp22(tmp6(15827), obj30);
        const tmp6Result3 = tmp6(15827);
      } else {
        if (0 === filteredAndSortedProducts.length) {
          ...
          obj34.snapToInterval = tmp(8770).COLLECTIBLES_SHOP_CARD_WIDTH + tmp6(576).space.PX_12;
          ...
          tmp22Result10 = tmp22(tmp(8880).FlashList, obj34);
        }
        const obj36 = { children: tmp22Result10 };
        tmp22Result9 = tmp22(closure_12, obj36);
      }
      const obj37 = { children: tmp22Result9 };
      obj29.children = closure_11(tmp(7156).LayerScope, obj37);
      items6[2] = closure_11(closure_5, obj29);
      obj6.children = items6;
      obj5.children = closure_13(closure_5, obj6);
      obj4.children = closure_11(tmp6(15826), obj5);
      return closure_11(tmp(7162).AnalyticsLocationProvider, obj4);
    } else {
      if (tmpResult16.isThemeDark(tmp7)) {
        let tmp6Result4 = tmp6(15824);
      } else {
        tmp6Result4 = tmp6(15825);
      }
      tmpResult16 = tmp(4411);
    }
  }
};
