...
// Dependencies: [19, 17, 1076, 8804, 21, 4560, 576, 8227, 5587, 11016, 8202, 8828, 8172, 8803, 1886, 8824, 1889, 1077, 8852, 8851, 8816, 1114, 4556, 7553, 8854, 11017, 8179, 4218, 8870, 2]
// Exports: default

// Module 11015 (CollectiblesShopCheckoutDetails)
...
import useMaybeFetchProfileFrameDefault from "useMaybeFetchProfileFrame" /* 8202 */;
import useProfileEffectDefault from "useProfileEffect" /* 8227 */;
import BundleSampleV2Default from "BundleSampleV2" /* 8803 */;
import AvatarDecorationSampleV2Default from "AvatarDecorationSampleV2" /* 8816 */;
import NameplateDefault from "Nameplate" /* 8824 */;
...
function CollectibleProductPreviewContent(userAvatarSource) {
  ...
  if (ALL.has(product.skuId)) {
    const size = { skuId: product.skuId, width: tmp2(8852).FRACTIONAL_NITRO_COIN_SIZE.CHECKOUT, height: tmp2(8852).FRACTIONAL_NITRO_COIN_SIZE.CHECKOUT };
    return React5(tmp2(8852).FractionalNitroCoinIllustration, size);
  } else if (product.skuId === EXTERNAL_PRODUCT_SKU_IDS.ORB_PROFILE_BADGE) {
    const obj = { source: null, style: null };
    const obj2 = { uri: _modDef8851 };
    obj.source = obj2;
...
function ProductPriceAmountTag(product) {
  ...
  const memo = noop.useMemo(() => {
    if (useOrbPrice) {
      ...
      const orbPrice = tmp(7553).getProductOrbPrice(obj);
      const tmpResult = tmp(7553);
    } else {
      const priceText = tmp(8854).getFormattedPriceForCollectiblesProduct(product, hasShopDiscount, true);
      const tmpResult2 = tmp(8854);
    }
...
const CollectiblesPreviewConstants = fn(8804);
...
export default function CollectiblesShopCheckoutDetails(recipientUser) {
  ...
  const canUseShopDiscountsResult = flag2(4218).canUseShopDiscounts(currentUser);
  ...
  const memo = noop.useMemo(() => {
    ...
    if (flag2) {
      ...
      const productOrbPrice = tmp(7553).getProductOrbPrice(obj2);
      let str2;
      ...
      let str = intl.formatToPlainString(tmp(1114).t.W4DfeF, obj3);
      const tmpResult = tmp(7553);
    } else {
      str = tmp(8854).getFormattedPriceForCollectiblesProduct(tmp3, hasShopDiscount, true);
      if (str == null) {
        str = "";
      }
      const tmpResult2 = tmp(8854);
  ...
  if (flag3) {
    ...
    obj5.children = tmp10(tmp3(4556).Text, obj6);
