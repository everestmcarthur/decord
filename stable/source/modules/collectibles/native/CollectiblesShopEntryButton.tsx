...
function CoachmarkVariant(shopButtonRef) {
  ...
  const tmp = _slicedToArray(navigateToShop(7388).useSelectedVersionedDismissibleContent(navigateToShop(1943).DismissibleContent.COLLECTIBLES_SHOP_ENTRY_MARKETING, marketing.version, undefined, true), 2);
  importDefault = tmp2;
  ...
  const obj = navigateToShop(7388);
  obj3.IconComponent = navigateToShop(12142).ShopIcon;
...
export default function CollectiblesShopEntryButton(navigateToShop) {
  ...
  const stateFromStores = navigateToShop(563).useStateFromStores(items, () => marketingBySurface.getMarketingBySurface(navigateToShop(13989).CollectiblesMarketingSurface.MOBILE_SHOP_BUTTON));
  ...
  if (type === navigateToShop(7566).CollectiblesMarketingType.COACHMARK) {
    ...
    if (tmp4) {
      ...
      let tmp15Result = tmp15(tmp(10630).SelectedVersionedDismissibleContent, obj3);
    } else {
      let type2;
      if (stateFromStores != null) {
