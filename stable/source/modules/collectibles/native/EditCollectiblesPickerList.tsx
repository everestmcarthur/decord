...
// Dependencies: [32, 19, 17, 21, 4560, 13174, 4556, 12, 8880, 2]
...
let obj = { list: { flex: 1, marginTop: 12 }, listContent: { paddingBottom: 88 }, loadingContainer: { paddingVertical: 80, alignItems: "center" }, header: { paddingHorizontal: fn(13174).GUTTER_SIZE, paddingTop: 10, paddingBottom: 5 } };
  ...
  const memo = obj.useMemo(() => {
    ...
    if (flag) {
      ...
      let item = sections.forEach((header) => {
        ...
        const obj2 = items(12);
        const item = items(12).chunk(header.items, items(13174).ROW_SIZE).forEach((items, index) => {
