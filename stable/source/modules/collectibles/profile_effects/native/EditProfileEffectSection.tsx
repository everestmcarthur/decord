...
// Dependencies: [19, 17, 7548, 8804, 21, 4560, 13174, 14631, 13175, 7182, 8227, 5587, 8829, 8807, 2]
...
let obj = { row: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingHorizontal: fn(13174).GUTTER_SIZE }, rowSpacer: null, profileEffect: null, sampleProfile: null };
let obj3 = { flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingHorizontal: fn(13174).GUTTER_SIZE };
obj.rowSpacer = { height: fn(13174).GUTTER_SIZE };
obj.profileEffect = { overflow: "hidden", width: "100%", height: "100%" };
obj.sampleProfile = { aspectRatio: fn(8804).SAMPLE_PROFILE_ASPECT_RATIO, width: "100%" };
...
const memoResult = noop.memo((size) => {
  ...
  obj2.children = substr.map((item, index) => {
    if (item === useProfileEffectSections.NONE_ITEM) {
      ...
      return timestampProducer(tmp(13175).EditCollectiblesListItemNone, obj2, "none");
    } else if (item === tmp(14631).SHOP_ITEM) {
      const obj3 = { size, analyticsSource: AnalyticsLocationDefault.EDIT_PROFILE_EFFECT_SHEET };
      return timestampProducer(tmp(13175).EditCollectiblesListItemShop, obj3, "shop");
