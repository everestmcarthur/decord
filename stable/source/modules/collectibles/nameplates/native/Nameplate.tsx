...
// Function ID: 8825
// Name: Nameplate
// Dependencies: [19, 4552, 21, 4560, 504, 4495, 4296, 4561, 1886, 8825, 4987, 1115, 8814, 5587, 2]
// Exports: default

// Module 8824 (Nameplate)
...
function NameplateInner(isFocused) {
  ...
  sharedValue = flag6(4296).useSharedValue(num);
  ...
  const animatedStyle = flag6(4296).useAnimatedStyle(A);
  const tmpResult = flag6(4296);
  const backgroundGradientColors = flag6(1886).getBackgroundGradientColors(nameplate.palette, tmp6);
  const tmpResult4 = flag6(1886);
  const nameplateAssets = flag6(8825).getNameplateAssets(nameplate);
  ...
  if (null != backgroundGradientColors) {
    ...
    obj4.colors = items3;
    tmp15Result = closure_5(tmp5(4987), obj4);
    const tmp5Result = tmp5(4987);
  }
  const items4 = [tmp15Result, ];
  const tmp13 = closure_6;
  const tmpResult5 = flag6(8825);
  if (tmpResult6.isAndroid()) {
    if (tmp12) {
      const obj5 = { url: str, style: tmp4.img, autoplay: true };
      let tmp17 = closure_5(tmp(8814).APNGPlayer, obj5);
    }
    items4[1] = tmp17;
    obj3.children = items4;
    return tmp13(tmp5(4296).View, obj3);
  }
