...
// Dependencies: [5, 32, 19, 17, 2011, 4193, 1074, 1964, 21, 15230, 504, 9372, 4556, 4905, 1114, 5569, 4153, 1178, 16544, 4527, 16551, 1896, 16540, 5057, 4975, 10306, 2]
// Exports: default
...
function SwitchTiersButton(activeSubscription) {
  ...
  const obj2 = activeListingId(4153)(activeSubscription.currentPeriodEnd);
  const obj4 = { variant: "text-xs/normal", color: "text-muted", children: null };
  const intl = activeSubscription(1114).intl;
  const formatResult = activeListingId(4153)(activeSubscription.currentPeriodEnd).format("MMMM Do");
  obj4.children = intl.format(activeSubscription(1114).t.lA7ztO, { activeListingName: _slicedToArray(obj.useName(activeListingId), 1)[0], billingEndDate: activeListingId(4153)(activeSubscription.currentPeriodEnd).format("MMMM Do"), emphasisHook });
  const items = [closure_12(activeSubscription(4556).Text, obj4), closure_12(activeSubscription(1178).Spacer, { size: 16 }), ];
  const obj6 = { text: null, onPress: null };
  const intl2 = activeSubscription(1114).intl;
  obj6.text = intl2.string(activeSubscription(1114).t.SACegK);
  obj6.onPress = function onPress() {
    const obj = ActionSheetActionCreatorsDefault;
    obj.openLazy(asyncRequireImpl(16551, dependencyMap.paths), "ChangeSubscriptionCard:" + changeToListingId, { activeSubscription, activeListingId, changeToListingId });
  };
  items[2] = closure_12(activeSubscription(16544).ArrowButton, obj6);
  obj3.children = items;
