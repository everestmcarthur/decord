...
// Module 16548 (GuildRoleSubscriptionPurchasePreviewCard)
...
function ChannelBenefitShowCase(description) {
  ...
  if (null != stateFromStores) {
    ...
    const obj3 = { size: tmp(1178).Icon.Sizes.REFRESH_SMALL_16, source: tmp(5028).getChannelIcon(stateFromStores) };
    ...
    items2[2] = closure_8(tmp(4556).Text, obj4);
...
export default function GuildRoleSubscriptionPurchasePreviewCard(listingId) {
  ...
  const formattedSubscriptionPlan = listingId(16544).useFormattedSubscriptionPlan(listingId);
  const obj6 = listingId(16544);
  const tmp11 = guildId;
  if (str == null) {
    str = "";
  }
  const items = [closure_8(guildId(5587), { source: { uri: str }, style: tmp.image }), closure_8(listingId(1178).Spacer, { size: 16 }), ];
  const obj10 = { children: null };
  const items1 = [closure_8(listingId(4556).Text, { variant: "heading-md/semibold", color: "mobile-text-heading-primary", children: _slicedToArray(obj2.useName(listingId), 1)[0] }), closure_8(listingId(1178).Spacer, { size: 4 }), closure_8(listingId(4556).Text, { variant: "heading-md/medium", color: "text-default", children: formattedSubscriptionPlan })];
  obj10.children = items1;
  items[2] = closure_9(closure_6, obj10);
  obj8.children = items;
  const items2 = [closure_9(closure_6, obj8), closure_8(listingId(1178).Spacer, { size: 16 }), closure_8(tmp11(16550), { listingId }), ];
  ...
  if (tmp8Result6) {
    ...
    obj11.children = tmp8(tmp4(10351).GappedList, obj12);
    items3[1] = tmp10(tmp9, obj11);
    const obj45 = {
      onPress() {
          const obj = ActionSheetActionCreatorsDefault;
          obj.openLazy(asyncRequireImpl(16549, dependencyMap.paths), "PurchaseCard:" + listingId, { listingId, guildId });
        }
    };
