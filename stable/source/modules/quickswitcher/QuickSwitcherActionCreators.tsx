...
// Dependencies: [5, 32, 5443, 1961, 1957, 7397, 2011, 4381, 9840, 1074, 1964, 1076, 9841, 1242, 8377, 573, 7342, 4571, 4761, 5411, 4573, 10992, 8686, 8692, 7244, 10993, 7541, 7182, 10994, 8379, 2]
...
import ValidationUtilsDefault from "ValidationUtils" /* 8377 */;
...
function getQuickSwitcherOptions(str) {
  ...
  if (charAtResult === sortByMatchScore.AutocompleterQuerySymbols.USER) {
    if (charAtResult1 === tmp2(9841).AutocompleterQuerySymbols.USER) {
      const items = [str.slice(2), tmp2(9841).AutocompleterResultTypes.USER_GLOBAL];
      let items1 = items;
    }
...
function trackClose(QUICKSWITCHER_RESULT_SELECTED, type) {
  ...
  if (null == tmp6) {
    ...
    obj6.num_results_users = obj.getResultTotals(tmp4(9841).AutocompleterResultTypes.USER);
    obj6.num_results_text_channels = obj.getResultTotals(tmp4(9841).AutocompleterResultTypes.TEXT_CHANNEL);
    obj6.num_results_voice_channels = obj.getResultTotals(tmp4(9841).AutocompleterResultTypes.VOICE_CHANNEL);
    obj6.num_results_guilds = obj.getResultTotals(tmp4(9841).AutocompleterResultTypes.GUILD);
    obj6.num_results_group_dms = obj.getResultTotals(tmp4(9841).AutocompleterResultTypes.GROUP_DM);
    if (null != channelId) {
    ...
    if (null != type) {
      ...
      if (null == type) {
        ...
        if (tmp4(9841).AutocompleterResultTypes.GUILD === type3) {
          obj6.selected_guild_id = record.id;
        } else {
          if (tmp4(9841).AutocompleterResultTypes.TEXT_CHANNEL !== type3) {
            if (tmp4(9841).AutocompleterResultTypes.VOICE_CHANNEL !== type3) {
              if (tmp4(9841).AutocompleterResultTypes.GROUP_DM === type3) {
                obj6.selected_channel_id = record.id;
              } else if (tmp4(9841).AutocompleterResultTypes.USER === type3) {
      ...
      } else if (type.type === tmp4(9841).AutocompleterResultTypes.IN_APP_NAVIGATION) {
  ...
  } else if (tmp6.type === tmp4(9841).AutocompleterResultTypes.IN_APP_NAVIGATION) {
...
function switchToResult(record) {
  ...
  if (sortByMatchScore.AutocompleterResultTypes.GUILD === type) {
    tmp5(7342).transitionToGuild(record.id, { navigationReplace: true });
    const tmp5Result = tmp5(7342);
  } else if (tmp5(9841).AutocompleterResultTypes.TEXT_CHANNEL === type) {
    ...
    if (null != channel) {
      ...
      obj3.state = obj4;
      tmp5(4571).transitionToChannel(channel.id, obj3);
      const tmp5Result6 = tmp5(4571);
    }
  } else if (tmp5(9841).AutocompleterResultTypes.VOICE_CHANNEL === type) {
    const channel1 = ChannelStore.getChannel(record.id);
    if (null != channel1) {
      if (flag) {
        tmp(4761).updateChatOpen(record.id, true);
        const tmpResult = tmp(4761);
      } else {
        const voiceChannel = tmp(5411).selectVoiceChannel(record.id);
        const tmpResult7 = tmp(5411);
      }
      const obj5 = { state: null, navigationReplace: true };
      const obj6 = { analyticsSource: obj2 };
      obj5.state = obj6;
      tmp5(4571).transitionToChannel(channel1.id, obj5);
      const tmp5Result7 = tmp5(4571);
    }
  } else if (tmp5(9841).AutocompleterResultTypes.USER === type) {
    const obj7 = { recipientIds: null, location: "Quickswitcher" };
    const items = [record.id];
    obj7.recipientIds = items;
    tmp(4573).openPrivateChannel(obj7);
    const tmpResult8 = tmp(4573);
    tmp(10992).channelListScrollTo(closure_1_12, ChannelStore.getDMFromUserId(record.id));
    const tmpResult9 = tmp(10992);
  } else if (tmp5(9841).AutocompleterResultTypes.GROUP_DM === type) {
    tmp5(4571).transitionToChannel(record.id, { navigationReplace: true });
    const tmp5Result8 = tmp5(4571);
    tmp(10992).channelListScrollTo(closure_1_12, record.id);
    const tmpResult10 = tmp(10992);
  } else if (tmp5(9841).AutocompleterResultTypes.APPLICATION === type) {
  ...
  } else if (tmp5(9841).AutocompleterResultTypes.GAME_PROFILE === type) {
    ...
    obj8.source = tmp5(8692).GameProfileSources.QuickSwitcher;
    tmp(8686).openGameProfileModal(obj8);
    const tmpResult11 = tmp(8686);
  } else if (tmp5(9841).AutocompleterResultTypes.LINK === type) {
  ...
  } else if (tmp5(9841).AutocompleterResultTypes.IN_APP_NAVIGATION === type) {
    if (record.record.type !== InAppNavigationType.SETTINGS) {
      if (record.record.type === tmp37.PLAYGROUND) {
        const tmp5Result9 = tmp5(10993);
        ...
        obj10.analyticsSource = tmp(7182).QUICK_SWITCHER;
        tmp5(7541).openCollectiblesShop(obj10);
        const tmp5Result10 = tmp5(7541);
      } else {
        tmp(7244)(record.path, { navigationReplace: true });
      }
    }
...
let obj = {};
obj[fn(9841).AutocompleterQuerySymbols.USER] = fn(9841).AutocompleterResultTypes.USER;
obj[fn(9841).AutocompleterQuerySymbols.TEXT_CHANNEL] = fn(9841).AutocompleterResultTypes.TEXT_CHANNEL;
obj[fn(9841).AutocompleterQuerySymbols.VOICE_CHANNEL] = fn(9841).AutocompleterResultTypes.VOICE_CHANNEL;
obj[fn(9841).AutocompleterQuerySymbols.GUILD] = fn(9841).AutocompleterResultTypes.GUILD;
obj[fn(9841).AutocompleterQuerySymbols.GAME_PROFILE] = fn(9841).AutocompleterResultTypes.GAME_PROFILE;
let closure_19 = Object.freeze(obj);
const regExp = new RegExp("^" + fn(9841).AutocompleterQuerySymbols.USER + "|" + fn(9841).AutocompleterQuerySymbols.TEXT_CHANNEL + "|" + fn(9841).AutocompleterQuerySymbols.VOICE_CHANNEL + "|\\" + fn(9841).AutocompleterQuerySymbols.GUILD + "|\\" + fn(9841).AutocompleterQuerySymbols.GAME_PROFILE);
const size = fn(2);
...
export const switchToResultInNewTab = function switchToResultInNewTab(type) {
  ...
  if (require("sortByMatchScore").AutocompleterResultTypes.TEXT_CHANNEL !== type) {
    if (tmp(9841).AutocompleterResultTypes.VOICE_CHANNEL !== type) {
      if (tmp(9841).AutocompleterResultTypes.GROUP_DM !== type) {
        if (tmp(9841).AutocompleterResultTypes.DM !== type) {
          if (tmp(9841).AutocompleterResultTypes.USER === type) {
            DispatcherDefault.dispatch({ type: "QUICKSWITCHER_HIDE" });
                ...
                try {
                  ...
                  if (0 === dependencyMap) {
                    if (arg0 === 1) {
                      ...
                      dependencyMap = 1;
                      c3 = 1;
                      const obj6 = { value: tmp2(4573).openPrivateChannel(obj5), done: false };
                      return obj6;
                    }
                  } else if (arg0 === 1) {
                    c3 = 3;
                    ...
                    closure_128_0 = value;
                    tmp5(10994).openChannelTabActive(closure_128_0, null);
                    c3 = 3;
                    return { value: "HermesInternal", done: null };
                  }
                } catch (tmp16) {
