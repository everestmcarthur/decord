...
// Dependencies: [5108, 5506, 1183, 502, 1957, 4901, 2012, 2021, 1979, 4199, 4575, 2011, 4381, 4741, 1074, 9841, 12, 9850, 5442, 1114, 9851, 1369, 510, 504, 573, 2]

// Module 9840 (QuickSwitcherStore)
import _modDef12 from "module_12" /* 12 */;
import initializeDefault from "initialize" /* 504 */;
import Storage2 from "Storage" /* 510 */;
import DispatcherDefault from "Dispatcher" /* 573 */;
import sortByMatchScore from "sortByMatchScore" /* 9841 */;
import createAutocompleterResultForChannelIdDefault from "createAutocompleterResultForChannelId" /* 9850 */;
import ReadStateUtils from "ReadStateUtils" /* 9851 */;
...
function generateResultFromId(found) {
  ...
  if (null == tmp2) {
    ...
    if (tmp3) {
      if (tmp2.type !== tmp7(9841).AutocompleterResultTypes.USER) {
        return null;
      }
          ...
          let item = values.forEach((item) => {
            for (const key10004 in arg0) {
              ...
              if (null != tmp14) {
                ...
                if (!tmp3) {
                  tmp3 = tmp === tmp2(9841).AutocompleterResultTypes.USER_GLOBAL;
                }
                if (tmp3) {
                  tmp5 = tmp14;
                  if (tmp14.type !== tmp2(9841).AutocompleterResultTypes.USER) {
                    tmp5 = null;
                  }
                } else {
                  tmp5 = tmp14;
...
function handleUserSearchUpdate(arr, str) {
  ...
  if (!flag) {
    ...
    if (sortByMatchScore.AutocompleterResultTypes.USER_GLOBAL !== queryMode) {
      if (DOWN(9841).AutocompleterResultTypes.USER !== tmp7) {
        if (DOWN(9841).AutocompleterResultTypes.TEXT_CHANNEL === tmp7) {
          const intl5 = DOWN(1114).intl;
          arr.unshift(DOWN(9841).createHeaderResult(intl5.string(DOWN(1114).t.W26k4V)));
          let items = arr;
          const DOWNResult = DOWN(9841);
        } else if (DOWN(9841).AutocompleterResultTypes.VOICE_CHANNEL === tmp7) {
          const intl4 = DOWN(1114).intl;
          arr.unshift(DOWN(9841).createHeaderResult(intl4.string(DOWN(1114).t.zUoI5C)));
          items = arr;
          const DOWNResult1 = DOWN(9841);
        } else if (DOWN(9841).AutocompleterResultTypes.GUILD === tmp7) {
          const intl3 = DOWN(1114).intl;
          arr.unshift(DOWN(9841).createHeaderResult(intl3.string(DOWN(1114).t.olADPs)));
          items = arr;
          const DOWNResult2 = DOWN(9841);
        } else if (DOWN(9841).AutocompleterResultTypes.APPLICATION === tmp7) {
          const intl2 = DOWN(1114).intl;
          arr.unshift(DOWN(9841).createHeaderResult(intl2.string(DOWN(1114).t.VwK1ld)));
          items = arr;
          const DOWNResult3 = DOWN(9841);
        } else if (DOWN(9841).AutocompleterResultTypes.GAME_PROFILE === tmp7) {
          const intl = DOWN(1114).intl;
          arr.unshift(DOWN(9841).createHeaderResult(intl.string(DOWN(1114).t.gEp2SG)));
          items = arr;
          const DOWNResult4 = DOWN(9841);
        } else {
        ...
        if (str !== c28) {
          ...
          str = DOWN(9841).findNextSelectedResult;
          DOWN = DOWN(9841).FindResultDirections.DOWN;
          selectedIndex = str(DOWN, -1, arr);
          const DOWNResult5 = DOWN(9841);
        } else {
          let tmp31 = null != tmp29;
          if (tmp31) {
            tmp31 = tmp29.type === DOWN(9841).AutocompleterResultTypes.HEADER;
          }
          if (tmp31) {
            selectedIndex = DOWN(9841).findNextSelectedResult(DOWN(9841).FindResultDirections.DOWN, selectedIndex, arr);
            const DOWNResult6 = DOWN(9841);
          }
        }
    ...
    let guild = null;
    if (queryMode !== DOWN(9841).AutocompleterResultTypes.USER_GLOBAL) {
    ...
    arr.unshift(DOWN(9841).createHeaderResult(formatToPlainStringResult));
    const DOWNResult7 = DOWN(9841);
    length = arr;
  }
}
    ...
    if (null == _null) {
      ...
      if (queryMode !== queryMode) {
        if (null == queryMode) {
          ...
          if (tmp2) {
            const items1 = [tmp31(9841).AutocompleterResultTypes.USER];
        ...
        _null.setLimit(num);
        const guildId = SelectedGuildStore.getGuildId();
        if (queryMode === tmp6(9841).AutocompleterResultTypes.USER) {
          if (null != guildId) {
        ...
        if (queryMode === tmp6(9841).AutocompleterResultTypes.VOICE_CHANNEL) {
