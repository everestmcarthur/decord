...
import VibegrationsClarification from "VibegrationsClarification" /* 16610 */;
...
export default function VibegrationsClarificationCard(clarification) {
  ...
  const callback = noop.useCallback((arg0) => {
    if (null != onSubmit) {
      ...
      if (null == result) {
        const result1 = tmp13(16610).formatClarificationAnswers(tmp15, obj);
        if ("" !== result1) {
          tmp(result1);
        }
        const tmp13Result = tmp13(16610);
      } else {
        closure_6(result);
      }
      tmp13 = require;
  ...
  if (clarification.questions.length > 1) {
    ...
    obj3.children = intl.formatToPlainString(onSubmit(3547)["7bypa+"], obj4);
    tmp14 = closure_6(clarification(4556).Text, obj3);
  }
  ...
  if (bound > 0) {
    ...
    if (!tmp6) {
      ...
      obj7.text = intl2.string(onSubmit(3547).yKdgqw);
      obj7.onPress = callback1;
      tmp19Result = tmp19(tmp20(4975).Button, obj7);
    }
  }
  ...
  obj9.placeholder = intl3.string(onSubmit(3547).qifsdL);
  obj9.value = str;
  ...
  obj8.children = closure_6(clarification(14173).GhostInput, obj9);
