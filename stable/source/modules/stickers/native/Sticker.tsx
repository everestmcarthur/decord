...
// Function ID: 10182
// Name: Sticker
// Dependencies: [19, 17, 1183, 21, 5268, 4899, 1114, 8000, 10182, 5587, 4411, 7131, 7132, 2]
...
import NativeLottieViewDefault from "NativeLottieView" /* 8000 */;
...
export default function Sticker(opaque) {
  ...
  if (sticker.format_type === StickersTypes.StickerFormat.LOTTIE) {
    let str = tmp(4899).getStickerAssetUrl(sticker);
    ...
    const tmpResult6 = tmp(4899);
  } else {
    const obj2 = { isPreview: !animated, size: PixelRatio.getPixelSizeForLayoutSize(size) };
    str = tmp(4899).getStickerAssetUrl(sticker, obj2);
    const tmpResult7 = tmp(4899);
  }
  if (str == null) {
    str = "";
  }
  ...
  if (sticker.format_type === StickersTypes.StickerFormat.LOTTIE) {
    ...
    if (sticker.format_type === tmp(5268).StickerFormat.APNG) {
      ...
      obj4.name = "" + sticker.id + "." + tmp(4899).getStickerExtensionFromFormatType(sticker.format_type);
...
export const getStickerAssetUrl = function getStickerAssetUrl(sticker, AnimateStickers, arg2) {
  if (sticker.format_type === StickersTypes.StickerFormat.LOTTIE) {
    ...
    const obj = { isPreview: !arg2, size: AnimateStickers };
    str = tmp(4899).getStickerAssetUrl(sticker, obj);
    const tmpResult3 = tmp(4899);
  } else {
    const obj2 = { isPreview: !arg2, size: PixelRatio.getPixelSizeForLayoutSize(AnimateStickers) };
    str = tmp(4899).getStickerAssetUrl(sticker, obj2);
    const tmpResult4 = tmp(4899);
  }
  if (str == null) {
    str = "";
  }
