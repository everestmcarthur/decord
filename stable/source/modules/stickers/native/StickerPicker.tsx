...
// Function ID: 10392
// Name: StickerPicker
// Dependencies: [32, 19, 17, 1371, 5502, 1938, 1074, 21, 4560, 576, 1242, 10392, 10394, 504, 7162, 7182, 10288, 1249, 10398, 7337, 4899, 10399, 9319, 7189, 10408, 7050, 1114, 10418, 10419, 10424, 2]

// Module 10391 (StickerPicker)
...
import StickersSearchUtils from "StickersSearchUtils" /* 10398 */;
import openStickerPackDetailActionSheetDefault from "openStickerPackDetailActionSheet" /* 10399 */;
...
export default noop.memo(function StickerPicker(channel) {
  ...
  let tmp = closure_16();
  const fetchStickerPacks = channel(10392).useFetchStickerPacks();
  let obj = channel(10392);
  const stickerCategories = channel(10394).useStickerCategories(channel);
  let obj2 = channel(10394);
  ...
  ({ safeAreaStyle, safeAreaBottomKeyboardAware } = onPressSticker(10288)({ hasCategories: true }));
  const items1 = [, ];
    ...
    if (stickerSendability === StickerSendability.StickerSendability.SENDABLE) {
      ...
      if (tmpResult.isStandardSticker(pack_id)) {
        ...
        if (null != stickerPack) {
          ...
          obj2.analyticsPopoutType = tmp(10399).AnalyticsPopoutType.STICKER_PACK_DETAIL;
  ...
  const tmp12 = onPressSticker(10288)({ hasCategories: true });
  ...
  if (0 !== stickerCategories.length) {
    ...
    obj7.children = tmp18(tmp2(7050).SearchField, obj8);
  ...
  if (stateFromStores) {
    if (0 === stickerCategories.length) {
      ...
      tmp18Result2 = tmp18(tmp8(10419), obj10);
      const tmp8Result = tmp8(10419);
    }
  } else {
    ...
    items5[2] = tmp18(tmp8(10424), obj12);
    obj6.children = items5;
    obj5.children = closure_15(tmp20, obj6);
    return tmp18(channel(7162).AnalyticsLocationProvider, obj5);
  }
});
