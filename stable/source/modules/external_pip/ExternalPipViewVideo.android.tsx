...
// Dependencies: [32, 19, 17, 1957, 1371, 4581, 21, 4560, 576, 9604, 4515, 4556, 1114, 504, 1178, 9611, 4262, 17007, 9622, 17008, 9616, 2]
...
function ExternalPipViewVideoUnavailable(wasStream) {
  ...
  if (wasStream) {
    ...
    let tmp4Result = tmp4(tmp5(9604).StreamEnded, obj2);
    let tmp8 = tmp4;
    let tmp10 = tmp5;
  } else {
  ...
  items[1] = tmp8(tmp10(4556).Text, obj3);
  obj.children = items;
...
function ExternalPipViewVideoStream(streamId) {
  ...
  noop = undefined;
  const surfaceDirectRendererExperiment = streamId(9611).useSurfaceDirectRendererExperiment(streamId.userId, { location: "ExternalPipViewVideoStream" });
  ...
  const children = [closure_11(first(9622), { useSurfaceDirectRenderer: surfaceDirectRendererExperiment, style: memo, streamId, onReady: callback, onLayout: callback1 }), ];
