...
// Dependencies: [19, 17, 4552, 7703, 17035, 5444, 21, 4296, 8457, 4560, 576, 11488, 504, 4763, 17036, 1896, 4527, 17037, 4561, 5529, 15135, 17038, 14998, 1114, 11520, 2]

// Module 17034 (QuestActivityButton)
...
function QuestActivityButtonInner(quest) {
  ...
  const callback = num.useCallback(() => {
    ...
    if (null == enrolledAt) {
      ...
      ModalActionCreatorsDefault.pushLazy(asyncRequireImpl(17036, dependencyMap.paths), obj2, QUEST_ACTIVITY_UNENROLLED_MODAL_KEY);
    } else {
      const obj4 = { questId: tmp.id };
      ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(17037, dependencyMap.paths), "QuestProgressBottomSheet", obj4);
    }
  }, items1);
  if (quest != null) {
  ...
  closure_4 = tmp9;
  let obj2 = quest(504);
  sharedValue = quest(4296).useSharedValue(num);
  const tmpResult = quest(4296);
  let num2 = 0;
  if (tmp9) {
    num2 = 1;
  }
  sharedValue1 = quest(4296).useSharedValue(num2);
  ref = obj3.useRef(null);
  const tmp14 = closure_13();
  confetti = tmp14;
  const tmpResult4 = quest(4296);
  const fn = function u() {
  ...
  const tmpResult6 = quest(4296);
  obj6.source = quest(15135);
  obj5.children = closure_9(stateFromStores(5529), obj6);
  ...
  obj8.icon = stateFromStores(14998);
  obj8.onPress = callback;
  const intl = tmp(1114).intl;
  obj8.accessibilityLabel = intl.string(quest(1114).t.JALI2K);
  obj7.children = closure_9(stateFromStores(17038), obj8);
  ...
  size.children = closure_9(quest(8457).Circle, { cx: 16, cy: 16, r: 14.3, fill: "none", stroke: stateFromStores(576).unsafe_rawColors.OPACITY_32, strokeWidth: 3.4, strokeDasharray: result });
  obj9.children = closure_9(quest(8457).Svg, size);
  ...
  obj11.children = closure_9(quest(8457).Svg, size1);
  items6[3] = closure_9(closure_4, obj11);
  obj4.children = items6;
  return closure_10(stateFromStores(4296).View, obj4);
}
...
let c11 = 32;
let closure_12 = ReanimatedRexport.createAnimatedComponent(fn(8457).Circle);
