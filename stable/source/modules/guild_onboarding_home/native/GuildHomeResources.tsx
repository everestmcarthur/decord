...
// Dependencies: [19, 17, 1957, 2012, 4781, 4199, 1074, 21, 4560, 576, 504, 7898, 12013, 16569, 7456, 12288, 1396, 5123, 4556, 4550, 16562, 1100, 1114, 16570, 4975, 2]
// Exports: default
...
function ResourceChannelRow(channelId) {
  ...
  const forumPostMediaProperties = channelId(7898).useForumPostMediaProperties(firstResult, false);
  const obj5 = channelId(7898);
  let length;
  const firstMediaIsEmbed = channelId(7898).useFirstMediaIsEmbed(firstResult, false);
  ...
  const obj6 = channelId(7898);
  let flag = channelId(12013).useSharedMediaProps({ channel: stateFromStores, media: first }).shouldObscure;
  ...
  if (null != stateFromStores) {
    if (stateFromStores1) {
      ...
      const items4 = [closure_12(tmp2(4556).Text, obj9), , ];
      ...
      if (tmp19Result) {
        ...
        obj10.children = tmp10(4550).parse(tmp11, true, { guildId: null, channelId: null });
        tmp19Result = tmp19(tmp2(4556).Text, obj10);
        const obj11 = { guildId: null, channelId: null };
        const tmp10Result3 = tmp10(4550);
      }
      ...
      if (!(null == description || 0 === description.length)) {
        ...
        obj12.children = tmp10(4550).parse(description, true, { guildId: null, channelId: null });
        tmp19Result4 = tmp19(tmp2(4556).Text, obj12);
        const obj13 = { guildId: null, channelId: null };
        const tmp10Result4 = tmp10(4550);
      }
        ...
        if (null != firstResult) {
          ...
          if (!blocked) {
            ...
            if (null != first) {
              ...
              obj17.embedLeftBorderColor = tmp2(7898).getEmbedColor(firstResult, flag);
              let id;
              if (firstResult != null) {
                id = firstResult.id;
              }
              ...
              tmp19Result6 = tmp19(tmp2(12013).ForumPostMediaThumbnail, obj17);
              const tmp2Result2 = tmp2(7898);
            }
          }
        }
      }
      ...
      return closure_13(tmp2(5123).PressableOpacity, obj7);
    }
  }
  return null;
}
...
export default function GuildHomeResources(guildId) {
  ...
  if (0 === arr.length) {
    ...
    const items = [closure_12(guildId(4556).Text, obj3), , ];
    const obj4 = { style: tmp.emptyStateImage, source: _modDef16570 };
    ...
    items[2] = closure_12(guildId(4975).Button, obj5);
    obj2.children = items;
