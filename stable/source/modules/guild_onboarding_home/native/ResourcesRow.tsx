...
export default function ResourcesRow(guildId) {
  ...
  const items = [
    substr.map((children) => {
      ...
      const obj = {
        ...
        children: closure_1_5(guildId(4556).Text, { variant: "text-md/medium", color: "text-default", children: children.title })
      };
  ...
  if (tmp3) {
    const obj2 = {
      ...
      onPress() {
          ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(16563, dependencyMap.paths), closure_4, { guildId });
        },
      children: null
    };
    ...
    obj2.children = closure_5(guildId(4556).Text, obj3);
    tmp6 = closure_5(guildId(5123).PressableOpacity, obj2);
  }
  items[1] = tmp6;
  obj.children = items;
