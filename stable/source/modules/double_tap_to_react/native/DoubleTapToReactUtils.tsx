...
// Dependencies: [19, 1949, 5459, 7970, 1954, 1374, 21, 4213, 1935, 7971, 4211, 4528, 7764, 7973, 4217, 7975, 1896, 4906, 4380, 1943, 2]
...
// Module 7969 (DoubleTapToReactUtils)
  ...
  if (true !== disableDoubleTap) {
    if (tmpResult.canReactToMessage(message, channel)) {
      ...
      if (reactions.some((emoji) => ReactionUtils.emojiEquals(emoji.emoji, obj5) && emoji.me)) {
        const result2 = tmp(4528).triggerHapticFeedback(tmp(4528).HapticFeedbackTypes.IMPACT_LIGHT);
        const tmpResult10 = tmp(4528);
        const obj6 = { channelId: channel.id, messageId: message.id, emoji: tmp8, location: tmp(7764).ReactionLocations.DOUBLE_TAP };
        tmp(7764).removeReaction(obj6);
      } else {
        if (flag) {
          let byName = UnicodeEmojisDefault.getByName("heart");
          if (byName == null) {
        ...
        if (null != customEmojiById) {
          if (null != customEmojiById) {
            ...
            if (emojiUnavailableReason === constants.PREMIUM_LOCKED) {
              ...
              const result3 = tmp(7973).showDoubleTapErrorToast(obj13);
            }
          }
          const result4 = tmp(4528).triggerHapticFeedback(tmp(4528).HapticFeedbackTypes.IMPACT_LIGHT);
          const tmpResult15 = tmp(7764);
          const id = channel.id;
          tmpResult15.addReaction(id, message.id, tmp8, tmp(7764).ReactionLocations.DOUBLE_TAP);
          const tmpResult14 = tmp(4528);
          const obj14 = { dismissAction: ContentDismissActionType.INDIRECT_ACTION };
          const result5 = tmp(4380).UNSAFE_markDismissibleContentAsDismissed(tmp(1943).DismissibleContent.DOUBLE_TAP_TO_REACT_REMINDER, obj14);
          if (isContentShown(tmp(1943).DismissibleContent.DOUBLE_TAP_TO_REACT_EXPANDED_UPSELL)) {
            ...
            const result6 = tmp(4380).UNSAFE_markDismissibleContentAsDismissed(tmp(1943).DismissibleContent.DOUBLE_TAP_TO_REACT_EXPANDED_UPSELL, obj15);
            const tmpResult17 = tmp(4380);
          }
          tmp31 = ContentDismissActionType;
          const tmpResult16 = tmp(4380);
        } else if (!flag) {
          let emojiName1;
          if (setting != null) {
          ...
          const result7 = tmp(7973).showDoubleTapErrorToast(obj16);
          const tmpResult18 = tmp(7973);
        }
      }
    }
    tmpResult = tmp(7971);
