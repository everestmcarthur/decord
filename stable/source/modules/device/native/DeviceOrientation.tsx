...
// Dependencies: [19, 17, 560, 1115, 1249, 4539, 8334, 2]
...
// Module 8333 (DeviceOrientation)
import PlatformUtils from "PlatformUtils" /* 1115 */;
import ReactBatchUpdates from "ReactBatchUpdates" /* 1249 */;
import noop from "module_19" /* 19 */;
import get_ActivityIndicator_mod from "module_8334" /* 8334 */;
...
function handleOrientationChange(initialOrientation) {
  ...
  if (global !== initialOrientation) {
    ...
    if (!tmp9) {
      let isIpadOSResult = tmp(4539).isIpadOS();
      if (isIpadOSResult) {
      ...
      if ("LANDSCAPE" === orientationLock) {
        ...
        if (!obj13.isAndroid()) {
          if (tmp14Result.isIOS()) {
            tmp14(4539).getSystemVersionMajor() >= 16;
            const tmp14Result3 = tmp14(4539);
          }
      ...
      if ("PORTRAIT" === orientationLock) {
        ...
        if (!obj12.isAndroid()) {
          if (tmp12Result.isIOS()) {
            tmp12(4539).getSystemVersionMajor() >= 16;
            const tmp12Result3 = tmp12(4539);
          }
...
function lockOrientationForiOS(PORTRAIT) {
  ...
  if (!isAndroidResult) {
    ...
    const tmpResult = tmp(4539);
  }
  if (!isAndroidResult) {
    get_ActivityIndicator.ignoreAutoRotate(false);
    c8 = false;
    if ("LANDSCAPE" === PORTRAIT) {
      tmp6(8334).lockToLandscapeLeft();
      const tmp6Result = tmp6(8334);
      tmp(1249).batchUpdates(() => {
        obj3.setState({ orientationLock: "LANDSCAPE" });
      });
      const tmpResult3 = tmp(1249);
    } else {
      tmp6(8334).lockToPortrait();
      const tmp6Result2 = tmp6(8334);
      tmp(1249).batchUpdates(() => {
...
const listener = AppState.addEventListener("change", function applyLockStateOnAppActive(event) {
  ...
  if (tmp) {
    ...
    c8 = false;
    if ("LANDSCAPE" === orientationLock) {
      tmp3(8334).lockToLandscapeLeft();
      const tmp3Result = tmp3(8334);
      ReactBatchUpdates.batchUpdates(() => {
        obj3.setState({ orientationLock: "LANDSCAPE" });
      });
    } else {
      tmp3(8334).lockToPortrait();
      obj3 = ReactBatchUpdates;
      obj3.batchUpdates(() => {
        obj3.setState({ orientationLock: "PORTRAIT" });
      });
      const tmp3Result2 = tmp3(8334);
    }
  }
});
const size = fn(2);
...
export const lockOrientation = function lockOrientation(PORTRAIT, flag) {
  ...
  c8 = false;
  if ("LANDSCAPE" === PORTRAIT) {
    tmp(8334).lockToLandscapeLeft();
    const tmpResult = tmp(8334);
    ReactBatchUpdates.batchUpdates(() => {
      obj3.setState({ orientationLock: "LANDSCAPE" });
    });
  } else {
    tmp(8334).lockToPortrait();
    const tmpResult2 = tmp(8334);
    ReactBatchUpdates.batchUpdates(() => {
      obj3.setState({ orientationLock: "PORTRAIT" });
    });
  }
