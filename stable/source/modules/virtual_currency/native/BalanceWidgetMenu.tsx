...
// Dependencies: [19, 17, 1074, 1076, 1954, 5444, 21, 5123, 4556, 1114, 10630, 1943, 1242, 11772, 5447, 15699, 8856, 4527, 11106, 1896, 7182, 7541, 11105, 4380, 4991, 4560, 576, 2]
// Exports: default

// Module 15698 (BalanceWidgetMenu)
...
import OrbOnboardingPillDefault from "OrbOnboardingPill" /* 15699 */;
    ...
    obj.children = function children(markAsDismissed) {
      ...
      if (markAsDismissed.visibleContent === dismissible_content.DismissibleContent.VIRTUAL_CURRENCY_MOBILE_ONBOARDING_PILL) {
        function handleOnboardingPress() {
          ...
          const obj2 = style(11772);
          obj2.openQuestHome({ filter: constants3.VIRTUAL_CURRENCY, fromContent: style(5447).QuestContent.MOBILE_ORBS_ONBOARDING_DC });
        }
...
function BalanceWidgetMenu(style) {
  let str;
  str = str(8856).useFetchVirtualCurrencyBalance().balance;
  let items = [str];
  const callback = noop.useCallback(() => {
    ...
    obj4.onButtonPress = function onButtonPress() {
      ...
      const result = str(7541).openCollectiblesShopMobile(obj5);
    };
    ...
    obj5.onButtonPress = function onButtonPress() {
      ...
      ActionSheetActionCreatorsDefault.hideActionSheet();
      const obj4 = str(11772);
      obj4.openQuestHome({ mergeExistingRoutes: true, filter: constants3.VIRTUAL_CURRENCY, fromContent: str(5447).QuestContent.ORBS_BALANCE_MENU });
    };
...
export default function BalanceWidgetMenuWrapper(style) {
  const tmp = closure_15();
  const balance = _mod8856.useFetchVirtualCurrencyBalance().balance;
  ...
  if (null == balance) {
    ...
    const obj3 = { style: tmp.loadingContainer, children: closure_1_10(tmp2(4991).Ellipsis, { variant: "primary", size: "sm" }) };
