...
// Dependencies: [19, 17, 4575, 1963, 1074, 4742, 21, 4560, 9679, 7149, 1114, 9724, 9704, 9668, 9787, 9795, 4992, 1242, 7110, 7150, 9812, 5586, 2]
// Exports: default

// Module 9794 (GuildEventsListActionSheet)
...
function GuildEventsListHeader(arg0) {
  ...
  if (tmp3Result) {
    ...
    tmp3Result = tmp3(tmp4(9724).ActionSheetHeaderPressableText, obj3);
  }
...
export default function GuildEventsListActionSheet(guild) {
  guild = guild.guild;
  let events;
  events = events(9668)(guild.id);
  const items = [events, guild.id];
  const tmp = closure_10();
  const effect = noop.useEffect(() => {
    const item = arr.forEach((id) => arr(9787).getGuildEventUserCounts(id.id, id.id, []));
    const guildEventsForCurrentUser = GuildScheduledEventManagerDefault.getGuildEventsForCurrentUser(guild.id);
  }, items);
  const items1 = [guild];
  const callback = noop.useCallback(() => {
    const result = guild(9704).closeGuildEventListActionSheet();
  }, []);
  const callback1 = noop.useCallback((eventId, recurrenceId) => {
    let result = guild_scheduled_events_GuildScheduledEventModalActionCreators.openGuildEventDetails({
      ...
      recurrenceId,
      onClose() {
        const result = guild(9704).openGuildEventListActionSheet(closure_1_0);
      }
    });
  }, items1);
  events(4992)(() => {
  ...
  obj4.lastAckedId = events(5586)(ref);
  obj3.children = jsx(events(9812), { inActionSheet: true, events, onPressEvent: callback1, onCloseAction: callback, guild, lastAckedId: null });
  obj.children = <View style={tmp.container}>{null}</View>;
  return jsx(guild(7150).BottomSheet, { showGradient: true, scrollable: events.length > 0, startExpanded: true, dismissAccessibilityLabel: null, header: null, children: null });
};
