...
// Dependencies: [19, 17, 4552, 2021, 1979, 4583, 1371, 1963, 9705, 1085, 21, 4560, 576, 5524, 4527, 504, 9710, 9787, 9786, 9778, 9782, 4556, 1114, 1178, 4404, 8411, 5433, 9802, 2]
// Exports: closeGuildEventInfoActionSheet, default

// Module 9801 (EventDetailInfoSheet)
import nativeDefault from "native" /* 576 */;
import native from "native" /* 1178 */;
import UserUtilsDefault from "UserUtils" /* 4404 */;
import ActionSheetActionCreatorsDefault from "ActionSheetActionCreators" /* 4527 */;
import useGuildScheduledEventUserCountDefault from "useGuildScheduledEventUserCount" /* 9786 */;
import GuildScheduledEventManagerDefault from "GuildScheduledEventManager" /* 9787 */;
...
let closure_11 = fn(9705).GUILD_EVENT_INFO_ACTION_SHEET_KEY;
...
export default function EventDetailInfoSheet(guildEvent) {
  ...
  const result = guildEvent(9710).recurrenceRuleFromServer(guildEvent.recurrence_rule);
  const items3 = [, ];
  ...
  obj6.style = items7;
  const items8 = [closure_12(guildEvent(9778).GuildEventCardHeader, { event: guildEvent, style: tmp.eventHeader, showUserCount: false, showEndDate: true, showCreator: false, recurrenceId }), closure_12(guildEvent(9778).GuildEventCardTitle, { event: guildEvent, textStyle: tmp.eventTitle, style: tmp.eventTitleContainer }), closure_12(guildEvent(9778).GuildEventCardSimpleGuildInfo, { event: guildEvent, textStyle: tmp.guildTextStyle }), closure_12(guildEvent(9778).GuildEventSimpleLocation, { event: guildEvent }), , , , , , ];
  const obj11 = { style: tmp.interestedContainer, children: null };
  const items9 = [closure_12(guildEvent(9782).BellIcon, { size: "sm", style: tmp.interestedIcon }), ];
  ...
  items9[1] = closure_12(guildEvent(4556).Text, obj13);
  ...
  if (tmp11Result) {
    ...
    items10[1] = tmp13(tmp2(4556).Text, obj16);
    obj14.children = items10;
    tmp11Result = tmp11(tmp12, obj14);
  }
  items8[5] = tmp11Result;
  items8[6] = closure_12(guildEvent(9778).GuildEventCardDescription, { event: guildEvent, style: tmp.eventDescriptionContainer });
  ...
  if (hasItem) {
    ...
    const items11 = [tmp2(8411).DividerPosition.TOP, tmp2(8411).DividerPosition.BOTTOM];
    ...
    hasItem = tmp13(tmp7(8411), obj19);
    const tmp7Result = tmp7(8411);
  }
  items8[7] = hasItem;
  const obj20 = { direction: "horizontal", style: tmp.controlsContainer, children: null };
  const items12 = [
    closure_12(guildEvent(9778).GuildEventCardPrimaryAction, {
      event: guildEvent,
      onCloseAction() {
  ...
  if (stateFromStores1) {
    ...
    tmp13Result = tmp13(tmp2(9778).GuildEventShareAction, obj22);
  }
  items12[1] = tmp13Result;
  items12[2] = closure_12(guildEvent(9778).GuildEventModeratorAction, { event: guildEvent, recurrenceId });
  obj20.children = items12;
  items8[8] = closure_13(guildEvent(5433).ButtonGroup, obj20);
  ...
  if (tmp13Result2) {
    ...
    tmp13Result2 = tmp13(tmp7(9802), obj23);
  }
