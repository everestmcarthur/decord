...
// Dependencies: [19, 17, 4552, 4209, 1074, 21, 4560, 10126, 576, 1115, 7162, 4982, 563, 8180, 1896, 4404, 16046, 1114, 4296, 16443, 5123, 16444, 1178, 4556, 16445, 16446, 1242, 2]
// Exports: default

// Module 16442 (ForYouSuggestedFriendRow)
...
export default function ForYouSuggestedFriendRow(suggestedFriend) {
  ...
  let sharedValue;
  let stateFromStores;
  const messagesTabLayout = suggestedFriend(10126).useMessagesTabLayout(panelVariant);
  const tmp4 = closure_12(messagesTabLayout);
  const analyticsLocations = useAnalyticsLocationsDefault().analyticsLocations;
  const obj = suggestedFriend(10126);
  const layoutStyles = suggestedFriend(10126).getLayoutStyles(messagesTabLayout);
  const obj2 = suggestedFriend(10126);
  const fontScale = suggestedFriend(4982).useFontScale();
  const obj3 = suggestedFriend(4982);
  ...
  if (null != suggestedFriend.friendSuggestionName) {
    ...
    const suggestedContactNameForSuggestion = tmp(16046).getSuggestedContactNameForSuggestion(friendSuggestionName, suggestedFriend);
    let str2 = "";
    ...
    if (null != suggestedFriend.mutualFriendsCount) {
      ...
      sharedValue = tmp(4296).useSharedValue(false);
      const tmpResult8 = tmp(4296);
      ...
      const items6 = [closure_9(tmp(4556).Text, obj12), ];
      const tmp20 = closure_11;
      const tmpResult11 = tmp(16444);
      let num3 = 0;
      ...
      obj13.children = closure_9(tmp(16445).ActionStatusSubLabel, obj15);
      ...
      items5[2] = closure_9(tmp(16446).ContactSuggestionActions, obj16);
      ...
      return tmpResult10.renderChannelPressableWrapper(closure_9(tmp(5123).PressableHighlight, obj7), obj19);
    }
    const tmpResult = tmp(16046);
    formatToPlainStringResult = tmp5(4404).getName(suggestedFriend.user);
    const tmp5Result = tmp5(4404);
  }
