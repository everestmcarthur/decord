...
// Dependencies: [5, 32, 19, 17, 4552, 9296, 2015, 1482, 1074, 1607, 21, 4560, 576, 12158, 504, 4296, 4561, 4564, 4528, 4987, 4409, 4976, 1114, 4505, 11220, 4740, 6981, 12159, 7523, 8273, 12055, 1249, 12032, 12161, 1894, 9295, 11997, 9578, 1477, 11361, 10640, 12162, 10641, 5138, 1874, 5128, 12163, 9415, 6627, 4556, 12164, 12165, 12196, 1178, 38, 12132, 9422, 7521, 9301, 9411, 12197, 12198, 12118, 12119, 2]
// Exports: default

// Module 12157 (AppLauncherCommandViewScreen)
...
import _modDef12118 from "module_12118" /* 12118 */;
import _modDef12119 from "module_12119" /* 12119 */;
import AppLauncherCommandViewHeader from "AppLauncherCommandViewHeader" /* 12158 */;
import ApplicationCommandValidationUtils from "ApplicationCommandValidationUtils" /* 12159 */;
import application_commands_ApplicationCommandValidationUtils from "application_commands/ApplicationCommandValidationUtils" /* 12161 */;
import CommandOptionViewDefault from "CommandOptionView" /* 12164 */;
...
function AppLauncherCommandViewInner(command) {
  ...
  noop.useRef(false);
  ref6 = obj2.useRef(Date.now());
  ref7 = obj2.useRef(null);
  closure_22 = obj2.useRef(false);
  ...
  [arr4, c23] = onCommandExecuted(noop.useState([]), 2);
  let tmp15 = onCommandExecuted(noop.useState([]), 2);
  first = tmp15[0];
  closure_25 = tmp15[1];
  const tmp14 = onCommandExecuted(noop.useState([]), 2);
  ref8 = obj2.useRef(new Set());
  ...
  noop.useRef(new Set());
  const set1 = new Set();
  [tmp20, c30] = onCommandExecuted(noop.useState([]), 2);
          ...
          const item = options.forEach((required) => {
            ...
            if (true !== required.required) {
              ...
              if (someResult) {
                ...
                closure_31.current[required.name] = AppLauncherNativeUtils.getInitialOptionValues(obj3);
                const obj5 = { option: required, content: closure_31.current[required.name], guildId: guild_id, channelId: context.channel.id, allowEmptyValues: false, commandOrigin: ApplicationCommandTypes.CommandOrigin.APPLICATION_LAUNCHER };
              ...
              closure_31.current[required.name] = AppLauncherNativeUtils.getInitialOptionValues(obj7);
              const obj9 = { option: required, content: closure_31.current[required.name], guildId: guild_id, channelId: context.channel.id, allowEmptyValues: false, commandOrigin: ApplicationCommandTypes.CommandOrigin.APPLICATION_LAUNCHER };
  ...
  callback3 = obj2.useCallback(() => {
    if (null != command) {
      ...
      if (null != firstInvalidOption) {
        ...
        tmp14(4740).trackWithMetadata(constants2.APPLICATION_COMMAND_VALIDATION_FAILED, obj);
        const tmp14Result = tmp14(4740);
      } else {
        closure_34(false);
        ...
        const tmp14Result3 = tmp14(9295);
        obj4.optionValues = tmp14(11997).parseOptionValuesForSend(commandContext.channel, tmp, optionValues.current);
        obj4.context = commandContext;
        obj4.sectionName = sectionName;
        obj4.commandOrigin = tmp14(7523).CommandOrigin.APP_LAUNCHER_APPLICATION_VIEW;
        const result1 = tmp14Result3.executeAppLauncherCommand(obj4);
        const tmp14Result4 = tmp14(11997);
        result1.then(() => {
  ...
  onPressAttachmentOption = obj2.useCallback((option) => {
    ...
    if (fileTypeFiltering.mediaFilesAllowed) {
      ...
      obj3.onPressItem = function onPressItem(item) {
        ...
        if (allowedExtensions.length > 0) {
          const items = [tmp(5138).getFileFromUploadItem(result1).filename];
          if (!analyticsLocation(items)) {
            return onCommandExecuted();
          }
          const tmpResult = tmp(5138);
        }
        const result2 = MediaKeyboardUtils.addAttachmentForCommand(item.channelId, chatInputRef, result1, obj, tmp(5128).UploadOrigin.IMAGE_PICKER);
      };
  ...
  onDismiss = obj2.useCallback((name) => {
    ...
    command(sectionName[25]).trackWithMetadata(prefilledOptions.APPLICATION_COMMAND_OPTIONAL_OPTION_REMOVED, obj4);
  }, items15);
  const callback10 = obj2.useCallback((nativeEvent) => closure_38(nativeEvent.nativeEvent.layout.height), []);
  ...
  if (loading) {
    let tmp70Result2 = ref6(c30, {});
    let tmp67 = ref6;
  } else if (null == command) {
    let obj3 = { onPressBack: callback };
    tmp70Result2 = ref6(optionValues, obj3);
    tmp67 = ref6;
    ...
    obj4.contentContainerStyle = items16;
    ...
    if (tmp63) {
      ...
      tmp73Result = tmp73(tmp72, obj9);
    }
    items18[1] = tmp73Result;
    let tmp73Result3 = !tmp63;
    if (!tmp63) {
      tmp73Result3 = tmp65;
    }
    if (tmp73Result3) {
      ...
      obj10.children = tmp73(tmp3(tmp4[49]).Text, obj11);
      tmp73Result3 = tmp73(tmp72, obj10);
    }
    items18[2] = tmp73Result3;
    let tmp73Result4 = !tmp65;
    if (!tmp65) {
      ...
      obj12.children = tmp73(tmp3(tmp4[49]).Text, obj13);
      tmp73Result4 = tmp73(tmp72, obj12);
    }
    items18[3] = tmp73Result4;
    if (tmp70Result) {
      ...
      const items19 = [tmp73(tmp3(tmp4[49]).Text, obj15), , ];
      const obj16 = {
      ...
      items19[1] = tmp73(tmp31(tmp4[15]).View, obj16);
      ...
      obj17.children = tmp73(tmp31(tmp4[52]), obj18);
      items19[2] = tmp73(tmp31(tmp4[15]).View, obj17);
      obj14.children = items19;
      tmp70Result = tmp70(tmp71, obj14);
    }
    const obj19 = { children: null };
    items18[4] = tmp70Result;
    const obj20 = { size: sum1 };
    items18[5] = ref6(tmp3(tmp4[53]).Spacer, obj20);
    ...
    items20[1] = ref6(ref8, obj21);
    obj19.children = items20;
    tmp70Result2 = tmp70(tmp71, obj19);
    tmp67 = tmp73;
  } else {
    tmp67 = ref6;
    const obj22 = { command, onPressBack: callback };
    tmp70Result2 = ref6(first2, obj22);
  }
  const obj23 = { style: tmp.container, children: null };
  const items21 = [tmp67(command(sectionName[13]).AppLauncherCommandViewHeader, { command, onPressBack: callback, scrollOffsetY: sharedValue1, section }), tmp70Result2];
...
function NotFoundState(onPress) {
  ...
  const obj3 = { style: tmp.emptyState, lightSource: _modDef12197, darkSource: _modDef12198, title: null, body: null, children: null };
...
function NoPermsState(command) {
  ...
  const obj6 = { style: tmp.emptyState, lightSource: _modDef12118, darkSource: _modDef12119, title: null, body: null, children: null };
  const intl = util.intl;
...
const ApplicationCommandIndexStore = fn(9296);
...
obj2.optionsContainer = { paddingTop: fn(12158).EXPANDED_HEADER_TOTAL_CONSUMED_SPACE_IN_PARENT + nativeDefault.space.PX_4, paddingHorizontal: DEFAULT_CONTENT_PADDING, paddingBottom: DEFAULT_CONTENT_PADDING, backgroundColor: nativeDefault.colors.MOBILE_KEYBOARD_PANEL_BACKGROUND };
...
let obj4 = { paddingTop: fn(12158).EXPANDED_HEADER_TOTAL_CONSUMED_SPACE_IN_PARENT + nativeDefault.space.PX_4, paddingHorizontal: DEFAULT_CONTENT_PADDING, paddingBottom: DEFAULT_CONTENT_PADDING, backgroundColor: nativeDefault.colors.MOBILE_KEYBOARD_PANEL_BACKGROUND };
...
obj2.emptyStateContainer = { paddingTop: fn(12158).EXPANDED_HEADER_TOTAL_CONSUMED_SPACE_IN_PARENT + nativeDefault.space.PX_4, flex: 1 };
obj2.failureStateButtonWrapper = { marginTop: 24, alignSelf: "center" };
let obj7 = { paddingTop: fn(12158).EXPANDED_HEADER_TOTAL_CONSUMED_SPACE_IN_PARENT + nativeDefault.space.PX_4, flex: 1 };
