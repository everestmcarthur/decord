...
// Dependencies: [19, 17, 1074, 21, 4560, 576, 9295, 8188, 1396, 12087, 5123, 12096, 1894, 12090, 4556, 1114, 12060, 5612, 2]
// Exports: default
...
export default function RecommendationsBannerCard(application) {
  ...
  const tmp = closure_8();
  if (!showsPromoted) {
    showsPromoted = application(9295).isPromotedApplication(application);
    const obj = application(9295);
  }
  const shelfBadgeTypeIfActive = application(9295).getShelfBadgeTypeIfActive(application);
  ...
  if (isLandscape) {
    ...
    let tmp12Result2 = closure_6(tmp4(12087).BaseAppRow, obj5);
  } else {
    ...
    obj10.applicationEmbedded = tmp4(9295).isEmbeddedApp(application);
    ...
    if (showsPromoted) {
      ...
      const items3 = [tmp14(tmp10(12090), obj12), ];
      if (showsPromoted) {
        ...
        obj13.children = tmp14(tmp4(4556).Text, obj14);
    ...
    if (tmp14Result) {
      ...
      tmp14Result = tmp14(tmp10(12060), obj16);
    }
    ...
    tmp12Result2 = tmp12(tmp4(5123).PressableOpacity, obj7);
