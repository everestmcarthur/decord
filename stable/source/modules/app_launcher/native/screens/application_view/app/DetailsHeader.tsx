...
// Function ID: 9295
// Name: DetailsHeader
// Dependencies: [32, 19, 17, 21, 4560, 576, 4296, 9295, 9425, 5126, 8273, 5586, 4556, 1114, 4561, 4564, 5664, 4987, 1093, 2]

// Module 9294 (DetailsHeader)
import nativeDefault from "native" /* 576 */;
import ReanimatedRexport from "ReanimatedRexport" /* 4296 */;
import timing from "timing" /* 4561 */;
import timingPresets from "timingPresets" /* 4564 */;
import BioMarkupUtils from "BioMarkupUtils" /* 9425 */;
...
export default noop.memo(function DetailsHeader(viewContainerStyle) {
  ...
  const isPartnerApplicationResult = sharedValue(9295).isPartnerApplication(application);
  sharedValue = sharedValue(4296).useSharedValue(null);
  ...
  const obj4 = sharedValue(4296);
  const sectionName = sharedValue(9295).getSectionName(application);
  const obj5 = sharedValue(9295);
  const str = sharedValue(9295).getSectionDescription(application);
  ...
  const tmp24 = first(8273)(isScreenLandscape);
  closure_18 = tmp24;
  ...
  if (!hideName) {
    ...
    tmp30Result = tmp30(tmp2(4556).Heading, obj10);
  }
  ...
  if (isPartnerApplicationResult) {
    ...
    obj11.children = tmp30(tmp2(4556).Text, obj12);
  ...
  if (tmp28Result5) {
    ...
    if (tmp27) {
      ...
      items8[1] = tmp30(tmp23(4987), obj19);
      if (num2 == null) {
        num2 = 0;
      }
    ...
    obj22.children = tmp30(tmp2(4556).Text, obj23);
    const items10 = [tmp30(tmp23(5664), obj22), , ];
    ...
    if (tmp27) {
      ...
      obj24.children = tmp28(tmp2(4556).Text, obj25);
  ...
  if (!first2) {
    ...
    if (!hideName) {
      ...
      tmp30Result9 = tmp30(tmp2(4556).Heading, obj28);
    }
    ...
    if (tmp28Result5) {
      ...
      const items15 = [tmp30(tmp2(4556).Text, obj30), ];
      ...
      items15[1] = tmp30(tmp2(4556).Text, obj31);
