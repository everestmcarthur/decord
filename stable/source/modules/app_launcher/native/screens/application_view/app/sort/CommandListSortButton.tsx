...
const CommandListSortOrder = fn(12139).CommandListSortOrder;
...
export default function CommandListSortButton(sortOrder) {
  ...
  const obj = {
    ...
    style: tmp.container,
    onPress() {
      ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(12154, dependencyMap.paths), "CommandListSortActionSheet", {
        sortOrder,
        onSortOptionPress,
        onClose() {
  ...
  const items = [closure_6(sortOrder(4556).Text, { variant: "text-sm/medium", color: "text-default", children: stringResult }), closure_6(sortOrder(11158).ChevronSmallDownIcon, { size: "xs", color: onSortOptionPress(576).colors.TEXT_DEFAULT })];
  obj2.children = items;
