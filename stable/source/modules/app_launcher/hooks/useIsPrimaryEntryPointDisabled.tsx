...
// Dependencies: [1918, 4199, 1085, 504, 9588, 12061, 9552, 9416, 1115, 1114, 2]
// Exports: default

// Module 12147 (useIsPrimaryEntryPointDisabled)
...
export default function useIsPrimaryEntryPointDisabled(arg0) {
  ...
  const embeddedActivityLaunchability = channel(9588).useEmbeddedActivityLaunchability(id);
  let flag = false;
  if (channel(12061).ActivityAction.LEAVE !== activityAction) {
    if (tmp2(12061).ActivityAction.START === activityAction) {
      flag = false;
      if (null != channel) {
        ...
        if (isGuildVoiceResult) {
          flag = false;
          if (embeddedActivityLaunchability !== tmp2(9588).EmbeddedActivityLaunchability.CAN_LAUNCH) {
            flag = true;
          }
        } else {
          flag = false;
          ...
          tmp2Result = tmp2(9552);
        }
      }
    } else {
      flag = false;
      if (tmp2(12061).ActivityAction.JOIN === activityAction) {
  ...
  let disabled = flag;
  let reason;
  if (activityAction !== channel(12061).ActivityAction.LEAVE) {
