...
// Dependencies: [19, 17, 4199, 4579, 1074, 21, 4560, 576, 1115, 7874, 4495, 4411, 9477, 563, 5431, 5425, 5417, 12298, 4556, 1114, 4975, 5097, 5101, 2]
...
export default noop.memo(function VocalChannelJoinButton(channel) {
  ...
  const tmp = voiceStates(7874)();
  const tmp2 = voiceStates(4495)();
  let joinButtonText = closure_11(tmp, channel(4411).isThemeLight(tmp2));
  const obj = channel(4411);
  const isConnectedToVoiceChannel = channel(9477).useIsConnectedToVoiceChannel(channel);
  const obj2 = channel(9477);
  ...
  const tmp8 = channel(5417).useStageHasMedia(channel.id) && isGuildStageVoiceResult;
  ...
  const connectedUserLimit = channel(12298).useConnectedUserLimit({ channel, video: tmp12 });
  const tmp3Result3 = channel(12298);
  ...
  if (isConnectedToVoiceChannel) {
    ...
    obj7.children = tmp17(tmp3(4556).Text, obj8);
