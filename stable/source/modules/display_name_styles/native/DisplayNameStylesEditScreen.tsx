...
// Dependencies: [32, 19, 17, 4552, 1371, 1074, 1607, 21, 1390, 4560, 576, 1484, 504, 15338, 8167, 4794, 4495, 1391, 1388, 9183, 10907, 10908, 558, 15339, 15340, 15341, 15342, 1242, 4528, 8168, 8165, 4527, 15345, 1896, 15346, 15347, 15352, 15355, 1114, 14618, 4271, 4796, 15356, 2786, 4556, 1178, 7209, 1091, 14619, 4975, 8840, 7930, 1611, 4296, 4974, 2]
// Exports: default

// Module 15337 (DisplayNameStylesEditScreen)
...
export default function DisplayNameStylesEditScreen() {
  ...
  if (colors.length > 0) {
    ...
    const callback2 = obj8.useCallback(() => {
      const result = HapticUtils.triggerHapticFeedback(HapticUtils.HapticFeedbackTypes.IMPACT_MEDIUM);
      if (isTryItOut) {
        const result1 = tmp(8168).setTryItOutDisplayNameStyles(null);
        const tmpResult = tmp(8168);
      } else {
        tmp(8165).setPendingChanges({ displayNameStyles: null });
    ...
    const callback4 = obj8.useCallback(() => {
      ...
      ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(15345, dependencyMap.paths), "DisplayNameStylesFontPickerSheet", { selectedFontId, onSelectFont, displayName: displayNameStylesPendingName });
    }, items6);
    if (stateFromStores != null) {
    ...
    const callback5 = obj8.useCallback(() => {
      ...
      obj.openLazy(asyncRequireImpl(15346, dependencyMap.paths), "DisplayNameStylesEffectPickerSheet", { userId: id, selectedEffectId: first1, onSelectEffect });
    }, items7);
    const callback6 = obj8.useCallback(() => {
      if (first1 === DisplayNameEffect.DisplayNameEffect.GUMMY) {
        ...
        ActionSheetActionCreatorsDefault.openLazy(tmp2(1896)(15347, tmp3.paths), "DisplayNameStylesGummyColorPickerSheet", obj2);
      } else {
        ...
        if (closure_10) {
          ...
          openLazy(tmp2Result(15352, tmp3.paths), "DisplayNameStylesGradientPickerSheet", obj4);
        } else {
          const obj = { selectedColor: first3, selectedEffectId: tmp, onSelectColor };
          openLazy(tmp2Result(15355, tmp3.paths), "DisplayNameStylesColorPickerSheet", obj);
        }
      }
    }, items8);
