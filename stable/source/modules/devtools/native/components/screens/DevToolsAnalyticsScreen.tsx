...
// Dependencies: [32, 19, 17, 1371, 14401, 1074, 21, 4560, 576, 4556, 5605, 10389, 4153, 9809, 5123, 7190, 4507, 504, 8880, 4973, 5687, 7201, 12155, 4518, 15577, 7050, 2]
// Exports: default
...
function LoggedEvent(arg0) {
  ...
  if (!tmp2) {
    ...
    const obj7 = { variant: "text-sm/semibold", color: "mobile-text-heading-primary", children: map(4153)(timestamp).calendar() };
    obj6.children = tmp6(CopyIcon(4556).Text, obj7);
    let items1 = [tmp6(CommonProperty, obj6), , ];
    let tmp6Result = null != user;
    if (tmp6Result) {
      const obj8 = { name: "User ", children: null };
      const obj9 = { user };
      obj8.children = tmp6(tmp10(9809), obj9);
...
export default function DevToolsAnalyticsScreen() {
  ...
  const obj4 = { icon: closure_8(trimmed(12155).ArrowsUpDownIcon, {}), label: "Reverse Events", value: first, onValueChange: tmp5[1] };
  ...
  const items3 = [closure_9(trimmed(5687).TableRowGroup, obj3), , ];
  const obj5 = { arrow: true, variant: "danger", icon: closure_8(trimmed(4518).TrashIcon, { color: "text-feedback-critical" }), label: "Clear Analytics Log", onPress: trimmed(15577).clearAnalyticsLog };
  ...
  if (0 === loggedEvents.length) {
    ...
    if (0 === found.length) {
      ...
      tmp11Result = tmp11(tmp2(5605).TableRow, obj7);
    }
  }
  ...
  items3[2] = closure_8(trimmed(5687).TableRowGroup, { title: "Analytics Events", hasIcons: false, children: tmp11Result });
  obj9.children = items3;
  obj8.ListHeaderComponent = tmp13(trimmed(4973).Stack, obj9);
  ...
  obj2.children = closure_8(trimmed(8880).FlashList, obj8);
