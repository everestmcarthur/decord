...
function ConnectionBannerIcon(state) {
  ...
  if (constants.WAITING_FOR_NETWORK === state) {
    ...
    obj4.children = closure_1_12(tmp2(16400).ConnectionUnknownIcon, obj5);
    ...
    obj.children = closure_1_12(tmp2(16402).ConnectionFineIcon, obj6);
...
function BackOnlineGlow(opacity) {
  let token;
  const tmp = closure_21();
  token = token(4262).useToken(nativeDefault.colors.ICON_FEEDBACK_POSITIVE);
  let items = [token];
  ...
  obj2.children = closure_12(_modDef5664, obj3);
