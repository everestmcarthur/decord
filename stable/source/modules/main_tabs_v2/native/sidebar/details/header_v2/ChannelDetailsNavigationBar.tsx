...
// Dependencies: [19, 17, 4200, 4201, 1957, 4741, 7877, 10921, 1074, 7878, 21, 4560, 12374, 576, 4262, 504, 1483, 8098, 1114, 9066, 7950, 12303, 12362, 4771, 7052, 10918, 8631, 7381, 16732, 4271, 4296, 4561, 16629, 4564, 11617, 5123, 5628, 2]
...
function MuteButton(channelId) {
  ...
  obj5.variant = token1;
  obj5.size = token;
  obj5.icon = tmp5(stateFromStores ? 9066 : 7950);
  return closure_15(channelId(8098).IconButton, obj5, constants.MUTE);
}
function SearchButton(channelId) {
  ...
  obj5.variant = token1;
  obj5.size = token;
  obj5.icon = _modDef7052;
  obj5.disabled = shouldHideChannelContent;
  return closure_15(channelId(8098).IconButton, obj5, constants.SEARCH);
}
function SettingsButton(channel) {
  ...
  obj4.variant = token1;
  obj4.size = token;
  obj4.icon = navigation(7381);
  return closure_15(channel(8098).IconButton, obj4, constants.SETTINGS);
}
...
let closure_14 = fn(7878).SearchEntrypointAnalyticsLocations;
const jsxProd = fn(21);
({ jsx: closure_15, jsxs: closure_16 } = jsxProd);
const createStyles = fn(4560);
let obj = { container: { position: "relative", zIndex: 1, height: fn(12374).SEARCH_BAR_HEIGHT, marginTop: nativeDefault.space.PX_8 }, navigationHeader: null, buttonsContainer: null, searchHeader: null };
let obj3 = { position: "relative", zIndex: 1, height: fn(12374).SEARCH_BAR_HEIGHT, marginTop: nativeDefault.space.PX_8 };
obj.navigationHeader = { flexDirection: "row", alignItems: "center", paddingHorizontal: nativeDefault.space.PX_16, paddingVertical: nativeDefault.space.PX_4, position: "absolute", height: fn(12374).SEARCH_BAR_HEIGHT };
let obj4 = { flexDirection: "row", alignItems: "center", paddingHorizontal: nativeDefault.space.PX_16, paddingVertical: nativeDefault.space.PX_4, position: "absolute", height: fn(12374).SEARCH_BAR_HEIGHT };
...
let closure_24 = noop.forwardRef((cleanUp, ref) => {
  ...
  let obj = width(4296);
  S.__closure = { isActive: tmp2, withTiming: width(4561).withTiming, Easing: width(4296).Easing, runOnJS: width(4296).runOnJS, cleanUp, width };
  S.__workletHash = 1270940013897;
  S.__initData = __initData;
  const animatedStyle = obj.useAnimatedStyle(S);
  let obj3 = { style: null, children: closure_15(cleanUp(16629), { ref, channelId: channel.id, guildId: channel.guild_id, onSuggestionsLayoutMesure: onSuggestionsLayoutMeasure, suggestionsDismissed, setSuggestionsDismissed, showBackButton: true }) };
  const items = [tmp.searchHeader, animatedStyle];
  obj3.style = items;
  return closure_15(cleanUp(4296).View, obj3);
});
