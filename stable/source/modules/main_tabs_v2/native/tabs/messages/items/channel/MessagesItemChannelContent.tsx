...
// Dependencies: [19, 17, 4575, 4742, 21, 4560, 576, 1178, 7931, 6969, 10150, 10959, 4269, 4495, 504, 15320, 4153, 8375, 16039, 11, 4713, 16040, 4262, 10904, 10905, 4556, 9190, 9444, 10114, 7880, 10881, 1114, 16042, 2]
...
function MessagesItemChannelContentIcon(selected) {
  ...
  if (blocked) {
    const obj2 = { source: _modDef7931, size: native.Icon.Sizes.EXTRA_SMALL, style: null };
    const items = [, ];
    ({ channelIcon: arr4[0], channelMutedIcon: arr4[1] } = tmp);
    obj2.style = items;
    let tmp2 = timestampProducer(native.Icon, obj2);
  } else if (ignored) {
    const obj3 = { source: _modDef6969, size: native.Icon.Sizes.EXTRA_SMALL, style: null };
    const items1 = [, ];
    ({ channelIcon: arr3[0], channelIgnoredIcon: arr3[1] } = tmp);
    obj3.style = items1;
    tmp2 = timestampProducer(native.Icon, obj3);
  } else if (muted) {
    const obj4 = { source: _modDef10150, size: native.Icon.Sizes.EXTRA_SMALL, style: null };
    const items2 = [, ];
    ...
    tmp2 = null;
    if (favorite) {
      const obj = { source: _modDef10959, size: native.Icon.Sizes.EXTRA_SMALL, style: null };
      const items3 = [, ];
...
export default noop.memo(function MessagesItemChannelContent(channel) {
  ...
  const isThemeLightResult = channel(4269).isThemeLight(useThemeDefault());
  ...
  if (tmp12) {
    const obj3 = tmp3(4153)();
    tmp12 = tmp3(4153)().diff(tmp11.timestamp, "hours") < 1 || !hasActivity || hasUnreadMessages;
    const tmp13 = tmp3(4153)().diff(tmp11.timestamp, "hours") < 1 || !hasActivity || hasUnreadMessages;
  }
  if (tmp12) {
    tmp12 = !tmp3(8375)(channel.id);
  }
  const tmpResult = channel(504);
  const tmpResult3 = channel(16039);
  ...
  obj4.variant = channel(4262).useToken(nativeDefault.modules.mobile.MESSAGES_ITEM_CHANNEL_NAME_TEXT_STYLE);
  const items1 = [, ];
  ...
  if (channel.isDM()) {
    if (null != channel.recipients) {
      if (channel.recipients.length > 0) {
        ...
        closure_6(tmp3(10904), obj8);
        const tmp3Result4 = tmp3(10904);
      }
    }
  }
  const obj9 = {};
  ...
  obj9.children = tmp16;
  const items2 = [closure_6(channel(4556).Text, obj9), , ];
  let tmp20Result = null;
  if (tmp15) {
    const obj10 = { userId: channel.recipients[0], disabledTooltip: true };
    tmp20Result = tmp20(tmp3(9190), obj10);
  }
  items2[1] = tmp20Result;
  let tmp20Result5 = null;
  if (channel.isSystemDM()) {
    const obj11 = { style: tmp8.botTag, type: tmp3(9444).Types.SYSTEM_DM, verified: true };
    tmp20Result5 = tmp20(tmp3(9444), obj11);
    const tmp3Result5 = tmp3(9444);
  }
  ...
  if (!hasNameplate) {
    ...
    if (!waveShouldShow) {
      ...
      tmp20Result6 = tmp20(tmp(4556).Text, obj17);
    }
    ...
    if (tmp12) {
      ...
      obj19.color = str7;
      obj19.layout = tmp(7880).ChannelListLayoutTypes.COZY_DRAWER_SMOL;
      obj19.muted = muted;
      let tmp20Result7 = tmp20(tmp(10114).ChannelRowPreview, obj19);
      ...
      tmp20Result7 = tmp20(tmp3(10881), obj20);
      const tmp3Result6 = tmp3(10881);
    } else {
      tmp20Result7 = null;
      if (tmp3(8375)(channel.id)) {
        ...
        tmp20Result7 = tmp20(tmp(4556).Text, obj21);
      }
    }
    ...
    if (waveShouldShow) {
      ...
      tmp20Result8 = tmp20(tmp3(16042), obj22);
    }
