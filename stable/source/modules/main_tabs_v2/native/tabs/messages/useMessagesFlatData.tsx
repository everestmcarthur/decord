...
// Dependencies: [19, 16048, 16060, 16098, 16099, 2]
// Exports: default

// Module 16059 (useMessagesFlatData)
...
export default function useMessagesFlatData(channels, listItemHeight) {
  ...
  return channelFavorites.useMemo(() => {
    if (renderHeader === useMessagesData.MessagesDataHeader.HappeningNow) {
      let listHeaderHeight = tmp2(16060).getMessagesItemHappeningNowHeight();
      const tmp2Result = tmp2(16060);
    } else {
      listHeaderHeight = 0;
      if (renderHeader === tmp2(16048).MessagesDataHeader.EmptyState) {
        listHeaderHeight = tmp2(16098).MESSAGES_ITEM_EMPTY_STATE_HEIGHT;
      }
    }
    const listData = [];
    ...
    if (sections[useMessagesData.MessagesDataSections.Separator] > 0) {
      ...
      sum2 = tmp12 + tmp17(16099).MESSAGES_ITEM_SEPERATOR_HEIGHT;
    }
