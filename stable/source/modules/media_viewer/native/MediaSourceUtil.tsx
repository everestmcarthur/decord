...
// Dependencies: [19, 17, 1957, 1074, 4710, 1384, 7292, 7297, 1476, 8265, 7329, 7889, 4897, 7947, 4785, 1894, 8111, 8127, 1369, 8266, 1093, 4258, 8267, 4905, 1114, 8268, 2]
...
import renderMessageMarkupDefault from "renderMessageMarkup" /* 7889 */;
import renderer_EmbedUtils from "renderer/EmbedUtils" /* 7947 */;
import transformMessageComponents from "transformMessageComponents" /* 8111 */;
import RowGeneratorTypes from "RowGeneratorTypes" /* 8127 */;
import useStateFromSharedValueDefault from "useStateFromSharedValue" /* 8266 */;
import NativePortalView from "NativePortalView" /* 8268 */;
        ...
        if (found.height > 0) {
          ...
          if (null != found.proxy_url) {
            ...
            if (isVideoFileResult) {
              ...
              if (!hasFlagResult) {
                hasFlagResult = tmp43(7329).isChannelSpoilerGated(ChannelStore.getChannel(message2.channel_id));
                const tmp43Result5 = tmp43(7329);
              }
              ...
              const formatted = tmp(8265)(found).toLowerCase();
              const str3 = tmp(8265)(found);
              const endsWithResult = formatted.endsWith(".webp");
              const formatted1 = tmp(8265)(found).toLowerCase();
              const endsWithResult1 = formatted1.endsWith(".avif");
              const str5 = tmp(8265)(found);
...
function toMediaSourceFromUnfurledMedia(id, guildId, media, description, spoiler) {
  ...
  if (unfurledMediaItemType === RowGeneratorTypes.MediaGalleryItemType.VISUAL_PLACEHOLDER) {
    ...
    const tmp4 = unfurledMediaItemType === tmp(8127).MediaGalleryItemType.VIDEO;
    let str;
    if (tmp4) {
      str = "png";
    }
    ...
    const enabledHarmTypesForMessage = tmp(7292).getEnabledHarmTypesForMessage(id);
    const tmpResult = tmp(7292);
    const obj2 = { type: tmp(7297).ObscuredMediaTypes.GenericMedia, media };
    ...
    const result = tmp(7292).isMediaObscuredForHarmTypes(obj2, enabledHarmTypesForMessage);
    if (width2 == null) {
      width2 = 0;
    }
    size.width = width2;
...
export const downloadMediaAsset = function downloadMediaAsset(mediaUrl, VIDEO) {
  ...
  return MediaManager.downloadMediaAsset(mediaUrl, VIDEO).then(() => {
    if (ConstantsIOS.MediaType.IMAGE === closure_0) {
      tmp2(4258).presentImageSaved();
      const tmp2Result = tmp2(4258);
    } else if (tmp2(1093).MediaType.GIF === tmp) {
      tmp2(4258).presentGifSaved();
      const tmp2Result3 = tmp2(4258);
    } else if (tmp2(1093).MediaType.VIDEO === tmp) {
      tmp2(4258).presentVideoSaved();
      const tmp2Result4 = tmp2(4258);
    }
    const MediaViewerAnalytics = tmp2(8267).MediaViewerAnalytics;
...
export const isAnimatedImageSource = function isAnimatedImageSource(source) {
  ...
  if (!result) {
    ...
    if (result1) {
      result1 = tmp(4710).urlMatchesFileExtension(source.sourceURI, React5);
  ...
  if (!result) {
    ...
    if (result2) {
      result2 = tmp(4710).urlMatchesFileExtension(source.sourceURI, re8);
