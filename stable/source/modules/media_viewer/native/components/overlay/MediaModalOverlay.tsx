...
// Dependencies: [32, 19, 17, 21, 4560, 576, 7123, 9056, 1114, 12963, 10016, 12964, 12965, 12966, 8272, 11514, 1115, 4965, 12967, 8294, 12969, 4296, 12973, 8271, 12974, 12975, 12968, 8263, 8264, 2]
// Exports: default
...
import useVideoControls from "useVideoControls" /* 8271 */;
...
function MediaModalOverlay(getVideoControls) {
  ...
  const tmp2 = videoControls(8271)(index, source, videoControls);
  const items = [index];
  ...
  obj2.obscure = flag;
  obj2.spoilerActive = _slicedToArray(index(12968).useMediaItemSpoilerState(index), 1)[0];
...
let closure_13 = noop.memo((arg0) => {
  ...
  if (null != slider) {
    ...
    items[2] = tmp17;
    rect.children = items;
    let tmp22Result5 = React6(tmp2(7123).SafeAreaPaddingView, rect);
  ...
  if (null != slider) {
    ...
    if (tmp22Result3) {
      const obj11 = { isMuted: mediaPlayerMutedStore, onToggleMute: tmp2(8271).toggleMuted };
  ...
  if (null != mediaModalFooterActionStore) {
    ...
    tmp22Result5 = tmp22(tmp2(12974).MediaModalOverlayFooterAction, obj13);
  } else if (disableMediaOverlayFooter) {
    const obj14 = { bottom: true, style: tmp.invisibleFooter };
    tmp22Result5 = tmp22(tmp2(7123).SafeAreaPaddingView, obj14);
    ...
    tmp22Result5 = tmp22(tmp2(12975).MediaModalOverlayFooter, obj15);
  }
