...
// Dependencies: [32, 19, 17, 21, 4560, 576, 1178, 12011, 12012, 11250, 4556, 1477, 1369, 12013, 7275, 7898, 12017, 2]
// Exports: default
...
function ExtraMediaIcon(extraMediaCount) {
  ...
  items = [timestampProducer(native.Icon, { source: _modDef11250, color: tmp.icon.color, size: native.Icon.Sizes.REFRESH_SMALL_16 }), ];
  const obj2 = { source: _modDef11250, color: tmp.icon.color, size: native.Icon.Sizes.REFRESH_SMALL_16 };
...
export default function ForumPostGridBody(thread) {
  ...
  const tmp = closure_8();
  dependencyMap = tmp;
  const tmp4 = _slicedToArray(thread(7275).useSomeAppliedTags(thread, 2), 2);
  const first = tmp4[0];
  ...
  if (tmp13Result) {
    ...
    obj4.children = tmp13(tmp2(12017).ForumPostAppliedTagPills, obj5);
