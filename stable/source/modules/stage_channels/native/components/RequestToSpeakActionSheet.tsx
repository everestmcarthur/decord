...
// Dependencies: [32, 19, 17, 502, 1957, 5414, 21, 4560, 576, 9922, 7201, 1114, 504, 4707, 9923, 9925, 5605, 5422, 8412, 8414, 4527, 8399, 1178, 9927, 4262, 7162, 7182, 5431, 5425, 7150, 6627, 4973, 5687, 4556, 9928, 2]
// Exports: default

// Module 9921 (RequestToSpeakActionSheet)
...
import useRequestToSpeakPermission from "useRequestToSpeakPermission" /* 9922 */;
import _slicedToArray from "module_32" /* 32 */;
import noop from "module_19" /* 19 */;
import AuthenticationStore from "AuthenticationStore" /* 502 */;
import ChannelStore from "ChannelStore" /* 1957 */;

const _modDef9927 = tmp4(9927);
require = fn;
...
function ManageSelfSpeakerRow(channel) {
  ...
  if (tmp8) {
    let MicrophoneArrowRightIcon = tmp(9923).GroupArrowDownIcon;
  } else {
    MicrophoneArrowRightIcon = tmp(9925).MicrophoneArrowRightIcon;
  }
  let obj2 = {
    onPress() {
      if (!closure_1) {
  ...
  obj2.trailing = closure_9(channel(1178).Icon, { source: _modDef9927 });
  return closure_9(channel(5605).TableRow, obj2);
}
...
export default function RequestToSpeakActionSheet(channelId) {
  ...
  let first1;
  noop = undefined;
  const token = channelId(4262).useToken(first(576).modules.mobile.TABLE_ROW_PADDING);
  const obj = channelId(4262);
  const tmp3 = first;
  const items = [];
  ...
  const stageParticipantsCount = channelId(5431).useStageParticipantsCount(channelId, channelId(5425).StageChannelParticipantNamedIndex.ALL_REQUESTED_TO_SPEAK);
  ...
  if (null != stateFromStores) {
    ...
    const items3 = [closure_10(tmp(5687).TableRowGroup, obj9), ];
    ...
    obj12.children = closure_9(tmp(4556).Text, obj14);
    items3[1] = closure_9(View, obj12);
    obj8.children = items3;
    const items4 = [closure_10(tmp(4973).Stack, obj8), ];
    const obj16 = { channel: stateFromStores, height: null };
    const _Math = Math;
    obj16.height = Math.max(first1 - first - 8, 0);
    items4[1] = closure_9(tmp3(9928), obj16);
    obj7.children = items4;
    obj6.children = closure_10(tmp(4973).Stack, obj7);
    obj5.children = closure_9(tmp(6627).BottomSheetScrollView, obj6);
    obj4.children = closure_9(tmp(7150).BottomSheet, obj5);
    tmp14 = closure_9(tmp(7162).AnalyticsLocationProvider, obj4);
    const tmp3Result = tmp3(9928);
  }
  return tmp14;
};
