...
// Dependencies: [32, 19, 1979, 21, 504, 9668, 9676, 9677, 9679, 9680, 9671, 9681, 1114, 9703, 9904, 2]
// Exports: default

// Module 9667 (ModeratorStartStageView)
...
const StageViewWithPromptsDefault = tmp5(9681);
const require = fn;
const jsxProd = fn(21);
...
export default function ModeratorStartStageView(channel) {
  ...
  const first = _slicedToArray(guild_id(9668).useGuildChannelScheduledEvents(channel.id), 1)[0];
  const obj2 = guild_id(9668);
  const obj3 = guild_id(9677);
  const canManageGuildEventResult = guild_id(9677).useManageResourcePermissions(channel).canManageGuildEvent(first);
  const tmp7 = useCanCreateAnEventDefault(guild_id);
  const isLive = guild_id(9680).useStageChannelStartEvent(channel.id).isLive;
  const obj4 = guild_id(9680);
  const nextRecurrenceIdInEvent = guild_id(9671).getNextRecurrenceIdInEvent(first);
  ...
  if (null != stateFromStores) {
    ...
    if (canManageGuildEventResult) {
      ...
      if (null != first) {
        ...
        tmp10Result = tmp10(tmp(9703).StartEventPrompt, obj7);
      }
    }
    ...
    if (useCurrentUserStageRolesDefault(channel.id, true).moderator) {
      ...
      tmp10Result4 = tmp10(tmp(9904).StartStagePrompt, obj8);
    }
    items2[1] = tmp10Result4;
    let tmp10Result5 = null;
    if (tmp7) {
      const obj9 = { channel, isLive, guild: stateFromStores };
      tmp10Result5 = tmp10(tmp(9703).ScheduleEventPrompt, obj9);
    }
    const obj10 = { children: null };
    items2[2] = tmp10Result5;
    const obj11 = { onContinue: channel.onSkip };
    items2[3] = closure_5(tmp(9904).ContinueToStagePrompt, obj11);
