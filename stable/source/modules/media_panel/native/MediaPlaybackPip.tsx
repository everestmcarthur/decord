...
// Dependencies: [32, 19, 17, 1957, 4781, 4209, 1371, 1074, 17091, 21, 4560, 576, 4262, 504, 4713, 8265, 4556, 17229, 4987, 7456, 7244, 4296, 4561, 1114, 8908, 5628, 4513, 1242, 14539, 4186, 17226, 8277, 8275, 15061, 2]
// Exports: default
...
function MediaInfo(message) {
  ...
  const token = message(4262).useToken(first(576).colors.BACKGROUND_SURFACE_HIGH);
  ...
  if (null != message) {
    if (null != contentMessage) {
      if (null != activeMediaPlayerSource) {
        if (isVoiceMessage) {
          ...
          if (contentMessage.attachments.length > 0) {
            ...
            if (null != activeMediaPlayerSource.attachmentIndex) {
              str2 = tmp4(8265)(contentMessage.attachments[activeMediaPlayerSource.attachmentIndex]);
            }
          }
        }
        const obj3 = {
        ...
        const tmp14 = closure_15(tmp2(4556).Text, obj3);
        const obj4 = {
        ...
        if (memo) {
          ...
          const items3 = [tmp13(tmp2(17229).Marquee, obj7), ];
        ...
        if (tmp13Result) {
          ...
          tmp13Result = tmp13(tmp2(4556).Text, obj9);
        }
...
function PiPControls(message) {
  ...
  fn.__closure = { withTiming: message(4561).withTiming, visible };
  ...
  obj3.style = items1;
  const items2 = [closure_15(message(8908).BackgroundBlurFill, { blurAmount: 0.05 }), , ];
  const obj4 = { disabled: !visible, style: null, onPress: callback, accessible: true, accessibilityRole: "button", accessibilityLabel: stringResult, children: closure_15(message(5628).ArrowLargeLeftIcon, { size: "sm" }) };
  const items3 = [, ];
  ({ pipButton: arr4[0], backButton: arr4[1] } = tmp);
  obj4.style = items3;
  items2[1] = closure_15(closure_6, obj4);
  const obj5 = { disabled: !visible, style: null, onPress: message.handleClosePip, accessible: true, accessibilityRole: "button", accessibilityLabel: string2Result, children: closure_15(message(4513).XLargeIcon, { size: "sm" }) };
  const items4 = [, ];
  ({ pipButton: arr5[0], dismissButton: arr5[1] } = tmp);
  obj5.style = items4;
  items2[2] = closure_15(closure_6, obj5);
  obj3.children = items2;
  return closure_16(visible(4296).View, obj3);
}
...
export default function MediaPlaybackPip() {
  ...
  const memo = obj.useMemo(() => {
    if (progress) {
      let PlayIcon = tmp2(8277).PauseIcon;
    } else {
      PlayIcon = tmp2(8275).PlayIcon;
    }
