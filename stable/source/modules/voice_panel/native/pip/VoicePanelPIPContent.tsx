...
// Dependencies: [32, 19, 17, 1956, 4576, 9492, 4582, 502, 1957, 1371, 12277, 17091, 1074, 1920, 4581, 21, 4560, 576, 4296, 4987, 12276, 17085, 9623, 1109, 4271, 17095, 5589, 17090, 563, 1477, 9641, 8832, 8252, 9611, 9628, 1178, 9631, 12279, 7073, 17007, 9616, 9604, 9607, 2]

// Module 17168 (VoicePanelPIPContent)
...
import useProfileTileGradientDefault from "useProfileTileGradient" /* 8252 */;
import ExternalPipDefault from "ExternalPip" /* 9616 */;
import VideoRendererNativeComponentDefault from "VideoRendererNativeComponent" /* 9623 */;
import VoicePanelStateContextDefault from "VoicePanelStateContext" /* 12276 */;
import VoicePanelCardLayoutManager from "VoicePanelCardLayoutManager" /* 12279 */;
import VideoActionCreators from "VideoActionCreators" /* 17007 */;
import VoicePanelStreamOutputSinkStack from "VoicePanelStreamOutputSinkStack" /* 17085 */;
...
function User(participantId) {
  ...
  dominantColorFromImage = participantId(8832).useDominantColorFromImage(avatarURL);
  let id;
  const tmp3Result8 = participantId(8832);
  if (user1 != null) {
    id = user1.id;
  }
  ...
  const surfaceDirectRendererExperiment = participantId(9611).useSurfaceDirectRendererExperiment(id1, { location: "VoicePanelPIPContent" });
  ...
  const tmp3Result9 = participantId(9611);
  class S {
    constructor() {
      value = focused.get();
      id = undefined;
  ...
  const tmp3Result10 = participantId(4296);
  fn.__closure = { runOnJS: participantId(4296).runOnJS, updateIsActivityFocused, mode };
  fn.__workletHash = 5971237403457;
  fn.__initData = __initData4;
  const animatedReaction = tmp3Result10.useAnimatedReaction(S, fn);
  const obj2 = { runOnJS: participantId(4296).runOnJS, updateIsActivityFocused, mode };
  ...
  const tmp3Result11 = participantId(4296);
  I.__closure = { runOnJS: participantId(4296).runOnJS, updateIsActivityFocused, focused };
  I.__workletHash = 12552370107483;
  I.__initData = __initData6;
  const animatedReaction1 = tmp3Result11.useAnimatedReaction(fn2, I);
  const obj3 = { runOnJS: participantId(4296).runOnJS, updateIsActivityFocused, focused };
  const items4 = [c9];
  ...
  canRenderParticipantVideo = participantId(9628).useCanRenderParticipantVideo(stateFromStores);
  ...
  const tmp3Result13 = participantId(9628);
  if (canRenderParticipantVideo) {
    const items7 = [, ];
    ...
    if (null != tmp6ResultResult) {
      const obj7 = { colors: tmp6ResultResult, start: { x: 0, y: 0 }, end: { x: 0, y: 1 }, style: layoutManager.absoluteFill, layout: tmp3(17090).layoutTransition, pointerEvents: "none" };
    ...
    if (null != user1) {
      const obj9 = { source: tmp3(8832).getCachedSourceFromURI(user1.getAvatarURL(guildId, 80, false)), size: null, avatarDecoration: null };
      ...
      const tmp3Result14 = tmp3(8832);
    }
    obj8.children = tmp35Result;
    items10[1] = closure_22(tmp6(5589), obj8);
    ...
    if (tmp18) {
      ...
      if (consumedRequestToRespondToSeriousThermalState) {
        ...
        const obj12 = { style: tmp.thermalAlertIcon, source: tmp6(9631), color: tmp.thermalAlertIcon.color };
        obj11.children = tmp35(tmp3(1178).Icon, obj12);
        tmp35Result2 = tmp35(tmp6(5589), obj11);
        const tmp6Result6 = tmp6(5589);
      }
    }
    items10[3] = tmp35Result2;
    obj4.children = items10;
    tmp42 = obj4;
    const tmp6Result5 = tmp6(5589);
  }
...
function Stream(participantId) {
  ...
  let callback2;
  let tmp = closure_24();
  const mode = participantId(17095).usePIPState().mode;
  closure_129_0 = transitionState;
  closure_129_1 = transitionCleanUp;
  const items = [transitionState, transitionCleanUp];
  ...
  const context = obj2.useContext(mode(12276));
  ...
  const surfaceDirectRendererExperiment = participantId(9611).useSurfaceDirectRendererExperiment(stateFromStoresObject.userId, { location: "VoicePanelPIPContent.Stream" });
  const tmp2Result3 = participantId(9611);
  ...
  if (stateFromStores === constants.ENDED) {
    ...
    let tmp18Result = closure_22(tmp2(9604).StreamEnded, obj3);
  } else if (stateFromStores === tmp17.FAILED) {
    const obj4 = { style: tmp.streamEmptyImage, resizeMode: "contain" };
    tmp18Result = closure_22(tmp2(9607).StreamFailed, obj4);
  } else if (tmp15) {
    const obj5 = { style: tmp5, participantId, children: null };
