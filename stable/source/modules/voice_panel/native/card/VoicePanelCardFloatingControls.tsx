...
// Dependencies: [19, 17, 4552, 1956, 4582, 4583, 12277, 12275, 12280, 1074, 4581, 21, 4296, 1178, 4560, 576, 12276, 17111, 17156, 10069, 17157, 17109, 1114, 9992, 9126, 9125, 5589, 6610, 4974, 17107, 4702, 4612, 9521, 17038, 17158, 17159, 504, 17141, 7073, 17160, 4271, 17161, 9185, 8684, 5111, 8682, 8692, 9190, 17162, 4962, 7162, 8180, 9137, 4794, 9182, 4556, 9218, 17084, 2]
...
import useShouldOpenGameProfileModal from "useShouldOpenGameProfileModal" /* 8682 */;
...
const useStableParticipant = tmp(17107);
require = fn;
...
function AnimatedLabelIcon(icon) {
  ...
  if (useVoicePanelCardUserStateIcons.VoicePanelCardUserStateIconType.STREAM_ICON === type) {
  ...
  } else if (tmp2(17109).VoicePanelCardUserStateIconType.SPEAKER_MUTE_ICON === type) {
    ...
    obj3.children = value2(tmp2(9992).VoiceXIcon, obj4);
    return value2(Pressable, obj3);
  } else if (tmp2(17109).VoicePanelCardUserStateIconType.USER_VIDEO_ICON === type) {
    if (icon.videoIconState === tmp2(9126).VideoIconState.VIDEO_DISABLED_LOCAL_AUTO) {
    ...
    if (null != icon.onPress) {
      ...
      obj5.children = value2(tmp2(9125).VideoIcon, obj6);
      ...
      obj7.children = value2(tmp2(9125).VideoIcon, obj8);
      tmp16 = value2(NativeViewDefault, obj7);
    }
    return tmp16;
  } else if (tmp2(17109).VoicePanelCardUserStateIconType.MUTE_DEAFEN_ICON === type) {
    ...
    obj10.children = value2(tmp2(9125).MuteDeafenIcon, obj11);
    obj9.children = value2(Pressable, obj10);
    return value2(NativeViewDefault, obj9);
  } else if (tmp2(17109).VoicePanelCardUserStateIconType.USER_DISCONNECTED_ICON === type) {
    ...
    obj.children = value2(tmp2(6610).CircleErrorIcon, obj12);
...
let closure_27 = noop.memo((participant) => {
  ...
  let obj = participant(4296);
  fn.__closure = { controlsHidden, FLOATING_BAR_HEIGHT: v28, VOICE_PANEL_CARD_INNER_PADDING, withSpring: participant(4974).withSpring, MODE_CHANGE_PHYSICS };
  ...
  const callback = noop.useCallback(() => {
    if (obj.isStableStreamParticipant(participant)) {
      ...
      if (null != streamForUser) {
        ...
        tmpResult.stopStream(tmp(4612).encodeStreamKey(streamForUser));
  ...
  let obj2 = { controlsHidden, FLOATING_BAR_HEIGHT: v28, VOICE_PANEL_CARD_INNER_PADDING, withSpring: participant(4974).withSpring, MODE_CHANGE_PHYSICS };
  const tmp3 = closure_16;
  obj3.icon = guildId(17158);
...
let closure_39 = noop.memo((arg0) => {
  ...
  noop = undefined;
  const showGameTag = gameRecord(17161).useConfig({ location: "VoicePanelCardFloatingControls" }).showGameTag;
  const first = gameRecord(9185)(userId, arg0.guildId, showGameTag)[0];
  let tmp5;
  ...
  gameRecord = gameRecord(8684)({ applicationId: tmp5 }).gameRecord;
  ...
  if (showGameTag) {
    if (null != gameRecord) {
      if (!tmp8) {
        ...
        let tmp12 = closure_16(tmp(17162), obj2);
      }
      return tmp12;
    }
  }
  tmp12 = closure_16(tmp(9190), { userId, textColor });
});
