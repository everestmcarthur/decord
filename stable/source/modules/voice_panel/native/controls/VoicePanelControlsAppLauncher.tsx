...
// Dependencies: [19, 1957, 12275, 1074, 21, 4560, 7873, 12276, 8266, 504, 38, 12283, 1109, 11220, 9415, 4296, 12086, 12284, 1114, 2]

// Module 17223 (VoicePanelControlsAppLauncher)
...
export default noop.memo(function VoicePanelControlsAppLauncher(gestureSpecs) {
  ...
  const gradientTop = gestureSpecs(7873).useGradientTop();
  const context = noop.useContext(setControlsMode(12276));
  setControlsMode = context.setControlsMode;
  ({ channelId: c2, safeArea, windowDimensions } = context);
  const tmp4 = setControlsMode(8266)(windowDimensions);
  noop = tmp4;
  const rect = setControlsMode(8266)(safeArea);
  const obj = gestureSpecs(7873);
  const items = [rect];
  ...
  const controlsDrawerOpenWidth = gestureSpecs(12283).getControlsDrawerOpenWidth(tmp4.width, rect.left, rect.right);
  const callback = noop.useCallback(() => {
    const ComponentDispatch = ComponentDispatchUtils.ComponentDispatch;
    ComponentDispatch.dispatch(ComponentActions.SELECT_ACTIVITY);
    setControlsMode({ mode: hasOwnProperty.HIDDEN });
  }, items1);
  const obj3 = gestureSpecs(12283);
  const appLauncherChatInputRefDummy = gestureSpecs(11220).useAppLauncherChatInputRefDummy({ noop: true });
  const obj4 = gestureSpecs(11220);
  const ref = noop.useRef(gestureSpecs(11220).AppLauncherKeyboardCloseReason.DISMISSED);
  ...
  items3[1] = closure_7(setControlsMode(12284), obj13);
  obj11.children = items3;
  obj10.children = closure_8(setControlsMode(4296).View, obj11);
