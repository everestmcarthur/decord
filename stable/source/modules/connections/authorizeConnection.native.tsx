...
// Dependencies: [5408, 1074, 573, 4527, 9223, 9259, 9276, 4763, 9287, 1896, 5283, 9289, 8371, 4255, 5406, 2]
// Exports: default

// Module 9258 (authorizeConnection)
...
export default function authorizeConnection(overrideUrl) {
  ...
  if (platformType === PlatformTypes.XBOX) {
    ...
    overrideUrl(9223).showModal(items);
    if (null != onClose) {
    ...
    if (platformType !== tmp.PLAYSTATION) {
      if (platformType !== tmp.PLAYSTATION_STAGING) {
        if (platformType === tmp.CRUNCHYROLL) {
          ...
          overrideUrl(9276).showModal(items1);
          if (null != onClose) {
          ...
          overrideUrl(4763).pushLazy(handleModalClose5(1896)(9287, dependencyMap.paths), obj);
          if (null != onClose) {
          ...
          if (true === isFederated) {
            tmp28(4527).hideActionSheet();
            const tmp28Result = tmp28(4527);
            const obj2 = { platformType, location: _location, successRedirect };
            tmp28(4763).pushLazy(handleModalClose5(1896)(9289, tmp29.paths), obj2);
            if (null != onClose) {
            ...
            if (null != overrideUrl) {
              if (platformType === closure_3) {
                ...
                handleModalClose5(8371).handleClick(obj4);
                const obj3 = handleModalClose5(8371);
              }
            }
            const obj5 = { location: _location, successRedirect };
            const tmp28Result6 = tmp28(5406);
            tmp28(5406).authorize(platformType, obj5).then((body) => {
            ...
            const authorizeResult = tmp28(5406).authorize(platformType, obj5);
          }
    ...
    overrideUrl(9259).showModal(items3, platformType);
    if (null != onClose) {
