...
// Dependencies: [32, 19, 17, 1074, 1373, 21, 8763, 1114, 10119, 12536, 5101, 5097, 10386, 13379, 9377, 4503, 13592, 4560, 576, 13586, 5441, 4296, 4561, 4556, 4495, 4411, 4987, 5123, 4454, 13593, 13595, 4409, 1178, 13597, 13598, 5581, 12568, 2]
// Exports: default
...
let items = [
  {
    orderCollapsed: 0,
    isIncluded: true,
    IconComponent: fn(8763).ReactionIcon,
    getCopy() {
...
items[7] = {
  isIncluded: false,
  IconComponent: fn(9377).UploadIcon,
  getCopy() {
    const intl = util.intl;
...
obj37.cardsScroller = { flex: 1, marginTop: fn(13586).PROGRESS_BAR_SPACING };
...
const obj41 = { flex: 1, marginTop: fn(13586).PROGRESS_BAR_SPACING };
obj37.sparkleStar = { position: "absolute", tintColor: fn(5441).DARK_WHITE_500_LIGHT_GUILD_BOOSTING_PINK };
...
let closure_16 = noop.forwardRef((onCardPress, ref) => {
  ...
  if (!tmp16) {
    ...
    if (tmp11Result) {
      ...
      const tmp2Result4 = tmp2(4987);
      const items7 = [tmp7(4409).hexWithOpacity(tmp9, 0), , ];
      const tmp7Result7 = tmp7(4409);
      items7[1] = tmp7(4409).hexWithOpacity(tmp9, 1);
      const tmp7Result8 = tmp7(4409);
      ...
      const tmp7Result9 = tmp7(4409);
      const tmp2Result5 = tmp2(4987);
      const items10 = [tmp7(4409).hexWithOpacity(tmp9, 0), , ];
      const tmp7Result10 = tmp7(4409);
      items10[1] = tmp7(4409).hexWithOpacity(tmp9, 1);
      const tmp7Result11 = tmp7(4409);
    ...
    obj22 = tmp13(tmp7(4556).Text, obj22);
    obj21.children = obj22;
    tmp13(tmp2(4987), obj21);
    const tmp2Result6 = tmp2(4987);
  }
});
const size = fn(2);
