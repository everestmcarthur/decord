...
// Function ID: 13286
// Name: ForLaterScreen
// Dependencies: [32, 19, 17, 11665, 21, 4560, 576, 4296, 4974, 13286, 7861, 7855, 504, 7162, 7182, 8774, 1250, 1090, 13288, 13294, 8880, 13298, 2]

// Module 13285 (ForLaterScreen)
...
import useSavedMessagesForPageDefault from "useSavedMessagesForPage" /* 13286 */;
import ForLaterMessageCardDefault from "ForLaterMessageCard" /* 13288 */;
...
function ForLaterPage(type) {
  ...
  const tmp5 = type === throttledNow(7861).SavedMessageSortTypes.REMINDER;
  const forLaterLimit = throttledNow(7855).useForLaterLimit(ForLaterScreen, tmp5);
  const obj = throttledNow(7855);
  const isForLaterLimitUpgradable = throttledNow(7855).useIsForLaterLimitUpgradable(ForLaterScreen);
  ...
  const analyticsLocations = useAnalyticsLocationsDefault(tmp2(7182).FOR_LATER_POPOUT).analyticsLocations;
  ...
  if (0 === arr.length) {
    ...
    const obj6 = { type };
    obj5.children = closure_7(tmp2(13294), obj6);
    let tmp24Result = closure_7(tmp4(7162).AnalyticsLocationProvider, obj5);
  } else {
    ...
    obj8.children = closure_7(tmp4(8880).FlashList, obj9);
    const items2 = [closure_7(View, obj8), ];
    let tmp25Result = null;
    if (tmp9) {
      const obj10 = { isReminder: tmp5, isAtLimit: tmp8 };
      tmp25Result = tmp25(tmp2(13298), obj10);
    }
    items2[1] = tmp25Result;
    obj7.children = items2;
    tmp24Result = closure_8(tmp4(7162).AnalyticsLocationProvider, obj7);
    tmp25 = closure_7;
  }
  return tmp24Result;
}
