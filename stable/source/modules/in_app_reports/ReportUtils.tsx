...
// Dependencies: [5, 19, 4559, 1074, 8400, 1272, 8638, 8405, 573, 38, 8636, 4740, 2]
...
// Module 8639 (in_app_reports/ReportUtils)
...
import ReportMenuType from "ReportMenuType" /* 8638 */;
...
function genSubmitData(version, name, arr, email_token) {
  ...
  let obj2 = { channel_id: "names", message_id: "accessible", stage_instance_id: "useRef", guild_id: "match", guild_scheduled_event_id: "r", user_id: "isArray", email_token: "call", application_id: "__packager_asset", entrypoint: "alignItems", widget_id: "accessible" };
  obj.language = str;
  ...
  if (name.name !== MenuTypes.ReportNames.MESSAGE) {
    if (name.name !== tmp(8636).ReportNames.FIRST_DM) {
      if (name.name !== tmp(8636).ReportNames.GUILD) {
        if (name.name !== tmp(8636).ReportNames.GUILD_DISCOVERY) {
          if (name.name === tmp(8636).ReportNames.GUILD_DIRECTORY_ENTRY) {
            const obj3 = {};
            ...
            return obj3;
          } else if (name.name === tmp(8636).ReportNames.STAGE_CHANNEL) {
            const obj6 = {};
            ...
            return obj6;
          } else if (name.name === tmp(8636).ReportNames.GUILD_SCHEDULED_EVENT) {
            const obj7 = {};
            ...
            if (name.name === tmp(8636).ReportNames.USER) {
              const obj8 = {};
              ...
              let tmp3 = obj8;
            } else if (name.name === tmp(8636).UnauthenticatedReportNames.USER) {
              const obj9 = {};
              ...
              tmp3 = obj9;
            } else if (name.name === tmp(8636).UnauthenticatedReportNames.MESSAGE) {
              const obj10 = {};
              ...
              tmp3 = obj10;
            } else if (name.name === tmp(8636).UnauthenticatedReportNames.GUILD) {
              const obj11 = {};
              ...
              tmp3 = obj11;
            } else if (name.name === tmp(8636).ReportNames.APPLICATION) {
              const obj12 = {};
              ...
              tmp3 = obj12;
            } else if (name.name === tmp(8636).ReportNames.WIDGET) {
              const obj13 = {};
              ...
              tmp3 = null;
              if (name.name === tmp(8636).UnauthenticatedReportNames.MEDIA_TAKEDOWN) {
                const obj14 = {};
...
export const submitReport = function submitReport(language, name, arr) {
  if (DevSettingsStore.get("iar_skip_api_report_submit")) {
    ...
    const REPORT_TO_MOD = obj4(8638).ReportMenuTypeSets.REPORT_TO_MOD;
    if (REPORT_TO_MOD.has(name.name)) {
      ...
      let tmp15 = null;
      if (name.name === tmp4(8636).ModeratorReportNames.MESSAGE) {
        let obj2 = { channel_id: "Array", message_id: "accessible", guild_id: "then" };
        obj4 = {};
      ...
      const REPORT_TO_MOD2 = tmp4(8638).ReportMenuTypeSets.REPORT_TO_MOD;
      ...
      name = name.name;
      let _Object = Object;
      const values = Object.values(tmp4(8636).ReportNames);
...
export const trackCloseReportModalAnalytics = function trackCloseReportModalAnalytics(name, c12, onSubmit) {
  ...
  if (name.name !== MenuTypes.ReportNames.GUILD) {
    if (name.name !== tmp2(8636).ReportNames.GUILD_DISCOVERY) {
      if (name.name === tmp2(8636).ReportNames.GUILD_DIRECTORY_ENTRY) {
        let id2 = name.record.guildId;
      } else if (name.name === tmp2(8636).ReportNames.GUILD_SCHEDULED_EVENT) {
        id2 = name.record.guild_id;
      }
    }
    obj2.guild_id = id2;
    if (name.name === tmp2(8636).ReportNames.GUILD_SCHEDULED_EVENT) {
      let channelId = name.record.channel_id;
    } else if (name.name === tmp2(8636).ReportNames.GUILD_DIRECTORY_ENTRY) {
      channelId = name.record.channelId;
    }
    obj2.channel_id = channelId;
    let id4;
    if (name.name === tmp2(8636).ReportNames.APPLICATION) {
      id4 = name.record.id;
    }
