...
export default function ThreadNotificationsBottomSheet(channel) {
  ...
  const threadNotificationSetting = channel(10093).useThreadNotificationSetting(channel);
  const obj2 = { header: null, children: null };
  const obj3 = { title: null };
  const intl = channel(1114).intl;
  obj3.title = intl.string(channel(1114).t.h850Ss);
  obj2.header = jsx(channel(7149).BottomSheetTitleHeader, { title: null });
  const obj4 = {
    hasIcons: false,
  ...
  obj2.children = jsx(channel(5685).TableRadioGroup, {
    hasIcons: false,
    value: threadNotificationSetting,
    onChange(flags) {
      return ThreadActionCreatorsDefault.setNotificationSettings(channel, { flags });
  ...
  return jsx(channel(7198).ActionSheet, { header: null, children: null });
};
