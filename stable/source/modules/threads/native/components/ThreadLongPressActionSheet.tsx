...
// Dependencies: [19, 1957, 1979, 4575, 4579, 4201, 1074, 21, 10248, 1114, 10249, 10251, 6970, 7110, 10227, 4501, 7765, 10037, 7881, 4523, 4513, 10253, 5095, 10255, 8631, 4503, 10960, 9065, 4527, 10149, 1896, 9782, 10966, 11317, 504, 7269, 12, 7904, 4713, 1935, 10979, 5584, 1178, 7198, 11003, 7200, 10634, 7190, 4258, 2]
// Exports: default

// Module 16118 (ThreadLongPressActionSheet)
...
function ThreadLongPressActionSheetConnected(channel) {
  ...
  const canMarkChannelUnread = channel(10251).useCanMarkChannelUnread(channel);
  const obj4 = channel(10251);
  const canManageThread = channel(7269).useCanManageThread(channel);
  const obj5 = channel(7269);
  const isThreadModerator = channel(7269).useIsThreadModerator(channel);
  const obj6 = channel(7269);
  const canUnarchiveThread = channel(7269).useCanUnarchiveThread(channel);
  const obj7 = channel(7269);
  const canJoinThreadVoice = channel(7269).useCanJoinThreadVoice(channel);
  const obj8 = channel(7269);
  ...
  const tmp14 = onClose(7904)(channel);
  ...
  if (null != stateFromStores) {
    const obj11 = { guild: stateFromStores, size: tmp(5584).GuildIconSizes.LARGE };
    let tmp19 = closure_13(tmp13(5584), obj11);
  ...
  if (MarkChannelUnreadExperiment.getConfig({ location: "thread_action_sheet" }).enabled) {
    if (!stateFromStores1) {
      if (canMarkChannelUnread) {
        ...
        obj14.IconComponent = tmp(10249).ChatMarkUnreadIcon;
      ...
      if (hasJoined) {
        ...
        const obj18 = {
          label: stringResult,
          IconComponent: tmp(10037).GroupPlusIcon,
          onPress() {
        ...
        if (null == tmp23) {
          ...
          if (isThreadModerator) {
            ...
            if (isLockedThreadResult) {
              ...
              obj21.IconComponent = tmp(10253).LockUnlockedIcon;
          ...
          if (!isThreadModerator) {
            ...
            obj22.IconComponent = tmp(4503).LinkIcon;
            ...
            if (isMuted) {
              ...
              obj24.IconComponent = tmp(9782).BellIcon;
              ...
              obj24[1] = tmp(9065).BellSlashIcon;
              ...
              obj25.IconComponent = tmp(10966).ChannelNotificationIcon;
              ...
              obj26.header = tmp18(tmp(11003).ActionSheetIconHeader, obj27);
              const items8 = [
                items5.map((buttons) => {
              ...
              if (setting) {
                ...
                const obj30 = { IconComponent: tmp(10634).IdIcon };
                obj29.icon = tmp18(tmp(7200).ActionSheetRow.Icon, obj30);
                ...
                obj28.children = tmp18(tmp(7200).ActionSheetRow, obj29);
                setting = tmp18(tmp(7200).ActionSheetRow.Group, obj28, "developer-actions");
              }
              items8[1] = setting;
              obj26.children = items8;
              return closure_14(tmp(7198).ActionSheet, obj26);
            }
          } else {
            ...
            const obj31 = {
              label: string7Result,
              IconComponent: tmp(10255).PencilIcon,
              onPress() {
          ...
          const obj32 = { label: string3Result, IconComponent: tmp(7881).PhoneCallIcon, onPress: tmp23 };
  ...
  obj33.IconComponent = tmp(6970).EyeIcon;
