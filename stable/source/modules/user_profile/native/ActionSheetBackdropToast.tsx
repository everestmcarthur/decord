...
export const ActionSheetBackdropToast = function ActionSheetBackdropToast(children) {
  ...
  dependencyMap = diff;
  const sharedValue = isExpanded(4296).useSharedValue(0);
  let obj = isExpanded(4296);
  const sharedValue1 = isExpanded(4296).useSharedValue(0);
  ...
  const effect = sharedValue.useEffect(() => {
    ...
    return () => {
      ...
      obj4.easing = Easing.out(isExpanded(4296).Easing.exp);
      const result1 = sharedValue1.set(isExpanded(4561).withTiming(0, obj4));
    };
  }, items);
  let obj2 = isExpanded(4296);
  const fn = function x() {
    if (closure_7) {
      if (tmp) {
        let sum = c2 + 24;
  ...
  const obj6 = { style: null, children: jsx(isExpanded(4556).Text, { variant: "text-sm/medium", color: "mobile-text-heading-primary", children: children.text }) };
  const items2 = [tmp.toast, animatedStyle];
  obj6.style = items2;
  obj5.children = jsx(ReanimatedRexportDefault.View, { style: null, children: jsx(isExpanded(4556).Text, { variant: "text-sm/medium", color: "mobile-text-heading-primary", children: children.text }) });
