...
// Dependencies: [32, 19, 17, 1949, 5279, 7621, 8184, 7208, 1954, 21, 16796, 16372, 1115, 8257, 8242, 13068, 8680, 8191, 9183, 11203, 11971, 1943, 7388, 13000, 4975, 10255, 576, 1114, 16804, 16805, 16792, 1484, 8187, 11115, 8243, 504, 8228, 8239, 12949, 16806, 9038, 13073, 13074, 11121, 16807, 13077, 15698, 13002, 11212, 8605, 13083, 16809, 13055, 13088, 13094, 9798, 4296, 7156, 11117, 11157, 9776, 12618, 12620, 2]
// Exports: default
...
import UserProfileAvatarDefault from "UserProfileAvatar" /* 8257 */;
...
let UserProfileSections = fn(8184).UserProfileSections;
...
export default function YouScreenUserProfileContent(user) {
  ...
  callback4 = obj6.useCallback(() => {
    ...
    if (!shouldShowExpiringTrialOfferCard) {
      ...
      tmp3Result = tmp3(tmp4(13077), obj4);
    }
    items2[1] = tmp3Result;
    let tmp3Result2 = enabled;
    if (enabled) {
      const obj5 = { style: items };
      tmp3Result2 = tmp3(tmp4(15698), obj5);
    }
