...
// Dependencies: [32, 19, 17, 7208, 1954, 1085, 21, 576, 4560, 8836, 7388, 1943, 8167, 11047, 1889, 4527, 14634, 1896, 1114, 14621, 5577, 8828, 1178, 13176, 2]
// Exports: default

// Module 14633 (UserProfileFrameEditButton)
...
export default function UserProfileFrameEditButton(arg0) {
  ...
  const canViewProfileFramesInCollectiblesShop = user(8836).useCanViewProfileFramesInCollectiblesShop("UserProfileFrameEditButton");
  const obj = user(8836);
  const items = [user(1943).DismissibleContent.PROFILE_FRAME_USER_PROFILE_NEW_BADGE];
  const tmp5 = userProfileFrame(user(7388).useSelectedDismissibleContent(items), 2);
  dependencyMap = tmp6;
  const obj2 = user(7388);
  ...
  const profilePreviewValue = user(8167).getProfilePreviewValue(obj4);
  const obj3 = user(8167);
  let skuId;
  if (profilePreviewValue != null) {
    skuId = profilePreviewValue.skuId;
  }
  const fetchCollectiblesProduct = user(11047).useFetchCollectiblesProduct(skuId);
  ...
  const tmp2Result = user(11047);
  userProfileFrame = user(8167).useUserProfileFrame({ user, guildId });
  ...
  if (canViewProfileFramesInCollectiblesShop) {
    ...
    if (fetchCollectiblesProduct.isFetching) {
      ...
      obj6.onPress = NOOP;
      obj6.leading = tmp19(tmp2(5577).ActivityIndicator, { animating: true, size: "large" });
      let obj7 = obj6;
    } else {
      ...
      obj7.labelTrailing = tmp19(tmp2(14621).UserProfileEditFormLabelBadges, obj8);
      ...
      if (null != first1) {
        ...
        obj10.children = tmp19(guildId(8828), obj11);
        let tmp19Result = tmp19(View, obj10);
        const tmp24 = guildId(8828);
      } else {
        const obj12 = { source: guildId(13176), style: tmp.noneIcon };
        tmp19Result = tmp19(tmp2(1178).Icon, obj12);
      }
      obj7.leading = tmp19Result;
    }
    return jsx(tmp2(14621).UserProfileEditFormButton, obj7);
  } else {
    return null;
  }
  const tmp2Result2 = user(8167);
};
