...
// Dependencies: [19, 17, 2025, 502, 21, 4560, 576, 8930, 8931, 9023, 9024, 9025, 9026, 504, 8932, 7168, 7309, 8681, 8692, 9027, 7165, 9015, 8340, 7207, 7624, 9036, 4556, 1114, 2]
// Exports: default

// Module 8929 (UserProfileApplicationWidgetCard)
import nativeDefault from "native" /* 576 */;
import resolvedValuesFromUserApplicationIdentityProfile from "resolvedValuesFromUserApplicationIdentityProfile" /* 8932 */;
...
export default function UserProfileApplicationWidgetCard(userId) {
  ...
  const getOrFetchApplication = userId(7168).useGetOrFetchApplication(widget.applicationId);
  let iconURL;
  if (getOrFetchApplication != null) {
    iconURL = getOrFetchApplication.getIconURL(16);
  }
  const obj3 = userId(7168);
  let canonicalGameId;
  if (getOrFetchApplication != null) {
    canonicalGameId = getOrFetchApplication.getCanonicalGameId();
  }
  const data = userId(7309).useGame(canonicalGameId).data;
  let id;
  const tmp2Result = userId(7309);
  if (data != null) {
    id = data.id;
  }
  const tmp10 = stateFromStores(8681);
  const tmp10Result = tmp10({ location: "UserProfileApplicationWidgetCard", applicationId: id, source: userId(8692).GameProfileSources.UserProfileApplicationWidget, sourceUserId: userId, trackEntryPointImpression: true, stackingBehavior: "stack" });
  dependencyMap = tmp10Result;
  const obj4 = { location: "UserProfileApplicationWidgetCard", applicationId: id, source: userId(8692).GameProfileSources.UserProfileApplicationWidget, sourceUserId: userId, trackEntryPointImpression: true, stackingBehavior: "stack" };
  ({ surfaceConfigs, resolutionContext, isLoading, hasIdentity } = stateFromStores(9027)(userId, widget.applicationId));
  const tmp14 = stateFromStores(7165)(getOrFetchApplication);
  token = tmp14.token;
  ({ fetched, canStartAuthorization } = tmp14);
  const tmp15 = surfaceConfigs[userId(undefined, 9015).ApplicationWidgetConfigSurface.WIDGET_TOP];
  const tmp16 = surfaceConfigs[userId(undefined, 9015).ApplicationWidgetConfigSurface.WIDGET_BOTTOM];
  let tmp17 = null;
  if (null != iconURL) {
  ...
  if (stateFromStores1) {
    if (!isLoading) {
      if (!hasIdentity) {
        ...
        if (tmp20) {
          const _Array = Array;
          let someResult = Array.from(tmp2(8340).OAuth2ScopesSets.APPLICATION_IDENTITIES_SCOPES).some((item) => {
            const scopes = token.scopes;
            return scopes.includes(item);
          });
          if (!someResult) {
            let scopes = token.scopes;
            someResult = scopes.includes(tmp2(8340).OAuth2Scopes.SDK_SOCIAL_LAYER);
          }
          if (!someResult) {
            const scopes2 = token.scopes;
            someResult = scopes2.includes(tmp2(8340).OAuth2Scopes.SDK_SOCIAL_LAYER_PRESENCE);
          }
          tmp20 = someResult;
          const arr = Array.from(tmp2(8340).OAuth2ScopesSets.APPLICATION_IDENTITIES_SCOPES);
        }
        if (fetched) {
        ...
        const tmp9Result = tmp9(7207);
        obj7.title = tmp2(7624).getWidgetTitle(widget);
        ...
        const items3 = [closure_9(tmp2(9036).HourglassIcon, obj9), ];
        const obj10 = { variant: "text-sm/medium", color: "text-muted", children: null };
        const intl = tmp2(1114).intl;
        obj10.children = intl.string(tmp2(1114).t.z5K4Uv);
        items3[1] = closure_9(tmp2(4556).Text, obj10);
        obj8.children = items3;
        obj7.children = closure_10(closure_6, obj8);
        tmp23 = closure_9(tmp9Result, obj7);
        const tmp2Result5 = tmp2(7624);
      }
    }
  }
  const tmp13 = stateFromStores(9027)(userId, widget.applicationId);
  const result = userId(8932).bindResolveFieldValue(resolutionContext);
  const obj11 = {
    style: tmp.header,
    onPress() {
      let tmp;
  ...
  const tmp2Result6 = userId(8932);
  obj11.accessibilityLabel = userId(7624).getWidgetTitle(widget);
  const items4 = [tmp17, ];
  const obj12 = { variant: "text-sm/medium", color: "text-strong", lineClamp: 1, children: null };
  const tmp2Result7 = userId(7624);
  obj12.children = userId(7624).getWidgetTitle(widget);
  items4[1] = closure_9(userId(4556).Text, obj12);
  ...
  if (null != tmp15) {
    if (tmp2(8930).ApplicationWidgetLayoutName.WIDGET_TOP_HERO === layout) {
      const obj13 = { header: tmp31, topConfig: tmp15, resolveFieldValue: result, numberFormat: memo };
      tmp30Result = tmp30(tmp9(8931), obj13);
    } else {
      tmp30Result = null;
      if (tmp2(8930).ApplicationWidgetLayoutName.WIDGET_TOP_CONTAINED === layout) {
        const obj14 = { header: tmp31, topConfig: tmp15, resolveFieldValue: result, numberFormat: memo };
        tmp30Result = tmp30(tmp9(9023), obj14);
      }
    }
  }
  if (tmp16 != null) {
  ...
  if (null != tmp16) {
    if (tmp2(8930).ApplicationWidgetLayoutName.WIDGET_BOTTOM_STATS === layout2) {
      const obj15 = { bottomConfig: tmp16, resolveFieldValue: result, numberFormat: memo };
      tmp30Result2 = tmp30(tmp9(9024), obj15);
    } else if (tmp2(8930).ApplicationWidgetLayoutName.WIDGET_BOTTOM_PROGRESS === layout2) {
      const obj16 = { bottomConfig: tmp16, resolveFieldValue: result };
      tmp30Result2 = tmp30(tmp9(9025), obj16);
    } else {
      tmp30Result2 = null;
      if (tmp2(8930).ApplicationWidgetLayoutName.WIDGET_BOTTOM_COLLECTION === layout2) {
        const obj17 = { bottomConfig: tmp16, resolveFieldValue: result };
        tmp30Result2 = tmp30(tmp9(9026), obj17);
      }
    }
  }
  ...
  if (null != tmp30Result) {
    ...
    if (null != tmp30Result2) {
      ...
      tmp29Result = tmp29(tmp9(7207), obj18);
      const tmp9Result2 = tmp9(7207);
    }
  }
  return tmp29Result;
};
