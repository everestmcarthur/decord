...
// Dependencies: [19, 17, 1960, 1074, 1954, 21, 4560, 576, 16297, 10229, 504, 16314, 16299, 10243, 16311, 16139, 1114, 16315, 10240, 16316, 2]

// Module 16313 (GuildsBarFavorites)
...
export default noop.memo(function GuildsBarFavorites() {
  let obj = shouldShowPopover(16297);
  const guildsBarAnimatedWrapperStyles = shouldShowPopover(16297).useGuildsBarAnimatedWrapperStyles();
  const isFavoritesGuildSelected = shouldShowPopover(10229).useIsFavoritesGuildSelected();
  let obj2 = shouldShowPopover(10229);
  ...
  ({ badge, unread } = markPopoverAsDismissed(16314)(stateFromStores));
  const tmp6 = markPopoverAsDismissed(16314)(stateFromStores);
  ({ badge: badge2, cutouts } = markPopoverAsDismissed(16299)({ mentionCount: badge }));
  const ref = noop.useRef(null);
  const tmp7 = markPopoverAsDismissed(16299)({ mentionCount: badge });
  const tmp9 = closure_11();
  const favoritesIntroPopover = shouldShowPopover(10243).useFavoritesIntroPopover();
  ...
  const memo = noop.useMemo(() => ({
    onPress() {
      ...
      markPopoverAsDismissed(16311)(FAVORITES);
    },
    onLongPress() {
  ...
  const obj4 = shouldShowPopover(10243);
  ...
  obj5.expandedChildren = closure_8(shouldShowPopover(16315).HomeDrawerFavoritesRowExpandedChildren, {});
  const colors = markPopoverAsDismissed(576).colors;
  obj5.children = closure_8(shouldShowPopover(10240).StarIcon, { color: isFavoritesGuildSelected ? colors.WHITE : colors.MOBILE_GUILDBAR_ICON_DEFAULT });
  const children = [closure_8(markPopoverAsDismissed(16297), obj5), closure_8(View, { ref, style: tmp9.anchor, pointerEvents: "none", collapsable: false }), ];
  if (shouldShowPopover) {
    const obj8 = { targetRef: ref, markAsDismissed: markPopoverAsDismissed };
    shouldShowPopover = tmp15(tmp5(16316), obj8);
  }
