...
// Dependencies: [17, 4559, 1957, 1979, 4381, 1371, 11431, 4708, 4714, 1114, 7961, 7963, 576, 4710, 13248, 7600, 4773, 4709, 2]
// Exports: default
...
import MediaPostEmbedStore2 from "MediaPostEmbedStore" /* 11431 */;
    ...
    if (MediaPostEmbedStore.getEmbedFetchState(mediaPostEmbedChannelId) !== FetchState.FETCHED) {
      ...
      if (null == media) {
        ...
        if (canViewChannelResult) {
          canViewChannelResult = tmp(4714).canViewChannel(channel);
          const tmpResult = tmp(4714);
        }
        ...
        const mediaPostEmbedCommonData = tmp(4708).getMediaPostEmbedCommonData(obj2);
        if (null == mediaPostEmbedCommonData) {
          ...
          if (null != mediaPostEmbedCommonData.authorName) {
            if (null != mediaPostEmbedCommonData.channelName) {
              if (null != user) {
                const userAuthorWithProcessedColor = tmp(7961).getUserAuthorWithProcessedColor(user, mediaPostEmbedCommonData.postThread);
              ...
              if (mediaPostEmbedCommonData.shouldShowBlurredThumbnailImage) {
                ...
                obj5.blurredCoverImage = React3.resolveAssetSource(tmp10(13248)).uri;
                ...
                const isPendingScanVersionResult = tmp(7600).isPendingScanVersion(mediaPostEmbedCommonData.contentScanVersion);
                let result = value;
                if (value) {
                  result = tmp(7600).shouldAgeVerifyForExplicitMedia();
                  const tmpResult12 = tmp(7600);
                }
                let isVerifiedTeenResult = value;
                if (value) {
                  isVerifiedTeenResult = tmp(4773).isVerifiedTeen();
                  const tmpResult13 = tmp(4773);
                }
                if (mediaPostEmbedCommonData.shouldContainMediaWithBackground) {
                  if (null != mediaPostEmbedCommonData.coverImage) {
                    ...
                    obj6.backgroundImage = tmp(4709).getBackgroundImageUrl(mediaPostEmbedCommonData.coverImage);
                    let obj8 = obj6;
                    const tmpResult14 = tmp(4709);
                  }
                  return obj8;
                }
                obj8 = {};
