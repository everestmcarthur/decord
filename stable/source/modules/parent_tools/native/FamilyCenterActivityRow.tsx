...
const memoResult = noop.memo((action) => {
  ...
  if (null == stateFromStores) {
    ...
    obj6.children = tmp3(4404).getName(stateFromStores);
    const items2 = [closure_8(tmp6(4556).Text, obj6), ];
    const obj7 = { variant: "text-xs/medium", color: "channels-default", children: null };
    const tmp3Result2 = tmp3(4404);
    const _Date = Date;
    const date = new Date(extractTimestampResult);
    obj7.children = tmp6(7593).formatUserActivityTimestamp(date.getTime(), value.timestampFormatter);
    items2[1] = closure_8(tmp6(4556).Text, obj7);
...
const memoResult1 = noop.memo((action) => {
  ...
  if (undefined === stateFromStores) {
    ...
    obj4.size = tmp2(5584).GuildIconSizes.NORMAL;
    const items1 = [closure_8(tmp6(5584), obj4), ];
    ...
    if (hasItem) {
      const obj8 = { style: tmp.badge, guild: stateFromStores, size: tmp6(5590).Sizes.SMALL, disableColor: true };
      tmp11Result = tmp11(tmp6(5590), obj8);
      const tmp6Result2 = tmp6(5590);
    }
    ...
    items2[1] = closure_8(tmp2(4556).Text, obj9);
    ...
    if (undefined !== stateFromStores.approximateMemberCount) {
      ...
      tmp11Result2 = tmp11(tmp2(4556).Text, obj10);
    }
