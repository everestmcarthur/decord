...
// Dependencies: [32, 19, 17, 7538, 21, 4560, 576, 8652, 7593, 14897, 4556, 11921, 1114, 2396, 14903, 5123, 2]
// Exports: default
...
export default function FamilyCenterActivitySection(displayType) {
  ...
  const actionsForDisplayType = displayType(14897).useActionsForDisplayType(displayType);
  const obj = displayType(14897);
  const actionTotalsForDisplayType = displayType(14897).useActionTotalsForDisplayType(displayType);
  const obj2 = displayType(14897);
  const familyCenterActions = displayType(11921).useFamilyCenterActions({});
  ...
  if (0 === actionsForDisplayType.length) {
    ...
    const items1 = [closure_9(FamilyCenterActivitySectionHeader, obj6), substr.map((action) => closure_1_9(loadMore(14903), { action }, action.event_id)), ];
    if (substr.length >= actionTotalsForDisplayType) {
      ...
      if (familyCenterActions.isMoreLoading) {
        ...
        obj10.children = tmp17(tmp(4556).Text, obj11);
        tmp17Result = tmp17(tmp(5123).PressableOpacity, obj10);
      }
