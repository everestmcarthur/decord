...
function ScheduleRuleRow(rule) {
  ...
  const scheduleRuleDateRange = rule(10088).getScheduleRuleDateRange(rule);
  let obj = rule(10088);
  const obj2 = rule(10088);
  const obj3 = { label: scheduleRuleDateRange, subLabel: rule(10088).formatDays(rule.days), trailing: null, arrow: null, onPress: null };
  ...
  obj3.trailing = closure_5(rule(4556).Text, { variant: "text-sm/medium", color: "text-subtle", children: stringResult });
  obj3.arrow = !readOnly;
  let fn;
  if (!readOnly) {
    fn = () => {
  ...
  obj3.onPress = fn;
  return closure_5(rule(5605).TableRow, obj3);
}
