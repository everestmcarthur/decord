...
// Module 10825 (SlayerStorefrontDisclaimerUtils)
...
let closure_6 = { [fn(10826).StorefrontPlatform.DESKTOP]: "PC", [fn(10826).StorefrontPlatform.XBOX]: "Xbox", [fn(10826).StorefrontPlatform.PLAYSTATION]: "PlayStation", [fn(10826).StorefrontPlatform.SWITCH]: "Switch", [fn(10826).StorefrontPlatform.APPLE_ARCADE]: "Apple Arcade", [fn(10826).StorefrontPlatform.NETFLIX]: "Netflix", [fn(10826).StorefrontPlatform.AMAZON_KIDS_PLUS]: "Amazon Kids+" };
let items = [fn(10826).StorefrontPlatform.PLAYSTATION];
const size = fn(2);
...
export const getFinePrintMessageForApplication = function getFinePrintMessageForApplication(name, shouldAppendDisclaimer) {
  ...
  if (shouldAppendDisclaimer.shouldAppendDisclaimer) {
    ...
    if (0 !== arr.length) {
      ...
      str2 = intl3.formatToPlainString(tmp4(3417)["5h8p5P"], obj3);
    }
    const obj4 = { platforms_info: str2 };
    obj2.platforms_info = intl2.format(_modDef3417.Q0dHYO, obj4);
    let formatResult = format(_modDef3417["3ah/a2"], obj2);
    tmp4 = importDefault;
  } else {
...
export const getMobileFinePrintMessageForApplication = function getMobileFinePrintMessageForApplication(getOrFetchApplication, stringResult, shouldAppendDisclaimer) {
  ...
  if (shouldAppendDisclaimer.shouldAppendDisclaimer) {
    ...
    if (0 !== arr2.length) {
      ...
      str2 = intl3.formatToPlainString(tmp3(3417)["5h8p5P"], obj2);
    }
...
export const getRedeemPurchaseDescriptionForApplication = function getRedeemPurchaseDescriptionForApplication(applicationName) {
  ...
  if (0 !== arr.length) {
    ...
    str = intl2.formatToPlainString(_modDef3417["5h8p5P"], obj2);
  }
...
export const getGiftLinkAccountDescriptionForApplication = function getGiftLinkAccountDescriptionForApplication(applicationName, hasAlreadyLinked) {
  ...
  if (0 !== arr.length) {
    ...
    obj2.platforms = listFormat.format(arr.map((item) => closure_1_6[item]));
    obj2.count = arr.length;
    str = intl2.formatToPlainString(tmp4(3417)["5h8p5P"], obj2);
  }
  obj.platforms_info = str;
