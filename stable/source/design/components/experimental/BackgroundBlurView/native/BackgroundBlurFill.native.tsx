...
// Module 8603 (BackgroundBlurFill)
...
export const BackgroundBlurFillWithPress = function BackgroundBlurFillWithPress(style) {
  ...
  obj = blurTheme(4271);
  token = blurTheme(4262).useToken(nativeDefault.colors.BACKGROUND_SCRIM, blurTheme);
  const tmp2Result = blurTheme(4262);
  token1 = blurTheme(4262).useToken(nativeDefault.colors.BACKGROUND_SCRIM_LIGHTBOX, blurTheme);
  const tmp2Result5 = blurTheme(4262);
  const isBlurDisabledResult = blurTheme(4965).isBlurDisabled(merged);
  c5 = isBlurDisabledResult;
  const tmp2Result6 = blurTheme(4965);
  class B {
    constructor() {
  ...
  const tmp2Result7 = blurTheme(4296);
  B.__closure = { withSpring: blurTheme(4974).withSpring, interpolateColor: blurTheme(4296).interpolateColor, pressed, fallbackColor: token, fallbackColorPressed: token1, ON_PRESS_SPRING: blurTheme(4978).ON_PRESS_SPRING };
  B.__workletHash = 10497618157620;
  B.__initData = __initData;
  let animatedStyle = tmp2Result7.useAnimatedStyle(B);
  let obj2 = { withSpring: blurTheme(4974).withSpring, interpolateColor: blurTheme(4296).interpolateColor, pressed, fallbackColor: token, fallbackColorPressed: token1, ON_PRESS_SPRING: blurTheme(4978).ON_PRESS_SPRING };
  class C {
    constructor() {
  ...
  const tmp2Result8 = blurTheme(4296);
  C.__closure = { shouldUseFallback: isBlurDisabledResult, withSpring: blurTheme(4974).withSpring, interpolateColor: blurTheme(4296).interpolateColor, pressed, restingTint: tmp6, pressedTint: tmp7, ON_PRESS_SPRING: blurTheme(4978).ON_PRESS_SPRING };
  ...
  let obj3 = { shouldUseFallback: isBlurDisabledResult, withSpring: blurTheme(4974).withSpring, interpolateColor: blurTheme(4296).interpolateColor, pressed, restingTint: tmp6, pressedTint: tmp7, ON_PRESS_SPRING: blurTheme(4978).ON_PRESS_SPRING };
  const tmp12 = c5;
