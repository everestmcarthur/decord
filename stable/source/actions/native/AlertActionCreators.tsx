...
export default {
  ...
  show(hideActionSheet) {
    ...
    if (!tmp7) {
      if (flag) {
        merged(4527).hideActionSheet();
        const obj4 = merged(4527);
      }
      if (null != c6) {
        confirmText(4906).dismissAlert(c6);
        const obj5 = confirmText(4906);
      }
      ...
      const tmp31 = c5;
      const tmp32 = c4;
      obj8.variant = confirmText(4994).getAlertButtonVariant(confirmColor);
      ...
      const items = [c3(confirmText(4910).AlertActionButton, obj8), ];
      ...
      if (null != cancelText) {
        ...
        tmp30Result = tmp30(tmp29(4910).AlertActionButton, obj9);
      }
      ...
      obj7.openAlert(text, c3(confirmText(4910).AlertModal, obj3), () => {
        if (!c5) {
          if (c4 != null) {
            tmp();
          }
