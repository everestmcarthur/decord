...
// Dependencies: [5, 5558, 7526, 1961, 502, 1957, 2012, 2021, 1979, 8380, 4544, 4199, 4209, 4381, 1371, 1074, 1098, 1964, 4602, 7736, 5414, 1384, 8393, 7222, 1112, 1100, 5411, 1896, 8394, 9095, 4615, 4702, 12935, 7523, 5057, 5569, 9552, 7095, 9795, 573, 8743, 4771, 12936, 5520, 1242, 1272, 4462, 1090, 4753, 1250, 1969, 4545, 5522, 7211, 4241, 12938, 12939, 11436, 4883, 4801, 12940, 12941, 1255, 12943, 2]
// Exports: trackInviteEmbedActioned, trackInviteServerClicked, transitionToGuildFromEventInvite

// Module 8379 (InstantInviteActionCreators)
...
export default {
  resolveInvite(code, arg1, arg2) {
    ...
    if (obj.isDispatching()) {
      ...
      nextPromise = tmp(8743)(code, arg1, arg2).then((result) => {
        ({ invite, code } = result);
        if (null != invite) {
          const obj2 = { type: "INVITE_RESOLVE_SUCCESS", invite, code };
          closure_1(dependencyMap[39]).dispatch(obj2);
      ...
      const promise = tmp(8743)(code, arg1, arg2);
    }
    return nextPromise;
  },
  ...
  transitionToInvite(flags, arg1) {
    ...
    let hasFlagResult = dMFromUserId(1384).hasFlag(num, dMFromUserId(8393).GuildInviteFlags.IS_GUEST_INVITE);
    if (!hasFlagResult) {
      hasFlagResult = tmp6(1384).hasFlag(num, tmp6(8393).GuildInviteFlags.IS_APPLICATION_BYPASS);
      ...
      if (!hasFlagResult) {
        if (flags.new_member) {
          if (tmp6Result3.inviteGuildHasPendingMemberDisabledVerification(guild)) {
            ...
            const tmp6Result4 = tmp6(12939);
          }
          tmp6Result3 = tmp6(12939);
        }
      }
    }
    if (null != channel) {
  ...
  openApp(code, targetChannelId, fingerprint, username, inviteType) {
    ...
    if ("Android" !== family) {
      ...
      if ("iOS" !== family1) {
        ...
        tmp7(12943).launch(combined, (arg0) => {
          if (arg0) {
    ...
    const attemptId = tmp18(12941).generateAttemptId();
    inviteType = undefined;
    const tmp18Result = tmp18(12941);
    if (inviteType != null) {
