...
// Dependencies: [2025, 10370, 1074, 1084, 4740, 10372, 10373, 1272, 573, 12, 1256, 1365, 9948, 1940, 1222, 4904, 1114, 1242, 2]
...
// Module 10371 (GIFPickerActionCreators)
import DispatcherDefault from "Dispatcher" /* 573 */;
import frecency_user_settings from "frecency_user_settings" /* 1222 */;
import AnalyticsUtilsDefault from "AnalyticsUtils" /* 1242 */;
import URLUtilsDefault from "URLUtils" /* 1365 */;
import AppAnalyticsUtilsDefault from "AppAnalyticsUtils" /* 4740 */;
import AttachmentUrlUtilsAll from "AttachmentUrlUtils" /* 9948 */;
import GifProvider from "GifProvider" /* 10372 */;
...
export const gifUrlKey = function gifUrlKey(uri) {
  ...
  if (null != toURLSafeResult) {
    if (obj2.isAttachmentPathUrl(toURLSafeResult)) {
      const tmp5Result = tmp5(9948);
      str1 = tmp5(9948).removeSignedUrlParameters(toURLSafeResult).toString();
      const str = tmp5(9948).removeSignedUrlParameters(toURLSafeResult);
    }
    tmp4 = str1;
...
export const addFavoriteGIF = function addFavoriteGIF(size) {
  ...
  FrecencyUserSettingsActionCreators.updateAsync("favoriteGifs", async (gifs) => {
      ...
      if (null != tmp3.gifSrc) {
        if ("" !== tmp3.gifSrc) {
          ...
          if (tmp9) {
            ...
            if (!result) {
              result = tmp10(9948).isAttachmentPathUrl(toURLSafeResult);
              const tmp10Result = tmp10(9948);
            }
            tmp9 = result;
            tmp10 = importAll;
          }
          ...
          if (null != toURLSafeResult1) {
            if (obj12.isAttachmentPathUrl(toURLSafeResult1)) {
              const tmp24Result = tmp24(9948);
              url = tmp24(9948).removeSignedUrlParameters(toURLSafeResult1).toString();
              const str9 = tmp24(9948).removeSignedUrlParameters(toURLSafeResult1);
            }
            tmp23 = url;
    ...
    if (tmp5) {
      ...
      if (!result3) {
        result3 = tmp6(9948).isAttachmentPathUrl(toURLSafeResult2);
        const tmp6Result = tmp6(9948);
      }
      tmp5 = result3;
      tmp6 = importAll;
    }
  ...
  FrecencyUserSettingsActionCreators.updateAsync("favoriteGifs", async (gifs) => {
    if (closure_0 in gifs.gifs) {
      ...
      if (null != toURLSafeResult) {
        if (obj2.isAttachmentPathUrl(toURLSafeResult)) {
          const tmp10Result = tmp10(9948);
          const str1 = tmp10(9948).removeSignedUrlParameters(toURLSafeResult).toString();
          const str = tmp10(9948).removeSignedUrlParameters(toURLSafeResult);
        }
        obj2 = AttachmentUrlUtilsAll;
        tmp10 = importAll;
      }
