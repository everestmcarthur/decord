...
// Dependencies: [19, 17, 1957, 1074, 21, 4560, 576, 9604, 1178, 1114, 9607, 9116, 504, 4992, 4702, 4612, 4556, 4976, 2]
// Exports: default

// Module 9603 (VideoEmptyState)
...
export default function VideoEmptyState(style) {
  ...
  if (obj.STREAM_ENDED === type) {
    ...
    if (!removeSplashImage) {
      ...
      tmp16 = closure_5(stream(9604).StreamEnded, obj2);
    }
    ...
    if (!removeSplashImage) {
      ...
      tmp7 = closure_5(stream(9607).StreamFailed, obj);
    }
  ...
  if (null != avError) {
    let errorCode = stream(9116).getErrorInfo(avError).errorCode;
    const obj8 = stream(9116);
  } else {
    errorCode = null;
    if (type === tmp3.STREAM_FAILED) {
      errorCode = stream(9116).getErrorInfo(stream(9116).AVError.STREAM_FAILED_TO_START).errorCode;
      const obj7 = stream(9116);
    }
  }
  ...
  if (tmp34) {
    ...
    tmp34 = closure_5(tmp28(4556).Text, obj11);
  }
  items5[1] = tmp34;
  ...
  if (!removeCloseButton) {
    ...
    obj13.children = closure_5(tmp28(4976).BaseTextButton, obj14);
