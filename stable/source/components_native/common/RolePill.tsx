...
export default function RolePill(role) {
  ...
  const roleIcon = role(7186).useRoleIcon({ guildId, roleId: role.id, size: 12 });
  let name = role.name;
  let combined = name;
  ...
  if (undefined !== guild_connections) {
    ...
    let tmp10Result = tmp10(combined(7204), obj5);
  ...
  if (null != roleIcon) {
    const obj9 = { style: tmp9.roleIcon, children: roleIcon };
  ...
  return closure_6(role(5123).PressableHighlight, obj3);
};
