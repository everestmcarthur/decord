...
// Dependencies: [32, 5, 19, 17, 2016, 1957, 2015, 1979, 4209, 1371, 1074, 21, 4560, 576, 4904, 1114, 4713, 4204, 11615, 9746, 8631, 12, 5687, 5605, 15161, 1483, 12757, 10275, 15315, 504, 1894, 10947, 16829, 5581, 7864, 4573, 2]
// Exports: default
...
function ChannelPermissionSyncModule(channel) {
  ...
  const callback = noop.useCallback(() => {
    ...
    let obj = category(4904);
    obj3.channelName = channel(4713).computeChannelName(closure_0, UserStore, RelationshipStore, true);
    let obj4 = channel(4713);
    obj3.categoryName = channel(4713).computeChannelName(category, UserStore, RelationshipStore);
        ...
        try {
          ...
          if (0 === c2) {
            if (arg0 === 1) {
              ...
              c2 = 1;
              c3 = 1;
              const obj5 = { value: tmp2(9746).checkChattableChannelThresholdMetAfterChannelPermissionDeny(tmp2, syncedPermissionOverwrites[guild_id].deny, syncedPermissionOverwrites[guild_id].allow), done: false };
              return obj5;
            }
          } else if (arg0 === 1) {
            c3 = 3;
  ...
  const channelName = channel(4713).computeChannelName(category, UserStore, RelationshipStore);
  ...
  let obj6 = { icon: closure_16(channel(15161).RefreshIcon, {}), label: null, onPress: null };
  let intl2 = tmp3(1114).intl;
  obj6.label = intl2.string(channel(1114).t.NVwuHq);
  obj6.onPress = callback;
  obj5.children = closure_16(channel(5605).TableRow, obj6);
  obj2.children = closure_16(channel(5687).TableRowGroup, obj5);
...
function AddPermission(isEditing) {
  ...
  if (isEditing.isEditing) {
    ...
    const obj4 = { icon: closure_16(tmp2(12757).PlusMediumIcon, {}), label: null, onPress: null };
    const intl2 = tmp2(1114).intl;
    obj4.label = intl2.string(tmp2(1114).t.fVWxvT);
    obj4.onPress = function onPress() {
      closure_0.push(constants2.NEW_PERMISSION, { type: constants.ROLE });
    };
    const items = [closure_16(tmp2(5605).TableRow, obj4), ];
    const obj5 = { icon: closure_16(tmp2(12757).PlusMediumIcon, {}), label: null, onPress: null };
    const intl3 = tmp2(1114).intl;
    obj5.label = intl3.string(tmp2(1114).t.riesLt);
    obj5.onPress = function onPress() {
      closure_0.push(constants2.NEW_PERMISSION, { type: constants.MEMBER });
    };
    items[1] = closure_16(tmp2(5605).TableRow, obj5);
    obj3.children = items;
    obj2.children = closure_17(tmp2(5687).TableRowGroup, obj3);
    return closure_16(View, obj2);
  }
...
function RoleRow(onDelete) {
  ...
  if (isEditing) {
    ...
    if (!tmp) {
      ...
      tmp2Result = tmp2(tmp3(15315).CircleMinusIcon, obj2);
    }
  }
  obj.icon = tmp2Result;
  onDelete = onSelect;
...
function RoleOverwrites(guild) {
  ...
  obj4.children = closure_16(guild(5687).TableRowGroup, obj5);
...
function MemberOverwrites(channel) {
  ...
  if (valueResult.length > 0) {
    ...
    obj2.children = closure_16(guild_id(5687).TableRowGroup, obj3);
