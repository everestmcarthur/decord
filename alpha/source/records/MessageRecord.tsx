...
// Function ID: 4373
// Name: MessageRecord
// Dependencies: [1387, 1074, 1385, 4373, 7498, 7965, 9483, 2]
// Exports: isMessageComponentsV2

// Module 4372 (MessageRecord)
...
prototype["addReaction"] = function addReaction(emoji) {
  ...
  const mapped = reactions.map((emoji, index) => {
    ...
    if (obj.emojiEquals(emoji.emoji, closure_0)) {
      ...
      if (NORMAL === tmp(7965).ReactionTypes.BURST) {
        if (flag) {
          if (emoji.me_burst) {
            return emoji;
          }
        ...
        burst_colors = colors;
        tmp28 = flag;
      } else if (tmp12 === tmp(7965).ReactionTypes.VOTE) {
...
prototype["removeReaction"] = function removeReaction(arg0) {
  ...
  const mapped = reactions.map((emoji, index) => {
    ...
    if (!obj.emojiEquals(emoji.emoji, closure_0)) {
      ...
      if (NORMAL === tmp2(7965).ReactionTypes.BURST) {
        if (flag) {
          if (!merged1.me_burst) {
      ...
      } else if (tmp4 !== tmp2(7965).ReactionTypes.VOTE) {
        if (flag) {
          if (!merged1.me) {
