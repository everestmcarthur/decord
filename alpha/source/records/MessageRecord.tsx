...
// Function ID: 4407
// Name: MessageRecord
// Dependencies: [1387, 1074, 1385, 4407, 7542, 8003, 9317, 2]
...
prototype["addReaction"] = function addReaction(emoji) {
  ...
  const mapped = reactions.map((emoji, index) => {
    ...
    if (obj.emojiEquals(emoji.emoji, closure_0)) {
      ...
      if (NORMAL === tmp(8003).ReactionTypes.BURST) {
        if (flag) {
          if (emoji.me_burst) {
            return emoji;
          }
        ...
        burst_colors = colors;
        tmp28 = flag;
      } else if (tmp12 === tmp(8003).ReactionTypes.VOTE) {
...
prototype["removeReaction"] = function removeReaction(arg0) {
  ...
  const mapped = reactions.map((emoji, index) => {
    ...
    if (!obj.emojiEquals(emoji.emoji, closure_0)) {
      ...
      if (NORMAL === tmp2(8003).ReactionTypes.BURST) {
        if (flag) {
          if (!merged1.me_burst) {
      ...
      } else if (tmp4 !== tmp2(8003).ReactionTypes.VOTE) {
        if (flag) {
          if (!merged1.me) {
