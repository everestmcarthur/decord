...
// Dependencies: [5325, 5326, 1086, 5327, 5328, 5329, 5330, 5331, 5332, 2]
    ...
    if (data.type === obj.MINIMAL) {
      if (null != data.discord_classifications) {
        ...
        if (obj12.has(deserializeResult, DiscordContentClassificationFlags.DiscordContentClassificationFlags.EMERGENCY_ONLY_USE_IF_YOU_HAVE_TO_FORCE_MARK_AGE_RESTRICTED)) {
          ADULT = tmp28(5326).AgeRestrictionStatus.ADULT;
          obj.status = ADULT;
          let tmp12 = obj;
        } else {
          const tmp29Result = tmp29(1086);
          const AgeRestrictionStatus3 = tmp28(5326).AgeRestrictionStatus;
          obj.status = tmp29(1086).hasAny(deserializeResult, tmp28(5327).DiscordContentClassificationFlagMasks.RESTRICTED_TO_ADULT) ? AgeRestrictionStatus3.ADULT : AgeRestrictionStatus3.EVERYONE;
          tmp12 = obj;
          const hasAnyResult = tmp29(1086).hasAny(deserializeResult, tmp28(5327).DiscordContentClassificationFlagMasks.RESTRICTED_TO_ADULT);
        }
        items.push(tmp12);
      ...
      if (obj2.has(deserializeResult1, DiscordContentClassificationFlags.DiscordContentClassificationFlags.EMERGENCY_ONLY_USE_IF_YOU_HAVE_TO_FORCE_MARK_AGE_RESTRICTED)) {
        obj3.status = tmp4(5326).AgeRestrictionStatus.ADULT;
        let tmp9 = obj3;
      } else {
        const tmp6Result = tmp6(1086);
        const AgeRestrictionStatus2 = tmp4(5326).AgeRestrictionStatus;
        obj3.status = tmp6(1086).hasAny(deserializeResult1, tmp4(5327).DiscordContentClassificationFlagMasks.RESTRICTED_TO_ADULT) ? AgeRestrictionStatus2.ADULT : AgeRestrictionStatus2.EVERYONE;
        tmp9 = obj3;
        const hasAnyResult1 = tmp6(1086).hasAny(deserializeResult1, tmp4(5327).DiscordContentClassificationFlagMasks.RESTRICTED_TO_ADULT);
      }
      items.push(tmp9);
      tmp6 = importAll;
      ...
      if (obj10.has(deserializeResult2, DiscordContentClassificationFlags.DiscordContentClassificationFlags.EMERGENCY_ONLY_USE_IF_YOU_HAVE_TO_FORCE_MARK_AGE_RESTRICTED)) {
        obj4.status = tmp24(5326).AgeRestrictionStatus.ADULT;
        let tmp2 = obj4;
      } else {
        const tmp26Result = tmp26(1086);
        const AgeRestrictionStatus = tmp24(5326).AgeRestrictionStatus;
        obj4.status = tmp26(1086).hasAny(deserializeResult2, tmp24(5327).DiscordContentClassificationFlagMasks.RESTRICTED_TO_ADULT) ? AgeRestrictionStatus.ADULT : AgeRestrictionStatus.EVERYONE;
        tmp2 = obj4;
        const hasAnyResult2 = tmp26(1086).hasAny(deserializeResult2, tmp24(5327).DiscordContentClassificationFlagMasks.RESTRICTED_TO_ADULT);
      }
      items.push(tmp2);
