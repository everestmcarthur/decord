  ...
  if (null == dynamic) {
    ...
    while (iter2 !== undefined) {
      ...
      if (nextResult.type === ProfileDataDynamicType.ProfileDataDynamicType.STRING) {
        let obj2 = { type: tmp5(9206).ResolvedValueType.STRING, value: iter.value };
        obj[iter.name] = obj2;
      } else if (iter.type === tmp5(9289).ProfileDataDynamicType.NUMBER) {
        let obj3 = { type: tmp5(9206).ResolvedValueType.NUMBER, value: iter.value };
        obj[iter.name] = obj3;
      } else if (iter.type === tmp5(9289).ProfileDataDynamicType.MEDIA) {
        if (isVisualUnfurledMedia(iter.value)) {
          let obj4 = { type: tmp5(9206).ResolvedValueType.MEDIA, media: null };
