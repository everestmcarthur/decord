...
// Function ID: 4779
// Name: destroy
// Dependencies: [32, 5, 4747, 4779, 4780, 4781, 4785, 4, 1911, 4777, 4786, 1340, 1365, 4788, 4848, 2]
...
import VideoDefault from "Video" /* 4781 */;
import CameraDefault from "Camera" /* 4785 */;
import ConnectionDefault from "Connection" /* 4788 */;
import Devices from "Devices" /* 4848 */;
import _slicedToArray from "module_32" /* 32 */;
import asyncGeneratorStep from "asyncGeneratorStep" /* 5 */;
import TypedEventEmitter from "TypedEventEmitter" /* 4780 */;

const pollConnectionStatsDefault = tmp10(4786);
require = fn;
let Constants = fn(4747);
  ...
  constructor() {
    ...
    obj.handleRemoveListener = function handleRemoveListener(arg0) {
      if (arg0 === BaseConnectionEvent.MediaEngineEvent.VoiceActivity) {
        if (null != tmpResult.getVoiceEngine().setEmitVADLevel2) {
          const voiceEngine = tmp(1911).getVoiceEngine();
          voiceEngine.setEmitVADLevel2(obj.listenerCount(tmp(4777).MediaEngineEvent.VoiceActivity) > 0);
          const tmpResult3 = tmp(1911);
        } else {
          const voiceEngine1 = tmp(1911).getVoiceEngine();
          voiceEngine1.setEmitVADLevel(obj.listenerCount(tmp(4777).MediaEngineEvent.VoiceActivity) > 0, false, {});
          const tmpResult4 = tmp(1911);
        }
        tmpResult = tmp(1911);
      }
    };
          ...
          if (0 === dependencyMap) {
            if (arg0 === 1) {
              ...
              if (importDefault) {
                ...
                voiceEngine = v0(1911).getVoiceEngine();
            ...
            closure_129_0.emit(v0(4777).MediaEngineEvent.VoiceQueueMetrics, closure_128_1);
    ...
    if (tmp.ATTENUATION !== arg0) {
      if (tmp.VIDEO_HOOK !== arg0) {
        if (tmp.EXPERIMENTAL_SOUNDSHARE === arg0) {
          ...
          if (isDesktopResult) {
            isDesktopResult = tmp50(1911).supportsFeature(constants6.MEDIAPIPE);
            const tmp50Result = tmp50(1911);
          }
          if (!isDesktopResult) {
            isDesktopResult = tmp50(1911).supportsFeature(constants6.VIDEO_BACKGROUND_FILTER);
            const tmp50Result2 = tmp50(1911);
          }
...
prototype["connect"] = function connect(arg0, arg1, videoSupported) {
  ...
  obj3 = self(4788).create(arg0, arg1, videoSupported, flag);
  obj3.on(obj3(4777).BaseConnectionEvent.Destroy, (arg0) => {
  ...
  obj3.on(obj3(4777).BaseConnectionEvent.Connected, () => {
    obj3.setVideoBroadcast(self.shouldConnectionBroadcastVideo(obj3));
  });
  obj3.on(obj3(4777).BaseConnectionEvent.Silence, (arg0) => {
  ...
  obj3(1911).setProcessPriority(HIGH);
  if (null != videoSupported.threadPriorityConfiguration) {
    let voiceEngine = tmp(1911).getVoiceEngine();
  ...
  self.emit(obj3(4777).MediaEngineEvent.Connection, obj3);
  return obj3;
};
...
prototype["setAudioInputDevice"] = function setAudioInputDevice(audioInputDeviceId) {
  ...
  if (obj.supportsFeature(constants6.SET_AUDIO_DEVICE_BY_ID)) {
    ...
    const audioInputDevices = tmp(4848).getAudioInputDevices();
...
prototype["setAudioOutputDevice"] = function setAudioOutputDevice(audioOutputDeviceId) {
  ...
  if (obj.supportsFeature(constants6.SET_AUDIO_DEVICE_BY_ID)) {
    ...
    const audioOutputDevices = tmp(4848).getAudioOutputDevices();
        ...
        if (0 === dependencyMap) {
          ...
          if (closure_128_1 === closure_129_1.videoInputDeviceId) {
            ...
            if (!obj9.supportsFeature(constants.SET_VIDEO_DEVICE_BY_ID)) {
              const voiceEngine = tmp2(1911).getVoiceEngine();
              let num3 = -1;
            ...
            if (forEach == closure_128_0) {
              ...
              const voiceEngine1 = tmp2(1911).getVoiceEngine();
              voiceEngine1.setVideoInputDevice(closure_128_2);
              const obj3 = tmp2(1911);
            }
...
prototype["setLoopback"] = function setLoopback(arg0, arg1) {
  ...
  if (null != tmp2Result4.getVoiceEngine().setEmitVADLevel2) {
    const voiceEngine1 = tmp2(1911).getVoiceEngine();
    if (!tmp) {
      tmp = self.listenerCount(tmp2(4777).MediaEngineEvent.VoiceActivity) > 0;
    }
    voiceEngine1.setEmitVADLevel2(tmp);
    const tmp2Result5 = tmp2(1911);
  } else {
    const voiceEngine2 = tmp2(1911).getVoiceEngine();
    let tmp6 = tmp;
    if (!tmp) {
      tmp6 = self.listenerCount(tmp2(4777).MediaEngineEvent.VoiceActivity) > 0;
    }
...
prototype["createReplayConnection"] = function createReplayConnection(arg0, arg1) {
  ...
  let tmp2 = null;
  if (null != replay) {
    replay.on(self(4777).BaseConnectionEvent.Destroy, (arg0) => {
      const connections = self.connections;
      connections.delete(arg0);
      if (self.connectionsEmpty()) {
        inject.setProcessPriority(constants.NORMAL);
    ...
    self.emit(self(4777).MediaEngineEvent.Connection, replay);
    tmp2 = replay;
...
prototype["stopLocalAudioRecording"] = function stopLocalAudioRecording(arg0) {
  ...
  let tmp5 = this.listenerCount(tmp(4777).MediaEngineEvent.VoiceActivity) > 0;
  if (tmp5) {
    tmp5 = null != tmp(1911).getVoiceEngine().setEmitVADLevel2;
    const tmpResult = tmp(1911);
  }
  if (tmp5) {
    const voiceEngine1 = tmp(1911).getVoiceEngine();
