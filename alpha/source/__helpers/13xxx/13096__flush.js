...
// Function ID: 13097
// Name: _flush
// Dependencies: [5, 13075, 13097, 13076, 13048, 13059, 13062, 13055, 13085, 13049, 13071]
  ...
  if (c1 === 2) {
    ...
    try {
      ...
      if (arg0 === 1) {
        ...
        const client = require("module_13075").getClient();
        if (client) {
          client.flush(tmp13);
        } else {
        ...
        c1 = 3;
        const obj3 = require("module_13075");
        tmp13 = closure_0;
      }
    } catch (tmp8) {
      c1 = tmp;
...
function endSession() {
  ...
  if (tmp3) {
    tmp(13071).closeSession(tmp3);
    const tmpResult = tmp(13071);
  }
  const isolationScope1 = _mod13075.getIsolationScope();
  const tmpResult4 = _mod13075;
  const currentScope1 = _mod13075.getCurrentScope();
  const tmpResult5 = _mod13075;
...
export const captureException = function captureException(arg0, arg1) {
  const currentScope = _mod13075.getCurrentScope();
  return currentScope.captureException(arg0, _mod13097.parseEventHintOrCaptureContext(arg1));
};
...
export const startSession = function startSession(arg0) {
  ...
  if (undefined === environment) {
    environment = tmp(13085).DEFAULT_ENVIRONMENT;
  }
  const userAgent = _mod13049.GLOBAL_OBJ.navigator || {}.userAgent;
  const tmp3 = _mod13049.GLOBAL_OBJ.navigator || {};
  const obj2 = { release, environment, user: null };
  const tmpResult = _mod13071;
  ...
  if (tmp10) {
    tmp(13071).updateSession(session1, { status: "exited" });
    const tmpResult2 = tmp(13071);
  }
  endSession();
...
export const withMonitor = function withMonitor(monitorSlug, arg1, arg2) {
  ...
  function finishCheckIn(status) {
    const obj = { monitorSlug, status, checkInId: captureCheckInResult, duration: _mod13062.timestampInSeconds() - closure_3 };
    const currentScope = _mod13075.getCurrentScope();
    const client = _mod13075.getClient();
    if (client) {
      if (client.captureCheckIn) {
  ...
  let currentScope = require("module_13075").getCurrentScope();
  let obj = { monitorSlug, status: "in_progress" };
  let obj2 = require("module_13075");
  let client = require("module_13075").getClient();
  if (client) {
    if (client.captureCheckIn) {
      let captureCheckInResult = client.captureCheckIn(obj, arg2, currentScope);
    } else if (tmp(13076).DEBUG_BUILD) {
      let logger2 = tmp(13048).logger;
      logger2.warn("Cannot capture check-in. Client does not support sending check-ins.");
    }
    asyncGeneratorStep = captureCheckInResult;
    closure_3 = tmp(13062).timestampInSeconds();
    const tmpResult = tmp(13062);
    return tmp(13075).withIsolationScope(() => {
      try {
    ...
    let logger = tmp(13048).logger;
    logger.warn("Cannot capture check-in. No client defined.");
  }
  let obj3 = require("module_13075");
  captureCheckInResult = require("module_13059").uuid4();
};
