...
// Function ID: 13088
// Name: _flush
// Dependencies: [5, 13066, 13088, 13067, 13039, 13050, 13053, 13046, 13076, 13040, 13062]
  ...
  if (c1 === 2) {
    ...
    try {
      ...
      if (arg0 === 1) {
        ...
        const client = require("module_13066").getClient();
        if (client) {
          client.flush(tmp13);
        } else {
        ...
        c1 = 3;
        const obj3 = require("module_13066");
        tmp13 = closure_0;
      }
    } catch (tmp8) {
      c1 = tmp;
...
function endSession() {
  ...
  if (tmp3) {
    tmp(13062).closeSession(tmp3);
    const tmpResult = tmp(13062);
  }
  const isolationScope1 = _mod13066.getIsolationScope();
  const tmpResult4 = _mod13066;
  const currentScope1 = _mod13066.getCurrentScope();
  const tmpResult5 = _mod13066;
...
export const captureException = function captureException(arg0, arg1) {
  const currentScope = _mod13066.getCurrentScope();
  return currentScope.captureException(arg0, _mod13088.parseEventHintOrCaptureContext(arg1));
};
...
export const startSession = function startSession(arg0) {
  ...
  if (undefined === environment) {
    environment = tmp(13076).DEFAULT_ENVIRONMENT;
  }
  const userAgent = _mod13040.GLOBAL_OBJ.navigator || {}.userAgent;
  const tmp3 = _mod13040.GLOBAL_OBJ.navigator || {};
  ...
  if (tmp10) {
    tmp(13062).updateSession(session1, { status: "exited" });
    const tmpResult2 = tmp(13062);
  }
  endSession();
...
export const withMonitor = function withMonitor(monitorSlug, arg1, arg2) {
  ...
  function finishCheckIn(status) {
    const obj = { monitorSlug, status, checkInId: captureCheckInResult, duration: _mod13053.timestampInSeconds() - closure_3 };
    const currentScope = _mod13066.getCurrentScope();
    const client = _mod13066.getClient();
    if (client) {
      if (client.captureCheckIn) {
  ...
  let currentScope = require("module_13066").getCurrentScope();
  let obj = { monitorSlug, status: "in_progress" };
  let obj2 = require("module_13066");
  let client = require("module_13066").getClient();
  if (client) {
    if (client.captureCheckIn) {
      let captureCheckInResult = client.captureCheckIn(obj, arg2, currentScope);
    } else if (tmp(13067).DEBUG_BUILD) {
      let logger2 = tmp(13039).logger;
      logger2.warn("Cannot capture check-in. Client does not support sending check-ins.");
    }
    asyncGeneratorStep = captureCheckInResult;
    closure_3 = tmp(13053).timestampInSeconds();
    const tmpResult = tmp(13053);
    return tmp(13066).withIsolationScope(() => {
      try {
    ...
    let logger = tmp(13039).logger;
    logger.warn("Cannot capture check-in. No client defined.");
  }
  let obj3 = require("module_13066");
  captureCheckInResult = require("module_13050").uuid4();
};
