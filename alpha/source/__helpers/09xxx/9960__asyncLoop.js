...
// Function ID: 9961
// Name: _asyncLoop
// Dependencies: [41, 42, 5, 9961, 9962]
...
export const concatBytes = function concatBytes() {
  ...
  if (0 < items.length) {
    do {
      let arr2 = items[num];
      let bytesResult = Hash(9961).bytes(arr2);
...
export const randomBytes = function randomBytes(result) {
  ...
  if (Hash(9962).crypto) {
    if (typeof tmp(9962).crypto.getRandomValues === "function") {
      const _crypto2 = tmp(9962).crypto;
      const _Uint8Array = Uint8Array;
  ...
  if (Hash(9962).crypto) {
    if (typeof tmp(9962).crypto.randomBytes === "function") {
      const _crypto = tmp(9962).crypto;
