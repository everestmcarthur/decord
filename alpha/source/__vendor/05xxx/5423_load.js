...
// Function ID: 5424
// Name: load
// Dependencies: [5424, 5425, 5426, 5427, 5428, 5441, 5443, 5444, 5456, 5460, 5464, 5466, 5468, 5469, 5470, 5471, 5472, 5473, 5474, 5475, 5476, 5477, 5450]
// Exports: load

// Module 5423 (load)
import _modDef5424 from "module_5424" /* 5424 */;
...
function load(response, response) {
  ...
  if (typeof response === "string") {
    ...
    if (obj2.test(response)) {
      ...
      if (obj3.test(response)) {
        let resolved = Promise.resolve(response(5425).dataUriToBuffer(response));
        const obj5 = response(5425);
      } else {
        let obj7 = then;
        if (then === undefined) {
          obj7 = {};
...
function loadView(byteLength, response) {
  let obj = response;
  if (response === undefined) {
    obj = { expanded: false, async: false, includeUnknown: false, domParser: "accessible" };
  }
  ...
  function addPngTextTags(readTags) {
    if (flag) {
      ...
      if (closure_1.png) {
        objectAssignResult3 = _mod5425.objectAssign({}, closure_1.png, readTags);
      }
      closure_1.png = objectAssignResult3;
      let objectAssignResult4 = readTags;
      if (closure_1.pngText) {
        objectAssignResult4 = _mod5425.objectAssign({}, closure_1.png, readTags);
      }
  ...
  let obj2 = require("module_5427");
  ({ fileType, fileDataOffset, jfifDataOffset, tiffHeaderOffset, iptcDataOffset, xmpChunks, iccChunks, mpfDataOffset, pngHeaderOffset, pngTextChunks, pngChunkOffsets, vp8xChunkOffset, gifHeaderOffset } = require("module_5427").parseAppMarkers(byteLength, flag2));
  let flag4 = false;
  if (require("module_5428").USE_JPEG) {
    flag4 = false;
        ...
        if (readResult11 instanceof Promise) {
          items1.push(readResult11.then(function addIccTags(icc) {
            if (flag) {
              ...
              closure_1 = _mod5425.objectAssign({}, closure_1, icc);
            }
          }));
          flag9 = true;
        } else if (flag) {
  ...
  const parseAppMarkersResult = require("module_5427").parseAppMarkers(byteLength, flag2);
...
export default { load, loadView, errors: _modDef5424 };
export const errors = _modDef5424;
export { load };
export { loadView };
