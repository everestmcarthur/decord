...
// Dependencies: [41, 42, 5397, 5400, 5406, 5407, 5408, 5409, 5410]

// Module 5399 (FILE_TYPES_REQUIRED_ADDITIONAL_CHECK)
import AudioTypes from "AudioTypes" /* 5406 */;
import ImageTypes from "ImageTypes" /* 5407 */;
import VideoTypes from "VideoTypes" /* 5408 */;
import CompressedTypes from "CompressedTypes" /* 5409 */;
import OtherTypes from "OtherTypes" /* 5410 */;
...
const entry = {
  ...
  value: function getInfoByName(key10027) {
    return FileTypes(5397).fetchFromObject(FileTypes, key10027.toUpperCase());
  }
};
let items = [
  entry,
  {
    ...
    value: function getSignaturesByName(item10079) {
      return FileTypes(5397).fetchFromObject(FileTypes, item10079.toUpperCase()).signatures;
    }
  },
  {
    key: "detectSignature",
    ...
    value: function detectTypeByAdditionalCheck(fileChunk, found) {
      ...
      if (mapped.some((item) => {
        ...
        if (!mapped.includes("heic")) {
          let str6 = "flv";
          if (!FileTypes(5400).isFLV(fileChunk)) {
            let str8 = "mp4";
            if (tmp8(5400).isM4V(fileChunk)) {
              str8 = "mp4";
              if (!tmp8(5400).isHEIC(fileChunk)) {
                str8 = "m4v";
              }
            }
            str6 = str8;
        ...
        const result = FileTypes(5397).findMatroskaDocTypeElements(fileChunk);
        let str2 = "mkv";
        if ("mkv" !== result) {
          let str4;
          if ("webm" === result) {
            if (tmp3(5400).isWEBM(fileChunk)) {
              str4 = "webm";
            }
          }
          str2 = str4;
        ...
        if (mapped.some((item) => {
          ...
          if (FileTypes(5397).isAvifStringIncluded(fileChunk)) {
            str = "avif";
          }
        }
        return str;
