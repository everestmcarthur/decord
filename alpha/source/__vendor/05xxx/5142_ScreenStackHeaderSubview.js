...
// Dependencies: [109, 19, 17, 21, 5143, 5140, 5144, 5145, 5123, 5146]
...
// Module 5142 (ScreenStackHeaderSubview)
...
const forwardRefResult = noop.forwardRef((hidden, ref) => {
  ...
  const edgeInsetApplication = _mod5143.useEdgeInsetApplication(!hidden.hidden, flag, flag2, flag3, flag4);
  ...
  if (headerLeftBarButtonItems) {
    if (tmp(5140).isHeaderBarButtonsAvailableForCurrentPlatform) {
      result = tmp(5144).prepareHeaderBarButtonItems(headerLeftBarButtonItems, "left");
  ...
  if (headerRightBarButtonItems) {
    if (tmp(5140).isHeaderBarButtonsAvailableForCurrentPlatform) {
      result1 = tmp(5144).prepareHeaderBarButtonItems(headerRightBarButtonItems, "right");
      const tmpResult2 = tmp(5144);
    }
  }
  let isHeaderBarButtonsAvailableForCurrentPlatform = tmp(5140).isHeaderBarButtonsAvailableForCurrentPlatform;
...
export const ScreenStackHeaderSubview = _modDef5146;
...
export const ScreenStackHeaderBackButtonImage = (arg0) => {
  ...
  return jsx(_modDef5146, { type: "back", style: closure_9.headerSubview, synchronousShadowStateUpdatesEnabled: get_synchronousScreenUpdatesEnabledDefault.experiment.synchronousHeaderSubviewUpdatesEnabled, children: null });
};
