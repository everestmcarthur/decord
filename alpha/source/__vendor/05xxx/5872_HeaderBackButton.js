...
// Dependencies: [32, 19, 17, 21, 1485, 5873, 5844, 5874, 5845, 5878]
// Exports: HeaderBackButton

// Module 5872 (HeaderBackButton)
import Link from "Link" /* 1485 */;
import _modDef5844 from "module_5844" /* 5844 */;
import HeaderButton from "HeaderButton" /* 5878 */;
...
let c9 = 24;
let obj = { container: { paddingHorizontal: 0, minWidth: StyleSheet.hairlineWidth, marginVertical: 3, marginHorizontal: 11 }, label: { fontSize: 17, letterSpacing: 0.35 }, labelWrapper: { flexDirection: "row", alignItems: "flex-start", marginEnd: fn(5873).ICON_MARGIN }, icon: { width: 24, marginEnd: 3 }, iconWithLabel: {}, iconMaskContainer: { flex: 1, flexDirection: "row", justifyContent: "center" }, iconMaskFillerRect: { flex: 1, backgroundColor: "#000" }, iconMask: { height: 21, width: 13, marginStart: -14.5, marginVertical: 12, alignSelf: "center" }, flip: { transform: "scaleX(-1)" } };
...
export const HeaderBackButton = function HeaderBackButton(accessibilityLabel) {
  ...
  if (backImage) {
    ...
    const obj5 = { source: _modDef5844, tintColor, style: null };
    const items1 = [tmp10.icon, "minimal" !== displayMode && tmp10.iconWithLabel];
    obj5.style = items1;
    backImageResult = tmp9(tmp2(5873).HeaderIcon, obj5);
  }
  ...
  if ("minimal" !== displayMode) {
    ...
    if (titleLayout) {
      ...
      if (screenLayout) {
        ...
        diff = result - (c9 + tmp2(5873).ICON_MARGIN);
      }
    }
