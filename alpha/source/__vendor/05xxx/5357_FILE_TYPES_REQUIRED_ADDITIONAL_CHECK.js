...
// Dependencies: [41, 42, 5355, 5358, 5364, 5365, 5366, 5367, 5368]

// Module 5357 (FILE_TYPES_REQUIRED_ADDITIONAL_CHECK)
import AudioTypes from "AudioTypes" /* 5364 */;
import ImageTypes from "ImageTypes" /* 5365 */;
import VideoTypes from "VideoTypes" /* 5366 */;
import CompressedTypes from "CompressedTypes" /* 5367 */;
import OtherTypes from "OtherTypes" /* 5368 */;
...
const entry = {
  ...
  value: function getInfoByName(key10027) {
    return FileTypes(5355).fetchFromObject(FileTypes, key10027.toUpperCase());
  }
};
let items = [
  entry,
  {
    ...
    value: function getSignaturesByName(item10079) {
      return FileTypes(5355).fetchFromObject(FileTypes, item10079.toUpperCase()).signatures;
    }
  },
  {
    key: "detectSignature",
    ...
    value: function detectTypeByAdditionalCheck(fileChunk, found) {
      ...
      if (mapped.some((item) => {
        ...
        if (!mapped.includes("heic")) {
          let str6 = "flv";
          if (!FileTypes(5358).isFLV(fileChunk)) {
            let str8 = "mp4";
            if (tmp8(5358).isM4V(fileChunk)) {
              str8 = "mp4";
              if (!tmp8(5358).isHEIC(fileChunk)) {
                str8 = "m4v";
              }
            }
            str6 = str8;
        ...
        const result = FileTypes(5355).findMatroskaDocTypeElements(fileChunk);
        let str2 = "mkv";
        if ("mkv" !== result) {
          let str4;
          if ("webm" === result) {
            if (tmp3(5358).isWEBM(fileChunk)) {
              str4 = "webm";
            }
          }
          str2 = str4;
        ...
        if (mapped.some((item) => {
          ...
          if (FileTypes(5355).isAvifStringIncluded(fileChunk)) {
            str = "avif";
          }
        }
        return str;
