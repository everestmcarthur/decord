...
// Function ID: 6979
// Name: eventHandler
// Dependencies: [6960, 6898, 6913, 6900, 6962]
// Exports: eventHandler

// Module 6978 (eventHandler)
import _mod6898 from "module_6898" /* 6898 */;
import TouchEventType from "TouchEventType" /* 6900 */;
...
function handleStateChangeEvent(result, arg1, lastUpdateEvent, fn) {
  ...
  if (oldState === _mod6898.State.UNDETERMINED) {
    if (state === tmp(6898).State.BEGAN) {
      tmp(6960).runCallback(tmp(6913).CALLBACK_TYPE.BEGAN, arg1, result);
      const tmpResult = tmp(6960);
    }
  }
  if (oldState === _mod6898.State.BEGAN) {
    if (state === tmp(6898).State.ACTIVE) {
      if (fn != null) {
        fn(result);
      }
      tmp(6960).runCallback(tmp(6913).CALLBACK_TYPE.START, arg1, result);
      const tmpResult4 = tmp(6960);
    }
  }
  if (oldState !== state) {
    const obj2 = {};
    const merged = Object.assign(result);
    obj2.canceled = state === tmp(6898).State.FAILED || state === tmp(6898).State.CANCELLED;
    if (oldState === tmp(6898).State.ACTIVE) {
      if (fn != null) {
        fn(obj2);
      }
      tmp(6960).runCallback(tmp(6913).CALLBACK_TYPE.END, arg1, obj2);
      const tmpResult5 = tmp(6960);
    }
    const tmp7 = state === tmp(6898).State.FAILED || state === tmp(6898).State.CANCELLED;
    tmp(6960).runCallback(tmp(6913).CALLBACK_TYPE.FINALIZE, arg1, obj2);
    if (lastUpdateEvent) {
      lastUpdateEvent.lastUpdateEvent = undefined;
    }
    const tmpResult6 = tmp(6960);
  }
}
handleStateChangeEvent.__closure = { flattenAndFilterEvent: fn(6960).flattenAndFilterEvent, State: fn(6898).State, runCallback: fn(6960).runCallback, CALLBACK_TYPE: fn(6913).CALLBACK_TYPE };
...
function handleUpdateEvent(lastUpdateEvent, arg1, fn, lastUpdateEvent2) {
  ...
  DEFAULT_PROPS_TRANSFORMER.runCallback(_mod6913.CALLBACK_TYPE.UPDATE, arg1, result);
  if (lastUpdateEvent2) {
    lastUpdateEvent2.lastUpdateEvent = lastUpdateEvent;
  }
}
let obj = { flattenAndFilterEvent: fn(6960).flattenAndFilterEvent, State: fn(6898).State, runCallback: fn(6960).runCallback, CALLBACK_TYPE: fn(6913).CALLBACK_TYPE };
handleUpdateEvent.__closure = { flattenAndFilterEvent: fn(6960).flattenAndFilterEvent, runCallback: fn(6960).runCallback, CALLBACK_TYPE: fn(6913).CALLBACK_TYPE };
...
function handleTouchEvent(eventType, arg1) {
  if (eventType.eventType !== TouchEventType.TouchEventType.UNDETERMINED) {
    const tmpResult = tmp(6960);
    tmpResult.runCallback(tmp(6960).touchEventTypeToCallbackType(eventType.eventType), arg1, eventType);
    const tmpResult2 = tmp(6960);
  }
}
let obj2 = { flattenAndFilterEvent: fn(6960).flattenAndFilterEvent, runCallback: fn(6960).runCallback, CALLBACK_TYPE: fn(6913).CALLBACK_TYPE };
handleTouchEvent.__closure = { TouchEventType: fn(6900).TouchEventType, runCallback: fn(6960).runCallback, touchEventTypeToCallbackType: fn(6960).touchEventTypeToCallbackType };
    ...
    if (tmpResult.isStateChangeEvent(result)) {
      ...
      if (tmpResult6.isTouchEvent(result)) {
        if (typeof handleTouchEvent === "function") {
          if (result.eventType !== tmp(6900).TouchEventType.UNDETERMINED) {
            const tmpResult7 = tmp(6960);
            tmpResult7.runCallback(tmp(6960).touchEventTypeToCallbackType(result.eventType), arg2, result);
            const tmpResult8 = tmp(6960);
          }
        } else {
        ...
        if (typeof handleUpdateEvent === "function") {
          ...
          const result1 = tmp(6960).flattenAndFilterEvent(tmp9);
          const tmpResult9 = tmp(6960);
          tmp(6960).runCallback(tmp(6913).CALLBACK_TYPE.UPDATE, arg2, result1);
          if (lastUpdateEvent) {
            lastUpdateEvent.lastUpdateEvent = result;
          }
          const tmpResult10 = tmp(6960);
        } else {
          throw new TypeError("Trying to call a non-function");
        }
      }
...
const obj3 = { TouchEventType: fn(6900).TouchEventType, runCallback: fn(6960).runCallback, touchEventTypeToCallbackType: fn(6960).touchEventTypeToCallbackType };
eventHandler.__closure = { maybeExtractNativeEvent: fn(6960).maybeExtractNativeEvent, isEventForHandlerWithTag: fn(6960).isEventForHandlerWithTag, isStateChangeEvent: fn(6962).isStateChangeEvent, handleStateChangeEvent, isTouchEvent: fn(6962).isTouchEvent, handleTouchEvent, handleUpdateEvent };
