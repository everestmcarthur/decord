...
// Dependencies: [6959, 6951, 6897]
// Exports: configureRelations, ensureNativeDetectorComponent

// Module 6966 (traverseAndConfigureRelations)
  ...
  if (obj.isComposedGesture(gestures)) {
    ...
    let item = gestures.forEach((type) => {
      if (obj.isComposedGesture(type)) {
        let tmp13 = gestures.type !== tmp(6951).ComposedGestureName.Simultaneous;
        if (tmp13) {
          tmp13 = type.type === tmp(6951).ComposedGestureName.Simultaneous;
        }
        if (tmp13) {
          const handlerTags = type.handlerTags;
          const item = handlerTags.forEach((item) => set.add(item));
        }
        let tmp15 = tmp12.type === tmp(6951).ComposedGestureName.Simultaneous;
        if (tmp15) {
          tmp15 = type.type !== tmp(6951).ComposedGestureName.Simultaneous;
        }
        if (tmp15) {
        ...
        let tmp24 = type.type === tmp(6951).ComposedGestureName.Simultaneous;
        if (tmp24) {
          tmp24 = tmp12.type !== tmp(6951).ComposedGestureName.Simultaneous;
        }
        if (tmp24) {
          const handlerTags2 = tmp12.handlerTags;
          const item2 = handlerTags2.forEach((item) => set.delete(item));
        }
        let tmp26 = type.type !== tmp(6951).ComposedGestureName.Simultaneous;
        if (tmp26) {
          tmp26 = tmp12.type === tmp(6951).ComposedGestureName.Simultaneous;
        }
        if (tmp26) {
          const handlerTags3 = tmp12.handlerTags;
          const item3 = handlerTags3.forEach((item) => set.add(item));
        }
        if (gestures.type === tmp(6951).ComposedGestureName.Exclusive) {
          const handlerTags4 = type.handlerTags;
          const item4 = handlerTags4.forEach((item) => items.push(item));
        }
        let tmp29 = type.type === tmp(6951).ComposedGestureName.Exclusive;
        if (tmp29) {
          tmp29 = tmp12.type !== tmp(6951).ComposedGestureName.Exclusive;
        }
        if (tmp29) {
    ...
    gestures.gestureRelations = tmp(6959).prepareRelations(gestures.config, gestures.handlerTag);
