...
// Dependencies: [32, 19, 17, 21, 6910, 6970, 6906, 6965, 6907, 6897, 6935, 6966, 6967, 6959, 6968]
// Exports: InterceptingGestureDetector

// Module 6969 (InterceptingGestureDetector)
...
export const InterceptingGestureDetector = function InterceptingGestureDetector(gesture) {
  ...
  gesture(6910).useEnsureGestureHandlerRootView();
  const tmp6 = first1(closure_7(() => new Set()), 2);
  const first = tmp6[0];
  dependencyMap = tmp6[1];
  let items = [first];
  let prop;
  let obj = gesture(6910);
  ...
  if (prop) {
    let REANIMATED = tmp(6970).InterceptingDetectorMode.REANIMATED;
  } else {
    let prop1;
    if (gesture != null) {
      prop1 = gesture.config.dispatchesAnimatedEvents;
    }
    const InterceptingDetectorMode = tmp(6970).InterceptingDetectorMode;
  ...
  const tmp14 = first1 === gesture(6970).InterceptingDetectorMode.REANIMATED;
  if (first1 === gesture(6970).InterceptingDetectorMode.ANIMATED) {
    let ReanimatedNativeDetector = tmp(6906).AnimatedNativeDetector;
  } else if (tmp14) {
    ReanimatedNativeDetector = tmp(6965).ReanimatedNativeDetector;
  } else {
    ReanimatedNativeDetector = first(6907);
  }
  ...
  register(() => {
    ...
    if (prop) {
      closure_7.setMode(_mod6970.InterceptingDetectorMode.ANIMATED);
    } else {
      let prop1;
      if (tmp != null) {
      ...
      if (prop1) {
        closure_7.setMode(_mod6970.InterceptingDetectorMode.REANIMATED);
      }
    }
  }, items2);
  if (ReanimatedNativeDetector) {
    ...
    const result = tmp(6966).ensureNativeDetectorComponent(ReanimatedNativeDetector);
    const tmpResult = tmp(6966);
    const gestureRelationsUpdater = tmp(6967).useGestureRelationsUpdater(gesture);
    ...
    const tmpResult4 = tmp(6967);
    const detectorAttachmentGuard = tmp(6968).useDetectorAttachmentGuard(tmp8Result5);
    ...
    obj4.style = tmp(6906).nativeDetectorStyles.detector;
    ...
    return jsx(tmp(6970).InterceptingDetectorContext, obj3);
  } else {
    const _Error = Error;
    let error = new Error(tmp(6897).tagMessage("Gesture expects to run on the UI thread, but failed to create the Reanimated NativeDetector."));
    throw error;
  }
