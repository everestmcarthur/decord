...
// Function ID: 6950
// Name: hash
// Dependencies: [32, 6936, 6935, 6950, 6959]
...
import allowedNativeProps from "allowedNativeProps" /* 6950 */;
...
let c4 = 1.618;
fn(6936).NativeProxy.updateGestureHandlerConfig;
...
maybeUnpackValue.__closure = { Reanimated: fn(6935).Reanimated };
...
function getEnabledSharedValues(Reanimated) {
  if (undefined === _mod6935.Reanimated) {
    return [];
  } else {
    if (tmpResult.isComposedGesture(Reanimated)) {
      ...
      Reanimated = tmp(6935).Reanimated;
...
export const bindSharedValues = function bindSharedValues(shouldUseReanimatedDetector, handlerTag) {
  _require = handlerTag;
  if (undefined !== require("module_6935").Reanimated) {
    ...
    while (tmp27 !== undefined) {
      ...
      let tmp8 = tmp7;
      let tmp10 = _require;
      let Reanimated = require("module_6935").Reanimated;
      if (Reanimated.isSharedValue(tmp7)) {
        let Reanimated2 = tmp10(6935).Reanimated;
    ...
    function _loop(maybeSharedValue) {
      ...
      if (Reanimated.isSharedValue(maybeSharedValue)) {
        if (typeof hash === "function") {
          ...
          closure_1 = sum;
          const Reanimated2 = _mod6935.Reanimated;
          const fn = function t() {
