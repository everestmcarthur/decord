...
// Function ID: 6906
// Name: hash
// Dependencies: [32, 6892, 6891, 6906, 6915]
...
import allowedNativeProps from "allowedNativeProps" /* 6906 */;
...
let c4 = 1.618;
fn(6892).NativeProxy.updateGestureHandlerConfig;
...
maybeUnpackValue.__closure = { Reanimated: fn(6891).Reanimated };
...
function getEnabledSharedValues(Reanimated) {
  if (undefined === _mod6891.Reanimated) {
    return [];
  } else {
    if (tmpResult.isComposedGesture(Reanimated)) {
      ...
      Reanimated = tmp(6891).Reanimated;
...
export const bindSharedValues = function bindSharedValues(shouldUseReanimatedDetector, handlerTag) {
  _require = handlerTag;
  if (undefined !== require("module_6891").Reanimated) {
    ...
    while (tmp27 !== undefined) {
      ...
      if (Reanimated.isSharedValue(tmp7)) {
        let Reanimated2 = tmp10(6891).Reanimated;
    ...
    function _loop(maybeSharedValue) {
      ...
      if (Reanimated.isSharedValue(maybeSharedValue)) {
        if (typeof hash === "function") {
          ...
          closure_1 = sum;
          const Reanimated2 = _mod6891.Reanimated;
          const fn = function t() {
