...
// Dependencies: [32, 19, 17, 21, 6866, 6926, 6862, 6921, 6863, 6853, 6891, 6922, 6923, 6915, 6924]
// Exports: InterceptingGestureDetector

// Module 6925 (InterceptingGestureDetector)
...
export const InterceptingGestureDetector = function InterceptingGestureDetector(gesture) {
  ...
  gesture(6866).useEnsureGestureHandlerRootView();
  const tmp6 = first1(closure_7(() => new Set()), 2);
  const first = tmp6[0];
  dependencyMap = tmp6[1];
  let items = [first];
  let prop;
  let obj = gesture(6866);
  ...
  if (prop) {
    let REANIMATED = tmp(6926).InterceptingDetectorMode.REANIMATED;
  } else {
    let prop1;
    if (gesture != null) {
      prop1 = gesture.config.dispatchesAnimatedEvents;
    }
    const InterceptingDetectorMode = tmp(6926).InterceptingDetectorMode;
  ...
  const tmp14 = first1 === gesture(6926).InterceptingDetectorMode.REANIMATED;
  if (first1 === gesture(6926).InterceptingDetectorMode.ANIMATED) {
    let ReanimatedNativeDetector = tmp(6862).AnimatedNativeDetector;
  } else if (tmp14) {
    ReanimatedNativeDetector = tmp(6921).ReanimatedNativeDetector;
  } else {
    ReanimatedNativeDetector = first(6863);
  }
  ...
  register(() => {
    ...
    if (prop) {
      closure_7.setMode(_mod6926.InterceptingDetectorMode.ANIMATED);
    } else {
      let prop1;
      if (tmp != null) {
      ...
      if (prop1) {
        closure_7.setMode(_mod6926.InterceptingDetectorMode.REANIMATED);
      }
    }
  }, items2);
  if (ReanimatedNativeDetector) {
    ...
    const result = tmp(6922).ensureNativeDetectorComponent(ReanimatedNativeDetector);
    const tmpResult = tmp(6922);
    const gestureRelationsUpdater = tmp(6923).useGestureRelationsUpdater(gesture);
    ...
    const tmpResult4 = tmp(6923);
    const detectorAttachmentGuard = tmp(6924).useDetectorAttachmentGuard(tmp8Result5);
    ...
    obj4.style = tmp(6862).nativeDetectorStyles.detector;
    ...
    return jsx(tmp(6926).InterceptingDetectorContext, obj3);
  } else {
    const _Error = Error;
    let error = new Error(tmp(6853).tagMessage("Gesture expects to run on the UI thread, but failed to create the Reanimated NativeDetector."));
    throw error;
  }
