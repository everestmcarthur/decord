...
// Function ID: 6935
// Name: eventHandler
// Dependencies: [6916, 6854, 6869, 6856, 6918]
// Exports: eventHandler

// Module 6934 (eventHandler)
...
function handleStateChangeEvent(result, arg1, lastUpdateEvent, fn) {
  ...
  if (oldState === _mod6854.State.UNDETERMINED) {
    if (state === tmp(6854).State.BEGAN) {
      tmp(6916).runCallback(tmp(6869).CALLBACK_TYPE.BEGAN, arg1, result);
      const tmpResult = tmp(6916);
    }
  }
  if (oldState === _mod6854.State.BEGAN) {
    if (state === tmp(6854).State.ACTIVE) {
      if (fn != null) {
        fn(result);
      }
      tmp(6916).runCallback(tmp(6869).CALLBACK_TYPE.START, arg1, result);
      const tmpResult4 = tmp(6916);
    }
  }
  if (oldState !== state) {
    const obj2 = {};
    const merged = Object.assign(result);
    obj2.canceled = state === tmp(6854).State.FAILED || state === tmp(6854).State.CANCELLED;
    if (oldState === tmp(6854).State.ACTIVE) {
      if (fn != null) {
        fn(obj2);
      }
      tmp(6916).runCallback(tmp(6869).CALLBACK_TYPE.END, arg1, obj2);
      const tmpResult5 = tmp(6916);
    }
    const tmp7 = state === tmp(6854).State.FAILED || state === tmp(6854).State.CANCELLED;
    tmp(6916).runCallback(tmp(6869).CALLBACK_TYPE.FINALIZE, arg1, obj2);
    if (lastUpdateEvent) {
      lastUpdateEvent.lastUpdateEvent = undefined;
    }
    const tmpResult6 = tmp(6916);
  }
}
handleStateChangeEvent.__closure = { flattenAndFilterEvent: fn(6916).flattenAndFilterEvent, State: fn(6854).State, runCallback: fn(6916).runCallback, CALLBACK_TYPE: fn(6869).CALLBACK_TYPE };
...
function handleUpdateEvent(lastUpdateEvent, arg1, fn, lastUpdateEvent2) {
  ...
  DEFAULT_PROPS_TRANSFORMER.runCallback(_mod6869.CALLBACK_TYPE.UPDATE, arg1, result);
  if (lastUpdateEvent2) {
    lastUpdateEvent2.lastUpdateEvent = lastUpdateEvent;
  }
}
let obj = { flattenAndFilterEvent: fn(6916).flattenAndFilterEvent, State: fn(6854).State, runCallback: fn(6916).runCallback, CALLBACK_TYPE: fn(6869).CALLBACK_TYPE };
handleUpdateEvent.__closure = { flattenAndFilterEvent: fn(6916).flattenAndFilterEvent, runCallback: fn(6916).runCallback, CALLBACK_TYPE: fn(6869).CALLBACK_TYPE };
...
function handleTouchEvent(eventType, arg1) {
  if (eventType.eventType !== TouchEventType.TouchEventType.UNDETERMINED) {
    const tmpResult = tmp(6916);
    tmpResult.runCallback(tmp(6916).touchEventTypeToCallbackType(eventType.eventType), arg1, eventType);
    const tmpResult2 = tmp(6916);
  }
}
let obj2 = { flattenAndFilterEvent: fn(6916).flattenAndFilterEvent, runCallback: fn(6916).runCallback, CALLBACK_TYPE: fn(6869).CALLBACK_TYPE };
handleTouchEvent.__closure = { TouchEventType: fn(6856).TouchEventType, runCallback: fn(6916).runCallback, touchEventTypeToCallbackType: fn(6916).touchEventTypeToCallbackType };
    ...
    if (tmpResult.isStateChangeEvent(result)) {
      ...
      if (tmpResult6.isTouchEvent(result)) {
        if (typeof handleTouchEvent === "function") {
          if (result.eventType !== tmp(6856).TouchEventType.UNDETERMINED) {
            const tmpResult7 = tmp(6916);
            tmpResult7.runCallback(tmp(6916).touchEventTypeToCallbackType(result.eventType), arg2, result);
            const tmpResult8 = tmp(6916);
          }
        } else {
        ...
        if (typeof handleUpdateEvent === "function") {
          ...
          const result1 = tmp(6916).flattenAndFilterEvent(tmp9);
          const tmpResult9 = tmp(6916);
          tmp(6916).runCallback(tmp(6869).CALLBACK_TYPE.UPDATE, arg2, result1);
          if (lastUpdateEvent) {
            lastUpdateEvent.lastUpdateEvent = result;
          }
          const tmpResult10 = tmp(6916);
        } else {
          throw new TypeError("Trying to call a non-function");
        }
      }
...
const obj3 = { TouchEventType: fn(6856).TouchEventType, runCallback: fn(6916).runCallback, touchEventTypeToCallbackType: fn(6916).touchEventTypeToCallbackType };
eventHandler.__closure = { maybeExtractNativeEvent: fn(6916).maybeExtractNativeEvent, isEventForHandlerWithTag: fn(6916).isEventForHandlerWithTag, isStateChangeEvent: fn(6918).isStateChangeEvent, handleStateChangeEvent, isTouchEvent: fn(6918).isTouchEvent, handleTouchEvent, handleUpdateEvent };
