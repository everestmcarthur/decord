...
// Function ID: 4604
// Name: registerHost
// Dependencies: [4604, 4599]

// Module 4603 (registerHost)
import ACTIONS from "ACTIONS" /* 4599 */;
import print from "print" /* 4604 */;
...
function reducer(arg0, type) {
  ...
  if (ACTIONS.ACTIONS.REGISTER_HOST === type) {
  ...
  } else if (tmp4(4599).ACTIONS.DEREGISTER_HOST === type) {
    const hostName3 = type.hostName;
    delete tmp[tmp2];
    return obj;
  } else if (tmp4(4599).ACTIONS.ADD_UPDATE_PORTAL === type) {
    ...
    return obj;
  } else if (tmp4(4599).ACTIONS.REMOVE_PORTAL === type) {
    ...
    if (typeof removePortal === "function") {
      if (hostName in obj) {
        ...
        tmp4(4604).print(obj3);
        const tmp4Result = tmp4(4604);
      }
      return obj;
    } else {
      throw new TypeError("Trying to call a non-function");
