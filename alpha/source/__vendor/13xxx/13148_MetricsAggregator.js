...
// Dependencies: [32, 41, 42, 13145, 13062, 13149, 13150, 13053, 13151]

// Module 13148 (MetricsAggregator)
...
const entry = {
  ...
  value: function add(metricType, arg1, diff, none, tags) {
    ...
    if (arg5 === undefined) {
      timestampInSecondsResult = MetricsAggregator(13062).timestampInSeconds();
      const obj2 = MetricsAggregator(13062);
    }
    const self = this;
    const rounded = Math.floor(timestampInSecondsResult);
    const sanitizeMetricKeyResult = MetricsAggregator(13149).sanitizeMetricKey(arg1);
    const obj3 = MetricsAggregator(13149);
    const sanitizeTagsResult = MetricsAggregator(13149).sanitizeTags(obj);
    const obj4 = MetricsAggregator(13149);
    const sanitizeUnitResult = MetricsAggregator(13149).sanitizeUnit(str);
    const obj5 = MetricsAggregator(13149);
    const bucketKey = MetricsAggregator(13149).getBucketKey(metricType, sanitizeMetricKeyResult, sanitizeUnitResult, sanitizeTagsResult);
    const _buckets = this._buckets;
    value = _buckets.get(bucketKey);
    let num = 0;
    if (value) {
      num = 0;
      if (metricType === tmp5(13145).SET_METRIC_TYPE) {
    ...
    if (value) {
      ...
      const tmp15 = new tmp5(13150).METRIC_MAP[metricType](diff);
      obj7.metric = tmp15;
    ...
    const obj6 = MetricsAggregator(13149);
    const result1 = MetricsAggregator(13053).updateMetricSummaryOnActiveSpan(metricType, sanitizeMetricKeyResult, diff, sanitizeUnitResult, obj, bucketKey);
    self._bucketsTotalWeight = self._bucketsTotalWeight + obj7.metric.weight;
    if (self._bucketsTotalWeight >= MetricsAggregator(13145).MAX_WEIGHT) {
      self.flush();
    }
  }
};
  ...
  },
  {
    ...
    value: function _flush() {
      ...
      if (this._forceFlush) {
        ...
        const _Math = Math;
        const rounded = Math.floor(MetricsAggregator(13062).timestampInSeconds());
        const _Map = Map;
        const diff = rounded - MetricsAggregator(13145).DEFAULT_FLUSH_INTERVAL / 1000 - self._flushShift;
        const map = new Map();
  ...
  },
  {
    ...
    value: function _captureMetrics(_buckets) {
      if (_buckets.size > 0) {
        ...
        const result = MetricsAggregator(13151).captureAggregateMetrics(this._client, mapped);
        const obj = MetricsAggregator(13151);
      }
    }
  }
];
