...
// Dependencies: [13054, 13058, 13060, 13062, 13063, 13064, 13065, 13066, 13067, 13070, 13075, 13048]
...
function getStatusMessage(code) {
  if (code) {
    if (code.code !== _mod13065.SPAN_STATUS_UNSET) {
      let str = "ok";
      if (code.code !== tmp(13065).SPAN_STATUS_OK) {
...
export const addChildSpanToSpan = function addChildSpanToSpan(arg0, arg1) {
  ...
  const result = _mod13054.addNonEnumerableProperty(arg1, _sentryRootSpan, tmp2);
  if (arg0[_sentryChildSpans]) {
    ...
    const result1 = _mod13054.addNonEnumerableProperty(arg0, tmp6, set);
...
export const getActiveSpan = function getActiveSpan() {
  ...
  if (asyncContextStrategy.getActiveSpan) {
    ...
    const tmpResult = tmp(13070);
    activeSpan = tmpResult._getSpanForScope(tmp(13075).getCurrentScope());
