...
// Dependencies: [13104, 13055, 13093, 13054, 13057, 13076, 13048]

// Module 13138 (extraErrorDataIntegration)
        ...
        if (obj2.isError(obj.originalException)) {
          const tmp7 = (function _extractErrorData(originalException, arg1, maxValueLength) {
            try {
              ...
              if (num(13076).DEBUG_BUILD) {
                const logger = num(13048).logger;
          ...
          if (tmp7) {
            ...
            const normalizer = tmp4(13093);
            const normalizeResult = normalizer.normalize(tmp7, num);
            if (tmp4Result.isPlainObject(normalizeResult)) {
              const result = tmp4(13054).addNonEnumerableProperty(normalizeResult, "__sentry_skip_normalization__", true);
              obj3[tmp6] = normalizeResult;
              const tmp4Result2 = tmp4(13054);
            }
            let obj4 = {};
            const merged1 = Object.assign(contexts);
            obj4.contexts = obj3;
            tmp3 = obj4;
            tmp4Result = tmp4(13055);
          }
