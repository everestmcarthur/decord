...
// Dependencies: [2070, 2071]

// Module 2069 (CanonicalizeLocaleList)
import expBCP47Syntax2 from "expBCP47Syntax" /* 2070 */;
          ...
          if (StringResult in ObjectResult) {
            ...
            if (null == tmp4) {
              ...
              if (typeof tmp4 === "string") {
                ...
                if (isMatch1) {
                  let expVariantDupes = tmp5(2070).expVariantDupes;
                  let isMatch = expVariantDupes.test(str);
                  let tmp9 = !isMatch;
                  if (!isMatch) {
                    let expSingletonDupes = tmp5(2070).expSingletonDupes;
                ...
                if (isMatch1) {
                  ...
                  if (!tmp20) {
                    ...
                    let RegExpResult = RegExp(`(?:${tmp17(2070).expExtSequences.source})+`, "i");
...
function InitializeDateTimeFormat(prototype, arg1, arg2) {
  ...
  if (true === result["[[initializedIntlObject]]"]) {
    ...
    if ("[object Array]" !== (typeof call6 === "unknown" ? toString() : call6(formats))) {
      dateTimeFormats = _mod2071.createDateTimeFormats(formats);
    }
    obj10 = Object.create(List.prototype);
    List("basic", "best fit");
...
class List {
  constructor() {
    tmp = f27506(this, "length", { writable: true, value: 0 });
    if (!arguments.length) {
      return;
    } else {
      tmp3 = slice;
...
fn(obj, "__addLocaleData", {
  value(locale) {
    ...
    if (isMatch1) {
      const expVariantDupes = tmp(2070).expVariantDupes;
      const isMatch = expVariantDupes.test(locale);
      let tmp5 = !isMatch;
      if (!isMatch) {
        const expSingletonDupes = tmp(2070).expSingletonDupes;
