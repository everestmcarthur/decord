...
export const browserTracingIntegration = () => {
  ...
  closure_3 = { name: "a", source: "isArray" };
...
export const startBrowserTracingPageLoadSpan = function startBrowserTracingPageLoadSpan(f110492, tmp2Result, arg2) {
  f110492.emit("startPageLoadSpan", tmp2Result, arg2);
  const currentScope = _mod682.getCurrentScope();
  currentScope.setTransactionName(tmp2Result.name);
  if (f110492[_sentry_idleSpan]) {
    f110492.emit("afterStartPageLoadSpan", tmp3);
  }
  return f110492[_sentry_idleSpan];
};
