...
// Module 822 (convertPromptToMessages)
...
export const requestMessagesFromPrompt = function requestMessagesFromPrompt(setAttribute, data) {
  if (data[AI_MODEL_ID_ATTRIBUTE.AI_PROMPT_ATTRIBUTE]) {
    const attr = setAttribute.setAttribute("gen_ai.prompt", tmp(825).getTruncatedJsonString(data[tmp(undefined, 821).AI_PROMPT_ATTRIBUTE]));
  ...
  if (typeof tmp4 === "string") {
    if (!data[tmp(undefined, 823).GEN_AI_REQUEST_MESSAGES_ATTRIBUTE]) {
      if (!data[tmp(undefined, 821).AI_PROMPT_MESSAGES_ATTRIBUTE]) {
        ...
        if (arr.length) {
          const obj = {};
          obj[tmp(823).GEN_AI_REQUEST_MESSAGES_ATTRIBUTE] = tmp(825).getTruncatedJsonString(arr);
          obj[tmp(823).GEN_AI_REQUEST_MESSAGES_ORIGINAL_LENGTH_ATTRIBUTE] = arr.length;
          setAttribute.setAttributes(obj);
          const tmpResult3 = tmp(825);
        }
      }
    }
  }
  if (typeof data[AI_MODEL_ID_ATTRIBUTE.AI_PROMPT_MESSAGES_ATTRIBUTE] === "string") {
    try {
      ...
      if (Array.isArray(parsed)) {
        const obj2 = {};
        obj2[tmp(821).AI_PROMPT_MESSAGES_ATTRIBUTE] = undefined;
        obj2[tmp(823).GEN_AI_REQUEST_MESSAGES_ATTRIBUTE] = tmp(825).getTruncatedJsonString(parsed);
        obj2[tmp(823).GEN_AI_REQUEST_MESSAGES_ORIGINAL_LENGTH_ATTRIBUTE] = parsed.length;
        setAttribute.setAttributes(obj2);
        const tmpResult4 = tmp(825);
      }
    } catch (err) {
    }
  }
