...
// Dependencies: [377, 41, 42, 10695, 10693, 10697]

// Module 10694 (ReferenceWithTimezone)
  ...
  entry1,
  {
    ...
    value: function fromInput(instant, timezones) {
      if (instant instanceof Date) {
        ...
        const toTimezoneOffsetResult = ParsingComponents(10695).toTimezoneOffset(timezone, instant, timezones);
        const obj = Object.create(ParsingComponents.prototype);
        _classCallCheck(obj, ParsingComponents);
        if (null == instant) {
          const _Date2 = Date;
...
const items2 = [
  ...
  },
  {
    ...
    value: function addDurationAsImplied(EmptyDuration) {
      ...
      const addDurationResult = ParsingComponents(10693).addDuration(result, EmptyDuration);
...
const entry3 = {
  ...
  value: function createRelativeFromReference(reference, reverseDurationResult) {
    ...
    if (reverseDurationResult === undefined) {
      EmptyDuration = ParsingComponents(10693).EmptyDuration;
    }
    const addDurationResult = ParsingComponents(10693).addDuration(reference.getDateWithAdjustedTimezone(), EmptyDuration);
    const obj = Object.create(ParsingComponents.prototype);
    _classCallCheck(obj, ParsingComponents);
    obj._tags = new Set();
    obj.reference = reference;
      ...
      if (!("minute" in EmptyDuration)) {
        if (!("second" in EmptyDuration)) {
          if (!("millisecond" in EmptyDuration)) {
            ParsingComponents(10697).implySimilarTime(obj, addDurationResult);
    ...
    ParsingComponents(10697).assignSimilarTime(obj, addDurationResult);
    ParsingComponents(10697).assignSimilarDate(obj, addDurationResult);
