...
// Dependencies: [32, 109, 17, 21, 5852, 1485, 8161, 5843, 5116]
// Exports: useHeaderConfigProps

// Module 8160 (processBarButtonItems)
import ColorDefault from "Color" /* 5852 */;
...
export const useHeaderConfigProps = function useHeaderConfigProps(headerLargeTitle) {
  ...
  const tmpResult = colors(8161);
  let fontSize;
  [tmp10, tmp11, tmp12] = colors(8161).processFonts(items3);
  if ("fontSize" in tmp4) {
    fontSize = tmp4.fontSize;
  }
  const tmp9 = _slicedToArray(colors(8161).processFonts(items3), 3);
  const headerTitle1 = colors(5843).getHeaderTitle({ title, headerTitle }, route.name);
  if ("color" in tmp6) {
  ...
  const isSearchBarAvailableForCurrentPlatform = tmp(5116).isSearchBarAvailableForCurrentPlatform;
  ...
  if (typeof isSearchBarAvailableForCurrentPlatform === "boolean") {
    isSearchBarAvailableForCurrentPlatform2 = tmp(5116).isSearchBarAvailableForCurrentPlatform;
  }
  ...
  if (null != headerLeftResult) {
    ...
    if ("center" === headerTitleAlign) {
      ...
      let tmp32Result = tmp32(tmp(5116).ScreenStackHeaderLeftView, obj11);
    } else {
      if (typeof headerTitle === "function") {
        ...
        obj13.children = tmp62(tmp(5843).HeaderTitle, obj14);
      }
  ...
  if ("center" === headerTitleAlign) {
    if (typeof headerTitle !== "function") {
      ...
      headerTitleResult = tmp38(tmp(5843).HeaderTitle, obj15);
    }
    const obj16 = { children: headerTitleResult };
    tmp38Result = tmp38(tmp(5116).ScreenStackHeaderCenterView, obj16);
  }
  ...
  if (undefined !== headerBackIcon) {
    ...
    const obj17 = { source };
    let tmp40Result = closure_10(tmp(5116).ScreenStackHeaderBackButtonImage, obj17);
  } else {
    tmp40Result = null;
  }
  ...
  if (null != headerRightResult) {
    ...
    tmp42 = closure_10(tmp(5116).ScreenStackHeaderRightView, obj18);
  }
  items7[2] = tmp42;
  let tmp44 = null;
  if (isSearchBarAvailableForCurrentPlatform2) {
