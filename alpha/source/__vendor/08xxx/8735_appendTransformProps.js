...
// Dependencies: [8736, 8737, 8738]
// Exports: default, extractTransformSvgView

// Module 8735 (appendTransformProps)
import append from "append" /* 8736 */;
import peg$SyntaxError from "peg$SyntaxError" /* 8737 */;
import _mod8738 from "module_8738" /* 8738 */;
...
function transformToMatrix(arg0, arr) {
  ...
  if (arr) {
    ...
    if (Array.isArray(arr)) {
      ...
      try {
        const parsed = tmp3(8737).parse(arr);
        const tmp3Result5 = tmp3(8736);
        tmp3Result5.append(parsed[0], parsed[3], parsed[1], parsed[4], parsed[2], parsed[5]);
        const tmp3Result4 = tmp3(8737);
      } catch (tmp14) {
...
export const extractTransformSvgView = function extractTransformSvgView(transform) {
  if (typeof transform.transform === "string") {
    transform = _mod8738.parse(transform.transform);
  } else {
    transform = transform.transform;
  }
  return transform;
