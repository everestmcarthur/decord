...
// Function ID: 7157
// Name: JSFPSMonitor
// Dependencies: [7103, 7104, 7091, 7157]

// Module 7156 (JSFPSMonitor)
...
const items = [
  entry,
  {
    ...
    value: function startTracking() {
      ...
      if (0 !== this.startTime) {
        const _Error = Error;
        const error = new Error(JSFPSMonitor(7091).ErrorMessages.fpsMonitorAlreadyRunning);
        throw error;
      } else {
        const _Date = Date;
  ...
  },
  {
    ...
    value: function stopAndGetData() {
      ...
      const obj = { minFPS: JSFPSMonitor(7157).roundToDecimalPlaces(self.minFPS, 1), maxFPS: null, averageFPS: null };
      const obj2 = JSFPSMonitor(7157);
      obj.maxFPS = JSFPSMonitor(7157).roundToDecimalPlaces(self.maxFPS, 1);
      const obj3 = JSFPSMonitor(7157);
      obj.averageFPS = JSFPSMonitor(7157).roundToDecimalPlaces(self.averageFPS, 1);
      return obj;
    }
  }
];

export const JSFPSMonitor = _modDef7104(JSFPSMonitor, items);
