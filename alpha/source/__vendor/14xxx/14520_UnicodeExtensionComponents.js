...
// Module 14520 (UnicodeExtensionComponents)
...
export const UnicodeExtensionComponents = function UnicodeExtensionComponents(str) {
  _mod14515.invariant(str === str.toLowerCase(), "Expected extension to be lowercase");
  let num = 3;
  _mod14515.invariant("-u-" === str.slice(0, 3), "Expected extension to be a Unicode locale extension");
  const attributes = [];
  const keywords = [];
  if (3 < str.length) {
    while (true) {
      ...
      let tmp8 = require;
      let invariantResult2 = _mod14515.invariant(tmp6 >= 2, "Expected a subtag to have at least 2 characters");
      if (undefined === iter) {
        if (2 !== tmp6) {
          let tmp13 = iter;
      ...
      if (2 === tmp6) {
        ...
        if ("" === value) {
          ...
          let invariantResult3 = tmp8(14515).invariant(undefined !== iter, "Expected keyword to be defined");
