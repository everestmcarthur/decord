...
// Dependencies: [1161, 14479, 14501, 14480, 14494, 14496]
// Exports: PartitionNumberPattern

// Module 14506 (PartitionNumberPattern)
import digitsToString2 from "digitsToString" /* 14479 */;
...
let e = e_mod;
const module_14501 = e.__importDefault(_mod14501);
...
export const PartitionNumberPattern = function PartitionNumberPattern(internalSlots, isNaN) {
  ...
  if (isNaN.isNaN()) {
    ...
    if (!isNaN.isZero()) {
      _mod14480.invariant(isNaN.isFinite(), "Input must be a mathematical value");
  ...
  if ("never" !== signDisplay) {
    if ("auto" === signDisplay) {
      ...
      _mod14480.invariant("negative" === signDisplay, "signDisplay must be \"negative\"");
      let num8 = 0;
      if (roundedNumber.isNegative()) {
        num8 = 0;
        if (!roundedNumber.isZero()) {
  ...
  return module_14501.default({ roundedNumber, formattedString, exponent: num, magnitude: num2, sign: num7 }, internalSlots.dataLocaleData, pl, internalSlots);
};
