...
// Function ID: 14459
// Name: ResolveLocale
// Dependencies: [14459, 14464, 14465, 14460, 14466, 14467]
// Exports: ResolveLocale

// Module 14458 (ResolveLocale)
import LookupMatcher from "LookupMatcher" /* 14459 */;
import _mod14460 from "module_14460" /* 14460 */;
import BestFitMatcher from "BestFitMatcher" /* 14464 */;
...
export const ResolveLocale = function ResolveLocale(arg0, arg1, localeMatcher, arg3, arg4, fn) {
  ...
  if (LookupMatcherResult.extension) {
    let keywords = tmp7(14465).UnicodeExtensionComponents(LookupMatcherResult.extension).keywords;
  } else {
    keywords = [];
  }
  const items = [];
  ...
  if (0 < arg3.length) {
    do {
      ...
      let invariantResult = _mod14460.invariant(isArray, "keyLocaleData for ".concat(str, " must be an array"));
      let first = items1[0];
      let tmp19 = undefined === first;
      if (!tmp19) {
        tmp19 = typeof first === "string";
      }
      let invariantResult1 = _mod14460.invariant(tmp19, "value must be a string or undefined");
      ...
      let invariantResult2 = tmp14(14460).invariant(tmp24, "optionsValue must be a string or undefined");
      let str3 = tmp23;
      if (typeof tmp23 === "string") {
        let formatted = str.toLowerCase();
        str3 = tmp14(14466).CanonicalizeUValue(formatted, tmp23);
        if ("" === str3) {
          str3 = "true";
        }
      }
  ...
  let result = locale;
  if (items.length > 0) {
    result = tmp5(14467).InsertUnicodeExtensionAndCanonicalize(locale, [], items);
  }
  obj2.locale = result;
  return obj2;
};
