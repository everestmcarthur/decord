...
// Module 14539 (emitUnicodeLanguageId)
...
export const emitUnicodeLocaleId = function emitUnicodeLocaleId(parseUnicodeLocaleIdResult) {
  ...
  for (let num = 0; num < extensions.length; num = num + 1) {
    ...
    if ("u" === type) {
      ...
      let applyResult1 = push.apply(items1, obj3.__spreadArray(items3, fields.reduce((arr, item) => arr.concat(item), []), false));
    } else {
