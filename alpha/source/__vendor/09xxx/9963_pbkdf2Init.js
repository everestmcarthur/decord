...
// Function ID: 9964
// Name: pbkdf2Init
// Dependencies: [5, 9961, 9960, 9964]
// Exports: pbkdf2, pbkdf2Async

// Module 9963 (pbkdf2Init)
...
function pbkdf2Init(sha256, B, B, arg3) {
  ...
  if (c < 1) {
    ...
    const toBytesResult = tmp(9960).toBytes(B);
    const uint8Array = new Uint8Array(dkLen);
    const hmac = tmp(9964).hmac;
    const obj = hmac.create(sha256, toBytesResult);
    const toBytesResult1 = tmp(9960).toBytes(B);
