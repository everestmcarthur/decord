...
// Dependencies: [5, 4671, 1074, 1272, 17687, 1366, 1434, 573, 1271, 502, 1346, 2109, 1372, 1241, 1364, 17688, 1116, 17689, 17690, 7, 1231, 17693, 1980, 5082, 15940, 10005, 5734, 17699, 14093, 17700, 1462, 2]

// Module 17686 (superagentPatch)
...
import getTimeZoneDefault from "getTimeZone" /* 17689 */;
import trackHttpRequestDefault from "trackHttpRequest" /* 17690 */;
...
HTTPUtils.setRequestPatch({
  ...
  interceptResponse(statusCode, arg1, arg2) {
    ...
    if (401 === statusCode.statusCode) {
      ...
      if (code === AbortCodes.MFA_REQUIRED) {
        ...
        if (mfa) {
          ...
          statusCode(1980)(15940, dependencyMap.paths).then((openMFAModal) => {
    ...
    if (obj.isLimitedAccessErrorCode(statusCode.statusCode, code1)) {
      tmp7(1980)(5734, tmp8.paths).then((result) => {
        result.default();
      });
      flag = false;
      const promise3 = tmp7(1980)(5734, tmp8.paths);
    } else {
      ...
      if (tmp7Result.isLimitedAccessErrorCode(statusCode.statusCode, code2)) {
        tmp7(1980)(14093, tmp8.paths).then((result) => {
        ...
        flag = false;
        const promise2 = tmp7(1980)(14093, tmp8.paths);
      } else {
        ...
        if (flag) {
          tmp7(1980)(17700, tmp8.paths).then((openRestrictedHoursModal) => {
            const result = openRestrictedHoursModal.openRestrictedHoursModal();
          });
          flag = false;
          const promise = tmp7(1980)(17700, tmp8.paths);
        }
      }
