...
// Function ID: 8352
// Name: ChangeLogUtils
// Dependencies: [1929, 8352, 5217, 5236, 5237, 4746, 2]
// Exports: renderChangelogMessageMarkup

// Module 8351 (ChangeLogUtils)
...
function defaultRules(dependencyMap) {
  ...
  if (null != require("utils/ChangeLogUtils").customRules.strong) {
    let obj2 = {};
    const merged1 = Object.assign(tmp(8352).baseRules.strong);
    if (typeof tmp(8352).customRules.strong === "function") {
      const customRules = tmp(8352).customRules;
      let strong = customRules.strong(dependencyMap);
    } else {
      strong = tmp(8352).customRules.strong;
    }
    ...
    if (typeof tmp(8352).customRules.image === "function") {
      const customRules2 = tmp(8352).customRules;
      image = customRules2.image(dependencyMap);
    } else {
      image = tmp(8352).customRules.image;
    }
    const merged5 = Object.assign(image);
    obj.image = obj5;
    const obj6 = {};
    const merged6 = Object.assign(link);
    if (typeof tmp(8352).customRules.link === "function") {
      const customRules3 = tmp(8352).customRules;
      link = customRules3.link(dependencyMap);
    } else {
      link = tmp(8352).customRules.link;
    }
    const merged7 = Object.assign(link);
    obj.link = obj6;
    const obj7 = {};
    const merged8 = Object.assign(list);
    if (typeof tmp(8352).customRules.list === "function") {
      const customRules4 = tmp(8352).customRules;
      list = customRules4.list(dependencyMap);
    } else {
      list = tmp(8352).customRules.list;
    }
    ...
    if (typeof tmp(8352).customRules.lheading === "function") {
      const customRules5 = tmp(8352).customRules;
      lheading = customRules5.lheading(dependencyMap);
    } else {
      lheading = tmp(8352).customRules.lheading;
    }
    const merged11 = Object.assign(lheading);
    obj.lheading = obj9;
    const obj10 = {};
    const merged12 = Object.assign(heading);
    if (typeof tmp(8352).customRules.heading === "function") {
      const customRules6 = tmp(8352).customRules;
      heading = customRules6.heading(dependencyMap);
    } else {
      heading = tmp(8352).customRules.heading;
    }
    const merged13 = Object.assign(heading);
    obj.heading = obj10;
    const obj11 = {};
    const merged14 = Object.assign(blockQuote);
    if (typeof tmp(8352).customRules.blockQuote === "function") {
      const customRules7 = tmp(8352).customRules;
      blockQuote = customRules7.blockQuote(dependencyMap);
    } else {
      blockQuote = tmp(8352).customRules.blockQuote;
    }
    const merged15 = Object.assign(blockQuote);
    obj.blockQuote = obj11;
    const obj12 = {};
    const merged16 = Object.assign(paragraph);
    if (typeof tmp(8352).customRules.paragraph === "function") {
      const customRules8 = tmp(8352).customRules;
      paragraph = customRules8.paragraph(dependencyMap);
    } else {
      paragraph = tmp(8352).customRules.paragraph;
    }
...
export default {
  ...
  getSpecialRules(dependencyMap) {
    ...
    if (typeof require("utils/ChangeLogUtils").customRules.lheading === "function") {
      const customRules = tmp3(8352).customRules;
      lheading = customRules.lheading(dependencyMap);
    } else {
      lheading = tmp3(8352).customRules.lheading;
    }
