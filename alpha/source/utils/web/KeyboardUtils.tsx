...
// Dependencies: [32, 7744, 1364, 12, 14329, 14330, 14331, 1370, 2]
...
// Module 14328 (utils/KeyboardUtils)
import PlatformUtils2 from "PlatformUtils" /* 1364 */;
import GlobalUtils from "GlobalUtils" /* 1370 */;
import keyCodeDefault from "keyCode" /* 14329 */;
import KeyboardLayoutMapUtils from "KeyboardLayoutMapUtils" /* 14330 */;
...
function getCodeToKeyLanguageCorrection(keyCode, key, arg2) {
  ...
  if (BACKTICK_CODES.has(keyCode)) {
    const layoutMap = tmp2(14330).getLayoutMap();
    ...
    if (key === value) {
      ...
      const exactKeyboardEventMatchFromAny = tmp2(14330).getExactKeyboardEventMatchFromAny(obj);
      if ("\\" === key) {
        let tmp6 = key;
        return tmp6;
      }
      ...
      tmp6 = str5;
      const tmp2Result2 = tmp2(14330);
    }
    const tmp2Result = tmp2(14330);
  } else {
    return key;
  }
}
...
const KeyboardConstants = fn(7744);
...
export const toCombo = function toCombo(shortcut) {
  if (arg1 !== undefined) {
    ...
    const parts = shortcut.replace(/numpad plus/i, "").replace(/NUMPAD \+/i, "numpad plus").replace(/mod/i, KEYBOARD_KEY(14331).modKey).split("+");
