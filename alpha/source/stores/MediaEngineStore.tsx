...
// Dependencies: [32, 5, 1993, 1998, 4671, 1999, 4801, 1220, 502, 14259, 9911, 2041, 4779, 1372, 14260, 1074, 4798, 4965, 14261, 1084, 14262, 4781, 3, 1091, 1364, 4811, 13562, 2036, 5350, 14263, 14264, 14265, 9780, 14266, 12, 14267, 14078, 14268, 14269, 14270, 14271, 14272, 14273, 9685, 1241, 573, 10251, 14274, 14275, 14276, 5632, 14277, 14322, 14323, 14324, 14325, 14326, 14327, 4891, 4750, 4377, 510, 14328, 10238, 5777, 14332, 14333, 1115, 5078, 14334, 14335, 14336, 38, 14090, 14089, 14337, 14072, 1231, 13933, 9696, 504, 14338, 14339, 14340, 14341, 2]
...
import DesktopNativeUtilsDefault from "DesktopNativeUtils" /* 5777 */;
...
import VideoGuardExperiment2 from "VideoGuardExperiment" /* 13562 */;
import KrispUtilsDefault from "KrispUtils" /* 14072 */;
...
import GoLiveHdrExperiment from "GoLiveHdrExperiment" /* 14263 */;
import StreamZeroVadLeadingExperiment2 from "StreamZeroVadLeadingExperiment" /* 14264 */;
import AGC2MobileExperimentDefault from "AGC2MobileExperiment" /* 14265 */;
import PTTEducationExperimentDefault from "PTTEducationExperiment" /* 14266 */;
import MuteAwareNoiseCancellationExperiment from "MuteAwareNoiseCancellationExperiment" /* 14267 */;
import HookAll from "Hook" /* 14268 */;
import AudioFidelityExperiment from "AudioFidelityExperiment" /* 14274 */;
import _modDef14277 from "module_14277" /* 14277 */;
import AudioEffectsExperimentDefault from "AudioEffectsExperiment" /* 14332 */;
import SilenceWarningExperimentDefault from "SilenceWarningExperiment" /* 14337 */;
import InputWatcherDefault from "InputWatcher" /* 14338 */;
import DisableCameraSimulcastExperiment2 from "DisableCameraSimulcastExperiment" /* 14341 */;
...
import BitRateStore from "BitRateStore" /* 14259 */;
import CertifiedDeviceStore from "CertifiedDeviceStore" /* 9911 */;
import ChannelStore from "ChannelStore" /* 2041 */;
import RTCConnectionStore from "RTCConnectionStore" /* 4779 */;
import UserStore from "UserStore" /* 1372 */;
import VideoQualityModeStore from "VideoQualityModeStore" /* 14260 */;
import NativePermissionUtils_mod from "NativePermissionUtils" /* 5350 */;
        ...
        if (null != tmp2) {
          ...
          if (null != tmp2.desktopSource) {
            ...
            if (isWindowsResult) {
              ...
              isWindowsResult = _modDef14277.satisfies(release, __initData7);
            }
            if (!isWindowsResult) {
              ...
              if (!isWindowsResult1) {
                ...
                if (videoHook2) {
                  const VideoHookDX12Experiment = tmp69(14271).VideoHookDX12Experiment;
                  enabled2 = VideoHookDX12Experiment.getConfig({ location: "updateVideo" }).enabled;
                }
                const UpscaleSmallCapturedFramesExperiment = tmp69(14272).UpscaleSmallCapturedFramesExperiment;
                ...
                if (isMacResult) {
                  ...
                  isMacResult = _modDef14277.satisfies(release1, __initData4);
                }
                ...
                obj7.enableGlobalFramePoolLock = tmp69(14273).getGlobalFramePoolLockExperimentConfig({ location: "updateVideo" }).enabled;
                ...
                const tmp69Result7 = tmp69(14273);
              } else if (true === c134) {
                const WGCDirtyRegionsExperiment = tmp69(14269).WGCDirtyRegionsExperiment;
                enabled = WGCDirtyRegionsExperiment.getConfig({ location: "updateVideo" }).enabled;
              } else {
                const WGCDirtyRegionsAllExperiment = tmp69(14270).WGCDirtyRegionsAllExperiment;
              ...
              if (isWindowsResult2) {
                ...
                isWindowsResult2 = _modDef14277.satisfies(release2, __initData5);
              }
...
function handleVideoFilterError(code, arg1) {
  ...
  if (arg1 !== constants10.LIVE) {
    ...
    if (constants9.UNSUPPORTED === code) {
      AVUnderlyingError = tmp4(9685).AVUnderlyingError;
      let VideoBackgroundInitFailed = AVUnderlyingError.VideoBackgroundUnsupported;
    } else if (tmp5.INIT_FAILED === code) {
      VideoBackgroundInitFailed = tmp4(9685).AVUnderlyingError.VideoBackgroundInitFailed;
    }
...
function updateConnectionVoiceProcessing(setEchoCancellation) {
  ...
  const tmp10ResultResult = tmp10(10251)(noiseCancellation, mediaEngineStore.getSystemMicrophoneMode());
  ...
  const tmp10Result6 = tmp10(14265);
  ...
  const tmp10Result = tmp10(10251);
  let supportsResult = true === tmp2.audioMixerSettings.enabled && tmp10(14089).getConfig({ location: "MediaEngineStore" }).enabled;
  if (supportsResult) {
  ...
  const tmp10Result7 = tmp10(14089);
  ...
  const tmp10Result8 = tmp10(10251);
  const tmp10Result3Result = tmp10(10251)(tmpResult.noiseCancellation, mediaEngineStore.getSystemMicrophoneMode());
  ...
  let isWindowsResult = PlatformUtils.isWindows();
  if (isWindowsResult) {
    const tmp10Result10 = tmp10(4377);
    let release;
    if (tmp10Result10 != null) {
      release = tmp10Result10.os.release;
    }
    isWindowsResult = tmp10(14277).satisfies(release, c156);
    const tmp10Result9 = tmp10(14277);
  }
  if (isWindowsResult) {
    if (setEchoCancellation.context === MediaEngineContextTypes.DEFAULT) {
      const systemwideEchoCancellationExperimentConfig = tmp30(14275).getSystemwideEchoCancellationExperimentConfig({ location: "updateConnectionVoiceProcessing" });
...
function setLoopback() {
  ...
  const tmp10Result = tmp10(10251);
  const tmp10ResultResult = tmp10(10251)(tmpResult.noiseCancellation, mediaEngineStore.getSystemMicrophoneMode());
...
fn(4811);
const BaseConnectionEvent = fn(4811);
...
const outputDevices = { [DEFAULT_DEVICE_ID]: { id: DEFAULT_DEVICE_ID, deviceType: DeviceTypes.AUDIO_OUTPUT, index: 0, name: "No Output Devices", disabled: true, guid: "call", hardwareId: "useRef", containerId: "alignItems" } };
...
prototype["initialize"] = function initialize() {
  ...
  closure_73.on(pollMetrics(4811).MediaEngineEvent.Connection, (setAttenuation) => {
    ...
    tmp9Result19 = pollMetrics(14276);
    ...
    if (tmp9Result20.isWindows()) {
      ...
      if (tmp9Result21.isMac()) {
        ...
        if (satisfiesResult) {
          ...
          satisfiesResult = tmp25(14277).satisfies(release, closure_39);
          const tmp25Result = tmp25(14277);
        }
    ...
    if (tmp9Result26.isWeb()) {
      setAttenuation.setExperimentFlag(tmp22.BROWSER_HEVC, tmp25(14323).getConfig({ location: "MediaEngineStore" }).enabled);
      const tmp25Result2 = tmp25(14323);
    }
    ...
    if (enabled) {
      enabled = tmp9(14324).getWmfGpuEncode("MediaEngineStore").enabled;
      const tmp9Result28 = tmp9(14324);
    }
    if (enabled) {
    ...
    if (enabled2) {
      enabled2 = tmp9(14325).getWmfGpuEncodeIntel("MediaEngineStore").enabled;
      const tmp9Result30 = tmp9(14325);
    }
    if (enabled2) {
      setAttenuation.setExperimentFlag(tmp22.WMF_GPU_ENCODE, true);
    ...
    if (enabled3) {
      enabled3 = tmp9(14326).getWmfCpuEncodeIntel("MediaEngineStore").enabled;
      const tmp9Result32 = tmp9(14326);
    }
    if (enabled3) {
      setAttenuation.setExperimentFlag(tmp22.INTEL_GPU_DISABLE, true);
    }
    ...
    if (enabled4) {
      enabled4 = tmp9(14324).getWmfGpuEncode("MediaEngineStore").enabled;
      const tmp9Result34 = tmp9(14324);
    }
    if (enabled4) {
      setAttenuation.setExperimentFlag(tmp22.WMF_GPU_ENCODE, true);
    }
    const result2 = closure_73.setHasFullbandPerformance(tmp25(14327)());
    ...
    setAttenuation.on(pollMetrics(4811).BaseConnectionEvent.Speaking, (userId, speakingFlags, arg2, voiceDb) => {
    ...
    if (setAttenuation.context === constants4.DEFAULT) {
      c99 = false;
      setAttenuation.on(tmp84(4811).BaseConnectionEvent.SpeakingWhileMuted, () => {
        c99 = true;
        if (!c99) {
          closure_1_70.emitChange();
        }
    ...
    setAttenuation.on(pollMetrics(4811).BaseConnectionEvent.DesktopSourceEnd, (endReason, errorCode) => {
    ...
    setAttenuation.on(pollMetrics(4811).BaseConnectionEvent.InteractionRequired, (required) => {
      closure_1_1(573).dispatch({ type: "MEDIA_ENGINE_INTERACTION_REQUIRED", required });
    });
    setAttenuation.on(pollMetrics(4811).BaseConnectionEvent.VideoHookInitialize, (backend, format, framebuffer_format, sample_count, success, reinitialization) => {
      ...
      if (null != desktopSource) {
        ...
        const merged = Object.assign(closure_1_1(4891)(desktopSource1));
        obj.track(constants.VIDEOHOOK_INITIALIZED, obj2);
        const tmp15 = closure_1_1(4891);
      }
    });
    setAttenuation.on(pollMetrics(4811).BaseConnectionEvent.NoiseCancellationError, closure_153);
    setAttenuation.on(pollMetrics(4811).BaseConnectionEvent.VoiceActivityDetectorError, closure_153);
    setAttenuation.on(pollMetrics(4811).BaseConnectionEvent.SdpError, (operation, error, type, sdp) => {
      closure_1_1(1241).track(constants.SDP_ERROR, { operation, error, type, sdp });
    });
    setAttenuation.on(pollMetrics(4811).BaseConnectionEvent.VideoState, (videoState) => {
  ...
  closure_73.on(pollMetrics(4811).MediaEngineEvent.DeviceChange, (inputDevices, outputDevices, videoDevices) => {
    timeout.stop();
    closure_1(573).dispatch({ type: "MEDIA_ENGINE_DEVICES", inputDevices, outputDevices, videoDevices });
  });
  closure_73.on(pollMetrics(4811).MediaEngineEvent.VolumeChange, (inputVolume, outputVolume) => {
    closure_1(573).dispatch({ type: "AUDIO_VOLUME_CHANGE", inputVolume, outputVolume });
  });
  closure_73.on(pollMetrics(4811).MediaEngineEvent.DesktopSourceEnd, (endReason, errorCode) => {
    closure_1(573).dispatch({ type: "MEDIA_ENGINE_SET_GO_LIVE_SOURCE", settings: null, endReason, errorCode });
  });
  closure_73.on(pollMetrics(4811).MediaEngineEvent.AudioPermission, (granted) => {
    c127 = true;
    closure_1(573).dispatch({ type: "MEDIA_ENGINE_PERMISSION", kind: "audio", granted });
  });
  closure_73.on(pollMetrics(4811).MediaEngineEvent.VideoPermission, (granted) => {
    closure_1(573).dispatch({ type: "MEDIA_ENGINE_PERMISSION", kind: "video", granted });
  });
  closure_73.on(pollMetrics(4811).MediaEngineEvent.WatchdogTimeout, asyncGeneratorStep(async (arg0, value) => {
    if (c6 === 2) {
      c6 = 3;
      throw new TypeError("Generator functions may not be called on executing generators");
    } else if (tmp6 === 3) {
        ...
        if (0 === c5) {
          if (arg0 === 1) {
            ...
            if ("canary" === window.GLOBAL_ENV.RELEASE_CHANNEL) {
              ...
              c5 = 2;
              c6 = 1;
              const obj6 = { value: closure_1(4750).submitLiveCrashReport(obj4), done: false };
              return obj6;
            }
          }
        } else if (1 === tmp7) {
  ...
  closure_73.on(pollMetrics(4811).MediaEngineEvent.VideoInputInitialized, (description) => {
  ...
  closure_73.on(pollMetrics(4811).MediaEngineEvent.AudioInputInitialized, (description) => {
  ...
  closure_73.on(pollMetrics(4811).MediaEngineEvent.ClipsRecordingRestartNeeded, () => {
    closure_1(573).dispatch({ type: "CLIPS_RESTART" });
  });
  closure_73.on(pollMetrics(4811).MediaEngineEvent.ClipsInitFailure, (errMsg, applicationName) => {
  ...
  closure_73.on(pollMetrics(4811).MediaEngineEvent.ClipsRecordingHealthy, () => {
    if (0 !== c83) {
      const _HermesInternal = HermesInternal;
  ...
  closure_73.on(pollMetrics(4811).MediaEngineEvent.ClipsRecordingReadyChanged, (arg0) => {
    if (closure_84 !== arg0) {
      const _HermesInternal = HermesInternal;
  ...
  closure_73.on(pollMetrics(4811).MediaEngineEvent.ClipsBridgeIdleShutdown, () => {
    logger.info("Clips bridge idle shutdown");
  });
  closure_73.on(pollMetrics(4811).MediaEngineEvent.ClipsRecordingEnded, (arg0, soundshareId) => {
    let id;
    if (_null != null) {
  ...
  closure_73.on(pollMetrics(4811).MediaEngineEvent.NativeScreenSharePickerUpdate, (existing, content) => {
    closure_1(573).dispatch({ type: "NATIVE_SCREEN_SHARE_PICKER_UPDATE", existing, content });
  });
  closure_73.on(pollMetrics(4811).MediaEngineEvent.NativeScreenSharePickerCancel, (existing) => {
    closure_1(573).dispatch({ type: "NATIVE_SCREEN_SHARE_PICKER_CANCEL", existing });
  });
  closure_73.on(pollMetrics(4811).MediaEngineEvent.NativeScreenSharePickerError, (error) => {
    closure_1(573).dispatch({ type: "NATIVE_SCREEN_SHARE_PICKER_ERROR", error });
  });
  closure_73.on(pollMetrics(4811).MediaEngineEvent.AudioDeviceModuleError, (audio_device_module, code, device_name) => {
    closure_1(1241).track(constants.AUDIO_DEVICE_MODULE_ERROR, { audio_device_module, code, device_name });
  });
  closure_73.on(pollMetrics(4811).MediaEngineEvent.VideoCodecError, (mode) => {
    if ("encode" === mode.mode) {
      let VIDEO_DECODE_ERROR = pollMetrics(9685).AVError.VIDEO_ENCODE_ERROR;
      let tmp3 = pollMetrics;
    } else {
      VIDEO_DECODE_ERROR = pollMetrics(9685).AVError.VIDEO_DECODE_ERROR;
      tmp3 = pollMetrics;
    }
    const obj = { videoCodec: mode.codecStandard, errorMessage: mode.message };
    if (VIDEO_DECODE_ERROR === tmp3(9685).AVError.VIDEO_ENCODE_ERROR) {
    ...
    tmp3(9685).reportAVError(obj3);
  });
  closure_73.on(pollMetrics(4811).MediaEngineEvent.ConnectionStats, (arr) => {
  ...
  closure_73.on(pollMetrics(4811).MediaEngineEvent.VoiceProcessingError, handleVoiceProcessingError);
  closure_73.on(pollMetrics(4811).MediaEngineEvent.VideoFilterError, handleVideoFilterError);
  closure_73.on(pollMetrics(4811).MediaEngineEvent.SpatialAudioStatus, handleSpatialAudioStatus);
  closure_73.on(pollMetrics(4811).MediaEngineEvent.VoiceQueueMetrics, (arg0) => {
  ...
  closure_73.on(pollMetrics(4811).MediaEngineEvent.DeviceHardwareMutedChange, (deviceGuid, hardwareMuted) => {
    closure_1(573).dispatch({ type: "AUDIO_INPUT_DEVICE_HARDWARE_MUTED_CHANGED", deviceGuid, hardwareMuted });
  });
  closure_73.on(pollMetrics(4811).MediaEngineEvent.SystemMicrophoneModeChange, (arg0) => {
    closure_131 = arg0;
  ...
  _modDef12.each(value2, (modeOptions) => {
    ...
    if (tmp8) {
      modeOptions.modeOptions.shortcut = tmp2(14328).toCombo(modeOptions.modeOptions.shortcut);
      const tmp2Result = tmp2(14328);
    }
...
prototype["supportsExperimentalSoundshare"] = function supportsExperimentalSoundshare() {
  ...
  if (supportsResult) {
    ...
    supportsResult = _modDef14277.satisfies(release, __initData9);
  }
  return supportsResult;
};
prototype["supportsHookSoundshare"] = function supportsHookSoundshare() {
  ...
  if (isWindowsResult) {
    ...
    isWindowsResult = _modDef14277.satisfies(release, __initData8);
  }
...
prototype["getVideoEncoderExperiments"] = function getVideoEncoderExperiments(STREAM, streamer) {
  ...
  if (isWindowsResult) {
    ...
    const VideoCaptureDeviceNoReuseExperiment = tmp6(14340).VideoCaptureDeviceNoReuseExperiment;
...
prototype["supportsScreenSoundshare"] = function supportsScreenSoundshare() {
  if (obj.isMac()) {
    ...
    if (supportsResult) {
      ...
      supportsResult = _modDef14277.satisfies(release, closure_1_25);
    }
    if (supportsResult) {
      ...
      if (satisfiesResult) {
        ...
        satisfiesResult = _modDef14277.satisfies(release1, __initData4);
      }
...
const mediaEngineStore = new MediaEngineStore(DispatcherDefault, {
  ...
  CONNECTION_OPEN: function handleConnectionOpen(sessionId) {
    ...
    if (enabled) {
      enabled = tmp2(14335).getIOSAudioInterruptExperimentConfig("handleConnectionOpen").enabled;
      const tmp2Result4 = tmp2(14335);
    }
    if (enabled) {
    ...
    if (tmp2Result5.isIOS()) {
      ...
      if (setNcModels != null) {
        setNcModels(tmp2(14336).KRISP_NC_MODELS);
      }
      mediaEngineStore.emitChange();
    }
    ...
    if (tmp2Result6.isWindows()) {
      if (obj8.getConfig({ location: "MediaEngineStore.setInputDevice" }).probeAudioEffects) {
        ...
        if (null != guid) {
          tmp21(14333)(tmp23.guid, inputDeviceId, obj3);
        }
      }
  ...
  AUDIO_SET_LOCAL_VIDEO_DISABLED: function handleSetLocalVideoDisabled(arg0) {
    ...
    if (!tmp22) {
      ...
      if (isAutomatic) {
        tmp3(14090)(userId, tmp12 ? constants12.AUTO_DISABLE : constants12.AUTO_ENABLE, tmp11);
        if (tmp12) {
          obj.add(userId);
        } else {
          obj.delete(userId);
        ...
        if (hasItem) {
          if (!tmp12) {
            ...
            tmp3(14090)(userId, constants12.MANUAL_REENABLE, tmp11);
          }
        }
        tmp3(14090)(userId, tmp12 ? constants12.MANUAL_DISABLE : constants12.MANUAL_ENABLE, tmp11);
      }
      if (tmp19) {
        tmp19 = !tmp12;
      }
  ...
  MEDIA_ENGINE_DEVICES: function handleDevices(videoDevices) {
    ...
    if (closure_95) {
      if (!tmp9Result2.isEqual(valueResult4, tmp37)) {
        ...
        let tmp49 = "Firefox" === tmp9(5078).name;
        if (tmp49) {
          tmp49 = "" === closure_96;
        }
        if (tmp49) {
  ...
  CLIPS_INIT: function handleClipsInit(applicationName) {
    ...
    if (obj.isClipsEnabled()) {
      if (null != DiscordNativeDefault) {
        ...
        const pidFromDesktopSource = tmp3(4750).getPidFromDesktopSource(sourceId);
        const tmp3Result = tmp3(4750);
        ...
        enabled = videoHook2;
        if (videoHook2) {
          const VideoHookDX12Experiment = tmp(14271).VideoHookDX12Experiment;
          enabled = VideoHookDX12Experiment.getConfig({ location: "handleClipsInit" }).enabled;
        }
        const UpscaleSmallCapturedFramesExperiment = tmp(14272).UpscaleSmallCapturedFramesExperiment;
        ...
        ({ minCaptureWidth, minCaptureHeight } = config);
        const obj8 = result;
        const tmpResult4 = tmp(14263);
        let isWindowsResult = tmp(1364).isWindows();
        if (isWindowsResult) {
          const tmp3Result6 = tmp3(4377);
          let release;
          if (tmp3Result6 != null) {
            release = tmp3Result6.os.release;
          }
          isWindowsResult = tmp3(14277).satisfies(release, __initData7);
          const tmp3Result5 = tmp3(14277);
        }
        ...
        if (isMacResult) {
          const tmp3Result8 = tmp3(4377);
          let release1;
          if (tmp3Result8 != null) {
            release1 = tmp3Result8.os.release;
          }
          isMacResult = tmp3(14277).satisfies(release1, __initData4);
          const tmp3Result7 = tmp3(14277);
        }
