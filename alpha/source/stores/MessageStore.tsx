...
// Function ID: 4944
// Name: MessageStore
// Dependencies: [32, 5, 2015, 4944, 2026, 502, 1958, 5441, 2013, 2022, 1980, 4361, 4371, 2012, 4543, 1372, 1074, 3, 11, 5442, 5447, 1987, 5445, 4945, 1385, 12, 7796, 4955, 4373, 8036, 13971, 504, 11988, 1895, 573, 2]

// Module 4943 (MessageStore)
...
import ChannelMessagesDefault from "ChannelMessages" /* 5442 */;
...
import GuildAutomodMessageStoreUtils from "GuildAutomodMessageStoreUtils" /* 13971 */;
...
const IOSPushNotificationRawPayloadFixExperiment = tmp3(5445);
require = fn;
...
function receiveMediaMentionMessage(item10038) {
  ...
  if (null != message_id) {
    ...
    tmp5(5442).commit(mutation);
    const tmp5Result = tmp5(5442);
  }
}
  ...
  MESSAGE_CREATE: function handleIncomingMessage(isPushNotification) {
    ...
    if (isPushNotification.isPushNotification) {
      if (tmp3Result.isIOSPushNotificationRawPayloadFixExperimentEnabled()) {
        ...
        tmp(5442).commit(orCreate.receivePushNotification(message, isConnectedResult));
      ...
      if (ready) {
        ...
        if (tmp6) {
          ...
          tmp(5442).commit(receiveMessageResult);
      ...
      if (orCreate.has(id)) {
        ...
        if (null != message_id) {
          if ("content" in message) {
            const orCreate1 = tmp(5442).getOrCreate(message.media_mention.attachment_id);
            const tmpResult3 = tmp(5442);
        ...
        if (orCreate.revealedMessageId !== id) {
          ...
          if (null != value) {
            ...
            if (null != attachment_id) {
              value2 = tmp(5442).get(attachment_id);
              if (null != value2) {
