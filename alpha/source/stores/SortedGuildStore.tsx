...
// Dependencies: [4578, 4396, 1220, 5651, 5106, 2105, 2063, 1073, 4937, 1372, 5652, 1331, 38, 1370, 2017, 2]

// Module 5650 (SortedGuildStore)
...
function convertNodeToGuildFolder(type) {
  type = type.type;
  if (obj42(5652).GuildsNodeType.FOLDER === type) {
    ...
    return obj3;
  } else if (tmp(5652).GuildsNodeType.GUILD === type) {
...
function rebuildTree(arg0, arg1) {
  guildsTree = new obj42(5652).GuildsTree();
  if (0 === arg0.length) {
    if (arg1.length > 0) {
    ...
    while (iter2 !== undefined) {
      ...
      let tmp36 = nextResult.type === obj42(5652).GuildsNodeType.GUILD;
      if (tmp36) {
    ...
    for (const item10123 of values) {
      let tmp55 = item10123;
      let tmp58 = item10123.type === obj42(5652).GuildsNodeType.FOLDER;
      if (tmp58) {
...
function handleMoveById(sourceId) {
  ...
  if (null != node) {
    if (null != node1) {
      let tmp4 = combine;
      if (combine) {
        tmp4 = node.type === obj42(5652).GuildsNodeType.FOLDER;
      }
      ...
      tmp22Result2(!(node.type === obj42(5652).GuildsNodeType.FOLDER && null != node1.parentId), "[SORTED GUILDS] Can't move a folder " + node.id + " to inside of another folder " + node1.parentId);
      if (combine) {
        let convertToFolderResult = node1;
        if (node1.type !== tmp13(5652).GuildsNodeType.FOLDER) {
...
function handleGuildFolderDeleteLocal(targetId) {
  ...
  if (tmp) {
    const tmp4 = element.type === element(5652).GuildsNodeType.FOLDER;
    if (tmp4) {
...
function handleGuildFolderExpand(folderId) {
  ...
  if (null != node) {
    if (node.type === obj42(5652).GuildsNodeType.FOLDER) {
...
function handleCollapseAll() {
  ...
  while (iter !== undefined) {
    let tmp3 = nextResult;
    let expanded = nextResult.type === obj42(5652).GuildsNodeType.FOLDER;
    if (expanded) {
...
let guildsTree = new fn(5652).GuildsTree();
    ...
    tmp1.loadCache = function loadCache() {
      ...
      if (null != tree) {
        ...
        for (const item10011 of allNodesResult) {
          let tmp4 = item10011;
          if (item10011.type === obj42(5652).GuildsNodeType.FOLDER) {
...
export const GuildsNodeType = fn(5652).GuildsNodeType;
