...
// Dependencies: [1957, 4738, 10324, 7311, 7733, 1220, 1958, 1980, 10325, 4737, 2012, 1952, 1372, 1074, 8236, 8234, 4314, 12, 10326, 10328, 7525, 7466, 10329, 9855, 4945, 10337, 10338, 10340, 10333, 10341, 10345, 4876, 1936, 1186, 8058, 10346, 4902, 11, 504, 573, 2]

// Module 10323 (InAppNotificationStore)
...
import RestrictedScheduleNotificationUtils from "RestrictedScheduleNotificationUtils" /* 10326 */;
import MessageUtils from "MessageUtils" /* 10328 */;
import NotificationTextUtils from "NotificationTextUtils" /* 10329 */;
...
const playInAppMessageSound = tmp(10346);
require = fn;
function handleAlertMessage() {
  if (obj.allowInAppNotifications()) {
    const result = tmp(10341).shouldShowRaidInAppNotification();
    ...
    if (result.show) {
      if (null != guildId) {
        ...
        if (null == guild) {
          ...
          channel = ChannelStore.getChannel(channel(10345)(guild));
          if (null == channel) {
            return false;
            ...
            if (merged.wasRecentlyDismissed(obj2)) {
              ...
              const obj4 = {
                type: ALERT,
                key: guildId,
                duration: tmp(10338).getNotificationDuration(ALERT),
                onDismiss() {
              ...
              const tmpResult3 = tmp(10338);
              obj4.inAppNotificationId = tmp(10338).generateInAppNotificationId();
...
let closure_6 = fn(7311).trackMessageNotificationTimestamps;
...
prototype["dismissNotification"] = function dismissNotification(guild) {
  ...
  if (null != incidentAlertType) {
    ...
    const result = this.dissmissedAlertsMap[incidentAlertType].set(this.key(obj2), _modDef4314());
...
prototype["wasRecentlyDismissed"] = function wasRecentlyDismissed(guild) {
  ...
  if (null == incidentAlertType) {
    ...
    if (tmp4) {
      tmp4 = _modDef4314().diff(_modDef4314(value), self.threshold.unitOfTime) < self.threshold.amount;
      obj2 = _modDef4314();
    }
    return tmp4;
  }
...
let obj = {};
obj[fn(8236).GuildIncidentAlertTypes.JOIN_RAID] = new Map();
let map = new Map();
obj[fn(8236).GuildIncidentAlertTypes.DM_RAID] = new Map();
merged[0] = obj;
merged[1] = { amount: 1, unitOfTime: "hour" };
let c21 = null;
...
let c24 = false;
let EMPTY_SCHEDULE_SNAPSHOT = fn(10326).EMPTY_SCHEDULE_SNAPSHOT;
...
const inAppNotificationStore = new InAppNotificationStore(DispatcherDefault, {
          ...
          if (tmpResult8.allowInAppNotifications()) {
            if (!tmp3Result.isEnabled()) {
              if (!ChannelRTCStore.getChatOpen(channel_id)) {
                const result = tmp(10329).shouldIncludeSelectedChannel();
                const tmpResult9 = tmp(10329);
                if (tmpResult10.shouldNotify(message, channel_id, result)) {
                  const channel = ChannelStore.getChannel(channel_id);
                  if (null == channel) {
                    return false;
                  } else {
                    const messageRecord = tmp(4945).createMessageRecord(message);
                    const tmpResult11 = tmp(4945);
                    if (tmpResult12.isMessageContentPreviewable(messageRecord)) {
                      ...
                      const notificationDuration = tmp(10338).getNotificationDuration(MESSAGE);
                      ...
                      const tmpResult13 = tmp(10338);
                      obj3.inAppNotificationId = tmp(10338).generateInAppNotificationId();
                    ...
                    tmpResult12 = tmp(10337);
                  }
                  obj7 = ChannelStore;
                } else {
                  return false;
                }
                tmpResult10 = tmp(10329);
              }
            }
            tmp3Result = ExternalPipDefault;
          }
          tmpResult8 = tmp(10329);
        }
        return false;
      }
      tmpResult = tmp(7525);
  ...
  MESSAGE_ACK: function handleMessageAck(channelId) {
    ...
    if (tmp) {
      ...
      messageId(4902).trackWithMetadata(constants.IN_APP_NOTIFICATION_DISMISSED, obj4);
      const obj3 = messageId(4902);
    }
      ...
      if (null != message.reactions) {
        if (null != emoji) {
          if (obj14.allowInAppNotifications()) {
            let tryParseChannelPathResult = tmp19(4876).tryParseChannelPath(tmp);
                  ...
                  if (null != channel) {
                    if (!obj4.isEnabled()) {
                      if (!ChannelRTCStore.getChatOpen(channelId)) {
                        const result1 = tmp19(10329).shouldIncludeSelectedChannel();
                        const tmp19Result9 = tmp19(10329);
                        const obj = { message, channel, reactor: user, includeSelectedChannel: result1 };
                        if (tmp19Result10.shouldNotifyForReaction(obj)) {
                          const messageRecord = tmp19(4945).createMessageRecord(message);
                          const tmp19Result11 = tmp19(4945);
                          if (tmp19Result12.isMessageContentPreviewable(messageRecord)) {
                            ...
                            const obj3 = {
                              type: REACTION,
                              key: channelId,
                              duration: tmp19(10338).getNotificationDuration(REACTION),
                              onDismiss() {
                                                        InAppNotificationActionCreatorsDefault.clearNotification();
                                                      },
                              channel,
                            ...
                            const tmp19Result13 = tmp19(10338);
                            obj3.inAppNotificationId = tmp19(10338).generateInAppNotificationId();
                            obj2.notification = obj3;
                            handleEnqueueNotification(obj2);
                          } else {
                            return false;
                          }
                          tmp19Result12 = tmp19(10337);
                        } else {
                          return false;
                        }
                        tmp19Result10 = tmp19(10329);
                      }
                    }
                    return false;
                  }
      ...
      if (null != savedMessage.message) {
        ...
        if (null != channel) {
          ...
          const obj3 = {
            ...
            duration: tmp(10338).getNotificationDuration(MESSAGE_REMINDER),
            onDismiss() {
                    InAppNotificationActionCreatorsDefault.clearNotification();
                  },
            channel,
          ...
          const tmpResult = tmp(10338);
          obj3.inAppNotificationId = tmp(10338).generateInAppNotificationId();
          obj2.notification = obj3;
          handleEnqueueNotification(obj2);
          const tmpResult2 = tmp(10338);
        }
      }
    }
  },
              ...
              if (null == user) {
                ...
                if (null == guild) {
                  ...
                  const obj3 = {
                    ...
                    guild,
                    key: channel.id,
                    duration: tmp(10338).getNotificationDuration(FORUM_THREAD_CREATED),
                    onDismiss() {
                                    InAppNotificationActionCreatorsDefault.clearNotification();
                                  },
                    inAppNotificationId: null
                  };
                  const tmpResult5 = tmp(10338);
                  obj3.inAppNotificationId = tmp(10338).generateInAppNotificationId();
  ...
  CHANNEL_SELECT: function handleChannelSelect(channelId) {
    ...
    if (tmp) {
      const result = channelId(10338).extractMetadataFromNotification(_null);
      ...
      if (null != diffSchedulesResult) {
        ...
        if (EnableScreenDowntimeScheduleNotifications.getSetting()) {
          if (tmp2Result7.allowInAppNotifications()) {
            ...
            const notificationDuration = tmp2(10338).getNotificationDuration(RESTRICTED_SCHEDULE_UPDATED);
            const tmp2Result8 = tmp2(10338);
            obj3.key = tmp2(10326).restrictedScheduleNotificationKey(diffSchedulesResult.kind);
            obj3.duration = notificationDuration;
            obj3.onDismiss = function onDismiss() {
              InAppNotificationActionCreatorsDefault.clearNotification();
            };
            const tmp2Result9 = tmp2(10326);
            obj3.title = tmp2(10326).getRestrictedScheduleNotificationTitle(diffSchedulesResult.kind);
            const tmp2Result10 = tmp2(10326);
            obj3.subtitle = tmp2(10326).getRestrictedScheduleNotificationSubtitle(diffSchedulesResult.rule);
            const tmp2Result11 = tmp2(10326);
            obj3.inAppNotificationId = tmp2(10338).generateInAppNotificationId();
            obj2.notification = obj3;
            handleEnqueueNotification(obj2);
            const tmp2Result12 = tmp2(10338);
          }
          tmp2Result7 = tmp2(10329);
        }
      }
    } else {
      return false;
