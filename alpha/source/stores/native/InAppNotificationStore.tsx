...
// Dependencies: [2040, 4772, 10336, 7355, 7777, 1220, 2041, 2063, 10337, 4771, 2095, 2035, 1372, 1074, 8274, 8272, 4348, 12, 10338, 10340, 7569, 7510, 10341, 9696, 4978, 10349, 10350, 10352, 10345, 10353, 10357, 4910, 2019, 1186, 8096, 10358, 4936, 11, 504, 573, 2]

// Module 10335 (InAppNotificationStore)
...
import RestrictedScheduleNotificationUtils from "RestrictedScheduleNotificationUtils" /* 10338 */;
import MessageUtils from "MessageUtils" /* 10340 */;
import NotificationTextUtils from "NotificationTextUtils" /* 10341 */;
...
const playInAppMessageSound = tmp(10358);
require = fn;
function handleAlertMessage() {
  if (obj.allowInAppNotifications()) {
    const result = tmp(10353).shouldShowRaidInAppNotification();
    ...
    if (result.show) {
      if (null != guildId) {
        ...
        if (null == guild) {
          ...
          channel = ChannelStore.getChannel(channel(10357)(guild));
          if (null == channel) {
            return false;
            ...
            if (merged.wasRecentlyDismissed(obj2)) {
              ...
              const obj4 = {
                type: ALERT,
                key: guildId,
                duration: tmp(10350).getNotificationDuration(ALERT),
                onDismiss() {
              ...
              const tmpResult3 = tmp(10350);
              obj4.inAppNotificationId = tmp(10350).generateInAppNotificationId();
...
let closure_6 = fn(7355).trackMessageNotificationTimestamps;
...
let obj = {};
obj[fn(8274).GuildIncidentAlertTypes.JOIN_RAID] = new Map();
let map = new Map();
obj[fn(8274).GuildIncidentAlertTypes.DM_RAID] = new Map();
merged[0] = obj;
merged[1] = { amount: 1, unitOfTime: "hour" };
let c21 = null;
...
let c24 = false;
let EMPTY_SCHEDULE_SNAPSHOT = fn(10338).EMPTY_SCHEDULE_SNAPSHOT;
...
const inAppNotificationStore = new InAppNotificationStore(DispatcherDefault, {
          ...
          if (tmpResult8.allowInAppNotifications()) {
            if (!tmp3Result.isEnabled()) {
              if (!ChannelRTCStore.getChatOpen(channel_id)) {
                const result = tmp(10341).shouldIncludeSelectedChannel();
                const tmpResult9 = tmp(10341);
                if (tmpResult10.shouldNotify(message, channel_id, result)) {
                  const channel = ChannelStore.getChannel(channel_id);
                  if (null == channel) {
                    return false;
                  } else {
                    const messageRecord = tmp(4978).createMessageRecord(message);
                    const tmpResult11 = tmp(4978);
                    if (tmpResult12.isMessageContentPreviewable(messageRecord)) {
                      ...
                      const notificationDuration = tmp(10350).getNotificationDuration(MESSAGE);
                      ...
                      const tmpResult13 = tmp(10350);
                      obj3.inAppNotificationId = tmp(10350).generateInAppNotificationId();
                    ...
                    tmpResult12 = tmp(10349);
                  }
                  obj7 = ChannelStore;
                } else {
                  return false;
                }
                tmpResult10 = tmp(10341);
              }
            }
            tmp3Result = ExternalPipDefault;
          }
          tmpResult8 = tmp(10341);
        }
        return false;
      }
      tmpResult = tmp(7569);
  ...
  MESSAGE_ACK: function handleMessageAck(channelId) {
    ...
    if (tmp) {
      ...
      messageId(4936).trackWithMetadata(constants.IN_APP_NOTIFICATION_DISMISSED, obj4);
      const obj3 = messageId(4936);
    }
      ...
      if (null != message.reactions) {
        if (null != emoji) {
          if (obj14.allowInAppNotifications()) {
            let tryParseChannelPathResult = tmp19(4910).tryParseChannelPath(tmp);
                  ...
                  if (null != channel) {
                    if (!obj4.isEnabled()) {
                      if (!ChannelRTCStore.getChatOpen(channelId)) {
                        const result1 = tmp19(10341).shouldIncludeSelectedChannel();
                        const tmp19Result9 = tmp19(10341);
                        const obj = { message, channel, reactor: user, includeSelectedChannel: result1 };
                        if (tmp19Result10.shouldNotifyForReaction(obj)) {
                          const messageRecord = tmp19(4978).createMessageRecord(message);
                          const tmp19Result11 = tmp19(4978);
                          if (tmp19Result12.isMessageContentPreviewable(messageRecord)) {
                            ...
                            const obj3 = {
                              type: REACTION,
                              key: channelId,
                              duration: tmp19(10350).getNotificationDuration(REACTION),
                              onDismiss() {
                                                        InAppNotificationActionCreatorsDefault.clearNotification();
                                                      },
                              channel,
                            ...
                            const tmp19Result13 = tmp19(10350);
                            obj3.inAppNotificationId = tmp19(10350).generateInAppNotificationId();
                            obj2.notification = obj3;
                            handleEnqueueNotification(obj2);
                          } else {
                            return false;
                          }
                          tmp19Result12 = tmp19(10349);
                        } else {
                          return false;
                        }
                        tmp19Result10 = tmp19(10341);
                      }
                    }
                    return false;
                  }
      ...
      if (null != savedMessage.message) {
        ...
        if (null != channel) {
          ...
          const obj3 = {
            ...
            duration: tmp(10350).getNotificationDuration(MESSAGE_REMINDER),
            onDismiss() {
                    InAppNotificationActionCreatorsDefault.clearNotification();
                  },
            channel,
          ...
          const tmpResult = tmp(10350);
          obj3.inAppNotificationId = tmp(10350).generateInAppNotificationId();
          obj2.notification = obj3;
          handleEnqueueNotification(obj2);
          const tmpResult2 = tmp(10350);
        }
      }
    }
  },
              ...
              if (null == user) {
                ...
                if (null == guild) {
                  ...
                  const obj3 = {
                    ...
                    guild,
                    key: channel.id,
                    duration: tmp(10350).getNotificationDuration(FORUM_THREAD_CREATED),
                    onDismiss() {
                                    InAppNotificationActionCreatorsDefault.clearNotification();
                                  },
                    inAppNotificationId: null
                  };
                  const tmpResult5 = tmp(10350);
                  obj3.inAppNotificationId = tmp(10350).generateInAppNotificationId();
  ...
  CHANNEL_SELECT: function handleChannelSelect(channelId) {
    ...
    if (tmp) {
      const result = channelId(10350).extractMetadataFromNotification(_null);
      ...
      if (null != diffSchedulesResult) {
        ...
        if (EnableScreenDowntimeScheduleNotifications.getSetting()) {
          if (tmp2Result7.allowInAppNotifications()) {
            ...
            const notificationDuration = tmp2(10350).getNotificationDuration(RESTRICTED_SCHEDULE_UPDATED);
            const tmp2Result8 = tmp2(10350);
            obj3.key = tmp2(10338).restrictedScheduleNotificationKey(diffSchedulesResult.kind);
            obj3.duration = notificationDuration;
            obj3.onDismiss = function onDismiss() {
              InAppNotificationActionCreatorsDefault.clearNotification();
            };
            const tmp2Result9 = tmp2(10338);
            obj3.title = tmp2(10338).getRestrictedScheduleNotificationTitle(diffSchedulesResult.kind);
            const tmp2Result10 = tmp2(10338);
            obj3.subtitle = tmp2(10338).getRestrictedScheduleNotificationSubtitle(diffSchedulesResult.rule);
            const tmp2Result11 = tmp2(10338);
            obj3.inAppNotificationId = tmp2(10350).generateInAppNotificationId();
            obj2.notification = obj3;
            handleEnqueueNotification(obj2);
            const tmp2Result12 = tmp2(10350);
          }
          tmp2Result7 = tmp2(10341);
        }
      }
    } else {
      return false;
