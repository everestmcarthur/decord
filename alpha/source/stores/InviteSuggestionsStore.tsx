...
// Dependencies: [10089, 7893, 2041, 5638, 2105, 4395, 4405, 1074, 7976, 10077, 5730, 1370, 504, 573, 2]

// Module 10088 (InviteSuggestionsStore)
...
const inviteSuggestionsStore = new InviteSuggestionsStore(DispatcherDefault, {
  LOAD_INVITE_SUGGESTIONS: function refreshInviteSuggestions(arg0) {
    ...
    const tmp4 = (function _computeRows(query) {
      ...
      const mostRecentDMedUser = set1(10077).getMostRecentDMedUser(omitUserIds, tmp);
      ...
      const obj2 = set1(10077);
      return set1(10077).generateRowsForQuery({ query, omitUserIds, suggestedUserIds: set, maxRowsWithoutQuery: 100, omitGuildId: tmp, suggestedChannelIds: set1, inviteTargetType });
    })(query);
    rows = tmp4.rows;
    if (closure_4) {
    ...
    (function requestMembership(rows) {
      ...
      if (null != tmp) {
        ...
        while (iter !== undefined) {
          let tmp5 = nextResult;
          let tmp6 = id;
          let tmp8 = nextResult.type !== id(10077).RowTypes.DM;
          if (tmp8) {
            tmp8 = tmp5.type !== tmp6(10077).RowTypes.FRIEND;
          }
          if (!tmp8) {
