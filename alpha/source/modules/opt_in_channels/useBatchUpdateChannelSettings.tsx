...
// Dependencies: [19, 7357, 4937, 1074, 573, 504, 7353, 11807, 11804, 2]
// Exports: default

// Module 11806 (useBatchUpdateChannelSettings)
...
export default function useBatchUpdateChannelSettings(guildId) {
  ...
  return {
    onChannelClick: noop.useCallback((guildId, channelId, channel) => {
      ...
      if (isCollapsedResult) {
        guildId(11807).categoryExpand(channel);
        const obj = guildId(11807);
      }
      if (obj2.hasNotSetUpChannelOptIn(guildId)) {
        if (channelId === channel) {
          ...
          obj3.include = set;
          const result = tmp8(11804).optIntoAllChannelsForExistingMember(guildId, obj3);
          const tmp8Result = tmp8(11804);
        } else {
          ...
          obj4.exclude = set1;
          const result1 = tmp8(11804).optIntoAllChannelsForExistingMember(guildId, obj4);
          const tmp8Result3 = tmp8(11804);
        }
      } else {
        const tmp8Result4 = tmp8(7353);
