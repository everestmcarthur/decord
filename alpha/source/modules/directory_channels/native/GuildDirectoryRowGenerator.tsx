...
// Dependencies: [12548, 12547, 1115, 2]
// Exports: generateDirectoryRows

// Module 12912 (GuildDirectoryRowGenerator)
import GuildDirectoryConstants from "GuildDirectoryConstants" /* 12548 */;
...
export const generateDirectoryRows = function generateDirectoryRows(arg0, arr, currentCategoryId) {
  ...
  if (0 === arr.length) {
    ...
    const obj4 = set(12547);
    return set(12547).rankGuildEntries(arr).map((entry) => ({ type: constants.ENTRY, entry }));
  } else {
    const items = [];
    const rankByDateAddedResult = set(12547).rankByDateAdded(arr);
    const _Set = Set;
    ...
    const obj5 = set(12547);
    const result = set(12547).orderByTotalMemberCount(found);
