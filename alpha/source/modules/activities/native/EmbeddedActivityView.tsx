...
// Dependencies: [109, 32, 19, 17, 2040, 2004, 1349, 21, 4756, 9721, 1478, 573, 9723, 9724, 9726, 9719, 504, 9728, 9575, 9729, 9737, 2]

// Module 9722 (EmbeddedActivityView)
import DispatcherDefault from "Dispatcher" /* 573 */;
import EmbeddedActivitiesNativeManagerDefault from "EmbeddedActivitiesNativeManager" /* 9575 */;
import doesOrientationMatchLockStateDefault from "doesOrientationMatchLockState" /* 9723 */;
import WakeLockDefault from "WakeLock" /* 9726 */;
...
const memoResult = noop.memo(function EmbeddedActivityViewInner(portraitSafeAreasConfig) {
  ...
  const tmp4 = currentEmbeddedActivity(9719)();
  dependencyMap = tmp4;
  ...
  if (null == currentEmbeddedActivity) {
    ...
    const discordEnvQueryParams = tmp5(9724).getDiscordEnvQueryParams();
    ...
    const tmp5Result = tmp5(9724);
  }
  currentEmbeddedActivity(9728)({ connectedEmbeddedActivity: currentEmbeddedActivity });
  ...
  const obj5 = { orientationLockState: stateFromStores, showLoadingIndicator: tmp22, setShowLoadingStateForLockingOrientation: tmp9, application: tmp4, setOrientationLockState: layoutMode(9721).setOrientationLockState };
  setIsResetting = useBaseActivityView({ orientationLockState: stateFromStores, showLoadingIndicator: tmp22, setShowLoadingStateForLockingOrientation: tmp9, application: tmp4, setOrientationLockState: layoutMode(9721).setOrientationLockState }).setIsResetting;
  ...
  if (null != currentEmbeddedActivity) {
    ...
    if (null != id) {
      ...
      const tmp2Result = tmp2(9729);
      obj7.allowPopups = tmp5(9737).allowPopups(tmp4);
      obj7.isPipOrGridMode = layoutMode === constants.PIP || layoutMode === constants.GRID;
      obj7.webViewKey = tmp5(9575).EMBEDDED_ACTIVITY_WEB_VIEW_KEY;
      if (tmp26) {
