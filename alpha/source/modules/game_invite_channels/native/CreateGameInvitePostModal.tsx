...
// Dependencies: [32, 19, 17, 2041, 21, 4756, 576, 7402, 504, 5842, 13021, 7221, 13023, 4767, 4723, 11512, 1980, 1115, 5334, 5892, 4752, 3614, 7325, 7512, 5899, 5817, 7440, 5186, 2]
// Exports: default

// Module 13022 (CreateGameInvitePostModal)
...
function CreateGameInvitePostContent(parentChannel) {
  ...
  const insets = tags(7221)({ includeKeyboardHeight: true }).insets;
  ...
  const createGameInvitePost = parentChannel(13023).useCreateGameInvitePost({
    parentChannel,
    description: tmp5,
    appliedTagIds: memo,
    upload: NOOP_UPLOAD,
  ...
  obj5.children = closure_7(parentChannel(5892).XSmallIcon, {});
  const items4 = [closure_7(parentChannel(5334).PressableOpacity, obj5), ];
  ...
  obj8.value = tmp5;
  obj8.onChange = tmp6;
  obj8.maxLength = parentChannel(7512).GAME_INVITE_POST_MESSAGE_MAX_LENGTH;
  const items6 = [closure_7(parentChannel(7325).TextArea, obj8), ];
  ...
  if (tmp15Result) {
    ...
    tmp15Result = tmp15(tmp9(5817).TableRow, obj9);
  }
  ...
  items7[1] = closure_7(parentChannel(7440).TableSwitchRow, obj13);
  obj12.children = items7;
  items6[1] = closure_8(parentChannel(5899).TableRowGroup, obj12);
...
export default function CreateGameInvitePostModal(parentChannelId) {
  ...
  parentChannelId(5842).useNavigatorBackPressHandler(() => {
  ...
  if (null != stateFromStores) {
    ...
    if (stateFromStores.isGameInvitesChannel()) {
      ...
      tmp4 = closure_7(tmp2(7402).AnalyticsLocationProvider, obj2);
    }
  }
  return tmp4;
};
