...
// Dependencies: [5, 5770, 7766, 502, 2041, 1372, 6832, 1074, 4798, 2047, 12666, 3, 4454, 1115, 11517, 4614, 1110, 8663, 4767, 7582, 9886, 4613, 16749, 7874, 8447, 7422, 4769, 10001, 1241, 4936, 1364, 13178, 11, 16747, 7715, 10, 7716, 5489, 13888, 12669, 504, 13033, 10198, 4736, 1094, 11876, 8621, 8618, 573, 4959, 1101, 4958, 14113, 2]
// Exports: default

// Module 18345 (receiveNotification)
      ...
      if (null != data.deeplink) {
        ...
        if ("" !== data.deeplink) {
          ...
          if (payload.type === tmp(1094).LinkingTypes.MESSAGE) {
            ...
            if (tmp8) {
              tmp(11876).receivedNotification(payload.messageId, payload.channelId, tracking_type);
              const tmpResult = tmp(11876);
            }
            ({ guildId: obj8.guildId, channelId: obj8.channelId, messageId: obj8.messageId } = payload);
            const messages1 = tmp80(10198).fetchMessages({ guildId: null, channelId: null, messageId: null, isPreload: true });
            flag = true;
            flag2 = true;
            ...
            const tmp80Result = tmp80(10198);
          } else {
            if (payload.type === tmp(1094).LinkingTypes.ICYMI) {
              if (null != data.channel_id) {
                if (null != data.message_id) {
                  const forNotification = tmp80(8621).fetchForNotification(data.channel_id, data.message_id);
                  flag = false;
                  flag2 = false;
                  const tmp80Result3 = tmp80(8621);
                }
              }
            }
            flag = false;
            ...
            if (payload.type === tmp(1094).LinkingTypes.ICYMI) {
              ...
              if (null != data.user_id) {
                ...
                if (null != data.notification_center_id) {
                  ...
                  const obj5 = { id: data.notification_center_id, type: tmp(8618).ICYMIItemTypes.CUSTOM_STATUS, score: 1000, data: null };
                  ...
                  obj5.data = obj6;
                  const forStatusNotification = tmp80(8621).fetchForStatusNotification(obj5);
                  flag = false;
                  flag2 = false;
                  const tmp80Result4 = tmp80(8621);
                }
              }
            }
          }
...
let closure_17 = fn(12666).MultiAccountSwitchLocation;
...
export default function receiveNotification(getData, arg1) {
  if (null == getData.getData) {
    ...
    data(7715).trackAppOpened("notification");
    data = getData.getData();
    const obj3 = data(7715);
    ...
    if (null != data.receiving_user_id) {
      if (null != AuthenticationStore.getId()) {
        if (data.receiving_user_id !== obj.getId()) {
          ...
          receiving_user_id = tmp7(12669).switchAccount(receiving_user_id, false, arg1 ? tmp5.PUSH_NOTIFICATION_INITIAL : tmp5.PUSH_NOTIFICATION);
          receiving_user_id.then(() => {
            const Emitter = initializeDefault.Emitter;
            Emitter.batched(() => receiveNotification_(data));
          });
          const tmp7Result6 = tmp7(12669);
        }
      }
