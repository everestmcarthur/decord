...
// Dependencies: [32, 19, 17, 5722, 7468, 11061, 1074, 21, 672, 576, 4756, 5894, 11062, 8577, 5799, 4752, 1115, 11063, 1177, 2009, 4997, 1612, 5337, 9479, 504, 7408, 7405, 7466, 7422, 5203, 1241, 11058, 11059, 1364, 4427, 11064, 9478, 11065, 11074, 7471, 9105, 11078, 3548, 5186, 8178, 8340, 11081, 5843, 5892, 5198, 11083, 2]
// Exports: default

// Module 11060 (SocialLayerStorefrontProductDetailsModal)
...
import common_Video from "common/Video" /* 8577 */;
...
import carouselMediaItems from "carouselMediaItems" /* 11062 */;
import StorefrontNativeUtils from "StorefrontNativeUtils" /* 11063 */;
import redirectToSlayerStorefrontWebDefault from "redirectToSlayerStorefrontWeb" /* 11064 */;
...
function SKUNameAndDescriptionSection(sku) {
  ...
  if (!obj.isNullOrEmpty(sku.name)) {
    ...
    const items = [closure_1_14(tmp2(4752).Heading, obj3), ];
    ...
    if (!isNullOrEmptyResult) {
      ...
      tmp7Result = tmp7(tmp2(4752).Text, obj4);
    }
    items[1] = tmp7Result;
    obj2.children = items;
...
function ItemDetailsSection(selectedItem) {
  ...
  if (!isNullOrEmptyResult) {
    ...
    tmp14 = closure_1_14(tmp8(4752).Heading, obj4);
  }
  const items = [tmp14, , ];
  ...
  if (!isNullOrEmptyResult1) {
    ...
    items1[1] = closure_1_14(tmp8(4752).Text, obj8);
    obj5.children = items1;
  ...
  if (!isNullOrEmptyResult2) {
    ...
    tmp23 = closure_1_14(tmp8(4752).Text, obj9);
  }
  items[2] = tmp23;
  obj3.children = items;
...
function SocialLayerStorefrontProductDetailsModal(skuId) {
  ...
  const isScreenLandscape = skuId(5337).useIsScreenLandscape();
  let obj = skuId(5337);
  const mobileStoreFront = analyticsLocations(9479).useMobileStoreFront();
  let obj2 = analyticsLocations(9479);
  ...
  const getOrFetchApplication = skuId(7408).useGetOrFetchApplication(applicationId1);
  let tmp12 = getOrFetchApplication;
  const obj5 = skuId(7408);
  if (getOrFetchApplication == null) {
    tmp12 = null;
  }
  const tmp2Result = analyticsLocations(7405);
  ...
  const OTPACOMOrderExperiment = tmp4(9478).OTPACOMOrderExperiment;
  ...
  if (tmp30) {
    ...
    const items10 = [skuId];
    obj6.skuIds = items10;
    obj6.onOrderRetryCancellation = tmp4(11058).closeSocialLayerStorefrontProductDetailsModal;
    ...
    obj6.children = closure_14(tmp4(11074).HeadlessSlayerStorefrontPurchaseRunner, obj9);
    tmp43Result = tmp43(tmp2(11065), obj6, skuId);
    const tmp2Result2 = tmp2(11065);
  }
  const items11 = [skuId, memo1, trackPDPClick];
  if (null == stateFromStores) {
    return null;
  } else {
    let result = tmp4(7471).isSlayerSkuAvailableOnThisPlatform(stateFromStores);
    const intl4 = tmp4(1115).intl;
    const stringResult = intl4.string(tmp4(1115).t.boqtTA);
    const tmp4Result7 = tmp4(7471);
    let result1 = tmp4(4427).isSocialLayerStorefrontGiftingSupported();
    if (null != tmp25) {
      ...
      if (null != memo) {
        ...
        tmp49 = closure_14(tmp2(9105), obj11);
      }
    }
    let tmp53 = null;
    if (stateFromStores.exclusive) {
      const obj12 = { style: tmp.exclusiveBadgeContainer, children: closure_14(tmp4(11078).ExclusiveBadge, {}) };
    ...
    if (!result) {
      ...
      obj18.children = intl.string(tmp2(3548).gndWN7);
      tmp65Result = tmp65(tmp4(4752).Text, obj18);
    }
    ...
    obj20.children = closure_14(tmp4(5186).Button, obj21);
    const items14 = [closure_14(memo1, obj20), ];
    if (result1) {
      const obj22 = { icon: tmp2(8340), variant: "primary", size: "lg", disabled: tmp28, accessibilityLabel: null, onPress: null };
      const intl2 = tmp4(1115).intl;
      obj22.accessibilityLabel = intl2.string(tmp4(1115).t.QAZA5f);
      obj22.onPress = tmp48;
      result1 = tmp65(tmp4(8178).IconButton, obj22);
    }
    items14[1] = result1;
    ...
    if (result) {
      ...
      const mobileFinePrintMessageForApplication = tmp4(11081).getMobileFinePrintMessageForApplication(getOrFetchApplication, stringResult, obj24);
    ...
    const obj28 = {
      onPress: tmp4(11058).closeSocialLayerStorefrontProductDetailsModal,
      backImage() {
    ...
    const items17 = [closure_14(tmp4(5843).HeaderBackButton, obj28), ];
    ...
    obj29.children = name;
    items17[1] = closure_14(tmp4(4752).Heading, obj29);
    ...
    if (exclusive) {
      ...
      exclusive = tmp65(tmp2(5198), obj31);
    }
...
const SlayerShopPDPCTAType = fn(11061).SlayerShopPDPCTAType;
...
obj.header = { height: fn(5894).NAV_BAR_HEIGHT, flexDirection: "row", alignItems: "center", paddingHorizontal: nativeDefault.space.PX_8, gap: nativeDefault.space.PX_8, borderBottomWidth: 1, borderBottomColor: nativeDefault.colors.BORDER_SUBTLE };
obj.headerTitle = { flexShrink: 1 };
let obj2 = { height: fn(5894).NAV_BAR_HEIGHT, flexDirection: "row", alignItems: "center", paddingHorizontal: nativeDefault.space.PX_8, gap: nativeDefault.space.PX_8, borderBottomWidth: 1, borderBottomColor: nativeDefault.colors.BORDER_SUBTLE };
...
obj.hero = { marginHorizontal: nativeDefault.space.PX_16, height: fn(11062).MOBILE_HERO_HEIGHT_PX, borderRadius: nativeDefault.radii.md, overflow: "hidden", backgroundColor: nativeDefault.colors.BACKGROUND_SURFACE_HIGH };
obj.heroLandscape = { flex: 1, minHeight: 140, height: "add" };
let obj20 = { marginHorizontal: nativeDefault.space.PX_16, height: fn(11062).MOBILE_HERO_HEIGHT_PX, borderRadius: nativeDefault.radii.md, overflow: "hidden", backgroundColor: nativeDefault.colors.BACKGROUND_SURFACE_HIGH };
