...
// Dependencies: [19, 17, 1372, 1074, 1955, 21, 4442, 4573, 4656, 7175, 504, 4380, 1115, 2870, 7578, 7234, 4542, 1944, 7346, 7319, 7350, 5862, 1364, 5757, 17316, 9049, 4718, 5144, 4722, 576, 2]
// Exports: default

// Module 17315 (DisplayNameStylesFlywheelMobileActionSheet)
...
export default function DisplayNameStylesFlywheelMobileActionSheet(markAsDismissed) {
  ...
  const enabled = noop.useContext(markAsDismissed(4442).AccessibilityPreferencesContext).reducedMotion.enabled;
  const isThemeDarkResult = markAsDismissed(4573).isThemeDark(ref(4656)());
  const tmp6 = closure_11();
  const obj2 = markAsDismissed(4573);
  const typeConsolidationTextTransform = markAsDismissed(7175).useTypeConsolidationTextTransform("DisplayNameStylesFlywheel");
  const obj3 = markAsDismissed(7175);
  ...
  let result = ref(4380).canUsePremiumProfileCustomization(stateFromStores);
  dependencyMap = result;
  const intl = markAsDismissed(1115).intl;
  const string = intl.string;
  const tmp10 = ref(2870);
  if (result) {
  ...
  const callback = obj.useCallback(() => {
    openUserSettings.openUserSettings({ screen: c2 ? timestampProducer.PROFILE_CUSTOMIZATION : timestampProducer.PROFILE_CUSTOMIZATION_TRY_IT_OUT }, () => {
      markAsDismissed(7234).runAfterInteractions(() => {
        markAsDismissed(7578).openUserSettings({ screen: constants.DISPLAY_NAME_STYLES }, () => {
          closure_1_0(constants.TAKE_ACTION);
          const result = closure_2_0(4542).UNSAFE_markDismissibleContentAsDismissed(closure_2_0(1944).DismissibleContent.DISPLAY_NAME_STYLES_FLYWHEEL_MOBILE_PROFILE_COACHMARK, { dismissAction: constants.INDIRECT_ACTION });
        });
      });
    });
  }, items1);
  ...
  const items4 = [
    closure_9(markAsDismissed(7350).ActionSheetHeaderBar, {
      onPress() {
  ...
  if (enabled) {
    tmp15Result = tmp15(tmp2(5862).DisplayNameStylesV2AbstractUI, { resizeMode: "contain" });
  }
  ...
  if (enabled) {
    ...
    obj10.children = intl2.string(tmp4(2870).Uzms61);
    items4[2] = tmp15(tmp2(4718).Text, obj10);
    ...
    items4[3] = tmp15(tmp2(4718).Text, obj11);
    ...
    const items7 = [tmp15(tmp2(5144).Button, obj14), ];
    ...
    items7[1] = tmp15(tmp2(5144).Button, obj15);
    obj13.children = items7;
    items4[4] = tmp16(tmp17, obj13);
    obj7.children = items4;
    obj12.children = tmp16(tmp17, obj7);
    obj6.children = tmp15(tmp2(7319).SafeAreaPaddingView, obj12);
    return tmp15(tmp2(7346).BottomSheet, obj6);
  } else {
    if (tmp2Result.isIOS()) {
      const obj16 = { source: null, style: null, resizeMode: "contain", enableAnimation: null };
      const obj17 = { uri: tmp4(17316) };
      obj16.source = obj17;
      obj16.style = tmp6.image;
      obj16.enableAnimation = !enabled;
      let tmp15Result2 = tmp15(tmp4(5757), obj16);
      const tmp4Result = tmp4(5757);
    } else {
      const obj18 = { url: tmp4(17316), style: tmp6.image, autoplay: true };
      tmp15Result2 = tmp15(tmp2(9049).APNGPlayer, obj18);
    }
