...
// Dependencies: [19, 21, 4348, 9752, 9794, 1115, 2]
// Exports: default

// Module 9793 (GuildEventSchedule)
...
export default function GuildEventSchedule(schedule) {
  ...
  const addResult = onChange(4348)().add(schedule(9752).MAX_DAYS_AHEAD_AN_EVENT_CAN_START, "days");
  ...
  const addResult1 = onChange(4348)().add(schedule(9752).MAX_DAYS_AHEAD_AN_EVENT_CAN_END, "days");
  if (null != recurrenceId) {
    addResult.add(tmp3(9752).MAX_YEARS_AHEAD_RECURRING_EVENT, "years");
    addResult1.add(tmp3(9752).MAX_YEARS_AHEAD_RECURRING_EVENT, "years");
  }
  const obj2 = {
  ...
  const children = [closure_4(schedule(9794).GuildEventDatetime, obj2), ];
  ...
  if (tmp9Result) {
    ...
    tmp9Result = closure_4(tmp3(9794).GuildEventDatetime, obj4);
  }
