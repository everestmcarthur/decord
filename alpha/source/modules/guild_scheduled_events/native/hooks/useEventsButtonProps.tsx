...
// Dependencies: [19, 4771, 4937, 4938, 504, 9749, 5268, 5781, 9782, 4723, 12627, 1980, 1115, 12631, 2]
// Exports: default
...
export default function useEventsButtonProps(id) {
  ...
  const handlePress = noop.useCallback(() => {
    if (obj.shouldShowMembershipVerificationGate(user.id)) {
      let result = tmp(5781).openMemberVerificationModal(tmp3.id);
      const tmpResult = tmp(5781);
    } else {
      result = tmp(9782).openGuildEventListActionSheet(tmp3);
      const tmpResult2 = tmp(9782);
    }
    return result;
  }, items3);
