...
// Dependencies: [7766, 9752, 9756, 504, 2]
// Exports: default, getEventSchedule, useEventScheduleById

// Module 9755 (useEventSchedule)
  ...
  if (tmp5 == stateFromStores) {
    ...
    if (tmp5 != stateFromStores.recurrence_rule) {
      if (tmp5 != nextRecurrenceIdInEvent) {
        const baseScheduleForRecurrence = tmp2(9752).getBaseScheduleForRecurrence(nextRecurrenceIdInEvent, stateFromStores);
        const tmp2Result3 = tmp2(9752);
        const scheduleForRecurrenceWithException = tmp2(9752).getScheduleForRecurrenceWithException(baseScheduleForRecurrence, tmp7);
        ({ startDate, endDate } = scheduleForRecurrenceWithException);
...
export const getEventSchedule = function getEventSchedule(guildEvent, recurrenceId) {
  ...
  if (null != guildEvent.recurrence_rule) {
    if (null != recurrenceId) {
      const baseScheduleForRecurrence = tmp(9752).getBaseScheduleForRecurrence(recurrenceId, guildEvent);
      const tmpResult = tmp(9752);
      const scheduleForRecurrenceWithException = tmp(9752).getScheduleForRecurrenceWithException(baseScheduleForRecurrence, tmp4);
      ({ startDate, endDate } = scheduleForRecurrenceWithException);
