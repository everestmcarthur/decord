...
// Dependencies: [32, 19, 8422, 17311, 10908, 1220, 1952, 1074, 1374, 1084, 504, 17329, 1609, 8280, 7643, 17330, 13626, 10985, 10984, 10990, 8427, 8414, 4542, 1944, 17331, 17334, 17309, 17319, 9415, 12209, 2]
// Exports: useMainViewTooltipActionSheetMap

// Module 17328 (useMainViewTooltipActionSheetEligibilityMap)
import BadgeId from "BadgeId" /* 8414 */;
import BadgeDirectoryActionCreators from "BadgeDirectoryActionCreators" /* 8427 */;
import MainViewTooltipActionSheetsDisabledExperimentDefault from "MainViewTooltipActionSheetsDisabledExperiment" /* 17329 */;
import _slicedToArray from "module_32" /* 32 */;
import noop from "module_19" /* 19 */;
import BadgeDirectoryStore from "BadgeDirectoryStore" /* 8422 */;
import GooglePlayPriceChangeStore from "GooglePlayPriceChangeStore" /* 17311 */;
...
const useGiftingPromotionAssetsReadyDefault = tmp4(17331);
require = fn;
...
export const useMainViewTooltipActionSheetMap = function useMainViewTooltipActionSheetMap() {
  ...
  const premiumDiscountOffer = isEligibleToShowGiftingBadgeCoachmark(8280).usePremiumDiscountOffer();
  const tmpResult20 = isEligibleToShowGiftingBadgeCoachmark(8280);
  const premiumTrialOffer = isEligibleToShowGiftingBadgeCoachmark(7643).usePremiumTrialOffer();
  const PremiumTrialOfferActionSheetKillSwitchExperiment = tmp(17330).PremiumTrialOfferActionSheetKillSwitchExperiment;
  const tmpResult21 = isEligibleToShowGiftingBadgeCoachmark(7643);
  const promotionMarketingComponent = isEligibleToShowGiftingBadgeCoachmark(13626).usePromotionMarketingComponent(tmp(10985).MarketingComponentType.MOBILE_BOTTOM_SHEET);
  let oneofKind;
  ...
  const tmpResult22 = isEligibleToShowGiftingBadgeCoachmark(13626);
  ...
  const stateFromStores3 = isEligibleToShowGiftingBadgeCoachmark(504).useStateFromStores(items4, () => {
    const marketingComponentByType = PromotionsStore.getMarketingComponentByType(isEligibleToShowGiftingBadgeCoachmark(10985).MarketingComponentType.GIFT_ICON_COACHMARK);
  ...
  const stateFromStores4 = isEligibleToShowGiftingBadgeCoachmark(504).useStateFromStores(items5, () => {
    const marketingComponentByType = PromotionsStore.getMarketingComponentByType(isEligibleToShowGiftingBadgeCoachmark(10985).MarketingComponentType.GIFT_REMINDER_COACHMARK);
    let prop = null;
    if (null != marketingComponentByType) {
      prop = null;
  ...
  const GiftPromotionReminderExperiment = tmp(10984).GiftPromotionReminderExperiment;
  const tmpResult25 = isEligibleToShowGiftingBadgeCoachmark(504);
  isEligibleToShowGiftingBadgeCoachmark = isEligibleToShowGiftingBadgeCoachmark(10990).useIsEligibleToShowGiftingBadgeCoachmark({ platform: "native", location: tmp5 });
  ...
  const tmpResult26 = isEligibleToShowGiftingBadgeCoachmark(10990);
  const items7 = [BadgeDirectoryStore];
  let isDismissed = null != stateFromStores2;
  const stateFromStores5 = isEligibleToShowGiftingBadgeCoachmark(504).useStateFromStores(items7, () => null != badgeById.getBadgeById(isEligibleToShowGiftingBadgeCoachmark(8414).BadgeId.GIFTING));
  if (isDismissed) {
    isDismissed = tmp(4542).UNSAFE_isSnowflakeBoundDismissibleContentDismissed(tmp(1944).DismissibleContent.GIFTING_PROMOTION_MOBILE_FIRST_TIME_HALFSHEET, stateFromStores2).isDismissed;
    const tmpResult28 = tmp(4542);
  }
  let isDismissed2 = null != stateFromStores2;
  if (isDismissed2) {
    isDismissed2 = tmp(4542).UNSAFE_isSnowflakeBoundDismissibleContentDismissed(tmp(1944).DismissibleContent.GIFTING_PROMOTION_REMINDER, stateFromStores2).isDismissed;
    const tmpResult29 = tmp(4542);
  }
  let tmp22 = null;
  ...
  const nitroFileUploadAnnouncementEligible = isEligibleToShowGiftingBadgeCoachmark(17334).useNitroFileUploadAnnouncementEligible(tmp5);
  const tmpResult30 = isEligibleToShowGiftingBadgeCoachmark(17334);
  const nitroFileUploadUpsellEligible = isEligibleToShowGiftingBadgeCoachmark(17334).useNitroFileUploadUpsellEligible(tmp5);
  const tmpResult31 = isEligibleToShowGiftingBadgeCoachmark(17334);
  const items8 = [, ];
  ({ LEAGUE_OF_LEGENDS: arr9[0], RIOT_GAMES: arr9[1] } = PlatformTypes);
  const shouldShowRobloxConnectionCoachmark = isEligibleToShowGiftingBadgeCoachmark(17309).useShouldShowRobloxConnectionCoachmark();
  const tmp28 = PlatformTypes;
  const tmpResult32 = isEligibleToShowGiftingBadgeCoachmark(17309);
  const shouldShowConnectionDeprecationBottomSheet = isEligibleToShowGiftingBadgeCoachmark(17319).useShouldShowConnectionDeprecationBottomSheet({ deprecatedPlatformTypes: items8 });
  const tmpResult33 = isEligibleToShowGiftingBadgeCoachmark(17319);
  const obj5 = { deprecatedPlatformTypes: null };
  const items9 = [PlatformTypes.BATTLENET];
  obj5.deprecatedPlatformTypes = items9;
  const shouldShowConnectionDeprecationBottomSheet1 = isEligibleToShowGiftingBadgeCoachmark(17319).useShouldShowConnectionDeprecationBottomSheet(obj5);
  const tmpResult34 = isEligibleToShowGiftingBadgeCoachmark(17319);
  const isDisplayNameStylesFlywheelSettersEnabled = isEligibleToShowGiftingBadgeCoachmark(9415).useIsDisplayNameStylesFlywheelSettersEnabled(tmp5);
  isEligibleToShowGiftingBadgeCoachmark(12209);
  const obj6 = {};
  if (stateFromStores) {
    ...
    obj6[tmp(1944).DismissibleContent.DISPLAY_NAME_STYLES_FLYWHEEL_MOBILE_COACHMARK] = obj25;
    ...
    obj6[tmp(1944).DismissibleContent.RIOT_CONNECTION_DEPRECATION_DISABLE] = obj26;
    ...
    obj6[tmp(1944).DismissibleContent.BATTLENET_CONNECTION_DEPRECATION_DISABLE] = obj28;
    const obj30 = { isEligible: true, actionSheetProperties: {} };
    obj6[tmp(1944).DismissibleContent.COLLECTIBLES_PROFILE_FRAMES_ANNOUNCEMENT] = obj30;
    const obj31 = { isEligible: nitroFileUploadAnnouncementEligible, actionSheetProperties: {} };
    obj6[tmp(1944).DismissibleContent.NITRO_FILE_UPLOAD_1GB_ANNOUNCEMENT] = obj31;
    const obj32 = { isEligible: nitroFileUploadUpsellEligible, actionSheetProperties: {} };
    obj6[tmp(1944).DismissibleContent.NITRO_FILE_UPLOAD_1GB_UPSELL] = obj32;
    const obj33 = { isEligible: tmp33, actionSheetProperties: {} };
    obj6[tmp(1944).DismissibleContent.CUSTOM_TYPING_INDICATOR_MOBILE_COACHMARK] = obj33;
    let tmp35 = obj6;
  } else {
    tmp35 = obj6;
  }
