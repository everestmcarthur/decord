...
// Function ID: 16200
// Name: ChooseAccount
// Dependencies: [5, 19, 17, 12656, 12657, 1074, 21, 4722, 576, 1484, 16200, 1241, 12660, 5067, 1115, 1177, 4689, 7390, 7166, 4718, 14073, 16201, 5292, 10045, 8831, 16202, 2]
// Exports: default

// Module 16199 (ChooseAccount)
...
export default function ChooseAccount() {
      ...
      try {
        ...
        if (0 === dependencyMap) {
          if (arg0 === 1) {
            ...
            obj5 = tmp2(5067).confirm(obj5);
            dependencyMap = 1;
            c4 = 1;
            const obj8 = tmp2(5067);
          }
        } else if (arg0 === 1) {
          c4 = 3;
          throw value;
          ...
          if (value) {
            tmp2(4689).hideActionSheet();
            const obj = tmp2(4689);
            tmp5(12660).removeAccount(closure_129_0.id);
            const obj2 = tmp5(12660);
          }
          c4 = 3;
          return { value: "HermesInternal", done: null };
        }
  ...
  obj3.backgroundImageSource = multiAccountUsers(14073);
  ...
  let items = [
    multiAccountUsers.map((user) => {
      let obj = {
        ...
        onPressUser() {
          if (user.tokenStatus === constants.INVALID) {
            ...
            closure_1_2(12660).switchAccount(tmp.id, undefined, constants2.CHOOSE_ACCOUNT);
            const obj = closure_1_2(12660);
          }
          return tmp4;
        },
        trailing: null
      ...
      let obj2 = {
        ...
        onPress() {
          multiAccountUsers(4689).hideActionSheet();
          let obj = multiAccountUsers(4689);
          ...
          items[1] = obj5;
          obj3.options = items;
          const result = user(7390).showSimpleActionSheet(obj3);
        },
        children: null
      };
      const tmp = multiAccountUsers(16201);
      obj2.children = closure_1_11(user(1177).Icon, { size: user(1177).Icon.Sizes.SMALL_20, source: multiAccountUsers(10045), disableColor: true });
      obj.trailing = closure_1_11(user(5292).PressableOpacity, obj2);
      return closure_1_11(tmp, obj, user.id);
    }),

  ];
  let obj6 = { leading: null, label: null, labelStyle: null, onPress: null };
  const tmp2 = multiAccountUsers(7166);
  obj6.leading = closure_11(require("Form").FormRow.Icon, { themedColor: multiAccountUsers(576).colors.TEXT_LINK, size: require("native").Icon.Sizes.SMALL_20, source: multiAccountUsers(16202) });
