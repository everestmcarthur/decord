...
// Dependencies: [19, 17, 21, 8906, 8917, 5775, 9434, 4722, 9438, 8400, 1115, 4689, 9423, 1897, 7346, 6820, 5857, 2]
// Exports: default

// Module 9439 (GuildProfileGamesActionSheet)
...
import components_GameIconDefault from "components/GameIcon" /* 9434 */;
...
export default function GuildProfileGamesActionSheet(profile) {
  ...
  const tmp = closure_7();
  const allGuildProfileGames = id(9438).useAllGuildProfileGames(profile);
  let obj = id(9438);
  ...
  const callback = noop.useCallback(() => {
    ...
    obj.openLazy(asyncRequireImpl(9423, dependencyMap.paths), "GuildProfileActionSheet:" + id, { guildId: id });
  }, items);
  ...
  obj5.children = jsx(id(5857).TableRowGroup, { title: intl.format(id(1115).t.vuAVo7, { serverName: profile.name }).toString(), hasIcons: true, children: allGuildProfileGames.map((game) => <GuildProfileGameRow key={arg0.id} game={arg0} activityLevel={gameActivity[arg0.id]} />) });
  obj4.children = <View style={tmp.container}>{null}</View>;
  obj3.children = jsx(id(6820).BottomSheetScrollView, { children: null });
  return jsx(id(7346).BottomSheet, { ref: obj2.useBottomSheetRef().bottomSheetRef, scrollable: true, onDismiss: callback, startHeight: 300, children: null });
};
