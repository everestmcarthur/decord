...
// Dependencies: [32, 19, 17, 21, 4722, 576, 1612, 1875, 4689, 9962, 1897, 1115, 5801, 8067, 1364, 5794, 5292, 4718, 2]

// Module 11955 (MessageRemindersCustomDurationModal)
...
export default noop.memo((onClose) => {
  ...
  function handleOpenDatePicker(date) {
    ...
    obj2.openLazy(asyncRequireImpl(9962, dependencyMap.paths), "DatePicker", obj3);
  }
  const tmp = onPress();
  ...
  obj5 = onClose(1364);
  obj3.headerLeft = onClose(5794).getHeaderCloseButton(onClose);
  ...
  const items1 = [error(onClose(5801).Header, obj3), ];
  ...
  const items2 = [error(onClose(4718).Text, obj8), ];
  ...
  obj9.children = error(onClose(4718).Text, { variant: "text-md/medium", children: formatResult });
  items2[1] = error(onClose(5292).PressableOpacity, obj9);
  ...
  const items4 = [error(onClose(4718).Text, obj10), , ];
  ...
  obj11.children = error(onClose(4718).Text, { variant: "text-md/medium", children: formatResult1 });
  items4[1] = error(onClose(5292).PressableOpacity, obj11);
  ...
  if (tmp10Result) {
    ...
    tmp10Result = tmp10(tmp11(4718).Text, obj12);
  }
