...
// Function ID: 13530
// Name: ForLaterScreen
// Dependencies: [32, 19, 17, 11900, 21, 4722, 576, 4458, 5143, 13530, 8064, 8058, 504, 7358, 7378, 9008, 1249, 1091, 13532, 13538, 8957, 13542, 2]

// Module 13529 (ForLaterScreen)
...
import useSavedMessagesForPageDefault from "useSavedMessagesForPage" /* 13530 */;
import ForLaterMessageCardDefault from "ForLaterMessageCard" /* 13532 */;
...
function ForLaterPage(type) {
  ...
  const tmp5 = type === throttledNow(8064).SavedMessageSortTypes.REMINDER;
  const forLaterLimit = throttledNow(8058).useForLaterLimit(ForLaterScreen, tmp5);
  const obj = throttledNow(8058);
  const isForLaterLimitUpgradable = throttledNow(8058).useIsForLaterLimitUpgradable(ForLaterScreen);
  ...
  const analyticsLocations = useAnalyticsLocationsDefault(tmp2(7378).FOR_LATER_POPOUT).analyticsLocations;
  ...
  if (0 === arr.length) {
    ...
    const obj6 = { type };
    obj5.children = closure_7(tmp2(13538), obj6);
    let tmp24Result = closure_7(tmp4(7358).AnalyticsLocationProvider, obj5);
  } else {
    ...
    obj8.children = closure_7(tmp4(8957).FlashList, obj9);
    const items2 = [closure_7(View, obj8), ];
    let tmp25Result = null;
    if (tmp9) {
      const obj10 = { isReminder: tmp5, isAtLimit: tmp8 };
      tmp25Result = tmp25(tmp2(13542), obj10);
    }
    items2[1] = tmp25Result;
    obj7.children = items2;
    tmp24Result = closure_8(tmp4(7358).AnalyticsLocationProvider, obj7);
    tmp25 = closure_7;
  }
  return tmp24Result;
}
