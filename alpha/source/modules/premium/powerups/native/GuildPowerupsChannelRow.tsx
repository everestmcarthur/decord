...
// Dependencies: [19, 17, 10373, 21, 12747, 6847, 576, 1177, 4756, 16505, 16344, 7397, 12743, 12762, 12631, 12731, 7422, 7396, 1115, 2514, 16551, 12543, 2]
// Exports: default

// Module 16550 (GuildPowerupsChannelRow)
...
import SidebarCoachmarkOverlay from "SidebarCoachmarkOverlay" /* 16344 */;
import useGuildPowerupsCoachmarkDefault from "useGuildPowerupsCoachmark" /* 16505 */;
...
function GuildPowerupsChannelRowIndicator(indicator) {
  ...
  if (null == indicator) {
    ...
    if (GuildPowerupsNotification.GuildPowerupNotificationIndicatorType.WARNING === type) {
      ...
      return hasOwnProperty(tmp4(6847).CircleErrorIcon, obj2);
    } else if (tmp4(12747).GuildPowerupNotificationIndicatorType.UNREAD === type) {
...
let obj2 = { container: { marginVertical: fn(10373).CHANNEL_MARGIN_VERTICAL, marginHorizontal: 8, borderRadius: nativeDefault.radii.md } };
...
export default function GuildPowerupsChannelRow(guildId) {
  ...
  const tmp5 = dismissNewBadgeIfShown(12743)(guildId);
  let indicator;
  let obj = noop;
  const tmp = closure_8();
  if (tmp5 != null) {
  ...
  const tmp6Result = dismissNewBadgeIfShown(12762)(guildId, tmp8);
  dismissNewBadgeIfShown = tmp6Result.dismissNewBadgeIfShown;
  let showUnread;
  if (tmp5 != null) {
    showUnread = tmp5.showUnread;
  }
  const ChannelModes = guildId(12631).ChannelModes;
  if (true === showUnread) {
  ...
  const tmp18 = View;
  const tmp6 = dismissNewBadgeIfShown(12762);
  ...
  obj5.mode = DEFAULT;
  obj4.name = closure_5(tmp14(12631).BaseChannelName, obj5);
  const tmp3Result = dismissNewBadgeIfShown(12631);
  obj4.icon = closure_5(tmp14(12631).BaseChannelIcon, { mode: DEFAULT, IconComponent: tmp14(16551).BoostTier2Icon });
  if (tmp6Result.showNewBadgeOnRow) {
    let tmp16Result = tmp16(tmp14(12543).NewBadge, {});
  } else {
    let indicator1;
    if (tmp5 != null) {
  ...
  return closure_5(tmp14(7396).LayerScope, obj8);
};
