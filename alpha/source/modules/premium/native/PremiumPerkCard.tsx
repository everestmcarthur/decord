...
// Dependencies: [19, 17, 1374, 1074, 21, 5193, 13661, 4414, 7622, 7422, 1115, 13662, 13663, 13664, 13665, 13666, 13667, 13668, 13669, 13670, 13671, 13672, 13673, 13674, 13675, 13676, 13677, 13678, 4752, 2108, 4756, 576, 5799, 13679, 5186, 2]
// Exports: default, usePerkCardHeight, usePremiumPerkCard

// Module 13660 (PremiumPerkCard)
...
import PillTextDefault from "PillText" /* 13679 */;
...
export default function PremiumPerkCard(variant) {
  ...
  if (null != imageSrc) {
    if (null != imageOverlayText) {
      ...
      imageSrc = React7(tmp6(4752).Text, obj5);
    ...
    if (null != description) {
      ...
      let tmp21 = React7(tmp6(4752).Text, obj7);
    } else {
      tmp21 = null;
    ...
    items4[2] = React7(tmp6(4752).Text, obj11);
    ...
    if (tmp29Result) {
      ...
      obj13.children = tmp29(tmp6(5186).Button, obj14);
...
export const usePremiumPerkCard = function usePremiumPerkCard() {
  subscriptionPlansLoaded = subscriptionPlansLoaded(13661).useSubscriptionPlansLoaded();
  let obj = subscriptionPlansLoaded(13661);
  const maxFileSizeForPremiumType = subscriptionPlansLoaded(4414).getMaxFileSizeForPremiumType(PremiumTypes.TIER_2);
  const callback = noop.useCallback(() => {
    const obj2 = { screen: constants.COLLECTIBLES_SHOP, params: null };
    const obj = subscriptionPlansLoaded(7622);
    obj2.params = { analyticsSource: AnalyticsLocationDefault.PREMIUM_MARKETING_PERK_CARD };
    obj.openUserSettings(obj2);
  }, []);
  const callback1 = noop.useCallback(() => {
    subscriptionPlansLoaded(7622).openUserSettings({ screen: constants.PROFILE_CUSTOMIZATION });
  }, []);
  const items = [subscriptionPlansLoaded];
  const callback2 = noop.useCallback(() => {
    subscriptionPlansLoaded(7622).openUserSettings({ screen: constants.APPEARANCE_THEME_PICKER });
  }, []);
  ...
  const callback4 = noop.useCallback(() => {
    subscriptionPlansLoaded(7622).openUserSettings({ screen: constants.APP_ICONS });
  }, []);
  ...
  let obj2 = subscriptionPlansLoaded(4414);
  obj20.imageSrc = { uri: _modDef13678 };
  ...
  obj20.bodyComponent = closure_9(subscriptionPlansLoaded(4752).Text, obj22);
