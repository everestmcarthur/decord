...
// Dependencies: [5, 5489, 502, 5622, 4779, 1074, 1091, 3, 7702, 7703, 7704, 1231, 7705, 7708, 1241, 7709, 510, 7711, 1339, 573, 504, 1255, 2]
...
function validateClientSession(version) {
  ...
  if (null != version) {
    ...
    if (version.version !== SessionUtils.CLIENT_SESSION_STORAGE_VERSION) {
      const _HermesInternal = HermesInternal;
      logger.warn("Throwing away client session with invalid version: " + version.version + ", expected " + tmp2(7711).CLIENT_SESSION_STORAGE_VERSION);
      tmp4 = null;
    }
    tmp = tmp4;
    tmp2 = require;
...
export const getActiveSessionUnsafe = function getActiveSessionUnsafe() {
  if ("uninitialized" === closure_20.state) {
    ...
    if (null != value) {
      let tmp8 = value;
      if (value.version !== tmp2(7711).CLIENT_SESSION_STORAGE_VERSION) {
        const _HermesInternal = HermesInternal;
        logger.warn("Throwing away client session with invalid version: " + value.version + ", expected " + tmp2(7711).CLIENT_SESSION_STORAGE_VERSION);
        tmp8 = null;
      }
      tmp7 = tmp8;
    }
