...
// Dependencies: [2014, 1958, 1980, 4361, 4371, 1372, 1074, 1926, 1397, 1115, 5177, 4867, 4875, 5178, 4876, 5175, 2075, 2]
...
function getChannel(id, arr) {
  ...
  if (null != channel) {
    ...
    obj5.name = tmp(4875).computeChannelName(channel, UserStore, RelationshipStore);
    obj5.isDm = channel.isPrivate();
    obj5.isForumPost = channel.isForumPost();
    const tmpResult = tmp(4875);
    obj5.isMentionable = tmp(5178).isChannelTypeMentionable(channel.type);
    const tmpResult3 = tmp(5178);
    obj5.canViewChannel = tmp(4876).canViewChannel(channel);
    obj5.roleSubscriptionGated = isSubscriptionGated;
    obj5.iconType = str;
    obj5.parentId = channel.parent_id;
    tmp4 = obj5;
    const tmpResult4 = tmp(4876);
  }
  return tmp4;
}
        ...
        if (channel.guildId === guildIdFromChannelId) {
          if (tmp34) {
            if (channel.isForumPost) {
              ...
              if (null != channel) {
                const channelName = tmp35(4875).computeChannelName(channel, UserStore, RelationshipStore);
                const tmp35Result = tmp35(4875);
                let str3 = tmp35(4867).getMentionIconType(channel);
                if (str3 == null) {
                  str3 = "forum";
                }
                const obj18 = { inContent: null, content: null };
                const tmp35Result3 = tmp35(4867);
                const obj19 = { type: "text", content: tmp35(1926).truncateText(channelName, 32) };
