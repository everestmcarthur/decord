...
// Dependencies: [19, 17, 4748, 2041, 2099, 2063, 1074, 2048, 1085, 21, 4751, 4752, 4756, 576, 4767, 4910, 4614, 4959, 8447, 5108, 1115, 7429, 4453, 5796, 1177, 1366, 8640, 4451, 11863, 11833, 8353, 504, 4605, 8218, 4723, 11836, 1980, 11, 2019, 5799, 11814, 14103, 11476, 5207, 10382, 14104, 14106, 4698, 5209, 1364, 5239, 14108, 10384, 2]
// Exports: default, plainMentionRenderer, plainSpoilerRenderer

// Module 4747 (MarkupReactRules)
...
import MarkupReactCommandRuleDefault from "MarkupReactCommandRule" /* 11476 */;
import showLongPressURLActionSheetDefault from "showLongPressURLActionSheet" /* 11833 */;
...
function MarkupLink(arg0) {
  ...
  obj.onPress = function onPress(stopPropagation) {
    ...
    if (null != tmp) {
      ...
      const obj = {
        ...
        trusted() {
            return node(11863).isLinkTrusted(formatResult);
          }
      };
      node(8640).handleClick(obj);
      const obj2 = node(8640);
    }
  };
  ...
  obj2.inLink = true;
  obj.children = node(8353).smartOutput(node, output, obj2);
  return tmp2(node(4752).Text, obj, state.key);
}
    ...
    fn = () => {
      if (null != node.roleId) {
        if (null != tmp.guildId) {
          ...
          obj5.openLazy(asyncRequireImpl(11836, dependencyMap.paths), "RoleMembersActionSheet", obj3, "stack");
        }
      }
      if ("@everyone" === node.roleName) {
        if (null != tmp.guildId) {
          const obj2 = ActionSheetActionCreatorsDefault;
          const tmp12 = asyncRequireImpl(11836, dependencyMap.paths);
...
function MarkupInlineCode(arg0) {
  ...
  obj2.children = node(8353).smartOutput(node, output, state);
...
function MarkupChannelMention(state) {
  ...
  if (null == node.inContent) {
    items[1] = null;
    items[2] = tmp3(8353).smartOutput(node, output, state);
    obj.children = items;
    const tmp2Result = closure_16(state(4752).Text, obj, state.key);
...
function MarkupAttachmentLink(state) {
  ...
  let obj = { variant: str2, style: tmp.channelMentionText, children: null };
  const obj2 = { themedColor: node(576).colors.MENTION_FOREGROUND, source: node(14103), size: null };
  const fontScale = closure_4.getFontScale();
  if (fontScale < 1) {
  ...
  obj2.size = SMALL;
  const items = [closure_15(state(1177).ThemedIcon, obj2), state(8353).smartOutput(node, output, state)];
  obj.children = items;
  const tmp2Result = closure_16(state(4752).Text, obj, state.key);
  ...
  return {
    [closure_0(closure_2[43]).AST_KEY.TEXT]: {
      react(content, output, textColor) {
        if (typeof content.content === "string") {
          ...
          obj.children = obj(8353).smartOutput(content, output, textColor);
    ...
    [closure_0(closure_2[43]).AST_KEY.STRIKETHROUGH]: {
      react(node, output, textColor) {
        ...
        obj.children = obj(8353).smartOutput(node, output, textColor);
    ...
    [closure_0(closure_2[43]).AST_KEY.HIGHLIGHT]: {
      react(node, output, textColor) {
        ...
        obj = { color: textColor, children: obj(8353).smartOutput(node, output, textColor) };
        return closure_1_15(MarkupText, obj, textColor.key);
      }
    },
    ...
    [closure_0(closure_2[43]).AST_KEY.STATIC_ROUTE_LINK]: {
      react(channelId, output, textColor) {
        ...
        if (obj.isStaticRouteIconType(channelId.channelId)) {
          let SignPostIcon = tmp(14104).SignPostIcon;
          ...
          if (constants.GUILD_HOME !== channelId) {
              ...
              if (tmp4.CHANNEL_BROWSER !== channelId) {
                if (tmp4.CUSTOMIZE_COMMUNITY !== channelId) {
                  if (tmp4.LINKED_ROLES === channelId) {
                    SignPostIcon = tmp(4698).LinkIcon;
                  }
                }
              }
              SignPostIcon = tmp(14106).ChannelListMagnifyingGlassIcon;
            }
            ...
            const items = [__initData(SignPostIcon, obj3), tmp(8353).smartOutput(channelId, output, textColor)];
            obj2.children = items;
            return value2(MarkupText, obj2, textColor.key);
          }
          SignPostIcon = tmp(14104).SignPostIcon;
        } else {
          return null;
        }
      }
    ...
    [closure_0(closure_2[43]).AST_KEY.CODE_BLOCK]: {
      parse(arg0, arg1, arg2) {
        obj = obj2(5209).RULES[obj(undefined, 5207).AST_KEY.CODE_BLOCK];
    ...
    [closure_0(closure_2[43]).AST_KEY.SOUNDBOARD]: {
      react(node, output, key) {
        ...
        const items = ["<sound:", obj(8353).smartOutput(node, output, key), ">"];
        obj.children = items;
        return closure_1_16(obj(4752).Text, obj, key.key);
      }
    },
    [closure_0(closure_2[43]).AST_KEY.GUILD]: {
      react(icon, output, textColor) {
        ...
        if (null == icon.icon) {
          ...
          const obj4 = { color: textColor1, children: tmp(8353).smartOutput(icon, output, textColor) };
          ...
          if (fontScale < 1) {
            let XXSMALL = tmp(5796).GuildIconSizes.XXXSMALL;
          } else if (fontScale < 1.25) {
            XXSMALL = tmp(5796).GuildIconSizes.XXSMALL_12;
          } else {
            XXSMALL = tmp(5796).GuildIconSizes.XXSMALL;
          }
          obj5.size = XXSMALL;
          closure_1_15(obj2(5796), obj5);
          const tmp6 = obj2(5796);
        }
      }
    },
    [closure_0(closure_2[43]).AST_KEY.CHANNEL]: {
      react(iconType, output, textColor) {
        ...
        items[1] = obj(8353).smartOutput(iconType, output, textColor);
        obj3.children = items;
    ...
    [closure_0(closure_2[43]).AST_KEY.GAME_MENTION]: {
      react(node, arg1, state) {
        return closure_1_15(obj2(14108), { node, state }, state.key);
      }
    },
    ...
    [closure_0(closure_2[43]).AST_KEY.LIST]: {
      react(start, arg1, level) {
        ...
        if (level.formatInline) {
          ...
          return closure_15(start(4752).Text, obj, "list-" + level.key);
        }
      }
    },
    [closure_0(closure_2[43]).AST_KEY.HEADING]: {
      react(level, output, formatInline) {
        if (formatInline.formatInline) {
          ...
          const items = [obj(8353).smartOutput(level, output, obj3), " "];
          obj2.children = items;
          ...
          const items1 = [obj(8353).smartOutput(level, output, obj4), "\n"];
          obj.children = items1;
          return closure_1_16(obj(4752).Text, obj, formatInline.key);
        }
      }
    },
    [closure_0(closure_2[43]).AST_KEY.SUBTEXT]: {
      react(node, output, key) {
        ...
        const items = [obj(8353).smartOutput(node, output, key), "\n"];
        obj.children = items;
        return closure_1_16(obj(4752).Text, obj, key.key);
      }
    },
...
export const createFetchingGameMentionRule = fn(14108).createFetchingGameMentionRule;
