...
// Dependencies: [32, 19, 17, 5608, 16547, 21, 4722, 9596, 7177, 4689, 16548, 1897, 576, 5142, 6799, 1115, 5857, 5775, 1092, 14813, 5794, 504, 16545, 7196, 2]
// Exports: default

// Module 16546 (GuildsBarFolderSettingsModal)
...
import GuildsBarFolderSettingsModalActionCreators from "GuildsBarFolderSettingsModalActionCreators" /* 16545 */;
...
function GuildFolderSettingsScene(color) {
  ...
  const callback = noop.useCallback(() => {
    ...
    obj.openLazy(asyncRequireImpl(16548, dependencyMap.paths), "RoleColorPicker", { color: tmp3, defaultColor, onSelect: onColorChange });
  }, items);
  obj2.padding = onColorChange(576).space.PX_16;
  obj2.paddingBottom = 38 + onColorChange(7177)().insets.bottom;
  ...
  const items1 = [closure_10(color(6799).TextInput, obj4), ];
  ...
  obj5.trailing = closure_10(onColorChange(14813), { color: tmp11, style: tmp.colorBlock });
  obj6.children = closure_10(color(5775).TableRow, obj5);
  items1[1] = closure_10(color(5857).TableRowGroup, obj6);
  obj3.children = items1;
  obj.children = closure_11(color(5142).Stack, obj3);
  return closure_10(closure_6, obj);
}
get_ActivityIndicator = fn(17);
({ Keyboard: hasOwnProperty, ScrollView: metroRequire } = get_ActivityIndicator);
const GuildsBarConstants = fn(16547);
...
export default function GuildFolderSettingsModal(folderId) {
  ...
  const memo = obj2.useMemo(() => {
    ...
    if (closure_5) {
      ...
      let fn = tmp3(5794).getHeaderTextButton(intl2.string(tmp3(1115).t["R3BPH+"]), callback);
      const tmp3Result = tmp3(5794);
    } else {
      fn = () => null;
    }
    obj2.headerRight = fn;
  ...
  return closure_10(folderId(7196).Navigator, { screens: memo, initialRouteName });
};
