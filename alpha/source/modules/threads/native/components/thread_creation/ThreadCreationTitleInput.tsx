...
// Dependencies: [19, 2041, 1074, 21, 17078, 8017, 7514, 1482, 1610, 504, 9418, 1115, 6843, 5798, 2]

// Module 17077 (ThreadCreationTitleInput)
...
export default noop.memo(noop.forwardRef((chatInputRef, ref) => {
  ...
  const callback1 = ref.useCallback(() => {
    if (null != threadSettingsDraft.name) {
      if (null != tmp.parentChannelId) {
        ...
        if (tmp4 !== tmp.name) {
          const obj = { name: tmp4 };
          tmp2(8017).changeThreadSettings(tmp.parentChannelId, obj);
          const tmp2Result = tmp2(8017);
        }
        tmp2 = importDefault;
      }
    }
  ...
  const renderErrorResult = chatInputRef(17078).renderError(chatInputRef.threadNameError, { content: threadSettingsDraft.name });
  ...
  if (null != stateFromStores) {
    str = tmp(9418).getDefaultThreadName(stateFromStores, threadSettingsDraft.parentMessageId);
  ...
  const obj4 = { defaultValue: threadSettingsDraft(5798)(ref), errorMessage: renderErrorResult, label: stringResult, accessibilityHint: null, required: null, clearable: true, autoFocus: true, maxLength: null, onSubmitEditing: null, onFocus: null, onBlur: null, onChange: null, placeholder: null, ref: null, returnKeyType: "next", textContentType: "none" };
  let stringResult1;
  if (!optional) {
  ...
  obj4.ref = ref;
  return jsx(chatInputRef(6843).TextInput, { defaultValue: threadSettingsDraft(5798)(ref), errorMessage: renderErrorResult, label: stringResult, accessibilityHint: null, required: null, clearable: true, autoFocus: true, maxLength: null, onSubmitEditing: null, onFocus: null, onBlur: null, onChange: null, placeholder: null, ref: null, returnKeyType: "next", textContentType: "none" });
}));
