...
// Dependencies: [19, 1958, 1980, 4737, 4741, 4363, 1074, 21, 10484, 1115, 10485, 10487, 7164, 7306, 10463, 4662, 7967, 10276, 8085, 4684, 4674, 10489, 5264, 10491, 8863, 4664, 11207, 9297, 4689, 10384, 1897, 10021, 11213, 11564, 504, 7465, 12, 8108, 4875, 1936, 11226, 5754, 1177, 7393, 11252, 7395, 10872, 7385, 4419, 2]
// Exports: default

// Module 16372 (ThreadLongPressActionSheet)
...
function ThreadLongPressActionSheetConnected(channel) {
  ...
  const canMarkChannelUnread = channel(10487).useCanMarkChannelUnread(channel);
  const obj4 = channel(10487);
  const canManageThread = channel(7465).useCanManageThread(channel);
  const obj5 = channel(7465);
  const isThreadModerator = channel(7465).useIsThreadModerator(channel);
  const obj6 = channel(7465);
  const canUnarchiveThread = channel(7465).useCanUnarchiveThread(channel);
  const obj7 = channel(7465);
  const canJoinThreadVoice = channel(7465).useCanJoinThreadVoice(channel);
  ...
  const tmp14 = onClose(8108)(channel);
  const DeveloperMode = channel(1936).DeveloperMode;
  let setting = DeveloperMode.useSetting();
  const tmp15 = onClose(4875)(channel);
  if (null != stateFromStores) {
    const obj11 = { guild: stateFromStores, size: tmp(5754).GuildIconSizes.LARGE };
    let tmp19 = closure_13(tmp13(5754), obj11);
    let tmp18 = closure_13;
    const tmp13Result = tmp13(5754);
  } else {
    tmp18 = closure_13;
  ...
  if (MarkChannelUnreadExperiment.getConfig({ location: "thread_action_sheet" }).enabled) {
    if (!stateFromStores1) {
      if (canMarkChannelUnread) {
        ...
        obj14.IconComponent = tmp(10485).ChatMarkUnreadIcon;
      ...
      if (hasJoined) {
        ...
        const obj17 = {
          label: string2Result,
          IconComponent: tmp(4662).UserMinusIcon,
          isDestructive: true,
          onPress() {
        ...
        const obj18 = {
          label: stringResult,
          IconComponent: tmp(10276).GroupPlusIcon,
          onPress() {
        ...
        if (null == tmp23) {
          ...
          if (archived) {
            if (canUnarchiveThread) {
              ...
              const obj19 = {
                label: string5Result,
                IconComponent: tmp(4684).ClockIcon,
                onPress() {
            ...
            const obj20 = {
              label: string4Result,
              IconComponent: tmp(4674).XLargeIcon,
              onPress() {
          ...
          if (isThreadModerator) {
            ...
            if (isLockedThreadResult) {
              ...
              obj21.IconComponent = tmp(10489).LockUnlockedIcon;
          ...
          if (!isThreadModerator) {
            ...
            obj22.IconComponent = tmp(4664).LinkIcon;
            ...
            if (isMuted) {
              ...
              obj24.IconComponent = tmp(10021).BellIcon;
              ...
              obj24[1] = tmp(9297).BellSlashIcon;
              ...
              obj25.IconComponent = tmp(11213).ChannelNotificationIcon;
              ...
              obj26.header = tmp18(tmp(11252).ActionSheetIconHeader, obj27);
              const items8 = [
                items5.map((buttons) => {
              ...
              if (setting) {
                ...
                const obj30 = { IconComponent: tmp(10872).IdIcon };
                obj29.icon = tmp18(tmp(7395).ActionSheetRow.Icon, obj30);
                ...
                obj28.children = tmp18(tmp(7395).ActionSheetRow, obj29);
                setting = tmp18(tmp(7395).ActionSheetRow.Group, obj28, "developer-actions");
              }
              items8[1] = setting;
              obj26.children = items8;
              return closure_14(tmp(7393).ActionSheet, obj26);
            }
          } else {
            ...
            const obj31 = {
              label: string7Result,
              IconComponent: tmp(10491).PencilIcon,
              onPress() {
          ...
          const obj32 = { label: string3Result, IconComponent: tmp(8085).PhoneCallIcon, onPress: tmp23 };
  ...
  obj33.IconComponent = tmp(7164).EyeIcon;
