...
// Dependencies: [19, 2041, 2063, 4771, 4775, 4397, 1074, 21, 10500, 1115, 10501, 10503, 7208, 7350, 10479, 4696, 8005, 10288, 8123, 4718, 4708, 10505, 5306, 10507, 8902, 4698, 11220, 10409, 4723, 10396, 1980, 9873, 11226, 11573, 504, 7509, 12, 8146, 4909, 2019, 11239, 5796, 1177, 7437, 11265, 7439, 10888, 7429, 4453, 2]
// Exports: default

// Module 16448 (ThreadLongPressActionSheet)
...
function ThreadLongPressActionSheetConnected(channel) {
  ...
  const canMarkChannelUnread = channel(10503).useCanMarkChannelUnread(channel);
  const obj4 = channel(10503);
  const canManageThread = channel(7509).useCanManageThread(channel);
  const obj5 = channel(7509);
  const isThreadModerator = channel(7509).useIsThreadModerator(channel);
  const obj6 = channel(7509);
  const canUnarchiveThread = channel(7509).useCanUnarchiveThread(channel);
  const obj7 = channel(7509);
  const canJoinThreadVoice = channel(7509).useCanJoinThreadVoice(channel);
  const obj8 = channel(7509);
  ...
  const tmp14 = onClose(8146)(channel);
  const DeveloperMode = channel(2019).DeveloperMode;
  let setting = DeveloperMode.useSetting();
  const tmp15 = onClose(4909)(channel);
  if (null != stateFromStores) {
    const obj11 = { guild: stateFromStores, size: tmp(5796).GuildIconSizes.LARGE };
    let tmp19 = closure_13(tmp13(5796), obj11);
    let tmp18 = closure_13;
    const tmp13Result = tmp13(5796);
  } else {
    tmp18 = closure_13;
  ...
  if (MarkChannelUnreadExperiment.getConfig({ location: "thread_action_sheet" }).enabled) {
    if (!stateFromStores1) {
      if (canMarkChannelUnread) {
        ...
        obj14.IconComponent = tmp(10501).ChatMarkUnreadIcon;
      ...
      if (hasJoined) {
        ...
        const obj18 = {
          label: stringResult,
          IconComponent: tmp(10288).GroupPlusIcon,
          onPress() {
        ...
        if (null == tmp23) {
          ...
          if (isThreadModerator) {
            ...
            if (isLockedThreadResult) {
              ...
              obj21.IconComponent = tmp(10505).LockUnlockedIcon;
          ...
          if (!isThreadModerator) {
            ...
            if (isMuted) {
              ...
              obj24.IconComponent = tmp(9873).BellIcon;
              ...
              obj24[1] = tmp(10409).BellSlashIcon;
              ...
              obj25.IconComponent = tmp(11226).ChannelNotificationIcon;
              ...
              obj26.header = tmp18(tmp(11265).ActionSheetIconHeader, obj27);
              const items8 = [
                items5.map((buttons) => {
              ...
              if (setting) {
                ...
                const obj30 = { IconComponent: tmp(10888).IdIcon };
                obj29.icon = tmp18(tmp(7439).ActionSheetRow.Icon, obj30);
                ...
                obj28.children = tmp18(tmp(7439).ActionSheetRow, obj29);
                setting = tmp18(tmp(7439).ActionSheetRow.Group, obj28, "developer-actions");
              }
              items8[1] = setting;
              obj26.children = items8;
              return closure_14(tmp(7437).ActionSheet, obj26);
            }
          } else {
            ...
            const obj31 = {
              label: string7Result,
              IconComponent: tmp(10507).PencilIcon,
              onPress() {
          ...
          const obj32 = { label: string3Result, IconComponent: tmp(8123).PhoneCallIcon, onPress: tmp23 };
  ...
  obj33.IconComponent = tmp(7208).EyeIcon;
