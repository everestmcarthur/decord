...
// Dependencies: [32, 19, 17, 16851, 21, 4722, 576, 16988, 16990, 1115, 3676, 16989, 5777, 4718, 1612, 504, 16991, 16992, 4689, 16993, 8435, 11476, 16985, 8957, 7246, 16864, 4670, 2]
// Exports: default

// Module 16987 (VibegrationsTraceTab)
...
import VibegrationsTraceFormat from "VibegrationsTraceFormat" /* 16988 */;
import vibegrations_VibegrationsTraceFormat from "vibegrations/VibegrationsTraceFormat" /* 16989 */;
import VibegrationsTraceUtils from "VibegrationsTraceUtils" /* 16990 */;
import VibegrationsTraceDetailSheet from "VibegrationsTraceDetailSheet" /* 16993 */;
...
function TraceRow(entry) {
  ...
  if ("model" === entry.kind) {
    if (null != entry.promptTokens) {
      ...
      const obj3 = { tokens: tmp2(16989).formatTokens(entry.promptTokens) };
      let formatToPlainStringResult = intl.formatToPlainString(_modDef3676["PYO+Jv"], obj3);
      const tmp2Result = tmp2(16989);
    }
    ...
    const items1 = [React5(tmp2(16988).TraceStatusDot, obj8), , , ];
    const obj9 = { variant: "text-xs/semibold", style: traceCategoryTextStyles[traceCategoryResult], children: tmp2(16989).categoryLabel(traceCategoryResult) };
    items1[1] = React5(tmp2(4718).Text, obj9);
    const obj10 = { variant: "text-xs/semibold", color: "text-default", style: tmp.rowTitle, lineClamp: 1, children: tmp6 };
    items1[2] = React5(tmp2(4718).Text, obj10);
    ...
    if (null != formatToPlainStringResult) {
      ...
      tmp10Result = tmp10(tmp2(4718).Text, obj11);
    }
    ...
    if ("tool" === entry.kind) {
      ...
      if (null != entry.summary) {
        ...
        tmp10Result3 = tmp10(tmp2(4718).Text, obj12);
      }
    }
    ...
    if (null != entry.error) {
      ...
      tmp10Result4 = tmp10(tmp2(4718).Text, obj13);
    }
    items2[2] = tmp10Result4;
    obj6.children = items2;
    obj5.children = React6(View, obj6);
    obj4.children = React5(tmp2(5777).Card, obj5);
    return React5(View, obj4);
  }
  formatToPlainStringResult = null;
  if (null != entry.durationMs) {
    formatToPlainStringResult = tmp2(16989).formatDuration(entry.durationMs);
...
function TraceOverview(arg0) {
  ...
  obj4.children = TRACE_CATEGORIES.map((item) => {
    ...
    const tmp4 = React6;
    const tmp5 = View;
    items1[3] = React5(Text_Text.Text, { variant: "text-xs/normal", color: "text-subtle", children: intl2.formatToPlainString(_modDef3676.w8vPbe, { count: num4 }) });
    let tmp6Result = null;
    if (0 !== num) {
      const obj7 = { variant: "text-xs/normal", color: "text-subtle", children: tmp7(16989).formatDuration(num) };
      tmp6Result = tmp6(tmp7(4718).Text, obj7);
      const tmp7Result = tmp7(16989);
    }
    items1[4] = tmp6Result;
    obj.children = items1;
  ...
  const memo = onPress.useMemo(() => {
    ...
    const item = projectId(stateFromStoresArray[8]).groupTraceByTurn(stateFromStoresArray).forEach((turnId, index) => {
      ...
      if (0 !== filterTraceResult.length) {
        ...
        obj2.started = tmp(16992).formatClockTime(turnId.startedAt);
  ...
  const callback1 = onPress.useCallback((item) => {
    ...
    if ("entry" === item.kind) {
      ...
      if (null != item.started) {
        ...
        tmp2 = React5(tmp13(4718).Text, obj);
      }
      items[1] = tmp2;
      let tmp3 = null;
      if (null != item.spanMs) {
        const obj5 = { variant: "text-xs/normal", color: "text-subtle", children: tmp13(16989).formatDuration(item.spanMs) };
        tmp3 = React5(tmp13(4718).Text, obj5);
        const tmp13Result = tmp13(16989);
      }
      items[2] = tmp3;
      obj3.children = items;
