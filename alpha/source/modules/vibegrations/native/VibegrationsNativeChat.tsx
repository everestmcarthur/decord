...
// Dependencies: [32, 19, 17, 1896, 16850, 16849, 21, 576, 16937, 672, 4722, 16939, 16944, 1115, 3676, 5142, 4718, 4712, 16945, 5144, 5777, 4417, 16950, 16951, 16952, 16953, 1364, 5834, 5156, 16955, 16943, 16940, 16186, 16956, 16957, 16959, 16961, 504, 1612, 16962, 16963, 16964, 16965, 16854, 16966, 8957, 16969, 16971, 16972, 16973, 16975, 2]
// Exports: default

// Module 16936 (VibegrationsNativeChat)
...
import VibegrationsNativeStatusLineDefault from "VibegrationsNativeStatusLine" /* 16937 */;
import VibegrationsMessageActionSheet from "VibegrationsMessageActionSheet" /* 16943 */;
import useVibegrationsPlanDesign from "useVibegrationsPlanDesign" /* 16944 */;
import VibegrationsNativeMarkdown from "VibegrationsNativeMarkdown" /* 16945 */;
import VibegrationsTimelineTree from "VibegrationsTimelineTree" /* 16950 */;
import VibegrationsSubagentMark from "VibegrationsSubagentMark" /* 16953 */;
import VibegrationsTodoAgents from "VibegrationsTodoAgents" /* 16955 */;
import VibegrationsChatGrouping from "VibegrationsChatGrouping" /* 16962 */;
import vibegrations_VibegrationsRepliedMessage from "vibegrations/VibegrationsRepliedMessage" /* 16964 */;
import _slicedToArray from "module_32" /* 32 */;
import noop from "module_19" /* 19 */;
import AppStateStore from "AppStateStore" /* 1896 */;
import VibegrationsChatStore from "VibegrationsChatStore" /* 16850 */;
...
function PlanDesign(arg0) {
  ...
  if (vibegrationsPlanDesign.gone) {
    ...
    items = [closure_1_17(tmp2(4718).Text, obj2), ];
    if (null == src) {
    ...
    items[1] = tmp9Result;
    obj7.children = items;
    return collapsedCategories(tmp2(5142).Stack, obj7);
  }
}
function ProposalCard(proposal) {
  ...
  if ("" === trimmed) {
    ...
    let stringResult = intl2.string(tmp8(3676).IHCafX);
  } else {
    stringResult = tmp8(4712).parse(trimmed, true, tmp6(16945).VIBEGRATIONS_MARKUP_OPTIONS);
  ...
  if (proposal.changes.length > 0) {
    ...
    obj5.children = intl3.string(tmp8(3676).KLyB8Y);
    const items1 = [tmp3(tmp6(4718).Text, obj5), ];
    ...
    tmp5Result = tmp5(tmp6(5142).Stack, obj4);
  }
  items[3] = tmp5Result;
  ...
  if (bot_permissions.length > 0) {
    ...
    obj7.children = intl4.string(tmp8(3676).ieqTtP);
    const items2 = [tmp3(tmp6(4718).Text, obj7), ];
    const obj8 = { variant: "text-sm/normal", color: "text-default", children: bot_permissions.join(", ") };
    items2[1] = tmp3(tmp6(4718).Text, obj8);
    obj6.children = items2;
    tmp5Result3 = tmp5(tmp6(5142).Stack, obj6);
  }
  items[4] = tmp5Result3;
  ...
  if (privileged_intents.length > 0) {
    ...
    obj10.children = intl5.string(tmp8(3676).Cn9qix);
    const items3 = [tmp3(tmp6(4718).Text, obj10), ];
    const obj11 = { variant: "text-sm/normal", color: "text-default", children: privileged_intents.join(", ") };
    items3[1] = tmp3(tmp6(4718).Text, obj11);
    obj9.children = items3;
    tmp5Result4 = tmp5(tmp6(5142).Stack, obj9);
  }
  items[5] = tmp5Result4;
  ...
  if (actionable) {
    ...
    obj12.text = intl6.string(tmp8(3676).GYoWRk);
    obj12.onPress = onApprove;
    tmp3Result2 = tmp3(tmp6(5144).Button, obj12);
  }
  items[6] = tmp3Result2;
...
function TimelineRow(live) {
  ...
  if (null != node.durationMs) {
    const obj3 = { variant: "text-xs/normal", color: "text-subtle", children: tmp7(16951).describeDuration(node.durationMs) };
    tmp4Result = tmp4(tmp7(4718).Text, obj3);
    const tmp7Result = tmp7(16951);
  }
...
function TurnStatusLine(epoch) {
  ...
  if (null != tmp8) {
    groupLabel = tmp5(16951).describeTurnDuration(tmp8);
    const tmp5Result = tmp5(16951);
  } else if (null != currentStepResult) {
    groupLabel = tmp5(16950).describeNode(currentStepResult);
    const tmp5Result2 = tmp5(16950);
...
function LaneStatusLine(arg0) {
  ...
  if ("running" === lane.task.status) {
    ...
    const obj5 = { line: mark(16952).describeTaskOutcome(lane.task), live: turnActive, settled: null, failed: null, glyph: null, crestColor: null, inGutter: true, epoch: null, expanded: null, onToggle: null };
...
obj.row = { position: "relative", paddingLeft: fn(16937).MESSAGE_CONTENT_INSET, paddingRight: fn(16937).MESSAGE_EDGE_INSET, paddingVertical: 2, gap: PX_8 };
...
let obj5 = { position: "relative", paddingLeft: fn(16937).MESSAGE_CONTENT_INSET, paddingRight: fn(16937).MESSAGE_EDGE_INSET, paddingVertical: 2, gap: PX_8 };
...
export default function VibegrationsNativeChat(projectId) {
  ...
  const items38 = [ref(stateFromStores(stateFromStores1[44]), { thinking: stateFromStores2, bleedBottom: stateFromStores(stateFromStores1[38])().bottom }), , ];
  ...
  const items42 = [ref(stateFromStores(stateFromStores1[49]), { projectId, thinking: stateFromStores2, compacting: stateFromStores3, activity: stateFromStores4, projectUsage: stateFromStores5, connLabel: connectionLabelResult, controlling: vibegrationsControlActive, connFailed: "failed" === stateFromStores6, thinkingOpen: tmp20, onToggleThinking: callback }), ];
