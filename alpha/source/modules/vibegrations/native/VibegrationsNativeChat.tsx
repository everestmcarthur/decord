...
// Dependencies: [32, 19, 17, 1979, 13360, 13359, 21, 576, 17012, 672, 4756, 17014, 17020, 1115, 3678, 5184, 4752, 4746, 17021, 5186, 5819, 4451, 17026, 17027, 17028, 17029, 1364, 5876, 5198, 17031, 16930, 17018, 17015, 17019, 16262, 17032, 17033, 17035, 17037, 504, 1612, 17038, 17039, 17040, 17041, 13181, 17042, 8996, 17045, 17047, 17048, 17049, 17051, 2]
// Exports: default

// Module 17011 (VibegrationsNativeChat)
...
import VibegrationsNativeStatusLineDefault from "VibegrationsNativeStatusLine" /* 17012 */;
import VibegrationsMessageActionSheet from "VibegrationsMessageActionSheet" /* 17018 */;
import useVibegrationsPlanDesign from "useVibegrationsPlanDesign" /* 17020 */;
import VibegrationsNativeMarkdown from "VibegrationsNativeMarkdown" /* 17021 */;
import VibegrationsTimelineTree from "VibegrationsTimelineTree" /* 17026 */;
import VibegrationsSubagentMark from "VibegrationsSubagentMark" /* 17029 */;
import VibegrationsTodoAgents from "VibegrationsTodoAgents" /* 17031 */;
import VibegrationsChatGrouping from "VibegrationsChatGrouping" /* 17038 */;
import vibegrations_VibegrationsRepliedMessage from "vibegrations/VibegrationsRepliedMessage" /* 17040 */;
...
function PlanDesign(arg0) {
  ...
  if (vibegrationsPlanDesign.gone) {
    ...
    items = [closure_1_17(tmp2(4752).Text, obj2), ];
    if (null == src) {
    ...
    items[1] = tmp9Result;
    obj7.children = items;
    return collapsedCategories(tmp2(5184).Stack, obj7);
  }
}
function ProposalCard(proposal) {
  ...
  if ("" === trimmed) {
    ...
    stringResult = tmp8(4746).parse(trimmed, true, tmp6(17021).VIBEGRATIONS_MARKUP_OPTIONS);
    const tmp8Result = tmp8(4746);
  }
  ...
  if (proposal.changes.length > 0) {
    ...
    const items1 = [tmp3(tmp6(4752).Text, obj5), ];
    ...
    tmp5Result = tmp5(tmp6(5184).Stack, obj4);
  }
  items[3] = tmp5Result;
  ...
  if (bot_permissions.length > 0) {
    ...
    const items2 = [tmp3(tmp6(4752).Text, obj7), ];
    const obj8 = { variant: "text-sm/normal", color: "text-default", children: bot_permissions.join(", ") };
    items2[1] = tmp3(tmp6(4752).Text, obj8);
    obj6.children = items2;
    tmp5Result3 = tmp5(tmp6(5184).Stack, obj6);
  }
  items[4] = tmp5Result3;
  ...
  if (privileged_intents.length > 0) {
    ...
    const items3 = [tmp3(tmp6(4752).Text, obj10), ];
    const obj11 = { variant: "text-sm/normal", color: "text-default", children: privileged_intents.join(", ") };
    items3[1] = tmp3(tmp6(4752).Text, obj11);
    obj9.children = items3;
    tmp5Result4 = tmp5(tmp6(5184).Stack, obj9);
  }
  items[5] = tmp5Result4;
  ...
  if (actionable) {
    ...
    tmp3Result2 = tmp3(tmp6(5186).Button, obj12);
  }
  items[6] = tmp3Result2;
...
function TimelineRow(live) {
  ...
  if (null != node.durationMs) {
    const obj3 = { variant: "text-xs/normal", color: "text-subtle", children: tmp7(17027).describeDuration(node.durationMs) };
    tmp4Result = tmp4(tmp7(4752).Text, obj3);
    const tmp7Result = tmp7(17027);
  }
...
function TurnStatusLine(epoch) {
  ...
  if (null != tmp8) {
    groupLabel = tmp5(17027).describeTurnDuration(tmp8);
    const tmp5Result = tmp5(17027);
  } else if (null != currentStepResult) {
    groupLabel = tmp5(17026).describeNode(currentStepResult);
    const tmp5Result2 = tmp5(17026);
...
function LaneStatusLine(arg0) {
  ...
  if ("running" === lane.task.status) {
    ...
    const obj5 = { line: mark(17028).describeTaskOutcome(lane.task), live: turnActive, settled: null, failed: null, glyph: null, crestColor: null, inGutter: true, epoch: null, expanded: null, onToggle: null };
...
obj.row = { position: "relative", paddingLeft: fn(17012).MESSAGE_CONTENT_INSET, paddingRight: fn(17012).MESSAGE_EDGE_INSET, paddingVertical: 2, gap: PX_8 };
...
let obj5 = { position: "relative", paddingLeft: fn(17012).MESSAGE_CONTENT_INSET, paddingRight: fn(17012).MESSAGE_EDGE_INSET, paddingVertical: 2, gap: PX_8 };
