...
// Dependencies: [19, 17, 9308, 21, 4756, 576, 4752, 1115, 3678, 17065, 17064, 1612, 504, 17066, 17070, 17071, 17068, 7437, 7389, 6864, 2]
// Exports: default

// Module 17069 (VibegrationsTraceDetailSheet)
...
export default function VibegrationsTraceDetailSheet(projectId) {
  ...
  const traceCategoryTextStyles = projectId(17064).useTraceCategoryTextStyles();
  let obj = projectId(17064);
  ...
  let findTraceEntryResult = projectId(17066).findTraceEntry(stateFromStoresArray, entryId);
  ...
  if ("tool" === findTraceEntryResult.kind) {
    ...
    findTraceEntryResult1 = tmp2(17066).findTraceEntry(stateFromStoresArray, parentId);
    const tmp2Result = tmp2(17066);
  }
  const obj3 = projectId(17066);
  const length = tmp2(17066).traceChildren(stateFromStoresArray, findTraceEntryResult.id).length;
  const tmp2Result13 = tmp2(17066);
  const traceDetailSectionsResult = tmp2(17070).traceDetailSections(findTraceEntryResult, { childCount: length, hasParent: null != findTraceEntryResult1 });
  const obj4 = { childCount: length, hasParent: null != findTraceEntryResult1 };
  const tmp2Result14 = tmp2(17070);
  let detailId;
  if ("tool" === findTraceEntryResult.kind) {
    detailId = findTraceEntryResult.detailId;
  }
  const vibegrationsTraceDetail = tmp2(17071).useVibegrationsTraceDetail(projectId, detailId);
  const tmp12 = "model" === findTraceEntryResult.kind ? findTraceEntryResult.model : findTraceEntryResult.tool;
  const tmp2Result15 = tmp2(17071);
  const formatClockTimeResult = tmp2(17068).formatClockTime(findTraceEntryResult.startedAt, "millis");
  const tmp2Result16 = tmp2(17068);
  const traceCategoryResult = tmp2(17066).traceCategory(findTraceEntryResult);
  const tmp2Result17 = tmp2(17066);
  const traceRichStatusLabelResult = tmp2(17065).traceRichStatusLabel(vibegrationsTraceDetail);
  const obj5 = { scrollable: true, header: closure_5(tmp2(7389).BottomSheetTitleHeader, { title: tmp12 }), children: null };
  ...
  const items2 = [closure_5(tmp2(17064).TraceStatusDot, { status: findTraceEntryResult.status }), , ];
  ...
  const tmp2Result18 = tmp2(17065);
  obj10.children = tmp2(17065).categoryLabel(traceCategoryResult);
  ...
  if (null == findTraceEntryResult.durationMs) {
    ...
    stringResult = tmp2(17065).formatDuration(findTraceEntryResult.durationMs);
    const tmp2Result20 = tmp2(17065);
  }
  ...
  if (traceDetailSectionsResult.includes("usage")) {
    ...
    if ("model" === findTraceEntryResult.kind) {
      ...
      if (null != findTraceEntryResult.promptTokens) {
        ...
        const obj24 = { tokens: tmp2(17065).formatTokens(findTraceEntryResult.promptTokens) };
        obj23.value = intl6.formatToPlainString(tmp5(3678)["PYO+Jv"], obj24);
        tmp16Result23 = tmp16(Row, obj23);
        const tmp2Result21 = tmp2(17065);
      }
      ...
      if (null != findTraceEntryResult.systemTokens) {
        ...
        const obj26 = { system: tmp2(17065).formatTokens(findTraceEntryResult.systemTokens), tools: null, toolCount: null, messages: null, messageCount: null };
        const tmp2Result22 = tmp2(17065);
        const tmp37 = Row;
        let num2 = findTraceEntryResult.toolsTokens;
        if (num2 == null) {
          num2 = 0;
        }
        obj26.tools = tmp2(17065).formatTokens(num2);
        let num3 = findTraceEntryResult.tools;
        if (num3 == null) {
          num3 = 0;
        }
        obj26.toolCount = num3;
        const tmp2Result23 = tmp2(17065);
        let num4 = findTraceEntryResult.messagesTokens;
        if (num4 == null) {
          num4 = 0;
        }
        obj26.messages = tmp2(17065).formatTokens(num4);
  ...
  obj5.children = closure_5(tmp2(6864).BottomSheetScrollView, obj6);
  return closure_5(tmp2(7437).ActionSheet, obj5);
};
