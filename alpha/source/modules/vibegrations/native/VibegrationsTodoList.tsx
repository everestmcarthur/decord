  ...
  if (0 !== shown.length) {
    ...
    const items1 = [
      shown.map((key) => {
          ...
          obj.accessibilityLabel = intl.formatToPlainString(_modDef3678.yTB8eu, { name: key.name, task: key.task });
    ...
    if (overflow > 0) {
      ...
      obj3.accessibilityLabel = intl.formatToPlainString(items(3678).Vpu1Pd, obj4);
      const _HermesInternal = HermesInternal;
      obj3.children = "+" + overflow;
      tmp9 = closure_6(tmp2(4752).Text, obj3);
    }
    items1[1] = tmp9;
...
function TodoMarker(status) {
  ...
  if ("completed" === status) {
    ...
    let stringResult = intl3.string(_modDef3678.TkPGOH);
    let tmp10 = importDefault;
    let tmp11 = require;
  } else if ("in_progress" === status) {
    const intl2 = util.intl;
    stringResult = intl2.string(_modDef3678["oK+fmd"]);
    tmp10 = importDefault;
    tmp11 = require;
  } else {
    const intl = util.intl;
    stringResult = intl.string(_modDef3678.d7lieu);
    tmp10 = importDefault;
    tmp11 = require;
  }
...
export default function VibegrationsTodoList(announceProgress) {
  ...
  if (0 !== todos.length) {
    ...
    if (length2 > 0) {
      ...
      obj3.children = intl.string(_modDef3678.qCRC6c);
      let items1 = [closure_6(agents(4752).Text, obj3), ];
      let str2 = "none";
      if (flag) {
        str2 = "polite";
      }
      ...
      obj4.accessibilityLabel = intl2.formatToPlainString(_modDef3678["QG/EiF"], obj5);
      const intl3 = tmp9(1115).intl;
      const obj6 = { completed: length, total: length2 };
      obj4.children = intl3.formatToPlainString(_modDef3678.bQvqly, obj6);
      items1[1] = closure_6(agents(4752).Text, obj4);
      obj2.children = items1;
    ...
    if (null != provisional) {
      ...
      if ("" !== provisional) {
        ...
        items4[1] = closure_6(agents(4752).Text, obj9);
