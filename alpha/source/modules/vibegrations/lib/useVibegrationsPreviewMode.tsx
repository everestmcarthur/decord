...
// Dependencies: [32, 19, 502, 504, 16953, 16954, 9286, 7403, 9593, 2]
// Exports: useVibegrationsPreviewMode

// Module 16952 (useVibegrationsPreviewMode)
import initialize from "initialize" /* 504 */;
import useUserApplicationWidgetDataDefault from "useUserApplicationWidgetData" /* 16953 */;
import vibegrationsPreviewModes from "vibegrationsPreviewModes" /* 16954 */;
...
export const useVibegrationsPreviewMode = function useVibegrationsPreviewMode(arg0) {
  ...
  let tmp14;
  if (surfaces != null) {
    tmp14 = surfaces[tmp8(undefined, 9286).ApplicationWidgetConfigSurface.WIDGET_TOP];
  }
  const obj2 = { widgetTop: null != tmp14, widgetBottom: null, miniProfile: null };
  let tmp15;
  if (surfaces != null) {
    tmp15 = surfaces[tmp8(undefined, 9286).ApplicationWidgetConfigSurface.WIDGET_BOTTOM];
  }
  obj2.widgetBottom = null != tmp15;
  let tmp16;
  if (surfaces != null) {
    tmp16 = surfaces[tmp8(undefined, 9286).ApplicationWidgetConfigSurface.MINI_PROFILE];
  }
  obj2.miniProfile = null != tmp16;
  const result = vibegrationsPreviewModes.profileSurfaceAvailability(obj2);
  if (null == tmp7) {
    const data = tmp8(7403).useApplication(previewApplicationId).data;
    ...
    const tmp8Result6 = tmp8(7403);
    const application = tmp8(7403).useApplication(applicationId);
    if (!declaredActivity) {
      declaredActivity = tmp8(9593).canLaunchFrame(tmp25);
      const tmp8Result8 = tmp8(9593);
    }
    const tmp8Result7 = tmp8(7403);
    const obj3 = { installScope, hasFrame: declaredActivity, hasProfileWidget: tmp18, hasBotDm: tmp21, ownerAuthorizationRevoked };
    const result1 = tmp8(16954).previewModeAvailability(obj3);
    ...
    if (!(null != applicationId && application.isLoading)) {
      previewMode = tmp8(16954).resolvePreviewMode(tmp2, result1);
      const tmp8Result10 = tmp8(16954);
    }
