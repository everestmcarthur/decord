...
// Dependencies: [1958, 2022, 1980, 2012, 4567, 1372, 1074, 5685, 12583, 10084, 4566, 4841, 5689, 5612, 12584, 504, 573, 2]
...
import SearchTokensDefault from "SearchTokens" /* 12584 */;
  ...
  if (constants.FILTER === type) {
    ...
    if (null != filter) {
      ...
      if (obj.isGuildLikeSearchContext(searchContext)) {
        ...
        if (tmp8) {
          if (0 !== str.length) {
            const searchContextId = tmp6(12583).getSearchContextId(searchContext);
            value = map1.get(searchContextId);
            if (value == null) {
...
const searchAutocompleteStoreClass = new SearchAutocompleteStoreClass(DispatcherDefault, {
  ...
  SEARCH_AUTOCOMPLETE_QUERY_UPDATE: function handleSearchAutocompleteQueryUpdate(arg0) {
    ...
    if (null != value) {
      ...
      const searchContextId1 = tmp5(12583).getSearchContextId(searchContext);
      let value4 = map1.get(searchContextId1);
      if (value4 == null) {
        const obj6 = { results: [], context: tmp(10084).getUserSearchContext(handleUserSearchResults.bind(null, searchContext)) };
        value4 = obj6;
        const tmpResult = tmp(10084);
      }
    ...
    if (tmp9) {
      const searchContextId2 = tmp5(12583).getSearchContextId(searchContext);
      let value5 = map1.get(searchContextId2);
      if (value5 == null) {
        const obj9 = { results: [], context: tmp(10084).getUserSearchContext(handleUserSearchResults.bind(null, searchContext)) };
        value5 = obj9;
        const tmpResult3 = tmp(10084);
      }
      ...
      if (null != trimmed) {
        if (trimmed.length > 0) {
          const guildIdFromSearchContext = tmp5(12583).getGuildIdFromSearchContext(searchContext);
          if (null != guildIdFromSearchContext) {
            const members = tmp(5689).requestMembers(guildIdFromSearchContext, trimmed, c17);
            const tmpResult4 = tmp(5689);
          }
          ...
          obj10.filters = obj11;
          const tmp5Result5 = tmp5(12583);
          obj10.boosters = tmp5(5612).getBoosterMap(AutocompleterResultTypes.USER);
