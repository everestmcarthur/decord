...
// Function ID: 17073
// Name: MembersScreen
// Dependencies: [19, 17, 7475, 1958, 2022, 1980, 2012, 1372, 12607, 12582, 8083, 8082, 1074, 21, 4722, 576, 563, 7358, 12583, 17029, 4366, 1875, 12601, 8409, 1115, 4433, 17073, 17071, 17021, 11828, 17033, 17074, 7378, 11832, 12428, 17076, 2]

// Module 17072 (MembersScreen)
...
import SearchMemberTabStore from "SearchMemberTabStore" /* 12607 */;
...
function SearchableMembersScreen(searchContext) {
  ...
  const analyticsLocations = guildId(7358)().analyticsLocations;
  const tmp = closure_21();
  dependencyMap = searchContext(12583).getSearchContextId(searchContext);
  let obj = searchContext(12583);
  ...
  fullscreenPlaceholderCount = searchContext(17029).useFullscreenPlaceholderCount({ placeholderHeight, numColumns: 1 });
  let obj4 = { placeholderHeight, numColumns: 1 };
  const tmp4Result8 = searchContext(17029);
  ...
  const contentContainerStyles = searchContext(17073).useContentContainerStyles();
  const tmp4Result13 = searchContext(17073);
  const messageTabCountsErrorText = searchContext(17071).useMessageTabCountsErrorText({ searchContext });
  if (null != messageTabCountsErrorText) {
    const obj5 = { text: messageTabCountsErrorText };
    let tmp25 = jsx(tmp2(17021), { text: messageTabCountsErrorText });
  } else {
    if (stateFromStores5) {
      if (null != stateFromStores4) {
        ...
        tmp25 = jsx(tmp2(11828), { onUserPress: callback1, onUserLongPress: callback2, channelId: stateFromStores4, guildId, disableStickySections: true, listStyleOverride: tmp.userList, isNameplatedList: true, canShowDisplayNameStylesFont: true });
      }
    }
    const obj7 = { contentContainerStyle: contentContainerStyles.membersContentContainer, data: memo };
    tmp25 = jsx(tmp2(17033), { contentContainerStyle: contentContainerStyles.membersContentContainer, data: memo });
  }
  return tmp25;
}
function ThreadMembersScreen(searchContext) {
  ...
  if (!stateFromStores) {
    if (obj2.useStateFromStores(items1, () => SearchQueryStore.isInitialSearchQuery(searchContext) && !SearchQueryStore.isTagsEmpty(searchContext), items2)) {
      ...
      let tmp7 = jsx(channelId(17074), { channelId, guildId, onUserPress: tmp(1875).dismissGlobalKeyboard, disableStickySections: true });
      const tmp6 = channelId(17074);
    }
    return tmp7;
  }
...
let closure_16 = fn(8082).SearchResultContentEntityTypes;
...
export default noop.memo(function MembersScreen(searchContext) {
  ...
  let stateFromStores;
  let tmp = closure_21();
  const analyticsLocations = stateFromStores(7358)(stateFromStores(7378).SEARCH_MEMBERS).analyticsLocations;
  let channelId;
  if (searchContext.type === constants3.CHANNEL) {
    channelId = searchContext.channelId;
  }
  const tmp4 = stateFromStores(7358);
  const items = [ChannelStore];
  ...
  if (constants3.CHANNEL === type) {
    ...
    if (stateFromStores) {
      ...
      tmp21Result = tmp21(tmp2(17076), obj7);
    }
    obj6.listHeaderContent = tmp21Result;
    obj4.children = jsx(tmp2(12428), { channelId: searchContext.channelId, disableStickySections: true, listStyleOverride: tmp.userList, onUserPress: tmp7(1875).dismissGlobalKeyboard, listHeaderContent: null });
    obj3.children = <View style={tmp.container}>{null}</View>;
    return jsx(tmp7(7358).AnalyticsLocationProvider, { value: analyticsLocations, children: null });
