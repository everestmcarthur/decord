...
// Dependencies: [7521, 12591, 8121, 12605, 1074, 12592, 8535, 1385, 6862, 1364, 1875, 4698, 1366, 11079, 576, 2019, 12610, 12613, 12599, 12611, 12, 558, 2]
...
// Module 12590 (SearchPlatformUtils)
...
function fetchInitialMessages(searchContext) {
  ...
  if (isInitialSearchQueryResult) {
    ...
    if (!SearchMessageStore.getIsFetching(searchTabFetchId)) {
      const result = queryString(12599).clearAllSearchMesssages();
      const obj3 = queryString(12599);
      const tmp9 = queryString;
      const obj5 = { searchContext };
      queryString(12610).trackSearchStarted(obj5);
      const obj4 = queryString(12610);
      const obj7 = {
        searchContext,
        searchTabs: tmp3,
      ...
      obj7.searchAnalyticsIds = tmp6(12592).getSearchAnalyticsIds(searchContext, tmp9(12611));
...
const SearchPlatformConstants = fn(12605);
...
obj.fetchNextMessages = function fetchNextMessages(searchContext, tab, onFetchSuccess) {
  ...
  if (!SearchMessageStore.getIsInitialFetchComplete(searchTabFetchId1)) {
    ...
    if (!flag) {
      ...
      obj5.searchAnalyticsIds = tmp2(12592).getSearchAnalyticsIds(searchContext, tmp17(12611));
      tabMessages = obj4.fetchTabMessages(obj5);
      const tmp2Result = tmp2(12592);
    }
    return tabMessages;
  } else {
...
obj.subscribeSearchQueryState = function subscribeSearchQueryState(searchContext, fn, fn2, arg3) {
  ...
  let tmp = arg3;
  function callback() {
    const tmp = f95054(SearchQueryStore.getManager(closure_0));
    if (null == closure_3) {
      closure_3 = tmp;
      f95055(tmp, closure_3);
    }
  }
...
obj.subscribeTextInputValue = function subscribeTextInputValue(searchContext, debounceResult, arg2) {
  ...
  let flag = arg2;
  const f95054 = (getTextInputValue) => ({ textInputValue: getTextInputValue.getTextInputValue(), textInputChangedFromInput: getTextInputValue.getTextValueChangedFromInput() });
  const f95055 = (textInputValue, textInputValue2) => {
...
export const getMedia = function getMedia(searchContext, items1) {
  ...
  let item = items1.forEach((getContentMessage) => {
    ...
    let result = guildIdFromSearchContext(8535).extractMediaFromMessageComponents(getContentMessage, contentMessage, closure_0);
    const iter = result[Symbol.iterator]();
    const obj = guildIdFromSearchContext(8535);
