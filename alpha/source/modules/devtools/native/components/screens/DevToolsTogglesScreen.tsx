...
// Dependencies: [32, 19, 17, 5839, 4755, 21, 5729, 4756, 576, 5817, 4454, 7441, 504, 5899, 15998, 7221, 16015, 5184, 16016, 7290, 2]
// Exports: default

// Module 16014 (DevToolsTogglesScreen)
...
function fuzzySearchToggle(str, str2, str3) {
  ...
  if (!tmp) {
    ...
    if (!tmp3ResultResult) {
      ...
      tmp3ResultResult = tmp3(5729)(formatted1, str3.toLowerCase());
      const tmp3Result = tmp3(5729);
    }
...
function ToggleTableRow(toggleName) {
  ...
  return closure_9(toggleName(5817).TableRow, {
    label: description,
    labelLineClamp: 1,
    subLabel: toggleName,
    subLabelLineClamp: 1,
    ...
    trailing: closure_9(toggleName(7441).FormSwitch, { value, onValueChange })
  }, toggleName);
}
function DevTogglesForCategory(title) {
  ...
  if (0 !== stateFromStores.length) {
    ...
    tmp3 = closure_9(category(5899).TableRowGroup, obj2);
  }
  return tmp3;
}
const ScrollView = fn(17).ScrollView;
const CATEGORY_LABELS = fn(4755).CATEGORY_LABELS;
const jsxProd = fn(21);
...
export default function DevToolsTogglesScreen() {
  ...
  const query = tmp3[0];
  const manaTextMigrationHighlightRestartNotice = query(16015).useManaTextMigrationHighlightRestartNotice();
  let obj = query(16015);
  const tmp5 = query;
  ...
  const stateFromStores = query(504).useStateFromStores(items, () => DesignTogglesStore.allWithDescriptions().filter((item) => {
    ...
    if (!tmp) {
      ...
      if (!tmp2ResultResult) {
        ...
        tmp2ResultResult = tmp2(5729)(formatted1, str2.toLowerCase());
        const tmp2Result = tmp2(5729);
      }
  ...
  const items3 = [
    closure_9(query(5817).TableRow, {
      label: "Clear All",
      variant: "danger",
      onPress() {
        first(16016).clearAll();
        const obj = first(16016);
        first(15998).clearAll();
      },
      arrow: true
    }),
  ...
  items3[1] = closure_9(query(5817).TableRow, { label: closure_9(query(7290).SearchField, { size: "md", placeholder: "Search design toggles", onChange: tmp3[1] }) });
  obj5.children = items3;
  const items4 = [closure_10(query(5899).TableRowGroup, obj5), , ];
  let tmp7Result = null;
    ...
    const obj8 = {
      ...
      children: stateFromStores.map((item) => {
          ...
          return closure_9(closure_13, {
            ...
            value: tmp2,
            onValueChange(arg0) {
              return first(16016).toggle(query, arg0);
            }
          }, tmp);
        })
    };
    tmp7Result = tmp7(tmp5(5899).TableRowGroup, obj8);
  }
  ...
  obj3.children = tmp9(query(5184).Stack, obj9);
