...
// Dependencies: [19, 17, 4748, 10351, 21, 4756, 576, 8544, 4752, 4457, 1364, 5174, 5292, 7208, 563, 7840, 10431, 10386, 8393, 8535, 1477, 5799, 7532, 7537, 8577, 10432, 1177, 9093, 2]
...
const VisualEffectViewDefault = tmp4(5174);
require = fn;
function VideoBadge() {
...
function ObscuredMediaOverlay(isSpoiler) {
  ...
  if (isObscured) {
    let tmp14Result = tmp14(tmp2(5292).ImageWarningIcon, { size: "sm", color: "white" });
  } else {
    const obj5 = { style: tmp.spoilerPill, children: tmp14(tmp2(7208).EyeIcon, { size: "sm", color: "white" }) };
...
function SinglePreviewableMedia(arg0) {
  ...
  if (usePreviewableMedia.PreviewableMediaTypes.VOICE_MESSAGE === type) {
    ...
    if (tmp2(10386).PreviewableMediaTypes.AUDIO !== type) {
      if (tmp2(10386).PreviewableMediaTypes.FILE !== type) {
        if (tmp2(10386).PreviewableMediaTypes.IMAGE !== type) {
          if (tmp2(10386).PreviewableMediaTypes.VIDEO !== type) {
            if (tmp2(10386).PreviewableMediaTypes.GIF === type) {
              ...
              if (null == thumbnail) {
                ...
                const enabledHarmTypesForChannelAndAuthorId = tmp2(7532).getEnabledHarmTypesForChannelAndAuthorId(message.channel_id, id);
                const tmp2Result = tmp2(7532);
                const obj5 = { type: null, media: null };
                const getMediaObscuredReasonFromBitmask = tmp2(7532).getMediaObscuredReasonFromBitmask;
                obj5.type = tmp2(7537).ObscuredMediaTypes.Embed;
                obj5.media = media;
                ...
                url2 = thumbnail.url;
                const tmp2Result5 = tmp2(7532);
              }
            } else if (tmp2(10386).PreviewableMediaTypes.STICKER === type) {
          ...
          if (width > 0) {
            if (null != height) {
              if (height > 0) {
                ...
                const attachmentObscurityProps = tmp2(8393).getAttachmentObscurityProps(obj13);
                const tmp2Result6 = tmp2(8393);
                const attachmentUrl = tmp2(8535).getAttachmentUrl(media);
                const tmp2Result7 = tmp2(8535);
                ...
                let str3;
                if (type === tmp2(10386).PreviewableMediaTypes.VIDEO) {
                  str3 = "png";
                }
                obj14.format = str3;
