...
// Function ID: 10300
// Name: StageGridRow
// Dependencies: [19, 17, 21, 4722, 10291, 5595, 5295, 10300, 10290, 2]
...
export default noop.memo((row) => {
  ...
  if (1 === num) {
    THIRD = tmp2(10291).StageTileSize.FULL;
  } else if (2 === num) {
    THIRD = tmp2(10291).StageTileSize.HALF;
  } else {
    THIRD = tmp2(10291).StageTileSize.THIRD;
  }
  ...
  return tmp4(tmp5, {
    ...
    children: participants.map((type) => {
      ...
      if (flag) {
        ...
        let tmp5Result = tmp5(tmp6(10300), obj2, "stage-media-participant-" + type.id);
        const tmp6Result = tmp6(10300);
      } else {
        const obj = { channel, participant: type, size: THIRD };
        const _HermesInternal = HermesInternal;
        tmp5Result = tmp5(tmp6(10290), obj, "stage-user-participant-" + type.id);
        const tmp6Result2 = tmp6(10290);
      }
      return tmp5Result;
    })
  });
