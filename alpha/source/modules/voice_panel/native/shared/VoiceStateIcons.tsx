...
// Dependencies: [19, 17, 21, 4722, 576, 9358, 9359, 9361, 9363, 9365, 1370, 1177, 8689, 9367, 9368, 2]

// Module 9357 (VoiceStateIcons)
...
const memoResult = noop.memo((arg0) => {
  ...
  if (VoiceStateIconUtils.MuteDeafenIconState.DEAFENED_SERVER === state) {
    ...
    items[1] = redTint;
    obj.style = items;
    return React4(tmp2(9359).HeadphonesDenyIcon, obj);
  } else if (tmp2(9358).MuteDeafenIconState.DEAFENED === state) {
    const obj2 = { style: null, size: "xs" };
    const items1 = [style];
    obj2.style = items1;
    return React4(tmp2(9361).HeadphonesSlashIcon, obj2);
  } else if (tmp2(9358).MuteDeafenIconState.MUTED_SERVER === state) {
    const items2 = [style, ];
    let redTint1 = null;
    if (!alwaysWhite) {
    ...
    items2[1] = redTint1;
    obj3.style = items2;
    return React4(tmp2(9363).MicrophoneDenyIcon, obj3);
  } else if (tmp2(9358).MuteDeafenIconState.MUTED_LOCAL === state) {
    const obj4 = { style: null, size: "xs" };
    const items3 = [style];
    obj4.style = items3;
    return React4(tmp2(9363).MicrophoneDenyIcon, obj4);
  } else if (tmp2(9358).MuteDeafenIconState.MUTED === state) {
    const obj5 = { style: null, size: "xs" };
    const items4 = [style];
    obj5.style = items4;
    return React4(tmp2(9365).MicrophoneSlashIcon, obj5);
  } else {
...
export const VideoIcon = noop.memo((state) => {
  ...
  if (VoiceStateIconUtils.VideoIconState.VIDEO_DISABLED_LOCAL_AUTO === state) {
    const obj = {};
    const merged1 = Object.assign(merged);
    obj.source = _modDef9367;
    const items = [merged.style, tmp2.noTint];
    obj.style = items;
    return React4(tmp3(1177).Icon, obj);
  } else if (tmp3(9358).VideoIconState.VIDEO_DISABLED_LOCAL === state) {
    const obj2 = {};
    const merged2 = Object.assign(merged);
    return React4(VideoDisabledSvgIcon, obj2);
  } else if (tmp3(9358).VideoIconState.VIDEO_ACTIVE === state) {
    const obj3 = {};
    const merged3 = Object.assign(merged);
    obj3.source = _modDef9368;
