...
// Dependencies: [32, 19, 1957, 9724, 1958, 12515, 12513, 1074, 1921, 21, 4722, 576, 12514, 17077, 504, 17519, 9742, 7364, 9877, 7358, 7378, 9864, 14194, 4458, 9714, 6848, 1478, 9710, 7269, 17520, 9880, 17521, 17522, 17526, 4432, 2]

// Module 17518 (VoicePanelAnimatedActivityCard)
...
function VoicePanelAnimatedActivityCardInner(applicationId) {
  ...
  const context = focused.useContext(sharedVisible(12514));
  ...
  const tmp7 = sharedVisible(17077)();
  closure_10 = tmp7;
  ...
  const activityShelfItemData = applicationId(17519).useActivityShelfItemData(guild_id1, applicationId);
  const items2 = [guild_id];
  const effect = obj.useEffect(() => {
    const shelf = EmbeddedActivitiesActionCreators.fetchShelf({ guildId: guild_id });
  }, items2);
  const items3 = [applicationId];
  application = tmp2(tmp4(7364)(items3), 1)[0];
  const tmp8Result = applicationId(17519);
  ...
  const tmp17 = sharedVisible(9877)();
  ...
  const tmp4Result = sharedVisible(7358);
  let tmp21 = null != tmp17;
  const analyticsContext = applicationId(9864).useAnalyticsContext();
  if (tmp21) {
    let id;
    if (application != null) {
      id = application.id;
  ...
  if (!tmp21) {
    ...
    if (stateFromStores1 != null) {
      ...
      if (participants != null) {
        found = participants.find((item) => applicationId(14194).isActivityParticipantCurrentUserCurrentSession(item));
      }
    }
    tmp21 = null != found;
  }
  ...
  const tmp8Result12 = applicationId(9864);
  let fn = function q() {
  ...
  const tmp8Result13 = applicationId(4458);
  fn2.__closure = { VoicePanelModes: controlsSpecs, runOnJS: applicationId(4458).runOnJS, incrementActivityKey };
  ...
  if (null != stateFromStores1) {
    ...
    embeddedActivityParticipantId = tmp8(9714).getEmbeddedActivityParticipantId({ applicationId: null, instanceId: null });
    const obj4 = { applicationId: null, instanceId: null };
    const tmp8Result14 = tmp8(9714);
  }
  const obj3 = { VoicePanelModes: controlsSpecs, runOnJS: applicationId(4458).runOnJS, incrementActivityKey };
  ...
  const tmp8Result15 = applicationId(4458);
  re.__closure = { runOnJS: applicationId(4458).runOnJS, setIsActivityFocused: tmp2Result[1] };
  ...
  const memo1 = obj.useMemo(() => {
    const Gesture = applicationId(6848).Gesture;
    return Gesture.Tap();
  }, []);
  ...
  const obj5 = { runOnJS: applicationId(4458).runOnJS, setIsActivityFocused: tmp2Result[1] };
  class Se {
    constructor() {
  ...
  const tmp8Result16 = applicationId(4458);
  ve.__closure = { runOnJS: applicationId(4458).runOnJS, handleTargetAspectRatioParams: callback1, gridOrientationLockState, hasJoined: tmp21, updateNotJoinedActivityDimensions: callback2 };
  ve.__workletHash = 13125606009235;
  ve.__initData = callback2;
  const animatedReaction2 = tmp8Result16.useAnimatedReaction(Se, ve);
  const tmp39 = !sharedVisible(9710)();
  __initData3 = tmp39;
  backgroundColor = tmp.activityContainerBackground.backgroundColor;
  const obj6 = { runOnJS: applicationId(4458).runOnJS, handleTargetAspectRatioParams: callback1, gridOrientationLockState, hasJoined: tmp21, updateNotJoinedActivityDimensions: callback2 };
  function fe() {
    let num = 0;
    if (React6) {
      num = closure_10.get();
  ...
  const animatedStyle = applicationId(4458).useAnimatedStyle(fe);
  ...
  const tmp8Result17 = applicationId(4458);
  const items11 = [mode];
  ...
  if (null == stateFromStores) {
    ...
    if (tmp21) {
      ...
      obj11.children = focusedOrientationLockState(tmp4(9880), obj12, tmp3[0]);
      const items13 = [focusedOrientationLockState(tmp4(17520), obj11), ];
      let tmp47Result = null;
      if (stateFromStores2) {
        tmp47Result = tmp47(tmp4(17521), {});
      }
      items13[1] = tmp47Result;
      obj9.children = items13;
      obj8.children = tmp48(tmp4Result3, obj9);
      let tmp47Result2 = tmp47(tmp8(6848).GestureDetector, obj8);
      let tmp44 = tmp47;
      const tmp4Result4 = tmp4(17520);
    } else if (null == activityShelfItemData) {
      const obj13 = { activity: stateFromStores1, application };
      tmp47Result2 = focusedOrientationLockState(tmp4(17522), obj13);
      ...
      obj14.children = focusedOrientationLockState(tmp4(17526), obj15);
      tmp47Result2 = focusedOrientationLockState(tmp8(6848).GestureDetector, obj14);
    }
    const obj16 = { value: tmp4Result(tmp4(7378).ACTIVITY_TILE).analyticsLocations, children: tmp47Result2 };
    return tmp44(tmp8(7358).AnalyticsLocationProvider, obj16);
  }
