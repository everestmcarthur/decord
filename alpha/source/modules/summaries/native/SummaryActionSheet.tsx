...
// Dependencies: [19, 17, 1958, 4943, 11597, 1074, 21, 4689, 11892, 1897, 4722, 576, 7465, 11865, 4419, 1115, 4867, 8593, 7967, 7979, 4579, 11, 1101, 7346, 7319, 11893, 4718, 11895, 10102, 11896, 8168, 2]
// Exports: default, openSummaryDividerActionSheet

// Module 11892 (SummaryActionSheet)
...
export default function SummaryActionSheet(summary) {
  ...
  const callback = obj.useCallback(() => {
    ...
    if (null != guild_id) {
      ...
      tmpResult.openLazy(asyncRequireImpl(11865, dependencyMap.paths), "GuildHighlightsNotifications", obj2);
      const tmp7 = asyncRequireImpl(11865, dependencyMap.paths);
    }
  }, items);
  ...
  const callback2 = obj.useCallback(() => {
    ...
    if (null != channel) {
      if (null != message) {
        const result = tmp(7967).openThreadCreationForMobile(tmp4, summary.startId, constants.SUMMARY_ACTION_SHEET);
        const tmpResult = tmp(7967);
        const obj3 = { name: summary.topic };
        tmp(7979).changeThreadSettings(tmp4.id, obj3);
        const tmp11 = require;
        const tmpResult4 = tmp(7979);
...
export const openSummaryDividerActionSheet = function openSummaryDividerActionSheet(channelId, summaryId) {
  ...
  if (null != findSummaryResult) {
    ...
    obj.openLazy(asyncRequireImpl(11892, dependencyMap.paths), "SummaryDivider" + summaryId, obj2);
    const tmp5 = asyncRequireImpl(11892, dependencyMap.paths);
  }
};
