...
// Function ID: 12018
// Name: QuestCopyHooks
// Dependencies: [5, 32, 19, 2026, 1372, 5614, 1074, 1374, 1115, 11729, 7918, 7920, 11542, 2025, 12018, 1886, 504, 12019, 8906, 8917, 5617, 11547, 1881, 9777, 11759, 11742, 7924, 7923, 2]
...
import isActivitySupportedOnClientPlatformDefault from "isActivitySupportedOnClientPlatform" /* 9777 */;
...
import useInGameQuestConnectState from "useInGameQuestConnectState" /* 12019 */;
...
const utils_QuestUtils = v1votF6(7918);
const GameProfileAnalyticUtils = tmp2(8917);
const SponsoredQuestUtils = v1votF6(12018);
require = fn;
...
export const useQuestInstructionTitle = function useQuestInstructionTitle(quest, arg1, questContent, sourceQuestContent) {
  ...
  if (tmpResult7.isConsoleQuest(quest)) {
    ...
    if (tmpResult8.hasWatchVideoTasks(quest)) {
      const defaultWatchVideoTask = tmp(7920).getDefaultWatchVideoTask(quest.config);
      let videoTitle;
      if (defaultWatchVideoTask != null) {
      ...
      if (tmpResult10.hasPlayActivityTask(quest)) {
        const result = tmp(7918).isPlayAnyActivityQuest(quest);
        ...
        tmpResult12 = tmp(7920);
      }
      tmpResult10 = tmp(7920);
    }
    tmpResult8 = tmp(7920);
  }
};
...
export const useQuestBarSubtitle = function useQuestBarSubtitle(arg0) {
  ...
  if (tmp8) {
    ...
    if (isExpanded) {
      ...
      if (activeScreen !== tmp(5617).TaskPlatformScreen.SELECT) {
        if (tmpResult9.isSponsoredPlayQuest(quest)) {
          if (tmp10) {
            if (!tmp11) {
              const defaultRewardNameWithArticle = tmp(11542).getDefaultRewardNameWithArticle(quest.config, stateFromStores);
              const intl5 = tmp(1115).intl;
      ...
      if (tmpResult8.hasWatchVideoTasks(quest)) {
        ...
        if (!tmpResult11.hasAchievementInGameTask(quest)) {
          if (isQuestProgressing) {
            ...
            let contextualEntrypointHeading = tmp(11547).getContextualEntrypointHeading(obj10);
            const tmpResult12 = tmp(11547);
          } else {
            const intl2 = tmp(1115).intl;
...
export const useQuestBarTitle = function useQuestBarTitle(questDockQuest, arg1) {
  ...
  if (null != completedAt) {
    ...
    if (_slicedToArray(obj3.useTaskPlatformScreen(questDockQuest, questTaskDetails), 1)[0] === tmp(5617).TaskPlatformScreen.SELECT) {
      const intl2 = tmp(1115).intl;
      ...
      isConsoleQuestResult = tmp(7920).isConsoleQuest(questDockQuest);
    }
    return stringResult;
  }
};
export const usePrimaryCtaCopy = function usePrimaryCtaCopy(arg0) {
      ...
      if (!tmp2Result.hasPlayActivityTask(tmp)) {
        ...
        if (tmp2Result4.hasStreamOnDesktopTask(obj2)) {
          ...
          if (tmp2Result5.hasWatchVideoOnMobileTasks(tmp)) {
            ...
            PLAY = tmp2(7920).isInGameQuest(tmp) ? tmp4.IN_GAME : tmp4.PLAY;
            const tmp2Result6 = tmp2(7920);
          }
          tmp2Result5 = tmp2(7920);
        }
        tmp2Result4 = tmp2(7920);
      }
      return PLAY;
    }
    PLAY = constants.PLAY;
  ...
  if (constants3.PLAY === memo) {
    ...
    return tmp2(11759).getVideoQuestWatchCtaText(tmp5);
...
export const getRewardCodeRedemptionInstructions = function getRewardCodeRedemptionInstructions(arg0) {
  ...
  if (platform == null) {
    platform = tmp(5617).QuestRewardCodePlatforms.CROSS_PLATFORM;
  }
  ...
  if (result) {
    ...
    obj2.idx = tier;
    rewardCodeQuestReward = tmp(11542).getRewardCodeQuestReward(obj2);
...
export const useModalCtaConfig = function useModalCtaConfig(quest) {
    ...
    if (dependencyMap === 2) {
      ...
      try {
        ...
        if (0 === c1) {
          ...
          const obj6 = { content: closure_128_1, ctaContent: tmp4(7924).QuestContentCTA.OPEN_GAME_LINK, impressionId: null, sourceQuestContent: null };
          let tmp5;
          ...
          tmp4(11742).openGameLinkDirectly(closure_128_0, obj6);
          dependencyMap = 3;
          return { value: "HermesInternal", done: null };
        }
      } catch (tmp11) {
  ...
  const memo = noop.useMemo(() => {
    ...
    if (null != subtitle) {
      ...
      if (obj3.hasAchievementInGameTask(messages)) {
        ...
        const questType = tmp5(7918).getQuestType(messages.config);
        if (questType === tmp5(7923).QuestType.GAMEPLAY) {
  ...
  obj.ctaText = quest(11547).getExternalCtaLabel(quest);
