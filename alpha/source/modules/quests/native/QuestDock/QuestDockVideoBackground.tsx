...
// Dependencies: [32, 19, 17, 4748, 5656, 15356, 1074, 21, 4756, 15357, 4492, 5185, 7313, 15360, 15443, 8537, 1478, 1612, 504, 15355, 672, 1364, 12018, 8577, 5799, 5198, 2]

// Module 15463 (QuestDockVideoBackground)
import _modDef672 from "module_672" /* 672 */;
import spring from "spring" /* 5185 */;
import ReanimatedNativeViewDefault from "ReanimatedNativeView" /* 7313 */;
import QuestDockUtils from "QuestDockUtils" /* 15355 */;
...
function QuestDockBackgroundMediaFade(arg0) {
  ...
  ({ children, style } = arg0);
  activeQuestDockMode = noop.useContext(activeQuestDockMode(15357).QuestDockGestureContext).activeQuestDockMode;
...
let QuestDockMode = fn(5656).QuestDockMode;
...
export default noop.memo(function QuestDockVideoBackground(gradientBaseColor) {
  ...
  dependencyMap = tmp2;
  let tmp3 = closure_17();
  const context = windowDimensions.useContext(gradientBaseColor(15357).QuestDockGestureContext);
  const activeQuestDockMode = context.activeQuestDockMode;
  windowDimensions = context.windowDimensions;
  const setRestingQuestDockMode = windowDimensions.useContext(gradientBaseColor(15360).QuestDockExternalCoordinationContext).setRestingQuestDockMode;
  const tmp8 = expandedHeight(8537)(activeQuestDockMode);
  ...
  fn.__closure = { withSpring: gradientBaseColor(5185).withSpring, activeQuestDockMode, QuestDockMode, QUEST_DOCK_UNENROLLED_HEADER_INSET_EXPANDED: first1, QUEST_DOCK_MODE_CHANGE_PHYSICS_CLAMPED: isRendered, windowDimensions };
  ...
  let obj4 = { withSpring: gradientBaseColor(5185).withSpring, activeQuestDockMode, QuestDockMode, QUEST_DOCK_UNENROLLED_HEADER_INSET_EXPANDED: first1, QUEST_DOCK_MODE_CHANGE_PHYSICS_CLAMPED: isRendered, windowDimensions };
  ...
  isRendered = !tmp22;
  if (!tmp22) {
    isRendered = windowDimensions.useContext(expandedHeight(15443)).isRendered;
  }
  if (isRendered) {
  ...
  if (isRendered) {
    isRendered = tmp4(12018).isHeroVideoSupported(gradientBaseColor.videoMimetype);
    const tmp4Result3 = tmp4(12018);
  }
  if (isRendered) {
    let tmp25 = !tmp2;
    if (tmp2) {
  ...
  te.__closure = { withSpring: gradientBaseColor(5185).withSpring, shouldShowVideo: isRendered, videoLoaded: first1, isMediaHiddenWhenCollapsed: collapsedMediaMode === windowDimensions.HIDDEN, activeQuestDockMode, QuestDockMode, QUEST_DOCK_MODE_CHANGE_PHYSICS_CLAMPED: tmp13 };
  ...
  if (isRendered) {
    ...
    tmp31 = closure_12(tmp4(8577).VideoComponent, obj6);
  }
  const items4 = [tmp31, ];
  let tmp33 = null;
  if (null != imageUrl) {
    ...
    obj9.source = obj10;
    obj8.children = closure_12(tmp7(5799), obj9);
    tmp33 = closure_12(tmp7(7313), obj8);
    const tmp7Result = tmp7(7313);
  }
  items4[1] = tmp33;
  ...
  obj11.style = items7;
  let tmp38 = null;
  const obj5 = { withSpring: gradientBaseColor(5185).withSpring, shouldShowVideo: isRendered, videoLoaded: first1, isMediaHiddenWhenCollapsed: collapsedMediaMode === obj.HIDDEN, activeQuestDockMode, QuestDockMode, QUEST_DOCK_MODE_CHANGE_PHYSICS_CLAMPED: tmp13 };
  ...
  return closure_13(expandedHeight(7313), obj11);
});
