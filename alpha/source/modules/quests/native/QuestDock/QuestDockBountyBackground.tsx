...
// Dependencies: [19, 15301, 21, 15308, 11537, 4423, 576, 672, 15408, 2]

// Module 15412 (QuestDockBountyBackground)
...
require = fn;
const expandedHeight = fn(15301).QUEST_DOCK_PORTRAIT_MEDIA_EXPANDED_HEIGHT;
const jsx = fn(21).jsx;
const size = fn(2);
...
export default noop.memo(function QuestDockBountyBackground(imageUrl) {
  let questDockBounty;
  let token;
  questDockBounty = questDockBounty(15308).useQuestDockBounty();
  ...
  let obj = questDockBounty(15308);
  token = questDockBounty(4423).useToken(token(576).colors.BACKGROUND_BRAND);
  const items1 = [token];
  ...
  const obj2 = questDockBounty(4423);
  obj3.collapsedMediaMode = questDockBounty(15408).QuestDockBackgroundCollapsedMediaMode.HIDDEN;
  obj3.gradientBaseColor = memo1;
  obj3.backdropColor = memo1;
  obj3.expandedHeight = expandedHeight;
  return jsx(token(15408), { imageUrl: imageUrl.previewImageUrl, videoUrl: questDockBounty.videoPreview, videoMimetype: memo, collapsedMediaMode: null, gradientBaseColor: null, backdropColor: null, expandedHeight: null });
});
