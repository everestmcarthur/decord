...
// Function ID: 15354
// Name: QuestDockHooks
// Dependencies: [32, 19, 15354, 5656, 15356, 504, 15357, 15360, 11614, 15358, 15361, 4492, 9663, 15355, 1091, 11739, 15363, 7963, 7964, 7974, 7962, 5659, 5663, 7952, 4723, 15364, 1980, 1478, 11546, 8114, 4457, 576, 2]
...
import QuestDockCreativeContext from "QuestDockCreativeContext" /* 15363 */;
import _slicedToArray from "module_32" /* 32 */;
import noop from "module_19" /* 19 */;
import QuestDockStore from "QuestDockStore" /* 15354 */;
  ...
  const fn2 = function t(safeAreaState, restingQuestDockMode) {
    if (!obj.cheapWorkletShallowEqual(safeAreaState, restingQuestDockMode)) {
      ...
      if (constants.RESET_TO_PREVIOUS === restingQuestDockMode) {
        ...
        tmp(4492).runOnJS(setRestingQuestDockMode)(restingQuestDockMode1);
      } else if (tmp5.COLLAPSED === restingQuestDockMode) {
        const obj2 = {};
        const merged = Object.assign(questDockWrapperSpecs.get());
        obj2.x = 0;
        obj2.y = 0;
        obj2.width = tmp(15355).getQuestDockCollapsedWidth(windowWidth, youBarHorizontalMargin, youBarHorizontalMargin);
        obj2.height = height;
        const result = questDockWrapperSpecs.set(obj2);
        const result1 = activeQuestDockMode.set(tmp5.COLLAPSED);
        const tmpResult6 = tmp(15355);
      } else {
        if (tmp5.CLOSED !== restingQuestDockMode) {
          if (tmp5.SOFT_DISMISSED !== restingQuestDockMode) {
            if (tmp5.EXPANDED === restingQuestDockMode) {
              const questDockExpandedHeightLimits = tmp(15355).getQuestDockExpandedHeightLimits(tmp4, safeArea.top, minExpandedContentHeight);
              ...
              if (!tmp7) {
                ...
                if (tmp6) {
                  tmp6 = obj11.get().height === tmp(15355).getQuestDockExpandedHeightLimits(restingQuestDockMode.windowHeight, restingQuestDockMode.safeArea.top, minExpandedContentHeight).maxHeight;
                  const tmpResult8 = tmp(15355);
                }
                tmp7 = tmp6;
              }
              if (!tmp7) {
              ...
              obj3.y = tmp10;
              const tmpResult7 = tmp(15355);
              obj3.width = tmp(15355).getQuestDockExpandedWidth(windowWidth, safeArea.left, safeArea.right);
        ...
        obj4.x = 0;
        obj4.y = 0;
        obj4.width = tmp(15355).getQuestDockClosedWidth(windowWidth, youBarHorizontalMargin, youBarHorizontalMargin);
        obj4.height = height;
        const result4 = questDockWrapperSpecs.set(obj4);
        const result5 = activeQuestDockMode.set(restingQuestDockMode);
        const tmpResult10 = tmp(15355);
      }
    }
  };
...
export const useQuestDockDismissalReset = function useQuestDockDismissalReset() {
  setRestingQuestDockMode = noop.useContext(setRestingQuestDockMode(15360).QuestDockExternalCoordinationContext).setRestingQuestDockMode;
  const activeQuestDockMode = noop.useContext(setRestingQuestDockMode(15357).QuestDockGestureContext).activeQuestDockMode;
  ...
  const effect = noop.useEffect(() => {
    ...
    if (!isSoftDismissedResult) {
      isSoftDismissedResult = setRestingQuestDockMode(15355).isSoftDismissed(QuestDockStore.questDockSoftDismissedAt);
      let obj = setRestingQuestDockMode(15355);
    }
    if (!isSoftDismissedResult) {
    ...
    function maybeResetSoftDismissal() {
      ...
      if (!isSoftDismissedResult) {
        isSoftDismissedResult = setRestingQuestDockMode(15355).isSoftDismissed(QuestDockStore.questDockSoftDismissedAt);
        const obj = setRestingQuestDockMode(15355);
      }
      if (!isSoftDismissedResult) {
...
export const useActionSheetPressHandler = function useActionSheetPressHandler(questCreative) {
  ...
  return noop.useCallback(() => {
    ...
    if (obj2.shouldMigrateToAdAnalyticsInterface(AdAnalyticsInterfaceExperiment.AdAnalyticsInterfaceExperimentStep.STEP_2_CLICKED_INTERNAL, "quest_dock_action_sheet")) {
      const obj3 = { type: tmp(7974).AdUserActionType.CLICK_INTERNAL };
      const merged = Object.assign(creativeAnalyticsParams);
      obj3.questContentCTA = tmp(7962).QuestContentCTA.OPEN_CONTEXT_MENU;
      obj3.surfaceId = tmp(5659).QuestContent.QUEST_BAR_MOBILE;
      obj3.sourceQuestContent = tmp(5659).QuestContent.QUEST_BAR_MOBILE;
      obj3.impressionId = questImpressionId;
      tmp(7964).captureAdUserAction(obj3);
      let tmp7 = questImpressionId;
      const tmpResult = tmp(7964);
    } else if (creativeAnalyticsParams.adCreativeType === tmp(5663).AdCreativeType.QUEST) {
      const obj4 = { questId: creativeAnalyticsParams.adCreativeId, questContent: tmp(5659).QuestContent.QUEST_BAR_MOBILE, questContentCTA: tmp(7962).QuestContentCTA.OPEN_CONTEXT_MENU, sourceQuestContent: tmp(5659).QuestContent.QUEST_BAR_MOBILE, impressionId: questImpressionId };
      ...
      obj5.questContentCTA = tmp(7962).QuestContentCTA.OPEN_CONTEXT_MENU;
    ...
    ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(15364, tmp2.paths), "QuestDockContextMenuActionSheet", { creative: tmp3, impressionId: tmp7 });
  }, items);
};
