...
// Function ID: 15244
// Name: AdVideoPlayer
// Dependencies: [32, 19, 17, 1896, 1074, 21, 576, 8540, 4722, 672, 15232, 4458, 5143, 5147, 4723, 4726, 1110, 504, 1364, 1231, 1612, 1115, 5292, 15244, 15246, 15248, 15249, 10420, 8506, 8508, 15250, 15251, 2]

// Module 15243 (AdVideoPlayer)
...
const _isNativeReflectConstruct = noop.memo(_modDef8540);
...
export const AdVideoPlayer = noop.memo((initialProgress) => {
  ...
  callback6 = obj.useCallback(() => {
    ...
    if (first2 !== AdsVideoTypes.PlayerState.ERRORED) {
      if (tmp !== tmp2(15232).PlayerState.PAUSED) {
        if (tmp !== tmp2(15232).PlayerState.LOADING) {
          if (tmp === tmp2(15232).PlayerState.PLAYING) {
            callback(tmp2(15232).PlayerState.PAUSED);
            if (onPausePlayback != null) {
              tmp16(tmp2(15232).PlaybackTriggerSource.USER_INTERACTION);
            }
          }
        }
      }
      callback(tmp2(15232).PlayerState.PLAYING);
      if (onResumePlayback != null) {
        tmp6(tmp2(15232).PlaybackTriggerSource.USER_INTERACTION);
      }
    }
    callback4(0);
    callback(AdsVideoTypes.PlayerState.PLAYING);
    if (onResumePlayback != null) {
      tmp11(tmp2(15232).PlaybackTriggerSource.USER_INTERACTION);
    }
  }, items6);
  ...
  callback8 = obj.useCallback(() => {
    ...
    if (first2 === AdsVideoTypes.PlayerState.PLAYING) {
      ...
      callback(tmp2(15232).PlayerState.PAUSED);
      if (onPausePlayback != null) {
        tmp6(tmp2(15232).PlaybackTriggerSource.SYSTEM_INITIATED);
      }
    }
  }, items8);
  ...
  callback9 = obj.useCallback(() => {
    if (ref5.current) {
      tmp.current = false;
      if (first2 !== AdsVideoTypes.PlayerState.ERRORED) {
        callback(tmp3(15232).PlayerState.PLAYING);
        if (onResumePlayback != null) {
          tmp7(tmp3(15232).PlaybackTriggerSource.SYSTEM_INITIATED);
        }
      }
    }
  }, items9);
