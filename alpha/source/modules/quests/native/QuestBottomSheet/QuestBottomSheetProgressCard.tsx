...
// Dependencies: [19, 17, 1372, 21, 4756, 576, 11736, 11749, 504, 11551, 7956, 1115, 4752, 5664, 5819, 15392, 11546, 15384, 12018, 15379, 5334, 8577, 5198, 5799, 15422, 8544, 2]
...
// Module 15421 (QuestBottomSheetProgressCard)
...
import QuestProgressIndicatorDefault from "QuestProgressIndicator" /* 15392 */;
...
export const QuestBottomSheetProgressCardPlayStreamTask = function QuestBottomSheetProgressCardPlayStreamTask(quest) {
  ...
  c8 = undefined;
  const tmp = closure_9();
  questTaskDetails = questTaskDetails(11736).useQuestTaskDetails(quest);
  let obj = questTaskDetails(11736);
  let isQuestProgressing = questTaskDetails(11736).useIsQuestProgressing(quest);
  ...
  noop = tmp9;
  let obj2 = questTaskDetails(11736);
  const result = questTaskDetails(11749).supportedTaskPlatforms(quest);
  c4 = result;
  const tmp2Result = questTaskDetails(11749);
  questFormattedDate = questTaskDetails(11736).useQuestFormattedDate(quest.config.rewardsConfig.rewardsExpireAt);
  gameTitle = quest.config.messages.gameTitle;
  const tmp2Result5 = questTaskDetails(11736);
  ...
  defaultRewardName = questTaskDetails(11551).getDefaultRewardName(quest.config, stateFromStores);
  const tmp2Result7 = questTaskDetails(11551);
  const isSponsoredPlayQuestResult = questTaskDetails(7956).isSponsoredPlayQuest(quest);
  ...
  const memo = noop.useMemo(() => {
    if (dependencyMap) {
      if (!closure_3) {
        ...
        const obj = {
          rewardHook() {
                return defaultRewardName(questTaskDetails(4752).Text, { variant: "text-sm/semibold", color: "text-strong", children });
              },
    ...
    if (isQuestProgressing) {
      ...
      const obj2 = {
        ...
        minutesHook(children) {
            return children(questTaskDetails(4752).Text, { variant: "text-sm/semibold", color: "text-strong", children });
          }
      };
  ...
  if (isQuestProgressing) {
    ...
    obj7.children = tmp18(tmp2(4752).Text, obj8);
    isQuestProgressing = tmp18(tmp17, obj7);
  }
  items3[1] = isQuestProgressing;
  obj3.children = items3;
  return c8(questTaskDetails(5819).Card, obj3);
};
export const QuestBottomSheetProgressCardWatchTask = function QuestBottomSheetProgressCardWatchTask(quest) {
  ...
  const obj = quest(11736);
  let isHeroVideoSupportedResult = null != memo;
  const obj2 = quest(15384);
  const obj3 = { questId: quest.id, sourceQuestContent: quest.sourceQuestContent };
  if (isHeroVideoSupportedResult) {
    isHeroVideoSupportedResult = tmp2(12018).isHeroVideoSupported(memo.mimetype);
    const tmp2Result = tmp2(12018);
  }
  const userStatus = quest.userStatus;
  let completedAt;
  if (userStatus != null) {
  ...
  const watchTaskPressHandler = quest(15384).useWatchTaskPressHandler({ questId: quest.id, sourceQuestContent: quest.sourceQuestContent });
  ...
  if (isHeroVideoSupportedResult) {
    ...
    obj7.source = obj8;
    const items4 = [tmp14(tmp2(8577).VideoComponent, obj7), ];
  ...
  tmp2Result2 = quest(11736);
  items7[1] = closure_7(quest(8544).PlayIcon, { color: nativeDefault.colors.WHITE });
  ...
  obj4.children = closure_8(quest(5819).Card, obj5);
  return closure_7(quest(5334).PressableOpacity, obj4);
};
