...
// Dependencies: [19, 17, 1372, 21, 4722, 576, 11729, 11742, 504, 11542, 7918, 1115, 4718, 5622, 5777, 15337, 11537, 15329, 12009, 15324, 5292, 8539, 5156, 5757, 15367, 8506, 2]
...
// Module 15366 (QuestBottomSheetProgressCard)
...
import QuestProgressIndicatorDefault from "QuestProgressIndicator" /* 15337 */;
import QuestDockBlurredContentBackgroundDefault from "QuestDockBlurredContentBackground" /* 15367 */;
...
export const QuestBottomSheetProgressCardPlayStreamTask = function QuestBottomSheetProgressCardPlayStreamTask(quest) {
  ...
  c8 = undefined;
  const tmp = closure_9();
  questTaskDetails = questTaskDetails(11729).useQuestTaskDetails(quest);
  let obj = questTaskDetails(11729);
  let isQuestProgressing = questTaskDetails(11729).useIsQuestProgressing(quest);
  ...
  noop = tmp9;
  let obj2 = questTaskDetails(11729);
  const result = questTaskDetails(11742).supportedTaskPlatforms(quest);
  c4 = result;
  const tmp2Result = questTaskDetails(11742);
  questFormattedDate = questTaskDetails(11729).useQuestFormattedDate(quest.config.rewardsConfig.rewardsExpireAt);
  gameTitle = quest.config.messages.gameTitle;
  const tmp2Result5 = questTaskDetails(11729);
  ...
  defaultRewardName = questTaskDetails(11542).getDefaultRewardName(quest.config, stateFromStores);
  const tmp2Result7 = questTaskDetails(11542);
  const isSponsoredPlayQuestResult = questTaskDetails(7918).isSponsoredPlayQuest(quest);
  ...
  const memo = noop.useMemo(() => {
    if (dependencyMap) {
      if (!closure_3) {
        ...
        const obj = {
          rewardHook() {
                return defaultRewardName(questTaskDetails(4718).Text, { variant: "text-sm/semibold", color: "text-strong", children });
              },
    ...
    if (isQuestProgressing) {
      ...
      const obj2 = {
        ...
        minutesHook(children) {
            return children(questTaskDetails(4718).Text, { variant: "text-sm/semibold", color: "text-strong", children });
          }
      };
  ...
  if (isQuestProgressing) {
    ...
    obj7.children = tmp18(tmp2(4718).Text, obj8);
    isQuestProgressing = tmp18(tmp17, obj7);
  }
  items3[1] = isQuestProgressing;
  obj3.children = items3;
  return c8(questTaskDetails(5777).Card, obj3);
};
export const QuestBottomSheetProgressCardWatchTask = function QuestBottomSheetProgressCardWatchTask(quest) {
  ...
  const obj = quest(11729);
  let isHeroVideoSupportedResult = null != memo;
  const obj2 = quest(15329);
  const obj3 = { questId: quest.id, sourceQuestContent: quest.sourceQuestContent };
  if (isHeroVideoSupportedResult) {
    isHeroVideoSupportedResult = tmp2(12009).isHeroVideoSupported(memo.mimetype);
    const tmp2Result = tmp2(12009);
  }
  const userStatus = quest.userStatus;
  let completedAt;
  if (userStatus != null) {
  ...
  const watchTaskPressHandler = quest(15329).useWatchTaskPressHandler({ questId: quest.id, sourceQuestContent: quest.sourceQuestContent });
  ...
  if (isHeroVideoSupportedResult) {
    ...
    obj7.source = obj8;
    const items4 = [tmp14(tmp2(8539).VideoComponent, obj7), ];
  ...
  tmp2Result2 = quest(11729);
  items7[1] = closure_7(quest(8506).PlayIcon, { color: nativeDefault.colors.WHITE });
  ...
  obj4.children = closure_8(quest(5777).Card, obj5);
  return closure_7(quest(5292).PressableOpacity, obj4);
};
