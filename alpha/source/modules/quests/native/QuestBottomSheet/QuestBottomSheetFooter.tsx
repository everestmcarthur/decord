...
// Dependencies: [32, 19, 17, 4714, 1372, 7899, 7347, 21, 576, 4722, 5617, 11729, 11542, 11547, 11296, 504, 4423, 7748, 12009, 15329, 5144, 11759, 11742, 15297, 12017, 15324, 15326, 8140, 1115, 5798, 12016, 11732, 15364, 7925, 7926, 7936, 5621, 7924, 1612, 1478, 4458, 4723, 5149, 2]
// Exports: default

// Module 15328 (QuestBottomSheetFooter)
...
import ContentImpressionTrackerHooks from "ContentImpressionTrackerHooks" /* 11732 */;
...
import AnalyticsHooks from "AnalyticsHooks" /* 12016 */;
import QuestBottomSheetHooks from "QuestBottomSheetHooks" /* 15329 */;
import RefreshIcon from "RefreshIcon" /* 15364 */;
  ...
  let obj3 = {
    ...
    onPress(arg0) {
      if (obj.shouldMigrateToAdAnalyticsInterface(AdAnalyticsInterfaceExperiment.AdAnalyticsInterfaceExperimentStep.STEP_2_CLICKED_INTERNAL, "quest_bottom_sheet_footer")) {
        const obj2 = { type: tmp(7936).AdUserActionType.CLICK_INTERNAL, adCreativeType: tmp(5621).AdCreativeType.QUEST, adCreativeId: questId, questContentCTA: tmp(7924).QuestContentCTA.DEFIBRILLATOR, surfaceId: tmp(5617).QuestContent.QUEST_BOTTOM_SHEET, sourceQuestContent, impressionId };
        tmp(7926).captureAdUserAction(obj2);
        const tmpResult = tmp(7926);
      } else {
        const obj3 = { questId, questContent: tmp(5617).QuestContent.QUEST_BOTTOM_SHEET, questContentCTA: tmp(7924).QuestContentCTA.DEFIBRILLATOR, sourceQuestContent };
        closure_3(obj3);
      }
...
function ClaimButton(arg0) {
  ...
  let obj3 = {
    ...
    onPress() {
      if (obj.shouldMigrateToAdAnalyticsInterface(AdAnalyticsInterfaceExperiment.AdAnalyticsInterfaceExperimentStep.STEP_2_CLICKED_INTERNAL, "quest_bottom_sheet_footer")) {
        const obj2 = { type: tmp(7936).AdUserActionType.CLICK_INTERNAL, adCreativeType: tmp(5621).AdCreativeType.QUEST, adCreativeId: questId, questContentCTA: tmp(7924).QuestContentCTA.CLAIM_REWARD, surfaceId: tmp(5617).QuestContent.QUEST_BOTTOM_SHEET, sourceQuestContent, impressionId };
        tmp(7926).captureAdUserAction(obj2);
        const tmpResult = tmp(7926);
      } else {
        const obj3 = { questId, questContent: tmp(5617).QuestContent.QUEST_BOTTOM_SHEET, questContentCTA: tmp(7924).QuestContentCTA.CLAIM_REWARD, sourceQuestContent };
        closure_3(obj3);
      }
      importDefault();
    },
...
let closure_9 = fn(7347).ACTION_SHEET_MINIMUM_BOTTOM_PADDING;
const jsxProd = fn(21);
...
export default function QuestBottomSheetFooter(quest) {
  ...
  const obj2 = quest(11729);
  const obj3 = quest(11729);
  ...
  const obj5 = quest(15297);
  const mobileActivityQuest = quest(15297).useMobileActivityQuest(quest);
  ({ isMobileActivityQuest, launchMobileActivity, questApplication } = mobileActivityQuest);
  const obj6 = quest(15297);
  const primaryCtaCopy = quest(12017).usePrimaryCtaCopy({ quest, application: questApplication });
  const obj7 = quest(12017);
  const userStatus = quest.userStatus;
  let completedAt;
  const obj8 = quest(15329);
  ...
  const mobileActivityPressHandler = quest(15329).useMobileActivityPressHandler({ questId: quest.id, sourceQuestContent, launchMobileActivity });
  const tmp11 = null != completedAt;
  const isQuestAccessSuspended = quest(11729).useIsQuestAccessSuspended();
  const obj10 = { disabled: true, onPressDisabled: xboxAndPlaystationAccounts(15324) };
  let tmp40Result6 = null;
  if (step !== quest(15326).QuestBottomSheetStep.TASK_SELECT) {
    const obj11 = { onLayout: quest.onLayout, ctaButton: null, backButton: null, style: null, withSafeArea: null };
    if (tmp2(15326).QuestBottomSheetStep.CONSOLE_CONNECT === step) {
      const obj12 = { onPress: onConnectConsoleNext, disabled: 0 === memo.length };
      let tmp40Result = tmp40(NextButton, obj12);
    } else {
      tmp40Result = null;
      if (tmp2(15326).QuestBottomSheetStep.TASK_STATUS === step) {
        if (tmp11) {
          ...
          const obj15 = { grow: true, size: "lg", onPress: mobileActivityPressHandler, text: primaryCtaCopy, icon: tmp2(12009).getPrimaryCtaIcon(quest) };
          let tmp23 = null;
          if (isQuestAccessSuspended) {
            tmp23 = obj10;
          }
          const merged2 = Object.assign(tmp23);
          tmp40Result4 = tmp40(tmp2(5144).Button, obj15);
          const tmp2Result2 = tmp2(12009);
        } else {
          if (_slicedToArray(obj3.useTaskPlatformScreen(quest, questTaskDetails), 1)[0] === tmp2(5617).TaskPlatformScreen.CONSOLE) {
    ...
    if (tmp40Result5) {
      ...
      obj18.icon = tmp40(tmp2(5798).ArrowLargeLeftIcon, {});
      obj18.onPress = onBack;
      tmp40Result5 = tmp40(tmp2(8140).IconButton, obj18);
    }
