...
// Dependencies: [32, 19, 17, 4748, 1372, 7937, 7391, 21, 576, 4756, 5659, 11736, 11551, 11556, 11309, 504, 4457, 7792, 12018, 15384, 5186, 11766, 11749, 15352, 12026, 15379, 15381, 8178, 1115, 5840, 12025, 11739, 15419, 7963, 7964, 7974, 5663, 7962, 1612, 1478, 4492, 4757, 5191, 2]
// Exports: default

// Module 15383 (QuestBottomSheetFooter)
...
import ContentImpressionTrackerHooks from "ContentImpressionTrackerHooks" /* 11739 */;
...
import AnalyticsHooks from "AnalyticsHooks" /* 12025 */;
import QuestBottomSheetHooks from "QuestBottomSheetHooks" /* 15384 */;
import RefreshIcon from "RefreshIcon" /* 15419 */;
  ...
  let obj3 = {
    ...
    onPress(arg0) {
      if (obj.shouldMigrateToAdAnalyticsInterface(AdAnalyticsInterfaceExperiment.AdAnalyticsInterfaceExperimentStep.STEP_2_CLICKED_INTERNAL, "quest_bottom_sheet_footer")) {
        const obj2 = { type: tmp(7974).AdUserActionType.CLICK_INTERNAL, adCreativeType: tmp(5663).AdCreativeType.QUEST, adCreativeId: questId, questContentCTA: tmp(7962).QuestContentCTA.DEFIBRILLATOR, surfaceId: tmp(5659).QuestContent.QUEST_BOTTOM_SHEET, sourceQuestContent, impressionId };
        tmp(7964).captureAdUserAction(obj2);
        const tmpResult = tmp(7964);
      } else {
        const obj3 = { questId, questContent: tmp(5659).QuestContent.QUEST_BOTTOM_SHEET, questContentCTA: tmp(7962).QuestContentCTA.DEFIBRILLATOR, sourceQuestContent };
        closure_3(obj3);
      }
...
function ClaimButton(arg0) {
  ...
  let obj3 = {
    ...
    onPress() {
      if (obj.shouldMigrateToAdAnalyticsInterface(AdAnalyticsInterfaceExperiment.AdAnalyticsInterfaceExperimentStep.STEP_2_CLICKED_INTERNAL, "quest_bottom_sheet_footer")) {
        const obj2 = { type: tmp(7974).AdUserActionType.CLICK_INTERNAL, adCreativeType: tmp(5663).AdCreativeType.QUEST, adCreativeId: questId, questContentCTA: tmp(7962).QuestContentCTA.CLAIM_REWARD, surfaceId: tmp(5659).QuestContent.QUEST_BOTTOM_SHEET, sourceQuestContent, impressionId };
        tmp(7964).captureAdUserAction(obj2);
        const tmpResult = tmp(7964);
      } else {
        const obj3 = { questId, questContent: tmp(5659).QuestContent.QUEST_BOTTOM_SHEET, questContentCTA: tmp(7962).QuestContentCTA.CLAIM_REWARD, sourceQuestContent };
        closure_3(obj3);
      }
      importDefault();
    },
...
let closure_9 = fn(7391).ACTION_SHEET_MINIMUM_BOTTOM_PADDING;
const jsxProd = fn(21);
...
export default function QuestBottomSheetFooter(quest) {
  ...
  const obj5 = quest(15352);
  const mobileActivityQuest = quest(15352).useMobileActivityQuest(quest);
  ({ isMobileActivityQuest, launchMobileActivity, questApplication } = mobileActivityQuest);
  const obj6 = quest(15352);
  const primaryCtaCopy = quest(12026).usePrimaryCtaCopy({ quest, application: questApplication });
  const obj7 = quest(12026);
  const userStatus = quest.userStatus;
  let completedAt;
  const obj8 = quest(15384);
  ...
  const mobileActivityPressHandler = quest(15384).useMobileActivityPressHandler({ questId: quest.id, sourceQuestContent, launchMobileActivity });
  const tmp11 = null != completedAt;
  const isQuestAccessSuspended = quest(11736).useIsQuestAccessSuspended();
  const obj10 = { disabled: true, onPressDisabled: xboxAndPlaystationAccounts(15379) };
  let tmp40Result6 = null;
  if (step !== quest(15381).QuestBottomSheetStep.TASK_SELECT) {
    const obj11 = { onLayout: quest.onLayout, ctaButton: null, backButton: null, style: null, withSafeArea: null };
    if (tmp2(15381).QuestBottomSheetStep.CONSOLE_CONNECT === step) {
      const obj12 = { onPress: onConnectConsoleNext, disabled: 0 === memo.length };
      let tmp40Result = tmp40(NextButton, obj12);
    } else {
      tmp40Result = null;
      if (tmp2(15381).QuestBottomSheetStep.TASK_STATUS === step) {
        if (tmp11) {
          ...
          const obj15 = { grow: true, size: "lg", onPress: mobileActivityPressHandler, text: primaryCtaCopy, icon: tmp2(12018).getPrimaryCtaIcon(quest) };
          let tmp23 = null;
          if (isQuestAccessSuspended) {
            tmp23 = obj10;
          }
          const merged2 = Object.assign(tmp23);
          tmp40Result4 = tmp40(tmp2(5186).Button, obj15);
          const tmp2Result2 = tmp2(12018);
        } else {
          if (_slicedToArray(obj3.useTaskPlatformScreen(quest, questTaskDetails), 1)[0] === tmp2(5659).TaskPlatformScreen.CONSOLE) {
    ...
    if (tmp40Result5) {
      ...
      obj18.icon = tmp40(tmp2(5840).ArrowLargeLeftIcon, {});
      obj18.onPress = onBack;
      tmp40Result5 = tmp40(tmp2(8178).IconButton, obj18);
    }
