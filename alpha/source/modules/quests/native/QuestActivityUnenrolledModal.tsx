...
// Dependencies: [5, 32, 19, 17, 7899, 17407, 5614, 21, 4722, 576, 1365, 4656, 4430, 504, 7364, 7920, 11729, 12017, 5617, 1397, 11528, 7924, 4925, 4440, 12016, 15324, 5142, 5757, 12012, 4718, 1115, 5144, 5787, 7573, 7188, 12020, 11451, 2]
// Exports: default

// Module 17408 (QuestActivityUnenrolledModal)
...
import openQuestAccessSuspendedBottomSheetDefault from "openQuestAccessSuspendedBottomSheet" /* 15324 */;
import asyncGeneratorStep from "asyncGeneratorStep" /* 5 */;
import _slicedToArray from "module_32" /* 32 */;
import noop from "module_19" /* 19 */;
import QuestStore from "QuestStore" /* 7899 */;
import UnenrolledActivityQuestStore from "UnenrolledActivityQuestStore" /* 17407 */;
...
function QuestActivityUnenrolledModalInner(quest) {
  ...
  const theme = quest(4656).useTheme();
  let obj = quest(4656);
  const tmp4 = closure_14(quest(4430).isThemeDark(theme));
  let obj2 = quest(4430);
  ...
  let obj3 = quest(504);
  let obj4 = quest(7364);
  const getOrFetchApplication = obj4.useGetOrFetchApplication(quest(7920).getActivityApplicationId(quest));
  const obj5 = quest(7920);
  const questTaskDetails = quest(11729).useQuestTaskDetails(quest);
  let obj6 = quest(11729);
  const obj7 = quest(12017);
  const items1 = [getOrFetchApplication];
  const questsInstructionsToWinReward = obj7.useQuestsInstructionsToWinReward({ quest, taskDetails: questTaskDetails, location: QuestsExperimentLocations.QUEST_ACTIVITY_UNENROLLED_MODAL, sourceQuestContent: quest(5617).QuestContent.QUEST_ACTIVITY_UNENROLLED_MODAL });
      ...
      try {
        ...
        if (0 === v1) {
          if (arg0 === 1) {
            ...
            const obj4 = { questContent: tmp4(5617).QuestContent.QUEST_ACTIVITY_UNENROLLED_MODAL, questContentCTA: tmp4(7924).QuestContentCTA.START_QUEST, sourceQuestContent: tmp4(5617).QuestContent.QUEST_ACTIVITY_UNENROLLED_MODAL };
            v1 = 1;
            dependencyMap = 1;
            const obj6 = { value: tmp4(11528).enrollInQuest(quest.id, obj4), done: false };
            return obj6;
          }
        } else if (arg0 === 1) {
          dependencyMap = 3;
  ...
  const obj8 = { quest, taskDetails: questTaskDetails, location: QuestsExperimentLocations.QUEST_ACTIVITY_UNENROLLED_MODAL, sourceQuestContent: quest(5617).QuestContent.QUEST_ACTIVITY_UNENROLLED_MODAL };
  const checkboxA11yNative = quest(4440).useCheckboxA11yNative({ checked });
  ({ accessibilityRole, accessibilityState } = checkboxA11yNative);
  const obj9 = quest(4440);
  const isQuestAccessSuspended = quest(11729).useIsQuestAccessSuspended();
  const obj10 = quest(11729);
  trackQuestContentClickedWithImpression = quest(12016).useTrackQuestContentClickedWithImpression();
  ...
  if (tmp18Result) {
    ...
    obj16.children = tmp18(checked(5757), obj17);
  ...
  obj19.children = closure_11(checked(12012), size);
  ...
  const items6 = [closure_11(quest(4718).Text, obj21), ];
  ...
  items6[1] = closure_12(quest(4718).Text, obj22);
  ...
  const items9 = [closure_11(quest(5144).Button, obj26), ];
  ...
  items9[1] = closure_11(quest(5144).Button, obj27);
  ...
  if (null == stateFromStores) {
    ...
    const obj3 = {
      ...
      headerTitle() {
          ...
          return closure_1_11(questId(4718).Text, obj);
        },
      render() {
    ...
    return closure_11(tmp(11451).Modal, obj4);
  }
  let obj = questId(504);
  tmp = questId;
};
