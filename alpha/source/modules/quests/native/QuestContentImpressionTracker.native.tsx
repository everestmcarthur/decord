...
// Dependencies: [32, 19, 1979, 7968, 1074, 21, 8996, 8542, 504, 11739, 11740, 5663, 2]
// Exports: BillableAdPlacementImpressionTrackerNative, QuestContentImpressionTrackerNative

// Module 12029 (QuestContentImpressionTracker)
...
function AdContentImpressionTrackerBaseNative(skipRemountKey) {
  ...
  let tmp6 = visibilityRef(8542)(tmp3);
  const adContentImpressionTrackerProps = adContentIds2(11739).useAdContentImpressionTrackerProps(skipRemountKey);
  ...
  const obj2 = adContentIds2(11739);
  let items1 = [joined];
  const tmp10 = _slicedToArray(adContentIds2(8996).useRecyclingState(false, items1), 2);
  dependencyMap = tmp11;
  ...
  const obj4 = adContentIds2(8996);
  const obj6 = {};
  const merged1 = Object.assign(skipRemountKey);
  const merged2 = Object.assign({ visible: overrideVisibility, visibleChanged: overrideVisibility !== tmp5(8542)(overrideVisibility) });
  obj6.focused = tmp3;
  ...
  if (tmp17) {
    obj7.adCreativeType = tmp(5663).AdCreativeType.QUEST;
    let tmp19 = obj7;
  } else {
    obj7.adCreativeType = skipRemountKey.adCreativeType;
    tmp19 = obj7;
  }
  return createElement(adContentIds2(11740).QuestContentImpressionTracker, tmp19);
}
let closure_6 = fn(7968).MIN_QUEST_CONTENT_VISIBILITY_PERCENTAGE;
