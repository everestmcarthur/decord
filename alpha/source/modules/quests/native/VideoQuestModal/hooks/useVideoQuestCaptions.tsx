...
// Dependencies: [32, 19, 11546, 1271, 15407, 2]
// Exports: useVideoQuestCaptions

// Module 15406 (useVideoQuestCaptions)
...
export const useVideoQuestCaptions = (quest) => {
  const questAsset = url(11546).getQuestAsset(quest, url(11546).QuestAssetType.VIDEO_PLAYER_CAPTION, undefined, true);
  url = undefined;
  if (questAsset != null) {
    url = questAsset.url;
  }
  ...
  const effect = noop.useEffect(() => {
    if (null != url) {
      ...
      value.then((text) => {
        try {
          closure_1_2(url(15407).parseVtt(text.text).cues);
