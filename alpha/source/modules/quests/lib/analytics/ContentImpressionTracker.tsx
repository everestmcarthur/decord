...
// Dependencies: [5, 19, 7937, 7968, 1074, 21, 1255, 7963, 7933, 5663, 11741, 11744, 11745, 11746, 7969, 7962, 1364, 7911, 7966, 11540, 7974, 7964, 7943, 7952, 7965, 5084, 5089, 11739, 504, 5203, 2]
// Exports: QuestContentImpressionTracker

// Module 11740 (ContentImpressionTracker)
...
import IosAttributionEligibility from "IosAttributionEligibility" /* 11741 */;
import IosAttributionImpressionRegistry from "IosAttributionImpressionRegistry" /* 11746 */;
...
require = fn;
const ContentImpressionTrackerConstants = fn(7968);
    ...
    obj1.maybeRegisterIosAttributionImpression = function maybeRegisterIosAttributionImpression(item, adProvenanceMetadataSealed) {
      if (!obj5.iosAttributionRegistered) {
        if (obj.isIosAttributionEligible()) {
          const activeIosAttributionFramework = tmp2(11744).getActiveIosAttributionFramework();
          if (null != activeIosAttributionFramework) {
            if (null != adProvenanceMetadataSealed) {
              if (tmp2Result6.isCampaignIosAttributionEnabled(tmp.sourceQuestContent, item)) {
                ...
                const result = tmp2(11746).registerViewThroughImpression(obj2);
                tmp.iosAttributionRegistered = true;
                const tmp2Result7 = tmp2(11746);
              } else {
                const result1 = tmp2(11745).trackIosAttributionImpression(tmp2(11745).IosAttributionImpressionResult.NOT_SKAN_ENABLED, activeIosAttributionFramework, tmp.id);
                const tmp2Result8 = tmp2(11745);
              }
              tmp2Result6 = tmp2(11741);
            } else {
              const result2 = tmp2(11745).trackIosAttributionImpression(tmp2(11745).IosAttributionImpressionResult.NO_METADATA, activeIosAttributionFramework, tmp.id);
              const tmp2Result9 = tmp2(11745);
            }
          } else {
            const result3 = tmp2(11745).trackIosAttributionImpression(tmp2(11745).IosAttributionImpressionResult.NO_FRAMEWORK, activeIosAttributionFramework, tmp.id);
            const tmp2Result10 = tmp2(11745);
          }
          const tmp2Result = tmp2(11744);
        }
        ...
        try {
          ...
          if (0 === dependencyMap) {
            if (arg0 === 1) {
              ...
              dependencyMap = 1;
              c3 = 1;
              let obj6 = { value: obj10.getAdUser(tmp2(7962).getQuestContentName(obj5.questContent)), done: false };
              return obj6;
            }
          } else if (arg0 === 1) {
            c3 = 3;
            ...
            let merged = Object.assign(tmp5(7911)());
            let merged1 = Object.assign(tmp2(7966).getBrandSafetyContext(closure_129_0.questContent));
            closure_128_2 = obj9;
            const adContentIds = closure_129_0.entity.adContentIds;
            const item = adContentIds.forEach((item, index) => {
              let adMetadataSealed = obj5(7933).getAdMetadataSealed(adUser.sourceQuestContent, item);
              ...
              const QUEST = obj5(5663).AdCreativeType.QUEST;
              if (adUser.migrateQuestContentViewedToCaptureAdUserAction) {
                if (adCreativeType === QUEST) {
                  ...
                  if (!isQuestExpiredResult) {
                    isQuestExpiredResult = tmp(7933).isQuestExpired(quest);
                    const tmpResult = tmp(7933);
                  }
                  if (!isQuestExpiredResult) {
                    const items = [tmp30];
                    tmp(11540).markAdContentSeen(tmp(5663).AdCreativeType.QUEST, items);
                    const tmpResult18 = tmp(11540);
                  }
                }
                const result = tmp(7933).isBillableQuestContent(obj2.questContent, obj2.entity.adCreativeType);
                const AdUserActionType = tmp(7974).AdUserActionType;
                ...
                if (obj2.entity.adCreativeType === tmp(5663).AdCreativeType.QUEST) {
                  const obj4 = {};
                  const merged = Object.assign(obj3);
                  obj4.adCreativeType = obj2.entity.adCreativeType;
                  obj4.adCreativeId = obj2.entity.adContentIds[index];
                  tmp(7964).captureAdUserAction(obj4);
                  const tmpResult20 = tmp(7964);
                  ...
                  tmp(7964).captureAdUserAction(obj5);
                  const tmpResult21 = tmp(7964);
                } else {
                  const obj6 = {};
                  const merged2 = Object.assign(obj3);
                  obj6.adCreativeType = obj2.entity.adCreativeType;
                  obj6.adCreativeId = obj2.entity.adContentIds[index];
                  tmp(7964).captureAdUserAction(obj6);
                  const tmpResult22 = tmp(7964);
                }
                const tmpResult19 = tmp(7933);
                const questLogger = tmp(7943).getQuestLogger();
                const tmpResult23 = tmp(7943);
                const _HermesInternal2 = HermesInternal;
                const obj7 = { impressionId: obj2.id };
                questLogger.info("" + item + " ad content viewed for at least " + obj2.minViewTimeSeconds + "s at " + tmp(7962).getQuestContentName(obj2.questContent), obj7);
                ...
                if (!isQuestExpiredResult1) {
                  isQuestExpiredResult1 = tmp(7933).isQuestExpired(quest1);
                  const tmpResult25 = tmp(7933);
                }
                if (!isQuestExpiredResult1) {
                  const items1 = [tmp6];
                  tmp(11540).markAdContentSeen(tmp(5663).AdCreativeType.QUEST, items1);
                  const tmpResult26 = tmp(11540);
                }
                const questLogger1 = tmp(7943).getQuestLogger();
                let questName;
                if (quest1 != null) {
                ...
                const tmpResult27 = tmp(7943);
                const _HermesInternal = HermesInternal;
                const obj8 = { impressionId: obj2.id };
                questLogger1.info("" + questName + " Quest viewed for at least " + obj2.minViewTimeSeconds + "s at " + tmp(7962).getQuestContentName(obj2.questContent), obj8);
                const tmpResult28 = tmp(7962);
                const obj9 = {};
                ...
                const tmpResult29 = tmp(7952);
                const currentQuestHomeSearchSession = tmp(7965).getCurrentQuestHomeSearchSession();
                let uuid;
                ...
                const tmpResult30 = tmp(7965);
                let id;
                if (quest1 != null) {
                  id = quest1.id;
                }
                let adTrafficMetadataSealed = tmp(7933).getAdTrafficMetadataSealed(obj2.sourceQuestContent, id);
                ...
                const tmpResult31 = tmp(7933);
              } else {
                const questLogger2 = tmp(7943).getQuestLogger();
                const tmpResult32 = tmp(7943);
                const _HermesInternal3 = HermesInternal;
                const obj11 = { impressionId: obj2.id };
                questLogger2.info("" + obj2.entity.adContentIds[index] + " ad content viewed for at least " + obj2.minViewTimeSeconds + "s at " + tmp(7962).getQuestContentName(obj2.questContent), obj11);
                const tmpResult33 = tmp(7962);
                const obj12 = {};
                ...
                tmp(7952).trackAdContentEvent(obj12);
                const tmpResult34 = tmp(7952);
              }
              const obj = obj5(7933);
            });
      ...
      if (null != flag.lastBeatTime) {
        ...
        const item = adContentIds.forEach((item, index) => {
          if (obj5.entity.adCreativeType === AdCreativeType.AdCreativeType.QUEST) {
            ...
            tmp(7952).trackAdContentEvent(obj6);
            const tmpResult6 = tmp(7952);
          }
        });
      }
    ...
    obj1.start = function start() {
      ...
      const item = adContentIds.forEach((item, index) => {
        ...
        if (obj5.migrateQuestContentLoadedToCaptureAdUserAction) {
          ...
          tmpResult = tmp(7964);
          obj5 = { type: tmp(7974).AdUserActionType.END_CONTENT_LOAD, surfaceId: null, sourceQuestContent: null, impressionId: null, triggeredByStatusChange: null, trackGuildAndChannelMetadata: null, questContentPosition: null, questContentRowIndex: null };
          ...
          const tmpResult8 = tmp(7943);
          const _HermesInternal = HermesInternal;
          const obj6 = { impressionId: questContentRowIndex.id };
          questLogger.info("" + questName + " Quest became visible at " + tmp(7962).getQuestContentName(questContentRowIndex.questContent), obj6);
          const tmpResult9 = tmp(7962);
          const obj7 = {};
          ...
          const tmpResult10 = tmp(7952);
          let id;
          if (quest != null) {
            id = quest.id;
          }
          let adTrafficMetadataSealed = tmp(7933).getAdTrafficMetadataSealed(questContentRowIndex.sourceQuestContent, id);
          if (adTrafficMetadataSealed == null) {
            adTrafficMetadataSealed = null;
          }
          ...
          const tmpResult11 = tmp(7933);
        } else {
          const questLogger1 = tmp(7943).getQuestLogger();
          const tmpResult12 = tmp(7943);
          const _HermesInternal2 = HermesInternal;
          const obj9 = { impressionId: questContentRowIndex.id };
          questLogger1.info("" + questContentRowIndex.entity.adContentIds[tmpResult] + " ad content became visible at " + tmp(7962).getQuestContentName(questContentRowIndex.questContent), obj9);
          const tmpResult13 = tmp(7962);
          const obj10 = {};
          ...
          tmp(7952).trackAdContentEvent(obj10);
          const tmpResult14 = tmp(7952);
        }
      });
      let obj2 = { name: obj5(5089).MetricEvents.QUEST_CONTENT_IMPRESSION, tags: null };
      let obj = MonitoringAgentDefault;
      const items = ["quest_content:" + obj5(7962).getQuestContentName(obj5.questContent)];
      obj2.tags = items;
      obj.increment(obj2);
      obj5.isRunning = true;
    };
