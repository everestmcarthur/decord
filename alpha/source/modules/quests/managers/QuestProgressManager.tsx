...
// Dependencies: [5, 32, 1957, 9748, 1916, 4744, 1932, 4746, 7899, 17407, 5614, 9749, 1091, 7905, 7895, 9763, 5615, 11528, 5617, 7924, 4853, 4852, 7314, 5622, 7920, 4774, 1370, 4851, 7918, 573, 2]

// Module 18257 (QuestProgressManager)
import DispatcherDefault from "Dispatcher" /* 573 */;
import DurationsDefault from "Durations" /* 1091 */;
import GameAnalyticsUtils from "GameAnalyticsUtils" /* 4851 */;
import RobloxSubgameUtils from "RobloxSubgameUtils" /* 4852 */;
import RobloxSubgameTypes from "RobloxSubgameTypes" /* 4853 */;
import QuestVariants from "QuestVariants" /* 5615 */;
  ...
  for (const item10020 of eligibleQuestsForApplicationId) {
    if (tmp) {
      ...
      if (features.includes(QuestVariants.QuestVariants.MOBILE_ACTIVITY_QUEST)) {
        let tmp6Result = tmp6(11528);
        let obj3 = { questContent: tmp6(5617).QuestContent.RUNNING_ACTIVITY, questContentCTA: tmp6(7924).QuestContentCTA.START_QUEST, sourceQuestContent: tmp6(5617).QuestContent.RUNNING_ACTIVITY };
      ...
      if (null != value) {
        if (null != value.config) {
          if (null != value.userStatus) {
            const questTaskDetails = applyArgumentsResult(7920).getQuestTaskDetails(value, applyArgumentsResult(5622).FirstPartyQuestTaskTypesSets.DESKTOP);
            const _Math = Math;
    ...
    obj1 = {
      ...
      GAME_FETCH_SUCCESS() {
            return DispatcherDefault.wait(() => {
              const items = [applyArgumentsResult(5622).FirstPartyQuestTaskTypes.PLAY_ON_DESKTOP];
      ...
      applyArgumentsResult.syncHeartbeats(items, "QUEST_APPLICATION_START_TIMER", (id) => {
        ...
        if (tmp) {
          ...
          if (hasItem) {
            ...
            hasItem = features.includes(applyArgumentsResult(5615).QuestVariants.MANUAL_HEARTBEAT_INITIALIZATION);
          }
          tmp = hasItem;
        }
        return tmp;
...
prototype["getActivelyProgressingQuests"] = function getActivelyProgressingQuests(arg0) {
  ...
  } else if (tmp(5622).FirstPartyQuestTaskTypes.STREAM_ON_DESKTOP === arg0) {
    return self.getActivelyProgressingStreamOnDesktopQuests();
  } else if (tmp(5622).FirstPartyQuestTaskTypes.PLAY_ACTIVITY === arg0) {
      ...
      if (isQuestProgressable(nextResult)) {
        if (null != desktopApplicationIds) {
          ...
          if (null != found) {
            ...
            obj4.applicationId = tmp7(4853).ROBLOX_APPLICATION_ID;
