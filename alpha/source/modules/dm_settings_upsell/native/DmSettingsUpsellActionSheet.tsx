...
// Dependencies: [19, 17, 1980, 21, 4722, 576, 504, 17671, 17675, 7393, 11626, 4718, 1115, 5754, 5144, 7191, 1936, 4420, 9814, 4689, 14116, 2]
// Exports: default

// Module 17674 (DmSettingsUpsellActionSheet)
...
import DmSettingsUpsellManager from "DmSettingsUpsellManager" /* 17671 */;
import DmSettingsUpsellUtils from "DmSettingsUpsellUtils" /* 17675 */;
...
export default function DmSettingsUpsellActionSheet(guildId) {
  ...
  if (null != stateFromStores) {
    ...
    const obj4 = { source: stateFromStores(11626), style: tmp.headerImage };
    ...
    items2[1] = closure_7(tmp2(4718).Text, obj5);
    ...
    items2[2] = closure_7(tmp2(4718).Text, obj6);
    ...
    const items3 = [closure_7(tmp2(4718).Text, obj9), ];
    const obj10 = { style: tmp.guildInfo, children: null };
    const obj11 = { guild: stateFromStores, size: tmp2(5754).GuildIconSizes.SMALL_32 };
    const items4 = [closure_7(stateFromStores(5754), obj11), ];
    const obj12 = { variant: "text-md/semibold", color: "mobile-text-heading-primary", children: stateFromStores.name };
    items4[1] = closure_7(tmp2(4718).Text, obj12);
    ...
    const obj13 = {
      ...
      onPress() {
          ...
          RestrictedGuildIds.updateSetting(Array.from(sanitizedRestrictedGuilds)).then(() => {
            const obj2 = { key: "DM_SETTINGS_UPSELL_SUCCESS_TOAST", icon: stateFromStores(9814), content: null };
            const intl = guildId(1115).intl;
            obj2.content = intl.string(guildId(1115).t.rlYD1W);
            stateFromStores(4420).open(obj2);
          });
    ...
    items2[4] = closure_7(tmp2(5144).Button, obj13);
    const obj14 = {
      size: "lg",
      variant: "secondary",
      onPress() {
    ...
    items2[5] = closure_7(tmp2(5144).Button, obj14);
    ...
    items2[6] = closure_7(tmp2(4718).Text, obj15);
    obj3.children = items2;
    obj2.children = closure_8(closure_4, obj3);
    tmp6 = closure_7(tmp2(7393).ActionSheet, obj2);
    const tmp12 = stateFromStores(5754);
  }
  return tmp6;
};
