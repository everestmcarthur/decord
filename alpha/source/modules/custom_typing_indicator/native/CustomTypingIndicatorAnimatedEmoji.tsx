      ...
      if (animation !== user.TypingIndicatorAnimation.UNSPECIFIED) {
        if (stateFromStores) {
          ...
          if (tmp5(1380).TypingIndicatorAnimation.PULSE === tmp4) {
            ...
            const tmp5Result15 = tmp5(4492);
            const tmp5Result16 = tmp5(4492);
            const obj4 = { duration: tmp8 };
            const tmp5Result17 = tmp5(4757);
            const withTimingResult = tmp5(4757).withTiming(1.16, obj4);
            const obj5 = { duration: tmp8 };
            const tmp5Result18 = tmp5(4757);
            const withTimingResult1 = tmp5(4757).withTiming(1, obj5);
            const obj6 = { duration: result4 };
            const result5 = sharedValue.set(tmp5Result.withDelay(result3, tmp5Result15.withRepeat(tmp5Result16.withSequence(withTimingResult, withTimingResult1, tmp5(4757).withTiming(1, obj6)), -1)));
            const tmp5Result19 = tmp5(4757);
          } else if (tmp5(1380).TypingIndicatorAnimation.RING === tmp4) {
            const tmp5Result20 = tmp5(4492);
            const tmp5Result21 = tmp5(4492);
            const _Math = Math;
            const obj7 = { duration: 1600, easing: null };
            const result6 = 2 * Math.PI;
            obj7.easing = tmp5(4492).Easing.linear;
            const result7 = sharedValue1.set(tmp5Result20.withDelay(result3, tmp5Result21.withRepeat(tmp5(4757).withTiming(result6, obj7), -1)));
            const tmp5Result22 = tmp5(4757);
          } else if (tmp5(1380).TypingIndicatorAnimation.WAVE === tmp4) {
            const tmp5Result23 = tmp5(4492);
            const tmp5Result24 = tmp5(4492);
            const tmp5Result25 = tmp5(4492);
            const obj8 = { duration: tmp8 };
            const tmp5Result26 = tmp5(4757);
            const withTimingResult2 = tmp5(4757).withTiming(-0.12 * num, obj8);
            const obj9 = { duration: tmp8 };
            const tmp5Result27 = tmp5(4757);
            const withTimingResult3 = tmp5(4757).withTiming(0, obj9);
            const obj10 = { duration: result4 };
            const result8 = sharedValue2.set(tmp5Result23.withDelay(result3, tmp5Result24.withRepeat(tmp5Result25.withSequence(withTimingResult2, withTimingResult3, tmp5(4757).withTiming(0, obj10)), -1)));
            const tmp5Result28 = tmp5(4757);
          }
          return () => {
