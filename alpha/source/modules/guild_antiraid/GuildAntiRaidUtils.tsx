...
export const hasDetectedActivity = function hasDetectedActivity(incidentsData) {
  ...
  if (tmp) {
    const obj = _modDef4348(incidentsData.dmSpamDetectedAt);
    tmp = _modDef4348(incidentsData.dmSpamDetectedAt).add(React3, "hours") > _modDef4348();
    const addResult = _modDef4348(incidentsData.dmSpamDetectedAt).add(React3, "hours");
  }
  if (!tmp) {
    let tmp6 = null != incidentsData.raidDetectedAt;
    if (tmp6) {
      const obj2 = _modDef4348(incidentsData.raidDetectedAt);
      tmp6 = _modDef4348(incidentsData.raidDetectedAt).add(React3, "hours") > _modDef4348();
      const addResult1 = _modDef4348(incidentsData.raidDetectedAt).add(React3, "hours");
    }
    tmp = tmp6;
  }
  return tmp;
...
export const hasDetectedRaid = function hasDetectedRaid(raidDetectedAt) {
  ...
  if (tmp) {
    const obj = _modDef4348(raidDetectedAt.raidDetectedAt);
    tmp = _modDef4348(raidDetectedAt.raidDetectedAt).add(React3, "hours") > _modDef4348();
    const addResult = _modDef4348(raidDetectedAt.raidDetectedAt).add(React3, "hours");
  }
  return tmp;
};
export const hasDetectedDMRaid = function hasDetectedDMRaid(dmSpamDetectedAt) {
  ...
  if (tmp) {
    const obj = _modDef4348(dmSpamDetectedAt.dmSpamDetectedAt);
    tmp = _modDef4348(dmSpamDetectedAt.dmSpamDetectedAt).add(React3, "hours") > _modDef4348();
    const addResult = _modDef4348(dmSpamDetectedAt.dmSpamDetectedAt).add(React3, "hours");
  }
  return tmp;
};
export const getIncidentAlertType = function getIncidentAlertType(guildIncident) {
  if (null != guildIncident) {
    ...
    if (tmp) {
      const obj = _modDef4348(guildIncident.raidDetectedAt);
      tmp = _modDef4348(guildIncident.raidDetectedAt).add(React3, "hours") > _modDef4348();
      const addResult = _modDef4348(guildIncident.raidDetectedAt).add(React3, "hours");
    }
