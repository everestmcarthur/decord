...
// Dependencies: [19, 10531, 1074, 1375, 1218, 1374, 21, 7358, 7378, 10532, 10522, 10520, 10533, 10521, 10534, 9551, 10210, 8056, 4690, 4691, 4379, 4588, 10539, 4420, 1115, 10526, 10544, 1936, 10550, 10547, 10551, 10554, 10548, 10557, 10558, 10565, 10567, 8060, 10569, 2]

// Module 10530 (EmojiPickerList)
...
import PremiumUpsellSectionDividerDefault from "PremiumUpsellSectionDivider" /* 10547 */;
import noop from "module_19" /* 19 */;

require = fn;
const IMAGE_SIZE = fn(10531).IMAGE_SIZE;
...
export default noop.memo(function EmojiPickerList(analyticsObject) {
  ...
  onPressEmoji = obj.useCallback((emoji, category) => {
    ...
    if (null === emojiUnavailableReason) {
      ...
      tmp2(4588).dismissKeyboard();
      if (null != emoji.guildId) {
        const obj4 = { guildId: emoji.guildId };
        const result1 = tmp4(10539).handleShowEmojiUpsellAlert(obj4);
        const tmp4Result = tmp4(10539);
      }
  ...
  const callback2 = obj.useCallback((item) => {
    ...
    if (useEmojiPickerData.EmojiPickerItemType.NATIVE_SECTION !== type) {
      if (tmp(10544).EmojiPickerItemType.PLACEHOLDER !== type) {
        if (tmp(10544).EmojiPickerItemType.EMOJI_ROW_SLIM !== type) {
          if (tmp(10544).EmojiPickerItemType.TITLE === type) {
            ...
            return map1(tmp(10550).Section, obj2);
          } else if (tmp(10544).EmojiPickerItemType.PREMIUM_INLINE_ROADBLOCK === type) {
            const obj3 = { position: item.position, useTier0UpsellContent };
            return map1(PremiumUpsellSectionDividerDefault, obj3);
          } else {
            if (tmp(10544).EmojiPickerItemType.EMOJI_ROW === type) {
              ...
              let tmp27Result = map1(tmp(10551).EmojiPickerListRow, obj4);
            } else if (tmp(10544).EmojiPickerItemType.EMOJI_ROW_NSFW === type) {
              tmp27Result = map1(tmp(10550).NSFWRow, {});
            } else if (tmp(10544).EmojiPickerItemType.FOOTER_UPSELL === type) {
              let guild_id;
              if (channel != null) {
                guild_id = channel.guild_id;
              }
              const obj = { guildId: guild_id, analyticsLocations, useTier0UpsellContent };
              tmp27Result = map1(tmp(10554).PremiumSearchUpsell, obj);
            }
            let tmp16 = true === item.isSectionNitroLocked;
            if (tmp16) {
              const obj5 = { useTier0UpsellContent };
              tmp16 = map1(tmp(10548).PremiumUpsellGradientBackground, obj5);
            }
