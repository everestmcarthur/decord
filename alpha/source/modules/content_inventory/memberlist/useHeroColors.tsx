...
// Function ID: 8402
// Name: useHeroColors
// Dependencies: [32, 19, 4748, 1182, 8400, 7792, 8402, 1092, 672, 504, 2]
// Exports: default, getHeroColors

// Module 8401 (useHeroColors)
...
export default function useHeroColors(pendingAvatarSrc) {
  ...
  let obj2 = first(504);
  const fallbackHeroColor = first(8402).getFallbackHeroColor(stateFromStores1, stateFromStores);
  let obj3 = first(8402);
  let tmp4 = _slicedToArray(first(8400).useAvatarColors(pendingAvatarSrc, fallbackHeroColor), 2);
  first = tmp4[0];
  closure_1 = tmp6;
...
export const getHeroColors = function getHeroColors(game_name) {
  const fallbackHeroColor = num(8402).getFallbackHeroColor(ThemeStore.theme, AccessibilityStore.saturation);
  num = 1;
  if (AccessibilityStore.desaturateUserColors) {
    num = tmp.saturation;
  }
  const useColorStore = tmp2(8400).useColorStore;
