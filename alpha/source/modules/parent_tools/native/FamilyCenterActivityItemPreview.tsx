...
// Dependencies: [19, 17, 8490, 21, 4756, 15174, 576, 9099, 8469, 9102, 12389, 9488, 8939, 1973, 1970, 2]
// Exports: default

// Module 15175 (FamilyCenterActivityItemPreview)
import nativeDefault from "native" /* 576 */;
import CollectiblesItemType from "CollectiblesItemType" /* 1973 */;
import useMaybeFetchProfileFrameDefault from "useMaybeFetchProfileFrame" /* 8469 */;
import NameplateUtils from "NameplateUtils" /* 9099 */;
...
function ProfileFramePreviewImage(arg0) {
  ...
  if (null != tmp3) {
    ...
    obj.children = jsx(tmp(9102), { profileFrame: tmp3, previewWidth: FamilyCenterActivityPurchaseRowUtils.PREVIEW_SIZE * closure_5, previewHeight: FamilyCenterActivityPurchaseRowUtils.PREVIEW_SIZE, profileBackgroundColor: tmp(576).colors.BACKGROUND_BASE_LOW });
    tmp4 = <React3 style={styles.profileFrameContainer}>{null}</React3>;
    const tmpResult = tmp(9102);
  }
  return tmp4;
}
function SubscriptionPreview(arg0) {
  ...
  if (null == subscriptionPlanId) {
    ...
    if (obj4.isGuildBoostSubscription(subscriptionPlanId)) {
      let NitroWheelIcon = tmp5(9488).BoostGemIcon;
    } else {
      NitroWheelIcon = tmp5(8939).NitroWheelIcon;
    }
...
function CollectiblePreview(arg0) {
  ...
  if (null == product) {
    ...
    if (CollectiblesItemType.CollectiblesItemType.AVATAR_DECORATION === type) {
      ...
      obj.children = jsx(tmp17(12389).ShopIcon, { size: "custom", style: { width: 20, height: 20 } });
      return <React3 style={styles.purchasePlaceholder}>{null}</React3>;
    }
  }
}
...
let closure_5 = fn(8490).PROFILE_FRAME_ASPECT_RATIO;
const jsx = fn(21).jsx;
...
let size = { width: fn(15174).PREVIEW_SIZE, height: fn(15174).PREVIEW_SIZE, borderRadius: nativeDefault.radii.xs, backgroundColor: nativeDefault.colors.BACKGROUND_SURFACE_HIGH, display: "flex", alignItems: "center", justifyContent: "center", marginRight: 12 };
obj2.purchasePlaceholder = size;
const size1 = { width: fn(15174).PREVIEW_SIZE, height: fn(15174).PREVIEW_SIZE, marginRight: 12 };
obj2.avatarDecorationPreview = size1;
const size2 = { width: fn(15174).PREVIEW_SIZE, height: fn(15174).PREVIEW_SIZE, marginRight: 12, borderRadius: nativeDefault.radii.xs, overflow: "hidden", position: "relative" };
obj2.nameplateContainer = size2;
const size3 = { position: "absolute", right: 0, width: fn(15174).PREVIEW_SIZE * fn(15174).NAMEPLATE_ASPECT_RATIO, height: fn(15174).PREVIEW_SIZE };
obj2.nameplatePreview = size3;
const size4 = { width: fn(15174).PREVIEW_SIZE, height: fn(15174).PREVIEW_SIZE, marginRight: 12, alignItems: "center", justifyContent: "center" };
