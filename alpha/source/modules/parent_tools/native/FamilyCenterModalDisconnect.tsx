...
// Dependencies: [32, 19, 17, 21, 4722, 576, 4925, 4566, 8883, 4419, 1115, 12160, 12163, 2480, 8654, 8655, 15140, 7188, 4718, 15092, 5142, 9667, 12170, 5603, 5144, 5794, 11451, 2]
// Exports: default

// Module 15139 (FamilyCenterModalDisconnect)
...
import FamilyCenterAvatarPairDefault from "FamilyCenterAvatarPair" /* 15140 */;
...
function FamilyCenterModalDisconnectScreen(otherUser) {
  ...
  const requiresParentalConsent = otherUser(8883).useRequiresParentalConsent(otherUser.id);
  const obj2 = otherUser(8883);
  ...
  const obj3 = otherUser(12160);
  let intl = otherUser(1115).intl;
  const obj4 = otherUser(12163);
  const intl2 = otherUser(1115).intl;
  const ageSpecificText = obj4.useAgeSpecificText(intl.format(_modDef2480.F2lccv, { username: name }), intl2.string(_modDef2480["WH+Gba"]));
  const formatResult = intl.format(_modDef2480.F2lccv, { username: name });
  const intl3 = otherUser(1115).intl;
  const obj5 = otherUser(12163);
  const intl4 = otherUser(1115).intl;
  const ageSpecificText1 = obj5.useAgeSpecificText(intl3.string(_modDef2480.hOEHFn), intl4.format(_modDef2480.Or6hgl, { username: name }));
  const stringResult = intl3.string(_modDef2480.hOEHFn);
  const intl5 = otherUser(1115).intl;
  const obj6 = otherUser(12163);
  const intl6 = otherUser(1115).intl;
  const ageSpecificText2 = obj6.useAgeSpecificText(intl5.format(_modDef2480.XyRW4c, { username: name }), intl6.format(_modDef2480.PlrZal, { username: name }));
  const formatResult1 = intl5.format(_modDef2480.XyRW4c, { username: name });
  const intl7 = otherUser(1115).intl;
  const obj7 = otherUser(12163);
  const intl8 = otherUser(1115).intl;
  const ageSpecificText3 = obj7.useAgeSpecificText(intl7.string(_modDef2480.eiABQz), intl8.string(_modDef2480.PGQBnk));
  const stringResult1 = intl7.string(_modDef2480.eiABQz);
  const intl9 = otherUser(1115).intl;
  const obj8 = otherUser(12163);
  const intl10 = otherUser(1115).intl;
  const items = [disconnectLinkRequest, otherUser.id];
  const ageSpecificText4 = obj8.useAgeSpecificText(intl9.string(_modDef2480.sCbKs4), intl10.string(_modDef2480["0ki7+P"]));
  ...
  const stringResult2 = intl9.string(_modDef2480.sCbKs4);
  obj10.iconSrc = _modDef7188;
  ...
  obj11.children = intl11.format(_modDef2480.o0JXuK, { username: name });
  items3[1] = closure_6(otherUser(4718).Text, obj11);
  items3[2] = closure_6(otherUser(4718).Text, { style: tmp.subtitle, variant: "text-sm/bold", color: "text-default", children: ageSpecificText });
  ...
  if (requiresParentalConsent) {
    ...
    tmp28Result = tmp28(tmp3(15092), obj13);
  }
  const obj14 = { children: null };
  items4[1] = tmp28Result;
  items4[2] = closure_6(otherUser(4718).Text, { style: tmp.body, variant: "text-md/normal", color: "text-default", children: ageSpecificText2 });
  const obj16 = { spacing: nativeDefault.space.PX_12, children: null };
  const items5 = [closure_6(otherUser(9667).Checkbox, { label: ageSpecificText3, checked: tmp9, onToggle: callback3 }), closure_6(otherUser(9667).Checkbox, { label: ageSpecificText4, checked: tmp11, onToggle: callback4 })];
  obj16.children = items5;
  items4[3] = closure_7(otherUser(5142).Stack, obj16);
  obj14.children = items4;
  const items6 = [closure_7(otherUser(8655).ModalContent, obj14), ];
  let tmp31 = !tmp9;
  if (tmp9) {
    tmp31 = !tmp11;
  }
  ...
  obj20.text = intl12.string(_modDef2480["c5L+sl"]);
  obj20.onPress = callback2;
  const items7 = [closure_6(otherUser(5144).Button, obj20), ];
  ...
  items7[1] = closure_6(otherUser(5144).Button, obj21);
  obj19.children = items7;
  obj18.children = closure_7(otherUser(5603).ButtonGroup, obj19);
  items6[1] = closure_6(otherUser(12170).ModalFooter, obj18);
  obj17.children = items6;
  return closure_7(otherUser(8654).ModalScreen, obj17);
}
...
export default function FamilyCenterModalDisconnect(otherUser) {
  ...
  return closure_6(otherUser(11451).Modal, obj);
};
