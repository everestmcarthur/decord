...
// Dependencies: [32, 19, 17, 7778, 21, 4756, 576, 8923, 7832, 15166, 4752, 12169, 1115, 2482, 15172, 5334, 2]
// Exports: default

// Module 15171 (FamilyCenterActivitySection)
...
export default function FamilyCenterActivitySection(displayType) {
  ...
  const actionsForDisplayType = displayType(15166).useActionsForDisplayType(displayType);
  const obj = displayType(15166);
  const actionTotalsForDisplayType = displayType(15166).useActionTotalsForDisplayType(displayType);
  const obj2 = displayType(15166);
  const familyCenterActions = displayType(12169).useFamilyCenterActions({});
  ...
  if (0 === actionsForDisplayType.length) {
    ...
    const items1 = [closure_9(FamilyCenterActivitySectionHeader, obj6), substr.map((action) => closure_1_9(loadMore(15172), { action }, action.event_id)), ];
    if (substr.length >= actionTotalsForDisplayType) {
      ...
      if (familyCenterActions.isMoreLoading) {
        ...
        obj10.children = tmp17(tmp(4752).Text, obj11);
        tmp17Result = tmp17(tmp(5334).PressableOpacity, obj10);
      }
