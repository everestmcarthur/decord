...
// Module 15145 (FamilyCenterModalCancel)
...
function FamilyCenterModalCancelScreen(otherUser) {
  ...
  obj5.iconSrc = cancelLinkRequest(7188);
  const items1 = [closure_5(cancelLinkRequest(15140), obj5), , ];
  const obj6 = { style: tmp.headerText, variant: "text-lg/bold", children: null };
  let intl = otherUser(1115).intl;
  obj6.children = intl.string(cancelLinkRequest(2480).HynllX);
  items1[1] = closure_5(otherUser(4718).Text, obj6);
  items1[2] = closure_5(cancelLinkRequest(15110), { user: otherUser });
  obj4.children = items1;
  obj3.children = closure_6(View, obj4);
  const items2 = [closure_5(otherUser(8655).ModalContent, obj3), ];
  ...
  obj9.text = intl2.string(cancelLinkRequest(2480).mK40bk);
  obj9.onPress = callback2;
  const items3 = [closure_5(otherUser(5144).Button, obj9), ];
  const obj10 = { variant: "tertiary", text: null, onPress: null };
  const intl3 = otherUser(1115).intl;
  obj10.text = intl3.string(cancelLinkRequest(2480).czincX);
  obj10.onPress = cancelLinkRequest(4925).pop;
  items3[1] = closure_5(otherUser(5144).Button, obj10);
  obj8.children = items3;
  obj7.children = closure_6(otherUser(5603).ButtonGroup, obj8);
  items2[1] = closure_5(otherUser(12170).ModalFooter, obj7);
