...
// Dependencies: [5, 19, 17, 1372, 7734, 21, 4722, 576, 1484, 563, 7735, 12159, 7788, 12160, 8654, 8655, 1177, 1397, 1115, 2480, 4718, 12161, 12162, 12170, 5603, 5144, 4925, 5862, 8884, 38, 11804, 7188, 5747, 8504, 1895, 8643, 8645, 4935, 5794, 12171, 11451, 2]
// Exports: default

// Module 12158 (FamilyCenterModalRequest)
...
import NavigatorHeader2 from "NavigatorHeader" /* 5794 */;
...
import FamilyCenterModalRequestRouting from "FamilyCenterModalRequestRouting" /* 12159 */;
...
function FamilyCenterConfirmAgeScreen(teenIdentity) {
  ...
  const tmp7 = stateFromStores(8504)(stateFromStores);
  dependencyMap = tmp7;
  ...
  const callback = noop.useCallback(() => {
    const obj = stateFromStores(8643);
    const result = obj.showAgeVerificationGetStartedModal({ entryPoint: navigation(8645).AgeVerificationModalEntryPoint.FAMILY_CENTER_CONNECTION });
  }, []);
  const obj3 = { style: tmp.content, children: null };
  const obj2 = navigation(563);
  const items2 = [closure_12(closure_6, { style: tmp.art, children: closure_12(navigation(5862).FamilyShieldSpotIllustration, {}) }), , ];
  ...
  obj5.children = intl.formatToPlainString(stateFromStores(2480).pQQMJ7, { username: str });
  items2[1] = closure_12(navigation(4718).Text, obj5);
  const obj7 = { variant: "text-sm/medium", color: "text-muted", style: tmp.description, children: null };
  const intl2 = tmp2(1115).intl;
  obj7.children = intl2.format(stateFromStores(2480)["0o3yg8"], { link: "https://support.discord.com/hc/articles/14155060633623" });
  items2[2] = closure_12(navigation(4718).Text, obj7);
  ...
  obj10.text = intl3.string(stateFromStores(2480)["3oUE4o"]);
  obj10.onPress = callback;
  const items4 = [closure_12(navigation(5144).Button, obj10), ];
  const obj11 = { variant: "tertiary", text: null, onPress: null };
  const intl4 = tmp2(1115).intl;
  obj11.text = intl4.string(navigation(1115).t.oEAioF);
  obj11.onPress = function onPress() {
    return stateFromStores(4925).pop();
  };
  items4[1] = closure_12(navigation(5144).Button, obj11);
  obj9.children = items4;
  obj8.children = closure_13(navigation(5603).ButtonGroup, obj9);
  items3[1] = closure_12(navigation(12170).ModalFooter, obj8);
  obj6.children = items3;
  return closure_13(navigation(8654).ModalScreen, obj6);
}
...
function FamilyCenterPrereqInvalidCodeScreen() {
  ...
  obj.title = intl.string(_modDef2480.ewSb6o);
  const intl2 = util.intl;
  obj.description = intl2.string(_modDef2480.jcUN2F);
...
function FamilyCenterPrereqMustBeAdultScreen() {
  ...
  const tmp3 = stateFromStores(8504)(stateFromStores);
  dependencyMap = tmp3;
  ...
  const intl = navigation(1115).intl;
  obj3.title = intl.string(stateFromStores(2480).BQFHXW);
  const intl2 = navigation(1115).intl;
  obj3.description = intl2.format(stateFromStores(2480).WDjaKn, { link: { onClick: callback } });
  const obj5 = { text: null, onPress: null };
  const intl3 = navigation(1115).intl;
  obj5.text = intl3.string(navigation(1115).t["NX+WJN"]);
  obj5.onPress = stateFromStores(4925).pop;
...
export default function FamilyCenterRequestModal(userId) {
  ...
  const FamilyCenterConnectionPrereqExperiment = userId(12171).FamilyCenterConnectionPrereqExperiment;
  ...
  const FamilyCenterModalRequestSections = userId(12159).FamilyCenterModalRequestSections;
  ...
  return closure_12(userId(11451).Modal, obj);
};
