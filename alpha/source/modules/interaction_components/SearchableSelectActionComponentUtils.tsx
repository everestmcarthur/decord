...
// Dependencies: [2041, 2105, 2099, 2063, 4405, 1372, 8381, 1978, 5654, 4908, 4987, 4909, 8389, 1370, 2]
    ...
    const mapped = defaultValues.map((type) => {
      ...
      if (SnowflakeSelectDefaultValueTypes.SnowflakeSelectDefaultValueTypes.USER === type) {
        ...
        if (null == user) {
          ...
          const obj2 = { type: tmp(4987).SelectOptionType.USER, value: user.id, label: null };
          if (nick == null) {
      ...
      } else if (tmp(8389).SnowflakeSelectDefaultValueTypes.ROLE === type) {
        if (null == closure_1) {
          return null;
        } else {
          ...
          let tmp12 = null;
          if (null != role) {
            const obj3 = { type: tmp(4987).SelectOptionType.ROLE, value: null, label: null };
            ({ id: obj4.value, name: obj4.label } = role);
            tmp12 = obj3;
          }
          return tmp12;
        }
      } else if (tmp(8389).SnowflakeSelectDefaultValueTypes.CHANNEL === type) {
        if (null == closure_1) {
          return null;
        } else {
          ...
          if (null != channel) {
            ...
            if (channel.guild_id === tmp3.id) {
              if (channelTypes.length <= 0) {
                const obj5 = { type: tmp(4987).SelectOptionType.CHANNEL, value: channel.id, label: tmp(4909).computeChannelName(channel, UserStore, RelationshipStore) };
                tmp5 = obj5;
