...
// Dependencies: [32, 19, 21, 4722, 576, 8349, 1895, 4689, 12065, 7326, 4718, 1115, 2]
// Exports: default

// Module 12064 (StringSelectComponentActionSheet)
...
export default function StringSelectComponentActionSheet(selectionActionComponent) {
  ...
  let tmp = callback();
  dependencyMap = tmp;
  let obj = selectionActionComponent(8349);
  let tmp3 = first(noop.useState(new Set(selectionActionComponent(8349).getInitialStringSelectOptions(selectionActionComponent, containerId))), 2);
  first = tmp3[0];
  noop = tmp3[1];
  ...
  let set = new Set(selectionActionComponent(8349).getInitialStringSelectOptions(selectionActionComponent, containerId));
  const tmp9 = memo;
  ...
  return tmp9(onSubmit(12065), obj2);
};
