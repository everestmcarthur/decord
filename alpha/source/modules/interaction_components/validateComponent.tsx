...
// Dependencies: [1895, 4947, 1115, 38, 2]
// Exports: default

// Module 8345 (validateComponent)
...
export default function validateComponent(type, arg1, modal) {
  ...
  if (Server.ComponentType.BUTTON === type) {
    ...
    if (tmp4(1895).ComponentType.STRING_SELECT !== type) {
      if (tmp4(1895).ComponentType.USER_SELECT !== type) {
        if (tmp4(1895).ComponentType.ROLE_SELECT !== type) {
          if (tmp4(1895).ComponentType.MENTIONABLE_SELECT !== type) {
            if (tmp4(1895).ComponentType.CHANNEL_SELECT !== type) {
              if (tmp4(1895).ComponentType.TEXT_INPUT === type) {
                ...
                if (null != values) {
                  if (0 !== values.value.length) {
                    if (values.value.length < minLength) {
                      ...
                      let formatToPlainStringResult = intl8.formatToPlainString(tmp4(1115).t.ONSqYd, range);
                    } else {
              ...
              } else if (tmp4(1895).ComponentType.FILE_UPLOAD === type) {
                ...
                if (0 === num3) {
                  ...
                  if (type.required) {
                    ...
                    stringResult1 = intl7.string(tmp4(1115).t.eJEUvD);
                  }
                  ...
                  formatToPlainStringResult1 = intl6.formatToPlainString(tmp4(1115).t.pmAt62, obj2);
                } else {
                  formatToPlainStringResult1 = null;
                  if (num3 > maxValues2) {
                    const intl5 = tmp4(1115).intl;
                    const obj3 = { maxValues: maxValues2 };
                    formatToPlainStringResult1 = intl5.formatToPlainString(tmp4(1115).t.dy6viJ, obj3);
                  }
                }
                return formatToPlainStringResult1;
              } else if (tmp4(1895).ComponentType.RADIO_GROUP === type) {
                if (null == values) {
                  let stringResult2 = null;
                  if (tmp10) {
                    const intl4 = tmp4(1115).intl;
                    stringResult2 = intl4.string(tmp4(1115).t.eJEUvD);
                  }
                } else {
                  stringResult2 = null;
                }
                return stringResult2;
              } else if (tmp4(1895).ComponentType.CHECKBOX_GROUP === type) {
                ...
                if (0 === num) {
                  ...
                  if (type.required) {
                    ...
                    stringResult3 = intl3.string(tmp4(1115).t.eJEUvD);
                  }
                  let formatToPlainStringResult2 = stringResult3;
                } else if (num < minValues) {
                  const intl2 = tmp4(1115).intl;
                  const obj4 = { count: minValues };
                  formatToPlainStringResult2 = intl2.formatToPlainString(tmp4(1115).t.Jmwzdx, obj4);
                } else {
                  formatToPlainStringResult2 = null;
                  if (num > maxValues) {
                    const intl = tmp4(1115).intl;
                    const obj = { count: maxValues };
                    formatToPlainStringResult2 = intl.formatToPlainString(tmp4(1115).t.LDvfRP, obj);
                  }
                }
                return formatToPlainStringResult2;
              } else if (tmp4(1895).ComponentType.CHECKBOX === type) {
                return null;
              } else {
    ...
    if (null == values) {
      {
        ...
        tmp4Result = tmp4(4947);
      }
      return tmp18;
    } else if (values.type === tmp4(1895).ComponentType.STRING_SELECT) {
