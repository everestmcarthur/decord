...
// Dependencies: [32, 19, 17, 1074, 1374, 21, 9036, 1115, 10369, 12784, 5312, 5308, 10639, 13845, 9484, 4698, 13847, 4756, 576, 13839, 5653, 4492, 4757, 4752, 4690, 4607, 5198, 5334, 4649, 13848, 13850, 4605, 1177, 13852, 13853, 5793, 12816, 2]
// Exports: default

// Module 13844 (GuildBoostingMarketingTierCards)
...
import ServerBoostStreamQualityMarketingExperiment from "ServerBoostStreamQualityMarketingExperiment" /* 13847 */;
...
let items = [
  {
    orderCollapsed: 0,
    isIncluded: true,
    IconComponent: fn(9036).ReactionIcon,
    getCopy() {
...
items[1] = {
  isIncluded: true,
  IconComponent: fn(10369).StickerIcon,
  getCopy() {
    const intl = util.intl;
...
items[2] = {
  isIncluded: true,
  IconComponent: fn(12784).ScreenArrowIcon,
  getCopy() {
    const intl = util.intl;
...
items[3] = {
  orderCollapsed: 2,
  isIncluded: true,
  IconComponent: fn(5312).VoiceNormalIcon,
  getCopy() {
    const intl = util.intl;
...
items[4] = {
  isIncluded: true,
  IconComponent: fn(5308).StageIcon,
  getCopy() {
    const intl = util.intl;
...
items[5] = {
  orderCollapsed: 1,
  isIncluded: true,
  IconComponent: fn(10639).GifIcon,
  getCopy() {
    const intl = util.intl;
...
items[6] = {
  isIncluded: true,
  IconComponent: fn(13845).ServerGridIcon,
  getCopy() {
    const intl = util.intl;
...
items[7] = {
  isIncluded: false,
  IconComponent: fn(9484).UploadIcon,
  getCopy() {
    const intl = util.intl;
...
items[10] = {
  isIncluded: false,
  IconComponent: fn(4698).LinkIcon,
  getCopy() {
    const intl = util.intl;
...
obj37.cardsScroller = { flex: 1, marginTop: fn(13839).PROGRESS_BAR_SPACING };
...
const obj41 = { flex: 1, marginTop: fn(13839).PROGRESS_BAR_SPACING };
obj37.sparkleStar = { position: "absolute", tintColor: fn(5653).DARK_WHITE_500_LIGHT_GUILD_BOOSTING_PINK };
...
let closure_16 = noop.forwardRef((onCardPress, ref) => {
  ...
  if (isExpanded) {
    let ChevronLargeDownIcon = tmp7(13848).ChevronLargeUpIcon;
  } else {
    ChevronLargeDownIcon = tmp7(13850).ChevronLargeDownIcon;
  }
  ...
  if (!tmp16) {
    ...
    if (tmp11Result) {
      ...
      const items7 = [tmp7(4605).hexWithOpacity(tmp9, 0), , ];
      const tmp7Result7 = tmp7(4605);
      items7[1] = tmp7(4605).hexWithOpacity(tmp9, 1);
      const tmp7Result8 = tmp7(4605);
      items7[2] = tmp7(4605).hexWithOpacity(tmp9, 0);
      obj15.colors = items7;
      obj15.start = { x: 0, y: 0 };
      obj15.end = { x: 1, y: 0 };
      ...
      const tmp7Result9 = tmp7(4605);
      const tmp2Result5 = tmp2(5198);
      const items10 = [tmp7(4605).hexWithOpacity(tmp9, 0), , ];
      const tmp7Result10 = tmp7(4605);
      items10[1] = tmp7(4605).hexWithOpacity(tmp9, 1);
      const tmp7Result11 = tmp7(4605);
      items10[2] = tmp7(4605).hexWithOpacity(tmp9, 0);
    ...
    obj22 = tmp13(tmp7(4752).Text, obj22);
