...
// Dependencies: [32, 19, 14978, 16271, 16272, 16268, 1484, 7195, 14976, 14975, 16270, 1485, 1115, 2]
// Exports: useUsernameRegistrationStep

// Module 16296 (useUsernameRegistrationStep)
...
export const useUsernameRegistrationStep = function useUsernameRegistrationStep(REGISTER_ACCOUNT_INFORMATION) {
  ...
  const callback = obj.useCallback((arg0) => {
    ...
    if (arg0) {
      const result = tmp3(16270).handleRegistrationSubmit(closure_0, navigation, context);
      const tmp3Result = tmp3(16270);
    } else {
      const obj2 = { step: React5(closure_0), toStep: tmp3(16270).getNextRegistrationTransitionStep(closure_0), actionType: constants.SUCCESS };
      context(obj2);
      const tmp3Result3 = tmp3(16270);
      const nextAuthState = tmp3(16270).getNextAuthState(closure_0);
      const StackActions = tmp3(1485).StackActions;
      navigation.dispatch(StackActions.push(nextAuthState));
      const tmp3Result4 = tmp3(16270);
    }
  }, items);
