...
// Dependencies: [5, 32, 19, 16271, 16272, 1074, 21, 16268, 16287, 5203, 7285, 1115, 7319, 16300, 7320, 2]
// Exports: default

// Module 16299 (components/VerifyPhone)
import RegistrationBailoutButtonDefault from "RegistrationBailoutButton" /* 16300 */;
...
const require = fn;
let closure_6 = fn(16271).doesRegistrationHaveIdentityType;
...
export default function VerifyPhone(phone) {
  ...
  onPhoneTokenReceived(16287)(closure_7(sourceState));
  const items = [context];
  ...
  onBail(function*(arg0) {
    ...
    yield onPhoneTokenReceived(7285).verifyPhone(closure_0, closure_0, false);
    if (1 === tmp7) {
      v0 = 0;
  ...
  onPhoneTokenReceived(7319)(callback1);
  const items3 = [onBail];
  ...
  const tmp7 = onPhoneTokenReceived(16287);
  obj.codeType = require("CodeField").CodeType.NUMERIC;
  obj.footer = memo;
  obj.disabled = tmp5;
  return jsx(onPhoneTokenReceived(7320), { title, description, error: tmp3, onCodeEntered, codeType: null, footer: null, disabled: null, loading: null, disableKeyboardAvoidingView: true });
};
