...
// Dependencies: [19, 1372, 7753, 563, 4380, 9081, 7750, 7749, 2]
// Exports: useBadBundleFilter

// Module 15282 (useBadBundleFilter)
    ...
    if (null != arr) {
      ...
      if (0 !== arr.length) {
        found = arr.filter((product) => {
          const obj = canUseShopDiscountsResult(9081);
            ...
            if (!tmpResult.isFreeCollectiblesProduct(product)) {
              if (!obj.getProductPurchaseState(CollectiblesPurchaseStore, product).isPurchased) {
                if (tmpResult5.isOrbsExclusiveProduct(product)) {
                  ...
                  return null != tmp(7749).getProductOrbPrice(obj3);
                } else {
                  const defaultPriceSetAssignmentPurchaseType = tmp(7750).getDefaultPriceSetAssignmentPurchaseType(hasShopDiscount);
                  const tmpResult7 = tmp(7750);
                  let result = tmp(7750).extractPriceByPurchaseTypes(product, defaultPriceSetAssignmentPurchaseType);
                  if (null != result) {
                    if (0 !== result.amount) {
                      let num2 = 0;
