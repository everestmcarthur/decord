...
// Dependencies: [19, 17, 7788, 9078, 21, 4756, 13472, 14898, 13473, 7422, 8494, 5799, 9103, 9081, 2]

// Module 14899 (EditProfileEffectSection)
...
import ProfileEffectDefault from "ProfileEffect" /* 9081 */;
...
import useProfileEffectSections from "useProfileEffectSections" /* 14898 */;
...
let obj = { row: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingHorizontal: fn(13472).GUTTER_SIZE }, rowSpacer: null, profileEffect: null, sampleProfile: null };
...
obj.sampleProfile = { aspectRatio: fn(9078).SAMPLE_PROFILE_ASPECT_RATIO, width: "100%" };
...
const memoResult = noop.memo((size) => {
  ...
  obj2.children = substr.map((item, index) => {
    if (item === useProfileEffectSections.NONE_ITEM) {
      ...
      return timestampProducer(tmp(13473).EditCollectiblesListItemNone, obj2, "none");
    } else if (item === tmp(14898).SHOP_ITEM) {
      const obj3 = { size, analyticsSource: AnalyticsLocationDefault.EDIT_PROFILE_EFFECT_SHEET };
      return timestampProducer(tmp(13473).EditCollectiblesListItemShop, obj3, "shop");
