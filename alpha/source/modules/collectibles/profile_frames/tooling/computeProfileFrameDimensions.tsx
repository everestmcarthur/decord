...
// Dependencies: [8479, 8476, 8477, 2]
// Exports: computeProfileFrameDimensions

// Module 8478 (computeProfileFrameDimensions)
...
export const computeProfileFrameDimensions = function computeProfileFrameDimensions(arr) {
  innerWidth = innerWidth(8479).DefaultProfileFrameDimensions.INNER_WIDTH;
  ...
  const found = arr.filter((layer) => {
    layer = layer.layer;
    let tmp3 = layer.type === innerWidth(8476).ProfileFrameLayerType.STAPLE;
    if (tmp3) {
      tmp3 = layer.anchor === innerWidth(8477).ProfileFrameLayerAnchor.TOP;
    }
    return tmp3;
  });
  const mapped1 = found.map((dims) => Math.max(0, dims.dims.height - (716 - innerWidth(8479).DefaultProfileFrameDimensions.OVERFLOW_TOP)));
  let overflowTop = 0;
  if (mapped1.length > 0) {
    const _Math3 = Math;
    const items1 = [];
  ...
  const found1 = arr.filter((layer) => {
    ...
    let tmp3 = layer.type === innerWidth(8476).ProfileFrameLayerType.STAPLE;
    if (tmp3) {
      tmp3 = layer.anchor === innerWidth(8477).ProfileFrameLayerAnchor.BOTTOM;
    }
    return tmp3;
  });
  const mapped2 = found1.map((dims) => Math.max(0, dims.dims.height - (424 - innerWidth(8479).DefaultProfileFrameDimensions.OVERFLOW_BOTTOM)));
