...
// Function ID: 9060
// Name: Nameplate
// Dependencies: [19, 4714, 21, 4722, 504, 4656, 4458, 4723, 1887, 9060, 5156, 1364, 9049, 5757, 2]
// Exports: default

// Module 9059 (Nameplate)
...
function NameplateInner(isFocused) {
  ...
  let obj = flag6(504);
  const tmp6 = sharedValue(4656)();
  let num = 1;
  if (flag6) {
    num = 0;
  }
  sharedValue = flag6(4458).useSharedValue(num);
  ...
  const animatedStyle = flag6(4458).useAnimatedStyle(A);
  const tmpResult = flag6(4458);
  const backgroundGradientColors = flag6(1887).getBackgroundGradientColors(nameplate.palette, tmp6);
  const tmpResult4 = flag6(1887);
  const nameplateAssets = flag6(9060).getNameplateAssets(nameplate);
  ...
  if (null != backgroundGradientColors) {
    ...
    obj4.colors = items3;
    tmp15Result = closure_5(tmp5(5156), obj4);
    const tmp5Result = tmp5(5156);
  }
  const items4 = [tmp15Result, ];
  const tmp13 = closure_6;
  const tmpResult5 = flag6(9060);
  if (tmpResult6.isAndroid()) {
    if (tmp12) {
      const obj5 = { url: str, style: tmp4.img, autoplay: true };
      let tmp17 = closure_5(tmp(9049).APNGPlayer, obj5);
    }
    items4[1] = tmp17;
    obj3.children = items4;
    return tmp13(tmp5(4458).View, obj3);
  }
  tmp17 = closure_5(tmp5(5757), { source: { uri: str }, style: tmp4.img, accessibilityRole: "image" });
}
const jsxProd = fn(21);
