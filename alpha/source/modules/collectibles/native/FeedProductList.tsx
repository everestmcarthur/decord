...
// Dependencies: [19, 17, 21, 4756, 9043, 16144, 9150, 16145, 2]
// Exports: default

// Module 16143 (FeedProductList)
import CollectiblesShopCardsGridDefault from "CollectiblesShopCardsGrid" /* 16145 */;
...
function SkeletonGrid(accessibilityLabel) {
  ...
  num = undefined;
  const tmp = closure_5();
  const cardLayout = num(16144).useCardLayout();
  ...
  for (let num = 0; num < loadingCardsNum; num = num + columns) {
    ...
    obj3.children = arr.map((item, index) => {
      ...
      const obj2 = { marginBottom: num(9043).COLLECTIBLES_SHOP_CARD_GAP };
      obj.style = obj2;
      return closure_4(closure_1(9150), obj, "" + num + "-" + index);
    });
...
let obj2 = { skeletonGrid: { flex: 1, alignItems: "center" }, skeletonRow: { flexDirection: "row", gap: fn(9043).COLLECTIBLES_SHOP_CARD_GAP, paddingBottom: fn(9043).COLLECTIBLES_SHOP_CARD_GAP } };
