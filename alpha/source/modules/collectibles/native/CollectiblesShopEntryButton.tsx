...
// Dependencies: [32, 19, 7824, 2038, 21, 7628, 2027, 17250, 12389, 1115, 17252, 563, 14249, 7805, 10884, 2]
// Exports: default

// Module 17251 (CollectiblesShopEntryButton)
import YouScreenNavIconDefault from "YouScreenNavIcon" /* 17250 */;
import MobileShopButtonCoachmarkDefault from "MobileShopButtonCoachmark" /* 17252 */;
import _slicedToArray from "module_32" /* 32 */;
import noop from "module_19" /* 19 */;
import CollectiblesMarketingsStore from "CollectiblesMarketingsStore" /* 7824 */;

const require = fn;
function CoachmarkVariant(shopButtonRef) {
  ({ marketing, navigateToShop } = shopButtonRef);
  shopButtonRef = shopButtonRef.shopButtonRef;
  const tmp = _slicedToArray(navigateToShop(7628).useSelectedVersionedDismissibleContent(navigateToShop(2027).DismissibleContent.COLLECTIBLES_SHOP_ENTRY_MARKETING, marketing.version, undefined, true), 2);
  importDefault = tmp2;
  ...
  const obj = navigateToShop(7628);
  obj3.IconComponent = navigateToShop(12389).ShopIcon;
...
export default function CollectiblesShopEntryButton(navigateToShop) {
  ...
  const stateFromStores = navigateToShop(563).useStateFromStores(items, () => marketingBySurface.getMarketingBySurface(navigateToShop(14249).CollectiblesMarketingSurface.MOBILE_SHOP_BUTTON));
  ...
  if (type === navigateToShop(7805).CollectiblesMarketingType.COACHMARK) {
    ...
    function content(visibleContent) {
      ...
      const obj = { ref: markAsDismissed, IconComponent: navigateToShop(12389).ShopIcon, accessibilityLabel: null, onPress: null, showRedDot: null };
      const intl = navigateToShop(1115).intl;
      ...
      return closure_1_6(shopButtonRef(17250), obj);
    }
    if (tmp4) {
      let type1;
      if (stateFromStores != null) {
      ...
      let prop = null;
      if (type1 === tmp(7805).CollectiblesMarketingType.BADGE) {
      ...
      let tmp15Result = tmp15(tmp(10884).SelectedVersionedDismissibleContent, obj3);
    } else {
      let type2;
      if (stateFromStores != null) {
        type2 = stateFromStores.type;
      }
      if (type2 === tmp(7805).CollectiblesMarketingType.BADGE) {
