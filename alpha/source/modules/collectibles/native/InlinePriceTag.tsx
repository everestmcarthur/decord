...
// Function ID: 13452
// Name: InlinePriceTag
// Dependencies: [19, 17, 7477, 1076, 1074, 21, 4756, 576, 4752, 1973, 7793, 7794, 9115, 1115, 8446, 4414, 13452, 9147, 9126, 13453, 504, 9139, 1364, 9140, 8939, 2]
// Exports: default
...
function OrbsPriceTag(arg0) {
  ...
  if (null == vcData.price) {
    ...
    if (result) {
      result = tmp16(7793).isOrbsExclusiveProduct(product);
      const tmp16Result = tmp16(7793);
    }
    ...
    if (result) {
      result = discountPercentage >= tmp16(7794).DISCOUNT_DISPLAY_MINIMUM_THRESHOLD;
    }
    ...
    if (result) {
      ...
      obj2.icon = React7(tmp16(9115).OrbsIcon, obj3);
    ...
    if (!result) {
      ...
      tmp10Result = tmp10(tmp16(9115).OrbsIcon, obj6);
    }
...
export default function InlinePriceTag(arg0) {
  ...
  if (obj7.useStateFromStores(items, () => fetchingGoogleSkus.isFetchingGoogleSkus())) {
    if (null == formattedPriceForCollectiblesProduct) {
      return closure_9(tmp2(9139).CollectiblesShopPricePlaceholder, {});
    }
  }
  if (null == formattedPriceForCollectiblesProduct) {
    let tmp21 = null;
    ...
    const formattedPriceForCollectiblesProduct1 = tmp2(9126).getFormattedPriceForCollectiblesProduct(product, true, true);
    const tmp2Result = tmp2(9126);
    ...
    if (!(null != formattedPriceForCollectiblesProduct1 && canUseShopDiscountsResult)) {
      ...
      if (tmp28Result5) {
        ...
        obj17.style = items3;
        const items4 = [tmp28(tmp2(8939).NitroWheelIcon, obj17), ];
      ...
      if (shopDiscountSource === tmp2(7794).ShopDiscountSource.THIRDPARTY) {
        const obj23 = { color: "interactive-text-active", style: tmp.nitroIcon };
        let tmp28Result7 = tmp28(tmp2(9140).TagIcon, obj23);
      } else {
        const obj24 = { color: "interactive-text-active", style: tmp.nitroIcon };
        tmp28Result7 = tmp28(tmp2(8939).NitroWheelIcon, obj24);
      }
      obj21.icon = tmp28Result7;
      tmp28(tmp29, obj21);
      tmp2Result6 = tmp2(1364);
    }
    tmp2Result4 = tmp2(7794);
  }
};
