...
// Function ID: 16058
// Name: HeroBlock
// Dependencies: [19, 17, 7738, 1076, 1074, 21, 9004, 4722, 576, 9111, 16058, 1484, 9007, 504, 11730, 4656, 16059, 4423, 4571, 16060, 15281, 7358, 7378, 16062, 1241, 4573, 16064, 16065, 16066, 5156, 4718, 5144, 1115, 12009, 5619, 5292, 7405, 7352, 16067, 8957, 2]
// Exports: default

// Module 16057 (HeroBlock)
import nativeDefault from "native" /* 576 */;
import AnalyticsUtilsDefault from "AnalyticsUtils" /* 1241 */;
import CollectiblesShopCardV2 from "CollectiblesShopCardV2" /* 9004 */;
import CollectiblesAnalyticsContext from "CollectiblesAnalyticsContext" /* 9007 */;
import SkeletonCardDefault from "SkeletonCard" /* 9111 */;
import FeaturedFirstCardCoachmarkAnchorDefault from "FeaturedFirstCardCoachmarkAnchor" /* 16062 */;
...
const result = 0.75 * fn(9004).COLLECTIBLES_SHOP_CARD_WIDTH;
...
export default function _default(heroBlock) {
  ...
  closure_5 = undefined;
  closure_6 = undefined;
  let obj = heroBlock(16058);
  dependencyMap = heroBlock(1484).useNavigation();
  let obj2 = heroBlock(1484);
  noop = heroBlock(9007).useCollectiblesAnalyticsContext();
  ...
  let isEligibleForQuests = heroBlock(11730).getIsEligibleForQuests();
  const tmpResult9 = heroBlock(11730);
  const tmp7 = preferVCPrice(4656)();
  const tmp8 = closure_14();
  const tmpResult10 = heroBlock(16059);
  const token = heroBlock(4423).useToken(preferVCPrice(576).colors.BACKGROUND_BASE_LOW);
  const tmpResult11 = heroBlock(4423);
  const tmpResult12 = heroBlock(4571);
  const tmpResult13 = heroBlock(4571);
  const hexToRgbaStringResult = tmpResult12.hexToRgbaString(heroBlock(4571).hexWithOpacity(token, 0));
  const token1 = heroBlock(4423).useToken(preferVCPrice(576).colors.BACKGROUND_BASE_LOWEST);
  const tmp12 = preferVCPrice(16060)();
  closure_5 = tmp12;
  const items1 = [heroBlock.rankedSkuIds, tmp12];
  const memo = noop.useMemo(() => closure_5(heroBlock.rankedSkuIds), items1);
  const tmpResult14 = heroBlock(4423);
  const filteredAndSortedProducts = heroBlock(15281).useFilteredAndSortedProducts({ products: memo, bypassAndroidUnsyncedFilter: tmp4 });
  closure_6 = tmp14;
  const tmpResult15 = heroBlock(15281);
  let unpublishedAt;
  ...
  if (undefined === stateFromStores) {
    ...
    if (!tmp4) {
      const obj4 = { value: tmp15(preferVCPrice(7378).COLLECTIBLES_SHOP_HERO).analyticsLocations, children: null };
      ...
      if (tmp24Result) {
        ...
        if (tmp4) {
          ...
          tmp22Result = tmp22(tmp6(5156), obj8);
        }
        ...
        items3[2] = tmp22(tmp6(5156), obj12);
      ...
      if (tmp4) {
        ...
        if (tmp22Result6) {
          ...
          tmp22Result6 = tmp22(tmp(4718).Text, obj15);
        }
        ...
        if (tmp22Result7) {
          ...
          tmp22Result7 = tmp22(tmp(4718).Text, obj16);
        }
        ...
        if (isEligibleForQuests) {
          ...
          isEligibleForQuests = tmp22(tmp(5144).Button, obj19);
        }
        ...
        const obj28 = { style: tmp8.heroViewAllIcon, children: tmp22(tmp(7405).ChevronSmallRightIcon, { size: "sm", color: "white" }) };
        items9[1] = tmp22(tmp25, obj28);
        obj24.children = items9;
        obj20.children = tmp24(tmp25, obj24);
        tmp24Result2 = tmp22(tmp(5292).PressableOpacity, obj20, stateFromStores.storeListingId);
      }
      ...
      if (tmp4) {
        ...
        let tmp22Result9 = tmp22(tmp6(16067), obj30);
        const tmp6Result3 = tmp6(16067);
      } else {
        if (0 === filteredAndSortedProducts.length) {
          ...
          obj34.snapToInterval = tmp(9004).COLLECTIBLES_SHOP_CARD_WIDTH + tmp6(576).space.PX_12;
          ...
          tmp22Result10 = tmp22(tmp(8957).FlashList, obj34);
        }
        const obj36 = { children: tmp22Result10 };
        tmp22Result9 = tmp22(closure_12, obj36);
      }
      const obj37 = { children: tmp22Result9 };
      obj29.children = closure_11(tmp(7352).LayerScope, obj37);
      items6[2] = closure_11(closure_5, obj29);
      obj6.children = items6;
      obj5.children = closure_13(closure_5, obj6);
      obj4.children = closure_11(tmp6(16066), obj5);
      return closure_11(tmp(7358).AnalyticsLocationProvider, obj4);
    } else {
      if (tmpResult16.isThemeDark(tmp7)) {
        let tmp6Result4 = tmp6(16064);
      } else {
        tmp6Result4 = tmp6(16065);
      }
      tmpResult16 = tmp(4573);
    }
  }
};
