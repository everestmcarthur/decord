...
// Function ID: 16130
// Name: ShopCategory
// Dependencies: [19, 17, 1076, 1074, 21, 9043, 4756, 576, 7402, 1484, 15336, 16125, 8996, 16130, 9046, 4723, 8444, 7781, 7422, 5334, 1115, 5799, 7449, 2]
// Exports: ShopCategory

// Module 16129 (ShopCategory)
...
const sum = 100 + fn(9043).COLLECTIBLES_SHOP_CARD_HEIGHT;
...
export const ShopCategory = function ShopCategory(category) {
  ...
  const filteredAndSortedProducts = category(15336).useFilteredAndSortedProducts({ products: category.products, bypassAndroidUnsyncedFilter: category.isOrbsExclusive });
  const mobileBannerUrl = category.mobileBannerUrl;
  let obj2 = category(15336);
  let obj3 = { products: category.products, bypassAndroidUnsyncedFilter: category.isOrbsExclusive };
  const collectiblesShopDeepLinkProps = category(16125).useCollectiblesShopDeepLinkProps({ products: filteredAndSortedProducts });
  ...
  const obj4 = category(16125);
  let items = [category.storeListingId];
  const recyclingState = category(8996).useRecyclingState(null, items, () => {
    const current = ref.current;
    if (current != null) {
      current.scrollToOffset({ offset: 0, animated: false });
    }
  });
  const obj6 = category(8996);
  let tmp9 = null != productIndex;
  if (tmp9) {
    tmp9 = productIndex > 0;
  }
  const obj7 = category(16130);
  const scrollToInitialIndexOnce = obj7.useScrollToInitialIndexOnce({ shouldScroll: tmp9, initialScrollIndex: productIndex, flashListRef: ref, afterMs: category(16130).INITIAL_SCROLL_DELAY_MS, resetKey: category.storeListingId });
  const obj8 = { shouldScroll: tmp9, initialScrollIndex: productIndex, flashListRef: ref, afterMs: category(16130).INITIAL_SCROLL_DELAY_MS, resetKey: category.storeListingId };
  collectiblesAnalyticsContext = category(9046).useCollectiblesAnalyticsContext();
  ...
  const tmp4Result = category(9046);
  ...
  if (tmp14Result) {
    ...
    tmp14Result = tmp14(tmp(5799), obj14);
  }
  const items4 = [tmp14Result, ];
  const obj13 = { radius: analyticsLocations(576).radii.lg };
  items4[1] = ref(filteredAndSortedProducts, { style: tmp3.viewAllIcon, children: ref(category(7449).ChevronSmallRightIcon, { size: "sm", color: "white" }) });
  obj11.children = items4;
  const items5 = [collectiblesAnalyticsContext(category(5334).PressableOpacity, obj11, category.storeListingId), ];
  ...
  obj17.snapToInterval = category(9043).COLLECTIBLES_SHOP_CARD_WIDTH + 12;
  ...
  items5[1] = ref(category(8996).FlashList, obj17);
  obj10.children = items5;
  obj9.children = collectiblesAnalyticsContext(filteredAndSortedProducts, obj10);
  return ref(category(9046).CollectiblesAnalyticsProvider, obj9);
};
