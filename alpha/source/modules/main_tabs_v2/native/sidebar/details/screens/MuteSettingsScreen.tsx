...
// Dependencies: [19, 17, 1958, 1980, 4371, 1372, 1074, 21, 4722, 576, 7967, 7315, 7310, 9295, 5775, 1177, 10385, 4718, 1115, 4875, 10386, 1484, 11564, 10384, 1485, 563, 8067, 1612, 2]

// Module 17244 (MuteSettingsScreen)
import nativeDefault from "native" /* 576 */;
import MuteSettingsUtils from "MuteSettingsUtils" /* 9295 */;
...
function UnmuteOptions(channel) {
  ...
  obj2.icon = closure_10(channel(1177).Icon, { disableColor: true, source: navigation(10385) });
  const obj4 = { variant: "text-md/semibold", color: "mobile-text-heading-primary", children: null };
  const intl = channel(1115).intl;
  const obj5 = { name: null };
  const obj3 = { disableColor: true, source: navigation(10385) };
  obj5.name = channel(4875).computeChannelName(channel, UserStore, RelationshipStore, true);
  obj4.children = intl.format(channel(1115).t["eC+9rj"], obj5);
  obj2.label = closure_10(channel(4718).Text, obj4);
  obj2.onPress = callback;
  const items1 = [closure_10(channel(5775).TableRow, obj2), ];
  const obj7 = { muteConfig: channel.muteConfig, type: null };
  const obj6 = channel(4875);
  const tmp6 = navigation(10386);
  const MuteSettingType = channel(10386).MuteSettingType;
