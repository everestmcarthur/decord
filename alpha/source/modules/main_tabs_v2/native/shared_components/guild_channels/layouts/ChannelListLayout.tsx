...
// Dependencies: [8122, 10377, 10379, 10380, 2019, 2]
// Exports: getScaledChannelRowHeight, isLayoutCompact, isLayoutCozy, makeSizeStyle, useMessagesTabLayout

// Module 10376 (ChannelListLayout)
import UserSettings from "UserSettings" /* 2019 */;
import ChannelListLayoutTypes2 from "ChannelListLayoutTypes" /* 8122 */;
...
function getLayoutStyles(layout, launchpad) {
  ...
  if (ChannelListLayoutTypes2.ChannelListLayoutTypes.COZY_DRAWER === layout) {
    return tmp(10377).CHANNEL_LIST_STYLES_COZY_DRAWER;
  } else if (tmp(8122).ChannelListLayoutTypes.COZY_DRAWER_SMOL === layout) {
    return tmp(10377).CHANNEL_LIST_STYLES_COZY_DRAWER_SMOL;
  } else if (tmp(8122).ChannelListLayoutTypes.COMPACT === layout) {
    const tmpResult = tmp(10379);
    return flag ? tmpResult.CHANNEL_LIST_STYLES_COMPACT_LAUNCHPAD : tmpResult.CHANNEL_LIST_STYLES_COMPACT;
  } else {
    if (tmp(8122).ChannelListLayoutTypes.MINIMAL !== layout) {
      const COZY = tmp(8122).ChannelListLayoutTypes.COZY;
    }
    const tmpResult2 = tmp(10380);
...
export const useMessagesTabLayout = function useMessagesTabLayout(panelVariant) {
  ...
  if (panelVariant) {
    ...
    COZY = tmp(8122).ChannelListLayoutTypes.COMPACT;
  } else {
    COZY = tmp(8122).ChannelListLayoutTypes.COZY;
  }
  return COZY;
};
