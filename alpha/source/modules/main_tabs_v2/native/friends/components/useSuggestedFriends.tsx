...
// Dependencies: [32, 19, 7858, 12941, 563, 12, 4566, 2]
// Exports: default

// Module 16303 (useSuggestedFriends)
  ...
  const memo = noop.useMemo(() => {
    if (flag) {
      ...
      return _modDef12.unionBy(found, mapped, (user) => user.user.id).sort((user, user2) => {
        const name = added(4566).getName(user.user);
        const obj = added(4566);
        return name.localeCompare(added(4566).getName(user2.user));
      });
    } else {
      return [];
    }
