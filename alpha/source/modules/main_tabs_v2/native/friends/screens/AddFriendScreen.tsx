...
// Dependencies: [32, 19, 17, 1372, 1074, 12920, 21, 4722, 576, 12922, 4566, 1241, 1115, 8593, 8067, 1364, 5294, 4718, 14064, 14066, 2]
// Exports: default

// Module 17145 (AddFriendScreen)
...
export default function AddFriendScreen(navigation) {
  ...
  let callback;
  let tmp = closure_13();
  const contactSyncAccount = navigation(12922).useContactSyncAccount();
  let obj = navigation(12922);
  let obj2 = navigation(12922);
  [tmp5, c2] = callback(noop.useState(!navigation(12922).isContactSyncEnabled(contactSyncAccount)), 2);
  ...
  const layoutEffect = noop.useLayoutEffect(() => {
    navigation.setOptions({
      headerRight(arg0) {
        ...
        const renderHeaderTextButton = navigation(8067).getRenderHeaderTextButton(intl.string(navigation(1115).t.RDE0Sc), callback);
    ...
    result.then((result) => {
      ...
      if (!tmp5) {
        tmp5 = !tmp2(12922).isContactSyncEnabled(contactSyncAccount);
        const tmp2Result = tmp2(12922);
      }
      dependencyMap(tmp5);
    });
  }, items);
  const items1 = [closure_10(contactSyncAccount(5294), { absolute: true }), ];
  ...
  const items2 = [closure_10(navigation(4718).Text, obj4), , , ];
  ...
  items2[1] = closure_10(navigation(4718).Text, obj5);
  items2[2] = closure_10(contactSyncAccount(14064), { style: tmp.input, autoFocusInput: false, sourcePage: navigation.route.params.sourcePage });
  ...
  const items3 = [closure_10(navigation(4718).Text, obj8), ];
  let tmp10Result = null;
  if (tmp5) {
    const obj9 = { style: tmp.rowContainer, location: "Add Friend Modal" };
    tmp10Result = closure_10(contactSyncAccount(14066), obj9);
  }
