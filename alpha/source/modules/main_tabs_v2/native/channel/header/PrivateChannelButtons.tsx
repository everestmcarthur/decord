...
// Dependencies: [19, 17, 4772, 2041, 1992, 4405, 1372, 4775, 8119, 1074, 4777, 2094, 8120, 4781, 21, 1177, 4756, 576, 504, 11656, 13562, 8238, 13563, 11130, 13433, 5110, 13564, 1115, 13565, 12551, 12610, 4936, 4723, 13580, 1980, 5334, 7291, 13581, 8644, 4451, 1241, 13247, 7620, 4963, 5312, 8125, 8123, 4752, 13582, 10365, 2]

// Module 13561 (PrivateChannelButtons)
...
import getPrivateChannelCallDefault from "getPrivateChannelCall" /* 11130 */;
...
const NO_PARTICIPANTS = fn(4772).NO_PARTICIPANTS;
let closure_12 = fn(8119).setIsChannelDetailsSearchActive;
...
let closure_18 = fn(8120).SearchEntrypointAnalyticsLocations;
...
export default noop.memo(function PrivateChannelButtons(channelId) {
  ...
  inappropriateConversationSafetyToolsWarningForChannel = channelId(11656).useInappropriateConversationSafetyToolsWarningForChannel(channelId);
  closure_6 = tmp7;
  const tmp2Result6 = channelId(11656);
  ...
  const VideoGuardExperiment = tmp2(13562).VideoGuardExperiment;
  ...
  const isCallActiveNullable = channelId(8238).useIsCallActiveNullable(id);
  const tmp2Result9 = channelId(8238);
  ...
  closure_11 = tmp15;
  const tmp17 = screenIndex(13563)({ context: { type: "channel", channel: stateFromStores } });
  ...
  const callback3 = obj9.useCallback(() => {
    ...
    if (tmp2) {
      ...
      ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(13580, dependencyMap.paths), "AppDMOptionsBottomSheet", obj4);
    }
  }, items11);
  ...
  const memo1 = obj9.useMemo(() => {
    if (closure_6) {
      ...
      let tmpResult = tmp(tmp2(13581).SafetyToolsButton, obj3);
    } else {
      ...
      tmpResult = tmp(tmp2(5334).PressableOpacity, obj);
    }
    return tmpResult;
  }, items12);
  if (screenIndex(8644)(channelId)) {
    let obj3 = {
      style: tmp.button,
      onPress() {
    ...
    obj3.children = closure_20(tmp2(13247).WindowLaunchIcon, { size: "sm" });
    let tmp25Result = closure_20(tmp2(5334).PressableOpacity, obj3);
    ...
    if (null != application) {
      ...
      obj5.children = closure_20(tmp2(7291).MagnifyingGlassIcon, { size: "sm" });
      const items13 = [closure_20(tmp2(5334).PressableOpacity, obj5), ];
      ...
      obj6.children = closure_20(tmp2(7620).SettingsIcon, { size: "sm" });
      items13[1] = closure_20(tmp2(5334).PressableOpacity, obj6);
    ...
    if (!tmp15) {
      ...
      if (tmp15) {
        ...
        tmp30(tmp2(5312).VoiceNormalIcon, unsafe_rawColors1);
        ...
        if (stateFromStores1) {
          ...
          let tmp30Result2 = tmp30(tmp2(8125).PhoneHangUpIcon, obj11);
          let tmp33 = tmp30;
        } else {
          let GREEN_360;
          ...
          tmp30Result2 = tmp30(tmp2(8123).PhoneCallIcon, obj12);
          tmp33 = tmp30;
        }
        const items15 = [
          tmp30Result2,
        ...
        if (tmp33Result) {
          ...
          obj13.children = tmp25(tmp2(4752).Text, obj14);
          tmp33Result = tmp33(tmp26, obj13);
        }
        items15[2] = tmp33Result;
        obj8.children = items15;
        const items17 = [tmp25(tmp2(5334).PressableOpacity, obj8), , ];
        let tmp38 = null;
        if (!isMultiUserDMResult) {
          tmp38 = null;
          if (!stateFromStores1) {
            ...
            if (videoEnabled) {
              let VideoDenyIcon = tmp2(10365).VideoIcon;
            } else {
              VideoDenyIcon = tmp2(13582).VideoDenyIcon;
            }
            obj15.children = tmp33(VideoDenyIcon, { size: "sm" });
            tmp33(tmp2(5334).PressableOpacity, obj15);
          }
        }
        items17[1] = tmp38;
        items17[2] = memo1;
