...
// Dependencies: [19, 17, 2041, 2105, 2063, 4405, 4937, 1372, 16823, 1074, 21, 576, 16785, 1364, 16786, 8535, 8618, 504, 16829, 16830, 4752, 1115, 1177, 4908, 5732, 8620, 8621, 11176, 11906, 16824, 16826, 11, 5334, 9866, 16832, 2]
// Exports: default

// Module 16828 (ICYMIMessageRow)
...
function ReplyMessageContent(message) {
  ...
  const context = noop.useContext(message(16786).ICYMIContext);
  ...
  const width = obj.useContext(tmp2(16786).ICYMIContext).width;
  let tmp8 = null;
  if (null != stateFromStores) {
    ...
    const items2 = [closure_14(tmp2(4752).Text, obj5), ];
    ...
    obj10.style = obj11;
    obj10.children = tmp2(4908).getName(guild.id, channel.id, stateFromStores);
    const items4 = [closure_14(tmp2(4752).Text, obj10), ];
    ...
    items4[1] = closure_14(tmp2(16786).ICYMIContext.Provider, obj12);
...
export default function MessageRowWrapper(arg0) {
  ...
  gravityMessage = gravityMessage(8620).useGravityMessage(message);
  const obj = gravityMessage(8620);
