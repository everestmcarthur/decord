...
// Dependencies: [7766, 2063, 4405, 1372, 1074, 1115, 4600, 7874, 2009, 38, 2]
// Exports: default, getFriendRequestSentBody

// Module 16757 (getNotificationCenterItemBody)
...
export default function getNotificationCenterItemBody(arg0) {
  ...
  if (NotificationCenterItemsTypes.NotificationCenterLocalItems.FRIEND_REQUESTS_GROUPED === type) {
    ...
    const name2 = tmp2(4600).getName(obj2.getUser(id2));
    ...
    let num;
    const name3 = tmp2(4600).getName(obj2.getUser(id3));
    ...
    return intl12.format(tmp6(1115).t.g5xyIC, obj3);
  } else if (tmp6(7874).NotificationCenterLocalItems.MOBILE_NATIVE_UPDATE_AVAILABLE === type) {
    let str7;
  ...
  } else if (tmp6(7874).NotificationCenterItems.FRIEND_SUGGESTION_CREATED === type) {
    if (RelationshipStore.getRelationshipType(id) === constants.PENDING_OUTGOING) {
      const intl11 = tmp6(1115).intl;
      const obj4 = { user: name1 };
      let str5 = intl11.format(tmp6(1115).t.gZVTy2, obj4);
    } else {
      str5 = item.body;
      if (str5 == null) {
        str5 = "";
    ...
    return str5;
  } else if (tmp6(7874).NotificationCenterItems.GUILD_SCHEDULED_EVENT_STARTED === type) {
    ...
    if (!tmp6Result.isNullOrEmpty(name4)) {
      if (!tmp6Result2.isNullOrEmpty(name)) {
        if (tmp27Result) {
          ...
          let formatResult = intl10.format(tmp6(1115).t.AyvfXR, obj5);
        }
        return formatResult;
      }
      tmp6Result2 = tmp6(2009);
    }
    let str4 = item.body;
    if (str4 == null) {
      str4 = "";
    ...
    formatResult = str4;
    tmp6Result = tmp6(2009);
  } else if (tmp6(7874).NotificationCenterLocalItems.INCOMING_FRIEND_REQUESTS === type) {
    if (null != applicationId) {
      ...
      let formatResult1 = intl9.format(tmp6(1115).t["9Dgf1L"], obj6);
    } else {
  ...
  } else if (tmp6(7874).NotificationCenterLocalItems.INCOMING_FRIEND_REQUESTS_ACCEPTED === type) {
    if (null != applicationId) {
      ...
      let formatResult2 = intl7.format(tmp6(1115).t.nnC1q9, obj8);
    } else {
  ...
  } else if (tmp6(7874).NotificationCenterItems.FRIEND_REQUEST_ACCEPTED === type) {
    if (null != applicationId) {
      ...
      let formatResult3 = intl5.format(tmp6(1115).t.jXlYiF, obj10);
    } else {
  ...
  } else if (tmp6(7874).NotificationCenterLocalItems.INCOMING_GAME_FRIEND_REQUESTS_ACCEPTED === type) {
    ...
    return intl3.format(tmp6(1115).t["BB/0vn"], obj12);
  } else if (tmp6(7874).NotificationCenterLocalItems.INCOMING_GAME_FRIEND_REQUESTS === type) {
    ...
    return intl2.format(tmp6(1115).t["7cqOLI"], obj13);
  } else if (tmp6(7874).NotificationCenterItems.GAME_FRIEND_REQUEST_ACCEPTED === type) {
    if (null == applicationId) {
      let body = item.body;
    } else {
      ...
      body = intl.format(tmp6(1115).t.Wi64vN, obj14);
    }
    return body;
  } else {
    let str = item.body;
