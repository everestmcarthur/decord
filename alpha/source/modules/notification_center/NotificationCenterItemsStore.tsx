...
// Dependencies: [4637, 7722, 4372, 4371, 1372, 1074, 7836, 4945, 7837, 11, 504, 573, 2]

// Module 7835 (NotificationCenterItemsStore)
import SnowflakeUtilsDefault from "SnowflakeUtils" /* 11 */;
import initializeDefault from "initialize" /* 504 */;
import DispatcherDefault from "Dispatcher" /* 573 */;
import NotificationCenterItemsTypes from "NotificationCenterItemsTypes" /* 7836 */;
import NotificationCenterUtils from "NotificationCenterUtils" /* 7837 */;
...
function toNotificationCenterItem(item_enum) {
  ...
  if (tmp3) {
    tmp3 = item_enum.type === tmp(7836).NotificationCenterItems.LIFECYCLE_ITEM;
  }
  if (tmp3) {
  ...
  if (null != item_enum.message) {
    messageRecord = tmp(4945).createMessageRecord(item_enum.message);
...
function handleAddItem(type) {
  if ("NOTIFICATION_CENTER_ITEM_CREATE" === type.type) {
    ...
    if (tmp3) {
      tmp3 = item2.type === tmp(7836).NotificationCenterItems.LIFECYCLE_ITEM;
    }
    if (tmp3) {
    ...
    if (null != item2.message) {
      messageRecord = tmp(4945).createMessageRecord(item2.message);
      const tmpResult = tmp(4945);
    }
    obj.message = messageRecord;
    let id;
...
function handleRelationshipAddOrUpdate(relationship) {
  ...
  if (tmp14) {
    ...
    obj.notifCenterLocalItems = prop1.filter((type) => {
      ...
      if (!tmp4) {
        let tmp7 = type.type === tmp(7836).NotificationCenterLocalItems.INCOMING_FRIEND_REQUESTS_ACCEPTED;
        if (tmp7) {
      ...
      if (!tmp4) {
        let tmp10 = type.type === tmp(7836).NotificationCenterLocalItems.INCOMING_GAME_FRIEND_REQUESTS;
        if (tmp10) {
      ...
      if (!tmp4) {
        let tmp13 = type.type === tmp(7836).NotificationCenterLocalItems.INCOMING_GAME_FRIEND_REQUESTS_ACCEPTED;
        if (tmp13) {
...
let obj = { loading: false, initialized: false, errored: false, isDataStale: false, notifCenterItems: [], staleNotifCenterItems: [], notifCenterIds: new Set(), notifCenterLocalItems: [], paginationHasMore: true, paginationCursor: "flex", notifCenterActive: "flag_cv", notifCenterTabFocused: null };
...
const notificationCenterItemsStore = new NotificationCenterItemsStore(DispatcherDefault, {
  ...
  LOGOUT() {
    ...
    obj = { loading: false, initialized: false, errored: false, isDataStale: false, notifCenterItems: [], staleNotifCenterItems: [], notifCenterIds: new Set(), notifCenterLocalItems: null, paginationHasMore: true, paginationCursor: "flex", notifCenterActive: "flag_cv", notifCenterTabFocused: null };
    if (flag) {
