...
// Dependencies: [5, 19, 17, 2041, 1372, 1074, 21, 4756, 1110, 4492, 4757, 5184, 5186, 1115, 4752, 563, 8233, 16377, 4736, 14113, 11131, 7874, 4769, 10002, 4454, 11918, 1241, 2]
// Exports: ForYouItemActionButtons, useItemActionButtonPropsV2

// Module 16756 (ForYouItemActionButtons)
...
import NotificationCenterItemsTypes from "NotificationCenterItemsTypes" /* 7874 */;
...
import handleSupportedURLDefault from "handleSupportedURL" /* 14113 */;
...
export const useItemActionButtonPropsV2 = function useItemActionButtonPropsV2(other_user, callback, navigation, forceHoistItem, isForceHoisted, onSoftAckItem, arg6, compactMode) {
  ...
  sharedValue = tmp2(4492).useSharedValue(false);
  ...
  const callback4 = noop.useCallback(() => {
    ...
    dMChannel.then((channelId) => {
      closure_1(14113)({ payload: closure_1(4736)("https://discord.com/channels/@me/" + channelId).payload, safe: true, navigationReplace: false });
      let obj;
      let tmp3;
      if (null != channelId) {
        obj = { channelId };
  ...
  if (other_user.disable_action) {
    ...
    if (other_user.type !== tmp2(7874).NotificationCenterLocalItems.INCOMING_FRIEND_REQUESTS) {
      if (other_user.type !== tmp2(7874).NotificationCenterLocalItems.INCOMING_FRIEND_REQUESTS_ACCEPTED) {
        if (other_user.type !== tmp2(7874).NotificationCenterLocalItems.INCOMING_GAME_FRIEND_REQUESTS) {
          if (other_user.type === tmp2(7874).NotificationCenterLocalItems.FRIEND_REQUESTS_GROUPED) {
            ...
            return obj4;
          } else if (other_user.type === tmp2(7874).NotificationCenterItems.GO_LIVE_PUSH) {
            ...
            if (other_user.type !== tmp2(7874).NotificationCenterLocalItems.INCOMING_FRIEND_REQUESTS_ACCEPTED) {
              if (other_user.type !== tmp2(7874).NotificationCenterItems.DM_FRIEND_NUDGE) {
                if (other_user.type !== tmp2(7874).NotificationCenterItems.FRIEND_REQUEST_ACCEPTED) {
                  if (other_user.type !== tmp2(7874).NotificationCenterItems.GAME_FRIEND_REQUEST_ACCEPTED) {
                    if (other_user.type === tmp2(7874).NotificationCenterItems.FRIEND_SUGGESTION_CREATED) {
                      ...
                      return obj10;
                    } else if (other_user.type === tmp2(7874).NotificationCenterItems.GUILD_SCHEDULED_EVENT_STARTED) {
                      ...
                      return obj13;
                    } else if (other_user.type === tmp2(7874).NotificationCenterItems.LIFECYCLE_ITEM) {
                      const item_enum = other_user.item_enum;
                      if (tmp2(7874).ItemEnum.UPDATE_PROFILE === item_enum) {
                        const intl5 = tmp2(1115).intl;
                        let stringResult = intl5.string(tmp2(1115).t.zMRcWL);
                        let str = "update_profile";
                      } else if (tmp2(7874).ItemEnum.FIND_FRIENDS === item_enum) {
                        const intl4 = tmp2(1115).intl;
                        stringResult = intl4.string(tmp2(1115).t["vwL/4s"]);
                        str = "find_friends";
                      } else if (tmp2(7874).ItemEnum.ADD_FRIEND === item_enum) {
                        ...
                        str = null;
                        stringResult = null;
                        if (tmp2(7874).ItemEnum.FIRST_MESSAGE === item_enum) {
                      ...
                      if (other_user.type !== tmp2(7874).NotificationCenterItems.RECENT_MENTION) {
                        if (other_user.type !== tmp2(7874).NotificationCenterItems.REPLY_MENTION) {
                          if (other_user.type === tmp2(7874).NotificationCenterItems.TRENDING_CONTENT) {
