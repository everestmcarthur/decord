...
// Dependencies: [19, 17, 14177, 1980, 16816, 1074, 21, 4722, 576, 4458, 1478, 4656, 14178, 1364, 1397, 5760, 4723, 4726, 504, 7139, 11768, 4573, 16817, 16818, 5754, 4718, 1177, 5292, 4420, 1115, 16819, 2]

// Module 16815 (GuildFeedBanner)
import nativeDefault from "native" /* 576 */;
import PlatformUtils from "PlatformUtils" /* 1364 */;
import AvatarUtilsDefault from "AvatarUtils" /* 1397 */;
import timing from "timing" /* 4723 */;
import timingPresets from "timingPresets" /* 4726 */;
import GuildPopoutActionCreators from "GuildPopoutActionCreators" /* 14178 */;
import noop from "module_19" /* 19 */;
import GuildPopoutStore from "GuildPopoutStore" /* 14177 */;
...
function GuildFeedBanner(guild) {
  ...
  let tmp = closure_13();
  const sharedValue = guild(4458).useSharedValue(0);
  let obj = guild(4458);
  class G {
    constructor() {
  ...
  const animatedStyle = guild(4458).useAnimatedStyle(G);
  ...
  const tmp8 = sharedValue(4656)();
  const guildBadgeSource = guild(5760).getGuildBadgeSource(guild);
  const tmp2Result = guild(5760);
  ...
  const tmp12 = sharedValue(7139)();
  dependencyMap = tmp12;
  const size = tmp6(1478)();
  width = size.width;
  height = size.height;
  const tmp2Result4 = guild(504);
  drawerWidth = guild(11768).useDrawerWidth();
  ...
  if (null != memo) {
    ...
    if (tmp2Result6.isThemeDark(tmp8)) {
      let tmp6Result = tmp6(16817);
    } else {
      tmp6Result = tmp6(16818);
    }
    obj6.source = tmp6Result;
    obj6.onLoad = handleLoad;
    tmp2Result6 = tmp2(4573);
  }
  ...
  const tmp2Result5 = guild(11768);
  obj10.size = guild(5754).GuildIconSizes.XLARGE;
  obj9.children = closure_11(sharedValue(5754), obj10);
  const items10 = [closure_11(height, obj9), , , ];
  const obj11 = { style: tmp.textContainer, children: null };
  const items11 = [closure_11(guild(4718).Text, { lineClamp: 1, variant: "heading-xl/extrabold", color: "mobile-text-heading-primary", style: tmp.title, children: guild.name }), ];
  ...
  if (null != description) {
    ...
    tmp17Result2 = tmp17(tmp2(4718).Text, obj14);
  }
  ...
  if (tmp15Result4) {
    ...
    if (features.has(GuildFeatures.DISCOVERABLE)) {
      const obj16 = {
        ...
        onPress() {
              ...
              sharedValue(4420).open(obj2);
            },
        children: null
      };
      const obj17 = { style: tmp.publicIcon, source: tmp6(16819) };
      ...
      items12[1] = tmp17(tmp2(4718).Text, obj18);
      obj16.children = items12;
      tmp15Result = tmp15(tmp2(5292).PressableOpacity, obj16);
    }
    ...
    if (null != discoverableGuild.presenceCount) {
      ...
      if (null != discoverableGuild.memberCount) {
        ...
        items14[1] = tmp17(tmp2(4718).Text, obj21);
...
const GuildFeedConstants = fn(16816);
