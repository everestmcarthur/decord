...
// Dependencies: [21, 8959, 9167, 8956, 1115, 8354, 9169, 8356, 8358, 9171, 9173, 9175, 2]
// Exports: default
...
export default function getGameProfileWebsiteData(category, color) {
  ...
  if (ThirdPartyGameApplicationWebsiteCategory.ThirdPartyGameApplicationWebsiteCategory.OFFICIAL === category) {
    ...
    obj2.icon = jsx(tmp(9167).GlobeEarthIcon, { size: "md", color });
    obj2.action = tmp(8956).GameProfileTrackActionActions.WebsiteLink;
    const intl8 = tmp(1115).intl;
    obj2.title = intl8.string(tmp(1115).t.fOUKvg);
    obj2.url = category.url;
    return obj2;
  } else if (tmp(8959).ThirdPartyGameApplicationWebsiteCategory.TWITTER === category) {
    const obj4 = { icon: null, action: null, title: null, url: null };
    const obj5 = { size: "md", color };
    obj4.icon = jsx(tmp(8354).XNeutralIcon, { size: "md", color });
    obj4.action = tmp(8956).GameProfileTrackActionActions.XLink;
    const intl7 = tmp(1115).intl;
    obj4.title = intl7.string(tmp(1115).t.INic4y);
    obj4.url = category.url;
    return obj4;
  } else if (tmp(8959).ThirdPartyGameApplicationWebsiteCategory.YOUTUBE === category) {
    const obj6 = { icon: null, action: null, title: null, url: null };
    const obj7 = { size: "md", color };
    obj6.icon = jsx(tmp(9169).YoutubeNeutralIcon, { size: "md", color });
    obj6.action = tmp(8956).GameProfileTrackActionActions.YouTubeLink;
    const intl6 = tmp(1115).intl;
    obj6.title = intl6.string(tmp(1115).t.lNmxbE);
    obj6.url = category.url;
    return obj6;
  } else if (tmp(8959).ThirdPartyGameApplicationWebsiteCategory.FACEBOOK === category) {
    const obj8 = { icon: null, action: null, title: null, url: null };
    const obj9 = { size: "md", color };
    obj8.icon = jsx(tmp(8356).FacebookNeutralIcon, { size: "md", color });
    obj8.action = tmp(8956).GameProfileTrackActionActions.FacebookLink;
    const intl5 = tmp(1115).intl;
    obj8.title = intl5.string(tmp(1115).t.FjyREK);
    obj8.url = category.url;
    return obj8;
  } else if (tmp(8959).ThirdPartyGameApplicationWebsiteCategory.INSTAGRAM === category) {
    const obj10 = { icon: null, action: null, title: null, url: null };
    const obj11 = { size: "md", color };
    obj10.icon = jsx(tmp(8358).InstagramNeutralIcon, { size: "md", color });
    obj10.action = tmp(8956).GameProfileTrackActionActions.InstagramLink;
    const intl4 = tmp(1115).intl;
    obj10.title = intl4.string(tmp(1115).t["cgR+IK"]);
    obj10.url = category.url;
    return obj10;
  } else if (tmp(8959).ThirdPartyGameApplicationWebsiteCategory.BLUESKY === category) {
    const obj12 = { icon: null, action: null, title: null, url: null };
    const obj13 = { size: "md", color };
    obj12.icon = jsx(tmp(9171).BlueskyNeutralIcon, { size: "md", color });
    obj12.action = tmp(8956).GameProfileTrackActionActions.BlueskyLink;
    const intl3 = tmp(1115).intl;
    obj12.title = intl3.string(tmp(1115).t["D/PHq5"]);
    obj12.url = category.url;
    return obj12;
  } else if (tmp(8959).ThirdPartyGameApplicationWebsiteCategory.REDDIT === category) {
    const obj14 = { icon: null, action: null, title: null, url: null };
    const obj15 = { size: "md", color };
    obj14.icon = jsx(tmp(9173).RedditNeutralIcon, { size: "md", color });
    obj14.action = tmp(8956).GameProfileTrackActionActions.RedditLink;
    const intl2 = tmp(1115).intl;
    obj14.title = intl2.string(tmp(1115).t["Hgb+fc"]);
    obj14.url = category.url;
    return obj14;
  } else if (tmp(8959).ThirdPartyGameApplicationWebsiteCategory.TWITCH === category) {
    const obj = { icon: null, action: null, title: null, url: null };
    const obj16 = { size: "md", color };
    obj.icon = jsx(tmp(9175).TwitchNeutralIcon, { size: "md", color });
    obj.action = tmp(8956).GameProfileTrackActionActions.TwitchLink;
