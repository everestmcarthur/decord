...
// Dependencies: [502, 2041, 4405, 2095, 1372, 7839, 7835, 1438, 11, 7837, 7836, 1370, 4978, 504, 573, 2]

// Module 7838 (ConversationsStore)
...
import ConversationVisibilityStore from "ConversationVisibilityStore" /* 7839 */;
  ...
  if (null != peekResult) {
    ...
    if (null != message) {
      ...
      if (flag2) {
        ...
        const result = tmp4(7837).replaceHydratedMessage(tmp7, messageId, applyReactionResult);
        flag2 = true;
        const tmp4Result = tmp4(7837);
      }
      flag = flag2;
      tmp4 = require;
    }
    ...
    const item = prop.forEach((message, index) => {
      if (null != message.message) {
        ...
        if (null != result) {
          ...
          const result1 = tmp5(7837).replaceHydratedMessage(tmp, index, result);
          const tmp5Result = tmp5(7837);
        }
        tmp5 = require;
      }
    });
...
const conversationsStore = new ConversationsStore(DispatcherDefault, {
  ...
  CONVERSATIONS_FETCH_SUCCESS: function handleFetchSuccess(requestKey) {
    ...
    if (removePendingListFetch(channelId, requestKey.requestKey)) {
      const mapped = rawConversations.map(set(7836).mapConversation);
      ...
      if (null != peekResult) {
        ...
        if (null != message1) {
          ...
          if (flag2) {
            ...
            const result = tmp6(7837).replaceHydratedMessage(tmp9, id, updateMessageRecordResult);
            flag2 = true;
