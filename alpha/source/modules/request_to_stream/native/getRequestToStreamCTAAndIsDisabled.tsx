...
// Dependencies: [32, 502, 12017, 11, 12005, 1115, 2968, 2]
// Exports: default

// Module 13542 (getRequestToStreamCTAAndIsDisabled)
import SnowflakeUtilsDefault from "SnowflakeUtils" /* 11 */;
import util from "util" /* 1115 */;
import _modDef2968 from "module_2968" /* 2968 */;
import isInviteActive from "isInviteActive" /* 12005 */;
import useCanFulfillStreamRequest from "useCanFulfillStreamRequest" /* 12017 */;
...
export default function getRequestToStreamCTAAndIsDisabled(id) {
  ...
  if (tmp9) {
    ...
    text = stringResult;
    if (!tmp3[0]) {
      if (tmp(12017).StreamRequestUnfulfillableReason.ALREADY_STREAMING === tmp4) {
        const intl4 = tmp(1115).intl;
        text = intl4.string(tmp6(2968).P0wwmM);
        isDisabled = true;
      } else if (tmp(12017).StreamRequestUnfulfillableReason.NOT_RUNNING_GAME === tmp4) {
        const intl3 = tmp(1115).intl;
        text = intl3.string(tmp6(2968)["43zohO"]);
        isDisabled = true;
      } else if (tmp(12017).StreamRequestUnfulfillableReason.NOT_IN_VOICE_CHANNEL === tmp4) {
        ...
        isDisabled = false;
        text = stringResult;
        if (tmp(12017).StreamRequestUnfulfillableReason.NO_PERMISSION === tmp4) {
