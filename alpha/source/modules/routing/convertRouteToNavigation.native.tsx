...
// Dependencies: [1074, 4616, 4613, 4614, 4582, 4595, 2]
// Exports: convertRouteToNavigation

// Module 13042 (convertRouteToNavigation)
    ...
    if (rootNavigationRef.isReady()) {
      if (pathname.startsWith("/channels/")) {
        ...
        if (null != tmpResult.matchPath(pathname, obj2)) {
          ...
          const RouteParam6 = tmp(4595).RouteParam;
          const tmpResult15 = tmp(4582);
          const RouteParam7 = tmp(4595).RouteParam;
            ...
            if (tmpResult16.getChatLayout().isChatLockedOpen) {
              if (null != channelId) {
                if (false === navigationReplace) {
                  const tmpResult17 = tmp(4613);
                  const rootNavigationRef1 = tmp(4614).getRootNavigationRef();
                  let currentRoute;
                  ...
                  if (channelId1 === channelId) {
                    ...
                    tmp(4613).navigateToRootTab(obj4);
                    const tmpResult19 = tmp(4613);
                  } else {
                    const obj6 = { channelId, guildId, messageId, replaceChannelAndFixRoot: navigationReplace };
                    tmp(4613).navigateToChannel(obj6);
                    const tmpResult20 = tmp(4613);
                  }
                  const tmpResult18 = tmp(4614);
                }
              }
              const obj7 = { screen: "guilds", guildId, channelId, resetRoot: navigationReplace };
              tmp(4613).navigateToRootTab(obj7);
              const tmpResult21 = tmp(4613);
      ...
      if (pathname.startsWith("/member-verification/")) {
        ...
        const RouteParam5 = tmp(4595).RouteParam;
        obj12.path = Routes.GUILD_MEMBER_VERIFICATION(RouteParam5.guildId());
        const matchPathResult1 = tmp(4582).matchPath(pathname, obj12);
        if (null != matchPathResult1) {
          const result = tmp(4613).navigateToMemberVerification(matchPathResult1.params.guildId, matchPathResult1.params.inviteCode);
          const tmpResult27 = tmp(4613);
        }
        return true;
      } else {
        ...
        tmp(4613).resetToAuthRoute();
        flag = true;
        const tmpResult28 = tmp(4613);
      }
    }
  }
  return true;
