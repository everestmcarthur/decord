...
// Dependencies: [1074, 4582, 4579, 4580, 4548, 4561, 2]
// Exports: convertRouteToNavigation

// Module 13033 (convertRouteToNavigation)
  ...
  if (null != rootNavigationRef) {
    if (rootNavigationRef.isReady()) {
      if (pathname.startsWith("/channels/")) {
        ...
        const RouteParam = tmp(4561).RouteParam;
        const obj5 = Routes;
        const tmpResult = tmp(4548);
        const RouteParam2 = tmp(4561).RouteParam;
        ...
        if (null != tmpResult.matchPath(pathname, obj2)) {
          ...
          const RouteParam6 = tmp(4561).RouteParam;
          const tmpResult15 = tmp(4548);
          const RouteParam7 = tmp(4561).RouteParam;
            ...
            if (tmpResult16.getChatLayout().isChatLockedOpen) {
              if (null != channelId) {
                if (false === navigationReplace) {
                  const tmpResult17 = tmp(4579);
                  const rootNavigationRef1 = tmp(4580).getRootNavigationRef();
                  let currentRoute;
      ...
      if (pathname.startsWith("/member-verification/")) {
        ...
        const RouteParam5 = tmp(4561).RouteParam;
        obj12.path = Routes.GUILD_MEMBER_VERIFICATION(RouteParam5.guildId());
        const matchPathResult1 = tmp(4548).matchPath(pathname, obj12);
        if (null != matchPathResult1) {
          const result = tmp(4579).navigateToMemberVerification(matchPathResult1.params.guildId, matchPathResult1.params.inviteCode);
          const tmpResult27 = tmp(4579);
        }
        return true;
      } else {
