...
// Dependencies: [32, 19, 17, 7448, 2038, 1085, 21, 576, 4756, 7628, 2027, 8434, 11309, 1973, 4723, 14901, 1980, 1115, 14888, 5789, 9102, 1177, 13474, 2]
// Exports: default

// Module 14900 (UserProfileFrameEditButton)
...
export default function UserProfileFrameEditButton(arg0) {
  ...
  const tmp4 = userProfileFrame(user(7628).useSelectedDismissibleContent(items), 2);
  dependencyMap = tmp5;
  const obj = user(7628);
  ...
  const fetchCollectiblesProduct = user(11309).useFetchCollectiblesProduct(skuId);
  ...
  const tmp2Result = user(11309);
  userProfileFrame = user(8434).useUserProfileFrame({ user, guildId });
  ...
  const callback = noop.useCallback(() => {
    ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(14901, dependencyMap.paths), "Profile Frame", { user, currentProfileFrame: userProfileFrame, guildId });
  ...
  if (fetchCollectiblesProduct.isFetching) {
    ...
    obj5.onPress = NOOP;
    obj5.leading = tmp18(tmp2(5789).ActivityIndicator, { animating: true, size: "large" });
    let obj6 = obj5;
  } else {
    ...
    obj6.labelTrailing = tmp18(tmp2(14888).UserProfileEditFormLabelBadges, obj7);
    ...
    if (null != first1) {
      ...
      obj9.children = tmp18(guildId(9102), obj10);
      let tmp18Result = tmp18(View, obj9);
      const tmp23 = guildId(9102);
    } else {
      const obj11 = { source: guildId(13474), style: tmp.noneIcon };
      tmp18Result = tmp18(tmp2(1177).Icon, obj11);
    }
    obj6.leading = tmp18Result;
  }
  return jsx(user(14888).UserProfileEditFormButton, obj6);
};
