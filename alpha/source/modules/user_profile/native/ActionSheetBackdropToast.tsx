...
// Dependencies: [19, 17, 7391, 21, 1364, 4756, 576, 1612, 1478, 5894, 4492, 4757, 4752, 2]
...
export const ActionSheetBackdropToast = function ActionSheetBackdropToast(children) {
  ...
  const sharedValue1 = isExpanded(4492).useSharedValue(0);
  ...
  const effect = sharedValue.useEffect(() => {
    ...
    return () => {
      const obj = isExpanded(4492);
      const result = sharedValue.set(obj.withDelay(200, isExpanded(4757).withTiming(0)));
      const obj2 = isExpanded(4757);
      const obj4 = { duration: 200, easing: null };
      const Easing = isExpanded(4492).Easing;
      obj4.easing = Easing.out(isExpanded(4492).Easing.exp);
      const result1 = sharedValue1.set(isExpanded(4757).withTiming(0, obj4));
    };
  }, items);
  let obj2 = isExpanded(4492);
  const fn = function x() {
    if (closure_7) {
      if (tmp) {
        let sum = c2 + 24;
  ...
  obj5.style = items1;
  const animatedStyle = isExpanded(4492).useAnimatedStyle(fn);
  const obj6 = { style: null, children: jsx(isExpanded(4752).Text, { variant: "text-sm/medium", color: "mobile-text-heading-primary", children: children.text }) };
  const items2 = [tmp.toast, animatedStyle];
  obj6.style = items2;
  obj5.children = jsx(ReanimatedRexportDefault.View, { style: null, children: jsx(isExpanded(4752).Text, { variant: "text-sm/medium", color: "mobile-text-heading-primary", children: children.text }) });
