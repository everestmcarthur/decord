...
// Dependencies: [19, 17, 7347, 21, 1364, 4722, 576, 1612, 1478, 5852, 4458, 4723, 4718, 2]
// Exports: ActionSheetBackdropToast

// Module 13380 (ActionSheetBackdropToast)
...
const ACTION_SHEET_START_HEIGHT_RATIO = fn(7347).ACTION_SHEET_START_HEIGHT_RATIO;
const jsx = fn(21).jsx;
...
export const ActionSheetBackdropToast = function ActionSheetBackdropToast(children) {
  ...
  const diff = height - isExpanded(5852).NAV_BAR_HEIGHT_MULTILINE - useSafeAreaInsetsDefault().top;
  dependencyMap = diff;
  const sharedValue = isExpanded(4458).useSharedValue(0);
  let obj = isExpanded(4458);
  const sharedValue1 = isExpanded(4458).useSharedValue(0);
  ...
  const effect = sharedValue.useEffect(() => {
    ...
    return () => {
      const obj = isExpanded(4458);
      const result = sharedValue.set(obj.withDelay(200, isExpanded(4723).withTiming(0)));
      const obj2 = isExpanded(4723);
      const obj4 = { duration: 200, easing: null };
      const Easing = isExpanded(4458).Easing;
      obj4.easing = Easing.out(isExpanded(4458).Easing.exp);
      const result1 = sharedValue1.set(isExpanded(4723).withTiming(0, obj4));
    };
  }, items);
  let obj2 = isExpanded(4458);
  const fn = function x() {
    if (closure_7) {
      if (tmp) {
        let sum = c2 + 24;
  ...
  obj5.style = items1;
  const animatedStyle = isExpanded(4458).useAnimatedStyle(fn);
  const obj6 = { style: null, children: jsx(isExpanded(4718).Text, { variant: "text-sm/medium", color: "mobile-text-heading-primary", children: children.text }) };
  const items2 = [tmp.toast, animatedStyle];
  obj6.style = items2;
  obj5.children = jsx(ReanimatedRexportDefault.View, { style: null, children: jsx(isExpanded(4718).Text, { variant: "text-sm/medium", color: "mobile-text-heading-primary", children: children.text }) });
