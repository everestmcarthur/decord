...
// Dependencies: [19, 17, 21, 4722, 576, 1092, 7400, 5292, 1115, 10491, 4718, 8416, 8457, 8362, 4841, 14811, 4689, 14840, 1897, 8142, 2]
// Exports: default

// Module 14839 (UserProfileEditTheme)
...
export default function UserProfileEditTheme(pendingThemeColors) {
  ...
  const tmp = closure_6();
  let tmp4 = f99201(primaryColor[11])(user.id, guildId);
  f99201 = tmp4;
  const tmp5 = f99201(primaryColor[12])({ user, displayProfile: tmp4, pendingThemeColors: pendingThemeColors.pendingThemeColors, isPreview: flag });
  primaryColor = tmp5.primaryColor;
  secondaryColor = tmp5.secondaryColor;
  if (pendingAvatarSrc == null) {
    pendingAvatarSrc = user.getAvatarURL(guildId, 80);
  }
  suggestedColors = secondaryColor(primaryColor[13]).useAvatarColors(pendingAvatarSrc, f99201(tmp3[4]).unsafe_rawColors.PRIMARY_530, false);
    ...
    if (null != secondaryColor) {
      ...
      if (showResetMenu) {
        ...
        obj5.onPress = function onPress() {
          ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(14840, dependencyMap.paths), "Profile Theme", {
            onResetTheme() {
      ...
      f99201 = (arg0) => {
        if (arg0 !== primaryColor) {
          ...
          let themeColors;
          if (f99201 != null) {
            themeColors = f99201.themeColors;
          }
          let tmp8;
      ...
      obj8.onPress = () => {
        closure_1(primaryColor[15])({ color: secondaryColor, onSelect: f99201, suggestedColors });
      };
