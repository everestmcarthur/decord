...
// Dependencies: [19, 17, 5718, 2109, 2041, 1181, 4938, 21, 4756, 17122, 576, 16677, 7509, 504, 4909, 8128, 15583, 7569, 5193, 17439, 5219, 12310, 10364, 8122, 17441, 9599, 16565, 17435, 5334, 17442, 17121, 17436, 17437, 2]

// Module 17438 (shared/TextChannel)
...
export default noop.memo(function TextChannel(subtitle) {
  ...
  const channelUnreadBadgeState = channel(16677).useChannelUnreadBadgeState(channel, flag);
  ...
  let obj = channel(16677);
  const obj2 = channel(7509);
  ...
  const unreadThreadsCountForParent = channel(8128).useUnreadThreadsCountForParent(channel.guild_id, channel.id);
  let tmp12 = unread;
  const tmp2Result = channel(8128);
  if (unread) {
    tmp12 = !flag;
  }
  const tmp8ResultResult = arr4(15583)(channel, { unread: tmp12 });
  const tmp8Result = arr4(15583);
  const isChannelSpoilerGated = channel(7569).useIsChannelSpoilerGated(channel);
  const tmp2Result8 = channel(7569);
  ...
  const tmp17 = arr4(17439)();
  const tmp2Result10 = channel(504);
  ({ isSubscriptionGated, needSubscriptionToAccess } = arr4(5219)(channel.id));
  arr4 = tmp8(12310)(channel);
  if (null != tmp8ResultResult) {
    if (!isChannelSpoilerGated) {
      const obj5 = { channel, message: tmp8ResultResult, color: "text-muted", muted: flag, layout: tmp2(8122).ChannelListLayoutTypes.COMPACT };
      let result = closure_10(tmp2(10364).ChannelRowPreview, obj5);
    }
    dependencyMap = tmp22;
    const items3 = [arr4.length > 0, arr4];
    const isActivitiesInTextEnabled = tmp2(9599).useIsActivitiesInTextEnabled(channel.id);
    ...
    const tmp8Result4 = tmp8(17435);
    const merged = Object.assign(tmp2(17442).useTextChannelPressEvents(channel, flag2));
    const tmp2Result12 = tmp2(17442);
    const obj8 = { channel, unread, mentionCount };
    const merged1 = Object.assign(tmp2(17121).getChannelAccessibilityProps(obj8));
    if (selected) {
    ...
    const tmp2Result13 = tmp2(17121);
    const obj11 = { unread, resolvedUnreadSetting, muted: flag };
    obj10.unreadBadge = closure_10(tmp8(17436), obj11);
    const tmp8Result5 = tmp8(17121);
    if (newChannel) {
    ...
    obj10.mentionBadge = tmp8(17437)(obj12);
    obj10.unread = unread;
    ...
    return tmp8Result4(tmp26(tmp2(5334).PressableHighlight, obj6));
  }
  const tmp18 = arr4(5219)(channel.id);
  result = channel(17441).renderChannelSubtitle({ subtitle: subtitle.subtitle, muted: flag, channelId: channel.id, guildId: channel.guild_id });
});
