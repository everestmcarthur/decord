...
// Dependencies: [32, 4361, 1372, 1074, 1374, 8049, 3, 38, 1434, 7880, 1385, 1241, 504, 4314, 1115, 11, 1886, 4945, 8050, 1370, 2]
...
// Module 8048 (ScheduledMessageUtils)
...
import parseContentForSuppressNotifications from "parseContentForSuppressNotifications" /* 7880 */;
...
const ScheduledMessagesConstants = fn(8049);
...
export const getPresetScheduledTimes = function getPresetScheduledTimes() {
  const addResult = _modDef4314().add(map1, "seconds");
  const obj = _modDef4314();
  const obj2 = _modDef4314();
  const result = _modDef4314().startOf("day").set("hours", 9);
  const startOfResult = _modDef4314().startOf("day");
  const obj5 = _modDef4314();
  const result1 = _modDef4314().startOf("day").set("hours", 13);
  const obj3 = { label: null, value: null };
  const startOfResult1 = _modDef4314().startOf("day");
  ...
  const obj11 = _modDef4314();
  const startOfResult2 = _modDef4314().startOf("isoWeek");
  obj6.value = _modDef4314().startOf("isoWeek").add(1, "week").set("hours", 9);
  items[2] = obj6;
  return items;
};
export const getDefaultScheduledTime = function getDefaultScheduledTime() {
  const obj = _modDef4314();
  const addResult = _modDef4314().startOf("hour").add(1, "hour");
  const startOfResult = _modDef4314().startOf("hour");
...
export const getScheduledTimeError = function getScheduledTimeError(isBefore, arg1) {
  if (isBefore.isBefore(obj.add(map1, "seconds"))) {
    ...
    const addResult = tmp(4314)().add(closure_1_11, "seconds");
    let minResult = addResult;
    if (null != arg1) {
      const tmpResult = tmp(4314);
      const tmpResult3 = tmp(11);
      const tmpResultResult = tmpResult(tmp(11).extractTimestamp(arg1));
      minResult = tmp(4314).min(addResult, tmpResultResult.add(closure_1_10, "seconds"));
      const tmpResult4 = tmp(4314);
    }
    stringResult = null;
...
export const getEarliestScheduledTime = function getEarliestScheduledTime() {
  return _modDef4314().add(map1, "seconds");
};
export const getLatestScheduledTime = function getLatestScheduledTime(arg0) {
  const addResult = _modDef4314().add(closure_1_11, "seconds");
  if (null == arg0) {
    return addResult;
  } else {
    const tmpResult = tmp(4314);
    ...
    return tmp(4314).min(addResult, tmpResultResult.add(closure_1_10, "seconds"));
  }
  const obj = _modDef4314();
};
...
export const getMessageForState = function getMessageForState(state) {
  if (ScheduledMessageTypes.ScheduledMessageSendState.SCHEDULED === state) {
    ...
    return obj;
  } else if (tmp(8050).ScheduledMessageSendState.ERROR_CHANNEL_NOT_FOUND === state) {
    const obj2 = { isError: true, stateMessage: null };
    const intl5 = tmp(1115).intl;
    obj2.stateMessage = intl5.string(tmp(1115).t.v5O2dK);
    return obj2;
  } else if (tmp(8050).ScheduledMessageSendState.ERROR_USER_NOT_FOUND === state) {
    const obj3 = { isError: true, stateMessage: null };
    const intl4 = tmp(1115).intl;
    obj3.stateMessage = intl4.string(tmp(1115).t.j8uIfG);
    return obj3;
  } else if (tmp(8050).ScheduledMessageSendState.ERROR_USER_CANNOT_USE_SCHEDULED_MESSAGES === state) {
    const obj4 = { isError: true, stateMessage: null };
    const intl3 = tmp(1115).intl;
    obj4.stateMessage = intl3.string(tmp(1115).t["w6zHX/"]);
    return obj4;
  } else if (tmp(8050).ScheduledMessageSendState.ERROR_SEND_FAILED === state) {
    const obj5 = { isError: true, stateMessage: null };
    const intl2 = tmp(1115).intl;
    obj5.stateMessage = intl2.string(tmp(1115).t.pflV7z);
    return obj5;
  } else if (tmp(8050).ScheduledMessageSendState.ERROR_SCHEDULED_MESSAGES_DISABLED === state) {
