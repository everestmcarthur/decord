...
// Dependencies: [19, 4543, 1099, 17636, 1074, 21, 1364, 4420, 17637, 1115, 7407, 5690, 4925, 1241, 1249, 5794, 17638, 17640, 17641, 16235, 10536, 4933, 4935, 7196, 2]
// Exports: default

// Module 17635 (ExistingUserAgeGateModal)
...
let closure_7 = fn(17636).ExistingUserAgeGateScreens;
...
export default function ExistingUserAgeGateModal(source) {
  ...
  const shouldAgeVerifyForAgeGate = source(4933).useShouldAgeVerifyForAgeGate();
  noop.useRef(shouldAgeVerifyForAgeGate);
  let obj = source(4933);
  const items = [source];
  const watchAgeVerificationStatusChange = source(4935).useWatchAgeVerificationStatusChange(noop.useCallback(() => {
    if (ref.current) {
  ...
  obj3.screens = noop.useMemo(() => {
    if (constants.NSFW_SERVER_INVITE !== source) {
              ...
              if (tmp2.NSFW_SERVER !== tmp) {
                if (tmp2.NSFW_CHANNEL === tmp) {
                  let fn = () => {
                    ...
                    if (null != guildId) {
                      closure_1(5690).nsfwReturnToSafety(guildId);
                      const obj = closure_1(5690);
                    }
                    source(7407).closeAgeGateModal(closure_0);
                  };
                } else if (tmp2.NSFW_VOICE_CHANNEL === tmp) {
                  fn = () => {
                    closure_1(4925).popAll();
                    const obj = closure_1(4925);
            ...
            if (closure_0 === constants2.NSFW_SERVER_INVITE) {
              if (obj.isIOS()) {
                let fn = () => {
                  ...
                  const obj2 = { key: "AGE_GATE_AGE_VERIFIED", icon: ref(17637), content: null };
              ...
              return jsx(closure_1(17638), { onSuccess: fn, onClose, source: tmp });
            }
            fn = () => {
              closure_0(7407).closeAgeGateModal();
              const obj = closure_0(7407);
              const obj3 = { key: "AGE_GATE_AGE_VERIFIED", icon: closure_1_1(17637), content: null };
              const intl = closure_0(1115).intl;
              obj3.content = intl.string(closure_0(1115).t.gUiIGZ);
              closure_1_1(4420).open(obj3);
            };
          }
      };
      ...
      obj5.render = function render() {
        return jsx(closure_1(17641), { source });
      };
      ...
      obj8.render = function render() {
        return closure_1_9(ref(10536), { onClose });
      };
  ...
  return jsx(source(7196).Navigator, { screens: null, initialRouteName: shouldAgeVerifyForAgeGate ? closure_7.Pawtect : closure_7.AgeGate, headerBackTitle: null });
};
