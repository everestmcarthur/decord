...
// Function ID: 10187
// Name: MessageManager
// Dependencies: [32, 4738, 5447, 1962, 7476, 1958, 1980, 4737, 2012, 4543, 1074, 1965, 1091, 3, 5442, 10187, 4652, 8606, 7652, 10188, 510, 4548, 1101, 7478, 5066, 1115, 573, 7314, 2]

// Module 10186 (MessageManager)
...
import getAdaptiveMessageLimit from "getAdaptiveMessageLimit" /* 10188 */;
      ...
      if (type !== constants.GUILD_STORE) {
        ...
        if (null == type1) {
          ...
          if (orCreate.some(AttachmentUrlUtilsAll.messageHasExpiredAttachmentUrl)) {
            ...
            tmp9(5442).clear(channelId);
            const tmp9Result = tmp9(5442);
            orCreate1 = tmp9(5442).getOrCreate(channelId);
            const tmp9Result9 = tmp9(5442);
          }
          let obj7 = orCreate1;
          if (tmp15) {
            const obj = { jumpTargetId: null, jumped: false, jumpType: Client.JumpType.ANIMATED };
            const mutation = orCreate1.mutate(obj);
            tmp9(5442).commit(mutation);
            obj7 = mutation;
            const tmp9Result10 = tmp9(5442);
          }
          let obj10 = obj7;
          if (tmp19) {
            const mutation1 = obj7.mutate({ focusTargetId: null });
            tmp9(5442).commit(mutation1);
            obj10 = mutation1;
            const tmp9Result11 = tmp9(5442);
          }
          if (isPreload) {
            if (!GatewayConnectionStore.isConnected()) {
              let flag = true;
            }
            let hasUnreadResult = tmp9(8606)(channelId);
            if (hasUnreadResult) {
            ...
            if (flag) {
              tmp9(5442).commit(obj10.mutate({ loadingMore: true }));
              if (null == messageId) {
                let isThreadResult;
                if (channel != null) {
                ...
                tmp9(7652).jumpToMessage(obj12);
                const tmp9Result16 = tmp9(7652);
              }
              const tmp9Result12 = tmp9(5442);
            }
          }
