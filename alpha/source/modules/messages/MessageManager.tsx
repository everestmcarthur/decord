...
// Function ID: 10199
// Name: MessageManager
// Dependencies: [32, 4772, 5489, 2045, 7520, 2041, 2063, 4771, 2095, 4577, 1074, 2048, 1091, 3, 5484, 10199, 4686, 8644, 7696, 10200, 510, 4582, 1101, 7522, 5108, 1115, 573, 7358, 2]
      ...
      if (type !== constants.GUILD_STORE) {
        ...
        if (null == type1) {
          ...
          if (orCreate.some(AttachmentUrlUtilsAll.messageHasExpiredAttachmentUrl)) {
            ...
            tmp9(5484).clear(channelId);
            const tmp9Result = tmp9(5484);
            orCreate1 = tmp9(5484).getOrCreate(channelId);
            const tmp9Result9 = tmp9(5484);
          }
          let obj7 = orCreate1;
          if (tmp15) {
            const obj = { jumpTargetId: null, jumped: false, jumpType: Client.JumpType.ANIMATED };
            const mutation = orCreate1.mutate(obj);
            tmp9(5484).commit(mutation);
            obj7 = mutation;
            const tmp9Result10 = tmp9(5484);
          }
          let obj10 = obj7;
          if (tmp19) {
            const mutation1 = obj7.mutate({ focusTargetId: null });
            tmp9(5484).commit(mutation1);
            obj10 = mutation1;
            const tmp9Result11 = tmp9(5484);
          }
          if (isPreload) {
            if (!GatewayConnectionStore.isConnected()) {
              let flag = true;
            }
            let hasUnreadResult = tmp9(8644)(channelId);
            if (hasUnreadResult) {
            ...
            if (flag) {
              tmp9(5484).commit(obj10.mutate({ loadingMore: true }));
              if (null == messageId) {
                let isThreadResult;
                if (channel != null) {
                ...
                tmp9(7696).jumpToMessage(obj12);
                const tmp9Result16 = tmp9(7696);
              }
              const tmp9Result12 = tmp9(5484);
            }
          }
