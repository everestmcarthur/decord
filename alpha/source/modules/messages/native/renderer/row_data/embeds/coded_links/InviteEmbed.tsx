...
// Function ID: 13455
// Name: InviteEmbed
// Dependencies: [4706, 1372, 1074, 7938, 13455, 7937, 13457, 13458, 13459, 13461, 13463, 11558, 11559, 2]
        ...
        if (invite.state !== tmp5.BANNED) {
          if (invite.state === tmp5.ERROR) {
            ...
            if (InviteTypes.GROUP_DM === inviteType) {
              return tmp29(13457).createGroupDMInvite(invite, tmp28, theme);
            } else if (tmp32.FRIEND === inviteType) {
              const tmp29Result10 = tmp29(13458);
              return tmp29Result10.createFriendInvite(invite, tmp28, id, theme);
            } else {
              const guildInviteExtendedType = tmp29(7937).getGuildInviteExtendedType(invite);
              if (tmp29(7937).GuildInviteExtendedType.EVENT === guildInviteExtendedType) {
                return tmp29(13459).createGuildScheduledEventInviteEmbed(invite, theme);
              } else if (tmp29(7937).GuildInviteExtendedType.APPLICATION === guildInviteExtendedType) {
                const obj2 = { inviteCode: invite.code, theme };
                return tmp29(13461).createEmbeddedActivityInviteEmbed(obj2);
              } else if (tmp29(7937).GuildInviteExtendedType.PROFILE === guildInviteExtendedType) {
                return tmp29(13463).createGuildProfileInvite(invite, theme);
              } else if (tmp29(7937).GuildInviteExtendedType.VOICE_CHANNEL === guildInviteExtendedType) {
                ...
                if (null != id1) {
                  ...
                  if (tmp29Result15.getVoiceChannelListInviteExperiment(obj3).enabled) {
                    const voiceChannelListInviteEmbed = tmp29(11559).createVoiceChannelListInviteEmbed(invite, theme);
                    if (null != voiceChannelListInviteEmbed) {
                      return voiceChannelListInviteEmbed;
                    }
                    const tmp29Result16 = tmp29(11559);
                  }
                  tmp29Result15 = tmp29(11558);
                }
                return tmp29(13455).createGuildInvite(invite, tmp28, theme);
              } else {
                return tmp29(13455).createGuildInvite(invite, tmp28, theme);
              }
              const tmp29Result11 = tmp29(7937);
            }
          }
        }
      }
