...
// Dependencies: [17, 2059, 4778, 2041, 2105, 2063, 4740, 4405, 1372, 11570, 1074, 7976, 8202, 1115, 8193, 576, 4600, 4607, 12060, 12061, 2055, 2108, 8203, 12976, 12975, 11572, 11571, 13512, 1385, 8662, 1397, 1879, 5239, 9020, 4909, 2]
...
// Module 13511 (invite/GuildInvite)
...
import CodedLinksConstants from "CodedLinksConstants" /* 11570 */;
import GuestUtilsDefault from "GuestUtils" /* 11572 */;
import InviteErrorUtils from "InviteErrorUtils" /* 12975 */;
import _modDef12976 from "module_12976" /* 12976 */;
import getHeaderTextForInvite from "getHeaderTextForInvite" /* 13512 */;
...
import Constants_mod from "Constants" /* 7976 */;
...
export const createExpiredGuildInvite = function createExpiredGuildInvite(author, arg1, theme) {
  ...
  obj.titleColor = tmp6(8193).processColorOrThrow(nativeDefault.unsafe_rawColors.RED_400);
  if (arg1) {
    ...
    if (null != username) {
      ...
      const obj2 = { username: tmp(4600).getFormattedName(author.author) };
  ...
  const tmp6Result = tmp6(8193);
  if (tmp6Result2.isThemeDark(theme)) {
    let tmpResult2 = tmp(12060);
  } else {
    tmpResult2 = tmp(12061);
  }
...
export const createDisabledGuildInvite = function createDisabledGuildInvite(invite, theme) {
  ...
  obj2.thumbnailUrl = renderer_EmbedUtils.getAssetUriForEmbed(_modDef12976);
  let tmp17;
  if (null == tmp11) {
    let tmp18;
    if (null != fromInviteGuildResult) {
...
export const createGuildInvite = function createGuildInvite(invite, isOwnInvite, theme) {
  ...
  const channel = tmp(11571)(invite).channel;
  ...
  if (tmp7) {
    if (flag2) {
      if (null != target_user) {
        if (tmp12) {
          ...
          obj6.name = tmp(4600).getFormattedName(target_user);
          const tmpResult8 = tmp(4600);
    ...
    const assetSource = Image.resolveAssetSource(tmp10(5239).getChannelIcon(channel));
    let uri1;
    if (assetSource != null) {
    ...
    uri = guildIconURL;
    tmp25 = name;
    const tmp10Result7 = tmp10(5239);
  } else {
    if (null != channel) {
      if (channel.type === constants2.GUILD_STAGE_VOICE) {
        const assetSource1 = Image.resolveAssetSource(tmp10(5239).getChannelIcon(channel));
        let uri2;
    ...
    if (null == approximate_member_count) {
      ...
      tmp25 = name;
      if (null != channel) {
        const assetSource2 = Image.resolveAssetSource(tmp10(5239).getChannelIcon(channel));
        let uri3;
  ...
  if (tmp7) {
    ...
    if (null != guild) {
      guildBadgeImageSource = tmp10(9020).getGuildBadgeImageSource(guild, theme);
      const tmp10Result10 = tmp10(9020);
    }
    const obj11 = {};
    const merged = Object.assign(baseColors);
    let formatted;
    ...
    let channelName;
    if (null != channel) {
      channelName = tmp10(4909).computeChannelName(channel, UserStore, RelationshipStore);
      const tmp10Result11 = tmp10(4909);
    }
    ...
    if (null != guildBadgeImageSource) {
      assetUriForEmbed = tmp10(8203).getAssetUriForEmbed(guildBadgeImageSource);
      const tmp10Result12 = tmp10(8203);
    }
