...
// Function ID: 11673
// Name: ChatBanner
// Dependencies: [19, 17, 5447, 4737, 11673, 1074, 21, 4722, 576, 11674, 11675, 504, 11, 1241, 4902, 7309, 4718, 1115, 5144, 7465, 7967, 5292, 7306, 8192, 2]
// Exports: default

// Module 11672 (ChatBanner)
...
import useShowChannelOptInNoticeDefault from "useShowChannelOptInNotice" /* 11674 */;
import useAllowedChatOverlaysDefault from "useAllowedChatOverlays" /* 11675 */;
...
function ArchivedLockedThreadChatBanner(channel) {
  ...
  let canUnarchiveThread = channel(7465).useCanUnarchiveThread(channel);
  ...
  const items1 = [closure_12(channel(4718).Text, obj3), ];
  if (canUnarchiveThread) {
    ...
    obj4.children = tmp8(tmp3(5144).Button, obj5);
...
function LockedThreadChatBanner(channel) {
  ...
  let isThreadModerator = channel(7465).useIsThreadModerator(channel);
  let obj2 = { style: tmp.threadBannerContainer, children: null };
  let obj3 = { lineClamp: 4, style: tmp.threadBannerTitle, variant: "text-sm/medium", color: "text-default", children: null };
  let obj = channel(7465);
  const tmp6 = closure_13;
...
function NewMessagesChatBar(channel) {
  ...
  if (obj.useStateFromStores(items, () => connected.isConnected(), [])) {
    if (unreadCount <= 0) {
      ...
      obj2.children = closure_12(XSmallBoldIcon(4718).Text, obj3);
      const items1 = [closure_12(XSmallBoldIcon(5292).PressableOpacity, obj2), ];
      handleScrollToNewMessages = XSmallBoldIcon(5292).PressableOpacity;
      ...
      XSmallBoldIcon = XSmallBoldIcon(8192).XSmallBoldIcon;
...
const ChatOverlays = fn(11673).ChatOverlays;
