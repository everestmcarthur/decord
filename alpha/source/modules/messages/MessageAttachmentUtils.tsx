...
// Dependencies: [4395, 1074, 7532, 7537, 7536, 8140, 563, 2019, 8541, 1115, 2]
...
function getForumPostShouldObscure(media, arg1, enabledHarmTypesBitmaskForChannelType) {
  if (null == media) {
    ...
    if (ForumPostMediaUtils.ForumPostMediaTypes.EMBED === type) {
      const obj2 = { type: tmp4(7537).ObscuredMediaTypes.Embed, media };
      let tmp = obj2;
    } else if (tmp4(8140).ForumPostMediaTypes.ATTACHMENT === type) {
      const obj = { type: tmp4(7537).ObscuredMediaTypes.Attachment, media };
      tmp = obj;
    } else {
      tmp = null;
      if (tmp4(8140).ForumPostMediaTypes.COMPONENT === type) {
        const obj3 = { type: tmp4(7537).ObscuredMediaTypes.GenericMedia, media: media.srcUnfurledMediaItem };
        tmp = obj3;
      }
    }
    if (null == tmp) {
      ...
      const mediaObscuredReasonFromBitmask = tmp4(7532).getMediaObscuredReasonFromBitmask(tmp, enabledHarmTypesBitmaskForChannelType);
      tmp4(7532);
      if (mediaObscuredReasonFromBitmask.length > 0) {
        ...
        if (tmp8) {
          items3[0] = true;
          items3[1] = tmp4(7536).ObscureReason.POTENTIAL_EXPLICIT_CONTENT;
          tmp2 = items3;
        } else if (media.spoiler) {
          items3[0] = arg1;
          items3[1] = tmp4(7536).ObscureReason.SPOILER;
          tmp2 = items3;
        } else {
          items3[0] = false;
          items3[1] = undefined;
...
export const getObscureReasonForAttachment = function getObscureReasonForAttachment(attachment, enabledHarmTypesBitmaskForChannelAndAuthorId, c2) {
  ...
  if (mediaObscuredReasonFromBitmask.length > 0) {
    ...
    first = tmp(7536).ObscureReason.POTENTIAL_EXPLICIT_CONTENT;
  } else {
    first = null;
    if (flag) {
      first = tmp(7536).ObscureReason.SPOILER;
    }
  }
  return first;
};
export const getObscureReasonForEmbed = function getObscureReasonForEmbed(embed, message, flag2, enabledContentHarmTypeFlags) {
  ...
  if (!bot) {
    const obj3 = { type: tmp(7537).ObscuredMediaTypes.Embed, media: embed };
...
export const getObscureReasonForUnfurledMediaItem = function getObscureReasonForUnfurledMediaItem(unfurledMediaItem, enabledHarmTypesBitmaskForChannelAndAuthorId, arg2, isBot) {
  ...
  if (!flag2) {
    const obj3 = { type: tmp(7537).ObscuredMediaTypes.GenericMedia, media: unfurledMediaItem };
    isMediaScanPendingResult = tmp(7532).isMediaScanPending(obj3, enabledHarmTypesBitmaskForChannelAndAuthorId);
    const tmpResult = tmp(7532);
  }
  if (mediaObscuredReasonFromBitmask.includes(ObscureMediaModels.ObscureReason.EXPLICIT_CONTENT)) {
    let EXPLICIT_CONTENT = tmp(7536).ObscureReason.EXPLICIT_CONTENT;
  } else if (mediaObscuredReasonFromBitmask.includes(tmp(7536).ObscureReason.GORE_CONTENT)) {
    EXPLICIT_CONTENT = tmp(7536).ObscureReason.GORE_CONTENT;
  } else if (mediaObscuredReasonFromBitmask.includes(tmp(7536).ObscureReason.SELF_HARM_CONTENT)) {
    EXPLICIT_CONTENT = tmp(7536).ObscureReason.SELF_HARM_CONTENT;
  } else if (isMediaScanPendingResult) {
    EXPLICIT_CONTENT = tmp(7536).ObscureReason.POTENTIAL_EXPLICIT_CONTENT;
  } else {
    EXPLICIT_CONTENT = null;
    if (flag) {
      EXPLICIT_CONTENT = tmp(7536).ObscureReason.SPOILER;
    }
  }
...
export const getObscuredAlt = function getObscuredAlt(arg0) {
  if (ObscureMediaModels.ObscureReason.EXPLICIT_CONTENT !== arg0) {
    if (tmp(7536).ObscureReason.GORE_CONTENT !== arg0) {
      if (tmp(7536).ObscureReason.SELF_HARM_CONTENT !== arg0) {
        if (tmp(7536).ObscureReason.SPOILER === arg0) {
