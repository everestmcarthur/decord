...
// Dependencies: [5, 1220, 502, 5448, 1958, 1909, 4745, 4772, 2012, 5449, 4761, 1074, 1084, 13832, 13881, 3, 510, 7534, 5581, 1364, 13870, 12, 4774, 13849, 504, 573, 2]

// Module 5447 (GatewayConnectionStore)
import LoggerDefault from "Logger" /* 3 */;
import _modDef12 from "module_12" /* 12 */;
import initializeDefault from "initialize" /* 504 */;
import DispatcherDefault from "Dispatcher" /* 573 */;
import PlatformUtils from "PlatformUtils" /* 1364 */;
import GatewaySocketSingleton from "GatewaySocketSingleton" /* 13832 */;
import ConnectionStateDefault from "ConnectionState" /* 13849 */;
import PauseGatewaySocketAll from "PauseGatewaySocket" /* 13870 */;
import dispatchSocketMessageDefault from "dispatchSocketMessage" /* 13881 */;
...
fn(13832).socket.dispatcher.getDispatchHandler = dispatchSocketMessageDefault;
...
const gatewayConnectionStore = new GatewayConnectionStore(DispatcherDefault, {
  ...
  LOGIN_SUCCESS: function handleSessionRefresh() {
    ...
    if (connectResult) {
      const socket3 = tmp(13832).socket;
      socket3.close();
      const socket4 = tmp(13832).socket;
  ...
  VOICE_CHANNEL_SELECT: function handleVoiceChannelSelect(guildId) {
    ...
    if (isIOSResult) {
      if (null == guildId.channelId) {
        ...
        const socket = tmp(13832).socket;
        if (socket.isClosed()) {
          PauseGatewaySocketAll.setIsPaused(false);
          const socket2 = tmp(13832).socket;
          socket2.connect();
        }
      }
    }
      ...
      if (id.getId() !== userId.userId) {
        ...
        if (userId.sessionId === closure_1_21) {
          ...
          if (userId.guildId !== require("GatewaySocketSingleton").localVoiceState.guildId) {
            ...
            const localVoiceState = tmp(13832).localVoiceState;
  ...
  STREAM_WATCH: function handleStreamWatch(arg0) {
    ...
    if (socket.isSessionEstablished()) {
      if (!allowMultiple) {
        ...
        _require = allActiveStreamKeys.find((item) => closure_0(4774).decodeStreamKey(item).ownerId === id.getId());
        ...
        const item = found.forEach((item) => {
          const socket = closure_0(13832).socket;
          if (socket.isSessionEstablished()) {
            const socket2 = closure_0(13832).socket;
