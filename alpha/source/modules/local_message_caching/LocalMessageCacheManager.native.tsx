...
// Dependencies: [5, 32, 4372, 502, 1958, 4943, 1074, 14771, 3, 1091, 510, 4314, 4404, 7954, 7652, 573, 11989, 11916, 1897, 1899, 5442, 10186, 2]

// Module 14770 (LocalMessageCacheManager)
...
function createFailedMessage(channel_id) {
  ...
  const tmp3 = file(7954)({ channelId: channel_id, content, tts, state: MessageStates.SEND_FAILED });
  const id = tmp3;
  ({ timestamp: tmp3.timestamp, file } = channel_id);
  file(7652).receiveMessage(channel_id, tmp3, true, { isHydratingExpiredPendingMessage: state === MessageStates.SENDING });
  if (null != file) {
