...
// Dependencies: [32, 19, 17, 4748, 7921, 12213, 21, 4756, 576, 4457, 12497, 12506, 12490, 4700, 1115, 504, 4466, 12510, 4492, 2]

// Module 12505 (ChatInputSendButton)
...
import useChatInputFloatingWidthDefault from "useChatInputFloatingWidth" /* 12510 */;
...
let closure_8 = fn(12213).CHAT_INPUT_FLOATING_BOUNCE_ENTER_DELAY_MS;
const jsx = fn(21).jsx;
...
let closure_12 = noop.memo((type) => {
  ...
  if (type === constants.BUTTON_SEND_VOICE_MESSAGE || type === constants.BUTTON_SEND_VOICE_MESSAGE_DISABLED) {
    ...
    let tmp7Result = tmp7(tmp3(12506), obj5);
  } else {
...
const forwardRefResult = noop.forwardRef((channel, ref) => {
  ...
  const token = channel(4457).useToken(onSendMessage(576).modules.mobile.CHAT_INPUT_SEND_BUTTON_WIDTH);
  const obj = channel(4457);
  const token1 = channel(4457).useToken(onSendMessage(576).modules.mobile.CHAT_INPUT_SEND_BUTTON_HEIGHT);
  const obj2 = channel(4457);
  const token2 = channel(4457).useToken(onSendMessage(576).modules.mobile.CHAT_INPUT_ACTION_BUTTON_MARGIN);
  const obj3 = channel(4457);
  ...
  if (stateFromStores) {
    ...
    if (canSendVoiceMessage) {
      ...
      let tmp15Result = tmp15(tmp3(12506), obj8);
    } else {
      ...
      obj9.IconComponent = tmp(4700).SendMessageIcon;
      const intl = tmp(1115).intl;
      obj9.accessibilityLabel = intl.string(tmp(1115).t.TXNS7S);
      obj9.onPress = onSendMessage;
      obj9.disabled = !tmp12;
      tmp15Result = tmp15(tmp3(12490), obj9);
      const tmp3Result = tmp3(12490);
    }
    ...
    const obj12 = { buttonWidth: token, buttonHeight: token1, buttonMargin: token2, sendVoiceMessageEnabled: canSendVoiceMessage, children: tmp15(tmp(4466).TransitionGroup, obj10) };
