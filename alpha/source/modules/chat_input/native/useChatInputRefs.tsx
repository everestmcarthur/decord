...
// Dependencies: [19, 8020, 5714, 5105, 7915, 7921, 1372, 9653, 1074, 5211, 4749, 5810, 4622, 12, 12239, 12241, 12242, 12245, 12246, 9417, 8017, 4624, 1610, 1482, 1364, 4625, 1875, 1241, 11916, 7696, 11918, 12247, 7916, 12248, 5103, 12280, 6862, 2]
// Exports: default

// Module 12238 (useChatInputRefs)
...
import ChatInputSendUtils from "ChatInputSendUtils" /* 12248 */;
...
export default function useChatInputRefs(chatInputProps) {
  ...
  const tmp2 = chatInputTextFieldHeight(5810)(() => ChatInputUtils.createInputRefTracker(chatInputProps.channel.id, chatInputProps.screenIndex));
  closure_12 = tmp2;
  ...
  const state = noop.useRef(chatInputTextFieldHeight(5810)(() => ({ editId: null, focused: false, selectionStart: 0, selectionEnd: 0, text: chatInputProps.defaultValue, textPrev: chatInputProps.defaultValue, textFieldContentSize: 0, textFieldHeight: chatInputTextFieldHeight })));
    ...
    let obj4 = {
      ...
      handleCancelEditing() {
        ...
        if (null != editingMessage) {
          ...
          tmp6(7696).endEditMessage(channel.id);
        ...
        if (!SlowmodeStore.isChannelOnCooldown(channel)) {
          ...
          if (null != sticker) {
            if (!obj7.isStandardSticker(current2)) {
              ...
              obj.params = obj2;
              const result = tmp27(12248).chatInputValidateContentLength(obj);
              if (null != result) {
                closure_1.cancel();
                if (null != threadCreationCallback) {
                  ...
                  tmp27(11918).deletePendingReply(channel.id);
                  const tmp27Result2 = tmp27(11918);
                  tmp31(8017).saveDraft(channel.id, "", DraftType.ChannelMessage);
