...
// Function ID: 12210
// Name: ChatInput
// Dependencies: [5, 32, 19, 17, 8020, 12210, 9772, 7914, 12211, 5105, 7915, 4395, 5104, 9653, 12213, 1074, 1483, 2038, 1375, 1608, 21, 4756, 576, 1364, 12214, 2019, 4574, 4457, 12215, 504, 7509, 7402, 8001, 4492, 4624, 9599, 12235, 12236, 8086, 12237, 12238, 7461, 1110, 1611, 12281, 12282, 5349, 9420, 5339, 1255, 4724, 1241, 1610, 4936, 9592, 12286, 12447, 10894, 4622, 1874, 12448, 10591, 8174, 12482, 10909, 9417, 12485, 12486, 4427, 12502, 12503, 12504, 12505, 12511, 12512, 1092, 12513, 12514, 12518, 12534, 12539, 12542, 12635, 12637, 12657, 12659, 12660, 12673, 12677, 12681, 12682, 12683, 12684, 12685, 2]

// Module 12209 (ChatInput)
...
import DiceRollStore from "DiceRollStore" /* 12210 */;
...
import EditMessageStore from "EditMessageStore" /* 7915 */;
...
let useVoiceMessagesUIStore = fn(12211).useVoiceMessagesUIStore;
const DraftType = fn(5105).DraftType;
const useChatBottomManagerUIStore = fn(9653);
...
const forwardRefResult = noop.forwardRef((channel, ref) => {
  ...
  const mobileEmojiSuggestionsConfig = channel(12214).useMobileEmojiSuggestionsConfig({ location: "ChatInput" });
  const InlineEmojiSuggestionsEnabled = channel(2019).InlineEmojiSuggestionsEnabled;
  let tmp66Result30 = mobileEmojiSuggestionsConfig.enabled && InlineEmojiSuggestionsEnabled.useSetting();
  let obj = channel(12214);
  const gradientValue = channel(4574).useGradientValue(tmp(4574).GradientPercentage.END);
  let tmpResult = channel(4574);
  const token = channel(4457).useToken(screenIndex(576).modules.mobile.CHAT_INPUT_ACTION_BUTTON_SIZE);
  const tmpResult30 = channel(4457);
  let result = (channel(4457).useToken(screenIndex(576).modules.mobile.CHAT_INPUT_SEND_BUTTON_HEIGHT) - token) / 2;
  const tmpResult31 = channel(4457);
  const token1 = channel(4457).useToken(screenIndex(576).modules.mobile.CHAT_INPUT_FLOATING_SCRIM_GRADIENT_HEIGHT);
  const tmp10 = closure_40(gradientValue, token1);
  const tmpResult32 = channel(4457);
  let token2 = gradientValue;
  if (gradientValue == null) {
    token2 = tmpResult33.useToken(screenIndex(576).colors.BACKGROUND_BASE_LOWER);
  }
  tmpResult33 = channel(4457);
  const token3 = channel(4457).useToken(tmp6(576).modules.mobile.CHAT_INPUT_FLOATING_TYPING_GRADIENT_HEIGHT_REDUCED);
  const tmpResult34 = channel(4457);
  const token4 = channel(4457).useToken(tmp6(576).modules.mobile.CHAT_INPUT_FLOATING_INLINE_FULL_GRADIENT_HEIGHT);
  const tmpResult35 = channel(4457);
  const token5 = channel(4457).useToken(tmp6(576).modules.mobile.CHAT_INPUT_FLOATING_SCRIM_GRADIENT_HEIGHT_AT_BOTTOM);
  const tmpResult36 = channel(4457);
  ...
  const typingUserIdsForDisplay = channel(12215).useTypingUserIdsForDisplay(channel.id, 1);
  const tmp25 = closure_22(screenIndex);
  const tmpResult37 = channel(12215);
  let result1 = channel(12215).hasTypingIndicatorContent(channel, typingUserIdsForDisplay, tmp25);
  ...
  const tmpResult38 = channel(12215);
  ...
  const analyticsLocations = tmp6(7402)().analyticsLocations;
  let tmp35 = tmp21;
  if (null == threadCreationCallback) {
    tmp35 = null != stateFromStores;
  }
  if (!tmp35) {
    tmp35 = !tmp(7509).getIsActiveChannelOrUnarchivableThread(channel);
    const tmpResult43 = tmp(7509);
  }
  const tmpResult42 = channel(504);
  let canStartThread = channel(7509).useCanStartThread(channel);
  if (canStartThread) {
  ...
  const tmpResult44 = channel(7509);
  const tmp39 = channel(8001).useCanPostPollsInChannel(channel) && null == threadCreationCallback;
  const tmpResult45 = channel(8001);
  sharedValue = channel(4492).useSharedValue(token);
  const tmpResult46 = channel(4492);
  sharedValue1 = channel(4492).useSharedValue(token);
  ...
  const tmp43 = screenIndex(4624)();
  const tmp44 = useVoiceMessagesUIStore((startTimeMillis) => null != startTimeMillis.startTimeMillis);
  let result3 = !tmp21;
  let isAppLauncherEnabled = result3;
  if (null == threadCreationCallback) {
    isAppLauncherEnabled = tmp(9599).getIsAppLauncherEnabled(channel);
    const tmpResult48 = tmp(9599);
  }
  const tmpResult47 = channel(4492);
  ...
  ({ placeholder, accessibilityLabel } = screenIndex(12235)({ channel, isReadonly: !editable, isCreatingThread: null != threadCreationCallback }));
  let tmp48 = screenIndex(12235)({ channel, isReadonly: !editable, isCreatingThread: null != threadCreationCallback });
  class Ze {
    constructor() {
  ...
  const animatedStyle = channel(4492).useAnimatedStyle(Ze);
  ref = obj10.useRef(null);
  const tmpResult50 = channel(4492);
  const refreshChatInputCoachmark = channel(12236).useRefreshChatInputCoachmark({ disabled: !editable });
  let obj3 = { disabled: !editable };
  const tmpResult51 = channel(12236);
  const canUseScheduledMessages = channel(8086).useCanUseScheduledMessages();
  const tmpResult52 = channel(8086);
  const items10 = [memo1];
  ...
  const scheduledMessageDraftCoachmarkState = channel(12237).useScheduledMessageDraftCoachmarkState(obj4);
  ...
  const tmp56 = screenIndex(12238)({ chatInputProps: { analyticsLocations, canUpload, channel, defaultValue: memo, hasAttachmentsToUpload: stateFromStores2, pendingEdit: stateFromStores, pendingReply: stateFromStores1, screenIndex, secondaryTextFieldRef, threadCreationCallback }, chatInputTextFieldHeight: sharedValue1, ref });
  ...
  memo1 = obj10.useMemo(() => {
    let obj = {
      ...
      handleChangeContentSize(nativeEvent) {
        ...
        if (!obj.getIsChatInputHeightWorkletEnabled()) {
          ...
          const result = textFieldHeight.set(channel(12282).getChatInputHeightAnimationTiming(height, sharedValue.get()));
          const tmp2Result = channel(12282);
        }
      },
      ...
      handleMaxHeightChanged() {
        if (!obj.getIsChatInputHeightWorkletEnabled()) {
          ...
          if (0 !== textFieldContentSize) {
            const textFieldHeight = closure_1_13.state.current.textFieldHeight;
            const result = textFieldHeight.set(channel(12282).getChatInputHeightAnimationTiming(textFieldContentSize, sharedValue.get()));
            const tmpResult = channel(12282);
          }
        }
      },
        ...
        try {
          ...
          if (0 === c3) {
            ...
            if (1 === tmp5) {
              if (arg0 === 1) {
                ...
                if (closure_1_13.state.current.focused) {
                  if (closure_1_13.props.current.canUpload) {
                    c3 = 2;
                    c4 = 1;
                    const obj6 = { value: closure_0(5349).getImageDimensionsIfMissing(closure_129_0, closure_129_1, closure_129_2), done: false };
                    return obj6;
                  }
                }
                c4 = 3;
              ...
              const size = { uri: closure_129_0, originalUri: closure_129_0, width: closure_129_4.width, height: closure_129_4.height, mimeType: closure_129_3, platform: closure_0(5339).UploadPlatform.REACT_NATIVE, id: null };
              const obj7 = screenIndex(9420);
    ...
    obj.handlePressAction = function handlePressAction(arg0, arg1, current2) {
      if (constants.PHOTOS === arg1) {
        const result = channel(4724).triggerHapticFeedback(channel(4724).HapticFeedbackTypes.IMPACT_LIGHT);
        const obj21 = channel(4724);
        ...
        const keyboardType = channel(4624).getKeyboardType();
        if (keyboardType === channel(1610).KeyboardTypes.APP_LAUNCHER) {
          ...
          const keyboardType1 = tmp42(4624).getKeyboardType();
          ...
          const tmp42Result = tmp42(4624);
        }
        const obj24 = channel(4624);
      } else {
        let tmp35 = current2;
        if (tmp.APPS === arg1) {
          const result1 = channel(4724).triggerHapticFeedback(channel(4724).HapticFeedbackTypes.IMPACT_LIGHT);
          const obj12 = channel(4724);
          channel(4936).trackWithMetadata(constants2.APP_LAUNCHER_ENTRYPOINT_BUTTON_CLICKED);
          const obj13 = channel(4936);
          ...
          const result2 = c2(9592).dismissNewActivityIndicator();
          const obj16 = c2(9592);
          if (tmp35 == null) {
            tmp35 = null;
          }
          const result3 = channel(12286).setAppLauncherA11yFocusReturnRef(tmp35);
          ...
          const obj17 = channel(12286);
          const appDMApplication = channel(12447).getAppDMApplication(tmp30.props.current.channel);
          let name;
          ...
          const tmp23Result = channel(12447);
        } else if (tmp.ALL_PHOTOS === arg1) {
          const result4 = channel(4724).triggerHapticFeedback(channel(4724).HapticFeedbackTypes.IMPACT_LIGHT);
          const obj9 = channel(4724);
          const obj20 = {
            channel: closure_1_13.props.current.channel,
            uploadLimit,
            onDismissKeyboard() {
                  return closure_1_0(4622).dismissKeyboard();
                },
            onRestoreKeyboard() {
                  return closure_1_14.handleToggleKeyboard({ type: closure_0(1610).KeyboardTypes.SYSTEM });
                },
            onSelectFiles(items) {
                  closure_0(10894).addImagesFromPicker(closure_1_13.props.current.channel.id, items, closure_0(5339).UploadOrigin.IMAGE_PICKER);
                },
            draftType: registerViewTag.ChannelMessage
          };
          channel(10894).handleViewAllDialog(obj20);
          const obj10 = channel(10894);
        } else if (tmp.CAMERA === arg1) {
          const result5 = channel(4724).triggerHapticFeedback(channel(4724).HapticFeedbackTypes.IMPACT_LIGHT);
          const obj6 = channel(4724);
          const obj23 = {
            channel: closure_1_13.props.current.channel,
          ...
          channel(10894).handleCameraDialog(obj23);
          const obj7 = channel(10894);
        } else if (tmp.NITRO_GIFT === arg1) {
          const result6 = screenIndex(1874).markPotentialBadState();
          const obj = screenIndex(1874);
          const result7 = channel(4724).triggerHapticFeedback(channel(4724).HapticFeedbackTypes.IMPACT_LIGHT);
          const obj2 = channel(4724);
          if (obj3.isAndroid()) {
            tmp5(4622).dismissKeyboard();
            const tmp5Result = tmp5(4622);
          }
          obj3 = channel(1364);
          channel(10894).handleSelectGift(closure_1_13.props.current.analyticsLocations, closure_1_13.chatInput, tmp35);
          const tmp5Result2 = channel(10894);
        } else if (tmp.THREAD === arg1) {
          const result8 = channel(4724).triggerHapticFeedback(channel(4724).HapticFeedbackTypes.IMPACT_LIGHT);
          const obj28 = channel(4724);
          channel(10894).handleSelectThread(closure_1_13.props.current.channel, closure_1_13.chatInput);
          const obj29 = channel(10894);
        }
      }
    };
    obj.handlePollsPress = function handlePollsPress() {
      const result = channel(4724).triggerHapticFeedback(channel(4724).HapticFeedbackTypes.IMPACT_LIGHT);
      const obj = channel(4724);
      ...
      channel(4622).dismissKeyboard();
      const obj4 = channel(4622);
      channel(12448).openCreatePollModal({
        channel: closure_1_13.props.current.channel,
        onCancel() {
          return closure_1_14.handleToggleKeyboard({ type: closure_0(1610).KeyboardTypes.SYSTEM });
        }
    ...
    obj.handleAttachPress = function handleAttachPress() {
      const result = channel(4724).triggerHapticFeedback(channel(4724).HapticFeedbackTypes.IMPACT_LIGHT);
      const obj = channel(4724);
      channel(10894).handleAttachFile({
        channel: closure_1_13.props.current.channel,
        uploadLimit,
        onDismissKeyboard() {
        ...
        onSelectFiles(items) {
          closure_0(10894).addImagesFromPicker(props.props.current.channel.id, items, closure_0(5339).UploadOrigin.FILE_ATTACHMENT);
        }
      });
    };
    obj.handlePressExpression = function handlePressExpression(context) {
      const result = channel(10591).initiateEmojiInteraction(EmojiInteractionPoint.ChatInputExpressionPressed);
      const obj = channel(10591);
    ...
    obj.handleSelectionOrTextChange = function handleSelectionOrTextChange(nativeEvent) {
      ...
      if (closure_1_13.state.current.editId !== editId) {
        ...
        channel(8174).hideContextMenu();
        const obj3 = channel(8174);
      }
    };
    ...
    obj.handleTextOrFocusChange = function handleTextOrFocusChange(text, focused) {
      if (text.length > 0) {
        if (!focused) {
          const maxMessageLength = channel(9417).getMaxMessageLength();
          if (tmp16) {
        ...
        if (isCoachmarkVisible.isOpen()) {
          screenIndex(10909).hideNativeMenu();
          const obj = screenIndex(10909);
        }
  ...
  const tmp62 = screenIndex(12281)({ textFieldHeight: sharedValue1, textFieldMinHeight: sharedValue });
  ...
  const tmpResult54 = channel(12237);
  if (editable) {
    ...
    let result2 = result3;
    if (!tmp21) {
      result2 = tmp(4427).isPremiumGiftingSupported();
      const tmpResult55 = tmp(4427);
    }
    ...
    tmp66Result = tmp66(tmp6(12486), obj7);
  ...
  const tmp67 = closure_37(screenIndex(12485), { canUpload, channelId: channel.id, screenIndex });
  obj9.customKeyboard = channel(12503).PORTAL_KEYBOARD_PLACEHOLDER_INSTANCE;
  ...
  const items19 = [closure_37(screenIndex(12502), obj9), closure_37(screenIndex(12504), { keyboardType: tmp43, onSelectKeyboard: memo1.handleToggleKeyboard, ref: tmp56.chatInputCover })];
  obj8.children = items19;
  let obj11 = { keyboardType: tmp43, onSelectKeyboard: memo1.handleToggleKeyboard, ref: tmp56.chatInputCover };
  const tmp6Result7 = screenIndex(12502);
  if (editable) {
    ...
    let tmp66Result17 = tmp66(tmp6(12505), obj12);
    const tmp6Result8 = tmp6(12505);
  } else {
    tmp66Result17 = null;
  }
  let obj13 = { collapsable: false, onLayout: callback2, style: null, children: null };
  const items20 = [screenIndex(12511)({ isCreatingThread: null != threadCreationCallback }), tmp10.overflowVisible, ];
  ...
  if (!result1) {
    ...
    tmp66Result18 = tmp66(tmp(12512).ChatInputScrimGradient, obj14);
  }
  ...
  items21[2] = closure_37(channel(12513).ChatInputAccessibilityDivider, {});
  ...
  if (tmp22) {
    ...
    tmp66Result20 = tmp66(tmp6(12514), obj16);
    const tmp6Result9 = tmp6(12514);
  }
  ...
  if (isResourceChannel) {
    let obj21 = { channel };
    tmp66Result24 = tmp66(tmp6(12534), obj21, channel.id);
  }
  items21[5] = tmp66Result24;
  items21[6] = closure_37(channel(12539).MemberActionsChatInputBannerGuardedOuter, { channel });
  items21[7] = closure_37(channel(12542).DoubleTapToReactChatInputBanner, { channel });
  let tmp66Result25 = null;
  if (tmp23) {
    let obj22 = { channelId: channel.id };
    tmp66Result25 = tmp66(tmp6(12635), obj22);
  }
  ...
  if (tmp43 !== channel(1610).KeyboardTypes.EXPRESSION) {
    ...
    tmp66Result26 = tmp66(tmp6(12637), obj23);
  }
  items21[9] = tmp66Result26;
  items21[10] = closure_37(screenIndex(12657), { ref: tmp56.chatInputAppCommandManager, canOnlyUseTextCommands: null != stateFromStores1, channel, chatInputRef: tmp56.chatInput, chatInputStateRef: tmp56.state, commandsDisabled: tmp35 });
  ...
  items24[1] = closure_37(screenIndex(12660), { channel });
  ...
  const items26 = [closure_37(screenIndex(12673), { channel, chatInputRef: tmp56.chatInput, pendingEdit: stateFromStores, pendingReply: stateFromStores1 }), , ];
  ...
  if (tmp66Result27) {
    ...
    tmp66Result27 = tmp66(tmp(12677).EmojiSuggestionBarLarge, obj28);
  }
  ...
  obj32.style = items28;
  const items29 = [tmp74, closure_37(screenIndex(12681), { analyticsLocations, ref: tmp56.chatInputCharCounter })];
  ...
  if (editable) {
    ...
    if (!tmp21) {
      result3 = tmp(4427).isPremiumGiftingSupported();
      const tmpResult58 = tmp(4427);
    }
    ...
    obj35.children = tmp66(tmp6(12682), obj38);
    tmp66Result29 = tmp66(tmp77, obj35);
    const tmp6Result11 = tmp6(12682);
  }
  ...
  if (tmp66Result30) {
    ...
    tmp66Result30 = tmp66(tmp(12683).EmojiSuggestionBarSmall, obj39);
  }
  items24[3] = tmp66Result30;
  obj25.children = closure_38(closure_39, { children: items24 });
  items21[11] = closure_37(screenIndex(12659), obj25);
  ...
  if (null != refreshChatInputCoachmark) {
    ...
    tmp66Result31 = tmp66(tmp6(12236), obj40);
    const tmp6Result12 = tmp6(12236);
  }
  items21[12] = tmp66Result31;
  items21[13] = closure_37(screenIndex(12684), { buttonRef: ref, isVisible: isCoachmarkVisible, onDismiss: dismissCoachmark });
  ...
  if (null == threadCreationCallback) {
    ...
    tmp66Result32 = tmp66(tmp6(12685), obj41);
  }
