...
// Function ID: 12201
// Name: ChatInput
// Dependencies: [5, 32, 19, 17, 7982, 12201, 9929, 7876, 12202, 5063, 7877, 4361, 5062, 9723, 12204, 1074, 1483, 1955, 1375, 1608, 21, 4722, 576, 1364, 12205, 1936, 4540, 4423, 12206, 504, 7465, 7358, 7963, 4458, 4590, 9794, 12226, 12227, 8048, 12228, 12229, 7417, 1110, 1611, 12272, 12273, 5307, 9545, 5297, 1255, 4690, 1241, 1610, 4902, 9742, 12277, 12438, 10878, 4588, 1874, 12439, 10575, 8136, 12473, 10893, 9542, 12476, 12477, 4393, 12493, 12494, 12495, 12496, 12502, 12503, 1092, 12504, 12505, 12509, 12525, 12530, 12533, 12626, 12628, 12648, 12650, 12651, 12664, 12668, 12672, 12673, 12674, 12675, 12676, 2]

// Module 12200 (ChatInput)
...
import DiceRollStore from "DiceRollStore" /* 12201 */;
...
import EditMessageStore from "EditMessageStore" /* 7877 */;
...
const ChatInputConstants = fn(12204);
...
const forwardRefResult = noop.forwardRef((channel, ref) => {
  ...
  const mobileEmojiSuggestionsConfig = channel(12205).useMobileEmojiSuggestionsConfig({ location: "ChatInput" });
  const InlineEmojiSuggestionsEnabled = channel(1936).InlineEmojiSuggestionsEnabled;
  let tmp66Result30 = mobileEmojiSuggestionsConfig.enabled && InlineEmojiSuggestionsEnabled.useSetting();
  let obj = channel(12205);
  const gradientValue = channel(4540).useGradientValue(tmp(4540).GradientPercentage.END);
  let tmpResult = channel(4540);
  const token = channel(4423).useToken(screenIndex(576).modules.mobile.CHAT_INPUT_ACTION_BUTTON_SIZE);
  const tmpResult30 = channel(4423);
  let result = (channel(4423).useToken(screenIndex(576).modules.mobile.CHAT_INPUT_SEND_BUTTON_HEIGHT) - token) / 2;
  const tmpResult31 = channel(4423);
  const token1 = channel(4423).useToken(screenIndex(576).modules.mobile.CHAT_INPUT_FLOATING_SCRIM_GRADIENT_HEIGHT);
  const tmp10 = closure_40(gradientValue, token1);
  const tmpResult32 = channel(4423);
  let token2 = gradientValue;
  if (gradientValue == null) {
    token2 = tmpResult33.useToken(screenIndex(576).colors.BACKGROUND_BASE_LOWER);
  }
  tmpResult33 = channel(4423);
  const token3 = channel(4423).useToken(tmp6(576).modules.mobile.CHAT_INPUT_FLOATING_TYPING_GRADIENT_HEIGHT_REDUCED);
  const tmpResult34 = channel(4423);
  const token4 = channel(4423).useToken(tmp6(576).modules.mobile.CHAT_INPUT_FLOATING_INLINE_FULL_GRADIENT_HEIGHT);
  const tmpResult35 = channel(4423);
  const token5 = channel(4423).useToken(tmp6(576).modules.mobile.CHAT_INPUT_FLOATING_SCRIM_GRADIENT_HEIGHT_AT_BOTTOM);
  const tmpResult36 = channel(4423);
  ...
  const typingUserIdsForDisplay = channel(12206).useTypingUserIdsForDisplay(channel.id, 1);
  const tmp25 = closure_22(screenIndex);
  const tmpResult37 = channel(12206);
  let result1 = channel(12206).hasTypingIndicatorContent(channel, typingUserIdsForDisplay, tmp25);
  ...
  const tmpResult38 = channel(12206);
  ...
  const analyticsLocations = tmp6(7358)().analyticsLocations;
  let tmp35 = tmp21;
  if (null == threadCreationCallback) {
    tmp35 = null != stateFromStores;
  }
  if (!tmp35) {
    tmp35 = !tmp(7465).getIsActiveChannelOrUnarchivableThread(channel);
    const tmpResult43 = tmp(7465);
  }
  const tmpResult42 = channel(504);
  let canStartThread = channel(7465).useCanStartThread(channel);
  if (canStartThread) {
  ...
  const tmpResult44 = channel(7465);
  const tmp39 = channel(7963).useCanPostPollsInChannel(channel) && null == threadCreationCallback;
  const tmpResult45 = channel(7963);
  sharedValue = channel(4458).useSharedValue(token);
  const tmpResult46 = channel(4458);
  sharedValue1 = channel(4458).useSharedValue(token);
  ...
  const tmp43 = screenIndex(4590)();
  const tmp44 = useVoiceMessagesUIStore((startTimeMillis) => null != startTimeMillis.startTimeMillis);
  let result3 = !tmp21;
  let isAppLauncherEnabled = result3;
  if (null == threadCreationCallback) {
    isAppLauncherEnabled = tmp(9794).getIsAppLauncherEnabled(channel);
    const tmpResult48 = tmp(9794);
  }
  const tmpResult47 = channel(4458);
  ...
  ({ placeholder, accessibilityLabel } = screenIndex(12226)({ channel, isReadonly: !editable, isCreatingThread: null != threadCreationCallback }));
  let tmp48 = screenIndex(12226)({ channel, isReadonly: !editable, isCreatingThread: null != threadCreationCallback });
  class Ze {
    constructor() {
  ...
  const animatedStyle = channel(4458).useAnimatedStyle(Ze);
  ref = obj10.useRef(null);
  const tmpResult50 = channel(4458);
  const refreshChatInputCoachmark = channel(12227).useRefreshChatInputCoachmark({ disabled: !editable });
  let obj3 = { disabled: !editable };
  const tmpResult51 = channel(12227);
  const canUseScheduledMessages = channel(8048).useCanUseScheduledMessages();
  const tmpResult52 = channel(8048);
  const items10 = [memo1];
  ...
  const scheduledMessageDraftCoachmarkState = channel(12228).useScheduledMessageDraftCoachmarkState(obj4);
  ...
  const tmp56 = screenIndex(12229)({ chatInputProps: { analyticsLocations, canUpload, channel, defaultValue: memo, hasAttachmentsToUpload: stateFromStores2, pendingEdit: stateFromStores, pendingReply: stateFromStores1, screenIndex, secondaryTextFieldRef, threadCreationCallback }, chatInputTextFieldHeight: sharedValue1, ref });
  ...
  memo1 = obj10.useMemo(() => {
    let obj = {
      ...
      handleChangeContentSize(nativeEvent) {
        ...
        if (!obj.getIsChatInputHeightWorkletEnabled()) {
          ...
          const result = textFieldHeight.set(channel(12273).getChatInputHeightAnimationTiming(height, sharedValue.get()));
          const tmp2Result = channel(12273);
        }
      },
      ...
      handleMaxHeightChanged() {
        if (!obj.getIsChatInputHeightWorkletEnabled()) {
          ...
          if (0 !== textFieldContentSize) {
            const textFieldHeight = closure_1_13.state.current.textFieldHeight;
            const result = textFieldHeight.set(channel(12273).getChatInputHeightAnimationTiming(textFieldContentSize, sharedValue.get()));
            const tmpResult = channel(12273);
          }
        }
      },
        ...
        try {
          ...
          if (0 === c3) {
            ...
            if (1 === tmp5) {
              if (arg0 === 1) {
                ...
                if (closure_1_13.state.current.focused) {
                  if (closure_1_13.props.current.canUpload) {
                    c3 = 2;
                    c4 = 1;
                    const obj6 = { value: closure_0(5307).getImageDimensionsIfMissing(closure_129_0, closure_129_1, closure_129_2), done: false };
                    return obj6;
                  }
                }
                c4 = 3;
              ...
              const size = { uri: closure_129_0, originalUri: closure_129_0, width: closure_129_4.width, height: closure_129_4.height, mimeType: closure_129_3, platform: closure_0(5297).UploadPlatform.REACT_NATIVE, id: null };
              const obj7 = screenIndex(9545);
    ...
    obj.handlePressAction = function handlePressAction(arg0, arg1, current2) {
      if (constants.PHOTOS === arg1) {
        const result = channel(4690).triggerHapticFeedback(channel(4690).HapticFeedbackTypes.IMPACT_LIGHT);
        const obj21 = channel(4690);
        ...
        const keyboardType = channel(4590).getKeyboardType();
        if (keyboardType === channel(1610).KeyboardTypes.APP_LAUNCHER) {
          ...
          const keyboardType1 = tmp42(4590).getKeyboardType();
          ...
          const tmp42Result = tmp42(4590);
        }
        const obj24 = channel(4590);
      } else {
        let tmp35 = current2;
        if (tmp.APPS === arg1) {
          const result1 = channel(4690).triggerHapticFeedback(channel(4690).HapticFeedbackTypes.IMPACT_LIGHT);
          const obj12 = channel(4690);
          channel(4902).trackWithMetadata(constants2.APP_LAUNCHER_ENTRYPOINT_BUTTON_CLICKED);
          const obj13 = channel(4902);
          ...
          const result2 = c2(9742).dismissNewActivityIndicator();
          const obj16 = c2(9742);
          if (tmp35 == null) {
            tmp35 = null;
          }
          const result3 = channel(12277).setAppLauncherA11yFocusReturnRef(tmp35);
          ...
          const obj17 = channel(12277);
          const appDMApplication = channel(12438).getAppDMApplication(tmp30.props.current.channel);
          let name;
          ...
          const tmp23Result = channel(12438);
        } else if (tmp.ALL_PHOTOS === arg1) {
          const result4 = channel(4690).triggerHapticFeedback(channel(4690).HapticFeedbackTypes.IMPACT_LIGHT);
          const obj9 = channel(4690);
          const obj20 = {
            channel: closure_1_13.props.current.channel,
            uploadLimit,
            onDismissKeyboard() {
                  return closure_1_0(4588).dismissKeyboard();
                },
            onRestoreKeyboard() {
                  return closure_1_14.handleToggleKeyboard({ type: closure_0(1610).KeyboardTypes.SYSTEM });
                },
            onSelectFiles(items) {
                  closure_0(10878).addImagesFromPicker(closure_1_13.props.current.channel.id, items, closure_0(5297).UploadOrigin.IMAGE_PICKER);
                },
            draftType: registerViewTag.ChannelMessage
          };
          channel(10878).handleViewAllDialog(obj20);
          const obj10 = channel(10878);
        } else if (tmp.CAMERA === arg1) {
          const result5 = channel(4690).triggerHapticFeedback(channel(4690).HapticFeedbackTypes.IMPACT_LIGHT);
          const obj6 = channel(4690);
          const obj23 = {
            channel: closure_1_13.props.current.channel,
          ...
          channel(10878).handleCameraDialog(obj23);
          const obj7 = channel(10878);
        } else if (tmp.NITRO_GIFT === arg1) {
          const result6 = screenIndex(1874).markPotentialBadState();
          const obj = screenIndex(1874);
          const result7 = channel(4690).triggerHapticFeedback(channel(4690).HapticFeedbackTypes.IMPACT_LIGHT);
          const obj2 = channel(4690);
          if (obj3.isAndroid()) {
            tmp5(4588).dismissKeyboard();
            const tmp5Result = tmp5(4588);
          }
          obj3 = channel(1364);
          channel(10878).handleSelectGift(closure_1_13.props.current.analyticsLocations, closure_1_13.chatInput, tmp35);
          const tmp5Result2 = channel(10878);
        } else if (tmp.THREAD === arg1) {
          const result8 = channel(4690).triggerHapticFeedback(channel(4690).HapticFeedbackTypes.IMPACT_LIGHT);
          const obj28 = channel(4690);
          channel(10878).handleSelectThread(closure_1_13.props.current.channel, closure_1_13.chatInput);
          const obj29 = channel(10878);
        }
      }
    };
    obj.handlePollsPress = function handlePollsPress() {
      const result = channel(4690).triggerHapticFeedback(channel(4690).HapticFeedbackTypes.IMPACT_LIGHT);
      const obj = channel(4690);
      ...
      channel(4588).dismissKeyboard();
      const obj4 = channel(4588);
      channel(12439).openCreatePollModal({
        channel: closure_1_13.props.current.channel,
        onCancel() {
          return closure_1_14.handleToggleKeyboard({ type: closure_0(1610).KeyboardTypes.SYSTEM });
        }
    ...
    obj.handleAttachPress = function handleAttachPress() {
      const result = channel(4690).triggerHapticFeedback(channel(4690).HapticFeedbackTypes.IMPACT_LIGHT);
      const obj = channel(4690);
      channel(10878).handleAttachFile({
        channel: closure_1_13.props.current.channel,
        uploadLimit,
        onDismissKeyboard() {
        ...
        onSelectFiles(items) {
          closure_0(10878).addImagesFromPicker(props.props.current.channel.id, items, closure_0(5297).UploadOrigin.FILE_ATTACHMENT);
        }
      });
    };
    obj.handlePressExpression = function handlePressExpression(context) {
      const result = channel(10575).initiateEmojiInteraction(EmojiInteractionPoint.ChatInputExpressionPressed);
      const obj = channel(10575);
    ...
    obj.handleSelectionOrTextChange = function handleSelectionOrTextChange(nativeEvent) {
      ...
      if (closure_1_13.state.current.editId !== editId) {
        ...
        channel(8136).hideContextMenu();
        const obj3 = channel(8136);
      }
    };
    ...
    obj.handleTextOrFocusChange = function handleTextOrFocusChange(text, focused) {
      if (text.length > 0) {
        if (!focused) {
          const maxMessageLength = channel(9542).getMaxMessageLength();
          if (tmp16) {
        ...
        if (isCoachmarkVisible.isOpen()) {
          screenIndex(10893).hideNativeMenu();
          const obj = screenIndex(10893);
        }
  ...
  const tmp62 = screenIndex(12272)({ textFieldHeight: sharedValue1, textFieldMinHeight: sharedValue });
  ...
  const tmpResult54 = channel(12228);
  if (editable) {
    ...
    let result2 = result3;
    if (!tmp21) {
      result2 = tmp(4393).isPremiumGiftingSupported();
      const tmpResult55 = tmp(4393);
    }
    ...
    tmp66Result = tmp66(tmp6(12477), obj7);
    const tmp6Result = tmp6(12477);
  }
  ...
  const tmp67 = closure_37(screenIndex(12476), { canUpload, channelId: channel.id, screenIndex });
  obj9.customKeyboard = channel(12494).PORTAL_KEYBOARD_PLACEHOLDER_INSTANCE;
  ...
  const items19 = [closure_37(screenIndex(12493), obj9), closure_37(screenIndex(12495), { keyboardType: tmp43, onSelectKeyboard: memo1.handleToggleKeyboard, ref: tmp56.chatInputCover })];
  obj8.children = items19;
  let obj11 = { keyboardType: tmp43, onSelectKeyboard: memo1.handleToggleKeyboard, ref: tmp56.chatInputCover };
  const tmp6Result7 = screenIndex(12493);
  if (editable) {
    ...
    let tmp66Result17 = tmp66(tmp6(12496), obj12);
    const tmp6Result8 = tmp6(12496);
  } else {
    tmp66Result17 = null;
  }
  let obj13 = { collapsable: false, onLayout: callback2, style: null, children: null };
  const items20 = [screenIndex(12502)({ isCreatingThread: null != threadCreationCallback }), tmp10.overflowVisible, ];
  ...
  if (!result1) {
    ...
    tmp66Result18 = tmp66(tmp(12503).ChatInputScrimGradient, obj14);
  }
  ...
  items21[2] = closure_37(channel(12504).ChatInputAccessibilityDivider, {});
  ...
  if (tmp22) {
    ...
    tmp66Result20 = tmp66(tmp6(12505), obj16);
    const tmp6Result9 = tmp6(12505);
  }
  ...
  if (isResourceChannel) {
    let obj21 = { channel };
    tmp66Result24 = tmp66(tmp6(12525), obj21, channel.id);
  }
  items21[5] = tmp66Result24;
  items21[6] = closure_37(channel(12530).MemberActionsChatInputBannerGuardedOuter, { channel });
  items21[7] = closure_37(channel(12533).DoubleTapToReactChatInputBanner, { channel });
  let tmp66Result25 = null;
  if (tmp23) {
    let obj22 = { channelId: channel.id };
    tmp66Result25 = tmp66(tmp6(12626), obj22);
  }
  ...
  if (tmp43 !== channel(1610).KeyboardTypes.EXPRESSION) {
    ...
    tmp66Result26 = tmp66(tmp6(12628), obj23);
  }
  items21[9] = tmp66Result26;
  items21[10] = closure_37(screenIndex(12648), { ref: tmp56.chatInputAppCommandManager, canOnlyUseTextCommands: null != stateFromStores1, channel, chatInputRef: tmp56.chatInput, chatInputStateRef: tmp56.state, commandsDisabled: tmp35 });
  ...
  items24[1] = closure_37(screenIndex(12651), { channel });
  ...
  const items26 = [closure_37(screenIndex(12664), { channel, chatInputRef: tmp56.chatInput, pendingEdit: stateFromStores, pendingReply: stateFromStores1 }), , ];
  ...
  if (tmp66Result27) {
    ...
    tmp66Result27 = tmp66(tmp(12668).EmojiSuggestionBarLarge, obj28);
  }
  ...
  obj32.style = items28;
  const items29 = [tmp74, closure_37(screenIndex(12672), { analyticsLocations, ref: tmp56.chatInputCharCounter })];
  ...
  if (editable) {
    ...
    if (!tmp21) {
      result3 = tmp(4393).isPremiumGiftingSupported();
      const tmpResult58 = tmp(4393);
    }
    ...
    obj35.children = tmp66(tmp6(12673), obj38);
    tmp66Result29 = tmp66(tmp77, obj35);
    const tmp6Result11 = tmp6(12673);
  }
  ...
  if (tmp66Result30) {
    ...
    tmp66Result30 = tmp66(tmp(12674).EmojiSuggestionBarSmall, obj39);
  }
  items24[3] = tmp66Result30;
  obj25.children = closure_38(closure_39, { children: items24 });
  items21[11] = closure_37(screenIndex(12650), obj25);
  ...
  if (null != refreshChatInputCoachmark) {
    ...
    tmp66Result31 = tmp66(tmp6(12227), obj40);
    const tmp6Result12 = tmp6(12227);
  }
  items21[12] = tmp66Result31;
  items21[13] = closure_37(screenIndex(12675), { buttonRef: ref, isVisible: isCoachmarkVisible, onDismiss: dismissCoachmark });
  ...
  if (null == threadCreationCallback) {
    ...
    tmp66Result32 = tmp66(tmp6(12676), obj41);
  }
