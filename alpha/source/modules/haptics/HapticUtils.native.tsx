...
export const triggerHapticFeedback = function triggerHapticFeedback(IMPACT_LIGHT) {
  if (haptics_HapticFeedbackTypesDefault.IMPACT_LIGHT === IMPACT_LIGHT) {
    ...
    if (obj42.isAndroid()) {
      ...
      str36 = str37;
      tmp69Result = tmp69(4701);
    }
    obj41.trigger(str36);
    obj42 = PlatformUtils;
  } else if (tmp(4691).IMPACT_MEDIUM === IMPACT_LIGHT) {
    ...
    if (obj39.isAndroid()) {
      ...
      str34 = str35;
      tmp66Result = tmp66(4701);
    }
    obj38.trigger(str34);
    obj39 = PlatformUtils;
  } else if (tmp(4691).IMPACT_HEAVY === IMPACT_LIGHT) {
    Patterns.trigger("impactHeavy");
  } else if (tmp(4691).NOTIFICATION_ERROR === IMPACT_LIGHT) {
    Patterns.trigger("notificationError");
  } else if (tmp(4691).DRAG_AND_DROP_START === IMPACT_LIGHT) {
    ...
    obj34.trigger(str31);
    obj35 = PlatformUtils;
  } else if (tmp(4691).DRAG_AND_DROP_END === IMPACT_LIGHT) {
    ...
    if (obj32.isAndroid()) {
      ...
      str29 = str30;
      tmp57Result = tmp57(4701);
    }
    obj31.trigger(str29);
    obj32 = PlatformUtils;
  } else if (tmp(4691).DRAG_AND_DROP_MOVE === IMPACT_LIGHT) {
    ...
    if (obj29.isAndroid()) {
      ...
      str27 = str28;
      tmp54Result = tmp54(4701);
    }
    obj28.trigger(str27);
    obj29 = PlatformUtils;
  } else if (tmp(4691).SOFT === IMPACT_LIGHT) {
    Patterns.trigger("soft");
  } else if (tmp(4691).SELECTION === IMPACT_LIGHT) {
    ...
    if (obj25.isAndroid()) {
      ...
      str24 = str25;
      tmp49Result = tmp49(4701);
    }
    obj24.trigger(str24);
    obj25 = PlatformUtils;
  } else if (tmp(4691).RIGID === IMPACT_LIGHT) {
    Patterns.trigger("rigid");
  } else if (tmp(4691).NOTIFICATION_SUCCESS === IMPACT_LIGHT) {
    Patterns.trigger("notificationSuccess");
  } else if (tmp(4691).NOTIFICATION_WARNING === IMPACT_LIGHT) {
    Patterns.trigger("notificationWarning");
  } else if (tmp(4691).CONFIRM === IMPACT_LIGHT) {
    Patterns.trigger("confirm");
  } else if (tmp(4691).REJECT === IMPACT_LIGHT) {
    Patterns.trigger("reject");
  } else if (tmp(4691).GESTURE_START === IMPACT_LIGHT) {
    Patterns.trigger("gestureStart");
  } else if (tmp(4691).GESTURE_END === IMPACT_LIGHT) {
    Patterns.trigger("gestureEnd");
  } else if (tmp(4691).SEGMENT_TICK === IMPACT_LIGHT) {
    Patterns.trigger("segmentTick");
  } else if (tmp(4691).SEGMENT_FREQUENT_TICK === IMPACT_LIGHT) {
    Patterns.trigger("segmentFrequentTick");
  } else if (tmp(4691).TOGGLE_ON === IMPACT_LIGHT) {
    Patterns.trigger("toggleOn");
  } else if (tmp(4691).TOGGLE_OFF === IMPACT_LIGHT) {
    Patterns.trigger("toggleOff");
  } else if (tmp(4691).CLOCK_TICK === IMPACT_LIGHT) {
    Patterns.trigger("clockTick");
  } else if (tmp(4691).CONTEXT_CLICK === IMPACT_LIGHT) {
    Patterns.trigger("contextClick");
  } else if (tmp(4691).KEYBOARD_PRESS === IMPACT_LIGHT) {
    Patterns.trigger("keyboardPress");
  } else if (tmp(4691).KEYBOARD_RELEASE === IMPACT_LIGHT) {
    Patterns.trigger("keyboardRelease");
  } else if (tmp(4691).KEYBOARD_TAP === IMPACT_LIGHT) {
    Patterns.trigger("keyboardTap");
  } else if (tmp(4691).LONG_PRESS === IMPACT_LIGHT) {
    Patterns.trigger("longPress");
  } else if (tmp(4691).TEXT_HANDLE_MOVE === IMPACT_LIGHT) {
    Patterns.trigger("textHandleMove");
  } else if (tmp(4691).VIRTUAL_KEY === IMPACT_LIGHT) {
    Patterns.trigger("virtualKey");
  } else if (tmp(4691).VIRTUAL_KEY_RELEASE === IMPACT_LIGHT) {
    Patterns.trigger("virtualKeyRelease");
  } else if (tmp(4691).EFFECT_CLICK === IMPACT_LIGHT) {
    Patterns.trigger("effectClick");
  } else if (tmp(4691).EFFECT_DOUBLE_CLICK === IMPACT_LIGHT) {
    Patterns.trigger("effectDoubleClick");
  } else if (tmp(4691).EFFECT_HEAVY_CLICK === IMPACT_LIGHT) {
    Patterns.trigger("effectHeavyClick");
  } else if (tmp(4691).EFFECT_TICK === IMPACT_LIGHT) {
