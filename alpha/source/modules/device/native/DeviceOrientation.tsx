...
function handleOrientationChange(initialOrientation) {
  ...
  if (global !== initialOrientation) {
    ...
    if (!tmp9) {
      let isIpadOSResult = tmp(4735).isIpadOS();
      if (isIpadOSResult) {
      ...
      if ("LANDSCAPE" === orientationLock) {
        ...
        if (!obj13.isAndroid()) {
          if (tmp14Result.isIOS()) {
            tmp14(4735).getSystemVersionMajor() >= 16;
            const tmp14Result3 = tmp14(4735);
          }
      ...
      if ("PORTRAIT" === orientationLock) {
        ...
        if (!obj12.isAndroid()) {
          if (tmp12Result.isIOS()) {
            tmp12(4735).getSystemVersionMajor() >= 16;
            const tmp12Result3 = tmp12(4735);
          }
...
export const restoreDefaultOrientation = function restoreDefaultOrientation() {
  ...
  if (!tmpResult5.isAndroid()) {
    if (tmpResult6.isIOS()) {
      tmp(4735).getSystemVersionMajor() >= 16;
