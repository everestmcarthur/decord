...
// Dependencies: [19, 14906, 11715, 14801, 14908, 1484, 2]
// Exports: useAutoScrollToSearchResultSetting

// Module 14912 (useAutoScrollToSetting)
...
export const useAutoScrollToSearchResultSetting = function useAutoScrollToSearchResultSetting(ref, memo, scrollTarget) {
  ...
  if (null != current) {
    flag = false;
    if (tmp(14801).SETTING_RENDERER_CONFIG[current].type !== current.ROUTE) {
      let initialScrollIndex = tmp(14908).getInitialScrollIndex(current, memo);
      let tmp7 = 0 !== initialScrollIndex;
      if (tmp7) {
        tmp7 = 1 !== initialScrollIndex;
      }
      flag = tmp7;
      const tmpResult = tmp(14908);
    }
  }
