...
// Dependencies: [32, 19, 17, 2040, 9403, 4755, 12324, 4395, 12295, 1483, 1074, 2038, 21, 576, 12328, 4756, 4752, 1115, 12307, 9402, 10371, 8122, 5817, 12302, 1397, 11258, 12335, 9739, 504, 7763, 12336, 5799, 9183, 12337, 12311, 7183, 1612, 12338, 8537, 7289, 11479, 1478, 9522, 9592, 12340, 12339, 12341, 12342, 12344, 12345, 12303, 7761, 12346, 12, 4936, 7278, 12348, 12349, 9047, 1249, 4576, 2027, 12350, 12351, 12353, 7290, 12355, 1370, 12361, 12362, 12364, 12367, 12368, 12329, 12369, 12290, 12370, 12373, 9599, 12322, 9530, 12374, 12359, 12375, 1385, 12376, 12377, 2]
// Exports: default

// Module 12334 (AppLauncherHomeScreen)
...
import MessagePreviewMarkup from "MessagePreviewMarkup" /* 10371 */;
...
import EntityBorderAppIconDefault from "EntityBorderAppIcon" /* 12307 */;
import ApplicationDirectoryActionCreators from "ApplicationDirectoryActionCreators" /* 12322 */;
import HeroMedia from "HeroMedia" /* 12335 */;
import AppLauncherHomeTypes from "AppLauncherHomeTypes" /* 12339 */;
import AppLauncherOnboardingActionCreators from "AppLauncherOnboardingActionCreators" /* 12350 */;
import InThisServerSection from "InThisServerSection" /* 12361 */;
import ApplicationDirectoryCollectionType from "ApplicationDirectoryCollectionType" /* 12374 */;
import _slicedToArray from "module_32" /* 32 */;
import noop from "module_19" /* 19 */;
import EmbeddedActivitiesStore from "EmbeddedActivitiesStore" /* 2040 */;
import DevSettingsStore from "DevSettingsStore" /* 4755 */;
import ApplicationDirectoryCollectionsStore from "ApplicationDirectoryCollectionsStore" /* 12324 */;
...
class BaseAppRow {
  constructor(arg0) {
    ...
    memo = closure_5.useMemo(() => {
      ...
      if (application == null) {
        FAKE_BUILT_IN_APP = tmp(9402).FAKE_BUILT_IN_APP;
      }
      ...
      if (null != sectionDescription) {
        ...
        if ("" !== sectionDescription) {
          const obj2 = { content: sectionDescription, muted: false, layout: tmp(8122).ChannelListLayoutTypes.COMPACT, color: "text-muted" };
          result = tmp(10371).renderMessagePreviewMarkup(obj2);
...
function ActivityRow(section) {
  ...
  if (tmp5) {
    ...
    tmp5 = closure_19(onPress(12307), obj3);
  }
  const items = [section];
  ...
  return closure_19(section(5817).TableRow, { icon: tmp5, label: section.application.name, subLabel: memo, subLabelLineClamp: 1, start: flag, end: flag2, arrow: true, onPress: callback });
}
...
function RecommendationItem(onPress) {
  ...
  if (first) {
    let tmp13 = closure_1_19(tmp7(12336), {});
  } else {
    tmp13 = null;
    if (!tmp12) {
      const obj6 = {
      ...
      obj6.source = obj7;
      tmp13 = closure_1_19(tmp7(5799), obj6);
    }
  }
  ...
  if (isEmbeddedAppResult) {
    ...
    const tmp7Result = tmp7(12337);
    obj10.labelType = tmp2(9402).getShelfBadgeTypeIfActive(application);
...
let closure_23 = [];
const APP_LAUNCHER_IN_TEXT = fn(12328).ApplicationCollectionSurface.APP_LAUNCHER_IN_TEXT;
    ...
    if (0 !== handleViewableItemsChanged.length) {
      if (initialSearchQuery === AppLauncherTypes.AppLauncherEntrypoint.VOICE) {
        ...
        if (arr.length % 2 === 1) {
          ...
          obj2.type = tmp8(12339).AppLauncherHomeListItemType.SHELF_ITEM_TUPLE;
        ...
        const obj5 = { type: tmp8(12339).AppLauncherHomeListItemType.SECTION_HEADER, section: null, sectionName: "activities", numItems: null, numVisibleItems: null };
    ...
    if (memo8) {
      ...
      let item = onActivityItemSelected.forEach((title, sectionOverallPosition) => {
        ...
        if (type === ApplicationDirectoryCollectionType.ApplicationDirectoryCollectionType.BANNER_CARDS) {
          ...
          const bound = Math.min(length, tmp(12359).COLLAPSED_LIST_ITEM_MAX);
          obj.type = tmp(12339).AppLauncherHomeListItemType.RECOMMENDATION_SECTION_HEADER;
          ...
          const mapped = application_directory_collection_items.map((type, sectionPosition) => {
            if (type.type === found1(9530).ApplicationDirectoryCollectionItemType.APPLICATION) {
              let collectionItemAssetUrl;
              if (tmp4) {
                ({ id: obj2.itemId, image_hash: obj2.hash } = type);
                collectionItemAssetUrl = tmp(12375).getCollectionItemAssetUrl({ itemId: null, hash: null });
                const obj = { itemId: null, hash: null };
                const tmpResult = tmp(12375);
              }
              const obj3 = { application: type.application, showsPromoted: tmp(1385).hasFlag(type.flags, tmp(12376).ApplicationCollectionItemFlags.PROMOTED), overrideImageUrl: collectionItemAssetUrl, sectionPosition };
              return obj3;
            }
          });
        ...
        } else if (type === tmp(12374).ApplicationDirectoryCollectionType.EXPANDABLE_LIST) {
          const prop = title.application_directory_collection_items;
          const mapped1 = prop.map((type) => {
            if (type.type === found1(9530).ApplicationDirectoryCollectionItemType.APPLICATION) {
              const obj = { application: type.application, showsPromoted: tmp(1385).hasFlag(type.flags, tmp(12376).ApplicationCollectionItemFlags.PROMOTED) };
              return obj;
            }
          });
          const found1 = mapped1.filter(tmp(1370).isNotNullish);
          if (tmpResult.hasFlag(title.flags, tmp(12377).ApplicationCollectionFlags.APPENDS_REMAINING_ACTIVITIES)) {
  ...
  if (tmp51) {
    ...
    let obj13 = {
      ...
      onViewAllSelected(sectionItemType) {
          ...
          } else if (sectionItemType === tmp(12303).SectionItemType.COMMANDS) {
    ...
    let obj14 = {
      ...
      onAppSelected: callback3,
      onViewAllSelected() {
          const found = memo.find((type) => type.type === context(12339).AppLauncherHomeListItemType.VIEW_ALL);
          let mapped;
          if (found != null) {
