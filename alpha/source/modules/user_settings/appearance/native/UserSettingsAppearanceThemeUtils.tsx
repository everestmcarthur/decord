...
// Dependencies: [1227, 1182, 1185, 1074, 1229, 1374, 1230, 1186, 15384, 12190, 9596, 1228, 4570, 15385, 1241, 2]
...
export const handleSaveTheme = function handleSaveTheme(mobileThemes, analyticsLocations, isSynced) {
  if (mobileThemes.type === ClientThemesTypes.ClientThemeType.CUSTOM_BACKGROUND_GRADIENT) {
    ...
    if ("system" === mobileThemes.theme) {
      const result = tmp(15384).resetBackgroundGradientPreset();
      const tmpResult = tmp(15384);
      tmp(12190).resetCustomTheme();
      const tmpResult13 = tmp(12190);
      const obj3 = { theme: mobileThemes.theme };
      return tmp(9596).saveClientTheme(obj3);
      ...
      const result1 = tmp(15384).resetBackgroundGradientPreset();
      const tmpResult16 = tmp(15384);
      tmp(12190).updateCustomTheme(mobileThemes.customThemeSettings, customThemeBaseTheme);
      const tmpResult17 = tmp(12190);
      const obj4 = { customUserThemeSettings: mobileThemes.customThemeSettings, theme: customThemeBaseTheme };
      return tmp(9596).saveClientTheme(obj4);
    } else {
      if (mobileThemes.type === tmp(1230).ClientThemeType.BACKGROUND_GRADIENT_PRESET) {
        const result2 = tmp(15384).updateBackgroundGradientPreset(mobileThemes.id);
        const tmpResult19 = tmp(15384);
        tmp(12190).resetCustomTheme();
        const tmpResult20 = tmp(12190);
        ({ id: obj10.backgroundGradientPresetId, theme: obj10.theme } = mobileThemes);
        let saveClientThemeResult = tmp(9596).saveClientTheme({ backgroundGradientPresetId: null, theme: null });
        const obj5 = { backgroundGradientPresetId: null, theme: null };
        const tmpResult21 = tmp(9596);
      } else {
        const result3 = tmp(15384).resetBackgroundGradientPreset();
        const tmpResult22 = tmp(15384);
        tmp(12190).resetCustomTheme();
        const tmpResult23 = tmp(12190);
        const obj6 = { theme: mobileThemes.theme };
        saveClientThemeResult = tmp(9596).saveClientTheme(obj6);
        const tmpResult24 = tmp(9596);
      }
...
export const handleSaveSyncedModeTheme = function handleSaveSyncedModeTheme(mobileThemes, systemTheme, analyticsLocations) {
  if ("system" !== mobileThemes.theme) {
    if (mobileThemes.type === ClientThemesTypes.ClientThemeType.CUSTOM_BACKGROUND_GRADIENT) {
      ...
      const result = tmp14(4570).updateThemePreferences(obj3);
      if (mobileThemes.type === tmp14(1230).ClientThemeType.CUSTOM_BACKGROUND_GRADIENT) {
        const obj4 = { customUserThemeSettings: mobileThemes.customThemeSettings };
        const result1 = tmp14(4570).updateSyncedClientTheme(systemTheme, obj4);
        const tmp14Result6 = tmp14(4570);
        ...
        const result2 = tmp14(4570).updateSyncedClientTheme(systemTheme, obj5);
        const tmp14Result7 = tmp14(4570);
      } else {
        const obj6 = { theme: mobileThemes.theme };
        const result3 = tmp14(4570).updateSyncedClientTheme(systemTheme, obj6);
        const tmp14Result8 = tmp14(4570);
      }
      const tmp14Result5 = tmp14(4570);
    } else if (mobileThemes.type === tmp14(1230).ClientThemeType.BACKGROUND_GRADIENT_PRESET) {
