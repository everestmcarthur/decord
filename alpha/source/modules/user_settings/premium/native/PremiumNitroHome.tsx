...
// Dependencies: [32, 19, 17, 4748, 4420, 13657, 1074, 2038, 1374, 21, 672, 576, 4756, 1484, 13658, 5334, 5840, 4752, 1115, 1241, 7620, 13659, 8314, 13660, 13681, 10998, 13685, 13686, 13687, 13692, 13696, 13713, 13714, 13716, 13717, 7625, 1612, 13718, 504, 7633, 8323, 13719, 9889, 4492, 5185, 4607, 13720, 4576, 2027, 8313, 2029, 5799, 13721, 13722, 12867, 9890, 5198, 1364, 5173, 2]
// Exports: default

// Module 13656 (PremiumNitroHome)
...
import PremiumPerkCardDefault from "PremiumPerkCard" /* 13660 */;
import useScrollToSectionDefault from "useScrollToSection" /* 13685 */;
import PremiumNitroHomeUtils from "PremiumNitroHomeUtils" /* 13686 */;
import _slicedToArray from "module_32" /* 32 */;
import noop from "module_19" /* 19 */;
import AccessibilityStore from "AccessibilityStore" /* 4748 */;
import SubscriptionStore from "SubscriptionStore" /* 4420 */;
import PremiumNitroNavigationStore from "PremiumNitroNavigationStore" /* 13657 */;
...
function NitroHomeHeader(arg0) {
  ...
  const mobileNitroManageSubscriptionsSettingsExperiment = onClose(13658).useMobileNitroManageSubscriptionsSettingsExperiment({ location: "NitroHomeHeader" });
  let hasActiveTrial;
  ...
  const items1 = [closure_17(onClose(5334).PressableOpacity, { style: tmp.backButtonWrapper, onPress: callback, children: closure_17(onClose(5840).ArrowLargeLeftIcon, { size: "md", color: "white" }) }), , ];
  ...
  items1[1] = closure_17(onClose(4752).Text, obj6);
  if (mobileNitroManageSubscriptionsSettingsExperiment) {
    ...
    const obj8 = {
      ...
      children: tmp11(tmp2(7620).SettingsIcon, { size: "md", color: "white" })
    };
    tmp11Result = tmp11(tmp2(5334).PressableOpacity, obj8);
  }
  ...
  if (tmp11Result2) {
    const obj9 = { style: tmp.pillParent, children: tmp11(tmp2(13659).PremiumReferralTrialPill, { hasExtraMargin: true }) };
...
function NewTab(arg0) {
  ...
  if (tmp12) {
    ...
    tmp12 = closure_17(tmp8(13687), obj6);
    const tmp8Result = tmp8(13687);
  }
  const items1 = [tmp12, closure_17(require("TieredTenureBadgePerkCard").TieredTenureBadgePerkCard, {}), , , , , ];
  let tmp15Result = null;
  if (isEligibleSenderForReferralProgram) {
    const obj8 = { onLayout: useScrollToSectionDefault(ref, scrollToSectionId).createSectionLayoutHandler(NitroHomeSectionId.REFERRAL_PROGRAM), children: tmp15(tmp2(13696).ReferralProgramPerkCard, {}) };
...
export default function PremiumNitroHome(onClose) {
  ...
  sharedValue = bottom(4492).useSharedValue(num);
  const obj16 = bottom(4492);
  class Q {
    constructor() {
  ...
  const animatedStyle = bottom(4492).useAnimatedStyle(Q);
  ...
  let tmpResult = bottom(4492);
  const theme = bottom(4607).useThemeContext().theme;
  ...
  const effect2 = obj5.useEffect(() => {
    if (!obj.UNSAFE_isDismissibleContentDismissed(dismissible_content.DismissibleContent.MOBILE_NITRO_HOME_SETTINGS_BADGE)) {
      const result = tmp(4576).UNSAFE_markDismissibleContentAsDismissed(tmp(2027).DismissibleContent.MOBILE_NITRO_HOME_SETTINGS_BADGE);
    ...
    if (!tmpResult4.UNSAFE_isDismissibleContentDismissed(dismissible_content.DismissibleContent.REFERRAL_TRIAL_MOBILE_NITRO_HOME_BADGE)) {
      const result1 = tmp(4576).UNSAFE_markDismissibleContentAsDismissed(tmp(2027).DismissibleContent.REFERRAL_TRIAL_MOBILE_NITRO_HOME_BADGE);
      const tmpResult5 = tmp(4576);
    }
    if (isEligibleSenderForReferralProgram) {
      const result2 = tmp(8313).markReferralProgramEntrypointBadgeAcknowledged();
      const tmpResult6 = tmp(8313);
    }
  }, items5);
  ...
  promotionMarketingComponent = bottom(13681).usePromotionMarketingComponent(tmp(10998).MarketingComponentType.PREMIUM_TAB);
  ...
  const items8 = [closure_17(navigation(5799), obj18), closure_17(NitroHomeHeader, { onClose: onClose.onClose, subscription: stateFromStores1 }), , , ];
  ...
  if (tmp26Result) {
    ...
    tmp26Result = tmp26(tmp6(13722), obj19);
  }
  ...
  const items11 = [closure_17(c5, { style: tmp5.segmentedControlActual, onLayout: callback, children: closure_17(bottom(9890).SegmentedControl, { state: segmentedControlState, variant: "experimental_Small" }) }), , ];
  ...
  if (!isThemeDarkResult) {
    ...
    items12[1] = obj25;
    obj24.style = items12;
    tmp26Result5 = tmp26(tmp6(5198), obj24);
  }
  items11[1] = tmp26Result5;
  const obj23 = { style: tmp5.segmentedControlActual, onLayout: callback, children: closure_17(bottom(9890).SegmentedControl, { state: segmentedControlState, variant: "experimental_Small" }) };
  if (tmpResult10.isAndroid()) {
    ...
    tmp26Result6 = tmp26(tmp6(5173), obj28);
    const tmp6Result4 = tmp6(5173);
  }
  items11[2] = tmp26Result6;
  obj22.children = items11;
  items8[4] = closure_18(navigation(4492).View, obj22);
