...
// Dependencies: [5, 32, 19, 21, 4722, 15787, 4433, 1115, 5142, 576, 4718, 4618, 5777, 15788, 2]
// Exports: default, useDiskUsageMeasurement

// Module 15786 (CacheActionsDiskUsageSection)
...
import CacheActionsStorageDiagnosticsDefault from "CacheActionsStorageDiagnostics" /* 15788 */;
...
function SizeRow(bytes) {
  ...
  if (null != bytes) {
    let formatKbSizeResult = tmp2(4618).formatKbSize(bytes);
...
function DiskUsageResults(report) {
  ...
  const items1 = [closure_6(obj(4718).Heading, obj5), , ];
  ...
  if (tmp4Result) {
    ...
    tmp4Result = closure_6(tmp(4718).Text, obj6);
    const obj13 = { errors: null, unavailable: null };
  }
  items1[2] = tmp4Result;
  items[2] = closure_7(obj(5142).Stack, { children: items1 });
  obj2.children = items;
  return closure_7(obj(5142).Stack, obj2);
}
const jsxProd = fn(21);
...
export default function CacheActionsDiskUsageSection(state) {
  ...
  if (tmp4Result) {
    ...
    tmp4Result = tmp4(tmp2(4718).Text, obj2);
  }
  ...
  if (tmp4Result4) {
    ...
    tmp4Result4 = tmp4(tmp2(4718).Text, obj3);
  }
