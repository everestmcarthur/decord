...
// Dependencies: [1372, 4544, 4314, 4546, 4357, 4545, 1091, 504, 573, 2]

// Module 5712 (GuildJoinRequestStore)
  ...
  if (DELETED !== applicationStatus) {
    if (null != DELETED) {
      if (DELETED === MemberVerificationTypes.GuildJoinRequestApplicationStatuses.SUBMITTED) {
        ...
        const result = map.set(guildId, _modDef4314());
      }
      if (applicationStatus === tmp12(4546).GuildJoinRequestApplicationStatuses.SUBMITTED) {
        const _Math = Math;
        dependencyMap[guildId] = Math.max(0, dependencyMap[guildId] - 1);
        const result1 = map.set(guildId, _modDef4314());
      }
      tmp12 = require;
    }
  }
...
const joinRequestFromServer = fn(4544).joinRequestFromServer;
const map = new Map();
const dependencyMap = {};
let closure_8 = {};
let c9 = false;
...
const secondaryIndexMap = new fn(4357).SecondaryIndexMap(guildJoinRequestsIndex, (joinRequestId) => "" + joinRequestId.joinRequestId);
const secondaryIndexMap1 = new fn(4357).SecondaryIndexMap(guildJoinRequestsIndex, (joinRequestId) => "" + joinRequestId.joinRequestId);
const secondaryIndexMap2 = new fn(4357).SecondaryIndexMap(guildJoinRequestsIndex, (actionedAt) => "" + actionedAt.actionedAt);
...
prototype["hasFetched"] = function hasFetched(arg0) {
  if (map.has(arg0)) {
    ...
    let tmp3 = null != value;
    if (tmp3) {
      tmp3 = _modDef4314().diff(value, "seconds") < closure_20;
...
const guildJoinRequestStoreV2 = new GuildJoinRequestStoreV2(DispatcherDefault, {
  ...
  GUILD_JOIN_REQUESTS_FETCH_SUCCESS: function handleFetchSuccess(arg0) {
    ...
    if (status === MemberVerificationTypes.GuildJoinRequestApplicationStatuses.SUBMITTED) {
      ...
      let result = map.set(guildId, _modDef4314());
    }
