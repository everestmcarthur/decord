...
// Dependencies: [5, 32, 19, 17, 1372, 12090, 1074, 21, 4722, 576, 4423, 1484, 504, 38, 9301, 12088, 4419, 4874, 5754, 4718, 12092, 1177, 1115, 4566, 5857, 5774, 5144, 2]
// Exports: default

// Module 12091 (TransferOwnership)
...
const TransferOwnershipConstants = fn(12090);
...
export default function TransferOwnership(guild) {
          ...
          if (arg0 === 1) {
            ...
            if (!mfaEnabled.mfaEnabled) {
              if (null != tmp27.email) {
                v1 = 1;
                dependencyMap = 1;
                const obj6 = { value: v1(9301).sendTransferOwnershipPincode(user.id), done: false };
                return obj6;
              }
            }
            let MFA = null;
            ...
            const obj4 = v1(9301);
            v1(9301).transferOwnership(user.id, id.id, MFA).then(() => {
              v1(12088).close();
              const obj = v1(12088);
              v1(9301).close();
              const obj2 = v1(9301);
              const result = closure_1_0(4419).showTransferOwnershipSuccess();
            }, NOOP);
            dependencyMap = 3;
            const transferOwnershipResult = v1(9301).transferOwnership(user.id, id.id, MFA);
          }
        } else if (arg0 === 1) {
          dependencyMap = 3;
          throw value;
  ...
  const token = guild(4423).useToken(toUser(576).modules.mobile.TABLE_ROW_PADDING);
  let tmp5 = closure_16();
  let obj = guild(4423);
  ...
  nickname = toUser(4874).getNickname(guild.id, undefined, toUser);
  ...
  const items1 = [closure_13(c7, { source: toUser(12092), style: tmp5.arrow }), , , ];
  const obj10 = { style: tmp5.avatarsWrapper, children: null };
  const obj9 = { source: toUser(12092), style: tmp5.arrow };
  ...
  items1[2] = closure_13(guild(4718).Text, { variant: "text-xs/medium", color: "text-default", children: guild.name });
  if (null == nickname) {
    if (!hasAvatarForGuildResult) {
      const intl = tmp(1115).intl;
      const obj15 = { GuildHook: GuildWithSmallIcon, user: tmp3(4566).getUserTag(toUser) };
    ...
    const obj19 = { username: tmp3(4566).getUserTag(toUser) };
    obj18.label = intl3.format(tmp(1115).t.xm6ACJ, obj19);
    obj18.onPress = function handleConfirmToggle(arg0) {
      _undefined(arg0);
    };
    obj17.children = tmp11(tmp(5774).TableCheckboxRow, obj18);
    items3[1] = tmp11(tmp(5857).TableRowGroup, obj17);
    const obj20 = {
    ...
    obj20.disabled = !tmp8;
    items3[2] = tmp11(tmp(5144).Button, obj20);
  ...
  obj21.user = toUser(4566).getUserTag(toUser);
