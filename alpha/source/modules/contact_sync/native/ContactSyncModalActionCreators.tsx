...
// Dependencies: [5, 5451, 1372, 12919, 12921, 12920, 1074, 4932, 12922, 12924, 1241, 12926, 1115, 5067, 4420, 9814, 7241, 4622, 4925, 12927, 1897, 10065, 1094, 12946, 2]
...
// Module 12918 (ContactSyncModalActionCreators)
import ConstantsIOS from "ConstantsIOS" /* 1094 */;
import asyncRequireImpl from "asyncRequireImpl" /* 1897 */;
import ModalActionCreatorsDefault from "ModalActionCreators" /* 4925 */;
import PhoneActionCreatorsDefault from "PhoneActionCreators" /* 7241 */;
...
import ContactSyncAnalyticsUtils from "ContactSyncAnalyticsUtils" /* 12924 */;
      ...
      if (0 === c5) {
        ...
        if (1 === tmp7) {
          ...
          if (value.failed_requests.length > 0) {
            ...
            const obj8 = { title: null, body: null, confirmText: null, onConfirm: null };
...
function closeContactSyncModal(skip) {
  ...
  if (null != onComplete) {
    ...
    flag2(12924).trackFlowEnd(flag, obj2);
    if (tmp16) {
      onComplete(flag);
    } else {
    ...
    if (!back) {
      ...
      flag2(12924).trackFlowEnd(flag2, obj4);
      const obj = flag2(12924);
    }
    if (tmp2) {
      let updateAnimation = back(4925).updateAnimation;
    ...
    const timerId1 = setTimeout(() => {
      ...
      if (closure_2) {
        ...
        if (back) {
          ...
          const result = tmp7(12946).previousOnboardingStep();
          const tmp7Result = tmp7(12946);
        } else {
          updateAnimation(tmp3, ModalAnimation.SLIDE_IN_OUT);
          const obj2 = { skip: flag2 };
          tmp7(12946).nextOnboardingStep(obj2);
          const tmp7Result2 = tmp7(12946);
        }
...
const ContactSyncConstants = fn(12920);
...
export const openContactSyncModal = function openContactSyncModal(initialRoutes, FRIENDS_ADD_FRIENDS_MODAL, arg2) {
  ...
  if (FRIENDS_ADD_FRIENDS_MODAL) {
    ...
    tmp(12924).trackFlowStart(obj2);
    const tmpResult = tmp(12924);
  }
  ...
  ModalActionCreatorsDefault.pushLazy(asyncRequireImpl(12927, tmp2.paths), { initialRoutes: initialRoutes.initialRoutes, openSettingsSheet: initialRoutes.openSettings, customLandingPage: initialRoutes.customLandingPage }, closure_1_20).then(arg2);
};
...
export const openContactSyncModalDeeplink = function openContactSyncModalDeeplink() {
  ...
  ModalActionCreatorsDefault.pushLazy(asyncRequireImpl(12927, tmp2.paths), { initialRoutes: obj.initialRoutes, openSettingsSheet: obj.openSettings, customLandingPage: obj.customLandingPage }, closure_1_20).then(undefined);
};
