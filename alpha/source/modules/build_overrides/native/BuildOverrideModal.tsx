...
// Dependencies: [19, 17, 11677, 21, 4722, 576, 4656, 4573, 14079, 14080, 504, 12032, 4314, 7319, 4718, 1115, 5144, 4925, 2]
// Exports: default

// Module 14078 (BuildOverrideModal)
...
export default function BuildOverrideModal(overrideUrl) {
  ...
  let stateFromStores;
  const tmp = closure_9();
  const tmp4 = stateFromStores(4656)();
  if (obj.isThemeDark(tmp4)) {
    let tmp2Result = tmp2(14079);
  } else {
    tmp2Result = tmp2(14080);
  }
  obj = str(4573);
  ...
  if (override != null) {
    ...
    if (targetBuildOverride != null) {
      const tmp9 = targetBuildOverride[tmp5(undefined, 12032).DEVICE_FIELD];
      if (tmp9 != null) {
        id = tmp9.id;
      }
    }
  ...
  const tmp2Result2 = stateFromStores(4314);
  let expiresAt;
  if (override != null) {
    expiresAt = override.expiresAt;
  }
  const obj4 = stateFromStores(4314)();
  const durationResult = tmp2Result2.duration(stateFromStores(4314)().diff(expiresAt));
  const rect = { top: true, bottom: true, style: tmp.container, children: null };
  const obj2 = { style: tmp.content, children: null };
  const humanizeResult = tmp2Result2.duration(stateFromStores(4314)().diff(expiresAt)).humanize();
  ...
  if (null != id) {
    ...
    items3[1] = tmp14(tmp5(4718).Text, obj8);
    ...
    tmp14Result = tmp14(tmp5(4718).Text, obj10);
  }
  ...
  if (null != id) {
    ...
    obj13.children = tmp14(tmp5(5144).Button, obj14);
    ...
    items5[1] = tmp14(tmp5(5144).Button, obj15);
    ...
    tmp12Result2 = tmp14(tmp5(5144).Button, obj16);
  }
  obj11.children = tmp12Result2;
  items4[1] = closure_6(closure_4, obj11);
  rect.children = items4;
  return closure_8(str(7319).SafeAreaPaddingView, rect);
};
