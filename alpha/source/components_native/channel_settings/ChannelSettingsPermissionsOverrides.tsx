...
// Dependencies: [32, 5, 19, 17, 1976, 1958, 2016, 1980, 4361, 4371, 1372, 1074, 21, 4722, 576, 1484, 1612, 504, 11851, 11850, 4366, 1086, 9985, 4735, 1895, 4566, 5066, 1115, 4417, 2025, 1966, 17210, 8067, 4718, 4875, 11193, 5775, 1177, 5857, 17214, 1364, 17215, 2]
// Exports: default

// Module 17209 (ChannelSettingsPermissionsOverrides)
import nativeDefault from "native" /* 576 */;
import BigFlagUtilsAll from "BigFlagUtils" /* 1086 */;
import StageChannelPermissions from "StageChannelPermissions" /* 1966 */;
import PermissionUtils from "PermissionUtils" /* 4366 */;
import TableRowGroup from "TableRowGroup" /* 5857 */;
import AppChannelPermissionUtils from "AppChannelPermissionUtils" /* 11850 */;
import PermissionSpecUtilsDefault from "PermissionSpecUtils" /* 17210 */;
...
export default function ChannelSettingsPermissionsOverrides(fromCreate) {
    ...
    if (fromCreate) {
      let obj = {
        headerRight() {
            ...
            return closure_2_19(closure_0(8067).HeaderTextButton, obj);
          }
      };
  ...
  if (fromCreate.type === constants.MEMBER) {
    ...
    let tmp12Result = tmp12(tmp5(11193), obj6);
  } else {
    ...
    tmp12Result = tmp12(tmp2(5775).TableRow, obj7);
  }
  ...
  items6[2] = first.map((permissions, index) => {
    ...
    obj.children = closure_2_19(TableRowGroup.TableRowGroup, {
      ...
      children: permissions.map((description, index) => {
        ...
        const obj = { variant: "text-xs/medium", color: "text-subtle", children: closure_1_0(17214).renderDescription(description.description) };
        const items = [closure_1_19(closure_1_0(4718).Text, obj), ];
        let tmp5Result = null;
        if (false !== tmp) {
          tmp5Result = null;
          if ("" !== tmp) {
            const obj3 = { variant: "text-xs/medium", color: "text-feedback-critical", children: tmp };
            tmp5Result = tmp5(tmp6(4718).Text, obj3);
          }
        }
        items[1] = tmp5Result;
        const obj2 = closure_1_0(17214);
        ...
        obj4.trailing = closure_1_19(id(17215), obj5);
        return closure_1_19(closure_1_0(5775).TableRow, obj4, "row-" + index);
      })
    });
