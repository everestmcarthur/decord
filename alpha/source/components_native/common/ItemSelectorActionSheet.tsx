...
// Dependencies: [19, 21, 4423, 576, 1612, 7346, 7345, 7394, 6820, 5855, 5858, 2]
// Exports: default

// Module 9664 (ItemSelectorActionSheet)
...
export default function ItemSelectorActionSheet(arg0) {
  ...
  ({ body, hasIcons } = arg0);
  const token = items(4423).useToken(nativeDefault.modules.mobile.TABLE_ROW_PADDING);
  ...
  if (null != onClose) {
    ...
    tmp6Result = tmp6(tmp(7394).ActionSheetCloseButton, obj3);
  }
  const obj4 = { scrollable: true, header: closure_3(items(7345).BottomSheetTitleHeader, obj2), children: null };
  obj2.trailing = tmp6Result;
  const obj5 = { contentContainerStyle: null, children: null };
  const obj = items(4423);
  ...
  items1[1] = closure_3(items(5855).TableRadioGroup, {
    value: num,
    ...
    children: items.map((label, value) => closure_1_3(items(5858).TableRadioRow, { label: label.label, value }, value))
  });
  obj5.children = items1;
  obj4.children = tmp8(items(6820).BottomSheetScrollView, obj5);
  return closure_3(items(7346).BottomSheet, obj4);
};
