// Module ID: 6961
// Function ID: 6962
// Dependencies: [32, 19, 6959, 6949, 6962, 6935, 6950, 6897]
...
import hash from "hash" /* 6949 */;
import allowedNativeProps2 from "allowedNativeProps" /* 6950 */;
  ...
  while (tmp12 !== undefined) {
    ...
    if (!allowedNativeProps.has(first)) {
      if (!EMPTY_WHITE_LIST.has(tmp17)) {
        let PropsToFilter = tmp19(6950).PropsToFilter;
        if (PropsToFilter.has(tmp17)) {
          continue;
        } else {
          let _console = console;
          let tmp19Result = tmp19(6897);
    ...
    let Reanimated = tmp19(6935).Reanimated;
