// Module ID: 6948
// Function ID: 6949
// Dependencies: [32, 19, 6949, 6960, 6951, 6964, 6935]
...
function isSupportedGesture(gestures) {
  if (obj.isComposedGesture(gestures)) {
    ...
    if (tmp(6951).SingleGestureName.Tap !== type) {
      if (tmp(6951).SingleGestureName.LongPress !== type) {
        if (tmp(6951).SingleGestureName.Fling !== type) {
          if (tmp(6951).SingleGestureName.Native !== type) {
            if (tmp(6951).SingleGestureName.Hover !== type) {
              return false;
            }
          }
        }
...
let closure_8 = fn(6949).SHARED_VALUE_OFFSET + 0.5;
...
export const useJSResponderHandler = function useJSResponderHandler(gesture) {
  _require = gesture;
  let tmp = closure_3(require("module_6964").JSResponderContext);
  dependencyMap = tmp;
  ...
  const items = [gesture];
  closure_5(() => {
    const Reanimated = gesture(6935).Reanimated;
    const enabledSharedValues = gesture(6949).getEnabledSharedValues(Reanimated);
  ...
  let handleStartShouldSetResponder = closure_4(() => {
    if (closure_4()) {
      const result = _mod6964.updateResponderEventValue(closure_1, true);
    }
    return false;
  }, items2);
  if (null == tmp) {
