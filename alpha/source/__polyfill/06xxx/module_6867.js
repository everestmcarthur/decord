// Module ID: 6867
// Function ID: 6868
// Dependencies: [32, 19, 17, 21, 1637, 6868, 6869, 6892, 6865, 6881, 38, 6875, 7045, 7047, 7050, 7052, 7056, 7059, 7063]

// Module 6867
import _modDef38 from "module_38" /* 38 */;
import cancelAnimation2 from "cancelAnimation" /* 1637 */;
import value22 from "value2" /* 6865 */;
import DEFAULT_HANDLE_HEIGHT from "DEFAULT_HANDLE_HEIGHT" /* 6868 */;
import normalizeSnapPoint from "normalizeSnapPoint" /* 6881 */;
  ...
  constructor(arg0, arg1) {
      ...
      if (initialPosition !== sharedValue2.value) {
        if (undefined !== initialPosition) {
          if (sharedValue13.value !== value22.ANIMATION_STATE.RUNNING) {
            if (iter2.value === tmp18(6865).ANIMATION_STATE.RUNNING) {
              workletCallback();
            }
            iter2.value = tmp18(6865).ANIMATION_STATE.RUNNING;
            sharedValue14.value = GESTURE;
            sharedValue3.value = initialPosition;
            let tmp8 = state.value === tmp18(6865).KEYBOARD_STATE.SHOWN;
            if (tmp8) {
              tmp8 = keyboardBehavior !== tmp18(6865).KEYBOARD_BEHAVIOR.extend;
            }
            if (tmp8) {
            ...
            iter.value = tmp18(6881).animate(obj);
            const tmp18Result2 = tmp18(6881);
          }
        }
      }
    };
    ...
    getEvaluatedPosition = function getEvaluatedPosition(arg0) {
      ...
      if (arg0 === value22.ANIMATION_SOURCE.KEYBOARD) {
        if (DEFAULT_KEYBOARD_BLUR_BEHAVIOR === tmp(6865).KEYBOARD_BLUR_BEHAVIOR.restore) {
          if (value3 === tmp(6865).KEYBOARD_STATE.HIDDEN) {
            if (sharedValue10.value !== tmp(6892).State.ACTIVE) {
              if (sharedValue11.value !== tmp(6892).State.ACTIVE) {
      ...
      if (keyboardBehavior === value22.KEYBOARD_BEHAVIOR.extend) {
        if (value3 === tmp(6865).KEYBOARD_STATE.SHOWN) {
          return value4;
        }
      }
    ...
    handleSnapToPosition = function handleSnapToPosition(arg0, arg1) {
      ...
      if (tmp4) {
        ...
        tmp(1637).runOnUI(workletCallback2)(normalizeSnapPointResult, tmp(6865).ANIMATION_SOURCE.USER, 0, arg1);
