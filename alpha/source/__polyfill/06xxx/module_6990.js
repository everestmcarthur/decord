// Module ID: 6990
// Function ID: 6991
// Dependencies: [17, 6821, 6828, 1637, 6991, 6992]
...
export const useGestureEventsHandlersDefault = () => {
  ...
  function handleOnChange(arg0, translationY) {
    ...
    if (sharedValue.value.initialPosition !== initialPosition) {
      ...
      animatedPosition.value = _mod6991.clamp(sum1, initialPosition, value3);
      const clampResult = _mod6991.clamp(sum1, initialPosition, value3);
    }
  ...
  function handleOnEnd(arg0, translationY) {
    ...
    if (isInTemporaryPosition.value) {
      if (sharedValue.value.initialPosition >= iter.value) {
        if (iter3.value.initialPosition > iter.value) {
          animateToPosition(iter3.value.initialPosition, tmp2(6821).ANIMATION_SOURCE.GESTURE, velocityY / 2);
        }
      }
    }
    if (animatedScrollableType.value !== value22.SCROLLABLE_TYPE.UNDETERMINED) {
      value = iter4.value;
      const VIEW = tmp2(6821).SCROLLABLE_TYPE.VIEW;
    }
    if (tmp5) {
    ...
    const snapPointResult = _mod6992.snapPoint(translationY.translationY + sharedValue.value.initialPosition, velocityY, substr);
    if (snapPointResult !== animatedPosition.value) {
      let tmp12 = arg0 === tmp2(6821).GESTURE_SOURCE.CONTENT;
      if (tmp12) {
      ...
      if (!tmp12) {
        animateToPosition(snapPointResult, tmp2(6821).ANIMATION_SOURCE.GESTURE, velocityY / 2);
      }
    }
  }
