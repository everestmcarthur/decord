// Module ID: 8158
// Function ID: 8159
// Dependencies: [32, 19, 17, 21, 1485, 1615, 5843, 8159, 5116, 8160, 8162, 8163, 8164, 8165]
...
function SceneView(arg0) {
  ...
  const context = noop.useContext(num6(5843).HeaderShownContext);
  let num5 = noop.useContext(num6(5843).HeaderHeightContext);
  const context1 = noop.useContext(num6(5843).HeaderBackContext);
  const obj2 = num6(1615);
  const frameSize = num6(5843).useFrameSize((width) => width.width > width.height);
  num6 = 0;
  if (!context) {
    let top;
  ...
  const obj4 = num6(5843);
  const frameSize1 = num6(5843).useFrameSize((arg0) => 56 + num6);
  const tmp8Result = num6(5843);
  let num7 = 2;
  const tmp8Result5 = num6(1485);
  [tmp17, tmp18] = noop.useState(frameSize1);
  dependencyMap = tmp18;
  const tmp16 = _slicedToArray(noop.useState(frameSize1), 2);
  const callback = obj3.useCallback(num6(8159).debounce(tmp18, 100), []);
  let tmp21 = "usesNewAndroidHeaderHeightImplementation" in tmp8(5116).compatibilityFlags;
  if (tmp21) {
    tmp21 = true === tmp8(5116).compatibilityFlags.usesNewAndroidHeaderHeightImplementation;
  }
  _slicedToArray = 0;
  let num8 = 0;
  if (null == header) {
  ...
  if (previousDescriptor) {
    title = tmp8(5843).getHeaderTitle(previousDescriptor.options, previousDescriptor.route.name);
    const tmp8Result7 = tmp8(5843);
  ...
  let eventResult;
  const headerConfigProps = num6(8160).useHeaderConfigProps(obj5);
  if (null == header) {
  ...
  const tmp8Result8 = num6(8160);
  obj20.children = closure_10(num6(5843).HeaderBackContext.Provider, { value: memo1, children: descriptor.render() });
  items5[2] = closure_10(num6(5843).HeaderShownContext.Provider, obj20);
  obj13.children = items5;
  obj12.children = tmp38(num6(5843).HeaderHeightContext.Provider, obj13);
  obj10.children = closure_10(num6(8162).AnimatedHeaderHeightContext.Provider, obj12);
  obj9.children = closure_10(num6(5116).ScreenStackItem, obj10);
...
export const NativeStackView = function NativeStackView(state) {
  ...
  const setNextDismissedKey = require("module_8163").useDismissedRouteError(state).setNextDismissedKey;
  let obj = require("module_8163");
  const invalidPreventRemoveError = require("module_8164").useInvalidPreventRemoveError(descriptors);
  const obj2 = require("module_8164");
  const modalRouteKeys = require("module_8165").getModalRouteKeys(state.routes, descriptors);
  ...
  return closure_10(require("module_5843").SafeAreaProviderCompat, obj4);
};
