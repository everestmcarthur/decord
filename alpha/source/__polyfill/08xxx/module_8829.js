// Module ID: 8829
// Function ID: 8830
// Dependencies: [109, 41, 42, 93, 95, 98, 19, 21, 8735, 8743, 8749, 8742, 8830, 8826, 8751]
...
class TextPath {
  constructor() {
    ...
    tmp3Result.setNativeProps = (matrix) => {
      ...
      const merged = Object.assign(matrix, TextPath(8743).pickNotNil(extractFontDefault(matrix, true)));
      if (closure_0.root) {
...
_inherits(TextPath, _modDef8751);
const entry = {
  key: "render",
  value: function render() {
    ...
    let match = href;
    if (href) {
      match = href.match(TextPath(8743).idPattern);
    }
    let tmp5 = match;
    if (match) {
      tmp5 = match[1];
    ...
    if (tmp5) {
      const withoutXYResult = TextPath(8742).withoutXY(self, tmp);
      ...
      return jsx(_modDef8826, { ref: self.refMethod, children });
    }
