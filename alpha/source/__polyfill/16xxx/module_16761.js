// Module ID: 16761
// Function ID: 16762
// Dependencies: [41, 42, 93, 95, 98, 19, 17, 21, 16762]
...
function captureRef(current, options) {
  if (ViewShot(16762)) {
    ...
    return tmp(16762).captureRef(tmp15, options);
  } else {
    const _Error = Error;
...
function releaseCapture(str) {
  if (typeof str === "string") {
    ViewShot(16762).releaseCapture(str);
    const obj = ViewShot(16762);
  }
}
...
export const captureScreen = function captureScreen(options) {
  if (ViewShot(16762)) {
    ...
    return tmp(16762).captureScreen(options);
  } else {
    const _Error = Error;
    const error = new Error("react-native-view-shot: NativeModules.RNViewShot is undefined. Make sure the library is linked on the native side.");
    throw error;
