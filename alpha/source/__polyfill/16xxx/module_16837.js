// Module ID: 16837
// Function ID: 16838
// Dependencies: [41, 42, 93, 95, 98, 19, 17, 21, 16838]
// Exports: captureScreen, ensureModuleIsLoaded

// Module 16837
import _modDef16838 from "module_16838" /* 16838 */;
...
function captureRef(current, options) {
  if (ViewShot(16838)) {
    ...
    return tmp(16838).captureRef(tmp15, options);
  } else {
    const _Error = Error;
...
function releaseCapture(str) {
  if (typeof str === "string") {
    ViewShot(16838).releaseCapture(str);
    const obj = ViewShot(16838);
  }
}
...
export const captureScreen = function captureScreen(options) {
  if (ViewShot(16838)) {
    ...
    return tmp(16838).captureScreen(options);
  } else {
    const _Error = Error;
    const error = new Error("react-native-view-shot: NativeModules.RNViewShot is undefined. Make sure the library is linked on the native side.");
    throw error;
