// Module ID: 5414
// Function ID: 5415
// Dependencies: [5415, 5416]

// Module 5414
import _modDef5415 from "module_5415" /* 5415 */;
    ...
    if (tmp < byteLength.byteLength) {
      while (true) {
        ...
        if (byteLength.getUint8(tmp) !== 28) {
          ...
          obj3.name = combined;
          obj3.value = items;
          obj3.description = getTagDescription(tmp8(5415).iptc[uint16], items, obj, encoding);
          let tmp17 = tmp8(5415).iptc[uint16] && tmp8(5415).iptc[uint16].repeatable;
          if (tmp17) {
            obj3.repeatable = true;
          }
          let tmp18 = tmp8(5415).iptc[uint16] && undefined !== tmp8(5415).iptc[uint16].encoding_name;
          if (tmp18) {
            let obj5 = tmp8(5415).iptc[uint16];
