...
export default {
  read(buffer, sum) {
    const shortAt = _modDef5442.getShortAt(buffer, sum);
    let tmp6;
    if (15 <= shortAt) {
      const byteAt = tmp3(5442).getByteAt(buffer, sum + 14);
    ...
    let tmp9;
    if (16 <= shortAt) {
      const byteAt1 = tmp3(5442).getByteAt(buffer, sum + 15);
      const obj3 = { value: byteAt1, description: null };
      const _HermesInternal2 = HermesInternal;
      obj3.description = "" + byteAt1 + "px";
      tmp9 = obj3;
      const tmp3Result7 = tmp3(5442);
    }
    let tmp12;
    if (9 <= shortAt) {
      const byteAt2 = tmp3(5442).getByteAt(buffer, sum + 7);
      const tmp3Result8 = tmp3(5442);
      const byteAt3 = tmp3(5442).getByteAt(buffer, sum + 7 + 1);
      const obj4 = { value: 256 * byteAt2 + byteAt3, description: `${tmp13}.${tmp14}` };
      tmp12 = obj4;
      const tmp3Result9 = tmp3(5442);
    }
    ...
    let tmp15;
    if (10 <= shortAt) {
      const byteAt4 = tmp3(5442).getByteAt(buffer, sum + 9);
      ...
      tmp15 = obj6;
      const tmp3Result10 = tmp3(5442);
    }
    obj5["Resolution Unit"] = tmp15;
    let tmp17;
    if (12 <= shortAt) {
      const shortAt1 = tmp3(5442).getShortAt(buffer, sum + 10);
      const obj7 = { value: shortAt1, description: "" + shortAt1 };
      tmp17 = obj7;
      const tmp3Result11 = tmp3(5442);
    }
    obj5.XResolution = tmp17;
    let tmp19;
    if (14 <= shortAt) {
      const shortAt2 = tmp3(5442).getShortAt(buffer, sum + 12);
      const obj8 = { value: shortAt2, description: "" + shortAt2 };
      tmp19 = obj8;
      const tmp3Result12 = tmp3(5442);
    }
