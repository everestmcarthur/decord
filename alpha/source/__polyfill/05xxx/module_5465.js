...
obj2[2999] = {
  ...
  description(getUint8) {
    return _slicedToArray(_mod5425.getPascalStringFromDataView(getUint8, 0), 2)[1];
  }
};
let closure_4 = {
  [obj.CLOSED_SUBPATH_LENGTH]: {
    ...
    path(dataView, sum) {
      const items = [_modDef5442.getShortAt(dataView, sum)];
      return items;
    }
  },
  ...
  [obj.CLIPBOARD]: {
    ...
    path: function parseClipboard(dataView, sum) {
      const longAt = _modDef5442.getLongAt(dataView, sum);
      let num = -1;
      let num2 = -1;
      if (longAt >>> 31 === 0) {
        num2 = 1;
      }
      const str2 = longAt & parseInt(_mod5425.strRepeat("1", 24), 2);
      const text = `${str.toString(2)}.`;
      const obj3 = _mod5425;
      const items = [num2 * obj3.parseFloatRadix(`${(2130706432 & longAt) >>> 24.toString(2)}.` + _mod5425.padStart(str2.toString(2), 24, "0"), 2), , , ];
      sum = sum + 4;
      const longAt1 = _modDef5442.getLongAt(dataView, sum);
      let num3 = num;
      if (longAt1 >>> 31 === 0) {
        num3 = 1;
      }
      const tmpResult = _modDef5442;
      const tmp4Result = _mod5425;
      const str4 = longAt1 & parseInt(_mod5425.strRepeat("1", 24), 2);
      const text1 = `${str3.toString(2)}.`;
      const tmp4Result12 = _mod5425;
      items[1] = num3 * tmp4Result12.parseFloatRadix(`${(2130706432 & longAt1) >>> 24.toString(2)}.` + _mod5425.padStart(str4.toString(2), 24, "0"), 2);
      const sum1 = sum + 8;
      const tmp4Result13 = _mod5425;
      const longAt2 = _modDef5442.getLongAt(dataView, sum1);
      let num4 = num;
      if (longAt2 >>> 31 === 0) {
        num4 = 1;
      }
      const tmpResult4 = _modDef5442;
      const tmp4Result14 = _mod5425;
      const str6 = longAt2 & parseInt(_mod5425.strRepeat("1", 24), 2);
      const text2 = `${str5.toString(2)}.`;
      const tmp4Result15 = _mod5425;
      items[2] = num4 * tmp4Result15.parseFloatRadix(`${(2130706432 & longAt2) >>> 24.toString(2)}.` + _mod5425.padStart(str6.toString(2), 24, "0"), 2);
      const sum2 = sum + 12;
      const tmp4Result16 = _mod5425;
      const longAt3 = _modDef5442.getLongAt(dataView, sum2);
      let num5 = num;
      if (longAt3 >>> 31 === 0) {
        num5 = 1;
      }
      const tmpResult5 = _modDef5442;
      const tmp4Result17 = _mod5425;
      const str8 = longAt3 & parseInt(_mod5425.strRepeat("1", 24), 2);
      const text3 = `${str7.toString(2)}.`;
      const tmp4Result18 = _mod5425;
      items[3] = num5 * tmp4Result18.parseFloatRadix(`${(2130706432 & longAt3) >>> 24.toString(2)}.` + _mod5425.padStart(str8.toString(2), 24, "0"), 2);
      const items1 = [items, ];
      const sum3 = sum + 16;
      const tmp4Result19 = _mod5425;
      const longAt4 = _modDef5442.getLongAt(dataView, sum3);
      if (longAt4 >>> 31 === 0) {
        num = 1;
      }
      const tmpResult6 = _modDef5442;
      const tmp4Result20 = _mod5425;
      const str10 = longAt4 & parseInt(_mod5425.strRepeat("1", 24), 2);
      const text4 = `${str9.toString(2)}.`;
      const tmp4Result21 = _mod5425;
      items1[1] = num * tmp4Result21.parseFloatRadix(`${(2130706432 & longAt4) >>> 24.toString(2)}.` + _mod5425.padStart(str10.toString(2), 24, "0"), 2);
      return items1;
    }
  }
};
