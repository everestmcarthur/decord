...
export default {
  read(dataView, sum) {
    ...
    let tmp4;
    if (8 <= shortAt) {
      const byteAt = tmp(5442).getByteAt(dataView, sum + 7);
    ...
    let tmp6;
    if (3 <= shortAt) {
      const byteAt1 = tmp(5442).getByteAt(dataView, sum + 2);
    ...
    let tmp8;
    if (5 <= shortAt) {
      const shortAt1 = tmp(5442).getShortAt(dataView, sum + 3);
    ...
    let tmp11;
    if (7 <= shortAt) {
      const shortAt2 = tmp(5442).getShortAt(dataView, sum + 5);
