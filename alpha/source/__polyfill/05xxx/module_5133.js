// Module ID: 5133
// Function ID: 5134
// Dependencies: [109, 19, 17, 21, 5134, 5135, 5136, 5132, 5137, 5138, 5140, 5123, 5141]

// Module 5133
import get_synchronousScreenUpdatesEnabledDefault from "get synchronousScreenUpdatesEnabled" /* 5123 */;
import _modDef5134 from "module_5134" /* 5134 */;
import animatedComponentDefault from "animatedComponent" /* 5135 */;
import _mod5136 from "module_5136" /* 5136 */;
import DelayedFreezeDefault from "DelayedFreeze" /* 5138 */;
...
const jsx = fn(21).jsx;
const __INTERNAL_VIEW_CONFIG = Animated.createAnimatedComponent(_modDef5134);
...
const forwardRefResult = noop.forwardRef(function InnerScreen(activityState, ref) {
  ...
  const previous = _mod5136.usePrevious(activityState.activityState);
  ...
  if (undefined === enabled) {
    enabled = tmp2(5132).screensEnabled();
    const tmp2Result = tmp2(5132);
  }
  let freezeOnBlur = activityState.freezeOnBlur;
  if (undefined === freezeOnBlur) {
    freezeOnBlur = tmp2(5132).freezeEnabled();
    const tmp2Result6 = tmp2(5132);
  }
  ...
  if (undefined === SHEET_DIMMED_ALWAYS) {
    SHEET_DIMMED_ALWAYS = tmp2(5137).SHEET_DIMMED_ALWAYS;
  }
  ...
  if (enabled) {
    if (tmp2(5132).isNativePlatformSupported) {
      const sheetAllowedDetents1 = tmp2(5137).resolveSheetAllowedDetents(sheetAllowedDetents);
      const tmp2Result7 = tmp2(5137);
      const sheetLargestUndimmedDetent = tmp2(5137).resolveSheetLargestUndimmedDetent(SHEET_DIMMED_ALWAYS, sheetAllowedDetents1.length - 1);
      const tmp2Result8 = tmp2(5137);
      ...
      const sheetInitialDetentIndex1 = tmp2(5137).resolveSheetInitialDetentIndex(num3, sheetAllowedDetents1.length - 1);
      ...
      obj4.fullScreenSwipeEnabled = tmp2(5140).parseBooleanToOptionalBooleanNativeProp(fullScreenSwipeEnabled);
      let num7;
      ...
      if (isNativeStack) {
        ...
        tmp31Result = tmp31(tmp32(5141).Provider, obj7);
      }
