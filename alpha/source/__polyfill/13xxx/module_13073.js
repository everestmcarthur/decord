// Module ID: 13073
// Function ID: 13074
// Dependencies: [32, 13066, 13072, 13061, 13074, 13044, 13056, 13057, 13058, 13051, 13049, 13067, 13039, 13071, 13075, 13077, 13068, 13078, 13080, 13054]
  ...
  if (obj.hasTracingEnabled()) {
    ...
    if (parentSpan) {
      if (!forceTransaction.forceTransaction) {
        ...
        if (!tmp9) {
          spanIsSampledResult = tmp(13044).spanIsSampled(parentSpan);
        ...
        if (spanIsSampledResult) {
          ...
          let sentrySpan = new tmp(13080).SentrySpan(obj2);
        } else {
          const obj3 = { traceId };
          sentrySpan = new tmp(13072).SentryNonRecordingSpan(obj3);
        }
        const spanContextResult = parentSpan.spanContext();
        tmp(13044).addChildSpanToSpan(parentSpan, sentrySpan);
        const tmpResult12 = tmp(13044);
        ...
        const tmpResult13 = tmp(13066);
        tmp(13044).addChildSpanToSpan(parentSpan, sentrySpan);
        const tmpResult14 = tmp(13044);
      }
      tmp(13077).logSpanStart(sentrySpan);
      const tmpResult15 = tmp(13077);
      const result = tmp(13068).setCapturedScopesOnSpan(sentrySpan, scope, isolationScope);
      return sentrySpan;
    }
    if (parentSpan) {
      const dynamicSamplingContextFromSpan = tmp(13075).getDynamicSamplingContextFromSpan(parentSpan);
      const tmpResult17 = tmp(13075);
      ...
      const tmpResult18 = tmp(13044);
      const merged1 = Object.assign(spanArguments);
      const tmp42 = _startRootSpan(obj4, scope, tmp(13044).spanIsSampled(parentSpan));
      const spanIsSampledResult1 = tmp(13044).spanIsSampled(parentSpan);
      tmp(13075).freezeDscOnSpan(tmp42, dynamicSamplingContextFromSpan);
      sentrySpan = tmp42;
      const tmpResult19 = tmp(13075);
    } else {
      const obj5 = {};
      ...
      sentrySpan = tmp33;
      if (dsc) {
        tmp(13075).freezeDscOnSpan(tmp33, dsc);
        sentrySpan = tmp33;
        const tmpResult20 = tmp(13075);
      }
    }
    const tmpResult = tmp(13066);
  } else {
    const sentryNonRecordingSpan = new tmp(13072).SentryNonRecordingSpan();
...
function _startRootSpan(name, arg1, parentSampled) {
  ...
  if (arg1.getScopeData().sdkProcessingMetadata[__SENTRY_SUPPRESS_TRACING__]) {
    ...
    sampleSpanResult = tmp(13078).sampleSpan(options, obj2);
    const tmpResult = tmp(13078);
  }
  ...
  obj4.sampled = tmp6;
  const sentrySpan = new tmp(13080).SentrySpan(obj4);
  if (undefined !== tmp7) {
    const attr = sentrySpan.setAttribute(tmp(13054).SEMANTIC_ATTRIBUTE_SENTRY_SAMPLE_RATE, tmp7);
  }
  if (client) {
    client.emit("spanStart", sentrySpan);
  }
...
export const continueTrace = (arg0, arg1) => {
  _require = arg1;
  const mainCarrier = require("module_13057").getMainCarrier();
  const obj = require("module_13057");
  const tmp = _require;
  const asyncContextStrategy = require("module_13058").getAsyncContextStrategy(mainCarrier);
  if (asyncContextStrategy.continueTrace) {
    ...
    return tmp(13066).withScope((setPropagationContext) => {
      const result = setPropagationContext.setPropagationContext(_mod13051.propagationContextFromHeaders(dependencyMap, _slicedToArray));
      return closure_0();
    });
  }
  const obj2 = require("module_13058");
};
export const startInactiveSpan = function startInactiveSpan(experimental) {
  _require = experimental;
  let mainCarrier = require("module_13057").getMainCarrier();
  let obj = require("module_13057");
  const tmp3 = _require;
  const tmp4 = obj4;
  let asyncContextStrategy = require("module_13058").getAsyncContextStrategy(mainCarrier);
  if (asyncContextStrategy.startInactiveSpan) {
    ...
    return fn(() => {
      ...
      if (experimental.onlyIfParent) {
        if (!tmp5) {
          let sentryNonRecordingSpan = new tmp(13072).SentryNonRecordingSpan();
        }
        return sentryNonRecordingSpan;
      }
...
export const startNewTrace = function startNewTrace(arg0) {
  _require = arg0;
  return require("module_13066").withScope((setPropagationContext) => {
  ...
  if (asyncContextStrategy.startSpan) {
    ...
    return tmp3(13066).withScope(experimental.scope, () => {
      ...
      return undefined !== createChildOrRootSpan ? ((arg0) => {
        ...
        const obj = sentryNonRecordingSpan(13066);
        ...
        if (sentryNonRecordingSpan.onlyIfParent) {
          ...
          return tmp(13074).handleCallbackErrors(() => dependencyMap(sentryNonRecordingSpan), () => {
    ...
    return tmp3(13066).withScope(experimental.scope, () => {
      ...
      return undefined !== createChildOrRootSpan ? ((arg0) => {
        ...
        if (sentryNonRecordingSpan.onlyIfParent) {
          ...
          tmp(13061)._setSpanForScope(currentScope, sentryNonRecordingSpan);
          const tmpResult5 = tmp(13061);
          return tmp(13074).handleCallbackErrors(() => dependencyMap(sentryNonRecordingSpan, finishAndSetSpan), () => {
            const status = sentryNonRecordingSpan(dependencyMap[5]).spanToJSON(sentryNonRecordingSpan).status;
            const isRecordingResult = sentryNonRecordingSpan.isRecording();
            let tmp4 = !isRecordingResult;
            if (isRecordingResult) {
