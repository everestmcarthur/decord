// Module ID: 13097
// Function ID: 13098
// Dependencies: [13059, 13062, 13075, 13098, 13099, 13085, 13057, 13100, 13093, 13069]
...
function applyClientOptions(environment, environment2) {
  ...
  if (environment.message) {
    environment.message = _mod13057.truncate(environment.message, num);
  }
  value = iter;
  ...
  if (value) {
    iter.value = _mod13057.truncate(iter.value, num);
  }
  ...
  if (url) {
    request.url = _mod13057.truncate(request.url, num);
  }
}
function applyDebugIds(exception, arg1) {
  const filenameToDebugIdMap = _mod13100.getFilenameToDebugIdMap(arg1);
  try {
