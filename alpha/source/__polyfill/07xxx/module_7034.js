// Module ID: 7034
// Function ID: 7035
// Dependencies: [17, 6865, 6872, 1637, 7035, 7036]
...
export const useGestureEventsHandlersDefault = () => {
  ...
  function handleOnChange(arg0, translationY) {
    ...
    if (sharedValue.value.initialPosition !== initialPosition) {
      ...
      animatedPosition.value = _mod7035.clamp(sum1, initialPosition, value3);
      const clampResult = _mod7035.clamp(sum1, initialPosition, value3);
    }
  ...
  function handleOnEnd(arg0, translationY) {
    ...
    if (isInTemporaryPosition.value) {
      if (sharedValue.value.initialPosition >= iter.value) {
        if (iter3.value.initialPosition > iter.value) {
          animateToPosition(iter3.value.initialPosition, tmp2(6865).ANIMATION_SOURCE.GESTURE, velocityY / 2);
        }
      }
    }
    if (animatedScrollableType.value !== value22.SCROLLABLE_TYPE.UNDETERMINED) {
      value = iter4.value;
      const VIEW = tmp2(6865).SCROLLABLE_TYPE.VIEW;
    }
    if (tmp5) {
      tmp2(1637).runOnJS(dismiss)();
      const tmp2Result = tmp2(1637);
    }
    if (isInTemporaryPosition.value) {
      iter2.value = false;
    }
    ...
    const snapPointResult = _mod7036.snapPoint(translationY.translationY + sharedValue.value.initialPosition, velocityY, substr);
    if (snapPointResult !== animatedPosition.value) {
      let tmp12 = arg0 === tmp2(6865).GESTURE_SOURCE.CONTENT;
      if (tmp12) {
      ...
      if (!tmp12) {
        animateToPosition(snapPointResult, tmp2(6865).ANIMATION_SOURCE.GESTURE, velocityY / 2);
      }
    }
  }
