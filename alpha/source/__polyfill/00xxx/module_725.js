  ...
  },
  {
    ...
    value: function getSpanJSON() {
      ...
      if (_isStandaloneSpan) {
        _isStandaloneSpan = tmp(684).getRootSpan(self) === self;
        const tmpResult = tmp(684);
      }
      obj.is_segment = _isStandaloneSpan;
      let spanId;
      if (self._isStandaloneSpan) {
        const rootSpan = tmp(684).getRootSpan(self);
  ...
  },
  {
    ...
    value: function addEvent(name, num, arg2) {
      ...
      if (!isArray) {
        ...
        if (!arg2) {
          timestampInSecondsResult = tmp(703).timestampInSeconds();
    ...
    value: function _onSpanEnded() {
      ...
      if (self._isStandaloneSpan) {
        if (self._isStandaloneSpan) {
          if (self._sampled) {
            const items = [self];
            const spanEnvelope = tmp(728).createSpanEnvelope(items, client);
            const tmpResult = tmp(728);
            const client1 = tmp(713).getClient();
            if (client1) {
              if (spanEnvelope[1]) {
                if (0 !== arr2.length) {
          ...
          if (result) {
            let scope = tmp(685).getCapturedScopesOnSpan(self).scope;
            if (!scope) {
              scope = tmp(713).getCurrentScope();
              const tmpResult7 = tmp(713);
            }
            scope.captureEvent(result);
            const tmpResult6 = tmp(685);
          }
        }
      } else {
        const tmpResult8 = tmp(684);
      }
    }
  },
  {
    ...
    value: function _convertSpanToTransaction() {
      ...
      if (tmp6) {
        ...
        if (true === self._sampled) {
          ...
          const obj2 = { contexts: null, spans: null, start_timestamp: null, timestamp: null, transaction: null, type: "transaction", sdkProcessingMetadata: null, request: null };
          ...
          const obj5 = { capturedSpanScope: scope, capturedSpanIsolationScope: capturedScopesOnSpan.isolationScope, dynamicSamplingContext: null };
