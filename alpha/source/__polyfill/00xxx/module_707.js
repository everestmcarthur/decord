// Module ID: 707
// Function ID: 708
// Dependencies: [41, 42, 708, 692, 690, 712]
...
function withScope(arg0) {
  ...
  if (!stack) {
    const defaultCurrentScope = tmp(712).getDefaultCurrentScope();
    const tmpResult = tmp(712);
    stack = new _moduleResult(defaultCurrentScope, tmp(712).getDefaultIsolationScope());
    const tmpResult2 = tmp(712);
  }
...
function withIsolationScope(arg0) {
  ...
  if (!stack) {
    const defaultCurrentScope = tmp(712).getDefaultCurrentScope();
    const tmpResult = tmp(712);
    stack = new closure_3(defaultCurrentScope, tmp(712).getDefaultIsolationScope());
    const tmpResult2 = tmp(712);
  }
  sentryCarrier.stack = stack;
  return stack.withScope(() => {
    ...
    let stack = sentryCarrier.stack;
    if (!stack) {
      const defaultCurrentScope = tmp2(712).getDefaultCurrentScope();
      const tmp2Result = tmp2(712);
      stack = new closure_2_3(defaultCurrentScope, tmp2(712).getDefaultIsolationScope());
      const tmp2Result2 = tmp2(712);
    }
    sentryCarrier.stack = stack;
...
export function getStackAsyncContextStrategy() {
  return {
    ...
    withSetIsolationScope(arg0, arg1) {
      closure_0 = arg1;
      let mainCarrier = closure_0(690).getMainCarrier();
      let obj = closure_0(690);
      let sentryCarrier = closure_0(690).getSentryCarrier(mainCarrier);
      let stack = sentryCarrier.stack;
      if (!stack) {
        let defaultCurrentScope = tmp(712).getDefaultCurrentScope();
        const tmpResult = tmp(712);
        stack = new closure_3(defaultCurrentScope, tmp(712).getDefaultIsolationScope());
        const tmpResult2 = tmp(712);
      }
    ...
    getCurrentScope() {
      ...
      if (!stack) {
        const defaultCurrentScope = tmp(712).getDefaultCurrentScope();
        const tmpResult = tmp(712);
        stack = new closure_1_3(defaultCurrentScope, tmp(712).getDefaultIsolationScope());
        const tmpResult2 = tmp(712);
      }
      sentryCarrier.stack = stack;
