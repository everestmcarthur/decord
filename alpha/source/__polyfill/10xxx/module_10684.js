// Module ID: 10684
// Function ID: 10685
// Dependencies: [41, 42, 93, 95, 98, 10675, 10674, 10676, 10682]
...
const regExp = new RegExp("(" + repeatedTimeunitPattern.matchAnyPattern(_mod10674.MONTH_DICTIONARY) + ")(?:-|/|\\s*,?\\s*)(" + _mod10674.ORDINAL_NUMBER_PATTERN + ")(?!\\s*(?:am|pm))\\s*(?:(?:to|\\-)\\s*(" + _mod10674.ORDINAL_NUMBER_PATTERN + ")\\s*)?(?:(?:-|/|\\s*,\\s*|\\s+)(" + _mod10674.YEAR_PATTERN + "))?(?=\\W|$)(?!\\:\\d)", "i");
...
const items = [
  entry,
  {
    ...
    value: function innerExtract(createParsingComponents, index) {
      const tmp3 = ENMonthNameMiddleEndianParser(10674).MONTH_DICTIONARY[index[1].toLowerCase(index[1])];
      const result = ENMonthNameMiddleEndianParser(10674).parseOrdinalNumberPattern(index[2]);
      if (result > 31) {
        return null;
      } else {
        const self = this;
        ...
        if (index[4]) {
          addTagResult.assign("year", tmp(10674).parseYear(index[4]));
        } else {
          addTagResult.imply("year", tmp(10676).findYearClosestToRef(createParsingComponents.refDate, result, tmp3));
        }
        if (index[3]) {
          const result1 = tmp(10674).parseOrdinalNumberPattern(index[3]);
