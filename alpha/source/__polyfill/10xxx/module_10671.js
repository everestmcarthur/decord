// Module ID: 10671
// Function ID: 10672
// Dependencies: [41, 42, 10672, 10678]

// Module 10671
import ENDefaultConfiguration2 from "ENDefaultConfiguration" /* 10672 */;
...
const entry1 = {
  ...
  value: function executeParser(debug, pattern) {
    ...
    if (match) {
      ...
      if (extractResult instanceof ParsingContext(10678).ParsingResult) {
      ...
      } else if (!(extractResult instanceof tmp6(10678).ParsingComponents)) {
...
const entry2 = {
  ...
  value: function createParsingComponents(date) {
    ...
    if (!(date instanceof ParsingContext(10678).ParsingComponents)) {
      const self = this;
      parsingComponents = new ParsingContext(10678).ParsingComponents(this.reference, date);
    }
...
const items2 = [
  entry2,
  {
    ...
    value: function createParsingResult(sum, match, extractResult, date) {
      ...
      return new ParsingContext(10678).ParsingResult(self.reference, sum, substr, parsingComponents, parsingComponents1);
    }
  },
  {
    key: "debug",
