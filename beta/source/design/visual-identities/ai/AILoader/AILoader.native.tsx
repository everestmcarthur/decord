...
// Function ID: 14435
// Name: AILoader
// Dependencies: [19, 17, 14435, 21, 4788, 4524, 4789, 14436, 4498, 2]

// Module 14434 (AILoader)
import ReanimatedRexport from "ReanimatedRexport" /* 4524 */;
import timing from "timing" /* 4789 */;
import AIGlyphText from "AIGlyphText" /* 14436 */;
...
function Slot(index) {
  ...
  const glyph = tmp;
  const sharedValue = index(4524).useSharedValue(0);
  ...
  const animatedStyle = index(4524).useAnimatedStyle(T);
  let obj2 = index(4524);
  let obj3 = { trackStepAt, progress: sharedValue, size };
  obj4.children = jsx(size(4524).View, { style: animatedStyle, children: closure_7.map((children) => jsx(AIGlyphText.AIGlyphText, { size, color, allowFontScaling: false, style: glyph.glyph, children }, children)) });
  return <stagger style={tmp.slot}>{null}</stagger>;
}
const View = fn(17).View;
const AILoaderConstants = fn(14435);
...
export const AILoader = noop.memo((size) => {
  ...
  let num2;
  const reducedMotion = num2.useContext(num(4498).AccessibilityPreferencesContext).reducedMotion;
