...
// Dependencies: [19, 17, 21, 576, 4637, 4498, 5206, 4489, 5205, 4524, 5217, 5221, 2]
// Exports: BackgroundBlurFill, BackgroundBlurFillAnimated, BackgroundBlurFillWithPress

// Module 8907 (BackgroundBlurFill)
import nativeDefault from "native" /* 576 */;
import ReanimatedRexport from "ReanimatedRexport" /* 4524 */;
import VisualEffectView from "VisualEffectView" /* 5206 */;
import spring from "spring" /* 5217 */;
import springPresets from "springPresets" /* 5221 */;
import noop from "module_19" /* 19 */;

const VisualEffectViewAnimatedDefault = tmp3(5205);
const VisualEffectViewDefault = tmp3(5206);
require = fn;
...
export const BlurTheme = fn(5206).BlurTheme;
export const BlurStyle = fn(5206).BlurStyle;
export const BackgroundBlurFill = function BackgroundBlurFill(arg0) {
  ...
  obj = blurTheme(4498);
  obj2 = noop;
  const tmpResult = blurTheme(4489);
...
export const BackgroundBlurFillWithPress = function BackgroundBlurFillWithPress(style) {
  ...
  obj = pressed(4498);
  const normalizeBlurThemeResult = pressed(5206).normalizeBlurTheme(blurTheme);
  const tmp2Result = pressed(5206);
  const tmp6 = pressed(5206).isBlurThemeLight(blurTheme) ? closure_6 : closure_8;
  importDefault = tmp6;
  const tmp2Result8 = pressed(5206);
  const tmp7 = pressed(5206).isBlurThemeLight(blurTheme) ? closure_7 : closure_9;
  dependencyMap = tmp7;
  const tmp2Result9 = pressed(5206);
  token = pressed(4489).useToken(nativeDefault.colors.BACKGROUND_SCRIM, normalizeBlurThemeResult);
  const tmp2Result10 = pressed(4489);
  token1 = pressed(4489).useToken(nativeDefault.colors.BACKGROUND_SCRIM_LIGHTBOX, normalizeBlurThemeResult);
  const tmp2Result11 = pressed(4489);
  const isBlurDisabledResult = pressed(5206).isBlurDisabled(merged);
  c5 = isBlurDisabledResult;
  const tmp2Result12 = pressed(5206);
  const fn = function p() {
  ...
  const tmp2Result13 = pressed(4524);
  fn.__closure = { withSpring: pressed(5217).withSpring, interpolateColor: pressed(4524).interpolateColor, pressed, fallbackColor: token, fallbackColorPressed: token1, ON_PRESS_SPRING: pressed(5221).ON_PRESS_SPRING };
  fn.__workletHash = 10497618157620;
  fn.__initData = __initData;
  let animatedStyle = tmp2Result13.useAnimatedStyle(fn);
  let obj2 = { withSpring: pressed(5217).withSpring, interpolateColor: pressed(4524).interpolateColor, pressed, fallbackColor: token, fallbackColorPressed: token1, ON_PRESS_SPRING: pressed(5221).ON_PRESS_SPRING };
  class C {
    constructor() {
  ...
  const tmp2Result14 = pressed(4524);
  C.__closure = { shouldUseFallback: isBlurDisabledResult, withSpring: pressed(5217).withSpring, interpolateColor: pressed(4524).interpolateColor, pressed, restingTint: tmp6, pressedTint: tmp7, ON_PRESS_SPRING: pressed(5221).ON_PRESS_SPRING };
  ...
  let obj3 = { shouldUseFallback: isBlurDisabledResult, withSpring: pressed(5217).withSpring, interpolateColor: pressed(4524).interpolateColor, pressed, restingTint: tmp6, pressedTint: tmp7, ON_PRESS_SPRING: pressed(5221).ON_PRESS_SPRING };
  const tmp12 = c5;
