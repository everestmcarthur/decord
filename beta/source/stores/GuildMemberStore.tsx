...
// Dependencies: [2106, 2098, 502, 2041, 2099, 2063, 4414, 3, 4415, 2103, 1385, 11, 1965, 1966, 1389, 4416, 1370, 1977, 12, 4417, 504, 573, 2]
...
function trackCommunicationDisabled(guildId, tmp10Result) {
  if (null != tmp10Result.communicationDisabledUntil) {
    if (obj2.isMemberCommunicationDisabled(tmp10Result)) {
      ...
      if (result) {
        result = tmp10(4415).isMemberCommunicationDisabled(tmp10Result);
        tmp10Result = tmp10(4415);
      }
      if (result) {
...
function computeDerivedMemberState(unsafeMutableRoles, roles) {
  ...
  let tmp4;
  if (0 === roles.length) {
    return { colorString: null, colorStrings: null, colorRoleId: "id", hoistRoleId: "T", iconRoleId: "key", highestRoleId: "padding" };
  } else {
    ...
    if (null == guild) {
      ...
      if (null != tmp14[user.id].communicationDisabledUntil) {
        if (tmp33Result.isMemberCommunicationDisabled(tmp16)) {
          ...
          if (result) {
            result = tmp33(4415).isMemberCommunicationDisabled(tmp16);
            const tmp33Result2 = tmp33(4415);
          }
          if (result) {
      ...
      const item = members.forEach((user) => {
        ...
        if (null != dependencyMap[user.user.id].communicationDisabledUntil) {
          if (tmp5Result7.isMemberCommunicationDisabled(tmp9)) {
            ...
            if (result) {
              result = tmp5(4415).isMemberCommunicationDisabled(tmp9);
              const tmp5Result8 = tmp5(4415);
            }
            if (result) {
      ...
      if (null == guild) {
        ...
        if (null != tmp[guildMember.user.id].communicationDisabledUntil) {
          if (tmp18Result.isMemberCommunicationDisabled(tmp22)) {
            ...
            if (result) {
              result = tmp18(4415).isMemberCommunicationDisabled(tmp22);
              const tmp18Result2 = tmp18(4415);
            }
            if (result) {
