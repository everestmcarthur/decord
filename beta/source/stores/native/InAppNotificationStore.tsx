...
// Dependencies: [2040, 4804, 10376, 7392, 7811, 1220, 2041, 2063, 10377, 4803, 2095, 2035, 1372, 1074, 8307, 8305, 4380, 12, 10378, 10380, 7603, 7544, 10381, 9729, 5010, 10389, 10390, 10392, 10385, 10393, 10397, 4942, 2019, 1186, 8130, 10398, 4968, 11, 504, 573, 2]

// Module 10375 (InAppNotificationStore)
...
import RestrictedScheduleNotificationUtils from "RestrictedScheduleNotificationUtils" /* 10378 */;
import MessageUtils from "MessageUtils" /* 10380 */;
import NotificationTextUtils from "NotificationTextUtils" /* 10381 */;
...
const playInAppMessageSound = tmp(10398);
require = fn;
function handleAlertMessage() {
  if (obj.allowInAppNotifications()) {
    const result = tmp(10393).shouldShowRaidInAppNotification();
    ...
    if (result.show) {
      if (null != guildId) {
        ...
        if (null == guild) {
          ...
          channel = ChannelStore.getChannel(channel(10397)(guild));
          if (null == channel) {
            return false;
            ...
            if (merged.wasRecentlyDismissed(obj2)) {
              ...
              const obj4 = {
                type: ALERT,
                key: guildId,
                duration: tmp(10390).getNotificationDuration(ALERT),
                onDismiss() {
              ...
              const tmpResult3 = tmp(10390);
              obj4.inAppNotificationId = tmp(10390).generateInAppNotificationId();
...
let closure_6 = fn(7392).trackMessageNotificationTimestamps;
...
prototype["dismissNotification"] = function dismissNotification(guild) {
  ...
  if (null != incidentAlertType) {
    ...
    const result = this.dissmissedAlertsMap[incidentAlertType].set(this.key(obj2), _modDef4380());
...
prototype["wasRecentlyDismissed"] = function wasRecentlyDismissed(guild) {
  ...
  if (null == incidentAlertType) {
    ...
    if (tmp4) {
      tmp4 = _modDef4380().diff(_modDef4380(value), self.threshold.unitOfTime) < self.threshold.amount;
      obj2 = _modDef4380();
    }
    return tmp4;
  }
...
let obj = {};
obj[fn(8307).GuildIncidentAlertTypes.JOIN_RAID] = new Map();
let map = new Map();
obj[fn(8307).GuildIncidentAlertTypes.DM_RAID] = new Map();
merged[0] = obj;
merged[1] = { amount: 1, unitOfTime: "hour" };
let c21 = null;
...
let c24 = false;
let EMPTY_SCHEDULE_SNAPSHOT = fn(10378).EMPTY_SCHEDULE_SNAPSHOT;
...
const inAppNotificationStore = new InAppNotificationStore(DispatcherDefault, {
          ...
          if (tmpResult8.allowInAppNotifications()) {
            if (!tmp3Result.isEnabled()) {
              if (!ChannelRTCStore.getChatOpen(channel_id)) {
                const result = tmp(10381).shouldIncludeSelectedChannel();
                const tmpResult9 = tmp(10381);
                if (tmpResult10.shouldNotify(message, channel_id, result)) {
                  const channel = ChannelStore.getChannel(channel_id);
                  if (null == channel) {
                    return false;
                  } else {
                    const messageRecord = tmp(5010).createMessageRecord(message);
                    const tmpResult11 = tmp(5010);
                    if (tmpResult12.isMessageContentPreviewable(messageRecord)) {
                      ...
                      const notificationDuration = tmp(10390).getNotificationDuration(MESSAGE);
                      ...
                      const tmpResult13 = tmp(10390);
                      obj3.inAppNotificationId = tmp(10390).generateInAppNotificationId();
                    ...
                    tmpResult12 = tmp(10389);
                  }
                  obj7 = ChannelStore;
                } else {
                  return false;
                }
                tmpResult10 = tmp(10381);
              }
            }
            tmp3Result = ExternalPipDefault;
          }
          tmpResult8 = tmp(10381);
        }
        return false;
      }
      tmpResult = tmp(7603);
  ...
  MESSAGE_ACK: function handleMessageAck(channelId) {
    ...
    if (tmp) {
      ...
      messageId(4968).trackWithMetadata(constants.IN_APP_NOTIFICATION_DISMISSED, obj4);
      const obj3 = messageId(4968);
    }
      ...
      if (null != message.reactions) {
        if (null != emoji) {
          if (obj14.allowInAppNotifications()) {
            let tryParseChannelPathResult = tmp19(4942).tryParseChannelPath(tmp);
            ...
            if (null != channelId) {
              if (null != guildId) {
                const ReactionNotifications = tmp19(2019).ReactionNotifications;
                  ...
                  if (null != channel) {
                    if (!obj4.isEnabled()) {
                      if (!ChannelRTCStore.getChatOpen(channelId)) {
                        const result1 = tmp19(10381).shouldIncludeSelectedChannel();
                        const tmp19Result9 = tmp19(10381);
                        const obj = { message, channel, reactor: user, includeSelectedChannel: result1 };
                        if (tmp19Result10.shouldNotifyForReaction(obj)) {
                          const messageRecord = tmp19(5010).createMessageRecord(message);
                          const tmp19Result11 = tmp19(5010);
                          if (tmp19Result12.isMessageContentPreviewable(messageRecord)) {
                            ...
                            const obj3 = {
                              type: REACTION,
                              key: channelId,
                              duration: tmp19(10390).getNotificationDuration(REACTION),
                              onDismiss() {
                                                        InAppNotificationActionCreatorsDefault.clearNotification();
                                                      },
                              channel,
                            ...
                            const tmp19Result13 = tmp19(10390);
                            obj3.inAppNotificationId = tmp19(10390).generateInAppNotificationId();
                            obj2.notification = obj3;
                            handleEnqueueNotification(obj2);
                          } else {
                            return false;
                          }
                          tmp19Result12 = tmp19(10389);
                        } else {
                          return false;
                        }
                        tmp19Result10 = tmp19(10381);
                      }
                    }
                    return false;
                  }
      ...
      if (null != savedMessage.message) {
        ...
        if (null != channel) {
          ...
          const obj3 = {
            ...
            duration: tmp(10390).getNotificationDuration(MESSAGE_REMINDER),
            onDismiss() {
                    InAppNotificationActionCreatorsDefault.clearNotification();
                  },
            channel,
          ...
          const tmpResult = tmp(10390);
          obj3.inAppNotificationId = tmp(10390).generateInAppNotificationId();
          obj2.notification = obj3;
          handleEnqueueNotification(obj2);
          const tmpResult2 = tmp(10390);
        }
      }
    }
  },
              ...
              if (null == user) {
                ...
                if (null == guild) {
                  ...
                  const obj3 = {
                    ...
                    guild,
                    key: channel.id,
                    duration: tmp(10390).getNotificationDuration(FORUM_THREAD_CREATED),
                    onDismiss() {
                                    InAppNotificationActionCreatorsDefault.clearNotification();
                                  },
                    inAppNotificationId: null
                  };
                  const tmpResult5 = tmp(10390);
                  obj3.inAppNotificationId = tmp(10390).generateInAppNotificationId();
  ...
  CHANNEL_SELECT: function handleChannelSelect(channelId) {
    ...
    if (tmp) {
      const result = channelId(10390).extractMetadataFromNotification(_null);
  ...
  CURRENT_USER_UPDATE: function handleCurrentUserUpdate() {
    if (c24) {
      ...
      if (null != diffSchedulesResult) {
        const EnableScreenDowntimeScheduleNotifications = tmp2(2019).EnableScreenDowntimeScheduleNotifications;
        if (EnableScreenDowntimeScheduleNotifications.getSetting()) {
          if (tmp2Result7.allowInAppNotifications()) {
            ...
            const notificationDuration = tmp2(10390).getNotificationDuration(RESTRICTED_SCHEDULE_UPDATED);
            const tmp2Result8 = tmp2(10390);
            obj3.key = tmp2(10378).restrictedScheduleNotificationKey(diffSchedulesResult.kind);
            obj3.duration = notificationDuration;
            obj3.onDismiss = function onDismiss() {
              InAppNotificationActionCreatorsDefault.clearNotification();
            };
            const tmp2Result9 = tmp2(10378);
            obj3.title = tmp2(10378).getRestrictedScheduleNotificationTitle(diffSchedulesResult.kind);
            const tmp2Result10 = tmp2(10378);
            obj3.subtitle = tmp2(10378).getRestrictedScheduleNotificationSubtitle(diffSchedulesResult.rule);
            const tmp2Result11 = tmp2(10378);
            obj3.inAppNotificationId = tmp2(10390).generateInAppNotificationId();
            obj2.notification = obj3;
            handleEnqueueNotification(obj2);
            const tmp2Result12 = tmp2(10390);
          }
          tmp2Result7 = tmp2(10381);
        }
      }
    } else {
      return false;
