...
// Dependencies: [5, 4703, 1074, 1272, 17703, 1366, 1434, 573, 1271, 502, 1346, 2109, 1372, 1241, 1364, 17704, 1116, 17705, 17706, 7, 1231, 17709, 1980, 5114, 15949, 10038, 5771, 17715, 14129, 17716, 1462, 2]

// Module 17702 (superagentPatch)
...
import getTimeZoneDefault from "getTimeZone" /* 17705 */;
import trackHttpRequestDefault from "trackHttpRequest" /* 17706 */;
...
const idGenerator = new fn(17703).IdGenerator();
...
HTTPUtils.setRequestPatch({
  prepareRequest(promise) {
    ...
    if ("/" === promise.url[0]) {
      ...
      const result1 = tmp(17704).updateDynamicSuperProperties();
  ...
  interceptResponse(statusCode, arg1, arg2) {
    ...
    if (400 === statusCode.statusCode) {
      ...
      if (captcha_key) {
        const items = [statusCode(1980)(17709, dependencyMap.paths), statusCode(1980)(5114, dependencyMap.paths)];
    ...
    if (401 === statusCode.statusCode) {
      ...
      if (code === AbortCodes.MFA_REQUIRED) {
        ...
        if (mfa) {
          const promise4 = statusCode(1980)(15949, dependencyMap.paths);
          statusCode(1980)(15949, dependencyMap.paths).then((openMFAModal) => {
            openMFAModal.openMFAModal(statusCode.body.mfa, closure_1, closure_2);
          }).catch(arg2);
          flag = true;
          const nextPromise2 = statusCode(1980)(15949, dependencyMap.paths).then((openMFAModal) => {
            openMFAModal.openMFAModal(statusCode.body.mfa, closure_1, closure_2);
          });
        }
      }
    ...
    if (obj.isLimitedAccessErrorCode(statusCode.statusCode, code1)) {
      tmp7(1980)(5771, tmp8.paths).then((result) => {
        result.default();
      });
      flag = false;
      const promise3 = tmp7(1980)(5771, tmp8.paths);
    } else {
      ...
      if (tmp7Result.isLimitedAccessErrorCode(statusCode.statusCode, code2)) {
        tmp7(1980)(14129, tmp8.paths).then((result) => {
        ...
        flag = false;
        const promise2 = tmp7(1980)(14129, tmp8.paths);
      } else {
        ...
        if (flag) {
          tmp7(1980)(17716, tmp8.paths).then((openRestrictedHoursModal) => {
            const result = openRestrictedHoursModal.openRestrictedHoursModal();
          });
          flag = false;
          const promise = tmp7(1980)(17716, tmp8.paths);
        }
      }
      tmp7Result = tmp7(17715);
    }
  }
});
HTTPUtils = fn(1271);
