...
// Dependencies: [109, 19, 21, 12, 7586, 558, 1231, 2]
// Exports: default, useSubscribeGuildMembers

// Module 7585 (subscribeGuildMembers)
      ...
      if (!discord_common_shallowEqualDefault(this.props, arg0)) {
        ...
        if (!isEqualResult) {
          if (null != self._subscriptions) {
            const item = tmp(12).forEach(self._subscriptions, (userIds, guildId) => displayName(7586).unsubscribeMembers(guildId, userIds));
            const tmpResult3 = tmp(12);
          }
          const item1 = tmp(12).forEach(tmp4, (userIds, guildId) => displayName(7586).subscribeMembers(guildId, userIds));
...
export const useSubscribeGuildMembers = function useSubscribeGuildMembers(memo, AddMembersActionSheet) {
  ...
  const effect = noop.useEffect(() => {
    let item = _modDef12.forEach(closure_0, (userIds, guildId) => {
      ...
      closure_0(7586).subscribeMembers(guildId, userIds);
    });
    return () => {
