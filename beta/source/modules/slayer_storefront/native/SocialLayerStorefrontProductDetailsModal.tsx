...
// Dependencies: [32, 19, 17, 5591, 7331, 10932, 1074, 21, 672, 576, 4636, 5763, 10933, 8425, 5668, 4632, 1114, 10934, 1176, 1925, 4878, 1611, 5207, 9496, 504, 7271, 7268, 7329, 7285, 5073, 1240, 10929, 10930, 1363, 4307, 10935, 9495, 10936, 10945, 7334, 8952, 10949, 3460, 5056, 8202, 8188, 10952, 5712, 5761, 5068, 10954, 2]
// Exports: default

// Module 10931 (SocialLayerStorefrontProductDetailsModal)
...
import carouselMediaItems from "carouselMediaItems" /* 10933 */;
import StorefrontNativeUtils from "StorefrontNativeUtils" /* 10934 */;
import redirectToSlayerStorefrontWebDefault from "redirectToSlayerStorefrontWeb" /* 10935 */;
...
function SKUNameAndDescriptionSection(sku) {
  ...
  if (!obj.isNullOrEmpty(sku.name)) {
    ...
    const items = [closure_1_14(tmp2(4632).Heading, obj3), ];
    ...
    if (!isNullOrEmptyResult) {
      ...
      tmp7Result = tmp7(tmp2(4632).Text, obj4);
    }
    items[1] = tmp7Result;
    obj2.children = items;
...
function ItemDetailsSection(selectedItem) {
  ...
  if (!isNullOrEmptyResult) {
    ...
    tmp14 = closure_1_14(tmp8(4632).Heading, obj4);
  }
  const items = [tmp14, , ];
  ...
  if (!isNullOrEmptyResult1) {
    ...
    items1[1] = closure_1_14(tmp8(4632).Text, obj8);
    obj5.children = items1;
  ...
  if (!isNullOrEmptyResult2) {
    ...
    tmp23 = closure_1_14(tmp8(4632).Text, obj9);
  }
  items[2] = tmp23;
  obj3.children = items;
...
function SocialLayerStorefrontProductDetailsModal(skuId) {
  ...
  const isScreenLandscape = skuId(5207).useIsScreenLandscape();
  let obj = skuId(5207);
  const mobileStoreFront = analyticsLocations(9496).useMobileStoreFront();
  let obj2 = analyticsLocations(9496);
  ...
  const getOrFetchApplication = skuId(7271).useGetOrFetchApplication(applicationId1);
  let tmp12 = getOrFetchApplication;
  const obj5 = skuId(7271);
  if (getOrFetchApplication == null) {
    tmp12 = null;
  }
  const tmp2Result = analyticsLocations(7268);
  ...
  trackPDPClick = stateFromStores.useCallback((cta_type) => {
    const obj2 = { slayer_storefront_session_id: "Array", sku_id: skuId, guild_id: true, application_id: true, cta_type: "requestPermissions", location_stack: null };
    let applicationId;
    if (stateFromStores != null) {
      applicationId = stateFromStores.applicationId;
    }
  ...
  if (tmp30) {
    ...
    const items10 = [skuId];
    obj6.skuIds = items10;
    obj6.onOrderRetryCancellation = tmp4(10929).closeSocialLayerStorefrontProductDetailsModal;
    ...
    obj6.children = closure_14(tmp4(10945).HeadlessSlayerStorefrontPurchaseRunner, obj9);
    tmp43Result = tmp43(tmp2(10936), obj6, skuId);
    const tmp2Result2 = tmp2(10936);
  }
  ...
  if (null == stateFromStores) {
    ...
    if (null != tmp25) {
      ...
      if (null != memo) {
        ...
        tmp49 = closure_14(tmp2(8952), obj11);
      }
    }
    let tmp53 = null;
    if (stateFromStores.exclusive) {
      const obj12 = { style: tmp.exclusiveBadgeContainer, children: closure_14(tmp4(10949).ExclusiveBadge, {}) };
    ...
    if (!result) {
      ...
      tmp65Result = tmp65(tmp4(4632).Text, obj18);
    }
    ...
    obj20.children = closure_14(tmp4(5056).Button, obj21);
    const items14 = [closure_14(memo1, obj20), ];
    if (result1) {
      const obj22 = { icon: tmp2(8188), variant: "primary", size: "lg", disabled: tmp28, accessibilityLabel: null, onPress: null };
      const intl2 = tmp4(1114).intl;
      obj22.accessibilityLabel = intl2.string(tmp4(1114).t.QAZA5f);
      obj22.onPress = tmp48;
      result1 = tmp65(tmp4(8202).IconButton, obj22);
    }
    items14[1] = result1;
    ...
    if (result) {
      ...
      const mobileFinePrintMessageForApplication = tmp4(10952).getMobileFinePrintMessageForApplication(getOrFetchApplication, stringResult, obj24);
    ...
    const obj28 = {
      onPress: tmp4(10929).closeSocialLayerStorefrontProductDetailsModal,
      backImage() {
    ...
    const items17 = [closure_14(tmp4(5712).HeaderBackButton, obj28), ];
    ...
    obj29.children = name;
    items17[1] = closure_14(tmp4(4632).Heading, obj29);
    ...
    if (exclusive) {
      ...
      exclusive = tmp65(tmp2(5068), obj31);
    }
...
obj.header = { height: fn(5763).NAV_BAR_HEIGHT, flexDirection: "row", alignItems: "center", paddingHorizontal: nativeDefault.space.PX_8, gap: nativeDefault.space.PX_8, borderBottomWidth: 1, borderBottomColor: nativeDefault.colors.BORDER_SUBTLE };
obj.headerTitle = { flexShrink: 1 };
let obj2 = { height: fn(5763).NAV_BAR_HEIGHT, flexDirection: "row", alignItems: "center", paddingHorizontal: nativeDefault.space.PX_8, gap: nativeDefault.space.PX_8, borderBottomWidth: 1, borderBottomColor: nativeDefault.colors.BORDER_SUBTLE };
...
obj.hero = { marginHorizontal: nativeDefault.space.PX_16, height: fn(10933).MOBILE_HERO_HEIGHT_PX, borderRadius: nativeDefault.radii.md, overflow: "hidden", backgroundColor: nativeDefault.colors.BACKGROUND_SURFACE_HIGH };
obj.heroLandscape = { flex: 1, minHeight: 140, height: "ao" };
let obj20 = { marginHorizontal: nativeDefault.space.PX_16, height: fn(10933).MOBILE_HERO_HEIGHT_PX, borderRadius: nativeDefault.radii.md, overflow: "hidden", backgroundColor: nativeDefault.colors.BACKGROUND_SURFACE_HIGH };
