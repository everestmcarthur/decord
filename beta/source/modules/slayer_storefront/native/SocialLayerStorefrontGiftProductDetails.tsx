...
// Dependencies: [19, 17, 1074, 21, 4788, 576, 7445, 11098, 1397, 9138, 4784, 2]
// Exports: default

// Module 11300 (SocialLayerStorefrontGiftProductDetails)
...
export default function SocialLayerStorefrontGiftProductDetails(sku) {
  ...
  const tmp = closure_9();
  getOrFetchApplication = getOrFetchApplication(7445).useGetOrFetchApplication(sku.applicationId);
  let obj = getOrFetchApplication(7445);
  const userPrice = getOrFetchApplication(11098).useFormattedSKUPrice({ sku, priceSetAssignmentPurchaseType: constants.GIFT }).userPrice;
  ...
  if (tmp6Result) {
    ...
    items2[1] = tmp8(tmp2(4784).Text, obj9);
    obj6.children = items2;
    tmp6Result = tmp6(tmp7, obj6);
  }
  const items3 = [tmp6Result, closure_7(getOrFetchApplication(4784).Text, { variant: "text-md/semibold", children: sku.name })];
  ...
  if (tmp8Result2) {
    ...
    tmp8Result2 = tmp8(tmp2(4784).Text, obj11);
  }
