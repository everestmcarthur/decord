...
import ReportMenuType from "ReportMenuType" /* 8755 */;
...
function genSubmitData(version, name, arr, email_token) {
  ...
  let obj2 = { channel_id: "position", message_id: "call", stage_instance_id: "current", guild_id: "getCurrentUser", guild_scheduled_event_id: "call", user_id: "length", email_token: "createStyles", application_id: "call", entrypoint: "ix", widget_id: "getCurrentUser" };
  obj.language = str;
  ...
  if (name.name !== MenuTypes.ReportNames.MESSAGE) {
    if (name.name !== tmp(8753).ReportNames.FIRST_DM) {
      if (name.name !== tmp(8753).ReportNames.GUILD) {
        if (name.name !== tmp(8753).ReportNames.GUILD_DISCOVERY) {
          if (name.name === tmp(8753).ReportNames.GUILD_DIRECTORY_ENTRY) {
            const obj3 = {};
            ...
            return obj3;
          } else if (name.name === tmp(8753).ReportNames.STAGE_CHANNEL) {
            const obj6 = {};
            ...
            return obj6;
          } else if (name.name === tmp(8753).ReportNames.GUILD_SCHEDULED_EVENT) {
            const obj7 = {};
            ...
            if (name.name === tmp(8753).ReportNames.USER) {
              const obj8 = {};
              ...
              let tmp3 = obj8;
            } else if (name.name === tmp(8753).UnauthenticatedReportNames.USER) {
              const obj9 = {};
              ...
              tmp3 = obj9;
            } else if (name.name === tmp(8753).UnauthenticatedReportNames.MESSAGE) {
              const obj10 = {};
              ...
              tmp3 = obj10;
            } else if (name.name === tmp(8753).UnauthenticatedReportNames.GUILD) {
              const obj11 = {};
              ...
              tmp3 = obj11;
            } else if (name.name === tmp(8753).ReportNames.APPLICATION) {
              const obj12 = {};
              ...
              tmp3 = obj12;
            } else if (name.name === tmp(8753).ReportNames.WIDGET) {
              const obj13 = {};
              ...
              tmp3 = null;
              if (name.name === tmp(8753).UnauthenticatedReportNames.MEDIA_TAKEDOWN) {
                const obj14 = {};
...
export const submitReport = function submitReport(language, name, arr) {
  if (DevSettingsStore.get("iar_skip_api_report_submit")) {
    ...
    const REPORT_TO_MOD = obj4(8755).ReportMenuTypeSets.REPORT_TO_MOD;
    if (REPORT_TO_MOD.has(name.name)) {
      ...
      if (name.name === tmp4(8753).ModeratorReportNames.MESSAGE) {
        let obj2 = { channel_id: "Array", message_id: "call", guild_id: "s" };
        obj4 = {};
      ...
      name = name.name;
      let _Object = Object;
      const values = Object.values(tmp4(8753).ReportNames);
...
export const trackCloseReportModalAnalytics = function trackCloseReportModalAnalytics(name, c12, onSubmit) {
  ...
  if (name.name !== MenuTypes.ReportNames.GUILD) {
    if (name.name !== tmp2(8753).ReportNames.GUILD_DISCOVERY) {
      if (name.name === tmp2(8753).ReportNames.GUILD_DIRECTORY_ENTRY) {
        let id2 = name.record.guildId;
      } else if (name.name === tmp2(8753).ReportNames.GUILD_SCHEDULED_EVENT) {
        id2 = name.record.guild_id;
      }
    }
    obj2.guild_id = id2;
    if (name.name === tmp2(8753).ReportNames.GUILD_SCHEDULED_EVENT) {
      let channelId = name.record.channel_id;
    } else if (name.name === tmp2(8753).ReportNames.GUILD_DIRECTORY_ENTRY) {
      channelId = name.record.channelId;
    }
    obj2.channel_id = channelId;
    let id4;
    if (name.name === tmp2(8753).ReportNames.APPLICATION) {
      id4 = name.record.id;
    }
