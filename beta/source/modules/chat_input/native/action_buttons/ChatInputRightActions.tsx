...
// Dependencies: [32, 19, 17, 12244, 21, 4788, 576, 4489, 4498, 12456, 1610, 12529, 4524, 12528, 12527, 2]

// Module 12713 (ChatInputRightActions)
import nativeDefault from "native" /* 576 */;
import native from "native" /* 4498 */;
import ReanimatedRexportDefault from "ReanimatedRexport" /* 4524 */;
import ChatInputActionButtonGiftOrThreadDefault from "ChatInputActionButtonGiftOrThread" /* 12527 */;
import ChatInputActionButtonTransitionItem from "ChatInputActionButtonTransitionItem" /* 12528 */;
import useChatInputFloatingBounceDefault from "useChatInputFloatingBounce" /* 12529 */;
...
const enterDelayMs = fn(12244).CHAT_INPUT_FLOATING_BOUNCE_ENTER_DELAY_MS;
const jsxProd = fn(21);
...
const forwardRefResult = noop.forwardRef((channel, ref) => {
  ...
  const token = channel(4489).useToken(onPressAction(576).modules.mobile.CHAT_INPUT_ACTION_BUTTON_SIZE);
  const obj = channel(4489);
  const tmp3 = onPressAction;
  const sum = token + 2 * channel(4489).useToken(onPressAction(576).modules.mobile.CHAT_INPUT_ACTION_BUTTON_MARGIN);
  dependencyMap = sum;
  ...
  if (shouldShowGiftButton) {
    ...
    tmp14Result = closure_7(tmp(4498).TransitionItem, obj4);
  }
  ...
  items1[1] = closure_7(tmp3(12456), obj5);
