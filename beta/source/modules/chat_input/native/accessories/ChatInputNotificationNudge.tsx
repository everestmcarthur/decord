...
// Dependencies: [32, 19, 17, 4429, 4969, 12692, 1074, 2038, 12693, 21, 4788, 576, 1241, 10449, 4784, 5371, 12694, 1115, 12695, 5929, 504, 12427, 12703, 7662, 2027, 2]
// Exports: default

// Module 12691 (ChatInputNotificationNudge)
...
const PostReactionPermissionNudgeExperimentDefault = tmp4(12703);
require = fn;
...
export default function ChatInputNotificationNudge(channel) {
  ...
  const shouldShowPushNotificationNudgeByPromptType = channel(12694).useShouldShowPushNotificationNudgeByPromptType(PermissionPromptType.CHANNEL_BANNER);
  const tmpResult = channel(12694);
  const tmp4Result = PostReactionPermissionNudgeExperimentDefault;
  const shouldShowPushNotificationNudgeByPromptType1 = channel(12694).useShouldShowPushNotificationNudgeByPromptType(PermissionPromptType.POST_REACTION_BANNER);
  const tmpResult4 = channel(12694);
  let prop = null;
  if (tmp5) {
    prop = null;
    if (shouldShowPushNotificationNudgeByPromptType) {
      prop = tmp(2027).DismissibleContent.NOTIFICATION_NUDGE_CHAT_BOTTOM_BANNER;
    }
  }
  const obj2 = { cooldownDurationMs };
  const tmp10 = cooldownDurationMs;
  const tmp11 = _slicedToArray;
  const tmpResult5 = channel(7662);
  [tmp13, tmp14] = channel(7662).useSelectedTimeRecurringDismissibleContent(prop, { cooldownDurationMs }, undefined, true);
  importDefault = tmp14;
  const tmp12 = _slicedToArray(channel(7662).useSelectedTimeRecurringDismissibleContent(prop, { cooldownDurationMs }, undefined, true), 2);
  let prop1 = null;
  if (tmp5) {
    prop1 = null;
    if (tmp4Result.useConfig({ location: "ChatInputNotificationNudge" }).enabled) {
      ...
      if (shouldShowPushNotificationNudgeByPromptType1) {
        prop1 = null;
        if (null == tmp13) {
          prop1 = tmp(2027).DismissibleContent.NOTIFICATION_NUDGE_POST_REACTION_BANNER;
        }
      }
    }
  }
  const tmp11Result = tmp11(channel(7662).useSelectedTimeRecurringDismissibleContent(prop1, { cooldownDurationMs: tmp10 }, undefined, true), 2);
  dependencyMap = tmp17;
  const items1 = [tmp14];
  const items2 = [tmp11Result[1]];
  const obj3 = { cooldownDurationMs: tmp10 };
  const tmpResult6 = channel(7662);
  const tmp18 = closure_4(() => _undefined(ContentDismissActionType.USER_DISMISS), items1);
  if (tmp13 === channel(2027).DismissibleContent.NOTIFICATION_NUDGE_CHAT_BOTTOM_BANNER) {
    ...
    tmp24 = null;
    if (tmp11Result[0] === tmp(2027).DismissibleContent.NOTIFICATION_NUDGE_POST_REACTION_BANNER) {
