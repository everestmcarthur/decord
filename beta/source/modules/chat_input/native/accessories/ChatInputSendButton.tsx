...
// Dependencies: [32, 19, 17, 4780, 7955, 12244, 21, 4788, 576, 4489, 12528, 12537, 12521, 4732, 1115, 504, 4498, 12541, 4524, 2]

// Module 12536 (ChatInputSendButton)
...
import useChatInputFloatingWidthDefault from "useChatInputFloatingWidth" /* 12541 */;
...
let closure_8 = fn(12244).CHAT_INPUT_FLOATING_BOUNCE_ENTER_DELAY_MS;
const jsx = fn(21).jsx;
...
let closure_12 = noop.memo((type) => {
  ...
  if (type === constants.BUTTON_SEND_VOICE_MESSAGE || type === constants.BUTTON_SEND_VOICE_MESSAGE_DISABLED) {
    ...
    let tmp7Result = tmp7(tmp3(12537), obj5);
  } else {
...
const forwardRefResult = noop.forwardRef((channel, ref) => {
  ...
  const token = channel(4489).useToken(onSendMessage(576).modules.mobile.CHAT_INPUT_SEND_BUTTON_WIDTH);
  const obj = channel(4489);
  const token1 = channel(4489).useToken(onSendMessage(576).modules.mobile.CHAT_INPUT_SEND_BUTTON_HEIGHT);
  const obj2 = channel(4489);
  const token2 = channel(4489).useToken(onSendMessage(576).modules.mobile.CHAT_INPUT_ACTION_BUTTON_MARGIN);
  const obj3 = channel(4489);
  ...
  if (stateFromStores) {
    ...
    if (canSendVoiceMessage) {
      ...
      let tmp15Result = tmp15(tmp3(12537), obj8);
    } else {
      ...
      obj9.IconComponent = tmp(4732).SendMessageIcon;
      const intl = tmp(1115).intl;
      obj9.accessibilityLabel = intl.string(tmp(1115).t.TXNS7S);
      obj9.onPress = onSendMessage;
      obj9.disabled = !tmp12;
      tmp15Result = tmp15(tmp3(12521), obj9);
      const tmp3Result = tmp3(12521);
    }
    ...
    const obj12 = { buttonWidth: token, buttonHeight: token1, buttonMargin: token2, sendVoiceMessageEnabled: canSendVoiceMessage, children: tmp15(tmp(4498).TransitionGroup, obj10) };
