...
// Dependencies: [32, 19, 17, 5013, 1085, 21, 4788, 576, 1612, 15970, 15971, 4991, 10283, 5351, 4498, 15974, 15975, 15977, 15992, 15993, 15994, 1115, 5929, 15995, 2]
// Exports: default

// Module 15969 (CheckpointModal)
import nativeDefault from "native" /* 576 */;
import ModalActionCreatorsDefault from "ModalActionCreators" /* 4991 */;
import CheckpointFlows from "CheckpointFlows" /* 15970 */;
import _slicedToArray from "module_32" /* 32 */;
import noop from "module_19" /* 19 */;

require = fn;
const View = fn(17).View;
const CheckpointConstants = fn(5013);
...
export default function CheckpointModal(didPlayerShareDataWithDiscord) {
  ...
  checkpointFlow = checkpointFlow(15970).getCheckpointFlow(flag);
  [route, dependencyMap] = noop.useState(checkpointFlow(15971).CheckpointRoute.HOME);
  let obj = checkpointFlow(15970);
  ...
  if (tmp9) {
    let VoiceNormalIcon = tmp4(10283).VoiceXIcon;
  } else {
    VoiceNormalIcon = tmp4(5351).VoiceNormalIcon;
  }
  const tmp8 = _slicedToArray(noop.useState(false), 2);
  const tmp4Result = checkpointFlow(15970);
  const tmp13 = null == checkpointFlow(15970).getAdjacentCheckpointRoute(checkpointFlow, route, 1);
  const checkpointRoutePresentation = checkpointFlow(15971).getCheckpointRoutePresentation(route);
  ({ characterStage, statsScreen } = checkpointRoutePresentation);
  let tmp16Result2 = route === tmp4(15971).CheckpointRoute.HOME || null != statsScreen;
  ...
  const tmp4Result2 = checkpointFlow(15971);
  obj8.uri = route(15993);
  obj8.style = tmp.logo;
  const items6 = [closure_8(route(15992), obj8), ];
  ...
  const tmp2Result = route(15992);
  const intl = tmp4(1115).intl;
  const t = tmp4(1115).t;
  const tmp2Result3 = route(15994);
  ...
  const tmp22 = route === checkpointFlow(15971).CheckpointRoute.HOME;
  obj12.onPress = route(4991).pop;
  const intl2 = tmp4(1115).intl;
  obj12.accessibilityLabel = intl2.string(checkpointFlow(1115).t.cpT0Cq);
  obj12.children = closure_8(checkpointFlow(5929).XSmallIcon, { color, size: "xs" });
  items7[1] = closure_8(route(15994), obj12);
  obj9.children = items7;
  items6[1] = closure_9(callback, obj9);
  obj7.children = items6;
  items4[3] = closure_9(callback, obj7);
  items4[4] = closure_8(route(15995), { onBack: callback1, onNext: callback2, isTerminal: tmp13, isHome: tmp22 });
  obj3.children = items4;
  obj2.children = closure_9(callback, obj3);
  return closure_8(checkpointFlow(4498).ThemeContextProvider, obj2);
};
