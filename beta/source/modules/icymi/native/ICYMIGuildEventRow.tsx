...
// Dependencies: [19, 17, 7800, 2041, 2063, 21, 16795, 576, 9788, 1115, 9785, 4941, 9822, 9898, 8654, 9919, 7616, 9910, 16836, 11, 4784, 9900, 5339, 1177, 504, 2]
// Exports: default
...
function ICYMIGuildEventRow(event) {
  ...
  const tmp4 = guild(9788)(event, null);
  ...
  const tmp7 = guild(4941)(channel);
  const locationFromEvent = event(9822).getLocationFromEvent(event);
  let obj = event(9822);
  const eventLocationIconSource = event(9898).getEventLocationIconSource(event, channel, true);
  let obj2 = event(9898);
  const eventLocationIconComponent = event(9898).getEventLocationIconComponent(event, channel, true);
  let obj3 = event(9898);
  const items1 = [event];
  const nextRecurrenceIdInEvent = event(9785).getNextRecurrenceIdInEvent(event);
  ...
  const tmp2ResultResult = guild(9910)(guild_id, id, nextRecurrenceIdInEvent);
  ...
  obj7.children = closure_11(event(4784).Text, {
    ...
    children: noop.useMemo(() => {
      if (closure_2) {
        ...
        eventTimeData = tmp(9785).getEventTimeData(c3);
  ...
  items3[1] = closure_11(event(4784).Text, { style: title, variant: "text-lg/semibold", children: event.name });
  ...
  if (tmp19Result) {
    ...
    obj9.children = tmp8(9900).guildEventDetailsParser(event.description, true, obj10);
    tmp19Result = tmp19(tmp8(4784).Text, obj9);
    const tmp8Result = tmp8(9900);
  }
  ...
  const items4 = [closure_11(event(5339).GroupIcon, { size: "xs", style: tmp.eventsChannelIcon }), ];
  ...
  items4[1] = closure_11(event(4784).Text, obj15);
  ...
  if (tmp7 == null) {
    ...
    if (null != locationFromEvent) {
      result = tmp8(9900).guildEventLocationParser(locationFromEvent, true);
      const tmp8Result2 = tmp8(9900);
    }
    tmp27 = result;
  }
  items6[1] = closure_11(event(4784).Text, { lineClamp: 2, variant: "text-xs/normal", color: "text-muted", children: tmp27 });
