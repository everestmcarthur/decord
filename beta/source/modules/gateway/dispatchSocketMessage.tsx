...
// Dependencies: [109, 2045, 4687, 1386, 2041, 2105, 1372, 4807, 4451, 4452, 1074, 3, 13940, 13922, 9, 504, 13972, 1222, 13973, 7766, 13977, 573, 1393, 12, 8056, 1966, 1389, 4416, 8136, 8037, 14057, 5111, 7531, 8482, 8498, 5129, 5012, 4949, 1965, 14058, 2]
// Exports: default

// Module 13971 (dispatchSocketMessage)
...
import SubscriptionPlanActionCreatorsAll from "SubscriptionPlanActionCreators" /* 7531 */;
import convertServerThreadMemberDefault from "convertServerThreadMember" /* 8056 */;
...
import ReadyPayloadUtilsAll from "ReadyPayloadUtils" /* 13972 */;
import splitAgeRestrictedActivitiesDefault from "splitAgeRestrictedActivities" /* 14058 */;
...
import ActionBatcher_mod from "ActionBatcher" /* 13940 */;
...
function dispatchGuildMemberAdd(guild_id, author, member) {
  ...
  const result2 = obj5(4416).parseServerMemberGamingLeaderboardData(member_gaming_leaderboard_data);
...
const importDefaultResult4 = new ActionBatcher(fn(13922).socket, (arg0, id) => {
  let tmp = arg0;
  if (arg0 == null) {
...
let ActionBatcher = ActionBatcher_mod;
const importDefaultResult11 = new ActionBatcher(fn(13922).socket, (arg0, guildId) => {
  let tmp = arg0;
  if (null == arg0) {
...
let ActionBatcher = ActionBatcher_mod;
const importDefaultResult21 = new ActionBatcher(fn(13922).socket, (arg0, arg1) => {
  let tmp = arg0;
  if (arg0 == null) {
...
let ActionBatcher = ActionBatcher_mod;
const importDefaultResult31 = new ActionBatcher(fn(13922).socket, (arg0, arg1) => {
  let tmp = arg0;
  if (null == arg0) {
...
let result = definePreloadableDispatch(["INITIAL_GUILD"], (data_mode) => {
  ...
  initialGuild.measure(() => {
    ...
    Emitter.batched(() => {
      const hydrateInitialGuildResult = ReadyPayloadUtilsAll.hydrateInitialGuild(closure_1_0, closure_0(13922).socket.identifyStartTime);
  ...
  readySupplemental.measure(() => {
    ...
    Emitter.batched(() => {
      ...
      const measureResult = hydrateReadySupplemental.measure(() => {
        obj = closure_2_2(13972);
        return obj.hydrateReadySupplementalPayload(obj, closure_2_0(13922).socket.identifyStartTime);
      });
      ...
      const localVoiceState = closure_1_0(13922).localVoiceState;
  ...
  if (user.user.bot) {
    ...
    ready.measure(() => {
      ...
      Emitter.batched(() => {
        ...
        const measureResult = hydrateReady.measure(() => closure_2_2(13972).hydrateReadyPayloadPrioritized(obj4, user(13922).socket.identifyStartTime, closure_1));
        ...
        let result1;
        if (null != prop) {
          result1 = user(13973).b64ToDeclarativeSettingsProto(obj4.notification_settings.declarative_settings_proto);
          let obj2 = user(13973);
        }
        closure_1_1(7766)("AllGatewayConnectionStores", () => obj4(geoRestrictedGuilds[20]));
        ...
        const localPresenceState = user(13922).localPresenceState;
        localPresenceState.update();
        const localVoiceState = user(13922).localVoiceState;
        localVoiceState.update();
      });
    });
  }
...
defineSimpleDispatch(["RESUMED"], () => {
  const localPresenceState = obj(13922).localPresenceState;
  localPresenceState.forceUpdate();
  const localVoiceState = obj(13922).localVoiceState;
...
defineSimpleDispatch(["GUILD_BAN_ADD", "GUILD_BAN_REMOVE", "GUILD_MEMBER_ADD", "GUILD_MEMBER_UPDATE", "GUILD_MEMBER_REMOVE"], (guildId, type) => {
  ...
  const obj3 = obj(1389);
  obj.gamingLeaderboardData = obj(4416).parseServerMemberGamingLeaderboardData(guildId.member_gaming_leaderboard_data);
  const obj4 = obj(4416);
  DispatcherDefault.dispatch(obj).catch((error) => {
    logger.error("dispatchOrResetSocket error during " + obj.type + ":", error);
    const socket = obj4(closure_3[13]).socket;
...
defineSimpleDispatch(["SAVED_MESSAGE_CREATE"], (body) => {
  obj = { type: "SAVED_MESSAGE_CREATE", savedMessage: obj(8136).savedMessageCreateObjectToClient(body) };
  const obj2 = obj(8136);
  DispatcherDefault.dispatch(obj).catch((error) => {
    logger.error("dispatchOrResetSocket error during " + obj.type + ":", error);
    const socket = obj4(closure_3[13]).socket;
...
defineSimpleDispatch(["SAVED_MESSAGE_DELETE"], (channelId) => {
  obj = { type: "SAVED_MESSAGE_DELETE", savedMessageData: obj(8136).savedMessageDeleteObjectToClient(channelId) };
  const obj2 = obj(8136);
  DispatcherDefault.dispatch(obj).catch((error) => {
    logger.error("dispatchOrResetSocket error during " + obj.type + ":", error);
    const socket = obj4(closure_3[13]).socket;
...
defineSimpleDispatch(["MESSAGE_POLL_VOTE_ADD", "MESSAGE_POLL_VOTE_REMOVE"], (channelId, arg1) => {
  ...
  obj = { type: str, channelId: channelId.channel_id, messageId: channelId.message_id, userId: channelId.user_id, emoji: { id: channelId.answer_id, name: "" }, reactionType: obj(8037).ReactionTypes.VOTE };
  DispatcherDefault.dispatch(obj).catch((error) => {
    logger.error("dispatchOrResetSocket error during " + obj.type + ":", error);
    const socket = obj4(closure_3[13]).socket;
    obj = { error, action: obj.type };
...
defineSimpleDispatch(["INTERACTION_MODAL_CREATE"], (id) => {
  obj = { type: "INTERACTION_MODAL_CREATE", id: id.id, channelId: id.channel_id, customId: id.custom_id, application: id.application, title: id.title, components: obj(5012).transformComponents(id.components), nonce: null, resolved: null };
...
defineSimpleDispatch(["NOTIFICATION_SETTINGS_UPDATE"], (flags) => {
  ...
  let result;
  if (null != prop) {
    result = obj2(13973).b64ToDeclarativeSettingsProto(flags.declarative_settings_proto);
    const obj = obj2(13973);
  }
...
defineSimpleDispatch(["GUILD_ROOM_CONNECT"], (body, type) => {
  obj = { type, room: obj(4949).serverGuildRoomToClient(body) };
  const obj2 = obj(4949);
  DispatcherDefault.dispatch(obj).catch((error) => {
    logger.error("dispatchOrResetSocket error during " + obj.type + ":", error);
    const socket = obj4(closure_3[13]).socket;
