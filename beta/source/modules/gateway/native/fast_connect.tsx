...
// Function ID: 16
// Name: fast_connect
// Dependencies: [16, 17, 499, 3, 500, 1249, 8031, 13961, 13931, 7917, 1364, 13943, 13930, 13927, 10, 9, 2]
...
function createFastConnectSocket() {
  if (null != window.WebSocket) {
    ...
    if (obj.isAndroid()) {
      let supportsZstd = obj4(13943).getConstants().supportsZstd;
      const obj2 = obj4(13943);
    } else {
    ...
    const _Date = Date;
    _require = Date.now();
    const tmp12 = obj4(13927)(combined);
    ...
    if (isNaN(parsed)) {
      ...
      if (supportsZstd) {
        if (isAndroidResult) {
          const result = tmp7(13943).enableZstdStreamSupport(parsed);
          const tmp7Result = tmp7(13943);
        } else {
        ...
        const result2 = tmp7(13943).enableZlibStreamSupport(parsed);
        const tmp7Result3 = tmp7(13943);
      } else {
