...
// Dependencies: [32, 13933, 3, 13934, 13937, 4817, 13938, 13936, 13939, 504, 13940, 2]

// Module 13932 (GatewaySocketDispatcher)
import LoggerDefault from "Logger" /* 3 */;
import TimeUtils from "TimeUtils" /* 4817 */;
import WorkSchedulerTelemetry from "WorkSchedulerTelemetry" /* 13936 */;
import GatewaySocketAnalytics from "GatewaySocketAnalytics" /* 13937 */;
import VoiceServerUpdateImmediateExperiment from "VoiceServerUpdateImmediateExperiment" /* 13938 */;
import ConnectionStateDefault from "ConnectionState" /* 13939 */;
import ActionBatcherDefault from "ActionBatcher" /* 13940 */;
import _slicedToArray from "module_32" /* 32 */;

require = fn;
let closure_4 = fn(13933).DISPATCHER_IDEAL_TIME_LIMIT_MS;
  ...
  if (0 === items.length) {
    ...
    try {
      ...
      if (closure_5.length > 0) {
        ...
        telemetry.measure(tmp21(13936).WorkSchedulerTelemetryMeasurement.COUNT_DISPATCHES_LEFT_AFTER_YIELD, closure_5.length);
        const queue = self.queue;
...
prototype["dispatchOne"] = function dispatchOne(arg0) {
  ...
  if (this.socket.connectionState !== ConnectionStateDefault.RESUMING) {
    ...
    if (self.socket.connectionState === tmp2(13939).RESUMING) {
