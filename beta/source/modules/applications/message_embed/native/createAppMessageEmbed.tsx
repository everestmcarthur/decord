...
// Dependencies: [32, 1372, 8441, 5015, 1483, 9346, 11601, 7440, 12223, 8235, 1115, 12224, 9435, 8440, 9626, 12225, 12226, 1397, 1366, 12227, 7459, 11801, 7797, 9603, 4654, 1610, 9351, 7466, 4485, 1370, 2]
// Exports: createAppMessageEmbed, getAppLinkGateResult, handleTapAppMessageEmbed

// Module 12222 (createAppMessageEmbed)
...
import AppLauncherPlayUtils from "AppLauncherPlayUtils" /* 11801 */;
import ContentClassificationVisibility from "ContentClassificationVisibility" /* 12223 */;
import joinOrStartActivityInChannel from "joinOrStartActivityInChannel" /* 12227 */;
...
export const getAppLinkGateResult = function getAppLinkGateResult(arg0) {
  ...
  if (null == application) {
    ...
    if (contentClassificationVisibility !== ContentClassificationVisibility.ContentClassificationVisibility.DISPLAY) {
      ...
      if (contentClassificationVisibility === tmp3(12223).ContentClassificationVisibility.BLOCK_UNDERAGE) {
      ...
      obj4.displayType = tmp3(12224).AppMessageEmbedDisplayType.BLOCKED;
      obj4.appId = "";
...
export const createAppMessageEmbed = function createAppMessageEmbed(arg0) {
  ...
  if (isEmbeddedAppResult) {
    ...
    if (applicationAssetFetchState === FetchState.NOT_FETCHED) {
      const assetIds = tmp2(8440).fetchAssetIds(id, closure_11);
      return null;
  ...
  if (null != maxParticipants) {
    ...
    if (isEmbeddedAppResult) {
      if (tmp2Result9.canLaunchFrame(app)) {
        ...
        const playInContext = tmp2(12225).getPlayInContext(id, message.channel_id);
        ...
        const tmp2Result10 = tmp2(12225);
      }
      tmp2Result9 = tmp2(9626);
    }
    ({ id: id2, bot: bot2 } = app);
    const joined = tags.join(" \u2219 ");
    if (tmp2Result11.isEmbeddedApp(app)) {
      let assetIds1 = tmp2(8440).getAssetIds(id2, closure_11);
      if (assetIds1 == null) {
        assetIds1 = [];
      }
      ...
      let assetImage = null;
      if (null != first) {
        assetImage = tmp2(8440).getAssetImage(id2, first, c12);
        const tmp2Result13 = tmp2(8440);
      }
      if (null != assetImage) {
      ...
      let appIconSrc = null;
      if (null != icon) {
        appIconSrc = tmp2(12226).getAppIconSrc(id, icon, bot);
        const tmp2Result14 = tmp2(12226);
      }
      let staticBannerSrc = appIconSrc;
      if (appIconSrc == null) {
        staticBannerSrc = obj10.staticBannerSrc;
      }
      const tmp2Result12 = tmp2(8440);
      const obj7 = {};
      const appGradientColors = tmp2(12226).getAppGradientColors(staticBannerSrc);
      const merged = Object.assign(baseColors);
      const merged1 = Object.assign(obj10);
      obj7.displayType = tmp2(12224).AppMessageEmbedDisplayType.DISPLAY;
      obj7.appId = app.id;
...
export const handleTapAppMessageEmbed = function handleTapAppMessageEmbed(appId) {
  ...
  if ("play_in_channel" === actionId) {
    ...
    const items = [tmp2(7459).APP_MESSAGE_EMBED];
    ...
    if (null != bot) {
      ...
      const items1 = [tmp2(7459).APP_MESSAGE_EMBED];
    ...
    const items2 = [tmp2(7459).APP_MESSAGE_EMBED];
    obj11.analyticsLocations = items2;
    obj10.analyticsContext = obj11;
    tmp2(9603).launchFrame(obj10);
    const tmp2Result = tmp2(9603);
