...
// Dependencies: [19, 17, 14044, 21, 4788, 576, 7466, 4485, 5854, 7410, 1115, 4489, 504, 14986, 5216, 4784, 5936, 2]
// Exports: default

// Module 14985 (UserSettingsAccountBackupCodes)
...
function CodeRow(code) {
  ...
  if (showCheckMark) {
    ...
    tmp2Result = tmp2(code(7410).CheckmarkSmallIcon, obj2);
  }
...
export default function UserSettingsAccountBackupCodes(headerLabel) {
  ...
  const token = stateFromStores(4489).useToken(nativeDefault.modules.mobile.TABLE_ROW_PADDING);
  const obj = stateFromStores(4489);
  ...
  const items2 = [headerLabel.map((children, index) => closure_1_6(stateFromStores(4784).Text, { variant: "text-sm/medium", children }, index)), , , ];
  ...
  if (tmp10Result3) {
    ...
    tmp10Result3 = tmp10(tmp3(5936).TableRowGroup, obj6);
  }
  ...
  if (tmp10Result4) {
    ...
    obj8.label = tmp10(tmp3(4784).Text, obj9);
    ...
    tmp10Result4 = tmp10(tmp3(5936).TableRowGroup, obj7);
  }
  const obj4 = { paddingHorizontal: token, paddingTop: nativeDefault.space.PX_16 };
  items2[3] = tmp10Result4;
  obj3.children = items2;
  return closure_6(tmp11, { children: tmp12(stateFromStores(5216).Stack, obj3) });
};
