...
// Dependencies: [19, 17, 21, 4788, 9362, 8642, 1115, 4784, 4747, 576, 9364, 15219, 2]
// Exports: default

// Module 15225 (UserSettingsAuthedAppPermissions)
  ...
  return closure_5(c3, {
    ...
    children: (() => {
      ...
      children[2] = items.map((children, index) => {
        ...
        items = [, ];
        items[0] = closure_2_5(items(4747).CircleCheckIcon, { style: items.permissionIcon, size: "xs", color: c1(576).colors.STATUS_POSITIVE });
        items[1] = closure_2_5(items(4784).Text, { style: items.permissionText, variant: "text-sm/normal", color: "text-default", children });
        obj.children = items;
      ...
      if (_undefined3 != null) {
        mapped = _undefined3.map((disclosure, index) => {
          ...
          if (null != textForDisclosure) {
            ...
            items = [hasOwnProperty(tmp(15219).DisclosureIcon, obj3), ];
            const obj4 = { style: items.permissionText, variant: "text-sm/normal", children: textForDisclosure };
            items[1] = hasOwnProperty(tmp(4784).Text, obj4);
            obj2.children = items;
