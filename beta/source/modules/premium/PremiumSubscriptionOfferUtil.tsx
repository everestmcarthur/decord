...
// Dependencies: [32, 19, 4452, 1374, 7721, 8350, 8351, 504, 4380, 8352, 8353, 1978, 8356, 2]
...
// Module 8349 (PremiumSubscriptionOfferUtil)
import initialize from "initialize" /* 504 */;
import Server from "Server" /* 1978 */;
import _modDef4380 from "module_4380" /* 4380 */;
import usePremiumTrialOffer from "usePremiumTrialOffer" /* 7721 */;
import PremiumSubscriptionTrialUtil from "PremiumSubscriptionTrialUtil" /* 8350 */;
...
import ReverseTrialUtils from "ReverseTrialUtils" /* 8356 */;
...
function useHasDiscountApplied() {
  ...
  if (tmp4) {
    const _Date = Date;
    tmp4 = _modDef4380(Date.now()) <= _modDef4380(prop);
    const tmp6Result = _modDef4380(Date.now());
  }
  return tmp4;
}
const size = fn(2);
...
export const useIsInPremiumOfferExperience = function useIsInPremiumOfferExperience() {
  ...
  if (typeof useHasDiscountApplied === "function") {
    ...
    if (tmp12) {
      const _Date = Date;
      tmp12 = _modDef4380(Date.now()) <= _modDef4380(prop);
      const tmp14Result = _modDef4380(Date.now());
    }
...
export const useShouldFetchChurnOffer = function useShouldFetchChurnOffer() {
  ...
  if (typeof useHasDiscountApplied === "function") {
    ...
    if (tmp8) {
      const _Date = Date;
      tmp8 = _modDef4380(Date.now()) <= _modDef4380(prop);
      const tmp10Result = _modDef4380(Date.now());
    }
