...
// Dependencies: [19, 17, 7726, 13732, 1074, 1076, 21, 4788, 576, 1177, 4784, 13733, 13734, 1115, 2108, 6603, 6189, 7727, 504, 7439, 7459, 13735, 1241, 4755, 13736, 1980, 7815, 13743, 13744, 13747, 5218, 2]
// Exports: ReferralProgramPerkCard

// Module 13731 (ReferralProgramPerkCard)
...
export const ReferralProgramPerkCard = function ReferralProgramPerkCard() {
  ...
  let callback = noop.useCallback(() => {
    ...
    ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(13736, dependencyMap.paths), "referral-program-share-action-sheet");
  }, items2);
  ...
  const shouldShowSpendOrbsCta = tmp2(13743).getShouldShowSpendOrbsCta(obj5, tmp11);
  ...
  if (!useAltReferralCardArt) {
    ...
    if (tmp11 === tmp2(13734).ReferralRewardType.ORBS) {
      const intl4 = tmp2(1115).intl;
      let stringResult = intl4.string(tmp2(1115).t.tAlkl4);
    } else if (tmp11 === tmp2(13734).ReferralRewardType.DISCOUNT) {
    ...
    const items5 = [tmp17(tmp2(4784).Text, obj11), , ];
    ...
    if (null != tmp11) {
      if (obj5.numRewardGranted !== tmp2(13733).MAX_REFERRALS_SENT) {
        if (obj5.numSent === tmp2(13733).MAX_REFERRALS_SENT) {
          if (tmp11 === tmp2(13734).ReferralRewardType.ORBS) {
      ...
      if (obj5.numRedeemed === tmp2(13733).MAX_REFERRALS_SENT) {
      ...
      items5[1] = tmp17(tmp2(4784).Text, obj13);
      if (isEligibleForIncentive) {
        const obj25 = { nRewardsGranted: obj5.numRewardGranted, referralRewardType: tmp11 };
        isEligibleForIncentive = tmp17(tmp5(13747), obj25);
      }
      ...
      if (!shouldShowSpendOrbsCta) {
        if (tmp24) {
          tmp24 = obj5.numSent !== tmp2(13733).MAX_REFERRALS_SENT;
        }
        tmp38 = tmp24;
      }
      ...
      obj26.children = tmp17(tmp2(5218).Button, obj27);
      items4[4] = tmp17(tmp14, obj26);
      obj6.children = items4;
      return tmp13(tmp14, obj6);
    }
    const tmp5Result = tmp5(2108);
  } else if (tmp11 === tmp2(13734).ReferralRewardType.ORBS) {
    let tmp5Result2 = tmp5(6603);
  } else if (tmp11 === tmp2(13734).ReferralRewardType.DISCOUNT) {
    tmp5Result2 = tmp5(6189);
  }
};
