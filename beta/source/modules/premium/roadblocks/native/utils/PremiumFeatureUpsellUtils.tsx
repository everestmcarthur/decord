...
// Dependencies: [1074, 5264, 8128, 38, 1094, 2]
// Exports: getAnalyticsPage, getUpsellType, isSoundboardSectionNitroLocked

// Module 10262 (PremiumFeatureUpsellUtils)
...
export const getAnalyticsPage = function getAnalyticsPage(featureName) {
  ...
  } else if (tmp(8128).EntitlementFeatureNames.SOUNDBOARD_EVERYWHERE === featureName) {
    return AnalyticsPages.PREMIUM_UPSELL_SOUNDBOARD_EVERYWHERE;
  } else if (tmp(8128).EntitlementFeatureNames.CLIENT_THEMES === featureName) {
  ...
  } else if (tmp(8128).EntitlementFeatureNames.SAVED_MESSAGES === featureName) {
    return AnalyticsPages.PREMIUM_UPSELL_FOR_LATER;
  } else if (tmp(8128).EntitlementFeatureNames.SCHEDULED_MESSAGES === featureName) {
...
export const getUpsellType = function getUpsellType(EMOJIS_EVERYWHERE) {
  ...
  } else if (tmp(8128).EntitlementFeatureNames.EMOJIS_EVERYWHERE === EMOJIS_EVERYWHERE) {
    return tmp(1094).UpsellTypes.GLOBAL_EMOJI;
  } else if (tmp(8128).EntitlementFeatureNames.STICKERS_EVERYWHERE === EMOJIS_EVERYWHERE) {
    return tmp(1094).UpsellTypes.GLOBAL_STICKER;
  } else if (tmp(8128).EntitlementFeatureNames.INCREASED_FILE_UPLOAD_SIZE === EMOJIS_EVERYWHERE) {
    return tmp(1094).UpsellTypes.UPLOAD;
  } else if (tmp(8128).EntitlementFeatureNames.SOUNDBOARD_EVERYWHERE === EMOJIS_EVERYWHERE) {
    return tmp(1094).UpsellTypes.SOUNDBOARD;
  } else if (tmp(8128).EntitlementFeatureNames.CLIENT_THEMES === EMOJIS_EVERYWHERE) {
    return tmp(1094).UpsellTypes.CLIENT_THEMES;
  } else if (tmp(8128).EntitlementFeatureNames.APP_ICONS === EMOJIS_EVERYWHERE) {
    return tmp(1094).UpsellTypes.APP_ICONS;
  } else if (tmp(8128).EntitlementFeatureNames.SAVED_MESSAGES === EMOJIS_EVERYWHERE) {
    return tmp(1094).UpsellTypes.FOR_LATER;
  } else if (tmp(8128).EntitlementFeatureNames.SCHEDULED_MESSAGES === EMOJIS_EVERYWHERE) {
    return tmp(1094).UpsellTypes.SCHEDULED_MESSAGES;
  } else if (tmp(8128).EntitlementFeatureNames.STREAM_HIGH_QUALITY === EMOJIS_EVERYWHERE) {
    return tmp(1094).UpsellTypes.STREAM_HIGH_QUALITY;
  } else if (tmp(8128).EntitlementFeatureNames.SHOP_MEMBER_PRICING === EMOJIS_EVERYWHERE) {
