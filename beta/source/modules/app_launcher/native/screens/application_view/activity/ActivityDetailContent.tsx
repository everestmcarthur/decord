...
// Dependencies: [5, 32, 19, 17, 9166, 2002, 1074, 21, 576, 4788, 4784, 11510, 12423, 12339, 12333, 7797, 9435, 1115, 1241, 9555, 5218, 4801, 11802, 9633, 7439, 7459, 11288, 5374, 504, 5851, 7477, 11803, 6880, 7445, 12424, 12425, 12426, 1177, 12366, 9434, 5339, 12428, 2]
// Exports: default

// Module 12422 (ActivityDetailContent)
...
import DeveloperActivityShelfActionCreatorsAll from "DeveloperActivityShelfActionCreators" /* 11803 */;
import useActivityShelfItem from "useActivityShelfItem" /* 12339 */;
...
function TryItOutButton(botUserId) {
  ...
  const callback = noop.useCallback(asyncGeneratorStep(async (arg0, value) => {
    tmp3(1241).track(constants.APP_DETAIL_PAGE_ENTRY_POINT_COMMAND_BUTTON_CLICKED, { application_id: applicationId, button_action: botUserId(9555).EntryPointCommandButtonActions.OPEN_APP_DM });
    const _setTimeout = setTimeout;
    closure_4.current = setTimeout(() => {
      dependencyMap(true);
    }, 250);
    await tmp3(4801).openPrivateChannel({ recipientIds: botUserId });
    if (1 === tmp7) {
      dependencyMap = 0;
      ...
      if (arg0 === 1) {
        ...
        c4 = 3;
        c5 = 1;
        return { value: tmp3(11802)({ targetApplicationId: closure_129_1, channelId: closure_128_0, analyticsLocations: closure_129_2 }), done: false };
      }
    } else if (arg0 === 1) {
      c5 = 3;
      throw value;
  ...
  return closure_11(botUserId(5218).Button, obj);
}
...
export default function ActivityDetailContent(application) {
  ...
  const tmp = closure_15();
  const width = application(11510).useRequiredAppLauncherContext().width;
  let obj = application(11510);
  const getPrimaryAppCommand = application(9633).useGetPrimaryAppCommand(context, application.id);
  const obj2 = application(9633);
  ...
  if (tmp2Result7.isRealApplication(application)) {
    ...
    if (isDeveloperOfThisApp) {
      ...
      obj5.style = obj4;
      const items1 = [closure_11(tmp2(5851).TableRowDivider, {}), , ];
      ...
      items1[1] = closure_11(tmp2(7477).TableSwitchRow, obj6);
      let tmp22Result = null;
      if (useActivityUrlOverride) {
        const obj7 = { style: tmp.activityUrlOverrideInputContainer, children: null };
        const obj8 = { placeholder: "e.g. http://192.168.1.1:3000", value: activityUrlOverride, onChange: tmp23(11803).setActivityUrlOverride };
        obj7.children = tmp22(tmp2(6880).TextInput, obj8);
    ...
    const getOrFetchApplication = tmp2(7445).useGetOrFetchApplication(application.id);
    let bot;
    if (getOrFetchApplication != null) {
      bot = getOrFetchApplication.bot;
    }
    const tmp2Result8 = tmp2(7445);
    const obj9 = { context, applicationId: application.id };
    const activityAction = tmp2(12339).useActivityAction(obj9);
    const tmp2Result9 = tmp2(12339);
    const delayedSwapToActivityActionLeave = tmp2(12424).useDelayedSwapToActivityActionLeave(activityAction);
    const obj10 = { context, application, activityAction: delayedSwapToActivityActionLeave };
    const tmp2Result10 = tmp2(12424);
    ({ reason, disabled } = tmp5(12425)(obj10));
    ...
    if (null != getPrimaryAppCommand) {
      ...
      if (tmp39Result2) {
        ...
        tmp39Result2 = tmp39(tmp2(4784).Text, obj17);
      }
      ...
      if (isDeveloperOfThisApp) {
        isDeveloperOfThisApp = tmp2(9435).isEmbeddedApp(application);
        const tmp2Result11 = tmp2(9435);
      }
      if (isDeveloperOfThisApp) {
    ...
    let result = width;
    const tmp29 = tmp5(12425)(obj10);
    if (detailsContainerLandscape) {
      result = 65 * width / 100;
    }
    obj25.width = result;
    obj25.contentWidth = tmp8;
    obj24.children = closure_11(tmp5(12366), obj25);
    ...
    const items7 = [closure_11(tmp5(9434), obj27), , , , ];
    ...
    obj29.icon = closure_11(tmp2(5339).GroupIcon, obj30);
    const tmp52 = Tag;
    const tmp5Result3 = tmp5(12366);
    let num5 = num;
    if (num == null) {
      num5 = 0;
    }
    obj29.tagName = tmp2(12428).getItemSubtitleForMaxPlayersShort(num5);
    const tmp2Result12 = tmp2(12428);
    if (num == null) {
      num = 0;
    }
    obj29.accessibilityLabel = tmp5(12428)(num);
