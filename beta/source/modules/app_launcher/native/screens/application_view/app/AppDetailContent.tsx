...
// Dependencies: [5, 19, 17, 9436, 1483, 1074, 5242, 21, 4788, 12412, 576, 12336, 5854, 11510, 12387, 7795, 9634, 1115, 1612, 9562, 1978, 12416, 9435, 7797, 12333, 1364, 12418, 4784, 12420, 12384, 12422, 9434, 1177, 12429, 12431, 4755, 4968, 9351, 2]
// Exports: default

// Module 12411 (AppDetailContent)
...
import CommandRowButtonDefault from "CommandRowButton" /* 12387 */;
...
export default function AppDetailContent(context) {
  ...
  const memo1 = onCommandExecuted.useMemo(() => {
    ...
    if (isAndroidResult) {
      isAndroidResult = tmp(9435).isApplicationMonetizedWithIAP(application);
      const tmpResult = tmp(9435);
    }
    ...
    if (result) {
      ...
      const items = [map1(tmp(12418).BillIcon, { size: "sm", color: "icon-muted" }), ];
      ...
      items[1] = map1(tmp(4784).Text, obj3);
      obj2.children = items;
    ...
    if (isAndroidResult) {
      ...
      const items1 = [map1(tmp(12420).ShopIcon, { size: "sm", color: "icon-muted" }), ];
      ...
      items1[1] = map1(tmp(4784).Text, obj5);
