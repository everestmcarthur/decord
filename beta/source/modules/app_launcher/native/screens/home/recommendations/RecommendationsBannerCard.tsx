...
// Dependencies: [19, 17, 1074, 21, 4788, 576, 9435, 8488, 1397, 12365, 5371, 12374, 1978, 12368, 4784, 1115, 12338, 5861, 2]
// Exports: default

// Module 12373 (RecommendationsBannerCard)
...
export default function RecommendationsBannerCard(application) {
  ...
  const tmp = closure_8();
  if (!showsPromoted) {
    showsPromoted = application(9435).isPromotedApplication(application);
    const obj = application(9435);
  }
  const shelfBadgeTypeIfActive = application(9435).getShelfBadgeTypeIfActive(application);
  ...
  if (isLandscape) {
    ...
    let tmp12Result2 = closure_6(tmp4(12365).BaseAppRow, obj5);
  } else {
    ...
    const tmp10Result = tmp10(12374);
    obj10.applicationEmbedded = tmp4(9435).isEmbeddedApp(application);
    ...
    if (showsPromoted) {
      ...
      const items3 = [tmp14(tmp10(12368), obj12), ];
      if (showsPromoted) {
        ...
        obj13.children = tmp14(tmp4(4784).Text, obj14);
    ...
    if (tmp14Result) {
      ...
      tmp14Result = tmp14(tmp10(12338), obj16);
    }
    ...
    tmp12Result2 = tmp12(tmp4(5371).PressableOpacity, obj7);
