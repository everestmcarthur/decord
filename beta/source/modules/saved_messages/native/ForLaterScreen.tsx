...
// Function ID: 13620
// Name: ForLaterScreen
// Dependencies: [32, 19, 17, 11940, 21, 4788, 576, 4524, 5217, 13620, 8136, 8130, 504, 7439, 7459, 9080, 1249, 1091, 13622, 13628, 9029, 13632, 2]

// Module 13619 (ForLaterScreen)
...
import useSavedMessagesForPageDefault from "useSavedMessagesForPage" /* 13620 */;
import ForLaterMessageCardDefault from "ForLaterMessageCard" /* 13622 */;
...
function ForLaterPage(type) {
  ...
  const tmp5 = type === throttledNow(8136).SavedMessageSortTypes.REMINDER;
  const forLaterLimit = throttledNow(8130).useForLaterLimit(ForLaterScreen, tmp5);
  const obj = throttledNow(8130);
  const isForLaterLimitUpgradable = throttledNow(8130).useIsForLaterLimitUpgradable(ForLaterScreen);
  ...
  const analyticsLocations = useAnalyticsLocationsDefault(tmp2(7459).FOR_LATER_POPOUT).analyticsLocations;
  ...
  if (0 === arr.length) {
    ...
    const obj6 = { type };
    obj5.children = closure_7(tmp2(13628), obj6);
    let tmp24Result = closure_7(tmp4(7439).AnalyticsLocationProvider, obj5);
  } else {
    ...
    obj8.children = closure_7(tmp4(9029).FlashList, obj9);
    const items2 = [closure_7(View, obj8), ];
    let tmp25Result = null;
    if (tmp9) {
      const obj10 = { isReminder: tmp5, isAtLimit: tmp8 };
      tmp25Result = tmp25(tmp2(13632), obj10);
    }
    items2[1] = tmp25Result;
    obj7.children = items2;
    tmp24Result = closure_8(tmp4(7439).AnalyticsLocationProvider, obj7);
    tmp25 = closure_7;
  }
  return tmp24Result;
}
