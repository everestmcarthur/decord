...
// Dependencies: [7816, 11583, 7976, 7967, 4023, 1115, 11584, 11585, 11586, 11587, 11588, 5696, 2]
...
import FractionalPremiumUtils from "FractionalPremiumUtils" /* 11586 */;
import QuestCopyUtils from "QuestCopyUtils" /* 11587 */;
import QuestRewardAssignmentMethods from "QuestRewardAssignmentMethods" /* 11588 */;
...
let items = [fn(11583).QuestRewardExpirationMode.PREMIUM_EXTENSION, fn(11583).QuestRewardExpirationMode.PREMIUM_PERMANENT];
  ...
  if (null != tmp5) {
    if ("expiresAt" in tmp5) {
      if (null != tmp5.expiresAt) {
        ...
        const differenceInDaysResult = tmp3(4023).differenceInDays(date1, date);
        const _Math3 = Math;
        let num = 0;
        ...
        if (sum >= 12) {
          ...
          const differenceInDaysResult1 = tmp3(4023).differenceInDays(date1, date);
          ...
          const tmp3Result2 = tmp3(4023);
        }
        const tmp3Result = tmp3(4023);
      }
    }
  }
  return null;
...
export const isCollectibleQuestRewardPermanentWithPremiumSubscription = function isCollectibleQuestRewardPermanentWithPremiumSubscription(config) {
  ...
  if (hasItem) {
    ...
    let tmp11 = null;
    if (type1 === tmp3(7976).QuestRewardTypes.COLLECTIBLE) {
      tmp11 = found1;
    }
    let expirationMode1;
    if (tmp11 != null) {
    ...
    hasItem = expirationMode1 === tmp3(11583).QuestRewardExpirationMode.PREMIUM_PERMANENT;
  }
  return hasItem;
};
...
export const getPlatformString = function getPlatformString(arg0) {
  ...
  } else if (tmp(5696).QuestRewardCodePlatforms.PLAYSTATION === arg0) {
    const intl4 = tmp(1115).intl;
    return intl4.string(tmp(1115).t["6IeKx2"]);
  } else if (tmp(5696).QuestRewardCodePlatforms.SWITCH === arg0) {
    const intl3 = tmp(1115).intl;
    return intl3.string(tmp(1115).t["1pp0su"]);
  } else if (tmp(5696).QuestRewardCodePlatforms.PC === arg0) {
    const intl2 = tmp(1115).intl;
    return intl2.string(tmp(1115).t["YK+wUg"]);
  } else if (tmp(5696).QuestRewardCodePlatforms.CROSS_PLATFORM === arg0) {
