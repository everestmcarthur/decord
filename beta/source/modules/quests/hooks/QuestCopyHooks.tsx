...
// Function ID: 12058
// Name: QuestCopyHooks
// Dependencies: [5, 32, 19, 2109, 1372, 5693, 1074, 1374, 1115, 11767, 7990, 7992, 11582, 2108, 12058, 1969, 504, 12059, 8978, 8989, 5696, 11587, 1881, 9666, 11797, 11780, 7996, 7995, 2]
...
import useInGameQuestConnectState from "useInGameQuestConnectState" /* 12059 */;
...
const utils_QuestUtils = v1votF6(7990);
const GameProfileAnalyticUtils = tmp2(8989);
const SponsoredQuestUtils = v1votF6(12058);
require = fn;
...
export const useQuestInstructionTitle = function useQuestInstructionTitle(quest, arg1, questContent, sourceQuestContent) {
  ...
  if (tmpResult7.isConsoleQuest(quest)) {
    ...
    if (tmpResult8.hasWatchVideoTasks(quest)) {
      const defaultWatchVideoTask = tmp(7992).getDefaultWatchVideoTask(quest.config);
      let videoTitle;
      if (defaultWatchVideoTask != null) {
      ...
      if (tmpResult10.hasPlayActivityTask(quest)) {
        const result = tmp(7990).isPlayAnyActivityQuest(quest);
        ...
        tmpResult12 = tmp(7992);
      }
      tmpResult10 = tmp(7992);
    }
    tmpResult8 = tmp(7992);
  }
};
...
export const useQuestBarSubtitle = function useQuestBarSubtitle(arg0) {
  ...
  if (tmp8) {
    ...
    if (isExpanded) {
      ...
      if (activeScreen !== tmp(5696).TaskPlatformScreen.SELECT) {
        if (tmpResult9.isSponsoredPlayQuest(quest)) {
          if (tmp10) {
            if (!tmp11) {
              const defaultRewardNameWithArticle = tmp(11582).getDefaultRewardNameWithArticle(quest.config, stateFromStores);
              const intl5 = tmp(1115).intl;
      ...
      if (tmpResult8.hasWatchVideoTasks(quest)) {
        ...
        if (!tmpResult11.hasAchievementInGameTask(quest)) {
          if (isQuestProgressing) {
            ...
            let contextualEntrypointHeading = tmp(11587).getContextualEntrypointHeading(obj10);
            const tmpResult12 = tmp(11587);
          } else {
            const intl2 = tmp(1115).intl;
...
export const useQuestBarTitle = function useQuestBarTitle(questDockQuest, arg1) {
  ...
  if (null != completedAt) {
    ...
    if (_slicedToArray(obj3.useTaskPlatformScreen(questDockQuest, questTaskDetails), 1)[0] === tmp(5696).TaskPlatformScreen.SELECT) {
      const intl2 = tmp(1115).intl;
      ...
      isConsoleQuestResult = tmp(7992).isConsoleQuest(questDockQuest);
    }
    return stringResult;
  }
};
export const usePrimaryCtaCopy = function usePrimaryCtaCopy(arg0) {
      ...
      if (!tmp2Result.hasPlayActivityTask(tmp)) {
        ...
        if (tmp2Result4.hasStreamOnDesktopTask(obj2)) {
          ...
          if (tmp2Result5.hasWatchVideoOnMobileTasks(tmp)) {
            ...
            PLAY = tmp2(7992).isInGameQuest(tmp) ? tmp4.IN_GAME : tmp4.PLAY;
            const tmp2Result6 = tmp2(7992);
          }
          tmp2Result5 = tmp2(7992);
        }
        tmp2Result4 = tmp2(7992);
      }
      return PLAY;
    }
    PLAY = constants.PLAY;
  ...
  if (constants3.PLAY === memo) {
    ...
    return tmp2(11797).getVideoQuestWatchCtaText(tmp5);
...
export const getRewardCodeRedemptionInstructions = function getRewardCodeRedemptionInstructions(arg0) {
  ...
  if (platform == null) {
    platform = tmp(5696).QuestRewardCodePlatforms.CROSS_PLATFORM;
  }
  ...
  if (result) {
    ...
    obj2.idx = tier;
    rewardCodeQuestReward = tmp(11582).getRewardCodeQuestReward(obj2);
    const tmpResult = tmp(11582);
  }
  let prop;
    ...
    if (dependencyMap === 2) {
      ...
      try {
        ...
        if (0 === c1) {
          ...
          const obj6 = { content: closure_128_1, ctaContent: tmp4(7996).QuestContentCTA.OPEN_GAME_LINK, impressionId: null, sourceQuestContent: null };
          let tmp5;
          ...
          tmp4(11780).openGameLinkDirectly(closure_128_0, obj6);
          dependencyMap = 3;
          return { value: "HermesInternal", done: null };
        }
      } catch (tmp11) {
  ...
  const memo = noop.useMemo(() => {
    ...
    if (null != subtitle) {
      ...
      if (obj3.hasAchievementInGameTask(messages)) {
        ...
        const questType = tmp5(7990).getQuestType(messages.config);
        if (questType === tmp5(7995).QuestType.GAMEPLAY) {
          ...
          if (!features.includes(constants2.NON_GAMING_PLAY_QUEST)) {
            ...
            tmp5Result2 = tmp5(7990);
          }
        }
        const intl2 = tmp5(1115).intl;
        taskTitle = intl2.string(tmp5(1115).t.y8Xf3k);
        const tmp5Result = tmp5(7990);
      }
      obj3 = QuestTaskUtils;
    }
  }, items);
  obj.ctaText = quest(11587).getExternalCtaLabel(quest);
