...
// Dependencies: [32, 19, 17, 7971, 5693, 21, 4788, 576, 7977, 11571, 4486, 1115, 5846, 11767, 11780, 5696, 15361, 504, 12060, 7992, 7427, 15391, 15392, 12056, 11770, 7996, 7997, 7998, 8008, 5700, 15430, 15432, 15433, 7990, 8898, 4784, 2]
// Exports: default

// Module 15390 (QuestBottomSheet)
...
import QuestBottomSheetProgressCard from "QuestBottomSheetProgressCard" /* 15430 */;
...
function useEnrolledQuestContentProps(quest) {
  ...
  _slicedToArray = quest(11770).useQuestImpressionId();
  let obj2 = quest(11770);
  const questTaskDetails = quest(11767).useQuestTaskDetails(quest);
  let obj3 = quest(11767);
  const isQuestProgressing = quest(11767).useIsQuestProgressing(quest);
  const obj4 = quest(11767);
  let tmp6 = _slicedToArray(quest(11767).useTaskPlatformScreen(quest, questTaskDetails), 3);
  noop = tmp6[2];
  const obj5 = quest(11767);
  closure_129_0 = quest;
  ...
  const tmpResult = quest(11767);
  const questTaskDetails1 = quest(11767).useQuestTaskDetails(quest);
  const tmpResult7 = quest(11767);
  let isQuestProgressing1 = quest(11767).useIsQuestProgressing(quest);
  const tmpResult8 = quest(11767);
  const tmp5Result = _slicedToArray(quest(11767).useTaskPlatformScreen(quest, questTaskDetails1), 3);
  ...
  if (tmp16) {
    tmp16 = first === tmp(5696).TaskPlatformScreen.CONSOLE;
  }
  if (!tmp16) {
  ...
  tmp21 = 0 !== memo.length || null != completedAt || first !== quest(5696).TaskPlatformScreen.CONSOLE || first1;
  const hasWatchVideoOnMobileTasks1 = quest(15361).useHasWatchVideoOnMobileTasks(quest.config);
  closure_129_14 = hasWatchVideoOnMobileTasks1;
  const tmpResult10 = quest(15361);
  const isMobileActivityQuest = quest(15361).useMobileActivityQuest(quest).isMobileActivityQuest;
  ...
  obj7.onNext = onNext;
  items5[1] = obj7;
  const tmpResult11 = quest(15361);
  closure_130_0 = quest;
  [tmp32, tmp33] = items5;
  const tmp5Result6 = _slicedToArray(items5, 2);
  const questLogger = quest(7977).getQuestLogger({ quest, location: constants.QUEST_HOME_MOBILE });
  ...
  const obj10 = {
    ...
    start: noop.useCallback(() => {
      ...
      result.then((errorHints) => dependencyMap(errorHints.errorHints)).catch((error) => {
        ...
        obj2.icon = sourceQuestContent(5846);
        sourceQuestContent(4486).open(obj2);
      }).finally(() => impressionId(false));
    }, items6)
  };
  const tmp40 = null == completedAt1 && !isQuestProgressing && tmp6[0] === quest(5696).TaskPlatformScreen.CONSOLE && !hasWatchVideoOnMobileTasks;
  return {
    quest,
    defibrillator: obj10,
    step: tmp32,
    stepActions: tmp33,
    showMicrophone: null == completedAt1 && !isQuestProgressing && tmp6[0] === quest(5696).TaskPlatformScreen.CONSOLE && !hasWatchVideoOnMobileTasks,
    handleTaskSelect(arg0) {
      ...
      if (tmp4Result.shouldMigrateToAdAnalyticsInterface(tmp4(7997).AdAnalyticsInterfaceExperimentStep.STEP_2_CLICKED_INTERNAL, "quest_bottom_sheet")) {
        const obj = { type: tmp4(8008).AdUserActionType.CLICK_INTERNAL, adCreativeType: tmp4(5700).AdCreativeType.QUEST, adCreativeId: quest.id, questContentCTA: DESELECT_PLATFORM, surfaceId: tmp4(5696).QuestContent.QUEST_BOTTOM_SHEET, sourceQuestContent, impressionId };
        tmp4(7998).captureAdUserAction(obj);
        const tmp4Result2 = tmp4(7998);
      } else {
        const obj2 = { questId: quest.id, questContent: tmp4(5696).QuestContent.QUEST_BOTTOM_SHEET, questContentCTA: DESELECT_PLATFORM, sourceQuestContent };
        closure_2(obj2);
      }
      closure_4(arg0);
    }
...
function MicrophoneUnit(arg0) {
  ...
  if (num > 0) {
    ...
    const items1 = [closure_1_10(tmp2(8898).WarningIcon, obj4), ];
    ...
    items1[1] = closure_1_10(tmp2(4784).Text, obj6);
