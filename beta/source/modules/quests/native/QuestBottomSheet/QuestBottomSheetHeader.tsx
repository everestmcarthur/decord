...
// Dependencies: [32, 19, 17, 1372, 21, 4788, 576, 11767, 7992, 15361, 504, 11582, 7990, 1115, 15390, 5696, 15372, 15362, 5203, 5212, 4784, 5371, 8213, 2]
// Exports: default

// Module 15391 (QuestBottomSheetHeader)
...
export default function QuestBottomSheetHeader(arg0) {
  ...
  let ref;
  let tmp = closure_9();
  const questCreative = isScreenReaderEnabled(15372).useQuestCreative(quest);
  let obj = isScreenReaderEnabled(15372);
  closure_129_0 = quest;
  closure_129_1 = step;
  ...
  let completedAt;
  let obj2 = isScreenReaderEnabled(15362);
  if (userStatus != null) {
  ...
  const actionSheetPressHandler = isScreenReaderEnabled(15362).useActionSheetPressHandler(questCreative);
  const questTaskDetails = isScreenReaderEnabled(11767).useQuestTaskDetails(quest);
  const tmp2Result = isScreenReaderEnabled(11767);
  const hasWatchVideoTasksResult = isScreenReaderEnabled(7992).hasWatchVideoTasks(quest);
  closure_129_4 = hasWatchVideoTasksResult;
  const tmp2Result9 = isScreenReaderEnabled(7992);
  const isInGameQuestResult = isScreenReaderEnabled(7992).isInGameQuest(quest);
  closure_129_5 = isInGameQuestResult;
  const tmp2Result10 = isScreenReaderEnabled(7992);
  const first = _slicedToArray(isScreenReaderEnabled(11767).useTaskPlatformScreen(quest, questTaskDetails), 1)[0];
  closure_129_6 = first;
  ...
  closure_129_8 = memo;
  const tmp2Result11 = isScreenReaderEnabled(11767);
  const hasWatchVideoOnMobileTasks = isScreenReaderEnabled(15361).useHasWatchVideoOnMobileTasks(quest.config);
  closure_129_9 = hasWatchVideoOnMobileTasks;
  const tmp2Result12 = isScreenReaderEnabled(15361);
  ...
  const defaultRewardNameWithArticle = isScreenReaderEnabled(11582).getDefaultRewardNameWithArticle(quest.config, stateFromStores);
  closure_129_10 = defaultRewardNameWithArticle;
  const tmp2Result14 = isScreenReaderEnabled(11582);
  const isSponsoredPlayQuestResult = isScreenReaderEnabled(7990).isSponsoredPlayQuest(quest);
  ...
  const tmp2Result15 = isScreenReaderEnabled(7990);
  ...
  if (tmp24Result) {
    ...
    obj4.style = items5;
    obj4.children = memo1;
    tmp24Result = closure_7(tmp2(4784).Text, obj4);
  }
  ...
  if (withActionSheet) {
    ...
    obj5.children = closure_7(tmp2(8213).MoreHorizontalIcon, obj6);
    withActionSheet = closure_7(tmp2(5371).PressableOpacity, obj5);
  }
