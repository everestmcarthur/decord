...
// Dependencies: [19, 17, 1372, 21, 4788, 576, 11767, 11780, 504, 11582, 7990, 1115, 4784, 5701, 5856, 15401, 11577, 15393, 12049, 15388, 5371, 8610, 5230, 5836, 15431, 8577, 2]
...
// Module 15430 (QuestBottomSheetProgressCard)
...
export const QuestBottomSheetProgressCardPlayStreamTask = function QuestBottomSheetProgressCardPlayStreamTask(quest) {
  ...
  c8 = undefined;
  const tmp = closure_9();
  questTaskDetails = questTaskDetails(11767).useQuestTaskDetails(quest);
  let obj = questTaskDetails(11767);
  let isQuestProgressing = questTaskDetails(11767).useIsQuestProgressing(quest);
  ...
  noop = tmp9;
  let obj2 = questTaskDetails(11767);
  const result = questTaskDetails(11780).supportedTaskPlatforms(quest);
  c4 = result;
  const tmp2Result = questTaskDetails(11780);
  questFormattedDate = questTaskDetails(11767).useQuestFormattedDate(quest.config.rewardsConfig.rewardsExpireAt);
  gameTitle = quest.config.messages.gameTitle;
  const tmp2Result5 = questTaskDetails(11767);
  ...
  defaultRewardName = questTaskDetails(11582).getDefaultRewardName(quest.config, stateFromStores);
  const tmp2Result7 = questTaskDetails(11582);
  const isSponsoredPlayQuestResult = questTaskDetails(7990).isSponsoredPlayQuest(quest);
  ...
  const memo = noop.useMemo(() => {
    if (dependencyMap) {
      if (!closure_3) {
        ...
        const obj = {
          rewardHook() {
                return defaultRewardName(questTaskDetails(4784).Text, { variant: "text-sm/semibold", color: "text-strong", children });
              },
    ...
    if (isQuestProgressing) {
      ...
      const obj2 = {
        ...
        minutesHook(children) {
            return children(questTaskDetails(4784).Text, { variant: "text-sm/semibold", color: "text-strong", children });
          }
      };
  ...
  if (isQuestProgressing) {
    ...
    obj7.children = tmp18(tmp2(4784).Text, obj8);
...
export const QuestBottomSheetProgressCardWatchTask = function QuestBottomSheetProgressCardWatchTask(quest) {
  ...
  const obj = quest(11767);
  let isHeroVideoSupportedResult = null != memo;
  const obj2 = quest(15393);
  const obj3 = { questId: quest.id, sourceQuestContent: quest.sourceQuestContent };
  if (isHeroVideoSupportedResult) {
    isHeroVideoSupportedResult = tmp2(12049).isHeroVideoSupported(memo.mimetype);
    const tmp2Result = tmp2(12049);
  }
  const userStatus = quest.userStatus;
  let completedAt;
  if (userStatus != null) {
  ...
  const watchTaskPressHandler = quest(15393).useWatchTaskPressHandler({ questId: quest.id, sourceQuestContent: quest.sourceQuestContent });
  ...
  if (isHeroVideoSupportedResult) {
    ...
    obj7.source = obj8;
    const items4 = [tmp14(tmp2(8610).VideoComponent, obj7), ];
  ...
  tmp2Result2 = quest(11767);
  items7[1] = closure_7(quest(8577).PlayIcon, { color: nativeDefault.colors.WHITE });
  ...
  return closure_7(quest(5371).PressableOpacity, obj4);
};
