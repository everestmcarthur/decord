...
// Dependencies: [19, 17, 1074, 21, 4788, 576, 4755, 7656, 10263, 4483, 2108, 5218, 1115, 7256, 1612, 7431, 4498, 4784, 11585, 3516, 7427, 15434, 2]
// Exports: default

// Module 15437 (QuestOrbMultiplierPerkInfoActionSheet)
import nativeDefault from "native" /* 576 */;
import useSafeAreaInsetsDefault from "useSafeAreaInsets" /* 1612 */;
import HelpdeskUtilsDefault from "HelpdeskUtils" /* 2108 */;
import _modDef3516 from "module_3516" /* 3516 */;
...
function Footer(eligibleToReceivePremiumRewards) {
  ...
  if (eligibleToReceivePremiumRewards.eligibleToReceivePremiumRewards) {
    ...
    items[1] = tmp9(tmp10(5218).Button, obj3);
    obj.children = items;
    let tmp11 = obj;
  } else {
    ...
    items1[1] = tmp9(tmp10(5218).Button, obj5);
    obj.children = items1;
    tmp11 = obj;
  }
...
export default function QuestOrbMultiplierPerkInfoActionSheet(multiplier) {
  ...
  const result = multiplier(11585).shouldReceiveQuestOrbMultiplier(orbMultiplierEligibility);
  dependencyMap = result;
  let obj = multiplier(11585);
  ...
  const memo = noop.useMemo(() => {
    if (orbMultiplierEligibility === QuestOrbMultiplierUtils.QuestOrbMultiplierEligibilityType.XBOX_GAME_PASS) {
      ...
      let stringResult = intl2.string(_modDef3516.c5usUr);
    } else {
  ...
  const memo1 = noop.useMemo(() => {
    if (orbMultiplierEligibility === QuestOrbMultiplierUtils.QuestOrbMultiplierEligibilityType.XBOX_GAME_PASS) {
      ...
      let formatResult = intl2.format(_modDef3516.UkrcSH, obj2);
    } else {
      const intl = tmp(1115).intl;
  ...
  let obj3 = { visible: orbMultiplierEligibility === multiplier(11585).QuestOrbMultiplierEligibilityType.NITRO || orbMultiplierEligibility === multiplier(11585).QuestOrbMultiplierEligibilityType.UPSELL, children: null };
  const tmp4 = orbMultiplierEligibility === multiplier(11585).QuestOrbMultiplierEligibilityType.NITRO || orbMultiplierEligibility === multiplier(11585).QuestOrbMultiplierEligibilityType.UPSELL;
  obj3.children = closure_8(SheetContent, { title: memo, body: memo1, eligibleToReceivePremiumRewards: result });
  obj2.children = closure_8(orbMultiplierEligibility(15434), obj3);
  return closure_8(multiplier(7427).BottomSheet, obj2);
};
