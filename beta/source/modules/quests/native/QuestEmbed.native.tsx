...
// Function ID: 13557
// Name: QuestEmbed
// Dependencies: [17, 1182, 7971, 5693, 1074, 8010, 1364, 1115, 7992, 12057, 5696, 4639, 11577, 1609, 13557, 7967, 7986, 7990, 8235, 2]
// Exports: createQuestsEmbed

// Module 13556 (QuestEmbed)
...
export const createQuestsEmbed = function createQuestsEmbed(questId) {
  ...
  if (obj.isMetaQuest()) {
    ...
    obj2.thumbnailUrl = Image.resolveAssetSource(tmp(13557)).uri;
    ...
    const result = tmp2(7967).findQuestOrReplacement(questId, QuestStore.quests, QuestStore.excludedQuests);
    ...
    if (null == result) {
      if (null == value) {
        const result1 = tmp2(7986).trackQuestEmbedFallbackViewed(questId, constants2.UNKNOWN_QUEST);
        const intl12 = tmp2(1115).intl;
        const tmp2Result18 = tmp2(7986);
        ...
        obj8.thumbnailUrl = Image.resolveAssetSource(tmp(13557)).uri;
      ...
      const result2 = tmp2(7986).trackQuestEmbedFallbackViewed(questId, constants2.EXCLUDED_QUEST);
      const intl7 = tmp2(1115).intl;
      const tmp2Result20 = tmp2(7986);
      ...
      obj10.thumbnailUrl = Image.resolveAssetSource(tmp(13557)).uri;
      ...
      if (tmp2Result22.isShareableQuest(result.config)) {
        ...
        const tmp2Result23 = tmp2(4639);
        const intl3 = tmp2(1115).intl;
        const obj12 = { questName: result.config.messages.questName };
        const gamePublisher = result.config.messages.gamePublisher;
        const _Date2 = Date;
        const tmp29 = tmp2(4639).isThemeDark(ThemeStore.getState().theme) ? ThemeTypes.DARK : ThemeTypes.LIGHT;
        ...
        if (result.config.expiresAt < date1.toISOString()) {
          ...
          const questTaskDetails = tmp2(7992).getQuestTaskDetails(result);
          const tmp2Result24 = tmp2(7992);
          const thirdPartyTaskDetails = tmp2(7992).getThirdPartyTaskDetails(result);
          const tmp2Result25 = tmp2(7992);
          const obj15 = { quest: result, taskDetails: questTaskDetails, location: constants.EMBED_MOBILE, sourceQuestContent: tmp2(5696).QuestContent.QUEST_EMBED_MOBILE, thirdPartyTaskDetails, withoutMarkdown: true, currentUser };
          formatToPlainStringResult1 = tmp2(12057).getQuestsInstructionsToWinReward(obj15);
          const tmp2Result26 = tmp2(12057);
        }
        ...
        obj16.thumbnailUrl = tmp2(11577).getQuestAsset(result, tmp2(11577).QuestAssetType.GAME_TILE, tmp29).url;
        obj16.embedCanBeTapped = true;
        obj16.canBeAccepted = true;
        obj16.type = InviteTypes.GUILD;
        const tmp2Result27 = tmp2(11577);
        const obj17 = {};
        ...
        const result3 = tmp2(7986).trackQuestEmbedFallbackViewed(questId, constants2.NOT_SHAREABLE_QUEST);
        const intl = tmp2(1115).intl;
        const tmp2Result29 = tmp2(7986);
        ...
        obj20.thumbnailUrl = Image.resolveAssetSource(tmp(13557)).uri;
      ...
      tmp2Result22 = tmp2(7990);
    }
    const tmp2Result16 = tmp2(7967);
  }
};
