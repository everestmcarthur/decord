...
// Function ID: 15363
// Name: QuestDockHooks
// Dependencies: [32, 19, 15363, 5693, 15365, 504, 15366, 15369, 11645, 15367, 15370, 4524, 9696, 15364, 1091, 11770, 15372, 7997, 7998, 8008, 7996, 5696, 5700, 7986, 4755, 15373, 1980, 1478, 11577, 8148, 4489, 576, 2]
  ...
  const fn2 = function t(safeAreaState, restingQuestDockMode) {
    if (!obj.cheapWorkletShallowEqual(safeAreaState, restingQuestDockMode)) {
      ...
      if (constants.RESET_TO_PREVIOUS === restingQuestDockMode) {
        ...
        tmp(4524).runOnJS(setRestingQuestDockMode)(restingQuestDockMode1);
      } else if (tmp5.COLLAPSED === restingQuestDockMode) {
        const obj2 = {};
        const merged = Object.assign(questDockWrapperSpecs.get());
        obj2.x = 0;
        obj2.y = 0;
        obj2.width = tmp(15364).getQuestDockCollapsedWidth(windowWidth, youBarHorizontalMargin, youBarHorizontalMargin);
        obj2.height = height;
        const result = questDockWrapperSpecs.set(obj2);
        const result1 = activeQuestDockMode.set(tmp5.COLLAPSED);
        const tmpResult6 = tmp(15364);
      } else {
        if (tmp5.CLOSED !== restingQuestDockMode) {
          if (tmp5.SOFT_DISMISSED !== restingQuestDockMode) {
            if (tmp5.EXPANDED === restingQuestDockMode) {
              const questDockExpandedHeightLimits = tmp(15364).getQuestDockExpandedHeightLimits(tmp4, safeArea.top, minExpandedContentHeight);
              ...
              if (!tmp7) {
                ...
                if (tmp6) {
                  tmp6 = obj11.get().height === tmp(15364).getQuestDockExpandedHeightLimits(restingQuestDockMode.windowHeight, restingQuestDockMode.safeArea.top, minExpandedContentHeight).maxHeight;
                  const tmpResult8 = tmp(15364);
                }
                tmp7 = tmp6;
              }
              if (!tmp7) {
              ...
              obj3.y = tmp10;
              const tmpResult7 = tmp(15364);
              obj3.width = tmp(15364).getQuestDockExpandedWidth(windowWidth, safeArea.left, safeArea.right);
        ...
        obj4.x = 0;
        obj4.y = 0;
        obj4.width = tmp(15364).getQuestDockClosedWidth(windowWidth, youBarHorizontalMargin, youBarHorizontalMargin);
        obj4.height = height;
        const result4 = questDockWrapperSpecs.set(obj4);
        const result5 = activeQuestDockMode.set(restingQuestDockMode);
        const tmpResult10 = tmp(15364);
      }
    }
  };
...
export const useQuestDockDismissalReset = function useQuestDockDismissalReset() {
  setRestingQuestDockMode = noop.useContext(setRestingQuestDockMode(15369).QuestDockExternalCoordinationContext).setRestingQuestDockMode;
  const activeQuestDockMode = noop.useContext(setRestingQuestDockMode(15366).QuestDockGestureContext).activeQuestDockMode;
  ...
  const effect = noop.useEffect(() => {
    ...
    if (!isSoftDismissedResult) {
      isSoftDismissedResult = setRestingQuestDockMode(15364).isSoftDismissed(QuestDockStore.questDockSoftDismissedAt);
      let obj = setRestingQuestDockMode(15364);
    }
    if (!isSoftDismissedResult) {
    ...
    function maybeResetSoftDismissal() {
      ...
      if (!isSoftDismissedResult) {
        isSoftDismissedResult = setRestingQuestDockMode(15364).isSoftDismissed(QuestDockStore.questDockSoftDismissedAt);
        const obj = setRestingQuestDockMode(15364);
      }
      if (!isSoftDismissedResult) {
...
export const useActionSheetPressHandler = function useActionSheetPressHandler(questCreative) {
  ...
  return noop.useCallback(() => {
    ...
    if (obj2.shouldMigrateToAdAnalyticsInterface(AdAnalyticsInterfaceExperiment.AdAnalyticsInterfaceExperimentStep.STEP_2_CLICKED_INTERNAL, "quest_dock_action_sheet")) {
      ...
      obj3.questContentCTA = tmp(7996).QuestContentCTA.OPEN_CONTEXT_MENU;
      obj3.surfaceId = tmp(5696).QuestContent.QUEST_BAR_MOBILE;
      obj3.sourceQuestContent = tmp(5696).QuestContent.QUEST_BAR_MOBILE;
      ...
      const obj4 = { questId: creativeAnalyticsParams.adCreativeId, questContent: tmp(5696).QuestContent.QUEST_BAR_MOBILE, questContentCTA: tmp(7996).QuestContentCTA.OPEN_CONTEXT_MENU, sourceQuestContent: tmp(5696).QuestContent.QUEST_BAR_MOBILE, impressionId: questImpressionId };
      ...
      obj5.questContentCTA = tmp(7996).QuestContentCTA.OPEN_CONTEXT_MENU;
    ...
    ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(15373, tmp2.paths), "QuestDockContextMenuActionSheet", { creative: tmp3, impressionId: tmp7 });
  }, items);
};
