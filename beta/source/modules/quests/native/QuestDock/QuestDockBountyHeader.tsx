...
// Dependencies: [19, 17, 5693, 15365, 21, 576, 4788, 15372, 15366, 4524, 5217, 15362, 15462, 5836, 7350, 4784, 2]

// Module 15473 (QuestDockBountyHeader)
import nativeDefault from "native" /* 576 */;
import spring from "spring" /* 5217 */;
import ReanimatedNativeViewDefault from "ReanimatedNativeView" /* 7350 */;
import QuestDockBackgroundBlurHeaderDefault from "QuestDockBackgroundBlurHeader" /* 15462 */;
import noop from "module_19" /* 19 */;

require = fn;
const View = fn(17).View;
const QuestDockMode = fn(5693).QuestDockMode;
const QUEST_DOCK_MODE_CHANGE_PHYSICS_CLAMPED = fn(15365).QUEST_DOCK_MODE_CHANGE_PHYSICS_CLAMPED;
const jsxProd = fn(21);
...
export default noop.memo(function QuestDockBountyHeader() {
  const questDockBounty = activeQuestDockMode(15372).useQuestDockBounty();
  const tmp4 = closure_9();
  let str = questDockBounty.productName;
  if (str == null) {
    str = "";
  }
  activeQuestDockMode = noop.useContext(tmp(15366).QuestDockGestureContext).activeQuestDockMode;
  let obj = activeQuestDockMode(15372);
  const fn = function n() {
    let num = 1;
  ...
  const tmpResult = activeQuestDockMode(4524);
  ...
  const obj2 = { withSpring: activeQuestDockMode(5217).withSpring, activeQuestDockMode, QuestDockMode, QUEST_DOCK_MODE_CHANGE_PHYSICS_CLAMPED };
  const bountyCreative = activeQuestDockMode(15372).useBountyCreative(questDockBounty);
  const tmpResult3 = activeQuestDockMode(15372);
  const tmpResult4 = activeQuestDockMode(15362);
  const obj3 = { onSubmenuPress: activeQuestDockMode(15362).useActionSheetPressHandler(bountyCreative), hideBlurWhenCollapsed: true, children: null };
  ...
  const actionSheetPressHandler = activeQuestDockMode(15362).useActionSheetPressHandler(bountyCreative);
  if (tmp8Result) {
    ...
    obj5.source = obj6;
    tmp8Result = tmp8(tmp9(5836), obj5);
  }
  ...
  obj8.children = closure_7(activeQuestDockMode(4784).Text, { variant: "text-md/medium", color: "text-strong", lineClamp: 1, accessible: false, children: str });
