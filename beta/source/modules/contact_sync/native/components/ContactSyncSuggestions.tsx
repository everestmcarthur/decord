...
// Dependencies: [32, 19, 17, 1074, 12981, 21, 4788, 576, 5931, 5225, 4784, 4632, 1397, 5853, 1177, 1115, 4489, 4637, 11, 8903, 9029, 5230, 1094, 5218, 1241, 2]
// Exports: default

// Module 12980 (ContactSyncSuggestions)
...
function SuggestionRow(suggestion) {
  ...
  if (tmp8Result) {
    ...
    tmp8Result = tmp8(tmp3(4784).Text, obj5);
  }
  items[1] = tmp8Result;
...
let obj2 = { container: { flex: 1, backgroundColor: nativeDefault.colors.BACKGROUND_BASE_LOWER, paddingTop: fn(5931).NAV_BAR_HEIGHT + 32, justifyContent: "center" }, header: null, title: null, subtitle: null, list: null, divider: null, linearGradient: null, redesignButton: null, sectionHeader: null };
let obj3 = { flex: 1, backgroundColor: nativeDefault.colors.BACKGROUND_BASE_LOWER, paddingTop: fn(5931).NAV_BAR_HEIGHT + 32, justifyContent: "center" };
...
export default function ContactSyncSuggestions(friendSuggestions) {
  ...
  const token = friendSuggestions(4489).useToken(onSubmit(576).colors.BACKGROUND_BASE_LOW);
  let obj = friendSuggestions(4489);
  let items = [friendSuggestions(4637).hexOpacityToRgba(token, 0), ];
  let obj2 = friendSuggestions(4637);
  items[1] = friendSuggestions(4637).hexOpacityToRgba(token, 100);
  const tmp4 = first(noop.useState(reduced), 2);
  first = tmp4[0];
  noop = tmp4[1];
  let obj3 = friendSuggestions(4637);
  const fontScale = friendSuggestions(5225).useFontScale();
  ...
  let obj4 = friendSuggestions(5225);
  ...
  const items5 = [closure_8(friendSuggestions(9029).FlashList, obj7), , ];
  ...
  obj9.colors = items;
  items5[1] = closure_8(onSubmit(5230), obj9);
  ...
  obj10.children = closure_8(friendSuggestions(5218).Button, obj11);
