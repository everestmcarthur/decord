...
// Function ID: 10473
// Name: Sticker
// Dependencies: [19, 17, 1182, 21, 5517, 5135, 1115, 8289, 10473, 5836, 4639, 7408, 7409, 2]
// Exports: default, getStickerAssetUrl

// Module 10472 (Sticker)
import util from "util" /* 1115 */;
import StickersTypes from "StickersTypes" /* 5517 */;
import FastImageDefault from "FastImage" /* 5836 */;
import NativeLottieViewDefault from "NativeLottieView" /* 8289 */;
import NativeAPNGViewDefault from "NativeAPNGView" /* 10473 */;
...
export default function Sticker(opaque) {
  ...
  if (sticker.format_type === StickersTypes.StickerFormat.LOTTIE) {
    let str = tmp(5135).getStickerAssetUrl(sticker);
    const tmpResult = tmp(5135);
  } else if (sticker.format_type === tmp(5517).StickerFormat.APNG) {
    const obj = { isPreview: !animated, size };
    str = tmp(5135).getStickerAssetUrl(sticker, obj);
    const tmpResult6 = tmp(5135);
  } else {
    const obj2 = { isPreview: !animated, size: PixelRatio.getPixelSizeForLayoutSize(size) };
    str = tmp(5135).getStickerAssetUrl(sticker, obj2);
    const tmpResult7 = tmp(5135);
  }
  if (str == null) {
    str = "";
  }
  ...
  if (sticker.format_type === StickersTypes.StickerFormat.LOTTIE) {
    ...
    if (sticker.format_type === tmp(5517).StickerFormat.APNG) {
      if ("type" in sticker) {
        const tmpResult8 = tmp(5135);
      }
      ...
      obj4.name = "" + sticker.id + "." + tmp(5135).getStickerExtensionFromFormatType(sticker.format_type);
      obj4.accessibilityLabel = formatToPlainStringResult;
      const obj5 = {};
      const tmpResult9 = tmp(5135);
    ...
    if (tmpResult10.isThemeDark(ThemeStore.theme)) {
      let tmp13Result = tmp13(7408);
    } else {
      tmp13Result = tmp13(7409);
    }
