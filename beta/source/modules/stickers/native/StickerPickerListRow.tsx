...
// Dependencies: [19, 17, 1372, 2022, 10570, 1218, 21, 4788, 576, 672, 5345, 1364, 2019, 9467, 4756, 4757, 5135, 7611, 5517, 10472, 10697, 1231, 5371, 2]
// Exports: default

// Module 10696 (StickerPickerListRow)
...
const StickerSendability = tmp(7611);
require = fn;
...
export default function StickerPickerListRow(stickers) {
  ...
  closure_9 = stickers(9467).useMobileStickerPickerUpsellRestyleEnabled("native.StickerPickerListRow");
  let items = [];
  if (nativeRow) {
    let num4 = 0;
    if (0 < rowSize) {
      do {
        ...
        if (undefined !== tmp12) {
          ...
          obj4.stickerType = stickers(5517).StickerFormat.PNG;
      ...
      do {
        let tmp8 = (function _loop() {
          ...
          if (undefined === stickers[c12]) {
            ...
            let isSendableStickerResult = isSectionNitroLocked;
            const obj5 = stickers(5135);
            if (!isSectionNitroLocked) {
              isSendableStickerResult = null == closure_6;
            }
            if (!isSendableStickerResult) {
              isSendableStickerResult = tmp26(7611).isSendableSticker(tmp2, currentUser.getCurrentUser(), closure_6);
              const tmp26Result = tmp26(7611);
            }
            let tmp11 = closure_9;
            if (closure_9) {
            ...
            obj2.onPress = function onPress() {
              const result = closure_0(4756).triggerHapticFeedback(isSectionNitroLocked(4757).IMPACT_LIGHT);
              if (dependencyMap != null) {
            ...
            const obj3 = { sticker: tmp2, size, animated: stickers(5135).shouldAnimateSticker(closure_8, tmp2.id === id), opaque: isSendableStickerResult };
            const items1 = [closure_8(isSectionNitroLocked(10472), obj3, tmp), ];
            let tmp17 = null;
            if (tmp11) {
            ...
            items.push(closure_9(stickers(5371).PressableOpacity, obj2, tmp2.id));
          }
        })();
        sum = num2 + 1;
        c12 = sum;
