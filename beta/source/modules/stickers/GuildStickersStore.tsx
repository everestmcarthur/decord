...
// Dependencies: [32, 2057, 2064, 2063, 5517, 4441, 2067, 2]

// Module 5752 (GuildStickersStore)
...
function deriveStickerMetadata(arg0, tags) {
  const items = [];
  const obj = { type: items(5517).StickerMetadataTypes.STICKER_NAME, value: null };
  const trimmed = tags.name.trim();
  obj.value = trimmed.toLocaleLowerCase();
  items.push(obj);
  if (null != tags.tags) {
    const obj2 = { type: tmp(5517).StickerMetadataTypes.TAG, value: null };
    ...
    if (null != guild) {
      ...
      if (tmp5) {
        const obj3 = { type: tmp(5517).StickerMetadataTypes.GUILD_NAME, value: toLocaleLowerCaseResult };
        items.push(obj3);
      }
    }
    const byName = UnicodeEmojisDefault.getByName(str);
    if (null != byName) {
      const obj4 = { type: tmp(5517).StickerMetadataTypes.CORRELATED_EMOJI, value: byName.surrogates };
      items.push(obj4);
