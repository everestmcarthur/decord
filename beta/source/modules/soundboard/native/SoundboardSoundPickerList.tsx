...
// Dependencies: [19, 17, 1372, 17530, 21, 4788, 576, 5264, 1115, 10639, 504, 4446, 10262, 10601, 17537, 5225, 12, 5833, 1177, 17545, 10686, 4750, 9023, 10600, 7349, 4784, 2]

// Module 17536 (SoundboardSoundPickerList)
...
function getSectionLabel(category) {
  ...
  } else if (tmp(5264).SoundboardSoundGridSectionType.DEFAULTS === type) {
    const intl4 = tmp(1115).intl;
    return intl4.string(tmp(1115).t.Rtvk9X);
  } else if (tmp(5264).SoundboardSoundGridSectionType.FAVORITES === type) {
    const intl3 = tmp(1115).intl;
    return intl3.string(tmp(1115).t.y3LQCG);
  } else if (tmp(5264).SoundboardSoundGridSectionType.FREQUENTLY_USED === type) {
    const intl2 = tmp(1115).intl;
    return intl2.string(tmp(1115).t["+cGVV6"]);
  } else if (tmp(5264).SoundboardSoundGridSectionType.SEARCH === type) {
    return null;
  } else if (tmp(5264).SoundboardSoundGridSectionType.TOP_SOUNDS === type) {
    ...
    const items2 = [
      ...
      section.soundsByRow[row].map((type, index) => {
          ...
          if (SoundboardTypes.SoundboardSoundItemType.SOUND === type) {
            ...
            return React5(tmp(17537).SoundButton, obj, "" + section.category.key + "-" + sound.soundId);
          } else if (tmp(5264).SoundboardSoundItemType.ADD_SOUND === type) {
            const _Error = Error;
...
const SoundboardStyleConstants = fn(17530);
  ...
  return closure_7(FastListDefault, {
    ...
    renderSection(arg0) {
      ...
      if (SoundboardTypes.SoundboardSoundGridSectionType.GUILD === type) {
        ...
        const obj4 = { source: _modDef17545, style: tmp11.sectionIcon };
        tmp8Result = tmp8(native.Icon, obj4);
      } else if (SoundboardTypes.SoundboardSoundGridSectionType.FAVORITES === type) {
        const obj5 = { source: _modDef10686, style: tmp11.sectionIcon };
