...
// Dependencies: [19, 17, 1372, 1074, 2038, 21, 4508, 4639, 4722, 7256, 504, 4446, 1115, 2872, 7656, 7315, 4608, 2027, 7427, 7400, 7431, 5941, 1364, 5836, 17409, 9121, 4784, 5218, 4788, 576, 2]
// Exports: default

// Module 17408 (DisplayNameStylesFlywheelMobileActionSheet)
...
export default function DisplayNameStylesFlywheelMobileActionSheet(markAsDismissed) {
  ...
  const enabled = noop.useContext(markAsDismissed(4508).AccessibilityPreferencesContext).reducedMotion.enabled;
  const isThemeDarkResult = markAsDismissed(4639).isThemeDark(ref(4722)());
  const tmp6 = closure_11();
  const obj2 = markAsDismissed(4639);
  const typeConsolidationTextTransform = markAsDismissed(7256).useTypeConsolidationTextTransform("DisplayNameStylesFlywheel");
  const obj3 = markAsDismissed(7256);
  ...
  let result = ref(4446).canUsePremiumProfileCustomization(stateFromStores);
  ...
  const callback = obj.useCallback(() => {
    openUserSettings.openUserSettings({ screen: c2 ? timestampProducer.PROFILE_CUSTOMIZATION : timestampProducer.PROFILE_CUSTOMIZATION_TRY_IT_OUT }, () => {
      markAsDismissed(7315).runAfterInteractions(() => {
        markAsDismissed(7656).openUserSettings({ screen: constants.DISPLAY_NAME_STYLES }, () => {
          closure_1_0(constants.TAKE_ACTION);
          const result = closure_2_0(4608).UNSAFE_markDismissibleContentAsDismissed(closure_2_0(2027).DismissibleContent.DISPLAY_NAME_STYLES_FLYWHEEL_MOBILE_PROFILE_COACHMARK, { dismissAction: constants.INDIRECT_ACTION });
        });
      });
    });
  }, items1);
  ...
  const items4 = [
    closure_9(markAsDismissed(7431).ActionSheetHeaderBar, {
      onPress() {
  ...
  if (enabled) {
    tmp15Result = tmp15(tmp2(5941).DisplayNameStylesV2AbstractUI, { resizeMode: "contain" });
  }
  ...
  if (enabled) {
    ...
    items4[2] = tmp15(tmp2(4784).Text, obj10);
    ...
    items4[3] = tmp15(tmp2(4784).Text, obj11);
    ...
    const items7 = [tmp15(tmp2(5218).Button, obj14), ];
    ...
    items7[1] = tmp15(tmp2(5218).Button, obj15);
    obj13.children = items7;
    items4[4] = tmp16(tmp17, obj13);
    obj7.children = items4;
    obj12.children = tmp16(tmp17, obj7);
    obj6.children = tmp15(tmp2(7400).SafeAreaPaddingView, obj12);
    return tmp15(tmp2(7427).BottomSheet, obj6);
  } else {
    if (tmp2Result.isIOS()) {
      const obj16 = { source: null, style: null, resizeMode: "contain", enableAnimation: null };
      const obj17 = { uri: tmp4(17409) };
      obj16.source = obj17;
      obj16.style = tmp6.image;
      obj16.enableAnimation = !enabled;
      let tmp15Result2 = tmp15(tmp4(5836), obj16);
      const tmp4Result = tmp4(5836);
    } else {
      const obj18 = { url: tmp4(17409), style: tmp6.image, autoplay: true };
      tmp15Result2 = tmp15(tmp2(9121).APNGPlayer, obj18);
    }
