...
// Dependencies: [19, 2041, 2096, 2095, 1074, 21, 14260, 4696, 504, 10118, 10115, 5682, 8211, 1115, 9521, 576, 4968, 4755, 5683, 10327, 8239, 7396, 7655, 9887, 2]
// Exports: default

// Module 14272 (GuildActionSheetTabItems)
...
export default function GuildActionSheetTabItems(guild) {
  guild = guild.guild;
  let stateFromStores;
  let canAccessSettings = guild(14260).useGuildActionSheetPermissions(guild).canAccessSettings;
  const total = stateFromStores(4696)(guild.id).total;
  let obj = guild(14260);
  const items = [GuildChannelStore];
  stateFromStores = guild(504).useStateFromStores(items, () => GuildChannelStore.getChannels(guild.id));
  let obj2 = guild(504);
  let shouldRenderInviteResult = guild(10118).shouldRenderInvite(stateFromStores, guild);
  ...
  let obj3 = guild(10118);
  const tmp6 = closure_13;
  obj6.icon = closure_12(guild(9521).BoostGemIcon, { color: stateFromStores(576).unsafe_rawColors.GUILD_BOOSTING_PINK });
  ...
  const items2 = [closure_12(guild(8211).IconButton, obj6), , , ];
  if (shouldRenderInviteResult) {
    ...
    obj8.icon = tmp3(10327);
    obj8.onPress = function onPress() {
      ActionSheetActionCreatorsDefault.hideActionSheet();
      closure_2();
    };
    shouldRenderInviteResult = tmp7(tmp(8211).IconButton, obj8);
  }
  ...
  obj9.icon = stateFromStores(8239);
  obj9.onPress = function onPress() {
    ActionSheetActionCreatorsDefault.hideActionSheet();
    NotificationSettingsModalActionCreatorsDefault.open(guild.id);
  };
  items2[2] = closure_12(guild(8211).IconButton, obj9);
  if (canAccessSettings) {
    ...
    obj10.icon = tmp3(7655);
    obj10.onPress = function onPress() {
      ActionSheetActionCreatorsDefault.hideActionSheet();
      GuildSettingsActionCreatorsDefault.open(guild.id);
    };
    canAccessSettings = tmp7(tmp(8211).IconButton, obj10);
  }
  items2[3] = canAccessSettings;
  obj4.children = items2;
  return tmp6(guild(5682).ButtonGroup, obj4);
};
