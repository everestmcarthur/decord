...
// Dependencies: [2040, 1235, 7800, 2046, 2045, 4810, 502, 2041, 2063, 4427, 4437, 2095, 4969, 4807, 1074, 14003, 1095, 4417, 14004, 9782, 11, 5665, 9632, 504, 558, 573, 2]

// Module 14002 (GuildMediaStateStore)
...
function computeGuildMediaState(guildId) {
  ...
  let tmp2 = (function getSharedState() {
    ...
    obj = { skipMutedVcs: guildId(14003).getIsDontBadgeMutedVcsEnabled("GuildMediaStateStore"), currentUserId: id.getId(), selectedVoiceChannelId: voiceChannelId, selectedVoiceGuildId: null, selectedVoiceChannelHasVideo: null, isSelectedVoiceChannelStage: null, blockedOrIgnoredUserIds: null, streamChannelIdsByGuild: null };
    let guild_id;
    if (channel != null) {
  ...
  const found = embeddedActivitiesForGuild.filter((location) => {
    ...
    if (tmp5) {
      ...
      if (!tmp7) {
        ...
        tmp7 = !tmp(14004).hasBlockedOrIgnoredUserIds(items, tmp6.blockedOrIgnoredUserIds);
        const tmpResult = tmp(14004);
      }
      tmp5 = tmp7;
    }
    return tmp5;
    ...
    for (const item10047 of usersWithVideo) {
      ...
      if (!blockedOrIgnoredUserIds.has(item10047)) {
        ...
        let someResult1 = keys1.some((item) => {
          ...
          if (tmp2) {
            tmp2 = closure_1(5665)(basicChannel, PermissionStore);
          }
          return tmp2;
        });
        ...
        if (tmp34Result.isActivitiesInTextEnabled(ChannelStore.getChannel(embeddedActivityLocationChannelId))) {
          ...
          someResult2 = found.some((location) => {
            const channel = ChannelStore.getChannel(guildId(4417).getEmbeddedActivityLocationChannelId(location.location));
