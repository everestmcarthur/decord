...
// Dependencies: [7867, 4437, 1372, 1074, 8571, 11824, 1097, 8568, 4483, 5019, 8562, 4941, 5271, 2]
// Exports: handleMessagesTapImage

// Module 11826 (handleMessagesTapImage)
...
export const handleMessagesTapImage = function handleMessagesTapImage(tapImageData) {
  ...
  if (null != portal) {
    embedId(8571).markPortalAlive(portal);
    const obj = embedId(8571);
  }
  if (true === allowWithinModal) {
    ...
    if (type1 !== embedId(1097).MessageReferenceTypes.FORWARD) {
      ...
      const found = attachments.filter((item) => !embedId(8568).isThumbnailAttachment(item));
      if ("attachment" === type) {
        if (index < found.length) {
          const tmp10Result = tmp10(8568);
      ...
      if ("embed" === type) {
        if (null != embedIndex) {
          ...
          if (importDefault < result.length) {
            tmp10(8568).setMediaSourcePortal(result[importDefault], portal);
            found2 = result;
            tmp25 = tmp10;
            const tmp10Result8 = tmp10(8568);
          }
        }
        if (-1 !== importDefault) {
          ...
          let channelName;
          if (showContextName) {
            const tmp25Result3 = tmp25(4941);
            channelName = tmp25Result3.computeChannelName(messageChannel, UserStore, RelationshipStore, false);
          }
          obj3.contextName = channelName;
          let channelIcon;
          if (showContextName) {
            channelIcon = tmp25(5271).getChannelIcon(messageChannel);
            const tmp25Result4 = tmp25(5271);
          }
          obj3.contextIcon = channelIcon;
          tmp25(8562).openMediaModal(obj3);
          const tmp25Result = tmp25(8562);
        }
      }
      if ("component" === type) {
        if (null == componentId) {
          ...
          if (null != embedId) {
            ...
            if (null != components) {
              if (0 !== components.length) {
                const tmp10Result9 = tmp10(8568);
                const result1 = tmp10Result9.extractMediaSourcesFromComponent(tmp7, components, messageChannel.guild_id, tmp10(5019).asComponentId(componentId), componentMediaIndex);
                if (null != result1) {
                  ({ sources, initialIndex: closure_1 } = result1);
                  tmp10(8568).setMediaSourcePortal(sources[importDefault], portal);
                  found2 = sources;
                  tmp25 = tmp10;
                  const tmp10Result11 = tmp10(8568);
                }
                const tmp10Result10 = tmp10(5019);
              }
            }
          }
        ...
        if (0 < result2.length) {
          ...
          if (flattenSourceResult.noCarousel) {
            ...
            importDefault = 0;
            tmp20(8568).setMediaSourcePortal(items[0], portal);
            found2 = items;
            tmp25 = tmp20;
            const tmp20Result = tmp20(8568);
          } else {
            importDefault = num2;
            tmp20(8568).setMediaSourcePortal(result2[num2], portal);
            ...
            tmp25 = tmp20;
            const tmp20Result2 = tmp20(8568);
          }
        }
        const tmp10Result12 = tmp10(8568);
      }
    } else {
