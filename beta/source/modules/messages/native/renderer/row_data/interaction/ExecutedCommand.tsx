...
// Dependencies: [17, 1386, 2041, 1372, 1074, 1400, 1397, 5020, 11899, 576, 7795, 1978, 8251, 8253, 9632, 1115, 9435, 2]
// Exports: createExecutedCommand

// Module 13521 (ExecutedCommand)
...
export const createExecutedCommand = function createExecutedCommand(message, channel, roleStyle, forcedTheme, defaultUsernameColor) {
  ...
  if (null != message.interaction) {
    ...
    if (null == user2) {
      ...
      if (result1) {
        processColorStringsResult = tmp16(8251).processColorStrings(userAuthor.colorStrings);
        const tmp16Result13 = tmp16(8251);
      }
      let processColorStringsResult1 = null;
      if (result2) {
        processColorStringsResult1 = tmp16(8251).processColorStrings(userAuthor1.colorStrings);
        const tmp16Result14 = tmp16(8251);
      }
      ...
      if (null != tmp24) {
        ...
        const displayNameFontIdForMobileUser1 = tmp16(8253).getDisplayNameFontIdForMobileUser(user3, guildId);
        const tmp16Result16 = tmp16(8253);
      }
      ...
      if (tmp) {
        if (!result) {
          const result3 = tmp16(9632).isActivitiesInTextEnabled(channel);
          ...
          const tmp16Result18 = tmp16(9632);
        }
      ...
      if (result) {
        result4 = tmp16(9435).formatPrimaryEntryPointCommandName(displayName);
        const tmp16Result19 = tmp16(9435);
      }
      ...
      if (null != tmp24) {
        ...
        obj17.targetUsername = tmp16(5020).getUserAuthor(tmp24, channel).nick;
        let id3;
        if (tmp24 != null) {
          id3 = tmp24.id;
        }
        ...
        const tmp16Result20 = tmp16(5020);
      }
