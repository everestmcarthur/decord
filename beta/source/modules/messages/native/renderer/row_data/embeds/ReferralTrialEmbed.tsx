...
// Dependencies: [2041, 1372, 4452, 7724, 7514, 1074, 1374, 4788, 576, 7517, 8236, 8358, 1115, 8359, 4632, 4446, 7723, 8346, 2108, 1609, 8342, 2]
// Exports: createReferralTrialEmbedRedeemable

// Module 8357 (ReferralTrialEmbed)
...
export const createReferralTrialEmbedRedeemable = function createReferralTrialEmbedRedeemable(message, theme, id, relevantUserTrialOffer) {
  if (null != message.author) {
    ...
    if (null != channel) {
      if (channel.isDM()) {
        const obj = { backgroundColor, borderColor: backgroundColor, thumbnailCornerRadius: 3, headerLogoUrl: tmp44(8236).getAssetUriForEmbed(tmp46(8358)), headerText: null, headerColor: null, thumbnailUrl: null };
        const intl = tmp44(1115).intl;
        const tmp44Result = tmp44(8236);
        ...
        obj.thumbnailUrl = tmp44(8236).getAssetUriForEmbed(tmp46(8359));
        let userId;
        ...
        if (null != user) {
          if (null != user1) {
            const name = tmp46(4632).getName(user1);
            const tmp46Result = tmp46(4632);
            const name1 = tmp46(4632).getName(user);
            ...
            if (null == relevantUserTrialOffer) {
              ...
              const values = Object.values(tmp44(7517).TrialIdToProductOfferId[closure_9]);
              id = undefined;
              ...
              const isPremiumResult = tmp46(4446).isPremium(user);
              ...
              const tmp46Result7 = tmp46(4446);
              const result = tmp44(7723).hasUserTrialOfferExpired(relevantUserTrialOffer);
              if (!tmp14) {
                const expiresAt = relevantUserTrialOffer.expiresAt;
                const referralTrialOfferExpirationCopy = tmp44(8346).getReferralTrialOfferExpirationCopy(expiresAt.getTime());
                const intl2 = tmp44(1115).intl;
                const tmp44Result9 = tmp44(8346);
                ...
                obj7.duration = tmp44(4446).formatIntervalDuration(obj6);
                const tmp44Result10 = tmp44(4446);
              ...
              if (tmp10) {
                if (!tmp20) {
                  ...
                  if (tmp29) {
                    ...
                    obj11.acceptLabelIconUrl = tmp44(8236).getAssetUriForEmbed(tmp46(8342));
                    tmp33 = obj11;
                    const tmp44Result11 = tmp44(8236);
                  }
                  return tmp33;
                }
              }
              ...
              const tmp44Result8 = tmp44(7723);
            }
            const tmp46Result6 = tmp46(4632);
          }
        }
        const obj17 = {};
