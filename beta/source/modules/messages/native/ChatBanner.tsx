...
// Function ID: 11713
// Name: ChatBanner
// Dependencies: [19, 17, 5526, 4803, 11713, 1074, 21, 4788, 576, 11714, 11715, 504, 11, 1241, 4968, 7390, 4784, 1115, 5218, 7543, 8039, 5371, 7387, 8263, 2]
// Exports: default

// Module 11712 (ChatBanner)
...
import useShowChannelOptInNoticeDefault from "useShowChannelOptInNotice" /* 11714 */;
import useAllowedChatOverlaysDefault from "useAllowedChatOverlays" /* 11715 */;
...
function ArchivedLockedThreadChatBanner(channel) {
  ...
  let canUnarchiveThread = channel(7543).useCanUnarchiveThread(channel);
  ...
  const items1 = [closure_12(channel(4784).Text, obj3), ];
  if (canUnarchiveThread) {
    ...
    obj4.children = tmp8(tmp3(5218).Button, obj5);
...
function LockedThreadChatBanner(channel) {
  ...
  let isThreadModerator = channel(7543).useIsThreadModerator(channel);
  let obj2 = { style: tmp.threadBannerContainer, children: null };
  let obj3 = { lineClamp: 4, style: tmp.threadBannerTitle, variant: "text-sm/medium", color: "text-default", children: null };
  let obj = channel(7543);
  const tmp6 = closure_13;
...
function NewMessagesChatBar(channel) {
  ...
  if (obj.useStateFromStores(items, () => connected.isConnected(), [])) {
    if (unreadCount <= 0) {
      ...
      obj2.children = closure_12(XSmallBoldIcon(4784).Text, obj3);
      const items1 = [closure_12(XSmallBoldIcon(5371).PressableOpacity, obj2), ];
      handleScrollToNewMessages = XSmallBoldIcon(5371).PressableOpacity;
      ...
      XSmallBoldIcon = XSmallBoldIcon(8263).XSmallBoldIcon;
