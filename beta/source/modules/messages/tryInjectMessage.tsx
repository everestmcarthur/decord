...
// Dependencies: [502, 1074, 8026, 5010, 1385, 12236, 12237, 2]
// Exports: tryCreateInjectedMessage

// Module 12235 (tryInjectMessage)
        ...
        if (id2.isDM()) {
          ...
          if (id.author.id !== AuthenticationStore.getId()) {
            ...
            if (null == id.activity) {
              ...
              tmp4 = null;
              if (!tmpResult.hasFlag(num, tmp(12236).ChannelRecipientPrivateUserDataFlags.DISMISSED_IN_GAME_MESSAGE_NUX)) {
                tmp4 = null;
                if (!obj.has(id2.id)) {
                  ...
                  const messageRecord1 = tmp(5010).createMessageRecord(tmp9);
                  ({ applicationId: tmp10.applicationId, timestamp: tmp10.timestamp } = id);
                  const result = obj.set(id2.id, id.id);
                  const tmpResult3 = tmp(5010);
                  let num2 = id2.recipientFlags;
                  if (num2 == null) {
                    num2 = 0;
                  }
                  const tmpResult4 = tmp(1385);
                  const setFlagResult = tmp(1385).setFlag(num2, tmp(12236).ChannelRecipientPrivateUserDataFlags.DISMISSED_IN_GAME_MESSAGE_NUX, true);
                  const result1 = tmp6(12237).updatePrivateChannelRecipientFlags(id2.id, setFlagResult);
                  tmp4 = messageRecord1;
                  const tmp6Result = tmp6(12237);
                }
              }
              tmpResult = tmp(1385);
            }
