...
// Dependencies: [32, 19, 17, 7812, 21, 4788, 576, 8956, 7866, 15175, 4784, 12200, 1115, 2482, 15181, 5371, 2]
// Exports: default

// Module 15180 (FamilyCenterActivitySection)
...
export default function FamilyCenterActivitySection(displayType) {
  ...
  const actionsForDisplayType = displayType(15175).useActionsForDisplayType(displayType);
  const obj = displayType(15175);
  const actionTotalsForDisplayType = displayType(15175).useActionTotalsForDisplayType(displayType);
  const obj2 = displayType(15175);
  const familyCenterActions = displayType(12200).useFamilyCenterActions({});
  ...
  if (0 === actionsForDisplayType.length) {
    ...
    const formatToPlainStringResult = intl.formatToPlainString(loadMore(2482)["7dMmJY"], obj4);
    const obj5 = { style: loadMoreButton.container, children: null };
    const obj6 = { displayType };
    const items1 = [closure_9(FamilyCenterActivitySectionHeader, obj6), substr.map((action) => closure_1_9(loadMore(15181), { action }, action.event_id)), ];
    if (substr.length >= actionTotalsForDisplayType) {
      ...
      if (familyCenterActions.isMoreLoading) {
        ...
        obj10.children = tmp17(tmp(4784).Text, obj11);
        tmp17Result = tmp17(tmp(5371).PressableOpacity, obj10);
      }
