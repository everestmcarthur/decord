...
// Dependencies: [19, 17, 7811, 7812, 1074, 2038, 21, 2027, 7813, 4646, 7656, 4994, 4788, 576, 1612, 1115, 2482, 1241, 573, 2029, 4755, 4991, 17873, 1980, 7427, 5216, 5941, 4784, 5218, 2]
// Exports: default

// Module 17872 (ParentalConsentWarningModal)
...
export default function ParentalConsentWarningModal(daysRemaining) {
  ...
  const callback3 = callback1.useCallback(() => {
    ...
    if (values.some((link_status) => {
      ...
      const tab = tmp2(7813).selectTab(constants.REQUESTS);
      const tmp2Result = tmp2(7813);
      const tmp9 = require;
      const rootNavigationRef = RootNavigationRef.getRootNavigationRef();
      if (null != rootNavigationRef) {
        if (rootNavigationRef.isReady()) {
          const obj2 = { screen: constants3.FAMILY_CENTER };
          tmp9(7656).openUserSettings(obj2);
          const tmp9Result = tmp9(7656);
        }
      }
      tmp2(4994).enqueue(() => daysRemaining(callback[10]).openUserSettings({ screen: constants3.FAMILY_CENTER }));
      const tmp2Result3 = tmp2(4994);
    } else {
      tmp2(4991).pushLazy(asyncRequireImpl(17873, dependencyMap.paths));
      const tmp2Result4 = tmp2(4991);
    }
  }, items2);
