...
// Dependencies: [19, 17, 1372, 7811, 7812, 1074, 21, 4788, 576, 1177, 38, 563, 11, 4784, 4632, 7866, 5833, 5839, 1115, 2482, 15182, 15185, 15186, 2]
// Exports: default

// Module 15181 (FamilyCenterActivityRow)
import _modDef38 from "module_38" /* 38 */;
import nativeDefault from "native" /* 576 */;
import FamilyCenterUtils from "FamilyCenterUtils" /* 7866 */;
import FamilyCenterActivityPurchaseRowDefault from "FamilyCenterActivityPurchaseRow" /* 15182 */;
import FamilyCenterActivityGiftRowDefault from "FamilyCenterActivityGiftRow" /* 15186 */;
...
const ACTION_TO_TEXT = fn(7812).ACTION_TO_TEXT;
...
const memoResult = noop.memo((action) => {
  ...
  if (null == stateFromStores) {
    ...
    obj6.children = tmp3(4632).getName(stateFromStores);
    const items2 = [closure_8(tmp6(4784).Text, obj6), ];
    const obj7 = { variant: "text-xs/medium", color: "channels-default", children: null };
    const tmp3Result2 = tmp3(4632);
    const _Date = Date;
    const date = new Date(extractTimestampResult);
    obj7.children = tmp6(7866).formatUserActivityTimestamp(date.getTime(), value.timestampFormatter);
    items2[1] = closure_8(tmp6(4784).Text, obj7);
...
const memoResult1 = noop.memo((action) => {
  ...
  if (undefined === stateFromStores) {
    ...
    obj4.size = tmp2(5833).GuildIconSizes.NORMAL;
    const items1 = [closure_8(tmp6(5833), obj4), ];
    ...
    if (hasItem) {
      const obj8 = { style: tmp.badge, guild: stateFromStores, size: tmp6(5839).Sizes.SMALL, disableColor: true };
      tmp11Result = tmp11(tmp6(5839), obj8);
      const tmp6Result2 = tmp6(5839);
    }
    ...
    items2[1] = closure_8(tmp2(4784).Text, obj9);
    ...
    if (undefined !== stateFromStores.approximateMemberCount) {
      ...
      tmp11Result2 = tmp11(tmp2(4784).Text, obj10);
    }
  ...
  if (tmpResult9.isPurchase(action)) {
    ...
    if (tmpResult10.isGift(action)) {
      ...
      if (null == giftInfo) {
        ...
        const giftRowDisplayInfo = tmp(15185).getGiftRowDisplayInfo(giftInfo);
