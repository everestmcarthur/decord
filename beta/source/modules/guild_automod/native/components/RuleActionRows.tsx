...
// Function ID: 17974
// Name: RuleActionRows
// Dependencies: [19, 2041, 2096, 12146, 21, 4788, 576, 17950, 11621, 1115, 4755, 17974, 1980, 17975, 17953, 5854, 5216, 4784, 5866, 17949, 5936, 2]
// Exports: default

// Module 17973 (RuleActionRows)
...
function RuleActionRow(onPress) {
  ...
  if (null == actionInfo) {
    ...
    if (tmp9Result) {
      ...
      tmp9Result = tmp9(tmp3(4784).Text, obj5);
    }
    const obj6 = { variant: "text-xs/medium", color: "text-subtle", children: null };
    items[1] = tmp9Result;
    obj6.children = items;
    obj4.children = React5(tmp3(4784).Text, obj6);
    obj2.subLabel = timestampProducer(tmp3(5216).Stack, obj4);
    const obj7 = { checked: null != found };
    obj2.trailing = timestampProducer(tmp3(5866).FormCheckbox, obj7);
...
export default function RuleActionRows(rule) {
  ...
  const availableActionTypes = rule(17949).getAvailableActionTypes(rule.triggerType);
  let tmp3 = null;
  if (0 !== availableActionTypes.length) {
    ...
    tmp3 = closure_6(tmp(5936).TableRowGroup, obj2);
  }
  return tmp3;
};
