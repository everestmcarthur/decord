...
// Dependencies: [19, 17, 21, 4788, 1115, 6890, 576, 4747, 4784, 5847, 9362, 9350, 8642, 2]
// Exports: default

// Module 9570 (AuthorizeScopes)
...
function Scope(arg0) {
  ...
  if (isFake) {
    ...
    let tmp10Result = tmp10(tmp6(6890).CircleXIcon, obj3);
  } else {
    const obj4 = { color: nativeDefault.colors.TEXT_MUTED, size: "refresh_sm" };
    tmp10Result = tmp10(tmp6(4747).CircleCheckIcon, obj4);
  }
  ...
  const items1 = [React4(tmp6(4784).Text, { variant: "text-md/normal", color: str3, children: text }), ];
  let tmp10Result2 = null;
  if (null != error) {
    const obj6 = { variant: "text-xs/normal", children: error };
    tmp10Result2 = tmp10(tmp6(4784).Text, obj6);
  }
...
export default function AuthorizeScopes(accountScopes) {
  ...
  if (0 === accountScopes.length) {
    ...
    let hasItem = integrationType === tmp5(9350).ApplicationIntegrationType.USER_INSTALL;
    if (hasItem) {
      hasItem = requestedScopes.includes(tmp5(8642).OAuth2Scopes.APPLICATIONS_COMMANDS);
    }
    ...
    const items = [closure_4(tmp5(4784).Text, obj2), ];
    const obj4 = { style: tmp.scopes, children: null };
    const mapped = accountScopes.map((item) => {
      closure_0 = item;
      const scopeNames = accountScopes(9362).getScopeNames(item, closure_0);
