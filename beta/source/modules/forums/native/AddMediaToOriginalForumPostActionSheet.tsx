...
// Dependencies: [32, 5, 19, 17, 2041, 5137, 2063, 5008, 1074, 21, 4788, 576, 8113, 5382, 5410, 4755, 9456, 9453, 11, 5377, 9455, 8039, 1271, 12281, 7730, 7874, 5141, 1115, 504, 7439, 8041, 5386, 7427, 12282, 4784, 5219, 2]
// Exports: default

// Module 12280 (AddMediaToOriginalForumPostActionSheet)
      ...
      if (0 === c8) {
        ...
        if (arg0 === 1) {
          ...
          closure_132_5.on("progress", (currentSize) => {
            const maxFileSizeResult = closure_0(5382).maxFileSize(id.id);
            const obj = closure_0(5382);
            const tmp2 = id;
            const effectiveUploadLimit = closure_0(5410).getEffectiveUploadLimit(maxFileSizeResult);
            if (currentSize.currentSize > effectiveUploadLimit) {
              closure_1_5.cancel();
              dependencyMap(false);
              closure_1(4755).hideActionSheet();
          ...
          closure_132_5.on("complete", () => {
            dependencyMap(false);
            closure_1(9453).clearAll(closure_1_0, ChannelMessage.ChannelMessage);
            const obj = closure_1(9453);
            closure_1(4755).hideActionSheet();
          });
        ...
        if (arg0 === 1) {
          ...
          const mapped = closure_132_9.map((item, index) => closure_1_0(5377).getAttachmentPayload(item, index));
