...
// Dependencies: [2109, 5136, 1978, 9560, 38, 1115, 9558, 7795, 9561, 2]

// Module 12438 (ApplicationCommandValidators)
import _modDef38 from "module_38" /* 38 */;
import util from "util" /* 1115 */;
import Server from "Server" /* 1978 */;
import ApplicationCommandUtils from "ApplicationCommandUtils" /* 7795 */;
import ApplicationCommandOptionUtilsAll from "ApplicationCommandOptionUtils" /* 9558 */;
import ApplicationCommandChoiceUtils from "ApplicationCommandChoiceUtils" /* 9560 */;
...
export default {
  ...
  [fn(1978).ApplicationCommandOptionType.STRING]: (type, type2, id) => {
    ...
    if (null != type2.choices) {
      const obj = { success: null != tmp3(9560).findChoiceStringValue(type2.choices, surrogate) };
      return obj;
    } else {
  ...
  [fn(1978).ApplicationCommandOptionType.INTEGER]: (type, type2, id) => {
    ...
    if (null != trimmed) {
      if (0 !== trimmed.length) {
        if (null != type2.choices) {
          const obj = { success: null != tmp3(9560).findChoiceNumberValue(type2.choices, trimmed) };
          return obj;
        } else {
          if (type2.autocomplete) {
  ...
  [fn(1978).ApplicationCommandOptionType.CHANNEL]: (type, arg1, id2, id) => {
    if ("text" === type.type) {
      if (obj2.isSnowflake(type.text)) {
        ...
        const applicationCommandOption = tmp(9561).resolveApplicationCommandOption(type.text, id, id2);
        type = undefined;
        if (applicationCommandOption != null) {
          type = applicationCommandOption.type;
        }
  ...
  [fn(1978).ApplicationCommandOptionType.MENTIONABLE]: (type, arg1, id2, id) => {
    if ("text" === type.type) {
      if (obj2.isSnowflake(type.text)) {
        ...
        const applicationCommandOption = tmp4(9561).resolveApplicationCommandOption(type.text, id, id2);
