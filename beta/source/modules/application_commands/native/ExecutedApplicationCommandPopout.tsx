...
// Dependencies: [19, 17, 4780, 2041, 2099, 2063, 5008, 4437, 1372, 9436, 5242, 1074, 1483, 5243, 21, 4788, 576, 1978, 1177, 4755, 8480, 4632, 4784, 4941, 1115, 7439, 504, 5021, 10028, 8251, 5836, 1400, 5856, 1364, 11902, 4485, 5854, 1610, 7797, 5216, 5936, 7459, 8418, 7427, 2]
// Exports: default

// Module 11901 (ExecutedApplicationCommandPopout)
...
import NativeCommandClipboardModuleDefault from "NativeCommandClipboardModule" /* 11902 */;
    ...
    if (iter.type !== tmp6(1978).ApplicationCommandOptionType.SUB_COMMAND_GROUP) {
      if (null != iter.value) {
        function getUserComponent(user, styles) {
          ...
          const items = [closure_1_22, channel(4632).getUserTag(user, { decoration: "never" })];
          obj.children = items;
        ...
        if (tmp6(1978).ApplicationCommandOptionType.USER === type) {
          ...
          if (null != channel1) {
            ...
            const items1 = [closure_20, tmp6(4941).computeChannelName(channel1, UserStore, RelationshipStore)];
            obj.children = items1;
            const _HermesInternal3 = HermesInternal;
            userComponent = closure_24(tmp6(1177).LegacyText, obj, "optionValue-" + iter.name);
            const tmp6Result = tmp6(4941);
          }
        } else {
    ...
    if (item10021.type !== tmp5(1978).ApplicationCommandOptionType.SUB_COMMAND_GROUP) {
      ...
      if (null != item10021.value) {
        ...
        if (tmp5(1978).ApplicationCommandOptionType.USER === type) {
          ...
          sum = null;
          if (null != channel) {
            sum = closure_1_20 + tmp5(4941).computeChannelName(channel, UserStore, RelationshipStore);
          ...
          if (items.includes(tmp3.options[0].type)) {
            ...
            if (chatInputRef != null) {
              ...
              if (current2 != null) {
                ...
                const obj3 = { initialRouteName: AppLauncherRouteName.COMMAND_VIEW, analyticsLocation: tmp4(7797).ApplicationCommandTriggerLocations.RECALL, preSelectedCommand: null };
...
export default function ExecutedCommandPopout(channelId) {
  ...
  const obj2 = { value: messageId(7439)(messageId(7459).EXECUTED_COMMAND).analyticsLocations, children: null };
  ...
  obj2.children = closure_23(channelId(7427).BottomSheet, obj3);
  return closure_23(channelId(7439).AnalyticsLocationProvider, obj2);
};
