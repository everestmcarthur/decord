...
// Dependencies: [5, 32, 19, 17, 2054, 11151, 21, 4788, 576, 11274, 11275, 11276, 1370, 4485, 1115, 10520, 11271, 1478, 1364, 11278, 3356, 5373, 11279, 11290, 11292, 2]
// Exports: default

// Module 11273 (FavoritesGuildAddChannelModal)
...
let closure_7 = fn(2054).MAX_FAVORITES_ADD_CHANNEL_COUNT;
...
export default function FavoritesGuildAddChannelModal(parentId) {
      ...
      try {
        ...
        if (0 === c3) {
          if (arg0 === 1) {
            ...
            c3 = 1;
            c4 = 1;
            const obj6 = { value: Promise.all(first.map(parentId(11276).getOrResolveChannelIdFromDestinationId)), done: false };
            return obj6;
          }
        } else if (arg0 === 1) {
          c4 = 3;
          ...
          if (0 !== closure_129_0.length) {
            ...
            parentId(10520).addFavoriteChannels(obj8);
            const obj2 = parentId(10520);
            const result = parentId(11271).closeFavoritesGuildAddChannelModal();
            const obj4 = parentId(11271);
          } else {
            const intl = parentId(1115).intl;
            parentId(4485).presentError(intl.string(parentId(1115).t.R0RpRX));
            const obj = parentId(4485);
          }
          c4 = 3;
        }
      } catch (tmp31) {
  ...
  const tmp2 = first;
  const tmp4 = first(11274)();
  let intl = parentId(1115).intl;
  obj2.title = intl.string(first(3356).Rp35U1);
  obj2.onClose = parentId(11271).closeFavoritesGuildAddChannelModal;
  const items2 = [closure_9(first(11278), obj2), ];
  let obj3 = { style: tmp.container, children: null };
  const items3 = [closure_9(first(5373), { absolute: true }), , ];
  ...
  let num = 0;
  const tmp12 = first(11278);
  if (first.length > 0) {
    num = tmp2(576).space.PX_80;
  }
  obj4.insetEnd = num;
  obj4.disableSelection = first.length >= closure_7;
  items3[1] = closure_9(first(11279), obj4);
  let obj5 = { isVisible: first.length > 0, floatingBackgroundColor: tmp.container.backgroundColor, text: null, onPress: null };
  const tmp14 = first(11279);
  obj5.text = parentId(11292).getFavoritesAddButtonLabel(first.length);
  obj5.onPress = callback1;
  items3[2] = closure_9(parentId(11290).ModalFloatingAction, obj5);
