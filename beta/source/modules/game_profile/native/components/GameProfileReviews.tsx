...
// Dependencies: [19, 17, 21, 4788, 576, 4508, 8986, 4483, 9033, 9034, 8989, 1115, 8997, 4784, 2018, 9035, 9041, 8993, 8994, 2]
// Exports: default

// Module 9032 (GameProfileReviews)
import nativeDefault from "native" /* 576 */;
import GameProfileAnalyticUtils from "GameProfileAnalyticUtils" /* 8989 */;
import useSteamWebsiteUrl from "useSteamWebsiteUrl" /* 8993 */;
...
function SteamReviewRow(url) {
  ...
  const tmp = closure_9();
  const tmp5Result = trackAction(8986)(trackAction(4483).openURL);
  dependencyMap = tmp5Result;
  const tmp4 = trackAction;
  const tmp5 = trackAction(8986);
  const result = url(9033).calculateSteamReviewScoreDescription(rating, ratingCount, isRecentRating);
  const obj = url(9033);
  const items = [tmp5Result, url, trackAction];
  const steamReviewScoreDescriptionColor = url(9034).getSteamReviewScoreDescriptionColor(result);
  const obj3 = {
  ...
  const items2 = [closure_7(url(8997).SteamNeutralIcon, { size: "sm", color: tmp4(576).colors.ICON_STRONG }), closure_7(url(4784).Text, { variant: "heading-sm/medium", color: "mobile-text-heading-primary", children: title })];
  ...
  let linkText;
  if (noop.useContext(url(4508).AccessibilityPreferencesContext).alwaysShowLinkDecorations) {
    linkText = tmp.linkText;
  }
  obj7.style = linkText;
  const obj5 = { size: "sm", color: tmp4(576).colors.ICON_STRONG };
  obj7.children = url(9034).getSteamReviewScoreDescriptionIntl(result);
  const items4 = [closure_7(url(4784).Text, obj7), ];
  ...
  if (tmp12Result) {
    ...
    tmp12Result = tmp12(tmp2(4784).Text, obj8);
...
function OpenCriticReview(url) {
  ...
  if (opencritic == null) {
    opencritic = { topCriticRating: "Array", topCriticRatingCount: "ip", tier: "text" };
  }
  ...
  const tmp4Result = trackAction(8986)(trackAction(4483).openURL);
  ...
  if (null != tier) {
    str = url(9035).getOpenCriticTierText(tier);
    const obj2 = url(9035);
  }
  if (null != tier) {
    let openCriticCircleRatingColor = url(9035).getOpenCriticCircleRatingColor(tier);
    const obj4 = url(9035);
  } else {
  ...
  const items1 = [closure_7(url(4784).Text, obj3), ];
  ...
  if (null != tier) {
    ...
    const obj8 = { uri: tmp11(9035).getOpenCriticTierImage(tier) };
    obj7.source = obj8;
  ...
  if (null != tier) {
    ...
    if (topCriticRating > 0) {
      ...
      if (num > 0) {
        ...
        const items4 = [tmp12(trackAction(9041), obj11), ];
        ...
        obj12.children = tmp12(tmp11(4784).Text, obj13);
  ...
  if (topCriticRating <= 0) {
    ...
    if (null == tier) {
      const obj14 = { variant: "text-xs/medium", color: tmp11(9034).getSteamReviewScoreDescriptionColor(tmp11(2018).SteamReviewScoreDescription.NO_USER_REVIEWS), children: null };
      const intl4 = tmp11(1115).intl;
      obj14.children = intl4.string(tmp11(1115).t["0xYzpO"]);
      tmp12Result2 = tmp12(tmp11(4784).Text, obj14);
      const tmp11Result2 = tmp11(9034);
    }
  } else {
    tmp12Result2 = null;
  }
...
export default function GameProfileReviews(arg0) {
  ...
  if (null == game) {
    ...
    const tmp6 = game.steamReleaseStatus !== tmp2(8994).SteamReleaseStatus.RETIRED_ABANDONED && null != steamWebsiteUrl;
    const reviews = game.reviews;
    let steam;
    if (reviews != null) {
      steam = reviews.steam;
    }
    const calculateSteamReviewScoreDescription = tmp2(9033).calculateSteamReviewScoreDescription;
    if (steam != null) {
    ...
    let tmp10 = tmp6;
    if (tmp6) {
      tmp10 = tmp9 !== tmp2(2018).SteamReviewScoreDescription.NO_USER_REVIEWS;
    }
    const tmp2Result = tmp2(9033);
    const result = tmp2(9034).canShowLocalizedSteamReview(steam);
    if (result) {
      let localizedRating;
      if (!tmp12) {
    ...
    const items = [React5(tmp2(4784).Text, obj3), ];
