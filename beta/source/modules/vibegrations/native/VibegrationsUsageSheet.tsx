...
// Dependencies: [19, 17, 13391, 21, 4788, 576, 4784, 5307, 504, 7474, 7426, 1115, 3710, 5216, 2]
// Exports: default
...
export default function VibegrationsUsageSheet(projectId) {
  ...
  if (null == stateFromStores) {
    ...
    const tmp2Result = tmp2(5307);
    const tmp2Result7 = tmp2(5307);
    const sumTokenUsageResult = tmp2(5307).sumTokenUsage(stateFromStores.orchestrator, stateFromStores.codegen);
    const tmp2Result8 = tmp2(5307);
    const obj2 = { header: null, children: null };
    const obj3 = { title: null };
    const intl = tmp2(1115).intl;
    obj3.title = intl.string(_modDef3710["9yoLWZ"]);
    obj2.header = closure_5(tmp2(7426).BottomSheetTitleHeader, obj3);
    ...
    const sumTokenUsageResult1 = tmp2Result.sumTokenUsage(sumTokenUsageResult, tmp2(5307).usageOrEmpty(stateFromStores.compaction));
    const tmp2Result9 = tmp2(5307);
    obj7.runes = tmp2(5307).runesFromUsd(stateFromStores.cost_usd).toLocaleString();
    obj6.children = intl2.formatToPlainString(_modDef3710["4PFO2p"], obj7);
    const items2 = [closure_5(tmp2(4784).Text, obj6), , ];
    const obj8 = { direction: "vertical", spacing: nativeDefault.space.PX_4, children: null };
    const obj9 = { label: null, usage: null };
    const intl3 = tmp2(1115).intl;
    obj9.label = intl3.string(_modDef3710.hk4jJr);
    obj9.usage = stateFromStores.orchestrator;
    const items3 = [closure_5(RoleRow, obj9), , , ];
    const obj10 = { label: null, usage: null };
    const intl4 = tmp2(1115).intl;
    obj10.label = intl4.string(_modDef3710.R9aduM);
    obj10.usage = stateFromStores.codegen;
    items3[1] = closure_5(RoleRow, obj10);
    const obj11 = { label: null, usage: null };
    const intl5 = tmp2(1115).intl;
    obj11.label = intl5.string(_modDef3710.Tj6b30);
    const runesFromUsdResult = tmp2(5307).runesFromUsd(stateFromStores.cost_usd);
    obj11.usage = tmp2(5307).usageOrEmpty(stateFromStores.compaction);
    items3[2] = closure_5(RoleRow, obj11);
    const obj12 = { label: null, usage: null };
    const intl6 = tmp2(1115).intl;
    obj12.label = intl6.string(_modDef3710.vVUMwj);
    const tmp2Result10 = tmp2(5307);
    obj12.usage = tmp2(5307).usageOrEmpty(stateFromStores.classifier);
    items3[3] = closure_5(RoleRow, obj12);
    obj8.children = items3;
    items2[1] = closure_6(tmp2(5216).Stack, obj8);
    ...
    obj15.children = intl7.string(_modDef3710["kILb+R"]);
    obj14.children = closure_5(tmp2(4784).Text, obj15);
    const items4 = [closure_5(View, obj14), ];
    const obj16 = { variant: "text-sm/medium", color: "text-default", children: null };
    const _Math = Math;
    const tmp2Result11 = tmp2(5307);
    const _HermesInternal = HermesInternal;
    obj16.children = "" + Math.round(100 * tmp2(5307).cacheHitRate(sumTokenUsageResult1)) + "%";
    items4[1] = closure_5(tmp2(4784).Text, obj16);
    obj13.children = items4;
    items2[2] = closure_6(View, obj13);
    obj5.children = items2;
    obj4.children = closure_6(tmp2(5216).Stack, obj5);
    obj2.children = closure_5(View, obj4);
    return closure_5(tmp2(7474).ActionSheet, obj2);
  }
