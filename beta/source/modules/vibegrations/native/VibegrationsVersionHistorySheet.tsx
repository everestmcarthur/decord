...
// Dependencies: [32, 19, 17, 13390, 21, 4788, 576, 7909, 1612, 5146, 1115, 3710, 4755, 4784, 5936, 5854, 7474, 7426, 6901, 2]
// Exports: default
...
const fetchSourceHistory = fn(13390).fetchSourceHistory;
const jsx = fn(21).jsx;
...
export default function VibegrationsVersionHistorySheet(projectId) {
  ...
  if ("loading" === tmp5.status) {
    ...
    obj4.children = intl2.string(tmp2(3710)["mSJn+K"]);
    obj3.children = jsx(projectId(4784).Text, { variant: "text-md/normal", color: "text-muted", children: null });
    ...
    obj6.children = intl.string(tmp2(3710).TOmYPT);
    obj5.children = jsx(projectId(4784).Text, { variant: "text-md/normal", color: "text-muted", children: null });
    ...
    tmp9 = jsx(projectId(5936).TableRowGroup, { hasIcons: false, children: null });
  }
  const obj7 = { scrollable: true, header: null, children: null };
  const obj8 = { title: null };
  let intl3 = projectId(1115).intl;
  obj8.title = intl3.string(onRestore(3710).jAWwzi);
  obj7.header = tmp7(projectId(7426).BottomSheetTitleHeader, obj8);
  const tmp4 = _slicedToArray(noop.useState({ status: "loading" }), 2);
  obj7.children = tmp7(projectId(6901).BottomSheetScrollView, { contentContainerStyle: { paddingBottom: onRestore(1612)().bottom }, children: tmp9 });
  return tmp7(projectId(7474).ActionSheet, obj7);
};
