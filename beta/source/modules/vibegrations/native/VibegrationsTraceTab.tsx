...
// Dependencies: [32, 19, 17, 9341, 21, 4788, 576, 17076, 17078, 1115, 3710, 17077, 5856, 4784, 1612, 504, 17079, 17080, 4755, 17081, 8506, 11519, 17073, 9029, 7327, 16942, 4736, 2]
// Exports: default
...
function TraceRow(entry) {
  ...
  if ("model" === entry.kind) {
    if (null != entry.promptTokens) {
      ...
      let formatToPlainStringResult = intl.formatToPlainString(_modDef3710["PYO+Jv"], obj3);
    ...
    if (null != formatToPlainStringResult) {
      ...
      tmp10Result = tmp10(tmp2(4784).Text, obj11);
    }
    ...
    if ("tool" === entry.kind) {
      ...
      if (null != entry.summary) {
        ...
        tmp10Result3 = tmp10(tmp2(4784).Text, obj12);
      }
    }
    ...
    if (null != entry.error) {
      ...
      tmp10Result4 = tmp10(tmp2(4784).Text, obj13);
    }
    items2[2] = tmp10Result4;
    obj6.children = items2;
    obj5.children = React6(View, obj6);
    obj4.children = React5(tmp2(5856).Card, obj5);
...
function TraceOverview(arg0) {
  ...
  obj4.children = TRACE_CATEGORIES.map((item) => {
    ...
    obj5.children = intl.formatToPlainString(_modDef3710.UffawN, { percent: num2 });
    ...
    const tmp4 = React6;
    const tmp5 = View;
    items1[3] = React5(Text_Text.Text, { variant: "text-xs/normal", color: "text-subtle", children: intl2.formatToPlainString(_modDef3710.w8vPbe, { count: num4 }) });
    let tmp6Result = null;
    if (0 !== num) {
  ...
  const memo = onPress.useMemo(() => {
    ...
    const item = projectId(stateFromStoresArray[8]).groupTraceByTurn(stateFromStoresArray).forEach((turnId, index) => {
      ...
      if (0 !== filterTraceResult.length) {
        ...
        obj2.label = intl.formatToPlainString(_modDef3710["Y/j+TD"], obj3);
