...
// Dependencies: [32, 19, 17, 14435, 21, 4788, 576, 17053, 14434, 14438, 4755, 17062, 5371, 1115, 3710, 4784, 4742, 2]
// Exports: default
...
function ThinkingIndicator(line) {
  ...
  const items2 = [closure_7(line(14434).AILoader, { size: 10, color: "text-subtle" }), ];
  const obj2 = { style: tmp.label, children: closure_7(line(14438).AIShimmer, { ref, text, variant: "text-xs/medium", color: "text-subtle", duration: line(17053).INDICATOR_PASS_MS, delay: null }) };
  items2[1] = closure_7(ref, obj2);
  obj.children = items2;
  return closure_8(ref, obj);
}
const View = fn(17).View;
const AI_LOADER_CYCLE_MS = fn(14435).AI_LOADER_CYCLE_MS;
const jsxProd = fn(21);
...
export default function VibegrationsNativeStatusStrip(projectId) {
  ...
  if (thinking) {
    ...
    if (tmp6) {
      ...
      stringResult = intl.string(_modDef3710["0PGVTy"]);
    }
    ...
    tmp10Result = tmp10(tmp2(5371).PressableOpacity, obj4);
  }
  ...
  if (null != connLabel) {
    ...
    tmp10Result2 = tmp10(tmp2(4784).Text, obj7);
  }
  ...
  if (null != runesUsedLabelsResult) {
    ...
    const items2 = [tmp10(tmp2(4784).Text, obj9), ];
    const obj10 = { size: "xxs", color: nativeDefault.colors.TEXT_MUTED };
    items2[1] = tmp10(tmp2(4742).CircleInformationIcon, obj10);
    obj8.children = items2;
    tmp8Result = tmp8(tmp2(5371).PressableOpacity, obj8);
  }
  items1[2] = tmp8Result;
