...
// Dependencies: [32, 19, 502, 504, 16964, 16965, 9319, 7440, 9626, 2]
...
export const useVibegrationsPreviewMode = function useVibegrationsPreviewMode(arg0) {
  ...
  let tmp14;
  if (surfaces != null) {
    tmp14 = surfaces[tmp8(undefined, 9319).ApplicationWidgetConfigSurface.WIDGET_TOP];
  }
  const obj2 = { widgetTop: null != tmp14, widgetBottom: null, miniProfile: null };
  let tmp15;
  if (surfaces != null) {
    tmp15 = surfaces[tmp8(undefined, 9319).ApplicationWidgetConfigSurface.WIDGET_BOTTOM];
  }
  obj2.widgetBottom = null != tmp15;
  let tmp16;
  if (surfaces != null) {
    tmp16 = surfaces[tmp8(undefined, 9319).ApplicationWidgetConfigSurface.MINI_PROFILE];
  }
  obj2.miniProfile = null != tmp16;
  const result = vibegrationsPreviewModes.profileSurfaceAvailability(obj2);
  if (null == tmp7) {
    const data = tmp8(7440).useApplication(previewApplicationId).data;
    ...
    const tmp8Result6 = tmp8(7440);
    const application = tmp8(7440).useApplication(applicationId);
    if (!declaredActivity) {
      declaredActivity = tmp8(9626).canLaunchFrame(tmp25);
      const tmp8Result8 = tmp8(9626);
    }
    const tmp8Result7 = tmp8(7440);
