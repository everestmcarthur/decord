...
// Function ID: 11977
// Name: ForwardPreview
// Dependencies: [19, 17, 21, 4788, 576, 4722, 8427, 8222, 8962, 11977, 1978, 11978, 12, 11979, 1115, 11981, 9026, 5337, 10407, 5836, 1477, 9126, 4784, 3000, 8444, 2]
// Exports: ForwardPreview

// Module 11976 (ForwardPreview)
...
import ForwardPreviewUtils from "ForwardPreviewUtils" /* 11977 */;
import MosaicMediaType from "MosaicMediaType" /* 11979 */;
...
function MessagePreview(content) {
  ...
  const tmp4 = attachmentCount(4788).createNativeStyleProperties({ seeMoreLabelColor: TEXT_SUBTLE })(tmp3);
  importDefault = tmp4;
...
export const ForwardPreview = function ForwardPreview(message) {
  ...
  if (contentMessage.components.length > 0) {
    checkpointData = null;
    if (contentMessage.components[0].type === tmp2(1978).ComponentType.CHECKPOINT_CARD) {
  ...
  if (null != checkpointData) {
    ...
    tmp6 = tmp2(11978).CHECKPOINT_PERSONA_COLORS[num];
  }
  if (tmp6 != null) {
  ...
  if (attachments.length > 0) {
    ...
    if (num2 > 0) {
      if (num3 > 0) {
        ...
        let AttachmentIcon = tmp2(11981).ImagesIcon;
      }
      if (num3 > 0) {
        if (length === num3) {
          ...
          items1[1] = hasOwnProperty(tmp2(9026).CirclePlayIcon, obj5);
  ...
  if (attachments.length > 1) {
    tmp33 = tmp7;
    if (null != tmp7) {
      const size3 = { shape: tmp2(9126).CutoutShape.RoundedRect, x: 28, y: 28, width: 32, height: 32, cornerRadius: 12 };
      ...
      items3[1] = timestampProducer(tmp2(4784).Text, obj17);
  ...
  if (tmp36Result) {
    ...
    tmp36Result = tmp36(tmp2(4784).Text, obj22);
  }
  ...
  if (tmp34Result) {
    ...
    if (tmp36Result7) {
      ...
      tmp36Result7 = tmp36(tmp2(4784).Text, obj26);
    }
  ...
  if (tmp36Result8) {
    ...
    const obj29 = { uri: tmp2(8444).getCardAssetUrl(num5) };
    size4.source = obj29;
    obj27.children = tmp36(tmp46, size4);
    tmp36Result8 = tmp36(tmp35, obj27);
    const tmp2Result2 = tmp2(8444);
  }
