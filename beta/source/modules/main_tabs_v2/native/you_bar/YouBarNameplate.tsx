...
// Dependencies: [19, 4780, 15368, 21, 4489, 576, 15454, 504, 4524, 5217, 9131, 2]
...
require = fn;
const YOU_BAR_SPRING_CONFIG = fn(15368).YOU_BAR_SPRING_CONFIG;
const jsx = fn(21).jsx;
...
export default noop.memo(function YouBarNameplate(isQuestRendered) {
  ...
  token = isQuestRendered(4489).useToken(token(576).modules.mobile.YOU_BAR_BORDER_RADIUS);
  const tmp4 = token(15454)(token);
  dependencyMap = tmp4;
  let obj = isQuestRendered(4489);
  const tmp2 = token;
  ...
  const obj3 = isQuestRendered(4524);
  fn.__closure = { withSpring: isQuestRendered(5217).withSpring, isQuestRendered, questDockAnimatedBorderRadius: tmp4, borderRadius: token, YOU_BAR_SPRING_CONFIG };
  ...
  const obj4 = { withSpring: isQuestRendered(5217).withSpring, isQuestRendered, questDockAnimatedBorderRadius: tmp4, borderRadius: token, YOU_BAR_SPRING_CONFIG };
  if (str) {
    str = "always";
  }
  obj6.animate = str;
  obj5.children = jsx(tmp2(9131), { nameplate, isFocused: true, animate: null });
  return jsx(token(4524).View, { style: null, pointerEvents: "none", children: null });
});
