...
// Dependencies: [19, 17, 4780, 4437, 1372, 21, 4788, 576, 4784, 4941, 1095, 5321, 5323, 5271, 504, 2]
// Exports: SimpleNotificationHeader, default

// Module 10468 (MessageNotificationHeader)
  ...
  const memo = noop.useMemo(() => {
    ...
    if (PRIVATE_CHANNEL.has(channel.type)) {
      ...
      if (tmp.type !== tmp3(1095).ChannelTypes.PUBLIC_THREAD) {
        const simpleChannelIconComponent = tmp3(5271).getSimpleChannelIconComponent(tmp);
        const tmp3Result = tmp3(5271);
      }
      if (null == tmp2) {
        const ThreadIcon = tmp3(5323).ThreadIcon;
      }
      tmp3Result2 = tmp3(5321);
...
export default function MessageNotificationHeader(locationTextColor) {
              ...
              if (tmp2(1095).ChannelTypes.GUILD_VOICE !== type) {
                if (tmp2(1095).ChannelTypes.GUILD_STAGE_VOICE !== type) {
                  ...
                  if (null != parentChannel) {
                    channelName1 = tmp2(4941).computeChannelName(parentChannel, UserStore, RelationshipStore);
                    const tmp2Result2 = tmp2(4941);
                  }
  ...
  if (tmp17Result) {
    ...
    tmp17Result = tmp17(tmp2(4784).Text, obj5);
  }
