...
// Dependencies: [19, 17, 21, 5833, 9126, 576, 4788, 10393, 1115, 4799, 10466, 8898, 10402, 2]
// Exports: default
...
let size = { shape: fn(9126).CutoutShape.RoundedRect, x: fn(5833).ImageSizes[fn(undefined, 5833).GuildIconSizes.NORMAL] - -6 - 24, y: -6, width: 24, height: 24, cornerRadius: nativeDefault.radii.sm };
...
export default function AlertNotification(notification) {
  ...
  const tmp = closure_9();
  const incidentData = channel(10393).useGuildIncidentsState(notification.key).incidentData;
  let raidDetectedAt;
  ...
  const items2 = [size];
  obj4.cutouts = items2;
  let obj = channel(10393);
  obj4.children = closure_5(guild(5833), { guild, selected: false });
  const items3 = [closure_5(guild(9126), obj4), ];
  const obj5 = { style: tmp.warningBadge, children: null };
  const tmp8 = guild(9126);
  obj5.children = closure_5(channel(8898).WarningIcon, { style: tmp.warningIcon, color: guild(576).colors.ICON_FEEDBACK_WARNING });
  items3[1] = closure_5(View, obj5);
  obj3.children = items3;
  obj2.icon = closure_7(closure_6, obj3);
  obj2.header = memo;
  obj2.children = closure_5(channel(10402).SystemMessageText, { text: stringResult });
  obj2.onPress = callback;
  obj2.notification = notification;
  return closure_5(channel(10466).NotificationPressable, obj2);
};
