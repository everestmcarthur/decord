...
// Dependencies: [19, 17, 4780, 10391, 21, 4788, 576, 8577, 4784, 4489, 1364, 5206, 5331, 7245, 563, 7874, 10471, 10426, 8426, 8568, 1477, 5836, 7566, 7571, 8610, 10472, 1177, 9126, 2]
// Exports: MediaPreviewRightAccessory

// Module 10470 (MediaPreviewRightAccessory)
...
import common_VideoDefault from "common/Video" /* 8610 */;
import ClipView from "ClipView" /* 9126 */;
import usePreviewableMedia from "usePreviewableMedia" /* 10426 */;
import useContentHarmTypes from "useContentHarmTypes" /* 10471 */;
...
const VisualEffectViewDefault = tmp4(5206);
require = fn;
function VideoBadge() {
...
function ObscuredMediaOverlay(isSpoiler) {
  ...
  if (isObscured) {
    let tmp14Result = tmp14(tmp2(5331).ImageWarningIcon, { size: "sm", color: "white" });
  } else {
    const obj5 = { style: tmp.spoilerPill, children: tmp14(tmp2(7245).EyeIcon, { size: "sm", color: "white" }) };
...
function SinglePreviewableMedia(arg0) {
  ...
  if (usePreviewableMedia.PreviewableMediaTypes.VOICE_MESSAGE === type) {
    ...
    if (tmp2(10426).PreviewableMediaTypes.AUDIO !== type) {
      if (tmp2(10426).PreviewableMediaTypes.FILE !== type) {
        if (tmp2(10426).PreviewableMediaTypes.IMAGE !== type) {
          if (tmp2(10426).PreviewableMediaTypes.VIDEO !== type) {
            if (tmp2(10426).PreviewableMediaTypes.GIF === type) {
              ...
              if (null == thumbnail) {
                ...
                const enabledHarmTypesForChannelAndAuthorId = tmp2(7566).getEnabledHarmTypesForChannelAndAuthorId(message.channel_id, id);
                const tmp2Result = tmp2(7566);
                const obj5 = { type: null, media: null };
                const getMediaObscuredReasonFromBitmask = tmp2(7566).getMediaObscuredReasonFromBitmask;
                obj5.type = tmp2(7571).ObscuredMediaTypes.Embed;
                obj5.media = media;
                ...
                url2 = thumbnail.url;
                const tmp2Result5 = tmp2(7566);
              }
            } else if (tmp2(10426).PreviewableMediaTypes.STICKER === type) {
          ...
          if (width > 0) {
            if (null != height) {
              if (height > 0) {
                ...
                const attachmentObscurityProps = tmp2(8426).getAttachmentObscurityProps(obj13);
                const tmp2Result6 = tmp2(8426);
                const attachmentUrl = tmp2(8568).getAttachmentUrl(media);
                const tmp2Result7 = tmp2(8568);
                ...
                let str3;
                if (type === tmp2(10426).PreviewableMediaTypes.VIDEO) {
                  str3 = "png";
                }
                obj14.format = str3;
...
let closure_15 = createStyles.createStyles({ rightAccessoryContainer: { marginLeft: fn(10391).RIGHT_ACCESSORY_LEFT_MARGIN } });
size = fn(2);
