...
// Dependencies: [2041, 2099, 2063, 1372, 1074, 5019, 1370, 1400, 7464, 8424, 1092, 576, 8425, 5271, 2]
// Exports: getChannelIconData, transformSearchableSelectOptions

// Module 8423 (NativeSearchableSelectActionComponentUtils)
...
export const transformSearchableSelectOptions = function transformSearchableSelectOptions(initialSnowflakeSelectOptions, guildId) {
  ...
  const mapped = initialSnowflakeSelectOptions.map((type) => {
    ...
    if (InteractionComponentTypes.SelectOptionType.USER === type) {
      ...
      return tmp34;
    } else if (tmp(5019).SelectOptionType.ROLE === type) {
      let role = null;
      if (null != closure_1) {
      ...
      if (null != role) {
        ...
        if (null != tmp14) {
          ...
          if (tmpResult7.canGuildUseRoleIcons(tmp14, role)) {
            roleIconData = tmp(7464).getRoleIconData(role);
            const tmpResult8 = tmp(7464);
          }
          if (null == roleIconData) {
            const obj2 = {};
            const merged1 = Object.assign(type);
            obj2.iconSrc = tmp(1400).ensureAvatarSource(_modDef8424).uri;
      ...
      return tmp18;
    } else if (tmp(5019).SelectOptionType.CHANNEL === type) {
      ...
      if (null == channel) {
        ...
        if (channel.type === constants.GUILD_CATEGORY) {
          ...
          channelIconWithGuild = tmp(5271).getChannelIconWithGuild(channel, tmp4);
          const tmpResult12 = tmp(5271);
        }
