...
// Function ID: 9132
// Name: Nameplate
// Dependencies: [19, 4780, 21, 4788, 504, 4722, 4524, 4789, 1970, 9132, 5230, 1364, 9121, 5836, 2]
// Exports: default

// Module 9131 (Nameplate)
...
function NameplateInner(isFocused) {
  ...
  let obj = flag6(504);
  const tmp6 = sharedValue(4722)();
  let num = 1;
  if (flag6) {
    num = 0;
  }
  sharedValue = flag6(4524).useSharedValue(num);
  ...
  const animatedStyle = flag6(4524).useAnimatedStyle(A);
  const tmpResult = flag6(4524);
  const backgroundGradientColors = flag6(1970).getBackgroundGradientColors(nameplate.palette, tmp6);
  const tmpResult4 = flag6(1970);
  const nameplateAssets = flag6(9132).getNameplateAssets(nameplate);
  ...
  if (null != backgroundGradientColors) {
    ...
    obj4.colors = items3;
    tmp15Result = closure_5(tmp5(5230), obj4);
    const tmp5Result = tmp5(5230);
  }
  const items4 = [tmp15Result, ];
  const tmp13 = closure_6;
  const tmpResult5 = flag6(9132);
  if (tmpResult6.isAndroid()) {
    if (tmp12) {
      const obj5 = { url: str, style: tmp4.img, autoplay: true };
      let tmp17 = closure_5(tmp(9121).APNGPlayer, obj5);
    }
    items4[1] = tmp17;
    obj3.children = items4;
    return tmp13(tmp5(4524).View, obj3);
  }
  tmp17 = closure_5(tmp5(5836), { source: { uri: str }, style: tmp4.img, accessibilityRole: "image" });
}
const jsxProd = fn(21);
