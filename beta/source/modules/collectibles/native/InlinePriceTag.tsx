...
// Function ID: 13483
// Name: InlinePriceTag
// Dependencies: [19, 17, 7514, 1076, 1074, 21, 4788, 576, 4784, 1973, 7827, 7828, 9148, 1115, 8479, 4446, 13483, 9180, 9159, 13484, 504, 9172, 1364, 9173, 8972, 2]
// Exports: default

// Module 13482 (InlinePriceTag)
...
import useProductDisableState from "useProductDisableState" /* 9180 */;
import useOpenNitroSubscribeActionSheetDefault from "useOpenNitroSubscribeActionSheet" /* 13483 */;
import useVirtualCurrencyData from "useVirtualCurrencyData" /* 13484 */;
...
function OrbsPriceTag(arg0) {
  ...
  if (null == vcData.price) {
    ...
    if (result) {
      result = tmp16(7827).isOrbsExclusiveProduct(product);
      const tmp16Result = tmp16(7827);
    }
    ...
    if (result) {
      result = discountPercentage >= tmp16(7828).DISCOUNT_DISPLAY_MINIMUM_THRESHOLD;
    }
    ...
    if (result) {
      ...
      obj2.icon = React7(tmp16(9148).OrbsIcon, obj3);
    ...
    if (!result) {
      ...
      tmp10Result = tmp10(tmp16(9148).OrbsIcon, obj6);
    }
...
export default function InlinePriceTag(arg0) {
  ...
  if (obj7.useStateFromStores(items, () => fetchingGoogleSkus.isFetchingGoogleSkus())) {
    if (null == formattedPriceForCollectiblesProduct) {
      return closure_9(tmp2(9172).CollectiblesShopPricePlaceholder, {});
    }
  }
  if (null == formattedPriceForCollectiblesProduct) {
    let tmp21 = null;
    ...
    const formattedPriceForCollectiblesProduct1 = tmp2(9159).getFormattedPriceForCollectiblesProduct(product, true, true);
    const tmp2Result = tmp2(9159);
    ...
    if (!(null != formattedPriceForCollectiblesProduct1 && canUseShopDiscountsResult)) {
      ...
      if (tmp28Result5) {
        ...
        obj17.style = items3;
        const items4 = [tmp28(tmp2(8972).NitroWheelIcon, obj17), ];
        ...
        items4[1] = tmp28(tmp2(4784).Text, obj18);
      ...
      if (shopDiscountSource === tmp2(7828).ShopDiscountSource.THIRDPARTY) {
        const obj23 = { color: "interactive-text-active", style: tmp.nitroIcon };
        let tmp28Result7 = tmp28(tmp2(9173).TagIcon, obj23);
      } else {
        const obj24 = { color: "interactive-text-active", style: tmp.nitroIcon };
        tmp28Result7 = tmp28(tmp2(8972).NitroWheelIcon, obj24);
      }
      obj21.icon = tmp28Result7;
      tmp28(tmp29, obj21);
      tmp2Result6 = tmp2(1364);
    }
    tmp2Result4 = tmp2(7828);
  }
};
