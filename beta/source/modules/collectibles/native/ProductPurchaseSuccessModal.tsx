...
// Dependencies: [32, 718, 19, 17, 4780, 1074, 21, 4788, 576, 1973, 11374, 5880, 5929, 1115, 4524, 5217, 4789, 4756, 7826, 11376, 4489, 5230, 8479, 11378, 504, 11379, 11380, 9162, 8635, 11386, 8472, 7400, 11387, 9110, 9123, 11405, 11514, 11515, 4784, 7828, 5218, 2]
// Exports: default

// Module 11375 (ProductPurchaseSuccessModal)
...
export default function ProductPurchaseSuccessModal(stageCollectibleChangeForEditProfile) {
  ...
  const backgroundColors = callback(11376)(product.styles).backgroundColors;
  let tertiary;
  if (backgroundColors != null) {
    tertiary = backgroundColors.tertiary;
  }
  ...
  if (useCategoryImage) {
    ...
    if (showOrbBalancePill) {
      ...
      showOrbBalancePill = tmp31(tmp2(11387).BalanceWidgetPill, obj12);
    }
    ...
    if (tmp2(1973).CollectiblesItemType.BUNDLE === type) {
      ...
      if (tmp30Result) {
        ...
        tmp30Result = tmp30(tmp4(9110), obj17);
      }
    ...
    if (null != renderMessages) {
      ...
      const items10 = [tmp31(tmp2(4784).Text, obj23), ];
      const obj25 = { variant: "text-md/medium", color: "text-overlay-light", style: tmp6.title, children: null };
      let result = tmp2(7828).isPremiumCollectiblesProduct(product);
