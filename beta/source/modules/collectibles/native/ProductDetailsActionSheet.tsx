...
// Dependencies: [32, 19, 17, 7816, 1076, 1074, 1085, 21, 3, 4788, 576, 5223, 4498, 4639, 1115, 7245, 1973, 8479, 8480, 8477, 7827, 9511, 7439, 7459, 13468, 11386, 9079, 9080, 1249, 1241, 504, 13469, 9143, 9145, 7828, 9147, 9148, 6901, 9150, 13470, 13480, 13485, 1177, 13486, 13497, 7427, 9185, 11030, 8533, 5218, 13500, 8475, 2]
// Exports: default

// Module 8478 (ProductDetailsActionSheet)
...
function PreviewProfileTrigger(handlePreviewPress) {
  ...
  dependencyMap = tmp;
  const theme = handlePreviewPress(4498).useThemeContext().theme;
  const obj = handlePreviewPress(4498);
  const isThemeLightResult = handlePreviewPress(4639).isThemeLight(theme);
  ...
  const obj2 = handlePreviewPress(4639);
  obj3.children = closure_13(handlePreviewPress(7245).EyeIcon, { size: "md", color: onTrackPress(576).colors.INTERACTIVE_ICON_DEFAULT });
...
function ManagedProductDetailsActionSheetInner(skuId) {
  ...
  const collectiblesShopProduct = skuId(9185).useCollectiblesShopProduct(skuId, { needsCategory: false, seedCategoryStore: true });
  const product = collectiblesShopProduct.product;
  dependencyMap = product;
  ({ state, retry } = collectiblesShopProduct);
  const obj = skuId(9185);
  const getOrFetchPurchases = skuId(11030).useGetOrFetchPurchases();
  ({ hasPreviouslyFetched, fetchPurchasesError } = getOrFetchPurchases);
  const obj2 = skuId(11030);
  const ref1 = noop.useRef(null);
  ...
  if ("ready" === state) {
    if (hasPreviouslyFetched) {
      if (null != product) {
        ...
        return closure_13(tmp(7427).BottomSheet, obj4);
      }
    }
  }
  if ("error" === state) {
    const obj5 = { Illustration: tmp(8533).NoResults, body: null, children: null };
    ...
    obj6.onPress = retry;
    obj5.children = closure_13(tmp(5218).Button, obj6);
    closure_13(tmp(1177).EmptyState, obj5);
  } else {
    closure_13(initialVariantIndex(13500), {});
  }
}
...
let size = { width: fn(5223).MEDIUM_BUTTON_HEIGHT, height: fn(5223).MEDIUM_BUTTON_HEIGHT, alignItems: "center", justifyContent: "center", borderRadius: nativeDefault.radii.round, borderWidth: 1, borderColor: nativeDefault.colors.CONTROL_SECONDARY_BORDER_DEFAULT };
