...
// Dependencies: [19, 17, 1076, 9111, 21, 4788, 576, 8527, 5836, 11309, 8502, 9135, 8472, 9110, 1970, 9131, 1973, 1077, 9157, 9156, 9123, 1115, 4784, 7827, 9159, 11310, 8479, 4446, 9175, 2]
// Exports: default

// Module 11308 (CollectiblesShopCheckoutDetails)
...
function CollectibleProductPreviewContent(userAvatarSource) {
  ...
  if (ALL.has(product.skuId)) {
    const size = { skuId: product.skuId, width: tmp2(9157).FRACTIONAL_NITRO_COIN_SIZE.CHECKOUT, height: tmp2(9157).FRACTIONAL_NITRO_COIN_SIZE.CHECKOUT };
    return React5(tmp2(9157).FractionalNitroCoinIllustration, size);
  } else if (product.skuId === EXTERNAL_PRODUCT_SKU_IDS.ORB_PROFILE_BADGE) {
    const obj = { source: null, style: null };
    const obj2 = { uri: _modDef9156 };
    obj.source = obj2;
...
function ProductPriceAmountTag(product) {
  ...
  const memo = noop.useMemo(() => {
    if (useOrbPrice) {
      ...
      const orbPrice = tmp(7827).getProductOrbPrice(obj);
      const tmpResult = tmp(7827);
    } else {
      const priceText = tmp(9159).getFormattedPriceForCollectiblesProduct(product, hasShopDiscount, true);
      const tmpResult2 = tmp(9159);
    }
...
const CollectiblesPreviewConstants = fn(9111);
...
export default function CollectiblesShopCheckoutDetails(recipientUser) {
  ...
  const canUseShopDiscountsResult = flag2(4446).canUseShopDiscounts(currentUser);
  ...
  const memo = noop.useMemo(() => {
    ...
    if (flag2) {
      ...
      const productOrbPrice = tmp(7827).getProductOrbPrice(obj2);
      let str2;
      ...
      str = tmp(9159).getFormattedPriceForCollectiblesProduct(tmp3, hasShopDiscount, true);
      if (str == null) {
        str = "";
      }
      const tmpResult2 = tmp(9159);
  ...
  if (flag3) {
    ...
    obj5.children = tmp10(tmp3(4784).Text, obj6);
