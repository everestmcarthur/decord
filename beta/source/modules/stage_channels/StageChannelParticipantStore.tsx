...
// Dependencies: [4658, 502, 1957, 1979, 4275, 4285, 5500, 1371, 4655, 4660, 5502, 1962, 4271, 12, 5506, 4688, 1330, 504, 573, 2]

// Module 5499 (StageChannelParticipantStore)
import _modDef12 from "module_12" /* 12 */;
import initializeDefault from "initialize" /* 504 */;
import DispatcherDefault from "Dispatcher" /* 573 */;
import _modDef1330 from "module_1330" /* 1330 */;
import StageChannelParticipantsDefault from "StageChannelParticipants" /* 5506 */;
...
function updateParticipant(arg0) {
  ...
  f80033 = (dependencyMap) => dependencyMap.updateParticipant(f80033);
  if (mapped === undefined) {
...
function handleUserUpdate(user) {
  ...
  const f80033 = (dependencyMap) => dependencyMap.updateParticipant(f80033);
  let mapped1 = mapped;
  if (mapped === undefined) {
    const values2 = secondaryIndexMap.values(undefined, true);
...
function handleStreamClose(streamKey) {
  const decodeStreamKeyResult = f80033(4688).decodeStreamKey(streamKey.streamKey);
...
const secondaryIndexMap = new fn(4271).SecondaryIndexMap((getGuildId) => {
...
const stageChannelParticipantStore = new StageChannelParticipantStore(DispatcherDefault, {
  ...
  CHANNEL_UPDATES: function handleChannelUpdate(channels) {
    ...
    const reduced = channels.reduce((arr, isGuildStageVoice) => {
      if (isGuildStageVoice.isGuildStageVoice()) {
        if (set.has(isGuildStageVoice.guild_id)) {
          ...
          let tmp4 = null == value;
          if (!tmp4) {
            tmp4 = _modDef1330(isGuildStageVoice.permissionOverwrites, value.permissionOverwrites);
          }
          if (!tmp4) {
    ...
    const f80037 = (rebuild) => rebuild.rebuild();
    let mapped = reduced;
