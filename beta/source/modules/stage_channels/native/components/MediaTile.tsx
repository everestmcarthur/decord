...
// Function ID: 10353
// Name: MediaTile
// Dependencies: [19, 17, 4804, 4809, 21, 4788, 576, 10342, 1478, 5374, 504, 10353, 1177, 2]

// Module 10352 (MediaTile)
...
export default noop.memo((channel) => {
  ...
  const tmp = closure_7();
  const speakerTileStyles = channel(10342).useSpeakerTileStyles();
  const obj = channel(10342);
  const tmp5 = participant;
  const isScreenLandscape = channel(5374).useIsScreenLandscape();
  const obj2 = channel(5374);
  ...
  if (null != stateFromStores) {
    ...
    if (stateFromStores.type !== ParticipantTypes.ACTIVITY) {
      ...
      const items2 = [tmp.container, tmp2(10342).getSizeStyle(size, speakerTileStyles), ];
      const tmp2Result = tmp2(10342);
      items2[2] = tmp2(10342).getTileWidthStyle(size, participant(1478)().width, isScreenLandscape);
      obj4.style = items2;
      ...
      const tmp2Result2 = tmp2(10342);
      obj5.avatarSize = tmp2(1177).AvatarSizes.XLARGE;
      obj5.channel = channel;
      obj5.contentStyle = tmp.media;
      obj4.children = jsx(tmp5(10353), { hasBottomSafeArea: false, hasLeftSafeArea: false, hasRightSafeArea: false, hasTopSafeArea: false, participant: stateFromStores, avatarSize: null, channel: null, shrinkStreamEmptyState: false, contentStyle: null });
      tmp8 = <View style={null}>{null}</View>;
      const tmp5Result = tmp5(10353);
    }
  }
  return tmp8;
});
