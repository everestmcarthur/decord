...
// Dependencies: [32, 19, 17, 4437, 5670, 5663, 21, 4788, 576, 504, 1115, 1177, 8929, 4784, 4755, 8930, 7400, 5218, 7427, 7349, 2]
// Exports: default

// Module 8928 (StageBlockedUsersActionSheet)
...
function RestrictedUser(guildId) {
  ...
  if (speaker) {
    ...
    obj6.style = items3;
    const obj7 = { style: tmp.icon, source: user(8929), color: user(576).unsafe_rawColors.WHITE };
...
let closure_8 = fn(5663).STAGE_BLOCKED_USERS_SHEET_KEY;
const jsxProd = fn(21);
...
export default function StageBlockedUsersActionSheet(channel) {
  ...
  const stageBlockedUsers = channel(8930).useStageBlockedUsers(channel.id);
  const obj = channel(8930);
  const stageIgnoredUsers = channel(8930).useStageIgnoredUsers(channel.id);
  ...
  const items = [closure_9(channel(5218).Button, obj4), ];
  ...
  obj5.onPress = function handleDismiss() {
    onAccept(4755).hideActionSheet(closure_1_8);
  };
  items[1] = closure_9(channel(5218).Button, obj5);
  obj3.children = items;
  items1 = [];
  const obj2 = channel(8930);
  HermesBuiltin.arraySpread(stageIgnoredUsers, HermesBuiltin.arraySpread(stageBlockedUsers, 0));
  const obj6 = { scrollable: true, header: closure_9(StageBlockedUsersActionSheetHeader, { blockedUserCount: stageBlockedUsers.length, ignoredUserCount: stageIgnoredUsers.length }), footer: closure_10(channel(7400).SafeAreaPaddingView, obj3), children: null };
  ...
  const tmp4 = closure_10(channel(7400).SafeAreaPaddingView, obj3);
  ...
  const items3 = [closure_9(onAccept(7349), obj7), closure_9(View, { style: { height: tmp2[0] } })];
  obj6.children = items3;
  return closure_10(channel(7427).BottomSheet, obj6);
};
