...
// Dependencies: [32, 19, 17, 1182, 1372, 8734, 8723, 1074, 21, 8222, 4788, 504, 4639, 12176, 12177, 8722, 4938, 8562, 8611, 12178, 8962, 5010, 11, 2]
// Exports: default

// Module 12175 (ClassificationMessageEvidence)
...
export default function ClassificationEvidence(arg0) {
  ...
  if (obj3.useStateFromStores(items1, () => ref(4639).isThemeLight(id.theme))) {
    let assetSource = resolveAssetSource(tmp11(12176));
    let tmp13 = tmp11;
  } else {
    assetSource = resolveAssetSource(tmp11(12177));
    tmp13 = tmp11;
  }
  ...
  memo = obj2.useMemo(() => {
    ...
    return attachments.map((filename) => {
      ...
      obj.filename = str;
      obj.flags = ref(8722).getSpoilerFlagsForAttachment(filename);
      obj.size = 0;
  ...
  if (first3 > 0) {
    ...
    first2 = attachments.filter((filename) => ref(4938).isVideoFile(filename.filename));
    obj5.children = first2.map((uri, index) => {
      closure_0 = uri;
      return closure_1_18(first(8611), {
        source: { uri: uri.url },
        onLoad(arg0) {
          closure_0 = arg0;
          closure_1_4((arg0) => {
