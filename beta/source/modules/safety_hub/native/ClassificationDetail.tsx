...
// Dependencies: [19, 17, 2109, 8734, 8723, 1074, 21, 4788, 576, 4784, 8724, 1115, 504, 3098, 10043, 4483, 9548, 5854, 5218, 12164, 12166, 8733, 8716, 1241, 12167, 5116, 5121, 12169, 7400, 12174, 8722, 2]
// Exports: default

// Module 12163 (ClassificationDetail)
...
function ClassificationHeader(classificationTypeText) {
  ...
  obj.children = closure_13(classificationTypeText(4784).Text, { variant: "text-lg/normal", style: tmp.headerText, color: "mobile-text-heading-primary", children: memo });
...
function ManualReviewDecidedUnderageActionsTaken() {
  ...
  obj4.children = intl2.string(_modDef3098.rn3Gto);
...
function ManualReviewDecidedUnderageGuidance() {
  ...
  const obj4 = { large: true, children: null };
  const intl2 = util.intl;
  obj4.children = intl2.string(_modDef3098["yV/t/V"]);
  obj3.children = map1(BulletRow, obj4);
  items[1] = map1(React4, obj3);
  obj.children = items;
  return closure_1_14(React4, obj);
...
function ClassificationPolicyCard(classificationDescription) {
  ...
  obj3.children = closure_13(policyExplainerLink(9548).ShieldIcon, { size: "sm", color: nativeDefault.colors.TEXT_LINK });
  ...
  obj5.children = closure_13(policyExplainerLink(4784).Text, obj6);
...
function ManualReviewDecidedUnderageFooter(arg0) {
  ...
  const obj = { variant: "text-sm/normal", color: "text-muted", children: null };
  const intl = util.intl;
  obj.children = intl.format(_modDef3098.vPOpia, { tosLink, communityGuidelinesLink });
  return map1(Text_Text.Text, obj);
}
  ...
  if (null == classification) {
    ...
    if (tmp16) {
      ...
      function onPressLetUsKnow() {
        ...
        AnalyticsUtilsDefault.track(AnalyticEvents.SAFETY_HUB_ACTION, obj3);
        if (hasItem1) {
          tmp(12167).openV2(tmp3, onClose);
          const tmpResult = tmp(12167);
        } else if (hasItem) {
          tmp(12167).open(tmp3, onClose);
          const tmpResult5 = tmp(12167);
        } else if (isAppealEligible) {
          const obj4 = { name: MetricEvents.MetricEvents.APPEAL_INGESTION_VIEW };
          tmp(5116).increment(obj4);
          const tmpResult6 = tmp(5116);
          const obj5 = { classificationId: tmp3 };
          tmp(12169).open(obj5);
          const tmpResult7 = tmp(12169);
        } else {
          tmp(4483).openURL(constants.APPEALS_LINK);
          const tmpResult8 = tmp(4483);
        }
      }
