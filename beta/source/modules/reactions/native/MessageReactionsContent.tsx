...
// Dependencies: [32, 19, 17, 4780, 2041, 2105, 8036, 4427, 1074, 21, 4524, 4784, 4788, 576, 504, 4439, 1331, 8037, 8038, 5847, 12, 7439, 11552, 4940, 4632, 8480, 7414, 1177, 1397, 9933, 1364, 5929, 8909, 11553, 1115, 11554, 4637, 2019, 7407, 11555, 4789, 1478, 4745, 4755, 11556, 1980, 7486, 7465, 11557, 4756, 4757, 1612, 8533, 7427, 6901, 510, 7543, 8058, 8099, 11558, 7349, 2]
// Exports: MessageReactionsContent, MessageReactionsEmpty

// Module 11551 (MessageReactionsContent)
...
import ReactionToProfileExperimentDefault from "ReactionToProfileExperiment" /* 11552 */;
...
function ReactionTab(arg0) {
  ...
  if (null != emojiColorPalette) {
    const obj3 = { backgroundColor: tmp3(4637).hexOpacityToRgba(emojiColorPalette.backgroundColor, emojiColorPalette.opacity) };
    tmp8 = obj3;
...
function EmojiOptionsButton(channelId) {
  ...
  const tmp3 = messageId(11555)(stateFromStores);
  const canRemoveReactions = tmp3;
  dependencyMap = tmp4;
  const items2 = [channelId, messageId, reactions[reactionSelectedIndex], tmp3];
  const obj2 = {
    onPress: noop.useCallback(() => {
      ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(11556, dependencyMap.paths), "ReactionEmojiOptionsActionSheet", { channelId, messageId, reaction, canRemoveReactions }, "replaceTopSheet");
    }, items2),
  ...
  const items3 = [closure_17(channelId(4784).Text, obj3), ];
  const obj = channelId(504);
  items3[1] = closure_17(channelId(7486).ChevronSmallRightIcon, { color: messageId(576).colors.ICON_SUBTLE, size: "xs" });
...
function ReactionTabs(setReactionSelectedIndex) {
  ...
  const obj = setReactionSelectedIndex(7465);
  ...
  obj4.initialNumTabsToRender = reactionSelectedIndex(4439).MAX_REACTIONS;
  const items = [closure_17(setReactionSelectedIndex(11557), obj4), closure_17(setReactionSelectedIndex(8909), { outer: true }), ];
  ...
  const tmp5 = setReactionSelectedIndex(11557);
  obj7.children = reactionSelectedIndex(4439).getReactionEmojiName(reactions[reactionSelectedIndex].emoji);
  const items1 = [closure_17(reactionSelectedIndex(4784).Text, obj7), ];
  const obj12 = {};
  const obj6 = reactionSelectedIndex(4439);
...
let closure_19 = ReanimatedRexport.createAnimatedComponent(fn(4784).Text);
...
export const MessageReactionsContent = function MessageReactionsContent(channelId) {
  ...
  if (null != reactions[bound].me_vote) {
    let NORMAL = channelId(8037).ReactionTypes.VOTE;
    let tmp8 = channelId;
    let tmp10 = channelId;
  } else if (tmp5.burst_count > 0) {
    NORMAL = channelId(8037).ReactionTypes.BURST;
    tmp8 = channelId;
    tmp10 = channelId;
  } else {
    NORMAL = channelId(8037).ReactionTypes.NORMAL;
    tmp8 = channelId;
    tmp10 = channelId;
  }
  let tmp2 = closure_20();
  ...
  const isActiveChannelOrUnarchivableThread = tmp10(7543).useIsActiveChannelOrUnarchivableThread(stateFromStores);
  closure_130_0 = channelId;
  const tmp10Result3 = tmp10(7543);
  ...
  const analyticsLocations = tmp19(7439)().analyticsLocations;
  ...
  let obj3 = { scrollable: true, backdropOpacity: tmp10(8058).BACKDROP_OPACITY, backdropChildren: null, header: null, children: null };
  let tmp27Result = null;
  if (NORMAL === tmp10(8037).ReactionTypes.BURST) {
    ...
    obj4.children = tmp27(tmp19(8099), obj5);
  ...
  if (tmp26) {
    ...
    let tmp27Result4 = tmp27(tmp19(11558), obj7, name);
    const tmp19Result3 = tmp19(11558);
  } else {
    const obj8 = { inActionSheet: true };
    const merged1 = Object.assign(obj2);
    tmp27Result4 = tmp27(tmp19(7349), obj8, name);
    const tmp19Result4 = tmp19(7349);
  }
  obj3.children = tmp27Result4;
  return closure_17(tmp10(7427).BottomSheet, obj3);
};
