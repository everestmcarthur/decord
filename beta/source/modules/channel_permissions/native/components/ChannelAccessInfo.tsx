...
// Dependencies: [19, 17, 2059, 2105, 2099, 21, 4788, 576, 1115, 504, 9855, 1370, 4784, 5371, 11888, 1177, 12753, 5339, 9874, 9872, 10236, 2]
// Exports: default

// Module 12754 (ChannelAccessInfo)
...
export default function ChannelAccessInfo(guild) {
  ...
  let obj = guild(504);
  const existingMembers = guild(9855).getExistingMembers(memberIds, channel, guild, channel.accessPermissions);
  ...
  const items2 = [closure_8(guild(4784).Text, { variant: "eyebrow", children: stringResult }), ];
  let obj2 = {
  ...
  if (null != first) {
    ...
    function renderCounts(MEMBERS, length, arg2, GroupIcon) {
      if (0 === length) {
        ...
        items[1] = React6(tmp4(4784).Text, obj8);
        obj6.children = items;
    ...
    channel(12753);
    const items5 = [renderCounts(MEMBERS, existingMembers.length, 0, tmp2(5339).GroupIcon), ];
    const ROLES = constants.ROLES;
    channel(9874);
    items5[1] = renderCounts(ROLES, stateFromStoresArray.length, 0, tmp2(9872).ShieldUserIcon);
    obj9.children = items5;
  }
  const obj10 = { children: null };
  obj3.children = closure_9(closure_10, obj9);
  const items6 = [closure_8(View, obj3), ];
  const tmp2Result = guild(9855);
  items6[1] = closure_8(guild(1177).Icon, { source: channel(10236), size: guild(1177).Icon.Sizes.SMALL });
  obj2.children = items6;
  items2[1] = closure_9(guild(5371).PressableOpacity, obj2);
