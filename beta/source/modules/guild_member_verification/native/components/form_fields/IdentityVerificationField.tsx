...
// Dependencies: [19, 17, 21, 4788, 576, 4784, 1115, 4738, 5218, 4612, 5870, 4991, 7319, 1980, 7322, 7358, 7235, 2]
// Exports: default

// Module 5869 (IdentityVerificationField)
...
function BaseIdentityVerificationField(icon) {
  ...
  if (passesVerification) {
    ...
    obj4.children = tmp6(tmp7(4738).CheckmarkLargeIcon, { color: "status-positive" });
    let obj5 = obj4;
  } else {
    ...
    obj5.children = tmp6(tmp7(5218).Button, obj6);
  }
...
export default function IdentityVerificationField(arg0) {
  ...
  if (MemberVerificationTypes.UserVerificationFieldPlatforms.EMAIL === platform) {
    ...
    if (tmp(4612).UserVerificationFieldPlatforms.PHONE === platform) {
    ...
    if (tmp(4612).UserVerificationFieldPlatforms.EMAIL === platform) {
      let EnvelopeIcon = tmp(7358).EnvelopeIcon;
    } else if (tmp(4612).UserVerificationFieldPlatforms.PHONE === platform) {
      EnvelopeIcon = tmp(7235).MobilePhoneIcon;
    } else {
      EnvelopeIcon = tmp(7358).EnvelopeIcon;
    }
    if (tmp(4612).UserVerificationFieldPlatforms.EMAIL === platform) {
      let fn = () => {
      ...
      fn = tmp(4612).UserVerificationFieldPlatforms.PHONE === platform ? (() => {
