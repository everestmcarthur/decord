...
// Function ID: 17602
// Name: VoicePanelCard
// Dependencies: [32, 19, 17, 4810, 4811, 5668, 12555, 12553, 17558, 12558, 1074, 4809, 12556, 21, 4524, 4784, 5230, 1177, 4788, 576, 4930, 4840, 5838, 1115, 5218, 12554, 504, 9716, 4843, 13360, 9719, 9715, 17602, 9726, 9139, 8552, 4789, 5217, 7350, 5836, 4489, 17603, 11288, 9696, 4498, 7439, 17561, 17573, 17557, 17604, 17605, 8480, 6929, 17606, 12557, 17607, 17608, 17609, 17610, 17611, 17621, 2]

// Module 17601 (VoicePanelCard)
...
import computeCardBorderRadiusDefault from "computeCardBorderRadius" /* 17603 */;
import calculateContentCenterOffsetDefault from "calculateContentCenterOffset" /* 17604 */;
  ...
  const fn4 = function w(mode, transitionState) {
    if (!obj.cheapWorkletShallowEqual(mode, transitionState)) {
      ...
      if (null == transitionState) {
        if (transitionState !== tmp(4498).TransitionStates.YEETED) {
      ...
      if (transitionState === tmp(4498).TransitionStates.YEETED) {
        ...
        tmp(4524).runOnJS(isRTCConnected)();
        ...
        if (transitionState1 === tmp(4498).TransitionStates.YEETED) {
...
let closure_23 = ReanimatedRexport.createAnimatedComponent(fn(4784).Text);
let ReanimatedRexport = ReanimatedRexport_mod;
...
layoutTransitionFunction.__closure = { withSpring: fn(5217).withSpring };
...
let obj9 = { withSpring: fn(5217).withSpring };
size = fn(2);
