...
// Dependencies: [19, 17, 4780, 2040, 4810, 4811, 12555, 12553, 12558, 1074, 4809, 21, 4524, 1177, 4788, 576, 12554, 17577, 17622, 10360, 17623, 17575, 1115, 10283, 9972, 9971, 5838, 6884, 5217, 17573, 4930, 4840, 9608, 17503, 17624, 17625, 504, 17607, 7350, 17626, 4498, 17627, 10031, 8981, 5359, 8979, 8989, 10045, 17628, 5203, 7439, 8480, 9983, 5021, 10028, 4784, 10078, 17549, 2]

// Module 17621 (VoicePanelCardFloatingControls)
...
import useShouldOpenGameProfileModal from "useShouldOpenGameProfileModal" /* 8979 */;
...
import getRandomNumberInRangeDefault from "getRandomNumberInRange" /* 17607 */;
...
const useStableParticipant = tmp(17573);
require = fn;
function StreamIcon(voicePlatform) {
  ...
  const animatedStyle = controlsSpecs(4524).useAnimatedStyle(fn);
  let tmp2Result = _modDef17577;
  if (voicePlatform === constants2.XBOX) {
    tmp2Result = tmp2(17622);
  } else if (voicePlatform === tmp6.MOBILE) {
    tmp2Result = tmp2(10360);
  } else if (voicePlatform === tmp6.QUEST) {
    tmp2Result = tmp2(17623);
  }
...
function AnimatedLabelIcon(icon) {
  ...
  if (useVoicePanelCardUserStateIcons.VoicePanelCardUserStateIconType.STREAM_ICON === type) {
  ...
  } else if (tmp2(17575).VoicePanelCardUserStateIconType.SPEAKER_MUTE_ICON === type) {
    ...
    obj3.children = value2(tmp2(10283).VoiceXIcon, obj4);
    return value2(Pressable, obj3);
  } else if (tmp2(17575).VoicePanelCardUserStateIconType.USER_VIDEO_ICON === type) {
    if (icon.videoIconState === tmp2(9972).VideoIconState.VIDEO_DISABLED_LOCAL_AUTO) {
    ...
    if (null != icon.onPress) {
      ...
      obj5.children = value2(tmp2(9971).VideoIcon, obj6);
      ...
      obj7.children = value2(tmp2(9971).VideoIcon, obj8);
      tmp16 = value2(NativeViewDefault, obj7);
    }
    return tmp16;
  } else if (tmp2(17575).VoicePanelCardUserStateIconType.MUTE_DEAFEN_ICON === type) {
    ...
    obj10.children = value2(tmp2(9971).MuteDeafenIcon, obj11);
    obj9.children = value2(Pressable, obj10);
    return value2(NativeViewDefault, obj9);
  } else if (tmp2(17575).VoicePanelCardUserStateIconType.USER_DISCONNECTED_ICON === type) {
    ...
    obj.children = value2(tmp2(6884).CircleErrorIcon, obj12);
...
const VOICE_PANEL_CARD_INNER_PADDING = fn(12558).VOICE_PANEL_CARD_INNER_PADDING;
...
let closure_27 = noop.memo((participant) => {
  ...
  guildId = noop.useContext(guildId(12554)).guildId;
  ...
  let obj = participant(4524);
  fn.__closure = { controlsHidden, FLOATING_BAR_HEIGHT: v28, VOICE_PANEL_CARD_INNER_PADDING, withSpring: participant(5217).withSpring, MODE_CHANGE_PHYSICS };
  ...
  const callback = noop.useCallback(() => {
    if (obj.isStableStreamParticipant(participant)) {
      ...
      if (null != streamForUser) {
        ...
        tmpResult.stopStream(tmp(4840).encodeStreamKey(streamForUser));
  ...
  let obj2 = { controlsHidden, FLOATING_BAR_HEIGHT: v28, VOICE_PANEL_CARD_INNER_PADDING, withSpring: participant(5217).withSpring, MODE_CHANGE_PHYSICS };
  const tmp3 = closure_16;
  obj3.icon = guildId(17624);
  obj3.onPress = callback;
  obj3.style = animatedStyle;
  obj3.layout = layout;
  const tmp4 = guildId(17503);
  const result = participant(17573).isStableActivityParticipant(participant);
...
let closure_39 = noop.memo((arg0) => {
  ...
  noop = undefined;
  const showGameTag = gameRecord(17627).useConfig({ location: "VoicePanelCardFloatingControls" }).showGameTag;
  const first = gameRecord(10031)(userId, arg0.guildId, showGameTag)[0];
  let tmp5;
  let obj = gameRecord(17627);
  if (showGameTag) {
    let application_id;
    if (first != null) {
  ...
  gameRecord = gameRecord(8981)({ applicationId: tmp5 }).gameRecord;
  const tmp8 = gameRecord(5359)(gameRecord);
  dependencyMap = tmp8;
  ...
  if (showGameTag) {
    if (null != gameRecord) {
      if (!tmp8) {
        ...
        let tmp12 = closure_16(tmp(17628), obj2);
      }
      return tmp12;
    }
  }
  tmp12 = closure_16(tmp(10045), { userId, textColor });
});
