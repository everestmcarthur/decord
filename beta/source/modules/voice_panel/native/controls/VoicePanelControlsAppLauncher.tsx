...
// Dependencies: [19, 2041, 12553, 1074, 21, 4788, 8148, 12554, 8570, 504, 38, 12562, 1110, 11510, 9555, 4524, 12364, 12563, 1115, 2]

// Module 17688 (VoicePanelControlsAppLauncher)
...
export default noop.memo(function VoicePanelControlsAppLauncher(gestureSpecs) {
  ...
  const gradientTop = gestureSpecs(8148).useGradientTop();
  const context = noop.useContext(setControlsMode(12554));
  setControlsMode = context.setControlsMode;
  ({ channelId: c2, safeArea, windowDimensions } = context);
  const tmp4 = setControlsMode(8570)(windowDimensions);
  noop = tmp4;
  const rect = setControlsMode(8570)(safeArea);
  const obj = gestureSpecs(8148);
  const items = [rect];
  ...
  const controlsDrawerOpenWidth = gestureSpecs(12562).getControlsDrawerOpenWidth(tmp4.width, rect.left, rect.right);
  const callback = noop.useCallback(() => {
    const ComponentDispatch = ComponentDispatchUtils.ComponentDispatch;
    ComponentDispatch.dispatch(ComponentActions.SELECT_ACTIVITY);
    setControlsMode({ mode: hasOwnProperty.HIDDEN });
  }, items1);
  const obj3 = gestureSpecs(12562);
  const appLauncherChatInputRefDummy = gestureSpecs(11510).useAppLauncherChatInputRefDummy({ noop: true });
  const obj4 = gestureSpecs(11510);
  const ref = noop.useRef(gestureSpecs(11510).AppLauncherKeyboardCloseReason.DISMISSED);
  const ref1 = noop.useRef(undefined);
  const sharedValue = gestureSpecs(4524).useSharedValue(0);
  const obj5 = gestureSpecs(4524);
  const sharedValue1 = gestureSpecs(4524).useSharedValue(0);
  const obj6 = gestureSpecs(4524);
  const fn = function y() {
  ...
  const derivedValue = gestureSpecs(4524).useDerivedValue(fn);
  const obj7 = gestureSpecs(4524);
  class E {
    constructor() {
  ...
  derivedValue1 = gestureSpecs(4524).useDerivedValue(E);
  const obj8 = gestureSpecs(4524);
  class L {
    constructor() {
      if (closure_5.get()) {
        tmp2 = closure_3;
  ...
  items3[1] = closure_7(setControlsMode(12563), obj13);
  obj11.children = items3;
  obj10.children = closure_8(setControlsMode(4524).View, obj11);
