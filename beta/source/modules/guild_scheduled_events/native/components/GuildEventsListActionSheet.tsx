...
// Dependencies: [19, 17, 4803, 2047, 1074, 4970, 21, 4788, 9793, 7426, 1115, 9835, 9815, 9782, 9911, 9919, 5235, 1241, 7387, 7427, 10101, 5835, 2]
// Exports: default
...
function GuildEventsListHeader(arg0) {
  ...
  if (tmp3Result) {
    ...
    tmp3Result = tmp3(tmp4(9835).ActionSheetHeaderPressableText, obj3);
  }
  obj2.trailing = tmp3Result;
  return jsx(guild(7426).BottomSheetTitleHeader, { title: formatToPlainStringResult, trailing: null });
}
...
export default function GuildEventsListActionSheet(guild) {
  ...
  const callback = noop.useCallback(() => {
    const result = guild(9815).closeGuildEventListActionSheet();
  }, []);
  const callback1 = noop.useCallback((eventId, recurrenceId) => {
    let result = guild_scheduled_events_GuildScheduledEventModalActionCreators.openGuildEventDetails({
      ...
      recurrenceId,
      onClose() {
        const result = guild(9815).openGuildEventListActionSheet(closure_1_0);
      }
    });
  }, items1);
  events(5235)(() => {
  ...
  obj4.lastAckedId = events(5835)(ref);
  obj3.children = jsx(events(10101), { inActionSheet: true, events, onPressEvent: callback1, onCloseAction: callback, guild, lastAckedId: null });
  obj.children = <View style={tmp.container}>{null}</View>;
  return jsx(guild(7427).BottomSheet, { showGradient: true, scrollable: events.length > 0, startExpanded: true, dismissAccessibilityLabel: null, header: null, children: null });
};
