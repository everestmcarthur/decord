...
// Dependencies: [19, 17, 7405, 18049, 1074, 5657, 21, 4788, 576, 5656, 12, 6884, 4784, 1115, 5216, 5371, 2108, 5934, 5937, 18077, 5218, 11499, 4755, 18079, 1980, 18080, 18082, 504, 8903, 18063, 2]
// Exports: default

// Module 18076 (GuildSettingsRoleEditConnectionsControls)
...
import GuildSettingsRoleEditConnectionConfigurationDefault from "GuildSettingsRoleEditConnectionConfiguration" /* 18077 */;
...
function HeaderSection(arg0) {
  ...
  if (hasConnections) {
    ...
    obj6.children = tmp13(tmp10(4784).Text, obj7);
    tmp13Result = tmp13(tmp10(5371).PressableOpacity, obj6);
  }
...
function AndOrRadios(setPendingRoleConfigurations) {
  ...
  if (ConnectionsUtils.ConnectionConfigurationRuleOperator.OR === ConnectionsUtils.ConnectionConfigurationRuleOperator.AND) {
    ...
    const obj3 = { value: tmp(5656).ConnectionConfigurationRuleOperator.OR, label: null, disabled: null };
    ...
    obj3.disabled = tmp11;
    const items2 = [closure_1_10(tmp(5937).TableRadioRow, obj3), ];
    const obj4 = { value: tmp(5656).ConnectionConfigurationRuleOperator.AND, label: null, disabled: null };
    const intl3 = tmp(1115).intl;
    obj4.label = intl3.string(tmp(1115).t.gHXS9A);
    obj4.disabled = locked;
    items2[1] = closure_1_10(tmp(5937).TableRadioRow, obj4);
    obj2.children = items2;
    return closure_1_11(tmp(5934).TableRadioGroup, obj2);
  }
}
...
function AddConnectionButton(locked) {
  ...
  obj.onPress = function onPress() {
    ...
    obj.openLazy(asyncRequireImpl(18079, dependencyMap.paths), combined, obj2);
  };
...
export default function GuildSettingsRolesEditConnectionsControls(guild) {
  ...
  const obj9 = {
    handleConnectionTapped(connectionType, applicationId) {
      const items = [...memo];
      const obj = { connectionType, connectionMetadataField: "Array", applicationId, operator: "formatToPlainString", value: 1090584578 };
      items.push(obj);
      if (AND === ConnectionsUtils.ConnectionConfigurationRuleOperator.AND) {
        if (0 === items.length) {
          let items1 = [];
