...
// Dependencies: [32, 19, 17, 1182, 2100, 502, 4707, 2099, 2063, 4427, 7405, 18043, 18044, 1074, 21, 4788, 576, 5773, 1241, 18045, 504, 16486, 9887, 1484, 7220, 4432, 4968, 18046, 18055, 18056, 5936, 1115, 5371, 12433, 4784, 4639, 18057, 18058, 18059, 18060, 5836, 5218, 18061, 7651, 13055, 1364, 18053, 5769, 7406, 7327, 8903, 1177, 9874, 16720, 7317, 2]
// Exports: default

// Module 18042 (GuildSettingsRoles)
...
import GuildSettingsRolesManager from "GuildSettingsRolesManager" /* 18045 */;
import GuildSettingsRolesUtils from "GuildSettingsRolesUtils" /* 18053 */;
import actions_GuildActionCreators from "actions/GuildActionCreators" /* 18055 */;
import GuildSettingsModalRolesActionCreatorsDefault from "GuildSettingsModalRolesActionCreators" /* 18056 */;
import GuildSettingsRoleItemDefault from "GuildSettingsRoleItem" /* 18061 */;
...
import GuildSettingsModalRolesStore from "GuildSettingsModalRolesStore" /* 18043 */;
import TextStyles from "TextStyles" /* 5773 */;

const GuildSettingsRoleCreateModalActionCreatorsDefault = tmp(18046);
require = fn;
get_ActivityIndicator = fn(17);
({ View: metroRequire, StyleSheet } = get_ActivityIndicator);
const isEveryoneRole = fn(2100).isEveryoneRole;
let closure_16 = fn(18044).GuildSettingsRoleEditSections;
...
export default function ConnectedGuildSettingsModalRoles(guildId) {
  ...
  const guildSettingsRolesManagerState = guildId(18045).useGuildSettingsRolesManagerState((roleJustCreated) => roleJustCreated.roleJustCreated);
  ...
  const callback8 = obj.useCallback(() => {
    ...
    if (!first) {
      ...
      if (!closure_16) {
        ...
        const items2 = [tmp6(tmp7(12433).ArrowsUpDownIcon, obj5), ];
        ...
        items2[1] = tmp6(tmp7(4784).Text, obj6);
        obj4.children = items2;
        tmpResult = tmp(tmp7(5371).PressableOpacity, obj4);
      }
    }
    items1[1] = tmpResult;
    obj.children = items1;
    ...
    if (first) {
      ...
      tmp6Result = tmp6(tmp7(4784).Text, obj7);
    }
  ...
  const callback9 = obj.useCallback(() => {
    ...
    if (closure_3) {
      if (isThemeDarkResult) {
        ...
        tmp3Result = tmp3(18058);
      }
    } else {
      if (isThemeDarkResult) {
        let tmp3Result2 = tmp3(18059);
      } else {
        tmp3Result2 = tmp3(18060);
      }
      if (hasRoles) {
  ...
  const callback11 = obj.useCallback((role, from) => {
    if (null == guild) {
      ...
      obj2.onMoveDown = fn2;
      return tmp22(closure_1(18061), obj2, role.id);
    }
  }, items19);
  ...
  const effect2 = obj.useEffect(() => {
    ...
    if (first) {
      fn = () => {
        ...
        return hasRoles(guildId(7651).HeaderActionButton, obj);
      };
    }
    ...
    if (first) {
      ...
      fn2 = () => {
        const obj = { onPress, source: closure_1(13055), accessibilityLabel: null };
        const intl = guildId(1115).intl;
  ...
  if (!tmp25) {
    ...
    obj8.children = hasRoles(tmp3(7327).SearchField, obj9);
  ...
  if (sorting) {
    ...
    if (!hasRoles) {
      ...
      let obj11 = { style: tmp.emptyRolesIcon, size: tmp3(1177).Icon.Sizes.LARGE, source: tmp6(9874) };
      ...
      obj10.label = tmp47(tmp3(4784).Text, obj12);
      tmp47Result = tmp47(tmp3(8903).FormRow, obj10);
    }
  ...
  items24[3] = hasRoles(guildId(7317).NavScrim, {});
