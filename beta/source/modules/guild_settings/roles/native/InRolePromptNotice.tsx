...
// Dependencies: [19, 17, 1074, 21, 4788, 576, 1385, 18072, 1177, 9745, 4784, 1115, 2]
// Exports: default

// Module 18071 (InRolePromptNotice)
...
export default function InRolePromptNotice(role) {
  ...
  if (!obj.hasFlag(role.flags, RoleFlags.IN_PROMPT)) {
    ...
    const tmpResult = tmp(18072);
    let Icon = tmp(1177).Icon;
    if (isRolePowerfulResult) {
      const obj3 = { style: string.icon, source: _modDef9745, color: nativeDefault.unsafe_rawColors.YELLOW_300 };
      ...
      Icon = tmp6(tmp(4784).Text, obj4);
      items[1] = Icon;
      obj2.children = items;
      let tmp4Result = tmp4(tmp5, obj2);
    } else {
      const obj5 = { style: string.icon, source: _modDef9745 };
      ...
      items1[1] = tmp6(tmp(4784).Text, obj6);
      obj2.children = items1;
      tmp4Result = tmp4(tmp5, obj2);
    }
    isRolePowerfulResult = tmp(18072).isRolePowerful(role);
  }
};
