...
// Dependencies: [5, 32, 19, 17, 1371, 11980, 1074, 21, 4636, 576, 4338, 1483, 504, 38, 9192, 11978, 4334, 4788, 5665, 4632, 11982, 1176, 1114, 4481, 5768, 5685, 5056, 2]
// Exports: default

// Module 11981 (TransferOwnership)
...
const TransferOwnershipConstants = fn(11980);
...
export default function TransferOwnership(guild) {
          ...
          if (arg0 === 1) {
            ...
            if (!mfaEnabled.mfaEnabled) {
              if (null != tmp27.email) {
                v1 = 1;
                dependencyMap = 1;
                const obj6 = { value: v1(9192).sendTransferOwnershipPincode(user.id), done: false };
                return obj6;
              }
            }
            let MFA = null;
            ...
            dependencyMap = 3;
            const transferOwnershipResult = v1(9192).transferOwnership(user.id, id.id, MFA);
          }
        } else if (arg0 === 1) {
          dependencyMap = 3;
          throw value;
  ...
  const token = guild(4338).useToken(toUser(576).modules.mobile.TABLE_ROW_PADDING);
  let tmp5 = closure_16();
  let obj = guild(4338);
  ...
  nickname = toUser(4788).getNickname(guild.id, undefined, toUser);
  ...
  const items1 = [closure_13(c7, { source: toUser(11982), style: tmp5.arrow }), , , ];
  ...
  items1[2] = closure_13(guild(4632).Text, { variant: "text-xs/medium", color: "text-default", children: guild.name });
  if (null == nickname) {
    if (!hasAvatarForGuildResult) {
      const intl = tmp(1114).intl;
      const obj15 = { GuildHook: GuildWithSmallIcon, user: tmp3(4481).getUserTag(toUser) };
    ...
    const obj19 = { username: tmp3(4481).getUserTag(toUser) };
    obj18.label = intl3.format(tmp(1114).t.xm6ACJ, obj19);
    obj18.onPress = function handleConfirmToggle(arg0) {
      _undefined(arg0);
    };
    obj17.children = tmp11(tmp(5685).TableCheckboxRow, obj18);
    items3[1] = tmp11(tmp(5768).TableRowGroup, obj17);
    const obj20 = {
    ...
    obj20.disabled = !tmp8;
    items3[2] = tmp11(tmp(5056).Button, obj20);
