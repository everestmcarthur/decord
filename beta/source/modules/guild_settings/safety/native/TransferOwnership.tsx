...
// Dependencies: [5, 32, 19, 17, 1372, 12130, 1074, 21, 4788, 576, 4489, 1484, 504, 38, 9887, 12128, 4485, 4940, 5833, 4784, 12132, 1177, 1115, 4632, 5936, 5853, 5218, 2]
// Exports: default

// Module 12131 (TransferOwnership)
...
export default function TransferOwnership(guild) {
          ...
          if (arg0 === 1) {
            ...
            if (!mfaEnabled.mfaEnabled) {
              if (null != tmp27.email) {
                v1 = 1;
                dependencyMap = 1;
                const obj6 = { value: v1(9887).sendTransferOwnershipPincode(user.id), done: false };
                return obj6;
              }
            }
            let MFA = null;
            ...
            const obj4 = v1(9887);
            v1(9887).transferOwnership(user.id, id.id, MFA).then(() => {
              v1(12128).close();
              const obj = v1(12128);
              v1(9887).close();
              const obj2 = v1(9887);
              const result = closure_1_0(4485).showTransferOwnershipSuccess();
            }, NOOP);
            dependencyMap = 3;
            const transferOwnershipResult = v1(9887).transferOwnership(user.id, id.id, MFA);
          }
        } else if (arg0 === 1) {
          dependencyMap = 3;
          throw value;
  ...
  const token = guild(4489).useToken(toUser(576).modules.mobile.TABLE_ROW_PADDING);
  let tmp5 = closure_16();
  let obj = guild(4489);
  ...
  nickname = toUser(4940).getNickname(guild.id, undefined, toUser);
  ...
  const items1 = [closure_13(c7, { source: toUser(12132), style: tmp5.arrow }), , , ];
  const obj10 = { style: tmp5.avatarsWrapper, children: null };
  const obj9 = { source: toUser(12132), style: tmp5.arrow };
  ...
  items1[2] = closure_13(guild(4784).Text, { variant: "text-xs/medium", color: "text-default", children: guild.name });
  if (null == nickname) {
    if (!hasAvatarForGuildResult) {
      const intl = tmp(1115).intl;
      const obj15 = { GuildHook: GuildWithSmallIcon, user: tmp3(4632).getUserTag(toUser) };
    ...
    const obj19 = { username: tmp3(4632).getUserTag(toUser) };
    obj18.label = intl3.format(tmp(1115).t.xm6ACJ, obj19);
    obj18.onPress = function handleConfirmToggle(arg0) {
      _undefined(arg0);
    };
    obj17.children = tmp11(tmp(5853).TableCheckboxRow, obj18);
    items3[1] = tmp11(tmp(5936).TableRowGroup, obj17);
    const obj20 = {
    ...
    obj20.disabled = !tmp8;
    items3[2] = tmp11(tmp(5218).Button, obj20);
  ...
  obj21.user = toUser(4632).getUserTag(toUser);
