...
// Function ID: 18009
// Name: HeaderRow
// Dependencies: [5, 32, 19, 17, 18002, 1074, 1375, 21, 4788, 576, 9791, 10631, 1241, 1115, 1255, 5386, 5218, 4784, 504, 2]
// Exports: ConnectedHeaderRow

// Module 18008 (HeaderRow)
...
import GuildSettingsEmojiStore from "GuildSettingsEmojiStore" /* 18002 */;
...
function HeaderRow(guild) {
            ...
            if (arg0 === 1) {
              ...
              if (null != base64) {
                upload = function upload(image, originalMd5, roles) {
                  ...
                  let obj = closure_1_0(10631);
                  let obj2 = { guildId: image.id, image, name: combined, roles, originalMd5 };
                  const uploadEmojiResult = closure_1_0(10631).uploadEmoji({ guildId: image.id, image, name: combined, roles, originalMd5 });
                  closure_1_0(10631).uploadEmoji({ guildId: image.id, image, name: combined, roles, originalMd5 }).then(() => closure_2_1(1241).track(constants.EMOJI_UPLOAD_COMPLETED, { guild_id: image.id, upload_id })).catch((error) => {
  ...
  let canCreateExpressions = guild(9791).useManageResourcePermissions(guild).canCreateExpressions;
  ...
  let stringResult = intl.string(guild(1115).t.N2qTQ3);
  obj4.id = "GUILD_SETTINGS_EMOJI_UPLOAD_REQUIREMENTS_" + 2;
  obj4.text = intl2.formatToPlainString(guild(1115).t.gfAXoR, { maxSize: EMOJI_MAX_FILESIZE_KB });
  items[1] = obj4;
  let intl3 = guild(1115).intl;
  let obj5 = { id: null, text: null };
  const formatToPlainStringResult = intl2.formatToPlainString(guild(1115).t.gfAXoR, { maxSize: EMOJI_MAX_FILESIZE_KB });
  obj5.id = "GUILD_SETTINGS_EMOJI_UPLOAD_REQUIREMENTS_" + 3;
  obj5.text = intl3.string(guild(1115).t.rnwKPH);
  items[2] = obj5;
  const intl4 = guild(1115).intl;
  let obj6 = { id: null, text: null };
  const stringResult1 = intl3.string(guild(1115).t.rnwKPH);
  obj6.id = "GUILD_SETTINGS_EMOJI_UPLOAD_REQUIREMENTS_" + 4;
  obj6.text = intl4.string(guild(1115).t["8Vr5Qd"]);
  items[3] = obj6;
  ...
  if (canCreateExpressions) {
    ...
    canCreateExpressions = tmp13(tmp7(5218).Button, obj8);
  }
  ...
  if (tmp13Result) {
    ...
    tmp13Result = tmp13(tmp7(4784).Text, obj9);
  }
  const obj10 = { children: null };
  items1[1] = tmp13Result;
  let obj11 = { style: tmp.uploadInstructionsContainer, children: null };
  const items2 = [closure_14(guild(4784).Text, { variant: "text-sm/medium", color: "text-muted", children: description }), , ];
  ...
  items2[1] = closure_14(guild(4784).Text, obj12);
