...
// Dependencies: [19, 17, 2041, 1074, 4938, 1385, 7566, 7571, 1477, 8569, 7603, 8164, 5133, 8236, 5012, 1978, 8411, 8427, 1370, 8570, 1094, 4485, 8564, 5141, 1115, 8571, 2]
...
import transformMessageComponents from "transformMessageComponents" /* 8411 */;
...
import NativePortalView from "NativePortalView" /* 8571 */;
        ...
        if (found.height > 0) {
          ...
          if (null != found.proxy_url) {
            ...
            if (isVideoFileResult) {
              ...
              if (!hasFlagResult) {
                hasFlagResult = tmp43(7603).isChannelSpoilerGated(ChannelStore.getChannel(message2.channel_id));
                const tmp43Result5 = tmp43(7603);
              }
              ...
              if (!hasFlagResult1) {
                hasFlagResult1 = tmp43(7603).isChannelSpoilerGated(ChannelStore.getChannel(message2.channel_id));
                const tmp43Result7 = tmp43(7603);
              }
              ...
              const formatted = tmp(8569)(found).toLowerCase();
              const str3 = tmp(8569)(found);
              const endsWithResult = formatted.endsWith(".webp");
              const formatted1 = tmp(8569)(found).toLowerCase();
              const endsWithResult1 = formatted1.endsWith(".avif");
              const str5 = tmp(8569)(found);
...
function toMediaSourceFromUnfurledMedia(id, guildId, media, description, spoiler) {
  ...
  if (unfurledMediaItemType === RowGeneratorTypes.MediaGalleryItemType.VISUAL_PLACEHOLDER) {
    ...
    const tmp4 = unfurledMediaItemType === tmp(8427).MediaGalleryItemType.VIDEO;
    let str;
    if (tmp4) {
      str = "png";
    }
    ...
    const enabledHarmTypesForMessage = tmp(7566).getEnabledHarmTypesForMessage(id);
    const tmpResult = tmp(7566);
    const obj2 = { type: tmp(7571).ObscuredMediaTypes.GenericMedia, media };
    ...
    const result = tmp(7566).isMediaObscuredForHarmTypes(obj2, enabledHarmTypesForMessage);
    if (width2 == null) {
      width2 = 0;
    }
    size.width = width2;
...
export const downloadMediaAsset = function downloadMediaAsset(mediaUrl, VIDEO) {
  ...
  return MediaManager.downloadMediaAsset(mediaUrl, VIDEO).then(() => {
    if (ConstantsIOS.MediaType.IMAGE === closure_0) {
      tmp2(4485).presentImageSaved();
      const tmp2Result = tmp2(4485);
    } else if (tmp2(1094).MediaType.GIF === tmp) {
      tmp2(4485).presentGifSaved();
      const tmp2Result3 = tmp2(4485);
    } else if (tmp2(1094).MediaType.VIDEO === tmp) {
      tmp2(4485).presentVideoSaved();
      const tmp2Result4 = tmp2(4485);
    }
    const MediaViewerAnalytics = tmp2(8564).MediaViewerAnalytics;
...
export const isAnimatedImageSource = function isAnimatedImageSource(source) {
  ...
  if (!result) {
    ...
    if (result1) {
      result1 = tmp(4938).urlMatchesFileExtension(source.sourceURI, React5);
  ...
  if (!result) {
    ...
    if (result2) {
      result2 = tmp(4938).urlMatchesFileExtension(source.sourceURI, re8);
