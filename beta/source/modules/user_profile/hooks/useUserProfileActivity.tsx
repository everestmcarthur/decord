...
// Dependencies: [19, 9104, 1992, 4828, 4813, 504, 11169, 13363, 8436, 8644, 8640, 2]
// Exports: default

// Module 13362 (useUserProfileActivity)
import _mod19 from "module_19" /* 19 */;
import Constants from "Constants" /* 4813 */;
import UserProfileStackedActivityCardUtils from "UserProfileStackedActivityCardUtils" /* 13363 */;
          ...
          if (tmpResult.isListenedSessionEntry(length)) {
            ...
            if (tmp6) {
              ...
              tmp6 = !userProfileLiveActivities.some((item) => {
                ...
                if (result) {
                  result = userProfileLiveActivities(8640).isMatchingListeningActivity(closure_0, item);
                  const obj = userProfileLiveActivities(8640);
                }
                return result;
              });
            }
            ...
            if (tmpResult3.isWatchedMediaEntry(length)) {
              result = !userProfileLiveActivities.some((item) => {
                let result = null != item;
                if (result) {
                  result = userProfileLiveActivities(8640).isMatchingWatchActivity(closure_0, item);
                  const obj = userProfileLiveActivities(8640);
                }
                return result;
              });
