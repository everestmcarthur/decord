...
// Dependencies: [19, 17, 2109, 21, 4788, 504, 4446, 7828, 4470, 4784, 1115, 8474, 2]
// Exports: default

// Module 13507 (EditCollectiblesPreviewDetails)
...
function EditCollectiblesPreviewDescription(arg0) {
  ...
  if (!result) {
    result = tmp(7828).isPremiumCollectiblesPurchase(purchase);
    const tmpResult = tmp(7828);
  }
  let result1 = !canUseCollectiblesResult;
  if (!canUseCollectiblesResult) {
    result1 = tmp(7828).isPremiumCollectiblesPurchase(purchase);
    const tmpResult3 = tmp(7828);
  }
  if (null != purchase) {
    if (!result1) {
      let expiresAt1;
      ...
      if (null != expiresAt1) {
        const _Date = Date;
        const date = new Date();
        diffAsUnitsResult = tmp(4470).diffAsUnits(date, purchase.expiresAt);
        const tmpResult4 = tmp(4470);
      }
      ...
      if (tmp17) {
        ...
        tmp17 = hasOwnProperty(tmp(4784).Text, obj4);
      }
      const items1 = [tmp17, , ];
      ...
      items1[1] = hasOwnProperty(tmp(4784).Text, obj6);
      let expiresAt2;
      if (purchase != null) {
      ...
      if (tmp19Result) {
        ...
        tmp19Result = tmp19(tmp(4784).Text, obj8);
      }
  ...
  if (result) {
    if (canUseCollectiblesResult) {
      ...
      let tmp25Result = hasOwnProperty(tmp(4784).Text, obj11);
    } else {
      const tmp4Result = PremiumUtilsDefault;
      const Text = tmp(4784).Text;
    ...
    return hasOwnProperty(tmp(4784).Text, obj13);
  }
}
