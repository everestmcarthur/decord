...
// Dependencies: [19, 17, 1372, 8484, 7485, 1074, 8234, 8495, 21, 4788, 576, 11189, 11190, 9584, 5371, 11449, 4784, 11451, 8003, 4646, 11483, 1115, 11485, 8485, 11486, 7656, 2009, 4486, 8543, 7997, 7998, 8008, 5696, 1241, 7996, 1364, 11487, 11488, 11489, 11493, 11495, 1478, 5218, 9178, 8466, 10045, 4632, 2]
// Exports: default

// Module 11448 (UserProfilePrimaryInfo)
...
import openBadgeDirectoryScreen from "openBadgeDirectoryScreen" /* 11489 */;
      ...
      onPress() {
          if (null == closure_4) {
            if (id !== getBadgeName(BadgeId.BadgeId.GIFTING)) {
              ...
              if (tmp3 === tmp5(8543).QUEST_COMPLETED_BADGE) {
                if (tmp5Result7.shouldMigrateToAdAnalyticsInterface(tmp5(7997).AdAnalyticsInterfaceExperimentStep.STEP_4_VIEWED_NON_IMPRESSION, "quest_completed_badge_toast")) {
                  const obj2 = { type: tmp5(8008).AdUserActionType.VIEW_INTERNAL_SURFACE_IMPRESSION, surfaceId: tmp5(5696).QuestContent.QUEST_BADGE, isTargeted: false };
                  tmp5(7998).captureAdUserAction(obj2);
                  const tmp5Result8 = tmp5(7998);
                } else {
                  const obj3 = {};
                  const tmp11Result = tmp11(1241);
                  const merged = Object.assign(tmp5(7996).getContentProperties(tmp5(5696).QuestContent.QUEST_BADGE));
                  ...
                  const tmp5Result9 = tmp5(7996);
                }
                tmp5Result7 = tmp5(7997);
              }
            } else {
              const obj5 = { screen: constants2.PREMIUM_GIFTING, params: {} };
              tmp5(7656).openUserSettings(obj5);
              const tmp5Result12 = tmp5(7656);
            }
            tmp3 = id;
          } else {
            tmp();
...
const getBadgeName = fn(8484).getBadgeName;
