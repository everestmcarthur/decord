...
// Dependencies: [19, 17, 7806, 4803, 10413, 7374, 4970, 21, 4788, 576, 7609, 4608, 2027, 563, 7802, 4991, 11829, 1980, 12662, 12574, 1115, 14142, 2]
// Exports: GuildRolesAndChannelsRow

// Module 16550 (GuildActionRows)
...
let closure_7 = fn(7374).CHANNELS_AND_ROLES_MODAL_KEY;
const ReadStateTypes = fn(4970).ReadStateTypes;
const jsx = fn(21).jsx;
const createStyles = fn(4788);
let obj2 = { container: { marginVertical: fn(10413).CHANNEL_MARGIN_VERTICAL, marginHorizontal: 8, borderRadius: nativeDefault.radii.md }, channelInfoContainer: { paddingStart: 4 } };
...
export const GuildRolesAndChannelsRow = function GuildRolesAndChannelsRow(guild) {
  ...
  const callback = noop.useCallback(() => {
    ModalActionCreatorsDefault.pushLazy(asyncRequireImpl(11829, dependencyMap.paths), { guildId: id }, closure_7);
  }, items2);
  let SELECTED = guild(12662).ChannelModes.DEFAULT;
  if (selected) {
    SELECTED = tmp5(12662).ChannelModes.SELECTED;
  }
  let tmp10 = !result;
  if (result) {
  ...
  let tmp11 = null;
  if (tmp10) {
    const obj4 = { style: tmp.channelInfoContainer, children: jsx(tmp5(12574).NewBadge, {}) };
    tmp11 = <View style={tmp.channelInfoContainer}>{jsx(tmp5(12574).NewBadge, {})}</View>;
  }
  ...
  obj5.name = jsx(guild(12662).BaseChannelName, { name: string2Result, mode: SELECTED });
  const tmp2Result = tmp2(12662);
  obj5.icon = jsx(guild(12662).BaseChannelIcon, { mode: SELECTED, IconComponent: guild(14142).ChannelListMagnifyingGlassIcon });
