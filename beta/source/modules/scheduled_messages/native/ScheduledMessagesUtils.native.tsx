...
// Dependencies: [5, 8051, 1115, 4380, 8120, 4991, 12494, 1980, 12504, 8122, 8119, 8123, 7471, 4755, 11995, 2]
...
// Module 12493 (ScheduledMessagesUtils)
...
function openSendTimeActionSheet(title) {
  ...
  const result = title(8120).trackScheduledMessageTimePickerOpened({ entryPoint, isEditing, channelId });
  let obj = title(8120);
  let obj3 = { key, header: { title }, hasIcons: false, options: null };
  let obj2 = title(7471);
  const presetScheduledTimes = title(8120).getPresetScheduledTimes();
  const items = [
  ...
  obj5.onPress = function onPress() {
    ...
    const tmp2 = asyncRequireImpl(11995, dependencyMap.paths);
    ...
    obj3.getError = function getError(isBefore) {
      return title(8120).getScheduledTimeError(isBefore, dependencyMap);
    };
    obj3.createReminder = function createReminder(arg0) {
      return closure_1_3(defaultValue(4380)(arg0));
    };
...
export const openScheduleMessageActionSheet = function openScheduleMessageActionSheet(id, ATTACH_MENU, scheduledTimestamp) {
  ...
  if (null != currentTimestamp) {
    let defaultScheduledTime = _modDef4380(currentTimestamp);
  } else {
...
export const showScheduledMessagesModal = function showScheduledMessagesModal() {
  ModalActionCreatorsDefault.pushLazy(asyncRequireImpl(12494, dependencyMap.paths), {}, "scheduled-messages-modal", { presentation: "modal" });
};
export const openScheduledMessageEditContentModal = function openScheduledMessageEditContentModal(scheduledMessage) {
  ModalActionCreatorsDefault.pushLazy(asyncRequireImpl(12504, dependencyMap.paths), { scheduledMessage }, "scheduled-message-edit-content", { presentation: "modal" });
};
export const openRescheduleMessageActionSheet = function openRescheduleMessageActionSheet(scheduledMessageId, sendAtTimestamp, channelId) {
  ...
  obj.startDate = _modDef4380(sendAtTimestamp);
