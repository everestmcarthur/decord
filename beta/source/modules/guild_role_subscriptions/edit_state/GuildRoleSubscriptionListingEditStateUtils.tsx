...
// Dependencies: [5, 32, 19, 5708, 4421, 15501, 15478, 1074, 1374, 504, 5847, 5029, 15502, 15503, 1092, 4419, 15504, 15485, 15505, 1248, 38, 5769, 7529, 12, 10631, 15506, 1255, 2]
...
// Module 15500 (GuildRoleSubscriptionListingEditStateUtils)
...
import useSubscriptionRoleDefault from "useSubscriptionRole" /* 15502 */;
import Contants from "Contants" /* 15503 */;
import useTrialIntervalOptionsDefault from "useTrialIntervalOptions" /* 15505 */;
            ...
            if (undefined !== tierEmojiIds) {
              ...
              closure_129_15 = closure_129_13.map((item) => {
                ...
                if (null != customEmojiById) {
                  ...
                  obj2.roles = items;
                  return guildId(10631).updateEmoji(obj2);
                }
              });
              closure_129_16 = closure_129_14.map((item) => {
                ...
                if (null != customEmojiById) {
                  ...
                  if (found.length > 0) {
                    ...
                    let updateEmojiResult = guildId(10631).updateEmoji(obj3);
                    const obj2 = guildId(10631);
                  } else {
                    updateEmojiResult = guildId(10631).deleteEmoji(guildId, customEmojiById.id);
                    const obj = guildId(10631);
                  }
...
const GuildRoleSubscriptionEditStore = fn(15501);
({ AllChannelAccessOptions: closure_9, useEditStateStore: c10 } = GuildRoleSubscriptionEditStore);
let closure_11 = fn(15478).GuildRoleSubscriptionBenefitTypes;
...
export const usePriceTier = function usePriceTier(editStateId) {
  ...
  const items = [GuildRoleSubscriptionsStore];
  const stateFromStores = require("initialize").useStateFromStores(items, () => GuildRoleSubscriptionsStore.getSubscriptionListing(closure_0));
  const tmp2 = priceTier(5847)(() => first);
  const items1 = [stateFromStores, tmp2];
  let memo = noop.useMemo(() => closure_3(c2), items1);
  _require = editStateId;
  priceTier = "priceTier";
...
export const useDescription = function useDescription(arg0) {
  ...
  const items = [GuildRoleSubscriptionsStore];
  const stateFromStores = require("initialize").useStateFromStores(items, () => GuildRoleSubscriptionsStore.getSubscriptionListing(closure_0));
  const tmp2 = description(5847)(() => first);
  const items1 = [stateFromStores, tmp2];
  let memo = noop.useMemo(() => closure_3(c2), items1);
  _require = arg0;
...
export const useRoleColor = function useRoleColor(editStateId, guildId) {
  const tmp = roleColor(15502)(guildId, editStateId);
  editStateId = tmp;
  const items = [tmp];
  let memo = noop.useMemo(() => {
    let color;
...
export const useChannelAccessFormat = function useChannelAccessFormat(editStateId, guildId) {
  const tmp = channelAccessFormat(15502)(guildId, editStateId);
  const items = [tmp];
  let memo = noop.useMemo(() => {
    if (null == closure_0) {
...
export const useChannelBenefits = function useChannelBenefits(listingId) {
  ...
  const items = [GuildRoleSubscriptionsStore];
  const stateFromStores = require("initialize").useStateFromStores(items, () => GuildRoleSubscriptionsStore.getSubscriptionListing(closure_0));
  const tmp2 = channelBenefits(5847)(() => first);
  const items1 = [stateFromStores, tmp2];
  let memo = noop.useMemo(() => closure_3(c2), items1);
  _require = listingId;
...
export const useIntangibleBenefits = function useIntangibleBenefits(listingId) {
  ...
  const items = [GuildRoleSubscriptionsStore];
  const stateFromStores = require("initialize").useStateFromStores(items, () => GuildRoleSubscriptionsStore.getSubscriptionListing(closure_0));
  const tmp2 = intangibleBenefits(5847)(() => first);
  const items1 = [stateFromStores, tmp2];
  let memo = noop.useMemo(() => closure_3(c2), items1);
  _require = listingId;
...
export const useTierEmojiIds = function useTierEmojiIds(listingId, guildId) {
  _require = guildId;
  const tmp = tierEmojiIds(15502)(guildId, listingId);
              ...
              const obj6 = {
                value: (function updateListingFromEditState(editStateId) {
                            ...
                            if (null != channelBenefits) {
                              ...
                              const found = benefits.filter(closure_1_0(15504).isChannelBenefit);
                              const found1 = benefits1.filter(closure_1_0(15504).isIntangibleBenefit);
                              if (channelBenefits == null) {
                            ...
                            if (!obj2.isEmpty(obj)) {
                              ...
                              subscriptionListing = closure_1_2(7529).updateSubscriptionListing(obj4);
                              const obj3 = closure_1_2(7529);
                            }
