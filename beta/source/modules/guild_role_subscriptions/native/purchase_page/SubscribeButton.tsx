...
// Dependencies: [5, 32, 19, 17, 2095, 4421, 1074, 2048, 21, 15500, 504, 9512, 4784, 5141, 1115, 5818, 4380, 1177, 16894, 4755, 16901, 1980, 16890, 5300, 5218, 10595, 2]
// Exports: default
...
function SwitchTiersButton(activeSubscription) {
  ...
  let obj = changeToListingId(15500);
  const obj3 = { children: null };
  const obj2 = activeListingId(4380)(activeSubscription.currentPeriodEnd);
  const obj4 = { variant: "text-xs/normal", color: "text-muted", children: null };
  const intl = activeSubscription(1115).intl;
  const formatResult = activeListingId(4380)(activeSubscription.currentPeriodEnd).format("MMMM Do");
  obj4.children = intl.format(activeSubscription(1115).t.lA7ztO, { activeListingName: _slicedToArray(obj.useName(activeListingId), 1)[0], billingEndDate: activeListingId(4380)(activeSubscription.currentPeriodEnd).format("MMMM Do"), emphasisHook });
  const items = [closure_12(activeSubscription(4784).Text, obj4), closure_12(activeSubscription(1177).Spacer, { size: 16 }), ];
