...
// Dependencies: [32, 19, 17, 9888, 2063, 4421, 15478, 1074, 1374, 21, 4788, 576, 4755, 18200, 1980, 1115, 7511, 15504, 4784, 10043, 563, 15500, 5836, 10547, 1612, 14196, 1484, 15486, 18188, 14191, 15485, 12, 5873, 18202, 18203, 38, 10111, 18237, 18198, 2]
// Exports: default

// Module 18199 (GuildSettingsRoleSubscriptionTiers)
...
function EditListingButton(editStateId) {
  ...
  if (undefined === first) {
    const obj5 = {
      ...
      onLongPress() {
          ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(18200, dependencyMap.paths), "TierArchiveOrDelete", { editStateId, guildId, groupListingId });
        },
      children: null
    };
    ...
    const items2 = [closure_17(tmp2(4784).Text, obj9), ];
    ...
    items3[3] = closure_17(tmp2(4784).Text, obj12);
    obj10.children = items3;
    items2[1] = closure_18(closure_7, obj10);
    obj8.children = items2;
    items1[1] = closure_18(closure_7, obj8);
    items1[2] = closure_17(tmp2(10547).PencilIcon, {});
    obj5.children = items1;
    obj11.children = closure_18(Row, obj5);
    return closure_17(closure_7, obj11);
  } else if (null != first2) {
    const intl = tmp2(1115).intl;
    const obj13 = { price: tmp2(7511).formatPrice(first, first2.currency), interval: null };
    const tmp2Result = tmp2(7511);
    obj13.interval = tmp2(15504).formatPlanInterval(first2);
    let formatToPlainStringResult = intl.formatToPlainString(tmp2(1115).t.CgmBaG, obj13);
    const tmp2Result4 = tmp2(15504);
  } else {
    const intl2 = tmp2(1115).intl;
    const obj14 = { price: tmp2(7511).formatPrice(first, constants.USD), interval: null };
    const tmp2Result5 = tmp2(7511);
    const obj15 = { interval: SubscriptionIntervalTypes.MONTH, interval_count: 1 };
    obj14.interval = tmp2(15504).formatPlanInterval(obj15);
    formatToPlainStringResult = intl2.formatToPlainString(tmp2(1115).t.CgmBaG, obj14);
    const tmp2Result6 = tmp2(15504);
  }
}
function GuildSettingsRoleSubscriptionsTiersInner(guildId) {
  ...
  const layoutEffect = editStateIds.useLayoutEffect(() => {
    navigation.setOptions({
      headerTitle() {
        ...
        return closure_1_17(guildId(5873).NavigatorHeader, obj);
      }
    });
  });
  ...
  onPress = editStateIds.useCallback(() => {
    if (guildEligibleForTierTemplates) {
      ...
      const result = tmp(18202).pushTierTemplateSelectionScene(navigation, obj);
      const tmpResult = tmp(18202);
    } else {
      ...
      const result1 = tmp(18203).openTierCreationModal(obj2);
      const tmpResult2 = tmp(18203);
    }
  }, items1);
      ...
      mapped = memo.map((editStateId) => {
        ...
        return closure_1_17(EditListingButton, {
          ...
          onPress() {
            ...
            guildEligibleForTierTemplates(18202).pushTierEditScene(navigation, {
              groupListingId: id,
              initialEditStateId,
...
const MAX_SUBSCRIPTION_TIERS = fn(15478).MAX_SUBSCRIPTION_TIERS;
