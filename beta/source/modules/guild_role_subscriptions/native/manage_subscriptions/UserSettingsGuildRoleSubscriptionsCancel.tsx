...
// Dependencies: [5, 32, 19, 17, 4452, 21, 4788, 576, 15500, 4380, 1115, 5833, 1177, 4784, 15490, 5836, 15508, 7439, 7459, 9512, 5141, 5111, 4485, 4468, 5218, 7256, 15496, 15485, 15509, 15510, 504, 2]
// Exports: default

// Module 15499 (UserSettingsGuildRoleSubscriptionsCancel)
...
import GuildRoleSubscriptionCardAll from "GuildRoleSubscriptionCard" /* 15510 */;
import asyncGeneratorStep from "asyncGeneratorStep" /* 5 */;
import _slicedToArray from "module_32" /* 32 */;
import noop from "module_19" /* 19 */;
import SubscriptionStore from "SubscriptionStore" /* 4452 */;

const FastAssetImageDefault = tmp5(15509);
require = fn;
function WhatYouLose(subscription) {
  ...
  const obj5 = _modDef4380(subscription.subscription.currentPeriodEnd);
  const intl = util.intl;
  const formatResult = _modDef4380(subscription.subscription.currentPeriodEnd).format(c12);
  const intl2 = util.intl;
  const formatResult1 = intl.format(util.t.EtAXzC, { subscriptionEndDate: _modDef4380(subscription.subscription.currentPeriodEnd).format(c12) });
...
function CancelSubscriptionButtonFooter(guild) {
  ...
  const analyticsLocations = subscription(7439)(subscription(7459).GUILD_ROLE_SUBSCRIPTION_CANCELLATION_MODAL).analyticsLocations;
  const tmp5 = subscription(7439);
  const cancelSubscription1 = subscription(9512).useCancelSubscription(subscription.id, subscription.isACOM);
      ...
      try {
        ...
        if (0 === c4) {
          if (arg0 === 1) {
            ...
            const obj15 = tmp3(5141);
            const _HermesInternal = HermesInternal;
            obj6.body = "You can resubscribe any time before " + tmp3(4380)(subscription.currentPeriodEnd).format(closure_1_12) + ".";
          ...
          if (1 === tmp7) {
            if (arg0 === 1) {
              ...
              if (closure_129_6) {
                ...
                c4 = 4;
                c5 = 1;
                const obj13 = { value: tmp63(5111).cancelSubscription(closure_129_1.id, closure_129_4), done: false };
                return obj13;
              } else {
                const _Error = Error;
            ...
            if (2 === tmp7) {
              ...
              closure_128_1 = tmp63;
              closure_129_3(false);
              guild(4485).presentFailedToast(closure_128_1.message);
              let tmp38 = closure_128_1 instanceof tmp3(4468);
              if (tmp38) {
                tmp38 = closure_128_1.code === guild(4468).ErrorCodes.ALREADY_CANCELED;
              }
              if (tmp38) {
              ...
              if (3 === tmp7) {
                ...
                if (arg0 === 1) {
                  ...
                  c4 = 5;
                  c5 = 1;
                  const obj16 = { value: tmp63(5111).fetchSubscriptions(), done: false };
                  return obj16;
                }
              } else if (arg0 === 1) {
                c5 = 3;
              ...
              if (closure_128_0) {
                ...
                guild(4485).presentFailedToast(intl.string(guild(1115).t.R0RpRX));
                const obj5 = guild(4485);
              }
              dependencyMap = 0;
            }
            dependencyMap = 0;
  ...
  obj2.children = closure_10(guild(5218).Button, obj3);
...
function Content(subscription) {
  ...
  if (null != listing) {
    ...
    if (null != guild) {
      ...
      items1[2] = closure_1_10(tmp(4784).Text, obj8);
