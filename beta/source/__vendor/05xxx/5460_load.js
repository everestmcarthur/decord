...
// Function ID: 5461
// Name: load
// Dependencies: [5461, 5462, 5463, 5464, 5465, 5478, 5480, 5481, 5493, 5497, 5501, 5503, 5505, 5506, 5507, 5508, 5509, 5510, 5511, 5512, 5513, 5514, 5487]
// Exports: load
...
function load(response, response) {
  ...
  if (typeof response === "string") {
    ...
    if (obj2.test(response)) {
      ...
      if (obj3.test(response)) {
        let resolved = Promise.resolve(response(5462).dataUriToBuffer(response));
        const obj5 = response(5462);
      } else {
        let obj7 = then;
        if (then === undefined) {
          obj7 = {};
...
function loadView(byteLength, response) {
  ...
  function addPngTextTags(readTags) {
    if (flag) {
      ...
      if (closure_1.png) {
        objectAssignResult3 = _mod5462.objectAssign({}, closure_1.png, readTags);
      }
      closure_1.png = objectAssignResult3;
      let objectAssignResult4 = readTags;
      if (closure_1.pngText) {
        objectAssignResult4 = _mod5462.objectAssign({}, closure_1.png, readTags);
      }
  ...
  let obj2 = require("module_5464");
  ({ fileType, fileDataOffset, jfifDataOffset, tiffHeaderOffset, iptcDataOffset, xmpChunks, iccChunks, mpfDataOffset, pngHeaderOffset, pngTextChunks, pngChunkOffsets, vp8xChunkOffset, gifHeaderOffset } = require("module_5464").parseAppMarkers(byteLength, flag2));
  let flag4 = false;
  if (require("module_5465").USE_JPEG) {
    flag4 = false;
        ...
        if (readResult11 instanceof Promise) {
          items1.push(readResult11.then(function addIccTags(icc) {
            if (flag) {
              ...
              closure_1 = _mod5462.objectAssign({}, closure_1, icc);
            }
          }));
          flag9 = true;
        } else if (flag) {
  ...
  const parseAppMarkersResult = require("module_5464").parseAppMarkers(byteLength, flag2);
...
export default { load, loadView, errors: _modDef5461 };
export const errors = _modDef5461;
export { load };
export { loadView };
