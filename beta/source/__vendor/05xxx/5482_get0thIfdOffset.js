...
// Dependencies: [5479, 5483, 5465]
// Exports: get0thIfdOffset

// Module 5482 (get0thIfdOffset)
import _modDef5465 from "module_5465" /* 5465 */;
import _modDef5479 from "module_5479" /* 5479 */;
import IFD_TYPE_0TH2 from "IFD_TYPE_0TH" /* 5483 */;

require = arg1;
function readTag(byteLength, IFD_TYPE_0TH, sum, sum, byteOrder, arg5) {
  const typeSize = _modDef5479.getTypeSize("SHORT");
  sum = typeSize + _modDef5479.getTypeSize("SHORT");
  const sum1 = sum + _modDef5479.getTypeSize("LONG");
  const shortAt = _modDef5479.getShortAt(byteLength, sum, byteOrder);
  const shortAt1 = _modDef5479.getShortAt(byteLength, sum + typeSize, byteOrder);
  const longAt = _modDef5479.getLongAt(byteLength, sum + sum, byteOrder);
  if (undefined !== _modDef5479.typeSizes[shortAt1]) {
    if ((function tagValueFitsInOffsetSlot(shortAt1, longAt) {
      const result = _modDef5479.typeSizes[shortAt1] * longAt;
      return result <= _modDef5479.getTypeSize("LONG");
    })(shortAt1, longAt)) {
      const sum2 = sum + sum1;
      let str = getTagValue(byteLength, sum2, shortAt1, longAt, byteOrder);
      let tmp21 = sum2;
    } else {
      const longAt1 = tmp4(5479).getLongAt(byteLength, sum + sum1, byteOrder);
      str = "<faulty value>";
      tmp21 = longAt1;
      if ((function tagValueFitsInDataView(byteLength, sum, longAt1, shortAt1, longAt) {
        sum = sum + longAt1;
        return sum + _modDef5479.typeSizes[shortAt1] * longAt <= byteLength.byteLength;
    ...
    let tmp34 = str;
    if (shortAt1 === tmp4(5479).tagTypes.ASCII) {
    ...
    if (undefined !== tmp4(5483)[IFD_TYPE_0TH][shortAt]) {
      if (undefined !== tmp4(5483)[IFD_TYPE_0TH][shortAt].name) {
        if (undefined !== tmp4(5483)[IFD_TYPE_0TH][shortAt].description) {
          try {
            descriptionResult = tmp4(5483)[IFD_TYPE_0TH][shortAt].description(tmp34);
            combined = tmp39;
            const obj8 = tmp4(5483)[IFD_TYPE_0TH][shortAt];
          } catch (err) {
      ...
      if (shortAt1 !== tmp4(5479).tagTypes.RATIONAL) {
        if (shortAt1 !== tmp4(5479).tagTypes.SRATIONAL) {
          combined = tmp4(5483)[IFD_TYPE_0TH][shortAt];
...
function getTagValue(byteLength, sum2, shortAt1, longAt, byteOrder, arg5) {
  ...
  let BYTE = shortAt1;
  if (flag) {
    result = longAt * _modDef5479.typeSizes[shortAt1];
    BYTE = _modDef5479.tagTypes.BYTE;
  }
  let sum = sum2;
  const items = [];
  for (let num = 0; num < result; num = num + 1) {
    let arr = items.push(obj[BYTE](byteLength, sum, byteOrder));
    sum = sum + _modDef5479.typeSizes[BYTE];
  }
  if (BYTE === _modDef5479.tagTypes.ASCII) {
    let asciiValue = _modDef5479.getAsciiValue(items);
    const tmp9Result = _modDef5479;
  } else {
    asciiValue = items;
    if (1 === items.length) {
      asciiValue = items[0];
...
let obj = { 1: null, 2: _modDef5479.getByteAt, 3: _modDef5479.getAsciiAt, 4: _modDef5479.getShortAt, 5: _modDef5479.getLongAt, 7: _modDef5479.getRationalAt, 9: null, 10: _modDef5479.getUndefinedAt, 13: null };
obj[9] = _modDef5479.getSlongAt;
obj[10] = _modDef5479.getSrationalAt;
obj[13] = _modDef5479.getIfdPointerAt;
function readIfd(byteLength, IFD_TYPE_0TH, sum, sum, byteOrder, arg5) {
  const typeSize = _modDef5479.getTypeSize("SHORT");
  let num = 0;
  if (sum + obj2.getTypeSize("SHORT") <= byteLength.byteLength) {
    num = _modDef5479.getShortAt(byteLength, sum, byteOrder);
    const tmpResult = _modDef5479;
  }
  const obj3 = {};
  sum = sum + typeSize;
  let tmp5 = sum;
  ...
  if (_modDef5465.USE_THUMBNAIL) {
    if (tmp5 < byteLength.byteLength - tmp23Result.getTypeSize("LONG")) {
      const longAt = tmp23(5479).getLongAt(byteLength, tmp5, byteOrder);
      let tmp26 = 0 !== longAt;
      if (tmp26) {
      ...
      const tmp23Result2 = tmp23(5479);
    }
    tmp23Result = tmp23(5479);
  }
  return obj3;
}

export const get0thIfdOffset = function get0thIfdOffset(buffer, c5, byteOrder) {
  return c5 + _modDef5479.getLongAt(buffer, c5 + 4, byteOrder);
};
export { readIfd };
