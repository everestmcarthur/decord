...
// Dependencies: [109, 19, 17, 21, 5175, 5172, 5176, 5177, 5155, 5178]
...
const forwardRefResult = noop.forwardRef((hidden, ref) => {
  ...
  const edgeInsetApplication = _mod5175.useEdgeInsetApplication(!hidden.hidden, flag, flag2, flag3, flag4);
  ...
  if (headerLeftBarButtonItems) {
    if (tmp(5172).isHeaderBarButtonsAvailableForCurrentPlatform) {
      result = tmp(5176).prepareHeaderBarButtonItems(headerLeftBarButtonItems, "left");
  ...
  if (headerRightBarButtonItems) {
    if (tmp(5172).isHeaderBarButtonsAvailableForCurrentPlatform) {
      result1 = tmp(5176).prepareHeaderBarButtonItems(headerRightBarButtonItems, "right");
      const tmpResult2 = tmp(5176);
    }
  }
  let isHeaderBarButtonsAvailableForCurrentPlatform = tmp(5172).isHeaderBarButtonsAvailableForCurrentPlatform;
...
export const ScreenStackHeaderBackButtonImage = (arg0) => {
  ...
  return jsx(_modDef5178, { type: "back", style: closure_9.headerSubview, synchronousShadowStateUpdatesEnabled: get_synchronousScreenUpdatesEnabledDefault.experiment.synchronousHeaderSubviewUpdatesEnabled, children: null });
};
