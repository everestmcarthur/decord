  ...
  findJpegOffsets(byteLength) {
    ...
    if (c5 + c6 + 5 <= byteLength.byteLength) {
      while (true) {
        ...
        let tmp28 = tmp8;
        let sum7 = tmp9;
        if (_modDef5465.USE_FILE) {
          if (byteLength.getUint16(tmp) === c19) {
            break;
          }
        }
        if (tmp19(5465).USE_FILE) {
