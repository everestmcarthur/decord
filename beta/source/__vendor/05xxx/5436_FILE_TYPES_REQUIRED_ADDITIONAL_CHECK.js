...
// Dependencies: [41, 42, 5434, 5437, 5443, 5444, 5445, 5446, 5447]

// Module 5436 (FILE_TYPES_REQUIRED_ADDITIONAL_CHECK)
import AudioTypes from "AudioTypes" /* 5443 */;
import ImageTypes from "ImageTypes" /* 5444 */;
import VideoTypes from "VideoTypes" /* 5445 */;
import CompressedTypes from "CompressedTypes" /* 5446 */;
import OtherTypes from "OtherTypes" /* 5447 */;
...
const entry = {
  ...
  value: function getInfoByName(key10027) {
    return FileTypes(5434).fetchFromObject(FileTypes, key10027.toUpperCase());
  }
};
let items = [
  entry,
  {
    ...
    value: function getSignaturesByName(item10079) {
      return FileTypes(5434).fetchFromObject(FileTypes, item10079.toUpperCase()).signatures;
    }
  },
  {
    key: "detectSignature",
    ...
    value: function detectTypeByAdditionalCheck(fileChunk, found) {
      ...
      if (mapped.some((item) => {
        ...
        if (!mapped.includes("heic")) {
          let str6 = "flv";
          if (!FileTypes(5437).isFLV(fileChunk)) {
            let str8 = "mp4";
            if (tmp8(5437).isM4V(fileChunk)) {
              str8 = "mp4";
              if (!tmp8(5437).isHEIC(fileChunk)) {
                str8 = "m4v";
              }
            }
            str6 = str8;
        ...
        const result = FileTypes(5434).findMatroskaDocTypeElements(fileChunk);
        let str2 = "mkv";
        if ("mkv" !== result) {
          let str4;
          if ("webm" === result) {
            if (tmp3(5437).isWEBM(fileChunk)) {
              str4 = "webm";
            }
          }
          str2 = str4;
        ...
        if (mapped.some((item) => {
          ...
          if (FileTypes(5434).isAvifStringIncluded(fileChunk)) {
            str = "avif";
          }
        }
        return str;
