...
// Function ID: 8588
// Name: StepsIndicator
// Dependencies: [19, 17, 21, 8585, 8586, 8588, 8589]
// Exports: StepsIndicator

// Module 8587 (StepsIndicator)
import _mod8585 from "module_8585" /* 8585 */;
import _mod8586 from "module_8586" /* 8586 */;
import SliderTrackMark from "SliderTrackMark" /* 8588 */;
import StepNumber from "StepNumber" /* 8589 */;
...
export const StepsIndicator = function StepsIndicator(options) {
  ...
  const memo = currentValue.useMemo(() => {
    if (options.length > 9) {
      let fontSize = _mod8585.constants.STEP_NUMBER_TEXT_FONT_SMALL;
    } else {
      fontSize = _mod8585.constants.STEP_NUMBER_TEXT_FONT_BIG;
    }
    return { fontSize };
  }, items);
  ...
  const memo1 = currentValue.useMemo(() => {
    if ("web" === get_ActivityIndicator.Platform.OS) {
      let stepsIndicator = _mod8586.styles.stepsIndicator;
      let tmp6 = require;
    } else {
      const _Object = Object;
      const obj = { marginHorizontal: sliderWidth * _mod8585.constants.MARGIN_HORIZONTAL_PADDING };
      stepsIndicator = Object.assign({}, _mod8586.styles.stepsIndicator, obj);
      tmp6 = require;
    }
    const obj2 = { stepIndicatorContainerStyle: stepsIndicator, stepIndicatorElementStyle: null };
    if ("web" === get_ActivityIndicator.Platform.OS) {
      const _Object2 = Object;
      const obj3 = { width: tmp6(8585).constants.THUMB_SIZE, justifyContent: "space-between" };
      let stepIndicatorElement = Object.assign({}, tmp6(8586).styles.stepIndicatorElement, obj3);
    } else {
      stepIndicatorElement = tmp6(8586).styles.stepIndicatorElement;
    }
