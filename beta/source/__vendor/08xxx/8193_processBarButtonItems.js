...
// Dependencies: [32, 109, 17, 21, 5889, 1485, 8194, 5880, 5148]
// Exports: useHeaderConfigProps

// Module 8193 (processBarButtonItems)
import ColorDefault from "Color" /* 5889 */;
...
export const useHeaderConfigProps = function useHeaderConfigProps(headerLargeTitle) {
  ...
  let obj = colors(1485);
  const theme = colors(1485).useTheme();
  ({ colors, fonts } = theme);
  ...
  const tmpResult = colors(8194);
  let fontSize;
  [tmp10, tmp11, tmp12] = colors(8194).processFonts(items3);
  if ("fontSize" in tmp4) {
    fontSize = tmp4.fontSize;
  }
  const tmp9 = _slicedToArray(colors(8194).processFonts(items3), 3);
  const headerTitle1 = colors(5880).getHeaderTitle({ title, headerTitle }, route.name);
  if ("color" in tmp6) {
  ...
  const isSearchBarAvailableForCurrentPlatform = tmp(5148).isSearchBarAvailableForCurrentPlatform;
  ...
  if (typeof isSearchBarAvailableForCurrentPlatform === "boolean") {
    isSearchBarAvailableForCurrentPlatform2 = tmp(5148).isSearchBarAvailableForCurrentPlatform;
  }
  ...
  if (null != headerLeftResult) {
    ...
    if ("center" === headerTitleAlign) {
      ...
      let tmp32Result = tmp32(tmp(5148).ScreenStackHeaderLeftView, obj11);
    } else {
      if (typeof headerTitle === "function") {
        ...
        obj13.children = tmp62(tmp(5880).HeaderTitle, obj14);
      }
  ...
  if ("center" === headerTitleAlign) {
    if (typeof headerTitle !== "function") {
      ...
      headerTitleResult = tmp38(tmp(5880).HeaderTitle, obj15);
    }
    const obj16 = { children: headerTitleResult };
    tmp38Result = tmp38(tmp(5148).ScreenStackHeaderCenterView, obj16);
  }
  ...
  if (undefined !== headerBackIcon) {
    ...
    const obj17 = { source };
    let tmp40Result = closure_10(tmp(5148).ScreenStackHeaderBackButtonImage, obj17);
  } else {
    tmp40Result = null;
  }
  ...
  if (null != headerRightResult) {
    ...
    tmp42 = closure_10(tmp(5148).ScreenStackHeaderRightView, obj18);
  }
  items7[2] = tmp42;
  let tmp44 = null;
  if (isSearchBarAvailableForCurrentPlatform2) {
