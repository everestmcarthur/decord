...
// Dependencies: [8769, 8770, 8771]
// Exports: default, extractTransformSvgView

// Module 8768 (appendTransformProps)
import append from "append" /* 8769 */;
import peg$SyntaxError from "peg$SyntaxError" /* 8770 */;
import _mod8771 from "module_8771" /* 8771 */;
...
function transformToMatrix(arg0, arr) {
  ...
  if (arr) {
    ...
    if (Array.isArray(arr)) {
      ...
      try {
        const parsed = tmp3(8770).parse(arr);
        const tmp3Result5 = tmp3(8769);
        tmp3Result5.append(parsed[0], parsed[3], parsed[1], parsed[4], parsed[2], parsed[5]);
        const tmp3Result4 = tmp3(8770);
      } catch (tmp14) {
...
export const extractTransformSvgView = function extractTransformSvgView(transform) {
  if (typeof transform.transform === "string") {
    transform = _mod8771.parse(transform.transform);
  } else {
    transform = transform.transform;
  }
  return transform;
