...
// Dependencies: [1161, 14516, 14538, 14517, 14531, 14533]
// Exports: PartitionNumberPattern

// Module 14543 (PartitionNumberPattern)
...
let e = e_mod;
const module_14538 = e.__importDefault(_mod14538);
...
export const PartitionNumberPattern = function PartitionNumberPattern(internalSlots, isNaN) {
  ...
  if (isNaN.isNaN()) {
    ...
    if (!isNaN.isZero()) {
      _mod14517.invariant(isNaN.isFinite(), "Input must be a mathematical value");
  ...
  if ("never" !== signDisplay) {
    if ("auto" === signDisplay) {
      ...
      _mod14517.invariant("negative" === signDisplay, "signDisplay must be \"negative\"");
      let num8 = 0;
      if (roundedNumber.isNegative()) {
        num8 = 0;
        if (!roundedNumber.isZero()) {
  ...
  return module_14538.default({ roundedNumber, formattedString, exponent: num, magnitude: num2, sign: num7 }, internalSlots.dataLocaleData, pl, internalSlots);
};
