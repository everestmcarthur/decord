...
// Dependencies: [32, 19, 6954, 6955, 6965, 6966, 6967, 6969, 6970, 6971, 6950, 6934, 6972, 6932]
// Exports: checkGestureCallbacksForWorklets, extractGestureRelations, useForceRender, useWebEventHandlers

// Module 6953 (convertToHandlerTag)
...
function convertToHandlerTag(handlerTag) {
  if (typeof handlerTag === "number") {
    return handlerTag;
  } else if (handlerTag instanceof _mod6950.BaseGesture) {
...
let items = [...fn(6954).baseGestureHandlerWithDetectorProps, ...fn(6955).tapGestureHandlerProps, ...fn(6965).panGestureHandlerProps, ...fn(6965).panGestureHandlerCustomNativeProps, ...fn(6966).longPressGestureHandlerProps, ...fn(6967).forceTouchGestureHandlerProps, ...fn(6969).flingGestureHandlerProps, ...fn(6970).hoverGestureHandlerProps, ...fn(6971).nativeViewGestureHandlerProps];
...
export const useWebEventHandlers = function useWebEventHandlers() {
  return React4({
    onGestureHandlerEvent(nativeEvent) {
      const result = _mod6932.onGestureHandlerEvent(nativeEvent.nativeEvent);
    },
