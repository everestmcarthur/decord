...
// Function ID: 6987
// Name: hash
// Dependencies: [32, 6973, 6972, 6987, 6996]
...
import allowedNativeProps from "allowedNativeProps" /* 6987 */;
...
let c4 = 1.618;
fn(6973).NativeProxy.updateGestureHandlerConfig;
...
function maybeUnpackValue(enabled) {
  const Reanimated = _mod6972.Reanimated;
...
maybeUnpackValue.__closure = { Reanimated: fn(6972).Reanimated };
...
function getEnabledSharedValues(Reanimated) {
  if (undefined === _mod6972.Reanimated) {
    return [];
  } else {
    if (tmpResult.isComposedGesture(Reanimated)) {
      ...
      Reanimated = tmp(6972).Reanimated;
...
export const bindSharedValues = function bindSharedValues(shouldUseReanimatedDetector, handlerTag) {
  _require = handlerTag;
  if (undefined !== require("module_6972").Reanimated) {
    ...
    while (tmp27 !== undefined) {
      ...
      let tmp8 = tmp7;
      let tmp10 = _require;
      let Reanimated = require("module_6972").Reanimated;
      if (Reanimated.isSharedValue(tmp7)) {
        let Reanimated2 = tmp10(6972).Reanimated;
    ...
    function _loop(maybeSharedValue) {
      ...
      if (Reanimated.isSharedValue(maybeSharedValue)) {
        if (typeof hash === "function") {
          ...
          closure_1 = sum;
          const Reanimated2 = _mod6972.Reanimated;
          const fn = function t() {
