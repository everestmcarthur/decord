    ...
    if (event_id) {
      if (event_id.syntheticException) {
        ...
        if (arr.length) {
          ...
          obj2.values = items;
          obj.exception = obj2;
          const result = _mod13090.addExceptionMechanism(obj, { synthetic: true });
        }
...
export const eventFromUnknownInput = function eventFromUnknownInput(getOptions, fn, name, data) {
  ...
  if (obj2.isError(name)) {
    ...
    if (tmp2Result.isPlainObject(name)) {
      ...
      const obj = { __serialized__: tmp2(13124).normalizeToSize(name, normalizeDepth) };
      let tmp12;
      ...
      if (tmp12) {
        ...
        const result = _mod13085.extractExceptionKeysForMessage(name);
  ...
  const result1 = _mod13090.addExceptionTypeValue(obj5, undefined, undefined);
  const result2 = _mod13090.addExceptionMechanism(obj5, data);
  const obj9 = {};
