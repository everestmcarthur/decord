...
// Dependencies: [32, 41, 42, 13176, 13093, 13180, 13181, 13084, 13182]

// Module 13179 (MetricsAggregator)
...
const entry = {
  ...
  value: function add(metricType, arg1, diff, none, tags) {
    ...
    if (arg5 === undefined) {
      timestampInSecondsResult = MetricsAggregator(13093).timestampInSeconds();
      const obj2 = MetricsAggregator(13093);
    }
    const self = this;
    const rounded = Math.floor(timestampInSecondsResult);
    const sanitizeMetricKeyResult = MetricsAggregator(13180).sanitizeMetricKey(arg1);
    const obj3 = MetricsAggregator(13180);
    const sanitizeTagsResult = MetricsAggregator(13180).sanitizeTags(obj);
    const obj4 = MetricsAggregator(13180);
    const sanitizeUnitResult = MetricsAggregator(13180).sanitizeUnit(str);
    const obj5 = MetricsAggregator(13180);
    const bucketKey = MetricsAggregator(13180).getBucketKey(metricType, sanitizeMetricKeyResult, sanitizeUnitResult, sanitizeTagsResult);
    const _buckets = this._buckets;
    value = _buckets.get(bucketKey);
    let num = 0;
    if (value) {
      num = 0;
      if (metricType === tmp5(13176).SET_METRIC_TYPE) {
    ...
    if (value) {
      ...
      const tmp15 = new tmp5(13181).METRIC_MAP[metricType](diff);
      obj7.metric = tmp15;
    ...
    const obj6 = MetricsAggregator(13180);
    const result1 = MetricsAggregator(13084).updateMetricSummaryOnActiveSpan(metricType, sanitizeMetricKeyResult, diff, sanitizeUnitResult, obj, bucketKey);
    self._bucketsTotalWeight = self._bucketsTotalWeight + obj7.metric.weight;
    if (self._bucketsTotalWeight >= MetricsAggregator(13176).MAX_WEIGHT) {
      self.flush();
    }
  }
};
  ...
  },
  {
    ...
    value: function _flush() {
      ...
      if (this._forceFlush) {
        ...
        const _Math = Math;
        const rounded = Math.floor(MetricsAggregator(13093).timestampInSeconds());
        const _Map = Map;
        const diff = rounded - MetricsAggregator(13176).DEFAULT_FLUSH_INTERVAL / 1000 - self._flushShift;
        const map = new Map();
  ...
  },
  {
    ...
    value: function _captureMetrics(_buckets) {
      if (_buckets.size > 0) {
        ...
        const result = MetricsAggregator(13182).captureAggregateMetrics(this._client, mapped);
        const obj = MetricsAggregator(13182);
      }
    }
  }
];
