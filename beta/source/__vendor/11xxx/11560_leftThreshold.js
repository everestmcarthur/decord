...
// Function ID: 11561
// Name: leftThreshold
// Dependencies: [109, 19, 17, 21, 1637, 11561, 7010, 6942]
// Exports: default

// Module 11560 (leftThreshold)
import cancelAnimation from "cancelAnimation" /* 1637 */;
import _mod11561 from "module_11561" /* 11561 */;
...
function _default(hitSlop) {
  ...
  let fn = function h() {
    ...
    if (sharedValue5.value > 0) {
      ...
      num7 = tmp8(1637).interpolate(iter2.value, items2, [0, 0, 1]);
      const tmp8Result = tmp8(1637);
    }
    sharedValue7.value = num7;
    let num8 = 0;
    if (sharedValue6.value > 0) {
      const items3 = [-iter3.value, 0, 1];
      num8 = tmp8(1637).interpolate(iter2.value, items3, [1, 0, 0]);
      const tmp8Result2 = tmp8(1637);
    }
  ...
  let fn2 = function p(arg0, arg1) {
    ...
    if (!tmp2) {
      ...
      if (tmp12) {
        if (arg0 > 0) {
          let RIGHT = _mod11561.SwipeDirection.LEFT;
        } else {
          RIGHT = _mod11561.SwipeDirection.RIGHT;
        }
      ...
      if (arg1 > 0) {
        let LEFT = _mod11561.SwipeDirection.RIGHT;
      } else {
        LEFT = _mod11561.SwipeDirection.LEFT;
      }
