...
// Function ID: 7194
// Name: JSFPSMonitor
// Dependencies: [7140, 7141, 7128, 7194]

// Module 7193 (JSFPSMonitor)
...
const items = [
  entry,
  {
    ...
    value: function startTracking() {
      ...
      if (0 !== this.startTime) {
        const _Error = Error;
        const error = new Error(JSFPSMonitor(7128).ErrorMessages.fpsMonitorAlreadyRunning);
        throw error;
      } else {
        const _Date = Date;
  ...
  },
  {
    ...
    value: function stopAndGetData() {
      ...
      const obj = { minFPS: JSFPSMonitor(7194).roundToDecimalPlaces(self.minFPS, 1), maxFPS: null, averageFPS: null };
      const obj2 = JSFPSMonitor(7194);
      obj.maxFPS = JSFPSMonitor(7194).roundToDecimalPlaces(self.maxFPS, 1);
      const obj3 = JSFPSMonitor(7194);
      obj.averageFPS = JSFPSMonitor(7194).roundToDecimalPlaces(self.averageFPS, 1);
      return obj;
    }
  }
];

export const JSFPSMonitor = _modDef7141(JSFPSMonitor, items);
