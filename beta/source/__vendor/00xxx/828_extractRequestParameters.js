...
export const addChatCompletionAttributes = function addChatCompletionAttributes(setAttributes, created, recordOutputs) {
  ...
  if (created.usage) {
    ...
    if (undefined !== prompt_tokens) {
      const obj2 = {};
      obj2[tmp(823).OPENAI_USAGE_PROMPT_TOKENS_ATTRIBUTE] = prompt_tokens;
      obj2[tmp(823).GEN_AI_USAGE_INPUT_TOKENS_ATTRIBUTE] = prompt_tokens;
      setAttributes.setAttributes(obj2);
    }
    if (undefined !== completion_tokens) {
      const obj3 = {};
      obj3[tmp(823).OPENAI_USAGE_COMPLETION_TOKENS_ATTRIBUTE] = completion_tokens;
      obj3[tmp(823).GEN_AI_USAGE_OUTPUT_TOKENS_ATTRIBUTE] = completion_tokens;
      setAttributes.setAttributes(obj3);
    }
    if (undefined !== total_tokens) {
      const obj4 = {};
      obj4[tmp(823).GEN_AI_USAGE_TOTAL_TOKENS_ATTRIBUTE] = total_tokens;
  ...
  if (Array.isArray(created.choices)) {
    ...
    if (found.length > 0) {
      const obj5 = {};
      const _JSON = JSON;
      obj5[tmp(823).GEN_AI_RESPONSE_FINISH_REASONS_ATTRIBUTE] = JSON.stringify(found);
    ...
    if (recordOutputs) {
      ...
      if (flatResult.length > 0) {
        const obj6 = {};
        const _JSON2 = JSON;
        obj6[tmp(823).GEN_AI_RESPONSE_TOOL_CALLS_ATTRIBUTE] = JSON.stringify(flatResult);
...
export const addEmbeddingsAttributes = function addEmbeddingsAttributes(setAttributes, model) {
  ...
  if (model.usage) {
    ...
    if (undefined !== total_tokens) {
      const obj3 = {};
      obj3[tmp(823).GEN_AI_USAGE_TOTAL_TOKENS_ATTRIBUTE] = total_tokens;
      setAttributes.setAttributes(obj3);
    }
  }
};
export const addResponsesApiAttributes = function addResponsesApiAttributes(setAttributes, created_at, recordOutputs) {
  ...
  if (created_at.status) {
    ...
    obj2[tmp(823).GEN_AI_RESPONSE_FINISH_REASONS_ATTRIBUTE] = JSON.stringify(items);
    setAttributes.setAttributes(obj2);
  }
  if (created_at.usage) {
    ...
    if (undefined !== input_tokens) {
      const obj3 = {};
      obj3[tmp(823).OPENAI_USAGE_PROMPT_TOKENS_ATTRIBUTE] = input_tokens;
      obj3[tmp(823).GEN_AI_USAGE_INPUT_TOKENS_ATTRIBUTE] = input_tokens;
      setAttributes.setAttributes(obj3);
    }
    if (undefined !== output_tokens) {
      const obj4 = {};
      obj4[tmp(823).OPENAI_USAGE_COMPLETION_TOKENS_ATTRIBUTE] = output_tokens;
      obj4[tmp(823).GEN_AI_USAGE_OUTPUT_TOKENS_ATTRIBUTE] = output_tokens;
      setAttributes.setAttributes(obj4);
    }
    if (undefined !== total_tokens) {
      const obj5 = {};
      obj5[tmp(823).GEN_AI_USAGE_TOTAL_TOKENS_ATTRIBUTE] = total_tokens;
      setAttributes.setAttributes(obj5);
    }
  }
  if (recordOutputs) {
    ...
    if (Array.isArray(created_at.output)) {
      if (created_at.output.length > 0) {
        ...
        if (found.length > 0) {
          const obj6 = {};
          const _JSON2 = JSON;
          obj6[tmp(823).GEN_AI_RESPONSE_TOOL_CALLS_ATTRIBUTE] = JSON.stringify(found);
          setAttributes.setAttributes(obj6);
        }
      }
    }
