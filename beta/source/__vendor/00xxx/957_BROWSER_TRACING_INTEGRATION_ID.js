...
export const browserTracingIntegration = () => {
  ...
  closure_3 = { name: "icon", source: "call" };
...
export const startBrowserTracingPageLoadSpan = function startBrowserTracingPageLoadSpan(f108680, tmp2Result, arg2) {
  f108680.emit("startPageLoadSpan", tmp2Result, arg2);
  const currentScope = _mod682.getCurrentScope();
  currentScope.setTransactionName(tmp2Result.name);
  if (f108680[_sentry_idleSpan]) {
    f108680.emit("afterStartPageLoadSpan", tmp3);
  }
  return f108680[_sentry_idleSpan];
};
