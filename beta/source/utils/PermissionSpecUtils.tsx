...
// Dependencies: [2045, 2063, 1074, 8704, 17303, 7525, 5261, 17304, 1115, 2108, 7543, 5666, 17305, 7542, 17306, 9887, 8041, 2]
...
import ChannelPermissionsConstants from "ChannelPermissionsConstants" /* 8704 */;
...
function getPermissionOptions(id) {
  ...
  if (result1) {
    result1 = tmp(7525).isCreatorMonetizationEnabledGuild(guild);
...
export default {
  generateChannelPermissionSpec(guild_id, stateFromStores, arg2, arg3) {
    ...
    const VoiceInThreadsExperiment = set(7543).VoiceInThreadsExperiment;
    ...
    const tmp4Result = tmp4(5666);
    const isStageVideoEnabledResult = tmp4(5666).isStageVideoEnabled(guild_id);
    ...
    if (constants2.GUILD_CATEGORY === type) {
      ...
      if (tmp.showStageChannelPermissions) {
        ...
        const items1 = [obj33(17306).generateChannelStageSection(tmp3, intl28.string(tmp4(1115).t.yniauk))];
        let items2 = items1;
      ...
      if (tmp16.GUILD_VOICE === type) {
        ...
        items4[6] = tmp27(17306).generateChannelAppsSection(tmp3, intl15.string(tmp4(1115).t["rrh/W6"]), { showActivities: false });
        items5 = items4;
        ...
        if (tmp16.GUILD_FORUM !== type) {
          if (tmp16.GUILD_MEDIA !== type) {
            ...
            if (enabled) {
              ...
              const items6 = [tmp38(17306).generateChannelVoiceSection(tmp3, intl.string(tmp4(1115).t["46Ra1b"]))];
              let items7 = items6;
        ...
        if (enabled) {
          ...
          const items9 = [tmp22(17306).generateChannelVoiceSection(tmp3, intl7.string(tmp4(1115).t["46Ra1b"]))];
          let items10 = items9;
