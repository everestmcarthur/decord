...
// Dependencies: [19, 17, 2041, 1074, 21, 4788, 576, 9720, 1177, 1115, 9718, 504, 5235, 4930, 4840, 4784, 5218, 2]
// Exports: default

// Module 9719 (VideoEmptyState)
...
export default function VideoEmptyState(style) {
  ...
  if (obj.STREAM_ENDED === type) {
    ...
    if (!removeSplashImage) {
      ...
      tmp10 = closure_5(stream(9720).StreamEnded, obj2);
    }
  ...
  if (null != avError) {
    let errorCode = stream(9718).getErrorInfo(avError).errorCode;
    const obj6 = stream(9718);
  } else {
    errorCode = null;
    if (type === tmp3.STREAM_FAILED) {
      errorCode = stream(9718).getErrorInfo(stream(9718).AVError.STREAM_FAILED_TO_START).errorCode;
      const obj5 = stream(9718);
    }
  }
  ...
  if (tmp28) {
    ...
    tmp28 = closure_5(tmp22(4784).Text, obj9);
  }
  items4[1] = tmp28;
  ...
  if (!removeCloseButton) {
    ...
    obj11.children = closure_5(tmp22(5218).Button, obj12);
