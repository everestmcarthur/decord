...
// Dependencies: [32, 19, 17, 21, 4788, 576, 4755, 1364, 7426, 7475, 9835, 1115, 5212, 4524, 1177, 4789, 4784, 5219, 4722, 7239, 4380, 7427, 9836, 4639, 2]
// Exports: default

// Module 9834 (DatePickerActionSheet)
...
import BottomSheetTitleHeader2 from "BottomSheetTitleHeader" /* 7426 */;
...
function ActionSheetHeader(handleSubmit) {
  ...
  if (isAndroidResult) {
    ...
    obj2.trailing = tmp4(tmp(7475).ActionSheetCloseButton, obj3);
    ...
    obj4.leading = tmp4(tmp(9835).ActionSheetHeaderPressableText, obj5);
    const obj6 = { onPress: handleSubmit.handleSubmit, label: null };
    const intl2 = tmp(1115).intl;
    obj6.label = intl2.string(tmp(1115).t["R3BPH+"]);
    obj4.trailing = tmp4(tmp(9835).ActionSheetHeaderPressableText, obj6);
...
function DateRangeError(children) {
  ...
  let obj = show(4524);
  const tmp4 = show;
  A.__closure = { STANDARD_EASING: show(1177).STANDARD_EASING, show, withTiming: show(4789).withTiming };
  ...
  let obj2 = { STANDARD_EASING: show(1177).STANDARD_EASING, show, withTiming: show(4789).withTiming };
  obj3.children = closure_6(View, { ref, accessible: true, accessibilityRole: "alert", style: tmp.rangeError, children: closure_6(tmp4(4784).Text, { variant: "text-md/medium", color: "text-feedback-critical", children: children.errorText }) });
  return closure_6(ref(4524).View, obj3);
}
function ActionSheetFooter(arg0) {
  ...
  if (obj.isAndroid()) {
    ...
    obj3.textElement = timestampProducer(tmp2(4784).Text, obj4);
    const intl2 = tmp2(1115).intl;
    obj3.accessibilityLabel = intl2.string(tmp2(1115).t["ETE/oC"]);
    obj3.style = tmp.actionButton;
    obj3.onPress = handleCancel;
    const items = [timestampProducer(tmp2(5219).BaseTextButton, obj3), ];
...
export default function DatePickerActionSheet(mode) {
  ...
  const tmp26 = minimumDate(7239)(() => {
    let tmp = first1;
    if (first1) {
      tmp = c9;
    }
    if (tmp) {
      dependencyMap(_modDef4380(ref.current));
    }
    if (c9) {
      ActionSheetActionCreatorsDefault.hideActionSheet();
    } else {
  ...
  const tmp27 = minimumDate(7239)((getTime) => {
    if (null != getTime) {
  ...
  const obj5 = minimumDate(4380)(minimumDate);
  obj3.errorText = intl2.formatToPlainString(maximumDate(1115).t.FsJO55, { minDate: minimumDate(4380)(minimumDate).format(str3) });
  ...
  const obj4 = { minDate: minimumDate(4380)(minimumDate).format(str3) };
  if ("date" === str) {
    str2 = "L";
  }
  const obj8 = minimumDate(4380)(maximumDate);
  obj6.errorText = intl3.formatToPlainString(maximumDate(1115).t.R7r9VN, { maxDate: minimumDate(4380)(maximumDate).format(str2) });
  items1[1] = closure_6(ref, obj6);
  const obj9 = { style: tmp3.datetimePickerContainer, children: null };
  const obj7 = { maxDate: minimumDate(4380)(maximumDate).format(str2) };
  const tmp38 = date;
  const tmp13Result = minimumDate(9836);
  let str4 = "dark";
  ...
  return tmp28(maximumDate(7427).BottomSheet, obj2);
};
