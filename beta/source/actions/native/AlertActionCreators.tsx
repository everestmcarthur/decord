...
// Dependencies: [19, 21, 4755, 573, 5142, 1115, 5146, 5237, 5237, 1980, 2]

// Module 5141 (actions/AlertActionCreators)
...
export default {
  ...
  show(hideActionSheet) {
    ...
    if (!tmp7) {
      if (flag) {
        merged(4755).hideActionSheet();
        const obj4 = merged(4755);
      }
      if (null != c6) {
        confirmText(5142).dismissAlert(c6);
        const obj5 = confirmText(5142);
      }
      ...
      const tmp31 = c5;
      const tmp32 = c4;
      obj8.variant = confirmText(5237).getAlertButtonVariant(confirmColor);
      ...
      const items = [c3(confirmText(5146).AlertActionButton, obj8), ];
      ...
      if (null != cancelText) {
        ...
        tmp30Result = tmp30(tmp29(5146).AlertActionButton, obj9);
      }
      ...
      obj7.openAlert(text, c3(confirmText(5146).AlertModal, obj3), () => {
        if (!c5) {
          if (c4 != null) {
            tmp();
          }
      ...
      const obj10 = confirmText(5237);
    } else {
      if (null != c6) {
        confirmText(5142).dismissAlert(c6);
        c6 = null;
        const obj2 = confirmText(5142);
      }
      const self = this;
      const obj13 = {
        importer() {
            return asyncRequireImpl(5237, dependencyMap.paths).then((result) => {
