...
// Function ID: 4666
// Name: TimeUtils
// Dependencies: [5, 4666, 569, 4667, 2]
// Exports: convertMinutesToGivenTimeUnit, getTimeAndUnit, getTimeUnit

// Module 4665 (TimeUtils)
import u from "u" /* 569 */;
import findLastIndexDefault from "findLastIndex" /* 4667 */;
...
export const getTimeUnit = function getTimeUnit(arg0, arg1) {
  closure_0 = arg0;
  importDefault = arg1;
  const tmp2 = findLastIndexDefault(items, (unit) => f78891(unit.unit), items.findIndex((max) => {
    max = max.max;
  ...
  if (null != tmp2) {
    ...
    const found = arr.find((unit) => f78891(unit.unit));
    let unit = null;
    if (null != found) {
      unit = found.unit;
    }
...
export const getTimeAndUnit = function getTimeAndUnit(rounded, items) {
  ...
  if (null == rounded) {
    ...
    closure_0 = rounded;
    const f78891 = (dependencyMap) => closure_0.includes(dependencyMap);
    const tmp12 = f78891(4667)(items, (unit) => f78891(unit.unit), items.findIndex((max) => {
      max = max.max;
      let tmp = max.unit === obj.NONE;
      if (tmp) {
        tmp = closure_0 === max;
