...
// Function ID: 9994
// Name: _asyncLoop
// Dependencies: [41, 42, 5, 9994, 9995]
...
export const concatBytes = function concatBytes() {
  ...
  if (0 < items.length) {
    do {
      let arr2 = items[num];
      let bytesResult = Hash(9994).bytes(arr2);
...
export const randomBytes = function randomBytes(result) {
  ...
  if (Hash(9995).crypto) {
    if (typeof tmp(9995).crypto.getRandomValues === "function") {
      const _crypto2 = tmp(9995).crypto;
      const _Uint8Array = Uint8Array;
  ...
  if (Hash(9995).crypto) {
    if (typeof tmp(9995).crypto.randomBytes === "function") {
      const _crypto = tmp(9995).crypto;
