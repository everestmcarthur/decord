...
function createProfilePayload(event_id, arg1, resources, type) {
  if ("transaction" !== type.type) {
    ...
    if (arg1) {
      ...
      if (stackParser) {
        let debugImagesForResources = tmp22(682).getDebugImagesForResources(stackParser, resources.resources);
        const tmp22Result = tmp22(682);
      } else {
      ...
      if (!event_id) {
        event_id = tmp22(682).uuid4();
        const tmp22Result2 = tmp22(682);
      }
      obj7.id = event_id;
      obj7.trace_id = str4;
...
export const createProfileChunkPayload = function createProfileChunkPayload(frames, _client, _profilerId) {
  if (null == frames) {
    ...
    if (!_profilerId) {
      uuid4Result = tmp24(682).uuid4();
      const tmp24Result = tmp24(682);
    }
