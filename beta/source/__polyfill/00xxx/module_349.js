...
class ScrollView {
  constructor(arg0) {
    ...
    tmp3Result.getScrollResponder = () => f109827;
    tmp3Result.getScrollableNode = () => renderElement.findNodeHandle(f109827.getNativeScrollRef());
    tmp3Result.getInnerViewNode = () => renderElement.findNodeHandle(f109827._innerView.nativeInstance);
    tmp3Result.getInnerViewRef = () => f109827._innerView.nativeInstance;
    tmp3Result.getNativeScrollRef = () => f109827._scrollView.nativeInstance;
    tmp3Result.scrollTo = (num, arg1, arg2) => {
      ...
      const nativeScrollRef = f109827.getNativeScrollRef();
    ...
    tmp3Result.scrollResponderScrollNativeHandleToKeyboard = (measureLayout, arg1, _preventNegativeScrollOffset) => {
      ...
      f109827._additionalScrollOffset = num;
      f109827._preventNegativeScrollOffset = _preventNegativeScrollOffset;
      if (null != f109827._innerView.nativeInstance) {
    ...
    tmp3Result.scrollResponderZoomTo = (animated, arg1) => {
      ...
      if ("animated" in animated) {
        f109827._animated = animated.animated;
        delete tmp[tmp2];
      ...
      if (null == _keyboardMetrics._keyboardMetrics) {
        ...
        const timerId = setTimeout(() => {
          if (typeof scrollTextInputIntoVisibleRect === "function") {
            if (null != f109827._keyboardMetrics) {
              screenY = obj._keyboardMetrics.screenY;
            }
            const sum = closure_0 - screenY + closure_1 + obj._additionalScrollOffset;
            let bound = sum;
            if (true === f109827._preventNegativeScrollOffset) {
              const _Math = Math;
              bound = Math.max(0, sum);
            }
            const point = { x: 0, y: bound, animated: true };
            f109827.scrollTo(point);
            f109827._additionalScrollOffset = 0;
            f109827._preventNegativeScrollOffset = false;
          } else {
    ...
    tmp3Result._handleScroll = (arg0) => {
      f109827._observedScrollSinceBecomingResponder = true;
      if (f109827.props.onScroll) {
        const props = tmp.props;
        props.onScroll(arg0);
      }
    };
    tmp3Result._handleLayout = (nativeEvent) => {
      if (true === f109827.props.invertStickyHeaders) {
        const obj2 = { layoutHeight: nativeEvent.nativeEvent.layout.height };
        obj.setState(obj2);
      }
      if (f109827.props.onLayout) {
        const props = obj.props;
        props.onLayout(nativeEvent);
      }
    };
    tmp3Result._handleContentOnLayout = (arg0) => {
      if (f109827.props.onContentSizeChange) {
        const props = f109827.props;
    ...
    f109827 = (arg0) => Object.assign(arg0, { getScrollResponder: f109827.getScrollResponder, getScrollableNode: f109827.getScrollableNode, getInnerViewNode: f109827.getInnerViewNode, getInnerViewRef: f109827.getInnerViewRef, getNativeScrollRef: f109827.getNativeScrollRef, scrollTo: f109827.scrollTo, scrollToEnd: f109827.scrollToEnd, flashScrollIndicators: f109827.flashScrollIndicators, scrollResponderZoomTo: f109827.scrollResponderZoomTo, scrollResponderScrollNativeHandleToKeyboard: f109827.scrollResponderScrollNativeHandleToKeyboard });
    closure_1 = undefined;
    obj4 = {
      getForwardingRef: c2(closure_4[29])((arg0) => {
            closure_0 = arg0;
    ...
    tmp3Result.scrollResponderKeyboardWillShow = (endCoordinates) => {
      f109827._keyboardMetrics = endCoordinates.endCoordinates;
      if (f109827.props.onKeyboardWillShow) {
        const props = f109827.props;
        props.onKeyboardWillShow(endCoordinates);
      }
    };
    tmp3Result.scrollResponderKeyboardWillHide = (arg0) => {
      f109827._keyboardMetrics = null;
      if (f109827.props.onKeyboardWillHide) {
        const props = tmp.props;
    ...
    tmp3Result.scrollResponderKeyboardDidShow = (endCoordinates) => {
      f109827._keyboardMetrics = endCoordinates.endCoordinates;
      if (f109827.props.onKeyboardDidShow) {
        const props = f109827.props;
    ...
    tmp3Result.scrollResponderKeyboardDidHide = (arg0) => {
      f109827._keyboardMetrics = null;
      if (f109827.props.onKeyboardDidHide) {
        const props = tmp.props;
    ...
    tmp3Result._handleMomentumScrollBegin = (arg0) => {
      ...
      f109827._lastMomentumScrollBeginTime = _performance.now();
      if (f109827.props.onMomentumScrollBegin) {
        const props = f109827.props;
    ...
    tmp3Result._handleMomentumScrollEnd = (arg0) => {
      ...
      const _performance = ScrollView.performance;
      f109827._lastMomentumScrollEndTime = _performance.now();
      if (f109827.props.onMomentumScrollEnd) {
        const props = f109827.props;
    ...
    tmp3Result._handleScrollBeginDrag = (arg0) => {
      ...
      if ("on-drag" === f109827.props.keyboardDismissMode) {
        dismissKeyboardDefault();
      }
      if (f109827.props.onScrollBeginDrag) {
    ...
    tmp3Result._handleScrollEndDrag = (nativeEvent) => {
      ...
      let _isAnimatingResult = f109827._isAnimating();
      ...
      if (f109827.props.onScrollEndDrag) {
        const props = tmp.props;
    ...
    tmp3Result._isAnimating = () => {
      const _performance = ScrollView.performance;
      return _performance.now() - f109827._lastMomentumScrollEndTime < 16 || f109827._lastMomentumScrollEndTime < f109827._lastMomentumScrollBeginTime;
    };
    tmp3Result._handleResponderGrant = (arg0) => {
      f109827._observedScrollSinceBecomingResponder = false;
      if (f109827.props.onResponderGrant) {
        const props = obj.props;
        props.onResponderGrant(arg0);
      }
      f109827._becameResponderWhileAnimating = f109827._isAnimating();
    };
    tmp3Result._handleResponderReject = () => {

    };
    tmp3Result._handleResponderRelease = (nativeEvent) => {
      f109827._isTouching = 0 !== nativeEvent.nativeEvent.touches.length;
      if (f109827.props.onResponderRelease) {
        const props = obj.props;
    ...
    tmp3Result._handleResponderTerminationRequest = () => !f109827._observedScrollSinceBecomingResponder;
    tmp3Result._handleScrollShouldSetResponder = () => true !== f109827.props.disableScrollViewPanResponder && f109827._isTouching;
    tmp3Result._handleStartShouldSetResponder = (target) => {
      if (true === f109827.props.disableScrollViewPanResponder) {
        return false;
      } else {
    ...
    tmp3Result._softKeyboardIsDetached = () => {
      let tmp = null != f109827._keyboardMetrics;
      if (tmp) {
        tmp = 0 === f109827._keyboardMetrics.height;
      }
      return tmp;
    };
    ...
    tmp3Result._handleTouchEnd = (nativeEvent) => {
      f109827._isTouching = 0 !== nativeEvent.nativeEvent.touches.length;
      const keyboardShouldPersistTaps = f109827.props.keyboardShouldPersistTaps;
      let tmp = !keyboardShouldPersistTaps;
      if (keyboardShouldPersistTaps) {
        tmp = "never" === keyboardShouldPersistTaps;
      }
