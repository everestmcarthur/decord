...
if (null != installedNitro) {
  ...
  if (installedNitro.version !== _mod4529.version) {
    const _Error2 = Error;
    const version = installedNitro.version;
    const _HermesInternal2 = HermesInternal;
    const error = new Error("Nitro was installed twice: once with native version " + version + " and once with JS version " + _mod4529.version + ". This usually means react-native-nitro-modules exists multiple times in node_modules (e.g. in monorepos or double-linked setups).");
    throw error;
  }
} else {
  try {
    ...
    if (null != installResult) {
      ...
      if (null == installedNitro1) {
        ...
        const moduleNotFoundError = new _mod4530.ModuleNotFoundError(error2);
