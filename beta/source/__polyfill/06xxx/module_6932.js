// Module ID: 6932
// Function ID: 6933
// Dependencies: [17, 6933, 6935, 6936, 6937]
...
function onGestureHandlerEvent(handlerTag) {
  ...
  if (findHandlerResult) {
    if (null != handlerTag.oldState) {
      if (handlerTag.oldState === tmp(6935).State.UNDETERMINED) {
        if (handlerTag.state === tmp(6935).State.BEGAN) {
      ...
      if (handlerTag.oldState === tmp(6935).State.BEGAN) {
        if (handlerTag.state === tmp(6935).State.ACTIVE) {
      ...
      if (handlerTag.oldState !== handlerTag.state) {
        if (handlerTag.state === tmp(6935).State.END) {
          if (handlerTag.oldState === tmp(6935).State.ACTIVE) {
      ...
      let tmp18 = handlerTag.state !== tmp(6935).State.FAILED;
      if (tmp18) {
        tmp18 = handlerTag.state !== tmp(6935).State.CANCELLED;
      }
      if (!tmp18) {
      ...
      if (!map.has(handlerTag.handlerTag)) {
        const GestureStateManager = tmp(6936).GestureStateManager;
      ...
      if (tmp(6937).TouchEventType.TOUCHES_DOWN === eventType) {
      ...
      } else if (tmp(6937).TouchEventType.TOUCHES_MOVE === eventType) {
      ...
      } else if (tmp(6937).TouchEventType.TOUCHES_UP === eventType) {
      ...
      } else if (tmp(6937).TouchEventType.TOUCHES_CANCEL === eventType) {
    ...
    const result2 = tmp(6933).findOldGestureHandler(handlerTag.handlerTag);
    if (result2) {
