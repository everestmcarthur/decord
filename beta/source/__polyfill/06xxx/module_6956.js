// Module ID: 6956
// Function ID: 6957
// Dependencies: [41, 42, 93, 95, 98, 19, 17, 21, 6957, 6935, 6951, 6958, 6960, 6933, 6962, 6961, 6963, 6964, 6934, 6948]
// Exports: default

// Module 6956
import handlerIDToTag2 from "handlerIDToTag" /* 6933 */;
...
import ghQueueMicrotask from "ghQueueMicrotask" /* 6961 */;
...
const jsx = fn(21).jsx;
fn(6957).customDirectEventTypes.topGestureHandlerEvent = { registrationName: "onGestureHandlerEvent" };
let closure_10 = { [fn(6935).State.UNDETERMINED]: undefined, [fn(6935).State.BEGAN]: "onBegan", [fn(6935).State.FAILED]: "onFailed", [fn(6935).State.CANCELLED]: "onCancelled", [fn(6935).State.ACTIVE]: "onActivated", [fn(6935).State.END]: "onEnded" };
...
export default function createHandler(name) {
  ...
  class Handler {
    constructor(arg0) {
      ...
      tmp3Result.createGestureHandler = (config) => {
        closure_0.handlerTag = _mod6951.getNextHandlerTag();
          ...
          if (!onHandlerStateChange) {
            if (onGestureEvent) {
              if ("__isNative" in onGestureEvent) {
                let REANIMATED_WORKLET = tmp2(6962).ActionType.NATIVE_ANIMATED_EVENT;
              }
            }
            REANIMATED_WORKLET = tmp2(6962).ActionType.JS_FUNCTION_OLD_API;
          }
          RNGestureHandlerModuleDefault.attachGestureHandler(tmp.handlerTag, viewTag, REANIMATED_WORKLET);
          const result1 = tmp2(6960).scheduleFlushOperations();
          const tmp2Result = tmp2(6960);
          tmp2(6961).ghQueueMicrotask(() => {
            const MountRegistry = Handler(config[16]).MountRegistry;
            const result = MountRegistry.gestureHandlerWillMount(closure_1_0);
          });
        }
        REANIMATED_WORKLET = tmp2(6962).ActionType.REANIMATED_WORKLET;
      };
  ...
  const entry = {
    ...
    value: function componentDidMount() {
      ...
      if (someResult) {
        if (someResult) {
          Handler(6961).ghQueueMicrotask(() => {
            self.update(1);
          });
          const obj = Handler(6961);
        }
        const props2 = self.props;
        const items = [];
        const obj2 = Handler(6960);
        ...
        if (self.viewNode) {
          self.attachGestureHandler(allowedProps(6964)(self.viewNode));
        } else {
          const _Error = Error;
