// Module ID: 13122
// Function ID: 13123
// Dependencies: [13078, 13079, 13123, 13126, 13115, 13084]
...
export const createEventEnvelope = function createEventEnvelope(type, arg1, sdk, arg3) {
  const sdkMetadataForEnvelopeHeader = _mod13123.getSdkMetadataForEnvelopeHeader(sdk);
  let str = "event";
  if (type.type) {
    str = "event";
  ...
  const eventEnvelopeHeaders = _mod13123.createEventEnvelopeHeaders(type, sdkMetadataForEnvelopeHeader, arg3, arg1);
  delete tmp[tmp2];
  const items2 = [{ type: str }, type];
  const tmp3Result = _mod13123;
  const items3 = [items2];
  return _mod13123.createEnvelope(eventEnvelopeHeaders, items3);
};
export const createSessionEnvelope = function createSessionEnvelope(toJSON, arg1, arg2, arg3) {
  const sdkMetadataForEnvelopeHeader = _mod13123.getSdkMetadataForEnvelopeHeader(arg2);
  ...
  let tmp6 = arg3 && arg1;
  if (tmp6) {
    const obj4 = { dsn: tmp(13126).dsnToString(arg1) };
    tmp6 = obj4;
    const tmpResult = tmp(13126);
  }
  ...
  const date = new Date();
  const items2 = [items1];
  return _mod13123.createEnvelope(obj2, items2);
};
export const createSpanEnvelope = function createSpanEnvelope(arg0, getDsn) {
  const dynamicSamplingContextFromSpan = beforeSendSpan(13115).getDynamicSamplingContextFromSpan(arg0[0]);
  let dsn = getDsn;
  if (getDsn) {
    dsn = getDsn.getDsn();
  }
  ...
  const obj = beforeSendSpan(13115);
  ...
  let tmp9 = tunnel && dsn;
  if (tmp9) {
    const obj4 = { dsn: tmp2(13126).dsnToString(dsn) };
    tmp9 = obj4;
    const tmp2Result = tmp2(13126);
  }
