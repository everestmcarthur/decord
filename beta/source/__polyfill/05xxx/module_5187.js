// Module ID: 5187
// Function ID: 5188
// Dependencies: [109, 19, 17, 21, 5175, 5185, 5184, 5188, 5189, 5192, 5174, 5194, 5165]

// Module 5187
import _modDef5165 from "module_5165" /* 5165 */;
import _modDef5184 from "module_5184" /* 5184 */;
import styleDefault from "style" /* 5189 */;
...
export default noop.forwardRef(function ScreenStackItem(arg0, ref) {
  ...
  dependencyMap = flag5.useContext(tmp4(5185).RNSScreensRefContext);
  ...
  const effect = obj2.useEffect(() => {
    _modDef5184(false, "Dynamically changing header's visibility in modals will result in remounting the screen and losing all local state.");
  ...
  let obj = screenId(5175);
  const tmp3 = !hidden;
  if (isIOS26OrHigher) {
    isIOS26OrHigher = tmp10;
  }
  if (isIOS26OrHigher) {
    isIOS26OrHigher = tmp4(5188).isIOS26OrHigher;
  }
  _modDef5184(isIOS26OrHigher, "[RNScreens] Using both `blurEffect` and `scrollEdgeEffects` simultaneously may cause overlapping effects.");
  ...
  if (screenId(5188).isIOS26OrHigher) {
    const obj5 = { edges: {}, children };
    tmp19Result = tmp19(tmp4(5192).SafeAreaView, obj5);
  }
  obj4.children = tmp19Result;
  obj3.children = closure_4(styleDefault, obj4);
  const items1 = [closure_4(screenId(5175).EdgeInsetApplicationContext.Provider, obj3), , ];
  const merged1 = Object.assign(headerConfig);
  items1[1] = closure_4(screenId(5174).ScreenStackHeaderConfig, {});
  let tmp19Result2 = "formSheet" === stackPresentation && unstable_sheetFooter;
  if (tmp19Result2) {
    const obj7 = { children: unstable_sheetFooter() };
    tmp19Result2 = tmp19(tmp4(5194).FooterComponent, obj7);
  }
  ...
  return closure_4(_modDef5165, obj8);
});
