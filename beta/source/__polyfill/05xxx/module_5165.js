// Module ID: 5165
// Function ID: 5166
// Dependencies: [109, 19, 17, 21, 5166, 5167, 5168, 5164, 5169, 5170, 5172, 5155, 5173]

// Module 5165
import get_synchronousScreenUpdatesEnabledDefault from "get synchronousScreenUpdatesEnabled" /* 5155 */;
import _modDef5166 from "module_5166" /* 5166 */;
import animatedComponentDefault from "animatedComponent" /* 5167 */;
import _mod5168 from "module_5168" /* 5168 */;
import DelayedFreezeDefault from "DelayedFreeze" /* 5170 */;
...
const jsx = fn(21).jsx;
const __INTERNAL_VIEW_CONFIG = Animated.createAnimatedComponent(_modDef5166);
...
const forwardRefResult = noop.forwardRef(function InnerScreen(activityState, ref) {
  ...
  const previous = _mod5168.usePrevious(activityState.activityState);
  ...
  if (undefined === enabled) {
    enabled = tmp2(5164).screensEnabled();
    const tmp2Result = tmp2(5164);
  }
  let freezeOnBlur = activityState.freezeOnBlur;
  if (undefined === freezeOnBlur) {
    freezeOnBlur = tmp2(5164).freezeEnabled();
    const tmp2Result6 = tmp2(5164);
  }
  ...
  if (undefined === SHEET_DIMMED_ALWAYS) {
    SHEET_DIMMED_ALWAYS = tmp2(5169).SHEET_DIMMED_ALWAYS;
  }
  ...
  if (enabled) {
    if (tmp2(5164).isNativePlatformSupported) {
      const sheetAllowedDetents1 = tmp2(5169).resolveSheetAllowedDetents(sheetAllowedDetents);
      const tmp2Result7 = tmp2(5169);
      const sheetLargestUndimmedDetent = tmp2(5169).resolveSheetLargestUndimmedDetent(SHEET_DIMMED_ALWAYS, sheetAllowedDetents1.length - 1);
      const tmp2Result8 = tmp2(5169);
      ...
      const sheetInitialDetentIndex1 = tmp2(5169).resolveSheetInitialDetentIndex(num3, sheetAllowedDetents1.length - 1);
      ...
      obj4.fullScreenSwipeEnabled = tmp2(5172).parseBooleanToOptionalBooleanNativeProp(fullScreenSwipeEnabled);
      let num7;
      ...
      if (isNativeStack) {
        ...
        tmp31Result = tmp31(tmp32(5173).Provider, obj7);
      }
