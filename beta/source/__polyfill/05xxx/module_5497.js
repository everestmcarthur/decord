// Module ID: 5497
// Function ID: 5498
// Dependencies: [32, 42, 41, 93, 95, 98, 158, 5462, 5498, 5499, 5500]

// Module 5497
import _wrapNativeSuperDefault from "_wrapNativeSuper" /* 158 */;
import _modDef5498 from "module_5498" /* 5498 */;
import calculateGPSValueDefault from "calculateGPSValue" /* 5500 */;
...
function readTags(_raw, buffer, arg2) {
  try {
    ...
    ParseError(5462).objectAssign(_raw, parseXMPObject(convertToObject(tmp6, true)));
    return true;
  } catch (err) {
    return false;
  }
  ...
  try {
    ...
    if ("ParseError" === tmp12.name) {
      if (obj2.isMissingNamespaceError(tmp12)) {
        if (!tmp4) {
          return parseFromString(tmp2, tmp13(5499).addMissingNamespaces(tmp), true);
        }
      }
      obj2 = ParseError(5499);
      tmp13 = ParseError;
    }
    throw tmp12;
  }
    ...
    for (const key10002 in arg0) {
      ...
      let item = arr2.forEach((attributes) => {
        ...
        if (typeof attributes.value === "object") {
          ParseError(5462).objectAssign(obj, parseNodeChildrenAsTags(attributes.value));
          const tmpResult = ParseError(5462);
        }
      });
      continue;
    }
  ...
  if (Array.isArray(arr)) {
    ...
    if (tmp2) {
      ...
      if (typeof calculateGPSValueDefault[tmp2] === "function") {
        tmp12 = tmp13(5500)[tmp2](arr, tmp11);
        const tmp13Result = tmp13(5500);
      }
      tmp13 = importDefault;
    }
    return tmp12;
...
function parseNodeAsSimpleRdfDescription(attributes, key10005) {
  ...
  ParseError(5462).objectAssign(obj, obj4, obj6);
  const prop = iter.value["rdf:value"];
  const obj3 = ParseError(5462);
...
function parseNodeAsStructureRdfDescription(value, key10005) {
  ...
  if (undefined !== value.value["rdf:Description"]) {
    ParseError(5462).objectAssign(obj.value, parseNodeAttributesAsTags(value.value["rdf:Description"].attributes));
    const obj4 = ParseError(5462);
    const obj6 = {};
    ...
    ParseError(5462).objectAssign(obj.attributes, obj6);
    iter = value.value["rdf:Description"];
    const obj5 = ParseError(5462);
  }
  ParseError(5462).objectAssign(obj.value, parseNodeChildrenAsTags(iter.value));
