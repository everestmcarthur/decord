...
export default {
  read(dataView, sum) {
    ...
    let tmp4;
    if (8 <= shortAt) {
      const byteAt = tmp(5479).getByteAt(dataView, sum + 7);
    ...
    let tmp6;
    if (3 <= shortAt) {
      const byteAt1 = tmp(5479).getByteAt(dataView, sum + 2);
