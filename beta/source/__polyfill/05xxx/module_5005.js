// Module ID: 5005
// Function ID: 5006
// Dependencies: [109, 19, 17, 21, 5006, 5007, 5008, 5004, 5009, 5010, 5012, 4995, 5013]

// Module 5005
import get_synchronousScreenUpdatesEnabledDefault from "get synchronousScreenUpdatesEnabled" /* 4995 */;
import _modDef5006 from "module_5006" /* 5006 */;
import animatedComponentDefault from "animatedComponent" /* 5007 */;
import _mod5008 from "module_5008" /* 5008 */;
import DelayedFreezeDefault from "DelayedFreeze" /* 5010 */;
...
const jsx = fn(21).jsx;
const __INTERNAL_VIEW_CONFIG = Animated.createAnimatedComponent(_modDef5006);
...
const forwardRefResult = noop.forwardRef(function InnerScreen(activityState, ref) {
  ...
  if (undefined === enabled) {
    enabled = tmp2(5004).screensEnabled();
    const tmp2Result = tmp2(5004);
  }
  let freezeOnBlur = activityState.freezeOnBlur;
  if (undefined === freezeOnBlur) {
    freezeOnBlur = tmp2(5004).freezeEnabled();
    const tmp2Result6 = tmp2(5004);
  }
  ...
  if (undefined === SHEET_DIMMED_ALWAYS) {
    SHEET_DIMMED_ALWAYS = tmp2(5009).SHEET_DIMMED_ALWAYS;
  }
  ...
  if (enabled) {
    if (tmp2(5004).isNativePlatformSupported) {
      const sheetAllowedDetents1 = tmp2(5009).resolveSheetAllowedDetents(sheetAllowedDetents);
      const tmp2Result7 = tmp2(5009);
      const sheetLargestUndimmedDetent = tmp2(5009).resolveSheetLargestUndimmedDetent(SHEET_DIMMED_ALWAYS, sheetAllowedDetents1.length - 1);
      const tmp2Result8 = tmp2(5009);
      ...
      const sheetInitialDetentIndex1 = tmp2(5009).resolveSheetInitialDetentIndex(num3, sheetAllowedDetents1.length - 1);
      ...
      obj4.fullScreenSwipeEnabled = tmp2(5012).parseBooleanToOptionalBooleanNativeProp(fullScreenSwipeEnabled);
      let num7;
      ...
      if (isNativeStack) {
        ...
        tmp31Result = tmp31(tmp32(5013).Provider, obj7);
      }
