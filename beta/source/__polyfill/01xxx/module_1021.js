...
function createSpanJSON(span_id) {
  ...
  if (span_id.span_id) {
    ...
    span_id = tmp(682).uuid4().substring(16);
    const str = tmp(682).uuid4();
  }
  const obj2 = { span_id, trace_id: null, data: null };
  if (span_id.trace_id) {
    let trace_id = span_id.trace_id;
  } else {
    trace_id = tmp(682).uuid4();
    const tmpResult3 = tmp(682);
  }
  obj2.trace_id = trace_id;
  const obj = _mod682;
  const tmpResult4 = _mod682;
  obj2.data = tmpResult4.dropUndefinedKeys(Object.assign({ [_mod682.SEMANTIC_ATTRIBUTE_SENTRY_OP]: span_id.op, [_mod682.SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN]: span_id.origin }, span_id.data ? span_id.data : {}));
...
export const setSpanDurationAsMeasurementOnSpan = function setSpanDurationAsMeasurementOnSpan(time_to_initial_display, span, activeSpan) {
  ...
  if (tmp4) {
    ...
    obj2[tmp(682).SEMANTIC_ATTRIBUTE_SENTRY_MEASUREMENT_VALUE] = result;
