// Module ID: 1711
// Function ID: 1712
// Dependencies: [1647, 1676, 1712, 1713, 1714]
// Exports: withDecay

// Module 1711
import _mod1647 from "module_1647" /* 1647 */;
import rubberBandDecay from "rubberBandDecay" /* 1713 */;
import rigidDecay from "rigidDecay" /* 1714 */;
  ...
  if (clamp.clamp) {
    ...
    if (Array.isArray(clamp.clamp)) {
      if (2 !== clamp.clamp.length) {
        ...
        const reanimatedError = new _mod1647.ReanimatedError("`clamp array` must contain 2 items but is given " + clamp.clamp.length + ".");
        throw reanimatedError;
      }
    } else {
      const _HermesInternal = HermesInternal;
      const reanimatedError1 = new _mod1647.ReanimatedError("`config.clamp` must be an array but is " + typeof clamp.clamp + ".");
      throw reanimatedError1;
    }
  }
  if (clamp.velocityFactor <= 0) {
    const _HermesInternal2 = HermesInternal;
    const reanimatedError2 = new _mod1647.ReanimatedError("`config.velocityFactor` must be greater then 0 but is " + clamp.velocityFactor + ".");
    throw reanimatedError2;
  } else if (clamp.rubberBandEffect) {
    if (!clamp.clamp) {
      const reanimatedError3 = new _mod1647.ReanimatedError("You need to set `clamp` property when using `rubberBandEffect`.");
...
fn = function n(userConfig, callback) {
  ...
  const fn = function c() {
    ...
    const obj3 = {
      ...
      current: "disabled",
      lastTimestamp: null,
      startTimestamp: "PremiumGiftPurchaseButton",
      reduceMotion: "US"
    };
  ...
  fn.__closure = { userConfig, isValidRubberBandConfig: require("module_1712").isValidRubberBandConfig, rubberBandDecay: require("rubberBandDecay").rubberBandDecay, rigidDecay: require("rigidDecay").rigidDecay, validateConfig, callback, getReduceMotionForAnimation: require("module_1676").getReduceMotionForAnimation };
  fn.__workletHash = 17099614658252;
  fn.__initData = __initData;
  return obj.defineAnimation(0, fn);
};
fn.__closure = { defineAnimation: fn(1676).defineAnimation, isValidRubberBandConfig: fn(1712).isValidRubberBandConfig, rubberBandDecay: fn(1713).rubberBandDecay, rigidDecay: fn(1714).rigidDecay, validateConfig, getReduceMotionForAnimation: fn(1676).getReduceMotionForAnimation };
