...
// Function ID: 4522
// Name: registerHost
// Dependencies: [4522, 4517]

// Module 4521 (registerHost)
import ACTIONS from "ACTIONS" /* 4517 */;
import print from "print" /* 4522 */;
...
function reducer(arg0, type) {
  ...
  if (ACTIONS.ACTIONS.REGISTER_HOST === type) {
  ...
  } else if (tmp4(4517).ACTIONS.DEREGISTER_HOST === type) {
    const hostName3 = type.hostName;
    delete tmp[tmp2];
    return obj;
  } else if (tmp4(4517).ACTIONS.ADD_UPDATE_PORTAL === type) {
    ...
    return obj;
  } else if (tmp4(4517).ACTIONS.REMOVE_PORTAL === type) {
    ...
    if (typeof removePortal === "function") {
      if (hostName in obj) {
        ...
        tmp4(4522).print(obj3);
        const tmp4Result = tmp4(4522);
      }
      return obj;
    } else {
      throw new TypeError("Trying to call a non-function");
