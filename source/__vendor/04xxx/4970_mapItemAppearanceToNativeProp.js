...
// Dependencies: [109, 32, 19, 17, 21, 4964, 4971, 4937, 4929, 4939]
// Exports: default

// Module 4970 (mapItemAppearanceToNativeProp)
...
export default function TabsScreen(onWillAppear) {
  ...
  if (obj2.freezeEnabled()) {
    ...
    if (undefined === freezeContents) {
      let tmp10 = !tmp5;
      if (tmp8(4939).featureFlags.experiment.controlledBottomTabs) {
        if (!tmp5) {
          tmp10 = !tmp6;
        }
        flag = tmp10;
  ...
  ref(4964).bottomTabsDebugLog("TabsScreen [" + num + "] render; tabKey: " + tmp7.tabKey + " shouldFreeze: " + flag + ", isFocused: " + undefined !== isFocused && isFocused + " nativeViewIsVisible: " + tmp5);
  let android;
  if (icon != null) {
  ...
  obj7.children = jsx(ref(4937).Freeze, { freeze: flag, placeholder: tmp7.placeholder, children: tmp7.children });
  return jsx(ref1(4971), { collapsable: false, style: null, onWillAppear: callback, onDidAppear: callback1, onWillDisappear: callback2, onDidDisappear: callback3, isFocused: undefined !== isFocused && isFocused });
};
