...
// Function ID: 4520
// Name: registerHost
// Dependencies: [4520, 4515]

// Module 4519 (registerHost)
import ACTIONS from "ACTIONS" /* 4515 */;
import print from "print" /* 4520 */;
...
function reducer(arg0, type) {
  ...
  if (ACTIONS.ACTIONS.REGISTER_HOST === type) {
  ...
  } else if (tmp4(4515).ACTIONS.DEREGISTER_HOST === type) {
    const hostName3 = type.hostName;
    delete tmp[tmp2];
    return obj;
  } else if (tmp4(4515).ACTIONS.ADD_UPDATE_PORTAL === type) {
    ...
    return obj;
  } else if (tmp4(4515).ACTIONS.REMOVE_PORTAL === type) {
    ...
    if (typeof removePortal === "function") {
      if (hostName in obj) {
        ...
        tmp4(4520).print(obj3);
        const tmp4Result = tmp4(4520);
      }
      return obj;
    } else {
      throw new TypeError("Trying to call a non-function");
