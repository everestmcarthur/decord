...
// Function ID: 4486
// Name: registerHost
// Dependencies: [4486, 4481]

// Module 4485 (registerHost)
import ACTIONS from "ACTIONS" /* 4481 */;
import print from "print" /* 4486 */;
...
function reducer(arg0, type) {
  ...
  if (ACTIONS.ACTIONS.REGISTER_HOST === type) {
  ...
  } else if (tmp4(4481).ACTIONS.DEREGISTER_HOST === type) {
    const hostName3 = type.hostName;
    delete tmp[tmp2];
    return obj;
  } else if (tmp4(4481).ACTIONS.ADD_UPDATE_PORTAL === type) {
    ...
    return obj;
  } else if (tmp4(4481).ACTIONS.REMOVE_PORTAL === type) {
    ...
    if (typeof removePortal === "function") {
      if (hostName in obj) {
        ...
        tmp4(4486).print(obj3);
        const tmp4Result = tmp4(4486);
      }
      return obj;
    } else {
      throw new TypeError("Trying to call a non-function");
