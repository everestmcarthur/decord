...
// Function ID: 4455
// Name: registerHost
// Dependencies: [4455, 4450]

// Module 4454 (registerHost)
import ACTIONS from "ACTIONS" /* 4450 */;
import print from "print" /* 4455 */;
...
function reducer(arg0, type) {
  ...
  if (ACTIONS.ACTIONS.REGISTER_HOST === type) {
  ...
  } else if (tmp4(4450).ACTIONS.DEREGISTER_HOST === type) {
    const hostName3 = type.hostName;
    delete tmp[tmp2];
    return obj;
  } else if (tmp4(4450).ACTIONS.ADD_UPDATE_PORTAL === type) {
    ...
    return obj;
  } else if (tmp4(4450).ACTIONS.REMOVE_PORTAL === type) {
    ...
    if (typeof removePortal === "function") {
      if (hostName in obj) {
        ...
        tmp4(4455).print(obj3);
        const tmp4Result = tmp4(4455);
      }
      return obj;
    } else {
      throw new TypeError("Trying to call a non-function");
