...
// Dependencies: [109, 19, 17, 21, 4984, 4981, 4985, 4986, 4964, 4987]
...
// Module 4983 (ScreenStackHeaderSubview)
...
import _modDef4986 from "module_4986" /* 4986 */;
...
const forwardRefResult = noop.forwardRef((hidden, ref) => {
  ...
  const edgeInsetApplication = _mod4984.useEdgeInsetApplication(!hidden.hidden, flag, flag2, flag3, flag4);
  ...
  if (headerLeftBarButtonItems) {
    if (tmp(4981).isHeaderBarButtonsAvailableForCurrentPlatform) {
      result = tmp(4985).prepareHeaderBarButtonItems(headerLeftBarButtonItems, "left");
  ...
  if (headerRightBarButtonItems) {
    if (tmp(4981).isHeaderBarButtonsAvailableForCurrentPlatform) {
      result1 = tmp(4985).prepareHeaderBarButtonItems(headerRightBarButtonItems, "right");
      const tmpResult2 = tmp(4985);
    }
  }
  let isHeaderBarButtonsAvailableForCurrentPlatform = tmp(4981).isHeaderBarButtonsAvailableForCurrentPlatform;
...
export const ScreenStackHeaderSubview = _modDef4987;
...
export const ScreenStackHeaderBackButtonImage = (arg0) => {
  ...
  return jsx(_modDef4987, { type: "back", style: closure_9.headerSubview, synchronousShadowStateUpdatesEnabled: get_synchronousScreenUpdatesEnabledDefault.experiment.synchronousHeaderSubviewUpdatesEnabled, children: null });
};
