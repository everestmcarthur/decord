...
// Dependencies: [109, 19, 17, 21, 4986, 4983, 4987, 4988, 4966, 4989]
...
const forwardRefResult = noop.forwardRef((hidden, ref) => {
  ...
  const edgeInsetApplication = _mod4986.useEdgeInsetApplication(!hidden.hidden, flag, flag2, flag3, flag4);
  ...
  if (headerLeftBarButtonItems) {
    if (tmp(4983).isHeaderBarButtonsAvailableForCurrentPlatform) {
      result = tmp(4987).prepareHeaderBarButtonItems(headerLeftBarButtonItems, "left");
  ...
  if (headerRightBarButtonItems) {
    if (tmp(4983).isHeaderBarButtonsAvailableForCurrentPlatform) {
      result1 = tmp(4987).prepareHeaderBarButtonItems(headerRightBarButtonItems, "right");
      const tmpResult2 = tmp(4987);
    }
  }
  let isHeaderBarButtonsAvailableForCurrentPlatform = tmp(4983).isHeaderBarButtonsAvailableForCurrentPlatform;
...
export const ScreenStackHeaderSubview = _modDef4989;
...
export const ScreenStackHeaderBackButtonImage = (arg0) => {
  ...
  return jsx(_modDef4989, { type: "back", style: closure_9.headerSubview, synchronousShadowStateUpdatesEnabled: get_synchronousScreenUpdatesEnabledDefault.experiment.synchronousHeaderSubviewUpdatesEnabled, children: null });
};
