...
// Function ID: 4521
// Name: registerHost
// Dependencies: [4521, 4516]

// Module 4520 (registerHost)
import ACTIONS from "ACTIONS" /* 4516 */;
import print from "print" /* 4521 */;
...
function reducer(arg0, type) {
  ...
  if (ACTIONS.ACTIONS.REGISTER_HOST === type) {
  ...
  } else if (tmp4(4516).ACTIONS.DEREGISTER_HOST === type) {
    const hostName3 = type.hostName;
    delete tmp[tmp2];
    return obj;
  } else if (tmp4(4516).ACTIONS.ADD_UPDATE_PORTAL === type) {
    ...
    return obj;
  } else if (tmp4(4516).ACTIONS.REMOVE_PORTAL === type) {
    ...
    if (typeof removePortal === "function") {
      if (hostName in obj) {
        ...
        tmp4(4521).print(obj3);
        const tmp4Result = tmp4(4521);
      }
      return obj;
    } else {
      throw new TypeError("Trying to call a non-function");
