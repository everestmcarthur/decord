...
export const browserTracingIntegration = () => {
  ...
  closure_3 = { name: "push", source: "channel" };
  document = _require(893).WINDOW.document;
  obj = {};
      ...
      if (_undefined(_undefined2[2]).WINDOW.location) {
        ...
        if (c25) {
          let result2 = tmp(tmp2[3]).addHistoryInstrumentationHandler((to) => {
            ...
            obj = { name: pathname, attributes: { [tmp3(682).SEMANTIC_ATTRIBUTE_SENTRY_SOURCE]: "url", [tmp3(682).SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN]: "auto.navigation.browser" } };
      ...
      if (c9) {
        ...
        if (tmp(tmp2[2]).WINDOW.document) {
          const listener = globalThis.addEventListener("click", function registerInteractionTransaction() {
            ...
            obj1 = closure_5;
            if (closure_5) {
              const attr = obj1.setAttribute(emit(682).SEMANTIC_ATTRIBUTE_SENTRY_IDLE_SPAN_FINISH_REASON, "interactionInterrupted");
              closure_5.end();
              closure_5 = undefined;
            }
            if (name.name) {
...
export const startBrowserTracingPageLoadSpan = function startBrowserTracingPageLoadSpan(f108179, result) {
  f108179.emit("startPageLoadSpan", result, arg2);
  const currentScope = registerSpanErrorInstrumentation.getCurrentScope();
  currentScope.setTransactionName(result.name);
  if (f108179[_sentry_idleSpan]) {
    f108179.emit("afterStartPageLoadSpan", tmp3);
  }
  return f108179[_sentry_idleSpan];
};
