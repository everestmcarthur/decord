...
export const browserTracingIntegration = () => {
  ...
  closure_3 = { name: "guild", source: "call" };
...
export const startBrowserTracingPageLoadSpan = function startBrowserTracingPageLoadSpan(f108595, tmp2Result, arg2) {
  f108595.emit("startPageLoadSpan", tmp2Result, arg2);
  const currentScope = _mod682.getCurrentScope();
  currentScope.setTransactionName(tmp2Result.name);
  if (f108595[_sentry_idleSpan]) {
    f108595.emit("afterStartPageLoadSpan", tmp3);
  }
  return f108595[_sentry_idleSpan];
};
