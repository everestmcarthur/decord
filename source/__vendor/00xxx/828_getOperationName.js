...
// Dependencies: [823, 829]

// Module 828 (getOperationName)
...
function getOperationName(closure_1_1) {
  if (closure_1_1.includes("chat.completions")) {
    let CONVERSATIONS = _mod823.OPENAI_OPERATIONS.CHAT;
  } else if (closure_1_1.includes("responses")) {
    CONVERSATIONS = _mod823.OPENAI_OPERATIONS.RESPONSES;
  } else if (closure_1_1.includes("embeddings")) {
    CONVERSATIONS = _mod823.OPENAI_OPERATIONS.EMBEDDINGS;
  } else if (closure_1_1.includes("conversations")) {
    CONVERSATIONS = _mod823.OPENAI_OPERATIONS.CONVERSATIONS;
  } else {
...
arg5.addChatCompletionAttributes = function addChatCompletionAttributes(setAttributes, created, recordOutputs) {
  ...
  let obj = {};
  obj[_mod823.OPENAI_RESPONSE_TIMESTAMP_ATTRIBUTE] = new Date(1000 * created.created).toISOString();
...
arg5.addConversationAttributes = function addConversationAttributes(setAttributes, closure_1) {
  ...
  if (created_at) {
    ...
    obj[_mod823.OPENAI_RESPONSE_TIMESTAMP_ATTRIBUTE] = date.toISOString();
    setAttributes.setAttributes(obj);
  }
};
...
arg5.addResponsesApiAttributes = function addResponsesApiAttributes(setAttributes, created_at, recordOutputs) {
  ...
  setAttributes.setAttributes({ [closure_0(closure_1[0]).OPENAI_RESPONSE_MODEL_ATTRIBUTE]: model, [closure_0(closure_1[0]).GEN_AI_RESPONSE_MODEL_ATTRIBUTE]: model });
  let obj = {};
  obj[_mod823.OPENAI_RESPONSE_TIMESTAMP_ATTRIBUTE] = new Date(1000 * created_at.created_at).toISOString();
  setAttributes.setAttributes(obj);
...
arg5.extractRequestParameters = function extractRequestParameters(first) {
  ...
  const obj = {};
  obj[_mod823.GEN_AI_REQUEST_MODEL_ATTRIBUTE] = str;
  if ("temperature" in first) {
    obj[tmp(823).GEN_AI_REQUEST_TEMPERATURE_ATTRIBUTE] = first.temperature;
  }
  if ("top_p" in first) {
    obj[tmp(823).GEN_AI_REQUEST_TOP_P_ATTRIBUTE] = first.top_p;
  }
  if ("frequency_penalty" in first) {
    obj[tmp(823).GEN_AI_REQUEST_FREQUENCY_PENALTY_ATTRIBUTE] = first.frequency_penalty;
  }
  if ("presence_penalty" in first) {
    obj[tmp(823).GEN_AI_REQUEST_PRESENCE_PENALTY_ATTRIBUTE] = first.presence_penalty;
  }
  if ("stream" in first) {
    obj[tmp(823).GEN_AI_REQUEST_STREAM_ATTRIBUTE] = first.stream;
  }
  if ("encoding_format" in first) {
    obj[tmp(823).GEN_AI_REQUEST_ENCODING_FORMAT_ATTRIBUTE] = first.encoding_format;
  }
  if ("dimensions" in first) {
    obj[tmp(823).GEN_AI_REQUEST_DIMENSIONS_ATTRIBUTE] = first.dimensions;
  }
...
arg5.setCommonResponseAttributes = function setCommonResponseAttributes(closure_0, responseId, responseModel, responseTimestamp) {
  ...
  closure_0.setAttributes(obj);
  obj = {};
  obj[_mod823.OPENAI_RESPONSE_TIMESTAMP_ATTRIBUTE] = new Date(1000 * responseTimestamp).toISOString();
  closure_0.setAttributes(obj);
};
arg5.setTokenUsageAttributes = function setTokenUsageAttributes(setAttributes) {
  if (undefined !== arg1) {
    let obj = {};
    obj[_mod823.OPENAI_USAGE_PROMPT_TOKENS_ATTRIBUTE] = arg1;
    obj[_mod823.GEN_AI_USAGE_INPUT_TOKENS_ATTRIBUTE] = arg1;
    setAttributes.setAttributes(obj);
  }
  if (undefined !== arg2) {
    obj = {};
    obj[_mod823.OPENAI_USAGE_COMPLETION_TOKENS_ATTRIBUTE] = arg2;
    obj[_mod823.GEN_AI_USAGE_OUTPUT_TOKENS_ATTRIBUTE] = arg2;
    setAttributes.setAttributes(obj);
  }
  if (undefined !== arg3) {
    obj = {};
    obj[_mod823.GEN_AI_USAGE_TOTAL_TOKENS_ATTRIBUTE] = arg3;
    setAttributes.setAttributes(obj);
  }
};
