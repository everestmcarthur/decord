...
// Function ID: 9177
// Name: pbkdf2Init
// Dependencies: [5, 9174, 9173, 9177]
// Exports: pbkdf2, pbkdf2Async

// Module 9176 (pbkdf2Init)
...
function pbkdf2Init(sha256, B, B, arg3) {
  ...
  if (c < 1) {
    ...
    const toBytesResult = tmp(9173).toBytes(B);
    const uint8Array = new Uint8Array(dkLen);
    const hmac = tmp(9177).hmac;
    const obj = hmac.create(sha256, toBytesResult);
    const toBytesResult1 = tmp(9173).toBytes(B);
