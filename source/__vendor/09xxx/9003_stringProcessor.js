...
export const toJSONSchema = function toJSONSchema(_idmap, uri) {
  ...
  if (tmp) {
    ...
    tmp2(9002).process(_idmap, initializeContextResult);
    tmp2(9002).extractDefs(initializeContextResult, _idmap);
    return tmp2(9002).finalize(initializeContextResult, _idmap);
  }
...
export const objectProcessor = (_zod, io, properties, path) => {
  ...
  for (const key10015 in shape) {
    ...
    obj.path = items;
    arg2.properties[key10015] = _require(9002).process(shape[key10015], arg1, obj);
    continue;
  }
  ...
  if ("never" === type) {
    ...
    if (def.catchall) {
      ...
      obj.path = items2;
      properties.additionalProperties = _require(9002).process(def.catchall, io, obj);
    }
