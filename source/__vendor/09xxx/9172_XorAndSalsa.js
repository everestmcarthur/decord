...
// Function ID: 9173
// Name: XorAndSalsa
// Dependencies: [5, 9173, 9174, 9176, 9178]
// Exports: scrypt, scryptAsync

// Module 9172 (XorAndSalsa)
import _asyncLoop from "_asyncLoop" /* 9173 */;
import number from "number" /* 9174 */;
import pbkdf2Init from "pbkdf2Init" /* 9176 */;
            ...
            if (dkLen >= 0) {
              if (dkLen <= 137438953440) {
                ...
                if (result1 > maxmem) {
                  ...
                  const pbkdf2Result = tmp(9176).pbkdf2(tmp(9178).sha256, B, B, obj2);
                  const _Uint8Array = Uint8Array;
                  const uint8Array = new Uint8Array(result * N);
                  const u32Result = tmp(9173).u32(pbkdf2Result);
                  const _Uint8Array2 = Uint8Array;
                  const uint8Array1 = new Uint8Array(result);
                  function blockMixCb() {

                  }
                  const u32Result1 = tmp(9173).u32(uint8Array);
                  if (onProgress) {
                    const result2 = 2 * N * p;
                    const _Math = Math;
                    const _Math2 = Math;
                  ...
                  const obj = { N, r, p, dkLen, blockSize32: result / 4, V: u32Result1, B32: u32Result, B: pbkdf2Result, tmp: tmp(9173).u32(uint8Array1), blockMixCb, asyncTick };
                  return obj;
                }
              }
            }
...
function scryptOutput(B, dkLen, arr, arr2, arr3) {
  ...
  arr2.fill(0);
  arr3.fill(0);
  return pbkdf2Init.pbkdf2(_mod9178.sha256, B, arr, { c: 1, dkLen });
}
...
export const scrypt = function scrypt(B, B, arg2) {
  ...
  if (!_asyncLoop.isLE) {
    tmp87(9173).byteSwap32(B32);
  }
  B.fill(0);
  V.fill(0);
  tmp2.fill(0);
  return pbkdf2Init.pbkdf2(_mod9178.sha256, B, B, { c: 1, dkLen: tmp.dkLen });
};
