...
// Function ID: 9150
// Name: pbkdf2Init
// Dependencies: [5, 9147, 9146, 9150]
// Exports: pbkdf2, pbkdf2Async

// Module 9149 (pbkdf2Init)
import _asyncLoop from "_asyncLoop" /* 9146 */;
import number from "number" /* 9147 */;
...
function pbkdf2Init(sha256, B, B, arg3) {
  ...
  if (c < 1) {
    ...
    const toBytesResult = tmp(9146).toBytes(B);
    const uint8Array = new Uint8Array(dkLen);
    const hmac = tmp(9150).hmac;
    let obj = hmac.create(sha256, toBytesResult);
    const toBytesResult1 = tmp(9146).toBytes(B);
