...
// Dependencies: [17274, 17275]

// Module 17273 (CanonicalizeLocaleList)
import RegExpResult1 from "RegExpResult1" /* 17274 */;
import createDateTimeFormat from "createDateTimeFormat" /* 17275 */;
          ...
          if (StringResult in ObjectResult) {
            ...
            if (null == tmp4) {
              ...
              if (typeof tmp4 === "string") {
                ...
                let expBCP47Syntax = RegExpResult1.expBCP47Syntax;
                let isMatch = expBCP47Syntax.test(str);
                if (isMatch) {
                  let expVariantDupes = tmp5(17274).expVariantDupes;
                  isMatch = expVariantDupes.test(str);
                  let tmp9 = !isMatch;
                  if (!isMatch) {
                    let expSingletonDupes = tmp5(17274).expSingletonDupes;
                ...
                if (isMatch) {
                  ...
                  let match = str8.match(RegExpResult1.expExtSequences);
                  let tmp19 = str8;
                  let tmp20 = match;
                  if (match) {
                  ...
                  if (!tmp20) {
                    ...
                    let RegExpResult = RegExp(`(?:${tmp17(17274).expExtSequences.source})+`, "i");
...
class List {
  constructor() {
    tmp = f66982(this, "length", { writable: true, value: 0 });
    if (!arguments.length) {
      return;
    } else {
      tmp3 = slice;
...
fn(obj, "__addLocaleData", {
  value(locale) {
    ...
    const expBCP47Syntax = RegExpResult1.expBCP47Syntax;
    let isMatch = expBCP47Syntax.test(locale);
    if (isMatch) {
      const expVariantDupes = tmp(17274).expVariantDupes;
      isMatch = expVariantDupes.test(locale);
      let tmp5 = !isMatch;
      if (!isMatch) {
        const expSingletonDupes = tmp(17274).expSingletonDupes;
