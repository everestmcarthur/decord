...
// Dependencies: [109, 19, 17, 21, 5017, 5014, 5018, 5019, 4997, 5020]
...
// Module 5016 (ScreenStackHeaderSubview)
...
const forwardRefResult = noop.forwardRef((hidden, ref) => {
  ...
  const edgeInsetApplication = _mod5017.useEdgeInsetApplication(!hidden.hidden, flag, flag2, flag3, flag4);
  ...
  if (headerLeftBarButtonItems) {
    if (tmp(5014).isHeaderBarButtonsAvailableForCurrentPlatform) {
      result = tmp(5018).prepareHeaderBarButtonItems(headerLeftBarButtonItems, "left");
      const tmpResult = tmp(5018);
    }
  }
  require = result;
  let result1;
  if (headerRightBarButtonItems) {
    if (tmp(5014).isHeaderBarButtonsAvailableForCurrentPlatform) {
      result1 = tmp(5018).prepareHeaderBarButtonItems(headerRightBarButtonItems, "right");
      const tmpResult2 = tmp(5018);
    }
  }
  let isHeaderBarButtonsAvailableForCurrentPlatform = tmp(5014).isHeaderBarButtonsAvailableForCurrentPlatform;
...
export const ScreenStackHeaderSubview = _modDef5020;
...
export const ScreenStackHeaderBackButtonImage = (arg0) => {
  ...
  return jsx(_modDef5020, { type: "back", style: closure_9.headerSubview, synchronousShadowStateUpdatesEnabled: get_synchronousScreenUpdatesEnabledDefault.experiment.synchronousHeaderSubviewUpdatesEnabled, children: null });
};
