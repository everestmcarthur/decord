...
// Dependencies: [41, 42, 5273, 5276, 5282, 5283, 5284, 5285, 5286]

// Module 5275 (FILE_TYPES_REQUIRED_ADDITIONAL_CHECK)
import AudioTypes from "AudioTypes" /* 5282 */;
import ImageTypes from "ImageTypes" /* 5283 */;
import VideoTypes from "VideoTypes" /* 5284 */;
import CompressedTypes from "CompressedTypes" /* 5285 */;
import OtherTypes from "OtherTypes" /* 5286 */;
...
const entry = {
  ...
  value: function getInfoByName(key10027) {
    return FileTypes(5273).fetchFromObject(FileTypes, key10027.toUpperCase());
  }
};
let items = [
  entry,
  {
    ...
    value: function getSignaturesByName(item10079) {
      return FileTypes(5273).fetchFromObject(FileTypes, item10079.toUpperCase()).signatures;
    }
  },
  {
    key: "detectSignature",
    ...
    value: function detectTypeByAdditionalCheck(fileChunk, found) {
      ...
      if (mapped.some((item) => {
        ...
        if (!mapped.includes("heic")) {
          let str6 = "flv";
          if (!FileTypes(5276).isFLV(fileChunk)) {
            let str8 = "mp4";
            if (tmp8(5276).isM4V(fileChunk)) {
              str8 = "mp4";
              if (!tmp8(5276).isHEIC(fileChunk)) {
                str8 = "m4v";
              }
            }
            str6 = str8;
        ...
        const result = FileTypes(5273).findMatroskaDocTypeElements(fileChunk);
        let str2 = "mkv";
        if ("mkv" !== result) {
          let str4;
          if ("webm" === result) {
            if (tmp3(5276).isWEBM(fileChunk)) {
              str4 = "webm";
            }
          }
          str2 = str4;
        ...
        if (mapped.some((item) => {
          ...
          if (FileTypes(5273).isAvifStringIncluded(fileChunk)) {
            str = "avif";
          }
        }
        return str;
