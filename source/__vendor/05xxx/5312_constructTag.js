...
// Function ID: 5313
// Name: constructTag
// Dependencies: [5268, 5284, 5296, 5265, 5272, 5298]

// Module 5312 (constructTag)
...
function constructTag(decompressResult, type, items2, items1) {
  ...
  if (decompressResult instanceof DataView) {
    stringFromDataView = _mod5265.getStringFromDataView(decompressResult, 0, decompressResult.byteLength);
  }
  const stringValueFromArray = _mod5265.getStringValueFromArray(items1);
  let combined = stringValueFromArray;
  if (type !== PNG_CHUNK_TYPE_SIZE.TYPE_TEXT) {
    combined = stringValueFromArray;
    if (0 !== items2.length) {
      const _HermesInternal = HermesInternal;
      combined = "" + stringValueFromArray + " (" + tmp4(5265).getStringValueFromArray(items2) + ")";
      const tmp4Result = tmp4(5265);
    }
  }
  const obj3 = { name: combined, value: stringFromDataView, description: null };
  if (type === PNG_CHUNK_TYPE_SIZE.TYPE_ITXT) {
    const decoder = _modDef5298;
  ...
  read(byteLength, arg1, arg2, arg3) {
    ...
    if (0 < arg1.length) {
      while (true) {
        ...
        let COMPRESSION_METHOD_NONE = require("module_5265").COMPRESSION_METHOD_NONE;
        ...
        if (0 < length) {
          ...
          if (offset < byteLength.byteLength) {
              ...
              if (tmp !== STATE_COMPRESSION) {
                ...
                if (type === require("PNG_CHUNK_TYPE_SIZE").TYPE_ITXT) {
                  if (byteLength.getUint8(sum) === c9) {
                    ...
                    sum1 = num2;
                    if (type === tmp52(5272).TYPE_ITXT) {
                      sum1 = num2 + c8;
                    }
                    if (tmp !== STATE_KEYWORD) {
                      break;
                    } else {
                      let items5 = [tmp52(5272).TYPE_ITXT, tmp52(5272).TYPE_ZTXT];
                      tmp16 = tmp8;
                ...
                } else if (type === tmp52(5272).TYPE_ZTXT) {
                  COMPRESSION_METHOD_NONE2 = byteLength.getUint8(sum);
                }
                COMPRESSION_METHOD_NONE2 = tmp52(5265).COMPRESSION_METHOD_NONE;
              }
              let sum2 = sum1 + 1;
            ...
            let tmp18 = type === tmp52(5272).TYPE_ITXT ? STATE_LANG : STATE_TEXT;
          }
        }
        let tmp38 = _require;
        if (tmp5 !== require("module_5265").COMPRESSION_METHOD_NONE) {
          if (!arg2) {
            let obj2 = {};
                ...
                try {
                  if (_modDef5268.USE_EXIF) {
                    if (isExifGroupTag(name, value)) {
                      const obj2 = { __exif: tmp2(5284).read(decodeRawData(value), c10, closure_0).tags };
                      return obj2;
                    }
                  }
                  if (_modDef5268.USE_IPTC) {
                    if (isIptcGroupTag(name, value)) {
                      const obj3 = { __iptc: tmp2(5296).read(decodeRawData(value), 0, closure_0) };
                      return obj3;
                    }
                  }
                  if (name) {
        ...
        let tmp38Result = tmp38(5265);
        if (type === tmp38(5272).TYPE_TEXT) {
          let str = "latin1";
        } else {
          str = "utf-8";
        }
