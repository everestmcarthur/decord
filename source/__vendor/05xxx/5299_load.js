...
// Function ID: 5300
// Name: load
// Dependencies: [5300, 5301, 5302, 5303, 5304, 5317, 5319, 5320, 5332, 5336, 5340, 5342, 5344, 5345, 5346, 5347, 5348, 5349, 5350, 5351, 5352, 5353, 5326]
// Exports: load
...
function load(response, response) {
  ...
  if (typeof response === "string") {
    ...
    if (obj2.test(response)) {
      ...
      if (obj3.test(response)) {
        let resolved = Promise.resolve(response(5301).dataUriToBuffer(response));
        const obj5 = response(5301);
      } else {
        let obj7 = then;
        if (then === undefined) {
          obj7 = {};
...
function loadView(byteLength, response) {
  let obj = response;
  if (response === undefined) {
    obj = { expanded: false, async: false, includeUnknown: false, domParser: "w" };
  }
  ...
  function addPngTextTags(readTags) {
    if (flag) {
      ...
      if (closure_1.png) {
        objectAssignResult3 = _mod5301.objectAssign({}, closure_1.png, readTags);
      }
      closure_1.png = objectAssignResult3;
      let objectAssignResult4 = readTags;
      if (closure_1.pngText) {
        objectAssignResult4 = _mod5301.objectAssign({}, closure_1.png, readTags);
      }
  ...
  let obj2 = require("module_5303");
  ({ fileType, fileDataOffset, jfifDataOffset, tiffHeaderOffset, iptcDataOffset, xmpChunks, iccChunks, mpfDataOffset, pngHeaderOffset, pngTextChunks, pngChunkOffsets, vp8xChunkOffset, gifHeaderOffset } = require("module_5303").parseAppMarkers(byteLength, flag2));
  let flag4 = false;
  if (require("module_5304").USE_JPEG) {
    flag4 = false;
        ...
        if (readResult11 instanceof Promise) {
          items1.push(readResult11.then(function addIccTags(icc) {
            if (flag) {
              ...
              closure_1 = _mod5301.objectAssign({}, closure_1, icc);
            }
          }));
          flag9 = true;
        } else if (flag) {
  ...
  const parseAppMarkersResult = require("module_5303").parseAppMarkers(byteLength, flag2);
...
export default { load, loadView, errors: _modDef5300 };
export const errors = _modDef5300;
export { load };
export { loadView };
