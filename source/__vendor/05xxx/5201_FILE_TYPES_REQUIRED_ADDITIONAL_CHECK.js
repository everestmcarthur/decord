...
// Dependencies: [41, 42, 5199, 5202, 5208, 5209, 5210, 5211, 5212]

// Module 5201 (FILE_TYPES_REQUIRED_ADDITIONAL_CHECK)
import AudioTypes from "AudioTypes" /* 5208 */;
import ImageTypes from "ImageTypes" /* 5209 */;
import VideoTypes from "VideoTypes" /* 5210 */;
import CompressedTypes from "CompressedTypes" /* 5211 */;
import OtherTypes from "OtherTypes" /* 5212 */;
...
const entry = {
  ...
  value: function getInfoByName(key10027) {
    return FileTypes(5199).fetchFromObject(FileTypes, key10027.toUpperCase());
  }
};
let items = [
  entry,
  {
    ...
    value: function getSignaturesByName(item10079) {
      return FileTypes(5199).fetchFromObject(FileTypes, item10079.toUpperCase()).signatures;
    }
  },
  {
    key: "detectSignature",
    ...
    value: function detectTypeByAdditionalCheck(fileChunk, found) {
      ...
      if (mapped.some((item) => {
        ...
        if (!mapped.includes("heic")) {
          let str6 = "flv";
          if (!FileTypes(5202).isFLV(fileChunk)) {
            let str8 = "mp4";
            if (tmp8(5202).isM4V(fileChunk)) {
              str8 = "mp4";
              if (!tmp8(5202).isHEIC(fileChunk)) {
                str8 = "m4v";
              }
            }
            str6 = str8;
        ...
        const result = FileTypes(5199).findMatroskaDocTypeElements(fileChunk);
        let str2 = "mkv";
        if ("mkv" !== result) {
          let str4;
          if ("webm" === result) {
            if (tmp3(5202).isWEBM(fileChunk)) {
              str4 = "webm";
            }
          }
          str2 = str4;
        ...
        if (mapped.some((item) => {
          ...
          if (FileTypes(5199).isAvifStringIncluded(fileChunk)) {
            str = "avif";
          }
        }
        return str;
