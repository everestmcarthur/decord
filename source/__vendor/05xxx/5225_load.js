...
// Function ID: 5226
// Name: load
// Dependencies: [5226, 5227, 5228, 5229, 5230, 5243, 5245, 5246, 5258, 5262, 5266, 5268, 5270, 5271, 5272, 5273, 5274, 5275, 5276, 5277, 5278, 5279, 5252]
// Exports: load
...
function load(response, response) {
  ...
  if (typeof response === "string") {
    ...
    if (obj2.test(response)) {
      ...
      if (obj3.test(response)) {
        let resolved = Promise.resolve(response(5227).dataUriToBuffer(response));
        const obj5 = response(5227);
      } else {
        let obj7 = then;
        if (then === undefined) {
          obj7 = {};
...
function loadView(byteLength, response) {
  ...
  function addPngTextTags(readTags) {
    if (flag) {
      ...
      if (closure_1.png) {
        objectAssignResult3 = _mod5227.objectAssign({}, closure_1.png, readTags);
      }
      closure_1.png = objectAssignResult3;
      let objectAssignResult4 = readTags;
      if (closure_1.pngText) {
        objectAssignResult4 = _mod5227.objectAssign({}, closure_1.png, readTags);
      }
  ...
  let obj2 = require("module_5229");
  ({ fileType, fileDataOffset, jfifDataOffset, tiffHeaderOffset, iptcDataOffset, xmpChunks, iccChunks, mpfDataOffset, pngHeaderOffset, pngTextChunks, pngChunkOffsets, vp8xChunkOffset, gifHeaderOffset } = require("module_5229").parseAppMarkers(byteLength, flag2));
  let flag4 = false;
  if (require("module_5230").USE_JPEG) {
    flag4 = false;
        ...
        if (readResult11 instanceof Promise) {
          items1.push(readResult11.then(function addIccTags(icc) {
            if (flag) {
              ...
              closure_1 = _mod5227.objectAssign({}, closure_1, icc);
            }
          }));
          flag9 = true;
        } else if (flag) {
  ...
  const parseAppMarkersResult = require("module_5229").parseAppMarkers(byteLength, flag2);
...
export default { load, loadView, errors: _modDef5226 };
export const errors = _modDef5226;
export { load };
export { loadView };
