...
// Dependencies: [41, 42, 5271, 5274, 5280, 5281, 5282, 5283, 5284]

// Module 5273 (FILE_TYPES_REQUIRED_ADDITIONAL_CHECK)
import AudioTypes from "AudioTypes" /* 5280 */;
import ImageTypes from "ImageTypes" /* 5281 */;
import VideoTypes from "VideoTypes" /* 5282 */;
import CompressedTypes from "CompressedTypes" /* 5283 */;
import OtherTypes from "OtherTypes" /* 5284 */;
...
const entry = {
  ...
  value: function getInfoByName(key10027) {
    return FileTypes(5271).fetchFromObject(FileTypes, key10027.toUpperCase());
  }
};
let items = [
  entry,
  {
    ...
    value: function getSignaturesByName(item10079) {
      return FileTypes(5271).fetchFromObject(FileTypes, item10079.toUpperCase()).signatures;
    }
  },
  {
    key: "detectSignature",
    ...
    value: function detectTypeByAdditionalCheck(fileChunk, found) {
      ...
      if (mapped.some((item) => {
        ...
        if (!mapped.includes("heic")) {
          let str6 = "flv";
          if (!FileTypes(5274).isFLV(fileChunk)) {
            let str8 = "mp4";
            if (tmp8(5274).isM4V(fileChunk)) {
              str8 = "mp4";
              if (!tmp8(5274).isHEIC(fileChunk)) {
                str8 = "m4v";
              }
            }
            str6 = str8;
        ...
        const result = FileTypes(5271).findMatroskaDocTypeElements(fileChunk);
        let str2 = "mkv";
        if ("mkv" !== result) {
          let str4;
          if ("webm" === result) {
            if (tmp3(5274).isWEBM(fileChunk)) {
              str4 = "webm";
            }
          }
          str2 = str4;
        ...
        if (mapped.some((item) => {
          ...
          if (FileTypes(5271).isAvifStringIncluded(fileChunk)) {
            str = "avif";
          }
        }
        return str;
