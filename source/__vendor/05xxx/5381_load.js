...
// Function ID: 5382
// Name: load
// Dependencies: [5382, 5383, 5384, 5385, 5386, 5399, 5401, 5402, 5414, 5418, 5422, 5424, 5426, 5427, 5428, 5429, 5430, 5431, 5432, 5433, 5434, 5435, 5408]
// Exports: load
...
function load(response, response) {
  ...
  if (typeof response === "string") {
    ...
    if (obj2.test(response)) {
      ...
      if (obj3.test(response)) {
        let resolved = Promise.resolve(response(5383).dataUriToBuffer(response));
        const obj5 = response(5383);
      } else {
        let obj7 = then;
        if (then === undefined) {
          obj7 = {};
...
function loadView(byteLength, response) {
  ...
  function addPngTextTags(readTags) {
    if (flag) {
      ...
      if (closure_1.png) {
        objectAssignResult3 = _mod5383.objectAssign({}, closure_1.png, readTags);
      }
      closure_1.png = objectAssignResult3;
      let objectAssignResult4 = readTags;
      if (closure_1.pngText) {
        objectAssignResult4 = _mod5383.objectAssign({}, closure_1.png, readTags);
      }
  ...
  let obj2 = require("module_5385");
  ({ fileType, fileDataOffset, jfifDataOffset, tiffHeaderOffset, iptcDataOffset, xmpChunks, iccChunks, mpfDataOffset, pngHeaderOffset, pngTextChunks, pngChunkOffsets, vp8xChunkOffset, gifHeaderOffset } = require("module_5385").parseAppMarkers(byteLength, flag2));
  let flag4 = false;
  if (require("module_5386").USE_JPEG) {
    flag4 = false;
        ...
        if (readResult11 instanceof Promise) {
          items1.push(readResult11.then(function addIccTags(icc) {
            if (flag) {
              ...
              closure_1 = _mod5383.objectAssign({}, closure_1, icc);
            }
          }));
          flag9 = true;
        } else if (flag) {
  ...
  const parseAppMarkersResult = require("module_5385").parseAppMarkers(byteLength, flag2);
...
export default { load, loadView, errors: _modDef5382 };
export const errors = _modDef5382;
export { load };
export { loadView };
