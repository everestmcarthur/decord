...
// Dependencies: [41, 42, 5235, 5238, 5244, 5245, 5246, 5247, 5248]

// Module 5237 (FILE_TYPES_REQUIRED_ADDITIONAL_CHECK)
import AudioTypes from "AudioTypes" /* 5244 */;
import ImageTypes from "ImageTypes" /* 5245 */;
import VideoTypes from "VideoTypes" /* 5246 */;
import CompressedTypes from "CompressedTypes" /* 5247 */;
import OtherTypes from "OtherTypes" /* 5248 */;
...
const entry = {
  ...
  value: function getInfoByName(key10027) {
    return FileTypes(5235).fetchFromObject(FileTypes, key10027.toUpperCase());
  }
};
let items = [
  entry,
  {
    ...
    value: function getSignaturesByName(item10079) {
      return FileTypes(5235).fetchFromObject(FileTypes, item10079.toUpperCase()).signatures;
    }
  },
  {
    key: "detectSignature",
    ...
    value: function detectTypeByAdditionalCheck(fileChunk, found) {
      ...
      if (mapped.some((item) => {
        ...
        if (!mapped.includes("heic")) {
          let str6 = "flv";
          if (!FileTypes(5238).isFLV(fileChunk)) {
            let str8 = "mp4";
            if (tmp8(5238).isM4V(fileChunk)) {
              str8 = "mp4";
              if (!tmp8(5238).isHEIC(fileChunk)) {
                str8 = "m4v";
              }
            }
            str6 = str8;
        ...
        const result = FileTypes(5235).findMatroskaDocTypeElements(fileChunk);
        let str2 = "mkv";
        if ("mkv" !== result) {
          let str4;
          if ("webm" === result) {
            if (tmp3(5238).isWEBM(fileChunk)) {
              str4 = "webm";
            }
          }
          str2 = str4;
        ...
        if (mapped.some((item) => {
          ...
          if (FileTypes(5235).isAvifStringIncluded(fileChunk)) {
            str = "avif";
          }
        }
        return str;
