...
let obj = {};
let keys1 = Object.keys(_mod5692);
let iter = keys1[Symbol.iterator]();
const nextResult = iter.next();
while (iter !== undefined) {
  let items = [];
  let arraySpreadResult = HermesBuiltin.arraySpread(_mod5692[nextResult].labels, 0);
  class Color {
    constructor(arg0, arg1) {
      self = this;
      str = require;
...
const point = {
  ...
  object() {
    const self = this;
    obj = {};
    const channels = _mod5692[this.model].channels;
    for (let num = 0; num < channels; num = num + 1) {
      obj[_mod5692[this.model].labels[num]] = self.color[num];
    }
    if (1 !== self.valpha) {
  ...
  keyword(color) {
    if (undefined !== color) {
      ...
      const self = this;
      keywordResult = _mod5692[this.model].keyword(this.color);
    }
...
function _loop(item10136) {
  ...
  if (closure_3.includes(item10136)) {
    ...
    channels = require("module_5692")[item10136].channels;
    Color.prototype[item10136] = function() {
      ...
      if (this.model === closure_0) {
        ...
        const rawResult = _mod5692[self.model][tmp].raw(self.color);
        const _Array = Array;
        let tmp2 = rawResult;
