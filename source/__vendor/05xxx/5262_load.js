...
// Function ID: 5263
// Name: load
// Dependencies: [5263, 5264, 5265, 5266, 5267, 5280, 5282, 5283, 5295, 5299, 5303, 5305, 5307, 5308, 5309, 5310, 5311, 5312, 5313, 5314, 5315, 5316, 5289]
// Exports: load
...
function load(response, response) {
  ...
  if (typeof response === "string") {
    ...
    if (obj2.test(response)) {
      ...
      if (obj3.test(response)) {
        let resolved = Promise.resolve(response(5264).dataUriToBuffer(response));
        const obj5 = response(5264);
      } else {
        let obj7 = then;
        if (then === undefined) {
          obj7 = {};
...
function loadView(byteLength, response) {
  ...
  function addPngTextTags(readTags) {
    if (flag) {
      ...
      if (closure_1.png) {
        objectAssignResult3 = _mod5264.objectAssign({}, closure_1.png, readTags);
      }
      closure_1.png = objectAssignResult3;
      let objectAssignResult4 = readTags;
      if (closure_1.pngText) {
        objectAssignResult4 = _mod5264.objectAssign({}, closure_1.png, readTags);
      }
  ...
  let obj2 = require("module_5266");
  ({ fileType, fileDataOffset, jfifDataOffset, tiffHeaderOffset, iptcDataOffset, xmpChunks, iccChunks, mpfDataOffset, pngHeaderOffset, pngTextChunks, pngChunkOffsets, vp8xChunkOffset, gifHeaderOffset } = require("module_5266").parseAppMarkers(byteLength, flag2));
  let flag4 = false;
  if (require("module_5267").USE_JPEG) {
    flag4 = false;
        ...
        if (readResult11 instanceof Promise) {
          items1.push(readResult11.then(function addIccTags(icc) {
            if (flag) {
              ...
              closure_1 = _mod5264.objectAssign({}, closure_1, icc);
            }
          }));
          flag9 = true;
        } else if (flag) {
  ...
  const parseAppMarkersResult = require("module_5266").parseAppMarkers(byteLength, flag2);
...
export default { load, loadView, errors: _modDef5263 };
export const errors = _modDef5263;
export { load };
export { loadView };
