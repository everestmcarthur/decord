...
// Dependencies: [41, 42, 5236, 5239, 5245, 5246, 5247, 5248, 5249]

// Module 5238 (FILE_TYPES_REQUIRED_ADDITIONAL_CHECK)
import AudioTypes from "AudioTypes" /* 5245 */;
import ImageTypes from "ImageTypes" /* 5246 */;
import VideoTypes from "VideoTypes" /* 5247 */;
import CompressedTypes from "CompressedTypes" /* 5248 */;
import OtherTypes from "OtherTypes" /* 5249 */;
...
const entry = {
  ...
  value: function getInfoByName(key10027) {
    return FileTypes(5236).fetchFromObject(FileTypes, key10027.toUpperCase());
  }
};
let items = [
  entry,
  {
    ...
    value: function getSignaturesByName(item10079) {
      return FileTypes(5236).fetchFromObject(FileTypes, item10079.toUpperCase()).signatures;
    }
  },
  {
    key: "detectSignature",
    ...
    value: function detectTypeByAdditionalCheck(fileChunk, found) {
      ...
      if (mapped.some((item) => {
        ...
        if (!mapped.includes("heic")) {
          let str6 = "flv";
          if (!FileTypes(5239).isFLV(fileChunk)) {
            let str8 = "mp4";
            if (tmp8(5239).isM4V(fileChunk)) {
              str8 = "mp4";
              if (!tmp8(5239).isHEIC(fileChunk)) {
                str8 = "m4v";
              }
            }
            str6 = str8;
        ...
        const result = FileTypes(5236).findMatroskaDocTypeElements(fileChunk);
        let str2 = "mkv";
        if ("mkv" !== result) {
          let str4;
          if ("webm" === result) {
            if (tmp3(5239).isWEBM(fileChunk)) {
              str4 = "webm";
            }
          }
          str2 = str4;
        ...
        if (mapped.some((item) => {
          ...
          if (FileTypes(5236).isAvifStringIncluded(fileChunk)) {
            str = "avif";
          }
        }
        return str;
