...
// Dependencies: [41, 42, 5237, 5240, 5246, 5247, 5248, 5249, 5250]

// Module 5239 (FILE_TYPES_REQUIRED_ADDITIONAL_CHECK)
import AudioTypes from "AudioTypes" /* 5246 */;
import ImageTypes from "ImageTypes" /* 5247 */;
import VideoTypes from "VideoTypes" /* 5248 */;
import CompressedTypes from "CompressedTypes" /* 5249 */;
import OtherTypes from "OtherTypes" /* 5250 */;
...
const entry = {
  ...
  value: function getInfoByName(key10027) {
    return FileTypes(5237).fetchFromObject(FileTypes, key10027.toUpperCase());
  }
};
let items = [
  entry,
  {
    ...
    value: function getSignaturesByName(item10079) {
      return FileTypes(5237).fetchFromObject(FileTypes, item10079.toUpperCase()).signatures;
    }
  },
  {
    key: "detectSignature",
    ...
    value: function detectTypeByAdditionalCheck(fileChunk, found) {
      ...
      if (mapped.some((item) => {
        ...
        if (!mapped.includes("heic")) {
          let str6 = "flv";
          if (!FileTypes(5240).isFLV(fileChunk)) {
            let str8 = "mp4";
            if (tmp8(5240).isM4V(fileChunk)) {
              str8 = "mp4";
              if (!tmp8(5240).isHEIC(fileChunk)) {
                str8 = "m4v";
              }
            }
            str6 = str8;
        ...
        const result = FileTypes(5237).findMatroskaDocTypeElements(fileChunk);
        let str2 = "mkv";
        if ("mkv" !== result) {
          let str4;
          if ("webm" === result) {
            if (tmp3(5240).isWEBM(fileChunk)) {
              str4 = "webm";
            }
          }
          str2 = str4;
        ...
        if (mapped.some((item) => {
          ...
          if (FileTypes(5237).isAvifStringIncluded(fileChunk)) {
            str = "avif";
          }
        }
        return str;
