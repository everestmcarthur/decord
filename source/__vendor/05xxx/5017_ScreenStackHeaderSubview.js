...
// Dependencies: [109, 19, 17, 21, 5018, 5015, 5019, 5020, 4998, 5021]
...
// Module 5017 (ScreenStackHeaderSubview)
...
const forwardRefResult = noop.forwardRef((hidden, ref) => {
  ...
  const edgeInsetApplication = _mod5018.useEdgeInsetApplication(!hidden.hidden, flag, flag2, flag3, flag4);
  ...
  if (headerLeftBarButtonItems) {
    if (tmp(5015).isHeaderBarButtonsAvailableForCurrentPlatform) {
      result = tmp(5019).prepareHeaderBarButtonItems(headerLeftBarButtonItems, "left");
  ...
  if (headerRightBarButtonItems) {
    if (tmp(5015).isHeaderBarButtonsAvailableForCurrentPlatform) {
      result1 = tmp(5019).prepareHeaderBarButtonItems(headerRightBarButtonItems, "right");
      const tmpResult2 = tmp(5019);
    }
  }
  let isHeaderBarButtonsAvailableForCurrentPlatform = tmp(5015).isHeaderBarButtonsAvailableForCurrentPlatform;
...
export const ScreenStackHeaderSubview = _modDef5021;
...
export const ScreenStackHeaderBackButtonImage = (arg0) => {
  ...
  return jsx(_modDef5021, { type: "back", style: closure_9.headerSubview, synchronousShadowStateUpdatesEnabled: get_synchronousScreenUpdatesEnabledDefault.experiment.synchronousHeaderSubviewUpdatesEnabled, children: null });
};
