...
// Dependencies: [5315, 5319, 5301]
// Exports: get0thIfdOffset

// Module 5318 (get0thIfdOffset)
import _modDef5301 from "module_5301" /* 5301 */;
import _modDef5315 from "module_5315" /* 5315 */;
import IFD_TYPE_0TH2 from "IFD_TYPE_0TH" /* 5319 */;

require = arg1;
function readTag(byteLength, IFD_TYPE_0TH, sum, sum, byteOrder, arg5) {
  const typeSize = _modDef5315.getTypeSize("SHORT");
  sum = typeSize + _modDef5315.getTypeSize("SHORT");
  const sum1 = sum + _modDef5315.getTypeSize("LONG");
  const shortAt = _modDef5315.getShortAt(byteLength, sum, byteOrder);
  const shortAt1 = _modDef5315.getShortAt(byteLength, sum + typeSize, byteOrder);
  const longAt = _modDef5315.getLongAt(byteLength, sum + sum, byteOrder);
  if (undefined !== _modDef5315.typeSizes[shortAt1]) {
    if ((function tagValueFitsInOffsetSlot(shortAt1, longAt) {
      const result = _modDef5315.typeSizes[shortAt1] * longAt;
      return result <= _modDef5315.getTypeSize("LONG");
    })(shortAt1, longAt)) {
      const sum2 = sum + sum1;
      let str = getTagValue(byteLength, sum2, shortAt1, longAt, byteOrder);
      let tmp21 = sum2;
    } else {
      const longAt1 = tmp4(5315).getLongAt(byteLength, sum + sum1, byteOrder);
      str = "<faulty value>";
      tmp21 = longAt1;
      if ((function tagValueFitsInDataView(byteLength, sum, longAt1, shortAt1, longAt) {
        sum = sum + longAt1;
        return sum + _modDef5315.typeSizes[shortAt1] * longAt <= byteLength.byteLength;
      })(byteLength, sum, longAt1, shortAt1, longAt)) {
        str = getTagValue(byteLength, sum + longAt1, shortAt1, longAt, byteOrder, 33723 === shortAt);
        tmp21 = longAt1;
      }
      const tmp4Result = tmp4(5315);
    }
    let tmp34 = str;
    if (shortAt1 === tmp4(5315).tagTypes.ASCII) {
    ...
    if (undefined !== tmp4(5319)[IFD_TYPE_0TH][shortAt]) {
      if (undefined !== tmp4(5319)[IFD_TYPE_0TH][shortAt].name) {
        if (undefined !== tmp4(5319)[IFD_TYPE_0TH][shortAt].description) {
          try {
            descriptionResult = tmp4(5319)[IFD_TYPE_0TH][shortAt].description(tmp34);
            combined = tmp39;
            const obj8 = tmp4(5319)[IFD_TYPE_0TH][shortAt];
          } catch (err) {
      ...
      if (shortAt1 !== tmp4(5315).tagTypes.RATIONAL) {
        if (shortAt1 !== tmp4(5315).tagTypes.SRATIONAL) {
          combined = tmp4(5319)[IFD_TYPE_0TH][shortAt];
...
function getTagValue(byteLength, sum2, shortAt1, longAt, byteOrder, arg5) {
  ...
  let BYTE = shortAt1;
  if (flag) {
    result = longAt * _modDef5315.typeSizes[shortAt1];
    BYTE = _modDef5315.tagTypes.BYTE;
  }
  let sum = sum2;
  const items = [];
  for (let num = 0; num < result; num = num + 1) {
    let arr = items.push(obj[BYTE](byteLength, sum, byteOrder));
    sum = sum + _modDef5315.typeSizes[BYTE];
  }
  if (BYTE === _modDef5315.tagTypes.ASCII) {
    let asciiValue = _modDef5315.getAsciiValue(items);
    const tmp9Result = _modDef5315;
  } else {
    asciiValue = items;
    if (1 === items.length) {
      asciiValue = items[0];
...
let obj = { 1: null, 2: _modDef5315.getByteAt, 3: _modDef5315.getAsciiAt, 4: _modDef5315.getShortAt, 5: _modDef5315.getLongAt, 7: _modDef5315.getRationalAt, 9: null, 10: _modDef5315.getUndefinedAt, 13: null };
obj[9] = _modDef5315.getSlongAt;
obj[10] = _modDef5315.getSrationalAt;
obj[13] = _modDef5315.getIfdPointerAt;
function readIfd(byteLength, IFD_TYPE_0TH, sum, sum, byteOrder, arg5) {
  const typeSize = _modDef5315.getTypeSize("SHORT");
  let num = 0;
  if (sum + obj2.getTypeSize("SHORT") <= byteLength.byteLength) {
    num = _modDef5315.getShortAt(byteLength, sum, byteOrder);
  ...
  if (_modDef5301.USE_THUMBNAIL) {
    if (tmp5 < byteLength.byteLength - tmp23Result.getTypeSize("LONG")) {
      const longAt = tmp23(5315).getLongAt(byteLength, tmp5, byteOrder);
      let tmp26 = 0 !== longAt;
      if (tmp26) {
      ...
      const tmp23Result2 = tmp23(5315);
    }
    tmp23Result = tmp23(5315);
  }
  return obj3;
}

export const get0thIfdOffset = function get0thIfdOffset(buffer, c5, byteOrder) {
  return c5 + _modDef5315.getLongAt(buffer, c5 + 4, byteOrder);
};
export { readIfd };
