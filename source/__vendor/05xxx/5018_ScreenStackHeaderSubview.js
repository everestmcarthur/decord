...
// Dependencies: [109, 19, 17, 21, 5019, 5016, 5020, 5021, 4999, 5022]
...
// Module 5018 (ScreenStackHeaderSubview)
...
const forwardRefResult = noop.forwardRef((hidden, ref) => {
  ...
  const edgeInsetApplication = _mod5019.useEdgeInsetApplication(!hidden.hidden, flag, flag2, flag3, flag4);
  ...
  if (headerLeftBarButtonItems) {
    if (tmp(5016).isHeaderBarButtonsAvailableForCurrentPlatform) {
      result = tmp(5020).prepareHeaderBarButtonItems(headerLeftBarButtonItems, "left");
  ...
  if (headerRightBarButtonItems) {
    if (tmp(5016).isHeaderBarButtonsAvailableForCurrentPlatform) {
      result1 = tmp(5020).prepareHeaderBarButtonItems(headerRightBarButtonItems, "right");
      const tmpResult2 = tmp(5020);
    }
  }
  let isHeaderBarButtonsAvailableForCurrentPlatform = tmp(5016).isHeaderBarButtonsAvailableForCurrentPlatform;
...
export const ScreenStackHeaderSubview = _modDef5022;
...
export const ScreenStackHeaderBackButtonImage = (arg0) => {
  ...
  return jsx(_modDef5022, { type: "back", style: closure_9.headerSubview, synchronousShadowStateUpdatesEnabled: get_synchronousScreenUpdatesEnabledDefault.experiment.synchronousHeaderSubviewUpdatesEnabled, children: null });
};
