...
// Function ID: 14338
// Name: ResolveLocale
// Dependencies: [14338, 14343, 14344, 14339, 14345, 14346]
// Exports: ResolveLocale

// Module 14337 (ResolveLocale)
import LookupMatcher from "LookupMatcher" /* 14338 */;
import _mod14339 from "module_14339" /* 14339 */;
import BestFitMatcher from "BestFitMatcher" /* 14343 */;
...
export const ResolveLocale = function ResolveLocale(arg0, arg1, localeMatcher, arg3, arg4, fn) {
  ...
  if (LookupMatcherResult.extension) {
    let keywords = tmp7(14344).UnicodeExtensionComponents(LookupMatcherResult.extension).keywords;
  } else {
    keywords = [];
  }
  const items = [];
  ...
  if (0 < arg3.length) {
    do {
      ...
      let invariantResult = _mod14339.invariant(isArray, "keyLocaleData for ".concat(str, " must be an array"));
      let first = items1[0];
      let tmp19 = undefined === first;
      if (!tmp19) {
        tmp19 = typeof first === "string";
      }
      let invariantResult1 = _mod14339.invariant(tmp19, "value must be a string or undefined");
      ...
      let invariantResult2 = tmp14(14339).invariant(tmp24, "optionsValue must be a string or undefined");
      let str3 = tmp23;
      if (typeof tmp23 === "string") {
        let formatted = str.toLowerCase();
        str3 = tmp14(14345).CanonicalizeUValue(formatted, tmp23);
        if ("" === str3) {
          str3 = "true";
        }
      }
  ...
  let result = locale;
  if (items.length > 0) {
    result = tmp5(14346).InsertUnicodeExtensionAndCanonicalize(locale, [], items);
  }
  obj2.locale = result;
  return obj2;
};
