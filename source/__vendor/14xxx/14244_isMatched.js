...
// Function ID: 14245
// Name: isMatched
// Dependencies: [14245, 1162, 14246]

// Module 14244 (isMatched)
  ...
  if (arr) {
    if ("$" === arr[0]) {
      ...
      const mapped = arr2.map((arg0) => {
        let tmp = callback(14246).regions[arg0];
        if (!tmp) {
          const items = [arg0];
          tmp = items;
        }
      ...
      const reduced = mapped.reduce((arg0, arg1) => {
        const obj = callback(1162);
        return obj.__spreadArray(callback(1162).__spreadArray([], arg0, true), arg1, true);
      }, []);
...
function findMatchingDistance(arg0, arg1) {
  ...
  let tmp7 = obj1;
  if (!obj1) {
    let first = _mod14245.data.supplemental.languageMatching["written-new"][0];
    let paradigmLocales;
    if (null !== first) {
      if (undefined !== first) {
    ...
    const prop = tmp8(14245).data.supplemental.languageMatching["written-new"];
    let substr = prop.slice(1, 5);
    const prop1 = tmp8(14245).data.supplemental.languageMatching["written-new"];
    ...
    let tmp8Result = tmp8(1162);
    tmp8Result = tmp8(1162);
