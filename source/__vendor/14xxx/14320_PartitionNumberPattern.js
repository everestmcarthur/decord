...
// Dependencies: [1162, 14293, 14315, 14294, 14308, 14310]
// Exports: PartitionNumberPattern

// Module 14320 (PartitionNumberPattern)
import digitsToString2 from "digitsToString" /* 14293 */;
...
export const PartitionNumberPattern = function PartitionNumberPattern(internalSlots, isNaN) {
  ...
  if (isNaN.isNaN()) {
    ...
    if (!isNaN.isZero()) {
      _mod14294.invariant(isNaN.isFinite(), "Input must be a mathematical value");
  ...
  if ("never" !== signDisplay) {
    if ("auto" === signDisplay) {
      ...
      _mod14294.invariant("negative" === signDisplay, "signDisplay must be \"negative\"");
      let num8 = 0;
      if (roundedNumber.isNegative()) {
        num8 = 0;
        if (!roundedNumber.isZero()) {
  ...
  return module_14315.default({ roundedNumber, formattedString, exponent: num, magnitude: num2, sign: num7 }, internalSlots.dataLocaleData, pl, internalSlots);
};
