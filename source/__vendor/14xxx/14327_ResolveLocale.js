...
// Function ID: 14328
// Name: ResolveLocale
// Dependencies: [14328, 14333, 14334, 14329, 14335, 14336]
// Exports: ResolveLocale

// Module 14327 (ResolveLocale)
import LookupMatcher from "LookupMatcher" /* 14328 */;
import _mod14329 from "module_14329" /* 14329 */;
import BestFitMatcher from "BestFitMatcher" /* 14333 */;
...
export const ResolveLocale = function ResolveLocale(arg0, arg1, localeMatcher, arg3, arg4, fn) {
  ...
  if (LookupMatcherResult.extension) {
    let keywords = tmp7(14334).UnicodeExtensionComponents(LookupMatcherResult.extension).keywords;
  } else {
    keywords = [];
  }
  const items = [];
  ...
  if (0 < arg3.length) {
    do {
      ...
      let invariantResult = _mod14329.invariant(isArray, "keyLocaleData for ".concat(str, " must be an array"));
      let first = items1[0];
      let tmp19 = undefined === first;
      if (!tmp19) {
        tmp19 = typeof first === "string";
      }
      let invariantResult1 = _mod14329.invariant(tmp19, "value must be a string or undefined");
      ...
      let invariantResult2 = tmp14(14329).invariant(tmp24, "optionsValue must be a string or undefined");
      let str3 = tmp23;
      if (typeof tmp23 === "string") {
        let formatted = str.toLowerCase();
        str3 = tmp14(14335).CanonicalizeUValue(formatted, tmp23);
        if ("" === str3) {
          str3 = "true";
        }
      }
  ...
  let result = locale;
  if (items.length > 0) {
    result = tmp5(14336).InsertUnicodeExtensionAndCanonicalize(locale, [], items);
  }
  obj2.locale = result;
  return obj2;
};
