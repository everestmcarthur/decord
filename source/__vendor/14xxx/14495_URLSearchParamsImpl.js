...
// Dependencies: [41, 42, 14493]

// Module 14495 (URLSearchParamsImpl)
...
const entry = {
  ...
  value: function _updateSteps() {
    const self = this;
    if (null !== this._url) {
      let serializeUrlencodedResult = URLSearchParamsImpl(14493).serializeUrlencoded(self._list);
      if ("" === serializeUrlencodedResult) {
        serializeUrlencodedResult = null;
      }
      self._url._url.query = serializeUrlencodedResult;
      const obj = URLSearchParamsImpl(14493);
    }
  }
};
let items = [
...
items[9] = {
  ...
  value: function toString() {
    return URLSearchParamsImpl(14493).serializeUrlencoded(this._list);
  }
};
