...
export const ToRawPrecision = function ToRawPrecision(ZERO, minimumSignificantDigits, maximumSignificantDigits, result) {
  ...
  let tmp4;
  if (ZERO.isZero()) {
    let repeatResult = _mod14234.repeat("0", maximumSignificantDigits);
  ...
  if (roundingMagnitude >= maximumSignificantDigits - 1) {
    let sum = repeatResult + tmp16(14234).repeat("0", roundingMagnitude - maximumSignificantDigits + 1);
    ...
    tmp16(14234).invariant(roundingMagnitude < 0, "e should be less than 0");
    sum = `0.${tmp16(14234).repeat("0", -num3 - 1)}${arr}`;
