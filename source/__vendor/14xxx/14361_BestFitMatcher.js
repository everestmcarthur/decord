...
export const BestFitMatcher = function BestFitMatcher(arg0, arr, fn) {
  ...
  const reduced = arr.reduce((acc, item) => {
    const replaced = item.replace(_mod14357.UNICODE_EXTENSION_SEQUENCE_REGEX, "");
    items.push(replaced);
    acc[replaced] = item;
    return acc;
  }, {});
  const findBestMatchResult = items(14357).findBestMatch(items, arg0);
  let prop;
  if (tmp3) {
