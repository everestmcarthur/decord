...
// Dependencies: [14239, 14234, 14244, 14245]
// Exports: SetNumberFormatUnitOptions

// Module 14280 (SetNumberFormatUnitOptions)
...
export const SetNumberFormatUnitOptions = function SetNumberFormatUnitOptions(internalSlots, result1) {
  ...
  if (!result) {
    result = tmp4(14244).IsWellFormedCurrencyCode(str);
  }
  _mod14234.invariant(result, "Malformed currency code", RangeError);
  let tmp10 = "currency" !== GetOptionResult;
  if (!tmp10) {
    tmp10 = undefined !== str;
  }
  _mod14234.invariant(tmp10, "currency cannot be undefined", TypeError);
  ...
  if (!result1) {
    result1 = tmp4(14245).IsWellFormedUnitIdentifier(GetOptionResult3);
  }
  _mod14234.invariant(result1, "Invalid unit argument for Intl.NumberFormat()", RangeError);
  let tmp17 = "unit" !== GetOptionResult;
  if (!tmp17) {
    tmp17 = undefined !== GetOptionResult3;
  }
  _mod14234.invariant(tmp17, "unit cannot be undefined", TypeError);
