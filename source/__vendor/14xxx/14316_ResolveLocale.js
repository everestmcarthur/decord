...
// Function ID: 14317
// Name: ResolveLocale
// Dependencies: [14317, 14322, 14323, 14318, 14324, 14325]
// Exports: ResolveLocale

// Module 14316 (ResolveLocale)
import LookupMatcher from "LookupMatcher" /* 14317 */;
import _mod14318 from "module_14318" /* 14318 */;
import BestFitMatcher from "BestFitMatcher" /* 14322 */;
...
export const ResolveLocale = function ResolveLocale(arg0, arg1, localeMatcher, arg3, arg4, fn) {
  ...
  if (LookupMatcherResult.extension) {
    let keywords = tmp7(14323).UnicodeExtensionComponents(LookupMatcherResult.extension).keywords;
  } else {
    keywords = [];
  }
  const items = [];
  ...
  if (0 < arg3.length) {
    do {
      ...
      let invariantResult = _mod14318.invariant(isArray, "keyLocaleData for ".concat(str, " must be an array"));
      let first = items1[0];
      let tmp19 = undefined === first;
      if (!tmp19) {
        tmp19 = typeof first === "string";
      }
      let invariantResult1 = _mod14318.invariant(tmp19, "value must be a string or undefined");
      ...
      let invariantResult2 = tmp14(14318).invariant(tmp24, "optionsValue must be a string or undefined");
      let str3 = tmp23;
      if (typeof tmp23 === "string") {
        let formatted = str.toLowerCase();
        str3 = tmp14(14324).CanonicalizeUValue(formatted, tmp23);
        if ("" === str3) {
          str3 = "true";
        }
      }
  ...
  let result = locale;
  if (items.length > 0) {
    result = tmp5(14325).InsertUnicodeExtensionAndCanonicalize(locale, [], items);
  }
  obj2.locale = result;
  return obj2;
};
