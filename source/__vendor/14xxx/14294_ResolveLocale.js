...
// Function ID: 14295
// Name: ResolveLocale
// Dependencies: [14295, 14300, 14301, 14296, 14302, 14303]
// Exports: ResolveLocale

// Module 14294 (ResolveLocale)
import LookupMatcher from "LookupMatcher" /* 14295 */;
import _mod14296 from "module_14296" /* 14296 */;
import BestFitMatcher from "BestFitMatcher" /* 14300 */;
...
export const ResolveLocale = function ResolveLocale(arg0, arg1, localeMatcher, arg3, arg4, fn) {
  ...
  if (LookupMatcherResult.extension) {
    let keywords = tmp7(14301).UnicodeExtensionComponents(LookupMatcherResult.extension).keywords;
  } else {
    keywords = [];
  }
  const items = [];
  ...
  if (0 < arg3.length) {
    do {
      ...
      let invariantResult = _mod14296.invariant(isArray, "keyLocaleData for ".concat(str, " must be an array"));
      let first = items1[0];
      let tmp19 = undefined === first;
      if (!tmp19) {
        tmp19 = typeof first === "string";
      }
      let invariantResult1 = _mod14296.invariant(tmp19, "value must be a string or undefined");
      ...
      let invariantResult2 = tmp14(14296).invariant(tmp24, "optionsValue must be a string or undefined");
      let str3 = tmp23;
      if (typeof tmp23 === "string") {
        let formatted = str.toLowerCase();
        str3 = tmp14(14302).CanonicalizeUValue(formatted, tmp23);
        if ("" === str3) {
          str3 = "true";
        }
      }
  ...
  let result = locale;
  if (items.length > 0) {
    result = tmp5(14303).InsertUnicodeExtensionAndCanonicalize(locale, [], items);
  }
  obj2.locale = result;
  return obj2;
};
