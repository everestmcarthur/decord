...
// Dependencies: [14259, 14285, 14284, 14283, 14272]
...
export const PartitionNumberRangePattern = function PartitionNumberRangePattern(arg0, isNaN, isNaN2, getInternalSlots) {
  ...
  const result = tmp(14285).PartitionNumberPattern(internalSlots, isNaN);
  const result1 = tmp(14285).PartitionNumberPattern(internalSlots, isNaN2);
  if (FormatNumericResult === FormatNumeric.FormatNumeric(internalSlots, isNaN2)) {
    const FormatApproximatelyResult = tmp(14283).FormatApproximately(internalSlots, result);
    ...
    return tmp(14272).CollapseNumberRange(arg0, items, obj2);
  }
