...
// Dependencies: [14330, 14356, 14355, 14354, 14343]
// Exports: PartitionNumberRangePattern

// Module 14358 (PartitionNumberRangePattern)
...
export const PartitionNumberRangePattern = function PartitionNumberRangePattern(arg0, isNaN, isNaN2, getInternalSlots) {
  ...
  _mod14330.invariant(tmp4, "Input must be a number", RangeError);
  const internalSlots = getInternalSlots(arg0);
  const result = tmp(14356).PartitionNumberPattern(internalSlots, isNaN);
  const result1 = tmp(14356).PartitionNumberPattern(internalSlots, isNaN2);
  if (FormatNumericResult === FormatNumeric.FormatNumeric(internalSlots, isNaN2)) {
    const FormatApproximatelyResult = tmp(14354).FormatApproximately(internalSlots, result);
    ...
    return tmp(14343).CollapseNumberRange(arg0, items, obj2);
  }
