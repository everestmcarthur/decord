...
// Dependencies: [1161, 14424, 14446, 14425, 14439, 14441]
// Exports: PartitionNumberPattern

// Module 14451 (PartitionNumberPattern)
import digitsToString2 from "digitsToString" /* 14424 */;
...
let e = e_mod;
const module_14446 = e.__importDefault(_mod14446);
...
export const PartitionNumberPattern = function PartitionNumberPattern(internalSlots, isNaN) {
  ...
  if (isNaN.isNaN()) {
    ...
    if (!isNaN.isZero()) {
      _mod14425.invariant(isNaN.isFinite(), "Input must be a mathematical value");
  ...
  if ("never" !== signDisplay) {
    if ("auto" === signDisplay) {
      ...
      _mod14425.invariant("negative" === signDisplay, "signDisplay must be \"negative\"");
      let num8 = 0;
      if (roundedNumber.isNegative()) {
        num8 = 0;
        if (!roundedNumber.isZero()) {
  ...
  return module_14446.default({ roundedNumber, formattedString, exponent: num, magnitude: num2, sign: num7 }, internalSlots.dataLocaleData, pl, internalSlots);
};
