...
// Dependencies: [14335, 14330, 14340, 14341]
// Exports: SetNumberFormatUnitOptions

// Module 14376 (SetNumberFormatUnitOptions)
...
export const SetNumberFormatUnitOptions = function SetNumberFormatUnitOptions(internalSlots, result1) {
  ...
  if (!result) {
    result = tmp4(14340).IsWellFormedCurrencyCode(str);
  }
  _mod14330.invariant(result, "Malformed currency code", RangeError);
  let tmp10 = "currency" !== GetOptionResult;
  if (!tmp10) {
    tmp10 = undefined !== str;
  }
  _mod14330.invariant(tmp10, "currency cannot be undefined", TypeError);
  ...
  if (!result1) {
    result1 = tmp4(14341).IsWellFormedUnitIdentifier(GetOptionResult3);
  }
  _mod14330.invariant(result1, "Invalid unit argument for Intl.NumberFormat()", RangeError);
  let tmp17 = "unit" !== GetOptionResult;
  if (!tmp17) {
    tmp17 = undefined !== GetOptionResult3;
  }
  _mod14330.invariant(tmp17, "unit cannot be undefined", TypeError);
