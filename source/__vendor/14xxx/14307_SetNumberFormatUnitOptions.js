...
// Dependencies: [14266, 14261, 14271, 14272]
// Exports: SetNumberFormatUnitOptions

// Module 14307 (SetNumberFormatUnitOptions)
...
export const SetNumberFormatUnitOptions = function SetNumberFormatUnitOptions(internalSlots, result1) {
  ...
  if (!result) {
    result = tmp4(14271).IsWellFormedCurrencyCode(str);
  }
  _mod14261.invariant(result, "Malformed currency code", RangeError);
  let tmp10 = "currency" !== GetOptionResult;
  if (!tmp10) {
    tmp10 = undefined !== str;
  }
  _mod14261.invariant(tmp10, "currency cannot be undefined", TypeError);
  ...
  if (!result1) {
    result1 = tmp4(14272).IsWellFormedUnitIdentifier(GetOptionResult3);
  }
  _mod14261.invariant(result1, "Invalid unit argument for Intl.NumberFormat()", RangeError);
  let tmp17 = "unit" !== GetOptionResult;
  if (!tmp17) {
    tmp17 = undefined !== GetOptionResult3;
  }
  _mod14261.invariant(tmp17, "unit cannot be undefined", TypeError);
