...
// Dependencies: [14264, 14261, 14266, 14265]
// Exports: SetNumberFormatDigitOptions

// Module 14308 (SetNumberFormatDigitOptions)
...
export const SetNumberFormatDigitOptions = function SetNumberFormatDigitOptions(internalSlots, result1, minimumFractionDigits, arg3, GetOptionResult1) {
  ...
  _mod14261.invariant(hasItem, combined.concat(Array.from(set).join(", "), "."));
  ...
  if (flag2) {
    if (tmp11) {
      internalSlots.minimumSignificantDigits = tmp2(14265).DefaultNumberOption(minimumSignificantDigits, 1, 21, 1);
      internalSlots.maximumSignificantDigits = tmp2(14265).DefaultNumberOption(maximumSignificantDigits, internalSlots.minimumSignificantDigits, 21, 21);
    } else {
  ...
  if (flag) {
    if (tmp12) {
      const DefaultNumberOptionResult = tmp2(14265).DefaultNumberOption(minimumFractionDigits, 0, 100, undefined);
      const DefaultNumberOptionResult1 = tmp2(14265).DefaultNumberOption(maximumFractionDigits, 0, 100, undefined);
      if (undefined === DefaultNumberOptionResult) {
        tmp2(14261).invariant(undefined !== DefaultNumberOptionResult1, "maximumFractionDigits must be defined");
        const _Math2 = Math;
  ...
  if (!flag2) {
    ...
    if (tmp10) {
      ...
      tmp2(14261).invariant("fractionDigits" === internalSlots.roundingType, "Invalid roundingType", TypeError);
      const _RangeError = RangeError;
      tmp2(14261).invariant(internalSlots.maximumFractionDigits === internalSlots.minimumFractionDigits, "With roundingIncrement > 1, maximumFractionDigits and minimumFractionDigits must be equal.", RangeError);
    }
  }
