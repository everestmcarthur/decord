...
// Dependencies: [12983, 12934, 12972, 12933, 12936, 12955, 12927]

// Module 13017 (extraErrorDataIntegration)
        ...
        if (obj2.isError(obj.originalException)) {
          const tmp7 = (function _extractErrorData(originalException, arg1, maxValueLength) {
            try {
              ...
              if (num(12955).DEBUG_BUILD) {
                const logger = num(12927).logger;
          ...
          if (tmp7) {
            ...
            const normalizer = tmp4(12972);
            const normalizeResult = normalizer.normalize(tmp7, num);
            if (tmp4Result.isPlainObject(normalizeResult)) {
              const result = tmp4(12933).addNonEnumerableProperty(normalizeResult, "__sentry_skip_normalization__", true);
              obj3[tmp6] = normalizeResult;
              const tmp4Result2 = tmp4(12933);
            }
            let obj4 = {};
            const merged1 = Object.assign(contexts);
            obj4.contexts = obj3;
            tmp3 = obj4;
            tmp4Result = tmp4(12934);
          }
