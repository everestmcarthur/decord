...
// Dependencies: [41, 42, 13045, 12962, 13049, 13050, 12953, 13051]

// Module 13052 (BrowserMetricsAggregator)
...
const entry = {
  ...
  value: function add(metricType, arg1, diff, none, tags) {
    ...
    const bucketKey = BrowserMetricsAggregator(13049).getBucketKey(metricType, sanitizeMetricKeyResult, sanitizeUnitResult, sanitizeTagsResult);
    ...
    if (value) {
      ...
      const tmp16 = new tmp5(13050).METRIC_MAP[metricType](diff);
      obj7.metric = tmp16;
    ...
    const result1 = BrowserMetricsAggregator(12953).updateMetricSummaryOnActiveSpan(metricType, sanitizeMetricKeyResult, diff, sanitizeUnitResult, obj, bucketKey);
  }
};
const items = [
  entry,
