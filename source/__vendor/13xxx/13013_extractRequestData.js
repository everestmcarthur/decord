...
// Dependencies: [13014, 13015, 13016, 12939, 12977, 12941, 12931, 12932, 12938]
...
// Module 13013 (extractRequestData)
...
function extractRequestData(headers, arg1) {
  ...
  let item = include.forEach((item) => {
    if ("headers" === item) {
      ...
      if (!obj5.includes("ip")) {
        const ipHeaderNames = _mod13015.ipHeaderNames;
      ...
      if (!cookies) {
        ...
        if (cookie) {
          cookie = _mod13016.parseCookie(tmp21.cookie);
        }
        cookies = cookie;
      }
      if (!cookies) {
...
function headersToDict(arg0) {
  ...
  try {
    ...
    if (_mod12931.DEBUG_BUILD) {
      const logger = tmp4(12932).logger;
      logger.warn("Sentry failed extracting headers from a request object. If you see this, please file an issue.");
    }
    tmp4 = require;
  }
...
export const addNormalizedRequestDataToEvent = function addNormalizedRequestDataToEvent(request, normalizedRequest, user, include) {
  ...
  if (obj.request) {
    ...
    if (obj2.includes("cookies")) {
      ...
      if (!cookies) {
        ...
        if (obj4.cookie) {
          parseCookieResult = _mod13016.parseCookie(obj4.cookie);
        }
  ...
  if (obj.ip) {
    ...
    if (ipAddress) {
      ipAddress = _mod13015.getClientIPAddress(normalizedRequest.headers);
    }
    if (!ipAddress) {
...
export const addRequestDataToEvent = function addRequestDataToEvent(request, request, include) {
  ...
  if (obj.ip) {
    ...
    if (ip) {
      ip = _mod13015.getClientIPAddress(request.headers);
    }
    if (!ip) {
      ip = request.ip;
    }
...
export const httpRequestToRequestData = function httpRequestToRequestData(headers) {
  ...
  return _mod12938.dropUndefinedKeys(request);
};
