// Module ID: 13132
// Function ID: 13133
// Name: sessionTimingIntegration
// Dependencies: [13053, 13095]

// Module 13132 (sessionTimingIntegration)
import _mod13053 from "module_13053" /* 13053 */;
import setupIntegration from "module_13095" /* 13095 */;

const require = globalThis.__r;


export const sessionTimingIntegration = setupIntegration.defineIntegration(() => {
  _require = 1000 * require("module_13053").timestampInSeconds();
  return {
    name: "SessionTiming",
    processEvent(extra) {
      const result = 1000 * _mod13053.timestampInSeconds();
      const obj2 = {};
      const merged = Object.assign(extra);
      const obj3 = {};
      const merged1 = Object.assign(extra.extra);
      obj3["session:start"] = closure_0;
      obj3["session:duration"] = result - closure_0;
      obj3["session:end"] = result;
      obj2.extra = obj3;
      return obj2;
    }
  };
});
