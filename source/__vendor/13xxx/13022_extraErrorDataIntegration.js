...
// Dependencies: [12988, 12939, 12977, 12938, 12941, 12960, 12932]

// Module 13022 (extraErrorDataIntegration)
        ...
        if (obj2.isError(obj.originalException)) {
          const tmp7 = (function _extractErrorData(originalException, arg1, maxValueLength) {
            try {
              ...
              if (num(12960).DEBUG_BUILD) {
                const logger = num(12932).logger;
          ...
          if (tmp7) {
            ...
            const normalizer = tmp4(12977);
            const normalizeResult = normalizer.normalize(tmp7, num);
            if (tmp4Result.isPlainObject(normalizeResult)) {
              const result = tmp4(12938).addNonEnumerableProperty(normalizeResult, "__sentry_skip_normalization__", true);
              obj3[tmp6] = normalizeResult;
              const tmp4Result2 = tmp4(12938);
            }
            let obj4 = {};
            const merged1 = Object.assign(contexts);
            obj4.contexts = obj3;
            tmp3 = obj4;
            tmp4Result = tmp4(12939);
          }
