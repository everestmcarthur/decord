...
// Dependencies: [32, 41, 42, 13045, 12962, 13049, 13050, 12953, 13051]

// Module 13048 (MetricsAggregator)
...
const entry = {
  ...
  value: function add(metricType, arg1, diff, none, tags) {
    ...
    if (arg5 === undefined) {
      timestampInSecondsResult = MetricsAggregator(12962).timestampInSeconds();
      const obj2 = MetricsAggregator(12962);
    }
    const self = this;
    const rounded = Math.floor(timestampInSecondsResult);
    const sanitizeMetricKeyResult = MetricsAggregator(13049).sanitizeMetricKey(arg1);
    const obj3 = MetricsAggregator(13049);
    const sanitizeTagsResult = MetricsAggregator(13049).sanitizeTags(obj);
    const obj4 = MetricsAggregator(13049);
    const sanitizeUnitResult = MetricsAggregator(13049).sanitizeUnit(str);
    const obj5 = MetricsAggregator(13049);
    const bucketKey = MetricsAggregator(13049).getBucketKey(metricType, sanitizeMetricKeyResult, sanitizeUnitResult, sanitizeTagsResult);
    const _buckets = this._buckets;
    value = _buckets.get(bucketKey);
    let num = 0;
    if (value) {
      num = 0;
      if (metricType === tmp5(13045).SET_METRIC_TYPE) {
    ...
    if (value) {
      ...
      const tmp15 = new tmp5(13050).METRIC_MAP[metricType](diff);
      obj7.metric = tmp15;
    ...
    const obj6 = MetricsAggregator(13049);
    const result1 = MetricsAggregator(12953).updateMetricSummaryOnActiveSpan(metricType, sanitizeMetricKeyResult, diff, sanitizeUnitResult, obj, bucketKey);
    self._bucketsTotalWeight = self._bucketsTotalWeight + obj7.metric.weight;
    if (self._bucketsTotalWeight >= MetricsAggregator(13045).MAX_WEIGHT) {
      self.flush();
    }
  }
};
  ...
  },
  {
    ...
    value: function _flush() {
      ...
      if (this._forceFlush) {
        ...
        const _Math = Math;
        const rounded = Math.floor(MetricsAggregator(12962).timestampInSeconds());
        const _Map = Map;
        const diff = rounded - MetricsAggregator(13045).DEFAULT_FLUSH_INTERVAL / 1000 - self._flushShift;
        const map = new Map();
  ...
  },
  {
    ...
    value: function _captureMetrics(_buckets) {
      if (_buckets.size > 0) {
        ...
        const result = MetricsAggregator(13051).captureAggregateMetrics(this._client, mapped);
        const obj = MetricsAggregator(13051);
      }
    }
  }
];
