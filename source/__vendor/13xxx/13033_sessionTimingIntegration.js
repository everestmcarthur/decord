// Module ID: 13033
// Function ID: 13034
// Name: sessionTimingIntegration
// Dependencies: [12954, 12996]

// Module 13033 (sessionTimingIntegration)
import _mod12954 from "module_12954" /* 12954 */;
import setupIntegration from "module_12996" /* 12996 */;

const require = globalThis.__r;


export const sessionTimingIntegration = setupIntegration.defineIntegration(() => {
  _require = 1000 * require("module_12954").timestampInSeconds();
  return {
    name: "SessionTiming",
    processEvent(extra) {
      const result = 1000 * _mod12954.timestampInSeconds();
      const obj2 = {};
      const merged = Object.assign(extra);
      const obj3 = {};
      const merged1 = Object.assign(extra.extra);
      obj3["session:start"] = closure_0;
      obj3["session:duration"] = result - closure_0;
      obj3["session:end"] = result;
      obj2.extra = obj3;
      return obj2;
    }
  };
});
