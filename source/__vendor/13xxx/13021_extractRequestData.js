...
// Dependencies: [13022, 13023, 13024, 12947, 12985, 12949, 12939, 12940, 12946]
...
// Module 13021 (extractRequestData)
...
function extractRequestData(headers, arg1) {
  ...
  let item = include.forEach((item) => {
    if ("headers" === item) {
      ...
      if (!obj5.includes("ip")) {
        const ipHeaderNames = _mod13023.ipHeaderNames;
      ...
      if (!cookies) {
        ...
        if (cookie) {
          cookie = _mod13024.parseCookie(tmp21.cookie);
        }
        cookies = cookie;
      }
      if (!cookies) {
...
function headersToDict(arg0) {
  ...
  try {
    ...
    if (_mod12939.DEBUG_BUILD) {
      const logger = tmp4(12940).logger;
      logger.warn("Sentry failed extracting headers from a request object. If you see this, please file an issue.");
    }
    tmp4 = require;
  }
...
export const addNormalizedRequestDataToEvent = function addNormalizedRequestDataToEvent(request, normalizedRequest, user, include) {
  ...
  if (obj.request) {
    ...
    if (obj2.includes("cookies")) {
      ...
      if (!cookies) {
        ...
        if (obj4.cookie) {
          parseCookieResult = _mod13024.parseCookie(obj4.cookie);
        }
  ...
  if (obj.ip) {
    ...
    if (ipAddress) {
      ipAddress = _mod13023.getClientIPAddress(normalizedRequest.headers);
    }
    if (!ipAddress) {
...
export const addRequestDataToEvent = function addRequestDataToEvent(request, request, include) {
  ...
  if (obj.ip) {
    ...
    if (ip) {
      ip = _mod13023.getClientIPAddress(request.headers);
    }
    if (!ip) {
      ip = request.ip;
    }
...
export const httpRequestToRequestData = function httpRequestToRequestData(headers) {
  ...
  return _mod12946.dropUndefinedKeys(request);
};
