...
// Dependencies: [32, 41, 42, 13136, 13053, 13140, 13141, 13044, 13142]

// Module 13139 (MetricsAggregator)
...
const entry = {
  ...
  value: function add(metricType, arg1, diff, none, tags) {
    ...
    if (arg5 === undefined) {
      timestampInSecondsResult = MetricsAggregator(13053).timestampInSeconds();
      const obj2 = MetricsAggregator(13053);
    }
    const self = this;
    const rounded = Math.floor(timestampInSecondsResult);
    const sanitizeMetricKeyResult = MetricsAggregator(13140).sanitizeMetricKey(arg1);
    const obj3 = MetricsAggregator(13140);
    const sanitizeTagsResult = MetricsAggregator(13140).sanitizeTags(obj);
    const obj4 = MetricsAggregator(13140);
    const sanitizeUnitResult = MetricsAggregator(13140).sanitizeUnit(str);
    const obj5 = MetricsAggregator(13140);
    const bucketKey = MetricsAggregator(13140).getBucketKey(metricType, sanitizeMetricKeyResult, sanitizeUnitResult, sanitizeTagsResult);
    const _buckets = this._buckets;
    value = _buckets.get(bucketKey);
    let num = 0;
    if (value) {
      num = 0;
      if (metricType === tmp5(13136).SET_METRIC_TYPE) {
    ...
    if (value) {
      ...
      const tmp15 = new tmp5(13141).METRIC_MAP[metricType](diff);
      obj7.metric = tmp15;
    ...
    const obj6 = MetricsAggregator(13140);
    const result1 = MetricsAggregator(13044).updateMetricSummaryOnActiveSpan(metricType, sanitizeMetricKeyResult, diff, sanitizeUnitResult, obj, bucketKey);
    self._bucketsTotalWeight = self._bucketsTotalWeight + obj7.metric.weight;
    if (self._bucketsTotalWeight >= MetricsAggregator(13136).MAX_WEIGHT) {
      self.flush();
    }
  }
};
  ...
  },
  {
    ...
    value: function _flush() {
      ...
      if (this._forceFlush) {
        ...
        const _Math = Math;
        const rounded = Math.floor(MetricsAggregator(13053).timestampInSeconds());
        const _Map = Map;
        const diff = rounded - MetricsAggregator(13136).DEFAULT_FLUSH_INTERVAL / 1000 - self._flushShift;
        const map = new Map();
  ...
  },
  {
    ...
    value: function _captureMetrics(_buckets) {
      if (_buckets.size > 0) {
        ...
        const result = MetricsAggregator(13142).captureAggregateMetrics(this._client, mapped);
        const obj = MetricsAggregator(13142);
      }
    }
  }
];
