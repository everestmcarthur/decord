...
// Dependencies: [13004, 12955, 12993, 12954, 12957, 12976, 12948]

// Module 13038 (extraErrorDataIntegration)
        ...
        if (obj2.isError(obj.originalException)) {
          const tmp7 = (function _extractErrorData(originalException, arg1, maxValueLength) {
            try {
              ...
              if (num(12976).DEBUG_BUILD) {
                const logger = num(12948).logger;
          ...
          if (tmp7) {
            ...
            const normalizer = tmp4(12993);
            const normalizeResult = normalizer.normalize(tmp7, num);
            if (tmp4Result.isPlainObject(normalizeResult)) {
              const result = tmp4(12954).addNonEnumerableProperty(normalizeResult, "__sentry_skip_normalization__", true);
              obj3[tmp6] = normalizeResult;
              const tmp4Result2 = tmp4(12954);
            }
            let obj4 = {};
