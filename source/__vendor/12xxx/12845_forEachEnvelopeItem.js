...
// Module 12845 (forEachEnvelopeItem)
...
import dsnFromString from "dsnFromString" /* 12848 */;
...
export const parseEnvelope = function parseEnvelope(arr) {
  if (typeof arr !== "string") {
    ...
    let encodePolyfill = dependencyMap;
    if (!_require(12802).GLOBAL_OBJ.__SENTRY__) {
      const _TextEncoder = TextEncoder;
      const textEncoder = new TextEncoder();
    ...
    if (typeof tmp8[1] !== "string") {
      ...
      if (!(tmp10 instanceof Uint8Array)) {
        try {
          ...
          json = _JSON3.stringify(_require(12846).normalize(tmp3));
          const obj = _require(12846);
        }
      }
    }
    append(tmp8[1]);
