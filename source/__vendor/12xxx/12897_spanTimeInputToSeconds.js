...
// Dependencies: [12898, 12902, 12904, 12906, 12907, 12908, 12909, 12910, 12911, 12914, 12919, 12892]
...
// Module 12897 (spanTimeInputToSeconds)
  ...
  if (typeof getTime === "number") {
    ...
    if (Array.isArray(getTime)) {
      ...
      if (getTime instanceof Date) {
        ...
        sum = _mod12906.timestampInSeconds();
      }
    }
  }
  return sum;
  ...
  if ((function spanIsSentrySpan(getSpanJSON) {
    ...
    try {
      ...
      if ((function spanIsOpenTelemetrySdkTraceBaseSpan(attributes) {
        ...
        obj3.op = attributes[_mod12907.SEMANTIC_ATTRIBUTE_SENTRY_OP];
        obj3.origin = attributes[_mod12907.SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN];
        const obj2 = _mod12898;
        obj3._metrics_summary = _mod12908.getMetricSummaryJsonForSpan(getSpanJSON);
...
function getStatusMessage(code) {
  if (code) {
    if (code.code !== _mod12909.SPAN_STATUS_UNSET) {
      let str = "ok";
      if (code.code !== tmp(12909).SPAN_STATUS_OK) {
...
export const addChildSpanToSpan = function addChildSpanToSpan(arg0, arg1) {
  ...
  const result = _mod12898.addNonEnumerableProperty(arg1, _sentryRootSpan, tmp2);
  if (arg0[_sentryChildSpans]) {
    ...
    const result1 = _mod12898.addNonEnumerableProperty(arg0, tmp6, set);
...
export const getActiveSpan = function getActiveSpan() {
  ...
  if (asyncContextStrategy.getActiveSpan) {
    ...
    activeSpan = tmpResult._getSpanForScope(tmp(12919).getCurrentScope());
...
export const spanToTraceContext = function spanToTraceContext(spanContext) {
  ...
  return _mod12898.dropUndefinedKeys({ parent_span_id, span_id, trace_id: trace_id.traceId });
};
export const spanToTraceHeader = function spanToTraceHeader(spanContext) {
  ...
  return _mod12904.generateSentryTraceHeader(traceId, spanId, 1 === spanContext.spanContext().traceFlags);
};
export const spanToTransactionTraceContext = function spanToTransactionTraceContext(spanContext) {
  ...
  return _mod12898.dropUndefinedKeys({ parent_span_id, span_id, trace_id, data, op, status, origin });
};
