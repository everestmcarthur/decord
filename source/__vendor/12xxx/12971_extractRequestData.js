...
// Dependencies: [12972, 12973, 12974, 12897, 12935, 12899, 12889, 12890, 12896]
...
// Module 12971 (extractRequestData)
...
function extractRequestData(headers, arg1) {
  ...
  let item = include.forEach((item) => {
    if ("headers" === item) {
      ...
      if (!obj5.includes("ip")) {
        const ipHeaderNames = _mod12973.ipHeaderNames;
      ...
      if (!cookies) {
        ...
        if (cookie) {
          cookie = _mod12974.parseCookie(tmp21.cookie);
        }
        cookies = cookie;
      }
      if (!cookies) {
...
function headersToDict(arg0) {
  ...
  try {
    ...
    if (_mod12889.DEBUG_BUILD) {
      const logger = tmp4(12890).logger;
      logger.warn("Sentry failed extracting headers from a request object. If you see this, please file an issue.");
    }
    tmp4 = require;
  }
...
export const addNormalizedRequestDataToEvent = function addNormalizedRequestDataToEvent(request, normalizedRequest, user, include) {
  ...
  if (obj.request) {
    ...
    if (obj2.includes("cookies")) {
      ...
      if (!cookies) {
        ...
        if (obj4.cookie) {
          parseCookieResult = _mod12974.parseCookie(obj4.cookie);
        }
  ...
  if (obj.ip) {
    ...
    if (ipAddress) {
      ipAddress = _mod12973.getClientIPAddress(normalizedRequest.headers);
    }
    if (!ipAddress) {
...
export const addRequestDataToEvent = function addRequestDataToEvent(request, request, include) {
  ...
  if (obj.ip) {
    ...
    if (ip) {
      ip = _mod12973.getClientIPAddress(request.headers);
    }
    if (!ip) {
      ip = request.ip;
    }
...
export const httpRequestToRequestData = function httpRequestToRequestData(headers) {
  ...
  return _mod12896.dropUndefinedKeys(request);
};
