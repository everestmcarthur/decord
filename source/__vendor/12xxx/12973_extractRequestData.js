...
// Dependencies: [12974, 12975, 12976, 12899, 12937, 12901, 12891, 12892, 12898]
...
// Module 12973 (extractRequestData)
...
function extractRequestData(headers, arg1) {
  ...
  let item = include.forEach((item) => {
    if ("headers" === item) {
      ...
      if (!obj5.includes("ip")) {
        const ipHeaderNames = _mod12975.ipHeaderNames;
      ...
      if (!cookies) {
        ...
        if (cookie) {
          cookie = _mod12976.parseCookie(tmp21.cookie);
        }
        cookies = cookie;
      }
      if (!cookies) {
...
function headersToDict(arg0) {
  ...
  try {
    ...
    if (_mod12891.DEBUG_BUILD) {
      const logger = tmp4(12892).logger;
      logger.warn("Sentry failed extracting headers from a request object. If you see this, please file an issue.");
    }
    tmp4 = require;
  }
...
export const addNormalizedRequestDataToEvent = function addNormalizedRequestDataToEvent(request, normalizedRequest, user, include) {
  ...
  if (obj.request) {
    ...
    if (obj2.includes("cookies")) {
      ...
      if (!cookies) {
        ...
        if (obj4.cookie) {
          parseCookieResult = _mod12976.parseCookie(obj4.cookie);
        }
  ...
  if (obj.ip) {
    ...
    if (ipAddress) {
      ipAddress = _mod12975.getClientIPAddress(normalizedRequest.headers);
    }
    if (!ipAddress) {
...
export const addRequestDataToEvent = function addRequestDataToEvent(request, request, include) {
  ...
  if (obj.ip) {
    ...
    if (ip) {
      ip = _mod12975.getClientIPAddress(request.headers);
    }
    if (!ip) {
      ip = request.ip;
    }
...
export const httpRequestToRequestData = function httpRequestToRequestData(headers) {
  ...
  return _mod12898.dropUndefinedKeys(request);
};
