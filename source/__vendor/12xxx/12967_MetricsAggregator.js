...
// Dependencies: [32, 41, 42, 12964, 12881, 12968, 12969, 12872, 12970]

// Module 12967 (MetricsAggregator)
...
const entry = {
  ...
  value: function add(metricType, arg1, diff, none, tags) {
    ...
    if (arg5 === undefined) {
      timestampInSecondsResult = MetricsAggregator(12881).timestampInSeconds();
      const obj2 = MetricsAggregator(12881);
    }
    const self = this;
    const rounded = Math.floor(timestampInSecondsResult);
    const sanitizeMetricKeyResult = MetricsAggregator(12968).sanitizeMetricKey(arg1);
    const obj3 = MetricsAggregator(12968);
    const sanitizeTagsResult = MetricsAggregator(12968).sanitizeTags(obj);
    const obj4 = MetricsAggregator(12968);
    const sanitizeUnitResult = MetricsAggregator(12968).sanitizeUnit(str);
    const obj5 = MetricsAggregator(12968);
    const bucketKey = MetricsAggregator(12968).getBucketKey(metricType, sanitizeMetricKeyResult, sanitizeUnitResult, sanitizeTagsResult);
    const _buckets = this._buckets;
    value = _buckets.get(bucketKey);
    let num = 0;
    if (value) {
      num = 0;
      if (metricType === tmp5(12964).SET_METRIC_TYPE) {
    ...
    if (value) {
      ...
      const tmp15 = new tmp5(12969).METRIC_MAP[metricType](diff);
      obj7.metric = tmp15;
    ...
    const obj6 = MetricsAggregator(12968);
    const result1 = MetricsAggregator(12872).updateMetricSummaryOnActiveSpan(metricType, sanitizeMetricKeyResult, diff, sanitizeUnitResult, obj, bucketKey);
    self._bucketsTotalWeight = self._bucketsTotalWeight + obj7.metric.weight;
    if (self._bucketsTotalWeight >= MetricsAggregator(12964).MAX_WEIGHT) {
      self.flush();
    }
  }
};
  ...
  },
  {
    ...
    value: function _flush() {
      ...
      if (this._forceFlush) {
        ...
        const _Math = Math;
        const rounded = Math.floor(MetricsAggregator(12881).timestampInSeconds());
        const _Map = Map;
        const diff = rounded - MetricsAggregator(12964).DEFAULT_FLUSH_INTERVAL / 1000 - self._flushShift;
        const map = new Map();
  ...
  },
  {
    ...
    value: function _captureMetrics(_buckets) {
      if (_buckets.size > 0) {
        ...
        const result = MetricsAggregator(12970).captureAggregateMetrics(this._client, mapped);
        const obj = MetricsAggregator(12970);
      }
    }
  }
];
