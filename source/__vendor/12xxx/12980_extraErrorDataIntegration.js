...
// Dependencies: [12946, 12897, 12935, 12896, 12899, 12918, 12890]

// Module 12980 (extraErrorDataIntegration)
        ...
        if (obj2.isError(obj.originalException)) {
          const tmp7 = (function _extractErrorData(originalException, arg1, maxValueLength) {
            try {
              ...
              if (num(12918).DEBUG_BUILD) {
                const logger = num(12890).logger;
          ...
          if (tmp7) {
            ...
            const normalizer = tmp4(12935);
            const normalizeResult = normalizer.normalize(tmp7, num);
            if (tmp4Result.isPlainObject(normalizeResult)) {
              const result = tmp4(12896).addNonEnumerableProperty(normalizeResult, "__sentry_skip_normalization__", true);
              obj3[tmp6] = normalizeResult;
              const tmp4Result2 = tmp4(12896);
            }
            let obj4 = {};
