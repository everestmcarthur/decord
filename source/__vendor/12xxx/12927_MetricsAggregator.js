...
// Dependencies: [32, 41, 42, 12924, 12841, 12928, 12929, 12832, 12930]

// Module 12927 (MetricsAggregator)
...
const entry = {
  ...
  value: function add(metricType, arg1, diff, none, tags) {
    ...
    if (arg5 === undefined) {
      timestampInSecondsResult = MetricsAggregator(12841).timestampInSeconds();
      const obj2 = MetricsAggregator(12841);
    }
    const self = this;
    const rounded = Math.floor(timestampInSecondsResult);
    const sanitizeMetricKeyResult = MetricsAggregator(12928).sanitizeMetricKey(arg1);
    const obj3 = MetricsAggregator(12928);
    const sanitizeTagsResult = MetricsAggregator(12928).sanitizeTags(obj);
    const obj4 = MetricsAggregator(12928);
    const sanitizeUnitResult = MetricsAggregator(12928).sanitizeUnit(str);
    const obj5 = MetricsAggregator(12928);
    const bucketKey = MetricsAggregator(12928).getBucketKey(metricType, sanitizeMetricKeyResult, sanitizeUnitResult, sanitizeTagsResult);
    const _buckets = this._buckets;
    value = _buckets.get(bucketKey);
    let num = 0;
    if (value) {
      num = 0;
      if (metricType === tmp5(12924).SET_METRIC_TYPE) {
    ...
    if (value) {
      ...
      const tmp15 = new tmp5(12929).METRIC_MAP[metricType](diff);
      obj7.metric = tmp15;
    ...
    const obj6 = MetricsAggregator(12928);
    const result1 = MetricsAggregator(12832).updateMetricSummaryOnActiveSpan(metricType, sanitizeMetricKeyResult, diff, sanitizeUnitResult, obj, bucketKey);
    self._bucketsTotalWeight = self._bucketsTotalWeight + obj7.metric.weight;
    if (self._bucketsTotalWeight >= MetricsAggregator(12924).MAX_WEIGHT) {
      self.flush();
    }
  }
};
  ...
  },
  {
    ...
    value: function _flush() {
      ...
      if (this._forceFlush) {
        ...
        const _Math = Math;
        const rounded = Math.floor(MetricsAggregator(12841).timestampInSeconds());
        const _Map = Map;
        const diff = rounded - MetricsAggregator(12924).DEFAULT_FLUSH_INTERVAL / 1000 - self._flushShift;
        const map = new Map();
  ...
  },
  {
    ...
    value: function _captureMetrics(_buckets) {
      if (_buckets.size > 0) {
        ...
        const result = MetricsAggregator(12930).captureAggregateMetrics(this._client, mapped);
        const obj = MetricsAggregator(12930);
      }
    }
  }
];
