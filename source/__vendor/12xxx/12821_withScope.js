...
export function getStackAsyncContextStrategy() {
  return {
    ...
    withSetIsolationScope(arg0, arg1) {
      const callback = arg1;
      let mainCarrier = callback(12819).getMainCarrier();
      let obj = callback(12819);
      let sentryCarrier = callback(12819).getSentryCarrier(mainCarrier);
      let stack = sentryCarrier.stack;
      if (!stack) {
        let tmpResult = tmp(12827);
    ...
    getCurrentScope() {
      const mainCarrier = callback(12819).getMainCarrier();
      const obj = callback(12819);
      const sentryCarrier = callback(12819).getSentryCarrier(mainCarrier);
      let stack = sentryCarrier.stack;
      if (!stack) {
        let tmpResult = tmp(12827);
        const defaultCurrentScope = tmpResult.getDefaultCurrentScope();
