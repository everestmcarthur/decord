...
export const applyScopeDataToEvent = function applyScopeDataToEvent(extra, arg1) {
  ...
  const dropUndefinedKeysResult = _mod12896.dropUndefinedKeys(extra);
  ...
  const dropUndefinedKeysResult1 = _mod12896.dropUndefinedKeys(tags);
  ...
  const tmp3Result = _mod12896;
  const dropUndefinedKeysResult2 = _mod12896.dropUndefinedKeys(user);
  ...
  const tmp3Result7 = _mod12896;
  const dropUndefinedKeysResult3 = _mod12896.dropUndefinedKeys(contexts);
  ...
  if (span) {
    const obj6 = { trace: tmp3(12895).spanToTraceContext(span) };
    const merged8 = Object.assign(extra.contexts);
    extra.contexts = obj6;
    const obj7 = { dynamicSamplingContext: null };
    const tmp3Result9 = tmp3(12895);
    obj7.dynamicSamplingContext = tmp3(12926).getDynamicSamplingContextFromSpan(span);
    const merged9 = Object.assign(extra.sdkProcessingMetadata);
    extra.sdkProcessingMetadata = obj7;
    const tmp3Result10 = tmp3(12926);
    const rootSpan = tmp3(12895).getRootSpan(span);
    const tmp3Result11 = tmp3(12895);
    const description = tmp3(12895).spanToJSON(rootSpan).description;
...
export const mergeAndOverwriteScopeData = function mergeAndOverwriteScopeData(arg0, arg1, arg2) {
  arg0[arg1] = _mod12915.merge(arg0[arg1], arg2, 1);
};
export const mergeScopeData = function mergeScopeData(extra, arg1) {
  ...
  extra.extra = _mod12915.merge(extra.extra, extra, 1);
  extra.tags = _mod12915.merge(extra.tags, tags, 1);
  extra.user = _mod12915.merge(extra.user, user, 1);
  extra.contexts = _mod12915.merge(extra.contexts, contexts, 1);
  extra.sdkProcessingMetadata = _mod12915.merge(extra.sdkProcessingMetadata, sdkProcessingMetadata, 2);
  if (level) {
    extra.level = level;
  }
  if (transactionName) {
