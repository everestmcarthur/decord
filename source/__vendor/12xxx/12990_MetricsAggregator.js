...
// Dependencies: [32, 41, 42, 12987, 12904, 12991, 12992, 12895, 12993]

// Module 12990 (MetricsAggregator)
...
const entry = {
  ...
  value: function add(metricType, arg1, diff, none, tags) {
    ...
    if (arg5 === undefined) {
      timestampInSecondsResult = MetricsAggregator(12904).timestampInSeconds();
      const obj2 = MetricsAggregator(12904);
    }
    const self = this;
    const rounded = Math.floor(timestampInSecondsResult);
    const sanitizeMetricKeyResult = MetricsAggregator(12991).sanitizeMetricKey(arg1);
    const obj3 = MetricsAggregator(12991);
    const sanitizeTagsResult = MetricsAggregator(12991).sanitizeTags(obj);
    const obj4 = MetricsAggregator(12991);
    const sanitizeUnitResult = MetricsAggregator(12991).sanitizeUnit(str);
    const obj5 = MetricsAggregator(12991);
    const bucketKey = MetricsAggregator(12991).getBucketKey(metricType, sanitizeMetricKeyResult, sanitizeUnitResult, sanitizeTagsResult);
    const _buckets = this._buckets;
    value = _buckets.get(bucketKey);
    let num = 0;
    if (value) {
      num = 0;
      if (metricType === tmp5(12987).SET_METRIC_TYPE) {
    ...
    if (value) {
      ...
      const tmp15 = new tmp5(12992).METRIC_MAP[metricType](diff);
      obj7.metric = tmp15;
    ...
    const obj6 = MetricsAggregator(12991);
    const result1 = MetricsAggregator(12895).updateMetricSummaryOnActiveSpan(metricType, sanitizeMetricKeyResult, diff, sanitizeUnitResult, obj, bucketKey);
    self._bucketsTotalWeight = self._bucketsTotalWeight + obj7.metric.weight;
    if (self._bucketsTotalWeight >= MetricsAggregator(12987).MAX_WEIGHT) {
      self.flush();
    }
  }
};
  ...
  },
  {
    ...
    value: function _flush() {
      ...
      if (this._forceFlush) {
        ...
        const _Math = Math;
        const rounded = Math.floor(MetricsAggregator(12904).timestampInSeconds());
        const _Map = Map;
        const diff = rounded - MetricsAggregator(12987).DEFAULT_FLUSH_INTERVAL / 1000 - self._flushShift;
        const map = new Map();
  ...
  },
  {
    ...
    value: function _captureMetrics(_buckets) {
      if (_buckets.size > 0) {
        ...
        const result = MetricsAggregator(12993).captureAggregateMetrics(this._client, mapped);
        const obj = MetricsAggregator(12993);
      }
    }
  }
];
