// Module ID: 12985
// Function ID: 12986
// Name: sessionTimingIntegration
// Dependencies: [12906, 12948]

// Module 12985 (sessionTimingIntegration)
import _mod12906 from "module_12906" /* 12906 */;
import setupIntegration from "module_12948" /* 12948 */;

const require = globalThis.__r;


export const sessionTimingIntegration = setupIntegration.defineIntegration(() => {
  _require = 1000 * require("module_12906").timestampInSeconds();
  return {
    name: "SessionTiming",
    processEvent(extra) {
      const result = 1000 * _mod12906.timestampInSeconds();
      const obj2 = {};
      const merged = Object.assign(extra);
      const obj3 = {};
      const merged1 = Object.assign(extra.extra);
      obj3["session:start"] = closure_0;
      obj3["session:duration"] = result - closure_0;
      obj3["session:end"] = result;
      obj2.extra = obj3;
      return obj2;
    }
  };
});
