...
export const applyScopeDataToEvent = function applyScopeDataToEvent(extra, arg1) {
  ...
  const dropUndefinedKeysResult = _mod12898.dropUndefinedKeys(extra);
  ...
  const dropUndefinedKeysResult1 = _mod12898.dropUndefinedKeys(tags);
  ...
  const tmp3Result = _mod12898;
  const dropUndefinedKeysResult2 = _mod12898.dropUndefinedKeys(user);
  ...
  const tmp3Result7 = _mod12898;
  const dropUndefinedKeysResult3 = _mod12898.dropUndefinedKeys(contexts);
  ...
  if (span) {
    const obj6 = { trace: tmp3(12897).spanToTraceContext(span) };
    const merged8 = Object.assign(extra.contexts);
    extra.contexts = obj6;
    const obj7 = { dynamicSamplingContext: null };
    const tmp3Result9 = tmp3(12897);
    obj7.dynamicSamplingContext = tmp3(12928).getDynamicSamplingContextFromSpan(span);
    const merged9 = Object.assign(extra.sdkProcessingMetadata);
    extra.sdkProcessingMetadata = obj7;
    const tmp3Result10 = tmp3(12928);
    const rootSpan = tmp3(12897).getRootSpan(span);
    const tmp3Result11 = tmp3(12897);
    const description = tmp3(12897).spanToJSON(rootSpan).description;
...
export const mergeAndOverwriteScopeData = function mergeAndOverwriteScopeData(arg0, arg1, arg2) {
  arg0[arg1] = _mod12917.merge(arg0[arg1], arg2, 1);
};
export const mergeScopeData = function mergeScopeData(extra, arg1) {
  ...
  extra.extra = _mod12917.merge(extra.extra, extra, 1);
  extra.tags = _mod12917.merge(extra.tags, tags, 1);
  extra.user = _mod12917.merge(extra.user, user, 1);
  extra.contexts = _mod12917.merge(extra.contexts, contexts, 1);
  extra.sdkProcessingMetadata = _mod12917.merge(extra.sdkProcessingMetadata, sdkProcessingMetadata, 2);
  if (level) {
    extra.level = level;
  }
  if (transactionName) {
