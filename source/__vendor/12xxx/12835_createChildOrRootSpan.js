...
// Module 12835 (createChildOrRootSpan)
import _mod12816 from "module_12816" /* 12816 */;
import getClient from "getClient" /* 12828 */;
import hasTracingEnabled from "hasTracingEnabled" /* 12833 */;
...
function createChildOrRootSpan(forceTransaction) {
  ...
  if (obj.hasTracingEnabled()) {
    ...
    if (parentSpan) {
      if (!forceTransaction.forceTransaction) {
        ...
        if (spanIsSampledResult) {
          ...
          let sentrySpan = new tmp(12842).SentrySpan(obj);
        } else {
          obj = { traceId: null };
          obj[0] = traceId;
          sentrySpan = new tmp(12834).SentryNonRecordingSpan(obj);
        }
        const spanContextResult = parentSpan.spanContext();
        tmp(12806).addChildSpanToSpan(parentSpan, sentrySpan);
        const tmpResult1 = tmp(12806);
    ...
    if (parentSpan) {
      ...
      obj1[0] = traceId2;
      obj1[1] = spanId2;
      const tmpResult7 = tmp(12806);
      const merged1 = Object.assign(spanArguments);
      const tmp42 = _startRootSpan(obj1, scope, tmp(12806).spanIsSampled(parentSpan));
...
function _startRootSpan(name) {
  ...
  if (arg1.getScopeData().sdkProcessingMetadata[__SENTRY_SUPPRESS_TRACING__]) {
    ...
    obj[1] = arg2;
    obj[3] = obj;
    sampleSpanResult = tmp(12840).sampleSpan(options, obj);
  ...
  obj1.sampled = tmp6;
  const sentrySpan = new tmp(12842).SentrySpan(obj1);
  if (undefined !== tmp7) {
...
export const continueTrace = (arg0, arg1) => {
  const _require = arg1;
  const mainCarrier = _require(12819).getMainCarrier();
  const obj = _require(12819);
  const tmp = _require;
  const asyncContextStrategy = _require(12820).getAsyncContextStrategy(mainCarrier);
      ...
      fn = undefined !== parentSpan ? ((arg0) => {
        ...
        if (asyncContextStrategy.withActiveSpan) {
          ...
          withActiveSpanResult = experimental(obj[1]).withScope((arg0) => {
            _null(12823)._setSpanForScope(arg0, _null);
...
export const startNewTrace = function startNewTrace(arg0) {
  const _require = arg0;
  return _require(12828).withScope((setPropagationContext) => {
...
export const startSpanManual = function startSpanManual(experimental) {
  ...
  let obj = _require(12819);
  let mainCarrier = obj.getMainCarrier();
  let asyncContextStrategy = _require(12820).getAsyncContextStrategy(mainCarrier);
