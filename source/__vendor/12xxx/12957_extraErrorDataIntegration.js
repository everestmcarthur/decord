...
// Dependencies: [12923, 12874, 12912, 12873, 12876, 12895, 12867]

// Module 12957 (extraErrorDataIntegration)
        ...
        if (obj2.isError(obj.originalException)) {
          const tmp7 = (function _extractErrorData(originalException, arg1, maxValueLength) {
            try {
              ...
              if (num(12895).DEBUG_BUILD) {
                const logger = num(12867).logger;
          ...
          if (tmp7) {
            ...
            const normalizer = tmp4(12912);
            const normalizeResult = normalizer.normalize(tmp7, num);
            if (tmp4Result.isPlainObject(normalizeResult)) {
              const result = tmp4(12873).addNonEnumerableProperty(normalizeResult, "__sentry_skip_normalization__", true);
              obj3[tmp6] = normalizeResult;
              const tmp4Result2 = tmp4(12873);
            }
            let obj4 = {};
            const merged1 = Object.assign(contexts);
            obj4.contexts = obj3;
            tmp3 = obj4;
            tmp4Result = tmp4(12874);
          }
