...
// Dependencies: [12933, 12937, 12939, 12941, 12942, 12943, 12944, 12945, 12946, 12949, 12954, 12927]
...
// Module 12932 (spanTimeInputToSeconds)
  ...
  if (typeof getTime === "number") {
    ...
    if (Array.isArray(getTime)) {
      ...
      if (getTime instanceof Date) {
        ...
        sum = _mod12941.timestampInSeconds();
      }
    }
  }
  return sum;
  ...
  if ((function spanIsSentrySpan(getSpanJSON) {
    ...
    try {
      ...
      if ((function spanIsOpenTelemetrySdkTraceBaseSpan(attributes) {
        ...
        obj3.op = attributes[_mod12942.SEMANTIC_ATTRIBUTE_SENTRY_OP];
        obj3.origin = attributes[_mod12942.SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN];
        const obj2 = _mod12933;
        obj3._metrics_summary = _mod12943.getMetricSummaryJsonForSpan(getSpanJSON);
...
function getStatusMessage(code) {
  if (code) {
    if (code.code !== _mod12944.SPAN_STATUS_UNSET) {
      let str = "ok";
      if (code.code !== tmp(12944).SPAN_STATUS_OK) {
...
export const addChildSpanToSpan = function addChildSpanToSpan(arg0, arg1) {
  ...
  const result = _mod12933.addNonEnumerableProperty(arg1, _sentryRootSpan, tmp2);
  if (arg0[_sentryChildSpans]) {
    ...
    const result1 = _mod12933.addNonEnumerableProperty(arg0, tmp6, set);
...
export const getActiveSpan = function getActiveSpan() {
  ...
  if (asyncContextStrategy.getActiveSpan) {
    ...
    activeSpan = tmpResult._getSpanForScope(tmp(12954).getCurrentScope());
...
export const spanToTraceContext = function spanToTraceContext(spanContext) {
  ...
  return _mod12933.dropUndefinedKeys({ parent_span_id, span_id, trace_id: trace_id.traceId });
};
export const spanToTraceHeader = function spanToTraceHeader(spanContext) {
  ...
  return _mod12939.generateSentryTraceHeader(traceId, spanId, 1 === spanContext.spanContext().traceFlags);
};
export const spanToTransactionTraceContext = function spanToTransactionTraceContext(spanContext) {
  ...
  return _mod12933.dropUndefinedKeys({ parent_span_id, span_id, trace_id, data, op, status, origin });
};
