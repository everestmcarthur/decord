...
// Dependencies: [377, 41, 42, 10569, 10567, 10571]

// Module 10568 (ReferenceWithTimezone)
  ...
  entry1,
  {
    ...
    value: function fromInput(instant, timezones) {
      if (instant instanceof Date) {
        ...
        const toTimezoneOffsetResult = ParsingComponents(10569).toTimezoneOffset(timezone, instant, timezones);
        const obj = Object.create(ParsingComponents.prototype);
        _classCallCheck(obj, ParsingComponents);
        if (null == instant) {
          const _Date2 = Date;
...
const items2 = [
  ...
  },
  {
    ...
    value: function addDurationAsImplied(EmptyDuration) {
      ...
      const addDurationResult = ParsingComponents(10567).addDuration(result, EmptyDuration);
...
const entry3 = {
  ...
  value: function createRelativeFromReference(reference, reverseDurationResult) {
    ...
    if (reverseDurationResult === undefined) {
      EmptyDuration = ParsingComponents(10567).EmptyDuration;
    }
    const addDurationResult = ParsingComponents(10567).addDuration(reference.getDateWithAdjustedTimezone(), EmptyDuration);
    const obj = Object.create(ParsingComponents.prototype);
    _classCallCheck(obj, ParsingComponents);
    obj._tags = new Set();
    obj.reference = reference;
      ...
      if (!("minute" in EmptyDuration)) {
        if (!("second" in EmptyDuration)) {
          if (!("millisecond" in EmptyDuration)) {
            ParsingComponents(10571).implySimilarTime(obj, addDurationResult);
    ...
    ParsingComponents(10571).assignSimilarTime(obj, addDurationResult);
    ParsingComponents(10571).assignSimilarDate(obj, addDurationResult);
