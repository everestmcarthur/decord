...
// Dependencies: [377, 41, 42, 10525, 10523, 10527]

// Module 10524 (ReferenceWithTimezone)
  ...
  entry1,
  {
    ...
    value: function fromInput(instant, timezones) {
      if (instant instanceof Date) {
        ...
        const toTimezoneOffsetResult = ParsingComponents(10525).toTimezoneOffset(timezone, instant, timezones);
        const obj = Object.create(ParsingComponents.prototype);
        _classCallCheck(obj, ParsingComponents);
        if (null == instant) {
          const _Date2 = Date;
...
const items2 = [
  ...
  },
  {
    ...
    value: function addDurationAsImplied(EmptyDuration) {
      ...
      const addDurationResult = ParsingComponents(10523).addDuration(result, EmptyDuration);
...
const entry3 = {
  ...
  value: function createRelativeFromReference(reference, reverseDurationResult) {
    ...
    if (reverseDurationResult === undefined) {
      EmptyDuration = ParsingComponents(10523).EmptyDuration;
    }
    const addDurationResult = ParsingComponents(10523).addDuration(reference.getDateWithAdjustedTimezone(), EmptyDuration);
    const obj = Object.create(ParsingComponents.prototype);
    _classCallCheck(obj, ParsingComponents);
    obj._tags = new Set();
    obj.reference = reference;
      ...
      if (!("minute" in EmptyDuration)) {
        if (!("second" in EmptyDuration)) {
          if (!("millisecond" in EmptyDuration)) {
            ParsingComponents(10527).implySimilarTime(obj, addDurationResult);
    ...
    ParsingComponents(10527).assignSimilarTime(obj, addDurationResult);
    ParsingComponents(10527).assignSimilarDate(obj, addDurationResult);
