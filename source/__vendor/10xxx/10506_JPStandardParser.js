...
const items = [
  ...
  },
  {
    ...
    value: function extract(createParsingComponents) {
      const parsed = parseInt(JPStandardParser(10507).toHankaku(arg1[5]));
      const parsed1 = parseInt(JPStandardParser(10507).toHankaku(arg1[6]));
      ...
      if (arg1[2]) {
        ...
        if ("\u5143" != arg1[4]) {
          ...
          num = parseInt(tmp(10507).toHankaku(tmp8));
        }
        if ("\u4EE4\u548C" == arg1[3]) {
          let sum = num + 2018;
        ...
        parsingComponents.imply("year", tmp(10438).findYearClosestToRef(createParsingComponents.refDate, parsed1, parsed));
