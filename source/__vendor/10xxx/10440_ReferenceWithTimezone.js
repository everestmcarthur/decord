...
// Dependencies: [377, 41, 42, 10441, 10439, 10443]

// Module 10440 (ReferenceWithTimezone)
  ...
  obj,
  {
    ...
    value: function fromInput(instant, timezones) {
      if (instant instanceof Date) {
        ...
        const toTimezoneOffsetResult = ParsingComponents(10441).toTimezoneOffset(timezone, instant, timezones);
        const obj = Object.create(ParsingComponents.prototype);
        callback(obj, ParsingComponents);
        if (null == instant) {
          const _Date2 = Date;
...
const items2 = [
  ...
  },
  {
    ...
    value: function addDurationAsImplied(EmptyDuration) {
      ...
      const addDurationResult = ParsingComponents(10439).addDuration(result, EmptyDuration);
...
const items3 = [
  {
    ...
    value: function createRelativeFromReference(reference, reverseDurationResult) {
      ...
      if (reverseDurationResult === undefined) {
        EmptyDuration = ParsingComponents(10439).EmptyDuration;
      }
      const addDurationResult = ParsingComponents(10439).addDuration(reference.getDateWithAdjustedTimezone(), EmptyDuration);
        ...
        if (!("minute" in EmptyDuration)) {
          if (!("second" in EmptyDuration)) {
            if (!("millisecond" in EmptyDuration)) {
              ParsingComponents(10443).implySimilarTime(obj, addDurationResult);
      ...
      ParsingComponents(10443).assignSimilarTime(obj, addDurationResult);
      ParsingComponents(10443).assignSimilarDate(obj, addDurationResult);
