...
const items = [
  entry,
  {
    ...
    value: function extract(createParsingComponents, arg1) {
      const parsed = parseInt(JPStandardParser(10635).toHankaku(arg1[5]));
      const parsed1 = parseInt(JPStandardParser(10635).toHankaku(arg1[6]));
      ...
      if (arg1[2]) {
        ...
        if ("\u5143" != arg1[4]) {
          ...
          num = parseInt(tmp(10635).toHankaku(tmp8));
        }
        if ("\u4EE4\u548C" == arg1[3]) {
          let sum = num + 2018;
        ...
        parsingComponents.imply("year", tmp(10566).findYearClosestToRef(createParsingComponents.refDate, parsed1, parsed));
