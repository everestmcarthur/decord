...
// Dependencies: [377, 41, 42, 10468, 10466, 10470]

// Module 10467 (ReferenceWithTimezone)
  ...
  entry1,
  {
    ...
    value: function fromInput(instant, timezones) {
      if (instant instanceof Date) {
        ...
        const toTimezoneOffsetResult = ParsingComponents(10468).toTimezoneOffset(timezone, instant, timezones);
        const obj = Object.create(ParsingComponents.prototype);
        _classCallCheck(obj, ParsingComponents);
        if (null == instant) {
          const _Date2 = Date;
...
const items2 = [
  ...
  },
  {
    ...
    value: function addDurationAsImplied(EmptyDuration) {
      ...
      const addDurationResult = ParsingComponents(10466).addDuration(result, EmptyDuration);
...
const entry3 = {
  ...
  value: function createRelativeFromReference(reference, reverseDurationResult) {
    ...
    if (reverseDurationResult === undefined) {
      EmptyDuration = ParsingComponents(10466).EmptyDuration;
    }
    const addDurationResult = ParsingComponents(10466).addDuration(reference.getDateWithAdjustedTimezone(), EmptyDuration);
    const obj = Object.create(ParsingComponents.prototype);
    _classCallCheck(obj, ParsingComponents);
    obj._tags = new Set();
    obj.reference = reference;
      ...
      if (!("minute" in EmptyDuration)) {
        if (!("second" in EmptyDuration)) {
          if (!("millisecond" in EmptyDuration)) {
            ParsingComponents(10470).implySimilarTime(obj, addDurationResult);
    ...
    ParsingComponents(10470).assignSimilarTime(obj, addDurationResult);
    ParsingComponents(10470).assignSimilarDate(obj, addDurationResult);
