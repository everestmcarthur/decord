...
// Function ID: 6824
// Name: hash
// Dependencies: [32, 6810, 6809, 6824, 6833]
...
import allowedNativeProps from "allowedNativeProps" /* 6824 */;
...
let c4 = 1.618;
fn(6810).NativeProxy.updateGestureHandlerConfig;
...
maybeUnpackValue.__closure = { Reanimated: fn(6809).Reanimated };
...
function getEnabledSharedValues(Reanimated) {
  if (undefined === _mod6809.Reanimated) {
    return [];
  } else {
    if (tmpResult.isComposedGesture(Reanimated)) {
      ...
      Reanimated = tmp(6809).Reanimated;
...
export const bindSharedValues = function bindSharedValues(shouldUseReanimatedDetector, handlerTag) {
  _require = handlerTag;
  if (undefined !== require("module_6809").Reanimated) {
    ...
    while (tmp27 !== undefined) {
      ...
      let tmp8 = tmp7;
      let tmp10 = _require;
      let Reanimated = require("module_6809").Reanimated;
      if (Reanimated.isSharedValue(tmp7)) {
        let Reanimated2 = tmp10(6809).Reanimated;
    ...
    function _loop(maybeSharedValue) {
      ...
      if (Reanimated.isSharedValue(maybeSharedValue)) {
        if (typeof hash === "function") {
          ...
          closure_1 = sum;
          const Reanimated2 = _mod6809.Reanimated;
          const fn = function t() {
