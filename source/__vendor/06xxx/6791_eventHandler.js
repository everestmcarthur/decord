...
// Function ID: 6792
// Name: eventHandler
// Dependencies: [6773, 6711, 6726, 6713, 6775]
// Exports: eventHandler

// Module 6791 (eventHandler)
...
function handleStateChangeEvent(result, arg1, lastUpdateEvent, fn) {
  ...
  if (oldState === _mod6711.State.UNDETERMINED) {
    if (state === tmp(6711).State.BEGAN) {
      tmp(6773).runCallback(tmp(6726).CALLBACK_TYPE.BEGAN, arg1, result);
      const tmpResult = tmp(6773);
    }
  }
  if (oldState === _mod6711.State.BEGAN) {
    if (state === tmp(6711).State.ACTIVE) {
      if (fn != null) {
        fn(result);
      }
      tmp(6773).runCallback(tmp(6726).CALLBACK_TYPE.START, arg1, result);
  ...
  if (oldState !== state) {
    ...
    obj2.canceled = state === tmp(6711).State.FAILED || state === tmp(6711).State.CANCELLED;
    if (oldState === tmp(6711).State.ACTIVE) {
      if (fn != null) {
        fn(obj2);
      }
      tmp(6773).runCallback(tmp(6726).CALLBACK_TYPE.END, arg1, obj2);
      const tmpResult5 = tmp(6773);
    }
    const tmp7 = state === tmp(6711).State.FAILED || state === tmp(6711).State.CANCELLED;
    tmp(6773).runCallback(tmp(6726).CALLBACK_TYPE.FINALIZE, arg1, obj2);
    if (lastUpdateEvent) {
...
handleStateChangeEvent.__closure = { flattenAndFilterEvent: fn(6773).flattenAndFilterEvent, State: fn(6711).State, runCallback: fn(6773).runCallback, CALLBACK_TYPE: fn(6726).CALLBACK_TYPE };
...
function handleUpdateEvent(lastUpdateEvent, arg1, fn, lastUpdateEvent2) {
  ...
  DEFAULT_PROPS_TRANSFORMER.runCallback(_mod6726.CALLBACK_TYPE.UPDATE, arg1, result);
  if (lastUpdateEvent2) {
    lastUpdateEvent2.lastUpdateEvent = lastUpdateEvent;
  }
}
let obj = { flattenAndFilterEvent: fn(6773).flattenAndFilterEvent, State: fn(6711).State, runCallback: fn(6773).runCallback, CALLBACK_TYPE: fn(6726).CALLBACK_TYPE };
handleUpdateEvent.__closure = { flattenAndFilterEvent: fn(6773).flattenAndFilterEvent, runCallback: fn(6773).runCallback, CALLBACK_TYPE: fn(6726).CALLBACK_TYPE };
...
function handleTouchEvent(eventType, arg1) {
  if (eventType.eventType !== TouchEventType.TouchEventType.UNDETERMINED) {
    const tmpResult = tmp(6773);
    tmpResult.runCallback(tmp(6773).touchEventTypeToCallbackType(eventType.eventType), arg1, eventType);
    const tmpResult2 = tmp(6773);
  }
}
let obj2 = { flattenAndFilterEvent: fn(6773).flattenAndFilterEvent, runCallback: fn(6773).runCallback, CALLBACK_TYPE: fn(6726).CALLBACK_TYPE };
handleTouchEvent.__closure = { TouchEventType: fn(6713).TouchEventType, runCallback: fn(6773).runCallback, touchEventTypeToCallbackType: fn(6773).touchEventTypeToCallbackType };
    ...
    if (tmpResult.isStateChangeEvent(result)) {
      ...
      if (tmpResult6.isTouchEvent(result)) {
        if (typeof handleTouchEvent === "function") {
          if (result.eventType !== tmp(6713).TouchEventType.UNDETERMINED) {
            const tmpResult7 = tmp(6773);
            tmpResult7.runCallback(tmp(6773).touchEventTypeToCallbackType(result.eventType), arg2, result);
            const tmpResult8 = tmp(6773);
          }
        } else {
        ...
        if (typeof handleUpdateEvent === "function") {
          ...
          const result1 = tmp(6773).flattenAndFilterEvent(tmp9);
          const tmpResult9 = tmp(6773);
          tmp(6773).runCallback(tmp(6726).CALLBACK_TYPE.UPDATE, arg2, result1);
          if (lastUpdateEvent) {
            lastUpdateEvent.lastUpdateEvent = result;
          }
          const tmpResult10 = tmp(6773);
        } else {
          throw new TypeError("Trying to call a non-function");
        }
      }
...
const obj3 = { TouchEventType: fn(6713).TouchEventType, runCallback: fn(6773).runCallback, touchEventTypeToCallbackType: fn(6773).touchEventTypeToCallbackType };
eventHandler.__closure = { maybeExtractNativeEvent: fn(6773).maybeExtractNativeEvent, isEventForHandlerWithTag: fn(6773).isEventForHandlerWithTag, isStateChangeEvent: fn(6775).isStateChangeEvent, handleStateChangeEvent, isTouchEvent: fn(6775).isTouchEvent, handleTouchEvent, handleUpdateEvent };
