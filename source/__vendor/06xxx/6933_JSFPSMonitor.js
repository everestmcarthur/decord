...
// Function ID: 6934
// Name: JSFPSMonitor
// Dependencies: [6880, 6881, 6868, 6934]

// Module 6933 (JSFPSMonitor)
...
const items = [
  entry,
  {
    ...
    value: function startTracking() {
      ...
      if (0 !== this.startTime) {
        const _Error = Error;
        const error = new Error(JSFPSMonitor(6868).ErrorMessages.fpsMonitorAlreadyRunning);
        throw error;
      } else {
        const _Date = Date;
  ...
  },
  {
    ...
    value: function stopAndGetData() {
      ...
      const obj = { minFPS: JSFPSMonitor(6934).roundToDecimalPlaces(self.minFPS, 1), maxFPS: null, averageFPS: null };
      const obj2 = JSFPSMonitor(6934);
      obj.maxFPS = JSFPSMonitor(6934).roundToDecimalPlaces(self.maxFPS, 1);
      const obj3 = JSFPSMonitor(6934);
      obj.averageFPS = JSFPSMonitor(6934).roundToDecimalPlaces(self.averageFPS, 1);
      return obj;
    }
  }
];

export const JSFPSMonitor = _modDef6881(JSFPSMonitor, items);
