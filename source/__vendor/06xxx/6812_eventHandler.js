...
// Function ID: 6813
// Name: eventHandler
// Dependencies: [6794, 6732, 6747, 6734, 6796]
// Exports: eventHandler

// Module 6812 (eventHandler)
...
function handleStateChangeEvent(result, arg1, lastUpdateEvent, fn) {
  ...
  if (oldState === _mod6732.State.UNDETERMINED) {
    if (state === tmp(6732).State.BEGAN) {
      tmp(6794).runCallback(tmp(6747).CALLBACK_TYPE.BEGAN, arg1, result);
      const tmpResult = tmp(6794);
    }
  }
  if (oldState === _mod6732.State.BEGAN) {
    if (state === tmp(6732).State.ACTIVE) {
      if (fn != null) {
        fn(result);
      }
      tmp(6794).runCallback(tmp(6747).CALLBACK_TYPE.START, arg1, result);
  ...
  if (oldState !== state) {
    ...
    obj2.canceled = state === tmp(6732).State.FAILED || state === tmp(6732).State.CANCELLED;
    if (oldState === tmp(6732).State.ACTIVE) {
      if (fn != null) {
        fn(obj2);
      }
      tmp(6794).runCallback(tmp(6747).CALLBACK_TYPE.END, arg1, obj2);
      const tmpResult5 = tmp(6794);
    }
    const tmp7 = state === tmp(6732).State.FAILED || state === tmp(6732).State.CANCELLED;
    tmp(6794).runCallback(tmp(6747).CALLBACK_TYPE.FINALIZE, arg1, obj2);
    if (lastUpdateEvent) {
      lastUpdateEvent.lastUpdateEvent = undefined;
    }
    const tmpResult6 = tmp(6794);
  }
}
handleStateChangeEvent.__closure = { flattenAndFilterEvent: fn(6794).flattenAndFilterEvent, State: fn(6732).State, runCallback: fn(6794).runCallback, CALLBACK_TYPE: fn(6747).CALLBACK_TYPE };
...
function handleUpdateEvent(lastUpdateEvent, arg1, fn, lastUpdateEvent2) {
  ...
  DEFAULT_PROPS_TRANSFORMER.runCallback(_mod6747.CALLBACK_TYPE.UPDATE, arg1, result);
  if (lastUpdateEvent2) {
    lastUpdateEvent2.lastUpdateEvent = lastUpdateEvent;
  }
}
let obj = { flattenAndFilterEvent: fn(6794).flattenAndFilterEvent, State: fn(6732).State, runCallback: fn(6794).runCallback, CALLBACK_TYPE: fn(6747).CALLBACK_TYPE };
handleUpdateEvent.__closure = { flattenAndFilterEvent: fn(6794).flattenAndFilterEvent, runCallback: fn(6794).runCallback, CALLBACK_TYPE: fn(6747).CALLBACK_TYPE };
...
function handleTouchEvent(eventType, arg1) {
  if (eventType.eventType !== TouchEventType.TouchEventType.UNDETERMINED) {
    const tmpResult = tmp(6794);
    tmpResult.runCallback(tmp(6794).touchEventTypeToCallbackType(eventType.eventType), arg1, eventType);
    const tmpResult2 = tmp(6794);
  }
}
let obj2 = { flattenAndFilterEvent: fn(6794).flattenAndFilterEvent, runCallback: fn(6794).runCallback, CALLBACK_TYPE: fn(6747).CALLBACK_TYPE };
handleTouchEvent.__closure = { TouchEventType: fn(6734).TouchEventType, runCallback: fn(6794).runCallback, touchEventTypeToCallbackType: fn(6794).touchEventTypeToCallbackType };
    ...
    if (tmpResult.isStateChangeEvent(result)) {
      ...
      if (tmpResult6.isTouchEvent(result)) {
        if (typeof handleTouchEvent === "function") {
          if (result.eventType !== tmp(6734).TouchEventType.UNDETERMINED) {
            const tmpResult7 = tmp(6794);
            tmpResult7.runCallback(tmp(6794).touchEventTypeToCallbackType(result.eventType), arg2, result);
        ...
        if (typeof handleUpdateEvent === "function") {
          ...
          const result1 = tmp(6794).flattenAndFilterEvent(tmp9);
          const tmpResult9 = tmp(6794);
          tmp(6794).runCallback(tmp(6747).CALLBACK_TYPE.UPDATE, arg2, result1);
          if (lastUpdateEvent) {
            lastUpdateEvent.lastUpdateEvent = result;
          }
          const tmpResult10 = tmp(6794);
        } else {
          throw new TypeError("Trying to call a non-function");
        }
      }
...
const obj3 = { TouchEventType: fn(6734).TouchEventType, runCallback: fn(6794).runCallback, touchEventTypeToCallbackType: fn(6794).touchEventTypeToCallbackType };
eventHandler.__closure = { maybeExtractNativeEvent: fn(6794).maybeExtractNativeEvent, isEventForHandlerWithTag: fn(6794).isEventForHandlerWithTag, isStateChangeEvent: fn(6796).isStateChangeEvent, handleStateChangeEvent, isTouchEvent: fn(6796).isTouchEvent, handleTouchEvent, handleUpdateEvent };
