...
// Function ID: 6850
// Name: eventHandler
// Dependencies: [6831, 6769, 6784, 6771, 6833]
// Exports: eventHandler

// Module 6849 (eventHandler)
import _mod6769 from "module_6769" /* 6769 */;
import TouchEventType from "TouchEventType" /* 6771 */;
...
function handleStateChangeEvent(result, arg1, lastUpdateEvent, fn) {
  ...
  if (oldState === _mod6769.State.UNDETERMINED) {
    if (state === tmp(6769).State.BEGAN) {
      tmp(6831).runCallback(tmp(6784).CALLBACK_TYPE.BEGAN, arg1, result);
      const tmpResult = tmp(6831);
    }
  }
  if (oldState === _mod6769.State.BEGAN) {
    if (state === tmp(6769).State.ACTIVE) {
      if (fn != null) {
        fn(result);
      }
      tmp(6831).runCallback(tmp(6784).CALLBACK_TYPE.START, arg1, result);
  ...
  if (oldState !== state) {
    ...
    obj2.canceled = state === tmp(6769).State.FAILED || state === tmp(6769).State.CANCELLED;
    if (oldState === tmp(6769).State.ACTIVE) {
      if (fn != null) {
        fn(obj2);
      }
      tmp(6831).runCallback(tmp(6784).CALLBACK_TYPE.END, arg1, obj2);
      const tmpResult5 = tmp(6831);
    }
    const tmp7 = state === tmp(6769).State.FAILED || state === tmp(6769).State.CANCELLED;
    tmp(6831).runCallback(tmp(6784).CALLBACK_TYPE.FINALIZE, arg1, obj2);
    if (lastUpdateEvent) {
      lastUpdateEvent.lastUpdateEvent = undefined;
    }
    const tmpResult6 = tmp(6831);
  }
}
handleStateChangeEvent.__closure = { flattenAndFilterEvent: fn(6831).flattenAndFilterEvent, State: fn(6769).State, runCallback: fn(6831).runCallback, CALLBACK_TYPE: fn(6784).CALLBACK_TYPE };
...
function handleUpdateEvent(lastUpdateEvent, arg1, fn, lastUpdateEvent2) {
  ...
  DEFAULT_PROPS_TRANSFORMER.runCallback(_mod6784.CALLBACK_TYPE.UPDATE, arg1, result);
  if (lastUpdateEvent2) {
    lastUpdateEvent2.lastUpdateEvent = lastUpdateEvent;
  }
}
let obj = { flattenAndFilterEvent: fn(6831).flattenAndFilterEvent, State: fn(6769).State, runCallback: fn(6831).runCallback, CALLBACK_TYPE: fn(6784).CALLBACK_TYPE };
handleUpdateEvent.__closure = { flattenAndFilterEvent: fn(6831).flattenAndFilterEvent, runCallback: fn(6831).runCallback, CALLBACK_TYPE: fn(6784).CALLBACK_TYPE };
...
function handleTouchEvent(eventType, arg1) {
  if (eventType.eventType !== TouchEventType.TouchEventType.UNDETERMINED) {
    const tmpResult = tmp(6831);
    tmpResult.runCallback(tmp(6831).touchEventTypeToCallbackType(eventType.eventType), arg1, eventType);
    const tmpResult2 = tmp(6831);
  }
}
let obj2 = { flattenAndFilterEvent: fn(6831).flattenAndFilterEvent, runCallback: fn(6831).runCallback, CALLBACK_TYPE: fn(6784).CALLBACK_TYPE };
handleTouchEvent.__closure = { TouchEventType: fn(6771).TouchEventType, runCallback: fn(6831).runCallback, touchEventTypeToCallbackType: fn(6831).touchEventTypeToCallbackType };
    ...
    if (tmpResult.isStateChangeEvent(result)) {
      ...
      if (tmpResult6.isTouchEvent(result)) {
        if (typeof handleTouchEvent === "function") {
          if (result.eventType !== tmp(6771).TouchEventType.UNDETERMINED) {
            const tmpResult7 = tmp(6831);
            tmpResult7.runCallback(tmp(6831).touchEventTypeToCallbackType(result.eventType), arg2, result);
            const tmpResult8 = tmp(6831);
          }
        } else {
        ...
        if (typeof handleUpdateEvent === "function") {
          ...
          const result1 = tmp(6831).flattenAndFilterEvent(tmp9);
          const tmpResult9 = tmp(6831);
          tmp(6831).runCallback(tmp(6784).CALLBACK_TYPE.UPDATE, arg2, result1);
          if (lastUpdateEvent) {
            lastUpdateEvent.lastUpdateEvent = result;
          }
          const tmpResult10 = tmp(6831);
        } else {
          throw new TypeError("Trying to call a non-function");
        }
      }
...
const obj3 = { TouchEventType: fn(6771).TouchEventType, runCallback: fn(6831).runCallback, touchEventTypeToCallbackType: fn(6831).touchEventTypeToCallbackType };
eventHandler.__closure = { maybeExtractNativeEvent: fn(6831).maybeExtractNativeEvent, isEventForHandlerWithTag: fn(6831).isEventForHandlerWithTag, isStateChangeEvent: fn(6833).isStateChangeEvent, handleStateChangeEvent, isTouchEvent: fn(6833).isTouchEvent, handleTouchEvent, handleUpdateEvent };
