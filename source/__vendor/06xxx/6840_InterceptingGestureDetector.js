...
// Dependencies: [32, 19, 17, 21, 6781, 6841, 6777, 6836, 6778, 6768, 6806, 6837, 6838, 6830, 6839]
// Exports: InterceptingGestureDetector

// Module 6840 (InterceptingGestureDetector)
...
export const InterceptingGestureDetector = function InterceptingGestureDetector(gesture) {
  ...
  gesture(6781).useEnsureGestureHandlerRootView();
  const tmp6 = first1(closure_7(() => new Set()), 2);
  const first = tmp6[0];
  dependencyMap = tmp6[1];
  let items = [first];
  let prop;
  let obj = gesture(6781);
  ...
  if (prop) {
    let REANIMATED = tmp(6841).InterceptingDetectorMode.REANIMATED;
  } else {
    let prop1;
    if (gesture != null) {
      prop1 = gesture.config.dispatchesAnimatedEvents;
    }
    const InterceptingDetectorMode = tmp(6841).InterceptingDetectorMode;
  ...
  const tmp14 = first1 === gesture(6841).InterceptingDetectorMode.REANIMATED;
  if (first1 === gesture(6841).InterceptingDetectorMode.ANIMATED) {
    let ReanimatedNativeDetector = tmp(6777).AnimatedNativeDetector;
  } else if (tmp14) {
    ReanimatedNativeDetector = tmp(6836).ReanimatedNativeDetector;
  } else {
    ReanimatedNativeDetector = first(6778);
  }
  ...
  register(() => {
    ...
    if (prop) {
      closure_7.setMode(_mod6841.InterceptingDetectorMode.ANIMATED);
    } else {
      let prop1;
      if (tmp != null) {
      ...
      if (prop1) {
        closure_7.setMode(_mod6841.InterceptingDetectorMode.REANIMATED);
      }
    }
  }, items2);
  if (ReanimatedNativeDetector) {
    ...
    const result = tmp(6837).ensureNativeDetectorComponent(ReanimatedNativeDetector);
    const tmpResult = tmp(6837);
    const gestureRelationsUpdater = tmp(6838).useGestureRelationsUpdater(gesture);
    ...
    const detectorAttachmentGuard = tmp(6839).useDetectorAttachmentGuard(tmp8Result5);
    ...
    obj4.style = tmp(6777).nativeDetectorStyles.detector;
    ...
    return jsx(tmp(6841).InterceptingDetectorContext, obj3);
  } else {
    const _Error = Error;
    let error = new Error(tmp(6768).tagMessage("Gesture expects to run on the UI thread, but failed to create the Reanimated NativeDetector."));
    throw error;
  }
