...
// Function ID: 6847
// Name: eventHandler
// Dependencies: [6828, 6766, 6781, 6768, 6830]
// Exports: eventHandler

// Module 6846 (eventHandler)
import _mod6766 from "module_6766" /* 6766 */;
import TouchEventType from "TouchEventType" /* 6768 */;
...
function handleStateChangeEvent(result, arg1, lastUpdateEvent, fn) {
  ...
  if (oldState === _mod6766.State.UNDETERMINED) {
    if (state === tmp(6766).State.BEGAN) {
      tmp(6828).runCallback(tmp(6781).CALLBACK_TYPE.BEGAN, arg1, result);
      const tmpResult = tmp(6828);
    }
  }
  if (oldState === _mod6766.State.BEGAN) {
    if (state === tmp(6766).State.ACTIVE) {
      if (fn != null) {
        fn(result);
      }
      tmp(6828).runCallback(tmp(6781).CALLBACK_TYPE.START, arg1, result);
      const tmpResult4 = tmp(6828);
    }
  }
  if (oldState !== state) {
    const obj2 = {};
    const merged = Object.assign(result);
    obj2.canceled = state === tmp(6766).State.FAILED || state === tmp(6766).State.CANCELLED;
    if (oldState === tmp(6766).State.ACTIVE) {
      if (fn != null) {
        fn(obj2);
      }
      tmp(6828).runCallback(tmp(6781).CALLBACK_TYPE.END, arg1, obj2);
      const tmpResult5 = tmp(6828);
    }
    const tmp7 = state === tmp(6766).State.FAILED || state === tmp(6766).State.CANCELLED;
    tmp(6828).runCallback(tmp(6781).CALLBACK_TYPE.FINALIZE, arg1, obj2);
    if (lastUpdateEvent) {
      lastUpdateEvent.lastUpdateEvent = undefined;
    }
    const tmpResult6 = tmp(6828);
  }
}
handleStateChangeEvent.__closure = { flattenAndFilterEvent: fn(6828).flattenAndFilterEvent, State: fn(6766).State, runCallback: fn(6828).runCallback, CALLBACK_TYPE: fn(6781).CALLBACK_TYPE };
...
function handleUpdateEvent(lastUpdateEvent, arg1, fn, lastUpdateEvent2) {
  ...
  DEFAULT_PROPS_TRANSFORMER.runCallback(_mod6781.CALLBACK_TYPE.UPDATE, arg1, result);
  if (lastUpdateEvent2) {
    lastUpdateEvent2.lastUpdateEvent = lastUpdateEvent;
  }
}
let obj = { flattenAndFilterEvent: fn(6828).flattenAndFilterEvent, State: fn(6766).State, runCallback: fn(6828).runCallback, CALLBACK_TYPE: fn(6781).CALLBACK_TYPE };
handleUpdateEvent.__closure = { flattenAndFilterEvent: fn(6828).flattenAndFilterEvent, runCallback: fn(6828).runCallback, CALLBACK_TYPE: fn(6781).CALLBACK_TYPE };
...
function handleTouchEvent(eventType, arg1) {
  if (eventType.eventType !== TouchEventType.TouchEventType.UNDETERMINED) {
    const tmpResult = tmp(6828);
    tmpResult.runCallback(tmp(6828).touchEventTypeToCallbackType(eventType.eventType), arg1, eventType);
    const tmpResult2 = tmp(6828);
  }
}
let obj2 = { flattenAndFilterEvent: fn(6828).flattenAndFilterEvent, runCallback: fn(6828).runCallback, CALLBACK_TYPE: fn(6781).CALLBACK_TYPE };
handleTouchEvent.__closure = { TouchEventType: fn(6768).TouchEventType, runCallback: fn(6828).runCallback, touchEventTypeToCallbackType: fn(6828).touchEventTypeToCallbackType };
    ...
    if (tmpResult.isStateChangeEvent(result)) {
      ...
      if (tmpResult6.isTouchEvent(result)) {
        if (typeof handleTouchEvent === "function") {
          if (result.eventType !== tmp(6768).TouchEventType.UNDETERMINED) {
            const tmpResult7 = tmp(6828);
            tmpResult7.runCallback(tmp(6828).touchEventTypeToCallbackType(result.eventType), arg2, result);
            const tmpResult8 = tmp(6828);
          }
        } else {
        ...
        if (typeof handleUpdateEvent === "function") {
          ...
          const result1 = tmp(6828).flattenAndFilterEvent(tmp9);
          const tmpResult9 = tmp(6828);
          tmp(6828).runCallback(tmp(6781).CALLBACK_TYPE.UPDATE, arg2, result1);
          if (lastUpdateEvent) {
            lastUpdateEvent.lastUpdateEvent = result;
          }
          const tmpResult10 = tmp(6828);
        } else {
          throw new TypeError("Trying to call a non-function");
        }
      }
...
const obj3 = { TouchEventType: fn(6768).TouchEventType, runCallback: fn(6828).runCallback, touchEventTypeToCallbackType: fn(6828).touchEventTypeToCallbackType };
eventHandler.__closure = { maybeExtractNativeEvent: fn(6828).maybeExtractNativeEvent, isEventForHandlerWithTag: fn(6828).isEventForHandlerWithTag, isStateChangeEvent: fn(6830).isStateChangeEvent, handleStateChangeEvent, isTouchEvent: fn(6830).isTouchEvent, handleTouchEvent, handleUpdateEvent };
