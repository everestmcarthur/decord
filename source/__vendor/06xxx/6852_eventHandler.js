...
// Function ID: 6853
// Name: eventHandler
// Dependencies: [6834, 6772, 6787, 6774, 6836]
// Exports: eventHandler

// Module 6852 (eventHandler)
import _mod6772 from "module_6772" /* 6772 */;
import TouchEventType from "TouchEventType" /* 6774 */;
...
function handleStateChangeEvent(result, arg1, lastUpdateEvent, fn) {
  ...
  if (oldState === _mod6772.State.UNDETERMINED) {
    if (state === tmp(6772).State.BEGAN) {
      tmp(6834).runCallback(tmp(6787).CALLBACK_TYPE.BEGAN, arg1, result);
      const tmpResult = tmp(6834);
    }
  }
  if (oldState === _mod6772.State.BEGAN) {
    if (state === tmp(6772).State.ACTIVE) {
      if (fn != null) {
        fn(result);
      }
      tmp(6834).runCallback(tmp(6787).CALLBACK_TYPE.START, arg1, result);
      const tmpResult4 = tmp(6834);
    }
  }
  if (oldState !== state) {
    const obj2 = {};
    const merged = Object.assign(result);
    obj2.canceled = state === tmp(6772).State.FAILED || state === tmp(6772).State.CANCELLED;
    if (oldState === tmp(6772).State.ACTIVE) {
      if (fn != null) {
        fn(obj2);
      }
      tmp(6834).runCallback(tmp(6787).CALLBACK_TYPE.END, arg1, obj2);
      const tmpResult5 = tmp(6834);
    }
    const tmp7 = state === tmp(6772).State.FAILED || state === tmp(6772).State.CANCELLED;
    tmp(6834).runCallback(tmp(6787).CALLBACK_TYPE.FINALIZE, arg1, obj2);
    if (lastUpdateEvent) {
      lastUpdateEvent.lastUpdateEvent = undefined;
    }
    const tmpResult6 = tmp(6834);
  }
}
handleStateChangeEvent.__closure = { flattenAndFilterEvent: fn(6834).flattenAndFilterEvent, State: fn(6772).State, runCallback: fn(6834).runCallback, CALLBACK_TYPE: fn(6787).CALLBACK_TYPE };
...
function handleUpdateEvent(lastUpdateEvent, arg1, fn, lastUpdateEvent2) {
  ...
  DEFAULT_PROPS_TRANSFORMER.runCallback(_mod6787.CALLBACK_TYPE.UPDATE, arg1, result);
  if (lastUpdateEvent2) {
    lastUpdateEvent2.lastUpdateEvent = lastUpdateEvent;
  }
}
let obj = { flattenAndFilterEvent: fn(6834).flattenAndFilterEvent, State: fn(6772).State, runCallback: fn(6834).runCallback, CALLBACK_TYPE: fn(6787).CALLBACK_TYPE };
handleUpdateEvent.__closure = { flattenAndFilterEvent: fn(6834).flattenAndFilterEvent, runCallback: fn(6834).runCallback, CALLBACK_TYPE: fn(6787).CALLBACK_TYPE };
...
function handleTouchEvent(eventType, arg1) {
  if (eventType.eventType !== TouchEventType.TouchEventType.UNDETERMINED) {
    const tmpResult = tmp(6834);
    tmpResult.runCallback(tmp(6834).touchEventTypeToCallbackType(eventType.eventType), arg1, eventType);
    const tmpResult2 = tmp(6834);
  }
}
let obj2 = { flattenAndFilterEvent: fn(6834).flattenAndFilterEvent, runCallback: fn(6834).runCallback, CALLBACK_TYPE: fn(6787).CALLBACK_TYPE };
handleTouchEvent.__closure = { TouchEventType: fn(6774).TouchEventType, runCallback: fn(6834).runCallback, touchEventTypeToCallbackType: fn(6834).touchEventTypeToCallbackType };
    ...
    if (tmpResult.isStateChangeEvent(result)) {
      ...
      if (tmpResult6.isTouchEvent(result)) {
        if (typeof handleTouchEvent === "function") {
          if (result.eventType !== tmp(6774).TouchEventType.UNDETERMINED) {
            const tmpResult7 = tmp(6834);
            tmpResult7.runCallback(tmp(6834).touchEventTypeToCallbackType(result.eventType), arg2, result);
            const tmpResult8 = tmp(6834);
          }
        } else {
        ...
        if (typeof handleUpdateEvent === "function") {
          ...
          const result1 = tmp(6834).flattenAndFilterEvent(tmp9);
          const tmpResult9 = tmp(6834);
          tmp(6834).runCallback(tmp(6787).CALLBACK_TYPE.UPDATE, arg2, result1);
          if (lastUpdateEvent) {
            lastUpdateEvent.lastUpdateEvent = result;
          }
          const tmpResult10 = tmp(6834);
        } else {
          throw new TypeError("Trying to call a non-function");
        }
      }
...
const obj3 = { TouchEventType: fn(6774).TouchEventType, runCallback: fn(6834).runCallback, touchEventTypeToCallbackType: fn(6834).touchEventTypeToCallbackType };
eventHandler.__closure = { maybeExtractNativeEvent: fn(6834).maybeExtractNativeEvent, isEventForHandlerWithTag: fn(6834).isEventForHandlerWithTag, isStateChangeEvent: fn(6836).isStateChangeEvent, handleStateChangeEvent, isTouchEvent: fn(6836).isTouchEvent, handleTouchEvent, handleUpdateEvent };
