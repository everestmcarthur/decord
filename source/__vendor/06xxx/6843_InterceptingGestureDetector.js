...
// Dependencies: [32, 19, 17, 21, 6784, 6844, 6780, 6839, 6781, 6771, 6809, 6840, 6841, 6833, 6842]
// Exports: InterceptingGestureDetector

// Module 6843 (InterceptingGestureDetector)
import _mod6833 from "module_6833" /* 6833 */;
import _mod6844 from "module_6844" /* 6844 */;
...
export const InterceptingGestureDetector = function InterceptingGestureDetector(gesture) {
  ...
  gesture(6784).useEnsureGestureHandlerRootView();
  const tmp6 = first1(closure_7(() => new Set()), 2);
  const first = tmp6[0];
  dependencyMap = tmp6[1];
  let items = [first];
  let prop;
  let obj = gesture(6784);
  ...
  if (prop) {
    let REANIMATED = tmp(6844).InterceptingDetectorMode.REANIMATED;
  } else {
    let prop1;
    if (gesture != null) {
      prop1 = gesture.config.dispatchesAnimatedEvents;
    }
    const InterceptingDetectorMode = tmp(6844).InterceptingDetectorMode;
  ...
  const tmp14 = first1 === gesture(6844).InterceptingDetectorMode.REANIMATED;
  if (first1 === gesture(6844).InterceptingDetectorMode.ANIMATED) {
    let ReanimatedNativeDetector = tmp(6780).AnimatedNativeDetector;
  } else if (tmp14) {
    ReanimatedNativeDetector = tmp(6839).ReanimatedNativeDetector;
  } else {
    ReanimatedNativeDetector = first(6781);
  }
  ...
  register(() => {
    ...
    if (prop) {
      closure_7.setMode(_mod6844.InterceptingDetectorMode.ANIMATED);
    } else {
      let prop1;
      if (tmp != null) {
      ...
      if (prop1) {
        closure_7.setMode(_mod6844.InterceptingDetectorMode.REANIMATED);
      }
    }
  }, items2);
  if (ReanimatedNativeDetector) {
    ...
    const result = tmp(6840).ensureNativeDetectorComponent(ReanimatedNativeDetector);
    const tmpResult = tmp(6840);
    const gestureRelationsUpdater = tmp(6841).useGestureRelationsUpdater(gesture);
    ...
    const tmpResult4 = tmp(6841);
    const detectorAttachmentGuard = tmp(6842).useDetectorAttachmentGuard(tmp8Result5);
    ...
    obj4.style = tmp(6780).nativeDetectorStyles.detector;
    ...
    return jsx(tmp(6844).InterceptingDetectorContext, obj3);
  } else {
    const _Error = Error;
    let error = new Error(tmp(6771).tagMessage("Gesture expects to run on the UI thread, but failed to create the Reanimated NativeDetector."));
    throw error;
  }
