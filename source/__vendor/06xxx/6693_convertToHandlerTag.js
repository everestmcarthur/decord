...
// Dependencies: [32, 19, 6694, 6695, 6705, 6706, 6707, 6709, 6710, 6711, 6690, 6674, 6712, 6672]
// Exports: checkGestureCallbacksForWorklets, extractGestureRelations, useForceRender, useWebEventHandlers

// Module 6693 (convertToHandlerTag)
import _mod6672 from "module_6672" /* 6672 */;
...
function convertToHandlerTag(handlerTag) {
  if (typeof handlerTag === "number") {
    return handlerTag;
  } else if (handlerTag instanceof _mod6690.BaseGesture) {
...
let items = [...fn(6694).baseGestureHandlerWithDetectorProps, ...fn(6695).tapGestureHandlerProps, ...fn(6705).panGestureHandlerProps, ...fn(6705).panGestureHandlerCustomNativeProps, ...fn(6706).longPressGestureHandlerProps, ...fn(6707).forceTouchGestureHandlerProps, ...fn(6709).flingGestureHandlerProps, ...fn(6710).hoverGestureHandlerProps, ...fn(6711).nativeViewGestureHandlerProps];
...
export const useWebEventHandlers = function useWebEventHandlers() {
  return React4({
    onGestureHandlerEvent(nativeEvent) {
      const result = _mod6672.onGestureHandlerEvent(nativeEvent.nativeEvent);
    },
