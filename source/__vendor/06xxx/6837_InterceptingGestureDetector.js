...
// Dependencies: [32, 19, 17, 21, 6778, 6838, 6774, 6833, 6775, 6765, 6803, 6834, 6835, 6827, 6836]
// Exports: InterceptingGestureDetector

// Module 6837 (InterceptingGestureDetector)
import _mod6827 from "module_6827" /* 6827 */;
import _mod6838 from "module_6838" /* 6838 */;
...
export const InterceptingGestureDetector = function InterceptingGestureDetector(gesture) {
  ...
  gesture(6778).useEnsureGestureHandlerRootView();
  const tmp6 = first1(closure_7(() => new Set()), 2);
  const first = tmp6[0];
  dependencyMap = tmp6[1];
  let items = [first];
  let prop;
  let obj = gesture(6778);
  ...
  if (prop) {
    let REANIMATED = tmp(6838).InterceptingDetectorMode.REANIMATED;
  } else {
    let prop1;
    if (gesture != null) {
      prop1 = gesture.config.dispatchesAnimatedEvents;
    }
    const InterceptingDetectorMode = tmp(6838).InterceptingDetectorMode;
  ...
  const tmp14 = first1 === gesture(6838).InterceptingDetectorMode.REANIMATED;
  if (first1 === gesture(6838).InterceptingDetectorMode.ANIMATED) {
    let ReanimatedNativeDetector = tmp(6774).AnimatedNativeDetector;
  } else if (tmp14) {
    ReanimatedNativeDetector = tmp(6833).ReanimatedNativeDetector;
  } else {
    ReanimatedNativeDetector = first(6775);
  }
  ...
  register(() => {
    ...
    if (prop) {
      closure_7.setMode(_mod6838.InterceptingDetectorMode.ANIMATED);
    } else {
      let prop1;
      if (tmp != null) {
      ...
      if (prop1) {
        closure_7.setMode(_mod6838.InterceptingDetectorMode.REANIMATED);
      }
    }
  }, items2);
  if (ReanimatedNativeDetector) {
    ...
    const result = tmp(6834).ensureNativeDetectorComponent(ReanimatedNativeDetector);
    const tmpResult = tmp(6834);
    const gestureRelationsUpdater = tmp(6835).useGestureRelationsUpdater(gesture);
    ...
    const tmpResult4 = tmp(6835);
    const detectorAttachmentGuard = tmp(6836).useDetectorAttachmentGuard(tmp8Result5);
    ...
    obj4.style = tmp(6774).nativeDetectorStyles.detector;
    ...
    return jsx(tmp(6838).InterceptingDetectorContext, obj3);
  } else {
    const _Error = Error;
    let error = new Error(tmp(6765).tagMessage("Gesture expects to run on the UI thread, but failed to create the Reanimated NativeDetector."));
    throw error;
  }
