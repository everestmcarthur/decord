...
// Function ID: 6727
// Name: hash
// Dependencies: [32, 6713, 6712, 6727, 6736]
...
import allowedNativeProps from "allowedNativeProps" /* 6727 */;
...
let c4 = 1.618;
fn(6713).NativeProxy.updateGestureHandlerConfig;
...
maybeUnpackValue.__closure = { Reanimated: fn(6712).Reanimated };
...
function getEnabledSharedValues(Reanimated) {
  if (undefined === _mod6712.Reanimated) {
    return [];
  } else {
    if (tmpResult.isComposedGesture(Reanimated)) {
      ...
      Reanimated = tmp(6712).Reanimated;
...
export const bindSharedValues = function bindSharedValues(shouldUseReanimatedDetector, handlerTag) {
  _require = handlerTag;
  if (undefined !== require("module_6712").Reanimated) {
    ...
    while (tmp27 !== undefined) {
      ...
      let tmp8 = tmp7;
      let tmp10 = _require;
      let Reanimated = require("module_6712").Reanimated;
      if (Reanimated.isSharedValue(tmp7)) {
        let Reanimated2 = tmp10(6712).Reanimated;
    ...
    function _loop(maybeSharedValue) {
      ...
      if (Reanimated.isSharedValue(maybeSharedValue)) {
        if (typeof hash === "function") {
          ...
          closure_1 = sum;
          const Reanimated2 = _mod6712.Reanimated;
          const fn = function t() {
