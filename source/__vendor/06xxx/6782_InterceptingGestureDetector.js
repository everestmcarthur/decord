...
// Dependencies: [32, 19, 17, 21, 6723, 6783, 6719, 6778, 6720, 6710, 6748, 6779, 6780, 6772, 6781]
// Exports: InterceptingGestureDetector

// Module 6782 (InterceptingGestureDetector)
...
export const InterceptingGestureDetector = function InterceptingGestureDetector(gesture) {
  ...
  gesture(6723).useEnsureGestureHandlerRootView();
  const tmp6 = first1(closure_7(() => new Set()), 2);
  const first = tmp6[0];
  dependencyMap = tmp6[1];
  let items = [first];
  let prop;
  let obj = gesture(6723);
  ...
  if (prop) {
    let REANIMATED = tmp(6783).InterceptingDetectorMode.REANIMATED;
  } else {
    let prop1;
    if (gesture != null) {
      prop1 = gesture.config.dispatchesAnimatedEvents;
    }
    const InterceptingDetectorMode = tmp(6783).InterceptingDetectorMode;
  ...
  const tmp14 = first1 === gesture(6783).InterceptingDetectorMode.REANIMATED;
  if (first1 === gesture(6783).InterceptingDetectorMode.ANIMATED) {
    let ReanimatedNativeDetector = tmp(6719).AnimatedNativeDetector;
  } else if (tmp14) {
    ReanimatedNativeDetector = tmp(6778).ReanimatedNativeDetector;
  } else {
    ReanimatedNativeDetector = first(6720);
  }
  ...
  register(() => {
    ...
    if (prop) {
      closure_7.setMode(_mod6783.InterceptingDetectorMode.ANIMATED);
    } else {
      let prop1;
      if (tmp != null) {
      ...
      if (prop1) {
        closure_7.setMode(_mod6783.InterceptingDetectorMode.REANIMATED);
      }
    }
  }, items2);
  if (ReanimatedNativeDetector) {
    ...
    const result = tmp(6779).ensureNativeDetectorComponent(ReanimatedNativeDetector);
    const tmpResult = tmp(6779);
    const gestureRelationsUpdater = tmp(6780).useGestureRelationsUpdater(gesture);
    ...
    const tmpResult4 = tmp(6780);
    const detectorAttachmentGuard = tmp(6781).useDetectorAttachmentGuard(tmp8Result5);
    ...
    obj4.style = tmp(6719).nativeDetectorStyles.detector;
    ...
    return jsx(tmp(6783).InterceptingDetectorContext, obj3);
  } else {
    const _Error = Error;
    let error = new Error(tmp(6710).tagMessage("Gesture expects to run on the UI thread, but failed to create the Reanimated NativeDetector."));
    throw error;
  }
