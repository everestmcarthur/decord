...
// Function ID: 6992
// Name: JSFPSMonitor
// Dependencies: [6938, 6939, 6926, 6992]

// Module 6991 (JSFPSMonitor)
...
const items = [
  entry,
  {
    ...
    value: function startTracking() {
      ...
      if (0 !== this.startTime) {
        const _Error = Error;
        const error = new Error(JSFPSMonitor(6926).ErrorMessages.fpsMonitorAlreadyRunning);
        throw error;
      } else {
        const _Date = Date;
  ...
  },
  {
    ...
    value: function stopAndGetData() {
      ...
      const obj = { minFPS: JSFPSMonitor(6992).roundToDecimalPlaces(self.minFPS, 1), maxFPS: null, averageFPS: null };
      const obj2 = JSFPSMonitor(6992);
      obj.maxFPS = JSFPSMonitor(6992).roundToDecimalPlaces(self.maxFPS, 1);
      const obj3 = JSFPSMonitor(6992);
      obj.averageFPS = JSFPSMonitor(6992).roundToDecimalPlaces(self.averageFPS, 1);
      return obj;
    }
  }
];

export const JSFPSMonitor = _modDef6939(JSFPSMonitor, items);
