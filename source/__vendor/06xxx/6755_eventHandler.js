...
// Function ID: 6756
// Name: eventHandler
// Dependencies: [6737, 6675, 6690, 6677, 6739]
// Exports: eventHandler

// Module 6755 (eventHandler)
import _mod6675 from "module_6675" /* 6675 */;
import TouchEventType from "TouchEventType" /* 6677 */;
...
function handleStateChangeEvent(result, arg1, lastUpdateEvent, fn) {
  ...
  if (oldState === _mod6675.State.UNDETERMINED) {
    if (state === tmp(6675).State.BEGAN) {
      tmp(6737).runCallback(tmp(6690).CALLBACK_TYPE.BEGAN, arg1, result);
      const tmpResult = tmp(6737);
    }
  }
  if (oldState === _mod6675.State.BEGAN) {
    if (state === tmp(6675).State.ACTIVE) {
      if (fn != null) {
        fn(result);
      }
      tmp(6737).runCallback(tmp(6690).CALLBACK_TYPE.START, arg1, result);
      const tmpResult4 = tmp(6737);
    }
  }
  if (oldState !== state) {
    const obj2 = {};
    const merged = Object.assign(result);
    obj2.canceled = state === tmp(6675).State.FAILED || state === tmp(6675).State.CANCELLED;
    if (oldState === tmp(6675).State.ACTIVE) {
      if (fn != null) {
        fn(obj2);
      }
      tmp(6737).runCallback(tmp(6690).CALLBACK_TYPE.END, arg1, obj2);
      const tmpResult5 = tmp(6737);
    }
    const tmp7 = state === tmp(6675).State.FAILED || state === tmp(6675).State.CANCELLED;
    tmp(6737).runCallback(tmp(6690).CALLBACK_TYPE.FINALIZE, arg1, obj2);
    if (lastUpdateEvent) {
      lastUpdateEvent.lastUpdateEvent = undefined;
    }
    const tmpResult6 = tmp(6737);
  }
}
handleStateChangeEvent.__closure = { flattenAndFilterEvent: fn(6737).flattenAndFilterEvent, State: fn(6675).State, runCallback: fn(6737).runCallback, CALLBACK_TYPE: fn(6690).CALLBACK_TYPE };
...
function handleUpdateEvent(lastUpdateEvent, arg1, fn, lastUpdateEvent2) {
  ...
  DEFAULT_PROPS_TRANSFORMER.runCallback(_mod6690.CALLBACK_TYPE.UPDATE, arg1, result);
  if (lastUpdateEvent2) {
    lastUpdateEvent2.lastUpdateEvent = lastUpdateEvent;
  }
}
let obj = { flattenAndFilterEvent: fn(6737).flattenAndFilterEvent, State: fn(6675).State, runCallback: fn(6737).runCallback, CALLBACK_TYPE: fn(6690).CALLBACK_TYPE };
handleUpdateEvent.__closure = { flattenAndFilterEvent: fn(6737).flattenAndFilterEvent, runCallback: fn(6737).runCallback, CALLBACK_TYPE: fn(6690).CALLBACK_TYPE };
...
function handleTouchEvent(eventType, arg1) {
  if (eventType.eventType !== TouchEventType.TouchEventType.UNDETERMINED) {
    const tmpResult = tmp(6737);
    tmpResult.runCallback(tmp(6737).touchEventTypeToCallbackType(eventType.eventType), arg1, eventType);
    const tmpResult2 = tmp(6737);
  }
}
let obj2 = { flattenAndFilterEvent: fn(6737).flattenAndFilterEvent, runCallback: fn(6737).runCallback, CALLBACK_TYPE: fn(6690).CALLBACK_TYPE };
handleTouchEvent.__closure = { TouchEventType: fn(6677).TouchEventType, runCallback: fn(6737).runCallback, touchEventTypeToCallbackType: fn(6737).touchEventTypeToCallbackType };
    ...
    if (tmpResult.isStateChangeEvent(result)) {
      ...
      if (tmpResult6.isTouchEvent(result)) {
        if (typeof handleTouchEvent === "function") {
          if (result.eventType !== tmp(6677).TouchEventType.UNDETERMINED) {
            const tmpResult7 = tmp(6737);
            tmpResult7.runCallback(tmp(6737).touchEventTypeToCallbackType(result.eventType), arg2, result);
            const tmpResult8 = tmp(6737);
          }
        } else {
        ...
        if (typeof handleUpdateEvent === "function") {
          ...
          const result1 = tmp(6737).flattenAndFilterEvent(tmp9);
          const tmpResult9 = tmp(6737);
          tmp(6737).runCallback(tmp(6690).CALLBACK_TYPE.UPDATE, arg2, result1);
          if (lastUpdateEvent) {
            lastUpdateEvent.lastUpdateEvent = result;
          }
          const tmpResult10 = tmp(6737);
        } else {
          throw new TypeError("Trying to call a non-function");
        }
      }
...
const obj3 = { TouchEventType: fn(6677).TouchEventType, runCallback: fn(6737).runCallback, touchEventTypeToCallbackType: fn(6737).touchEventTypeToCallbackType };
eventHandler.__closure = { maybeExtractNativeEvent: fn(6737).maybeExtractNativeEvent, isEventForHandlerWithTag: fn(6737).isEventForHandlerWithTag, isStateChangeEvent: fn(6739).isStateChangeEvent, handleStateChangeEvent, isTouchEvent: fn(6739).isTouchEvent, handleTouchEvent, handleUpdateEvent };
