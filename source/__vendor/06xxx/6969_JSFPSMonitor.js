...
// Function ID: 6970
// Name: JSFPSMonitor
// Dependencies: [6916, 6917, 6904, 6970]

// Module 6969 (JSFPSMonitor)
...
const items = [
  entry,
  {
    ...
    value: function startTracking() {
      ...
      if (0 !== this.startTime) {
        const _Error = Error;
        const error = new Error(JSFPSMonitor(6904).ErrorMessages.fpsMonitorAlreadyRunning);
        throw error;
      } else {
        const _Date = Date;
  ...
  },
  {
    ...
    value: function stopAndGetData() {
      ...
      const obj = { minFPS: JSFPSMonitor(6970).roundToDecimalPlaces(self.minFPS, 1), maxFPS: null, averageFPS: null };
      const obj2 = JSFPSMonitor(6970);
      obj.maxFPS = JSFPSMonitor(6970).roundToDecimalPlaces(self.maxFPS, 1);
      const obj3 = JSFPSMonitor(6970);
      obj.averageFPS = JSFPSMonitor(6970).roundToDecimalPlaces(self.averageFPS, 1);
      return obj;
    }
  }
];

export const JSFPSMonitor = _modDef6917(JSFPSMonitor, items);
