...
// Dependencies: [32, 19, 17, 21, 6687, 6747, 6683, 6742, 6684, 6674, 6712, 6743, 6744, 6736, 6745]
// Exports: InterceptingGestureDetector

// Module 6746 (InterceptingGestureDetector)
import _mod6736 from "module_6736" /* 6736 */;
...
export const InterceptingGestureDetector = function InterceptingGestureDetector(gesture) {
  ...
  gesture(6687).useEnsureGestureHandlerRootView();
  const tmp6 = first1(closure_7(() => new Set()), 2);
  const first = tmp6[0];
  dependencyMap = tmp6[1];
  let items = [first];
  let prop;
  let obj = gesture(6687);
  ...
  if (prop) {
    let REANIMATED = tmp(6747).InterceptingDetectorMode.REANIMATED;
  } else {
    let prop1;
    if (gesture != null) {
      prop1 = gesture.config.dispatchesAnimatedEvents;
    }
    const InterceptingDetectorMode = tmp(6747).InterceptingDetectorMode;
  ...
  const tmp14 = first1 === gesture(6747).InterceptingDetectorMode.REANIMATED;
  if (first1 === gesture(6747).InterceptingDetectorMode.ANIMATED) {
    let ReanimatedNativeDetector = tmp(6683).AnimatedNativeDetector;
  } else if (tmp14) {
    ReanimatedNativeDetector = tmp(6742).ReanimatedNativeDetector;
  } else {
    ReanimatedNativeDetector = first(6684);
  }
  ...
  register(() => {
    ...
    if (prop) {
      closure_7.setMode(_mod6747.InterceptingDetectorMode.ANIMATED);
    } else {
      let prop1;
      if (tmp != null) {
      ...
      if (prop1) {
        closure_7.setMode(_mod6747.InterceptingDetectorMode.REANIMATED);
      }
    }
  }, items2);
  if (ReanimatedNativeDetector) {
    ...
    const result = tmp(6743).ensureNativeDetectorComponent(ReanimatedNativeDetector);
    const tmpResult = tmp(6743);
    const gestureRelationsUpdater = tmp(6744).useGestureRelationsUpdater(gesture);
    ...
    const tmpResult4 = tmp(6744);
    const detectorAttachmentGuard = tmp(6745).useDetectorAttachmentGuard(tmp8Result5);
    ...
    obj4.style = tmp(6683).nativeDetectorStyles.detector;
    ...
    return jsx(tmp(6747).InterceptingDetectorContext, obj3);
  } else {
    const _Error = Error;
    let error = new Error(tmp(6674).tagMessage("Gesture expects to run on the UI thread, but failed to create the Reanimated NativeDetector."));
    throw error;
  }
