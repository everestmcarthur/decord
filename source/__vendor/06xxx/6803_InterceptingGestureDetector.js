...
// Dependencies: [32, 19, 17, 21, 6744, 6804, 6740, 6799, 6741, 6731, 6769, 6800, 6801, 6793, 6802]
// Exports: InterceptingGestureDetector

// Module 6803 (InterceptingGestureDetector)
...
export const InterceptingGestureDetector = function InterceptingGestureDetector(gesture) {
  ...
  gesture(6744).useEnsureGestureHandlerRootView();
  const tmp6 = first1(closure_7(() => new Set()), 2);
  const first = tmp6[0];
  dependencyMap = tmp6[1];
  let items = [first];
  let prop;
  let obj = gesture(6744);
  ...
  if (prop) {
    let REANIMATED = tmp(6804).InterceptingDetectorMode.REANIMATED;
  } else {
    let prop1;
    if (gesture != null) {
      prop1 = gesture.config.dispatchesAnimatedEvents;
    }
    const InterceptingDetectorMode = tmp(6804).InterceptingDetectorMode;
  ...
  const tmp14 = first1 === gesture(6804).InterceptingDetectorMode.REANIMATED;
  if (first1 === gesture(6804).InterceptingDetectorMode.ANIMATED) {
    let ReanimatedNativeDetector = tmp(6740).AnimatedNativeDetector;
  } else if (tmp14) {
    ReanimatedNativeDetector = tmp(6799).ReanimatedNativeDetector;
  } else {
    ReanimatedNativeDetector = first(6741);
  }
  ...
  register(() => {
    ...
    if (prop) {
      closure_7.setMode(_mod6804.InterceptingDetectorMode.ANIMATED);
    } else {
      let prop1;
      if (tmp != null) {
      ...
      if (prop1) {
        closure_7.setMode(_mod6804.InterceptingDetectorMode.REANIMATED);
      }
    }
  }, items2);
  if (ReanimatedNativeDetector) {
    ...
    const result = tmp(6800).ensureNativeDetectorComponent(ReanimatedNativeDetector);
    const tmpResult = tmp(6800);
    const gestureRelationsUpdater = tmp(6801).useGestureRelationsUpdater(gesture);
    ...
    const tmpResult4 = tmp(6801);
    const detectorAttachmentGuard = tmp(6802).useDetectorAttachmentGuard(tmp8Result5);
    ...
    obj4.style = tmp(6740).nativeDetectorStyles.detector;
    ...
    return jsx(tmp(6804).InterceptingDetectorContext, obj3);
  } else {
    const _Error = Error;
    let error = new Error(tmp(6731).tagMessage("Gesture expects to run on the UI thread, but failed to create the Reanimated NativeDetector."));
    throw error;
  }
