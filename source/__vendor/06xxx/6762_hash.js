...
// Function ID: 6763
// Name: hash
// Dependencies: [32, 6749, 6748, 6763, 6772]
...
import allowedNativeProps from "allowedNativeProps" /* 6763 */;
...
let c4 = 1.618;
fn(6749).NativeProxy.updateGestureHandlerConfig;
...
maybeUnpackValue.__closure = { Reanimated: fn(6748).Reanimated };
...
function getEnabledSharedValues(Reanimated) {
  if (undefined === _mod6748.Reanimated) {
    return [];
  } else {
    if (tmpResult.isComposedGesture(Reanimated)) {
      ...
      Reanimated = tmp(6748).Reanimated;
...
export const bindSharedValues = function bindSharedValues(shouldUseReanimatedDetector, handlerTag) {
  _require = handlerTag;
  if (undefined !== require("module_6748").Reanimated) {
    ...
    while (tmp27 !== undefined) {
      ...
      if (Reanimated.isSharedValue(tmp7)) {
        let Reanimated2 = tmp10(6748).Reanimated;
    ...
    function _loop(maybeSharedValue) {
      ...
      if (Reanimated.isSharedValue(maybeSharedValue)) {
        if (typeof hash === "function") {
          ...
          closure_1 = sum;
          const Reanimated2 = _mod6748.Reanimated;
          const fn = function t() {
