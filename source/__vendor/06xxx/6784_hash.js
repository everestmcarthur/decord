...
// Function ID: 6785
// Name: hash
// Dependencies: [32, 6771, 6770, 6785, 6794]
...
import allowedNativeProps from "allowedNativeProps" /* 6785 */;
...
let c4 = 1.618;
fn(6771).NativeProxy.updateGestureHandlerConfig;
...
function maybeUnpackValue(enabled) {
  const Reanimated = _mod6770.Reanimated;
...
maybeUnpackValue.__closure = { Reanimated: fn(6770).Reanimated };
...
function getEnabledSharedValues(Reanimated) {
  if (undefined === _mod6770.Reanimated) {
    return [];
  } else {
    if (tmpResult.isComposedGesture(Reanimated)) {
      ...
      Reanimated = tmp(6770).Reanimated;
...
export const bindSharedValues = function bindSharedValues(shouldUseReanimatedDetector, handlerTag) {
  _require = handlerTag;
  if (undefined !== require("module_6770").Reanimated) {
    ...
    while (tmp27 !== undefined) {
      ...
      let tmp8 = tmp7;
      let tmp10 = _require;
      let Reanimated = require("module_6770").Reanimated;
      if (Reanimated.isSharedValue(tmp7)) {
        let Reanimated2 = tmp10(6770).Reanimated;
    ...
    function _loop(maybeSharedValue) {
      ...
      if (Reanimated.isSharedValue(maybeSharedValue)) {
        if (typeof hash === "function") {
          ...
          closure_1 = sum;
          const Reanimated2 = _mod6770.Reanimated;
          const fn = function t() {
