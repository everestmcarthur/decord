...
// Dependencies: [32, 19, 17, 21, 6745, 6805, 6741, 6800, 6742, 6732, 6770, 6801, 6802, 6794, 6803]
// Exports: InterceptingGestureDetector

// Module 6804 (InterceptingGestureDetector)
...
export const InterceptingGestureDetector = function InterceptingGestureDetector(gesture) {
  ...
  gesture(6745).useEnsureGestureHandlerRootView();
  const tmp6 = first1(closure_7(() => new Set()), 2);
  const first = tmp6[0];
  dependencyMap = tmp6[1];
  let items = [first];
  let prop;
  let obj = gesture(6745);
  ...
  if (prop) {
    let REANIMATED = tmp(6805).InterceptingDetectorMode.REANIMATED;
  } else {
    let prop1;
    if (gesture != null) {
      prop1 = gesture.config.dispatchesAnimatedEvents;
    }
    const InterceptingDetectorMode = tmp(6805).InterceptingDetectorMode;
  ...
  const tmp14 = first1 === gesture(6805).InterceptingDetectorMode.REANIMATED;
  if (first1 === gesture(6805).InterceptingDetectorMode.ANIMATED) {
    let ReanimatedNativeDetector = tmp(6741).AnimatedNativeDetector;
  } else if (tmp14) {
    ReanimatedNativeDetector = tmp(6800).ReanimatedNativeDetector;
  } else {
    ReanimatedNativeDetector = first(6742);
  }
  ...
  register(() => {
    ...
    if (prop) {
      closure_7.setMode(_mod6805.InterceptingDetectorMode.ANIMATED);
    } else {
      let prop1;
      if (tmp != null) {
      ...
      if (prop1) {
        closure_7.setMode(_mod6805.InterceptingDetectorMode.REANIMATED);
      }
    }
  }, items2);
  if (ReanimatedNativeDetector) {
    ...
    const result = tmp(6801).ensureNativeDetectorComponent(ReanimatedNativeDetector);
    const tmpResult = tmp(6801);
    const gestureRelationsUpdater = tmp(6802).useGestureRelationsUpdater(gesture);
    ...
    const tmpResult4 = tmp(6802);
    const detectorAttachmentGuard = tmp(6803).useDetectorAttachmentGuard(tmp8Result5);
    ...
    obj4.style = tmp(6741).nativeDetectorStyles.detector;
    ...
    return jsx(tmp(6805).InterceptingDetectorContext, obj3);
  } else {
    const _Error = Error;
    let error = new Error(tmp(6732).tagMessage("Gesture expects to run on the UI thread, but failed to create the Reanimated NativeDetector."));
    throw error;
  }
