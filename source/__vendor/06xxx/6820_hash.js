...
// Function ID: 6821
// Name: hash
// Dependencies: [32, 6807, 6806, 6821, 6830]
...
import allowedNativeProps from "allowedNativeProps" /* 6821 */;
...
let c4 = 1.618;
fn(6807).NativeProxy.updateGestureHandlerConfig;
...
maybeUnpackValue.__closure = { Reanimated: fn(6806).Reanimated };
...
function getEnabledSharedValues(Reanimated) {
  if (undefined === _mod6806.Reanimated) {
    return [];
  } else {
    if (tmpResult.isComposedGesture(Reanimated)) {
      ...
      Reanimated = tmp(6806).Reanimated;
...
export const bindSharedValues = function bindSharedValues(shouldUseReanimatedDetector, handlerTag) {
  _require = handlerTag;
  if (undefined !== require("module_6806").Reanimated) {
    ...
    while (tmp27 !== undefined) {
      ...
      let tmp8 = tmp7;
      let tmp10 = _require;
      let Reanimated = require("module_6806").Reanimated;
      if (Reanimated.isSharedValue(tmp7)) {
        let Reanimated2 = tmp10(6806).Reanimated;
    ...
    function _loop(maybeSharedValue) {
      ...
      if (Reanimated.isSharedValue(maybeSharedValue)) {
        if (typeof hash === "function") {
          ...
          closure_1 = sum;
          const Reanimated2 = _mod6806.Reanimated;
          const fn = function t() {
