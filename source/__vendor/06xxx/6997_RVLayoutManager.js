...
// Dependencies: [6971, 6972, 6976, 6998, 6985, 6959, 6999]

// Module 6997 (RVLayoutManager)
...
let items = [
  ...
  },
  {
    ...
    value: function getVisibleLayouts(bound1, arg1) {
      const result = RVLayoutManager(6998).findFirstVisibleIndex(this.layouts, bound1, this.horizontal);
      const obj = RVLayoutManager(6998);
      const findLastVisibleIndexResult = RVLayoutManager(6998).findLastVisibleIndex(this.layouts, arg1, this.horizontal);
      if (-1 !== result) {
        if (-1 !== findLastVisibleIndexResult) {
          let EMPTY = new tmp(6985).ConsecutiveNumbers(result, findLastVisibleIndexResult);
        }
        return EMPTY;
      }
      EMPTY = tmp(6985).ConsecutiveNumbers.EMPTY;
    }
  },
  {
    key: "deleteLayout",
  ...
  },
  {
    ...
    value: function getLayout(arg0) {
      ...
      if (arg0 >= this.layouts.length) {
        const _Error = Error;
        const error = new Error(RVLayoutManager(6959).ErrorMessages.indexOutOfBounds);
        throw error;
      } else {
...
export const RVLayoutManager = _modDef6972(RVLayoutManager, items);
