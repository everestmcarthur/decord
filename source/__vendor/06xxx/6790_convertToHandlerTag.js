...
// Dependencies: [32, 19, 6791, 6792, 6802, 6803, 6804, 6806, 6807, 6808, 6787, 6771, 6809, 6769]
// Exports: checkGestureCallbacksForWorklets, extractGestureRelations, useForceRender, useWebEventHandlers

// Module 6790 (convertToHandlerTag)
...
function convertToHandlerTag(handlerTag) {
  if (typeof handlerTag === "number") {
    return handlerTag;
  } else if (handlerTag instanceof _mod6787.BaseGesture) {
...
let items = [...fn(6791).baseGestureHandlerWithDetectorProps, ...fn(6792).tapGestureHandlerProps, ...fn(6802).panGestureHandlerProps, ...fn(6802).panGestureHandlerCustomNativeProps, ...fn(6803).longPressGestureHandlerProps, ...fn(6804).forceTouchGestureHandlerProps, ...fn(6806).flingGestureHandlerProps, ...fn(6807).hoverGestureHandlerProps, ...fn(6808).nativeViewGestureHandlerProps];
...
export const useWebEventHandlers = function useWebEventHandlers() {
  return React4({
    onGestureHandlerEvent(nativeEvent) {
      const result = _mod6769.onGestureHandlerEvent(nativeEvent.nativeEvent);
    },
