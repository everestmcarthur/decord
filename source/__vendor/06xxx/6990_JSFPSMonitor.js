...
// Function ID: 6991
// Name: JSFPSMonitor
// Dependencies: [6937, 6938, 6925, 6991]

// Module 6990 (JSFPSMonitor)
...
const items = [
  entry,
  {
    ...
    value: function startTracking() {
      ...
      if (0 !== this.startTime) {
        const _Error = Error;
        const error = new Error(JSFPSMonitor(6925).ErrorMessages.fpsMonitorAlreadyRunning);
        throw error;
      } else {
        const _Date = Date;
  ...
  },
  {
    ...
    value: function stopAndGetData() {
      ...
      const obj = { minFPS: JSFPSMonitor(6991).roundToDecimalPlaces(self.minFPS, 1), maxFPS: null, averageFPS: null };
      const obj2 = JSFPSMonitor(6991);
      obj.maxFPS = JSFPSMonitor(6991).roundToDecimalPlaces(self.maxFPS, 1);
      const obj3 = JSFPSMonitor(6991);
      obj.averageFPS = JSFPSMonitor(6991).roundToDecimalPlaces(self.averageFPS, 1);
      return obj;
    }
  }
];

export const JSFPSMonitor = _modDef6938(JSFPSMonitor, items);
