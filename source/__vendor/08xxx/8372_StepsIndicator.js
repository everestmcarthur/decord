...
// Function ID: 8373
// Name: StepsIndicator
// Dependencies: [19, 17, 21, 8370, 8371, 8373, 8374]
// Exports: StepsIndicator

// Module 8372 (StepsIndicator)
import _mod8370 from "module_8370" /* 8370 */;
import _mod8371 from "module_8371" /* 8371 */;
import SliderTrackMark from "SliderTrackMark" /* 8373 */;
import StepNumber from "StepNumber" /* 8374 */;
...
export const StepsIndicator = function StepsIndicator(options) {
  ...
  const memo = currentValue.useMemo(() => {
    if (options.length > 9) {
      let fontSize = _mod8370.constants.STEP_NUMBER_TEXT_FONT_SMALL;
    } else {
      fontSize = _mod8370.constants.STEP_NUMBER_TEXT_FONT_BIG;
    }
    return { fontSize };
  }, items);
  ...
  const memo1 = currentValue.useMemo(() => {
    if ("web" === get_ActivityIndicator.Platform.OS) {
      let stepsIndicator = _mod8371.styles.stepsIndicator;
      let tmp6 = require;
    } else {
      const _Object = Object;
      const obj = { marginHorizontal: sliderWidth * _mod8370.constants.MARGIN_HORIZONTAL_PADDING };
      stepsIndicator = Object.assign({}, _mod8371.styles.stepsIndicator, obj);
      tmp6 = require;
    }
    const obj2 = { stepIndicatorContainerStyle: stepsIndicator, stepIndicatorElementStyle: null };
    if ("web" === get_ActivityIndicator.Platform.OS) {
      const _Object2 = Object;
      const obj3 = { width: tmp6(8370).constants.THUMB_SIZE, justifyContent: "space-between" };
      let stepIndicatorElement = Object.assign({}, tmp6(8371).styles.stepIndicatorElement, obj3);
    } else {
      stepIndicatorElement = tmp6(8371).styles.stepIndicatorElement;
    }
