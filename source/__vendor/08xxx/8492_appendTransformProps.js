...
// Dependencies: [8493, 8494, 8495]
// Exports: default, extractTransformSvgView

// Module 8492 (appendTransformProps)
import append from "append" /* 8493 */;
import peg$SyntaxError from "peg$SyntaxError" /* 8494 */;
import _mod8495 from "module_8495" /* 8495 */;
...
function transformToMatrix(arg0, arr) {
  ...
  if (arr) {
    ...
    if (Array.isArray(arr)) {
      ...
      try {
        const parsed = tmp3(8494).parse(arr);
        const tmp3Result5 = tmp3(8493);
        tmp3Result5.append(parsed[0], parsed[3], parsed[1], parsed[4], parsed[2], parsed[5]);
        const tmp3Result4 = tmp3(8494);
      } catch (tmp14) {
...
export const extractTransformSvgView = function extractTransformSvgView(transform) {
  if (typeof transform.transform === "string") {
    transform = _mod8495.parse(transform.transform);
  } else {
    transform = transform.transform;
  }
  return transform;
