...
// Dependencies: [8553, 8554, 8555]
// Exports: default, extractTransformSvgView

// Module 8552 (appendTransformProps)
import append from "append" /* 8553 */;
import peg$SyntaxError from "peg$SyntaxError" /* 8554 */;
import _mod8555 from "module_8555" /* 8555 */;
...
function transformToMatrix(arg0, arr) {
  ...
  if (arr) {
    ...
    if (Array.isArray(arr)) {
      ...
      try {
        const parsed = tmp3(8554).parse(arr);
        const tmp3Result5 = tmp3(8553);
        tmp3Result5.append(parsed[0], parsed[3], parsed[1], parsed[4], parsed[2], parsed[5]);
        const tmp3Result4 = tmp3(8554);
      } catch (tmp14) {
...
export const extractTransformSvgView = function extractTransformSvgView(transform) {
  if (typeof transform.transform === "string") {
    transform = _mod8555.parse(transform.transform);
  } else {
    transform = transform.transform;
  }
  return transform;
