...
// Function ID: 8406
// Name: StepsIndicator
// Dependencies: [19, 17, 21, 8403, 8404, 8406, 8407]
// Exports: StepsIndicator

// Module 8405 (StepsIndicator)
import _mod8403 from "module_8403" /* 8403 */;
import _mod8404 from "module_8404" /* 8404 */;
import SliderTrackMark from "SliderTrackMark" /* 8406 */;
import StepNumber from "StepNumber" /* 8407 */;
...
export const StepsIndicator = function StepsIndicator(options) {
  ...
  const memo = currentValue.useMemo(() => {
    if (options.length > 9) {
      let fontSize = _mod8403.constants.STEP_NUMBER_TEXT_FONT_SMALL;
    } else {
      fontSize = _mod8403.constants.STEP_NUMBER_TEXT_FONT_BIG;
    }
    return { fontSize };
  }, items);
  ...
  const memo1 = currentValue.useMemo(() => {
    if ("web" === get_ActivityIndicator.Platform.OS) {
      let stepsIndicator = _mod8404.styles.stepsIndicator;
      let tmp6 = require;
    } else {
      const _Object = Object;
      const obj = { marginHorizontal: sliderWidth * _mod8403.constants.MARGIN_HORIZONTAL_PADDING };
      stepsIndicator = Object.assign({}, _mod8404.styles.stepsIndicator, obj);
      tmp6 = require;
    }
    const obj2 = { stepIndicatorContainerStyle: stepsIndicator, stepIndicatorElementStyle: null };
    if ("web" === get_ActivityIndicator.Platform.OS) {
      const _Object2 = Object;
      const obj3 = { width: tmp6(8403).constants.THUMB_SIZE, justifyContent: "space-between" };
      let stepIndicatorElement = Object.assign({}, tmp6(8404).styles.stepIndicatorElement, obj3);
    } else {
      stepIndicatorElement = tmp6(8404).styles.stepIndicatorElement;
    }
