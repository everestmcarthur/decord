...
// Dependencies: [6974, 6975, 6979, 7001, 6988, 6962, 7002]

// Module 7000 (RVLayoutManager)
...
let items = [
  ...
  },
  {
    ...
    value: function getVisibleLayouts(bound1, arg1) {
      const result = RVLayoutManager(7001).findFirstVisibleIndex(this.layouts, bound1, this.horizontal);
      const obj = RVLayoutManager(7001);
      const findLastVisibleIndexResult = RVLayoutManager(7001).findLastVisibleIndex(this.layouts, arg1, this.horizontal);
      if (-1 !== result) {
        if (-1 !== findLastVisibleIndexResult) {
          let EMPTY = new tmp(6988).ConsecutiveNumbers(result, findLastVisibleIndexResult);
        }
        return EMPTY;
      }
      EMPTY = tmp(6988).ConsecutiveNumbers.EMPTY;
    }
  },
  {
    key: "deleteLayout",
  ...
  },
  {
    ...
    value: function getLayout(arg0) {
      ...
      if (arg0 >= this.layouts.length) {
        const _Error = Error;
        const error = new Error(RVLayoutManager(6962).ErrorMessages.indexOutOfBounds);
        throw error;
      } else {
...
export const RVLayoutManager = _modDef6975(RVLayoutManager, items);
