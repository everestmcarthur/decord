...
// Dependencies: [32, 109, 17, 21, 5654, 1484, 7932, 5645, 4926]
// Exports: useHeaderConfigProps

// Module 7931 (processBarButtonItems)
import ColorDefault from "Color" /* 5654 */;
...
export const useHeaderConfigProps = function useHeaderConfigProps(headerLargeTitle) {
  ...
  const tmpResult = colors(7932);
  let fontSize;
  [tmp10, tmp11, tmp12] = colors(7932).processFonts(items3);
  if ("fontSize" in tmp4) {
    fontSize = tmp4.fontSize;
  }
  const tmp9 = _slicedToArray(colors(7932).processFonts(items3), 3);
  const headerTitle1 = colors(5645).getHeaderTitle({ title, headerTitle }, route.name);
  if ("color" in tmp6) {
  ...
  const isSearchBarAvailableForCurrentPlatform = tmp(4926).isSearchBarAvailableForCurrentPlatform;
  ...
  if (typeof isSearchBarAvailableForCurrentPlatform === "boolean") {
    isSearchBarAvailableForCurrentPlatform2 = tmp(4926).isSearchBarAvailableForCurrentPlatform;
  }
  ...
  if (null != headerLeftResult) {
    ...
    if ("center" === headerTitleAlign) {
      ...
      let tmp32Result = tmp32(tmp(4926).ScreenStackHeaderLeftView, obj11);
    } else {
      if (typeof headerTitle === "function") {
        ...
        obj13.children = tmp62(tmp(5645).HeaderTitle, obj14);
      }
  ...
  if ("center" === headerTitleAlign) {
    if (typeof headerTitle !== "function") {
      ...
      headerTitleResult = tmp38(tmp(5645).HeaderTitle, obj15);
    }
    const obj16 = { children: headerTitleResult };
    tmp38Result = tmp38(tmp(4926).ScreenStackHeaderCenterView, obj16);
  }
  ...
  if (undefined !== headerBackIcon) {
    ...
    const obj17 = { source };
    let tmp40Result = closure_10(tmp(4926).ScreenStackHeaderBackButtonImage, obj17);
  } else {
    tmp40Result = null;
  }
  ...
  if (null != headerRightResult) {
    ...
    tmp42 = closure_10(tmp(4926).ScreenStackHeaderRightView, obj18);
  }
  items7[2] = tmp42;
  let tmp44 = null;
  if (isSearchBarAvailableForCurrentPlatform2) {
    ...
    obj19.children = closure_10(tmp(4926).SearchBar, obj20);
    tmp44 = closure_10(tmp(4926).ScreenStackHeaderSearchBarView, obj19);
  }
  items7[3] = tmp44;
  const obj21 = { backButtonInCustomView: tmp24, backgroundColor, backTitle: headerBackTitle, backTitleVisible: "minimal" !== headerBackButtonDisplayMode, backButtonDisplayMode: "fill", backTitleFontFamily: tmp10, backTitleFontSize: fontSize, blurEffect: headerBlurEffect, color: text, direction: obj.useLocale().direction, disableBackButtonMenu: false === headerBackButtonMenuEnabled, hidden: false === headerShown, hideBackButton: false === headerBackVisible, hideShadow: true, largeTitle: true, largeTitleBackgroundColor: true, largeTitleColor: true, largeTitleFontFamily: true, largeTitleFontSize: true, largeTitleFontWeight: true, largeTitleHideShadow: "gpt-5.6-luna", title: "GPT-5.6 Luna", titleColor: "openai", titleFontFamily: true, titleFontSize: "/assets/.cache/intl/ZGVzaWdu", titleFontWeight: null, topInsetEnabled: "86b91484df1ac0b8ccad956aae12b775", disableTopInsetApplication: "bg.messages.86b91484df1ac0b8ccad956aae12b775.compiled.messages", disableLeftInsetApplication: "jsona", disableRightInsetApplication: "gpt-6-astra", disableBottomInsetApplication: "GPT-6 Astra", translucent: "openai", children: true, headerLeftBarButtonItems: "/assets/.cache/intl/ZGVzaWdu", headerRightBarButtonItems: null, experimental_userInterfaceStyle: "72a624908b343db34ae78e6d520bec11" };
  let tmp50 = false === headerShadowVisible;
  const tmpResult2 = colors(5645);
  if (!tmp50) {
