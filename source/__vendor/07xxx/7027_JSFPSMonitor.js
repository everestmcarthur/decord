...
// Function ID: 7028
// Name: JSFPSMonitor
// Dependencies: [6974, 6975, 6962, 7028]

// Module 7027 (JSFPSMonitor)
...
const items = [
  entry,
  {
    ...
    value: function startTracking() {
      ...
      if (0 !== this.startTime) {
        const _Error = Error;
        const error = new Error(JSFPSMonitor(6962).ErrorMessages.fpsMonitorAlreadyRunning);
        throw error;
      } else {
        const _Date = Date;
  ...
  },
  {
    ...
    value: function stopAndGetData() {
      ...
      const obj = { minFPS: JSFPSMonitor(7028).roundToDecimalPlaces(self.minFPS, 1), maxFPS: null, averageFPS: null };
      const obj2 = JSFPSMonitor(7028);
      obj.maxFPS = JSFPSMonitor(7028).roundToDecimalPlaces(self.maxFPS, 1);
      const obj3 = JSFPSMonitor(7028);
      obj.averageFPS = JSFPSMonitor(7028).roundToDecimalPlaces(self.averageFPS, 1);
      return obj;
    }
  }
];

export const JSFPSMonitor = _modDef6975(JSFPSMonitor, items);
