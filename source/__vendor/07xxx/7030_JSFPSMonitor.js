...
// Function ID: 7031
// Name: JSFPSMonitor
// Dependencies: [6977, 6978, 6965, 7031]

// Module 7030 (JSFPSMonitor)
...
const items = [
  entry,
  {
    ...
    value: function startTracking() {
      ...
      if (0 !== this.startTime) {
        const _Error = Error;
        const error = new Error(JSFPSMonitor(6965).ErrorMessages.fpsMonitorAlreadyRunning);
        throw error;
      } else {
        const _Date = Date;
  ...
  },
  {
    ...
    value: function stopAndGetData() {
      ...
      const obj = { minFPS: JSFPSMonitor(7031).roundToDecimalPlaces(self.minFPS, 1), maxFPS: null, averageFPS: null };
      const obj2 = JSFPSMonitor(7031);
      obj.maxFPS = JSFPSMonitor(7031).roundToDecimalPlaces(self.maxFPS, 1);
      const obj3 = JSFPSMonitor(7031);
      obj.averageFPS = JSFPSMonitor(7031).roundToDecimalPlaces(self.averageFPS, 1);
      return obj;
    }
  }
];

export const JSFPSMonitor = _modDef6978(JSFPSMonitor, items);
