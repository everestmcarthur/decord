...
// Dependencies: [7059, 7060, 7064, 7086, 7073, 7047, 7087]

// Module 7085 (RVLayoutManager)
...
let items = [
  ...
  },
  {
    ...
    value: function getVisibleLayouts(bound1, arg1) {
      const result = RVLayoutManager(7086).findFirstVisibleIndex(this.layouts, bound1, this.horizontal);
      const obj = RVLayoutManager(7086);
      const findLastVisibleIndexResult = RVLayoutManager(7086).findLastVisibleIndex(this.layouts, arg1, this.horizontal);
      if (-1 !== result) {
        if (-1 !== findLastVisibleIndexResult) {
          let EMPTY = new tmp(7073).ConsecutiveNumbers(result, findLastVisibleIndexResult);
        }
        return EMPTY;
      }
      EMPTY = tmp(7073).ConsecutiveNumbers.EMPTY;
    }
  },
  {
    key: "deleteLayout",
  ...
  },
  {
    ...
    value: function getLayout(arg0) {
      ...
      if (arg0 >= this.layouts.length) {
        const _Error = Error;
        const error = new Error(RVLayoutManager(7047).ErrorMessages.indexOutOfBounds);
        throw error;
      } else {
...
export const RVLayoutManager = _modDef7060(RVLayoutManager, items);
