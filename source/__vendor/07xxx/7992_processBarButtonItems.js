...
// Dependencies: [32, 109, 17, 21, 5692, 1484, 7993, 5683, 4959]
// Exports: useHeaderConfigProps

// Module 7992 (processBarButtonItems)
import ColorDefault from "Color" /* 5692 */;
...
export const useHeaderConfigProps = function useHeaderConfigProps(headerLargeTitle) {
  ...
  const tmpResult = colors(7993);
  let fontSize;
  [tmp10, tmp11, tmp12] = colors(7993).processFonts(items3);
  if ("fontSize" in tmp4) {
    fontSize = tmp4.fontSize;
  }
  const tmp9 = _slicedToArray(colors(7993).processFonts(items3), 3);
  const headerTitle1 = colors(5683).getHeaderTitle({ title, headerTitle }, route.name);
  if ("color" in tmp6) {
  ...
  const isSearchBarAvailableForCurrentPlatform = tmp(4959).isSearchBarAvailableForCurrentPlatform;
  ...
  if (typeof isSearchBarAvailableForCurrentPlatform === "boolean") {
    isSearchBarAvailableForCurrentPlatform2 = tmp(4959).isSearchBarAvailableForCurrentPlatform;
  }
  ...
  if (null != headerLeftResult) {
    ...
    if ("center" === headerTitleAlign) {
      ...
      let tmp32Result = tmp32(tmp(4959).ScreenStackHeaderLeftView, obj11);
    } else {
      if (typeof headerTitle === "function") {
        ...
        obj13.children = tmp62(tmp(5683).HeaderTitle, obj14);
      }
  ...
  if ("center" === headerTitleAlign) {
    if (typeof headerTitle !== "function") {
      ...
      headerTitleResult = tmp38(tmp(5683).HeaderTitle, obj15);
    }
    const obj16 = { children: headerTitleResult };
    tmp38Result = tmp38(tmp(4959).ScreenStackHeaderCenterView, obj16);
  }
  ...
  if (undefined !== headerBackIcon) {
    ...
    const obj17 = { source };
    let tmp40Result = closure_10(tmp(4959).ScreenStackHeaderBackButtonImage, obj17);
  } else {
    tmp40Result = null;
  }
  ...
  if (null != headerRightResult) {
    ...
    tmp42 = closure_10(tmp(4959).ScreenStackHeaderRightView, obj18);
  }
  items7[2] = tmp42;
  let tmp44 = null;
  if (isSearchBarAvailableForCurrentPlatform2) {
