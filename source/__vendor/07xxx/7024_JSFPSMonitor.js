...
// Function ID: 7025
// Name: JSFPSMonitor
// Dependencies: [6971, 6972, 6959, 7025]

// Module 7024 (JSFPSMonitor)
...
const items = [
  entry,
  {
    ...
    value: function startTracking() {
      ...
      if (0 !== this.startTime) {
        const _Error = Error;
        const error = new Error(JSFPSMonitor(6959).ErrorMessages.fpsMonitorAlreadyRunning);
        throw error;
      } else {
        const _Date = Date;
  ...
  },
  {
    ...
    value: function stopAndGetData() {
      ...
      const obj = { minFPS: JSFPSMonitor(7025).roundToDecimalPlaces(self.minFPS, 1), maxFPS: null, averageFPS: null };
      const obj2 = JSFPSMonitor(7025);
      obj.maxFPS = JSFPSMonitor(7025).roundToDecimalPlaces(self.maxFPS, 1);
      const obj3 = JSFPSMonitor(7025);
      obj.averageFPS = JSFPSMonitor(7025).roundToDecimalPlaces(self.averageFPS, 1);
      return obj;
    }
  }
];

export const JSFPSMonitor = _modDef6972(JSFPSMonitor, items);
