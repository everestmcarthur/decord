...
// Dependencies: [6977, 6978, 6982, 7004, 6991, 6965, 7005]

// Module 7003 (RVLayoutManager)
...
let items = [
  ...
  },
  {
    ...
    value: function getVisibleLayouts(bound1, arg1) {
      const result = RVLayoutManager(7004).findFirstVisibleIndex(this.layouts, bound1, this.horizontal);
      const obj = RVLayoutManager(7004);
      const findLastVisibleIndexResult = RVLayoutManager(7004).findLastVisibleIndex(this.layouts, arg1, this.horizontal);
      if (-1 !== result) {
        if (-1 !== findLastVisibleIndexResult) {
          let EMPTY = new tmp(6991).ConsecutiveNumbers(result, findLastVisibleIndexResult);
        }
        return EMPTY;
      }
      EMPTY = tmp(6991).ConsecutiveNumbers.EMPTY;
    }
  },
  {
    key: "deleteLayout",
  ...
  },
  {
    ...
    value: function getLayout(arg0) {
      ...
      if (arg0 >= this.layouts.length) {
        const _Error = Error;
        const error = new Error(RVLayoutManager(6965).ErrorMessages.indexOutOfBounds);
        throw error;
      } else {
...
export const RVLayoutManager = _modDef6978(RVLayoutManager, items);
