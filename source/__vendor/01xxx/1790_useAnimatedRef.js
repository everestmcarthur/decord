...
export const useAnimatedRef = isJest ? (function useAnimatedRefWeb() {
  const f74150 = (getScrollableNode) => {
  ...
  if (!tmp2.current) {
    fun = function fun(map) {
      ...
      if (map) {
        ...
        fun.getTag = () => f74153(current[3]).findNodeHandle(closure_0);
        fun.current = map;
        if (tag.size) {
          tag = undefined;
          if (tmp3 != null) {
  ...
  const first = callback(callback2(() => {
    ...
    if (!obj.isFabric()) {
      mutable = null;
      if (closure_5) {
        mutable = f74153(current[4]).makeMutable(null);
        const tmpResult = f74153(current[4]);
      }
    }
    return mutable;
  }), 1)[0];
  let f74153 = first;
  const first1 = callback(callback2(() => f74153(current[4]).makeMutable(null)), 1)[0];
  let current = first1;
  f74153 = (viewConfig) => {
    if (obj.isFabric()) {
      let fn = f74153(current[5]).getShadowNodeWrapperFromRef;
    } else {
  ...
  const shareableMappingCache = f74153(current[6]).shareableMappingCache;
