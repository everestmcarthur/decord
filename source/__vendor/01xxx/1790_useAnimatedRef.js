...
// Function ID: 1791
// Name: useAnimatedRef
// Dependencies: [32, 19, 1639, 1744, 1673, 1664, 1667, 1666]

// Module 1790 (useAnimatedRef)
...
export const useAnimatedRef = isJest ? (function useAnimatedRefWeb() {
  const f74146 = (getScrollableNode) => {
  ...
  if (!tmp2.current) {
    fun = function fun(map) {
      ...
      if (map) {
        ...
        fun.getTag = () => f74149(current[3]).findNodeHandle(closure_0);
        fun.current = map;
        if (tag.size) {
          tag = undefined;
          if (tmp3 != null) {
  ...
  const first = callback(callback2(() => {
    ...
    if (!obj.isFabric()) {
      mutable = null;
      if (closure_5) {
        mutable = f74149(current[4]).makeMutable(null);
        const tmpResult = f74149(current[4]);
      }
    }
    return mutable;
  }), 1)[0];
  let f74149 = first;
  const first1 = callback(callback2(() => f74149(current[4]).makeMutable(null)), 1)[0];
  let current = first1;
  f74149 = (viewConfig) => {
    if (obj.isFabric()) {
      let fn = f74149(current[5]).getShadowNodeWrapperFromRef;
    } else {
  ...
  const shareableMappingCache = f74149(current[6]).shareableMappingCache;
