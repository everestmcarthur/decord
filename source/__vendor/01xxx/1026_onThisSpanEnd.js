...
export const adjustTransactionDuration = (on, activeSpan) => {
  ...
  if (obj.isRootSpan(activeSpan)) {
    on.on("spanEnd", (arg0) => {
      let obj = activeSpan;
      if (arg0 === activeSpan) {
        let timestamp = activeSpan(682).spanToJSON(obj).timestamp;
        const obj3 = activeSpan(682);
        const tmp6 = activeSpan;
        const start_timestamp = activeSpan(682).spanToJSON(obj).start_timestamp;
        if (timestamp) {
          if (start_timestamp) {
...
export const ignoreEmptyBackNavigation = (on, c4) => {
  closure_0 = c4;
  const f71997 = (arg0) => {
    const data = c4(f71997[2]).spanToJSON(arg0).data;
    let prop;
    if (null !== data) {
      if (undefined !== data) {
        ...
        if (tmpResult.isSentrySpan(c4)) {
          on.on("spanEnd", (arg0) => {
            if (arg0 === c4) {
              if (f71999(tmp)) {
                c4 = tmp;
                const spanDescendants = c4(f71999[2]).getSpanDescendants(tmp);
...
export const ignoreEmptyRouteChangeTransactions = (on, c4) => {
  ...
  closure_0 = c4;
  f71999 = (arg0) => {
    const spanToJSONResult = c4(f71999[2]).spanToJSON(arg0);
    let tmp2 = spanToJSONResult.description === f71999;
    if (tmp2) {
  ...
  f72000 = (arg0) => {
    const debug = c4(f71999[2]).debug;
    debug.log("Discarding empty \"" + f71999 + "\" transaction that never received route information.");
    if (null != c4) {
