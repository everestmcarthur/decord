...
// Dependencies: [5, 5646, 7640, 1962, 502, 1958, 2013, 2022, 1980, 8528, 4624, 4278, 4288, 4461, 1372, 1074, 1099, 1965, 4682, 7856, 5502, 1385, 8541, 7336, 1113, 1101, 5499, 1897, 8542, 9243, 4695, 4782, 13082, 7637, 5144, 5657, 9700, 7209, 9940, 573, 13083, 4851, 13084, 5608, 1241, 1271, 4541, 1091, 4833, 1249, 1970, 4625, 5610, 7325, 4320, 13086, 13087, 11585, 4963, 4881, 13088, 13089, 1254, 13091, 2]
// Exports: trackInviteEmbedActioned, trackInviteServerClicked, transitionToGuildFromEventInvite

// Module 8527 (InstantInviteActionCreators)
...
import CodedLinkActionCreatorsDefault from "CodedLinkActionCreators" /* 11585 */;
import generateDynamicLinkDefault from "generateDynamicLink" /* 13089 */;
...
let closure_7 = fn(7640).isGuildScheduledEventActive;
...
const STAGE_INVITE_STATE_KEY = fn(5502).STAGE_INVITE_STATE_KEY;
let invite = "invite";
let c40 = null;
const size = fn(2);
...
export default {
  resolveInvite(code, arg1, arg2) {
    ...
    if (obj.isDispatching()) {
      ...
      nextPromise = tmp(13083)(code, arg1, arg2).then((result) => {
        ({ invite, code } = result);
        if (null != invite) {
          const obj2 = { type: "INVITE_RESOLVE_SUCCESS", invite, code };
          closure_1(dependencyMap[39]).dispatch(obj2);
      ...
      const promise = tmp(13083)(code, arg1, arg2);
    }
    return nextPromise;
  },
  ...
  transitionToInvite(flags, arg1) {
    ...
    if (null != guild) {
      ...
      if (hasItem) {
        obj(13086).onOpenHubInvite(flags);
        const obj6 = obj(13086);
      }
    }
    let num = flags.flags;
    if (num == null) {
    ...
    let hasFlagResult = dMFromUserId(1385).hasFlag(num, dMFromUserId(8541).GuildInviteFlags.IS_GUEST_INVITE);
    if (!hasFlagResult) {
      hasFlagResult = tmp6(1385).hasFlag(num, tmp6(8541).GuildInviteFlags.IS_APPLICATION_BYPASS);
      ...
      if (!hasFlagResult) {
        if (flags.new_member) {
          if (tmp6Result3.inviteGuildHasPendingMemberDisabledVerification(guild)) {
            const result1 = tmp6(13087).openVerificationModalOrTransitionToApplication(guild.id);
            const tmp6Result4 = tmp6(13087);
          }
          tmp6Result3 = tmp6(13087);
        }
      }
    }
    if (null != channel) {
  ...
  openApp(code, targetChannelId, fingerprint, username, inviteType) {
    ...
    if (null != _modDef4963.ua) {
      const formatted = tmp7(4963).ua.toLowerCase();
    ...
    const os = tmp7(4963).os;
    let family;
    if (os != null) {
      family = os.family;
    }
    if ("Android" !== family) {
      const os2 = tmp7(4963).os;
      let family1;
      if (os2 != null) {
        family1 = os2.family;
      }
      if ("iOS" !== family1) {
        ...
        tmp7(13091).launch(combined, (arg0) => {
          if (arg0) {
    ...
    const attemptId = tmp18(13089).generateAttemptId();
    inviteType = undefined;
    const tmp18Result = tmp18(13089);
    if (inviteType != null) {
