...
// Dependencies: [2026, 10607, 1074, 1084, 4902, 10609, 10610, 1271, 573, 12, 1255, 1366, 10187, 1941, 1221, 5066, 1115, 1241, 2]
...
// Module 10608 (GIFPickerActionCreators)
...
import AttachmentUrlUtilsAll from "AttachmentUrlUtils" /* 10187 */;
import GifProvider from "GifProvider" /* 10609 */;
import GIFPickerUtils from "GIFPickerUtils" /* 10610 */;
...
export const gifUrlKey = function gifUrlKey(uri) {
  ...
  if (null != toURLSafeResult) {
    if (obj2.isAttachmentPathUrl(toURLSafeResult)) {
      const tmp5Result = tmp5(10187);
      str1 = tmp5(10187).removeSignedUrlParameters(toURLSafeResult).toString();
      const str = tmp5(10187).removeSignedUrlParameters(toURLSafeResult);
    }
    tmp4 = str1;
...
export const addFavoriteGIF = function addFavoriteGIF(size) {
  ...
  FrecencyUserSettingsActionCreators.updateAsync("favoriteGifs", async (gifs) => {
      ...
      if (null != tmp3.gifSrc) {
        if ("" !== tmp3.gifSrc) {
          ...
          if (tmp9) {
            ...
            if (!result) {
              result = tmp10(10187).isAttachmentPathUrl(toURLSafeResult);
              const tmp10Result = tmp10(10187);
            }
            tmp9 = result;
            tmp10 = importAll;
          }
          ...
          if (null != toURLSafeResult1) {
            if (obj12.isAttachmentPathUrl(toURLSafeResult1)) {
              const tmp24Result = tmp24(10187);
              url = tmp24(10187).removeSignedUrlParameters(toURLSafeResult1).toString();
              const str9 = tmp24(10187).removeSignedUrlParameters(toURLSafeResult1);
            }
            tmp23 = url;
    ...
    if (tmp5) {
      ...
      if (!result3) {
        result3 = tmp6(10187).isAttachmentPathUrl(toURLSafeResult2);
        const tmp6Result = tmp6(10187);
      }
      tmp5 = result3;
      tmp6 = importAll;
    }
  ...
  FrecencyUserSettingsActionCreators.updateAsync("favoriteGifs", async (gifs) => {
    if (closure_0 in gifs.gifs) {
      ...
      if (null != toURLSafeResult) {
        if (obj2.isAttachmentPathUrl(toURLSafeResult)) {
          const tmp10Result = tmp10(10187);
          const str1 = tmp10(10187).removeSignedUrlParameters(toURLSafeResult).toString();
          const str = tmp10(10187).removeSignedUrlParameters(toURLSafeResult);
        }
        obj2 = AttachmentUrlUtilsAll;
        tmp10 = importAll;
      }
