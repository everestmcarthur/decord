...
// Dependencies: [19, 21, 4541, 573, 4920, 1114, 4924, 5008, 5008, 1896, 2]

// Module 4919 (actions/AlertActionCreators)
...
export default {
  ...
  show(hideActionSheet) {
    ...
    if (!tmp7) {
      if (flag) {
        merged(4541).hideActionSheet();
        const obj4 = merged(4541);
      }
      if (null != c6) {
        confirmText(4920).dismissAlert(c6);
        const obj5 = confirmText(4920);
      }
      ...
      const tmp31 = c5;
      const tmp32 = c4;
      obj8.variant = confirmText(5008).getAlertButtonVariant(confirmColor);
      ...
      const items = [c3(confirmText(4924).AlertActionButton, obj8), ];
      ...
      if (null != cancelText) {
        ...
        tmp30Result = tmp30(tmp29(4924).AlertActionButton, obj9);
      }
      ...
      obj7.openAlert(text, c3(confirmText(4924).AlertModal, obj3), () => {
        if (!c5) {
          if (c4 != null) {
            tmp();
          }
      ...
      const obj10 = confirmText(5008);
    } else {
      if (null != c6) {
        confirmText(4920).dismissAlert(c6);
        c6 = null;
        const obj2 = confirmText(4920);
      }
      const self = this;
      const obj13 = {
        importer() {
            return asyncRequireImpl(5008, dependencyMap.paths).then((result) => {
