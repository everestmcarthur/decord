...
// Dependencies: [19, 21, 4574, 573, 4953, 1114, 4957, 5046, 5046, 1896, 2]

// Module 4952 (actions/AlertActionCreators)
...
export default {
  ...
  show(hideActionSheet) {
    ...
    if (!tmp7) {
      if (flag) {
        merged(4574).hideActionSheet();
        const obj4 = merged(4574);
      }
      if (null != c6) {
        confirmText(4953).dismissAlert(c6);
        const obj5 = confirmText(4953);
      }
      ...
      const tmp31 = c5;
      const tmp32 = c4;
      obj8.variant = confirmText(5046).getAlertButtonVariant(confirmColor);
      ...
      const items = [c3(confirmText(4957).AlertActionButton, obj8), ];
      ...
      if (null != cancelText) {
        ...
        tmp30Result = tmp30(tmp29(4957).AlertActionButton, obj9);
      }
      ...
      obj7.openAlert(text, c3(confirmText(4957).AlertModal, obj3), () => {
        if (!c5) {
          if (c4 != null) {
            tmp();
          }
      ...
      const obj10 = confirmText(5046);
    } else {
      if (null != c6) {
        confirmText(4953).dismissAlert(c6);
        c6 = null;
        const obj2 = confirmText(4953);
      }
      const self = this;
      const obj13 = {
        importer() {
            return asyncRequireImpl(5046, dependencyMap.paths).then((result) => {
