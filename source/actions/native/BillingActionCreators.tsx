...
// Dependencies: [109, 5, 19, 7534, 7535, 1372, 4303, 7351, 1074, 1085, 21, 3, 1240, 510, 1241, 1271, 4312, 4964, 1364, 11208, 7371, 11209, 573, 12, 7354, 7531, 11587, 1115, 10812, 4985, 10858, 1897, 4843, 7526, 10854, 4540, 13458, 4319, 4833, 1249, 13460, 13461, 1970, 13462, 10962, 10970, 1255, 9519, 2]
...
// Module 7533 (BillingActionCreators)
...
import showSpendingLimitReachedAlert from "showSpendingLimitReachedAlert" /* 10854 */;
import IAPUtils from "IAPUtils" /* 11208 */;
...
import ErrorUtilsAll from "ErrorUtils" /* 13458 */;
import APBRequestOperations from "APBRequestOperations" /* 13460 */;
import _objectWithoutProperties from "_objectWithoutProperties" /* 109 */;
import asyncGeneratorStep from "asyncGeneratorStep" /* 5 */;
import noop from "module_19" /* 19 */;
import GiftPromotionStore from "GiftPromotionStore" /* 7534 */;
...
function handlePurchaseException(code, purchase_type) {
  ...
  if (!set.has(code.code)) {
    ...
    if (!(code instanceof V6OrEarlierAPIError.BillingError)) {
      billingError = new tmp(4540).BillingError(code);
    }
    if (obj.isSpendingLimitError(billingError)) {
      const result = tmp(10854).showSpendingLimitReachedAlert();
      const tmpResult = tmp(10854);
    } else {
      ...
      if (null != underlyingIOSError) {
        ...
        if (!(code instanceof tmp(4540).BillingError)) {
          billingError1 = new tmp(4540).BillingError(code);
        }
        let tmp19 = message2 === stringResult && flag;
        if (tmp19) {
          tmp19 = billingError1.code !== tmp(4319).ErrorCodes.UNKNOWN;
        }
        if (tmp19) {
        ...
        obj5.title = intl.string(tmp(1115).t.zrhHH3);
        obj5.body = message2;
        ...
        obj6.tags = obj7;
        const result1 = tmp(4312).captureBillingException(code, obj6);
        if (flag) {
          throw code;
        }
        const tmpResult2 = tmp(4312);
      }
    }
...
let closure_30 = async function _clearAndMakeIAPRequest() {
  ...
  await _mod11209.clearTransactionIOS();
...
function determineProductId(arg0) {
  if (APBRequestOperations.APBRequestOperations.CREATE !== arg0) {
    if (tmp(13460).APBRequestOperations.CANCEL !== arg0) {
      if (tmp(13460).APBRequestOperations.RESUBSCRIBE !== arg0) {
        if (tmp(13460).APBRequestOperations.REACTIVATE !== arg0) {
          if (tmp(13460).APBRequestOperations.CHARGE === arg0) {
            return tmp(7354).ProductIds.GENERIC_CONSUMABLE;
          } else {
            if (tmp(13461).ACRequestOperations.CREATE !== arg0) {
              if (tmp(13461).ACRequestOperations.CANCEL !== arg0) {
                if (tmp(13461).ACRequestOperations.REACTIVATE !== arg0) {
                  if (tmp(13461).ACRequestOperations.MODIFY !== arg0) {
                    if (tmp(13461).ACRequestOperations.CHARGE === arg0) {
                      return tmp(7354).ProductIds.GENERIC_CONSUMABLE;
                    } else {
                      const _Error = Error;
            ...
            return tmp(7354).ProductIds.GENERIC_SUBSCRIPTION;
          }
        }
      }
    }
...
let items = [fn(11209).ErrorCode.E_USER_CANCELLED, StoreKitErrors.PAYMENT_CANCELED];
...
export default {
  applyAppleReceipt,
  fetchMostRecentSubscription: fn(4964).fetchMostRecentSubscription,
  fetchIpCountryCode: fn(4964).fetchIpCountryCode,
  init() {
    const self = this;
    return (async (arg0, value) => {
      if (c6 === 2) {
        ...
        try {
          ...
          if (0 === c4) {
            if (arg0 === 1) {
              ...
              c4 = 2;
              c5 = 1;
              const obj4 = { value: _mod11209.initConnection(), done: false };
              return obj4;
            }
          } else if (1 === tmp7) {
            c3 = 0;
                  ...
                  if (closure_0 === undefined) {
                    if (closure_134_2.length > 0) {
                      ...
                      const item = closure_134_3.forEach((item) => {
                        const result = closure_1_0(4312).captureBillingException(item);
                      });
                      const _Error = Error;
