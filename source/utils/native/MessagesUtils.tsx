...
// Function ID: 12099
// Name: MessagesUtils
// Dependencies: [1074, 7436, 4626, 8006, 4786, 2]

// Module 12098 (MessagesUtils)
import CodedLink from "CodedLink" /* 4626 */;
import MediaPostEmbedUtils from "MediaPostEmbedUtils" /* 4786 */;
import GuildTemplatesConstants from "GuildTemplatesConstants" /* 7436 */;
      ...
      if (someResult) {
        ...
        someResult = codedLinks.some((item) => {
          ...
          if (CodedLink.CodedLinkType.BUILD_OVERRIDE !== type) {
            if (tmp(4626).CodedLinkType.MANUAL_BUILD_OVERRIDE !== type) {
              if (tmp(4626).CodedLinkType.EXPERIMENT === type) {
                const experimentFromEmbedURL = tmp(8006).getExperimentFromEmbedURL(code);
                ...
                if (tmp(4626).CodedLinkType.EVENT !== type) {
                  if (tmp(4626).CodedLinkType.CHANNEL_LINK !== type) {
                    if (tmp(4626).CodedLinkType.APP_DIRECTORY_PROFILE === type) {
                      ...
                      return tmp25;
                    } else if (tmp(4626).CodedLinkType.ACTIVITY_BOOKMARK === type) {
                          ...
                          if (tmp(4626).CodedLinkType.SOCIAL_LAYER_STOREFRONT !== type) {
                            if (tmp(4626).CodedLinkType.SOCIAL_LAYER_STOREFRONT_APP !== type) {
                              if (tmp(4626).CodedLinkType.QUESTS_EMBED === type) {
                                ...
                                if (tmp(4626).CodedLinkType.APP_DIRECTORY_STOREFRONT !== type) {
                                  if (tmp(4626).CodedLinkType.APP_DIRECTORY_STOREFRONT_SKU !== type) {
                                    if (tmp(4626).CodedLinkType.APP_OAUTH2_LINK === type) {
