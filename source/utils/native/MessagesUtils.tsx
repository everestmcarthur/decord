...
// Function ID: 12091
// Name: MessagesUtils
// Dependencies: [1074, 7433, 4627, 7997, 4787, 2]

// Module 12090 (MessagesUtils)
import CodedLink from "CodedLink" /* 4627 */;
import MediaPostEmbedUtils from "MediaPostEmbedUtils" /* 4787 */;
      ...
      if (someResult) {
        ...
        someResult = codedLinks.some((item) => {
          ...
          if (CodedLink.CodedLinkType.BUILD_OVERRIDE !== type) {
            if (tmp(4627).CodedLinkType.MANUAL_BUILD_OVERRIDE !== type) {
              if (tmp(4627).CodedLinkType.EXPERIMENT === type) {
                const experimentFromEmbedURL = tmp(7997).getExperimentFromEmbedURL(code);
                ...
                if (tmp(4627).CodedLinkType.EVENT !== type) {
                  if (tmp(4627).CodedLinkType.CHANNEL_LINK !== type) {
                    if (tmp(4627).CodedLinkType.APP_DIRECTORY_PROFILE === type) {
                      ...
                      return tmp25;
                    } else if (tmp(4627).CodedLinkType.ACTIVITY_BOOKMARK === type) {
                          ...
                          if (tmp(4627).CodedLinkType.SOCIAL_LAYER_STOREFRONT !== type) {
                            if (tmp(4627).CodedLinkType.SOCIAL_LAYER_STOREFRONT_APP !== type) {
                              if (tmp(4627).CodedLinkType.QUESTS_EMBED === type) {
                                ...
                                if (tmp(4627).CodedLinkType.APP_DIRECTORY_STOREFRONT !== type) {
                                  if (tmp(4627).CodedLinkType.APP_DIRECTORY_STOREFRONT_SKU !== type) {
                                    if (tmp(4627).CodedLinkType.APP_OAUTH2_LINK === type) {
