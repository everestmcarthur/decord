...
// Function ID: 12108
// Name: MessagesUtils
// Dependencies: [1074, 7440, 4628, 8012, 4788, 2]

// Module 12107 (MessagesUtils)
import CodedLink from "CodedLink" /* 4628 */;
import MediaPostEmbedUtils from "MediaPostEmbedUtils" /* 4788 */;
import GuildTemplatesConstants from "GuildTemplatesConstants" /* 7440 */;
      ...
      if (someResult) {
        ...
        someResult = codedLinks.some((item) => {
          ...
          if (CodedLink.CodedLinkType.BUILD_OVERRIDE !== type) {
            if (tmp(4628).CodedLinkType.MANUAL_BUILD_OVERRIDE !== type) {
              if (tmp(4628).CodedLinkType.EXPERIMENT === type) {
                const experimentFromEmbedURL = tmp(8012).getExperimentFromEmbedURL(code);
                ...
                if (tmp(4628).CodedLinkType.EVENT !== type) {
                  if (tmp(4628).CodedLinkType.CHANNEL_LINK !== type) {
                    if (tmp(4628).CodedLinkType.APP_DIRECTORY_PROFILE === type) {
                      ...
                      return tmp25;
                    } else if (tmp(4628).CodedLinkType.ACTIVITY_BOOKMARK === type) {
                          ...
                          if (tmp(4628).CodedLinkType.SOCIAL_LAYER_STOREFRONT !== type) {
                            if (tmp(4628).CodedLinkType.SOCIAL_LAYER_STOREFRONT_APP !== type) {
                              if (tmp(4628).CodedLinkType.QUESTS_EMBED === type) {
                                ...
                                if (tmp(4628).CodedLinkType.APP_DIRECTORY_STOREFRONT !== type) {
                                  if (tmp(4628).CodedLinkType.APP_DIRECTORY_STOREFRONT_SKU !== type) {
                                    if (tmp(4628).CodedLinkType.APP_OAUTH2_LINK === type) {
