...
// Function ID: 4620
// Name: parseURL
// Dependencies: [32, 1074, 1076, 4620, 4621, 1472, 4336, 1368, 4622, 4627, 13081, 4878, 13942, 1366, 4793, 9369, 7513, 1609, 1364, 9292, 1241, 1254, 13943, 2]
// Exports: default
...
export default function parseURL(arg0) {
  ...
  if (null == sanitizeUrlResult) {
    ...
    if (null != findCodedLinkResult) {
      ...
      if (tmp5(4627).CodedLinkType.INVITE === type) {
        ...
        obj5.payload = obj6;
        return obj5;
      } else if (tmp5(4627).CodedLinkType.TEMPLATE === type) {
        ...
        if (tmp5(4627).CodedLinkType.BUILD_OVERRIDE !== type) {
          if (tmp5(4627).CodedLinkType.MANUAL_BUILD_OVERRIDE !== type) {
            if (tmp5(4627).CodedLinkType.EXPERIMENT !== type) {
              if (tmp5(4627).CodedLinkType.EVENT !== type) {
                if (tmp5(4627).CodedLinkType.CHANNEL_LINK !== type) {
                  if (tmp5(4627).CodedLinkType.APP_DIRECTORY_PROFILE !== type) {
                    if (tmp5(4627).CodedLinkType.ACTIVITY_BOOKMARK === type) {
                      ...
                      obj9.payload = obj10;
                      return obj9;
                    } else if (tmp5(4627).CodedLinkType.EMBEDDED_ACTIVITY_INVITE !== type) {
                      if (tmp5(4627).CodedLinkType.GUILD_PRODUCT !== type) {
                        if (tmp5(4627).CodedLinkType.SERVER_SHOP !== type) {
                          if (tmp5(4627).CodedLinkType.SOCIAL_LAYER_STOREFRONT !== type) {
                            if (tmp5(4627).CodedLinkType.SOCIAL_LAYER_STOREFRONT_APP !== type) {
                              if (tmp5(4627).CodedLinkType.QUESTS_EMBED !== type) {
                                if (tmp5(4627).CodedLinkType.GAME_PROFILE === type) {
                                  ...
                                  obj11.payload = obj12;
                                  return obj11;
                                } else if (tmp5(4627).CodedLinkType.APP_DIRECTORY_STOREFRONT !== type) {
                                  if (tmp5(4627).CodedLinkType.APP_DIRECTORY_STOREFRONT_SKU !== type) {
                                    if (tmp5(4627).CodedLinkType.APP_OAUTH2_LINK !== type) {
                                      if (tmp5(4627).CodedLinkType.COLLECTIBLES_SHOP === type) {
                                        const tmp5Result = tmp5(13081);
                                      ...
                                      } else if (tmp5(4627).CodedLinkType.GAME_SERVER_SHARE !== type) {
                                        if (tmp5(4627).CodedLinkType.USER_PROFILE !== type) {
                                          const _Error2 = Error;
    ...
    if (findGiftCodesResult.length > 0) {
      ...
      const result = tmp5(13942).findRemoteAuthFingerprint(host, pathname);
      if (null != result) {
        if (result.length > 0) {
      ...
      if (null != pathname) {
        const tryParseDiceRollLinkResult = tmp5(4793).tryParseDiceRollLink(pathname);
        if (null != tryParseDiceRollLinkResult) {
          ...
          const tryParseChannelPathResult = tmp5(4793).tryParseChannelPath(pathname);
          if (null != tryParseChannelPathResult) {
            ...
            if (null != pathname.match(re27)) {
              ...
              if (null != pathname.match(re16)) {
                ...
                const result1 = tmp5(9369).parseOAuth2AuthorizeProps(str6);
                if (null != result1) {
              ...
              if (null != pathname.match(re17)) {
                  ...
                  if (match5.length > 1) {
                    ...
                    switch (match5[1]) {
                      ...
                      case "mobile-web-redirect-checkout":
                        let result2 = tmp5(7513).isMobileWebRedirectCheckoutEnabled();
                        if (result2) {
                ...
                const result3 = tmp5(4793).tryParseEventDetailsPath(pathname);
                if (null != result3) {
                  ...
                  if (null != match6) {
                    ...
                    if (null != match7) {
                      ...
                      if (null != match8) {
                        ...
                        const obj93 = { type: React5.GUILD_SETTINGS, guildId: tmp79[1], settingsSection: tmp(13943)(hasOwnProperty, tmp79[2]), settingsSubsection: tmp(13943)(timestampProducer, tmp79[3]) };
                        obj91.payload = obj93;
                        return obj91;
                      } else {
                        ...
                        if (null != match9) {
                          ...
                          const obj96 = { type: React5.GUILD_SETTINGS_PICKER, settingsSection: tmp(13943)(hasOwnProperty, tmp74[1]), settingsSubsection: tmp(13943)(timestampProducer, tmp74[2]), feature: tmp126(str18).feature };
                          obj95.payload = obj96;
                          return obj95;
                ...
                const tmp5Result19 = tmp5(4793);
              }
            }
          }
          const tmp5Result13 = tmp5(4793);
        }
        const tmp5Result12 = tmp5(4793);
      }
      tmpResult7 = tmp(1366);
    }
  }
