...
// Function ID: 4619
// Name: parseURL
// Dependencies: [32, 1074, 1076, 4619, 4620, 1472, 4334, 1368, 4621, 4626, 13089, 4877, 13955, 1366, 4792, 9380, 7516, 1609, 1364, 9303, 1241, 1254, 13956, 2]
// Exports: default

// Module 4618 (parseURL)
...
export default function parseURL(arg0) {
  ...
  const sanitizeUrlResult = _modDef4334.sanitizeUrl(arg0);
  if (null == sanitizeUrlResult) {
    ...
    if (null != findCodedLinkResult) {
      ...
      if (tmp5(4626).CodedLinkType.INVITE === type) {
        ...
        obj5.payload = obj6;
        return obj5;
      } else if (tmp5(4626).CodedLinkType.TEMPLATE === type) {
        ...
        if (tmp5(4626).CodedLinkType.BUILD_OVERRIDE !== type) {
          if (tmp5(4626).CodedLinkType.MANUAL_BUILD_OVERRIDE !== type) {
            if (tmp5(4626).CodedLinkType.EXPERIMENT !== type) {
              if (tmp5(4626).CodedLinkType.EVENT !== type) {
                if (tmp5(4626).CodedLinkType.CHANNEL_LINK !== type) {
                  if (tmp5(4626).CodedLinkType.APP_DIRECTORY_PROFILE !== type) {
                    if (tmp5(4626).CodedLinkType.ACTIVITY_BOOKMARK === type) {
                      ...
                      obj9.payload = obj10;
                      return obj9;
                    } else if (tmp5(4626).CodedLinkType.EMBEDDED_ACTIVITY_INVITE !== type) {
                      if (tmp5(4626).CodedLinkType.GUILD_PRODUCT !== type) {
                        if (tmp5(4626).CodedLinkType.SERVER_SHOP !== type) {
                          if (tmp5(4626).CodedLinkType.SOCIAL_LAYER_STOREFRONT !== type) {
                            if (tmp5(4626).CodedLinkType.SOCIAL_LAYER_STOREFRONT_APP !== type) {
                              if (tmp5(4626).CodedLinkType.QUESTS_EMBED !== type) {
                                if (tmp5(4626).CodedLinkType.GAME_PROFILE === type) {
                                  ...
                                  obj11.payload = obj12;
                                  return obj11;
                                } else if (tmp5(4626).CodedLinkType.APP_DIRECTORY_STOREFRONT !== type) {
                                  if (tmp5(4626).CodedLinkType.APP_DIRECTORY_STOREFRONT_SKU !== type) {
                                    if (tmp5(4626).CodedLinkType.APP_OAUTH2_LINK !== type) {
                                      if (tmp5(4626).CodedLinkType.COLLECTIBLES_SHOP === type) {
                                        const tmp5Result = tmp5(13089);
                                      ...
                                      } else if (tmp5(4626).CodedLinkType.GAME_SERVER_SHARE !== type) {
                                        if (tmp5(4626).CodedLinkType.USER_PROFILE !== type) {
                                          const _Error2 = Error;
    ...
    if (findGiftCodesResult.length > 0) {
      ...
      const result = tmp5(13955).findRemoteAuthFingerprint(host, pathname);
      if (null != result) {
        if (result.length > 0) {
      ...
      if (null != pathname) {
        const tryParseDiceRollLinkResult = tmp5(4792).tryParseDiceRollLink(pathname);
        if (null != tryParseDiceRollLinkResult) {
          ...
          const tryParseChannelPathResult = tmp5(4792).tryParseChannelPath(pathname);
          if (null != tryParseChannelPathResult) {
            ...
            if (null != pathname.match(re27)) {
              ...
              if (null != pathname.match(re16)) {
                ...
                const result1 = tmp5(9380).parseOAuth2AuthorizeProps(str6);
                if (null != result1) {
              ...
              if (null != pathname.match(re17)) {
                  ...
                  if (match5.length > 1) {
                    ...
                    switch (match5[1]) {
                      ...
                      case "mobile-web-redirect-checkout":
                        let result2 = tmp5(7516).isMobileWebRedirectCheckoutEnabled();
                        if (result2) {
                ...
                const result3 = tmp5(4792).tryParseEventDetailsPath(pathname);
                if (null != result3) {
                  ...
                  if (null != match6) {
                    ...
                    if (null != match7) {
                      ...
                      if (null != match8) {
                        ...
                        const obj93 = { type: React5.GUILD_SETTINGS, guildId: tmp79[1], settingsSection: tmp(13956)(hasOwnProperty, tmp79[2]), settingsSubsection: tmp(13956)(timestampProducer, tmp79[3]) };
                        obj91.payload = obj93;
                        return obj91;
                      } else {
                        ...
                        if (null != match9) {
                          ...
                          const obj96 = { type: React5.GUILD_SETTINGS_PICKER, settingsSection: tmp(13956)(hasOwnProperty, tmp74[1]), settingsSubsection: tmp(13956)(timestampProducer, tmp74[2]), feature: tmp126(str18).feature };
                          obj95.payload = obj96;
                          return obj95;
                ...
                const tmp5Result19 = tmp5(4792);
              }
            }
          }
          const tmp5Result13 = tmp5(4792);
        }
        const tmp5Result12 = tmp5(4792);
      }
      tmpResult7 = tmp(1366);
    }
  }
