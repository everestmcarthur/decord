...
// Function ID: 4703
// Name: parseURL
// Dependencies: [32, 1074, 1076, 4703, 4704, 1472, 2075, 1368, 4705, 4710, 13188, 4961, 14057, 1366, 4876, 9475, 7602, 1609, 1364, 9398, 1241, 1254, 14058, 2]
// Exports: default

// Module 4702 (parseURL)
...
import urlPartToSettingsEnumDefault from "urlPartToSettingsEnum" /* 14058 */;
...
export default function parseURL(arg0) {
  ...
  const sanitizeUrlResult = _modDef2075.sanitizeUrl(arg0);
  if (null == sanitizeUrlResult) {
    ...
    if (null != findCodedLinkResult) {
      ...
      if (tmp5(4710).CodedLinkType.INVITE === type) {
        ...
        obj5.payload = obj6;
        return obj5;
      } else if (tmp5(4710).CodedLinkType.TEMPLATE === type) {
        ...
        if (tmp5(4710).CodedLinkType.BUILD_OVERRIDE !== type) {
          if (tmp5(4710).CodedLinkType.MANUAL_BUILD_OVERRIDE !== type) {
            if (tmp5(4710).CodedLinkType.EXPERIMENT !== type) {
              if (tmp5(4710).CodedLinkType.EVENT !== type) {
                if (tmp5(4710).CodedLinkType.CHANNEL_LINK !== type) {
                  if (tmp5(4710).CodedLinkType.APP_DIRECTORY_PROFILE !== type) {
                    if (tmp5(4710).CodedLinkType.ACTIVITY_BOOKMARK === type) {
                      ...
                      obj9.payload = obj10;
                      return obj9;
                    } else if (tmp5(4710).CodedLinkType.EMBEDDED_ACTIVITY_INVITE !== type) {
                      if (tmp5(4710).CodedLinkType.GUILD_PRODUCT !== type) {
                        if (tmp5(4710).CodedLinkType.SERVER_SHOP !== type) {
                          if (tmp5(4710).CodedLinkType.SOCIAL_LAYER_STOREFRONT !== type) {
                            if (tmp5(4710).CodedLinkType.SOCIAL_LAYER_STOREFRONT_APP !== type) {
                              if (tmp5(4710).CodedLinkType.QUESTS_EMBED !== type) {
                                if (tmp5(4710).CodedLinkType.GAME_PROFILE === type) {
                                  ...
                                  obj11.payload = obj12;
                                  return obj11;
                                } else if (tmp5(4710).CodedLinkType.APP_DIRECTORY_STOREFRONT !== type) {
                                  if (tmp5(4710).CodedLinkType.APP_DIRECTORY_STOREFRONT_SKU !== type) {
                                    if (tmp5(4710).CodedLinkType.APP_OAUTH2_LINK !== type) {
                                      if (tmp5(4710).CodedLinkType.COLLECTIBLES_SHOP === type) {
                                        const tmp5Result = tmp5(13188);
                                      ...
                                      } else if (tmp5(4710).CodedLinkType.GAME_SERVER_SHARE !== type) {
                                        if (tmp5(4710).CodedLinkType.USER_PROFILE !== type) {
                                          const _Error2 = Error;
    ...
    if (findGiftCodesResult.length > 0) {
      ...
      const result = tmp5(14057).findRemoteAuthFingerprint(host, pathname);
      if (null != result) {
        if (result.length > 0) {
      ...
      if (null != pathname) {
        const tryParseDiceRollLinkResult = tmp5(4876).tryParseDiceRollLink(pathname);
        if (null != tryParseDiceRollLinkResult) {
          ...
          const tryParseChannelPathResult = tmp5(4876).tryParseChannelPath(pathname);
          if (null != tryParseChannelPathResult) {
            ...
            if (null != pathname.match(re27)) {
              ...
              if (null != pathname.match(re16)) {
                ...
                const result1 = tmp5(9475).parseOAuth2AuthorizeProps(str6);
                if (null != result1) {
                  ...
                  if (match5.length > 1) {
                    ...
                    switch (match5[1]) {
                      ...
                      case "mobile-web-redirect-checkout":
                        let result2 = tmp5(7602).isMobileWebRedirectCheckoutEnabled();
                        if (result2) {
          ...
          const tmp5Result11 = tmp5(4876);
        }
        const tmp5Result10 = tmp5(4876);
      }
      tmpResult6 = tmp(1366);
    }
  }
