...
// Function ID: 8203
// Name: ChangeLogUtils
// Dependencies: [4336, 8203, 5089, 5109, 5110, 4629, 2]
// Exports: renderChangelogMessageMarkup

// Module 8202 (ChangeLogUtils)
...
import MarkupListRuleDefault from "MarkupListRule" /* 5109 */;
import MarkupSubtextRuleDefault from "MarkupSubtextRule" /* 5110 */;
...
function defaultRules(dependencyMap) {
  ...
  if (null != require("utils/ChangeLogUtils").customRules.strong) {
    let obj2 = {};
    const merged1 = Object.assign(tmp(8203).baseRules.strong);
    if (typeof tmp(8203).customRules.strong === "function") {
      const customRules = tmp(8203).customRules;
      let strong = customRules.strong(dependencyMap);
    } else {
      strong = tmp(8203).customRules.strong;
    }
    ...
    if (typeof tmp(8203).customRules.image === "function") {
      const customRules2 = tmp(8203).customRules;
      image = customRules2.image(dependencyMap);
    } else {
      image = tmp(8203).customRules.image;
    }
    const merged5 = Object.assign(image);
    obj.image = obj5;
    const obj6 = {};
    const merged6 = Object.assign(link);
    if (typeof tmp(8203).customRules.link === "function") {
      const customRules3 = tmp(8203).customRules;
      link = customRules3.link(dependencyMap);
    } else {
      link = tmp(8203).customRules.link;
    }
    const merged7 = Object.assign(link);
    obj.link = obj6;
    const obj7 = {};
    const merged8 = Object.assign(list);
    if (typeof tmp(8203).customRules.list === "function") {
      const customRules4 = tmp(8203).customRules;
      list = customRules4.list(dependencyMap);
    } else {
      list = tmp(8203).customRules.list;
    }
    ...
    obj9.parse = (arg0, fn, inline) => {
      ...
      obj.level = num;
      obj.content = _modDef4336.parseInline(fn, formatted, inline);
      return obj;
    };
    if (typeof tmp(8203).customRules.lheading === "function") {
      const customRules5 = tmp(8203).customRules;
      lheading = customRules5.lheading(dependencyMap);
    } else {
      lheading = tmp(8203).customRules.lheading;
    }
    const merged11 = Object.assign(lheading);
    obj.lheading = obj9;
    const obj10 = {};
    const merged12 = Object.assign(heading);
    if (typeof tmp(8203).customRules.heading === "function") {
      const customRules6 = tmp(8203).customRules;
      heading = customRules6.heading(dependencyMap);
    } else {
      heading = tmp(8203).customRules.heading;
    }
    const merged13 = Object.assign(heading);
    obj.heading = obj10;
    const obj11 = {};
    const merged14 = Object.assign(blockQuote);
    if (typeof tmp(8203).customRules.blockQuote === "function") {
      const customRules7 = tmp(8203).customRules;
      blockQuote = customRules7.blockQuote(dependencyMap);
    } else {
      blockQuote = tmp(8203).customRules.blockQuote;
    }
    const merged15 = Object.assign(blockQuote);
    obj.blockQuote = obj11;
    const obj12 = {};
    const merged16 = Object.assign(paragraph);
    if (typeof tmp(8203).customRules.paragraph === "function") {
      const customRules8 = tmp(8203).customRules;
      paragraph = customRules8.paragraph(dependencyMap);
    } else {
      paragraph = tmp(8203).customRules.paragraph;
    }
...
let lheading = _modDef4336.defaultRules.lheading;
let heading = _modDef4336.defaultRules.heading;
let link = _modDef4336.defaultRules.link;
let image = _modDef4336.defaultRules.image;
let list = _modDef4336.defaultRules.list;
let blockQuote = _modDef4336.defaultRules.blockQuote;
let paragraph = _modDef4336.defaultRules.paragraph;
const re10 = /\{(.+?)}/;
const re11 = /^\$(\w+?)\$/;
const size = fn(2);
...
export default {
  ...
  getSpecialRules(dependencyMap) {
    ...
    if (typeof require("utils/ChangeLogUtils").customRules.lheading === "function") {
      const customRules = tmp3(8203).customRules;
      lheading = customRules.lheading(dependencyMap);
    } else {
      lheading = tmp3(8203).customRules.lheading;
    }
  ...
  getMessageRules(dependencyMap) {
    const obj2 = {};
    const merged = Object.assign(defaultRules(dependencyMap));
    const merged1 = Object.assign(_modDef4336.defaultRules.newline);
    obj2.newline = {};
