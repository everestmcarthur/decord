...
// Function ID: 4296
// Name: PremiumUtils
// Dependencies: [32, 19, 4296, 1372, 4297, 4298, 4300, 4301, 1074, 1374, 4309, 1085, 3, 4310, 4307, 1886, 38, 1115, 3075, 4229, 4319, 4326, 7344, 4308, 13560, 1091, 504, 11588, 5215, 4534, 14086, 1378, 1380, 9502, 2]
...
import PremiumSubscription from "PremiumSubscription" /* 4307 */;
...
import CheckoutError from "CheckoutError" /* 11588 */;
import useFPDurationLeft from "useFPDurationLeft" /* 13560 */;
import ProductCatalog from "ProductCatalog" /* 14086 */;
...
function getDisplayName(planId, arg1, arg2, duration) {
  ...
  if (SubscriptionPlans.PREMIUM_MONTH_TIER_0 === planId) {
    ...
    if (flag2) {
      ...
      stringResult = intl10.formatToPlainString(_modDef3075["8bPDtb"], obj5);
    }
    return stringResult;
    ...
    if (tmp24) {
      ...
      if (tmp24) {
        ...
        if (SubscriptionPlans.PREMIUM_MONTH_TIER_0 !== planId) {
          if (tmp46.PREMIUM_YEAR_TIER_0 !== planId) {
              ...
              if (tmp46.PREMIUM_YEAR_TIER_1 !== planId) {
                ...
                if (tmp17.CANCELED === CANCELED) {
                  ...
                  if (null != subscription.pauseEndsAt) {
                    diffResult = _modDef4229(subscription.pauseEndsAt).diff(subscription.currentPeriodEnd, "days");
                    const obj19 = _modDef4229(subscription.pauseEndsAt);
                  }
                  if (null != diffResult) {
                    const intl11 = util.intl;
                  ...
                  const obj16 = _modDef4229(subscription.currentPeriodStart);
                  obj15.endDate = _modDef4229(subscription.currentPeriodStart).add(__initData7, "days").toDate();
            ...
            if (tmp17.CANCELED === CANCELED) {
              ...
              if (null != subscription.pauseEndsAt) {
                diffResult1 = _modDef4229(subscription.pauseEndsAt).diff(subscription.currentPeriodEnd, "days");
                const obj32 = _modDef4229(subscription.pauseEndsAt);
              }
              if (null != diffResult1) {
        ...
        if (tmp17.CANCELED === CANCELED) {
          ...
          if (null != subscription.pauseEndsAt) {
            diffResult2 = _modDef4229(subscription.pauseEndsAt).diff(subscription.currentPeriodEnd, "days");
            const obj42 = _modDef4229(subscription.pauseEndsAt);
          }
          if (null != diffResult2) {
...
function getBillingGracePeriodDaysAndExpiresDate(subscription) {
  if (subscription.isPurchasedViaApple) {
    ...
    if (null != prop) {
      const obj11 = _modDef4229(subscription.metadata.apple_grace_period_expires_date);
      const obj3 = { days: null, expiresDate: null };
      obj3.days = _modDef4229.duration(obj11.diff(subscription.currentPeriodStart)).days();
  ...
  if (subscription.isPurchasedViaGoogle) {
    ...
    if (null != prop1) {
      ...
      if (null != prop2) {
        const obj7 = _modDef4229(subscription.metadata.google_grace_period_expires_date);
        const obj4 = { days: null, expiresDate: null };
        const tmp20 = _modDef4229(subscription.metadata.google_original_expires_date);
        obj4.days = _modDef4229.duration(obj7.diff(tmp20)).days();
  ...
  if (subscription.isPurchasedExternally) {
    ...
    const obj5 = { days: tmp15, expiresDate: _modDef4229(subscription.currentPeriodStart).add(tmp15, "days") };
    return obj5;
  } else {
    ...
    if (null != prop3) {
      ...
      obj8.days = _modDef4229(prop4).diff(subscription.currentPeriodStart, "days");
      obj8.expiresDate = tmp11(4229)(subscription.metadata.grace_period_expires_date);
      return obj8;
    } else {
      const tmp8 = null == subscription.paymentSourceId ? closure_1_19 : __initData8;
      obj = { days: tmp8, expiresDate: _modDef4229(subscription.currentPeriodStart).add(tmp8, "days") };
      return obj;
    }
  }
}
...
function getBillingInformationString(status, subscriptionPeriodStart, first1, flag, fractionalPremiumInfo) {
  ...
  if (fractionalPremiumInfo === undefined) {
    obj = { isFractionalPremiumActive: false, fetched: true, fractionalState: constants6.NONE, startsAt: _modDef4229(0), endsAt: _modDef4229(0), currentEntitlementId: "", currentEntitlementEndsAt: _modDef4229(0), unactivatedUnits: [] };
    tmp2 = obj;
  }
  ...
  if (isOnReverseTrialResult) {
    ...
    if (isPurchasedViaGoogle) {
      expiresDate = _modDef4229(status.metadata.google_grace_period_expires_date);
    }
    ...
    if (isPurchasedViaApple) {
      expiresDate = _modDef4229(status.metadata.apple_grace_period_expires_date);
    }
    ...
    const obj18 = _modDef4229(status.currentPeriodStart);
    obj13.endDate = _modDef4229(status.currentPeriodStart).add(__initData7, "days").toDate();
    ...
    if (status.isPurchasedViaGoogle) {
      if (!tmp14Result.isGooglePlayBillingSupported()) {
        ...
        const obj12 = _modDef4229(status.currentPeriodStart);
        obj14.endDate = _modDef4229(status.currentPeriodStart).add(dependencyMap, "days").toDate();
        ...
        const addResult1 = _modDef4229(status.currentPeriodStart).add(dependencyMap, "days");
      }
      return formatResult2;
    }
    const intl7 = tmp14(1115).intl;
    const obj16 = { endDate: null, price: null };
    const obj15 = _modDef4229(status.currentPeriodStart);
    obj16.endDate = _modDef4229(status.currentPeriodStart).add(dependencyMap, "days").toDate();
    obj16.price = formatPriceResult;
    formatResult2 = intl7.format(tmp14(1115).t.EMTLOT, obj16);
    const addResult2 = _modDef4229(status.currentPeriodStart).add(dependencyMap, "days");
  } else {
...
function extendDateWithUnconsumedFractionalPremium(date, unactivatedFractionalPremiumUnits, diff, excludeReverseTrialFromCountdown) {
  ...
  const tmp3Result = _modDef4229(tmp4);
  ...
  if (!excludeReverseTrialFromCountdown) {
    ...
    if (undefined !== diff) {
      const diffResult = diff.diff(_modDef4229(), "hours", true);
      addResult1 = addResult;
      if (diffResult > 0) {
...
function getBillingReviewSubheader(arg0, id, arg2) {
  ...
  if (SubscriptionPlans.PREMIUM_MONTH_TIER_0 === id) {
    ...
    return intl4.formatToPlainString(_modDef3075.LwdrNi, obj2);
...
let closure_47 = fn(4309).getPremiumGroupProductName;
Constants = fn(1085);
...
const frozen = Object.freeze({
  isNewUser,
  isPremiumAtLeast: fn(1886).isPremiumAtLeast,
  isPremium: fn(1886).isPremium,
  isPremiumExactly: fn(1886).isPremiumExactly,
  isPremiumEligible,
  getPrice,
  getDefaultPrice,
  getInterval,
  ...
  getUserMaxFileSize: fn(9502).getUserMaxFileSize,
  ...
  canUseShopDiscounts(currentUser) {
    ...
    if (!hasPerkResult) {
      hasPerkResult = tmp(14086).canUserUse(tmp(14086).COLLECTIBLES, currentUser);
      const tmpResult = tmp(14086);
    }
  ...
  canUseMoreQuestOrbs(perks) {
    ...
    if (!hasPerkResult) {
      hasPerkResult = tmp(14086).canUserUse(tmp(14086).QUEST_ORB_MULTIPLIER, perks);
      const tmpResult = tmp(14086);
    }
    return hasPerkResult;
  },
...
export const isPremiumAtLeast = fn(1886).isPremiumAtLeast;
export const isPremiumAtMost = fn(1886).isPremiumAtMost;
export const isPremium = fn(1886).isPremium;
export const isPremiumExactly = fn(1886).isPremiumExactly;
