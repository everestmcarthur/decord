...
// Function ID: 4263
// Name: PremiumUtils
// Dependencies: [32, 19, 4263, 1371, 4264, 4265, 4267, 4268, 1074, 1373, 4276, 1085, 3, 4277, 4274, 1885, 38, 1114, 3074, 4196, 4286, 4293, 7286, 4275, 13537, 1090, 504, 11514, 5180, 4502, 14055, 1377, 1379, 9429, 2]
              ...
              if (tmp46.PREMIUM_YEAR_TIER_1 !== planId) {
                ...
                if (tmp17.CANCELED === CANCELED) {
                  ...
                  if (null != subscription.pauseEndsAt) {
                    diffResult = _modDef4196(subscription.pauseEndsAt).diff(subscription.currentPeriodEnd, "days");
                    const obj19 = _modDef4196(subscription.pauseEndsAt);
                  }
                  if (null != diffResult) {
                    const intl11 = util.intl;
                  ...
                  const obj16 = _modDef4196(subscription.currentPeriodStart);
                  obj15.endDate = _modDef4196(subscription.currentPeriodStart).add(__initData7, "days").toDate();
...
function getBillingGracePeriodDaysAndExpiresDate(subscription) {
  if (subscription.isPurchasedViaApple) {
    ...
    if (null != prop) {
      ...
      obj3.days = _modDef4196.duration(obj11.diff(subscription.currentPeriodStart)).days();
  ...
  if (subscription.isPurchasedViaGoogle) {
    ...
    if (null != prop1) {
      ...
      if (null != prop2) {
        ...
        obj4.days = _modDef4196.duration(obj7.diff(tmp20)).days();
  ...
  if (subscription.isPurchasedExternally) {
    ...
    const obj5 = { days: tmp15, expiresDate: _modDef4196(subscription.currentPeriodStart).add(tmp15, "days") };
    return obj5;
  } else {
    ...
    if (null != prop3) {
      ...
      obj8.days = _modDef4196(prop4).diff(subscription.currentPeriodStart, "days");
      obj8.expiresDate = tmp11(4196)(subscription.metadata.grace_period_expires_date);
      return obj8;
    } else {
      const tmp8 = null == subscription.paymentSourceId ? closure_1_19 : __initData8;
      obj = { days: tmp8, expiresDate: _modDef4196(subscription.currentPeriodStart).add(tmp8, "days") };
      return obj;
    }
  }
}
...
function getBillingInformationString(status, subscriptionPeriodStart, first1, flag, fractionalPremiumInfo) {
  ...
  if (fractionalPremiumInfo === undefined) {
    obj = { isFractionalPremiumActive: false, fetched: true, fractionalState: constants6.NONE, startsAt: _modDef4196(0), endsAt: _modDef4196(0), currentEntitlementId: "", currentEntitlementEndsAt: _modDef4196(0), unactivatedUnits: [] };
    tmp2 = obj;
  }
  ...
  if (isOnReverseTrialResult) {
    ...
    obj13.endDate = _modDef4196(status.currentPeriodStart).add(__initData7, "days").toDate();
    ...
    if (status.isPurchasedViaGoogle) {
      if (!tmp14Result.isGooglePlayBillingSupported()) {
        ...
        const obj12 = _modDef4196(status.currentPeriodStart);
        obj14.endDate = _modDef4196(status.currentPeriodStart).add(dependencyMap, "days").toDate();
        ...
        const addResult1 = _modDef4196(status.currentPeriodStart).add(dependencyMap, "days");
      }
      return formatResult2;
    }
    const intl7 = tmp14(1114).intl;
    const obj16 = { endDate: null, price: null };
    const obj15 = _modDef4196(status.currentPeriodStart);
    obj16.endDate = _modDef4196(status.currentPeriodStart).add(dependencyMap, "days").toDate();
    obj16.price = formatPriceResult;
    formatResult2 = intl7.format(tmp14(1114).t.EMTLOT, obj16);
    const addResult2 = _modDef4196(status.currentPeriodStart).add(dependencyMap, "days");
  } else {
...
const frozen = Object.freeze({
  ...
  canUseShopDiscounts(currentUser) {
    ...
    if (!hasPerkResult) {
      hasPerkResult = tmp(14055).canUserUse(tmp(14055).COLLECTIBLES, currentUser);
      const tmpResult = tmp(14055);
    }
  ...
  canUseMoreQuestOrbs(perks) {
    ...
    if (!hasPerkResult) {
      hasPerkResult = tmp(14055).canUserUse(tmp(14055).QUEST_ORB_MULTIPLIER, perks);
      const tmpResult = tmp(14055);
    }
    return hasPerkResult;
  },
