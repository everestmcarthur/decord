...
// Function ID: 5220
// Name: FileUtils
// Dependencies: [1980, 1372, 1074, 1374, 12, 5220, 4563, 4295, 5214, 4534, 1115, 2]
...
// Module 5219 (FileUtils)
import _modDef12 from "module_12" /* 12 */;
import UploadUtils from "UploadUtils" /* 5214 */;
import noConflictDefault from "noConflict" /* 5220 */;
...
export const maxFileSize = function maxFileSize(guildId) {
  ...
  const userMaxFileSize = enabled(4295).getUserMaxFileSize(currentUser);
  if (null == guildId) {
    return userMaxFileSize;
  } else {
    const guild = GuildStore.getGuild(guildId);
    if (null != guild) {
      const FileUploadPowerupHoldoutExperiment = guild(4563).FileUploadPowerupHoldoutExperiment;
...
export const fileUploadLimitRoadblockDescription = function fileUploadLimitRoadblockDescription(arg0) {
  ...
  if (maxSize == null) {
    ...
    if (null == guildId) {
      ...
      if (null != _Math) {
        const FileUploadPowerupHoldoutExperiment = tmp(4563).FileUploadPowerupHoldoutExperiment;
        enabled = FileUploadPowerupHoldoutExperiment.getConfig({ location: "getGuildMaxFileSize" }).enabled;
        let reduced = items2.reduce((acc, item) => {
          [tmp, tmp2] = item;
          if (!enabled) {
    ...
    const obj5 = enabled(4295);
  }
  const formatSizeResult = _Math(4534).formatSize(maxSize / 1024, { useKibibytes: true });
  const obj = _Math(4534);
  const maxFileSizeForPremiumType = _Math(4295).getMaxFileSizeForPremiumType(PremiumTypes.TIER_2, { useSpace: false });
