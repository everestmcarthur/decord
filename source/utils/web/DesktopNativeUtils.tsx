...
// Dependencies: [32, 5, 1074, 38, 4537, 1115, 510, 5617, 1934, 4, 5618, 5619, 1365, 1272, 4632, 2]

// Module 5616 (DesktopNativeUtils)
import logger_Logger from "logger/Logger" /* 4 */;
import Storage3 from "Storage" /* 510 */;
import GameDetectionTypes from "GameDetectionTypes" /* 1934 */;
import Client from "Client" /* 4537 */;
import discord_common_DiscordNative from "discord_common/DiscordNative" /* 4632 */;
import DomainMigrationUtils from "DomainMigrationUtils" /* 5617 */;
import IPCEvents from "IPCEvents" /* 5618 */;
import _slicedToArray from "module_32" /* 32 */;
import asyncGeneratorStep from "asyncGeneratorStep" /* 5 */;

const FileExtensionUtils = tmp2(5619);
require = fn;
...
obj2.setBadge = function setBadge(arg0) {
  if ("darwin" === obj.getPlatformName()) {
    ...
    if ("win32" === tmpResult.getPlatformName()) {
      const self = this;
      this.sendIPC(tmp(5618).IPCEvents.APP_BADGE_SET, arg0);
    } else {
      ...
      try {
        ...
        if (0 === c3) {
          ...
          if (arg0 === 1) {
            ...
            closure_129_0 = value;
            closure_129_1 = closure_0(5619).decideFileExtension(closure_130_0, closure_130_1);
...
obj2.canSaveImage = function canSaveImage(uri, contentType) {
  if (null != uri) {
    if (require("PlatformUtils").isPlatformEmbedded) {
      const decideFileExtensionResult = tmp(5619).decideFileExtension(uri, contentType);
          ...
          if (arg0 === 1) {
            ...
            if (null == toURLSafeResult) {
              ...
              if (null != str2) {
                ...
                const decideFileExtensionResult = unknown(5619).decideFileExtension(tmp54, closure_1);
...
obj2.setTrafficLightPosition = function setTrafficLightPosition(arg0) {
  if (require("PlatformUtils").isPlatformEmbedded) {
    if ("darwin" === tmpResult.getPlatformName()) {
      try {
        const self = this;
        this.sendIPC(tmp(5618).IPCEvents.WINDOW_SET_TRAFFIC_LIGHT_POSITION, arg0);
      } catch (err) {
      }
    }
    tmpResult = tmp(1115);
