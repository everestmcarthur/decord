...
// Dependencies: [8520, 1958, 1980, 8521, 4861, 4288, 2012, 1074, 4336, 4984, 1115, 1241, 8522, 8523, 8524, 8526, 4328, 13100, 13104, 5088, 13105, 13107, 2]
// Exports: handleClick, isLinkTrusted

// Module 8519 (MaskedLinkUtils)
import openURLDefault from "openURL" /* 4328 */;
import LinkAnalyticsUtilsDefault from "LinkAnalyticsUtils" /* 8524 */;
import BlockedDomainStore from "BlockedDomainStore" /* 8520 */;
import ChannelStore from "ChannelStore" /* 1958 */;
import GuildStore from "GuildStore" /* 1980 */;
import MaskedLinkStore from "MaskedLinkStore" /* 8521 */;
...
export const handleClick = function handleClick(href, preventDefault, arg2, contextKey) {
  ...
  if (null == sanitizeUrlResult) {
    ...
    try {
      ...
      tmp3(8524).trackLinkClicked(tmp8);
      if (null == onClick) {
      ...
      if (null !== guild_id1.isBlockedDomain(tmp8)) {
        ...
        if (!tmp55) {
          ...
          if (tmp56) {
            const obj8 = { url: tmp8, trustUrl: tmp53(13107).trustProtocol, onConfirm: handleConfirm, onCancel, isProtocol: true, contextKey };
            tmp3(13105).show(obj8);
            const tmp3Result12 = tmp3(13105);
          } else {
            const punycodeLinkResult = tmp53(5088).punycodeLink(tmp8);
            let displayTarget = tmp8;
            if (null != punycodeLinkResult) {
              displayTarget = punycodeLinkResult.displayTarget;
            }
            const tmp53Result = tmp53(5088);
            const obj9 = { url: displayTarget, trustUrl: tmp53(13107).trustDomain, onConfirm: handleConfirm, onCancel, isProtocol: false, contextKey };
            tmp3(13105).show(obj9);
            const tmp3Result13 = tmp3(13105);
          }
        }
        if (null == preventDefault) {
          handleConfirm();
          ...
          let result = tmp3(8524).trackAnnouncementMessageLinkClicked(obj11);
          const tmp3Result14 = tmp3(8524);
        }
        const obj10 = require("MaskedLinkStoreMethodsAdditional");
      }
      const tmp3Result10 = tmp3(8524);
    } catch (err) {
