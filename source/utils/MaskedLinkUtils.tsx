...
// Dependencies: [8603, 1958, 1980, 8604, 4943, 4371, 2012, 1074, 2075, 5066, 1115, 1241, 8605, 8606, 8607, 8609, 4411, 13191, 13195, 5170, 13196, 13198, 2]
// Exports: handleClick, isLinkTrusted

// Module 8602 (MaskedLinkUtils)
import openURLDefault from "openURL" /* 4411 */;
import LinkAnalyticsUtilsDefault from "LinkAnalyticsUtils" /* 8607 */;
import BlockedDomainStore from "BlockedDomainStore" /* 8603 */;
import ChannelStore from "ChannelStore" /* 1958 */;
import GuildStore from "GuildStore" /* 1980 */;
import MaskedLinkStore from "MaskedLinkStore" /* 8604 */;
...
export const handleClick = function handleClick(href, preventDefault, arg2, contextKey) {
  ...
  const sanitizeUrlResult = channelId(2075).sanitizeUrl(href.href);
  if (null == sanitizeUrlResult) {
    ...
    try {
      ...
      tmp3(8607).trackLinkClicked(tmp8);
      if (null == onClick) {
      ...
      if (null !== guild_id1.isBlockedDomain(tmp8)) {
        ...
        if (!tmp55) {
          ...
          if (tmp56) {
            const obj8 = { url: tmp8, trustUrl: tmp53(13198).trustProtocol, onConfirm: handleConfirm, onCancel, isProtocol: true, contextKey };
            tmp3(13196).show(obj8);
            const tmp3Result12 = tmp3(13196);
          } else {
            const punycodeLinkResult = tmp53(5170).punycodeLink(tmp8);
            let displayTarget = tmp8;
            if (null != punycodeLinkResult) {
              displayTarget = punycodeLinkResult.displayTarget;
            }
            const tmp53Result = tmp53(5170);
            const obj9 = { url: displayTarget, trustUrl: tmp53(13198).trustDomain, onConfirm: handleConfirm, onCancel, isProtocol: false, contextKey };
            tmp3(13196).show(obj9);
            const tmp3Result13 = tmp3(13196);
          }
        }
        if (null == preventDefault) {
          handleConfirm();
          ...
          let result = tmp3(8607).trackAnnouncementMessageLinkClicked(obj11);
          const tmp3Result14 = tmp3(8607);
        }
        const obj10 = require("MaskedLinkStoreMethodsAdditional");
      }
      const tmp3Result10 = tmp3(8607);
    } catch (err) {
