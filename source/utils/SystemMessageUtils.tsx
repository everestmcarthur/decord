...
// Dependencies: [32, 4223, 502, 1957, 1979, 4222, 1371, 1074, 1114, 11, 8001, 8005, 4726, 4727, 8006, 8008, 4807, 4797, 8009, 8010, 7530, 2]

// Module 8000 (SystemMessageUtils)
...
import AutomodNotificationEmbedTypeKeys from "AutomodNotificationEmbedTypeKeys" /* 8005 */;
import GuildRoleSubscriptionSystemMessageUtils from "GuildRoleSubscriptionSystemMessageUtils" /* 8006 */;
import GuildProductSystemMessageUtils from "GuildProductSystemMessageUtils" /* 8008 */;
import ApplicationSubscriptionSystemMessageUtils from "ApplicationSubscriptionSystemMessageUtils" /* 8009 */;
import PrivateChannelIntegrationSystemMessageUtils from "PrivateChannelIntegrationSystemMessageUtils" /* 8010 */;
    ...
    if (null == first) {
      ...
      if (constants.RECIPIENT_ADD === type) {
        if (null != null) {
          ...
          obj2.otherUsername = tmp6(4726).getName(null, channel_id, null);
                  ...
                  if (null != channel) {
                    ...
                    if (null != guild) {
                      if (AutomodNotificationEmbedTypeKeys.AutomodNotificationEmbedTypeKeys.ACTIVITY_ALERTS_ENABLED === value) {
                        ...
                        astToStringResult6 = tmp34(8001).astToString(intl5.formatToParts(tmp34(1114).t.wt3ZUM, obj33));
                        const tmp34Result = tmp34(8001);
                      } else if (tmp34(8005).AutomodNotificationEmbedTypeKeys.INTERACTION_BLOCKED === value) {
                        const intl4 = tmp34(1114).intl;
                        const obj36 = { guildName: guild.name };
                        astToStringResult6 = tmp34(8001).astToString(intl4.formatToParts(tmp34(1114).t.AkqI0g, obj36));
                        const tmp34Result3 = tmp34(8001);
                      } else {
                        const intl3 = tmp34(1114).intl;
                        const obj37 = { guildName: guild.name };
                        astToStringResult6 = tmp34(8001).astToString(intl3.formatToParts(tmp34(1114).t["a+lJKl"], obj37));
                        const tmp34Result4 = tmp34(8001);
                      }
                    }
                  }
