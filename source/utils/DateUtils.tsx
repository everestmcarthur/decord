...
function syncHourCycleToIntlConfig() {
  ...
  if (result) {
    result = tmp4(4321).supportsSystemDateFormatter();
    const tmp4Result = tmp4(4321);
  }
...
export const calendarFormat = function calendarFormat(timestamp, arg1, setting) {
  ...
  const localeDataResult = _modDef4228.localeData();
...
export const accessibilityLabelCalendarFormat = function accessibilityLabelCalendarFormat(timestamp) {
  ...
  let str5 = "LLL";
  if ("sameElse" !== str) {
    str5 = localeDataResult.calendar(str, tmp(4228)(timestamp), tmp(4228)(date));
    const tmp6 = tmp(4228)(timestamp);
  }
...
export const formatDateForDatetimeLocal = function formatDateForDatetimeLocal(arg0) {
  ...
  if (null != arg0) {
    if (arg0.length >= 200) {
      ...
      str = _modDef4228(arg0).format("YYYY-MM-DDTHH:mm");
