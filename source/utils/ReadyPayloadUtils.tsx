...
export const preloadReadyPayloadData = function preloadReadyPayloadData() {
  ...
  if (obj2.isCacheEnabled()) {
    let committedVersions = tmp(7761).getCommittedVersions();
    const tmpResult = tmp(7761);
  } else {
    committedVersions = Promise.resolve({});
  }
  obj2 = isCacheEnabled;
  if (tmp4Result.isCacheEnabled()) {
    let guildIds = tmp(2008).getGuildIds();
    const tmpResult3 = tmp(2008);
  } else {
    const _Set = Set;
    const set = new Set();
  ...
  if (null != databaseResult) {
    let okAsyncResult = tmp(7762).okAsync(databaseResult);
    const tmpResult4 = tmp(7762);
  } else {
...
export const hydrateReadyPayloadPrioritized = function hydrateReadyPayloadPrioritized(arg0, identifyStartTime, databaseOk) {
  ...
  if (tmp4) {
    const result = tmp2(2004).replaceDisableAllDatabases("ReadyPayloadUtils: database was not ok");
    const tmp2Result = tmp2(2004);
  }
