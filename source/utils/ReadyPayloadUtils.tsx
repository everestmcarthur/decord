...
export const preloadReadyPayloadData = function preloadReadyPayloadData() {
  ...
  if (obj2.isCacheEnabled()) {
    let committedVersions = tmp(7704).getCommittedVersions();
  ...
  if (tmp4Result.isCacheEnabled()) {
    let guildIds = tmp(2007).getGuildIds();
    const tmpResult3 = tmp(2007);
  } else {
    const _Set = Set;
    const set = new Set();
  ...
  if (null != databaseResult) {
    let okAsyncResult = tmp(7705).okAsync(databaseResult);
    const tmpResult4 = tmp(7705);
  } else {
...
export const hydrateReadyPayloadPrioritized = function hydrateReadyPayloadPrioritized(arg0, identifyStartTime, databaseOk) {
  ...
  if (tmp4) {
    const result = tmp2(2003).replaceDisableAllDatabases("ReadyPayloadUtils: database was not ok");
    const tmp2Result = tmp2(2003);
  }
