...
// Dependencies: [32, 5, 19, 17, 1976, 1958, 2016, 1980, 4276, 4286, 1372, 1074, 21, 4638, 576, 1484, 1612, 504, 11753, 11752, 4281, 1086, 9880, 4651, 1895, 4482, 4982, 1115, 4332, 2025, 1966, 17040, 7978, 4634, 4791, 11092, 5691, 1177, 5773, 17044, 1364, 17045, 2]
// Exports: default

// Module 17039 (ChannelSettingsPermissionsOverrides)
import nativeDefault from "native" /* 576 */;
import BigFlagUtilsAll from "BigFlagUtils" /* 1086 */;
import StageChannelPermissions from "StageChannelPermissions" /* 1966 */;
import PermissionUtils from "PermissionUtils" /* 4281 */;
import TableRowGroup from "TableRowGroup" /* 5773 */;
import AppChannelPermissionUtils from "AppChannelPermissionUtils" /* 11752 */;
import PermissionSpecUtilsDefault from "PermissionSpecUtils" /* 17040 */;
...
export default function ChannelSettingsPermissionsOverrides(fromCreate) {
    ...
    if (fromCreate) {
      let obj = {
        headerRight() {
            ...
            return closure_2_19(closure_0(7978).HeaderTextButton, obj);
          }
      };
  ...
  if (fromCreate.type === constants.MEMBER) {
    ...
    let tmp12Result = tmp12(tmp5(11092), obj6);
  } else {
    ...
    tmp12Result = tmp12(tmp2(5691).TableRow, obj7);
  }
  ...
  items6[2] = first.map((permissions, index) => {
    ...
    obj.children = closure_2_19(TableRowGroup.TableRowGroup, {
      ...
      children: permissions.map((description, index) => {
        ...
        const obj = { variant: "text-xs/medium", color: "text-subtle", children: closure_1_0(17044).renderDescription(description.description) };
        const items = [closure_1_19(closure_1_0(4634).Text, obj), ];
        let tmp5Result = null;
        if (false !== tmp) {
          tmp5Result = null;
          if ("" !== tmp) {
            const obj3 = { variant: "text-xs/medium", color: "text-feedback-critical", children: tmp };
            tmp5Result = tmp5(tmp6(4634).Text, obj3);
          }
        }
        items[1] = tmp5Result;
        const obj2 = closure_1_0(17044);
        ...
        obj4.trailing = closure_1_19(id(17045), obj5);
        return closure_1_19(closure_1_0(5691).TableRow, obj4, "row-" + index);
      })
    });
