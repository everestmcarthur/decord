...
export default function RolePill(role) {
  ...
  const roleIconProps = role(7289).useRoleIconProps({ guildId, roleId: role.id, size: 12 });
  let name = role.name;
  let combined = name;
  ...
  if (undefined !== guild_connections) {
    ...
    let tmp10Result = tmp10(combined(7306), obj5);
    const tmp15 = combined(7306);
  } else {
  ...
  if (null != roleIconProps) {
    ...
    obj9.children = tmp10(combined(7308), obj10);
    tmp10Result2 = tmp10(tmp12, obj9);
    const tmp18 = combined(7308);
  }
  items1[1] = tmp10Result2;
  items1[2] = closure_6(role(4632).Text, { variant: "text-xs/semibold", color: "interactive-text-active", children: name });
  obj4.children = items1;
  obj3.children = closure_7(View, obj4);
  return closure_6(role(5204).PressableHighlight, obj3);
};
