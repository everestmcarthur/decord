...
// Dependencies: [19, 21, 4341, 576, 1612, 7257, 7256, 7305, 6732, 5770, 5773, 2]
// Exports: default

// Module 9558 (ItemSelectorActionSheet)
...
export default function ItemSelectorActionSheet(arg0) {
  ...
  ({ body, hasIcons } = arg0);
  const token = items(4341).useToken(nativeDefault.modules.mobile.TABLE_ROW_PADDING);
  ...
  if (null != onClose) {
    ...
    tmp6Result = tmp6(tmp(7305).ActionSheetCloseButton, obj3);
  }
  const obj4 = { scrollable: true, header: closure_3(items(7256).BottomSheetTitleHeader, obj2), children: null };
  obj2.trailing = tmp6Result;
  const obj5 = { contentContainerStyle: null, children: null };
  const obj = items(4341);
  ...
  items1[1] = closure_3(items(5770).TableRadioGroup, {
    value: num,
    ...
    children: items.map((label, value) => closure_1_3(items(5773).TableRadioRow, { label: label.label, value }, value))
  });
  obj5.children = items1;
  obj4.children = tmp8(items(6732).BottomSheetScrollView, obj5);
  return closure_3(items(7257).BottomSheet, obj4);
};
