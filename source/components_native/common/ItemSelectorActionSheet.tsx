...
// Dependencies: [19, 21, 4306, 576, 1611, 7202, 7201, 7250, 6677, 5735, 5738, 2]
// Exports: default

// Module 9496 (ItemSelectorActionSheet)
...
export default function ItemSelectorActionSheet(arg0) {
  ...
  ({ body, hasIcons } = arg0);
  const token = items(4306).useToken(nativeDefault.modules.mobile.TABLE_ROW_PADDING);
  ...
  if (null != onClose) {
    ...
    tmp6Result = tmp6(tmp(7250).ActionSheetCloseButton, obj3);
  }
  const obj4 = { scrollable: true, header: closure_3(items(7201).BottomSheetTitleHeader, obj2), children: null };
  obj2.trailing = tmp6Result;
  const obj5 = { contentContainerStyle: null, children: null };
  const obj = items(4306);
  ...
  items1[1] = closure_3(items(5735).TableRadioGroup, {
    value: num,
    ...
    children: items.map((label, value) => closure_1_3(items(5738).TableRadioRow, { label: label.label, value }, value))
  });
  obj5.children = items1;
  obj4.children = tmp8(items(6677).BottomSheetScrollView, obj5);
  return closure_3(items(7202).BottomSheet, obj4);
};
