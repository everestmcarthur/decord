...
// Dependencies: [19, 21, 4339, 576, 1612, 7260, 7259, 7308, 6735, 5771, 5774, 2]
// Exports: default

// Module 9569 (ItemSelectorActionSheet)
...
export default function ItemSelectorActionSheet(arg0) {
  ...
  ({ body, hasIcons } = arg0);
  const token = items(4339).useToken(nativeDefault.modules.mobile.TABLE_ROW_PADDING);
  ...
  if (null != onClose) {
    ...
    tmp6Result = tmp6(tmp(7308).ActionSheetCloseButton, obj3);
  }
  const obj4 = { scrollable: true, header: closure_3(items(7259).BottomSheetTitleHeader, obj2), children: null };
  obj2.trailing = tmp6Result;
  const obj5 = { contentContainerStyle: null, children: null };
  const obj = items(4339);
  ...
  items1[1] = closure_3(items(5771).TableRadioGroup, {
    value: num,
    ...
    children: items.map((label, value) => closure_1_3(items(5774).TableRadioRow, { label: label.label, value }, value))
  });
  obj5.children = items1;
  obj4.children = tmp8(items(6735).BottomSheetScrollView, obj5);
  return closure_3(items(7260).BottomSheet, obj4);
};
