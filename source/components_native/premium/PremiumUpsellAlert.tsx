...
// Dependencies: [32, 19, 17, 1185, 1371, 1074, 9355, 1373, 21, 4574, 576, 4284, 4570, 7461, 1114, 4231, 9346, 9347, 4509, 4425, 9382, 9383, 9384, 9385, 9336, 9386, 9387, 9388, 9389, 504, 7214, 9390, 9345, 1477, 7176, 7196, 5006, 1242, 9391, 4471, 5008, 1178, 5601, 9392, 9393, 9349, 2]

// Module 9354 (PremiumUpsellAlert)
...
import createStyles2 from "createStyles" /* 4574 */;
...
function GlobalEmojiUpsell(arg0) {
  ...
  if (null != skuId) {
    if (TIER_0.TIER_0 === skuId) {
      ...
      const obj2 = { planName: tmp(4231).getPremiumTypeDisplayName(map1.TIER_0) };
      ...
      if (tmp8.TIER_2 === skuId) {
        ...
        const obj3 = { planName: tmp(4231).getPremiumTypeDisplayName(map1.TIER_2) };
        formatResult1 = intl5.format(tmp(1114).t["1P7x8p"], obj3);
        const tmpResult3 = tmp(4231);
      }
    }
  } else {
    formatResult1 = formatResult;
    if (useTier0Description) {
      ...
      const obj4 = { planName: tmp(4231).getPremiumTypeDisplayName(map1.TIER_0) };
      formatResult1 = intl2.format(tmp(1114).t.kWBwlJ, obj4);
      const tmpResult4 = tmp(4231);
    }
  }
  const obj5 = { alertWidth, upsellItem: null };
  const obj6 = { image: _modDef9346, title: null, description: null };
...
function AnimatedEmojiUpsell(alertWidth) {
  ...
  const obj2 = { image: _modDef9347, title: null, description: null };
...
function PremiumGuildIdentityUpsell(alertWidth) {
  ...
  if (obj3.isThemeDark(tmp5)) {
    let tmp4Result = tmp4(9382);
  } else {
    tmp4Result = tmp4(9383);
  }
...
function LongerMessageUpsell(alertWidth) {
  ...
  if (obj3.isThemeDark(tmp5)) {
    let tmp4Result = tmp4(9386);
  } else {
    tmp4Result = tmp4(9387);
  }
  const obj4 = { image: tmp4Result, title: null, description: null };
  const intl = tmp(1114).intl;
...
function GuildCapUpsell(alertWidth) {
  ...
  const tmp7 = PremiumUpsellItem;
  if (obj3.isThemeDark(tmp5)) {
    let tmp4Result = tmp4(9388);
  } else {
    tmp4Result = tmp4(9389);
  }
  const obj4 = { image: tmp4Result, title: null, description: null };
  const intl = tmp(1114).intl;
...
function UploadUpsell(arg0) {
  ...
  if (_slicedToArray(noop.useState(!stateFromStores), 1)[0]) {
    ...
    tmp6Result = closure_1_14(tmp(7214).TableSwitchRow, obj2);
  }
