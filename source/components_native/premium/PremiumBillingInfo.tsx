...
// Dependencies: [32, 19, 17, 1074, 21, 4638, 576, 4308, 4634, 1115, 4295, 13490, 7272, 7292, 7514, 13494, 2]
// Exports: default
...
export default function PremiumBillingInfo(subscription) {
  ...
  if (null == first) {
    ...
    const externalManagementMessage = tmp2(7514).getExternalManagementMessage(subscription, { shouldAllowExternalManagement: true });
    ...
    const items = [timestampProducer(tmp2(4634).Text, obj4), , ];
    ...
    const items1 = [timestampProducer(tmp2(4634).Text, obj6), , ];
    const obj7 = { style: tmp.billingRenewalInfo, variant: "text-sm/medium", children: null };
    const tmp10 = View;
    const tmp11 = timestampProducer;
    const tmp2Result = tmp2(7514);
    const tmp9 = React5;
    obj7.children = tmp2(13494).getBillingInformationStringNative(subscription, first, tmp6);
    items1[1] = timestampProducer(tmp2(4634).Text, obj7);
    ...
    if (null != externalManagementMessage) {
      ...
      tmp11Result = tmp11(tmp2(4634).Text, obj9);
    }
    items[2] = tmp11Result;
    obj3.children = items;
