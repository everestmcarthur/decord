...
// Dependencies: [32, 19, 17, 21, 4606, 576, 4573, 1115, 7222, 7271, 9809, 1114, 5022, 4343, 1178, 4607, 4602, 5027, 4541, 7035, 4198, 7223, 9810, 4458, 2]
// Exports: default

// Module 9808 (DatePickerActionSheet)
...
import BottomSheetTitleHeader2 from "BottomSheetTitleHeader" /* 7222 */;
...
function ActionSheetHeader(handleSubmit) {
  ...
  if (isAndroidResult) {
    ...
    obj2.trailing = tmp4(tmp(7271).ActionSheetCloseButton, obj3);
    ...
    obj4.leading = tmp4(tmp(9809).ActionSheetHeaderPressableText, obj5);
    const obj6 = { onPress: handleSubmit.handleSubmit, label: null };
    const intl2 = tmp(1114).intl;
    obj6.label = intl2.string(tmp(1114).t["R3BPH+"]);
    obj4.trailing = tmp4(tmp(9809).ActionSheetHeaderPressableText, obj6);
...
function DateRangeError(children) {
  ...
  let obj = show(4343);
  const tmp4 = show;
  A.__closure = { STANDARD_EASING: show(1178).STANDARD_EASING, show, withTiming: show(4607).withTiming };
  ...
  let obj2 = { STANDARD_EASING: show(1178).STANDARD_EASING, show, withTiming: show(4607).withTiming };
  obj3.children = closure_6(View, { ref, accessible: true, accessibilityRole: "alert", style: tmp.rangeError, children: closure_6(tmp4(4602).Text, { variant: "text-md/medium", color: "text-feedback-critical", children: children.errorText }) });
  return closure_6(ref(4343).View, obj3);
}
function ActionSheetFooter(arg0) {
  ...
  if (obj.isAndroid()) {
    ...
    obj3.textElement = timestampProducer(tmp2(4602).Text, obj4);
    const intl2 = tmp2(1114).intl;
    obj3.accessibilityLabel = intl2.string(tmp2(1114).t["ETE/oC"]);
    obj3.style = tmp.actionButton;
    obj3.onPress = handleCancel;
    const items = [timestampProducer(tmp2(5027).BaseTextButton, obj3), ];
...
export default function DatePickerActionSheet(mode) {
  ...
  const tmp26 = minimumDate(7035)(() => {
    let tmp = first1;
    if (first1) {
      tmp = c9;
    }
    if (tmp) {
      dependencyMap(_modDef4198(ref.current));
    }
    if (c9) {
      ActionSheetActionCreatorsDefault.hideActionSheet();
    } else {
  ...
  const tmp27 = minimumDate(7035)((getTime) => {
    if (null != getTime) {
  ...
  const obj5 = minimumDate(4198)(minimumDate);
  obj3.errorText = intl2.formatToPlainString(maximumDate(1114).t.FsJO55, { minDate: minimumDate(4198)(minimumDate).format(str3) });
  ...
  const obj4 = { minDate: minimumDate(4198)(minimumDate).format(str3) };
  if ("date" === str) {
    str2 = "L";
  }
  const obj8 = minimumDate(4198)(maximumDate);
  obj6.errorText = intl3.formatToPlainString(maximumDate(1114).t.R7r9VN, { maxDate: minimumDate(4198)(maximumDate).format(str2) });
  items1[1] = closure_6(ref, obj6);
  const obj9 = { style: tmp3.datetimePickerContainer, children: null };
  const obj7 = { maxDate: minimumDate(4198)(maximumDate).format(str2) };
  const tmp38 = date;
  const tmp13Result = minimumDate(9810);
  let str4 = "dark";
  ...
  return tmp28(maximumDate(7223).BottomSheet, obj2);
};
