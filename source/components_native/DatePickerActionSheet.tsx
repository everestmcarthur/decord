...
// Dependencies: [32, 19, 17, 21, 4638, 576, 4605, 1364, 7259, 7308, 9858, 1115, 5054, 4374, 1177, 4639, 4634, 5061, 4572, 7072, 4229, 7260, 9859, 4489, 2]
// Exports: default

// Module 9857 (DatePickerActionSheet)
...
import BottomSheetTitleHeader2 from "BottomSheetTitleHeader" /* 7259 */;
...
function ActionSheetHeader(handleSubmit) {
  ...
  if (isAndroidResult) {
    ...
    obj2.trailing = tmp4(tmp(7308).ActionSheetCloseButton, obj3);
    ...
    obj4.leading = tmp4(tmp(9858).ActionSheetHeaderPressableText, obj5);
    const obj6 = { onPress: handleSubmit.handleSubmit, label: null };
    const intl2 = tmp(1115).intl;
    obj6.label = intl2.string(tmp(1115).t["R3BPH+"]);
    obj4.trailing = tmp4(tmp(9858).ActionSheetHeaderPressableText, obj6);
...
function DateRangeError(children) {
  ...
  let obj = show(4374);
  const tmp4 = show;
  A.__closure = { STANDARD_EASING: show(1177).STANDARD_EASING, show, withTiming: show(4639).withTiming };
  ...
  let obj2 = { STANDARD_EASING: show(1177).STANDARD_EASING, show, withTiming: show(4639).withTiming };
  obj3.children = closure_6(View, { ref, accessible: true, accessibilityRole: "alert", style: tmp.rangeError, children: closure_6(tmp4(4634).Text, { variant: "text-md/medium", color: "text-feedback-critical", children: children.errorText }) });
  return closure_6(ref(4374).View, obj3);
}
function ActionSheetFooter(arg0) {
  ...
  if (obj.isAndroid()) {
    ...
    obj3.textElement = timestampProducer(tmp2(4634).Text, obj4);
    const intl2 = tmp2(1115).intl;
    obj3.accessibilityLabel = intl2.string(tmp2(1115).t["ETE/oC"]);
    obj3.style = tmp.actionButton;
    obj3.onPress = handleCancel;
    const items = [timestampProducer(tmp2(5061).BaseTextButton, obj3), ];
...
export default function DatePickerActionSheet(mode) {
  ...
  const tmp26 = minimumDate(7072)(() => {
    let tmp = first1;
    if (first1) {
      tmp = c9;
    }
    if (tmp) {
      dependencyMap(_modDef4229(ref.current));
    }
    if (c9) {
      ActionSheetActionCreatorsDefault.hideActionSheet();
    } else {
  ...
  const tmp27 = minimumDate(7072)((getTime) => {
    if (null != getTime) {
  ...
  const obj5 = minimumDate(4229)(minimumDate);
  obj3.errorText = intl2.formatToPlainString(maximumDate(1115).t.FsJO55, { minDate: minimumDate(4229)(minimumDate).format(str3) });
  ...
  const obj4 = { minDate: minimumDate(4229)(minimumDate).format(str3) };
  if ("date" === str) {
    str2 = "L";
  }
  const obj8 = minimumDate(4229)(maximumDate);
  obj6.errorText = intl3.formatToPlainString(maximumDate(1115).t.R7r9VN, { maxDate: minimumDate(4229)(maximumDate).format(str2) });
  items1[1] = closure_6(ref, obj6);
  const obj9 = { style: tmp3.datetimePickerContainer, children: null };
  const obj7 = { maxDate: minimumDate(4229)(maximumDate).format(str2) };
  const tmp38 = date;
  const tmp13Result = minimumDate(9859);
  let str4 = "dark";
  ...
  return tmp28(maximumDate(7260).BottomSheet, obj2);
};
