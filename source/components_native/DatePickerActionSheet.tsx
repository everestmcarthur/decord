...
// Dependencies: [32, 19, 17, 21, 4640, 576, 4607, 1364, 7263, 7312, 9869, 1115, 5056, 4376, 1177, 4641, 4636, 5063, 4574, 7076, 4231, 7264, 9870, 4491, 2]
// Exports: default

// Module 9868 (DatePickerActionSheet)
...
import BottomSheetTitleHeader2 from "BottomSheetTitleHeader" /* 7263 */;
...
function ActionSheetHeader(handleSubmit) {
  ...
  if (isAndroidResult) {
    ...
    obj2.trailing = tmp4(tmp(7312).ActionSheetCloseButton, obj3);
    ...
    obj4.leading = tmp4(tmp(9869).ActionSheetHeaderPressableText, obj5);
    const obj6 = { onPress: handleSubmit.handleSubmit, label: null };
    const intl2 = tmp(1115).intl;
    obj6.label = intl2.string(tmp(1115).t["R3BPH+"]);
    obj4.trailing = tmp4(tmp(9869).ActionSheetHeaderPressableText, obj6);
...
function DateRangeError(children) {
  ...
  let obj = show(4376);
  const tmp4 = show;
  A.__closure = { STANDARD_EASING: show(1177).STANDARD_EASING, show, withTiming: show(4641).withTiming };
  ...
  let obj2 = { STANDARD_EASING: show(1177).STANDARD_EASING, show, withTiming: show(4641).withTiming };
  obj3.children = closure_6(View, { ref, accessible: true, accessibilityRole: "alert", style: tmp.rangeError, children: closure_6(tmp4(4636).Text, { variant: "text-md/medium", color: "text-feedback-critical", children: children.errorText }) });
...
function ActionSheetFooter(arg0) {
  ...
  if (obj.isAndroid()) {
    ...
    obj3.textElement = timestampProducer(tmp2(4636).Text, obj4);
    const intl2 = tmp2(1115).intl;
    obj3.accessibilityLabel = intl2.string(tmp2(1115).t["ETE/oC"]);
    obj3.style = tmp.actionButton;
    obj3.onPress = handleCancel;
    const items = [timestampProducer(tmp2(5063).BaseTextButton, obj3), ];
...
export default function DatePickerActionSheet(mode) {
  ...
  const tmp27 = minimumDate(7076)((getTime) => {
    if (null != getTime) {
  ...
  const tmp38 = date;
  const tmp13Result = minimumDate(9870);
  let str4 = "dark";
  ...
  return tmp28(maximumDate(7264).BottomSheet, obj2);
};
