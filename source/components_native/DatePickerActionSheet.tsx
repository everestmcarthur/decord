...
// Dependencies: [32, 19, 17, 21, 4607, 576, 4574, 1115, 7223, 7272, 9811, 1114, 5023, 4343, 1178, 4608, 4603, 5028, 4542, 7036, 4198, 7224, 9812, 4458, 2]
// Exports: default

// Module 9810 (DatePickerActionSheet)
...
import BottomSheetTitleHeader2 from "BottomSheetTitleHeader" /* 7223 */;
...
function ActionSheetHeader(handleSubmit) {
  ...
  if (isAndroidResult) {
    ...
    obj2.trailing = tmp4(tmp(7272).ActionSheetCloseButton, obj3);
    ...
    obj4.leading = tmp4(tmp(9811).ActionSheetHeaderPressableText, obj5);
    const obj6 = { onPress: handleSubmit.handleSubmit, label: null };
    const intl2 = tmp(1114).intl;
    obj6.label = intl2.string(tmp(1114).t["R3BPH+"]);
    obj4.trailing = tmp4(tmp(9811).ActionSheetHeaderPressableText, obj6);
...
function DateRangeError(children) {
  ...
  let obj = show(4343);
  const tmp4 = show;
  A.__closure = { STANDARD_EASING: show(1178).STANDARD_EASING, show, withTiming: show(4608).withTiming };
  ...
  let obj2 = { STANDARD_EASING: show(1178).STANDARD_EASING, show, withTiming: show(4608).withTiming };
  obj3.children = closure_6(View, { ref, accessible: true, accessibilityRole: "alert", style: tmp.rangeError, children: closure_6(tmp4(4603).Text, { variant: "text-md/medium", color: "text-feedback-critical", children: children.errorText }) });
...
function ActionSheetFooter(arg0) {
  ...
  if (obj.isAndroid()) {
    ...
    obj3.textElement = timestampProducer(tmp2(4603).Text, obj4);
    const intl2 = tmp2(1114).intl;
    obj3.accessibilityLabel = intl2.string(tmp2(1114).t["ETE/oC"]);
    obj3.style = tmp.actionButton;
    obj3.onPress = handleCancel;
    const items = [timestampProducer(tmp2(5028).BaseTextButton, obj3), ];
...
export default function DatePickerActionSheet(mode) {
  ...
  const tmp27 = minimumDate(7036)((getTime) => {
    if (null != getTime) {
  ...
  const tmp38 = date;
  const tmp13Result = minimumDate(9812);
  let str4 = "dark";
  ...
  return tmp28(maximumDate(7224).BottomSheet, obj2);
};
