...
// Dependencies: [32, 19, 17, 21, 4574, 576, 4541, 1115, 7163, 7212, 9750, 1114, 4985, 4310, 1178, 4575, 4570, 4990, 4509, 6978, 4166, 7164, 9751, 4425, 2]
// Exports: default

// Module 9749 (DatePickerActionSheet)
...
import BottomSheetTitleHeader2 from "BottomSheetTitleHeader" /* 7163 */;
...
function ActionSheetHeader(handleSubmit) {
  ...
  if (isAndroidResult) {
    ...
    obj2.trailing = tmp4(tmp(7212).ActionSheetCloseButton, obj3);
    ...
    obj4.leading = tmp4(tmp(9750).ActionSheetHeaderPressableText, obj5);
    const obj6 = { onPress: handleSubmit.handleSubmit, label: null };
    const intl2 = tmp(1114).intl;
    obj6.label = intl2.string(tmp(1114).t["R3BPH+"]);
    obj4.trailing = tmp4(tmp(9750).ActionSheetHeaderPressableText, obj6);
...
function DateRangeError(children) {
  ...
  let obj = show(4310);
  const tmp4 = show;
  A.__closure = { STANDARD_EASING: show(1178).STANDARD_EASING, show, withTiming: show(4575).withTiming };
  ...
  let obj2 = { STANDARD_EASING: show(1178).STANDARD_EASING, show, withTiming: show(4575).withTiming };
  obj3.children = closure_6(View, { ref, accessible: true, accessibilityRole: "alert", style: tmp.rangeError, children: closure_6(tmp4(4570).Text, { variant: "text-md/medium", color: "text-feedback-critical", children: children.errorText }) });
  return closure_6(ref(4310).View, obj3);
}
function ActionSheetFooter(arg0) {
  ...
  if (obj.isAndroid()) {
    ...
    obj3.textElement = timestampProducer(tmp2(4570).Text, obj4);
    const intl2 = tmp2(1114).intl;
    obj3.accessibilityLabel = intl2.string(tmp2(1114).t["ETE/oC"]);
    obj3.style = tmp.actionButton;
    obj3.onPress = handleCancel;
    const items = [timestampProducer(tmp2(4990).BaseTextButton, obj3), ];
...
export default function DatePickerActionSheet(mode) {
  ...
  const obj5 = minimumDate(4166)(minimumDate);
  obj3.errorText = intl2.formatToPlainString(maximumDate(1114).t.FsJO55, { minDate: minimumDate(4166)(minimumDate).format(str3) });
  ...
  const obj4 = { minDate: minimumDate(4166)(minimumDate).format(str3) };
  if ("date" === str) {
    str2 = "L";
  }
  const obj8 = minimumDate(4166)(maximumDate);
  obj6.errorText = intl3.formatToPlainString(maximumDate(1114).t.R7r9VN, { maxDate: minimumDate(4166)(maximumDate).format(str2) });
  items1[1] = closure_6(ref, obj6);
  const obj9 = { style: tmp3.datetimePickerContainer, children: null };
  const obj7 = { maxDate: minimumDate(4166)(maximumDate).format(str2) };
  const tmp38 = date;
  ...
  return tmp28(maximumDate(7164).BottomSheet, obj2);
};
