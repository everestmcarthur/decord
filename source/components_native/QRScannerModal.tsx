...
// Function ID: 13975
// Name: QRScannerModal
// Dependencies: [32, 19, 17, 1074, 7648, 21, 1364, 13975, 576, 7148, 1612, 1366, 13955, 4841, 13970, 1897, 7492, 12056, 4332, 4983, 1115, 9193, 7199, 1177, 2]
// Exports: default

// Module 13974 (QRScannerModal)
...
import QRLoginUtils from "QRLoginUtils" /* 13955 */;
import QRScannerNativeComponentDefault from "QRScannerNativeComponent" /* 13975 */;
...
let closure_7 = fn(7648).FAMILY_CENTER_LINK_REQUEST_REGEX;
const jsxProd = fn(21);
    ...
    obj3.onQRCodeFound = function onQRCodeFound(nativeEvent) {
      if (constants.SUCCEEDED === nativeEvent.nativeEvent.status) {
        if (undefined !== onScanSuccess) {
          ...
          if (null != result) {
            tmp21(4841).pop();
            const tmp21Result = tmp21(4841);
            obj2 = { remoteAuthFingerprint: result };
            tmp21(4841).pushLazy(tmp3(1897)(13970, tmp22.paths), obj2);
          } else {
            let match;
            if (str != null) {
            ...
            if (null != match) {
              if (null != str) {
                tmp21(4841).pop();
                const tmp21Result5 = tmp21(4841);
                const obj3 = { screen: UserSettingsSections.FAMILY_CENTER };
                tmp3(7492).openUserSettings(obj3);
                const tmp3Result = tmp3(7492);
                const result1 = tmp3(12056).handleFamilyCenterQRCodeScan(str, "UserSettingsQRCodeScan");
              }
            }
            tmp21(4332).openURL(nativeEvent.nativeEvent.result, undefined, false);
            let tmp9 = tmp21;
            const tmp21Result6 = tmp21(4332);
          }
        }
      } else {
