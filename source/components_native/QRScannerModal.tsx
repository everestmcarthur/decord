...
// Function ID: 13983
// Name: QRScannerModal
// Dependencies: [32, 19, 17, 1074, 7652, 21, 1364, 13983, 576, 7152, 1612, 1366, 13963, 4843, 13978, 1897, 7496, 12065, 4334, 4985, 1115, 9204, 7203, 1177, 2]
// Exports: default

// Module 13982 (QRScannerModal)
...
import QRLoginUtils from "QRLoginUtils" /* 13963 */;
import QRScannerNativeComponentDefault from "QRScannerNativeComponent" /* 13983 */;
...
let closure_7 = fn(7652).FAMILY_CENTER_LINK_REQUEST_REGEX;
const jsxProd = fn(21);
    ...
    obj3.onQRCodeFound = function onQRCodeFound(nativeEvent) {
      if (constants.SUCCEEDED === nativeEvent.nativeEvent.status) {
        if (undefined !== onScanSuccess) {
          ...
          if (null != result) {
            tmp21(4843).pop();
            const tmp21Result = tmp21(4843);
            obj2 = { remoteAuthFingerprint: result };
            tmp21(4843).pushLazy(tmp3(1897)(13978, tmp22.paths), obj2);
          } else {
            let match;
            if (str != null) {
            ...
            if (null != match) {
              if (null != str) {
                tmp21(4843).pop();
                const tmp21Result5 = tmp21(4843);
                const obj3 = { screen: UserSettingsSections.FAMILY_CENTER };
                tmp3(7496).openUserSettings(obj3);
                const tmp3Result = tmp3(7496);
                const result1 = tmp3(12065).handleFamilyCenterQRCodeScan(str, "UserSettingsQRCodeScan");
              }
            }
            tmp21(4334).openURL(nativeEvent.nativeEvent.result, undefined, false);
            let tmp9 = tmp21;
            const tmp21Result6 = tmp21(4334);
          }
        }
      } else {
