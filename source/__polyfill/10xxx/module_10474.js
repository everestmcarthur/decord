...
const regExp = new RegExp("((?:in)\\s*)?(" + repeatedTimeunitPattern.matchAnyPattern(_mod10463.MONTH_DICTIONARY) + ")\\s*(?:(?:,|-|of)?\\s*(" + _mod10463.YEAR_PATTERN + ")?)?(?=[^\\s\\w]|\\s+[^0-9]|\\s+$|$)", "i");
  ...
  entry,
  {
    ...
    value: function innerExtract(createParsingResult, index) {
      ...
      if (index[3]) {
        ...
        start5.assign("year", tmp8(10463).parseYear(index[3]));
      } else {
        const start4 = parsingResult.start;
        start4.imply("year", tmp8(10465).findYearClosestToRef(createParsingResult.refDate, 1, tmp10));
      }
