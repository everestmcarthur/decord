...
const regExp = new RegExp("(" + repeatedTimeunitPattern.matchAnyPattern(_mod10572.MONTH_DICTIONARY) + ")(?:-|/|\\s*,?\\s*)(" + _mod10572.ORDINAL_NUMBER_PATTERN + ")(?!\\s*(?:am|pm))\\s*(?:(?:to|\\-)\\s*(" + _mod10572.ORDINAL_NUMBER_PATTERN + ")\\s*)?(?:(?:-|/|\\s*,\\s*|\\s+)(" + _mod10572.YEAR_PATTERN + "))?(?=\\W|$)(?!\\:\\d)", "i");
...
const items = [
  entry,
  {
    ...
    value: function innerExtract(createParsingComponents, index) {
      const tmp3 = ENMonthNameMiddleEndianParser(10572).MONTH_DICTIONARY[index[1].toLowerCase(index[1])];
      const result = ENMonthNameMiddleEndianParser(10572).parseOrdinalNumberPattern(index[2]);
      if (result > 31) {
        return null;
      } else {
        const self = this;
        ...
        if (index[4]) {
          addTagResult.assign("year", tmp(10572).parseYear(index[4]));
        } else {
          addTagResult.imply("year", tmp(10574).findYearClosestToRef(createParsingComponents.refDate, result, tmp3));
        }
        if (index[3]) {
          const result1 = tmp(10572).parseOrdinalNumberPattern(index[3]);
