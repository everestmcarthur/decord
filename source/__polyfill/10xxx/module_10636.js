  ...
  value: function extractTimeComponent(assign, arg1) {
    ...
    if (num < 60) {
      if (parsed <= 24) {
        let PM1 = null;
        if (parsed >= 12) {
          PM1 = FRSpecificTimeExpressionParser(10578).Meridiem.PM;
        }
        let PM = PM1;
        let tmp5 = parsed;
        if (null != arg1[5]) {
          if (parsed > 12) {
            ...
            if ("a" == formatted) {
              ...
              tmp8 = num2;
              PM1 = FRSpecificTimeExpressionParser(10578).Meridiem.AM;
            }
            PM = PM1;
            tmp5 = tmp8;
            if ("p" == formatted) {
              ...
              tmp5 = sum;
              PM = FRSpecificTimeExpressionParser(10578).Meridiem.PM;
            }
          }
        }
        ...
        if (null !== PM) {
          ...
          assign.imply("meridiem", FRSpecificTimeExpressionParser(10578).Meridiem.AM);
        } else {
          assign.imply("meridiem", FRSpecificTimeExpressionParser(10578).Meridiem.PM);
        }
        if (null != arg1[4]) {
