...
const regExp = new RegExp("((?:in)\\s*)?(" + repeatedTimeunitPattern.matchAnyPattern(_mod10633.MONTH_DICTIONARY) + ")\\s*(?:[,-]?\\s*(" + _mod10633.YEAR_PATTERN + ")?)?(?=[^\\s\\w]|\\s+[^0-9]|\\s+$|$)", "i");
...
const items = [
  entry,
  {
    ...
    value: function innerExtract(createParsingResult, index) {
      ...
      if (index[0].length <= 3) {
        if (!ENMonthNameParser(10633).FULL_MONTH_NAME_DICTIONARY[formatted]) {
          return null;
        }
      }
      let str2 = index[1];
      ...
      start.imply("day", 1);
      const tmp9 = ENMonthNameParser(10633).MONTH_DICTIONARY[formatted];
      const start2 = parsingResult.start;
      start2.assign("month", tmp9);
      if (index[3]) {
        const start4 = parsingResult.start;
        start4.assign("year", tmp7(10633).parseYear(index[3]));
      } else {
