// Module ID: 10697
// Function ID: 10698
// Dependencies: [41, 42, 93, 95, 98, 10565, 10694, 10566, 10696]
...
const entry = {
  ...
  value: function innerPatternString(arg0) {
    return "((?:\u0432)\\s*)?(" + RUMonthNameParser(10565).matchAnyPattern(RUMonthNameParser(10694).MONTH_DICTIONARY) + ")\\s*(?:[,-]?\\s*(" + RUMonthNameParser(10694).YEAR_PATTERN + ")?)?(?=[^\\s\\w]|\\s+[^0-9]|\\s+$|$)";
  }
};
const items = [
  entry,
  {
    ...
    value: function innerExtract(createParsingResult, index) {
      ...
      if (index[3]) {
        ...
        start4.assign("year", tmp7(10694).parseYear(index[3]));
      } else {
        const start3 = parsingResult.start;
        start3.imply("year", tmp7(10566).findYearClosestToRef(createParsingResult.refDate, 1, tmp9));
      }
