// Module ID: 10731
// Function ID: 10732
// Dependencies: [41, 42, 93, 95, 98, 10586, 10587, 10588]
...
const items = [
  entry,
  {
    ...
    value: function innerExtract(refDate, arg1) {
      ...
      if ("tarde" === formatted) {
        parsingComponents.imply("meridiem", ESCasualTimeParser(10586).Meridiem.PM);
        parsingComponents.imply("hour", 15);
      } else if ("noche" === formatted) {
        parsingComponents.imply("meridiem", ESCasualTimeParser(10586).Meridiem.PM);
        parsingComponents.imply("hour", 22);
      } else if ("ma\u00F1ana" === formatted) {
        parsingComponents.imply("meridiem", ESCasualTimeParser(10586).Meridiem.AM);
        parsingComponents.imply("hour", 6);
      } else if ("medianoche" === formatted) {
        const _Date = Date;
        const date = new Date(refDate.getTime());
        date.setDate(date.getDate() + 1);
        ESCasualTimeParser(10587).assignSimilarDate(parsingComponents, date);
        ESCasualTimeParser(10587).implySimilarTime(parsingComponents, date);
