// Module ID: 10721
// Function ID: 10722
// Dependencies: [41, 42, 93, 95, 98, 10573, 10717, 10574, 10580]
...
const regExp = new RegExp("([0-9]{1,2})(?:\u00BA|\u00AA|\u00B0)?(?:\\s*(?:desde|de|\\-|\\\u2013|ao?|\\s)\\s*([0-9]{1,2})(?:\u00BA|\u00AA|\u00B0)?)?\\s*(?:de)?\\s*(?:-|/|\\s*(?:de|,)?\\s*)(" + repeatedTimeunitPattern.matchAnyPattern(_mod10717.MONTH_DICTIONARY) + ")(?:\\s*(?:de|,)?\\s*(" + _mod10717.YEAR_PATTERN + "))?(?=\\W|$)", "i");
...
const items = [
  entry,
  {
    ...
    value: function innerExtract(createParsingResult, index) {
      ...
      const tmp4 = ESMonthNameLittleEndianParser(10717).MONTH_DICTIONARY[index[3].toLowerCase(index[3])];
      ...
      if (parsed > 31) {
        ...
        if (index[4]) {
          ...
          start2.assign("year", tmp2(10717).parseYear(index[4]));
        } else {
          const start = parsingResult.start;
          start.imply("year", tmp2(10574).findYearClosestToRef(createParsingResult.refDate, parsed, tmp4));
        }
        if (index[2]) {
