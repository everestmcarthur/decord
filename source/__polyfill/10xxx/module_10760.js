// Module ID: 10760
// Function ID: 10761
// Dependencies: [41, 42, 93, 95, 98, 10675, 10756, 10676, 10682]
...
const regExp = new RegExp("([0-9]{1,2})(?:\u00BA|\u00AA|\u00B0)?(?:\\s*(?:desde|de|\\-|\\\u2013|ao?|\\s)\\s*([0-9]{1,2})(?:\u00BA|\u00AA|\u00B0)?)?\\s*(?:de)?\\s*(?:-|/|\\s*(?:de|,)?\\s*)(" + repeatedTimeunitPattern.matchAnyPattern(_mod10756.MONTH_DICTIONARY) + ")(?:\\s*(?:de|,)?\\s*(" + _mod10756.YEAR_PATTERN + "))?(?=\\W|$)", "i");
...
const items = [
  entry,
  {
    ...
    value: function innerExtract(createParsingResult, index) {
      ...
      const tmp4 = PTMonthNameLittleEndianParser(10756).MONTH_DICTIONARY[index[3].toLowerCase(index[3])];
      ...
      if (parsed > 31) {
        ...
        if (index[4]) {
          ...
          start2.assign("year", tmp2(10756).parseYear(index[4]));
        } else {
          const start = parsingResult.start;
          start.imply("year", tmp2(10676).findYearClosestToRef(createParsingResult.refDate, parsed, tmp4));
        }
        if (index[2]) {
