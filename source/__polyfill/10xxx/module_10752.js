// Module ID: 10752
// Function ID: 10753
// Dependencies: [41, 42, 93, 95, 98, 10581, 10750, 10582, 10588]
...
const regExp = new RegExp("(" + repeatedTimeunitPattern.matchAnyPattern(_mod10750.MONTH_DICTIONARY) + ")(?:-|/|\\s*,?\\s*)(" + _mod10750.ORDINAL_NUMBER_PATTERN + ")(?!\\s*(?:am|pm))\\s*(?:(?:al|\\-|\\alle|\\del|\\s)\\s*(" + _mod10750.ORDINAL_NUMBER_PATTERN + ")\\s*)?(?:(?:-|/|\\s*,?\\s*)(" + _mod10750.YEAR_PATTERN + "))?(?=\\W|$)(?!\\:\\d)", "i");
  ...
  entry,
  {
    ...
    value: function innerExtract(createParsingComponents, index) {
      ...
      if (result > 31) {
        ...
        if (index[4]) {
          parsingComponents.assign("year", tmp(10750).parseYear(index[4]));
        } else {
          parsingComponents.imply("year", tmp(10582).findYearClosestToRef(createParsingComponents.refDate, result, tmp3));
        }
        if (index[3]) {
          const result1 = tmp(10750).parseOrdinalNumberPattern(index[3]);
