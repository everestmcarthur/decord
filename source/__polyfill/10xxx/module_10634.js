// Module ID: 10634
// Function ID: 10635
// Dependencies: [41, 42, 93, 95, 98, 10633, 10464, 10465, 10471]
...
const regExp = new RegExp("(?:on\\s{0,3})?(" + _mod10633.ORDINAL_NUMBER_PATTERN + ")(?:\\s{0,3}(?:al|\\-|\\\u2013|fino|alle|allo)?\\s{0,3}(" + _mod10633.ORDINAL_NUMBER_PATTERN + "))?(?:-|/|\\s{0,3}(?:dal)?\\s{0,3})(" + repeatedTimeunitPattern.matchAnyPattern(_mod10633.MONTH_DICTIONARY) + ")(?:(?:-|/|,?\\s{0,3})(" + _mod10633.YEAR_PATTERN + "(?![^\\s]\\d)))?(?=\\W|$)", "i");
...
const items = [
  entry,
  {
    ...
    value: function innerExtract(createParsingResult, index) {
      ...
      const tmp4 = ENMonthNameLittleEndianParser(10633).MONTH_DICTIONARY[index[3].toLowerCase(index[3])];
      const result = ENMonthNameLittleEndianParser(10633).parseOrdinalNumberPattern(index[1]);
      if (result > 31) {
        ...
        if (index[4]) {
          ...
          start2.assign("year", tmp2(10633).parseYear(index[4]));
        } else {
        ...
        if (index[2]) {
          ...
          const result1 = tmp2(10633).parseOrdinalNumberPattern(index[2]);
