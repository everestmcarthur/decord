// Module ID: 10605
// Function ID: 10606
// Dependencies: [41, 42, 93, 95, 98, 10572, 10575, 10576, 10592]
...
const items = [
  ...
  },
  {
    ...
    value: function mergeResults(arg0, text, start) {
      const parseDurationResult = ENMergeRelativeFollowByDateRefiner(10572).parseDuration(text.text);
      let reverseDurationResult = parseDurationResult;
      if (null != str.match(/\s+(before|from)$/i)) {
        reverseDurationResult = tmp(10575).reverseDuration(parseDurationResult);
      }
      const ParsingComponents = tmp(10576).ParsingComponents;
      const ReferenceWithTimezone = tmp(10576).ReferenceWithTimezone;
      start = start.start;
      ...
      return new ENMergeRelativeFollowByDateRefiner(10576).ParsingResult(start.reference, text.index, "" + text.text + arg0 + start.text, relativeFromReference);
    }
  }
];
