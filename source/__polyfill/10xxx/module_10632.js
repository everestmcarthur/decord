...
const items = [
  entry,
  {
    ...
    value: function innerExtract(createParsingComponents, arg1) {
      ...
      DECasualTimeParser(10587).implySimilarTime(parsingComponents, createParsingComponents.refDate);
...
const entry1 = {
  ...
  value: function extractTimeComponents(nowResult, formatted) {
    if ("morgen" === formatted) {
      ...
      nowResult.imply("meridiem", DECasualTimeParser(10586).Meridiem.AM);
      ...
      if ("mittag" !== formatted) {
        if ("mittags" !== formatted) {
          if ("nachmittag" === formatted) {
            ...
            nowResult.imply("minute", 0);
            nowResult.imply("second", 0);
            nowResult.imply("meridiem", DECasualTimeParser(10586).Meridiem.PM);
