// Module ID: 10693
// Function ID: 10694
// Dependencies: [41, 42, 93, 95, 98, 10692, 10523, 10524, 10530]
...
const regExp = new RegExp("(?:on\\s{0,3})?(" + _mod10692.ORDINAL_NUMBER_PATTERN + ")(?:\\s{0,3}(?:al|\\-|\\\u2013|fino|alle|allo)?\\s{0,3}(" + _mod10692.ORDINAL_NUMBER_PATTERN + "))?(?:-|/|\\s{0,3}(?:dal)?\\s{0,3})(" + repeatedTimeunitPattern.matchAnyPattern(_mod10692.MONTH_DICTIONARY) + ")(?:(?:-|/|,?\\s{0,3})(" + _mod10692.YEAR_PATTERN + "(?![^\\s]\\d)))?(?=\\W|$)", "i");
...
const items = [
  entry,
  {
    ...
    value: function innerExtract(createParsingResult, index) {
      ...
      const tmp4 = ENMonthNameLittleEndianParser(10692).MONTH_DICTIONARY[index[3].toLowerCase(index[3])];
      const result = ENMonthNameLittleEndianParser(10692).parseOrdinalNumberPattern(index[1]);
      if (result > 31) {
        ...
        if (index[4]) {
          ...
          start2.assign("year", tmp2(10692).parseYear(index[4]));
        } else {
        ...
        if (index[2]) {
          ...
          const result1 = tmp2(10692).parseOrdinalNumberPattern(index[2]);
