...
const regExp = new RegExp("((?:in)\\s*)?(" + repeatedTimeunitPattern.matchAnyPattern(_mod10690.MONTH_DICTIONARY) + ")\\s*(?:[,-]?\\s*(" + _mod10690.YEAR_PATTERN + ")?)?(?=[^\\s\\w]|\\s+[^0-9]|\\s+$|$)", "i");
...
const items = [
  entry,
  {
    ...
    value: function innerExtract(createParsingResult, index) {
      ...
      start.imply("day", 1);
      const tmp9 = ENMonthNameParser(10690).MONTH_DICTIONARY[formatted];
