// Module ID: 10597
// Function ID: 10598
// Dependencies: [41, 42, 93, 95, 98, 10564, 10567, 10568, 10584]
...
const items = [
  ...
  },
  {
    ...
    value: function mergeResults(arg0, text, start) {
      const parseDurationResult = ENMergeRelativeFollowByDateRefiner(10564).parseDuration(text.text);
      let reverseDurationResult = parseDurationResult;
      if (null != str.match(/\s+(before|from)$/i)) {
        reverseDurationResult = tmp(10567).reverseDuration(parseDurationResult);
      }
      const ParsingComponents = tmp(10568).ParsingComponents;
      const ReferenceWithTimezone = tmp(10568).ReferenceWithTimezone;
      start = start.start;
      ...
      return new ENMergeRelativeFollowByDateRefiner(10568).ParsingResult(start.reference, text.index, "" + text.text + arg0 + start.text, relativeFromReference);
    }
  }
];
