// Module ID: 10578
// Function ID: 10579
// Dependencies: [41, 42, 93, 95, 98, 10562, 10579]
    ...
    value: function mergeResults(arg0, start, start2) {
      ...
      if (dateResult > start6.date()) {
        ...
        if (start20.isOnlyWeekdayComponent()) {
          if (AbstractMergeDateRangeRefiner(10562).addDuration(dateResult2, { day: 7 }) > dateResult1) {
            const addDurationResult = tmp6(10562).addDuration(dateResult2, { day: 7 });
        ...
        if (start7.isOnlyWeekdayComponent()) {
          if (AbstractMergeDateRangeRefiner(10562).addDuration(dateResult1, { day: -7 }) < dateResult2) {
            const addDurationResult1 = tmp8(10562).addDuration(dateResult1, { day: -7 });
        ...
        if (start8.isDateWithUnknownYear()) {
          if (AbstractMergeDateRangeRefiner(10562).addDuration(dateResult2, { year: 1 }) > dateResult1) {
            const start11 = start2.start;
            start11.imply("year", tmp10(10562).addDuration(dateResult2, { year: 1 }).getFullYear());
            tmp5 = start2;
            obj = start;
            const addDurationResult2 = tmp10(10562).addDuration(dateResult2, { year: 1 });
          }
          tmp10 = AbstractMergeDateRangeRefiner;
        }
        const start9 = start.start;
        if (start9.isDateWithUnknownYear()) {
          if (AbstractMergeDateRangeRefiner(10562).addDuration(dateResult1, { year: -1 }) < dateResult2) {
            const start10 = start.start;
            start10.imply("year", tmp12(10562).addDuration(dateResult1, { year: -1 }).getFullYear());
            tmp5 = start2;
            obj = start;
            const addDurationResult3 = tmp12(10562).addDuration(dateResult1, { year: -1 });
          }
