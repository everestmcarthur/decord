// Module ID: 10666
// Function ID: 10667
// Dependencies: [41, 42, 93, 95, 98, 10581, 10662, 10582, 10588]
...
const regExp = new RegExp("([0-9]{1,2})(?:\u00BA|\u00AA|\u00B0)?(?:\\s*(?:desde|de|\\-|\\\u2013|ao?|\\s)\\s*([0-9]{1,2})(?:\u00BA|\u00AA|\u00B0)?)?\\s*(?:de)?\\s*(?:-|/|\\s*(?:de|,)?\\s*)(" + repeatedTimeunitPattern.matchAnyPattern(_mod10662.MONTH_DICTIONARY) + ")(?:\\s*(?:de|,)?\\s*(" + _mod10662.YEAR_PATTERN + "))?(?=\\W|$)", "i");
...
const items = [
  entry,
  {
    ...
    value: function innerExtract(createParsingResult, index) {
      ...
      const tmp4 = PTMonthNameLittleEndianParser(10662).MONTH_DICTIONARY[index[3].toLowerCase(index[3])];
      ...
      if (parsed > 31) {
        ...
        if (index[4]) {
          ...
          start2.assign("year", tmp2(10662).parseYear(index[4]));
        } else {
          const start = parsingResult.start;
          start.imply("year", tmp2(10582).findYearClosestToRef(createParsingResult.refDate, parsed, tmp4));
        }
        if (index[2]) {
