// Module ID: 6730
// Function ID: 6731
// Dependencies: [17, 6731, 6733, 6734, 6735]
...
function onGestureHandlerEvent(handlerTag) {
  ...
  if (findHandlerResult) {
    if (null != handlerTag.oldState) {
      if (handlerTag.oldState === tmp(6733).State.UNDETERMINED) {
        if (handlerTag.state === tmp(6733).State.BEGAN) {
      ...
      if (handlerTag.oldState === tmp(6733).State.BEGAN) {
        if (handlerTag.state === tmp(6733).State.ACTIVE) {
      ...
      if (handlerTag.oldState !== handlerTag.state) {
        if (handlerTag.state === tmp(6733).State.END) {
          if (handlerTag.oldState === tmp(6733).State.ACTIVE) {
      ...
      let tmp18 = handlerTag.state !== tmp(6733).State.FAILED;
      if (tmp18) {
        tmp18 = handlerTag.state !== tmp(6733).State.CANCELLED;
      }
      if (!tmp18) {
      ...
      if (!map.has(handlerTag.handlerTag)) {
        const GestureStateManager = tmp(6734).GestureStateManager;
      ...
      if (tmp(6735).TouchEventType.TOUCHES_DOWN === eventType) {
      ...
      } else if (tmp(6735).TouchEventType.TOUCHES_MOVE === eventType) {
      ...
      } else if (tmp(6735).TouchEventType.TOUCHES_UP === eventType) {
      ...
      } else if (tmp(6735).TouchEventType.TOUCHES_CANCEL === eventType) {
    ...
    const result2 = tmp(6731).findOldGestureHandler(handlerTag.handlerTag);
    if (result2) {
