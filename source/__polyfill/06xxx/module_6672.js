// Module ID: 6672
// Function ID: 6673
// Dependencies: [17, 6673, 6675, 6676, 6677]
...
function onGestureHandlerEvent(handlerTag) {
  ...
  if (findHandlerResult) {
    if (null != handlerTag.oldState) {
      if (handlerTag.oldState === tmp(6675).State.UNDETERMINED) {
        if (handlerTag.state === tmp(6675).State.BEGAN) {
      ...
      if (handlerTag.oldState === tmp(6675).State.BEGAN) {
        if (handlerTag.state === tmp(6675).State.ACTIVE) {
      ...
      if (handlerTag.oldState !== handlerTag.state) {
        if (handlerTag.state === tmp(6675).State.END) {
          if (handlerTag.oldState === tmp(6675).State.ACTIVE) {
      ...
      let tmp18 = handlerTag.state !== tmp(6675).State.FAILED;
      if (tmp18) {
        tmp18 = handlerTag.state !== tmp(6675).State.CANCELLED;
      }
      if (!tmp18) {
      ...
      if (!map.has(handlerTag.handlerTag)) {
        const GestureStateManager = tmp(6676).GestureStateManager;
      ...
      if (tmp(6677).TouchEventType.TOUCHES_DOWN === eventType) {
      ...
      } else if (tmp(6677).TouchEventType.TOUCHES_MOVE === eventType) {
      ...
      } else if (tmp(6677).TouchEventType.TOUCHES_UP === eventType) {
      ...
      } else if (tmp(6677).TouchEventType.TOUCHES_CANCEL === eventType) {
    ...
    const result2 = tmp(6673).findOldGestureHandler(handlerTag.handlerTag);
    if (result2) {
