// Module ID: 6859
// Function ID: 6860
// Dependencies: [19, 17, 21, 1636, 6653, 6646, 6642, 6669]
...
const memoResult = memo(function BottomSheetRefreshControlComponent(arg0) {
  ...
  let iter;
  const tmp4 = iter(scrollableGesture(6653).BottomSheetDraggableContext);
  dependencyMap = tmp4;
  const bottomSheetInternal = scrollableGesture(6646).useBottomSheetInternal();
  ...
  const obj = scrollableGesture(6646);
  const fn = function f() {
    return { enabled: iter.value === value2.SCROLLABLE_STATE.UNLOCKED };
  };
  const tmp2Result = scrollableGesture(1636);
  fn.__closure = { animatedScrollableState: iter, SCROLLABLE_STATE: scrollableGesture(6642).SCROLLABLE_STATE };
  ...
  if (tmp7) {
    ...
    let tmp8Result = tmp8(tmp2(6669).GestureDetector, obj3);
  } else {
    const obj5 = {};
