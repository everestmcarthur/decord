// Module ID: 6729
// Function ID: 6730
// Dependencies: [17, 6730, 6732, 6733, 6734]
...
function onGestureHandlerEvent(handlerTag) {
  ...
  if (findHandlerResult) {
    if (null != handlerTag.oldState) {
      if (handlerTag.oldState === tmp(6732).State.UNDETERMINED) {
        if (handlerTag.state === tmp(6732).State.BEGAN) {
      ...
      if (handlerTag.oldState === tmp(6732).State.BEGAN) {
        if (handlerTag.state === tmp(6732).State.ACTIVE) {
      ...
      if (handlerTag.oldState !== handlerTag.state) {
        if (handlerTag.state === tmp(6732).State.END) {
          if (handlerTag.oldState === tmp(6732).State.ACTIVE) {
      ...
      let tmp18 = handlerTag.state !== tmp(6732).State.FAILED;
      if (tmp18) {
        tmp18 = handlerTag.state !== tmp(6732).State.CANCELLED;
      }
      if (!tmp18) {
      ...
      if (!map.has(handlerTag.handlerTag)) {
        const GestureStateManager = tmp(6733).GestureStateManager;
      ...
      if (tmp(6734).TouchEventType.TOUCHES_DOWN === eventType) {
      ...
      } else if (tmp(6734).TouchEventType.TOUCHES_MOVE === eventType) {
      ...
      } else if (tmp(6734).TouchEventType.TOUCHES_UP === eventType) {
      ...
      } else if (tmp(6734).TouchEventType.TOUCHES_CANCEL === eventType) {
    ...
    const result2 = tmp(6730).findOldGestureHandler(handlerTag.handlerTag);
    if (result2) {
