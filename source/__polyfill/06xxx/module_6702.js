// Module ID: 6702
// Function ID: 6703
// Dependencies: [32, 19, 17, 21, 1636, 6703, 6704, 6727, 6700, 6716, 38, 6710, 6880, 6882, 6885, 6887, 6891, 6894, 6898]

// Module 6702
import _modDef38 from "module_38" /* 38 */;
import cancelAnimation2 from "cancelAnimation" /* 1636 */;
import value22 from "value2" /* 6700 */;
import DEFAULT_HANDLE_HEIGHT from "DEFAULT_HANDLE_HEIGHT" /* 6703 */;
import normalizeSnapPoint from "normalizeSnapPoint" /* 6716 */;
  ...
  constructor(arg0, arg1) {
      ...
      if (initialPosition !== sharedValue2.value) {
        if (undefined !== initialPosition) {
          if (sharedValue13.value !== value22.ANIMATION_STATE.RUNNING) {
            if (iter2.value === tmp18(6700).ANIMATION_STATE.RUNNING) {
              workletCallback();
            }
            iter2.value = tmp18(6700).ANIMATION_STATE.RUNNING;
            sharedValue14.value = GESTURE;
            sharedValue3.value = initialPosition;
            let tmp8 = state.value === tmp18(6700).KEYBOARD_STATE.SHOWN;
            if (tmp8) {
              tmp8 = keyboardBehavior !== tmp18(6700).KEYBOARD_BEHAVIOR.extend;
            }
            if (tmp8) {
            ...
            iter.value = tmp18(6716).animate(obj);
            const tmp18Result2 = tmp18(6716);
          }
        }
      }
    };
    ...
    getEvaluatedPosition = function getEvaluatedPosition(arg0) {
      ...
      if (arg0 === value22.ANIMATION_SOURCE.KEYBOARD) {
        if (DEFAULT_KEYBOARD_BLUR_BEHAVIOR === tmp(6700).KEYBOARD_BLUR_BEHAVIOR.restore) {
          if (value3 === tmp(6700).KEYBOARD_STATE.HIDDEN) {
            if (sharedValue10.value !== tmp(6727).State.ACTIVE) {
              if (sharedValue11.value !== tmp(6727).State.ACTIVE) {
      ...
      if (keyboardBehavior === value22.KEYBOARD_BEHAVIOR.extend) {
        if (value3 === tmp(6700).KEYBOARD_STATE.SHOWN) {
          return value4;
        }
      }
    ...
    handleSnapToPosition = function handleSnapToPosition(arg0, arg1) {
      ...
      if (tmp4) {
        ...
        tmp(1636).runOnUI(workletCallback2)(normalizeSnapPointResult, tmp(6700).ANIMATION_SOURCE.USER, 0, arg1);
