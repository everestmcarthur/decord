// Module ID: 6898
// Function ID: 6899
// Dependencies: [19, 17, 21, 6899, 6900]
...
const memoResult = noop.memo(function BottomSheetHandleComponent(style) {
  ...
  const obj = {
    style: useMemo(() => {
      const items = [_mod6900.styles.container, React3.flatten(style)];
      return items;
    }, items),
    accessible: null,
  ...
  const items2 = [
    closure_5(closure_4, {
      style: useMemo(() => {
        const items = [_mod6900.styles.indicator, React3.flatten(indicatorStyle)];
        return items;
      }, items1)
    }),
    style.children
