// Module ID: 6753
// Function ID: 6754
// Dependencies: [41, 42, 93, 95, 98, 19, 17, 21, 6754, 6732, 6748, 6755, 6757, 6730, 6759, 6758, 6760, 6761, 6731, 6745]
// Exports: default

// Module 6753
import handlerIDToTag2 from "handlerIDToTag" /* 6730 */;
...
import ghQueueMicrotask from "ghQueueMicrotask" /* 6758 */;
...
const jsx = fn(21).jsx;
fn(6754).customDirectEventTypes.topGestureHandlerEvent = { registrationName: "onGestureHandlerEvent" };
let closure_10 = { [fn(6732).State.UNDETERMINED]: undefined, [fn(6732).State.BEGAN]: "onBegan", [fn(6732).State.FAILED]: "onFailed", [fn(6732).State.CANCELLED]: "onCancelled", [fn(6732).State.ACTIVE]: "onActivated", [fn(6732).State.END]: "onEnded" };
...
export default function createHandler(name) {
  ...
  class Handler {
    constructor(arg0) {
      ...
      tmp3Result.createGestureHandler = (config) => {
        closure_0.handlerTag = _mod6748.getNextHandlerTag();
          ...
          if (!onHandlerStateChange) {
            if (onGestureEvent) {
              if ("__isNative" in onGestureEvent) {
                let REANIMATED_WORKLET = tmp2(6759).ActionType.NATIVE_ANIMATED_EVENT;
              }
            }
            REANIMATED_WORKLET = tmp2(6759).ActionType.JS_FUNCTION_OLD_API;
          }
          RNGestureHandlerModuleDefault.attachGestureHandler(tmp.handlerTag, viewTag, REANIMATED_WORKLET);
          const result1 = tmp2(6757).scheduleFlushOperations();
          const tmp2Result = tmp2(6757);
          tmp2(6758).ghQueueMicrotask(() => {
            const MountRegistry = Handler(config[16]).MountRegistry;
            const result = MountRegistry.gestureHandlerWillMount(closure_1_0);
          });
        }
        REANIMATED_WORKLET = tmp2(6759).ActionType.REANIMATED_WORKLET;
      };
  ...
  const entry = {
    ...
    value: function componentDidMount() {
      ...
      if (someResult) {
        if (someResult) {
          Handler(6758).ghQueueMicrotask(() => {
            self.update(1);
          });
          const obj = Handler(6758);
        }
        const props2 = self.props;
        const items = [];
        const obj2 = Handler(6757);
        ...
        if (self.viewNode) {
          self.attachGestureHandler(allowedProps(6761)(self.viewNode));
        } else {
          const _Error = Error;
