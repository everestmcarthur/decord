// Module ID: 6868
// Function ID: 6869
// Dependencies: [17, 6699, 6706, 1636, 6869, 6870]
...
export const useGestureEventsHandlersDefault = () => {
  ...
  function handleOnChange(arg0, translationY) {
    ...
    if (sharedValue.value.initialPosition !== initialPosition) {
      ...
      animatedPosition.value = _mod6869.clamp(sum1, initialPosition, value3);
      const clampResult = _mod6869.clamp(sum1, initialPosition, value3);
    }
  ...
  function handleOnEnd(arg0, translationY) {
    ...
    if (isInTemporaryPosition.value) {
      if (sharedValue.value.initialPosition >= iter.value) {
        if (iter3.value.initialPosition > iter.value) {
          animateToPosition(iter3.value.initialPosition, tmp2(6699).ANIMATION_SOURCE.GESTURE, velocityY / 2);
        }
      }
    }
    if (animatedScrollableType.value !== value22.SCROLLABLE_TYPE.UNDETERMINED) {
      value = iter4.value;
      const VIEW = tmp2(6699).SCROLLABLE_TYPE.VIEW;
    }
    if (tmp5) {
    ...
    const snapPointResult = _mod6870.snapPoint(translationY.translationY + sharedValue.value.initialPosition, velocityY, substr);
    if (snapPointResult !== animatedPosition.value) {
      let tmp12 = arg0 === tmp2(6699).GESTURE_SOURCE.CONTENT;
      if (tmp12) {
      ...
      if (!tmp12) {
        animateToPosition(snapPointResult, tmp2(6699).ANIMATION_SOURCE.GESTURE, velocityY / 2);
      }
    }
  }
