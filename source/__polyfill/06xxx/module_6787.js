// Module ID: 6787
// Function ID: 6788
// Dependencies: [41, 42, 93, 95, 98, 19, 17, 21, 6788, 6766, 6782, 6789, 6791, 6764, 6793, 6792, 6794, 6795, 6765, 6779]
// Exports: default

// Module 6787
import handlerIDToTag2 from "handlerIDToTag" /* 6764 */;
...
import ghQueueMicrotask from "ghQueueMicrotask" /* 6792 */;
...
const jsx = fn(21).jsx;
fn(6788).customDirectEventTypes.topGestureHandlerEvent = { registrationName: "onGestureHandlerEvent" };
let closure_10 = { [fn(6766).State.UNDETERMINED]: undefined, [fn(6766).State.BEGAN]: "onBegan", [fn(6766).State.FAILED]: "onFailed", [fn(6766).State.CANCELLED]: "onCancelled", [fn(6766).State.ACTIVE]: "onActivated", [fn(6766).State.END]: "onEnded" };
...
export default function createHandler(name) {
  ...
  class Handler {
    constructor(arg0) {
      ...
      tmp3Result.createGestureHandler = (config) => {
        closure_0.handlerTag = _mod6782.getNextHandlerTag();
          ...
          if (!onHandlerStateChange) {
            if (onGestureEvent) {
              if ("__isNative" in onGestureEvent) {
                let REANIMATED_WORKLET = tmp2(6793).ActionType.NATIVE_ANIMATED_EVENT;
              }
            }
            REANIMATED_WORKLET = tmp2(6793).ActionType.JS_FUNCTION_OLD_API;
          }
          RNGestureHandlerModuleDefault.attachGestureHandler(tmp.handlerTag, viewTag, REANIMATED_WORKLET);
          const result1 = tmp2(6791).scheduleFlushOperations();
          const tmp2Result = tmp2(6791);
          tmp2(6792).ghQueueMicrotask(() => {
            const MountRegistry = Handler(config[16]).MountRegistry;
            const result = MountRegistry.gestureHandlerWillMount(closure_1_0);
          });
        }
        REANIMATED_WORKLET = tmp2(6793).ActionType.REANIMATED_WORKLET;
      };
  ...
  const entry = {
    ...
    value: function componentDidMount() {
      ...
      if (someResult) {
        if (someResult) {
          Handler(6792).ghQueueMicrotask(() => {
            self.update(1);
          });
          const obj = Handler(6792);
        }
        const props2 = self.props;
        const items = [];
        const obj2 = Handler(6791);
        ...
        if (self.viewNode) {
          self.attachGestureHandler(allowedProps(6795)(self.viewNode));
        } else {
          const _Error = Error;
