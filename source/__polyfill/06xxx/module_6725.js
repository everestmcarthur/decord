// Module ID: 6725
// Function ID: 6726
// Dependencies: [32, 19, 6726, 6737, 6728, 6741, 6712]
...
function isSupportedGesture(gestures) {
  if (obj.isComposedGesture(gestures)) {
    ...
    if (tmp(6728).SingleGestureName.Tap !== type) {
      if (tmp(6728).SingleGestureName.LongPress !== type) {
        if (tmp(6728).SingleGestureName.Fling !== type) {
          if (tmp(6728).SingleGestureName.Native !== type) {
            if (tmp(6728).SingleGestureName.Hover !== type) {
              return false;
            }
          }
        }
...
export const useJSResponderHandler = function useJSResponderHandler(gesture) {
  _require = gesture;
  let tmp = closure_3(require("module_6741").JSResponderContext);
  dependencyMap = tmp;
  ...
  closure_5(() => {
    ...
    const enabledSharedValues = gesture(6726).getEnabledSharedValues(Reanimated);
