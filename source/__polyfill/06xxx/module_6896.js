// Module ID: 6896
// Function ID: 6897
// Dependencies: [41, 42, 93, 95, 98, 19, 17, 21, 6769, 6876]
...
class GenericTouchable {
  constructor() {
    ...
    tmp3Result.onHandlerStateChange = (nativeEvent) => {
      ...
      if (state !== GenericTouchable(6769).State.CANCELLED) {
        if (state !== tmp(6769).State.FAILED) {
          if (state === tmp(6769).State.BEGAN) {
            if (closure_0.STATE === closure_0.UNDETERMINED) {
              obj.handlePressIn();
            }
          }
          if (state === tmp(6769).State.END) {
...
let items = [
  ...
  },
  {
    ...
    value: function render() {
      ...
      return jsx(GenericTouchable(6876).LegacyBaseButton, { style: self.props.containerStyle, onHandlerStateChange: null, onGestureEvent: null, hitSlop: null, userSelect: null, shouldActivateOnStart: null, disallowInterruption: null, testID: null, touchSoundDisabled: null, enabled: null });
    }
  }
];
