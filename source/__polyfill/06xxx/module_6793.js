// Module ID: 6793
// Function ID: 6794
// Dependencies: [41, 42, 93, 95, 98, 19, 17, 21, 6794, 6772, 6788, 6795, 6797, 6770, 6799, 6798, 6800, 6801, 6771, 6785]
// Exports: default

// Module 6793
import handlerIDToTag2 from "handlerIDToTag" /* 6770 */;
...
import ghQueueMicrotask from "ghQueueMicrotask" /* 6798 */;
...
const jsx = fn(21).jsx;
fn(6794).customDirectEventTypes.topGestureHandlerEvent = { registrationName: "onGestureHandlerEvent" };
let closure_10 = { [fn(6772).State.UNDETERMINED]: undefined, [fn(6772).State.BEGAN]: "onBegan", [fn(6772).State.FAILED]: "onFailed", [fn(6772).State.CANCELLED]: "onCancelled", [fn(6772).State.ACTIVE]: "onActivated", [fn(6772).State.END]: "onEnded" };
...
export default function createHandler(name) {
  ...
  class Handler {
    constructor(arg0) {
      ...
      tmp3Result.createGestureHandler = (config) => {
        closure_0.handlerTag = _mod6788.getNextHandlerTag();
          ...
          if (!onHandlerStateChange) {
            if (onGestureEvent) {
              if ("__isNative" in onGestureEvent) {
                let REANIMATED_WORKLET = tmp2(6799).ActionType.NATIVE_ANIMATED_EVENT;
              }
            }
            REANIMATED_WORKLET = tmp2(6799).ActionType.JS_FUNCTION_OLD_API;
          }
          RNGestureHandlerModuleDefault.attachGestureHandler(tmp.handlerTag, viewTag, REANIMATED_WORKLET);
          const result1 = tmp2(6797).scheduleFlushOperations();
          const tmp2Result = tmp2(6797);
          tmp2(6798).ghQueueMicrotask(() => {
            const MountRegistry = Handler(config[16]).MountRegistry;
            const result = MountRegistry.gestureHandlerWillMount(closure_1_0);
          });
        }
        REANIMATED_WORKLET = tmp2(6799).ActionType.REANIMATED_WORKLET;
      };
  ...
  const entry = {
    ...
    value: function componentDidMount() {
      ...
      if (someResult) {
        if (someResult) {
          Handler(6798).ghQueueMicrotask(() => {
            self.update(1);
          });
          const obj = Handler(6798);
        }
        const props2 = self.props;
        const items = [];
        const obj2 = Handler(6797);
        ...
        if (self.viewNode) {
          self.attachGestureHandler(allowedProps(6801)(self.viewNode));
        } else {
          const _Error = Error;
