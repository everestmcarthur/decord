// Module ID: 6782
// Function ID: 6783
// Dependencies: [32, 19, 6783, 6794, 6785, 6798, 6769]
...
function isSupportedGesture(gestures) {
  if (obj.isComposedGesture(gestures)) {
    ...
    if (tmp(6785).SingleGestureName.Tap !== type) {
      if (tmp(6785).SingleGestureName.LongPress !== type) {
        if (tmp(6785).SingleGestureName.Fling !== type) {
          if (tmp(6785).SingleGestureName.Native !== type) {
            if (tmp(6785).SingleGestureName.Hover !== type) {
              return false;
            }
          }
        }
...
let closure_8 = fn(6783).SHARED_VALUE_OFFSET + 0.5;
...
export const useJSResponderHandler = function useJSResponderHandler(gesture) {
  _require = gesture;
  let tmp = closure_3(require("module_6798").JSResponderContext);
  dependencyMap = tmp;
  ...
  const items = [gesture];
  closure_5(() => {
    const Reanimated = gesture(6769).Reanimated;
    const enabledSharedValues = gesture(6783).getEnabledSharedValues(Reanimated);
  ...
  let handleStartShouldSetResponder = closure_4(() => {
    if (closure_4()) {
      const result = _mod6798.updateResponderEventValue(closure_1, true);
    }
    return false;
  }, items2);
  if (null == tmp) {
