// Module ID: 6761
// Function ID: 6762
// Dependencies: [32, 19, 6762, 6773, 6764, 6777, 6748]
...
function isSupportedGesture(gestures) {
  if (obj.isComposedGesture(gestures)) {
    ...
    if (tmp(6764).SingleGestureName.Tap !== type) {
      if (tmp(6764).SingleGestureName.LongPress !== type) {
        if (tmp(6764).SingleGestureName.Fling !== type) {
          if (tmp(6764).SingleGestureName.Native !== type) {
            if (tmp(6764).SingleGestureName.Hover !== type) {
              return false;
            }
          }
        }
...
let closure_8 = fn(6762).SHARED_VALUE_OFFSET + 0.5;
...
export const useJSResponderHandler = function useJSResponderHandler(gesture) {
  _require = gesture;
  let tmp = closure_3(require("module_6777").JSResponderContext);
  dependencyMap = tmp;
  ...
  const items = [gesture];
  closure_5(() => {
    const Reanimated = gesture(6748).Reanimated;
    const enabledSharedValues = gesture(6762).getEnabledSharedValues(Reanimated);
  ...
  let handleStartShouldSetResponder = closure_4(() => {
    if (closure_4()) {
      const result = _mod6777.updateResponderEventValue(closure_1, true);
    }
    return false;
  }, items2);
  if (null == tmp) {
