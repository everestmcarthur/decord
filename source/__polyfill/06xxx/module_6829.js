// Module ID: 6829
// Function ID: 6830
// Dependencies: [32, 19, 6827, 6817, 6830, 6803, 6818, 6765]
...
import hash from "hash" /* 6817 */;
import allowedNativeProps2 from "allowedNativeProps" /* 6818 */;
...
export const resolveInternalConfigProps = function resolveInternalConfigProps(useAnimated) {
  ...
  if (!useAnimated) {
    useAnimated = _mod6830.isNativeAnimatedEvent(useAnimated.onUpdate);
  }
  ...
  if (!disableReanimated) {
    result = undefined !== _mod6803.Reanimated;
  }
  if (result) {
  ...
  useAnimated.needsPointerData = _mod6830.shouldHandleTouchEvents(useAnimated);
};
  ...
  while (tmp12 !== undefined) {
    ...
    if (!allowedNativeProps.has(first)) {
      if (!EMPTY_WHITE_LIST.has(tmp17)) {
        let PropsToFilter = tmp19(6818).PropsToFilter;
        if (PropsToFilter.has(tmp17)) {
          continue;
        } else {
          let _console = console;
          let tmp19Result = tmp19(6765);
    ...
    let Reanimated = tmp19(6803).Reanimated;
...
export const useClonedAndRemappedConfig = function useClonedAndRemappedConfig(gestureHandlerProps, map, transformHoverProps) {
  ...
  return useMemo(() => {
    ...
    if (!useAnimated) {
      useAnimated = _mod6830.isNativeAnimatedEvent(tmp3.onUpdate);
    }
    ...
    tmp3.needsPointerData = _mod6830.shouldHandleTouchEvents(tmp3);
    return tmp3;
  }, items);
};
