// Module ID: 6777
// Function ID: 6778
// Dependencies: [6747, 6734, 6731, 6769, 6732, 6733]
...
function getHandler(arg0, onBegin) {
  ...
  } else if (tmp(6747).CALLBACK_TYPE.CHANGE === arg0) {
...
getHandler.__closure = { CALLBACK_TYPE: fn(6747).CALLBACK_TYPE };
...
touchEventTypeToCallbackType.__closure = { TouchEventType: fn(6734).TouchEventType, CALLBACK_TYPE: fn(6747).CALLBACK_TYPE };
...
const obj2 = { TouchEventType: fn(6734).TouchEventType, CALLBACK_TYPE: fn(6747).CALLBACK_TYPE };
runWorklet.__closure = { getHandler, tagMessage: fn(6731).tagMessage };
            ...
            if (typeof isStateChangeEvent === "function") {
              if (null != handlerTag.oldState) {
                ...
                if (handlerTag.oldState === _mod6732.State.UNDETERMINED) {
                  if (handlerTag.state === tmp15(6732).State.BEGAN) {
                    let tmp38 = runWorklet(tmp15(6747).CALLBACK_TYPE.BEGAN, tmp2, handlerTag);
                  }
                }
                if (handlerTag.oldState === tmp15(6732).State.BEGAN) {
                  if (handlerTag.state === tmp15(6732).State.ACTIVE) {
                    let tmp18 = runWorklet(tmp15(6747).CALLBACK_TYPE.START, tmp2, handlerTag);
                    sharedValue1.value[tmp2.handlerTag] = undefined;
                  }
                }
                if (handlerTag.oldState !== handlerTag.state) {
                  if (handlerTag.state === tmp15(6732).State.END) {
                    if (handlerTag.oldState === tmp15(6732).State.ACTIVE) {
                      let flag3 = true;
                      let tmp32 = runWorklet(tmp15(6747).CALLBACK_TYPE.END, tmp2, handlerTag, true);
                    }
                    let flag4 = true;
                    let tmp36 = runWorklet(tmp15(6747).CALLBACK_TYPE.FINALIZE, tmp2, handlerTag, true);
                  }
                }
                let tmp20 = handlerTag.state !== tmp15(6732).State.FAILED;
                if (tmp20) {
                  tmp20 = handlerTag.state !== tmp15(6732).State.CANCELLED;
                }
                if (!tmp20) {
                ...
                if (!tmp20) {
                  if (handlerTag.oldState === tmp15(6732).State.ACTIVE) {
                    let flag = false;
                    let tmp24 = runWorklet(tmp15(6747).CALLBACK_TYPE.END, tmp2, handlerTag, false);
                  }
                  let flag2 = false;
                  let tmp28 = runWorklet(tmp15(6747).CALLBACK_TYPE.FINALIZE, tmp2, handlerTag, false);
                }
                ...
                if (null != handlerTag.eventType) {
                  ...
                  if (tmp4) {
                    ...
                    let tmp41Result = tmp41(tmp42(6747).CALLBACK_TYPE.CHANGE, tmp2, result);
