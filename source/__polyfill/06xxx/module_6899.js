// Module ID: 6899
// Function ID: 6900
// Dependencies: [19, 17, 21, 6900, 6901]
...
const memoResult = noop.memo(function BottomSheetHandleComponent(style) {
  ...
  const obj = {
    style: useMemo(() => {
      const items = [_mod6901.styles.container, React3.flatten(style)];
      return items;
    }, items),
    accessible: null,
  ...
  const items2 = [
    closure_5(closure_4, {
      style: useMemo(() => {
        const items = [_mod6901.styles.indicator, React3.flatten(indicatorStyle)];
        return items;
      }, items1)
    }),
    style.children
