// Module ID: 6953
// Function ID: 6954
// Dependencies: [19, 17, 21, 1637, 6747, 6740, 6736, 6763]
...
const memoResult = memo(function BottomSheetRefreshControlComponent(arg0) {
  ...
  let iter;
  const tmp4 = iter(scrollableGesture(6747).BottomSheetDraggableContext);
  dependencyMap = tmp4;
  const bottomSheetInternal = scrollableGesture(6740).useBottomSheetInternal();
  ...
  const obj = scrollableGesture(6740);
  const fn = function f() {
    return { enabled: iter.value === value2.SCROLLABLE_STATE.UNLOCKED };
  };
  const tmp2Result = scrollableGesture(1637);
  fn.__closure = { animatedScrollableState: iter, SCROLLABLE_STATE: scrollableGesture(6736).SCROLLABLE_STATE };
  ...
  if (tmp7) {
    ...
    let tmp8Result = tmp8(tmp2(6763).GestureDetector, obj3);
  } else {
    const obj5 = {};
