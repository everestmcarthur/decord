// Module ID: 6981
// Function ID: 6982
// Dependencies: [41, 42, 93, 95, 98, 19, 17, 21, 6854, 6961]
...
class GenericTouchable {
  constructor() {
    ...
    tmp3Result.onHandlerStateChange = (nativeEvent) => {
      ...
      if (state !== GenericTouchable(6854).State.CANCELLED) {
        if (state !== tmp(6854).State.FAILED) {
          if (state === tmp(6854).State.BEGAN) {
            if (closure_0.STATE === closure_0.UNDETERMINED) {
              obj.handlePressIn();
            }
          }
          if (state === tmp(6854).State.END) {
...
let items = [
  ...
  },
  {
    ...
    value: function render() {
      ...
      return jsx(GenericTouchable(6961).LegacyBaseButton, { style: self.props.containerStyle, onHandlerStateChange: null, onGestureEvent: null, hitSlop: null, userSelect: null, shouldActivateOnStart: null, disallowInterruption: null, testID: null, touchSoundDisabled: null, enabled: null });
    }
  }
];
