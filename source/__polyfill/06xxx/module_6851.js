// Module ID: 6851
// Function ID: 6852
// Dependencies: [17, 6852, 6854, 6855, 6856]
...
function onGestureHandlerEvent(handlerTag) {
  ...
  if (findHandlerResult) {
    if (null != handlerTag.oldState) {
      if (handlerTag.oldState === tmp(6854).State.UNDETERMINED) {
        if (handlerTag.state === tmp(6854).State.BEGAN) {
      ...
      if (handlerTag.oldState === tmp(6854).State.BEGAN) {
        if (handlerTag.state === tmp(6854).State.ACTIVE) {
      ...
      if (handlerTag.oldState !== handlerTag.state) {
        if (handlerTag.state === tmp(6854).State.END) {
          if (handlerTag.oldState === tmp(6854).State.ACTIVE) {
      ...
      let tmp18 = handlerTag.state !== tmp(6854).State.FAILED;
      if (tmp18) {
        tmp18 = handlerTag.state !== tmp(6854).State.CANCELLED;
      }
      if (!tmp18) {
      ...
      if (!map.has(handlerTag.handlerTag)) {
        const GestureStateManager = tmp(6855).GestureStateManager;
      ...
      if (tmp(6856).TouchEventType.TOUCHES_DOWN === eventType) {
      ...
      } else if (tmp(6856).TouchEventType.TOUCHES_MOVE === eventType) {
      ...
      } else if (tmp(6856).TouchEventType.TOUCHES_UP === eventType) {
      ...
      } else if (tmp(6856).TouchEventType.TOUCHES_CANCEL === eventType) {
    ...
    const result2 = tmp(6852).findOldGestureHandler(handlerTag.handlerTag);
    if (result2) {
