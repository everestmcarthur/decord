// Module ID: 6819
// Function ID: 6820
// Dependencies: [32, 19, 6820, 6831, 6822, 6835, 6806]
...
function isSupportedGesture(gestures) {
  if (obj.isComposedGesture(gestures)) {
    ...
    if (tmp(6822).SingleGestureName.Tap !== type) {
      if (tmp(6822).SingleGestureName.LongPress !== type) {
        if (tmp(6822).SingleGestureName.Fling !== type) {
          if (tmp(6822).SingleGestureName.Native !== type) {
            if (tmp(6822).SingleGestureName.Hover !== type) {
              return false;
            }
          }
        }
...
let closure_8 = fn(6820).SHARED_VALUE_OFFSET + 0.5;
...
export const useJSResponderHandler = function useJSResponderHandler(gesture) {
  _require = gesture;
  let tmp = closure_3(require("module_6835").JSResponderContext);
  dependencyMap = tmp;
  ...
  const items = [gesture];
  closure_5(() => {
    const Reanimated = gesture(6806).Reanimated;
    const enabledSharedValues = gesture(6820).getEnabledSharedValues(Reanimated);
  ...
  let handleStartShouldSetResponder = closure_4(() => {
    if (closure_4()) {
      const result = _mod6835.updateResponderEventValue(closure_1, true);
    }
    return false;
  }, items2);
  if (null == tmp) {
