// Module ID: 6708
// Function ID: 6709
// Dependencies: [17, 6709, 6711, 6712, 6713]
...
function onGestureHandlerEvent(handlerTag) {
  ...
  if (findHandlerResult) {
    if (null != handlerTag.oldState) {
      if (handlerTag.oldState === tmp(6711).State.UNDETERMINED) {
        if (handlerTag.state === tmp(6711).State.BEGAN) {
      ...
      if (handlerTag.oldState === tmp(6711).State.BEGAN) {
        if (handlerTag.state === tmp(6711).State.ACTIVE) {
      ...
      if (handlerTag.oldState !== handlerTag.state) {
        if (handlerTag.state === tmp(6711).State.END) {
          if (handlerTag.oldState === tmp(6711).State.ACTIVE) {
      ...
      let tmp18 = handlerTag.state !== tmp(6711).State.FAILED;
      if (tmp18) {
        tmp18 = handlerTag.state !== tmp(6711).State.CANCELLED;
      }
      if (!tmp18) {
      ...
      if (!map.has(handlerTag.handlerTag)) {
        const GestureStateManager = tmp(6712).GestureStateManager;
      ...
      if (tmp(6713).TouchEventType.TOUCHES_DOWN === eventType) {
      ...
      } else if (tmp(6713).TouchEventType.TOUCHES_MOVE === eventType) {
      ...
      } else if (tmp(6713).TouchEventType.TOUCHES_UP === eventType) {
      ...
      } else if (tmp(6713).TouchEventType.TOUCHES_CANCEL === eventType) {
    ...
    const result2 = tmp(6709).findOldGestureHandler(handlerTag.handlerTag);
    if (result2) {
