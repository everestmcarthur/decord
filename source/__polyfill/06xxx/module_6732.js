// Module ID: 6732
// Function ID: 6733
// Dependencies: [41, 42, 93, 95, 98, 19, 17, 21, 6733, 6711, 6727, 6734, 6736, 6709, 6738, 6737, 6739, 6740, 6710, 6724]
// Exports: default

// Module 6732
import handlerIDToTag2 from "handlerIDToTag" /* 6709 */;
...
import ghQueueMicrotask from "ghQueueMicrotask" /* 6737 */;
...
const jsx = fn(21).jsx;
fn(6733).customDirectEventTypes.topGestureHandlerEvent = { registrationName: "onGestureHandlerEvent" };
let closure_10 = { [fn(6711).State.UNDETERMINED]: undefined, [fn(6711).State.BEGAN]: "onBegan", [fn(6711).State.FAILED]: "onFailed", [fn(6711).State.CANCELLED]: "onCancelled", [fn(6711).State.ACTIVE]: "onActivated", [fn(6711).State.END]: "onEnded" };
...
export default function createHandler(name) {
  ...
  class Handler {
    constructor(arg0) {
      ...
      tmp3Result.createGestureHandler = (config) => {
        closure_0.handlerTag = _mod6727.getNextHandlerTag();
          ...
          if (!onHandlerStateChange) {
            if (onGestureEvent) {
              if ("__isNative" in onGestureEvent) {
                let REANIMATED_WORKLET = tmp2(6738).ActionType.NATIVE_ANIMATED_EVENT;
              }
            }
            REANIMATED_WORKLET = tmp2(6738).ActionType.JS_FUNCTION_OLD_API;
          }
          RNGestureHandlerModuleDefault.attachGestureHandler(tmp.handlerTag, viewTag, REANIMATED_WORKLET);
          const result1 = tmp2(6736).scheduleFlushOperations();
          const tmp2Result = tmp2(6736);
          tmp2(6737).ghQueueMicrotask(() => {
            const MountRegistry = Handler(config[16]).MountRegistry;
            const result = MountRegistry.gestureHandlerWillMount(closure_1_0);
          });
        }
        REANIMATED_WORKLET = tmp2(6738).ActionType.REANIMATED_WORKLET;
      };
  ...
  const entry = {
    ...
    value: function componentDidMount() {
      ...
      if (someResult) {
        if (someResult) {
          Handler(6737).ghQueueMicrotask(() => {
            self.update(1);
          });
          const obj = Handler(6737);
        }
        const props2 = self.props;
        const items = [];
        const obj2 = Handler(6736);
        ...
        if (self.viewNode) {
          self.attachGestureHandler(allowedProps(6740)(self.viewNode));
        } else {
          const _Error = Error;
