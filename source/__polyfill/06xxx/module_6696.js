// Module ID: 6696
// Function ID: 6697
// Dependencies: [41, 42, 93, 95, 98, 19, 17, 21, 6697, 6675, 6691, 6698, 6700, 6673, 6702, 6701, 6703, 6704, 6674, 6688]
// Exports: default

// Module 6696
import handlerIDToTag2 from "handlerIDToTag" /* 6673 */;
...
import ghQueueMicrotask from "ghQueueMicrotask" /* 6701 */;
...
const jsx = fn(21).jsx;
fn(6697).customDirectEventTypes.topGestureHandlerEvent = { registrationName: "onGestureHandlerEvent" };
let closure_10 = { [fn(6675).State.UNDETERMINED]: undefined, [fn(6675).State.BEGAN]: "onBegan", [fn(6675).State.FAILED]: "onFailed", [fn(6675).State.CANCELLED]: "onCancelled", [fn(6675).State.ACTIVE]: "onActivated", [fn(6675).State.END]: "onEnded" };
...
export default function createHandler(name) {
  ...
  class Handler {
    constructor(arg0) {
      ...
      tmp3Result.createGestureHandler = (config) => {
        closure_0.handlerTag = _mod6691.getNextHandlerTag();
          ...
          if (!onHandlerStateChange) {
            if (onGestureEvent) {
              if ("__isNative" in onGestureEvent) {
                let REANIMATED_WORKLET = tmp2(6702).ActionType.NATIVE_ANIMATED_EVENT;
              }
            }
            REANIMATED_WORKLET = tmp2(6702).ActionType.JS_FUNCTION_OLD_API;
          }
          RNGestureHandlerModuleDefault.attachGestureHandler(tmp.handlerTag, viewTag, REANIMATED_WORKLET);
          const result1 = tmp2(6700).scheduleFlushOperations();
          const tmp2Result = tmp2(6700);
          tmp2(6701).ghQueueMicrotask(() => {
            const MountRegistry = Handler(config[16]).MountRegistry;
            const result = MountRegistry.gestureHandlerWillMount(closure_1_0);
          });
        }
        REANIMATED_WORKLET = tmp2(6702).ActionType.REANIMATED_WORKLET;
      };
  ...
  const entry = {
    ...
    value: function componentDidMount() {
      ...
      if (someResult) {
        if (someResult) {
          Handler(6701).ghQueueMicrotask(() => {
            self.update(1);
          });
          const obj = Handler(6701);
        }
        const props2 = self.props;
        const items = [];
        const obj2 = Handler(6700);
        ...
        if (self.viewNode) {
          self.attachGestureHandler(allowedProps(6704)(self.viewNode));
        } else {
          const _Error = Error;
