// Module ID: 6822
// Function ID: 6823
// Dependencies: [32, 19, 6823, 6834, 6825, 6838, 6809]
...
function isSupportedGesture(gestures) {
  if (obj.isComposedGesture(gestures)) {
    ...
    if (tmp(6825).SingleGestureName.Tap !== type) {
      if (tmp(6825).SingleGestureName.LongPress !== type) {
        if (tmp(6825).SingleGestureName.Fling !== type) {
          if (tmp(6825).SingleGestureName.Native !== type) {
            if (tmp(6825).SingleGestureName.Hover !== type) {
              return false;
            }
          }
        }
...
let closure_8 = fn(6823).SHARED_VALUE_OFFSET + 0.5;
...
export const useJSResponderHandler = function useJSResponderHandler(gesture) {
  _require = gesture;
  let tmp = closure_3(require("module_6838").JSResponderContext);
  dependencyMap = tmp;
  ...
  const items = [gesture];
  closure_5(() => {
    const Reanimated = gesture(6809).Reanimated;
    const enabledSharedValues = gesture(6823).getEnabledSharedValues(Reanimated);
  ...
  let handleStartShouldSetResponder = closure_4(() => {
    if (closure_4()) {
      const result = _mod6838.updateResponderEventValue(closure_1, true);
    }
    return false;
  }, items2);
  if (null == tmp) {
