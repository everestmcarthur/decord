// Module ID: 6763
// Function ID: 6764
// Dependencies: [17, 6764, 6766, 6767, 6768]
...
function onGestureHandlerEvent(handlerTag) {
  ...
  if (findHandlerResult) {
    if (null != handlerTag.oldState) {
      if (handlerTag.oldState === tmp(6766).State.UNDETERMINED) {
        if (handlerTag.state === tmp(6766).State.BEGAN) {
      ...
      if (handlerTag.oldState === tmp(6766).State.BEGAN) {
        if (handlerTag.state === tmp(6766).State.ACTIVE) {
      ...
      if (handlerTag.oldState !== handlerTag.state) {
        if (handlerTag.state === tmp(6766).State.END) {
          if (handlerTag.oldState === tmp(6766).State.ACTIVE) {
      ...
      let tmp18 = handlerTag.state !== tmp(6766).State.FAILED;
      if (tmp18) {
        tmp18 = handlerTag.state !== tmp(6766).State.CANCELLED;
      }
      if (!tmp18) {
      ...
      if (!map.has(handlerTag.handlerTag)) {
        const GestureStateManager = tmp(6767).GestureStateManager;
      ...
      if (tmp(6768).TouchEventType.TOUCHES_DOWN === eventType) {
      ...
      } else if (tmp(6768).TouchEventType.TOUCHES_MOVE === eventType) {
      ...
      } else if (tmp(6768).TouchEventType.TOUCHES_UP === eventType) {
      ...
      } else if (tmp(6768).TouchEventType.TOUCHES_CANCEL === eventType) {
    ...
    const result2 = tmp(6764).findOldGestureHandler(handlerTag.handlerTag);
    if (result2) {
