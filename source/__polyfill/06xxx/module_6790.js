// Module ID: 6790
// Function ID: 6791
// Dependencies: [41, 42, 93, 95, 98, 19, 17, 21, 6791, 6769, 6785, 6792, 6794, 6767, 6796, 6795, 6797, 6798, 6768, 6782]
// Exports: default

// Module 6790
import handlerIDToTag2 from "handlerIDToTag" /* 6767 */;
import tagMessage from "tagMessage" /* 6768 */;
import _mod6785 from "module_6785" /* 6785 */;
...
import ghQueueMicrotask from "ghQueueMicrotask" /* 6795 */;
...
const jsx = fn(21).jsx;
fn(6791).customDirectEventTypes.topGestureHandlerEvent = { registrationName: "onGestureHandlerEvent" };
let closure_10 = { [fn(6769).State.UNDETERMINED]: undefined, [fn(6769).State.BEGAN]: "onBegan", [fn(6769).State.FAILED]: "onFailed", [fn(6769).State.CANCELLED]: "onCancelled", [fn(6769).State.ACTIVE]: "onActivated", [fn(6769).State.END]: "onEnded" };
...
export default function createHandler(name) {
  ...
  class Handler {
    constructor(arg0) {
      ...
      tmp3Result.createGestureHandler = (config) => {
        closure_0.handlerTag = _mod6785.getNextHandlerTag();
          ...
          if (!onHandlerStateChange) {
            if (onGestureEvent) {
              if ("__isNative" in onGestureEvent) {
                let REANIMATED_WORKLET = tmp2(6796).ActionType.NATIVE_ANIMATED_EVENT;
              }
            }
            REANIMATED_WORKLET = tmp2(6796).ActionType.JS_FUNCTION_OLD_API;
          }
          RNGestureHandlerModuleDefault.attachGestureHandler(tmp.handlerTag, viewTag, REANIMATED_WORKLET);
          const result1 = tmp2(6794).scheduleFlushOperations();
          const tmp2Result = tmp2(6794);
          tmp2(6795).ghQueueMicrotask(() => {
            const MountRegistry = Handler(config[16]).MountRegistry;
            const result = MountRegistry.gestureHandlerWillMount(closure_1_0);
          });
        }
        REANIMATED_WORKLET = tmp2(6796).ActionType.REANIMATED_WORKLET;
      };
  ...
  const entry = {
    ...
    value: function componentDidMount() {
      ...
      if (someResult) {
        if (someResult) {
          Handler(6795).ghQueueMicrotask(() => {
            self.update(1);
          });
          const obj = Handler(6795);
        }
        const props2 = self.props;
        const items = [];
        const obj2 = Handler(6794);
        ...
        if (self.viewNode) {
          self.attachGestureHandler(allowedProps(6798)(self.viewNode));
        } else {
          const _Error = Error;
