// Module ID: 6875
// Function ID: 6876
// Dependencies: [41, 42, 93, 95, 98, 19, 17, 21, 6876, 6854, 6870, 6877, 6879, 6852, 6881, 6880, 6882, 6883, 6853, 6867]
// Exports: default

// Module 6875
import handlerIDToTag2 from "handlerIDToTag" /* 6852 */;
...
const jsx = fn(21).jsx;
fn(6876).customDirectEventTypes.topGestureHandlerEvent = { registrationName: "onGestureHandlerEvent" };
let closure_10 = { [fn(6854).State.UNDETERMINED]: undefined, [fn(6854).State.BEGAN]: "onBegan", [fn(6854).State.FAILED]: "onFailed", [fn(6854).State.CANCELLED]: "onCancelled", [fn(6854).State.ACTIVE]: "onActivated", [fn(6854).State.END]: "onEnded" };
...
export default function createHandler(name) {
  ...
  class Handler {
    constructor(arg0) {
      ...
      tmp3Result.createGestureHandler = (config) => {
        closure_0.handlerTag = _mod6870.getNextHandlerTag();
          ...
          if (!onHandlerStateChange) {
            if (onGestureEvent) {
              if ("__isNative" in onGestureEvent) {
                let REANIMATED_WORKLET = tmp2(6881).ActionType.NATIVE_ANIMATED_EVENT;
              }
            }
            REANIMATED_WORKLET = tmp2(6881).ActionType.JS_FUNCTION_OLD_API;
          }
          RNGestureHandlerModuleDefault.attachGestureHandler(tmp.handlerTag, viewTag, REANIMATED_WORKLET);
          const result1 = tmp2(6879).scheduleFlushOperations();
          const tmp2Result = tmp2(6879);
          tmp2(6880).ghQueueMicrotask(() => {
            const MountRegistry = Handler(config[16]).MountRegistry;
            const result = MountRegistry.gestureHandlerWillMount(closure_1_0);
          });
        }
        REANIMATED_WORKLET = tmp2(6881).ActionType.REANIMATED_WORKLET;
      };
  ...
  const entry = {
    ...
    value: function componentDidMount() {
      ...
      if (someResult) {
        if (someResult) {
          Handler(6880).ghQueueMicrotask(() => {
            self.update(1);
          });
          const obj = Handler(6880);
        }
        const props2 = self.props;
        const items = [];
        const obj2 = Handler(6879);
        ...
        if (self.viewNode) {
          self.attachGestureHandler(allowedProps(6883)(self.viewNode));
        } else {
          const _Error = Error;
