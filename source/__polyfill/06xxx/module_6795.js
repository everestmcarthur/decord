// Module ID: 6795
// Function ID: 6796
// Dependencies: [32, 19, 6793, 6783, 6796, 6769, 6784, 6731]
...
import hash from "hash" /* 6783 */;
import allowedNativeProps2 from "allowedNativeProps" /* 6784 */;
...
export const resolveInternalConfigProps = function resolveInternalConfigProps(useAnimated) {
  ...
  if (!useAnimated) {
    useAnimated = _mod6796.isNativeAnimatedEvent(useAnimated.onUpdate);
  }
  ...
  if (!disableReanimated) {
    result = undefined !== _mod6769.Reanimated;
  }
  if (result) {
  ...
  useAnimated.needsPointerData = _mod6796.shouldHandleTouchEvents(useAnimated);
};
  ...
  while (tmp12 !== undefined) {
    ...
    if (!allowedNativeProps.has(first)) {
      if (!EMPTY_WHITE_LIST.has(tmp17)) {
        let PropsToFilter = tmp19(6784).PropsToFilter;
        if (PropsToFilter.has(tmp17)) {
          continue;
        } else {
          let _console = console;
          let tmp19Result = tmp19(6731);
    ...
    let Reanimated = tmp19(6769).Reanimated;
...
export const useClonedAndRemappedConfig = function useClonedAndRemappedConfig(gestureHandlerProps, map, transformHoverProps) {
  ...
  return useMemo(() => {
    ...
    if (!useAnimated) {
      useAnimated = _mod6796.isNativeAnimatedEvent(tmp3.onUpdate);
    }
    ...
    tmp3.needsPointerData = _mod6796.shouldHandleTouchEvents(tmp3);
    return tmp3;
  }, items);
};
