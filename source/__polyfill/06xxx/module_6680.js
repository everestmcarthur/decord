// Module ID: 6680
// Function ID: 6681
// Dependencies: [32, 19, 17, 21, 1636, 6681, 6682, 6705, 6678, 6694, 38, 6688, 6858, 6860, 6863, 6865, 6869, 6872, 6876]
...
import normalizeSnapPoint from "normalizeSnapPoint" /* 6694 */;
  ...
  constructor(arg0, arg1) {
      ...
      if (initialPosition !== sharedValue2.value) {
        if (undefined !== initialPosition) {
          if (sharedValue13.value !== value22.ANIMATION_STATE.RUNNING) {
            if (iter2.value === tmp18(6678).ANIMATION_STATE.RUNNING) {
              workletCallback();
            }
            iter2.value = tmp18(6678).ANIMATION_STATE.RUNNING;
            sharedValue14.value = GESTURE;
            sharedValue3.value = initialPosition;
            let tmp8 = state.value === tmp18(6678).KEYBOARD_STATE.SHOWN;
            if (tmp8) {
              tmp8 = keyboardBehavior !== tmp18(6678).KEYBOARD_BEHAVIOR.extend;
            }
            if (tmp8) {
            ...
            iter.value = tmp18(6694).animate(obj);
            const tmp18Result2 = tmp18(6694);
          }
        }
      }
    };
    ...
    getEvaluatedPosition = function getEvaluatedPosition(arg0) {
      ...
      if (arg0 === value22.ANIMATION_SOURCE.KEYBOARD) {
        if (DEFAULT_KEYBOARD_BLUR_BEHAVIOR === tmp(6678).KEYBOARD_BLUR_BEHAVIOR.restore) {
          if (value3 === tmp(6678).KEYBOARD_STATE.HIDDEN) {
            if (sharedValue10.value !== tmp(6705).State.ACTIVE) {
              if (sharedValue11.value !== tmp(6705).State.ACTIVE) {
      ...
      if (keyboardBehavior === value22.KEYBOARD_BEHAVIOR.extend) {
        if (value3 === tmp(6678).KEYBOARD_STATE.SHOWN) {
          return value4;
        }
      }
    ...
    handleSnapToPosition = function handleSnapToPosition(arg0, arg1) {
      ...
      if (tmp4) {
        ...
        tmp(1636).runOnUI(workletCallback2)(normalizeSnapPointResult, tmp(6678).ANIMATION_SOURCE.USER, 0, arg1);
