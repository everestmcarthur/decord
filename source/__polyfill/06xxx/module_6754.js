// Module ID: 6754
// Function ID: 6755
// Dependencies: [41, 42, 93, 95, 98, 19, 17, 21, 6755, 6733, 6749, 6756, 6758, 6731, 6760, 6759, 6761, 6762, 6732, 6746]
// Exports: default

// Module 6754
import handlerIDToTag2 from "handlerIDToTag" /* 6731 */;
...
const jsx = fn(21).jsx;
fn(6755).customDirectEventTypes.topGestureHandlerEvent = { registrationName: "onGestureHandlerEvent" };
let closure_10 = { [fn(6733).State.UNDETERMINED]: undefined, [fn(6733).State.BEGAN]: "onBegan", [fn(6733).State.FAILED]: "onFailed", [fn(6733).State.CANCELLED]: "onCancelled", [fn(6733).State.ACTIVE]: "onActivated", [fn(6733).State.END]: "onEnded" };
...
export default function createHandler(name) {
  ...
  class Handler {
    constructor(arg0) {
      ...
      tmp3Result.createGestureHandler = (config) => {
        closure_0.handlerTag = _mod6749.getNextHandlerTag();
          ...
          if (!onHandlerStateChange) {
            if (onGestureEvent) {
              if ("__isNative" in onGestureEvent) {
                let REANIMATED_WORKLET = tmp2(6760).ActionType.NATIVE_ANIMATED_EVENT;
              }
            }
            REANIMATED_WORKLET = tmp2(6760).ActionType.JS_FUNCTION_OLD_API;
          }
          RNGestureHandlerModuleDefault.attachGestureHandler(tmp.handlerTag, viewTag, REANIMATED_WORKLET);
          const result1 = tmp2(6758).scheduleFlushOperations();
          const tmp2Result = tmp2(6758);
          tmp2(6759).ghQueueMicrotask(() => {
            const MountRegistry = Handler(config[16]).MountRegistry;
            const result = MountRegistry.gestureHandlerWillMount(closure_1_0);
          });
        }
        REANIMATED_WORKLET = tmp2(6760).ActionType.REANIMATED_WORKLET;
      };
  ...
  const entry = {
    ...
    value: function componentDidMount() {
      ...
      if (someResult) {
        if (someResult) {
          Handler(6759).ghQueueMicrotask(() => {
            self.update(1);
          });
          const obj = Handler(6759);
        }
        const props2 = self.props;
        const items = [];
        const obj2 = Handler(6758);
        ...
        if (self.viewNode) {
          self.attachGestureHandler(allowedProps(6762)(self.viewNode));
        } else {
          const _Error = Error;
