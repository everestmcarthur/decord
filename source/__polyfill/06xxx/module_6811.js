// Module ID: 6811
// Function ID: 6812
// Dependencies: [6781, 6768, 6765, 6803, 6766, 6767]
...
function getHandler(arg0, onBegin) {
  if (_mod6781.CALLBACK_TYPE.BEGAN === arg0) {
    return onBegin.onBegin;
  } else if (tmp(6781).CALLBACK_TYPE.START === arg0) {
    return onBegin.onStart;
  } else if (tmp(6781).CALLBACK_TYPE.UPDATE === arg0) {
    return onBegin.onUpdate;
  } else if (tmp(6781).CALLBACK_TYPE.CHANGE === arg0) {
    return onBegin.onChange;
  } else if (tmp(6781).CALLBACK_TYPE.END === arg0) {
    return onBegin.onEnd;
  } else if (tmp(6781).CALLBACK_TYPE.FINALIZE === arg0) {
    return onBegin.onFinalize;
  } else if (tmp(6781).CALLBACK_TYPE.TOUCHES_DOWN === arg0) {
    return onBegin.onTouchesDown;
  } else if (tmp(6781).CALLBACK_TYPE.TOUCHES_MOVE === arg0) {
    return onBegin.onTouchesMove;
  } else if (tmp(6781).CALLBACK_TYPE.TOUCHES_UP === arg0) {
    return onBegin.onTouchesUp;
  } else if (tmp(6781).CALLBACK_TYPE.TOUCHES_CANCEL === arg0) {
    return onBegin.onTouchesCancelled;
  }
}
getHandler.__closure = { CALLBACK_TYPE: fn(6781).CALLBACK_TYPE };
...
function touchEventTypeToCallbackType(arg0) {
  if (TouchEventType.TouchEventType.TOUCHES_DOWN === arg0) {
    return tmp(6781).CALLBACK_TYPE.TOUCHES_DOWN;
  } else if (tmp(6768).TouchEventType.TOUCHES_MOVE === arg0) {
    return tmp(6781).CALLBACK_TYPE.TOUCHES_MOVE;
  } else if (tmp(6768).TouchEventType.TOUCHES_UP === arg0) {
    return tmp(6781).CALLBACK_TYPE.TOUCHES_UP;
  } else if (tmp(6768).TouchEventType.TOUCHES_CANCEL === arg0) {
    return tmp(6781).CALLBACK_TYPE.TOUCHES_CANCEL;
  } else {
    return tmp(6781).CALLBACK_TYPE.UNDEFINED;
  }
}
let obj = { CALLBACK_TYPE: fn(6781).CALLBACK_TYPE };
touchEventTypeToCallbackType.__closure = { TouchEventType: fn(6768).TouchEventType, CALLBACK_TYPE: fn(6781).CALLBACK_TYPE };
...
const obj2 = { TouchEventType: fn(6768).TouchEventType, CALLBACK_TYPE: fn(6781).CALLBACK_TYPE };
runWorklet.__closure = { getHandler, tagMessage: fn(6765).tagMessage };
          ...
          if (handlerTag.handlerTag === tmp2.handlerTag) {
            if (typeof isStateChangeEvent === "function") {
              if (null != handlerTag.oldState) {
                let tmp15 = require;
                if (handlerTag.oldState === _mod6766.State.UNDETERMINED) {
                  if (handlerTag.state === tmp15(6766).State.BEGAN) {
                    let tmp38 = runWorklet(tmp15(6781).CALLBACK_TYPE.BEGAN, tmp2, handlerTag);
                  }
                }
                if (handlerTag.oldState === tmp15(6766).State.BEGAN) {
                  if (handlerTag.state === tmp15(6766).State.ACTIVE) {
                    let tmp18 = runWorklet(tmp15(6781).CALLBACK_TYPE.START, tmp2, handlerTag);
                    sharedValue1.value[tmp2.handlerTag] = undefined;
                  }
                }
                if (handlerTag.oldState !== handlerTag.state) {
                  if (handlerTag.state === tmp15(6766).State.END) {
                    if (handlerTag.oldState === tmp15(6766).State.ACTIVE) {
                      let flag3 = true;
                      let tmp32 = runWorklet(tmp15(6781).CALLBACK_TYPE.END, tmp2, handlerTag, true);
                    }
                    let flag4 = true;
                    let tmp36 = runWorklet(tmp15(6781).CALLBACK_TYPE.FINALIZE, tmp2, handlerTag, true);
                  }
                }
                let tmp20 = handlerTag.state !== tmp15(6766).State.FAILED;
                if (tmp20) {
                  tmp20 = handlerTag.state !== tmp15(6766).State.CANCELLED;
                }
                if (!tmp20) {
                ...
                if (!tmp20) {
                  if (handlerTag.oldState === tmp15(6766).State.ACTIVE) {
                    let flag = false;
                    let tmp24 = runWorklet(tmp15(6781).CALLBACK_TYPE.END, tmp2, handlerTag, false);
                  }
                  let flag2 = false;
                  let tmp28 = runWorklet(tmp15(6781).CALLBACK_TYPE.FINALIZE, tmp2, handlerTag, false);
                }
                ...
                if (null != handlerTag.eventType) {
                  ...
                  if (!tmp10) {
                    let GestureStateManager = _mod6767.GestureStateManager;
                  ...
                  let tmp42 = require;
                  let tmp44 = runWorklet(_mod6781.CALLBACK_TYPE.UPDATE, tmp2, handlerTag);
                  ...
                  if (tmp4) {
                    ...
                    let tmp41Result = tmp41(tmp42(6781).CALLBACK_TYPE.CHANGE, tmp2, result);
