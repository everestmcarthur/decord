// Module ID: 6756
// Function ID: 6757
// Dependencies: [6726, 6713, 6710, 6748, 6711, 6712]
// Exports: useAnimatedGesture

// Module 6756
import tagMessage from "tagMessage" /* 6710 */;
...
function getHandler(arg0, onBegin) {
  ...
  } else if (tmp(6726).CALLBACK_TYPE.CHANGE === arg0) {
...
getHandler.__closure = { CALLBACK_TYPE: fn(6726).CALLBACK_TYPE };
...
let obj = { CALLBACK_TYPE: fn(6726).CALLBACK_TYPE };
touchEventTypeToCallbackType.__closure = { TouchEventType: fn(6713).TouchEventType, CALLBACK_TYPE: fn(6726).CALLBACK_TYPE };
...
const obj2 = { TouchEventType: fn(6713).TouchEventType, CALLBACK_TYPE: fn(6726).CALLBACK_TYPE };
runWorklet.__closure = { getHandler, tagMessage: fn(6710).tagMessage };
          ...
          if (handlerTag.handlerTag === tmp2.handlerTag) {
            if (typeof isStateChangeEvent === "function") {
              if (null != handlerTag.oldState) {
                let tmp15 = require;
                if (handlerTag.oldState === _mod6711.State.UNDETERMINED) {
                  if (handlerTag.state === tmp15(6711).State.BEGAN) {
                    let tmp38 = runWorklet(tmp15(6726).CALLBACK_TYPE.BEGAN, tmp2, handlerTag);
                  }
                }
                if (handlerTag.oldState === tmp15(6711).State.BEGAN) {
                  if (handlerTag.state === tmp15(6711).State.ACTIVE) {
                    let tmp18 = runWorklet(tmp15(6726).CALLBACK_TYPE.START, tmp2, handlerTag);
                    sharedValue1.value[tmp2.handlerTag] = undefined;
                  }
                }
                if (handlerTag.oldState !== handlerTag.state) {
                  if (handlerTag.state === tmp15(6711).State.END) {
                    if (handlerTag.oldState === tmp15(6711).State.ACTIVE) {
                      let flag3 = true;
                      let tmp32 = runWorklet(tmp15(6726).CALLBACK_TYPE.END, tmp2, handlerTag, true);
                    }
                    let flag4 = true;
                    let tmp36 = runWorklet(tmp15(6726).CALLBACK_TYPE.FINALIZE, tmp2, handlerTag, true);
                  }
                }
                let tmp20 = handlerTag.state !== tmp15(6711).State.FAILED;
                if (tmp20) {
                  tmp20 = handlerTag.state !== tmp15(6711).State.CANCELLED;
                }
                if (!tmp20) {
                ...
                if (!tmp20) {
                  if (handlerTag.oldState === tmp15(6711).State.ACTIVE) {
                    let flag = false;
                    let tmp24 = runWorklet(tmp15(6726).CALLBACK_TYPE.END, tmp2, handlerTag, false);
                  }
                  let flag2 = false;
                  let tmp28 = runWorklet(tmp15(6726).CALLBACK_TYPE.FINALIZE, tmp2, handlerTag, false);
                }
                ...
                if (null != handlerTag.eventType) {
                  ...
                  if (!tmp10) {
                    let GestureStateManager = _mod6712.GestureStateManager;
                  ...
                  let tmp42 = require;
                  let tmp44 = runWorklet(_mod6726.CALLBACK_TYPE.UPDATE, tmp2, handlerTag);
                  ...
                  if (tmp4) {
                    ...
                    let tmp41Result = tmp41(tmp42(6726).CALLBACK_TYPE.CHANGE, tmp2, result);
