// Module ID: 6644
// Function ID: 6645
// Dependencies: [32, 19, 17, 21, 1636, 6645, 6646, 6669, 6642, 6658, 38, 6652, 6822, 6824, 6827, 6829, 6833, 6836, 6840]

// Module 6644
import _modDef38 from "module_38" /* 38 */;
import cancelAnimation2 from "cancelAnimation" /* 1636 */;
import value22 from "value2" /* 6642 */;
import DEFAULT_HANDLE_HEIGHT from "DEFAULT_HANDLE_HEIGHT" /* 6645 */;
import normalizeSnapPoint from "normalizeSnapPoint" /* 6658 */;
  ...
  constructor(arg0, arg1) {
      ...
      if (initialPosition !== sharedValue2.value) {
        if (undefined !== initialPosition) {
          if (sharedValue13.value !== value22.ANIMATION_STATE.RUNNING) {
            if (iter2.value === tmp18(6642).ANIMATION_STATE.RUNNING) {
              workletCallback();
            }
            iter2.value = tmp18(6642).ANIMATION_STATE.RUNNING;
            sharedValue14.value = GESTURE;
            sharedValue3.value = initialPosition;
            let tmp8 = state.value === tmp18(6642).KEYBOARD_STATE.SHOWN;
            if (tmp8) {
              tmp8 = keyboardBehavior !== tmp18(6642).KEYBOARD_BEHAVIOR.extend;
            }
            if (tmp8) {
            ...
            tmp18(1636).runOnJS(closure_73)(sharedValue4.value, initialPosition, GESTURE);
            const tmp18Result = tmp18(1636);
            ...
            iter.value = tmp18(6658).animate(obj);
            const tmp18Result2 = tmp18(6658);
          }
        }
      }
    };
    ...
    getEvaluatedPosition = function getEvaluatedPosition(arg0) {
      ...
      if (arg0 === value22.ANIMATION_SOURCE.KEYBOARD) {
        if (DEFAULT_KEYBOARD_BLUR_BEHAVIOR === tmp(6642).KEYBOARD_BLUR_BEHAVIOR.restore) {
          if (value3 === tmp(6642).KEYBOARD_STATE.HIDDEN) {
            if (sharedValue10.value !== tmp(6669).State.ACTIVE) {
              if (sharedValue11.value !== tmp(6669).State.ACTIVE) {
      ...
      if (keyboardBehavior === value22.KEYBOARD_BEHAVIOR.extend) {
        if (value3 === tmp(6642).KEYBOARD_STATE.SHOWN) {
          return value4;
        }
      }
    ...
    handleSnapToPosition = function handleSnapToPosition(arg0, arg1) {
      ...
      if (tmp4) {
        ...
        tmp(1636).runOnUI(workletCallback2)(normalizeSnapPointResult, tmp(6642).ANIMATION_SOURCE.USER, 0, arg1);
