// Module ID: 6816
// Function ID: 6817
// Dependencies: [32, 19, 6817, 6828, 6819, 6832, 6803]
...
function isSupportedGesture(gestures) {
  if (obj.isComposedGesture(gestures)) {
    ...
    if (tmp(6819).SingleGestureName.Tap !== type) {
      if (tmp(6819).SingleGestureName.LongPress !== type) {
        if (tmp(6819).SingleGestureName.Fling !== type) {
          if (tmp(6819).SingleGestureName.Native !== type) {
            if (tmp(6819).SingleGestureName.Hover !== type) {
              return false;
            }
          }
        }
...
let closure_8 = fn(6817).SHARED_VALUE_OFFSET + 0.5;
...
export const useJSResponderHandler = function useJSResponderHandler(gesture) {
  _require = gesture;
  let tmp = closure_3(require("module_6832").JSResponderContext);
  dependencyMap = tmp;
  ...
  const items = [gesture];
  closure_5(() => {
    const Reanimated = gesture(6803).Reanimated;
    const enabledSharedValues = gesture(6817).getEnabledSharedValues(Reanimated);
  ...
  let handleStartShouldSetResponder = closure_4(() => {
    if (closure_4()) {
      const result = _mod6832.updateResponderEventValue(closure_1, true);
    }
    return false;
  }, items2);
  if (null == tmp) {
