// Module ID: 6932
// Function ID: 6933
// Dependencies: [19, 17, 21, 6933, 6934]
...
const memoResult = noop.memo(function BottomSheetHandleComponent(style) {
  ...
  const obj = {
    style: useMemo(() => {
      const items = [_mod6934.styles.container, React3.flatten(style)];
      return items;
    }, items),
    accessible: null,
  ...
  const items2 = [
    closure_5(closure_4, {
      style: useMemo(() => {
        const items = [_mod6934.styles.indicator, React3.flatten(indicatorStyle)];
        return items;
      }, items1)
    }),
    style.children
