// Module ID: 6950
// Function ID: 6951
// Dependencies: [19, 17, 21, 1637, 6744, 6737, 6733, 6760]
...
const memoResult = memo(function BottomSheetRefreshControlComponent(arg0) {
  ...
  let iter;
  const tmp4 = iter(scrollableGesture(6744).BottomSheetDraggableContext);
  dependencyMap = tmp4;
  const bottomSheetInternal = scrollableGesture(6737).useBottomSheetInternal();
  ...
  const obj = scrollableGesture(6737);
  const fn = function f() {
    return { enabled: iter.value === value2.SCROLLABLE_STATE.UNLOCKED };
  };
  const tmp2Result = scrollableGesture(1637);
  fn.__closure = { animatedScrollableState: iter, SCROLLABLE_STATE: scrollableGesture(6733).SCROLLABLE_STATE };
  ...
  if (tmp7) {
    ...
    let tmp8Result = tmp8(tmp2(6760).GestureDetector, obj3);
  } else {
    const obj5 = {};
