// Module ID: 6832
// Function ID: 6833
// Dependencies: [32, 19, 6830, 6820, 6833, 6806, 6821, 6768]
...
import hash from "hash" /* 6820 */;
import allowedNativeProps2 from "allowedNativeProps" /* 6821 */;
...
export const resolveInternalConfigProps = function resolveInternalConfigProps(useAnimated) {
  ...
  if (!useAnimated) {
    useAnimated = _mod6833.isNativeAnimatedEvent(useAnimated.onUpdate);
  }
  ...
  if (!disableReanimated) {
    result = undefined !== _mod6806.Reanimated;
  }
  if (result) {
  ...
  useAnimated.needsPointerData = _mod6833.shouldHandleTouchEvents(useAnimated);
};
  ...
  while (tmp12 !== undefined) {
    ...
    if (!allowedNativeProps.has(first)) {
      if (!EMPTY_WHITE_LIST.has(tmp17)) {
        let PropsToFilter = tmp19(6821).PropsToFilter;
        if (PropsToFilter.has(tmp17)) {
          continue;
        } else {
          let _console = console;
          let tmp19Result = tmp19(6768);
    ...
    let Reanimated = tmp19(6806).Reanimated;
...
export const useClonedAndRemappedConfig = function useClonedAndRemappedConfig(gestureHandlerProps, map, transformHoverProps) {
  ...
  return useMemo(() => {
    ...
    if (!useAnimated) {
      useAnimated = _mod6833.isNativeAnimatedEvent(tmp3.onUpdate);
    }
    ...
    tmp3.needsPointerData = _mod6833.shouldHandleTouchEvents(tmp3);
    return tmp3;
  }, items);
};
