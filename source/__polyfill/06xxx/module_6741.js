// Module ID: 6741
// Function ID: 6742
// Dependencies: [32, 19, 17, 21, 1637, 6742, 6743, 6766, 6739, 6755, 38, 6749, 6919, 6921, 6924, 6926, 6930, 6933, 6937]

// Module 6741
import _modDef38 from "module_38" /* 38 */;
import cancelAnimation2 from "cancelAnimation" /* 1637 */;
import value22 from "value2" /* 6739 */;
import DEFAULT_HANDLE_HEIGHT from "DEFAULT_HANDLE_HEIGHT" /* 6742 */;
import normalizeSnapPoint from "normalizeSnapPoint" /* 6755 */;
  ...
  constructor(arg0, arg1) {
      ...
      if (initialPosition !== sharedValue2.value) {
        if (undefined !== initialPosition) {
          if (sharedValue13.value !== value22.ANIMATION_STATE.RUNNING) {
            if (iter2.value === tmp18(6739).ANIMATION_STATE.RUNNING) {
              workletCallback();
            }
            iter2.value = tmp18(6739).ANIMATION_STATE.RUNNING;
            sharedValue14.value = GESTURE;
            sharedValue3.value = initialPosition;
            let tmp8 = state.value === tmp18(6739).KEYBOARD_STATE.SHOWN;
            if (tmp8) {
              tmp8 = keyboardBehavior !== tmp18(6739).KEYBOARD_BEHAVIOR.extend;
            }
            if (tmp8) {
            ...
            tmp18(1637).runOnJS(closure_73)(sharedValue4.value, initialPosition, GESTURE);
            const tmp18Result = tmp18(1637);
            ...
            iter.value = tmp18(6755).animate(obj);
            const tmp18Result2 = tmp18(6755);
          }
        }
      }
    };
    ...
    getEvaluatedPosition = function getEvaluatedPosition(arg0) {
      ...
      if (arg0 === value22.ANIMATION_SOURCE.KEYBOARD) {
        if (DEFAULT_KEYBOARD_BLUR_BEHAVIOR === tmp(6739).KEYBOARD_BLUR_BEHAVIOR.restore) {
          if (value3 === tmp(6739).KEYBOARD_STATE.HIDDEN) {
            if (sharedValue10.value !== tmp(6766).State.ACTIVE) {
              if (sharedValue11.value !== tmp(6766).State.ACTIVE) {
      ...
      if (keyboardBehavior === value22.KEYBOARD_BEHAVIOR.extend) {
        if (value3 === tmp(6739).KEYBOARD_STATE.SHOWN) {
          return value4;
        }
      }
    ...
    handleSnapToPosition = function handleSnapToPosition(arg0, arg1) {
      ...
      if (tmp4) {
        ...
        tmp(1637).runOnUI(workletCallback2)(normalizeSnapPointResult, tmp(6739).ANIMATION_SOURCE.USER, 0, arg1);
        const runOnUIResult = tmp(1637).runOnUI(workletCallback2);
      }
    };
