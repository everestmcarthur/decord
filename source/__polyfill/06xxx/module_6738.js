// Module ID: 6738
// Function ID: 6739
// Dependencies: [32, 19, 6736, 6726, 6739, 6712, 6727, 6674]
...
import hash from "hash" /* 6726 */;
import allowedNativeProps2 from "allowedNativeProps" /* 6727 */;
  ...
  while (tmp12 !== undefined) {
    ...
    if (!allowedNativeProps.has(first)) {
      if (!EMPTY_WHITE_LIST.has(tmp17)) {
        let PropsToFilter = tmp19(6727).PropsToFilter;
        if (PropsToFilter.has(tmp17)) {
          continue;
        } else {
          let _console = console;
          let tmp19Result = tmp19(6674);
    ...
    let Reanimated = tmp19(6712).Reanimated;
