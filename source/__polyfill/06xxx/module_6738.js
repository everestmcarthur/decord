// Module ID: 6738
// Function ID: 6739
// Dependencies: [32, 19, 17, 21, 1637, 6739, 6740, 6763, 6736, 6752, 38, 6746, 6916, 6918, 6921, 6923, 6927, 6930, 6934]
...
import normalizeSnapPoint from "normalizeSnapPoint" /* 6752 */;
  ...
  constructor(arg0, arg1) {
      ...
      if (initialPosition !== sharedValue2.value) {
        if (undefined !== initialPosition) {
          if (sharedValue13.value !== value22.ANIMATION_STATE.RUNNING) {
            if (iter2.value === tmp18(6736).ANIMATION_STATE.RUNNING) {
              workletCallback();
            }
            iter2.value = tmp18(6736).ANIMATION_STATE.RUNNING;
            sharedValue14.value = GESTURE;
            sharedValue3.value = initialPosition;
            let tmp8 = state.value === tmp18(6736).KEYBOARD_STATE.SHOWN;
            if (tmp8) {
              tmp8 = keyboardBehavior !== tmp18(6736).KEYBOARD_BEHAVIOR.extend;
            }
            if (tmp8) {
            ...
            iter.value = tmp18(6752).animate(obj);
            const tmp18Result2 = tmp18(6752);
          }
        }
      }
    };
    ...
    getEvaluatedPosition = function getEvaluatedPosition(arg0) {
      ...
      if (arg0 === value22.ANIMATION_SOURCE.KEYBOARD) {
        if (DEFAULT_KEYBOARD_BLUR_BEHAVIOR === tmp(6736).KEYBOARD_BLUR_BEHAVIOR.restore) {
          if (value3 === tmp(6736).KEYBOARD_STATE.HIDDEN) {
            if (sharedValue10.value !== tmp(6763).State.ACTIVE) {
              if (sharedValue11.value !== tmp(6763).State.ACTIVE) {
      ...
      if (keyboardBehavior === value22.KEYBOARD_BEHAVIOR.extend) {
        if (value3 === tmp(6736).KEYBOARD_STATE.SHOWN) {
          return value4;
        }
      }
    ...
    handleSnapToPosition = function handleSnapToPosition(arg0, arg1) {
      ...
      if (tmp4) {
        ...
        tmp(1637).runOnUI(workletCallback2)(normalizeSnapPointResult, tmp(6736).ANIMATION_SOURCE.USER, 0, arg1);
