// Module ID: 6766
// Function ID: 6767
// Dependencies: [17, 6767, 6769, 6770, 6771]
...
function onGestureHandlerEvent(handlerTag) {
  ...
  if (findHandlerResult) {
    if (null != handlerTag.oldState) {
      if (handlerTag.oldState === tmp(6769).State.UNDETERMINED) {
        if (handlerTag.state === tmp(6769).State.BEGAN) {
      ...
      if (handlerTag.oldState === tmp(6769).State.BEGAN) {
        if (handlerTag.state === tmp(6769).State.ACTIVE) {
      ...
      if (handlerTag.oldState !== handlerTag.state) {
        if (handlerTag.state === tmp(6769).State.END) {
          if (handlerTag.oldState === tmp(6769).State.ACTIVE) {
      ...
      let tmp18 = handlerTag.state !== tmp(6769).State.FAILED;
      if (tmp18) {
        tmp18 = handlerTag.state !== tmp(6769).State.CANCELLED;
      }
      if (!tmp18) {
      ...
      if (!map.has(handlerTag.handlerTag)) {
        const GestureStateManager = tmp(6770).GestureStateManager;
      ...
      if (tmp(6771).TouchEventType.TOUCHES_DOWN === eventType) {
      ...
      } else if (tmp(6771).TouchEventType.TOUCHES_MOVE === eventType) {
      ...
      } else if (tmp(6771).TouchEventType.TOUCHES_UP === eventType) {
      ...
      } else if (tmp(6771).TouchEventType.TOUCHES_CANCEL === eventType) {
    ...
    const result2 = tmp(6767).findOldGestureHandler(handlerTag.handlerTag);
    if (result2) {
