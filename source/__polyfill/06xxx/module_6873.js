// Module ID: 6873
// Function ID: 6874
// Dependencies: [109, 41, 42, 93, 95, 98, 19, 17, 21, 6874, 6856, 6766]
// Exports: LegacyBorderlessButton, LegacyPureNativeButton, LegacyRectButton

// Module 6873
import _possibleConstructorReturnDefault from "_possibleConstructorReturn" /* 93 */;
import ButtonComponentDefault from "ButtonComponent" /* 6856 */;
...
import module_6874 from "module_6874" /* 6874 */;
...
const ButtonComponent = module_6874(ButtonComponentDefault, { shouldCancelWhenOutside: false, shouldActivateOnStart: false });
...
class InnerBaseButton {
  constructor(arg0) {
    ...
    tmp3Result.handleEvent = (nativeEvent) => {
      ...
      if (pointerInside) {
        let tmp4 = state === InnerBorderlessButton(6766).State.BEGAN;
        if (!tmp4) {
          tmp4 = state === InnerBorderlessButton(6766).State.ACTIVE;
        }
        tmp = tmp4;
      }
      if (tmp8) {
      ...
      if (!longPressDetected) {
        onPress = nativeEvent.oldState === InnerBorderlessButton(6766).State.ACTIVE;
      }
      if (onPress) {
        onPress = state !== InnerBorderlessButton(6766).State.CANCELLED;
      }
      if (onPress) {
      ...
      if (!closure_0.lastIsPressed) {
        if (state === InnerBorderlessButton(6766).State.BEGAN) {
          if (pointerInside) {
      ...
      let tmp18 = state !== InnerBorderlessButton(6766).State.ACTIVE || pointerInside || undefined === tmp7.longPressTimeout;
      if (tmp18) {
        let tmp19 = undefined === tmp7.longPressTimeout;
        if (!tmp19) {
          let tmp22 = state !== InnerBorderlessButton(6766).State.END;
          if (tmp22) {
            tmp22 = state !== InnerBorderlessButton(6766).State.CANCELLED;
          }
          if (tmp22) {
            tmp22 = state !== InnerBorderlessButton(6766).State.FAILED;
          }
          tmp19 = tmp22;
        }
        tmp18 = tmp19;
