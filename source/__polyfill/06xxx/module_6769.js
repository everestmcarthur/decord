// Module ID: 6769
// Function ID: 6770
// Dependencies: [17, 6770, 6772, 6773, 6774]
...
function onGestureHandlerEvent(handlerTag) {
  ...
  if (findHandlerResult) {
    if (null != handlerTag.oldState) {
      if (handlerTag.oldState === tmp(6772).State.UNDETERMINED) {
        if (handlerTag.state === tmp(6772).State.BEGAN) {
      ...
      if (handlerTag.oldState === tmp(6772).State.BEGAN) {
        if (handlerTag.state === tmp(6772).State.ACTIVE) {
      ...
      if (handlerTag.oldState !== handlerTag.state) {
        if (handlerTag.state === tmp(6772).State.END) {
          if (handlerTag.oldState === tmp(6772).State.ACTIVE) {
      ...
      let tmp18 = handlerTag.state !== tmp(6772).State.FAILED;
      if (tmp18) {
        tmp18 = handlerTag.state !== tmp(6772).State.CANCELLED;
      }
      if (!tmp18) {
      ...
      if (!map.has(handlerTag.handlerTag)) {
        const GestureStateManager = tmp(6773).GestureStateManager;
      ...
      if (tmp(6774).TouchEventType.TOUCHES_DOWN === eventType) {
      ...
      } else if (tmp(6774).TouchEventType.TOUCHES_MOVE === eventType) {
      ...
      } else if (tmp(6774).TouchEventType.TOUCHES_UP === eventType) {
      ...
      } else if (tmp(6774).TouchEventType.TOUCHES_CANCEL === eventType) {
    ...
    const result2 = tmp(6770).findOldGestureHandler(handlerTag.handlerTag);
    if (result2) {
