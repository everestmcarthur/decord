// Module ID: 6917
// Function ID: 6918
// Dependencies: [19, 17, 21, 1636, 6711, 6704, 6700, 6727]
...
const memoResult = memo(function BottomSheetRefreshControlComponent(arg0) {
  ...
  let iter;
  const tmp4 = iter(scrollableGesture(6711).BottomSheetDraggableContext);
  dependencyMap = tmp4;
  const bottomSheetInternal = scrollableGesture(6704).useBottomSheetInternal();
  ...
  const obj = scrollableGesture(6704);
  const fn = function f() {
    return { enabled: iter.value === value2.SCROLLABLE_STATE.UNLOCKED };
  };
  const tmp2Result = scrollableGesture(1636);
  fn.__closure = { animatedScrollableState: iter, SCROLLABLE_STATE: scrollableGesture(6700).SCROLLABLE_STATE };
  ...
  if (tmp7) {
    ...
    let tmp8Result = tmp8(tmp2(6727).GestureDetector, obj3);
  } else {
    const obj5 = {};
