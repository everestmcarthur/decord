// Module ID: 6917
// Function ID: 6918
// Dependencies: [32, 19, 6915, 6905, 6918, 6891, 6906, 6853]
...
import hash from "hash" /* 6905 */;
import allowedNativeProps2 from "allowedNativeProps" /* 6906 */;
...
export const resolveInternalConfigProps = function resolveInternalConfigProps(useAnimated) {
  ...
  if (!useAnimated) {
    useAnimated = _mod6918.isNativeAnimatedEvent(useAnimated.onUpdate);
  }
  ...
  if (!disableReanimated) {
    result = undefined !== _mod6891.Reanimated;
  }
  if (result) {
  ...
  useAnimated.needsPointerData = _mod6918.shouldHandleTouchEvents(useAnimated);
};
  ...
  while (tmp12 !== undefined) {
    ...
    if (!allowedNativeProps.has(first)) {
      if (!EMPTY_WHITE_LIST.has(tmp17)) {
        let PropsToFilter = tmp19(6906).PropsToFilter;
        if (PropsToFilter.has(tmp17)) {
          continue;
        } else {
          let _console = console;
          let tmp19Result = tmp19(6853);
    ...
    let Reanimated = tmp19(6891).Reanimated;
...
export const useClonedAndRemappedConfig = function useClonedAndRemappedConfig(gestureHandlerProps, map, transformHoverProps) {
  ...
  return useMemo(() => {
    ...
    if (!useAnimated) {
      useAnimated = _mod6918.isNativeAnimatedEvent(tmp3.onUpdate);
    }
    ...
    tmp3.needsPointerData = _mod6918.shouldHandleTouchEvents(tmp3);
    return tmp3;
  }, items);
};
