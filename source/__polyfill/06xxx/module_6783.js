// Module ID: 6783
// Function ID: 6784
// Dependencies: [32, 19, 6784, 6795, 6786, 6799, 6770]
...
function isSupportedGesture(gestures) {
  if (obj.isComposedGesture(gestures)) {
    ...
    if (tmp(6786).SingleGestureName.Tap !== type) {
      if (tmp(6786).SingleGestureName.LongPress !== type) {
        if (tmp(6786).SingleGestureName.Fling !== type) {
          if (tmp(6786).SingleGestureName.Native !== type) {
            if (tmp(6786).SingleGestureName.Hover !== type) {
              return false;
            }
          }
        }
...
let closure_8 = fn(6784).SHARED_VALUE_OFFSET + 0.5;
...
export const useJSResponderHandler = function useJSResponderHandler(gesture) {
  _require = gesture;
  let tmp = closure_3(require("module_6799").JSResponderContext);
  dependencyMap = tmp;
  ...
  const items = [gesture];
  closure_5(() => {
    const Reanimated = gesture(6770).Reanimated;
    const enabledSharedValues = gesture(6784).getEnabledSharedValues(Reanimated);
  ...
  let handleStartShouldSetResponder = closure_4(() => {
    if (closure_4()) {
      const result = _mod6799.updateResponderEventValue(closure_1, true);
    }
    return false;
  }, items2);
  if (null == tmp) {
