// Module ID: 6701
// Function ID: 6702
// Dependencies: [32, 19, 17, 21, 1636, 6702, 6703, 6726, 6699, 6715, 38, 6709, 6879, 6881, 6884, 6886, 6890, 6893, 6897]

// Module 6701
import _modDef38 from "module_38" /* 38 */;
import cancelAnimation2 from "cancelAnimation" /* 1636 */;
import value22 from "value2" /* 6699 */;
import DEFAULT_HANDLE_HEIGHT from "DEFAULT_HANDLE_HEIGHT" /* 6702 */;
import normalizeSnapPoint from "normalizeSnapPoint" /* 6715 */;
  ...
  constructor(arg0, arg1) {
      ...
      if (initialPosition !== sharedValue2.value) {
        if (undefined !== initialPosition) {
          if (sharedValue13.value !== value22.ANIMATION_STATE.RUNNING) {
            if (iter2.value === tmp18(6699).ANIMATION_STATE.RUNNING) {
              workletCallback();
            }
            iter2.value = tmp18(6699).ANIMATION_STATE.RUNNING;
            sharedValue14.value = GESTURE;
            sharedValue3.value = initialPosition;
            let tmp8 = state.value === tmp18(6699).KEYBOARD_STATE.SHOWN;
            if (tmp8) {
              tmp8 = keyboardBehavior !== tmp18(6699).KEYBOARD_BEHAVIOR.extend;
            }
            if (tmp8) {
            ...
            iter.value = tmp18(6715).animate(obj);
            const tmp18Result2 = tmp18(6715);
          }
        }
      }
    };
    ...
    getEvaluatedPosition = function getEvaluatedPosition(arg0) {
      ...
      if (arg0 === value22.ANIMATION_SOURCE.KEYBOARD) {
        if (DEFAULT_KEYBOARD_BLUR_BEHAVIOR === tmp(6699).KEYBOARD_BLUR_BEHAVIOR.restore) {
          if (value3 === tmp(6699).KEYBOARD_STATE.HIDDEN) {
            if (sharedValue10.value !== tmp(6726).State.ACTIVE) {
              if (sharedValue11.value !== tmp(6726).State.ACTIVE) {
      ...
      if (keyboardBehavior === value22.KEYBOARD_BEHAVIOR.extend) {
        if (value3 === tmp(6699).KEYBOARD_STATE.SHOWN) {
          return value4;
        }
      }
    ...
    handleSnapToPosition = function handleSnapToPosition(arg0, arg1) {
      ...
      if (tmp4) {
        ...
        tmp(1636).runOnUI(workletCallback2)(normalizeSnapPointResult, tmp(6699).ANIMATION_SOURCE.USER, 0, arg1);
