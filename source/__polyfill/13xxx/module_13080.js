// Module ID: 13080
// Function ID: 13081
// Dependencies: [41, 42, 13049, 13053, 13054, 13044, 13077, 13045, 13055, 13081, 13067, 13039, 13066, 13082, 13068, 13075]
...
let items = [
  ...
  },
  {
    ...
    value: function updateStartTime(arg0) {
      this._startTime = SentrySpan(13044).spanTimeInputToSeconds(arg0);
    }
  },
  {
    key: "setStatus",
  ...
  },
  {
    ...
    value: function updateName(_name) {
      this._name = _name;
      const attr = this.setAttribute(SentrySpan(13054).SEMANTIC_ATTRIBUTE_SENTRY_SOURCE, "custom");
      return this;
    }
  },
  {
    ...
    value: function end(arg0) {
      const self = this;
      if (!this._endTime) {
        self._endTime = SentrySpan(13044).spanTimeInputToSeconds(arg0);
        const obj = SentrySpan(13044);
        SentrySpan(13077).logSpanEnd(self);
        self._onSpanEnded();
        const obj2 = SentrySpan(13077);
      }
    }
  },
  {
    ...
    value: function getSpanJSON() {
      const self = this;
      const obj6 = { data: this._attributes, description: this._name, op: this._attributes[SentrySpan(undefined, 13054).SEMANTIC_ATTRIBUTE_SENTRY_OP], parent_span_id: this._parentSpanId, span_id: this._spanId, start_timestamp: this._startTime, status: null, timestamp: null, trace_id: null, origin: null, _metrics_summary: null, profile_id: null, exclusive_time: null, measurements: null, is_segment: null, segment_id: null };
      const obj = SentrySpan(13045);
      obj6.status = SentrySpan(13044).getStatusMessage(this._status);
      ({ _endTime: obj2.timestamp, _traceId: obj2.trace_id, _attributes } = this);
      obj6.origin = _attributes[SentrySpan(undefined, 13054).SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN];
      const obj3 = SentrySpan(13044);
      obj6._metrics_summary = SentrySpan(13055).getMetricSummaryJsonForSpan(this);
      obj6.profile_id = this._attributes[SentrySpan(undefined, 13054).SEMANTIC_ATTRIBUTE_PROFILE_ID];
      obj6.exclusive_time = this._attributes[SentrySpan(undefined, 13054).SEMANTIC_ATTRIBUTE_EXCLUSIVE_TIME];
      const obj4 = SentrySpan(13055);
      obj6.measurements = SentrySpan(13081).timedEventsToMeasurements(this._events);
      let _isStandaloneSpan = this._isStandaloneSpan;
      if (_isStandaloneSpan) {
        _isStandaloneSpan = tmp(13044).getRootSpan(self) === self;
        const tmpResult = tmp(13044);
      }
      obj6.is_segment = _isStandaloneSpan;
      let spanId;
      if (self._isStandaloneSpan) {
        const rootSpan = tmp(13044).getRootSpan(self);
        spanId = rootSpan.spanContext().spanId;
        const tmpResult2 = tmp(13044);
      }
  ...
  },
  {
    ...
    value: function addEvent(name, num, arg2) {
      if (SentrySpan(13067).DEBUG_BUILD) {
      ...
      if (!isArray) {
        ...
        if (!arg2) {
          timestampInSecondsResult = tmp(13053).timestampInSeconds();
      ...
      const obj2 = { name, time: SentrySpan(13044).spanTimeInputToSeconds(tmp7), attributes: obj };
  ...
  },
  {
    ...
    value: function _onSpanEnded() {
      const self = this;
      const client = SentrySpan(13066).getClient();
      if (client) {
      ...
      if (self._isStandaloneSpan) {
        if (self._isStandaloneSpan) {
          if (self._sampled) {
            const items = [self];
            const spanEnvelope = tmp(13082).createSpanEnvelope(items, client);
            const tmpResult = tmp(13082);
            const client1 = tmp(13066).getClient();
            if (client1) {
              if (spanEnvelope[1]) {
                if (0 !== arr2.length) {
          ...
          if (result) {
            let scope = tmp(13068).getCapturedScopesOnSpan(self).scope;
            if (!scope) {
              scope = tmp(13066).getCurrentScope();
              const tmpResult7 = tmp(13066);
            }
            scope.captureEvent(result);
            const tmpResult6 = tmp(13068);
          }
        }
      } else {
  ...
  },
  {
    ...
    value: function _convertSpanToTransaction() {
      const self = this;
      const spanToJSONResult = self(13044).spanToJSON(this);
      if (tmp6) {
        if (!self._name) {
          if (tmp3(13067).DEBUG_BUILD) {
            const logger = tmp3(13039).logger;
            logger.warn("Transaction has no name, falling back to `<unlabeled transaction>`.");
          }
          self._name = "<unlabeled transaction>";
        }
        const capturedScopesOnSpan = tmp3(13068).getCapturedScopesOnSpan(self);
        const scope = capturedScopesOnSpan.scope;
        let currentScope = scope;
        if (!scope) {
          currentScope = tmp3(13066).getCurrentScope();
          const tmp3Result9 = tmp3(13066);
        }
        let client = currentScope.getClient();
        if (!client) {
          client = tmp3(13066).getClient();
          const tmp3Result10 = tmp3(13066);
        }
        if (true !== self._sampled) {
          if (tmp3(13067).DEBUG_BUILD) {
            const logger3 = tmp3(13039).logger;
          ...
          const spanDescendants = tmp3(13044).getSpanDescendants(self);
          ...
          const mapped = found.map((item) => self(13044).spanToJSON(item));
          const found1 = mapped.filter(isFullFinishedSpan);
          const tmp24 = self._attributes[tmp3(undefined, 13054).SEMANTIC_ATTRIBUTE_SENTRY_SOURCE];
          const _attributes = self._attributes;
          let SEMANTIC_ATTRIBUTE_SENTRY_CUSTOM_SPAN_NAME = tmp3(13054).SEMANTIC_ATTRIBUTE_SENTRY_CUSTOM_SPAN_NAME;
          delete tmp2[tmp];
          const item = found1.forEach((data) => {
            if (data.data) {
              data = data.data;
              const SEMANTIC_ATTRIBUTE_SENTRY_CUSTOM_SPAN_NAME = self(13054).SEMANTIC_ATTRIBUTE_SENTRY_CUSTOM_SPAN_NAME;
              delete tmp2[tmp];
            }
          });
          ...
          const tmp3Result11 = tmp3(13044);
          obj3.trace = tmp3(13044).spanToTransactionTraceContext(self);
          obj2.contexts = obj3;
          let substr = found1;
          ...
          const tmp3Result12 = tmp3(13044);
          const obj5 = { dynamicSamplingContext: null };
          const tmp3Result13 = tmp3(13045);
          obj5.dynamicSamplingContext = tmp3(13075).getDynamicSamplingContextFromSpan(self);
          const merged = Object.assign(tmp3Result13.dropUndefinedKeys(obj5));
          obj2.sdkProcessingMetadata = obj4;
          const tmp3Result14 = tmp3(13075);
          obj2._metrics_summary = tmp3(13055).getMetricSummaryJsonForSpan(self);
          let tmp12 = tmp24;
          if (tmp24) {
          ...
          const tmp3Result15 = tmp3(13055);
          const result = tmp3(13081).timedEventsToMeasurements(self._events);
          let length = result;
          if (result) {
          ...
          if (length) {
            if (tmp3(13067).DEBUG_BUILD) {
              const logger2 = tmp3(13039).logger;
              const _JSON = JSON;
        ...
        const tmp3Result = tmp3(13068);
      }
      const obj = self(13044);
