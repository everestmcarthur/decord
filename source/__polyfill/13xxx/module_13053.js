// Module ID: 13053
// Function ID: 13054
// Dependencies: [718, 12944, 12947, 12948, 12962, 12972, 12976, 12980, 13030, 12953, 12982, 12963, 12981, 13019, 12965, 12961, 12955]
...
import "module_12947";
import consoleSandbox from "module_12948" /* 12948 */;
...
function _addTracingHeadersToFetchRequest(headers, headers2, span) {
  const traceData = _mod13019.getTraceData({ span });
  ...
  if (tmp4) {
    ...
    if (!headers) {
      ...
      if (typeof Request !== "undefined") {
        ...
        isInstanceOfResult = tmp(12955).isInstanceOf(headers, Request);
        const tmpResult = tmp(12955);
      }
      let headers1;
    ...
    if (headers) {
      ...
      if (typeof Headers !== "undefined") {
        ...
        isInstanceOfResult1 = tmp(12955).isInstanceOf(headers, Headers);
        const tmpResult2 = tmp(12955);
      }
...
export const instrumentFetchRequest = function instrumentFetchRequest(fetchData, fn, fn2, arg3) {
  ...
  if (fetchData.fetchData) {
    ...
    let hasTracingEnabledResult = _mod12980.hasTracingEnabled();
        ...
        if (__span) {
          if (arg3[__span]) {
            if (endResult.response) {
              setHttpStatus = setHttpStatus(12965).setHttpStatus;
              ...
              const setHttpStatusResult = setHttpStatus(12965);
            } else if (endResult.error) {
              const obj2 = { code: setHttpStatus(12965).SPAN_STATUS_ERROR, message: "internal_error" };
    ...
    if (tmp7) {
      const host = setHttpStatus(13030).parseUrl(tmp7).host;
      const setHttpStatusResult2 = setHttpStatus(13030);
    }
    const activeSpan = setHttpStatus(12953).getActiveSpan();
    if (hasTracingEnabledResult) {
      if (activeSpan) {
        ...
        obj4[setHttpStatus(12963).SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN] = str;
        obj4[setHttpStatus(12963).SEMANTIC_ATTRIBUTE_SENTRY_OP] = "http.client";
        obj3.attributes = obj4;
        let startInactiveSpanResult = setHttpStatus(12982).startInactiveSpan(obj3);
        const setHttpStatusResult4 = setHttpStatus(12982);
      }
      ...
      if (fn2(endResult.fetchData.url)) {
        ...
        setHttpStatusResult5 = setHttpStatus(12980);
      }
      return startInactiveSpanResult;
    }
    startInactiveSpanResult = new setHttpStatus(12981).SentryNonRecordingSpan();
    const setHttpStatusResult3 = setHttpStatus(12953);
  }
};
