// Module ID: 13124
// Function ID: 13125
// Dependencies: [13039, 13040, 13125, 13066, 13095, 13126, 13050, 13048, 13087]
    ...
    setup(arg0) {
      ...
      if ("console" in CONSOLE_LEVELS(handled[1]).GLOBAL_OBJ) {
        let result = CONSOLE_LEVELS(handled[2]).addConsoleInstrumentationHandler((arg0) => {
          ({ args, level } = arg0);
          let hasItem = _mod13066.getClient() === args;
          if (hasItem) {
            hasItem = CONSOLE_LEVELS.includes(level);
          }
          if (hasItem) {
            closure_2 = handled;
            let obj2 = { level: tmp(13126).severityLevelFromString(level), extra: null };
            const obj3 = { arguments: args };
            obj2.extra = obj3;
            const tmpResult = tmp(13126);
            tmp(13066).withScope((addEventProcessor) => {
              ...
              if ("assert" !== level) {
                ...
                if (found) {
                  tmp14(13087).captureException(found, obj2);
                  const tmp14Result = tmp14(13087);
                } else {
                  const tmp14Result2 = tmp14(13048);
                  const safeJoinResult = tmp14(13048).safeJoin(tmp12, " ");
                  args(13087).captureMessage(safeJoinResult, obj2);
                  const obj4 = args(13087);
                }
                tmp12 = args;
              } else if (!args[0]) {
                const obj = args(13048);
                const _HermesInternal = HermesInternal;
                const combined = "Assertion failed: " + args(13048).safeJoin(arr.slice(1), " ") || "console.assert";
                addEventProcessor.setExtra("arguments", arr.slice(1));
                obj2 = args(13087);
                obj2.captureMessage(combined, obj2);
                const tmp4 = args(13048).safeJoin(arr.slice(1), " ") || "console.assert";
              }
            });
