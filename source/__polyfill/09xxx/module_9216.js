// Module ID: 9216
// Function ID: 9217
// Dependencies: [41, 42, 93, 95, 98, 9210, 9211]
// Exports: Chi, Maj
...
const entry = {
  ...
  value: function update(B) {
    let tmp8;
    const self = this;
    HashMD(9211).exists(this);
    ({ buffer, blockLen, view } = this);
    const toBytesResult = HashMD(9210).toBytes(B);
    let num = 0;
...
let items = [
  entry,
  {
    ...
    value: function digestInto(content) {
      const self = this;
      HashMD(9211).exists(this);
      HashMD(9211).output(content, this);
      this.finished = true;
      ...
      self.process(view, 0);
      const view1 = HashMD(9210).createView(content);
