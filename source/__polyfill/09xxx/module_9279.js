// Module ID: 9279
// Function ID: 9280
// Dependencies: [41, 42, 93, 95, 98, 9273, 9274]
// Exports: Chi, Maj
...
const entry = {
  ...
  value: function update(B) {
    let tmp8;
    const self = this;
    HashMD(9274).exists(this);
    ({ buffer, blockLen, view } = this);
    const toBytesResult = HashMD(9273).toBytes(B);
    let num = 0;
...
let items = [
  entry,
  {
    ...
    value: function digestInto(content) {
      const self = this;
      HashMD(9274).exists(this);
      HashMD(9274).output(content, this);
      this.finished = true;
      ...
      self.process(view, 0);
      const view1 = HashMD(9273).createView(content);
