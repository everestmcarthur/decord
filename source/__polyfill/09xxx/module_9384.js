// Module ID: 9384
// Function ID: 9385
// Dependencies: [41, 42, 93, 95, 98, 9379, 9385]

// Module 9384
import _asyncLoop from "_asyncLoop" /* 9379 */;
import _mod9385 from "module_9385" /* 9385 */;
...
SHA224 = SHA256;
_inherits(SHA256, _mod9385.HashMD);
const entry = {
  key: "get",
  value: function get() {
    const items = [, , , , , , , ];
  ...
  },
  {
    ...
    value: function process(getUint32, sum) {
      ...
      do {
        ...
        let rotrResult = SHA224(9379).rotr(tmp4, 7);
        let tmp9 = rotrResult ^ SHA224(9379).rotr(tmp4, 18) ^ tmp4 >>> 3;
        let rotrResult1 = SHA224(9379).rotr(tmp5, 17);
        uint32Array2[num2] = (rotrResult1 ^ SHA224(9379).rotr(tmp5, 19) ^ tmp5 >>> 10) + uint32Array2[num2 - 7] + tmp9 + uint32Array2[num2 - 16] | 0;
        num2 = num2 + 1;
      } while (num2 < 64);
      const self = this;
      ...
      let num3 = 0;
      do {
        let rotrResult2 = SHA224(9379).rotr(E, 6);
        let tmp14 = rotrResult2 ^ SHA224(9379).rotr(E, 11);
        let sum1 = H + (tmp14 ^ SHA224(9379).rotr(E, 25));
        let tmp18 = sum1 + SHA224(9385).Chi(E, F, G) + uint32Array[num3] + uint32Array2[num3] | 0;
        let rotrResult3 = SHA224(9379).rotr(A, 2);
        let tmp20 = rotrResult3 ^ SHA224(9379).rotr(A, 13);
        let tmp21 = tmp20 ^ SHA224(9379).rotr(A, 22);
        tmp22 = D + tmp18 | 0;
        tmp23 = tmp18 + (tmp21 + SHA224(9385).Maj(A, B, C) | 0) | 0;
        num3 = num3 + 1;
        H = G;
        tmp24 = G;
        G = F;
