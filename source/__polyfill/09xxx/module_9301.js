// Module ID: 9301
// Function ID: 9302
// Dependencies: [41, 42, 93, 95, 98, 9295, 9296]
// Exports: Chi, Maj
...
const entry = {
  ...
  value: function update(B) {
    let tmp8;
    const self = this;
    HashMD(9296).exists(this);
    ({ buffer, blockLen, view } = this);
    const toBytesResult = HashMD(9295).toBytes(B);
    let num = 0;
...
let items = [
  entry,
  {
    ...
    value: function digestInto(content) {
      const self = this;
      HashMD(9296).exists(this);
      HashMD(9296).output(content, this);
      this.finished = true;
      ...
      self.process(view, 0);
      const view1 = HashMD(9295).createView(content);
