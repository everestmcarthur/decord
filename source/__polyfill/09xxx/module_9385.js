// Module ID: 9385
// Function ID: 9386
// Dependencies: [41, 42, 93, 95, 98, 9379, 9380]
// Exports: Chi, Maj
...
const entry = {
  ...
  value: function update(B) {
    let tmp8;
    const self = this;
    HashMD(9380).exists(this);
    ({ buffer, blockLen, view } = this);
    const toBytesResult = HashMD(9379).toBytes(B);
    let num = 0;
...
let items = [
  entry,
  {
    ...
    value: function digestInto(content) {
      const self = this;
      HashMD(9380).exists(this);
      HashMD(9380).output(content, this);
      this.finished = true;
      ...
      self.process(view, 0);
      const view1 = HashMD(9379).createView(content);
