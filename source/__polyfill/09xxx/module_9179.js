// Module ID: 9179
// Function ID: 9180
// Dependencies: [41, 42, 93, 95, 98, 9173, 9174]
// Exports: Chi, Maj
...
const entry = {
  ...
  value: function update(B) {
    let tmp8;
    const self = this;
    HashMD(9174).exists(this);
    ({ buffer, blockLen, view } = this);
    const toBytesResult = HashMD(9173).toBytes(B);
    let num = 0;
...
let items = [
  entry,
  {
    ...
    value: function digestInto(content) {
      const self = this;
      HashMD(9174).exists(this);
      HashMD(9174).output(content, this);
      this.finished = true;
      ...
      self.process(view, 0);
      const view1 = HashMD(9173).createView(content);
