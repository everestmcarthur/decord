// Module ID: 9290
// Function ID: 9291
// Dependencies: [41, 42, 93, 95, 98, 9284, 9285]
// Exports: Chi, Maj
...
const entry = {
  ...
  value: function update(B) {
    let tmp8;
    const self = this;
    HashMD(9285).exists(this);
    ({ buffer, blockLen, view } = this);
    const toBytesResult = HashMD(9284).toBytes(B);
    let num = 0;
...
let items = [
  entry,
  {
    ...
    value: function digestInto(content) {
      const self = this;
      HashMD(9285).exists(this);
      HashMD(9285).output(content, this);
      this.finished = true;
      ...
      self.process(view, 0);
      const view1 = HashMD(9284).createView(content);
