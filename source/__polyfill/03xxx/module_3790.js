// Module ID: 3790
// Function ID: 3791
// Dependencies: [3791, 3792, 3793, 3794, 3795]

// Module 3790
import module_3791 from "module_3791" /* 3791 */;
import module_3792 from "module_3792" /* 3792 */;
import module_3793 from "module_3793" /* 3793 */;
import date_mod from "module_3794" /* 3794 */;
import date_mod from "module_3795" /* 3795 */;

if (!module_3791) {
  const obj = { default: module_3791 };
  let tmp3 = obj;
} else {
  tmp3 = module_3791;
}
if (!module_3792) {
  const obj2 = { default: module_3792 };
  let tmp5 = obj2;
} else {
  tmp5 = module_3792;
}
if (!module_3793) {
  const obj3 = { default: module_3793 };
  let tmp7 = obj3;
} else {
  tmp7 = module_3793;
}
let date = date_mod;
if (!date) {
  const obj4 = { default: date };
  let tmp9 = obj4;
} else {
  tmp9 = date;
}
let date = date_mod;
if (!date) {
  const obj5 = { default: date };
  let tmp11 = obj5;
} else {
  tmp11 = date;
}

export default { code: "it", formatDistance: tmp3.default, formatLong: tmp5.default, formatRelative: tmp7.default, localize: tmp9.default, match: tmp11.default, options: { weekStartsOn: 1, firstWeekContainsDate: 4 } };
export default exports.default;
