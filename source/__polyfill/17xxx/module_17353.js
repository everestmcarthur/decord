// Module ID: 17353
// Function ID: 17354
// Dependencies: [17354, 17355]

// Module 17353
import expBCP47Syntax2 from "expBCP47Syntax" /* 17354 */;
          ...
          if (StringResult in ObjectResult) {
            ...
            if (null == tmp4) {
              ...
              if (typeof tmp4 === "string") {
                ...
                if (isMatch1) {
                  let expVariantDupes = tmp5(17354).expVariantDupes;
                  let isMatch = expVariantDupes.test(str);
                  let tmp9 = !isMatch;
                  if (!isMatch) {
                    let expSingletonDupes = tmp5(17354).expSingletonDupes;
                ...
                if (isMatch1) {
                  ...
                  if (!tmp20) {
                    ...
                    let RegExpResult = RegExp(`(?:${tmp17(17354).expExtSequences.source})+`, "i");
...
function InitializeDateTimeFormat(prototype, arg1, arg2) {
  ...
  if (true === result["[[initializedIntlObject]]"]) {
    ...
    if ("[object Array]" !== (typeof call6 === "unknown" ? toString() : call6(formats))) {
      dateTimeFormats = _mod17355.createDateTimeFormats(formats);
    }
    obj10 = Object.create(List.prototype);
    List("basic", "best fit");
...
class List {
  constructor() {
    tmp = f67311(this, "length", { writable: true, value: 0 });
    if (!arguments.length) {
      return;
    } else {
      tmp3 = slice;
...
fn(obj, "__addLocaleData", {
  value(locale) {
    ...
    if (isMatch1) {
      const expVariantDupes = tmp(17354).expVariantDupes;
      const isMatch = expVariantDupes.test(locale);
      let tmp5 = !isMatch;
      if (!isMatch) {
        const expSingletonDupes = tmp(17354).expSingletonDupes;
