// Module ID: 16614
// Function ID: 16615
// Dependencies: [41, 42, 93, 95, 98, 19, 17, 21, 16615]
...
function captureRef(current, options) {
  if (ViewShot(16615)) {
    ...
    return tmp(16615).captureRef(tmp15, options);
  } else {
    const _Error = Error;
...
function releaseCapture(str) {
  if (typeof str === "string") {
    ViewShot(16615).releaseCapture(str);
    const obj = ViewShot(16615);
  }
}
...
export const captureScreen = function captureScreen(options) {
  if (ViewShot(16615)) {
    ...
    return tmp(16615).captureScreen(options);
  } else {
    const _Error = Error;
    const error = new Error("react-native-view-shot: NativeModules.RNViewShot is undefined. Make sure the library is linked on the native side.");
    throw error;
