// Module ID: 16575
// Function ID: 16576
// Dependencies: [41, 42, 93, 95, 98, 19, 17, 21, 16576]
...
function captureRef(current, options) {
  if (ViewShot(16576)) {
    ...
    return tmp(16576).captureRef(tmp15, options);
  } else {
    const _Error = Error;
...
function releaseCapture(str) {
  if (typeof str === "string") {
    ViewShot(16576).releaseCapture(str);
    const obj = ViewShot(16576);
  }
}
...
export const captureScreen = function captureScreen(options) {
  if (ViewShot(16576)) {
    ...
    return tmp(16576).captureScreen(options);
  } else {
    const _Error = Error;
    const error = new Error("react-native-view-shot: NativeModules.RNViewShot is undefined. Make sure the library is linked on the native side.");
    throw error;
