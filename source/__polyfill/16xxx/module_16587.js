// Module ID: 16587
// Function ID: 16588
// Dependencies: [41, 42, 93, 95, 98, 19, 17, 21, 16588]
...
function captureRef(current, options) {
  if (ViewShot(16588)) {
    ...
    return tmp(16588).captureRef(tmp15, options);
  } else {
    const _Error = Error;
...
function releaseCapture(str) {
  if (typeof str === "string") {
    ViewShot(16588).releaseCapture(str);
    const obj = ViewShot(16588);
  }
}
...
export const captureScreen = function captureScreen(options) {
  if (ViewShot(16588)) {
    ...
    return tmp(16588).captureScreen(options);
  } else {
    const _Error = Error;
    const error = new Error("react-native-view-shot: NativeModules.RNViewShot is undefined. Make sure the library is linked on the native side.");
    throw error;
