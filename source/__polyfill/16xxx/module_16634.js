// Module ID: 16634
// Function ID: 16635
// Dependencies: [41, 42, 93, 95, 98, 19, 17, 21, 16635]
...
function captureRef(current, options) {
  if (ViewShot(16635)) {
    ...
    return tmp(16635).captureRef(tmp15, options);
  } else {
    const _Error = Error;
...
function releaseCapture(str) {
  if (typeof str === "string") {
    ViewShot(16635).releaseCapture(str);
    const obj = ViewShot(16635);
  }
}
...
export const captureScreen = function captureScreen(options) {
  if (ViewShot(16635)) {
    ...
    return tmp(16635).captureScreen(options);
  } else {
    const _Error = Error;
    const error = new Error("react-native-view-shot: NativeModules.RNViewShot is undefined. Make sure the library is linked on the native side.");
    throw error;
