// Module ID: 8601
// Function ID: 8602
// Dependencies: [93, 95, 98, 41, 42, 19, 17, 8602, 8595, 8603, 8604]
// Exports: invert, matrixTransform, multiplyMatrices

// Module 8601
import extractBrushDefault from "extractBrush" /* 8595 */;
  ...
  constructor(arg0) {
    ...
    tmp3Result.setNativeProps = (obj) => {
      for (const key10004 in arg0) {
        let BrushProperties = Shape(8602).BrushProperties;
    ...
    tmp3Result.getBBox = (arg0) => {
      ...
      return Shape(8603).default.getBBox(tmp5, { fill: fill2, stroke: stroke2, markers: markers2, clipped: clipped2 });
    };
    tmp3Result.getCTM = () => {
      const tmp = findNodeHandle(closure_0.root);
      const _default = Shape(8603).default;
      return new importDefaultResult1Result(Shape(8603).default.getCTM(tmp));
    };
    tmp3Result.getScreenCTM = () => {
      const tmp = findNodeHandle(closure_0.root);
      const _default = Shape(8603).default;
      return new importDefaultResult1Result(Shape(8603).default.getScreenCTM(tmp));
    };
    tmp3Result.isPointInFill = (arg0) => {
      const tmp = findNodeHandle(closure_0.root);
      return Shape(8603).default.isPointInFill(tmp, arg0);
    };
    tmp3Result.isPointInStroke = (arg0) => {
      const tmp = findNodeHandle(closure_0.root);
      return Shape(8603).default.isPointInStroke(tmp, arg0);
    };
    tmp3Result.getTotalLength = () => {
      const tmp = findNodeHandle(closure_0.root);
      return Shape(8603).default.getTotalLength(tmp);
    };
    tmp3Result.getPointAtLength = (length) => {
      const tmp = findNodeHandle(closure_0.root);
      const _default = Shape(8603).default;
      const obj = { length };
      return new importDefaultResult1Result1(Shape(8603).default.getPointAtLength(tmp, { length }));
    };
