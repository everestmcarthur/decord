// Module ID: 8031
// Function ID: 8032
// Dependencies: [32, 19, 17, 21, 1485, 1615, 5717, 8032, 4990, 8033, 8035, 8036, 8037, 8038]
...
function SceneView(arg0) {
  ...
  const context = noop.useContext(num6(5717).HeaderShownContext);
  let num5 = noop.useContext(num6(5717).HeaderHeightContext);
  const context1 = noop.useContext(num6(5717).HeaderBackContext);
  const obj2 = num6(1615);
  const frameSize = num6(5717).useFrameSize((width) => width.width > width.height);
  num6 = 0;
  if (!context) {
    let top;
  ...
  const obj4 = num6(5717);
  const frameSize1 = num6(5717).useFrameSize((arg0) => 56 + num6);
  const tmp8Result = num6(5717);
  let num7 = 2;
  const tmp8Result5 = num6(1485);
  [tmp17, tmp18] = noop.useState(frameSize1);
  dependencyMap = tmp18;
  const tmp16 = _slicedToArray(noop.useState(frameSize1), 2);
  const callback = obj3.useCallback(num6(8032).debounce(tmp18, 100), []);
  let tmp21 = "usesNewAndroidHeaderHeightImplementation" in tmp8(4990).compatibilityFlags;
  if (tmp21) {
    tmp21 = true === tmp8(4990).compatibilityFlags.usesNewAndroidHeaderHeightImplementation;
  }
  _slicedToArray = 0;
  let num8 = 0;
  if (null == header) {
  ...
  if (previousDescriptor) {
    title = tmp8(5717).getHeaderTitle(previousDescriptor.options, previousDescriptor.route.name);
    const tmp8Result7 = tmp8(5717);
  ...
  let eventResult;
  const headerConfigProps = num6(8033).useHeaderConfigProps(obj5);
  if (null == header) {
  ...
  const tmp8Result8 = num6(8033);
  obj20.children = closure_10(num6(5717).HeaderBackContext.Provider, { value: memo1, children: descriptor.render() });
  items5[2] = closure_10(num6(5717).HeaderShownContext.Provider, obj20);
  obj13.children = items5;
  obj12.children = tmp38(num6(5717).HeaderHeightContext.Provider, obj13);
  obj10.children = closure_10(num6(8035).AnimatedHeaderHeightContext.Provider, obj12);
  obj9.children = closure_10(num6(4990).ScreenStackItem, obj10);
...
export const NativeStackView = function NativeStackView(state) {
  ...
  const setNextDismissedKey = require("module_8036").useDismissedRouteError(state).setNextDismissedKey;
  let obj = require("module_8036");
  const invalidPreventRemoveError = require("module_8037").useInvalidPreventRemoveError(descriptors);
  const obj2 = require("module_8037");
  const modalRouteKeys = require("module_8038").getModalRouteKeys(state.routes, descriptors);
  ...
  return closure_10(require("module_5717").SafeAreaProviderCompat, obj4);
};
