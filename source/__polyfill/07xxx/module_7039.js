// Module ID: 7039
// Function ID: 7040
// Dependencies: [32, 19, 17, 21, 7033, 7034, 1505, 7040, 7019, 5654, 7041, 7042, 7043, 7044]
// Exports: Card

// Module 7039
import PanGestureHandler from "PanGestureHandler" /* 7019 */;
...
export const Card = function Card(shadowEnabled) {
  ...
  const first1 = interpolationIndex(opening.useState(() => {
    value = new RN.Value(_mod7034.getInvertedMultiplier(gestureDirection, "rtl" === direction));
    return value;
  }), 1)[0];
  ...
  closure_32 = num(1505)(() => {
    if (null == ref3.current) {
      const InteractionManager = _mod7040.InteractionManager;
  ...
  const layoutEffect = opening.useLayoutEffect(() => {
    ...
    first1.setValue(_mod7034.getInvertedMultiplier(gestureDirection, "rtl" === direction));
  }, items);
  opening.useRef(null);
  ...
  let tmp11 = num(1505)(() => {
    ...
    if (ref.current) {
      ...
      if (ref7.current) {
        if (typeof getAnimateToValue === "function") {
          if (tmp5.current.closing) {
            let num2 = _mod7033.getDistanceForDirection(tmp9, tmp10, "rtl" === tmp11);
          } else {
            num2 = 0;
          }
          tmp7 = num2;
      ...
      if (typeof getAnimateToValue === "function") {
        if (tmp16) {
          let num3 = _mod7033.getDistanceForDirection(tmp17, tmp18, "rtl" === tmp19);
        } else {
          num3 = 0;
        }
        if (tmp7 === num3) {
          if (ref2.current === num3) {
            ...
            if (tmp24) {
              gesture.setValue(_mod7033.getDistanceForDirection(tmp17, tmp18, "rtl" === tmp19));
  ...
  if (typeof backgroundColor === "string") {
    tmp16 = 0 === tmp5(5654)(backgroundColor).alpha();
    const obj14 = tmp5(5654)(backgroundColor);
  }
  ...
  const obj8 = {
    ...
    onHandlerStateChange: num(1505)((nativeEvent) => {
      ...
      if (PanGestureHandler.GestureState.ACTIVE === state) {
        ...
        if (tmp(7019).GestureState.CANCELLED !== state) {
          if (tmp(7019).GestureState.FAILED !== state) {
            if (tmp(7019).GestureState.END === state) {
              first3.setValue(0);
              if ("vertical" !== gestureDirection) {
  ...
  let merged = Object.assign(flag2(7042).gestureActivationCriteria({ layout, direction, gestureDirection, gestureResponseDistance }));
  if (typeof direction === "function") {
    ...
    items13[1] = tmp19(tmp18(7043).CardContent, obj12);
    obj9.children = items13;
    obj8.children = tmp17(tmp25, obj9);
    obj7.children = tmp19(tmp18(7019).PanGestureHandler, obj8);
    items5[2] = tmp19(tmp20.View, obj7);
    obj3.children = items5;
    return tmp17(flag2(7041).CardAnimationContext.Provider, obj3);
  } else {
    throw new TypeError("Trying to call a non-function");
  }
  const tmp18Result = flag2(7042);
};
