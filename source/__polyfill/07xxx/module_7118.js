// Module ID: 7118
// Function ID: 7119
// Dependencies: [109, 41, 42, 93, 95, 98, 19, 17, 21, 7119, 7122, 1485, 5719, 1615, 7126, 7128]
...
const jsx = fn(21).jsx;
let GestureHandlerRootView = fn(7119).GestureHandlerRootView;
...
class StackView {
  constructor() {
    ...
    tmp3Result.renderHeader = (arg0) => {
      ...
      return jsx(closure_0(7122).HeaderContainer, {});
    };
            ...
            children(arg0) {
              ...
              return self(closure_0(state[12]).HeaderShownContext.Consumer, {
                children(isParentHeaderShown) {
                  ...
                  return jsx(StackView(7128).CardStack, { insets: preloadedDescriptors, isParentHeaderShown, isParentModal: preloadedDescriptors, getPreviousRoute: self.getPreviousRoute, routes, openingRouteKeys, closingRouteKeys, onOpenRoute: self.handleOpenRoute, onCloseRoute: self.handleCloseRoute, onTransitionStart: self.handleTransitionStart, onTransitionEnd: self.handleTransitionEnd, renderHeader: self.renderHeader, state, descriptors, onGestureStart: self.handleGestureStart, onGestureEnd: self.handleGestureEnd, onGestureCancel: self.handleGestureCancel, preloadedDescriptors });
                }
              });
            }
          });
        ...
        if (tmp6.key !== tmp7.key) {
          if (arr3.some((key) => key.key === closure_7.key)) {
            if (!substr1.some((key) => key.key === closure_6.key)) {
              ...
              tmp23 = arr3(7128).getAnimationEnabled(animation) && !found2.includes(tmp6.key);
            }
          }
          let animation1;
      ...
      if (tmp8) {
        ...
        HermesBuiltin.arraySpread(routes5.filter((key) => {
          if (typeof isAnimationEnabled === "function") {
            ...
            let animationEnabled = StackView(7128).getAnimationEnabled(animation);
            if (animationEnabled) {
