// Module ID: 7077
// Function ID: 7078
// Dependencies: [32, 19, 17, 21, 7071, 7072, 1505, 7078, 7057, 5690, 7079, 7080, 7081, 7082]
// Exports: Card
...
export const Card = function Card(shadowEnabled) {
  ...
  let tmp11 = num(1505)(() => {
    ...
    if (ref.current) {
      ...
      if (ref7.current) {
        if (typeof getAnimateToValue === "function") {
          if (tmp5.current.closing) {
            let num2 = _mod7071.getDistanceForDirection(tmp9, tmp10, "rtl" === tmp11);
          } else {
            num2 = 0;
          }
          tmp7 = num2;
      ...
      if (typeof getAnimateToValue === "function") {
        if (tmp16) {
          let num3 = _mod7071.getDistanceForDirection(tmp17, tmp18, "rtl" === tmp19);
        } else {
          num3 = 0;
        }
        if (tmp7 === num3) {
          if (ref2.current === num3) {
            ...
            if (tmp24) {
              gesture.setValue(_mod7071.getDistanceForDirection(tmp17, tmp18, "rtl" === tmp19));
  ...
  const obj8 = {
    ...
    onHandlerStateChange: num(1505)((nativeEvent) => {
      ...
      if (PanGestureHandler.GestureState.ACTIVE === state) {
        ...
        if (tmp(7057).GestureState.CANCELLED !== state) {
          if (tmp(7057).GestureState.FAILED !== state) {
            if (tmp(7057).GestureState.END === state) {
              first3.setValue(0);
              if ("vertical" !== gestureDirection) {
  ...
  if (typeof direction === "function") {
    ...
    items13[1] = tmp19(tmp18(7081).CardContent, obj12);
    obj9.children = items13;
    obj8.children = tmp17(tmp25, obj9);
    obj7.children = tmp19(tmp18(7057).PanGestureHandler, obj8);
    items5[2] = tmp19(tmp20.View, obj7);
    obj3.children = items5;
    return tmp17(flag2(7079).CardAnimationContext.Provider, obj3);
  } else {
    throw new TypeError("Trying to call a non-function");
  }
