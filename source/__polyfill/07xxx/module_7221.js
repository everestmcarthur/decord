// Module ID: 7221
// Function ID: 7222
// Dependencies: [32, 19, 17, 21, 7215, 7216, 1506, 7222, 7201, 5810, 7223, 7224, 7225, 7226]
// Exports: Card

// Module 7221
import PanGestureHandler from "PanGestureHandler" /* 7201 */;
import _mod7215 from "module_7215" /* 7215 */;
import _mod7216 from "module_7216" /* 7216 */;
import _mod7222 from "module_7222" /* 7222 */;
...
export const Card = function Card(shadowEnabled) {
  ...
  const first1 = interpolationIndex(opening.useState(() => {
    value = new RN.Value(_mod7216.getInvertedMultiplier(gestureDirection, "rtl" === direction));
    return value;
  }), 1)[0];
  ...
  closure_32 = num(1506)(() => {
    if (null == ref3.current) {
      const InteractionManager = _mod7222.InteractionManager;
  ...
  const layoutEffect = opening.useLayoutEffect(() => {
    ...
    first1.setValue(_mod7216.getInvertedMultiplier(gestureDirection, "rtl" === direction));
  }, items);
  opening.useRef(null);
  ...
  let tmp11 = num(1506)(() => {
    ...
    if (ref.current) {
      ...
      if (ref7.current) {
        if (typeof getAnimateToValue === "function") {
          if (tmp5.current.closing) {
            let num2 = _mod7215.getDistanceForDirection(tmp9, tmp10, "rtl" === tmp11);
          } else {
            num2 = 0;
          }
          tmp7 = num2;
      ...
      if (typeof getAnimateToValue === "function") {
        if (tmp16) {
          let num3 = _mod7215.getDistanceForDirection(tmp17, tmp18, "rtl" === tmp19);
        } else {
          num3 = 0;
        }
        if (tmp7 === num3) {
          if (ref2.current === num3) {
            ...
            if (tmp24) {
              gesture.setValue(_mod7215.getDistanceForDirection(tmp17, tmp18, "rtl" === tmp19));
  ...
  if (typeof backgroundColor === "string") {
    tmp16 = 0 === tmp5(5810)(backgroundColor).alpha();
    const obj14 = tmp5(5810)(backgroundColor);
  }
  ...
  const obj8 = {
    ...
    onHandlerStateChange: num(1506)((nativeEvent) => {
      ...
      if (PanGestureHandler.GestureState.ACTIVE === state) {
        ...
        if (tmp(7201).GestureState.CANCELLED !== state) {
          if (tmp(7201).GestureState.FAILED !== state) {
            if (tmp(7201).GestureState.END === state) {
              first3.setValue(0);
              if ("vertical" !== gestureDirection) {
  ...
  let merged = Object.assign(flag2(7224).gestureActivationCriteria({ layout, direction, gestureDirection, gestureResponseDistance }));
  if (typeof direction === "function") {
    ...
    items13[1] = tmp19(tmp18(7225).CardContent, obj12);
    obj9.children = items13;
    obj8.children = tmp17(tmp25, obj9);
    obj7.children = tmp19(tmp18(7201).PanGestureHandler, obj8);
    items5[2] = tmp19(tmp20.View, obj7);
    obj3.children = items5;
    return tmp17(flag2(7223).CardAnimationContext.Provider, obj3);
  } else {
    throw new TypeError("Trying to call a non-function");
  }
  const tmp18Result = flag2(7224);
};
