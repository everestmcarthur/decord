// Module ID: 7989
// Function ID: 7990
// Dependencies: [32, 19, 17, 21, 1484, 1614, 5682, 7990, 4958, 7991, 7993, 7994, 7995, 7996]
...
function SceneView(arg0) {
  ...
  const context = noop.useContext(num6(5682).HeaderShownContext);
  let num5 = noop.useContext(num6(5682).HeaderHeightContext);
  const context1 = noop.useContext(num6(5682).HeaderBackContext);
  const obj2 = num6(1614);
  const frameSize = num6(5682).useFrameSize((width) => width.width > width.height);
  num6 = 0;
  if (!context) {
    let top;
  ...
  const obj4 = num6(5682);
  const frameSize1 = num6(5682).useFrameSize((arg0) => 56 + num6);
  const tmp8Result = num6(5682);
  let num7 = 2;
  const tmp8Result5 = num6(1484);
  [tmp17, tmp18] = noop.useState(frameSize1);
  dependencyMap = tmp18;
  const tmp16 = _slicedToArray(noop.useState(frameSize1), 2);
  const callback = obj3.useCallback(num6(7990).debounce(tmp18, 100), []);
  let tmp21 = "usesNewAndroidHeaderHeightImplementation" in tmp8(4958).compatibilityFlags;
  if (tmp21) {
    tmp21 = true === tmp8(4958).compatibilityFlags.usesNewAndroidHeaderHeightImplementation;
  }
  _slicedToArray = 0;
  let num8 = 0;
  if (null == header) {
  ...
  if (previousDescriptor) {
    title = tmp8(5682).getHeaderTitle(previousDescriptor.options, previousDescriptor.route.name);
    const tmp8Result7 = tmp8(5682);
  ...
  let eventResult;
  const headerConfigProps = num6(7991).useHeaderConfigProps(obj5);
  if (null == header) {
  ...
  const tmp8Result8 = num6(7991);
  obj20.children = closure_10(num6(5682).HeaderBackContext.Provider, { value: memo1, children: descriptor.render() });
  items5[2] = closure_10(num6(5682).HeaderShownContext.Provider, obj20);
  obj13.children = items5;
  obj12.children = tmp38(num6(5682).HeaderHeightContext.Provider, obj13);
  obj10.children = closure_10(num6(7993).AnimatedHeaderHeightContext.Provider, obj12);
  obj9.children = closure_10(num6(4958).ScreenStackItem, obj10);
...
export const NativeStackView = function NativeStackView(state) {
  ...
  const setNextDismissedKey = require("module_7994").useDismissedRouteError(state).setNextDismissedKey;
  let obj = require("module_7994");
  const invalidPreventRemoveError = require("module_7995").useInvalidPreventRemoveError(descriptors);
  const obj2 = require("module_7995");
  const modalRouteKeys = require("module_7996").getModalRouteKeys(state.routes, descriptors);
  ...
  return closure_10(require("module_5682").SafeAreaProviderCompat, obj4);
};
