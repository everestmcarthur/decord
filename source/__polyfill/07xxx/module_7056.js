// Module ID: 7056
// Function ID: 7057
// Dependencies: [109, 41, 42, 93, 95, 98, 19, 17, 21, 7057, 7060, 1484, 5681, 1614, 7064, 7066]
...
const jsx = fn(21).jsx;
let GestureHandlerRootView = fn(7057).GestureHandlerRootView;
...
class StackView {
  constructor() {
    ...
    obj1 = { routes: [], previousState: "y", openingRouteKeys: "totp", closingRouteKeys: null, replacingRouteKeys: "lch", descriptors: "hcl" };
    ...
    tmp3Result.renderHeader = (arg0) => {
      ...
      return jsx(closure_0(7060).HeaderContainer, {});
    };
            ...
            children(arg0) {
              ...
              return self(closure_0(state[12]).HeaderShownContext.Consumer, {
                children(isParentHeaderShown) {
                  ...
                  return jsx(StackView(7066).CardStack, { insets: preloadedDescriptors, isParentHeaderShown, isParentModal: preloadedDescriptors, getPreviousRoute: self.getPreviousRoute, routes, openingRouteKeys, closingRouteKeys, onOpenRoute: self.handleOpenRoute, onCloseRoute: self.handleCloseRoute, onTransitionStart: self.handleTransitionStart, onTransitionEnd: self.handleTransitionEnd, renderHeader: self.renderHeader, state, descriptors, onGestureStart: self.handleGestureStart, onGestureEnd: self.handleGestureEnd, onGestureCancel: self.handleGestureCancel, preloadedDescriptors });
                }
              });
            }
          });
        ...
        if (tmp6.key !== tmp7.key) {
          if (arr3.some((key) => key.key === closure_7.key)) {
            if (!substr1.some((key) => key.key === closure_6.key)) {
              ...
              tmp23 = arr3(7066).getAnimationEnabled(animation) && !found2.includes(tmp6.key);
            }
          }
          let animation1;
      ...
      if (tmp8) {
        ...
        HermesBuiltin.arraySpread(routes5.filter((key) => {
          if (typeof isAnimationEnabled === "function") {
            ...
            let animationEnabled = StackView(7066).getAnimationEnabled(animation);
            if (animationEnabled) {
