// Module ID: 7078
// Function ID: 7079
// Dependencies: [109, 41, 42, 93, 95, 98, 19, 17, 21, 7079, 7082, 1484, 5683, 1614, 7086, 7088]
...
const jsx = fn(21).jsx;
let GestureHandlerRootView = fn(7079).GestureHandlerRootView;
...
class StackView {
  constructor() {
    ...
    tmp3Result.renderHeader = (arg0) => {
      ...
      return jsx(closure_0(7082).HeaderContainer, {});
    };
            ...
            children(arg0) {
              ...
              return self(closure_0(state[12]).HeaderShownContext.Consumer, {
                children(isParentHeaderShown) {
                  ...
                  return jsx(StackView(7088).CardStack, { insets: preloadedDescriptors, isParentHeaderShown, isParentModal: preloadedDescriptors, getPreviousRoute: self.getPreviousRoute, routes, openingRouteKeys, closingRouteKeys, onOpenRoute: self.handleOpenRoute, onCloseRoute: self.handleCloseRoute, onTransitionStart: self.handleTransitionStart, onTransitionEnd: self.handleTransitionEnd, renderHeader: self.renderHeader, state, descriptors, onGestureStart: self.handleGestureStart, onGestureEnd: self.handleGestureEnd, onGestureCancel: self.handleGestureCancel, preloadedDescriptors });
                }
              });
            }
          });
        ...
        if (tmp6.key !== tmp7.key) {
          if (arr3.some((key) => key.key === closure_7.key)) {
            if (!substr1.some((key) => key.key === closure_6.key)) {
              ...
              tmp23 = arr3(7088).getAnimationEnabled(animation) && !found2.includes(tmp6.key);
            }
          }
          let animation1;
      ...
      if (tmp8) {
        ...
        HermesBuiltin.arraySpread(routes5.filter((key) => {
          if (typeof isAnimationEnabled === "function") {
            ...
            let animationEnabled = StackView(7088).getAnimationEnabled(animation);
            if (animationEnabled) {
