...
class ScrollView {
  constructor(arg0) {
    ...
    tmp3Result.getScrollResponder = () => f107619;
    tmp3Result.getScrollableNode = () => renderElement.findNodeHandle(f107619.getNativeScrollRef());
    tmp3Result.getInnerViewNode = () => renderElement.findNodeHandle(f107619._innerView.nativeInstance);
    tmp3Result.getInnerViewRef = () => f107619._innerView.nativeInstance;
    tmp3Result.getNativeScrollRef = () => f107619._scrollView.nativeInstance;
    tmp3Result.scrollTo = (num, arg1, arg2) => {
      ...
      const nativeScrollRef = f107619.getNativeScrollRef();
    ...
    tmp3Result.scrollResponderScrollNativeHandleToKeyboard = (measureLayout, arg1, _preventNegativeScrollOffset) => {
      ...
      f107619._additionalScrollOffset = num;
      f107619._preventNegativeScrollOffset = _preventNegativeScrollOffset;
      if (null != f107619._innerView.nativeInstance) {
    ...
    tmp3Result.scrollResponderZoomTo = (animated, arg1) => {
      ...
      if ("animated" in animated) {
        f107619._animated = animated.animated;
        delete tmp[tmp2];
      ...
      if (null == _keyboardMetrics._keyboardMetrics) {
        ...
        const timerId = setTimeout(() => {
          if (typeof scrollTextInputIntoVisibleRect === "function") {
            if (null != f107619._keyboardMetrics) {
              screenY = obj._keyboardMetrics.screenY;
            }
            const sum = closure_0 - screenY + closure_1 + obj._additionalScrollOffset;
            let bound = sum;
            if (true === f107619._preventNegativeScrollOffset) {
              const _Math = Math;
              bound = Math.max(0, sum);
            }
            const point = { x: 0, y: bound, animated: true };
            f107619.scrollTo(point);
            f107619._additionalScrollOffset = 0;
            f107619._preventNegativeScrollOffset = false;
          } else {
    ...
    tmp3Result._handleScroll = (arg0) => {
      f107619._observedScrollSinceBecomingResponder = true;
      if (f107619.props.onScroll) {
        const props = tmp.props;
        props.onScroll(arg0);
      }
    };
    tmp3Result._handleLayout = (nativeEvent) => {
      if (true === f107619.props.invertStickyHeaders) {
        const obj2 = { layoutHeight: nativeEvent.nativeEvent.layout.height };
        obj.setState(obj2);
      }
      if (f107619.props.onLayout) {
        const props = obj.props;
        props.onLayout(nativeEvent);
      }
    };
    tmp3Result._handleContentOnLayout = (arg0) => {
      if (f107619.props.onContentSizeChange) {
        const props = f107619.props;
    ...
    f107619 = (arg0) => Object.assign(arg0, { getScrollResponder: f107619.getScrollResponder, getScrollableNode: f107619.getScrollableNode, getInnerViewNode: f107619.getInnerViewNode, getInnerViewRef: f107619.getInnerViewRef, getNativeScrollRef: f107619.getNativeScrollRef, scrollTo: f107619.scrollTo, scrollToEnd: f107619.scrollToEnd, flashScrollIndicators: f107619.flashScrollIndicators, scrollResponderZoomTo: f107619.scrollResponderZoomTo, scrollResponderScrollNativeHandleToKeyboard: f107619.scrollResponderScrollNativeHandleToKeyboard });
    closure_1 = undefined;
    obj4 = {
      getForwardingRef: c2(closure_4[29])((arg0) => {
            closure_0 = arg0;
    ...
    tmp3Result.scrollResponderKeyboardWillShow = (endCoordinates) => {
      f107619._keyboardMetrics = endCoordinates.endCoordinates;
      if (f107619.props.onKeyboardWillShow) {
        const props = f107619.props;
        props.onKeyboardWillShow(endCoordinates);
      }
    };
    tmp3Result.scrollResponderKeyboardWillHide = (arg0) => {
      f107619._keyboardMetrics = null;
      if (f107619.props.onKeyboardWillHide) {
        const props = tmp.props;
    ...
    tmp3Result.scrollResponderKeyboardDidShow = (endCoordinates) => {
      f107619._keyboardMetrics = endCoordinates.endCoordinates;
      if (f107619.props.onKeyboardDidShow) {
        const props = f107619.props;
    ...
    tmp3Result.scrollResponderKeyboardDidHide = (arg0) => {
      f107619._keyboardMetrics = null;
      if (f107619.props.onKeyboardDidHide) {
        const props = tmp.props;
    ...
    tmp3Result._handleMomentumScrollBegin = (arg0) => {
      ...
      f107619._lastMomentumScrollBeginTime = _performance.now();
      if (f107619.props.onMomentumScrollBegin) {
        const props = f107619.props;
    ...
    tmp3Result._handleMomentumScrollEnd = (arg0) => {
      ...
      const _performance = ScrollView.performance;
      f107619._lastMomentumScrollEndTime = _performance.now();
      if (f107619.props.onMomentumScrollEnd) {
        const props = f107619.props;
    ...
    tmp3Result._handleScrollBeginDrag = (arg0) => {
      ...
      if ("on-drag" === f107619.props.keyboardDismissMode) {
        dismissKeyboardDefault();
      }
      if (f107619.props.onScrollBeginDrag) {
    ...
    tmp3Result._handleScrollEndDrag = (nativeEvent) => {
      ...
      let _isAnimatingResult = f107619._isAnimating();
      ...
      if (f107619.props.onScrollEndDrag) {
        const props = tmp.props;
    ...
    tmp3Result._isAnimating = () => {
      const _performance = ScrollView.performance;
      return _performance.now() - f107619._lastMomentumScrollEndTime < 16 || f107619._lastMomentumScrollEndTime < f107619._lastMomentumScrollBeginTime;
    };
    tmp3Result._handleResponderGrant = (arg0) => {
      f107619._observedScrollSinceBecomingResponder = false;
      if (f107619.props.onResponderGrant) {
        const props = obj.props;
        props.onResponderGrant(arg0);
      }
      f107619._becameResponderWhileAnimating = f107619._isAnimating();
    };
    tmp3Result._handleResponderReject = () => {

    };
    tmp3Result._handleResponderRelease = (nativeEvent) => {
      f107619._isTouching = 0 !== nativeEvent.nativeEvent.touches.length;
      if (f107619.props.onResponderRelease) {
        const props = obj.props;
    ...
    tmp3Result._handleResponderTerminationRequest = () => !f107619._observedScrollSinceBecomingResponder;
    tmp3Result._handleScrollShouldSetResponder = () => true !== f107619.props.disableScrollViewPanResponder && f107619._isTouching;
    tmp3Result._handleStartShouldSetResponder = (target) => {
      if (true === f107619.props.disableScrollViewPanResponder) {
        return false;
      } else {
    ...
    tmp3Result._softKeyboardIsDetached = () => {
      let tmp = null != f107619._keyboardMetrics;
      if (tmp) {
        tmp = 0 === f107619._keyboardMetrics.height;
      }
      return tmp;
    };
    ...
    tmp3Result._handleTouchEnd = (nativeEvent) => {
      f107619._isTouching = 0 !== nativeEvent.nativeEvent.touches.length;
      const keyboardShouldPersistTaps = f107619.props.keyboardShouldPersistTaps;
      let tmp = !keyboardShouldPersistTaps;
      if (keyboardShouldPersistTaps) {
        tmp = "never" === keyboardShouldPersistTaps;
      }
