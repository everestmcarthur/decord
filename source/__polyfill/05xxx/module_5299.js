// Module ID: 5299
// Function ID: 5300
// Dependencies: [32, 42, 41, 93, 95, 98, 158, 5264, 5300, 5301, 5302]

// Module 5299
import _wrapNativeSuperDefault from "_wrapNativeSuper" /* 158 */;
import _modDef5300 from "module_5300" /* 5300 */;
import calculateGPSValueDefault from "calculateGPSValue" /* 5302 */;
...
function readTags(_raw, buffer, arg2) {
  try {
    ...
    ParseError(5264).objectAssign(_raw, parseXMPObject(convertToObject(tmp6, true)));
    return true;
  } catch (err) {
    return false;
  }
  ...
  try {
    ...
    if ("ParseError" === tmp12.name) {
      if (obj2.isMissingNamespaceError(tmp12)) {
        if (!tmp4) {
          return parseFromString(tmp2, tmp13(5301).addMissingNamespaces(tmp), true);
        }
      }
      obj2 = ParseError(5301);
      tmp13 = ParseError;
    }
    throw tmp12;
  }
    ...
    for (const key10002 in arg0) {
      ...
      let item = arr2.forEach((attributes) => {
        ...
        if (typeof attributes.value === "object") {
          ParseError(5264).objectAssign(obj, parseNodeChildrenAsTags(attributes.value));
          const tmpResult = ParseError(5264);
        }
      });
      continue;
    }
  ...
  if (Array.isArray(arr)) {
    ...
    if (tmp2) {
      ...
      if (typeof calculateGPSValueDefault[tmp2] === "function") {
        tmp12 = tmp13(5302)[tmp2](arr, tmp11);
        const tmp13Result = tmp13(5302);
      }
      tmp13 = importDefault;
    }
    return tmp12;
...
function parseNodeAsSimpleRdfDescription(attributes, key10005) {
  ...
  ParseError(5264).objectAssign(obj, obj4, obj6);
  const prop = iter.value["rdf:value"];
  const obj3 = ParseError(5264);
...
function parseNodeAsStructureRdfDescription(value, key10005) {
  ...
  if (undefined !== value.value["rdf:Description"]) {
    ParseError(5264).objectAssign(obj.value, parseNodeAttributesAsTags(value.value["rdf:Description"].attributes));
    const obj4 = ParseError(5264);
    const obj6 = {};
    ...
    ParseError(5264).objectAssign(obj.attributes, obj6);
    iter = value.value["rdf:Description"];
    const obj5 = ParseError(5264);
  }
  ParseError(5264).objectAssign(obj.value, parseNodeChildrenAsTags(iter.value));
