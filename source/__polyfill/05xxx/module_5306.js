// Module ID: 5306
// Function ID: 5307
// Dependencies: [5265, 5307]

// Module 5306
import _mod5265 from "module_5265" /* 5265 */;
import iccTags from "iccTags" /* 5307 */;
            ...
            if (uint321 > buffer.length) {
              ...
              if (stringFromDataView1 === desc) {
                ...
                if (uint323 > uint322) {
                  ...
                  if (tmp55(5307).iccTags[stringFromDataView]) {
                    let obj3 = { value: applyResult, description: applyResult };
                    obj2[tmp55(5307).iccTags[stringFromDataView].name] = obj3;
                  } else {
                ...
                if (1 === uint324) {
                  text = items[0].text;
                  if (tmp25(5307).iccTags[stringFromDataView]) {
                    let obj9 = { value: text, description: text };
                    obj2[tmp25(5307).iccTags[stringFromDataView].name] = obj9;
                  } else {
                  ...
                  if (iccTags.iccTags[stringFromDataView]) {
                    ...
                    obj2[tmp33(5307).iccTags[stringFromDataView].name] = obj12;
                  } else {
                ...
                if (tmp55(5307).iccTags[stringFromDataView]) {
                  ...
                  obj2[tmp55(5307).iccTags[stringFromDataView].name] = obj14;
                } else {
                ...
                if (tmp55(5307).iccTags[stringFromDataView]) {
                  ...
                  obj2[tmp55(5307).iccTags[stringFromDataView].name] = obj16;
                } else {
...
export default {
  read(buffer, arr, arg2) {
    if (arg2) {
      if (arr[0].compressionMethod !== _mod5265.COMPRESSION_METHOD_NONE) {
        if (arr[0].compressionMethod === tmp(5265).COMPRESSION_METHOD_DEFLATE) {
          let _DataView = DataView;
          buffer = buffer.buffer;
          let dataView = new DataView(buffer.slice(arr[0].offset, arr[0].offset + arr[0].length));
          const tmpResult = tmp(5265);
