// Module ID: 5031
// Function ID: 5032
// Dependencies: [109, 19, 17, 21, 5019, 5029, 5028, 5032, 5033, 5036, 5018, 5038, 5009]

// Module 5031
import _modDef5009 from "module_5009" /* 5009 */;
import _modDef5028 from "module_5028" /* 5028 */;
import styleDefault from "style" /* 5033 */;
...
export default noop.forwardRef(function ScreenStackItem(arg0, ref) {
  ...
  dependencyMap = flag5.useContext(tmp4(5029).RNSScreensRefContext);
  ...
  const effect = obj2.useEffect(() => {
    _modDef5028(false, "Dynamically changing header's visibility in modals will result in remounting the screen and losing all local state.");
  ...
  let obj = screenId(5019);
  const tmp3 = !hidden;
  if (isIOS26OrHigher) {
    isIOS26OrHigher = tmp10;
  }
  if (isIOS26OrHigher) {
    isIOS26OrHigher = tmp4(5032).isIOS26OrHigher;
  }
  _modDef5028(isIOS26OrHigher, "[RNScreens] Using both `blurEffect` and `scrollEdgeEffects` simultaneously may cause overlapping effects.");
  ...
  if (screenId(5032).isIOS26OrHigher) {
    const obj5 = { edges: {}, children };
    tmp19Result = tmp19(tmp4(5036).SafeAreaView, obj5);
  }
  obj4.children = tmp19Result;
  obj3.children = closure_4(styleDefault, obj4);
  const items1 = [closure_4(screenId(5019).EdgeInsetApplicationContext.Provider, obj3), , ];
  const merged1 = Object.assign(headerConfig);
  items1[1] = closure_4(screenId(5018).ScreenStackHeaderConfig, {});
  let tmp19Result2 = "formSheet" === stackPresentation && unstable_sheetFooter;
  if (tmp19Result2) {
    const obj7 = { children: unstable_sheetFooter() };
    tmp19Result2 = tmp19(tmp4(5038).FooterComponent, obj7);
  }
  ...
  return closure_4(_modDef5009, obj8);
});
