...
export default {
  isWebpFile(dataView) {
    let tmp = dataView;
    if (tmp) {
      tmp = _mod5301.getStringFromDataView(dataView, 0, 4) === "RIFF";
    }
    if (tmp) {
      tmp = _mod5301.getStringFromDataView(dataView, 8, 4) === "WEBP";
    }
    return tmp;
  },
