    ...
    if (tmp < byteLength.byteLength) {
      while (true) {
        ...
        if (byteLength.getUint8(tmp) !== 28) {
          ...
          obj3.name = combined;
          obj3.value = items;
          obj3.description = getTagDescription(tmp8(5295).iptc[uint16], items, obj, encoding);
          let tmp17 = tmp8(5295).iptc[uint16] && tmp8(5295).iptc[uint16].repeatable;
          if (tmp17) {
            obj3.repeatable = true;
          }
          let tmp18 = tmp8(5295).iptc[uint16] && undefined !== tmp8(5295).iptc[uint16].encoding_name;
          if (tmp18) {
            let obj5 = tmp8(5295).iptc[uint16];
