...
export default {
  read(byteLength, sum) {
    ...
    if (sum + 4 <= byteLength.byteLength) {
      const longAt = _modDef5280.getLongAt(byteLength, sum);
    ...
    if (sum + 4 + 4 <= byteLength.byteLength) {
      const longAt1 = _modDef5280.getLongAt(byteLength, sum + 4);
    ...
    if (sum + 8 + 1 <= byteLength.byteLength) {
      const byteAt = _modDef5280.getByteAt(byteLength, sum + 8);
