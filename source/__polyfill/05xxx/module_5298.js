// Module ID: 5298
// Function ID: 5299
// Dependencies: [32, 42, 41, 93, 95, 98, 158, 5263, 5299, 5300, 5301]

// Module 5298
import _wrapNativeSuperDefault from "_wrapNativeSuper" /* 158 */;
import _modDef5299 from "module_5299" /* 5299 */;
import calculateGPSValueDefault from "calculateGPSValue" /* 5301 */;
...
function readTags(_raw, buffer, arg2) {
  try {
    ...
    ParseError(5263).objectAssign(_raw, parseXMPObject(convertToObject(tmp6, true)));
    return true;
  } catch (err) {
    return false;
  }
  ...
  try {
    ...
    if ("ParseError" === tmp12.name) {
      if (obj2.isMissingNamespaceError(tmp12)) {
        if (!tmp4) {
          return parseFromString(tmp2, tmp13(5300).addMissingNamespaces(tmp), true);
        }
      }
      obj2 = ParseError(5300);
      tmp13 = ParseError;
    }
    throw tmp12;
  }
    ...
    for (const key10002 in arg0) {
      ...
      let item = arr2.forEach((attributes) => {
        ...
        if (typeof attributes.value === "object") {
          ParseError(5263).objectAssign(obj, parseNodeChildrenAsTags(attributes.value));
          const tmpResult = ParseError(5263);
        }
      });
      continue;
    }
  ...
  if (Array.isArray(arr)) {
    ...
    if (tmp2) {
      ...
      if (typeof calculateGPSValueDefault[tmp2] === "function") {
        tmp12 = tmp13(5301)[tmp2](arr, tmp11);
        const tmp13Result = tmp13(5301);
      }
      tmp13 = importDefault;
    }
    return tmp12;
...
function parseNodeAsSimpleRdfDescription(attributes, key10005) {
  ...
  ParseError(5263).objectAssign(obj, obj4, obj6);
  const prop = iter.value["rdf:value"];
  const obj3 = ParseError(5263);
...
function parseNodeAsStructureRdfDescription(value, key10005) {
  ...
  if (undefined !== value.value["rdf:Description"]) {
    ParseError(5263).objectAssign(obj.value, parseNodeAttributesAsTags(value.value["rdf:Description"].attributes));
    const obj4 = ParseError(5263);
    const obj6 = {};
    ...
    ParseError(5263).objectAssign(obj.attributes, obj6);
    iter = value.value["rdf:Description"];
    const obj5 = ParseError(5263);
  }
  ParseError(5263).objectAssign(obj.value, parseNodeChildrenAsTags(iter.value));
