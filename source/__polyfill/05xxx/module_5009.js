// Module ID: 5009
// Function ID: 5010
// Dependencies: [109, 19, 17, 21, 5010, 5011, 5012, 5008, 5013, 5014, 5016, 4999, 5017]

// Module 5009
import get_synchronousScreenUpdatesEnabledDefault from "get synchronousScreenUpdatesEnabled" /* 4999 */;
import _modDef5010 from "module_5010" /* 5010 */;
import animatedComponentDefault from "animatedComponent" /* 5011 */;
import _mod5012 from "module_5012" /* 5012 */;
import DelayedFreezeDefault from "DelayedFreeze" /* 5014 */;
...
const jsx = fn(21).jsx;
const __INTERNAL_VIEW_CONFIG = Animated.createAnimatedComponent(_modDef5010);
...
const forwardRefResult = noop.forwardRef(function InnerScreen(activityState, ref) {
  ...
  const previous = _mod5012.usePrevious(activityState.activityState);
  ...
  if (undefined === enabled) {
    enabled = tmp2(5008).screensEnabled();
    const tmp2Result = tmp2(5008);
  }
  let freezeOnBlur = activityState.freezeOnBlur;
  if (undefined === freezeOnBlur) {
    freezeOnBlur = tmp2(5008).freezeEnabled();
    const tmp2Result6 = tmp2(5008);
  }
  ...
  if (undefined === SHEET_DIMMED_ALWAYS) {
    SHEET_DIMMED_ALWAYS = tmp2(5013).SHEET_DIMMED_ALWAYS;
  }
  ...
  if (enabled) {
    if (tmp2(5008).isNativePlatformSupported) {
      const sheetAllowedDetents1 = tmp2(5013).resolveSheetAllowedDetents(sheetAllowedDetents);
      const tmp2Result7 = tmp2(5013);
      const sheetLargestUndimmedDetent = tmp2(5013).resolveSheetLargestUndimmedDetent(SHEET_DIMMED_ALWAYS, sheetAllowedDetents1.length - 1);
      const tmp2Result8 = tmp2(5013);
      ...
      const sheetInitialDetentIndex1 = tmp2(5013).resolveSheetInitialDetentIndex(num3, sheetAllowedDetents1.length - 1);
      ...
      obj4.fullScreenSwipeEnabled = tmp2(5016).parseBooleanToOptionalBooleanNativeProp(fullScreenSwipeEnabled);
      let num7;
      ...
      if (isNativeStack) {
        ...
        tmp31Result = tmp31(tmp32(5017).Provider, obj7);
      }
