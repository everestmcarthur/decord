// Module ID: 5008
// Function ID: 5009
// Dependencies: [109, 19, 17, 21, 5009, 5010, 5011, 5007, 5012, 5013, 5015, 4998, 5016]

// Module 5008
import get_synchronousScreenUpdatesEnabledDefault from "get synchronousScreenUpdatesEnabled" /* 4998 */;
import _modDef5009 from "module_5009" /* 5009 */;
import animatedComponentDefault from "animatedComponent" /* 5010 */;
import _mod5011 from "module_5011" /* 5011 */;
import DelayedFreezeDefault from "DelayedFreeze" /* 5013 */;
...
const jsx = fn(21).jsx;
const __INTERNAL_VIEW_CONFIG = Animated.createAnimatedComponent(_modDef5009);
...
const forwardRefResult = noop.forwardRef(function InnerScreen(activityState, ref) {
  ...
  const previous = _mod5011.usePrevious(activityState.activityState);
  ...
  if (undefined === enabled) {
    enabled = tmp2(5007).screensEnabled();
    const tmp2Result = tmp2(5007);
  }
  let freezeOnBlur = activityState.freezeOnBlur;
  if (undefined === freezeOnBlur) {
    freezeOnBlur = tmp2(5007).freezeEnabled();
    const tmp2Result6 = tmp2(5007);
  }
  ...
  if (undefined === SHEET_DIMMED_ALWAYS) {
    SHEET_DIMMED_ALWAYS = tmp2(5012).SHEET_DIMMED_ALWAYS;
  }
  ...
  if (enabled) {
    if (tmp2(5007).isNativePlatformSupported) {
      const sheetAllowedDetents1 = tmp2(5012).resolveSheetAllowedDetents(sheetAllowedDetents);
      const tmp2Result7 = tmp2(5012);
      const sheetLargestUndimmedDetent = tmp2(5012).resolveSheetLargestUndimmedDetent(SHEET_DIMMED_ALWAYS, sheetAllowedDetents1.length - 1);
      const tmp2Result8 = tmp2(5012);
      ...
      const sheetInitialDetentIndex1 = tmp2(5012).resolveSheetInitialDetentIndex(num3, sheetAllowedDetents1.length - 1);
      ...
      obj4.fullScreenSwipeEnabled = tmp2(5015).parseBooleanToOptionalBooleanNativeProp(fullScreenSwipeEnabled);
      let num7;
      ...
      if (isNativeStack) {
        ...
        tmp31Result = tmp31(tmp32(5016).Provider, obj7);
      }
