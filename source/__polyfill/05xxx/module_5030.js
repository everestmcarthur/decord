// Module ID: 5030
// Function ID: 5031
// Dependencies: [109, 19, 17, 21, 5018, 5028, 5027, 5031, 5032, 5035, 5017, 5037, 5008]

// Module 5030
import _modDef5008 from "module_5008" /* 5008 */;
import _modDef5027 from "module_5027" /* 5027 */;
import styleDefault from "style" /* 5032 */;
...
export default noop.forwardRef(function ScreenStackItem(arg0, ref) {
  ...
  dependencyMap = flag5.useContext(tmp4(5028).RNSScreensRefContext);
  ...
  const effect = obj2.useEffect(() => {
    _modDef5027(false, "Dynamically changing header's visibility in modals will result in remounting the screen and losing all local state.");
  ...
  let obj = screenId(5018);
  const tmp3 = !hidden;
  if (isIOS26OrHigher) {
    isIOS26OrHigher = tmp10;
  }
  if (isIOS26OrHigher) {
    isIOS26OrHigher = tmp4(5031).isIOS26OrHigher;
  }
  _modDef5027(isIOS26OrHigher, "[RNScreens] Using both `blurEffect` and `scrollEdgeEffects` simultaneously may cause overlapping effects.");
  ...
  if (screenId(5031).isIOS26OrHigher) {
    const obj5 = { edges: {}, children };
    tmp19Result = tmp19(tmp4(5035).SafeAreaView, obj5);
  }
  obj4.children = tmp19Result;
  obj3.children = closure_4(styleDefault, obj4);
  const items1 = [closure_4(screenId(5018).EdgeInsetApplicationContext.Provider, obj3), , ];
  const merged1 = Object.assign(headerConfig);
  items1[1] = closure_4(screenId(5017).ScreenStackHeaderConfig, {});
  let tmp19Result2 = "formSheet" === stackPresentation && unstable_sheetFooter;
  if (tmp19Result2) {
    const obj7 = { children: unstable_sheetFooter() };
    tmp19Result2 = tmp19(tmp4(5037).FooterComponent, obj7);
  }
  ...
  return closure_4(_modDef5008, obj8);
});
