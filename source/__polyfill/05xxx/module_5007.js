// Module ID: 5007
// Function ID: 5008
// Dependencies: [109, 19, 17, 21, 5008, 5009, 5010, 5006, 5011, 5012, 5014, 4997, 5015]

// Module 5007
import get_synchronousScreenUpdatesEnabledDefault from "get synchronousScreenUpdatesEnabled" /* 4997 */;
import _modDef5008 from "module_5008" /* 5008 */;
import animatedComponentDefault from "animatedComponent" /* 5009 */;
import _mod5010 from "module_5010" /* 5010 */;
import DelayedFreezeDefault from "DelayedFreeze" /* 5012 */;
...
const jsx = fn(21).jsx;
const __INTERNAL_VIEW_CONFIG = Animated.createAnimatedComponent(_modDef5008);
...
const forwardRefResult = noop.forwardRef(function InnerScreen(activityState, ref) {
  ...
  const previous = _mod5010.usePrevious(activityState.activityState);
  ...
  if (undefined === enabled) {
    enabled = tmp2(5006).screensEnabled();
    const tmp2Result = tmp2(5006);
  }
  let freezeOnBlur = activityState.freezeOnBlur;
  if (undefined === freezeOnBlur) {
    freezeOnBlur = tmp2(5006).freezeEnabled();
    const tmp2Result6 = tmp2(5006);
  }
  ...
  if (undefined === SHEET_DIMMED_ALWAYS) {
    SHEET_DIMMED_ALWAYS = tmp2(5011).SHEET_DIMMED_ALWAYS;
  }
  ...
  if (enabled) {
    if (tmp2(5006).isNativePlatformSupported) {
      const sheetAllowedDetents1 = tmp2(5011).resolveSheetAllowedDetents(sheetAllowedDetents);
      const tmp2Result7 = tmp2(5011);
      const sheetLargestUndimmedDetent = tmp2(5011).resolveSheetLargestUndimmedDetent(SHEET_DIMMED_ALWAYS, sheetAllowedDetents1.length - 1);
      const tmp2Result8 = tmp2(5011);
      ...
      const sheetInitialDetentIndex1 = tmp2(5011).resolveSheetInitialDetentIndex(num3, sheetAllowedDetents1.length - 1);
      ...
      obj4.fullScreenSwipeEnabled = tmp2(5014).parseBooleanToOptionalBooleanNativeProp(fullScreenSwipeEnabled);
      let num7;
      ...
      if (isNativeStack) {
        ...
        tmp31Result = tmp31(tmp32(5015).Provider, obj7);
      }
