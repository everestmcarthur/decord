// Module ID: 5113
// Function ID: 5114
// Dependencies: [109, 19, 17, 21, 5101, 5111, 5110, 5114, 5115, 5118, 5100, 5120, 5091]

// Module 5113
import _modDef5091 from "module_5091" /* 5091 */;
import _modDef5110 from "module_5110" /* 5110 */;
import styleDefault from "style" /* 5115 */;
...
export default noop.forwardRef(function ScreenStackItem(arg0, ref) {
  ...
  dependencyMap = flag5.useContext(tmp4(5111).RNSScreensRefContext);
  ...
  const effect = obj2.useEffect(() => {
    _modDef5110(false, "Dynamically changing header's visibility in modals will result in remounting the screen and losing all local state.");
  ...
  let obj = screenId(5101);
  const tmp3 = !hidden;
  if (isIOS26OrHigher) {
    isIOS26OrHigher = tmp10;
  }
  if (isIOS26OrHigher) {
    isIOS26OrHigher = tmp4(5114).isIOS26OrHigher;
  }
  _modDef5110(isIOS26OrHigher, "[RNScreens] Using both `blurEffect` and `scrollEdgeEffects` simultaneously may cause overlapping effects.");
  ...
  if (screenId(5114).isIOS26OrHigher) {
    const obj5 = { edges: {}, children };
    tmp19Result = tmp19(tmp4(5118).SafeAreaView, obj5);
  }
  obj4.children = tmp19Result;
  obj3.children = closure_4(styleDefault, obj4);
  const items1 = [closure_4(screenId(5101).EdgeInsetApplicationContext.Provider, obj3), , ];
  const merged1 = Object.assign(headerConfig);
  items1[1] = closure_4(screenId(5100).ScreenStackHeaderConfig, {});
  let tmp19Result2 = "formSheet" === stackPresentation && unstable_sheetFooter;
  if (tmp19Result2) {
    const obj7 = { children: unstable_sheetFooter() };
    tmp19Result2 = tmp19(tmp4(5120).FooterComponent, obj7);
  }
  ...
  return closure_4(_modDef5091, obj8);
});
