// Module ID: 5029
// Function ID: 5030
// Dependencies: [109, 19, 17, 21, 5017, 5027, 5026, 5030, 5031, 5034, 5016, 5036, 5007]

// Module 5029
import _modDef5007 from "module_5007" /* 5007 */;
import _modDef5026 from "module_5026" /* 5026 */;
import styleDefault from "style" /* 5031 */;
...
export default noop.forwardRef(function ScreenStackItem(arg0, ref) {
  ...
  dependencyMap = flag5.useContext(tmp4(5027).RNSScreensRefContext);
  ...
  const effect = obj2.useEffect(() => {
    _modDef5026(false, "Dynamically changing header's visibility in modals will result in remounting the screen and losing all local state.");
  ...
  let obj = screenId(5017);
  const tmp3 = !hidden;
  if (isIOS26OrHigher) {
    isIOS26OrHigher = tmp10;
  }
  if (isIOS26OrHigher) {
    isIOS26OrHigher = tmp4(5030).isIOS26OrHigher;
  }
  _modDef5026(isIOS26OrHigher, "[RNScreens] Using both `blurEffect` and `scrollEdgeEffects` simultaneously may cause overlapping effects.");
  ...
  if (screenId(5030).isIOS26OrHigher) {
    const obj5 = { edges: {}, children };
    tmp19Result = tmp19(tmp4(5034).SafeAreaView, obj5);
  }
  obj4.children = tmp19Result;
  obj3.children = closure_4(styleDefault, obj4);
  const items1 = [closure_4(screenId(5017).EdgeInsetApplicationContext.Provider, obj3), , ];
  const merged1 = Object.assign(headerConfig);
  items1[1] = closure_4(screenId(5016).ScreenStackHeaderConfig, {});
  let tmp19Result2 = "formSheet" === stackPresentation && unstable_sheetFooter;
  if (tmp19Result2) {
    const obj7 = { children: unstable_sheetFooter() };
    tmp19Result2 = tmp19(tmp4(5036).FooterComponent, obj7);
  }
  ...
  return closure_4(_modDef5007, obj8);
});
