// Module ID: 5091
// Function ID: 5092
// Dependencies: [109, 19, 17, 21, 5092, 5093, 5094, 5090, 5095, 5096, 5098, 5081, 5099]

// Module 5091
import get_synchronousScreenUpdatesEnabledDefault from "get synchronousScreenUpdatesEnabled" /* 5081 */;
import _modDef5092 from "module_5092" /* 5092 */;
import animatedComponentDefault from "animatedComponent" /* 5093 */;
import _mod5094 from "module_5094" /* 5094 */;
import DelayedFreezeDefault from "DelayedFreeze" /* 5096 */;
...
const jsx = fn(21).jsx;
const __INTERNAL_VIEW_CONFIG = Animated.createAnimatedComponent(_modDef5092);
...
const forwardRefResult = noop.forwardRef(function InnerScreen(activityState, ref) {
  ...
  const previous = _mod5094.usePrevious(activityState.activityState);
  ...
  if (undefined === enabled) {
    enabled = tmp2(5090).screensEnabled();
    const tmp2Result = tmp2(5090);
  }
  let freezeOnBlur = activityState.freezeOnBlur;
  if (undefined === freezeOnBlur) {
    freezeOnBlur = tmp2(5090).freezeEnabled();
    const tmp2Result6 = tmp2(5090);
  }
  ...
  if (undefined === SHEET_DIMMED_ALWAYS) {
    SHEET_DIMMED_ALWAYS = tmp2(5095).SHEET_DIMMED_ALWAYS;
  }
  ...
  if (enabled) {
    if (tmp2(5090).isNativePlatformSupported) {
      const sheetAllowedDetents1 = tmp2(5095).resolveSheetAllowedDetents(sheetAllowedDetents);
      const tmp2Result7 = tmp2(5095);
      const sheetLargestUndimmedDetent = tmp2(5095).resolveSheetLargestUndimmedDetent(SHEET_DIMMED_ALWAYS, sheetAllowedDetents1.length - 1);
      const tmp2Result8 = tmp2(5095);
      ...
      const sheetInitialDetentIndex1 = tmp2(5095).resolveSheetInitialDetentIndex(num3, sheetAllowedDetents1.length - 1);
      ...
      obj4.fullScreenSwipeEnabled = tmp2(5098).parseBooleanToOptionalBooleanNativeProp(fullScreenSwipeEnabled);
      let num7;
      ...
      if (isNativeStack) {
        ...
        tmp31Result = tmp31(tmp32(5099).Provider, obj7);
      }
