// Module ID: 4998
// Function ID: 4999
// Dependencies: [109, 19, 17, 21, 4986, 4996, 4995, 4999, 5000, 5003, 4985, 5005, 4976]

// Module 4998
import _modDef4976 from "module_4976" /* 4976 */;
import _modDef4995 from "module_4995" /* 4995 */;
import styleDefault from "style" /* 5000 */;
...
export default noop.forwardRef(function ScreenStackItem(arg0, ref) {
  ...
  dependencyMap = flag5.useContext(tmp4(4996).RNSScreensRefContext);
  ...
  const effect = obj2.useEffect(() => {
    _modDef4995(false, "Dynamically changing header's visibility in modals will result in remounting the screen and losing all local state.");
  ...
  let obj = screenId(4986);
  const tmp3 = !hidden;
  if (isIOS26OrHigher) {
    isIOS26OrHigher = tmp10;
  }
  if (isIOS26OrHigher) {
    isIOS26OrHigher = tmp4(4999).isIOS26OrHigher;
  }
  _modDef4995(isIOS26OrHigher, "[RNScreens] Using both `blurEffect` and `scrollEdgeEffects` simultaneously may cause overlapping effects.");
  ...
  if (screenId(4999).isIOS26OrHigher) {
    const obj5 = { edges: {}, children };
    tmp19Result = tmp19(tmp4(5003).SafeAreaView, obj5);
  }
  obj4.children = tmp19Result;
  obj3.children = closure_4(styleDefault, obj4);
  const items1 = [closure_4(screenId(4986).EdgeInsetApplicationContext.Provider, obj3), , ];
  const merged1 = Object.assign(headerConfig);
  items1[1] = closure_4(screenId(4985).ScreenStackHeaderConfig, {});
  let tmp19Result2 = "formSheet" === stackPresentation && unstable_sheetFooter;
  if (tmp19Result2) {
    const obj7 = { children: unstable_sheetFooter() };
    tmp19Result2 = tmp19(tmp4(5005).FooterComponent, obj7);
  }
  ...
  return closure_4(_modDef4976, obj8);
});
