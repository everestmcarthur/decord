// Module ID: 4997
// Function ID: 4998
// Dependencies: [109, 19, 17, 21, 4985, 4995, 4994, 4998, 4999, 5002, 4984, 5004, 4975]

// Module 4997
import _modDef4975 from "module_4975" /* 4975 */;
import _modDef4994 from "module_4994" /* 4994 */;
import styleDefault from "style" /* 4999 */;
...
export default noop.forwardRef(function ScreenStackItem(arg0, ref) {
  ...
  dependencyMap = flag5.useContext(tmp4(4995).RNSScreensRefContext);
  ...
  const effect = obj2.useEffect(() => {
    _modDef4994(false, "Dynamically changing header's visibility in modals will result in remounting the screen and losing all local state.");
  ...
  let obj = screenId(4985);
  const tmp3 = !hidden;
  if (isIOS26OrHigher) {
    isIOS26OrHigher = tmp10;
  }
  if (isIOS26OrHigher) {
    isIOS26OrHigher = tmp4(4998).isIOS26OrHigher;
  }
  _modDef4994(isIOS26OrHigher, "[RNScreens] Using both `blurEffect` and `scrollEdgeEffects` simultaneously may cause overlapping effects.");
  ...
  if (screenId(4998).isIOS26OrHigher) {
    const obj5 = { edges: {}, children };
    tmp19Result = tmp19(tmp4(5002).SafeAreaView, obj5);
  }
  obj4.children = tmp19Result;
  obj3.children = closure_4(styleDefault, obj4);
  const items1 = [closure_4(screenId(4985).EdgeInsetApplicationContext.Provider, obj3), , ];
  const merged1 = Object.assign(headerConfig);
  items1[1] = closure_4(screenId(4984).ScreenStackHeaderConfig, {});
  let tmp19Result2 = "formSheet" === stackPresentation && unstable_sheetFooter;
  if (tmp19Result2) {
    const obj7 = { children: unstable_sheetFooter() };
    tmp19Result2 = tmp19(tmp4(5004).FooterComponent, obj7);
  }
  ...
  return closure_4(_modDef4975, obj8);
});
