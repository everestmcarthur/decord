// Module ID: 4930
// Function ID: 4931
// Dependencies: [109, 19, 17, 21, 4931, 4932, 4933, 4929, 4934, 4935, 4936, 4938, 4939, 4940]

// Module 4930
import _modDef4931 from "module_4931" /* 4931 */;
import animatedComponentDefault from "animatedComponent" /* 4932 */;
import _mod4933 from "module_4933" /* 4933 */;
import _modDef4935 from "module_4935" /* 4935 */;
import DelayedFreezeDefault from "DelayedFreeze" /* 4936 */;
...
const jsx = fn(21).jsx;
const __INTERNAL_VIEW_CONFIG = Animated.createAnimatedComponent(_modDef4931);
...
const forwardRefResult = noop.forwardRef(function InnerScreen(activityState, ref) {
  ...
  const previous = _mod4933.usePrevious(activityState.activityState);
  ...
  if (undefined === SHEET_DIMMED_ALWAYS) {
    SHEET_DIMMED_ALWAYS = tmp2(4934).SHEET_DIMMED_ALWAYS;
  }
  ...
  if (enabled) {
    if (tmp2(4929).isNativePlatformSupported) {
      const sheetAllowedDetents1 = tmp2(4934).resolveSheetAllowedDetents(sheetAllowedDetents);
      const tmp2Result7 = tmp2(4934);
      const sheetLargestUndimmedDetent = tmp2(4934).resolveSheetLargestUndimmedDetent(SHEET_DIMMED_ALWAYS, sheetAllowedDetents1.length - 1);
      const tmp2Result8 = tmp2(4934);
      ...
      const sheetInitialDetentIndex1 = tmp2(4934).resolveSheetInitialDetentIndex(num3, sheetAllowedDetents1.length - 1);
      ...
      _modDef4935(false, "[RNScreens] Using featureFlags `ios26AllowInteractionsDuringTransition` with `iosPreventReattachmentOfDismissedScreens` disabled is discouraged and will result in visual bugs on screen transitions. See flags description for details.");
      if (isNativeStack) {
      ...
      const items = [style, { zIndex: "disabled" }];
      obj4.style = items;
      ...
      obj4.fullScreenSwipeEnabled = tmp2(4938).parseBooleanToOptionalBooleanNativeProp(fullScreenSwipeEnabled);
      let num7;
      ...
      obj4.synchronousShadowStateUpdatesEnabled = get_controlledBottomTabsDefault.experiment.synchronousScreenUpdatesEnabled;
      obj4.androidResetScreenShadowStateOnOrientationChangeEnabled = get_controlledBottomTabsDefault.experiment.androidResetScreenShadowStateOnOrientationChangeEnabled;
      obj4.ios26AllowInteractionsDuringTransition = get_controlledBottomTabsDefault.experiment.ios26AllowInteractionsDuringTransition;
      ...
      if (isNativeStack) {
        ...
        tmp33Result = tmp33(tmp31(4940).Provider, obj7);
      }
      obj4.children = tmp33Result;
      obj3.children = <tmp25 />;
      return <tmp31Result freeze={freezeOnBlur}>{null}</tmp31Result>;
    }
  }
