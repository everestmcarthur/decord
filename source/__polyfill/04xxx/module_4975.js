// Module ID: 4975
// Function ID: 4976
// Dependencies: [109, 19, 17, 21, 4976, 4977, 4978, 4974, 4979, 4980, 4982, 4965, 4983]

// Module 4975
import get_synchronousScreenUpdatesEnabledDefault from "get synchronousScreenUpdatesEnabled" /* 4965 */;
import _modDef4976 from "module_4976" /* 4976 */;
import animatedComponentDefault from "animatedComponent" /* 4977 */;
import _mod4978 from "module_4978" /* 4978 */;
import DelayedFreezeDefault from "DelayedFreeze" /* 4980 */;
...
const jsx = fn(21).jsx;
const __INTERNAL_VIEW_CONFIG = Animated.createAnimatedComponent(_modDef4976);
...
const forwardRefResult = noop.forwardRef(function InnerScreen(activityState, ref) {
  ...
  const previous = _mod4978.usePrevious(activityState.activityState);
  ...
  if (undefined === enabled) {
    enabled = tmp2(4974).screensEnabled();
    const tmp2Result = tmp2(4974);
  }
  let freezeOnBlur = activityState.freezeOnBlur;
  if (undefined === freezeOnBlur) {
    freezeOnBlur = tmp2(4974).freezeEnabled();
    const tmp2Result6 = tmp2(4974);
  }
  ...
  if (undefined === SHEET_DIMMED_ALWAYS) {
    SHEET_DIMMED_ALWAYS = tmp2(4979).SHEET_DIMMED_ALWAYS;
  }
  ...
  if (enabled) {
    if (tmp2(4974).isNativePlatformSupported) {
      const sheetAllowedDetents1 = tmp2(4979).resolveSheetAllowedDetents(sheetAllowedDetents);
      const tmp2Result7 = tmp2(4979);
      const sheetLargestUndimmedDetent = tmp2(4979).resolveSheetLargestUndimmedDetent(SHEET_DIMMED_ALWAYS, sheetAllowedDetents1.length - 1);
      const tmp2Result8 = tmp2(4979);
      ...
      const sheetInitialDetentIndex1 = tmp2(4979).resolveSheetInitialDetentIndex(num3, sheetAllowedDetents1.length - 1);
      ...
      obj4.fullScreenSwipeEnabled = tmp2(4982).parseBooleanToOptionalBooleanNativeProp(fullScreenSwipeEnabled);
      let num7;
      ...
      if (isNativeStack) {
        ...
        tmp31Result = tmp31(tmp32(4983).Provider, obj7);
      }
