// Module ID: 4976
// Function ID: 4977
// Dependencies: [109, 19, 17, 21, 4977, 4978, 4979, 4975, 4980, 4981, 4983, 4966, 4984]

// Module 4976
import get_synchronousScreenUpdatesEnabledDefault from "get synchronousScreenUpdatesEnabled" /* 4966 */;
import _modDef4977 from "module_4977" /* 4977 */;
import animatedComponentDefault from "animatedComponent" /* 4978 */;
import _mod4979 from "module_4979" /* 4979 */;
import DelayedFreezeDefault from "DelayedFreeze" /* 4981 */;
...
const jsx = fn(21).jsx;
const __INTERNAL_VIEW_CONFIG = Animated.createAnimatedComponent(_modDef4977);
...
const forwardRefResult = noop.forwardRef(function InnerScreen(activityState, ref) {
  ...
  const previous = _mod4979.usePrevious(activityState.activityState);
  ...
  if (undefined === enabled) {
    enabled = tmp2(4975).screensEnabled();
    const tmp2Result = tmp2(4975);
  }
  let freezeOnBlur = activityState.freezeOnBlur;
  if (undefined === freezeOnBlur) {
    freezeOnBlur = tmp2(4975).freezeEnabled();
    const tmp2Result6 = tmp2(4975);
  }
  ...
  if (undefined === SHEET_DIMMED_ALWAYS) {
    SHEET_DIMMED_ALWAYS = tmp2(4980).SHEET_DIMMED_ALWAYS;
  }
  ...
  if (enabled) {
    if (tmp2(4975).isNativePlatformSupported) {
      const sheetAllowedDetents1 = tmp2(4980).resolveSheetAllowedDetents(sheetAllowedDetents);
      const tmp2Result7 = tmp2(4980);
      const sheetLargestUndimmedDetent = tmp2(4980).resolveSheetLargestUndimmedDetent(SHEET_DIMMED_ALWAYS, sheetAllowedDetents1.length - 1);
      const tmp2Result8 = tmp2(4980);
      ...
      const sheetInitialDetentIndex1 = tmp2(4980).resolveSheetInitialDetentIndex(num3, sheetAllowedDetents1.length - 1);
      ...
      obj4.fullScreenSwipeEnabled = tmp2(4983).parseBooleanToOptionalBooleanNativeProp(fullScreenSwipeEnabled);
      let num7;
      ...
      if (isNativeStack) {
        ...
        tmp31Result = tmp31(tmp32(4984).Provider, obj7);
      }
