// Module ID: 4974
// Function ID: 4975
// Dependencies: [109, 19, 17, 21, 4975, 4976, 4977, 4973, 4978, 4979, 4981, 4964, 4982]

// Module 4974
import get_synchronousScreenUpdatesEnabledDefault from "get synchronousScreenUpdatesEnabled" /* 4964 */;
import _modDef4975 from "module_4975" /* 4975 */;
import animatedComponentDefault from "animatedComponent" /* 4976 */;
import _mod4977 from "module_4977" /* 4977 */;
import DelayedFreezeDefault from "DelayedFreeze" /* 4979 */;
...
const jsx = fn(21).jsx;
const __INTERNAL_VIEW_CONFIG = Animated.createAnimatedComponent(_modDef4975);
...
const forwardRefResult = noop.forwardRef(function InnerScreen(activityState, ref) {
  ...
  const previous = _mod4977.usePrevious(activityState.activityState);
  ...
  if (undefined === enabled) {
    enabled = tmp2(4973).screensEnabled();
    const tmp2Result = tmp2(4973);
  }
  let freezeOnBlur = activityState.freezeOnBlur;
  if (undefined === freezeOnBlur) {
    freezeOnBlur = tmp2(4973).freezeEnabled();
    const tmp2Result6 = tmp2(4973);
  }
  ...
  if (undefined === SHEET_DIMMED_ALWAYS) {
    SHEET_DIMMED_ALWAYS = tmp2(4978).SHEET_DIMMED_ALWAYS;
  }
  ...
  if (enabled) {
    if (tmp2(4973).isNativePlatformSupported) {
      const sheetAllowedDetents1 = tmp2(4978).resolveSheetAllowedDetents(sheetAllowedDetents);
      const tmp2Result7 = tmp2(4978);
      const sheetLargestUndimmedDetent = tmp2(4978).resolveSheetLargestUndimmedDetent(SHEET_DIMMED_ALWAYS, sheetAllowedDetents1.length - 1);
      const tmp2Result8 = tmp2(4978);
      ...
      const sheetInitialDetentIndex1 = tmp2(4978).resolveSheetInitialDetentIndex(num3, sheetAllowedDetents1.length - 1);
      ...
      obj4.fullScreenSwipeEnabled = tmp2(4981).parseBooleanToOptionalBooleanNativeProp(fullScreenSwipeEnabled);
      let num7;
      ...
      if (isNativeStack) {
        ...
        tmp31Result = tmp31(tmp32(4982).Provider, obj7);
      }
