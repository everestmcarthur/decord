...
let fn = () => {
  ...
  function set$1(isValid, Date, module_3732) {
    if (isValid.isValid()) {
      const _isNaN = isNaN;
      if (!isNaN(module_3732)) {
        ({ _d, _isUTC } = isValid);
        if ("Milliseconds" === Date) {
          if (_isUTC) {
            _d.setUTCMilliseconds(module_3732);
          } else {
            _d.setMilliseconds(module_3732);
          }
        } else if ("Seconds" === Date) {
          if (_isUTC) {
            _d.setUTCSeconds(module_3732);
          } else {
            _d.setSeconds(module_3732);
          }
        } else if ("Minutes" === Date) {
          if (_isUTC) {
            _d.setUTCMinutes(module_3732);
          } else {
            _d.setMinutes(module_3732);
          }
        } else if ("Hours" === Date) {
          if (_isUTC) {
            _d.setUTCHours(module_3732);
          } else {
            _d.setHours(module_3732);
          }
        } else if ("Date" === Date) {
          if (_isUTC) {
            _d.setUTCDate(module_3732);
          } else {
            _d.setDate(module_3732);
          }
          ...
          if (29 === dateResult) {
            if (1 === monthResult) {
              const result = module_3732 % 4;
              let tmp7 = result === 0;
              if (result === 0) {
                tmp7 = module_3732 % 100 !== 0;
              }
              if (!tmp7) {
                tmp7 = module_3732 % 400 === 0;
              }
              let num7 = 28;
            }
            if (_isUTC) {
              _d.setUTCFullYear(module_3732, monthResult, num7);
            } else {
              _d.setFullYear(module_3732, monthResult, num7);
            }
          }
          num7 = dateResult;
        }
