// Module ID: 12997
// Function ID: 12998
// Dependencies: [12959, 12962, 12975, 12998, 12999, 12985, 12957, 13000, 12993, 12969]
...
function applyClientOptions(environment, environment2) {
  ...
  if (environment.message) {
    environment.message = _mod12957.truncate(environment.message, num);
  }
  value = iter;
  ...
  if (value) {
    iter.value = _mod12957.truncate(iter.value, num);
  }
  ...
  if (url) {
    request.url = _mod12957.truncate(request.url, num);
  }
}
function applyDebugIds(exception, arg1) {
  const filenameToDebugIdMap = _mod13000.getFilenameToDebugIdMap(arg1);
  try {
