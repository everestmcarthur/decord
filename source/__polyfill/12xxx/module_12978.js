// Module ID: 12978
// Function ID: 12979
// Dependencies: [12968, 12945, 12940]
...
export const logSpanEnd = function logSpanEnd(spanContext) {
  if (_mod12968.DEBUG_BUILD) {
    const spanToJSONResult = tmp(12945).spanToJSON(spanContext);
...
export const logSpanStart = function logSpanStart(spanContext) {
  if (_mod12968.DEBUG_BUILD) {
    ...
    const spanIsSampledResult = tmp(12945).spanIsSampled(spanContext);
    ...
    if (rootSpan !== spanContext) {
      ...
      ({ op: op2, description: description2 } = tmp(12945).spanToJSON(rootSpan));
      ...
      const spanToJSONResult1 = tmp(12945).spanToJSON(rootSpan);
    }
    const logger = tmp(12940).logger;
    const _HermesInternal9 = HermesInternal;
    logger.log("" + combined + "\n  " + items.join("\n  "));
    const tmpResult5 = tmp(12945);
  }
};
