// Module ID: 12951
// Function ID: 12952
// Dependencies: [12941, 12949, 12946]
...
export const addContextToFrame = function addContextToFrame(arr, lineno) {
  ...
  if (undefined !== lineno.lineno) {
    ...
    lineno.pre_context = substr.map((item) => _mod12949.snipLine(item, 0));
    const _Math5 = Math;
    ...
    lineno.context_line = _mod12949.snipLine(arr[bound1], num2);
    const _Math = Math;
    const substr1 = arr.slice(Math.min(bound + 1, length), bound + 1 + num);
    lineno.post_context = substr1.map((item) => _mod12949.snipLine(item, 0));
  }
};
...
export const checkOrSetAlreadyCaught = function checkOrSetAlreadyCaught(__sentry_captured__) {
  if ((function isAlreadyCaptured(__sentry_captured__) {
    ...
    try {
      const result = _mod12946.addNonEnumerableProperty(__sentry_captured__, "__sentry_captured__", true);
      return false;
    } catch (err) {
    }
  }
...
export const uuid4 = function uuid4() {
  const GLOBAL_OBJ = _mod12941.GLOBAL_OBJ;
