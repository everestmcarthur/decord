// Module ID: 12964
// Function ID: 12965
// Dependencies: [12954, 12931, 12926]
...
export const logSpanEnd = function logSpanEnd(spanContext) {
  if (_mod12954.DEBUG_BUILD) {
    const spanToJSONResult = tmp(12931).spanToJSON(spanContext);
...
export const logSpanStart = function logSpanStart(spanContext) {
  if (_mod12954.DEBUG_BUILD) {
    ...
    const spanIsSampledResult = tmp(12931).spanIsSampled(spanContext);
    ...
    if (rootSpan !== spanContext) {
      const tmpResult6 = tmp(12931);
      ({ op: op2, description: description2 } = tmp(12931).spanToJSON(rootSpan));
      ...
      const spanToJSONResult1 = tmp(12931).spanToJSON(rootSpan);
    }
    const logger = tmp(12926).logger;
    const _HermesInternal9 = HermesInternal;
    logger.log("" + combined + "\n  " + items.join("\n  "));
    const tmpResult5 = tmp(12931);
  }
};
