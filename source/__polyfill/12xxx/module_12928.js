// Module ID: 12928
// Function ID: 12929
// Dependencies: [12918, 12895, 12890]
...
export const logSpanEnd = function logSpanEnd(spanContext) {
  if (_mod12918.DEBUG_BUILD) {
    const spanToJSONResult = tmp(12895).spanToJSON(spanContext);
...
export const logSpanStart = function logSpanStart(spanContext) {
  if (_mod12918.DEBUG_BUILD) {
    ...
    const spanIsSampledResult = tmp(12895).spanIsSampled(spanContext);
    ...
    if (rootSpan !== spanContext) {
      const tmpResult6 = tmp(12895);
      ({ op: op2, description: description2 } = tmp(12895).spanToJSON(rootSpan));
      ...
      const spanToJSONResult1 = tmp(12895).spanToJSON(rootSpan);
    }
    const logger = tmp(12890).logger;
    const _HermesInternal9 = HermesInternal;
    logger.log("" + combined + "\n  " + items.join("\n  "));
    const tmpResult5 = tmp(12895);
  }
};
