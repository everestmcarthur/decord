// Module ID: 12847
// Function ID: 12848
// Dependencies: [41, 42, 12848, 12834, 12845, 12853]
...
function withScope(arg0) {
  const mainCarrier = AsyncContextStack(12845).getMainCarrier();
  const obj = AsyncContextStack(12845);
  const sentryCarrier = AsyncContextStack(12845).getSentryCarrier(mainCarrier);
  let stack = sentryCarrier.stack;
  if (!stack) {
    const defaultCurrentScope = tmp(12853).getDefaultCurrentScope();
    const tmpResult = tmp(12853);
    stack = new _moduleResult(defaultCurrentScope, tmp(12853).getDefaultIsolationScope());
    const tmpResult2 = tmp(12853);
  }
...
function withIsolationScope(arg0) {
  ...
  if (!stack) {
    const defaultCurrentScope = tmp(12853).getDefaultCurrentScope();
    const tmpResult = tmp(12853);
    stack = new closure_3(defaultCurrentScope, tmp(12853).getDefaultIsolationScope());
    const tmpResult2 = tmp(12853);
  }
  sentryCarrier.stack = stack;
  return stack.withScope(() => {
    ...
    let stack = sentryCarrier.stack;
    if (!stack) {
      const defaultCurrentScope = tmp2(12853).getDefaultCurrentScope();
      const tmp2Result = tmp2(12853);
      stack = new closure_2_3(defaultCurrentScope, tmp2(12853).getDefaultIsolationScope());
      const tmp2Result2 = tmp2(12853);
    }
    sentryCarrier.stack = stack;
...
export function getStackAsyncContextStrategy() {
  return {
    ...
    withSetIsolationScope(arg0, arg1) {
      closure_0 = arg1;
      let mainCarrier = closure_0(12845).getMainCarrier();
      let obj = closure_0(12845);
      let sentryCarrier = closure_0(12845).getSentryCarrier(mainCarrier);
      let stack = sentryCarrier.stack;
      if (!stack) {
        let defaultCurrentScope = tmp(12853).getDefaultCurrentScope();
        const tmpResult = tmp(12853);
        stack = new closure_3(defaultCurrentScope, tmp(12853).getDefaultIsolationScope());
        const tmpResult2 = tmp(12853);
      }
    ...
    getCurrentScope() {
      ...
      if (!stack) {
        const defaultCurrentScope = tmp(12853).getDefaultCurrentScope();
        const tmpResult = tmp(12853);
        stack = new closure_1_3(defaultCurrentScope, tmp(12853).getDefaultIsolationScope());
        const tmpResult2 = tmp(12853);
      }
      sentryCarrier.stack = stack;
