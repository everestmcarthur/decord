// Module ID: 12966
// Function ID: 12967
// Dependencies: [32, 12959, 12965, 12954, 12967, 12937, 12949, 12950, 12951, 12944, 12942, 12960, 12932, 12964, 12968, 12970, 12961, 12971, 12973, 12947]
...
function createChildOrRootSpan(forceTransaction) {
  ...
  if (obj.hasTracingEnabled()) {
    const isolationScope = tmp(12959).getIsolationScope();
    if (parentSpan) {
      if (!forceTransaction.forceTransaction) {
        ...
        if (!tmp9) {
          spanIsSampledResult = tmp(12937).spanIsSampled(parentSpan);
        ...
        if (spanIsSampledResult) {
          ...
          let sentrySpan = new tmp(12973).SentrySpan(obj2);
        } else {
          const obj3 = { traceId };
          sentrySpan = new tmp(12965).SentryNonRecordingSpan(obj3);
        }
        const spanContextResult = parentSpan.spanContext();
        tmp(12937).addChildSpanToSpan(parentSpan, sentrySpan);
        const tmpResult12 = tmp(12937);
        ...
        tmp(12937).addChildSpanToSpan(parentSpan, sentrySpan);
        const tmpResult14 = tmp(12937);
      }
      tmp(12970).logSpanStart(sentrySpan);
      const tmpResult15 = tmp(12970);
      const result = tmp(12961).setCapturedScopesOnSpan(sentrySpan, scope, isolationScope);
      return sentrySpan;
    }
    if (parentSpan) {
      const dynamicSamplingContextFromSpan = tmp(12968).getDynamicSamplingContextFromSpan(parentSpan);
      const tmpResult17 = tmp(12968);
      ...
      const tmpResult18 = tmp(12937);
      const merged1 = Object.assign(spanArguments);
      const tmp42 = _startRootSpan(obj4, scope, tmp(12937).spanIsSampled(parentSpan));
      const spanIsSampledResult1 = tmp(12937).spanIsSampled(parentSpan);
      tmp(12968).freezeDscOnSpan(tmp42, dynamicSamplingContextFromSpan);
      sentrySpan = tmp42;
      const tmpResult19 = tmp(12968);
    } else {
      const obj5 = {};
      ...
      sentrySpan = tmp33;
      if (dsc) {
        tmp(12968).freezeDscOnSpan(tmp33, dsc);
        sentrySpan = tmp33;
        const tmpResult20 = tmp(12968);
      }
    }
    const tmpResult = tmp(12959);
  } else {
    const sentryNonRecordingSpan = new tmp(12965).SentryNonRecordingSpan();
...
function _startRootSpan(name, arg1, parentSampled) {
  ...
  if (arg1.getScopeData().sdkProcessingMetadata[__SENTRY_SUPPRESS_TRACING__]) {
    ...
    sampleSpanResult = tmp(12971).sampleSpan(options, obj2);
    const tmpResult = tmp(12971);
  }
  ...
  obj4.sampled = tmp6;
  const sentrySpan = new tmp(12973).SentrySpan(obj4);
  if (undefined !== tmp7) {
    const attr = sentrySpan.setAttribute(tmp(12947).SEMANTIC_ATTRIBUTE_SENTRY_SAMPLE_RATE, tmp7);
  }
  if (client) {
    client.emit("spanStart", sentrySpan);
  }
...
export const continueTrace = (arg0, arg1) => {
  _require = arg1;
  const mainCarrier = require("module_12950").getMainCarrier();
  const obj = require("module_12950");
  const tmp = _require;
  const asyncContextStrategy = require("module_12951").getAsyncContextStrategy(mainCarrier);
  if (asyncContextStrategy.continueTrace) {
    ...
    return tmp(12959).withScope((setPropagationContext) => {
      const result = setPropagationContext.setPropagationContext(_mod12944.propagationContextFromHeaders(dependencyMap, _slicedToArray));
      return closure_0();
    });
  }
  const obj2 = require("module_12951");
};
export const startInactiveSpan = function startInactiveSpan(experimental) {
  _require = experimental;
  let mainCarrier = require("module_12950").getMainCarrier();
  let obj = require("module_12950");
  const tmp3 = _require;
  const tmp4 = obj4;
  let asyncContextStrategy = require("module_12951").getAsyncContextStrategy(mainCarrier);
  if (asyncContextStrategy.startInactiveSpan) {
    ...
    return fn(() => {
      ...
      if (experimental.onlyIfParent) {
        if (!tmp5) {
          let sentryNonRecordingSpan = new tmp(12965).SentryNonRecordingSpan();
        }
        return sentryNonRecordingSpan;
      }
...
export const startNewTrace = function startNewTrace(arg0) {
  _require = arg0;
  return require("module_12959").withScope((setPropagationContext) => {
  ...
  if (asyncContextStrategy.startSpan) {
    ...
    return tmp3(12959).withScope(experimental.scope, () => {
      ...
      return undefined !== createChildOrRootSpan ? ((arg0) => {
        ...
        const obj = sentryNonRecordingSpan(12959);
        ...
        if (sentryNonRecordingSpan.onlyIfParent) {
          ...
          return tmp(12967).handleCallbackErrors(() => dependencyMap(sentryNonRecordingSpan), () => {
    ...
    return tmp3(12959).withScope(experimental.scope, () => {
      ...
      return undefined !== createChildOrRootSpan ? ((arg0) => {
        ...
        if (sentryNonRecordingSpan.onlyIfParent) {
          ...
          tmp(12954)._setSpanForScope(currentScope, sentryNonRecordingSpan);
          const tmpResult5 = tmp(12954);
          return tmp(12967).handleCallbackErrors(() => dependencyMap(sentryNonRecordingSpan, finishAndSetSpan), () => {
            const status = sentryNonRecordingSpan(dependencyMap[5]).spanToJSON(sentryNonRecordingSpan).status;
            const isRecordingResult = sentryNonRecordingSpan.isRecording();
            let tmp4 = !isRecordingResult;
            if (isRecordingResult) {
