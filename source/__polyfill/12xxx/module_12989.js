// Module ID: 12989
// Function ID: 12990
// Dependencies: [12951, 12954, 12967, 12990, 12991, 12977, 12949, 12992, 12985, 12961]
...
function applyClientOptions(environment, environment2) {
  ...
  if (environment.message) {
    environment.message = _mod12949.truncate(environment.message, num);
  }
  value = iter;
  ...
  if (value) {
    iter.value = _mod12949.truncate(iter.value, num);
  }
  ...
  if (url) {
    request.url = _mod12949.truncate(request.url, num);
  }
}
function applyDebugIds(exception, arg1) {
  const filenameToDebugIdMap = _mod12992.getFilenameToDebugIdMap(arg1);
  try {
