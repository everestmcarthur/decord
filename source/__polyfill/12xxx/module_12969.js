// Module ID: 12969
// Function ID: 12970
// Dependencies: [12925, 12926, 12970, 12973, 12962, 12931]
...
export const createEventEnvelope = function createEventEnvelope(type, arg1, sdk, arg3) {
  ...
  const eventEnvelopeHeaders = _mod12970.createEventEnvelopeHeaders(type, sdkMetadataForEnvelopeHeader, arg3, arg1);
  delete tmp[tmp2];
  const items2 = [{ type: str }, type];
  const tmp3Result = _mod12970;
  const items3 = [items2];
  return _mod12970.createEnvelope(eventEnvelopeHeaders, items3);
};
export const createSessionEnvelope = function createSessionEnvelope(toJSON, arg1, arg2, arg3) {
  const sdkMetadataForEnvelopeHeader = _mod12970.getSdkMetadataForEnvelopeHeader(arg2);
  ...
  let tmp6 = arg3 && arg1;
  if (tmp6) {
    const obj4 = { dsn: tmp(12973).dsnToString(arg1) };
    tmp6 = obj4;
    const tmpResult = tmp(12973);
  }
  ...
  const date = new Date();
  const items2 = [items1];
  return _mod12970.createEnvelope(obj2, items2);
};
export const createSpanEnvelope = function createSpanEnvelope(arg0, getDsn) {
  const dynamicSamplingContextFromSpan = beforeSendSpan(12962).getDynamicSamplingContextFromSpan(arg0[0]);
  let dsn = getDsn;
  if (getDsn) {
    dsn = getDsn.getDsn();
  }
  ...
  const obj = beforeSendSpan(12962);
  ...
  let tmp9 = tunnel && dsn;
  if (tmp9) {
    const obj4 = { dsn: tmp2(12973).dsnToString(dsn) };
    tmp9 = obj4;
