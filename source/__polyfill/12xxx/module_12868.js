// Module ID: 12868
// Function ID: 12869
// Dependencies: [41, 42, 12837, 12841, 12842, 12832, 12865, 12833, 12843, 12869, 12855, 12827, 12854, 12870, 12856, 12863]
...
let items = [
  ...
  },
  {
    ...
    value: function updateStartTime(arg0) {
      this._startTime = SentrySpan(12832).spanTimeInputToSeconds(arg0);
    }
  },
  {
    key: "setStatus",
  ...
  },
  {
    ...
    value: function updateName(_name) {
      this._name = _name;
      const attr = this.setAttribute(SentrySpan(12842).SEMANTIC_ATTRIBUTE_SENTRY_SOURCE, "custom");
      return this;
    }
  },
  {
    ...
    value: function end(arg0) {
      const self = this;
      if (!this._endTime) {
        self._endTime = SentrySpan(12832).spanTimeInputToSeconds(arg0);
        const obj = SentrySpan(12832);
        SentrySpan(12865).logSpanEnd(self);
        self._onSpanEnded();
        const obj2 = SentrySpan(12865);
      }
    }
  },
  {
    ...
    value: function getSpanJSON() {
      const self = this;
      const obj6 = { data: this._attributes, description: this._name, op: this._attributes[SentrySpan(undefined, 12842).SEMANTIC_ATTRIBUTE_SENTRY_OP], parent_span_id: this._parentSpanId, span_id: this._spanId, start_timestamp: this._startTime, status: null, timestamp: null, trace_id: null, origin: null, _metrics_summary: null, profile_id: null, exclusive_time: null, measurements: null, is_segment: null, segment_id: null };
      const obj = SentrySpan(12833);
      obj6.status = SentrySpan(12832).getStatusMessage(this._status);
      ({ _endTime: obj2.timestamp, _traceId: obj2.trace_id, _attributes } = this);
      obj6.origin = _attributes[SentrySpan(undefined, 12842).SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN];
      const obj3 = SentrySpan(12832);
      obj6._metrics_summary = SentrySpan(12843).getMetricSummaryJsonForSpan(this);
      obj6.profile_id = this._attributes[SentrySpan(undefined, 12842).SEMANTIC_ATTRIBUTE_PROFILE_ID];
      obj6.exclusive_time = this._attributes[SentrySpan(undefined, 12842).SEMANTIC_ATTRIBUTE_EXCLUSIVE_TIME];
      const obj4 = SentrySpan(12843);
      obj6.measurements = SentrySpan(12869).timedEventsToMeasurements(this._events);
      let _isStandaloneSpan = this._isStandaloneSpan;
      if (_isStandaloneSpan) {
        _isStandaloneSpan = tmp(12832).getRootSpan(self) === self;
        const tmpResult = tmp(12832);
      }
      obj6.is_segment = _isStandaloneSpan;
      let spanId;
      if (self._isStandaloneSpan) {
        const rootSpan = tmp(12832).getRootSpan(self);
  ...
  },
  {
    ...
    value: function addEvent(name, num, arg2) {
      if (SentrySpan(12855).DEBUG_BUILD) {
      ...
      if (!isArray) {
        ...
        if (!arg2) {
          timestampInSecondsResult = tmp(12841).timestampInSeconds();
      ...
      const obj2 = { name, time: SentrySpan(12832).spanTimeInputToSeconds(tmp7), attributes: obj };
  ...
  },
  {
    ...
    value: function _onSpanEnded() {
      const self = this;
      const client = SentrySpan(12854).getClient();
      if (client) {
      ...
      if (self._isStandaloneSpan) {
        if (self._isStandaloneSpan) {
          if (self._sampled) {
            const items = [self];
            const spanEnvelope = tmp(12870).createSpanEnvelope(items, client);
            const tmpResult = tmp(12870);
            const client1 = tmp(12854).getClient();
            if (client1) {
              if (spanEnvelope[1]) {
                if (0 !== arr2.length) {
          ...
          if (result) {
            let scope = tmp(12856).getCapturedScopesOnSpan(self).scope;
            if (!scope) {
              scope = tmp(12854).getCurrentScope();
              const tmpResult7 = tmp(12854);
            }
            scope.captureEvent(result);
            const tmpResult6 = tmp(12856);
          }
        }
      } else {
  ...
  },
  {
    ...
    value: function _convertSpanToTransaction() {
      const self = this;
      const spanToJSONResult = self(12832).spanToJSON(this);
      if (tmp6) {
        if (!self._name) {
          if (tmp3(12855).DEBUG_BUILD) {
            const logger = tmp3(12827).logger;
            logger.warn("Transaction has no name, falling back to `<unlabeled transaction>`.");
          }
          self._name = "<unlabeled transaction>";
        }
        const capturedScopesOnSpan = tmp3(12856).getCapturedScopesOnSpan(self);
        const scope = capturedScopesOnSpan.scope;
        let currentScope = scope;
        if (!scope) {
          currentScope = tmp3(12854).getCurrentScope();
          const tmp3Result9 = tmp3(12854);
        }
        let client = currentScope.getClient();
        if (!client) {
          client = tmp3(12854).getClient();
          const tmp3Result10 = tmp3(12854);
        }
        if (true !== self._sampled) {
          if (tmp3(12855).DEBUG_BUILD) {
            const logger3 = tmp3(12827).logger;
          ...
          const spanDescendants = tmp3(12832).getSpanDescendants(self);
          ...
          const mapped = found.map((item) => self(12832).spanToJSON(item));
          const found1 = mapped.filter(isFullFinishedSpan);
          const tmp24 = self._attributes[tmp3(undefined, 12842).SEMANTIC_ATTRIBUTE_SENTRY_SOURCE];
          const _attributes = self._attributes;
          let SEMANTIC_ATTRIBUTE_SENTRY_CUSTOM_SPAN_NAME = tmp3(12842).SEMANTIC_ATTRIBUTE_SENTRY_CUSTOM_SPAN_NAME;
          delete tmp2[tmp];
          const item = found1.forEach((data) => {
            if (data.data) {
              data = data.data;
              const SEMANTIC_ATTRIBUTE_SENTRY_CUSTOM_SPAN_NAME = self(12842).SEMANTIC_ATTRIBUTE_SENTRY_CUSTOM_SPAN_NAME;
              delete tmp2[tmp];
            }
          });
          ...
          const tmp3Result11 = tmp3(12832);
          obj3.trace = tmp3(12832).spanToTransactionTraceContext(self);
          obj2.contexts = obj3;
          let substr = found1;
          ...
          const tmp3Result12 = tmp3(12832);
          const obj5 = { dynamicSamplingContext: null };
          const tmp3Result13 = tmp3(12833);
          obj5.dynamicSamplingContext = tmp3(12863).getDynamicSamplingContextFromSpan(self);
          const merged = Object.assign(tmp3Result13.dropUndefinedKeys(obj5));
          obj2.sdkProcessingMetadata = obj4;
          const tmp3Result14 = tmp3(12863);
          obj2._metrics_summary = tmp3(12843).getMetricSummaryJsonForSpan(self);
          let tmp12 = tmp24;
          if (tmp24) {
          ...
          const tmp3Result15 = tmp3(12843);
          const result = tmp3(12869).timedEventsToMeasurements(self._events);
          let length = result;
          if (result) {
          ...
          if (length) {
            if (tmp3(12855).DEBUG_BUILD) {
              const logger2 = tmp3(12827).logger;
              const _JSON = JSON;
        ...
        const tmp3Result = tmp3(12856);
      }
      const obj = self(12832);
