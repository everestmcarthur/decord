// Module ID: 12876
// Function ID: 12877
// Dependencies: [12838, 12841, 12854, 12877, 12878, 12864, 12836, 12879, 12872, 12848]
...
function applyClientOptions(environment, environment2) {
  ...
  if (environment.message) {
    environment.message = _mod12836.truncate(environment.message, num);
  }
  value = iter;
  ...
  if (value) {
    iter.value = _mod12836.truncate(iter.value, num);
  }
  ...
  if (url) {
    request.url = _mod12836.truncate(request.url, num);
  }
}
function applyDebugIds(exception, arg1) {
  const filenameToDebugIdMap = _mod12879.getFilenameToDebugIdMap(arg1);
  try {
