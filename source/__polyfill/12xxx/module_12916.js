// Module ID: 12916
// Function ID: 12917
// Dependencies: [12878, 12881, 12894, 12917, 12918, 12904, 12876, 12919, 12912, 12888]
...
function applyClientOptions(environment, environment2) {
  ...
  if (environment.message) {
    environment.message = _mod12876.truncate(environment.message, num);
  }
  value = iter;
  ...
  if (value) {
    iter.value = _mod12876.truncate(iter.value, num);
  }
  ...
  if (url) {
    request.url = _mod12876.truncate(request.url, num);
  }
}
function applyDebugIds(exception, arg1) {
  const filenameToDebugIdMap = _mod12919.getFilenameToDebugIdMap(arg1);
  try {
