// Module ID: 12910
// Function ID: 12911
// Dependencies: [12866, 12867, 12911, 12914, 12903, 12872]
...
export const createEventEnvelope = function createEventEnvelope(type, arg1, sdk, arg3) {
  ...
  const eventEnvelopeHeaders = _mod12911.createEventEnvelopeHeaders(type, sdkMetadataForEnvelopeHeader, arg3, arg1);
  delete tmp[tmp2];
  const items2 = [{ type: str }, type];
  const tmp3Result = _mod12911;
  const items3 = [items2];
  return _mod12911.createEnvelope(eventEnvelopeHeaders, items3);
};
export const createSessionEnvelope = function createSessionEnvelope(toJSON, arg1, arg2, arg3) {
  const sdkMetadataForEnvelopeHeader = _mod12911.getSdkMetadataForEnvelopeHeader(arg2);
  ...
  let tmp6 = arg3 && arg1;
  if (tmp6) {
    const obj4 = { dsn: tmp(12914).dsnToString(arg1) };
    tmp6 = obj4;
  ...
  const date = new Date();
  const items2 = [items1];
  return _mod12911.createEnvelope(obj2, items2);
};
export const createSpanEnvelope = function createSpanEnvelope(arg0, getDsn) {
  const dynamicSamplingContextFromSpan = beforeSendSpan(12903).getDynamicSamplingContextFromSpan(arg0[0]);
  let dsn = getDsn;
  if (getDsn) {
    dsn = getDsn.getDsn();
  }
  ...
  const obj = beforeSendSpan(12903);
  ...
  let tmp9 = tunnel && dsn;
  if (tmp9) {
    const obj4 = { dsn: tmp2(12914).dsnToString(dsn) };
    tmp9 = obj4;
