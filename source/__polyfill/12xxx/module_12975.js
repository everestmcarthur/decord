// Module ID: 12975
// Function ID: 12976
// Dependencies: [12931, 12932, 12976, 12979, 12968, 12937]
...
export const createEventEnvelope = function createEventEnvelope(type, arg1, sdk, arg3) {
  const sdkMetadataForEnvelopeHeader = _mod12976.getSdkMetadataForEnvelopeHeader(sdk);
  let str = "event";
  if (type.type) {
    str = "event";
  ...
  const eventEnvelopeHeaders = _mod12976.createEventEnvelopeHeaders(type, sdkMetadataForEnvelopeHeader, arg3, arg1);
  delete tmp[tmp2];
  const items2 = [{ type: str }, type];
  const tmp3Result = _mod12976;
  const items3 = [items2];
  return _mod12976.createEnvelope(eventEnvelopeHeaders, items3);
};
export const createSessionEnvelope = function createSessionEnvelope(toJSON, arg1, arg2, arg3) {
  const sdkMetadataForEnvelopeHeader = _mod12976.getSdkMetadataForEnvelopeHeader(arg2);
  ...
  let tmp6 = arg3 && arg1;
  if (tmp6) {
    const obj4 = { dsn: tmp(12979).dsnToString(arg1) };
    tmp6 = obj4;
  ...
  const date = new Date();
  const items2 = [items1];
  return _mod12976.createEnvelope(obj2, items2);
};
export const createSpanEnvelope = function createSpanEnvelope(arg0, getDsn) {
  const dynamicSamplingContextFromSpan = beforeSendSpan(12968).getDynamicSamplingContextFromSpan(arg0[0]);
  let dsn = getDsn;
  if (getDsn) {
    dsn = getDsn.getDsn();
  }
  ...
  const obj = beforeSendSpan(12968);
  ...
  let tmp9 = tunnel && dsn;
  if (tmp9) {
    const obj4 = { dsn: tmp2(12979).dsnToString(dsn) };
    tmp9 = obj4;
    const tmp2Result = tmp2(12979);
  }
