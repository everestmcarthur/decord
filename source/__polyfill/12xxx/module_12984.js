// Module ID: 12984
// Function ID: 12985
// Dependencies: [12954, 12985, 12975, 12953, 12961, 12963, 12980]
...
const _mod12985 = tmp3(12985);
require = arg1;
const dependencyMap = arg6;
function getDynamicSamplingContextFromSpan(spanContext) {
  const client = _mod12975.getClient();
  if (client) {
    const rootSpan = tmp(12953).getRootSpan(spanContext);
    if (rootSpan[_frozenDsc]) {
      ...
      let result = value;
      if (value) {
        result = tmp(12961).baggageHeaderToDynamicSamplingContext(value);
      ...
      if (result) {
        ...
        if (!DEFAULT_ENVIRONMENT) {
          DEFAULT_ENVIRONMENT = tmp(12985).DEFAULT_ENVIRONMENT;
        }
        ...
        const dropUndefinedKeysResult = tmp(12954).dropUndefinedKeys(obj2);
        client.emit("createDsc", dropUndefinedKeysResult);
        const tmpResult7 = tmp(12954);
        const spanToJSONResult = tmp(12953).spanToJSON(rootSpan);
        const tmp13 = spanToJSONResult.data || {};
        const tmp14 = tmp13[tmp(undefined, 12963).SEMANTIC_ATTRIBUTE_SENTRY_SAMPLE_RATE];
        if (null != tmp14) {
        ...
        tmp17 = "url" !== tmp13[tmp(undefined, 12963).SEMANTIC_ATTRIBUTE_SENTRY_SOURCE] && description;
        if (tmpResult9.hasTracingEnabled()) {
          const _String = String;
          dropUndefinedKeysResult.sampled = String(tmp(12953).spanIsSampled(rootSpan));
...
export const freezeDscOnSpan = function freezeDscOnSpan(arg0, arg1) {
  const result = _mod12954.addNonEnumerableProperty(arg0, _frozenDsc, arg1);
};
...
export const getDynamicSamplingContextFromScope = function getDynamicSamplingContextFromScope(getOptions, getPropagationContext) {
  ...
  if (!dsc) {
    ...
    let DEFAULT_ENVIRONMENT = options.environment;
    if (!DEFAULT_ENVIRONMENT) {
      DEFAULT_ENVIRONMENT = tmp5(12985).DEFAULT_ENVIRONMENT;
    }
