// Module ID: 12905
// Function ID: 12906
// Dependencies: [12895, 12872, 12867]
...
export const logSpanEnd = function logSpanEnd(spanContext) {
  if (_mod12895.DEBUG_BUILD) {
    const spanToJSONResult = tmp(12872).spanToJSON(spanContext);
...
export const logSpanStart = function logSpanStart(spanContext) {
  if (_mod12895.DEBUG_BUILD) {
    ...
    const spanIsSampledResult = tmp(12872).spanIsSampled(spanContext);
    ...
    if (rootSpan !== spanContext) {
      const tmpResult6 = tmp(12872);
      ({ op: op2, description: description2 } = tmp(12872).spanToJSON(rootSpan));
      ...
      const spanToJSONResult1 = tmp(12872).spanToJSON(rootSpan);
    }
    const logger = tmp(12867).logger;
    const _HermesInternal9 = HermesInternal;
    logger.log("" + combined + "\n  " + items.join("\n  "));
    const tmpResult5 = tmp(12872);
  }
};
