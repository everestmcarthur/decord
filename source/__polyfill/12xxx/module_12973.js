// Module ID: 12973
// Function ID: 12974
// Dependencies: [41, 42, 12942, 12946, 12947, 12937, 12970, 12938, 12948, 12974, 12960, 12932, 12959, 12975, 12961, 12968]
...
let items = [
  ...
  },
  {
    ...
    value: function updateStartTime(arg0) {
      this._startTime = SentrySpan(12937).spanTimeInputToSeconds(arg0);
    }
  },
  {
    key: "setStatus",
  ...
  },
  {
    ...
    value: function updateName(_name) {
      this._name = _name;
      const attr = this.setAttribute(SentrySpan(12947).SEMANTIC_ATTRIBUTE_SENTRY_SOURCE, "custom");
      return this;
    }
  },
  {
    ...
    value: function end(arg0) {
      const self = this;
      if (!this._endTime) {
        self._endTime = SentrySpan(12937).spanTimeInputToSeconds(arg0);
        const obj = SentrySpan(12937);
        SentrySpan(12970).logSpanEnd(self);
        self._onSpanEnded();
        const obj2 = SentrySpan(12970);
      }
    }
  },
  {
    ...
    value: function getSpanJSON() {
      const self = this;
      const obj6 = { data: this._attributes, description: this._name, op: this._attributes[SentrySpan(undefined, 12947).SEMANTIC_ATTRIBUTE_SENTRY_OP], parent_span_id: this._parentSpanId, span_id: this._spanId, start_timestamp: this._startTime, status: null, timestamp: null, trace_id: null, origin: null, _metrics_summary: null, profile_id: null, exclusive_time: null, measurements: null, is_segment: null, segment_id: null };
      const obj = SentrySpan(12938);
      obj6.status = SentrySpan(12937).getStatusMessage(this._status);
      ({ _endTime: obj2.timestamp, _traceId: obj2.trace_id, _attributes } = this);
      obj6.origin = _attributes[SentrySpan(undefined, 12947).SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN];
      const obj3 = SentrySpan(12937);
      obj6._metrics_summary = SentrySpan(12948).getMetricSummaryJsonForSpan(this);
      obj6.profile_id = this._attributes[SentrySpan(undefined, 12947).SEMANTIC_ATTRIBUTE_PROFILE_ID];
      obj6.exclusive_time = this._attributes[SentrySpan(undefined, 12947).SEMANTIC_ATTRIBUTE_EXCLUSIVE_TIME];
      const obj4 = SentrySpan(12948);
      obj6.measurements = SentrySpan(12974).timedEventsToMeasurements(this._events);
      let _isStandaloneSpan = this._isStandaloneSpan;
      if (_isStandaloneSpan) {
        _isStandaloneSpan = tmp(12937).getRootSpan(self) === self;
        const tmpResult = tmp(12937);
      }
      obj6.is_segment = _isStandaloneSpan;
      let spanId;
      if (self._isStandaloneSpan) {
        const rootSpan = tmp(12937).getRootSpan(self);
  ...
  },
  {
    ...
    value: function addEvent(name, num, arg2) {
      if (SentrySpan(12960).DEBUG_BUILD) {
      ...
      if (!isArray) {
        ...
        if (!arg2) {
          timestampInSecondsResult = tmp(12946).timestampInSeconds();
      ...
      const obj2 = { name, time: SentrySpan(12937).spanTimeInputToSeconds(tmp7), attributes: obj };
  ...
  },
  {
    ...
    value: function _onSpanEnded() {
      const self = this;
      const client = SentrySpan(12959).getClient();
      if (client) {
      ...
      if (self._isStandaloneSpan) {
        if (self._isStandaloneSpan) {
          if (self._sampled) {
            const items = [self];
            const spanEnvelope = tmp(12975).createSpanEnvelope(items, client);
            const tmpResult = tmp(12975);
            const client1 = tmp(12959).getClient();
            if (client1) {
              if (spanEnvelope[1]) {
                if (0 !== arr2.length) {
          ...
          if (result) {
            let scope = tmp(12961).getCapturedScopesOnSpan(self).scope;
            if (!scope) {
              scope = tmp(12959).getCurrentScope();
              const tmpResult7 = tmp(12959);
            }
            scope.captureEvent(result);
            const tmpResult6 = tmp(12961);
          }
        }
      } else {
  ...
  },
  {
    ...
    value: function _convertSpanToTransaction() {
      const self = this;
      const spanToJSONResult = self(12937).spanToJSON(this);
      if (tmp6) {
        if (!self._name) {
          if (tmp3(12960).DEBUG_BUILD) {
            const logger = tmp3(12932).logger;
            logger.warn("Transaction has no name, falling back to `<unlabeled transaction>`.");
          }
          self._name = "<unlabeled transaction>";
        }
        const capturedScopesOnSpan = tmp3(12961).getCapturedScopesOnSpan(self);
        const scope = capturedScopesOnSpan.scope;
        let currentScope = scope;
        if (!scope) {
          currentScope = tmp3(12959).getCurrentScope();
          const tmp3Result9 = tmp3(12959);
        }
        let client = currentScope.getClient();
        if (!client) {
          client = tmp3(12959).getClient();
          const tmp3Result10 = tmp3(12959);
        }
        if (true !== self._sampled) {
          if (tmp3(12960).DEBUG_BUILD) {
            const logger3 = tmp3(12932).logger;
          ...
          const spanDescendants = tmp3(12937).getSpanDescendants(self);
          ...
          const mapped = found.map((item) => self(12937).spanToJSON(item));
          const found1 = mapped.filter(isFullFinishedSpan);
          const tmp24 = self._attributes[tmp3(undefined, 12947).SEMANTIC_ATTRIBUTE_SENTRY_SOURCE];
          const _attributes = self._attributes;
          let SEMANTIC_ATTRIBUTE_SENTRY_CUSTOM_SPAN_NAME = tmp3(12947).SEMANTIC_ATTRIBUTE_SENTRY_CUSTOM_SPAN_NAME;
          delete tmp2[tmp];
          const item = found1.forEach((data) => {
            if (data.data) {
              data = data.data;
              const SEMANTIC_ATTRIBUTE_SENTRY_CUSTOM_SPAN_NAME = self(12947).SEMANTIC_ATTRIBUTE_SENTRY_CUSTOM_SPAN_NAME;
              delete tmp2[tmp];
            }
          });
          ...
          const tmp3Result11 = tmp3(12937);
          obj3.trace = tmp3(12937).spanToTransactionTraceContext(self);
          obj2.contexts = obj3;
          let substr = found1;
          ...
          const tmp3Result12 = tmp3(12937);
          const obj5 = { dynamicSamplingContext: null };
          const tmp3Result13 = tmp3(12938);
          obj5.dynamicSamplingContext = tmp3(12968).getDynamicSamplingContextFromSpan(self);
          const merged = Object.assign(tmp3Result13.dropUndefinedKeys(obj5));
          obj2.sdkProcessingMetadata = obj4;
          const tmp3Result14 = tmp3(12968);
          obj2._metrics_summary = tmp3(12948).getMetricSummaryJsonForSpan(self);
          let tmp12 = tmp24;
          if (tmp24) {
          ...
          const tmp3Result15 = tmp3(12948);
          const result = tmp3(12974).timedEventsToMeasurements(self._events);
          let length = result;
          if (result) {
          ...
          if (length) {
            if (tmp3(12960).DEBUG_BUILD) {
              const logger2 = tmp3(12932).logger;
              const _JSON = JSON;
        ...
        const tmp3Result = tmp3(12961);
      }
      const obj = self(12937);
