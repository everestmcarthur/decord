...
// Function ID: 4602
// Name: RTCConnection
// Dependencies: [5, 4602, 4612, 9183, 9115, 502, 1957, 1979, 1908, 4597, 10070, 4624, 1371, 1074, 4621, 13828, 4599, 559, 4603, 4632, 1232, 3, 13829, 1256, 13831, 13833, 9641, 10071, 1461, 1115, 4195, 1365, 573, 1907, 1384, 4628, 14, 5434, 13834, 13835, 9637, 1242, 4568, 7686, 13836, 13838, 4231, 4712, 13839, 13840, 4629, 13841, 13842, 13844, 7762, 13845, 9142, 7488, 13846, 13847, 9553, 13849, 4918, 1114, 551, 1241, 2]

// Module 4601 (RTCConnection)
...
import RTCConnectionEvent from "RTCConnectionEvent" /* 4628 */;
...
import ServerLadderExperiment2 from "ServerLadderExperiment" /* 13839 */;
import AV1BitrateTuningExperiment from "AV1BitrateTuningExperiment" /* 13840 */;
import NativeMuteManagerDefault from "NativeMuteManager" /* 13841 */;
import VoiceQuality from "VoiceQuality" /* 13842 */;
import SystemResponsivenessDefault from "SystemResponsiveness" /* 13844 */;
import VoiceDurationDefault from "VoiceDuration" /* 13845 */;
import VideoQuality from "VideoQuality" /* 13846 */;
import VideoHealthManager from "VideoHealthManager" /* 13847 */;
import BandwidthEstimationExperimentDefault from "BandwidthEstimationExperiment" /* 13849 */;
import asyncGeneratorStep from "asyncGeneratorStep" /* 5 */;
import DeviceFrecencyStore from "DeviceFrecencyStore" /* 4602 */;
...
const VideoStabilizationExperimentDefault = tmp3(13835);
require = fn;
...
prototype["destroy"] = function destroy() {
  ...
  const WindowVisibilityVideoManager = tmp5(9641).WindowVisibilityVideoManager;
  ...
  const WindowVisibilityVideoManager2 = tmp5(9641).WindowVisibilityVideoManager;
...
prototype["_chooseExperiments"] = function _chooseExperiments() {
  ...
  if (enabled) {
    const BrowserTransceiverPaddingRemovalExperiment = tmp8(13834).BrowserTransceiverPaddingRemovalExperiment;
  ...
  if (isAndroidResult) {
    isAndroidResult = tmp8(9637).isSurfaceDirectRendererExperimentEnabled();
    const tmp8Result4 = tmp8(9637);
  }
  if (isAndroidResult) {
    items.push(tmp8(9637).ANDROID_SURFACE_DIRECT_RENDERER_EXPERIMENT);
  }
...
prototype["_connectMediaEngineWithEndpoint"] = function _connectMediaEngineWithEndpoint(_sfuEndpoint, _readyData) {
  ...
  connectResult.on(require("BaseConnectionEvent").BaseConnectionEvent.VideoCodecError, (codecStandard) => {
    ...
    if ("encode" === codecStandard.mode) {
      const obj4 = { type: tmp(9142).AVError.VIDEO_ENCODE_ERROR };
      const merged = Object.assign(obj);
      obj4.videoEncoder = codecStandard.implName;
      let obj5 = obj4;
    } else {
      obj5 = { type: tmp(9142).AVError.VIDEO_DECODE_ERROR };
  ...
  if (null != this._connection) {
    if (null == self._videoQuality) {
      ...
      if (defaultConfig.featureEnabled) {
        const videoHealthManager = new tmp10(13847).VideoHealthManager(windowLength, allowedPoorFpsRatio, fpsThreshold, backoffTimeSec);
        self._videoHealthManager = videoHealthManager;
        if (null != self._localMediaSinkWantsManager) {
          self._localMediaSinkWantsManager.videoHealthManager = self._videoHealthManager;
        }
        const _videoQuality = self._videoQuality;
        _videoQuality.on(tmp10(13846).VideoQualityEvent.FpsUpdate, (arg0, arg1, arg2) => {
          const _localMediaSinkWantsManager = self._localMediaSinkWantsManager;
          let result;
          if (_localMediaSinkWantsManager != null) {
    ...
    _connection.processMLSWelcome(arg0, arg1, (arg0, protocolVersion, arg2) => {
      if (_connection === self._connection) {
        ...
        if (arg0) {
          ...
          obj._mlsSessionResetStartTime = tmp10(4603).now();
