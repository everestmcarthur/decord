...
// Dependencies: [109, 19, 21, 12, 7385, 558, 1232, 2]
// Exports: default, useSubscribeGuildMembers

// Module 7384 (subscribeGuildMembers)
      ...
      if (!discord_common_shallowEqualDefault(this.props, arg0)) {
        ...
        if (!isEqualResult) {
          if (null != self._subscriptions) {
            const item = tmp(12).forEach(self._subscriptions, (userIds, guildId) => displayName(7385).unsubscribeMembers(guildId, userIds));
            const tmpResult3 = tmp(12);
          }
          const item1 = tmp(12).forEach(tmp4, (userIds, guildId) => displayName(7385).subscribeMembers(guildId, userIds));
