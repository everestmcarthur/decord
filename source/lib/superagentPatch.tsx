...
// Dependencies: [5, 4523, 1074, 1273, 17363, 1365, 1433, 573, 1272, 502, 1347, 2025, 1371, 1242, 1115, 17364, 17, 1117, 17365, 17366, 7, 1232, 17369, 1896, 11373, 15731, 9137, 5574, 17375, 13886, 17376, 1461, 2]

// Module 17362 (superagentPatch)
...
import getTimeZoneDefault from "getTimeZone" /* 17365 */;
import trackHttpRequestDefault from "trackHttpRequest" /* 17366 */;
...
const idGenerator = new fn(17363).IdGenerator();
...
HTTPUtils.setRequestPatch({
  prepareRequest(promise) {
    ...
    if ("/" === promise.url[0]) {
      ...
      const result1 = tmp(17364).updateDynamicSuperProperties();
  ...
  interceptResponse(statusCode, arg1, arg2) {
    ...
    if (400 === statusCode.statusCode) {
      ...
      if (captcha_key) {
        const items = [statusCode(1896)(17369, dependencyMap.paths), statusCode(1896)(11373, dependencyMap.paths)];
    ...
    if (obj.isLimitedAccessErrorCode(statusCode.statusCode, code1)) {
      tmp7(1896)(5574, tmp8.paths).then((result) => {
        result.default();
      });
      flag = false;
      const promise3 = tmp7(1896)(5574, tmp8.paths);
    } else {
      ...
      if (tmp7Result.isLimitedAccessErrorCode(statusCode.statusCode, code2)) {
        tmp7(1896)(13886, tmp8.paths).then((result) => {
        ...
        flag = false;
        const promise2 = tmp7(1896)(13886, tmp8.paths);
      } else {
        ...
        if (flag) {
          tmp7(1896)(17376, tmp8.paths).then((openRestrictedHoursModal) => {
            const result = openRestrictedHoursModal.openRestrictedHoursModal();
          });
          flag = false;
          const promise = tmp7(1896)(17376, tmp8.paths);
        }
      }
      tmp7Result = tmp7(17375);
    }
  }
});
HTTPUtils = fn(1272);
