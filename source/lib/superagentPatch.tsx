...
// Dependencies: [5, 4553, 1074, 1272, 17488, 1366, 1434, 573, 1271, 502, 1346, 2026, 1372, 1241, 1364, 17489, 1116, 17490, 17491, 7, 1231, 17494, 1897, 11423, 15784, 9188, 5608, 17500, 13937, 17501, 1462, 2]

// Module 17487 (superagentPatch)
...
import getTimeZoneDefault from "getTimeZone" /* 17490 */;
import trackHttpRequestDefault from "trackHttpRequest" /* 17491 */;
...
const idGenerator = new fn(17488).IdGenerator();
...
HTTPUtils.setRequestPatch({
  prepareRequest(promise) {
    ...
    if ("/" === promise.url[0]) {
      ...
      const result1 = tmp(17489).updateDynamicSuperProperties();
      ...
      const result6 = promise.set("X-Discord-Locale", closure_0(2026).default.locale);
  ...
  interceptResponse(statusCode, arg1, arg2) {
    ...
    if (400 === statusCode.statusCode) {
      ...
      if (captcha_key) {
        const items = [statusCode(1897)(17494, dependencyMap.paths), statusCode(1897)(11423, dependencyMap.paths)];
    ...
    if (401 === statusCode.statusCode) {
      ...
      if (code === AbortCodes.MFA_REQUIRED) {
        ...
        if (mfa) {
          const promise4 = statusCode(1897)(15784, dependencyMap.paths);
          statusCode(1897)(15784, dependencyMap.paths).then((openMFAModal) => {
            openMFAModal.openMFAModal(statusCode.body.mfa, closure_1, closure_2);
          }).catch(arg2);
          flag = true;
          const nextPromise2 = statusCode(1897)(15784, dependencyMap.paths).then((openMFAModal) => {
            openMFAModal.openMFAModal(statusCode.body.mfa, closure_1, closure_2);
          });
        }
      }
    ...
    if (obj.isLimitedAccessErrorCode(statusCode.statusCode, code1)) {
      tmp7(1897)(5608, tmp8.paths).then((result) => {
        result.default();
      });
      flag = false;
      const promise3 = tmp7(1897)(5608, tmp8.paths);
    } else {
      ...
      if (tmp7Result.isLimitedAccessErrorCode(statusCode.statusCode, code2)) {
        tmp7(1897)(13937, tmp8.paths).then((result) => {
        ...
        flag = false;
        const promise2 = tmp7(1897)(13937, tmp8.paths);
      } else {
        ...
        if (flag) {
          tmp7(1897)(17501, tmp8.paths).then((openRestrictedHoursModal) => {
            const result = openRestrictedHoursModal.openRestrictedHoursModal();
          });
          flag = false;
          const promise = tmp7(1897)(17501, tmp8.paths);
        }
      }
      tmp7Result = tmp7(17500);
    }
  }
});
HTTPUtils = fn(1271);
