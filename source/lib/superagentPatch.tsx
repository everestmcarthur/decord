...
// Dependencies: [5, 4637, 1074, 1272, 17610, 1366, 1434, 573, 1271, 502, 1346, 2026, 1372, 1241, 1364, 17611, 1116, 17612, 17613, 7, 1231, 17616, 1897, 11529, 15885, 9283, 5692, 17622, 14039, 17623, 1462, 2]

// Module 17609 (superagentPatch)
...
import getTimeZoneDefault from "getTimeZone" /* 17612 */;
import trackHttpRequestDefault from "trackHttpRequest" /* 17613 */;
...
HTTPUtils.setRequestPatch({
  ...
  interceptResponse(statusCode, arg1, arg2) {
    ...
    if (401 === statusCode.statusCode) {
      ...
      if (code === AbortCodes.MFA_REQUIRED) {
        ...
        if (mfa) {
          ...
          statusCode(1897)(15885, dependencyMap.paths).then((openMFAModal) => {
    ...
    if (obj.isLimitedAccessErrorCode(statusCode.statusCode, code1)) {
      tmp7(1897)(5692, tmp8.paths).then((result) => {
        result.default();
      });
      flag = false;
      const promise3 = tmp7(1897)(5692, tmp8.paths);
    } else {
      ...
      if (tmp7Result.isLimitedAccessErrorCode(statusCode.statusCode, code2)) {
        tmp7(1897)(14039, tmp8.paths).then((result) => {
        ...
        flag = false;
        const promise2 = tmp7(1897)(14039, tmp8.paths);
      } else {
        ...
        if (flag) {
          tmp7(1897)(17623, tmp8.paths).then((openRestrictedHoursModal) => {
            const result = openRestrictedHoursModal.openRestrictedHoursModal();
          });
          flag = false;
          const promise = tmp7(1897)(17623, tmp8.paths);
        }
      }
