...
// Dependencies: [5, 4555, 1074, 1272, 17432, 1366, 1434, 573, 1271, 502, 1346, 2026, 1372, 1241, 1364, 17433, 1116, 17434, 17435, 7, 1231, 17438, 1897, 11415, 15766, 9177, 5607, 17444, 13924, 17445, 1462, 2]
...
HTTPUtils.setRequestPatch({
  ...
  interceptResponse(statusCode, arg1, arg2) {
    ...
    if (obj.isLimitedAccessErrorCode(statusCode.statusCode, code1)) {
      tmp7(1897)(5607, tmp8.paths).then((result) => {
        result.default();
      });
      flag = false;
      const promise3 = tmp7(1897)(5607, tmp8.paths);
    } else {
      ...
      if (tmp7Result.isLimitedAccessErrorCode(statusCode.statusCode, code2)) {
        tmp7(1897)(13924, tmp8.paths).then((result) => {
        ...
        flag = false;
        const promise2 = tmp7(1897)(13924, tmp8.paths);
      } else {
        ...
        if (flag) {
          tmp7(1897)(17445, tmp8.paths).then((openRestrictedHoursModal) => {
            const result = openRestrictedHoursModal.openRestrictedHoursModal();
          });
          flag = false;
          const promise = tmp7(1897)(17445, tmp8.paths);
        }
      }
