...
// Dependencies: [5, 4555, 1074, 1272, 17529, 1366, 1434, 573, 1271, 502, 1346, 2026, 1372, 1241, 1364, 17530, 1116, 17531, 17532, 7, 1231, 17535, 1897, 11431, 15790, 9199, 5610, 17541, 13945, 17542, 1462, 2]

// Module 17528 (superagentPatch)
...
import getTimeZoneDefault from "getTimeZone" /* 17531 */;
import trackHttpRequestDefault from "trackHttpRequest" /* 17532 */;
...
const idGenerator = new fn(17529).IdGenerator();
...
HTTPUtils.setRequestPatch({
  prepareRequest(promise) {
    ...
    if ("/" === promise.url[0]) {
      ...
      const result1 = tmp(17530).updateDynamicSuperProperties();
  ...
  interceptResponse(statusCode, arg1, arg2) {
    ...
    if (400 === statusCode.statusCode) {
      ...
      if (captcha_key) {
        const items = [statusCode(1897)(17535, dependencyMap.paths), statusCode(1897)(11431, dependencyMap.paths)];
    ...
    if (401 === statusCode.statusCode) {
      ...
      if (code === AbortCodes.MFA_REQUIRED) {
        ...
        if (mfa) {
          const promise4 = statusCode(1897)(15790, dependencyMap.paths);
          statusCode(1897)(15790, dependencyMap.paths).then((openMFAModal) => {
            openMFAModal.openMFAModal(statusCode.body.mfa, closure_1, closure_2);
          }).catch(arg2);
          flag = true;
          const nextPromise2 = statusCode(1897)(15790, dependencyMap.paths).then((openMFAModal) => {
            openMFAModal.openMFAModal(statusCode.body.mfa, closure_1, closure_2);
          });
        }
      }
    ...
    if (obj.isLimitedAccessErrorCode(statusCode.statusCode, code1)) {
      tmp7(1897)(5610, tmp8.paths).then((result) => {
        result.default();
      });
      flag = false;
      const promise3 = tmp7(1897)(5610, tmp8.paths);
    } else {
      ...
      if (tmp7Result.isLimitedAccessErrorCode(statusCode.statusCode, code2)) {
        tmp7(1897)(13945, tmp8.paths).then((result) => {
        ...
        flag = false;
        const promise2 = tmp7(1897)(13945, tmp8.paths);
      } else {
        ...
        if (flag) {
          tmp7(1897)(17542, tmp8.paths).then((openRestrictedHoursModal) => {
            const result = openRestrictedHoursModal.openRestrictedHoursModal();
          });
          flag = false;
          const promise = tmp7(1897)(17542, tmp8.paths);
        }
      }
      tmp7Result = tmp7(17541);
    }
  }
});
HTTPUtils = fn(1271);
