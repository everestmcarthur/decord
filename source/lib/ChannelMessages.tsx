...
// Dependencies: [1074, 3, 4863, 12, 4570, 11, 5361, 5362, 5363, 5364, 2]
// Exports: flatMapChannelMessages

// Module 5360 (ChannelMessages)
import LoggerDefault from "Logger" /* 3 */;
import SnowflakeUtilsDefault from "SnowflakeUtils" /* 11 */;
import _modDef12 from "module_12" /* 12 */;
import Client from "Client" /* 4570 */;
import MessageRecordUtils from "MessageRecordUtils" /* 4863 */;
import SortedArrayUtilsAll from "SortedArrayUtils" /* 5362 */;
...
prototype2["mergeDelta"] = function mergeDelta(new_messages, modified_messages, deleted_message_ids) {
  ...
  return this.mutate((_before) => {
    ...
    const mapped = items.map((item) => set(4863).createMessageRecord(item));
    const combined = found.concat(mapped, items1.map((item) => set(4863).createMessageRecord(item)));
...
prototype2["receiveMessage"] = function receiveMessage(nonce) {
    ...
    if (id === id1) {
      if (null != nonce.nonce) {
        if (value.id === nonce.nonce) {
          const messageRecord = messageRecord1(4863).createMessageRecord(nonce);
  ...
  if (self.hasMoreAfter) {
    ...
    messageRecord1 = messageRecord1(4863).createMessageRecord(nonce);
    const lastResult = self.last();
    if (null != lastResult) {
...
prototype2["loadComplete"] = function loadComplete(newMessages) {
  ...
  if (flag) {
    if (null == jump) {
      ...
      if (jumpType == null) {
        jumpType = id(4570).JumpType.ANIMATED;
      }
...
prototype2["addCachedMessages"] = function addCachedMessages(messages, stale) {
  const self = this;
  const result = reversed(5364).requireSortedDescending(messages);
