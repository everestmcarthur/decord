...
// Dependencies: [1074, 3, 4861, 12, 4568, 11, 5359, 5360, 5361, 5362, 2]
// Exports: flatMapChannelMessages

// Module 5358 (ChannelMessages)
import LoggerDefault from "Logger" /* 3 */;
import SnowflakeUtilsDefault from "SnowflakeUtils" /* 11 */;
import _modDef12 from "module_12" /* 12 */;
import Client from "Client" /* 4568 */;
import MessageRecordUtils from "MessageRecordUtils" /* 4861 */;
import SortedArrayUtilsAll from "SortedArrayUtils" /* 5360 */;
...
prototype2["mergeDelta"] = function mergeDelta(new_messages, modified_messages, deleted_message_ids) {
  ...
  return this.mutate((_before) => {
    ...
    const mapped = items.map((item) => set(4861).createMessageRecord(item));
    const combined = found.concat(mapped, items1.map((item) => set(4861).createMessageRecord(item)));
...
prototype2["receiveMessage"] = function receiveMessage(nonce) {
    ...
    if (id === id1) {
      if (null != nonce.nonce) {
        if (value.id === nonce.nonce) {
          const messageRecord = messageRecord1(4861).createMessageRecord(nonce);
  ...
  if (self.hasMoreAfter) {
    ...
    messageRecord1 = messageRecord1(4861).createMessageRecord(nonce);
    const lastResult = self.last();
    if (null != lastResult) {
...
prototype2["loadComplete"] = function loadComplete(newMessages) {
  ...
  if (flag) {
    if (null == jump) {
      ...
      if (jumpType == null) {
        jumpType = id(4568).JumpType.ANIMATED;
      }
...
prototype2["addCachedMessages"] = function addCachedMessages(messages, stale) {
  const self = this;
  const result = reversed(5362).requireSortedDescending(messages);
