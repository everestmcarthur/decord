...
// Function ID: 5647
// Name: Spellchecker
// Dependencies: [5, 32, 2026, 3, 4258, 5647, 5648, 5649, 5650, 1370, 12, 1929, 2]
// Exports: install

// Module 5646 (Spellchecker)
import LoggerDefault from "Logger" /* 3 */;
import DOMUtils from "DOMUtils" /* 1929 */;
import fallbackLocalesDefault from "fallbackLocales" /* 5647 */;
import _mod5648 from "module_5648" /* 5648 */;
import asyncGeneratorStep from "asyncGeneratorStep" /* 5 */;
import _slicedToArray from "module_32" /* 32 */;
import LocaleStore from "LocaleStore" /* 2026 */;
import DiscordNative from "DiscordNative" /* 4258 */;
...
prototype["applyLanguages"] = function applyLanguages(locale) {
  ...
  const mapped = items.map((item) => {
    ...
    const parsed = _mod5648.parse(str2.replace(/[_-]/g, "-"));
    if (null != parsed) {
      if (null != parsed.langtag.language) {
        if (null != parsed.langtag.region) {
          const langtag = parsed.langtag;
  ...
  if (null != hasAttribute) {
    if (!obj.isElement(hasAttribute, globalThis.HTMLInputElement)) {
      if (!tmp2Result.isElement(hasAttribute, globalThis.HTMLTextAreaElement)) {
        ...
        tmp2Result2 = tmp2(1929);
      }
      tmp2Result = tmp2(1929);
    }
