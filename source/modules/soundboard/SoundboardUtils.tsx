...
// Dependencies: [5, 1220, 1962, 4276, 1372, 5097, 5099, 1074, 1218, 1936, 4295, 7455, 7448, 7456, 573, 7483, 7484, 7485, 563, 4482, 1944, 1941, 5105, 1241, 4818, 2]
...
import VoiceChannelEffectsActionCreators from "VoiceChannelEffectsActionCreators" /* 7456 */;
import getCurrentVoiceChannelDefault from "getCurrentVoiceChannel" /* 7483 */;
              ...
              if (null != customJoinSound) {
                if (tmp50Result.canUseCustomCallSounds(currentUser)) {
                  if (tmp53Result.canSelectedVoiceChannelUseSoundboard()) {
                    c3 = 1;
                    c4 = 1;
                    const obj4 = { value: tmp53(7448).maybeFetchSoundboardSounds(), done: false };
                    return obj4;
                  }
                  tmp53Result = tmp53(7485);
                }
                tmp50Result = tmp50(4295);
              }
            }
          }
            ...
            if (closure_130_15(closure_129_0, sound, closure_129_1, true)) {
              ...
              if (closure_130_16(closure_129_1)) {
                (function playCustomJoinSound(sound, id) {
                  closure_1_0(7448).playSoundLocally(id, sound);
                  const obj = closure_1_0(7448);
                  const result = closure_1_0(7456).sendVoiceChannelCustomCallSoundEffect(id, sound, false);
...
export const useSoundBoardDismissContentTypes = function useSoundBoardDismissContentTypes(isSoundboardButtonDisabled) {
  ...
  if (!flag) {
    ...
    if (!values.some((joinSound) => null != joinSound.joinSound)) {
      const result = tmp(4482).ageEligibleForPremiumUpsell(stateFromStores);
...
export const updateCustomJoinSound = function updateCustomJoinSound(guildId, arg1, location_stack) {
  ...
  const result = require("UserSettingsProtoActionCreators").updateUserGuildSettings(guildId, (joinSound) => {
    ...
    if (null != joinSound.joinSound) {
      let ADDED = tmp6(5105).AnalyticsChangeType.UPDATED;
    } else {
      ADDED = tmp6(5105).AnalyticsChangeType.ADDED;
    }
    ...
    obj2.sound_type = tmp6(5105).AnalyticsSoundType.ENTRY;
