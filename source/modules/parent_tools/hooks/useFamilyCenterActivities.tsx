...
// Dependencies: [7611, 7612, 563, 7666, 7308, 2]
...
require = fn;
const TeenActionDisplayType = fn(7612).TeenActionDisplayType;
const size = fn(2);
...
export const useFormattedTotalForDisplayType = function useFormattedTotalForDisplayType(displayType) {
  ...
  if (displayType === TeenActionDisplayType.TOTAL_VOICE_MINUTES) {
    return tmp(7666).formatTotalTime(num);
    ...
    if (null != totalSpendAmount) {
      ...
      if (null != totalSpendCurrency) {
        result = tmp(7308).shortenAndFormatPrice(totalSpendAmount, totalSpendCurrency);
        const tmpResult3 = tmp(7308);
      }
    }
    return result;
    ...
    if (null != totalGiftValue) {
      result1 = tmp(7308).shortenAndFormatPrice(totalGiftValue.amount, totalGiftValue.currency);
      const tmpResult4 = tmp(7308);
    }
    return result1;
  } else {
    return num;
