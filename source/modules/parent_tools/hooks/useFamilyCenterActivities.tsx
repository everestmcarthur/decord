...
// Dependencies: [7551, 7552, 563, 7607, 7248, 2]
...
// Module 14923 (useFamilyCenterActivities)
...
require = fn;
const TeenActionDisplayType = fn(7552).TeenActionDisplayType;
const size = fn(2);
...
export const useFormattedTotalForDisplayType = function useFormattedTotalForDisplayType(displayType) {
  ...
  if (displayType === TeenActionDisplayType.TOTAL_VOICE_MINUTES) {
    return tmp(7607).formatTotalTime(num);
    ...
    if (null != totalSpendAmount) {
      ...
      if (null != totalSpendCurrency) {
        result = tmp(7248).shortenAndFormatPrice(totalSpendAmount, totalSpendCurrency);
        const tmpResult3 = tmp(7248);
      }
    }
    return result;
    ...
    if (null != totalGiftValue) {
      result1 = tmp(7248).shortenAndFormatPrice(totalGiftValue.amount, totalGiftValue.currency);
      const tmpResult4 = tmp(7248);
    }
    return result1;
  } else {
    return num;
