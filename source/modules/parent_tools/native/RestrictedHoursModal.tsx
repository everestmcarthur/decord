...
// Dependencies: [32, 19, 17, 1371, 21, 4560, 576, 5577, 1611, 17298, 504, 4296, 4561, 1114, 2396, 8421, 17299, 4271, 4556, 7000, 17300, 6593, 17296, 4972, 11933, 2]
// Exports: default

// Module 17297 (RestrictedHoursLogoutBlockingLayer)
...
import useIsInRestrictedHoursDefault from "useIsInRestrictedHours" /* 17300 */;
...
function RestrictedHoursLogoutBlockingLayer(visible) {
  ...
  if (visible.visible) {
    ...
    obj[3] = callback2(ActivityIndicator.ActivityIndicator, { size: "large" });
...
function RestrictedHoursScreen(visible) {
  ...
  const effect = sharedValue3.useEffect(() => {
    ...
    obj1 = { duration: 1000, easing: null };
...
export default function RestrictedHoursModal() {
  ...
  callback = React.useCallback(() => {
    if (!tmp4.current) {
      ...
      callback(true);
      const obj = tmp4(6593);
      tmp4(6593).logout("restricted_hours").finally(() => {
        if (ref.current) {
          closure_1.current = false;
          callback(false);
        }
      });
      const logoutResult = tmp4(6593).logout("restricted_hours");
    }
  }, []);
  importDefault = tmp4;
  ...
  const effect1 = React.useEffect(() => {
    ...
    if (!current) {
      const result = callback(17296).closeRestrictedHoursModal();
      const obj = callback(17296);
    }
  }, items1);
  useBackPressHandlerDefault(() => true);
  obj = { screens: navigatorScreens, initialRouteName: constants.MAIN };
  return callback2(callback(11933).Modal, obj);
};
