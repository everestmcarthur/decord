...
// Dependencies: [32, 19, 17, 7590, 21, 4605, 576, 8716, 7644, 14972, 4601, 11987, 1114, 2396, 14978, 5173, 2]
// Exports: default
...
export default function FamilyCenterActivitySection(displayType) {
  ...
  const actionsForDisplayType = displayType(14972).useActionsForDisplayType(displayType);
  const obj = displayType(14972);
  const actionTotalsForDisplayType = displayType(14972).useActionTotalsForDisplayType(displayType);
  const obj2 = displayType(14972);
  const familyCenterActions = displayType(11987).useFamilyCenterActions({});
  ...
  if (0 === actionsForDisplayType.length) {
    ...
    const items1 = [closure_9(FamilyCenterActivitySectionHeader, obj6), substr.map((action) => closure_1_9(loadMore(14978), { action }, action.event_id)), ];
    if (substr.length >= actionTotalsForDisplayType) {
      ...
      if (familyCenterActions.isMoreLoading) {
        ...
        obj10.children = tmp17(tmp(4601).Text, obj11);
        tmp17Result = tmp17(tmp(5173).PressableOpacity, obj10);
      }
