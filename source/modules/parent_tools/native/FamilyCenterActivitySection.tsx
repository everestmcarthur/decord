...
// Dependencies: [32, 19, 17, 7734, 21, 4722, 576, 8884, 7788, 15112, 4718, 12160, 1115, 2480, 15118, 5292, 2]
// Exports: default

// Module 15117 (FamilyCenterActivitySection)
...
export default function FamilyCenterActivitySection(displayType) {
  ...
  const actionsForDisplayType = displayType(15112).useActionsForDisplayType(displayType);
  const obj = displayType(15112);
  const actionTotalsForDisplayType = displayType(15112).useActionTotalsForDisplayType(displayType);
  const obj2 = displayType(15112);
  const familyCenterActions = displayType(12160).useFamilyCenterActions({});
  ...
  if (0 === actionsForDisplayType.length) {
    ...
    const formatToPlainStringResult = intl.formatToPlainString(loadMore(2480)["7dMmJY"], obj4);
    const obj5 = { style: loadMoreButton.container, children: null };
    const obj6 = { displayType };
    const items1 = [closure_9(FamilyCenterActivitySectionHeader, obj6), substr.map((action) => closure_1_9(loadMore(15118), { action }, action.event_id)), ];
    if (substr.length >= actionTotalsForDisplayType) {
      ...
      if (familyCenterActions.isMoreLoading) {
        ...
        obj10.children = tmp17(tmp(4718).Text, obj11);
        tmp17Result = tmp17(tmp(5292).PressableOpacity, obj10);
      }
