...
// Dependencies: [32, 19, 17, 7612, 21, 4607, 576, 8739, 7666, 14949, 4603, 12012, 1114, 2396, 14955, 5175, 2]
// Exports: default

// Module 14954 (FamilyCenterActivitySection)
...
export default function FamilyCenterActivitySection(displayType) {
  ...
  const actionsForDisplayType = displayType(14949).useActionsForDisplayType(displayType);
  const obj = displayType(14949);
  const actionTotalsForDisplayType = displayType(14949).useActionTotalsForDisplayType(displayType);
  const obj2 = displayType(14949);
  const familyCenterActions = displayType(12012).useFamilyCenterActions({});
  ...
  if (0 === actionsForDisplayType.length) {
    ...
    const items1 = [closure_9(FamilyCenterActivitySectionHeader, obj6), substr.map((action) => closure_1_9(loadMore(14955), { action }, action.event_id)), ];
    if (substr.length >= actionTotalsForDisplayType) {
      ...
      if (familyCenterActions.isMoreLoading) {
        ...
        obj10.children = tmp17(tmp(4603).Text, obj11);
        tmp17Result = tmp17(tmp(5175).PressableOpacity, obj10);
      }
