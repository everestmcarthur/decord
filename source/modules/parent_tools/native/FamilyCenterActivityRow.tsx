...
// Dependencies: [19, 17, 1371, 7611, 7612, 1074, 21, 4607, 576, 1178, 38, 563, 11, 4603, 4451, 7666, 5636, 5642, 1114, 2396, 14956, 14959, 14960, 2]
// Exports: default

// Module 14955 (FamilyCenterActivityRow)
import _modDef38 from "module_38" /* 38 */;
import nativeDefault from "native" /* 576 */;
import FamilyCenterUtils from "FamilyCenterUtils" /* 7666 */;
import FamilyCenterActivityPurchaseRowDefault from "FamilyCenterActivityPurchaseRow" /* 14956 */;
import FamilyCenterActivityGiftRowDefault from "FamilyCenterActivityGiftRow" /* 14960 */;
...
const ACTION_TO_TEXT = fn(7612).ACTION_TO_TEXT;
...
const memoResult = noop.memo((action) => {
  ...
  if (null == stateFromStores) {
    ...
    obj6.children = tmp3(4451).getName(stateFromStores);
    const items2 = [closure_8(tmp6(4603).Text, obj6), ];
    const obj7 = { variant: "text-xs/medium", color: "channels-default", children: null };
    const tmp3Result2 = tmp3(4451);
    const _Date = Date;
    const date = new Date(extractTimestampResult);
    obj7.children = tmp6(7666).formatUserActivityTimestamp(date.getTime(), value.timestampFormatter);
    items2[1] = closure_8(tmp6(4603).Text, obj7);
...
const memoResult1 = noop.memo((action) => {
  ...
  if (undefined === stateFromStores) {
    ...
    obj4.size = tmp2(5636).GuildIconSizes.NORMAL;
    const items1 = [closure_8(tmp6(5636), obj4), ];
    ...
    if (hasItem) {
      const obj8 = { style: tmp.badge, guild: stateFromStores, size: tmp6(5642).Sizes.SMALL, disableColor: true };
      tmp11Result = tmp11(tmp6(5642), obj8);
      const tmp6Result2 = tmp6(5642);
    }
    ...
    items2[1] = closure_8(tmp2(4603).Text, obj9);
    ...
    if (undefined !== stateFromStores.approximateMemberCount) {
      ...
      tmp11Result2 = tmp11(tmp2(4603).Text, obj10);
    }
  ...
  if (tmpResult9.isPurchase(action)) {
    ...
    if (tmpResult10.isGift(action)) {
      ...
      if (null == giftInfo) {
        ...
        const giftRowDisplayInfo = tmp(14959).getGiftRowDisplayInfo(giftInfo);
