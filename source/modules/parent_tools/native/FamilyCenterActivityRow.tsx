...
// Dependencies: [19, 17, 1372, 7733, 7734, 1074, 21, 4722, 576, 1177, 38, 563, 11, 4718, 4566, 7788, 5754, 5760, 1115, 2480, 15119, 15122, 15123, 2]
// Exports: default

// Module 15118 (FamilyCenterActivityRow)
import _modDef38 from "module_38" /* 38 */;
import nativeDefault from "native" /* 576 */;
import FamilyCenterUtils from "FamilyCenterUtils" /* 7788 */;
import FamilyCenterActivityPurchaseRowDefault from "FamilyCenterActivityPurchaseRow" /* 15119 */;
import FamilyCenterActivityGiftRowDefault from "FamilyCenterActivityGiftRow" /* 15123 */;
...
const ACTION_TO_TEXT = fn(7734).ACTION_TO_TEXT;
...
const memoResult = noop.memo((action) => {
  ...
  if (null == stateFromStores) {
    ...
    obj6.children = tmp3(4566).getName(stateFromStores);
    const items2 = [closure_8(tmp6(4718).Text, obj6), ];
    const obj7 = { variant: "text-xs/medium", color: "channels-default", children: null };
    const tmp3Result2 = tmp3(4566);
    const _Date = Date;
    const date = new Date(extractTimestampResult);
    obj7.children = tmp6(7788).formatUserActivityTimestamp(date.getTime(), value.timestampFormatter);
    items2[1] = closure_8(tmp6(4718).Text, obj7);
...
const memoResult1 = noop.memo((action) => {
  ...
  if (undefined === stateFromStores) {
    ...
    obj4.size = tmp2(5754).GuildIconSizes.NORMAL;
    const items1 = [closure_8(tmp6(5754), obj4), ];
    ...
    if (hasItem) {
      const obj8 = { style: tmp.badge, guild: stateFromStores, size: tmp6(5760).Sizes.SMALL, disableColor: true };
      tmp11Result = tmp11(tmp6(5760), obj8);
      const tmp6Result2 = tmp6(5760);
    }
    ...
    items2[1] = closure_8(tmp2(4718).Text, obj9);
    ...
    if (undefined !== stateFromStores.approximateMemberCount) {
      ...
      obj10.children = intl.format(tmp6(2480)["5JmNgg"], obj19);
      tmp11Result2 = tmp11(tmp2(4718).Text, obj10);
    }
  ...
  if (tmpResult9.isPurchase(action)) {
    ...
    if (tmpResult10.isGift(action)) {
      ...
      if (null == giftInfo) {
        ...
        const giftRowDisplayInfo = tmp(15122).getGiftRowDisplayInfo(giftInfo);
