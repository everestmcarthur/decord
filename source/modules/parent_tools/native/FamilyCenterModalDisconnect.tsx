...
// Dependencies: [32, 19, 17, 21, 4639, 576, 4842, 4484, 8771, 4337, 1115, 12051, 12054, 2397, 8542, 8543, 15022, 7099, 4635, 14974, 5057, 9561, 12061, 5518, 5059, 5709, 12063, 2]
// Exports: default

// Module 15021 (FamilyCenterModalDisconnect)
...
import FamilyCenterAvatarPairDefault from "FamilyCenterAvatarPair" /* 15022 */;
...
function FamilyCenterModalDisconnectScreen(otherUser) {
  ...
  const requiresParentalConsent = otherUser(8771).useRequiresParentalConsent(otherUser.id);
  const obj2 = otherUser(8771);
  ...
  const familyCenterActions = otherUser(12051).useFamilyCenterActions({ onSuccess: callback, onError: callback1 });
  ...
  const obj3 = otherUser(12051);
  let intl = otherUser(1115).intl;
  const obj4 = otherUser(12054);
  ...
  items3[1] = closure_6(otherUser(4635).Text, obj11);
  items3[2] = closure_6(otherUser(4635).Text, { style: tmp.subtitle, variant: "text-sm/bold", color: "text-default", children: ageSpecificText });
  ...
  if (requiresParentalConsent) {
    ...
    tmp28Result = tmp28(tmp3(14974), obj13);
  }
  const obj14 = { children: null };
  items4[1] = tmp28Result;
  items4[2] = closure_6(otherUser(4635).Text, { style: tmp.body, variant: "text-md/normal", color: "text-default", children: ageSpecificText2 });
  const obj16 = { spacing: nativeDefault.space.PX_12, children: null };
  const items5 = [closure_6(otherUser(9561).Checkbox, { label: ageSpecificText3, checked: tmp9, onToggle: callback3 }), closure_6(otherUser(9561).Checkbox, { label: ageSpecificText4, checked: tmp11, onToggle: callback4 })];
  obj16.children = items5;
  items4[3] = closure_7(otherUser(5057).Stack, obj16);
  obj14.children = items4;
  const items6 = [closure_7(otherUser(8543).ModalContent, obj14), ];
  let tmp31 = !tmp9;
  if (tmp9) {
    tmp31 = !tmp11;
  }
  ...
  const items7 = [closure_6(otherUser(5059).Button, obj20), ];
  ...
  items7[1] = closure_6(otherUser(5059).Button, obj21);
  obj19.children = items7;
  obj18.children = closure_7(otherUser(5518).ButtonGroup, obj19);
  items6[1] = closure_6(otherUser(12061).ModalFooter, obj18);
  obj17.children = items6;
  return closure_7(otherUser(8542).ModalScreen, obj17);
}
...
export default function FamilyCenterModalDisconnect(otherUser) {
  ...
  return closure_6(otherUser(12063).Modal, obj);
};
