...
// Dependencies: [32, 19, 17, 21, 4638, 576, 4841, 4482, 8780, 4335, 1115, 12059, 12062, 2397, 8551, 8552, 15038, 7102, 4634, 14990, 5058, 9572, 12069, 5519, 5060, 5710, 12071, 2]
// Exports: default

// Module 15037 (FamilyCenterModalDisconnect)
...
import FamilyCenterAvatarPairDefault from "FamilyCenterAvatarPair" /* 15038 */;
...
function FamilyCenterModalDisconnectScreen(otherUser) {
  ...
  const requiresParentalConsent = otherUser(8780).useRequiresParentalConsent(otherUser.id);
  const obj2 = otherUser(8780);
  ...
  const familyCenterActions = otherUser(12059).useFamilyCenterActions({ onSuccess: callback, onError: callback1 });
  ...
  const obj3 = otherUser(12059);
  let intl = otherUser(1115).intl;
  const obj4 = otherUser(12062);
  ...
  items3[1] = closure_6(otherUser(4634).Text, obj11);
  items3[2] = closure_6(otherUser(4634).Text, { style: tmp.subtitle, variant: "text-sm/bold", color: "text-default", children: ageSpecificText });
  ...
  if (requiresParentalConsent) {
    ...
    tmp28Result = tmp28(tmp3(14990), obj13);
  }
  const obj14 = { children: null };
  items4[1] = tmp28Result;
  items4[2] = closure_6(otherUser(4634).Text, { style: tmp.body, variant: "text-md/normal", color: "text-default", children: ageSpecificText2 });
  const obj16 = { spacing: nativeDefault.space.PX_12, children: null };
  const items5 = [closure_6(otherUser(9572).Checkbox, { label: ageSpecificText3, checked: tmp9, onToggle: callback3 }), closure_6(otherUser(9572).Checkbox, { label: ageSpecificText4, checked: tmp11, onToggle: callback4 })];
  obj16.children = items5;
  items4[3] = closure_7(otherUser(5058).Stack, obj16);
  obj14.children = items4;
  const items6 = [closure_7(otherUser(8552).ModalContent, obj14), ];
  let tmp31 = !tmp9;
  if (tmp9) {
    tmp31 = !tmp11;
  }
  ...
  const items7 = [closure_6(otherUser(5060).Button, obj20), ];
  ...
  items7[1] = closure_6(otherUser(5060).Button, obj21);
  obj19.children = items7;
  obj18.children = closure_7(otherUser(5519).ButtonGroup, obj19);
  items6[1] = closure_6(otherUser(12069).ModalFooter, obj18);
  obj17.children = items6;
  return closure_7(otherUser(8551).ModalScreen, obj17);
}
...
export default function FamilyCenterModalDisconnect(otherUser) {
  ...
  return closure_6(otherUser(12071).Modal, obj);
};
