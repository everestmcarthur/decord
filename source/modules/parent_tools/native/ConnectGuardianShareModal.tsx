...
// Dependencies: [19, 17, 7647, 21, 4638, 576, 1115, 2397, 4335, 4841, 12059, 563, 14996, 8551, 8552, 5058, 4634, 14997, 5663, 5710, 12071, 2]
// Exports: default

// Module 17672 (ConnectGuardianShareModal)
...
function ConnectGuardianShareScreen() {
  ...
  const callback = noop.useCallback(() => {
    ...
    getLinkCode(4335).presentFailedToast(intl.string(getLinkCode(1115).t.R0RpRX));
    const obj = getLinkCode(4335);
    ModalActionCreatorsDefault.pop();
  }, []);
  let obj = getLinkCode(1115);
  getLinkCode = getLinkCode(12059).useFamilyCenterActions({ onError: callback }).getLinkCode;
  const obj2 = getLinkCode(12059);
  ...
  const items2 = [closure_6(getLinkCode(4634).Text, obj7), ];
  ...
  items2[1] = closure_6(getLinkCode(4634).Text, obj8);
  obj6.children = items2;
  const items3 = [closure_7(getLinkCode(5058).Stack, obj6), ];
  ...
  const items4 = [closure_6(getLinkCode(4634).Text, obj10), ];
  if (null != stateFromStores) {
    if (null != stateFromStores1) {
      const obj11 = { shareActions: "full", linkCode: stateFromStores, expiresAt: stateFromStores1, onRefresh: getLinkCode };
      let tmp11Result = tmp11(tmp2(14997).ConnectGuardianCard, obj11);
    }
    const obj12 = { children: null };
    const obj13 = { children: null };
    items4[1] = tmp11Result;
    obj9.children = items4;
    items3[1] = tmp12(getLinkCode(5058).Stack, obj9);
    obj5.children = items3;
    obj13.children = tmp12(getLinkCode(5058).Stack, obj5);
    obj12.children = tmp11(getLinkCode(8552).ModalContent, obj13);
    return tmp11(getLinkCode(8551).ModalScreen, obj12);
  }
  tmp11Result = tmp11(View, { style: tmp.loading, children: closure_6(getLinkCode(5663).ActivityIndicator, {}) });
}
