...
// Dependencies: [19, 17, 7589, 21, 4605, 576, 1114, 2396, 4302, 4808, 11987, 563, 14958, 8486, 8487, 5023, 4601, 14959, 5627, 5674, 11999, 2]
// Exports: default

// Module 17556 (ConnectGuardianShareModal)
...
function ConnectGuardianShareScreen() {
  ...
  const callback = noop.useCallback(() => {
    ...
    getLinkCode(4302).presentFailedToast(intl.string(getLinkCode(1114).t.R0RpRX));
    const obj = getLinkCode(4302);
    ModalActionCreatorsDefault.pop();
  }, []);
  let obj = getLinkCode(1114);
  getLinkCode = getLinkCode(11987).useFamilyCenterActions({ onError: callback }).getLinkCode;
  const obj2 = getLinkCode(11987);
  ...
  const items2 = [closure_6(getLinkCode(4601).Text, obj7), ];
  ...
  items2[1] = closure_6(getLinkCode(4601).Text, obj8);
  obj6.children = items2;
  const items3 = [closure_7(getLinkCode(5023).Stack, obj6), ];
  ...
  const items4 = [closure_6(getLinkCode(4601).Text, obj10), ];
  if (null != stateFromStores) {
    if (null != stateFromStores1) {
      const obj11 = { shareActions: "full", linkCode: stateFromStores, expiresAt: stateFromStores1, onRefresh: getLinkCode };
      let tmp11Result = tmp11(tmp2(14959).ConnectGuardianCard, obj11);
    }
    const obj12 = { children: null };
    const obj13 = { children: null };
    items4[1] = tmp11Result;
    obj9.children = items4;
    items3[1] = tmp12(getLinkCode(5023).Stack, obj9);
    obj5.children = items3;
    obj13.children = tmp12(getLinkCode(5023).Stack, obj5);
    obj12.children = tmp11(getLinkCode(8487).ModalContent, obj13);
    return tmp11(getLinkCode(8486).ModalScreen, obj12);
  }
  tmp11Result = tmp11(View, { style: tmp.loading, children: closure_6(getLinkCode(5627).ActivityIndicator, {}) });
}
