...
// Dependencies: [5, 19, 17, 1371, 7538, 21, 4560, 576, 1483, 563, 7539, 11920, 7593, 11921, 8421, 8422, 1178, 1396, 1114, 2396, 4556, 11922, 11923, 11931, 5433, 4975, 4763, 5692, 8651, 38, 11569, 6992, 5577, 8272, 1894, 8411, 8413, 4773, 5624, 11932, 11933, 2]
// Exports: default

// Module 11919 (FamilyCenterModalRequestConfirm)
...
class FamilyCenterModalRequestConfirm {
  constructor(arg0) {
    ...
    obj23[3] = require("module_4763").pop;
...
function FamilyCenterConfirmAgeScreen(teenIdentity) {
  ...
  const effect = React.useEffect(() => {
    ...
    if (tmp2) {
      tmp2 = stateFromStores !== navigation(1894).AgeVerificationStatusUkAndAusOnly.UNVERIFIED;
    }
    if (tmp2) {
      const replaced = navigation.replace(navigation(11920).FamilyCenterModalRequestSections.VERIFYING);
    }
  }, items1);
  const callback = React.useCallback(() => {
    let obj = stateFromStores(8411);
    obj = { entryPoint: navigation(8413).AgeVerificationModalEntryPoint.FAMILY_CENTER_CONNECTION };
    const result = obj.showAgeVerificationGetStartedModal(obj);
  }, []);
  obj = { style: tmp.content, children: null };
  obj = { style: tmp.art, children: callback(navigation(5692).FamilyShieldSpotIllustration, {}) };
  ...
  obj1[3] = intl.formatToPlainString(stateFromStores(2396).pQQMJ7, { username: str });
  ...
  obj3[3] = intl2.format(stateFromStores(2396)["0o3yg8"], { link: "https://support.discord.com/hc/articles/14155060633623" });
  items2[2] = callback(navigation(4556).Text, obj3);
  obj[1] = items2;
  ...
  obj6[1] = intl3.string(stateFromStores(2396)["3oUE4o"]);
  obj6[2] = callback;
  const items4 = [callback(navigation(4975).Button, obj6), ];
  const obj7 = { variant: "tertiary", text: null, onPress: null };
  const intl4 = tmp2(1114).intl;
  obj7[1] = intl4.string(navigation(1114).t.oEAioF);
  obj7[2] = function onPress() {
    return stateFromStores(4763).pop();
  };
  items4[1] = callback(navigation(4975).Button, obj7);
  obj5[0] = items4;
  obj4[0] = closure_13(navigation(5433).ButtonGroup, obj5);
  items3[1] = callback(navigation(11931).ModalFooter, obj4);
  obj2[0] = items3;
...
function FamilyCenterPrereqMustBeAdultScreen() {
  ...
  const intl = navigation(1114).intl;
  obj[0] = intl.string(stateFromStores(2396).BQFHXW);
  const intl2 = navigation(1114).intl;
  obj[1] = intl2.format(stateFromStores(2396).WDjaKn, { link: { onClick: callback } });
  obj = { text: null, onPress: null };
  const intl3 = navigation(1114).intl;
  obj[0] = intl3.string(navigation(1114).t["NX+WJN"]);
  obj[1] = stateFromStores(4763).pop;
  obj[2] = obj;
  return callback(FamilyCenterPrereqScreen, obj);
}
...
createCacheKey[1] = HeaderBackImage.getHeaderCloseButton(_modDef4763.pop);
...
export default function FamilyCenterRequestModal(userId) {
  ...
  obj[2] = intl.string(userId(1114).t["13/7kX"]);
  return callback(userId(11933).Modal, obj);
};
