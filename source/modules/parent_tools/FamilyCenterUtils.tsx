...
// Dependencies: [7647, 7648, 1115, 2397, 4229, 7649, 2]
...
export const getActivityWindowTimestampFormatter = function getActivityWindowTimestampFormatter(arg0) {
  ...
  if (arg0) {
    ...
    obj.yesterday = intl3.string(tmp3(2397).stOECr);
    obj.days = tmp3(2397).n8n5Ba;
    let tmp5 = obj;
  } else {
    obj.today = string(tmp4.g1ZX6m);
    const intl2 = tmp(1115).intl;
    obj.yesterday = intl2.string(tmp3(2397).s3qSVt);
    obj.days = tmp3(2397).f1UJiC;
    tmp5 = obj;
  }
  return tmp5;
};
export const formatUserActivityTimestamp = function formatUserActivityTimestamp(time, timestampFormatter, arg2) {
  const diffResult = _modDef4229().diff(_modDef4229(time), "s");
  const tmp3 = timestampFormatter();
  const obj = _modDef4229();
  _modDef4229(time).format("LL");
  if (diffResult < c10) {
...
export const getTopUserOrGuildDescription = function getTopUserOrGuildDescription(dms_sent, call_count) {
  ...
  const intl = util.intl;
  formatToPlainStringResult = intl.formatToPlainString(_modDef2397.IYqGMG, { messageCount: dms_sent, callCount: call_count });
};
