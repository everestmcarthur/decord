// Module ID: 11156
// Function ID: 11157
// Name: getFavoritesAddButtonLabel
// Dependencies: [1115, 3239, 2]
// Exports: getFavoritesAddButtonLabel

// Module 11156 (getFavoritesAddButtonLabel)
import util from "util" /* 1115 */;
import _modDef3239 from "module_3239" /* 3239 */;
import size from "module_2" /* 2 */;

const result = size.fileFinishedImporting("modules/favorites/utils/getFavoritesAddButtonLabel.tsx");

export const getFavoritesAddButtonLabel = function getFavoritesAddButtonLabel(length) {
  if (length >= 2) {
    const intl2 = util.intl;
    const obj = { count: length };
    let formatToPlainStringResult = intl2.formatToPlainString(_modDef3239.LbCa8x, obj);
  } else {
    const intl = util.intl;
    formatToPlainStringResult = intl.string(_modDef3239.xKXcSu);
  }
  return formatToPlainStringResult;
};
