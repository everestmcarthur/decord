...
// Dependencies: [19, 1960, 21, 16209, 16210, 16211, 10315, 504, 7271, 7223, 1114, 7273, 5125, 12246, 16212, 5732, 7040, 4565, 2]
// Exports: default
...
export default function FavoritesGuildActionSheet(onClose) {
  ...
  obj3.header = closure_4(onClose(7223).BottomSheetTitleHeader, obj4);
  let tmp8Result = null;
  if (tmp4.isAvailable) {
    ...
    const obj8 = { IconComponent: tmp5(5125).ThreadIcon };
    obj7.icon = tmp8(tmp5(7273).ActionSheetRow.Icon, obj8);
    ({ isEnabled: obj6.value, toggle: obj6.onValueChange } = tmp4);
    obj5.children = tmp8(tmp5(7273).ActionSheetSwitchRow, obj7);
    tmp8Result = tmp8(tmp5(7273).ActionSheetRow.Group, obj5);
  }
  ...
  if (obj.useFavoritesAccess("FavoritesGuildActionSheet").hasAccess) {
    ...
    if (stateFromStores) {
      ...
      const obj11 = { IconComponent: tmp5(12246).ArrowsUpDownIcon };
      obj10.icon = tmp8(tmp5(7273).ActionSheetRow.Icon, obj11);
      obj10.onPress = function onPress() {
        onClose();
        openFavoritesGuildChannelSortModalDefault();
      };
      obj9.children = tmp8(tmp5(7273).ActionSheetRow, obj10);
      tmp8Result3 = tmp8(tmp5(7273).ActionSheetRow.Group, obj9);
    }
  }
  items1[1] = tmp8Result3;
  const obj13 = { label: tmp2.label, subLabel: tmp2.subLabel, icon: null, variant: null, onPress: null };
  if (tmp2.isPreview) {
    let EyeSlashIcon = tmp5(5732).XSmallIcon;
  } else {
    EyeSlashIcon = tmp5(7040).EyeSlashIcon;
  }
  obj13.icon = closure_4(onClose(7273).ActionSheetRow.Icon, { IconComponent: EyeSlashIcon });
  let str = "danger";
  if (tmp2.isPreview) {
    str = "default";
  }
  ...
  const items2 = [closure_4(onClose(7273).ActionSheetRow, obj13), ];
  ...
  if (tmp3.isAvailable) {
    ...
    const obj25 = { IconComponent: tmp5(4565).TrashIcon };
    obj24.icon = tmp8(tmp5(7273).ActionSheetRow.Icon, obj25);
    obj24.onPress = function onPress() {
      onClose();
      closure_2.perform();
    };
    tmp8Result4 = tmp8(tmp5(7273).ActionSheetRow, obj24);
  }
  items2[1] = tmp8Result4;
  items1[2] = closure_5(onClose(7273).ActionSheetRow.Group, { hasIcons: true, children: items2 });
  obj3.children = items1;
  return closure_5(onClose(7271).ActionSheet, obj3);
};
