...
// Dependencies: [5, 32, 19, 17, 1970, 10949, 21, 4606, 576, 11072, 11073, 11074, 1369, 4304, 1114, 10312, 11069, 1477, 1115, 11076, 3236, 5176, 11077, 11088, 11090, 2]
// Exports: default

// Module 11071 (FavoritesGuildAddChannelModal)
...
export default function FavoritesGuildAddChannelModal(parentId) {
      ...
      try {
        ...
        if (0 === c3) {
          if (arg0 === 1) {
            ...
            c3 = 1;
            c4 = 1;
            const obj6 = { value: Promise.all(first.map(parentId(11074).getOrResolveChannelIdFromDestinationId)), done: false };
            return obj6;
          }
        } else if (arg0 === 1) {
          c4 = 3;
          ...
          if (0 !== closure_129_0.length) {
            ...
            parentId(10312).addFavoriteChannels(obj8);
            const obj2 = parentId(10312);
            const result = parentId(11069).closeFavoritesGuildAddChannelModal();
            const obj4 = parentId(11069);
          } else {
  ...
  const tmp2 = first;
  const tmp4 = first(11072)();
  let intl = parentId(1114).intl;
  obj2.title = intl.string(first(3236).Rp35U1);
  obj2.onClose = parentId(11069).closeFavoritesGuildAddChannelModal;
  const items2 = [closure_9(first(11076), obj2), ];
  let obj3 = { style: tmp.container, children: null };
  const items3 = [closure_9(first(5176), { absolute: true }), , ];
  ...
  let num = 0;
  const tmp12 = first(11076);
  if (first.length > 0) {
    num = tmp2(576).space.PX_80;
  }
  obj4.insetEnd = num;
  obj4.disableSelection = first.length >= closure_7;
  items3[1] = closure_9(first(11077), obj4);
  let obj5 = { isVisible: first.length > 0, floatingBackgroundColor: tmp.container.backgroundColor, text: null, onPress: null };
  const tmp14 = first(11077);
  obj5.text = parentId(11090).getFavoritesAddButtonLabel(first.length);
  obj5.onPress = callback1;
  items3[2] = closure_9(parentId(11088).ModalFloatingAction, obj5);
