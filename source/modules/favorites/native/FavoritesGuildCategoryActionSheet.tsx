...
// Dependencies: [19, 1960, 21, 4758, 11047, 1935, 7249, 7201, 7251, 11023, 1114, 7432, 16193, 10699, 7241, 4302, 504, 2]
// Exports: default

// Module 16192 (FavoritesGuildCategoryActionSheet)
import ToastUtils from "ToastUtils" /* 4302 */;
import ClipboardUtils from "ClipboardUtils" /* 7241 */;
import openFavoritesGuildCategorySettingsModalDefault from "openFavoritesGuildCategorySettingsModal" /* 16193 */;
...
function FavoritesGuildCategoryActionSheetConnected(category) {
  ...
  const tmp3 = onClose(11047)(category);
  dependencyMap = tmp3;
  const DeveloperMode = category(1935).DeveloperMode;
  const setting = DeveloperMode.useSetting();
  let obj = { header: closure_5(category(7201).BottomSheetTitleHeader, { title: onClose(4758)(category, true) }), children: null };
  let tmp7Result = null;
  if (null != tmp3) {
    const obj2 = { hasIcons: true, children: null };
    const obj3 = { label: tmp3.label, icon: null, onPress: null };
    const obj4 = { IconComponent: tmp4(11023).PlusLargeIcon };
    obj3.icon = tmp7(tmp4(7251).ActionSheetRow.Icon, obj4);
    obj3.onPress = function onPress() {
      closure_2.perform();
      onClose();
    };
    obj2.children = tmp7(tmp4(7251).ActionSheetRow, obj3);
    tmp7Result = tmp7(tmp4(7251).ActionSheetRow.Group, obj2);
  }
  ...
  const tmp2 = onClose(4758)(category, true);
  const tmp6 = closure_6;
  obj6.icon = closure_5(category(7251).ActionSheetRow.Icon, { IconComponent: category(7432).SettingsIcon });
  obj6.onPress = function onPress() {
    openFavoritesGuildCategorySettingsModalDefault(category.id);
    onClose();
  };
  obj5.children = closure_5(category(7251).ActionSheetRow, obj6);
  items[1] = closure_5(category(7251).ActionSheetRow.Group, obj5);
  ...
  if (setting) {
    ...
    const obj10 = { IconComponent: tmp4(10699).IdIcon };
    obj9.icon = tmp7(tmp4(7251).ActionSheetRow.Icon, obj10);
    obj9.onPress = function onPress() {
      ClipboardUtils.copy(category.id);
      ToastUtils.presentIdCopied();
      onClose();
    };
    obj8.children = tmp7(tmp4(7251).ActionSheetRow, obj9);
    tmp7Result2 = tmp7(tmp4(7251).ActionSheetRow.Group, obj8);
  }
  items[2] = tmp7Result2;
  obj.children = items;
  return tmp6(category(7249).ActionSheet, obj);
}
const jsxProd = fn(21);
