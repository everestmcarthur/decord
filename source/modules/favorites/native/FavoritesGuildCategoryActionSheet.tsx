...
// Dependencies: [19, 1960, 21, 4727, 11007, 1935, 7211, 7163, 7213, 10983, 1114, 7394, 16144, 10661, 7203, 4271, 504, 2]
// Exports: default

// Module 16143 (FavoritesGuildCategoryActionSheet)
import ToastUtils from "ToastUtils" /* 4271 */;
import ClipboardUtils from "ClipboardUtils" /* 7203 */;
import openFavoritesGuildCategorySettingsModalDefault from "openFavoritesGuildCategorySettingsModal" /* 16144 */;
...
function FavoritesGuildCategoryActionSheetConnected(category) {
  ...
  const tmp3 = onClose(11007)(category);
  dependencyMap = tmp3;
  const DeveloperMode = category(1935).DeveloperMode;
  const setting = DeveloperMode.useSetting();
  let obj = { header: closure_5(category(7163).BottomSheetTitleHeader, { title: onClose(4727)(category, true) }), children: null };
  let tmp7Result = null;
  if (null != tmp3) {
    const obj2 = { hasIcons: true, children: null };
    const obj3 = { label: tmp3.label, icon: null, onPress: null };
    const obj4 = { IconComponent: tmp4(10983).PlusLargeIcon };
    obj3.icon = tmp7(tmp4(7213).ActionSheetRow.Icon, obj4);
    obj3.onPress = function onPress() {
      closure_2.perform();
      onClose();
    };
    obj2.children = tmp7(tmp4(7213).ActionSheetRow, obj3);
    tmp7Result = tmp7(tmp4(7213).ActionSheetRow.Group, obj2);
  }
  ...
  const tmp2 = onClose(4727)(category, true);
  const tmp6 = closure_6;
  obj6.icon = closure_5(category(7213).ActionSheetRow.Icon, { IconComponent: category(7394).SettingsIcon });
  obj6.onPress = function onPress() {
    openFavoritesGuildCategorySettingsModalDefault(category.id);
    onClose();
  };
  obj5.children = closure_5(category(7213).ActionSheetRow, obj6);
  items[1] = closure_5(category(7213).ActionSheetRow.Group, obj5);
  ...
  if (setting) {
    ...
    const obj10 = { IconComponent: tmp4(10661).IdIcon };
    obj9.icon = tmp7(tmp4(7213).ActionSheetRow.Icon, obj10);
    obj9.onPress = function onPress() {
      ClipboardUtils.copy(category.id);
      ToastUtils.presentIdCopied();
      onClose();
    };
    obj8.children = tmp7(tmp4(7213).ActionSheetRow, obj9);
    tmp7Result2 = tmp7(tmp4(7213).ActionSheetRow.Group, obj8);
  }
  items[2] = tmp7Result2;
  obj.children = items;
  return tmp6(category(7211).ActionSheet, obj);
}
const jsxProd = fn(21);
