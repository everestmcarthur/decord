...
// Dependencies: [19, 17, 16459, 1074, 10361, 21, 576, 4722, 16363, 1115, 3322, 5850, 7245, 16460, 16461, 5144, 11233, 2]
// Exports: default, getFavoritesSuggestionsNoticeHeight

// Module 16458 (FavoritesGuildSuggestedChannels)
...
import SearchableDestinationListRowDefault from "SearchableDestinationListRow" /* 16460 */;
import handleFavoritesGuildAddSuggestedChannelDefault from "handleFavoritesGuildAddSuggestedChannel" /* 16461 */;
...
let closure_8 = fn(10361).getScaledCategoryRowHeight;
const jsxProd = fn(21);
...
export default function FavoritesGuildSuggestedChannels() {
  let tmp = closure_13();
  const categoryStyles = arr(16363).useCategoryStyles();
  arr = closure_5();
  ...
  const memo = noop.useMemo(() => {
    ...
    obj.label = intl.string(_modDef3322.F3dWTe);
    obj.perform = perform;
  ...
  if (0 !== arr.length) {
    ...
    obj3.name = intl.string(_modDef3322.oHWnLy);
    obj3.styles = categoryStyles;
    obj3.trailingAction = memo;
    const items2 = [tmp2(16363).renderCategoryItem(obj3), ];
    const obj4 = {
      style: tmp.rows,
      children: arr.map((item, index) => {
          ...
          obj3.onPress = function onPress() {
            const tmp = closure_1(16461);
            return tmp(arr(11233).getDestinationIdFromResult(closure_0));
          };
