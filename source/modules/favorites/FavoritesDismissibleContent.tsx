...
// Dependencies: [32, 19, 1955, 1944, 7499, 10365, 10363, 10378, 7498, 10377, 2]
...
// Module 10379 (FavoritesDismissibleContent)
import dismissible_content from "dismissible_content" /* 1944 */;
import useSelectedDismissibleContent from "useSelectedDismissibleContent" /* 7498 */;
import useGetDismissibleContent from "useGetDismissibleContent" /* 7499 */;
import FavoritesHooks from "FavoritesHooks" /* 10363 */;
import FavoritesGuildExperiment from "FavoritesGuildExperiment" /* 10365 */;
import FavoritesGuildIntroPopover from "FavoritesGuildIntroPopover" /* 10377 */;
import useCanShowFavoritesGuildOnboardingDefault from "useCanShowFavoritesGuildOnboarding" /* 10378 */;
    ...
    if (set) {
      if (tmp4) {
        if (!isFavoritesGuildSelected) {
          if (first === tmp(1944).DismissibleContent.FAVORITES_SERVER_ONBOARDING_MENU_ITEM) {
            let items = [tmp(1944).DismissibleContent.FAVORITES_SERVER_ONBOARDING_MENU_ITEM];
          }
          const obj4 = { shouldShowPopover: null, markPopoverAsDismissed: null };
          [tmp11, tmp12] = tmp8(items);
          obj4.shouldShowPopover = tmp11 === tmp(1944).DismissibleContent.FAVORITES_SERVER_ONBOARDING_MENU_ITEM;
...
export const useFavoritesBetaTagDismissibleContent = function useFavoritesBetaTagDismissibleContent(arg0) {
  ...
  if (obj.useFavoritesGuildConfig({ location: "FavoritesDismissibleContent" }).isFreemium) {
    if (arg0) {
      const items = [tmp(1944).DismissibleContent.FAVORITES_GUILD_NEW_BADGE];
    }
    [tmp9, require] = tmp4([]);
    const tmp10 = tmp9 === tmp(1944).DismissibleContent.FAVORITES_GUILD_NEW_BADGE;
    closure_1 = tmp10;
    const obj2 = {
