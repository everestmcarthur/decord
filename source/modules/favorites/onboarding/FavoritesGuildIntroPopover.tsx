...
// Dependencies: [32, 19, 1950, 1961, 1074, 560, 1944, 10363, 504, 10378, 7498, 2]
...
// Module 10377 (FavoritesGuildIntroPopover)
import dismissible_content from "dismissible_content" /* 1944 */;
import useCanShowFavoritesGuildOnboardingDefault from "useCanShowFavoritesGuildOnboarding" /* 10378 */;
import _slicedToArray from "module_32" /* 32 */;
import noop from "module_19" /* 19 */;
import DismissibleContentShownStateStore_mod from "DismissibleContentShownStateStore" /* 1950 */;
import FavoriteStore from "FavoriteStore" /* 1961 */;

const require = globalThis.__r;

require = fn;
let DismissibleContentShownStateStore = fn(1950);
  ...
  if (hasAccess) {
    if (isFreemium) {
      if (!stateFromStores) {
        if (tmp5) {
          if (tmp6) {
            let items1 = [tmp(1944).DismissibleContent.FAVORITES_SERVER_ONBOARDING_INTRO];
          }
          const tmp10 = _slicedToArray(tmp8(items1), 2);
          _require = tmp11;
          if (tmp10[0] === tmp(1944).DismissibleContent.FAVORITES_SERVER_ONBOARDING_INTRO) {
            const items2 = [tmp(1944).DismissibleContent.FAVORITES_SERVER_ONBOARDING_MENU_ITEM];
            let items3 = items2;
          } else {
            items3 = [];
          }
          const tmp12 = _slicedToArray(tmp(7498).useSelectedDismissibleContent(items3, undefined, true), 1)[0] === tmp(1944).DismissibleContent.FAVORITES_SERVER_ONBOARDING_MENU_ITEM;
          importDefault = tmp12;
          const items4 = [tmp12];
