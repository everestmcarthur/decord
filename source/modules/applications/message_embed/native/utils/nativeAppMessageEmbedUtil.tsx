...
// Dependencies: [4487, 576, 8261, 8262, 1397, 2]
// Exports: getAppGradientColors, getAppIconSrc

// Module 12086 (nativeAppMessageEmbedUtil)
import nativeDefault from "native" /* 576 */;
import AvatarUtilsDefault from "AvatarUtils" /* 1397 */;
import useAvatarColor from "useAvatarColor" /* 8261 */;
import ColorUtils_mod from "ColorUtils" /* 4487 */;
...
export const getAppGradientColors = function getAppGradientColors(appIconSrc) {
  if (null == appIconSrc) {
    ...
    if (obj5.hasFetchedColors(appIconSrc)) {
      const heroColors = tmp6(8262).getHeroColors(appIconSrc);
      ({ primaryColor, secondaryColor } = heroColors);
      let tmp5 = tmp;
      if (false === tmp4) {
        items = [tmp6(4487).hexToRgba(primaryColor), ];
        const tmp6Result4 = tmp6(4487);
        items[1] = tmp6(4487).hexToRgba(secondaryColor);
        tmp5 = items;
        const tmp6Result5 = tmp6(4487);
      }
      return tmp5;
    } else {
      tmp6(8261).maybeFetchColors(appIconSrc);
      return tmp;
    }
    obj5 = useAvatarColor;
  }
