...
// Dependencies: [32, 1372, 8260, 4867, 1483, 9643, 11447, 7270, 12075, 8052, 1115, 12076, 9421, 8259, 9638, 12077, 12078, 1397, 1366, 12079, 7289, 11657, 7630, 9639, 4505, 1610, 9705, 7296, 4337, 1370, 2]
// Exports: createAppMessageEmbed, getAppLinkGateResult, handleTapAppMessageEmbed

// Module 12074 (createAppMessageEmbed)
...
import ApplicationUtils from "ApplicationUtils" /* 9705 */;
import AppLauncherPlayUtils from "AppLauncherPlayUtils" /* 11657 */;
import ContentClassificationVisibility from "ContentClassificationVisibility" /* 12075 */;
import joinOrStartActivityInChannel from "joinOrStartActivityInChannel" /* 12079 */;
...
export const getAppLinkGateResult = function getAppLinkGateResult(arg0) {
  ...
  if (null == application) {
    ...
    if (contentClassificationVisibility !== ContentClassificationVisibility.ContentClassificationVisibility.DISPLAY) {
      ...
      if (contentClassificationVisibility === tmp3(12075).ContentClassificationVisibility.BLOCK_UNDERAGE) {
      ...
      obj4.displayType = tmp3(12076).AppMessageEmbedDisplayType.BLOCKED;
      obj4.appId = "";
...
export const createAppMessageEmbed = function createAppMessageEmbed(arg0) {
  ...
  if (isEmbeddedAppResult) {
    ...
    if (applicationAssetFetchState === FetchState.NOT_FETCHED) {
      const assetIds = tmp2(8259).fetchAssetIds(id, closure_11);
      return null;
  ...
  if (null != maxParticipants) {
    ...
    if (isEmbeddedAppResult) {
      if (tmp2Result9.canLaunchFrame(app)) {
        ...
        const playInContext = tmp2(12077).getPlayInContext(id, message.channel_id);
        ...
        const tmp2Result10 = tmp2(12077);
      }
      tmp2Result9 = tmp2(9638);
    }
    ({ id: id2, bot: bot2 } = app);
    const joined = tags.join(" \u2219 ");
    if (tmp2Result11.isEmbeddedApp(app)) {
      let assetIds1 = tmp2(8259).getAssetIds(id2, closure_11);
      if (assetIds1 == null) {
        assetIds1 = [];
      }
      ...
      let assetImage = null;
      if (null != first) {
        assetImage = tmp2(8259).getAssetImage(id2, first, c12);
        const tmp2Result13 = tmp2(8259);
      }
      if (null != assetImage) {
      ...
      let appIconSrc = null;
      if (null != icon) {
        appIconSrc = tmp2(12078).getAppIconSrc(id, icon, bot);
        const tmp2Result14 = tmp2(12078);
      }
      let staticBannerSrc = appIconSrc;
      if (appIconSrc == null) {
        staticBannerSrc = obj10.staticBannerSrc;
      }
      const tmp2Result12 = tmp2(8259);
      const obj7 = {};
      const appGradientColors = tmp2(12078).getAppGradientColors(staticBannerSrc);
      const merged = Object.assign(baseColors);
      const merged1 = Object.assign(obj10);
      obj7.displayType = tmp2(12076).AppMessageEmbedDisplayType.DISPLAY;
      obj7.appId = app.id;
...
export const handleTapAppMessageEmbed = function handleTapAppMessageEmbed(appId) {
  ...
  if ("play_in_channel" === actionId) {
    ...
    const items = [tmp2(7289).APP_MESSAGE_EMBED];
    ...
    if (null != bot) {
      ...
      const items1 = [tmp2(7289).APP_MESSAGE_EMBED];
    ...
    const items2 = [tmp2(7289).APP_MESSAGE_EMBED];
    obj11.analyticsLocations = items2;
    obj10.analyticsContext = obj11;
    tmp2(9639).launchFrame(obj10);
    const tmp2Result = tmp2(9639);
