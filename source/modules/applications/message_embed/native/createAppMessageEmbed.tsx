...
// Dependencies: [32, 1372, 8269, 4866, 1483, 9654, 11455, 7273, 12083, 8061, 1115, 12084, 9432, 8268, 9649, 12085, 12086, 1397, 1366, 12087, 7292, 11665, 7633, 9650, 4504, 1610, 9716, 7299, 4335, 1370, 2]
// Exports: createAppMessageEmbed, getAppLinkGateResult, handleTapAppMessageEmbed

// Module 12082 (createAppMessageEmbed)
...
import ApplicationUtils from "ApplicationUtils" /* 9716 */;
import AppLauncherPlayUtils from "AppLauncherPlayUtils" /* 11665 */;
import ContentClassificationVisibility from "ContentClassificationVisibility" /* 12083 */;
import joinOrStartActivityInChannel from "joinOrStartActivityInChannel" /* 12087 */;
import _slicedToArray from "module_32" /* 32 */;
import UserStore from "UserStore" /* 1372 */;
import ApplicationAssetsStore from "ApplicationAssetsStore" /* 8269 */;
import ApplicationStore from "ApplicationStore" /* 4866 */;

require = fn;
const FetchState = fn(8269).FetchState;
...
export const getAppLinkGateResult = function getAppLinkGateResult(arg0) {
  ...
  if (null == application) {
    ...
    if (contentClassificationVisibility !== ContentClassificationVisibility.ContentClassificationVisibility.DISPLAY) {
      ...
      if (contentClassificationVisibility === tmp3(12083).ContentClassificationVisibility.BLOCK_UNDERAGE) {
      ...
      obj4.displayType = tmp3(12084).AppMessageEmbedDisplayType.BLOCKED;
      obj4.appId = "";
...
export const createAppMessageEmbed = function createAppMessageEmbed(arg0) {
  ...
  if (isEmbeddedAppResult) {
    ...
    if (applicationAssetFetchState === FetchState.NOT_FETCHED) {
      const assetIds = tmp2(8268).fetchAssetIds(id, closure_11);
      return null;
  ...
  if (null != maxParticipants) {
    ...
    if (isEmbeddedAppResult) {
      if (tmp2Result9.canLaunchFrame(app)) {
        ...
        const playInContext = tmp2(12085).getPlayInContext(id, message.channel_id);
        ...
        const tmp2Result10 = tmp2(12085);
      }
      tmp2Result9 = tmp2(9649);
    }
    ({ id: id2, bot: bot2 } = app);
    const joined = tags.join(" \u2219 ");
    if (tmp2Result11.isEmbeddedApp(app)) {
      let assetIds1 = tmp2(8268).getAssetIds(id2, closure_11);
      if (assetIds1 == null) {
        assetIds1 = [];
      }
      ...
      let assetImage = null;
      if (null != first) {
        assetImage = tmp2(8268).getAssetImage(id2, first, c12);
        const tmp2Result13 = tmp2(8268);
      }
      if (null != assetImage) {
      ...
      let appIconSrc = null;
      if (null != icon) {
        appIconSrc = tmp2(12086).getAppIconSrc(id, icon, bot);
        const tmp2Result14 = tmp2(12086);
      }
      let staticBannerSrc = appIconSrc;
      if (appIconSrc == null) {
        staticBannerSrc = obj10.staticBannerSrc;
      }
      const tmp2Result12 = tmp2(8268);
      const obj7 = {};
      const appGradientColors = tmp2(12086).getAppGradientColors(staticBannerSrc);
      const merged = Object.assign(baseColors);
      const merged1 = Object.assign(obj10);
      obj7.displayType = tmp2(12084).AppMessageEmbedDisplayType.DISPLAY;
      obj7.appId = app.id;
...
export const handleTapAppMessageEmbed = function handleTapAppMessageEmbed(appId) {
  ...
  if ("play_in_channel" === actionId) {
    ...
    const items = [tmp2(7292).APP_MESSAGE_EMBED];
    ...
    if (null != bot) {
      ...
      const items1 = [tmp2(7292).APP_MESSAGE_EMBED];
    ...
    const items2 = [tmp2(7292).APP_MESSAGE_EMBED];
