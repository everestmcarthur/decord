...
// Dependencies: [32, 11, 1472, 4626, 2]
...
// Module 4625 (InviteCodeUtils)
import SnowflakeUtilsDefault from "SnowflakeUtils" /* 11 */;
import _modDef1472 from "module_1472" /* 1472 */;
import QueryStringUtils from "QueryStringUtils" /* 4626 */;
...
export const generateInviteKeyFromUrlParams = function generateInviteKeyFromUrlParams(match2, search) {
  if (null == search) {
    ...
    try {
      ...
      let tmp10Result;
      if (null != tmp12) {
        tmp10Result = tmp10(tmp6(4626).getFirstQueryStringValue(parsed[message]));
...
export const parseExtraDataFromInviteKey = function parseExtraDataFromInviteKey(inviteKey) {
  ...
  if (null == tmp3) {
    ...
    let tmp5;
    if (null != tmp4) {
      const firstQueryStringValue2 = tmp12(4626).getFirstQueryStringValue(parsed[message]);
      let tmp8;
