...
// Dependencies: [19, 17, 8507, 21, 4607, 576, 4325, 8686, 8506, 11976, 573, 4574, 11998, 1896, 1114, 11982, 4603, 2]
// Exports: default

// Module 11997 (AppealIngestionCollectSignal)
...
export default function AppealIngestionCollectSignal(isDsaEligible) {
    ...
    if (tmp4) {
      const obj3 = {
        onSave(userInput) {
            ...
            formRow(4574).hideActionSheet("AppealIngestionFreeTextAppealReasonActionSheet");
          },
        onClose() {
            return formRow(4574).hideActionSheet("AppealIngestionFreeTextAppealReasonActionSheet");
          }
      };
      ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(11998, dependencyMap.paths), "AppealIngestionFreeTextAppealReasonActionSheet", obj3);
      const tmpResult = ActionSheetActionCreatorsDefault;
    }
  }
  const tmp = closure_8();
  const formRow = tmp;
  dependencyMap = isDsaEligible(11976).useSafetyHubAppealSignal();
  const intl = isDsaEligible(1114).intl;
  let obj = isDsaEligible(11976);
  const intl2 = isDsaEligible(1114).intl;
  const stringResult = intl.string(isDsaEligible(1114).t["C5q+pW"]);
  let obj2 = { children: null };
  const items = [closure_6(isDsaEligible(11982).AppealIngestionModalHeader, { headerText: stringResult, subHeaderText: intl2.string(isDsaEligible(1114).t.VEcRhw) }), ];
  let obj3 = { style: tmp.container, children: null };
  const obj4 = { style: tmp.form, children: null };
  const stringResult1 = intl2.string(isDsaEligible(1114).t.VEcRhw);
  const items1 = [closure_6(isDsaEligible(8686).FormSection, { sectionBodyStyle: tmp.formSection, accessibilityRole: "radiogroup", children: closure_5.map((signal, index) => timestampProducer(AppealSignalRadioRow, { signal, selected: signal === closure_2, rowStyle: formRow.formRow, onSelect: handleAppealSignalSelect }, "formrow-" + index)) }), ];
  ...
  obj6.children = closure_6(isDsaEligible(4603).Text, obj7);
  items1[1] = closure_6(handleAppealSignalSelect, obj6);
  obj4.children = items1;
  obj3.children = closure_7(isDsaEligible(8686).Form, obj4);
  items[1] = closure_6(handleAppealSignalSelect, obj3);
  obj2.children = items;
  return closure_7(isDsaEligible(11982).AppealIngestionModalScreen, obj2);
};
