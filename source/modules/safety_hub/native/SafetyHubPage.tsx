...
// Function ID: 14866
// Name: SafetyHubPage
// Dependencies: [19, 17, 8551, 8540, 1074, 21, 6698, 12018, 8713, 1177, 5059, 1115, 504, 2979, 4635, 14866, 1380, 14867, 4639, 576, 14868, 12045, 12017, 14862, 5076, 12016, 1241, 7707, 7712, 4606, 14869, 1897, 14870, 14872, 2]
// Exports: default

// Module 14865 (SafetyHubPage)
...
import SafetyHubActionCreatorsAll from "SafetyHubActionCreators" /* 12016 */;
import AutomatedUnderageAppealModalActionCreatorsDefault from "AutomatedUnderageAppealModalActionCreators" /* 12018 */;
import useAvailableAgeVerificationMethods from "useAvailableAgeVerificationMethods" /* 14866 */;
import useShouldShowInitialGoogleWalletBanner from "useShouldShowInitialGoogleWalletBanner" /* 14867 */;
...
function AutomatedUnderageAppealStatus() {
  ...
  if (stateFromStores === AgeCheckStatus.NONE) {
    ...
    const obj3 = { messageType: tmp(1177).HelpMessageTypes.SUCCESS, children: null };
    ...
    obj3.children = intl6.format(tmp(1115).t.hyh4ls, obj4);
    tmp10 = closure_1_12(tmp(1177).HelpMessage, obj3);
  } else if (stateFromStores === tmp7.VERIFIED) {
    const obj5 = { messageType: tmp(1177).HelpMessageTypes.SUCCESS, button: null, children: null };
    const obj6 = { variant: "secondary", size: "sm", text: null, onPress: null };
    const intl4 = tmp(1115).intl;
    obj6.text = intl4.string(tmp(1115).t["2jvQ6K"]);
    obj6.onPress = handleLogInClick;
    obj5.button = closure_1_12(tmp(5059).Button, obj6);
    const intl5 = tmp(1115).intl;
    obj5.children = intl5.string(tmp(1115).t["2Qe65J"]);
    tmp10 = closure_1_12(tmp(1177).HelpMessage, obj5);
  } else if (stateFromStores === tmp7.VERIFIED_OTHER_VIOLATIONS_REMAIN) {
    const obj7 = { messageType: tmp(1177).HelpMessageTypes.SUCCESS, children: null };
    const intl3 = tmp(1115).intl;
    obj7.children = intl3.string(tmp(1115).t.Ie7p1Q);
    tmp10 = closure_1_12(tmp(1177).HelpMessage, obj7);
  } else if (stateFromStores === tmp7.ERROR) {
    const obj8 = { messageType: tmp(1177).HelpMessageTypes.ERROR, children: null };
    const intl2 = tmp(1115).intl;
    obj8.children = intl2.string(tmp(1115).t["4sILBU"]);
    tmp10 = closure_1_12(tmp(1177).HelpMessage, obj8);
  } else if (stateFromStores === tmp7.FAILURE) {
    const obj9 = { messageType: tmp(1177).HelpMessageTypes.ERROR, children: null };
    const intl = tmp(1115).intl;
    obj9.children = intl.string(tmp(1115).t["40R63o"]);
    tmp10 = closure_1_12(tmp(1177).HelpMessage, obj9);
  } else {
...
const AgeCheckStatus = fn(8540).AgeCheckStatus;
  ...
  const effect = noop.useEffect(() => {
    if (visible) {
      if (null != safetyHubFetchError) {
        ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(14869, dependencyMap.paths), "SafetyHubErrorActionSheet", {});
      }
    }
