...
// Dependencies: [19, 17, 2025, 8495, 8484, 1074, 21, 4605, 576, 4601, 8485, 1114, 504, 9119, 4299, 9472, 5025, 11951, 11953, 8494, 8478, 1242, 11954, 7653, 7658, 11956, 7175, 11961, 8483, 2]
// Exports: default
...
function ClassificationHeader(classificationTypeText) {
  ...
  obj.children = closure_13(classificationTypeText(4601).Text, { variant: "text-lg/normal", style: tmp.headerText, color: "mobile-text-heading-primary", children: memo });
...
function ClassificationActionsTaken(arg0) {
  ...
  if (0 !== found.length) {
    ...
    const items1 = [map1(tmp(4601).Text, obj3), ];
    ...
    if (null != classificationExpiration) {
      ...
      const items4 = [tmp6(tmp(4601).Text, obj6), ];
      ...
      items4[1] = tmp8(tmp(4601).Text, obj7);
...
function ClassificationPolicyCard(classificationDescription) {
  ...
  obj3.children = closure_13(policyExplainerLink(9472).ShieldIcon, { size: "sm", color: nativeDefault.colors.TEXT_LINK });
  ...
  obj5.children = closure_13(policyExplainerLink(4601).Text, obj6);
...
export default function ConnectedClassificationDetail(classificationId) {
  ...
  if (null == classification) {
    ...
    function onPressLetUsKnow() {
      ...
      AnalyticsUtilsDefault.track(AnalyticEvents.SAFETY_HUB_ACTION, obj3);
      if (hasItem1) {
        tmp(11954).openV2(tmp3, onClose);
        const tmpResult = tmp(11954);
      } else if (hasItem) {
        tmp(11954).open(tmp3, onClose);
        const tmpResult5 = tmp(11954);
      } else if (isAppealEligible) {
        const obj4 = { name: MetricEvents.MetricEvents.APPEAL_INGESTION_VIEW };
        tmp(7653).increment(obj4);
        const tmpResult6 = tmp(7653);
        const obj5 = { classificationId: tmp3 };
        tmp(11956).open(obj5);
        const tmpResult7 = tmp(11956);
      } else {
        tmp(4299).openURL(constants.APPEALS_LINK);
