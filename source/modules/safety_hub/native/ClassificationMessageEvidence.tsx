...
// Dependencies: [32, 19, 17, 1183, 1371, 8495, 8484, 1074, 21, 7984, 4605, 504, 4456, 11963, 11964, 8483, 4755, 8324, 8373, 11965, 8722, 4828, 11, 2]
// Exports: default

// Module 11962 (ClassificationMessageEvidence)
...
export default function ClassificationEvidence(arg0) {
  ...
  if (obj3.useStateFromStores(items1, () => ref(4456).isThemeLight(id.theme))) {
    let assetSource = resolveAssetSource(tmp11(11963));
    let tmp13 = tmp11;
  } else {
    assetSource = resolveAssetSource(tmp11(11964));
    tmp13 = tmp11;
  }
  ...
  memo = obj2.useMemo(() => {
    ...
    return attachments.map((filename) => {
      ...
      obj.filename = str;
      obj.flags = ref(8483).getSpoilerFlagsForAttachment(filename);
      obj.size = 0;
  ...
  if (first3 > 0) {
    ...
    first2 = attachments.filter((filename) => ref(4755).isVideoFile(filename.filename));
    obj5.children = first2.map((uri, index) => {
      closure_0 = uri;
      return closure_1_18(first(8373), {
        source: { uri: uri.url },
        onLoad(arg0) {
          closure_0 = arg0;
          closure_1_4((arg0) => {
