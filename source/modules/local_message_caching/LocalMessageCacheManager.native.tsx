...
// Dependencies: [5, 32, 4223, 502, 1957, 4795, 1074, 14579, 3, 1090, 510, 4166, 4255, 7766, 7470, 573, 11778, 11707, 1896, 1898, 5286, 9973, 2]

// Module 14578 (LocalMessageCacheManager)
...
function createFailedMessage(channel_id) {
  ...
  const tmp3 = file(7766)({ channelId: channel_id, content, tts, state: MessageStates.SEND_FAILED });
  const id = tmp3;
  ({ timestamp: tmp3.timestamp, file } = channel_id);
  file(7470).receiveMessage(channel_id, tmp3, true, { isHydratingExpiredPendingMessage: state === MessageStates.SENDING });
  if (null != file) {
