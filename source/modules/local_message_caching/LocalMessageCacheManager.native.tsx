...
// Dependencies: [5, 32, 4289, 502, 1958, 4861, 1074, 14676, 3, 1091, 510, 4231, 4321, 7872, 7570, 573, 11898, 11826, 1897, 1899, 5360, 10092, 2]

// Module 14675 (LocalMessageCacheManager)
...
function createFailedMessage(channel_id) {
  ...
  const tmp3 = file(7872)({ channelId: channel_id, content, tts, state: MessageStates.SEND_FAILED });
  const id = tmp3;
  ({ timestamp: tmp3.timestamp, file } = channel_id);
  file(7570).receiveMessage(channel_id, tmp3, true, { isHydratingExpiredPendingMessage: state === MessageStates.SENDING });
  if (null != file) {
