...
// Dependencies: [32, 19, 17, 7853, 5511, 5329, 5554, 1074, 5051, 10354, 5052, 10515, 1374, 21, 7112, 10516, 4607, 1115, 576, 12481, 1477, 7055, 5734, 504, 4308, 10210, 12483, 12484, 1935, 5738, 10514, 12485, 7408, 10353, 5568, 10424, 12486, 7595, 1894, 7756, 12086, 1609, 4787, 12487, 12488, 9406, 5074, 12326, 4343, 12489, 7597, 12496, 4603, 1114, 8692, 12497, 12498, 2]

// Module 12480 (AutocompleteWrapper)
import nativeDefault from "native" /* 576 */;
import PlatformUtils from "PlatformUtils" /* 1115 */;
import KeyboardTypes from "KeyboardTypes" /* 1609 */;
import Server from "Server" /* 1894 */;
import AppAnalyticsUtils from "AppAnalyticsUtils" /* 4787 */;
import utils_AutocompleteUtilsDefault from "utils/AutocompleteUtils" /* 5568 */;
...
import autocompleter_AutocompleteUtils from "autocompleter/AutocompleteUtils" /* 10353 */;
import AutocompleteOptions from "AutocompleteOptions" /* 10514 */;
import TimestampSuggestionUtils from "TimestampSuggestionUtils" /* 10516 */;
import application_commands_ApplicationCommandUtils from "application_commands/ApplicationCommandUtils" /* 12326 */;
import Autocomplete from "Autocomplete" /* 12481 */;
import ChannelAutocompleteAnalytics from "ChannelAutocompleteAnalytics" /* 12487 */;
...
const BOOLEAN_CHOICES = fn(5051).BOOLEAN_CHOICES;
const ApplicationCommandsConstants = fn(10354);
...
const AutocompleteTypes = fn(10515).AutocompleteTypes;
...
let closure_33 = createStyles.createStyles((borderRadius, borderWidth, borderTopWidth, marginHorizontal, marginBottom) => {
  const obj = { autocompletePositionRelative: { position: "relative" }, autocompleteWrapper: null, autocompleteContainer: null, autocomplete: null, sectionDivider: null, sectionTitle: null, stickersAutocompleteList: null };
  let str = "absolute";
  if (obj2.isAndroid()) {
    str = "relative";
  }
...
const forwardRefResult = noop.forwardRef((analyticsLocations, ref) => {
          ...
          if (" " === tmp[tmp2 - 1]) {
            ...
            if (null != result) {
              ...
              if ("gameMentionInput" === result1) {
                ...
                if (enabled) {
                  if (null == anchor2) {
                    const tmp33Result2 = tmp33(10353);
  ...
  const memo1 = onChangeAutoCompleteVisibility.useMemo(() => {
    ...
    if (null != text) {
      if (0 !== str.trim().length) {
        if (null != activeOption) {
          let applicationCommandOptionQueryOptions = analyticsLocations(canMentionEveryone[37]).getApplicationCommandOptionQueryOptions(tmp78);
            ...
            if (null != tmp5) {
              ...
              if (null == tmp78.choices) {
                ...
                if (flag) {
                  ...
                  const obj7 = { query: tmp71, autocompleteType: CHOICES, autocompleteSelectionStart: num + num2, queryOptions: null, showOptionValuesPicker: null };
                  const obj8 = { activeCommand: tmp7, optionValues, isActiveApplicationCommand: flag, option: tmp78, choices, channelTypes };
        ...
        if (setting1) {
          if (null != anchor) {
            const obj9 = { query: str.slice(tmp24 + text.length, selectionEnd).toLowerCase(), autocompleteType: selectionEnd.GAME_MENTIONS, autocompleteSelectionStart: tmp24, queryOptions: null };
            const obj10 = {};
        ...
        if (null != anchor2) {
          if (null == tmp7) {
            const obj11 = { query: str.slice(tmp25 + selectionStart.length, selectionEnd), autocompleteType: selectionEnd.TIMESTAMPS, autocompleteSelectionStart: tmp25, queryOptions: null };
            const obj12 = {};
        ...
        while (true) {
          ...
          if (tmp29 === num5) {
            ...
            if (null != query) {
              ...
              let obj13 = { query: tmp52, autocompleteType: tmp35, autocompleteSelectionStart: tmp47, queryOptions: null };
              let obj14 = {};
  ...
  const effect3 = obj5.useEffect(() => {
    ...
    if (null != tmp2) {
      const batchedStoreListener = new analyticsLocations(canMentionEveryone[23]).BatchedStoreListener(tmp2, () => callback(false));
  ...
  const token3 = require("useToken").useToken(tmp4(tmp2[18]).modules.mobile.CHAT_INPUT_FLOATING_ACCESSORY_TOP_BORDER_WIDTH);
  const tmpResult7 = require("useToken");
  const token4 = require("useToken").useToken(tmp4(tmp2[18]).modules.mobile.CHAT_INPUT_FLOATING_ACCESSORY_MARGIN_HORIZONTAL);
    ...
    if (type.type !== constants.EMOJI_PREMIUM_UPSELL) {
      ...
      if (type.type === tmp.TIMESTAMP_MENTION) {
        ...
        if (null != applicationCommandManager) {
          const result = tmp8(5074).formatTimestampMention(type.mention);
          tmp18 = autocompleteResultText;
          tmp19 = tmp13;
          if (null != result) {
  ...
  const callback5 = obj5.useCallback((item) => {
    ...
    if (scaledTextLineHeight.USER === type) {
      ...
      const _HermesInternal = HermesInternal;
      return setting1(channel(canMentionEveryone[19]).Sticker, obj9, "" + item.sticker.id + "-" + first4 === item.sticker.id);
