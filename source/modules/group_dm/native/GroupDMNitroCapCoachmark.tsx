...
// Dependencies: [32, 19, 17, 11735, 1955, 21, 4638, 11733, 11740, 11736, 7498, 1944, 1115, 1177, 10170, 8797, 576, 11732, 11280, 2]
// Exports: default

// Module 13421 (GroupDMNitroCapCoachmark)
...
let closure_6 = fn(11735).MAX_GROUP_DM_NITRO_PARTICIPANTS;
...
export default function GroupDMNitroCapCoachmark(channelId) {
  ...
  const groupDMNitroAudience = channelId(11733).useGroupDMNitroAudience();
  noop = tmp6;
  const obj3 = { audience: groupDMNitroAudience, location: _location, acquisitionStrategy: null };
  let obj = noop;
  const obj2 = channelId(11733);
  obj3.acquisitionStrategy = channelId(11733).GroupDMNitroAcquisitionStrategy.MARKETING;
  const tmp7Result = _location(11740)(obj3);
  closure_5 = tmp7Result;
  let tmp7 = _location(11740);
  channelId(7498);
  if (obj4.useConfig({ location: _location }).enabled) {
    ...
    const memo = obj.useMemo(() => {
      ...
      obj.onButtonPress = function onButtonPress() {
        if (closure_1_4) {
          _location(11732)(channelId, closure_1_1);
        } else {
          closure_1_5();
        }
    ...
    const coachmark = tmp3(11280).useCoachmark(ref, memo);
