...
// Function ID: 13835
// Name: GatewaySocket
// Dependencies: [32, 5, 1346, 1074, 3, 13835, 13837, 1091, 13838, 559, 13839, 13842, 13849, 13851, 13870, 10, 9, 4716, 13847, 1364, 1271, 1241, 38, 504, 7845, 7849, 7852, 7850, 500, 13871, 13872, 13854, 1338, 7798, 7803, 1231, 573, 2]
// Exports: setAccountSwitchUserId

// Module 13834 (GatewaySocket)
...
import GatewayEncodingDefault from "GatewayEncoding" /* 13835 */;
import GatewaySocketOpCodes2 from "GatewaySocketOpCodes" /* 13838 */;
import AltGatewayTrackerDefault from "AltGatewayTracker" /* 13839 */;
import GatewaySocketDispatcherDefault from "GatewaySocketDispatcher" /* 13842 */;
import GatewaySocketAnalytics from "GatewaySocketAnalytics" /* 13847 */;
import ConnectionStateDefault from "ConnectionState" /* 13849 */;
import GatewayCompressionHandler from "GatewayCompressionHandler" /* 13851 */;
...
prototype["_connect"] = function _connect() {
  ...
  if (this.willReconnect()) {
    if (obj.getIsPaused()) {
      ...
      self.connectionState = identify(13849).CONNECTING;
      ...
      closure_129_2 = (byteLength, compressed_byte_size) => {
        ...
        if (op !== GatewaySocketOpCodes2.Opcode.DISPATCH) {
          const _HermesInternal = HermesInternal;
          AppStartPerformanceDefault.mark("\u{1F310}", "GatewaySocket.onMessage " + op + " " + tmp3(13838).Opcode[op]);
        }
        if (DeveloperOptionsStore.isLoggingGatewayEvents) {
          const items = [op];
          if (op === tmp3(13838).Opcode.DISPATCH) {
            items.push(t);
          }
          items.push(d);
        ...
        } else if (tmp3(13838).Opcode.RECONNECT === op) {
          identify._handleReconnect();
        } else if (tmp3(13838).Opcode.INVALID_SESSION === op) {
          const result2 = identify._handleInvalidSession(d);
        } else if (tmp3(13838).Opcode.HEARTBEAT === op) {
          const result3 = identify._handleHeartbeatReceive();
        } else if (tmp3(13838).Opcode.HEARTBEAT_ACK === op) {
          identify._handleHeartbeatAck(d);
        } else if (tmp3(13838).Opcode.DISPATCH === op) {
          let tmp29 = null;
          if (tmp18) {
        ...
        if (0 === v1) {
          if (arg0 === 1) {
            ...
            if (null !== handleIdentifyResult) {
              self.connectionState = tmp2(13849).IDENTIFYING;
              const _Date = Date;
              const timestamp = Date.now();
              closure_128_1 = timestamp;
              self.identifyStartTime = timestamp;
              if (obj10.isCacheEnabled()) {
                let committedVersions = tmp2(7849).getCommittedVersions();
                const obj12 = tmp2(7849);
              } else {
                committedVersions = {};
              }
              ...
              if (obj13.isCacheEnabled()) {
                let committedVersions1 = tmp2(7852).getCommittedVersions();
                const obj15 = tmp2(7852);
              } else {
                committedVersions1 = {};
              }
              items[1] = committedVersions1;
              obj13 = tmp3(7845);
              let canUseGuildVersionsResult = tmp3(7845).isCacheEnabled();
              if (canUseGuildVersionsResult) {
                canUseGuildVersionsResult = tmp2(7850).canUseGuildVersions();
                const obj17 = tmp2(7850);
              }
          ...
          if (arg0 === 1) {
            ...
            closure_128_8 = obj;
            if (closure_129_0.connectionState !== tmp2(13849).IDENTIFYING) {
            ...
            const obj5 = tmp3(13871);
            obj18.useChannelObfuscation = tmp3(13872).isChannelMetadataObfuscationEnabled("GatewaySocket");
            ...
            const obj7 = tmp3(13872);
            closure_129_0.identifyCompressedByteSize = v1(13854).deflate(closure_128_14).length;
            closure_129_0.identifyCount = closure_129_0.identifyCount + num3;
            num3 = closure_129_0;
            closure_129_0.send(tmp3(13838).Opcode.IDENTIFY, closure_128_13, false);
            tmp65 = tmp2(1241);
...
prototype["isConnected"] = function isConnected() {
  ...
  if (!tmp3) {
    tmp3 = self.connectionState === tmp(13849).RESUMING;
  }
  if (!tmp3) {
    tmp3 = self.connectionState === tmp(13849).SESSION_ESTABLISHED;
  }
  return tmp3;
};
