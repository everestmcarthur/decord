...
// Dependencies: [32, 13743, 3, 13744, 13747, 4667, 13748, 13746, 13749, 504, 13750, 2]

// Module 13742 (GatewaySocketDispatcher)
import LoggerDefault from "Logger" /* 3 */;
import TimeUtils from "TimeUtils" /* 4667 */;
import WorkSchedulerTelemetry from "WorkSchedulerTelemetry" /* 13746 */;
import GatewaySocketAnalytics from "GatewaySocketAnalytics" /* 13747 */;
import VoiceServerUpdateImmediateExperiment from "VoiceServerUpdateImmediateExperiment" /* 13748 */;
import ConnectionStateDefault from "ConnectionState" /* 13749 */;
import ActionBatcherDefault from "ActionBatcher" /* 13750 */;
import _slicedToArray from "module_32" /* 32 */;

require = fn;
let closure_4 = fn(13743).DISPATCHER_IDEAL_TIME_LIMIT_MS;
  ...
  if (0 === items.length) {
    ...
    try {
      ...
      if (closure_5.length > 0) {
        ...
        telemetry.measure(tmp21(13746).WorkSchedulerTelemetryMeasurement.COUNT_DISPATCHES_LEFT_AFTER_YIELD, closure_5.length);
        const queue = self.queue;
...
prototype["dispatchOne"] = function dispatchOne(arg0) {
  ...
  if (this.socket.connectionState !== ConnectionStateDefault.RESUMING) {
    ...
    if (self.socket.connectionState === tmp2(13749).RESUMING) {
