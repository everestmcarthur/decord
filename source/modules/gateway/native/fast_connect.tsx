...
// Function ID: 16
// Name: fast_connect
// Dependencies: [16, 17, 499, 3, 500, 1249, 7871, 13771, 13741, 7757, 1364, 13753, 13740, 13737, 10, 9, 2]
...
function createFastConnectSocket() {
  if (null != window.WebSocket) {
    ...
    if (obj.isAndroid()) {
      let supportsZstd = obj4(13753).getConstants().supportsZstd;
      const obj2 = obj4(13753);
    } else {
    ...
    const _Date = Date;
    _require = Date.now();
    const tmp12 = obj4(13737)(combined);
    ...
    if (isNaN(parsed)) {
      ...
      if (supportsZstd) {
        if (isAndroidResult) {
          const result = tmp7(13753).enableZstdStreamSupport(parsed);
          const tmp7Result = tmp7(13753);
        } else {
        ...
        const result2 = tmp7(13753).enableZlibStreamSupport(parsed);
        const tmp7Result3 = tmp7(13753);
      } else {
