...
// Function ID: 16
// Name: fast_connect
// Dependencies: [16, 17, 499, 3, 500, 1250, 7830, 13724, 13694, 7722, 1115, 13706, 13693, 13690, 10, 9, 2]
...
function createFastConnectSocket() {
  if (null != window.WebSocket) {
    ...
    if (obj.isAndroid()) {
      let supportsZstd = obj4(13706).getConstants().supportsZstd;
      const obj2 = obj4(13706);
    } else {
    ...
    const _Date = Date;
    _require = Date.now();
    const tmp12 = obj4(13690)(combined);
    ...
    if (isNaN(parsed)) {
      ...
      if (supportsZstd) {
        if (isAndroidResult) {
          const result = tmp7(13706).enableZstdStreamSupport(parsed);
          const tmp7Result = tmp7(13706);
        } else {
        ...
        const result2 = tmp7(13706).enableZlibStreamSupport(parsed);
        const tmp7Result3 = tmp7(13706);
      } else {
