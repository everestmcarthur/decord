...
// Function ID: 16
// Name: fast_connect
// Dependencies: [16, 17, 499, 3, 500, 1250, 7808, 13743, 13713, 7700, 1115, 13725, 13712, 13709, 10, 9, 2]
...
function createFastConnectSocket() {
  if (null != window.WebSocket) {
    ...
    if (obj.isAndroid()) {
      let supportsZstd = obj4(13725).getConstants().supportsZstd;
      const obj2 = obj4(13725);
    } else {
    ...
    const _Date = Date;
    _require = Date.now();
    const tmp12 = obj4(13709)(combined);
    ...
    if (isNaN(parsed)) {
      ...
      if (supportsZstd) {
        if (isAndroidResult) {
          const result = tmp7(13725).enableZstdStreamSupport(parsed);
          const tmp7Result = tmp7(13725);
        } else {
        ...
        const result2 = tmp7(13725).enableZlibStreamSupport(parsed);
        const tmp7Result3 = tmp7(13725);
      } else {
