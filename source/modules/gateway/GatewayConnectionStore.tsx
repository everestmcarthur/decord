...
// Dependencies: [5, 1221, 502, 5292, 1957, 1908, 4597, 4624, 2011, 5293, 4613, 1074, 1084, 13655, 13704, 3, 510, 7352, 5425, 1115, 13693, 12, 4626, 13672, 504, 573, 2]

// Module 5291 (GatewayConnectionStore)
import LoggerDefault from "Logger" /* 3 */;
import _modDef12 from "module_12" /* 12 */;
import initializeDefault from "initialize" /* 504 */;
import DispatcherDefault from "Dispatcher" /* 573 */;
import PlatformUtils from "PlatformUtils" /* 1115 */;
import GatewaySocketSingleton from "GatewaySocketSingleton" /* 13655 */;
import ConnectionStateDefault from "ConnectionState" /* 13672 */;
import PauseGatewaySocketAll from "PauseGatewaySocket" /* 13693 */;
import dispatchSocketMessageDefault from "dispatchSocketMessage" /* 13704 */;
...
fn(13655).socket.dispatcher.getDispatchHandler = dispatchSocketMessageDefault;
...
const gatewayConnectionStore = new GatewayConnectionStore(DispatcherDefault, {
  ...
  LOGIN_SUCCESS: function handleSessionRefresh() {
    ...
    if (connectResult) {
      const socket3 = tmp(13655).socket;
      socket3.close();
      const socket4 = tmp(13655).socket;
  ...
  VOICE_CHANNEL_SELECT: function handleVoiceChannelSelect(guildId) {
    ...
    if (isIOSResult) {
      if (null == guildId.channelId) {
        ...
        const socket = tmp(13655).socket;
        if (socket.isClosed()) {
          PauseGatewaySocketAll.setIsPaused(false);
          const socket2 = tmp(13655).socket;
          socket2.connect();
        }
      }
    }
      ...
      if (id.getId() !== userId.userId) {
        ...
        if (userId.sessionId === closure_1_21) {
          ...
          if (userId.guildId !== require("GatewaySocketSingleton").localVoiceState.guildId) {
            ...
            const localVoiceState = tmp(13655).localVoiceState;
  ...
  STREAM_WATCH: function handleStreamWatch(arg0) {
    ...
    if (socket.isSessionEstablished()) {
      if (!allowMultiple) {
        ...
        _require = allActiveStreamKeys.find((item) => closure_0(4626).decodeStreamKey(item).ownerId === id.getId());
        ...
        const item = found.forEach((item) => {
          const socket = closure_0(13655).socket;
          if (socket.isSessionEstablished()) {
            const socket2 = closure_0(13655).socket;
