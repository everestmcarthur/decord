...
// Dependencies: [109, 1961, 4506, 1385, 1957, 2021, 1371, 4625, 4269, 4270, 1074, 3, 13701, 13683, 9, 504, 13733, 1223, 13734, 7565, 13738, 573, 1392, 12, 7854, 1882, 1388, 4234, 7933, 7835, 13814, 4930, 7330, 8266, 8282, 4939, 4831, 4767, 1881, 13815, 2]
// Exports: default

// Module 13732 (dispatchSocketMessage)
...
import SubscriptionPlanActionCreatorsAll from "SubscriptionPlanActionCreators" /* 7330 */;
import convertServerThreadMemberDefault from "convertServerThreadMember" /* 7854 */;
...
import ReadyPayloadUtilsAll from "ReadyPayloadUtils" /* 13733 */;
import splitAgeRestrictedActivitiesDefault from "splitAgeRestrictedActivities" /* 13815 */;
...
import ActionBatcher_mod from "ActionBatcher" /* 13701 */;
...
function dispatchGuildMemberAdd(guild_id, author, member) {
  ...
  const result2 = obj5(4234).parseServerMemberGamingLeaderboardData(member_gaming_leaderboard_data);
...
const importDefaultResult4 = new ActionBatcher(fn(13683).socket, (arg0, id) => {
  let tmp = arg0;
  if (arg0 == null) {
...
let ActionBatcher = ActionBatcher_mod;
const importDefaultResult11 = new ActionBatcher(fn(13683).socket, (arg0, guildId) => {
  let tmp = arg0;
  if (null == arg0) {
...
let ActionBatcher = ActionBatcher_mod;
const importDefaultResult21 = new ActionBatcher(fn(13683).socket, (arg0, arg1) => {
  let tmp = arg0;
  if (arg0 == null) {
...
let ActionBatcher = ActionBatcher_mod;
const importDefaultResult31 = new ActionBatcher(fn(13683).socket, (arg0, arg1) => {
  let tmp = arg0;
  if (null == arg0) {
...
let result = definePreloadableDispatch(["INITIAL_GUILD"], (data_mode) => {
  ...
  initialGuild.measure(() => {
    ...
    Emitter.batched(() => {
      const hydrateInitialGuildResult = ReadyPayloadUtilsAll.hydrateInitialGuild(closure_1_0, closure_0(13683).socket.identifyStartTime);
  ...
  readySupplemental.measure(() => {
    ...
    Emitter.batched(() => {
      ...
      const measureResult = hydrateReadySupplemental.measure(() => {
        obj = closure_2_2(13733);
        return obj.hydrateReadySupplementalPayload(obj, closure_2_0(13683).socket.identifyStartTime);
      });
      ...
      const localVoiceState = closure_1_0(13683).localVoiceState;
  ...
  if (user.user.bot) {
    ...
    ready.measure(() => {
      ...
      Emitter.batched(() => {
        ...
        const measureResult = hydrateReady.measure(() => closure_2_2(13733).hydrateReadyPayloadPrioritized(obj4, user(13683).socket.identifyStartTime, closure_1));
        ...
        let result1;
        if (null != prop) {
          result1 = user(13734).b64ToDeclarativeSettingsProto(obj4.notification_settings.declarative_settings_proto);
          let obj2 = user(13734);
        }
        closure_1_1(7565)("AllGatewayConnectionStores", () => obj4(geoRestrictedGuilds[20]));
        ...
        const localPresenceState = user(13683).localPresenceState;
        localPresenceState.update();
        const localVoiceState = user(13683).localVoiceState;
        localVoiceState.update();
      });
    });
  }
...
defineSimpleDispatch(["RESUMED"], () => {
  const localPresenceState = obj(13683).localPresenceState;
  localPresenceState.forceUpdate();
  const localVoiceState = obj(13683).localVoiceState;
...
defineSimpleDispatch(["GUILD_BAN_ADD", "GUILD_BAN_REMOVE", "GUILD_MEMBER_ADD", "GUILD_MEMBER_UPDATE", "GUILD_MEMBER_REMOVE"], (guildId, type) => {
  ...
  const obj3 = obj(1388);
  obj.gamingLeaderboardData = obj(4234).parseServerMemberGamingLeaderboardData(guildId.member_gaming_leaderboard_data);
  const obj4 = obj(4234);
  DispatcherDefault.dispatch(obj).catch((error) => {
    logger.error("dispatchOrResetSocket error during " + obj.type + ":", error);
    const socket = obj4(closure_3[13]).socket;
...
defineSimpleDispatch(["SAVED_MESSAGE_CREATE"], (body) => {
  obj = { type: "SAVED_MESSAGE_CREATE", savedMessage: obj(7933).savedMessageCreateObjectToClient(body) };
  const obj2 = obj(7933);
  DispatcherDefault.dispatch(obj).catch((error) => {
    logger.error("dispatchOrResetSocket error during " + obj.type + ":", error);
    const socket = obj4(closure_3[13]).socket;
...
defineSimpleDispatch(["SAVED_MESSAGE_DELETE"], (channelId) => {
  obj = { type: "SAVED_MESSAGE_DELETE", savedMessageData: obj(7933).savedMessageDeleteObjectToClient(channelId) };
  const obj2 = obj(7933);
  DispatcherDefault.dispatch(obj).catch((error) => {
    logger.error("dispatchOrResetSocket error during " + obj.type + ":", error);
    const socket = obj4(closure_3[13]).socket;
...
defineSimpleDispatch(["MESSAGE_POLL_VOTE_ADD", "MESSAGE_POLL_VOTE_REMOVE"], (channelId, arg1) => {
  ...
  obj = { type: str, channelId: channelId.channel_id, messageId: channelId.message_id, userId: channelId.user_id, emoji: { id: channelId.answer_id, name: "" }, reactionType: obj(7835).ReactionTypes.VOTE };
  DispatcherDefault.dispatch(obj).catch((error) => {
    logger.error("dispatchOrResetSocket error during " + obj.type + ":", error);
    const socket = obj4(closure_3[13]).socket;
    obj = { error, action: obj.type };
...
defineSimpleDispatch(["INTERACTION_MODAL_CREATE"], (id) => {
  obj = { type: "INTERACTION_MODAL_CREATE", id: id.id, channelId: id.channel_id, customId: id.custom_id, application: id.application, title: id.title, components: obj(4831).transformComponents(id.components), nonce: null, resolved: null };
...
defineSimpleDispatch(["NOTIFICATION_SETTINGS_UPDATE"], (flags) => {
  ...
  let result;
  if (null != prop) {
    result = obj2(13734).b64ToDeclarativeSettingsProto(flags.declarative_settings_proto);
    const obj = obj2(13734);
  }
...
defineSimpleDispatch(["GUILD_ROOM_CONNECT"], (body, type) => {
  obj = { type, room: obj(4767).serverGuildRoomToClient(body) };
  const obj2 = obj(4767);
  DispatcherDefault.dispatch(obj).catch((error) => {
    logger.error("dispatchOrResetSocket error during " + obj.type + ":", error);
    const socket = obj4(closure_3[13]).socket;
