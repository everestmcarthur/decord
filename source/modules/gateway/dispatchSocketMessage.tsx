...
// Dependencies: [109, 1961, 4474, 1385, 1957, 2021, 1371, 4593, 4236, 4237, 1074, 3, 13673, 13655, 9, 504, 13705, 1223, 13706, 7506, 13710, 573, 1392, 12, 7796, 1882, 1388, 7874, 7777, 13786, 4898, 7271, 8207, 8223, 4907, 4799, 4735, 1881, 13787, 2]
// Exports: default

// Module 13704 (dispatchSocketMessage)
...
import SubscriptionPlanActionCreatorsAll from "SubscriptionPlanActionCreators" /* 7271 */;
import convertServerThreadMemberDefault from "convertServerThreadMember" /* 7796 */;
...
import ReadyPayloadUtilsAll from "ReadyPayloadUtils" /* 13705 */;
import splitAgeRestrictedActivitiesDefault from "splitAgeRestrictedActivities" /* 13787 */;
import _objectWithoutProperties from "_objectWithoutProperties" /* 109 */;
import GuildBoostSlotRecord from "GuildBoostSlotRecord" /* 4474 */;
...
import ActionBatcher_mod from "ActionBatcher" /* 13673 */;
...
function dispatchGuildMemberAdd(guild_id, author, member) {
  ...
  const result = obj4(1882).parseServerUserCollectibles(collectibles);
  const obj = obj4(1882);
  const tmp2 = obj4;
  const result1 = obj4(1388).parseServerDisplayNameStyles(display_name_styles);
  ...
  if (isEqualResult) {
    ...
    isEqualResult = _modDef12.isEqual(collectibles1, tmp14);
  }
  if (isEqualResult) {
...
const importDefaultResult4 = new ActionBatcher(fn(13655).socket, (arg0, id) => {
  let tmp = arg0;
  if (arg0 == null) {
...
let ActionBatcher = ActionBatcher_mod;
const importDefaultResult11 = new ActionBatcher(fn(13655).socket, (arg0, guildId) => {
  let tmp = arg0;
  if (null == arg0) {
...
let ActionBatcher = ActionBatcher_mod;
const importDefaultResult21 = new ActionBatcher(fn(13655).socket, (arg0, arg1) => {
  let tmp = arg0;
  if (arg0 == null) {
...
let ActionBatcher = ActionBatcher_mod;
const importDefaultResult31 = new ActionBatcher(fn(13655).socket, (arg0, arg1) => {
  let tmp = arg0;
  if (null == arg0) {
...
let result = definePreloadableDispatch(["INITIAL_GUILD"], (data_mode) => {
  ...
  initialGuild.measure(() => {
    ...
    Emitter.batched(() => {
      const hydrateInitialGuildResult = ReadyPayloadUtilsAll.hydrateInitialGuild(closure_1_0, closure_0(13655).socket.identifyStartTime);
  ...
  readySupplemental.measure(() => {
    ...
    Emitter.batched(() => {
      ...
      const measureResult = hydrateReadySupplemental.measure(() => {
        obj = closure_2_2(13705);
        return obj.hydrateReadySupplementalPayload(obj, closure_2_0(13655).socket.identifyStartTime);
      });
      ...
      const localVoiceState = closure_1_0(13655).localVoiceState;
  ...
  if (user.user.bot) {
    ...
    ready.measure(() => {
      ...
      Emitter.batched(() => {
        ...
        const measureResult = hydrateReady.measure(() => closure_2_2(13705).hydrateReadyPayloadPrioritized(obj4, user(13655).socket.identifyStartTime, closure_1));
        ...
        let result1;
        if (null != prop) {
          result1 = user(13706).b64ToDeclarativeSettingsProto(obj4.notification_settings.declarative_settings_proto);
          let obj2 = user(13706);
        }
        closure_1_1(7506)("AllGatewayConnectionStores", () => obj4(geoRestrictedGuilds[20]));
        ...
        const localPresenceState = user(13655).localPresenceState;
        localPresenceState.update();
        const localVoiceState = user(13655).localVoiceState;
        localVoiceState.update();
      });
    });
  }
...
defineSimpleDispatch(["RESUMED"], () => {
  const localPresenceState = obj(13655).localPresenceState;
  localPresenceState.forceUpdate();
  const localVoiceState = obj(13655).localVoiceState;
...
defineSimpleDispatch(["SAVED_MESSAGE_CREATE"], (body) => {
  obj = { type: "SAVED_MESSAGE_CREATE", savedMessage: obj(7874).savedMessageCreateObjectToClient(body) };
  const obj2 = obj(7874);
  DispatcherDefault.dispatch(obj).catch((error) => {
    logger.error("dispatchOrResetSocket error during " + obj.type + ":", error);
    const socket = obj4(closure_3[13]).socket;
...
defineSimpleDispatch(["SAVED_MESSAGE_DELETE"], (channelId) => {
  obj = { type: "SAVED_MESSAGE_DELETE", savedMessageData: obj(7874).savedMessageDeleteObjectToClient(channelId) };
  const obj2 = obj(7874);
  DispatcherDefault.dispatch(obj).catch((error) => {
    logger.error("dispatchOrResetSocket error during " + obj.type + ":", error);
    const socket = obj4(closure_3[13]).socket;
...
defineSimpleDispatch(["MESSAGE_POLL_VOTE_ADD", "MESSAGE_POLL_VOTE_REMOVE"], (channelId, arg1) => {
  ...
  obj = { type: str, channelId: channelId.channel_id, messageId: channelId.message_id, userId: channelId.user_id, emoji: { id: channelId.answer_id, name: "" }, reactionType: obj(7777).ReactionTypes.VOTE };
  DispatcherDefault.dispatch(obj).catch((error) => {
    logger.error("dispatchOrResetSocket error during " + obj.type + ":", error);
    const socket = obj4(closure_3[13]).socket;
    obj = { error, action: obj.type };
...
defineSimpleDispatch(["INTERACTION_MODAL_CREATE"], (id) => {
  obj = { type: "INTERACTION_MODAL_CREATE", id: id.id, channelId: id.channel_id, customId: id.custom_id, application: id.application, title: id.title, components: obj(4799).transformComponents(id.components), nonce: null, resolved: null };
...
defineSimpleDispatch(["NOTIFICATION_SETTINGS_UPDATE"], (flags) => {
  ...
  let result;
  if (null != prop) {
    result = obj2(13706).b64ToDeclarativeSettingsProto(flags.declarative_settings_proto);
    const obj = obj2(13706);
  }
...
defineSimpleDispatch(["GUILD_ROOM_CONNECT"], (body, type) => {
  obj = { type, room: obj(4735).serverGuildRoomToClient(body) };
  const obj2 = obj(4735);
  DispatcherDefault.dispatch(obj).catch((error) => {
    logger.error("dispatchOrResetSocket error during " + obj.type + ":", error);
    const socket = obj4(closure_3[13]).socket;
