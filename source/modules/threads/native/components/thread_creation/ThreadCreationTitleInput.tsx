...
// Dependencies: [19, 1958, 1074, 21, 17002, 7979, 7470, 1482, 1610, 504, 9543, 1115, 6799, 5756, 2]

// Module 17001 (ThreadCreationTitleInput)
...
export default noop.memo(noop.forwardRef((chatInputRef, ref) => {
  ...
  const callback1 = ref.useCallback(() => {
    if (null != threadSettingsDraft.name) {
      if (null != tmp.parentChannelId) {
        ...
        if (tmp4 !== tmp.name) {
          const obj = { name: tmp4 };
          tmp2(7979).changeThreadSettings(tmp.parentChannelId, obj);
  ...
  const renderErrorResult = chatInputRef(17002).renderError(chatInputRef.threadNameError, { content: threadSettingsDraft.name });
  ...
  if (null != stateFromStores) {
    str = tmp(9543).getDefaultThreadName(stateFromStores, threadSettingsDraft.parentMessageId);
  ...
  const obj4 = { defaultValue: threadSettingsDraft(5756)(ref), errorMessage: renderErrorResult, label: stringResult, accessibilityHint: null, required: null, clearable: true, autoFocus: true, maxLength: null, onSubmitEditing: null, onFocus: null, onBlur: null, onChange: null, placeholder: null, ref: null, returnKeyType: "next", textContentType: "none" };
  let stringResult1;
  if (!optional) {
  ...
  obj4.ref = ref;
  return jsx(chatInputRef(6799).TextInput, { defaultValue: threadSettingsDraft(5756)(ref), errorMessage: renderErrorResult, label: stringResult, accessibilityHint: null, required: null, clearable: true, autoFocus: true, maxLength: null, onSubmitEditing: null, onFocus: null, onBlur: null, onChange: null, placeholder: null, ref: null, returnKeyType: "next", textContentType: "none" });
}));
