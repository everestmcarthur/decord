...
// Dependencies: [19, 1958, 1980, 4655, 4659, 4280, 1074, 21, 10390, 1115, 10391, 10393, 7082, 7224, 10369, 4580, 7885, 10182, 8002, 4602, 4592, 10395, 5182, 10397, 8780, 4582, 11114, 9213, 4607, 10290, 1897, 9927, 11120, 11466, 504, 7383, 12, 8025, 4793, 1936, 11133, 5672, 1177, 7311, 11159, 7313, 10778, 7303, 4337, 2]
// Exports: default

// Module 16275 (ThreadLongPressActionSheet)
...
function ThreadLongPressActionSheetConnected(channel) {
  ...
  const canMarkChannelUnread = channel(10393).useCanMarkChannelUnread(channel);
  const obj4 = channel(10393);
  const canManageThread = channel(7383).useCanManageThread(channel);
  const obj5 = channel(7383);
  const isThreadModerator = channel(7383).useIsThreadModerator(channel);
  const obj6 = channel(7383);
  const canUnarchiveThread = channel(7383).useCanUnarchiveThread(channel);
  const obj7 = channel(7383);
  const canJoinThreadVoice = channel(7383).useCanJoinThreadVoice(channel);
  const obj8 = channel(7383);
  ...
  const tmp14 = onClose(8025)(channel);
  const DeveloperMode = channel(1936).DeveloperMode;
  let setting = DeveloperMode.useSetting();
  const tmp15 = onClose(4793)(channel);
  if (null != stateFromStores) {
    const obj11 = { guild: stateFromStores, size: tmp(5672).GuildIconSizes.LARGE };
    let tmp19 = closure_13(tmp13(5672), obj11);
    let tmp18 = closure_13;
    const tmp13Result = tmp13(5672);
  } else {
    tmp18 = closure_13;
  ...
  if (MarkChannelUnreadExperiment.getConfig({ location: "thread_action_sheet" }).enabled) {
    if (!stateFromStores1) {
      if (canMarkChannelUnread) {
        ...
        obj14.IconComponent = tmp(10391).ChatMarkUnreadIcon;
      ...
      if (hasJoined) {
        ...
        const obj17 = {
          label: string2Result,
          IconComponent: tmp(4580).UserMinusIcon,
          isDestructive: true,
          onPress() {
        ...
        const obj18 = {
          label: stringResult,
          IconComponent: tmp(10182).GroupPlusIcon,
          onPress() {
        ...
        if (null == tmp23) {
          ...
          if (archived) {
            if (canUnarchiveThread) {
              ...
              const obj19 = {
                label: string5Result,
                IconComponent: tmp(4602).ClockIcon,
                onPress() {
            ...
            const obj20 = {
              label: string4Result,
              IconComponent: tmp(4592).XLargeIcon,
              onPress() {
          ...
          if (isThreadModerator) {
            ...
            if (isLockedThreadResult) {
              ...
              obj21.IconComponent = tmp(10395).LockUnlockedIcon;
          ...
          if (!isThreadModerator) {
            ...
            obj22.IconComponent = tmp(4582).LinkIcon;
            ...
            if (isMuted) {
              ...
              obj24.IconComponent = tmp(9927).BellIcon;
              ...
              obj24[1] = tmp(9213).BellSlashIcon;
              ...
              obj25.IconComponent = tmp(11120).ChannelNotificationIcon;
              ...
              obj26.header = tmp18(tmp(11159).ActionSheetIconHeader, obj27);
              const items8 = [
                items5.map((buttons) => {
              ...
              if (setting) {
                ...
                const obj30 = { IconComponent: tmp(10778).IdIcon };
                obj29.icon = tmp18(tmp(7313).ActionSheetRow.Icon, obj30);
                ...
                obj28.children = tmp18(tmp(7313).ActionSheetRow, obj29);
                setting = tmp18(tmp(7313).ActionSheetRow.Group, obj28, "developer-actions");
              }
              items8[1] = setting;
              obj26.children = items8;
              return closure_14(tmp(7311).ActionSheet, obj26);
            }
          } else {
            ...
            const obj31 = {
              label: string7Result,
              IconComponent: tmp(10397).PencilIcon,
              onPress() {
          ...
          const obj32 = { label: string3Result, IconComponent: tmp(8002).PhoneCallIcon, onPress: tmp23 };
  ...
  obj33.IconComponent = tmp(7082).EyeIcon;
