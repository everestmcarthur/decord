...
// Dependencies: [19, 1957, 1979, 4589, 4593, 4214, 1074, 21, 10275, 1114, 10276, 10278, 6984, 7124, 10254, 4515, 7779, 10064, 7894, 4537, 4527, 10280, 5109, 10282, 8658, 4517, 10987, 9091, 4541, 10176, 1896, 9808, 10993, 11344, 504, 7283, 12, 7917, 4727, 1935, 11006, 5598, 1178, 7211, 11030, 7213, 10661, 7203, 4271, 2]
// Exports: default

// Module 16148 (ThreadLongPressActionSheet)
...
function ThreadLongPressActionSheetConnected(channel) {
  ...
  const canMarkChannelUnread = channel(10278).useCanMarkChannelUnread(channel);
  const obj4 = channel(10278);
  const canManageThread = channel(7283).useCanManageThread(channel);
  const obj5 = channel(7283);
  const isThreadModerator = channel(7283).useIsThreadModerator(channel);
  const obj6 = channel(7283);
  const canUnarchiveThread = channel(7283).useCanUnarchiveThread(channel);
  const obj7 = channel(7283);
  const canJoinThreadVoice = channel(7283).useCanJoinThreadVoice(channel);
  const obj8 = channel(7283);
  ...
  const tmp14 = onClose(7917)(channel);
  const DeveloperMode = channel(1935).DeveloperMode;
  let setting = DeveloperMode.useSetting();
  const tmp15 = onClose(4727)(channel, true);
  if (null != stateFromStores) {
    const obj11 = { guild: stateFromStores, size: tmp(5598).GuildIconSizes.LARGE };
    let tmp19 = closure_13(tmp13(5598), obj11);
    let tmp18 = closure_13;
    const tmp13Result = tmp13(5598);
  } else {
    tmp18 = closure_13;
  ...
  if (MarkChannelUnreadExperiment.getConfig({ location: "thread_action_sheet" }).enabled) {
    if (!stateFromStores1) {
      if (canMarkChannelUnread) {
        ...
        obj14.IconComponent = tmp(10276).ChatMarkUnreadIcon;
      ...
      if (hasJoined) {
        ...
        const obj17 = {
          label: string2Result,
          IconComponent: tmp(4515).UserMinusIcon,
          isDestructive: true,
          onPress() {
        ...
        const obj18 = {
          label: stringResult,
          IconComponent: tmp(10064).GroupPlusIcon,
          onPress() {
        ...
        if (null == tmp23) {
          ...
          if (archived) {
            if (canUnarchiveThread) {
              ...
              const obj19 = {
                label: string5Result,
                IconComponent: tmp(4537).ClockIcon,
                onPress() {
            ...
            const obj20 = {
              label: string4Result,
              IconComponent: tmp(4527).XLargeIcon,
              onPress() {
          ...
          if (isThreadModerator) {
            ...
            if (isLockedThreadResult) {
              ...
              obj21.IconComponent = tmp(10280).LockUnlockedIcon;
          ...
          if (!isThreadModerator) {
            ...
            obj22.IconComponent = tmp(4517).LinkIcon;
            ...
            if (isMuted) {
              ...
              obj24.IconComponent = tmp(9808).BellIcon;
              ...
              obj24[1] = tmp(9091).BellSlashIcon;
              ...
              obj25.IconComponent = tmp(10993).ChannelNotificationIcon;
              ...
              obj26.header = tmp18(tmp(11030).ActionSheetIconHeader, obj27);
              const items8 = [
                items5.map((buttons) => {
              ...
              if (setting) {
                ...
                const obj30 = { IconComponent: tmp(10661).IdIcon };
                obj29.icon = tmp18(tmp(7213).ActionSheetRow.Icon, obj30);
                ...
                obj28.children = tmp18(tmp(7213).ActionSheetRow, obj29);
                setting = tmp18(tmp(7213).ActionSheetRow.Group, obj28, "developer-actions");
              }
              items8[1] = setting;
              obj26.children = items8;
              return closure_14(tmp(7211).ActionSheet, obj26);
            }
          } else {
            ...
            const obj31 = {
              label: string7Result,
              IconComponent: tmp(10282).PencilIcon,
              onPress() {
          ...
          const obj32 = { label: string3Result, IconComponent: tmp(7894).PhoneCallIcon, onPress: tmp23 };
  ...
  obj33.IconComponent = tmp(6984).EyeIcon;
