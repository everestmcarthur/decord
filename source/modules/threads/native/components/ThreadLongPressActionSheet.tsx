...
// Dependencies: [19, 1957, 1979, 4622, 4626, 4247, 1074, 21, 10334, 1114, 10335, 10337, 7042, 7184, 10313, 4548, 7838, 10124, 7955, 4570, 4560, 10339, 5147, 10341, 8718, 4550, 11050, 9151, 4574, 10235, 1896, 9869, 11056, 11408, 504, 7343, 12, 7978, 4760, 1935, 11069, 5636, 1178, 7271, 11095, 7273, 10720, 7263, 4304, 2]
// Exports: default

// Module 16186 (ThreadLongPressActionSheet)
...
function ThreadLongPressActionSheetConnected(channel) {
  ...
  const canMarkChannelUnread = channel(10337).useCanMarkChannelUnread(channel);
  const obj4 = channel(10337);
  const canManageThread = channel(7343).useCanManageThread(channel);
  const obj5 = channel(7343);
  const isThreadModerator = channel(7343).useIsThreadModerator(channel);
  const obj6 = channel(7343);
  const canUnarchiveThread = channel(7343).useCanUnarchiveThread(channel);
  const obj7 = channel(7343);
  const canJoinThreadVoice = channel(7343).useCanJoinThreadVoice(channel);
  const obj8 = channel(7343);
  ...
  const tmp14 = onClose(7978)(channel);
  const DeveloperMode = channel(1935).DeveloperMode;
  let setting = DeveloperMode.useSetting();
  const tmp15 = onClose(4760)(channel, true);
  if (null != stateFromStores) {
    const obj11 = { guild: stateFromStores, size: tmp(5636).GuildIconSizes.LARGE };
    let tmp19 = closure_13(tmp13(5636), obj11);
    let tmp18 = closure_13;
    const tmp13Result = tmp13(5636);
  } else {
    tmp18 = closure_13;
    const obj12 = { size: tmp(1178).AvatarSizes.LARGE, channel };
    tmp19 = closure_13(tmp(1178).Avatar, obj12);
  }
  ...
  if (MarkChannelUnreadExperiment.getConfig({ location: "thread_action_sheet" }).enabled) {
    if (!stateFromStores1) {
      if (canMarkChannelUnread) {
        ...
        obj14.IconComponent = tmp(10335).ChatMarkUnreadIcon;
      ...
      if (hasJoined) {
        ...
        const obj17 = {
          label: string2Result,
          IconComponent: tmp(4548).UserMinusIcon,
          isDestructive: true,
          onPress() {
        ...
        const obj18 = {
          label: stringResult,
          IconComponent: tmp(10124).GroupPlusIcon,
          onPress() {
        ...
        if (null == tmp23) {
          ...
          if (archived) {
            if (canUnarchiveThread) {
              ...
              const obj19 = {
                label: string5Result,
                IconComponent: tmp(4570).ClockIcon,
                onPress() {
            ...
            const obj20 = {
              label: string4Result,
              IconComponent: tmp(4560).XLargeIcon,
              onPress() {
          ...
          if (isThreadModerator) {
            ...
            if (isLockedThreadResult) {
              ...
              obj21.IconComponent = tmp(10339).LockUnlockedIcon;
          ...
          if (!isThreadModerator) {
            ...
            obj22.IconComponent = tmp(4550).LinkIcon;
            ...
            if (isMuted) {
              ...
              obj24.IconComponent = tmp(9869).BellIcon;
              ...
              obj24[1] = tmp(9151).BellSlashIcon;
              ...
              obj25.IconComponent = tmp(11056).ChannelNotificationIcon;
              ...
              obj26.header = tmp18(tmp(11095).ActionSheetIconHeader, obj27);
              const items8 = [
                items5.map((buttons) => {
              ...
              if (setting) {
                ...
                const obj30 = { IconComponent: tmp(10720).IdIcon };
                obj29.icon = tmp18(tmp(7273).ActionSheetRow.Icon, obj30);
                ...
                obj28.children = tmp18(tmp(7273).ActionSheetRow, obj29);
                setting = tmp18(tmp(7273).ActionSheetRow.Group, obj28, "developer-actions");
              }
              items8[1] = setting;
              obj26.children = items8;
              return closure_14(tmp(7271).ActionSheet, obj26);
            }
          } else {
            ...
            const obj31 = {
              label: string7Result,
              IconComponent: tmp(10341).PencilIcon,
              onPress() {
          ...
          const obj32 = { label: string3Result, IconComponent: tmp(7955).PhoneCallIcon, onPress: tmp23 };
  ...
  obj33.IconComponent = tmp(7042).EyeIcon;
