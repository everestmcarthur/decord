...
// Dependencies: [19, 1957, 1979, 4620, 4624, 4245, 1074, 21, 10313, 1114, 10314, 10316, 7020, 7162, 10292, 4546, 7816, 10102, 7932, 4568, 4558, 10318, 5145, 10320, 8695, 4548, 11027, 9128, 4572, 10214, 1896, 9846, 11033, 11384, 504, 7321, 12, 7955, 4758, 1935, 11046, 5634, 1178, 7249, 11070, 7251, 10699, 7241, 4302, 2]
// Exports: default

// Module 16197 (ThreadLongPressActionSheet)
...
function ThreadLongPressActionSheetConnected(channel) {
  ...
  const canMarkChannelUnread = channel(10316).useCanMarkChannelUnread(channel);
  const obj4 = channel(10316);
  const canManageThread = channel(7321).useCanManageThread(channel);
  const obj5 = channel(7321);
  const isThreadModerator = channel(7321).useIsThreadModerator(channel);
  const obj6 = channel(7321);
  const canUnarchiveThread = channel(7321).useCanUnarchiveThread(channel);
  const obj7 = channel(7321);
  const canJoinThreadVoice = channel(7321).useCanJoinThreadVoice(channel);
  const obj8 = channel(7321);
  ...
  const tmp14 = onClose(7955)(channel);
  const DeveloperMode = channel(1935).DeveloperMode;
  let setting = DeveloperMode.useSetting();
  const tmp15 = onClose(4758)(channel, true);
  if (null != stateFromStores) {
    const obj11 = { guild: stateFromStores, size: tmp(5634).GuildIconSizes.LARGE };
    let tmp19 = closure_13(tmp13(5634), obj11);
    let tmp18 = closure_13;
    const tmp13Result = tmp13(5634);
  } else {
    tmp18 = closure_13;
  ...
  if (MarkChannelUnreadExperiment.getConfig({ location: "thread_action_sheet" }).enabled) {
    if (!stateFromStores1) {
      if (canMarkChannelUnread) {
        ...
        obj14.IconComponent = tmp(10314).ChatMarkUnreadIcon;
      ...
      if (hasJoined) {
        ...
        const obj17 = {
          label: string2Result,
          IconComponent: tmp(4546).UserMinusIcon,
          isDestructive: true,
          onPress() {
        ...
        const obj18 = {
          label: stringResult,
          IconComponent: tmp(10102).GroupPlusIcon,
          onPress() {
        ...
        if (null == tmp23) {
          ...
          if (archived) {
            if (canUnarchiveThread) {
              ...
              const obj19 = {
                label: string5Result,
                IconComponent: tmp(4568).ClockIcon,
                onPress() {
            ...
            const obj20 = {
              label: string4Result,
              IconComponent: tmp(4558).XLargeIcon,
              onPress() {
          ...
          if (isThreadModerator) {
            ...
            if (isLockedThreadResult) {
              ...
              obj21.IconComponent = tmp(10318).LockUnlockedIcon;
          ...
          if (!isThreadModerator) {
            ...
            obj22.IconComponent = tmp(4548).LinkIcon;
            ...
            if (isMuted) {
              ...
              obj24.IconComponent = tmp(9846).BellIcon;
              ...
              obj24[1] = tmp(9128).BellSlashIcon;
              ...
              obj25.IconComponent = tmp(11033).ChannelNotificationIcon;
              ...
              obj26.header = tmp18(tmp(11070).ActionSheetIconHeader, obj27);
              const items8 = [
                items5.map((buttons) => {
              ...
              if (setting) {
                ...
                const obj30 = { IconComponent: tmp(10699).IdIcon };
                obj29.icon = tmp18(tmp(7251).ActionSheetRow.Icon, obj30);
                ...
                obj28.children = tmp18(tmp(7251).ActionSheetRow, obj29);
                setting = tmp18(tmp(7251).ActionSheetRow.Group, obj28, "developer-actions");
              }
              items8[1] = setting;
              obj26.children = items8;
              return closure_14(tmp(7249).ActionSheet, obj26);
            }
          } else {
            ...
            const obj31 = {
              label: string7Result,
              IconComponent: tmp(10320).PencilIcon,
              onPress() {
          ...
          const obj32 = { label: string3Result, IconComponent: tmp(7932).PhoneCallIcon, onPress: tmp23 };
  ...
  obj33.IconComponent = tmp(7020).EyeIcon;
