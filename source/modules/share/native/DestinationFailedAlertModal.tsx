...
// Dependencies: [19, 17, 1957, 1979, 4647, 4255, 1371, 1085, 21, 4607, 576, 4760, 11004, 1178, 4603, 504, 4451, 11096, 4957, 4957, 1114, 2]
// Exports: default

// Module 11798 (DestinationFailedAlertModal)
...
function FailedUserRow(user) {
  ...
  if (null != user) {
    ...
    items2[1] = closure_10(tmp2(4603).Text, obj5);
...
function FailedChannelRow(channel) {
  ...
  const items1 = [closure_10(channel(11096).GuildIconWithChannelType, { "aria-label": "", guild: stateFromStores, channel, size: channel(11096).GuildIconWithChannelTypeSizes.SMALL_32 }), closure_10(channel(4603).Text, { style: tmp.label, variant: "text-md/medium", lineClamp: 1, ellipsizeMode: "tail", children: tmp3 })];
...
export default function DestinationFailedAlertModal(arg0) {
  ...
  if (null != onRetry) {
    ...
    const items = [tmp2(tmp3(4957).AlertActionButton, obj4, "confirm"), ];
    const obj5 = { variant: "secondary", text: null };
    const intl3 = tmp3(1114).intl;
    obj5.text = intl3.string(tmp3(1114).t.WAI6xu);
    items[1] = tmp2(tmp3(4957).AlertActionButton, obj5, "cancel");
    obj3.children = items;
    ...
    tmp2Result = tmp2(tmp3(4957).AlertActionButton, obj6, "confirm");
  }
