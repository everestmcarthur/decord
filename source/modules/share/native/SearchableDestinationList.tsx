...
// Dependencies: [32, 19, 17, 1074, 10951, 21, 4607, 576, 11080, 7112, 11076, 9928, 7734, 1369, 7123, 10210, 11088, 10957, 11089, 1114, 5177, 7124, 2]
// Exports: default

// Module 11079 (SearchableDestinationList)
...
export default function SearchableDestinationList(getRowIsUnavailable) {
  ...
  const callback5 = obj.useCallback((arg0, arg1) => {
    ...
    if (type !== sortByMatchScore.AutocompleterResultTypes.HEADER) {
      const tmp2Result = tmp2(11076);
      const destinationKeyResult = tmp2Result.destinationKey(tmp2(11076).getDestinationIdFromResult(tmp));
      let tmp6;
      ...
      if (tmp2(9928).AutocompleterResultTypes.USER === type) {
        const element = { type: "user", props: null };
        const obj3 = {};
        const merged1 = Object.assign(obj);
        obj3.user = record;
        obj3.type = tmp2(7734).getRelationshipType(record.id);
        obj3.onPress = callback3;
        element.props = obj3;
        return element;
      } else if (tmp2(9928).AutocompleterResultTypes.GROUP_DM === type) {
        ...
        if (tmp2(9928).AutocompleterResultTypes.TEXT_CHANNEL !== type) {
          if (tmp2(9928).AutocompleterResultTypes.VOICE_CHANNEL !== type) {
