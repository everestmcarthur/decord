...
// Function ID: 11133
// Name: formatResults
// Dependencies: [5, 9974, 1958, 4276, 1372, 11133, 1074, 9975, 4651, 9984, 1115, 1370, 12, 2]
...
const isAllowedType = fn(11133).isAllowedType;
  ...
  if (hasQuery.hasQuery) {
    ...
    if (null != pinnedDestinations) {
      if (pinnedDestinations.length > 0) {
        let mapped = pinnedDestinations.map((type) => {
          if ("user" === type.type) {
            ...
            let tmp6 = null;
            if (null != user) {
              const obj = { type: queryMode(9975).AutocompleterResultTypes.USER, record: user, score: 0 };
              tmp6 = obj;
            }
            let tmp3 = tmp6;
          } else {
            tmp3 = originDestination(9984)(type.id);
          }
          return tmp3;
        });
      }
      ...
      if (channelHistory.length > 0) {
        let mapped1 = channelHistory.map((item) => originDestination(9984)(item));
      } else {
        mapped1 = [];
      }
      if (frequentChannels.length > 0) {
        let mapped2 = frequentChannels.map((id) => originDestination(9984)(id.id));
      } else {
        mapped2 = [];
      }
      const items = [];
      ...
      if (null == targetDestination) {
        ...
        if (null != user) {
          let obj = { type: queryMode(9975).AutocompleterResultTypes.USER, record: user, score: 0 };
          targetDestination = obj;
        }
      } else {
        const tmp9 = originDestination(9984)(targetDestination.id);
      }
    }
    mapped = [];
  }
...
export const formatResultsWithHeaders = function formatResultsWithHeaders(hasNonEmptyQuery) {
  ...
  const headerResult = queryMode(9975).createHeaderResult(intl.string(queryMode(1115).t.qm9dSj));
  if (hasNonEmptyQuery.hasNonEmptyQuery) {
    ...
    if (null != queryMode) {
      ...
      items4 = [tmp(9975).createHeaderResult(intl2.string(tmp(1115).t["80lOZ1"])), , ];
      let tmp13 = null;
      if (null != selectedChannelId) {
        ...
        if (null != tmp7) {
          ...
          if (isAllowedType(tmp7)) {
            let canResult = tmp7.type === tmp(9975).AutocompleterResultTypes.USER;
            if (!canResult) {
