...
// Dependencies: [19, 17, 7289, 4587, 1085, 21, 4605, 576, 5491, 504, 13432, 13480, 13481, 13468, 1114, 13482, 5735, 5738, 13376, 7286, 1115, 2]
// Exports: default

// Module 13479 (BuyNitroPlanSelector)
...
obj2.hrkWarning = { borderRadius: nativeDefault.modules.mobile.TABLE_ROW_BORDER_RADIUS, backgroundColor: fn(5491).DARK_PRIMARY_630_LIGHT_PRIMARY_230, marginTop: nativeDefault.space.PX_8, overflow: "hidden" };
...
export default function BuyNitroPlanSelector(planSelection) {
  ...
  const isBuyNitroPurchaseBlocked = planSelection(13432).useIsBuyNitroPurchaseBlocked();
  const obj2 = planSelection(13432);
  closure_4 = planSelection(13480).useBuyNitroPlanLabelRenderer();
  const obj3 = planSelection(13480);
  closure_5 = planSelection(13481).useBuyNitroPlanSubLabelRenderer();
  const obj5 = { style: tmp.header, children: null };
  const obj6 = { title: null };
  const obj4 = planSelection(13481);
  const tmp6 = closure_8;
  const tmp9 = selection;
  const intl = planSelection(1114).intl;
  obj6.title = intl.string(planSelection(1114).t.u95Dt4);
  const items2 = [closure_7(selection(13468), obj6), closure_7(selection(13482), { selectedTier: selection.tier, setSelectedTier: selection.setTier, disabled: isBuyNitroPurchaseBlocked })];
  ...
  if (tmp8Result) {
    ...
    tmp8Result = tmp8(tmp2(5735).TableRadioGroup, obj9);
  }
