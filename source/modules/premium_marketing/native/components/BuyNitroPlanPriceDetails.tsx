...
// Dependencies: [19, 17, 7289, 1373, 1085, 21, 4605, 576, 504, 7286, 4601, 1114, 2]
// Exports: default

// Module 13462 (BuyNitroPlanPriceDetails)
...
export default function BuyNitroPlanPriceDetails(centered) {
  ...
  if (null != trialTier) {
    if (item.premiumTier === trialTier) {
      ...
      const obj4 = { price: tmp2(7286).formatPrice(0, stateFromStores, { minimumFractionDigits: 0, maximumFractionDigits: 0 }) };
      obj3.children = intl3.formatToPlainString(tmp2(1114).t.hXcaLT, obj4);
      const items1 = [closure_7(tmp2(4601).Text, obj3), ];
      if (null == value) {
        items1[1] = tmp17;
        ...
        tmp16(tmp2(4601).Text, obj5);
      }
      const formatPriceResult = tmp2(7286).formatPrice(0, stateFromStores, { minimumFractionDigits: 0, maximumFractionDigits: 0 });
      tmp14 = closure_8;
      tmp15 = View;
      tmp16 = closure_7;
    }
  ...
  if (item.productId === discounted.id) {
    if (null != discounted.priceString) {
      ...
      const tmp2Result3 = tmp2(7286);
      if (null != value) {
        formatRateResult1 = tmp2(7286).formatRate(value, item.interval, num);
        const tmp2Result4 = tmp2(7286);
      }
      const obj7 = { style: tmp.container, children: null };
      const obj8 = { variant: "text-xs/semibold", color: "text-default", style: centeredText, children: null };
      const intl = tmp2(1114).intl;
      const obj9 = { discountedPrice: tmp2(7286).formatRate(discounted.priceString, item.interval, num), numMonths: discounted.numMonths };
      obj8.children = intl.formatToPlainString(tmp2(1114).t["02Gmgm"], obj9);
      const items2 = [closure_7(tmp2(4601).Text, obj8), ];
      ...
      if (tmp11Result) {
        ...
        tmp11Result = closure_7(tmp2(4601).Text, obj10);
      }
