...
// Function ID: 14525
// Name: channels
// Dependencies: [1961, 1957, 1979, 4245, 2011, 4625, 4511, 1074, 8425, 9645, 9650, 12, 14521, 9648, 14515, 5462, 4751, 1100, 8464, 2]

// Module 14524 (channels)
...
import Constants_mod from "Constants" /* 4511 */;
  ...
  if (null == channel) {
    ...
    if (channel.isPrivate()) {
      ...
      if (!scopes.includes(OAuth2Scopes.OAuth2Scopes.RPC)) {
        if (!scopes.includes(tmp(8425).OAuth2Scopes.DM_CHANNELS_READ)) {
...
obj[RPCCommands.SELECT_TEXT_CHANNEL] = {
  ...
  handler(args) {
    ...
    if (channel_id) {
      const storeWaitResult = server.storeWait(socket, () => ChannelStore.getChannel(channel_id), num);
      const catchPromise = server.storeWait(socket, () => ChannelStore.getChannel(channel_id), num).catch(() => {
        throw new channel_id(9645)({ errorCode: constants2.SELECT_CHANNEL_TIMED_OUT }, "Request to select text channel timed out.");
      });
      let nextPromise1 = server.storeWait(socket, () => ChannelStore.getChannel(channel_id), num).catch(() => {
        ...
        if (tmp2.guild_id) {
          if (!PermissionStore.can(constants.VIEW_CHANNEL, tmp)) {
            const obj = { errorCode: constants2.INVALID_CHANNEL };
            const tmp11 = new channel_id(9645)(obj, "No permission to see channel");
            throw tmp11;
          }
        }
        if (tmp2.guild_id) {
          ...
          const privateChannel = channel_id(5462).selectPrivateChannel(tmp.id);
          const obj2 = channel_id(5462);
        }
        return tmp2;
      });
