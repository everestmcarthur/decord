...
// Function ID: 14529
// Name: invites
// Dependencies: [5, 1956, 1957, 4511, 1074, 1085, 14529, 8425, 9646, 9616, 14533, 1094, 9595, 2]

// Module 14528 (invites)
...
let obj = {};
const CONTEXT_MENU_ICON_NAMES = fn(14529);
let obj3 = { scope: null, handler: null };
let obj4 = {};
const items = [fn(8425).OAuth2Scopes.DM_CHANNELS_MESSAGES_WRITE, fn(8425).OAuth2Scopes.ACTIVITIES_INVITES_WRITE];
obj4[fn(4511).RPC_SCOPE_CONFIG.ANY] = items;
obj3.scope = obj4;
        ...
        if (0 === dependencyMap) {
          if (arg0 === 1) {
            ...
            if (null == id) {
              ...
              const rPCError = new tmp3(9646).RPCError(obj4, "No application.");
              throw rPCError;
            } else {
              connectedActivityLocation = connectedActivityLocation.getConnectedActivityLocation();
              if (null == connectedActivityLocation) {
                const obj7 = { errorCode: constants2.NO_ELIGIBLE_ACTIVITY };
                const rPCError1 = new tmp3(9646).RPCError(obj7, "No eligible activity for application. Ensure an activity was set using setActivity.");
                throw rPCError1;
              } else {
                const kind = connectedActivityLocation.kind;
                if (tmp3(9616).EmbeddedActivityLocationKind.GUILD_CHANNEL !== kind) {
                  if (tmp3(9616).EmbeddedActivityLocationKind.GUILD_CHANNEL_MESSAGE !== kind) {
                    if (tmp3(9616).EmbeddedActivityLocationKind.PRIVATE_CHANNEL !== kind) {
                      if (tmp3(9616).EmbeddedActivityLocationKind.PRIVATE_CHANNEL_MESSAGE !== kind) {
                        const obj8 = { errorCode: constants2.NO_ELIGIBLE_ACTIVITY };
                        const rPCError2 = new tmp3(9646).RPCError(obj8, "Unsupported activity location");
                        throw rPCError2;
                      }
                    }
                    ...
                    if (null == channel) {
                      ...
                      const rPCError3 = new tmp3(9646).RPCError(obj9, "Invalid channel");
                      throw rPCError3;
                    } else if (channel.type === tmp3(1094).ChannelTypes.DM) {
                      const obj10 = { errorCode: constants2.INVALID_CHANNEL };
                      const rPCError4 = new tmp3(9646).RPCError(obj10, "Cannot send invite to a DM");
                      throw rPCError4;
                    }
                  }
                  c2 = 1;
                  ...
                  const obj13 = { value: tmp3(9595).sendEmbeddedActivityInviteUser(obj11), done: false };
                  return obj13;
                }
                channel = tmp3(14533).validateOpenInviteDialog(tmp68).channel;
                const obj5 = tmp3(14533);
              }
            }
          }
        } else if (1 === tmp7) {
          ...
          const rPCError5 = new tmp3(9646).RPCError(obj14, "Failed to invite user");
          throw rPCError5;
        } else if (arg0 === 1) {
          connectedActivityLocation = 3;
          throw value;
