...
// Function ID: 14601
// Name: invites
// Dependencies: [5, 1957, 1958, 4544, 1074, 1085, 14601, 8488, 9709, 9679, 14605, 1095, 9658, 2]

// Module 14600 (invites)
...
let obj4 = {};
const items = [fn(8488).OAuth2Scopes.DM_CHANNELS_MESSAGES_WRITE, fn(8488).OAuth2Scopes.ACTIVITIES_INVITES_WRITE];
        ...
        if (0 === dependencyMap) {
          if (arg0 === 1) {
            ...
            if (null == id) {
              ...
              const rPCError = new tmp3(9709).RPCError(obj4, "No application.");
              throw rPCError;
            } else {
              connectedActivityLocation = connectedActivityLocation.getConnectedActivityLocation();
              if (null == connectedActivityLocation) {
                const obj7 = { errorCode: constants2.NO_ELIGIBLE_ACTIVITY };
                const rPCError1 = new tmp3(9709).RPCError(obj7, "No eligible activity for application. Ensure an activity was set using setActivity.");
                throw rPCError1;
              } else {
                const kind = connectedActivityLocation.kind;
                if (tmp3(9679).EmbeddedActivityLocationKind.GUILD_CHANNEL !== kind) {
                  if (tmp3(9679).EmbeddedActivityLocationKind.GUILD_CHANNEL_MESSAGE !== kind) {
                    if (tmp3(9679).EmbeddedActivityLocationKind.PRIVATE_CHANNEL !== kind) {
                      if (tmp3(9679).EmbeddedActivityLocationKind.PRIVATE_CHANNEL_MESSAGE !== kind) {
                        const obj8 = { errorCode: constants2.NO_ELIGIBLE_ACTIVITY };
                        const rPCError2 = new tmp3(9709).RPCError(obj8, "Unsupported activity location");
                        throw rPCError2;
                      }
                    }
                    ...
                    if (null == channel) {
                      ...
                      const rPCError3 = new tmp3(9709).RPCError(obj9, "Invalid channel");
                      throw rPCError3;
                    } else if (channel.type === tmp3(1095).ChannelTypes.DM) {
                      const obj10 = { errorCode: constants2.INVALID_CHANNEL };
                      const rPCError4 = new tmp3(9709).RPCError(obj10, "Cannot send invite to a DM");
                      throw rPCError4;
                    }
                  }
                  c2 = 1;
                  ...
                  const obj13 = { value: tmp3(9658).sendEmbeddedActivityInviteUser(obj11), done: false };
                  return obj13;
                }
                channel = tmp3(14605).validateOpenInviteDialog(tmp68).channel;
                const obj5 = tmp3(14605);
              }
            }
          }
        } else if (1 === tmp7) {
          ...
          const rPCError5 = new tmp3(9709).RPCError(obj14, "Failed to invite user");
          throw rPCError5;
        } else if (arg0 === 1) {
          connectedActivityLocation = 3;
          throw value;
