...
// Dependencies: [19, 17, 9284, 21, 4639, 576, 9290, 4606, 7257, 7256, 7305, 5207, 1115, 4635, 5057, 5179, 9297, 9300, 2]
// Exports: default

// Module 9299 (SecureFramesVerificationBottomSheet)
...
const SecureFramesConstants = fn(9284);
...
export default function SecureFramesVerificationBottomSheet(onShareClick) {
  ...
  const readableSecureFramesFingerprint = onShareClick(9290).useReadableSecureFramesFingerprint({ fingerprintBase64: epochAuthenticator, chunkSize, desiredLength });
  ...
  const obj4 = { title: null, leading: closure_8(onShareClick(7305).ActionSheetCloseButton, { onPress: callback1 }), trailing: null };
  ...
  obj5.children = closure_8(onShareClick(4635).Text, obj6);
  obj4.trailing = closure_8(onShareClick(5207).PressableOpacity, obj5);
  obj3.header = closure_8(onShareClick(7256).BottomSheetTitleHeader, obj4);
  ...
  const obj = onShareClick(9290);
  let obj2 = { fingerprintBase64: epochAuthenticator, chunkSize, desiredLength };
  const tmp2 = onShareClick;
  const tmp8 = closure_9;
  const tmp9 = View;
  obj9.children = closure_8(onShareClick(5179).LockIcon, { style: tmp.icon, color: readableSecureFramesFingerprint(576).colors.TEXT_SUBTLE });
  const items1 = [closure_8(View, obj9), closure_8(onShareClick(4635).Text, { variant: "heading-xl/bold", color: "mobile-text-heading-primary", children: title }), closure_8(onShareClick(4635).Text, { style: tmp.subtitle, variant: "text-md/medium", color: "text-default", children: subtitle })];
  obj8.children = items1;
  const items2 = [closure_9(onShareClick(5057).Stack, obj8), , ];
  ...
  if (tmp7Result) {
    ...
    tmp7Result = tmp7(tmp10(9300), obj13);
  }
  obj12.trailing = tmp7Result;
  obj12.chunks = readableSecureFramesFingerprint;
  obj12.columns = columns;
  items2[1] = closure_8(readableSecureFramesFingerprint(9297), obj12);
  items2[2] = closure_8(tmp2(4635).Text, { style: tmp.footer, variant: "text-xs/normal", color: "text-muted", children: footer });
  obj7.children = items2;
  obj3.children = tmp8(tmp9, obj7);
  return closure_8(onShareClick(7257).BottomSheet, obj3);
};
