...
// Dependencies: [19, 17, 9244, 21, 4607, 576, 9250, 4574, 7224, 7223, 7272, 5175, 1114, 4603, 5025, 5147, 9257, 9260, 2]
// Exports: default

// Module 9259 (SecureFramesVerificationBottomSheet)
...
export default function SecureFramesVerificationBottomSheet(onShareClick) {
  ...
  const readableSecureFramesFingerprint = onShareClick(9250).useReadableSecureFramesFingerprint({ fingerprintBase64: epochAuthenticator, chunkSize, desiredLength });
  ...
  const obj4 = { title: null, leading: closure_8(onShareClick(7272).ActionSheetCloseButton, { onPress: callback1 }), trailing: null };
  ...
  obj5.children = closure_8(onShareClick(4603).Text, obj6);
  obj4.trailing = closure_8(onShareClick(5175).PressableOpacity, obj5);
  obj3.header = closure_8(onShareClick(7223).BottomSheetTitleHeader, obj4);
  ...
  const obj = onShareClick(9250);
  let obj2 = { fingerprintBase64: epochAuthenticator, chunkSize, desiredLength };
  const tmp2 = onShareClick;
  const tmp8 = closure_9;
  const tmp9 = View;
  obj9.children = closure_8(onShareClick(5147).LockIcon, { style: tmp.icon, color: readableSecureFramesFingerprint(576).colors.TEXT_SUBTLE });
  const items1 = [closure_8(View, obj9), closure_8(onShareClick(4603).Text, { variant: "heading-xl/bold", color: "mobile-text-heading-primary", children: title }), closure_8(onShareClick(4603).Text, { style: tmp.subtitle, variant: "text-md/medium", color: "text-default", children: subtitle })];
  obj8.children = items1;
  const items2 = [closure_9(onShareClick(5025).Stack, obj8), , ];
  ...
  if (tmp7Result) {
    ...
    tmp7Result = tmp7(tmp10(9260), obj13);
  }
  obj12.trailing = tmp7Result;
  obj12.chunks = readableSecureFramesFingerprint;
  obj12.columns = columns;
  items2[1] = closure_8(readableSecureFramesFingerprint(9257), obj12);
  items2[2] = closure_8(tmp2(4603).Text, { style: tmp.footer, variant: "text-xs/normal", color: "text-muted", children: footer });
  obj7.children = items2;
  obj3.children = tmp8(tmp9, obj7);
  return closure_8(onShareClick(7224).BottomSheet, obj3);
};
