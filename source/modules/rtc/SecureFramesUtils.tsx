...
// Dependencies: [32, 5, 502, 1909, 4661, 4677, 1372, 9294, 9295, 1074, 2025, 9296, 9304, 9278, 9297, 1115, 4482, 4229, 1091, 38, 206, 1271, 1231, 4982, 4790, 2]
...
import SecureFramesActionCreatorsDefault from "SecureFramesActionCreators" /* 9296 */;
...
export const deleteVerification = function deleteVerification(userId, arg1, isOtherUserKeyPersistent) {
  if (isOtherUserKeyPersistent) {
    ...
    const serializeKeyResult = _mod9278.serializeKey(uint8Array);
...
export const getSecureFramesUserVerifiedTimestamp = function getSecureFramesUserVerifiedTimestamp(timestamp) {
  const tmp3 = _modDef4229(timestamp);
  const diffResult = _modDef4229().diff(tmp3, "s");
  ...
  } else if (diffResult > tmp(1091).Seconds.DAYS_30) {
    const _Math5 = Math;
  ...
  } else if (diffResult > 7 * tmp(1091).Seconds.DAY) {
    const _Math4 = Math;
  ...
  } else if (diffResult > tmp(1091).Seconds.DAY) {
    const _Math3 = Math;
  ...
  } else if (diffResult > tmp(1091).Seconds.HOUR) {
    const _Math2 = Math;
  ...
  } else if (diffResult > tmp(1091).Seconds.MINUTE) {
    const _Math = Math;
