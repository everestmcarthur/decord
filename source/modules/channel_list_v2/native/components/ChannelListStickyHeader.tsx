...
// Dependencies: [19, 17, 1074, 21, 4376, 4640, 576, 16264, 1983, 1115, 16296, 14021, 16294, 5698, 4341, 10382, 4636, 8896, 1177, 7323, 16310, 12449, 16315, 16316, 2]
// Exports: default

// Module 16295 (ChannelListStickyHeader)
import nativeDefault from "native" /* 576 */;
import GuildBadgeV2Default from "GuildBadgeV2" /* 8896 */;
import useIsGameCommunityServerPreviewDefault from "useIsGameCommunityServerPreview" /* 16264 */;
import useStickyServerHeaderSubtitleDefault from "useStickyServerHeaderSubtitle" /* 16294 */;
...
export default function ChannelListStickyHeader(guild) {
  ...
  const iOSPressEffects = guild(5698).useIOSPressEffects(4);
  ({ onPressIn, onPressOut, pressableStyles } = iOSPressEffects);
  const tmp6Result = guild(5698);
  const favoritesAwareGuildName = guild(1983).getFavoritesAwareGuildName(guild);
  const tmp6Result4 = guild(1983);
  const token = guild(4341).useToken(tmp2(576).modules.mobile.CHANNEL_LIST_TITLE_TEXT_STYLE);
  const tmp6Result5 = guild(4341);
  const token1 = guild(4341).useToken(tmp2(576).modules.mobile.CHANNEL_LIST_SUBTITLE_TEXT_STYLE);
  ...
  if (isFavoritesGuildIdResult) {
    ...
    tmp22 = closure_6(tmp6(10382).StarIcon, obj5);
  }
  const items2 = [tmp22, , , ];
  const tmp16 = closure_8;
  const tmp6Result6 = guild(4341);
  items2[1] = closure_6(closure_4, { ref, collapsable: false, style: tmp.flex, children: closure_6(guild(4636).Text, { experimental_useNativeText: true, color: "mobile-text-heading-primary", variant: token, lineClamp: 1, children: favoritesAwareGuildName }) });
  const obj7 = { guild, size: null, style: null };
  const obj6 = { ref, collapsable: false, style: tmp.flex, children: closure_6(guild(4636).Text, { experimental_useNativeText: true, color: "mobile-text-heading-primary", variant: token, lineClamp: 1, children: favoritesAwareGuildName }) };
  ...
  if (flag2) {
    ...
    tmp24Result = tmp24(tmp6(7323).ChevronSmallRightIcon, obj8);
  }
  ...
  if (null != tmp10) {
    ...
    if (tmp10 > 0) {
      ...
      const items4 = [tmp24(tmp6(4636).Text, obj10), , ];
      ...
      items4[2] = tmp24(tmp6(4636).Text, obj13);
  ...
  if (isFavoritesGuildIdResult) {
    tmp24Result5 = tmp24(tmp6(16310).FavoritesGuildHeaderActionButton, {});
  }
  ...
  if (flag) {
    ...
    tmp24Result6 = tmp24(tmp2(12449), obj16);
  }
  ...
  if (tmp24Result7) {
    ...
    obj17.children = tmp24(tmp2(16315), obj18);
  ...
  if (flag3) {
    ...
    tmp24Result8 = tmp24(tmp2(16316), obj20);
  }
