...
// Dependencies: [19, 17, 4245, 4625, 1074, 21, 4606, 576, 1115, 7946, 4541, 4458, 9562, 563, 5482, 5476, 5468, 12387, 4602, 1114, 5026, 5148, 5152, 2]

// Module 12386 (VocalChannelJoinButton)
...
export default noop.memo(function VocalChannelJoinButton(channel) {
  ...
  const tmp = voiceStates(7946)();
  const tmp2 = voiceStates(4541)();
  let joinButtonText = closure_11(tmp, channel(4458).isThemeLight(tmp2));
  const obj = channel(4458);
  const isConnectedToVoiceChannel = channel(9562).useIsConnectedToVoiceChannel(channel);
  const obj2 = channel(9562);
  ...
  const obj4 = channel(5482);
  const obj5 = channel(5468);
  const tmp8 = channel(5468).useStageHasMedia(channel.id) && isGuildStageVoiceResult;
  ...
  const connectedUserLimit = channel(12387).useConnectedUserLimit({ channel, video: tmp12 });
  const tmp3Result3 = channel(12387);
  let connectedUserLimitFormatted = channel(12387).useConnectedUserLimitFormatted({ channel, video: tmp12, userCount: sum });
  ...
  if (isConnectedToVoiceChannel) {
    ...
    obj7.children = tmp17(tmp3(4602).Text, obj8);
    ...
    if (!noIcon) {
      if (!isConnectedToVoiceChannel) {
        ...
        if (isGuildStageVoiceResult) {
          let VoiceNormalIcon = tmp3(5148).StageIcon;
        } else {
          VoiceNormalIcon = tmp3(5152).VoiceNormalIcon;
        }
