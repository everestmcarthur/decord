...
// Dependencies: [19, 17, 4276, 4657, 1074, 21, 4638, 576, 1364, 7988, 4572, 4489, 9614, 563, 5517, 5511, 5503, 12437, 4634, 1115, 5060, 5182, 5186, 2]

// Module 12436 (VocalChannelJoinButton)
...
export default noop.memo(function VocalChannelJoinButton(channel) {
  ...
  const tmp = voiceStates(7988)();
  const tmp2 = voiceStates(4572)();
  let joinButtonText = closure_11(tmp, channel(4489).isThemeLight(tmp2));
  const obj = channel(4489);
  const isConnectedToVoiceChannel = channel(9614).useIsConnectedToVoiceChannel(channel);
  const obj2 = channel(9614);
  ...
  const obj4 = channel(5517);
  const obj5 = channel(5503);
  const tmp8 = channel(5503).useStageHasMedia(channel.id) && isGuildStageVoiceResult;
  ...
  const connectedUserLimit = channel(12437).useConnectedUserLimit({ channel, video: tmp12 });
  const tmp3Result3 = channel(12437);
  let connectedUserLimitFormatted = channel(12437).useConnectedUserLimitFormatted({ channel, video: tmp12, userCount: sum });
  ...
  if (isConnectedToVoiceChannel) {
    ...
    obj7.children = tmp17(tmp3(4634).Text, obj8);
    ...
    if (!noIcon) {
      if (!isConnectedToVoiceChannel) {
        ...
        if (isGuildStageVoiceResult) {
          let VoiceNormalIcon = tmp3(5182).StageIcon;
        } else {
          VoiceNormalIcon = tmp3(5186).VoiceNormalIcon;
        }
