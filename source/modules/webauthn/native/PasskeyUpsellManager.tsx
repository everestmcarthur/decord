...
// Dependencies: [502, 1372, 14754, 1074, 7225, 7056, 4460, 1944, 4497, 6702, 14761, 2]

// Module 14760 (PasskeyUpsellManager)
    ...
    if (MFAUtils.hasWebAuthn) {
      if (AuthenticationStore.getLoginStatus() === LoginStates.NONE) {
        if (obj.attemptedPasswordLogin()) {
          if (!tmpResult.UNSAFE_isDismissibleContentDismissed(tmp(1944).DismissibleContent.PASSWORDLESS_UPSELL)) {
              ...
              if (!tmpResult3.isModalOpen()) {
                ...
                if (tmp6) {
                  if (obj3.hasFetchedCredentials()) {
                    ...
                    c7 = true;
                    const webAuthnCredentials = tmp(6702).fetchWebAuthnCredentials();
                    const tmpResult4 = tmp(6702);
                  }
                }
