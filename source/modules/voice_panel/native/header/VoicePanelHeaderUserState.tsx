...
// Dependencies: [19, 4655, 21, 4376, 9034, 4639, 576, 17255, 17253, 9251, 5674, 12406, 504, 4640, 2]

// Module 17254 (VoicePanelHeaderUserState)
...
function useVoicePanelHeaderUserStateIcons(participant, guildId, userIcons) {
  ...
  while (iter !== undefined) {
    ...
    if (nextResult.type === useVoicePanelCardUserStateIcons.VoicePanelCardUserStateIconType.USER_VIDEO_ICON) {
      ...
      obj2.children = jsx(tmp11(9251).VideoIcon, { style: tmp.floatingIcon, state: null });
      let arr = items.push(jsx(tmp11(9034).BackgroundBlurView, { blurTheme: "dark", style: tmp.floatingIconWrapper, children: null }, "video"));
    }
    if (tmp9.type === tmp11(17255).VoicePanelCardUserStateIconType.MUTE_DEAFEN_ICON) {
      let tmp35 = jsx;
      let tmp36 = jsx;
      ...
      obj4.children = tmp36(tmp11(9251).MuteDeafenIcon, obj5);
      let arr2 = items.push(tmp35(tmp11(9034).BackgroundBlurView, obj4, "mute-deafen"));
    }
    continue;
  }
  if (0 !== items.length) {
...
const jsx = fn(21).jsx;
let closure_6 = ReanimatedRexport.createAnimatedComponent(fn(9034).BackgroundBlurView);
...
export default noop.memo(function VoicePanelHeaderUserState(isHeaderHidden) {
  ...
  const tmp4 = useVoicePanelHeaderUserStateIcons(channelId(17253)(stateFromStores, channelId, guildId), guildId);
  isHeaderHidden(4376);
  const fn = function h() {
    let num = 0;
  ...
  fn.__closure = { withTiming: isHeaderHidden(4640).withTiming, isHeaderHidden, OPACITY_TIMING };
