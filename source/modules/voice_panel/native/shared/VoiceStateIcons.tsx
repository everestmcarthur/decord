...
// Dependencies: [19, 17, 21, 4560, 576, 9125, 9126, 9128, 9130, 9132, 1369, 1178, 8456, 9134, 9135, 2]

// Module 9124 (VideoDisabledSvgIcon)
import ThemesDefault from "Themes" /* 576 */;
import Button from "Button" /* 1178 */;
import inlineStyles from "inlineStyles" /* 8456 */;
import inlineStylesDefault from "inlineStyles" /* 8456 */;
import isStableVoiceStateEqual from "isStableVoiceStateEqual" /* 9125 */;
import registerAssetDefault from "registerAsset" /* 9134 */;
import registerAssetDefault2 from "registerAsset" /* 9135 */;
...
const memoResult = importAllResult.memo((arg0) => {
  ...
  if (isStableVoiceStateEqual.MuteDeafenIconState.DEAFENED_SERVER === state) {
    ...
    items[1] = redTint;
    obj[0] = items;
    return callback(tmp2(9126).HeadphonesDenyIcon, obj);
  } else if (tmp2(9125).MuteDeafenIconState.DEAFENED === state) {
    obj = { style: null, size: "xs" };
    const items1 = [style];
    obj[0] = items1;
    return callback(tmp2(9128).HeadphonesSlashIcon, obj);
  } else if (tmp2(9125).MuteDeafenIconState.MUTED_SERVER === state) {
    const items2 = [style, ];
    let redTint1 = null;
    if (!alwaysWhite) {
    ...
    items2[1] = redTint1;
    obj1[0] = items2;
    return callback(tmp2(9130).MicrophoneDenyIcon, obj1);
  } else if (tmp2(9125).MuteDeafenIconState.MUTED_LOCAL === state) {
    const obj2 = { style: null, size: "xs" };
    const items3 = [style];
    obj2[0] = items3;
    return callback(tmp2(9130).MicrophoneDenyIcon, obj2);
  } else if (tmp2(9125).MuteDeafenIconState.MUTED === state) {
    const obj3 = { style: null, size: "xs" };
    const items4 = [style];
    obj3[0] = items4;
    return callback(tmp2(9132).MicrophoneSlashIcon, obj3);
  } else {
...
const memoResult1 = importAllResult.memo((state) => {
  ...
  } else if (tmp3(9125).VideoIconState.VIDEO_DISABLED_LOCAL === state) {
    obj = {};
    const merged2 = Object.assign(merged);
    return callback(VideoDisabledSvgIcon, obj);
  } else if (tmp3(9125).VideoIconState.VIDEO_ACTIVE === state) {
    obj1 = {};
