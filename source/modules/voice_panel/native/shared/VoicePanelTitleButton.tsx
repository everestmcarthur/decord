...
// Dependencies: [19, 17, 4867, 1963, 1958, 4660, 21, 4639, 9344, 1115, 5060, 7249, 504, 4792, 9263, 9034, 576, 12406, 4791, 17257, 5057, 5117, 17180, 17254, 5121, 8388, 1095, 17253, 17258, 2]

// Module 17256 (VoicePanelTitleButton)
...
import ShieldLockIcon from "ShieldLockIcon" /* 9344 */;
import VoicePanelStateContextDefault from "VoicePanelStateContext" /* 12406 */;
import QuestActivityButtonDefault from "QuestActivityButton" /* 17180 */;
import VoicePanelHeaderUserState from "VoicePanelHeaderUserState" /* 17254 */;
import _modDef17257 from "module_17257" /* 17257 */;
import VoicePanelSettingsActionCreators from "VoicePanelSettingsActionCreators" /* 17258 */;
import noop from "module_19" /* 19 */;
import ApplicationStore from "ApplicationStore" /* 4867 */;
import StageInstanceStore from "StageInstanceStore" /* 1963 */;
import ChannelStore from "ChannelStore" /* 1958 */;

const _modDef5121 = tmp5(5121);
require = fn;
function ChannelButtonIcons() {
  ...
  const items = [React7(ShieldLockIcon.ShieldLockIcon, obj2), React7(BaseTextButton.BaseTextButton.Icon, { source: _modDef7249 })];
  obj.children = items;
...
function ChannelButton(channelId) {
  ...
  const isCallSecureFramesVerified = channelId(9263).useIsCallSecureFramesVerified({ channelId });
...
function ActivityButton(participant) {
  ...
  obj3.text = str;
  obj3.icon = _modDef5117;
  obj3.onPress = participant.onPress;
  const items1 = [closure_9(participant(9034).HeaderButton, obj3), closure_9(QuestActivityButtonDefault, { applicationId: participant.applicationId })];
  obj2.children = items1;
  return closure_10(participant(5057).Stack, obj2);
}
...
export default noop.memo(function VoicePanelTitleButton() {
  const context = noop.useContext(channelId(12406));
  const guildId = context.guildId;
  channelId = context.channelId;
  ({ channelType, focused } = context);
  const derivedStateFromSharedValue = guildId(8388).useDerivedStateFromSharedValue(focused, (id) => {
    id = undefined;
    if (id != null) {
      id = id.id;
    }
  ...
  const tmp3 = channelId(17253)(derivedStateFromSharedValue, channelId, guildId);
