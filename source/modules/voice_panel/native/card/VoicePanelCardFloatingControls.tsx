...
// Dependencies: [19, 17, 4598, 1956, 4628, 4629, 12366, 12364, 12369, 1074, 4627, 21, 4343, 1178, 4606, 576, 12365, 17190, 17235, 10154, 17236, 17188, 1114, 10077, 9210, 9209, 5640, 6681, 5025, 17186, 4748, 4658, 9606, 17117, 17237, 17238, 504, 17220, 7146, 17239, 4317, 17240, 9269, 8769, 5162, 8767, 8777, 9274, 17241, 5013, 7235, 8264, 9221, 4840, 9266, 4602, 9302, 17163, 2]

// Module 17234 (VoicePanelCardFloatingControls)
...
import useShouldOpenGameProfileModal from "useShouldOpenGameProfileModal" /* 8767 */;
...
import useVoicePanelCardUserStateIcons from "useVoicePanelCardUserStateIcons" /* 17188 */;
import _modDef17190 from "module_17190" /* 17190 */;
import getRandomNumberInRangeDefault from "getRandomNumberInRange" /* 17220 */;
...
const useStableParticipant = tmp(17186);
require = fn;
function StreamIcon(voicePlatform) {
  ...
  const animatedStyle = controlsSpecs(4343).useAnimatedStyle(fn);
  let tmp2Result = _modDef17190;
  if (voicePlatform === constants2.XBOX) {
    tmp2Result = tmp2(17235);
  } else if (voicePlatform === tmp6.MOBILE) {
    tmp2Result = tmp2(10154);
  } else if (voicePlatform === tmp6.QUEST) {
    tmp2Result = tmp2(17236);
  }
...
function AnimatedLabelIcon(icon) {
  ...
  if (useVoicePanelCardUserStateIcons.VoicePanelCardUserStateIconType.STREAM_ICON === type) {
  ...
  } else if (tmp2(17188).VoicePanelCardUserStateIconType.SPEAKER_MUTE_ICON === type) {
    ...
    obj3.accessibilityLabel = intl4.string(tmp2(1114).t.Q8Uzof);
    const obj4 = { style: tmp.iconWithoutBackground };
    obj3.children = value2(tmp2(10077).VoiceXIcon, obj4);
    return value2(Pressable, obj3);
  } else if (tmp2(17188).VoicePanelCardUserStateIconType.USER_VIDEO_ICON === type) {
    if (icon.videoIconState === tmp2(9210).VideoIconState.VIDEO_DISABLED_LOCAL_AUTO) {
    ...
    if (null != icon.onPress) {
      ...
      obj5.children = value2(tmp2(9209).VideoIcon, obj6);
      ...
      obj7.children = value2(tmp2(9209).VideoIcon, obj8);
      tmp16 = value2(NativeViewDefault, obj7);
    }
    return tmp16;
  } else if (tmp2(17188).VoicePanelCardUserStateIconType.MUTE_DEAFEN_ICON === type) {
    ...
    obj10.children = value2(tmp2(9209).MuteDeafenIcon, obj11);
    obj9.children = value2(Pressable, obj10);
    return value2(NativeViewDefault, obj9);
  } else if (tmp2(17188).VoicePanelCardUserStateIconType.USER_DISCONNECTED_ICON === type) {
    ...
    obj.children = value2(tmp2(6681).CircleErrorIcon, obj12);
...
const VOICE_PANEL_CARD_INNER_PADDING = fn(12369).VOICE_PANEL_CARD_INNER_PADDING;
...
let closure_27 = noop.memo((participant) => {
  ...
  guildId = noop.useContext(guildId(12365)).guildId;
  ...
  let obj = participant(4343);
  fn.__closure = { controlsHidden, FLOATING_BAR_HEIGHT: v28, VOICE_PANEL_CARD_INNER_PADDING, withSpring: participant(5025).withSpring, MODE_CHANGE_PHYSICS };
  ...
  const callback = noop.useCallback(() => {
    if (obj.isStableStreamParticipant(participant)) {
      ...
      if (null != streamForUser) {
        ...
        tmpResult.stopStream(tmp(4658).encodeStreamKey(streamForUser));
        const tmpResult3 = tmp(4658);
      }
    }
  ...
  let obj2 = { controlsHidden, FLOATING_BAR_HEIGHT: v28, VOICE_PANEL_CARD_INNER_PADDING, withSpring: participant(5025).withSpring, MODE_CHANGE_PHYSICS };
  const tmp3 = closure_16;
  obj3.icon = guildId(17237);
  obj3.onPress = callback;
  obj3.style = animatedStyle;
  obj3.layout = layout;
  const tmp4 = guildId(17117);
  const result = participant(17186).isStableActivityParticipant(participant);
...
let closure_39 = noop.memo((arg0) => {
  ...
  noop = undefined;
  const showGameTag = gameRecord(17240).useConfig({ location: "VoicePanelCardFloatingControls" }).showGameTag;
  const first = gameRecord(9269)(userId, arg0.guildId, showGameTag)[0];
  let tmp5;
  let obj = gameRecord(17240);
  if (showGameTag) {
    let application_id;
    if (first != null) {
  ...
  gameRecord = gameRecord(8769)({ applicationId: tmp5 }).gameRecord;
  const tmp8 = gameRecord(5162)(gameRecord);
  dependencyMap = tmp8;
  ...
  if (showGameTag) {
    if (null != gameRecord) {
      if (!tmp8) {
        ...
        let tmp12 = closure_16(tmp(17241), obj2);
      }
      return tmp12;
    }
  }
  tmp12 = closure_16(tmp(9274), { userId, textColor });
});
