...
// Dependencies: [32, 19, 1957, 9640, 1958, 12424, 12422, 1074, 1921, 21, 4640, 576, 12423, 16937, 504, 17389, 9658, 7282, 9783, 7276, 7296, 9770, 14099, 4376, 9630, 6766, 1478, 9626, 7187, 17390, 9786, 17391, 17392, 17396, 4350, 2]

// Module 17388 (VoicePanelAnimatedActivityCard)
...
function VoicePanelAnimatedActivityCardInner(applicationId) {
  ...
  const context = focused.useContext(sharedVisible(12423));
  ...
  const tmp7 = sharedVisible(16937)();
  closure_10 = tmp7;
  ...
  const activityShelfItemData = applicationId(17389).useActivityShelfItemData(guild_id1, applicationId);
  const items2 = [guild_id];
  const effect = obj.useEffect(() => {
    const shelf = EmbeddedActivitiesActionCreators.fetchShelf({ guildId: guild_id });
  }, items2);
  const items3 = [applicationId];
  application = tmp2(tmp4(7282)(items3), 1)[0];
  const tmp8Result = applicationId(17389);
  ...
  const tmp17 = sharedVisible(9783)();
  ...
  const tmp4Result = sharedVisible(7276);
  let tmp21 = null != tmp17;
  const analyticsContext = applicationId(9770).useAnalyticsContext();
  if (tmp21) {
    let id;
    if (application != null) {
      id = application.id;
  ...
  if (!tmp21) {
    ...
    if (stateFromStores1 != null) {
      ...
      if (participants != null) {
        found = participants.find((item) => applicationId(14099).isActivityParticipantCurrentUserCurrentSession(item));
      }
    }
    tmp21 = null != found;
  }
  ...
  const tmp8Result12 = applicationId(9770);
  let fn = function q() {
  ...
  if (null != stateFromStores1) {
    ...
    embeddedActivityParticipantId = tmp8(9630).getEmbeddedActivityParticipantId({ applicationId: null, instanceId: null });
    const obj4 = { applicationId: null, instanceId: null };
    const tmp8Result14 = tmp8(9630);
  }
  ...
  const memo1 = obj.useMemo(() => {
    const Gesture = applicationId(6766).Gesture;
    return Gesture.Tap();
  }, []);
  ...
  const tmp39 = !sharedVisible(9626)();
  __initData3 = tmp39;
  ...
  const animatedStyle = applicationId(4376).useAnimatedStyle(fe);
  ...
  if (null == stateFromStores) {
    ...
    if (tmp21) {
      ...
      obj11.children = focusedOrientationLockState(tmp4(9786), obj12, tmp3[0]);
      const items13 = [focusedOrientationLockState(tmp4(17390), obj11), ];
      let tmp47Result = null;
      if (stateFromStores2) {
        tmp47Result = tmp47(tmp4(17391), {});
      }
      items13[1] = tmp47Result;
      obj9.children = items13;
      obj8.children = tmp48(tmp4Result3, obj9);
      let tmp47Result2 = tmp47(tmp8(6766).GestureDetector, obj8);
      let tmp44 = tmp47;
      const tmp4Result4 = tmp4(17390);
    } else if (null == activityShelfItemData) {
      const obj13 = { activity: stateFromStores1, application };
      tmp47Result2 = focusedOrientationLockState(tmp4(17392), obj13);
      ...
      obj14.children = focusedOrientationLockState(tmp4(17396), obj15);
      tmp47Result2 = focusedOrientationLockState(tmp8(6766).GestureDetector, obj14);
    }
    const obj16 = { value: tmp4Result(tmp4(7296).ACTIVITY_TILE).analyticsLocations, children: tmp47Result2 };
    return tmp44(tmp8(7276).AnalyticsLocationProvider, obj16);
  }
