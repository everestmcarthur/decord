...
// Dependencies: [19, 1958, 12405, 1074, 21, 4639, 7978, 12406, 8388, 504, 38, 12414, 1110, 11353, 9541, 4376, 12216, 12415, 1115, 2]

// Module 17369 (VoicePanelControlsAppLauncher)
import ComponentDispatchUtils from "ComponentDispatchUtils" /* 1110 */;
import noop from "module_19" /* 19 */;
import ChannelStore from "ChannelStore" /* 1958 */;

require = fn;
const VoicePanelControlsConstants = fn(12405);
...
export default noop.memo(function VoicePanelControlsAppLauncher(gestureSpecs) {
  ...
  const gradientTop = gestureSpecs(7978).useGradientTop();
  const context = noop.useContext(setControlsMode(12406));
  setControlsMode = context.setControlsMode;
  ({ channelId: c2, safeArea, windowDimensions } = context);
  const tmp4 = setControlsMode(8388)(windowDimensions);
  noop = tmp4;
  const rect = setControlsMode(8388)(safeArea);
  const obj = gestureSpecs(7978);
  const items = [rect];
  ...
  const controlsDrawerOpenWidth = gestureSpecs(12414).getControlsDrawerOpenWidth(tmp4.width, rect.left, rect.right);
  const callback = noop.useCallback(() => {
    const ComponentDispatch = ComponentDispatchUtils.ComponentDispatch;
    ComponentDispatch.dispatch(ComponentActions.SELECT_ACTIVITY);
    setControlsMode({ mode: hasOwnProperty.HIDDEN });
  }, items1);
  const obj3 = gestureSpecs(12414);
  const appLauncherChatInputRefDummy = gestureSpecs(11353).useAppLauncherChatInputRefDummy({ noop: true });
  const obj4 = gestureSpecs(11353);
  const ref = noop.useRef(gestureSpecs(11353).AppLauncherKeyboardCloseReason.DISMISSED);
  const ref1 = noop.useRef(undefined);
  const sharedValue = gestureSpecs(4376).useSharedValue(0);
  const obj5 = gestureSpecs(4376);
  const sharedValue1 = gestureSpecs(4376).useSharedValue(0);
  const obj6 = gestureSpecs(4376);
  const fn = function y() {
  ...
  const derivedValue = gestureSpecs(4376).useDerivedValue(fn);
  const obj7 = gestureSpecs(4376);
  class E {
    constructor() {
  ...
  derivedValue1 = gestureSpecs(4376).useDerivedValue(E);
  const obj8 = gestureSpecs(4376);
  class L {
    constructor() {
      if (closure_5.get()) {
        tmp2 = closure_3;
  ...
  items3[1] = closure_7(setControlsMode(12415), obj13);
  obj11.children = items3;
  obj10.children = closure_8(setControlsMode(4376).View, obj11);
