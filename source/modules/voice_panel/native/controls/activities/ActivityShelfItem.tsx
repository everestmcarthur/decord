...
// Dependencies: [19, 1074, 1181, 21, 4639, 576, 4489, 12275, 12191, 5674, 17297, 1879, 9647, 7630, 9784, 8982, 5207, 4350, 17296, 12220, 1177, 17300, 17298, 4791, 12913, 4635, 12280, 1115, 2]
// Exports: default

// Module 17299 (ActivityShelfItem)
...
import useLaunchingActivityButtonStateDefault from "useLaunchingActivityButtonState" /* 12275 */;
import _modDef12913 from "module_12913" /* 12913 */;
import ActivityShelfItemBackgroundDefault from "ActivityShelfItemBackground" /* 17296 */;
import ActivityShelfItemSummaryDefault from "ActivityShelfItemSummary" /* 17297 */;
import useActivityUsersDefault from "useActivityUsers" /* 17298 */;
...
function ActivityActionOverlay(arg0) {
  ...
  if (useActivityShelfItem.ActivityAction.JOIN !== action) {
    if (tmp4(12191).ActivityAction.LEAVE !== action) {
      return null;
    }
  }
  let tmp8 = action === tmp4(12191).ActivityAction.LEAVE;
  if (tmp8) {
    const obj = { style: tmp3.ongoingActivityJoinedContainer };
    tmp8 = timestampProducer(tmp(5674), obj);
  }
  const items = [tmp8, ];
  let id1;
...
function ParticipantsText(arg0) {
  ...
  const items1 = [timestampProducer(native.Icon, { source: _modDef12913, size: native.Icon.Sizes.EXTRA_SMALL, color: "white" }), ];
  ...
  if (action === useActivityShelfItem.ActivityAction.START) {
    ...
    let itemSubtitleForMaxPlayersShort = tmp9(12280).getItemSubtitleForMaxPlayersShort(num2);
    const tmp9Result = tmp9(12280);
  } else {
...
export default function ActivityShelfItem(arg0) {
  ...
  if (!disableBadges) {
    const items = [tmp8(12191).ActivityAction.LEAVE, tmp8(12191).ActivityAction.JOIN];
  ...
  if (tmp11) {
    ...
    tmp15Result = tmp15(tmp3(12220), obj8);
  }
  ...
  if (tmp11) {
    ...
    if (isTestModeForApplication) {
      ...
      const obj10 = { size: tmp8(1177).Icon.Sizes.REFRESH_SMALL_16, source: tmp3(17300), color: tmp.developerIconColor.color };
      obj9.children = tmp15(tmp8(1177).Icon, obj10);
      tmp15Result3 = tmp15(tmp3(5674), obj9);
      const tmp3Result4 = tmp3(5674);
    }
  }
  items3[2] = tmp15Result3;
  obj4.children = items3;
  const items4 = [React6(native2.ThemeContextProvider, obj4), ];
  let tmp15Result4 = activityAction === tmp8(12191).ActivityAction.START;
  if (tmp15Result4) {
