...
// Dependencies: [19, 1074, 1181, 21, 4638, 576, 4487, 12283, 12199, 5675, 17353, 1879, 9658, 7633, 9792, 8993, 5208, 4348, 17352, 12228, 1177, 17356, 17354, 4790, 12921, 4634, 12288, 1115, 2]
// Exports: default

// Module 17355 (ActivityShelfItem)
...
import TestModeUtils from "TestModeUtils" /* 8993 */;
...
import useLaunchingActivityButtonStateDefault from "useLaunchingActivityButtonState" /* 12283 */;
import _modDef12921 from "module_12921" /* 12921 */;
import ActivityShelfItemBackgroundDefault from "ActivityShelfItemBackground" /* 17352 */;
import ActivityShelfItemSummaryDefault from "ActivityShelfItemSummary" /* 17353 */;
import useActivityUsersDefault from "useActivityUsers" /* 17354 */;
...
function ActivityActionOverlay(arg0) {
  ...
  if (useActivityShelfItem.ActivityAction.JOIN !== action) {
    if (tmp4(12199).ActivityAction.LEAVE !== action) {
      return null;
    }
  }
  let tmp8 = action === tmp4(12199).ActivityAction.LEAVE;
  if (tmp8) {
    const obj = { style: tmp3.ongoingActivityJoinedContainer };
    tmp8 = timestampProducer(tmp(5675), obj);
  }
  const items = [tmp8, ];
  let id1;
...
function ParticipantsText(arg0) {
  ...
  const items1 = [timestampProducer(native.Icon, { source: _modDef12921, size: native.Icon.Sizes.EXTRA_SMALL, color: "white" }), ];
  ...
  if (action === useActivityShelfItem.ActivityAction.START) {
    ...
    let itemSubtitleForMaxPlayersShort = tmp9(12288).getItemSubtitleForMaxPlayersShort(num2);
    const tmp9Result = tmp9(12288);
  } else {
...
export default function ActivityShelfItem(arg0) {
  ...
  if (!disableBadges) {
    const items = [tmp8(12199).ActivityAction.LEAVE, tmp8(12199).ActivityAction.JOIN];
  ...
  if (tmp11) {
    ...
    tmp15Result = tmp15(tmp3(12228), obj8);
  }
  ...
  if (tmp11) {
    ...
    if (isTestModeForApplication) {
      ...
      const obj10 = { size: tmp8(1177).Icon.Sizes.REFRESH_SMALL_16, source: tmp3(17356), color: tmp.developerIconColor.color };
      obj9.children = tmp15(tmp8(1177).Icon, obj10);
      tmp15Result3 = tmp15(tmp3(5675), obj9);
      const tmp3Result4 = tmp3(5675);
    }
  }
  items3[2] = tmp15Result3;
  obj4.children = items3;
  const items4 = [React6(native2.ThemeContextProvider, obj4), ];
  let tmp15Result4 = activityAction === tmp8(12199).ActivityAction.START;
  if (tmp15Result4) {
