...
// Dependencies: [32, 19, 17, 12424, 21, 4376, 5677, 1177, 4640, 576, 9847, 17338, 5061, 17334, 6766, 17414, 2]
// Exports: default

// Module 17413 (VoicePanelPIPPushToTalkOverlay)
...
require = fn;
const PUSH_TO_TALK_PIP_PHYSICS = fn(12424).PUSH_TO_TALK_PIP_PHYSICS;
const jsxProd = fn(21);
...
export default function VoicePanelPIPPushToTalkOverlay() {
  pIPState = pIPState(17338).usePIPState();
  const tmp2 = closure_12();
  let obj = pIPState(17338);
  const sharedValue = pIPState(4376).useSharedValue(false);
  ...
  let obj3 = pIPState(4376);
  fn.__closure = { isPushingToTalk, EXPANDED_ICON_SIZE: 48, BASE_ICON_SIZE: 32, withSpring: pIPState(5061).withSpring, PUSH_TO_TALK_PIP_PHYSICS, white: WHITE };
  fn.__workletHash = 3936373516983;
  fn.__initData = __initData;
  const animatedStyle = obj3.useAnimatedStyle(fn);
  let obj4 = { isPushingToTalk, EXPANDED_ICON_SIZE: 48, BASE_ICON_SIZE: 32, withSpring: pIPState(5061).withSpring, PUSH_TO_TALK_PIP_PHYSICS, white: WHITE };
  class P {
    constructor() {
  ...
  let obj5 = pIPState(4376);
  P.__closure = { withSpring: pIPState(5061).withSpring, isPushingToTalk, black: BLACK, white: WHITE, PUSH_TO_TALK_PIP_PHYSICS };
  P.__workletHash = 11469896791985;
  P.__initData = __initData2;
  const animatedStyle1 = obj5.useAnimatedStyle(P);
  const obj6 = { withSpring: pIPState(5061).withSpring, isPushingToTalk, black: BLACK, white: WHITE, PUSH_TO_TALK_PIP_PHYSICS };
  class H {
    constructor() {
      tmp = closure_0;
      tmp2 = closure_2;
  ...
  const obj7 = pIPState(4376);
  H.__closure = { withSpring: pIPState(5061).withSpring, isPushingToTalk, PUSH_TO_TALK_PIP_PHYSICS, getVoicePanelPIPBorderRadius: pIPState(17334).getVoicePanelPIPBorderRadius, pipState: pIPState };
  ...
  const memo = BLACK.useMemo(() => {
    ...
    const fn = function o(arg0, arg1) {
      if (arg1) {
        pIPState(4376).runOnJS(dependencyMap)(false);
        const obj = pIPState(4376);
      }
    };
    ...
    const fn2 = function t() {
      pIPState(4376).runOnJS(dependencyMap)(true);
    };
  ...
  obj12.style = items5;
  const obj8 = { withSpring: pIPState(5061).withSpring, isPushingToTalk, PUSH_TO_TALK_PIP_PHYSICS, getVoicePanelPIPBorderRadius: pIPState(17334).getVoicePanelPIPBorderRadius, pipState: pIPState };
  obj12.children = closure_6(closure_10, { style: animatedStyle1, size: pIPState(1177).Icon.Sizes.SMALL_20, source: isPushingToTalk(17414), disableColor: true });
  obj11.children = closure_6(NativeView, obj12);
  items4[1] = closure_6(pIPState(6766).GestureDetector, obj11);
