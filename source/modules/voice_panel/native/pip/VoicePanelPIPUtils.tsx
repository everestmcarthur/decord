...
// Dependencies: [4576, 4582, 502, 12277, 17091, 4581, 12278, 4974, 9627, 13988, 576, 2]
...
// Module 17090 (PIPReferenceDimensions)
import ThemesDefault from "Themes" /* 576 */;
import CONFIG_NEVER_ANIMATE from "CONFIG_NEVER_ANIMATE" /* 4974 */;
import canRenderParticipantVideo from "canRenderParticipantVideo" /* 9627 */;
  ...
  if (type1 !== ParticipantTypes.ACTIVITY) {
    if (!showSecondaryPIP.showSecondaryPIP) {
      ...
      for (const item10060 of activityParticipants) {
        ...
        if (participants.some((userId) => callback(13988).isActivityParticipantCurrentUserCurrentSession(userId))) {
          let tmp18 = item10060;
  ...
  if (null != participant2) {
    if (!blockList.has(participant2.id)) {
      ...
      if (ParticipantTypes.STREAM === type) {
        ...
        if (participants2.some((userId) => callback(13988).isActivityParticipantCurrentUserCurrentSession(userId))) {
