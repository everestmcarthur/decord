...
// Dependencies: [19, 17, 11521, 21, 4607, 576, 4542, 4458, 13926, 13927, 504, 11885, 4198, 7197, 4603, 1114, 5027, 4810, 2]
// Exports: default

// Module 13925 (BuildOverrideModal)
...
export default function BuildOverrideModal(overrideUrl) {
  ...
  let stateFromStores;
  const tmp = closure_9();
  const tmp4 = stateFromStores(4542)();
  if (obj.isThemeDark(tmp4)) {
    let tmp2Result = tmp2(13926);
  } else {
    tmp2Result = tmp2(13927);
  }
  obj = str(4458);
  ...
  if (override != null) {
    ...
    if (targetBuildOverride != null) {
      const tmp9 = targetBuildOverride[tmp5(undefined, 11885).DEVICE_FIELD];
      if (tmp9 != null) {
        id = tmp9.id;
      }
    }
  ...
  const tmp2Result2 = stateFromStores(4198);
  let expiresAt;
  if (override != null) {
    expiresAt = override.expiresAt;
  }
  const obj4 = stateFromStores(4198)();
  const durationResult = tmp2Result2.duration(stateFromStores(4198)().diff(expiresAt));
  const rect = { top: true, bottom: true, style: tmp.container, children: null };
  const obj2 = { style: tmp.content, children: null };
  const humanizeResult = tmp2Result2.duration(stateFromStores(4198)().diff(expiresAt)).humanize();
  ...
  if (null != id) {
    ...
    items3[1] = tmp14(tmp5(4603).Text, obj8);
    ...
    tmp14Result = tmp14(tmp5(4603).Text, obj10);
  }
  ...
  if (null != id) {
    ...
    obj13.children = tmp14(tmp5(5027).Button, obj14);
    ...
    items5[1] = tmp14(tmp5(5027).Button, obj15);
    ...
    tmp12Result2 = tmp14(tmp5(5027).Button, obj16);
  }
  obj11.children = tmp12Result2;
  items4[1] = closure_6(closure_4, obj11);
  rect.children = items4;
  return closure_8(str(7197).SafeAreaPaddingView, rect);
};
