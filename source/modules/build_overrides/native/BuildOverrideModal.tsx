...
// Dependencies: [19, 17, 11571, 21, 4638, 576, 4572, 4489, 13977, 13978, 504, 11932, 4229, 7233, 4634, 1115, 5060, 4841, 2]
// Exports: default

// Module 13976 (BuildOverrideModal)
...
export default function BuildOverrideModal(overrideUrl) {
  ...
  let stateFromStores;
  const tmp = closure_9();
  const tmp4 = stateFromStores(4572)();
  if (obj.isThemeDark(tmp4)) {
    let tmp2Result = tmp2(13977);
  } else {
    tmp2Result = tmp2(13978);
  }
  obj = str(4489);
  ...
  if (override != null) {
    ...
    if (targetBuildOverride != null) {
      const tmp9 = targetBuildOverride[tmp5(undefined, 11932).DEVICE_FIELD];
      if (tmp9 != null) {
        id = tmp9.id;
      }
    }
  ...
  const tmp2Result2 = stateFromStores(4229);
  let expiresAt;
  if (override != null) {
    expiresAt = override.expiresAt;
  }
  const obj4 = stateFromStores(4229)();
  const durationResult = tmp2Result2.duration(stateFromStores(4229)().diff(expiresAt));
  const rect = { top: true, bottom: true, style: tmp.container, children: null };
  const obj2 = { style: tmp.content, children: null };
  const humanizeResult = tmp2Result2.duration(stateFromStores(4229)().diff(expiresAt)).humanize();
  ...
  if (null != id) {
    ...
    items3[1] = tmp14(tmp5(4634).Text, obj8);
    ...
    tmp14Result = tmp14(tmp5(4634).Text, obj10);
  }
  ...
  if (null != id) {
    ...
    obj13.children = tmp14(tmp5(5060).Button, obj14);
    ...
    items5[1] = tmp14(tmp5(5060).Button, obj15);
    ...
    tmp12Result2 = tmp14(tmp5(5060).Button, obj16);
  }
  obj11.children = tmp12Result2;
  items4[1] = closure_6(closure_4, obj11);
  rect.children = items4;
  return closure_8(str(7233).SafeAreaPaddingView, rect);
};
