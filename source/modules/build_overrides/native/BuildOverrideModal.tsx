...
// Dependencies: [19, 17, 11559, 21, 4636, 576, 4571, 4488, 13959, 13960, 504, 11920, 4228, 7226, 4632, 1114, 5056, 4839, 2]
// Exports: default

// Module 13958 (BuildOverrideModal)
...
export default function BuildOverrideModal(overrideUrl) {
  ...
  let stateFromStores;
  const tmp = closure_9();
  const tmp4 = stateFromStores(4571)();
  if (obj.isThemeDark(tmp4)) {
    let tmp2Result = tmp2(13959);
  } else {
    tmp2Result = tmp2(13960);
  }
  obj = str(4488);
  ...
  if (override != null) {
    ...
    if (targetBuildOverride != null) {
      const tmp9 = targetBuildOverride[tmp5(undefined, 11920).DEVICE_FIELD];
      if (tmp9 != null) {
        id = tmp9.id;
      }
    }
  ...
  const tmp2Result2 = stateFromStores(4228);
  let expiresAt;
  if (override != null) {
    expiresAt = override.expiresAt;
  }
  const obj4 = stateFromStores(4228)();
  const durationResult = tmp2Result2.duration(stateFromStores(4228)().diff(expiresAt));
  const rect = { top: true, bottom: true, style: tmp.container, children: null };
  const obj2 = { style: tmp.content, children: null };
  const humanizeResult = tmp2Result2.duration(stateFromStores(4228)().diff(expiresAt)).humanize();
  ...
  if (null != id) {
    ...
    items3[1] = tmp14(tmp5(4632).Text, obj8);
    ...
    tmp14Result = tmp14(tmp5(4632).Text, obj10);
  }
  ...
  if (null != id) {
    ...
    obj13.children = tmp14(tmp5(5056).Button, obj14);
    ...
    items5[1] = tmp14(tmp5(5056).Button, obj15);
    ...
    tmp12Result2 = tmp14(tmp5(5056).Button, obj16);
  }
  obj11.children = tmp12Result2;
  items4[1] = closure_6(closure_4, obj11);
  rect.children = items4;
  return closure_8(str(7226).SafeAreaPaddingView, rect);
};
