...
// Dependencies: [2021, 1979, 4426, 1371, 1074, 4230, 510, 4449, 7170, 4463, 4462, 1384, 4572, 13843, 1896, 2]
    ...
    applyArgumentsResult.handleNavigationStateChanged = function handleNavigationStateChanged() {
      if ("guilds" === obj.getCurrentNavigationRouteName()) {
        ...
        if (hasItem1) {
          ...
          hasItem1 = tmp(1384).hasFlag(num, GuildMemberFlags.STARTED_ONBOARDING);
        ...
        if (hasItem1) {
          ...
          hasItem1 = !tmp(1384).hasFlag(num2, GuildMemberFlags.COMPLETED_ONBOARDING);
        ...
        if (tmp5) {
          ...
          if (!value) {
            isNewUserResult = tmp(4449).isNewUser(UserStore.getCurrentUser());
            const tmpResult4 = tmp(4449);
          }
          if (isNewUserResult) {
            ActionSheetActionCreatorsDefault.openLazy(tmp(1896)(13843, dependencyMap.paths), "NUFChannelsActionSheet");
...
prototype["_initialize"] = function _initialize() {
  ...
  if (isNewUserResult) {
    const rootNavigationRef = tmp(4463).getRootNavigationRef();
