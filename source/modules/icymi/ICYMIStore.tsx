...
// Function ID: 8466
// Name: ICYMIStore
// Dependencies: [32, 8466, 4553, 7636, 502, 1958, 8475, 1980, 4859, 4276, 4653, 4286, 4819, 8477, 8479, 1074, 8488, 1091, 8480, 8478, 8259, 8264, 8471, 8489, 7451, 4861, 504, 573, 2]

// Module 8465 (ICYMIStore)
...
import ICYMIUnreadStateStore from "ICYMIUnreadStateStore" /* 8479 */;
  ...
  if (null != arg0) {
    ...
    let item = dehydratedItems.forEach((id) => {
      ...
      if (tmp4) {
        let tmp7 = tmp;
        if (!tmp) {
          tmp7 = !tmp2(8480).isItemUnreadInChannel(id.data.channel_id, id.data.message_id);
          const tmp2Result = tmp2(8480);
        }
        tmp6 = tmp7;
      }
      if (tmp6) {
        ...
        if (id.type === tmp2(8478).ICYMIItemTypes.MESSAGE) {
    ...
    if (!ACTIVITY.has(content.content.id)) {
      if (!tmpResult.isEntryExpired(content.content)) {
        ...
        if (null == dependencyMap[content.content.id]) {
          const obj2 = { id: content.content.id, type: tmp(8478).ICYMIItemTypes.ACTIVITY, score: 15, activity: content.content };
          dependencyMap[content.content.id] = obj2;
        }
        const obj3 = { id: content.content.id, type: tmp(8478).ICYMIItemTypes.ACTIVITY, score: 15, data: null };
  ...
  while (iter !== undefined) {
    ...
    if (nextResult.type !== ICYMITypes.ICYMIItemTypes.RECOMMENDED_GUILDS) {
      if (!set.has(tmp2.id)) {
        ...
        if (tmp7) {
          let tmp9 = tmp2.type !== tmp3(8478).ICYMIItemTypes.MESSAGE;
          if (!tmp9) {
            let tmp3Result = tmp3(8480);
...
const ContentInventoryFeedKey = fn(8488).ContentInventoryFeedKey;
...
obj = {
  ...
  LOAD_ICYMI_DEHYDRATED: function handleLoadDehydrated(items) {
    ...
    set1 = new Set(set1(8478).SUPPORTED_ITEM_TYPES);
    ...
    set = new Set(set1(8478).SUPPORTED_ITEM_TYPES);
    [arr9, arr10] = items5;
    ...
    if (c41) {
      if (0 !== c38) {
        if (!isInitialLoad) {
          ...
          const tmp8 = arr11.length > tmp(8478).MIN_ITEMS_FOR_NEW_PILL;
          if (!isReloading) {
            hasNewContent = tmp8;
          }
          if (tmp8) {
            ...
            tmp(8480).hydrateItems(items6, 0, tmp(8478).ICYMI_PAGE_SIZE);
            if (arr9.length + arr10.length === 0) {
              c54 = true;
            }
            const tmpResult = tmp(8480);
          }
          const ICYMIAnalytics = tmp(8489).ICYMIAnalytics;
