...
// Dependencies: [19, 17, 1958, 2022, 1980, 4286, 4819, 1372, 16620, 1074, 21, 576, 16582, 1364, 16583, 8395, 8478, 504, 16626, 16627, 4634, 1115, 1177, 4790, 5606, 8480, 8481, 11062, 11799, 16621, 16623, 11, 5208, 9910, 16629, 2]
// Exports: default

// Module 16625 (ICYMIMessageRow)
...
function ReplyMessageContent(message) {
  ...
  const context = noop.useContext(message(16583).ICYMIContext);
  ...
  const width = obj.useContext(tmp2(16583).ICYMIContext).width;
  let tmp8 = null;
  if (null != stateFromStores) {
    ...
    const items2 = [closure_14(tmp2(4634).Text, obj5), ];
    ...
    obj10.style = obj11;
    obj10.children = tmp2(4790).getName(guild.id, channel.id, stateFromStores);
    const items4 = [closure_14(tmp2(4634).Text, obj10), ];
    ...
    items4[1] = closure_14(tmp2(16583).ICYMIContext.Provider, obj12);
...
export default function MessageRowWrapper(arg0) {
  ...
  gravityMessage = gravityMessage(8480).useGravityMessage(message);
  const obj = gravityMessage(8480);
