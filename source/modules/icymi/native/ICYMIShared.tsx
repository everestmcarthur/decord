...
// Function ID: 16622
// Name: ICYMIShared
// Dependencies: [19, 17, 7416, 1958, 2022, 4276, 1372, 1074, 21, 8478, 9341, 5172, 10270, 16550, 7220, 7357, 7148, 7566, 4568, 16582, 1364, 576, 1177, 8954, 5670, 5067, 5208, 4634, 7749, 16622, 8807, 504, 4790, 8481, 8308, 5179, 8480, 16218, 9925, 1115, 16583, 4572, 4339, 4487, 4374, 4639, 4651, 5156, 7319, 4628, 2]
...
import safeTransitionToDefault from "safeTransitionTo" /* 7357 */;
...
import openDetailsActionSheet from "openDetailsActionSheet" /* 16622 */;
...
export const SimplePost = function SimplePost(arg0) {
  ...
  const tmp = closure_21();
  const tmp2 = token;
  const tmp4 = token(4572)();
  token = highlight(4339).useToken(token(576).colors.MESSAGE_HIGHLIGHT_BACKGROUND_DEFAULT, tmp4);
  let obj = highlight(4339);
  const hexWithOpacityResult = highlight(4487).hexWithOpacity(token(576).unsafe_rawColors.BRAND_360, 0.25);
  dependencyMap = hexWithOpacityResult;
  let obj2 = highlight(4487);
  const sharedValue = highlight(4374).useSharedValue(0);
  let obj3 = highlight(4374);
  const fn = function c() {
  ...
  let obj4 = highlight(4374);
  fn.__closure = { interpolateColor: highlight(4374).interpolateColor, progress: sharedValue, bgColor: token, bgColorHighlighted: hexWithOpacityResult };
  ...
  if (highlight) {
    ...
    const items2 = [tmp12(tmp2(4374).View, obj7), ];
  ...
  if (obj2.useStateFromStores(items2, () => PermissionStore.canWithPartialContext(constants5.VIEW_CHANNEL, { channelId: parentMessage.id }))) {
    if (null != thread) {
      if (null != mostRecentMessage) {
        ...
        items4[1] = closure_18(tmp2(4634).Text, obj5);
        const obj7 = { style: tmp.commentCount, children: null };
        const obj8 = { style: tmp.commentsIcon };
        const items5 = [closure_18(tmp2(5156).ChatIcon, obj8), , ];
        const obj9 = { variant: "text-sm/bold", color: "interactive-text-default", children: str };
        items5[1] = closure_18(tmp2(4634).Text, obj9);
        const obj10 = { style: tmp.chevron, size: "xxs" };
        items5[2] = closure_18(tmp2(7319).ChevronSmallRightIcon, obj10);
        obj7.children = items5;
        items4[2] = closure_19(View, obj7);
        obj3.children = items4;
        return closure_19(tmp2(5208).PressableHighlight, obj3);
      }
    }
    ...
    const items7 = [closure_18(tmp2(4634).Text, obj12), ];
    const obj13 = { style: tmp.commentCount, children: null };
    const obj14 = { style: tmp.commentsIcon };
    const items8 = [closure_18(tmp2(5156).ChatIcon, obj14), ];
    const obj15 = { style: tmp.chevron, size: "xxs" };
    items8[1] = closure_18(tmp2(7319).ChevronSmallRightIcon, obj15);
    obj13.children = items8;
    items7[1] = closure_19(View, obj13);
    obj11.children = items7;
    return closure_19(tmp2(5208).PressableHighlight, obj11);
  } else {
    return null;
  }
