...
function ICYMIGuildEventRow(event) {
  ...
  const locationFromEvent = event(9835).getLocationFromEvent(event);
  let obj = event(9835);
  const eventLocationIconSource = event(9899).getEventLocationIconSource(event, channel, true);
  let obj2 = event(9899);
  const eventLocationIconComponent = event(9899).getEventLocationIconComponent(event, channel, true);
  let obj3 = event(9899);
  const items1 = [event];
  const nextRecurrenceIdInEvent = event(9795).getNextRecurrenceIdInEvent(event);
  ...
  obj7.children = closure_11(event(4632).Text, {
    ...
    children: noop.useMemo(() => {
      if (closure_2) {
        ...
        eventTimeData = tmp(9795).getEventTimeData(c3);
  ...
  items3[1] = closure_11(event(4632).Text, { style: title, variant: "text-lg/semibold", children: event.name });
  ...
  if (tmp19Result) {
    ...
    obj9.children = tmp8(9901).guildEventDetailsParser(event.description, true, obj10);
    tmp19Result = tmp19(tmp8(4632).Text, obj9);
    const tmp8Result = tmp8(9901);
  }
  ...
  items4[1] = closure_11(event(4632).Text, obj15);
  ...
  if (tmp7 == null) {
    ...
    if (null != locationFromEvent) {
      result = tmp8(9901).guildEventLocationParser(locationFromEvent, true);
      const tmp8Result2 = tmp8(9901);
    }
    tmp27 = result;
  }
  items6[1] = closure_11(event(4632).Text, { lineClamp: 2, variant: "text-xs/normal", color: "text-muted", children: tmp27 });
