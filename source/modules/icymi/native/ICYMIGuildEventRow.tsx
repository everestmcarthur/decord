...
// Dependencies: [19, 17, 7640, 1958, 1980, 21, 16612, 576, 9819, 1115, 9816, 4793, 9856, 9920, 8500, 9940, 7456, 9931, 16653, 11, 4636, 9922, 5176, 1177, 504, 2]
// Exports: default

// Module 16676 (ICYMIGuildEventRow)
...
function ICYMIGuildEventRow(event) {
  ...
  const tmp4 = guild(9819)(event, null);
  ...
  const tmp7 = guild(4793)(channel);
  const locationFromEvent = event(9856).getLocationFromEvent(event);
  let obj = event(9856);
  const eventLocationIconSource = event(9920).getEventLocationIconSource(event, channel, true);
  let obj2 = event(9920);
  const eventLocationIconComponent = event(9920).getEventLocationIconComponent(event, channel, true);
  let obj3 = event(9920);
  const items1 = [event];
  const nextRecurrenceIdInEvent = event(9816).getNextRecurrenceIdInEvent(event);
  ...
  const tmp2ResultResult = guild(9931)(guild_id, id, nextRecurrenceIdInEvent);
  ...
  obj7.children = closure_11(event(4636).Text, {
    ...
    children: noop.useMemo(() => {
      if (closure_2) {
        ...
        eventTimeData = tmp(9816).getEventTimeData(c3);
  ...
  items3[1] = closure_11(event(4636).Text, { style: title, variant: "text-lg/semibold", children: event.name });
  ...
  if (tmp19Result) {
    ...
    obj9.children = tmp8(9922).guildEventDetailsParser(event.description, true, obj10);
    tmp19Result = tmp19(tmp8(4636).Text, obj9);
    const tmp8Result = tmp8(9922);
  }
  ...
  const items4 = [closure_11(event(5176).GroupIcon, { size: "xs", style: tmp.eventsChannelIcon }), ];
  ...
  items4[1] = closure_11(event(4636).Text, obj15);
  ...
  if (tmp7 == null) {
    ...
    if (null != locationFromEvent) {
      result = tmp8(9922).guildEventLocationParser(locationFromEvent, true);
      const tmp8Result2 = tmp8(9922);
    }
    tmp27 = result;
  }
  items6[1] = closure_11(event(4636).Text, { lineClamp: 2, variant: "text-xs/normal", color: "text-muted", children: tmp27 });
