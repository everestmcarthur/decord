...
// Dependencies: [19, 17, 7633, 1958, 1980, 21, 16562, 576, 9800, 1115, 9797, 4792, 9837, 9901, 8472, 9921, 7449, 9912, 16603, 11, 4635, 9903, 5173, 1177, 504, 2]
// Exports: default

// Module 16626 (ICYMIGuildEventRow)
...
function ICYMIGuildEventRow(event) {
  ...
  const tmp4 = guild(9800)(event, null);
  ...
  const tmp7 = guild(4792)(channel);
  const locationFromEvent = event(9837).getLocationFromEvent(event);
  let obj = event(9837);
  const eventLocationIconSource = event(9901).getEventLocationIconSource(event, channel, true);
  let obj2 = event(9901);
  const eventLocationIconComponent = event(9901).getEventLocationIconComponent(event, channel, true);
  let obj3 = event(9901);
  const items1 = [event];
  const nextRecurrenceIdInEvent = event(9797).getNextRecurrenceIdInEvent(event);
  ...
  const tmp2ResultResult = guild(9912)(guild_id, id, nextRecurrenceIdInEvent);
  ...
  obj7.children = closure_11(event(4635).Text, {
    ...
    children: noop.useMemo(() => {
      if (closure_2) {
        ...
        eventTimeData = tmp(9797).getEventTimeData(c3);
  ...
  items3[1] = closure_11(event(4635).Text, { style: title, variant: "text-lg/semibold", children: event.name });
  ...
  if (tmp19Result) {
    ...
    obj9.children = tmp8(9903).guildEventDetailsParser(event.description, true, obj10);
    tmp19Result = tmp19(tmp8(4635).Text, obj9);
    const tmp8Result = tmp8(9903);
  }
  ...
  const items4 = [closure_11(event(5173).GroupIcon, { size: "xs", style: tmp.eventsChannelIcon }), ];
  ...
  items4[1] = closure_11(event(4635).Text, obj15);
  ...
  if (tmp7 == null) {
    ...
    if (null != locationFromEvent) {
      result = tmp8(9903).guildEventLocationParser(locationFromEvent, true);
      const tmp8Result2 = tmp8(9903);
    }
    tmp27 = result;
  }
  items6[1] = closure_11(event(4635).Text, { lineClamp: 2, variant: "text-xs/normal", color: "text-muted", children: tmp27 });
