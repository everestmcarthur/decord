...
// Dependencies: [19, 17, 7636, 1958, 1980, 21, 16582, 576, 9808, 1115, 9805, 4791, 9845, 9909, 8481, 9929, 7452, 9920, 16623, 11, 4634, 9911, 5174, 1177, 504, 2]
// Exports: default

// Module 16646 (ICYMIGuildEventRow)
...
function ICYMIGuildEventRow(event) {
  ...
  const tmp4 = guild(9808)(event, null);
  ...
  const tmp7 = guild(4791)(channel);
  const locationFromEvent = event(9845).getLocationFromEvent(event);
  let obj = event(9845);
  const eventLocationIconSource = event(9909).getEventLocationIconSource(event, channel, true);
  let obj2 = event(9909);
  const eventLocationIconComponent = event(9909).getEventLocationIconComponent(event, channel, true);
  let obj3 = event(9909);
  const items1 = [event];
  const nextRecurrenceIdInEvent = event(9805).getNextRecurrenceIdInEvent(event);
  ...
  const tmp2ResultResult = guild(9920)(guild_id, id, nextRecurrenceIdInEvent);
  ...
  obj7.children = closure_11(event(4634).Text, {
    ...
    children: noop.useMemo(() => {
      if (closure_2) {
        ...
        eventTimeData = tmp(9805).getEventTimeData(c3);
  ...
  items3[1] = closure_11(event(4634).Text, { style: title, variant: "text-lg/semibold", children: event.name });
  ...
  if (tmp19Result) {
    ...
    obj9.children = tmp8(9911).guildEventDetailsParser(event.description, true, obj10);
    tmp19Result = tmp19(tmp8(4634).Text, obj9);
  ...
  const items4 = [closure_11(event(5174).GroupIcon, { size: "xs", style: tmp.eventsChannelIcon }), ];
  ...
  items4[1] = closure_11(event(4634).Text, obj15);
  ...
  if (tmp7 == null) {
    ...
    if (null != locationFromEvent) {
      result = tmp8(9911).guildEventLocationParser(locationFromEvent, true);
      const tmp8Result2 = tmp8(9911);
    }
    tmp27 = result;
  }
  items6[1] = closure_11(event(4634).Text, { lineClamp: 2, variant: "text-xs/normal", color: "text-muted", children: tmp27 });
