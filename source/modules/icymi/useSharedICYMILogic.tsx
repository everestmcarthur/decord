...
// Dependencies: [32, 19, 8465, 16581, 16616, 504, 8489, 8487, 9938, 8481, 8480, 8478, 16617, 2]
// Exports: useSharedICYMILogic

// Module 16615 (useSharedICYMILogic)
...
require = fn;
const SCROLL_EVENT_THROTTLE_MS = fn(16581).SCROLL_EVENT_THROTTLE_MS;
const size = fn(2);
  ...
  const callback1 = allUnreadItemsHydrated.useCallback((viewableItems) => {
    ...
    if (0 !== viewableItems.length) {
      ...
      const result = ICYMIAnalytics.trackItemLongImpression(viewableItems, viewableFeedItemsArray.map((id) => {
        const obj = { id: id.id, type: notificationItem(8478).typeToString(id) };
        return obj;
      }), stateFromStores);
      const found = viewableItems.filter((item) => {
        const NON_ELIGIBLE_SCROLL_ITEMS = notificationItem(8487).NON_ELIGIBLE_SCROLL_ITEMS;
        return !NON_ELIGIBLE_SCROLL_ITEMS.has(item.item.data.kind);
      });
      const result1 = ICYMIActionCreatorsDefault.triggerItemsLongImpression(found.map((item) => {
        item = item.item;
        const obj = { itemId: item.id, itemType: notificationItem(8480).itemToType(item), triggerType: "list", itemFeedIndex: item.index, itemScore: null, itemChannelType: null, isInitiallyVisible: false };
