...
function createItem(id, type, unread) {
  ...
  if (ICYMITypes.ICYMIItemTypes.MESSAGE === type) {
    ...
    if (tmp(8466).ICYMIItemTypes.ACTIVITY !== type) {
      if (tmp(8466).ICYMIItemTypes.CUSTOM_STATUS !== type) {
        if (tmp(8466).ICYMIItemTypes.GUILD_EVENT === type) {
          ...
          obj6.unread = unread;
          return obj6;
        } else if (tmp(8466).ICYMIItemTypes.RECOMMENDED_GUILDS === type) {
