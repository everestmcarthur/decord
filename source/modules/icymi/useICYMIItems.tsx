...
// Function ID: 16617
// Name: useICYMIItems
// Dependencies: [19, 8465, 8478, 504, 8481, 2]
// Exports: default

// Module 16616 (useICYMIItems)
...
function createItem(id, type, unread) {
  ...
  if (ICYMITypes.ICYMIItemTypes.MESSAGE === type) {
    ...
    if (tmp(8478).ICYMIItemTypes.ACTIVITY !== type) {
      if (tmp(8478).ICYMIItemTypes.CUSTOM_STATUS !== type) {
        if (tmp(8478).ICYMIItemTypes.GUILD_EVENT === type) {
          ...
          obj6.unread = unread;
          return obj6;
        } else if (tmp(8478).ICYMIItemTypes.RECOMMENDED_GUILDS === type) {
    ...
    if (0 < stateFromStores2) {
      while (true) {
        ...
        if (!stateFromStores3[tmp6.id]) {
          ...
          let tmp10 = null == tmp9;
          if (tmp10) {
            tmp10 = tmp6.type === stateFromStores1(8478).ICYMIItemTypes.MESSAGE;
          }
          if (tmp10) {
    ...
    if (num < stateFromStores2) {
      while (true) {
        ...
        if (!stateFromStores3[tmp19.id]) {
          ...
          if (tmp23) {
            tmp23 = tmp19.type === stateFromStores1(8478).ICYMIItemTypes.MESSAGE;
          }
          if (tmp23) {
