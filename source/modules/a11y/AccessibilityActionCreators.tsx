...
// Dependencies: [4566, 1074, 1938, 1084, 573, 1242, 9390, 2]
...
export const setPrefersReducedMotion = function setPrefersReducedMotion(reduce) {
  ...
  if (!useReducedMotion) {
    if (useReducedMotion2) {
      ...
      const result = tmp(9390).applySettingsOverride(obj3);
      const tmpResult = tmp(9390);
    }
  }
  if (useReducedMotion) {
  ...
  if (useReducedMotion) {
    const result1 = tmp(9390).clearSettingsOverride("gifAutoPlay", "animateEmoji", "animateStickers");
    const tmpResult2 = tmp(9390);
  }
};
