...
// Dependencies: [502, 1957, 4253, 1371, 1115, 1935, 4758, 4456, 1114, 1898, 573, 2]

// Module 14519 (AccessibilityCallManager)
      ...
      if (null != id && id in channelId.ongoingRings) {
        if (!set.has(channelId)) {
          if (!obj2.isIOS()) {
            ...
            if (null != channel) {
              const channelName = tmp4(4758).computeChannelName(channel, UserStore, RelationshipStore);
              if (null != channelName) {
                obj.add(channelId);
                const AccessibilityAnnouncer = tmp4(4456).AccessibilityAnnouncer;
