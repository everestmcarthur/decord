...
// Dependencies: [502, 1957, 4255, 1371, 1115, 1935, 4760, 4458, 1114, 1898, 573, 2]

// Module 14497 (AccessibilityCallManager)
      ...
      if (null != id && id in channelId.ongoingRings) {
        if (!set.has(channelId)) {
          if (!obj2.isIOS()) {
            ...
            if (null != channel) {
              const channelName = tmp4(4760).computeChannelName(channel, UserStore, RelationshipStore);
              if (null != channelName) {
                obj.add(channelId);
                const AccessibilityAnnouncer = tmp4(4458).AccessibilityAnnouncer;
