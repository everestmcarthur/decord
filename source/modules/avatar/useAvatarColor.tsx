...
// Function ID: 8218
// Name: useAvatarColor
// Dependencies: [32, 5, 19, 4598, 560, 1249, 1474, 4456, 504, 7625, 2]
  ...
  let memo = noop.useMemo(() => {
    ...
    if (dependencyMap != null) {
      mapped = dependencyMap.map((item) => {
        [tmp, tmp2, tmp3] = item;
        const obj = flag(7625)({ r: tmp, g: tmp2, b: tmp3 });
        ({ h, s, l } = flag(7625)({ r: tmp, g: tmp2, b: tmp3 }).toHsl());
        const obj2 = { h, s: s * stateFromStores, l };
        const toHslResult = flag(7625)({ r: tmp, g: tmp2, b: tmp3 }).toHsl();
        return flag(7625)({ h, s: s * stateFromStores, l }).toHexString();
      });
    }
    return mapped;
  }, items2);
