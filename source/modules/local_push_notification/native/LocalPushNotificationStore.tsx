...
// Dependencies: [1980, 5501, 13832, 1074, 9838, 4231, 1115, 504, 573, 2]

// Module 13831 (LocalPushNotificationStore)
  ...
  CONNECTION_OPEN: function handleCheckScheduledNotifs() {
    const scheduledLocalNotifications = PushNotificationDefault.getScheduledLocalNotifications((arr) => {
      ...
      const item = found.forEach((userInfo) => {
        ...
        const result = closure_1_1(9838).cancelLocalNotifications(userInfo);
        const obj = closure_1_1(9838);
        const result1 = closure_1_1(9838).cancelLocalNotifications(userInfo);
        set.delete(userInfo);
      });
    });
  },
      ...
      if (!check.canChat) {
        if (guild.verificationLevel === VerificationLevels.MEDIUM) {
          ...
          if (tmp2.MEDIUM === verificationLevel) {
            let obj = _modDef4231(check.accountDeadline);
          } else if (tmp2.HIGH === verificationLevel) {
            obj = _modDef4231(check.memberDeadline);
          }
          if (null != obj) {
            if (!obj.isSameOrBefore(_modDef4231(), "minute")) {
              ...
              const result = tmp15(9838).scheduleLocalNotification(obj3);
              const tmp15Result = tmp15(9838);
            }
            tmp15 = importDefault;
          }
        }
