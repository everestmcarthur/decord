...
// Dependencies: [1980, 5583, 13925, 1074, 9932, 4314, 1115, 504, 573, 2]

// Module 13924 (LocalPushNotificationStore)
  ...
  CONNECTION_OPEN: function handleCheckScheduledNotifs() {
    const scheduledLocalNotifications = PushNotificationDefault.getScheduledLocalNotifications((arr) => {
      ...
      const item = found.forEach((userInfo) => {
        ...
        const result = closure_1_1(9932).cancelLocalNotifications(userInfo);
        const obj = closure_1_1(9932);
        const result1 = closure_1_1(9932).cancelLocalNotifications(userInfo);
        set.delete(userInfo);
      });
    });
  },
      ...
      if (!check.canChat) {
        if (guild.verificationLevel === VerificationLevels.MEDIUM) {
          ...
          if (tmp2.MEDIUM === verificationLevel) {
            let obj = _modDef4314(check.accountDeadline);
          } else if (tmp2.HIGH === verificationLevel) {
            obj = _modDef4314(check.memberDeadline);
          }
          if (null != obj) {
            if (!obj.isSameOrBefore(_modDef4314(), "minute")) {
              ...
              const result = tmp15(9932).scheduleLocalNotification(obj3);
              const tmp15Result = tmp15(9932);
            }
            tmp15 = importDefault;
          }
        }
