...
// Dependencies: [19, 17, 21, 4640, 11468, 8007, 10365, 2]
// Exports: ForumPostActionBarReactions, MaxForumPostReactions, MostCommonForumPostReaction

// Module 11568 (ForumPostReactions)
import ForumHooks from "ForumHooks" /* 8007 */;
import useReactionPermissionsDefault from "useReactionPermissions" /* 11468 */;
import noop from "module_19" /* 19 */;

const ForumPostReactionButton = tmp4(10365);
require = fn;
...
export const MaxForumPostReactions = function MaxForumPostReactions(thread) {
  ...
  const tmp = closure_6();
  dependencyMap = tmp;
  const disableReactionCreates = reactionContainerStyle(11468)(thread).disableReactionCreates;
  let num = 28;
  if (disableReactionCreates) {
    num = 0;
  }
  const maxPossibleForumPostReactions = thread(8007).useMaxPossibleForumPostReactions({ containerWidth: containerWidth - num, reactionEmojiWidth: 46, digitWidth: 7.5, message: firstMessage, parentChannel });
  ...
  ,
    ...
    if (tmp8) {
      ...
      tmp8 = closure_4(tmp3(10365).AdditionalReactionCount, obj4);
    }
    items1[1] = tmp8;
    let tmp10 = !disableReactionCreates;
    if (!disableReactionCreates) {
      const obj5 = { containerStyle: reactionContainerStyle, threadId: thread.id };
      tmp10 = closure_4(tmp3(10365).AddReactionButton, obj5);
    }
    items1[2] = tmp10;
...
export const ForumPostActionBarReactions = function ForumPostActionBarReactions(thread) {
  ...
  obj2.count = num;
  const someForumPostReactions = thread(8007).useSomeForumPostReactions(obj2);
