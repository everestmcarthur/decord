...
// Dependencies: [19, 17, 21, 4605, 11386, 7937, 10288, 2]
// Exports: ForumPostActionBarReactions, MaxForumPostReactions, MostCommonForumPostReaction

// Module 11486 (ForumPostReactions)
import ForumHooks from "ForumHooks" /* 7937 */;
import useReactionPermissionsDefault from "useReactionPermissions" /* 11386 */;
import noop from "module_19" /* 19 */;

const ForumPostReactionButton = tmp4(10288);
require = fn;
...
export const MaxForumPostReactions = function MaxForumPostReactions(thread) {
  ...
  const tmp = closure_6();
  dependencyMap = tmp;
  const disableReactionCreates = reactionContainerStyle(11386)(thread).disableReactionCreates;
  let num = 28;
  if (disableReactionCreates) {
    num = 0;
  }
  const maxPossibleForumPostReactions = thread(7937).useMaxPossibleForumPostReactions({ containerWidth: containerWidth - num, reactionEmojiWidth: 46, digitWidth: 7.5, message: firstMessage, parentChannel });
  ...
  ,
    ...
    if (tmp8) {
      ...
      tmp8 = closure_4(tmp3(10288).AdditionalReactionCount, obj4);
    }
    items1[1] = tmp8;
    let tmp10 = !disableReactionCreates;
    if (!disableReactionCreates) {
      const obj5 = { containerStyle: reactionContainerStyle, threadId: thread.id };
      tmp10 = closure_4(tmp3(10288).AddReactionButton, obj5);
    }
    items1[2] = tmp10;
...
export const ForumPostActionBarReactions = function ForumPostActionBarReactions(thread) {
  ...
  obj2.count = num;
  const someForumPostReactions = thread(7937).useSomeForumPostReactions(obj2);
