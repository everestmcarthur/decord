...
// Dependencies: [32, 19, 17, 21, 4639, 576, 1177, 12141, 12142, 11383, 4635, 1478, 1370, 12143, 7382, 8004, 12147, 2]
// Exports: default

// Module 12140 (ForumPostGridBody)
...
import ForumPostMedia from "ForumPostMedia" /* 12143 */;
...
function GIFIcon() {
  const tmp = closure_8();
  return timestampProducer(native.Icon, { size: native.Icon.Sizes.CUSTOM, source: _modDef12141, disableColor: true, style: closure_8().gifIcon });
}
function PlayIcon() {
  return timestampProducer(native.Icon, { size: native.Icon.Sizes.SMALL_20, source: _modDef12142, disableColor: true });
}
function ExtraMediaIcon(extraMediaCount) {
  const tmp = closure_8();
  const obj = { style: tmp.extraMediaCountContainer, children: null };
  items = [timestampProducer(native.Icon, { source: _modDef11383, color: tmp.icon.color, size: native.Icon.Sizes.REFRESH_SMALL_16 }), ];
  const obj2 = { source: _modDef11383, color: tmp.icon.color, size: native.Icon.Sizes.REFRESH_SMALL_16 };
...
export default function ForumPostGridBody(thread) {
  ...
  const tmp = closure_8();
  dependencyMap = tmp;
  const tmp4 = _slicedToArray(thread(7382).useSomeAppliedTags(thread, 2), 2);
  const first = tmp4[0];
  ...
  if (tmp13Result) {
    ...
    obj4.children = tmp13(tmp2(12147).ForumPostAppliedTagPills, obj5);
