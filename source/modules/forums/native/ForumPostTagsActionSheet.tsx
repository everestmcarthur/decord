...
// Dependencies: [32, 19, 17, 7383, 21, 4638, 1115, 7385, 7307, 7259, 11395, 5060, 8014, 4605, 2]
// Exports: default

// Module 11394 (ForumPostTagsActionSheet)
...
const MAX_FORUM_POST_TAGS = fn(7383).MAX_FORUM_POST_TAGS;
const jsxProd = fn(21);
...
export default function ForumPostTagsActionSheet(thread) {
  ...
  let appliedTags = thread(7385).useAppliedTags(thread);
  if (null != tags) {
    appliedTags = tags;
  }
  let obj = thread(7385);
  ...
  const visibleForumTags = thread(7385).useVisibleForumTags(thread.parentChannel);
  const obj2 = {
    onDismiss() {
      let tmp;
  ...
  obj2.header = toggleTag(thread(7259).BottomSheetTitleHeader, obj3);
  const tmp4Result = thread(7385);
  let items = [
    toggleTag(closure_5, {
  ...
  obj5.children = toggleTag(thread(5060).Button, obj6);
  items[1] = toggleTag(closure_5, obj5);
  obj2.children = items;
  return closure_8(thread(7307).ActionSheet, obj2);
};
