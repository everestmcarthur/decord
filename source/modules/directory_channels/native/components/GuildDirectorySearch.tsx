...
// Dependencies: [32, 19, 17, 1979, 12333, 1074, 21, 4574, 576, 12336, 4570, 1114, 1178, 504, 12337, 12338, 1611, 12365, 12366, 5638, 7390, 12346, 12330, 1242, 2]
// Exports: default
...
function DefaultState() {
  ...
  const items = [closure_12(closure_7, { style: tmp.emptyStateImage, source: _modDef12336 }), ];
  const obj3 = { style: tmp.emptyStateText, variant: "text-sm/medium", color: "text-default", children: null };
  const intl = require("util").intl;
  obj3.children = intl.format(require("util").t.aYLd8O, {
    protipHook(children) {
      return closure_2_12(native.LegacyText, { style: proTip.proTip, children }, "protip");
    }
  });
  items[1] = closure_12(require("Text/Text").Text, obj3);
  obj.children = items;
...
function EmptyState(channel) {
  ...
  const canCreateOrAddGuildInDirectory = channel(12337).useCanCreateOrAddGuildInDirectory(channel);
  ...
  const obj2 = channel(12337);
  const items1 = [closure_12(closure_7, { style: tmp.emptyStateImage, source: _modDef12336 }), , ];
  ...
  items1[1] = closure_12(channel(4570).Text, obj6);
...
export default function GuildDirectorySearch(channel) {
  ...
  const stateFromStoresObject = channel(searchFetching[13]).useStateFromStoresObject(items, () => {
  ...
  if (!tmp2[0]) {
    ...
    obj4.placeholder = intl.string(tmp4(tmp5[11]).t.nL2wKD);
    ...
    const items2 = [tmp8(tmp4(tmp5[19]).FauxHeader, obj3), tmp9];
