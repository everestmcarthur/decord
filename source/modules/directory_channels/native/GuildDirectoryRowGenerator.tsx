...
// Dependencies: [12400, 12399, 1114, 2]
// Exports: generateDirectoryRows

// Module 12765 (GuildDirectoryRowGenerator)
import GuildDirectoryConstants from "GuildDirectoryConstants" /* 12400 */;
...
export const generateDirectoryRows = function generateDirectoryRows(arg0, arr, currentCategoryId) {
  ...
  if (0 === arr.length) {
    ...
    const obj4 = set(12399);
    return set(12399).rankGuildEntries(arr).map((entry) => ({ type: constants.ENTRY, entry }));
  } else {
    const items = [];
    const rankByDateAddedResult = set(12399).rankByDateAdded(arr);
    const _Set = Set;
    ...
    const obj5 = set(12399);
    const result = set(12399).orderByTotalMemberCount(found);
