...
// Dependencies: [12448, 12447, 1115, 2]
// Exports: generateDirectoryRows

// Module 12813 (GuildDirectoryRowGenerator)
...
export const generateDirectoryRows = function generateDirectoryRows(arg0, arr, currentCategoryId) {
  ...
  if (0 === arr.length) {
    ...
    const obj4 = set(12447);
    return set(12447).rankGuildEntries(arr).map((entry) => ({ type: constants.ENTRY, entry }));
  } else {
    const items = [];
    const rankByDateAddedResult = set(12447).rankByDateAdded(arr);
    const _Set = Set;
    ...
    const obj5 = set(12447);
    const result = set(12447).orderByTotalMemberCount(found);
