...
// Dependencies: [12398, 12397, 1114, 2]
// Exports: generateDirectoryRows

// Module 12763 (GuildDirectoryRowGenerator)
import GuildDirectoryConstants from "GuildDirectoryConstants" /* 12398 */;
...
export const generateDirectoryRows = function generateDirectoryRows(arg0, arr, currentCategoryId) {
  ...
  if (0 === arr.length) {
    ...
    const obj4 = set(12397);
    return set(12397).rankGuildEntries(arr).map((entry) => ({ type: constants.ENTRY, entry }));
  } else {
    const items = [];
    const rankByDateAddedResult = set(12397).rankByDateAdded(arr);
    const _Set = Set;
    ...
    const obj5 = set(12397);
    const result = set(12397).orderByTotalMemberCount(found);
