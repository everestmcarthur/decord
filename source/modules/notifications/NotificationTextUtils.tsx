...
// Dependencies: [1956, 4246, 7292, 4247, 4814, 1961, 1957, 1979, 4629, 4255, 2011, 4428, 5330, 4787, 1951, 1371, 10176, 1074, 9596, 1113, 4817, 1935, 1384, 10177, 10178, 4844, 4465, 10179, 7757, 4759, 1114, 8352, 4758, 7343, 8060, 3, 7375, 4451, 1894, 7754, 10180, 504, 2]
...
// Module 10175 (NotificationTextUtils)
...
import ChannelVisibilityUtils from "ChannelVisibilityUtils" /* 10177 */;
import ThreadNotificationSettings from "ThreadNotificationSettings" /* 10178 */;
...
import RpcNotificationSettingsStore from "RpcNotificationSettingsStore" /* 10176 */;
...
function renderTitle(channelName, channel, channel) {
  ...
  if (null != channel) {
    ...
    str = ", " + tmpResult.isolate(tmp(4759).computeChannelName(channel, tmp4, tmp5));
...
export const shouldIncludeSelectedChannel = function shouldIncludeSelectedChannel() {
  ...
  if (!state.isVoicePanelFullscreen()) {
    if (null == EmbeddedActivitiesStore.getConnectedActivityLocation()) {
      ...
      if (!tmp5) {
        tmp5 = !tmp3(10179).isChannelFocused();
        ...
        if (tmp4.GUILD_VOICE !== type) {
          if (tmp4.ANNOUNCEMENT_THREAD !== type) {
            if (tmp4.PUBLIC_THREAD !== type) {
              ...
              if (tmp(7343)(content)) {
                content1 = tmp(8060).stringify(content, getGuildId);
                if (null == content1) {
              ...
              if (result) {
                ...
                if (content.type === constants4.PREMIUM_REFERRAL) {
                  ...
                  const tmpResult4 = tmp(7754);
                  ({ content, emoji } = tmp(7754).unparseWithMeta(content1, getGuildId.id, true));
                  tmp26 = emoji;
                  ...
                  const unparseWithMetaResult = tmp(7754).unparseWithMeta(content1, getGuildId.id, true);
                }
              }
