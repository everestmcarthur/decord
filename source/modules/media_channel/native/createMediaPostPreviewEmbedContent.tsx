...
// Dependencies: [17, 4605, 1957, 1979, 4428, 1371, 11520, 4754, 4760, 1114, 8034, 8036, 576, 4756, 13341, 7672, 4819, 4755, 2]
// Exports: default
    ...
    if (MediaPostEmbedStore.getEmbedFetchState(mediaPostEmbedChannelId) !== FetchState.FETCHED) {
      ...
      if (null == media) {
        ...
        if (canViewChannelResult) {
          canViewChannelResult = tmp(4760).canViewChannel(channel);
        ...
        const mediaPostEmbedCommonData = tmp(4754).getMediaPostEmbedCommonData(obj2);
        if (null == mediaPostEmbedCommonData) {
          ...
          if (null != mediaPostEmbedCommonData.authorName) {
            if (null != mediaPostEmbedCommonData.channelName) {
              if (null != user) {
                const userAuthorWithProcessedColor = tmp(8034).getUserAuthorWithProcessedColor(user, mediaPostEmbedCommonData.postThread);
              ...
              if (isAnimatedImageUrlResult) {
                isAnimatedImageUrlResult = tmp(4756).isAnimatedImageUrl(mediaPostEmbedCommonData.coverImage);
              ...
              if (mediaPostEmbedCommonData.shouldShowBlurredThumbnailImage) {
                ...
                obj5.blurredCoverImage = React3.resolveAssetSource(tmp10(13341)).uri;
                ...
                const isPendingScanVersionResult = tmp(7672).isPendingScanVersion(mediaPostEmbedCommonData.contentScanVersion);
                let result = value;
                if (value) {
                  result = tmp(7672).shouldAgeVerifyForExplicitMedia();
                  const tmpResult12 = tmp(7672);
                }
                let isVerifiedTeenResult = value;
                if (value) {
                  isVerifiedTeenResult = tmp(4819).isVerifiedTeen();
                  const tmpResult13 = tmp(4819);
                }
                if (mediaPostEmbedCommonData.shouldContainMediaWithBackground) {
                  if (null != mediaPostEmbedCommonData.coverImage) {
                    ...
                    obj6.backgroundImage = tmp(4755).getBackgroundImageUrl(mediaPostEmbedCommonData.coverImage);
                    let obj8 = obj6;
                    const tmpResult14 = tmp(4755);
                  }
                  return obj8;
                }
                obj8 = {};
