...
// Dependencies: [4661, 502, 1958, 4559, 4689, 2012, 4678, 1372, 4681, 1074, 12, 4781, 1091, 4691, 1953, 573, 7225, 9242, 17988, 2]

// Module 18008 (go_live/ApplicationStreamingManager)
...
const apply = fn(12);
apply.debounce(fn(4781).notifyStreamStart, 1000);
let closure_12 = {};
let closure_13 = {};
...
const prototype = function BaseApplicationStreamingManager() {
  ...
  applyArgumentsResult.handleStreamWatch = function handleStreamWatch(streamKey) {
    ...
    channel = channel.getChannel(streamKey(4691).decodeStreamKey(streamKey).channelId);
    if (channel != null) {
    ...
    if (allActiveStreamKeys.includes(streamKey)) {
      ...
      if (!streamKey.allowMultiple) {
        ...
        const item = allActiveStreams.forEach((ownerId) => {
          const encodeStreamKeyResult = applyArgumentsResult(4691).encodeStreamKey(ownerId);
          let tmp4 = ownerId.ownerId !== AuthenticationStore.getId();
          if (tmp4) {
            tmp4 = encodeStreamKeyResult !== streamKey;
          }
          if (tmp4) {
            applyArgumentsResult(4781).stopStream(encodeStreamKeyResult, false);
            const tmpResult = applyArgumentsResult(4781);
          }
        });
      }
    } else {
  ...
  applyArgumentsResult.handleStreamCreate = function handleStreamCreate(streamKey) {
    ...
    const obj2 = applyArgumentsResult(4691);
    memberCount = memberCount.getMemberCount(applyArgumentsResult(4691).decodeStreamKey(streamKey).guildId);
  };
            ...
            if (channel.isDM()) {
              if (null == obj3.getActiveStreamForUser(ownerId, channel.getGuildId())) {
                ...
                if (null != streamForUser) {
                  const encodeStreamKeyResult = applyArgumentsResult(4691).encodeStreamKey(streamForUser);
                  if (encodeStreamKeyResult !== c17) {
                    ...
                    if (!isStreamMarkedFullResult) {
                      ...
                      tmp2(4781).watchStream(streamForUser, { noFocus: true });
                      const tmp2Result = tmp2(4781);
                    }
                  }
                  let obj = applyArgumentsResult(4691);
                  ...
                  if (null == authStore.getActiveStreamForUser(userId, channel.getGuildId())) {
                    ...
                    if (null != streamForUser) {
                      ...
                      if (tmp16) {
                        ...
                        if (!isStreamMarkedFullResult) {
                          c17 = encodeStreamKeyResult;
                          tmp12(4781).watchStream(streamForUser, { noFocus: true });
                          flag2 = true;
            ...
            if (activeStreamForUser.channelId === channelId) {
              if (!selfStream) {
                if (activeStreamForUser.state !== constants.ENDED) {
                  const encodeStreamKeyResult1 = applyArgumentsResult(4691).encodeStreamKey(activeStreamForUser);
                  ...
                  const obj5 = applyArgumentsResult(4691);
                  tmp21 = applyArgumentsResult;
                }
              }
              if (selfStream) {
                if (activeStreamForUser.state === constants.ENDED) {
                  const obj10 = applyArgumentsResult(4691);
                  ...
                  if (null != streamForUser1) {
                    if (!obj4.isStreamMarkedFull(tmp34Result.encodeStreamKey(streamForUser1))) {
                      tmp34(4781).watchStream(streamForUser1);
                      const tmp34Result2 = tmp34(4781);
                    }
                    tmp34Result = tmp34(4691);
                  }
                }
              }
            }
  ...
  applyArgumentsResult.handleCallUpdate = function handleCallUpdate(region) {
    ...
    if (channelId === region.channelId) {
      const encodeStreamKeyResult = applyArgumentsResult(4691).encodeStreamKey(currentUserActiveStream);
      if (region == null) {
      ...
      if (tmp7) {
        tmp3(4781).changeStreamRegion(encodeStreamKeyResult, region);
        const tmp3Result = tmp3(4781);
      }
      const obj = applyArgumentsResult(4691);
