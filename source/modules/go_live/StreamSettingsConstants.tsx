...
// Module 4607 (RESOLUTION_720)
import set from "set" /* 2 */;
import ME from "ME" /* 1074 */;
import GuildFeatures from "GuildFeatures" /* 1373 */;

let FPS_602 = require;
let f32389 = dependencyMap;
...
f32389 = () => {
  const intl = FPS_602(f32389[2]).intl;
  return intl.string(FPS_602(f32389[2]).t.XjXqzh);
};
...
f32389 = () => {
  ...
  if (RESOLUTION_480 === obj.RESOLUTION_SOURCE) {
    const intl2 = FPS_602(f32389[2]).intl;
    let stringResult = intl2.string(FPS_602(f32389[2]).t.XjXqzh);
  } else {
    const intl = FPS_602(f32389[2]).intl;
    obj = { resolution: null };
    obj[0] = RESOLUTION_480;
    stringResult = intl.formatToPlainString(FPS_602(f32389[2]).t.TEOC0I, obj);
  }
  return stringResult;
};
...
f32389 = () => {
  ...
  return intl.formatToPlainString(FPS_602(f32389[2]).t["bW+JCW"], obj);
};
