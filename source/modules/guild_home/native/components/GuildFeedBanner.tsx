...
// Dependencies: [19, 17, 14061, 1980, 16669, 1074, 21, 4639, 576, 4376, 1478, 4574, 14062, 1364, 1397, 5675, 4640, 4643, 504, 7050, 11662, 4491, 16670, 16671, 5669, 4635, 1177, 5207, 4338, 1115, 16672, 2]

// Module 16668 (GuildFeedBanner)
import nativeDefault from "native" /* 576 */;
import PlatformUtils from "PlatformUtils" /* 1364 */;
import AvatarUtilsDefault from "AvatarUtils" /* 1397 */;
import timing from "timing" /* 4640 */;
import timingPresets from "timingPresets" /* 4643 */;
import GuildPopoutActionCreators from "GuildPopoutActionCreators" /* 14062 */;
import noop from "module_19" /* 19 */;
import GuildPopoutStore from "GuildPopoutStore" /* 14061 */;
...
function GuildFeedBanner(guild) {
  ...
  let tmp = closure_13();
  const sharedValue = guild(4376).useSharedValue(0);
  let obj = guild(4376);
  class G {
    constructor() {
  ...
  const animatedStyle = guild(4376).useAnimatedStyle(G);
  ...
  const tmp8 = sharedValue(4574)();
  const guildBadgeSource = guild(5675).getGuildBadgeSource(guild);
  const tmp2Result = guild(5675);
  ...
  const tmp12 = sharedValue(7050)();
  dependencyMap = tmp12;
  const size = tmp6(1478)();
  width = size.width;
  height = size.height;
  const tmp2Result4 = guild(504);
  drawerWidth = guild(11662).useDrawerWidth();
  ...
  if (null != memo) {
    ...
    if (tmp2Result6.isThemeDark(tmp8)) {
      let tmp6Result = tmp6(16670);
    } else {
      tmp6Result = tmp6(16671);
    }
    obj6.source = tmp6Result;
    obj6.onLoad = handleLoad;
    tmp2Result6 = tmp2(4491);
  }
  ...
  const tmp2Result5 = guild(11662);
  obj10.size = guild(5669).GuildIconSizes.XLARGE;
  obj9.children = closure_11(sharedValue(5669), obj10);
  const items10 = [closure_11(height, obj9), , , ];
  const obj11 = { style: tmp.textContainer, children: null };
  const items11 = [closure_11(guild(4635).Text, { lineClamp: 1, variant: "heading-xl/extrabold", color: "mobile-text-heading-primary", style: tmp.title, children: guild.name }), ];
  ...
  if (null != description) {
    ...
    tmp17Result2 = tmp17(tmp2(4635).Text, obj14);
  }
  ...
  if (tmp15Result4) {
    ...
    if (features.has(GuildFeatures.DISCOVERABLE)) {
      const obj16 = {
        ...
        onPress() {
              ...
              sharedValue(4338).open(obj2);
            },
        children: null
      };
      const obj17 = { style: tmp.publicIcon, source: tmp6(16672) };
      ...
      items12[1] = tmp17(tmp2(4635).Text, obj18);
      obj16.children = items12;
      tmp15Result = tmp15(tmp2(5207).PressableOpacity, obj16);
    }
    ...
    if (null != discoverableGuild.presenceCount) {
      ...
      if (null != discoverableGuild.memberCount) {
        ...
        items14[1] = tmp17(tmp2(4635).Text, obj21);
...
const GuildFeedConstants = fn(16669);
