...
// Dependencies: [19, 17, 14074, 1980, 16689, 1074, 21, 4638, 576, 4374, 1478, 4572, 14075, 1364, 1397, 5676, 4639, 4642, 504, 7053, 11670, 4489, 16690, 16691, 5670, 4634, 1177, 5208, 4336, 1115, 16692, 2]

// Module 16688 (GuildFeedBanner)
import nativeDefault from "native" /* 576 */;
import PlatformUtils from "PlatformUtils" /* 1364 */;
import AvatarUtilsDefault from "AvatarUtils" /* 1397 */;
import timing from "timing" /* 4639 */;
import timingPresets from "timingPresets" /* 4642 */;
import GuildPopoutActionCreators from "GuildPopoutActionCreators" /* 14075 */;
import noop from "module_19" /* 19 */;
import GuildPopoutStore from "GuildPopoutStore" /* 14074 */;
...
function GuildFeedBanner(guild) {
  ...
  let tmp = closure_13();
  const sharedValue = guild(4374).useSharedValue(0);
  let obj = guild(4374);
  class G {
    constructor() {
  ...
  const animatedStyle = guild(4374).useAnimatedStyle(G);
  ...
  const tmp8 = sharedValue(4572)();
  const guildBadgeSource = guild(5676).getGuildBadgeSource(guild);
  const tmp2Result = guild(5676);
  ...
  const tmp12 = sharedValue(7053)();
  dependencyMap = tmp12;
  const size = tmp6(1478)();
  width = size.width;
  height = size.height;
  const tmp2Result4 = guild(504);
  drawerWidth = guild(11670).useDrawerWidth();
  ...
  if (null != memo) {
    ...
    if (tmp2Result6.isThemeDark(tmp8)) {
      let tmp6Result = tmp6(16690);
    } else {
      tmp6Result = tmp6(16691);
    }
    obj6.source = tmp6Result;
    obj6.onLoad = handleLoad;
    tmp2Result6 = tmp2(4489);
  }
  ...
  const tmp2Result5 = guild(11670);
  obj10.size = guild(5670).GuildIconSizes.XLARGE;
  obj9.children = closure_11(sharedValue(5670), obj10);
  const items10 = [closure_11(height, obj9), , , ];
  const obj11 = { style: tmp.textContainer, children: null };
  const items11 = [closure_11(guild(4634).Text, { lineClamp: 1, variant: "heading-xl/extrabold", color: "mobile-text-heading-primary", style: tmp.title, children: guild.name }), ];
  ...
  if (null != description) {
    ...
    tmp17Result2 = tmp17(tmp2(4634).Text, obj14);
  }
  ...
  if (tmp15Result4) {
    ...
    if (features.has(GuildFeatures.DISCOVERABLE)) {
      const obj16 = {
        ...
        onPress() {
              ...
              sharedValue(4336).open(obj2);
            },
        children: null
      };
      const obj17 = { style: tmp.publicIcon, source: tmp6(16692) };
      ...
      items12[1] = tmp17(tmp2(4634).Text, obj18);
      obj16.children = items12;
      tmp15Result = tmp15(tmp2(5208).PressableOpacity, obj16);
    }
    ...
    if (null != discoverableGuild.presenceCount) {
      ...
      if (null != discoverableGuild.memberCount) {
        ...
        items14[1] = tmp17(tmp2(4634).Text, obj21);
...
const GuildFeedConstants = fn(16689);
