...
// Dependencies: [19, 17, 14082, 1980, 16719, 1074, 21, 4640, 576, 4376, 1478, 4574, 14083, 1364, 1397, 5678, 4641, 4644, 504, 7057, 11678, 4491, 16720, 16721, 5672, 4636, 1177, 5210, 4338, 1115, 16722, 2]

// Module 16718 (GuildFeedBanner)
import nativeDefault from "native" /* 576 */;
import PlatformUtils from "PlatformUtils" /* 1364 */;
import AvatarUtilsDefault from "AvatarUtils" /* 1397 */;
import timing from "timing" /* 4641 */;
import timingPresets from "timingPresets" /* 4644 */;
import GuildPopoutActionCreators from "GuildPopoutActionCreators" /* 14083 */;
import noop from "module_19" /* 19 */;
import GuildPopoutStore from "GuildPopoutStore" /* 14082 */;
...
function GuildFeedBanner(guild) {
  ...
  const guildBadgeSource = guild(5678).getGuildBadgeSource(guild);
  const tmp2Result = guild(5678);
  ...
  const tmp12 = sharedValue(7057)();
  dependencyMap = tmp12;
  const size = tmp6(1478)();
  width = size.width;
  height = size.height;
  const tmp2Result4 = guild(504);
  drawerWidth = guild(11678).useDrawerWidth();
  ...
  if (null != memo) {
    ...
    if (tmp2Result6.isThemeDark(tmp8)) {
      let tmp6Result = tmp6(16720);
    } else {
      tmp6Result = tmp6(16721);
    }
  ...
  const tmp2Result5 = guild(11678);
  obj10.size = guild(5672).GuildIconSizes.XLARGE;
  obj9.children = closure_11(sharedValue(5672), obj10);
  const items10 = [closure_11(height, obj9), , , ];
  const obj11 = { style: tmp.textContainer, children: null };
  const items11 = [closure_11(guild(4636).Text, { lineClamp: 1, variant: "heading-xl/extrabold", color: "mobile-text-heading-primary", style: tmp.title, children: guild.name }), ];
  ...
  if (null != description) {
    ...
    tmp17Result2 = tmp17(tmp2(4636).Text, obj14);
  }
  ...
  if (tmp15Result4) {
    ...
    if (features.has(GuildFeatures.DISCOVERABLE)) {
      ...
      const obj17 = { style: tmp.publicIcon, source: tmp6(16722) };
      ...
      items12[1] = tmp17(tmp2(4636).Text, obj18);
      obj16.children = items12;
      tmp15Result = tmp15(tmp2(5210).PressableOpacity, obj16);
    }
    ...
    if (null != discoverableGuild.presenceCount) {
      ...
      if (null != discoverableGuild.memberCount) {
        ...
        items14[1] = tmp17(tmp2(4636).Text, obj21);
...
const GuildFeedConstants = fn(16719);
