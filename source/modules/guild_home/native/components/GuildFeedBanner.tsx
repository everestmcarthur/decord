...
// Dependencies: [19, 17, 14021, 1979, 16630, 1074, 21, 4606, 576, 4343, 1477, 4541, 14022, 1115, 1396, 5641, 4607, 4610, 504, 7016, 11621, 4458, 16631, 16632, 5635, 4602, 1178, 5174, 4305, 1114, 16633, 2]

// Module 16629 (GuildFeedBanner)
import nativeDefault from "native" /* 576 */;
import PlatformUtils from "PlatformUtils" /* 1115 */;
import AvatarUtilsDefault from "AvatarUtils" /* 1396 */;
import timing from "timing" /* 4607 */;
import timingPresets from "timingPresets" /* 4610 */;
import GuildPopoutActionCreators from "GuildPopoutActionCreators" /* 14022 */;
import noop from "module_19" /* 19 */;
import GuildPopoutStore from "GuildPopoutStore" /* 14021 */;
...
function GuildFeedBanner(guild) {
  ...
  let tmp = closure_13();
  const sharedValue = guild(4343).useSharedValue(0);
  let obj = guild(4343);
  class G {
    constructor() {
  ...
  const animatedStyle = guild(4343).useAnimatedStyle(G);
  ...
  const tmp8 = sharedValue(4541)();
  const guildBadgeSource = guild(5641).getGuildBadgeSource(guild);
  const tmp2Result = guild(5641);
  ...
  const tmp12 = sharedValue(7016)();
  dependencyMap = tmp12;
  const size = tmp6(1477)();
  width = size.width;
  height = size.height;
  const tmp2Result4 = guild(504);
  drawerWidth = guild(11621).useDrawerWidth();
  ...
  if (null != memo) {
    ...
    if (tmp2Result6.isThemeDark(tmp8)) {
      let tmp6Result = tmp6(16631);
    } else {
      tmp6Result = tmp6(16632);
    }
    obj6.source = tmp6Result;
    obj6.onLoad = handleLoad;
    tmp2Result6 = tmp2(4458);
  }
  ...
  const tmp2Result5 = guild(11621);
  obj10.size = guild(5635).GuildIconSizes.XLARGE;
  obj9.children = closure_11(sharedValue(5635), obj10);
  const items10 = [closure_11(height, obj9), , , ];
  const obj11 = { style: tmp.textContainer, children: null };
  const items11 = [closure_11(guild(4602).Text, { lineClamp: 1, variant: "heading-xl/extrabold", color: "mobile-text-heading-primary", style: tmp.title, children: guild.name }), ];
  let tmp17Result = null;
  if (null != guildBadgeSource) {
    const obj13 = { style: tmp.icon, source: guildBadgeSource, disableColor: true };
    tmp17Result = tmp17(tmp2(1178).Icon, obj13);
  }
  ...
  if (null != description) {
    ...
    tmp17Result2 = tmp17(tmp2(4602).Text, obj14);
  }
  ...
  if (tmp15Result4) {
    ...
    if (features.has(GuildFeatures.DISCOVERABLE)) {
      const obj16 = {
        ...
        onPress() {
              ...
              sharedValue(4305).open(obj2);
            },
        children: null
      };
      const obj17 = { style: tmp.publicIcon, source: tmp6(16633) };
      const items12 = [tmp17(tmp2(1178).Icon, obj17), ];
      const obj18 = { variant: "text-xs/medium", color: "text-default", children: null };
      let intl = tmp2(1114).intl;
      obj18.children = intl.string(tmp2(1114).t["B/vjCu"]);
      items12[1] = tmp17(tmp2(4602).Text, obj18);
      obj16.children = items12;
      tmp15Result = tmp15(tmp2(5174).PressableOpacity, obj16);
    }
    ...
    if (null != discoverableGuild.presenceCount) {
      ...
      if (null != discoverableGuild.memberCount) {
        ...
        items14[1] = tmp17(tmp2(4602).Text, obj21);
...
const GuildFeedConstants = fn(16630);
