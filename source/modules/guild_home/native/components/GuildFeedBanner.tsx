...
// Dependencies: [19, 17, 13994, 1979, 16587, 1074, 21, 4574, 576, 4310, 1477, 4509, 13995, 1115, 1396, 5604, 4575, 4578, 504, 6959, 11559, 4425, 16588, 16589, 5598, 4570, 1178, 5137, 4272, 1114, 16590, 2]

// Module 16586 (GuildFeedBanner)
...
function GuildFeedBanner(guild) {
  ...
  let tmp = closure_13();
  const sharedValue = guild(4310).useSharedValue(0);
  let obj = guild(4310);
  class G {
    constructor() {
  ...
  const animatedStyle = guild(4310).useAnimatedStyle(G);
  ...
  const tmp8 = sharedValue(4509)();
  const guildBadgeSource = guild(5604).getGuildBadgeSource(guild);
  const tmp2Result = guild(5604);
  ...
  const tmp12 = sharedValue(6959)();
  dependencyMap = tmp12;
  const size = tmp6(1477)();
  width = size.width;
  height = size.height;
  const tmp2Result4 = guild(504);
  drawerWidth = guild(11559).useDrawerWidth();
  ...
  if (null != memo) {
    ...
    if (tmp2Result6.isThemeDark(tmp8)) {
      let tmp6Result = tmp6(16588);
    } else {
      tmp6Result = tmp6(16589);
    }
    obj6.source = tmp6Result;
    obj6.onLoad = handleLoad;
    tmp2Result6 = tmp2(4425);
  }
  ...
  const tmp2Result5 = guild(11559);
  obj10.size = guild(5598).GuildIconSizes.XLARGE;
  obj9.children = closure_11(sharedValue(5598), obj10);
  const items10 = [closure_11(height, obj9), , , ];
  const obj11 = { style: tmp.textContainer, children: null };
  const items11 = [closure_11(guild(4570).Text, { lineClamp: 1, variant: "heading-xl/extrabold", color: "mobile-text-heading-primary", style: tmp.title, children: guild.name }), ];
  ...
  if (null != description) {
    ...
    tmp17Result2 = tmp17(tmp2(4570).Text, obj14);
  }
  ...
  if (tmp15Result4) {
    ...
    if (features.has(GuildFeatures.DISCOVERABLE)) {
      const obj16 = {
        ...
        onPress() {
              ...
              sharedValue(4272).open(obj2);
            },
        children: null
      };
      const obj17 = { style: tmp.publicIcon, source: tmp6(16590) };
      ...
      items12[1] = tmp17(tmp2(4570).Text, obj18);
      obj16.children = items12;
      tmp15Result = tmp15(tmp2(5137).PressableOpacity, obj16);
    }
    ...
    if (null != discoverableGuild.presenceCount) {
      ...
      if (null != discoverableGuild.memberCount) {
        ...
        items14[1] = tmp17(tmp2(4570).Text, obj21);
...
const GuildFeedConstants = fn(16587);
