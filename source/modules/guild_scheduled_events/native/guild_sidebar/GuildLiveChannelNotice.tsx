...
// Dependencies: [19, 17, 5468, 1962, 4627, 4243, 4629, 1963, 1085, 21, 576, 1178, 4595, 10198, 8149, 10189, 1115, 5030, 4605, 4601, 12600, 10191, 1874, 4812, 8458, 4540, 7924, 5025, 4456, 4758, 504, 1114, 5078, 9855, 9775, 9785, 9859, 5481, 5475, 5467, 9732, 5147, 16269, 10985, 5657, 2]
// Exports: getScaledLiveChannelNoticeHeight

// Module 16270 (GuildLiveChannelNotice)
...
function UserSummaryRow(arg0) {
  ...
  ,
    ...
    if (tmp8Result) {
      ...
      obj3.style = items2;
      let obj4 = { size: "custom", style: max(10191).makeSizeStyle(14) };
      const items3 = [closure_13(max(12600).HeadphonesIcon, obj4), ];
      let obj6 = { variant: "text-xs/semibold", style: { marginLeft: 4 }, maxFontSizeMultiplier: 1, children: audienceCount };
      items3[1] = closure_13(max(4601).Text, obj6);
...
function JoinChannelButton(label) {
  ...
  const callback = noop.useCallback(() => {
    if (null != activeEventOrStageInstanceChannel) {
      ...
      if (obj.isGuildVoice()) {
        tmp4(4812).openGuildVoiceModal(obj);
        const tmp4Result = tmp4(4812);
      } else {
        tmp4(8458).connectAndOpen(obj);
        const tmp4Result2 = tmp4(8458);
      }
    }
  }, items);
  ...
  obj.children = closure_13(channel(5025).Button, obj2);
...
function GuildVoiceEventNotice(channel) {
  ...
  obj4.LocationIcon = channel(5078).getChannelIconComponent(channel);
  obj4.LiveIcon = channel(9855).CalendarIcon;
...
function GuildExternalEventNotice(guildEvent) {
  ...
  if (null != locationFromEvent) {
    ...
    obj2.LocationIcon = tmp(9785).LocationIcon;
    obj2.LiveIcon = tmp(9855).CalendarIcon;
...
function SeeDetailButton(guildEvent) {
  ...
  obj.children = closure_13(guildEvent(5025).Button, obj2);
  return closure_13(View, obj);
}
function GuildLiveStageNotice(channel) {
  channel = channel.channel;
  const tmp2 = useChannelNameDefault(channel);
  const stageParticipants = channel(5481).useStageParticipants(channel.id, channel(5475).StageChannelParticipantNamedIndex.SPEAKER);
  ...
  const obj3 = channel(504);
  const stageHasStream = channel(5467).useStageHasStream(channel.id);
  const obj4 = channel(5467);
  const guildActiveEvent = channel(9732).useGuildActiveEvent(channel.guild_id);
  ...
  if (null != guildActiveEvent) {
    channelIconComponent = tmp3(5078).getChannelIconComponent(channel);
    const tmp3Result = tmp3(5078);
  }
  obj6.LocationIcon = channelIconComponent;
  if (null != guildActiveEvent) {
    let StageIcon = tmp3(9855).CalendarIcon;
  } else {
    StageIcon = tmp3(5147).StageIcon;
  }
...
obj2.react = fn(10198).inlineChannelMentionReact;
...
obj.channel = { react: fn(10198).inlineChannelReact };
...
let closure_31 = noop.memo((arg0) => {
  ...
  if (tmp2Result) {
    ...
    items4[1] = tmp4(tmp7(4601).Text, obj8);
...
let obj3 = { react: fn(10198).inlineChannelReact };
size = fn(2);
...
export default noop.memo((guild) => {
  ...
  activeEventOrStageInstanceChannel = activeEventOrStageInstanceChannel(16269).useActiveEventOrStageInstanceChannel(guild.id);
  let obj = activeEventOrStageInstanceChannel(16269);
  const guildActiveEvent = activeEventOrStageInstanceChannel(9732).useGuildActiveEvent(guild.id);
  let obj2 = activeEventOrStageInstanceChannel(9732);
  ...
  if (null != tmp13) {
    ...
    obj7.children = tmp13;
    tmp20 = closure_13(tmp2(5657).Card, obj7);
  }
  return tmp20;
});
...
export const getScaledLiveChannelNoticeHeight = function getScaledLiveChannelNoticeHeight(fontScale, guildLiveChannelNoticeInfo) {
  ...
  if (!hasSpeakers) {
    ...
    const sum = PX_82 + tmp(10189).scaleTextLineHeight(c22, fontScale);
    const tmp5 = PX_82;
    const tmp8 = PX_4;
    const tmpResult = tmp(10189);
    let num2 = 0;
    ...
    tmpResult3 = tmp(1115);
    let num3 = 0;
    const sum2 = sum1 + tmp(10189).scaleTextLineHeight(c23, fontScale);
    if (hasButton) {
      num3 = tmp5 + tmp(5030).SMALL_BUTTON_HEIGHT;
    }
