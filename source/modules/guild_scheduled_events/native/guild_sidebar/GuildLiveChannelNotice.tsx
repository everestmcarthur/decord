...
// Dependencies: [19, 17, 5469, 1962, 4628, 4245, 4630, 1963, 1085, 21, 576, 1178, 4596, 10217, 8171, 10208, 1115, 5031, 4606, 4602, 12623, 10210, 1874, 4813, 8479, 4541, 7946, 5026, 4458, 4759, 504, 1114, 5079, 9876, 9796, 9806, 9880, 5482, 5476, 5468, 9753, 5148, 16258, 11005, 5658, 2]
...
function UserSummaryRow(arg0) {
  ...
  ,
    ...
    if (tmp8Result) {
      ...
      obj3.style = items2;
      let obj4 = { size: "custom", style: max(10210).makeSizeStyle(14) };
      const items3 = [closure_13(max(12623).HeadphonesIcon, obj4), ];
      let obj6 = { variant: "text-xs/semibold", style: { marginLeft: 4 }, maxFontSizeMultiplier: 1, children: audienceCount };
      items3[1] = closure_13(max(4602).Text, obj6);
...
function JoinChannelButton(label) {
  ...
  const callback = noop.useCallback(() => {
    if (null != activeEventOrStageInstanceChannel) {
      ...
      if (obj.isGuildVoice()) {
        tmp4(4813).openGuildVoiceModal(obj);
        const tmp4Result = tmp4(4813);
      } else {
        tmp4(8479).connectAndOpen(obj);
        const tmp4Result2 = tmp4(8479);
      }
    }
  }, items);
  ...
  obj.children = closure_13(channel(5026).Button, obj2);
...
function GuildVoiceEventNotice(channel) {
  ...
  obj4.LocationIcon = channel(5079).getChannelIconComponent(channel);
  obj4.LiveIcon = channel(9876).CalendarIcon;
...
function GuildExternalEventNotice(guildEvent) {
  ...
  if (null != locationFromEvent) {
    ...
    obj2.LiveIcon = tmp(9876).CalendarIcon;
...
function SeeDetailButton(guildEvent) {
  ...
  obj.children = closure_13(guildEvent(5026).Button, obj2);
  return closure_13(View, obj);
}
function GuildLiveStageNotice(channel) {
  channel = channel.channel;
  const tmp2 = useChannelNameDefault(channel);
  const stageParticipants = channel(5482).useStageParticipants(channel.id, channel(5476).StageChannelParticipantNamedIndex.SPEAKER);
...
obj2.react = fn(10217).inlineChannelMentionReact;
...
let closure_31 = noop.memo((arg0) => {
  ...
  if (tmp2Result) {
    ...
    items4[1] = tmp4(tmp7(4602).Text, obj8);
...
let obj3 = { react: fn(10217).inlineChannelReact };
size = fn(2);
...
export default noop.memo((guild) => {
  ...
  const guildActiveEvent = activeEventOrStageInstanceChannel(9753).useGuildActiveEvent(guild.id);
  let obj2 = activeEventOrStageInstanceChannel(9753);
  ...
  if (null != tmp13) {
    ...
    obj7.children = tmp13;
    tmp20 = closure_13(tmp2(5658).Card, obj7);
  }
  return tmp20;
});
...
export const getScaledLiveChannelNoticeHeight = function getScaledLiveChannelNoticeHeight(fontScale, guildLiveChannelNoticeInfo) {
  ...
  if (!hasSpeakers) {
    ...
    const sum = PX_82 + tmp(10208).scaleTextLineHeight(c22, fontScale);
    const tmp5 = PX_82;
    const tmp8 = PX_4;
    const tmpResult = tmp(10208);
    let num2 = 0;
    ...
    tmpResult3 = tmp(1115);
    let num3 = 0;
    const sum2 = sum1 + tmp(10208).scaleTextLineHeight(c23, fontScale);
    if (hasButton) {
      num3 = tmp5 + tmp(5031).SMALL_BUTTON_HEIGHT;
    }
