...
// Dependencies: [19, 17, 5470, 1962, 4629, 4245, 4631, 1963, 1085, 21, 576, 1178, 4597, 10219, 8172, 10210, 1115, 5032, 4607, 4603, 12625, 10212, 1874, 4814, 8481, 4542, 7947, 5027, 4458, 4760, 504, 1114, 5080, 9878, 9798, 9808, 9882, 5483, 5477, 5469, 9755, 5149, 16258, 11007, 5659, 2]
// Exports: getScaledLiveChannelNoticeHeight

// Module 16259 (GuildLiveChannelNotice)
...
function UserSummaryRow(arg0) {
  ...
  ,
    ...
    if (tmp8Result) {
      ...
      obj3.style = items2;
      let obj4 = { size: "custom", style: max(10212).makeSizeStyle(14) };
      const items3 = [closure_13(max(12625).HeadphonesIcon, obj4), ];
      let obj6 = { variant: "text-xs/semibold", style: { marginLeft: 4 }, maxFontSizeMultiplier: 1, children: audienceCount };
      items3[1] = closure_13(max(4603).Text, obj6);
...
function JoinChannelButton(label) {
  ...
  const callback = noop.useCallback(() => {
    if (null != activeEventOrStageInstanceChannel) {
      ...
      if (obj.isGuildVoice()) {
        tmp4(4814).openGuildVoiceModal(obj);
        const tmp4Result = tmp4(4814);
      } else {
        tmp4(8481).connectAndOpen(obj);
        const tmp4Result2 = tmp4(8481);
      }
    }
  }, items);
  ...
  obj.children = closure_13(channel(5027).Button, obj2);
...
function GuildVoiceEventNotice(channel) {
  ...
  obj4.LocationIcon = channel(5080).getChannelIconComponent(channel);
  obj4.LiveIcon = channel(9878).CalendarIcon;
...
function GuildExternalEventNotice(guildEvent) {
  ...
  if (null != locationFromEvent) {
    ...
    obj2.LocationIcon = tmp(9808).LocationIcon;
    obj2.LiveIcon = tmp(9878).CalendarIcon;
...
function SeeDetailButton(guildEvent) {
  ...
  obj.children = closure_13(guildEvent(5027).Button, obj2);
  return closure_13(View, obj);
}
function GuildLiveStageNotice(channel) {
  channel = channel.channel;
  const tmp2 = useChannelNameDefault(channel);
  const stageParticipants = channel(5483).useStageParticipants(channel.id, channel(5477).StageChannelParticipantNamedIndex.SPEAKER);
  ...
  const obj3 = channel(504);
  const stageHasStream = channel(5469).useStageHasStream(channel.id);
  const obj4 = channel(5469);
  const guildActiveEvent = channel(9755).useGuildActiveEvent(channel.guild_id);
  ...
  if (null != guildActiveEvent) {
    channelIconComponent = tmp3(5080).getChannelIconComponent(channel);
    const tmp3Result = tmp3(5080);
  }
  obj6.LocationIcon = channelIconComponent;
  if (null != guildActiveEvent) {
    let StageIcon = tmp3(9878).CalendarIcon;
  } else {
    StageIcon = tmp3(5149).StageIcon;
  }
...
obj2.react = fn(10219).inlineChannelMentionReact;
...
obj.channel = { react: fn(10219).inlineChannelReact };
...
let closure_31 = noop.memo((arg0) => {
  ...
  if (tmp2Result) {
    ...
    items4[1] = tmp4(tmp7(4603).Text, obj8);
...
let obj3 = { react: fn(10219).inlineChannelReact };
size = fn(2);
...
export default noop.memo((guild) => {
  ...
  activeEventOrStageInstanceChannel = activeEventOrStageInstanceChannel(16258).useActiveEventOrStageInstanceChannel(guild.id);
  let obj = activeEventOrStageInstanceChannel(16258);
  const guildActiveEvent = activeEventOrStageInstanceChannel(9755).useGuildActiveEvent(guild.id);
  let obj2 = activeEventOrStageInstanceChannel(9755);
  ...
  if (null != tmp13) {
    ...
    obj7.children = tmp13;
    tmp20 = closure_13(tmp2(5659).Card, obj7);
  }
  return tmp20;
});
...
export const getScaledLiveChannelNoticeHeight = function getScaledLiveChannelNoticeHeight(fontScale, guildLiveChannelNoticeInfo) {
  ...
  if (!hasSpeakers) {
    ...
    const sum = PX_82 + tmp(10210).scaleTextLineHeight(c22, fontScale);
    const tmp5 = PX_82;
    const tmp8 = PX_4;
    const tmpResult = tmp(10210);
    let num2 = 0;
    ...
    tmpResult3 = tmp(1115);
    let num3 = 0;
    const sum2 = sum1 + tmp(10210).scaleTextLineHeight(c23, fontScale);
    if (hasButton) {
      num3 = tmp5 + tmp(5032).SMALL_BUTTON_HEIGHT;
    }
