...
// Dependencies: [19, 17, 4276, 4286, 1372, 7636, 1074, 21, 4638, 576, 6729, 9866, 9852, 4791, 504, 5113, 9854, 9855, 1115, 4634, 5208, 1875, 9869, 9838, 4605, 9569, 1897, 1177, 9851, 2]
// Exports: default

// Module 9868 (EditGuildEventChannelSelection)
import nativeDefault from "native" /* 576 */;
import util from "util" /* 1115 */;
import KeyboardManagerUtilsAll from "KeyboardManagerUtils" /* 1875 */;
import asyncRequireImpl from "asyncRequireImpl" /* 1897 */;
import ActionSheetActionCreatorsDefault from "ActionSheetActionCreators" /* 4605 */;
import StageChannelUpsellDefault from "StageChannelUpsell" /* 9869 */;
...
export default function EditGuildEventChannelSelection(guild) {
  ...
  let tmp = closure_13();
  const inputStyles = guild(6729).useInputStyles({ hasLeadingIcon: true });
  closure_5 = tmp5;
  let obj = guild(6729);
  closure_6 = guild(9866).useGetEventChannelsByType(guild.id, channelType);
  let obj2 = guild(9866);
  guild(9852).useChannelsUserCanStartStageIn(guild);
  const tmp7 = channel(4791)(channel);
  let obj3 = guild(9852);
  ...
  if (null != channel) {
    let channelIcon = tmp2(5113).getChannelIcon(channel);
    const tmp2Result = tmp2(5113);
  } else {
    channelIcon = tmp6(9854);
  }
  if (null != channel) {
    let LocationIcon = tmp2(5113).getChannelIconComponent(channel);
    const tmp2Result2 = tmp2(5113);
  } else {
    LocationIcon = tmp2(9855).LocationIcon;
  }
  ...
  obj6.style = items3;
  const items4 = [closure_11(guild(4634).Heading, { style: tmp.channelTypeText, variant: "text-sm/semibold", color: "text-subtle", children: stringResult }), ];
  ...
  obj8.onPress = function onPress() {
      ...
      if (closure_8) {
        let obj2 = {
          ...
          onCreate(channel) {
                ...
                const result = guild(9838).openCreateOrEditGuildEventModal(closure_1_0, obj2);
              }
        };
    ...
    obj3.openLazy(asyncRequireImpl(9569, tmp.paths), "SelectUpdatesChannel", obj4);
  };
  ...
  const items6 = [tmp12Result, closure_11(guild(4634).Text, { style: tmp.channelNameText, variant: "text-md/medium", color: "interactive-text-active", children: tmp7 }), ];
  ...
  const tmp11 = View;
  items6[2] = closure_11(guild(1177).Icon, { source: channel(9851) });
  obj8.children = items6;
  items4[1] = closure_12(guild(5208).PressableOpacity, obj8);
