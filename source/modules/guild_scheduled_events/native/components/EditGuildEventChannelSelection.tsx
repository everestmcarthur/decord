...
// Dependencies: [19, 17, 4245, 4255, 1371, 7599, 1074, 21, 4606, 576, 6692, 9817, 9803, 4759, 504, 5079, 9805, 9806, 1114, 4602, 5174, 1874, 9820, 9789, 4573, 9517, 1896, 1178, 9802, 2]
// Exports: default
...
export default function EditGuildEventChannelSelection(guild) {
  ...
  let tmp = closure_13();
  const inputStyles = guild(6692).useInputStyles({ hasLeadingIcon: true });
  closure_5 = tmp5;
  let obj = guild(6692);
  closure_6 = guild(9817).useGetEventChannelsByType(guild.id, channelType);
  let obj2 = guild(9817);
  guild(9803).useChannelsUserCanStartStageIn(guild);
  ...
  if (null != channel) {
    let channelIcon = tmp2(5079).getChannelIcon(channel);
    const tmp2Result = tmp2(5079);
  } else {
    channelIcon = tmp6(9805);
  }
  if (null != channel) {
    let LocationIcon = tmp2(5079).getChannelIconComponent(channel);
    const tmp2Result2 = tmp2(5079);
  } else {
  ...
  obj6.style = items3;
  const items4 = [closure_11(guild(4602).Heading, { style: tmp.channelTypeText, variant: "text-sm/semibold", color: "text-subtle", children: stringResult }), ];
  ...
  obj8.onPress = function onPress() {
      ...
      if (closure_8) {
        let obj2 = {
          ...
          onCreate(channel) {
                ...
                const result = guild(9789).openCreateOrEditGuildEventModal(closure_1_0, obj2);
              }
        };
    ...
    obj3.openLazy(asyncRequireImpl(9517, tmp.paths), "SelectUpdatesChannel", obj4);
  };
  ...
  const items6 = [tmp12Result, closure_11(guild(4602).Text, { style: tmp.channelNameText, variant: "text-md/medium", color: "interactive-text-active", children: tmp7 }), ];
  ...
  items4[1] = closure_12(guild(5174).PressableOpacity, obj8);
