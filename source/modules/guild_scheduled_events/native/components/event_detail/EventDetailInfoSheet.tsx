...
// Dependencies: [19, 17, 4598, 2021, 1979, 4629, 1371, 1963, 9790, 1085, 21, 4606, 576, 5575, 4573, 504, 9795, 9872, 9871, 9863, 9867, 4602, 1114, 1178, 4451, 8496, 5484, 9887, 2]
// Exports: closeGuildEventInfoActionSheet, default

// Module 9886 (EventDetailInfoSheet)
import nativeDefault from "native" /* 576 */;
import native from "native" /* 1178 */;
import UserUtilsDefault from "UserUtils" /* 4451 */;
import ActionSheetActionCreatorsDefault from "ActionSheetActionCreators" /* 4573 */;
import useGuildScheduledEventUserCountDefault from "useGuildScheduledEventUserCount" /* 9871 */;
...
export default function EventDetailInfoSheet(guildEvent) {
  ...
  obj6.style = items7;
  const items8 = [closure_12(guildEvent(9863).GuildEventCardHeader, { event: guildEvent, style: tmp.eventHeader, showUserCount: false, showEndDate: true, showCreator: false, recurrenceId }), closure_12(guildEvent(9863).GuildEventCardTitle, { event: guildEvent, textStyle: tmp.eventTitle, style: tmp.eventTitleContainer }), closure_12(guildEvent(9863).GuildEventCardSimpleGuildInfo, { event: guildEvent, textStyle: tmp.guildTextStyle }), closure_12(guildEvent(9863).GuildEventSimpleLocation, { event: guildEvent }), , , , , , ];
  const obj11 = { style: tmp.interestedContainer, children: null };
  const items9 = [closure_12(guildEvent(9867).BellIcon, { size: "sm", style: tmp.interestedIcon }), ];
  ...
  items9[1] = closure_12(guildEvent(4602).Text, obj13);
  ...
  if (tmp11Result) {
    ...
    items10[1] = tmp13(tmp2(4602).Text, obj16);
    obj14.children = items10;
    tmp11Result = tmp11(tmp12, obj14);
  }
  items8[5] = tmp11Result;
  items8[6] = closure_12(guildEvent(9863).GuildEventCardDescription, { event: guildEvent, style: tmp.eventDescriptionContainer });
  ...
  if (hasItem) {
    ...
    const items11 = [tmp2(8496).DividerPosition.TOP, tmp2(8496).DividerPosition.BOTTOM];
    ...
    hasItem = tmp13(tmp7(8496), obj19);
    const tmp7Result = tmp7(8496);
  }
  items8[7] = hasItem;
  const obj20 = { direction: "horizontal", style: tmp.controlsContainer, children: null };
  const items12 = [
    closure_12(guildEvent(9863).GuildEventCardPrimaryAction, {
      event: guildEvent,
      onCloseAction() {
  ...
  if (stateFromStores1) {
    ...
    tmp13Result = tmp13(tmp2(9863).GuildEventShareAction, obj22);
  }
  items12[1] = tmp13Result;
  items12[2] = closure_12(guildEvent(9863).GuildEventModeratorAction, { event: guildEvent, recurrenceId });
  obj20.children = items12;
  items8[8] = closure_13(guildEvent(5484).ButtonGroup, obj20);
  ...
  if (tmp13Result2) {
    ...
    tmp13Result2 = tmp13(tmp7(9887), obj23);
  }
