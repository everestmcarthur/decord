...
// Dependencies: [5, 32, 19, 17, 502, 1958, 2022, 1980, 4278, 1372, 7633, 1964, 1074, 1085, 21, 4639, 576, 9905, 9907, 8205, 1115, 9830, 7296, 4337, 9186, 4583, 504, 9803, 8799, 4590, 9908, 9909, 9797, 5605, 7449, 9910, 4984, 9835, 5078, 4846, 1370, 1478, 1397, 9911, 9912, 1177, 5173, 4574, 9801, 9800, 9914, 5207, 4635, 9917, 9903, 4792, 9837, 9901, 9902, 5669, 2]
...
// Module 9904 (GuildEventCardComponents)
...
import GroupIcon from "GroupIcon" /* 5173 */;
...
import guildEventDetailsParser from "guildEventDetailsParser" /* 9903 */;
import useCanInviteForGuildEventDefault from "useCanInviteForGuildEvent" /* 9905 */;
import ThrottledButtonDefault from "ThrottledButton" /* 9909 */;
import useGuildScheduledEventUserCountDefault from "useGuildScheduledEventUserCount" /* 9912 */;
...
const getGuildEventImageDefault = tmp2(9911);
require = fn;
...
export const GuildEventShareAction = function GuildEventShareAction(event) {
  ...
  importDefault = tmp3;
  dependencyMap = event(9907).SHARE_EVENT_DETAILS_LINK({ guildId: event.guild_id, guildEventId: event.id });
  ...
  obj3.onPress = function onPress() {
    if (closure_1) {
      tmp(9830).openShareEvent(event);
      const tmpResult = tmp(9830);
    } else {
      tmp(7296).copy(closure_2);
      const tmpResult2 = tmp(7296);
      ToastUtils.presentLinkCopied();
    }
  };
  obj3.icon = importDefault(tmp3 ? 9186 : 4583);
  return closure_22(event(8205).IconButton, obj3);
};
...
export const GuildEventSimpleLocation = function GuildEventSimpleLocation(event) {
  ...
  let tmp7 = stateFromStores(4792)(stateFromStores);
  const obj2 = channel_id(504);
  const tmp6 = stateFromStores;
  const locationFromEvent = channel_id(9837).getLocationFromEvent(event);
  if (null == stateFromStores) {
    if (null == locationFromEvent) {
      return null;
    }
  }
  const obj3 = channel_id(9837);
  const eventLocationIconComponent = channel_id(9901).getEventLocationIconComponent(event, stateFromStores, stateFromStores1);
  ...
  if (tmp7 == null) {
    ...
    if (null != locationFromEvent) {
      ...
      result = tmp2(9903).guildEventLocationParser(locationFromEvent, true, obj8);
      const tmp2Result2 = tmp2(9903);
    }
    tmp7 = result;
  }
  obj6.children = tmp7;
  items4[1] = closure_22(channel_id(4635).Text, obj6);
...
export const GuildEventCardSimpleGuildInfo = function GuildEventCardSimpleGuildInfo(arg0) {
  ...
  if (null != stateFromStores) {
    ...
    obj2.style = items2;
    const obj3 = { guild: stateFromStores, size: tmp2(5669).GuildIconSizes.XSMALL_20, style: tmp.guildIcon };
    ...
    obj4.children = closure_22(tmp2(4635).Text, obj5);
