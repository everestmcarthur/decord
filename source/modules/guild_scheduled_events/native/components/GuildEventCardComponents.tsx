...
// Dependencies: [5, 32, 19, 17, 502, 1958, 2022, 1980, 4278, 1372, 7640, 1964, 1074, 1085, 21, 4640, 576, 9924, 9926, 8057, 1115, 9849, 7303, 4337, 9208, 4583, 504, 9822, 8060, 4590, 9927, 9928, 9816, 5608, 7456, 9929, 4985, 9854, 5081, 4847, 1370, 1478, 1397, 9930, 9931, 1177, 5176, 4574, 9820, 9819, 9933, 5210, 4636, 9936, 9922, 4793, 9856, 9920, 9921, 5672, 2]
...
// Module 9923 (GuildEventCardComponents)
...
import GroupIcon from "GroupIcon" /* 5176 */;
...
import guildEventDetailsParser from "guildEventDetailsParser" /* 9922 */;
import useCanInviteForGuildEventDefault from "useCanInviteForGuildEvent" /* 9924 */;
import ThrottledButtonDefault from "ThrottledButton" /* 9928 */;
import useGuildScheduledEventUserCountDefault from "useGuildScheduledEventUserCount" /* 9931 */;
...
const getGuildEventImageDefault = tmp2(9930);
require = fn;
...
export const GuildEventShareAction = function GuildEventShareAction(event) {
  ...
  importDefault = tmp3;
  dependencyMap = event(9926).SHARE_EVENT_DETAILS_LINK({ guildId: event.guild_id, guildEventId: event.id });
  ...
  obj3.onPress = function onPress() {
    if (closure_1) {
      tmp(9849).openShareEvent(event);
      const tmpResult = tmp(9849);
    } else {
      tmp(7303).copy(closure_2);
      const tmpResult2 = tmp(7303);
      ToastUtils.presentLinkCopied();
    }
  };
  obj3.icon = importDefault(tmp3 ? 9208 : 4583);
  return closure_22(event(8057).IconButton, obj3);
};
...
export const GuildEventIndicateInterestAction = function GuildEventIndicateInterestAction(event) {
  ...
  if (first) {
    let BellIcon = tmp(4590).CheckmarkLargeIcon;
  } else {
    BellIcon = tmp(9927).BellIcon;
  }
...
export const GuildEventSimpleLocation = function GuildEventSimpleLocation(event) {
  ...
  let tmp7 = stateFromStores(4793)(stateFromStores);
  const obj2 = channel_id(504);
  const tmp6 = stateFromStores;
  const locationFromEvent = channel_id(9856).getLocationFromEvent(event);
  if (null == stateFromStores) {
    if (null == locationFromEvent) {
      return null;
    }
  }
  const obj3 = channel_id(9856);
  const eventLocationIconComponent = channel_id(9920).getEventLocationIconComponent(event, stateFromStores, stateFromStores1);
  ...
  if (tmp7 == null) {
    ...
    if (null != locationFromEvent) {
      ...
      result = tmp2(9922).guildEventLocationParser(locationFromEvent, true, obj8);
      const tmp2Result2 = tmp2(9922);
    }
    tmp7 = result;
  }
  obj6.children = tmp7;
  items4[1] = closure_22(channel_id(4636).Text, obj6);
...
export const GuildEventCardSimpleGuildInfo = function GuildEventCardSimpleGuildInfo(arg0) {
  ...
  if (null != stateFromStores) {
    ...
    obj2.style = items2;
    const obj3 = { guild: stateFromStores, size: tmp2(5672).GuildIconSizes.XSMALL_20, style: tmp.guildIcon };
    ...
    obj4.children = closure_22(tmp2(4636).Text, obj5);
