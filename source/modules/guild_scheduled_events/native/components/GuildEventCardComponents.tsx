...
// Dependencies: [5, 32, 19, 17, 502, 1958, 2022, 1980, 4276, 1372, 7636, 1964, 1074, 1085, 21, 4638, 576, 9913, 9915, 8214, 1115, 9838, 7299, 4335, 9197, 4581, 504, 9811, 8808, 4588, 9916, 9917, 9805, 5606, 7452, 9918, 4983, 9843, 5079, 4845, 1370, 1478, 1397, 9919, 9920, 1177, 5174, 4572, 9809, 9808, 9922, 5208, 4634, 9925, 9911, 4791, 9845, 9909, 9910, 5670, 2]
...
// Module 9912 (GuildEventCardComponents)
...
import GroupIcon from "GroupIcon" /* 5174 */;
...
import guildEventDetailsParser from "guildEventDetailsParser" /* 9911 */;
import useCanInviteForGuildEventDefault from "useCanInviteForGuildEvent" /* 9913 */;
import ThrottledButtonDefault from "ThrottledButton" /* 9917 */;
import useGuildScheduledEventUserCountDefault from "useGuildScheduledEventUserCount" /* 9920 */;
...
import GuildScheduledEventStore_mod from "GuildScheduledEventStore" /* 7636 */;

const require = globalThis.__r;

const getGuildEventImageDefault = tmp2(9919);
require = fn;
...
export const GuildEventShareAction = function GuildEventShareAction(event) {
  ...
  importDefault = tmp3;
  dependencyMap = event(9915).SHARE_EVENT_DETAILS_LINK({ guildId: event.guild_id, guildEventId: event.id });
  ...
  obj3.onPress = function onPress() {
    if (closure_1) {
      tmp(9838).openShareEvent(event);
      const tmpResult = tmp(9838);
    } else {
      tmp(7299).copy(closure_2);
      const tmpResult2 = tmp(7299);
      ToastUtils.presentLinkCopied();
    }
  };
  obj3.icon = importDefault(tmp3 ? 9197 : 4581);
  return closure_22(event(8214).IconButton, obj3);
};
...
export const GuildEventIndicateInterestAction = function GuildEventIndicateInterestAction(event) {
  ...
  if (first) {
    let BellIcon = tmp(4588).CheckmarkLargeIcon;
  } else {
    BellIcon = tmp(9916).BellIcon;
  }
...
export const GuildEventSimpleLocation = function GuildEventSimpleLocation(event) {
  ...
  let tmp7 = stateFromStores(4791)(stateFromStores);
  const obj2 = channel_id(504);
  const tmp6 = stateFromStores;
  const locationFromEvent = channel_id(9845).getLocationFromEvent(event);
  if (null == stateFromStores) {
    if (null == locationFromEvent) {
      return null;
    }
  }
  const obj3 = channel_id(9845);
  const eventLocationIconComponent = channel_id(9909).getEventLocationIconComponent(event, stateFromStores, stateFromStores1);
  ...
  if (tmp7 == null) {
    ...
    if (null != locationFromEvent) {
      ...
      result = tmp2(9911).guildEventLocationParser(locationFromEvent, true, obj8);
      const tmp2Result2 = tmp2(9911);
    }
    tmp7 = result;
  }
  obj6.children = tmp7;
  items4[1] = closure_22(channel_id(4634).Text, obj6);
...
export const GuildEventCardSimpleGuildInfo = function GuildEventCardSimpleGuildInfo(arg0) {
  ...
  if (null != stateFromStores) {
    ...
    obj2.style = items2;
    const obj3 = { guild: stateFromStores, size: tmp2(5670).GuildIconSizes.XSMALL_20, style: tmp.guildIcon };
    ...
    obj4.children = closure_22(tmp2(4634).Text, obj5);
