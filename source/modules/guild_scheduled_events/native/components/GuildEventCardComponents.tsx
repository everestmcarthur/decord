...
// Dependencies: [5, 32, 19, 17, 502, 1957, 2021, 1979, 4212, 1371, 7540, 1963, 1074, 1085, 21, 4574, 576, 9805, 9807, 8113, 1114, 9730, 7203, 4271, 9086, 4518, 504, 9703, 8706, 4525, 9808, 9809, 9697, 5534, 7356, 9810, 4919, 9735, 5008, 4781, 1369, 1477, 1396, 9811, 9812, 1178, 5103, 4509, 9701, 9700, 9814, 5137, 4570, 9817, 9803, 4727, 9737, 9801, 9802, 5598, 2]
...
// Module 9804 (GuildEventCardComponents)
...
import guildEventDetailsParser from "guildEventDetailsParser" /* 9803 */;
import useCanInviteForGuildEventDefault from "useCanInviteForGuildEvent" /* 9805 */;
import ThrottledButtonDefault from "ThrottledButton" /* 9809 */;
...
const getGuildEventImageDefault = tmp2(9811);
require = fn;
...
export const GuildEventShareAction = function GuildEventShareAction(event) {
  ...
  importDefault = tmp3;
  dependencyMap = event(9807).SHARE_EVENT_DETAILS_LINK({ guildId: event.guild_id, guildEventId: event.id });
  ...
  obj3.onPress = function onPress() {
    if (closure_1) {
      tmp(9730).openShareEvent(event);
      const tmpResult = tmp(9730);
    } else {
      tmp(7203).copy(closure_2);
      const tmpResult2 = tmp(7203);
      ToastUtils.presentLinkCopied();
    }
  };
  obj3.icon = importDefault(tmp3 ? 9086 : 4518);
  return closure_22(event(8113).IconButton, obj3);
};
...
export const GuildEventSimpleLocation = function GuildEventSimpleLocation(event) {
  ...
  const locationFromEvent = channel_id(9737).getLocationFromEvent(event);
  if (null == stateFromStores) {
    if (null == locationFromEvent) {
      return null;
    }
  }
  const obj3 = channel_id(9737);
  const eventLocationIconComponent = channel_id(9801).getEventLocationIconComponent(event, stateFromStores, stateFromStores1);
  ...
  if (tmp7 == null) {
    ...
    if (null != locationFromEvent) {
      ...
      result = tmp2(9803).guildEventLocationParser(locationFromEvent, true, obj8);
      const tmp2Result2 = tmp2(9803);
    }
    tmp7 = result;
  }
  obj6.children = tmp7;
  items4[1] = closure_22(channel_id(4570).Text, obj6);
...
export const GuildEventCardSimpleGuildInfo = function GuildEventCardSimpleGuildInfo(arg0) {
  ...
  if (null != stateFromStores) {
    ...
    obj2.style = items2;
    const obj3 = { guild: stateFromStores, size: tmp2(5598).GuildIconSizes.XSMALL_20, style: tmp.guildIcon };
    ...
    obj4.children = closure_22(tmp2(4570).Text, obj5);
