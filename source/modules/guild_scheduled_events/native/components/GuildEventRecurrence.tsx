...
// Dependencies: [19, 17, 1958, 1980, 7722, 21, 4722, 576, 504, 9914, 9916, 9913, 9910, 1115, 5292, 10017, 4718, 9943, 1177, 10045, 2]
// Exports: default

// Module 10044 (GuildEventRecurrence)
...
export default function GuildEventRecurrence(recurrenceId) {
  ...
  let obj2 = guildEventId(9914)(recurrenceId, id);
  const tmp6 = guildEventId(9914);
  const items1 = [c6];
  ...
  closure_5 = recurrenceId(9916).useManageResourcePermissions(stateFromStores2).canManageGuildEvent(stateFromStores);
  const tmp2Result5 = recurrenceId(9916);
  const eventScheduleById = recurrenceId(9913).useEventScheduleById(guildEventId, recurrenceId);
  ...
  if (null == stateFromStores) {
    ...
    obj4.children = closure_8(tmp2(10017).GuildEventCardHeader, obj5);
    const items5 = [closure_8(tmp2(5292).PressableOpacity, obj4), ];
    ...
    if (tmp21Result) {
      ...
      tmp21Result = tmp21(tmp2(4718).Text, obj7);
    }
    ...
    const obj9 = { source: tmp5(10045), size: tmp2(1177).Icon.Sizes.REFRESH_SMALL_16, style: tmp.secondarySmallIcon };
    obj8.children = closure_8(tmp2(1177).Icon, obj9);
    items6[1] = closure_8(tmp2(5292).PressableOpacity, obj8);
    obj6.children = items6;
    items5[1] = closure_9(stateFromStores1, obj6);
    obj3.children = items5;
    return closure_9(stateFromStores1, obj3);
  }
  const tmp2Result6 = recurrenceId(9913);
};
