...
// Dependencies: [32, 19, 7722, 1964, 21, 4722, 1115, 4314, 9910, 1484, 4718, 5144, 1875, 9949, 4433, 9953, 10010, 9955, 2]
// Exports: default

// Module 10012 (EditGuildEventDetails)
...
let constants = fn(1964).GuildScheduledEventEntityTypes;
const jsxProd = fn(21);
...
export default function EditGuildEventDetails(guildEvent) {
  ...
  memo = memo.useMemo(() => _modDef4314(scheduledStartTime), items);
  const items1 = [scheduledEndTime, scheduledStartTime];
  const memo1 = memo.useMemo(() => {
    if (null != scheduledEndTime) {
      let addResult = _modDef4314(tmp);
    } else {
      addResult = _modDef4314(scheduledStartTime).add(1, "hour");
      const obj = _modDef4314(scheduledStartTime);
    }
    return addResult;
  }, items1);
  const tmp = c11();
  [c7, c8] = recurrenceRule(memo.useState(() => ScheduleUtils.recurrenceRuleToOption(_modDef4314(scheduledStartTime), recurrenceRule)), 2);
  ...
  const memo4 = memo.useMemo(() => _modDef4314(memo).add(15, "minutes"), items2);
  ...
  closure_10 = tmp10;
  const tmp4 = recurrenceRule(memo.useState(() => ScheduleUtils.recurrenceRuleToOption(_modDef4314(scheduledStartTime), recurrenceRule)), 2);
  ...
  const obj8 = {
    ...
    onChange(toISOString) {
      if (closure_10) {
        ...
        if (isBeforeResult) {
          const obj2 = _modDef4314(toISOString);
          obj.scheduledEndTime = _modDef4314(toISOString).add(1, "hour").toISOString();
          const addResult = _modDef4314(toISOString).add(1, "hour");
        }
