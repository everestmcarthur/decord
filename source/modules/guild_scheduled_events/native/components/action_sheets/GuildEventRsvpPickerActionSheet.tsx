...
// Dependencies: [32, 19, 17, 1964, 21, 4638, 576, 9846, 1115, 7260, 7259, 7233, 5771, 5774, 5060, 9838, 4605, 2]
// Exports: default

// Module 9959 (GuildEventRsvpPickerActionSheet)
...
export default function GuildEventRsvpPickerActionSheet(event) {
  ...
  closure_5 = undefined;
  let tmp = closure_9();
  const tmp4 = _slicedToArray(defaultValue.useState(event(9846).ResponseOptions.SERIES), 2);
  defaultValue = tmp4[0];
  const existingRsvp = event(9846).getExistingRsvp(event.id, null);
  let response;
  ...
  const obj2 = { header: closure_7(event(7259).BottomSheetTitleHeader, { title: stringResult }), children: null };
  ...
  const obj = event(9846);
  const responseOptions = event(9846).getResponseOptions();
  ...
  const items = [closure_7(event(5771).TableRadioGroup, obj4), ];
  ...
  obj5.children = closure_7(event(5060).Button, obj6);
  items[1] = closure_7(closure_5, obj5);
  obj3.children = items;
  obj2.children = closure_8(event(7233).SafeAreaPaddingView, obj3);
  return closure_7(event(7260).BottomSheet, obj2);
};
