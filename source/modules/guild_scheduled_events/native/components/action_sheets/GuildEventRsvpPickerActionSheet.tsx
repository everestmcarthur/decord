...
// Dependencies: [32, 19, 17, 1963, 21, 4606, 576, 9797, 1114, 7223, 7222, 7196, 5736, 5739, 5026, 9789, 4573, 2]
// Exports: default

// Module 9910 (GuildEventRsvpPickerActionSheet)
...
export default function GuildEventRsvpPickerActionSheet(event) {
  ...
  closure_5 = undefined;
  let tmp = closure_9();
  const tmp4 = _slicedToArray(defaultValue.useState(event(9797).ResponseOptions.SERIES), 2);
  defaultValue = tmp4[0];
  const existingRsvp = event(9797).getExistingRsvp(event.id, null);
  let response;
  ...
  const obj2 = { header: closure_7(event(7222).BottomSheetTitleHeader, { title: stringResult }), children: null };
  ...
  const obj = event(9797);
  const responseOptions = event(9797).getResponseOptions();
  ...
  const items = [closure_7(event(5736).TableRadioGroup, obj4), ];
  ...
  obj5.children = closure_7(event(5026).Button, obj6);
  items[1] = closure_7(closure_5, obj5);
  obj3.children = items;
  obj2.children = closure_8(event(7196).SafeAreaPaddingView, obj3);
  return closure_7(event(7223).BottomSheet, obj2);
};
