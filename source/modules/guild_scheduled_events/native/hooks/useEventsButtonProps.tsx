...
// Dependencies: [19, 4655, 4821, 4822, 504, 9813, 5144, 5657, 9849, 4607, 12527, 1897, 1115, 12531, 2]
// Exports: default

// Module 12526 (useEventsButtonProps)
import asyncRequireImpl from "asyncRequireImpl" /* 1897 */;
import ActionSheetActionCreatorsDefault from "ActionSheetActionCreators" /* 4607 */;
import useGuildScheduledEventsDefault from "useGuildScheduledEvents" /* 9813 */;
...
export default function useEventsButtonProps(id) {
  ...
  const handlePress = noop.useCallback(() => {
    if (obj.shouldShowMembershipVerificationGate(user.id)) {
      let result = tmp(5657).openMemberVerificationModal(tmp3.id);
      const tmpResult = tmp(5657);
    } else {
      result = tmp(9849).openGuildEventListActionSheet(tmp3);
  ...
  const handleLongPress = noop.useCallback(() => {
    ...
    obj.openLazy(asyncRequireImpl(12527, dependencyMap.paths), "UpcomingEventsLongPress-" + user.id, { guildId: user.id });
  }, items4);
  if (arr4.length > 0) {
