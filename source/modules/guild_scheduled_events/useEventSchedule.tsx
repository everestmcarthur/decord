...
// Dependencies: [7633, 9797, 9801, 504, 2]
// Exports: default, getEventSchedule, useEventScheduleById

// Module 9800 (useEventSchedule)
import ScheduleUtils from "ScheduleUtils" /* 9797 */;
import useEventException from "useEventException" /* 9801 */;
...
export const useEventScheduleById = function useEventScheduleById(guildEventId, recurrenceId) {
  ...
  if (recurrenceId == null) {
    nextRecurrenceIdInEvent = tmp2(9797).getNextRecurrenceIdInEvent(stateFromStores);
    const tmp2Result = tmp2(9797);
  }
  ...
  if (tmp5 == stateFromStores) {
    ...
    if (tmp5 != stateFromStores.recurrence_rule) {
      if (tmp5 != nextRecurrenceIdInEvent) {
        const baseScheduleForRecurrence = tmp2(9797).getBaseScheduleForRecurrence(nextRecurrenceIdInEvent, stateFromStores);
        const tmp2Result3 = tmp2(9797);
        const scheduleForRecurrenceWithException = tmp2(9797).getScheduleForRecurrenceWithException(baseScheduleForRecurrence, tmp7);
        ({ startDate, endDate } = scheduleForRecurrenceWithException);
...
export const getEventSchedule = function getEventSchedule(guildEvent, recurrenceId) {
  ...
  if (null != guildEvent.recurrence_rule) {
    if (null != recurrenceId) {
      const baseScheduleForRecurrence = tmp(9797).getBaseScheduleForRecurrence(recurrenceId, guildEvent);
      const tmpResult = tmp(9797);
      const scheduleForRecurrenceWithException = tmp(9797).getScheduleForRecurrenceWithException(baseScheduleForRecurrence, tmp4);
      ({ startDate, endDate } = scheduleForRecurrenceWithException);
