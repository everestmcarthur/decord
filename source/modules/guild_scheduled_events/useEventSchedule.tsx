...
// Dependencies: [7599, 9756, 9760, 504, 2]
...
export const useEventScheduleById = function useEventScheduleById(guildEventId, recurrenceId) {
  ...
  if (recurrenceId == null) {
    nextRecurrenceIdInEvent = tmp2(9756).getNextRecurrenceIdInEvent(stateFromStores);
    const tmp2Result = tmp2(9756);
  }
  ...
  if (tmp5 == stateFromStores) {
    ...
    if (tmp5 != stateFromStores.recurrence_rule) {
      if (tmp5 != nextRecurrenceIdInEvent) {
        const baseScheduleForRecurrence = tmp2(9756).getBaseScheduleForRecurrence(nextRecurrenceIdInEvent, stateFromStores);
        const tmp2Result3 = tmp2(9756);
...
export const getEventSchedule = function getEventSchedule(guildEvent, recurrenceId) {
  ...
  if (null != guildEvent.recurrence_rule) {
    if (null != recurrenceId) {
      const baseScheduleForRecurrence = tmp(9756).getBaseScheduleForRecurrence(recurrenceId, guildEvent);
      const tmpResult = tmp(9756);
      const scheduleForRecurrenceWithException = tmp(9756).getScheduleForRecurrenceWithException(baseScheduleForRecurrence, tmp4);
      ({ startDate, endDate } = scheduleForRecurrenceWithException);
