...
// Dependencies: [7636, 9805, 9809, 504, 2]
// Exports: default, getEventSchedule, useEventScheduleById

// Module 9808 (useEventSchedule)
...
export const useEventScheduleById = function useEventScheduleById(guildEventId, recurrenceId) {
  ...
  if (recurrenceId == null) {
    nextRecurrenceIdInEvent = tmp2(9805).getNextRecurrenceIdInEvent(stateFromStores);
    const tmp2Result = tmp2(9805);
  }
  ...
  if (tmp5 == stateFromStores) {
    ...
    if (tmp5 != stateFromStores.recurrence_rule) {
      if (tmp5 != nextRecurrenceIdInEvent) {
        const baseScheduleForRecurrence = tmp2(9805).getBaseScheduleForRecurrence(nextRecurrenceIdInEvent, stateFromStores);
        const tmp2Result3 = tmp2(9805);
        const scheduleForRecurrenceWithException = tmp2(9805).getScheduleForRecurrenceWithException(baseScheduleForRecurrence, tmp7);
        ({ startDate, endDate } = scheduleForRecurrenceWithException);
...
export const getEventSchedule = function getEventSchedule(guildEvent, recurrenceId) {
  ...
  if (null != guildEvent.recurrence_rule) {
    if (null != recurrenceId) {
      const baseScheduleForRecurrence = tmp(9805).getBaseScheduleForRecurrence(recurrenceId, guildEvent);
      const tmpResult = tmp(9805);
      const scheduleForRecurrenceWithException = tmp(9805).getScheduleForRecurrenceWithException(baseScheduleForRecurrence, tmp4);
      ({ startDate, endDate } = scheduleForRecurrenceWithException);
