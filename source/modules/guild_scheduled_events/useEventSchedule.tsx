...
// Dependencies: [7722, 9910, 9914, 504, 2]
// Exports: default, getEventSchedule, useEventScheduleById

// Module 9913 (useEventSchedule)
  ...
  if (tmp5 == stateFromStores) {
    ...
    if (tmp5 != stateFromStores.recurrence_rule) {
      if (tmp5 != nextRecurrenceIdInEvent) {
        const baseScheduleForRecurrence = tmp2(9910).getBaseScheduleForRecurrence(nextRecurrenceIdInEvent, stateFromStores);
        const tmp2Result3 = tmp2(9910);
        const scheduleForRecurrenceWithException = tmp2(9910).getScheduleForRecurrenceWithException(baseScheduleForRecurrence, tmp7);
        ({ startDate, endDate } = scheduleForRecurrenceWithException);
...
export const getEventSchedule = function getEventSchedule(guildEvent, recurrenceId) {
  ...
  if (null != guildEvent.recurrence_rule) {
    if (null != recurrenceId) {
      const baseScheduleForRecurrence = tmp(9910).getBaseScheduleForRecurrence(recurrenceId, guildEvent);
      const tmpResult = tmp(9910);
      const scheduleForRecurrenceWithException = tmp(9910).getScheduleForRecurrenceWithException(baseScheduleForRecurrence, tmp4);
      ({ startDate, endDate } = scheduleForRecurrenceWithException);
