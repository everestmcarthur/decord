...
// Function ID: 9698
// Name: ScheduleUtils
// Dependencies: [1371, 9698, 1963, 9699, 1114, 4166, 4255, 11, 1090, 12, 2]
...
let items = [fn(9699).RRule.MO.weekday, fn(9699).RRule.TU.weekday, fn(9699).RRule.WE.weekday, fn(9699).RRule.TH.weekday, fn(9699).RRule.FR.weekday];
let items1 = [fn(9699).RRule.SU.weekday, fn(9699).RRule.MO.weekday, fn(9699).RRule.TU.weekday, fn(9699).RRule.WE.weekday, fn(9699).RRule.TH.weekday];
let items2 = [fn(9699).RRule.TU.weekday, fn(9699).RRule.WE.weekday, fn(9699).RRule.TH.weekday, fn(9699).RRule.FR.weekday, fn(9699).RRule.SA.weekday];
...
const items6 = [fn(9699).RRule.SU.weekday, fn(9699).RRule.MO.weekday, fn(9699).RRule.TU.weekday, fn(9699).RRule.WE.weekday, fn(9699).RRule.TH.weekday, fn(9699).RRule.FR.weekday, fn(9699).RRule.SA.weekday];
...
export const getInitialEventEndDate = function getInitialEventEndDate(arg0) {
  ...
  return tmp(4166)(addResult).add(60 - result, "minutes").seconds(0);
};
...
export const getEventTimeData = function getEventTimeData(scheduled_start_time, toISOStringResult1, arg2) {
  ...
  if (null != toISOStringResult1) {
    if ("" !== toISOStringResult1) {
      const obj3 = tmp3(4166)(toISOStringResult1);
    }
  }
  ...
  if (result <= 1) {
    ...
    if (null == obj3) {
      ...
      obj5.upcomingEvent = obj2 <= tmp3(4166)().add(1, "hour");
      const obj10 = tmp3(4166)();
      obj5.withinStartWindow = obj2 <= tmp3(4166)().add(15, "minute");
      ...
      formatResult = tmp6(4255).dateFormat(obj3, str6);
      const tmp6Result3 = tmp6(4255);
    }
  }
...
export const getBaseScheduleForRecurrence = function getBaseScheduleForRecurrence(nextRecurrenceIdInEvent, guildEvent) {
  ...
  if (null != scheduled_start_time) {
    ...
    if (null != scheduled_end_time) {
      obj.endDate = tmp2(4166)(scheduled_end_time);
      tmp = obj;
    }
    tmp2 = importDefault;
  }
...
export const getScheduleFromEventData = function getScheduleFromEventData(arg0) {
  ...
  if (null != scheduledStartTime) {
    ...
    if (null != scheduledEndTime) {
      obj.endDate = tmp2(4166)(scheduledEndTime);
      tmp = obj;
    }
    tmp2 = importDefault;
  }
      ...
      if (uTCHours === date1.getUTCHours()) {
        ...
        if (uTCMinutes === date1.getUTCMinutes()) {
          ...
          if (uTCSeconds === date1.getUTCSeconds()) {
            ...
            } else if (tmp(9699).RRule.YEARLY === frequency) {
...
export const recurrenceOptionToRecurrenceRule = function recurrenceOptionToRecurrenceRule(c7, toDate) {
  ...
  if (RecurrenceOptions.NONE !== c7) {
    if (tmp8.WEEKLY === c7) {
      const obj = { dtstart: toDateResult2, freq: tmp4(9699).RRule.WEEKLY };
      rRule = new tmp4(9699).RRule(obj);
    } else if (tmp8.BIWEEKLY === c7) {
      const obj2 = { dtstart: toDateResult2, freq: tmp4(9699).RRule.WEEKLY, interval: 2 };
      rRule = new tmp4(9699).RRule(obj2);
    } else if (tmp8.MONTHLY === c7) {
      const obj3 = { dtstart: toDateResult2, freq: tmp4(9699).RRule.MONTHLY, byweekday: null };
      items = [weekday.nth(rounded)];
      obj3.byweekday = items;
      rRule = new tmp4(9699).RRule(obj3);
    } else if (tmp8.YEARLY === c7) {
      const obj4 = { dtstart: toDateResult2, freq: tmp4(9699).RRule.YEARLY };
      rRule = new tmp4(9699).RRule(obj4);
    } else if (tmp8.DAILY === c7) {
      const obj5 = { dtstart: toDateResult2, freq: tmp4(9699).RRule.DAILY };
      rRule = new tmp4(9699).RRule(obj5);
    } else if (tmp8.WEEKDAY_ONLY === c7) {
      const obj6 = { dtstart: toDateResult2, freq: tmp4(9699).RRule.DAILY, byweekday: tmp };
      rRule = new tmp4(9699).RRule(obj6);
    } else if (tmp8.WEEKEND_ONLY === c7) {
      const obj7 = { dtstart: toDateResult2, freq: tmp4(9699).RRule.DAILY, byweekday: tmp2 };
      rRule = new tmp4(9699).RRule(obj7);
    }
  }
  if (null == rRule) {
    return null;
...
export const recurrenceRuleToOption = function recurrenceRuleToOption(startDate, recurrenceRule) {
  if (null == recurrenceRule) {
    ...
    } else if (tmp14(9699).RRule.YEARLY === freq) {
      return RecurrenceOptions.YEARLY;
    } else if (tmp14(9699).RRule.MONTHLY === freq) {
      return RecurrenceOptions.MONTHLY;
    } else if (tmp14(9699).RRule.DAILY === freq) {
