...
// Function ID: 9736
// Name: ScheduleUtils
// Dependencies: [1371, 9736, 1963, 9737, 1114, 4196, 4286, 11, 1090, 12, 2]
...
const RecurrenceOptions = fn(9736).RecurrenceOptions;
...
let items = [fn(9737).RRule.MO.weekday, fn(9737).RRule.TU.weekday, fn(9737).RRule.WE.weekday, fn(9737).RRule.TH.weekday, fn(9737).RRule.FR.weekday];
let items1 = [fn(9737).RRule.SU.weekday, fn(9737).RRule.MO.weekday, fn(9737).RRule.TU.weekday, fn(9737).RRule.WE.weekday, fn(9737).RRule.TH.weekday];
let items2 = [fn(9737).RRule.TU.weekday, fn(9737).RRule.WE.weekday, fn(9737).RRule.TH.weekday, fn(9737).RRule.FR.weekday, fn(9737).RRule.SA.weekday];
let items3 = [fn(9737).RRule.SA.weekday, fn(9737).RRule.SU.weekday];
const items4 = [fn(9737).RRule.FR.weekday, fn(9737).RRule.SA.weekday];
const items5 = [fn(9737).RRule.SU.weekday, fn(9737).RRule.MO.weekday];
const items6 = [fn(9737).RRule.SU.weekday, fn(9737).RRule.MO.weekday, fn(9737).RRule.TU.weekday, fn(9737).RRule.WE.weekday, fn(9737).RRule.TH.weekday, fn(9737).RRule.FR.weekday, fn(9737).RRule.SA.weekday];
...
export const getInitialEventEndDate = function getInitialEventEndDate(arg0) {
  ...
  const obj3 = tmp(4196)(addResult);
  return tmp(4196)(addResult).add(60 - result, "minutes").seconds(0);
};
export const getNextBucketedTime = function getNextBucketedTime(minutes, arg1) {
  const diff = arg1 - minutes.minutes() % arg1;
  const obj = _modDef4196(minutes);
  return _modDef4196(minutes).add(diff, "minutes").seconds(0);
};
export const getEventTimeData = function getEventTimeData(scheduled_start_time, toISOStringResult1, arg2) {
  ...
  const obj2 = _modDef4196(scheduled_start_time);
  if (null != toISOStringResult1) {
    if ("" !== toISOStringResult1) {
      const obj3 = tmp3(4196)(toISOStringResult1);
    }
  }
  ...
  if (result <= 1) {
    ...
    if (null == obj3) {
      ...
      obj5.upcomingEvent = obj2 <= tmp3(4196)().add(1, "hour");
      const obj10 = tmp3(4196)();
      obj5.withinStartWindow = obj2 <= tmp3(4196)().add(15, "minute");
      ...
      formatResult = tmp6(4286).dateFormat(obj3, str6);
      const tmp6Result3 = tmp6(4286);
    }
  }
...
export const getBaseScheduleForRecurrence = function getBaseScheduleForRecurrence(nextRecurrenceIdInEvent, guildEvent) {
  ...
  if (null != scheduled_start_time) {
    const obj = { startDate: _modDef4196(scheduled_start_time), endDate: "Array" };
    tmp = obj;
    if (null != scheduled_end_time) {
      obj.endDate = tmp2(4196)(scheduled_end_time);
      tmp = obj;
    }
    tmp2 = importDefault;
  }
  const tmp4 = _modDef4196;
...
export const getScheduleForRecurrenceWithException = function getScheduleForRecurrenceWithException(baseScheduleForRecurrence, scheduled_end_time) {
  if (null == scheduled_end_time) {
    ...
    if (null != scheduled_end_time.scheduled_start_time) {
      let startDate = _modDef4196(scheduled_end_time.scheduled_start_time);
    } else {
...
export const getScheduleFromEventData = function getScheduleFromEventData(arg0) {
  ...
  if (null != scheduledStartTime) {
    const obj = { startDate: _modDef4196(scheduledStartTime), endDate: "Array" };
    tmp = obj;
    if (null != scheduledEndTime) {
      obj.endDate = tmp2(4196)(scheduledEndTime);
      tmp = obj;
    }
    tmp2 = importDefault;
  }
...
export const hasValidSchedule = function hasValidSchedule(arg0, arg1) {
  ...
  if (tmp) {
    let tmp4 = startDate >= _modDef4196();
    if (tmp4) {
      let tmp6 = !tmp5;
      ...
      if (uTCHours === date1.getUTCHours()) {
        ...
        if (uTCMinutes === date1.getUTCMinutes()) {
          ...
          if (uTCSeconds === date1.getUTCSeconds()) {
            ...
            } else if (tmp(9737).RRule.YEARLY === frequency) {
...
export const recurrenceOptionToRecurrenceRule = function recurrenceOptionToRecurrenceRule(c7, toDate) {
  ...
  if (RecurrenceOptions.NONE !== c7) {
    if (tmp8.WEEKLY === c7) {
      const obj = { dtstart: toDateResult2, freq: tmp4(9737).RRule.WEEKLY };
      rRule = new tmp4(9737).RRule(obj);
    } else if (tmp8.BIWEEKLY === c7) {
      const obj2 = { dtstart: toDateResult2, freq: tmp4(9737).RRule.WEEKLY, interval: 2 };
      rRule = new tmp4(9737).RRule(obj2);
    } else if (tmp8.MONTHLY === c7) {
      const obj3 = { dtstart: toDateResult2, freq: tmp4(9737).RRule.MONTHLY, byweekday: null };
      items = [weekday.nth(rounded)];
      obj3.byweekday = items;
      rRule = new tmp4(9737).RRule(obj3);
    } else if (tmp8.YEARLY === c7) {
      const obj4 = { dtstart: toDateResult2, freq: tmp4(9737).RRule.YEARLY };
      rRule = new tmp4(9737).RRule(obj4);
    } else if (tmp8.DAILY === c7) {
      const obj5 = { dtstart: toDateResult2, freq: tmp4(9737).RRule.DAILY };
      rRule = new tmp4(9737).RRule(obj5);
    } else if (tmp8.WEEKDAY_ONLY === c7) {
      const obj6 = { dtstart: toDateResult2, freq: tmp4(9737).RRule.DAILY, byweekday: tmp };
      rRule = new tmp4(9737).RRule(obj6);
    } else if (tmp8.WEEKEND_ONLY === c7) {
      const obj7 = { dtstart: toDateResult2, freq: tmp4(9737).RRule.DAILY, byweekday: tmp2 };
      rRule = new tmp4(9737).RRule(obj7);
    }
  }
  if (null == rRule) {
    return null;
...
export const recurrenceRuleToOption = function recurrenceRuleToOption(startDate, recurrenceRule) {
  if (null == recurrenceRule) {
    ...
    } else if (tmp14(9737).RRule.YEARLY === freq) {
      return RecurrenceOptions.YEARLY;
    } else if (tmp14(9737).RRule.MONTHLY === freq) {
      return RecurrenceOptions.MONTHLY;
    } else if (tmp14(9737).RRule.DAILY === freq) {
