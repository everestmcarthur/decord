...
// Function ID: 9798
// Name: ScheduleUtils
// Dependencies: [1372, 9798, 1964, 9799, 1115, 4231, 4321, 11, 1091, 12, 2]
...
const RecurrenceOptions = fn(9798).RecurrenceOptions;
...
let items = [fn(9799).RRule.MO.weekday, fn(9799).RRule.TU.weekday, fn(9799).RRule.WE.weekday, fn(9799).RRule.TH.weekday, fn(9799).RRule.FR.weekday];
let items1 = [fn(9799).RRule.SU.weekday, fn(9799).RRule.MO.weekday, fn(9799).RRule.TU.weekday, fn(9799).RRule.WE.weekday, fn(9799).RRule.TH.weekday];
let items2 = [fn(9799).RRule.TU.weekday, fn(9799).RRule.WE.weekday, fn(9799).RRule.TH.weekday, fn(9799).RRule.FR.weekday, fn(9799).RRule.SA.weekday];
let items3 = [fn(9799).RRule.SA.weekday, fn(9799).RRule.SU.weekday];
const items4 = [fn(9799).RRule.FR.weekday, fn(9799).RRule.SA.weekday];
const items5 = [fn(9799).RRule.SU.weekday, fn(9799).RRule.MO.weekday];
const items6 = [fn(9799).RRule.SU.weekday, fn(9799).RRule.MO.weekday, fn(9799).RRule.TU.weekday, fn(9799).RRule.WE.weekday, fn(9799).RRule.TH.weekday, fn(9799).RRule.FR.weekday, fn(9799).RRule.SA.weekday];
...
export const getInitialEventStartDate = function getInitialEventStartDate() {
  const addResult = _modDef4231().add(1, "hour");
...
export const getInitialEventEndDate = function getInitialEventEndDate(arg0) {
  ...
  const obj3 = tmp(4231)(addResult);
  return tmp(4231)(addResult).add(60 - result, "minutes").seconds(0);
};
export const getNextBucketedTime = function getNextBucketedTime(minutes, arg1) {
  const diff = arg1 - minutes.minutes() % arg1;
  const obj = _modDef4231(minutes);
  return _modDef4231(minutes).add(diff, "minutes").seconds(0);
};
export const getEventTimeData = function getEventTimeData(scheduled_start_time, toISOStringResult1, arg2) {
  ...
  const obj2 = _modDef4231(scheduled_start_time);
  if (null != toISOStringResult1) {
    if ("" !== toISOStringResult1) {
      const obj3 = tmp3(4231)(toISOStringResult1);
    }
  }
  ...
  if (result <= 1) {
    ...
    if (null == obj3) {
      ...
      obj5.upcomingEvent = obj2 <= tmp3(4231)().add(1, "hour");
      const obj10 = tmp3(4231)();
      obj5.withinStartWindow = obj2 <= tmp3(4231)().add(15, "minute");
      ...
      formatResult = tmp6(4321).dateFormat(obj3, str6);
      const tmp6Result3 = tmp6(4321);
    }
  }
...
export const getBaseScheduleForRecurrence = function getBaseScheduleForRecurrence(nextRecurrenceIdInEvent, guildEvent) {
  ...
  if (null != scheduled_start_time) {
    const obj = { startDate: _modDef4231(scheduled_start_time), endDate: "Array" };
    tmp = obj;
    if (null != scheduled_end_time) {
      obj.endDate = tmp2(4231)(scheduled_end_time);
      tmp = obj;
    }
    tmp2 = importDefault;
  }
  const tmp4 = _modDef4231;
...
export const getScheduleForRecurrenceWithException = function getScheduleForRecurrenceWithException(baseScheduleForRecurrence, scheduled_end_time) {
  if (null == scheduled_end_time) {
    ...
    if (null != scheduled_end_time.scheduled_start_time) {
      let startDate = _modDef4231(scheduled_end_time.scheduled_start_time);
    } else {
...
export const getScheduleFromEventData = function getScheduleFromEventData(arg0) {
  ...
  if (null != scheduledStartTime) {
    const obj = { startDate: _modDef4231(scheduledStartTime), endDate: "Array" };
    tmp = obj;
    if (null != scheduledEndTime) {
      obj.endDate = tmp2(4231)(scheduledEndTime);
      tmp = obj;
    }
    tmp2 = importDefault;
  }
...
export const hasValidSchedule = function hasValidSchedule(arg0, arg1) {
  ...
  if (tmp) {
    let tmp4 = startDate >= _modDef4231();
    if (tmp4) {
      let tmp6 = !tmp5;
      ...
      if (uTCHours === date1.getUTCHours()) {
        ...
        if (uTCMinutes === date1.getUTCMinutes()) {
          ...
          if (uTCSeconds === date1.getUTCSeconds()) {
            ...
            } else if (tmp(9799).RRule.YEARLY === frequency) {
...
export const recurrenceOptionToRecurrenceRule = function recurrenceOptionToRecurrenceRule(c7, toDate) {
  ...
  if (RecurrenceOptions.NONE !== c7) {
    if (tmp8.WEEKLY === c7) {
      const obj = { dtstart: toDateResult2, freq: tmp4(9799).RRule.WEEKLY };
      rRule = new tmp4(9799).RRule(obj);
    } else if (tmp8.BIWEEKLY === c7) {
      const obj2 = { dtstart: toDateResult2, freq: tmp4(9799).RRule.WEEKLY, interval: 2 };
      rRule = new tmp4(9799).RRule(obj2);
    } else if (tmp8.MONTHLY === c7) {
      const obj3 = { dtstart: toDateResult2, freq: tmp4(9799).RRule.MONTHLY, byweekday: null };
      items = [weekday.nth(rounded)];
      obj3.byweekday = items;
      rRule = new tmp4(9799).RRule(obj3);
    } else if (tmp8.YEARLY === c7) {
      const obj4 = { dtstart: toDateResult2, freq: tmp4(9799).RRule.YEARLY };
      rRule = new tmp4(9799).RRule(obj4);
    } else if (tmp8.DAILY === c7) {
      const obj5 = { dtstart: toDateResult2, freq: tmp4(9799).RRule.DAILY };
      rRule = new tmp4(9799).RRule(obj5);
    } else if (tmp8.WEEKDAY_ONLY === c7) {
      const obj6 = { dtstart: toDateResult2, freq: tmp4(9799).RRule.DAILY, byweekday: tmp };
      rRule = new tmp4(9799).RRule(obj6);
    } else if (tmp8.WEEKEND_ONLY === c7) {
      const obj7 = { dtstart: toDateResult2, freq: tmp4(9799).RRule.DAILY, byweekday: tmp2 };
      rRule = new tmp4(9799).RRule(obj7);
    }
  }
  if (null == rRule) {
    return null;
...
export const recurrenceRuleToOption = function recurrenceRuleToOption(startDate, recurrenceRule) {
  if (null == recurrenceRule) {
    ...
    } else if (tmp14(9799).RRule.YEARLY === freq) {
      return RecurrenceOptions.YEARLY;
    } else if (tmp14(9799).RRule.MONTHLY === freq) {
      return RecurrenceOptions.MONTHLY;
    } else if (tmp14(9799).RRule.DAILY === freq) {
