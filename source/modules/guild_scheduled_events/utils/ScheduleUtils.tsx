...
// Function ID: 9671
// Name: getRRule
// Dependencies: [1371, 9671, 1963, 9672, 1114, 4153, 4242, 11, 1090, 12, 2]
...
import m from "m" /* 9672 */;
import closure_3 from "mergeGuildAvatar" /* 1371 */;
import { RecurrenceOptions } from "RecurrenceOptions" /* 9671 */;
...
export const getRecurrenceOptions = function getRecurrenceOptions(startDate) {
  ...
  if (set.has(toDateResult.getDay())) {
    ...
    if (isStaffResult) {
      ...
      obj6[2] = intl8.string(tmp4(1114).t.hRpynV);
      items.push(obj6);
    }
  } else {
    ...
    obj7[2] = intl7.string(tmp4(1114).t["jYR/MY"]);
    items.push(obj7);
  }
  return items;
};
...
export const getEventTimeData = function getEventTimeData(scheduled_start_time, toISOStringResult1) {
  ...
  if (result <= 1) {
    ...
    if (null == obj3) {
      obj[1] = undefined;
      obj[2] = obj2 <= obj;
      obj[3] = obj2 <= tmp3(4153)().add(1, "hour");
      const obj10 = tmp3(4153)();
      obj[4] = obj2 <= tmp3(4153)().add(15, "minute");
      ...
      if (uTCHours === date1.getUTCHours()) {
        ...
        if (uTCMinutes === date1.getUTCMinutes()) {
          ...
          if (uTCSeconds === date1.getUTCSeconds()) {
            ...
            } else if (tmp(9672).RRule.YEARLY === frequency) {
...
export const recurrenceOptionToRecurrenceRule = function recurrenceOptionToRecurrenceRule(c7, toDate) {
  ...
  if (RecurrenceOptions.NONE !== c7) {
    if (tmp8.WEEKLY === c7) {
      ...
      obj[0] = toDateResult2;
      obj[1] = tmp4(9672).RRule.WEEKLY;
      rRule = new tmp4(9672).RRule(obj);
...
export const recurrenceRuleToOption = function recurrenceRuleToOption(startDate, recurrenceRule) {
  if (null == recurrenceRule) {
    ...
    } else if (tmp14(9672).RRule.YEARLY === freq) {
      return RecurrenceOptions.YEARLY;
    } else if (tmp14(9672).RRule.MONTHLY === freq) {
      return RecurrenceOptions.MONTHLY;
    } else if (tmp14(9672).RRule.DAILY === freq) {
