...
function dateValidator(getFullMatch, arg1) {
  ...
  if (null != tmp2) {
    ...
    if (set.has(formatted)) {
      const localResult = tmp32(4228)(formatted, "MMMM").local();
      const items = [localResult, ];
      const obj17 = tmp32(4228)(formatted, "MMMM");
      ...
      const _Set = Set;
      const tmp32Result = tmp32(4228);
      const set1 = new Set(tmp32(4228).weekdays().map((item) => item.toLowerCase()));
      if (set1.has(formatted)) {
        const localResult1 = tmp32(4228)(formatted, "dddd").local();
        const items1 = [localResult1, ];
        const obj14 = tmp32(4228)(formatted, "dddd");
        ...
        if (set2.has(formatted)) {
          const localResult2 = tmp32(4228)(formatted, "YYYY").local();
          const items2 = [localResult2, ];
          const obj11 = tmp32(4228)(formatted, "YYYY");
          items2[1] = localResult2.clone().add(1, "year");
          const cloneResult2 = localResult2.clone();
          [obj9, obj10] = items2;
          const tmp17 = _slicedToArray(items2, 2);
        } else {
          const localResult3 = tmp32(4228)(formatted, value2).local();
          const items3 = [localResult3, ];
          const obj6 = tmp32(4228)(formatted, value2);
      ...
      const weekdaysResult = tmp32(4228).weekdays();
    }
