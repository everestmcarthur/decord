...
function getShortcuts() {
  ...
  obj[intl.string(util.t.HYiVEQ)] = () => {
    ...
    const addResult = _modDef4198().startOf("day").add(0, "day");
  ...
  obj[intl2.string(util.t.cu86KC)] = () => {
    const obj = _modDef4198();
    const addResult = _modDef4198().startOf("day").add(-1, "day");
    const items = [addResult, ];
    const startOfResult = _modDef4198().startOf("day");
    items[1] = addResult.clone().add(1, "day");
    return items;
  ...
  obj[intl3.string(util.t["FvBj/6"])] = () => {
    const obj = _modDef4198();
    const addResult = _modDef4198().startOf("week").add(0, "week");
    const items = [addResult, ];
    const startOfResult = _modDef4198().startOf("week");
  ...
  obj[intl4.string(util.t["20uWCw"])] = () => {
    const obj = _modDef4198();
    const addResult = _modDef4198().startOf("month").add(0, "month");
    const items = [addResult, ];
    const startOfResult = _modDef4198().startOf("month");
  ...
  obj[intl5.string(util.t["dXC/hn"])] = () => {
    const obj = _modDef4198();
    const addResult = _modDef4198().startOf("year").add(0, "year");
    const items = [addResult, ];
    const startOfResult = _modDef4198().startOf("year");
...
function dateValidator(getFullMatch, arg1) {
  ...
  if (null != tmp2) {
    ...
    const _Set3 = Set;
    const set = new Set(_modDef4198.months().map((item) => item.toLowerCase()));
    if (set.has(formatted)) {
      const localResult = tmp32(4198)(formatted, "MMMM").local();
      const items = [localResult, ];
      const obj17 = tmp32(4198)(formatted, "MMMM");
      ...
      const _Set = Set;
      const tmp32Result = tmp32(4198);
      const set1 = new Set(tmp32(4198).weekdays().map((item) => item.toLowerCase()));
      if (set1.has(formatted)) {
        const localResult1 = tmp32(4198)(formatted, "dddd").local();
        const items1 = [localResult1, ];
        const obj14 = tmp32(4198)(formatted, "dddd");
        ...
        if (set2.has(formatted)) {
          const localResult2 = tmp32(4198)(formatted, "YYYY").local();
          const items2 = [localResult2, ];
          const obj11 = tmp32(4198)(formatted, "YYYY");
          items2[1] = localResult2.clone().add(1, "year");
          const cloneResult2 = localResult2.clone();
          [obj9, obj10] = items2;
          const tmp17 = _slicedToArray(items2, 2);
        } else {
          const localResult3 = tmp32(4198)(formatted, value2).local();
          const items3 = [localResult3, ];
          const obj6 = tmp32(4198)(formatted, value2);
      ...
      const weekdaysResult = tmp32(4198).weekdays();
    }
    const monthsResult = _modDef4198.months();
  }
...
function generateDateAutocompletions() {
  const monthsResult = _modDef4198.months();
  const items = [...Array.from(new Set(_modDef4198.months().map((item) => item.toLowerCase())))];
  const set = new Set(_modDef4198.months().map((item) => item.toLowerCase()));
  const weekdaysResult = _modDef4198.weekdays();
  const set1 = new Set(_modDef4198.weekdays().map((item) => item.toLowerCase()));
  const arraySpreadResult = HermesBuiltin.arraySpread(Array.from(new Set(_modDef4198.weekdays().map((item) => item.toLowerCase()))), tmp2);
