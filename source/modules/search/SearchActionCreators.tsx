...
// Dependencies: [12433, 12441, 573, 12, 7854, 12444, 1940, 2]

// Module 12440 (SearchActionCreators)
...
export default {
  fetchTabMessages(searchContext) {
    ...
    const tokenizeQueryResult = searchContext(12433).tokenizeQuery(searchQueryString);
    const searchQueryFromTokens = searchContext(12433).getSearchQueryFromTokens(tokenizeQueryResult);
    ...
    let obj2 = searchContext(12433);
    const result = searchContext(12433).searchModeToSearchQueryParams(searchMode);
    let obj3 = {};
    const merged = Object.assign(searchQueryFromTokens);
    const merged1 = Object.assign(result);
    const tmpResult = searchContext(12433);
    guildIdFromSearchContext = searchContext(12433).getGuildIdFromSearchContext(searchContext);
    if (null != guildIdFromSearchContext) {
      tmp(12433).setIncludeNSFW(obj3, guildIdFromSearchContext);
      const tmpResult5 = tmp(12433);
    }
    const tmpResult4 = searchContext(12433);
    ...
    obj4.id = searchContext(12433).getSearchContextId(searchContext);
    ...
    const tmpResult6 = searchContext(12433);
  ...
  fetchMessages(arg0) {
    ...
    let obj2 = {};
    const obj = guildIdFromSearchContext(12433);
    const tokenizeQueryResult = guildIdFromSearchContext(12433).tokenizeQuery(searchQueryString);
    const merged = Object.assign(guildIdFromSearchContext(12433).getSearchQueryFromTokens(tokenizeQueryResult));
    let obj3 = guildIdFromSearchContext(12433);
    const merged1 = Object.assign(guildIdFromSearchContext(12433).searchModeToSearchQueryParams(searchMode));
    obj2.offset = pagination.offset;
    const obj4 = guildIdFromSearchContext(12433);
    guildIdFromSearchContext = guildIdFromSearchContext(12433).getGuildIdFromSearchContext(searchContext);
    if (null != guildIdFromSearchContext) {
      tmp(12433).setIncludeNSFW(obj2, guildIdFromSearchContext);
      const tmpResult = tmp(12433);
    }
    if (searchEverywhere) {
      obj2.search_everywhere = true;
    }
    const obj5 = guildIdFromSearchContext(12433);
    searchContextId = guildIdFromSearchContext(12433).getSearchContextId(searchContext);
    const tmp9 = searchContextId;
    const tmpResult2 = guildIdFromSearchContext(12433);
    const obj6 = { id: searchContextId, searchType: searchContext.type, searchQuery: obj2 };
    const obj8 = searchContextId(12444);
    if (onFetchStart != null) {
      const obj9 = { searchContext, searchQueryString, searchQuery: obj2 };
      onFetchStart(obj9);
    }
    const obj7 = searchContextId(12444).create({ id: searchContextId, searchType: searchContext.type, searchQuery: obj2 });
