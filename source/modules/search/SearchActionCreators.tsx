...
// Dependencies: [12475, 12483, 573, 12, 7887, 12486, 1941, 2]

// Module 12482 (SearchActionCreators)
...
export default {
  fetchTabMessages(searchContext) {
    ...
    const tokenizeQueryResult = searchContext(12475).tokenizeQuery(searchQueryString);
    const searchQueryFromTokens = searchContext(12475).getSearchQueryFromTokens(tokenizeQueryResult);
    ...
    let obj2 = searchContext(12475);
    const result = searchContext(12475).searchModeToSearchQueryParams(searchMode);
    let obj3 = {};
    const merged = Object.assign(searchQueryFromTokens);
    const merged1 = Object.assign(result);
    const tmpResult = searchContext(12475);
    guildIdFromSearchContext = searchContext(12475).getGuildIdFromSearchContext(searchContext);
    if (null != guildIdFromSearchContext) {
      tmp(12475).setIncludeNSFW(obj3, guildIdFromSearchContext);
      const tmpResult5 = tmp(12475);
    }
    const tmpResult4 = searchContext(12475);
    ...
    obj4.id = searchContext(12475).getSearchContextId(searchContext);
    ...
    const tmpResult6 = searchContext(12475);
  ...
  fetchMessages(arg0) {
    ...
    let obj2 = {};
    const obj = guildIdFromSearchContext(12475);
    const tokenizeQueryResult = guildIdFromSearchContext(12475).tokenizeQuery(searchQueryString);
    const merged = Object.assign(guildIdFromSearchContext(12475).getSearchQueryFromTokens(tokenizeQueryResult));
    let obj3 = guildIdFromSearchContext(12475);
    const merged1 = Object.assign(guildIdFromSearchContext(12475).searchModeToSearchQueryParams(searchMode));
    obj2.offset = pagination.offset;
    const obj4 = guildIdFromSearchContext(12475);
    guildIdFromSearchContext = guildIdFromSearchContext(12475).getGuildIdFromSearchContext(searchContext);
    if (null != guildIdFromSearchContext) {
      tmp(12475).setIncludeNSFW(obj2, guildIdFromSearchContext);
      const tmpResult = tmp(12475);
    }
    if (searchEverywhere) {
      obj2.search_everywhere = true;
    }
    const obj5 = guildIdFromSearchContext(12475);
    searchContextId = guildIdFromSearchContext(12475).getSearchContextId(searchContext);
    const tmp9 = searchContextId;
    const tmpResult2 = guildIdFromSearchContext(12475);
    const obj6 = { id: searchContextId, searchType: searchContext.type, searchQuery: obj2 };
    const obj8 = searchContextId(12486);
    if (onFetchStart != null) {
      const obj9 = { searchContext, searchQueryString, searchQuery: obj2 };
      onFetchStart(obj9);
    }
    const obj7 = searchContextId(12486).create({ id: searchContextId, searchType: searchContext.type, searchQuery: obj2 });
