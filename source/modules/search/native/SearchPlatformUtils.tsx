...
// Dependencies: [7355, 12434, 7953, 12448, 1074, 12435, 8353, 1384, 6697, 1115, 1874, 4550, 1365, 10909, 576, 1935, 12453, 12456, 12442, 12, 558, 2]
...
// Module 12433 (SearchPlatformUtils)
...
function fetchInitialMessages(searchContext) {
  ...
  if (isInitialSearchQueryResult) {
    ...
    if (!SearchMessageStore.getIsFetching(searchTabFetchId)) {
      const result = queryString(12442).clearAllSearchMesssages();
      const obj3 = queryString(12442);
      const obj5 = { searchContext };
      queryString(12453).trackSearchStarted(obj5);
      const obj4 = queryString(12453);
      const obj7 = {
        searchContext,
        searchTabs: tmp3,
...
const SearchPlatformConstants = fn(12448);
...
obj.subscribeSearchQueryState = function subscribeSearchQueryState(searchContext, fn, fn2, arg3) {
  ...
  let tmp = arg3;
  function callback() {
    const tmp = f93601(SearchQueryStore.getManager(closure_0));
    if (null == closure_3) {
      closure_3 = tmp;
      f93602(tmp, closure_3);
    }
  }
...
obj.subscribeTextInputValue = function subscribeTextInputValue(searchContext, debounceResult, arg2) {
  ...
  let flag = arg2;
  const f93601 = (getTextInputValue) => ({ textInputValue: getTextInputValue.getTextInputValue(), textInputChangedFromInput: getTextInputValue.getTextValueChangedFromInput() });
  const f93602 = (textInputValue, textInputValue2) => {
    ...
    if (embeds != null) {
      const item1 = embeds.forEach((embed, index) => {
        if (obj.isValidImageEmbed(embed)) {
          ...
          const tmpResult2 = tmp(8353);
        }
      });
    }
    let result = guildIdFromSearchContext(8353).extractMediaFromMessageComponents(getContentMessage, contentMessage, closure_0);
    const iter = result[Symbol.iterator]();
    const obj = guildIdFromSearchContext(8353);
