...
// Dependencies: [7354, 12432, 7952, 12446, 1074, 12433, 8351, 1384, 6696, 1115, 1874, 4549, 1365, 10907, 576, 1935, 12451, 12454, 12440, 12, 558, 2]
...
// Module 12431 (SearchPlatformUtils)
...
function fetchInitialMessages(searchContext) {
  ...
  if (isInitialSearchQueryResult) {
    ...
    if (!SearchMessageStore.getIsFetching(searchTabFetchId)) {
      const result = queryString(12440).clearAllSearchMesssages();
      const obj3 = queryString(12440);
      const obj5 = { searchContext };
      queryString(12451).trackSearchStarted(obj5);
      const obj4 = queryString(12451);
      const obj7 = {
        searchContext,
        searchTabs: tmp3,
...
const SearchPlatformConstants = fn(12446);
...
obj.subscribeSearchQueryState = function subscribeSearchQueryState(searchContext, fn, fn2, arg3) {
  ...
  let tmp = arg3;
  function callback() {
    const tmp = f93613(SearchQueryStore.getManager(closure_0));
    if (null == closure_3) {
      closure_3 = tmp;
      f93614(tmp, closure_3);
    }
  }
...
obj.subscribeTextInputValue = function subscribeTextInputValue(searchContext, debounceResult, arg2) {
  ...
  let flag = arg2;
  const f93613 = (getTextInputValue) => ({ textInputValue: getTextInputValue.getTextInputValue(), textInputChangedFromInput: getTextInputValue.getTextValueChangedFromInput() });
  const f93614 = (textInputValue, textInputValue2) => {
    ...
    if (embeds != null) {
      const item1 = embeds.forEach((embed, index) => {
        if (obj.isValidImageEmbed(embed)) {
          ...
          const tmpResult2 = tmp(8351);
        }
      });
    }
    let result = guildIdFromSearchContext(8351).extractMediaFromMessageComponents(getContentMessage, contentMessage, closure_0);
    const iter = result[Symbol.iterator]();
    const obj = guildIdFromSearchContext(8351);
