...
// Dependencies: [7281, 12343, 7878, 12357, 1074, 12344, 8263, 1384, 6625, 1115, 1874, 4503, 1365, 10823, 576, 1935, 12362, 12365, 12351, 12, 558, 2]
...
// Module 12342 (performKeyboardAwareNavigation)
...
import closure_3 from "handleReaction" /* 7281 */;
import closure_4 from "prototype" /* 12343 */;
import MessageEmbedTypes from "MessageEmbedTypes" /* 7878 */;
import SEARCH_TEXT_INPUT_DEBOUNCE_TIME from "SEARCH_TEXT_INPUT_DEBOUNCE_TIME" /* 12357 */;
...
function onFetchMessagesStart(searchQueryString) {
  ...
  _modDef12365.updateSearchQuery(searchQueryString.searchContext, (setSearchResultsQuery) => setSearchResultsQuery.setSearchResultsQuery(searchQueryString));
}
function fetchInitialMessages(closure_0) {
  ...
  if (isInitialSearchQueryResult) {
    ...
    if (!store.getIsFetching(searchTabFetchId)) {
      const result = queryString(12351).clearAllSearchMesssages();
      const obj3 = queryString(12351);
      obj = { searchContext: null };
      obj[0] = closure_0;
      queryString(12362).trackSearchStarted(obj);
      const obj4 = queryString(12362);
      ...
      obj1[0] = cursor;
      obj[7] = obj1;
      const obj6 = queryString(12351);
      obj[8] = _require(1935).SearchResultExactCountEnabled.getSetting();
      obj[9] = constants2.NEWEST;
      const tabMessages = obj6.fetchTabMessages(obj);
      const SearchResultExactCountEnabled = _require(1935).SearchResultExactCountEnabled;
    }
  }
}
function syncAutocomplete(closure_0) {
  ...
  const result = _modDef12351.updateAutocompleteQuery(obj);
}
...
let obj = {
  ...
  fetchNextMessages(searchContext, tab, arg2) {
    ...
    const searchTabFetchId1 = _require(12344).getSearchTabFetchId(searchContext, tab, queryString);
    if (!store.getIsInitialFetchComplete(searchTabFetchId1)) {
      ...
      if (!flag) {
        ...
        obj[0] = cursor;
        obj[7] = obj;
        const obj4 = queryString(12351);
        obj[8] = tmp2(1935).SearchResultExactCountEnabled.getSetting();
        obj[9] = constants2.NEWEST;
...
obj[12] = function navigateToSearchWithPrefetch(closure_8, closure_9) {
  let obj = _modDef12351;
  const result = obj.initializeAutocomplete(closure_9);
  const result1 = _modDef12365.initializeSearchQuery(closure_9);
...
export const getMedia = function getMedia(items, items1) {
  guildIdFromSearchContext = guildIdFromSearchContext(12344).getGuildIdFromSearchContext(items);
  items = [];
