...
// Dependencies: [32, 19, 17, 1958, 7391, 1074, 21, 4638, 4572, 4489, 7406, 11387, 5166, 5048, 504, 8401, 1477, 12154, 1364, 1115, 8895, 10313, 1385, 7439, 8395, 10270, 10335, 2]
...
function SearchMediaObscurityIcon(obscureReason) {
  ...
  const memo1 = noop.useMemo(() => {
    if (ObscureMediaModels.ObscureReason.SPOILER === obscureReason) {
      return closure_2_11(tmp2(11387).SpoilerIcon, { size: "lg" });
    } else {
      if (tmp2(7406).ObscureReason.EXPLICIT_CONTENT !== tmp) {
        if (tmp2(7406).ObscureReason.GORE_CONTENT !== tmp) {
          if (tmp2(7406).ObscureReason.SELF_HARM_CONTENT !== tmp) {
            if (tmp2(7406).ObscureReason.POTENTIAL_EXPLICIT_CONTENT === tmp) {
              return null;
            }
          }
        }
      }
      return closure_2_11(tmp2(5166).ImageWarningIcon, { size: "lg" });
    }
  }, items1);
...
export const SearchAttachmentMediaImage = function SearchAttachmentMediaImage(attachment) {
  ...
  const enabledHarmTypesBitmaskForChannelAndAuthorId = attachment(10313).useEnabledHarmTypesBitmaskForChannelAndAuthorId(channelId, attachment.authorId);
  const obj = attachment(10313);
  ...
  if (!hasFlagResult) {
    hasFlagResult = tmp2(7439).isChannelSpoilerGated(ChannelStore.getChannel(channelId));
  ...
  const obj3 = {};
  const attachmentUrl = attachment(8395).getAttachmentUrl(attachment);
...
export const SearchEmbedMediaImage = function SearchEmbedMediaImage(embed) {
  ...
  closure_3 = embed(10313).useEnabledHarmTypesBitmaskForChannelAndAuthorId(channelId, embed.authorId);
  const obj = embed(10313);
  const tmp2 = embed;
  closure_4 = embed(7439).useIsChannelSpoilerGated(ChannelStore.getChannel(channelId));
  const obj2 = embed(7439);
  ...
  const size = embed(8395).getEmbedMedia(embed);
  let embedUrl = null;
  if (null != size) {
    embedUrl = tmp2(8395).getEmbedUrl(size);
    let tmp2Result = tmp2(8395);
  }
  if (null != thumbnail) {
