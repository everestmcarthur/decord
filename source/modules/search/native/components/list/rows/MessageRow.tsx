...
// Function ID: 16755
// Name: MessageRow
// Dependencies: [19, 17, 4598, 1960, 1957, 1979, 4787, 1074, 21, 4606, 576, 504, 5079, 4759, 1178, 4602, 10234, 10480, 9529, 4451, 8266, 16755, 13386, 16756, 4839, 8035, 7402, 16732, 1114, 10198, 7953, 2]
...
import useMessageAuthorDefault from "useMessageAuthor" /* 4839 */;
...
function GuildChannelMessageRowHeader(channel) {
  ...
  const channelIcon = channel(5079).getChannelIcon(channel, { isRulesChannel: stateFromStores === channel.id });
  ...
  const items1 = [closure_11(channel(1178).Icon, { source: channelIcon, size: channel(1178).Icon.Sizes.REFRESH_SMALL_16, style: tmp.channelIcon }), closure_11(channel(4602).Text, { lineClamp: 1, variant: "text-sm/semibold", color: "interactive-text-default", children: tmp7 }), , , ];
  if (muted) {
    const obj6 = { source: tmp6(10234), size: tmp2(1178).Icon.Sizes.EXTRA_SMALL, style: tmp.channelStatus };
    muted = tmp10(tmp2(1178).Icon, obj6);
  }
  items1[2] = muted;
  if (isFavorite) {
    const obj7 = { source: tmp6(10480), size: tmp2(1178).Icon.Sizes.EXTRA_SMALL, style: tmp.channelStatus };
    isFavorite = tmp10(tmp2(1178).Icon, obj7);
  }
  items1[3] = isFavorite;
  let isSystemDMResult = channel.isSystemDM();
  if (isSystemDMResult) {
    const obj8 = { type: tmp6(9529).Types.SYSTEM_DM, verified: true };
    isSystemDMResult = tmp10(tmp6(9529), obj8);
    const tmp6Result = tmp6(9529);
  }
...
function PrivateChannelMessageRowLabel(message) {
  ...
  const items2 = [closure_11(message(4602).Text, { lineClamp: 1, variant: "text-md/semibold", color: "interactive-text-active", children: memo }), , ];
  if (muted) {
    const obj4 = { source: channel(10234), size: tmp4(1178).Icon.Sizes.EXTRA_SMALL, style: tmp.channelStatus };
    muted = tmp9(tmp4(1178).Icon, obj4);
  }
  items2[1] = muted;
  let isSystemDMResult = channel.isSystemDM();
  if (isSystemDMResult) {
    const obj5 = { type: channel(9529).Types.SYSTEM_DM, verified: true };
    isSystemDMResult = tmp9(channel(9529), obj5);
    const tmp13 = channel(9529);
  }
  items2[2] = isSystemDMResult;
  obj3.children = items2;
  const items3 = [closure_12(closure_4, obj3), closure_11(message(4602).Text, { variant: "text-xs/medium", color: "interactive-text-active", lineClamp: 1, style: tmp.timestamp, accessibilityLabel: timestampAccessibilityLabel, children: timestamp }), , ];
  let tmp9Result = null;
  if (message.hasFlag(MessageFlags.SUPPRESS_NOTIFICATIONS)) {
    const obj7 = { size: "xs", style: tmp.suppressNotificationsIcon };
    tmp9Result = tmp9(tmp4(13386).BellZIcon, obj7);
  }
  items3[2] = tmp9Result;
...
function GuildChannelMessageRowLabel(arg0) {
  ...
  if ("username" === stateFromStores) {
    ...
    const processColorStringsArray = tmp2(8035).useProcessColorStringsArray(colorStrings);
    const tmp2Result3 = tmp2(8035);
    ...
    obj6.children = tmp7;
    items1[1] = closure_1_11(tmp2(4602).Text, obj6);
    obj4.children = items1;
    ...
    items2[1] = closure_1_11(tmp2(4602).Text, obj7);
    let tmp21Result = null;
    if (message.hasFlag(MessageFlags.SUPPRESS_NOTIFICATIONS)) {
      const obj8 = { size: "xs", style: tmp.suppressNotificationsIcon };
      tmp21Result = tmp21(tmp2(13386).BellZIcon, obj8);
    }
