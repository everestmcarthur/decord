...
// Function ID: 16918
// Name: MessageRow
// Dependencies: [19, 17, 4632, 1961, 1958, 1980, 4821, 1074, 21, 4640, 576, 504, 5115, 4793, 1177, 4636, 10291, 10539, 9592, 4484, 8329, 16918, 13444, 16919, 4873, 8097, 7443, 16895, 1115, 10258, 8001, 2]

// Module 16917 (MessageRow)
import initialize from "initialize" /* 504 */;
import nativeDefault from "native" /* 576 */;
import native from "native" /* 1177 */;
import UserUtilsDefault from "UserUtils" /* 4484 */;
import useChannelNameDefault from "useChannelName" /* 4793 */;
import useMessageAuthorDefault from "useMessageAuthor" /* 4873 */;
...
const PollBadgeDefault = tmp5(16919);
require = fn;
function GuildChannelMessageRowHeader(channel) {
  ...
  const channelIcon = channel(5115).getChannelIcon(channel, { isRulesChannel: stateFromStores === channel.id });
  const obj4 = { style: tmp.header, children: null };
  const obj2 = channel(5115);
  const obj3 = { isRulesChannel: stateFromStores === channel.id };
  const tmp7 = useChannelNameDefault(channel);
  const tmp8 = closure_12;
  const tmp9 = closure_4;
  const items1 = [closure_11(channel(1177).Icon, { source: channelIcon, size: channel(1177).Icon.Sizes.REFRESH_SMALL_16, style: tmp.channelIcon }), closure_11(channel(4636).Text, { lineClamp: 1, variant: "text-sm/semibold", color: "interactive-text-default", children: tmp7 }), , , ];
  if (muted) {
    const obj6 = { source: tmp6(10291), size: tmp2(1177).Icon.Sizes.EXTRA_SMALL, style: tmp.channelStatus };
    muted = tmp10(tmp2(1177).Icon, obj6);
  }
  items1[2] = muted;
  if (isFavorite) {
    const obj7 = { source: tmp6(10539), size: tmp2(1177).Icon.Sizes.EXTRA_SMALL, style: tmp.channelStatus };
    isFavorite = tmp10(tmp2(1177).Icon, obj7);
  }
  items1[3] = isFavorite;
  let isSystemDMResult = channel.isSystemDM();
  if (isSystemDMResult) {
    const obj8 = { type: tmp6(9592).Types.SYSTEM_DM, verified: true };
    isSystemDMResult = tmp10(tmp6(9592), obj8);
    const tmp6Result = tmp6(9592);
  }
...
function PrivateChannelMessageRowLabel(message) {
  ...
  const searchMessageTimestamp = message(16918).useSearchMessageTimestamp(message, channel);
  ...
  const items2 = [closure_11(message(4636).Text, { lineClamp: 1, variant: "text-md/semibold", color: "interactive-text-active", children: memo }), , ];
  if (muted) {
    const obj4 = { source: channel(10291), size: tmp4(1177).Icon.Sizes.EXTRA_SMALL, style: tmp.channelStatus };
    muted = tmp9(tmp4(1177).Icon, obj4);
  }
  items2[1] = muted;
  let isSystemDMResult = channel.isSystemDM();
  if (isSystemDMResult) {
    const obj5 = { type: channel(9592).Types.SYSTEM_DM, verified: true };
    isSystemDMResult = tmp9(channel(9592), obj5);
    const tmp13 = channel(9592);
  }
  items2[2] = isSystemDMResult;
  obj3.children = items2;
  const items3 = [closure_12(closure_4, obj3), closure_11(message(4636).Text, { variant: "text-xs/medium", color: "interactive-text-active", lineClamp: 1, style: tmp.timestamp, accessibilityLabel: timestampAccessibilityLabel, children: timestamp }), , ];
  let tmp9Result = null;
  if (message.hasFlag(MessageFlags.SUPPRESS_NOTIFICATIONS)) {
    const obj7 = { size: "xs", style: tmp.suppressNotificationsIcon };
    tmp9Result = tmp9(tmp4(13444).BellZIcon, obj7);
  }
  items3[2] = tmp9Result;
  let tmp9Result2 = null;
  if (message.isPoll()) {
    const obj8 = { style: tmp.pollBadge };
    tmp9Result2 = tmp9(channel(16919), obj8);
  }
...
function GuildChannelMessageRowLabel(arg0) {
  ...
  if ("username" === stateFromStores) {
    ...
    const processColorStringsArray = tmp2(8097).useProcessColorStringsArray(colorStrings);
    const tmp2Result3 = tmp2(8097);
    ...
    const tmp2Result = tmp2(8097);
    const searchMessageTimestamp = tmp2(16918).useSearchMessageTimestamp(message, channel);
    ...
    obj6.children = tmp7;
    items1[1] = closure_1_11(tmp2(4636).Text, obj6);
    obj4.children = items1;
    ...
    items2[1] = closure_1_11(tmp2(4636).Text, obj7);
    let tmp21Result = null;
    if (message.hasFlag(MessageFlags.SUPPRESS_NOTIFICATIONS)) {
      const obj8 = { size: "xs", style: tmp.suppressNotificationsIcon };
      tmp21Result = tmp21(tmp2(13444).BellZIcon, obj8);
    }
