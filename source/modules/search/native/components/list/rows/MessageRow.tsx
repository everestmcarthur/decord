...
// Function ID: 16755
// Name: MessageRow
// Dependencies: [19, 17, 4599, 1960, 1957, 1979, 4788, 1074, 21, 4607, 576, 504, 5080, 4760, 1178, 4603, 10236, 10482, 9531, 4451, 8267, 16755, 13388, 16756, 4840, 8036, 7403, 16732, 1114, 10200, 7954, 2]

// Module 16754 (MessageRow)
import initialize from "initialize" /* 504 */;
import nativeDefault from "native" /* 576 */;
import native from "native" /* 1178 */;
import UserUtilsDefault from "UserUtils" /* 4451 */;
import useChannelNameDefault from "useChannelName" /* 4760 */;
import useMessageAuthorDefault from "useMessageAuthor" /* 4840 */;
...
const PollBadgeDefault = tmp5(16756);
require = fn;
function GuildChannelMessageRowHeader(channel) {
  ...
  const channelIcon = channel(5080).getChannelIcon(channel, { isRulesChannel: stateFromStores === channel.id });
  ...
  const items1 = [closure_11(channel(1178).Icon, { source: channelIcon, size: channel(1178).Icon.Sizes.REFRESH_SMALL_16, style: tmp.channelIcon }), closure_11(channel(4603).Text, { lineClamp: 1, variant: "text-sm/semibold", color: "interactive-text-default", children: tmp7 }), , , ];
  if (muted) {
    const obj6 = { source: tmp6(10236), size: tmp2(1178).Icon.Sizes.EXTRA_SMALL, style: tmp.channelStatus };
    muted = tmp10(tmp2(1178).Icon, obj6);
  }
  items1[2] = muted;
  if (isFavorite) {
    const obj7 = { source: tmp6(10482), size: tmp2(1178).Icon.Sizes.EXTRA_SMALL, style: tmp.channelStatus };
    isFavorite = tmp10(tmp2(1178).Icon, obj7);
  }
  items1[3] = isFavorite;
  let isSystemDMResult = channel.isSystemDM();
  if (isSystemDMResult) {
    const obj8 = { type: tmp6(9531).Types.SYSTEM_DM, verified: true };
    isSystemDMResult = tmp10(tmp6(9531), obj8);
    const tmp6Result = tmp6(9531);
  }
...
function PrivateChannelMessageRowLabel(message) {
  ...
  const searchMessageTimestamp = message(16755).useSearchMessageTimestamp(message, channel);
  ...
  const items2 = [closure_11(message(4603).Text, { lineClamp: 1, variant: "text-md/semibold", color: "interactive-text-active", children: memo }), , ];
  if (muted) {
    const obj4 = { source: channel(10236), size: tmp4(1178).Icon.Sizes.EXTRA_SMALL, style: tmp.channelStatus };
    muted = tmp9(tmp4(1178).Icon, obj4);
  }
  items2[1] = muted;
  let isSystemDMResult = channel.isSystemDM();
  if (isSystemDMResult) {
    const obj5 = { type: channel(9531).Types.SYSTEM_DM, verified: true };
    isSystemDMResult = tmp9(channel(9531), obj5);
    const tmp13 = channel(9531);
  }
  items2[2] = isSystemDMResult;
  obj3.children = items2;
  const items3 = [closure_12(closure_4, obj3), closure_11(message(4603).Text, { variant: "text-xs/medium", color: "interactive-text-active", lineClamp: 1, style: tmp.timestamp, accessibilityLabel: timestampAccessibilityLabel, children: timestamp }), , ];
  let tmp9Result = null;
  if (message.hasFlag(MessageFlags.SUPPRESS_NOTIFICATIONS)) {
    const obj7 = { size: "xs", style: tmp.suppressNotificationsIcon };
    tmp9Result = tmp9(tmp4(13388).BellZIcon, obj7);
  }
  items3[2] = tmp9Result;
  let tmp9Result2 = null;
  if (message.isPoll()) {
    const obj8 = { style: tmp.pollBadge };
    tmp9Result2 = tmp9(channel(16756), obj8);
  }
...
function GuildChannelMessageRowLabel(arg0) {
  ...
  if ("username" === stateFromStores) {
    ...
    const processColorStringsArray = tmp2(8036).useProcessColorStringsArray(colorStrings);
    const tmp2Result3 = tmp2(8036);
    ...
    const tmp2Result = tmp2(8036);
    const searchMessageTimestamp = tmp2(16755).useSearchMessageTimestamp(message, channel);
    ...
    obj6.children = tmp7;
    items1[1] = closure_1_11(tmp2(4603).Text, obj6);
    obj4.children = items1;
    ...
    items2[1] = closure_1_11(tmp2(4603).Text, obj7);
    let tmp21Result = null;
    if (message.hasFlag(MessageFlags.SUPPRESS_NOTIFICATIONS)) {
      const obj8 = { size: "xs", style: tmp.suppressNotificationsIcon };
      tmp21Result = tmp21(tmp2(13388).BellZIcon, obj8);
    }
