...
// Dependencies: [32, 19, 1957, 4255, 1371, 12435, 12432, 7952, 1074, 21, 504, 16726, 12431, 12454, 12451, 4451, 4759, 12433, 16810, 12439, 12434, 16768, 16718, 1114, 16730, 2]

// Module 16809 (AutocompleteScreen)
...
import AutocompleteScreenUtils from "AutocompleteScreenUtils" /* 16810 */;
          ...
          if (0 !== results.length) {
            item = results.forEach((item) => {
              ...
              if (tmp22) {
                ...
                const obj2 = { text, icon: tmp(16810).getSearchFilterHasIcon(text), onPress: callback1 };
                element.props = obj2;
                items.push(element);
                const tmpResult3 = tmp(16810);
              }
              if (tmp27) {
                const element1 = { type: constants.GENERIC, props: null };
                const obj3 = { text, icon: tmp(16810).getSearchFilterAuthorTypeIcon(text), onPress: callback1 };
                element1.props = obj3;
                items.push(element1);
                const tmpResult4 = tmp(16810);
              }
            });
          }
        }
