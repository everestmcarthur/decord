...
// Function ID: 16933
// Name: MembersScreen
// Dependencies: [19, 17, 7393, 1958, 2022, 1980, 2012, 1372, 12516, 12491, 8000, 7999, 1074, 21, 4640, 576, 563, 7276, 12492, 16889, 4283, 1875, 12510, 8327, 1115, 4351, 16933, 16931, 16881, 11738, 16893, 16934, 7296, 11742, 12337, 16936, 2]

// Module 16932 (MembersScreen)
...
import SearchMemberTabStore from "SearchMemberTabStore" /* 12516 */;
...
function SearchableMembersScreen(searchContext) {
  ...
  const analyticsLocations = guildId(7276)().analyticsLocations;
  const tmp = closure_21();
  dependencyMap = searchContext(12492).getSearchContextId(searchContext);
  let obj = searchContext(12492);
  ...
  fullscreenPlaceholderCount = searchContext(16889).useFullscreenPlaceholderCount({ placeholderHeight, numColumns: 1 });
  let obj4 = { placeholderHeight, numColumns: 1 };
  const tmp4Result8 = searchContext(16889);
  ...
  const tmp4Result13 = searchContext(16933);
  const messageTabCountsErrorText = searchContext(16931).useMessageTabCountsErrorText({ searchContext });
  if (null != messageTabCountsErrorText) {
    const obj5 = { text: messageTabCountsErrorText };
    let tmp25 = jsx(tmp2(16881), { text: messageTabCountsErrorText });
  } else {
    if (stateFromStores5) {
      if (null != stateFromStores4) {
        ...
        tmp25 = jsx(tmp2(11738), { onUserPress: callback1, onUserLongPress: callback2, channelId: stateFromStores4, guildId, disableStickySections: true, listStyleOverride: tmp.userList, isNameplatedList: true, canShowDisplayNameStylesFont: true });
      }
    }
    const obj7 = { contentContainerStyle: contentContainerStyles.membersContentContainer, data: memo };
    tmp25 = jsx(tmp2(16893), { contentContainerStyle: contentContainerStyles.membersContentContainer, data: memo });
  }
  return tmp25;
}
function ThreadMembersScreen(searchContext) {
  ...
  if (!stateFromStores) {
    if (obj2.useStateFromStores(items1, () => SearchQueryStore.isInitialSearchQuery(searchContext) && !SearchQueryStore.isTagsEmpty(searchContext), items2)) {
      ...
      let tmp7 = jsx(channelId(16934), { channelId, guildId, onUserPress: tmp(1875).dismissGlobalKeyboard, disableStickySections: true });
      const tmp6 = channelId(16934);
    }
    return tmp7;
  }
...
let closure_16 = fn(7999).SearchResultContentEntityTypes;
...
export default noop.memo(function MembersScreen(searchContext) {
  ...
  let stateFromStores;
  let tmp = closure_21();
  const analyticsLocations = stateFromStores(7276)(stateFromStores(7296).SEARCH_MEMBERS).analyticsLocations;
  let channelId;
  if (searchContext.type === constants3.CHANNEL) {
    channelId = searchContext.channelId;
  }
  const tmp4 = stateFromStores(7276);
  const items = [ChannelStore];
  ...
  if (constants3.CHANNEL === type) {
    ...
    if (stateFromStores) {
      ...
      tmp21Result = tmp21(tmp2(16936), obj7);
    }
    obj6.listHeaderContent = tmp21Result;
    obj4.children = jsx(tmp2(12337), { channelId: searchContext.channelId, disableStickySections: true, listStyleOverride: tmp.userList, onUserPress: tmp7(1875).dismissGlobalKeyboard, listHeaderContent: null });
    obj3.children = <View style={tmp.container}>{null}</View>;
    return jsx(tmp7(7276).AnalyticsLocationProvider, { value: analyticsLocations, children: null });
