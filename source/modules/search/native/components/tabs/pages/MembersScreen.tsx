...
// Function ID: 16894
// Name: MembersScreen
// Dependencies: [19, 17, 7389, 1958, 2022, 1980, 2012, 1372, 12507, 12482, 7994, 7993, 1074, 21, 4638, 576, 563, 7272, 12483, 16850, 4281, 1875, 12501, 8308, 1115, 4349, 16894, 16892, 16842, 11730, 16854, 16895, 7292, 11734, 12328, 16897, 2]

// Module 16893 (MembersScreen)
...
import SearchMemberTabStore from "SearchMemberTabStore" /* 12507 */;
...
function SearchableMembersScreen(searchContext) {
  ...
  const analyticsLocations = guildId(7272)().analyticsLocations;
  const tmp = closure_21();
  dependencyMap = searchContext(12483).getSearchContextId(searchContext);
  let obj = searchContext(12483);
  ...
  fullscreenPlaceholderCount = searchContext(16850).useFullscreenPlaceholderCount({ placeholderHeight, numColumns: 1 });
  let obj4 = { placeholderHeight, numColumns: 1 };
  const tmp4Result8 = searchContext(16850);
  ...
  const tmp4Result13 = searchContext(16894);
  const messageTabCountsErrorText = searchContext(16892).useMessageTabCountsErrorText({ searchContext });
  if (null != messageTabCountsErrorText) {
    const obj5 = { text: messageTabCountsErrorText };
    let tmp25 = jsx(tmp2(16842), { text: messageTabCountsErrorText });
  } else {
    if (stateFromStores5) {
      if (null != stateFromStores4) {
        ...
        tmp25 = jsx(tmp2(11730), { onUserPress: callback1, onUserLongPress: callback2, channelId: stateFromStores4, guildId, disableStickySections: true, listStyleOverride: tmp.userList, isNameplatedList: true, canShowDisplayNameStylesFont: true });
      }
    }
    const obj7 = { contentContainerStyle: contentContainerStyles.membersContentContainer, data: memo };
    tmp25 = jsx(tmp2(16854), { contentContainerStyle: contentContainerStyles.membersContentContainer, data: memo });
  }
  return tmp25;
}
function ThreadMembersScreen(searchContext) {
  ...
  if (!stateFromStores) {
    if (obj2.useStateFromStores(items1, () => SearchQueryStore.isInitialSearchQuery(searchContext) && !SearchQueryStore.isTagsEmpty(searchContext), items2)) {
      ...
      let tmp7 = jsx(channelId(16895), { channelId, guildId, onUserPress: tmp(1875).dismissGlobalKeyboard, disableStickySections: true });
      const tmp6 = channelId(16895);
    }
    return tmp7;
  }
...
let closure_16 = fn(7993).SearchResultContentEntityTypes;
...
export default noop.memo(function MembersScreen(searchContext) {
  ...
  let stateFromStores;
  let tmp = closure_21();
  const analyticsLocations = stateFromStores(7272)(stateFromStores(7292).SEARCH_MEMBERS).analyticsLocations;
  let channelId;
  if (searchContext.type === constants3.CHANNEL) {
    channelId = searchContext.channelId;
  }
  const tmp4 = stateFromStores(7272);
  const items = [ChannelStore];
  ...
  if (constants3.CHANNEL === type) {
    ...
    if (stateFromStores) {
      ...
      tmp21Result = tmp21(tmp2(16897), obj7);
    }
    obj6.listHeaderContent = tmp21Result;
    obj4.children = jsx(tmp2(12328), { channelId: searchContext.channelId, disableStickySections: true, listStyleOverride: tmp.userList, onUserPress: tmp7(1875).dismissGlobalKeyboard, listHeaderContent: null });
    obj3.children = <View style={tmp.container}>{null}</View>;
    return jsx(tmp7(7272).AnalyticsLocationProvider, { value: analyticsLocations, children: null });
