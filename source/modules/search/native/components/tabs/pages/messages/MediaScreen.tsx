...
// Function ID: 16805
// Name: MediaScreen
// Dependencies: [19, 7433, 1957, 7384, 12468, 7982, 12482, 1074, 21, 16797, 16752, 16805, 504, 16806, 12467, 4494, 12469, 8378, 16749, 14910, 8529, 8531, 16807, 16811, 16756, 2]

// Module 16804 (MediaScreen)
...
export default noop.memo(function MediaScreen(searchContext) {
  ...
  const searchMessagesLoadingState = searchContext(16806).useSearchMessagesLoadingState({ searchContext, tab, placeholderHeight: tmp2, numColumns });
  ...
  const onPressMediaItem = searchContext(16749).useOnPressMediaItem({ searchContext, allMediaResults: memo, onEndReached: callback, onEndReachedThreshold: 500 });
  ...
  const callback1 = searchMessages.useCallback((media, index) => {
    ...
    if (obj.shouldAgeVerifyForSearchMedia(media, found)) {
      const obj2 = { entryPoint: tmp2(8531).AgeVerificationModalEntryPoint.SEARCH_MEDIA_PREVIEW };
      ...
      obj5.index = index;
      const result1 = tmp2(16807).trackMessageItemPress(obj5);
      onPressMediaItem(media, media.originView);
      const tmp2Result = tmp2(16807);
    }
  }, items4);
  ...
  obj7.ItemSeparatorComponent = searchContext(16756).MediaVerticalSeparator;
  obj7.numColumns = numColumns;
  obj7.isFirstPageLoading = isFirstPageLoading;
  obj7.isNextPageLoading = isNextPageLoading;
  return jsx(tab(16807), { data: memo1, searchContext, tab, isFocused, contentContainerStyle: contentContainerStyles.mediaContentContainer, ItemSeparatorComponent: null, numColumns: null, isFirstPageLoading: null, isNextPageLoading: null });
});
