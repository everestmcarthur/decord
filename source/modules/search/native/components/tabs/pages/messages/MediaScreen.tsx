...
// Function ID: 16790
// Name: MediaScreen
// Dependencies: [19, 7382, 1957, 7333, 12409, 7930, 12423, 1074, 21, 16782, 16737, 16790, 504, 16791, 12408, 4462, 12410, 8325, 16734, 14899, 8476, 8478, 16792, 16796, 16741, 2]

// Module 16789 (MediaScreen)
...
export default noop.memo(function MediaScreen(searchContext) {
  ...
  const tmp2 = tab(16737)(width);
  dependencyMap = tmp2;
  let obj = searchContext(16782);
  const searchMessages = searchContext(16790).useSearchMessages(searchContext, tab);
  let obj2 = searchContext(16790);
  ...
  const searchMessagesLoadingState = searchContext(16791).useSearchMessagesLoadingState({ searchContext, tab, placeholderHeight: tmp2, numColumns });
  ...
  let obj4 = searchContext(16791);
  let obj5 = { searchContext, tab, placeholderHeight: tmp2, numColumns };
  const onPressMediaItem = searchContext(16734).useOnPressMediaItem({ searchContext, allMediaResults: memo, onEndReached: callback, onEndReachedThreshold: 500 });
  ...
  const callback1 = searchMessages.useCallback((media, index) => {
    ...
    if (obj.shouldAgeVerifyForSearchMedia(media, found)) {
      const obj2 = { entryPoint: tmp2(8478).AgeVerificationModalEntryPoint.SEARCH_MEDIA_PREVIEW };
      ...
      obj5.index = index;
      const result1 = tmp2(16792).trackMessageItemPress(obj5);
      onPressMediaItem(media, media.originView);
      const tmp2Result = tmp2(16792);
    }
  }, items4);
  ...
  const obj6 = searchContext(16734);
  obj7.ItemSeparatorComponent = searchContext(16741).MediaVerticalSeparator;
  obj7.numColumns = numColumns;
  obj7.isFirstPageLoading = isFirstPageLoading;
  obj7.isNextPageLoading = isNextPageLoading;
  return jsx(tab(16792), { data: memo1, searchContext, tab, isFocused, contentContainerStyle: contentContainerStyles.mediaContentContainer, ItemSeparatorComponent: null, numColumns: null, isFirstPageLoading: null, isNextPageLoading: null });
});
