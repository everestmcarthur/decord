...
export default noop.memo(function MediaScreen(searchContext) {
  ...
  const contentContainerStyles = searchContext(16770).useContentContainerStyles();
  const tmp2 = tab(16725)(width);
  dependencyMap = tmp2;
  let obj = searchContext(16770);
  const searchMessages = searchContext(16778).useSearchMessages(searchContext, tab);
  let obj2 = searchContext(16778);
  ...
  const searchMessagesLoadingState = searchContext(16779).useSearchMessagesLoadingState({ searchContext, tab, placeholderHeight: tmp2, numColumns });
  ...
  let obj4 = searchContext(16779);
  let obj5 = { searchContext, tab, placeholderHeight: tmp2, numColumns };
  const onPressMediaItem = searchContext(16722).useOnPressMediaItem({ searchContext, allMediaResults: memo, onEndReached: callback, onEndReachedThreshold: 500 });
  ...
  const callback1 = searchMessages.useCallback((media, index) => {
    ...
    if (obj.shouldAgeVerifyForSearchMedia(media, found)) {
      ...
      obj5.index = index;
      const result1 = tmp2(16780).trackMessageItemPress(obj5);
      onPressMediaItem(media, media.originView);
      const tmp2Result = tmp2(16780);
    }
  }, items4);
  ...
  const obj6 = searchContext(16722);
  obj7.ItemSeparatorComponent = searchContext(16729).MediaVerticalSeparator;
  obj7.numColumns = numColumns;
  obj7.isFirstPageLoading = isFirstPageLoading;
  obj7.isNextPageLoading = isNextPageLoading;
  return jsx(tab(16780), { data: memo1, searchContext, tab, isFocused, contentContainerStyle: contentContainerStyles.mediaContentContainer, ItemSeparatorComponent: null, numColumns: null, isFirstPageLoading: null, isNextPageLoading: null });
});
