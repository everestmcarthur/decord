...
// Dependencies: [4254, 7930, 16808, 12410, 16809, 12, 2]

// Module 16807 (MessageSearchResultParser)
import _mod12 from "module_12" /* 12 */;
import MessageRecord from "MessageRecord" /* 4254 */;

require = fn;
let closure_3 = fn(7930).EMBED_TYPES_WITHOUT_DESCRIPTION;
fn(16808).CachedSearchResultParser;
const prototype = function SearchResultMessageParser(searchQueryString, lineClamp) {
  ...
  tmp4.truncateMessage = function truncateMessage(content, tokenizedQueryContent, lineClamp) {
    ...
    if (null == content) {
      ...
      const _Set = Set;
      set = new Set(set(16809).analyze(tokenizedQueryContent));
      const parts = str4.split(/(\W+)/g);
      const found = parts.find((item) => set(16809).shouldHighlight(item, set));
      if (null == found) {
        return null;
      } else {
