...
// Dependencies: [19, 17, 1957, 4828, 11441, 1074, 21, 4574, 11747, 1896, 4607, 576, 7343, 11720, 4304, 1114, 4752, 8449, 7838, 7850, 4464, 11, 1100, 7224, 7197, 11748, 4603, 11750, 9950, 11751, 8024, 2]
// Exports: default, openSummaryDividerActionSheet

// Module 11747 (SummaryActionSheet)
...
export default function SummaryActionSheet(summary) {
  ...
  const callback = obj.useCallback(() => {
    ...
    if (null != guild_id) {
      ...
      tmpResult.openLazy(asyncRequireImpl(11720, dependencyMap.paths), "GuildHighlightsNotifications", obj2);
      const tmp7 = asyncRequireImpl(11720, dependencyMap.paths);
    }
  }, items);
  ...
  const callback2 = obj.useCallback(() => {
    ...
    if (null != channel) {
      if (null != message) {
        const result = tmp(7838).openThreadCreationForMobile(tmp4, summary.startId, constants.SUMMARY_ACTION_SHEET);
        const tmpResult = tmp(7838);
        const obj3 = { name: summary.topic };
        tmp(7850).changeThreadSettings(tmp4.id, obj3);
        const tmp11 = require;
        const tmpResult4 = tmp(7850);
...
export const openSummaryDividerActionSheet = function openSummaryDividerActionSheet(channelId, summaryId) {
  ...
  if (null != findSummaryResult) {
    ...
    obj.openLazy(asyncRequireImpl(11747, dependencyMap.paths), "SummaryDivider" + summaryId, obj2);
    const tmp5 = asyncRequireImpl(11747, dependencyMap.paths);
  }
};
