...
// Dependencies: [19, 17, 1957, 4827, 11439, 1074, 21, 4573, 11745, 1896, 4606, 576, 7342, 11718, 4304, 1114, 4751, 8447, 7837, 7849, 4464, 11, 1100, 7223, 7196, 11746, 4602, 11748, 9948, 11749, 8023, 2]
...
export default function SummaryActionSheet(summary) {
  ...
  const callback = obj.useCallback(() => {
    ...
    if (null != guild_id) {
      ...
      tmpResult.openLazy(asyncRequireImpl(11718, dependencyMap.paths), "GuildHighlightsNotifications", obj2);
  ...
  const callback2 = obj.useCallback(() => {
    ...
    if (null != channel) {
      if (null != message) {
        const result = tmp(7837).openThreadCreationForMobile(tmp4, summary.startId, constants.SUMMARY_ACTION_SHEET);
        const tmpResult = tmp(7837);
        const obj3 = { name: summary.topic };
        tmp(7849).changeThreadSettings(tmp4.id, obj3);
        const tmp11 = require;
        const tmpResult4 = tmp(7849);
...
export const openSummaryDividerActionSheet = function openSummaryDividerActionSheet(channelId, summaryId) {
  ...
  if (null != findSummaryResult) {
    ...
    obj.openLazy(asyncRequireImpl(11745, dependencyMap.paths), "SummaryDivider" + summaryId, obj2);
