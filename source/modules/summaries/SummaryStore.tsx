...
// Function ID: 11500
// Name: SummaryStore
// Dependencies: [32, 4555, 9985, 1958, 1980, 4655, 2012, 4821, 1372, 11500, 9986, 504, 11, 1091, 11501, 573, 11502, 12, 2]

// Module 11499 (SummaryStore)
...
import Summary from "Summary" /* 11502 */;
...
const SUMMARY_POLL_INTERVAL = fn(11500).SUMMARY_POLL_INTERVAL;
...
obj2 = {
  ...
  RECEIVE_CHANNEL_SUMMARIES_BULK(requestArgs) {
    ...
    dependencyMap = _modDef12.toPairs(requestArgs.summaries).reduce((acc, item) => {
      ...
      const first = tmp[0];
      obj = error(12);
      const chainResult = error(12).chain(tmp[1].map((item) => lastReceivedAt(11502).createSummaryFromServer(item, first)));
      const sortByResult = error(12).chain(tmp[1].map((item) => lastReceivedAt(11502).createSummaryFromServer(item, first))).sortBy((startId) => error(11).extractTimestamp(startId.startId));
      const reversed = error(12).chain(tmp[1].map((item) => lastReceivedAt(11502).createSummaryFromServer(item, first))).sortBy((startId) => error(11).extractTimestamp(startId.startId)).takeRight(75).reverse();
      const takeRightResult = error(12).chain(tmp[1].map((item) => lastReceivedAt(11502).createSummaryFromServer(item, first))).sortBy((startId) => error(11).extractTimestamp(startId.startId)).takeRight(75);
