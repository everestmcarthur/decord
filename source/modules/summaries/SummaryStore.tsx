...
// Function ID: 11598
// Name: SummaryStore
// Dependencies: [32, 4637, 10079, 1958, 1980, 4737, 2012, 4903, 1372, 11598, 10080, 504, 11, 1091, 11599, 573, 11600, 12, 2]

// Module 11597 (SummaryStore)
...
import Summary from "Summary" /* 11600 */;
...
const SUMMARY_POLL_INTERVAL = fn(11598).SUMMARY_POLL_INTERVAL;
...
obj2 = {
  ...
  RECEIVE_CHANNEL_SUMMARIES_BULK(requestArgs) {
    ...
    dependencyMap = _modDef12.toPairs(requestArgs.summaries).reduce((acc, item) => {
      ...
      const first = tmp[0];
      obj = error(12);
      const chainResult = error(12).chain(tmp[1].map((item) => lastReceivedAt(11600).createSummaryFromServer(item, first)));
      const sortByResult = error(12).chain(tmp[1].map((item) => lastReceivedAt(11600).createSummaryFromServer(item, first))).sortBy((startId) => error(11).extractTimestamp(startId.startId));
      const reversed = error(12).chain(tmp[1].map((item) => lastReceivedAt(11600).createSummaryFromServer(item, first))).sortBy((startId) => error(11).extractTimestamp(startId.startId)).takeRight(75).reverse();
      const takeRightResult = error(12).chain(tmp[1].map((item) => lastReceivedAt(11600).createSummaryFromServer(item, first))).sortBy((startId) => error(11).extractTimestamp(startId.startId)).takeRight(75);
