...
// Function ID: 11766
// Name: ForwardPreview
// Dependencies: [19, 17, 21, 4605, 576, 4540, 8189, 7984, 8722, 11766, 1894, 11767, 12, 11768, 1114, 11770, 10202, 5137, 10182, 5637, 1476, 8884, 4601, 2914, 8206, 2]
// Exports: ForwardPreview

// Module 11765 (ForwardPreview)
...
import ForwardPreviewUtils from "ForwardPreviewUtils" /* 11766 */;
import MosaicMediaType from "MosaicMediaType" /* 11768 */;
...
function MessagePreview(content) {
  ...
  const tmp4 = attachmentCount(4605).createNativeStyleProperties({ seeMoreLabelColor: TEXT_SUBTLE })(tmp3);
  importDefault = tmp4;
...
export const ForwardPreview = function ForwardPreview(message) {
  ...
  if (null != checkpointData) {
    ...
    tmp6 = tmp2(11767).CHECKPOINT_PERSONA_COLORS[num];
  }
  if (tmp6 != null) {
  ...
  if (attachments.length > 0) {
    ...
    if (num2 > 0) {
      if (num3 > 0) {
        ...
        let AttachmentIcon = tmp2(11770).ImagesIcon;
      }
      if (num3 > 0) {
        if (length === num3) {
          ...
          items1[1] = hasOwnProperty(tmp2(10202).CirclePlayIcon, obj5);
    ...
    if (num3 > 0) {
      ...
      AttachmentIcon = tmp2(10202).CirclePlayIcon;
    } else if (num2 > 0) {
      const intl2 = tmp2(1114).intl;
      const obj12 = { count: num2 };
      if (1 === num2) {
        let ImagesIcon = tmp2(5137).ImageIcon;
      } else {
        ImagesIcon = tmp2(11770).ImagesIcon;
      }
      ...
      AttachmentIcon = tmp2(10182).AttachmentIcon;
    }
  ...
  if (attachments.length > 1) {
    tmp33 = tmp7;
    if (null != tmp7) {
      const size3 = { shape: tmp2(8884).CutoutShape.RoundedRect, x: 28, y: 28, width: 32, height: 32, cornerRadius: 12 };
      ...
      items3[1] = timestampProducer(tmp2(4601).Text, obj17);
  ...
  if (tmp36Result) {
    ...
    tmp36Result = tmp36(tmp2(4601).Text, obj22);
  }
  ...
  if (tmp34Result) {
    ...
    if (tmp36Result7) {
      ...
      tmp36Result7 = tmp36(tmp2(4601).Text, obj26);
    }
  ...
  if (tmp36Result8) {
    ...
    const obj29 = { uri: tmp2(8206).getCardAssetUrl(num5) };
    size4.source = obj29;
    obj27.children = tmp36(tmp46, size4);
    tmp36Result8 = tmp36(tmp35, obj27);
    const tmp2Result2 = tmp2(8206);
  }
