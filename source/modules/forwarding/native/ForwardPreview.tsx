...
// Function ID: 11790
// Name: ForwardPreview
// Dependencies: [19, 17, 21, 4607, 576, 4542, 8212, 8007, 8745, 11790, 1894, 11791, 12, 11792, 1114, 11794, 10223, 5139, 10203, 5639, 1476, 8907, 4603, 2914, 8229, 2]
// Exports: ForwardPreview

// Module 11789 (ForwardPreview)
...
import ChatItemDefault from "ChatItem" /* 8745 */;
import ClipViewDefault from "ClipView" /* 8907 */;
import ForwardPreviewUtils from "ForwardPreviewUtils" /* 11790 */;
import MosaicMediaType from "MosaicMediaType" /* 11792 */;
...
function MessagePreview(content) {
  ...
  const tmp4 = attachmentCount(4607).createNativeStyleProperties({ seeMoreLabelColor: TEXT_SUBTLE })(tmp3);
  importDefault = tmp4;
...
export const ForwardPreview = function ForwardPreview(message) {
  ...
  if (null != checkpointData) {
    ...
    tmp6 = tmp2(11791).CHECKPOINT_PERSONA_COLORS[num];
  }
  if (tmp6 != null) {
  ...
  if (attachments.length > 0) {
    ...
    if (num2 > 0) {
      if (num3 > 0) {
        ...
        let AttachmentIcon = tmp2(11794).ImagesIcon;
      }
      if (num3 > 0) {
        if (length === num3) {
          ...
          items1[1] = hasOwnProperty(tmp2(10223).CirclePlayIcon, obj5);
    ...
    if (num3 > 0) {
      ...
      AttachmentIcon = tmp2(10223).CirclePlayIcon;
    } else if (num2 > 0) {
      const intl2 = tmp2(1114).intl;
      const obj12 = { count: num2 };
      if (1 === num2) {
        let ImagesIcon = tmp2(5139).ImageIcon;
      } else {
        ImagesIcon = tmp2(11794).ImagesIcon;
      }
      ...
      AttachmentIcon = tmp2(10203).AttachmentIcon;
    }
  ...
  if (attachments.length > 1) {
    tmp33 = tmp7;
    if (null != tmp7) {
      const size3 = { shape: tmp2(8907).CutoutShape.RoundedRect, x: 28, y: 28, width: 32, height: 32, cornerRadius: 12 };
      ...
      items3[1] = timestampProducer(tmp2(4603).Text, obj17);
  ...
  if (tmp36Result) {
    ...
    tmp36Result = tmp36(tmp2(4603).Text, obj22);
  }
  ...
  if (tmp34Result) {
    ...
    if (tmp36Result7) {
      ...
      tmp36Result7 = tmp36(tmp2(4603).Text, obj26);
    }
  ...
  if (tmp36Result8) {
    ...
    const obj29 = { uri: tmp2(8229).getCardAssetUrl(num5) };
    size4.source = obj29;
