...
// Function ID: 11726
// Name: ForwardPreview
// Dependencies: [19, 17, 21, 4574, 576, 4509, 8152, 7946, 8685, 11726, 1894, 11727, 12, 11728, 1114, 11730, 10164, 5101, 10144, 5601, 1476, 8847, 4570, 2914, 8169, 2]
// Exports: ForwardPreview

// Module 11725 (ForwardPreview)
...
import ForwardPreviewUtils from "ForwardPreviewUtils" /* 11726 */;
import MosaicMediaType from "MosaicMediaType" /* 11728 */;
...
function MessagePreview(content) {
  ...
  const tmp4 = attachmentCount(4574).createNativeStyleProperties({ seeMoreLabelColor: TEXT_SUBTLE })(tmp3);
  importDefault = tmp4;
...
export const ForwardPreview = function ForwardPreview(message) {
  ...
  if (null != checkpointData) {
    ...
    tmp6 = tmp2(11727).CHECKPOINT_PERSONA_COLORS[num];
  }
  if (tmp6 != null) {
  ...
  if (attachments.length > 0) {
    ...
    if (num2 > 0) {
      if (num3 > 0) {
        ...
        let AttachmentIcon = tmp2(11730).ImagesIcon;
      }
      if (num3 > 0) {
        if (length === num3) {
          ...
          items1[1] = hasOwnProperty(tmp2(10164).CirclePlayIcon, obj5);
    ...
    if (num3 > 0) {
      ...
      AttachmentIcon = tmp2(10164).CirclePlayIcon;
    } else if (num2 > 0) {
      const intl2 = tmp2(1114).intl;
      const obj12 = { count: num2 };
      if (1 === num2) {
        let ImagesIcon = tmp2(5101).ImageIcon;
      } else {
        ImagesIcon = tmp2(11730).ImagesIcon;
      }
      ...
      AttachmentIcon = tmp2(10144).AttachmentIcon;
    }
  ...
  if (attachments.length > 1) {
    tmp33 = tmp7;
    if (null != tmp7) {
      const size3 = { shape: tmp2(8847).CutoutShape.RoundedRect, x: 28, y: 28, width: 32, height: 32, cornerRadius: 12 };
      ...
      items3[1] = timestampProducer(tmp2(4570).Text, obj17);
  ...
  if (tmp36Result) {
    ...
    tmp36Result = tmp36(tmp2(4570).Text, obj22);
  }
  ...
  if (tmp34Result) {
    ...
    if (tmp36Result7) {
      ...
      tmp36Result7 = tmp36(tmp2(4570).Text, obj26);
    }
  ...
  if (tmp36Result8) {
    ...
    const obj29 = { uri: tmp2(8169).getCardAssetUrl(num5) };
    size4.source = obj29;
    obj27.children = tmp36(tmp46, size4);
    tmp36Result8 = tmp36(tmp35, obj27);
    const tmp2Result2 = tmp2(8169);
  }
