...
// Function ID: 11788
// Name: ForwardPreview
// Dependencies: [19, 17, 21, 4606, 576, 4541, 8211, 8006, 8743, 11788, 1894, 11789, 12, 11790, 1114, 11792, 10221, 5138, 10201, 5638, 1476, 8905, 4602, 2914, 8228, 2]
// Exports: ForwardPreview

// Module 11787 (ForwardPreview)
...
import ForwardPreviewUtils from "ForwardPreviewUtils" /* 11788 */;
import MosaicMediaType from "MosaicMediaType" /* 11790 */;
...
function MessagePreview(content) {
  ...
  const tmp4 = attachmentCount(4606).createNativeStyleProperties({ seeMoreLabelColor: TEXT_SUBTLE })(tmp3);
  importDefault = tmp4;
...
export const ForwardPreview = function ForwardPreview(message) {
  ...
  if (null != checkpointData) {
    ...
    tmp6 = tmp2(11789).CHECKPOINT_PERSONA_COLORS[num];
  }
  if (tmp6 != null) {
  ...
  if (attachments.length > 0) {
    ...
    if (num2 > 0) {
      if (num3 > 0) {
        ...
        let AttachmentIcon = tmp2(11792).ImagesIcon;
      }
      if (num3 > 0) {
        if (length === num3) {
          ...
          items1[1] = hasOwnProperty(tmp2(10221).CirclePlayIcon, obj5);
    ...
    if (num3 > 0) {
      ...
      AttachmentIcon = tmp2(10221).CirclePlayIcon;
    } else if (num2 > 0) {
      const intl2 = tmp2(1114).intl;
      const obj12 = { count: num2 };
      if (1 === num2) {
        let ImagesIcon = tmp2(5138).ImageIcon;
      } else {
        ImagesIcon = tmp2(11792).ImagesIcon;
      }
      ...
      AttachmentIcon = tmp2(10201).AttachmentIcon;
    }
  ...
  if (attachments.length > 1) {
    tmp33 = tmp7;
    if (null != tmp7) {
      const size3 = { shape: tmp2(8905).CutoutShape.RoundedRect, x: 28, y: 28, width: 32, height: 32, cornerRadius: 12 };
      ...
      items3[1] = timestampProducer(tmp2(4602).Text, obj17);
  ...
  if (tmp36Result) {
    ...
    tmp36Result = tmp36(tmp2(4602).Text, obj22);
  }
  ...
  if (tmp34Result) {
    ...
    if (tmp36Result7) {
      ...
      tmp36Result7 = tmp36(tmp2(4602).Text, obj26);
    }
  ...
  if (tmp36Result8) {
    ...
    const obj29 = { uri: tmp2(8228).getCardAssetUrl(num5) };
    size4.source = obj29;
