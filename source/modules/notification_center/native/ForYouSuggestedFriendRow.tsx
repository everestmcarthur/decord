...
// Dependencies: [19, 17, 4566, 4222, 1074, 21, 4574, 10153, 576, 1115, 7176, 4996, 563, 8205, 1896, 4418, 16076, 1114, 4310, 16474, 5137, 16475, 1178, 4570, 16476, 16477, 1242, 2]
// Exports: default

// Module 16473 (ForYouSuggestedFriendRow)
...
export default function ForYouSuggestedFriendRow(suggestedFriend) {
  ...
  let sharedValue;
  let stateFromStores;
  const messagesTabLayout = suggestedFriend(10153).useMessagesTabLayout(panelVariant);
  const tmp4 = closure_12(messagesTabLayout);
  const analyticsLocations = useAnalyticsLocationsDefault().analyticsLocations;
  const obj = suggestedFriend(10153);
  const layoutStyles = suggestedFriend(10153).getLayoutStyles(messagesTabLayout);
  const obj2 = suggestedFriend(10153);
  const fontScale = suggestedFriend(4996).useFontScale();
  const obj3 = suggestedFriend(4996);
  ...
  if (null != suggestedFriend.friendSuggestionName) {
    ...
    const suggestedContactNameForSuggestion = tmp(16076).getSuggestedContactNameForSuggestion(friendSuggestionName, suggestedFriend);
    let str2 = "";
    ...
    if (null != suggestedFriend.mutualFriendsCount) {
      ...
      sharedValue = tmp(4310).useSharedValue(false);
      const tmpResult8 = tmp(4310);
      ...
      const items6 = [closure_9(tmp(4570).Text, obj12), ];
      const tmp20 = closure_11;
      const tmpResult11 = tmp(16475);
      let num3 = 0;
      ...
      obj13.children = closure_9(tmp(16476).ActionStatusSubLabel, obj15);
      ...
      items5[2] = closure_9(tmp(16477).ContactSuggestionActions, obj16);
      ...
      return tmpResult10.renderChannelPressableWrapper(closure_9(tmp(5137).PressableHighlight, obj7), obj19);
    }
    const tmpResult = tmp(16076);
    formatToPlainStringResult = tmp5(4418).getName(suggestedFriend.user);
    const tmp5Result = tmp5(4418);
  }
