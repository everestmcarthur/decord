...
// Dependencies: [19, 17, 4632, 4288, 1074, 21, 4640, 10270, 576, 1364, 7276, 5069, 563, 8327, 1897, 4484, 16203, 1115, 4376, 16600, 5210, 16601, 1177, 4636, 16602, 16603, 1241, 2]
// Exports: default

// Module 16599 (ForYouSuggestedFriendRow)
...
export default function ForYouSuggestedFriendRow(suggestedFriend) {
  ...
  let sharedValue;
  let stateFromStores;
  const messagesTabLayout = suggestedFriend(10270).useMessagesTabLayout(panelVariant);
  const tmp4 = closure_12(messagesTabLayout);
  const analyticsLocations = useAnalyticsLocationsDefault().analyticsLocations;
  const obj = suggestedFriend(10270);
  const layoutStyles = suggestedFriend(10270).getLayoutStyles(messagesTabLayout);
  const obj2 = suggestedFriend(10270);
  const fontScale = suggestedFriend(5069).useFontScale();
  const obj3 = suggestedFriend(5069);
  ...
  if (null != suggestedFriend.friendSuggestionName) {
    ...
    const suggestedContactNameForSuggestion = tmp(16203).getSuggestedContactNameForSuggestion(friendSuggestionName, suggestedFriend);
    let str2 = "";
    ...
    if (null != suggestedFriend.mutualFriendsCount) {
      ...
      sharedValue = tmp(4376).useSharedValue(false);
      const tmpResult8 = tmp(4376);
      ...
      const items6 = [closure_9(tmp(4636).Text, obj12), ];
      const tmp20 = closure_11;
      const tmpResult11 = tmp(16601);
      let num3 = 0;
      ...
      obj13.children = closure_9(tmp(16602).ActionStatusSubLabel, obj15);
      ...
      items5[2] = closure_9(tmp(16603).ContactSuggestionActions, obj16);
      ...
      return tmpResult10.renderChannelPressableWrapper(closure_9(tmp(5210).PressableHighlight, obj7), obj19);
    }
    const tmpResult = tmp(16203);
    formatToPlainStringResult = tmp5(4484).getName(suggestedFriend.user);
