...
// Dependencies: [19, 17, 4598, 4255, 1074, 21, 4606, 10210, 576, 1115, 7235, 5033, 563, 8264, 1896, 4451, 16114, 1114, 4343, 16511, 5174, 16512, 1178, 4602, 16513, 16514, 1242, 2]
// Exports: default

// Module 16510 (ForYouSuggestedFriendRow)
...
export default function ForYouSuggestedFriendRow(suggestedFriend) {
  ...
  let sharedValue;
  let stateFromStores;
  const messagesTabLayout = suggestedFriend(10210).useMessagesTabLayout(panelVariant);
  const tmp4 = closure_12(messagesTabLayout);
  const analyticsLocations = useAnalyticsLocationsDefault().analyticsLocations;
  const obj = suggestedFriend(10210);
  const layoutStyles = suggestedFriend(10210).getLayoutStyles(messagesTabLayout);
  const obj2 = suggestedFriend(10210);
  const fontScale = suggestedFriend(5033).useFontScale();
  const obj3 = suggestedFriend(5033);
  ...
  if (null != suggestedFriend.friendSuggestionName) {
    ...
    const suggestedContactNameForSuggestion = tmp(16114).getSuggestedContactNameForSuggestion(friendSuggestionName, suggestedFriend);
    let str2 = "";
    ...
    if (null != suggestedFriend.mutualFriendsCount) {
      if (suggestedFriend.mutualFriendsCount > 0) {
        ...
        let formatToPlainStringResult = intl.formatToPlainString(tmp(1114).t.z7y34b, obj6);
      }
      sharedValue = tmp(4343).useSharedValue(false);
      const tmpResult8 = tmp(4343);
      ...
      obj9.children = closure_9(tmp(1178).Avatar, obj10);
      ...
      const items6 = [closure_9(tmp(4602).Text, obj12), ];
      const tmp20 = closure_11;
      const tmpResult11 = tmp(16512);
      let num3 = 0;
      ...
      obj15.actionStatus = intl2.string(tmp(1114).t.Kzyxm9);
      obj15.animate = !stateFromStoresObject;
      obj13.children = closure_9(tmp(16513).ActionStatusSubLabel, obj15);
      ...
      items5[2] = closure_9(tmp(16514).ContactSuggestionActions, obj16);
      ...
      return tmpResult10.renderChannelPressableWrapper(closure_9(tmp(5174).PressableHighlight, obj7), obj19);
    }
    const tmpResult = tmp(16114);
    formatToPlainStringResult = tmp5(4451).getName(suggestedFriend.user);
    const tmp5Result = tmp5(4451);
  }
