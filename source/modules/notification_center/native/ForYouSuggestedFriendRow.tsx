...
// Dependencies: [19, 17, 4628, 4285, 1074, 21, 4636, 10249, 576, 1150, 7265, 5063, 563, 8296, 1896, 4481, 16138, 1114, 4373, 16535, 5204, 16536, 1178, 4632, 16537, 16538, 1242, 2]
// Exports: default

// Module 16534 (ForYouSuggestedFriendRow)
...
export default function ForYouSuggestedFriendRow(suggestedFriend) {
  ...
  let sharedValue;
  let stateFromStores;
  const messagesTabLayout = suggestedFriend(10249).useMessagesTabLayout(panelVariant);
  const tmp4 = closure_12(messagesTabLayout);
  const analyticsLocations = useAnalyticsLocationsDefault().analyticsLocations;
  const obj = suggestedFriend(10249);
  const layoutStyles = suggestedFriend(10249).getLayoutStyles(messagesTabLayout);
  const obj2 = suggestedFriend(10249);
  const fontScale = suggestedFriend(5063).useFontScale();
  const obj3 = suggestedFriend(5063);
  ...
  if (null != suggestedFriend.friendSuggestionName) {
    ...
    const suggestedContactNameForSuggestion = tmp(16138).getSuggestedContactNameForSuggestion(friendSuggestionName, suggestedFriend);
    let str2 = "";
    ...
    if (null != suggestedFriend.mutualFriendsCount) {
      ...
      sharedValue = tmp(4373).useSharedValue(false);
      const tmpResult8 = tmp(4373);
      ...
      const items6 = [closure_9(tmp(4632).Text, obj12), ];
      const tmp20 = closure_11;
      const tmpResult11 = tmp(16536);
      let num3 = 0;
      ...
      obj13.children = closure_9(tmp(16537).ActionStatusSubLabel, obj15);
      ...
      items5[2] = closure_9(tmp(16538).ContactSuggestionActions, obj16);
      ...
      return tmpResult10.renderChannelPressableWrapper(closure_9(tmp(5204).PressableHighlight, obj7), obj19);
    }
    const tmpResult = tmp(16138);
    formatToPlainStringResult = tmp5(4481).getName(suggestedFriend.user);
    const tmp5Result = tmp5(4481);
  }
