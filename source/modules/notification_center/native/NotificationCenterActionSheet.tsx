...
// Dependencies: [19, 7751, 1074, 21, 504, 16568, 7975, 4607, 7970, 7973, 7296, 7980, 7965, 12362, 7496, 7311, 7263, 1115, 7313, 5177, 12574, 9927, 16569, 11858, 7981, 4602, 12360, 7494, 2]
// Exports: default

// Module 16567 (NotificationCenterActionSheet)
...
import MentionActionCreatorsDefault from "MentionActionCreators" /* 16568 */;
...
export default function NotificationCenterActionSheet() {
  ...
  const isForLaterExperimentOn = roleFilter(7975).useIsForLaterExperimentOn("NotificationCenterActionSheet");
  let obj2 = roleFilter(7975);
  hasForLaterAccess = roleFilter(7975).useHasForLaterAccess("NotificationCenterActionSheet");
  ...
  closure_4 = hasForLaterAccess.useCallback((BOOKMARK) => {
    ...
    if (hasForLaterAccess) {
      ...
      const items = [tmp(7296).FOR_LATER_ROADBLOCK];
      tmp(7970)(EntitlementFeatureNames.EntitlementFeatureNames.SAVED_MESSAGES, undefined, items);
      const tmpResult = tmp(7970);
    }
  }, items2);
  const obj3 = roleFilter(7975);
  const canUseScheduledMessages = roleFilter(7965).useCanUseScheduledMessages();
  const callback = hasForLaterAccess.useCallback(() => {
    everyoneFilter(4607).hideActionSheet();
    const obj = everyoneFilter(4607);
    const result = roleFilter(12362).showScheduledMessagesModal();
  }, []);
  const callback1 = hasForLaterAccess.useCallback(() => {
    everyoneFilter(4607).hideActionSheet();
    const obj = everyoneFilter(4607);
    roleFilter(7496).openUserSettings({ screen: constants.NOTIFICATIONS });
  }, []);
  ...
  obj5.header = closure_6(roleFilter(7263).BottomSheetTitleHeader, obj6);
  ...
  const obj4 = roleFilter(7965);
  obj8.icon = closure_6(roleFilter(7313).ActionSheetRow.Icon, { IconComponent: roleFilter(5177).AtIcon, source: everyoneFilter(12574) });
  const items3 = [closure_6(roleFilter(7313).ActionSheetSwitchRow, obj8), ];
  const obj10 = {
    onValueChange() {
  ...
  const obj9 = { IconComponent: roleFilter(5177).AtIcon, source: everyoneFilter(12574) };
  obj10.icon = closure_6(roleFilter(7313).ActionSheetRow.Icon, { IconComponent: roleFilter(9927).BellIcon, source: everyoneFilter(16569) });
  items3[1] = closure_6(roleFilter(7313).ActionSheetSwitchRow, obj10);
  obj7.children = items3;
  const items4 = [closure_7(roleFilter(7313).ActionSheetRow.Group, obj7), ];
  let tmp10Result = null;
  if (isForLaterExperimentOn) {
    const obj12 = { icon: null, label: null, onPress: null, arrow: true };
    const obj13 = { IconComponent: tmp(11858).BookmarkIcon };
    obj12.icon = tmp10(tmp(7313).ActionSheetRow.Icon, obj13);
    const intl5 = tmp(1115).intl;
    obj12.label = intl5.string(tmp(1115).t["2pAkDA"]);
    obj12.onPress = function onPress() {
      return closure_4(SavedMessagesTypes.SavedMessageSortTypes.BOOKMARK);
    };
    tmp10Result = tmp10(tmp(7313).ActionSheetRow, obj12, "bookmarks");
  }
  ...
  if (isForLaterExperimentOn) {
    ...
    obj14.icon = tmp10(tmp(7313).ActionSheetRow.Icon, obj15);
    const intl6 = tmp(1115).intl;
    obj14.label = intl6.string(tmp(1115).t.aUXxzT);
    obj14.onPress = function onPress() {
      return closure_4(SavedMessagesTypes.SavedMessageSortTypes.REMINDER);
    };
    tmp10Result3 = tmp10(tmp(7313).ActionSheetRow, obj14, "reminders");
  }
  items5[1] = tmp10Result3;
  let tmp10Result4 = null;
  if (canUseScheduledMessages) {
    const obj16 = { icon: null, label: null, onPress: null, arrow: true };
    const obj17 = { IconComponent: tmp(12360).CalendarPlusIcon };
    obj16.icon = tmp10(tmp(7313).ActionSheetRow.Icon, obj17);
    const intl7 = tmp(1115).intl;
    obj16.label = intl7.string(tmp(1115).t.SZVs3K);
    obj16.onPress = callback;
    tmp10Result4 = tmp10(tmp(7313).ActionSheetRow, obj16, "scheduled-messages");
  }
  const obj18 = { hasIcons: true, children: null };
  items5[2] = tmp10Result4;
  const obj19 = { icon: null, label: null, onPress: null, arrow: true };
  const obj11 = { IconComponent: roleFilter(9927).BellIcon, source: everyoneFilter(16569) };
  obj19.icon = closure_6(roleFilter(7313).ActionSheetRow.Icon, { IconComponent: roleFilter(7494).SettingsIcon });
  const intl8 = tmp(1115).intl;
  obj19.label = intl8.string(roleFilter(1115).t.h850Ss);
  obj19.onPress = callback1;
  items5[3] = closure_6(roleFilter(7313).ActionSheetRow, obj19, "settings");
  obj18.children = items5;
  items4[1] = closure_7(roleFilter(7313).ActionSheetRow.Group, obj18);
  obj5.children = items4;
  return closure_7(roleFilter(7311).ActionSheet, obj5);
};
