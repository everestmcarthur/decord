...
// Dependencies: [5, 19, 4605, 1074, 8485, 1272, 8723, 8490, 573, 38, 8721, 4786, 2]
...
// Module 8724 (in_app_reports/ReportUtils)
...
import ReportMenuType from "ReportMenuType" /* 8723 */;
...
function genSubmitData(version, name, arr, email_token) {
  ...
  let obj2 = { channel_id: "Array", message_id: "call", stage_instance_id: "end", guild_id: "window", guild_scheduled_event_id: "HermesInternal", user_id: "w", email_token: "__closure", application_id: "__closure", entrypoint: "__closure", widget_id: "__closure" };
  obj.language = str;
  ...
  if (name.name !== MenuTypes.ReportNames.MESSAGE) {
    if (name.name !== tmp(8721).ReportNames.FIRST_DM) {
      if (name.name !== tmp(8721).ReportNames.GUILD) {
        if (name.name !== tmp(8721).ReportNames.GUILD_DISCOVERY) {
          if (name.name === tmp(8721).ReportNames.GUILD_DIRECTORY_ENTRY) {
            const obj3 = {};
            ...
            return obj3;
          } else if (name.name === tmp(8721).ReportNames.STAGE_CHANNEL) {
            const obj6 = {};
            ...
            return obj6;
          } else if (name.name === tmp(8721).ReportNames.GUILD_SCHEDULED_EVENT) {
            const obj7 = {};
            ...
            if (name.name === tmp(8721).ReportNames.USER) {
              const obj8 = {};
              ...
              let tmp3 = obj8;
            } else if (name.name === tmp(8721).UnauthenticatedReportNames.USER) {
              const obj9 = {};
              ...
              tmp3 = obj9;
            } else if (name.name === tmp(8721).UnauthenticatedReportNames.MESSAGE) {
              const obj10 = {};
              ...
              tmp3 = obj10;
            } else if (name.name === tmp(8721).UnauthenticatedReportNames.GUILD) {
              const obj11 = {};
              ...
              tmp3 = obj11;
            } else if (name.name === tmp(8721).ReportNames.APPLICATION) {
              const obj12 = {};
              ...
              tmp3 = obj12;
            } else if (name.name === tmp(8721).ReportNames.WIDGET) {
              const obj13 = {};
              ...
              tmp3 = null;
              if (name.name === tmp(8721).UnauthenticatedReportNames.MEDIA_TAKEDOWN) {
                const obj14 = {};
...
export const submitReport = function submitReport(language, name, arr) {
  if (DevSettingsStore.get("iar_skip_api_report_submit")) {
    ...
    const REPORT_TO_MOD = obj4(8723).ReportMenuTypeSets.REPORT_TO_MOD;
    if (REPORT_TO_MOD.has(name.name)) {
      ...
      let tmp15 = null;
      if (name.name === tmp4(8721).ModeratorReportNames.MESSAGE) {
        let obj2 = { channel_id: "Array", message_id: "call", guild_id: "__packager_asset" };
        obj4 = {};
      ...
      const REPORT_TO_MOD2 = tmp4(8723).ReportMenuTypeSets.REPORT_TO_MOD;
      ...
      name = name.name;
      let _Object = Object;
      const values = Object.values(tmp4(8721).ReportNames);
...
export const trackCloseReportModalAnalytics = function trackCloseReportModalAnalytics(name, c12, onSubmit) {
  ...
  if (name.name !== MenuTypes.ReportNames.GUILD) {
    if (name.name !== tmp2(8721).ReportNames.GUILD_DISCOVERY) {
      if (name.name === tmp2(8721).ReportNames.GUILD_DIRECTORY_ENTRY) {
        let id2 = name.record.guildId;
      } else if (name.name === tmp2(8721).ReportNames.GUILD_SCHEDULED_EVENT) {
        id2 = name.record.guild_id;
      }
    }
    obj2.guild_id = id2;
    if (name.name === tmp2(8721).ReportNames.GUILD_SCHEDULED_EVENT) {
      let channelId = name.record.channel_id;
    } else if (name.name === tmp2(8721).ReportNames.GUILD_DIRECTORY_ENTRY) {
      channelId = name.record.channelId;
    }
    obj2.channel_id = channelId;
    let id4;
    if (name.name === tmp2(8721).ReportNames.APPLICATION) {
      id4 = name.record.id;
    }
