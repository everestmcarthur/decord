...
// Dependencies: [5, 19, 4637, 1074, 8529, 1271, 8767, 8534, 573, 38, 8765, 4818, 2]
...
// Module 8768 (in_app_reports/ReportUtils)
...
import MenuTypes from "MenuTypes" /* 8765 */;
import ReportMenuType from "ReportMenuType" /* 8767 */;
...
function genSubmitData(version, name, arr, email_token) {
  ...
  let obj2 = { channel_id: "apply", message_id: "enabled", stage_instance_id: "intl", guild_id: "a", guild_scheduled_event_id: "isArray", user_id: "assign", email_token: "ks", application_id: "a", entrypoint: "isArray", widget_id: "getCurrentUser" };
  obj.language = str;
  ...
  if (name.name !== MenuTypes.ReportNames.MESSAGE) {
    if (name.name !== tmp(8765).ReportNames.FIRST_DM) {
      if (name.name !== tmp(8765).ReportNames.GUILD) {
        if (name.name !== tmp(8765).ReportNames.GUILD_DISCOVERY) {
          if (name.name === tmp(8765).ReportNames.GUILD_DIRECTORY_ENTRY) {
            const obj3 = {};
            ...
            return obj3;
          } else if (name.name === tmp(8765).ReportNames.STAGE_CHANNEL) {
            const obj6 = {};
            ...
            return obj6;
          } else if (name.name === tmp(8765).ReportNames.GUILD_SCHEDULED_EVENT) {
            const obj7 = {};
            ...
            if (name.name === tmp(8765).ReportNames.USER) {
              const obj8 = {};
              ...
              let tmp3 = obj8;
            } else if (name.name === tmp(8765).UnauthenticatedReportNames.USER) {
              const obj9 = {};
              ...
              tmp3 = obj9;
            } else if (name.name === tmp(8765).UnauthenticatedReportNames.MESSAGE) {
              const obj10 = {};
              ...
              tmp3 = obj10;
            } else if (name.name === tmp(8765).UnauthenticatedReportNames.GUILD) {
              const obj11 = {};
              ...
              tmp3 = obj11;
            } else if (name.name === tmp(8765).ReportNames.APPLICATION) {
              const obj12 = {};
              ...
              tmp3 = obj12;
            } else if (name.name === tmp(8765).ReportNames.WIDGET) {
              const obj13 = {};
              ...
              tmp3 = null;
              if (name.name === tmp(8765).UnauthenticatedReportNames.MEDIA_TAKEDOWN) {
                const obj14 = {};
...
export const submitReport = function submitReport(language, name, arr) {
  if (DevSettingsStore.get("iar_skip_api_report_submit")) {
    ...
    const REPORT_TO_MOD = obj4(8767).ReportMenuTypeSets.REPORT_TO_MOD;
    if (REPORT_TO_MOD.has(name.name)) {
      ...
      let tmp15 = null;
      if (name.name === tmp4(8765).ModeratorReportNames.MESSAGE) {
        let obj2 = { channel_id: "Array", message_id: "accessible", guild_id: "diversity" };
        obj4 = {};
      ...
      const REPORT_TO_MOD2 = tmp4(8767).ReportMenuTypeSets.REPORT_TO_MOD;
      ...
      name = name.name;
      let _Object = Object;
      const values = Object.values(tmp4(8765).ReportNames);
...
export const trackCloseReportModalAnalytics = function trackCloseReportModalAnalytics(name, c12, onSubmit) {
  ...
  if (name.name !== MenuTypes.ReportNames.GUILD) {
    if (name.name !== tmp2(8765).ReportNames.GUILD_DISCOVERY) {
      if (name.name === tmp2(8765).ReportNames.GUILD_DIRECTORY_ENTRY) {
        let id2 = name.record.guildId;
      } else if (name.name === tmp2(8765).ReportNames.GUILD_SCHEDULED_EVENT) {
        id2 = name.record.guild_id;
      }
    }
    obj2.guild_id = id2;
    if (name.name === tmp2(8765).ReportNames.GUILD_SCHEDULED_EVENT) {
      let channelId = name.record.channel_id;
    } else if (name.name === tmp2(8765).ReportNames.GUILD_DIRECTORY_ENTRY) {
      channelId = name.record.channelId;
    }
    obj2.channel_id = channelId;
    let id4;
    if (name.name === tmp2(8765).ReportNames.APPLICATION) {
      id4 = name.record.id;
    }
