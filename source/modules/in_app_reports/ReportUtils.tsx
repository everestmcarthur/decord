...
// Dependencies: [5, 19, 4606, 1074, 8487, 1272, 8725, 8492, 573, 38, 8723, 4787, 2]
...
// Module 8726 (in_app_reports/ReportUtils)
...
import ReportMenuType from "ReportMenuType" /* 8725 */;
...
function genSubmitData(version, name, arr, email_token) {
  ...
  let obj2 = { channel_id: "Array", message_id: "call", stage_instance_id: "forEach", guild_id: "", guild_scheduled_event_id: "disabled", user_id: "HTTP", email_token: "isArray", application_id: "isArray", entrypoint: "isArray", widget_id: "Icon" };
  obj.language = str;
  ...
  if (name.name !== MenuTypes.ReportNames.MESSAGE) {
    if (name.name !== tmp(8723).ReportNames.FIRST_DM) {
      if (name.name !== tmp(8723).ReportNames.GUILD) {
        if (name.name !== tmp(8723).ReportNames.GUILD_DISCOVERY) {
          if (name.name === tmp(8723).ReportNames.GUILD_DIRECTORY_ENTRY) {
            const obj3 = {};
            ...
            return obj3;
          } else if (name.name === tmp(8723).ReportNames.STAGE_CHANNEL) {
            const obj6 = {};
            ...
            return obj6;
          } else if (name.name === tmp(8723).ReportNames.GUILD_SCHEDULED_EVENT) {
            const obj7 = {};
            ...
            if (name.name === tmp(8723).ReportNames.USER) {
              const obj8 = {};
              ...
              let tmp3 = obj8;
            } else if (name.name === tmp(8723).UnauthenticatedReportNames.USER) {
              const obj9 = {};
              ...
              tmp3 = obj9;
            } else if (name.name === tmp(8723).UnauthenticatedReportNames.MESSAGE) {
              const obj10 = {};
              ...
              tmp3 = obj10;
            } else if (name.name === tmp(8723).UnauthenticatedReportNames.GUILD) {
              const obj11 = {};
              ...
              tmp3 = obj11;
            } else if (name.name === tmp(8723).ReportNames.APPLICATION) {
              const obj12 = {};
              ...
              tmp3 = obj12;
            } else if (name.name === tmp(8723).ReportNames.WIDGET) {
              const obj13 = {};
              ...
              tmp3 = null;
              if (name.name === tmp(8723).UnauthenticatedReportNames.MEDIA_TAKEDOWN) {
                const obj14 = {};
...
export const submitReport = function submitReport(language, name, arr) {
  if (DevSettingsStore.get("iar_skip_api_report_submit")) {
    ...
    const REPORT_TO_MOD = obj4(8725).ReportMenuTypeSets.REPORT_TO_MOD;
    if (REPORT_TO_MOD.has(name.name)) {
      ...
      let tmp15 = null;
      if (name.name === tmp4(8723).ModeratorReportNames.MESSAGE) {
        let obj2 = { channel_id: "isArray", message_id: "accessible", guild_id: "min" };
        obj4 = {};
      ...
      const REPORT_TO_MOD2 = tmp4(8725).ReportMenuTypeSets.REPORT_TO_MOD;
      ...
      name = name.name;
      let _Object = Object;
      const values = Object.values(tmp4(8723).ReportNames);
...
export const trackCloseReportModalAnalytics = function trackCloseReportModalAnalytics(name, c12, onSubmit) {
  ...
  if (name.name !== MenuTypes.ReportNames.GUILD) {
    if (name.name !== tmp2(8723).ReportNames.GUILD_DISCOVERY) {
      if (name.name === tmp2(8723).ReportNames.GUILD_DIRECTORY_ENTRY) {
        let id2 = name.record.guildId;
      } else if (name.name === tmp2(8723).ReportNames.GUILD_SCHEDULED_EVENT) {
        id2 = name.record.guild_id;
      }
    }
    obj2.guild_id = id2;
    if (name.name === tmp2(8723).ReportNames.GUILD_SCHEDULED_EVENT) {
      let channelId = name.record.channel_id;
    } else if (name.name === tmp2(8723).ReportNames.GUILD_DIRECTORY_ENTRY) {
      channelId = name.record.channelId;
    }
    obj2.channel_id = channelId;
    let id4;
    if (name.name === tmp2(8723).ReportNames.APPLICATION) {
      id4 = name.record.id;
    }
