...
// Dependencies: [19, 7191, 4788, 1074, 573, 504, 7187, 11653, 11650, 2]
// Exports: default

// Module 11652 (useBatchUpdateChannelSettings)
...
export default function useBatchUpdateChannelSettings(guildId) {
  ...
  return {
    onChannelClick: noop.useCallback((guildId, channelId, channel) => {
      ...
      if (isCollapsedResult) {
        guildId(11653).categoryExpand(channel);
        const obj = guildId(11653);
      }
      if (obj2.hasNotSetUpChannelOptIn(guildId)) {
        if (channelId === channel) {
          ...
          obj3.include = set;
          const result = tmp8(11650).optIntoAllChannelsForExistingMember(guildId, obj3);
          const tmp8Result = tmp8(11650);
        } else {
          ...
          obj4.exclude = set1;
          const result1 = tmp8(11650).optIntoAllChannelsForExistingMember(guildId, obj4);
          const tmp8Result3 = tmp8(11650);
        }
      } else {
        const tmp8Result4 = tmp8(7187);
