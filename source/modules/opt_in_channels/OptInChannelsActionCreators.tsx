...
// Dependencies: [5, 2014, 1957, 4788, 1074, 1084, 5604, 1384, 7188, 7190, 573, 1242, 4787, 12, 7193, 1940, 1187, 2]
...
// Module 7187 (OptInChannelsActionCreators)
...
import ImpersonateActionCreators from "ImpersonateActionCreators" /* 5604 */;
import NotificationSettingsUtils from "NotificationSettingsUtils" /* 7188 */;
import UserGuildSettingsManagerDefault from "UserGuildSettingsManager" /* 7190 */;
  ...
  if (null != guild_id) {
    if (ImpersonateStore.isFullServerPreview(guild_id)) {
      ...
      const result3 = UserSettingsProtoActionCreators.updateUserGuildSettings(guild_id, (guildOnboardingProgress) => {
        ...
        if (hasFlagResult) {
          hasFlagResult = !tmp(1384).hasFlag(guildOnboardingProgress.guildOnboardingProgress, tmp(1187).GuildOnboardingProgress.GUILD_NOTICE_SHOWN);
          const tmpResult = tmp(1384);
        }
        let flag = !hasFlagResult;
        if (!hasFlagResult) {
          guildOnboardingProgress.guildOnboardingProgress = tmp(1384).addFlag(guildOnboardingProgress.guildOnboardingProgress, tmp(1187).GuildOnboardingProgress.GUILD_NOTICE_CLEARED);
          const tmpResult3 = tmp(1384);
          guildOnboardingProgress.guildOnboardingProgress = tmp(1384).setFlag(guildOnboardingProgress.guildOnboardingProgress, tmp(1187).GuildOnboardingProgress.GUILD_NOTICE_SHOWN, false);
          flag = true;
  ...
  if (null != guildId) {
    if (ImpersonateStore.isFullServerPreview(guildId)) {
      ...
      if (!UserGuildSettingsStore.isOptInEnabled(guildId)) {
        const tmp6Result = tmp6(1384);
        const setFlagResult1 = tmp6(1384).setFlag(obj.getGuildFlags(guildId), constants2.OPT_IN_CHANNELS_ON, true);
        const obj6 = {};
      ...
      const NotificationLabel = tmp6(7188).NotificationLabel;
      obj12.label = NotificationLabel.optedIn(optInEnabled);
      obj12.location = location;
      ...
      const result4 = UserSettingsProtoActionCreators.updateUserGuildSettings(guildId, (guildOnboardingProgress) => {
        ...
        return flag;
      }, tmp6(1940).UserSettingsDelay.INFREQUENT_USER_ACTION);
  ...
  if (null != id) {
    if (ImpersonateStore.isFullServerPreview(id)) {
      ...
      if (flag) {
        ...
        const result2 = channel_overrides(7193).updateGuildAndChannelNotificationSettings(id, obj4, require("NotificationSettingsUtils").NotificationLabels.OptedIn);
        const obj7 = channel_overrides(7193);
        const obj8 = {};
        ...
        const result3 = channel_overrides(7193).updateChannelOverrideSettingsBulk(id, channel_overrides, require("NotificationSettingsUtils").NotificationLabels.OptedIn);
        const obj2 = channel_overrides(7193);
        const obj10 = {};
...
export const bulkOptOutChannels = function bulkOptOutChannels(id, arr, location) {
  ...
  if (null != id) {
    if (ImpersonateStore.isFullServerPreview(id)) {
      ...
      const result1 = obj(7193).updateChannelOverrideSettingsBulk(id, obj, require("NotificationSettingsUtils").NotificationLabels.OptedOut);
      const obj2 = obj(7193);
      const obj4 = {};
      const obj3 = obj(1242);
      const merged = Object.assign(require("AppAnalyticsUtils").collectGuildAnalyticsMetadata(id));
...
export const setIsFavorite = function setIsFavorite(id, id, arg2, location) {
  if (null != id) {
    if (!ImpersonateStore.isFullServerPreview(id)) {
      ...
      let setFlagResult = channelIdFlags;
      if (tmp9) {
        setFlagResult = tmp5(1384).setFlag(channelIdFlags, tmp7.OPT_IN_ENABLED, true);
        const tmp5Result = tmp5(1384);
      }
      ...
      const NotificationLabel = tmp5(7188).NotificationLabel;
...
export const enableGuildNotice = function enableGuildNotice(guildId) {
  const result = UserSettingsProtoActionCreators.updateUserGuildSettings(guildId, (guildOnboardingProgress) => {
    ...
    let flag = !hasFlagResult;
    if (!hasFlagResult) {
      guildOnboardingProgress.guildOnboardingProgress = tmp(1384).addFlag(guildOnboardingProgress.guildOnboardingProgress, tmp(1187).GuildOnboardingProgress.GUILD_NOTICE_SHOWN);
      flag = true;
      const tmpResult = tmp(1384);
    }
    return flag;
...
export const clearGuildNotice = function clearGuildNotice(guildId) {
  ...
  if (null != guildId) {
    if (ImpersonateStore.isFullServerPreview(guildId)) {
      ...
      const result1 = obj(7193).updateChannelOverrideSettingsBulk(guildId, obj, require("NotificationSettingsUtils").NotificationLabels.OptedOut);
      const obj2 = obj(7193);
      const obj4 = {};
      const obj3 = obj(1242);
  ...
  const result2 = require("UserSettingsProtoActionCreators").updateUserGuildSettings(guildId, (guildOnboardingProgress) => {
    guildOnboardingProgress.guildOnboardingProgress = guildId(1384).setFlag(guildOnboardingProgress.guildOnboardingProgress, guildId(1187).GuildOnboardingProgress.GUILD_NOTICE_SHOWN, false);
    obj = guildId(1384);
    guildOnboardingProgress.guildOnboardingProgress = guildId(1384).setFlag(guildOnboardingProgress.guildOnboardingProgress, guildId(1187).GuildOnboardingProgress.GUILD_NOTICE_CLEARED, false);
    return true;
