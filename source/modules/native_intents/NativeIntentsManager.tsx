...
// Dependencies: [32, 1958, 1980, 4278, 4288, 2012, 1372, 1074, 18133, 18134, 4793, 13185, 1397, 1370, 4484, 7232, 2]

// Module 18132 (NativeIntentsManager)
import AvatarUtilsDefault from "AvatarUtils" /* 1397 */;
import useChannelName from "useChannelName" /* 4793 */;
import getChannelIcon from "getChannelIcon" /* 13185 */;
import NativeIntentsExperimentDefault from "NativeIntentsExperiment" /* 18133 */;
import IntentsBindingsDefault from "IntentsBindings" /* 18134 */;
  ...
  if (obj.getCurrentConfig({ location: "NativeIntentsManager" }, obj2).activityEnabled) {
    ...
    if (null != channel) {
      ...
      if (channel.type === constants.DM) {
        ...
        if (null != first) {
          const globalName = tmp(4484).getGlobalName(first);
          if (null != globalName) {
          ...
          const name = tmp(4484).getName(first);
          if (null != name) {
            items1.push(name);
          }
      ...
      tmp(18134).setActivity(obj4);
      obj6 = RelationshipStore;
      tmp10 = require;
      tmp11 = UserStore;
      const tmpResult5 = tmp(18134);
    } else {
      tmp(18134).resignActivity();
      const tmpResult6 = tmp(18134);
    }
  }
}
...
prototype["handleInit"] = function handleInit() {
  ...
  if (obj.getCurrentConfig({ location: "NativeIntentsManager" }, obj2).clearEnabled) {
    tmp2(18134).clearSearchIndex();
    const tmp2Result = tmp2(18134);
  }
  ...
  if (tmp2Result5.getCurrentConfig({ location: "NativeIntentsManager" }, obj4).searchEnabled) {
    ...
    const tmp2Result7 = tmp2(18133);
    obj5.disable = !tmp2(18134).hasSearch();
    ...
    if (PermissionStore.can(constants3.VIEW_CHANNEL, channel)) {
      ...
      if (null != guild) {
        if (null == guild) {
          ...
          const items1 = [obj4];
          tmp(18134).indexDomains(items1);
          const tmpResult = tmp(18134);
        } else {
...
prototype["handleGuildCreateOrUpdate"] = function handleGuildCreateOrUpdate(guild) {
  ...
  if (obj.getCurrentConfig({ location: "NativeIntentsManager" }, obj2).searchEnabled) {
    ...
    if (null != guild1) {
      ...
      tmp(18134).indexDomains(items);
      const tmpResult = tmp(18134);
    } else {
      const items1 = [guild.id];
      tmp(18134).deleteSearchDomains(items1);
      const tmpResult2 = tmp(18134);
    }
  }
};
