...
// Function ID: 8220
// Name: useHeroColors
// Dependencies: [32, 19, 4599, 1183, 8218, 7626, 8220, 1091, 672, 504, 2]
// Exports: default, getHeroColors

// Module 8219 (useHeroColors)
...
export default function useHeroColors(pendingAvatarSrc) {
  ...
  let obj2 = first(504);
  const fallbackHeroColor = first(8220).getFallbackHeroColor(stateFromStores1, stateFromStores);
  let obj3 = first(8220);
  let tmp4 = _slicedToArray(first(8218).useAvatarColors(pendingAvatarSrc, fallbackHeroColor), 2);
  first = tmp4[0];
  closure_1 = tmp6;
...
export const getHeroColors = function getHeroColors(game_name) {
  const fallbackHeroColor = num(8220).getFallbackHeroColor(ThemeStore.theme, AccessibilityStore.saturation);
  num = 1;
  if (AccessibilityStore.desaturateUserColors) {
    num = tmp.saturation;
  }
  const useColorStore = tmp2(8218).useColorStore;
