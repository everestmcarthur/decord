...
// Dependencies: [19, 4833, 1074, 1920, 8406, 8202, 1114, 13070, 7361, 10962, 1396, 13072, 5333, 13073, 7220, 504, 1242, 8409, 2]
// Exports: getApplicationImage, useImageForActivity, useImageForContentEntry

// Module 13069 (ContentInventoryActivityImageUtils)
...
import isCrunchyrollActivityDefault from "isCrunchyrollActivity" /* 8409 */;
import StageChannelRichPresenceUtils from "StageChannelRichPresenceUtils" /* 10962 */;
import useEntryActivityAndApplicationDefault from "useEntryActivityAndApplication" /* 13070 */;
...
function useComputedImagesForActivity(activity, activityApplication) {
  ...
  if (null != largeImage) {
    ...
    if (obj17.isStageActivity(activity)) {
      const result = tmp15(10962).unpackStageChannelParty(activity);
      let guildIconURL;
      if (null != result) {
      ...
      obj11 = obj5;
      const tmp15Result = tmp15(10962);
    } else if (isOnXboxDefault(activity)) {
      const obj6 = { largeImage: null, smallImage: "Array" };
      const obj7 = { src: tmp2(5333).get(constants2.XBOX).icon.customPNG, alt: null };
      const intl4 = tmp15(1114).intl;
      obj7.alt = intl4.string(tmp15(1114).t.Nfvo72);
      obj6.largeImage = obj7;
      obj11 = obj6;
      const tmp2Result = tmp2(5333);
    } else {
      if (null == smallImage) {
        if (tmp2(13073)(activity)) {
          const obj8 = { largeImage: null, smallImage: "Array" };
          const obj9 = { src: tmp2(5333).get(constants2.PLAYSTATION).icon.lightPNG, alt: null };
          const intl3 = tmp15(1114).intl;
          obj9.alt = intl3.string(tmp15(1114).t.fFl4jo);
          obj8.largeImage = obj9;
          obj11 = obj8;
          const tmp2Result2 = tmp2(5333);
        }
      }
      let iconURL;
      ...
      if (null == iconURL) {
        ...
        if (null == name) {
          ...
          stringResult = intl.formatToPlainString(tmp15(1114).t.tiKyYg, obj13);
        }
...
function useRichImageForActivity(activity, activityApplication) {
  ...
  if (null == activity) {
    ...
    if (null != large_image) {
      ...
      obj.src = tmp(8202).getAssetImage(activity.application_id, large_image, items);
      const assets2 = activity.assets;
      let trimmed;
      if (assets2 != null) {
    ...
    if (null != tmp11) {
      ...
      obj2.src = tmp(8202).getAssetImage(activity.application_id, tmp11, items1);
    ...
    if (tmp6 == null) {
      ...
      if (null != iconURL) {
        ...
        if (null == name) {
          ...
          stringResult = intl.formatToPlainString(tmp(1114).t.tiKyYg, obj4);
        }
...
export const useImageForContentEntry = function useImageForContentEntry(trackingSource) {
  ...
  if (tmp5Result4.isWatchedMediaEntry(entry)) {
    const obj10 = { src: tmp5(8202).getAssetImage(entry.extra.application_id, entry.extra.media_assets_large_image, ImageSizes.LARGE), alt: entry.extra.media_title };
    tmp7 = obj10;
    const tmp5Result5 = tmp5(8202);
  } else {
    if (tmp5Result6.isTopArtistEntry(entry)) {
      const obj11 = { src: entry.extra.media.image_url };
      tmp7 = obj11;
    }
    tmp5Result6 = tmp5(8406);
  }
};
