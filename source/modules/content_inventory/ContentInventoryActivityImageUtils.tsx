...
// Dependencies: [19, 4835, 1074, 1920, 8429, 8225, 1114, 13095, 7383, 10983, 1396, 13097, 5335, 13098, 7242, 504, 1242, 8432, 2]
// Exports: getApplicationImage, useImageForActivity, useImageForContentEntry

// Module 13094 (ContentInventoryActivityImageUtils)
...
import ContentInventoryTypes from "ContentInventoryTypes" /* 8429 */;
import isCrunchyrollActivityDefault from "isCrunchyrollActivity" /* 8432 */;
import StageChannelRichPresenceUtils from "StageChannelRichPresenceUtils" /* 10983 */;
import useEntryActivityAndApplicationDefault from "useEntryActivityAndApplication" /* 13095 */;
...
function useComputedImagesForActivity(activity, activityApplication) {
  ...
  if (null != largeImage) {
    ...
    if (obj17.isStageActivity(activity)) {
      const result = tmp15(10983).unpackStageChannelParty(activity);
      let guildIconURL;
      if (null != result) {
      ...
      obj11 = obj5;
      const tmp15Result = tmp15(10983);
    } else if (isOnXboxDefault(activity)) {
      const obj6 = { largeImage: null, smallImage: "a" };
      const obj7 = { src: tmp2(5335).get(constants2.XBOX).icon.customPNG, alt: null };
      ...
      if (null == smallImage) {
        if (tmp2(13098)(activity)) {
          const obj8 = { largeImage: null, smallImage: "a" };
          const obj9 = { src: tmp2(5335).get(constants2.PLAYSTATION).icon.lightPNG, alt: null };
...
function useRichImageForActivity(activity, activityApplication) {
  ...
  if (null == activity) {
    return { largeImage: "guild", smallImage: "call" };
  } else {
    let large_image;
    if (activity != null) {
      const assets = activity.assets;
    ...
    if (null != large_image) {
      ...
      obj.src = tmp(8225).getAssetImage(activity.application_id, large_image, items);
      const assets2 = activity.assets;
      let trimmed;
      if (assets2 != null) {
    ...
    if (null != tmp11) {
      ...
      obj2.src = tmp(8225).getAssetImage(activity.application_id, tmp11, items1);
...
export const useImageForContentEntry = function useImageForContentEntry(trackingSource) {
  ...
  if (tmp5Result4.isWatchedMediaEntry(entry)) {
    const obj10 = { src: tmp5(8225).getAssetImage(entry.extra.application_id, entry.extra.media_assets_large_image, ImageSizes.LARGE), alt: entry.extra.media_title };
    tmp7 = obj10;
    const tmp5Result5 = tmp5(8225);
  } else {
    if (tmp5Result6.isTopArtistEntry(entry)) {
      const obj11 = { src: entry.extra.media.image_url };
      tmp7 = obj11;
    }
    tmp5Result6 = tmp5(8429);
  }
};
