...
// Dependencies: [19, 4868, 1074, 1921, 8490, 8287, 1115, 13155, 7423, 11046, 1397, 13157, 5371, 13158, 7282, 504, 1241, 8493, 2]
// Exports: getApplicationImage, useImageForActivity, useImageForContentEntry

// Module 13154 (ContentInventoryActivityImageUtils)
...
import isCrunchyrollActivityDefault from "isCrunchyrollActivity" /* 8493 */;
import StageChannelRichPresenceUtils from "StageChannelRichPresenceUtils" /* 11046 */;
import useEntryActivityAndApplicationDefault from "useEntryActivityAndApplication" /* 13155 */;
...
function useComputedImagesForActivity(activity, activityApplication) {
  ...
  if (null != largeImage) {
    ...
    if (obj17.isStageActivity(activity)) {
      const result = tmp15(11046).unpackStageChannelParty(activity);
      let guildIconURL;
      if (null != result) {
...
function useRichImageForActivity(activity, activityApplication) {
  ...
  if (null == activity) {
    ...
    if (null != large_image) {
      ...
      obj.src = tmp(8287).getAssetImage(activity.application_id, large_image, items);
      const assets2 = activity.assets;
      let trimmed;
      if (assets2 != null) {
    ...
    if (null != tmp11) {
      ...
      obj2.src = tmp(8287).getAssetImage(activity.application_id, tmp11, items1);
...
export const useImageForContentEntry = function useImageForContentEntry(trackingSource) {
  ...
  if (tmp5Result4.isWatchedMediaEntry(entry)) {
    const obj10 = { src: tmp5(8287).getAssetImage(entry.extra.application_id, entry.extra.media_assets_large_image, ImageSizes.LARGE), alt: entry.extra.media_title };
    tmp7 = obj10;
    const tmp5Result5 = tmp5(8287);
  } else {
    if (tmp5Result6.isTopArtistEntry(entry)) {
      const obj11 = { src: entry.extra.media.image_url };
      tmp7 = obj11;
    }
    tmp5Result6 = tmp5(8490);
  }
};
