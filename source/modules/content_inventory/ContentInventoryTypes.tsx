...
// Module 8429 (ContentInventoryTypes)
...
export const isXboxEntry = function isXboxEntry(content_type) {
  ...
  if (!tmp4) {
    ...
    tmp4 = content_type1 === tmp2(8216).ContentInventoryEntryType.TOP_GAME;
  }
  let tmp6 = tmp4;
  if (tmp6) {
    tmp6 = content_type.extra.platform === tmp2(8430).PlatformType.XBOX;
  }
  return tmp6;
};
export const isPlaystationEntry = function isPlaystationEntry(content_type) {
  ...
  let tmp6 = tmp4;
  if (tmp6) {
    tmp6 = content_type.extra.platform === tmp2(8430).PlatformType.PLAYSTATION;
  }
  return tmp6;
};
export const isConsoleEntry = function isConsoleEntry(content_type) {
  ...
  if (!tmp6) {
    ...
    let tmp8 = content_type2 === tmp2(8216).ContentInventoryEntryType.PLAYED_GAME;
    if (!tmp8) {
      let content_type3;
      if (content_type != null) {
        content_type3 = content_type.content_type;
      }
      tmp8 = content_type3 === tmp2(8216).ContentInventoryEntryType.TOP_GAME;
    }
    let tmp10 = tmp8;
    if (tmp10) {
      tmp10 = content_type.extra.platform === tmp2(8430).PlatformType.PLAYSTATION;
    }
    tmp6 = tmp10;
  }
  return tmp6;
...
export const isRecentActivityEntry = function isRecentActivityEntry(length) {
  ...
  if (!tmp4) {
    ...
    tmp4 = content_type2 === tmp2(8216).ContentInventoryEntryType.LISTENED_SESSION;
  }
  if (!tmp4) {
    let content_type3;
    if (length != null) {
    ...
    tmp4 = content_type3 === tmp2(8216).ContentInventoryEntryType.WATCHED_MEDIA;
  }
  if (!tmp4) {
    let extra;
    if (length != null) {
