...
// Dependencies: [19, 17, 1372, 1074, 1955, 21, 4360, 4491, 4574, 7093, 504, 4297, 1115, 2787, 7496, 7152, 4460, 1944, 7264, 7237, 7268, 5780, 1364, 5675, 17186, 8965, 4636, 5062, 4640, 576, 2]
// Exports: default

// Module 17185 (DisplayNameStylesFlywheelMobileActionSheet)
...
export default function DisplayNameStylesFlywheelMobileActionSheet(markAsDismissed) {
  ...
  const enabled = noop.useContext(markAsDismissed(4360).AccessibilityPreferencesContext).reducedMotion.enabled;
  const isThemeDarkResult = markAsDismissed(4491).isThemeDark(ref(4574)());
  const tmp6 = closure_11();
  const obj2 = markAsDismissed(4491);
  const typeConsolidationTextTransform = markAsDismissed(7093).useTypeConsolidationTextTransform("DisplayNameStylesFlywheel");
  const obj3 = markAsDismissed(7093);
  ...
  let result = ref(4297).canUsePremiumProfileCustomization(stateFromStores);
  ...
  const callback = obj.useCallback(() => {
    openUserSettings.openUserSettings({ screen: c2 ? timestampProducer.PROFILE_CUSTOMIZATION : timestampProducer.PROFILE_CUSTOMIZATION_TRY_IT_OUT }, () => {
      markAsDismissed(7152).runAfterInteractions(() => {
        markAsDismissed(7496).openUserSettings({ screen: constants.DISPLAY_NAME_STYLES }, () => {
          closure_1_0(constants.TAKE_ACTION);
          const result = closure_2_0(4460).UNSAFE_markDismissibleContentAsDismissed(closure_2_0(1944).DismissibleContent.DISPLAY_NAME_STYLES_FLYWHEEL_MOBILE_PROFILE_COACHMARK, { dismissAction: constants.INDIRECT_ACTION });
        });
      });
    });
  }, items1);
  ...
  const items4 = [
    closure_9(markAsDismissed(7268).ActionSheetHeaderBar, {
      onPress() {
  ...
  if (enabled) {
    tmp15Result = tmp15(tmp2(5780).DisplayNameStylesV2AbstractUI, { resizeMode: "contain" });
  }
  ...
  if (enabled) {
    ...
    items4[2] = tmp15(tmp2(4636).Text, obj10);
    ...
    items4[3] = tmp15(tmp2(4636).Text, obj11);
    ...
    const items7 = [tmp15(tmp2(5062).Button, obj14), ];
    ...
    items7[1] = tmp15(tmp2(5062).Button, obj15);
    obj13.children = items7;
    items4[4] = tmp16(tmp17, obj13);
    obj7.children = items4;
    obj12.children = tmp16(tmp17, obj7);
    obj6.children = tmp15(tmp2(7237).SafeAreaPaddingView, obj12);
    return tmp15(tmp2(7264).BottomSheet, obj6);
  } else {
    if (tmp2Result.isIOS()) {
      const obj16 = { source: null, style: null, resizeMode: "contain", enableAnimation: null };
      const obj17 = { uri: tmp4(17186) };
      obj16.source = obj17;
      obj16.style = tmp6.image;
      obj16.enableAnimation = !enabled;
      let tmp15Result2 = tmp15(tmp4(5675), obj16);
      const tmp4Result = tmp4(5675);
    } else {
      const obj18 = { url: tmp4(17186), style: tmp6.image, autoplay: true };
      tmp15Result2 = tmp15(tmp2(8965).APNGPlayer, obj18);
    }
