...
// Dependencies: [19, 17, 1372, 1074, 1955, 21, 4358, 4489, 4572, 7089, 504, 4295, 1115, 2787, 7492, 7148, 4458, 1944, 7260, 7233, 7264, 5778, 1364, 5673, 17146, 8949, 4634, 5060, 4638, 576, 2]
// Exports: default

// Module 17145 (DisplayNameStylesFlywheelMobileActionSheet)
...
export default function DisplayNameStylesFlywheelMobileActionSheet(markAsDismissed) {
  ...
  const enabled = noop.useContext(markAsDismissed(4358).AccessibilityPreferencesContext).reducedMotion.enabled;
  const isThemeDarkResult = markAsDismissed(4489).isThemeDark(ref(4572)());
  const tmp6 = closure_11();
  const obj2 = markAsDismissed(4489);
  const typeConsolidationTextTransform = markAsDismissed(7089).useTypeConsolidationTextTransform("DisplayNameStylesFlywheel");
  const obj3 = markAsDismissed(7089);
  ...
  let result = ref(4295).canUsePremiumProfileCustomization(stateFromStores);
  dependencyMap = result;
  const intl = markAsDismissed(1115).intl;
  const string = intl.string;
  const tmp10 = ref(2787);
  if (result) {
  ...
  const callback = obj.useCallback(() => {
    openUserSettings.openUserSettings({ screen: c2 ? timestampProducer.PROFILE_CUSTOMIZATION : timestampProducer.PROFILE_CUSTOMIZATION_TRY_IT_OUT }, () => {
      markAsDismissed(7148).runAfterInteractions(() => {
        markAsDismissed(7492).openUserSettings({ screen: constants.DISPLAY_NAME_STYLES }, () => {
          closure_1_0(constants.TAKE_ACTION);
          const result = closure_2_0(4458).UNSAFE_markDismissibleContentAsDismissed(closure_2_0(1944).DismissibleContent.DISPLAY_NAME_STYLES_FLYWHEEL_MOBILE_PROFILE_COACHMARK, { dismissAction: constants.INDIRECT_ACTION });
        });
      });
    });
  }, items1);
  ...
  const items4 = [
    closure_9(markAsDismissed(7264).ActionSheetHeaderBar, {
      onPress() {
  ...
  if (enabled) {
    tmp15Result = tmp15(tmp2(5778).DisplayNameStylesV2AbstractUI, { resizeMode: "contain" });
  }
  ...
  if (enabled) {
    ...
    obj10.children = intl2.string(tmp4(2787).Uzms61);
    items4[2] = tmp15(tmp2(4634).Text, obj10);
    ...
    items4[3] = tmp15(tmp2(4634).Text, obj11);
    ...
    const items7 = [tmp15(tmp2(5060).Button, obj14), ];
    ...
    items7[1] = tmp15(tmp2(5060).Button, obj15);
    obj13.children = items7;
    items4[4] = tmp16(tmp17, obj13);
    obj7.children = items4;
    obj12.children = tmp16(tmp17, obj7);
    obj6.children = tmp15(tmp2(7233).SafeAreaPaddingView, obj12);
    return tmp15(tmp2(7260).BottomSheet, obj6);
  } else {
    if (tmp2Result.isIOS()) {
      const obj16 = { source: null, style: null, resizeMode: "contain", enableAnimation: null };
      const obj17 = { uri: tmp4(17146) };
      obj16.source = obj17;
      obj16.style = tmp6.image;
      obj16.enableAnimation = !enabled;
      let tmp15Result2 = tmp15(tmp4(5673), obj16);
      const tmp4Result = tmp4(5673);
    } else {
      const obj18 = { url: tmp4(17146), style: tmp6.image, autoplay: true };
      tmp15Result2 = tmp15(tmp2(8949).APNGPlayer, obj18);
    }
