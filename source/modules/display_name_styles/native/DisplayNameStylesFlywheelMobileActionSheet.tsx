...
// Dependencies: [19, 17, 1372, 1074, 1955, 21, 4360, 4491, 4574, 7086, 504, 4297, 1115, 2787, 7489, 7145, 4460, 1944, 7257, 7230, 7261, 5777, 1364, 5672, 17090, 8938, 4635, 5059, 4639, 576, 2]
// Exports: default

// Module 17089 (DisplayNameStylesFlywheelMobileActionSheet)
...
export default function DisplayNameStylesFlywheelMobileActionSheet(markAsDismissed) {
  ...
  const typeConsolidationTextTransform = markAsDismissed(7086).useTypeConsolidationTextTransform("DisplayNameStylesFlywheel");
  const obj3 = markAsDismissed(7086);
  ...
  const callback = obj.useCallback(() => {
    openUserSettings.openUserSettings({ screen: c2 ? timestampProducer.PROFILE_CUSTOMIZATION : timestampProducer.PROFILE_CUSTOMIZATION_TRY_IT_OUT }, () => {
      markAsDismissed(7145).runAfterInteractions(() => {
        markAsDismissed(7489).openUserSettings({ screen: constants.DISPLAY_NAME_STYLES }, () => {
  ...
  const items4 = [
    closure_9(markAsDismissed(7261).ActionSheetHeaderBar, {
      onPress() {
  ...
  if (enabled) {
    tmp15Result = tmp15(tmp2(5777).DisplayNameStylesV2AbstractUI, { resizeMode: "contain" });
  }
  ...
  if (enabled) {
    ...
    items4[2] = tmp15(tmp2(4635).Text, obj10);
    ...
    items4[3] = tmp15(tmp2(4635).Text, obj11);
    ...
    const items7 = [tmp15(tmp2(5059).Button, obj14), ];
    ...
    items7[1] = tmp15(tmp2(5059).Button, obj15);
    obj13.children = items7;
    items4[4] = tmp16(tmp17, obj13);
    obj7.children = items4;
    obj12.children = tmp16(tmp17, obj7);
    obj6.children = tmp15(tmp2(7230).SafeAreaPaddingView, obj12);
    return tmp15(tmp2(7257).BottomSheet, obj6);
  } else {
    if (tmp2Result.isIOS()) {
      const obj16 = { source: null, style: null, resizeMode: "contain", enableAnimation: null };
      const obj17 = { uri: tmp4(17090) };
      obj16.source = obj17;
      obj16.style = tmp6.image;
      obj16.enableAnimation = !enabled;
      let tmp15Result2 = tmp15(tmp4(5672), obj16);
      const tmp4Result = tmp4(5672);
    } else {
      const obj18 = { url: tmp4(17090), style: tmp6.image, autoplay: true };
      tmp15Result2 = tmp15(tmp2(8938).APNGPlayer, obj18);
    }
