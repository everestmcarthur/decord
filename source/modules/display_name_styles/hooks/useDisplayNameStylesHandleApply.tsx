...
// Dependencies: [19, 1074, 1391, 8315, 8312, 1241, 1392, 2]
// Exports: useDisplayNameStylesHandleApply

// Module 15453 (useDisplayNameStylesHandleApply)
...
export const useDisplayNameStylesHandleApply = function useDisplayNameStylesHandleApply(hasChanges) {
  ...
  return selectedColors.useCallback(() => {
    if (hasChanges) {
      ...
      if (isTryItOut) {
        const result = tmp2(8315).setTryItOutDisplayNameStyles(obj);
        const tmp2Result = tmp2(8315);
      } else {
        const obj2 = { guildId, displayNameStyles: obj };
        tmp2(8312).setPendingChanges(obj2);
        const tmp2Result2 = tmp2(8312);
      }
