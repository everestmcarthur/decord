...
// Dependencies: [19, 17, 8485, 21, 7374, 14740, 4301, 2024, 8497, 8499, 576, 1178, 5026, 1114, 4602, 2]
// Exports: default

// Module 14872 (AgeConfirmationNotice)
import nativeDefault from "native" /* 576 */;
import HelpdeskUtilsDefault from "HelpdeskUtils" /* 2024 */;
import LinkingDefault from "Linking" /* 4301 */;
import Text_Text from "Text/Text" /* 4602 */;
import SafetySettingsUtils from "SafetySettingsUtils" /* 14740 */;
...
export default function AgeConfirmationNotice() {
  sensitiveContentFilterHelpArticle = sensitiveContentFilterHelpArticle(7374).useSensitiveContentFilterHelpArticle();
  const effect = noop.useEffect(() => {
    const result = sensitiveContentFilterHelpArticle(14740).trackSafetySettingsNoticeAnalytics(constants2.AGE_CONFIRMATION_NOTICE, constants.VIEWED);
  }, []);
  ...
  const callback = noop.useCallback(() => {
    const obj = onPress(8497);
    const result = obj.showAgeVerificationGetStartedModal({ entryPoint: sensitiveContentFilterHelpArticle(8499).AgeVerificationModalEntryPoint.CONTENT_AND_SOCIAL_NOTICE });
    const obj2 = { entryPoint: sensitiveContentFilterHelpArticle(8499).AgeVerificationModalEntryPoint.CONTENT_AND_SOCIAL_NOTICE };
    const result1 = sensitiveContentFilterHelpArticle(14740).trackSafetySettingsNoticeAnalytics(constants2.AGE_CONFIRMATION_NOTICE, constants.CONFIRM_AGE);
  }, []);
  ...
  obj4.button = jsx(sensitiveContentFilterHelpArticle(5026).Button, { variant: "secondary", size: "sm", text: null, onPress: null });
