...
// Dependencies: [32, 19, 17, 4630, 4301, 13497, 1074, 1955, 1374, 21, 672, 576, 4638, 1484, 13498, 5208, 5714, 4634, 1115, 1241, 7490, 13499, 8173, 13500, 13521, 10883, 13525, 13526, 13527, 13532, 13536, 13553, 13554, 13556, 13557, 7495, 1612, 13558, 504, 7503, 8182, 13559, 9932, 4374, 5059, 4489, 13560, 4458, 1944, 8172, 1946, 5673, 13561, 13562, 12759, 9933, 5072, 1364, 5047, 2]
// Exports: default

// Module 13496 (PremiumNitroHome)
...
import PremiumPerkCardDefault from "PremiumPerkCard" /* 13500 */;
import useScrollToSectionDefault from "useScrollToSection" /* 13525 */;
...
import PremiumNitroNavigationStore from "PremiumNitroNavigationStore" /* 13497 */;
...
function NitroHomeHeader(arg0) {
  ...
  const mobileNitroManageSubscriptionsSettingsExperiment = onClose(13498).useMobileNitroManageSubscriptionsSettingsExperiment({ location: "NitroHomeHeader" });
  let hasActiveTrial;
  ...
  const obj2 = onClose(13498);
  const tmp8 = !hasActiveTrial;
  const items1 = [closure_17(onClose(5208).PressableOpacity, { style: tmp.backButtonWrapper, onPress: callback, children: closure_17(onClose(5714).ArrowLargeLeftIcon, { size: "md", color: "white" }) }), , ];
  ...
  items1[1] = closure_17(onClose(4634).Text, obj6);
  if (mobileNitroManageSubscriptionsSettingsExperiment) {
    ...
    const obj8 = {
      ...
      children: tmp11(tmp2(7490).SettingsIcon, { size: "md", color: "white" })
    };
    tmp11Result = tmp11(tmp2(5208).PressableOpacity, obj8);
  }
  ...
  if (tmp11Result2) {
    const obj9 = { style: tmp.pillParent, children: tmp11(tmp2(13499).PremiumReferralTrialPill, { hasExtraMargin: true }) };
...
function NewTab(arg0) {
  ...
  if (tmp12) {
    ...
    const obj7 = { width: tmp2(13500).PERK_CARD_SIZES[tmp2(undefined, 13500).PerkCardVariant.WIDE].width };
    obj6.style = obj7;
    ...
    tmp12 = closure_17(tmp8(13527), obj6);
    const tmp8Result = tmp8(13527);
  }
  const items1 = [tmp12, closure_17(require("TieredTenureBadgePerkCard").TieredTenureBadgePerkCard, {}), , , , , ];
  let tmp15Result = null;
  if (isEligibleSenderForReferralProgram) {
    const obj8 = { onLayout: useScrollToSectionDefault(ref, scrollToSectionId).createSectionLayoutHandler(NitroHomeSectionId.REFERRAL_PROGRAM), children: tmp15(tmp2(13536).ReferralProgramPerkCard, {}) };
...
function PerksTab(extraBottomHeight) {
  ...
  if (fractionalState === FractionalPremiumStates.FP_ONLY) {
    ...
    items4[2] = tmp8(tmp9(13553), obj5);
    const obj6 = { title: null, perks: null, onItemChange: null };
    const tmp9Result3 = tmp9(13553);
    ...
    tmp9Result4 = tmp9(13553);
  }
}
get_ActivityIndicator = fn(17);
({ View: hasOwnProperty, ScrollView: metroRequire } = get_ActivityIndicator);
const NitroHomeSectionId = fn(13497).NitroHomeSectionId;
...
export default function PremiumNitroHome(onClose) {
  ...
  const commonTriggerPoint = bottom(13556).useCommonTriggerPoint(bottom(13557).OpenNitroTriggerPoint);
  let obj = bottom(13556);
  const giftCardMobileConsumptionHalfsheet = bottom(7495).useGiftCardMobileConsumptionHalfsheet();
  const tmp5 = closure_24();
  bottom = navigation(1612)().bottom;
  let obj2 = bottom(7495);
  const youBarSettingsCustomHeaderPaddingTop = bottom(13558).useYouBarSettingsCustomHeaderPaddingTop();
  let obj3 = bottom(13558);
  ...
  const tmp21 = navigation(7503)();
  const obj7 = bottom(504);
  const isInReverseTrial = bottom(8182).useIsInReverseTrial();
  const obj8 = bottom(8182);
  const maybeFetchTieredTenureBadgeData = bottom(13559).useMaybeFetchTieredTenureBadgeData();
  const field = PremiumNitroNavigationStore.useField("scrollToSectionId");
  const effect = stateFromStores.useEffect(() => () => {
    closure_1_9.resetState();
  }, []);
  const obj9 = bottom(13559);
  ...
  const segmentedControlState = bottom(9932).useSegmentedControlState(obj11);
  const obj10 = bottom(9932);
  ...
  sharedValue = bottom(4374).useSharedValue(num);
  const obj16 = bottom(4374);
  class Q {
    constructor() {
  ...
  const animatedStyle = bottom(4374).useAnimatedStyle(Q);
  ...
  let tmpResult = bottom(4374);
  const theme = bottom(4489).useThemeContext().theme;
  let tmpResult6 = bottom(4489);
  const isThemeDarkResult = bottom(4489).isThemeDark(theme);
  const tmpResult7 = bottom(4489);
  const tmp6Result = navigation(13560);
  const tmp6ResultResult = navigation(13560)(tmp21.endsAt, bottom(13560).CountDownMessageTypes.ENDS_IN);
  isEligibleSenderForReferralProgram = bottom(8173).useIsEligibleSenderForReferralProgram();
  ...
  const effect2 = obj5.useEffect(() => {
    if (!obj.UNSAFE_isDismissibleContentDismissed(dismissible_content.DismissibleContent.MOBILE_NITRO_HOME_SETTINGS_BADGE)) {
      const result = tmp(4458).UNSAFE_markDismissibleContentAsDismissed(tmp(1944).DismissibleContent.MOBILE_NITRO_HOME_SETTINGS_BADGE);
    ...
    if (!tmpResult4.UNSAFE_isDismissibleContentDismissed(dismissible_content.DismissibleContent.REFERRAL_TRIAL_MOBILE_NITRO_HOME_BADGE)) {
      const result1 = tmp(4458).UNSAFE_markDismissibleContentAsDismissed(tmp(1944).DismissibleContent.REFERRAL_TRIAL_MOBILE_NITRO_HOME_BADGE);
      const tmpResult5 = tmp(4458);
    }
    if (isEligibleSenderForReferralProgram) {
      const result2 = tmp(8172).markReferralProgramEntrypointBadgeAcknowledged();
      const tmpResult6 = tmp(8172);
    }
  }, items5);
  ...
  const tmpResult8 = bottom(8173);
  promotionMarketingComponent = bottom(13521).usePromotionMarketingComponent(tmp(10883).MarketingComponentType.PREMIUM_TAB);
  ...
  const tmpResult9 = bottom(13521);
  obj18.source = navigation(13561);
  const items8 = [closure_17(navigation(5673), obj18), closure_17(NitroHomeHeader, { onClose: onClose.onClose, subscription: stateFromStores1 }), , , ];
  ...
  if (tmp26Result) {
    ...
    tmp26Result = tmp26(tmp6(13562), obj19);
  }
  items8[2] = tmp26Result;
  const obj20 = { style: tmp5.tabContent, children: null };
  const items9 = [closure_17(bottom(12759).SegmentedControlPages, { state: segmentedControlState }), ];
  if (tmp26Result4) {
  ...
  const tmp6Result3 = navigation(5673);
  const items11 = [closure_17(c5, { style: tmp5.segmentedControlActual, onLayout: callback, children: closure_17(bottom(9933).SegmentedControl, { state: segmentedControlState, variant: "experimental_Small" }) }), , ];
  ...
  if (!isThemeDarkResult) {
    ...
    items12[1] = obj25;
    obj24.style = items12;
    tmp26Result5 = tmp26(tmp6(5072), obj24);
  }
  items11[1] = tmp26Result5;
  const obj23 = { style: tmp5.segmentedControlActual, onLayout: callback, children: closure_17(bottom(9933).SegmentedControl, { state: segmentedControlState, variant: "experimental_Small" }) };
  if (tmpResult10.isAndroid()) {
    ...
    tmp26Result6 = tmp26(tmp6(5047), obj28);
    const tmp6Result4 = tmp6(5047);
  }
  items11[2] = tmp26Result6;
  obj22.children = items11;
  items8[4] = closure_18(navigation(4374).View, obj22);
