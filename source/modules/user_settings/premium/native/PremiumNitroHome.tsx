...
// Dependencies: [32, 19, 17, 4714, 4386, 13602, 1074, 1955, 1374, 21, 672, 576, 4722, 1484, 13603, 5292, 5798, 4718, 1115, 1241, 7576, 13604, 8276, 13605, 13626, 10985, 13630, 13631, 13632, 13637, 13641, 13658, 13659, 13661, 13662, 7581, 1612, 13663, 504, 7589, 8285, 13664, 10037, 4458, 5143, 4573, 13665, 4542, 1944, 8275, 1946, 5757, 13666, 13667, 12858, 10038, 5156, 1364, 5131, 2]
// Exports: default

// Module 13601 (PremiumNitroHome)
...
import PremiumPerkCardDefault from "PremiumPerkCard" /* 13605 */;
import useScrollToSectionDefault from "useScrollToSection" /* 13630 */;
import PremiumNitroHomeUtils from "PremiumNitroHomeUtils" /* 13631 */;
...
function NitroHomeHeader(arg0) {
  ...
  const mobileNitroManageSubscriptionsSettingsExperiment = onClose(13603).useMobileNitroManageSubscriptionsSettingsExperiment({ location: "NitroHomeHeader" });
  let hasActiveTrial;
  ...
  const obj2 = onClose(13603);
  const tmp8 = !hasActiveTrial;
  const items1 = [closure_17(onClose(5292).PressableOpacity, { style: tmp.backButtonWrapper, onPress: callback, children: closure_17(onClose(5798).ArrowLargeLeftIcon, { size: "md", color: "white" }) }), , ];
  ...
  items1[1] = closure_17(onClose(4718).Text, obj6);
  if (mobileNitroManageSubscriptionsSettingsExperiment) {
    ...
    const obj8 = {
      ...
      children: tmp11(tmp2(7576).SettingsIcon, { size: "md", color: "white" })
    };
    tmp11Result = tmp11(tmp2(5292).PressableOpacity, obj8);
  }
  ...
  if (tmp11Result2) {
    const obj9 = { style: tmp.pillParent, children: tmp11(tmp2(13604).PremiumReferralTrialPill, { hasExtraMargin: true }) };
...
function NewTab(arg0) {
  ...
  if (tmp12) {
    ...
    const obj7 = { width: tmp2(13605).PERK_CARD_SIZES[tmp2(undefined, 13605).PerkCardVariant.WIDE].width };
    obj6.style = obj7;
    ...
    tmp12 = closure_17(tmp8(13632), obj6);
    const tmp8Result = tmp8(13632);
  }
  const items1 = [tmp12, closure_17(require("TieredTenureBadgePerkCard").TieredTenureBadgePerkCard, {}), , , , , ];
  let tmp15Result = null;
  if (isEligibleSenderForReferralProgram) {
    const obj8 = { onLayout: useScrollToSectionDefault(ref, scrollToSectionId).createSectionLayoutHandler(NitroHomeSectionId.REFERRAL_PROGRAM), children: tmp15(tmp2(13641).ReferralProgramPerkCard, {}) };
...
function PerksTab(extraBottomHeight) {
  ...
  if (fractionalState === FractionalPremiumStates.FP_ONLY) {
    ...
    items4[2] = tmp8(tmp9(13658), obj5);
    const obj6 = { title: null, perks: null, onItemChange: null };
    const tmp9Result3 = tmp9(13658);
...
export default function PremiumNitroHome(onClose) {
  ...
  const commonTriggerPoint = bottom(13661).useCommonTriggerPoint(bottom(13662).OpenNitroTriggerPoint);
  let obj = bottom(13661);
  const giftCardMobileConsumptionHalfsheet = bottom(7581).useGiftCardMobileConsumptionHalfsheet();
  const tmp5 = closure_24();
  bottom = navigation(1612)().bottom;
  let obj2 = bottom(7581);
  const youBarSettingsCustomHeaderPaddingTop = bottom(13663).useYouBarSettingsCustomHeaderPaddingTop();
  let obj3 = bottom(13663);
  ...
  const tmp21 = navigation(7589)();
  const obj7 = bottom(504);
  const isInReverseTrial = bottom(8285).useIsInReverseTrial();
  const obj8 = bottom(8285);
  const maybeFetchTieredTenureBadgeData = bottom(13664).useMaybeFetchTieredTenureBadgeData();
  const field = PremiumNitroNavigationStore.useField("scrollToSectionId");
  const effect = stateFromStores.useEffect(() => () => {
    closure_1_9.resetState();
  }, []);
  const obj9 = bottom(13664);
  ...
  const segmentedControlState = bottom(10037).useSegmentedControlState(obj11);
  const obj10 = bottom(10037);
  ...
  sharedValue = bottom(4458).useSharedValue(num);
  const obj16 = bottom(4458);
  class Q {
    constructor() {
  ...
  const animatedStyle = bottom(4458).useAnimatedStyle(Q);
  ...
  let tmpResult = bottom(4458);
  const theme = bottom(4573).useThemeContext().theme;
  let tmpResult6 = bottom(4573);
  const isThemeDarkResult = bottom(4573).isThemeDark(theme);
  const tmpResult7 = bottom(4573);
  const tmp6Result = navigation(13665);
  const tmp6ResultResult = navigation(13665)(tmp21.endsAt, bottom(13665).CountDownMessageTypes.ENDS_IN);
  isEligibleSenderForReferralProgram = bottom(8276).useIsEligibleSenderForReferralProgram();
  ...
  const effect2 = obj5.useEffect(() => {
    if (!obj.UNSAFE_isDismissibleContentDismissed(dismissible_content.DismissibleContent.MOBILE_NITRO_HOME_SETTINGS_BADGE)) {
      const result = tmp(4542).UNSAFE_markDismissibleContentAsDismissed(tmp(1944).DismissibleContent.MOBILE_NITRO_HOME_SETTINGS_BADGE);
    ...
    if (!tmpResult4.UNSAFE_isDismissibleContentDismissed(dismissible_content.DismissibleContent.REFERRAL_TRIAL_MOBILE_NITRO_HOME_BADGE)) {
      const result1 = tmp(4542).UNSAFE_markDismissibleContentAsDismissed(tmp(1944).DismissibleContent.REFERRAL_TRIAL_MOBILE_NITRO_HOME_BADGE);
      const tmpResult5 = tmp(4542);
    }
    if (isEligibleSenderForReferralProgram) {
      const result2 = tmp(8275).markReferralProgramEntrypointBadgeAcknowledged();
      const tmpResult6 = tmp(8275);
    }
  }, items5);
  ...
  const tmpResult8 = bottom(8276);
  promotionMarketingComponent = bottom(13626).usePromotionMarketingComponent(tmp(10985).MarketingComponentType.PREMIUM_TAB);
  ...
  const tmpResult9 = bottom(13626);
  obj18.source = navigation(13666);
  const items8 = [closure_17(navigation(5757), obj18), closure_17(NitroHomeHeader, { onClose: onClose.onClose, subscription: stateFromStores1 }), , , ];
  ...
  if (tmp26Result) {
    ...
    tmp26Result = tmp26(tmp6(13667), obj19);
  }
  items8[2] = tmp26Result;
  const obj20 = { style: tmp5.tabContent, children: null };
  const items9 = [closure_17(bottom(12858).SegmentedControlPages, { state: segmentedControlState }), ];
  if (tmp26Result4) {
  ...
  const tmp6Result3 = navigation(5757);
  const items11 = [closure_17(c5, { style: tmp5.segmentedControlActual, onLayout: callback, children: closure_17(bottom(10038).SegmentedControl, { state: segmentedControlState, variant: "experimental_Small" }) }), , ];
  ...
  if (!isThemeDarkResult) {
    ...
    items12[1] = obj25;
    obj24.style = items12;
    tmp26Result5 = tmp26(tmp6(5156), obj24);
  }
  items11[1] = tmp26Result5;
  const obj23 = { style: tmp5.segmentedControlActual, onLayout: callback, children: closure_17(bottom(10038).SegmentedControl, { state: segmentedControlState, variant: "experimental_Small" }) };
  if (tmpResult10.isAndroid()) {
    ...
    tmp26Result6 = tmp26(tmp6(5131), obj28);
    const tmp6Result4 = tmp6(5131);
  }
  items11[2] = tmp26Result6;
  obj22.children = items11;
  items8[4] = closure_18(navigation(4458).View, obj22);
