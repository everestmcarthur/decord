...
// Dependencies: [32, 19, 17, 4566, 4237, 13440, 1074, 1954, 1373, 21, 672, 576, 4574, 1483, 13441, 5137, 5642, 4570, 1114, 1242, 7394, 13442, 8071, 13443, 13379, 10774, 13464, 13465, 13425, 13466, 13470, 13486, 13390, 13437, 13438, 7399, 1611, 13377, 504, 7407, 8080, 13487, 9824, 4310, 4988, 4425, 13488, 4394, 1943, 8070, 1945, 5601, 13489, 13490, 12646, 9825, 5001, 1115, 4978, 2]
// Exports: default

// Module 13439 (PremiumNitroHome)
...
import PremiumPerkCardDefault from "PremiumPerkCard" /* 13443 */;
import useScrollToSectionDefault from "useScrollToSection" /* 13464 */;
import PremiumNitroHomeUtils from "PremiumNitroHomeUtils" /* 13465 */;
...
function NitroHomeHeader(arg0) {
  ...
  const mobileNitroManageSubscriptionsSettingsExperiment = onClose(13441).useMobileNitroManageSubscriptionsSettingsExperiment({ location: "NitroHomeHeader" });
  let hasActiveTrial;
  ...
  const obj2 = onClose(13441);
  const tmp8 = !hasActiveTrial;
  const items1 = [closure_17(onClose(5137).PressableOpacity, { style: tmp.backButtonWrapper, onPress: callback, children: closure_17(onClose(5642).ArrowLargeLeftIcon, { size: "md", color: "white" }) }), , ];
  ...
  items1[1] = closure_17(onClose(4570).Text, obj6);
  if (mobileNitroManageSubscriptionsSettingsExperiment) {
    ...
    const obj8 = {
      ...
      children: tmp11(tmp2(7394).SettingsIcon, { size: "md", color: "white" })
    };
    tmp11Result = tmp11(tmp2(5137).PressableOpacity, obj8);
  }
  ...
  if (tmp11Result2) {
    const obj9 = { style: tmp.pillParent, children: tmp11(tmp2(13442).PremiumReferralTrialPill, { hasExtraMargin: true }) };
...
function NewTab(arg0) {
  ...
  if (tmp12) {
    ...
    const obj7 = { width: tmp2(13443).PERK_CARD_SIZES[tmp2(undefined, 13443).PerkCardVariant.WIDE].width };
    obj6.style = obj7;
    ...
    tmp12 = closure_17(tmp8(13425), obj6);
    const tmp8Result = tmp8(13425);
  }
  const items1 = [tmp12, closure_17(require("TieredTenureBadgePerkCard").TieredTenureBadgePerkCard, {}), , , , , ];
  let tmp15Result = null;
  if (isEligibleSenderForReferralProgram) {
    const obj8 = { onLayout: useScrollToSectionDefault(ref, scrollToSectionId).createSectionLayoutHandler(NitroHomeSectionId.REFERRAL_PROGRAM), children: tmp15(tmp2(13470).ReferralProgramPerkCard, {}) };
...
function PerksTab(extraBottomHeight) {
  ...
  if (fractionalState === FractionalPremiumStates.FP_ONLY) {
    ...
    items4[2] = tmp8(tmp9(13486), obj5);
    const obj6 = { title: null, perks: null, onItemChange: null };
    const tmp9Result3 = tmp9(13486);
...
export default function PremiumNitroHome(onClose) {
  ...
  const commonTriggerPoint = bottom(13437).useCommonTriggerPoint(bottom(13438).OpenNitroTriggerPoint);
  let obj = bottom(13437);
  const giftCardMobileConsumptionHalfsheet = bottom(7399).useGiftCardMobileConsumptionHalfsheet();
  const tmp5 = closure_24();
  bottom = navigation(1611)().bottom;
  let obj2 = bottom(7399);
  const youBarSettingsCustomHeaderPaddingTop = bottom(13377).useYouBarSettingsCustomHeaderPaddingTop();
  let obj3 = bottom(13377);
  ...
  const tmp21 = navigation(7407)();
  const obj7 = bottom(504);
  const isInReverseTrial = bottom(8080).useIsInReverseTrial();
  const obj8 = bottom(8080);
  const maybeFetchTieredTenureBadgeData = bottom(13487).useMaybeFetchTieredTenureBadgeData();
  const field = PremiumNitroNavigationStore.useField("scrollToSectionId");
  const effect = stateFromStores.useEffect(() => () => {
    closure_1_9.resetState();
  }, []);
  const obj9 = bottom(13487);
  ...
  const segmentedControlState = bottom(9824).useSegmentedControlState(obj11);
  const obj10 = bottom(9824);
  ...
  sharedValue = bottom(4310).useSharedValue(num);
  const obj16 = bottom(4310);
  class Q {
    constructor() {
  ...
  const animatedStyle = bottom(4310).useAnimatedStyle(Q);
  ...
  let tmpResult = bottom(4310);
  const theme = bottom(4425).useThemeContext().theme;
  let tmpResult6 = bottom(4425);
  const isThemeDarkResult = bottom(4425).isThemeDark(theme);
  const tmpResult7 = bottom(4425);
  const tmp6Result = navigation(13488);
  const tmp6ResultResult = navigation(13488)(tmp21.endsAt, bottom(13488).CountDownMessageTypes.ENDS_IN);
  isEligibleSenderForReferralProgram = bottom(8071).useIsEligibleSenderForReferralProgram();
  ...
  const effect2 = obj5.useEffect(() => {
    if (!obj.UNSAFE_isDismissibleContentDismissed(dismissible_content.DismissibleContent.MOBILE_NITRO_HOME_SETTINGS_BADGE)) {
      const result = tmp(4394).UNSAFE_markDismissibleContentAsDismissed(tmp(1943).DismissibleContent.MOBILE_NITRO_HOME_SETTINGS_BADGE);
    ...
    if (!tmpResult4.UNSAFE_isDismissibleContentDismissed(dismissible_content.DismissibleContent.REFERRAL_TRIAL_MOBILE_NITRO_HOME_BADGE)) {
      const result1 = tmp(4394).UNSAFE_markDismissibleContentAsDismissed(tmp(1943).DismissibleContent.REFERRAL_TRIAL_MOBILE_NITRO_HOME_BADGE);
      const tmpResult5 = tmp(4394);
    }
    if (isEligibleSenderForReferralProgram) {
      const result2 = tmp(8070).markReferralProgramEntrypointBadgeAcknowledged();
      const tmpResult6 = tmp(8070);
    }
  }, items5);
  ...
  const tmpResult8 = bottom(8071);
  promotionMarketingComponent = bottom(13379).usePromotionMarketingComponent(tmp(10774).MarketingComponentType.PREMIUM_TAB);
  ...
  const tmpResult9 = bottom(13379);
  obj18.source = navigation(13489);
  const items8 = [closure_17(navigation(5601), obj18), closure_17(NitroHomeHeader, { onClose: onClose.onClose, subscription: stateFromStores1 }), , , ];
  ...
  if (tmp26Result) {
    ...
    tmp26Result = tmp26(tmp6(13490), obj19);
  }
  items8[2] = tmp26Result;
  const obj20 = { style: tmp5.tabContent, children: null };
  const items9 = [closure_17(bottom(12646).SegmentedControlPages, { state: segmentedControlState }), ];
  if (tmp26Result4) {
  ...
  const tmp6Result3 = navigation(5601);
  const items11 = [closure_17(c5, { style: tmp5.segmentedControlActual, onLayout: callback, children: closure_17(bottom(9825).SegmentedControl, { state: segmentedControlState, variant: "experimental_Small" }) }), , ];
  ...
  if (!isThemeDarkResult) {
    ...
    items12[1] = obj25;
    obj24.style = items12;
    tmp26Result5 = tmp26(tmp6(5001), obj24);
  }
  items11[1] = tmp26Result5;
  const obj23 = { style: tmp5.segmentedControlActual, onLayout: callback, children: closure_17(bottom(9825).SegmentedControl, { state: segmentedControlState, variant: "experimental_Small" }) };
  if (tmpResult10.isAndroid()) {
    ...
    tmp26Result6 = tmp26(tmp6(4978), obj28);
    const tmp6Result4 = tmp6(4978);
  }
  items11[2] = tmp26Result6;
  obj22.children = items11;
  items8[4] = closure_18(navigation(4310).View, obj22);
