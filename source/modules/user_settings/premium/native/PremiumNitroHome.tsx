...
// Dependencies: [32, 19, 17, 4597, 4268, 13489, 1074, 1954, 1373, 21, 672, 576, 4605, 1483, 13490, 5173, 5678, 4601, 1114, 1242, 7432, 13491, 8109, 13492, 13428, 10812, 13513, 13514, 13474, 13515, 13519, 13535, 13439, 13486, 13487, 7437, 1611, 13426, 504, 7445, 8118, 13536, 9862, 4341, 5024, 4456, 13537, 4425, 1943, 8108, 1945, 5637, 13538, 13539, 12686, 9863, 5037, 1115, 5014, 2]
// Exports: default

// Module 13488 (PremiumNitroHome)
...
import PremiumPerkCardDefault from "PremiumPerkCard" /* 13492 */;
import useScrollToSectionDefault from "useScrollToSection" /* 13513 */;
import PremiumNitroHomeUtils from "PremiumNitroHomeUtils" /* 13514 */;
import _slicedToArray from "module_32" /* 32 */;
import noop from "module_19" /* 19 */;
import AccessibilityStore from "AccessibilityStore" /* 4597 */;
import SubscriptionStore from "SubscriptionStore" /* 4268 */;
import PremiumNitroNavigationStore from "PremiumNitroNavigationStore" /* 13489 */;
...
function NitroHomeHeader(arg0) {
  ...
  const mobileNitroManageSubscriptionsSettingsExperiment = onClose(13490).useMobileNitroManageSubscriptionsSettingsExperiment({ location: "NitroHomeHeader" });
  let hasActiveTrial;
  ...
  const obj2 = onClose(13490);
  const tmp8 = !hasActiveTrial;
  const items1 = [closure_17(onClose(5173).PressableOpacity, { style: tmp.backButtonWrapper, onPress: callback, children: closure_17(onClose(5678).ArrowLargeLeftIcon, { size: "md", color: "white" }) }), , ];
  ...
  items1[1] = closure_17(onClose(4601).Text, obj6);
  if (mobileNitroManageSubscriptionsSettingsExperiment) {
    ...
    const obj8 = {
      ...
      children: tmp11(tmp2(7432).SettingsIcon, { size: "md", color: "white" })
    };
    tmp11Result = tmp11(tmp2(5173).PressableOpacity, obj8);
  }
  ...
  if (tmp11Result2) {
    const obj9 = { style: tmp.pillParent, children: tmp11(tmp2(13491).PremiumReferralTrialPill, { hasExtraMargin: true }) };
...
function NewTab(arg0) {
  ...
  if (tmp12) {
    ...
    const obj7 = { width: tmp2(13492).PERK_CARD_SIZES[tmp2(undefined, 13492).PerkCardVariant.WIDE].width };
    obj6.style = obj7;
    ...
    tmp12 = closure_17(tmp8(13474), obj6);
    const tmp8Result = tmp8(13474);
  }
  const items1 = [tmp12, closure_17(require("TieredTenureBadgePerkCard").TieredTenureBadgePerkCard, {}), , , , , ];
  let tmp15Result = null;
  if (isEligibleSenderForReferralProgram) {
    const obj8 = { onLayout: useScrollToSectionDefault(ref, scrollToSectionId).createSectionLayoutHandler(NitroHomeSectionId.REFERRAL_PROGRAM), children: tmp15(tmp2(13519).ReferralProgramPerkCard, {}) };
...
function PerksTab(extraBottomHeight) {
  ...
  if (fractionalState === FractionalPremiumStates.FP_ONLY) {
    ...
    items4[2] = tmp8(tmp9(13535), obj5);
    const obj6 = { title: null, perks: null, onItemChange: null };
    const tmp9Result3 = tmp9(13535);
    ...
    tmp9Result4 = tmp9(13535);
  }
}
get_ActivityIndicator = fn(17);
({ View: hasOwnProperty, ScrollView: metroRequire } = get_ActivityIndicator);
const NitroHomeSectionId = fn(13489).NitroHomeSectionId;
...
export default function PremiumNitroHome(onClose) {
  ...
  const commonTriggerPoint = bottom(13486).useCommonTriggerPoint(bottom(13487).OpenNitroTriggerPoint);
  let obj = bottom(13486);
  const giftCardMobileConsumptionHalfsheet = bottom(7437).useGiftCardMobileConsumptionHalfsheet();
  const tmp5 = closure_24();
  bottom = navigation(1611)().bottom;
  let obj2 = bottom(7437);
  const youBarSettingsCustomHeaderPaddingTop = bottom(13426).useYouBarSettingsCustomHeaderPaddingTop();
  let obj3 = bottom(13426);
  ...
  const tmp21 = navigation(7445)();
  const obj7 = bottom(504);
  const isInReverseTrial = bottom(8118).useIsInReverseTrial();
  const obj8 = bottom(8118);
  const maybeFetchTieredTenureBadgeData = bottom(13536).useMaybeFetchTieredTenureBadgeData();
  const field = PremiumNitroNavigationStore.useField("scrollToSectionId");
  const effect = stateFromStores.useEffect(() => () => {
    closure_1_9.resetState();
  }, []);
  const obj9 = bottom(13536);
  ...
  const segmentedControlState = bottom(9862).useSegmentedControlState(obj11);
  const obj10 = bottom(9862);
  ...
  sharedValue = bottom(4341).useSharedValue(num);
  const obj16 = bottom(4341);
  class Q {
    constructor() {
  ...
  const animatedStyle = bottom(4341).useAnimatedStyle(Q);
  ...
  let tmpResult = bottom(4341);
  const theme = bottom(4456).useThemeContext().theme;
  let tmpResult6 = bottom(4456);
  const isThemeDarkResult = bottom(4456).isThemeDark(theme);
  const tmpResult7 = bottom(4456);
  const tmp6Result = navigation(13537);
  const tmp6ResultResult = navigation(13537)(tmp21.endsAt, bottom(13537).CountDownMessageTypes.ENDS_IN);
  isEligibleSenderForReferralProgram = bottom(8109).useIsEligibleSenderForReferralProgram();
  ...
  const effect2 = obj5.useEffect(() => {
    if (!obj.UNSAFE_isDismissibleContentDismissed(dismissible_content.DismissibleContent.MOBILE_NITRO_HOME_SETTINGS_BADGE)) {
      const result = tmp(4425).UNSAFE_markDismissibleContentAsDismissed(tmp(1943).DismissibleContent.MOBILE_NITRO_HOME_SETTINGS_BADGE);
    ...
    if (!tmpResult4.UNSAFE_isDismissibleContentDismissed(dismissible_content.DismissibleContent.REFERRAL_TRIAL_MOBILE_NITRO_HOME_BADGE)) {
      const result1 = tmp(4425).UNSAFE_markDismissibleContentAsDismissed(tmp(1943).DismissibleContent.REFERRAL_TRIAL_MOBILE_NITRO_HOME_BADGE);
      const tmpResult5 = tmp(4425);
    }
    if (isEligibleSenderForReferralProgram) {
      const result2 = tmp(8108).markReferralProgramEntrypointBadgeAcknowledged();
      const tmpResult6 = tmp(8108);
    }
  }, items5);
  ...
  const tmpResult8 = bottom(8109);
  promotionMarketingComponent = bottom(13428).usePromotionMarketingComponent(tmp(10812).MarketingComponentType.PREMIUM_TAB);
  ...
  const tmpResult9 = bottom(13428);
  obj18.source = navigation(13538);
  const items8 = [closure_17(navigation(5637), obj18), closure_17(NitroHomeHeader, { onClose: onClose.onClose, subscription: stateFromStores1 }), , , ];
  ...
  if (tmp26Result) {
    ...
    tmp26Result = tmp26(tmp6(13539), obj19);
  }
  items8[2] = tmp26Result;
  const obj20 = { style: tmp5.tabContent, children: null };
  const items9 = [closure_17(bottom(12686).SegmentedControlPages, { state: segmentedControlState }), ];
  if (tmp26Result4) {
  ...
  const tmp6Result3 = navigation(5637);
  const items11 = [closure_17(c5, { style: tmp5.segmentedControlActual, onLayout: callback, children: closure_17(bottom(9863).SegmentedControl, { state: segmentedControlState, variant: "experimental_Small" }) }), , ];
  ...
  if (!isThemeDarkResult) {
    ...
    items12[1] = obj25;
    obj24.style = items12;
    tmp26Result5 = tmp26(tmp6(5037), obj24);
  }
  items11[1] = tmp26Result5;
  const obj23 = { style: tmp5.segmentedControlActual, onLayout: callback, children: closure_17(bottom(9863).SegmentedControl, { state: segmentedControlState, variant: "experimental_Small" }) };
  if (tmpResult10.isAndroid()) {
    ...
    tmp26Result6 = tmp26(tmp6(5014), obj28);
    const tmp6Result4 = tmp6(5014);
  }
  items11[2] = tmp26Result6;
  obj22.children = items11;
  items8[4] = closure_18(navigation(4341).View, obj22);
