...
// Dependencies: [32, 19, 17, 1074, 7505, 1373, 21, 576, 4606, 5038, 1093, 4602, 5638, 1114, 13525, 13526, 13527, 13528, 5013, 1608, 10846, 1178, 7235, 1477, 1242, 2]
// Exports: default

// Module 13524 (PremiumFeaturesCarouselSection)
...
function PremiumFeaturesCarousel(arg0) {
  ...
  const isScreenReaderEnabled = onEndReached(5013).useIsScreenReaderEnabled();
  ...
  if (!onEndReached(1608).isThumbstickScrollDevice) {
    if (!isScreenReaderEnabled) {
      ...
      let tmp19 = closure_10(currentIndex(10846), obj3);
    }
    let items3 = [tmp19, ];
    let obj4 = { containerStyle: tmp.indicators, numberOfItems: mapped.length, currentIndex };
    items3[1] = tmp17(tmp2(1178).CarouselPagination, obj4);
    obj2.children = items3;
    return tmp15(tmp16, obj2);
  }
  let obj = onEndReached(5013);
  tmp15 = closure_11;
  tmp16 = closure_6;
  tmp2 = onEndReached;
...
export default function PremiumFeaturesCarouselSection(style) {
  ...
  const tmp = closure_14();
  const analyticsLocations = first(7235)().analyticsLocations;
  ...
  const items2 = [closure_10(analyticsLocations(4602).Text, obj3), closure_10(PremiumFeaturesCarousel, { width: tmp4[0], onEndReached: callback })];
