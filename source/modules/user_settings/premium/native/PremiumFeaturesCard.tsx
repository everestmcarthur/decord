...
// Dependencies: [32, 19, 17, 4632, 4302, 4303, 1074, 1374, 1085, 21, 4640, 576, 5612, 9518, 4297, 7348, 4636, 1115, 8186, 3077, 1380, 38, 7561, 8197, 8195, 7507, 7560, 7276, 504, 9524, 1609, 7523, 9533, 7552, 8988, 9535, 9536, 9538, 9545, 5062, 8817, 7536, 2]
// Exports: default

// Module 9517 (PremiumFeaturesCard)
...
import usePremiumPlanPriceDefault from "usePremiumPlanPrice" /* 9518 */;
...
function PriceText(fractionalPremiumInfo) {
  ...
  if (null != discountedPriceString) {
    if (null != discountOffer) {
      ...
      return closure_1_19(tmp7(4636).Text, obj3);
    }
  }
    ...
    if (null != premiumSubscription) {
      if (premiumSubscription.planIdFromItems === PREMIUM_YEAR_TIER_2.PREMIUM_YEAR_TIER_2) {
        ...
        if (flag) {
          ...
          return closure_1_19(tmp7(4636).Text, obj6);
        }
      }
      if (premiumSubscription.hasAnyPremiumGroup) {
        ...
        if (null != prop) {
          let priceString1 = tmp7(8186).getPriceString(premiumSubscription);
          ...
          formatResult = intl6.format(_modDef3077.FwjZzr, obj7);
          const tmp7Result6 = tmp7(8186);
        }
      }
      if (activeDiscountInfo.discountId === closure_1_11) {
        ...
        if (source === tmp7(9518).PremiumPlanPriceSource.API) {
          ...
          const obj8 = { numMonths: duration2, discountedPrice: tmp7(7348).formatPrice(rounded, tmp4.currency), billingPeriod: null, fullPrice: null };
  ...
  if (null != subscriptionTrial) {
    if (premiumType === map1[subscriptionTrial.skuId]) {
      ...
      let tmp11Result = closure_1_19(tmp7(4636).Text, obj10);
...
export default function PremiumFeaturesCard(premiumType) {
  ...
  const premiumTrialOffer = premiumType(7561).usePremiumTrialOffer();
  const obj = premiumType(7561);
  const premiumDiscountOffer = premiumType(8197).usePremiumDiscountOffer();
  const obj2 = premiumType(8197);
  const activeDiscountInfo = premiumType(8195).useActiveDiscountInfo();
  const obj3 = premiumType(8195);
  const tmp12 = useFractionalPremiumInfoDefault();
  let subscriptionTrial;
  const premiumTrialOfferPremiumType = premiumType(7560).usePremiumTrialOfferPremiumType();
  if (premiumTrialOffer != null) {
    subscriptionTrial = premiumTrialOffer.subscriptionTrial;
  }
  const obj4 = premiumType(7560);
  let interval;
  if (subscriptionTrial != null) {
    interval = subscriptionTrial.interval;
  }
  ...
  analyticsLocations = tmp4(7276)().analyticsLocations;
  ...
  premiumBundleWithPredicate = premiumType(7523).getPremiumBundleWithPredicate((additionalPlans) => {
  ...
  const tmp8Result12 = premiumType(7523);
  const items3 = [premiumBundleWithPredicate];
  const discountedPriceString = premiumType(9533).useDiscountedPremiumProductInfo(premiumDiscountOffer, items3).discountedPriceString;
  let tmp32 = tmp24;
  ...
  const items4 = [closure_19(premiumType(7552).PremiumPill, { style: tmp3.pill, discountOffer: premiumDiscountOffer, isActiveDiscount: null != activeDiscountInfo, shouldShowDiscountUpsell: null != premiumDiscountOffer && null != discountedPriceString, premiumType, trialOffer: premiumTrialOffer }), ];
  ...
  const tmp8Result13 = premiumType(9533);
  if (flag3) {
    let tmp37Result = tmp37(tmp4(9535), { width: 185, height: 20, alwaysWhite: true });
  } else {
    const obj11 = { premiumType, style: tmp3.logo };
    tmp37Result = tmp37(tmp4(9536), obj11);
  }
  ...
  if (hideButton) {
    ...
    if (tmp37Result6) {
      ...
      obj14.children = tmp37(tmp8(4636).Text, obj15);
    ...
    if (tmp32) {
      ...
      obj17.children = tmp37(tmp8(4636).Text, obj18);
      let obj19 = obj17;
    } else {
      ...
      if (tmp37Result6) {
        ...
        if (null != premiumDiscountOffer) {
          ...
          const tmp37Result7 = tmp37(tmp8(8817).NitroWheelIcon, obj21);
        }
