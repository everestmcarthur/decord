...
// Dependencies: [32, 19, 17, 4597, 4267, 4268, 1074, 1373, 1085, 21, 4605, 576, 5574, 9434, 4262, 7286, 4601, 1114, 8102, 3074, 1379, 38, 7499, 8113, 8111, 7445, 7498, 7214, 504, 9440, 1608, 7461, 9449, 7490, 8902, 9451, 9452, 9454, 9461, 5025, 8732, 7474, 2]
// Exports: default
...
import usePremiumPlanPriceDefault from "usePremiumPlanPrice" /* 9434 */;
...
function PriceText(fractionalPremiumInfo) {
  ...
  if (null != discountedPriceString) {
    if (null != discountOffer) {
      ...
      return closure_1_19(tmp7(4601).Text, obj3);
    }
  }
    ...
    if (null != premiumSubscription) {
      if (premiumSubscription.planIdFromItems === PREMIUM_YEAR_TIER_2.PREMIUM_YEAR_TIER_2) {
        ...
        if (null != premiumSubscription) {
          ...
          if (tmp21) {
            tmp21 = tmp7(4262).getPremiumType(planIdFromItems) === premiumType;
            const tmp7Result = tmp7(4262);
          }
          flag = tmp21;
        }
        if (flag) {
          ...
          if (!hasActiveTrial) {
            ...
            const obj5 = { percent: percentage, regularPrice: formatRateResult, renewalDate: tmp7(4262).getExpectedRenewalDate(premiumSubscription, fractionalPremiumInfo.fractionalPremiumInfo) };
            let formatResult = intl2.format(tmp7(1114).t.z2oQtA, obj5);
            const tmp7Result5 = tmp7(4262);
          }
          const obj6 = { variant: "text-md/normal", color: "text-overlay-light", style: tmp.discountPriceText, children: formatResult };
          return closure_1_19(tmp7(4601).Text, obj6);
        }
      }
      if (premiumSubscription.hasAnyPremiumGroup) {
        ...
        if (null != prop) {
          let priceString1 = tmp7(8102).getPriceString(premiumSubscription);
      ...
      if (activeDiscountInfo.discountId === closure_1_11) {
        ...
        if (source === tmp7(9434).PremiumPlanPriceSource.API) {
          ...
          const obj8 = { numMonths: duration2, discountedPrice: tmp7(7286).formatPrice(rounded, tmp4.currency), billingPeriod: null, fullPrice: null };
  ...
  if (null != subscriptionTrial) {
    if (premiumType === map1[subscriptionTrial.skuId]) {
      ...
      obj12.trialPeriod = tmp7(4262).formatIntervalDuration(obj11);
      let priceString2;
      if (tmp4 != null) {
      ...
      let tmp11Result = closure_1_19(tmp7(4601).Text, obj10);
...
export default function PremiumFeaturesCard(premiumType) {
  ...
  const premiumTrialOffer = premiumType(7499).usePremiumTrialOffer();
  const obj = premiumType(7499);
  const premiumDiscountOffer = premiumType(8113).usePremiumDiscountOffer();
  const obj2 = premiumType(8113);
  const activeDiscountInfo = premiumType(8111).useActiveDiscountInfo();
  const obj3 = premiumType(8111);
  const tmp12 = useFractionalPremiumInfoDefault();
  let subscriptionTrial;
  const premiumTrialOfferPremiumType = premiumType(7498).usePremiumTrialOfferPremiumType();
  if (premiumTrialOffer != null) {
    subscriptionTrial = premiumTrialOffer.subscriptionTrial;
  }
  const obj4 = premiumType(7498);
  let interval;
  if (subscriptionTrial != null) {
    interval = subscriptionTrial.interval;
  }
  ...
  const result = premiumType(4262).formatIntervalDuration(obj5);
  const intl = tmp8(1114).intl;
  const stringResult = intl.string(premiumType(1114).t.J61px0);
  analyticsLocations = tmp4(7214)().analyticsLocations;
  const tmp8Result = premiumType(4262);
  ...
  const tmp8Result12 = premiumType(7461);
  ...
  const items4 = [closure_19(premiumType(7490).PremiumPill, { style: tmp3.pill, discountOffer: premiumDiscountOffer, isActiveDiscount: null != activeDiscountInfo, shouldShowDiscountUpsell: null != premiumDiscountOffer && null != discountedPriceString, premiumType, trialOffer: premiumTrialOffer }), ];
  ...
  const tmp8Result13 = premiumType(9449);
  if (flag3) {
    let tmp37Result = tmp37(tmp4(9451), { width: 185, height: 20, alwaysWhite: true });
  } else {
    const obj11 = { premiumType, style: tmp3.logo };
    tmp37Result = tmp37(tmp4(9452), obj11);
  }
  ...
  if (hideButton) {
    ...
    if (tmp37Result6) {
      ...
      obj14.children = tmp37(tmp8(4601).Text, obj15);
    ...
    if (tmp32) {
      ...
      obj17.children = tmp37(tmp8(4601).Text, obj18);
      let obj19 = obj17;
    } else {
      ...
      if (tmp37Result6) {
        ...
        if (null != premiumDiscountOffer) {
          ...
          const tmp37Result7 = tmp37(tmp8(8732).NitroWheelIcon, obj21);
        }
