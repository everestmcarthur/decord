...
// Dependencies: [32, 19, 17, 4566, 4236, 4237, 1074, 1373, 1085, 21, 4574, 576, 5538, 9396, 4231, 7248, 4570, 1114, 8064, 3074, 1379, 38, 7461, 8075, 8073, 7407, 7460, 7176, 504, 9402, 1608, 7423, 9411, 7452, 8865, 9413, 9414, 9416, 9423, 4989, 8695, 7436, 2]
// Exports: default
...
import usePremiumPlanPriceDefault from "usePremiumPlanPrice" /* 9396 */;
...
function PriceText(fractionalPremiumInfo) {
  ...
  if (null != discountedPriceString) {
    if (null != discountOffer) {
      ...
      return closure_1_19(tmp7(4570).Text, obj3);
    }
  }
    ...
    if (null != premiumSubscription) {
      if (premiumSubscription.planIdFromItems === PREMIUM_YEAR_TIER_2.PREMIUM_YEAR_TIER_2) {
        ...
        if (null != premiumSubscription) {
          ...
          if (tmp21) {
            tmp21 = tmp7(4231).getPremiumType(planIdFromItems) === premiumType;
            const tmp7Result = tmp7(4231);
          }
          flag = tmp21;
        }
        if (flag) {
          ...
          if (!hasActiveTrial) {
            ...
            const obj5 = { percent: percentage, regularPrice: formatRateResult, renewalDate: tmp7(4231).getExpectedRenewalDate(premiumSubscription, fractionalPremiumInfo.fractionalPremiumInfo) };
            let formatResult = intl2.format(tmp7(1114).t.z2oQtA, obj5);
            const tmp7Result5 = tmp7(4231);
          }
          const obj6 = { variant: "text-md/normal", color: "text-overlay-light", style: tmp.discountPriceText, children: formatResult };
          return closure_1_19(tmp7(4570).Text, obj6);
        }
      }
      if (premiumSubscription.hasAnyPremiumGroup) {
        ...
        if (null != prop) {
          let priceString1 = tmp7(8064).getPriceString(premiumSubscription);
      ...
      if (activeDiscountInfo.discountId === closure_1_11) {
        ...
        if (source === tmp7(9396).PremiumPlanPriceSource.API) {
          ...
          const obj8 = { numMonths: duration2, discountedPrice: tmp7(7248).formatPrice(rounded, tmp4.currency), billingPeriod: null, fullPrice: null };
  ...
  if (null != subscriptionTrial) {
    if (premiumType === map1[subscriptionTrial.skuId]) {
      ...
      obj12.trialPeriod = tmp7(4231).formatIntervalDuration(obj11);
      let priceString2;
      if (tmp4 != null) {
      ...
      let tmp11Result = closure_1_19(tmp7(4570).Text, obj10);
...
export default function PremiumFeaturesCard(premiumType) {
  ...
  const premiumTrialOffer = premiumType(7461).usePremiumTrialOffer();
  const obj = premiumType(7461);
  const premiumDiscountOffer = premiumType(8075).usePremiumDiscountOffer();
  const obj2 = premiumType(8075);
  const activeDiscountInfo = premiumType(8073).useActiveDiscountInfo();
  const obj3 = premiumType(8073);
  const tmp12 = useFractionalPremiumInfoDefault();
  let subscriptionTrial;
  const premiumTrialOfferPremiumType = premiumType(7460).usePremiumTrialOfferPremiumType();
  if (premiumTrialOffer != null) {
    subscriptionTrial = premiumTrialOffer.subscriptionTrial;
  }
  const obj4 = premiumType(7460);
  let interval;
  if (subscriptionTrial != null) {
    interval = subscriptionTrial.interval;
  }
  ...
  const result = premiumType(4231).formatIntervalDuration(obj5);
  const intl = tmp8(1114).intl;
  const stringResult = intl.string(premiumType(1114).t.J61px0);
  analyticsLocations = tmp4(7176)().analyticsLocations;
  const tmp8Result = premiumType(4231);
  ...
  const tmp8Result12 = premiumType(7423);
  ...
  const items4 = [closure_19(premiumType(7452).PremiumPill, { style: tmp3.pill, discountOffer: premiumDiscountOffer, isActiveDiscount: null != activeDiscountInfo, shouldShowDiscountUpsell: null != premiumDiscountOffer && null != discountedPriceString, premiumType, trialOffer: premiumTrialOffer }), ];
  ...
  const tmp8Result13 = premiumType(9411);
  if (flag3) {
    let tmp37Result = tmp37(tmp4(9413), { width: 185, height: 20, alwaysWhite: true });
  } else {
    const obj11 = { premiumType, style: tmp3.logo };
    tmp37Result = tmp37(tmp4(9414), obj11);
  }
  ...
  if (hideButton) {
    ...
    if (tmp37Result6) {
      ...
      obj14.children = tmp37(tmp8(4570).Text, obj15);
    ...
    if (tmp32) {
      ...
      obj17.children = tmp37(tmp8(4570).Text, obj18);
      let obj19 = obj17;
    } else {
      ...
      if (tmp37Result6) {
        ...
        if (null != premiumDiscountOffer) {
          ...
          const tmp37Result7 = tmp37(tmp8(8695).NitroWheelIcon, obj21);
        }
