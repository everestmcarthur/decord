...
// Dependencies: [5, 32, 19, 17, 5369, 1980, 1074, 5496, 21, 4640, 1177, 576, 5612, 5494, 504, 5062, 1115, 4636, 5672, 5371, 7314, 4350, 15088, 9613, 4984, 5081, 9406, 15089, 15092, 5495, 11725, 4334, 2025, 5210, 10801, 15094, 3013, 1397, 4491, 5060, 7106, 5693, 5775, 2]

// Module 15087 (ConnectedAccount)
...
import shouldWarnConnectedAccountTwoWayDefault from "shouldWarnConnectedAccountTwoWay" /* 15088 */;
import XboxTwoWayLinkUpsell from "XboxTwoWayLinkUpsell" /* 15089 */;
import PlayStationTwoWayLinkUpsell from "PlayStationTwoWayLinkUpsell" /* 15092 */;
...
function Integration(integration) {
  ...
  legacyClassComponentStyles = integration(4640).useLegacyClassComponentStyles(legacyClassComponentStyles);
  const obj = integration(4640);
  ...
  if (!tmp7) {
    ...
    if (tmp12) {
      ...
      tmp12 = closure_14(tmp(4636).Text, obj4);
    }
    ...
    const obj7 = { guild: integration.guild, size: tmp(5672).GuildIconSizes.SMALL, style: legacyClassComponentStyles.integrationGuildIcon };
    ...
    const items5 = [closure_14(tmp(4636).Text, obj9), ];
    ...
    items5[1] = tmp16(tmp(4636).Text, obj10);
    ...
    obj13 = tmp8(tmp(5062).Button, obj13);
...
const MetadataFields = fn(5496).MetadataFields;
const jsxProd = fn(21);
  ...
  constructor() {
    ...
    applyArgumentsResult.handleDisconnect = function handleDisconnect() {
      ...
      if (shouldWarnConnectedAccountTwoWayDefault(account)) {
        ...
        const items = [closure_2_14(tmp6(4636).Text, obj4), ];
        ...
        items[1] = closure_2_14(tmp3(9613), obj5);
        obj3.children = items;
        tmp8 = __initData(View, obj3);
        const tmp3Result = tmp3(9613);
      }
...
prototype["renderMetadata"] = function renderMetadata() {
  ...
  const createdAtDate = account(5495).getCreatedAtDate(metadata[MetadataFields.CREATED_AT], props.locale);
  const type = account.type;
  if (constants.REDDIT === type) {
    let redditMetadataItems = tmp2(11725).generateRedditMetadataItems(metadata);
    const tmp2Result = tmp2(11725);
  } else if (tmp6.STEAM === type) {
    redditMetadataItems = tmp2(11725).generateSteamMetadataItems(metadata);
    const tmp2Result6 = tmp2(11725);
  } else {
    if (tmp6.BLUESKY !== type) {
      if (tmp6.TWITTER !== type) {
        if (tmp6.MASTODON !== type) {
          if (tmp6.EBAY === type) {
            redditMetadataItems = tmp2(11725).generateEbayMetadataItems(metadata);
            const tmp2Result7 = tmp2(11725);
          } else if (tmp6.PAYPAL === type) {
            redditMetadataItems = tmp2(11725).generatePaypalMetadataItems(metadata);
            const tmp2Result8 = tmp2(11725);
          } else {
            redditMetadataItems = [];
            if (tmp6.TIKTOK === type) {
              redditMetadataItems = tmp2(11725).generateTikTokMetadataItems(metadata);
              const tmp2Result9 = tmp2(11725);
            }
          }
        }
      }
    }
    redditMetadataItems = tmp2(11725).generateTwitterMetadataItems(metadata);
    const tmp2Result10 = tmp2(11725);
  }
  if (null !== createdAtDate) {
    ...
    redditMetadataItems.push(closure_14(tmp2(4636).Text, obj, MetadataFields.CREATED_AT));
  }
  ...
  if (0 === redditMetadataItems.length) {
    value = self(5371).get(account.type);
    let hasMetadata;
    if (value != null) {
      hasMetadata = value.hasMetadata;
    }
    ...
    if (self.state.metadataAlreadyRefreshed) {
      let tmp9Result = tmp9(10801);
    } else {
      tmp9Result = tmp9(15094);
    }
    ...
    const items3 = [closure_14(View, obj16), closure_14(tmp2(5210).PressableOpacity, obj12)];
...
prototype["render"] = function render() {
  ...
  if (migrationExperimentEnabled) {
    ...
    let name = intl.format(tmp2(3013).Glhokn, obj2);
  } else {
    name = account.name;
  }
  ...
  obj7.children = closure_1_14(native2.Icon, { style: tmp.deleteConnectionIcon, source: _modDef7106 });
  obj6.children = closure_1_14(Pressables.PressableOpacity, obj7);
  const obj8 = { style: tmp.deleteConnectionIcon, source: _modDef7106 };
...
ConnectedAccount.contextType = fn(4350).ThemeContext;
const size = fn(2);
