...
// Dependencies: [32, 19, 17, 4653, 4830, 21, 4639, 576, 7304, 7256, 7306, 4586, 7296, 6711, 5059, 4831, 4606, 504, 8393, 5772, 5690, 4635, 8201, 5057, 2]

// Module 15807 (UserSettingsSurveyChangelogOverride)
import nativeDefault from "native" /* 576 */;
import ActionSheetActionCreatorsDefault from "ActionSheetActionCreators" /* 4606 */;
import Text_Text from "Text/Text" /* 4635 */;
import SurveyActionCreatorsAll from "SurveyActionCreators" /* 4831 */;
...
function SurveyOverrideInfoActionSheet(survey) {
  ...
  obj2.children = entries.map((item) => {
    ...
    return closure_9(closure_0(7306).ActionSheetRow, obj, tmp);
  });
...
function SurveyOverrideActionSheet() {
  ...
  const first = tmp[0];
  let obj = { header: closure_9(first(7256).BottomSheetTitleHeader, { title: "Survey Override" }), children: null };
  const items = [closure_9(first(6711).TextInput, { label: "Survey Override", size: "md", placeholder: "Enter the ID of the Survey you want to test", onChange: tmp[1], isClearable: true }), ];
  ...
  items[1] = closure_9(first(5059).Button, obj2);
  obj.children = items;
  return closure_10(first(7304).ActionSheet, obj);
}
function SurveyInfo() {
...
function ChangelogOverrideActionSheet() {
  ...
  const first = tmp[0];
  let obj = { header: closure_9(first(7256).BottomSheetTitleHeader, { title: "Changelog Override" }), children: null };
  const items = [closure_9(first(6711).TextInput, { label: "Changelog Override", size: "md", placeholder: "Enter the ID of the changelog you want to test", onChange: tmp[1], isClearable: true }), ];
