...
// Dependencies: [19, 17, 13862, 21, 4640, 576, 7303, 4337, 5693, 7247, 1115, 4341, 504, 14801, 5060, 4636, 5775, 2]
// Exports: default

// Module 14800 (UserSettingsAccountBackupCodes)
...
function CodeRow(code) {
  ...
  if (showCheckMark) {
    ...
    tmp2Result = tmp2(code(7247).CheckmarkSmallIcon, obj2);
  }
  obj.trailing = tmp2Result;
  return closure_6(code(5693).TableRow, obj);
}
...
export default function UserSettingsAccountBackupCodes(headerLabel) {
  ...
  const token = stateFromStores(4341).useToken(nativeDefault.modules.mobile.TABLE_ROW_PADDING);
  const obj = stateFromStores(4341);
  ...
  const items2 = [headerLabel.map((children, index) => closure_1_6(stateFromStores(4636).Text, { variant: "text-sm/medium", children }, index)), , , ];
  ...
  if (tmp10Result3) {
    ...
    tmp10Result3 = tmp10(tmp3(5775).TableRowGroup, obj6);
  }
  ...
  if (tmp10Result4) {
    ...
    obj8.label = tmp10(tmp3(4636).Text, obj9);
    ...
    obj7.children = tmp10(tmp3(5693).TableRow, obj8);
    tmp10Result4 = tmp10(tmp3(5775).TableRowGroup, obj7);
  }
  const obj4 = { paddingHorizontal: token, paddingTop: nativeDefault.space.PX_16 };
  items2[3] = tmp10Result4;
  obj3.children = items2;
  return closure_6(tmp11, { children: tmp12(stateFromStores(5060).Stack, obj3) });
};
