...
// Dependencies: [19, 17, 13822, 21, 4605, 576, 7241, 4302, 5655, 7185, 1114, 4306, 504, 14758, 5023, 4601, 5737, 2]
// Exports: default

// Module 14757 (UserSettingsAccountBackupCodes)
...
function CodeRow(code) {
  ...
  if (showCheckMark) {
    ...
    tmp2Result = tmp2(code(7185).CheckmarkSmallIcon, obj2);
  }
  obj.trailing = tmp2Result;
  return closure_6(code(5655).TableRow, obj);
}
...
export default function UserSettingsAccountBackupCodes(headerLabel) {
  ...
  const token = stateFromStores(4306).useToken(nativeDefault.modules.mobile.TABLE_ROW_PADDING);
  const obj = stateFromStores(4306);
  ...
  const items2 = [headerLabel.map((children, index) => closure_1_6(stateFromStores(4601).Text, { variant: "text-sm/medium", children }, index)), , , ];
  ...
  if (tmp10Result3) {
    ...
    tmp10Result3 = tmp10(tmp3(5737).TableRowGroup, obj6);
  }
  ...
  if (tmp10Result4) {
    ...
    obj8.label = tmp10(tmp3(4601).Text, obj9);
    ...
    obj7.children = tmp10(tmp3(5655).TableRow, obj8);
    tmp10Result4 = tmp10(tmp3(5737).TableRowGroup, obj7);
  }
  const obj4 = { paddingHorizontal: token, paddingTop: nativeDefault.space.PX_16 };
  items2[3] = tmp10Result4;
  obj3.children = items2;
  return closure_6(tmp11, { children: tmp12(stateFromStores(5023).Stack, obj3) });
};
