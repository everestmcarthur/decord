...
// Dependencies: [19, 17, 13803, 21, 4607, 576, 7263, 4304, 5657, 7207, 1114, 4308, 504, 14735, 5025, 4603, 5739, 2]
// Exports: default

// Module 14734 (UserSettingsAccountBackupCodes)
...
function CodeRow(code) {
  ...
  if (showCheckMark) {
    ...
    tmp2Result = tmp2(code(7207).CheckmarkSmallIcon, obj2);
  }
  obj.trailing = tmp2Result;
  return closure_6(code(5657).TableRow, obj);
}
...
export default function UserSettingsAccountBackupCodes(headerLabel) {
  ...
  const token = stateFromStores(4308).useToken(nativeDefault.modules.mobile.TABLE_ROW_PADDING);
  const obj = stateFromStores(4308);
  ...
  const items2 = [headerLabel.map((children, index) => closure_1_6(stateFromStores(4603).Text, { variant: "text-sm/medium", children }, index)), , , ];
  ...
  if (tmp10Result3) {
    ...
    tmp10Result3 = tmp10(tmp3(5739).TableRowGroup, obj6);
  }
  ...
  if (tmp10Result4) {
    ...
    obj8.label = tmp10(tmp3(4603).Text, obj9);
    ...
    obj7.children = tmp10(tmp3(5657).TableRow, obj8);
    tmp10Result4 = tmp10(tmp3(5739).TableRowGroup, obj7);
  }
  const obj4 = { paddingHorizontal: token, paddingTop: nativeDefault.space.PX_16 };
  items2[3] = tmp10Result4;
  obj3.children = items2;
  return closure_6(tmp11, { children: tmp12(stateFromStores(5025).Stack, obj3) });
};
