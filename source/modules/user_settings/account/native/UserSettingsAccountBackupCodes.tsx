...
// Dependencies: [19, 17, 13955, 21, 4722, 576, 7385, 4419, 5775, 7329, 1115, 4423, 504, 14895, 5142, 4718, 5857, 2]
// Exports: default

// Module 14894 (UserSettingsAccountBackupCodes)
...
function CodeRow(code) {
  ...
  if (showCheckMark) {
    ...
    tmp2Result = tmp2(code(7329).CheckmarkSmallIcon, obj2);
  }
  obj.trailing = tmp2Result;
  return closure_6(code(5775).TableRow, obj);
}
...
export default function UserSettingsAccountBackupCodes(headerLabel) {
  ...
  const token = stateFromStores(4423).useToken(nativeDefault.modules.mobile.TABLE_ROW_PADDING);
  const obj = stateFromStores(4423);
  ...
  const items2 = [headerLabel.map((children, index) => closure_1_6(stateFromStores(4718).Text, { variant: "text-sm/medium", children }, index)), , , ];
  ...
  if (tmp10Result3) {
    ...
    tmp10Result3 = tmp10(tmp3(5857).TableRowGroup, obj6);
  }
  ...
  if (tmp10Result4) {
    ...
    obj8.label = tmp10(tmp3(4718).Text, obj9);
    ...
    obj7.children = tmp10(tmp3(5775).TableRow, obj8);
    tmp10Result4 = tmp10(tmp3(5857).TableRowGroup, obj7);
  }
  const obj4 = { paddingHorizontal: token, paddingTop: nativeDefault.space.PX_16 };
  items2[3] = tmp10Result4;
  obj3.children = items2;
  return closure_6(tmp11, { children: tmp12(stateFromStores(5142).Stack, obj3) });
};
