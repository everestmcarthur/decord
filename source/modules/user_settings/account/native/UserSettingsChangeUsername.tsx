...
// Dependencies: [5, 32, 19, 17, 1371, 1074, 21, 4607, 576, 4792, 14761, 4603, 1114, 1483, 504, 4264, 14762, 7064, 7058, 1272, 1484, 7937, 6678, 7072, 7010, 2]
// Exports: default

// Module 14760 (UserSettingsChangeUsername)
...
function UsernameStatusMessage(showHint) {
  ...
  const match = showHint(4792).match(showHint.usernameStatus);
  let obj = { type: showHint(14761).NameValidationState.ERROR, message: null };
  const P = showHint(4792).P;
  obj.message = P.select();
  const str = showHint(4792);
  const obj2 = { type: showHint(14761).NameValidationState.AVAILABLE, message: null };
  const P2 = showHint(4792).P;
  obj2.message = P2.select();
  const withResult = match.with(obj, (children) => closure_1_11(showHint(4603).Text, { variant: "text-xs/medium", color: "text-feedback-critical", children }));
  return match.with(obj, (children) => closure_1_11(showHint(4603).Text, { variant: "text-xs/medium", color: "text-feedback-critical", children })).with(obj2, (children) => closure_1_11(showHint(4603).Text, { variant: "text-xs/medium", color: "text-feedback-positive", children })).otherwise(() => {
    let tmp = null;
    if (showHint) {
  ...
  onSubmitEditing = obj5.useCallback(() => {
    if (isMatch) {
      ...
      navigation = closure_4(function*(arg0) {
        ...
        closure_129_0 = yield closure_2_2(7058).saveAccountChanges(user, { close: false });
        if (!closure_129_0.ok) {
