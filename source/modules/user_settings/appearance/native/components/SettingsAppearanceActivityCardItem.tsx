...
// Dependencies: [19, 17, 2025, 15379, 21, 4373, 1178, 4636, 576, 563, 8940, 5668, 4632, 1880, 15380, 15381, 15382, 2]
// Exports: default

// Module 15378 (SettingsAppearanceActivityCardItem)
  ...
  if (null != avatars) {
    mapped = substr.map((source, index) => {
      ...
      if (index !== diff) {
        ...
        const point = { shape: tmp3(8940).CutoutShape.Circle, x: tmp3(1178).AVATAR_SIZE_MAP[tmp3(undefined, 1178).AvatarSizes.XSMALL_20] - 4 - 2, y: -2, size: tmp3(1178).AVATAR_SIZE_MAP[tmp3(undefined, 1178).AvatarSizes.XSMALL_20] + 4 };
        const items = [point];
        obj3.cutouts = items;
        obj3.children = tmp5;
