...
// Dependencies: [19, 17, 2025, 15375, 21, 4341, 1178, 4605, 576, 563, 8884, 5637, 4601, 1880, 15376, 15377, 15378, 2]
// Exports: default

// Module 15374 (SettingsAppearanceActivityCardItem)
  ...
  if (null != avatars) {
    mapped = substr.map((source, index) => {
      ...
      if (index !== diff) {
        ...
        const point = { shape: tmp3(8884).CutoutShape.Circle, x: tmp3(1178).AVATAR_SIZE_MAP[tmp3(undefined, 1178).AvatarSizes.XSMALL_20] - 4 - 2, y: -2, size: tmp3(1178).AVATAR_SIZE_MAP[tmp3(undefined, 1178).AvatarSizes.XSMALL_20] + 4 };
        const items = [point];
        obj3.cutouts = items;
        obj3.children = tmp5;
