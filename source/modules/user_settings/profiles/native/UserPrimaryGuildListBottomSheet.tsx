...
// Dependencies: [19, 17, 8019, 21, 4607, 1115, 576, 8251, 4325, 5657, 4574, 1114, 5636, 9276, 5741, 12, 7224, 4603, 8966, 8686, 2]
// Exports: default

// Module 14698 (UserPrimaryGuildListBottomSheet)
...
let closure_9 = noop.memo((item) => {
  ...
  if (guildTagBadgeUrl) {
    ...
    guildTagBadgeUrl = item(8251).getGuildTagBadgeUrl(item.id, badge, GuildTagBadgeSize.SIZE_24);
    const obj = item(8251);
  }
  const radioA11yNative = item(4325).useRadioA11yNative({ selected });
  ...
  if (null != item) {
    const obj4 = { style: tmp.guildIcon, guild: item, size: tmp7(5636).GuildIconSizes.SMALL_32 };
  ...
  if (tmp10Result2) {
    ...
    tmp10Result2 = tmp10(tmp7(9276).BaseGuildTagChiplet, obj10);
  }
  const items = [tmp10Result2, closure_6(item(5741).FormRadio, { selected })];
  obj5.children = items;
  obj3.trailing = closure_7(View, obj5);
  return closure_6(item(5657).TableRow, obj3);
});
const size = fn(2);
...
export default function UserPrimaryGuildListBottomSheet(availableGuilds) {
  ...
  obj2.children = closure_6(availableGuilds(4603).Text, obj3);
  obj.header = closure_6(memo, obj2);
  obj.children = closure_6(availableGuilds(8966).BottomSheetFlashList, {
  ...
  return closure_6(availableGuilds(7224).BottomSheet, obj);
};
