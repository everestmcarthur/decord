...
// Dependencies: [109, 5, 19, 8289, 7729, 1980, 5524, 1372, 1074, 504, 12014, 1953, 573, 9353, 14761, 11239, 7098, 14719, 8296, 4538, 1115, 2]
// Exports: default

// Module 14760 (useGuildProfileEditForm)
...
const IGNORE_GUILD_IDS = fn(8289).IGNORE_GUILD_IDS;
...
export default function useGuildProfileEditForm() {
  ...
  const memo = pendingThemeColors.useMemo(() => {
    const delayedCall = new stateFromStores(pendingNickname[11]).DelayedCall(200, () => {
      pendingAvatar(573).wait(stateFromStores(9353).resetAllPending);
    });
    return delayedCall;
  }, []);
  const items3 = [memo];
