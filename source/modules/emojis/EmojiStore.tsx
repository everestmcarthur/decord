...
// Function ID: 5546
// Name: EmojiStore
// Dependencies: [32, 5, 4553, 5546, 2026, 1220, 2022, 1960, 2016, 1980, 5524, 1372, 5547, 5548, 1375, 1074, 5549, 1084, 4290, 5550, 11, 12, 1091, 1987, 2007, 10, 5551, 573, 4675, 38, 4625, 1370, 5552, 5553, 5585, 4267, 4229, 504, 4294, 4293, 2]

// Module 5545 (EmojiStore)
...
import dedupeEmojisByNameOrIdDefault from "dedupeEmojisByNameOrId" /* 5552 */;
import EmojiTermsDefault from "EmojiTerms" /* 5553 */;
...
import UnicodeEmojis from "UnicodeEmojis" /* 4290 */;
import SnowflakeUtils from "SnowflakeUtils" /* 11 */;
import Frecency_mod from "Frecency" /* 4675 */;
        ...
        if (arg0 === 1) {
          ...
          if (Loading === __initData8.Unloaded) {
            ...
            if (null != databaseResult) {
              ...
              c2 = 1;
              c3 = 1;
              const obj6 = { value: React(2007).tryLoadOrResetCacheGatewayAsync("EmojiStore.loadSavedEmojis", async () => closure_1(10).timeAsync("\u{1F4BE}", "loadSavedEmojis", async () => closure_2_1(5551).getAsync(closure_1_0))), done: false };
              return obj6;
            }
          }
          c3 = 3;
...
function updateGuildEmoji(guildId) {
  ...
  if (null != guildEmojis) {
    ...
    if (null != currentUser) {
      let flag = closure_0(5585).canUseRoleSubscriptionIAP(guildId);
...
prototype["isUsable"] = function isUsable(emoji) {
  if (0 === emoji.roles.length) {
    ...
    if (tmp6) {
      ...
      if (!someResult) {
        let result = emoji(5550).isPurchasableRoleSubscriptionEmoji(emoji);
        if (result) {
...
prototype3["getSearchResultsOrder"] = function getSearchResultsOrder(locked, query, count, intention) {
  ...
  const escapeResult = formatted(4625).escape(formatted);
...
prototype3["searchWithoutFetchingLatest"] = function searchWithoutFetchingLatest(channel) {
  ...
  const reduced = value.nameMatchesChain(matchComparator).reduce((locked, emoji) => {
    ...
    if (!tmp5) {
      ...
      if (_slicedToArray) {
        tmp6 = emoji.type !== channel(4293).EmojiTypes.UNICODE;
      }
      tmp5 = tmp6;
    }
    if (!tmp5) {
...
const emojiStore = new EmojiStore(DispatcherDefault, {
  ...
  TOP_EMOJIS_FETCH_SUCCESS: function handleTopEmojisLoaded(topEmojisMetadata) {
    ...
    const tmpResult = _modDef4229(_modDef4229());
    obj.topEmojisTTL = _modDef4229(_modDef4229()).add(1, "days").valueOf();
