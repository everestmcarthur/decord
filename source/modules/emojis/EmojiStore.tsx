...
// Function ID: 5474
// Name: EmojiStore
// Dependencies: [32, 5, 4490, 5474, 2025, 1221, 2021, 1959, 2015, 1979, 5452, 1371, 5475, 5476, 1374, 1074, 5477, 1084, 4226, 5478, 11, 12, 1090, 1986, 2006, 10, 5479, 573, 4611, 38, 4561, 1369, 5480, 5481, 5513, 4203, 4166, 504, 4230, 4229, 2]

// Module 5473 (EmojiStore)
...
import EmojiTermsDefault from "EmojiTerms" /* 5481 */;
...
import UnicodeEmojis from "UnicodeEmojis" /* 4226 */;
import SnowflakeUtils from "SnowflakeUtils" /* 11 */;
import Frecency_mod from "Frecency" /* 4611 */;
        ...
        if (arg0 === 1) {
          ...
          if (Loading === __initData8.Unloaded) {
            ...
            if (null != databaseResult) {
              ...
              c2 = 1;
              c3 = 1;
              const obj6 = { value: React(2006).tryLoadOrResetCacheGatewayAsync("EmojiStore.loadSavedEmojis", async () => closure_1(10).timeAsync("\u{1F4BE}", "loadSavedEmojis", async () => closure_2_1(5479).getAsync(closure_1_0))), done: false };
              return obj6;
            }
          }
          c3 = 3;
...
function updateGuildEmoji(guildId) {
  ...
  if (null != guildEmojis) {
    ...
    if (null != currentUser) {
      let flag = closure_0(5513).canUseRoleSubscriptionIAP(guildId);
...
prototype["isUsable"] = function isUsable(emoji) {
  if (0 === emoji.roles.length) {
    ...
    if (tmp6) {
      ...
      if (!someResult) {
        let result = emoji(5478).isPurchasableRoleSubscriptionEmoji(emoji);
        if (result) {
...
prototype3["getSearchResultsOrder"] = function getSearchResultsOrder(locked, query, count, intention) {
  ...
  const escapeResult = formatted(4561).escape(formatted);
...
prototype3["searchWithoutFetchingLatest"] = function searchWithoutFetchingLatest(channel) {
  ...
  const reduced = value.nameMatchesChain(matchComparator).reduce((locked, emoji) => {
    ...
    if (!tmp5) {
      ...
      if (_slicedToArray) {
        tmp6 = emoji.type !== channel(4229).EmojiTypes.UNICODE;
      }
      tmp5 = tmp6;
    }
    if (!tmp5) {
...
const emojiStore = new EmojiStore(DispatcherDefault, {
  ...
  TOP_EMOJIS_FETCH_SUCCESS: function handleTopEmojisLoaded(topEmojisMetadata) {
    ...
    const tmpResult = _modDef4166(_modDef4166());
    obj.topEmojisTTL = _modDef4166(_modDef4166()).add(1, "days").valueOf();
