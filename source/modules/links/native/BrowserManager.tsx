...
// Function ID: 4540
// Name: BrowserManager
// Dependencies: [17, 560, 1115, 4540, 1093, 4268, 1369, 2]
...
// Module 4539 (BrowserManager)
import ConstantsIOS from "ConstantsIOS" /* 1093 */;
import PlatformUtils from "PlatformUtils" /* 1115 */;
import LinkingDefault from "Linking" /* 4268 */;
import NativeBrowserManagerModuleDefault from "NativeBrowserManagerModule" /* 4540 */;
...
export const browserManagerSelectBrowser = function browserManagerSelectBrowser(selectedBrowser) {
  if (obj.isAndroid()) {
    if (tmp(1093).WebBrowserType.SAFARI === selectedBrowser) {
      const browser = NativeBrowserManagerModuleDefault.selectBrowser(tmp(4540).BrowserType.SAFARI);
    } else if (tmp(1093).WebBrowserType.IN_APP === selectedBrowser) {
      const browser1 = NativeBrowserManagerModuleDefault.selectBrowser(tmp(4540).BrowserType.IN_APP);
    } else if (tmp(1093).WebBrowserType.CHROME === selectedBrowser) {
      const browser2 = NativeBrowserManagerModuleDefault.selectBrowser(tmp(4540).BrowserType.CHROME);
    }
  } else {
