...
let items = [
  {
    orderCollapsed: 0,
    isIncluded: true,
    IconComponent: fn(8883).ReactionIcon,
    getCopy() {
...
items[1] = {
  isIncluded: true,
  IconComponent: fn(10242).StickerIcon,
  getCopy() {
    const intl = util.intl;
...
items[3] = {
  orderCollapsed: 2,
  isIncluded: true,
  IconComponent: fn(5182).VoiceNormalIcon,
  getCopy() {
    const intl = util.intl;
...
items[4] = {
  isIncluded: true,
  IconComponent: fn(5178).StageIcon,
  getCopy() {
    const intl = util.intl;
...
items[7] = {
  isIncluded: false,
  IconComponent: fn(9501).UploadIcon,
  getCopy() {
    const intl = util.intl;
...
items[10] = {
  isIncluded: false,
  IconComponent: fn(4579).LinkIcon,
  getCopy() {
    const intl = util.intl;
...
let closure_16 = noop.forwardRef((onCardPress, ref) => {
  ...
  if (!tmp16) {
    ...
    if (tmp11Result) {
      ...
      const tmp2Result4 = tmp2(5068);
      const items7 = [tmp7(4486).hexWithOpacity(tmp9, 0), , ];
      const tmp7Result7 = tmp7(4486);
      items7[1] = tmp7(4486).hexWithOpacity(tmp9, 1);
      const tmp7Result8 = tmp7(4486);
      items7[2] = tmp7(4486).hexWithOpacity(tmp9, 0);
      obj15.colors = items7;
      obj15.start = { x: 0, y: 0 };
      obj15.end = { x: 1, y: 0 };
      ...
      const tmp7Result9 = tmp7(4486);
      const tmp2Result5 = tmp2(5068);
      const items10 = [tmp7(4486).hexWithOpacity(tmp9, 0), , ];
      const tmp7Result10 = tmp7(4486);
      items10[1] = tmp7(4486).hexWithOpacity(tmp9, 1);
      const tmp7Result11 = tmp7(4486);
      items10[2] = tmp7(4486).hexWithOpacity(tmp9, 0);
    ...
    obj22 = tmp13(tmp7(4632).Text, obj22);
    obj21.children = obj22;
    tmp13(tmp2(5068), obj21);
    const tmp2Result6 = tmp2(5068);
  }
});
const size = fn(2);
