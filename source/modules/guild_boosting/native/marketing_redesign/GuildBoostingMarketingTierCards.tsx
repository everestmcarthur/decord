...
// Dependencies: [32, 19, 17, 1074, 1374, 21, 8897, 1115, 10252, 12675, 5186, 5182, 10521, 13690, 9514, 4580, 13692, 4638, 576, 13684, 5527, 4374, 4639, 4634, 4572, 4489, 5072, 5208, 4531, 13693, 13695, 4487, 1177, 13697, 13698, 5667, 12707, 2]
// Exports: default

// Module 13689 (GuildBoostingMarketingTierCards)
...
import ServerBoostStreamQualityMarketingExperiment from "ServerBoostStreamQualityMarketingExperiment" /* 13692 */;
...
let items = [
  {
    orderCollapsed: 0,
    isIncluded: true,
    IconComponent: fn(8897).ReactionIcon,
    getCopy() {
...
items[1] = {
  isIncluded: true,
  IconComponent: fn(10252).StickerIcon,
  getCopy() {
    const intl = util.intl;
...
items[2] = {
  isIncluded: true,
  IconComponent: fn(12675).ScreenArrowIcon,
  getCopy() {
    const intl = util.intl;
...
items[3] = {
  orderCollapsed: 2,
  isIncluded: true,
  IconComponent: fn(5186).VoiceNormalIcon,
  getCopy() {
    const intl = util.intl;
...
items[4] = {
  isIncluded: true,
  IconComponent: fn(5182).StageIcon,
  getCopy() {
    const intl = util.intl;
...
items[5] = {
  orderCollapsed: 1,
  isIncluded: true,
  IconComponent: fn(10521).GifIcon,
  getCopy() {
    const intl = util.intl;
...
items[6] = {
  isIncluded: true,
  IconComponent: fn(13690).ServerGridIcon,
  getCopy() {
    const intl = util.intl;
...
items[7] = {
  isIncluded: false,
  IconComponent: fn(9514).UploadIcon,
  getCopy() {
    const intl = util.intl;
...
items[10] = {
  isIncluded: false,
  IconComponent: fn(4580).LinkIcon,
  getCopy() {
    const intl = util.intl;
...
obj37.cardsScroller = { flex: 1, marginTop: fn(13684).PROGRESS_BAR_SPACING };
...
const obj41 = { flex: 1, marginTop: fn(13684).PROGRESS_BAR_SPACING };
obj37.sparkleStar = { position: "absolute", tintColor: fn(5527).DARK_WHITE_500_LIGHT_GUILD_BOOSTING_PINK };
...
let closure_16 = noop.forwardRef((onCardPress, ref) => {
  ...
  if (isExpanded) {
    let ChevronLargeDownIcon = tmp7(13693).ChevronLargeUpIcon;
  } else {
    ChevronLargeDownIcon = tmp7(13695).ChevronLargeDownIcon;
  }
  ...
  if (!tmp16) {
    ...
    if (tmp11Result) {
      ...
      const tmp2Result4 = tmp2(5072);
      const items7 = [tmp7(4487).hexWithOpacity(tmp9, 0), , ];
      const tmp7Result7 = tmp7(4487);
      items7[1] = tmp7(4487).hexWithOpacity(tmp9, 1);
      const tmp7Result8 = tmp7(4487);
      items7[2] = tmp7(4487).hexWithOpacity(tmp9, 0);
      obj15.colors = items7;
      obj15.start = { x: 0, y: 0 };
      obj15.end = { x: 1, y: 0 };
      ...
      const tmp7Result9 = tmp7(4487);
      const tmp2Result5 = tmp2(5072);
      const items10 = [tmp7(4487).hexWithOpacity(tmp9, 0), , ];
      const tmp7Result10 = tmp7(4487);
      items10[1] = tmp7(4487).hexWithOpacity(tmp9, 1);
      const tmp7Result11 = tmp7(4487);
      items10[2] = tmp7(4487).hexWithOpacity(tmp9, 0);
    ...
    obj22 = tmp13(tmp7(4634).Text, obj22);
    obj21.children = obj22;
    tmp13(tmp2(5072), obj21);
    const tmp2Result6 = tmp2(5072);
  }
});
const size = fn(2);
