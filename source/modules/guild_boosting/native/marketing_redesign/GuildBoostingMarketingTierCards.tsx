...
// Dependencies: [32, 19, 17, 1074, 1374, 21, 8886, 1115, 10244, 12667, 5185, 5181, 10513, 13681, 9503, 4582, 13683, 4639, 576, 13675, 5526, 4376, 4640, 4635, 4574, 4491, 5071, 5207, 4533, 13684, 13686, 4489, 1177, 13688, 13689, 5666, 12699, 2]
// Exports: default

// Module 13680 (GuildBoostingMarketingTierCards)
...
import ServerBoostStreamQualityMarketingExperiment from "ServerBoostStreamQualityMarketingExperiment" /* 13683 */;
...
let items = [
  {
    orderCollapsed: 0,
    isIncluded: true,
    IconComponent: fn(8886).ReactionIcon,
    getCopy() {
...
items[1] = {
  isIncluded: true,
  IconComponent: fn(10244).StickerIcon,
  getCopy() {
    const intl = util.intl;
...
items[2] = {
  isIncluded: true,
  IconComponent: fn(12667).ScreenArrowIcon,
  getCopy() {
    const intl = util.intl;
...
items[3] = {
  orderCollapsed: 2,
  isIncluded: true,
  IconComponent: fn(5185).VoiceNormalIcon,
  getCopy() {
    const intl = util.intl;
...
items[4] = {
  isIncluded: true,
  IconComponent: fn(5181).StageIcon,
  getCopy() {
    const intl = util.intl;
...
items[5] = {
  orderCollapsed: 1,
  isIncluded: true,
  IconComponent: fn(10513).GifIcon,
  getCopy() {
    const intl = util.intl;
...
items[6] = {
  isIncluded: true,
  IconComponent: fn(13681).ServerGridIcon,
  getCopy() {
    const intl = util.intl;
...
items[7] = {
  isIncluded: false,
  IconComponent: fn(9503).UploadIcon,
  getCopy() {
    const intl = util.intl;
...
obj37.cardsScroller = { flex: 1, marginTop: fn(13675).PROGRESS_BAR_SPACING };
...
const obj41 = { flex: 1, marginTop: fn(13675).PROGRESS_BAR_SPACING };
obj37.sparkleStar = { position: "absolute", tintColor: fn(5526).DARK_WHITE_500_LIGHT_GUILD_BOOSTING_PINK };
...
let closure_16 = noop.forwardRef((onCardPress, ref) => {
  ...
  if (isExpanded) {
    let ChevronLargeDownIcon = tmp7(13684).ChevronLargeUpIcon;
  } else {
    ChevronLargeDownIcon = tmp7(13686).ChevronLargeDownIcon;
  }
  ...
  if (!tmp16) {
    ...
    obj22 = tmp13(tmp7(4635).Text, obj22);
    obj21.children = obj22;
    tmp13(tmp2(5071), obj21);
    const tmp2Result6 = tmp2(5071);
  }
});
const size = fn(2);
