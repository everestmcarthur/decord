...
// Dependencies: [109, 9705, 1962, 1958, 4547, 2013, 2012, 1074, 9706, 3, 4561, 13025, 5581, 1101, 13030, 4925, 4929, 5690, 9926, 4579, 573, 4923, 4652, 13031, 2]
...
// Module 13024 (RouteManagerUtils)
...
import DefaultRouteActionCreators from "DefaultRouteActionCreators" /* 13030 */;
...
function updateSelectedChannelListener(location) {
  const matchPathResult = channel2(13025).matchPath(location.pathname, { path: items, strict: false, exact: false });
  let params;
  if (matchPathResult != null) {
  ...
  if (null == voiceChannelId2) {
    if (null == voiceGuildId2) {
      ...
      if (isGuildVoiceResult) {
        ...
        obj21.popWithKey(tmp(4929).getVoiceChannelKey(channel.id));
...
function extractParams(arg0) {
  ...
  if (null == match) {
    ...
    if (_location.jumpType === Client.JumpType.INSTANT) {
      let ANIMATED = tmp5(4652).JumpType.INSTANT;
    } else {
      ANIMATED = tmp5(4652).JumpType.ANIMATED;
    }
