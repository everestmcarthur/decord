...
// Dependencies: [1074, 4498, 4495, 4496, 4464, 4477, 2]
// Exports: convertRouteToNavigation

// Module 12934 (convertRouteToNavigation)
  ...
  if (null != rootNavigationRef) {
    if (rootNavigationRef.isReady()) {
      if (pathname.startsWith("/channels/")) {
        ...
        const RouteParam = tmp(4477).RouteParam;
        const obj5 = Routes;
        const tmpResult = tmp(4464);
        const RouteParam2 = tmp(4477).RouteParam;
        const guildIdResult = RouteParam.guildId();
        const RouteParam3 = tmp(4477).RouteParam;
        const CHANNELResult = Routes.CHANNEL(RouteParam.guildId(), RouteParam2.channelId({ optional: true }));
        const RouteParam4 = tmp(4477).RouteParam;
        ...
        if (null != tmpResult.matchPath(pathname, obj2)) {
          ...
          const RouteParam6 = tmp(4477).RouteParam;
          const tmpResult15 = tmp(4464);
          const RouteParam7 = tmp(4477).RouteParam;
              ...
              if (null != channelId) {
                if (false === navigationReplace) {
                  ...
                  if (channelId1 === channelId) {
                    ...
                    tmp(4495).navigateToRootTab(obj4);
                    const tmpResult19 = tmp(4495);
                  } else {
                    const obj6 = { channelId, guildId, messageId, replaceChannelAndFixRoot: navigationReplace };
                    tmp(4495).navigateToChannel(obj6);
                    const tmpResult20 = tmp(4495);
                  }
                  const tmpResult18 = tmp(4496);
                }
              }
              const obj7 = { screen: "guilds", guildId, channelId, resetRoot: navigationReplace };
              tmp(4495).navigateToRootTab(obj7);
              const tmpResult21 = tmp(4495);
      ...
      if (pathname.startsWith("/member-verification/")) {
        ...
        const RouteParam5 = tmp(4477).RouteParam;
        obj12.path = Routes.GUILD_MEMBER_VERIFICATION(RouteParam5.guildId());
        const matchPathResult1 = tmp(4464).matchPath(pathname, obj12);
        if (null != matchPathResult1) {
          const result = tmp(4495).navigateToMemberVerification(matchPathResult1.params.guildId, matchPathResult1.params.inviteCode);
          const tmpResult27 = tmp(4495);
        }
        return true;
      } else {
        ...
        tmp(4495).resetToAuthRoute();
        flag = true;
        const tmpResult28 = tmp(4495);
      }
    }
  }
  return true;
