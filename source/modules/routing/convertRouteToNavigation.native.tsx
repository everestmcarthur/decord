...
// Dependencies: [1074, 4497, 4494, 4495, 4463, 4476, 2]
// Exports: convertRouteToNavigation

// Module 12921 (convertRouteToNavigation)
  ...
  if (null != rootNavigationRef) {
    if (rootNavigationRef.isReady()) {
      if (pathname.startsWith("/channels/")) {
        ...
        const RouteParam4 = tmp(4476).RouteParam;
        ...
        if (null != tmpResult.matchPath(pathname, obj2)) {
          ...
          const RouteParam6 = tmp(4476).RouteParam;
          const tmpResult15 = tmp(4463);
          const RouteParam7 = tmp(4476).RouteParam;
            ...
            if (tmpResult16.getChatLayout().isChatLockedOpen) {
              if (null != channelId) {
                if (false === navigationReplace) {
                  ...
                  const rootNavigationRef1 = tmp(4495).getRootNavigationRef();
                  let currentRoute;
                  ...
                  if (channelId1 === channelId) {
                    ...
                    tmp(4494).navigateToRootTab(obj4);
                    const tmpResult19 = tmp(4494);
                  } else {
                    const obj6 = { channelId, guildId, messageId, replaceChannelAndFixRoot: navigationReplace };
                    tmp(4494).navigateToChannel(obj6);
                    const tmpResult20 = tmp(4494);
                  }
                  const tmpResult18 = tmp(4495);
                }
              }
              const obj7 = { screen: "guilds", guildId, channelId, resetRoot: navigationReplace };
              tmp(4494).navigateToRootTab(obj7);
              const tmpResult21 = tmp(4494);
      ...
      if (pathname.startsWith("/member-verification/")) {
        ...
        const RouteParam5 = tmp(4476).RouteParam;
        obj12.path = Routes.GUILD_MEMBER_VERIFICATION(RouteParam5.guildId());
        const matchPathResult1 = tmp(4463).matchPath(pathname, obj12);
        if (null != matchPathResult1) {
          const result = tmp(4494).navigateToMemberVerification(matchPathResult1.params.guildId, matchPathResult1.params.inviteCode);
          const tmpResult27 = tmp(4494);
        }
        return true;
      } else {
        ...
        tmp(4494).resetToAuthRoute();
        flag = true;
        const tmpResult28 = tmp(4494);
      }
    }
  }
  return true;
