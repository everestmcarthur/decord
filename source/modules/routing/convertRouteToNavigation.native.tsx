...
// Dependencies: [1074, 4465, 4462, 4463, 4431, 4444, 2]
  ...
  if (null != rootNavigationRef) {
    if (rootNavigationRef.isReady()) {
      if (pathname.startsWith("/channels/")) {
        ...
        const RouteParam4 = tmp(4444).RouteParam;
        ...
        if (null != tmpResult.matchPath(pathname, obj2)) {
          ...
          const RouteParam6 = tmp(4444).RouteParam;
          const tmpResult15 = tmp(4431);
          const RouteParam7 = tmp(4444).RouteParam;
            ...
            if (tmpResult16.getChatLayout().isChatLockedOpen) {
              if (null != channelId) {
                if (false === navigationReplace) {
                  ...
                  const rootNavigationRef1 = tmp(4463).getRootNavigationRef();
                  let currentRoute;
                  ...
                  if (channelId1 === channelId) {
                    ...
                    tmp(4462).navigateToRootTab(obj4);
                    const tmpResult19 = tmp(4462);
                  } else {
                    const obj6 = { channelId, guildId, messageId, replaceChannelAndFixRoot: navigationReplace };
                    tmp(4462).navigateToChannel(obj6);
                    const tmpResult20 = tmp(4462);
                  }
                  const tmpResult18 = tmp(4463);
                }
              }
              const obj7 = { screen: "guilds", guildId, channelId, resetRoot: navigationReplace };
              tmp(4462).navigateToRootTab(obj7);
              const tmpResult21 = tmp(4462);
      ...
      if (pathname.startsWith("/member-verification/")) {
        ...
        const RouteParam5 = tmp(4444).RouteParam;
        obj12.path = Routes.GUILD_MEMBER_VERIFICATION(RouteParam5.guildId());
        const matchPathResult1 = tmp(4431).matchPath(pathname, obj12);
        if (null != matchPathResult1) {
          const result = tmp(4462).navigateToMemberVerification(matchPathResult1.params.guildId, matchPathResult1.params.inviteCode);
          const tmpResult27 = tmp(4462);
        }
        return true;
      } else {
        ...
        tmp(4462).resetToAuthRoute();
        flag = true;
        const tmpResult28 = tmp(4462);
      }
    }
  }
  return true;
