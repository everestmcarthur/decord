...
// Dependencies: [19, 17, 21, 4639, 576, 9058, 9146, 4635, 2]
// Exports: default
...
export default function UserProfileApplicationWidgetBottomProgressLayout(arg0) {
  ...
  if (null != fieldValue) {
    ...
    tmp14 = React4(tmp3(9146).ImageSkeleton, obj6);
    tmp15 = React4;
  }
  const items3 = [tmp14, ];
  ...
  if ("value" === singleStringOrSkeleton.status) {
    ...
    let tmp15Result = tmp15(tmp3(4635).Text, obj13);
  } else {
    tmp15Result = tmp15(tmp3(9146).TextSkeleton, { variant: "heading-sm/medium" });
  }
  ...
  if ("value" === singleStringOrSkeleton1.status) {
    ...
    let tmp15Result3 = tmp15(tmp3(4635).Text, obj14);
  } else {
    tmp15Result3 = tmp15(tmp3(9146).TextSkeleton, { variant: "text-xs/medium" });
  }
  ...
  if (null != iter) {
    ...
    if (null != iter2) {
      ...
      iter2Result = "" + tmp3(9058).decimalToClampedPercentage(iter.value) + "%";
      const tmp3Result2 = tmp3(9058);
    }
    obj15.children = iter2Result;
    tmp15(tmp3(4635).Text, obj15);
  } else {
    items7[1] = tmp15(tmp3(9146).TextSkeleton, { variant: "text-sm/medium", widthChars: 4 });
