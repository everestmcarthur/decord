...
// Module 16737 (VibegrationsTimelineTree)
...
let obj = { assembling: _modDef3591["9G3ZmA"], installing_deps: _modDef3591.VZuRug, building: _modDef3591["s+ylXP"], bundling: _modDef3591.rEQlMx, committing: _modDef3591.Yvyw1H, built: _modDef3591.hOcVAj, build_error: _modDef3591["6L9Vwt"], healthchecking: _modDef3591.sz8yAj, awaiting_auth: _modDef3591["orD+xo"], healthcheck_passed: _modDef3591["x+sqTG"], healthcheck_failed: _modDef3591.FUWbq1, deploying: _modDef3591.wcXX8Z, preview_ready: _modDef3591["78YNh7"], working: _modDef3591.nv6pUM, error: _modDef3591.j3hBoA };
let size = fn(2);
...
export const describeTaskStatus = function describeTaskStatus(status) {
  if ("running" === status) {
    ...
    return intl5.string(_modDef3591["fW7T+d"]);
  } else if ("done" === status) {
    const intl4 = util.intl;
    return intl4.string(_modDef3591.X3c4hc);
  } else if ("failed" === status) {
    const intl3 = util.intl;
    return intl3.string(_modDef3591.LK4Wsd);
  } else if ("cancelled" === status) {
    const intl2 = util.intl;
    return intl2.string(_modDef3591.msWvKA);
  } else if ("incomplete" === status) {
    const intl = util.intl;
    return intl.string(_modDef3591.esfcU6);
  }
};
