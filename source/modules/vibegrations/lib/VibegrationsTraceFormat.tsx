...
// Module 16803 (vibegrations/VibegrationsTraceFormat)
...
export const categoryLabel = function categoryLabel(traceCategoryResult) {
  if ("subagent" === traceCategoryResult) {
    ...
    return intl5.string(_modDef3591["EoY7D+"]);
  } else if ("context" === traceCategoryResult) {
    const intl4 = util.intl;
    return intl4.string(_modDef3591.KVFrD3);
  } else if ("tool" === traceCategoryResult) {
    const intl3 = util.intl;
    return intl3.string(_modDef3591["/N6ZU9"]);
  } else if ("delegated" === traceCategoryResult) {
    const intl2 = util.intl;
    return intl2.string(_modDef3591.HcEbf2);
  } else {
    const intl = util.intl;
    return intl.string(_modDef3591.AhOqQs);
  }
};
export const statusLabel = function statusLabel(status) {
  if ("started" === status) {
    const intl3 = util.intl;
    return intl3.string(_modDef3591.HpKDyl);
  } else if ("error" === status) {
    const intl2 = util.intl;
    return intl2.string(_modDef3591["5T4Dd0"]);
  } else {
    const intl = util.intl;
    return intl.string(_modDef3591.VbEmf0);
  }
};
export const omissionLabel = function omissionLabel(content) {
  if ("prose" === content) {
    const intl3 = util.intl;
    return intl3.string(_modDef3591.xO6bcQ);
  } else if ("content" === content) {
    const intl2 = util.intl;
    return intl2.string(_modDef3591.gpBZRr);
  } else {
    const intl = util.intl;
    return intl.string(_modDef3591.OZvPXt);
  }
};
    ...
    if ("loaded" !== vibegrationsTraceDetail.status) {
      ...
      if ("forbidden" !== vibegrationsTraceDetail.status) {
        ...
        if ("loading" === vibegrationsTraceDetail.status) {
          ...
          fj5wM8 = _modDef3591.jEQTot;
        } else {
          fj5wM8 = _modDef3591.fj5wM8;
        }
        intl.string(fj5wM8);
      }
    }
