...
// Dependencies: [32, 19, 502, 504, 16886, 16887, 9247, 7359, 9744, 2]
// Exports: useVibegrationsPreviewMode

// Module 16885 (useVibegrationsPreviewMode)
import initialize from "initialize" /* 504 */;
import useUserApplicationWidgetDataDefault from "useUserApplicationWidgetData" /* 16886 */;
import vibegrationsPreviewModes from "vibegrationsPreviewModes" /* 16887 */;
...
export const useVibegrationsPreviewMode = function useVibegrationsPreviewMode(arg0) {
  ...
  let tmp14;
  if (surfaces != null) {
    tmp14 = surfaces[tmp8(undefined, 9247).ApplicationWidgetConfigSurface.WIDGET_TOP];
  }
  const obj2 = { widgetTop: null != tmp14, widgetBottom: null, miniProfile: null };
  let tmp15;
  if (surfaces != null) {
    tmp15 = surfaces[tmp8(undefined, 9247).ApplicationWidgetConfigSurface.WIDGET_BOTTOM];
  }
  obj2.widgetBottom = null != tmp15;
  let tmp16;
  if (surfaces != null) {
    tmp16 = surfaces[tmp8(undefined, 9247).ApplicationWidgetConfigSurface.MINI_PROFILE];
  }
  obj2.miniProfile = null != tmp16;
  const result = vibegrationsPreviewModes.profileSurfaceAvailability(obj2);
  if (null == tmp7) {
    const data = tmp8(7359).useApplication(previewApplicationId).data;
    ...
    const tmp8Result6 = tmp8(7359);
    const application = tmp8(7359).useApplication(applicationId);
    if (!declaredActivity) {
      declaredActivity = tmp8(9744).canLaunchFrame(tmp25);
      const tmp8Result8 = tmp8(9744);
    }
    const tmp8Result7 = tmp8(7359);
    const obj3 = { installScope, hasFrame: declaredActivity, hasProfileWidget: tmp18, hasBotDm: tmp21, ownerAuthorizationRevoked };
    const result1 = tmp8(16887).previewModeAvailability(obj3);
    ...
    if (!(null != applicationId && application.isLoading)) {
      previewMode = tmp8(16887).resolvePreviewMode(tmp2, result1);
      const tmp8Result10 = tmp8(16887);
    }
