...
// Dependencies: [32, 19, 17, 1896, 16753, 16752, 21, 576, 16793, 672, 4640, 16795, 16800, 1115, 3593, 5060, 4636, 4630, 16801, 5062, 5695, 4334, 16803, 16804, 16805, 16806, 1364, 5752, 5074, 16808, 16799, 16796, 16089, 16809, 16810, 16812, 16814, 504, 1612, 16815, 16816, 16817, 16818, 16819, 8873, 16822, 16824, 16825, 16826, 16828, 2]
// Exports: default

// Module 16792 (VibegrationsNativeChat)
...
import VibegrationsNativeStatusLineDefault from "VibegrationsNativeStatusLine" /* 16793 */;
import VibegrationsMessageActionSheet from "VibegrationsMessageActionSheet" /* 16799 */;
import useVibegrationsPlanDesign from "useVibegrationsPlanDesign" /* 16800 */;
import VibegrationsTimelineTree from "VibegrationsTimelineTree" /* 16803 */;
import VibegrationsSubagentMark from "VibegrationsSubagentMark" /* 16806 */;
import VibegrationsTodoAgents from "VibegrationsTodoAgents" /* 16808 */;
import VibegrationsChatGrouping from "VibegrationsChatGrouping" /* 16815 */;
import vibegrations_VibegrationsRepliedMessage from "vibegrations/VibegrationsRepliedMessage" /* 16817 */;
...
function PlanDesign(arg0) {
  ...
  if (vibegrationsPlanDesign.gone) {
    ...
    const stringResult = intl.string(_modDef3593.FW8UcU);
    const obj2 = { variant: "text-sm/semibold", color: "text-muted", children: null };
    const intl2 = tmp2(1115).intl;
    obj2.children = intl2.string(_modDef3593["9W8SbY"]);
    items = [closure_1_17(tmp2(4636).Text, obj2), ];
    if (null == src) {
    ...
    items[1] = tmp9Result;
    obj7.children = items;
    return collapsedCategories(tmp2(5060).Stack, obj7);
  }
}
function ProposalCard(proposal) {
  ...
  obj2.children = intl.string(_modDef3593["60htw+"]);
  items = [closure_1_17(Text_Text.Text, obj2), , , , , , ];
  if ("" === trimmed) {
    const intl2 = tmp6(1115).intl;
    let stringResult = intl2.string(tmp8(3593).IHCafX);
  } else {
    stringResult = tmp8(4630).parse(trimmed, true, tmp6(16801).VIBEGRATIONS_MARKUP_OPTIONS);
    const tmp8Result = tmp8(4630);
  }
  ...
  if (proposal.changes.length > 0) {
    ...
    obj5.children = intl3.string(tmp8(3593).KLyB8Y);
    const items1 = [tmp3(tmp6(4636).Text, obj5), ];
    ...
    tmp5Result = tmp5(tmp6(5060).Stack, obj4);
  }
  items[3] = tmp5Result;
  ...
  if (bot_permissions.length > 0) {
    ...
    obj7.children = intl4.string(tmp8(3593).ieqTtP);
    const items2 = [tmp3(tmp6(4636).Text, obj7), ];
    const obj8 = { variant: "text-sm/normal", color: "text-default", children: bot_permissions.join(", ") };
    items2[1] = tmp3(tmp6(4636).Text, obj8);
    obj6.children = items2;
    tmp5Result3 = tmp5(tmp6(5060).Stack, obj6);
  }
  items[4] = tmp5Result3;
  ...
  if (privileged_intents.length > 0) {
    ...
    obj10.children = intl5.string(tmp8(3593).Cn9qix);
    const items3 = [tmp3(tmp6(4636).Text, obj10), ];
    const obj11 = { variant: "text-sm/normal", color: "text-default", children: privileged_intents.join(", ") };
    items3[1] = tmp3(tmp6(4636).Text, obj11);
    obj9.children = items3;
    tmp5Result4 = tmp5(tmp6(5060).Stack, obj9);
  }
  items[5] = tmp5Result4;
  ...
  if (actionable) {
    ...
    obj12.text = intl6.string(tmp8(3593).GYoWRk);
    obj12.onPress = onApprove;
    tmp3Result2 = tmp3(tmp6(5062).Button, obj12);
  }
  items[6] = tmp3Result2;
...
function IdeaCards(arg0) {
  ...
  const obj2 = { variant: "text-sm/semibold", color: "text-muted", children: null };
  let intl = util.intl;
  obj2.children = intl.string(_modDef3593.DAvYsi);
  items = [
    ...
    ideas.map((title) => {
      ...
      const intl = util.intl;
      obj.accessibilityLabel = intl.formatToPlainString(_modDef3593.pztRGi, { title: title.title });
  ...
  return closure_17(closure_8, {
    ...
    children: attachments.map((id, index) => {
      if (null != id.id) {
        ...
        obj.accessibilityLabel = intl.formatToPlainString(closure_1(3593).QUFLUq, obj2);
        const obj3 = { variant: "text-xs/medium", color: "text-default", children: id.name };
        obj.children = closure_1_17(projectId(4636).Text, obj3);
        let tmp12 = closure_1_17(projectId(5695).Card, obj, id.id);
      } else {
        ...
        obj5.children = intl2.formatToPlainString(closure_1(3593).OBr7WW, obj6);
        obj4.children = closure_1_17(projectId(4636).Text, obj5);
...
function IdeasOffer(onAsk) {
  ...
  const intl = util.intl;
  obj3.children = intl.string(_modDef3593.tG5PBo);
  ...
  obj4.text = intl2.string(_modDef3593.cwTe5o);
...
function TimelineRow(live) {
  ...
  if (null != node.durationMs) {
    const obj3 = { variant: "text-xs/normal", color: "text-subtle", children: tmp7(16804).describeDuration(node.durationMs) };
    tmp4Result = tmp4(tmp7(4636).Text, obj3);
    const tmp7Result = tmp7(16804);
  }
...
function TurnStatusLine(epoch) {
  ...
  const currentStepResult = turnActive(16803).currentStep(tree.steps);
  ...
  if (null != tmp8) {
    groupLabel = tmp5(16804).describeTurnDuration(tmp8);
    const tmp5Result = tmp5(16804);
  } else if (null != currentStepResult) {
    groupLabel = tmp5(16803).describeNode(currentStepResult);
    const tmp5Result2 = tmp5(16803);
  } else if (groupLabel == null) {
    const intl = tmp5(1115).intl;
    groupLabel = intl.string(epoch(3593).nv6pUM);
  }
  ...
  let tmp19;
  let obj = turnActive(16803);
  const tmp15 = closure_18;
  if (someResult) {
    tmp19 = callback;
  }
  obj2.onToggle = tmp19;
  const children = [closure_17(epoch(16793), obj2), ];
...
function LaneStatusLine(arg0) {
  ...
  if (turnActive) {
    currentStepResult = mark(16803).currentStep(lane.steps);
    const obj = mark(16803);
  }
  _slicedToArray = currentStepResult;
  const tmp9 = lane.task.detail.length > 0 || lane.steps.length > 0;
  if ("running" === lane.task.status) {
    if (null != currentStepResult) {
      let describeNodeResult = mark(16803).describeNode(currentStepResult);
      const obj4 = mark(16803);
    } else {
      describeNodeResult = mark(16805).taskTitle(lane.task);
      const obj3 = mark(16805);
    }
  } else {
    const obj2 = mark(16805);
    const obj5 = { line: mark(16805).describeTaskOutcome(lane.task), live: turnActive, settled: null, failed: null, glyph: null, crestColor: null, inGutter: true, epoch: null, expanded: null, onToggle: null };
    let tmp26 = !turnActive;
    const describeTaskOutcomeResult = mark(16805).describeTaskOutcome(lane.task);
    if (!turnActive) {
    ...
    obj5.onToggle = tmp27;
    items = [closure_17(epoch(16793), obj5), ];
...
function ActivityBox(arg0) {
  ...
  const tmp = closure_24();
  const tasks = tree.tasks;
  dependencyMap = turnActive(16806).subagentIllocons(tasks.map((taskId) => taskId.taskId));
...
let diff = fn(16793).MESSAGE_CONTENT_INSET - fn(16793).MESSAGE_EDGE_INSET;
...
obj.row = { position: "relative", paddingLeft: fn(16793).MESSAGE_CONTENT_INSET, paddingRight: fn(16793).MESSAGE_EDGE_INSET, paddingVertical: 2, gap: PX_8 };
obj.rowGroupStart = { marginTop: nativeDefault.space.PX_12 };
const rect = { position: "absolute", left: fn(16793).MESSAGE_EDGE_INSET, top: 2 };
obj.avatar = rect;
obj.spoken = { position: "relative", gap: PX_8 };
obj.spokenAfterWork = { marginTop: PX_8 };
const rect1 = { left: fn(16793).MESSAGE_EDGE_INSET - fn(16793).MESSAGE_CONTENT_INSET, top: 0 };
obj.avatarSpoken = rect1;
let obj5 = { position: "relative", paddingLeft: fn(16793).MESSAGE_CONTENT_INSET, paddingRight: fn(16793).MESSAGE_EDGE_INSET, paddingVertical: 2, gap: PX_8 };
obj.avatarSpokenReplying = { top: fn(16795).REPLY_PREVIEW_HEIGHT + PX_8 };
let obj6 = { top: fn(16795).REPLY_PREVIEW_HEIGHT + PX_8 };
...
let closure_35 = noop.memo((onJumpToReplied) => {
  ...
  let tmp = closure_24();
  items = [message];
  const memo = trimmed.useMemo(() => {
    const obj = VibegrationsTimelineTree;
    return obj.buildTimelineTree(message.steps, { turnActive: !turnSettled(message) });
  }, items);
  const items1 = [message];
  const memo1 = trimmed.useMemo(() => VibegrationsTimelineTree.announcementNotes(message.steps), items1);
  const items2 = [message];
  const memo2 = trimmed.useMemo(() => VibegrationsTimelineTree.streamedMessages(message.steps), items2);
  const items3 = [message];
  const memo3 = trimmed.useMemo(() => VibegrationsTimelineTree.latestTodos(message.steps), items3);
  const items4 = [memo];
  const items5 = [onJumpToReplied, replied];
  const memo4 = trimmed.useMemo(() => VibegrationsTodoAgents.runningTodoAgents(memo.tasks), items4);
  const callback = trimmed.useCallback(() => {
    if (null != replied) {
  ...
  if ("" !== trimmed) {
    const callback1 = obj.useCallback(() => VibegrationsMessageActionSheet.showVibegrationsMessageActions({ content: trimmed, userId: user_id }), items7);
  }
  if ("user" === message.role) {
    if ("" !== trimmed) {
      ...
      if (tmp9) {
        let combined;
        if (!groupStart) {
          const intl3 = tmp87(tmp88[13]).intl;
          const _HermesInternal = HermesInternal;
          combined = "" + intl3.string(tmp87(tmp88[13]).t.KD6OJJ) + ": " + trimmed;
        }
    ...
    if (memo1.length > 0) {
      ...
      const items11 = [tmp39, memo1.map((children) => closure_1_17(message(onJumpToReplied[16]).Text, { variant: "text-md/normal", color: "text-default", children: children.content }, children.key))];
      obj19.children = items11;
      tmp24Result3 = tmp24(tmp33, obj19);
    }
    ...
    if (tmp23) {
      ...
      const items14 = [tmp43, memo2.map((content) => closure_1_17(replied(onJumpToReplied[18]), { source: content.content }, content.key)), , , , , , , , , , , ];
      let tmp32Result = null;
      if (tmp20) {
        const obj22 = { source: trimmed };
        tmp32Result = tmp32(replied(tmp35[18]), obj22);
      }
      ...
      if (null != items15) {
        ...
        obj25.children = tmp32(replied(tmp35[33]), obj26);
        let tmp32Result13 = tmp32(tmp33, obj25);
      ...
      if (null != found) {
        ...
        if ("message" in found) {
          ...
          tmp32Result20 = tmp32(tmp34(tmp35[16]).Text, obj65);
        }
      }
      items14[12] = tmp32Result20;
      obj21.children = items14;
      tmp24Result4 = tmp24(tmp33, obj21);
    }
...
export default function VibegrationsNativeChat(projectId) {
  ...
  let tmp = closure_24();
  items = [state];
  const stateFromStores = projectId(stateFromStores1[37]).useStateFromStores(items, () => "active" === state.getState(), []);
  ...
  stateFromStores1 = projectId(stateFromStores1[37]).useStateFromStores(items2, () => VibegrationsChatStore.getMessages(projectId), items3);
  const obj3 = projectId(stateFromStores1[37]);
  let tmp7 = closure_10;
  const items4 = [closure_10];
  const items5 = [projectId];
  const stateFromStores2 = projectId(stateFromStores1[37]).useStateFromStores(items4, () => VibegrationsChatStore.isThinking(projectId), items5);
  const obj4 = projectId(stateFromStores1[37]);
  const items6 = [closure_10];
  const items7 = [projectId];
  const stateFromStores3 = projectId(stateFromStores1[37]).useStateFromStores(items6, () => VibegrationsChatStore.isCompacting(projectId), items7);
  const obj5 = projectId(stateFromStores1[37]);
  const items8 = [closure_10];
  const items9 = [projectId];
  const stateFromStores4 = projectId(stateFromStores1[37]).useStateFromStores(items8, () => VibegrationsChatStore.getThinkingActivity(projectId), items9);
  const obj6 = projectId(stateFromStores1[37]);
  const items10 = [closure_10];
  const items11 = [projectId];
  const stateFromStores5 = projectId(stateFromStores1[37]).useStateFromStores(items10, () => VibegrationsChatStore.getProjectUsage(projectId), items11);
  ...
  const stateFromStores6 = projectId(stateFromStores1[37]).useStateFromStores(items13, () => VibegrationsConnectionStore.getConnState(projectId), items14);
  const tmp2Result = projectId(stateFromStores1[37]);
  const items15 = [ref];
  const items16 = [projectId];
  const stateFromStores7 = projectId(stateFromStores1[37]).useStateFromStores(items15, () => VibegrationsConnectionStore.isChatStopped(projectId), items16);
  const tmp2Result7 = projectId(stateFromStores1[37]);
  const items17 = [tmp7];
  const items18 = [projectId];
  render_id = null;
  const stateFromStores8 = projectId(stateFromStores1[37]).useStateFromStores(items17, () => VibegrationsChatStore.hasLoadedHistory(projectId), items18);
  ...
  onApprove = obj2.useCallback(() => {
    const intl = util.intl;
    __initData(projectId, intl.string(_modDef3593.Jj8Ftb));
  }, items20);
  ...
  closure_8 = obj2.useCallback(() => {
    const intl = util.intl;
    __initData(projectId, intl.string(_modDef3593["3sTTBu"]));
  }, items22);
  ...
  tmp2Result9 = projectId(stateFromStores1[26]);
  [tmp40, c18] = render_id.useState(false);
  ...
  callback2 = obj2.useCallback(() => {
    ...
    if (tmp5) {
      tmp5 = tmp.y >= current2.offsetY + current2.viewportHeight - ref3.current;
    }
    _undefined(tmp5);
  }, []);
  ...
  const callback4 = obj2.useCallback((arg0, contentHeight) => {
    ...
    if (null != current) {
      ...
      if (current.contentHeight - current.offsetY - current.viewportHeight <= 0.2 * current.viewportHeight) {
        const _Math = Math;
        obj.offsetY = Math.max(0, contentHeight - current.viewportHeight);
        let tmp2 = obj;
      } else {
        tmp2 = obj;
      }
      ...
      if (current.contentHeight - current.offsetY - current.viewportHeight <= 0.2 * current.viewportHeight) {
        const current2 = ref.current;
        if (current2 != null) {
          current2.scrollToEnd({ animated: false });
        }
      }
      callback2();
    }
  ...
  const memo3 = obj2.useMemo(() => ({ itemVisiblePercentThreshold: projectId(stateFromStores1[40]).MIN_VISIBLE_PERCENT }), []);
  ...
  if ("open" !== stateFromStores6) {
    ...
    const tmp2Result10 = tmp2(tmp3[42]);
  }
  const obj8 = { style: tmp.container, children: null };
  const items38 = [ref(stateFromStores(stateFromStores1[43]), { thinking: stateFromStores2, bleedBottom: stateFromStores(stateFromStores1[38])().bottom }), , ];
  ...
  const tmp62 = TranscriptFade;
  const tmp2Result11 = projectId(stateFromStores1[26]);
  ...
  const tmp63 = projectId(stateFromStores1[26]).isIOS() && undefined;
  const isIOSResult = projectId(stateFromStores1[26]).isIOS();
  let tmp65 = !isIOSResult;
  if (!isIOSResult) {
  ...
  const tmp2Result12 = projectId(stateFromStores1[26]);
  ...
  obj12.renderItem = function renderItem(arg0) {
    ...
    obj.replied = closure_22.get(item.render_id);
  ...
  obj10.children = ref(projectId(stateFromStores1[44]).FlashList, obj12);
  const items41 = [ref(tmp62, obj10), , , ];
  let tmp69 = stateFromStores2;
  const obj14 = { paddingBottom: bound + stateFromStores(stateFromStores1[7]).space.PX_8 };
  if (stateFromStores2) {
    tmp69 = !tmp20;
  }
  items41[1] = ref(stateFromStores(stateFromStores1[45]), { open: tmp69 });
  let tmp61Result3 = null;
  if (tmp20) {
    const obj18 = { projectId };
    tmp61Result3 = tmp61(tmp5(tmp3[46]), obj18);
  }
  items41[2] = tmp61Result3;
  let tmp61Result4 = null;
  if (null != tmp53) {
    const obj19 = { line: tmp53, onJumpToActivity: callback6, bottom: tmp5(tmp3[7]).space.PX_12 + tmp34, todos: memo4, agents: memo5 };
    tmp61Result4 = tmp61(tmp5(tmp3[47]), obj19);
    const tmp5Result3 = tmp5(tmp3[47]);
  }
  items41[3] = tmp61Result4;
  obj9.children = items41;
  items38[1] = c18(closure_8, obj9);
  const obj20 = { style: tmp.bottomStack, onLayout: callback1, children: null };
  const items42 = [ref(stateFromStores(stateFromStores1[48]), { projectId, thinking: stateFromStores2, compacting: stateFromStores3, activity: stateFromStores4, projectUsage: stateFromStores5, connLabel: connectionLabelResult, connFailed: "failed" === stateFromStores6, thinkingOpen: tmp20, onToggleThinking: callback }), ];
  const obj22 = { projectId, canSend: tmp26, running: stateFromStores2, stopped: stateFromStores7, onSend: callback7, onInterrupt: null };
  let tmp74;
  const obj21 = { projectId, thinking: stateFromStores2, compacting: stateFromStores3, activity: stateFromStores4, projectUsage: stateFromStores5, connLabel: connectionLabelResult, connFailed: "failed" === stateFromStores6, thinkingOpen: tmp20, onToggleThinking: callback };
  const tmp5Result = stateFromStores(stateFromStores1[45]);
  if (stateFromStores2) {
    tmp74 = callback8;
  }
  obj22.onInterrupt = tmp74;
  items42[1] = ref(stateFromStores(stateFromStores1[49]), obj22);
  obj20.children = items42;
  items38[2] = c18(closure_8, obj20);
  obj8.children = items38;
  return c18(closure_8, obj8);
};
