...
// Dependencies: [32, 19, 17, 16725, 21, 4638, 576, 4319, 4229, 1612, 16761, 4988, 1115, 3591, 7305, 4605, 9857, 1897, 4634, 5773, 7749, 5691, 7307, 7259, 6735, 6714, 5060, 2]
// Exports: default

// Module 16760 (VibegrationsRestorePointsSheet)
...
export default function VibegrationsRestorePointsSheet(projectId) {
    ...
    obj2.onConfirm = function onConfirm() {
      ...
      closure_0().then((ok) => {
        if (ok.ok) {
          ...
          closure_1_15(dependencyMap, "positive", intl4.string(_undefined(3591).kIWqXR));
          closure_1_12();
        } else if ("expired" === ok.code) {
          const intl3 = closure_0(1115).intl;
          const obj = { days: closure_0(16761).RESTORE_WINDOW_DAYS };
          closure_1_15(dependencyMap, "danger", intl3.formatToPlainString(_undefined(3591).PeVYaC, obj));
          closure_1_12();
        } else if ("unconfirmed" === ok.code) {
          const intl2 = closure_0(1115).intl;
          closure_1_15(dependencyMap, "danger", intl2.string(_undefined(3591)["2xSPXh"]));
          closure_1_12();
        } else {
          const intl = closure_0(1115).intl;
          closure_1_15(dependencyMap, "danger", intl.string(_undefined(3591).kXofol));
        }
      }).catch(() => {
        const intl = closure_0(1115).intl;
        closure_1_15(dependencyMap, "danger", intl.string(_undefined(3591).kXofol));
  ...
  let stringResult = intl.string(require("module_3591").CNvRyJ);
  c17 = stringResult;
  ...
  callback4 = obj2.useCallback((mode, startDate, minimumDate, onSubmit) => {
    ...
    obj.openLazy(asyncRequireImpl(9857, dependencyMap.paths), str, { mode, title: stringResult1, startDate, minimumDate: minimumDate[0], maximumDate: minimumDate[1], onSubmit }, "stack");
  }, items4);
  const items5 = [prop, num, callback4, first2];
  const callback5 = obj2.useCallback(() => {
    const obj = _modDef4229(prop);
    let items = [_modDef4229(prop).startOf("day").toDate(), ];
    const startOfResult = _modDef4229(prop).startOf("day");
    const tmp = num;
    const obj3 = _modDef4229(num);
    items[1] = _modDef4229(num).endOf("day").toDate();
    let tmp3 = first2;
    if (first2 == null) {
      tmp3 = tmp;
    }
    const endOfResult = _modDef4229(num).endOf("day");
  ...
  if ("loading" === obj4.status) {
    ...
    obj10.children = points.map((createdAt) => {
      ...
      obj2.onPress = function onPress() {
        if (null != parsed) {
          let createdAt = DateUtils.dateFormat(_modDef4229(tmp3), "LLL");
        } else {
  ...
  obj12.title = intl5.string(require("module_3591").FRjicO);
  ...
  const items7 = [intl6.formatToPlainString(require("module_3591").l07ism, { days: projectId(environment[10]).RESTORE_WINDOW_DAYS }), ];
  let str3 = "";
  if (null != _window) {
  ...
  const obj24 = { label: null, value: null, onChange: null, maxLength: 200, isDisabled: null };
  const intl8 = tmp4(tmp3[12]).intl;
  obj24.label = intl8.string(require("module_3591").hJb78b);
  obj24.value = first1;
  obj24.isDisabled = tmp15;
  ...
  obj25.text = intl9.string(require("module_3591")["14UarN"]);
  ...
  obj29.text = intl10.string(require("module_3591")["3D/vYN"]);
  if (!tmp15) {
  ...
  obj29.onPress = function onPress() {
    if (null != first2) {
      closure_16(DateUtils.dateFormat(_modDef4229(tmp), "LLL"), () => c11(projectId, environment, first2));
    }
  };
