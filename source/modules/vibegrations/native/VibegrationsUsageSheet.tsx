...
// Dependencies: [19, 17, 16702, 21, 4639, 576, 4635, 7352, 504, 7304, 7256, 1115, 3591, 5057, 2]
// Exports: default

// Module 16725 (VibegrationsUsageSheet)
...
export default function VibegrationsUsageSheet(projectId) {
  ...
  if (null == stateFromStores) {
    ...
    const tmp2Result = tmp2(7352);
    const tmp2Result6 = tmp2(7352);
    const sumTokenUsageResult = tmp2(7352).sumTokenUsage(stateFromStores.orchestrator, stateFromStores.codegen);
    const tmp2Result7 = tmp2(7352);
    const obj2 = { header: null, children: null };
    const obj3 = { title: null };
    const intl = tmp2(1115).intl;
    obj3.title = intl.string(_modDef3591["9yoLWZ"]);
    obj2.header = closure_5(tmp2(7256).BottomSheetTitleHeader, obj3);
    ...
    const sumTokenUsageResult1 = tmp2Result.sumTokenUsage(sumTokenUsageResult, tmp2(7352).usageOrEmpty(stateFromStores.compaction));
    const tmp2Result8 = tmp2(7352);
    obj7.runes = tmp2(7352).runesFromUsd(stateFromStores.cost_usd).toLocaleString();
    obj6.children = intl2.formatToPlainString(_modDef3591["4PFO2p"], obj7);
    const items2 = [closure_5(tmp2(4635).Text, obj6), , ];
    ...
    const runesFromUsdResult = tmp2(7352).runesFromUsd(stateFromStores.cost_usd);
    obj11.usage = tmp2(7352).usageOrEmpty(stateFromStores.compaction);
    items3[2] = closure_5(RoleRow, obj11);
    obj8.children = items3;
    items2[1] = closure_6(tmp2(5057).Stack, obj8);
    ...
    obj13.children = closure_5(tmp2(4635).Text, obj14);
    const items4 = [closure_5(View, obj13), ];
    const obj15 = { variant: "text-sm/medium", color: "text-default", children: null };
    const _Math = Math;
    const tmp2Result9 = tmp2(7352);
    const _HermesInternal = HermesInternal;
    obj15.children = "" + Math.round(100 * tmp2(7352).cacheHitRate(sumTokenUsageResult1)) + "%";
    items4[1] = closure_5(tmp2(4635).Text, obj15);
    obj12.children = items4;
    items2[2] = closure_6(View, obj12);
    obj5.children = items2;
    obj4.children = closure_6(tmp2(5057).Stack, obj5);
    obj2.children = closure_5(View, obj4);
    return closure_5(tmp2(7304).ActionSheet, obj2);
  }
