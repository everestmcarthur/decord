...
// Dependencies: [19, 17, 21, 16752, 8577, 4376, 2]
// Exports: default

// Module 16751 (VibegrationsConjureDither)
import ReanimatedRexportDefault from "ReanimatedRexport" /* 4376 */;
import inlineStyles from "inlineStyles" /* 8577 */;
import _mod16752 from "module_16752" /* 16752 */;
...
function BandPicture(width) {
  ...
  size1.children = state(height[3]).TILES[band].map((item) => {
    const size = { x: item.x, y: item.y, width: _mod16752.CELL, height: _mod16752.CELL, rx: _mod16752.CORNER, fill, fillOpacity };
  ...
  items[2] = BLOBS.map((peak, index) => {
    ...
    const obj3 = _mod16752;
    ({ peak, radius } = peak);
    const blobReachResult = _mod16752.blobReach(peak, radius, closure_5 + _mod16752.FADE_HALF);
    obj2.offset = obj3.isoStop(blobReachResult, _mod16752.blobReach(peak.peak, peak.radius, bound));
  ...
  items2[1] = BLOBS1.map((peak, index) => {
    const blobReachResult = _mod16752.blobReach(peak.peak, peak.radius, bound);
