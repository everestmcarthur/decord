...
// Dependencies: [19, 21, 8233, 1177, 1370, 2]
// Exports: createInlineChannelReact, inlineChannelMentionReact, inlineChannelReact

// Module 10277 (MarkupInlineChannelMentionRules)
...
export const inlineChannelMentionReact = function inlineChannelMentionReact(inContent, fn, key) {
  if (null == inContent.inContent) {
    let smartOutputResult = u1F4AC(8233).smartOutput(inContent, fn, key);
    const obj3 = u1F4AC(8233);
  } else {
    const obj = { children: null };
    const items = [fn(inContent.inContent, key), " \u203A ", u1F4AC(8233).smartOutput(inContent, fn, key)];
    obj.children = items;
    smartOutputResult = jsxs(u1F4AC(1177).LegacyText, { children: null }, key.key);
    const obj2 = u1F4AC(8233);
  }
