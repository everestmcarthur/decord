...
import HighlightTextDefault from "HighlightText" /* 10254 */;
...
function createMessagePreviewReactRules(customEmojiSize) {
  ...
  obj[num(5077).AST_KEY.HIGHLIGHT] = {
    react(node, output, key) {
      ...
      obj.children = num(8201).smartOutput(node, output, key);
  ...
  obj23.react = function react(arg0, output, state) {
    ...
    let smartOutputResult = null;
    if (null != first1) {
      smartOutputResult = num(8201).smartOutput(first1, output, state);
      const obj = num(8201);
    }
    return smartOutputResult;
  };
