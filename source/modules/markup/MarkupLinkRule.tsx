...
// Function ID: 5087
// Name: MarkupLinkRule
// Dependencies: [32, 1438, 5087, 5081, 1370, 4621, 1366, 5088, 4334, 12, 2]

// Module 5086 (MarkupLinkRule)
...
import errorDefault from "error" /* 5088 */;
...
function getRawText(content) {
  ...
  for (; iter !== undefined; str = str + tmp2.content) {
    ...
    if (MarkupTypes.AST_KEY.TEXT !== type) {
      if (tmp3(5081).AST_KEY.INLINE_CODE !== type) {
        if (tmp3(5081).AST_KEY.CUSTOM_EMOJI === type) {
          str = `${tmp2.name}`;
        } else if (tmp3(5081).AST_KEY.EMOJI === type) {
          str = `${tmp2.name}${tmp2.surrogate}`;
        } else if (tmp3(5081).AST_KEY.LINE_BREAK === type) {
          str = `${tmp2.name}${tmp2.surrogate}
  `;
        } else {
          if (tmp3(5081).AST_KEY.STRONG !== type) {
            if (tmp3(5081).AST_KEY.ITALICS !== type) {
              if (tmp3(5081).AST_KEY.UNDERLINE !== type) {
                if (tmp3(5081).AST_KEY.STRIKETHROUGH !== type) {
                  if (tmp3(5081).AST_KEY.SPOILER !== type) {
                    if (tmp3(5081).AST_KEY.TIMESTAMP === type) {
                      str = `${tmp2.name}${tmp2.surrogate}
  <timestamp>`;
                    } else {
                      if (tmp3(5081).AST_KEY.BLOCK_QUOTE !== type) {
                        if (tmp3(5081).AST_KEY.LIST !== type) {
                          if (tmp3(5081).AST_KEY.HEADING !== type) {
                            if (tmp3(5081).AST_KEY.SUBTEXT !== type) {
...
function isSuspiciousUrl(url) {
  if (obj.isSuspiciousCodedLink(url)) {
    ...
    if (null == value) {
      const sanitizeWhitespaceResult = tmp(5087).sanitizeWhitespace(url);
      const obj3 = { whitespaceSanitized: sanitizeWhitespaceResult, fullySanitized: null };
      const tmpResult = tmp(5087);
      obj3.fullySanitized = tmp(5087).sanitizeUnicodeConfusables(sanitizeWhitespaceResult);
      const result = obj2.set(url, obj3);
      value = obj3;
      const tmpResult2 = tmp(5087);
    }
...
let items1 = [fn(5081).AST_KEY.TEXT, fn(5081).AST_KEY.UNDERLINE, fn(5081).AST_KEY.STRONG, fn(5081).AST_KEY.ITALICS, fn(5081).AST_KEY.STRIKETHROUGH, fn(5081).AST_KEY.INLINE_CODE, fn(5081).AST_KEY.SPOILER, fn(5081).AST_KEY.LINE_BREAK, fn(5081).AST_KEY.TIMESTAMP];
let items2 = [...items1, fn(5081).AST_KEY.EMOJI, fn(5081).AST_KEY.CUSTOM_EMOJI];
let items3 = [fn(5081).AST_KEY.LIST, fn(5081).AST_KEY.HEADING, fn(5081).AST_KEY.BLOCK_QUOTE, fn(5081).AST_KEY.SUBTEXT];
const items4 = [fn(5081).AST_KEY.TEXT];
const items5 = [fn(5081).AST_KEY.UNDERLINE, fn(5081).AST_KEY.STRONG, fn(5081).AST_KEY.ITALICS, fn(5081).AST_KEY.STRIKETHROUGH, fn(5081).AST_KEY.INLINE_CODE, fn(5081).AST_KEY.SPOILER, fn(5081).AST_KEY.LINE_BREAK, fn(5081).AST_KEY.TIMESTAMP, fn(5081).AST_KEY.EMOJI, fn(5081).AST_KEY.CUSTOM_EMOJI, fn(5081).AST_KEY.LIST, fn(5081).AST_KEY.HEADING, fn(5081).AST_KEY.BLOCK_QUOTE, fn(5081).AST_KEY.SUBTEXT];
let obj = {};
let merged = Object.assign(_modDef4334.defaultRules.link);
obj.match = function match(arr, allowLinks, arg2) {
  if (allowLinks.allowLinks) {
    if (-1 === arr.indexOf("](")) {
      ...
      return _modDef4334.defaultRules.link.match(arr, allowLinks, arg2);
    }
  } else {
    return null;
  }
...
obj.parse = function parse(arg0, rules, allowEmojiLinks) {
  ...
  if (isSuspiciousUrl(tmp4)) {
    ...
    if (0 !== str2.trim().length) {
      if (0 !== trimmed.length) {
        const tmp48 = punycodeLink(_modDef4334.unescapeUrl(tmp4));
        if (null != tmp48) {
          if (tmp46(4621)(tmp5).length <= 0) {
            const obj12 = {};
            ...
            if (null != tmp38) {
              if (null != validateContentTypes(rules(value4.fullySanitized, obj12), items1)) {
                ...
                if (0 === str4.trim().length) {
                  const obj13 = { type: tmp37(5081).AST_KEY.TEXT, content: tmp2 };
                  return obj13;
                } else if (tmp46(4621)(str4).length > 0) {
                  const obj14 = { type: tmp37(5081).AST_KEY.TEXT, content: tmp2 };
                  return obj14;
                } else {
                  if (str4 !== str3) {
                    if (tmp46(4621)(str3).length > 0) {
                      const obj15 = { type: tmp37(5081).AST_KEY.TEXT, content: tmp2 };
                      return obj15;
                    } else {
                      items3 = [];
                      const tmp54 = rules(str4, obj12);
                      items3[HermesBuiltin.arraySpread(items, 0)] = tmp37(5081).AST_KEY.EMOJI;
                      if (null == validateContentTypes(tmp54, items3)) {
                        const obj16 = { type: tmp37(5081).AST_KEY.TEXT, content: tmp2 };
                        return obj16;
                      }
                  ...
                  const obj17 = { content: tmp46(4334).parserFor(pickResult)(value3.whitespaceSanitized, obj12), target: tmp48.target, title: value4.whitespaceSanitized };
                  return obj17;
                }
              }
            }
