...
// Dependencies: [2013, 1957, 1979, 4245, 4255, 1371, 1074, 1925, 1396, 1114, 5059, 4752, 4760, 5060, 4761, 5057, 4303, 2]
// Exports: getGuildIdFromChannelId

// Module 5058 (MarkupChannelMentionRule)
...
import useChannelRoleSubscriptionStatus from "useChannelRoleSubscriptionStatus" /* 5059 */;
...
function getChannel(id, arr) {
  ...
  if (null != channel) {
    ...
    obj5.name = tmp(4760).computeChannelName(channel, UserStore, RelationshipStore);
    obj5.isDm = channel.isPrivate();
    obj5.isForumPost = channel.isForumPost();
    const tmpResult = tmp(4760);
    obj5.isMentionable = tmp(5060).isChannelTypeMentionable(channel.type);
    const tmpResult3 = tmp(5060);
    obj5.canViewChannel = tmp(4761).canViewChannel(channel);
    obj5.roleSubscriptionGated = isSubscriptionGated;
    obj5.iconType = str;
    obj5.parentId = channel.parent_id;
    tmp4 = obj5;
    const tmpResult4 = tmp(4761);
  }
  return tmp4;
}
        ...
        if (channel.guildId === guildIdFromChannelId) {
          if (tmp34) {
            if (channel.isForumPost) {
              ...
              if (null != channel) {
                const channelName = tmp35(4760).computeChannelName(channel, UserStore, RelationshipStore);
                const tmp35Result = tmp35(4760);
                let str3 = tmp35(4752).getMentionIconType(channel);
                if (str3 == null) {
                  str3 = "forum";
                }
                const obj18 = { inContent: null, content: null };
                const tmp35Result3 = tmp35(4752);
