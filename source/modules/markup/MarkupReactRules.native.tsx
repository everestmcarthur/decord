...
// Dependencies: [19, 17, 4598, 1957, 2015, 1979, 1074, 1964, 1085, 21, 4601, 4602, 4606, 576, 4617, 4760, 4465, 4809, 8264, 4950, 1114, 7262, 4304, 5635, 1178, 1365, 8456, 4301, 11707, 11677, 8171, 504, 4456, 8035, 4573, 11680, 1896, 11, 1935, 5638, 11658, 13894, 11306, 5047, 10216, 13895, 13897, 4549, 5049, 1115, 5079, 13899, 10218, 2]
// Exports: default, plainMentionRenderer, plainSpoilerRenderer

// Module 4597 (MarkupReactRules)
...
import MarkupReactCommandRuleDefault from "MarkupReactCommandRule" /* 11306 */;
...
function MarkupLink(arg0) {
  ...
  obj2.inLink = true;
  obj.children = node(8171).smartOutput(node, output, obj2);
  return tmp2(node(4602).Text, obj, state.key);
}
    ...
    fn = () => {
      if (null != node.roleId) {
        if (null != tmp.guildId) {
          ...
          obj5.openLazy(asyncRequireImpl(11680, dependencyMap.paths), "RoleMembersActionSheet", obj3, "stack");
        }
      }
...
function MarkupInlineCode(arg0) {
  ...
  obj2.children = node(8171).smartOutput(node, output, state);
...
function MarkupAttachmentLink(state) {
  ...
  let obj = { variant: str2, style: tmp.channelMentionText, children: null };
  const obj2 = { themedColor: node(576).colors.MENTION_FOREGROUND, source: node(13894), size: null };
  const fontScale = closure_4.getFontScale();
  if (fontScale < 1) {
  ...
  obj2.size = SMALL;
  const items = [closure_15(state(1178).ThemedIcon, obj2), state(8171).smartOutput(node, output, state)];
  obj.children = items;
  const tmp2Result = closure_16(state(4602).Text, obj, state.key);
  ...
  return {
    ...
    [closure_0(closure_2[43]).AST_KEY.STATIC_ROUTE_LINK]: {
      react(channelId, output, textColor) {
        ...
        if (obj.isStaticRouteIconType(channelId.channelId)) {
          let SignPostIcon = tmp(13895).SignPostIcon;
          ...
          if (constants.GUILD_HOME !== channelId) {
              ...
              if (tmp4.CHANNEL_BROWSER !== channelId) {
                if (tmp4.CUSTOMIZE_COMMUNITY !== channelId) {
                  if (tmp4.LINKED_ROLES === channelId) {
                    SignPostIcon = tmp(4549).LinkIcon;
                  }
                }
              }
              SignPostIcon = tmp(13897).ChannelListMagnifyingGlassIcon;
            }
            ...
            const items = [__initData(SignPostIcon, obj3), tmp(8171).smartOutput(channelId, output, textColor)];
            obj2.children = items;
            return value2(MarkupText, obj2, textColor.key);
          }
          SignPostIcon = tmp(13895).SignPostIcon;
        } else {
          return null;
        }
      }
    ...
    [closure_0(closure_2[43]).AST_KEY.CODE_BLOCK]: {
      parse(arg0, arg1, arg2) {
        obj = obj2(5049).RULES[obj(undefined, 5047).AST_KEY.CODE_BLOCK];
    ...
    [closure_0(closure_2[43]).AST_KEY.GUILD]: {
      react(icon, output, textColor) {
        ...
        if (null == icon.icon) {
          ...
          const obj4 = { color: textColor1, children: tmp(8171).smartOutput(icon, output, textColor) };
          ...
          obj5.size = XXSMALL;
          closure_1_15(obj2(5635), obj5);
          const tmp6 = obj2(5635);
        }
      }
    },
    ...
    [closure_0(closure_2[43]).AST_KEY.GAME_MENTION]: {
      react(node, arg1, state) {
        return closure_1_15(obj2(13899), { node, state }, state.key);
      }
    },
    ...
    [closure_0(closure_2[43]).AST_KEY.HEADING]: {
      react(level, output, formatInline) {
        if (formatInline.formatInline) {
          ...
          const items = [obj(8171).smartOutput(level, output, obj3), " "];
          obj2.children = items;
          ...
          const items1 = [obj(8171).smartOutput(level, output, obj4), "\n"];
          obj.children = items1;
