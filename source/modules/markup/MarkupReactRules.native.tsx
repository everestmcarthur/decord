...
// Dependencies: [19, 17, 4599, 1957, 2015, 1979, 1074, 1964, 1085, 21, 4602, 4603, 4607, 576, 4618, 4761, 4465, 4810, 8265, 4951, 1114, 7263, 4304, 5636, 1178, 1365, 8458, 4301, 11709, 11679, 8172, 504, 4456, 8036, 4574, 11682, 1896, 11, 1935, 5639, 11660, 13896, 11308, 5048, 10218, 13897, 13899, 4550, 5050, 1115, 5080, 13901, 10220, 2]
// Exports: default, plainMentionRenderer, plainSpoilerRenderer

// Module 4598 (MarkupReactRules)
...
import MarkupReactCommandRuleDefault from "MarkupReactCommandRule" /* 11308 */;
...
function MarkupLink(arg0) {
  ...
  obj.onPress = function onPress(stopPropagation) {
    ...
    if (null != tmp) {
      ...
      const obj = {
        ...
        trusted() {
            return node(11709).isLinkTrusted(formatResult);
          }
      };
      node(8458).handleClick(obj);
      const obj2 = node(8458);
    }
  };
  ...
  obj2.inLink = true;
  obj.children = node(8172).smartOutput(node, output, obj2);
  return tmp2(node(4603).Text, obj, state.key);
}
    ...
    fn = () => {
      if (null != node.roleId) {
        if (null != tmp.guildId) {
          ...
          obj5.openLazy(asyncRequireImpl(11682, dependencyMap.paths), "RoleMembersActionSheet", obj3, "stack");
        }
      }
      if ("@everyone" === node.roleName) {
        if (null != tmp.guildId) {
          const obj2 = ActionSheetActionCreatorsDefault;
          const tmp12 = asyncRequireImpl(11682, dependencyMap.paths);
...
function MarkupInlineCode(arg0) {
  ...
  obj2.children = node(8172).smartOutput(node, output, state);
...
function MarkupChannelMention(state) {
  ...
  if (null == node.inContent) {
    items[1] = null;
    items[2] = tmp3(8172).smartOutput(node, output, state);
    obj.children = items;
    const tmp2Result = closure_16(state(4603).Text, obj, state.key);
...
function MarkupAttachmentLink(state) {
  ...
  let obj = { variant: str2, style: tmp.channelMentionText, children: null };
  const obj2 = { themedColor: node(576).colors.MENTION_FOREGROUND, source: node(13896), size: null };
  const fontScale = closure_4.getFontScale();
  if (fontScale < 1) {
  ...
  obj2.size = SMALL;
  const items = [closure_15(state(1178).ThemedIcon, obj2), state(8172).smartOutput(node, output, state)];
  obj.children = items;
  const tmp2Result = closure_16(state(4603).Text, obj, state.key);
  ...
  return {
    [closure_0(closure_2[43]).AST_KEY.TEXT]: {
      react(content, output, textColor) {
        if (typeof content.content === "string") {
          ...
          obj.children = obj(8172).smartOutput(content, output, textColor);
    ...
    [closure_0(closure_2[43]).AST_KEY.STRIKETHROUGH]: {
      react(node, output, textColor) {
        ...
        obj.children = obj(8172).smartOutput(node, output, textColor);
    ...
    [closure_0(closure_2[43]).AST_KEY.HIGHLIGHT]: {
      react(node, output, textColor) {
        ...
        obj = { color: textColor, children: obj(8172).smartOutput(node, output, textColor) };
        return closure_1_15(MarkupText, obj, textColor.key);
      }
    },
    ...
    [closure_0(closure_2[43]).AST_KEY.STATIC_ROUTE_LINK]: {
      react(channelId, output, textColor) {
        ...
        if (obj.isStaticRouteIconType(channelId.channelId)) {
          let SignPostIcon = tmp(13897).SignPostIcon;
          ...
          if (constants.GUILD_HOME !== channelId) {
              ...
              if (tmp4.CHANNEL_BROWSER !== channelId) {
                if (tmp4.CUSTOMIZE_COMMUNITY !== channelId) {
                  if (tmp4.LINKED_ROLES === channelId) {
                    SignPostIcon = tmp(4550).LinkIcon;
                  }
                }
              }
              SignPostIcon = tmp(13899).ChannelListMagnifyingGlassIcon;
            }
            ...
            const items = [__initData(SignPostIcon, obj3), tmp(8172).smartOutput(channelId, output, textColor)];
            obj2.children = items;
            return value2(MarkupText, obj2, textColor.key);
          }
          SignPostIcon = tmp(13897).SignPostIcon;
        } else {
          return null;
        }
      }
    ...
    [closure_0(closure_2[43]).AST_KEY.CODE_BLOCK]: {
      parse(arg0, arg1, arg2) {
        obj = obj2(5050).RULES[obj(undefined, 5048).AST_KEY.CODE_BLOCK];
    ...
    [closure_0(closure_2[43]).AST_KEY.SOUNDBOARD]: {
      react(node, output, key) {
        ...
        const items = ["<sound:", obj(8172).smartOutput(node, output, key), ">"];
        obj.children = items;
        return closure_1_16(obj(4603).Text, obj, key.key);
      }
    },
    [closure_0(closure_2[43]).AST_KEY.GUILD]: {
      react(icon, output, textColor) {
        ...
        if (null == icon.icon) {
          ...
          const obj4 = { color: textColor1, children: tmp(8172).smartOutput(icon, output, textColor) };
          ...
          if (fontScale < 1) {
            let XXSMALL = tmp(5636).GuildIconSizes.XXXSMALL;
          } else if (fontScale < 1.25) {
            XXSMALL = tmp(5636).GuildIconSizes.XXSMALL_12;
          } else {
            XXSMALL = tmp(5636).GuildIconSizes.XXSMALL;
          }
          obj5.size = XXSMALL;
          closure_1_15(obj2(5636), obj5);
          const tmp6 = obj2(5636);
        }
      }
    },
    [closure_0(closure_2[43]).AST_KEY.CHANNEL]: {
      react(iconType, output, textColor) {
        ...
        items[1] = obj(8172).smartOutput(iconType, output, textColor);
        obj3.children = items;
    ...
    [closure_0(closure_2[43]).AST_KEY.GAME_MENTION]: {
      react(node, arg1, state) {
        return closure_1_15(obj2(13901), { node, state }, state.key);
      }
    },
    ...
    [closure_0(closure_2[43]).AST_KEY.LIST]: {
      react(start, arg1, level) {
        ...
        if (level.formatInline) {
          ...
          return closure_15(start(4603).Text, obj, "list-" + level.key);
        }
      }
    },
    [closure_0(closure_2[43]).AST_KEY.HEADING]: {
      react(level, output, formatInline) {
        if (formatInline.formatInline) {
          ...
          const items = [obj(8172).smartOutput(level, output, obj3), " "];
          obj2.children = items;
          ...
          const items1 = [obj(8172).smartOutput(level, output, obj4), "\n"];
          obj.children = items1;
          return closure_1_16(obj(4603).Text, obj, formatInline.key);
        }
      }
    },
    [closure_0(closure_2[43]).AST_KEY.SUBTEXT]: {
      react(node, output, key) {
        ...
        const items = [obj(8172).smartOutput(node, output, key), "\n"];
        obj.children = items;
        return closure_1_16(obj(4603).Text, obj, key.key);
      }
    },
...
export const createFetchingGameMentionRule = fn(13901).createFetchingGameMentionRule;
