...
// Dependencies: [5, 32, 19, 17, 15653, 502, 21, 11919, 9493, 11125, 15654, 12018, 15656, 10050, 15658, 13895, 15660, 15662, 15664, 15666, 10441, 15668, 15670, 15672, 15674, 7453, 4606, 576, 5024, 6677, 5026, 4573, 15676, 7223, 7222, 5738, 5656, 4557, 504, 7054, 2]
// Exports: default
...
function UserActionSheet(pool) {
  ...
  let obj = { header: closure_10(pool(7222).BottomSheetTitleHeader, { title: pool.summary, subtitle: "" + usersForPool.length + " users" }), children: null };
  ...
  obj3.children = closure_10(pool(5738).TableRowGroup, {
    ...
    children: usersForPool.map((id, index) => {
      ...
      if (id.id === dependencyMap) {
        tmp2Result = tmp2(pool(4557).CheckmarkLargeIcon, { size: "md", color: "text-feedback-positive" });
      }
...
function PoolUsers(pool) {
  ...
  return closure_10(pool(5656).TableRow, { icon: closure_10(items[Number(undefined, id) % items.length], { size: "md", color: length[Number(undefined, id) % length.length] }), label: pool.summary, subLabel: "" + usersForPool.length + " users", arrow: true, onPress: callback, start, end });
}
get_ActivityIndicator = fn(17);
({ View: metroRequire, ScrollView: closure_7 } = get_ActivityIndicator);
const jsxProd = fn(21);
({ jsx: c10, jsxs: closure_11 } = jsxProd);
let items = [fn(11919).UserIcon, fn(9493).ShieldIcon, fn(11125).GiftIcon, fn(15654).AchievementsIcon, fn(12018).PiggyBankIcon, fn(15656).TreehouseIcon, fn(10050).SpeedometerIcon, fn(15658).CompassIcon, fn(13895).SignPostIcon, fn(15660).CarIcon, fn(15662).TrainIcon, fn(15664).TeacupIcon, fn(15666).InventoryIcon, fn(10441).FoodIcon, fn(15668).BurgerIcon, fn(15670).MagicDoorIcon, fn(15672).PawPrintIcon, fn(15674).RecordPlayerIcon, fn(7453).SettingsIcon];
...
export default function DevToolsGeneratedTestUsersScreen() {
  ...
  if (tmp5Result) {
    tmp5Result = tmp5(stateFromStoresArray(5656).TableRow, { label: "No pools available." });
  }
  ...
  items2[1] = closure_11(stateFromStoresArray(5738).TableRowGroup, obj6);
  obj5.children = items2;
  obj3.children = closure_11(stateFromStoresArray(5024).Stack, obj5);
