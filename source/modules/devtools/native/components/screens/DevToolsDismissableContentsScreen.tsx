...
// Dependencies: [32, 19, 17, 1947, 21, 4605, 576, 1943, 15698, 7252, 5737, 10307, 5655, 1940, 4563, 5662, 15699, 7102, 4601, 10387, 5567, 1611, 10001, 504, 15090, 15091, 8943, 2]
// Exports: default
...
export default function DevToolsDismissableContentsScreen() {
  ...
  let obj = ref(10001);
  [dependencyMap, tmp5] = ref(10001).useLocalStorageState("devtools-dc-search", "");
  _slicedToArray = tmp5;
  ...
  let tmp4 = _slicedToArray(ref(10001).useLocalStorageState("devtools-dc-search", ""), 2);
  let items = [closure_6];
  ...
  const obj2 = ref(504);
  let tmp9 = _slicedToArray(ref(15090).useForceBountiesSwipeUpNux(), 2);
  const first2 = tmp9[0];
  closure_10 = tmp11;
  const items1 = [tmp9[1]];
  ...
  obj4.children = dailyCapOverridden(ref(8943).FlashList, obj5);
