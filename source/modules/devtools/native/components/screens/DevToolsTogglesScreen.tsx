...
// Dependencies: [32, 19, 17, 5712, 4638, 21, 5602, 4639, 576, 5690, 4338, 7308, 504, 5772, 15792, 7088, 15809, 5057, 15810, 7157, 2]
// Exports: default
...
function fuzzySearchToggle(str, str2, str3) {
  ...
  if (!tmp) {
    ...
    if (!tmp3ResultResult) {
      ...
      tmp3ResultResult = tmp3(5602)(formatted1, str3.toLowerCase());
      const tmp3Result = tmp3(5602);
    }
...
export default function DevToolsTogglesScreen() {
  ...
  const stateFromStores = query(504).useStateFromStores(items, () => DesignTogglesStore.allWithDescriptions().filter((item) => {
    ...
    if (!tmp) {
      ...
      if (!tmp2ResultResult) {
        ...
        tmp2ResultResult = tmp2(5602)(formatted1, str2.toLowerCase());
        const tmp2Result = tmp2(5602);
      }
  ...
  if (stateFromStores.length > 0) {
    ...
    tmp7Result = tmp7(tmp5(5772).TableRowGroup, obj8);
  }
  ...
  obj3.children = tmp9(query(5057).Stack, obj9);
