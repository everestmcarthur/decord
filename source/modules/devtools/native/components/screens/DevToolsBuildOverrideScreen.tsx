...
// Dependencies: [32, 19, 17, 11579, 21, 4640, 576, 9017, 15705, 7095, 504, 11941, 5060, 5775, 5693, 4586, 7303, 4337, 15270, 4597, 5773, 5776, 6717, 5062, 1370, 2]

// Module 15704 (DevToolsBuildOverrideScreen)
...
let items = [{ label: "Branch Name", value: "branch", icon: jsx(fn(9017).TagIcon, {}) }, ];
let obj5 = { label: "Branch Name", value: "branch", icon: jsx(fn(9017).TagIcon, {}) };
items[1] = { label: "Commit SHA", value: "id", icon: jsx(fn(15705).HashmarkIcon, {}) };
let obj6 = { label: "Commit SHA", value: "id", icon: jsx(fn(15705).HashmarkIcon, {}) };
const size = fn(2);
...
export default noop.memo(() => {
  ...
  const stateFromStores = first(504).useStateFromStores(items, () => {
    ...
    let tmp;
    if (overrides != null) {
      tmp = overrides[first(undefined, 11941).DEVICE_FIELD];
    }
    return tmp;
  });
  ...
  obj3.paddingBottom = tmp.contentContainer.padding + first(7095)({ includeKeyboardHeight: true }).insets.bottom;
  ...
  if (null != stateFromStores) {
    ...
    const items1 = [tmp7(tmp3(5693).TableRow, obj4), , ];
    const obj6 = { icon: tmp7(tmp3(15270).RefreshIcon, {}), label: "Refresh Override", onPress: tmp3(11941).refreshBuildOverride, arrow: true };
    items1[1] = tmp7(tmp3(5693).TableRow, obj6);
    const obj7 = { icon: tmp7(tmp3(4597).TrashIcon, { color: "text-feedback-critical" }), label: "Clear Override", variant: "danger", onPress: tmp3(11941).clearBuildOverride, arrow: true };
    items1[2] = tmp7(tmp3(5693).TableRow, obj7);
    obj5.children = items1;
    tmp10Result = tmp10(tmp3(5775).TableRowGroup, obj5);
  }
  ...
  let obj = first(504);
  items2[1] = jsx(first(5773).TableRadioGroup, {
    ...
    children: items.map((value) => {
      value = value.value;
      ({ icon, label } = value);
      return jsx(first(5776).TableRadioRow, { value, label, icon }, value);
    })
  });
  ...
  obj10.label = jsx(first(6717).TextInput, {
    size: "md",
  ...
  obj9.children = jsx(first(5693).TableRow, { icon, label: null });
  items2[2] = jsx(first(5775).TableRowGroup, { title: label1, hasIcons: true, children: null });
  items2[3] = jsx(first(5062).Button, {
  ...
  obj2.children = jsxs(first(5060).Stack, { spacing: 16, children: null });
