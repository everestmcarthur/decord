...
// Dependencies: [32, 19, 17, 1371, 14475, 1074, 21, 4605, 576, 4601, 5655, 10454, 4196, 9873, 5173, 7241, 4552, 504, 8943, 5023, 5737, 7252, 12221, 4563, 15656, 7102, 2]
// Exports: default

// Module 15661 (DevToolsAnalyticsScreen)
...
import AnalyticsLogStore from "AnalyticsLogStore" /* 14475 */;
...
function LoggedEvent(arg0) {
  ...
  let obj = { arrow: !tmp2, icon: closure_8(fingerprint(10454).AnalyticsIcon, {}), label: event, subLabel: null, onPress: null, start: null, end: null };
  ...
  obj.start = start;
  obj.end = end;
  let items = [closure_8(fingerprint(5655).TableRow, obj), ];
  if (!tmp2) {
    ...
    const obj7 = { variant: "text-sm/semibold", color: "mobile-text-heading-primary", children: map(4196)(timestamp).calendar() };
    obj6.children = tmp6(CopyIcon(4601).Text, obj7);
    let items1 = [tmp6(CommonProperty, obj6), , ];
    let tmp6Result = null != user;
    if (tmp6Result) {
      const obj8 = { name: "User ", children: null };
      const obj9 = { user };
      obj8.children = tmp6(tmp10(9873), obj9);
...
export default function DevToolsAnalyticsScreen() {
  ...
  const items2 = [closure_8(trimmed(7252).TableSwitchRow, { icon: closure_8(trimmed(12221).ArrowsUpDownIcon, {}), label: "Reverse Events", value: first, onValueChange: tmp5[1] }), ];
  const obj4 = { icon: closure_8(trimmed(12221).ArrowsUpDownIcon, {}), label: "Reverse Events", value: first, onValueChange: tmp5[1] };
  items2[1] = closure_8(trimmed(5655).TableRow, { arrow: true, variant: "danger", icon: closure_8(trimmed(4563).TrashIcon, { color: "text-feedback-critical" }), label: "Clear Analytics Log", onPress: trimmed(15656).clearAnalyticsLog });
  obj3.children = items2;
  const items3 = [closure_9(trimmed(5737).TableRowGroup, obj3), , ];
  const obj5 = { arrow: true, variant: "danger", icon: closure_8(trimmed(4563).TrashIcon, { color: "text-feedback-critical" }), label: "Clear Analytics Log", onPress: trimmed(15656).clearAnalyticsLog };
  items3[1] = closure_8(View, { style: tmp.searchFieldContainer, children: closure_8(trimmed(7102).SearchField, { placeholder: "Search by event name", onChange: tmp8 }) });
  if (0 === loggedEvents.length) {
    let tmp11Result = tmp11(tmp2(5655).TableRow, { label: "No events logged." });
  } else {
    tmp11Result = null;
    if (0 === found.length) {
      ...
      tmp11Result = tmp11(tmp2(5655).TableRow, obj7);
    }
  }
  ...
  items3[2] = closure_8(trimmed(5737).TableRowGroup, { title: "Analytics Events", hasIcons: false, children: tmp11Result });
  obj9.children = items3;
  obj8.ListHeaderComponent = tmp13(trimmed(5023).Stack, obj9);
  ...
  obj2.children = closure_8(trimmed(8943).FlashList, obj8);
