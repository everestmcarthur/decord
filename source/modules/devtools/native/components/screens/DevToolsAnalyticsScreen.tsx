...
// Dependencies: [32, 19, 17, 1371, 14451, 1074, 21, 4606, 576, 4602, 5656, 10473, 4198, 9894, 5174, 7262, 4553, 504, 8964, 5024, 5738, 7273, 12244, 4564, 15639, 7123, 2]
// Exports: default
...
import AnalyticsLogStore from "AnalyticsLogStore" /* 14451 */;
...
function LoggedEvent(arg0) {
  ...
  let obj = { arrow: !tmp2, icon: closure_8(fingerprint(10473).AnalyticsIcon, {}), label: event, subLabel: null, onPress: null, start: null, end: null };
  ...
  obj.start = start;
  obj.end = end;
  let items = [closure_8(fingerprint(5656).TableRow, obj), ];
  if (!tmp2) {
    ...
    if (null != fingerprint) {
      ...
      const items3 = [tmp6(CopyIcon(4602).Text, obj11), ];
      CopyIcon = CopyIcon(4553).CopyIcon;
      tmp6Result2 = tmp6(CopyIcon, { size: "sm" });
      items3[1] = tmp6Result2;
      obj10.children = items3;
      let tmp6Result3 = tmp4(CopyIcon(5174).PressableOpacity, obj10);
    } else {
      const obj12 = { variant: "text-sm/medium", color: "text-muted", style: map.monospace, children: "null" };
      tmp6Result3 = tmp6(CopyIcon(4602).Text, obj12);
    }
...
export default function DevToolsAnalyticsScreen() {
  ...
  const items2 = [closure_8(trimmed(7273).TableSwitchRow, { icon: closure_8(trimmed(12244).ArrowsUpDownIcon, {}), label: "Reverse Events", value: first, onValueChange: tmp5[1] }), ];
  const obj4 = { icon: closure_8(trimmed(12244).ArrowsUpDownIcon, {}), label: "Reverse Events", value: first, onValueChange: tmp5[1] };
  items2[1] = closure_8(trimmed(5656).TableRow, { arrow: true, variant: "danger", icon: closure_8(trimmed(4564).TrashIcon, { color: "text-feedback-critical" }), label: "Clear Analytics Log", onPress: trimmed(15639).clearAnalyticsLog });
  obj3.children = items2;
  const items3 = [closure_9(trimmed(5738).TableRowGroup, obj3), , ];
  const obj5 = { arrow: true, variant: "danger", icon: closure_8(trimmed(4564).TrashIcon, { color: "text-feedback-critical" }), label: "Clear Analytics Log", onPress: trimmed(15639).clearAnalyticsLog };
  items3[1] = closure_8(View, { style: tmp.searchFieldContainer, children: closure_8(trimmed(7123).SearchField, { placeholder: "Search by event name", onChange: tmp8 }) });
  if (0 === loggedEvents.length) {
    let tmp11Result = tmp11(tmp2(5656).TableRow, { label: "No events logged." });
  } else {
    tmp11Result = null;
    if (0 === found.length) {
      ...
      tmp11Result = tmp11(tmp2(5656).TableRow, obj7);
    }
  }
  ...
  items3[2] = closure_8(trimmed(5738).TableRowGroup, { title: "Analytics Events", hasIcons: false, children: tmp11Result });
  obj9.children = items3;
  obj8.ListHeaderComponent = tmp13(trimmed(5024).Stack, obj9);
  ...
  obj2.children = closure_8(trimmed(8964).FlashList, obj8);
