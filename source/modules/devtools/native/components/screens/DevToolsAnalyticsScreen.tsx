...
// Dependencies: [32, 19, 17, 1372, 14496, 1074, 21, 4639, 576, 4635, 5690, 10516, 4231, 9935, 5207, 7296, 4586, 504, 9006, 5057, 5772, 7307, 12285, 4597, 15674, 7157, 2]
// Exports: default

// Module 15679 (DevToolsAnalyticsScreen)
...
import AnalyticsLogStore from "AnalyticsLogStore" /* 14496 */;
...
function LoggedEvent(arg0) {
  ...
  let obj = { arrow: !tmp2, icon: closure_8(fingerprint(10516).AnalyticsIcon, {}), label: event, subLabel: null, onPress: null, start: null, end: null };
  ...
  obj.start = start;
  obj.end = end;
  let items = [closure_8(fingerprint(5690).TableRow, obj), ];
  if (!tmp2) {
    ...
    const obj7 = { variant: "text-sm/semibold", color: "mobile-text-heading-primary", children: map(4231)(timestamp).calendar() };
    obj6.children = tmp6(CopyIcon(4635).Text, obj7);
    let items1 = [tmp6(CommonProperty, obj6), , ];
    let tmp6Result = null != user;
    if (tmp6Result) {
      const obj8 = { name: "User ", children: null };
      const obj9 = { user };
      obj8.children = tmp6(tmp10(9935), obj9);
...
export default function DevToolsAnalyticsScreen() {
  ...
  const items2 = [closure_8(trimmed(7307).TableSwitchRow, { icon: closure_8(trimmed(12285).ArrowsUpDownIcon, {}), label: "Reverse Events", value: first, onValueChange: tmp5[1] }), ];
  const obj4 = { icon: closure_8(trimmed(12285).ArrowsUpDownIcon, {}), label: "Reverse Events", value: first, onValueChange: tmp5[1] };
  items2[1] = closure_8(trimmed(5690).TableRow, { arrow: true, variant: "danger", icon: closure_8(trimmed(4597).TrashIcon, { color: "text-feedback-critical" }), label: "Clear Analytics Log", onPress: trimmed(15674).clearAnalyticsLog });
  obj3.children = items2;
  const items3 = [closure_9(trimmed(5772).TableRowGroup, obj3), , ];
  const obj5 = { arrow: true, variant: "danger", icon: closure_8(trimmed(4597).TrashIcon, { color: "text-feedback-critical" }), label: "Clear Analytics Log", onPress: trimmed(15674).clearAnalyticsLog };
  items3[1] = closure_8(View, { style: tmp.searchFieldContainer, children: closure_8(trimmed(7157).SearchField, { placeholder: "Search by event name", onChange: tmp8 }) });
  if (0 === loggedEvents.length) {
    let tmp11Result = tmp11(tmp2(5690).TableRow, { label: "No events logged." });
  } else {
    tmp11Result = null;
    if (0 === found.length) {
      ...
      tmp11Result = tmp11(tmp2(5690).TableRow, obj7);
    }
  }
  ...
  items3[2] = closure_8(trimmed(5772).TableRowGroup, { title: "Analytics Events", hasIcons: false, children: tmp11Result });
  obj9.children = items3;
  obj8.ListHeaderComponent = tmp13(trimmed(5057).Stack, obj9);
  ...
  obj2.children = closure_8(trimmed(9006).FlashList, obj8);
