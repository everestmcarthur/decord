...
// Dependencies: [17, 1074, 4747, 9325, 12, 1231, 504, 573, 2]
      ...
      if (RTCConnectionStates.CONNECTING === state) {
        ...
        if (tmp8) {
          if (tmp4Result.isString(device)) {
            ...
            tmp4(9325).setActiveAudioDevice(tmp10);
            const tmp4Result4 = tmp4(9325);
          }
          tmp4Result = tmp4(12);
        }
  ...
  NATIVE_AUDIO_SET_OUTPUT_DEVICE: function handleSetActiveAudioDevice(device) {
    ...
    if (c9) {
      if (obj.isString(device)) {
        ...
        tmp(9325).setActiveAudioDevice(device);
        const tmpResult2 = tmp(9325);
      }
      obj = _modDef12;
    }
  }
