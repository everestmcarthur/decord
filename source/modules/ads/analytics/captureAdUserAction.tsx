...
// Dependencies: [5, 7776, 1074, 5503, 7791, 7772, 7801, 7807, 1242, 1256, 1115, 7750, 11290, 7802, 7782, 11293, 2]
// Exports: captureAdUserAction

// Module 11289 (captureAdUserAction)
...
import AdDataUtils from "AdDataUtils" /* 7802 */;
import BrandSafetyContext from "BrandSafetyContext" /* 11290 */;
...
function handleViewImpression(minViewTimeSeconds) {
  ...
  if (minViewTimeSeconds.adCreativeType !== AdCreativeType.AdCreativeType.QUEST) {
    ...
    tmp9(7791).trackAdContentEvent(obj6);
    const tmp9Result = tmp9(7791);
  } else {
    ...
    const tmp9Result5 = tmp9(7791);
    let adMetadataSealed = tmp9(7772).getAdMetadataSealed(sourceQuestContent, adCreativeId);
    if (adMetadataSealed == null) {
      adMetadataSealed = null;
    }
    obj10.metadata_sealed = adMetadataSealed;
    const tmp9Result6 = tmp9(7772);
    const currentQuestHomeSearchSession = tmp9(7807).getCurrentQuestHomeSearchSession();
    let uuid;
    if (currentQuestHomeSearchSession != null) {
      uuid = currentQuestHomeSearchSession.uuid;
    }
    ...
    const tmp9Result7 = tmp9(7807);
    let adTrafficMetadataSealed = tmp9(7772).getAdTrafficMetadataSealed(sourceQuestContent, adCreativeId);
    if (adTrafficMetadataSealed == null) {
...
export const captureAdUserAction = function captureAdUserAction(captureAdUserActionResult1) {
  try {
    ...
    } else if (tmp2(11293).AdUserActionType.CLICK_INTERNAL === type) {
    ...
    } else if (tmp2(11293).AdUserActionType.CLICK_EXTERNAL_ADVERTISER_CTA === type) {
    ...
    } else if (tmp2(11293).AdUserActionType.VIEW_INTERNAL_SURFACE_IMPRESSION === type) {
    ...
    } else if (tmp2(11293).AdUserActionType.VIEW_EXTERNAL_PAID_AD_PLACEMENT_IMPRESSION === type) {
