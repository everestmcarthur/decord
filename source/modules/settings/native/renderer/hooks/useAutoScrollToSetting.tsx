...
// Dependencies: [19, 14788, 11607, 14682, 14790, 1484, 2]
// Exports: useAutoScrollToSearchResultSetting

// Module 14794 (useAutoScrollToSetting)
...
export const useAutoScrollToSearchResultSetting = function useAutoScrollToSearchResultSetting(ref, memo, scrollTarget) {
  ...
  if (null != current) {
    flag = false;
    if (tmp(14682).SETTING_RENDERER_CONFIG[current].type !== current.ROUTE) {
      let initialScrollIndex = tmp(14790).getInitialScrollIndex(current, memo);
      let tmp7 = 0 !== initialScrollIndex;
      if (tmp7) {
        tmp7 = 1 !== initialScrollIndex;
      }
      flag = tmp7;
      const tmpResult = tmp(14790);
    }
  }
