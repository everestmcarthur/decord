...
// Dependencies: [19, 14804, 11615, 14698, 14806, 1484, 2]
// Exports: useAutoScrollToSearchResultSetting

// Module 14810 (useAutoScrollToSetting)
...
export const useAutoScrollToSearchResultSetting = function useAutoScrollToSearchResultSetting(ref, memo, scrollTarget) {
  ...
  if (null != current) {
    flag = false;
    if (tmp(14698).SETTING_RENDERER_CONFIG[current].type !== current.ROUTE) {
      let initialScrollIndex = tmp(14806).getInitialScrollIndex(current, memo);
      let tmp7 = 0 !== initialScrollIndex;
      if (tmp7) {
        tmp7 = 1 !== initialScrollIndex;
      }
      flag = tmp7;
      const tmpResult = tmp(14806);
    }
  }
