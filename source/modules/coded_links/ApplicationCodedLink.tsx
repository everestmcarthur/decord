...
// Dependencies: [4562, 1369, 7705, 7706, 2]
    ...
    if (tmp(4562).CodedLinkType.APP_OAUTH2_LINK !== type) {
      if (tmp(4562).CodedLinkType.APP_DIRECTORY_STOREFRONT !== type) {
        if (tmp(4562).CodedLinkType.APP_DIRECTORY_STOREFRONT_SKU === type) {
          const result = tmp(7705).parseStorefrontSkuCodedLink(code);
          let tmp5 = null;
          if (null != result) {
          ...
          const obj = { type, applicationId: code, params: tmp(7706).extractActivityBookmarkParams(url) };
          return obj;
        }
      }
    }
