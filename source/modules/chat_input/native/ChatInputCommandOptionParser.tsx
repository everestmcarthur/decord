...
function findGameMentionTokens(text, name, items) {
  ...
  const arr2 = callback2(name);
  const found = items.filter((arg0) => arg0 !== arr2);
  const mapped = found.map(callback2);
  ...
  if (-1 !== index) {
    do {
      ...
      let someResult = mapped.some((arg0) => {
        let startsWithResult = arg0.length > arr2.length;
        if (startsWithResult) {
...
function buildGameMentionResult(id) {
  ...
  const obj = { location: 0, length: callback2(id.name).length, icon: null, iconSize: 16, iconCornerRadius: 4, iconSpacing: 4 };
...
function findAllTimestampPillMatches(size, content) {
  closure_0 = content;
  if (0 === size.size) {
    return [];
  } else {
    ...
    const items1 = [];
    function _loop3(arg0) {
      closure_0 = arg0;
      for (const item10010 of tmp) {
        let tmp2 = items1;
        let obj = { location: null, pillText: null, mention: null };
        obj[0] = item10010;
        obj[1] = arg0;
        let tmp3 = closure_2;
        obj[2] = closure_2;
...
({ GAME_MENTION_INPUT_PREFIX: c10, TIMESTAMP_MENTION_INPUT_PREFIX: unpackModuleId, formatGameMentionRaw: closure_12, formatGameMentionToken: map1 } = regExp);
...
const here = "here";
const result = require("set").fileFinishedImporting("modules/chat_input/native/ChatInputCommandOptionParser.tsx");
          ...
          if (0 < options.length) {
            do {
              ...
              if (!hasItem) {
                ...
                obj1 = { type: null, option: null };
                let tmp13 = require;
                let tmp14 = dependencyMap;
                obj1[0] = addRule.ChatInputParseResultDataType.COMMAND_OPTION;
                obj1[1] = options[num];
                obj[2] = obj1;
          ...
          if (match[0].length > 1) {
            ...
            for (let num = 0; num < options.length; num = num + 1) {
              ...
              if (!hasItem) {
                ...
                obj[0] = addRule.ChatInputParseResultDataType.COMMAND_OPTION;
                obj[1] = options[num];
                obj[2] = obj;
                let arr = items.push(obj);
                let addResult = set.add(num);
...
export const getTextBeforeFirstOption = (arr) => {
  regex.lastIndex = 0;
  const match = regex.exec(arr);
  const obj = { match, text: null };
  let substr = arr;
  if (null != match) {
    substr = arr.slice(0, regex.lastIndex - match[0].length);
  }
  obj[1] = substr;
  return obj;
};
...
export const getUsernameHighlightNodes = function getUsernameHighlightNodes(channel, arg1) {
  ...
  if (null != match) {
    do {
      ...
      _require = str.trim();
      let tmp2 = match;
      if (null != users.find((text) => callback(text.text) === callback)) {
        obj = { location: null, length: null };
        obj[0] = match.index;
        obj[1] = match[0].length;
        let arr = items.push(obj);
  ...
  _require = (arg0) => arg0.split("#")[0];
  let match1 = regex4.exec(arg1);
  if (null != match1) {
    do {
      let str2 = match1[1];
      _require = str2.trim();
...
export const serializeComposerGameMentions = function serializeComposerGameMentions(content, mentionGames) {
  let items = [];
  ...
  function _loop2() {
    let arr = content;
    const name = arr2.name;
    arr2 = name;
    arr2 = closure_1_13(name);
    const found = arr2.filter((arg0) => arg0 !== arr2);
    const mapped = found.map(closure_1_13);
    items = [];
    let index = content.indexOf(arr2);
    if (-1 !== index) {
      do {
        arr2 = arr;
        content = index;
    ...
    content = closure_1_12(arr2.id);
    content = items.reduceRight((arr) => {
...
export const uniqueTimestampPillText = function uniqueTimestampPillText(mentionTimestamps, arg1, timestamp) {
  let value = mentionTimestamps.get(arg1);
  let tmp2 = arg1;
  if (null != value) {
    let tmp4 = arg1;
...
export const serializeComposerTimestampMentions = function serializeComposerTimestampMentions(content, mentionTimestamps) {
  const arr = findAllTimestampPillMatches(mentionTimestamps, content);
  ...
  return arr.reduce((arr, pillText) => {
    ...
    const sum = substr + callback(table[15]).unparseTimestamp(mention.timestamp, mention.format);
    return sum + arr.slice(_location + pillText.pillText.length);
  }, content);
};
export const getTimestampHighlightNodes = function getTimestampHighlightNodes(mentionTimestamps, content) {
...
export const getRoleHighlightNodes = (getGuildId) => {
  const items = [];
  if (closure_8.can(Permissions.MENTION_EVERYONE, getGuildId)) {
  ...
  function _loop5() {
    ...
    if (index < 0) {
      ...
      if ("\"" === arr[closure_3 + 1]) {
        ...
        closure_1_20.lastIndex = closure_3;
        const match = closure_1_20.exec(arr);
        if (null != match) {
          items = match[1].trim();
