...
// Dependencies: [32, 17, 4714, 5629, 2016, 4361, 1074, 5169, 1375, 12234, 4379, 12235, 1925, 1926, 8361, 5192, 7530, 10503, 1936, 576, 4875, 2]
...
// Module 12233 (ChatInputCommandOptionParser)
...
function findGameMentionTokens(text, name, items) {
  ...
  if (-1 !== index) {
    do {
      closure_0 = text;
      let someResult = mapped.some((item) => {
        let startsWithResult = item.length > arr2.length;
        if (startsWithResult) {
          startsWithResult = arr2.startsWith(item, closure_1);
        }
...
function findAllTimestampPillMatches(size, text) {
  closure_0 = text;
  if (0 === size.size) {
    return [];
  } else {
    const items = [];
    HermesBuiltin.arraySpread(size.entries(), 0);
    ...
    while (tmp5 !== undefined) {
      ...
      let mention = tmp10[1];
      let _loop3Result = _loop3(tmp10[0]);
      continue;
    }
    return items1;
  }
...
({ GAME_MENTION_INPUT_PREFIX: c10, TIMESTAMP_MENTION_INPUT_PREFIX: closure_11, formatGameMentionRaw: closure_12, formatGameMentionToken: map1 } = ChannelAutocompleteConstants);
...
const re15 = tmp3;
const re16 = /:((?:[\0-\x08\x0E-\x1F!-9;-\x9F\xA1-\u167F\u1681-\u1FFF\u200B-\u2027\u202A-\u202E\u2030-\u205E\u2060-\u2FFF\u3001-\uD7FF\uE000-\uFEFE\uFF00-\uFFFF]|[\uD800-\uDBFF][\uDC00-\uDFFF]|[\uD800-\uDBFF](?![\uDC00-\uDFFF])|(?:[^\uD800-\uDBFF]|^)[\uDC00-\uDFFF])+?(?:::skin-tone-[0-9])?):/g;
const re17 = /@((?:(?:[\0-\x08\x0E-\x1F!-\x9F\xA1-\u167F\u1681-\u1FFF\u200B-\u2027\u202A-\u202E\u2030-\u205E\u2060-\u2FFF\u3001-\uD7FF\uE000-\uFEFE\uFF00-\uFFFF]|[\uD800-\uDBFF][\uDC00-\uDFFF]|[\uD800-\uDBFF](?![\uDC00-\uDFFF])|(?:[^\uD800-\uDBFF]|^)[\uDC00-\uDFFF])(?:[\0-"\$-9;-\?A-\uD7FF\uE000-\uFFFF]|[\uD800-\uDBFF][\uDC00-\uDFFF]|[\uD800-\uDBFF](?![\uDC00-\uDFFF])|(?:[^\uD800-\uDBFF]|^)[\uDC00-\uDFFF])+)#([0-9]{4}))/g;
const re18 = /@([\.0-9_a-z]{2,32})/g;
const re19 = /@(((?:[\0-"\$-9;-\?A-\uD7FF\uE000-\uFFFF]|[\uD800-\uDBFF][\uDC00-\uDFFF]|[\uD800-\uDBFF](?![\uDC00-\uDFFF])|(?:[^\uD800-\uDBFF]|^)[\uDC00-\uDFFF]){1,100}))(?![0-9A-Z_a-z]*#)/g;
const re20 = /#(((?:[\0-\x08\x0E-\x1F!"\$-9;-\?A-\x9F\xA1-\u167F\u1681-\u1FFF\u200B-\u2027\u202A-\u202E\u2030-\u205E\u2060-\u2FFF\u3001-\uD7FF\uE000-\uFEFE\uFF00-\uFFFF]|[\uD800-\uDBFF][\uDC00-\uDFFF]|[\uD800-\uDBFF](?![\uDC00-\uDFFF])|(?:[^\uD800-\uDBFF]|^)[\uDC00-\uDFFF])+))/g;
const re21 = /^@(silent(?![^\s]))/;
          ...
          if (match[0].length > 1) {
            ...
            for (let num = 0; num < options.length; num = num + 1) {
              ...
              if (!hasItem) {
                ...
                let obj2 = { type: null, option: null };
                obj2.type = ChatInputParser.ChatInputParseResultDataType.COMMAND_OPTION;
                obj2.option = options[num];
                obj.data = obj2;
            ...
            do {
              ...
              if (!hasItem) {
                if (null != tmp16) {
                  ...
                  ({ location: obj6.location, location: _location } = tmp16);
                ...
                let obj3 = { type: null, option: null };
                obj3.type = ChatInputParser.ChatInputParseResultDataType.COMMAND_OPTION;
                obj3.option = options[num];
                obj2.data = obj3;
                let addResult = set.add(num);
                tmp8 = obj2;
              }
...
export const getCommandOptionValueEnd = function getCommandOptionValueEnd(c22, sum, activeCommand) {
  closure_0 = sum;
  const found = getMatchedOptions(c22, { activeCommand }).find((location) => location.location > closure_0);
  if (null != found) {
    let length = found.location - 1;
  } else {
    length = c22.length;
  }
  return length;
...
export const getEmojiHighlightNodes = function getEmojiHighlightNodes(channel, arg1) {
  if (0 === arg1.length) {
    ...
    const items = [];
    let match = re16.exec(arg1);
    let tmp11 = null;
    while (null != match) {
      let disambiguatedEmojiContext = tmp11;
      if (tmp11 == null) {
        disambiguatedEmojiContext = EmojiStore.getDisambiguatedEmojiContext(channel.getGuildId());
      }
      let str = match[1];
...
export const serializeComposerGameMentions = function serializeComposerGameMentions(text, mentionGames) {
  ...
  function _loop2() {
    ...
    closure_1 = closure_2_12(name.id);
  ...
  while (iter !== undefined) {
    ...
    if (0 !== locations.length) {
      ...
      for (const item10039 of locations) {
        let obj = { location: item10039, length: null, icon: null, iconSize: null, iconCornerRadius: null, iconSpacing: null };
        obj.length = length;
        obj.icon = icon;
...
export const TIMESTAMP_PILL_SENTINEL = "\u2060";
export const formatTimestampPillText = function formatTimestampPillText(formatted) {
  return "\u2060" + formatted;
};
export const uniqueTimestampPillText = function uniqueTimestampPillText(mentionTimestamps, arg1, timestamp) {
  value = mentionTimestamps.get(arg1);
  let tmp2 = arg1;
  if (null != value) {
    let tmp4 = arg1;
...
export { findAllTimestampPillMatches };
export const serializeComposerTimestampMentions = function serializeComposerTimestampMentions(text, mentionTimestamps) {
  const arr = findAllTimestampPillMatches(mentionTimestamps, text);
  const sorted = arr.sort((location, location2) => location2.location - location.location);
  return arr.reduce((arr, pillText) => {
    ({ location: _location, mention } = pillText);
    const substr = arr.slice(0, _location);
    const sum = substr + require("TimestampUtils").unparseTimestamp(mention.timestamp, mention.format);
    return sum + arr.slice(_location + pillText.pillText.length);
  }, text);
};
export const getTimestampHighlightNodes = function getTimestampHighlightNodes(mentionTimestamps, text) {
  return findAllTimestampPillMatches(mentionTimestamps, text).map((location) => ({ location: location.location, length: location.pillText.length }));
};
...
export const getSilentHighlightNodes = function getSilentHighlightNodes(arg0) {
  const items = [];
  const match = re21.exec(arg0);
  if (null != match) {
    const obj = { location: match.index, length: match[0].length };
    items.push(obj);
  }
  return items;
};
export const getChannelHighlightNodes = function getChannelHighlightNodes(channel, arg1) {
  ...
  function _loop5() {
    ...
    if (index < 0) {
      ...
      if ("\"" === arr[lastIndex + 1]) {
        const sum = lastIndex + 2;
        lastIndex = sum;
        let str2 = arr;
        if (sum < arr.length) {
          while (true) {
            let arr2 = closure_0;
            if ("\\" !== closure_0[lastIndex]) {
              str2 = arr2;
              if ("\"" === arr2[lastIndex]) {
                break;
              }
        ...
        re20.lastIndex = lastIndex;
        const match = re20.exec(arr);
        if (null != match) {
