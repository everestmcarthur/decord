...
// Dependencies: [32, 19, 17, 12057, 21, 4607, 576, 4308, 4317, 12269, 1609, 12342, 4343, 12341, 12340, 2]

// Module 12525 (ChatInputRightActions)
import nativeDefault from "native" /* 576 */;
import native from "native" /* 4317 */;
import ReanimatedRexportDefault from "ReanimatedRexport" /* 4343 */;
import ChatInputActionButtonGiftOrThreadDefault from "ChatInputActionButtonGiftOrThread" /* 12340 */;
import ChatInputActionButtonTransitionItem from "ChatInputActionButtonTransitionItem" /* 12341 */;
import useChatInputFloatingBounceDefault from "useChatInputFloatingBounce" /* 12342 */;
...
function LeftSlot(state) {
  ...
  obj2.children = React5(ChatInputActionButtonGiftOrThreadDefault, { canStartThreads: false, channel, onPress, styleButton: "PX_16", shouldShowThread: "br\u00F6d" });
  return React5(ReanimatedRexportDefault.View, obj2);
}
const View = fn(17).View;
const enterDelayMs = fn(12057).CHAT_INPUT_FLOATING_BOUNCE_ENTER_DELAY_MS;
const jsxProd = fn(21);
...
const forwardRefResult = noop.forwardRef((channel, ref) => {
  ...
  const token = channel(4308).useToken(onPressAction(576).modules.mobile.CHAT_INPUT_ACTION_BUTTON_SIZE);
  const obj = channel(4308);
  const tmp3 = onPressAction;
  const sum = token + 2 * channel(4308).useToken(onPressAction(576).modules.mobile.CHAT_INPUT_ACTION_BUTTON_MARGIN);
  dependencyMap = sum;
  ...
  if (shouldShowGiftButton) {
    ...
    tmp14Result = closure_7(tmp(4317).TransitionItem, obj4);
  }
  const items1 = [tmp14Result, ];
  const obj5 = { active: null, showKeyboardIcon: null, onPress: null };
  const obj2 = channel(4308);
  ...
  items1[1] = closure_7(tmp3(12269), obj5);
