...
// Dependencies: [32, 19, 17, 12032, 21, 4605, 576, 4306, 4315, 12244, 1609, 12317, 4341, 12316, 12315, 2]

// Module 12500 (ChatInputRightActions)
import nativeDefault from "native" /* 576 */;
import native from "native" /* 4315 */;
import ReanimatedRexportDefault from "ReanimatedRexport" /* 4341 */;
import ChatInputActionButtonGiftOrThreadDefault from "ChatInputActionButtonGiftOrThread" /* 12315 */;
import ChatInputActionButtonTransitionItem from "ChatInputActionButtonTransitionItem" /* 12316 */;
...
function LeftSlot(state) {
  ...
  obj2.children = React5(ChatInputActionButtonGiftOrThreadDefault, { canStartThreads: false, channel, onPress, styleButton: "PX_16", shouldShowThread: "Experiment Overrides" });
  return React5(ReanimatedRexportDefault.View, obj2);
}
const View = fn(17).View;
const enterDelayMs = fn(12032).CHAT_INPUT_FLOATING_BOUNCE_ENTER_DELAY_MS;
const jsxProd = fn(21);
...
const forwardRefResult = noop.forwardRef((channel, ref) => {
  ...
  const token = channel(4306).useToken(onPressAction(576).modules.mobile.CHAT_INPUT_ACTION_BUTTON_SIZE);
  const obj = channel(4306);
  const tmp3 = onPressAction;
  const sum = token + 2 * channel(4306).useToken(onPressAction(576).modules.mobile.CHAT_INPUT_ACTION_BUTTON_MARGIN);
  dependencyMap = sum;
  ...
  if (shouldShowGiftButton) {
    ...
    tmp14Result = closure_7(tmp(4315).TransitionItem, obj4);
  }
  ...
  items1[1] = closure_7(tmp3(12244), obj5);
