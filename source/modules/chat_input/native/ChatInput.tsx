...
// Function ID: 11989
// Name: ChatInput
// Dependencies: [5, 32, 19, 17, 7794, 11989, 9716, 7694, 11990, 4915, 7695, 4212, 4914, 9517, 11992, 1074, 1482, 1954, 1374, 1607, 21, 4574, 576, 1115, 11993, 4392, 4275, 11994, 504, 7283, 7176, 7775, 4310, 4441, 9578, 12014, 12015, 7859, 12016, 12017, 7235, 1109, 1610, 12060, 12061, 5152, 9339, 5142, 1256, 4542, 1242, 1609, 4754, 9536, 12065, 12226, 10667, 4439, 1873, 12227, 10365, 8700, 12261, 10682, 9336, 12264, 12265, 4244, 12281, 12282, 12283, 12284, 12290, 12291, 1091, 12292, 12293, 12297, 12312, 12317, 12320, 12413, 12415, 12435, 12437, 12438, 12451, 12455, 12459, 12460, 12461, 12462, 12463, 2]

// Module 11988 (ChatInput)
...
let useVoiceMessagesUIStore = fn(11990).useVoiceMessagesUIStore;
const DraftType = fn(4915).DraftType;
const useChatBottomManagerUIStore = fn(9517);
...
const forwardRefResult = noop.forwardRef((channel, ref) => {
  ...
  const mobileEmojiSuggestionsConfig = channel(11993).useMobileEmojiSuggestionsConfig({ location: "ChatInput" });
  let obj = channel(11993);
  const gradientValue = channel(4392).useGradientValue(channel(4392).GradientPercentage.END);
  let obj2 = channel(4392);
  const token = channel(4275).useToken(screenIndex(576).modules.mobile.CHAT_INPUT_ACTION_BUTTON_SIZE);
  let obj3 = channel(4275);
  let result = (channel(4275).useToken(screenIndex(576).modules.mobile.CHAT_INPUT_SEND_BUTTON_HEIGHT) - token) / 2;
  let obj4 = channel(4275);
  const token1 = channel(4275).useToken(screenIndex(576).modules.mobile.CHAT_INPUT_FLOATING_SCRIM_GRADIENT_HEIGHT);
  const tmp9 = closure_40(gradientValue, token1);
  let obj5 = channel(4275);
  let token2 = gradientValue;
  if (gradientValue == null) {
    token2 = obj6.useToken(screenIndex(576).colors.BACKGROUND_BASE_LOWER);
  }
  obj6 = channel(4275);
  const token3 = channel(4275).useToken(tmp5(576).modules.mobile.CHAT_INPUT_FLOATING_TYPING_GRADIENT_HEIGHT_REDUCED);
  let tmpResult = channel(4275);
  const token4 = channel(4275).useToken(tmp5(576).modules.mobile.CHAT_INPUT_FLOATING_INLINE_FULL_GRADIENT_HEIGHT);
  const tmpResult25 = channel(4275);
  const token5 = channel(4275).useToken(tmp5(576).modules.mobile.CHAT_INPUT_FLOATING_SCRIM_GRADIENT_HEIGHT_AT_BOTTOM);
  const tmpResult26 = channel(4275);
  ...
  const typingUserIdsForDisplay = channel(11994).useTypingUserIdsForDisplay(channel.id, 1);
  const tmp24 = closure_22(screenIndex);
  const tmpResult27 = channel(11994);
  let result1 = channel(11994).hasTypingIndicatorContent(channel, typingUserIdsForDisplay, tmp24);
  ...
  const tmpResult28 = channel(11994);
  ...
  const analyticsLocations = tmp5(7176)().analyticsLocations;
  let tmp34 = tmp20;
  if (null == threadCreationCallback) {
    tmp34 = null != stateFromStores;
  }
  if (!tmp34) {
    tmp34 = !tmp(7283).getIsActiveChannelOrUnarchivableThread(channel);
    const tmpResult33 = tmp(7283);
  }
  const tmpResult32 = channel(504);
  let canStartThread = channel(7283).useCanStartThread(channel);
  if (canStartThread) {
  ...
  const tmpResult34 = channel(7283);
  const tmp38 = channel(7775).useCanPostPollsInChannel(channel) && null == threadCreationCallback;
  const tmpResult35 = channel(7775);
  sharedValue = channel(4310).useSharedValue(token);
  const tmpResult36 = channel(4310);
  sharedValue1 = channel(4310).useSharedValue(token);
  ...
  const tmp42 = screenIndex(4441)();
  const tmp43 = useVoiceMessagesUIStore((startTimeMillis) => null != startTimeMillis.startTimeMillis);
  let result3 = !tmp20;
  let isAppLauncherEnabled = result3;
  if (null == threadCreationCallback) {
    isAppLauncherEnabled = tmp(9578).getIsAppLauncherEnabled(channel);
    const tmpResult38 = tmp(9578);
  }
  const tmpResult37 = channel(4310);
  ...
  ({ placeholder, accessibilityLabel } = screenIndex(12014)({ channel, isReadonly: !editable, isCreatingThread: null != threadCreationCallback }));
  const tmp47 = screenIndex(12014)({ channel, isReadonly: !editable, isCreatingThread: null != threadCreationCallback });
  function ze() {
  ...
  const animatedStyle = channel(4310).useAnimatedStyle(ze);
  ref = obj10.useRef(null);
  const tmpResult40 = channel(4310);
  const refreshChatInputCoachmark = channel(12015).useRefreshChatInputCoachmark({ disabled: !editable });
  let obj8 = { disabled: !editable };
  const tmpResult41 = channel(12015);
  const canUseScheduledMessages = channel(7859).useCanUseScheduledMessages();
  const tmpResult42 = channel(7859);
  const items10 = [memo1];
  ...
  const scheduledMessageDraftCoachmarkState = channel(12016).useScheduledMessageDraftCoachmarkState(obj9);
  ...
  const tmp55 = screenIndex(12017)({ chatInputProps: { analyticsLocations, canUpload, channel, defaultValue: memo, hasAttachmentsToUpload: stateFromStores2, pendingEdit: stateFromStores, pendingReply: stateFromStores1, screenIndex, secondaryTextFieldRef, threadCreationCallback }, chatInputTextFieldHeight: sharedValue1, ref });
  ...
  memo1 = obj10.useMemo(() => {
    let obj = {
      ...
      handleChangeContentSize(nativeEvent) {
        ...
        if (!obj.getIsChatInputHeightWorkletEnabled()) {
          ...
          const result = textFieldHeight.set(channel(12061).getChatInputHeightAnimationTiming(height, sharedValue.get()));
          const tmp2Result = channel(12061);
        }
      },
      ...
      handleMaxHeightChanged() {
        if (!obj.getIsChatInputHeightWorkletEnabled()) {
          ...
          if (0 !== textFieldContentSize) {
            const textFieldHeight = closure_1_13.state.current.textFieldHeight;
            const result = textFieldHeight.set(channel(12061).getChatInputHeightAnimationTiming(textFieldContentSize, sharedValue.get()));
            const tmpResult = channel(12061);
          }
        }
      },
        ...
        try {
          ...
          if (0 === c3) {
            ...
            if (1 === tmp5) {
              if (arg0 === 1) {
                ...
                if (closure_1_13.state.current.focused) {
                  if (closure_1_13.props.current.canUpload) {
                    c3 = 2;
                    c4 = 1;
                    const obj6 = { value: closure_0(5152).getImageDimensionsIfMissing(closure_129_0, closure_129_1, closure_129_2), done: false };
                    return obj6;
                  }
                }
                c4 = 3;
              ...
              const size = { uri: closure_129_0, originalUri: closure_129_0, width: closure_129_4.width, height: closure_129_4.height, mimeType: closure_129_3, platform: closure_0(5142).UploadPlatform.REACT_NATIVE, id: null };
              const obj7 = screenIndex(9339);
    ...
    obj.handlePressAction = function handlePressAction(arg0, arg1, current2) {
      if (constants.PHOTOS === arg1) {
        const result = channel(4542).triggerHapticFeedback(channel(4542).HapticFeedbackTypes.IMPACT_LIGHT);
        const obj21 = channel(4542);
        ...
        const keyboardType = channel(4441).getKeyboardType();
        if (keyboardType === channel(1609).KeyboardTypes.APP_LAUNCHER) {
          ...
          const keyboardType1 = tmp42(4441).getKeyboardType();
          ...
          const tmp42Result = tmp42(4441);
        }
        const obj24 = channel(4441);
      } else {
        let tmp35 = current2;
        if (tmp.APPS === arg1) {
          const result1 = channel(4542).triggerHapticFeedback(channel(4542).HapticFeedbackTypes.IMPACT_LIGHT);
          const obj12 = channel(4542);
          channel(4754).trackWithMetadata(constants2.APP_LAUNCHER_ENTRYPOINT_BUTTON_CLICKED);
          const obj13 = channel(4754);
          ...
          const result2 = c2(9536).dismissNewActivityIndicator();
          const obj16 = c2(9536);
          if (tmp35 == null) {
            tmp35 = null;
          }
          const result3 = channel(12065).setAppLauncherA11yFocusReturnRef(tmp35);
          ...
          const obj17 = channel(12065);
          const appDMApplication = channel(12226).getAppDMApplication(tmp30.props.current.channel);
          let name;
          ...
          const tmp23Result = channel(12226);
        } else if (tmp.ALL_PHOTOS === arg1) {
          const result4 = channel(4542).triggerHapticFeedback(channel(4542).HapticFeedbackTypes.IMPACT_LIGHT);
          const obj9 = channel(4542);
          const obj20 = {
            channel: closure_1_13.props.current.channel,
            uploadLimit,
            onDismissKeyboard() {
                  return closure_1_0(4439).dismissKeyboard();
                },
            onRestoreKeyboard() {
                  return closure_1_14.handleToggleKeyboard({ type: closure_0(1609).KeyboardTypes.SYSTEM });
                },
            onSelectFiles(items) {
                  closure_0(10667).addImagesFromPicker(closure_1_13.props.current.channel.id, items, closure_0(5142).UploadOrigin.IMAGE_PICKER);
                },
            draftType: registerViewTag.ChannelMessage
          };
          channel(10667).handleViewAllDialog(obj20);
          const obj10 = channel(10667);
        } else if (tmp.CAMERA === arg1) {
          const result5 = channel(4542).triggerHapticFeedback(channel(4542).HapticFeedbackTypes.IMPACT_LIGHT);
          const obj6 = channel(4542);
          const obj23 = {
            channel: closure_1_13.props.current.channel,
          ...
          channel(10667).handleCameraDialog(obj23);
          const obj7 = channel(10667);
        } else if (tmp.NITRO_GIFT === arg1) {
          const result6 = screenIndex(1873).markPotentialBadState();
          const obj = screenIndex(1873);
          const result7 = channel(4542).triggerHapticFeedback(channel(4542).HapticFeedbackTypes.IMPACT_LIGHT);
          const obj2 = channel(4542);
          if (obj3.isAndroid()) {
            tmp5(4439).dismissKeyboard();
            const tmp5Result = tmp5(4439);
          }
          obj3 = channel(1115);
          channel(10667).handleSelectGift(closure_1_13.props.current.analyticsLocations, closure_1_13.chatInput, tmp35);
          const tmp5Result2 = channel(10667);
        } else if (tmp.THREAD === arg1) {
          const result8 = channel(4542).triggerHapticFeedback(channel(4542).HapticFeedbackTypes.IMPACT_LIGHT);
          const obj28 = channel(4542);
          channel(10667).handleSelectThread(closure_1_13.props.current.channel, closure_1_13.chatInput);
          const obj29 = channel(10667);
        }
      }
    };
    obj.handlePollsPress = function handlePollsPress() {
      ...
      channel(4439).dismissKeyboard();
      const obj4 = channel(4439);
      channel(12227).openCreatePollModal({
        channel: closure_1_13.props.current.channel,
        onCancel() {
          return closure_1_14.handleToggleKeyboard({ type: closure_0(1609).KeyboardTypes.SYSTEM });
        }
    ...
    obj.handleAttachPress = function handleAttachPress() {
      const result = channel(4542).triggerHapticFeedback(channel(4542).HapticFeedbackTypes.IMPACT_LIGHT);
      const obj = channel(4542);
      channel(10667).handleAttachFile({
        channel: closure_1_13.props.current.channel,
        uploadLimit,
        onDismissKeyboard() {
        ...
        onSelectFiles(items) {
          closure_0(10667).addImagesFromPicker(props.props.current.channel.id, items, closure_0(5142).UploadOrigin.FILE_ATTACHMENT);
        }
      });
    };
    obj.handlePressExpression = function handlePressExpression(context) {
      const result = channel(10365).initiateEmojiInteraction(EmojiInteractionPoint.ChatInputExpressionPressed);
      const obj = channel(10365);
    ...
    obj.handleSelectionOrTextChange = function handleSelectionOrTextChange(nativeEvent) {
      ...
      if (closure_1_13.state.current.editId !== editId) {
        ...
        channel(8700).hideContextMenu();
        const obj3 = channel(8700);
      }
    };
    ...
    obj.handleTextOrFocusChange = function handleTextOrFocusChange(text, focused) {
      if (text.length > 0) {
        if (!focused) {
          const maxMessageLength = channel(9336).getMaxMessageLength();
          if (tmp16) {
        ...
        if (isCoachmarkVisible.isOpen()) {
          screenIndex(10682).hideNativeMenu();
          const obj = screenIndex(10682);
        }
  ...
  const tmp61 = screenIndex(12060)({ textFieldHeight: sharedValue1, textFieldMinHeight: sharedValue });
  ...
  const tmpResult44 = channel(12016);
  if (editable) {
    ...
    let result2 = result3;
    if (!tmp20) {
      result2 = tmp(4244).isPremiumGiftingSupported();
      const tmpResult45 = tmp(4244);
    }
    ...
    tmp65Result = tmp65(tmp5(12265), obj13);
    let tmp5Result = tmp5(12265);
  }
  ...
  const tmp66 = closure_37(screenIndex(12264), { canUpload, channelId: channel.id, screenIndex });
  obj15.customKeyboard = channel(12282).PORTAL_KEYBOARD_PLACEHOLDER_INSTANCE;
  ...
  const items19 = [closure_37(screenIndex(12281), obj15), closure_37(screenIndex(12283), { keyboardType: tmp42, onSelectKeyboard: memo1.handleToggleKeyboard, ref: tmp55.chatInputCover })];
  obj14.children = items19;
  let obj16 = { keyboardType: tmp42, onSelectKeyboard: memo1.handleToggleKeyboard, ref: tmp55.chatInputCover };
  const tmp5Result7 = screenIndex(12281);
  if (editable) {
    ...
    let tmp65Result15 = tmp65(tmp5(12284), obj17);
    const tmp5Result8 = tmp5(12284);
  } else {
    tmp65Result15 = null;
  }
  let obj18 = { collapsable: false, onLayout: callback2, style: null, children: null };
  const items20 = [screenIndex(12290)({ isCreatingThread: null != threadCreationCallback }), tmp9.overflowVisible, ];
  ...
  if (!result1) {
    ...
    tmp65Result16 = tmp65(tmp(12291).ChatInputScrimGradient, obj19);
  }
  ...
  items21[2] = closure_37(channel(12292).ChatInputAccessibilityDivider, {});
  ...
  if (tmp21) {
    ...
    tmp65Result18 = tmp65(tmp5(12293), obj21);
    const tmp5Result9 = tmp5(12293);
  }
  ...
  if (isResourceChannel) {
    const obj26 = { channel };
    tmp65Result22 = tmp65(tmp5(12312), obj26, channel.id);
  }
  items21[5] = tmp65Result22;
  items21[6] = closure_37(channel(12317).MemberActionsChatInputBannerGuardedOuter, { channel });
  items21[7] = closure_37(channel(12320).DoubleTapToReactChatInputBanner, { channel });
  let tmp65Result23 = null;
  if (tmp22) {
    const obj27 = { channelId: channel.id };
    tmp65Result23 = tmp65(tmp5(12413), obj27);
  }
  ...
  if (tmp42 !== channel(1609).KeyboardTypes.EXPRESSION) {
    ...
    tmp65Result24 = tmp65(tmp5(12415), obj28);
  }
  items21[9] = tmp65Result24;
  items21[10] = closure_37(screenIndex(12435), { ref: tmp55.chatInputAppCommandManager, canOnlyUseTextCommands: null != stateFromStores1, channel, chatInputRef: tmp55.chatInput, chatInputStateRef: tmp55.state, commandsDisabled: tmp34 });
  ...
  items24[1] = closure_37(screenIndex(12438), { channel });
  ...
  const items26 = [closure_37(screenIndex(12451), { channel, chatInputRef: tmp55.chatInput, pendingEdit: stateFromStores, pendingReply: stateFromStores1 }), , ];
  ...
  if (enabled) {
    ...
    enabled = tmp65(tmp(12455).EmojiSuggestionBarLarge, obj33);
  }
  items26[1] = enabled;
  ...
  obj39.style = items28;
  const items29 = [tmp73, closure_37(screenIndex(12459), { analyticsLocations, ref: tmp55.chatInputCharCounter })];
  ...
  if (editable) {
    ...
    if (!tmp20) {
      result3 = tmp(4244).isPremiumGiftingSupported();
      const tmpResult48 = tmp(4244);
    }
    ...
    obj41.children = tmp65(tmp5(12460), obj43);
    tmp65Result26 = tmp65(tmp76, obj41);
    const tmp5Result11 = tmp5(12460);
  }
  ...
  if (enabled2) {
    ...
    enabled2 = tmp65(tmp(12461).EmojiSuggestionBarSmall, obj44);
  }
  items24[3] = enabled2;
  obj30.children = closure_38(closure_39, { children: items24 });
  items21[11] = closure_37(screenIndex(12437), obj30);
  ...
  if (null != refreshChatInputCoachmark) {
    ...
    tmp65Result27 = tmp65(tmp5(12015), obj45);
    const tmp5Result12 = tmp5(12015);
  }
  items21[12] = tmp65Result27;
  items21[13] = closure_37(screenIndex(12462), { buttonRef: ref, isVisible: isCoachmarkVisible, onDismiss: dismissCoachmark });
  ...
  if (null == threadCreationCallback) {
    ...
    tmp65Result28 = tmp65(tmp5(12463), obj46);
  }
