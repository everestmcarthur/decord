...
// Function ID: 12052
// Name: ChatInput
// Dependencies: [5, 32, 19, 17, 7852, 12052, 9775, 7752, 12053, 4947, 7753, 4245, 4946, 9576, 12055, 1074, 1482, 1954, 1374, 1607, 21, 4606, 576, 1115, 12056, 4425, 4308, 12057, 504, 7342, 7235, 7833, 4343, 4474, 9637, 12077, 12078, 7917, 12079, 12080, 7294, 1109, 1610, 12123, 12124, 5189, 9397, 5179, 1256, 4574, 1242, 1609, 4786, 9595, 12128, 12289, 10724, 4472, 1873, 12290, 10422, 8758, 12324, 10739, 9394, 12327, 12328, 4277, 12344, 12345, 12346, 12347, 12353, 12354, 1091, 12355, 12356, 12360, 12375, 12380, 12383, 12476, 12478, 12498, 12500, 12501, 12514, 12518, 12522, 12523, 12524, 12525, 12526, 2]

// Module 12051 (ChatInput)
...
import DiceRollStore from "DiceRollStore" /* 12052 */;
...
const forwardRefResult = noop.forwardRef((channel, ref) => {
  ...
  const mobileEmojiSuggestionsConfig = channel(12056).useMobileEmojiSuggestionsConfig({ location: "ChatInput" });
  let obj = channel(12056);
  ...
  const typingUserIdsForDisplay = channel(12057).useTypingUserIdsForDisplay(channel.id, 1);
  const tmp24 = closure_22(screenIndex);
  const tmpResult27 = channel(12057);
  let result1 = channel(12057).hasTypingIndicatorContent(channel, typingUserIdsForDisplay, tmp24);
  ...
  const tmpResult28 = channel(12057);
  ...
  const analyticsLocations = tmp5(7235)().analyticsLocations;
  let tmp34 = tmp20;
  if (null == threadCreationCallback) {
    tmp34 = null != stateFromStores;
  }
  if (!tmp34) {
    tmp34 = !tmp(7342).getIsActiveChannelOrUnarchivableThread(channel);
    const tmpResult33 = tmp(7342);
  }
  const tmpResult32 = channel(504);
  let canStartThread = channel(7342).useCanStartThread(channel);
  if (canStartThread) {
  ...
  const tmpResult34 = channel(7342);
  const tmp38 = channel(7833).useCanPostPollsInChannel(channel) && null == threadCreationCallback;
  const tmpResult35 = channel(7833);
  ...
  if (null == threadCreationCallback) {
    isAppLauncherEnabled = tmp(9637).getIsAppLauncherEnabled(channel);
    const tmpResult38 = tmp(9637);
  }
  ...
  ({ placeholder, accessibilityLabel } = screenIndex(12077)({ channel, isReadonly: !editable, isCreatingThread: null != threadCreationCallback }));
  const tmp47 = screenIndex(12077)({ channel, isReadonly: !editable, isCreatingThread: null != threadCreationCallback });
  function ze() {
  ...
  const refreshChatInputCoachmark = channel(12078).useRefreshChatInputCoachmark({ disabled: !editable });
  let obj8 = { disabled: !editable };
  const tmpResult41 = channel(12078);
  const canUseScheduledMessages = channel(7917).useCanUseScheduledMessages();
  const tmpResult42 = channel(7917);
  const items10 = [memo1];
  ...
  const scheduledMessageDraftCoachmarkState = channel(12079).useScheduledMessageDraftCoachmarkState(obj9);
  ...
  const tmp55 = screenIndex(12080)({ chatInputProps: { analyticsLocations, canUpload, channel, defaultValue: memo, hasAttachmentsToUpload: stateFromStores2, pendingEdit: stateFromStores, pendingReply: stateFromStores1, screenIndex, secondaryTextFieldRef, threadCreationCallback }, chatInputTextFieldHeight: sharedValue1, ref });
  ...
  memo1 = obj10.useMemo(() => {
    let obj = {
      ...
      handleChangeContentSize(nativeEvent) {
        ...
        if (!obj.getIsChatInputHeightWorkletEnabled()) {
          ...
          const result = textFieldHeight.set(channel(12124).getChatInputHeightAnimationTiming(height, sharedValue.get()));
          const tmp2Result = channel(12124);
        }
      },
      ...
      handleMaxHeightChanged() {
        if (!obj.getIsChatInputHeightWorkletEnabled()) {
          ...
          if (0 !== textFieldContentSize) {
            const textFieldHeight = closure_1_13.state.current.textFieldHeight;
            const result = textFieldHeight.set(channel(12124).getChatInputHeightAnimationTiming(textFieldContentSize, sharedValue.get()));
            const tmpResult = channel(12124);
          }
        }
      },
    ...
    closure_0 = closure_4(function*(arg0, value) {
      ...
      if (1 === tmp5) {
        if (arg0 === 1) {
          ...
          if (closure_1_13.state.current.focused) {
            if (closure_1_13.props.current.canUpload) {
              c3 = 2;
              c4 = 1;
              return { value: closure_0(5189).getImageDimensionsIfMissing(closure_129_0, closure_129_1, closure_129_2), done: false };
            }
          }
          c4 = 3;
        }
        ...
        const size = { uri: closure_129_0, originalUri: closure_129_0, width: closure_129_4.width, height: closure_129_4.height, mimeType: closure_129_3, platform: closure_0(5179).UploadPlatform.REACT_NATIVE, id: null };
        size.id = closure_0(1256).v4();
        obj8.file = size;
        obj8.draftType = ChannelMessage.ChannelMessage;
        screenIndex(9397).addFile(obj8);
        closure_0(1256);
        screenIndex(9397);
      }
      return value;
    ...
    obj.handlePressAction = function handlePressAction(arg0, arg1, current2) {
      if (constants.PHOTOS === arg1) {
        const result = channel(4574).triggerHapticFeedback(channel(4574).HapticFeedbackTypes.IMPACT_LIGHT);
        const obj21 = channel(4574);
        ...
        let tmp35 = current2;
        if (tmp.APPS === arg1) {
          const result1 = channel(4574).triggerHapticFeedback(channel(4574).HapticFeedbackTypes.IMPACT_LIGHT);
          const obj12 = channel(4574);
          channel(4786).trackWithMetadata(constants2.APP_LAUNCHER_ENTRYPOINT_BUTTON_CLICKED);
          const obj13 = channel(4786);
          ...
          const result2 = c2(9595).dismissNewActivityIndicator();
          const obj16 = c2(9595);
          if (tmp35 == null) {
            tmp35 = null;
          }
          const result3 = channel(12128).setAppLauncherA11yFocusReturnRef(tmp35);
          ...
          const obj17 = channel(12128);
          const appDMApplication = channel(12289).getAppDMApplication(tmp30.props.current.channel);
          let name;
          ...
          const tmp23Result = channel(12289);
        } else if (tmp.ALL_PHOTOS === arg1) {
          const result4 = channel(4574).triggerHapticFeedback(channel(4574).HapticFeedbackTypes.IMPACT_LIGHT);
          const obj9 = channel(4574);
          const obj20 = {
            ...
            onSelectFiles(items) {
                  closure_0(10724).addImagesFromPicker(closure_1_13.props.current.channel.id, items, closure_0(5179).UploadOrigin.IMAGE_PICKER);
                },
            draftType: registerViewTag.ChannelMessage
          };
          channel(10724).handleViewAllDialog(obj20);
          const obj10 = channel(10724);
        } else if (tmp.CAMERA === arg1) {
          const result5 = channel(4574).triggerHapticFeedback(channel(4574).HapticFeedbackTypes.IMPACT_LIGHT);
          const obj6 = channel(4574);
          const obj23 = {
            channel: closure_1_13.props.current.channel,
          ...
          channel(10724).handleCameraDialog(obj23);
          const obj7 = channel(10724);
        } else if (tmp.NITRO_GIFT === arg1) {
          const result6 = screenIndex(1873).markPotentialBadState();
          const obj = screenIndex(1873);
          const result7 = channel(4574).triggerHapticFeedback(channel(4574).HapticFeedbackTypes.IMPACT_LIGHT);
          const obj2 = channel(4574);
          if (obj3.isAndroid()) {
            tmp5(4472).dismissKeyboard();
            const tmp5Result = tmp5(4472);
          }
          obj3 = channel(1115);
          channel(10724).handleSelectGift(closure_1_13.props.current.analyticsLocations, closure_1_13.chatInput, tmp35);
          const tmp5Result2 = channel(10724);
        } else if (tmp.THREAD === arg1) {
          const result8 = channel(4574).triggerHapticFeedback(channel(4574).HapticFeedbackTypes.IMPACT_LIGHT);
          const obj28 = channel(4574);
          channel(10724).handleSelectThread(closure_1_13.props.current.channel, closure_1_13.chatInput);
          const obj29 = channel(10724);
        }
      }
    };
    obj.handlePollsPress = function handlePollsPress() {
      ...
      channel(12290).openCreatePollModal({
        channel: closure_1_13.props.current.channel,
        onCancel() {
          return closure_1_14.handleToggleKeyboard({ type: closure_0(1609).KeyboardTypes.SYSTEM });
        }
    ...
    obj.handleAttachPress = function handleAttachPress() {
      const result = channel(4574).triggerHapticFeedback(channel(4574).HapticFeedbackTypes.IMPACT_LIGHT);
      const obj = channel(4574);
      channel(10724).handleAttachFile({
        channel: closure_1_13.props.current.channel,
        uploadLimit,
        onDismissKeyboard() {
        ...
        onSelectFiles(items) {
          closure_0(10724).addImagesFromPicker(props.props.current.channel.id, items, closure_0(5179).UploadOrigin.FILE_ATTACHMENT);
        }
      });
    };
    obj.handlePressExpression = function handlePressExpression(context) {
      const result = channel(10422).initiateEmojiInteraction(EmojiInteractionPoint.ChatInputExpressionPressed);
      const obj = channel(10422);
    ...
    obj.handleSelectionOrTextChange = function handleSelectionOrTextChange(nativeEvent) {
      ...
      if (closure_1_13.state.current.editId !== editId) {
        ...
        channel(8758).hideContextMenu();
        const obj3 = channel(8758);
      }
    };
    ...
    obj.handleTextOrFocusChange = function handleTextOrFocusChange(text, focused) {
      if (text.length > 0) {
        if (!focused) {
          const maxMessageLength = channel(9394).getMaxMessageLength();
          if (tmp16) {
        ...
        if (isCoachmarkVisible.isOpen()) {
          screenIndex(10739).hideNativeMenu();
          const obj = screenIndex(10739);
        }
  ...
  const tmp61 = screenIndex(12123)({ textFieldHeight: sharedValue1, textFieldMinHeight: sharedValue });
  ...
  const tmpResult44 = channel(12079);
  if (editable) {
    ...
    tmp65Result = tmp65(tmp5(12328), obj13);
    let tmp5Result = tmp5(12328);
  }
  ...
  const tmp66 = closure_37(screenIndex(12327), { canUpload, channelId: channel.id, screenIndex });
  obj15.customKeyboard = channel(12345).PORTAL_KEYBOARD_PLACEHOLDER_INSTANCE;
  ...
  const items19 = [closure_37(screenIndex(12344), obj15), closure_37(screenIndex(12346), { keyboardType: tmp42, onSelectKeyboard: memo1.handleToggleKeyboard, ref: tmp55.chatInputCover })];
  obj14.children = items19;
  let obj16 = { keyboardType: tmp42, onSelectKeyboard: memo1.handleToggleKeyboard, ref: tmp55.chatInputCover };
  const tmp5Result7 = screenIndex(12344);
  if (editable) {
    ...
    let tmp65Result15 = tmp65(tmp5(12347), obj17);
    const tmp5Result8 = tmp5(12347);
  } else {
    tmp65Result15 = null;
  }
  let obj18 = { collapsable: false, onLayout: callback2, style: null, children: null };
  const items20 = [screenIndex(12353)({ isCreatingThread: null != threadCreationCallback }), tmp9.overflowVisible, ];
  ...
  if (!result1) {
    ...
    tmp65Result16 = tmp65(tmp(12354).ChatInputScrimGradient, obj19);
  }
  ...
  items21[2] = closure_37(channel(12355).ChatInputAccessibilityDivider, {});
  ...
  if (tmp21) {
    ...
    tmp65Result18 = tmp65(tmp5(12356), obj21);
    const tmp5Result9 = tmp5(12356);
  }
  ...
  if (result1) {
    ...
    tmp65Result19 = tmp65(tmp(12354).ChatInputScrimGradient, obj23);
  }
  const items22 = [tmp65Result19, , ];
  let tmp65Result20 = null;
  if (null == threadCreationCallback) {
    let obj24 = { channel, screenIndex };
    tmp65Result20 = tmp65(tmp5(12057), obj24);
  }
  ...
  if (tmpResult47.isIOS()) {
    ...
    tmp65Result21 = tmp65(tmp5(12360), obj25);
  }
  ...
  if (isResourceChannel) {
    const obj26 = { channel };
    tmp65Result22 = tmp65(tmp5(12375), obj26, channel.id);
  }
  items21[5] = tmp65Result22;
  items21[6] = closure_37(channel(12380).MemberActionsChatInputBannerGuardedOuter, { channel });
  items21[7] = closure_37(channel(12383).DoubleTapToReactChatInputBanner, { channel });
  let tmp65Result23 = null;
  if (tmp22) {
    const obj27 = { channelId: channel.id };
    tmp65Result23 = tmp65(tmp5(12476), obj27);
  }
  ...
  if (tmp42 !== channel(1609).KeyboardTypes.EXPRESSION) {
    ...
    tmp65Result24 = tmp65(tmp5(12478), obj28);
  }
  items21[9] = tmp65Result24;
  items21[10] = closure_37(screenIndex(12498), { ref: tmp55.chatInputAppCommandManager, canOnlyUseTextCommands: null != stateFromStores1, channel, chatInputRef: tmp55.chatInput, chatInputStateRef: tmp55.state, commandsDisabled: tmp34 });
  ...
  items24[1] = closure_37(screenIndex(12501), { channel });
  ...
  const items26 = [closure_37(screenIndex(12514), { channel, chatInputRef: tmp55.chatInput, pendingEdit: stateFromStores, pendingReply: stateFromStores1 }), , ];
  ...
  if (enabled) {
    ...
    enabled = tmp65(tmp(12518).EmojiSuggestionBarLarge, obj33);
  }
  items26[1] = enabled;
  ...
  obj39.style = items28;
  const items29 = [tmp73, closure_37(screenIndex(12522), { analyticsLocations, ref: tmp55.chatInputCharCounter })];
  ...
  if (editable) {
    ...
    obj41.children = tmp65(tmp5(12523), obj43);
    tmp65Result26 = tmp65(tmp76, obj41);
    const tmp5Result11 = tmp5(12523);
  }
  ...
  if (enabled2) {
    ...
    enabled2 = tmp65(tmp(12524).EmojiSuggestionBarSmall, obj44);
  }
  items24[3] = enabled2;
  obj30.children = closure_38(closure_39, { children: items24 });
  items21[11] = closure_37(screenIndex(12500), obj30);
  ...
  if (null != refreshChatInputCoachmark) {
    ...
    tmp65Result27 = tmp65(tmp5(12078), obj45);
    const tmp5Result12 = tmp5(12078);
  }
  items21[12] = tmp65Result27;
  items21[13] = closure_37(screenIndex(12525), { buttonRef: ref, isVisible: isCoachmarkVisible, onDismiss: dismissCoachmark });
  ...
  if (null == threadCreationCallback) {
    ...
    tmp65Result28 = tmp65(tmp5(12526), obj46);
  }
