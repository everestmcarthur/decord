...
// Dependencies: [19, 17, 1372, 8550, 21, 4640, 4990, 1115, 4653, 4337, 5210, 6727, 5693, 9903, 9889, 4636, 1177, 9904, 9905, 4358, 5705, 2]
// Exports: ChannelOverwritesCheckboxItem

// Module 9902 (ChannelOverwritesItem)
...
function RemoveIcon(item) {
  ...
  if (null != channelId) {
    ...
    let obj2 = { style: prop };
    obj.children = closure_7(item(6727).CircleXIcon, obj2);
    tmp3Result = tmp3(item(5210).PressableOpacity, obj);
  }
  return tmp3Result;
}
...
function MemberItem(arg0) {
  ...
  if (item.rowType === RowType.OWNER) {
    const obj3 = { size: tmp5(1177).Icon.Sizes.REFRESH_SMALL_16, source: _modDef9904, disableColor: true, style: tmp.ownerIcon };
...
function EmptyRoleItem(item) {
  ...
  const tmp = closure_9();
  obj.icon = React5(native.Icon, { source: _modDef9905, color: item.colorString, size: native.IconSizes.MEDIUM, style: closure_9().roleIcon });
  obj.label = item.name;
  return React5(TableRow.TableRow, obj);
}
