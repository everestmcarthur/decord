...
// Dependencies: [32, 19, 17, 9302, 1980, 4355, 15414, 1074, 1374, 21, 4722, 576, 4689, 18105, 1897, 1115, 7430, 15440, 4718, 9288, 563, 15436, 5757, 10491, 1612, 14106, 1484, 15422, 18093, 14101, 15421, 12, 5794, 18107, 18108, 38, 10061, 18142, 18103, 2]
// Exports: default

// Module 18104 (GuildSettingsRoleSubscriptionTiers)
...
import GuildSettingsRoleSubscriptionContainerDefault from "GuildSettingsRoleSubscriptionContainer" /* 18103 */;
...
function EditListingButton(editStateId) {
  ...
  if (undefined === first) {
    const obj5 = {
      ...
      onLongPress() {
          ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(18105, dependencyMap.paths), "TierArchiveOrDelete", { editStateId, guildId, groupListingId });
        },
      children: null
    };
    ...
    const items2 = [closure_17(tmp2(4718).Text, obj9), ];
    ...
    items3[3] = closure_17(tmp2(4718).Text, obj12);
    obj10.children = items3;
    items2[1] = closure_18(closure_7, obj10);
    obj8.children = items2;
    items1[1] = closure_18(closure_7, obj8);
    items1[2] = closure_17(tmp2(10491).PencilIcon, {});
    obj5.children = items1;
    obj11.children = closure_18(Row, obj5);
    return closure_17(closure_7, obj11);
  } else if (null != first2) {
    const intl = tmp2(1115).intl;
    const obj13 = { price: tmp2(7430).formatPrice(first, first2.currency), interval: null };
    const tmp2Result = tmp2(7430);
    obj13.interval = tmp2(15440).formatPlanInterval(first2);
    let formatToPlainStringResult = intl.formatToPlainString(tmp2(1115).t.CgmBaG, obj13);
    const tmp2Result4 = tmp2(15440);
  } else {
    const intl2 = tmp2(1115).intl;
    const obj14 = { price: tmp2(7430).formatPrice(first, constants.USD), interval: null };
    const tmp2Result5 = tmp2(7430);
    const obj15 = { interval: SubscriptionIntervalTypes.MONTH, interval_count: 1 };
    obj14.interval = tmp2(15440).formatPlanInterval(obj15);
    formatToPlainStringResult = intl2.formatToPlainString(tmp2(1115).t.CgmBaG, obj14);
    const tmp2Result6 = tmp2(15440);
  }
}
function GuildSettingsRoleSubscriptionsTiersInner(guildId) {
  ...
  const layoutEffect = editStateIds.useLayoutEffect(() => {
    navigation.setOptions({
      headerTitle() {
        ...
        return closure_1_17(guildId(5794).NavigatorHeader, obj);
      }
    });
  });
  ...
  onPress = editStateIds.useCallback(() => {
    if (guildEligibleForTierTemplates) {
      ...
      const result = tmp(18107).pushTierTemplateSelectionScene(navigation, obj);
      const tmpResult = tmp(18107);
    } else {
      ...
      const result1 = tmp(18108).openTierCreationModal(obj2);
      const tmpResult2 = tmp(18108);
    }
  }, items1);
      ...
      mapped = memo.map((editStateId) => {
        ...
        return closure_1_17(EditListingButton, {
          ...
          onPress() {
            ...
            guildEligibleForTierTemplates(18107).pushTierEditScene(navigation, {
              groupListingId: id,
              initialEditStateId,
...
const MAX_SUBSCRIPTION_TIERS = fn(15414).MAX_SUBSCRIPTION_TIERS;
