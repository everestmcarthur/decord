...
// Dependencies: [32, 19, 17, 9096, 1979, 4206, 15237, 1074, 1373, 21, 4574, 576, 4541, 17772, 1896, 1114, 7248, 15263, 4570, 9082, 563, 15259, 5601, 10282, 1611, 13924, 1483, 15245, 17760, 13919, 15244, 12, 5638, 17774, 17775, 38, 9848, 17809, 17770, 2]
// Exports: default

// Module 17771 (GuildSettingsRoleSubscriptionTiers)
...
function EditListingButton(editStateId) {
  ...
  if (undefined === first) {
    const obj5 = {
      ...
      onLongPress() {
          ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(17772, dependencyMap.paths), "TierArchiveOrDelete", { editStateId, guildId, groupListingId });
        },
      children: null
    };
    ...
    const items2 = [closure_17(tmp2(4570).Text, obj9), ];
    ...
    items3[3] = closure_17(tmp2(4570).Text, obj12);
    obj10.children = items3;
    items2[1] = closure_18(closure_7, obj10);
    obj8.children = items2;
    items1[1] = closure_18(closure_7, obj8);
    items1[2] = closure_17(tmp2(10282).PencilIcon, {});
    obj5.children = items1;
    obj11.children = closure_18(Row, obj5);
    return closure_17(closure_7, obj11);
  } else if (null != first2) {
    const intl = tmp2(1114).intl;
    const obj13 = { price: tmp2(7248).formatPrice(first, first2.currency), interval: null };
    const tmp2Result = tmp2(7248);
    obj13.interval = tmp2(15263).formatPlanInterval(first2);
    let formatToPlainStringResult = intl.formatToPlainString(tmp2(1114).t.CgmBaG, obj13);
    const tmp2Result4 = tmp2(15263);
  } else {
    const intl2 = tmp2(1114).intl;
    const obj14 = { price: tmp2(7248).formatPrice(first, constants.USD), interval: null };
    const tmp2Result5 = tmp2(7248);
    const obj15 = { interval: SubscriptionIntervalTypes.MONTH, interval_count: 1 };
    obj14.interval = tmp2(15263).formatPlanInterval(obj15);
    formatToPlainStringResult = intl2.formatToPlainString(tmp2(1114).t.CgmBaG, obj14);
    const tmp2Result6 = tmp2(15263);
  }
}
function GuildSettingsRoleSubscriptionsTiersInner(guildId) {
  ...
  const layoutEffect = editStateIds.useLayoutEffect(() => {
    navigation.setOptions({
      headerTitle() {
        ...
        return closure_1_17(guildId(5638).NavigatorHeader, obj);
      }
    });
  });
  ...
  onPress = editStateIds.useCallback(() => {
    if (guildEligibleForTierTemplates) {
      ...
      const result = tmp(17774).pushTierTemplateSelectionScene(navigation, obj);
      const tmpResult = tmp(17774);
    } else {
      ...
      const result1 = tmp(17775).openTierCreationModal(obj2);
      const tmpResult2 = tmp(17775);
    }
  }, items1);
      ...
      mapped = memo.map((editStateId) => {
        ...
        return closure_1_17(EditListingButton, {
          ...
          onPress() {
            ...
            guildEligibleForTierTemplates(17774).pushTierEditScene(navigation, {
              groupListingId: id,
              initialEditStateId,
...
const MAX_SUBSCRIPTION_TIERS = fn(15237).MAX_SUBSCRIPTION_TIERS;
