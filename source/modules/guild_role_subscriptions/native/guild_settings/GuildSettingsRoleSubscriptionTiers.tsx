...
// Dependencies: [32, 19, 17, 9218, 1980, 4272, 15320, 1074, 1374, 21, 4640, 576, 4607, 18011, 1897, 1115, 7348, 15346, 4636, 9204, 563, 15342, 5675, 10397, 1612, 14012, 1484, 15328, 17999, 14007, 15327, 12, 5712, 18013, 18014, 38, 9967, 18048, 18009, 2]
// Exports: default

// Module 18010 (GuildSettingsRoleSubscriptionTiers)
...
function EditListingButton(editStateId) {
  ...
  if (undefined === first) {
    const obj5 = {
      ...
      onLongPress() {
          ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(18011, dependencyMap.paths), "TierArchiveOrDelete", { editStateId, guildId, groupListingId });
        },
      children: null
    };
    ...
    const items2 = [closure_17(tmp2(4636).Text, obj9), ];
    ...
    items3[3] = closure_17(tmp2(4636).Text, obj12);
    obj10.children = items3;
    items2[1] = closure_18(closure_7, obj10);
    obj8.children = items2;
    items1[1] = closure_18(closure_7, obj8);
    items1[2] = closure_17(tmp2(10397).PencilIcon, {});
    obj5.children = items1;
    obj11.children = closure_18(Row, obj5);
    return closure_17(closure_7, obj11);
  } else if (null != first2) {
    const intl = tmp2(1115).intl;
    const obj13 = { price: tmp2(7348).formatPrice(first, first2.currency), interval: null };
    const tmp2Result = tmp2(7348);
    obj13.interval = tmp2(15346).formatPlanInterval(first2);
    let formatToPlainStringResult = intl.formatToPlainString(tmp2(1115).t.CgmBaG, obj13);
    const tmp2Result4 = tmp2(15346);
  } else {
    const intl2 = tmp2(1115).intl;
    const obj14 = { price: tmp2(7348).formatPrice(first, constants.USD), interval: null };
    const tmp2Result5 = tmp2(7348);
    const obj15 = { interval: SubscriptionIntervalTypes.MONTH, interval_count: 1 };
    obj14.interval = tmp2(15346).formatPlanInterval(obj15);
    formatToPlainStringResult = intl2.formatToPlainString(tmp2(1115).t.CgmBaG, obj14);
    const tmp2Result6 = tmp2(15346);
  }
}
function GuildSettingsRoleSubscriptionsTiersInner(guildId) {
  ...
  const layoutEffect = editStateIds.useLayoutEffect(() => {
    navigation.setOptions({
      headerTitle() {
        ...
        return closure_1_17(guildId(5712).NavigatorHeader, obj);
      }
    });
  });
  ...
  onPress = editStateIds.useCallback(() => {
    if (guildEligibleForTierTemplates) {
      ...
      const result = tmp(18013).pushTierTemplateSelectionScene(navigation, obj);
      const tmpResult = tmp(18013);
    } else {
      ...
      const result1 = tmp(18014).openTierCreationModal(obj2);
      const tmpResult2 = tmp(18014);
    }
  }, items1);
      ...
      mapped = memo.map((editStateId) => {
        ...
        return closure_1_17(EditListingButton, {
          ...
          onPress() {
            ...
            guildEligibleForTierTemplates(18013).pushTierEditScene(navigation, {
              groupListingId: id,
              initialEditStateId,
...
const MAX_SUBSCRIPTION_TIERS = fn(15320).MAX_SUBSCRIPTION_TIERS;
