...
// Dependencies: [32, 19, 17, 9196, 1980, 4272, 15299, 1074, 1374, 21, 4639, 576, 4606, 17891, 1897, 1115, 7341, 15325, 4635, 9182, 563, 15321, 5672, 10381, 1612, 13991, 1484, 15307, 17879, 13986, 15306, 12, 5709, 17893, 17894, 38, 9948, 17928, 17889, 2]
// Exports: default

// Module 17890 (GuildSettingsRoleSubscriptionTiers)
...
function EditListingButton(editStateId) {
  ...
  if (undefined === first) {
    const obj5 = {
      ...
      onLongPress() {
          ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(17891, dependencyMap.paths), "TierArchiveOrDelete", { editStateId, guildId, groupListingId });
        },
      children: null
    };
    ...
    const items2 = [closure_17(tmp2(4635).Text, obj9), ];
    ...
    items3[3] = closure_17(tmp2(4635).Text, obj12);
    obj10.children = items3;
    items2[1] = closure_18(closure_7, obj10);
    obj8.children = items2;
    items1[1] = closure_18(closure_7, obj8);
    items1[2] = closure_17(tmp2(10381).PencilIcon, {});
    obj5.children = items1;
    obj11.children = closure_18(Row, obj5);
    return closure_17(closure_7, obj11);
  } else if (null != first2) {
    const intl = tmp2(1115).intl;
    const obj13 = { price: tmp2(7341).formatPrice(first, first2.currency), interval: null };
    const tmp2Result = tmp2(7341);
    obj13.interval = tmp2(15325).formatPlanInterval(first2);
    let formatToPlainStringResult = intl.formatToPlainString(tmp2(1115).t.CgmBaG, obj13);
    const tmp2Result4 = tmp2(15325);
  } else {
    const intl2 = tmp2(1115).intl;
    const obj14 = { price: tmp2(7341).formatPrice(first, constants.USD), interval: null };
    const tmp2Result5 = tmp2(7341);
    const obj15 = { interval: SubscriptionIntervalTypes.MONTH, interval_count: 1 };
    obj14.interval = tmp2(15325).formatPlanInterval(obj15);
    formatToPlainStringResult = intl2.formatToPlainString(tmp2(1115).t.CgmBaG, obj14);
    const tmp2Result6 = tmp2(15325);
  }
}
function GuildSettingsRoleSubscriptionsTiersInner(guildId) {
  ...
  const layoutEffect = editStateIds.useLayoutEffect(() => {
    navigation.setOptions({
      headerTitle() {
        ...
        return closure_1_17(guildId(5709).NavigatorHeader, obj);
      }
    });
  });
  ...
  onPress = editStateIds.useCallback(() => {
    if (guildEligibleForTierTemplates) {
      ...
      const result = tmp(17893).pushTierTemplateSelectionScene(navigation, obj);
      const tmpResult = tmp(17893);
    } else {
      ...
      const result1 = tmp(17894).openTierCreationModal(obj2);
      const tmpResult2 = tmp(17894);
    }
  }, items1);
      ...
      mapped = memo.map((editStateId) => {
        ...
        return closure_1_17(EditListingButton, {
          ...
          onPress() {
            ...
            guildEligibleForTierTemplates(17893).pushTierEditScene(navigation, {
              groupListingId: id,
              initialEditStateId,
...
const MAX_SUBSCRIPTION_TIERS = fn(15299).MAX_SUBSCRIPTION_TIERS;
