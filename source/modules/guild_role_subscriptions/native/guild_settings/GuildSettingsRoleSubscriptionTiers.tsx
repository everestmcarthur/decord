...
// Dependencies: [32, 19, 17, 9133, 1979, 4237, 15286, 1074, 1373, 21, 4605, 576, 4572, 17831, 1896, 1114, 7286, 15312, 4601, 9119, 563, 15308, 5637, 10320, 1611, 13973, 1483, 15294, 17819, 13968, 15293, 12, 5674, 17833, 17834, 38, 9886, 17868, 17829, 2]
// Exports: default

// Module 17830 (GuildSettingsRoleSubscriptionTiers)
...
function EditListingButton(editStateId) {
  ...
  if (undefined === first) {
    const obj5 = {
      ...
      onLongPress() {
          ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(17831, dependencyMap.paths), "TierArchiveOrDelete", { editStateId, guildId, groupListingId });
        },
      children: null
    };
    ...
    const items2 = [closure_17(tmp2(4601).Text, obj9), ];
    ...
    items3[3] = closure_17(tmp2(4601).Text, obj12);
    ...
    const obj13 = { price: tmp2(7286).formatPrice(first, first2.currency), interval: null };
    const tmp2Result = tmp2(7286);
    obj13.interval = tmp2(15312).formatPlanInterval(first2);
    let formatToPlainStringResult = intl.formatToPlainString(tmp2(1114).t.CgmBaG, obj13);
    const tmp2Result4 = tmp2(15312);
  } else {
    const intl2 = tmp2(1114).intl;
    const obj14 = { price: tmp2(7286).formatPrice(first, constants.USD), interval: null };
    const tmp2Result5 = tmp2(7286);
    const obj15 = { interval: SubscriptionIntervalTypes.MONTH, interval_count: 1 };
    obj14.interval = tmp2(15312).formatPlanInterval(obj15);
    formatToPlainStringResult = intl2.formatToPlainString(tmp2(1114).t.CgmBaG, obj14);
    const tmp2Result6 = tmp2(15312);
  }
}
function GuildSettingsRoleSubscriptionsTiersInner(guildId) {
  ...
  onPress = editStateIds.useCallback(() => {
    if (guildEligibleForTierTemplates) {
      ...
      const result = tmp(17833).pushTierTemplateSelectionScene(navigation, obj);
      const tmpResult = tmp(17833);
    } else {
      ...
      const result1 = tmp(17834).openTierCreationModal(obj2);
      const tmpResult2 = tmp(17834);
    }
  }, items1);
      ...
      mapped = memo.map((editStateId) => {
        ...
        return closure_1_17(EditListingButton, {
          ...
          onPress() {
            ...
            guildEligibleForTierTemplates(17833).pushTierEditScene(navigation, {
              groupListingId: id,
              initialEditStateId,
