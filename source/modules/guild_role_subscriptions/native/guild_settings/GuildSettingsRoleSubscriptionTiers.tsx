...
// Dependencies: [32, 19, 17, 9207, 1980, 4270, 15315, 1074, 1374, 21, 4638, 576, 4605, 17947, 1897, 1115, 7344, 15341, 4634, 9193, 563, 15337, 5673, 10389, 1612, 14004, 1484, 15323, 17935, 13999, 15322, 12, 5710, 17949, 17950, 38, 9956, 17984, 17945, 2]
// Exports: default

// Module 17946 (GuildSettingsRoleSubscriptionTiers)
...
function EditListingButton(editStateId) {
  ...
  if (undefined === first) {
    const obj5 = {
      ...
      onLongPress() {
          ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(17947, dependencyMap.paths), "TierArchiveOrDelete", { editStateId, guildId, groupListingId });
        },
      children: null
    };
    ...
    const items2 = [closure_17(tmp2(4634).Text, obj9), ];
...
function GuildSettingsRoleSubscriptionsTiersInner(guildId) {
  ...
  const layoutEffect = editStateIds.useLayoutEffect(() => {
    navigation.setOptions({
      headerTitle() {
        ...
        return closure_1_17(guildId(5710).NavigatorHeader, obj);
      }
    });
  });
  ...
  onPress = editStateIds.useCallback(() => {
    if (guildEligibleForTierTemplates) {
      ...
      const result = tmp(17949).pushTierTemplateSelectionScene(navigation, obj);
      ...
      const result1 = tmp(17950).openTierCreationModal(obj2);
      const tmpResult2 = tmp(17950);
    }
  }, items1);
      ...
      mapped = memo.map((editStateId) => {
        ...
        return closure_1_17(EditListingButton, {
          ...
          onPress() {
            ...
            guildEligibleForTierTemplates(17949).pushTierEditScene(navigation, {
              groupListingId: id,
              initialEditStateId,
...
const MAX_SUBSCRIPTION_TIERS = fn(15315).MAX_SUBSCRIPTION_TIERS;
