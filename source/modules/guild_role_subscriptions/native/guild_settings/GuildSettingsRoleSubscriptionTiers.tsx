...
// Dependencies: [32, 19, 17, 9154, 1979, 4239, 15266, 1074, 1373, 21, 4606, 576, 4573, 17822, 1896, 1114, 7307, 15292, 4602, 9140, 563, 15288, 5638, 10339, 1611, 13951, 1483, 15274, 17810, 13946, 15273, 12, 5675, 17824, 17825, 38, 9907, 17859, 17820, 2]
// Exports: default

// Module 17821 (GuildSettingsRoleSubscriptionTiers)
...
import GuildSettingsRoleSubscriptionContainerDefault from "GuildSettingsRoleSubscriptionContainer" /* 17820 */;
...
function EditListingButton(editStateId) {
  ...
  if (undefined === first) {
    const obj5 = {
      ...
      onLongPress() {
          ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(17822, dependencyMap.paths), "TierArchiveOrDelete", { editStateId, guildId, groupListingId });
        },
      children: null
    };
    ...
    const items2 = [closure_17(tmp2(4602).Text, obj9), ];
    ...
    items3[3] = closure_17(tmp2(4602).Text, obj12);
    ...
    const obj13 = { price: tmp2(7307).formatPrice(first, first2.currency), interval: null };
    const tmp2Result = tmp2(7307);
    obj13.interval = tmp2(15292).formatPlanInterval(first2);
    let formatToPlainStringResult = intl.formatToPlainString(tmp2(1114).t.CgmBaG, obj13);
    const tmp2Result4 = tmp2(15292);
  } else {
    const intl2 = tmp2(1114).intl;
    const obj14 = { price: tmp2(7307).formatPrice(first, constants.USD), interval: null };
    const tmp2Result5 = tmp2(7307);
    const obj15 = { interval: SubscriptionIntervalTypes.MONTH, interval_count: 1 };
    obj14.interval = tmp2(15292).formatPlanInterval(obj15);
    formatToPlainStringResult = intl2.formatToPlainString(tmp2(1114).t.CgmBaG, obj14);
    const tmp2Result6 = tmp2(15292);
  }
}
function GuildSettingsRoleSubscriptionsTiersInner(guildId) {
  ...
  onPress = editStateIds.useCallback(() => {
    if (guildEligibleForTierTemplates) {
      ...
      const result = tmp(17824).pushTierTemplateSelectionScene(navigation, obj);
      const tmpResult = tmp(17824);
    } else {
      ...
      const result1 = tmp(17825).openTierCreationModal(obj2);
      const tmpResult2 = tmp(17825);
    }
  }, items1);
      ...
      mapped = memo.map((editStateId) => {
        ...
        return closure_1_17(EditListingButton, {
          ...
          onPress() {
            ...
            guildEligibleForTierTemplates(17824).pushTierEditScene(navigation, {
              groupListingId: id,
              initialEditStateId,
