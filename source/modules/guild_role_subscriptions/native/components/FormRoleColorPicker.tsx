...
// Dependencies: [19, 1074, 21, 4636, 4603, 16386, 1896, 13983, 14684, 1091, 2]
// Exports: default

// Module 17884 (FormRoleColorPicker)
...
export default function FormRoleColorPicker(color) {
  ...
  const callback = noop.useCallback(() => {
    ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(16386, dependencyMap.paths), "RoleColorPicker", { color, onSelect: onChange });
  }, items);
  const obj = { leading: null, label: null, disabled: null, onPress: null };
  const tmp = closure_6();
  obj.leading = jsx(onChange(14684), { color, style: tmp.rowColorBlock, onSelect: callback });
  const obj2 = { color, style: tmp.rowColorBlock, onSelect: callback };
  const tmp3 = onChange(13983);
