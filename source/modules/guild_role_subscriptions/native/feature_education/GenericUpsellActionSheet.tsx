...
// Dependencies: [19, 17, 1955, 21, 4640, 576, 8318, 7264, 5675, 7268, 4636, 1177, 5062, 2]
// Exports: default

// Module 16405 (GenericUpsellActionSheet)
...
export default function GenericUpsellActionSheet(markAsDismissed) {
  ...
  const tmp = closure_7();
  const bottomSheetRef1 = markAsDismissed(8318).useBottomSheetRef();
  ...
  const items = [closure_5(FastImageDefault, { source: imageSource, style: tmp.image }), closure_5(markAsDismissed(7268).ActionSheetHeaderBar, { variant: "floating", onPress: bottomSheetClose })];
  obj3.children = items;
  obj2.header = closure_6(View, obj3);
  const obj5 = { style: tmp.content, children: null };
  const items1 = [closure_5(markAsDismissed(4636).Text, { accessibilityRole: "header", variant: "heading-xl/medium", color: "mobile-text-heading-primary", children: header }), closure_5(markAsDismissed(1177).Spacer, { size: 12 }), closure_5(markAsDismissed(4636).Text, { style: tmp.description, variant: "text-sm/medium", color: "text-default", children: body }), closure_5(markAsDismissed(5062).Button, { variant: "primary", grow: true, onPress: onCTAPress, text: cta })];
  obj5.children = items1;
  obj2.children = closure_6(View, obj5);
  return closure_5(markAsDismissed(7264).BottomSheet, obj2);
};
