...
// Dependencies: [32, 19, 17, 1958, 21, 4638, 576, 7089, 4634, 4605, 16682, 1897, 10486, 15350, 1177, 504, 4791, 1115, 5113, 15337, 16677, 5673, 16683, 2]
// Exports: default

// Module 16681 (GuildRoleSubscriptionPurchasePreviewCard)
...
function ChannelBenefitShowCase(description) {
  ...
  if (null != stateFromStores) {
    ...
    const obj3 = { size: tmp(1177).Icon.Sizes.REFRESH_SMALL_16, source: tmp(5113).getChannelIcon(stateFromStores) };
    ...
    items2[2] = closure_8(tmp(4634).Text, obj4);
...
export default function GuildRoleSubscriptionPurchasePreviewCard(listingId) {
  ...
  const formattedSubscriptionPlan = listingId(16677).useFormattedSubscriptionPlan(listingId);
  const obj6 = listingId(16677);
  const tmp11 = guildId;
  if (str == null) {
    str = "";
  }
  const items = [closure_8(guildId(5673), { source: { uri: str }, style: tmp.image }), closure_8(listingId(1177).Spacer, { size: 16 }), ];
  const obj10 = { children: null };
  const items1 = [closure_8(listingId(4634).Text, { variant: "heading-md/semibold", color: "mobile-text-heading-primary", children: _slicedToArray(obj2.useName(listingId), 1)[0] }), closure_8(listingId(1177).Spacer, { size: 4 }), closure_8(listingId(4634).Text, { variant: "heading-md/medium", color: "text-default", children: formattedSubscriptionPlan })];
  obj10.children = items1;
  items[2] = closure_9(closure_6, obj10);
  obj8.children = items;
  const items2 = [closure_9(closure_6, obj8), closure_8(listingId(1177).Spacer, { size: 16 }), closure_8(tmp11(16683), { listingId }), ];
  ...
  if (tmp8Result6) {
    ...
    obj11.children = tmp8(tmp4(10486).GappedList, obj12);
    items3[1] = tmp10(tmp9, obj11);
    const obj45 = {
      onPress() {
          const obj = ActionSheetActionCreatorsDefault;
          obj.openLazy(asyncRequireImpl(16682, dependencyMap.paths), "PurchaseCard:" + listingId, { listingId, guildId });
        }
    };
