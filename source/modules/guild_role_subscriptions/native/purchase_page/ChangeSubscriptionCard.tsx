...
// Dependencies: [32, 19, 17, 1074, 21, 4605, 576, 4601, 1611, 15308, 4196, 7202, 1114, 1178, 16630, 4808, 16638, 1896, 4572, 2]
// Exports: default

// Module 16637 (ChangeSubscriptionCard)
...
export default function ChangeSubscriptionCard(activeSubscription) {
  ...
  const obj3 = _modDef4196(activeSubscription.currentPeriodEnd);
  const obj4 = { style: null, children: null };
  const items = [tmp.container, ];
  const formatResult = _modDef4196(activeSubscription.currentPeriodEnd).format("MMMM Do");
  ...
  const items1 = [closure_7(activeSubscription(4601).Text, obj6), closure_7(activeSubscription(1178).Spacer, { size: 16 }), , ];
  ...
  items1[2] = closure_8(activeSubscription(4601).Text, obj7);
  let tmp7Result = null;
  if (activeSubscription.status !== SubscriptionStatusTypes.CANCELED) {
    ...
    obj11.onPress = function onPress() {
      ModalActionCreatorsDefault.pushLazy(asyncRequireImpl(16638, dependencyMap.paths), { subscriptionId: activeSubscription.id });
      const obj2 = { subscriptionId: activeSubscription.id };
      ActionSheetActionCreatorsDefault.hideActionSheet();
    };
    items3[1] = tmp5(tmp6(16630).ArrowButton, obj11);
  ...
  return closure_7(activeSubscription(7202).BottomSheet, { startExpanded: true, children: closure_8(tmp8, obj4) });
};
