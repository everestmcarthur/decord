...
// Dependencies: [32, 19, 17, 1074, 21, 4722, 576, 4718, 1612, 15436, 4314, 7346, 1115, 1177, 16804, 4925, 16812, 1897, 4689, 2]
// Exports: default

// Module 16811 (ChangeSubscriptionCard)
...
export default function ChangeSubscriptionCard(activeSubscription) {
  ...
  const obj3 = _modDef4314(activeSubscription.currentPeriodEnd);
  const obj4 = { style: null, children: null };
  const items = [tmp.container, ];
  const formatResult = _modDef4314(activeSubscription.currentPeriodEnd).format("MMMM Do");
  ...
  const items1 = [closure_7(activeSubscription(4718).Text, obj6), closure_7(activeSubscription(1177).Spacer, { size: 16 }), , ];
  ...
  items1[2] = closure_8(activeSubscription(4718).Text, obj7);
  let tmp7Result = null;
  if (activeSubscription.status !== SubscriptionStatusTypes.CANCELED) {
    ...
    obj11.onPress = function onPress() {
      ModalActionCreatorsDefault.pushLazy(asyncRequireImpl(16812, dependencyMap.paths), { subscriptionId: activeSubscription.id });
      const obj2 = { subscriptionId: activeSubscription.id };
      ActionSheetActionCreatorsDefault.hideActionSheet();
    };
    items3[1] = tmp5(tmp6(16804).ArrowButton, obj11);
  ...
  return closure_7(activeSubscription(7346).BottomSheet, { startExpanded: true, children: closure_8(tmp8, obj4) });
};
