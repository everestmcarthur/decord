...
// Dependencies: [5, 32, 19, 5544, 4272, 15322, 15299, 1074, 1374, 504, 5683, 4881, 15323, 15324, 1092, 4270, 15325, 15306, 15326, 1248, 38, 5605, 7362, 12, 10468, 15327, 1255, 2]
...
// Module 15321 (GuildRoleSubscriptionListingEditStateUtils)
import utils_ColorUtils from "utils/ColorUtils" /* 1092 */;
import v1 from "v1" /* 1255 */;
import RolePermissionUtils from "RolePermissionUtils" /* 4270 */;
import StoreUtils from "StoreUtils" /* 4881 */;
import GuildRoleSubscriptionsHooks from "GuildRoleSubscriptionsHooks" /* 15306 */;
import useSubscriptionRoleDefault from "useSubscriptionRole" /* 15323 */;
import Contants from "Contants" /* 15324 */;
import useTrialIntervalOptionsDefault from "useTrialIntervalOptions" /* 15326 */;
            ...
            if (undefined !== tierEmojiIds) {
              ...
              closure_129_15 = closure_129_13.map((item) => {
                ...
                if (null != customEmojiById) {
                  ...
                  obj2.roles = items;
                  return guildId(10468).updateEmoji(obj2);
                }
              });
              closure_129_16 = closure_129_14.map((item) => {
                ...
                if (null != customEmojiById) {
                  ...
                  if (found.length > 0) {
                    ...
                    let updateEmojiResult = guildId(10468).updateEmoji(obj3);
                    const obj2 = guildId(10468);
                  } else {
                    updateEmojiResult = guildId(10468).deleteEmoji(guildId, customEmojiById.id);
                    const obj = guildId(10468);
                  }
...
const GuildRoleSubscriptionEditStore = fn(15322);
({ AllChannelAccessOptions: closure_9, useEditStateStore: c10 } = GuildRoleSubscriptionEditStore);
let closure_11 = fn(15299).GuildRoleSubscriptionBenefitTypes;
...
export const usePriceTier = function usePriceTier(editStateId) {
  ...
  const items = [GuildRoleSubscriptionsStore];
  const stateFromStores = require("initialize").useStateFromStores(items, () => GuildRoleSubscriptionsStore.getSubscriptionListing(closure_0));
  const tmp2 = priceTier(5683)(() => first);
  const items1 = [stateFromStores, tmp2];
  let memo = noop.useMemo(() => closure_3(c2), items1);
  _require = editStateId;
  priceTier = "priceTier";
...
export const useDescription = function useDescription(arg0) {
  ...
  const items = [GuildRoleSubscriptionsStore];
  const stateFromStores = require("initialize").useStateFromStores(items, () => GuildRoleSubscriptionsStore.getSubscriptionListing(closure_0));
  const tmp2 = description(5683)(() => first);
  const items1 = [stateFromStores, tmp2];
  let memo = noop.useMemo(() => closure_3(c2), items1);
  _require = arg0;
...
export const useRoleColor = function useRoleColor(editStateId, guildId) {
  const tmp = roleColor(15323)(guildId, editStateId);
  editStateId = tmp;
  const items = [tmp];
  let memo = noop.useMemo(() => {
    let color;
...
export const useChannelAccessFormat = function useChannelAccessFormat(editStateId, guildId) {
  const tmp = channelAccessFormat(15323)(guildId, editStateId);
  const items = [tmp];
  let memo = noop.useMemo(() => {
    if (null == closure_0) {
...
export const useChannelBenefits = function useChannelBenefits(listingId) {
  ...
  const items = [GuildRoleSubscriptionsStore];
  const stateFromStores = require("initialize").useStateFromStores(items, () => GuildRoleSubscriptionsStore.getSubscriptionListing(closure_0));
  const tmp2 = channelBenefits(5683)(() => first);
  const items1 = [stateFromStores, tmp2];
  let memo = noop.useMemo(() => closure_3(c2), items1);
  _require = listingId;
...
export const useIntangibleBenefits = function useIntangibleBenefits(listingId) {
  ...
  const items = [GuildRoleSubscriptionsStore];
  const stateFromStores = require("initialize").useStateFromStores(items, () => GuildRoleSubscriptionsStore.getSubscriptionListing(closure_0));
  const tmp2 = intangibleBenefits(5683)(() => first);
  const items1 = [stateFromStores, tmp2];
  let memo = noop.useMemo(() => closure_3(c2), items1);
  _require = listingId;
...
export const useTierEmojiIds = function useTierEmojiIds(listingId, guildId) {
  _require = guildId;
  const tmp = tierEmojiIds(15323)(guildId, listingId);
              ...
              const obj6 = {
                value: (function updateListingFromEditState(editStateId) {
                            ...
                            if (null != channelBenefits) {
                              ...
                              const found = benefits.filter(closure_1_0(15325).isChannelBenefit);
                              const found1 = benefits1.filter(closure_1_0(15325).isIntangibleBenefit);
                              if (channelBenefits == null) {
                            ...
                            if (!obj2.isEmpty(obj)) {
                              ...
                              subscriptionListing = closure_1_2(7362).updateSubscriptionListing(obj4);
                              const obj3 = closure_1_2(7362);
                            }
