...
// Dependencies: [5, 32, 19, 5547, 4272, 15343, 15320, 1074, 1374, 504, 5686, 4882, 15344, 15345, 1092, 4270, 15346, 15327, 15347, 1248, 38, 5608, 7369, 12, 10484, 15348, 1255, 2]
...
// Module 15342 (GuildRoleSubscriptionListingEditStateUtils)
...
import useSubscriptionRoleDefault from "useSubscriptionRole" /* 15344 */;
import Contants from "Contants" /* 15345 */;
import useTrialIntervalOptionsDefault from "useTrialIntervalOptions" /* 15347 */;
            ...
            if (undefined !== tierEmojiIds) {
              ...
              closure_129_15 = closure_129_13.map((item) => {
                ...
                if (null != customEmojiById) {
                  ...
                  obj2.roles = items;
                  return guildId(10484).updateEmoji(obj2);
                }
              });
              closure_129_16 = closure_129_14.map((item) => {
                ...
                if (null != customEmojiById) {
                  ...
                  if (found.length > 0) {
                    ...
                    let updateEmojiResult = guildId(10484).updateEmoji(obj3);
                    const obj2 = guildId(10484);
                  } else {
                    updateEmojiResult = guildId(10484).deleteEmoji(guildId, customEmojiById.id);
                    const obj = guildId(10484);
                  }
...
const GuildRoleSubscriptionEditStore = fn(15343);
({ AllChannelAccessOptions: closure_9, useEditStateStore: c10 } = GuildRoleSubscriptionEditStore);
let closure_11 = fn(15320).GuildRoleSubscriptionBenefitTypes;
...
export const usePriceTier = function usePriceTier(editStateId) {
  ...
  const items = [GuildRoleSubscriptionsStore];
  const stateFromStores = require("initialize").useStateFromStores(items, () => GuildRoleSubscriptionsStore.getSubscriptionListing(closure_0));
  const tmp2 = priceTier(5686)(() => first);
  const items1 = [stateFromStores, tmp2];
  let memo = noop.useMemo(() => closure_3(c2), items1);
  _require = editStateId;
  priceTier = "priceTier";
...
export const useDescription = function useDescription(arg0) {
  ...
  const items = [GuildRoleSubscriptionsStore];
  const stateFromStores = require("initialize").useStateFromStores(items, () => GuildRoleSubscriptionsStore.getSubscriptionListing(closure_0));
  const tmp2 = description(5686)(() => first);
  const items1 = [stateFromStores, tmp2];
  let memo = noop.useMemo(() => closure_3(c2), items1);
  _require = arg0;
...
export const useRoleColor = function useRoleColor(editStateId, guildId) {
  const tmp = roleColor(15344)(guildId, editStateId);
  editStateId = tmp;
  const items = [tmp];
  let memo = noop.useMemo(() => {
    let color;
...
export const useChannelAccessFormat = function useChannelAccessFormat(editStateId, guildId) {
  const tmp = channelAccessFormat(15344)(guildId, editStateId);
  const items = [tmp];
  let memo = noop.useMemo(() => {
    if (null == closure_0) {
...
export const useChannelBenefits = function useChannelBenefits(listingId) {
  ...
  const items = [GuildRoleSubscriptionsStore];
  const stateFromStores = require("initialize").useStateFromStores(items, () => GuildRoleSubscriptionsStore.getSubscriptionListing(closure_0));
  const tmp2 = channelBenefits(5686)(() => first);
  const items1 = [stateFromStores, tmp2];
  let memo = noop.useMemo(() => closure_3(c2), items1);
  _require = listingId;
...
export const useIntangibleBenefits = function useIntangibleBenefits(listingId) {
  ...
  const items = [GuildRoleSubscriptionsStore];
  const stateFromStores = require("initialize").useStateFromStores(items, () => GuildRoleSubscriptionsStore.getSubscriptionListing(closure_0));
  const tmp2 = intangibleBenefits(5686)(() => first);
  const items1 = [stateFromStores, tmp2];
  let memo = noop.useMemo(() => closure_3(c2), items1);
  _require = listingId;
...
export const useTierEmojiIds = function useTierEmojiIds(listingId, guildId) {
  _require = guildId;
  const tmp = tierEmojiIds(15344)(guildId, listingId);
              ...
              const obj6 = {
                value: (function updateListingFromEditState(editStateId) {
                            ...
                            if (null != channelBenefits) {
                              ...
                              const found = benefits.filter(closure_1_0(15346).isChannelBenefit);
                              const found1 = benefits1.filter(closure_1_0(15346).isIntangibleBenefit);
                              if (channelBenefits == null) {
                            ...
                            if (!obj2.isEmpty(obj)) {
                              ...
                              subscriptionListing = closure_1_2(7369).updateSubscriptionListing(obj4);
                              const obj3 = closure_1_2(7369);
                            }
