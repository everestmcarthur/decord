...
// Dependencies: [5, 32, 19, 5545, 4270, 15338, 15315, 1074, 1374, 504, 5684, 4880, 15339, 15340, 1092, 4268, 15341, 15322, 15342, 1248, 38, 5606, 7365, 12, 10476, 15343, 1255, 2]
...
// Module 15337 (GuildRoleSubscriptionListingEditStateUtils)
import utils_ColorUtils from "utils/ColorUtils" /* 1092 */;
import v1 from "v1" /* 1255 */;
import RolePermissionUtils from "RolePermissionUtils" /* 4268 */;
import StoreUtils from "StoreUtils" /* 4880 */;
import GuildRoleSubscriptionsHooks from "GuildRoleSubscriptionsHooks" /* 15322 */;
import useSubscriptionRoleDefault from "useSubscriptionRole" /* 15339 */;
import Contants from "Contants" /* 15340 */;
import useTrialIntervalOptionsDefault from "useTrialIntervalOptions" /* 15342 */;
            ...
            if (undefined !== tierEmojiIds) {
              ...
              closure_129_15 = closure_129_13.map((item) => {
                ...
                if (null != customEmojiById) {
                  ...
                  obj2.roles = items;
                  return guildId(10476).updateEmoji(obj2);
                }
              });
              closure_129_16 = closure_129_14.map((item) => {
                ...
                if (null != customEmojiById) {
                  ...
                  if (found.length > 0) {
                    ...
                    let updateEmojiResult = guildId(10476).updateEmoji(obj3);
                    const obj2 = guildId(10476);
                  } else {
                    updateEmojiResult = guildId(10476).deleteEmoji(guildId, customEmojiById.id);
                    const obj = guildId(10476);
                  }
...
const GuildRoleSubscriptionEditStore = fn(15338);
({ AllChannelAccessOptions: closure_9, useEditStateStore: c10 } = GuildRoleSubscriptionEditStore);
let closure_11 = fn(15315).GuildRoleSubscriptionBenefitTypes;
...
export const usePriceTier = function usePriceTier(editStateId) {
  ...
  const items = [GuildRoleSubscriptionsStore];
  const stateFromStores = require("initialize").useStateFromStores(items, () => GuildRoleSubscriptionsStore.getSubscriptionListing(closure_0));
  const tmp2 = priceTier(5684)(() => first);
  const items1 = [stateFromStores, tmp2];
  let memo = noop.useMemo(() => closure_3(c2), items1);
  _require = editStateId;
  priceTier = "priceTier";
...
export const useDescription = function useDescription(arg0) {
  ...
  const items = [GuildRoleSubscriptionsStore];
  const stateFromStores = require("initialize").useStateFromStores(items, () => GuildRoleSubscriptionsStore.getSubscriptionListing(closure_0));
  const tmp2 = description(5684)(() => first);
  const items1 = [stateFromStores, tmp2];
  let memo = noop.useMemo(() => closure_3(c2), items1);
  _require = arg0;
...
export const useRoleColor = function useRoleColor(editStateId, guildId) {
  const tmp = roleColor(15339)(guildId, editStateId);
  editStateId = tmp;
  const items = [tmp];
  let memo = noop.useMemo(() => {
    let color;
...
export const useChannelAccessFormat = function useChannelAccessFormat(editStateId, guildId) {
  const tmp = channelAccessFormat(15339)(guildId, editStateId);
  const items = [tmp];
  let memo = noop.useMemo(() => {
    if (null == closure_0) {
...
export const useChannelBenefits = function useChannelBenefits(listingId) {
  ...
  const items = [GuildRoleSubscriptionsStore];
  const stateFromStores = require("initialize").useStateFromStores(items, () => GuildRoleSubscriptionsStore.getSubscriptionListing(closure_0));
  const tmp2 = channelBenefits(5684)(() => first);
  const items1 = [stateFromStores, tmp2];
  let memo = noop.useMemo(() => closure_3(c2), items1);
  _require = listingId;
...
export const useIntangibleBenefits = function useIntangibleBenefits(listingId) {
  ...
  const items = [GuildRoleSubscriptionsStore];
  const stateFromStores = require("initialize").useStateFromStores(items, () => GuildRoleSubscriptionsStore.getSubscriptionListing(closure_0));
  const tmp2 = intangibleBenefits(5684)(() => first);
  const items1 = [stateFromStores, tmp2];
  let memo = noop.useMemo(() => closure_3(c2), items1);
  _require = listingId;
...
export const useTierEmojiIds = function useTierEmojiIds(listingId, guildId) {
  _require = guildId;
  const tmp = tierEmojiIds(15339)(guildId, listingId);
              ...
              const obj6 = {
                value: (function updateListingFromEditState(editStateId) {
                            ...
                            if (null != channelBenefits) {
                              ...
                              const found = benefits.filter(closure_1_0(15341).isChannelBenefit);
                              const found1 = benefits1.filter(closure_1_0(15341).isIntangibleBenefit);
                              if (channelBenefits == null) {
                            ...
                            if (!obj2.isEmpty(obj)) {
                              ...
                              subscriptionListing = closure_1_2(7365).updateSubscriptionListing(obj4);
                              const obj3 = closure_1_2(7365);
                            }
