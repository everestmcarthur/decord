...
function getLimitedOfferTimeLeft(arg0) {
  if (null == arg0) {
    ...
    const diffResult = _modDef4166(arg0).diff(_modDef4166(), "seconds");
    let tmp4 = null;
    if (diffResult > 0) {
...
function formatLimitedOfferTimeLeft(arg0) {
  ...
  if (null == tmp) {
    ...
    if (days > 0) {
      ...
      const obj3 = { hours };
      formatToPlainStringResult = intl2.formatToPlainString(_modDef3430.PPaJSw, obj3);
    } else {
      const intl = util.intl;
      const obj = { minutes: null };
      const _Math = Math;
      obj.minutes = Math.max(tmp12, 1);
      formatToPlainStringResult = intl.formatToPlainString(_modDef3430["7Z+aIf"], obj);
    }
