...
// Dependencies: [19, 17, 1074, 21, 4605, 576, 7220, 10872, 1396, 8896, 4601, 2]
// Exports: default
...
export default function SocialLayerStorefrontGiftProductDetails(sku) {
  ...
  const tmp = closure_9();
  getOrFetchApplication = getOrFetchApplication(7220).useGetOrFetchApplication(sku.applicationId);
  let obj = getOrFetchApplication(7220);
  const userPrice = getOrFetchApplication(10872).useFormattedSKUPrice({ sku, priceSetAssignmentPurchaseType: constants.GIFT }).userPrice;
  ...
  if (tmp6Result) {
    ...
    items2[1] = tmp8(tmp2(4601).Text, obj9);
  ...
  if (tmp8Result2) {
    ...
    tmp8Result2 = tmp8(tmp2(4601).Text, obj11);
  }
