...
// Dependencies: [19, 17, 1074, 21, 4638, 576, 7278, 10948, 1397, 8966, 4634, 2]
// Exports: default

// Module 11155 (SocialLayerStorefrontGiftProductDetails)
...
export default function SocialLayerStorefrontGiftProductDetails(sku) {
  ...
  const tmp = closure_9();
  getOrFetchApplication = getOrFetchApplication(7278).useGetOrFetchApplication(sku.applicationId);
  let obj = getOrFetchApplication(7278);
  const userPrice = getOrFetchApplication(10948).useFormattedSKUPrice({ sku, priceSetAssignmentPurchaseType: constants.GIFT }).userPrice;
  ...
  if (tmp6Result) {
    ...
    items2[1] = tmp8(tmp2(4634).Text, obj9);
    obj6.children = items2;
    tmp6Result = tmp6(tmp7, obj6);
  }
  const items3 = [tmp6Result, closure_7(getOrFetchApplication(4634).Text, { variant: "text-md/semibold", children: sku.name })];
  ...
  if (tmp8Result2) {
    ...
    tmp8Result2 = tmp8(tmp2(4634).Text, obj11);
  }
