...
// Dependencies: [19, 17, 1074, 21, 4640, 576, 7282, 10956, 1397, 8982, 4636, 2]
// Exports: default

// Module 11163 (SocialLayerStorefrontGiftProductDetails)
...
export default function SocialLayerStorefrontGiftProductDetails(sku) {
  ...
  const tmp = closure_9();
  getOrFetchApplication = getOrFetchApplication(7282).useGetOrFetchApplication(sku.applicationId);
  let obj = getOrFetchApplication(7282);
  const userPrice = getOrFetchApplication(10956).useFormattedSKUPrice({ sku, priceSetAssignmentPurchaseType: constants.GIFT }).userPrice;
  ...
  if (tmp6Result) {
    ...
    items2[1] = tmp8(tmp2(4636).Text, obj9);
    obj6.children = items2;
    tmp6Result = tmp6(tmp7, obj6);
  }
  const items3 = [tmp6Result, closure_7(getOrFetchApplication(4636).Text, { variant: "text-md/semibold", children: sku.name })];
  ...
  if (tmp8Result2) {
    ...
    tmp8Result2 = tmp8(tmp2(4636).Text, obj11);
  }
