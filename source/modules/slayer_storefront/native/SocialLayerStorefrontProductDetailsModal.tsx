...
// Function ID: 10805
// Name: HeroMedia
// Dependencies: [32, 19, 17, 5510, 7228, 10805, 1074, 21, 672, 576, 4560, 5682, 10806, 8307, 5587, 4556, 1114, 10807, 1178, 1925, 4802, 1611, 9371, 504, 7168, 7165, 7226, 7182, 4992, 1242, 10802, 10803, 1115, 4231, 10808, 9370, 10809, 10818, 7231, 5631, 5680, 4987, 8830, 10822, 3417, 4975, 8097, 8083, 10825, 10827, 2]
// Exports: default

// Module 10804 (HeroMedia)
...
import useFormattedSKUPrice from "useFormattedSKUPrice" /* 10807 */;
...
import closure_9 from "handleUserSettingsStoreUpdate" /* 7228 */;
import { SlayerShopPDPCTAType } from "SlayerShopCardCTAType" /* 10805 */;
...
function SKUNameAndDescriptionSection(sku) {
  ...
  if (!obj.isNullOrEmpty(sku.name)) {
    ...
    obj[2] = sku.name;
    const items = [callback(tmp2(4556).Heading, obj), ];
...
function ItemDetailsSection(selectedItem) {
  ...
  if (!isNullOrEmptyResult) {
    ...
    obj[2] = trimmed;
    tmp14 = callback(tmp8(4556).Heading, obj);
  }
  const items = [tmp14, , ];
  ...
  if (!isNullOrEmptyResult1) {
    ...
    obj4[2] = trimmed1;
    items1[1] = callback(tmp8(4556).Text, obj4);
    obj1[1] = items1;
  ...
  if (!isNullOrEmptyResult2) {
    ...
    obj5[2] = trimmed2;
    tmp23 = callback(tmp8(4556).Text, obj5);
  }
  items[2] = tmp23;
  obj[1] = items;
...
function SocialLayerStorefrontProductDetailsModal(skuId) {
  ...
  callback = stateFromStores.useCallback((arg0) => {
    let obj = analyticsLocations(closeButtonIcon[29]);
    obj = { slayer_storefront_session_id: "Array", sku_id: "a", guild_id: true, application_id: true, cta_type: "function changeEventCalculator_Pnpm_pinchGestureTs1(current,previous){let changePayload;if(previous===undefined){changePayload={scaleChange:current.scale};}else{changePayload={scaleChange:current.scale/previous.scale};}return{...current,...changePayload};}", location_stack: "function changeEventCalculator_Pnpm_rotationGestureTs1(current,previous){let changePayload;if(previous===undefined){changePayload={rotationChange:current.rotation};}else{changePayload={rotationChange:current.rotation-previous.rotation};}return{...current,...changePayload};}" };
    obj[1] = skuId;
    let applicationId;
    if (stateFromStores != null) {
      applicationId = stateFromStores.applicationId;
  ...
  if (tmp28) {
    ...
    const items10 = [skuId];
    obj[3] = items10;
    obj[6] = tmp4(10802).closeSocialLayerStorefrontProductDetailsModal;
    ...
    obj1[4] = callback2;
    obj1[5] = callback1;
    obj[8] = callback(tmp4(10818).HeadlessSlayerStorefrontPurchaseRunner, obj1);
  ...
  if (null == stateFromStores) {
    ...
    obj5[0] = tmp4(10802).closeSocialLayerStorefrontProductDetailsModal;
    ...
    obj5[2] = intl.string(tmp4(1114).t.cpT0Cq);
    const items14 = [callback(tmp4(5631).HeaderBackButton, obj5), ];
    ...
    obj6[3] = name;
    items14[1] = callback(tmp4(4556).Heading, obj6);
    obj4[1] = items14;
    ...
    if (null != tmp23) {
      ...
      if (tmp47Result) {
        ...
        tmp47Result = tmp47(tmp2(8830), obj11);
      }
    }
    ...
    if (exclusive2) {
      ...
      obj12[1] = tmp47(tmp4(10822).ExclusiveBadge, {});
    ...
    obj19[0] = rect.bottom + tmp2(576).space.PX_16;
    items19[1] = obj19;
    obj18[0] = items19;
    ...
    if (!result) {
      ...
      obj21[3] = intl2.string(tmp2(3417).gndWN7);
    ...
    obj24[4] = tmp64;
    obj24[5] = callback3;
    obj23[1] = callback(tmp4(4975).Button, obj24);
    ...
    if (result1) {
      ...
      obj25[4] = intl3.string(tmp4(1114).t.QAZA5f);
      obj25[5] = tmp46;
    ...
    if (result) {
      ...
      const mobileFinePrintMessageForApplication = tmp4(10825).getMobileFinePrintMessageForApplication(getOrFetchApplication, stringResult, obj27);
