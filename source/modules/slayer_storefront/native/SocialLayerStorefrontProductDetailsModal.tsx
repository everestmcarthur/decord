...
// Dependencies: [32, 19, 17, 5680, 7424, 11048, 1074, 21, 672, 576, 4722, 5852, 11049, 8539, 5757, 4718, 1115, 11050, 1177, 1926, 4964, 1612, 5295, 9604, 504, 7364, 7361, 7422, 7378, 5161, 1241, 11045, 11046, 1364, 4393, 11051, 9603, 11052, 11061, 7427, 9066, 11065, 3546, 5144, 8140, 8302, 11068, 5801, 5850, 5156, 11070, 2]
// Exports: default

// Module 11047 (SocialLayerStorefrontProductDetailsModal)
...
import common_Video from "common/Video" /* 8539 */;
...
import carouselMediaItems from "carouselMediaItems" /* 11049 */;
import StorefrontNativeUtils from "StorefrontNativeUtils" /* 11050 */;
import redirectToSlayerStorefrontWebDefault from "redirectToSlayerStorefrontWeb" /* 11051 */;
import NativePaymentContext from "NativePaymentContext" /* 11070 */;
...
function SKUNameAndDescriptionSection(sku) {
  ...
  if (!obj.isNullOrEmpty(sku.name)) {
    ...
    const items = [closure_1_14(tmp2(4718).Heading, obj3), ];
    const isNullOrEmptyResult = tmp2(1926).isNullOrEmpty(sku.description);
    let tmp7Result = !isNullOrEmptyResult;
    if (!isNullOrEmptyResult) {
      const obj4 = { variant: "text-md/medium", color: "text-muted", children: sku.description };
      tmp7Result = tmp7(tmp2(4718).Text, obj4);
    }
    items[1] = tmp7Result;
    obj2.children = items;
...
function ItemDetailsSection(selectedItem) {
  ...
  if (!isNullOrEmptyResult) {
    ...
    tmp14 = closure_1_14(tmp8(4718).Heading, obj4);
  }
  const items = [tmp14, , ];
  ...
  if (!isNullOrEmptyResult1) {
    ...
    items1[1] = closure_1_14(tmp8(4718).Text, obj8);
    obj5.children = items1;
  ...
  if (!isNullOrEmptyResult2) {
    ...
    tmp23 = closure_1_14(tmp8(4718).Text, obj9);
  }
  items[2] = tmp23;
  obj3.children = items;
...
function SocialLayerStorefrontProductDetailsModal(skuId) {
  ...
  const isScreenLandscape = skuId(5295).useIsScreenLandscape();
  let obj = skuId(5295);
  const mobileStoreFront = analyticsLocations(9604).useMobileStoreFront();
  let obj2 = analyticsLocations(9604);
  ...
  const getOrFetchApplication = skuId(7364).useGetOrFetchApplication(applicationId1);
  let tmp12 = getOrFetchApplication;
  const obj5 = skuId(7364);
  if (getOrFetchApplication == null) {
    tmp12 = null;
  }
  const tmp2Result = analyticsLocations(7361);
  ...
  const OTPACOMOrderExperiment = tmp4(9603).OTPACOMOrderExperiment;
  ...
  if (tmp30) {
    ...
    const items10 = [skuId];
    obj6.skuIds = items10;
    obj6.onOrderRetryCancellation = tmp4(11045).closeSocialLayerStorefrontProductDetailsModal;
    ...
    obj6.children = closure_14(tmp4(11061).HeadlessSlayerStorefrontPurchaseRunner, obj9);
    tmp43Result = tmp43(tmp2(11052), obj6, skuId);
    const tmp2Result2 = tmp2(11052);
  }
  const items11 = [skuId, memo1, trackPDPClick];
  if (null == stateFromStores) {
    return null;
  } else {
    let result = tmp4(7427).isSlayerSkuAvailableOnThisPlatform(stateFromStores);
    const intl4 = tmp4(1115).intl;
    const stringResult = intl4.string(tmp4(1115).t.boqtTA);
    const tmp4Result7 = tmp4(7427);
    let result1 = tmp4(4393).isSocialLayerStorefrontGiftingSupported();
    if (null != tmp25) {
      ...
      if (null != memo) {
        ...
        tmp49 = closure_14(tmp2(9066), obj11);
      }
    }
    let tmp53 = null;
    if (stateFromStores.exclusive) {
      const obj12 = { style: tmp.exclusiveBadgeContainer, children: closure_14(tmp4(11065).ExclusiveBadge, {}) };
    ...
    if (!result) {
      ...
      obj18.children = intl.string(tmp2(3546).gndWN7);
      tmp65Result = tmp65(tmp4(4718).Text, obj18);
    }
    ...
    obj20.children = closure_14(tmp4(5144).Button, obj21);
    const items14 = [closure_14(memo1, obj20), ];
    if (result1) {
      const obj22 = { icon: tmp2(8302), variant: "primary", size: "lg", disabled: tmp28, accessibilityLabel: null, onPress: null };
      const intl2 = tmp4(1115).intl;
      obj22.accessibilityLabel = intl2.string(tmp4(1115).t.QAZA5f);
      obj22.onPress = tmp48;
      result1 = tmp65(tmp4(8140).IconButton, obj22);
    }
    items14[1] = result1;
    ...
    if (result) {
      ...
      const mobileFinePrintMessageForApplication = tmp4(11068).getMobileFinePrintMessageForApplication(getOrFetchApplication, stringResult, obj24);
    ...
    const obj28 = {
      onPress: tmp4(11045).closeSocialLayerStorefrontProductDetailsModal,
      backImage() {
    ...
    const items17 = [closure_14(tmp4(5801).HeaderBackButton, obj28), ];
    ...
    obj29.children = name;
    items17[1] = closure_14(tmp4(4718).Heading, obj29);
    ...
    if (exclusive) {
      ...
      exclusive = tmp65(tmp2(5156), obj31);
    }
...
const SlayerShopPDPCTAType = fn(11048).SlayerShopPDPCTAType;
...
obj.header = { height: fn(5852).NAV_BAR_HEIGHT, flexDirection: "row", alignItems: "center", paddingHorizontal: nativeDefault.space.PX_8, gap: nativeDefault.space.PX_8, borderBottomWidth: 1, borderBottomColor: nativeDefault.colors.BORDER_SUBTLE };
obj.headerTitle = { flexShrink: 1 };
let obj2 = { height: fn(5852).NAV_BAR_HEIGHT, flexDirection: "row", alignItems: "center", paddingHorizontal: nativeDefault.space.PX_8, gap: nativeDefault.space.PX_8, borderBottomWidth: 1, borderBottomColor: nativeDefault.colors.BORDER_SUBTLE };
...
obj.hero = { marginHorizontal: nativeDefault.space.PX_16, height: fn(11049).MOBILE_HERO_HEIGHT_PX, borderRadius: nativeDefault.radii.md, overflow: "hidden", backgroundColor: nativeDefault.colors.BACKGROUND_SURFACE_HIGH };
obj.heroLandscape = { flex: 1, minHeight: 140, height: "assign" };
let obj20 = { marginHorizontal: nativeDefault.space.PX_16, height: fn(11049).MOBILE_HERO_HEIGHT_PX, borderRadius: nativeDefault.radii.md, overflow: "hidden", backgroundColor: nativeDefault.colors.BACKGROUND_SURFACE_HIGH };
