...
// Dependencies: [32, 19, 17, 5595, 7335, 10933, 1074, 21, 672, 576, 4639, 5767, 10934, 8428, 5672, 4635, 1115, 10935, 1177, 1926, 4881, 1612, 5210, 9498, 504, 7275, 7272, 7333, 7289, 5076, 1241, 10930, 10931, 1364, 4310, 10936, 9497, 10937, 10946, 7338, 8955, 10950, 3461, 5059, 8205, 8191, 10953, 5716, 5765, 5071, 10955, 2]
// Exports: default

// Module 10932 (SocialLayerStorefrontProductDetailsModal)
...
import carouselMediaItems from "carouselMediaItems" /* 10934 */;
import StorefrontNativeUtils from "StorefrontNativeUtils" /* 10935 */;
...
function SKUNameAndDescriptionSection(sku) {
  ...
  if (!obj.isNullOrEmpty(sku.name)) {
    ...
    const items = [closure_1_14(tmp2(4635).Heading, obj3), ];
    ...
    if (!isNullOrEmptyResult) {
      ...
      tmp7Result = tmp7(tmp2(4635).Text, obj4);
    }
    items[1] = tmp7Result;
    obj2.children = items;
...
function ItemDetailsSection(selectedItem) {
  ...
  if (!isNullOrEmptyResult) {
    ...
    tmp14 = closure_1_14(tmp8(4635).Heading, obj4);
  }
  const items = [tmp14, , ];
  ...
  if (!isNullOrEmptyResult1) {
    ...
    items1[1] = closure_1_14(tmp8(4635).Text, obj8);
    obj5.children = items1;
  ...
  if (!isNullOrEmptyResult2) {
    ...
    tmp23 = closure_1_14(tmp8(4635).Text, obj9);
  }
  items[2] = tmp23;
  obj3.children = items;
...
function SocialLayerStorefrontProductDetailsModal(skuId) {
  ...
  const isScreenLandscape = skuId(5210).useIsScreenLandscape();
  let obj = skuId(5210);
  const mobileStoreFront = analyticsLocations(9498).useMobileStoreFront();
  let obj2 = analyticsLocations(9498);
  ...
  const getOrFetchApplication = skuId(7275).useGetOrFetchApplication(applicationId1);
  let tmp12 = getOrFetchApplication;
  const obj5 = skuId(7275);
  if (getOrFetchApplication == null) {
    tmp12 = null;
  }
  const tmp2Result = analyticsLocations(7272);
  ...
  trackPDPClick = stateFromStores.useCallback((cta_type) => {
    const obj2 = { slayer_storefront_session_id: "Array", sku_id: skuId, guild_id: true, application_id: true, cta_type: "minDurationMs", location_stack: "maxDist" };
    let applicationId;
    if (stateFromStores != null) {
      applicationId = stateFromStores.applicationId;
    }
  ...
  const OTPACOMOrderExperiment = tmp4(9497).OTPACOMOrderExperiment;
  ...
  if (tmp30) {
    ...
    const items10 = [skuId];
    obj6.skuIds = items10;
    obj6.onOrderRetryCancellation = tmp4(10930).closeSocialLayerStorefrontProductDetailsModal;
    ...
    obj6.children = closure_14(tmp4(10946).HeadlessSlayerStorefrontPurchaseRunner, obj9);
    tmp43Result = tmp43(tmp2(10937), obj6, skuId);
    const tmp2Result2 = tmp2(10937);
  }
  const items11 = [skuId, memo1, trackPDPClick];
  if (null == stateFromStores) {
    return null;
  } else {
    let result = tmp4(7338).isSlayerSkuAvailableOnThisPlatform(stateFromStores);
    const intl4 = tmp4(1115).intl;
    const stringResult = intl4.string(tmp4(1115).t.boqtTA);
    const tmp4Result7 = tmp4(7338);
    ...
    if (null != tmp25) {
      ...
      if (null != memo) {
        ...
        tmp49 = closure_14(tmp2(8955), obj11);
      }
    }
    let tmp53 = null;
    if (stateFromStores.exclusive) {
      const obj12 = { style: tmp.exclusiveBadgeContainer, children: closure_14(tmp4(10950).ExclusiveBadge, {}) };
    ...
    if (!result) {
      ...
      tmp65Result = tmp65(tmp4(4635).Text, obj18);
    }
    ...
    obj20.children = closure_14(tmp4(5059).Button, obj21);
    const items14 = [closure_14(memo1, obj20), ];
    if (result1) {
      const obj22 = { icon: tmp2(8191), variant: "primary", size: "lg", disabled: tmp28, accessibilityLabel: null, onPress: null };
      const intl2 = tmp4(1115).intl;
      obj22.accessibilityLabel = intl2.string(tmp4(1115).t.QAZA5f);
      obj22.onPress = tmp48;
      result1 = tmp65(tmp4(8205).IconButton, obj22);
    }
    items14[1] = result1;
    ...
    if (result) {
      ...
      const mobileFinePrintMessageForApplication = tmp4(10953).getMobileFinePrintMessageForApplication(getOrFetchApplication, stringResult, obj24);
    ...
    const obj28 = {
      onPress: tmp4(10930).closeSocialLayerStorefrontProductDetailsModal,
      backImage() {
    ...
    const items17 = [closure_14(tmp4(5716).HeaderBackButton, obj28), ];
    ...
    obj29.children = name;
    items17[1] = closure_14(tmp4(4635).Heading, obj29);
    ...
    if (exclusive) {
      ...
      exclusive = tmp65(tmp2(5071), obj31);
    }
...
const SlayerShopPDPCTAType = fn(10933).SlayerShopPDPCTAType;
...
obj.header = { height: fn(5767).NAV_BAR_HEIGHT, flexDirection: "row", alignItems: "center", paddingHorizontal: nativeDefault.space.PX_8, gap: nativeDefault.space.PX_8, borderBottomWidth: 1, borderBottomColor: nativeDefault.colors.BORDER_SUBTLE };
obj.headerTitle = { flexShrink: 1 };
let obj2 = { height: fn(5767).NAV_BAR_HEIGHT, flexDirection: "row", alignItems: "center", paddingHorizontal: nativeDefault.space.PX_8, gap: nativeDefault.space.PX_8, borderBottomWidth: 1, borderBottomColor: nativeDefault.colors.BORDER_SUBTLE };
...
obj.hero = { marginHorizontal: nativeDefault.space.PX_16, height: fn(10934).MOBILE_HERO_HEIGHT_PX, borderRadius: nativeDefault.radii.md, overflow: "hidden", backgroundColor: nativeDefault.colors.BACKGROUND_SURFACE_HIGH };
obj.heroLandscape = { flex: 1, minHeight: 140, height: "alignItems" };
let obj20 = { marginHorizontal: nativeDefault.space.PX_16, height: fn(10934).MOBILE_HERO_HEIGHT_PX, borderRadius: nativeDefault.radii.md, overflow: "hidden", backgroundColor: nativeDefault.colors.BACKGROUND_SURFACE_HIGH };
