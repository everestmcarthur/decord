...
// Dependencies: [5, 5332, 1371, 12770, 12772, 12771, 1074, 4816, 12773, 12775, 1242, 12777, 1114, 4951, 4305, 9657, 7118, 4507, 4809, 12778, 1896, 9911, 1093, 12797, 2]
...
// Module 12769 (ContactSyncModalActionCreators)
import ConstantsIOS from "ConstantsIOS" /* 1093 */;
import asyncRequireImpl from "asyncRequireImpl" /* 1896 */;
import ModalActionCreatorsDefault from "ModalActionCreators" /* 4809 */;
import PhoneActionCreatorsDefault from "PhoneActionCreators" /* 7118 */;
...
import ContactSyncAnalyticsUtils from "ContactSyncAnalyticsUtils" /* 12775 */;
...
function closeContactSyncModal(skip) {
  ...
  if (null != onComplete) {
    ...
    flag2(12775).trackFlowEnd(flag, obj2);
    if (tmp16) {
      onComplete(flag);
    } else {
    ...
    if (!back) {
      ...
      flag2(12775).trackFlowEnd(flag2, obj4);
      const obj = flag2(12775);
    }
    if (tmp2) {
      let updateAnimation = back(4809).updateAnimation;
    ...
    const timerId1 = setTimeout(() => {
      ...
      if (closure_2) {
        ...
        if (back) {
          ...
          const result = tmp7(12797).previousOnboardingStep();
          const tmp7Result = tmp7(12797);
        } else {
          updateAnimation(tmp3, ModalAnimation.SLIDE_IN_OUT);
          const obj2 = { skip: flag2 };
          tmp7(12797).nextOnboardingStep(obj2);
          const tmp7Result2 = tmp7(12797);
        }
...
export const openContactSyncModal = function openContactSyncModal(initialRoutes, FRIENDS_ADD_FRIENDS_MODAL, arg2) {
  ...
  if (FRIENDS_ADD_FRIENDS_MODAL) {
    ...
    tmp(12775).trackFlowStart(obj2);
    const tmpResult = tmp(12775);
  }
  ...
  ModalActionCreatorsDefault.pushLazy(asyncRequireImpl(12778, tmp2.paths), { initialRoutes: initialRoutes.initialRoutes, openSettingsSheet: initialRoutes.openSettings, customLandingPage: initialRoutes.customLandingPage }, closure_1_20).then(arg2);
};
...
export const openContactSyncModalDeeplink = function openContactSyncModalDeeplink() {
  ...
  ModalActionCreatorsDefault.pushLazy(asyncRequireImpl(12778, tmp2.paths), { initialRoutes: obj.initialRoutes, openSettingsSheet: obj.openSettings, customLandingPage: obj.customLandingPage }, closure_1_20).then(undefined);
};
