...
// Dependencies: [32, 19, 17, 1074, 12794, 21, 4607, 576, 5734, 5034, 4603, 4451, 1396, 5656, 1178, 1114, 4308, 4456, 11, 8686, 8966, 5039, 1093, 5027, 1242, 2]
// Exports: default

// Module 12793 (ContactSyncSuggestions)
...
function SuggestionRow(suggestion) {
  ...
  if (tmp8Result) {
    ...
    tmp8Result = tmp8(tmp3(4603).Text, obj5);
  }
  items[1] = tmp8Result;
...
let obj2 = { container: { flex: 1, backgroundColor: nativeDefault.colors.BACKGROUND_BASE_LOWER, paddingTop: fn(5734).NAV_BAR_HEIGHT + 32, justifyContent: "center" }, header: null, title: null, subtitle: null, list: null, divider: null, linearGradient: null, redesignButton: null, sectionHeader: null };
let obj3 = { flex: 1, backgroundColor: nativeDefault.colors.BACKGROUND_BASE_LOWER, paddingTop: fn(5734).NAV_BAR_HEIGHT + 32, justifyContent: "center" };
...
export default function ContactSyncSuggestions(friendSuggestions) {
  ...
  const token = friendSuggestions(4308).useToken(onSubmit(576).colors.BACKGROUND_BASE_LOW);
  let obj = friendSuggestions(4308);
  let items = [friendSuggestions(4456).hexOpacityToRgba(token, 0), ];
  let obj2 = friendSuggestions(4456);
  items[1] = friendSuggestions(4456).hexOpacityToRgba(token, 100);
  const tmp4 = first(noop.useState(reduced), 2);
  first = tmp4[0];
  noop = tmp4[1];
  let obj3 = friendSuggestions(4456);
  const fontScale = friendSuggestions(5034).useFontScale();
  ...
  let obj4 = friendSuggestions(5034);
  ...
  const items5 = [closure_8(friendSuggestions(8966).FlashList, obj7), , ];
  ...
  obj9.colors = items;
  items5[1] = closure_8(onSubmit(5039), obj9);
  ...
  obj10.children = closure_8(friendSuggestions(5027).Button, obj11);
