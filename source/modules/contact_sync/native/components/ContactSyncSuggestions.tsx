...
// Dependencies: [32, 19, 17, 1074, 12834, 21, 4639, 576, 5767, 5066, 4635, 4484, 1397, 5689, 1177, 1115, 4341, 4489, 11, 8719, 9006, 5071, 1094, 5059, 1241, 2]
// Exports: default

// Module 12833 (ContactSyncSuggestions)
...
function SuggestionRow(suggestion) {
  ...
  if (tmp8Result) {
    ...
    tmp8Result = tmp8(tmp3(4635).Text, obj5);
  }
  items[1] = tmp8Result;
...
const SuggestedFriendSource = fn(12834).SuggestedFriendSource;
const jsxProd = fn(21);
({ jsx: closure_8, Fragment: closure_9, jsxs: c10 } = jsxProd);
const createStyles = fn(4639);
let obj2 = { container: { flex: 1, backgroundColor: nativeDefault.colors.BACKGROUND_BASE_LOWER, paddingTop: fn(5767).NAV_BAR_HEIGHT + 32, justifyContent: "center" }, header: null, title: null, subtitle: null, list: null, divider: null, linearGradient: null, redesignButton: null, sectionHeader: null };
let obj3 = { flex: 1, backgroundColor: nativeDefault.colors.BACKGROUND_BASE_LOWER, paddingTop: fn(5767).NAV_BAR_HEIGHT + 32, justifyContent: "center" };
...
export default function ContactSyncSuggestions(friendSuggestions) {
  ...
  const token = friendSuggestions(4341).useToken(onSubmit(576).colors.BACKGROUND_BASE_LOW);
  let obj = friendSuggestions(4341);
  let items = [friendSuggestions(4489).hexOpacityToRgba(token, 0), ];
  let obj2 = friendSuggestions(4489);
  items[1] = friendSuggestions(4489).hexOpacityToRgba(token, 100);
  const tmp4 = first(noop.useState(reduced), 2);
  first = tmp4[0];
  noop = tmp4[1];
  let obj3 = friendSuggestions(4489);
  const fontScale = friendSuggestions(5066).useFontScale();
  ...
  let obj4 = friendSuggestions(5066);
  ...
  const items5 = [closure_8(friendSuggestions(9006).FlashList, obj7), , ];
  ...
  obj9.colors = items;
  items5[1] = closure_8(onSubmit(5071), obj9);
  ...
  obj10.children = closure_8(friendSuggestions(5059).Button, obj11);
