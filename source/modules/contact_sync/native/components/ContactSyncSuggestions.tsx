...
// Dependencies: [32, 19, 17, 1074, 12842, 21, 4638, 576, 5768, 5067, 4634, 4482, 1397, 5690, 1177, 1115, 4339, 4487, 11, 8728, 9017, 5072, 1094, 5060, 1241, 2]
// Exports: default

// Module 12841 (ContactSyncSuggestions)
...
function SuggestionRow(suggestion) {
  ...
  if (tmp8Result) {
    ...
    tmp8Result = tmp8(tmp3(4634).Text, obj5);
  }
  items[1] = tmp8Result;
...
let obj2 = { container: { flex: 1, backgroundColor: nativeDefault.colors.BACKGROUND_BASE_LOWER, paddingTop: fn(5768).NAV_BAR_HEIGHT + 32, justifyContent: "center" }, header: null, title: null, subtitle: null, list: null, divider: null, linearGradient: null, redesignButton: null, sectionHeader: null };
let obj3 = { flex: 1, backgroundColor: nativeDefault.colors.BACKGROUND_BASE_LOWER, paddingTop: fn(5768).NAV_BAR_HEIGHT + 32, justifyContent: "center" };
...
export default function ContactSyncSuggestions(friendSuggestions) {
  ...
  const token = friendSuggestions(4339).useToken(onSubmit(576).colors.BACKGROUND_BASE_LOW);
  let obj = friendSuggestions(4339);
  let items = [friendSuggestions(4487).hexOpacityToRgba(token, 0), ];
  let obj2 = friendSuggestions(4487);
  items[1] = friendSuggestions(4487).hexOpacityToRgba(token, 100);
  const tmp4 = first(noop.useState(reduced), 2);
  first = tmp4[0];
  noop = tmp4[1];
  let obj3 = friendSuggestions(4487);
  const fontScale = friendSuggestions(5067).useFontScale();
  ...
  let obj4 = friendSuggestions(5067);
  ...
  const items5 = [closure_8(friendSuggestions(9017).FlashList, obj7), , ];
  ...
  obj9.colors = items;
  items5[1] = closure_8(onSubmit(5072), obj9);
  ...
  obj10.children = closure_8(friendSuggestions(5060).Button, obj11);
