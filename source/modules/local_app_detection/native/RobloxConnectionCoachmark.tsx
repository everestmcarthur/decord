...
// Dependencies: [32, 19, 17, 5332, 1371, 13766, 1074, 1954, 21, 4606, 576, 4317, 1611, 4573, 7223, 7222, 7271, 5024, 4602, 1114, 5026, 13049, 9342, 7455, 4315, 5334, 1396, 1178, 8542, 504, 13767, 5457, 4427, 1943, 2]
// Exports: default, useShouldShowRobloxConnectionCoachmark

// Module 17015 (RobloxConnectionCoachmark)
...
export default function RobloxConnectionActionSheet(markAsDismissed) {
  ...
  let tmp = closure_15();
  const theme = markAsDismissed(4317).useThemeContext().theme;
  let obj2 = { startExpanded: true, contentStyles: tmp.content, header: null, onDismiss: null, children: null };
  let obj = markAsDismissed(4317);
  obj2.header = closure_13(markAsDismissed(7222).BottomSheetTitleHeader, { title: null, leading: closure_13(markAsDismissed(7271).ActionSheetCloseButton, { onPress: handleCancel }) });
  ...
  obj5.children = items;
  const items1 = [closure_14(markAsDismissed(5024).Stack, obj5), , ];
  const obj6 = { justify: "center", children: null };
  const obj7 = { variant: "heading-xl/bold", style: tmp.text, children: null };
  const intl = markAsDismissed(1114).intl;
  obj7.children = intl.string(markAsDismissed(1114).t.t3asUZ);
  const items2 = [closure_13(markAsDismissed(4602).Text, obj7), ];
  const obj8 = { variant: "text-md/medium", style: tmp.text, children: null };
  const intl2 = markAsDismissed(1114).intl;
  obj8.children = intl2.string(markAsDismissed(1114).t.no96NU);
  items2[1] = closure_13(markAsDismissed(4602).Text, obj8);
  obj6.children = items2;
  items1[1] = closure_14(markAsDismissed(5024).Stack, obj6);
  ...
  const obj3 = { title: null, leading: closure_13(markAsDismissed(7271).ActionSheetCloseButton, { onPress: handleCancel }) };
  obj10.icon = closure_13(markAsDismissed(13049).WindowLaunchIcon, { size: "sm", color: nativeDefault.colors.WHITE });
  ...
  const items3 = [closure_13(markAsDismissed(5026).Button, obj10), ];
  const obj12 = { text: null, variant: "secondary", size: "lg", onPress: null };
  const intl4 = markAsDismissed(1114).intl;
  obj12.text = intl4.string(markAsDismissed(1114).t.DiGJy3);
  obj12.onPress = handleCancel;
  items3[1] = closure_13(markAsDismissed(5026).Button, obj12);
  obj9.children = items3;
  items1[2] = closure_14(markAsDismissed(5024).Stack, obj9);
  obj4.children = items1;
  obj2.children = closure_14(markAsDismissed(5024).Stack, obj4);
  return closure_13(markAsDismissed(7223).BottomSheet, obj2);
};
export { UnionIcon };
export const useShouldShowRobloxConnectionCoachmark = function useShouldShowRobloxConnectionCoachmark() {
  ...
  stateFromStores = stateFromStores(504).useStateFromStores(items, () => appInstalled.isAppInstalled(stateFromStores(13767).DetectableAppNames.ROBLOX));
