...
// Function ID: 7791
// Name: MessageParser
// Dependencies: [5545, 5592, 1958, 2013, 2022, 2016, 1980, 4276, 4286, 4483, 1372, 1074, 5085, 1375, 4791, 4334, 5083, 7791, 7792, 1936, 4290, 5090, 4482, 4790, 12, 5096, 1115, 7793, 5528, 1370, 4294, 2]
// Exports: parseAndRebuild

// Module 7790 (MessageParser)
...
import MarkupTextRule2 from "MarkupTextRule" /* 5090 */;
import AutocompleteBoundaryUtils from "AutocompleteBoundaryUtils" /* 7791 */;
    ...
    (function handleEmoji(emojiContext, type, fn) {
      if (null != fn) {
        ...
        if ("emoticon" === type.type) {
          const result = obj(4290).translateSurrogatesToInlineEmoji(type.content);
...
function createParserState(getGuildId, arr) {
  ...
  if (null != guildId) {
    ...
    const found1 = tmp9(12)(guildId(5528).COMMAND_SUPPORTED_CHANNEL_TYPE_KEYS).filter((item) => item !== closure_1_7);
    const tmp9Result1Result = tmp9(12)(guildId(5528).COMMAND_SUPPORTED_CHANNEL_TYPE_KEYS);
...
function unparseWithMeta(content1, id, isNotification) {
  ...
  if (isNotification) {
    ...
    translateSurrogatesToInlineEmoji = translateSurrogatesToInlineEmoji(4290).translateSurrogatesToInlineEmoji;
  }
  const obj = { inline: true, guild, channelId: id, isNotification };
  const obj4 = translateSurrogatesToInlineEmoji(4334);
  dependencyMap = undefined;
  content = "";
  const items = [];
  const item = translateSurrogatesToInlineEmoji(4334).parserFor(omitResult)(content1, obj).forEach((content) => {
    (function handleEmoji(emojiContext, type, fn) {
      if (null != fn) {
        if ("customEmoticon" === type.type) {
          fn(type.emoji, false);
...
const GAME_MENTION_SENTINEL = fn(5085).GAME_MENTION_SENTINEL;
...
obj9.parse = function parse(arg0, arg1, channelId) {
  ...
  if (null == user) {
    ...
    if (isNotification) {
      ...
      let nickname = tmp13(4790).getNickname(id, channelId.channelId, user);
      if (nickname == null) {
        nickname = tmp13(4482).getGlobalName(user);
        const tmp13Result3 = tmp13(4482);
      }
      if (null != nickname) {
...
let t = t_mod;
obj13.match = t.anyScopeRegex(fn(5096).soundmojiRawFormatRegex);
...
export default {
  parse(getGuildId, content, arg2, arr) {
    ...
    closure_129_0 = tmp;
    closure_129_1 = obj(4290).translateInlineEmojiToSurrogates;
