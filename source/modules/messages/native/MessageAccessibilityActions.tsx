...
export const getMessageAccessibilityActionFromLabel = function getMessageAccessibilityActionFromLabel(action) {
  ...
  if (true !== disableDoubleTap) {
    if (null != setting) {
      const result = tmp(7982).disambiguatedEmojiFromSettingsValue(setting);
      if (null != result) {
        const intl9 = tmp(1114).intl;
        const obj2 = { emojiName: result.name };
        formatToPlainStringResult = intl9.formatToPlainString(tmp(1114).t.eQIttH, obj2);
      }
...
export const createMessageAccessibilityActions = function createMessageAccessibilityActions(message, channel) {
  if (null == channel) {
    ...
    if (canAddNewReactionsDefault(channel)) {
      ...
      if (true !== disableDoubleTap) {
        if (null != setting) {
          const result = tmp10(7982).disambiguatedEmojiFromSettingsValue(setting);
          if (null != result) {
            const intl3 = tmp10(1114).intl;
            const obj3 = { emojiName: result.name };
            formatToPlainStringResult = intl3.formatToPlainString(tmp10(1114).t.eQIttH, obj3);
          }
          const tmp10Result = tmp10(7982);
        }
    ...
    if (tmp10Result2.canReplyToMessage(channel, message)) {
      ...
      obj5.label = intl4.string(tmp10(1114).t["5IEsGx"]);
