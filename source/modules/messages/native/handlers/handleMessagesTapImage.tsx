...
// Dependencies: [7789, 4371, 1372, 1074, 8500, 11784, 1097, 8497, 4417, 4954, 8491, 4875, 5197, 2]
// Exports: handleMessagesTapImage

// Module 11786 (handleMessagesTapImage)
...
export const handleMessagesTapImage = function handleMessagesTapImage(tapImageData) {
  ...
  if (null != portal) {
    embedId(8500).markPortalAlive(portal);
    const obj = embedId(8500);
  }
  if (true === allowWithinModal) {
    ...
    if (type1 !== embedId(1097).MessageReferenceTypes.FORWARD) {
      ...
      const found = attachments.filter((item) => !embedId(8497).isThumbnailAttachment(item));
      if ("attachment" === type) {
        if (index < found.length) {
          const tmp10Result = tmp10(8497);
      ...
      if ("embed" === type) {
        if (null != embedIndex) {
          ...
          if (importDefault < result.length) {
            tmp10(8497).setMediaSourcePortal(result[importDefault], portal);
            found2 = result;
            tmp25 = tmp10;
            const tmp10Result8 = tmp10(8497);
          }
        }
        if (-1 !== importDefault) {
          ...
          let channelName;
          if (showContextName) {
            const tmp25Result3 = tmp25(4875);
            channelName = tmp25Result3.computeChannelName(messageChannel, UserStore, RelationshipStore, false);
          }
          obj3.contextName = channelName;
          let channelIcon;
          if (showContextName) {
            channelIcon = tmp25(5197).getChannelIcon(messageChannel);
            const tmp25Result4 = tmp25(5197);
          }
          obj3.contextIcon = channelIcon;
          tmp25(8491).openMediaModal(obj3);
          const tmp25Result = tmp25(8491);
        }
      }
      if ("component" === type) {
        if (null == componentId) {
          ...
          if (null != embedId) {
            ...
            if (null != components) {
              if (0 !== components.length) {
                const tmp10Result9 = tmp10(8497);
                const result1 = tmp10Result9.extractMediaSourcesFromComponent(tmp7, components, messageChannel.guild_id, tmp10(4954).asComponentId(componentId), componentMediaIndex);
                if (null != result1) {
                  ({ sources, initialIndex: closure_1 } = result1);
                  tmp10(8497).setMediaSourcePortal(sources[importDefault], portal);
                  found2 = sources;
                  tmp25 = tmp10;
                  const tmp10Result11 = tmp10(8497);
                }
                const tmp10Result10 = tmp10(4954);
              }
            }
          }
        ...
        const result2 = tmp10(8497).extractMediaSourcesFromMessage(tmp7, tmp12, messageChannel.guild_id);
        let num2 = 0;
        found2 = result2;
        tmp25 = tmp10;
        if (0 < result2.length) {
          ...
          if (flattenSourceResult.noCarousel) {
            ...
            importDefault = 0;
            tmp20(8497).setMediaSourcePortal(items[0], portal);
            found2 = items;
            tmp25 = tmp20;
            const tmp20Result = tmp20(8497);
          } else {
            importDefault = num2;
            tmp20(8497).setMediaSourcePortal(result2[num2], portal);
            ...
            tmp25 = tmp20;
            const tmp20Result2 = tmp20(8497);
          }
        }
        const tmp10Result12 = tmp10(8497);
      }
    } else {
