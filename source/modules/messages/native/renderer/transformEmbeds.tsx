...
// Dependencies: [17, 1074, 8133, 8134, 4911, 8135, 1115, 7960, 4724, 8104, 4255, 7306, 7311, 4787, 1114, 2]
// Exports: default

// Module 8132 (transformEmbeds)
import _mod17 from "module_17" /* 17 */;
import Constants from "Constants" /* 1074 */;
import EmbedUtils from "EmbedUtils" /* 4911 */;
import sanitizeMediaDimension from "sanitizeMediaDimension" /* 8133 */;
                ...
                if (tmp3Result13.isSocialLayerStorefrontArticleEmbed(type)) {
                  ...
                  if (closure_1_4) {
                    ...
                    if (null != type.thumbnail) {
                      ...
                      obj2.width = tmp3(8133).sanitizeMediaDimension(width);
                      const tmp3Result14 = tmp3(8133);
                      obj2.height = tmp3(8133).sanitizeMediaDimension(height);
                      let imageSrc = proxyURL;
                      if (null != proxyURL) {
                  ...
                  if (closure_1_4) {
                        ...
                        if (type.type !== tmp.GIFV) {
                          ...
                          if (tmp32) {
                            ...
                            if (null == type.video.proxyURL) {
                              ...
                              const effectiveVideoProvider = tmp3(4911).getEffectiveVideoProvider(name, type.video.url);
                              const tmp3Result17 = tmp3(4911);
                              tmp34 = tmp8;
                    ...
                    if (null != type.url) {
                      if ("" !== type.url) {
                        let parseEmbedTitleMarkup = tmp3(8104).parseEmbedTitleMarkupWithoutLinks;
                      }
                      if (type.type === tmp.RICH) {
                            ...
                            if (tmp.GIFV !== type) {
                              if (tmp.RICH === type) {
                                if (null != type.rawDescription) {
                                  ...
                                  let rawDescription = tmp3(8104).parseEmbedDescriptionMarkup(obj9);
                                  const tmp3Result19 = tmp3(8104);
                                }
                              } else {
                        ...
                        const mapped1 = fields.map((rawName) => {
                          ...
                          if (null != rawName.rawName) {
                            result = channelId(8104).parseEmbedTitleMarkup(rawName.rawName, channelId);
                            const obj = channelId(8104);
                          }
                          let result1 = null;
                          if (null != rawName.rawValue) {
                            ...
                            result1 = channelId(8104).parseEmbedDescriptionMarkup(obj3);
                            const obj2 = channelId(8104);
                          }
                          const obj4 = {};
                        ...
                        if (null != type.timestamp) {
                          calendarFormatResult = tmp3(4255).calendarFormat(type.timestamp);
                          const tmp3Result20 = tmp3(4255);
                        }
                        if (null != type.footer) {
                        ...
                        if (null == type.author) {
                          if (type.type !== tmp.COMPONENTS) {
                            const obj12 = { type: tmp3(7311).ObscuredMediaTypes.Embed, media: type };
                            let isMediaScanPendingResult = !closure_1_10;
                            const mediaObscuredReasonFromBitmask = tmp3(7306).getMediaObscuredReasonFromBitmask(obj12, closure_1_9);
                            if (!closure_1_10) {
                              const obj13 = { type: tmp3(7311).ObscuredMediaTypes.Embed, media: type };
                              isMediaScanPendingResult = tmp3(7306).isMediaScanPending(obj13, closure_1_9);
                              const tmp3Result22 = tmp3(7306);
                            }
                            let isVerifiedTeenResult = tmp88;
                            if (mediaObscuredReasonFromBitmask.length > 0) {
                              isVerifiedTeenResult = tmp3(4787).isVerifiedTeen();
                              const tmp3Result23 = tmp3(4787);
                            }
                            let str10 = type.id;
                            if (str10 == null) {
                              str10 = "";
                    ...
                    parseEmbedTitleMarkup = tmp3(8104).parseEmbedTitleMarkup;
