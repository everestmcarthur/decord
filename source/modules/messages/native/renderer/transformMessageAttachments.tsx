...
// Dependencies: [8069, 1074, 1385, 11429, 4790, 8256, 1364, 8255, 1115, 8275, 8415, 5222, 8273, 2]
// Exports: default

// Module 13335 (transformMessageAttachments)
  ...
  return found.map((attachment, index) => {
    ...
    if (isImageFileResult) {
      if (null != width) {
        ...
        const result1 = tmp(8255).sanitizeMediaDimension(num4);
        const tmpResult8 = tmp(8255);
        let num5 = 0;
        if (closure_1_1) {
          num5 = 0;
          if (tmp8) {
        ...
        const result2 = tmp(8255).sanitizeMediaDimension(num5);
        const tmpResult9 = tmp(8255);
        if (flags == null) {
          flags = 0;
        }
        let tmp28;
        ...
        if (tmp30) {
          tmp30 = null != tmp(8275).isSuspiciousDownload(localUri);
          const tmpResult11 = tmp(8275);
        }
        ...
        const merged = Object.assign(tmp(8273).getAttachmentObscurityProps(obj8));
