...
// Dependencies: [8152, 1074, 1385, 11527, 4872, 8338, 1364, 8337, 1115, 8357, 8498, 5304, 8355, 2]
// Exports: default

// Module 13426 (transformMessageAttachments)
  ...
  return found.map((attachment, index) => {
    ...
    if (isImageFileResult) {
      if (null != width) {
        ...
        const result1 = tmp(8337).sanitizeMediaDimension(num4);
        const tmpResult8 = tmp(8337);
        let num5 = 0;
        if (closure_1_1) {
          num5 = 0;
          if (tmp8) {
        ...
        const result2 = tmp(8337).sanitizeMediaDimension(num5);
        const tmpResult9 = tmp(8337);
        if (flags == null) {
          flags = 0;
        }
        let tmp28;
        ...
        if (tmp30) {
          tmp30 = null != tmp(8357).isSuspiciousDownload(localUri);
          const tmpResult11 = tmp(8357);
        }
        ...
        const merged = Object.assign(tmp(8355).getAttachmentObscurityProps(obj8));
