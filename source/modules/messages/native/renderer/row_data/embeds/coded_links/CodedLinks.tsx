...
// Function ID: 13306
// Name: CodedLinks
// Dependencies: [32, 4246, 1979, 1371, 7763, 12035, 4595, 13306, 13307, 13317, 13319, 13312, 13321, 13314, 11575, 11903, 11626, 13322, 1369, 2]
// Exports: createCodedLinkEmbeds

// Module 13305 (CodedLinks)
import ApplicationCodedLink from "ApplicationCodedLink" /* 7763 */;
        ...
        if (obj.isApplicationCodedLink(type)) {
          if (null == closure_1) {
            ...
            if (tmpResult.isApplicationCodedLinkMobileSupported(type)) {
              const applicationCodedLinkData = tmp(7763).getApplicationCodedLinkData(type, code, url);
              if (null == applicationCodedLinkData) {
                return null;
              } else {
                const obj2 = { appId: applicationCodedLinkData.applicationId, channel: tmp34, message, theme };
                const appLinkGateResult = tmp(12035).getAppLinkGateResult(obj2);
                if ("unavailable" === appLinkGateResult.state) {
                  ...
                  if (tmp(4595).CodedLinkType.ACTIVITY_BOOKMARK === type2) {
                    const obj3 = { theme: tmp38, embedUrl: url, message: tmp37, app, params: applicationCodedLinkData.params };
                    return tmp(13306).createActivityMessageEmbed(obj3);
                  } else {
                    if (tmp(4595).CodedLinkType.APP_DIRECTORY_PROFILE !== type2) {
                      if (tmp(4595).CodedLinkType.APP_OAUTH2_LINK !== type2) {
                        return null;
                      }
                    }
                    const obj4 = { theme: tmp38, embedUrl: url, message: tmp37, app };
                    return tmp(12035).createAppMessageEmbed(obj4);
                  }
                }
                const tmpResult19 = tmp(12035);
              }
              const tmpResult18 = tmp(7763);
            } else {
              return null;
            }
            tmpResult = tmp(7763);
          ...
          return tmp(13307).createInviteEmbed(message, code, theme);
        } else if (tmp(4595).CodedLinkType.TEMPLATE === type) {
          return tmp(13317).createGuildTemplateEmbed(code, theme);
        } else if (tmp(4595).CodedLinkType.BUILD_OVERRIDE === type) {
          return tmp(13319).createBuildOverrideEmbed(code, theme);
        } else if (tmp(4595).CodedLinkType.MANUAL_BUILD_OVERRIDE === type) {
          ...
          if (isStaffResult) {
            buildOverrideEmbed = tmp(13319).createBuildOverrideEmbed(code, theme);
            const tmpResult25 = tmp(13319);
          }
          return buildOverrideEmbed;
        } else if (tmp(4595).CodedLinkType.EVENT === type) {
          return tmp(13312).createGuildScheduledEventLinkEmbed(code, theme);
        } else if (tmp(4595).CodedLinkType.CHANNEL_LINK === type) {
          return tmp(13321).createVoiceChannelLinkEmbed(code, theme);
        } else if (tmp(4595).CodedLinkType.EMBEDDED_ACTIVITY_INVITE === type) {
          const obj5 = { theme, inviteCode: code };
          return tmp(13314).createEmbeddedActivityInviteEmbed(obj5);
        } else if (tmp(4595).CodedLinkType.EXPERIMENT === type) {
          let experimentEmbed = null;
          if (tmpResult29.canSeeExperimentEmbeds()) {
            experimentEmbed = tmp(11903).createExperimentEmbed(url, theme);
            const tmpResult30 = tmp(11903);
          }
          ...
          if (tmp(4595).CodedLinkType.GUILD_PRODUCT !== type) {
            if (tmp(4595).CodedLinkType.SERVER_SHOP !== type) {
              if (tmp(4595).CodedLinkType.SOCIAL_LAYER_STOREFRONT !== type) {
                if (tmp(4595).CodedLinkType.SOCIAL_LAYER_STOREFRONT_APP !== type) {
                  if (tmp(4595).CodedLinkType.QUESTS_EMBED === type) {
                    ...
                    return tmp(13322).createQuestsEmbed(obj6);
                  } else {
              ...
              if (type === tmp(4595).CodedLinkType.SOCIAL_LAYER_STOREFRONT_APP) {
                ...
                obj7.theme = theme;
                return tmp(11626).createSocialLayerStorefrontProductDetailsEmbed(obj7);
              } else {
                ...
                obj9.theme = theme;
                return tmp(11626).createSocialLayerStorefrontProductDetailsEmbed(obj9);
              }
            }
          }
          return null;
