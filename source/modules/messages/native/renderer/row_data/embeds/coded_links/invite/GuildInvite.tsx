...
// Dependencies: [17, 1976, 4661, 1958, 2022, 1980, 4623, 4288, 1372, 11447, 1074, 7841, 8052, 1115, 8043, 576, 4484, 4491, 11943, 11944, 1972, 2025, 8053, 12860, 12859, 11449, 11448, 13346, 1385, 8513, 1397, 1879, 5112, 8870, 4792, 2]
...
export const createExpiredGuildInvite = function createExpiredGuildInvite(author, arg1, theme) {
  ...
  if (tmp6Result2.isThemeDark(theme)) {
    let tmpResult2 = tmp(11943);
  } else {
    tmpResult2 = tmp(11944);
  }
...
export const createGuildInvite = function createGuildInvite(invite, isOwnInvite, theme) {
  ...
  const channel = tmp(11448)(invite).channel;
  ...
  if (tmp7) {
    ...
    const assetSource = Image.resolveAssetSource(tmp10(5112).getChannelIcon(channel));
    let uri1;
    if (assetSource != null) {
    ...
    uri = guildIconURL;
    tmp25 = name;
    const tmp10Result7 = tmp10(5112);
  } else {
    if (null != channel) {
      if (channel.type === constants2.GUILD_STAGE_VOICE) {
        const assetSource1 = Image.resolveAssetSource(tmp10(5112).getChannelIcon(channel));
        let uri2;
    ...
    if (null == approximate_member_count) {
      ...
      tmp25 = name;
      if (null != channel) {
        const assetSource2 = Image.resolveAssetSource(tmp10(5112).getChannelIcon(channel));
        let uri3;
  ...
  if (tmp7) {
    ...
    if (null != guild) {
      guildBadgeImageSource = tmp10(8870).getGuildBadgeImageSource(guild, theme);
      const tmp10Result10 = tmp10(8870);
    }
    const obj11 = {};
    const merged = Object.assign(baseColors);
    let formatted;
    ...
    let channelName;
    if (null != channel) {
      channelName = tmp10(4792).computeChannelName(channel, UserStore, RelationshipStore);
      const tmp10Result11 = tmp10(4792);
    }
    ...
    if (null != guildBadgeImageSource) {
      assetUriForEmbed = tmp10(8053).getAssetUriForEmbed(guildBadgeImageSource);
      const tmp10Result12 = tmp10(8053);
    }
