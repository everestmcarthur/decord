...
// Dependencies: [17, 1975, 4658, 1957, 2021, 1979, 4620, 4285, 1371, 11442, 1074, 7838, 8049, 1114, 8040, 576, 4481, 4488, 11938, 11939, 1971, 2024, 8050, 12854, 12853, 11444, 11443, 13340, 1384, 8510, 1396, 1878, 5109, 8867, 4789, 2]
...
// Module 13339 (invite/GuildInvite)
...
import GuestUtilsDefault from "GuestUtils" /* 11444 */;
import InviteErrorUtils from "InviteErrorUtils" /* 12853 */;
import _modDef12854 from "module_12854" /* 12854 */;
import getHeaderTextForInvite from "getHeaderTextForInvite" /* 13340 */;
...
export const createExpiredGuildInvite = function createExpiredGuildInvite(author, arg1, theme) {
  ...
  if (arg1) {
    ...
    if (null != username) {
      ...
      const obj2 = { username: tmp(4481).getFormattedName(author.author) };
  ...
  if (tmp6Result2.isThemeDark(theme)) {
    let tmpResult2 = tmp(11938);
  } else {
    tmpResult2 = tmp(11939);
  }
...
export const createDisabledGuildInvite = function createDisabledGuildInvite(invite, theme) {
  ...
  obj2.thumbnailUrl = renderer_EmbedUtils.getAssetUriForEmbed(_modDef12854);
  let tmp17;
  if (null == tmp11) {
    let tmp18;
    if (null != fromInviteGuildResult) {
...
export const createGuildInvite = function createGuildInvite(invite, isOwnInvite, theme) {
  ...
  const channel = tmp(11443)(invite).channel;
  ...
  if (tmp7) {
    if (flag2) {
      if (null != target_user) {
        if (tmp12) {
          ...
          obj6.name = tmp(4481).getFormattedName(target_user);
          const tmpResult8 = tmp(4481);
    ...
    const assetSource = Image.resolveAssetSource(tmp10(5109).getChannelIcon(channel));
    let uri1;
    if (assetSource != null) {
    ...
    uri = guildIconURL;
    tmp25 = name;
    const tmp10Result7 = tmp10(5109);
  } else {
    if (null != channel) {
      if (channel.type === constants2.GUILD_STAGE_VOICE) {
        const assetSource1 = Image.resolveAssetSource(tmp10(5109).getChannelIcon(channel));
        let uri2;
    ...
    if (null == approximate_member_count) {
      ...
      tmp25 = name;
      if (null != channel) {
        const assetSource2 = Image.resolveAssetSource(tmp10(5109).getChannelIcon(channel));
        let uri3;
  ...
  if (tmp7) {
    ...
    if (null != guild) {
      guildBadgeImageSource = tmp10(8867).getGuildBadgeImageSource(guild, theme);
      const tmp10Result10 = tmp10(8867);
    }
    const obj11 = {};
    const merged = Object.assign(baseColors);
    let formatted;
    ...
    let channelName;
    if (null != channel) {
      channelName = tmp10(4789).computeChannelName(channel, UserStore, RelationshipStore);
      const tmp10Result11 = tmp10(4789);
    }
    ...
    if (null != guildBadgeImageSource) {
      assetUriForEmbed = tmp10(8050).getAssetUriForEmbed(guildBadgeImageSource);
      const tmp10Result12 = tmp10(8050);
    }
