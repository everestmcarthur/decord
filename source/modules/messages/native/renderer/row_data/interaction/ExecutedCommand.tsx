...
// Dependencies: [17, 1386, 1958, 1372, 1074, 1400, 1397, 4872, 11753, 576, 7628, 1895, 8068, 8070, 9678, 1115, 9421, 2]
...
export const createExecutedCommand = function createExecutedCommand(message, channel, roleStyle, forcedTheme, defaultUsernameColor) {
  ...
  if (null != message.interaction) {
    ...
    if (null == user2) {
      ...
      if (result1) {
        processColorStringsResult = tmp16(8068).processColorStrings(userAuthor.colorStrings);
        const tmp16Result13 = tmp16(8068);
      }
      let processColorStringsResult1 = null;
      if (result2) {
        processColorStringsResult1 = tmp16(8068).processColorStrings(userAuthor1.colorStrings);
        const tmp16Result14 = tmp16(8068);
      }
      ...
      if (null != tmp24) {
        ...
        const displayNameFontIdForMobileUser1 = tmp16(8070).getDisplayNameFontIdForMobileUser(user3, guildId);
        const tmp16Result16 = tmp16(8070);
      }
      ...
      if (tmp) {
        if (!result) {
          const result3 = tmp16(9678).isActivitiesInTextEnabled(channel);
          ...
          const tmp16Result18 = tmp16(9678);
        }
      ...
      if (result) {
        result4 = tmp16(9421).formatPrimaryEntryPointCommandName(displayName);
        const tmp16Result19 = tmp16(9421);
      }
      ...
      if (null != tmp24) {
        ...
        obj17.targetUsername = tmp16(4872).getUserAuthor(tmp24, channel).nick;
        let id3;
        if (tmp24 != null) {
          id3 = tmp24.id;
        }
        ...
        const tmp16Result20 = tmp16(4872);
      }
