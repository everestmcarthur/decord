...
// Dependencies: [17, 1385, 1957, 1371, 1074, 1399, 1396, 4839, 11712, 576, 7594, 1894, 8035, 8037, 9637, 1114, 9379, 2]
// Exports: createExecutedCommand

// Module 13285 (ExecutedCommand)
...
import createDisplayNameStylesMobile from "createDisplayNameStylesMobile" /* 8037 */;
...
export const createExecutedCommand = function createExecutedCommand(message, channel, roleStyle, forcedTheme, defaultUsernameColor) {
  ...
  if (null != message.interaction) {
    ...
    if (null == user2) {
      ...
      if (result1) {
        processColorStringsResult = tmp16(8035).processColorStrings(userAuthor.colorStrings);
        const tmp16Result13 = tmp16(8035);
      }
      let processColorStringsResult1 = null;
      if (result2) {
        processColorStringsResult1 = tmp16(8035).processColorStrings(userAuthor1.colorStrings);
        const tmp16Result14 = tmp16(8035);
      }
      ...
      if (null != tmp24) {
        ...
        const displayNameFontIdForMobileUser1 = tmp16(8037).getDisplayNameFontIdForMobileUser(user3, guildId);
        const tmp16Result16 = tmp16(8037);
      }
      ...
      if (tmp) {
        if (!result) {
          const result3 = tmp16(9637).isActivitiesInTextEnabled(channel);
          ...
          const tmp16Result18 = tmp16(9637);
        }
      ...
      if (result) {
        result4 = tmp16(9379).formatPrimaryEntryPointCommandName(displayName);
        const tmp16Result19 = tmp16(9379);
      }
      ...
      if (null != tmp24) {
        ...
        obj17.targetUsername = tmp16(4839).getUserAuthor(tmp24, channel).nick;
        let id3;
        if (tmp24 != null) {
          id3 = tmp24.id;
        }
        ...
        const tmp16Result20 = tmp16(4839);
      }
