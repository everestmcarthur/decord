...
// Dependencies: [1074, 8062, 4589, 8065, 1231, 4581, 1115, 2025, 8066, 7199, 1397, 8067, 4288, 4294, 2]
// Exports: createDefaultReaction, createForumPostActions

// Module 8064 (ForumPostActions)
  ...
  if (null != defaultReactionEmoji) {
    ...
    if (null != emojiId) {
      if (null != customGuildEmoji) {
        ...
        const obj4 = { id: emojiId, name: "a", animated: customGuildEmoji.animated, src: "baby_angel", displayName: "r" };
...
export const createForumPostActions = function createForumPostActions(arg0) {
  ...
  if (isFollowing) {
    let assetUriForEmbed = getAssetUriForEmbed(tmp4(4589));
    let tmp6 = tmp4;
    let tmp8 = tmp;
  } else {
    assetUriForEmbed = getAssetUriForEmbed(tmp4(8065));
    tmp6 = tmp4;
    tmp8 = tmp;
  }
  if (null != assetUriForEmbed) {
    let stringResult;
    const assetUriForEmbed1 = tmp8(8062).getAssetUriForEmbed(tmp6(4581));
    if (!hasReactions) {
      let emoji;
    ...
    if (showMediaPostSharePrompt) {
      ...
      obj.icon = tmp8(8062).getAssetUriForEmbed(tmp6(8066));
      const tmp8Result3 = tmp8(8062);
      obj.closeIcon = tmp8(8062).getAssetUriForEmbed(tmp6(7199));
      tmp14 = obj;
      const tmp8Result4 = tmp8(8062);
    }
    ...
    tmp6(1231).captureMessage("Forum follow is null. isFollowing: " + isFollowing + " icon: " + tmp6(isFollowing ? 4589 : 8065));
