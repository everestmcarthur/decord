...
export const createCtaButtons = function createCtaButtons(id, channel_id, arg2) {
  ...
  if (CtaButtonUtils.CtaButtonType.MARK_AS_FALSE_POSITIVE === ctaButtonType) {
    ...
    if (ExplicitMediaStore.canSubmitFpReport(id)) {
      prop = tmp(12005).CtaButtonType.MARK_AS_FALSE_POSITIVE;
    }
    const obj7 = { ctaButton: null };
    obj4.callback = prop;
    obj7.ctaButton = obj4;
    return obj7;
  } else if (tmp(12005).CtaButtonType.AGE_VERIFICATION_RETRY === ctaButtonType) {
    ...
    let prop1;
    if (result) {
      prop1 = tmp(12005).CtaButtonType.AGE_VERIFICATION_RETRY;
    }
    ...
    if (tmpResult.isAgeVerificationMessageWithManualReviewCta(channel_id, id)) {
      ...
      let prop2;
      if (result) {
        prop2 = tmp(12005).CtaButtonType.AGE_VERIFICATION_MANUAL_REVIEW;
      }
    ...
    return obj11;
  } else if (tmp(12005).CtaButtonType.CONNECT_TO_TEEN === ctaButtonType) {
    ...
    obj20.callback = tmp(12005).CtaButtonType.CONNECT_TO_TEEN;
