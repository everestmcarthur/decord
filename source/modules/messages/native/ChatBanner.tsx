...
// Function ID: 11575
// Name: ChatBanner
// Dependencies: [19, 17, 5365, 4655, 11575, 1074, 21, 4640, 576, 11576, 11577, 504, 11, 1241, 4820, 7227, 4636, 1115, 5062, 7383, 7885, 5210, 7224, 8109, 2]
// Exports: default

// Module 11574 (ChatBanner)
...
import useShowChannelOptInNoticeDefault from "useShowChannelOptInNotice" /* 11576 */;
import useAllowedChatOverlaysDefault from "useAllowedChatOverlays" /* 11577 */;
...
function ArchivedLockedThreadChatBanner(channel) {
  ...
  let canUnarchiveThread = channel(7383).useCanUnarchiveThread(channel);
  ...
  const items1 = [closure_12(channel(4636).Text, obj3), ];
  if (canUnarchiveThread) {
    ...
    obj4.children = tmp8(tmp3(5062).Button, obj5);
...
function LockedThreadChatBanner(channel) {
  ...
  let isThreadModerator = channel(7383).useIsThreadModerator(channel);
  let obj2 = { style: tmp.threadBannerContainer, children: null };
  let obj3 = { lineClamp: 4, style: tmp.threadBannerTitle, variant: "text-sm/medium", color: "text-default", children: null };
  let obj = channel(7383);
  const tmp6 = closure_13;
...
function NewMessagesChatBar(channel) {
  ...
  if (obj.useStateFromStores(items, () => connected.isConnected(), [])) {
    if (unreadCount <= 0) {
      ...
      obj2.children = closure_12(XSmallBoldIcon(4636).Text, obj3);
      const items1 = [closure_12(XSmallBoldIcon(5210).PressableOpacity, obj2), ];
      handleScrollToNewMessages = XSmallBoldIcon(5210).PressableOpacity;
      ...
      XSmallBoldIcon = XSmallBoldIcon(8109).XSmallBoldIcon;
...
const ChatOverlays = fn(11575).ChatOverlays;
