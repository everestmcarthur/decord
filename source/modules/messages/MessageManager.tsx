...
// Function ID: 10093
// Name: MessageManager
// Dependencies: [32, 4656, 5365, 1962, 7394, 1958, 1980, 4655, 2012, 4461, 1074, 1965, 1091, 3, 5360, 10093, 4570, 8523, 7570, 10094, 510, 4466, 1101, 7396, 4984, 1115, 573, 7232, 2]

// Module 10092 (MessageManager)
...
import getAdaptiveMessageLimit from "getAdaptiveMessageLimit" /* 10094 */;
      ...
      if (type !== constants.GUILD_STORE) {
        ...
        if (null == type1) {
          ...
          if (orCreate.some(AttachmentUrlUtilsAll.messageHasExpiredAttachmentUrl)) {
            ...
            tmp9(5360).clear(channelId);
            const tmp9Result = tmp9(5360);
            orCreate1 = tmp9(5360).getOrCreate(channelId);
            const tmp9Result9 = tmp9(5360);
          }
          let obj7 = orCreate1;
          if (tmp15) {
            const obj = { jumpTargetId: null, jumped: false, jumpType: Client.JumpType.ANIMATED };
            const mutation = orCreate1.mutate(obj);
            tmp9(5360).commit(mutation);
            obj7 = mutation;
            const tmp9Result10 = tmp9(5360);
          }
          let obj10 = obj7;
          if (tmp19) {
            const mutation1 = obj7.mutate({ focusTargetId: null });
            tmp9(5360).commit(mutation1);
            obj10 = mutation1;
            const tmp9Result11 = tmp9(5360);
          }
          if (isPreload) {
            if (!GatewayConnectionStore.isConnected()) {
              let flag = true;
            }
            let hasUnreadResult = tmp9(8523)(channelId);
            if (hasUnreadResult) {
            ...
            if (flag) {
              tmp9(5360).commit(obj10.mutate({ loadingMore: true }));
              if (null == messageId) {
                let isThreadResult;
                if (channel != null) {
                ...
                tmp9(7570).jumpToMessage(obj12);
                const tmp9Result16 = tmp9(7570);
              }
              const tmp9Result12 = tmp9(5360);
            }
          }
