...
// Function ID: 10082
// Name: MessageManager
// Dependencies: [32, 4654, 5363, 1962, 7390, 1958, 1980, 4653, 2012, 4459, 1074, 1965, 1091, 3, 5358, 10082, 4568, 8504, 7566, 10083, 510, 4464, 1101, 7392, 4982, 1115, 573, 7228, 2]

// Module 10081 (MessageManager)
...
import SidebarActionTypes from "SidebarActionTypes" /* 7392 */;
...
import getAdaptiveMessageLimit from "getAdaptiveMessageLimit" /* 10083 */;
      ...
      if (type !== constants.GUILD_STORE) {
        ...
        if (null == type1) {
          ...
          if (orCreate.some(AttachmentUrlUtilsAll.messageHasExpiredAttachmentUrl)) {
            ...
            tmp9(5358).clear(channelId);
            const tmp9Result = tmp9(5358);
            orCreate1 = tmp9(5358).getOrCreate(channelId);
            const tmp9Result9 = tmp9(5358);
          }
          let obj7 = orCreate1;
          if (tmp15) {
            const obj = { jumpTargetId: null, jumped: false, jumpType: Client.JumpType.ANIMATED };
            const mutation = orCreate1.mutate(obj);
            tmp9(5358).commit(mutation);
            obj7 = mutation;
            const tmp9Result10 = tmp9(5358);
          }
          let obj10 = obj7;
          if (tmp19) {
            const mutation1 = obj7.mutate({ focusTargetId: null });
            tmp9(5358).commit(mutation1);
            obj10 = mutation1;
            const tmp9Result11 = tmp9(5358);
          }
          if (isPreload) {
            if (!GatewayConnectionStore.isConnected()) {
              let flag = true;
            }
            let hasUnreadResult = tmp9(8504)(channelId);
            if (hasUnreadResult) {
            ...
            if (flag) {
              tmp9(5358).commit(obj10.mutate({ loadingMore: true }));
              if (null == messageId) {
                let isThreadResult;
                if (channel != null) {
                ...
                tmp9(7566).jumpToMessage(obj12);
                const tmp9Result16 = tmp9(7566);
              }
              const tmp9Result12 = tmp9(5358);
            }
          }
