...
// Function ID: 10074
// Name: MessageManager
// Dependencies: [32, 4655, 5362, 1962, 7387, 1958, 1980, 4654, 2012, 4461, 1074, 1965, 1091, 3, 5357, 10074, 4570, 8495, 7563, 10075, 510, 4466, 1101, 7389, 4983, 1115, 573, 7225, 2]

// Module 10073 (MessageManager)
...
import getAdaptiveMessageLimit from "getAdaptiveMessageLimit" /* 10075 */;
      ...
      if (type !== constants.GUILD_STORE) {
        ...
        if (null == type1) {
          ...
          if (orCreate.some(AttachmentUrlUtilsAll.messageHasExpiredAttachmentUrl)) {
            ...
            tmp9(5357).clear(channelId);
            const tmp9Result = tmp9(5357);
            orCreate1 = tmp9(5357).getOrCreate(channelId);
            const tmp9Result9 = tmp9(5357);
          }
          let obj7 = orCreate1;
          if (tmp15) {
            const obj = { jumpTargetId: null, jumped: false, jumpType: Client.JumpType.ANIMATED };
            const mutation = orCreate1.mutate(obj);
            tmp9(5357).commit(mutation);
            obj7 = mutation;
            const tmp9Result10 = tmp9(5357);
          }
          let obj10 = obj7;
          if (tmp19) {
            const mutation1 = obj7.mutate({ focusTargetId: null });
            tmp9(5357).commit(mutation1);
            obj10 = mutation1;
            const tmp9Result11 = tmp9(5357);
          }
          if (isPreload) {
            if (!GatewayConnectionStore.isConnected()) {
              let flag = true;
            }
            let hasUnreadResult = tmp9(8495)(channelId);
            if (hasUnreadResult) {
            ...
            if (flag) {
              tmp9(5357).commit(obj10.mutate({ loadingMore: true }));
              if (null == messageId) {
                let isThreadResult;
                if (channel != null) {
                ...
                tmp9(7563).jumpToMessage(obj12);
                const tmp9Result16 = tmp9(7563);
              }
              const tmp9Result12 = tmp9(5357);
            }
          }
