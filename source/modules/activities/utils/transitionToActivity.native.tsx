// Module ID: 9689
// Function ID: 9690
// Name: transitionToActivity
// Dependencies: [9621, 1957, 9659, 9622, 4268, 4497, 9620, 9682, 9690, 4841, 9630, 4607, 9658, 2]
// Exports: default

// Module 9689 (transitionToActivity)
import embeddedActivityLocationUtils from "embeddedActivityLocationUtils" /* 4268 */;
import ChannelCallModalDefault from "ChannelCallModal" /* 9620 */;
import ChannelCallStore from "ChannelCallStore" /* 9621 */;
import ChannelCallConstants from "ChannelCallConstants" /* 9622 */;
import ActivityPanelConstants from "ActivityPanelConstants" /* 9659 */;
import EmbeddedActivitiesStore from "EmbeddedActivitiesStore" /* 1957 */;
import size from "module_2" /* 2 */;

const setVoiceChatDrawerState = ChannelCallStore.setVoiceChatDrawerState;
const ActivityPanelModes = ActivityPanelConstants.ActivityPanelModes;
const VoiceChatDrawerState = ChannelCallConstants.VoiceChatDrawerState;
let result = size.fileFinishedImporting("modules/activities/utils/transitionToActivity.native.tsx");

export default function transitionToActivity(arg0, _location) {
  const embeddedActivityLocationChannelId = embeddedActivityLocationUtils.getEmbeddedActivityLocationChannelId(_location);
  if (null != embeddedActivityLocationChannelId) {
    const isModalOpenResult = tmp(4497).isModalOpen(ChannelCallModalDefault);
    let tmp4 = !isModalOpenResult;
    if (!isModalOpenResult) {
      tmp4 = tmp15(9682)(embeddedActivityLocationChannelId);
    }
    if (tmp4) {
      tmp15(9690)(embeddedActivityLocationChannelId);
    }
    const selfEmbeddedActivityForLocation = EmbeddedActivitiesStore.getSelfEmbeddedActivityForLocation(_location);
    if (null != selfEmbeddedActivityForLocation) {
      if (tmp15(9682)(embeddedActivityLocationChannelId)) {
        const tmp15Result = tmp15(4841);
        ({ applicationId: obj5.applicationId, compositeInstanceId: obj5.instanceId } = selfEmbeddedActivityForLocation);
        const participant = tmp15Result.selectParticipant(embeddedActivityLocationChannelId, tmp(9630).getEmbeddedActivityParticipantId({ applicationId: null, instanceId: null }));
        const obj2 = { applicationId: null, instanceId: null };
        const tmpResult3 = tmp(9630);
        tmp15(4607).hideActionSheet();
        setVoiceChatDrawerState(embeddedActivityLocationChannelId, VoiceChatDrawerState.CLOSED);
        const tmp15Result2 = tmp15(4607);
      } else {
        const result = tmp(9658).updateActivityPanelMode(ActivityPanelModes.PANEL);
        const tmpResult4 = tmp(9658);
      }
    }
    const tmpResult = tmp(4497);
  }
};
