...
// Dependencies: [13038, 8225, 13035, 2]

// Module 13037 (CustomActivityLinkRecord)
import utils_CustomActivityLinkUtils from "utils/CustomActivityLinkUtils" /* 13038 */;
...
CustomActivityLinkRecord.prototype["getAssetURL"] = function getAssetURL() {
  ...
  if (this.type === utils_CustomActivityLinkUtils.CustomLinkType.MANAGED) {
    let assetImage = tmp(8225).getAssetImage(self.applicationId, self.assetId, 512);
    const tmpResult = tmp(8225);
  } else if (self.type === tmp(13038).CustomLinkType.QUICK) {
    assetImage = tmp(13035).getQuickLinkImage(self.assetPath);
    const tmpResult2 = tmp(13035);
  }
  return assetImage;
};
