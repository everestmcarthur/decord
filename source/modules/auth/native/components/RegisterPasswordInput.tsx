...
// Dependencies: [109, 32, 19, 7051, 16076, 21, 4638, 576, 4374, 16100, 1115, 4634, 16094, 7065, 504, 4856, 6714, 4344, 7076, 7078, 2]

// Module 16103 (RegisterPasswordInput)
import nativeDefault from "native" /* 576 */;
import util from "util" /* 1115 */;
import Text_Text from "Text/Text" /* 4634 */;
import usePasswordScore from "usePasswordScore" /* 16100 */;
      ...
      if (0 !== password.length) {
        if (null == passwordError) {
          if (passwordScore <= usePasswordScore.PasswordScore.WEAK) {
            ...
            let strong = tmp.weak;
          } else if (passwordScore === tmp9(16100).PasswordScore.MEDIUM) {
            const intl = tmp9(1115).intl;
            str = intl.string(tmp9(1115).t["2fmTpT"]);
            strong = tmp.medium;
          } else {
            str = "";
            if (passwordScore === tmp9(16100).PasswordScore.STRONG) {
...
obj7.layout = LinearTransition.easing(Easing.inOut(fn(4374).Easing.quad)).duration(300);
const easingResult = LinearTransition.easing(Easing.inOut(fn(4374).Easing.quad));
const size = fn(2);
