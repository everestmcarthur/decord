...
// Dependencies: [109, 32, 19, 7048, 16056, 21, 4639, 576, 4376, 16080, 1115, 4635, 16074, 7062, 504, 4857, 6711, 4346, 7073, 7075, 2]

// Module 16083 (RegisterPasswordInput)
import nativeDefault from "native" /* 576 */;
import util from "util" /* 1115 */;
import Text_Text from "Text/Text" /* 4635 */;
import usePasswordScore from "usePasswordScore" /* 16080 */;
      ...
      if (0 !== password.length) {
        if (null == passwordError) {
          if (passwordScore <= usePasswordScore.PasswordScore.WEAK) {
            ...
            let strong = tmp.weak;
          } else if (passwordScore === tmp9(16080).PasswordScore.MEDIUM) {
            const intl = tmp9(1115).intl;
            str = intl.string(tmp9(1115).t["2fmTpT"]);
            strong = tmp.medium;
          } else {
            str = "";
            if (passwordScore === tmp9(16080).PasswordScore.STRONG) {
...
obj7.layout = LinearTransition.easing(Easing.inOut(fn(4376).Easing.quad)).duration(300);
const easingResult = LinearTransition.easing(Easing.inOut(fn(4376).Easing.quad));
const size = fn(2);
