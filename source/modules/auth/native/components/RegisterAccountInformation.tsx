...
// Dependencies: [5, 32, 19, 17, 6703, 4622, 8879, 16076, 16077, 1074, 21, 4638, 576, 4374, 7052, 16073, 504, 16099, 16101, 16084, 1115, 16092, 16075, 16091, 7080, 16102, 16103, 5060, 7049, 5664, 2]
// Exports: default

// Module 16098 (RegisterAccountInformation)
...
import AuthFormViewDefault from "AuthFormView" /* 7080 */;
import useInitialRegistrationStepDefault from "useInitialRegistrationStep" /* 16091 */;
import useAuthFlowBackHandlerDefault from "useAuthFlowBackHandler" /* 16092 */;
...
const LinearTransition = fn(4374).LinearTransition;
const Easing = fn(4374).Easing;
obj2.layout = LinearTransition.easing(Easing.inOut(fn(4374).Easing.quad)).duration(300);
const size = fn(2);
...
export default function RegisterAccountInformation() {
  ...
  const passwordRegistrationStep = context(16099).usePasswordRegistrationStep();
  ({ password, validatePassword: _slicedToArray, setPassword, passwordScore, preventSubmitPassword } = passwordRegistrationStep);
  let obj4 = context(16099);
  const usernameRegistrationStep = context(16101).useUsernameRegistrationStep(AuthStates.REGISTER_ACCOUNT_INFORMATION);
  ...
  const result = context(16084).hasAllRegistrationFieldsCompleted(obj6, { isConsentRequired: true === stateFromStores1 });
  ConsentStore = result;
  ...
  const tmp5Result3 = context(16084);
  const tmpResult = useAuthFlowBackHandlerDefault;
  tmpResult(context(16075).getPreviousRegistrationTransitionStep(AuthStates.REGISTER_ACCOUNT_INFORMATION));
  ...
  const tmp5Result4 = context(16075);
  ...
  const items4 = [closure_17(closure_6, { style: tmp4.container, children: closure_17(context(16102).RegisterUsernameInput, { username, setUsername, onSubmitEditing: callback, usernameStatus, submitBehavior: "submit", autoFocus: true }) }), , ];
  ...
  obj11.children = closure_17(context(16103).RegisterPasswordInput, obj12);
  ...
  const items5 = [closure_17(context(5060).Button, obj14), ];
  let tmp27Result = null;
  if (null != tmp11.message) {
    tmp27Result = null;
    if ("" !== tmp11.message) {
      const obj15 = { style: tmp4.errors, children: tmp11.message };
      tmp27Result = tmp27(tmp(7049), obj15);
    }
  }
  ...
  if (!tmp3) {
    ...
    tmp27Result4 = tmp27(tmp(5664), obj16);
  }
  return tmp27Result4;
};
