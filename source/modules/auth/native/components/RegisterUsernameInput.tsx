...
// Dependencies: [109, 32, 19, 16028, 21, 4605, 4341, 14784, 5023, 6660, 576, 4601, 1114, 16046, 6656, 1115, 2]
// Exports: RegisterUsernameInput

// Module 16054 (RegisterUsernameInput)
import nativeDefault from "native" /* 576 */;
import UniqueUsernamesTypes from "UniqueUsernamesTypes" /* 14784 */;
import useFocusRefOnNavigationDefault from "useFocusRefOnNavigation" /* 16046 */;
...
function UsernameStatusMessage(arg0) {
  ...
  if (type === UniqueUsernamesTypes.NameValidationState.ERROR) {
    ...
    const items = [React7(tmp3(6660).CircleErrorIcon, obj3), ];
    ...
    items[1] = React7(tmp3(4601).Text, obj4);
    obj2.children = items;
    let tmp6 = closure_1_10(tmp3(5023).Stack, obj2);
  } else {
    if (isUsernameFocused) {
      ...
      if (type1 === tmp3(14784).NameValidationState.AVAILABLE) {
        const obj5 = {};
        ...
        obj5.children = React7(tmp3(4601).Text, obj6);
        tmp6 = React7(tmp3(4601).Text, obj5);
      }
    }
    tmp6 = null;
    if (isUsernameFocused) {
      ...
      tmp6 = React7(tmp3(4601).Text, obj);
    }
  }
  return tmp6;
}
...
const FadeIn = fn(4341).FadeIn;
obj2.entering = FadeIn.duration(300);
const FadeOut = fn(4341).FadeOut;
...
obj3.layout = LinearTransition.easing(Easing.inOut(fn(4341).Easing.quad)).duration(300);
const size = fn(2);
...
export const RegisterUsernameInput = function RegisterUsernameInput(setUsername) {
  ...
  let str2;
  if (type === tmp14(14784).NameValidationState.ERROR) {
    str2 = "error";
  }
  const obj6 = { children: null };
  obj3.status = str2;
  obj3.submitBehavior = submitBehavior;
  const items3 = [closure_9(setUsername(6656).TextInput, obj3), closure_9(UsernameStatusMessage, { usernameStatus, isUsernameFocused: tmp5[0] })];
