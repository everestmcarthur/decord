...
// Dependencies: [5, 32, 19, 16098, 16099, 1074, 21, 16095, 16114, 5079, 7159, 1115, 7193, 16127, 7194, 2]
// Exports: default

// Module 16126 (components/VerifyPhone)
import RegistrationBailoutButtonDefault from "RegistrationBailoutButton" /* 16127 */;
...
const require = fn;
let closure_6 = fn(16098).doesRegistrationHaveIdentityType;
...
export default function VerifyPhone(phone) {
  ...
  onPhoneTokenReceived(16114)(closure_7(sourceState));
  const items = [context];
  ...
  onBail(function*(arg0) {
    ...
    yield onPhoneTokenReceived(7159).verifyPhone(closure_0, closure_0, false);
    if (1 === tmp7) {
      v0 = 0;
  ...
  onPhoneTokenReceived(7193)(callback1);
  const items3 = [onBail];
  ...
  const tmp7 = onPhoneTokenReceived(16114);
  obj.codeType = require("CodeField").CodeType.NUMERIC;
  obj.footer = memo;
  obj.disabled = tmp5;
  return jsx(onPhoneTokenReceived(7194), { title, description, error: tmp3, onCodeEntered, codeType: null, footer: null, disabled: null, loading: null, disableKeyboardAvoidingView: true });
};
