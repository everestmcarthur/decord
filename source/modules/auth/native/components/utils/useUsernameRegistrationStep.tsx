...
// Dependencies: [32, 19, 14924, 16195, 16196, 16192, 1484, 7151, 14922, 14921, 16194, 1485, 1115, 2]
// Exports: useUsernameRegistrationStep

// Module 16220 (useUsernameRegistrationStep)
...
require = fn;
const useRegistrationUIStore = fn(16195).useRegistrationUIStore;
...
export const useUsernameRegistrationStep = function useUsernameRegistrationStep(REGISTER_ACCOUNT_INFORMATION) {
  ...
  const callback = obj.useCallback((arg0) => {
    ...
    if (arg0) {
      const result = tmp3(16194).handleRegistrationSubmit(closure_0, navigation, context);
      const tmp3Result = tmp3(16194);
    } else {
      const obj2 = { step: React5(closure_0), toStep: tmp3(16194).getNextRegistrationTransitionStep(closure_0), actionType: constants.SUCCESS };
      context(obj2);
      const tmp3Result3 = tmp3(16194);
      const nextAuthState = tmp3(16194).getNextAuthState(closure_0);
      const StackActions = tmp3(1485).StackActions;
      navigation.dispatch(StackActions.push(nextAuthState));
      const tmp3Result4 = tmp3(16194);
    }
  }, items);
