...
// Dependencies: [5, 32, 19, 17, 16056, 16057, 21, 4639, 5066, 7049, 1484, 16066, 1094, 16053, 16071, 16072, 16055, 5076, 7068, 7077, 16073, 5059, 1115, 7046, 5663, 16075, 16076, 9924, 9925, 2]
// Exports: RegisterIdentity

// Module 16065 (RegisterIdentity)
...
function RegisterIdentityBase(inputMode) {
  ...
  const tmp2 = closure_16(45 * Math.min(2, inputMode(5066).useFontScale()));
  let obj = inputMode(5066);
  const tmp4 = setInputMode(7049)();
  dependencyMap = inputMode(1484).useNavigation();
  const tmp5 = closure_11((errors) => errors.errors);
  let message = tmp5;
  let obj2 = inputMode(1484);
  const identityRegistrationStep = inputMode(16066).useIdentityRegistrationStep(inputMode(1094).AuthStates.REGISTER_IDENTITY, inputMode);
  ...
  let obj3 = inputMode(16066);
  [tmp8, c8] = loginEmail(identityErrorMessage.useState(false), 2);
  closure_9 = identityErrorMessage.useContext(inputMode(16053).TrackRegistrationContext);
  ...
  setInputMode(16071)(inputMode(1094).AuthStates.REGISTER_IDENTITY);
  const tmp10 = setInputMode(16071);
  const tmp12 = setInputMode(16072);
  tmp12(inputMode(16055).getPreviousRegistrationTransitionStep(inputMode(1094).AuthStates.REGISTER_IDENTITY));
  setInputMode(5076)(() => {
  ...
  let obj4 = inputMode(16055);
  items1[1] = closure_14(inputMode(16073).RegisterPhoneOrEmailInput, { loginPhone, loginEmail, setLoginPhone: updateLoginPhone, setLoginEmail, inputMode, onSubmit: handleSubmit, inputError: identityError, autoFocus: true });
  ...
  obj7.children = closure_14(inputMode(5059).Button, obj8);
...
export const RegisterIdentity = function RegisterIdentity() {
  let obj = hasItem(5066);
  const tmp3 = closure_16(45 * Math.min(2, hasItem(5066).useFontScale()));
  const deviceCountry = hasItem(16075).getDeviceCountry();
  hasItem = null != deviceCountry;
  if (hasItem) {
    const EMAIL_FIRST_COUNTRIES = tmp(16076).EMAIL_FIRST_COUNTRIES;
  ...
  let obj2 = hasItem(16075);
  const tmpResult = hasItem(9924);
  ...
  obj4.controlComponent = closure_14(closure_6, { style: tmp3.segmentedControl, children: closure_14(hasItem(9925).SegmentedControl, { state: segmentedControlState, keyboardShouldPersistTaps: "handled" }) });
