...
// Dependencies: [5, 32, 19, 17, 16076, 16077, 21, 4638, 5067, 7052, 1484, 16086, 1094, 16073, 16091, 16092, 16075, 5077, 7071, 7080, 16093, 5060, 1115, 7049, 5664, 16095, 16096, 9932, 9933, 2]
// Exports: RegisterIdentity

// Module 16085 (RegisterIdentity)
...
function RegisterIdentityBase(inputMode) {
  ...
  const tmp2 = closure_16(45 * Math.min(2, inputMode(5067).useFontScale()));
  let obj = inputMode(5067);
  const tmp4 = setInputMode(7052)();
  dependencyMap = inputMode(1484).useNavigation();
  const tmp5 = closure_11((errors) => errors.errors);
  let message = tmp5;
  let obj2 = inputMode(1484);
  const identityRegistrationStep = inputMode(16086).useIdentityRegistrationStep(inputMode(1094).AuthStates.REGISTER_IDENTITY, inputMode);
  ...
  let obj3 = inputMode(16086);
  [tmp8, c8] = loginEmail(identityErrorMessage.useState(false), 2);
  closure_9 = identityErrorMessage.useContext(inputMode(16073).TrackRegistrationContext);
  ...
  setInputMode(16091)(inputMode(1094).AuthStates.REGISTER_IDENTITY);
  const tmp10 = setInputMode(16091);
  const tmp12 = setInputMode(16092);
  tmp12(inputMode(16075).getPreviousRegistrationTransitionStep(inputMode(1094).AuthStates.REGISTER_IDENTITY));
  setInputMode(5077)(() => {
  ...
  let obj4 = inputMode(16075);
  items1[1] = closure_14(inputMode(16093).RegisterPhoneOrEmailInput, { loginPhone, loginEmail, setLoginPhone: updateLoginPhone, setLoginEmail, inputMode, onSubmit: handleSubmit, inputError: identityError, autoFocus: true });
  ...
  obj7.children = closure_14(inputMode(5060).Button, obj8);
...
export const RegisterIdentity = function RegisterIdentity() {
  let obj = hasItem(5067);
  const tmp3 = closure_16(45 * Math.min(2, hasItem(5067).useFontScale()));
  const deviceCountry = hasItem(16095).getDeviceCountry();
  hasItem = null != deviceCountry;
  if (hasItem) {
    const EMAIL_FIRST_COUNTRIES = tmp(16096).EMAIL_FIRST_COUNTRIES;
  ...
  let obj2 = hasItem(16095);
  const tmpResult = hasItem(9932);
  ...
  obj4.controlComponent = closure_14(closure_6, { style: tmp3.segmentedControl, children: closure_14(hasItem(9933).SegmentedControl, { state: segmentedControlState, keyboardShouldPersistTaps: "handled" }) });
