...
// Dependencies: [19, 17, 7646, 1220, 1976, 7750, 2016, 4655, 4821, 1372, 1074, 7211, 8080, 4822, 21, 4640, 576, 504, 7313, 1115, 11123, 7449, 4460, 1944, 7642, 4607, 4843, 11699, 1897, 12443, 4792, 7276, 7296, 9361, 1177, 9954, 14025, 1186, 1941, 9221, 14026, 8313, 9338, 4636, 1936, 7109, 7233, 10293, 7228, 14074, 10290, 14075, 9824, 5495, 9888, 9849, 11719, 14076, 4986, 38, 14023, 10248, 10247, 7649, 11705, 14077, 8784, 8153, 11981, 7302, 7303, 4337, 1397, 2]
...
// Module 14024 (GuildActionSheetActions)
...
import TidaWebformExperimentDefault from "TidaWebformExperiment" /* 7302 */;
import ClipboardUtils from "ClipboardUtils" /* 7303 */;
import useGuildOnboardingAvailableDefault from "useGuildOnboardingAvailable" /* 7449 */;
import ChannelListState from "ChannelListState" /* 7642 */;
import GuildAntiRaidTypes from "GuildAntiRaidTypes" /* 8153 */;
import ReportModals from "ReportModals" /* 8784 */;
import useOpenProfileSettingsDefault from "useOpenProfileSettings" /* 9361 */;
...
import CreateChannelModalActionCreatorsDefault from "CreateChannelModalActionCreators" /* 9888 */;
import ChannelCollapseActionCreatorsDefault from "ChannelCollapseActionCreators" /* 11123 */;
import OptInOnboardingUtils from "OptInOnboardingUtils" /* 11705 */;
import GuildRoleConnectionsModalActionCreators from "GuildRoleConnectionsModalActionCreators" /* 11719 */;
import useIsServerThemeAvailableForGuildDefault from "useIsServerThemeAvailableForGuild" /* 14025 */;
import markGuildsAsReadDefault from "markGuildsAsRead" /* 14074 */;
import GuildAntiRaidModalActionCreators from "GuildAntiRaidModalActionCreators" /* 14077 */;
...
const DiscordTagDefault = tmp2(9954);
require = fn;
function HideMutedChannelsOption(guild) {
  ...
  return closure_21(guild(7313).ActionSheetSwitchRow, obj2);
}
function BrowseChannelsOption(guild) {
  ...
  const callback = noop.useCallback(() => {
    ...
    ModalActionCreatorsDefault.pushLazy(asyncRequireImpl(11699, dependencyMap.paths), { guildId: guild.id }, closure_18);
  }, items2);
  if (!features.has(constants2.COMMUNITY)) {
    ...
    tmp8Result2 = tmp8(tmp3(12443).NewBadge, {});
  }
}
...
function GuildThemePreferenceOption(guild) {
  ...
  if (tmp2) {
    ...
    tmp6 = closure_21(tmp3(7313).ActionSheetSwitchRow, obj2);
  }
  return tmp6;
}
function ServerTagOption(guild) {
  ...
  const tmp = closure_23();
  [][0] = guild.id;
  const result = guild(9221).canViewMobileServerTag(guild.id);
  if (result) {
    ...
    const guildTagBadgeUrl = tmp2(8313).getGuildTagBadgeUrl(guild.id, badge, GuildTagBadgeSize.SIZE_16);
    ...
    obj4.children = closure_21(tmp2(9338).BaseGuildTagChiplet, obj5);
    ...
    items[1] = closure_21(tmp2(4636).Text, obj6);
    obj2.children = items;
    obj3.label = closure_22(View, obj2);
    obj3.onPress = tmp5;
    return closure_21(tmp2(7313).ActionSheetRow, obj3);
  } else {
    return null;
  }
  let obj = guild(9221);
...
let closure_18 = fn(7211).CHANNELS_AND_ROLES_MODAL_KEY;
    ...
    t = {
      ...
      onPress() {
          if (stateFromStores1) {
            ...
            obj2.openLazy(asyncRequireImpl(10290, dependencyMap.paths), "muteSettings" + guild.id, obj3);
            const tmp5 = asyncRequireImpl(10290, dependencyMap.paths);
          }
        }
    };
...
export const GuildActionSheetPrimaryActions = function GuildActionSheetPrimaryActions(guild) {
  ...
  if (obj.useGuildActionSheetPermissions(guild).canManageChannels) {
    ...
    items1.push(closure_21(tmp(7313).ActionSheetRow, obj3));
    ...
    items1.push(closure_21(tmp(7313).ActionSheetRow, obj4));
  }
  if (tmp3) {
    ...
    items1.push(closure_21(tmp(7313).ActionSheetRow, obj5));
  }
  if (stateFromStores) {
...
export const GuildActionSheetDirectoryActions = function GuildActionSheetDirectoryActions(guild) {
  ...
  if (tmp4) {
    ...
    if (0 !== items.length) {
      ...
      tmp5Result = tmp5(tmp9(7313).ActionSheetRow.Group, obj2);
    }
    return tmp5Result;
  } else {
    const features = guild.features;
    ...
    t = items.push(tmp5(tmp9(7313).ActionSheetRow, obj3));
  }
};
export const GuildActionSheetSecondaryActions = function GuildActionSheetSecondaryActions(guild) {
  ...
  const canReportRaid = guild(10248).useCanReportRaid(guild);
  let obj = guild(10248);
  const guildIncidentsState = guild(10247).useGuildIncidentsState(guild.id);
  const items = [];
  ({ shouldShowIncidentActions, isUnderLockdown } = guildIncidentsState);
  let obj2 = guild(10247);
  const optInEnabledForGuild = guild(7649).useOptInEnabledForGuild(guild.id);
  items.push(closure_21(ChangeIdentityOption, { guild, user: currentUser }));
  ...
  if (features.has(constants2.COMMUNITY)) {
    ...
    items.push(tmp8(tmp3(7313).ActionSheetSwitchRow, obj4));
  }
  items.push(closure_21(HideMutedChannelsOption, { guild }));
  items.push(closure_21(RestrictedGuildPrivacyOption, { guild }));
  const obj3 = guild(7649);
  ...
  if (canReportRaid) {
    ...
    items.push(tmp8(tmp3(7313).ActionSheetRow, obj5));
  }
  if (!tmp2) {
    const obj6 = { label: null, onPress: null };
    ...
    items.push(tmp8(tmp3(7313).ActionSheetRow, obj6));
  }
  if (!shouldShowIncidentActions) {
    if (tmp2) {
      ...
      if (0 !== items.length) {
        ...
        tmp8Result = tmp8(tmp3(7313).ActionSheetRow.Group, obj7);
      }
      return tmp8Result;
    } else {
      ...
      t2 = items.push(tmp8(tmp3(7313).ActionSheetRow, obj8));
    }
  } else {
    ...
    const obj9 = {
      ...
      onPress() {
          ...
          ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(11981, dependencyMap.paths), "GuildIncidentActionsActionSheet", { guild, analyticsData: obj2 });
        }
    };
    t = items.push(tmp8(tmp3(7313).ActionSheetRow, obj9));
  }
};
export const GuildDeveloperOptionAction = function GuildDeveloperOptionAction(guild) {
  ...
  if (setting) {
    ...
    items.push(closure_21(tmp(7313).ActionSheetRow, obj));
    if (tmp5) {
      if (null != guild.icon) {
        ...
        items.push(tmp7(tmp(7313).ActionSheetRow, obj2));
      }
      if (null != guild.banner) {
        ...
        items.push(tmp7(tmp(7313).ActionSheetRow, obj3));
      }
    }
    ...
    return closure_21(tmp(7313).ActionSheetRow.Group, obj4);
  } else {
    return null;
  }
};
