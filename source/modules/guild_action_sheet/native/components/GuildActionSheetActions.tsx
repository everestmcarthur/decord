...
// Dependencies: [19, 17, 7642, 1220, 1976, 7744, 2016, 4653, 4819, 1372, 1074, 7207, 8060, 4820, 21, 4638, 576, 504, 7309, 1115, 11115, 7445, 4458, 1944, 7638, 4605, 4841, 11691, 1897, 12434, 4790, 7272, 7292, 9350, 1177, 9943, 14017, 1186, 1941, 9210, 14018, 8294, 9327, 4634, 1936, 7105, 7229, 10285, 7224, 14066, 10282, 14067, 9813, 5493, 9877, 9838, 11711, 14068, 4984, 38, 14015, 10237, 10236, 7645, 11697, 14069, 8764, 8133, 11972, 7298, 7299, 4335, 1397, 2]
...
// Module 14016 (GuildActionSheetActions)
...
import ConnectionsUtils from "ConnectionsUtils" /* 5493 */;
...
import TidaWebformExperimentDefault from "TidaWebformExperiment" /* 7298 */;
import ClipboardUtils from "ClipboardUtils" /* 7299 */;
import useGuildOnboardingAvailableDefault from "useGuildOnboardingAvailable" /* 7445 */;
import ChannelListState from "ChannelListState" /* 7638 */;
import GuildAntiRaidTypes from "GuildAntiRaidTypes" /* 8133 */;
import ReportModals from "ReportModals" /* 8764 */;
import useOpenProfileSettingsDefault from "useOpenProfileSettings" /* 9350 */;
...
import ChannelCollapseActionCreatorsDefault from "ChannelCollapseActionCreators" /* 11115 */;
import OptInOnboardingUtils from "OptInOnboardingUtils" /* 11697 */;
import GuildRoleConnectionsModalActionCreators from "GuildRoleConnectionsModalActionCreators" /* 11711 */;
import useIsServerThemeAvailableForGuildDefault from "useIsServerThemeAvailableForGuild" /* 14017 */;
import markGuildsAsReadDefault from "markGuildsAsRead" /* 14066 */;
import GuildAntiRaidModalActionCreators from "GuildAntiRaidModalActionCreators" /* 14069 */;
...
const DiscordTagDefault = tmp2(9943);
require = fn;
function HideMutedChannelsOption(guild) {
  ...
  return closure_21(guild(7309).ActionSheetSwitchRow, obj2);
}
function BrowseChannelsOption(guild) {
  guild = guild.guild;
  const tmp2 = useGuildOnboardingAvailableDefault(guild);
  const result = guild(4458).useIsDismissibleContentDismissed_UNSAFE(guild(1944).DismissibleContent.CHANNEL_BROWSER_NEW_BADGE_NUX);
  let obj = guild(4458);
  ...
  const callback = noop.useCallback(() => {
    ...
    ModalActionCreatorsDefault.pushLazy(asyncRequireImpl(11691, dependencyMap.paths), { guildId: guild.id }, closure_18);
  }, items2);
  if (!features.has(constants2.COMMUNITY)) {
    ...
    tmp8Result2 = tmp8(tmp3(12434).NewBadge, {});
  }
}
...
function GuildThemePreferenceOption(guild) {
  ...
  if (tmp2) {
    ...
    tmp6 = closure_21(tmp3(7309).ActionSheetSwitchRow, obj2);
  }
  return tmp6;
}
function ServerTagOption(guild) {
  ...
  const tmp = closure_23();
  [][0] = guild.id;
  const result = guild(9210).canViewMobileServerTag(guild.id);
  if (result) {
    ...
    const guildTagBadgeUrl = tmp2(8294).getGuildTagBadgeUrl(guild.id, badge, GuildTagBadgeSize.SIZE_16);
    ...
    obj4.children = closure_21(tmp2(9327).BaseGuildTagChiplet, obj5);
    ...
    items[1] = closure_21(tmp2(4634).Text, obj6);
    obj2.children = items;
    obj3.label = closure_22(View, obj2);
    obj3.onPress = tmp5;
    return closure_21(tmp2(7309).ActionSheetRow, obj3);
  } else {
    return null;
  }
  let obj = guild(9210);
...
let closure_18 = fn(7207).CHANNELS_AND_ROLES_MODAL_KEY;
    ...
    t = {
      ...
      onPress() {
          if (stateFromStores1) {
            ...
            obj2.openLazy(asyncRequireImpl(10282, dependencyMap.paths), "muteSettings" + guild.id, obj3);
            const tmp5 = asyncRequireImpl(10282, dependencyMap.paths);
          }
        }
    };
...
export const GuildActionSheetPrimaryActions = function GuildActionSheetPrimaryActions(guild) {
  ...
  if (obj.useGuildActionSheetPermissions(guild).canManageChannels) {
    ...
    items1.push(closure_21(tmp(7309).ActionSheetRow, obj3));
    ...
    items1.push(closure_21(tmp(7309).ActionSheetRow, obj4));
  }
  if (tmp3) {
    ...
    items1.push(closure_21(tmp(7309).ActionSheetRow, obj5));
  }
  if (stateFromStores) {
...
export const GuildActionSheetDirectoryActions = function GuildActionSheetDirectoryActions(guild) {
  ...
  const messageRequestPrivacyOption = guild(14015).useMessageRequestPrivacyOption({ guild });
  ...
  if (tmp4) {
    ...
    if (0 !== items.length) {
      ...
      tmp5Result = tmp5(tmp9(7309).ActionSheetRow.Group, obj2);
    }
    return tmp5Result;
  } else {
    const features = guild.features;
    ...
    t = items.push(tmp5(tmp9(7309).ActionSheetRow, obj3));
  }
};
export const GuildActionSheetSecondaryActions = function GuildActionSheetSecondaryActions(guild) {
  ...
  const canReportRaid = guild(10237).useCanReportRaid(guild);
  let obj = guild(10237);
  const guildIncidentsState = guild(10236).useGuildIncidentsState(guild.id);
  const items = [];
  ({ shouldShowIncidentActions, isUnderLockdown } = guildIncidentsState);
  let obj2 = guild(10236);
  const optInEnabledForGuild = guild(7645).useOptInEnabledForGuild(guild.id);
  items.push(closure_21(ChangeIdentityOption, { guild, user: currentUser }));
  ...
  if (features.has(constants2.COMMUNITY)) {
    ...
    items.push(tmp8(tmp3(7309).ActionSheetSwitchRow, obj4));
  }
  items.push(closure_21(HideMutedChannelsOption, { guild }));
  items.push(closure_21(RestrictedGuildPrivacyOption, { guild }));
  const obj3 = guild(7645);
  ...
  if (canReportRaid) {
    ...
    items.push(tmp8(tmp3(7309).ActionSheetRow, obj5));
  }
  if (!tmp2) {
    const obj6 = { label: null, onPress: null };
    ...
    items.push(tmp8(tmp3(7309).ActionSheetRow, obj6));
  }
  if (!shouldShowIncidentActions) {
    if (tmp2) {
      ...
      if (0 !== items.length) {
        ...
        tmp8Result = tmp8(tmp3(7309).ActionSheetRow.Group, obj7);
      }
      return tmp8Result;
    } else {
      ...
      t2 = items.push(tmp8(tmp3(7309).ActionSheetRow, obj8));
    }
  } else {
    ...
    const obj9 = {
      ...
      onPress() {
          ...
          ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(11972, dependencyMap.paths), "GuildIncidentActionsActionSheet", { guild, analyticsData: obj2 });
        }
    };
    t = items.push(tmp8(tmp3(7309).ActionSheetRow, obj9));
  }
};
export const GuildDeveloperOptionAction = function GuildDeveloperOptionAction(guild) {
  guild = guild.guild;
  const DeveloperMode = guild(1936).DeveloperMode;
  ...
  if (setting) {
    ...
    items.push(closure_21(tmp(7309).ActionSheetRow, obj));
    if (tmp5) {
      if (null != guild.icon) {
        ...
        items.push(tmp7(tmp(7309).ActionSheetRow, obj2));
      }
      if (null != guild.banner) {
        ...
        items.push(tmp7(tmp(7309).ActionSheetRow, obj3));
      }
    }
    ...
    return closure_21(tmp(7309).ActionSheetRow.Group, obj4);
  } else {
    return null;
  }
};
