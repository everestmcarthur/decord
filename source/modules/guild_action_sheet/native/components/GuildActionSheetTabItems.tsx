...
// Dependencies: [19, 1958, 2013, 2012, 1074, 21, 14067, 4546, 504, 9963, 9960, 5519, 8214, 1115, 9518, 576, 4818, 4605, 5520, 10170, 8065, 7229, 7491, 9206, 2]
// Exports: default

// Module 14079 (GuildActionSheetTabItems)
...
export default function GuildActionSheetTabItems(guild) {
  guild = guild.guild;
  let stateFromStores;
  let canAccessSettings = guild(14067).useGuildActionSheetPermissions(guild).canAccessSettings;
  const total = stateFromStores(4546)(guild.id).total;
  let obj = guild(14067);
  const items = [GuildChannelStore];
  stateFromStores = guild(504).useStateFromStores(items, () => GuildChannelStore.getChannels(guild.id));
  let obj2 = guild(504);
  let shouldRenderInviteResult = guild(9963).shouldRenderInvite(stateFromStores, guild);
  ...
  obj6.icon = closure_12(guild(9518).BoostGemIcon, { color: stateFromStores(576).unsafe_rawColors.GUILD_BOOSTING_PINK });
  ...
  const items2 = [closure_12(guild(8214).IconButton, obj6), , , ];
  if (shouldRenderInviteResult) {
    ...
    obj8.icon = tmp3(10170);
    obj8.onPress = function onPress() {
      ActionSheetActionCreatorsDefault.hideActionSheet();
      closure_2();
    };
    shouldRenderInviteResult = tmp7(tmp(8214).IconButton, obj8);
  }
  ...
  obj9.icon = stateFromStores(8065);
  obj9.onPress = function onPress() {
    ActionSheetActionCreatorsDefault.hideActionSheet();
    NotificationSettingsModalActionCreatorsDefault.open(guild.id);
  };
  items2[2] = closure_12(guild(8214).IconButton, obj9);
  if (canAccessSettings) {
    ...
    obj10.icon = tmp3(7491);
    obj10.onPress = function onPress() {
      ActionSheetActionCreatorsDefault.hideActionSheet();
      GuildSettingsActionCreatorsDefault.open(guild.id);
    };
    canAccessSettings = tmp7(tmp(8214).IconButton, obj10);
  }
  items2[3] = canAccessSettings;
  obj4.children = items2;
  return tmp6(guild(5519).ButtonGroup, obj4);
};
