...
// Dependencies: [19, 1957, 2012, 2011, 1074, 21, 14014, 4515, 504, 9914, 9911, 5484, 8172, 1114, 9466, 576, 4786, 4573, 5485, 10121, 8023, 7192, 7454, 9153, 2]
// Exports: default

// Module 14026 (GuildActionSheetTabItems)
...
export default function GuildActionSheetTabItems(guild) {
  ...
  const total = stateFromStores(4515)(guild.id).total;
  let obj = guild(14014);
  const items = [GuildChannelStore];
  stateFromStores = guild(504).useStateFromStores(items, () => GuildChannelStore.getChannels(guild.id));
  let obj2 = guild(504);
  let shouldRenderInviteResult = guild(9914).shouldRenderInvite(stateFromStores, guild);
  ...
  obj6.icon = closure_12(guild(9466).BoostGemIcon, { color: stateFromStores(576).unsafe_rawColors.GUILD_BOOSTING_PINK });
  ...
  const items2 = [closure_12(guild(8172).IconButton, obj6), , , ];
  if (shouldRenderInviteResult) {
    ...
    obj8.icon = tmp3(10121);
    obj8.onPress = function onPress() {
      ActionSheetActionCreatorsDefault.hideActionSheet();
      closure_2();
    };
    shouldRenderInviteResult = tmp7(tmp(8172).IconButton, obj8);
  }
  ...
  obj9.icon = stateFromStores(8023);
  obj9.onPress = function onPress() {
    ActionSheetActionCreatorsDefault.hideActionSheet();
    NotificationSettingsModalActionCreatorsDefault.open(guild.id);
  };
  items2[2] = closure_12(guild(8172).IconButton, obj9);
  if (canAccessSettings) {
    ...
    obj10.icon = tmp3(7454);
    obj10.onPress = function onPress() {
      ActionSheetActionCreatorsDefault.hideActionSheet();
      GuildSettingsActionCreatorsDefault.open(guild.id);
    };
    canAccessSettings = tmp7(tmp(8172).IconButton, obj10);
  }
  items2[3] = canAccessSettings;
  obj4.children = items2;
  return tmp6(guild(5484).ButtonGroup, obj4);
};
