...
export default function openGuildActionSheet(id) {
  ...
  if (!obj.isFavoritesGuildId(id.id)) {
    ...
    let tmpResult = tmp(10299);
    if (tmpResult.shouldNSFWGateGuild(id.id)) {
      let tmp3Result = tmp3(4527);
      obj = { guild: null };
      obj[0] = id;
      tmp3Result.openLazy(tmp(1896)(13911, tmp2.paths), "NsfwGateGuildSettingsActionSheet", obj);
    } else {
      ...
      if (hasItem) {
        ...
        obj1[0] = id;
        obj1[1] = flag;
        openLazy(tmpResult(13969, tmp2.paths), "GuildActionSheet:" + id.id, obj1);
        const tmpResult1Result = tmpResult(13969, tmp2.paths);
      } else {
        const _HermesInternal = HermesInternal;
        const obj2 = { guild: null, expanded: null };
        obj2[0] = id;
        obj2[1] = flag;
        openLazy(tmpResult(13975, tmp2.paths), "GuildActionSheet:" + id.id, obj2);
        const tmpResult1Result1 = tmpResult(13975, tmp2.paths);
      }
    }
  }
};
