...
// Dependencies: [19, 17, 1371, 1074, 21, 4606, 11279, 13483, 504, 11253, 1242, 4573, 11252, 1896, 11252, 1114, 13485, 4602, 11278, 13450, 5638, 2]
// Exports: TieredTenureBadgePerkCard

// Module 13482 (TieredTenureBadgePerkCard)
...
export const TieredTenureBadgePerkCard = function TieredTenureBadgePerkCard() {
  tieredTenureBadgeData = tieredTenureBadgeData(11279).useTieredTenureBadgeData();
  let obj = tieredTenureBadgeData(11279);
  const premiumSince = tieredTenureBadgeData(11279).usePremiumSince();
  let obj2 = tieredTenureBadgeData(11279);
  const timeUntilNextBadge = tieredTenureBadgeData(13483).useTimeUntilNextBadge();
  const tmp6 = closure_9();
  let obj3 = tieredTenureBadgeData(13483);
  ...
  const mobileTenureBadgeImages = tieredTenureBadgeData(11253).useMobileTenureBadgeImages(id);
  ...
  if (null == tieredTenureBadgeData) {
    ...
    if (tieredTenureBadgeData.status === tmp(11279).TieredTenureBadgeStatus.UPCOMING) {
      const intl2 = tmp(1114).intl;
      stringResult = intl2.string(tmp(1114).t.O9TBwQ);
    } else if (tieredTenureBadgeData.status === tmp(11279).TieredTenureBadgeStatus.WITHHELD) {
    ...
    if (null != premiumSince) {
      ...
      if (tmp(11279).TieredTenureBadgeStatus.EARNED !== status) {
        if (tmp(11279).TieredTenureBadgeStatus.WITHHELD !== status) {
          formatResult = null;
          tmp14 = large;
          if (tmp(11279).TieredTenureBadgeStatus.UPCOMING === status) {
            formatResult = null;
            tmp14 = large;
    ...
    const items2 = [closure_7(tmp(4602).Text, obj10), ];
    const obj11 = { variant: "heading-md/medium", color: "text-muted", style: tmp6.tenureRequirements, children: tmp(11278).getTenureBadgeRequirementString(tieredTenureBadgeData.id, tieredTenureBadgeData.tenureReqNumMonths) };
    items2[1] = closure_7(tmp(4602).Text, obj11);
    ...
    if (tmp30Result) {
      ...
      tmp30Result = tmp30(tmp(4602).Text, obj12);
    }
    const obj13 = { children: null };
    items3[1] = tmp30Result;
    obj13.children = items3;
    const tmpResult = tmp(11278);
    ...
    const tmp34 = stateFromStores(13450);
    let upcomingBadge = tieredTenureBadgeData.status === tmp(11279).TieredTenureBadgeStatus.UPCOMING;
    if (!upcomingBadge) {
      upcomingBadge = tieredTenureBadgeData.status === tmp(11279).TieredTenureBadgeStatus.WITHHELD;
    }
    if (upcomingBadge) {
