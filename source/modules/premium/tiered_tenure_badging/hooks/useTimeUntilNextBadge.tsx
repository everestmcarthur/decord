...
// Dependencies: [19, 4198, 13486, 11281, 2]
// Exports: computeDaysUntilNextBadgeDate, useTimeUntilNextBadge

// Module 13485 (useTimeUntilNextBadge)
...
export const useTimeUntilNextBadge = function useTimeUntilNextBadge() {
  nextTenureBadge = nextTenureBadge(13486).useNextTenureBadge();
  let obj = nextTenureBadge(13486);
  const premiumSince = nextTenureBadge(11281).usePremiumSince();
  const items = [nextTenureBadge, premiumSince];
  return useMemo(() => {
    if (null != nextTenureBadge) {
      if (null != premiumSince) {
        const tenureReqNumMonths = tmp.tenureReqNumMonths;
        const obj = _modDef4198(tmp2);
        const addResult = _modDef4198(tmp2).add(tenureReqNumMonths, "months");
        const obj2 = { days: null, months: null };
        const _Math = Math;
        const addResult1 = _modDef4198(tmp2).add(tenureReqNumMonths, "months").add(1, "day");
        const obj5 = _modDef4198(tmp2);
        const addResult2 = _modDef4198(tmp2).add(tenureReqNumMonths, "months");
        obj2.days = Math.max(0, _modDef4198(tmp2).add(tenureReqNumMonths, "months").add(1, "day").diff(_modDef4198(), "days"));
        const _Math2 = Math;
        const _Math3 = Math;
        obj2.months = Math.max(0, Math.round(addResult1.diff(_modDef4198(), "months", true)));
        return obj2;
      }
    }
    return null;
