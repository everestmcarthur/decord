...
// Dependencies: [10756, 1373, 1074, 10763, 13481, 7496, 7314, 13482, 7456, 2]
// Exports: getButtonActionHandler

// Module 13480 (PremiumMarketingButtonActions)
...
import navigateToSocialLayerStorefrontDefault from "navigateToSocialLayerStorefront" /* 13481 */;
...
export const getButtonActionHandler = function getButtonActionHandler(arg0) {
  ...
  if (cta_button.ButtonAction.OPEN_SOCIAL_LAYER_STOREFRONT === buttonAction) {
  ...
  } else if (tmp(10763).ButtonAction.OPEN_TIER_1_PAYMENT_MODAL === buttonAction) {
    return () => {
    ...
    if (tmp(10763).ButtonAction.OPEN_TIER_2_PAYMENT_MODAL !== buttonAction) {
      if (tmp(10763).ButtonAction.OPEN_TIER_2_PAYMENT_MODAL_CUSTOM_CONFIRMATION_FOOTER !== buttonAction) {
        if (tmp(10763).ButtonAction.OPEN_PLAN_SELECTION_MODAL === buttonAction) {
          return () => {
          ...
          const OPEN_MARKETING_PAGE = tmp(10763).ButtonAction.OPEN_MARKETING_PAGE;
    ...
    return () => {
      ...
      let obj = {
        ...
        onPaymentDismiss(arg0) {
          ...
          if (isSuccess) {
            const result = tmp3(13482).showMarketingMomentRewardScreen(length[0]);
            const tmp3Result = tmp3(13482);
          }
        }
      };
