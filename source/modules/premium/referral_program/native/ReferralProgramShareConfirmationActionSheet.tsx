...
// Dependencies: [17, 1074, 21, 4606, 576, 4451, 7526, 1178, 4602, 1114, 5026, 5122, 4573, 4619, 2024, 7223, 7222, 5024, 5743, 2]
// Exports: default

// Module 13495 (ReferralProgramShareConfirmationActionSheet)
...
function SharedUser(user) {
  ...
  const tmp6 = user.trialCreationResult === user(7526).CreateReferralStatus.FAIL;
  ...
  if (tmp6) {
    ...
    const items2 = [tmp9(tmp5(4602).Text, obj5), ];
    ...
    items2[1] = tmp9(tmp5(4602).Text, obj6);
    ...
    tmp9Result = tmp9(tmp5(4602).Text, obj8);
  }
  items1[1] = tmp9Result;
  ...
  obj9.icon = closure_5(user(5122).ChatIcon, { size: "xs", color: nativeDefault.colors.CONTROL_SECONDARY_TEXT_DEFAULT });
  obj9.onPress = function onPress() {
    ActionSheetActionCreatorsDefault.hideActionSheet();
    ChannelActionCreatorsDefault.openPrivateChannel({ recipientIds: user.id });
  };
  items1[2] = closure_5(user(5026).Button, obj9);
...
export default function ReferralProgramShareConfirmationActionSheet(trialCreationResult) {
  ...
  const obj3 = { startExpanded: true, contentStyles: tmp.content, header: closure_5(tmp5(7222).BottomSheetTitleHeader, { title: null }), children: null };
  const obj4 = { children: null };
  const formatResult = intl3.format(tmp5(1114).t.AwGSWl, obj);
  const items = [closure_5(View, { style: tmp.headerAsset, children: closure_5(tmp5(5743).FistBumpSpotIllustration, {}) }), closure_5(tmp5(4602).Text, { variant: "heading-lg/bold", color: "mobile-text-heading-primary", style: tmp.header, children: stringResult }), closure_5(tmp5(4602).Text, { variant: "text-md/medium", color: "text-default", style: tmp.subheader, children: formatResult }), ];
  const obj8 = { style: tmp.recipientContainer, children: null };
  const obj5 = { style: tmp.headerAsset, children: closure_5(tmp5(5743).FistBumpSpotIllustration, {}) };
  ...
  obj4.children = items;
  obj3.children = closure_7(tmp5(5024).Stack, obj4);
  return closure_5(tmp5(7223).BottomSheet, obj3);
};
