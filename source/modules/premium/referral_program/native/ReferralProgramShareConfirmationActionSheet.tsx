...
// Dependencies: [17, 1074, 21, 4640, 576, 4484, 7567, 1177, 4636, 1115, 5062, 5158, 4607, 4653, 2025, 7264, 7263, 5060, 5780, 2]
// Exports: default

// Module 13553 (ReferralProgramShareConfirmationActionSheet)
...
function SharedUser(user) {
  ...
  const tmp6 = user.trialCreationResult === user(7567).CreateReferralStatus.FAIL;
  ...
  if (tmp6) {
    ...
    const items2 = [tmp9(tmp5(4636).Text, obj5), ];
    ...
    items2[1] = tmp9(tmp5(4636).Text, obj6);
    ...
    tmp9Result = tmp9(tmp5(4636).Text, obj8);
  }
  items1[1] = tmp9Result;
  ...
  obj9.icon = closure_5(user(5158).ChatIcon, { size: "xs", color: nativeDefault.colors.CONTROL_SECONDARY_TEXT_DEFAULT });
  obj9.onPress = function onPress() {
    ActionSheetActionCreatorsDefault.hideActionSheet();
    ChannelActionCreatorsDefault.openPrivateChannel({ recipientIds: user.id });
  };
  items1[2] = closure_5(user(5062).Button, obj9);
...
export default function ReferralProgramShareConfirmationActionSheet(trialCreationResult) {
  ...
  const obj3 = { startExpanded: true, contentStyles: tmp.content, header: closure_5(tmp5(7263).BottomSheetTitleHeader, { title: null }), children: null };
  const obj4 = { children: null };
  const formatResult = intl3.format(tmp5(1115).t.AwGSWl, obj);
  const items = [closure_5(View, { style: tmp.headerAsset, children: closure_5(tmp5(5780).FistBumpSpotIllustration, {}) }), closure_5(tmp5(4636).Text, { variant: "heading-lg/bold", color: "mobile-text-heading-primary", style: tmp.header, children: stringResult }), closure_5(tmp5(4636).Text, { variant: "text-md/medium", color: "text-default", style: tmp.subheader, children: formatResult }), ];
  const obj8 = { style: tmp.recipientContainer, children: null };
  const obj5 = { style: tmp.headerAsset, children: closure_5(tmp5(5780).FistBumpSpotIllustration, {}) };
  ...
  obj4.children = items;
  obj3.children = closure_7(tmp5(5060).Stack, obj4);
  return closure_5(tmp5(7264).BottomSheet, obj3);
};
