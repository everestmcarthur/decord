...
// Dependencies: [19, 17, 7525, 13487, 1074, 21, 4606, 576, 1178, 4602, 13488, 13489, 1114, 2024, 6401, 5989, 7526, 504, 7235, 7255, 13490, 1242, 4573, 13491, 1896, 13498, 13501, 5026, 2]
...
export const ReferralProgramPerkCard = function ReferralProgramPerkCard() {
  ...
  const item = stateFromStores.forEach((item) => {
    ...
    } else if (item === tmp(7526).ReferralOfferStatus.CONVERTED) {
      closure_1 = closure_1 + 1;
      closure_2 = closure_2 + 1;
    } else if (item === tmp(7526).ReferralOfferStatus.REDEEMED) {
      closure_2 = closure_2 + 1;
    }
  });
  ...
  if (!useAltReferralCardArt) {
    ...
    const items4 = [tmp17(tmp2(4602).Text, obj10), , ];
    ...
    if (null != tmp12) {
      ...
      if (closure_129_2 === tmp2(13488).MAX_REFERRALS_SENT) {
      ...
      items4[1] = tmp17(tmp2(4602).Text, obj12);
      if (isEligibleForIncentive) {
        const obj24 = { nRewardsGranted: tmp10, referralRewardType: tmp12 };
        isEligibleForIncentive = tmp17(tmp5(13501), obj24);
      }
      ...
      obj25.children = tmp17(tmp2(5026).Button, obj26);
    ...
    let tmp5Result2 = tmp5(6401);
  } else if (tmp12 === tmp2(13489).ReferralRewardType.DISCOUNT) {
    tmp5Result2 = tmp5(5989);
  }
};
