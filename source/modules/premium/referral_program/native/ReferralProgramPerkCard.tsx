...
// Dependencies: [19, 17, 7466, 13471, 1074, 21, 4574, 576, 1178, 4570, 13472, 13473, 1114, 2024, 6344, 5932, 7467, 504, 7176, 7196, 13474, 1242, 4541, 13475, 1896, 13482, 13485, 4989, 2]
// Exports: ReferralProgramPerkCard

// Module 13470 (ReferralProgramPerkCard)
...
export const ReferralProgramPerkCard = function ReferralProgramPerkCard() {
  ...
  const callback = noop.useCallback(() => {
    ...
    ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(13475, dependencyMap.paths), "referral-program-share-action-sheet");
  }, items2);
  const item = stateFromStores.forEach((item) => {
    ...
    } else if (item === tmp(7467).ReferralOfferStatus.CONVERTED) {
      closure_1 = closure_1 + 1;
      closure_2 = closure_2 + 1;
    } else if (item === tmp(7467).ReferralOfferStatus.REDEEMED) {
      closure_2 = closure_2 + 1;
    }
  });
  ...
  if (!useAltReferralCardArt) {
    ...
    const items4 = [tmp17(tmp2(4570).Text, obj10), , ];
    ...
    if (null != tmp12) {
      ...
      items4[1] = tmp17(tmp2(4570).Text, obj12);
      if (isEligibleForIncentive) {
        const obj22 = { nRewardsGranted: closure_129_0, referralRewardType: tmp12 };
        isEligibleForIncentive = tmp17(tmp5(13485), obj22);
      }
      ...
      obj23.children = tmp17(tmp2(4989).Button, obj24);
    ...
    let tmp5Result2 = tmp5(6344);
  } else if (tmp12 === tmp2(13473).ReferralRewardType.DISCOUNT) {
    tmp5Result2 = tmp5(5932);
  }
};
