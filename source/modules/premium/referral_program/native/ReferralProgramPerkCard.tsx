...
// Dependencies: [19, 17, 7526, 13489, 1074, 21, 4607, 576, 1178, 4603, 13490, 13491, 1114, 2024, 6402, 5990, 7527, 504, 7236, 7256, 13492, 1242, 4574, 13493, 1896, 13500, 13503, 5027, 2]
// Exports: ReferralProgramPerkCard

// Module 13488 (ReferralProgramPerkCard)
...
import ReferralTrialActionCreators from "ReferralTrialActionCreators" /* 7527 */;
import useReferralProgramBannerDetails from "useReferralProgramBannerDetails" /* 13490 */;
...
export const ReferralProgramPerkCard = function ReferralProgramPerkCard() {
  ...
  const callback = noop.useCallback(() => {
    ...
    ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(13493, dependencyMap.paths), "referral-program-share-action-sheet");
  }, items2);
  const item = stateFromStores.forEach((item) => {
    ...
    } else if (item === tmp(7527).ReferralOfferStatus.CONVERTED) {
      closure_1 = closure_1 + 1;
      closure_2 = closure_2 + 1;
    } else if (item === tmp(7527).ReferralOfferStatus.REDEEMED) {
      closure_2 = closure_2 + 1;
    }
  });
  ...
  if (!useAltReferralCardArt) {
    ...
    if (tmp12 === tmp2(13491).ReferralRewardType.ORBS) {
      const intl4 = tmp2(1114).intl;
      let stringResult = intl4.string(tmp2(1114).t.tAlkl4);
    } else if (tmp12 === tmp2(13491).ReferralRewardType.DISCOUNT) {
    ...
    const items4 = [tmp17(tmp2(4603).Text, obj10), , ];
    ...
    if (null != tmp12) {
      if (tmp10 !== tmp2(13490).MAX_REFERRALS_SENT) {
        if (size === tmp2(13490).MAX_REFERRALS_SENT) {
          if (tmp12 === tmp2(13491).ReferralRewardType.ORBS) {
      ...
      if (closure_129_2 === tmp2(13490).MAX_REFERRALS_SENT) {
      ...
      items4[1] = tmp17(tmp2(4603).Text, obj12);
      if (isEligibleForIncentive) {
        const obj24 = { nRewardsGranted: tmp10, referralRewardType: tmp12 };
        isEligibleForIncentive = tmp17(tmp5(13503), obj24);
      }
      ...
      let tmp38 = !tmp24;
      if (tmp24) {
        tmp38 = size === tmp2(13490).MAX_REFERRALS_SENT;
      }
      ...
      obj25.children = tmp17(tmp2(5027).Button, obj26);
      items3[4] = tmp17(tmp14, obj25);
      obj5.children = items3;
      return tmp13(tmp14, obj5);
    }
    const tmp5Result = tmp5(2024);
  } else if (tmp12 === tmp2(13491).ReferralRewardType.ORBS) {
    let tmp5Result2 = tmp5(6402);
  } else if (tmp12 === tmp2(13491).ReferralRewardType.DISCOUNT) {
    tmp5Result2 = tmp5(5990);
  }
};
