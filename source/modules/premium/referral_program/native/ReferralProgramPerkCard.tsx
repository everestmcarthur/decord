...
// Dependencies: [19, 17, 7559, 13526, 1074, 21, 4639, 576, 1177, 4635, 13527, 13528, 1115, 2025, 6435, 6023, 7560, 504, 7269, 7289, 13529, 1241, 4606, 13530, 1897, 13537, 13540, 5059, 2]
// Exports: ReferralProgramPerkCard

// Module 13525 (ReferralProgramPerkCard)
...
import useReferralProgramBannerDetails from "useReferralProgramBannerDetails" /* 13527 */;
...
export const ReferralProgramPerkCard = function ReferralProgramPerkCard() {
  ...
  const callback = noop.useCallback(() => {
    ...
    ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(13530, dependencyMap.paths), "referral-program-share-action-sheet");
  }, items2);
  const item = stateFromStores.forEach((item) => {
    ...
    } else if (item === tmp(7560).ReferralOfferStatus.CONVERTED) {
      closure_1 = closure_1 + 1;
      closure_2 = closure_2 + 1;
    } else if (item === tmp(7560).ReferralOfferStatus.REDEEMED) {
      closure_2 = closure_2 + 1;
    }
  });
  ...
  if (isEligibleForIncentive) {
    ...
    obj6.text = intl.string(tmp2(1115).t.oW0eUd);
    obj6.color = tmp2(1177).BadgeColors.BRAND;
    obj6.style = tmp.betaBadge;
    tmp15 = closure_10(tmp2(1177).TextBadge, obj6);
  }
  const items3 = [tmp15, , , , ];
  ...
  if (!useAltReferralCardArt) {
    ...
    if (tmp12 === tmp2(13528).ReferralRewardType.ORBS) {
      const intl4 = tmp2(1115).intl;
      let stringResult = intl4.string(tmp2(1115).t.tAlkl4);
    } else if (tmp12 === tmp2(13528).ReferralRewardType.DISCOUNT) {
      const intl3 = tmp2(1115).intl;
      const obj11 = { discountPercent };
      stringResult = intl3.formatToPlainString(tmp2(1115).t["/JJ9I5"], obj11);
    } else {
      const intl2 = tmp2(1115).intl;
      stringResult = intl2.string(tmp2(1115).t.USo4s7);
    }
    obj10.children = stringResult;
    const items4 = [tmp17(tmp2(4635).Text, obj10), , ];
    let str2 = "text-md/medium";
    if (isEligibleForIncentive) {
      str2 = "text-sm/medium";
    }
    const obj12 = { variant: str2, color: "text-subtle", style: tmp.bodyText, children: null };
    const articleURL = tmp5(2025).getArticleURL(constants2.REFERRAL_PROGRAM);
    if (null != tmp12) {
      if (tmp10 !== tmp2(13527).MAX_REFERRALS_SENT) {
        if (size === tmp2(13527).MAX_REFERRALS_SENT) {
          if (tmp12 === tmp2(13528).ReferralRewardType.ORBS) {
            const intl13 = tmp2(1115).intl;
            const obj13 = { helpdeskArticle: articleURL };
            let formatResult = intl13.format(tmp2(1115).t["1aV1j9"], obj13);
          } else {
            const intl12 = tmp2(1115).intl;
            const obj14 = { helpdeskArticle: articleURL };
            formatResult = intl12.format(tmp2(1115).t.QNrPuS, obj14);
          }
        } else if (tmp24) {
          if (tmp12 === tmp2(13528).ReferralRewardType.ORBS) {
            const intl11 = tmp2(1115).intl;
            const obj15 = { numOrbs, helpdeskArticle: articleURL };
            let formatResult1 = intl11.format(tmp2(1115).t.cfE0uG, obj15);
          } else {
            const intl10 = tmp2(1115).intl;
            const obj16 = { helpdeskArticle: articleURL };
            formatResult1 = intl10.format(tmp2(1115).t["+fcvlI"], obj16);
          }
        } else {
          const intl9 = tmp2(1115).intl;
          const obj17 = { helpdeskArticle: articleURL };
          intl9.format(tmp2(1115).t["a0+Jwv"], obj17);
        }
      }
      if (tmp12 === tmp2(13528).ReferralRewardType.ORBS) {
        const intl15 = tmp2(1115).intl;
        const obj18 = { helpdeskArticle: articleURL };
        let formatResult3 = intl15.format(tmp2(1115).t.OluhLp, obj18);
      } else {
        const intl14 = tmp2(1115).intl;
        const obj19 = { helpdeskArticle: articleURL };
        formatResult3 = intl14.format(tmp2(1115).t["8BYihN"], obj19);
      }
    } else if (tmp24) {
      if (size !== tmp2(13527).MAX_REFERRALS_SENT) {
        const intl6 = tmp2(1115).intl;
        const obj20 = { helpdeskArticle: articleURL };
        intl6.format(tmp2(1115).t["omMr+V"], obj20);
      }
      if (closure_129_2 === tmp2(13527).MAX_REFERRALS_SENT) {
        const intl8 = tmp2(1115).intl;
        const obj21 = { helpdeskArticle: articleURL };
        let formatResult5 = intl8.format(tmp2(1115).t["1aEjsH"], obj21);
      } else {
        const intl7 = tmp2(1115).intl;
        const obj22 = { helpdeskArticle: articleURL };
        formatResult5 = intl7.format(tmp2(1115).t["+u3AOO"], obj22);
      }
    } else {
      const intl5 = tmp2(1115).intl;
      const obj23 = { helpdeskArticle: articleURL };
      obj12.children = intl5.format(tmp2(1115).t["zWhX/Q"], obj23);
      items4[1] = tmp17(tmp2(4635).Text, obj12);
      if (isEligibleForIncentive) {
        const obj24 = { nRewardsGranted: tmp10, referralRewardType: tmp12 };
        isEligibleForIncentive = tmp17(tmp5(13540), obj24);
      }
      ...
      let tmp38 = !tmp24;
      if (tmp24) {
        tmp38 = size === tmp2(13527).MAX_REFERRALS_SENT;
      }
      const obj26 = { variant: "primary", size: "sm", disabled: tmp38, text: null, onPress: null };
      const intl16 = tmp2(1115).intl;
      obj26.text = intl16.string(tmp2(1115).t.Lm2nFc);
      obj26.onPress = callback;
      obj25.children = tmp17(tmp2(5059).Button, obj26);
      items3[4] = tmp17(tmp14, obj25);
      obj5.children = items3;
      return tmp13(tmp14, obj5);
    }
    const tmp5Result = tmp5(2025);
  } else if (tmp12 === tmp2(13528).ReferralRewardType.ORBS) {
    let tmp5Result2 = tmp5(6435);
  } else if (tmp12 === tmp2(13528).ReferralRewardType.DISCOUNT) {
    tmp5Result2 = tmp5(6023);
  }
};
