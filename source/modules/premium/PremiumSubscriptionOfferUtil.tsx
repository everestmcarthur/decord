...
// Dependencies: [32, 19, 4303, 1374, 7561, 8196, 8197, 504, 4231, 8198, 8199, 1895, 8202, 2]
...
function useHasDiscountApplied() {
  ...
  if (tmp4) {
    const _Date = Date;
    tmp4 = _modDef4231(Date.now()) <= _modDef4231(prop);
    const tmp6Result = _modDef4231(Date.now());
  }
  return tmp4;
}
const size = fn(2);
...
export const useIsInPremiumOfferExperience = function useIsInPremiumOfferExperience() {
  ...
  if (typeof useHasDiscountApplied === "function") {
    ...
    if (tmp12) {
      const _Date = Date;
      tmp12 = _modDef4231(Date.now()) <= _modDef4231(prop);
      const tmp14Result = _modDef4231(Date.now());
    }
...
export const useShouldFetchChurnOffer = function useShouldFetchChurnOffer() {
  ...
  if (typeof useHasDiscountApplied === "function") {
    ...
    if (tmp8) {
      const _Date = Date;
      tmp8 = _modDef4231(Date.now()) <= _modDef4231(prop);
      const tmp10Result = _modDef4231(Date.now());
    }
