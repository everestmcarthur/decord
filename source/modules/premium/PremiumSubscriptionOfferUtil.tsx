...
// Dependencies: [32, 19, 4301, 1374, 7557, 8176, 8177, 504, 4229, 8178, 8179, 1895, 8182, 2]
...
function useHasDiscountApplied() {
  ...
  if (tmp4) {
    const _Date = Date;
    tmp4 = _modDef4229(Date.now()) <= _modDef4229(prop);
    const tmp6Result = _modDef4229(Date.now());
  }
  return tmp4;
}
const size = fn(2);
...
export const useIsInPremiumOfferExperience = function useIsInPremiumOfferExperience() {
  ...
  if (typeof useHasDiscountApplied === "function") {
    ...
    if (tmp12) {
      const _Date = Date;
      tmp12 = _modDef4229(Date.now()) <= _modDef4229(prop);
      const tmp14Result = _modDef4229(Date.now());
    }
...
export const useShouldFetchChurnOffer = function useShouldFetchChurnOffer() {
  ...
  if (typeof useHasDiscountApplied === "function") {
    ...
    if (tmp8) {
      const _Date = Date;
      tmp8 = _modDef4229(Date.now()) <= _modDef4229(prop);
      const tmp10Result = _modDef4229(Date.now());
    }
