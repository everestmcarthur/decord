...
function useHasDiscountApplied() {
  ...
  if (tmp4) {
    const _Date = Date;
    tmp4 = _modDef4198(Date.now()) <= _modDef4198(prop);
    const tmp6Result = _modDef4198(Date.now());
  }
  return tmp4;
}
const size = fn(2);
...
export const useIsInPremiumOfferExperience = function useIsInPremiumOfferExperience() {
  ...
  if (typeof useHasDiscountApplied === "function") {
    ...
    if (tmp12) {
      const _Date = Date;
      tmp12 = _modDef4198(Date.now()) <= _modDef4198(prop);
      const tmp14Result = _modDef4198(Date.now());
    }
...
export const useShouldFetchChurnOffer = function useShouldFetchChurnOffer() {
  ...
  if (typeof useHasDiscountApplied === "function") {
    ...
    if (tmp8) {
      const _Date = Date;
      tmp8 = _modDef4198(Date.now()) <= _modDef4198(prop);
      const tmp10Result = _modDef4198(Date.now());
    }
