...
// Dependencies: [19, 17, 10207, 21, 12588, 6681, 576, 1178, 4606, 16243, 16082, 7230, 12584, 12603, 12472, 12572, 7255, 7229, 1114, 2428, 16289, 12384, 2]
// Exports: default
...
function GuildPowerupsChannelRowIndicator(indicator) {
  ...
  if (null == indicator) {
    ...
    if (GuildPowerupsNotification.GuildPowerupNotificationIndicatorType.WARNING === type) {
      ...
      return hasOwnProperty(tmp4(6681).CircleErrorIcon, obj2);
    } else if (tmp4(12588).GuildPowerupNotificationIndicatorType.UNREAD === type) {
...
export default function GuildPowerupsChannelRow(guildId) {
  ...
  const tmp5 = dismissNewBadgeIfShown(12584)(guildId);
  let indicator;
  let obj = noop;
  const tmp = closure_8();
  if (tmp5 != null) {
  ...
  const tmp6Result = dismissNewBadgeIfShown(12603)(guildId, tmp8);
  dismissNewBadgeIfShown = tmp6Result.dismissNewBadgeIfShown;
  let showUnread;
  if (tmp5 != null) {
    showUnread = tmp5.showUnread;
  }
  const ChannelModes = guildId(12472).ChannelModes;
  if (true === showUnread) {
  ...
  const tmp18 = View;
  const tmp6 = dismissNewBadgeIfShown(12603);
  ...
  obj5.mode = DEFAULT;
  obj4.name = closure_5(tmp14(12472).BaseChannelName, obj5);
  const tmp3Result = dismissNewBadgeIfShown(12472);
  obj4.icon = closure_5(tmp14(12472).BaseChannelIcon, { mode: DEFAULT, IconComponent: tmp14(16289).BoostTier2Icon });
  if (tmp6Result.showNewBadgeOnRow) {
    let tmp16Result = tmp16(tmp14(12384).NewBadge, {});
  } else {
    let indicator1;
    if (tmp5 != null) {
  ...
  return closure_5(tmp14(7229).LayerScope, obj8);
};
