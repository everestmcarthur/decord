...
// Dependencies: [19, 17, 10267, 21, 12647, 6721, 576, 1177, 4640, 16332, 16171, 7271, 12643, 12662, 12531, 12631, 7296, 7270, 1115, 2429, 16378, 12443, 2]
// Exports: default

// Module 16377 (GuildPowerupsChannelRow)
...
import SidebarCoachmarkOverlay from "SidebarCoachmarkOverlay" /* 16171 */;
import useGuildPowerupsCoachmarkDefault from "useGuildPowerupsCoachmark" /* 16332 */;
...
function GuildPowerupsChannelRowIndicator(indicator) {
  ...
  if (null == indicator) {
    ...
    if (GuildPowerupsNotification.GuildPowerupNotificationIndicatorType.WARNING === type) {
      ...
      return hasOwnProperty(tmp4(6721).CircleErrorIcon, obj2);
    } else if (tmp4(12647).GuildPowerupNotificationIndicatorType.UNREAD === type) {
...
export default function GuildPowerupsChannelRow(guildId) {
  ...
  const tmp5 = dismissNewBadgeIfShown(12643)(guildId);
  let indicator;
  let obj = noop;
  const tmp = closure_8();
  if (tmp5 != null) {
  ...
  const tmp6Result = dismissNewBadgeIfShown(12662)(guildId, tmp8);
  dismissNewBadgeIfShown = tmp6Result.dismissNewBadgeIfShown;
  let showUnread;
  if (tmp5 != null) {
    showUnread = tmp5.showUnread;
  }
  const ChannelModes = guildId(12531).ChannelModes;
  if (true === showUnread) {
  ...
  const tmp18 = View;
  const tmp6 = dismissNewBadgeIfShown(12662);
  ...
  obj5.mode = DEFAULT;
  obj4.name = closure_5(tmp14(12531).BaseChannelName, obj5);
  const tmp3Result = dismissNewBadgeIfShown(12531);
  obj4.icon = closure_5(tmp14(12531).BaseChannelIcon, { mode: DEFAULT, IconComponent: tmp14(16378).BoostTier2Icon });
  if (tmp6Result.showNewBadgeOnRow) {
    let tmp16Result = tmp16(tmp14(12443).NewBadge, {});
  } else {
    let indicator1;
    if (tmp5 != null) {
  ...
  return closure_5(tmp14(7270).LayerScope, obj8);
};
