...
// Dependencies: [19, 17, 10361, 21, 12738, 6803, 576, 1177, 4722, 16429, 16268, 7353, 12734, 12753, 12622, 12722, 7378, 7352, 1115, 2512, 16475, 12534, 2]
// Exports: default

// Module 16474 (GuildPowerupsChannelRow)
...
import SidebarCoachmarkOverlay from "SidebarCoachmarkOverlay" /* 16268 */;
import useGuildPowerupsCoachmarkDefault from "useGuildPowerupsCoachmark" /* 16429 */;
...
function GuildPowerupsChannelRowIndicator(indicator) {
  ...
  if (null == indicator) {
    ...
    if (GuildPowerupsNotification.GuildPowerupNotificationIndicatorType.WARNING === type) {
      ...
      return hasOwnProperty(tmp4(6803).CircleErrorIcon, obj2);
    } else if (tmp4(12738).GuildPowerupNotificationIndicatorType.UNREAD === type) {
...
export default function GuildPowerupsChannelRow(guildId) {
  ...
  const tmp5 = dismissNewBadgeIfShown(12734)(guildId);
  let indicator;
  let obj = noop;
  const tmp = closure_8();
  if (tmp5 != null) {
  ...
  const tmp6Result = dismissNewBadgeIfShown(12753)(guildId, tmp8);
  dismissNewBadgeIfShown = tmp6Result.dismissNewBadgeIfShown;
  let showUnread;
  if (tmp5 != null) {
    showUnread = tmp5.showUnread;
  }
  const ChannelModes = guildId(12622).ChannelModes;
  if (true === showUnread) {
  ...
  const tmp18 = View;
  const tmp6 = dismissNewBadgeIfShown(12753);
  const intl = tmp14(1115).intl;
  obj4.accessibilityLabel = intl.string(dismissNewBadgeIfShown(2512).yv3DJJ);
  obj4.accessibilityState = { selected: false };
  const obj5 = { name: null, mode: null };
  const intl2 = tmp14(1115).intl;
  obj5.name = intl2.string(dismissNewBadgeIfShown(2512).yv3DJJ);
  obj5.mode = DEFAULT;
  obj4.name = closure_5(tmp14(12622).BaseChannelName, obj5);
  const tmp3Result = dismissNewBadgeIfShown(12622);
  obj4.icon = closure_5(tmp14(12622).BaseChannelIcon, { mode: DEFAULT, IconComponent: tmp14(16475).BoostTier2Icon });
  if (tmp6Result.showNewBadgeOnRow) {
    let tmp16Result = tmp16(tmp14(12534).NewBadge, {});
  } else {
    let indicator1;
    if (tmp5 != null) {
  ...
  return closure_5(tmp14(7352).LayerScope, obj8);
};
