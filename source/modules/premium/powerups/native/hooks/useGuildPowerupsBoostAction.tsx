...
// Dependencies: [5, 19, 4534, 4529, 1074, 12673, 7269, 7526, 4537, 4533, 5519, 7510, 2]
// Exports: default

// Module 16283 (useGuildPowerupsBoostAction)
import useAnalyticsLocationsDefault from "useAnalyticsLocations" /* 7269 */;
import useGuildBoostPurchaseHandlerDefault from "useGuildBoostPurchaseHandler" /* 12673 */;
  ...
  return shouldUseMobileWebRedirectCheckout.useCallback(asyncGeneratorStep(async (arg0, value) => {
    if (c3 === 2) {
      ...
      try {
            ...
            if (null != tmp2) {
              ...
              if (closure_2 > 0) {
                if (!handleMobileWebRedirectCheckout.hasFetched) {
                  const items = [tmp2(7526).init(), ];
                  let obj2 = tmp2(7526);
                  items[1] = tmp3(4537).fetchGuildBoostSlots();
                  dependencyMap = 1;
                  c3 = 1;
        ...
        closure_128_0 = PERK;
        availableGuildBoostSlots = tmp3(4533).getAvailableGuildBoostSlots(handleMobileWebRedirectCheckout.boostSlots);
        if (availableGuildBoostSlots.length >= closure_129_2) {
          ...
          const result = tmp3(7510).launchGuildBoostFlowOrAlert(obj10);
          const obj6 = tmp3(7510);
        }
        const obj5 = tmp3(4533);
      } catch (tmp50) {
        c3 = tmp;
        throw tmp50;
      }
