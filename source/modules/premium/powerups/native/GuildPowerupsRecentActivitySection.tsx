...
// Dependencies: [17, 4598, 21, 4606, 576, 7053, 12673, 4288, 504, 8035, 9466, 12675, 12677, 1178, 4602, 12679, 1114, 2]
// Exports: default

// Module 12672 (GuildPowerupsRecentActivitySection)
...
import useMaybeGetSortedBoosts from "useMaybeGetSortedBoosts" /* 12673 */;
import getBoostRowMessageTextDefault from "getBoostRowMessageText" /* 12679 */;
...
function GuildPowerupsRecentActivityRow(row) {
  ...
  if ("username" === stateFromStores) {
    ...
    const processColorStringsArray = tmp(8035).useProcessColorStringsArray(roleColorStrings);
    const tmpResult2 = tmp(8035);
    ...
    if ("gave" === phase) {
      let BoostGemSlashIcon = tmp(9466).BoostGemIcon;
    } else if ("expiring" === phase) {
      BoostGemSlashIcon = tmp(12675).BoostTier1Icon;
    } else {
      BoostGemSlashIcon = tmp(12677).BoostGemSlashIcon;
    }
    ...
    items2[1] = hasOwnProperty(tmp(4602).Text, obj11);
    items2[2] = hasOwnProperty(tmp(4602).Text, { variant: "text-md/medium", color: "interactive-text-active", children: " " });
    const obj12 = { variant: "text-md/medium", lineClamp: 1, style: tmp4.messageText, children: getBoostRowMessageTextDefault(row) };
    items2[3] = hasOwnProperty(tmp(4602).Text, obj12);
    ...
    items1[2] = hasOwnProperty(tmp(4602).Text, obj13);
...
export default function GuildPowerupsRecentActivitySection(guildId) {
  ...
  if (0 !== arr.length) {
    ...
    const items = [hasOwnProperty(tmp(4602).Text, obj3), ];
