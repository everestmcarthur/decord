...
// Dependencies: [19, 4526, 2015, 1979, 504, 7200, 4499, 1114, 2428, 2]
// Exports: default

// Module 12635 (useDeactivateWarningText)
...
export default function useDeactivateWarningText(arg0, skuId) {
  ...
  return noop.useMemo(() => {
    ...
    if (Powerups.GUILD_POWERUP_ROLE_COLOR_SKU_ID === skuId) {
      ...
      if (tmp2(4499).VANITY_URL_POWERUP_SKU_ID === skuId) {
        ...
        if (tmp2(4499).GUILD_TAGS_BADGE_PACK_PETS_POWERUP_SKU_ID !== skuId) {
          if (tmp2(4499).GUILD_TAGS_BADGE_PACK_FLEX_POWERUP_SKU_ID !== skuId) {
            if (tmp2(4499).GUILD_TAGS_BADGE_PACK_PLANT_POWERUP_SKU_ID !== skuId) {
              if (tmp2(4499).GUILD_TAGS_BADGE_PACK_CREEPY_CRAWLIES_POWERUP_SKU_ID !== skuId) {
      ...
      const obj4 = { text: formatToPlainStringResult1, critical: tmp.skuId === tmp2(4499).VANITY_URL_POWERUP_SKU_ID };
      const items = [obj4];
      let tmp20 = stateFromStores1;
      if (stateFromStores1) {
        tmp20 = tmp.skuId === tmp2(4499).GUILD_POWERUP_LEVEL_3_SKU_ID;
      }
      if (tmp20) {
