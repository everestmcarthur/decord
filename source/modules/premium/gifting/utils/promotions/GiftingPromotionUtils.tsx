...
// Dependencies: [32, 19, 10908, 1374, 504, 10977, 10981, 10982, 10983, 10984, 10985, 4542, 1944, 2]
...
// Module 10976 (GiftingPromotionUtils)
import PremiumGiftingGogoPromotionExperiment2 from "PremiumGiftingGogoPromotionExperiment" /* 10983 */;
import GiftPromotionReminderExperiment2 from "GiftPromotionReminderExperiment" /* 10984 */;
...
export const useFetchClaimableGiftingPromotionRewardSkuIds = function useFetchClaimableGiftingPromotionRewardSkuIds() {
  ...
  const someResult = stateFromStoresArray.some((item) => item === first(10981).GiftingPromotionRewardSKUIds.SUMMER_2026_GOGO_FAKE_SKU_ID);
...
export const shouldShowGiftPromotionReminderNotice = function shouldShowGiftPromotionReminderNotice() {
  ...
  if (GiftPromotionReminderExperiment.getConfig({ location: "shouldShowGiftPromotionReminderNotice" }).enabled) {
    if (null == PromotionsStore.getMarketingComponentByType(tmp(10985).MarketingComponentType.GIFT_REMINDER_NAGBAR)) {
      return false;
    } else {
      ...
      let tmp5 = null != id;
      if (tmp5) {
        let isDismissed = tmp(4542).UNSAFE_isSnowflakeBoundDismissibleContentDismissed(tmp(1944).DismissibleContent.GIFTING_PROMOTION_DESKTOP_FIRST_TIME_COACHMARK, id).isDismissed;
        if (isDismissed) {
          isDismissed = !tmp(4542).UNSAFE_isSnowflakeBoundDismissibleContentDismissed(tmp(1944).DismissibleContent.GIFTING_PROMOTION_REMINDER, id).isDismissed;
