...
// Dependencies: [32, 1374, 7354, 4297, 2]
...
// Module 7523 (PremiumBundledPlansUtils)
...
export const getToggledIntervalProduct = function getToggledIntervalProduct(productId) {
  if (productId in ProductIds.AppStorePremiumProductIdsToPremiumBundledItems) {
    const tmp8 = tmp(7354).AppStorePremiumProductIdsToPremiumBundledItems[productId];
    closure_0 = tmp8;
    let tmp11 = null;
    if (tmp8.premiumTier !== React4.TIER_1) {
      const _Object = Object;
      const values = Object.values(tmp(7354).AppStorePremiumProductIdsToPremiumBundledItems);
...
export const getProductIdsForBothIntervals = function getProductIdsForBothIntervals(monthly) {
  if (monthly in ProductIds.AppStorePremiumProductIdsToPremiumBundledItems) {
    const tmp8 = tmp(7354).AppStorePremiumProductIdsToPremiumBundledItems[monthly];
    let numPremiumGuild;
    if (monthly in tmp(7354).AppStorePremiumProductIdsToPremiumBundledItems) {
      const tmp14 = tmp(7354).AppStorePremiumProductIdsToPremiumBundledItems[monthly];
      ...
      if (null != productId) {
        if (null != productIdFromSubscription) {
          if (productId === productIdFromSubscription) {
            ...
            const tmp12 = tmp20(7354).AppStorePremiumProductIdsToPremiumBundledItems[productId];
            if (productIdFromSubscription in tmp20(7354).AppStorePremiumProductIdsToPremiumBundledItems) {
              const tmp18 = tmp20(7354).AppStorePremiumProductIdsToPremiumBundledItems[productIdFromSubscription];
...
export const getModifySubscriptionItemsForProduct = function getModifySubscriptionItemsForProduct(productId, subscription) {
  if (productId in found(7354).AppStorePremiumProductIdsToPremiumBundledItems) {
    const tmp8 = tmp(7354).AppStorePremiumProductIdsToPremiumBundledItems[productId];
