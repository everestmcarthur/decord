...
// Dependencies: [32, 1373, 7292, 4262, 2]
...
// Module 7461 (PremiumBundledPlansUtils)
...
export const getToggledIntervalProduct = function getToggledIntervalProduct(productId) {
  if (productId in ProductIds.AppStorePremiumProductIdsToPremiumBundledItems) {
    ...
    if (tmp8.premiumTier !== React4.TIER_1) {
      ...
      const values = Object.values(tmp(7292).AppStorePremiumProductIdsToPremiumBundledItems);
...
export const getProductIdsForBothIntervals = function getProductIdsForBothIntervals(monthly) {
  if (monthly in ProductIds.AppStorePremiumProductIdsToPremiumBundledItems) {
    const tmp8 = tmp(7292).AppStorePremiumProductIdsToPremiumBundledItems[monthly];
    let numPremiumGuild;
    if (monthly in tmp(7292).AppStorePremiumProductIdsToPremiumBundledItems) {
      const tmp14 = tmp(7292).AppStorePremiumProductIdsToPremiumBundledItems[monthly];
      ...
      if (null != productId) {
        if (null != productIdFromSubscription) {
          if (productId === productIdFromSubscription) {
            ...
            const tmp12 = tmp20(7292).AppStorePremiumProductIdsToPremiumBundledItems[productId];
            if (productIdFromSubscription in tmp20(7292).AppStorePremiumProductIdsToPremiumBundledItems) {
              const tmp18 = tmp20(7292).AppStorePremiumProductIdsToPremiumBundledItems[productIdFromSubscription];
  ...
  if (productId in found(7292).AppStorePremiumProductIdsToPremiumBundledItems) {
    ...
    if (null != tmp8.premiumTier) {
      if (tmpResult.isBoostOnlySubscription(subscription)) {
        const itemsWithUpsertedPremiumPlanId = tmp(4262).getItemsWithUpsertedPremiumPlanId(subscription, tmp8.basePlanId);
