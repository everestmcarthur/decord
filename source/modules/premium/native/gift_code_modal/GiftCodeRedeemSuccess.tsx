...
// Dependencies: [32, 19, 17, 5562, 21, 4607, 576, 504, 11537, 7242, 11139, 7628, 1889, 8257, 11180, 7197, 7300, 8919, 7246, 8891, 4792, 1178, 11205, 11315, 11316, 11544, 4603, 1114, 4846, 5027, 4810, 2]
// Exports: default

// Module 11548 (GiftCodeRedeemSuccess)
import nativeDefault from "native" /* 576 */;
import native from "native" /* 1178 */;
import ProfileEffectUserPreviewDefault from "ProfileEffectUserPreview" /* 11205 */;
import ProfileFrameUserPreviewDefault from "ProfileFrameUserPreview" /* 11315 */;
import NameplatePreview from "NameplatePreview" /* 11316 */;
import GiftBoxAnimationDefault from "GiftBoxAnimation" /* 11544 */;
...
export default function GiftCodeRedeemSuccess(giftCode) {
  ...
  const getOrFetchSubscriptionPlan = giftCode(11537).useGetOrFetchSubscriptionPlan(giftCode.subscriptionPlanId);
  const obj2 = giftCode(11537);
  const getOrFetchApplication = giftCode(7242).useGetOrFetchApplication(giftCode.applicationId);
  const obj3 = giftCode(7242);
  const obj4 = giftCode(11139);
  let skuId = null;
  ...
  if (null == first) {
    if (null != getOrFetchApplication) {
      if (tmp2Result7.isGameItemSKU(stateFromStores)) {
        ...
        obj10.children = tmp24(user(8919), obj11);
        let tmp24Result = tmp24(closure_5, obj10);
      } else {
        const obj12 = { game: getOrFetchApplication, size: tmp2(7246).GameIconSizes.LARGE, skuId: giftCode.skuId };
        tmp24Result = tmp24(user(7246), obj12);
        const tmp26 = user(7246);
      }
      tmp2Result7 = tmp2(7300);
    }
  }
  if (type === giftCode(1889).CollectiblesItemType.BUNDLE) {
    if (null != product) {
      ...
      if (tmp20Result) {
        ...
        tmp20Result = tmp20(user(8891), obj15);
      }
    ...
    if (null == stateFromStores) {
      ...
      let tmp31 = closure_8(tmp2(4603).Text, obj16);
    } else {
      if (tmp2Result8.isGameItemSKU(stateFromStores)) {
        ...
        tmp31 = closure_8(tmp2(4603).Text, obj17);
      } else {
        if (giftCode.isSubscription) {
          if (null != getOrFetchSubscriptionPlan) {
            ...
            tmp31 = closure_8(tmp2(4603).Text, obj18);
          }
        }
        if (null != first) {
          ...
          tmp31 = closure_8(tmp2(4603).Text, obj20);
        }
      }
      tmp2Result8 = tmp2(7300);
    }
    items1[1] = tmp31;
    if (tmp2Result9.isGameItemSKU(stateFromStores)) {
      if (null != getOrFetchApplication) {
        ...
        let tmp36Result = tmp42(tmp2(4603).Text, obj21);
        let tmp36 = tmp42;
      }
      ...
      if (null != first) {
        ...
        return tmp17(tmp2(7197).SafeAreaPaddingView, obj8);
      }
      obj25 = { text: null, size: "md", onPress: null };
      const intl8 = tmp2(1114).intl;
      obj25.text = intl8.string(tmp2(1114).t["NX+WJN"]);
      obj25.onPress = user(4810).pop;
      tmp43 = closure_5;
    }
    if (giftCode.isSubscription) {
      if (null != getOrFetchSubscriptionPlan) {
        const obj26 = { variant: "text-md/medium", style: tmp.message, children: tmp2(4846).getSubscriptionGiftSuccessText(getOrFetchSubscriptionPlan) };
        tmp36Result = closure_8(tmp2(4603).Text, obj26);
        tmp36 = closure_8;
        const tmp2Result10 = tmp2(4846);
      }
    }
    tmp36 = closure_8;
    ...
    tmp2Result9 = tmp2(7300);
  }
  const tmp14 = _slicedToArray(noop.useState(), 2);
  tmp18 = closure_6;
  const match = giftCode(4792).match(first);
  const str = giftCode(4792);
