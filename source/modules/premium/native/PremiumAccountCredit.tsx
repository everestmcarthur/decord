...
// Dependencies: [19, 17, 7504, 1074, 21, 4638, 576, 7282, 4295, 1115, 3075, 9518, 4634, 504, 12, 2]
// Exports: default

// Module 13495 (PremiumAccountCredit)
...
function AccountCreditTier(arg0) {
  ...
  if (hasPremiumGroup) {
    ...
    let stringResult = intl3.string(tmp4(3075)["5asczk"]);
  } else {
    if (null != currentSubscription) {
      if (currentSubscription.planId === planId) {
        if (currentSubscription.status === SubscriptionStatusTypes.PAUSED) {
          ...
          let num = tmp2(4295).extendDateWithUnconsumedFractionalPremium(date, unconsumedFractionalPremiumUnits);
  ...
  if (result1) {
    ...
    obj8.children = tmp24(tmp2(9518).BoostGemIcon, obj9);
    let tmp24Result = tmp24(tmp22, obj8);
    let tmp27 = tmp24;
  } else {
    const obj10 = { size: tmp2(7282).GameIconSizes.SMALL, skuId: result };
    tmp24Result = tmp24(tmp4(7282), obj10);
    tmp27 = tmp24;
    const tmp4Result = tmp4(7282);
  }
  ...
  if (!tmp18) {
    ...
    tmp27Result = tmp27(tmp2(4634).Text, obj13);
  }
...
let size = { width: fn(7282).GameIconImageSize[fn(undefined, 7282).GameIconSizes.SMALL], height: fn(7282).GameIconImageSize[fn(undefined, 7282).GameIconSizes.SMALL], alignItems: "center", justifyContent: "center" };
obj2.boostIcon = size;
...
export default function PremiumAccountCredit(currentSubscription) {
  ...
  if (null != entitlements) {
    if (obj8.hasAccountCredit(entitlements)) {
      ...
      const items1 = [closure_6(tmp2(4634).Text, obj3), , , ];
      ...
      items1[2] = closure_6(tmp2(4634).Text, obj5);
      let tmp9Result = null;
      if (null != currentSubscription) {
        tmp9Result = null;
        if (currentSubscription.isPurchasedExternally) {
          ...
          tmp9Result = closure_6(tmp2(4634).Text, obj6);
        }
      }
      items1[3] = tmp9Result;
