...
// Dependencies: [19, 17, 7590, 1074, 21, 4722, 576, 7368, 4380, 1115, 3160, 9613, 4718, 504, 12, 2]
// Exports: default

// Module 13600 (PremiumAccountCredit)
...
function AccountCreditTier(arg0) {
  ...
  if (hasPremiumGroup) {
    ...
    let stringResult = intl3.string(tmp4(3160)["5asczk"]);
  } else {
    if (null != currentSubscription) {
      if (currentSubscription.planId === planId) {
        if (currentSubscription.status === SubscriptionStatusTypes.PAUSED) {
          ...
          let num = tmp2(4380).extendDateWithUnconsumedFractionalPremium(date, unconsumedFractionalPremiumUnits);
  ...
  if (result1) {
    ...
    obj8.children = tmp24(tmp2(9613).BoostGemIcon, obj9);
    let tmp24Result = tmp24(tmp22, obj8);
    let tmp27 = tmp24;
  } else {
    const obj10 = { size: tmp2(7368).GameIconSizes.SMALL, skuId: result };
    tmp24Result = tmp24(tmp4(7368), obj10);
    tmp27 = tmp24;
    const tmp4Result = tmp4(7368);
  }
  ...
  if (!tmp18) {
    ...
    tmp27Result = tmp27(tmp2(4718).Text, obj13);
  }
...
let size = { width: fn(7368).GameIconImageSize[fn(undefined, 7368).GameIconSizes.SMALL], height: fn(7368).GameIconImageSize[fn(undefined, 7368).GameIconSizes.SMALL], alignItems: "center", justifyContent: "center" };
obj2.boostIcon = size;
...
export default function PremiumAccountCredit(currentSubscription) {
  ...
  if (null != entitlements) {
    if (obj8.hasAccountCredit(entitlements)) {
      ...
      const items1 = [closure_6(tmp2(4718).Text, obj3), , , ];
      ...
      items1[2] = closure_6(tmp2(4718).Text, obj5);
      let tmp9Result = null;
      if (null != currentSubscription) {
        tmp9Result = null;
        if (currentSubscription.isPurchasedExternally) {
          ...
          tmp9Result = closure_6(tmp2(4718).Text, obj6);
        }
      }
      items1[3] = tmp9Result;
