...
// Dependencies: [19, 17, 7508, 1074, 21, 4640, 576, 7286, 4297, 1115, 3077, 9529, 4636, 504, 12, 2]
// Exports: default

// Module 13503 (PremiumAccountCredit)
...
function AccountCreditTier(arg0) {
  ...
  if (hasPremiumGroup) {
    ...
    let stringResult = intl3.string(tmp4(3077)["5asczk"]);
  } else {
    if (null != currentSubscription) {
      if (currentSubscription.planId === planId) {
        if (currentSubscription.status === SubscriptionStatusTypes.PAUSED) {
          ...
          let num = tmp2(4297).extendDateWithUnconsumedFractionalPremium(date, unconsumedFractionalPremiumUnits);
  ...
  if (result1) {
    ...
    obj8.children = tmp24(tmp2(9529).BoostGemIcon, obj9);
    let tmp24Result = tmp24(tmp22, obj8);
    let tmp27 = tmp24;
  } else {
    const obj10 = { size: tmp2(7286).GameIconSizes.SMALL, skuId: result };
    tmp24Result = tmp24(tmp4(7286), obj10);
    tmp27 = tmp24;
    const tmp4Result = tmp4(7286);
  }
  ...
  if (!tmp18) {
    ...
    tmp27Result = tmp27(tmp2(4636).Text, obj13);
  }
...
let size = { width: fn(7286).GameIconImageSize[fn(undefined, 7286).GameIconSizes.SMALL], height: fn(7286).GameIconImageSize[fn(undefined, 7286).GameIconSizes.SMALL], alignItems: "center", justifyContent: "center" };
obj2.boostIcon = size;
...
export default function PremiumAccountCredit(currentSubscription) {
  ...
  if (null != entitlements) {
    if (obj8.hasAccountCredit(entitlements)) {
      ...
      const items1 = [closure_6(tmp2(4636).Text, obj3), , , ];
      ...
      items1[2] = closure_6(tmp2(4636).Text, obj5);
      let tmp9Result = null;
      if (null != currentSubscription) {
        tmp9Result = null;
        if (currentSubscription.isPurchasedExternally) {
          ...
          tmp9Result = closure_6(tmp2(4636).Text, obj6);
        }
      }
      items1[3] = tmp9Result;
