...
// Dependencies: [19, 17, 8255, 10735, 1074, 21, 4605, 576, 7033, 1483, 10769, 11119, 504, 10812, 10822, 10810, 10808, 10813, 8247, 10733, 10732, 11114, 1114, 10823, 10824, 2460, 10320, 10826, 7234, 4601, 2024, 5025, 4470, 2]
// Exports: default

// Module 11118 (PremiumGiftPurchaseButton)
...
import PremiumGiftingGogoPromotionExperimentDefault from "PremiumGiftingGogoPromotionExperiment" /* 10810 */;
...
export default function PremiumGiftPurchaseButton(defaultSelection) {
  ...
  const nativeGiftContext = defaultSelection(10769).useNativeGiftContext();
  ...
  let obj2 = defaultSelection(10769);
  const canPurchaseIAP = defaultSelection(11119).useCanPurchaseIAP(productId);
  const obj3 = defaultSelection(11119);
  let items = [c7];
  ...
  const GiftingBadgeExperiment = tmp4(10813).GiftingBadgeExperiment;
  ...
  const product = defaultSelection(11114).useFetchCollectiblesProduct(selectedGiftingPromotionReward).product;
  ...
  const tmp4Result3 = defaultSelection(11114);
  let asset;
  if (stateFromStores != null) {
    asset = stateFromStores.asset;
  }
  const themeAndReducedMotionAwareAssetUrl = defaultSelection(10823).useThemeAndReducedMotionAwareAssetUrl(asset);
  const obj5 = { style: tmp3.container, children: null };
  if (tmp10) {
    if (config.enabled) {
      if (selectedGiftingPromotionReward === tmp4(10808).GiftingPromotionRewardSKUIds.SUMMER_2026_GOGO_FAKE_SKU_ID) {
        ...
        let tmp22Result = closure_9(tmp(10824), obj6);
        const tmpResult5 = tmp(10824);
      }
      ...
      if (!tmp11) {
        ...
        tmp33 = closure_9(tmp4(4601).Text, obj7);
      ...
      if (!isPurchasing) {
        fn = () => {
          ...
          if (closure_10) {
            if (closure_8) {
              if (closure_8) {
                _undefined2(tmp(10733).PaymentFlowStep.REWARD_SKU_SELECT);
                ...
                obj2.onSelect = function onSelect(arg0) {
                  ...
                  navigation.navigate(defaultSelection(10732).PremiumGiftScreens.CUSTOMIZATION);
                };
                navigation.navigate(tmp(10732).PremiumGiftScreens.REWARD_SELECT, obj2);
              }
            }
          }
          _undefined(() => {
            navigation.navigate(defaultSelection(10732).PremiumGiftScreens.SUCCESS);
          });
        };
      }
      obj9.onPress = fn;
      items3[2] = closure_9(tmp4(5025).Button, obj9);
  ...
  if (tmp11) {
    if (tmp8) {
      ...
      tmp22Result = closure_9(tmp(10824), obj10);
      const tmpResult7 = tmp(10824);
    }
  }
  if (tmp15) {
    if (tmp10) {
      if (null != selectedGiftingPromotionReward) {
        ...
        obj12.subtitle = name;
        const items5 = [closure_9(tmp4(10824).PremiumGiftPromotionCollectibleRewardDetails, obj12), ];
        let tmp26Result = !tmp9;
        if (!tmp9) {
          tmp26Result = tmp26(tmp4(10320).PencilIcon, { size: "sm" });
        }
  ...
  if (GiftingBadgeExperiment.useConfig({ location: "PremiumGiftPurchaseButton" }).enabled) {
    ...
    if (null != nextTier) {
      ...
      obj13.analyticsLocation = tmp(7234).PREMIUM_GIFT_CUSTOMIZATION;
      tmp22Result = closure_9(tmp(10826), obj13);
      const tmpResult8 = tmp(10826);
    }
  }
};
