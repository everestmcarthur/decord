...
// Dependencies: [19, 17, 5562, 1373, 1074, 21, 4607, 576, 10790, 1483, 10834, 4846, 10753, 8449, 5027, 1114, 4264, 7263, 4304, 10921, 4603, 8994, 4555, 2]
// Exports: PremiumGiftSuccessActions, default

// Module 11170 (PremiumGiftPurchaseSuccess)
...
export default function PremiumGiftSuccessBody(giftCodeRecord) {
  ...
  let tmp = closure_11();
  const nativeGiftContext = giftCodeRecord(10790).useNativeGiftContext();
  ({ giftStyle, premiumType, planInterval } = nativeGiftContext);
  let obj = giftCodeRecord(10790);
  const giftCodeURL = giftCodeRecord(4846).getGiftCodeURL(giftCodeRecord.code);
  ...
  if (null != giftCodeRecord.subscriptionPlanId) {
    ...
    subscriptionPlanId = tmp2(4264).getPlanIdForPremiumType(premiumType, planInterval);
    const tmp2Result = tmp2(4264);
  }
  let obj2 = giftCodeRecord(4846);
  const tmp6 = giftCodeURL;
  const tierDisplayNameByPlanId = giftCodeURL(4264).getTierDisplayNameByPlanId(subscriptionPlanId);
  const obj4 = giftCodeURL(4264);
  ...
  obj3.children = closure_8(giftCodeURL(10921), { giftStyle });
  ...
  items1[1] = closure_8(giftCodeRecord(4603).Text, obj6);
  ...
  items1[2] = closure_8(giftCodeRecord(4603).Text, obj7);
  ...
  const items2 = [closure_8(giftCodeRecord(4603).Text, obj10), ];
  obj5 = giftCodeURL(4264);
  const tmp10 = closure_9;
  const tmp12 = View;
  items2[1] = closure_8(giftCodeRecord(8994).InputButton, { text: giftCodeURL, icon: tmp6(4555), iconPosition: "end", onPress: callback });
  ...
  items1[4] = closure_8(giftCodeRecord(4603).Text, obj12);
