...
// Dependencies: [109, 5, 32, 19, 17, 7498, 2025, 7311, 7495, 1373, 1074, 1182, 4589, 1085, 21, 4607, 576, 4264, 4603, 1114, 38, 7505, 1880, 504, 13400, 7308, 5027, 5014, 5023, 10794, 7491, 4542, 7521, 8136, 9472, 8256, 7236, 7256, 5650, 10754, 7480, 5044, 10899, 4574, 1242, 7479, 1608, 4952, 10795, 4931, 7483, 4458, 10813, 10814, 10815, 10816, 10817, 10818, 1115, 2024, 7224, 11531, 7228, 5639, 8695, 13401, 1178, 9458, 10798, 10895, 2]
// Exports: default, getItemsByPremiumTypePredicate

// Module 7497 (PremiumPlanSelectionActionSheet)
...
import MobileWebRedirectCheckoutUtils from "MobileWebRedirectCheckoutUtils" /* 7480 */;
import PremiumPlanActionSheetHeaderDefault from "PremiumPlanActionSheetHeader" /* 7505 */;
import ACOMExperiments from "ACOMExperiments" /* 9458 */;
import NativeCheckoutStoreProviderDefault from "NativeCheckoutStoreProvider" /* 10895 */;
...
function Header(arg0) {
  ...
  if (tmp9) {
    ...
    tmp9 = skuId === tmp6(4264).getSkuIdForPremiumType(premiumType);
    const tmp6Result = tmp6(4264);
  }
  let tmp11 = tmp9;
  if (tmp11) {
...
function PlanOption(premiumItem) {
  ...
  let checkoutPlanPriceString = premiumItem(13400).useCheckoutPlanPriceString(premiumItem.productId, stateFromStores);
  const obj2 = premiumItem(13400);
  ...
  const checkoutPlanDiscountPrices = premiumItem(13400).useCheckoutPlanDiscountPrices(premiumItem.productId, obj4);
  const obj3 = premiumItem(13400);
  ({ orderRequired, orderRecord } = useNativeCheckoutStore((orderRequired) => ({ orderRequired: orderRequired.orderRequired, orderRecord: orderRequired.orderRecord })));
  ...
  if (tmp11) {
    ...
    tmp11 = skuId === first(4264).getSkuIdForPremiumType(premiumTier);
    const obj5 = first(4264);
  }
  let tmp14 = tmp11;
  if (tmp14) {
  ...
  const tierDisplayNameByPlanId = premiumItem(4264).getTierDisplayNameByPlanId(premiumItem.basePlanId);
  const tmp4Result = premiumItem(4264);
  const intervalString = first(4264).getIntervalString(premiumItem.interval, false);
  let tmp20 = tmp14;
  if (!tmp14) {
  ...
  if (null != checkoutPlanDiscountPrices) {
    formatRateResult = tmp4(7308).formatRate(checkoutPlanDiscountPrices.discountedPrice, tmp26.interval, tmp26.intervalCount);
    const tmp4Result5 = tmp4(7308);
  }
  if (tmp14) {
    const intl = tmp4(1114).intl;
    const obj6 = { price: tmp4(7308).formatPrice(0, USD, { minimumFractionDigits: 0, maximumFractionDigits: 0 }) };
    let formatToPlainStringResult = intl.formatToPlainString(tmp4(1114).t.hXcaLT, obj6);
    const tmp4Result6 = tmp4(7308);
  } else {
  ...
  const tmp4Result7 = premiumItem(7308);
  if (first > 0) {
  ...
  const items3 = [closure_35(premiumItem(4603).Text, obj11), ];
  ...
  items4[1] = closure_35(premiumItem(4603).Text, obj17);
  ...
  if (!tmp14) {
    ...
    if (null != formatRateResult) {
      ...
      if (null != discountOffer) {
        ...
        if (null == premiumItem.selectedPremiumType) {
          ...
          tmp33Result4 = tmp33(tmp4(4603).Text, obj19);
        }
      }
    }
    ...
    if (tmp33Result5) {
      ...
      tmp33Result5 = tmp33(tmp4(4603).Text, obj21);
    }
                ...
                if (orderRequired) {
                  ...
                  const obj5 = {
                    value: LocaleStore(subscriptionItemsForProduct.map((planId) => {
                                    ...
                                    const obj2 = closure_1_0(4264);
                                    obj.sku_id = obj2.castPremiumSubscriptionAsSkuId(closure_1_1(4264).getSkuIdForPlan(planId.planId));
...
let useNativeCheckoutStore = fn(7498).useNativeCheckoutStore;
...
const PaymentConstants = fn(4589);
...
export default function PremiumPlanSelectionActionSheetWithOrderCTX(predicate) {
  ...
  if (obj.isIOS()) {
    ...
    const premiumTrialOffer = tmp(7521).usePremiumTrialOffer();
    ...
    const isEligibleForBogoOffer = tmp(10798).useIsEligibleForBogoOffer();
    if (null == fn3) {
      let premiumBundlesWithPredicate = tmp(7483).getPremiumBundlesWithPredicate(fn);
      const tmpResult7 = tmp(7483);
    } else {
      const premiumBundlesWithPredicate1 = tmp(7483).getPremiumBundlesWithPredicate(fn);
      premiumBundlesWithPredicate = premiumBundlesWithPredicate1.sort(fn3);
      const tmpResult8 = tmp(7483);
    }
    ...
    if (null != found) {
      ...
      const tmpResult9 = tmp(7483);
    } else {
      const obj5 = { subscriptionPlanId: __initData.PREMIUM_YEAR_TIER_2, skuId: tmp(4264).castPremiumSubscriptionAsSkuId(TIER_22.TIER_2), quantity: 1 };
      mapped = [obj5];
      const tmpResult10 = tmp(4264);
    }
    const obj6 = {
