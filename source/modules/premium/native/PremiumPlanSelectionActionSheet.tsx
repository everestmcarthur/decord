...
// Dependencies: [109, 5, 32, 19, 17, 7497, 2025, 7310, 7494, 1373, 1074, 1182, 4588, 1085, 21, 4606, 576, 4264, 4602, 1114, 38, 7504, 1880, 504, 13398, 7307, 5026, 5013, 5022, 10792, 7490, 4541, 7520, 8135, 9470, 8255, 7235, 7255, 5649, 10752, 7479, 5043, 10897, 4573, 1242, 7478, 1608, 4951, 10793, 4930, 7482, 4458, 10811, 10812, 10813, 10814, 10815, 10816, 1115, 2024, 7223, 11529, 7227, 5638, 8693, 13399, 1178, 9456, 10796, 10893, 2]
// Exports: default, getItemsByPremiumTypePredicate

// Module 7496 (PremiumPlanSelectionActionSheet)
...
import PremiumPlanActionSheetHeaderDefault from "PremiumPlanActionSheetHeader" /* 7504 */;
import ACOMExperiments from "ACOMExperiments" /* 9456 */;
import NativeCheckoutStoreProviderDefault from "NativeCheckoutStoreProvider" /* 10893 */;
...
function PlanOption(premiumItem) {
  ...
  let checkoutPlanPriceString = premiumItem(13398).useCheckoutPlanPriceString(premiumItem.productId, stateFromStores);
  const obj2 = premiumItem(13398);
  ...
  const checkoutPlanDiscountPrices = premiumItem(13398).useCheckoutPlanDiscountPrices(premiumItem.productId, obj4);
  const obj3 = premiumItem(13398);
  ({ orderRequired, orderRecord } = useNativeCheckoutStore((orderRequired) => ({ orderRequired: orderRequired.orderRequired, orderRecord: orderRequired.orderRecord })));
  ...
  if (null != checkoutPlanDiscountPrices) {
    formatRateResult = tmp4(7307).formatRate(checkoutPlanDiscountPrices.discountedPrice, tmp26.interval, tmp26.intervalCount);
    const tmp4Result5 = tmp4(7307);
  }
  if (tmp14) {
    const intl = tmp4(1114).intl;
    const obj6 = { price: tmp4(7307).formatPrice(0, USD, { minimumFractionDigits: 0, maximumFractionDigits: 0 }) };
    let formatToPlainStringResult = intl.formatToPlainString(tmp4(1114).t.hXcaLT, obj6);
    const tmp4Result6 = tmp4(7307);
  } else {
  ...
  const tmp4Result7 = premiumItem(7307);
  if (first > 0) {
  ...
  const items3 = [closure_35(premiumItem(4602).Text, obj11), ];
  ...
  items4[1] = closure_35(premiumItem(4602).Text, obj17);
  ...
  if (!tmp14) {
    ...
    if (null != formatRateResult) {
      ...
      if (null != discountOffer) {
        ...
        if (null == premiumItem.selectedPremiumType) {
          ...
          tmp33Result4 = tmp33(tmp4(4602).Text, obj19);
        }
      }
    }
    ...
    if (tmp33Result5) {
      ...
      tmp33Result5 = tmp33(tmp4(4602).Text, obj21);
    }
...
const PaymentConstants = fn(4588);
...
export default function PremiumPlanSelectionActionSheetWithOrderCTX(predicate) {
  ...
  if (obj.isIOS()) {
    ...
    const premiumTrialOffer = tmp(7520).usePremiumTrialOffer();
    ...
    const isEligibleForBogoOffer = tmp(10796).useIsEligibleForBogoOffer();
    if (null == fn3) {
      let premiumBundlesWithPredicate = tmp(7482).getPremiumBundlesWithPredicate(fn);
      const tmpResult7 = tmp(7482);
    } else {
      const premiumBundlesWithPredicate1 = tmp(7482).getPremiumBundlesWithPredicate(fn);
      premiumBundlesWithPredicate = premiumBundlesWithPredicate1.sort(fn3);
      const tmpResult8 = tmp(7482);
    }
    ...
    if (null != found) {
      const subscriptionItemsForProduct = tmp(7482).getSubscriptionItemsForProduct(found.productId);
