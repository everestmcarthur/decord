...
// Dependencies: [109, 5, 32, 19, 17, 7620, 2026, 7433, 7617, 1374, 1074, 1181, 4704, 1085, 21, 4722, 576, 4380, 4718, 1115, 38, 7627, 1881, 504, 13547, 7430, 5144, 5129, 5138, 10946, 7613, 4656, 7643, 8280, 9617, 8400, 7358, 7378, 5768, 10906, 7602, 5161, 11056, 4689, 1241, 7601, 1609, 5067, 10947, 5046, 7605, 4573, 10965, 10966, 10967, 10968, 10969, 10970, 1364, 2025, 7346, 11687, 7350, 5757, 8840, 13548, 1177, 9603, 10950, 11052, 2]
// Exports: default, getItemsByPremiumTypePredicate

// Module 7619 (PremiumPlanSelectionActionSheet)
...
import PremiumPlanActionSheetHeaderDefault from "PremiumPlanActionSheetHeader" /* 7627 */;
import ACOMExperiments from "ACOMExperiments" /* 9603 */;
import NativeCheckoutStoreProviderDefault from "NativeCheckoutStoreProvider" /* 11052 */;
...
function Header(arg0) {
  ...
  if (tmp9) {
    ...
    tmp9 = skuId === tmp6(4380).getSkuIdForPremiumType(premiumType);
    const tmp6Result = tmp6(4380);
  }
  let tmp11 = tmp9;
  if (tmp11) {
...
function PlanOption(premiumItem) {
  ...
  let checkoutPlanPriceString = premiumItem(13547).useCheckoutPlanPriceString(premiumItem.productId, stateFromStores);
  const obj2 = premiumItem(13547);
  ...
  const checkoutPlanDiscountPrices = premiumItem(13547).useCheckoutPlanDiscountPrices(premiumItem.productId, obj4);
  const obj3 = premiumItem(13547);
  ({ orderRequired, orderRecord } = useNativeCheckoutStore((orderRequired) => ({ orderRequired: orderRequired.orderRequired, orderRecord: orderRequired.orderRecord })));
  ...
  if (tmp11) {
    ...
    tmp11 = skuId === first(4380).getSkuIdForPremiumType(premiumTier);
    const obj5 = first(4380);
  }
  let tmp14 = tmp11;
  if (tmp14) {
  ...
  const tierDisplayNameByPlanId = premiumItem(4380).getTierDisplayNameByPlanId(premiumItem.basePlanId);
  const tmp4Result = premiumItem(4380);
  const intervalString = first(4380).getIntervalString(premiumItem.interval, false);
  let tmp20 = tmp14;
  if (!tmp14) {
  ...
  if (null != checkoutPlanDiscountPrices) {
    formatRateResult = tmp4(7430).formatRate(checkoutPlanDiscountPrices.discountedPrice, tmp26.interval, tmp26.intervalCount);
    const tmp4Result5 = tmp4(7430);
  }
  if (tmp14) {
    const intl = tmp4(1115).intl;
    const obj6 = { price: tmp4(7430).formatPrice(0, USD, { minimumFractionDigits: 0, maximumFractionDigits: 0 }) };
    let formatToPlainStringResult = intl.formatToPlainString(tmp4(1115).t.hXcaLT, obj6);
    const tmp4Result6 = tmp4(7430);
  } else {
  ...
  const tmp4Result7 = premiumItem(7430);
  if (first > 0) {
  ...
  const items3 = [closure_35(premiumItem(4718).Text, obj11), ];
  ...
  items4[1] = closure_35(premiumItem(4718).Text, obj17);
  ...
  if (!tmp14) {
    ...
    if (null != formatRateResult) {
      ...
      if (null != discountOffer) {
        ...
        if (null == premiumItem.selectedPremiumType) {
          ...
          tmp33Result4 = tmp33(tmp4(4718).Text, obj19);
        }
      }
    }
    ...
    if (tmp33Result5) {
      ...
      tmp33Result5 = tmp33(tmp4(4718).Text, obj21);
    }
        ...
        if (0 === c5) {
          ...
          if (1 === tmp7) {
            if (arg0 === 1) {
              ...
              premiumType(tmp3[20])(null != closure_130_17, "cannot start payment without a selectedItem");
              ...
              const obj10 = { from_step: closure_0(tmp3[39]).PaymentFlowStep.PLAN_SELECT, to_step: closure_129_3, subscription_plan_gateway_plan_id: closure_130_17.productId, sku_id: closure_130_22 };
              paymentFlowStepAnalyticsFields = closure_0(tmp3[39]).getPaymentFlowStepAnalyticsFields(closure_130_23, obj10);
              if (!closure_129_2) {
...
const PremiumPlanPurchasedStore = fn(7617);
...
export default function PremiumPlanSelectionActionSheetWithOrderCTX(predicate) {
  ...
  if (obj.isIOS()) {
    ...
    const premiumTrialOffer = tmp(7643).usePremiumTrialOffer();
    ...
    const isEligibleForBogoOffer = tmp(10950).useIsEligibleForBogoOffer();
    if (null == fn3) {
      let premiumBundlesWithPredicate = tmp(7605).getPremiumBundlesWithPredicate(fn);
      const tmpResult7 = tmp(7605);
    } else {
      const premiumBundlesWithPredicate1 = tmp(7605).getPremiumBundlesWithPredicate(fn);
    ...
    if (null != found) {
      ...
      const tmpResult9 = tmp(7605);
    } else {
      const obj5 = { subscriptionPlanId: __initData.PREMIUM_YEAR_TIER_2, skuId: tmp(4380).castPremiumSubscriptionAsSkuId(TIER_2.TIER_2), quantity: 1 };
      mapped = [obj5];
