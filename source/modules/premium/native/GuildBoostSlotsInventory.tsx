...
// Dependencies: [19, 17, 1182, 1980, 4616, 4386, 1074, 21, 4722, 576, 5694, 7635, 4718, 1115, 11, 5292, 5604, 5754, 13705, 504, 1397, 13706, 13710, 5046, 4619, 12, 2]
// Exports: default

// Module 13704 (GuildBoostSlotsInventory)
...
import actions_BillingActionCreatorsAll from "actions/BillingActionCreators" /* 5046 */;
...
const SubscriptionPlaceholderPattern = tmp2(13706);
require = fn;
...
function GuildBoostSlot(guildBoostSlot) {
  ...
  if (null == guildBoostSlot.guild) {
    ...
    if (!guildBoostSlot.isOnCooldown()) {
      ...
      obj5.children = tmp6(guildBoostSlot(4718).Text, obj6);
      tmp6Result2 = tmp6(guildBoostSlot(5292).PressableOpacity, obj5);
    }
...
function UnusedGuildBoostSlots(unusedSlots) {
  ...
  if (found1.length > 0) {
    ...
    obj4.children = closure_11(found(4718).Text, obj5);
    obj2.action = closure_11(found(5292).PressableOpacity, obj4);
...
function BoostedGuildInfo(guild) {
  ...
  if (null != guild) {
    ...
    const obj7 = { source: _modDef13705, style: tmp.guildInfoRowIcon };
...
function BoostedGuild(arg0) {
  ...
  items3[2] = closure_11(guildBoostSlots(13710), { guild: stateFromStores, theme: stateFromStores1 });
...
export default function GuildBoostSlotsInventory() {
  ...
  const effect = noop.useEffect(() => {
    ...
    const guildBoostSlots = valueResult(4619).fetchGuildBoostSlots();
  }, []);
  ...
  if (0 !== found.length) {
    ...
    if (null != stateFromStores) {
      ...
      if (found.length > 0) {
        ...
        const items3 = [closure_11(tmp3(4718).Text, obj7), found.map((guildId) => closure_2_11(BoostedGuild, { guildId, guildBoostSlots: valueResult[guildId] }, guildId))];
        obj6.children = items3;
