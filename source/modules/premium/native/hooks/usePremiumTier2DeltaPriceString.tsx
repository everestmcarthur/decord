...
// Dependencies: [19, 7498, 7311, 1373, 7483, 7314, 1115, 7308, 4279, 504, 2]
// Exports: usePremiumTier2DeltaPriceString

// Module 13605 (usePremiumTier2DeltaPriceString)
...
function getViewerProductId(subscription) {
  if (null == subscription) {
    ...
    try {
      ...
      try {
        const productIdFromSubscription1 = tmp3(7483).getProductIdFromSubscription(subscription, true);
        const tmp8 = tmp3(7314).AppStorePremiumProductIdsToPremiumBundledItems[productIdFromSubscription];
        const tmp10 = tmp3(7314).AppStorePremiumProductIdsToPremiumBundledItems[productIdFromSubscription1];
        if (null != tmp8) {
          if (null != tmp10) {
...
export const usePremiumTier2DeltaPriceString = function usePremiumTier2DeltaPriceString(plan, subscription, currencyCode, flag4) {
  ...
  if (flag4) {
    ...
    if (plan.premiumTier === PremiumTypes.TIER_2) {
      ...
      if (plan.numPremiumGuild >= 1) {
        ...
        let tmp11 = null;
        if (null != tmp2Result) {
          tmp11 = tmp4(7314).AppStorePremiumProductIdsToPremiumBundledItems[tmp2Result];
        }
  ...
  if (flag) {
          ...
          if (currencyCode.currencyCode !== stateFromStores.currencyCode) {
            ...
            if (diff > 0) {
              if (diff < currencyCode.price) {
                ...
                obj5.priceString = tmp4(7308).formatPrice(result, currencyCode.currencyCode, { convertToMajorUnits: false });
                obj6 = obj5;
                const tmp4Result8 = tmp4(7308);
              }
            }
    ...
    if (null == checkoutContext) {
      ...
      const availablePlanForItems = checkoutContext.getAvailablePlanForItems(tmp4(7483).getSubscriptionItemsForProduct(plan.productId));
      if (null == availablePlanForItems) {
        ...
        if (null != addOnPrice) {
          if (addOnPrice.majorUnits > 0) {
            let tmp17 = null;
            if (null != tmp3) {
              tmp17 = tmp4(7314).AppStorePremiumProductIdsToPremiumBundledItems[tmp3];
            }
            if (null != tmp3) {
              if (null != tmp17) {
                if (0 !== tmp17.numPremiumGuild) {
                  const availablePlanForItems1 = checkoutContext.getAvailablePlanForItems(tmp4(7483).getSubscriptionItemsForProduct(tmp3));
                  let addOnPrice1;
                  ...
                  if (null != addOnPrice1) {
                    ...
                    if (diff1 > 0) {
                      const obj8 = { priceString: tmp4(7308).formatPrice(diff1, addOnPrice.currency, { convertToMajorUnits: false }), failure: null };
                      const tmp4Result11 = tmp4(7308);
                    }
                  }
                  const tmp4Result10 = tmp4(7483);
                }
              }
            }
            const obj9 = { priceString: tmp4(7308).formatPrice(addOnPrice.majorUnits, addOnPrice.currency, { convertToMajorUnits: false }), failure: null };
            const tmp4Result12 = tmp4(7308);
          }
        }
      }
      const tmp4Result9 = tmp4(7483);
    }
  } else {
