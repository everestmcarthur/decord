...
// Dependencies: [19, 7497, 7310, 1373, 7482, 7313, 1115, 7307, 4279, 504, 2]
// Exports: usePremiumTier2DeltaPriceString

// Module 13603 (usePremiumTier2DeltaPriceString)
...
function getViewerProductId(subscription) {
  if (null == subscription) {
    ...
    try {
      ...
      try {
        const productIdFromSubscription1 = tmp3(7482).getProductIdFromSubscription(subscription, true);
        const tmp8 = tmp3(7313).AppStorePremiumProductIdsToPremiumBundledItems[productIdFromSubscription];
        const tmp10 = tmp3(7313).AppStorePremiumProductIdsToPremiumBundledItems[productIdFromSubscription1];
        if (null != tmp8) {
          if (null != tmp10) {
...
export const usePremiumTier2DeltaPriceString = function usePremiumTier2DeltaPriceString(plan, subscription, currencyCode, flag4) {
  ...
  if (flag4) {
    ...
    if (plan.premiumTier === PremiumTypes.TIER_2) {
      ...
      if (plan.numPremiumGuild >= 1) {
        ...
        let tmp11 = null;
        if (null != tmp2Result) {
          tmp11 = tmp4(7313).AppStorePremiumProductIdsToPremiumBundledItems[tmp2Result];
        }
  ...
  if (flag) {
    if (!tmp7) {
      if (null != currencyCode) {
        if (null != stateFromStores) {
          const platformName = tmp4(1115).getPlatformName();
          if (currencyCode.currencyCode !== stateFromStores.currencyCode) {
            ...
            if (diff > 0) {
              if (diff < currencyCode.price) {
                ...
                tmp4Result7 = tmp4(1115);
                obj5.priceString = tmp4(7307).formatPrice(result, currencyCode.currencyCode, { convertToMajorUnits: false });
                obj6 = obj5;
                const tmp4Result8 = tmp4(7307);
              }
            }
    ...
    if (null == checkoutContext) {
      ...
      const availablePlanForItems = checkoutContext.getAvailablePlanForItems(tmp4(7482).getSubscriptionItemsForProduct(plan.productId));
      if (null == availablePlanForItems) {
        ...
        if (null != addOnPrice) {
          if (addOnPrice.majorUnits > 0) {
            let tmp17 = null;
            if (null != tmp3) {
              tmp17 = tmp4(7313).AppStorePremiumProductIdsToPremiumBundledItems[tmp3];
            }
            if (null != tmp3) {
              if (null != tmp17) {
                if (0 !== tmp17.numPremiumGuild) {
                  const availablePlanForItems1 = checkoutContext.getAvailablePlanForItems(tmp4(7482).getSubscriptionItemsForProduct(tmp3));
                  let addOnPrice1;
                  ...
                  if (null != addOnPrice1) {
                    ...
                    if (diff1 > 0) {
                      const obj8 = { priceString: tmp4(7307).formatPrice(diff1, addOnPrice.currency, { convertToMajorUnits: false }), failure: null };
                      const tmp4Result11 = tmp4(7307);
                    }
                  }
                  const tmp4Result10 = tmp4(7482);
                }
              }
            }
            const obj9 = { priceString: tmp4(7307).formatPrice(addOnPrice.majorUnits, addOnPrice.currency, { convertToMajorUnits: false }), failure: null };
            const tmp4Result12 = tmp4(7307);
          }
        }
      }
      const tmp4Result9 = tmp4(7482);
    }
  } else {
