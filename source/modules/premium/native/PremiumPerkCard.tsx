...
// Dependencies: [19, 17, 1374, 1074, 21, 5151, 13606, 4380, 7578, 7378, 1115, 13607, 13608, 13609, 13610, 13611, 13612, 13613, 13614, 13615, 13616, 13617, 13618, 13619, 13620, 13621, 13622, 13623, 4718, 2025, 4722, 576, 5757, 13624, 5144, 2]
// Exports: default, usePerkCardHeight, usePremiumPerkCard

// Module 13605 (PremiumPerkCard)
...
import PillTextDefault from "PillText" /* 13624 */;
...
export default function PremiumPerkCard(variant) {
  ...
  if (null != imageSrc) {
    if (null != imageOverlayText) {
      ...
      imageSrc = React7(tmp6(4718).Text, obj5);
    ...
    if (null != description) {
      ...
      let tmp21 = React7(tmp6(4718).Text, obj7);
    } else {
      tmp21 = null;
    ...
    items4[2] = React7(tmp6(4718).Text, obj11);
    ...
    if (tmp29Result) {
      ...
      obj13.children = tmp29(tmp6(5144).Button, obj14);
...
export const usePremiumPerkCard = function usePremiumPerkCard() {
  subscriptionPlansLoaded = subscriptionPlansLoaded(13606).useSubscriptionPlansLoaded();
  let obj = subscriptionPlansLoaded(13606);
  const maxFileSizeForPremiumType = subscriptionPlansLoaded(4380).getMaxFileSizeForPremiumType(PremiumTypes.TIER_2);
  const callback = noop.useCallback(() => {
    const obj2 = { screen: constants.COLLECTIBLES_SHOP, params: null };
    const obj = subscriptionPlansLoaded(7578);
    obj2.params = { analyticsSource: AnalyticsLocationDefault.PREMIUM_MARKETING_PERK_CARD };
    obj.openUserSettings(obj2);
  }, []);
  const callback1 = noop.useCallback(() => {
    subscriptionPlansLoaded(7578).openUserSettings({ screen: constants.PROFILE_CUSTOMIZATION });
  }, []);
  const items = [subscriptionPlansLoaded];
  const callback2 = noop.useCallback(() => {
    subscriptionPlansLoaded(7578).openUserSettings({ screen: constants.APPEARANCE_THEME_PICKER });
  }, []);
  ...
  const callback4 = noop.useCallback(() => {
    subscriptionPlansLoaded(7578).openUserSettings({ screen: constants.APP_ICONS });
  }, []);
  ...
  let obj2 = subscriptionPlansLoaded(4380);
  obj20.imageSrc = { uri: _modDef13623 };
  ...
  obj20.bodyComponent = closure_9(subscriptionPlansLoaded(4718).Text, obj22);
