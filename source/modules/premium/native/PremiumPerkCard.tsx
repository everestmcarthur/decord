...
// Dependencies: [19, 17, 1374, 1074, 21, 5069, 13509, 4297, 7496, 7296, 1115, 13510, 13511, 13512, 13513, 13514, 13515, 13516, 13517, 13518, 13519, 13520, 13521, 13522, 13523, 13524, 13525, 13526, 4636, 2025, 4640, 576, 5675, 13527, 5062, 2]
// Exports: default, usePerkCardHeight, usePremiumPerkCard

// Module 13508 (PremiumPerkCard)
...
import _modDef13518 from "module_13518" /* 13518 */;
...
import PillTextDefault from "PillText" /* 13527 */;
...
export default function PremiumPerkCard(variant) {
  ...
  if (null != imageSrc) {
    if (null != imageOverlayText) {
      ...
      imageSrc = React7(tmp6(4636).Text, obj5);
    ...
    if (null != description) {
      ...
      let tmp21 = React7(tmp6(4636).Text, obj7);
    } else {
      tmp21 = null;
    ...
    items4[2] = React7(tmp6(4636).Text, obj11);
    ...
    if (tmp29Result) {
      ...
      obj13.children = tmp29(tmp6(5062).Button, obj14);
...
export const usePremiumPerkCard = function usePremiumPerkCard() {
  subscriptionPlansLoaded = subscriptionPlansLoaded(13509).useSubscriptionPlansLoaded();
  let obj = subscriptionPlansLoaded(13509);
  const maxFileSizeForPremiumType = subscriptionPlansLoaded(4297).getMaxFileSizeForPremiumType(PremiumTypes.TIER_2);
  const callback = noop.useCallback(() => {
    const obj2 = { screen: constants.COLLECTIBLES_SHOP, params: null };
    const obj = subscriptionPlansLoaded(7496);
    obj2.params = { analyticsSource: AnalyticsLocationDefault.PREMIUM_MARKETING_PERK_CARD };
    obj.openUserSettings(obj2);
  }, []);
  const callback1 = noop.useCallback(() => {
    subscriptionPlansLoaded(7496).openUserSettings({ screen: constants.PROFILE_CUSTOMIZATION });
  }, []);
  const items = [subscriptionPlansLoaded];
  const callback2 = noop.useCallback(() => {
    subscriptionPlansLoaded(7496).openUserSettings({ screen: constants.APPEARANCE_THEME_PICKER });
  }, []);
  ...
  const callback4 = noop.useCallback(() => {
    subscriptionPlansLoaded(7496).openUserSettings({ screen: constants.APP_ICONS });
  }, []);
  ...
  let obj2 = subscriptionPlansLoaded(4297);
  obj20.imageSrc = { uri: _modDef13526 };
  ...
  obj20.bodyComponent = closure_9(subscriptionPlansLoaded(4636).Text, obj22);
