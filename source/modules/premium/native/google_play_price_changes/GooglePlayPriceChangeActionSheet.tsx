...
// Dependencies: [19, 17, 4270, 17018, 1074, 1954, 21, 4606, 576, 504, 4264, 7307, 7223, 4602, 1114, 2024, 5026, 2]
// Exports: default

// Module 17017 (GooglePlayPriceChangeActionSheet)
...
export default function GooglePlayPriceChangeActionSheet(markAsDismissed) {
  ...
  const tierDisplayNameByPlanId = markAsDismissed(4264).getTierDisplayNameByPlanId(str);
  const tmp2Result = markAsDismissed(4264);
  const tmp2Result5 = markAsDismissed(4264);
  const intervalStringAsNoun = markAsDismissed(4264).getIntervalStringAsNoun(tmp2Result5.getInterval(str).intervalType);
  const tmp2Result6 = markAsDismissed(4264);
  const tmp2Result7 = markAsDismissed(7307);
  const formatPriceResult = markAsDismissed(7307).formatPrice(stateFromStores.oldPrice, stateFromStores.oldCurrency);
  const tmp2Result8 = markAsDismissed(7307);
  ...
  obj6.children = intl.format(markAsDismissed(1114).t.x0bFvn, { subscriptionName: tierDisplayNameByPlanId });
  const items2 = [closure_8(markAsDismissed(4602).Text, obj6), ];
  ...
  const formatPriceResult1 = markAsDismissed(7307).formatPrice(stateFromStores.newPrice, stateFromStores.newCurrency);
  ...
  obj7.children = intl2.format(markAsDismissed(1114).t["n+Hrjb"], obj8);
  items2[1] = closure_8(markAsDismissed(4602).Text, obj7);
  obj5.children = items2;
  const items3 = [closure_9(View, obj5), ];
  const obj9 = { variant: "primary", text: null, onPress: null };
  const intl3 = tmp2(1114).intl;
  obj9.text = intl3.string(markAsDismissed(1114).t.BddRzS);
  obj9.onPress = function onPress() {
    markAsDismissed(ContentDismissActionType.USER_DISMISS);
  };
  items3[1] = closure_8(markAsDismissed(5026).Button, obj9);
  obj4.children = items3;
  obj3.children = closure_9(View, obj4);
  return closure_8(markAsDismissed(7223).BottomSheet, obj3);
};
