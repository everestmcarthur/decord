...
// Dependencies: [19, 17, 4301, 17141, 1074, 1955, 21, 4638, 576, 504, 4295, 7344, 7260, 4634, 1115, 2025, 5060, 2]
// Exports: default

// Module 17140 (GooglePlayPriceChangeActionSheet)
...
export default function GooglePlayPriceChangeActionSheet(markAsDismissed) {
  ...
  const tierDisplayNameByPlanId = markAsDismissed(4295).getTierDisplayNameByPlanId(str);
  const tmp2Result = markAsDismissed(4295);
  const tmp2Result5 = markAsDismissed(4295);
  const intervalStringAsNoun = markAsDismissed(4295).getIntervalStringAsNoun(tmp2Result5.getInterval(str).intervalType);
  const tmp2Result6 = markAsDismissed(4295);
  const tmp2Result7 = markAsDismissed(7344);
  const formatPriceResult = markAsDismissed(7344).formatPrice(stateFromStores.oldPrice, stateFromStores.oldCurrency);
  const tmp2Result8 = markAsDismissed(7344);
  ...
  const items2 = [closure_8(markAsDismissed(4634).Text, obj6), ];
  ...
  const formatPriceResult1 = markAsDismissed(7344).formatPrice(stateFromStores.newPrice, stateFromStores.newCurrency);
  ...
  items2[1] = closure_8(markAsDismissed(4634).Text, obj7);
  ...
  items3[1] = closure_8(markAsDismissed(5060).Button, obj9);
