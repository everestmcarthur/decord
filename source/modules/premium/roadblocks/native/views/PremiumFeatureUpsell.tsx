...
// Dependencies: [19, 17, 1374, 1074, 7542, 21, 7967, 4295, 1115, 4638, 576, 9456, 10105, 7971, 9464, 7964, 10106, 1177, 10107, 10103, 4634, 8797, 5072, 1094, 5059, 5063, 7272, 9759, 8397, 4374, 1241, 10108, 2]
// Exports: default

// Module 10104 (PremiumFeatureUpsell)
...
function PremiumFeatureUpsellPill(featureName) {
  ...
  let loading;
  let obj = featureName(9456);
  const premiumUpsellConfig = obj.usePremiumUpsellConfig(featureName(10105).getUpsellType(featureName));
  const useTier0UpsellContent = premiumUpsellConfig.useTier0UpsellContent;
  const tmp4 = closure_13(useTier0UpsellContent);
  dependencyMap = tmp4;
  let obj2 = featureName(10105);
  let mobileEmojiPickerUpsellRestyleEnabledForFeature = featureName(7971).getMobileEmojiPickerUpsellRestyleEnabledForFeature(featureName, "native.PremiumFeatureUpsell");
  if (!mobileEmojiPickerUpsellRestyleEnabledForFeature) {
    mobileEmojiPickerUpsellRestyleEnabledForFeature = tmp(9464).getMobileStickerPickerUpsellRestyleEnabledForFeature(featureName, "native.PremiumFeatureUpsell");
    let tmpResult = tmp(9464);
  }
  ...
  const obj3 = featureName(7971);
  const premiumTypeDisplayName = featureName(4295).getPremiumTypeDisplayName(tmp7);
  if (featureName(7967).EntitlementFeatureNames.SOUNDBOARD_EVERYWHERE === featureName) {
    const intl5 = tmp(1115).intl;
    const obj4 = { nitroTierName: premiumTypeDisplayName, onClick: fn };
    let formatResult = intl5.format(tmp(1115).t["tw/SSq"], obj4);
  } else if (tmp(7967).EntitlementFeatureNames.EMOJIS_EVERYWHERE === featureName) {
    const intl4 = tmp(1115).intl;
    const obj5 = { nitroTierName: premiumTypeDisplayName, onClick: fn };
    formatResult = intl4.format(tmp(1115).t.gMVjeS, obj5);
  } else if (tmp(7967).EntitlementFeatureNames.STICKERS_EVERYWHERE === featureName) {
    const intl3 = tmp(1115).intl;
    const obj6 = { nitroTierName: premiumTypeDisplayName, onClick: fn };
    formatResult = intl3.format(tmp(1115).t.eontIh, obj6);
  } else if (tmp(7967).EntitlementFeatureNames.INCREASED_FILE_UPLOAD_SIZE === featureName) {
    const intl2 = tmp(1115).intl;
    const obj7 = { maxFileSize: tmp(4295).getMaxFileSizeForPremiumType(tmp7), nitroTierName: premiumTypeDisplayName, onClick: fn };
    formatResult = intl2.format(tmp(1115).t.zzyLEK, obj7);
    const tmpResult5 = tmp(4295);
  } else if (tmp(7967).EntitlementFeatureNames.STREAM_HIGH_QUALITY === featureName) {
    const intl = tmp(1115).intl;
    const obj8 = { nitroTierName: premiumTypeDisplayName, onClick: fn };
    formatResult = intl.format(tmp(1115).t.lyxfbj, obj8);
  } else if (tmp(7967).EntitlementFeatureNames.APP_ICONS === featureName) {
    const intl7 = tmp(1115).intl;
    const obj9 = { onClick: fn };
    formatResult = intl7.format(tmp(1115).t.x2dQxN, obj9);
  }
  const tmp10 = useTier0UpsellContent;
  const tmpResult4 = featureName(4295);
  const tmp11 = useTier0UpsellContent(10106);
  const tmp11Result = tmp11(useTier0UpsellContent, premiumUpsellConfig.onViewAllPerks, featureName(10105).getAnalyticsPage(featureName));
  ...
  if (mobileEmojiPickerUpsellRestyleEnabledForFeature) {
    ...
    items1[1] = closure_11(tmp(4634).Text, obj12);
    ...
    obj13.renderIcon = function renderIcon() {
      if (mobileEmojiPickerUpsellRestyleEnabledForFeature) {
        ...
        obj2.style = items;
        let tmpResult = tmp(tmp2(8797).NitroWheelIcon, obj2);
      } else {
    ...
    const obj14 = { source: tmp10(useTier0UpsellContent ? 10107 : 10103), style: tmp4.nitroWheel, disableColor: true };
...
animationEnterExit.__closure = { withSpring: fn(5059).withSpring, springStandard: fn(5063).springStandard };
...
export default function PremiumFeatureUpsell(shouldShow) {
  ...
  const effect = _location.useEffect(() => {
    ...
    if (tmp2) {
      ...
      } else if (tmp6(7967).EntitlementFeatureNames.EMOJIS_EVERYWHERE === featureName) {
        APP_ICON_INLINE_UPSELL = constants.EMOJI_EVERYWHERE_INLINE_UPSELL;
      } else if (tmp6(7967).EntitlementFeatureNames.STICKERS_EVERYWHERE === featureName) {
        APP_ICON_INLINE_UPSELL = constants.STICKERS_EVERYWHERE_INLINE_UPSELL;
      } else {
        if (tmp6(7967).EntitlementFeatureNames.INCREASED_FILE_UPLOAD_SIZE === featureName) {
          APP_ICON_INLINE_UPSELL = constants.LARGER_FILE_UPLOAD_INLINE_UPSELL;
        } else if (tmp6(7967).EntitlementFeatureNames.APP_ICONS !== featureName) {
          if (tmp6(7967).EntitlementFeatureNames.STREAM_HIGH_QUALITY === featureName) {
