...
// Dependencies: [4621, 4627, 4644, 1074, 1090, 4657, 17931, 9178, 9179, 17928, 2]

// Module 17934 (AVErrorStreamSendLowFPS)
          ...
          if (null == rTCConnection) {
            ...
            if (null == mediaEngineConnectionId) {
              ...
              if (rTCConnection.hasActiveRemoteWants()) {
                const participant = ChannelRTCStore.getParticipant(currentUserActiveStream.channelId, tmp11(4657).encodeStreamKey(currentUserActiveStream));
                if (null == participant) {
                  return null;
                } else {
                  const accumulatedStatsWithMinDatapoints = tmp11(17931).getAccumulatedStatsWithMinDatapoints(mediaEngineConnectionId, currentUserActiveStream.ownerId);
                  if (null == accumulatedStatsWithMinDatapoints) {
                    return null;
                  } else {
                    const maxQuality = tmp11(9178).getMaxQuality(participant);
                    let tmp9 = null;
                    if (null != maxQuality) {
                      if (accumulatedStatsWithMinDatapoints.short.frameRate < tmp11Result9.getWarningFrameRate(maxQuality.maxFrameRate)) {
                        const obj2 = { type: tmp11(9179).AVError.STREAM_SEND_LOW_FPS };
                        const tmp11Result10 = tmp11(17928);
                        const merged = Object.assign(tmp11Result10.getStreamErrorContext(tmp11(4657).encodeStreamKey(currentUserActiveStream)));
                        const items = [obj2];
                        let tmp6 = items;
                        const tmp11Result11 = tmp11(4657);
                      } else {
                        tmp6 = null;
                        const tmp11Result12 = tmp11(17931);
                      }
                      tmp9 = tmp6;
                      tmp11Result9 = tmp11(17931);
                    }
                    return tmp9;
                  }
                  const tmp11Result7 = tmp11(17931);
                }
                const tmp11Result = tmp11(4657);
              } else {
                return null;
              }
            }
