...
// Dependencies: [19, 4246, 4247, 7379, 502, 1979, 4621, 1074, 1964, 21, 10311, 10332, 1114, 10333, 10335, 7041, 7183, 4547, 7837, 9867, 4569, 4559, 10337, 5146, 10339, 10340, 11, 7255, 7453, 8716, 11344, 4573, 11342, 1896, 4549, 11346, 9149, 10233, 11054, 11406, 11046, 4950, 4564, 7529, 1178, 10718, 7262, 4304, 504, 7377, 7342, 7959, 1935, 4759, 11067, 5635, 7270, 1608, 11093, 7272, 2]
// Exports: default

// Module 10310 (LongPressForumPostActionSheet)
...
import buildFavoritesSectionButtonsDefault from "buildFavoritesSectionButtons" /* 10311 */;
...
import useFavoritesGuildChannelActionsDefault from "useFavoritesGuildChannelActions" /* 11067 */;
...
export default function ForumPostLongPressActionSheet(thread) {
  ...
  const canMarkChannelUnread = thread(10335).useCanMarkChannelUnread(thread);
  const obj5 = thread(10335);
  ...
  const firstMessage = thread(7377).useFirstForumPostMessage(thread).firstMessage;
  const obj7 = thread(7377);
  const isThreadModerator = thread(7342).useIsThreadModerator(parentChannel);
  const obj8 = thread(7342);
  const canManageThread = thread(7342).useCanManageThread(thread);
  const obj9 = thread(7342);
  const canUnarchiveThread = thread(7342).useCanUnarchiveThread(thread);
  const obj10 = thread(7342);
  const existingPin = thread(7959).useExistingPin(thread);
  const obj11 = thread(7959);
  ...
  if (null != stateFromStores) {
    const obj14 = { guild: stateFromStores, size: tmp(5635).GuildIconSizes.LARGE };
    let tmp21 = jsx(tmp17(5635), { guild: stateFromStores, size: tmp(5635).GuildIconSizes.LARGE });
    let tmp20 = jsx;
    const tmp17Result = tmp17(5635);
  } else {
    tmp20 = jsx;
  ...
  const MarkChannelUnreadExperiment = tmp(10332).MarkChannelUnreadExperiment;
  if (MarkChannelUnreadExperiment.getConfig({ location: "forum_post_action_sheet" }).enabled) {
    if (!stateFromStores3) {
      if (canMarkChannelUnread) {
        ...
        obj18.IconComponent = tmp(10333).ChatMarkUnreadIcon;
      ...
      if (!stateFromStores4) {
        ...
        if (stateFromStores1) {
          ...
          obj20.IconComponent = tmp(4547).UserMinusIcon;
          ...
          obj20.IconComponent = tmp(9867).BellIcon;
          obj20.onPress = function onPress() {
      ...
      if (archived) {
        if (canUnarchiveThread) {
          ...
          obj21.IconComponent = tmp(4569).ClockIcon;
        ...
        obj22.IconComponent = tmp(4559).XLargeIcon;
      ...
      if (canManageThread) {
        ...
        if (locked) {
          ...
          obj23.IconComponent = tmp(10337).LockUnlockedIcon;
          ...
          obj23.IconComponent = tmp(5146).LockIcon;
          obj23.onPress = function onPress() {
      ...
      if (tmp38) {
        ...
        obj24.IconComponent = tmp(10339).PencilIcon;
      ...
      if (canManageThread) {
        ...
        obj25.IconComponent = tmp(7453).SettingsIcon;
        ...
        if (parentChannel.availableTags.length > 0) {
          ...
          obj26.IconComponent = tmp(11344).TagsIcon;
          obj26.onPress = function onPress() {
            ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(11342, dependencyMap.paths), "ForumPostTagsActionSheet", { thread, parentChannel, canManageThread });
          };
          buttons7.push(obj26);
        }
      }
      ...
      obj27.IconComponent = tmp(4549).LinkIcon;
      ...
      if (stateFromStores4) {
        ...
        if (isThreadModerator) {
          ...
          if (hasFlagResult) {
            ...
            obj29.IconComponent = tmp(11046).PinIcon;
        ...
        if (!isThreadModerator) {
          if (!tmp24) {
            ...
            if (setting) {
              ...
              obj31.IconComponent = tmp(10718).IdIcon;
            ...
            obj32.header = tmp20(tmp(11093).ActionSheetIconHeader, obj33);
            obj32.children = items7.map((buttons) => {
              ...
              return jsx(ActionSheetRow.ActionSheetRow.Group, {
                ...
                children: buttons.map((item, index) => {
                  ...
                  return closure_1_14(thread(7272).ActionSheetRow, {
                    variant: str,
                    icon: closure_1_14(thread(7272).ActionSheetRow.Icon, { IconComponent, disableColor }),
                    label,
                    trailing,
                    onPress() {
                      closure_1_0();
            ...
            return tmp20(tmp(7270).ActionSheet, obj32);
          }
        }
        if (tmp24) {
        ...
        const obj34 = {
          label: string5Result,
          IconComponent: tmp(4564).TrashIcon,
          onPress() {
                  const intl = util.intl;
        ...
        if (stateFromStores2) {
          ...
          obj36.IconComponent = tmp(9867).BellIcon;
          ...
          obj36.IconComponent = tmp(9149).BellSlashIcon;
          obj36.onPress = function onPress() {
            ...
            obj.openLazy(asyncRequireImpl(10233, dependencyMap.paths), combined, obj2);
          };
          push3(obj36);
        }
        ...
        obj37.IconComponent = tmp(11054).ChannelNotificationIcon;
  ...
  obj38.IconComponent = tmp(7041).EyeIcon;
