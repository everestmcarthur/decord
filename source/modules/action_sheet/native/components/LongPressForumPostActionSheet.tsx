...
// Dependencies: [19, 4277, 4278, 7416, 502, 1980, 4653, 1074, 1965, 21, 10361, 10382, 1115, 10383, 10385, 7078, 7220, 4578, 7879, 9916, 4600, 4590, 10387, 5180, 10389, 10390, 11, 7292, 7490, 8760, 11396, 4605, 11394, 1897, 4580, 11398, 9202, 10282, 11112, 11458, 11104, 4982, 4595, 7566, 1177, 10770, 7299, 4335, 504, 7414, 7379, 8001, 1936, 4791, 11125, 5670, 7307, 1609, 11151, 7309, 2]
// Exports: default

// Module 10360 (LongPressForumPostActionSheet)
...
import buildFavoritesSectionButtonsDefault from "buildFavoritesSectionButtons" /* 10361 */;
import markChannelUnreadDefault from "markChannelUnread" /* 10385 */;
import ForumComposerModalActionCreators from "ForumComposerModalActionCreators" /* 10390 */;
import useFavoritesGuildChannelActionsDefault from "useFavoritesGuildChannelActions" /* 11125 */;
...
export default function ForumPostLongPressActionSheet(thread) {
  ...
  const canMarkChannelUnread = thread(10385).useCanMarkChannelUnread(thread);
  const obj5 = thread(10385);
  ...
  const firstMessage = thread(7414).useFirstForumPostMessage(thread).firstMessage;
  const obj7 = thread(7414);
  const isThreadModerator = thread(7379).useIsThreadModerator(parentChannel);
  const obj8 = thread(7379);
  const canManageThread = thread(7379).useCanManageThread(thread);
  const obj9 = thread(7379);
  const canUnarchiveThread = thread(7379).useCanUnarchiveThread(thread);
  const obj10 = thread(7379);
  const existingPin = thread(8001).useExistingPin(thread);
  const obj11 = thread(8001);
  ...
  if (null != stateFromStores) {
    const obj14 = { guild: stateFromStores, size: tmp(5670).GuildIconSizes.LARGE };
    let tmp21 = jsx(tmp17(5670), { guild: stateFromStores, size: tmp(5670).GuildIconSizes.LARGE });
    let tmp20 = jsx;
    const tmp17Result = tmp17(5670);
  } else {
    tmp20 = jsx;
  ...
  const MarkChannelUnreadExperiment = tmp(10382).MarkChannelUnreadExperiment;
  if (MarkChannelUnreadExperiment.getConfig({ location: "forum_post_action_sheet" }).enabled) {
    if (!stateFromStores3) {
      if (canMarkChannelUnread) {
        ...
        obj18.IconComponent = tmp(10383).ChatMarkUnreadIcon;
      ...
      if (!stateFromStores4) {
        ...
        if (stateFromStores1) {
          ...
          obj20.IconComponent = tmp(4578).UserMinusIcon;
          ...
          obj20.IconComponent = tmp(9916).BellIcon;
          obj20.onPress = function onPress() {
      ...
      if (archived) {
        if (canUnarchiveThread) {
          ...
          obj21.IconComponent = tmp(4600).ClockIcon;
        ...
        obj22.IconComponent = tmp(4590).XLargeIcon;
      ...
      if (canManageThread) {
        ...
        if (locked) {
          ...
          obj23.IconComponent = tmp(10387).LockUnlockedIcon;
          ...
          obj23.IconComponent = tmp(5180).LockIcon;
          obj23.onPress = function onPress() {
      ...
      if (tmp38) {
        ...
        obj24.IconComponent = tmp(10389).PencilIcon;
      ...
      if (canManageThread) {
        ...
        obj25.IconComponent = tmp(7490).SettingsIcon;
        ...
        if (parentChannel.availableTags.length > 0) {
          ...
          obj26.IconComponent = tmp(11396).TagsIcon;
          obj26.onPress = function onPress() {
            ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(11394, dependencyMap.paths), "ForumPostTagsActionSheet", { thread, parentChannel, canManageThread });
          };
          buttons7.push(obj26);
        }
      }
      ...
      obj27.IconComponent = tmp(4580).LinkIcon;
      ...
      if (stateFromStores4) {
        ...
        if (isThreadModerator) {
          ...
          if (hasFlagResult) {
            ...
            obj29.IconComponent = tmp(11104).PinIcon;
        ...
        if (!isThreadModerator) {
          if (!tmp24) {
            ...
            if (setting) {
              ...
              obj31.IconComponent = tmp(10770).IdIcon;
            ...
            obj32.header = tmp20(tmp(11151).ActionSheetIconHeader, obj33);
            obj32.children = items7.map((buttons) => {
              ...
              return jsx(ActionSheetRow.ActionSheetRow.Group, {
                ...
                children: buttons.map((item, index) => {
                  ...
                  return closure_1_14(thread(7309).ActionSheetRow, {
                    variant: str,
                    icon: closure_1_14(thread(7309).ActionSheetRow.Icon, { IconComponent, disableColor }),
                    label,
                    trailing,
                    onPress() {
                      closure_1_0();
            ...
            return tmp20(tmp(7307).ActionSheet, obj32);
          }
        }
        if (tmp24) {
        ...
        const obj34 = {
          label: string5Result,
          IconComponent: tmp(4595).TrashIcon,
          onPress() {
                  const intl = util.intl;
        ...
        if (stateFromStores2) {
          ...
          obj36.IconComponent = tmp(9916).BellIcon;
          ...
          obj36.IconComponent = tmp(9202).BellSlashIcon;
          obj36.onPress = function onPress() {
            ...
            obj.openLazy(asyncRequireImpl(10282, dependencyMap.paths), combined, obj2);
          };
          push3(obj36);
        }
        ...
        obj37.IconComponent = tmp(11112).ChannelNotificationIcon;
  ...
  obj38.IconComponent = tmp(7078).EyeIcon;
