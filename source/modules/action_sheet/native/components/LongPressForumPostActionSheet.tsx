...
// Dependencies: [19, 4279, 4280, 7413, 502, 1980, 4654, 1074, 1965, 21, 10353, 10374, 1115, 10375, 10377, 7075, 7217, 4580, 7870, 9908, 4602, 4592, 10379, 5179, 10381, 10382, 11, 7289, 7487, 8751, 11388, 4606, 11386, 1897, 4582, 11390, 9191, 10274, 11098, 11450, 11090, 4983, 4597, 7563, 1177, 10762, 7296, 4337, 504, 7411, 7376, 7992, 1936, 4792, 11111, 5669, 7304, 1609, 11137, 7306, 2]
// Exports: default

// Module 10352 (LongPressForumPostActionSheet)
...
import buildFavoritesSectionButtonsDefault from "buildFavoritesSectionButtons" /* 10353 */;
import markChannelUnreadDefault from "markChannelUnread" /* 10377 */;
import ForumComposerModalActionCreators from "ForumComposerModalActionCreators" /* 10382 */;
import useFavoritesGuildChannelActionsDefault from "useFavoritesGuildChannelActions" /* 11111 */;
...
export default function ForumPostLongPressActionSheet(thread) {
  ...
  const canMarkChannelUnread = thread(10377).useCanMarkChannelUnread(thread);
  const obj5 = thread(10377);
  ...
  const firstMessage = thread(7411).useFirstForumPostMessage(thread).firstMessage;
  const obj7 = thread(7411);
  const isThreadModerator = thread(7376).useIsThreadModerator(parentChannel);
  const obj8 = thread(7376);
  const canManageThread = thread(7376).useCanManageThread(thread);
  const obj9 = thread(7376);
  const canUnarchiveThread = thread(7376).useCanUnarchiveThread(thread);
  const obj10 = thread(7376);
  const existingPin = thread(7992).useExistingPin(thread);
  const obj11 = thread(7992);
  ...
  if (null != stateFromStores) {
    const obj14 = { guild: stateFromStores, size: tmp(5669).GuildIconSizes.LARGE };
    let tmp21 = jsx(tmp17(5669), { guild: stateFromStores, size: tmp(5669).GuildIconSizes.LARGE });
    let tmp20 = jsx;
    const tmp17Result = tmp17(5669);
  } else {
    tmp20 = jsx;
  ...
  const MarkChannelUnreadExperiment = tmp(10374).MarkChannelUnreadExperiment;
  if (MarkChannelUnreadExperiment.getConfig({ location: "forum_post_action_sheet" }).enabled) {
    if (!stateFromStores3) {
      if (canMarkChannelUnread) {
        ...
        obj18.IconComponent = tmp(10375).ChatMarkUnreadIcon;
      ...
      if (!stateFromStores4) {
        ...
        if (stateFromStores1) {
          ...
          obj20.IconComponent = tmp(9908).BellIcon;
          obj20.onPress = function onPress() {
      ...
      if (canManageThread) {
        ...
        if (locked) {
          ...
          obj23.IconComponent = tmp(10379).LockUnlockedIcon;
          ...
          obj23.IconComponent = tmp(5179).LockIcon;
          obj23.onPress = function onPress() {
      ...
      if (tmp38) {
        ...
        obj24.IconComponent = tmp(10381).PencilIcon;
      ...
      if (canManageThread) {
        ...
        obj25.IconComponent = tmp(7487).SettingsIcon;
        ...
        if (parentChannel.availableTags.length > 0) {
          ...
          obj26.IconComponent = tmp(11388).TagsIcon;
          obj26.onPress = function onPress() {
            ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(11386, dependencyMap.paths), "ForumPostTagsActionSheet", { thread, parentChannel, canManageThread });
          };
          buttons7.push(obj26);
        }
      }
      ...
      if (stateFromStores4) {
        ...
        if (isThreadModerator) {
          ...
          if (hasFlagResult) {
            ...
            obj29.IconComponent = tmp(11090).PinIcon;
        ...
        if (!isThreadModerator) {
          if (!tmp24) {
            ...
            if (setting) {
              ...
              obj31.IconComponent = tmp(10762).IdIcon;
            ...
            obj32.header = tmp20(tmp(11137).ActionSheetIconHeader, obj33);
            obj32.children = items7.map((buttons) => {
              ...
              return jsx(ActionSheetRow.ActionSheetRow.Group, {
                ...
                children: buttons.map((item, index) => {
                  ...
                  return closure_1_14(thread(7306).ActionSheetRow, {
                    variant: str,
                    icon: closure_1_14(thread(7306).ActionSheetRow.Icon, { IconComponent, disableColor }),
                    label,
                    trailing,
                    onPress() {
                      closure_1_0();
            ...
            return tmp20(tmp(7304).ActionSheet, obj32);
          }
        }
        if (tmp24) {
        ...
        if (stateFromStores2) {
          ...
          obj36.IconComponent = tmp(9908).BellIcon;
          ...
          obj36.IconComponent = tmp(9191).BellSlashIcon;
          obj36.onPress = function onPress() {
            ...
            obj.openLazy(asyncRequireImpl(10274, dependencyMap.paths), combined, obj2);
          };
          push3(obj36);
        }
        ...
        obj37.IconComponent = tmp(11098).ChannelNotificationIcon;
  ...
  obj38.IconComponent = tmp(7075).EyeIcon;
