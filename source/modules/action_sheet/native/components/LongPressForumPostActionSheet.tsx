...
// Dependencies: [19, 4276, 4277, 7409, 502, 1979, 4651, 1074, 1964, 21, 10351, 10372, 1114, 10373, 10375, 7071, 7213, 4577, 7867, 9906, 4599, 4589, 10377, 5176, 10379, 10380, 11, 7285, 7483, 8748, 11384, 4603, 11382, 1896, 4579, 11386, 9188, 10272, 11094, 11446, 11086, 4980, 4594, 7559, 1176, 10758, 7292, 4334, 504, 7407, 7372, 7989, 1935, 4789, 11107, 5665, 7300, 1608, 11133, 7302, 2]
// Exports: default

// Module 10350 (LongPressForumPostActionSheet)
...
import buildFavoritesSectionButtonsDefault from "buildFavoritesSectionButtons" /* 10351 */;
...
import useFavoritesGuildChannelActionsDefault from "useFavoritesGuildChannelActions" /* 11107 */;
...
export default function ForumPostLongPressActionSheet(thread) {
  ...
  const canMarkChannelUnread = thread(10375).useCanMarkChannelUnread(thread);
  const obj5 = thread(10375);
  ...
  const firstMessage = thread(7407).useFirstForumPostMessage(thread).firstMessage;
  const obj7 = thread(7407);
  const isThreadModerator = thread(7372).useIsThreadModerator(parentChannel);
  const obj8 = thread(7372);
  const canManageThread = thread(7372).useCanManageThread(thread);
  const obj9 = thread(7372);
  const canUnarchiveThread = thread(7372).useCanUnarchiveThread(thread);
  const obj10 = thread(7372);
  const existingPin = thread(7989).useExistingPin(thread);
  const obj11 = thread(7989);
  ...
  if (null != stateFromStores) {
    const obj14 = { guild: stateFromStores, size: tmp(5665).GuildIconSizes.LARGE };
    let tmp21 = jsx(tmp17(5665), { guild: stateFromStores, size: tmp(5665).GuildIconSizes.LARGE });
    let tmp20 = jsx;
    const tmp17Result = tmp17(5665);
  } else {
    tmp20 = jsx;
    const obj15 = { size: tmp(1176).AvatarSizes.LARGE, channel: thread };
    tmp21 = jsx(tmp(1176).Avatar, { size: tmp(1176).AvatarSizes.LARGE, channel: thread });
  }
  ...
  if (MarkChannelUnreadExperiment.getConfig({ location: "forum_post_action_sheet" }).enabled) {
    if (!stateFromStores3) {
      if (canMarkChannelUnread) {
        ...
        obj18.IconComponent = tmp(10373).ChatMarkUnreadIcon;
      ...
      if (!stateFromStores4) {
        ...
        if (stateFromStores1) {
          ...
          obj20.IconComponent = tmp(4577).UserMinusIcon;
      ...
      if (archived) {
        if (canUnarchiveThread) {
          ...
          obj21.IconComponent = tmp(4599).ClockIcon;
        ...
        obj22.IconComponent = tmp(4589).XLargeIcon;
      ...
      if (canManageThread) {
        ...
        if (locked) {
          ...
          obj23.IconComponent = tmp(10377).LockUnlockedIcon;
          ...
          obj23.IconComponent = tmp(5176).LockIcon;
          obj23.onPress = function onPress() {
      ...
      if (tmp38) {
        ...
        obj24.IconComponent = tmp(10379).PencilIcon;
      ...
      if (canManageThread) {
        ...
        obj25.IconComponent = tmp(7483).SettingsIcon;
        ...
        if (parentChannel.availableTags.length > 0) {
          ...
          obj26.IconComponent = tmp(11384).TagsIcon;
          obj26.onPress = function onPress() {
            ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(11382, dependencyMap.paths), "ForumPostTagsActionSheet", { thread, parentChannel, canManageThread });
          };
          buttons7.push(obj26);
        }
      }
      ...
      obj27.IconComponent = tmp(4579).LinkIcon;
      ...
      if (stateFromStores4) {
        ...
        if (isThreadModerator) {
          ...
          if (hasFlagResult) {
            ...
            obj29.IconComponent = tmp(11086).PinIcon;
        ...
        if (!isThreadModerator) {
          if (!tmp24) {
            ...
            if (setting) {
              ...
              obj31.IconComponent = tmp(10758).IdIcon;
            ...
            obj32.header = tmp20(tmp(11133).ActionSheetIconHeader, obj33);
            obj32.children = items7.map((buttons) => {
              ...
              return jsx(ActionSheetRow.ActionSheetRow.Group, {
                ...
                children: buttons.map((item, index) => {
                  ...
                  return closure_1_14(thread(7302).ActionSheetRow, {
                    variant: str,
                    icon: closure_1_14(thread(7302).ActionSheetRow.Icon, { IconComponent, disableColor }),
                    label,
                    trailing,
                    onPress() {
                      closure_1_0();
            ...
            return tmp20(tmp(7300).ActionSheet, obj32);
          }
        }
        if (tmp24) {
        ...
        const obj34 = {
          label: string5Result,
          IconComponent: tmp(4594).TrashIcon,
          onPress() {
                  const intl = util.intl;
        ...
        if (stateFromStores2) {
          ...
          obj36.IconComponent = tmp(9188).BellSlashIcon;
          obj36.onPress = function onPress() {
        ...
        obj37.IconComponent = tmp(11094).ChannelNotificationIcon;
  ...
  obj38.IconComponent = tmp(7071).EyeIcon;
