...
// Dependencies: [19, 4246, 4247, 7380, 502, 1979, 4622, 1074, 1964, 21, 10313, 10334, 1114, 10335, 10337, 7042, 7184, 4548, 7838, 9869, 4570, 4560, 10339, 5147, 10341, 10342, 11, 7256, 7454, 8718, 11346, 4574, 11344, 1896, 4550, 11348, 9151, 10235, 11056, 11408, 11048, 4951, 4565, 7530, 1178, 10720, 7263, 4304, 504, 7378, 7343, 7960, 1935, 4760, 11069, 5636, 7271, 1608, 11095, 7273, 2]
// Exports: default

// Module 10312 (LongPressForumPostActionSheet)
...
import buildFavoritesSectionButtonsDefault from "buildFavoritesSectionButtons" /* 10313 */;
import markChannelUnreadDefault from "markChannelUnread" /* 10337 */;
import ForumComposerModalActionCreators from "ForumComposerModalActionCreators" /* 10342 */;
import useFavoritesGuildChannelActionsDefault from "useFavoritesGuildChannelActions" /* 11069 */;
...
export default function ForumPostLongPressActionSheet(thread) {
  ...
  const canMarkChannelUnread = thread(10337).useCanMarkChannelUnread(thread);
  const obj5 = thread(10337);
  ...
  const firstMessage = thread(7378).useFirstForumPostMessage(thread).firstMessage;
  const obj7 = thread(7378);
  const isThreadModerator = thread(7343).useIsThreadModerator(parentChannel);
  const obj8 = thread(7343);
  const canManageThread = thread(7343).useCanManageThread(thread);
  const obj9 = thread(7343);
  const canUnarchiveThread = thread(7343).useCanUnarchiveThread(thread);
  const obj10 = thread(7343);
  const existingPin = thread(7960).useExistingPin(thread);
  const obj11 = thread(7960);
  ...
  if (null != stateFromStores) {
    const obj14 = { guild: stateFromStores, size: tmp(5636).GuildIconSizes.LARGE };
    let tmp21 = jsx(tmp17(5636), { guild: stateFromStores, size: tmp(5636).GuildIconSizes.LARGE });
    let tmp20 = jsx;
    const tmp17Result = tmp17(5636);
  } else {
    tmp20 = jsx;
    const obj15 = { size: tmp(1178).AvatarSizes.LARGE, channel: thread };
  ...
  const MarkChannelUnreadExperiment = tmp(10334).MarkChannelUnreadExperiment;
  if (MarkChannelUnreadExperiment.getConfig({ location: "forum_post_action_sheet" }).enabled) {
    if (!stateFromStores3) {
      if (canMarkChannelUnread) {
        ...
        obj18.label = intl.string(tmp(1114).t.RpE9k7);
        obj18.IconComponent = tmp(10335).ChatMarkUnreadIcon;
      ...
      if (!stateFromStores4) {
        ...
        if (stateFromStores1) {
          ...
          obj20.IconComponent = tmp(4548).UserMinusIcon;
          ...
          obj20.IconComponent = tmp(9869).BellIcon;
          obj20.onPress = function onPress() {
      ...
      if (archived) {
        if (canUnarchiveThread) {
          ...
          obj21.label = intl5.string(tmp(1114).t.cnRubV);
          obj21.IconComponent = tmp(4570).ClockIcon;
        ...
        obj22.label = intl4.string(tmp(1114).t.BTs4Kb);
        obj22.IconComponent = tmp(4560).XLargeIcon;
      ...
      if (canManageThread) {
        ...
        if (locked) {
          ...
          obj23.IconComponent = tmp(10339).LockUnlockedIcon;
          ...
          obj23.IconComponent = tmp(5147).LockIcon;
          obj23.onPress = function onPress() {
      ...
      if (tmp38) {
        ...
        obj24.label = intl7.string(tmp(1114).t.NP1yHG);
        obj24.IconComponent = tmp(10341).PencilIcon;
      ...
      if (canManageThread) {
        ...
        obj25.label = intl8.string(tmp(1114).t.SGuVbR);
        obj25.IconComponent = tmp(7454).SettingsIcon;
        ...
        if (parentChannel.availableTags.length > 0) {
          ...
          obj26.label = intl9.string(tmp(1114).t["436ZFw"]);
          obj26.IconComponent = tmp(11346).TagsIcon;
          obj26.onPress = function onPress() {
            ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(11344, dependencyMap.paths), "ForumPostTagsActionSheet", { thread, parentChannel, canManageThread });
          };
          buttons7.push(obj26);
        }
      }
      ...
      obj27.label = intl10.string(tmp(1114).t.WqhZss);
      obj27.IconComponent = tmp(4550).LinkIcon;
      ...
      if (stateFromStores4) {
        ...
        if (isThreadModerator) {
          ...
          if (hasFlagResult) {
            ...
            obj29.IconComponent = tmp(11048).PinIcon;
        ...
        if (!isThreadModerator) {
          if (!tmp24) {
            ...
            if (setting) {
              ...
              obj31.label = intl14.string(tmp(1114).t.DQ797g);
              obj31.IconComponent = tmp(10720).IdIcon;
            ...
            const obj32 = { showGradient: true, startExpanded: tmp(1608).isMetaQuest(), header: null, children: null };
            const obj33 = { title: tmp18, icon: tmp21 };
            obj32.header = tmp20(tmp(11095).ActionSheetIconHeader, obj33);
            obj32.children = items7.map((buttons) => {
              ...
              return jsx(ActionSheetRow.ActionSheetRow.Group, {
                ...
                children: buttons.map((item, index) => {
                  ...
                  return closure_1_14(thread(7273).ActionSheetRow, {
                    variant: str,
                    icon: closure_1_14(thread(7273).ActionSheetRow.Icon, { IconComponent, disableColor }),
                    label,
                    trailing,
                    onPress() {
                      closure_1_0();
            ...
            return tmp20(tmp(7271).ActionSheet, obj32);
          }
        }
        if (tmp24) {
        ...
        const obj34 = {
          label: string5Result,
          IconComponent: tmp(4565).TrashIcon,
          onPress() {
                  const intl = util.intl;
        ...
        if (stateFromStores2) {
          ...
          obj36.IconComponent = tmp(9869).BellIcon;
          ...
          obj36.IconComponent = tmp(9151).BellSlashIcon;
          obj36.onPress = function onPress() {
            ...
            obj.openLazy(asyncRequireImpl(10235, dependencyMap.paths), combined, obj2);
          };
          push3(obj36);
        }
        ...
        obj37.label = string3(tmp(1114).t.HcoRu0);
        obj37.IconComponent = tmp(11056).ChannelNotificationIcon;
  ...
  obj38.label = intl2.string(tmp(1114).t.e6RscS);
  obj38.IconComponent = tmp(7042).EyeIcon;
