...
export const getSecurityMessage = function getSecurityMessage(scopes) {
  if (!scopes.includes(OAuth2Scopes.OAuth2Scopes.DM_CHANNELS_MESSAGES_WRITE)) {
    if (!scopes.includes(tmp(8425).OAuth2Scopes.SDK_SOCIAL_LAYER)) {
      if (!scopes.includes(tmp(8425).OAuth2Scopes.MESSAGES_READ)) {
        if (!scopes.includes(tmp(8425).OAuth2Scopes.DM_CHANNELS_MESSAGES_READ)) {
