...
// Dependencies: [19, 17, 21, 4722, 576, 9478, 9128, 9669, 11, 9476, 4664, 1115, 5264, 9480, 9671, 4684, 8571, 9673, 9640, 4718, 2]
// Exports: default

// Module 9668 (ApplicationDetails)
...
import disclosures from "disclosures" /* 9478 */;
import Utils from "Utils" /* 9480 */;
...
export default function ApplicationDetails(arg0) {
  ...
  let tmp15 = null;
  if (null != joined) {
    let obj4 = { iconComponent: tmp5(4664).LinkIcon, text: null };
  ...
  if (null != connectedAccount) {
    const obj7 = { iconComponent: tmp5(9671).HammerIcon, text: null };
  ...
  if (scopes.includes(OAuth2Scopes.OAuth2Scopes.BOT)) {
    ...
    if (null != approximateGuildCount) {
      const obj9 = { iconComponent: tmp5(9673).RobotIcon, text: null };
  ...
  if (null != disclosures) {
    mapped = disclosures.map((toFixed) => {
      ...
      if (disclosures.ApplicationDisclosure.IP_LOCATION === toFixed) {
        const obj2 = { iconComponent: tmp(9128).GlobeEarthIcon };
        let tmp4 = obj2;
      } else {
        tmp4 = null;
        if (tmp(9478).ApplicationDisclosure.DISPLAYS_ADVERTISEMENTS === toFixed) {
          const obj3 = { iconComponent: tmp(9669).EmbedIcon };
          tmp4 = obj3;
        }
      }
      let tmp5 = null;
