...
// Dependencies: [19, 17, 21, 4574, 576, 9272, 8918, 9463, 11, 9270, 4517, 1114, 5109, 9274, 9465, 4537, 8367, 9467, 9434, 4570, 2]
// Exports: default

// Module 9462 (ApplicationDetails)
...
import scopes from "scopes" /* 9270 */;
import disclosures from "disclosures" /* 9272 */;
import Utils from "Utils" /* 9274 */;
...
export default function ApplicationDetails(arg0) {
  ...
  let tmp15 = null;
  if (null != joined) {
    let obj4 = { iconComponent: tmp5(4517).LinkIcon, text: null };
  ...
  if (null != connectedAccount) {
    const obj7 = { iconComponent: tmp5(9465).HammerIcon, text: null };
  ...
  if (scopes.includes(OAuth2Scopes.OAuth2Scopes.BOT)) {
    ...
    if (null != approximateGuildCount) {
      const obj9 = { iconComponent: tmp5(9467).RobotIcon, text: null };
  ...
  if (null != disclosures) {
    mapped = disclosures.map((toFixed) => {
      ...
      if (disclosures.ApplicationDisclosure.IP_LOCATION === toFixed) {
        const obj2 = { iconComponent: tmp(8918).GlobeEarthIcon };
        let tmp4 = obj2;
      } else {
        tmp4 = null;
        if (tmp(9272).ApplicationDisclosure.DISPLAYS_ADVERTISEMENTS === toFixed) {
          const obj3 = { iconComponent: tmp(9463).EmbedIcon };
          tmp4 = obj3;
        }
      }
      let tmp5 = null;
