...
// Dependencies: [19, 17, 21, 4605, 576, 9309, 8955, 9501, 11, 9307, 4548, 1114, 5145, 9311, 9503, 4568, 8404, 9505, 9472, 4601, 2]
// Exports: default

// Module 9500 (ApplicationDetails)
...
import scopes from "scopes" /* 9307 */;
import disclosures from "disclosures" /* 9309 */;
import Utils from "Utils" /* 9311 */;
...
export default function ApplicationDetails(arg0) {
  ...
  let tmp15 = null;
  if (null != joined) {
    let obj4 = { iconComponent: tmp5(4548).LinkIcon, text: null };
  ...
  if (null != connectedAccount) {
    const obj7 = { iconComponent: tmp5(9503).HammerIcon, text: null };
  ...
  if (scopes.includes(OAuth2Scopes.OAuth2Scopes.BOT)) {
    ...
    if (null != approximateGuildCount) {
      const obj9 = { iconComponent: tmp5(9505).RobotIcon, text: null };
  ...
  if (null != disclosures) {
    mapped = disclosures.map((toFixed) => {
      ...
      if (disclosures.ApplicationDisclosure.IP_LOCATION === toFixed) {
        const obj2 = { iconComponent: tmp(8955).GlobeEarthIcon };
        let tmp4 = obj2;
      } else {
        tmp4 = null;
        if (tmp(9309).ApplicationDisclosure.DISPLAYS_ADVERTISEMENTS === toFixed) {
          const obj3 = { iconComponent: tmp(9501).EmbedIcon };
          tmp4 = obj3;
        }
      }
      let tmp5 = null;
