...
// Dependencies: [19, 17, 21, 4574, 1114, 6630, 576, 4534, 4570, 5612, 9270, 9277, 8367, 2]
// Exports: default

// Module 9456 (AuthorizeScopes)
...
function Scope(arg0) {
  ...
  if (isFake) {
    ...
    let tmp10Result = tmp10(tmp6(6630).CircleXIcon, obj3);
  } else {
    const obj4 = { color: nativeDefault.colors.TEXT_MUTED, size: "refresh_sm" };
    tmp10Result = tmp10(tmp6(4534).CircleCheckIcon, obj4);
  }
  ...
  const items1 = [React4(tmp6(4570).Text, { variant: "text-md/normal", color: str3, children: text }), ];
  let tmp10Result2 = null;
  if (null != error) {
    const obj6 = { variant: "text-xs/normal", children: error };
    tmp10Result2 = tmp10(tmp6(4570).Text, obj6);
  }
...
export default function AuthorizeScopes(accountScopes) {
  ...
  if (0 === accountScopes.length) {
    ...
    let hasItem = integrationType === tmp5(9277).ApplicationIntegrationType.USER_INSTALL;
    if (hasItem) {
      hasItem = requestedScopes.includes(tmp5(8367).OAuth2Scopes.APPLICATIONS_COMMANDS);
    }
    ...
    const items = [closure_4(tmp5(4570).Text, obj2), ];
    const obj4 = { style: tmp.scopes, children: null };
    const mapped = accountScopes.map((item) => {
      closure_0 = item;
      const scopeNames = accountScopes(9270).getScopeNames(item, closure_0);
