...
// Dependencies: [9665, 4809, 9339, 1896, 1365, 9667, 4571, 1093, 4301, 2]
// Exports: default
...
export default function authorizeCallback(arg0) {
  ...
  if (null != _location) {
      ...
      if (tmp8Result.isDiscordHostname(host)) {
        if (null != pathname) {
          if (null != pathname.match(re5)) {
            ...
            tmp8(4809).pushLazy(asyncRequireImpl(9667, tmp9.paths), obj3, React3);
          } else if (null != pathname.match(re6)) {
            if (!canceled) {
              let str1;
              const tmp8Result5 = tmp8(4809);
              ...
              tmp8Result5.pushLazy(asyncRequireImpl(9339, tmp9.paths), obj4, React4);
              const tmp17 = asyncRequireImpl(9339, tmp9.paths);
            }
          }
        }
      }
    ...
    ModalActionCreatorsDefault.pushLazy(asyncRequireImpl(9339, dependencyMap.paths), undefined, React4);
  }
};
