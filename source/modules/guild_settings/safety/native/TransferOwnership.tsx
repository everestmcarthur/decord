...
// Dependencies: [5, 32, 19, 17, 1372, 11989, 1074, 21, 4638, 576, 4339, 1484, 504, 38, 9206, 11987, 4335, 4790, 5670, 4634, 11991, 1177, 1115, 4482, 5773, 5690, 5060, 2]
// Exports: default

// Module 11990 (TransferOwnership)
...
const TransferOwnershipConstants = fn(11989);
...
export default function TransferOwnership(guild) {
          ...
          if (arg0 === 1) {
            ...
            if (!mfaEnabled.mfaEnabled) {
              if (null != tmp27.email) {
                v1 = 1;
                dependencyMap = 1;
                const obj6 = { value: v1(9206).sendTransferOwnershipPincode(user.id), done: false };
                return obj6;
              }
            }
            let MFA = null;
            ...
            const obj4 = v1(9206);
            v1(9206).transferOwnership(user.id, id.id, MFA).then(() => {
              v1(11987).close();
              const obj = v1(11987);
              v1(9206).close();
              const obj2 = v1(9206);
              const result = closure_1_0(4335).showTransferOwnershipSuccess();
            }, NOOP);
            dependencyMap = 3;
            const transferOwnershipResult = v1(9206).transferOwnership(user.id, id.id, MFA);
          }
        } else if (arg0 === 1) {
          dependencyMap = 3;
          throw value;
  ...
  const token = guild(4339).useToken(toUser(576).modules.mobile.TABLE_ROW_PADDING);
  let tmp5 = closure_16();
  let obj = guild(4339);
  ...
  nickname = toUser(4790).getNickname(guild.id, undefined, toUser);
  ...
  const items1 = [closure_13(c7, { source: toUser(11991), style: tmp5.arrow }), , , ];
  const obj10 = { style: tmp5.avatarsWrapper, children: null };
  const obj9 = { source: toUser(11991), style: tmp5.arrow };
  ...
  items1[2] = closure_13(guild(4634).Text, { variant: "text-xs/medium", color: "text-default", children: guild.name });
  if (null == nickname) {
    if (!hasAvatarForGuildResult) {
      const intl = tmp(1115).intl;
      const obj15 = { GuildHook: GuildWithSmallIcon, user: tmp3(4482).getUserTag(toUser) };
      let formatResult = intl.format(tmp(1115).t["2XLnG0"], obj15);
      const tmp3Result = tmp3(4482);
    }
    ...
    const obj19 = { username: tmp3(4482).getUserTag(toUser) };
    obj18.label = intl3.format(tmp(1115).t.xm6ACJ, obj19);
    obj18.onPress = function handleConfirmToggle(arg0) {
      _undefined(arg0);
    };
    obj17.children = tmp11(tmp(5690).TableCheckboxRow, obj18);
    items3[1] = tmp11(tmp(5773).TableRowGroup, obj17);
    const obj20 = {
    ...
    obj20.disabled = !tmp8;
    items3[2] = tmp11(tmp(5060).Button, obj20);
  ...
  obj21.user = toUser(4482).getUserTag(toUser);
