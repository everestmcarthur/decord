...
// Dependencies: [5, 32, 19, 17, 1372, 11981, 1074, 21, 4639, 576, 4341, 1484, 504, 38, 9195, 11979, 4337, 4791, 5669, 4635, 11983, 1177, 1115, 4484, 5772, 5689, 5059, 2]
// Exports: default

// Module 11982 (TransferOwnership)
...
const TransferOwnershipConstants = fn(11981);
...
export default function TransferOwnership(guild) {
          ...
          if (arg0 === 1) {
            ...
            if (!mfaEnabled.mfaEnabled) {
              if (null != tmp27.email) {
                v1 = 1;
                dependencyMap = 1;
                const obj6 = { value: v1(9195).sendTransferOwnershipPincode(user.id), done: false };
                return obj6;
              }
            }
            let MFA = null;
            ...
            const obj4 = v1(9195);
            v1(9195).transferOwnership(user.id, id.id, MFA).then(() => {
              v1(11979).close();
              const obj = v1(11979);
              v1(9195).close();
              const obj2 = v1(9195);
              const result = closure_1_0(4337).showTransferOwnershipSuccess();
            }, NOOP);
            dependencyMap = 3;
            const transferOwnershipResult = v1(9195).transferOwnership(user.id, id.id, MFA);
          }
        } else if (arg0 === 1) {
          dependencyMap = 3;
          throw value;
  ...
  const token = guild(4341).useToken(toUser(576).modules.mobile.TABLE_ROW_PADDING);
  let tmp5 = closure_16();
  let obj = guild(4341);
  ...
  nickname = toUser(4791).getNickname(guild.id, undefined, toUser);
  ...
  const items1 = [closure_13(c7, { source: toUser(11983), style: tmp5.arrow }), , , ];
  const obj10 = { style: tmp5.avatarsWrapper, children: null };
  const obj9 = { source: toUser(11983), style: tmp5.arrow };
  ...
  items1[2] = closure_13(guild(4635).Text, { variant: "text-xs/medium", color: "text-default", children: guild.name });
  if (null == nickname) {
    if (!hasAvatarForGuildResult) {
      const intl = tmp(1115).intl;
      const obj15 = { GuildHook: GuildWithSmallIcon, user: tmp3(4484).getUserTag(toUser) };
    ...
    const obj19 = { username: tmp3(4484).getUserTag(toUser) };
    obj18.label = intl3.format(tmp(1115).t.xm6ACJ, obj19);
    obj18.onPress = function handleConfirmToggle(arg0) {
      _undefined(arg0);
    };
    obj17.children = tmp11(tmp(5689).TableCheckboxRow, obj18);
    items3[1] = tmp11(tmp(5772).TableRowGroup, obj17);
    const obj20 = {
    ...
    obj20.disabled = !tmp8;
    items3[2] = tmp11(tmp(5059).Button, obj20);
  ...
  obj21.user = toUser(4484).getUserTag(toUser);
