...
// Dependencies: [5, 32, 19, 17, 1371, 11917, 1074, 21, 4605, 576, 4306, 1483, 504, 38, 9132, 11915, 4302, 4757, 5634, 4601, 11919, 1178, 1114, 4449, 5737, 5654, 5025, 2]
// Exports: default

// Module 11918 (TransferOwnership)
...
const TransferOwnershipConstants = fn(11917);
...
export default function TransferOwnership(guild) {
          ...
          if (arg0 === 1) {
            ...
            if (!mfaEnabled.mfaEnabled) {
              if (null != tmp27.email) {
                v1 = 1;
                dependencyMap = 1;
                const obj6 = { value: v1(9132).sendTransferOwnershipPincode(user.id), done: false };
                return obj6;
              }
            }
            let MFA = null;
            ...
            const obj4 = v1(9132);
            v1(9132).transferOwnership(user.id, id.id, MFA).then(() => {
              v1(11915).close();
              const obj = v1(11915);
              v1(9132).close();
              const obj2 = v1(9132);
              const result = closure_1_0(4302).showTransferOwnershipSuccess();
            }, NOOP);
            dependencyMap = 3;
            const transferOwnershipResult = v1(9132).transferOwnership(user.id, id.id, MFA);
          }
        } else if (arg0 === 1) {
          dependencyMap = 3;
          throw value;
  ...
  const token = guild(4306).useToken(toUser(576).modules.mobile.TABLE_ROW_PADDING);
  let tmp5 = closure_16();
  let obj = guild(4306);
  ...
  nickname = toUser(4757).getNickname(guild.id, undefined, toUser);
  ...
  const items1 = [closure_13(c7, { source: toUser(11919), style: tmp5.arrow }), , , ];
  const obj10 = { style: tmp5.avatarsWrapper, children: null };
  const obj9 = { source: toUser(11919), style: tmp5.arrow };
  ...
  items1[2] = closure_13(guild(4601).Text, { variant: "text-xs/medium", color: "text-default", children: guild.name });
  if (null == nickname) {
    if (!hasAvatarForGuildResult) {
      const intl = tmp(1114).intl;
      const obj15 = { GuildHook: GuildWithSmallIcon, user: tmp3(4449).getUserTag(toUser) };
      let formatResult = intl.format(tmp(1114).t["2XLnG0"], obj15);
      const tmp3Result = tmp3(4449);
    }
    ...
    const obj19 = { username: tmp3(4449).getUserTag(toUser) };
    obj18.label = intl3.format(tmp(1114).t.xm6ACJ, obj19);
    obj18.onPress = function handleConfirmToggle(arg0) {
      _undefined(arg0);
    };
    obj17.children = tmp11(tmp(5654).TableCheckboxRow, obj18);
    items3[1] = tmp11(tmp(5737).TableRowGroup, obj17);
    const obj20 = {
    ...
    obj20.text = intl4.string(tmp(1114).t.jqqLb6);
    obj20.disabled = !tmp8;
    items3[2] = tmp11(tmp(5025).Button, obj20);
  ...
  obj21.user = toUser(4449).getUserTag(toUser);
  ...
  formatResult = intl2.format(tmp(1114).t.E90vgp, obj21);
};
