...
// Dependencies: [19, 17, 9193, 1074, 21, 4636, 576, 17674, 17714, 5364, 1396, 4488, 5768, 5686, 7303, 1114, 9192, 4981, 5075, 4338, 1483, 504, 4571, 5705, 7480, 7485, 2024, 8716, 5054, 4632, 7143, 2]
// Exports: default

// Module 17713 (GuildSettingsModalIntegrationPlatform)
...
import GuildSettingsModalIntegrations from "GuildSettingsModalIntegrations" /* 17674 */;
...
IntegrationItem.prototype["render"] = function render() {
  ...
  if (SUPPORTED_SETTINGS_INTEGRATION_PLATFORMS.includes(integration.type)) {
    ...
    if (tmp(17714).IntegrationTypes.YOUTUBE === type) {
      ...
      let combined = name;
    } else if (tmp(17714).IntegrationTypes.TWITCH === type) {
    ...
    if (null == value) {
      ...
      const items = [closure_1_11(tmp(5686).TableRow, obj2), ];
      ...
      items[1] = closure_1_11(tmp(7303).TableSwitchRow, obj5);
      obj4.children = items;
      return closure_1_12(tmp(5768).TableRowGroup, obj4);
    } else {
      ...
      icon.source = tmpResult.makeSource(tmp(4488).isThemeDark(props.theme) ? icon.darkPNG : icon.lightPNG);
      icon.style = styles.platformIcon;
      tmp12(tmp13, icon);
      const tmp14 = tmp(4488).isThemeDark(props.theme) ? icon.darkPNG : icon.lightPNG;
    }
  } else {
    return null;
  }
