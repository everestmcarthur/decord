...
// Dependencies: [19, 17, 9218, 1074, 21, 4640, 576, 17810, 17850, 5371, 1397, 4491, 5775, 5693, 7314, 1115, 9217, 4985, 5081, 4341, 1484, 504, 4574, 5712, 7491, 7496, 2025, 8748, 5060, 4636, 7154, 2]
// Exports: default

// Module 17849 (GuildSettingsModalIntegrationPlatform)
...
import GuildSettingsModalIntegrations from "GuildSettingsModalIntegrations" /* 17810 */;
...
IntegrationItem.prototype["render"] = function render() {
  ...
  if (SUPPORTED_SETTINGS_INTEGRATION_PLATFORMS.includes(integration.type)) {
    ...
    if (tmp(17850).IntegrationTypes.YOUTUBE === type) {
      ...
      let combined = name;
    } else if (tmp(17850).IntegrationTypes.TWITCH === type) {
    ...
    if (null == value) {
      ...
      const items = [closure_1_11(tmp(5693).TableRow, obj2), ];
      ...
      items[1] = closure_1_11(tmp(7314).TableSwitchRow, obj5);
      obj4.children = items;
      return closure_1_12(tmp(5775).TableRowGroup, obj4);
    } else {
      ...
      icon.source = tmpResult.makeSource(tmp(4491).isThemeDark(props.theme) ? icon.darkPNG : icon.lightPNG);
      icon.style = styles.platformIcon;
      tmp12(tmp13, icon);
      const tmp14 = tmp(4491).isThemeDark(props.theme) ? icon.darkPNG : icon.lightPNG;
    }
  } else {
    return null;
  }
...
export default function GuildSettingsModalIntegrationPlatform(platformType) {
  ...
  if (null == guild) {
    ...
    value = tmp3(5371).get(platformType);
    let name;
    if (value != null) {
      name = value.name;
    }
    ...
    items2[1] = closure_11(tmp(4636).Text, obj12);
    obj9.children = items2;
    obj8.children = closure_12(tmp(5060).Stack, obj9);
    const items3 = [closure_11(tmp(8748).Form, obj8), closure_11(tmp(7154).NavScrim, {})];
