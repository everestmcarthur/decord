...
// Dependencies: [19, 17, 9207, 1074, 21, 4638, 576, 17769, 17809, 5369, 1397, 4489, 5773, 5691, 7310, 1115, 9206, 4983, 5079, 4339, 1484, 504, 4572, 5710, 7487, 7492, 2025, 8728, 5058, 4634, 7150, 2]
// Exports: default

// Module 17808 (GuildSettingsModalIntegrationPlatform)
...
import GuildSettingsModalIntegrations from "GuildSettingsModalIntegrations" /* 17769 */;
...
IntegrationItem.prototype["render"] = function render() {
  ...
  if (SUPPORTED_SETTINGS_INTEGRATION_PLATFORMS.includes(integration.type)) {
    ...
    if (tmp(17809).IntegrationTypes.YOUTUBE === type) {
      ...
      let combined = name;
    } else if (tmp(17809).IntegrationTypes.TWITCH === type) {
    ...
    if (null == value) {
      ...
      const items = [closure_1_11(tmp(5691).TableRow, obj2), ];
      ...
      items[1] = closure_1_11(tmp(7310).TableSwitchRow, obj5);
      obj4.children = items;
      return closure_1_12(tmp(5773).TableRowGroup, obj4);
    } else {
      const tmp12 = closure_1_11;
      const tmp13 = React4;
      const tmpResult = tmp(1397);
      const tmpResult2 = tmp(4489);
      const icon = { source: null, style: null };
      icon.source = tmpResult.makeSource(tmp(4489).isThemeDark(props.theme) ? icon.darkPNG : icon.lightPNG);
      icon.style = styles.platformIcon;
      tmp12(tmp13, icon);
      const tmp14 = tmp(4489).isThemeDark(props.theme) ? icon.darkPNG : icon.lightPNG;
    }
  } else {
    return null;
  }
