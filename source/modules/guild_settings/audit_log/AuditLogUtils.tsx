...
// Function ID: 17789
// Name: AuditLogUtils
// Dependencies: [5547, 7214, 4827, 1963, 5590, 17787, 1958, 2016, 4288, 1372, 17786, 1074, 1965, 12014, 7215, 1964, 3, 4669, 1115, 8541, 11, 17789, 1086, 1385, 4793, 1895, 4484, 14, 1092, 9973, 17790, 4321, 4231, 2]
...
import InstantInviteUtilsDefault from "InstantInviteUtils" /* 9973 */;
import GuildFeedItemTypes from "GuildFeedItemTypes" /* 17789 */;
import AutomodRuleUtils from "AutomodRuleUtils" /* 17790 */;
...
const AuditLogChange = fn(17787).AuditLogChange;
...
const GuildOnboardingMode = fn(7215).GuildOnboardingMode;
...
let items = [fn(4669).TimeUnits.DAYS, fn(4669).TimeUnits.HOURS, fn(4669).TimeUnits.MINUTES, fn(4669).TimeUnits.SECONDS];
    ...
    if (tmp.CHANNEL !== targetType) {
      if (tmp.CHANNEL_OVERWRITE !== targetType) {
        if (tmp.USER === targetType) {
          ...
          const obj23 = {};
          obj23[__3TkD(8541).GuildInviteFlags.IS_GUEST_INVITE] = __3TkD(1115).t.XYZMbL;
          __3TkD = obj23;
      ...
      if (null != timeAndUnit.unit) {
        if (null != timeAndUnit.time) {
          if (timeAndUnit.unit in obj) {
            ...
            if (unit2 === tmp25(4669).TimeUnits.SECONDS) {
              const _Math2 = Math;
      ...
      if (tmp.GUILD_SCHEDULED_EVENT_EXCEPTION_UPDATE !== action) {
        if (tmp.GUILD_SCHEDULED_EVENT_EXCEPTION_DELETE === action) {
          ...
          if (null == found2) {
            ...
            } else if (tmp198(17789).GuildFeedItemTypes.FORUM_POST === newValue) {
    ...
    if (constants2.GUILD !== targetType) {
      ...
      if (tmp2.GUILD_HOME !== targetType) {
        ...
        if (tmp2.GUILD_PROFILE !== targetType) {
                      ...
                      if (tmp2.GUILD_SCHEDULED_EVENT !== targetType) {
                        if (tmp2.GUILD_SCHEDULED_EVENT_EXCEPTION !== targetType) {
                          if (tmp2.THREAD === targetType) {
                            ...
                            let tmp10 = null;
                            if (tmp9) {
                              tmp10 = ((channel) => result3(4793).computeChannelName(channel, user, closure_1_12, true))(tmp7);
                            }
                            let tmp11 = tmp10;
                            if (null == tmp10) {
                              tmp11 = tmp10;
          ...
          let tmp208 = null;
          if (tmp207) {
            tmp208 = ((channel) => result3(4793).computeChannelName(channel, user, closure_1_12, true))(tmp205);
          }
          let tmp209 = tmp208;
          if (null == tmp208) {
            tmp209 = tmp208;
