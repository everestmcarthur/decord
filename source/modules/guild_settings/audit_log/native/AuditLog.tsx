...
// Function ID: 17709
// Name: AuditLog
// Dependencies: [19, 17, 1182, 1386, 4288, 1372, 1074, 21, 4639, 576, 1400, 1397, 5368, 4491, 7275, 4635, 1364, 4350, 17691, 1115, 4484, 1177, 1972, 4792, 4784, 4231, 1370, 1092, 10760, 7237, 5692, 17700, 5207, 4606, 8299, 14700, 504, 2]

// Module 17708 (AuditLog)
...
import AppliedForumTag from "AppliedForumTag" /* 10760 */;
          ...
          if (log.targetType === constants.GUILD) {
            if (typeof target === "object") {
              if (obj6.isGuildRecord(tmp5)) {
                ...
                tmp8Result = closure_2_14(tmp15(4635).Text, obj5, `target${tmp5.id}`);
              }
  ...
  if (log.action !== constants2.AUTO_MODERATION_BLOCK_MESSAGE) {
    if (log.action !== tmp21.AUTO_MODERATION_FLAG_TO_CHANNEL) {
      if (log.action !== tmp21.AUTO_MODERATION_USER_COMMUNICATION_DISABLED) {
        ...
        items2[1] = tmp17(tmp10(5207).PressableOpacity, obj7);
        ...
        items2[2] = tmp15(tmp10(5207).PressableOpacity, obj9);
        ...
        return tmp15(tmp10(5692).Card, obj4);
      }
    }
  }
