...
// Function ID: 17765
// Name: AuditLog
// Dependencies: [19, 17, 1182, 1386, 4286, 1372, 1074, 21, 4638, 576, 1400, 1397, 5369, 4489, 7278, 4634, 1364, 4348, 17747, 1115, 4482, 1177, 1972, 4791, 4783, 4229, 1370, 1092, 10768, 7240, 5693, 17756, 5208, 4605, 8308, 14716, 504, 2]

// Module 17764 (AuditLog)
...
import AppliedForumTag from "AppliedForumTag" /* 10768 */;
import AuditLogUtilsAll from "AuditLogUtils" /* 17747 */;
          ...
          if (log.targetType === constants.GUILD) {
            if (typeof target === "object") {
              if (obj6.isGuildRecord(tmp5)) {
                ...
                tmp8Result = closure_2_14(tmp15(4634).Text, obj5, `target${tmp5.id}`);
              }
...
prototype["render"] = function render() {
  ...
  const items2 = [closure_14(user(17756), { action: log.action }), , , ];
      ...
      if (log.action !== tmp21.AUTO_MODERATION_USER_COMMUNICATION_DISABLED) {
        if (log.action !== tmp21.AUTO_MODERATION_QUARANTINE_USER) {
          if (null != log.options.integration_type) {
            value = tmp18(5369).get(log.options.integration_type);
            if (null != value) {
              const icon = value.icon;
              const tmp9Result = tmp9(4489);
              const tmp25 = tmp9(4489).isThemeDark(theme) ? icon.darkPNG : icon.lightPNG;
              let source = tmp9(1397).makeSource(tmp25);
              const tmp9Result5 = tmp9(1397);
            }
            const tmp18Result = tmp18(5369);
          }
          if (null != guildId) {
        ...
        items2[1] = tmp17(tmp10(5208).PressableOpacity, obj7);
        ...
        items2[2] = tmp15(tmp10(5208).PressableOpacity, obj9);
        ...
        return tmp15(tmp10(5693).Card, obj4);
      }
    }
  }
...
AuditLog.contextType = fn(4348).ThemeContext;
