...
// Dependencies: [5, 32, 19, 17, 9208, 1980, 9207, 1074, 8060, 21, 4638, 576, 1484, 9210, 504, 9330, 8294, 9331, 9206, 4335, 1115, 12622, 4530, 5710, 7487, 1485, 4983, 1177, 7149, 9346, 4634, 5773, 7310, 5691, 5058, 14021, 8728, 14019, 17795, 2]
// Exports: default

// Module 17794 (GuildSettingsModalServerTag)
import nativeDefault from "native" /* 576 */;
import Powerups from "Powerups" /* 4530 */;
import GuildSettingsActionCreatorsDefault from "GuildSettingsActionCreators" /* 9206 */;
import GuildProfileActionCreators from "GuildProfileActionCreators" /* 9331 */;
...
let GuildProfileFetchStatus = fn(9208).GuildProfileFetchStatus;
const GuildSettingsSections = fn(1074).GuildSettingsSections;
const GuildTagConstants = fn(8060);
...
export default function GuildSettingsModalServerTag(guildId) {
  ...
  const result = guildId(9210).canUseMobileServerTagSettings(guildId);
  dependencyMap = result;
  let obj2 = guildId(9210);
  const items = [result1];
  const items1 = [guildId];
  const stateFromStores = guildId(504).useStateFromStores(items, () => GuildStore.getGuild(guildId), items1);
  const obj3 = guildId(504);
  const guildProfile1 = guildId(9330).useGuildProfile(guildId);
  ({ guildProfile, fetchStatus } = guildProfile1);
  const obj4 = guildId(9330);
  ...
  if (guildSupportsTagsResult) {
    guildSupportsTagsResult = tmp2(8294).guildSupportsTags(stateFromStores);
  ...
  result1 = guildId(9210).isServerTagDraftDirty(stateFromStores1, stateFromStores2);
    ...
    if (c3 === 2) {
      ...
      try {
        ...
        if (0 === dependencyMap) {
              ...
              if (!ref.current) {
                if (closure_9) {
                  if (dependencyMap) {
                    ...
                    dependencyMap = 1;
                    c3 = 1;
                    const obj7 = { value: tmp2(9331).saveGuildProfile(guildId, { tag: null, badge: null, badgeColorPrimary: null, badgeColorSecondary: null }), done: false };
                    return obj7;
                  }
                }
              }
          ...
          tmp2(4335).presentError(intl.string(tmp2(1115).t.RTSuVn));
          const obj = tmp2(4335);
        }
        c3 = 3;
      } catch (tmp27) {
  ...
  const effect2 = obj7.useEffect(() => {
    if (c2) {
      if (closure_12) {
        if (c7) {
          let obj = {
            headerRight: first ? (() => closure_1_13(guildId(5710).HeaderSubmittingIndicator, {})) : (() => {
                    const obj = { text: null, onPress: null, disabled: null };
                    const intl = guildId(1115).intl;
                    obj.text = intl.string(guildId(1115).t["R3BPH+"]);
                    obj.onPress = onPress;
                    obj.disabled = !closure_1_9;
                    return closure_13(guildId(7487).HeaderActionButton, obj);
                  })
          };
  ...
  const tmp2Result3 = guildId(9210);
  let tmp26 = result1;
  if (result1) {
    tmp26 = !first;
  }
  ...
  if (result) {
    if (tmp24) {
      if (null == stateFromStores) {
        ...
        return closure_13(navigation(9346), obj8);
      } else if (null == stateFromStores1) {
        return closure_13(tmp2(7149).SceneLoadingIndicator, {});
      } else {
        let tmp29;
        if (tmp14) {
          if (null != stateFromStores1.tag) {
            if ("" !== stateFromStores1.tag) {
              ...
              tmp29 = closure_13(tmp2(4634).Text, obj9);
            }
          }
        }
        ...
        const items11 = [closure_13(tmp2(7310).TableSwitchRow, obj11), ];
        ...
        if (null != tmp29) {
          ...
          if (tmp32Result) {
            ...
            tmp32Result = tmp32(tmp2(14021).GuildBadge, size);
          }
          const items12 = [tmp32Result, tmp29];
          obj13.children = items12;
          tmp31Result = tmp31(tmp2(5058).Stack, obj13);
        }
        obj12.trailing = tmp31Result;
        items11[1] = closure_13(tmp2(5691).TableRow, obj12);
        obj10.children = items11;
        const tmp31Result2 = closure_14(tmp2(5773).TableRowGroup, obj10);
        ...
        if (guildSupportsTagsResult) {
          ...
          const items14 = [tmp32(tmp2(4634).Text, obj17), tmp31Result2, , ];
          ...
          items14[2] = tmp32(tmp2(4634).Text, obj18);
          ...
          items14[3] = tmp32(tmp39(14019), obj19);
          obj15.children = items14;
          let tmp40 = obj15;
        } else {
          const obj20 = { guildId, onUnlockPress: callback2 };
          const items15 = [tmp32(tmp39(17795), obj20), tmp31Result2];
          obj15.children = items15;
          tmp40 = obj15;
        }
        obj14.children = closure_14(tmp2(5058).Stack, tmp40);
        return closure_13(tmp2(8728).Form, obj14);
      }
    } else {
      return closure_13(tmp2(7149).SceneLoadingIndicator, {});
    }
