...
// Dependencies: [1372, 1115, 4606, 16689, 1897, 7372, 16690, 7301, 2]
// Exports: default, getMembersManagementActions

// Module 16688 (showMembersManagementActionSheet)
...
export default function showMembersManagementActionSheet(guild) {
  ...
  obj.onPress = function onPress() {
    ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(16689, dependencyMap.paths), "MembersFilter", { guild, selectedRoleId, onFilterRoleId });
  };
  const items = [obj];
  if (canPrune == null) {
    canPrune = tmp(7372).canPruneGuildMembers(guild, UserStore.getCurrentUser());
    const tmpResult = tmp(7372);
  }
  if (canPrune) {
    ...
    obj2.onPress = function onPress() {
      ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(16690, dependencyMap.paths), "MembersPrune", { guild });
    };
    items.push(obj2);
  }
  const result = guild(7301).showSimpleActionSheet({ key: "GuildSettingsMembersMore", options: items, hasIcons: false });
};
