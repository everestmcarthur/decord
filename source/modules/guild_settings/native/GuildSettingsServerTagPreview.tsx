...
export default function GuildSettingsServerTagPreview(guildId) {
  ...
  const items4 = [closure_10(closure_6, { source: onAdopted(10287), style: tmp.avatar, importantForAccessibility: "no" }), ];
  const obj8 = { style: tmp.messageBody, children: null };
  const items5 = [closure_10(guildId(4632).Text, { variant: "text-md/semibold", color: "text-default", children: "Locke" }), ];
  ...
  const items8 = [closure_10(guildId(4632).Text, { variant: "text-md/semibold", color: "text-default", children: name }), ];
  ...
  if ("plain" !== variant) {
    ...
    tmp15Result4 = tmp15(tmp2(5688).Card, obj23);
  }
  return tmp15Result4;
};
