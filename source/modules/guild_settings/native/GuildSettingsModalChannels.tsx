...
// Dependencies: [19, 17, 1958, 1980, 4278, 4288, 1372, 16303, 1074, 21, 4640, 5612, 576, 1983, 4350, 1115, 4491, 1092, 8748, 1177, 16306, 504, 5115, 4793, 5210, 1612, 5062, 7308, 9888, 9022, 7491, 4283, 16307, 16308, 7154, 8780, 16309, 16304, 11956, 15414, 10370, 11760, 5608, 4984, 1484, 38, 5069, 2]
// Exports: default

// Module 16305 (GuildSettingsModalChannels)
...
import ChannelSettingsActionCreators from "ChannelSettingsActionCreators" /* 8780 */;
import _modDef11956 from "module_11956" /* 11956 */;
import _modDef15414 from "module_15414" /* 15414 */;
import GuildSettingsModalChannelsActionCreatorsDefault from "GuildSettingsModalChannelsActionCreators" /* 16304 */;
import ChannelSortingUtils from "ChannelSortingUtils" /* 16307 */;
import _modDef16309 from "module_16309" /* 16309 */;
...
function ChannelItem(isFavoritesGuild) {
  ...
  const legacyClassComponentStyles = channel(4640).useLegacyClassComponentStyles(closure_18);
  let obj = channel(4640);
  let obj2 = channel(4491);
  ...
  const channelIcon = channel(5115).getChannelIcon(channel);
  const tmpResult = channel(5115);
  const channelIconComponent = channel(5115).getChannelIconComponent(channel);
  const tmp8 = closure_6;
  const tmpResult5 = channel(5115);
  const tmpResult6 = channel(4491);
  const isThemeDarkResult = channel(4491).isThemeDark(obj2.useThemeContext().theme);
  ...
  if (null != channelIconComponent) {
    ...
    tmp7Result = tmp7(tmp(8748).FormRow.Icon, obj8);
  }
  ...
  obj9.label = channel(4793).computeChannelName(channel, UserStore, RelationshipStore);
  let fn;
  if (!sortingEnabled) {
  ...
  if (stateFromStores) {
    tmp7Result2 = null;
    if (sortingEnabled) {
      const obj10 = { source: tmp13(16306), style: actionIconStyle };
      tmp7Result2 = tmp7(tmp(8748).FormRow.Icon, obj10);
    }
  }
  ...
  obj6.children = closure_16(channel(8748).FormRow, obj9);
...
function CreateButton(guild) {
  guild = guild.guild;
  let bottom;
  const legacyClassComponentStyles = guild(4640).useLegacyClassComponentStyles(closure_18);
  ...
  if (PermissionStore.can(constants2.MANAGE_CHANNELS, guild)) {
    ...
    obj3.onPress = function onPress() {
      ...
      obj4.onPress = function onPress() {
        bottom(9888).open(constants.GUILD_CATEGORY, user.id, null, null);
      };
      const items = [obj4, ];
      ...
      obj5.onPress = function onPress() {
        bottom(9888).open(null, user.id, null, null);
      };
      items[1] = obj5;
      obj2.options = items;
      const result = showSimpleActionSheet.showSimpleActionSheet(obj2);
    };
    obj3.icon = closure_16(tmp(9022).PlusSmallIcon, { color: "white" });
    obj2.children = closure_16(tmp(5062).Button, obj3);
...
Category.prototype["render"] = function render() {
  ...
  if (null != onPress) {
    ...
    if (null != sortHandlers) {
      const obj6 = { source: tmp11(16306), style: actionIconStyle };
...
Category.contextType = fn(4350).ThemeContext;
...
SectionEditAction.contextType = fn(4350).ThemeContext;
  ...
  constructor() {
    ...
    applyArgumentsResult.handleSortStart = function handleSortStart() {
      ...
      if (PermissionStore.can(constants2.MANAGE_CHANNELS, applyArgumentsResult.props.guild)) {
        ...
        obj.icon = _modDef16309;
        obj.onPress = function onPress() {
          closure_1_1(16304).startReordering(constants.GUILD_CATEGORY);
        };
        items.push(obj);
      }
      ...
      obj2.icon = _modDef11956;
      obj2.onPress = function onPress() {
        closure_1_1(16304).startReordering(constants.GUILD_TEXT, constants.GUILD_ANNOUNCEMENT, constants.GUILD_FORUM, constants.GUILD_MEDIA, constants.GUILD_APP);
      };
      items.push(obj2);
      const obj3 = { label: null, icon: null, onPress: null };
      const intl3 = util.intl;
      obj3.label = intl3.string(util.t.CYnO4s);
      obj3.icon = _modDef15414;
      obj3.onPress = function onPress() {
        closure_1_1(16304).startReordering(constants.GUILD_VOICE, constants.GUILD_STAGE_VOICE);
      };
      items.push(obj3);
            ...
            if (null != localChannel1) {
              ...
              if (tmp14) {
                ...
                if (tmp3Result7.isFavoritesGuildId(guild.id)) {
                  ...
                  const result = tmp3(10370).updateFavoriteChannels(dnDUpdates);
                } else {
                        ...
                        if (null != channel) {
                          if (PermissionStore.can(constants2.MANAGE_ROLES, tmp)) {
                            if (obj.can(tmp2.MANAGE_ROLES, channel)) {
                              const appChannelBotUserId = applyArgumentsResult(11760).getAppChannelBotUserId(tmp);
                              const obj2 = applyArgumentsResult(11760);
                  ...
                  if (null != parent_id) {
                    ...
                    const tmp3Result9 = tmp3(4793);
                    obj4.channelName = tmp3Result9.computeChannelName(localChannel, UserStore, RelationshipStore, true);
                    let str = "";
                    if (null != channel) {
                      str = tmp3(4793).computeChannelName(channel, tmp29, tmp30);
                      const tmp3Result10 = tmp3(4793);
                    }
...
GuildSettingsModalChannels.contextType = fn(4350).ThemeContext;
const size = fn(2);
