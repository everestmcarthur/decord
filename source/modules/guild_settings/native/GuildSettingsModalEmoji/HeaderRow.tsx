...
// Function ID: 17653
// Name: HeaderRow
// Dependencies: [5, 32, 19, 17, 17646, 1074, 1374, 21, 4606, 576, 9762, 10425, 1242, 1114, 1256, 5189, 5026, 4602, 504, 2]
// Exports: ConnectedHeaderRow

// Module 17652 (HeaderRow)
...
import GuildSettingsEmojiStore from "GuildSettingsEmojiStore" /* 17646 */;
...
function HeaderRow(guild) {
            ...
            if (arg0 === 1) {
              ...
              if (null != base64) {
                upload = function upload(image, originalMd5, roles) {
                  ...
                  let obj = closure_1_0(10425);
                  let obj2 = { guildId: image.id, image, name: combined, roles, originalMd5 };
                  const uploadEmojiResult = closure_1_0(10425).uploadEmoji({ guildId: image.id, image, name: combined, roles, originalMd5 });
                  closure_1_0(10425).uploadEmoji({ guildId: image.id, image, name: combined, roles, originalMd5 }).then(() => closure_2_1(1242).track(constants.EMOJI_UPLOAD_COMPLETED, { guild_id: image.id, upload_id })).catch((error) => {
  ...
  let canCreateExpressions = guild(9762).useManageResourcePermissions(guild).canCreateExpressions;
  ...
  if (canCreateExpressions) {
    ...
    canCreateExpressions = tmp13(tmp7(5026).Button, obj8);
  }
  ...
  if (tmp13Result) {
    ...
    tmp13Result = tmp13(tmp7(4602).Text, obj9);
  }
  const obj10 = { children: null };
  items1[1] = tmp13Result;
  let obj11 = { style: tmp.uploadInstructionsContainer, children: null };
  const items2 = [closure_14(guild(4602).Text, { variant: "text-sm/medium", color: "text-muted", children: description }), , ];
  ...
  items2[1] = closure_14(guild(4602).Text, obj12);
