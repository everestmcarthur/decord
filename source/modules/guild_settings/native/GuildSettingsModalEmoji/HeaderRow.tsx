...
// Function ID: 17819
// Name: HeaderRow
// Dependencies: [5, 32, 19, 17, 17812, 1074, 1375, 21, 4640, 576, 9822, 10484, 1241, 1115, 1255, 5225, 5062, 4636, 504, 2]
// Exports: ConnectedHeaderRow

// Module 17818 (HeaderRow)
...
import GuildSettingsEmojiStore from "GuildSettingsEmojiStore" /* 17812 */;
...
function HeaderRow(guild) {
            ...
            if (arg0 === 1) {
              ...
              if (null != base64) {
                upload = function upload(image, originalMd5, roles) {
                  ...
                  const uploadEmojiResult = closure_1_0(10484).uploadEmoji({ guildId: image.id, image, name: combined, roles, originalMd5 });
                  closure_1_0(10484).uploadEmoji({ guildId: image.id, image, name: combined, roles, originalMd5 }).then(() => closure_2_1(1241).track(constants.EMOJI_UPLOAD_COMPLETED, { guild_id: image.id, upload_id })).catch((error) => {
  ...
  if (canCreateExpressions) {
    ...
    canCreateExpressions = tmp13(tmp7(5062).Button, obj8);
  }
  ...
  if (tmp13Result) {
    ...
    tmp13Result = tmp13(tmp7(4636).Text, obj9);
  }
  const obj10 = { children: null };
  items1[1] = tmp13Result;
  let obj11 = { style: tmp.uploadInstructionsContainer, children: null };
  const items2 = [closure_14(guild(4636).Text, { variant: "text-sm/medium", color: "text-muted", children: description }), , ];
