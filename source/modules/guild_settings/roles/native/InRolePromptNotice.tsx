...
// Dependencies: [19, 17, 1074, 21, 4605, 576, 1384, 17726, 1178, 9695, 4601, 1114, 2]
// Exports: default

// Module 17725 (InRolePromptNotice)
...
export default function InRolePromptNotice(role) {
  ...
  if (!obj.hasFlag(role.flags, RoleFlags.IN_PROMPT)) {
    ...
    const tmpResult = tmp(17726);
    let Icon = tmp(1178).Icon;
    if (isRolePowerfulResult) {
      const obj3 = { style: string.icon, source: _modDef9695, color: nativeDefault.unsafe_rawColors.YELLOW_300 };
      ...
      Icon = tmp6(tmp(4601).Text, obj4);
      items[1] = Icon;
      obj2.children = items;
      let tmp4Result = tmp4(tmp5, obj2);
    } else {
      const obj5 = { style: string.icon, source: _modDef9695 };
      ...
      items1[1] = tmp6(tmp(4601).Text, obj6);
      obj2.children = items1;
      tmp4Result = tmp4(tmp5, obj2);
    }
    isRolePowerfulResult = tmp(17726).isRolePowerful(role);
  }
};
