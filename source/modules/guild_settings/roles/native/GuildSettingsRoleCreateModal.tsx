...
// Dependencies: [5, 32, 19, 17, 1976, 2016, 1372, 9207, 17818, 1074, 21, 4638, 5768, 576, 1241, 4818, 4634, 1115, 17816, 504, 38, 4281, 1484, 6733, 5710, 5606, 4335, 17815, 4605, 16421, 1897, 5058, 6714, 5773, 5691, 14711, 1092, 5060, 17822, 17823, 9206, 17824, 7091, 5045, 5054, 7149, 5077, 7110, 2]
// Exports: default

// Module 17817 (GuildSettingsRoleCreateModal)
...
import GuildSettingsRoleCreateModalActionCreatorsDefault from "GuildSettingsRoleCreateModalActionCreators" /* 17816 */;
...
function RoleCreateScene() {
  ...
  const callback1 = onSelect.useCallback(asyncGeneratorStep(async () => {
    ...
    await tmp3(5606).createRole(stateFromStores.id, str, num5);
    if (1 === tmp7) {
      c2 = 0;
      closure_129_5(false);
      const result = tmp3(4335).roleCreateFailedToast();
      c4 = 3;
      tmp3(4335);
    } else if (arg0 === 1) {
      c4 = 3;
      ...
      const result1 = GuildSettingsStore.addConditionalChangeListener(() => {
        ...
        if (null != selectedRoleId) {
          if (null != role) {
            if (null != guild) {
              closure_0(4335).roleCreatedToast();
              const obj5 = closure_0(4335);
              const tmp13 = closure_0;
              closure_0(17815).setRoleJustCreated(true);
              ...
              const merged = Object.assign(tmp13(4818).collectGuildAnalyticsMetadata(guild.id));
  ...
  const callback2 = onSelect.useCallback(() => {
    React6.dismiss();
    ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(16421, dependencyMap.paths), "RoleColorPicker", { color, onSelect });
  }, items3);
...
export default function GuildSettingsRoleCreateModal() {
  ...
  return closure_21(stateFromStores(7110).Navigator, { screens, initialRouteName: obj7.STEP_DISPLAY });
};
