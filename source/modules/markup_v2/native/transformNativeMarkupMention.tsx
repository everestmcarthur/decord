...
// Dependencies: [5041, 5012, 5010, 5020, 2]
...
export const transformNativeMention = function transformNativeMention(value, allowGameMentions) {
  ...
  if ("user" === type) {
    ...
    if (allowGameMentions.allowGameMentions) {
      let hydrateGameMentionResult = tmp21(5041).hydrateGameMention(str16, allowGameMentions);
      const tmp21Result = tmp21(5041);
    } else {
      hydrateGameMentionResult = { type: tmp21(5010).AST_KEY.TEXT, content: null };
    ...
    if (null == channel) {
      ...
      handleUnknownChannelResult = tmp4(5020).parseChannel(channel, null, guildIdFromChannelId);
      const tmp4Result4 = tmp4(5020);
    }
    const obj5 = {};
    const merged = Object.assign(handleUnknownChannelResult);
