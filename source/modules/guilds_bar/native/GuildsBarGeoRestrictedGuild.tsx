...
// Dependencies: [19, 16392, 21, 4639, 576, 16408, 16404, 1397, 5669, 4983, 1115, 9195, 16458, 16427, 5672, 2]

// Module 16457 (GuildsBarGeoRestrictedGuild)
...
import HomeDrawerGuildRowDefault from "HomeDrawerGuildRow" /* 16427 */;
import GuildsBarGeoRestrictedBadgeDefault from "GuildsBarGeoRestrictedBadge" /* 16458 */;
import noop from "module_19" /* 19 */;

require = fn;
const GUILD_ITEM_BADGE_SIZE = fn(16392).GUILD_ITEM_BADGE_SIZE;
const jsx = fn(21).jsx;
...
export default noop.memo(function GuildsBarGeoRestrictedGuild(restrictedGuild) {
  ...
  const obj3 = { selected: false, unread: false, circle: false, styles: restrictedGuild(16404).useGuildsBarAnimatedWrapperStyles({ disableSelectedColor: true, disableBGColor: true }), label: restrictedGuild.name, isDragTarget: false, config: memo, cutouts: items, overState: "a", externalChildren: 0.721, expandedChildren: 0.721, children: 0.167 };
  const guildsBarAnimatedWrapperStyles = restrictedGuild(16404).useGuildsBarAnimatedWrapperStyles({ disableSelectedColor: true, disableBGColor: true });
  ...
  if (null != animatableSourceWithFallback) {
    ...
    let tmp8Result = tmp8(tmp9(5672), obj6);
  } else {
    const obj7 = { value: restrictedGuild.name, selected: false, animate: false, size: tmp2(5669).GuildIconSizes.LARGE };
    tmp8Result = tmp8(tmp9(5669), obj7);
    const tmp9Result = tmp9(5669);
  }
  obj3.children = tmp8Result;
  return jsx(GuildsBarAnimatedItemWrapperDefault, { selected: false, unread: false, circle: false, styles: restrictedGuild(16404).useGuildsBarAnimatedWrapperStyles({ disableSelectedColor: true, disableBGColor: true }), label: restrictedGuild.name, isDragTarget: false, config: memo, cutouts: items, overState: "a", externalChildren: 0.721, expandedChildren: 0.721, children: 0.167 });
});
