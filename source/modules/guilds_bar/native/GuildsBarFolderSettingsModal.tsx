...
// Dependencies: [32, 19, 17, 5524, 16420, 21, 4638, 9501, 7091, 4605, 16421, 1897, 576, 5058, 6714, 1115, 5773, 5691, 1092, 14711, 5710, 504, 16418, 7110, 2]
// Exports: default

// Module 16419 (GuildsBarFolderSettingsModal)
...
import GuildsBarFolderSettingsModalActionCreators from "GuildsBarFolderSettingsModalActionCreators" /* 16418 */;
...
function GuildFolderSettingsScene(color) {
  ...
  const callback = noop.useCallback(() => {
    ...
    obj.openLazy(asyncRequireImpl(16421, dependencyMap.paths), "RoleColorPicker", { color: tmp3, defaultColor, onSelect: onColorChange });
  }, items);
  obj2.padding = onColorChange(576).space.PX_16;
  obj2.paddingBottom = 38 + onColorChange(7091)().insets.bottom;
  ...
  const items1 = [closure_10(color(6714).TextInput, obj4), ];
  ...
  obj5.trailing = closure_10(onColorChange(14711), { color: tmp11, style: tmp.colorBlock });
  obj6.children = closure_10(color(5691).TableRow, obj5);
  items1[1] = closure_10(color(5773).TableRowGroup, obj6);
  obj3.children = items1;
  obj.children = closure_11(color(5058).Stack, obj3);
  return closure_10(closure_6, obj);
}
get_ActivityIndicator = fn(17);
({ Keyboard: hasOwnProperty, ScrollView: metroRequire } = get_ActivityIndicator);
const GuildsBarConstants = fn(16420);
...
export default function GuildFolderSettingsModal(folderId) {
  ...
  const memo = obj2.useMemo(() => {
    ...
    if (closure_5) {
      ...
      let fn = tmp3(5710).getHeaderTextButton(intl2.string(tmp3(1115).t["R3BPH+"]), callback);
      const tmp3Result = tmp3(5710);
    } else {
      fn = () => null;
    }
    obj2.headerRight = fn;
  ...
  return closure_10(folderId(7110).Navigator, { screens: memo, initialRouteName });
};
