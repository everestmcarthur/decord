...
// Dependencies: [32, 19, 17, 5526, 16450, 21, 4640, 9512, 7095, 4607, 16451, 1897, 576, 5060, 6717, 1115, 5775, 5693, 1092, 14719, 5712, 504, 16448, 7114, 2]
// Exports: default

// Module 16449 (GuildsBarFolderSettingsModal)
...
import GuildsBarFolderSettingsModalActionCreators from "GuildsBarFolderSettingsModalActionCreators" /* 16448 */;
...
function GuildFolderSettingsScene(color) {
  ...
  const callback = noop.useCallback(() => {
    ...
    obj.openLazy(asyncRequireImpl(16451, dependencyMap.paths), "RoleColorPicker", { color: tmp3, defaultColor, onSelect: onColorChange });
  }, items);
  obj2.padding = onColorChange(576).space.PX_16;
  obj2.paddingBottom = 38 + onColorChange(7095)().insets.bottom;
  ...
  const items1 = [closure_10(color(6717).TextInput, obj4), ];
  ...
  obj5.trailing = closure_10(onColorChange(14719), { color: tmp11, style: tmp.colorBlock });
  obj6.children = closure_10(color(5693).TableRow, obj5);
  items1[1] = closure_10(color(5775).TableRowGroup, obj6);
  obj3.children = items1;
  obj.children = closure_11(color(5060).Stack, obj3);
  return closure_10(closure_6, obj);
}
get_ActivityIndicator = fn(17);
({ Keyboard: hasOwnProperty, ScrollView: metroRequire } = get_ActivityIndicator);
const GuildsBarConstants = fn(16450);
...
export default function GuildFolderSettingsModal(folderId) {
  ...
  const memo = obj2.useMemo(() => {
    ...
    if (closure_5) {
      ...
      let fn = tmp3(5712).getHeaderTextButton(intl2.string(tmp3(1115).t["R3BPH+"]), callback);
      const tmp3Result = tmp3(5712);
    } else {
      fn = () => null;
    }
    obj2.headerRight = fn;
  ...
  return closure_10(folderId(7114).Navigator, { screens: memo, initialRouteName });
};
