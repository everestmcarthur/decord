...
// Dependencies: [5, 19, 17, 1980, 5524, 16415, 16412, 4374, 551, 4606, 4489, 12, 1231, 1115, 7182, 16150, 4339, 576, 4260, 11144, 1248, 5606, 9501, 8802, 1364, 1612, 5045, 16416, 16417, 15206, 6763, 2]
// Exports: default

// Module 16414 (useGuildsBarGesture)
    ...
    if (item.section >= GUILDS) {
      ...
      if (null != GUILDS) {
        ...
        if (FastList.FastListItemTypes.SECTION !== type) {
          if (tmp9(7182).FastListItemTypes.ITEM === type) {
...
let closure_16 = debounce(fn(4606).triggerHapticFeedback, 16);
...
export default function useGuildsBarGesture() {
      ...
      if (null != overSpecs) {
        if (null != dragSpecs) {
          ...
          if (FastList.FastListItemTypes.SECTION === type2) {
            ...
            if (tmp34(7182).FastListItemTypes.SECTION === type) {
              let str = "before";
              let str2 = "before";
  ...
  const callback4 = fastListRef.useCallback((arg0) => {
    ...
    if (null != obj2) {
      ...
      if (id !== id1) {
        const ContextMenuStore = tmp66(8802).ContextMenuStore;
        if (null != ContextMenuStore.getState().menu) {
          tmp66(8802).hideContextMenu();
          const tmp66Result = tmp66(8802);
        }
        node2 = node;
            ...
            if (null != node3) {
              if (null != node2) {
                ...
                if (node3.id !== node2.id) {
                  ...
                  if (tmp66(7182).FastListItemTypes.SECTION === type2) {
                    let type = item2.type;
                    if (tmp66(7182).FastListItemTypes.SECTION === type) {
                      const str5 = "self";
