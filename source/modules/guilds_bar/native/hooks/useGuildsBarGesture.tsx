...
// Dependencies: [5, 19, 17, 1980, 5523, 16395, 16392, 4376, 551, 4607, 4491, 12, 1231, 1115, 7179, 16130, 4341, 576, 4262, 11130, 1248, 5605, 9490, 8793, 1364, 1612, 5046, 16396, 16397, 15190, 6760, 2]
// Exports: default

// Module 16394 (useGuildsBarGesture)
    ...
    if (item.section >= GUILDS) {
      ...
      if (null != GUILDS) {
        ...
        if (FastList.FastListItemTypes.SECTION !== type) {
          if (tmp9(7179).FastListItemTypes.ITEM === type) {
...
let closure_16 = debounce(fn(4607).triggerHapticFeedback, 16);
...
export default function useGuildsBarGesture() {
      ...
      if (null != overSpecs) {
        if (null != dragSpecs) {
          ...
          if (FastList.FastListItemTypes.SECTION === type2) {
            ...
            if (tmp34(7179).FastListItemTypes.SECTION === type) {
              let str = "before";
              let str2 = "before";
  ...
  const callback4 = fastListRef.useCallback((arg0) => {
    ...
    if (null != obj2) {
      ...
      if (id !== id1) {
        const ContextMenuStore = tmp66(8793).ContextMenuStore;
        if (null != ContextMenuStore.getState().menu) {
          tmp66(8793).hideContextMenu();
          const tmp66Result = tmp66(8793);
        }
        node2 = node;
            ...
            if (null != node3) {
              if (null != node2) {
                ...
                if (node3.id !== node2.id) {
                  ...
                  if (tmp66(7179).FastListItemTypes.SECTION === type2) {
                    let type = item2.type;
                    if (tmp66(7179).FastListItemTypes.SECTION === type) {
                      const str5 = "self";
