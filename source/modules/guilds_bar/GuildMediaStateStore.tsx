...
// Dependencies: [1956, 1236, 7540, 1962, 1961, 4596, 502, 1957, 1979, 4212, 4222, 2011, 4755, 4593, 1074, 13735, 1094, 9694, 13736, 11, 5430, 4202, 9578, 504, 558, 573, 2]

// Module 13734 (GuildMediaStateStore)
...
function computeGuildMediaState(guildId) {
  ...
  let tmp2 = (function getSharedState() {
    ...
    obj = { skipMutedVcs: guildId(13735).getIsDontBadgeMutedVcsEnabled("GuildMediaStateStore"), currentUserId: id.getId(), selectedVoiceChannelId: voiceChannelId, selectedVoiceGuildId: null, selectedVoiceChannelHasVideo: null, isSelectedVoiceChannelStage: null, blockedOrIgnoredUserIds: null, streamChannelIdsByGuild: null };
    let guild_id;
    if (channel != null) {
    ...
    for (const item10049 of usersWithVideo) {
      ...
      if (!blockedOrIgnoredUserIds.has(item10049)) {
        ...
        let someResult1 = keys1.some((item) => {
          ...
          if (tmp2) {
            tmp2 = closure_1(5430)(basicChannel, PermissionStore);
          }
          return tmp2;
        });
        ...
        if (tmp34Result.isActivitiesInTextEnabled(ChannelStore.getChannel(embeddedActivityLocationChannelId))) {
          ...
          someResult2 = found.some((location) => {
            const channel = ChannelStore.getChannel(guildId(4202).getEmbeddedActivityLocationChannelId(location.location));
