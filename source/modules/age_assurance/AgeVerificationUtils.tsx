...
import closure_6 from "initialize" /* 4774 */;
import closure_7 from "getRegionalFeatureConfig" /* 4775 */;
...
function useShouldCallReactiveCheck() {
  const items = [closure_10];
  const stateFromStores = _require(504).useStateFromStores(items, () => currentUser.getCurrentUser());
  let prop;
  if (stateFromStores != null) {
    prop = stateFromStores.ageVerificationStatus;
  }
...
function shouldCallReactiveCheck() {
  ...
  let tmp7 = !tmp5;
  if (!tmp5) {
    let isFeatureAgeGatedResult = closure_7.isFeatureAgeGated(tmp3(5424).AgeGatedFeature.REACTIVE_CHECK);
...
export const shouldShowTiggerPawtect = function shouldShowTiggerPawtect() {
  ...
  if (tmp5) {
    const isFeatureAgeGatedResult = closure_7.isFeatureAgeGated(tmp3(5424).AgeGatedFeature.REACTIVE_CHECK);
...
export const isVerifiedAdult = function isVerifiedAdult() {
  ...
  let tmp5 = prop === PermissionOverwriteType.AgeVerificationStatusUkAndAusOnly.VERIFIED_ADULT;
  if (!tmp5) {
    tmp5 = closure_7.isFeatureAgeGated(tmp3(5424).AgeGatedFeature.REACTIVE_CHECK) && prop === tmp3(1894).AgeVerificationStatusUkAndAusOnly.INFERRED_ADULT;
    const tmp7 = closure_7.isFeatureAgeGated(tmp3(5424).AgeGatedFeature.REACTIVE_CHECK) && prop === tmp3(1894).AgeVerificationStatusUkAndAusOnly.INFERRED_ADULT;
  }
  return tmp5;
};
...
export const useWatchAgeVerificationStatusChange = function useWatchAgeVerificationStatusChange(callback1) {
  ...
  const stateFromStores1 = _require(504).useStateFromStores(items1, () => null != closure_8.getSuspendedUserToken());
  const obj2 = _require(504);
  const items2 = [closure_8];
  let tmp5 = null != tmp2;
  const stateFromStores2 = _require(504).useStateFromStores(items2, () => closure_8.isAuthenticated());
  if (tmp5) {
