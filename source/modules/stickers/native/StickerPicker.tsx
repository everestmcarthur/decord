...
// Function ID: 10457
// Name: StickerPicker
// Dependencies: [32, 19, 17, 1371, 5552, 1938, 1074, 21, 4605, 576, 1242, 10457, 10459, 504, 7214, 7234, 10353, 1249, 10463, 7389, 4944, 10464, 9383, 7240, 10473, 7102, 1114, 10483, 10484, 10489, 2]

// Module 10456 (StickerPicker)
...
import StickersSearchUtils from "StickersSearchUtils" /* 10463 */;
import openStickerPackDetailActionSheetDefault from "openStickerPackDetailActionSheet" /* 10464 */;
...
export default noop.memo(function StickerPicker(channel) {
  ...
  let tmp = closure_16();
  const fetchStickerPacks = channel(10457).useFetchStickerPacks();
  let obj = channel(10457);
  const stickerCategories = channel(10459).useStickerCategories(channel);
  let obj2 = channel(10459);
  ...
  analyticsLocations = onPressSticker(7214)(onPressSticker(7234).STICKER_PICKER).analyticsLocations;
  const tmp9 = onPressSticker(7214);
  [tmp11, c4] = analyticsLocations(noop.useState(null), 2);
  const tmp10 = analyticsLocations(noop.useState(null), 2);
  ({ safeAreaStyle, safeAreaBottomKeyboardAware } = onPressSticker(10353)({ hasCategories: true }));
  const items1 = [, ];
  ...
  const callback1 = noop.useCallback((pack_id) => {
    ...
    if (stickerSendability === StickerSendability.StickerSendability.SENDABLE) {
    ...
    } else if (stickerSendability === tmp(7389).StickerSendability.SENDABLE_WITH_PREMIUM) {
      if (tmpResult.isStandardSticker(pack_id)) {
        ...
        if (null != stickerPack) {
          ...
          obj2.analyticsPopoutType = tmp(10464).AnalyticsPopoutType.STICKER_PACK_DETAIL;
  ...
  const tmp12 = onPressSticker(10353)({ hasCategories: true });
  ...
  if (0 !== stickerCategories.length) {
    ...
    obj7.children = tmp18(tmp2(7102).SearchField, obj8);
  ...
  if (stateFromStores) {
    if (0 === stickerCategories.length) {
      ...
      tmp18Result2 = tmp18(tmp8(10484), obj10);
      const tmp8Result = tmp8(10484);
    }
  } else {
    ...
    items5[2] = tmp18(tmp8(10489), obj12);
    obj6.children = items5;
    obj5.children = closure_15(tmp20, obj6);
    return tmp18(channel(7214).AnalyticsLocationProvider, obj5);
  }
});
