...
// Function ID: 10419
// Name: StickerPicker
// Dependencies: [32, 19, 17, 1371, 5516, 1938, 1074, 21, 4574, 576, 1242, 10419, 10421, 504, 7176, 7196, 10315, 1249, 10425, 7351, 4913, 10426, 9345, 7202, 10435, 7064, 1114, 10445, 10446, 10451, 2]

// Module 10418 (StickerPicker)
...
import StickersSearchUtils from "StickersSearchUtils" /* 10425 */;
import openStickerPackDetailActionSheetDefault from "openStickerPackDetailActionSheet" /* 10426 */;
...
export default noop.memo(function StickerPicker(channel) {
  ...
  let tmp = closure_16();
  const fetchStickerPacks = channel(10419).useFetchStickerPacks();
  let obj = channel(10419);
  const stickerCategories = channel(10421).useStickerCategories(channel);
  let obj2 = channel(10421);
  ...
  analyticsLocations = onPressSticker(7176)(onPressSticker(7196).STICKER_PICKER).analyticsLocations;
  const tmp9 = onPressSticker(7176);
  [tmp11, c4] = analyticsLocations(noop.useState(null), 2);
  const tmp10 = analyticsLocations(noop.useState(null), 2);
  ({ safeAreaStyle, safeAreaBottomKeyboardAware } = onPressSticker(10315)({ hasCategories: true }));
  const items1 = [, ];
  ...
  const callback1 = noop.useCallback((pack_id) => {
    ...
    if (stickerSendability === StickerSendability.StickerSendability.SENDABLE) {
    ...
    } else if (stickerSendability === tmp(7351).StickerSendability.SENDABLE_WITH_PREMIUM) {
      if (tmpResult.isStandardSticker(pack_id)) {
        ...
        if (null != stickerPack) {
          ...
          obj2.analyticsPopoutType = tmp(10426).AnalyticsPopoutType.STICKER_PACK_DETAIL;
  ...
  const tmp12 = onPressSticker(10315)({ hasCategories: true });
  ...
  if (0 !== stickerCategories.length) {
    ...
    obj7.children = tmp18(tmp2(7064).SearchField, obj8);
  ...
  if (stateFromStores) {
    if (0 === stickerCategories.length) {
      ...
      tmp18Result2 = tmp18(tmp8(10446), obj10);
      const tmp8Result = tmp8(10446);
    }
  } else {
    ...
    items5[2] = tmp18(tmp8(10451), obj12);
    obj6.children = items5;
    obj5.children = closure_15(tmp20, obj6);
    return tmp18(channel(7176).AnalyticsLocationProvider, obj5);
  }
});
