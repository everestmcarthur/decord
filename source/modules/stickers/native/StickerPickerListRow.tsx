...
// Dependencies: [19, 17, 1372, 1939, 10420, 1218, 21, 4640, 576, 672, 5182, 1364, 1936, 9475, 4608, 4609, 4979, 7451, 5356, 10322, 10550, 1231, 5210, 2]
// Exports: default

// Module 10549 (StickerPickerListRow)
...
const StickerSendability = tmp(7451);
require = fn;
...
export default function StickerPickerListRow(stickers) {
  ...
  closure_9 = stickers(9475).useMobileStickerPickerUpsellRestyleEnabled("native.StickerPickerListRow");
  let items = [];
  if (nativeRow) {
    let num4 = 0;
    if (0 < rowSize) {
      do {
        ...
        if (undefined !== tmp12) {
          ...
          obj4.stickerType = stickers(5356).StickerFormat.PNG;
    ...
    try {
      ...
      return closure_8(isSectionNitroLocked(10550), obj7);
    } catch (tmp26) {
      ...
      do {
        let tmp8 = (function _loop() {
          ...
          if (undefined === stickers[c12]) {
            ...
            let isSendableStickerResult = isSectionNitroLocked;
            const obj5 = stickers(4979);
            if (!isSectionNitroLocked) {
              isSendableStickerResult = null == closure_6;
            }
            if (!isSendableStickerResult) {
              isSendableStickerResult = tmp26(7451).isSendableSticker(tmp2, currentUser.getCurrentUser(), closure_6);
              const tmp26Result = tmp26(7451);
            }
            let tmp11 = closure_9;
            if (closure_9) {
            ...
            obj2.onPress = function onPress() {
              const result = closure_0(4608).triggerHapticFeedback(isSectionNitroLocked(4609).IMPACT_LIGHT);
              if (dependencyMap != null) {
            ...
            const obj3 = { sticker: tmp2, size, animated: stickers(4979).shouldAnimateSticker(closure_8, tmp2.id === id), opaque: isSendableStickerResult };
            const items1 = [closure_8(isSectionNitroLocked(10322), obj3, tmp), ];
            let tmp17 = null;
            if (tmp11) {
            ...
            items.push(closure_9(stickers(5210).PressableOpacity, obj2, tmp2.id));
          }
        })();
        sum = num2 + 1;
        c12 = sum;
