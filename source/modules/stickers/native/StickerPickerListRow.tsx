...
// Dependencies: [19, 17, 1371, 1938, 10401, 1219, 21, 4636, 576, 672, 5176, 1150, 1935, 9451, 4604, 4605, 4975, 7440, 5349, 10304, 10530, 1232, 5204, 2]
// Exports: default
...
const StickerSendability = tmp(7440);
require = fn;
...
export default function StickerPickerListRow(stickers) {
  ...
  if (nativeRow === undefined) {
    nativeRow = stickers(1150).isAndroid();
  ...
  closure_9 = stickers(9451).useMobileStickerPickerUpsellRestyleEnabled("native.StickerPickerListRow");
  let items = [];
  if (nativeRow) {
    let num4 = 0;
    if (0 < rowSize) {
      do {
        ...
        if (undefined !== tmp12) {
          ...
          obj4.stickerType = stickers(5349).StickerFormat.PNG;
    ...
    try {
      ...
      return closure_8(isSectionNitroLocked(10530), obj7);
    } catch (tmp26) {
      ...
      do {
        let tmp8 = (function _loop() {
          ...
          if (undefined === stickers[c12]) {
            ...
            const obj3 = { sticker: tmp2, size, animated: stickers(4975).shouldAnimateSticker(closure_8, tmp2.id === id), opaque: isSendableStickerResult };
            const items1 = [closure_8(isSectionNitroLocked(10304), obj3, tmp), ];
            let tmp17 = null;
            if (tmp11) {
            ...
            items.push(closure_9(stickers(5204).PressableOpacity, obj2, tmp2.id));
          }
        })();
        sum = num2 + 1;
        c12 = sum;
