...
// Function ID: 10247
// Name: Sticker
// Dependencies: [19, 17, 1183, 21, 5318, 4944, 1114, 8051, 10247, 5637, 4456, 7183, 7184, 2]
// Exports: default, getStickerAssetUrl

// Module 10246 (Sticker)
import util from "util" /* 1114 */;
import StickersTypes from "StickersTypes" /* 5318 */;
import FastImageDefault from "FastImage" /* 5637 */;
import NativeLottieViewDefault from "NativeLottieView" /* 8051 */;
import NativeAPNGViewDefault from "NativeAPNGView" /* 10247 */;
...
export default function Sticker(opaque) {
  ...
  if (sticker.format_type === StickersTypes.StickerFormat.LOTTIE) {
    let str = tmp(4944).getStickerAssetUrl(sticker);
    const tmpResult = tmp(4944);
  } else if (sticker.format_type === tmp(5318).StickerFormat.APNG) {
    const obj = { isPreview: !animated, size };
    str = tmp(4944).getStickerAssetUrl(sticker, obj);
    const tmpResult6 = tmp(4944);
  } else {
    const obj2 = { isPreview: !animated, size: PixelRatio.getPixelSizeForLayoutSize(size) };
    str = tmp(4944).getStickerAssetUrl(sticker, obj2);
    const tmpResult7 = tmp(4944);
  }
  if (str == null) {
    str = "";
  }
  ...
  if (sticker.format_type === StickersTypes.StickerFormat.LOTTIE) {
    ...
    if (sticker.format_type === tmp(5318).StickerFormat.APNG) {
      if ("type" in sticker) {
        const tmpResult8 = tmp(4944);
      }
      ...
      obj4.name = "" + sticker.id + "." + tmp(4944).getStickerExtensionFromFormatType(sticker.format_type);
      obj4.accessibilityLabel = formatToPlainStringResult;
      const obj5 = {};
      const tmpResult9 = tmp(4944);
    ...
    if (tmpResult10.isThemeDark(ThemeStore.theme)) {
      let tmp13Result = tmp13(7183);
    } else {
      tmp13Result = tmp13(7184);
    }
