...
// Function ID: 10307
// Name: Sticker
// Dependencies: [19, 17, 1182, 21, 5353, 4978, 1115, 8106, 10307, 5672, 4491, 7238, 7239, 2]
// Exports: default, getStickerAssetUrl

// Module 10306 (Sticker)
import util from "util" /* 1115 */;
import StickersTypes from "StickersTypes" /* 5353 */;
import FastImageDefault from "FastImage" /* 5672 */;
import NativeLottieViewDefault from "NativeLottieView" /* 8106 */;
import NativeAPNGViewDefault from "NativeAPNGView" /* 10307 */;
...
export default function Sticker(opaque) {
  ...
  if (sticker.format_type === StickersTypes.StickerFormat.LOTTIE) {
    let str = tmp(4978).getStickerAssetUrl(sticker);
    const tmpResult = tmp(4978);
  } else if (sticker.format_type === tmp(5353).StickerFormat.APNG) {
    const obj = { isPreview: !animated, size };
    str = tmp(4978).getStickerAssetUrl(sticker, obj);
    const tmpResult6 = tmp(4978);
  } else {
    const obj2 = { isPreview: !animated, size: PixelRatio.getPixelSizeForLayoutSize(size) };
    str = tmp(4978).getStickerAssetUrl(sticker, obj2);
    const tmpResult7 = tmp(4978);
  }
  if (str == null) {
    str = "";
  }
  ...
  if (sticker.format_type === StickersTypes.StickerFormat.LOTTIE) {
    ...
    if (sticker.format_type === tmp(5353).StickerFormat.APNG) {
      if ("type" in sticker) {
        const tmpResult8 = tmp(4978);
      }
      ...
      obj4.name = "" + sticker.id + "." + tmp(4978).getStickerExtensionFromFormatType(sticker.format_type);
      obj4.accessibilityLabel = formatToPlainStringResult;
      const obj5 = {};
      const tmpResult9 = tmp(4978);
    ...
    if (tmpResult10.isThemeDark(ThemeStore.theme)) {
      let tmp13Result = tmp13(7238);
    } else {
      tmp13Result = tmp13(7239);
    }
