...
// Function ID: 10209
// Name: Sticker
// Dependencies: [19, 17, 1183, 21, 5282, 4913, 1114, 8013, 10209, 5601, 4425, 7145, 7146, 2]
// Exports: default, getStickerAssetUrl

// Module 10208 (Sticker)
import util from "util" /* 1114 */;
import StickersTypes from "StickersTypes" /* 5282 */;
import FastImageDefault from "FastImage" /* 5601 */;
import NativeLottieViewDefault from "NativeLottieView" /* 8013 */;
import NativeAPNGViewDefault from "NativeAPNGView" /* 10209 */;
...
export default function Sticker(opaque) {
  ...
  if (sticker.format_type === StickersTypes.StickerFormat.LOTTIE) {
    let str = tmp(4913).getStickerAssetUrl(sticker);
    const tmpResult = tmp(4913);
  } else if (sticker.format_type === tmp(5282).StickerFormat.APNG) {
    const obj = { isPreview: !animated, size };
    str = tmp(4913).getStickerAssetUrl(sticker, obj);
    const tmpResult6 = tmp(4913);
  } else {
    const obj2 = { isPreview: !animated, size: PixelRatio.getPixelSizeForLayoutSize(size) };
    str = tmp(4913).getStickerAssetUrl(sticker, obj2);
    const tmpResult7 = tmp(4913);
  }
  if (str == null) {
    str = "";
  }
  ...
  if (sticker.format_type === StickersTypes.StickerFormat.LOTTIE) {
    ...
    if (sticker.format_type === tmp(5282).StickerFormat.APNG) {
      if ("type" in sticker) {
        const tmpResult8 = tmp(4913);
      }
      ...
      obj4.name = "" + sticker.id + "." + tmp(4913).getStickerExtensionFromFormatType(sticker.format_type);
      obj4.accessibilityLabel = formatToPlainStringResult;
      const obj5 = {};
      const tmpResult9 = tmp(4913);
    ...
    if (tmpResult10.isThemeDark(ThemeStore.theme)) {
      let tmp13Result = tmp13(7145);
    } else {
      tmp13Result = tmp13(7146);
    }
