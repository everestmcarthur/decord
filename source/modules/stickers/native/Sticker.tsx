...
export default function Sticker(opaque) {
  ...
  if (sticker.format_type === StickersTypes.StickerFormat.LOTTIE) {
    let str = tmp(4975).getStickerAssetUrl(sticker);
    const tmpResult = tmp(4975);
  } else if (sticker.format_type === tmp(5349).StickerFormat.APNG) {
    const obj = { isPreview: !animated, size };
    str = tmp(4975).getStickerAssetUrl(sticker, obj);
    const tmpResult6 = tmp(4975);
  } else {
    const obj2 = { isPreview: !animated, size: PixelRatio.getPixelSizeForLayoutSize(size) };
    str = tmp(4975).getStickerAssetUrl(sticker, obj2);
    const tmpResult7 = tmp(4975);
  }
  if (str == null) {
    str = "";
  }
  ...
  if (sticker.format_type === StickersTypes.StickerFormat.LOTTIE) {
    ...
    if (sticker.format_type === tmp(5349).StickerFormat.APNG) {
      if ("type" in sticker) {
        const tmpResult8 = tmp(4975);
      }
      ...
      obj4.name = "" + sticker.id + "." + tmp(4975).getStickerExtensionFromFormatType(sticker.format_type);
      obj4.accessibilityLabel = formatToPlainStringResult;
      const obj5 = {};
      const tmpResult9 = tmp(4975);
    ...
    if (tmpResult10.isThemeDark(ThemeStore.theme)) {
      let tmp13Result = tmp13(7234);
    } else {
      tmp13Result = tmp13(7235);
    }
