...
// Function ID: 4945
// Name: StickersUtils
// Dependencies: [1221, 1979, 4945, 5317, 1938, 1074, 5318, 1396, 1879, 1115, 1430, 5320, 2]
...
import StickerMessagePreviewStore from "StickerMessagePreviewStore" /* 5317 */;

require = fn;
function getStickerExtensionFromFormatType(format_type) {
  if (StickersTypes.StickerFormat.PNG === format_type) {
    const StickerExtensions = tmp(5318).StickerExtensions;
    return tmp(1396).SUPPORTS_WEBP ? StickerExtensions.WEBP : StickerExtensions.PNG;
  } else if (tmp(5318).StickerFormat.APNG === format_type) {
    return tmp(5318).StickerExtensions.APNG;
  } else if (tmp(5318).StickerFormat.LOTTIE === format_type) {
    return tmp(5318).StickerExtensions.LOTTIE;
  } else if (tmp(5318).StickerFormat.GIF === format_type) {
    return tmp(5318).StickerExtensions.GIF;
  } else {
    const _Error = Error;
...
const values = Object.values(fn(5318).StickerExtensions);
...
export const getStickerAssetUrl = (format_type) => {
  ...
  if (null == format_type.format_type) {
    ...
    if (tmp) {
      PNG = tmp23(5318).StickerFormat.PNG;
    }
    ...
    try {
      ...
      let str3 = "";
      if (tmp3 === tmp23(5318).StickerExtensions.WEBP) {
        str3 = "&quality=lossless";
      }
      if ("development" !== closure_1_12) {
        if (format_type.format_type === tmp23(5318).StickerFormat.LOTTIE) {
          const _location3 = location;
          const _HermesInternal4 = HermesInternal;
          return "" + location.protocol + ASSET_ENDPOINT + STICKER_ASSETResult;
        } else {
          let str6 = "";
          if (format_type.format_type === tmp23(5318).StickerFormat.APNG) {
            str6 = "";
            if (flag) {
              str6 = "";
...
export const shouldAttachSticker = function shouldAttachSticker(arg0, str, c1, ChannelMessage) {
  if (UploadAttachmentStore.getUploadCount(c1, ChannelMessage) > 0) {
    ...
    if (StickersTypes.StickerSelectLocation.STICKER_PICKER === arg0) {
      return "" !== str.trim();
    } else if (tmp3(5318).StickerSelectLocation.AUTOCOMPLETE === arg0) {
      return tmp3(5320).getQueriesFromUserInput(str).length > 1;
    } else {
      const BUILT_IN_INTEGRATION = tmp3(5318).StickerSelectLocation.BUILT_IN_INTEGRATION;
      return false;
    }
  }
};
