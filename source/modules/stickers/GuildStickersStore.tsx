...
// Dependencies: [32, 1973, 1980, 1979, 5320, 4259, 1983, 2]

// Module 5555 (GuildStickersStore)
...
function deriveStickerMetadata(arg0, tags) {
  const items = [];
  const obj = { type: items(5320).StickerMetadataTypes.STICKER_NAME, value: null };
  const trimmed = tags.name.trim();
  obj.value = trimmed.toLocaleLowerCase();
  items.push(obj);
  if (null != tags.tags) {
    const obj2 = { type: tmp(5320).StickerMetadataTypes.TAG, value: null };
    ...
    if (null != guild) {
      ...
      if (tmp5) {
        const obj3 = { type: tmp(5320).StickerMetadataTypes.GUILD_NAME, value: toLocaleLowerCaseResult };
        items.push(obj3);
      }
    }
    const byName = UnicodeEmojisDefault.getByName(str);
    if (null != byName) {
      const obj4 = { type: tmp(5320).StickerMetadataTypes.CORRELATED_EMOJI, value: byName.surrogates };
      items.push(obj4);
