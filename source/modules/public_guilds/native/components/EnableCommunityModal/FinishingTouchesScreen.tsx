...
// Dependencies: [32, 19, 17, 9096, 2015, 8050, 1074, 21, 4275, 576, 504, 4217, 9095, 1086, 17658, 17702, 17701, 17699, 1114, 4570, 4987, 5701, 17711, 7214, 2024, 2]
// Exports: default

// Module 17713 (FinishingTouchesScreen)
...
const PublicGuildsConstants = fn(8050);
...
export default function FinishingTouchesScreen() {
  ...
  const token = guild(4275).useToken(defaultMessageNotifications(576).modules.mobile.TABLE_ROW_PADDING);
  let obj2 = guild(4275);
  ...
  const callback1 = obj.useCallback((features) => {
    ...
    if (null != everyoneRole) {
      ...
      first(9095).saveGuild(features.id, obj4);
      if (removeResult !== everyoneRole.permissions) {
        const items = [obj2];
        guild(17658).saveRoleSettings(features.id, items);
        const obj = guild(17658);
      }
      const obj5 = first(9095);
    }
  }, []);
  const tmp11 = _slicedToArray(noop.useState(!closure_11.some((item) => PermissionUtilsAll.canEveryone(item, guild))), 2);
  const tmp19 = defaultMessageNotifications(17702)();
  const enableCommunitySharedStyles = guild(17701).useEnableCommunitySharedStyles();
  let obj4 = { headerRef: ref, currentStep: guild(17699).EnableCommunityModalSteps.STEP_3, onSuccess: callback1, disableNextStep: !first1, buttonText: null, children: null };
  ...
  obj6.children = intl2.formatToPlainString(guild(1114).t.tInpJj, { number: 3, total: 3 });
  const items2 = [closure_16(guild(4570).Text, obj6), closure_16(closure_6, { resizeMode: "contain", source: tmp19.finishingTouches }), , ];
  ...
  obj8.children = intl3.string(guild(1114).t["Pj/s/a"]);
  items2[2] = closure_16(guild(4570).Heading, obj8);
  const obj9 = { style: enableCommunitySharedStyles.description, variant: "text-md/medium", color: "text-subtle", children: null };
  const intl4 = tmp2(1114).intl;
  obj9.children = intl4.string(guild(1114).t["IL7/no"]);
  items2[3] = closure_16(guild(4570).Text, obj9);
  ...
  obj11.children = closure_16(guild(7214).TableSwitchRow, obj12);
  const items4 = [closure_16(defaultMessageNotifications(17711), obj11), ];
  ...
  const tmp4Result = defaultMessageNotifications(17711);
  ...
  obj15.children = closure_16(guild(7214).TableSwitchRow, obj16);
  items4[1] = closure_16(defaultMessageNotifications(17711), obj15);
  obj14.children = items4;
  const items5 = [closure_17(guild(5701).TableRowGroup, obj14), ];
  const obj18 = { title: null, hasIcons: false, children: null };
  const intl7 = tmp2(1114).intl;
  obj18.title = intl7.string(guild(1114).t["k+b2Cf"]);
  const obj19 = { label: null, value: null, onValueChange: null };
  const intl8 = tmp2(1114).intl;
  obj19.label = intl8.string(guild(1114).t["9AG3wI"]);
  obj19.value = first1;
  obj18.children = closure_16(guild(7214).TableSwitchRow, obj19);
  items5[1] = closure_16(guild(5701).TableRowGroup, obj18);
  obj10.children = items5;
  items3[1] = closure_17(guild(4987).Stack, obj10);
  ...
  const tmp4Result4 = defaultMessageNotifications(17711);
  ...
  items3[2] = closure_16(guild(4570).Text, obj20);
  obj4.children = items3;
  return closure_17(guild(17699).EnableCommunityModalScreen, obj4);
};
