...
// Dependencies: [32, 19, 17, 7690, 4253, 1371, 10930, 1074, 10944, 21, 4605, 576, 16875, 1896, 4951, 563, 7691, 1935, 16497, 7692, 11, 7214, 7234, 1242, 13133, 7220, 16876, 1332, 8242, 1114, 1483, 5737, 5655, 4601, 9862, 5175, 9863, 5173, 11066, 5742, 10936, 2]
// Exports: default

// Module 16874 (FriendRequestsScreen)
...
import NotificationCenterItemsActions from "NotificationCenterItemsActions" /* 16497 */;
import getPendingRelationshipIds from "getPendingRelationshipIds" /* 16876 */;
...
let closure_13 = fn(10944).MINIMUM_PENDING_INCOMING_COUNT_FOR_CLEAR_ALL;
const jsxProd = fn(21);
...
export default function FriendRequestsScreen() {
  ...
  const analyticsLocations = first(7214)(first(7234).FRIEND_REQUESTS).analyticsLocations;
  let tmp3 = first(7214);
  ...
  gameRelationshipsByType = analyticsLocations(13133).useGameRelationshipsByType(ignoredUsers.PENDING_INCOMING);
  let obj3 = analyticsLocations(13133);
  gameRelationshipsByType1 = analyticsLocations(13133).useGameRelationshipsByType(ignoredUsers.PENDING_OUTGOING);
  ...
  let obj4 = analyticsLocations(13133);
  ...
  items10[1] = obj9;
  obj7.items = items10;
  const segmentedControlState = analyticsLocations(9862).useSegmentedControlState(obj7);
  const obj10 = { value: analyticsLocations, children: null };
  const items11 = [outgoingData(first(5175), { absolute: true }), ];
  const obj11 = { style: tmp.container, children: null };
  const tmp4Result = analyticsLocations(9862);
  const items12 = [outgoingData(pendingIncomingIds, { style: tmp.tabs, children: outgoingData(analyticsLocations(9863).SegmentedControl, { state: segmentedControlState }) }), , ];
  ...
  if (tmp28) {
    ...
    obj14.children = tmp32(tmp4(4601).Text, obj15);
    obj13.children = tmp32(tmp4(5173).PressableOpacity, obj14);
