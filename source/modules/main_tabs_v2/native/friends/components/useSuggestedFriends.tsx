...
// Dependencies: [32, 19, 7713, 12769, 563, 12, 4449, 2]
// Exports: default

// Module 16128 (useSuggestedFriends)
  ...
  const memo = noop.useMemo(() => {
    if (flag) {
      ...
      return _modDef12.unionBy(found, mapped, (user) => user.user.id).sort((user, user2) => {
        const name = added(4449).getName(user.user);
        const obj = added(4449);
        return name.localeCompare(added(4449).getName(user2.user));
      });
    } else {
      return [];
    }
