...
// Dependencies: [32, 19, 17, 1074, 21, 3, 4574, 576, 1484, 4434, 11559, 11558, 16037, 4439, 16038, 4432, 4585, 1109, 16039, 4587, 16042, 4310, 5640, 7888, 16043, 6669, 16044, 16045, 16561, 16562, 16563, 16774, 2]

// Module 16036 (MainTabsNavigatorPanel)
...
import PanelsNavigationUtils from "PanelsNavigationUtils" /* 16037 */;
import useChannelScreensFromNavigation from "useChannelScreensFromNavigation" /* 16038 */;
import ChannelScreenAnimatedFrameDefault from "ChannelScreenAnimatedFrame" /* 16042 */;
    ...
    if (arg0) {
      if (null != ref2.current) {
        if ("channel" !== navigation.getState().routes[navigation.getState(navigation).index].name) {
          ...
          if ("modal" !== name) {
            tmp2.current = true;
            tmp14(4585).transitionToChannel(tmp5.current.channelId);
            const tmp14Result = tmp14(4585);
          }
          tmp14 = require;
        }
      }
