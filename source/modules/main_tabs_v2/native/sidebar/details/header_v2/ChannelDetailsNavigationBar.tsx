...
// Dependencies: [19, 17, 4213, 4214, 1957, 4755, 7890, 10948, 1074, 7891, 21, 4574, 12400, 576, 4275, 504, 1483, 8113, 1114, 9092, 7963, 12329, 12388, 4785, 7066, 10945, 8658, 7395, 16769, 4284, 4310, 4575, 16661, 4578, 11643, 5137, 5642, 2]

// Module 16768 (ChannelDetailsNavigationBar)
...
import ChannelDetailsUtils from "ChannelDetailsUtils" /* 11643 */;
import useSearchContext from "useSearchContext" /* 12329 */;
...
function MuteButton(channelId) {
  ...
  const token = channelId(4275).useToken(navigation(576).modules.mobile.CHANNEL_DETAILS_NAV_ICON_BUTTON_SIZE);
  const obj3 = channelId(4275);
  const tmp5 = navigation;
  const token1 = channelId(4275).useToken(navigation(576).modules.mobile.CHANNEL_DETAILS_NAV_ICON_BUTTON_VARIANT);
  ...
  obj5.variant = token1;
  obj5.size = token;
  obj5.icon = tmp5(stateFromStores ? 9092 : 7963);
  return closure_15(channelId(8113).IconButton, obj5, constants.MUTE);
}
function SearchButton(channelId) {
  ...
  const shouldHideChannelContent = channelId(4785).useShouldHideChannelContent(stateFromStores);
  let obj2 = channelId(4785);
  const token = channelId(4275).useToken(nativeDefault.modules.mobile.CHANNEL_DETAILS_NAV_ICON_BUTTON_SIZE);
  const obj3 = channelId(4275);
  const token1 = channelId(4275).useToken(nativeDefault.modules.mobile.CHANNEL_DETAILS_NAV_ICON_BUTTON_VARIANT);
  ...
  obj5.variant = token1;
  obj5.size = token;
  obj5.icon = _modDef7066;
  obj5.disabled = shouldHideChannelContent;
  return closure_15(channelId(8113).IconButton, obj5, constants.SEARCH);
}
function SettingsButton(channel) {
  ...
  const token = channel(4275).useToken(navigation(576).modules.mobile.CHANNEL_DETAILS_NAV_ICON_BUTTON_SIZE);
  let obj2 = channel(4275);
  const token1 = channel(4275).useToken(navigation(576).modules.mobile.CHANNEL_DETAILS_NAV_ICON_BUTTON_VARIANT);
  ...
  obj4.variant = token1;
  obj4.size = token;
  obj4.icon = navigation(7395);
  return closure_15(channel(8113).IconButton, obj4, constants.SETTINGS);
}
...
let closure_14 = fn(7891).SearchEntrypointAnalyticsLocations;
const jsxProd = fn(21);
({ jsx: closure_15, jsxs: closure_16 } = jsxProd);
const createStyles = fn(4574);
let obj = { container: { position: "relative", zIndex: 1, height: fn(12400).SEARCH_BAR_HEIGHT, marginTop: nativeDefault.space.PX_8 }, navigationHeader: null, buttonsContainer: null, searchHeader: null };
let obj3 = { position: "relative", zIndex: 1, height: fn(12400).SEARCH_BAR_HEIGHT, marginTop: nativeDefault.space.PX_8 };
obj.navigationHeader = { flexDirection: "row", alignItems: "center", paddingHorizontal: nativeDefault.space.PX_16, paddingVertical: nativeDefault.space.PX_4, position: "absolute", height: fn(12400).SEARCH_BAR_HEIGHT };
let obj4 = { flexDirection: "row", alignItems: "center", paddingHorizontal: nativeDefault.space.PX_16, paddingVertical: nativeDefault.space.PX_4, position: "absolute", height: fn(12400).SEARCH_BAR_HEIGHT };
...
let closure_24 = noop.forwardRef((cleanUp, ref) => {
  ...
  const tmp2 = transitionState !== width(4284).TransitionStates.YEETED;
  dependencyMap = tmp2;
  let tmp = closure_17();
  class S {
    constructor() {
  ...
  let obj = width(4310);
  S.__closure = { isActive: tmp2, withTiming: width(4575).withTiming, Easing: width(4310).Easing, runOnJS: width(4310).runOnJS, cleanUp, width };
  S.__workletHash = 1270940013897;
  S.__initData = __initData;
  const animatedStyle = obj.useAnimatedStyle(S);
  let obj3 = { style: null, children: closure_15(cleanUp(16661), { ref, channelId: channel.id, guildId: channel.guild_id, onSuggestionsLayoutMesure: onSuggestionsLayoutMeasure, suggestionsDismissed, setSuggestionsDismissed, showBackButton: true }) };
  const items = [tmp.searchHeader, animatedStyle];
  obj3.style = items;
  return closure_15(cleanUp(4310).View, obj3);
});
