...
// Dependencies: [19, 4630, 15205, 21, 4339, 576, 15291, 504, 4374, 5059, 8959, 2]

// Module 16514 (YouBarNameplate)
import spring from "spring" /* 5059 */;
import noop from "module_19" /* 19 */;
import AccessibilityStore from "AccessibilityStore" /* 4630 */;

require = fn;
const YOU_BAR_SPRING_CONFIG = fn(15205).YOU_BAR_SPRING_CONFIG;
const jsx = fn(21).jsx;
...
export default noop.memo(function YouBarNameplate(isQuestRendered) {
  ...
  token = isQuestRendered(4339).useToken(token(576).modules.mobile.YOU_BAR_BORDER_RADIUS);
  const tmp4 = token(15291)(token);
  dependencyMap = tmp4;
  let obj = isQuestRendered(4339);
  const tmp2 = token;
  ...
  const obj3 = isQuestRendered(4374);
  fn.__closure = { withSpring: isQuestRendered(5059).withSpring, isQuestRendered, questDockAnimatedBorderRadius: tmp4, borderRadius: token, YOU_BAR_SPRING_CONFIG };
  ...
  const obj4 = { withSpring: isQuestRendered(5059).withSpring, isQuestRendered, questDockAnimatedBorderRadius: tmp4, borderRadius: token, YOU_BAR_SPRING_CONFIG };
  if (str) {
    str = "always";
  }
  obj6.animate = str;
  obj5.children = jsx(tmp2(8959), { nameplate, isFocused: true, animate: null });
  return jsx(token(4374).View, { style: null, pointerEvents: "none", children: null });
});
