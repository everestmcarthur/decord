...
// Function ID: 16461
// Name: YouBar
// Dependencies: [5, 32, 19, 17, 4456, 4458, 1371, 15179, 1074, 1954, 21, 4636, 576, 1477, 15172, 11658, 4497, 15178, 15182, 4373, 5055, 8470, 16461, 14804, 504, 8334, 16463, 4495, 4604, 1369, 12, 16466, 1896, 4494, 11352, 7445, 1943, 1114, 7485, 16107, 6756, 1176, 16480, 16481, 16482, 16487, 16490, 4347, 16492, 16493, 2]
...
import getNavigatorCurrentRouteDefault from "getNavigatorCurrentRoute" /* 11352 */;
import YouBarFloatingShadeDefault from "YouBarFloatingShade" /* 16492 */;
import ConnectionBannerDefault from "ConnectionBanner" /* 16493 */;
...
let closure_26 = noop.memo(() => {
  let tmp = closure_21();
  const mobileQuestDock = isMobileQuestDockRenderedBase(15172).useMobileQuestDock();
  let obj = isMobileQuestDockRenderedBase(15172);
  isMobileQuestDockRenderedBase = isMobileQuestDockRenderedBase(15172).useIsMobileQuestDockRenderedBase(mobileQuestDock);
  let obj2 = isMobileQuestDockRenderedBase(15172);
  const drawerWidth = isMobileQuestDockRenderedBase(11658).useDrawerWidth();
  let obj3 = isMobileQuestDockRenderedBase(11658);
  const youBarHorizontalMargin = isMobileQuestDockRenderedBase(15178).useYouBarHorizontalMargin();
  let result = 2 * youBarHorizontalMargin;
  const tmp10 = youBarHorizontalMargin(4497)().isChatBesideChannelList ? drawerWidth - result : youBarHorizontalMargin(1477)().width - result;
  dependencyMap = tmp10;
  let obj4 = isMobileQuestDockRenderedBase(15178);
  const youBarBottomMargin = isMobileQuestDockRenderedBase(15178).useYouBarBottomMargin();
  const tmp4Result = isMobileQuestDockRenderedBase(15178);
  const connectionBannerHeight = isMobileQuestDockRenderedBase(15182).useConnectionBannerHeight();
  let items = [tmp10, youBarHorizontalMargin];
  const memo = sharedValue.useMemo(() => {
    const size = { marginHorizontal: youBarHorizontalMargin, height, padding, width };
    return size;
  }, items);
  const tmp4Result11 = isMobileQuestDockRenderedBase(15182);
  sharedValue = isMobileQuestDockRenderedBase(4373).useSharedValue(false);
  const tmp4Result12 = isMobileQuestDockRenderedBase(4373);
  let fn = function r() {
  ...
  const tmp4Result13 = isMobileQuestDockRenderedBase(4373);
  fn.__closure = { withSpring: isMobileQuestDockRenderedBase(5055).withSpring, barMarginBottom: youBarBottomMargin, connectionBannerHeight, YOU_BAR_SPRING_CONFIG: callback3, isPressedValue: sharedValue, isQuestRendered: isMobileQuestDockRenderedBase };
  fn.__workletHash = 7314807713815;
  fn.__initData = __initData;
  const animatedStyle = tmp4Result13.useAnimatedStyle(fn);
  const obj5 = { withSpring: isMobileQuestDockRenderedBase(5055).withSpring, barMarginBottom: youBarBottomMargin, connectionBannerHeight, YOU_BAR_SPRING_CONFIG: callback3, isPressedValue: sharedValue, isQuestRendered: isMobileQuestDockRenderedBase };
  const iCYMIEnabled = isMobileQuestDockRenderedBase(8470).useICYMIEnabled("TabsNavigator");
  const tmp4Result14 = isMobileQuestDockRenderedBase(8470);
  const youBarCoachmark = isMobileQuestDockRenderedBase(16461).useYouBarCoachmark({ isQuestRendered: isMobileQuestDockRenderedBase });
  const visibleContent = youBarCoachmark.visibleContent;
  const markAsDismissed = youBarCoachmark.markAsDismissed;
  const tmp4Result15 = isMobileQuestDockRenderedBase(16461);
  const showTinyBroncoPromoSheet = isMobileQuestDockRenderedBase(14804).useShowTinyBroncoPromoSheet({ visibleContent, markAsDismissed });
  ...
  const tmp4Result16 = isMobileQuestDockRenderedBase(14804);
  ...
  const nameplate = isMobileQuestDockRenderedBase(8334).useNameplate({ user: stateFromStores });
  const tmp4Result18 = isMobileQuestDockRenderedBase(8334);
  const youBarAccessibilityLabel = isMobileQuestDockRenderedBase(16463).useYouBarAccessibilityLabel(stateFromStores);
  ...
  callback3 = sharedValue.useCallback(() => {
    ...
    if (null != coerceGuildsRouteResult) {
      const result = tmp(4604).triggerHapticFeedback(tmp(4604).HapticFeedbackTypes.SOFT);
      ...
      if (guildId === ME) {
        ...
        if (null != lastSelectedGuildId) {
          tmp(7445).transitionToGuild(lastSelectedGuildId);
        ...
        tmp(7445).transitionToGuild(tmp5);
  ...
  const context = sharedValue.useContext(tmp2(16107));
  const gesture = context.gesture;
  const translateX = context.translateX;
  const tmp4Result19 = isMobileQuestDockRenderedBase(16463);
  hitSlop = isMobileQuestDockRenderedBase(4373).useSharedValue(0);
  ...
  const AVATAR_SIZE_MAP = tmp4(1176).AVATAR_SIZE_MAP;
  if (isMobileQuestDockRenderedBase) {
  ...
  obj6.style = items9;
  const items10 = [closure_19(youBarHorizontalMargin(16480), { hasNameplate: null != nameplate, isLargeAvatar: !isMobileQuestDockRenderedBase, barWidth: tmp10, isQuestRendered: isMobileQuestDockRenderedBase, avatarSize: tmp37 }), , , ];
  let tmp40Result = tmp22;
  if (null != nameplate) {
    const obj7 = { nameplate, barWidth: tmp10, isQuestRendered: isMobileQuestDockRenderedBase, avatarSize: tmp37 };
    tmp40Result = tmp40(tmp2(16481), obj7);
  }
  ...
  obj9.accessibilityHint = intl.string(isMobileQuestDockRenderedBase(1114).t.cSgdvE);
  ...
  obj9.children = closure_19(youBarHorizontalMargin(16482), { isQuestRendered: isMobileQuestDockRenderedBase, onAvatarPress: memo1 });
  obj8.children = closure_19(markAsDismissed, obj9);
  items10[2] = closure_19(isMobileQuestDockRenderedBase(6756).GestureDetector, obj8);
  const obj10 = { style: tmp.youRowRight, children: null };
  let tmp40Result2 = null;
  if (iCYMIEnabled) {
    const obj11 = { hasNameplate: tmp22 };
    tmp40Result2 = tmp40(tmp2(16487), obj11);
  }
  const items11 = [tmp40Result2, closure_19(youBarHorizontalMargin(16490), { hasNameplate: null != nameplate })];
  obj10.children = items11;
  items10[3] = closure_20(visibleContent, obj10);
  obj6.children = items10;
  return closure_20(youBarHorizontalMargin(4373).View, obj6);
});
