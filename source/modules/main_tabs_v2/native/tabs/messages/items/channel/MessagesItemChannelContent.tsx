...
export default noop.memo(function MessagesItemChannelContent(channel) {
  ...
  const isThemeLightResult = channel(4315).isThemeLight(useThemeDefault());
  ...
  if (tmp12) {
    const obj3 = tmp3(4198)();
    tmp12 = tmp3(4198)().diff(tmp11.timestamp, "hours") < 1 || !hasActivity || hasUnreadMessages;
  ...
  obj4.variant = channel(4308).useToken(nativeDefault.modules.mobile.MESSAGES_ITEM_CHANNEL_NAME_TEXT_STYLE);
  const items1 = [, ];
  ...
  if (!hasNameplate) {
    ...
    if (waveShouldShow) {
      ...
      tmp20Result8 = tmp20(tmp3(16110), obj22);
    }
