...
// Dependencies: [19, 17, 4654, 4821, 21, 4639, 576, 1177, 8037, 7074, 10275, 11091, 4348, 4574, 504, 15413, 4231, 8495, 16146, 11, 4792, 16147, 4341, 11032, 11033, 4635, 9316, 9570, 10239, 7986, 11009, 1115, 16149, 2]

// Module 16145 (MessagesItemChannelContent)
...
import _modDef8037 from "module_8037" /* 8037 */;
...
import usePrivateChannelWaveDefault from "usePrivateChannelWave" /* 16147 */;
...
function MessagesItemChannelContentIcon(selected) {
  ...
  if (blocked) {
    const obj2 = { source: _modDef8037, size: native.Icon.Sizes.EXTRA_SMALL, style: null };
    const items = [, ];
    ({ channelIcon: arr4[0], channelMutedIcon: arr4[1] } = tmp);
    obj2.style = items;
    let tmp2 = timestampProducer(native.Icon, obj2);
  } else if (ignored) {
    const obj3 = { source: _modDef7074, size: native.Icon.Sizes.EXTRA_SMALL, style: null };
    const items1 = [, ];
    ({ channelIcon: arr3[0], channelIgnoredIcon: arr3[1] } = tmp);
    obj3.style = items1;
    tmp2 = timestampProducer(native.Icon, obj3);
  } else if (muted) {
    const obj4 = { source: _modDef10275, size: native.Icon.Sizes.EXTRA_SMALL, style: null };
    const items2 = [, ];
    ...
    tmp2 = null;
    if (favorite) {
      const obj = { source: _modDef11091, size: native.Icon.Sizes.EXTRA_SMALL, style: null };
      const items3 = [, ];
...
export default noop.memo(function MessagesItemChannelContent(channel) {
  ...
  if (tmp12) {
    const obj3 = tmp3(4231)();
    tmp12 = tmp3(4231)().diff(tmp11.timestamp, "hours") < 1 || !hasActivity || hasUnreadMessages;
    const tmp13 = tmp3(4231)().diff(tmp11.timestamp, "hours") < 1 || !hasActivity || hasUnreadMessages;
  }
  if (tmp12) {
    tmp12 = !tmp3(8495)(channel.id);
  }
  const tmpResult = channel(504);
  const tmpResult3 = channel(16146);
  ...
  obj9.children = tmp16;
  const items2 = [closure_6(channel(4635).Text, obj9), , ];
  let tmp20Result = null;
  if (tmp15) {
    const obj10 = { userId: channel.recipients[0], disabledTooltip: true };
    tmp20Result = tmp20(tmp3(9316), obj10);
  }
  items2[1] = tmp20Result;
  let tmp20Result5 = null;
  if (channel.isSystemDM()) {
    const obj11 = { style: tmp8.botTag, type: tmp3(9570).Types.SYSTEM_DM, verified: true };
    tmp20Result5 = tmp20(tmp3(9570), obj11);
    const tmp3Result5 = tmp3(9570);
  }
  ...
  if (!hasNameplate) {
    ...
    if (!waveShouldShow) {
      ...
      tmp20Result6 = tmp20(tmp(4635).Text, obj17);
    }
    ...
    if (tmp12) {
      ...
      obj19.color = str7;
      obj19.layout = tmp(7986).ChannelListLayoutTypes.COZY_DRAWER_SMOL;
      obj19.muted = muted;
      let tmp20Result7 = tmp20(tmp(10239).ChannelRowPreview, obj19);
      ...
      tmp20Result7 = tmp20(tmp3(11009), obj20);
      const tmp3Result6 = tmp3(11009);
    } else {
      tmp20Result7 = null;
      if (tmp3(8495)(channel.id)) {
        ...
        tmp20Result7 = tmp20(tmp(4635).Text, obj21);
      }
    }
    ...
    if (waveShouldShow) {
      ...
      tmp20Result8 = tmp20(tmp3(16149), obj22);
    }
