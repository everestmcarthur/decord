...
// Dependencies: [19, 16155, 16167, 16205, 16206, 2]
// Exports: default

// Module 16166 (useMessagesFlatData)
...
export default function useMessagesFlatData(channels, listItemHeight) {
  ...
  return channelFavorites.useMemo(() => {
    if (renderHeader === useMessagesData.MessagesDataHeader.HappeningNow) {
      let listHeaderHeight = tmp2(16167).getMessagesItemHappeningNowHeight();
      const tmp2Result = tmp2(16167);
    } else {
      listHeaderHeight = 0;
      if (renderHeader === tmp2(16155).MessagesDataHeader.EmptyState) {
        listHeaderHeight = tmp2(16205).MESSAGES_ITEM_EMPTY_STATE_HEIGHT;
      }
    }
    const listData = [];
    ...
    if (sections[useMessagesData.MessagesDataSections.Separator] > 0) {
      ...
      sum2 = tmp12 + tmp17(16206).MESSAGES_ITEM_SEPERATOR_HEIGHT;
    }
