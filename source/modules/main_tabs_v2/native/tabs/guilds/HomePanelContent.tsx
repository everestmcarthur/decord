...
// Dependencies: [19, 17, 16273, 1074, 16539, 21, 4722, 16279, 16540, 4458, 11245, 5294, 16282, 8078, 4423, 576, 1365, 5138, 12140, 2]

// Module 16538 (HomePanelContent)
...
import GuildsBarDefault from "GuildsBar" /* 16540 */;
...
function ContentMaskGradient(offsetX) {
  ...
  const guildsBarPullX = offsetX(16279).useHomeDrawerState().guildsBarPullX;
  let obj = offsetX(16279);
  const fn = function n() {
  ...
  let obj2 = offsetX(4458);
  fn.__closure = { roundToNearestPixel: guildsBarPullX(11245), offsetX, guildsBarPullX };
  fn.__workletHash = 7539125302557;
  fn.__initData = __initData;
  const animatedStyle = obj2.useAnimatedStyle(fn);
  const obj4 = { pointerEvents: "none", style: null, children: closure_9(guildsBarPullX(5294), { absolute: true, tall: true, wide: true, mix: true }) };
  let items = [absoluteFill.absoluteFill, animatedStyle];
  obj4.style = items;
  return closure_9(guildsBarPullX(4458).View, obj4);
}
function HomeDrawerPanelContent() {
  const tmp2 = closure_11();
  const drawerOpen = ref(16282).useDrawerOpen();
  let obj = ref(16282);
  const tmp3 = ref;
  const doesLandOnHomeDrawer = ref(16279).useDoesLandOnHomeDrawer();
  ref = isClientThemeOrCustomThemeActive.useRef(null);
  let obj2 = ref(16279);
  const homeDrawerState = ref(16279).useHomeDrawerState();
  const panelTranslateX = homeDrawerState.panelTranslateX;
  const tmp9 = HomeDrawerStore((maxX) => maxX.maxX);
  dependencyMap = tmp9;
  let obj3 = ref(16279);
  isClientThemeOrCustomThemeActive = ref(8078).useIsClientThemeOrCustomThemeActive();
  let obj4 = ref(8078);
  const token = ref(4423).useToken(panelTranslateX(576).colors.BACKGROUND_BASE_LOWEST);
  const obj5 = ref(4423);
  const token1 = ref(4423).useToken(panelTranslateX(576).colors.PANEL_BG);
  const obj6 = ref(4423);
  const fn = function n() {
  ...
  const obj7 = ref(4458);
  fn.__closure = { isGradientTheme: isClientThemeOrCustomThemeActive, maxX: tmp9, interpolateColor: ref(4458).interpolateColor, panelTranslateX, baseLowest: token, panelBg: token1 };
  ...
  const obj8 = { isGradientTheme: isClientThemeOrCustomThemeActive, maxX: tmp9, interpolateColor: ref(4458).interpolateColor, panelTranslateX, baseLowest: token, panelBg: token1 };
  ...
  const obj9 = ref(4458);
  fn2.__closure = { interpolate: ref(4458).interpolate, panelTranslateX, INITIAL_OPEN_WIDTH: ref(16279).INITIAL_OPEN_WIDTH, Extrapolation: ref(4458).Extrapolation, isGradientTheme: isClientThemeOrCustomThemeActive, interpolateColor: ref(4458).interpolateColor, maxX: tmp9, baseLowest: token, panelBg: token1 };
  ...
  obj12.style = items2;
  const items3 = [closure_9(panelTranslateX(16540), { enableHome: true }), , ];
  ...
  items3[1] = closure_9(panelTranslateX(4458).View, obj13);
  let tmp17Result2 = null;
  if (doesLandOnHomeDrawer) {
    tmp17Result2 = tmp17(tmp3(12140).TTIFirstContentfulPaint, { label: "home_drawer", checkFocusedScreen: "guilds" });
  }
  items3[2] = tmp17Result2;
  obj12.children = items3;
  obj11.children = closure_10(panelTranslateX(4458).View, obj12);
  return closure_9(panelTranslateX(4458).View, obj11);
}
