...
// Dependencies: [32, 19, 21, 16476, 16477, 4603, 5766, 5769, 10946, 2]
// Exports: default

// Module 16475 (YouSwitchClientsRadioGroup)
import ActionSheetActionCreatorsDefault from "ActionSheetActionCreators" /* 4603 */;
import DiscordVariants from "DiscordVariants" /* 16476 */;
import DiscordVariantTypes from "DiscordVariantTypes" /* 16477 */;
...
export default function YouSwitchClientsRadioGroup() {
  const memo = noop.useMemo(() => memo(16476).getCurrentVariant(), []);
  [arr, importDefault] = noop.useState(null);
  const items = [memo];
  const effect = noop.useEffect(() => {
    if (null != c0) {
      c0 = false;
      let DISCORD_VARIANT_LIST = memo(16477).DISCORD_VARIANT_LIST;
      ...
      if (arr.length >= 2) {
        let obj = {
          ...
          children: arr.map((value) => {
                  const obj = { value, label: memo(16477).DISCORD_VARIANTS[value].label, icon: jsx(memo(10946).ClydeIcon, { color: memo(16477).DISCORD_VARIANTS[value].color }) };
                  return jsx(memo(5769).TableRadioRow, { value, label: memo(16477).DISCORD_VARIANTS[value].label, icon: jsx(memo(10946).ClydeIcon, { color: memo(16477).DISCORD_VARIANTS[value].color }) }, value);
                })
        };
