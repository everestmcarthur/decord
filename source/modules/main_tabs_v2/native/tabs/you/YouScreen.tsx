...
// Function ID: 17032
// Name: YouScreen
// Dependencies: [32, 19, 17, 7735, 2026, 14812, 7750, 1372, 16530, 1074, 1076, 1955, 7322, 21, 4376, 8392, 4640, 1365, 576, 672, 4498, 563, 1115, 5210, 16563, 8377, 8924, 1249, 8334, 1612, 8374, 4574, 8390, 4500, 1478, 1936, 1485, 17032, 1397, 8394, 7057, 8338, 5213, 8349, 8361, 7296, 8371, 1944, 7502, 5675, 17033, 12118, 7270, 17036, 17037, 17038, 4350, 17046, 8368, 8355, 8384, 1092, 10800, 8958, 17048, 12048, 8335, 8932, 7496, 7655, 11274, 7589, 2]
// Exports: default

// Module 17031 (YouScreen)
...
import VisualEffectViewThemedDefault from "VisualEffectViewThemed" /* 8392 */;
import BackIconWithBadge from "BackIconWithBadge" /* 16563 */;
...
function UnconnectedYouScreen(arg0) {
  ...
  const tmp7Result = navigateToShop(8377)(tmp8);
  _slicedToArray = tmp7Result;
  const tmp10 = closure_23(tmp7Result);
  const obj2 = { type: null, name: null };
  const tmp7 = navigateToShop(8377);
  obj2.type = navigateToSettings(1249).ImpressionTypes.VIEW;
  obj2.name = navigateToSettings(1249).ImpressionNames.USER_YOU_SCREEN;
  navigateToShop(8924)(obj2);
  const obj3 = navigateToShop(8334)(user.id);
  rect = tmp5(1612)();
  const tmp5Result = navigateToShop(8924);
  ({ theme, primaryColor, secondaryColor } = navigateToShop(8374)({ user, displayProfile: obj3 }));
  const tmp14 = navigateToShop(8374)({ user, displayProfile: obj3 });
  const ref = rect.useRef(null);
  const tmp15 = navigateToShop(4574)();
  sharedValue = navigateToSettings(4376).useSharedValue(0);
  const obj4 = navigateToSettings(4376);
  ...
  const obj5 = navigateToSettings(4376);
  const animatedScrollHandler = navigateToSettings(4376).useAnimatedScrollHandler(fn);
  ({ bannerAnimatedStyle, bannerImageAnimatedStyle, contentAnimatedStyle, blurAnimatedProps, showBlur } = navigateToShop(8390)({ scrollPosition: sharedValue, bannerHeight: tmp7Result }));
  let size = tmp5(1478)();
  const height = size.height;
  let num = 0;
  const diff = size.width - rect.right - rect.left;
  if (navigateToShop(4500)().isChatBesideChannelList) {
    num = 16;
  }
  ...
  const GifAutoPlay = tmp12(1936).GifAutoPlay;
  const setting = GifAutoPlay.getSetting();
  const tmp19 = navigateToShop(8390)({ scrollPosition: sharedValue, bannerHeight: tmp7Result });
  ...
  tmp12Result14 = navigateToSettings(17032);
  ...
  if (isFocused) {
    ...
    let tmp37 = null == obj3;
    const userProfileBannerBackgroundColor = tmp12(8394).useUserProfileBannerBackgroundColor(obj8);
    if (!tmp37) {
      tmp37 = !obj3.isLoaded;
    }
    let items = [tmp7Result, bound, rect.bottom];
    const tmp12Result17 = tmp12(8394);
    ...
    const tmp38 = tmp5(7057)();
    const obj9 = { layout: "YOU_SCREEN", userId: user.id };
    const createUserProfileAnalyticsContext = tmp12(8338).useCreateUserProfileAnalyticsContext(obj9);
    const tmp12Result18 = tmp12(8338);
    const isScreenLandscape = tmp12(5213).useIsScreenLandscape();
    let tmp43;
    const tmp12Result19 = tmp12(5213);
    ...
    const tmp5Result1Result = tmp5(8349)(tmp43);
    let tmp47;
    const tmp5Result7 = tmp5(8349);
    if (!isScreenLandscape) {
      let skuId1;
      if (obj3 != null) {
    ...
    if (!tmp38) {
      ...
      const tmpResult4 = tmp(tmp12(7502).useSelectedDismissibleContent(memo1), 2);
      closure_10 = tmp58;
      ...
      const tmp12Result21 = tmp12(7502);
      const obj11 = { disabled: tmp63 };
      youSettingsCoachmark = tmp12(17033).useYouSettingsCoachmark(obj11);
      let tmp65 = null != youSettingsCoachmark;
      const tmp12Result22 = tmp12(17033);
      const customTypingIndicatorConfig = tmp12(12118).useCustomTypingIndicatorConfig("YouScreen");
      if ("settings" === customTypingIndicatorConfig.entryPoint) {
        if (customTypingIndicatorConfig.canSet) {
          if (null != obj3) {
            if (!tmp63) {
              if (!tmp65) {
                let items5 = [tmp12(1944).DismissibleContent.CUSTOM_TYPING_INDICATOR_MOBILE_PROFILE_COACHMARK];
              }
              const tmpResult5 = tmp(tmp12(7502).useSelectedDismissibleContent(items5), 2);
              closure_13 = tmp68;
              const tmp69 = tmpResult5[0] === tmp12(1944).DismissibleContent.CUSTOM_TYPING_INDICATOR_MOBILE_PROFILE_COACHMARK;
              nativeID = tmp69;
              let tmp70 = tmp63;
              if (!tmp63) {
                tmp70 = tmp65;
              ...
              const tmp12Result24 = tmp12(7502);
              ...
              const tmp85 = closure_20(tmp5(17038), obj16);
              if (tmp84Result) {
                const obj20 = { frame: tmp5Result1Result, profileThemeType: UserProfileThemeTypes.YOU_SCREEN, frameOrder: tmp12(8355).ProfileFrameLayerOrder.BACK, containerWidth: bound };
                tmp84Result = tmp84(tmp5(8368), obj20);
                const tmp5Result11 = tmp5(8368);
              }
              const items10 = [tmp84Result, , , , , ];
              const obj21 = { gradientHeight: height, bannerHeight: tmp7Result, style: tmp10.background };
              items10[1] = closure_20(tmp5(8384), obj21);
              ...
              if (null == source) {
                ...
                items12[1] = tmp89(tmp5(4376).View, obj26);
                obj23.children = items12;
                const items16 = [tmp89(tmp5(4376).View, obj23), , , ];
                if (tmp89Result) {
                ...
                items16[2] = tmp84(tmp5(17048), obj33);
                items16[3] = tmp84(tmp12(12048).TTIFirstContentfulPaint, { label: "you_screen" });
                obj22.children = items16;
                items10[2] = tmp89(tmp95, obj22);
                let tmp84Result7 = null != tmp5Result1Result;
                if (tmp84Result7) {
                  const obj34 = { frame: tmp5Result1Result, profileThemeType: UserProfileThemeTypes.YOU_SCREEN, frameOrder: tmp12(8355).ProfileFrameLayerOrder.FRONT, containerWidth: bound };
                  tmp84Result7 = tmp84(tmp5(8368), obj34);
                  const tmp5Result12 = tmp5(8368);
                }
                ...
                obj17.children = tmp84(tmp12(8338).UserProfileAnalyticsProvider, obj18);
                obj35.children = tmp84(tmp12(4350).ThemeContextProvider, obj17);
                return tmp84(tmp12(7270).LayerScope, obj35);
                ...
                const items20 = [tmp84(tmp5(5675), obj37), ];
                ...
                if (!obj38) {
                  ...
                  tmp84Result8 = tmp84(tmp12(10800).Caption, obj38);
                }
                items20[1] = tmp84Result8;
                obj36.children = items20;
                let tmp89Result2 = tmp89(tmp12(5210).PressableOpacity, obj36);
              } else {
                ...
                tmp89Result2 = tmp84(tmp5(5675), obj39);
              }
      ...
      items5 = [];
      const tmp12Result23 = tmp12(12118);
    }
    bound1 = youSettingsCoachmark;
    const tmp5Result8 = tmp5(8361);
  }
  if (!isFocused) {
    ref3.current = false;
  }
