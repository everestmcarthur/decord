...
// Dependencies: [19, 17, 4590, 1957, 1908, 4222, 1371, 4593, 7890, 1074, 4595, 2010, 7891, 4599, 21, 1178, 4574, 576, 504, 11427, 13287, 7995, 13288, 10901, 13158, 4920, 13289, 1114, 13290, 12329, 12388, 4754, 4541, 13304, 1896, 5137, 7065, 13305, 8402, 4268, 1242, 12986, 7394, 4781, 5115, 7896, 7894, 4570, 13306, 10142, 2]

// Module 13286 (PrivateChannelButtons)
...
import getPrivateChannelCallDefault from "getPrivateChannelCall" /* 10901 */;
...
import ConfirmStartCall from "ConfirmStartCall" /* 13158 */;
import VoicePanelVideoGuardErrorAlert from "VoicePanelVideoGuardErrorAlert" /* 13289 */;
import ChannelHeader from "ChannelHeader" /* 13290 */;
...
const NO_PARTICIPANTS = fn(4590).NO_PARTICIPANTS;
let closure_12 = fn(7890).setIsChannelDetailsSearchActive;
...
let closure_18 = fn(7891).SearchEntrypointAnalyticsLocations;
...
export default noop.memo(function PrivateChannelButtons(channelId) {
  ...
  inappropriateConversationSafetyToolsWarningForChannel = channelId(11427).useInappropriateConversationSafetyToolsWarningForChannel(channelId);
  closure_6 = tmp7;
  const tmp2Result6 = channelId(11427);
  ...
  const VideoGuardExperiment = tmp2(13287).VideoGuardExperiment;
  ...
  const isCallActiveNullable = channelId(7995).useIsCallActiveNullable(id);
  const tmp2Result9 = channelId(7995);
  ...
  closure_11 = tmp15;
  const tmp17 = screenIndex(13288)({ context: { type: "channel", channel: stateFromStores } });
  ...
  const callback3 = obj9.useCallback(() => {
    ...
    if (tmp2) {
      ...
      ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(13304, dependencyMap.paths), "AppDMOptionsBottomSheet", obj4);
    }
  }, items11);
  ...
  const memo1 = obj9.useMemo(() => {
    if (closure_6) {
      ...
      let tmpResult = tmp(tmp2(13305).SafetyToolsButton, obj3);
    } else {
      ...
      tmpResult = tmp(tmp2(5137).PressableOpacity, obj);
    }
    return tmpResult;
  }, items12);
  if (screenIndex(8402)(channelId)) {
    let obj3 = {
      style: tmp.button,
      onPress() {
    ...
    obj3.children = closure_20(tmp2(12986).WindowLaunchIcon, { size: "sm" });
    let tmp25Result = closure_20(tmp2(5137).PressableOpacity, obj3);
    ...
    if (null != application) {
      ...
      obj5.children = closure_20(tmp2(7065).MagnifyingGlassIcon, { size: "sm" });
      const items13 = [closure_20(tmp2(5137).PressableOpacity, obj5), ];
      ...
      obj6.children = closure_20(tmp2(7394).SettingsIcon, { size: "sm" });
      items13[1] = closure_20(tmp2(5137).PressableOpacity, obj6);
    ...
    if (!tmp15) {
      ...
      if (tmp15) {
        ...
        tmp30(tmp2(5115).VoiceNormalIcon, unsafe_rawColors1);
        ...
        if (stateFromStores1) {
          ...
          let tmp30Result2 = tmp30(tmp2(7896).PhoneHangUpIcon, obj11);
          let tmp33 = tmp30;
        } else {
          let GREEN_360;
          ...
          tmp30Result2 = tmp30(tmp2(7894).PhoneCallIcon, obj12);
          tmp33 = tmp30;
        }
        const items15 = [
          tmp30Result2,
        ...
        if (tmp33Result) {
          ...
          obj13.children = tmp25(tmp2(4570).Text, obj14);
          tmp33Result = tmp33(tmp26, obj13);
        }
        items15[2] = tmp33Result;
        obj8.children = items15;
        const items17 = [tmp25(tmp2(5137).PressableOpacity, obj8), , ];
        let tmp38 = null;
        if (!isMultiUserDMResult) {
          tmp38 = null;
          if (!stateFromStores1) {
            ...
            if (videoEnabled) {
              let VideoDenyIcon = tmp2(10142).VideoIcon;
            } else {
              VideoDenyIcon = tmp2(13306).VideoDenyIcon;
            }
            obj15.children = tmp33(VideoDenyIcon, { size: "sm" });
            tmp33(tmp2(5137).PressableOpacity, obj15);
          }
        }
        items17[1] = tmp38;
        items17[2] = memo1;
