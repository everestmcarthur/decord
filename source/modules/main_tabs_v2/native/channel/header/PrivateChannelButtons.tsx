...
// Dependencies: [19, 17, 4621, 1957, 1908, 4253, 1371, 4624, 7928, 1074, 4626, 2010, 7929, 4630, 21, 1178, 4605, 576, 504, 11467, 13335, 8033, 13336, 10939, 13205, 4951, 13337, 1114, 13338, 12369, 12428, 4785, 4572, 13353, 1896, 5173, 7103, 13354, 8439, 4299, 1242, 13026, 7432, 4812, 5151, 7934, 7932, 4601, 13355, 10180, 2]

// Module 13334 (PrivateChannelButtons)
...
import ConfirmStartCall from "ConfirmStartCall" /* 13205 */;
import VoicePanelVideoGuardErrorAlert from "VoicePanelVideoGuardErrorAlert" /* 13337 */;
import ChannelHeader from "ChannelHeader" /* 13338 */;
...
const NO_PARTICIPANTS = fn(4621).NO_PARTICIPANTS;
let closure_12 = fn(7928).setIsChannelDetailsSearchActive;
...
let closure_18 = fn(7929).SearchEntrypointAnalyticsLocations;
...
export default noop.memo(function PrivateChannelButtons(channelId) {
  ...
  inappropriateConversationSafetyToolsWarningForChannel = channelId(11467).useInappropriateConversationSafetyToolsWarningForChannel(channelId);
  closure_6 = tmp7;
  const tmp2Result6 = channelId(11467);
  ...
  const VideoGuardExperiment = tmp2(13335).VideoGuardExperiment;
  ...
  const isCallActiveNullable = channelId(8033).useIsCallActiveNullable(id);
  const tmp2Result9 = channelId(8033);
  ...
  closure_11 = tmp15;
  const tmp17 = screenIndex(13336)({ context: { type: "channel", channel: stateFromStores } });
  ...
  const callback3 = obj9.useCallback(() => {
    ...
    if (tmp2) {
      ...
      ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(13353, dependencyMap.paths), "AppDMOptionsBottomSheet", obj4);
    }
  }, items11);
  ...
  const memo1 = obj9.useMemo(() => {
    if (closure_6) {
      ...
      let tmpResult = tmp(tmp2(13354).SafetyToolsButton, obj3);
    } else {
      ...
      tmpResult = tmp(tmp2(5173).PressableOpacity, obj);
    }
    return tmpResult;
  }, items12);
  if (screenIndex(8439)(channelId)) {
    let obj3 = {
      style: tmp.button,
      onPress() {
    ...
    obj3.children = closure_20(tmp2(13026).WindowLaunchIcon, { size: "sm" });
    let tmp25Result = closure_20(tmp2(5173).PressableOpacity, obj3);
    ...
    if (null != application) {
      ...
      obj5.children = closure_20(tmp2(7103).MagnifyingGlassIcon, { size: "sm" });
      const items13 = [closure_20(tmp2(5173).PressableOpacity, obj5), ];
      ...
      obj6.children = closure_20(tmp2(7432).SettingsIcon, { size: "sm" });
      items13[1] = closure_20(tmp2(5173).PressableOpacity, obj6);
    ...
    if (!tmp15) {
      ...
      if (tmp15) {
        ...
        tmp30(tmp2(5151).VoiceNormalIcon, unsafe_rawColors1);
        ...
        if (stateFromStores1) {
          ...
          let tmp30Result2 = tmp30(tmp2(7934).PhoneHangUpIcon, obj11);
          let tmp33 = tmp30;
        } else {
          let GREEN_360;
          ...
          tmp30Result2 = tmp30(tmp2(7932).PhoneCallIcon, obj12);
          tmp33 = tmp30;
        }
        const items15 = [
          tmp30Result2,
        ...
        if (tmp33Result) {
          ...
          obj13.children = tmp25(tmp2(4601).Text, obj14);
          tmp33Result = tmp33(tmp26, obj13);
        }
        items15[2] = tmp33Result;
        obj8.children = items15;
        const items17 = [tmp25(tmp2(5173).PressableOpacity, obj8), , ];
        let tmp38 = null;
        if (!isMultiUserDMResult) {
          tmp38 = null;
          if (!stateFromStores1) {
            ...
            if (videoEnabled) {
              let VideoDenyIcon = tmp2(10180).VideoIcon;
            } else {
              VideoDenyIcon = tmp2(13355).VideoDenyIcon;
            }
            obj15.children = tmp33(VideoDenyIcon, { size: "sm" });
            tmp33(tmp2(5173).PressableOpacity, obj15);
          }
        }
        items17[1] = tmp38;
        items17[2] = memo1;
