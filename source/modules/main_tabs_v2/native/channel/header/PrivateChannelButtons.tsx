...
// Dependencies: [19, 17, 4738, 1958, 1909, 4371, 1372, 4741, 8081, 1074, 4743, 2011, 8082, 4747, 21, 1177, 4722, 576, 504, 11647, 13507, 8200, 13508, 11117, 13377, 5068, 13509, 1115, 13510, 12542, 12601, 4902, 4689, 13525, 1897, 5292, 7247, 13526, 8606, 4417, 1241, 13199, 7576, 4929, 5270, 8087, 8085, 4718, 13527, 10353, 2]

// Module 13506 (PrivateChannelButtons)
...
import MagnifyingGlassIcon from "MagnifyingGlassIcon" /* 7247 */;
import getPrivateChannelCallDefault from "getPrivateChannelCall" /* 11117 */;
...
import ConfirmStartCall from "ConfirmStartCall" /* 13377 */;
import VoicePanelVideoGuardErrorAlert from "VoicePanelVideoGuardErrorAlert" /* 13509 */;
import ChannelHeader from "ChannelHeader" /* 13510 */;
...
const NO_PARTICIPANTS = fn(4738).NO_PARTICIPANTS;
let closure_12 = fn(8081).setIsChannelDetailsSearchActive;
...
let closure_18 = fn(8082).SearchEntrypointAnalyticsLocations;
...
export default noop.memo(function PrivateChannelButtons(channelId) {
  ...
  inappropriateConversationSafetyToolsWarningForChannel = channelId(11647).useInappropriateConversationSafetyToolsWarningForChannel(channelId);
  closure_6 = tmp7;
  const tmp2Result6 = channelId(11647);
  ...
  const VideoGuardExperiment = tmp2(13507).VideoGuardExperiment;
  ...
  const isCallActiveNullable = channelId(8200).useIsCallActiveNullable(id);
  const tmp2Result9 = channelId(8200);
  ...
  closure_11 = tmp15;
  const tmp17 = screenIndex(13508)({ context: { type: "channel", channel: stateFromStores } });
  ...
  const callback3 = obj9.useCallback(() => {
    ...
    if (tmp2) {
      ...
      ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(13525, dependencyMap.paths), "AppDMOptionsBottomSheet", obj4);
    }
  }, items11);
  ...
  const memo1 = obj9.useMemo(() => {
    if (closure_6) {
      ...
      let tmpResult = tmp(tmp2(13526).SafetyToolsButton, obj3);
    } else {
      ...
      tmpResult = tmp(tmp2(5292).PressableOpacity, obj);
    }
    return tmpResult;
  }, items12);
  if (screenIndex(8606)(channelId)) {
    let obj3 = {
      style: tmp.button,
      onPress() {
    ...
    obj3.children = closure_20(tmp2(13199).WindowLaunchIcon, { size: "sm" });
    let tmp25Result = closure_20(tmp2(5292).PressableOpacity, obj3);
    ...
    if (null != application) {
      ...
      obj5.children = closure_20(tmp2(7247).MagnifyingGlassIcon, { size: "sm" });
      const items13 = [closure_20(tmp2(5292).PressableOpacity, obj5), ];
      ...
      obj6.children = closure_20(tmp2(7576).SettingsIcon, { size: "sm" });
      items13[1] = closure_20(tmp2(5292).PressableOpacity, obj6);
    ...
    if (!tmp15) {
      ...
      if (tmp15) {
        ...
        tmp30(tmp2(5270).VoiceNormalIcon, unsafe_rawColors1);
        ...
        if (stateFromStores1) {
          ...
          let tmp30Result2 = tmp30(tmp2(8087).PhoneHangUpIcon, obj11);
          let tmp33 = tmp30;
        } else {
          let GREEN_360;
          ...
          tmp30Result2 = tmp30(tmp2(8085).PhoneCallIcon, obj12);
          tmp33 = tmp30;
        }
        const items15 = [
          tmp30Result2,
        ...
        if (tmp33Result) {
          ...
          obj13.children = tmp25(tmp2(4718).Text, obj14);
          tmp33Result = tmp33(tmp26, obj13);
        }
        items15[2] = tmp33Result;
        obj8.children = items15;
        const items17 = [tmp25(tmp2(5292).PressableOpacity, obj8), , ];
        let tmp38 = null;
        if (!isMultiUserDMResult) {
          tmp38 = null;
          if (!stateFromStores1) {
            ...
            if (videoEnabled) {
              let VideoDenyIcon = tmp2(10353).VideoIcon;
            } else {
              VideoDenyIcon = tmp2(13527).VideoDenyIcon;
            }
            obj15.children = tmp33(VideoDenyIcon, { size: "sm" });
            tmp33(tmp2(5292).PressableOpacity, obj15);
          }
        }
        items17[1] = tmp38;
        items17[2] = memo1;
