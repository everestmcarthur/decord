...
// Dependencies: [19, 17, 4656, 1958, 1909, 4288, 1372, 4659, 7998, 1074, 4661, 2011, 7999, 4665, 21, 1177, 4640, 576, 504, 11549, 13416, 8117, 13417, 11023, 13286, 4986, 13418, 1115, 13419, 12451, 12510, 4820, 4607, 13434, 1897, 5210, 7165, 13435, 8523, 4334, 1241, 13108, 7494, 4847, 5188, 8004, 8002, 4636, 13436, 10259, 2]

// Module 13415 (PrivateChannelButtons)
...
import getPrivateChannelCallDefault from "getPrivateChannelCall" /* 11023 */;
...
import ConfirmStartCall from "ConfirmStartCall" /* 13286 */;
import VoicePanelVideoGuardErrorAlert from "VoicePanelVideoGuardErrorAlert" /* 13418 */;
import ChannelHeader from "ChannelHeader" /* 13419 */;
...
const NO_PARTICIPANTS = fn(4656).NO_PARTICIPANTS;
let closure_12 = fn(7998).setIsChannelDetailsSearchActive;
...
let closure_18 = fn(7999).SearchEntrypointAnalyticsLocations;
...
export default noop.memo(function PrivateChannelButtons(channelId) {
  ...
  inappropriateConversationSafetyToolsWarningForChannel = channelId(11549).useInappropriateConversationSafetyToolsWarningForChannel(channelId);
  closure_6 = tmp7;
  const tmp2Result6 = channelId(11549);
  ...
  const VideoGuardExperiment = tmp2(13416).VideoGuardExperiment;
  ...
  const isCallActiveNullable = channelId(8117).useIsCallActiveNullable(id);
  const tmp2Result9 = channelId(8117);
  ...
  closure_11 = tmp15;
  const tmp17 = screenIndex(13417)({ context: { type: "channel", channel: stateFromStores } });
  ...
  const callback3 = obj9.useCallback(() => {
    ...
    if (tmp2) {
      ...
      ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(13434, dependencyMap.paths), "AppDMOptionsBottomSheet", obj4);
    }
  }, items11);
  ...
  const memo1 = obj9.useMemo(() => {
    if (closure_6) {
      ...
      let tmpResult = tmp(tmp2(13435).SafetyToolsButton, obj3);
    } else {
      ...
      tmpResult = tmp(tmp2(5210).PressableOpacity, obj);
    }
    return tmpResult;
  }, items12);
  if (screenIndex(8523)(channelId)) {
    let obj3 = {
      style: tmp.button,
      onPress() {
    ...
    obj3.children = closure_20(tmp2(13108).WindowLaunchIcon, { size: "sm" });
    let tmp25Result = closure_20(tmp2(5210).PressableOpacity, obj3);
    ...
    if (null != application) {
      ...
      obj5.children = closure_20(tmp2(7165).MagnifyingGlassIcon, { size: "sm" });
      const items13 = [closure_20(tmp2(5210).PressableOpacity, obj5), ];
      ...
      obj6.children = closure_20(tmp2(7494).SettingsIcon, { size: "sm" });
      items13[1] = closure_20(tmp2(5210).PressableOpacity, obj6);
    ...
    if (!tmp15) {
      ...
      if (tmp15) {
        ...
        tmp30(tmp2(5188).VoiceNormalIcon, unsafe_rawColors1);
        ...
        if (stateFromStores1) {
          ...
          let tmp30Result2 = tmp30(tmp2(8004).PhoneHangUpIcon, obj11);
          let tmp33 = tmp30;
        } else {
          let GREEN_360;
          ...
          tmp30Result2 = tmp30(tmp2(8002).PhoneCallIcon, obj12);
          tmp33 = tmp30;
        }
        const items15 = [
          tmp30Result2,
        ...
        if (tmp33Result) {
          ...
          obj13.children = tmp25(tmp2(4636).Text, obj14);
          tmp33Result = tmp33(tmp26, obj13);
        }
        items15[2] = tmp33Result;
        obj8.children = items15;
        const items17 = [tmp25(tmp2(5210).PressableOpacity, obj8), , ];
        let tmp38 = null;
        if (!isMultiUserDMResult) {
          tmp38 = null;
          if (!stateFromStores1) {
            ...
            if (videoEnabled) {
              let VideoDenyIcon = tmp2(10259).VideoIcon;
            } else {
              VideoDenyIcon = tmp2(13436).VideoDenyIcon;
            }
            obj15.children = tmp33(VideoDenyIcon, { size: "sm" });
            tmp33(tmp2(5210).PressableOpacity, obj15);
          }
        }
        items17[1] = tmp38;
        items17[2] = memo1;
