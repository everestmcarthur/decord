...
// Dependencies: [19, 17, 1963, 1372, 15390, 1074, 1085, 21, 16184, 16185, 4639, 576, 7269, 504, 7275, 1241, 9668, 1897, 8299, 16179, 4791, 16186, 15391, 16180, 1177, 16189, 11024, 16190, 10043, 13138, 8834, 5181, 9355, 1115, 4489, 1364, 10190, 5672, 16181, 16192, 10193, 8259, 16194, 8367, 2]

// Module 16183 (HappeningNowCardActivity)
import nativeDefault from "native" /* 576 */;
import native from "native" /* 1177 */;
import AnalyticsUtilsDefault from "AnalyticsUtils" /* 1241 */;
import asyncRequireImpl from "asyncRequireImpl" /* 1897 */;
import ColorUtils from "ColorUtils" /* 4489 */;
import useFetchStreamPreviewDefault from "useFetchStreamPreview" /* 10193 */;
import useLiveStageData from "useLiveStageData" /* 16181 */;
import _modDef16184 from "module_16184" /* 16184 */;
import _modDef16185 from "module_16185" /* 16185 */;
import HappeningNowAvatarStack from "HappeningNowAvatarStack" /* 16192 */;
...
export default noop.memo((userId) => {
  ...
  const callback = activity.useCallback(() => {
    ...
    if (null != stream) {
      asyncRequireImpl(9668, tmp6.paths).then((result) => result.default(channelId.channelId, true));
      const promise2 = asyncRequireImpl(9668, tmp6.paths);
    } else {
      asyncRequireImpl(8299, tmp6.paths).then((result) => result.default({ userId, localUser, sourceAnalyticsLocations }));
      const promise = asyncRequireImpl(8299, tmp6.paths);
    }
  }, items2);
