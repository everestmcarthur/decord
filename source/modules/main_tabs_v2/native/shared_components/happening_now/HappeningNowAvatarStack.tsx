...
// Dependencies: [32, 19, 17, 2025, 13124, 21, 4607, 576, 1178, 4343, 563, 5026, 8907, 4603, 1880, 16154, 2]
...
export const HappeningNowAvatarStack = function HappeningNowAvatarStack(userLimit) {
  ...
  mapped = substr.map((user, index) => {
    ...
    if (index !== diff) {
      ...
      const point = { shape: tmp3(8907).CutoutShape.Circle, x: tmp3(1178).AVATAR_SIZE_MAP[tmp5] - num3 - num2, y: -num2, size: tmp3(1178).AVATAR_SIZE_MAP[tmp5] + 2 * num2 };
      const items = [point];
      obj3.cutouts = items;
      obj3.children = tmp6;
