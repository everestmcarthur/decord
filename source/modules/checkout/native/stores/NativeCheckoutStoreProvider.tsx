...
// Dependencies: [5, 32, 19, 17, 7438, 1074, 4556, 21, 4574, 5612, 10837, 573, 7443, 7444, 5591, 1256, 1232, 5006, 10840, 1242, 10844, 2]
// Exports: default
...
function NativeCheckoutStoreProvider(children) {
  ...
  const effect1 = noop.useEffect(() => () => {
    ...
    if (null != orderRecord) {
      ...
      checkoutInitParameters(10844).discardDraftOrder(obj2);
      const obj = checkoutInitParameters(10844);
...
export default function NativeCheckoutStoreProviderWrapper(orderRequired) {
  ...
  if (tmp4[0]) {
    ...
    if (!flag) {
      ...
      tmp17 = <onOrderRetryCancellation style={tmp.loadingSpinnerContainer}>{jsx(tmp9(tmp5[14]).ActivityIndicator, { animating: true, size: "large" })}</onOrderRetryCancellation>;
    }
    let tmp16 = tmp17;
  } else {
