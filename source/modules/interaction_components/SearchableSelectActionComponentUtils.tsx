...
// Dependencies: [1957, 2021, 2015, 1979, 4255, 1371, 8198, 1894, 5493, 4758, 4838, 4759, 8206, 1369, 2]
// Exports: getInitialSnowflakeSelectOptions, getSnowflakeSelectDefaultValues, queryChannels, queryMentionables

// Module 8205 (SearchableSelectActionComponentUtils)
...
import SnowflakeSelectDefaultValueTypes from "SnowflakeSelectDefaultValueTypes" /* 8206 */;
...
export const queryMentionables = function queryMentionables(type, query, channelId) {
  ...
  if (null == channel) {
    ...
    const obj = channel(5493);
    ({ users, roles } = channel(5493).queryMentionResults(obj2));
    const items = [];
    const queryMentionResultsResult = channel(5493).queryMentionResults(obj2);
    ...
    const mapped = defaultValues.map((type) => {
      ...
      if (SnowflakeSelectDefaultValueTypes.SnowflakeSelectDefaultValueTypes.USER === type) {
        ...
        if (null == user) {
          ...
          const obj2 = { type: tmp(4838).SelectOptionType.USER, value: user.id, label: null };
          if (nick == null) {
      ...
      } else if (tmp(8206).SnowflakeSelectDefaultValueTypes.ROLE === type) {
        if (null == closure_1) {
          return null;
        } else {
          ...
          let tmp12 = null;
          if (null != role) {
            const obj3 = { type: tmp(4838).SelectOptionType.ROLE, value: null, label: null };
            ({ id: obj4.value, name: obj4.label } = role);
            tmp12 = obj3;
          }
          return tmp12;
        }
      } else if (tmp(8206).SnowflakeSelectDefaultValueTypes.CHANNEL === type) {
        if (null == closure_1) {
          return null;
        } else {
          ...
          if (null != channel) {
            ...
            if (channel.guild_id === tmp3.id) {
              if (channelTypes.length <= 0) {
                const obj5 = { type: tmp(4838).SelectOptionType.CHANNEL, value: channel.id, label: tmp(4759).computeChannelName(channel, UserStore, RelationshipStore) };
                tmp5 = obj5;
