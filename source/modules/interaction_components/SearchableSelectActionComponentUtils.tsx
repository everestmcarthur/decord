...
// Dependencies: [1957, 2021, 2015, 1979, 4255, 1371, 8199, 1894, 5494, 4759, 4839, 4760, 8207, 1369, 2]
// Exports: getInitialSnowflakeSelectOptions, getSnowflakeSelectDefaultValues, queryChannels, queryMentionables

// Module 8206 (SearchableSelectActionComponentUtils)
import NicknameUtilsDefault from "NicknameUtils" /* 4759 */;
import InteractionComponentTypes from "InteractionComponentTypes" /* 4839 */;
import AutocompleteUtilsDefault from "AutocompleteUtils" /* 5494 */;
import SnowflakeSelectDefaultValueTypes from "SnowflakeSelectDefaultValueTypes" /* 8207 */;
...
export const queryMentionables = function queryMentionables(type, query, channelId) {
  ...
  if (null == channel) {
    ...
    const obj = channel(5494);
    ({ users, roles } = channel(5494).queryMentionResults(obj2));
    const items = [];
    const queryMentionResultsResult = channel(5494).queryMentionResults(obj2);
    ...
    const mapped = defaultValues.map((type) => {
      ...
      if (SnowflakeSelectDefaultValueTypes.SnowflakeSelectDefaultValueTypes.USER === type) {
        ...
        if (null == user) {
          ...
          const obj2 = { type: tmp(4839).SelectOptionType.USER, value: user.id, label: null };
          if (nick == null) {
      ...
      } else if (tmp(8207).SnowflakeSelectDefaultValueTypes.ROLE === type) {
        if (null == closure_1) {
          return null;
        } else {
          ...
          let tmp12 = null;
          if (null != role) {
            const obj3 = { type: tmp(4839).SelectOptionType.ROLE, value: null, label: null };
            ({ id: obj4.value, name: obj4.label } = role);
            tmp12 = obj3;
          }
          return tmp12;
        }
      } else if (tmp(8207).SnowflakeSelectDefaultValueTypes.CHANNEL === type) {
        if (null == closure_1) {
          return null;
        } else {
          ...
          if (null != channel) {
            ...
            if (channel.guild_id === tmp3.id) {
              if (channelTypes.length <= 0) {
                const obj5 = { type: tmp(4839).SelectOptionType.CHANNEL, value: channel.id, label: tmp(4760).computeChannelName(channel, UserStore, RelationshipStore) };
                tmp5 = obj5;
