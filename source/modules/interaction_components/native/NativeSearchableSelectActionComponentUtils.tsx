...
// Dependencies: [1957, 2015, 1979, 1371, 1074, 4838, 1369, 1399, 7260, 8208, 1091, 576, 8209, 5079, 2]
// Exports: getChannelIconData, transformSearchableSelectOptions

// Module 8207 (NativeSearchableSelectActionComponentUtils)
...
export const transformSearchableSelectOptions = function transformSearchableSelectOptions(initialSnowflakeSelectOptions, guildId) {
  ...
  const mapped = initialSnowflakeSelectOptions.map((type) => {
    ...
    if (InteractionComponentTypes.SelectOptionType.USER === type) {
      ...
      return tmp34;
    } else if (tmp(4838).SelectOptionType.ROLE === type) {
      let role = null;
      if (null != closure_1) {
      ...
      if (null != role) {
        ...
        if (null != tmp14) {
          ...
          if (tmpResult7.canGuildUseRoleIcons(tmp14, role)) {
            roleIconData = tmp(7260).getRoleIconData(role);
            const tmpResult8 = tmp(7260);
          }
          if (null == roleIconData) {
            const obj2 = {};
            const merged1 = Object.assign(type);
            obj2.iconSrc = tmp(1399).ensureAvatarSource(_modDef8208).uri;
      ...
      return tmp18;
    } else if (tmp(4838).SelectOptionType.CHANNEL === type) {
      ...
      if (null == channel) {
        ...
        if (channel.type === constants.GUILD_CATEGORY) {
          ...
          channelIconWithGuild = tmp(5079).getChannelIconWithGuild(channel, tmp4);
          const tmpResult12 = tmp(5079);
        }
