...
// Dependencies: [5, 19, 17, 1957, 4948, 1074, 21, 4607, 4504, 5657, 15603, 1114, 4832, 4567, 10287, 8173, 5732, 8198, 38, 504, 12253, 5231, 5186, 17481, 1894, 4951, 5190, 12092, 9399, 10727, 1874, 10726, 5188, 5025, 576, 5739, 2]
// Exports: default

// Module 17480 (FileUploadActionComponent)
...
export default function FileUploadActionComponent(maxValues) {
  ...
  const callback2 = allowedExtensions.useCallback(() => {
    ...
    let obj = {
      ...
      onDismissKeyboard() {
        return InteractionModal(10727).hideMediaKeyboardActionSheet();
      },
    ...
    if (mediaFilesAllowed) {
      const obj3 = {
        ...
        onPressItem(item) {
            ...
            if (item.isIncluded) {
              const found = currentUploads.find((item) => InteractionModal(5188).doesImageMatchUpload(item.node.image, item));
              if (null != found) {
                callback1(found.id);
              }
            } else {
        ...
        onManageLimited() {
            obj = InteractionModal(10726);
            const result = obj.handleLimitedPickerDialog({ onDismissKeyboard: InteractionModal(10727).hideMediaKeyboardActionSheet, onRestoreKeyboard });
          },
