...
// Dependencies: [5, 19, 17, 1957, 4946, 1074, 21, 4605, 4502, 5655, 15620, 1114, 4830, 4565, 10266, 8150, 5730, 8175, 38, 504, 12228, 5229, 5184, 17489, 1894, 4949, 5188, 12067, 9376, 10706, 1874, 10705, 5186, 5023, 576, 5737, 2]
// Exports: default

// Module 17488 (FileUploadActionComponent)
...
export default function FileUploadActionComponent(maxValues) {
  ...
  const callback2 = allowedExtensions.useCallback(() => {
    ...
    let obj = {
      ...
      onDismissKeyboard() {
        return InteractionModal(10706).hideMediaKeyboardActionSheet();
      },
    ...
    if (mediaFilesAllowed) {
      const obj3 = {
        ...
        onPressItem(item) {
            ...
            if (item.isIncluded) {
              const found = currentUploads.find((item) => InteractionModal(5186).doesImageMatchUpload(item.node.image, item));
              if (null != found) {
                callback1(found.id);
              }
            } else {
        ...
        onManageLimited() {
            obj = InteractionModal(10705);
            const result = obj.handleLimitedPickerDialog({ onDismissKeyboard: InteractionModal(10706).hideMediaKeyboardActionSheet, onRestoreKeyboard });
          },
