...
// Dependencies: [32, 19, 21, 4639, 576, 8239, 1895, 4606, 11957, 7237, 4635, 1115, 2]
// Exports: default

// Module 11956 (StringSelectComponentActionSheet)
...
export default function StringSelectComponentActionSheet(selectionActionComponent) {
  ...
  let tmp = callback();
  dependencyMap = tmp;
  let obj = selectionActionComponent(8239);
  let tmp3 = first(noop.useState(new Set(selectionActionComponent(8239).getInitialStringSelectOptions(selectionActionComponent, containerId))), 2);
  first = tmp3[0];
  noop = tmp3[1];
  ...
  let set = new Set(selectionActionComponent(8239).getInitialStringSelectOptions(selectionActionComponent, containerId));
  const tmp9 = memo;
  ...
  return tmp9(onSubmit(11957), obj2);
};
