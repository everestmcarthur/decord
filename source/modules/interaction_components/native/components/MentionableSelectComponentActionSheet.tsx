...
// Dependencies: [19, 17, 2016, 1980, 4679, 1372, 1074, 21, 4639, 576, 7234, 8240, 11959, 4871, 1177, 7294, 7312, 9884, 11957, 9935, 4635, 11960, 1115, 2]
// Exports: default

// Module 11958 (MentionableSelectComponentActionSheet)
...
export default function MentionableSelectComponentActionSheet(selectionActionComponent) {
  ...
  const callback1 = noop.useCallback((type) => {
    ...
    } else if (type.type === tmp(4871).SelectOptionType.ROLE) {
      if (null != guild) {
      ...
      if (null != guild) {
        if (null != role) {
          if (tmpResult.canGuildUseRoleIcons(tmp3, role)) {
            const roleIconData = tmp(7294).getRoleIconData(role);
          ...
          return closure_2_10(tmp(9884).ShieldUserIcon, obj4);
        }
      }
      return null;
    }
    ...
    renderOptionSuffix(type) {
      if (type.type === InteractionComponentTypes.SelectOptionType.ROLE) {
        ...
        if (null != tmp7) {
          ...
          const items = [closure_2_10(tmp(4635).Text, obj2), closure_2_10(tmp(11960).UserIcon, { size: "xs" })];
          obj.children = items;
