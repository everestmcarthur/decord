...
// Dependencies: [32, 502, 11896, 11, 11884, 1114, 2882, 2]
// Exports: default

// Module 13371 (getRequestToStreamCTAAndIsDisabled)
import SnowflakeUtilsDefault from "SnowflakeUtils" /* 11 */;
import util from "util" /* 1114 */;
import _modDef2882 from "module_2882" /* 2882 */;
import isInviteActive from "isInviteActive" /* 11884 */;
import useCanFulfillStreamRequest from "useCanFulfillStreamRequest" /* 11896 */;
...
export default function getRequestToStreamCTAAndIsDisabled(id) {
  ...
  if (tmp9) {
    ...
    text = stringResult;
    if (!tmp3[0]) {
      if (tmp(11896).StreamRequestUnfulfillableReason.ALREADY_STREAMING === tmp4) {
        const intl4 = tmp(1114).intl;
        text = intl4.string(tmp6(2882).P0wwmM);
        isDisabled = true;
      } else if (tmp(11896).StreamRequestUnfulfillableReason.NOT_RUNNING_GAME === tmp4) {
        const intl3 = tmp(1114).intl;
        text = intl3.string(tmp6(2882)["43zohO"]);
        isDisabled = true;
      } else if (tmp(11896).StreamRequestUnfulfillableReason.NOT_IN_VOICE_CHANNEL === tmp4) {
        ...
        isDisabled = false;
        text = stringResult;
        if (tmp(11896).StreamRequestUnfulfillableReason.NO_PERMISSION === tmp4) {
