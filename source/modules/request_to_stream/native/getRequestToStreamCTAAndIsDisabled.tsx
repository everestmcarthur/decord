...
// Dependencies: [32, 502, 11900, 11, 11888, 1115, 2883, 2]
// Exports: default

// Module 13376 (getRequestToStreamCTAAndIsDisabled)
import SnowflakeUtilsDefault from "SnowflakeUtils" /* 11 */;
import util from "util" /* 1115 */;
import _modDef2883 from "module_2883" /* 2883 */;
import isInviteActive from "isInviteActive" /* 11888 */;
import useCanFulfillStreamRequest from "useCanFulfillStreamRequest" /* 11900 */;
...
export default function getRequestToStreamCTAAndIsDisabled(id) {
  ...
  if (tmp9) {
    ...
    text = stringResult;
    if (!tmp3[0]) {
      if (tmp(11900).StreamRequestUnfulfillableReason.ALREADY_STREAMING === tmp4) {
        const intl4 = tmp(1115).intl;
        text = intl4.string(tmp6(2883).P0wwmM);
        isDisabled = true;
      } else if (tmp(11900).StreamRequestUnfulfillableReason.NOT_RUNNING_GAME === tmp4) {
        const intl3 = tmp(1115).intl;
        text = intl3.string(tmp6(2883)["43zohO"]);
        isDisabled = true;
      } else if (tmp(11900).StreamRequestUnfulfillableReason.NOT_IN_VOICE_CHANNEL === tmp4) {
        ...
        isDisabled = false;
        text = stringResult;
        if (tmp(11900).StreamRequestUnfulfillableReason.NO_PERMISSION === tmp4) {
