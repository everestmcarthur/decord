...
// Dependencies: [19, 1372, 7671, 563, 4297, 8997, 7668, 7667, 2]
// Exports: useBadBundleFilter

// Module 15188 (useBadBundleFilter)
    ...
    if (null != arr) {
      ...
      if (0 !== arr.length) {
        found = arr.filter((product) => {
          const obj = canUseShopDiscountsResult(8997);
            ...
            if (!tmpResult.isFreeCollectiblesProduct(product)) {
              if (!obj.getProductPurchaseState(CollectiblesPurchaseStore, product).isPurchased) {
                if (tmpResult5.isOrbsExclusiveProduct(product)) {
                  ...
                  return null != tmp(7667).getProductOrbPrice(obj3);
                } else {
                  const defaultPriceSetAssignmentPurchaseType = tmp(7668).getDefaultPriceSetAssignmentPurchaseType(hasShopDiscount);
                  const tmpResult7 = tmp(7668);
                  let result = tmp(7668).extractPriceByPurchaseTypes(product, defaultPriceSetAssignmentPurchaseType);
                  if (null != result) {
                    if (0 !== result.amount) {
                      let num2 = 0;
