...
// Dependencies: [19, 1371, 7572, 563, 4231, 8874, 7568, 7567, 2]
    ...
    if (null != arr) {
      ...
      if (0 !== arr.length) {
        found = arr.filter((product) => {
          const obj = canUseShopDiscountsResult(8874);
            ...
            if (!tmpResult.isFreeCollectiblesProduct(product)) {
              if (!obj.getProductPurchaseState(CollectiblesPurchaseStore, product).isPurchased) {
                if (tmpResult5.isOrbsExclusiveProduct(product)) {
                  ...
                  return null != tmp(7567).getProductOrbPrice(obj3);
                } else {
                  const defaultPriceSetAssignmentPurchaseType = tmp(7568).getDefaultPriceSetAssignmentPurchaseType(hasShopDiscount);
                  const tmpResult7 = tmp(7568);
                  let result = tmp(7568).extractPriceByPurchaseTypes(product, defaultPriceSetAssignmentPurchaseType);
                  if (null != result) {
                    if (0 !== result.amount) {
                      let num2 = 0;
