...
// Dependencies: [5, 32, 19, 8306, 1371, 7616, 7618, 1076, 1090, 7627, 504, 4264, 1079, 15128, 15129, 1080, 8965, 8305, 8877, 15130, 15132, 2]
// Exports: usePopularOrbShopProducts

// Module 15127 (usePopularOrbShopProducts)
import DurationsDefault from "Durations" /* 1090 */;
import StorefrontProductActionCreators from "StorefrontProductActionCreators" /* 8305 */;
...
import CollectiblesProductRecord from "CollectiblesProductRecord" /* 7618 */;
...
export const usePopularOrbShopProducts = function usePopularOrbShopProducts(enabled) {
  ...
  let tmp8 = skuIds;
  const canUseShopDiscountsResult = skuIds(4264).canUseShopDiscounts(stateFromStores);
        ...
        try {
          ...
          if (0 === c5) {
            if (arg0 === 1) {
              ...
              let v0 = 2;
              const obj4 = { item_types: [], colors: [], themes: [], orbs_eligible: true, currency: enabled(15129).CollectibleSearchCurrencyFilter.ORBS, offset: 0, limit: 10, sort_type, sort_direction: enabled(1080).CollectibleSearchSortDirection.DESC };
              const obj6 = { timeout };
              c5 = 3;
              c6 = 1;
              const obj7 = { value: enabled(15128).search(obj4, obj6), done: false };
              return obj7;
            }
          } else if (1 === tmp8) {
            v0 = 0;
  ...
  let obj3 = skuIds(4264);
  let fetchCollectiblesProducts = enabled(8965).useFetchCollectiblesProducts(skuIds);
  const items3 = [skuIds];
  ...
  const tmp5Result = enabled(8965);
  const getOrFetchStorefrontPricesForSkuIds = enabled(8877).useGetOrFetchStorefrontPricesForSkuIds({ skuIds });
  const tmp5Result5 = enabled(8877);
  const items4 = [c6];
  ...
  filteredAndSortedProducts = enabled(15132).useFilteredAndSortedProducts({ products: memo2, screen: constants.ORBS, bypassAndroidUnsyncedFilter: true });
