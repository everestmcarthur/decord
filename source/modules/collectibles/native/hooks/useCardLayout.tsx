...
// Function ID: 15893
// Name: useCardLayout
// Dependencies: [8857, 1477, 2]
// Exports: useCardLayout

// Module 15892 (useCardLayout)
...
export const useCardLayout = function useCardLayout() {
  ...
  if (num < 2) {
    const obj2 = { columns: num, cardWidth: "accessible", rowWidth: "__packager_asset" };
    return obj2;
  } else {
    let num2 = 2;
    if (width >= 768) {
    ...
    if (diff1 < 2 * CollectiblesShopCardV2.COLLECTIBLES_SHOP_CARD_GAP) {
      ...
      bound = 2 * tmp2(8857).COLLECTIBLES_SHOP_CARD_GAP;
    }
    const result1 = (width - (bound + tmp2(8857).COLLECTIBLES_SHOP_CARD_GAP * diff)) / num;
    let tmp10 = tmp2;
    let tmp11 = result1;
    let tmp12 = num;
    let tmp14 = tmp2;
    ...
    const _Math2 = Math;
    const _Math3 = Math;
    const bound1 = Math.max(tmp11, tmp14(8857).COLLECTIBLES_SHOP_CARD_WIDTH);
    const bound2 = Math.min(bound1, tmp14(8857).COLLECTIBLES_SHOP_CARD_MAX_WIDTH);
    const result3 = bound2 * tmp12;
    const diff2 = tmp12 - 1;
    if (result3 + (bound + tmp10(8857).COLLECTIBLES_SHOP_CARD_GAP * diff2) > width) {
      if (1 < tmp12) {
    ...
    obj.rowWidth = result4 + tmp14(8857).COLLECTIBLES_SHOP_CARD_GAP * diff2;
  }
};
