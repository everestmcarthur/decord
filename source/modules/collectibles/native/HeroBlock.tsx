...
// Function ID: 15963
// Name: HeroBlock
// Dependencies: [19, 17, 7656, 1076, 1074, 21, 8920, 4640, 576, 9027, 15963, 1484, 8923, 504, 11638, 4574, 15964, 4341, 4489, 15965, 15187, 7276, 7296, 15967, 1241, 4491, 15969, 15970, 15971, 5074, 4636, 5062, 1115, 11918, 5537, 5210, 7323, 7270, 15972, 8873, 2]
// Exports: default

// Module 15962 (HeroBlock)
import nativeDefault from "native" /* 576 */;
import AnalyticsUtilsDefault from "AnalyticsUtils" /* 1241 */;
import CollectiblesShopCardV2 from "CollectiblesShopCardV2" /* 8920 */;
import CollectiblesAnalyticsContext from "CollectiblesAnalyticsContext" /* 8923 */;
import SkeletonCardDefault from "SkeletonCard" /* 9027 */;
import FeaturedFirstCardCoachmarkAnchorDefault from "FeaturedFirstCardCoachmarkAnchor" /* 15967 */;
...
const result = 0.75 * fn(8920).COLLECTIBLES_SHOP_CARD_WIDTH;
...
export default function _default(heroBlock) {
  ...
  closure_5 = undefined;
  closure_6 = undefined;
  let obj = heroBlock(15963);
  dependencyMap = heroBlock(1484).useNavigation();
  let obj2 = heroBlock(1484);
  noop = heroBlock(8923).useCollectiblesAnalyticsContext();
  ...
  let isEligibleForQuests = heroBlock(11638).getIsEligibleForQuests();
  const tmpResult9 = heroBlock(11638);
  const tmp7 = preferVCPrice(4574)();
  const tmp8 = closure_14();
  const tmpResult10 = heroBlock(15964);
  const token = heroBlock(4341).useToken(preferVCPrice(576).colors.BACKGROUND_BASE_LOW);
  const tmpResult11 = heroBlock(4341);
  const tmpResult12 = heroBlock(4489);
  const tmpResult13 = heroBlock(4489);
  const hexToRgbaStringResult = tmpResult12.hexToRgbaString(heroBlock(4489).hexWithOpacity(token, 0));
  const token1 = heroBlock(4341).useToken(preferVCPrice(576).colors.BACKGROUND_BASE_LOWEST);
  const tmp12 = preferVCPrice(15965)();
  closure_5 = tmp12;
  const items1 = [heroBlock.rankedSkuIds, tmp12];
  const memo = noop.useMemo(() => closure_5(heroBlock.rankedSkuIds), items1);
  const tmpResult14 = heroBlock(4341);
  const filteredAndSortedProducts = heroBlock(15187).useFilteredAndSortedProducts({ products: memo, bypassAndroidUnsyncedFilter: tmp4 });
  closure_6 = tmp14;
  const tmpResult15 = heroBlock(15187);
  let unpublishedAt;
  ...
  if (undefined === stateFromStores) {
    ...
    if (!tmp4) {
      const obj4 = { value: tmp15(preferVCPrice(7296).COLLECTIBLES_SHOP_HERO).analyticsLocations, children: null };
      ...
      if (tmp24Result) {
        ...
        if (tmp4) {
          ...
          tmp22Result = tmp22(tmp6(5074), obj8);
        }
        ...
        items3[2] = tmp22(tmp6(5074), obj12);
      ...
      if (tmp4) {
        ...
        if (tmp22Result6) {
          ...
          tmp22Result6 = tmp22(tmp(4636).Text, obj15);
        }
        ...
        if (tmp22Result7) {
          ...
          tmp22Result7 = tmp22(tmp(4636).Text, obj16);
        }
        ...
        if (isEligibleForQuests) {
          ...
          isEligibleForQuests = tmp22(tmp(5062).Button, obj19);
        }
        ...
        const obj28 = { style: tmp8.heroViewAllIcon, children: tmp22(tmp(7323).ChevronSmallRightIcon, { size: "sm", color: "white" }) };
        items9[1] = tmp22(tmp25, obj28);
        obj24.children = items9;
        obj20.children = tmp24(tmp25, obj24);
        tmp24Result2 = tmp22(tmp(5210).PressableOpacity, obj20, stateFromStores.storeListingId);
      }
      ...
      if (tmp4) {
        ...
        let tmp22Result9 = tmp22(tmp6(15972), obj30);
        const tmp6Result3 = tmp6(15972);
      } else {
        if (0 === filteredAndSortedProducts.length) {
          ...
          obj34.snapToInterval = tmp(8920).COLLECTIBLES_SHOP_CARD_WIDTH + tmp6(576).space.PX_12;
          ...
          tmp22Result10 = tmp22(tmp(8873).FlashList, obj34);
        }
        const obj36 = { children: tmp22Result10 };
        tmp22Result9 = tmp22(closure_12, obj36);
      }
      const obj37 = { children: tmp22Result9 };
      obj29.children = closure_11(tmp(7270).LayerScope, obj37);
      items6[2] = closure_11(closure_5, obj29);
      obj6.children = items6;
      obj5.children = closure_13(closure_5, obj6);
      obj4.children = closure_11(tmp6(15971), obj5);
      return closure_11(tmp(7276).AnalyticsLocationProvider, obj4);
    } else {
      if (tmpResult16.isThemeDark(tmp7)) {
        let tmp6Result4 = tmp6(15969);
      } else {
        tmp6Result4 = tmp6(15970);
      }
      tmpResult16 = tmp(4491);
    }
  }
};
