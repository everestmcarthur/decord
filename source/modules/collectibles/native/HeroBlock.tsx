...
// Function ID: 15884
// Name: HeroBlock
// Dependencies: [19, 17, 7616, 1076, 1074, 21, 8857, 4607, 576, 15143, 15884, 1483, 8860, 504, 11580, 4542, 15885, 4308, 4456, 15130, 15132, 7236, 7256, 15886, 1242, 4458, 15888, 15889, 15890, 5039, 4603, 5027, 1114, 11862, 5501, 5175, 7283, 7230, 15891, 8966, 2]
// Exports: default

// Module 15883 (HeroBlock)
...
import FeaturedFirstCardCoachmarkAnchorDefault from "FeaturedFirstCardCoachmarkAnchor" /* 15886 */;
...
const result = 0.75 * fn(8857).COLLECTIBLES_SHOP_CARD_WIDTH;
...
export default function _default(heroBlock) {
  ...
  closure_5 = undefined;
  closure_6 = undefined;
  let obj = heroBlock(15884);
  dependencyMap = heroBlock(1483).useNavigation();
  let obj2 = heroBlock(1483);
  noop = heroBlock(8860).useCollectiblesAnalyticsContext();
  ...
  let isEligibleForQuests = heroBlock(11580).getIsEligibleForQuests();
  const tmpResult9 = heroBlock(11580);
  const tmp7 = preferVCPrice(4542)();
  const tmp8 = closure_14();
  const tmpResult10 = heroBlock(15885);
  const token = heroBlock(4308).useToken(preferVCPrice(576).colors.BACKGROUND_BASE_LOW);
  const tmpResult11 = heroBlock(4308);
  const tmpResult12 = heroBlock(4456);
  const tmpResult13 = heroBlock(4456);
  const hexToRgbaStringResult = tmpResult12.hexToRgbaString(heroBlock(4456).hexWithOpacity(token, 0));
  const token1 = heroBlock(4308).useToken(preferVCPrice(576).colors.BACKGROUND_BASE_LOWEST);
  const tmp12 = preferVCPrice(15130)();
  closure_5 = tmp12;
  const items1 = [heroBlock.rankedSkuIds, tmp12];
  const memo = noop.useMemo(() => closure_5(heroBlock.rankedSkuIds), items1);
  const tmpResult14 = heroBlock(4308);
  const filteredAndSortedProducts = heroBlock(15132).useFilteredAndSortedProducts({ products: memo, bypassAndroidUnsyncedFilter: tmp4 });
  closure_6 = tmp14;
  const tmpResult15 = heroBlock(15132);
  let unpublishedAt;
  ...
  if (undefined === stateFromStores) {
    ...
    if (!tmp4) {
      const obj4 = { value: tmp15(preferVCPrice(7256).COLLECTIBLES_SHOP_HERO).analyticsLocations, children: null };
      ...
      if (tmp24Result) {
        ...
        if (tmp4) {
          ...
          tmp22Result = tmp22(tmp6(5039), obj8);
        }
        ...
        items3[2] = tmp22(tmp6(5039), obj12);
      ...
      if (tmp4) {
        ...
        if (tmp22Result6) {
          ...
          tmp22Result6 = tmp22(tmp(4603).Text, obj15);
        }
        ...
        if (tmp22Result7) {
          ...
          tmp22Result7 = tmp22(tmp(4603).Text, obj16);
        }
        ...
        if (isEligibleForQuests) {
          ...
          isEligibleForQuests = tmp22(tmp(5027).Button, obj19);
        }
        ...
        const obj28 = { style: tmp8.heroViewAllIcon, children: tmp22(tmp(7283).ChevronSmallRightIcon, { size: "sm", color: "white" }) };
        items9[1] = tmp22(tmp25, obj28);
        obj24.children = items9;
        obj20.children = tmp24(tmp25, obj24);
        tmp24Result2 = tmp22(tmp(5175).PressableOpacity, obj20, stateFromStores.storeListingId);
      }
      ...
      if (tmp4) {
        ...
        let tmp22Result9 = tmp22(tmp6(15891), obj30);
        const tmp6Result3 = tmp6(15891);
      } else {
        if (0 === filteredAndSortedProducts.length) {
          ...
          obj34.snapToInterval = tmp(8857).COLLECTIBLES_SHOP_CARD_WIDTH + tmp6(576).space.PX_12;
          ...
          tmp22Result10 = tmp22(tmp(8966).FlashList, obj34);
        }
        const obj36 = { children: tmp22Result10 };
        tmp22Result9 = tmp22(closure_12, obj36);
      }
      const obj37 = { children: tmp22Result9 };
      obj29.children = closure_11(tmp(7230).LayerScope, obj37);
      items6[2] = closure_11(closure_5, obj29);
      obj6.children = items6;
      obj5.children = closure_13(closure_5, obj6);
      obj4.children = closure_11(tmp6(15890), obj5);
      return closure_11(tmp(7236).AnalyticsLocationProvider, obj4);
    } else {
      if (tmpResult16.isThemeDark(tmp7)) {
        let tmp6Result4 = tmp6(15888);
      } else {
        tmp6Result4 = tmp6(15889);
      }
      tmpResult16 = tmp(4458);
    }
  }
};
