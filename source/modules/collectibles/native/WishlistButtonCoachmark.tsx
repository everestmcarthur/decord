...
// Dependencies: [32, 19, 1954, 8861, 1943, 7461, 15884, 1114, 11222, 2]
// Exports: default
...
export default function WishlistButtonCoachmark(anchorRef) {
  ...
  let registerDismiss;
  hasNeverWishlisted = hasNeverWishlisted(8861).useHasNeverWishlisted();
  ...
  let obj = hasNeverWishlisted(8861);
  const tmp3 = _slicedToArray(hasNeverWishlisted(7461).useSelectedDismissibleContent(memo), 2);
  dependencyMap = tmp4;
  const tmp5 = tmp3[0] === hasNeverWishlisted(1943).DismissibleContent.WISHLIST_MOBILE_NUX_PRODUCT_CARD_COACHMARK;
  _slicedToArray = tmp5;
  const obj2 = hasNeverWishlisted(7461);
  ...
  const coachmark = hasNeverWishlisted(11222).useCoachmark(anchorRef.anchorRef, memo1);
  return null;
};
