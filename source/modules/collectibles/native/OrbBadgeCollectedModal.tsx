...
// Dependencies: [19, 17, 4631, 21, 4639, 576, 5709, 4842, 8978, 11229, 504, 5672, 11919, 8428, 11920, 7230, 8973, 4635, 1115, 5059, 8287, 8976, 7107, 2]
// Exports: default

// Module 13294 (OrbBadgeCollectedModal)
...
function OrbBadgeCollectedRootScreen(modalKey) {
  ...
  if (stateFromStores) {
    ...
    const obj4 = { uri: onPressViewBadge(11919) };
    obj3.source = obj4;
    obj3.style = tmp.background;
    let tmp9Result = tmp9(onPressViewBadge(5672), obj3);
    let tmp12 = onPressViewBadge;
    let tmp13 = tmp9;
    const tmp15 = onPressViewBadge(5672);
  } else {
    const obj5 = { source: null, poster: null, style: null, resizeMode: "contain", muted: true, pauseWhileAppInactive: true, paused: false };
    const obj6 = { uri: onPressViewBadge(11920) };
    obj5.source = obj6;
    obj5.poster = onPressViewBadge(11919);
    obj5.style = tmp.background;
    tmp9Result = tmp9(tmp2(8428).VideoComponent, obj5);
  ...
  const items4 = [tmp13(tmp12(5672), obj8), ];
  ...
  const items5 = [tmp13(modalKey(4635).Text, obj12), ];
  ...
  items5[1] = tmp13(modalKey(4635).Text, obj13);
  ...
  items3[1] = tmp13(modalKey(7230).SafeAreaPaddingView, rect);
  ...
  let obj = {
    screens: {
      [closure_9.ROOT]: {
        ...
        headerRight() {
          return timestampProducer(BalanceWidgetPill.BalanceWidgetPill, { initialRenderedBalance: orbBalancePriorToPurchase, balance: _mod8978.useFetchVirtualCurrencyBalance().balance });
        },
        title: ""
      }
    },
