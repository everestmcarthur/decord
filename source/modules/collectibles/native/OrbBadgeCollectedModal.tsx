...
// Dependencies: [19, 17, 4714, 21, 4722, 576, 5794, 4925, 9089, 11344, 504, 5757, 12027, 8539, 12028, 7319, 9084, 4718, 1115, 5144, 8397, 9087, 7196, 2]
// Exports: default

// Module 13404 (OrbBadgeCollectedModal)
...
function OrbBadgeCollectedRootScreen(modalKey) {
  ...
  if (stateFromStores) {
    ...
    const obj4 = { uri: onPressViewBadge(12027) };
    obj3.source = obj4;
    obj3.style = tmp.background;
    let tmp9Result = tmp9(onPressViewBadge(5757), obj3);
    let tmp12 = onPressViewBadge;
    let tmp13 = tmp9;
    const tmp15 = onPressViewBadge(5757);
  } else {
    const obj5 = { source: null, poster: null, style: null, resizeMode: "contain", muted: true, pauseWhileAppInactive: true, paused: false };
    const obj6 = { uri: onPressViewBadge(12028) };
    obj5.source = obj6;
    obj5.poster = onPressViewBadge(12027);
    obj5.style = tmp.background;
    tmp9Result = tmp9(tmp2(8539).VideoComponent, obj5);
  ...
  const items4 = [tmp13(tmp12(5757), obj8), ];
  ...
  const items5 = [tmp13(modalKey(4718).Text, obj12), ];
  ...
  items5[1] = tmp13(modalKey(4718).Text, obj13);
  ...
  items3[1] = tmp13(modalKey(7319).SafeAreaPaddingView, rect);
  ...
  let obj = {
    screens: {
      [closure_9.ROOT]: {
        ...
        headerRight() {
          return timestampProducer(BalanceWidgetPill.BalanceWidgetPill, { initialRenderedBalance: orbBalancePriorToPurchase, balance: _mod9089.useFetchVirtualCurrencyBalance().balance });
        },
        title: ""
      }
    },
