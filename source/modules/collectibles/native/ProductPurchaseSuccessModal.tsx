...
// Dependencies: [32, 718, 19, 17, 4714, 1074, 21, 4722, 576, 1890, 11331, 5801, 5850, 1115, 4458, 5143, 4723, 4690, 7748, 11333, 4423, 5156, 8408, 11335, 504, 11336, 11337, 9090, 8564, 11343, 8401, 7319, 11344, 9038, 9051, 11362, 11471, 11472, 4718, 7750, 5144, 2]
// Exports: default

// Module 11332 (ProductPurchaseSuccessModal)
...
function CancelButton(arg0) {
  ...
  return closure_12(_mod5801.HeaderBackButton, obj);
}
...
export default function ProductPurchaseSuccessModal(stageCollectibleChangeForEditProfile) {
  ...
  const backgroundColors = callback(11333)(product.styles).backgroundColors;
  let tertiary;
  if (backgroundColors != null) {
    tertiary = backgroundColors.tertiary;
  }
  ...
  if (useCategoryImage) {
    ...
    if (showOrbBalancePill) {
      ...
      showOrbBalancePill = tmp31(tmp2(11344).BalanceWidgetPill, obj12);
    }
    ...
    if (tmp2(1890).CollectiblesItemType.BUNDLE === type) {
      ...
      if (tmp30Result) {
        ...
        tmp30Result = tmp30(tmp4(9038), obj17);
      }
    ...
    if (null != renderMessages) {
      ...
      const items10 = [tmp31(tmp2(4718).Text, obj23), ];
      const obj25 = { variant: "text-md/medium", color: "text-overlay-light", style: tmp6.title, children: null };
      let result = tmp2(7750).isPremiumCollectiblesProduct(product);
