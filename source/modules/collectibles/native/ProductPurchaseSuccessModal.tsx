...
// Dependencies: [32, 718, 19, 17, 4632, 1074, 21, 4640, 576, 1890, 11238, 5719, 5768, 1115, 4376, 5061, 4641, 4608, 7666, 11240, 4341, 5074, 8326, 11242, 504, 11243, 11244, 9006, 8481, 11250, 8319, 7237, 11251, 8954, 8967, 11269, 11373, 11374, 4636, 7668, 5062, 2]
// Exports: default

// Module 11239 (ProductPurchaseSuccessModal)
...
function CancelButton(arg0) {
  ...
  return closure_12(_mod5719.HeaderBackButton, obj);
}
...
export default function ProductPurchaseSuccessModal(stageCollectibleChangeForEditProfile) {
  ...
  const backgroundColors = callback(11240)(product.styles).backgroundColors;
  let tertiary;
  if (backgroundColors != null) {
    tertiary = backgroundColors.tertiary;
  }
  ...
  if (useCategoryImage) {
    ...
    if (showOrbBalancePill) {
      ...
      showOrbBalancePill = tmp31(tmp2(11251).BalanceWidgetPill, obj12);
    }
    ...
    if (tmp2(1890).CollectiblesItemType.BUNDLE === type) {
      ...
      if (tmp30Result) {
        ...
        tmp30Result = tmp30(tmp4(8954), obj17);
      }
    ...
    if (null != renderMessages) {
      ...
      const items10 = [tmp31(tmp2(4636).Text, obj23), ];
      const obj25 = { variant: "text-md/medium", color: "text-overlay-light", style: tmp6.title, children: null };
      let result = tmp2(7668).isPremiumCollectiblesProduct(product);
    ...
    obj30.children = tmp31(tmp2(5062).Button, obj32);
    obj29.children = tmp31(tmp28, obj30);
    items6[2] = tmp31(tmp28, obj29);
    rect.children = items6;
    items3[1] = tmp27(tmp2(7237).SafeAreaPaddingView, rect);
