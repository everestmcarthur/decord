...
// Dependencies: [19, 17, 1074, 1373, 21, 4606, 576, 8936, 5638, 13257, 4541, 7660, 4458, 10813, 10814, 4602, 1114, 2024, 4301, 7223, 7504, 5174, 11200, 5026, 4573, 7227, 2]
// Exports: default

// Module 13256 (FractionalNitroCollectedActionSheet)
import nativeDefault from "native" /* 576 */;
import util from "util" /* 1114 */;
import shared from "shared" /* 4458 */;
import Text_Text from "Text/Text" /* 4602 */;
import FastImageDefault from "FastImage" /* 5638 */;
import FractionalNitroCoinIllustration from "FractionalNitroCoinIllustration" /* 8936 */;
...
function NitroAcquiredHeader(skuId) {
  ...
  const obj2 = { source: _modDef13257 };
...
let size = { width: fn(8936).FRACTIONAL_NITRO_COIN_SIZE.COLLECTED_SHEET, height: fn(8936).FRACTIONAL_NITRO_COIN_SIZE.COLLECTED_SHEET, position: "absolute", top: "50%", left: "50%", transform: null };
let obj5 = { flex: 1, gap: nativeDefault.space.PX_16, alignSelf: "stretch" };
let items = [{ translateX: -fn(8936).FRACTIONAL_NITRO_COIN_SIZE.COLLECTED_SHEET / 2 }, ];
const obj6 = { translateX: -fn(8936).FRACTIONAL_NITRO_COIN_SIZE.COLLECTED_SHEET / 2 };
items[1] = { translateY: -fn(8936).FRACTIONAL_NITRO_COIN_SIZE.COLLECTED_SHEET / 2 };
...
export default function FractionalNitroCollectedActionSheet(skuId) {
  ...
  const tmp = closure_12();
  dependencyMap = tmp;
  const tmp4 = consumed(4541)();
  noop = tmp4;
  const fetchFractionalPremiumInfo = skuId(7660).useFetchFractionalPremiumInfo();
  ...
  const memo = noop.useMemo(() => {
    if (consumed) {
      ...
      if (obj2.isThemeDark(closure_3)) {
        let tmp8Result = tmp8(10813);
      } else {
        tmp8Result = tmp8(10814);
      }
  ...
  if (consumed) {
    ...
    let tmp12Result = tmp12(tmp2(7504), obj2);
    let tmp15 = tmp12;
  } else {
    let obj3 = { skuId };
  ...
  obj5.children = tmp15(skuId(11200).CircleQuestionIcon, { style: tmp.questionIcon, color: consumed(576).colors.WHITE });
  items2[1] = tmp15(skuId(5174).PressableOpacity, obj5);
  ...
  if (fetchFractionalPremiumInfo.isLoading) {
    ...
    const items5 = [tmp15(tmp5(5026).Button, tmp18), ];
    ...
    items5[1] = tmp15(tmp5(5026).Button, obj11);
  ...
  items3[2] = tmp15(skuId(7227).ActionSheetHeaderBar, { variant: "floating" });
  obj12.children = items3;
  return closure_10(skuId(7223).BottomSheet, obj12);
};
