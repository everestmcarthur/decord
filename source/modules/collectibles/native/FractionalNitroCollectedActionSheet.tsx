...
// Dependencies: [19, 17, 1074, 1374, 21, 4639, 576, 8974, 5672, 13296, 4574, 7694, 4491, 10857, 10858, 4635, 1115, 2025, 4334, 7257, 7538, 5207, 11244, 5059, 4606, 7261, 2]
// Exports: default

// Module 13295 (FractionalNitroCollectedActionSheet)
import nativeDefault from "native" /* 576 */;
import util from "util" /* 1115 */;
import shared from "shared" /* 4491 */;
import Text_Text from "Text/Text" /* 4635 */;
import FastImageDefault from "FastImage" /* 5672 */;
import FractionalNitroCoinIllustration from "FractionalNitroCoinIllustration" /* 8974 */;
...
function NitroAcquiredHeader(skuId) {
  ...
  const obj2 = { source: _modDef13296 };
...
let size = { width: fn(8974).FRACTIONAL_NITRO_COIN_SIZE.COLLECTED_SHEET, height: fn(8974).FRACTIONAL_NITRO_COIN_SIZE.COLLECTED_SHEET, position: "absolute", top: "50%", left: "50%", transform: null };
let obj5 = { flex: 1, gap: nativeDefault.space.PX_16, alignSelf: "stretch" };
let items = [{ translateX: -fn(8974).FRACTIONAL_NITRO_COIN_SIZE.COLLECTED_SHEET / 2 }, ];
const obj6 = { translateX: -fn(8974).FRACTIONAL_NITRO_COIN_SIZE.COLLECTED_SHEET / 2 };
items[1] = { translateY: -fn(8974).FRACTIONAL_NITRO_COIN_SIZE.COLLECTED_SHEET / 2 };
...
export default function FractionalNitroCollectedActionSheet(skuId) {
  ...
  noop = tmp4;
  const fetchFractionalPremiumInfo = skuId(7694).useFetchFractionalPremiumInfo();
  ...
  const memo = noop.useMemo(() => {
    if (consumed) {
      ...
      if (obj2.isThemeDark(closure_3)) {
        let tmp8Result = tmp8(10857);
      } else {
        tmp8Result = tmp8(10858);
      }
  ...
  if (consumed) {
    ...
    let tmp12Result = tmp12(tmp2(7538), obj2);
    let tmp15 = tmp12;
  } else {
    let obj3 = { skuId };
  ...
  obj5.children = tmp15(skuId(11244).CircleQuestionIcon, { style: tmp.questionIcon, color: consumed(576).colors.WHITE });
  items2[1] = tmp15(skuId(5207).PressableOpacity, obj5);
  ...
  if (fetchFractionalPremiumInfo.isLoading) {
    ...
    const items5 = [tmp15(tmp5(5059).Button, tmp18), ];
    ...
    items5[1] = tmp15(tmp5(5059).Button, obj11);
  ...
  items3[2] = tmp15(skuId(7261).ActionSheetHeaderBar, { variant: "floating" });
  obj12.children = items3;
  return closure_10(skuId(7257).BottomSheet, obj12);
};
