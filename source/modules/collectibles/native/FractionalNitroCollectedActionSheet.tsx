...
// Dependencies: [19, 17, 1074, 1374, 21, 4722, 576, 9085, 5757, 13406, 4656, 7783, 4573, 10967, 10968, 4718, 1115, 2025, 4417, 7346, 7627, 5292, 11359, 5144, 4689, 7350, 2]
// Exports: default

// Module 13405 (FractionalNitroCollectedActionSheet)
import nativeDefault from "native" /* 576 */;
import util from "util" /* 1115 */;
import shared from "shared" /* 4573 */;
import Text_Text from "Text/Text" /* 4718 */;
import FastImageDefault from "FastImage" /* 5757 */;
import FractionalNitroCoinIllustration from "FractionalNitroCoinIllustration" /* 9085 */;
...
function NitroAcquiredHeader(skuId) {
  ...
  const obj2 = { source: _modDef13406 };
...
let size = { width: fn(9085).FRACTIONAL_NITRO_COIN_SIZE.COLLECTED_SHEET, height: fn(9085).FRACTIONAL_NITRO_COIN_SIZE.COLLECTED_SHEET, position: "absolute", top: "50%", left: "50%", transform: null };
let obj5 = { flex: 1, gap: nativeDefault.space.PX_16, alignSelf: "stretch" };
let items = [{ translateX: -fn(9085).FRACTIONAL_NITRO_COIN_SIZE.COLLECTED_SHEET / 2 }, ];
const obj6 = { translateX: -fn(9085).FRACTIONAL_NITRO_COIN_SIZE.COLLECTED_SHEET / 2 };
items[1] = { translateY: -fn(9085).FRACTIONAL_NITRO_COIN_SIZE.COLLECTED_SHEET / 2 };
...
export default function FractionalNitroCollectedActionSheet(skuId) {
  ...
  const tmp = closure_12();
  dependencyMap = tmp;
  const tmp4 = consumed(4656)();
  noop = tmp4;
  const fetchFractionalPremiumInfo = skuId(7783).useFetchFractionalPremiumInfo();
  ...
  const memo = noop.useMemo(() => {
    if (consumed) {
      ...
      if (obj2.isThemeDark(closure_3)) {
        let tmp8Result = tmp8(10967);
      } else {
        tmp8Result = tmp8(10968);
      }
  ...
  if (consumed) {
    ...
    let tmp12Result = tmp12(tmp2(7627), obj2);
    let tmp15 = tmp12;
  } else {
    let obj3 = { skuId };
  ...
  obj5.children = tmp15(skuId(11359).CircleQuestionIcon, { style: tmp.questionIcon, color: consumed(576).colors.WHITE });
  items2[1] = tmp15(skuId(5292).PressableOpacity, obj5);
  ...
  if (fetchFractionalPremiumInfo.isLoading) {
    ...
    const items5 = [tmp15(tmp5(5144).Button, tmp18), ];
    ...
    items5[1] = tmp15(tmp5(5144).Button, obj11);
  ...
  items3[2] = tmp15(skuId(7350).ActionSheetHeaderBar, { variant: "floating" });
  obj12.children = items3;
  return closure_10(skuId(7346).BottomSheet, obj12);
};
