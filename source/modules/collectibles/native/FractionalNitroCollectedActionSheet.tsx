...
// Dependencies: [19, 17, 1074, 1373, 21, 4607, 576, 8938, 5639, 13259, 4542, 7661, 4458, 10815, 10816, 4603, 1114, 2024, 4301, 7224, 7505, 5175, 11202, 5027, 4574, 7228, 2]
// Exports: default

// Module 13258 (FractionalNitroCollectedActionSheet)
import nativeDefault from "native" /* 576 */;
import util from "util" /* 1114 */;
import shared from "shared" /* 4458 */;
import Text_Text from "Text/Text" /* 4603 */;
import FastImageDefault from "FastImage" /* 5639 */;
import FractionalNitroCoinIllustration from "FractionalNitroCoinIllustration" /* 8938 */;
...
function NitroAcquiredHeader(skuId) {
  ...
  const obj2 = { source: _modDef13259 };
...
let size = { width: fn(8938).FRACTIONAL_NITRO_COIN_SIZE.COLLECTED_SHEET, height: fn(8938).FRACTIONAL_NITRO_COIN_SIZE.COLLECTED_SHEET, position: "absolute", top: "50%", left: "50%", transform: null };
let obj5 = { flex: 1, gap: nativeDefault.space.PX_16, alignSelf: "stretch" };
let items = [{ translateX: -fn(8938).FRACTIONAL_NITRO_COIN_SIZE.COLLECTED_SHEET / 2 }, ];
const obj6 = { translateX: -fn(8938).FRACTIONAL_NITRO_COIN_SIZE.COLLECTED_SHEET / 2 };
items[1] = { translateY: -fn(8938).FRACTIONAL_NITRO_COIN_SIZE.COLLECTED_SHEET / 2 };
...
export default function FractionalNitroCollectedActionSheet(skuId) {
  ...
  const tmp = closure_12();
  dependencyMap = tmp;
  const tmp4 = consumed(4542)();
  noop = tmp4;
  const fetchFractionalPremiumInfo = skuId(7661).useFetchFractionalPremiumInfo();
  ...
  const memo = noop.useMemo(() => {
    if (consumed) {
      ...
      if (obj2.isThemeDark(closure_3)) {
        let tmp8Result = tmp8(10815);
      } else {
        tmp8Result = tmp8(10816);
      }
  ...
  if (consumed) {
    ...
    let tmp12Result = tmp12(tmp2(7505), obj2);
    let tmp15 = tmp12;
  } else {
    let obj3 = { skuId };
  ...
  obj5.children = tmp15(skuId(11202).CircleQuestionIcon, { style: tmp.questionIcon, color: consumed(576).colors.WHITE });
  items2[1] = tmp15(skuId(5175).PressableOpacity, obj5);
  ...
  if (fetchFractionalPremiumInfo.isLoading) {
    ...
    const items5 = [tmp15(tmp5(5027).Button, tmp18), ];
    ...
    items5[1] = tmp15(tmp5(5027).Button, obj11);
  ...
  items3[2] = tmp15(skuId(7228).ActionSheetHeaderBar, { variant: "floating" });
  obj12.children = items3;
  return closure_10(skuId(7224).BottomSheet, obj12);
};
