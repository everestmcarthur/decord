...
// Function ID: 9112
// Name: SkeletonCard
// Dependencies: [19, 21, 4722, 576, 9004, 4458, 4723, 2]
// Exports: default

// Module 9111 (SkeletonCard)
...
export default function _default(width) {
  ...
  if (COLLECTIBLES_SHOP_CARD_WIDTH === undefined) {
    COLLECTIBLES_SHOP_CARD_WIDTH = sharedValue(9004).COLLECTIBLES_SHOP_CARD_WIDTH;
  }
  let COLLECTIBLES_SHOP_CARD_HEIGHT = width.height;
  sharedValue = undefined;
  if (COLLECTIBLES_SHOP_CARD_HEIGHT == null) {
    COLLECTIBLES_SHOP_CARD_HEIGHT = sharedValue(9004).COLLECTIBLES_SHOP_CARD_HEIGHT;
  }
  const tmp3Result = closure_5(COLLECTIBLES_SHOP_CARD_WIDTH, COLLECTIBLES_SHOP_CARD_HEIGHT);
  sharedValue = sharedValue(4458).useSharedValue(0.3);
  ...
  const animatedStyle = sharedValue(4458).useAnimatedStyle(C);
