...
// Function ID: 9012
// Name: SkeletonCard
// Dependencies: [19, 21, 4638, 576, 8904, 4374, 4639, 2]
// Exports: default

// Module 9011 (SkeletonCard)
...
export default function _default(width) {
  ...
  if (COLLECTIBLES_SHOP_CARD_WIDTH === undefined) {
    COLLECTIBLES_SHOP_CARD_WIDTH = sharedValue(8904).COLLECTIBLES_SHOP_CARD_WIDTH;
  }
  let COLLECTIBLES_SHOP_CARD_HEIGHT = width.height;
  sharedValue = undefined;
  if (COLLECTIBLES_SHOP_CARD_HEIGHT == null) {
    COLLECTIBLES_SHOP_CARD_HEIGHT = sharedValue(8904).COLLECTIBLES_SHOP_CARD_HEIGHT;
  }
  const tmp3Result = closure_5(COLLECTIBLES_SHOP_CARD_WIDTH, COLLECTIBLES_SHOP_CARD_HEIGHT);
  sharedValue = sharedValue(4374).useSharedValue(0.3);
  ...
  const animatedStyle = sharedValue(4374).useAnimatedStyle(C);
