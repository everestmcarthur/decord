...
// Function ID: 16054
// Name: ShopCategory
// Dependencies: [19, 17, 1076, 1074, 21, 9004, 4722, 576, 7358, 1484, 15281, 16049, 8957, 16054, 9007, 4689, 8406, 7737, 7378, 5292, 1115, 5757, 7405, 2]
// Exports: ShopCategory

// Module 16053 (ShopCategory)
...
const sum = 100 + fn(9004).COLLECTIBLES_SHOP_CARD_HEIGHT;
...
export const ShopCategory = function ShopCategory(category) {
  ...
  const filteredAndSortedProducts = category(15281).useFilteredAndSortedProducts({ products: category.products, bypassAndroidUnsyncedFilter: category.isOrbsExclusive });
  const mobileBannerUrl = category.mobileBannerUrl;
  let obj2 = category(15281);
  let obj3 = { products: category.products, bypassAndroidUnsyncedFilter: category.isOrbsExclusive };
  const collectiblesShopDeepLinkProps = category(16049).useCollectiblesShopDeepLinkProps({ products: filteredAndSortedProducts });
  ...
  const obj4 = category(16049);
  let items = [category.storeListingId];
  const recyclingState = category(8957).useRecyclingState(null, items, () => {
    const current = ref.current;
    if (current != null) {
      current.scrollToOffset({ offset: 0, animated: false });
    }
  });
  const obj6 = category(8957);
  let tmp9 = null != productIndex;
  if (tmp9) {
    tmp9 = productIndex > 0;
  }
  const obj7 = category(16054);
  const scrollToInitialIndexOnce = obj7.useScrollToInitialIndexOnce({ shouldScroll: tmp9, initialScrollIndex: productIndex, flashListRef: ref, afterMs: category(16054).INITIAL_SCROLL_DELAY_MS, resetKey: category.storeListingId });
  const obj8 = { shouldScroll: tmp9, initialScrollIndex: productIndex, flashListRef: ref, afterMs: category(16054).INITIAL_SCROLL_DELAY_MS, resetKey: category.storeListingId };
  collectiblesAnalyticsContext = category(9007).useCollectiblesAnalyticsContext();
  ...
  const tmp4Result = category(9007);
  ...
  if (tmp14Result) {
    ...
    tmp14Result = tmp14(tmp(5757), obj14);
  }
  const items4 = [tmp14Result, ];
  const obj13 = { radius: analyticsLocations(576).radii.lg };
  items4[1] = ref(filteredAndSortedProducts, { style: tmp3.viewAllIcon, children: ref(category(7405).ChevronSmallRightIcon, { size: "sm", color: "white" }) });
  obj11.children = items4;
  const items5 = [collectiblesAnalyticsContext(category(5292).PressableOpacity, obj11, category.storeListingId), ];
  ...
  obj17.snapToInterval = category(9004).COLLECTIBLES_SHOP_CARD_WIDTH + 12;
  ...
  items5[1] = ref(category(8957).FlashList, obj17);
  obj10.children = items5;
  obj9.children = collectiblesAnalyticsContext(filteredAndSortedProducts, obj10);
  return ref(category(9007).CollectiblesAnalyticsProvider, obj9);
};
