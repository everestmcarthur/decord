...
// Function ID: 15939
// Name: ShopCategory
// Dependencies: [19, 17, 1076, 1074, 21, 8904, 4638, 576, 7272, 1484, 15182, 15937, 9017, 15939, 8907, 4605, 8305, 7651, 7292, 5208, 1115, 5673, 7319, 2]
// Exports: ShopCategory

// Module 15938 (ShopCategory)
...
import openProductDetailsActionSheet from "openProductDetailsActionSheet" /* 8305 */;
...
const sum = 100 + fn(8904).COLLECTIBLES_SHOP_CARD_HEIGHT;
...
export const ShopCategory = function ShopCategory(category) {
  ...
  const filteredAndSortedProducts = category(15182).useFilteredAndSortedProducts({ products: category.products, bypassAndroidUnsyncedFilter: category.isOrbsExclusive });
  const mobileBannerUrl = category.mobileBannerUrl;
  let obj2 = category(15182);
  let obj3 = { products: category.products, bypassAndroidUnsyncedFilter: category.isOrbsExclusive };
  const collectiblesShopDeepLinkProps = category(15937).useCollectiblesShopDeepLinkProps({ products: filteredAndSortedProducts });
  ...
  const obj4 = category(15937);
  let items = [category.storeListingId];
  const recyclingState = category(9017).useRecyclingState(null, items, () => {
    const current = ref.current;
    if (current != null) {
      current.scrollToOffset({ offset: 0, animated: false });
    }
  });
  const obj6 = category(9017);
  let tmp9 = null != productIndex;
  if (tmp9) {
    tmp9 = productIndex > 0;
  }
  const obj7 = category(15939);
  const scrollToInitialIndexOnce = obj7.useScrollToInitialIndexOnce({ shouldScroll: tmp9, initialScrollIndex: productIndex, flashListRef: ref, afterMs: category(15939).INITIAL_SCROLL_DELAY_MS, resetKey: category.storeListingId });
  const obj8 = { shouldScroll: tmp9, initialScrollIndex: productIndex, flashListRef: ref, afterMs: category(15939).INITIAL_SCROLL_DELAY_MS, resetKey: category.storeListingId };
  collectiblesAnalyticsContext = category(8907).useCollectiblesAnalyticsContext();
  ...
  const tmp4Result = category(8907);
  ...
  if (tmp14Result) {
    ...
    tmp14Result = tmp14(tmp(5673), obj14);
  }
  const items4 = [tmp14Result, ];
  const obj13 = { radius: analyticsLocations(576).radii.lg };
  items4[1] = ref(filteredAndSortedProducts, { style: tmp3.viewAllIcon, children: ref(category(7319).ChevronSmallRightIcon, { size: "sm", color: "white" }) });
  obj11.children = items4;
  const items5 = [collectiblesAnalyticsContext(category(5208).PressableOpacity, obj11, category.storeListingId), ];
  ...
  obj17.snapToInterval = category(8904).COLLECTIBLES_SHOP_CARD_WIDTH + 12;
  ...
  items5[1] = ref(category(9017).FlashList, obj17);
  obj10.children = items5;
  obj9.children = collectiblesAnalyticsContext(filteredAndSortedProducts, obj10);
  return ref(category(8907).CollectiblesAnalyticsProvider, obj9);
};
