...
// Function ID: 15959
// Name: ShopCategory
// Dependencies: [19, 17, 1076, 1074, 21, 8920, 4640, 576, 7276, 1484, 15187, 15954, 8873, 15959, 8923, 4607, 8324, 7655, 7296, 5210, 1115, 5675, 7323, 2]
// Exports: ShopCategory

// Module 15958 (ShopCategory)
...
import openProductDetailsActionSheet from "openProductDetailsActionSheet" /* 8324 */;
...
const sum = 100 + fn(8920).COLLECTIBLES_SHOP_CARD_HEIGHT;
...
export const ShopCategory = function ShopCategory(category) {
  ...
  const filteredAndSortedProducts = category(15187).useFilteredAndSortedProducts({ products: category.products, bypassAndroidUnsyncedFilter: category.isOrbsExclusive });
  const mobileBannerUrl = category.mobileBannerUrl;
  let obj2 = category(15187);
  let obj3 = { products: category.products, bypassAndroidUnsyncedFilter: category.isOrbsExclusive };
  const collectiblesShopDeepLinkProps = category(15954).useCollectiblesShopDeepLinkProps({ products: filteredAndSortedProducts });
  ...
  const obj4 = category(15954);
  let items = [category.storeListingId];
  const recyclingState = category(8873).useRecyclingState(null, items, () => {
    const current = ref.current;
    if (current != null) {
      current.scrollToOffset({ offset: 0, animated: false });
    }
  });
  const obj6 = category(8873);
  let tmp9 = null != productIndex;
  if (tmp9) {
    tmp9 = productIndex > 0;
  }
  const obj7 = category(15959);
  const scrollToInitialIndexOnce = obj7.useScrollToInitialIndexOnce({ shouldScroll: tmp9, initialScrollIndex: productIndex, flashListRef: ref, afterMs: category(15959).INITIAL_SCROLL_DELAY_MS, resetKey: category.storeListingId });
  const obj8 = { shouldScroll: tmp9, initialScrollIndex: productIndex, flashListRef: ref, afterMs: category(15959).INITIAL_SCROLL_DELAY_MS, resetKey: category.storeListingId };
  collectiblesAnalyticsContext = category(8923).useCollectiblesAnalyticsContext();
  ...
  const tmp4Result = category(8923);
  ...
  if (tmp14Result) {
    ...
    tmp14Result = tmp14(tmp(5675), obj14);
  }
  const items4 = [tmp14Result, ];
  const obj13 = { radius: analyticsLocations(576).radii.lg };
  items4[1] = ref(filteredAndSortedProducts, { style: tmp3.viewAllIcon, children: ref(category(7323).ChevronSmallRightIcon, { size: "sm", color: "white" }) });
  obj11.children = items4;
  const items5 = [collectiblesAnalyticsContext(category(5210).PressableOpacity, obj11, category.storeListingId), ];
  ...
  obj17.snapToInterval = category(8920).COLLECTIBLES_SHOP_CARD_WIDTH + 12;
  ...
  items5[1] = ref(category(8873).FlashList, obj17);
  obj10.children = items5;
  obj9.children = collectiblesAnalyticsContext(filteredAndSortedProducts, obj10);
  return ref(category(8923).CollectiblesAnalyticsProvider, obj9);
};
