...
// Function ID: 13224
// Name: InlinePriceTag
// Dependencies: [19, 17, 7289, 1076, 1074, 21, 4605, 576, 4601, 1889, 7605, 7606, 8906, 1114, 8241, 4262, 13224, 8938, 8917, 13225, 504, 8930, 1115, 8931, 8732, 2]
// Exports: default
...
function OrbsPriceTag(arg0) {
  ...
  if (null == vcData.price) {
    ...
    if (result) {
      result = tmp16(7605).isOrbsExclusiveProduct(product);
      const tmp16Result = tmp16(7605);
    }
    ...
    if (result) {
      result = discountPercentage >= tmp16(7606).DISCOUNT_DISPLAY_MINIMUM_THRESHOLD;
    }
    ...
    if (result) {
      ...
      obj2.icon = React7(tmp16(8906).OrbsIcon, obj3);
    ...
    if (!result) {
      ...
      tmp10Result = tmp10(tmp16(8906).OrbsIcon, obj6);
    }
...
export default function InlinePriceTag(arg0) {
  ...
  if (obj7.useStateFromStores(items, () => fetchingGoogleSkus.isFetchingGoogleSkus())) {
    if (null == formattedPriceForCollectiblesProduct) {
      return closure_9(tmp2(8930).CollectiblesShopPricePlaceholder, {});
    }
  }
  if (null == formattedPriceForCollectiblesProduct) {
    let tmp21 = null;
    ...
    const formattedPriceForCollectiblesProduct1 = tmp2(8917).getFormattedPriceForCollectiblesProduct(product, true, true);
    const tmp2Result = tmp2(8917);
    ...
    if (!(null != formattedPriceForCollectiblesProduct1 && canUseShopDiscountsResult)) {
      ...
      if (tmp28Result5) {
        ...
        obj17.style = items3;
        const items4 = [tmp28(tmp2(8732).NitroWheelIcon, obj17), ];
        ...
        items4[1] = tmp28(tmp2(4601).Text, obj18);
      ...
      if (shopDiscountSource === tmp2(7606).ShopDiscountSource.THIRDPARTY) {
        const obj23 = { color: "interactive-text-active", style: tmp.nitroIcon };
        let tmp28Result7 = tmp28(tmp2(8931).TagIcon, obj23);
      } else {
        const obj24 = { color: "interactive-text-active", style: tmp.nitroIcon };
        tmp28Result7 = tmp28(tmp2(8732).NitroWheelIcon, obj24);
      }
      obj21.icon = tmp28Result7;
      tmp28(tmp29, obj21);
      tmp2Result6 = tmp2(1115);
    }
    tmp2Result4 = tmp2(7606);
  }
};
