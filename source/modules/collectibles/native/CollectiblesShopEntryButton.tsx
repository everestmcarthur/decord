...
// Dependencies: [32, 19, 7698, 1955, 21, 7502, 1944, 17042, 12289, 1115, 17044, 563, 14100, 7679, 10774, 2]
// Exports: default

// Module 17043 (CollectiblesShopEntryButton)
import YouScreenNavIconDefault from "YouScreenNavIcon" /* 17042 */;
import MobileShopButtonCoachmarkDefault from "MobileShopButtonCoachmark" /* 17044 */;
...
function CoachmarkVariant(shopButtonRef) {
  ...
  const tmp = _slicedToArray(navigateToShop(7502).useSelectedVersionedDismissibleContent(navigateToShop(1944).DismissibleContent.COLLECTIBLES_SHOP_ENTRY_MARKETING, marketing.version, undefined, true), 2);
  importDefault = tmp2;
  ...
  const obj = navigateToShop(7502);
  obj3.IconComponent = navigateToShop(12289).ShopIcon;
...
export default function CollectiblesShopEntryButton(navigateToShop) {
  ...
  const stateFromStores = navigateToShop(563).useStateFromStores(items, () => marketingBySurface.getMarketingBySurface(navigateToShop(14100).CollectiblesMarketingSurface.MOBILE_SHOP_BUTTON));
  ...
  if (type === navigateToShop(7679).CollectiblesMarketingType.COACHMARK) {
    ...
    function content(visibleContent) {
      ...
      const obj = { ref: markAsDismissed, IconComponent: navigateToShop(12289).ShopIcon, accessibilityLabel: null, onPress: null, showRedDot: null };
      const intl = navigateToShop(1115).intl;
      ...
      return closure_1_6(shopButtonRef(17042), obj);
    }
    if (tmp4) {
      let type1;
      if (stateFromStores != null) {
      ...
      let prop = null;
      if (type1 === tmp(7679).CollectiblesMarketingType.BADGE) {
      ...
      let tmp15Result = tmp15(tmp(10774).SelectedVersionedDismissibleContent, obj3);
    } else {
      let type2;
      if (stateFromStores != null) {
        type2 = stateFromStores.type;
      }
      if (type2 === tmp(7679).CollectiblesMarketingType.BADGE) {
