...
// Dependencies: [32, 19, 7599, 1954, 21, 7402, 1943, 16834, 12168, 1114, 16836, 563, 14012, 7580, 10657, 2]
// Exports: default
...
function CoachmarkVariant(shopButtonRef) {
  ...
  const tmp = _slicedToArray(navigateToShop(7402).useSelectedVersionedDismissibleContent(navigateToShop(1943).DismissibleContent.COLLECTIBLES_SHOP_ENTRY_MARKETING, marketing.version, undefined, true), 2);
  importDefault = tmp2;
  ...
  const obj = navigateToShop(7402);
  obj3.IconComponent = navigateToShop(12168).ShopIcon;
...
export default function CollectiblesShopEntryButton(navigateToShop) {
  ...
  const stateFromStores = navigateToShop(563).useStateFromStores(items, () => marketingBySurface.getMarketingBySurface(navigateToShop(14012).CollectiblesMarketingSurface.MOBILE_SHOP_BUTTON));
  ...
  if (type === navigateToShop(7580).CollectiblesMarketingType.COACHMARK) {
    ...
    if (tmp4) {
      ...
      let tmp15Result = tmp15(tmp(10657).SelectedVersionedDismissibleContent, obj3);
    } else {
      let type2;
      if (stateFromStores != null) {
