...
// Dependencies: [32, 19, 7658, 1954, 21, 7462, 1943, 16879, 12233, 1114, 16881, 563, 14041, 7639, 10716, 2]
// Exports: default
...
import CollectiblesMarketingsStore from "CollectiblesMarketingsStore" /* 7658 */;

const require = fn;
function CoachmarkVariant(shopButtonRef) {
  ({ marketing, navigateToShop } = shopButtonRef);
  shopButtonRef = shopButtonRef.shopButtonRef;
  const tmp = _slicedToArray(navigateToShop(7462).useSelectedVersionedDismissibleContent(navigateToShop(1943).DismissibleContent.COLLECTIBLES_SHOP_ENTRY_MARKETING, marketing.version, undefined, true), 2);
  importDefault = tmp2;
  ...
  const obj = navigateToShop(7462);
  obj3.IconComponent = navigateToShop(12233).ShopIcon;
...
export default function CollectiblesShopEntryButton(navigateToShop) {
  ...
  const stateFromStores = navigateToShop(563).useStateFromStores(items, () => marketingBySurface.getMarketingBySurface(navigateToShop(14041).CollectiblesMarketingSurface.MOBILE_SHOP_BUTTON));
  ...
  if (type === navigateToShop(7639).CollectiblesMarketingType.COACHMARK) {
    ...
    function content(visibleContent) {
      ...
      const obj = { ref: markAsDismissed, IconComponent: navigateToShop(12233).ShopIcon, accessibilityLabel: null, onPress: null, showRedDot: null };
      const intl = navigateToShop(1114).intl;
    ...
    if (tmp4) {
      ...
      let prop = null;
      if (type1 === tmp(7639).CollectiblesMarketingType.BADGE) {
      ...
      let tmp15Result = tmp15(tmp(10716).SelectedVersionedDismissibleContent, obj3);
    } else {
      let type2;
      if (stateFromStores != null) {
        type2 = stateFromStores.type;
      }
      if (type2 === tmp(7639).CollectiblesMarketingType.BADGE) {
