...
// Dependencies: [32, 19, 7657, 1954, 21, 7461, 1943, 16879, 12231, 1114, 16881, 563, 14039, 7638, 10714, 2]
// Exports: default

// Module 16880 (CollectiblesShopEntryButton)
import YouScreenNavIconDefault from "YouScreenNavIcon" /* 16879 */;
import MobileShopButtonCoachmarkDefault from "MobileShopButtonCoachmark" /* 16881 */;
import _slicedToArray from "module_32" /* 32 */;
import noop from "module_19" /* 19 */;
import CollectiblesMarketingsStore from "CollectiblesMarketingsStore" /* 7657 */;

const require = fn;
function CoachmarkVariant(shopButtonRef) {
  ({ marketing, navigateToShop } = shopButtonRef);
  shopButtonRef = shopButtonRef.shopButtonRef;
  const tmp = _slicedToArray(navigateToShop(7461).useSelectedVersionedDismissibleContent(navigateToShop(1943).DismissibleContent.COLLECTIBLES_SHOP_ENTRY_MARKETING, marketing.version, undefined, true), 2);
  importDefault = tmp2;
  ...
  const obj = navigateToShop(7461);
  obj3.IconComponent = navigateToShop(12231).ShopIcon;
...
export default function CollectiblesShopEntryButton(navigateToShop) {
  ...
  const stateFromStores = navigateToShop(563).useStateFromStores(items, () => marketingBySurface.getMarketingBySurface(navigateToShop(14039).CollectiblesMarketingSurface.MOBILE_SHOP_BUTTON));
  ...
  if (type === navigateToShop(7638).CollectiblesMarketingType.COACHMARK) {
    ...
    function content(visibleContent) {
      ...
      const obj = { ref: markAsDismissed, IconComponent: navigateToShop(12231).ShopIcon, accessibilityLabel: null, onPress: null, showRedDot: null };
      const intl = navigateToShop(1114).intl;
      ...
      return closure_1_6(shopButtonRef(16879), obj);
    }
    if (tmp4) {
      let type1;
      if (stateFromStores != null) {
      ...
      let prop = null;
      if (type1 === tmp(7638).CollectiblesMarketingType.BADGE) {
      ...
      let tmp15Result = tmp15(tmp(10714).SelectedVersionedDismissibleContent, obj3);
    } else {
      let type2;
      if (stateFromStores != null) {
        type2 = stateFromStores.type;
      }
      if (type2 === tmp(7638).CollectiblesMarketingType.BADGE) {
