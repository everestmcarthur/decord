...
// Dependencies: [19, 17, 1076, 8869, 21, 4605, 576, 8289, 5637, 11083, 8264, 8893, 8234, 8868, 1886, 8889, 1889, 1077, 8915, 8914, 8881, 1114, 4601, 7605, 8917, 11084, 8241, 4262, 8933, 2]
// Exports: default

// Module 11082 (CollectiblesShopCheckoutDetails)
...
import useProfileEffectDefault from "useProfileEffect" /* 8289 */;
...
import getProductName from "getProductName" /* 8933 */;
...
function CollectibleProductPreviewContent(userAvatarSource) {
  ...
  if (ALL.has(product.skuId)) {
    const size = { skuId: product.skuId, width: tmp2(8915).FRACTIONAL_NITRO_COIN_SIZE.CHECKOUT, height: tmp2(8915).FRACTIONAL_NITRO_COIN_SIZE.CHECKOUT };
    return React5(tmp2(8915).FractionalNitroCoinIllustration, size);
  } else if (product.skuId === EXTERNAL_PRODUCT_SKU_IDS.ORB_PROFILE_BADGE) {
    const obj = { source: null, style: null };
    const obj2 = { uri: _modDef8914 };
    obj.source = obj2;
...
function ProductPriceAmountTag(product) {
  ...
  const memo = noop.useMemo(() => {
    if (useOrbPrice) {
      ...
      const orbPrice = tmp(7605).getProductOrbPrice(obj);
      const tmpResult = tmp(7605);
    } else {
      const priceText = tmp(8917).getFormattedPriceForCollectiblesProduct(product, hasShopDiscount, true);
      const tmpResult2 = tmp(8917);
    }
...
const CollectiblesPreviewConstants = fn(8869);
...
export default function CollectiblesShopCheckoutDetails(recipientUser) {
  ...
  const canUseShopDiscountsResult = flag2(4262).canUseShopDiscounts(currentUser);
  ...
  const memo = noop.useMemo(() => {
    ...
    if (flag2) {
      ...
      const productOrbPrice = tmp(7605).getProductOrbPrice(obj2);
      let str2;
      ...
      str = tmp(8917).getFormattedPriceForCollectiblesProduct(tmp3, hasShopDiscount, true);
      if (str == null) {
        str = "";
      }
      const tmpResult2 = tmp(8917);
  ...
  if (flag3) {
    ...
    obj5.children = tmp10(tmp3(4601).Text, obj6);
