...
// Dependencies: [19, 17, 1076, 8939, 21, 4638, 576, 8354, 5673, 11164, 8330, 8963, 8300, 8938, 1887, 8959, 1890, 1077, 8985, 8984, 8951, 1115, 4634, 7663, 8987, 11165, 8307, 4295, 9003, 2]
// Exports: default

// Module 11163 (CollectiblesShopCheckoutDetails)
...
import BundleSampleV2Default from "BundleSampleV2" /* 8938 */;
import AvatarDecorationSampleV2Default from "AvatarDecorationSampleV2" /* 8951 */;
import NameplateDefault from "Nameplate" /* 8959 */;
import _modDef8984 from "module_8984" /* 8984 */;
import getProductName from "getProductName" /* 9003 */;
...
function CollectibleProductPreviewContent(userAvatarSource) {
  ...
  if (ALL.has(product.skuId)) {
    const size = { skuId: product.skuId, width: tmp2(8985).FRACTIONAL_NITRO_COIN_SIZE.CHECKOUT, height: tmp2(8985).FRACTIONAL_NITRO_COIN_SIZE.CHECKOUT };
    return React5(tmp2(8985).FractionalNitroCoinIllustration, size);
...
function ProductPriceAmountTag(product) {
  ...
  const memo = noop.useMemo(() => {
    if (useOrbPrice) {
      ...
      const orbPrice = tmp(7663).getProductOrbPrice(obj);
      const tmpResult = tmp(7663);
    } else {
      const priceText = tmp(8987).getFormattedPriceForCollectiblesProduct(product, hasShopDiscount, true);
      const tmpResult2 = tmp(8987);
    }
...
const CollectiblesPreviewConstants = fn(8939);
...
export default function CollectiblesShopCheckoutDetails(recipientUser) {
  ...
  const canUseShopDiscountsResult = flag2(4295).canUseShopDiscounts(currentUser);
  ...
  const memo = noop.useMemo(() => {
    ...
    if (flag2) {
      ...
      const productOrbPrice = tmp(7663).getProductOrbPrice(obj2);
      let str2;
      ...
      str = tmp(8987).getFormattedPriceForCollectiblesProduct(tmp3, hasShopDiscount, true);
      if (str == null) {
        str = "";
      }
      const tmpResult2 = tmp(8987);
  ...
  if (flag3) {
    ...
    obj5.children = tmp10(tmp3(4634).Text, obj6);
