...
// Dependencies: [19, 17, 1076, 8928, 21, 4639, 576, 8345, 5672, 11150, 8321, 8952, 8291, 8927, 1887, 8948, 1890, 1077, 8974, 8973, 8940, 1115, 4635, 7660, 8976, 11151, 8298, 4297, 8992, 2]
// Exports: default

// Module 11149 (CollectiblesShopCheckoutDetails)
...
import useMaybeFetchProfileFrameDefault from "useMaybeFetchProfileFrame" /* 8321 */;
import useProfileEffectDefault from "useProfileEffect" /* 8345 */;
import BundleSampleV2Default from "BundleSampleV2" /* 8927 */;
...
import getProductName from "getProductName" /* 8992 */;
...
function ProfileFrameAssetPreview(arg0) {
  ...
  if (null != tmp3) {
    ...
    tmp4 = React5(tmp(8952), obj);
    const tmpResult = tmp(8952);
  }
  return tmp4;
}
...
function CollectibleProductPreviewContent(userAvatarSource) {
  ...
  if (ALL.has(product.skuId)) {
    const size = { skuId: product.skuId, width: tmp2(8974).FRACTIONAL_NITRO_COIN_SIZE.CHECKOUT, height: tmp2(8974).FRACTIONAL_NITRO_COIN_SIZE.CHECKOUT };
    return React5(tmp2(8974).FractionalNitroCoinIllustration, size);
...
function ProductPriceAmountTag(product) {
  ...
  const memo = noop.useMemo(() => {
    if (useOrbPrice) {
      ...
      const orbPrice = tmp(7660).getProductOrbPrice(obj);
      const tmpResult = tmp(7660);
    } else {
      const priceText = tmp(8976).getFormattedPriceForCollectiblesProduct(product, hasShopDiscount, true);
...
const CollectiblesPreviewConstants = fn(8928);
...
export default function CollectiblesShopCheckoutDetails(recipientUser) {
  ...
  const canUseShopDiscountsResult = flag2(4297).canUseShopDiscounts(currentUser);
  ...
  const memo = noop.useMemo(() => {
    ...
    if (flag2) {
      ...
      const productOrbPrice = tmp(7660).getProductOrbPrice(obj2);
      let str2;
      ...
      str = tmp(8976).getFormattedPriceForCollectiblesProduct(tmp3, hasShopDiscount, true);
      if (str == null) {
        str = "";
      }
      const tmpResult2 = tmp(8976);
  ...
  if (flag3) {
    ...
    obj5.children = tmp10(tmp3(4635).Text, obj6);
