...
// Dependencies: [19, 17, 1076, 8892, 21, 4607, 576, 8312, 5639, 11108, 8287, 8916, 8257, 8891, 1886, 8912, 1889, 1077, 8938, 8937, 8904, 1114, 4603, 7627, 8940, 11109, 8264, 4264, 8956, 2]
// Exports: default

// Module 11107 (CollectiblesShopCheckoutDetails)
...
import useProfileEffectDefault from "useProfileEffect" /* 8312 */;
...
import NameplateDefault from "Nameplate" /* 8912 */;
...
function CollectibleProductPreviewContent(userAvatarSource) {
  ...
  if (ALL.has(product.skuId)) {
    const size = { skuId: product.skuId, width: tmp2(8938).FRACTIONAL_NITRO_COIN_SIZE.CHECKOUT, height: tmp2(8938).FRACTIONAL_NITRO_COIN_SIZE.CHECKOUT };
    return React5(tmp2(8938).FractionalNitroCoinIllustration, size);
  } else if (product.skuId === EXTERNAL_PRODUCT_SKU_IDS.ORB_PROFILE_BADGE) {
    const obj = { source: null, style: null };
    const obj2 = { uri: _modDef8937 };
    obj.source = obj2;
...
function ProductPriceAmountTag(product) {
  ...
  const memo = noop.useMemo(() => {
    if (useOrbPrice) {
      ...
      const orbPrice = tmp(7627).getProductOrbPrice(obj);
      const tmpResult = tmp(7627);
    } else {
      const priceText = tmp(8940).getFormattedPriceForCollectiblesProduct(product, hasShopDiscount, true);
      const tmpResult2 = tmp(8940);
    }
...
const CollectiblesPreviewConstants = fn(8892);
...
export default function CollectiblesShopCheckoutDetails(recipientUser) {
  ...
  const canUseShopDiscountsResult = flag2(4264).canUseShopDiscounts(currentUser);
  ...
  const memo = noop.useMemo(() => {
    ...
    if (flag2) {
      ...
      const productOrbPrice = tmp(7627).getProductOrbPrice(obj2);
      let str2;
      ...
      str = tmp(8940).getFormattedPriceForCollectiblesProduct(tmp3, hasShopDiscount, true);
      if (str == null) {
        str = "";
      }
      const tmpResult2 = tmp(8940);
  ...
  if (flag3) {
    ...
    obj5.children = tmp10(tmp3(4603).Text, obj6);
