...
// Dependencies: [32, 19, 17, 7671, 1076, 1074, 11245, 1374, 21, 4640, 576, 8057, 11191, 4607, 11168, 1115, 13306, 9024, 8997, 1484, 7276, 4843, 13309, 1897, 7655, 13313, 1077, 13314, 11238, 8992, 4636, 5063, 1890, 8326, 504, 11317, 4297, 7668, 7667, 9005, 11244, 13316, 13317, 1612, 5062, 13318, 2]
// Exports: default

// Module 13308 (ProductDetailsActionSheetPurchaseSection)
...
function VCButton(balance) {
  ...
  noop = tmp;
  const virtualCurrencyData = balance(13306).useVirtualCurrencyData(product, flag);
  ({ price, canAfford } = virtualCurrencyData);
  let obj = balance(13306);
  let isPartiallyOwnedBundle = balance(9024).useProductDisableState(product.skuId).isDisabled;
  const obj2 = balance(9024);
  ...
  closure_7 = noop.useCallback(() => {
    ActionSheetActionCreatorsDefault.hideAllActionSheets();
    ModalActionCreatorsDefault.pushLazy(asyncRequireImpl(13309, dependencyMap.paths), {
      skuId: product.skuId,
      analyticsLocations,
      onCheckoutSuccess(arg0) {
        const collectiblesPurchases = balance(7655).fetchCollectiblesPurchases();
        const obj = balance(7655);
        product(4843).popWithKey(ORB_CHECKOUT_MODAL);
        if (closure_1_1.skuId === constants.ORB_PROFILE_BADGE) {
          ...
          tmp4(4843).pushLazy(tmp(1897)(13313, tmp2.paths), obj3, modalKey);
          const tmp4Result = tmp4(4843);
        } else {
          ...
          if (ALL.has(tmp6.skuId)) {
            ...
            obj4.onPressExplorePerks = function onPressExplorePerks() {
              ...
              closure_2_1(4607).hideActionSheet();
            };
            obj4.onPressViewCredits = function onPressViewCredits() {
              navigation.navigate(constants2.PREMIUM_MANAGE_PLAN);
              closure_2_1(4607).hideActionSheet();
            };
            tmp4Result3.openLazy(tmp(1897)(13314, tmp2.paths), "FractionalNitroCollectedActionSheet", obj4);
            const tmp10 = tmp(1897)(13314, tmp2.paths);
          } else {
            ...
            tmp4(11238).open(obj5);
            const tmp4Result4 = tmp4(11238);
          }
        }
      }
    }, ORB_CHECKOUT_MODAL);
  ...
  if (null == price) {
    ...
    obj8.variant = str2;
    obj7.children = closure_13(tmp2(5063).BaseTextButton, obj8);
...
export default function ProductDetailsActionSheetPurchaseSection(product) {
  ...
  const balance = require("module_9005").useFetchVirtualCurrencyBalance().balance;
  const obj9 = require("module_9005");
