...
// Dependencies: [19, 17, 4714, 2022, 21, 4722, 576, 1887, 8446, 8389, 504, 4566, 4956, 1177, 9059, 11146, 11147, 4718, 2]
// Exports: NameplatePreview

// Module 11472 (NameplatePreview)
...
export const NameplatePreview = function NameplatePreview(aria_hidden) {
  ...
  const avatarDecoration = user(8446).useAvatarDecoration(user, guildId);
  pendingAvatarDecoration = guildId(8389)({ guildId }).pendingAvatarDecoration;
  const obj2 = user(8446);
  ...
  const obj4 = user(504);
  const name = guildId(4566).useName(user);
  ...
  const tmp15 = guildId(4956)({ userId: user.id, guildId, pendingDisplayNameStyles });
  ...
  const items3 = [closure_7(guildId(9059), { nameplate: nameplateData, style: tmp.nameplate, fullOpacity: true, animate: flag2 }), closure_7(pendingAvatarDecoration, { style: tmp.avatar, children: memo }), ];
  const obj10 = { style: tmp.content, children: null };
  let tmp19Result = null != tmp15;
  if (tmp19Result) {
    const obj11 = { userId: user.id, guildId, userName: pendingGlobalName, variant: "text-md/semibold", effectDisplayType: tmp4(11147).EffectDisplayType.STATIC, lineClamp: 1, pendingDisplayNameStyles };
    tmp19Result = tmp19(tmp7(11146), obj11);
    const tmp7Result = tmp7(11146);
  }
  ...
  if (tmp19Result2) {
    ...
    tmp19Result2 = tmp19(tmp4(4718).Text, obj12);
  }
