...
// Dependencies: [19, 17, 4599, 2021, 21, 4607, 576, 1886, 8303, 8245, 504, 4451, 4841, 1178, 8912, 10990, 10991, 4603, 2]
// Exports: NameplatePreview

// Module 11316 (NameplatePreview)
...
export const NameplatePreview = function NameplatePreview(aria_hidden) {
  ...
  const avatarDecoration = user(8303).useAvatarDecoration(user, guildId);
  pendingAvatarDecoration = guildId(8245)({ guildId }).pendingAvatarDecoration;
  const obj2 = user(8303);
  ...
  const tmp15 = guildId(4841)({ userId: user.id, guildId, pendingDisplayNameStyles });
  ...
  const items3 = [closure_7(guildId(8912), { nameplate: nameplateData, style: tmp.nameplate, fullOpacity: true, animate: flag2 }), closure_7(pendingAvatarDecoration, { style: tmp.avatar, children: memo }), ];
  const obj10 = { style: tmp.content, children: null };
  let tmp19Result = null != tmp15;
  if (tmp19Result) {
    const obj11 = { userId: user.id, guildId, userName: pendingGlobalName, variant: "text-md/semibold", effectDisplayType: tmp4(10991).EffectDisplayType.STATIC, lineClamp: 1, pendingDisplayNameStyles };
    tmp19Result = tmp19(tmp7(10990), obj11);
  ...
  if (tmp19Result2) {
    ...
    tmp19Result2 = tmp19(tmp4(4603).Text, obj12);
  }
