...
// Dependencies: [19, 17, 4632, 2022, 21, 4640, 576, 1887, 8364, 8307, 504, 4484, 4874, 1177, 8975, 11053, 11054, 4636, 2]
// Exports: NameplatePreview

// Module 11374 (NameplatePreview)
...
export const NameplatePreview = function NameplatePreview(aria_hidden) {
  ...
  const avatarDecoration = user(8364).useAvatarDecoration(user, guildId);
  pendingAvatarDecoration = guildId(8307)({ guildId }).pendingAvatarDecoration;
  const obj2 = user(8364);
  ...
  const obj4 = user(504);
  const name = guildId(4484).useName(user);
  ...
  const tmp15 = guildId(4874)({ userId: user.id, guildId, pendingDisplayNameStyles });
  ...
  const items3 = [closure_7(guildId(8975), { nameplate: nameplateData, style: tmp.nameplate, fullOpacity: true, animate: flag2 }), closure_7(pendingAvatarDecoration, { style: tmp.avatar, children: memo }), ];
  const obj10 = { style: tmp.content, children: null };
  let tmp19Result = null != tmp15;
  if (tmp19Result) {
    const obj11 = { userId: user.id, guildId, userName: pendingGlobalName, variant: "text-md/semibold", effectDisplayType: tmp4(11054).EffectDisplayType.STATIC, lineClamp: 1, pendingDisplayNameStyles };
    tmp19Result = tmp19(tmp7(11053), obj11);
    const tmp7Result = tmp7(11053);
  }
  ...
  if (tmp19Result2) {
    ...
    tmp19Result2 = tmp19(tmp4(4636).Text, obj12);
  }
