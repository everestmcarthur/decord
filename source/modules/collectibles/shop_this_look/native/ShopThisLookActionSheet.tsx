...
// Dependencies: [19, 17, 8348, 7261, 7318, 21, 4638, 576, 8913, 4336, 1115, 9013, 504, 13262, 7663, 13264, 11186, 8978, 8344, 13240, 7272, 7292, 4605, 7651, 11304, 4634, 2]
// Exports: default

// Module 13261 (ShopThisLookActionSheet)
...
import ShopThisLookUtils from "ShopThisLookUtils" /* 13262 */;
import ShopThisLookAnalyticsUtils from "ShopThisLookAnalyticsUtils" /* 13264 */;
...
function ShopThisLookCard(skuId) {
  ...
  const collectiblesShopProduct = skuId(9013).useCollectiblesShopProduct(skuId, { needsCategory: false, shouldFetchProduct: false });
  ...
  const memo1 = stateFromStores.useMemo(() => {
    if (null == c2) {
      ...
      if (obj.getIsVariantProduct(tmp)) {
        ...
        return tmp2(7663).getSelectedProduct(tmp, bound);
      } else {
        return tmp;
      }
  ...
  if ("loading" === collectiblesShopProduct.state) {
    ...
    let tmp23 = closure_9(onPress(8913), obj3);
  } else {
    tmp23 = null;
    if (null != stateFromStores) {
      if (memo) {
        ...
        const items7 = [closure_9(onPress(11186), obj5), ];
        ...
        if (tmp17Result) {
          ...
          tmp17Result = tmp17(tmp18(8978), callback1);
        }
        ...
        const obj6 = { sku: stateFromStores, size, overlay: tmp(8913).WishlistItemCardOverlay.LOCKED, onPress: tmp10 };
        tmp15Result = closure_9(onPress(11186), obj6);
        const tmp13 = onPress(11186);
      }
    }
  }
  return tmp23;
...
let obj6 = {};
const merged = Object.assign(fn(8913).CARD_TOP_RIGHT_OVERLAY_POSITION);
obj6.zIndex = 1;
...
export default function ShopThisLookActionSheet(arg0) {
  ...
  ({ cardWidth: c0, rowWidth, gap } = analyticsLocations(13240)({ maxWidth: ACTION_SHEET_MAX_WIDTH }));
  const tmp2 = analyticsLocations(13240)({ maxWidth: ACTION_SHEET_MAX_WIDTH });
  analyticsLocations = analyticsLocations(7272)(analyticsLocations(7292).USER_PROFILE_OVERFLOW_MENU).analyticsLocations;
  ...
  const items1 = [analyticsLocations(7292).SHOP_THIS_LOOK_ACTION_SHEET];
  obj3.value = items1;
  ...
  obj3.children = closure_9(analyticsLocations(11304), obj4);
