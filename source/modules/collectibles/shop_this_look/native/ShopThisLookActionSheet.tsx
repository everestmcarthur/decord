...
// Dependencies: [19, 17, 8304, 7224, 7281, 21, 4606, 576, 8864, 4305, 1114, 13208, 504, 13212, 7626, 13214, 11128, 8929, 8300, 13186, 7235, 7255, 4573, 7614, 11246, 4602, 2]
// Exports: default
...
function ShopThisLookCard(skuId) {
  ...
  const collectiblesShopProduct = skuId(13208).useCollectiblesShopProduct(skuId, { needsCategory: false, shouldFetchProduct: false });
  ...
  const memo1 = stateFromStores.useMemo(() => {
    if (null == c2) {
      ...
      if (obj.getIsVariantProduct(tmp)) {
        ...
        return tmp2(7626).getSelectedProduct(tmp, bound);
      } else {
        return tmp;
      }
  ...
  if ("loading" === collectiblesShopProduct.state) {
    ...
    let tmp23 = closure_9(onPress(8864), obj3);
  } else {
    tmp23 = null;
    if (null != stateFromStores) {
      if (memo) {
        ...
        const items7 = [closure_9(onPress(11128), obj5), ];
        ...
        if (tmp17Result) {
          ...
          tmp17Result = tmp17(tmp18(8929), callback1);
        }
        ...
        const obj6 = { sku: stateFromStores, size, overlay: tmp(8864).WishlistItemCardOverlay.LOCKED, onPress: tmp10 };
        tmp15Result = closure_9(onPress(11128), obj6);
...
export default function ShopThisLookActionSheet(arg0) {
  ...
  ({ cardWidth: c0, rowWidth, gap } = analyticsLocations(13186)({ maxWidth: ACTION_SHEET_MAX_WIDTH }));
  const tmp2 = analyticsLocations(13186)({ maxWidth: ACTION_SHEET_MAX_WIDTH });
  ...
  obj3.children = closure_9(analyticsLocations(11246), obj4);
