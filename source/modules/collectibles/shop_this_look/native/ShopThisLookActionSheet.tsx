...
// Dependencies: [19, 17, 8283, 7203, 7260, 21, 4605, 576, 8843, 4303, 1114, 13185, 504, 13189, 7605, 13191, 11105, 8908, 8279, 13163, 7214, 7234, 4572, 7593, 11223, 4601, 2]
// Exports: default
...
function ShopThisLookCard(skuId) {
  ...
  const collectiblesShopProduct = skuId(13185).useCollectiblesShopProduct(skuId, { needsCategory: false, shouldFetchProduct: false });
  ...
  const memo1 = stateFromStores.useMemo(() => {
    if (null == c2) {
      ...
      if (obj.getIsVariantProduct(tmp)) {
        ...
        return tmp2(7605).getSelectedProduct(tmp, bound);
      } else {
        return tmp;
      }
  ...
  if ("loading" === collectiblesShopProduct.state) {
    ...
    let tmp23 = closure_9(onPress(8843), obj3);
  } else {
    tmp23 = null;
    if (null != stateFromStores) {
      if (memo) {
        ...
        const items7 = [closure_9(onPress(11105), obj5), ];
        ...
        if (tmp17Result) {
          ...
          tmp17Result = tmp17(tmp18(8908), callback1);
        }
        ...
        const obj6 = { sku: stateFromStores, size, overlay: tmp(8843).WishlistItemCardOverlay.LOCKED, onPress: tmp10 };
        tmp15Result = closure_9(onPress(11105), obj6);
...
export default function ShopThisLookActionSheet(arg0) {
  ...
  ({ cardWidth: c0, rowWidth, gap } = analyticsLocations(13163)({ maxWidth: ACTION_SHEET_MAX_WIDTH }));
  const tmp2 = analyticsLocations(13163)({ maxWidth: ACTION_SHEET_MAX_WIDTH });
  ...
  obj3.children = closure_9(analyticsLocations(11223), obj4);
