...
// Dependencies: [19, 17, 8367, 7265, 7322, 21, 4640, 576, 8929, 4338, 1115, 9029, 504, 13270, 7667, 13272, 11194, 8994, 8363, 13248, 7276, 7296, 4607, 7655, 11312, 4636, 2]
// Exports: default

// Module 13269 (ShopThisLookActionSheet)
...
import ShopThisLookUtils from "ShopThisLookUtils" /* 13270 */;
import ShopThisLookAnalyticsUtils from "ShopThisLookAnalyticsUtils" /* 13272 */;
...
function ShopThisLookCard(skuId) {
  ...
  const collectiblesShopProduct = skuId(9029).useCollectiblesShopProduct(skuId, { needsCategory: false, shouldFetchProduct: false });
  ...
  const memo1 = stateFromStores.useMemo(() => {
    if (null == c2) {
      ...
      if (obj.getIsVariantProduct(tmp)) {
        ...
        return tmp2(7667).getSelectedProduct(tmp, bound);
      } else {
        return tmp;
      }
  ...
  if ("loading" === collectiblesShopProduct.state) {
    ...
    let tmp23 = closure_9(onPress(8929), obj3);
  } else {
    tmp23 = null;
    if (null != stateFromStores) {
      if (memo) {
        ...
        const items7 = [closure_9(onPress(11194), obj5), ];
        ...
        if (tmp17Result) {
          ...
          tmp17Result = tmp17(tmp18(8994), callback1);
        }
        ...
        const obj6 = { sku: stateFromStores, size, overlay: tmp(8929).WishlistItemCardOverlay.LOCKED, onPress: tmp10 };
        tmp15Result = closure_9(onPress(11194), obj6);
        const tmp13 = onPress(11194);
      }
    }
  }
  return tmp23;
...
let obj6 = {};
const merged = Object.assign(fn(8929).CARD_TOP_RIGHT_OVERLAY_POSITION);
obj6.zIndex = 1;
...
export default function ShopThisLookActionSheet(arg0) {
  ...
  ({ cardWidth: c0, rowWidth, gap } = analyticsLocations(13248)({ maxWidth: ACTION_SHEET_MAX_WIDTH }));
  const tmp2 = analyticsLocations(13248)({ maxWidth: ACTION_SHEET_MAX_WIDTH });
  analyticsLocations = analyticsLocations(7276)(analyticsLocations(7296).USER_PROFILE_OVERFLOW_MENU).analyticsLocations;
  ...
  const items1 = [analyticsLocations(7296).SHOP_THIS_LOOK_ACTION_SHEET];
  obj3.value = items1;
  const obj4 = { startExpanded: true, title: null, children: null };
  const tmp3 = analyticsLocations(7276);
  ...
  obj3.children = closure_9(analyticsLocations(11312), obj4);
