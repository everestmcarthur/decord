...
// Dependencies: [19, 17, 8308, 1372, 8340, 1074, 7265, 1374, 21, 4640, 576, 4608, 8054, 14743, 8052, 1115, 7080, 4594, 5695, 11351, 8057, 11353, 7076, 4376, 4641, 4644, 13228, 8053, 6766, 4351, 11352, 1612, 504, 4297, 7276, 7296, 7266, 9546, 9516, 8339, 1241, 8345, 4338, 1478, 11152, 14744, 4636, 5665, 7264, 7263, 6738, 2]
// Exports: default

// Module 14742 (CustomizeBadgesSheet)
...
import EyeSlashIcon from "EyeSlashIcon" /* 7080 */;
import ContextMenu from "ContextMenu" /* 8052 */;
import ContextMenuConstants from "ContextMenuConstants" /* 8054 */;
...
import BadgeCatalogIconDefault from "BadgeCatalogIcon" /* 11351 */;
import BadgeUtils from "BadgeUtils" /* 11353 */;
import PendingBadgeSettings from "PendingBadgeSettings" /* 13228 */;
import BadgeGrid from "BadgeGrid" /* 14743 */;
  ...
  if (flag) {
    if (null != onShowPress) {
      if (!alwaysVisible) {
        ...
        obj5.icon = tmp10(tmp8(7080).EyeSlashIcon, obj6);
        ...
        obj4.children = tmp10(tmp8(8057).IconButton, obj5);
...
getSlotOffset.__closure = { BADGE_GRID_COLUMNS: fn(14743).BADGE_GRID_COLUMNS, BADGE_GRID_GAP: fn(14743).BADGE_GRID_GAP };
    ...
    if (badge.hidden) {
      ...
      function renderTile(ref) {
        ...
        if (alwaysVisible) {
          ...
          stringResult = intl2.string(tmp6(11353).getAlwaysVisibleCopy(tmp9));
          const tmp6Result = tmp6(11353);
        }
  ...
  if (typeof closure_22 === "function") {
    ...
    let fn2 = function $() {
      ...
      if (result !== value) {
        ...
        tmp(4376).runOnJS(tmp(4608).triggerHapticFeedback)(tmp(4608).HapticFeedbackTypes.DRAG_AND_DROP_MOVE);
        const runOnJSResult = tmp(4376).runOnJS(tmp(4608).triggerHapticFeedback);
      }
    };
    ...
    closure_24 = slotOffset.useCallback((nativeEvent) => {
      ...
      if ("moveup" === actionName) {
        ...
        if (result !== value) {
          ...
          const AccessibilityAnnouncer = tmp4(4351).AccessibilityAnnouncer;
    ...
    function renderTile(ref) {
      ...
      if (alwaysVisible) {
        ...
        stringResult = intl2.string(tmp2(tmp3[21]).getAlwaysVisibleCopy(tmp10));
...
export default function CustomizeBadgesSheet(analyticsLocations) {
  ...
  ee.__closure = { autoScrollSpeed: sharedValue2, autoScrollElapsed: sharedValue3, MS_PER_FRAME_60FPS, AUTO_SCROLL_PIXELS_PER_SECOND: 700, scrollTo: tenureBadgeHideable(stateFromStores1[23]).scrollTo, scrollRef: animatedRef, roundToNearestPixel: stateFromStores(stateFromStores1[44]), scrollOffset: scrollViewOffset };
  ...
  const obj5 = { autoScrollSpeed: sharedValue2, autoScrollElapsed: sharedValue3, MS_PER_FRAME_60FPS, AUTO_SCROLL_PIXELS_PER_SECOND: 700, scrollTo: tenureBadgeHideable(stateFromStores1[23]).scrollTo, scrollRef: animatedRef, roundToNearestPixel: stateFromStores(stateFromStores1[44]), scrollOffset: scrollViewOffset };
  function ie() {
  ...
  if (hasCatalog) {
    ...
    if (!stateFromStores1) {
      ...
      obj8.children = sharedValue(tmp5(tmp3[46]).Text, obj9);
      tmp40 = sharedValue(tmp2(tmp3[45]), obj8);
    ...
    if (stateFromStoresObject.hasCatalogError) {
      ...
      obj12.children = tmp35(tmp5(tmp3[46]).Text, obj13);
      let obj14 = obj12;
    } else {
