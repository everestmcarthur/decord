...
// Dependencies: [1371, 4429, 4198, 4431, 4241, 4430, 1090, 504, 573, 2]

// Module 5594 (GuildJoinRequestStore)
import initializeDefault from "initialize" /* 504 */;
import DispatcherDefault from "Dispatcher" /* 573 */;
import DurationsDefault from "Durations" /* 1090 */;
import _modDef4198 from "module_4198" /* 4198 */;
import GuildJoinRequestUtils from "GuildJoinRequestUtils" /* 4430 */;
  ...
  if (DELETED !== applicationStatus) {
    if (null != DELETED) {
      if (DELETED === MemberVerificationTypes.GuildJoinRequestApplicationStatuses.SUBMITTED) {
        ...
        const result = map.set(guildId, _modDef4198());
      }
      if (applicationStatus === tmp12(4431).GuildJoinRequestApplicationStatuses.SUBMITTED) {
        const _Math = Math;
        dependencyMap[guildId] = Math.max(0, dependencyMap[guildId] - 1);
        const result1 = map.set(guildId, _modDef4198());
      }
      tmp12 = require;
    }
  }
...
const joinRequestFromServer = fn(4429).joinRequestFromServer;
const map = new Map();
const dependencyMap = {};
let closure_8 = {};
let c9 = false;
...
const secondaryIndexMap = new fn(4241).SecondaryIndexMap(guildJoinRequestsIndex, (joinRequestId) => "" + joinRequestId.joinRequestId);
const secondaryIndexMap1 = new fn(4241).SecondaryIndexMap(guildJoinRequestsIndex, (joinRequestId) => "" + joinRequestId.joinRequestId);
const secondaryIndexMap2 = new fn(4241).SecondaryIndexMap(guildJoinRequestsIndex, (actionedAt) => "" + actionedAt.actionedAt);
...
prototype["hasFetched"] = function hasFetched(arg0) {
  if (map.has(arg0)) {
    ...
    let tmp3 = null != value;
    if (tmp3) {
      tmp3 = _modDef4198().diff(value, "seconds") < closure_20;
...
const guildJoinRequestStoreV2 = new GuildJoinRequestStoreV2(DispatcherDefault, {
  ...
  GUILD_JOIN_REQUESTS_FETCH_SUCCESS: function handleFetchSuccess(arg0) {
    ...
    if (status === MemberVerificationTypes.GuildJoinRequestApplicationStatuses.SUBMITTED) {
      ...
      let result = map.set(guildId, _modDef4198());
    }
