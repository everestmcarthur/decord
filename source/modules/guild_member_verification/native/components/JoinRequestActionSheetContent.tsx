...
// Dependencies: [19, 17, 1957, 7203, 7260, 21, 4605, 576, 8304, 8293, 8290, 8301, 8242, 16667, 8309, 8319, 11183, 13130, 13194, 504, 12703, 4428, 5025, 5121, 1114, 4429, 9099, 4601, 6666, 4286, 11, 4565, 5483, 8150, 4556, 4558, 1611, 16670, 2]

// Module 16669 (JoinRequestActionSheetContent)
...
function OpenInterviewButton(arg0) {
  ...
  const joinRequestButtonActions = interviewChannelId(12703).useJoinRequestButtonActions(joinRequest, interviewChannelId);
  ({ handleOpenInterview, submitting } = joinRequestButtonActions);
  const obj2 = interviewChannelId(12703);
  if (!obj3.isActionedApplicationStatus(joinRequest.applicationStatus)) {
    ...
    obj4.icon = closure_8(tmp(5121).ChatIcon, obj5);
    if (label == null) {
    ...
    let tmp6Result = closure_8(tmp(5025).Button, obj4);
  } else {
    tmp6Result = null;
  }
  return tmp6Result;
...
const paddingTop = fn(7260).PROFILE_CONTENT_WITHOUT_STATUS_TOP_PADDING;
const jsxProd = fn(21);
...
let closure_12 = noop.memo((user) => {
  ...
  const tmp3 = joinRequest(8304)();
  const tmp4 = joinRequest(8293)(ACTION_SHEET_MAX_WIDTH);
  ({ primaryColor, secondaryColor, theme } = joinRequest(8290)({ user, displayProfile }));
  const tmp5 = joinRequest(8290)({ user, displayProfile });
  const tmp6 = user;
  const userProfileColors = user(8301).useUserProfileColors({ theme, primaryColor, secondaryColor });
  ...
  if (null != user) {
    ...
    const items1 = [closure_8(tmp(8309), obj3), ];
    ...
    obj5.onPress = tmp8;
    const items2 = [closure_8(tmp(8319), obj5), ];
    ...
    const items4 = [closure_8(tmp6(13130).PrimaryInfo, obj10), ];
    const obj11 = { user };
    items4[1] = closure_8(tmp(13194), obj11);
    obj9.children = items4;
    obj7.children = closure_9(View, obj9);
    items2[1] = closure_8(tmp(11183), obj7);
...
let closure_14 = noop.memo((joinRequest) => {
  ...
  if (MemberVerificationTypes.GuildJoinRequestApplicationStatuses.REJECTED === applicationStatus) {
    ...
    const items2 = [React6(tmp2(6666).CircleXIcon, obj10), ];
    const obj11 = { variant: "text-md/medium", color: "mobile-text-heading-primary", children: null };
    const intl3 = tmp2(1114).intl;
    obj11.children = intl3.string(tmp2(1114).t.bSZkla);
    const items3 = [React6(tmp2(4601).Text, obj11), , ];
    ...
    if (tmp20Result) {
      ...
      obj17.children = intl5.formatToPlainString(tmp2(1114).t.fU5PPM, obj18);
      tmp20Result = tmp20(tmp2(4601).Text, obj17);
    }
  ...
  } else if (tmp2(4429).GuildJoinRequestApplicationStatuses.APPROVED === applicationStatus) {
    ...
    const items6 = [React6(tmp2(4565).CircleCheckIcon, obj22), ];
    const obj23 = { variant: "text-md/medium", color: "mobile-text-heading-primary", children: null };
    const intl = tmp2(1114).intl;
    obj23.children = intl.string(tmp2(1114).t.aURgY2);
    const items7 = [React6(tmp2(4601).Text, obj23), ];
    let tmp6Result = null;
    if (null != actionedByUser) {
      tmp6Result = null;
      if (null != actionedAt) {
...
let closure_15 = noop.memo((joinRequest) => {
  ...
  const joinRequestButtonActions = joinRequest(12703).useJoinRequestButtonActions(joinRequest, joinRequest.interviewChannelId, callback);
  ...
  const obj = joinRequest(12703);
  const tmp5 = closure_9;
  obj2.icon = closure_8(joinRequest(4556).CheckmarkLargeIcon, { color: nativeDefault.colors.WHITE, size: "lg" });
  const intl = joinRequest(1114).intl;
  obj2.label = intl.string(joinRequest(1114).t.BzjDQJ);
  obj2.onPress = approveRequest;
  obj2.disabled = submitting;
  const children = [closure_8(joinRequest(8150).IconButton, obj2), , ];
  ...
  obj4.icon = closure_8(joinRequest(4558).XLargeIcon, { color: nativeDefault.colors.WHITE, size: "lg" });
  const intl2 = joinRequest(1114).intl;
  obj4.label = intl2.string(joinRequest(1114).t.hDtbsz);
  obj4.onPress = rejectRequest;
  obj4.disabled = submitting;
  children[1] = closure_8(joinRequest(8150).IconButton, obj4);
  ...
  if (tmp6Result) {
    ...
    obj6.icon = tmp6(tmp2(5121).ChatIcon, obj7);
    const intl3 = tmp2(1114).intl;
    obj6.label = intl3.string(tmp2(1114).t.KQeYoC);
    obj6.onPress = handleOpenInterview;
    obj6.disabled = submitting;
    tmp6Result = tmp6(tmp2(8150).IconButton, obj6);
  }
  children[2] = tmp6Result;
  return tmp5(joinRequest(5483).ButtonGroup, { direction: "horizontal", align: "flex-start", justify: "space-evenly", children });
});
let closure_16 = noop.memo((arg0) => {
  ...
  if (MemberVerificationTypes.VerificationFormFieldTypes.TERMS === field_type) {
    ...
    const items1 = [React6(tmp2(4601).Text, obj3), ];
    ...
    items1[1] = React6(tmp2(4565).CircleCheckIcon, obj4);
    obj2.children = items1;
    return React7(View, obj2);
  } else if (tmp2(4429).VerificationFormFieldTypes.MULTIPLE_CHOICE === field_type) {
    const obj5 = { style: tmp.formQuestion, variant: "text-sm/semibold", color: "text-subtle", children: field.label };
    const items2 = [React6(tmp2(4601).Text, obj5), ];
    ...
    obj6.children = React6(tmp2(4601).Text, obj8);
    ...
    const items4 = [React6(tmp2(4601).Text, obj9), ];
    ...
    obj10.children = React6(tmp2(4601).Text, obj11);
...
export default noop.memo(function JoinRequestActionSheetContent(displayProfile) {
  ...
  const obj = { style: { paddingBottom: memo(1611)().bottom }, children: null };
  const items1 = [closure_8(closure_12, { joinRequest, user, displayProfile: displayProfile.displayProfile }), , , , ];
  if (joinRequest.applicationStatus === joinRequest(4429).GuildJoinRequestApplicationStatuses.SUBMITTED) {
  ...
  items1[4] = closure_8(memo(16670), { guildId: joinRequest.guildId, userId: joinRequest.userId, selectedJoinRequestId: joinRequest.joinRequestId });
  obj.children = items1;
