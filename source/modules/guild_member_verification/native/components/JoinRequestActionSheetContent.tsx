...
// Dependencies: [19, 17, 1958, 7261, 7318, 21, 4638, 576, 8369, 8358, 8355, 8366, 8308, 16714, 8374, 8384, 11264, 13207, 13267, 504, 12776, 4461, 5060, 5156, 1115, 4462, 9173, 4634, 6724, 4319, 11, 4597, 5519, 8214, 4588, 4590, 1612, 16717, 2]

// Module 16716 (JoinRequestActionSheetContent)
...
function OpenInterviewButton(arg0) {
  ...
  const joinRequestButtonActions = interviewChannelId(12776).useJoinRequestButtonActions(joinRequest, interviewChannelId);
  ({ handleOpenInterview, submitting } = joinRequestButtonActions);
  const obj2 = interviewChannelId(12776);
  if (!obj3.isActionedApplicationStatus(joinRequest.applicationStatus)) {
    ...
    obj4.icon = closure_8(tmp(5156).ChatIcon, obj5);
    if (label == null) {
    ...
    let tmp6Result = closure_8(tmp(5060).Button, obj4);
  } else {
    tmp6Result = null;
  }
  return tmp6Result;
...
const paddingTop = fn(7318).PROFILE_CONTENT_WITHOUT_STATUS_TOP_PADDING;
const jsxProd = fn(21);
...
let closure_12 = noop.memo((user) => {
  ...
  const tmp3 = joinRequest(8369)();
  const tmp4 = joinRequest(8358)(ACTION_SHEET_MAX_WIDTH);
  ({ primaryColor, secondaryColor, theme } = joinRequest(8355)({ user, displayProfile }));
  const tmp5 = joinRequest(8355)({ user, displayProfile });
  const tmp6 = user;
  const userProfileColors = user(8366).useUserProfileColors({ theme, primaryColor, secondaryColor });
  ...
  if (null != user) {
    ...
    const items1 = [closure_8(tmp(8374), obj3), ];
    ...
    obj5.onPress = tmp8;
    const items2 = [closure_8(tmp(8384), obj5), ];
    ...
    const items4 = [closure_8(tmp6(13207).PrimaryInfo, obj10), ];
    const obj11 = { user };
    items4[1] = closure_8(tmp(13267), obj11);
    obj9.children = items4;
    obj7.children = closure_9(View, obj9);
    items2[1] = closure_8(tmp(11264), obj7);
...
let closure_14 = noop.memo((joinRequest) => {
  ...
  if (applicationStatus === MemberVerificationTypes.GuildJoinRequestApplicationStatuses.SUBMITTED) {
    if (null != joinRequest.interviewChannelId) {
      ...
      const items = [React6(tmp2(9173).HourglassIcon, obj4), ];
      ...
      obj5.children = React6(tmp2(4634).Text, obj6);
  ...
  if (MemberVerificationTypes.GuildJoinRequestApplicationStatuses.REJECTED === applicationStatus) {
    ...
    const items2 = [React6(tmp2(6724).CircleXIcon, obj10), ];
    ...
    const items3 = [React6(tmp2(4634).Text, obj11), , ];
    ...
    if (tmp20Result) {
      ...
      tmp20Result = tmp20(tmp2(4634).Text, obj17);
    }
  ...
  } else if (tmp2(4462).GuildJoinRequestApplicationStatuses.APPROVED === applicationStatus) {
    ...
    const items6 = [React6(tmp2(4597).CircleCheckIcon, obj22), ];
    ...
    const items7 = [React6(tmp2(4634).Text, obj23), ];
    let tmp6Result = null;
    if (null != actionedByUser) {
      tmp6Result = null;
      if (null != actionedAt) {
...
let closure_15 = noop.memo((joinRequest) => {
  ...
  const joinRequestButtonActions = joinRequest(12776).useJoinRequestButtonActions(joinRequest, joinRequest.interviewChannelId, callback);
  ...
  const obj = joinRequest(12776);
  const tmp5 = closure_9;
  obj2.icon = closure_8(joinRequest(4588).CheckmarkLargeIcon, { color: nativeDefault.colors.WHITE, size: "lg" });
  ...
  const children = [closure_8(joinRequest(8214).IconButton, obj2), , ];
  ...
  obj4.icon = closure_8(joinRequest(4590).XLargeIcon, { color: nativeDefault.colors.WHITE, size: "lg" });
  const intl2 = joinRequest(1115).intl;
  obj4.label = intl2.string(joinRequest(1115).t.hDtbsz);
  obj4.onPress = rejectRequest;
  obj4.disabled = submitting;
  children[1] = closure_8(joinRequest(8214).IconButton, obj4);
  ...
  if (tmp6Result) {
    ...
    obj6.icon = tmp6(tmp2(5156).ChatIcon, obj7);
    const intl3 = tmp2(1115).intl;
    obj6.label = intl3.string(tmp2(1115).t.KQeYoC);
    obj6.onPress = handleOpenInterview;
    obj6.disabled = submitting;
    tmp6Result = tmp6(tmp2(8214).IconButton, obj6);
  }
  children[2] = tmp6Result;
  return tmp5(joinRequest(5519).ButtonGroup, { direction: "horizontal", align: "flex-start", justify: "space-evenly", children });
});
let closure_16 = noop.memo((arg0) => {
  ...
  if (MemberVerificationTypes.VerificationFormFieldTypes.TERMS === field_type) {
    ...
    const items1 = [React6(tmp2(4634).Text, obj3), ];
    ...
    items1[1] = React6(tmp2(4597).CircleCheckIcon, obj4);
    obj2.children = items1;
    return React7(View, obj2);
  } else if (tmp2(4462).VerificationFormFieldTypes.MULTIPLE_CHOICE === field_type) {
    const obj5 = { style: tmp.formQuestion, variant: "text-sm/semibold", color: "text-subtle", children: field.label };
    const items2 = [React6(tmp2(4634).Text, obj5), ];
    ...
    obj6.children = React6(tmp2(4634).Text, obj8);
    ...
    const items4 = [React6(tmp2(4634).Text, obj9), ];
    ...
    obj10.children = React6(tmp2(4634).Text, obj11);
...
export default noop.memo(function JoinRequestActionSheetContent(displayProfile) {
  ...
  if (joinRequest.applicationStatus === joinRequest(4462).GuildJoinRequestApplicationStatuses.SUBMITTED) {
  ...
  items1[4] = closure_8(memo(16717), { guildId: joinRequest.guildId, userId: joinRequest.userId, selectedJoinRequestId: joinRequest.joinRequestId });
  obj.children = items1;
