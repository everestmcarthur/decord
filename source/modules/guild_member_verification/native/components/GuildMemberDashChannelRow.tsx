...
// Dependencies: [19, 1074, 1965, 10267, 21, 4640, 576, 16375, 5629, 4464, 1101, 12531, 1115, 5176, 1177, 2]
// Exports: default

// Module 16374 (GuildMemberDashChannelRow)
...
let obj2 = { container: { marginVertical: fn(10267).CHANNEL_MARGIN_VERTICAL, marginHorizontal: 8, borderRadius: nativeDefault.radii.md }, badge: null, badgeText: null };
...
export default function GuildMemberDashChannelRow(arg0) {
  ...
  const tmp = closure_8();
  const id = guild.id;
  let num = id(16375).useSubmittedGuildJoinRequestTotal({ guildId: id });
  if (num == null) {
    num = 0;
  }
  ...
  const ChannelModes = tmp2(12531).ChannelModes;
  ...
  obj3.mode = tmp7;
  obj2.name = jsx(id(12531).BaseChannelName, { name: null, mode: null });
  const tmp9 = hasItem(12531);
  obj2.icon = jsx(id(12531).BaseChannelIcon, { mode: tmp7, IconComponent: id(5176).GroupIcon });
  let tmp8Result = null;
  if (num > 0) {
