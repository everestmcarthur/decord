...
// Dependencies: [19, 17, 21, 4605, 576, 4601, 1114, 4556, 5025, 4429, 5671, 4808, 7094, 1896, 7097, 7133, 7010, 2]
// Exports: default

// Module 5670 (IdentityVerificationField)
...
function BaseIdentityVerificationField(icon) {
  ...
  if (passesVerification) {
    ...
    obj4.children = tmp6(tmp7(4556).CheckmarkLargeIcon, { color: "status-positive" });
    let obj5 = obj4;
  } else {
    ...
    obj5.children = tmp6(tmp7(5025).Button, obj6);
  }
...
export default function IdentityVerificationField(arg0) {
  ...
  if (MemberVerificationTypes.UserVerificationFieldPlatforms.EMAIL === platform) {
    ...
    if (tmp(4429).UserVerificationFieldPlatforms.PHONE === platform) {
    ...
    if (tmp(4429).UserVerificationFieldPlatforms.EMAIL === platform) {
      let EnvelopeIcon = tmp(7133).EnvelopeIcon;
    } else if (tmp(4429).UserVerificationFieldPlatforms.PHONE === platform) {
      EnvelopeIcon = tmp(7010).MobilePhoneIcon;
    } else {
      EnvelopeIcon = tmp(7133).EnvelopeIcon;
    }
    if (tmp(4429).UserVerificationFieldPlatforms.EMAIL === platform) {
      let fn = () => {
      ...
      fn = tmp(4429).UserVerificationFieldPlatforms.PHONE === platform ? (() => {
