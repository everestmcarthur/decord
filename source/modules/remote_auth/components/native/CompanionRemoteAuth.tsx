...
// Dependencies: [19, 17, 1074, 21, 4605, 16070, 5627, 4601, 1114, 5025, 1178, 4449, 1483, 4842, 1242, 16071, 16075, 7022, 2]
// Exports: CompanionRemoteAuth

// Module 16069 (CompanionRemoteAuth)
import NativeAuthenticationModuleDefault from "NativeAuthenticationModule" /* 16075 */;
...
export const CompanionRemoteAuth = function CompanionRemoteAuth() {
  ...
  const context = noop.useContext(fingerprint(4842));
  ...
  let obj2 = noop;
  const state = navigation(16071).useAuthWebsocket(callback, true).state;
  const items = [navigation];
  const callback1 = noop.useCallback(() => {
    navigation.goBack();
  }, items);
  fingerprint = null;
  if (state.step === navigation(16070).RemoteAuthStep.PENDING_REMOTE_INIT) {
  ...
  const obj3 = navigation(16071);
  const intl = tmp2(1114).intl;
  obj4.headerText = intl.string(navigation(1114).t["7fNJgA"]);
  const obj5 = { style: tmp.statusContainer, children: null };
  const step = state.step;
  if (navigation(16070).RemoteAuthStep.INITIALIZING !== step) {
    if (tmp2(16070).RemoteAuthStep.PENDING_REMOTE_INIT !== step) {
      if (tmp2(16070).RemoteAuthStep.PENDING_TICKET === step) {
        ...
        obj11.children = tmp11(tmp2(5025).Button, obj12);
        items2[3] = tmp11(tmp13, obj11);
        obj6.children = items2;
        let tmp11Result = closure_9(closure_8, obj6);
        const tmp5Result2 = tmp5(4449);
      } else if (tmp2(16070).RemoteAuthStep.PENDING_LOGIN === step) {
        tmp11Result = tmp11(tmp2(5627).ActivityIndicator, {});
      }
    }
  ...
  const items3 = [closure_7(navigation(5627).ActivityIndicator, {}), , ];
  ...
  items3[1] = closure_7(navigation(4601).Text, obj14);
  ...
  obj15.children = closure_7(navigation(5025).Button, obj16);
