...
// Dependencies: [19, 17, 1074, 21, 4638, 16118, 5663, 4634, 1115, 5060, 1177, 4482, 1484, 4875, 1241, 16119, 16123, 7080, 2]
// Exports: CompanionRemoteAuth

// Module 16117 (CompanionRemoteAuth)
import NativeAuthenticationModuleDefault from "NativeAuthenticationModule" /* 16123 */;
...
export const CompanionRemoteAuth = function CompanionRemoteAuth() {
  ...
  const context = noop.useContext(fingerprint(4875));
  ...
  let obj2 = noop;
  const state = navigation(16119).useAuthWebsocket(callback, true).state;
  const items = [navigation];
  const callback1 = noop.useCallback(() => {
    navigation.goBack();
  }, items);
  fingerprint = null;
  if (state.step === navigation(16118).RemoteAuthStep.PENDING_REMOTE_INIT) {
  ...
  const obj3 = navigation(16119);
  const intl = tmp2(1115).intl;
  obj4.headerText = intl.string(navigation(1115).t["7fNJgA"]);
  const obj5 = { style: tmp.statusContainer, children: null };
  const step = state.step;
  if (navigation(16118).RemoteAuthStep.INITIALIZING !== step) {
    if (tmp2(16118).RemoteAuthStep.PENDING_REMOTE_INIT !== step) {
      if (tmp2(16118).RemoteAuthStep.PENDING_TICKET === step) {
        ...
        obj11.children = tmp11(tmp2(5060).Button, obj12);
        items2[3] = tmp11(tmp13, obj11);
        obj6.children = items2;
        let tmp11Result = closure_9(closure_8, obj6);
        const tmp5Result2 = tmp5(4482);
      } else if (tmp2(16118).RemoteAuthStep.PENDING_LOGIN === step) {
        tmp11Result = tmp11(tmp2(5663).ActivityIndicator, {});
      }
    }
  ...
  const items3 = [closure_7(navigation(5663).ActivityIndicator, {}), , ];
  ...
  items3[1] = closure_7(navigation(4634).Text, obj14);
  ...
  obj15.children = closure_7(navigation(5060).Button, obj16);
