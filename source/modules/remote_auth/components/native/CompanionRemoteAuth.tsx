...
// Dependencies: [19, 17, 1074, 21, 4639, 16098, 5662, 4635, 1115, 5059, 1177, 4484, 1484, 4876, 1241, 16099, 16103, 7077, 2]
// Exports: CompanionRemoteAuth

// Module 16097 (CompanionRemoteAuth)
import NativeAuthenticationModuleDefault from "NativeAuthenticationModule" /* 16103 */;
...
export const CompanionRemoteAuth = function CompanionRemoteAuth() {
  ...
  const context = noop.useContext(fingerprint(4876));
  ...
  let obj2 = noop;
  const state = navigation(16099).useAuthWebsocket(callback, true).state;
  const items = [navigation];
  const callback1 = noop.useCallback(() => {
    navigation.goBack();
  }, items);
  fingerprint = null;
  if (state.step === navigation(16098).RemoteAuthStep.PENDING_REMOTE_INIT) {
  ...
  const obj3 = navigation(16099);
  const intl = tmp2(1115).intl;
  obj4.headerText = intl.string(navigation(1115).t["7fNJgA"]);
  const obj5 = { style: tmp.statusContainer, children: null };
  const step = state.step;
  if (navigation(16098).RemoteAuthStep.INITIALIZING !== step) {
    if (tmp2(16098).RemoteAuthStep.PENDING_REMOTE_INIT !== step) {
      if (tmp2(16098).RemoteAuthStep.PENDING_TICKET === step) {
        ...
        obj11.children = tmp11(tmp2(5059).Button, obj12);
        items2[3] = tmp11(tmp13, obj11);
        obj6.children = items2;
        let tmp11Result = closure_9(closure_8, obj6);
        const tmp5Result2 = tmp5(4484);
      } else if (tmp2(16098).RemoteAuthStep.PENDING_LOGIN === step) {
        tmp11Result = tmp11(tmp2(5662).ActivityIndicator, {});
      }
    }
  ...
  const items3 = [closure_7(navigation(5662).ActivityIndicator, {}), , ];
  ...
  items3[1] = closure_7(navigation(4635).Text, obj14);
  ...
  obj15.children = closure_7(navigation(5059).Button, obj16);
