...
// Dependencies: [32, 19, 17, 1074, 21, 4574, 576, 1611, 13890, 13888, 5595, 1272, 12, 13891, 4570, 1114, 1178, 5447, 4989, 4777, 13892, 13893, 5591, 2]
// Exports: default

// Module 13889 (RemoteAuthModal)
...
import _modDef13890 from "module_13890" /* 13890 */;
import _modDef13892 from "module_13892" /* 13892 */;
import _modDef13893 from "module_13893" /* 13893 */;
import _slicedToArray from "module_32" /* 32 */;
import noop from "module_19" /* 19 */;

const _modDef13891 = tmp7(13891);
require = fn;
function RemoteAuthBody(remoteAuthFingerprint) {
  ...
  const effect = noop.useEffect(() => {
    ...
    HTTP.post(request).then((body) => {
      ...
      const result = remoteAuthFingerprint(5595).DeprecatedLayoutAnimation();
    }).catch(() => {
...
function RemoteAuthLogin(arg0) {
  ...
  const items = [closure_9(closure_6, { source: _modDef13891, style: tmp.mainImage }), , , ];
...
function RemoteAuthLoginSucceeded() {
  ...
  const items = [React7(timestampProducer, { source: _modDef13892, style: tmp.mainImage }), , , ];
...
function RemoteAuthNotFound() {
  const tmp = closure_12();
  const obj = { children: null };
  const items = [React7(timestampProducer, { source: _modDef13893, style: tmp.mainImage }), , , ];
  const obj3 = { variant: "heading-xl/extrabold", children: null };
  const intl = util.intl;
...
export default function RemoteAuth(arg0) {
  const tmp = closure_12();
  const obj = { source: _modDef13890, imageStyle: null, style: null, children: null };
  ({ imageStyle: obj.imageStyle, background: obj.style } = tmp);
  const obj2 = { style: null, source: _modDef13888 };
