...
// Function ID: 9628
// Name: CameraPreview
// Dependencies: [32, 19, 17, 1957, 4654, 9628, 4660, 502, 9629, 9610, 9611, 1074, 4659, 21, 6763, 4374, 1177, 504, 9630, 9617, 9639, 9618, 5211, 1612, 9636, 8462, 8402, 9631, 5768, 4639, 9727, 1115, 9728, 9730, 9731, 9633, 9613, 9619, 9794, 9795, 2]
// Exports: default

// Module 9627 (CameraPreview)
...
import PictureInPicture from "PictureInPicture" /* 9631 */;
...
function CameraPreview(arg0) {
  ...
  const tmp6 = reveal(stateFromStores(9630)(), 2);
  dependencyMap = tmp7;
  reveal = noop.useContext(participantScreenIsFocused(9617).RevealContext).reveal;
  let tmp8 = stateFromStores(9639)();
  noop = tmp8;
  let tmp9 = closure_9();
  closure_5 = tmp9;
  const tmp10 = stateFromStores(9618)(channel.id);
  closure_6 = tmp10;
  const isScreenLandscape = participantScreenIsFocused(5211).useIsScreenLandscape();
  const rect = stateFromStores(1612)();
  const bottom = rect.bottom;
  const top = rect.top;
  ({ left, right } = rect);
  const obj3 = participantScreenIsFocused(5211);
  let isViewingActivity = participantScreenIsFocused(9636).useIsViewingActivity({ channelId: channel.id });
  const obj4 = participantScreenIsFocused(9636);
  ...
  const OrientationType = participantScreenIsFocused(8462).OrientationType;
  const tmp14 = isScreenLandscape ? OrientationType.LANDSCAPE : OrientationType.PORTRAIT;
  closure_9 = tmp14;
  const tmp15 = stateFromStores(8402)(tmp14);
  let tmp16 = tmp15;
  if (tmp15 == null) {
  ...
  const tmpResult = participantScreenIsFocused(4374);
  ee.__closure = { withTiming: participantScreenIsFocused(4639).withTiming, marginTopState: marginTop, TIMING_CONFIG, marginBottomState: first1 };
  ee.__workletHash = 17411027531876;
  ee.__initData = __initData;
  const animatedStyle = tmpResult.useAnimatedStyle(ee);
  const ref = noop.useRef(null);
  const obj7 = { withTiming: participantScreenIsFocused(4639).withTiming, marginTopState: marginTop, TIMING_CONFIG, marginBottomState: first1 };
  [tmp27, c15] = reveal(noop.useState(null), 2);
  if (constants3.HIDE_PIP === tmp27) {
    const obj8 = { text: null, onClick: null };
    function handleHidePip() {
      const result = participantScreenIsFocused(9727).setPipEnabledWhileFocusedOnActivityOrStream(false);
    }
    ...
    obj9.onClick = tmp(9728).openIgnoreThermalStateAlert;
    const items5 = [obj9];
    items6 = items5;
  } else {
    items6 = [];
  ...
  if (null != tmp27) {
    ...
    tmp32 = closure_21(tmp(6763).GestureDetector, obj10);
  }
  const items7 = [tmp32, ];
  ...
  const tmp4Result = stateFromStores(9631);
  if (isViewingActivity) {
    isViewingActivity = stateFromStores;
  }
  obj16.disabled = !isViewingActivity;
  obj16.trigger = closure_21(stateFromStores(9731), { channel, selfParticipant, pipParticipant: nonSelfPipParticipant });
  obj16.rows = items6;
  ...
  obj15.children = closure_21(stateFromStores(9730), obj16);
  ...
  obj12.children = closure_21(stateFromStores(4374).View, obj13, str);
...
let closure_9 = fn(9628).useBestActiveChatInputContainerHeight;
const useChannelCallStore = fn(9610).useChannelCallStore;
const ChannelCallConstants = fn(9611);
...
export default function CameraPreviewContainer(channel) {
  ...
  const isStreamFocused = channel(9794).useIsStreamFocused(channel.id);
  const tmp4Result = channel(9794);
  const isViewingActivity = channel(9636).useIsViewingActivity({ channelId: channel.id });
  const obj4 = { channelId: channel.id };
  const tmp4Result7 = channel(9636);
  ...
  const tmp19 = flag(9633)(channel.id);
  let tmp20 = null;
  if (null != tmp19) {
    tmp20 = null;
