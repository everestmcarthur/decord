...
// Function ID: 9576
// Name: CameraPreview
// Dependencies: [32, 19, 17, 1956, 4622, 9576, 4628, 502, 9577, 9558, 9559, 1074, 4627, 21, 6726, 4343, 1178, 504, 9578, 9565, 9587, 9566, 5177, 1611, 9584, 8418, 8358, 9579, 5733, 4607, 9675, 1114, 9676, 9678, 9679, 9581, 9561, 9567, 9745, 9746, 2]
// Exports: default

// Module 9575 (CameraPreview)
import ReanimatedRexport from "ReanimatedRexport" /* 4343 */;
import timing from "timing" /* 4607 */;
import NavigatorConstants from "NavigatorConstants" /* 5733 */;
import LegacyBaseButton from "LegacyBaseButton" /* 6726 */;
import PictureInPicture from "PictureInPicture" /* 9579 */;
...
function CameraPreview(arg0) {
  ...
  const tmp6 = reveal(stateFromStores(9578)(), 2);
  dependencyMap = tmp7;
  reveal = noop.useContext(participantScreenIsFocused(9565).RevealContext).reveal;
  let tmp8 = stateFromStores(9587)();
  noop = tmp8;
  let tmp9 = closure_9();
  closure_5 = tmp9;
  const tmp10 = stateFromStores(9566)(channel.id);
  closure_6 = tmp10;
  const isScreenLandscape = participantScreenIsFocused(5177).useIsScreenLandscape();
  const rect = stateFromStores(1611)();
  const bottom = rect.bottom;
  const top = rect.top;
  ({ left, right } = rect);
  const obj3 = participantScreenIsFocused(5177);
  let isViewingActivity = participantScreenIsFocused(9584).useIsViewingActivity({ channelId: channel.id });
  const obj4 = participantScreenIsFocused(9584);
  ...
  const OrientationType = participantScreenIsFocused(8418).OrientationType;
  const tmp14 = isScreenLandscape ? OrientationType.LANDSCAPE : OrientationType.PORTRAIT;
  closure_9 = tmp14;
  const tmp15 = stateFromStores(8358)(tmp14);
  let tmp16 = tmp15;
  if (tmp15 == null) {
  ...
  const tmpResult = participantScreenIsFocused(4343);
  ee.__closure = { withTiming: participantScreenIsFocused(4607).withTiming, marginTopState: marginTop, TIMING_CONFIG, marginBottomState: first1 };
  ee.__workletHash = 17411027531876;
  ee.__initData = __initData;
  const animatedStyle = tmpResult.useAnimatedStyle(ee);
  const ref = noop.useRef(null);
  const obj7 = { withTiming: participantScreenIsFocused(4607).withTiming, marginTopState: marginTop, TIMING_CONFIG, marginBottomState: first1 };
  [tmp27, c15] = reveal(noop.useState(null), 2);
  if (constants3.HIDE_PIP === tmp27) {
    const obj8 = { text: null, onClick: null };
    function handleHidePip() {
      const result = participantScreenIsFocused(9675).setPipEnabledWhileFocusedOnActivityOrStream(false);
    }
    const intl2 = tmp(1114).intl;
    obj8.text = intl2.string(tmp(1114).t.L3I0Jr);
    ...
    obj9.text = intl.string(tmp(1114).t["1fRDnT"]);
    obj9.onClick = tmp(9676).openIgnoreThermalStateAlert;
    const items5 = [obj9];
    items6 = items5;
  } else {
    items6 = [];
  ...
  if (null != tmp27) {
    ...
    tmp32 = closure_21(tmp(6726).GestureDetector, obj10);
  }
  const items7 = [tmp32, ];
  ...
  const tmp4Result = stateFromStores(9579);
  if (isViewingActivity) {
    isViewingActivity = stateFromStores;
  }
  obj16.disabled = !isViewingActivity;
  obj16.trigger = closure_21(stateFromStores(9679), { channel, selfParticipant, pipParticipant: nonSelfPipParticipant });
  obj16.rows = items6;
  ...
  obj15.children = closure_21(stateFromStores(9678), obj16);
  ...
  obj12.children = closure_21(stateFromStores(4343).View, obj13, str);
...
let closure_9 = fn(9576).useBestActiveChatInputContainerHeight;
const useChannelCallStore = fn(9558).useChannelCallStore;
...
export default function CameraPreviewContainer(channel) {
  ...
  const isStreamFocused = channel(9745).useIsStreamFocused(channel.id);
  const tmp4Result = channel(9745);
  const isViewingActivity = channel(9584).useIsViewingActivity({ channelId: channel.id });
  const obj4 = { channelId: channel.id };
  const tmp4Result7 = channel(9584);
  ...
  const tmp19 = flag(9581)(channel.id);
  let tmp20 = null;
  if (null != tmp19) {
    tmp20 = null;
