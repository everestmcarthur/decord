...
// Function ID: 9698
// Name: VideoRenderer
// Dependencies: [32, 19, 17, 21, 4607, 9698, 9699, 9701, 1477, 1115, 8420, 9706, 9707, 9709, 4343, 2]

// Module 9697 (VideoRenderer)
...
export default noop.memo((gestureEnabled) => {
  ...
  let onReady;
  const tmp2 = onReady();
  const surfaceDirectRendererExperiment = resizeMode(9698).useSurfaceDirectRendererExperiment(userId, { location: "VideoRenderer" });
  ...
  closure_8 = tmp15;
  const obj = resizeMode(9698);
  [tmp17, c9] = first1(noop.useState(true), 2);
  width(9699)({ location: "VideoRenderer", videoSpinnerContext, userId, streamId, paused, loading: tmp17 });
  onReady = width(9701)({ streamId, userId, videoSpinnerContext, paused, loading: tmp17, streamKey }).onReady;
  noop.useRef(null);
  ...
  const store = resizeMode(8420).useStore((orientation) => orientation.orientation);
  ...
  if (tmp17) {
    ...
    tmp35 = closure_8(tmp18(9706), obj3);
  }
  const tmp37 = store === resizeMode(8420).OrientationType.PORTRAIT;
  const obj2 = resizeMode(8420);
  if (tmp3Result.isAndroid()) {
    ...
    const items11 = [closure_8(tmp18(9709), obj7), tmp35];
    obj6.children = items11;
    obj5.children = c9(first2, obj6);
    const items12 = [closure_8(tmp18(9707), obj5), ];
    ...
    obj12.children = closure_8(tmp18(9709), obj13);
    ...
    const items17 = [closure_8(tmp18(9709), obj16), tmp35];
