...
// Function ID: 9727
// Name: StreamTile
// Dependencies: [19, 17, 4661, 502, 1074, 4664, 21, 4639, 576, 4489, 4635, 1177, 504, 9727, 9729, 1115, 4791, 9736, 9750, 9739, 9724, 6760, 5207, 9753, 2]
// Exports: default

// Module 9726 (StreamTile)
...
import useVideoStreamErrorDefault from "useVideoStreamError" /* 9727 */;
import VideoRenderer from "VideoRenderer" /* 9736 */;
import StreamQualityLiveIndicatorDefault from "StreamQualityLiveIndicator" /* 9750 */;
...
let closure_15 = noop.memo((participant) => {
  ...
  if (null != stateFromStores) {
    ...
    if (ApplicationStreamStates.FAILED === state) {
      const obj2 = { stream: stateFromStores, removeCloseButton: removeEmptyStateButton, removeSplashImage: removeEmptyStateImage, type: tmp(9729).VideoEmptyTypes.STREAM_FAILED, style: StyleSheet.absoluteFill };
      return closure_10(tmp4(9729), obj2);
    } else if (tmp6.ENDED === state) {
      const obj3 = { stream: stateFromStores, removeCloseButton: removeEmptyStateButton, removeSplashImage: removeEmptyStateImage, type: tmp(9729).VideoEmptyTypes.STREAM_ENDED, style: StyleSheet.absoluteFill };
      return closure_10(tmp4(9729), obj3);
    } else {
      if (tmp6.RECONNECTING === state) {
        ...
        if (tmp6.PAUSED === state) {
          ...
          const obj6 = { username: tmp4(4791).getName(stateFromStores.guildId, stateFromStores.channelId, user) };
          obj5.subtext = intl3.formatToPlainString(tmp(1115).t.meVVlb, obj6);
          tmp9 = closure_10(StreamTextOverlay, obj5);
          const tmp4Result6 = tmp4(4791);
        }
      }
      if (null != tmp5) {
        const obj7 = { stream: stateFromStores, removeCloseButton: removeEmptyStateButton, removeSplashImage: removeEmptyStateImage, type: tmp(9729).VideoEmptyTypes.STREAM_FAILED, style: StyleSheet.absoluteFill, avError: tmp5 };
        return closure_10(tmp4(9729), obj7);
      } else {
        ...
        if (stateFromStores.ownerId === id) {
          let REMOTE_STREAM = tmp(9739).VideoSpinnerContext.SELF_STREAM;
        } else {
          REMOTE_STREAM = tmp(9739).VideoSpinnerContext.REMOTE_STREAM;
        }
        const obj9 = { children: null };
        obj8.videoSpinnerContext = REMOTE_STREAM;
        obj8.userId = user.id;
        obj8.paused = stateFromStores.state === tmp6.PAUSED;
        const items1 = [closure_10(tmp4(9736), obj8), tmp9];
...
let closure_16 = noop.memo((arg0) => {
  ...
  obj.children = closure_1_10(native.Icon, { source: _modDef9753, size: native.Icon.Sizes.SMALL, color: nativeDefault.unsafe_rawColors.WHITE });
