...
// Function ID: 9627
// Name: StreamTile
// Dependencies: [19, 17, 4596, 502, 1074, 4599, 21, 4574, 576, 4423, 4570, 1178, 504, 9627, 9629, 1114, 4726, 9636, 9650, 9639, 9624, 6669, 5137, 9653, 2]
// Exports: default

// Module 9626 (StreamTile)
...
import useVideoStreamErrorDefault from "useVideoStreamError" /* 9627 */;
import VideoRenderer from "VideoRenderer" /* 9636 */;
import StreamQualityLiveIndicatorDefault from "StreamQualityLiveIndicator" /* 9650 */;
...
let closure_15 = noop.memo((participant) => {
  ...
  if (null != stateFromStores) {
    ...
    if (ApplicationStreamStates.FAILED === state) {
      const obj2 = { stream: stateFromStores, removeCloseButton: removeEmptyStateButton, removeSplashImage: removeEmptyStateImage, type: tmp(9629).VideoEmptyTypes.STREAM_FAILED, style: StyleSheet.absoluteFill };
      return closure_10(tmp4(9629), obj2);
    } else if (tmp6.ENDED === state) {
      const obj3 = { stream: stateFromStores, removeCloseButton: removeEmptyStateButton, removeSplashImage: removeEmptyStateImage, type: tmp(9629).VideoEmptyTypes.STREAM_ENDED, style: StyleSheet.absoluteFill };
      return closure_10(tmp4(9629), obj3);
    } else {
      if (tmp6.RECONNECTING === state) {
        ...
        if (tmp6.PAUSED === state) {
          ...
          const obj6 = { username: tmp4(4726).getName(stateFromStores.guildId, stateFromStores.channelId, user) };
          obj5.subtext = intl3.formatToPlainString(tmp(1114).t.meVVlb, obj6);
          tmp9 = closure_10(StreamTextOverlay, obj5);
          const tmp4Result6 = tmp4(4726);
        }
      }
      if (null != tmp5) {
        const obj7 = { stream: stateFromStores, removeCloseButton: removeEmptyStateButton, removeSplashImage: removeEmptyStateImage, type: tmp(9629).VideoEmptyTypes.STREAM_FAILED, style: StyleSheet.absoluteFill, avError: tmp5 };
        return closure_10(tmp4(9629), obj7);
      } else {
        ...
        if (stateFromStores.ownerId === id) {
          let REMOTE_STREAM = tmp(9639).VideoSpinnerContext.SELF_STREAM;
        } else {
          REMOTE_STREAM = tmp(9639).VideoSpinnerContext.REMOTE_STREAM;
        }
        const obj9 = { children: null };
        obj8.videoSpinnerContext = REMOTE_STREAM;
        obj8.userId = user.id;
        obj8.paused = stateFromStores.state === tmp6.PAUSED;
        const items1 = [closure_10(tmp4(9636), obj8), tmp9];
...
let closure_16 = noop.memo((arg0) => {
  ...
  obj.children = closure_1_10(native.Icon, { source: _modDef9653, size: native.Icon.Sizes.SMALL, color: nativeDefault.unsafe_rawColors.WHITE });
