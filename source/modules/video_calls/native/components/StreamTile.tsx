...
// Function ID: 9688
// Name: StreamTile
// Dependencies: [19, 17, 4629, 502, 1074, 4632, 21, 4607, 576, 4456, 4603, 1178, 504, 9688, 9690, 1114, 4759, 9697, 9711, 9700, 9685, 6727, 5175, 9714, 2]
// Exports: default

// Module 9687 (StreamTile)
...
import useVideoStreamErrorDefault from "useVideoStreamError" /* 9688 */;
import VideoRenderer from "VideoRenderer" /* 9697 */;
import StreamQualityLiveIndicatorDefault from "StreamQualityLiveIndicator" /* 9711 */;
...
let closure_15 = noop.memo((participant) => {
  ...
  if (null != stateFromStores) {
    ...
    if (ApplicationStreamStates.FAILED === state) {
      const obj2 = { stream: stateFromStores, removeCloseButton: removeEmptyStateButton, removeSplashImage: removeEmptyStateImage, type: tmp(9690).VideoEmptyTypes.STREAM_FAILED, style: StyleSheet.absoluteFill };
      return closure_10(tmp4(9690), obj2);
    } else if (tmp6.ENDED === state) {
      const obj3 = { stream: stateFromStores, removeCloseButton: removeEmptyStateButton, removeSplashImage: removeEmptyStateImage, type: tmp(9690).VideoEmptyTypes.STREAM_ENDED, style: StyleSheet.absoluteFill };
      return closure_10(tmp4(9690), obj3);
    } else {
      if (tmp6.RECONNECTING === state) {
        ...
        if (tmp6.PAUSED === state) {
          ...
          const obj6 = { username: tmp4(4759).getName(stateFromStores.guildId, stateFromStores.channelId, user) };
          obj5.subtext = intl3.formatToPlainString(tmp(1114).t.meVVlb, obj6);
          tmp9 = closure_10(StreamTextOverlay, obj5);
          const tmp4Result6 = tmp4(4759);
        }
      }
      if (null != tmp5) {
        const obj7 = { stream: stateFromStores, removeCloseButton: removeEmptyStateButton, removeSplashImage: removeEmptyStateImage, type: tmp(9690).VideoEmptyTypes.STREAM_FAILED, style: StyleSheet.absoluteFill, avError: tmp5 };
        return closure_10(tmp4(9690), obj7);
      } else {
        ...
        if (stateFromStores.ownerId === id) {
          let REMOTE_STREAM = tmp(9700).VideoSpinnerContext.SELF_STREAM;
        } else {
          REMOTE_STREAM = tmp(9700).VideoSpinnerContext.REMOTE_STREAM;
        }
        const obj9 = { children: null };
        obj8.videoSpinnerContext = REMOTE_STREAM;
        obj8.userId = user.id;
        obj8.paused = stateFromStores.state === tmp6.PAUSED;
        const items1 = [closure_10(tmp4(9697), obj8), tmp9];
...
let closure_16 = noop.memo((arg0) => {
  ...
  obj.children = closure_1_10(native.Icon, { source: _modDef9714, size: native.Icon.Sizes.SMALL, color: nativeDefault.unsafe_rawColors.WHITE });
