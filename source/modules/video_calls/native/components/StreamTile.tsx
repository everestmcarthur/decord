...
// Function ID: 9686
// Name: StreamTile
// Dependencies: [19, 17, 4628, 502, 1074, 4631, 21, 4606, 576, 4456, 4602, 1178, 504, 9686, 9688, 1114, 4758, 9695, 9709, 9698, 9683, 6726, 5174, 9712, 2]
// Exports: default
...
let closure_15 = noop.memo((participant) => {
  ...
  if (null != stateFromStores) {
    ...
    if (ApplicationStreamStates.FAILED === state) {
      const obj2 = { stream: stateFromStores, removeCloseButton: removeEmptyStateButton, removeSplashImage: removeEmptyStateImage, type: tmp(9688).VideoEmptyTypes.STREAM_FAILED, style: StyleSheet.absoluteFill };
      return closure_10(tmp4(9688), obj2);
    } else if (tmp6.ENDED === state) {
      const obj3 = { stream: stateFromStores, removeCloseButton: removeEmptyStateButton, removeSplashImage: removeEmptyStateImage, type: tmp(9688).VideoEmptyTypes.STREAM_ENDED, style: StyleSheet.absoluteFill };
      return closure_10(tmp4(9688), obj3);
    } else {
      if (tmp6.RECONNECTING === state) {
        ...
        if (tmp6.PAUSED === state) {
          ...
          const obj6 = { username: tmp4(4758).getName(stateFromStores.guildId, stateFromStores.channelId, user) };
          obj5.subtext = intl3.formatToPlainString(tmp(1114).t.meVVlb, obj6);
          tmp9 = closure_10(StreamTextOverlay, obj5);
          const tmp4Result6 = tmp4(4758);
        }
      }
      if (null != tmp5) {
        const obj7 = { stream: stateFromStores, removeCloseButton: removeEmptyStateButton, removeSplashImage: removeEmptyStateImage, type: tmp(9688).VideoEmptyTypes.STREAM_FAILED, style: StyleSheet.absoluteFill, avError: tmp5 };
        return closure_10(tmp4(9688), obj7);
      } else {
        ...
        const items1 = [closure_10(tmp4(9695), obj8), tmp9];
...
let closure_16 = noop.memo((arg0) => {
  ...
  obj.children = closure_1_10(native.Icon, { source: _modDef9712, size: native.Icon.Sizes.SMALL, color: nativeDefault.unsafe_rawColors.WHITE });
