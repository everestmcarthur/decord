...
// Dependencies: [1372, 1374, 4072, 13822, 13825, 13826, 4297, 2]
...
// Module 13821 (ProgramRewardsUtils)
import _modDef4072 from "module_4072" /* 4072 */;
import PremiumUtils from "PremiumUtils" /* 4297 */;
import ProgramRewardsTypes from "ProgramRewardsTypes" /* 13822 */;
import PremiumRewardsOrbsExperiment from "PremiumRewardsOrbsExperiment" /* 13825 */;
...
function canFetchNitroProgramReward(ProgramRewardsUtils) {
  ...
  if (ProgramRewardsTypes.RewardProgram.NITRO === NITRO) {
    let flag = tmp(13825).getPremiumRewardsOrbsExperiment(str).isInTreatment;
    const tmpResult = tmp(13825);
  } else {
    flag = false;
    if (tmp(13822).RewardProgram.XBOX === NITRO) {
      flag = true;
    }
  }
  if (flag) {
    const currentUser = UserStore.getCurrentUser();
    flag = tmp(4297).isPremiumExactly(currentUser, PremiumTypes.TIER_2);
    const tmpResult2 = tmp(4297);
  }
  return flag;
}
function canFetchXboxProgramReward(ProgramRewardsUtils) {
  ...
  if (ProgramRewardsTypes.RewardProgram.NITRO === XBOX) {
    ...
    flag = false;
    if (tmp(13822).RewardProgram.XBOX === XBOX) {
      flag = true;
    }
  }
  if (flag) {
    flag = tmp(13826).hasCrepeMonthlyOrbsPerk(UserStore.getCurrentUser());
    const tmpResult2 = tmp(13826);
  }
  return flag;
}
const PremiumTypes = fn(1374).PremiumTypes;
const dependencyMap = { [fn(13822).RewardProgram.NITRO]: canFetchNitroProgramReward, [fn(13822).RewardProgram.XBOX]: canFetchXboxProgramReward };
const size = fn(2);
...
export const isEligibleForProgramReward = function isEligibleForProgramReward(arg0, ProgramRewardsUtils) {
  ...
  if (ProgramRewardsTypes.RewardProgram.NITRO === arg0) {
    return tmp(13825).getPremiumRewardsOrbsExperiment(str).isInTreatment;
  } else if (tmp(13822).RewardProgram.XBOX === arg0) {
    return true;
  } else {
    return false;
  }
