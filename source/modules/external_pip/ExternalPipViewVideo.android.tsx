...
// Dependencies: [32, 19, 17, 1957, 1371, 4627, 21, 4606, 576, 9689, 4561, 4602, 1114, 504, 1178, 9696, 4308, 17086, 9707, 17087, 9701, 2]

// Module 17085 (ExternalPipViewVideo)
import initialize from "initialize" /* 504 */;
import nativeDefault from "native" /* 576 */;
import ExternalPipDefault from "ExternalPip" /* 9701 */;
import VideoActionCreators from "VideoActionCreators" /* 17086 */;
import useExternalPipParticipantDefault from "useExternalPipParticipant" /* 17087 */;
...
function ExternalPipViewVideoUnavailable(wasStream) {
  ...
  if (wasStream) {
    ...
    let tmp4Result = tmp4(tmp5(9689).StreamEnded, obj2);
    let tmp8 = tmp4;
    let tmp10 = tmp5;
  } else {
    tmp4Result = tmp4(tmp5(4561).CircleInformationIcon, {});
    tmp8 = tmp4;
    tmp10 = tmp5;
  }
  ...
  items[1] = tmp8(tmp10(4602).Text, obj3);
  obj.children = items;
...
function ExternalPipViewVideoStream(streamId) {
  ...
  noop = undefined;
  const surfaceDirectRendererExperiment = streamId(9696).useSurfaceDirectRendererExperiment(streamId.userId, { location: "ExternalPipViewVideoStream" });
  ...
  let obj = streamId(9696);
  const token = streamId(4308).useToken(first(576).colors.TEXT_FEEDBACK_INFO);
  value = closure_7.get();
  ...
  const children = [closure_11(first(9707), { useSurfaceDirectRenderer: surfaceDirectRendererExperiment, style: memo, streamId, onReady: callback, onLayout: callback1 }), ];
