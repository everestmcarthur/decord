...
// Dependencies: [32, 19, 17, 1958, 21, 4638, 576, 7272, 504, 5716, 12913, 7091, 12915, 4649, 4605, 11394, 1897, 1115, 5208, 5766, 4634, 3527, 7195, 7382, 5773, 5691, 7310, 5060, 2]
// Exports: default

// Module 12914 (CreateGameInvitePostModal)
...
function CreateGameInvitePostContent(parentChannel) {
  ...
  const insets = tags(7091)({ includeKeyboardHeight: true }).insets;
  ...
  const createGameInvitePost = parentChannel(12915).useCreateGameInvitePost({
    parentChannel,
    description: tmp5,
    appliedTagIds: memo,
    upload: NOOP_UPLOAD,
  ...
  obj5.children = closure_7(parentChannel(5766).XSmallIcon, {});
  const items4 = [closure_7(parentChannel(5208).PressableOpacity, obj5), ];
  const obj6 = { variant: "text-md/semibold", color: "mobile-text-heading-primary", children: null };
  const intl2 = parentChannel(1115).intl;
  obj6.children = intl2.string(tags(3527).tOsHsu);
  items4[1] = closure_7(parentChannel(4634).Text, obj6);
  ...
  obj8.label = intl3.string(tags(3527)["/mEbGf"]);
  const intl4 = parentChannel(1115).intl;
  obj8.placeholder = intl4.string(tags(3527)["SU/IAE"]);
  obj8.value = tmp5;
  obj8.onChange = tmp6;
  obj8.maxLength = parentChannel(7382).GAME_INVITE_POST_MESSAGE_MAX_LENGTH;
  const items6 = [closure_7(parentChannel(7195).TextArea, obj8), ];
  ...
  if (tmp15Result) {
    ...
    obj10.children = tmp15(tmp9(4634).Text, obj11);
    obj9.trailing = tmp15(tmp14, obj10);
    obj9.onPress = callback1;
    tmp15Result = tmp15(tmp9(5691).TableRow, obj9);
  }
  ...
  obj13.label = intl6.string(tags(3527).Xd2NFi);
  const intl7 = tmp9(1115).intl;
  obj13.subLabel = intl7.string(tags(3527).G91SYQ);
  obj13.value = voiceChatEnabled;
  obj13.onValueChange = callback;
  obj13.disabled = voiceToggleDisabled;
  items7[1] = closure_7(parentChannel(7310).TableSwitchRow, obj13);
  obj12.children = items7;
  items6[1] = closure_8(parentChannel(5773).TableRowGroup, obj12);
  ...
  obj14.children = closure_7(parentChannel(5060).Button, obj15);
...
export default function CreateGameInvitePostModal(parentChannelId) {
  ...
  parentChannelId(5716).useNavigatorBackPressHandler(() => {
  ...
  if (null != stateFromStores) {
    ...
    if (stateFromStores.isGameInvitesChannel()) {
      ...
      tmp4 = closure_7(tmp2(7272).AnalyticsLocationProvider, obj2);
    }
  }
  return tmp4;
};
