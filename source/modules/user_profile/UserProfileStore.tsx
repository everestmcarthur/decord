...
// Dependencies: [2025, 1385, 502, 1979, 1073, 4647, 5490, 1074, 7695, 12, 7696, 7706, 7703, 7702, 1369, 4494, 1889, 1882, 7707, 1114, 7708, 5335, 2]

// Module 7694 (UserProfileStore)
import _modDef12 from "module_12" /* 12 */;
import GlobalUtils from "GlobalUtils" /* 1369 */;
import CollectiblesItemType from "CollectiblesItemType" /* 1889 */;
import Timers from "Timers" /* 4494 */;
import WidgetType from "WidgetType" /* 7695 */;
import TieredTenureBadgeUtils from "TieredTenureBadgeUtils" /* 7707 */;
import parseUserProfileCollectiblesDefault from "parseUserProfileCollectibles" /* 7708 */;
...
function createUserWidgetFromServer(data) {
  ...
  if (WidgetType.WidgetType.CURRENT_GAMES !== type) {
    if (tmp(7695).WidgetType.FAVORITE_GAMES !== type) {
      if (tmp(7695).WidgetType.PLAYED_GAMES !== type) {
        if (tmp(7695).WidgetType.WANT_TO_PLAY_GAMES !== type) {
          if (tmp(7695).WidgetType.APPLICATION === type) {
            const obj2 = { id: data.id, applicationId: data.data.application_id };
            const applicationWidget = new tmp(7706).ApplicationWidget(obj2);
            return applicationWidget;
          } else if (tmp(7695).WidgetType.PERSONAL === type) {
            ...
            obj3.header = str;
            obj3.sections = tmp(7703).parsePersonalWidgetSections(data.data.sections);
            const userProfilePersonalWidget = new tmp(7703).UserProfilePersonalWidget(obj3);
            return userProfilePersonalWidget;
          } else if (tmp(7695).WidgetType.CLIPS_GALLERY === type) {
            ...
            const clipsGalleryWidget = new tmp(7702).ClipsGalleryWidget(obj);
  ...
  const baseGameWidget = new tmp(7696).BaseGameWidget({ id: data.id, type, games: _modDef12.uniqBy(mapped1, "gameId") });
...
function createUserWidgetFromSnapshot(type) {
  ...
  const baseGameWidget = new tmp(7696).BaseGameWidget({ id: id4, type: type3, games });
  return baseGameWidget;
}
