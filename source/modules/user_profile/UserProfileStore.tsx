...
// Dependencies: [2025, 1385, 502, 1979, 1073, 4646, 5489, 1074, 7694, 12, 7695, 7705, 7702, 7701, 1369, 1952, 1889, 1882, 7706, 1114, 7707, 5334, 2]

// Module 7693 (UserProfileStore)
...
import parseUserProfileCollectiblesDefault from "parseUserProfileCollectibles" /* 7707 */;
...
function createUserWidgetFromServer(data) {
  ...
  if (WidgetType.WidgetType.CURRENT_GAMES !== type) {
    if (tmp(7694).WidgetType.FAVORITE_GAMES !== type) {
      if (tmp(7694).WidgetType.PLAYED_GAMES !== type) {
        if (tmp(7694).WidgetType.WANT_TO_PLAY_GAMES !== type) {
          if (tmp(7694).WidgetType.APPLICATION === type) {
            const obj2 = { id: data.id, applicationId: data.data.application_id };
            const applicationWidget = new tmp(7705).ApplicationWidget(obj2);
            return applicationWidget;
          } else if (tmp(7694).WidgetType.PERSONAL === type) {
            ...
            obj3.header = str;
            obj3.sections = tmp(7702).parsePersonalWidgetSections(data.data.sections);
            const userProfilePersonalWidget = new tmp(7702).UserProfilePersonalWidget(obj3);
            return userProfilePersonalWidget;
          } else if (tmp(7694).WidgetType.CLIPS_GALLERY === type) {
            ...
            const clipsGalleryWidget = new tmp(7701).ClipsGalleryWidget(obj);
  ...
  const baseGameWidget = new tmp(7695).BaseGameWidget({ id: data.id, type, games: _modDef12.uniqBy(mapped1, "gameId") });
...
function createUserWidgetFromSnapshot(type) {
  ...
  const baseGameWidget = new tmp(7695).BaseGameWidget({ id: id4, type: type3, games });
  return baseGameWidget;
}
...
function handleProfileFetchFailure(arg0) {
  ...
  if (value4 == null) {
    obj = { connectedAccounts: [], applicationRoleConnections: [], premiumSince: null, premiumGuildSince: null, application: null, legacyUsername: null, userId, banner: null, accentColor: null, bio: "", pronouns: "", premiumType: null, fetchStartedAt: 0, fetchEndedAt: 0, fetchError: "accessible" };
    value4 = obj;
  }
  const timestamp = Date.now();
