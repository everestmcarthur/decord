...
// Dependencies: [2026, 1386, 502, 1980, 1073, 4678, 5524, 1074, 7730, 12, 7731, 7741, 7738, 7737, 1370, 1953, 1890, 1883, 7742, 1115, 7743, 5369, 2]
...
function createUserWidgetFromServer(data) {
  ...
  if (WidgetType.WidgetType.CURRENT_GAMES !== type) {
    if (tmp(7730).WidgetType.FAVORITE_GAMES !== type) {
      if (tmp(7730).WidgetType.PLAYED_GAMES !== type) {
        if (tmp(7730).WidgetType.WANT_TO_PLAY_GAMES !== type) {
          if (tmp(7730).WidgetType.APPLICATION === type) {
            const obj2 = { id: data.id, applicationId: data.data.application_id };
            const applicationWidget = new tmp(7741).ApplicationWidget(obj2);
            return applicationWidget;
          } else if (tmp(7730).WidgetType.PERSONAL === type) {
            ...
            obj3.header = str;
            obj3.sections = tmp(7738).parsePersonalWidgetSections(data.data.sections);
            const userProfilePersonalWidget = new tmp(7738).UserProfilePersonalWidget(obj3);
            return userProfilePersonalWidget;
          } else if (tmp(7730).WidgetType.CLIPS_GALLERY === type) {
            ...
            const clipsGalleryWidget = new tmp(7737).ClipsGalleryWidget(obj);
  ...
  const baseGameWidget = new tmp(7731).BaseGameWidget({ id: data.id, type, games: _modDef12.uniqBy(mapped1, "gameId") });
...
function createUserWidgetFromSnapshot(type) {
  ...
  const baseGameWidget = new tmp(7731).BaseGameWidget({ id: id4, type: type3, games });
  return baseGameWidget;
}
