...
// Function ID: 7676
// Name: WidgetUtils
// Dependencies: [32, 1371, 7672, 7676, 7677, 1954, 7678, 5160, 1114, 7674, 7673, 4425, 1943, 7679, 7680, 7681, 7683, 7361, 5162, 2]
...
// Module 7675 (WidgetUtils)
import GameWidgetLimits from "GameWidgetLimits" /* 5160 */;
import utils from "utils" /* 5162 */;
import WidgetType from "WidgetType" /* 7673 */;
import UserProfileGameWidgetTypes from "UserProfileGameWidgetTypes" /* 7674 */;
import WidgetActionCreatorsDefault from "WidgetActionCreators" /* 7679 */;
import UserProfileClipsGalleryWidgetTypes from "UserProfileClipsGalleryWidgetTypes" /* 7680 */;
import UserProfilePersonalWidget from "UserProfilePersonalWidget" /* 7681 */;
import WidgetGameTag from "WidgetGameTag" /* 7683 */;
import _slicedToArray from "module_32" /* 32 */;
import UserStore from "UserStore" /* 1371 */;
import UserProfileStore from "UserProfileStore" /* 7672 */;
import WidgetStore from "WidgetStore" /* 7676 */;
...
const UserProfileWidgetConstants = fn(7677);
...
let closure_11 = fn(7678).USER_WIDGET_CLIPS_GALLERY_MAX_LENGTH;
const size = fn(2);
  ...
  if (showEditingControls.showEditingControls) {
    if (games.games.length > 0) {
      if (1 === GameWidgetLimits.GAME_WIDGET_LIMITS_BY_TYPE[games.type]) {
        ...
        const obj = { numGames: tmp(5160).GAME_WIDGET_LIMITS_BY_TYPE[games.type] };
...
export const addWidgetToPending = function addWidgetToPending(type) {
  ...
  if (null == widgets.find((getUniqueKey) => {
    ...
    if (type.type === WidgetType.WidgetType.PERSONAL) {
      ...
      const result = tmp16(4425).UNSAFE_markDismissibleContentAsDismissed(tmp16(1943).DismissibleContent.USER_PROFILE_PERSONAL_WIDGET_COACHMARK, obj2);
      const tmp16Result = tmp16(4425);
      const obj3 = { dismissAction: ContentDismissActionType.INDIRECT_ACTION };
      const result1 = tmp16(4425).UNSAFE_markDismissibleContentAsDismissed(tmp16(1943).DismissibleContent.USER_PROFILE_PERSONAL_WIDGET_NEW_BADGE, obj3);
      const tmp16Result2 = tmp16(4425);
    }
    const items = [type];
    ...
    if (tmp9) {
      ...
      if (found2.length <= GameWidgetLimits.USER_WIDGET_GAME_TAGS_MAX_LENGTH) {
        ...
        if (null != found3) {
          ...
          const clipsGalleryWidget = new tmp10(7680).ClipsGalleryWidget(obj4);
          WidgetActionCreatorsDefault.setPendingWidgets(replaceWidgetInList(clipsGalleryWidget));
        }
      }
      tmp10 = require;
          ...
          if (found1.length <= Object.values(WidgetGameTag.WidgetGameTag).length) {
            ...
            if (null != tmpResult) {
              ...
              if (null != found2) {
                ...
                obj2.games = mapped;
                const baseGameWidget = new tmp4(7674).BaseGameWidget(obj2);
...
export const addPendingGameToWidget = function addPendingGameToWidget(ignoreMaxGames) {
  ...
  if (widgetType in GameWidgetLimits.GAME_WIDGET_LIMITS_BY_TYPE) {
    const num = tmp2(5160).GAME_WIDGET_LIMITS_BY_TYPE[widgetType];
  }
  if (null != tmp) {
    const games = tmp.games;
    let num2;
