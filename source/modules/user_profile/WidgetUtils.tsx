...
// Function ID: 7639
// Name: WidgetUtils
// Dependencies: [32, 1371, 7635, 7639, 7640, 1954, 7641, 5124, 1114, 7637, 7636, 4394, 1943, 7642, 7643, 7644, 7646, 7323, 5126, 2]
...
import WidgetActionCreatorsDefault from "WidgetActionCreators" /* 7642 */;
...
import WidgetGameTag from "WidgetGameTag" /* 7646 */;
...
const UserProfileWidgetConstants = fn(7640);
...
let closure_11 = fn(7641).USER_WIDGET_CLIPS_GALLERY_MAX_LENGTH;
const size = fn(2);
  ...
  if (showEditingControls.showEditingControls) {
    if (games.games.length > 0) {
      if (1 === GameWidgetLimits.GAME_WIDGET_LIMITS_BY_TYPE[games.type]) {
        ...
        const obj = { numGames: tmp(5124).GAME_WIDGET_LIMITS_BY_TYPE[games.type] };
...
export const addWidgetToPending = function addWidgetToPending(type) {
  ...
  if (null == widgets.find((getUniqueKey) => {
    ...
    if (type.type === WidgetType.WidgetType.PERSONAL) {
      ...
      const result = tmp16(4394).UNSAFE_markDismissibleContentAsDismissed(tmp16(1943).DismissibleContent.USER_PROFILE_PERSONAL_WIDGET_COACHMARK, obj2);
      const tmp16Result = tmp16(4394);
      const obj3 = { dismissAction: ContentDismissActionType.INDIRECT_ACTION };
      const result1 = tmp16(4394).UNSAFE_markDismissibleContentAsDismissed(tmp16(1943).DismissibleContent.USER_PROFILE_PERSONAL_WIDGET_NEW_BADGE, obj3);
      const tmp16Result2 = tmp16(4394);
    }
    const items = [type];
    ...
    if (tmp9) {
      ...
      if (found2.length <= GameWidgetLimits.USER_WIDGET_GAME_TAGS_MAX_LENGTH) {
        ...
        if (null != found3) {
          ...
          const clipsGalleryWidget = new tmp10(7643).ClipsGalleryWidget(obj4);
          WidgetActionCreatorsDefault.setPendingWidgets(replaceWidgetInList(clipsGalleryWidget));
        }
      }
      tmp10 = require;
          ...
          if (found1.length <= Object.values(WidgetGameTag.WidgetGameTag).length) {
            ...
            if (null != tmpResult) {
              ...
              if (null != found2) {
                ...
                obj2.games = mapped;
                const baseGameWidget = new tmp4(7637).BaseGameWidget(obj2);
...
export const addPendingGameToWidget = function addPendingGameToWidget(ignoreMaxGames) {
  ...
  if (widgetType in GameWidgetLimits.GAME_WIDGET_LIMITS_BY_TYPE) {
    const num = tmp2(5124).GAME_WIDGET_LIMITS_BY_TYPE[widgetType];
  }
  if (null != tmp) {
    const games = tmp.games;
    let num2;
