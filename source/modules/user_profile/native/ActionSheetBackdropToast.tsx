...
// Dependencies: [19, 17, 7224, 21, 1115, 4606, 576, 1611, 1477, 5733, 4343, 4607, 4602, 2]
// Exports: ActionSheetBackdropToast

// Module 13231 (ActionSheetBackdropToast)
...
const ACTION_SHEET_START_HEIGHT_RATIO = fn(7224).ACTION_SHEET_START_HEIGHT_RATIO;
const jsx = fn(21).jsx;
...
export const ActionSheetBackdropToast = function ActionSheetBackdropToast(children) {
  ...
  const diff = height - isExpanded(5733).NAV_BAR_HEIGHT_MULTILINE - useSafeAreaInsetsDefault().top;
  dependencyMap = diff;
  ...
  const effect = sharedValue.useEffect(() => {
    ...
    return () => {
      ...
      const result = sharedValue.set(obj.withDelay(200, isExpanded(4607).withTiming(0)));
      const obj2 = isExpanded(4607);
      const obj4 = { duration: 200, easing: null };
      const Easing = isExpanded(4343).Easing;
      obj4.easing = Easing.out(isExpanded(4343).Easing.exp);
      const result1 = sharedValue1.set(isExpanded(4607).withTiming(0, obj4));
    };
  }, items);
  ...
  const obj6 = { style: null, children: jsx(isExpanded(4602).Text, { variant: "text-sm/medium", color: "mobile-text-heading-primary", children: children.text }) };
  const items2 = [tmp.toast, animatedStyle];
  obj6.style = items2;
  obj5.children = jsx(ReanimatedRexportDefault.View, { style: null, children: jsx(isExpanded(4602).Text, { variant: "text-sm/medium", color: "mobile-text-heading-primary", children: children.text }) });
