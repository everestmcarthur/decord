...
// Dependencies: [19, 17, 7265, 21, 1364, 4640, 576, 1612, 1478, 5770, 4376, 4641, 4636, 2]
// Exports: ActionSheetBackdropToast

// Module 13289 (ActionSheetBackdropToast)
...
const ACTION_SHEET_START_HEIGHT_RATIO = fn(7265).ACTION_SHEET_START_HEIGHT_RATIO;
const jsx = fn(21).jsx;
...
export const ActionSheetBackdropToast = function ActionSheetBackdropToast(children) {
  ...
  const diff = height - isExpanded(5770).NAV_BAR_HEIGHT_MULTILINE - useSafeAreaInsetsDefault().top;
  dependencyMap = diff;
  const sharedValue = isExpanded(4376).useSharedValue(0);
  let obj = isExpanded(4376);
  const sharedValue1 = isExpanded(4376).useSharedValue(0);
  ...
  const effect = sharedValue.useEffect(() => {
    ...
    return () => {
      const obj = isExpanded(4376);
      const result = sharedValue.set(obj.withDelay(200, isExpanded(4641).withTiming(0)));
      const obj2 = isExpanded(4641);
      const obj4 = { duration: 200, easing: null };
      const Easing = isExpanded(4376).Easing;
      obj4.easing = Easing.out(isExpanded(4376).Easing.exp);
      const result1 = sharedValue1.set(isExpanded(4641).withTiming(0, obj4));
    };
  }, items);
  let obj2 = isExpanded(4376);
  const fn = function x() {
    if (closure_7) {
      if (tmp) {
        let sum = c2 + 24;
  ...
  obj5.style = items1;
  const animatedStyle = isExpanded(4376).useAnimatedStyle(fn);
  const obj6 = { style: null, children: jsx(isExpanded(4636).Text, { variant: "text-sm/medium", color: "mobile-text-heading-primary", children: children.text }) };
  const items2 = [tmp.toast, animatedStyle];
  obj6.style = items2;
  obj5.children = jsx(ReanimatedRexportDefault.View, { style: null, children: jsx(isExpanded(4636).Text, { variant: "text-sm/medium", color: "mobile-text-heading-primary", children: children.text }) });
