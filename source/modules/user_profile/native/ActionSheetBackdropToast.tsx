...
// Dependencies: [19, 17, 7258, 21, 1364, 4639, 576, 1612, 1478, 5767, 4376, 4640, 4635, 2]
// Exports: ActionSheetBackdropToast

// Module 13270 (ActionSheetBackdropToast)
...
const ACTION_SHEET_START_HEIGHT_RATIO = fn(7258).ACTION_SHEET_START_HEIGHT_RATIO;
const jsx = fn(21).jsx;
...
export const ActionSheetBackdropToast = function ActionSheetBackdropToast(children) {
  ...
  const diff = height - isExpanded(5767).NAV_BAR_HEIGHT_MULTILINE - useSafeAreaInsetsDefault().top;
  dependencyMap = diff;
  ...
  const effect = sharedValue.useEffect(() => {
    ...
    return () => {
      ...
      const result = sharedValue.set(obj.withDelay(200, isExpanded(4640).withTiming(0)));
      const obj2 = isExpanded(4640);
      const obj4 = { duration: 200, easing: null };
      const Easing = isExpanded(4376).Easing;
      obj4.easing = Easing.out(isExpanded(4376).Easing.exp);
      const result1 = sharedValue1.set(isExpanded(4640).withTiming(0, obj4));
    };
  }, items);
  ...
  const obj6 = { style: null, children: jsx(isExpanded(4635).Text, { variant: "text-sm/medium", color: "mobile-text-heading-primary", children: children.text }) };
  const items2 = [tmp.toast, animatedStyle];
  obj6.style = items2;
  obj5.children = jsx(ReanimatedRexportDefault.View, { style: null, children: jsx(isExpanded(4635).Text, { variant: "text-sm/medium", color: "mobile-text-heading-primary", children: children.text }) });
