...
// Dependencies: [19, 17, 8269, 7282, 21, 4607, 8276, 12697, 13089, 4574, 12703, 1896, 8265, 7416, 5175, 13122, 1178, 4603, 12704, 12713, 5636, 12698, 2]
// Exports: default

// Module 13219 (UserProfileMutuals)
...
const DIVIDER_DOT = fn(7282).DIVIDER_DOT;
const jsxProd = fn(21);
...
export default function UserProfileMutuals(user) {
  ...
  const userProfileAnalyticsContext = user(8276).useUserProfileAnalyticsContext();
  ({ context: c2, trackUserProfileAction: c3 } = userProfileAnalyticsContext);
  let obj = user(8276);
  ({ mutualFriends, mutualGuilds } = guildId(12697)(user));
  if (guildId(13089)(user)) {
    ...
    if (tmp7) {
      ...
      const obj3 = {
        ...
        onPress() {
              ...
              ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(12703, dependencyMap.paths), "UserProfileMutualsActionSheet", {
                user,
                ...
                onPressMutualGuild(arg0) {
                  ...
                  user(7416).transitionToGuild(arg0);
                  const obj = user(7416);
                  guildId(4574).hideAllActionSheets();
                }
              }, "stack");
            },
        children: null
      };
      const obj4 = { size: tmp2(1178).AvatarSizes.SIZE_16, totalCount: mapped.length, names: mapped.map((username) => username.username), children: mapped.map((user) => closure_1_6(user(_undefined[16]).Avatar, { user, size: user(_undefined[16]).AvatarSizes.SIZE_16, guildId: "Array" }, user.id)) };
      const items = [closure_6(tmp2(13122).AvatarPile, obj4), ];
      const obj5 = { variant, color, children: tmp5(12704)(mutualFriends.length) };
      items[1] = closure_6(tmp2(4603).Text, obj5);
      obj3.children = items;
      tmp9Result = tmp9(tmp2(5175).PressableOpacity, obj3);
    }
    ...
    if (tmp15) {
      ...
      tmp15 = closure_6(tmp2(4603).Text, obj6);
    }
    items1[1] = tmp15;
    ...
    if (tmp8) {
      ...
      if (!tmp7) {
        const obj8 = {
          size: tmp2(5636).GuildIconSizes.XXSMALL,
        ...
        tmp21 = closure_6(tmp2(12713).GuildIconPile, obj8);
      }
      const items2 = [tmp21, ];
      const obj9 = { variant, color, children: tmp5(12698)(mutualGuilds.length) };
      items2[1] = closure_6(tmp2(4603).Text, obj9);
      obj7.children = items2;
      tmp9Result2 = tmp9(tmp2(5175).PressableOpacity, obj7);
    }
