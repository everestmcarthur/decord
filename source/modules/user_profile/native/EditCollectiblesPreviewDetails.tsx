...
// Dependencies: [19, 17, 2026, 21, 4638, 504, 4295, 7664, 4319, 4634, 1115, 8302, 2]
// Exports: default

// Module 13321 (EditCollectiblesPreviewDetails)
...
function EditCollectiblesPreviewDescription(arg0) {
  ...
  if (!result) {
    result = tmp(7664).isPremiumCollectiblesPurchase(purchase);
    const tmpResult = tmp(7664);
  }
  let result1 = !canUseCollectiblesResult;
  if (!canUseCollectiblesResult) {
    result1 = tmp(7664).isPremiumCollectiblesPurchase(purchase);
    const tmpResult3 = tmp(7664);
  }
  if (null != purchase) {
    if (!result1) {
      let expiresAt1;
      ...
      if (null != expiresAt1) {
        const _Date = Date;
        const date = new Date();
        diffAsUnitsResult = tmp(4319).diffAsUnits(date, purchase.expiresAt);
        const tmpResult4 = tmp(4319);
      }
      ...
      if (tmp17) {
        ...
        tmp17 = hasOwnProperty(tmp(4634).Text, obj4);
      }
      const items1 = [tmp17, , ];
      ...
      items1[1] = hasOwnProperty(tmp(4634).Text, obj6);
      let expiresAt2;
      if (purchase != null) {
      ...
      if (tmp19Result) {
        ...
        tmp19Result = tmp19(tmp(4634).Text, obj8);
      }
  ...
  if (result) {
    if (canUseCollectiblesResult) {
      ...
      let tmp25Result = hasOwnProperty(tmp(4634).Text, obj11);
    } else {
      const tmp4Result = PremiumUtilsDefault;
      const Text = tmp(4634).Text;
    ...
    return hasOwnProperty(tmp(4634).Text, obj13);
  }
}
