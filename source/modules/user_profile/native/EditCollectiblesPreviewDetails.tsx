...
// Dependencies: [19, 17, 2026, 21, 4639, 504, 4297, 7661, 4321, 4635, 1115, 8293, 2]
// Exports: default
...
function EditCollectiblesPreviewDescription(arg0) {
  ...
  if (!result) {
    result = tmp(7661).isPremiumCollectiblesPurchase(purchase);
    const tmpResult = tmp(7661);
  }
  let result1 = !canUseCollectiblesResult;
  if (!canUseCollectiblesResult) {
    result1 = tmp(7661).isPremiumCollectiblesPurchase(purchase);
    const tmpResult3 = tmp(7661);
  }
  if (null != purchase) {
    if (!result1) {
      let expiresAt1;
      ...
      if (tmp17) {
        ...
        tmp17 = hasOwnProperty(tmp(4635).Text, obj4);
      }
      const items1 = [tmp17, , ];
      ...
      items1[1] = hasOwnProperty(tmp(4635).Text, obj6);
      let expiresAt2;
      if (purchase != null) {
      ...
      if (tmp19Result) {
        ...
        tmp19Result = tmp19(tmp(4635).Text, obj8);
      }
  ...
  if (result) {
    if (canUseCollectiblesResult) {
      ...
      let tmp25Result = hasOwnProperty(tmp(4635).Text, obj11);
    } else {
      const tmp4Result = PremiumUtilsDefault;
      const Text = tmp(4635).Text;
    ...
    return hasOwnProperty(tmp(4635).Text, obj13);
  }
}
