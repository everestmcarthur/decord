...
// Dependencies: [19, 17, 2026, 21, 4640, 504, 4297, 7668, 4321, 4636, 1115, 8321, 2]
// Exports: default

// Module 13329 (EditCollectiblesPreviewDetails)
...
function EditCollectiblesPreviewDescription(arg0) {
  ...
  if (!result) {
    result = tmp(7668).isPremiumCollectiblesPurchase(purchase);
    const tmpResult = tmp(7668);
  }
  let result1 = !canUseCollectiblesResult;
  if (!canUseCollectiblesResult) {
    result1 = tmp(7668).isPremiumCollectiblesPurchase(purchase);
  ...
  if (null != purchase) {
    if (!result1) {
      ...
      if (null != expiresAt1) {
        const _Date = Date;
        const date = new Date();
        diffAsUnitsResult = tmp(4321).diffAsUnits(date, purchase.expiresAt);
        const tmpResult4 = tmp(4321);
      }
      ...
      if (tmp17) {
        ...
        tmp17 = hasOwnProperty(tmp(4636).Text, obj4);
      }
      const items1 = [tmp17, , ];
      ...
      items1[1] = hasOwnProperty(tmp(4636).Text, obj6);
      let expiresAt2;
      if (purchase != null) {
      ...
      if (tmp19Result) {
        ...
        tmp19Result = tmp19(tmp(4636).Text, obj8);
      }
  ...
  if (result) {
    if (canUseCollectiblesResult) {
      ...
      let tmp25Result = hasOwnProperty(tmp(4636).Text, obj11);
    } else {
      const tmp4Result = PremiumUtilsDefault;
      const Text = tmp(4636).Text;
    ...
    return hasOwnProperty(tmp(4636).Text, obj13);
  }
}
