...
// Dependencies: [32, 19, 17, 4866, 4660, 1958, 1980, 4276, 4678, 5365, 4657, 7729, 7318, 1074, 21, 4638, 576, 1364, 4634, 8500, 13146, 4348, 11037, 1115, 11890, 5673, 1397, 8474, 8698, 4489, 11032, 7853, 13151, 13161, 7272, 7292, 13167, 8810, 8821, 13168, 504, 5208, 7317, 1177, 13154, 13169, 13171, 13179, 13182, 8387, 4679, 13149, 13150, 13184, 10156, 4693, 7834, 10121, 13185, 5497, 4780, 4605, 10125, 13172, 4791, 13186, 11040, 9910, 4841, 4845, 7452, 9651, 13187, 13189, 13190, 2]
// Exports: default

// Module 13145 (UserProfileActivity)
...
import isEmbeddedActivityDefault from "isEmbeddedActivity" /* 7853 */;
import MaskedLinkUtils from "MaskedLinkUtils" /* 8500 */;
import closeVoicePanelsDefault from "closeVoicePanels" /* 9651 */;
import UserActivitySpotify from "UserActivitySpotify" /* 11890 */;
import UserProfileActivityButtons from "UserProfileActivityButtons" /* 13179 */;
...
function ActivityCard(user) {
  ...
  const tmp3 = activity(13161)(activity);
  const analyticsLocations = activity(7272)(activity(7292).USER_PROFILE_LIVE_ACTIVITY_CARD).analyticsLocations;
  let id;
  const tmp4 = activity(7272);
  if (voiceChannel != null) {
    id = voiceChannel.id;
  }
  const tmp5Result = activity(13167)({ display: "live", voiceChannelId: id, user, activity, analyticsLocations });
  dependencyMap = tmp5Result;
  const application_id = activity.application_id;
  const tmp5 = activity(13167);
  const tmp2Result = activity(8810);
  let PlayOnSpotifyButton = user;
  const tmp2ResultResult = tmp2Result({ location: "User Profile Activity Card", applicationId: application_id, source: user(8821).GameProfileSources.UserProfile, trackEntryPointImpression: true, sourceUserId: user.id });
  ...
  activity(13168)({ userId: user.id, onAction: tmp5Result });
  const obj2 = { location: "User Profile Activity Card", applicationId: application_id, source: user(8821).GameProfileSources.UserProfile, trackEntryPointImpression: true, sourceUserId: user.id };
  ...
  if (activity.type !== constants.CUSTOM_STATUS) {
    ...
    if (activity.type !== tmp14.HANG_STATUS) {
      ...
      if (tmp2(13154)(activity)) {
        ({ start, end } = activity.timestamps);
        const obj12 = { start, end };
        tmp33Result5 = tmp33(tmp2(13169), obj12);
      }
      ...
      if (null != voiceChannel) {
        ...
        if (null != stateFromStores) {
          ...
          tmp33Result6 = tmp33(tmp2(13171), obj13);
        }
      }
      items4[2] = tmp33Result6;
      let tmp33Result7 = null;
      if (user.id !== currentUser.id) {
        if (tmp2(11037)(activity)) {
          PlayOnSpotifyButton = PlayOnSpotifyButton(13179).PlayOnSpotifyButton;
          obj = { activity, onAction: tmp5Result };
          tmp33Result7 = tmp33(PlayOnSpotifyButton, obj);
        } else if (tmp2(7853)(activity)) {
          const obj14 = { user, currentUser, activity, application: stateFromStores1, onAction: tmp5Result };
          tmp33Result7 = tmp33(PlayOnSpotifyButton(13179).JoinActivityButton, obj14);
        } else {
          if (tmp2(11032)(activity)) {
            let supported_platforms = activity.supported_platforms;
            const currentActivityGamePlatform = PlayOnSpotifyButton(13182).getCurrentActivityGamePlatform();
                ...
                if (null != deepLinkUri) {
                  if (null != activity.session_id) {
                    if (null != stateFromStores1) {
                      ...
                      tmp33Result7 = tmp33(PlayOnSpotifyButton(13179).JoinGameActivityButton, obj15);
                    }
                  }
                }
              }
            }
            const PlayOnSpotifyButtonResult1 = PlayOnSpotifyButton(13182);
          }
          if (tmp2(8387)(activity)) {
            const obj16 = { activity, onAction: tmp5Result };
            tmp33Result7 = tmp33(PlayOnSpotifyButton(13179).WatchActivityButton, obj16);
          } else {
            ...
            tmp33Result7 = null;
            if (!tmp2(4679)(activity)) {
              if (!tmp2(13149)(activity)) {
                tmp33Result7 = null;
                if (tmp2(13150)(activity)) {
                  const obj18 = { type: constants3.PLAYSTATION, onAction: tmp5Result };
                  tmp33Result7 = tmp33(PlayOnSpotifyButton(13179).ConnectPlatformButton, obj18);
                }
              }
            }
            const obj19 = { type: constants3.XBOX, onAction: tmp5Result };
            tmp33Result7 = tmp33(PlayOnSpotifyButton(13179).ConnectPlatformButton, obj19);
          }
        }
      }
      items4[3] = tmp33Result7;
      obj9.children = items4;
      obj9 = closure_22(tmp2(7317), obj9);
      obj7.children = obj9;
      obj7 = tmp33(PlayOnSpotifyButton(5208).PressableOpacity, obj7);
      obj6.children = obj7;
      closure_21(PlayOnSpotifyButton(7272).AnalyticsLocationProvider, obj6);
      const tmp2Result2 = tmp2(7317);
    }
  }
  return tmp15;
}
...
function VoiceCallActivityCard(arg0) {
  ...
  const tmp = closure_24();
  const tmp5 = stateFromStores(4791)(channel);
  const tmp4 = stateFromStores(13172)(channel);
  ...
  const tmp9 = stateFromStores(7272);
  ({ newestAnalyticsLocation: c2, analyticsLocations } = stateFromStores(7272)(stateFromStores(7292).USER_PROFILE_VOICE_ACTIVITY_CARD));
  const tmp11 = stateFromStores(13167)({ display: "voice", activity: { type: "VOICE" }, voiceChannelId: channel.id, user, analyticsLocations });
  closure_3 = tmp11;
  stateFromStores(13168)({ userId: user.id, onAction: tmp11 });
  ...
  const tmp9Result = stateFromStores(7272)(stateFromStores(7292).USER_PROFILE_VOICE_ACTIVITY_CARD);
  if (!channel.isDM()) {
    ...
    obj7.guildId = id;
    const items3 = [closure_21(tmp2(13186), obj7), ];
    ...
    if (stateFromStores1) {
      ...
      const obj10 = { channel };
      obj9.accessibilityLabel = tmp2(9910)(obj10);
      ...
      obj12.children = tmp18(tmp2(11040), obj13);
      const items4 = [tmp18(tmp17, obj12), tmp5];
      obj11.children = items4;
      obj9.children = tmp13(tmp6(4634).Text, obj11);
      let tmp13Result = tmp18(tmp6(5208).PressableOpacity, obj9);
    } else {
      ...
      obj15.children = tmp18(tmp2(11040), obj16);
      const items5 = [tmp18(tmp17, obj15), tmp5];
      obj14.children = items5;
      tmp13Result = tmp13(tmp6(4634).Text, obj14);
    }
    ...
    if (null != stateFromStores) {
      ...
      obj17.children = tmp18(tmp6(4634).Text, obj19);
      tmp18Result2 = tmp18(tmp6(5208).PressableOpacity, obj17);
    }
    ...
    items7[1] = closure_21(tmp6(13179).VoiceChannelButtons, obj21);
...
const CARD_PADDING = fn(7318).CARD_PADDING;
