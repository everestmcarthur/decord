...
// Dependencies: [32, 19, 17, 4868, 4662, 1958, 1980, 4278, 4680, 5367, 4659, 7735, 7322, 1074, 21, 4640, 576, 1364, 4636, 8519, 13154, 4350, 11045, 1115, 11899, 5675, 1397, 8493, 8718, 4491, 11040, 7859, 13159, 13169, 7276, 7296, 13175, 8822, 8833, 13176, 504, 5210, 7321, 1177, 13162, 13177, 13179, 13187, 13190, 8406, 4681, 13157, 13158, 13192, 10167, 4695, 7840, 10132, 13193, 5499, 4782, 4607, 10136, 13180, 4793, 13194, 11048, 9921, 4843, 4847, 7456, 9662, 13195, 13197, 13198, 2]
// Exports: default

// Module 13153 (UserProfileActivity)
...
import UserActivitySpotify from "UserActivitySpotify" /* 11899 */;
import UserProfileActivityButtons from "UserProfileActivityButtons" /* 13187 */;
...
function ActivityCard(user) {
  ...
  const tmp3 = activity(13169)(activity);
  const analyticsLocations = activity(7276)(activity(7296).USER_PROFILE_LIVE_ACTIVITY_CARD).analyticsLocations;
  let id;
  const tmp4 = activity(7276);
  if (voiceChannel != null) {
    id = voiceChannel.id;
  }
  const tmp5Result = activity(13175)({ display: "live", voiceChannelId: id, user, activity, analyticsLocations });
  dependencyMap = tmp5Result;
  const application_id = activity.application_id;
  const tmp5 = activity(13175);
  const tmp2Result = activity(8822);
  let PlayOnSpotifyButton = user;
  const tmp2ResultResult = tmp2Result({ location: "User Profile Activity Card", applicationId: application_id, source: user(8833).GameProfileSources.UserProfile, trackEntryPointImpression: true, sourceUserId: user.id });
  ...
  activity(13176)({ userId: user.id, onAction: tmp5Result });
  const obj2 = { location: "User Profile Activity Card", applicationId: application_id, source: user(8833).GameProfileSources.UserProfile, trackEntryPointImpression: true, sourceUserId: user.id };
  ...
  if (activity.type !== constants.CUSTOM_STATUS) {
    ...
    if (activity.type !== tmp14.HANG_STATUS) {
      ...
      if (tmp2(13162)(activity)) {
        ({ start, end } = activity.timestamps);
        const obj12 = { start, end };
        tmp33Result5 = tmp33(tmp2(13177), obj12);
      }
      ...
      if (null != voiceChannel) {
        ...
        if (null != stateFromStores) {
          ...
          tmp33Result6 = tmp33(tmp2(13179), obj13);
        }
      }
      items4[2] = tmp33Result6;
      let tmp33Result7 = null;
      if (user.id !== currentUser.id) {
        if (tmp2(11045)(activity)) {
          PlayOnSpotifyButton = PlayOnSpotifyButton(13187).PlayOnSpotifyButton;
          obj = { activity, onAction: tmp5Result };
          tmp33Result7 = tmp33(PlayOnSpotifyButton, obj);
        } else if (tmp2(7859)(activity)) {
          const obj14 = { user, currentUser, activity, application: stateFromStores1, onAction: tmp5Result };
          tmp33Result7 = tmp33(PlayOnSpotifyButton(13187).JoinActivityButton, obj14);
        } else {
          if (tmp2(11040)(activity)) {
            let supported_platforms = activity.supported_platforms;
            const currentActivityGamePlatform = PlayOnSpotifyButton(13190).getCurrentActivityGamePlatform();
                ...
                if (null != deepLinkUri) {
                  if (null != activity.session_id) {
                    if (null != stateFromStores1) {
                      ...
                      tmp33Result7 = tmp33(PlayOnSpotifyButton(13187).JoinGameActivityButton, obj15);
                    }
                  }
                }
              }
            }
            const PlayOnSpotifyButtonResult1 = PlayOnSpotifyButton(13190);
          }
          if (tmp2(8406)(activity)) {
            const obj16 = { activity, onAction: tmp5Result };
            tmp33Result7 = tmp33(PlayOnSpotifyButton(13187).WatchActivityButton, obj16);
          } else {
            ...
            tmp33Result7 = null;
            if (!tmp2(4681)(activity)) {
              if (!tmp2(13157)(activity)) {
                tmp33Result7 = null;
                if (tmp2(13158)(activity)) {
                  const obj18 = { type: constants3.PLAYSTATION, onAction: tmp5Result };
                  tmp33Result7 = tmp33(PlayOnSpotifyButton(13187).ConnectPlatformButton, obj18);
                }
              }
            }
            const obj19 = { type: constants3.XBOX, onAction: tmp5Result };
            tmp33Result7 = tmp33(PlayOnSpotifyButton(13187).ConnectPlatformButton, obj19);
          }
        }
      }
      items4[3] = tmp33Result7;
      obj9.children = items4;
      obj9 = closure_22(tmp2(7321), obj9);
      obj7.children = obj9;
      obj7 = tmp33(PlayOnSpotifyButton(5210).PressableOpacity, obj7);
      obj6.children = obj7;
      closure_21(PlayOnSpotifyButton(7276).AnalyticsLocationProvider, obj6);
      const tmp2Result2 = tmp2(7321);
    }
  }
  return tmp15;
}
...
function VoiceCallActivityCard(arg0) {
  ...
  const tmp = closure_24();
  const tmp5 = stateFromStores(4793)(channel);
  const tmp4 = stateFromStores(13180)(channel);
  ...
  const tmp9 = stateFromStores(7276);
  ({ newestAnalyticsLocation: c2, analyticsLocations } = stateFromStores(7276)(stateFromStores(7296).USER_PROFILE_VOICE_ACTIVITY_CARD));
  const tmp11 = stateFromStores(13175)({ display: "voice", activity: { type: "VOICE" }, voiceChannelId: channel.id, user, analyticsLocations });
  closure_3 = tmp11;
  stateFromStores(13176)({ userId: user.id, onAction: tmp11 });
  ...
  const tmp9Result = stateFromStores(7276)(stateFromStores(7296).USER_PROFILE_VOICE_ACTIVITY_CARD);
  if (!channel.isDM()) {
    ...
    obj7.guildId = id;
    const items3 = [closure_21(tmp2(13194), obj7), ];
    ...
    if (stateFromStores1) {
      ...
      const obj10 = { channel };
      obj9.accessibilityLabel = tmp2(9921)(obj10);
      ...
      obj12.children = tmp18(tmp2(11048), obj13);
      const items4 = [tmp18(tmp17, obj12), tmp5];
      obj11.children = items4;
      obj9.children = tmp13(tmp6(4636).Text, obj11);
      let tmp13Result = tmp18(tmp6(5210).PressableOpacity, obj9);
    } else {
      ...
      obj15.children = tmp18(tmp2(11048), obj16);
      const items5 = [tmp18(tmp17, obj15), tmp5];
      obj14.children = items5;
      tmp13Result = tmp13(tmp6(4636).Text, obj14);
    }
    ...
    if (null != stateFromStores) {
      ...
      obj17.children = tmp18(tmp6(4636).Text, obj19);
      tmp18Result2 = tmp18(tmp6(5210).PressableOpacity, obj17);
    }
    ...
    items7[1] = closure_21(tmp6(13187).VoiceChannelButtons, obj21);
...
const CARD_PADDING = fn(7322).CARD_PADDING;
