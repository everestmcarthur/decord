...
// Dependencies: [19, 17, 2026, 1386, 4485, 7735, 1074, 1181, 5496, 21, 4640, 576, 5495, 11725, 5371, 1397, 4491, 1177, 11728, 11729, 4341, 11730, 11731, 4608, 7303, 4337, 1115, 8519, 4820, 4334, 5210, 4636, 4350, 504, 7617, 11732, 2]
// Exports: default, useAppplicationRoleConnectionItems, useConnectedAccountItems

// Module 11724 (LegacyUserProfileConnections)
      ...
      callback1 = obj19.useCallback(() => {
        if (null != platformUserUrl) {
          let obj2 = {
            ...
            onConfirm() {
                account(4820).trackWithMetadata(constants.CONNECTED_ACCOUNT_VIEWED, { platform_type: type.type, other_user_id });
                const obj = account(4820);
                const obj2 = { platform_type: type.type, other_user_id };
                userId(4334).openURL(platformUserUrl);
              }
          };
...
export default function LegacyUserProfileConnections(user) {
  ...
  const obj2 = user(504);
  const theme = user(4350).useThemeContext().theme;
  const tmpResult = user(4350);
  ...
  const theme2 = user(4350).useThemeContext().theme;
  const tmpResult6 = user(4350);
  const items5 = [LocaleStore];
  closure_130_3 = user(504).useStateFromStores(items5, () => LocaleStore.locale);
  const tmpResult7 = user(504);
  closure_130_4 = user(7617).usePlatformAllowed({ forUserProfile: true });
  ...
  if (!stateFromStores1) {
    ...
    if (0 !== memo1.length) {
      ...
      if (tmp10) {
        ...
        tmp10 = closure_14(stateFromStores(11732), obj4);
        const tmp13 = stateFromStores(11732);
      }
      ...
      obj6.children = tmp8;
      items6[1] = closure_14(stateFromStores(11732), obj6);
      obj5.children = items6;
      tmp17Result = closure_15(closure_16, obj5);
      const tmp16 = stateFromStores(11732);
    }
  }
  return tmp17Result;
};
