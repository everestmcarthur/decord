...
// Dependencies: [5, 19, 17, 8280, 1074, 21, 4639, 576, 7269, 4297, 4606, 5222, 14691, 7096, 7304, 7256, 1115, 8788, 5772, 5690, 8719, 14692, 504, 8289, 8306, 8367, 1092, 8284, 14693, 14695, 4635, 1177, 14700, 2]
// Exports: default

// Module 14690 (ChangeBannerActionSheet)
...
function ChangeBannerColorRow(user) {
  ...
  const tmp7 = pendingAccentColor(8306)(user.id);
  ...
  const items2 = [closure_9(pendingAccentColor(14695), { style: tmp.bannerColor, color: pendingAccentColor }), , ];
  ...
  items2[2] = closure_9(require("native").Icon, { style: tmp.rowArrow, size: require("native").Icon.Sizes.CUSTOM, source: pendingAccentColor(14700) });
...
export default function ChangeBannerActionSheet(isTryItOut) {
      ...
      try {
        ...
        if (0 === dependencyMap) {
          if (dependencyMap === 1) {
            ...
            let base64;
            let originalMd5;
            tmp5(4606).hideActionSheet();
            const obj4 = tmp5(4606);
            dependencyMap = 1;
            c3 = 1;
            const obj6 = { value: tmp5(5222).openImagePicker(closure_1_8), done: false };
            return obj6;
          }
          ...
          if (null != base64) {
            const obj8 = { assetOrigin: tmp2(7096).AssetOriginTypes.NEW_ASSET, imageUri: base64, description: "", originalAsset: "Array", originalMd5 };
            closure_129_0(tmp2(14691).createPendingImage(obj8));
            const obj = tmp2(14691);
          }
          c3 = 3;
          return { value: "HermesInternal", done: null };
        }
  ...
  if (!flag) {
    flag = tmp2(4297).canUsePremiumProfileCustomization(user);
    const tmp2Result = tmp2(4297);
  }
  ...
  let tmp4Result = flag;
  if (flag) {
    tmp4Result = tmp4(tmp5(8788).NitroWheelIcon, {});
  }
  ...
  if (!flag) {
    ...
    tmp4Result5 = tmp4(tmp5(8788).NitroWheelIcon, obj6);
  }
  ...
  if (!flag) {
    ...
    obj9.children = tmp4(tmp2(14692), obj10);
  ...
  if (showRemoveBanner) {
    ...
    obj12.label = tmp4(tmp5(8719).FormLabel, obj11);
    obj12.onPress = function handleBannerDelete() {
      _require(null);
      ActionSheetActionCreatorsDefault.hideActionSheet();
    };
    showRemoveBanner = tmp4(tmp5(5690).TableRow, obj12);
  }
