...
// Dependencies: [5, 19, 17, 8289, 1074, 21, 4638, 576, 7272, 4295, 4605, 5223, 14707, 7099, 7307, 7259, 1115, 8797, 5773, 5691, 8728, 14708, 504, 8298, 8315, 8376, 1092, 8293, 14709, 14711, 4634, 1177, 14716, 2]
// Exports: default

// Module 14706 (ChangeBannerActionSheet)
...
function ChangeBannerColorRow(user) {
  ...
  const tmp7 = pendingAccentColor(8315)(user.id);
  ...
  const items2 = [closure_9(pendingAccentColor(14711), { style: tmp.bannerColor, color: pendingAccentColor }), , ];
  ...
  items2[2] = closure_9(require("native").Icon, { style: tmp.rowArrow, size: require("native").Icon.Sizes.CUSTOM, source: pendingAccentColor(14716) });
...
export default function ChangeBannerActionSheet(isTryItOut) {
      ...
      try {
        ...
        if (0 === dependencyMap) {
          if (dependencyMap === 1) {
            ...
            let base64;
            let originalMd5;
            tmp5(4605).hideActionSheet();
            const obj4 = tmp5(4605);
            dependencyMap = 1;
            c3 = 1;
            const obj6 = { value: tmp5(5223).openImagePicker(closure_1_8), done: false };
            return obj6;
          }
          ...
          if (null != base64) {
            const obj8 = { assetOrigin: tmp2(7099).AssetOriginTypes.NEW_ASSET, imageUri: base64, description: "", originalAsset: "Array", originalMd5 };
            closure_129_0(tmp2(14707).createPendingImage(obj8));
            const obj = tmp2(14707);
          }
          c3 = 3;
          return { value: "HermesInternal", done: null };
        }
  ...
  if (!flag) {
    flag = tmp2(4295).canUsePremiumProfileCustomization(user);
    const tmp2Result = tmp2(4295);
  }
  ...
  let tmp4Result = flag;
  if (flag) {
    tmp4Result = tmp4(tmp5(8797).NitroWheelIcon, {});
  }
  ...
  if (!flag) {
    ...
    tmp4Result5 = tmp4(tmp5(8797).NitroWheelIcon, obj6);
  }
  ...
  if (!flag) {
    ...
    obj9.children = tmp4(tmp2(14708), obj10);
  ...
  if (showRemoveBanner) {
    ...
    obj12.label = tmp4(tmp5(8728).FormLabel, obj11);
    obj12.onPress = function handleBannerDelete() {
      _require(null);
      ActionSheetActionCreatorsDefault.hideActionSheet();
    };
    showRemoveBanner = tmp4(tmp5(5691).TableRow, obj12);
  }
