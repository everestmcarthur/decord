...
// Dependencies: [32, 19, 17, 7664, 7655, 1074, 21, 4639, 576, 8306, 8290, 7269, 7289, 1241, 8291, 8287, 8284, 7257, 4635, 1115, 8292, 10867, 504, 14726, 8286, 8307, 14727, 13310, 13311, 8293, 11247, 5071, 2]
// Exports: default

// Module 14725 (EditProfileEffectActionSheet)
...
import EditProfileEffectSection from "EditProfileEffectSection" /* 14727 */;
...
function ProfileEffectSectionPreview(arg0) {
  ...
  const tmp2 = purchase(8293)(previewSkuId);
  ...
  const items1 = [closure_9(purchase(11247), { user, guildId, profileEffect: memo, maxWidth: 250 }), ];
  ...
  obj2.colors = items2;
  items1[1] = closure_9(purchase(5071), obj2);
  obj.children = items1;
...
export default function EditProfileEffectActionSheet(isTryItOut) {
  ...
  const tmp4Result = isTryItOut(8306)(str);
  ...
  const tmp4 = isTryItOut(8306);
  let obj = guildId(8290);
  const analyticsLocations = isTryItOut(7269)(tmp2(7289).EDIT_PROFILE_EFFECT_SHEET).analyticsLocations;
  ...
  const callback1 = memo.useCallback((arg0) => {
    ...
    if (isTryItOut) {
      const result = tmp(8287).setTryItOutProfileEffect(purchasedItem);
      const tmpResult = tmp(8287);
    } else {
      const obj2 = { guildId, profileEffect: purchasedItem };
      tmp(8284).setPendingChanges(obj2);
  ...
  items3[1] = closure_9(guildId(4635).Text, obj6);
  ...
  const tmp13 = closure_10;
  const tmp2Result = isTryItOut(7269);
  ...
  obj7.analyticsSource = isTryItOut(7289).EDIT_PROFILE_EFFECT_SHEET;
  items4[1] = closure_9(isTryItOut(8292), obj7);
  obj3.children = items4;
  obj2.children = tmp13(guildId(7257).BottomSheet, obj3);
  return closure_9(guildId(7269).AnalyticsLocationProvider, obj2);
};
