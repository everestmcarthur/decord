...
// Dependencies: [32, 19, 17, 7667, 7658, 1074, 21, 4638, 576, 8315, 8299, 7272, 7292, 1241, 8300, 8296, 8293, 7260, 4634, 1115, 8301, 10875, 504, 14742, 8295, 8316, 14743, 13321, 13322, 8302, 11261, 5072, 2]
// Exports: default

// Module 14741 (EditProfileEffectActionSheet)
...
import EditProfileEffectSection from "EditProfileEffectSection" /* 14743 */;
...
function ProfileEffectSectionPreview(arg0) {
  ...
  const tmp2 = purchase(8302)(previewSkuId);
  ...
  const items1 = [closure_9(purchase(11261), { user, guildId, profileEffect: memo, maxWidth: 250 }), ];
  ...
  obj2.colors = items2;
  items1[1] = closure_9(purchase(5072), obj2);
  obj.children = items1;
...
export default function EditProfileEffectActionSheet(isTryItOut) {
  ...
  const tmp4Result = isTryItOut(8315)(str);
  ...
  const tmp4 = isTryItOut(8315);
  let obj = guildId(8299);
  const analyticsLocations = isTryItOut(7272)(tmp2(7292).EDIT_PROFILE_EFFECT_SHEET).analyticsLocations;
  ...
  const callback1 = memo.useCallback((arg0) => {
    ...
    if (isTryItOut) {
      const result = tmp(8296).setTryItOutProfileEffect(purchasedItem);
      const tmpResult = tmp(8296);
    } else {
      const obj2 = { guildId, profileEffect: purchasedItem };
      tmp(8293).setPendingChanges(obj2);
  ...
  items3[1] = closure_9(guildId(4634).Text, obj6);
  ...
  const tmp13 = closure_10;
  const tmp2Result = isTryItOut(7272);
  ...
  obj7.analyticsSource = isTryItOut(7292).EDIT_PROFILE_EFFECT_SHEET;
  items4[1] = closure_9(isTryItOut(8301), obj7);
  obj3.children = items4;
  obj2.children = tmp13(guildId(7260).BottomSheet, obj3);
  return closure_9(guildId(7272).AnalyticsLocationProvider, obj2);
};
