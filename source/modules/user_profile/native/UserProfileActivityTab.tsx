...
// Dependencies: [19, 17, 1074, 21, 4639, 576, 4635, 1115, 8491, 2025, 13202, 13205, 13134, 13206, 2]
// Exports: default

// Module 13201 (UserProfileActivityTab)
import nativeDefault from "native" /* 576 */;
import Text_Text from "Text/Text" /* 4635 */;
import UserProfileRecentActivityCardDefault from "UserProfileRecentActivityCard" /* 13206 */;
...
export default function UserProfileActivityTab(user) {
  ...
  ({ recent, isCurrentUser, hasCurrentActivity, hasRecentActivity } = cardStyle(13202)({ userId: user.id, currentUserId: currentUser.id, guildId }));
  if (!hasCurrentActivity) {
    if (!hasRecentActivity) {
      if (tmp4) {
        let tmp10Result = tmp5(UserProfileActivityTabSkeleton, {});
      } else {
        const tmp7 = user(13205);
        if (isCurrentUser) {
  ...
  if (hasCurrentActivity) {
    ...
    obj3.children = closure_5(cardStyle(13134), obj4);
