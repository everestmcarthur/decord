...
// Dependencies: [19, 17, 1074, 21, 4638, 576, 4634, 1115, 8500, 2025, 13213, 13216, 13145, 13217, 2]
// Exports: default

// Module 13212 (UserProfileActivityTab)
import nativeDefault from "native" /* 576 */;
import Text_Text from "Text/Text" /* 4634 */;
import UserProfileRecentActivityCardDefault from "UserProfileRecentActivityCard" /* 13217 */;
    ...
    learnMoreHook(children, arg1) {
      return hasOwnProperty(Text_Text.Text, {
        ...
        onPress() {
          ...
          const obj = learnMore(8500);
          obj2.href = closure_1_1(2025).getArticleURL(constants.ACTIVITY_STATUS_SETTINGS);
...
export default function UserProfileActivityTab(user) {
  ...
  ({ recent, isCurrentUser, hasCurrentActivity, hasRecentActivity } = cardStyle(13213)({ userId: user.id, currentUserId: currentUser.id, guildId }));
  if (!hasCurrentActivity) {
    if (!hasRecentActivity) {
      if (tmp4) {
        let tmp10Result = tmp5(UserProfileActivityTabSkeleton, {});
      } else {
        const tmp7 = user(13216);
        if (isCurrentUser) {
  ...
  if (hasCurrentActivity) {
    ...
    obj3.children = closure_5(cardStyle(13145), obj4);
