...
// Dependencies: [19, 17, 1074, 21, 4640, 576, 4636, 1115, 8519, 2025, 13221, 13224, 13153, 13225, 2]
// Exports: default

// Module 13220 (UserProfileActivityTab)
import nativeDefault from "native" /* 576 */;
import Text_Text from "Text/Text" /* 4636 */;
import UserProfileRecentActivityCardDefault from "UserProfileRecentActivityCard" /* 13225 */;
...
export default function UserProfileActivityTab(user) {
  ...
  ({ recent, isCurrentUser, hasCurrentActivity, hasRecentActivity } = cardStyle(13221)({ userId: user.id, currentUserId: currentUser.id, guildId }));
  if (!hasCurrentActivity) {
    if (!hasRecentActivity) {
      if (tmp4) {
        let tmp10Result = tmp5(UserProfileActivityTabSkeleton, {});
      } else {
        const tmp7 = user(13224);
        if (isCurrentUser) {
  ...
  if (hasCurrentActivity) {
    ...
    obj3.children = closure_5(cardStyle(13153), obj4);
