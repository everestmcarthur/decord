...
// Dependencies: [19, 17, 21, 4605, 576, 12672, 11223, 12673, 12674, 12678, 2]
// Exports: default

// Module 12671 (UserProfileMutualGuildsActionSheet)
...
export default function UserProfileMutualGuildsActionSheet(user) {
  ...
  const tmp = closure_6();
  const mutualGuilds = onPressMutualGuild(12672)(user).mutualGuilds;
  let length;
  const tmp4 = onPressMutualGuild(11223);
  if (mutualGuilds != null) {
    length = mutualGuilds.length;
  }
  const obj = { scrollable: true, title: onPressMutualGuild(12673)(length), children: null };
  const obj2 = { style: tmp.container, children: null };
  if (null == mutualGuilds) {
    const obj3 = { style: tmp.loadingState, children: tmp3(closure_4, {}) };
    let tmp3Result = tmp3(tmp7, obj3);
  } else if (0 === mutualGuilds.length) {
    const obj4 = { style: tmp.emptyState, children: tmp3(user(12674).NoMutualServers, {}) };
    ...
    tmp3Result = tmp3(user(11223).UserProfileStackedActionSheetList, obj5);
  }
  obj2.children = tmp3Result;
  obj.children = <closure_3 style={tmp.container}>{null}</closure_3>;
  return <tmp4 scrollable title={onPressMutualGuild(12673)(length)}>{null}</tmp4>;
};
