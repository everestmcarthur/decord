...
// Dependencies: [19, 17, 21, 4636, 576, 12732, 11286, 12733, 12734, 12738, 2]
// Exports: default

// Module 12731 (UserProfileMutualGuildsActionSheet)
...
export default function UserProfileMutualGuildsActionSheet(user) {
  ...
  const tmp = closure_6();
  const mutualGuilds = onPressMutualGuild(12732)(user).mutualGuilds;
  let length;
  const tmp4 = onPressMutualGuild(11286);
  if (mutualGuilds != null) {
    length = mutualGuilds.length;
  }
  const obj = { scrollable: true, title: onPressMutualGuild(12733)(length), children: null };
  const obj2 = { style: tmp.container, children: null };
  if (null == mutualGuilds) {
    const obj3 = { style: tmp.loadingState, children: tmp3(closure_4, {}) };
    let tmp3Result = tmp3(tmp7, obj3);
  } else if (0 === mutualGuilds.length) {
    const obj4 = { style: tmp.emptyState, children: tmp3(user(12734).NoMutualServers, {}) };
    ...
    tmp3Result = tmp3(user(11286).UserProfileStackedActionSheetList, obj5);
  }
  obj2.children = tmp3Result;
  obj.children = <closure_3 style={tmp.container}>{null}</closure_3>;
  return <tmp4 scrollable title={onPressMutualGuild(12733)(length)}>{null}</tmp4>;
};
