...
// Dependencies: [32, 19, 17, 7404, 1955, 1085, 21, 576, 4722, 7584, 1944, 8396, 11296, 1890, 4689, 14847, 1897, 1115, 14834, 5747, 9063, 1177, 13418, 2]
// Exports: default

// Module 14846 (UserProfileFrameEditButton)
...
const COLLECTIBLES_PREVIEW_SIZE = fn(7404).COLLECTIBLES_PREVIEW_SIZE;
...
export default function UserProfileFrameEditButton(arg0) {
  ...
  const items = [user(1944).DismissibleContent.PROFILE_FRAME_USER_PROFILE_NEW_BADGE];
  const tmp4 = userProfileFrame(user(7584).useSelectedDismissibleContent(items), 2);
  dependencyMap = tmp5;
  const obj = user(7584);
  ...
  const profilePreviewValue = user(8396).getProfilePreviewValue(obj3);
  const obj2 = user(8396);
  let skuId;
  if (profilePreviewValue != null) {
    skuId = profilePreviewValue.skuId;
  }
  const fetchCollectiblesProduct = user(11296).useFetchCollectiblesProduct(skuId);
  ...
  if (type === user(1890).CollectiblesItemType.PROFILE_FRAME) {
    const first1 = product.items[0];
  }
  const tmp2Result = user(11296);
  userProfileFrame = user(8396).useUserProfileFrame({ user, guildId });
  ...
  const callback = noop.useCallback(() => {
    ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(14847, dependencyMap.paths), "Profile Frame", { user, currentProfileFrame: userProfileFrame, guildId });
  ...
  if (fetchCollectiblesProduct.isFetching) {
    ...
    obj5.onPress = NOOP;
    obj5.leading = tmp18(tmp2(5747).ActivityIndicator, { animating: true, size: "large" });
    let obj6 = obj5;
  } else {
    ...
    const obj7 = { showNewBadge: tmp4[0] === tmp2(1944).DismissibleContent.PROFILE_FRAME_USER_PROFILE_NEW_BADGE };
    obj6.labelTrailing = tmp18(tmp2(14834).UserProfileEditFormLabelBadges, obj7);
    ...
    if (null != first1) {
      ...
      obj9.children = tmp18(guildId(9063), obj10);
      let tmp18Result = tmp18(View, obj9);
      const tmp23 = guildId(9063);
    } else {
      const obj11 = { source: guildId(13418), style: tmp.noneIcon };
      tmp18Result = tmp18(tmp2(1177).Icon, obj11);
    }
    obj6.leading = tmp18Result;
  }
  return jsx(user(14834).UserProfileEditFormButton, obj6);
};
