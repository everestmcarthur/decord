...
// Dependencies: [32, 19, 17, 7260, 1954, 1085, 21, 576, 4605, 14708, 7440, 1943, 8229, 11114, 1889, 4572, 14709, 1896, 1114, 14695, 5627, 8893, 1178, 13246, 2]
// Exports: default

// Module 14707 (UserProfileFrameEditButton)
...
export default function UserProfileFrameEditButton(arg0) {
  ...
  const canViewProfileFramesInCollectiblesShop = user(14708).useCanViewProfileFramesInCollectiblesShop("UserProfileFrameEditButton");
  const obj = user(14708);
  const items = [user(1943).DismissibleContent.PROFILE_FRAME_USER_PROFILE_NEW_BADGE];
  const tmp5 = userProfileFrame(user(7440).useSelectedDismissibleContent(items), 2);
  dependencyMap = tmp6;
  const obj2 = user(7440);
  ...
  const profilePreviewValue = user(8229).getProfilePreviewValue(obj4);
  const obj3 = user(8229);
  let skuId;
  if (profilePreviewValue != null) {
    skuId = profilePreviewValue.skuId;
  }
  const fetchCollectiblesProduct = user(11114).useFetchCollectiblesProduct(skuId);
  ...
  const tmp2Result = user(11114);
  userProfileFrame = user(8229).useUserProfileFrame({ user, guildId });
  ...
  if (canViewProfileFramesInCollectiblesShop) {
    ...
    if (fetchCollectiblesProduct.isFetching) {
      ...
      obj6.onPress = NOOP;
      obj6.leading = tmp19(tmp2(5627).ActivityIndicator, { animating: true, size: "large" });
      let obj7 = obj6;
    } else {
      ...
      obj7.labelTrailing = tmp19(tmp2(14695).UserProfileEditFormLabelBadges, obj8);
      ...
      if (null != first1) {
        ...
        obj10.children = tmp19(guildId(8893), obj11);
        let tmp19Result = tmp19(View, obj10);
        const tmp24 = guildId(8893);
      } else {
        const obj12 = { source: guildId(13246), style: tmp.noneIcon };
        tmp19Result = tmp19(tmp2(1178).Icon, obj12);
      }
      obj7.leading = tmp19Result;
    }
    return jsx(tmp2(14695).UserProfileEditFormButton, obj7);
  } else {
    return null;
  }
  const tmp2Result2 = user(8229);
};
