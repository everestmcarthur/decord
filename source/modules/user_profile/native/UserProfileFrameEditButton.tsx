...
// Dependencies: [32, 19, 17, 7322, 1955, 1085, 21, 576, 4640, 7502, 1944, 8314, 11203, 1890, 4607, 14753, 1897, 1115, 14740, 5665, 8979, 1177, 13327, 2]
// Exports: default

// Module 14752 (UserProfileFrameEditButton)
...
const COLLECTIBLES_PREVIEW_SIZE = fn(7322).COLLECTIBLES_PREVIEW_SIZE;
...
export default function UserProfileFrameEditButton(arg0) {
  ...
  const tmp4 = userProfileFrame(user(7502).useSelectedDismissibleContent(items), 2);
  dependencyMap = tmp5;
  const obj = user(7502);
  ...
  const profilePreviewValue = user(8314).getProfilePreviewValue(obj3);
  const obj2 = user(8314);
  let skuId;
  if (profilePreviewValue != null) {
    skuId = profilePreviewValue.skuId;
  }
  const fetchCollectiblesProduct = user(11203).useFetchCollectiblesProduct(skuId);
  ...
  const tmp2Result = user(11203);
  userProfileFrame = user(8314).useUserProfileFrame({ user, guildId });
  ...
  const callback = noop.useCallback(() => {
    ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(14753, dependencyMap.paths), "Profile Frame", { user, currentProfileFrame: userProfileFrame, guildId });
  ...
  if (fetchCollectiblesProduct.isFetching) {
    ...
    obj5.onPress = NOOP;
    obj5.leading = tmp18(tmp2(5665).ActivityIndicator, { animating: true, size: "large" });
    let obj6 = obj5;
  } else {
    ...
    obj6.labelTrailing = tmp18(tmp2(14740).UserProfileEditFormLabelBadges, obj7);
    ...
    if (null != first1) {
      ...
      obj9.children = tmp18(guildId(8979), obj10);
      let tmp18Result = tmp18(View, obj9);
      const tmp23 = guildId(8979);
    } else {
      const obj11 = { source: guildId(13327), style: tmp.noneIcon };
      tmp18Result = tmp18(tmp2(1177).Icon, obj11);
    }
    obj6.leading = tmp18Result;
  }
  return jsx(user(14740).UserProfileEditFormButton, obj6);
};
