...
// Dependencies: [19, 17, 1074, 21, 4607, 9122, 7812, 5158, 10974, 10004, 9315, 13101, 13102, 576, 13103, 8221, 5141, 4603, 11748, 2]
// Exports: EpisodeBadge, PartyBadge, TimestampBadge

// Module 13100 (UserProfileActivityBadges)
import nativeDefault from "native" /* 576 */;
import isEmbeddedActivityDefault from "isEmbeddedActivity" /* 7812 */;
import utils from "utils" /* 8221 */;
import shouldShowActivityTimeBarDefault from "shouldShowActivityTimeBar" /* 13102 */;
...
export const TimestampBadge = function TimestampBadge(activity) {
  ...
  if (null != start) {
    if (!shouldShowActivityTimeBarDefault(activity)) {
      ...
      if (flag2) {
        let GameControllerIcon = tmp2(9122).HourglassIcon;
      } else if (tmp10(7812)(activity)) {
        GameControllerIcon = tmp2(5158).AppsIcon;
      } else if (activity.type === ActivityTypes.WATCHING) {
        GameControllerIcon = tmp2(10974).TvIcon;
      } else if (activity.type === tmp6.LISTENING) {
        GameControllerIcon = tmp2(10004).MusicIcon;
      } else {
        GameControllerIcon = tmp2(9315).GameControllerIcon;
      }
      ...
      obj4.entry = obj5;
      obj4.style = tmp.bold;
      items[1] = hasOwnProperty(tmp2(13103).ActiveTimestamp, obj4);
      obj2.children = items;
  ...
  if (!isEmbeddedActivityDefault(activity)) {
    if (null != activity.party) {
      ...
      if (null != richGameStateBadgeText) {
        ...
        const items = [hasOwnProperty(tmp9(5141).GroupIcon, obj2), ];
        const obj3 = { variant: "text-sm/medium", color: "text-muted", children: richGameStateBadgeText };
        items[1] = hasOwnProperty(tmp9(4603).Text, obj3);
        obj.children = items;
...
export const EpisodeBadge = function EpisodeBadge(activity) {
  ...
  if (null != episodeBadgeText) {
    ...
    const items = [hasOwnProperty(tmp2(11748).TopicsIcon, obj3), ];
    const obj4 = { variant: "text-sm/medium", color: "text-muted", children: episodeBadgeText };
    items[1] = hasOwnProperty(tmp2(4603).Text, obj4);
    obj2.children = items;
