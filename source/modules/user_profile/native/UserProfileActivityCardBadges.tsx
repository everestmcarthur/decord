...
// Dependencies: [19, 17, 1074, 21, 13152, 2]
// Exports: default

// Module 13151 (UserProfileActivityCardBadges)
...
export default function UserProfileActivityCardBadges(activity) {
  ...
  if (activity.type === ActivityTypes.PLAYING) {
    const items = [activity(13152).PartyBadge, activity(13152).TimestampBadge];
    let items3 = items;
  } else if (activity.type === tmp.LISTENING) {
    const items1 = [activity(13152).TimestampBadge];
    items3 = items1;
  } else if (activity.type === tmp.WATCHING) {
    const items2 = [activity(13152).TimestampBadge, activity(13152).EpisodeBadge];
    items3 = items2;
  } else {
    items3 = [];
  }
