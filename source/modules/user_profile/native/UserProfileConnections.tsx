...
// Dependencies: [19, 17, 2025, 4452, 7282, 1074, 5460, 21, 1178, 4607, 576, 11670, 4308, 4458, 11675, 11676, 8276, 5459, 5335, 1396, 8458, 4787, 4301, 4575, 7263, 4304, 1114, 4603, 5657, 4307, 8671, 4317, 504, 13182, 7281, 5739, 13185, 2]
// Exports: UserProfileAccountConnectionsCard, UserProfileApplicationRoleConnectionsCard

// Module 13181 (UserProfileConnections)
...
import UserProfileCardDefault from "UserProfileCard" /* 7281 */;
import MaskedLinkUtils from "MaskedLinkUtils" /* 8458 */;
import ConnectionMetadataVanityItems from "ConnectionMetadataVanityItems" /* 11670 */;
import useUserProfileApplicationRoleConnectionsDefault from "useUserProfileApplicationRoleConnections" /* 13185 */;
...
function VerifiedIcon(arg0) {
  ...
  if (!tmp2Result.isThemeLight(theme)) {
    ...
    const obj4 = { source: tmp4(11675), color: PLATFORM_TWITTER };
    const merged = Object.assign(obj);
    const items = [closure_1_12(tmp2(1178).Icon, obj4), ];
    const obj5 = { source: tmp4(11676), color: WHITE };
...
let closure_18 = noop.memo((userId) => {
  ...
  if (!showMetadata) {
    ...
    let tmp29Result6;
    if (null != platformUserUrl) {
      tmp29Result6 = tmp29(tmp2(tmp3[30]).LinkExternalSmallIcon, { size: "sm" });
    }
...
const memoResult = noop.memo((applicationRoleConnection) => {
  ...
  if (null != applicationRoleConnection.platform_name) {
    ...
    if (null != applicationRoleConnection.platform_username) {
      ...
      tmp7 = closure_1_12(tmp2(4603).Text, obj4);
    }
  }
  const items = [tmp7, , ];
  let tmp9 = null;
...
export const UserProfileAccountConnectionsCard = function UserProfileAccountConnectionsCard(userId) {
  userId = userId.userId;
  const tmp = closure_16();
  const theme = userId(4317).useThemeContext().theme;
  const obj = userId(4317);
  ...
  const tmp5 = theme;
  ({ connections, appIdentities } = theme(13182)(userId));
  if (!stateFromStores) {
    const items2 = [];
    ...
    obj4.children = closure_12(tmp2(5739).TableRowGroup, obj5);
    return closure_12(tmp5(7281), obj4);
  }
  return null;
};
export const UserProfileApplicationRoleConnectionsCard = function UserProfileApplicationRoleConnectionsCard(arg0) {
  ...
  if (!obj.useStateFromStores(items, () => StreamerModeStore.hidePersonalInformation)) {
    if (0 !== arr.length) {
      ...
      obj2.children = closure_1_12(tmp4(5739).TableRowGroup, obj3);
