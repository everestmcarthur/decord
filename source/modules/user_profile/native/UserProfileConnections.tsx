...
// Dependencies: [19, 17, 2026, 4483, 7318, 1074, 5494, 21, 1177, 4638, 576, 11717, 4339, 4489, 11722, 11723, 8319, 5493, 5369, 1397, 8500, 4818, 4332, 4606, 7299, 4335, 1115, 4634, 5691, 4338, 8713, 4348, 504, 13234, 7317, 5773, 13237, 2]
// Exports: UserProfileAccountConnectionsCard, UserProfileApplicationRoleConnectionsCard

// Module 13233 (UserProfileConnections)
...
import UserProfileCardDefault from "UserProfileCard" /* 7317 */;
import MaskedLinkUtils from "MaskedLinkUtils" /* 8500 */;
import ConnectionMetadataVanityItems from "ConnectionMetadataVanityItems" /* 11717 */;
import useUserProfileApplicationRoleConnectionsDefault from "useUserProfileApplicationRoleConnections" /* 13237 */;
...
function VerifiedIcon(arg0) {
  ...
  if (!tmp2Result.isThemeLight(theme)) {
    ...
    const obj4 = { source: tmp4(11722), color: PLATFORM_TWITTER };
    const merged = Object.assign(obj);
    const items = [closure_1_12(tmp2(1177).Icon, obj4), ];
    const obj5 = { source: tmp4(11723), color: WHITE };
...
const memoResult = noop.memo((applicationRoleConnection) => {
  ...
  if (null != applicationRoleConnection.platform_name) {
    ...
    if (null != applicationRoleConnection.platform_username) {
      ...
      tmp7 = closure_1_12(tmp2(4634).Text, obj4);
    }
  }
  const items = [tmp7, , ];
  let tmp9 = null;
...
export const UserProfileAccountConnectionsCard = function UserProfileAccountConnectionsCard(userId) {
  userId = userId.userId;
  const tmp = closure_16();
  const theme = userId(4348).useThemeContext().theme;
  const obj = userId(4348);
  ...
  const tmp5 = theme;
  ({ connections, appIdentities } = theme(13234)(userId));
  if (!stateFromStores) {
    const items2 = [];
    ...
    obj4.children = closure_12(tmp2(5773).TableRowGroup, obj5);
    return closure_12(tmp5(7317), obj4);
  }
  return null;
};
export const UserProfileApplicationRoleConnectionsCard = function UserProfileApplicationRoleConnectionsCard(arg0) {
  ...
  if (!obj.useStateFromStores(items, () => StreamerModeStore.hidePersonalInformation)) {
    if (0 !== arr.length) {
      ...
      obj2.children = closure_1_12(tmp4(5773).TableRowGroup, obj3);
