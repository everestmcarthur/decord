...
// Dependencies: [19, 17, 1980, 21, 4638, 576, 504, 17549, 17553, 7307, 11520, 4634, 1115, 5670, 5060, 7105, 1936, 4336, 9709, 4605, 14013, 2]
// Exports: default

// Module 17552 (DmSettingsUpsellActionSheet)
...
import DmSettingsUpsellManager from "DmSettingsUpsellManager" /* 17549 */;
import DmSettingsUpsellUtils from "DmSettingsUpsellUtils" /* 17553 */;
...
export default function DmSettingsUpsellActionSheet(guildId) {
  ...
  if (null != stateFromStores) {
    ...
    const obj4 = { source: stateFromStores(11520), style: tmp.headerImage };
    ...
    items2[1] = closure_7(tmp2(4634).Text, obj5);
    ...
    items2[2] = closure_7(tmp2(4634).Text, obj6);
    ...
    const items3 = [closure_7(tmp2(4634).Text, obj9), ];
    const obj10 = { style: tmp.guildInfo, children: null };
    const obj11 = { guild: stateFromStores, size: tmp2(5670).GuildIconSizes.SMALL_32 };
    const items4 = [closure_7(stateFromStores(5670), obj11), ];
    const obj12 = { variant: "text-md/semibold", color: "mobile-text-heading-primary", children: stateFromStores.name };
    items4[1] = closure_7(tmp2(4634).Text, obj12);
    ...
    const obj13 = {
      ...
      onPress() {
          ...
          RestrictedGuildIds.updateSetting(Array.from(sanitizedRestrictedGuilds)).then(() => {
            const obj2 = { key: "DM_SETTINGS_UPSELL_SUCCESS_TOAST", icon: stateFromStores(9709), content: null };
            const intl = guildId(1115).intl;
            obj2.content = intl.string(guildId(1115).t.rlYD1W);
            stateFromStores(4336).open(obj2);
          });
    ...
    items2[4] = closure_7(tmp2(5060).Button, obj13);
    const obj14 = {
      size: "lg",
      variant: "secondary",
      onPress() {
    ...
    items2[5] = closure_7(tmp2(5060).Button, obj14);
    ...
    items2[6] = closure_7(tmp2(4634).Text, obj15);
    obj3.children = items2;
    obj2.children = closure_8(closure_4, obj3);
    tmp6 = closure_7(tmp2(7307).ActionSheet, obj2);
    const tmp12 = stateFromStores(5670);
  }
  return tmp6;
};
