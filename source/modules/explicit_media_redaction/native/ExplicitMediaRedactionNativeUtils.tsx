...
// Dependencies: [1371, 7615, 7892, 1187, 1114, 7614, 8439, 8441, 4541, 14851, 1896, 7306, 7311, 2]
// Exports: handleSensitiveMediaFilterPress, shouldAgeVerifyForSearchMedia

// Module 14850 (ExplicitMediaRedactionNativeUtils)
...
require = fn;
let closure_4 = fn(7615).EXPLICIT_MEDIA_SETTINGS_ACTION_SHEET_KEY;
...
export const handleSensitiveMediaFilterPress = function handleSensitiveMediaFilterPress(arg0) {
  ...
  if (tmp5) {
    ...
    obj.onPress = function onPress() {
      if (obj.shouldAgeVerifyForExplicitMedia()) {
        const obj3 = { entryPoint: tmp(8441).AgeVerificationModalEntryPoint.OBSCURED_MEDIA };
  ...
  ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(14851, dependencyMap.paths), closure_4, { title, subtitle, options: items, currentValue });
};
  ...
  if (null == found) {
    ...
    if (0 === enabledHarmTypesForMessage) {
      ...
      if (media.type === SearchMediaTypes.ATTACHMENT) {
        const obj2 = { type: tmp4(7311).ObscuredMediaTypes.Attachment, media: media.attachment };
        let tmp = obj2;
      } else if (media.type === tmp8.EMBED) {
        const obj = { type: tmp4(7311).ObscuredMediaTypes.Embed, media: media.embed };
        tmp = obj;
      } else {
        tmp = null;
        if (media.type === tmp8.COMPONENT) {
          const obj3 = { type: tmp4(7311).ObscuredMediaTypes.GenericMedia, media: media.unfurledMediaItem };
          tmp = obj3;
        }
      }
      let tmp2 = null != tmp;
      if (tmp2) {
        let result = tmp4(7306).isMediaObscuredForHarmTypes(tmp, enabledHarmTypesForMessage);
        if (result) {
          result = tmp4(7614).shouldAgeVerifyForExplicitMedia();
          const tmp4Result2 = tmp4(7614);
        }
        tmp2 = result;
        const tmp4Result = tmp4(7306);
      }
      return tmp2;
    }
  }
