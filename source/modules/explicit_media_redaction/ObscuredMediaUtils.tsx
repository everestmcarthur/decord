...
// Dependencies: [32, 4721, 1958, 4943, 4371, 1372, 7489, 7491, 1370, 7493, 1895, 4953, 1385, 1186, 7498, 2]
...
// Module 7488 (ObscuredMediaUtils)
...
import isForwardMessage from "isForwardMessage" /* 7498 */;
  ...
  if (null != currentUser) {
    if (id !== currentUser.id) {
      ...
      if (null == tmp10) {
        ...
        if (null == tmp10) {
          NONE = tmp12(7491).ContentHarmTypeBitMask.NONE;
        } else {
...
function findComponentMedia(components) {
  ...
  return obj.flatMap((type) => {
    ...
    if (require("Server").ComponentType.MEDIA_GALLERY === type) {
      ...
      return type.media;
    } else if (tmp(1895).ComponentType.FILE === type) {
      return type.file;
    } else {
      if (tmp(1895).ComponentType.SECTION !== type) {
        if (tmp(1895).ComponentType.ACTION_ROW !== type) {
          if (tmp(1895).ComponentType.CONTAINER !== type) {
            return [];
          }
        }
      }
...
function isMediaScanPending(type, NONE) {
  if (NONE === media(7491).ContentHarmTypeBitMask.NONE) {
    return false;
    ...
    if (0 === arr.length) {
      ...
      type = type.type;
      if (tmp(7493).ObscuredMediaTypes.Embed === type) {
            ...
            if (0 !== arr.filter((item) => !isMediaFlaggedForHarmType(item, { type: ExplicitMediaRedactionModels.ObscuredMediaTypes.Embed, media })).length) {
              ...
              if (!("images" in media3)) {
                ...
                if (-1 === content_scan_version) {
                  ...
                  if (!arr.includes(tmp(7491).ContentHarmType.GORE)) {
                    if (!arr.includes(tmp(7491).ContentHarmType.SELF_HARM)) {
        ...
        return flag3;
      } else if (tmp(7493).ObscuredMediaTypes.Attachment === type) {
        ...
        return tmp11;
      } else if (tmp(7493).ObscuredMediaTypes.GenericMedia === type) {
        media = type.media;
        let flag2 = false;
        if (0 !== arr.length) {
          flag2 = false;
...
function isMediaFlaggedForHarmType(EXPLICIT, type) {
  if (null == EXPLICIT) {
    ...
    } else if (tmp3(7493).ObscuredMediaTypes.Attachment === type) {
      let num2 = type.media.flags;
      if (num2 == null) {
        num2 = 0;
      }
      return tmp3(1385).hasFlag(num2, tmp5.attachmentFlag);
    } else if (tmp3(7493).ObscuredMediaTypes.GenericMedia === type) {
...
function contentHarmTypesToFlags(memo) {
  ...
  while (iter !== undefined) {
    ...
    if (HarmTypeConfiguration.ContentHarmType.EXPLICIT === nextResult) {
      NONE = NONE | tmp3(7491).ContentHarmTypeBitMask.EXPLICIT;
    } else if (tmp3(7491).ContentHarmType.GORE === nextResult) {
      NONE = NONE | tmp3(7491).ContentHarmTypeBitMask.GORE;
    } else if (tmp3(7491).ContentHarmType.SELF_HARM === nextResult) {
      NONE = NONE | tmp3(7491).ContentHarmTypeBitMask.SELF_HARM;
    }
    continue;
  }
  return NONE;
...
export const getUnscannedMediaIds = function getUnscannedMediaIds(message) {
  ...
  if (null != channelId) {
    ...
    if (NONE === NONE(7491).ContentHarmTypeBitMask.NONE) {
  ...
  NONE = NONE(7491).ContentHarmTypeBitMask.NONE;
};
