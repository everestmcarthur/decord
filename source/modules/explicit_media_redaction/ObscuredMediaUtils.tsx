...
// Dependencies: [32, 4639, 1958, 4861, 4288, 1372, 7407, 7409, 1370, 7411, 1895, 4871, 1385, 1186, 7416, 2]
...
// Module 7406 (ObscuredMediaUtils)
  ...
  if (null != currentUser) {
    if (id !== currentUser.id) {
      ...
      if (null == tmp10) {
        ...
        if (null == tmp10) {
          NONE = tmp12(7409).ContentHarmTypeBitMask.NONE;
        } else {
...
function isMediaScanPending(type, NONE) {
  if (NONE === media(7409).ContentHarmTypeBitMask.NONE) {
    return false;
    ...
    if (0 === arr.length) {
      ...
      type = type.type;
      if (tmp(7411).ObscuredMediaTypes.Embed === type) {
            ...
            if (0 !== arr.filter((item) => !isMediaFlaggedForHarmType(item, { type: ExplicitMediaRedactionModels.ObscuredMediaTypes.Embed, media })).length) {
              ...
              if (!("images" in media3)) {
                ...
                if (-1 === content_scan_version) {
                  ...
                  if (!arr.includes(tmp(7409).ContentHarmType.GORE)) {
                    if (!arr.includes(tmp(7409).ContentHarmType.SELF_HARM)) {
        ...
        return flag3;
      } else if (tmp(7411).ObscuredMediaTypes.Attachment === type) {
        ...
        return tmp11;
      } else if (tmp(7411).ObscuredMediaTypes.GenericMedia === type) {
        media = type.media;
        let flag2 = false;
        if (0 !== arr.length) {
          flag2 = false;
...
function isMediaFlaggedForHarmType(EXPLICIT, type) {
  if (null == EXPLICIT) {
    ...
    } else if (tmp3(7411).ObscuredMediaTypes.Attachment === type) {
      let num2 = type.media.flags;
      if (num2 == null) {
        num2 = 0;
      }
      return tmp3(1385).hasFlag(num2, tmp5.attachmentFlag);
    } else if (tmp3(7411).ObscuredMediaTypes.GenericMedia === type) {
...
function contentHarmTypesToFlags(memo) {
  ...
  while (iter !== undefined) {
    ...
    if (HarmTypeConfiguration.ContentHarmType.EXPLICIT === nextResult) {
      NONE = NONE | tmp3(7409).ContentHarmTypeBitMask.EXPLICIT;
    } else if (tmp3(7409).ContentHarmType.GORE === nextResult) {
      NONE = NONE | tmp3(7409).ContentHarmTypeBitMask.GORE;
    } else if (tmp3(7409).ContentHarmType.SELF_HARM === nextResult) {
      NONE = NONE | tmp3(7409).ContentHarmTypeBitMask.SELF_HARM;
    }
    continue;
  }
  return NONE;
...
export const getEnabledHarmTypesBitmaskForChannelType = function getEnabledHarmTypesBitmaskForChannelType(GUILD) {
  ...
  if (null == GUILD) {
    let NONE = tmp(7409).ContentHarmTypeBitMask.NONE;
  } else {
    const mapped = found.map((harmType) => {
      const tmp = harmType.getUserSettingsWithDefaults()[closure_0];
      let hasItem = null != tmp;
...
export const getUnscannedMediaIds = function getUnscannedMediaIds(message) {
  ...
  if (null != channelId) {
    ...
    if (NONE === NONE(7409).ContentHarmTypeBitMask.NONE) {
  ...
  NONE = NONE(7409).ContentHarmTypeBitMask.NONE;
};
