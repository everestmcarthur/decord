...
// Dependencies: [5, 32, 19, 21, 12179, 9443, 12144, 7637, 5700, 5062, 1115, 4584, 2]
// Exports: default, useCommandRowSend

// Module 12256 (CommandRowButton)
...
export default function CommandRowIcon(hasOptions) {
  if (hasOptions.hasOptions) {
    let tmp3Result = tmp3(tmp4(5700).TableRowArrow, {});
  } else {
    ...
    obj.onPress = tmp2;
    obj.icon = tmp3(tmp4(4584).SendMessageIcon, { size: "sm" });
    obj.disabled = tmp;
    tmp3Result = tmp3(tmp4(5062).Button, obj);
  }
  return tmp3Result;
};
      ...
      try {
        ...
        if (0 === dependencyMap) {
          if (arg0 === 1) {
            ...
            const obj2 = tmp3(9443);
            obj6.optionValues = tmp3(12144).parseOptionValuesForSend(commandContext.channel, command, {});
            obj6.context = commandContext;
            obj6.sectionName = sectionName;
            obj6.commandOrigin = tmp3(7637).CommandOrigin.APP_LAUNCHER_APPLICATION_VIEW;
            dependencyMap = 2;
            c4 = 1;
