...
export default function useAppLauncherOnboardingContent(channelId) {
  ...
  if (tmp3.canShowAppsOrActivitiesBanner) {
    ...
    if ((function useHasUsedActivities(channel) {
      ...
      let result = channelId(4427).useIsDismissibleContentDismissed_UNSAFE(channelId(1943).DismissibleContent.APP_LAUNCHER_ONBOARDING_ACTIVITIES_BANNER);
      const obj = channelId(4427);
      const tmp = channelId;
      const result1 = channelId(4427).useIsDismissibleContentDismissed_UNSAFE(channelId(1943).DismissibleContent.APP_LAUNCHER_ONBOARDING_APPS_BANNER);
      const obj2 = channelId(4427);
