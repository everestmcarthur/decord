...
// Dependencies: [19, 17, 4597, 21, 1894, 4605, 576, 504, 4341, 12231, 12232, 4601, 12233, 1178, 10485, 2]
// Exports: default

// Module 12230 (CommandOptionView)
import nativeDefault from "native" /* 576 */;
import ReanimatedRexportDefault from "ReanimatedRexport" /* 4341 */;
import AppLauncherCommandOptionDefault from "AppLauncherCommandOption" /* 12233 */;
...
export default function CommandOptionView(option) {
  ...
  const ReduceMotion = option(4341).ReduceMotion;
  const tmp5 = stateFromStores ? ReduceMotion.Always : ReduceMotion.Never;
  const obj = option(504);
  const optionEnteringAnimation = option(12231).useOptionEnteringAnimation();
  ...
  if (set.has(option.type)) {
    ...
    const FadeOut = tmp2(4341).FadeOut;
    const FadeInUp = tmp2(4341).FadeInUp;
    ...
    const obj5 = {
      ...
      exiting: tmp2(12231).ExitingAnimation,
      layout: tmp2(12231).LayoutAnimation,
      onLayout(arg0) {
    ...
    if (hasItem1) {
      ...
      tmp17Result = tmp17(tmp2(4601).Text, obj7);
    }
    ...
    items2[2] = closure_5(tmp2(4601).Text, obj9);
    if (hasItem) {
      ...
      const obj11 = { style: tmp.optionErrorIcon, source: tmp18(10485), size: tmp2(1178).IconSizes.REFRESH_SMALL_16 };
      ...
      items3[1] = tmp17(tmp2(4601).Text, obj12);
      obj10.children = items3;
      hasItem = tmp19(tmp18(4341).View, obj10);
    }
    items2[3] = hasItem;
    obj6.children = items2;
    obj5.children = closure_6(View, obj6);
    obj4.children = closure_5(ReanimatedRexportDefault.View, obj5);
    obj3.children = closure_5(tmp2(12232).AwaitAnimationContext, obj4);
    return closure_5(tmp2(4341).LayoutAnimationConfig, obj3);
  } else {
    return null;
  }
  const tmp2Result = option(12231);
};
