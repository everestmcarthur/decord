...
// Dependencies: [19, 17, 4630, 21, 1895, 4638, 576, 504, 4374, 12303, 12304, 4634, 12305, 1177, 10555, 2]
// Exports: default

// Module 12302 (CommandOptionView)
import nativeDefault from "native" /* 576 */;
import ReanimatedRexportDefault from "ReanimatedRexport" /* 4374 */;
import AppLauncherCommandOptionDefault from "AppLauncherCommandOption" /* 12305 */;
...
export default function CommandOptionView(option) {
  ...
  const ReduceMotion = option(4374).ReduceMotion;
  const tmp5 = stateFromStores ? ReduceMotion.Always : ReduceMotion.Never;
  const obj = option(504);
  const optionEnteringAnimation = option(12303).useOptionEnteringAnimation();
  ...
  if (set.has(option.type)) {
    ...
    const FadeOut = tmp2(4374).FadeOut;
    const FadeInUp = tmp2(4374).FadeInUp;
    ...
    const obj5 = {
      ...
      exiting: tmp2(12303).ExitingAnimation,
      layout: tmp2(12303).LayoutAnimation,
      onLayout(arg0) {
    ...
    if (hasItem1) {
      ...
      tmp17Result = tmp17(tmp2(4634).Text, obj7);
    }
    ...
    items2[2] = closure_5(tmp2(4634).Text, obj9);
    if (hasItem) {
      ...
      const obj11 = { style: tmp.optionErrorIcon, source: tmp18(10555), size: tmp2(1177).IconSizes.REFRESH_SMALL_16 };
      ...
      items3[1] = tmp17(tmp2(4634).Text, obj12);
      obj10.children = items3;
      hasItem = tmp19(tmp18(4374).View, obj10);
    }
    items2[3] = hasItem;
    obj6.children = items2;
    obj5.children = closure_6(View, obj6);
    obj4.children = closure_5(ReanimatedRexportDefault.View, obj5);
    obj3.children = closure_5(tmp2(12304).AwaitAnimationContext, obj4);
    return closure_5(tmp2(4374).LayoutAnimationConfig, obj3);
  } else {
    return null;
  }
  const tmp2Result = option(12303);
};
