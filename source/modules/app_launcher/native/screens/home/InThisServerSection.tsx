...
// Dependencies: [19, 17, 21, 4607, 576, 12183, 1369, 4603, 1114, 5175, 12146, 8994, 9502, 12151, 2]
// Exports: default

// Module 12205 (InThisServerSection)
...
function AppInThisServer(onAppSelected) {
  ...
  const appLauncherIconSource = onAppSelected(12146).getAppLauncherIconSource(application);
  const obj2 = {
    accessible: true,
  ...
  if (null != appLauncherIconSource) {
    ...
    tmp6 = closure_5(application(12151), obj3);
  }
  const items = [tmp6, closure_5(onAppSelected(4603).Text, { variant: "text-md/semibold", color: "mobile-text-heading-primary", lineClamp: 1, children: application.name })];
  obj2.children = items;
  return closure_6(onAppSelected(8994).PressableScale, obj2, application.id);
}
...
export default function InThisServerSection(arg0) {
  ...
  if (0 === items.length) {
    ...
    if (tmp5Result) {
      ...
      obj4.children = tmp5(tmp11(4603).Text, obj5);
      tmp5Result = tmp5(tmp11(5175).PressableOpacity, obj4);
    }
    items2[1] = tmp5Result;
