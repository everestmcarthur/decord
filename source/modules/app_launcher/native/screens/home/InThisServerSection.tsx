...
// Dependencies: [19, 17, 21, 4640, 576, 12239, 1370, 4636, 1115, 5210, 12202, 9060, 9563, 12207, 2]
// Exports: default

// Module 12261 (InThisServerSection)
...
function AppInThisServer(onAppSelected) {
  ...
  const appLauncherIconSource = onAppSelected(12202).getAppLauncherIconSource(application);
  const obj2 = {
    accessible: true,
  ...
  if (null != appLauncherIconSource) {
    ...
    tmp6 = closure_5(application(12207), obj3);
  }
  const items = [tmp6, closure_5(onAppSelected(4636).Text, { variant: "text-md/semibold", color: "mobile-text-heading-primary", lineClamp: 1, children: application.name })];
  obj2.children = items;
  return closure_6(onAppSelected(9060).PressableScale, obj2, application.id);
}
...
export default function InThisServerSection(arg0) {
  ...
  if (0 === items.length) {
    ...
    if (tmp5Result) {
      ...
      obj4.children = tmp5(tmp11(4636).Text, obj5);
      tmp5Result = tmp5(tmp11(5210).PressableOpacity, obj4);
    }
    items2[1] = tmp5Result;
