...
// Dependencies: [19, 17, 21, 4722, 576, 12330, 1370, 4718, 1115, 5292, 12293, 9144, 9647, 12298, 2]
// Exports: default

// Module 12352 (InThisServerSection)
...
function AppInThisServer(onAppSelected) {
  ...
  const appLauncherIconSource = onAppSelected(12293).getAppLauncherIconSource(application);
  const obj2 = {
    accessible: true,
  ...
  if (null != appLauncherIconSource) {
    ...
    tmp6 = closure_5(application(12298), obj3);
  }
  const items = [tmp6, closure_5(onAppSelected(4718).Text, { variant: "text-md/semibold", color: "mobile-text-heading-primary", lineClamp: 1, children: application.name })];
  obj2.children = items;
  return closure_6(onAppSelected(9144).PressableScale, obj2, application.id);
}
...
export default function InThisServerSection(arg0) {
  ...
  if (0 === items.length) {
    ...
    if (tmp5Result) {
      ...
      obj4.children = tmp5(tmp11(4718).Text, obj5);
      tmp5Result = tmp5(tmp11(5292).PressableOpacity, obj4);
    }
    items2[1] = tmp5Result;
