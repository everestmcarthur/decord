...
// Dependencies: [19, 17, 21, 4605, 576, 12158, 1369, 4601, 1114, 5173, 12121, 8971, 9479, 12126, 2]
// Exports: default

// Module 12180 (InThisServerSection)
...
function AppInThisServer(onAppSelected) {
  ...
  const appLauncherIconSource = onAppSelected(12121).getAppLauncherIconSource(application);
  const obj2 = {
    accessible: true,
  ...
  if (null != appLauncherIconSource) {
    ...
    tmp6 = closure_5(application(12126), obj3);
  }
  const items = [tmp6, closure_5(onAppSelected(4601).Text, { variant: "text-md/semibold", color: "mobile-text-heading-primary", lineClamp: 1, children: application.name })];
  obj2.children = items;
  return closure_6(onAppSelected(8971).PressableScale, obj2, application.id);
}
...
export default function InThisServerSection(arg0) {
  ...
  if (0 === items.length) {
    ...
    if (tmp5Result) {
      ...
      obj4.children = tmp5(tmp11(4601).Text, obj5);
      tmp5Result = tmp5(tmp11(5173).PressableOpacity, obj4);
    }
    items2[1] = tmp5Result;
