...
// Dependencies: [19, 17, 1074, 21, 4606, 576, 9379, 8272, 1396, 12176, 5174, 12185, 1894, 12179, 4602, 1114, 12149, 5663, 2]
// Exports: default

// Module 12184 (RecommendationsBannerCard)
...
export default function RecommendationsBannerCard(application) {
  ...
  const tmp = closure_8();
  if (!showsPromoted) {
    showsPromoted = application(9379).isPromotedApplication(application);
    const obj = application(9379);
  }
  const shelfBadgeTypeIfActive = application(9379).getShelfBadgeTypeIfActive(application);
  ...
  if (isLandscape) {
    ...
    let tmp12Result2 = closure_6(tmp4(12176).BaseAppRow, obj5);
  } else {
    ...
    const tmp10Result = tmp10(12185);
    obj10.applicationEmbedded = tmp4(9379).isEmbeddedApp(application);
    ...
    if (showsPromoted) {
      ...
      const items3 = [tmp14(tmp10(12179), obj12), ];
      if (showsPromoted) {
        ...
        obj13.children = tmp14(tmp4(4602).Text, obj14);
    ...
    if (tmp14Result) {
      ...
      tmp14Result = tmp14(tmp10(12149), obj16);
    }
