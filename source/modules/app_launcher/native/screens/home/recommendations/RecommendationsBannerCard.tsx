...
// Dependencies: [19, 17, 1074, 21, 4640, 576, 9443, 8335, 1397, 12234, 5210, 12243, 1895, 12237, 4636, 1115, 12207, 5700, 2]
// Exports: default

// Module 12242 (RecommendationsBannerCard)
...
export default function RecommendationsBannerCard(application) {
  ...
  const tmp = closure_8();
  if (!showsPromoted) {
    showsPromoted = application(9443).isPromotedApplication(application);
    const obj = application(9443);
  }
  const shelfBadgeTypeIfActive = application(9443).getShelfBadgeTypeIfActive(application);
  ...
  if (isLandscape) {
    ...
    let tmp12Result2 = closure_6(tmp4(12234).BaseAppRow, obj5);
  } else {
    ...
    const tmp10Result = tmp10(12243);
    obj10.applicationEmbedded = tmp4(9443).isEmbeddedApp(application);
    ...
    if (showsPromoted) {
      ...
      const items3 = [tmp14(tmp10(12237), obj12), ];
      if (showsPromoted) {
        ...
        obj13.children = tmp14(tmp4(4636).Text, obj14);
    ...
    if (tmp14Result) {
      ...
      tmp14Result = tmp14(tmp10(12207), obj16);
    }
