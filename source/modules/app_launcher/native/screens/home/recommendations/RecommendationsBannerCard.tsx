...
// Dependencies: [19, 17, 1074, 21, 4638, 576, 9432, 8316, 1397, 12225, 5208, 12234, 1895, 12228, 4634, 1115, 12198, 5698, 2]
// Exports: default

// Module 12233 (RecommendationsBannerCard)
...
export default function RecommendationsBannerCard(application) {
  ...
  const tmp = closure_8();
  if (!showsPromoted) {
    showsPromoted = application(9432).isPromotedApplication(application);
    const obj = application(9432);
  }
  const shelfBadgeTypeIfActive = application(9432).getShelfBadgeTypeIfActive(application);
  ...
  if (isLandscape) {
    ...
    let tmp12Result2 = closure_6(tmp4(12225).BaseAppRow, obj5);
  } else {
    ...
    const tmp10Result = tmp10(12234);
    obj10.applicationEmbedded = tmp4(9432).isEmbeddedApp(application);
    ...
    if (showsPromoted) {
      ...
      const items3 = [tmp14(tmp10(12228), obj12), ];
      if (showsPromoted) {
        ...
        obj13.children = tmp14(tmp4(4634).Text, obj14);
    ...
    if (tmp14Result) {
      ...
      tmp14Result = tmp14(tmp10(12198), obj16);
    }
