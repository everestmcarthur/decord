...
// Dependencies: [19, 17, 1074, 21, 4636, 576, 9418, 8304, 1396, 12212, 5204, 12221, 1894, 12215, 4632, 1114, 12185, 5693, 2]
// Exports: default

// Module 12220 (RecommendationsBannerCard)
...
export default function RecommendationsBannerCard(application) {
  ...
  const tmp = closure_8();
  if (!showsPromoted) {
    showsPromoted = application(9418).isPromotedApplication(application);
    const obj = application(9418);
  }
  const shelfBadgeTypeIfActive = application(9418).getShelfBadgeTypeIfActive(application);
  ...
  if (isLandscape) {
    ...
    let tmp12Result2 = closure_6(tmp4(12212).BaseAppRow, obj5);
  } else {
    ...
    const tmp10Result = tmp10(12221);
    obj10.applicationEmbedded = tmp4(9418).isEmbeddedApp(application);
    ...
    if (showsPromoted) {
      ...
      const items3 = [tmp14(tmp10(12215), obj12), ];
      if (showsPromoted) {
        ...
        obj13.children = tmp14(tmp4(4632).Text, obj14);
    ...
    if (tmp14Result) {
      ...
      tmp14Result = tmp14(tmp10(12185), obj16);
    }
