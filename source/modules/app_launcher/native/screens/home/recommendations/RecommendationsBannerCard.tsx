...
// Dependencies: [19, 17, 1074, 21, 4639, 576, 9421, 8307, 1397, 12217, 5207, 12226, 1895, 12220, 4635, 1115, 12190, 5697, 2]
// Exports: default

// Module 12225 (RecommendationsBannerCard)
...
export default function RecommendationsBannerCard(application) {
  ...
  const tmp = closure_8();
  if (!showsPromoted) {
    showsPromoted = application(9421).isPromotedApplication(application);
    const obj = application(9421);
  }
  const shelfBadgeTypeIfActive = application(9421).getShelfBadgeTypeIfActive(application);
  ...
  if (isLandscape) {
    ...
    let tmp12Result2 = closure_6(tmp4(12217).BaseAppRow, obj5);
  } else {
    ...
    const tmp10Result = tmp10(12226);
    obj10.applicationEmbedded = tmp4(9421).isEmbeddedApp(application);
    ...
    if (showsPromoted) {
      ...
      const items3 = [tmp14(tmp10(12220), obj12), ];
      if (showsPromoted) {
        ...
        obj13.children = tmp14(tmp4(4635).Text, obj14);
    ...
    if (tmp14Result) {
      ...
      tmp14Result = tmp14(tmp10(12190), obj16);
    }
