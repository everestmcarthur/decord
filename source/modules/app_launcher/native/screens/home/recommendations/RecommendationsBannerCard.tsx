...
// Dependencies: [19, 17, 1074, 21, 4722, 576, 9527, 8417, 1397, 12325, 5292, 12334, 1895, 12328, 4718, 1115, 12298, 5782, 2]
// Exports: default

// Module 12333 (RecommendationsBannerCard)
...
export default function RecommendationsBannerCard(application) {
  ...
  const tmp = closure_8();
  if (!showsPromoted) {
    showsPromoted = application(9527).isPromotedApplication(application);
    const obj = application(9527);
  }
  const shelfBadgeTypeIfActive = application(9527).getShelfBadgeTypeIfActive(application);
  ...
  if (isLandscape) {
    ...
    let tmp12Result2 = closure_6(tmp4(12325).BaseAppRow, obj5);
  } else {
    ...
    const tmp10Result = tmp10(12334);
    obj10.applicationEmbedded = tmp4(9527).isEmbeddedApp(application);
    ...
    if (showsPromoted) {
      ...
      const items3 = [tmp14(tmp10(12328), obj12), ];
      if (showsPromoted) {
        ...
        obj13.children = tmp14(tmp4(4718).Text, obj14);
    ...
    if (tmp14Result) {
      ...
      tmp14Result = tmp14(tmp10(12298), obj16);
    }
