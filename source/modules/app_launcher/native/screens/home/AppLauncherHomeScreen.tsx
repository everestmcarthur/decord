...
// Dependencies: [32, 19, 17, 1957, 9433, 4637, 12215, 4276, 12186, 1483, 1074, 1955, 21, 576, 12219, 4638, 4634, 1115, 12198, 9432, 10254, 7995, 5691, 12193, 1397, 11144, 12226, 9792, 504, 7633, 12227, 5673, 9045, 12228, 12202, 7053, 1612, 12229, 8397, 7159, 11361, 1478, 9552, 9647, 12231, 12230, 12232, 12233, 12235, 12236, 12194, 7631, 12237, 12, 4818, 7148, 12239, 12240, 8908, 1249, 4458, 1944, 12241, 12242, 12244, 7160, 12246, 1370, 12252, 12253, 12255, 12258, 12259, 12220, 12260, 12181, 12261, 12264, 9689, 12213, 9560, 12265, 12250, 12266, 1385, 12267, 12268, 2]
// Exports: default

// Module 12225 (AppLauncherHomeScreen)
...
import AppLauncherContext from "AppLauncherContext" /* 11361 */;
import AppLauncherNativeUtils from "AppLauncherNativeUtils" /* 12193 */;
import FrecencySection from "FrecencySection" /* 12194 */;
import EntityBorderAppIconDefault from "EntityBorderAppIcon" /* 12198 */;
import ApplicationDirectoryActionCreators from "ApplicationDirectoryActionCreators" /* 12213 */;
import HeroMedia from "HeroMedia" /* 12226 */;
import AppLauncherHomeTypes from "AppLauncherHomeTypes" /* 12230 */;
import AppLauncherOnboardingActionCreators from "AppLauncherOnboardingActionCreators" /* 12241 */;
import InThisServerSection from "InThisServerSection" /* 12252 */;
import ApplicationDirectoryCollectionType from "ApplicationDirectoryCollectionType" /* 12265 */;
import _slicedToArray from "module_32" /* 32 */;
import noop from "module_19" /* 19 */;
import EmbeddedActivitiesStore from "EmbeddedActivitiesStore" /* 1957 */;
import DevSettingsStore from "DevSettingsStore" /* 4637 */;
import ApplicationDirectoryCollectionsStore from "ApplicationDirectoryCollectionsStore" /* 12215 */;
import PermissionStore from "PermissionStore" /* 4276 */;
import AppLauncherOnboardingPersistedStore from "AppLauncherOnboardingPersistedStore" /* 12186 */;
...
class BaseAppRow {
  constructor(arg0) {
    ...
    memo = closure_5.useMemo(() => {
      ...
      if (application == null) {
        FAKE_BUILT_IN_APP = tmp(9432).FAKE_BUILT_IN_APP;
      }
      ...
      if (null != sectionDescription) {
        ...
        if ("" !== sectionDescription) {
          const obj2 = { content: sectionDescription, muted: false, layout: tmp(7995).ChannelListLayoutTypes.COMPACT, color: "text-muted" };
          result = tmp(10254).renderMessagePreviewMarkup(obj2);
...
function ActivityRow(section) {
  ...
  if (tmp5) {
    ...
    tmp5 = closure_19(onPress(12198), obj3);
  }
  const items = [section];
  ...
  return closure_19(section(5691).TableRow, { icon: tmp5, label: section.application.name, subLabel: memo, subLabelLineClamp: 1, start: flag, end: flag2, arrow: true, onPress: callback });
}
...
function RecommendationItem(onPress) {
  ...
  if (first) {
    let tmp13 = closure_1_19(tmp7(12227), {});
  } else {
    tmp13 = null;
    if (!tmp12) {
      const obj6 = {
      ...
      obj6.source = obj7;
      tmp13 = closure_1_19(tmp7(5673), obj6);
    }
  }
  ...
  if (isEmbeddedAppResult) {
    ...
    const tmp7Result = tmp7(12228);
    obj10.labelType = tmp2(9432).getShelfBadgeTypeIfActive(application);
    isEmbeddedAppResult = closure_1_19(tmp7Result, obj10);
    const tmp2Result = tmp2(9432);
  }
...
let closure_23 = [];
const APP_LAUNCHER_IN_TEXT = fn(12219).ApplicationCollectionSurface.APP_LAUNCHER_IN_TEXT;
    ...
    if (0 !== handleViewableItemsChanged.length) {
      if (initialSearchQuery === AppLauncherTypes.AppLauncherEntrypoint.VOICE) {
        ...
        if (arr.length % 2 === 1) {
          ...
          obj2.type = tmp8(12230).AppLauncherHomeListItemType.SHELF_ITEM_TUPLE;
        ...
        const obj5 = { type: tmp8(12230).AppLauncherHomeListItemType.SECTION_HEADER, section: null, sectionName: "activities", numItems: null, numVisibleItems: null };
    ...
    if (memo8) {
      ...
      let item = onActivityItemSelected.forEach((title, sectionOverallPosition) => {
        ...
        if (type === ApplicationDirectoryCollectionType.ApplicationDirectoryCollectionType.BANNER_CARDS) {
          ...
          const bound = Math.min(length, tmp(12250).COLLAPSED_LIST_ITEM_MAX);
          obj.type = tmp(12230).AppLauncherHomeListItemType.RECOMMENDATION_SECTION_HEADER;
          ...
          const mapped = application_directory_collection_items.map((type, sectionPosition) => {
            if (type.type === found1(9560).ApplicationDirectoryCollectionItemType.APPLICATION) {
              let collectionItemAssetUrl;
              if (tmp4) {
                ({ id: obj2.itemId, image_hash: obj2.hash } = type);
                collectionItemAssetUrl = tmp(12266).getCollectionItemAssetUrl({ itemId: null, hash: null });
                const obj = { itemId: null, hash: null };
                const tmpResult = tmp(12266);
              }
              const obj3 = { application: type.application, showsPromoted: tmp(1385).hasFlag(type.flags, tmp(12267).ApplicationCollectionItemFlags.PROMOTED), overrideImageUrl: collectionItemAssetUrl, sectionPosition };
              return obj3;
            }
          });
        ...
        } else if (type === tmp(12265).ApplicationDirectoryCollectionType.EXPANDABLE_LIST) {
          const prop = title.application_directory_collection_items;
          const mapped1 = prop.map((type) => {
            if (type.type === found1(9560).ApplicationDirectoryCollectionItemType.APPLICATION) {
              const obj = { application: type.application, showsPromoted: tmp(1385).hasFlag(type.flags, tmp(12267).ApplicationCollectionItemFlags.PROMOTED) };
              return obj;
            }
          });
          const found1 = mapped1.filter(tmp(1370).isNotNullish);
          if (tmpResult.hasFlag(title.flags, tmp(12268).ApplicationCollectionFlags.APPENDS_REMAINING_ACTIVITIES)) {
  ...
  if (tmp51) {
    ...
    let obj13 = {
      ...
      onViewAllSelected(sectionItemType) {
          ...
          } else if (sectionItemType === tmp(12194).SectionItemType.COMMANDS) {
    ...
    let obj14 = {
      ...
      onAppSelected: callback3,
      onViewAllSelected() {
          const found = memo.find((type) => type.type === context(12230).AppLauncherHomeListItemType.VIEW_ALL);
          let mapped;
          if (found != null) {
