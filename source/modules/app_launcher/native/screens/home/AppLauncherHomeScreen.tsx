...
// Dependencies: [32, 19, 17, 1957, 9422, 4638, 12207, 4278, 12178, 1483, 1074, 1955, 21, 576, 12211, 4639, 4635, 1115, 12190, 9421, 10246, 7986, 5690, 12185, 1397, 11130, 12218, 9784, 504, 7630, 12219, 5672, 9034, 12220, 12194, 7050, 1612, 12221, 8388, 7156, 11353, 1478, 9541, 9636, 12223, 12222, 12224, 12225, 12227, 12228, 12186, 7628, 12229, 12, 4819, 7145, 12231, 12232, 8897, 1249, 4460, 1944, 12233, 12234, 12236, 7157, 12238, 1370, 12244, 12245, 12247, 12250, 12251, 12212, 12252, 12173, 12253, 12256, 9678, 12205, 9549, 12257, 12242, 12258, 1385, 12259, 12260, 2]
// Exports: default

// Module 12217 (AppLauncherHomeScreen)
...
import EntityBorderAppIconDefault from "EntityBorderAppIcon" /* 12190 */;
import ApplicationDirectoryActionCreators from "ApplicationDirectoryActionCreators" /* 12205 */;
import HeroMedia from "HeroMedia" /* 12218 */;
import AppLauncherHomeTypes from "AppLauncherHomeTypes" /* 12222 */;
import AppLauncherOnboardingActionCreators from "AppLauncherOnboardingActionCreators" /* 12233 */;
import InThisServerSection from "InThisServerSection" /* 12244 */;
import ApplicationDirectoryCollectionType from "ApplicationDirectoryCollectionType" /* 12257 */;
import _slicedToArray from "module_32" /* 32 */;
import noop from "module_19" /* 19 */;
import EmbeddedActivitiesStore from "EmbeddedActivitiesStore" /* 1957 */;
import DevSettingsStore from "DevSettingsStore" /* 4638 */;
import ApplicationDirectoryCollectionsStore from "ApplicationDirectoryCollectionsStore" /* 12207 */;
import PermissionStore from "PermissionStore" /* 4278 */;
import AppLauncherOnboardingPersistedStore from "AppLauncherOnboardingPersistedStore" /* 12178 */;
...
class BaseAppRow {
  constructor(arg0) {
    ...
    memo = closure_5.useMemo(() => {
      ...
      if (application == null) {
        FAKE_BUILT_IN_APP = tmp(9421).FAKE_BUILT_IN_APP;
      }
      ...
      if (null != sectionDescription) {
        ...
        if ("" !== sectionDescription) {
          const obj2 = { content: sectionDescription, muted: false, layout: tmp(7986).ChannelListLayoutTypes.COMPACT, color: "text-muted" };
          result = tmp(10246).renderMessagePreviewMarkup(obj2);
          const tmpResult = tmp(10246);
        }
      }
      return result;
    }, items);
...
function ActivityRow(section) {
  ...
  return closure_19(section(5690).TableRow, { icon: tmp5, label: section.application.name, subLabel: memo, subLabelLineClamp: 1, start: flag, end: flag2, arrow: true, onPress: callback });
}
...
function RecommendationItem(onPress) {
  ...
  if (first) {
    let tmp13 = closure_1_19(tmp7(12219), {});
  } else {
    tmp13 = null;
    if (!tmp12) {
      const obj6 = {
      ...
      obj6.source = obj7;
      tmp13 = closure_1_19(tmp7(5672), obj6);
    }
  }
  ...
  if (isEmbeddedAppResult) {
    ...
    const tmp7Result = tmp7(12220);
    obj10.labelType = tmp2(9421).getShelfBadgeTypeIfActive(application);
    isEmbeddedAppResult = closure_1_19(tmp7Result, obj10);
    const tmp2Result = tmp2(9421);
  }
  ...
  const memo4 = obj3.useMemo(() => {
    ...
    if (0 !== handleViewableItemsChanged.length) {
      if (initialSearchQuery === AppLauncherTypes.AppLauncherEntrypoint.VOICE) {
        ...
        const obj5 = { type: tmp8(12222).AppLauncherHomeListItemType.SECTION_HEADER, section: null, sectionName: "activities", numItems: null, numVisibleItems: null };
    ...
    if (memo8) {
      ...
      let item = onActivityItemSelected.forEach((title, sectionOverallPosition) => {
        ...
        if (type === ApplicationDirectoryCollectionType.ApplicationDirectoryCollectionType.BANNER_CARDS) {
          ...
          const bound = Math.min(length, tmp(12242).COLLAPSED_LIST_ITEM_MAX);
          obj.type = tmp(12222).AppLauncherHomeListItemType.RECOMMENDATION_SECTION_HEADER;
          ...
          const mapped = application_directory_collection_items.map((type, sectionPosition) => {
            if (type.type === found1(9549).ApplicationDirectoryCollectionItemType.APPLICATION) {
              ...
              if (tmp4) {
                ...
                collectionItemAssetUrl = tmp(12258).getCollectionItemAssetUrl({ itemId: null, hash: null });
                const obj = { itemId: null, hash: null };
                const tmpResult = tmp(12258);
              }
              const obj3 = { application: type.application, showsPromoted: tmp(1385).hasFlag(type.flags, tmp(12259).ApplicationCollectionItemFlags.PROMOTED), overrideImageUrl: collectionItemAssetUrl, sectionPosition };
              return obj3;
            }
          });
        ...
        } else if (type === tmp(12257).ApplicationDirectoryCollectionType.EXPANDABLE_LIST) {
          const prop = title.application_directory_collection_items;
          const mapped1 = prop.map((type) => {
            if (type.type === found1(9549).ApplicationDirectoryCollectionItemType.APPLICATION) {
              const obj = { application: type.application, showsPromoted: tmp(1385).hasFlag(type.flags, tmp(12259).ApplicationCollectionItemFlags.PROMOTED) };
              return obj;
            }
          });
          const found1 = mapped1.filter(tmp(1370).isNotNullish);
          if (tmpResult.hasFlag(title.flags, tmp(12260).ApplicationCollectionFlags.APPENDS_REMAINING_ACTIVITIES)) {
  ...
  if (tmp51) {
    ...
    let obj13 = {
      ...
      onViewAllSelected(sectionItemType) {
          ...
          } else if (sectionItemType === tmp(12186).SectionItemType.COMMANDS) {
