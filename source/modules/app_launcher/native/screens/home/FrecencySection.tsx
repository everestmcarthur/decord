...
// Dependencies: [32, 19, 17, 1957, 12204, 1074, 21, 4640, 576, 9443, 1370, 7282, 12205, 11369, 504, 9563, 4820, 4376, 4641, 1115, 9593, 12206, 12202, 1895, 9060, 12207, 4636, 8052, 11314, 5210, 12208, 7637, 5675, 12211, 2]
// Exports: default

// Module 12203 (FrecencySection)
...
import FrecencySectionStoreActionCreators from "FrecencySectionStoreActionCreators" /* 12206 */;
import _slicedToArray from "module_32" /* 32 */;
import noop from "module_19" /* 19 */;
import EmbeddedActivitiesStore from "EmbeddedActivitiesStore" /* 1957 */;
import FrecencySectionStore from "FrecencySectionStore" /* 12204 */;
...
function FrecentApp(app) {
  ...
  if (null == app.section) {
    ...
    const appLauncherIconSource = app(12202).getAppLauncherIconSource(app.section.application);
    ...
    if (tmp3) {
      ...
      tmp3 = closure_11(onAppSelected(5675), obj2);
    }
    const items = [tmp3, ];
    const obj3 = { submitting, style: tmp.submittingOverlay };
    items[1] = closure_11(app(12211).SubmittingOverlay, obj3);
    obj.children = items;
    return closure_12(app(5210).PressableOpacity, obj, app.applicationId);
  }
}
get_ActivityIndicator = fn(17);
({ View: hasOwnProperty, ScrollView: metroRequire } = get_ActivityIndicator);
const FrecencySectionSelection = fn(12204).FrecencySectionSelection;
  ...
  if (commands.length > 0) {
    ...
    const effect = obj2.useEffect(() => {
      ...
      if (!tmp) {
        ...
        if (tmp7 === tmp8.APPS) {
          let RECENT_COMMANDS = tmp5(9563).AppLauncherSectionName.RECENT_APPS;
        } else {
          RECENT_COMMANDS = tmp5(9563).AppLauncherSectionName.RECENT_COMMANDS;
        }
        obj2.section_name = RECENT_COMMANDS;
        HOME = tmp5(9563).AppLauncherLocations.HOME;
        obj2.location = HOME;
