...
// Dependencies: [32, 19, 17, 1956, 12123, 1074, 21, 4605, 576, 9358, 1369, 7220, 12124, 11287, 504, 9479, 4785, 4341, 4606, 1114, 9509, 12125, 12121, 1894, 8971, 12126, 4601, 8736, 11225, 5173, 12127, 7575, 5637, 12130, 2]
// Exports: default

// Module 12122 (FrecencySection)
...
import ChevronSmallDownIcon from "ChevronSmallDownIcon" /* 11225 */;
import usePlaceholderSize from "usePlaceholderSize" /* 12124 */;
import FrecencySectionStoreActionCreators from "FrecencySectionStoreActionCreators" /* 12125 */;
import _slicedToArray from "module_32" /* 32 */;
import noop from "module_19" /* 19 */;
import EmbeddedActivitiesStore from "EmbeddedActivitiesStore" /* 1956 */;
import FrecencySectionStore from "FrecencySectionStore" /* 12123 */;
...
function FrecentApp(app) {
  ...
  if (null == app.section) {
    ...
    const appLauncherIconSource = app(12121).getAppLauncherIconSource(app.section.application);
    ...
    if (tmp3) {
      ...
      tmp3 = closure_11(onAppSelected(5637), obj2);
    }
    const items = [tmp3, ];
    const obj3 = { submitting, style: tmp.submittingOverlay };
    items[1] = closure_11(app(12130).SubmittingOverlay, obj3);
    obj.children = items;
    return closure_12(app(5173).PressableOpacity, obj, app.applicationId);
  }
}
get_ActivityIndicator = fn(17);
({ View: hasOwnProperty, ScrollView: metroRequire } = get_ActivityIndicator);
const FrecencySectionSelection = fn(12123).FrecencySectionSelection;
  ...
  if (commands.length > 0) {
    ...
    const effect = obj2.useEffect(() => {
      ...
      if (!tmp) {
        ...
        if (tmp7 === tmp8.APPS) {
          let RECENT_COMMANDS = tmp5(9479).AppLauncherSectionName.RECENT_APPS;
        } else {
          RECENT_COMMANDS = tmp5(9479).AppLauncherSectionName.RECENT_COMMANDS;
        }
        obj2.section_name = RECENT_COMMANDS;
        HOME = tmp5(9479).AppLauncherLocations.HOME;
        obj2.location = HOME;
