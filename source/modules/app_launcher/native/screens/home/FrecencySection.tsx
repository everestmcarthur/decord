...
// Dependencies: [32, 19, 17, 1956, 12083, 1074, 21, 4574, 576, 9321, 1369, 7182, 12084, 11247, 504, 9441, 4754, 4310, 4575, 1114, 9471, 12085, 12081, 1894, 8934, 12086, 4570, 8699, 11185, 5137, 12087, 7537, 5601, 12090, 2]
// Exports: default
...
function FrecentApp(app) {
  ...
  if (null == app.section) {
    ...
    if (tmp3) {
      ...
      tmp3 = closure_11(onAppSelected(5601), obj2);
    }
    const items = [tmp3, ];
    const obj3 = { submitting, style: tmp.submittingOverlay };
    items[1] = closure_11(app(12090).SubmittingOverlay, obj3);
    obj.children = items;
  ...
  if (commands.length > 0) {
    ...
    const effect = obj2.useEffect(() => {
      ...
      if (!tmp) {
        ...
        if (tmp7 === tmp8.APPS) {
          let RECENT_COMMANDS = tmp5(9441).AppLauncherSectionName.RECENT_APPS;
        } else {
          RECENT_COMMANDS = tmp5(9441).AppLauncherSectionName.RECENT_COMMANDS;
        }
        obj2.section_name = RECENT_COMMANDS;
        HOME = tmp5(9441).AppLauncherLocations.HOME;
        obj2.location = HOME;
