...
// Dependencies: [5, 19, 17, 9322, 1482, 1074, 5013, 21, 4574, 12160, 576, 12084, 5619, 11247, 12135, 7535, 9604, 1114, 1611, 9448, 1894, 12164, 9321, 7537, 12081, 1115, 12166, 4570, 12168, 12132, 12170, 9320, 1178, 12177, 12179, 4541, 4754, 9605, 2]
// Exports: default

// Module 12159 (AppDetailContent)
...
import CommandRowButtonDefault from "CommandRowButton" /* 12135 */;
...
export default function AppDetailContent(context) {
  ...
  const memo1 = onCommandExecuted.useMemo(() => {
    ...
    if (isAndroidResult) {
      isAndroidResult = tmp(9321).isApplicationMonetizedWithIAP(application);
      const tmpResult = tmp(9321);
    }
    ...
    if (result) {
      ...
      const items = [map1(tmp(12166).BillIcon, { size: "sm", color: "icon-muted" }), ];
      ...
      items[1] = map1(tmp(4570).Text, obj3);
      obj2.children = items;
    ...
    if (isAndroidResult) {
      ...
      const items1 = [map1(tmp(12168).ShopIcon, { size: "sm", color: "icon-muted" }), ];
      ...
      items1[1] = map1(tmp(4570).Text, obj5);
