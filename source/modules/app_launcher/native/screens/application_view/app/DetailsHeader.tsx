...
// Function ID: 9432
// Name: DetailsHeader
// Dependencies: [32, 19, 17, 21, 4638, 576, 4374, 9432, 9562, 5211, 8402, 5672, 4634, 1115, 4639, 4642, 5750, 5072, 1094, 2]

// Module 9431 (DetailsHeader)
...
import BioMarkupUtils from "BioMarkupUtils" /* 9562 */;
...
export default noop.memo(function DetailsHeader(viewContainerStyle) {
  ...
  const isPartnerApplicationResult = sharedValue(9432).isPartnerApplication(application);
  sharedValue = sharedValue(4374).useSharedValue(null);
  let obj3 = sharedValue(4374);
  let fn = function t() {
  ...
  const animatedStyle = sharedValue(4374).useAnimatedStyle(fn);
  const obj4 = sharedValue(4374);
  const sectionName = sharedValue(9432).getSectionName(application);
  const obj5 = sharedValue(9432);
  const str = sharedValue(9432).getSectionDescription(application);
  ...
  isScreenLandscape = sharedValue(5211).useIsScreenLandscape();
  const tmp24 = first(8402)(isScreenLandscape);
  closure_18 = tmp24;
  ...
  let tmp27 = first;
  const tmp2Result = sharedValue(5211);
  if (first) {
    tmp27 = !first1;
  }
  ...
  if (!hideName) {
    ...
    tmp30Result = tmp30(tmp2(4634).Heading, obj10);
  }
  ...
  if (isPartnerApplicationResult) {
    ...
    obj11.children = tmp30(tmp2(4634).Text, obj12);
  ...
  if (tmp28Result5) {
    ...
    if (tmp27) {
      ...
      items8[1] = tmp30(tmp23(5072), obj19);
      if (num2 == null) {
        num2 = 0;
      }
    ...
    obj22.children = tmp30(tmp2(4634).Text, obj23);
    const items10 = [tmp30(tmp23(5750), obj22), , ];
    ...
    if (tmp27) {
      ...
      obj24.children = tmp28(tmp2(4634).Text, obj25);
  ...
  if (!first2) {
    ...
    if (!hideName) {
      ...
      tmp30Result9 = tmp30(tmp2(4634).Heading, obj28);
    }
    ...
    if (tmp28Result5) {
      ...
      const items15 = [tmp30(tmp2(4634).Text, obj30), ];
      ...
      items15[1] = tmp30(tmp2(4634).Text, obj31);
