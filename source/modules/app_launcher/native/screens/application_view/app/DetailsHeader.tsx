...
// Function ID: 9421
// Name: DetailsHeader
// Dependencies: [32, 19, 17, 21, 4639, 576, 4376, 9421, 9551, 5210, 8393, 5671, 4635, 1115, 4640, 4643, 5749, 5071, 1094, 2]

// Module 9420 (DetailsHeader)
...
export default noop.memo(function DetailsHeader(viewContainerStyle) {
  ...
  const isPartnerApplicationResult = sharedValue(9421).isPartnerApplication(application);
  sharedValue = sharedValue(4376).useSharedValue(null);
  let obj3 = sharedValue(4376);
  let fn = function t() {
  ...
  const obj4 = sharedValue(4376);
  const sectionName = sharedValue(9421).getSectionName(application);
  const obj5 = sharedValue(9421);
  const str = sharedValue(9421).getSectionDescription(application);
  ...
  isScreenLandscape = sharedValue(5210).useIsScreenLandscape();
  const tmp24 = first(8393)(isScreenLandscape);
  closure_18 = tmp24;
  ...
  let tmp27 = first;
  const tmp2Result = sharedValue(5210);
  if (first) {
    tmp27 = !first1;
  }
  ...
  if (!hideName) {
    ...
    tmp30Result = tmp30(tmp2(4635).Heading, obj10);
  }
  ...
  if (isPartnerApplicationResult) {
    ...
    obj11.children = tmp30(tmp2(4635).Text, obj12);
  ...
  if (tmp28Result5) {
    ...
    if (tmp27) {
      ...
      items8[1] = tmp30(tmp23(5071), obj19);
      if (num2 == null) {
        num2 = 0;
      }
    ...
    obj22.children = tmp30(tmp2(4635).Text, obj23);
    const items10 = [tmp30(tmp23(5749), obj22), , ];
    ...
    if (tmp27) {
      ...
      obj24.children = tmp28(tmp2(4635).Text, obj25);
  ...
  if (!first2) {
    ...
    if (!hideName) {
      ...
      tmp30Result9 = tmp30(tmp2(4635).Heading, obj28);
    }
    ...
    if (tmp28Result5) {
      ...
      const items15 = [tmp30(tmp2(4635).Text, obj30), ];
      ...
      items15[1] = tmp30(tmp2(4635).Text, obj31);
