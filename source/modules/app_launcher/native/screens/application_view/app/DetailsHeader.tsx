...
// Function ID: 9321
// Name: DetailsHeader
// Dependencies: [32, 19, 17, 21, 4574, 576, 4310, 9321, 9451, 5140, 8300, 5600, 4570, 1114, 4575, 4578, 5678, 5001, 1093, 2]

// Module 9320 (DetailsHeader)
import nativeDefault from "native" /* 576 */;
import ReanimatedRexport from "ReanimatedRexport" /* 4310 */;
import timing from "timing" /* 4575 */;
import timingPresets from "timingPresets" /* 4578 */;
import BioMarkupUtils from "BioMarkupUtils" /* 9451 */;
...
export default noop.memo(function DetailsHeader(viewContainerStyle) {
  ...
  const isPartnerApplicationResult = sharedValue(9321).isPartnerApplication(application);
  sharedValue = sharedValue(4310).useSharedValue(null);
  let obj3 = sharedValue(4310);
  let fn = function t() {
  ...
  const obj4 = sharedValue(4310);
  const sectionName = sharedValue(9321).getSectionName(application);
  const obj5 = sharedValue(9321);
  const str = sharedValue(9321).getSectionDescription(application);
  ...
  isScreenLandscape = sharedValue(5140).useIsScreenLandscape();
  const tmp24 = first(8300)(isScreenLandscape);
  closure_18 = tmp24;
  ...
  let tmp27 = first;
  const tmp2Result = sharedValue(5140);
  if (first) {
    tmp27 = !first1;
  }
  ...
  if (!hideName) {
    ...
    tmp30Result = tmp30(tmp2(4570).Heading, obj10);
  }
  ...
  if (isPartnerApplicationResult) {
    ...
    obj12.children = intl.string(tmp2(1114).t.LO4f0P);
    obj11.children = tmp30(tmp2(4570).Text, obj12);
  ...
  if (tmp28Result5) {
    ...
    if (tmp27) {
      ...
      const obj19 = { start: tmp2(1093).HorizontalGradient.START, end: tmp2(1093).HorizontalGradient.END, colors, style: tmp.maskFade };
      items8[1] = tmp30(tmp23(5001), obj19);
      if (num2 == null) {
        num2 = 0;
      }
    ...
    obj22.children = tmp30(tmp2(4570).Text, obj23);
    const items10 = [tmp30(tmp23(5678), obj22), , ];
    ...
    if (tmp27) {
      ...
      obj24.children = tmp28(tmp2(4570).Text, obj25);
    ...
    if (first) {
      ...
      if (first1) {
        ...
        obj26.children = intl3.string(tmp2(1114).t.D5xGUK);
        tmp30Result8 = tmp30(tmp2(4570).Text, obj26);
      }
    }
    items10[2] = tmp30Result8;
    obj13.children = items10;
    tmp28Result4 = tmp28(sharedValue, obj13);
    const tmp23Result = tmp23(5678);
  }
  items5[1] = tmp28Result4;
  obj8.children = items5;
  obj7.children = closure_9(closure_5, obj8);
  const children = [num2(first(4310).View, obj7), ];
  ...
  if (!first2) {
    ...
    if (!hideName) {
      ...
      tmp30Result9 = tmp30(tmp2(4570).Heading, obj28);
    }
    ...
    if (tmp28Result5) {
      ...
      const items15 = [tmp30(tmp2(4570).Text, obj30), ];
      const obj31 = { variant: "text-sm/medium", color: "text-brand", style: tmp.collapseDescriptionCTA, children: null };
      const intl4 = tmp2(1114).intl;
      obj31.children = intl4.string(tmp2(1114).t.D5xGUK);
      items15[1] = tmp30(tmp2(4570).Text, obj31);
