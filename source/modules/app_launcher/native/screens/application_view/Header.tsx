...
// Dependencies: [19, 17, 1372, 9551, 1483, 1074, 21, 576, 4638, 4374, 504, 12193, 4339, 8261, 12198, 9432, 8995, 12273, 4634, 1177, 8214, 4581, 1241, 7299, 12274, 4335, 1115, 12275, 2]
// Exports: default

// Module 12272 (application_view/Header)
...
import AppLauncherBackButtonDefault from "AppLauncherBackButton" /* 12273 */;
...
export default function Header(application) {
  ...
  if (null != application) {
    appLauncherIconSource = tmp(12193).getAppLauncherIconSource(application);
    const tmpResult = tmp(12193);
  }
  let obj = application(504);
  let str = application(4339).useToken(nativeDefault.colors.BACKGROUND_BASE_LOW);
  let tmp7 = appLauncherIconSource;
  const tmpResult9 = application(4339);
  ...
  if (null != appLauncherIconSource) {
    ...
    let tmp12 = closure_8(tmp5(12198), obj2);
    let tmp13 = closure_8;
  } else {
  ...
  const tmpResult10 = application(4374);
  fn.__closure = { interpolate: application(4374).interpolate, scrollOffsetY, HEADER_SCROLL_RANGE };
  fn.__workletHash = 2572905048492;
  fn.__initData = __initData;
  closure_130_0 = scrollOffsetY;
  const animatedStyle = tmpResult10.useAnimatedStyle(fn);
  let obj4 = { interpolate: application(4374).interpolate, scrollOffsetY, HEADER_SCROLL_RANGE };
  ...
  const tmpResult11 = application(4374);
  fn2.__closure = { interpolate: application(4374).interpolate, scrollOffsetY, HEADER_SCROLL_RANGE };
  fn2.__workletHash = 8190094903650;
  fn2.__initData = __initData2;
  const animatedStyle1 = tmpResult11.useAnimatedStyle(fn2);
  let obj5 = { interpolate: application(4374).interpolate, scrollOffsetY, HEADER_SCROLL_RANGE };
  ...
  const tmpResult12 = application(4374);
  fn3.__closure = { interpolate: application(4374).interpolate, scrollOffsetY, HEADER_SCROLL_RANGE };
  fn3.__workletHash = 14190901941859;
  fn3.__initData = __initData3;
  closure_131_0 = scrollOffsetY;
  const animatedStyle2 = tmpResult12.useAnimatedStyle(fn3);
  let obj6 = { interpolate: application(4374).interpolate, scrollOffsetY, HEADER_SCROLL_RANGE };
  ...
  const tmpResult13 = application(4374);
  fn4.__closure = { interpolate: application(4374).interpolate, scrollOffsetY, HEADER_SCROLL_RANGE };
  fn4.__workletHash = 9589752719246;
  fn4.__initData = __initData4;
  let str2 = "";
  const animatedStyle3 = tmpResult13.useAnimatedStyle(fn4);
  if (null != application) {
    str2 = tmp(9432).getSectionName(application);
    const tmpResult14 = tmp(9432);
  }
  ...
  if (hasApplicationFlagResult) {
    hasApplicationFlagResult = tmp(8995).hasApplicationFlag(application, constants2.EMBEDDED);
    const tmpResult15 = tmp(8995);
  }
  ...
  const obj7 = { interpolate: application(4374).interpolate, scrollOffsetY, HEADER_SCROLL_RANGE };
  const tmp23 = id;
  items6[2] = tmp13(ReanimatedRexportDefault.View, { style: animatedStyle2, pointerEvents: "none", children: tmp13(application(4634).Heading, { variant: "heading-lg/bold", color: "text-overlay-light", children: str2 }) });
  ...
  if (null != application) {
    ...
    if (tmpResult16.isRealApplication(application)) {
      ...
      const obj15 = {
        ...
        onPress() {
              ...
              if (c2) {
                ...
                const merged = Object.assign(tmp4(9432).getInstallAppProps(tmp2));
                activityLaunchURL = obj4.getApplicationInstallURL(obj6);
                const tmp4Result = tmp4(9432);
              }
      ...
      const items7 = [tmp13(tmp(8214).IconButton, obj15), ];
      const obj16 = { application, onAddAppMenuClick };
      items7[1] = tmp13(tmp5(12275), obj16);
      obj14.children = items7;
      tmp22Result = tmp22(tmp23, obj14);
    }
    tmpResult16 = tmp(9432);
  }
