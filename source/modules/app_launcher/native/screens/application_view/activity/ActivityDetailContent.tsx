...
// Dependencies: [5, 32, 19, 17, 8947, 1918, 1074, 21, 576, 4607, 4603, 11311, 12236, 12152, 12146, 7597, 9381, 1114, 1242, 9502, 5027, 4620, 9641, 9664, 7236, 7256, 11088, 5178, 504, 5654, 7274, 9642, 6678, 7242, 12237, 12238, 12239, 1178, 12179, 9380, 5141, 12241, 2]
// Exports: default

// Module 12235 (ActivityDetailContent)
...
import DeveloperActivityShelfActionCreatorsAll from "DeveloperActivityShelfActionCreators" /* 9642 */;
import roundToNearestPixelDefault from "roundToNearestPixel" /* 11088 */;
import AppLauncherContext from "AppLauncherContext" /* 11311 */;
import useActivityShelfItem from "useActivityShelfItem" /* 12152 */;
...
function TryItOutButton(botUserId) {
  ...
  const callback = noop.useCallback(asyncGeneratorStep(async (arg0, value) => {
    tmp3(1242).track(constants.APP_DETAIL_PAGE_ENTRY_POINT_COMMAND_BUTTON_CLICKED, { application_id: applicationId, button_action: botUserId(9502).EntryPointCommandButtonActions.OPEN_APP_DM });
    const _setTimeout = setTimeout;
    closure_4.current = setTimeout(() => {
      dependencyMap(true);
    }, 250);
    await tmp3(4620).openPrivateChannel({ recipientIds: botUserId });
    if (1 === tmp7) {
      dependencyMap = 0;
      ...
      if (arg0 === 1) {
        ...
        c4 = 3;
        c5 = 1;
        return { value: tmp3(9641)({ targetApplicationId: closure_129_1, channelId: closure_128_0, analyticsLocations: closure_129_2 }), done: false };
      }
    } else if (arg0 === 1) {
      c5 = 3;
      throw value;
  ...
  return closure_11(botUserId(5027).Button, obj);
}
...
export default function ActivityDetailContent(application) {
  ...
  const tmp = closure_15();
  const width = application(11311).useRequiredAppLauncherContext().width;
  let obj = application(11311);
  const getPrimaryAppCommand = application(9664).useGetPrimaryAppCommand(context, application.id);
  const obj2 = application(9664);
  ...
  const isScreenLandscape = application(5178).useIsScreenLandscape();
  let detailsContainerLandscape = entrypoint !== application(9502).AppLauncherEntrypoint.VOICE && isScreenLandscape;
  const obj3 = application(5178);
  ...
  if (tmp2Result7.isRealApplication(application)) {
    ...
    if (isDeveloperOfThisApp) {
      ...
      obj5.style = obj4;
      const items1 = [closure_11(tmp2(5654).TableRowDivider, {}), , ];
      ...
      items1[1] = closure_11(tmp2(7274).TableSwitchRow, obj6);
      let tmp22Result = null;
      if (useActivityUrlOverride) {
        const obj7 = { style: tmp.activityUrlOverrideInputContainer, children: null };
        const obj8 = { placeholder: "e.g. http://192.168.1.1:3000", value: activityUrlOverride, onChange: tmp23(9642).setActivityUrlOverride };
        obj7.children = tmp22(tmp2(6678).TextInput, obj8);
    ...
    const getOrFetchApplication = tmp2(7242).useGetOrFetchApplication(application.id);
    let bot;
    if (getOrFetchApplication != null) {
      bot = getOrFetchApplication.bot;
    }
    const tmp2Result8 = tmp2(7242);
    const obj9 = { context, applicationId: application.id };
    const activityAction = tmp2(12152).useActivityAction(obj9);
    const tmp2Result9 = tmp2(12152);
    const delayedSwapToActivityActionLeave = tmp2(12237).useDelayedSwapToActivityActionLeave(activityAction);
    const obj10 = { context, application, activityAction: delayedSwapToActivityActionLeave };
    const tmp2Result10 = tmp2(12237);
    ({ reason, disabled } = tmp5(12238)(obj10));
    ...
    if (null != getPrimaryAppCommand) {
      ...
      if (tmp39Result2) {
        ...
        tmp39Result2 = tmp39(tmp2(4603).Text, obj17);
      }
      ...
      if (isDeveloperOfThisApp) {
        isDeveloperOfThisApp = tmp2(9381).isEmbeddedApp(application);
        const tmp2Result11 = tmp2(9381);
      }
      if (isDeveloperOfThisApp) {
    ...
    let result = width;
    const tmp29 = tmp5(12238)(obj10);
    if (detailsContainerLandscape) {
      result = 65 * width / 100;
    }
    obj25.width = result;
    obj25.contentWidth = tmp8;
    obj24.children = closure_11(tmp5(12179), obj25);
    ...
    const items7 = [closure_11(tmp5(9380), obj27), , , , ];
    ...
    obj29.icon = closure_11(tmp2(5141).GroupIcon, obj30);
    const tmp52 = Tag;
    const tmp5Result3 = tmp5(12179);
    let num5 = num;
    if (num == null) {
      num5 = 0;
    }
    obj29.tagName = tmp2(12241).getItemSubtitleForMaxPlayersShort(num5);
    const tmp2Result12 = tmp2(12241);
    if (num == null) {
      num = 0;
    }
    obj29.accessibilityLabel = tmp5(12241)(num);
