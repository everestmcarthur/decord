...
// Dependencies: [5, 32, 19, 17, 8945, 1918, 1074, 21, 576, 4606, 4602, 11309, 12234, 12150, 12144, 7596, 9379, 1114, 1242, 9500, 5026, 4619, 9639, 9662, 7235, 7255, 11086, 5177, 504, 5653, 7273, 9640, 6677, 7241, 12235, 12236, 12237, 1178, 12177, 9378, 5140, 12239, 2]
// Exports: default

// Module 12233 (ActivityDetailContent)
...
import DeveloperActivityShelfActionCreatorsAll from "DeveloperActivityShelfActionCreators" /* 9640 */;
import roundToNearestPixelDefault from "roundToNearestPixel" /* 11086 */;
import AppLauncherContext from "AppLauncherContext" /* 11309 */;
import useActivityShelfItem from "useActivityShelfItem" /* 12150 */;
...
function TryItOutButton(botUserId) {
  ...
  const callback = noop.useCallback(asyncGeneratorStep(async (arg0, value) => {
    tmp3(1242).track(constants.APP_DETAIL_PAGE_ENTRY_POINT_COMMAND_BUTTON_CLICKED, { application_id: applicationId, button_action: botUserId(9500).EntryPointCommandButtonActions.OPEN_APP_DM });
    const _setTimeout = setTimeout;
    closure_4.current = setTimeout(() => {
      dependencyMap(true);
    }, 250);
    await tmp3(4619).openPrivateChannel({ recipientIds: botUserId });
    if (1 === tmp7) {
      dependencyMap = 0;
      ...
      if (arg0 === 1) {
        ...
        c4 = 3;
        c5 = 1;
        return { value: tmp3(9639)({ targetApplicationId: closure_129_1, channelId: closure_128_0, analyticsLocations: closure_129_2 }), done: false };
      }
    } else if (arg0 === 1) {
      c5 = 3;
      throw value;
  ...
  return closure_11(botUserId(5026).Button, obj);
}
...
export default function ActivityDetailContent(application) {
  ...
  const tmp = closure_15();
  const width = application(11309).useRequiredAppLauncherContext().width;
  let obj = application(11309);
  const getPrimaryAppCommand = application(9662).useGetPrimaryAppCommand(context, application.id);
  const obj2 = application(9662);
  ...
  const isScreenLandscape = application(5177).useIsScreenLandscape();
  let detailsContainerLandscape = entrypoint !== application(9500).AppLauncherEntrypoint.VOICE && isScreenLandscape;
  const obj3 = application(5177);
  ...
  if (tmp2Result7.isRealApplication(application)) {
    ...
    if (isDeveloperOfThisApp) {
      ...
      obj5.style = obj4;
      const items1 = [closure_11(tmp2(5653).TableRowDivider, {}), , ];
      ...
      items1[1] = closure_11(tmp2(7273).TableSwitchRow, obj6);
      let tmp22Result = null;
      if (useActivityUrlOverride) {
        const obj7 = { style: tmp.activityUrlOverrideInputContainer, children: null };
        const obj8 = { placeholder: "e.g. http://192.168.1.1:3000", value: activityUrlOverride, onChange: tmp23(9640).setActivityUrlOverride };
        obj7.children = tmp22(tmp2(6677).TextInput, obj8);
    ...
    const getOrFetchApplication = tmp2(7241).useGetOrFetchApplication(application.id);
    let bot;
    if (getOrFetchApplication != null) {
      bot = getOrFetchApplication.bot;
    }
    const tmp2Result8 = tmp2(7241);
    const obj9 = { context, applicationId: application.id };
    const activityAction = tmp2(12150).useActivityAction(obj9);
    const tmp2Result9 = tmp2(12150);
    const delayedSwapToActivityActionLeave = tmp2(12235).useDelayedSwapToActivityActionLeave(activityAction);
    const obj10 = { context, application, activityAction: delayedSwapToActivityActionLeave };
    const tmp2Result10 = tmp2(12235);
    ({ reason, disabled } = tmp5(12236)(obj10));
    ...
    if (null != getPrimaryAppCommand) {
      ...
      if (tmp39Result2) {
        ...
        tmp39Result2 = tmp39(tmp2(4602).Text, obj17);
      }
      ...
      if (isDeveloperOfThisApp) {
        isDeveloperOfThisApp = tmp2(9379).isEmbeddedApp(application);
        const tmp2Result11 = tmp2(9379);
      }
      if (isDeveloperOfThisApp) {
    ...
    let result = width;
    const tmp29 = tmp5(12236)(obj10);
    if (detailsContainerLandscape) {
      result = 65 * width / 100;
    }
    obj25.width = result;
    obj25.contentWidth = tmp8;
    obj24.children = closure_11(tmp5(12177), obj25);
    ...
    const items7 = [closure_11(tmp5(9378), obj27), , , , ];
    ...
    obj29.icon = closure_11(tmp2(5140).GroupIcon, obj30);
    const tmp52 = Tag;
    const tmp5Result3 = tmp5(12177);
    let num5 = num;
    if (num == null) {
      num5 = 0;
    }
    obj29.tagName = tmp2(12239).getItemSubtitleForMaxPlayersShort(num5);
    const tmp2Result12 = tmp2(12239);
    if (num == null) {
      num = 0;
    }
    obj29.accessibilityLabel = tmp5(12239)(num);
