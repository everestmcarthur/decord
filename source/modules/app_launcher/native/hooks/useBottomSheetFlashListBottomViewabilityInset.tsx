...
// Dependencies: [19, 1477, 11247, 11388, 12, 4310, 2]
// Exports: useBottomSheetFlashListBottomViewabilityInset

// Module 12128 (useBottomSheetFlashListBottomViewabilityInset)
...
export const useBottomSheetFlashListBottomViewabilityInset = function useBottomSheetFlashListBottomViewabilityInset() {
  const context = bottomVisibilityInsetRef.useContext(bottomSheetPosition(11247).AppLauncherContext);
  bottomSheetPosition = undefined;
  if (context != null) {
    bottomSheetPosition = context.bottomSheetPosition;
  }
  const flashListRef = obj.useRef(null);
  const diff = flashListRef(1477)().height - flashListRef(11388)().maximum;
  dependencyMap = diff;
  ...
  const tmp4Result = bottomSheetPosition(4310);
  fn2.__closure = { distanceBetweenExpandedScreenTopAndSheetTop: diff, runOnJS: bottomSheetPosition(4310).runOnJS, handleBottomViewabilityInsetDebounced: memo };
