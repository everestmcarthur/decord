...
// Dependencies: [32, 19, 4631, 2016, 1372, 1085, 21, 4639, 576, 504, 12313, 11053, 12314, 1177, 12315, 12316, 12310, 4606, 12314, 1897, 2]
// Exports: default

// Module 12312 (AppLauncherMentionableOption)
import initialize from "initialize" /* 504 */;
import nativeDefault from "native" /* 576 */;
import asyncRequireImpl from "asyncRequireImpl" /* 1897 */;
import ActionSheetActionCreatorsDefault from "ActionSheetActionCreators" /* 4606 */;
import UserCircleIcon from "UserCircleIcon" /* 11053 */;
import AppLauncherOptionIconDefault from "AppLauncherOptionIcon" /* 12313 */;
import AppLauncherMentionableListActionSheet from "AppLauncherMentionableListActionSheet" /* 12314 */;
import UsernameTextDefault from "UsernameText" /* 12316 */;
...
function MentionableIcon(mentionable) {
  ...
  if (null == mentionable) {
    ...
    if (tmp2(12314).MentionableItemTypes.USER === type) {
    ...
    } else if (tmp2(12314).MentionableItemTypes.ROLE === type) {
      const obj4 = { role: mentionable.result };
      return tmp5(tmp2(12315).RoleIcon, obj4);
    } else {
      const GLOBAL = tmp2(12314).MentionableItemTypes.GLOBAL;
      return tmp7;
    }
  }
}
...
export default function AppLauncherMentionableOption(option) {
  ...
  const memo = channel.useMemo(() => {
    if (null == first) {
      ...
      } else if (tmp5(12314).MentionableItemTypes.ROLE === type) {
        return tmp.result.name;
      } else if (tmp5(12314).MentionableItemTypes.GLOBAL === type) {
  ...
  obj.onPress = function onPress() {
    ...
    obj.openLazy(asyncRequireImpl(12314, dependencyMap.paths), AppLauncherMentionableListActionSheet.APP_LAUNCHER_MENTIONABLE_LIST_ACTION_SHEET_KEY, {
      option,
      channel,
