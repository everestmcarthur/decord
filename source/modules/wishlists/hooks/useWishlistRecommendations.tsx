...
// Dependencies: [32, 19, 7735, 502, 10947, 7341, 1091, 504, 8932, 1370, 8939, 12, 8940, 8335, 10948, 10949, 2]
// Exports: useRecommendationsForApplicationIds, useRecommendationsForSingleUser, useWishlistRecommendationsForSingleUser

// Module 10946 (useWishlistRecommendations)
...
import WishlistRecommendationsStore from "WishlistRecommendationsStore" /* 10947 */;
import WishlistRecommendationRecord from "WishlistRecommendationRecord" /* 7341 */;
...
let closure_8 = fn(7341).WishlistRecommendationReason;
...
export const useWishlistRecommendationsForSingleUser = function useWishlistRecommendationsForSingleUser(arg0) {
  ...
  if (source === undefined) {
    source = userId(8932).WishlistFetchSource.USER_PROFILE;
  }
  const items = [userId];
  ...
  obj2.applicationIds = userId(10948).useWishlistApplicationIds(userId);
  obj2.numItems = numItems;
  obj2.source = source;
  const obj3 = userId(10948);
  ...
  const wishlistSkuFilter = userId(10949).useWishlistSkuFilter({ wishlistAndRecommendations, skusToUserAndReason, userId, numItems });
...
export const useRecommendationsForSingleUser = function useRecommendationsForSingleUser(source) {
  ...
  if (USER_PROFILE === undefined) {
    USER_PROFILE = userId(8932).WishlistFetchSource.USER_PROFILE;
  }
  ...
  obj.applicationIds = userId(10948).useWishlistApplicationIds(userId);
  obj.numItems = numItems;
  obj.source = USER_PROFILE;
