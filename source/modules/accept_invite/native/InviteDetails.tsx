...
// Function ID: 12814
// Name: InviteDetails
// Dependencies: [32, 19, 17, 12508, 1385, 1371, 1074, 12814, 21, 4607, 576, 5493, 7808, 1178, 12754, 4451, 1114, 5642, 4603, 1396, 5639, 1242, 504, 12815, 9865, 5027, 5485, 2]
// Exports: default

// Module 12813 (InviteDetails)
...
import GuildBadgeDefault from "GuildBadge" /* 5642 */;
import InviteTypeUtils from "InviteTypeUtils" /* 7808 */;
import GuildInviteIconDefault from "GuildInviteIcon" /* 12754 */;
import InviteRolesListDefault from "InviteRolesList" /* 12815 */;
...
function InviteHeader(invite) {
  ...
  if (tmp2Result.isFriendInvite(invite)) {
    ...
    name = intl.formatToPlainString(tmp2(1114).t["4aF92R"], obj3);
  }
  let tmp9 = null;
  if (null != name) {
    ...
    items[1] = closure_1_12(tmp2(4603).Heading, obj7);
    obj4.children = items;
    tmp9 = map1(View, obj4);
  }
  return tmp9;
...
function InviteJoinContext(invite) {
  ...
  if (obj.isFriendInvite(invite)) {
    ...
    let obj2 = { username: isGuildMember(4451).getFormattedName(invite.inviter) };
    let formatResult = intl7.format(tmp3(1114).t.Quj7HX, obj2);
    const obj11 = isGuildMember(4451);
  } else {
    if (tmp3Result.isGroupDMInvite(invite)) {
      if (null != invite.channel) {
        if (null != invite.inviter) {
          const intl6 = tmp3(1114).intl;
          let obj3 = { username: isGuildMember(4451).getFormattedName(invite.inviter) };
          let formatResult1 = intl6.format(tmp3(1114).t.Lu4h18, obj3);
          const obj9 = isGuildMember(4451);
        }
      ...
      if (tmp3Result3.isStreamInvite(invite)) {
        if (null != invite.target_user) {
          ...
          const obj4 = { username: isGuildMember(4451).getFormattedName(invite.target_user) };
          formatResult = intl4.formatToPlainString(tmp3(1114).t.x2L32Q, obj4);
          const obj7 = isGuildMember(4451);
        }
      }
      ...
      if (tmp7) {
        ...
        if (tmp10) {
          ...
          const obj6 = { username: isGuildMember(4451).getFormattedName(invite.inviter) };
          formatResult = intl2.format(tmp3(1114).t.spU2mI, obj6);
          const obj5 = isGuildMember(4451);
        }
      }
      tmp3Result3 = tmp3(7808);
    }
    tmp3Result = tmp3(7808);
  }
  ...
  if (null != memo) {
    ...
    if (!tmp3Result4.isFriendInvite(invite)) {
      ...
      obj10.children = closure_12(isGuildMember(5639), obj12);
      tmp22 = closure_12(tmp21, obj10);
    }
    tmp3Result4 = tmp3(7808);
  }
  const items1 = [tmp22, closure_12(invite(4603).Text, { style: tmp.inviteJoinText, variant: "text-sm/normal", color: "text-default", children: formatResult })];
...
let closure_11 = fn(12814).INVITE_ROUTING_HUB_GUILD_ID;
const jsxProd = fn(21);
...
obj2.featureIcon = { flexGrow: 0, marginRight: 8, opacity: fn(5493).DARK_1_LIGHT_04 };
...
let obj4 = { flexGrow: 0, marginRight: 8, opacity: fn(5493).DARK_1_LIGHT_04 };
...
export default function InviteDetails(invite) {
  ...
  if (tmp8Result) {
    ...
    const items3 = [tmp10(tmp4(9865).GuildEventCardHeader, obj10), , ];
    const obj11 = { event: guild_scheduled_event };
    items3[1] = tmp10(tmp4(9865).GuildEventCardMetaInfo, obj11);
    const obj12 = { event: guild_scheduled_event };
    items3[2] = tmp10(tmp4(9865).GuildEventCardGuildInfo, obj12);
  ...
  if (null != userAvatarSource) {
    ...
    tmp10Result = tmp10(tmp4(5027).Button.Icon, obj13);
  }
  if (null != stateFromStores) {
  ...
  const items4 = [closure_12(invite(5027).Button, { icon: tmp10Result, variant: "primary", size: "lg", text: tmp24, accessibilityLabel: stringResult, onPress: handleAcceptInvitePress, loading: tmp3 === constants3.ACCEPT, disabled: tmp3 === constants3.ACCEPT }), ];
  ...
  items4[1] = closure_12(invite(5027).Button, {
    variant: "secondary",
    size: "lg",
    text: stringResult1,
  ...
  items2[6] = closure_13(invite(5485).ButtonGroup, obj17);
