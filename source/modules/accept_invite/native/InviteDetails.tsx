...
// Function ID: 12854
// Name: InviteDetails
// Dependencies: [32, 19, 17, 12548, 1386, 1372, 1074, 12854, 21, 4639, 576, 5526, 7840, 1177, 12794, 4484, 1115, 5675, 4635, 1397, 5672, 1241, 504, 12855, 9904, 5059, 5518, 2]
// Exports: default

// Module 12853 (InviteDetails)
...
import GuildBadgeDefault from "GuildBadge" /* 5675 */;
import InviteTypeUtils from "InviteTypeUtils" /* 7840 */;
import GuildInviteIconDefault from "GuildInviteIcon" /* 12794 */;
import InviteRolesListDefault from "InviteRolesList" /* 12855 */;
...
function InviteHeader(invite) {
  ...
  if (null != name) {
    ...
    items[1] = closure_1_12(tmp2(4635).Heading, obj7);
    obj4.children = items;
    tmp9 = map1(View, obj4);
  }
  return tmp9;
...
function InviteJoinContext(invite) {
  ...
  if (obj.isFriendInvite(invite)) {
    ...
    let obj2 = { username: isGuildMember(4484).getFormattedName(invite.inviter) };
    let formatResult = intl7.format(tmp3(1115).t.Quj7HX, obj2);
    const obj11 = isGuildMember(4484);
  } else {
    if (tmp3Result.isGroupDMInvite(invite)) {
      if (null != invite.channel) {
        if (null != invite.inviter) {
          const intl6 = tmp3(1115).intl;
          let obj3 = { username: isGuildMember(4484).getFormattedName(invite.inviter) };
          let formatResult1 = intl6.format(tmp3(1115).t.Lu4h18, obj3);
          const obj9 = isGuildMember(4484);
        }
      ...
      if (tmp3Result3.isStreamInvite(invite)) {
        if (null != invite.target_user) {
          ...
          const obj4 = { username: isGuildMember(4484).getFormattedName(invite.target_user) };
          formatResult = intl4.formatToPlainString(tmp3(1115).t.x2L32Q, obj4);
          const obj7 = isGuildMember(4484);
        }
      }
      ...
      if (tmp7) {
        ...
        if (tmp10) {
          ...
          const obj6 = { username: isGuildMember(4484).getFormattedName(invite.inviter) };
          formatResult = intl2.format(tmp3(1115).t.spU2mI, obj6);
          const obj5 = isGuildMember(4484);
        }
      }
      tmp3Result3 = tmp3(7840);
    }
    tmp3Result = tmp3(7840);
  }
  ...
  if (null != memo) {
    ...
    if (!tmp3Result4.isFriendInvite(invite)) {
      ...
      obj10.children = closure_12(isGuildMember(5672), obj12);
      tmp22 = closure_12(tmp21, obj10);
    }
    tmp3Result4 = tmp3(7840);
  }
  const items1 = [tmp22, closure_12(invite(4635).Text, { style: tmp.inviteJoinText, variant: "text-sm/normal", color: "text-default", children: formatResult })];
...
let closure_11 = fn(12854).INVITE_ROUTING_HUB_GUILD_ID;
const jsxProd = fn(21);
...
obj2.featureIcon = { flexGrow: 0, marginRight: 8, opacity: fn(5526).DARK_1_LIGHT_04 };
...
let obj4 = { flexGrow: 0, marginRight: 8, opacity: fn(5526).DARK_1_LIGHT_04 };
...
export default function InviteDetails(invite) {
  ...
  if (tmp8Result) {
    ...
    const items3 = [tmp10(tmp4(9904).GuildEventCardHeader, obj10), , ];
    const obj11 = { event: guild_scheduled_event };
    items3[1] = tmp10(tmp4(9904).GuildEventCardMetaInfo, obj11);
    const obj12 = { event: guild_scheduled_event };
    items3[2] = tmp10(tmp4(9904).GuildEventCardGuildInfo, obj12);
  ...
  if (null != userAvatarSource) {
    ...
    tmp10Result = tmp10(tmp4(5059).Button.Icon, obj13);
  }
  if (null != stateFromStores) {
  ...
  const items4 = [closure_12(invite(5059).Button, { icon: tmp10Result, variant: "primary", size: "lg", text: tmp24, accessibilityLabel: stringResult, onPress: handleAcceptInvitePress, loading: tmp3 === constants3.ACCEPT, disabled: tmp3 === constants3.ACCEPT }), ];
  ...
  items4[1] = closure_12(invite(5059).Button, {
    variant: "secondary",
    size: "lg",
    text: stringResult1,
  ...
  items2[6] = closure_13(invite(5518).ButtonGroup, obj17);
