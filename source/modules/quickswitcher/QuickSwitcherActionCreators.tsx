...
// Dependencies: [5, 32, 5443, 1961, 1957, 7397, 2011, 4381, 9839, 1074, 1964, 1076, 9840, 1242, 8376, 573, 7342, 4571, 4761, 5411, 4573, 10992, 8685, 8691, 7244, 10993, 7541, 7182, 10994, 8378, 2]
// Exports: hide, search, selectResult, switchToResultInNewTab, toggle, trackOpen

// Module 10991 (getQuickSwitcherOptions)
import dispatcherDefault from "dispatcher" /* 573 */;
import expandEventPropertiesDefault from "expandEventProperties" /* 1242 */;
import setDefault from "set" /* 8376 */;
import sortByMatchScore from "sortByMatchScore" /* 9840 */;
import closure_3 from "asyncGeneratorStep" /* 5 */;
import closure_4 from "_slicedToArray" /* 32 */;
import { InAppNavigationType } from "fromType" /* 5443 */;
import { ChannelRecordBase } from "createChannelRecord" /* 1961 */;
import closure_7 from "ensureGuildLoaded" /* 1957 */;
import closure_8 from "setLibraryApplications" /* 7397 */;
import closure_9 from "handleConnectionOpen" /* 2011 */;
import closure_10 from "handleConnectionOpen" /* 4381 */;
import closure_11 from "handleConnectionOpen" /* 9839 */;
...
function getQuickSwitcherOptions(str) {
  ...
  if (charAtResult === sortByMatchScore.AutocompleterQuerySymbols.USER) {
    if (charAtResult1 === tmp2(9840).AutocompleterQuerySymbols.USER) {
      const items = [str.slice(2), tmp2(9840).AutocompleterResultTypes.USER_GLOBAL];
      let items1 = items;
    }
...
function trackClose(QUICKSWITCHER_RESULT_SELECTED, type) {
  ...
  if (null == tmp6) {
    ...
    obj[13] = obj.getResultTotals(tmp4(9840).AutocompleterResultTypes.USER);
    obj[14] = obj.getResultTotals(tmp4(9840).AutocompleterResultTypes.TEXT_CHANNEL);
    obj[15] = obj.getResultTotals(tmp4(9840).AutocompleterResultTypes.VOICE_CHANNEL);
    obj[16] = obj.getResultTotals(tmp4(9840).AutocompleterResultTypes.GUILD);
    obj[17] = obj.getResultTotals(tmp4(9840).AutocompleterResultTypes.GROUP_DM);
    if (null != channelId) {
    ...
    if (null != type) {
      ...
      if (null == type) {
        ...
        if (tmp4(9840).AutocompleterResultTypes.GUILD === type3) {
          obj.selected_guild_id = record.id;
        } else {
          if (tmp4(9840).AutocompleterResultTypes.TEXT_CHANNEL !== type3) {
            if (tmp4(9840).AutocompleterResultTypes.VOICE_CHANNEL !== type3) {
              if (tmp4(9840).AutocompleterResultTypes.GROUP_DM === type3) {
                obj.selected_channel_id = record.id;
              } else if (tmp4(9840).AutocompleterResultTypes.USER === type3) {
      ...
      } else if (type.type === tmp4(9840).AutocompleterResultTypes.IN_APP_NAVIGATION) {
  ...
  } else if (tmp6.type === tmp4(9840).AutocompleterResultTypes.IN_APP_NAVIGATION) {
...
function switchToResult(record) {
  ...
  if (sortByMatchScore.AutocompleterResultTypes.GUILD === type) {
  ...
  } else if (tmp5(9840).AutocompleterResultTypes.TEXT_CHANNEL === type) {
  ...
  } else if (tmp5(9840).AutocompleterResultTypes.VOICE_CHANNEL === type) {
  ...
  } else if (tmp5(9840).AutocompleterResultTypes.USER === type) {
    ...
    tmp(10992).channelListScrollTo(closure_12, authStore.getDMFromUserId(record.id));
    const tmpResult2 = tmp(10992);
  } else if (tmp5(9840).AutocompleterResultTypes.GROUP_DM === type) {
    tmp5(4571).transitionToChannel(record.id, { navigationReplace: true });
    const tmp5Result2 = tmp5(4571);
    tmp(10992).channelListScrollTo(closure_12, record.id);
    const tmpResult3 = tmp(10992);
  } else if (tmp5(9840).AutocompleterResultTypes.APPLICATION === type) {
  ...
  } else if (tmp5(9840).AutocompleterResultTypes.GAME_PROFILE === type) {
    const obj5 = { gameId: null, gameProfileModalChecks: null, source: null };
    obj5[0] = record.id;
    const obj6 = { shouldOpenGameProfile: true, gameId: null };
    obj6[1] = record.id;
    obj5[1] = obj6;
    obj5[2] = tmp5(8691).GameProfileSources.QuickSwitcher;
    tmp(8685).openGameProfileModal(obj5);
    const tmpResult4 = tmp(8685);
  } else if (tmp5(9840).AutocompleterResultTypes.LINK === type) {
  ...
  } else if (tmp5(9840).AutocompleterResultTypes.IN_APP_NAVIGATION === type) {
    if (record.record.type !== InAppNavigationType.SETTINGS) {
      if (record.record.type === tmp37.PLAYGROUND) {
        const tmp5Result3 = tmp5(10993);
        ...
        obj7[1] = items1;
        obj7[2] = tmp(7182).QUICK_SWITCHER;
...
export const switchToResultInNewTab = function switchToResultInNewTab(type) {
  const _require = type;
  type = type.type;
  if (_require(9840).AutocompleterResultTypes.TEXT_CHANNEL !== type) {
