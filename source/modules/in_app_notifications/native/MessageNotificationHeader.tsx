...
// Dependencies: [19, 17, 4566, 4222, 1371, 21, 4574, 576, 4570, 4727, 1094, 5085, 5087, 5042, 504, 2]
  ...
  const memo = noop.useMemo(() => {
    ...
    if (PRIVATE_CHANNEL.has(channel.type)) {
      ...
      if (tmp.type !== tmp3(1094).ChannelTypes.PUBLIC_THREAD) {
        const simpleChannelIconComponent = tmp3(5042).getSimpleChannelIconComponent(tmp);
        const tmp3Result = tmp3(5042);
      }
      if (null == tmp2) {
        const ThreadIcon = tmp3(5087).ThreadIcon;
      }
      tmp3Result2 = tmp3(5085);
...
export default function MessageNotificationHeader(locationTextColor) {
              ...
              if (tmp2(1094).ChannelTypes.GUILD_VOICE !== type) {
                if (tmp2(1094).ChannelTypes.GUILD_STAGE_VOICE !== type) {
                  ...
                  if (null != parentChannel) {
                    channelName1 = tmp2(4727).computeChannelName(parentChannel, UserStore, RelationshipStore);
  ...
  if (tmp17Result) {
    ...
    tmp17Result = tmp17(tmp2(4570).Text, obj5);
  }
