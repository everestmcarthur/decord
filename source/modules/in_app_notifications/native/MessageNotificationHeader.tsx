...
// Dependencies: [19, 17, 4630, 4286, 1372, 21, 4638, 576, 4634, 4791, 1095, 5156, 5158, 5113, 504, 2]
// Exports: SimpleNotificationHeader, default

// Module 10310 (MessageNotificationHeader)
  ...
  const memo = noop.useMemo(() => {
    ...
    if (PRIVATE_CHANNEL.has(channel.type)) {
      ...
      if (tmp.type !== tmp3(1095).ChannelTypes.PUBLIC_THREAD) {
        const simpleChannelIconComponent = tmp3(5113).getSimpleChannelIconComponent(tmp);
        const tmp3Result = tmp3(5113);
      }
      if (null == tmp2) {
        const ThreadIcon = tmp3(5158).ThreadIcon;
      }
      tmp3Result2 = tmp3(5156);
...
export default function MessageNotificationHeader(locationTextColor) {
              ...
              if (tmp2(1095).ChannelTypes.GUILD_VOICE !== type) {
                if (tmp2(1095).ChannelTypes.GUILD_STAGE_VOICE !== type) {
                  ...
                  if (null != parentChannel) {
                    channelName1 = tmp2(4791).computeChannelName(parentChannel, UserStore, RelationshipStore);
  ...
  if (tmp17Result) {
    ...
    tmp17Result = tmp17(tmp2(4634).Text, obj5);
  }
