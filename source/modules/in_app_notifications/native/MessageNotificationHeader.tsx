...
// Dependencies: [19, 17, 4598, 4255, 1371, 21, 4606, 576, 4602, 4759, 1094, 5122, 5124, 5079, 504, 2]
// Exports: SimpleNotificationHeader, default

// Module 10261 (MessageNotificationHeader)
  ...
  const memo = noop.useMemo(() => {
    ...
    if (PRIVATE_CHANNEL.has(channel.type)) {
      ...
      if (tmp.type !== tmp3(1094).ChannelTypes.PUBLIC_THREAD) {
        const simpleChannelIconComponent = tmp3(5079).getSimpleChannelIconComponent(tmp);
        const tmp3Result = tmp3(5079);
      }
      if (null == tmp2) {
        const ThreadIcon = tmp3(5124).ThreadIcon;
      }
      tmp3Result2 = tmp3(5122);
...
export default function MessageNotificationHeader(locationTextColor) {
              ...
              if (tmp2(1094).ChannelTypes.GUILD_VOICE !== type) {
                if (tmp2(1094).ChannelTypes.GUILD_STAGE_VOICE !== type) {
                  ...
                  if (null != parentChannel) {
                    channelName1 = tmp2(4759).computeChannelName(parentChannel, UserStore, RelationshipStore);
  ...
  if (tmp17Result) {
    ...
    tmp17Result = tmp17(tmp2(4602).Text, obj5);
  }
