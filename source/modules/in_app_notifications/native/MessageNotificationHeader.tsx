...
// Dependencies: [19, 17, 4631, 4288, 1372, 21, 4639, 576, 4635, 4792, 1095, 5155, 5157, 5112, 504, 2]
  ...
  const memo = noop.useMemo(() => {
    ...
    if (PRIVATE_CHANNEL.has(channel.type)) {
      ...
      if (tmp.type !== tmp3(1095).ChannelTypes.PUBLIC_THREAD) {
        const simpleChannelIconComponent = tmp3(5112).getSimpleChannelIconComponent(tmp);
        const tmp3Result = tmp3(5112);
      }
      if (null == tmp2) {
        const ThreadIcon = tmp3(5157).ThreadIcon;
      }
      tmp3Result2 = tmp3(5155);
...
export default function MessageNotificationHeader(locationTextColor) {
              ...
              if (tmp2(1095).ChannelTypes.GUILD_VOICE !== type) {
                if (tmp2(1095).ChannelTypes.GUILD_STAGE_VOICE !== type) {
                  ...
                  if (null != parentChannel) {
                    channelName1 = tmp2(4792).computeChannelName(parentChannel, UserStore, RelationshipStore);
  ...
  if (tmp17Result) {
    ...
    tmp17Result = tmp17(tmp2(4635).Text, obj5);
  }
