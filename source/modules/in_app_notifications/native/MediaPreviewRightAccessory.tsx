...
// Dependencies: [19, 17, 4630, 10234, 21, 4638, 576, 8404, 4634, 4339, 1364, 5048, 5166, 7078, 563, 7708, 10313, 10269, 8254, 8395, 1477, 5673, 7402, 7407, 8437, 10314, 1177, 8954, 2]
// Exports: MediaPreviewRightAccessory

// Module 10312 (MediaPreviewRightAccessory)
...
import PlayIcon from "PlayIcon" /* 8404 */;
import common_VideoDefault from "common/Video" /* 8437 */;
import ClipView from "ClipView" /* 8954 */;
import usePreviewableMedia from "usePreviewableMedia" /* 10269 */;
import useContentHarmTypes from "useContentHarmTypes" /* 10313 */;
import StickerDefault from "Sticker" /* 10314 */;
import noop from "module_19" /* 19 */;
import AccessibilityStore from "AccessibilityStore" /* 4630 */;

const ClipViewDefault = ClipView;

const VisualEffectViewDefault = tmp4(5048);
require = fn;
function VideoBadge() {
...
function ObscuredMediaOverlay(isSpoiler) {
  ...
  if (isObscured) {
    let tmp14Result = tmp14(tmp2(5166).ImageWarningIcon, { size: "sm", color: "white" });
  } else {
    const obj5 = { style: tmp.spoilerPill, children: tmp14(tmp2(7078).EyeIcon, { size: "sm", color: "white" }) };
...
function SinglePreviewableMedia(arg0) {
  ...
  if (usePreviewableMedia.PreviewableMediaTypes.VOICE_MESSAGE === type) {
    ...
    if (tmp2(10269).PreviewableMediaTypes.AUDIO !== type) {
      if (tmp2(10269).PreviewableMediaTypes.FILE !== type) {
        if (tmp2(10269).PreviewableMediaTypes.IMAGE !== type) {
          if (tmp2(10269).PreviewableMediaTypes.VIDEO !== type) {
            if (tmp2(10269).PreviewableMediaTypes.GIF === type) {
              ...
              if (null == thumbnail) {
                ...
                const enabledHarmTypesForChannelAndAuthorId = tmp2(7402).getEnabledHarmTypesForChannelAndAuthorId(message.channel_id, id);
                const tmp2Result = tmp2(7402);
                const obj5 = { type: null, media: null };
                const getMediaObscuredReasonFromBitmask = tmp2(7402).getMediaObscuredReasonFromBitmask;
                obj5.type = tmp2(7407).ObscuredMediaTypes.Embed;
                obj5.media = media;
                ...
                url2 = thumbnail.url;
                const tmp2Result5 = tmp2(7402);
              }
            } else if (tmp2(10269).PreviewableMediaTypes.STICKER === type) {
          ...
          if (width > 0) {
            if (null != height) {
              if (height > 0) {
                ...
                const attachmentObscurityProps = tmp2(8254).getAttachmentObscurityProps(obj13);
                const tmp2Result6 = tmp2(8254);
                const attachmentUrl = tmp2(8395).getAttachmentUrl(media);
                const tmp2Result7 = tmp2(8395);
                ...
                let str3;
                if (type === tmp2(10269).PreviewableMediaTypes.VIDEO) {
                  str3 = "png";
                }
                obj14.format = str3;
...
let closure_15 = createStyles.createStyles({ rightAccessoryContainer: { marginLeft: fn(10234).RIGHT_ACCESSORY_LEFT_MARGIN } });
size = fn(2);
