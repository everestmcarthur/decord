...
// Dependencies: [19, 17, 4598, 10185, 21, 4606, 576, 8360, 4602, 4308, 1115, 5016, 5132, 7041, 563, 7672, 10264, 10220, 8210, 8351, 1476, 5638, 7365, 7370, 8393, 10265, 1178, 8905, 2]
// Exports: MediaPreviewRightAccessory

// Module 10263 (MediaPreviewRightAccessory)
...
import PlayIcon from "PlayIcon" /* 8360 */;
...
import useContentHarmTypes from "useContentHarmTypes" /* 10264 */;
import StickerDefault from "Sticker" /* 10265 */;
import noop from "module_19" /* 19 */;
import AccessibilityStore from "AccessibilityStore" /* 4598 */;

const ClipViewDefault = ClipView;

const VisualEffectViewDefault = tmp4(5016);
require = fn;
function VideoBadge() {
...
function ObscuredMediaOverlay(isSpoiler) {
  ...
  if (isObscured) {
    let tmp14Result = tmp14(tmp2(5132).ImageWarningIcon, { size: "sm", color: "white" });
  } else {
    const obj5 = { style: tmp.spoilerPill, children: tmp14(tmp2(7041).EyeIcon, { size: "sm", color: "white" }) };
...
function SinglePreviewableMedia(arg0) {
  ...
  if (usePreviewableMedia.PreviewableMediaTypes.VOICE_MESSAGE === type) {
    ...
    if (tmp2(10220).PreviewableMediaTypes.AUDIO !== type) {
      if (tmp2(10220).PreviewableMediaTypes.FILE !== type) {
        if (tmp2(10220).PreviewableMediaTypes.IMAGE !== type) {
          if (tmp2(10220).PreviewableMediaTypes.VIDEO !== type) {
            if (tmp2(10220).PreviewableMediaTypes.GIF === type) {
              ...
              if (null == thumbnail) {
                ...
                const enabledHarmTypesForChannelAndAuthorId = tmp2(7365).getEnabledHarmTypesForChannelAndAuthorId(message.channel_id, id);
                const tmp2Result = tmp2(7365);
                const obj5 = { type: null, media: null };
                const getMediaObscuredReasonFromBitmask = tmp2(7365).getMediaObscuredReasonFromBitmask;
                obj5.type = tmp2(7370).ObscuredMediaTypes.Embed;
                obj5.media = media;
                ...
                url2 = thumbnail.url;
                const tmp2Result5 = tmp2(7365);
              }
            } else if (tmp2(10220).PreviewableMediaTypes.STICKER === type) {
          ...
          if (width > 0) {
            if (null != height) {
              if (height > 0) {
                ...
                const attachmentObscurityProps = tmp2(8210).getAttachmentObscurityProps(obj13);
                const tmp2Result6 = tmp2(8210);
                const attachmentUrl = tmp2(8351).getAttachmentUrl(media);
                const tmp2Result7 = tmp2(8351);
                ...
                let str3;
                if (type === tmp2(10220).PreviewableMediaTypes.VIDEO) {
                  str3 = "png";
                }
                obj14.format = str3;
...
let closure_15 = createStyles.createStyles({ rightAccessoryContainer: { marginLeft: fn(10185).RIGHT_ACCESSORY_LEFT_MARGIN } });
size = fn(2);
