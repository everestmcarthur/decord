...
// Dependencies: [19, 17, 4631, 10226, 21, 4639, 576, 8395, 4635, 4341, 1364, 5049, 5165, 7075, 563, 7705, 10305, 10261, 8245, 8386, 1477, 5672, 7399, 7404, 8428, 10306, 1177, 8943, 2]
// Exports: MediaPreviewRightAccessory

// Module 10304 (MediaPreviewRightAccessory)
...
import PlayIcon from "PlayIcon" /* 8395 */;
import common_VideoDefault from "common/Video" /* 8428 */;
import ClipView from "ClipView" /* 8943 */;
import usePreviewableMedia from "usePreviewableMedia" /* 10261 */;
import useContentHarmTypes from "useContentHarmTypes" /* 10305 */;
import StickerDefault from "Sticker" /* 10306 */;
import noop from "module_19" /* 19 */;
import AccessibilityStore from "AccessibilityStore" /* 4631 */;

const ClipViewDefault = ClipView;

const VisualEffectViewDefault = tmp4(5049);
require = fn;
function VideoBadge() {
...
function ObscuredMediaOverlay(isSpoiler) {
  ...
  if (isObscured) {
    let tmp14Result = tmp14(tmp2(5165).ImageWarningIcon, { size: "sm", color: "white" });
  } else {
    const obj5 = { style: tmp.spoilerPill, children: tmp14(tmp2(7075).EyeIcon, { size: "sm", color: "white" }) };
...
function SinglePreviewableMedia(arg0) {
  ...
  if (usePreviewableMedia.PreviewableMediaTypes.VOICE_MESSAGE === type) {
    ...
    if (tmp2(10261).PreviewableMediaTypes.AUDIO !== type) {
      if (tmp2(10261).PreviewableMediaTypes.FILE !== type) {
        if (tmp2(10261).PreviewableMediaTypes.IMAGE !== type) {
          if (tmp2(10261).PreviewableMediaTypes.VIDEO !== type) {
            if (tmp2(10261).PreviewableMediaTypes.GIF === type) {
              ...
              if (null == thumbnail) {
                ...
                const enabledHarmTypesForChannelAndAuthorId = tmp2(7399).getEnabledHarmTypesForChannelAndAuthorId(message.channel_id, id);
                const tmp2Result = tmp2(7399);
                const obj5 = { type: null, media: null };
                const getMediaObscuredReasonFromBitmask = tmp2(7399).getMediaObscuredReasonFromBitmask;
                obj5.type = tmp2(7404).ObscuredMediaTypes.Embed;
                obj5.media = media;
                ...
                url2 = thumbnail.url;
                const tmp2Result5 = tmp2(7399);
              }
            } else if (tmp2(10261).PreviewableMediaTypes.STICKER === type) {
          ...
          if (width > 0) {
            if (null != height) {
              if (height > 0) {
                ...
                const attachmentObscurityProps = tmp2(8245).getAttachmentObscurityProps(obj13);
                const tmp2Result6 = tmp2(8245);
                const attachmentUrl = tmp2(8386).getAttachmentUrl(media);
                const tmp2Result7 = tmp2(8386);
                ...
                let str3;
                if (type === tmp2(10261).PreviewableMediaTypes.VIDEO) {
                  str3 = "png";
                }
                obj14.format = str3;
...
let closure_15 = createStyles.createStyles({ rightAccessoryContainer: { marginLeft: fn(10226).RIGHT_ACCESSORY_LEFT_MARGIN } });
size = fn(2);
