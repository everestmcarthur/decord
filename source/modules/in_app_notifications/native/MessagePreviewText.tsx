...
// Dependencies: [19, 17, 1957, 10185, 1085, 21, 4606, 1116, 576, 10197, 10198, 10184, 4602, 5638, 10220, 4839, 10228, 10229, 1095, 7375, 1114, 7953, 2]
// Exports: default

// Module 10196 (MessagePreviewText)
...
import usePreviewableMediaText from "usePreviewableMediaText" /* 10228 */;
import useGetInitialMessagePreview from "useGetInitialMessagePreview" /* 10229 */;
...
export default function MessagePreviewText(message) {
  ...
  if (memo.length > 0) {
    ...
    if (tmp29) {
      tmp29 = previewableMedia[0].type === tmp(10220).PreviewableMediaTypes.GIF;
    }
    ...
    if (null != nullableMessageAuthor) {
      ...
      tmp(10184);
      if (null != channel) {
        const obj10 = { channel, message, color: "text-default", layout: tmp(7953).ChannelListLayoutTypes.COZY, variant: tmp24, muted: false, lineClamp };
        return React6(tmp(10198).ChannelRowPreview, obj10);
      }
    }
    ...
    if (previewableMedia.length > 0) {
      if (null !== text) {
        ...
        if (tmp17Result) {
          ...
          tmp17Result = React6(tmp(4602).Text, obj13);
        }
    ...
    if (null != message.poll) {
      ...
      items3[1] = React6(tmp(4602).Text, obj18);
