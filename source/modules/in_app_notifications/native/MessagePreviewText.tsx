...
// Dependencies: [19, 17, 1957, 10187, 1085, 21, 4607, 1116, 576, 10199, 10200, 10186, 4603, 5639, 10222, 4840, 10230, 10231, 1095, 7376, 1114, 7954, 2]
// Exports: default

// Module 10198 (MessagePreviewText)
...
import usePreviewableMediaText from "usePreviewableMediaText" /* 10230 */;
import useGetInitialMessagePreview from "useGetInitialMessagePreview" /* 10231 */;
...
export default function MessagePreviewText(message) {
  ...
  if (memo.length > 0) {
    ...
    if (tmp29) {
      tmp29 = previewableMedia[0].type === tmp(10222).PreviewableMediaTypes.GIF;
    }
    ...
    if (null != nullableMessageAuthor) {
      ...
      tmp(10186);
      if (null != channel) {
        const obj10 = { channel, message, color: "text-default", layout: tmp(7954).ChannelListLayoutTypes.COZY, variant: tmp24, muted: false, lineClamp };
        return React6(tmp(10200).ChannelRowPreview, obj10);
      }
    }
    ...
    if (previewableMedia.length > 0) {
      if (null !== text) {
        ...
        if (tmp17Result) {
          ...
          tmp17Result = React6(tmp(4603).Text, obj13);
        }
    ...
    if (null != message.poll) {
      ...
      items3[1] = React6(tmp(4603).Text, obj18);
