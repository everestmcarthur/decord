...
// Dependencies: [19, 17, 1958, 10245, 1085, 21, 4640, 1365, 576, 10257, 10258, 10244, 4636, 5675, 10280, 4873, 10285, 10286, 1096, 7416, 1115, 8001, 2]
// Exports: default

// Module 10256 (MessagePreviewText)
...
import useTruncatedGradientColorsDefault from "useTruncatedGradientColors" /* 10257 */;
import ChannelRowPreview from "ChannelRowPreview" /* 10258 */;
import usePreviewableMedia from "usePreviewableMedia" /* 10280 */;
import usePreviewableMediaText from "usePreviewableMediaText" /* 10285 */;
import useGetInitialMessagePreview from "useGetInitialMessagePreview" /* 10286 */;
...
export default function MessagePreviewText(message) {
  ...
  if (memo.length > 0) {
    const first = memo[0];
    if (first.type === tmp(1096).MessageEmbedTypes.GIFV) {
      if (null != text) {
        const obj3 = { text };
    ...
    if (tmp29) {
      tmp29 = previewableMedia[0].type === tmp(10280).PreviewableMediaTypes.GIF;
    }
    if (previewableMedia.length > 0) {
      if (null != nullableMessageAuthor) {
        const intl4 = tmp(1115).intl;
        const obj7 = { username: nullableMessageAuthor.nick };
        let formatResult = intl4.format(tmp(1115).t.sLDHDi, obj7);
      } else {
    ...
    if (null != nullableMessageAuthor) {
      ...
      tmp(10244);
      if (null != channel) {
        const obj10 = { channel, message, color: "text-default", layout: tmp(8001).ChannelListLayoutTypes.COZY, variant: tmp24, muted: false, lineClamp };
        return React6(tmp(10258).ChannelRowPreview, obj10);
      }
    }
    ...
    if (previewableMedia.length > 0) {
      if (null !== text) {
        ...
        if (tmp17Result) {
          ...
          tmp17Result = React6(tmp(4636).Text, obj13);
        }
    ...
    if (null != message.poll) {
      if (null != nullableMessageAuthor) {
        ...
        let formatResult1 = intl2.format(tmp(1115).t["1wtRlq"], obj15);
      } else {
      ...
      items3[1] = React6(tmp(4636).Text, obj18);
