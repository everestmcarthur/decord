...
// Dependencies: [19, 17, 21, 5669, 8943, 576, 4639, 10228, 1115, 4650, 10300, 8714, 10237, 2]
// Exports: default

// Module 10347 (AlertNotification)
...
let size = { shape: fn(8943).CutoutShape.RoundedRect, x: fn(5669).ImageSizes[fn(undefined, 5669).GuildIconSizes.NORMAL] - -6 - 24, y: -6, width: 24, height: 24, cornerRadius: nativeDefault.radii.sm };
...
export default function AlertNotification(notification) {
  ...
  const tmp = closure_9();
  const incidentData = channel(10228).useGuildIncidentsState(notification.key).incidentData;
  let raidDetectedAt;
  ...
  obj4.children = closure_5(guild(5669), { guild, selected: false });
  const items3 = [closure_5(guild(8943), obj4), ];
  const obj5 = { style: tmp.warningBadge, children: null };
  const tmp8 = guild(8943);
  obj5.children = closure_5(channel(8714).WarningIcon, { style: tmp.warningIcon, color: guild(576).colors.ICON_FEEDBACK_WARNING });
  items3[1] = closure_5(View, obj5);
  obj3.children = items3;
  obj2.icon = closure_7(closure_6, obj3);
  obj2.header = memo;
  obj2.children = closure_5(channel(10237).SystemMessageText, { text: stringResult });
  obj2.onPress = callback;
  obj2.notification = notification;
  return closure_5(channel(10300).NotificationPressable, obj2);
};
