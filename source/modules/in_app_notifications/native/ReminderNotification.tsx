...
// Dependencies: [19, 17, 1957, 1979, 10165, 1074, 21, 4605, 1178, 4568, 576, 10164, 10244, 504, 1094, 10242, 10177, 10179, 7931, 38, 1114, 4808, 7910, 7911, 1242, 10240, 2]

// Module 11390 (ReminderNotification)
...
import MessagePreviewTextDefault from "MessagePreviewText" /* 10177 */;
import MessageNotificationHeaderDefault from "MessageNotificationHeader" /* 10242 */;
import noop from "module_19" /* 19 */;
import ChannelStore from "ChannelStore" /* 1957 */;
import GuildStore from "GuildStore" /* 1979 */;

const MediaPreviewRightAccessory = tmp2(10244);
require = fn;
...
function NotificationBody(channel) {
  ...
  const hasPreviewableMedia = channel(10164).useHasPreviewableMedia(message);
  ...
  obj3 = channel(10164);
  let tmp10 = null;
  const messagePreviewTextVariant = channel(10164).getMessagePreviewTextVariant();
  if (!tmp6) {
  ...
  if (!hasPreviewableMedia) {
    if (null == message.poll) {
      const obj5 = { channel, message, color: "text-default", layout: tmp(7931).ChannelListLayoutTypes.COZY, variant: messagePreviewTextVariant, muted: false, lineClamp: num };
      let tmp14 = closure_10(tmp(10179).ChannelRowPreview, obj5);
    }
...
export default noop.memo(function ReminderNotification(notification) {
  ...
  return closure_10(notification(10240).NotificationPressable, { icon: closure_10(NotificationAvatar, { user: notification.author, guildId: channel.guild_id }), header: memo, onPress: callback, notification, rightAccessory: closure_10(closure_16, { message }), children: closure_10(NotificationBody, { channel, message }) });
});
