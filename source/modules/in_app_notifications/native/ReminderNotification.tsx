...
// Dependencies: [19, 17, 1958, 1980, 10226, 1074, 21, 4639, 1177, 4602, 576, 10225, 10304, 504, 1095, 10302, 10237, 10239, 7986, 38, 1115, 4842, 7965, 7966, 1241, 10300, 2]

// Module 11456 (ReminderNotification)
...
import MessageNotificationHeaderDefault from "MessageNotificationHeader" /* 10302 */;
import noop from "module_19" /* 19 */;
import ChannelStore from "ChannelStore" /* 1958 */;
import GuildStore from "GuildStore" /* 1980 */;

const MediaPreviewRightAccessory = tmp2(10304);
require = fn;
...
function NotificationBody(channel) {
  ...
  const hasPreviewableMedia = channel(10225).useHasPreviewableMedia(message);
  ...
  obj3 = channel(10225);
  let tmp10 = null;
  const messagePreviewTextVariant = channel(10225).getMessagePreviewTextVariant();
  if (!tmp6) {
  ...
  if (!hasPreviewableMedia) {
    if (null == message.poll) {
      const obj5 = { channel, message, color: "text-default", layout: tmp(7986).ChannelListLayoutTypes.COZY, variant: messagePreviewTextVariant, muted: false, lineClamp: num };
      let tmp14 = closure_10(tmp(10239).ChannelRowPreview, obj5);
    }
...
export default noop.memo(function ReminderNotification(notification) {
  ...
  return closure_10(notification(10300).NotificationPressable, { icon: closure_10(NotificationAvatar, { user: notification.author, guildId: channel.guild_id }), header: memo, onPress: callback, notification, rightAccessory: closure_10(closure_16, { message }), children: closure_10(NotificationBody, { channel, message }) });
});
