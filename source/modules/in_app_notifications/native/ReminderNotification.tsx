...
// Dependencies: [19, 17, 1958, 1980, 10339, 1074, 21, 4722, 1177, 4684, 576, 10338, 10414, 504, 1095, 10412, 10350, 10352, 8084, 38, 1115, 4925, 8063, 8064, 1241, 10410, 2]

// Module 11570 (ReminderNotification)
...
import MessageNotificationHeaderDefault from "MessageNotificationHeader" /* 10412 */;
import noop from "module_19" /* 19 */;
import ChannelStore from "ChannelStore" /* 1958 */;
import GuildStore from "GuildStore" /* 1980 */;

const MediaPreviewRightAccessory = tmp2(10414);
require = fn;
...
function NotificationBody(channel) {
  ...
  const hasPreviewableMedia = channel(10338).useHasPreviewableMedia(message);
  ...
  obj3 = channel(10338);
  let tmp10 = null;
  const messagePreviewTextVariant = channel(10338).getMessagePreviewTextVariant();
  if (!tmp6) {
  ...
  if (!hasPreviewableMedia) {
    if (null == message.poll) {
      const obj5 = { channel, message, color: "text-default", layout: tmp(8084).ChannelListLayoutTypes.COZY, variant: messagePreviewTextVariant, muted: false, lineClamp: num };
      let tmp14 = closure_10(tmp(10352).ChannelRowPreview, obj5);
    }
...
export default noop.memo(function ReminderNotification(notification) {
  ...
  return closure_10(notification(10410).NotificationPressable, { icon: closure_10(NotificationAvatar, { user: notification.author, guildId: channel.guild_id }), header: memo, onPress: callback, notification, rightAccessory: closure_10(closure_16, { message }), children: closure_10(NotificationBody, { channel, message }) });
});
