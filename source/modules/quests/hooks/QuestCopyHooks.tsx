...
// Function ID: 11910
// Name: QuestCopyHooks
// Dependencies: [5, 32, 19, 2026, 1372, 5529, 1074, 1374, 1115, 11621, 7827, 7829, 11428, 2025, 11910, 1886, 504, 11911, 8801, 8812, 5532, 11433, 1881, 9661, 11651, 11634, 7833, 7832, 2]
...
import isActivitySupportedOnClientPlatformDefault from "isActivitySupportedOnClientPlatform" /* 9661 */;
...
import useInGameQuestConnectState from "useInGameQuestConnectState" /* 11911 */;
...
const utils_QuestUtils = v1votF6(7827);
const GameProfileAnalyticUtils = tmp2(8812);
const SponsoredQuestUtils = v1votF6(11910);
require = fn;
...
export const useQuestInstructionTitle = function useQuestInstructionTitle(quest, arg1, questContent, sourceQuestContent) {
  ...
  if (tmpResult7.isConsoleQuest(quest)) {
    ...
    if (tmpResult8.hasWatchVideoTasks(quest)) {
      const defaultWatchVideoTask = tmp(7829).getDefaultWatchVideoTask(quest.config);
      let videoTitle;
      if (defaultWatchVideoTask != null) {
      ...
      if (tmpResult10.hasPlayActivityTask(quest)) {
        const result = tmp(7827).isPlayAnyActivityQuest(quest);
        ...
        tmpResult12 = tmp(7829);
      }
      tmpResult10 = tmp(7829);
    }
    tmpResult8 = tmp(7829);
  }
};
...
export const useQuestBarSubtitle = function useQuestBarSubtitle(arg0) {
  ...
  if (tmp8) {
    ...
    if (isExpanded) {
      ...
      if (activeScreen !== tmp(5532).TaskPlatformScreen.SELECT) {
        if (tmpResult9.isSponsoredPlayQuest(quest)) {
          if (tmp10) {
            if (!tmp11) {
              const defaultRewardNameWithArticle = tmp(11428).getDefaultRewardNameWithArticle(quest.config, stateFromStores);
              const intl5 = tmp(1115).intl;
      ...
      if (tmpResult8.hasWatchVideoTasks(quest)) {
        ...
        if (!tmpResult11.hasAchievementInGameTask(quest)) {
          if (isQuestProgressing) {
            ...
            let contextualEntrypointHeading = tmp(11433).getContextualEntrypointHeading(obj10);
            const tmpResult12 = tmp(11433);
          } else {
            const intl2 = tmp(1115).intl;
...
export const useQuestBarTitle = function useQuestBarTitle(questDockQuest, arg1) {
  ...
  if (null != completedAt) {
    ...
    if (_slicedToArray(obj3.useTaskPlatformScreen(questDockQuest, questTaskDetails), 1)[0] === tmp(5532).TaskPlatformScreen.SELECT) {
      const intl2 = tmp(1115).intl;
      let stringResult = intl2.string(tmp(1115).t.EMrUHQ);
    } else {
      const tmpResult6 = tmp(7829);
      const intl = tmp(1115).intl;
      ...
      isConsoleQuestResult = tmp(7829).isConsoleQuest(questDockQuest);
    }
    return stringResult;
  }
};
export const usePrimaryCtaCopy = function usePrimaryCtaCopy(arg0) {
      ...
      if (!tmp2Result.hasPlayActivityTask(tmp)) {
        ...
        if (tmp2Result4.hasStreamOnDesktopTask(obj2)) {
          ...
          if (tmp2Result5.hasWatchVideoOnMobileTasks(tmp)) {
            ...
            PLAY = tmp2(7829).isInGameQuest(tmp) ? tmp4.IN_GAME : tmp4.PLAY;
            const tmp2Result6 = tmp2(7829);
          }
          tmp2Result5 = tmp2(7829);
        }
        tmp2Result4 = tmp2(7829);
      }
      return PLAY;
    }
    PLAY = constants.PLAY;
  ...
  if (constants3.PLAY === memo) {
    ...
    return tmp2(11651).getVideoQuestWatchCtaText(tmp5);
...
export const getRewardCodeRedemptionInstructions = function getRewardCodeRedemptionInstructions(arg0) {
  ...
  if (platform == null) {
    platform = tmp(5532).QuestRewardCodePlatforms.CROSS_PLATFORM;
  }
  ...
  if (result) {
    ...
    obj2.idx = tier;
    rewardCodeQuestReward = tmp(11428).getRewardCodeQuestReward(obj2);
    const tmpResult = tmp(11428);
  }
  let prop;
    ...
    if (dependencyMap === 2) {
      ...
      try {
        ...
        if (0 === c1) {
          ...
          const obj6 = { content: closure_128_1, ctaContent: tmp4(7833).QuestContentCTA.OPEN_GAME_LINK, impressionId: null, sourceQuestContent: null };
          let tmp5;
          ...
          tmp4(11634).openGameLinkDirectly(closure_128_0, obj6);
          dependencyMap = 3;
          return { value: "HermesInternal", done: null };
        }
      } catch (tmp11) {
  ...
  const memo = noop.useMemo(() => {
    ...
    if (null != subtitle) {
      ...
      if (obj3.hasAchievementInGameTask(messages)) {
        ...
        const questType = tmp5(7827).getQuestType(messages.config);
        if (questType === tmp5(7832).QuestType.GAMEPLAY) {
          ...
          if (!features.includes(constants2.NON_GAMING_PLAY_QUEST)) {
            ...
            tmp5Result2 = tmp5(7827);
          }
        }
        const intl2 = tmp5(1115).intl;
        taskTitle = intl2.string(tmp5(1115).t.y8Xf3k);
        const tmp5Result = tmp5(7827);
      }
      obj3 = QuestTaskUtils;
    }
  }, items);
  obj.ctaText = quest(11433).getExternalCtaLabel(quest);
