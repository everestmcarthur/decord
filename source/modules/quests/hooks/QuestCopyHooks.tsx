...
// Function ID: 11871
// Name: QuestCopyHooks
// Dependencies: [5, 32, 19, 2025, 1371, 5496, 1074, 1373, 1114, 11579, 7795, 7797, 11386, 2024, 11871, 1885, 504, 11872, 8768, 8779, 5499, 11391, 1880, 9622, 11609, 11592, 7801, 7800, 2]
...
import useInGameQuestConnectState from "useInGameQuestConnectState" /* 11872 */;
...
const utils_QuestUtils = v1votF6(7795);
const GameProfileAnalyticUtils = tmp2(8779);
const SponsoredQuestUtils = v1votF6(11871);
require = fn;
...
export const useQuestInstructionTitle = function useQuestInstructionTitle(quest, arg1, questContent, sourceQuestContent) {
  ...
  if (tmpResult7.isConsoleQuest(quest)) {
    ...
    if (tmpResult8.hasWatchVideoTasks(quest)) {
      const defaultWatchVideoTask = tmp(7797).getDefaultWatchVideoTask(quest.config);
      let videoTitle;
      if (defaultWatchVideoTask != null) {
      ...
      if (tmpResult10.hasPlayActivityTask(quest)) {
        const result = tmp(7795).isPlayAnyActivityQuest(quest);
        ...
        tmpResult12 = tmp(7797);
      }
      tmpResult10 = tmp(7797);
    }
    tmpResult8 = tmp(7797);
  }
};
...
export const useQuestBarSubtitle = function useQuestBarSubtitle(arg0) {
  ...
  if (tmp8) {
    ...
    if (isExpanded) {
      ...
      if (activeScreen !== tmp(5499).TaskPlatformScreen.SELECT) {
        if (tmpResult9.isSponsoredPlayQuest(quest)) {
          if (tmp10) {
            if (!tmp11) {
              const defaultRewardNameWithArticle = tmp(11386).getDefaultRewardNameWithArticle(quest.config, stateFromStores);
              const intl5 = tmp(1114).intl;
      ...
      if (tmpResult8.hasWatchVideoTasks(quest)) {
        ...
        if (!tmpResult11.hasAchievementInGameTask(quest)) {
          if (isQuestProgressing) {
            ...
            let contextualEntrypointHeading = tmp(11391).getContextualEntrypointHeading(obj10);
            const tmpResult12 = tmp(11391);
          } else {
            const intl2 = tmp(1114).intl;
...
export const useQuestBarTitle = function useQuestBarTitle(questDockQuest, arg1) {
  ...
  if (null != completedAt) {
    ...
    if (_slicedToArray(obj3.useTaskPlatformScreen(questDockQuest, questTaskDetails), 1)[0] === tmp(5499).TaskPlatformScreen.SELECT) {
      const intl2 = tmp(1114).intl;
      ...
      isConsoleQuestResult = tmp(7797).isConsoleQuest(questDockQuest);
    }
    return stringResult;
  }
};
export const usePrimaryCtaCopy = function usePrimaryCtaCopy(arg0) {
      ...
      if (!tmp2Result.hasPlayActivityTask(tmp)) {
        ...
        if (tmp2Result4.hasStreamOnDesktopTask(obj2)) {
          ...
          if (tmp2Result5.hasWatchVideoOnMobileTasks(tmp)) {
            ...
            PLAY = tmp2(7797).isInGameQuest(tmp) ? tmp4.IN_GAME : tmp4.PLAY;
            const tmp2Result6 = tmp2(7797);
          }
          tmp2Result5 = tmp2(7797);
        }
        tmp2Result4 = tmp2(7797);
      }
      return PLAY;
    }
    PLAY = constants.PLAY;
  ...
  if (constants3.PLAY === memo) {
    ...
    return tmp2(11609).getVideoQuestWatchCtaText(tmp5);
...
export const getRewardCodeRedemptionInstructions = function getRewardCodeRedemptionInstructions(arg0) {
  ...
  if (platform == null) {
    platform = tmp(5499).QuestRewardCodePlatforms.CROSS_PLATFORM;
  }
  ...
  if (result) {
    ...
    obj2.idx = tier;
    rewardCodeQuestReward = tmp(11386).getRewardCodeQuestReward(obj2);
    ...
    if (dependencyMap === 2) {
      ...
      try {
        ...
        if (0 === c1) {
          ...
          const obj6 = { content: closure_128_1, ctaContent: tmp4(7801).QuestContentCTA.OPEN_GAME_LINK, impressionId: null, sourceQuestContent: null };
          let tmp5;
          ...
          tmp4(11592).openGameLinkDirectly(closure_128_0, obj6);
          dependencyMap = 3;
          return { value: "HermesInternal", done: null };
        }
      } catch (tmp11) {
  ...
  const memo = noop.useMemo(() => {
    ...
    if (null != subtitle) {
      ...
      if (obj3.hasAchievementInGameTask(messages)) {
        ...
        const questType = tmp5(7795).getQuestType(messages.config);
        if (questType === tmp5(7800).QuestType.GAMEPLAY) {
          ...
          if (!features.includes(constants2.NON_GAMING_PLAY_QUEST)) {
            ...
            tmp5Result2 = tmp5(7795);
          }
        }
        const intl2 = tmp5(1114).intl;
        taskTitle = intl2.string(tmp5(1114).t.y8Xf3k);
        const tmp5Result = tmp5(7795);
      }
      obj3 = QuestTaskUtils;
    }
  }, items);
  obj.ctaText = quest(11391).getExternalCtaLabel(quest);
