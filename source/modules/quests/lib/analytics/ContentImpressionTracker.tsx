...
// Dependencies: [5, 19, 7776, 11292, 1074, 21, 1256, 11288, 7772, 5503, 11584, 11587, 11588, 11589, 7802, 7801, 1115, 7750, 11290, 11372, 11293, 11289, 7782, 7791, 7807, 7675, 7680, 11582, 504, 5044, 2]
// Exports: QuestContentImpressionTracker

// Module 11583 (ContentImpressionTracker)
...
import IosAttributionEligibility from "IosAttributionEligibility" /* 11584 */;
import IosAttributionImpressionRegistry from "IosAttributionImpressionRegistry" /* 11589 */;
...
require = fn;
const ContentImpressionTrackerConstants = fn(11292);
    ...
    obj1.maybeRegisterIosAttributionImpression = function maybeRegisterIosAttributionImpression(item, adProvenanceMetadataSealed) {
      if (!obj5.iosAttributionRegistered) {
        if (obj.isIosAttributionEligible()) {
          const activeIosAttributionFramework = tmp2(11587).getActiveIosAttributionFramework();
          if (null != activeIosAttributionFramework) {
            if (null != adProvenanceMetadataSealed) {
              if (tmp2Result6.isCampaignIosAttributionEnabled(tmp.sourceQuestContent, item)) {
                ...
                const result = tmp2(11589).registerViewThroughImpression(obj2);
                tmp.iosAttributionRegistered = true;
                const tmp2Result7 = tmp2(11589);
              } else {
                const result1 = tmp2(11588).trackIosAttributionImpression(tmp2(11588).IosAttributionImpressionResult.NOT_SKAN_ENABLED, activeIosAttributionFramework, tmp.id);
                const tmp2Result8 = tmp2(11588);
              }
              tmp2Result6 = tmp2(11584);
            } else {
              const result2 = tmp2(11588).trackIosAttributionImpression(tmp2(11588).IosAttributionImpressionResult.NO_METADATA, activeIosAttributionFramework, tmp.id);
              const tmp2Result9 = tmp2(11588);
            }
          } else {
            const result3 = tmp2(11588).trackIosAttributionImpression(tmp2(11588).IosAttributionImpressionResult.NO_FRAMEWORK, activeIosAttributionFramework, tmp.id);
            const tmp2Result10 = tmp2(11588);
          }
          const tmp2Result = tmp2(11587);
        }
        ...
        try {
          ...
          if (0 === dependencyMap) {
            if (arg0 === 1) {
              ...
              dependencyMap = 1;
              c3 = 1;
              let obj6 = { value: obj10.getAdUser(tmp2(7801).getQuestContentName(obj5.questContent)), done: false };
              return obj6;
            }
          } else if (arg0 === 1) {
            c3 = 3;
            ...
            let merged = Object.assign(tmp5(7750)());
            let merged1 = Object.assign(tmp2(11290).getBrandSafetyContext(closure_129_0.questContent));
            closure_128_2 = obj9;
            const adContentIds = closure_129_0.entity.adContentIds;
            const item = adContentIds.forEach((item, index) => {
              let adMetadataSealed = obj5(7772).getAdMetadataSealed(adUser.sourceQuestContent, item);
              ...
              const QUEST = obj5(5503).AdCreativeType.QUEST;
              if (adUser.migrateQuestContentViewedToCaptureAdUserAction) {
                if (adCreativeType === QUEST) {
                  ...
                  if (!isQuestExpiredResult) {
                    isQuestExpiredResult = tmp(7772).isQuestExpired(quest);
                    const tmpResult = tmp(7772);
                  }
                  if (!isQuestExpiredResult) {
                    const items = [tmp30];
                    tmp(11372).markAdContentSeen(tmp(5503).AdCreativeType.QUEST, items);
                    const tmpResult18 = tmp(11372);
                  }
                }
                const result = tmp(7772).isBillableQuestContent(obj2.questContent, obj2.entity.adCreativeType);
                const AdUserActionType = tmp(11293).AdUserActionType;
                ...
                if (obj2.entity.adCreativeType === tmp(5503).AdCreativeType.QUEST) {
                  const obj4 = {};
                  const merged = Object.assign(obj3);
                  obj4.adCreativeType = obj2.entity.adCreativeType;
                  obj4.adCreativeId = obj2.entity.adContentIds[index];
                  tmp(11289).captureAdUserAction(obj4);
                  const tmpResult20 = tmp(11289);
                  ...
                  tmp(11289).captureAdUserAction(obj5);
                  const tmpResult21 = tmp(11289);
                } else {
                  const obj6 = {};
                  const merged2 = Object.assign(obj3);
                  obj6.adCreativeType = obj2.entity.adCreativeType;
                  obj6.adCreativeId = obj2.entity.adContentIds[index];
                  tmp(11289).captureAdUserAction(obj6);
                  const tmpResult22 = tmp(11289);
                }
                const tmpResult19 = tmp(7772);
                const questLogger = tmp(7782).getQuestLogger();
                const tmpResult23 = tmp(7782);
                const _HermesInternal2 = HermesInternal;
                const obj7 = { impressionId: obj2.id };
                questLogger.info("" + item + " ad content viewed for at least " + obj2.minViewTimeSeconds + "s at " + tmp(7801).getQuestContentName(obj2.questContent), obj7);
                ...
                if (!isQuestExpiredResult1) {
                  isQuestExpiredResult1 = tmp(7772).isQuestExpired(quest1);
                  const tmpResult25 = tmp(7772);
                }
                if (!isQuestExpiredResult1) {
                  const items1 = [tmp6];
                  tmp(11372).markAdContentSeen(tmp(5503).AdCreativeType.QUEST, items1);
                  const tmpResult26 = tmp(11372);
                }
                const questLogger1 = tmp(7782).getQuestLogger();
                let questName;
                if (quest1 != null) {
                ...
                const tmpResult27 = tmp(7782);
                const _HermesInternal = HermesInternal;
                const obj8 = { impressionId: obj2.id };
                questLogger1.info("" + questName + " Quest viewed for at least " + obj2.minViewTimeSeconds + "s at " + tmp(7801).getQuestContentName(obj2.questContent), obj8);
                const tmpResult28 = tmp(7801);
                const obj9 = {};
                ...
                const tmpResult29 = tmp(7791);
                const currentQuestHomeSearchSession = tmp(7807).getCurrentQuestHomeSearchSession();
                let uuid;
                ...
                const tmpResult30 = tmp(7807);
                let id;
                if (quest1 != null) {
                  id = quest1.id;
                }
                let adTrafficMetadataSealed = tmp(7772).getAdTrafficMetadataSealed(obj2.sourceQuestContent, id);
                ...
                const tmpResult31 = tmp(7772);
              } else {
                const questLogger2 = tmp(7782).getQuestLogger();
                const tmpResult32 = tmp(7782);
                const _HermesInternal3 = HermesInternal;
                const obj11 = { impressionId: obj2.id };
                questLogger2.info("" + obj2.entity.adContentIds[index] + " ad content viewed for at least " + obj2.minViewTimeSeconds + "s at " + tmp(7801).getQuestContentName(obj2.questContent), obj11);
                const tmpResult33 = tmp(7801);
                const obj12 = {};
                ...
                tmp(7791).trackAdContentEvent(obj12);
                const tmpResult34 = tmp(7791);
              }
              const obj = obj5(7772);
            });
      ...
      if (null != flag.lastBeatTime) {
        ...
        const item = adContentIds.forEach((item, index) => {
          if (obj5.entity.adCreativeType === AdCreativeType.AdCreativeType.QUEST) {
            ...
            tmp(7791).trackAdContentEvent(obj6);
            const tmpResult6 = tmp(7791);
          }
        });
      }
    ...
    obj1.start = function start() {
      ...
      const item = adContentIds.forEach((item, index) => {
        ...
        if (obj5.migrateQuestContentLoadedToCaptureAdUserAction) {
          ...
          tmpResult = tmp(11289);
          obj5 = { type: tmp(11293).AdUserActionType.END_CONTENT_LOAD, surfaceId: null, sourceQuestContent: null, impressionId: null, triggeredByStatusChange: null, trackGuildAndChannelMetadata: null, questContentPosition: null, questContentRowIndex: null };
          ...
          const tmpResult8 = tmp(7782);
          const _HermesInternal = HermesInternal;
          const obj6 = { impressionId: questContentRowIndex.id };
          questLogger.info("" + questName + " Quest became visible at " + tmp(7801).getQuestContentName(questContentRowIndex.questContent), obj6);
          const tmpResult9 = tmp(7801);
          const obj7 = {};
          ...
          const tmpResult10 = tmp(7791);
          let id;
          if (quest != null) {
            id = quest.id;
          }
          let adTrafficMetadataSealed = tmp(7772).getAdTrafficMetadataSealed(questContentRowIndex.sourceQuestContent, id);
          if (adTrafficMetadataSealed == null) {
            adTrafficMetadataSealed = null;
          }
          ...
          const tmpResult11 = tmp(7772);
        } else {
          const questLogger1 = tmp(7782).getQuestLogger();
          const tmpResult12 = tmp(7782);
          const _HermesInternal2 = HermesInternal;
          const obj9 = { impressionId: questContentRowIndex.id };
          questLogger1.info("" + questContentRowIndex.entity.adContentIds[tmpResult] + " ad content became visible at " + tmp(7801).getQuestContentName(questContentRowIndex.questContent), obj9);
          const tmpResult13 = tmp(7801);
          const obj10 = {};
          ...
          tmp(7791).trackAdContentEvent(obj10);
          const tmpResult14 = tmp(7791);
        }
      });
      let obj2 = { name: obj5(7680).MetricEvents.QUEST_CONTENT_IMPRESSION, tags: null };
      let obj = MonitoringAgentDefault;
      const items = ["quest_content:" + obj5(7801).getQuestContentName(obj5.questContent)];
      obj2.tags = items;
      obj.increment(obj2);
      obj5.isRunning = true;
    };
