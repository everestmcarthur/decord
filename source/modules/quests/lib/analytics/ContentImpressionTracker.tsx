...
// Dependencies: [5, 19, 7811, 7842, 1074, 21, 1255, 7837, 7807, 5537, 11634, 11637, 11638, 11639, 7843, 7836, 1364, 7785, 7840, 11422, 7848, 7838, 7817, 7826, 7839, 7710, 7715, 11632, 504, 5077, 2]
// Exports: QuestContentImpressionTracker

// Module 11633 (ContentImpressionTracker)
...
import IosAttributionEligibility from "IosAttributionEligibility" /* 11634 */;
import IosAttributionImpressionRegistry from "IosAttributionImpressionRegistry" /* 11639 */;
...
require = fn;
const ContentImpressionTrackerConstants = fn(7842);
    ...
    obj1.maybeRegisterIosAttributionImpression = function maybeRegisterIosAttributionImpression(item, adProvenanceMetadataSealed) {
      if (!obj5.iosAttributionRegistered) {
        if (obj.isIosAttributionEligible()) {
          const activeIosAttributionFramework = tmp2(11637).getActiveIosAttributionFramework();
          if (null != activeIosAttributionFramework) {
            if (null != adProvenanceMetadataSealed) {
              if (tmp2Result6.isCampaignIosAttributionEnabled(tmp.sourceQuestContent, item)) {
                ...
                const result = tmp2(11639).registerViewThroughImpression(obj2);
                tmp.iosAttributionRegistered = true;
                const tmp2Result7 = tmp2(11639);
              } else {
                const result1 = tmp2(11638).trackIosAttributionImpression(tmp2(11638).IosAttributionImpressionResult.NOT_SKAN_ENABLED, activeIosAttributionFramework, tmp.id);
                const tmp2Result8 = tmp2(11638);
              }
              tmp2Result6 = tmp2(11634);
            } else {
              const result2 = tmp2(11638).trackIosAttributionImpression(tmp2(11638).IosAttributionImpressionResult.NO_METADATA, activeIosAttributionFramework, tmp.id);
              const tmp2Result9 = tmp2(11638);
            }
          } else {
            const result3 = tmp2(11638).trackIosAttributionImpression(tmp2(11638).IosAttributionImpressionResult.NO_FRAMEWORK, activeIosAttributionFramework, tmp.id);
            const tmp2Result10 = tmp2(11638);
          }
          const tmp2Result = tmp2(11637);
        }
        ...
        try {
          ...
          if (0 === dependencyMap) {
            if (arg0 === 1) {
              ...
              dependencyMap = 1;
              c3 = 1;
              let obj6 = { value: obj10.getAdUser(tmp2(7836).getQuestContentName(obj5.questContent)), done: false };
              return obj6;
            }
          } else if (arg0 === 1) {
            c3 = 3;
            ...
            let merged = Object.assign(tmp5(7785)());
            let merged1 = Object.assign(tmp2(7840).getBrandSafetyContext(closure_129_0.questContent));
            closure_128_2 = obj9;
            const adContentIds = closure_129_0.entity.adContentIds;
            const item = adContentIds.forEach((item, index) => {
              let adMetadataSealed = obj5(7807).getAdMetadataSealed(adUser.sourceQuestContent, item);
              ...
              const QUEST = obj5(5537).AdCreativeType.QUEST;
              if (adUser.migrateQuestContentViewedToCaptureAdUserAction) {
                if (adCreativeType === QUEST) {
                  ...
                  if (!isQuestExpiredResult) {
                    isQuestExpiredResult = tmp(7807).isQuestExpired(quest);
                    const tmpResult = tmp(7807);
                  }
                  if (!isQuestExpiredResult) {
                    const items = [tmp30];
                    tmp(11422).markAdContentSeen(tmp(5537).AdCreativeType.QUEST, items);
                    const tmpResult18 = tmp(11422);
                  }
                }
                const result = tmp(7807).isBillableQuestContent(obj2.questContent, obj2.entity.adCreativeType);
                const AdUserActionType = tmp(7848).AdUserActionType;
                ...
                if (obj2.entity.adCreativeType === tmp(5537).AdCreativeType.QUEST) {
                  const obj4 = {};
                  const merged = Object.assign(obj3);
                  obj4.adCreativeType = obj2.entity.adCreativeType;
                  obj4.adCreativeId = obj2.entity.adContentIds[index];
                  tmp(7838).captureAdUserAction(obj4);
                  const tmpResult20 = tmp(7838);
                  ...
                  tmp(7838).captureAdUserAction(obj5);
                  const tmpResult21 = tmp(7838);
                } else {
                  const obj6 = {};
                  const merged2 = Object.assign(obj3);
                  obj6.adCreativeType = obj2.entity.adCreativeType;
                  obj6.adCreativeId = obj2.entity.adContentIds[index];
                  tmp(7838).captureAdUserAction(obj6);
                  const tmpResult22 = tmp(7838);
                }
                const tmpResult19 = tmp(7807);
                const questLogger = tmp(7817).getQuestLogger();
                const tmpResult23 = tmp(7817);
                const _HermesInternal2 = HermesInternal;
                const obj7 = { impressionId: obj2.id };
                questLogger.info("" + item + " ad content viewed for at least " + obj2.minViewTimeSeconds + "s at " + tmp(7836).getQuestContentName(obj2.questContent), obj7);
                ...
                if (!isQuestExpiredResult1) {
                  isQuestExpiredResult1 = tmp(7807).isQuestExpired(quest1);
                  const tmpResult25 = tmp(7807);
                }
                if (!isQuestExpiredResult1) {
                  const items1 = [tmp6];
                  tmp(11422).markAdContentSeen(tmp(5537).AdCreativeType.QUEST, items1);
                  const tmpResult26 = tmp(11422);
                }
                const questLogger1 = tmp(7817).getQuestLogger();
                let questName;
                if (quest1 != null) {
                ...
                const tmpResult27 = tmp(7817);
                const _HermesInternal = HermesInternal;
                const obj8 = { impressionId: obj2.id };
                questLogger1.info("" + questName + " Quest viewed for at least " + obj2.minViewTimeSeconds + "s at " + tmp(7836).getQuestContentName(obj2.questContent), obj8);
                const tmpResult28 = tmp(7836);
                const obj9 = {};
                ...
                let adTrafficMetadataSealed = tmp(7807).getAdTrafficMetadataSealed(obj2.sourceQuestContent, id);
                ...
                const tmpResult31 = tmp(7807);
              } else {
                const questLogger2 = tmp(7817).getQuestLogger();
                const tmpResult32 = tmp(7817);
                const _HermesInternal3 = HermesInternal;
                const obj11 = { impressionId: obj2.id };
                questLogger2.info("" + obj2.entity.adContentIds[index] + " ad content viewed for at least " + obj2.minViewTimeSeconds + "s at " + tmp(7836).getQuestContentName(obj2.questContent), obj11);
                const tmpResult33 = tmp(7836);
                const obj12 = {};
                ...
                tmp(7826).trackAdContentEvent(obj12);
                const tmpResult34 = tmp(7826);
              }
              const obj = obj5(7807);
            });
      ...
      if (null != flag.lastBeatTime) {
        ...
        const item = adContentIds.forEach((item, index) => {
          if (obj5.entity.adCreativeType === AdCreativeType.AdCreativeType.QUEST) {
            ...
            tmp(7826).trackAdContentEvent(obj6);
    ...
    obj1.start = function start() {
      ...
      const item = adContentIds.forEach((item, index) => {
        ...
        if (obj5.migrateQuestContentLoadedToCaptureAdUserAction) {
          ...
          tmpResult = tmp(7838);
          obj5 = { type: tmp(7848).AdUserActionType.END_CONTENT_LOAD, surfaceId: null, sourceQuestContent: null, impressionId: null, triggeredByStatusChange: null, trackGuildAndChannelMetadata: null, questContentPosition: null, questContentRowIndex: null };
          ...
          const tmpResult8 = tmp(7817);
          const _HermesInternal = HermesInternal;
          const obj6 = { impressionId: questContentRowIndex.id };
          questLogger.info("" + questName + " Quest became visible at " + tmp(7836).getQuestContentName(questContentRowIndex.questContent), obj6);
          const tmpResult9 = tmp(7836);
          const obj7 = {};
          ...
          const tmpResult10 = tmp(7826);
          let id;
          if (quest != null) {
            id = quest.id;
          }
          let adTrafficMetadataSealed = tmp(7807).getAdTrafficMetadataSealed(questContentRowIndex.sourceQuestContent, id);
          if (adTrafficMetadataSealed == null) {
            adTrafficMetadataSealed = null;
          }
          ...
          const tmpResult11 = tmp(7807);
        } else {
          const questLogger1 = tmp(7817).getQuestLogger();
          const tmpResult12 = tmp(7817);
          const _HermesInternal2 = HermesInternal;
          const obj9 = { impressionId: questContentRowIndex.id };
          questLogger1.info("" + questContentRowIndex.entity.adContentIds[tmpResult] + " ad content became visible at " + tmp(7836).getQuestContentName(questContentRowIndex.questContent), obj9);
          const tmpResult13 = tmp(7836);
          const obj10 = {};
          ...
          tmp(7826).trackAdContentEvent(obj10);
          const tmpResult14 = tmp(7826);
        }
      });
      let obj2 = { name: obj5(7715).MetricEvents.QUEST_CONTENT_IMPRESSION, tags: null };
      let obj = MonitoringAgentDefault;
      const items = ["quest_content:" + obj5(7836).getQuestContentName(obj5.questContent)];
      obj2.tags = items;
      obj.increment(obj2);
      obj5.isRunning = true;
    };
