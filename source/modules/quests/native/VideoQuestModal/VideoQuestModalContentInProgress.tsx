...
// Dependencies: [32, 19, 17, 5458, 1085, 21, 672, 576, 15161, 4574, 1115, 11317, 4310, 4575, 1611, 7137, 4284, 15163, 15178, 7087, 15179, 15157, 15180, 4987, 5137, 4570, 15181, 5601, 4989, 11327, 8113, 1114, 9086, 10968, 11515, 11544, 15126, 8295, 5001, 15183, 10967, 15059, 15057, 9085, 8705, 15156, 4541, 15184, 1896, 11518, 7736, 11224, 11225, 11229, 5465, 7742, 5461, 7732, 8389, 15185, 11798, 15186, 5466, 2]

// Module 15160 (VideoQuestModalContentInProgress)
...
export default noop.memo(function VideoQuestModalContentInProgress(arg0) {
  ...
  const quest = sourceQuestContent(15156).useVideoQuestModalContext().quest;
  items = [quest];
  items1 = [quest];
  ...
  let obj = sourceQuestContent(15156);
  ...
  const callback1 = noop.useCallback(() => {
    ...
    obj.openLazy(asyncRequireImpl(15184, dependencyMap.paths), "transcript-" + quest.id, { quest });
  }, items2);
  const tmp5 = getQuestImpressionId(noop.useState(false), 2);
  getQuestImpressionId = sourceQuestContent(11518).useGetQuestImpressionId();
  let obj2 = sourceQuestContent(11518);
  [tmp11, noop] = getQuestImpressionId(noop.useState(false), 2);
  const tmp10 = getQuestImpressionId(noop.useState(false), 2);
  const isShareableQuestResult = sourceQuestContent(7736).isShareableQuest(quest.config);
  ...
  const callback2 = noop.useCallback(() => {
    if (isShareableQuestResult) {
      ...
      if (obj.shouldMigrateToAdAnalyticsInterface(AdAnalyticsInterfaceExperiment.AdAnalyticsInterfaceExperimentStep.STEP_2_CLICKED_INTERNAL, "video_quest_modal_in_progress")) {
        const questUrl = getQuestUrl(11225);
        const obj2 = { type: getQuestUrl(11229).AdUserActionType.CLICK_INTERNAL, adCreativeType: getQuestUrl(5465).AdCreativeType.QUEST, adCreativeId: quest.id, questContentCTA: getQuestUrl(7742).QuestContentCTA.MOBILE_SHARESHEET, surfaceId: getQuestUrl(5461).QuestContent.VIDEO_MODAL_MOBILE, sourceQuestContent, impressionId: getQuestImpressionId() };
        questUrl.captureAdUserAction(obj2);
        let tmp = quest;
      } else {
        const questUrl1 = getQuestUrl(7732);
        ...
        obj3.questContent = getQuestUrl(5461).QuestContent.VIDEO_MODAL_MOBILE;
        obj3.questContentCTA = getQuestUrl(7742).QuestContentCTA.MOBILE_SHARESHEET;
      ...
      noop(true);
      const questUrl2 = getQuestUrl(8389);
      const obj4 = { message: null, iOSOnlyShareCallback: null };
      const questUrl3 = getQuestUrl(11327);
  ...
  let obj3 = sourceQuestContent(7736);
  const videoQuestClickCtaAndMaybeCloseModal = sourceQuestContent(15185).useVideoQuestClickCtaAndMaybeCloseModal({ quest, onClose, sourceQuestContent });
  ...
  let obj4 = sourceQuestContent(15185);
  const videoExternallyPaused = sourceQuestContent(15186).useVideoExternallyPaused(quest.id, tmp11);
  const tmp19 = quest.config.taskConfigV2.tasks[sourceQuestContent(undefined, 5466).FirstPartyQuestTaskTypes.WATCH_VIDEO_ON_MOBILE];
  let tmp20 = null == tmp19;
  if (!tmp20) {
    tmp20 = "portrait" === tmp(11544).getVideoOrientation(tmp19);
