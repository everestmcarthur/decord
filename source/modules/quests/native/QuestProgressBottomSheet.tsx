...
// Dependencies: [5, 19, 17, 9653, 7811, 5530, 9648, 21, 4638, 576, 504, 11920, 5533, 8214, 8808, 1115, 5211, 7278, 7832, 9792, 11629, 11917, 15229, 9650, 4605, 11909, 8491, 11441, 7260, 5673, 5072, 1094, 11912, 9197, 15258, 5058, 4634, 5060, 15225, 2]
// Exports: default

// Module 17239 (QuestProgressBottomSheet)
...
const FramesActionCreatorsDefault = tmp(9650);
require = fn;
...
function QuestProgressBottomSheet(quest) {
  quest = quest.quest;
  claim = undefined;
  const isScreenLandscape = quest(5211).useIsScreenLandscape();
  const tmp4 = closure_13(isScreenLandscape);
  let obj = quest(5211);
  let obj2 = quest(7278);
  const getOrFetchApplication = obj2.useGetOrFetchApplication(quest(7832).getActivityApplicationId(quest));
  let id;
  let obj3 = quest(7832);
  if (getOrFetchApplication != null) {
    id = getOrFetchApplication.id;
  }
  const url = claim(9792)({ applicationId: id, size: 600, names: ["embedded_cover"] }).url;
  const tmp7 = claim(9792);
  const questTaskDetails = quest(11629).useQuestTaskDetails(quest);
  const intl = tmp(1115).intl;
  let obj4 = { questName: quest.config.messages.questName };
  let tmpResult = quest(11629);
  const formatToPlainStringResult = intl.formatToPlainString(quest(1115).t.EAYZAr, { questName: quest.config.messages.questName });
  const tmpResult4 = quest(11917);
  const userStatus = quest.userStatus;
  let completedAt;
  const questsInstructionsToWinReward = tmpResult4.useQuestsInstructionsToWinReward({ quest, taskDetails: questTaskDetails, location: QuestsExperimentLocations.QUEST_ACTIVITY_BOTTOM_SHEET, sourceQuestContent: quest(5533).QuestContent.RUNNING_ACTIVITY });
  if (userStatus != null) {
  ...
  let obj5 = { quest, taskDetails: questTaskDetails, location: QuestsExperimentLocations.QUEST_ACTIVITY_BOTTOM_SHEET, sourceQuestContent: quest(5533).QuestContent.RUNNING_ACTIVITY };
  const tmp11 = QuestsExperimentLocations;
  const isQuestAccessSuspended = quest(11629).useIsQuestAccessSuspended();
  const tmpResult5 = quest(11629);
  const tmpResult6 = quest(15229);
  const questRewardClaimHandler = tmpResult6.useQuestRewardClaimHandler({ quest, questContent: quest(5533).QuestContent.RUNNING_ACTIVITY, sourceQuestContent: quest(5533).QuestContent.RUNNING_ACTIVITY });
  ...
  if (tmp24) {
    ...
    tmp24 = closure_11(tmp6(5673), obj8);
  }
  const items3 = [tmp24, , , ];
  const obj10 = { style: tmp4.heroGradient, start: null, end: null, colors: null };
  const obj6 = { quest, questContent: quest(5533).QuestContent.RUNNING_ACTIVITY, sourceQuestContent: quest(5533).QuestContent.RUNNING_ACTIVITY };
  ...
  size.width = num;
  obj11.children = closure_11(claim(11912), size);
  items3[2] = closure_11(closure_5, obj11);
  const obj12 = { style: tmp4.contextMenuContainer, children: null };
  const obj13 = { icon: claim(9197), onPress: callback2, variant: "secondary-overlay", size: "sm", accessibilityLabel: null };
  const intl2 = tmp(1115).intl;
  obj13.accessibilityLabel = intl2.string(quest(1115).t.RDE0Sc);
  const items5 = [closure_11(quest(8214).IconButton, obj13), ];
  const obj14 = { quest, showShareLink: true, location: tmp11.QUEST_ACTIVITY_BOTTOM_SHEET, sourceQuestContent: null, children: null };
  const tmp6Result4 = claim(11912);
  obj14.sourceQuestContent = quest(5533).QuestContent.RUNNING_ACTIVITY;
  obj14.children = contextMenuButton;
  items5[1] = closure_11(claim(15258), obj14);
  ...
  const items7 = [closure_11(quest(4634).Text, { variant: str, color: "mobile-text-heading-primary", children: formatToPlainStringResult }), ];
  ...
  items7[1] = closure_11(quest(4634).Text, obj17);
  obj16.children = items7;
  const items8 = [closure_12(quest(5058).Stack, obj16), ];
  ...
  items9[1] = closure_11(quest(5060).Button, obj21);
  obj18.children = items9;
  items8[1] = closure_12(quest(5058).Stack, obj18);
  obj15.children = items8;
  items6[1] = closure_12(closure_5, obj15);
  obj20.children = items6;
  return closure_12(quest(7260).BottomSheet, obj20);
}
...
export default function QuestProgressBottomSheetConnected(questId) {
  ...
  if (null != stateFromStores) {
    const obj2 = {
      ...
      questContent: tmp(5533).QuestContent.RUNNING_ACTIVITY,
      sourceQuestContent: tmp(5533).QuestContent.RUNNING_ACTIVITY,
      children() {
          return closure_2_11(QuestProgressBottomSheet, { quest: stateFromStores });
        }
    };
    tmp4 = closure_11(tmp(11920).QuestContentImpressionTrackerNative, obj2);
  }
  return tmp4;
};
