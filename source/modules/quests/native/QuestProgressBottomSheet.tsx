...
// Dependencies: [5, 19, 17, 9664, 7817, 5532, 9659, 21, 4640, 576, 504, 11929, 5535, 8057, 8060, 1115, 5213, 7282, 7838, 9803, 11637, 11926, 15234, 9661, 4607, 11918, 8510, 11449, 7264, 5675, 5074, 1094, 11921, 9208, 15263, 5060, 4636, 5062, 15230, 2]
// Exports: default

// Module 17279 (QuestProgressBottomSheet)
...
const FramesActionCreatorsDefault = tmp(9661);
require = fn;
...
function QuestProgressBottomSheet(quest) {
  quest = quest.quest;
  claim = undefined;
  const isScreenLandscape = quest(5213).useIsScreenLandscape();
  const tmp4 = closure_13(isScreenLandscape);
  let obj = quest(5213);
  let obj2 = quest(7282);
  const getOrFetchApplication = obj2.useGetOrFetchApplication(quest(7838).getActivityApplicationId(quest));
  let id;
  let obj3 = quest(7838);
  if (getOrFetchApplication != null) {
    id = getOrFetchApplication.id;
  }
  const url = claim(9803)({ applicationId: id, size: 600, names: ["embedded_cover"] }).url;
  const tmp7 = claim(9803);
  const questTaskDetails = quest(11637).useQuestTaskDetails(quest);
  const intl = tmp(1115).intl;
  let obj4 = { questName: quest.config.messages.questName };
  let tmpResult = quest(11637);
  const formatToPlainStringResult = intl.formatToPlainString(quest(1115).t.EAYZAr, { questName: quest.config.messages.questName });
  const tmpResult4 = quest(11926);
  const userStatus = quest.userStatus;
  let completedAt;
  const questsInstructionsToWinReward = tmpResult4.useQuestsInstructionsToWinReward({ quest, taskDetails: questTaskDetails, location: QuestsExperimentLocations.QUEST_ACTIVITY_BOTTOM_SHEET, sourceQuestContent: quest(5535).QuestContent.RUNNING_ACTIVITY });
  if (userStatus != null) {
  ...
  let obj5 = { quest, taskDetails: questTaskDetails, location: QuestsExperimentLocations.QUEST_ACTIVITY_BOTTOM_SHEET, sourceQuestContent: quest(5535).QuestContent.RUNNING_ACTIVITY };
  const tmp11 = QuestsExperimentLocations;
  const isQuestAccessSuspended = quest(11637).useIsQuestAccessSuspended();
  const tmpResult5 = quest(11637);
  const tmpResult6 = quest(15234);
  const questRewardClaimHandler = tmpResult6.useQuestRewardClaimHandler({ quest, questContent: quest(5535).QuestContent.RUNNING_ACTIVITY, sourceQuestContent: quest(5535).QuestContent.RUNNING_ACTIVITY });
  ...
  if (tmp24) {
    ...
    tmp24 = closure_11(tmp6(5675), obj8);
  }
  const items3 = [tmp24, , , ];
  const obj10 = { style: tmp4.heroGradient, start: null, end: null, colors: null };
  const obj6 = { quest, questContent: quest(5535).QuestContent.RUNNING_ACTIVITY, sourceQuestContent: quest(5535).QuestContent.RUNNING_ACTIVITY };
  ...
  size.width = num;
  obj11.children = closure_11(claim(11921), size);
  items3[2] = closure_11(closure_5, obj11);
  const obj12 = { style: tmp4.contextMenuContainer, children: null };
  const obj13 = { icon: claim(9208), onPress: callback2, variant: "secondary-overlay", size: "sm", accessibilityLabel: null };
  const intl2 = tmp(1115).intl;
  obj13.accessibilityLabel = intl2.string(quest(1115).t.RDE0Sc);
  const items5 = [closure_11(quest(8057).IconButton, obj13), ];
  const obj14 = { quest, showShareLink: true, location: tmp11.QUEST_ACTIVITY_BOTTOM_SHEET, sourceQuestContent: null, children: null };
  const tmp6Result4 = claim(11921);
  obj14.sourceQuestContent = quest(5535).QuestContent.RUNNING_ACTIVITY;
  obj14.children = contextMenuButton;
  items5[1] = closure_11(claim(15263), obj14);
  ...
  const items7 = [closure_11(quest(4636).Text, { variant: str, color: "mobile-text-heading-primary", children: formatToPlainStringResult }), ];
  ...
  items7[1] = closure_11(quest(4636).Text, obj17);
  obj16.children = items7;
  const items8 = [closure_12(quest(5060).Stack, obj16), ];
  ...
  items9[1] = closure_11(quest(5062).Button, obj21);
  obj18.children = items9;
  items8[1] = closure_12(quest(5060).Stack, obj18);
  obj15.children = items8;
  items6[1] = closure_12(closure_5, obj15);
  obj20.children = items6;
  return closure_12(quest(7264).BottomSheet, obj20);
}
...
export default function QuestProgressBottomSheetConnected(questId) {
  ...
  if (null != stateFromStores) {
    const obj2 = {
      ...
      questContent: tmp(5535).QuestContent.RUNNING_ACTIVITY,
      sourceQuestContent: tmp(5535).QuestContent.RUNNING_ACTIVITY,
      children() {
          return closure_2_11(QuestProgressBottomSheet, { quest: stateFromStores });
        }
    };
    tmp4 = closure_11(tmp(11929).QuestContentImpressionTrackerNative, obj2);
  }
  return tmp4;
};
