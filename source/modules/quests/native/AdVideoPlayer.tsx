...
export const AdVideoPlayer = noop.memo((initialProgress) => {
  ...
  callback6 = obj.useCallback(() => {
    ...
    if (first2 !== AdsVideoTypes.PlayerState.ERRORED) {
      if (tmp !== tmp2(15071).PlayerState.PAUSED) {
        if (tmp !== tmp2(15071).PlayerState.LOADING) {
          if (tmp === tmp2(15071).PlayerState.PLAYING) {
            callback(tmp2(15071).PlayerState.PAUSED);
            if (onPausePlayback != null) {
              tmp16(tmp2(15071).PlaybackTriggerSource.USER_INTERACTION);
            }
          }
        }
      }
      callback(tmp2(15071).PlayerState.PLAYING);
      if (onResumePlayback != null) {
        tmp6(tmp2(15071).PlaybackTriggerSource.USER_INTERACTION);
      }
    }
    callback4(0);
    callback(AdsVideoTypes.PlayerState.PLAYING);
    if (onResumePlayback != null) {
      tmp11(tmp2(15071).PlaybackTriggerSource.USER_INTERACTION);
    }
  }, items6);
  ...
  callback8 = obj.useCallback(() => {
    ...
    if (first2 === AdsVideoTypes.PlayerState.PLAYING) {
      ...
      callback(tmp2(15071).PlayerState.PAUSED);
      if (onPausePlayback != null) {
        tmp6(tmp2(15071).PlaybackTriggerSource.SYSTEM_INITIATED);
      }
    }
  }, items8);
  ...
  callback9 = obj.useCallback(() => {
    if (ref5.current) {
      tmp.current = false;
      if (first2 !== AdsVideoTypes.PlayerState.ERRORED) {
        callback(tmp3(15071).PlayerState.PLAYING);
        if (onResumePlayback != null) {
          tmp7(tmp3(15071).PlaybackTriggerSource.SYSTEM_INITIATED);
        }
      }
    }
  }, items9);
