...
// Function ID: 15150
// Name: AdVideoPlayer
// Dependencies: [32, 19, 17, 1896, 1074, 21, 576, 8457, 4640, 672, 15138, 4376, 5061, 5065, 4641, 4644, 1110, 504, 1364, 1231, 1612, 1115, 5210, 15150, 15152, 15154, 15155, 10326, 8423, 8425, 15156, 15157, 2]

// Module 15149 (AdVideoPlayer)
...
const _isNativeReflectConstruct = noop.memo(_modDef8457);
...
export const AdVideoPlayer = noop.memo((initialProgress) => {
  ...
  callback6 = obj.useCallback(() => {
    ...
    if (first2 !== AdsVideoTypes.PlayerState.ERRORED) {
      if (tmp !== tmp2(15138).PlayerState.PAUSED) {
        if (tmp !== tmp2(15138).PlayerState.LOADING) {
          if (tmp === tmp2(15138).PlayerState.PLAYING) {
            callback(tmp2(15138).PlayerState.PAUSED);
            if (onPausePlayback != null) {
              tmp16(tmp2(15138).PlaybackTriggerSource.USER_INTERACTION);
            }
          }
        }
      }
      callback(tmp2(15138).PlayerState.PLAYING);
      if (onResumePlayback != null) {
        tmp6(tmp2(15138).PlaybackTriggerSource.USER_INTERACTION);
      }
    }
    callback4(0);
    callback(AdsVideoTypes.PlayerState.PLAYING);
    if (onResumePlayback != null) {
      tmp11(tmp2(15138).PlaybackTriggerSource.USER_INTERACTION);
    }
  }, items6);
  ...
  callback8 = obj.useCallback(() => {
    ...
    if (first2 === AdsVideoTypes.PlayerState.PLAYING) {
      ...
      callback(tmp2(15138).PlayerState.PAUSED);
      if (onPausePlayback != null) {
        tmp6(tmp2(15138).PlaybackTriggerSource.SYSTEM_INITIATED);
      }
    }
  }, items8);
  ...
  callback9 = obj.useCallback(() => {
    if (ref5.current) {
      tmp.current = false;
      if (first2 !== AdsVideoTypes.PlayerState.ERRORED) {
        callback(tmp3(15138).PlayerState.PLAYING);
        if (onResumePlayback != null) {
          tmp7(tmp3(15138).PlaybackTriggerSource.SYSTEM_INITIATED);
        }
      }
    }
  }, items9);
