...
// Dependencies: [32, 19, 1896, 7842, 1074, 21, 9017, 8402, 504, 11632, 11633, 5537, 2]
// Exports: BillableAdPlacementImpressionTrackerNative, QuestContentImpressionTrackerNative

// Module 11920 (QuestContentImpressionTracker)
...
function AdContentImpressionTrackerBaseNative(skipRemountKey) {
  ...
  let tmp6 = visibilityRef(8402)(tmp3);
  const adContentImpressionTrackerProps = adContentIds2(11632).useAdContentImpressionTrackerProps(skipRemountKey);
  ...
  const obj2 = adContentIds2(11632);
  let items1 = [joined];
  const tmp10 = _slicedToArray(adContentIds2(9017).useRecyclingState(false, items1), 2);
  dependencyMap = tmp11;
  ...
  const obj4 = adContentIds2(9017);
  const obj6 = {};
  const merged1 = Object.assign(skipRemountKey);
  const merged2 = Object.assign({ visible: overrideVisibility, visibleChanged: overrideVisibility !== tmp5(8402)(overrideVisibility) });
  obj6.focused = tmp3;
  ...
  if (tmp17) {
    obj7.adCreativeType = tmp(5537).AdCreativeType.QUEST;
    let tmp19 = obj7;
  } else {
    obj7.adCreativeType = skipRemountKey.adCreativeType;
    tmp19 = obj7;
  }
  return createElement(adContentIds2(11633).QuestContentImpressionTracker, tmp19);
}
let closure_6 = fn(7842).MIN_QUEST_CONTENT_VISIBILITY_PERCENTAGE;
