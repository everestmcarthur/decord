...
// Dependencies: [32, 19, 1896, 7930, 1074, 21, 8957, 8504, 504, 11732, 11733, 5621, 2]
// Exports: BillableAdPlacementImpressionTrackerNative, QuestContentImpressionTrackerNative

// Module 12020 (QuestContentImpressionTracker)
...
function AdContentImpressionTrackerBaseNative(skipRemountKey) {
  ...
  let tmp6 = visibilityRef(8504)(tmp3);
  const adContentImpressionTrackerProps = adContentIds2(11732).useAdContentImpressionTrackerProps(skipRemountKey);
  ...
  const obj2 = adContentIds2(11732);
  let items1 = [joined];
  const tmp10 = _slicedToArray(adContentIds2(8957).useRecyclingState(false, items1), 2);
  dependencyMap = tmp11;
  ...
  const obj4 = adContentIds2(8957);
  const obj6 = {};
  const merged1 = Object.assign(skipRemountKey);
  const merged2 = Object.assign({ visible: overrideVisibility, visibleChanged: overrideVisibility !== tmp5(8504)(overrideVisibility) });
  obj6.focused = tmp3;
  ...
  if (tmp17) {
    obj7.adCreativeType = tmp(5621).AdCreativeType.QUEST;
    let tmp19 = obj7;
  } else {
    obj7.adCreativeType = skipRemountKey.adCreativeType;
    tmp19 = obj7;
  }
  return createElement(adContentIds2(11733).QuestContentImpressionTracker, tmp19);
}
let closure_6 = fn(7930).MIN_QUEST_CONTENT_VISIBILITY_PERCENTAGE;
