...
// Dependencies: [32, 19, 1896, 11334, 1074, 21, 9006, 8393, 504, 11624, 11625, 5536, 2]
// Exports: BillableAdPlacementImpressionTrackerNative, QuestContentImpressionTrackerNative

// Module 11912 (QuestContentImpressionTracker)
...
function AdContentImpressionTrackerBaseNative(skipRemountKey) {
  ...
  let tmp6 = visibilityRef(8393)(tmp3);
  const adContentImpressionTrackerProps = adContentIds2(11624).useAdContentImpressionTrackerProps(skipRemountKey);
  ...
  const obj2 = adContentIds2(11624);
  let items1 = [joined];
  const tmp10 = _slicedToArray(adContentIds2(9006).useRecyclingState(false, items1), 2);
  dependencyMap = tmp11;
  ...
  const obj4 = adContentIds2(9006);
  const obj6 = {};
  const merged1 = Object.assign(skipRemountKey);
  const merged2 = Object.assign({ visible: overrideVisibility, visibleChanged: overrideVisibility !== tmp5(8393)(overrideVisibility) });
  obj6.focused = tmp3;
  ...
  if (tmp17) {
    obj7.adCreativeType = tmp(5536).AdCreativeType.QUEST;
    let tmp19 = obj7;
  } else {
    obj7.adCreativeType = skipRemountKey.adCreativeType;
    tmp19 = obj7;
  }
  return createElement(adContentIds2(11625).QuestContentImpressionTracker, tmp19);
}
let closure_6 = fn(11334).MIN_QUEST_CONTENT_VISIBILITY_PERCENTAGE;
