...
// Dependencies: [32, 19, 1896, 7848, 1074, 21, 8873, 8421, 504, 11640, 11641, 5539, 2]
// Exports: BillableAdPlacementImpressionTrackerNative, QuestContentImpressionTrackerNative

// Module 11929 (QuestContentImpressionTracker)
...
function AdContentImpressionTrackerBaseNative(skipRemountKey) {
  ...
  let tmp6 = visibilityRef(8421)(tmp3);
  const adContentImpressionTrackerProps = adContentIds2(11640).useAdContentImpressionTrackerProps(skipRemountKey);
  ...
  const obj2 = adContentIds2(11640);
  let items1 = [joined];
  const tmp10 = _slicedToArray(adContentIds2(8873).useRecyclingState(false, items1), 2);
  dependencyMap = tmp11;
  ...
  const obj4 = adContentIds2(8873);
  const obj6 = {};
  const merged1 = Object.assign(skipRemountKey);
  const merged2 = Object.assign({ visible: overrideVisibility, visibleChanged: overrideVisibility !== tmp5(8421)(overrideVisibility) });
  obj6.focused = tmp3;
  ...
  if (tmp17) {
    obj7.adCreativeType = tmp(5539).AdCreativeType.QUEST;
    let tmp19 = obj7;
  } else {
    obj7.adCreativeType = skipRemountKey.adCreativeType;
    tmp19 = obj7;
  }
  return createElement(adContentIds2(11641).QuestContentImpressionTracker, tmp19);
}
let closure_6 = fn(7848).MIN_QUEST_CONTENT_VISIBILITY_PERCENTAGE;
