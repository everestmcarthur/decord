...
// Dependencies: [32, 19, 17, 7754, 5494, 21, 4605, 576, 7760, 11348, 4303, 1114, 5647, 11555, 11568, 5497, 15170, 504, 11849, 7775, 7202, 15200, 15201, 11845, 11558, 7779, 11264, 11265, 11269, 5501, 15239, 15241, 15242, 7773, 8658, 4601, 2]
// Exports: default

// Module 15199 (QuestBottomSheet)
...
function useEnrolledQuestContentProps(quest) {
  ...
  _slicedToArray = quest(11558).useQuestImpressionId();
  let obj2 = quest(11558);
  const questTaskDetails = quest(11555).useQuestTaskDetails(quest);
  let obj3 = quest(11555);
  ...
  const tmpResult = quest(11555);
  const questTaskDetails1 = quest(11555).useQuestTaskDetails(quest);
  const tmpResult7 = quest(11555);
  let isQuestProgressing1 = quest(11555).useIsQuestProgressing(quest);
  const tmpResult8 = quest(11555);
  const tmp5Result = _slicedToArray(quest(11555).useTaskPlatformScreen(quest, questTaskDetails1), 3);
  ...
  obj7.onNext = onNext;
  items5[1] = obj7;
  const tmpResult11 = quest(15170);
  closure_130_0 = quest;
  [tmp32, tmp33] = items5;
  const tmp5Result6 = _slicedToArray(items5, 2);
  const questLogger = quest(7760).getQuestLogger({ quest, location: constants.QUEST_HOME_MOBILE });
  ...
  const obj10 = {
    ...
    start: noop.useCallback(() => {
      ...
      result.then((errorHints) => dependencyMap(errorHints.errorHints)).catch((error) => {
        ...
        obj2.icon = sourceQuestContent(5647);
        sourceQuestContent(4303).open(obj2);
      }).finally(() => impressionId(false));
    }, items6)
  };
  const tmp40 = null == completedAt1 && !isQuestProgressing && tmp6[0] === quest(5497).TaskPlatformScreen.CONSOLE && !hasWatchVideoOnMobileTasks;
  return {
    quest,
    defibrillator: obj10,
    step: tmp32,
    stepActions: tmp33,
    showMicrophone: null == completedAt1 && !isQuestProgressing && tmp6[0] === quest(5497).TaskPlatformScreen.CONSOLE && !hasWatchVideoOnMobileTasks,
    handleTaskSelect(arg0) {
      ...
      if (tmp4Result.shouldMigrateToAdAnalyticsInterface(tmp4(11264).AdAnalyticsInterfaceExperimentStep.STEP_2_CLICKED_INTERNAL, "quest_bottom_sheet")) {
        const obj = { type: tmp4(11269).AdUserActionType.CLICK_INTERNAL, adCreativeType: tmp4(5501).AdCreativeType.QUEST, adCreativeId: quest.id, questContentCTA: DESELECT_PLATFORM, surfaceId: tmp4(5497).QuestContent.QUEST_BOTTOM_SHEET, sourceQuestContent, impressionId };
        tmp4(11265).captureAdUserAction(obj);
        const tmp4Result2 = tmp4(11265);
      } else {
        const obj2 = { questId: quest.id, questContent: tmp4(5497).QuestContent.QUEST_BOTTOM_SHEET, questContentCTA: DESELECT_PLATFORM, sourceQuestContent };
        closure_2(obj2);
      }
      closure_4(arg0);
    }
...
function MicrophoneUnit(arg0) {
  ...
  if (num > 0) {
    ...
    const items1 = [closure_1_10(tmp2(8658).WarningIcon, obj4), ];
    ...
    items1[1] = closure_1_10(tmp2(4601).Text, obj6);
