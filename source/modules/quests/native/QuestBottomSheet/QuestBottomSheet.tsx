...
// Dependencies: [32, 19, 17, 7717, 5458, 21, 4574, 576, 7723, 11308, 4272, 1114, 5611, 11515, 11528, 5461, 15121, 504, 11809, 7738, 7164, 15151, 15152, 11805, 11518, 7742, 11224, 11225, 11229, 5465, 15190, 15192, 15193, 7736, 8621, 4570, 2]
// Exports: default

// Module 15150 (QuestBottomSheet)
...
import QuestBottomSheetHeaderDefault from "QuestBottomSheetHeader" /* 15151 */;
import QuestBottomSheetFooterDefault from "QuestBottomSheetFooter" /* 15152 */;
import QuestBottomSheetProgressCard from "QuestBottomSheetProgressCard" /* 15190 */;
...
function useEnrolledQuestContentProps(quest) {
  ...
  _slicedToArray = quest(11518).useQuestImpressionId();
  let obj2 = quest(11518);
  const questTaskDetails = quest(11515).useQuestTaskDetails(quest);
  let obj3 = quest(11515);
  ...
  const tmpResult = quest(11515);
  const questTaskDetails1 = quest(11515).useQuestTaskDetails(quest);
  const tmpResult7 = quest(11515);
  let isQuestProgressing1 = quest(11515).useIsQuestProgressing(quest);
  const tmpResult8 = quest(11515);
  const tmp5Result = _slicedToArray(quest(11515).useTaskPlatformScreen(quest, questTaskDetails1), 3);
  ...
  obj7.onNext = onNext;
  items5[1] = obj7;
  const tmpResult11 = quest(15121);
  closure_130_0 = quest;
  [tmp32, tmp33] = items5;
  const tmp5Result6 = _slicedToArray(items5, 2);
  const questLogger = quest(7723).getQuestLogger({ quest, location: constants.QUEST_HOME_MOBILE });
  ...
  const obj10 = {
    ...
    start: noop.useCallback(() => {
      ...
      result.then((errorHints) => dependencyMap(errorHints.errorHints)).catch((error) => {
        ...
        obj2.icon = sourceQuestContent(5611);
        sourceQuestContent(4272).open(obj2);
      }).finally(() => impressionId(false));
    }, items6)
  };
  const tmp40 = null == completedAt1 && !isQuestProgressing && tmp6[0] === quest(5461).TaskPlatformScreen.CONSOLE && !hasWatchVideoOnMobileTasks;
  return {
    quest,
    defibrillator: obj10,
    step: tmp32,
    stepActions: tmp33,
    showMicrophone: null == completedAt1 && !isQuestProgressing && tmp6[0] === quest(5461).TaskPlatformScreen.CONSOLE && !hasWatchVideoOnMobileTasks,
    handleTaskSelect(arg0) {
      ...
      if (tmp4Result.shouldMigrateToAdAnalyticsInterface(tmp4(11224).AdAnalyticsInterfaceExperimentStep.STEP_2_CLICKED_INTERNAL, "quest_bottom_sheet")) {
        const obj = { type: tmp4(11229).AdUserActionType.CLICK_INTERNAL, adCreativeType: tmp4(5465).AdCreativeType.QUEST, adCreativeId: quest.id, questContentCTA: DESELECT_PLATFORM, surfaceId: tmp4(5461).QuestContent.QUEST_BOTTOM_SHEET, sourceQuestContent, impressionId };
        tmp4(11225).captureAdUserAction(obj);
        const tmp4Result2 = tmp4(11225);
      } else {
        const obj2 = { questId: quest.id, questContent: tmp4(5461).QuestContent.QUEST_BOTTOM_SHEET, questContentCTA: DESELECT_PLATFORM, sourceQuestContent };
        closure_2(obj2);
      }
      closure_4(arg0);
    }
...
function MicrophoneUnit(arg0) {
  ...
  if (num > 0) {
    ...
    const items1 = [closure_1_10(tmp2(8621).WarningIcon, obj4), ];
    ...
    items1[1] = closure_1_10(tmp2(4570).Text, obj6);
