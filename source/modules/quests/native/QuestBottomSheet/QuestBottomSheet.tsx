...
// Dependencies: [32, 19, 17, 7899, 5614, 21, 4722, 576, 7905, 11528, 4420, 1115, 5767, 11729, 11742, 5617, 15297, 504, 12020, 7920, 7346, 15327, 15328, 12016, 11732, 7924, 7925, 7926, 7936, 5621, 15366, 15368, 15369, 7918, 8826, 4718, 2]
// Exports: default

// Module 15326 (QuestBottomSheet)
...
import QuestBottomSheetHeaderDefault from "QuestBottomSheetHeader" /* 15327 */;
import QuestBottomSheetFooterDefault from "QuestBottomSheetFooter" /* 15328 */;
import QuestBottomSheetProgressCard from "QuestBottomSheetProgressCard" /* 15366 */;
...
function useEnrolledQuestContentProps(quest) {
  ...
  _slicedToArray = quest(11732).useQuestImpressionId();
  let obj2 = quest(11732);
  const questTaskDetails = quest(11729).useQuestTaskDetails(quest);
  let obj3 = quest(11729);
  const isQuestProgressing = quest(11729).useIsQuestProgressing(quest);
  const obj4 = quest(11729);
  let tmp6 = _slicedToArray(quest(11729).useTaskPlatformScreen(quest, questTaskDetails), 3);
  noop = tmp6[2];
  const obj5 = quest(11729);
  closure_129_0 = quest;
  ...
  let completedAt;
  const hasWatchVideoOnMobileTasks = quest(15297).useHasWatchVideoOnMobileTasks(quest.config);
  if (userStatus != null) {
    completedAt = userStatus.completedAt;
  }
  const obj6 = quest(15297);
  const xboxAndPlaystationAccounts = quest(11729).useConnectedAccounts().xboxAndPlaystationAccounts;
  ...
  const tmpResult = quest(11729);
  const questTaskDetails1 = quest(11729).useQuestTaskDetails(quest);
  const tmpResult7 = quest(11729);
  let isQuestProgressing1 = quest(11729).useIsQuestProgressing(quest);
  const tmpResult8 = quest(11729);
  const tmp5Result = _slicedToArray(quest(11729).useTaskPlatformScreen(quest, questTaskDetails1), 3);
  ...
  if (tmp16) {
    tmp16 = first === tmp(5617).TaskPlatformScreen.CONSOLE;
  }
  if (!tmp16) {
  ...
  tmp21 = 0 !== memo.length || null != completedAt || first !== quest(5617).TaskPlatformScreen.CONSOLE || first1;
  const hasWatchVideoOnMobileTasks1 = quest(15297).useHasWatchVideoOnMobileTasks(quest.config);
  closure_129_14 = hasWatchVideoOnMobileTasks1;
  const tmpResult10 = quest(15297);
  const isMobileActivityQuest = quest(15297).useMobileActivityQuest(quest).isMobileActivityQuest;
  ...
  obj7.onNext = onNext;
  items5[1] = obj7;
  const tmpResult11 = quest(15297);
  closure_130_0 = quest;
  [tmp32, tmp33] = items5;
  const tmp5Result6 = _slicedToArray(items5, 2);
  const questLogger = quest(7905).getQuestLogger({ quest, location: constants.QUEST_HOME_MOBILE });
  ...
  const obj10 = {
    ...
    start: noop.useCallback(() => {
      ...
      result.then((errorHints) => dependencyMap(errorHints.errorHints)).catch((error) => {
        ...
        obj2.icon = sourceQuestContent(5767);
        sourceQuestContent(4420).open(obj2);
      }).finally(() => impressionId(false));
    }, items6)
  };
  const tmp40 = null == completedAt1 && !isQuestProgressing && tmp6[0] === quest(5617).TaskPlatformScreen.CONSOLE && !hasWatchVideoOnMobileTasks;
  return {
    quest,
    defibrillator: obj10,
    step: tmp32,
    stepActions: tmp33,
    showMicrophone: null == completedAt1 && !isQuestProgressing && tmp6[0] === quest(5617).TaskPlatformScreen.CONSOLE && !hasWatchVideoOnMobileTasks,
    handleTaskSelect(arg0) {
      ...
      if (tmp4Result.shouldMigrateToAdAnalyticsInterface(tmp4(7925).AdAnalyticsInterfaceExperimentStep.STEP_2_CLICKED_INTERNAL, "quest_bottom_sheet")) {
        const obj = { type: tmp4(7936).AdUserActionType.CLICK_INTERNAL, adCreativeType: tmp4(5621).AdCreativeType.QUEST, adCreativeId: quest.id, questContentCTA: DESELECT_PLATFORM, surfaceId: tmp4(5617).QuestContent.QUEST_BOTTOM_SHEET, sourceQuestContent, impressionId };
        tmp4(7926).captureAdUserAction(obj);
        const tmp4Result2 = tmp4(7926);
      } else {
        const obj2 = { questId: quest.id, questContent: tmp4(5617).QuestContent.QUEST_BOTTOM_SHEET, questContentCTA: DESELECT_PLATFORM, sourceQuestContent };
        closure_2(obj2);
      }
      closure_4(arg0);
    }
...
function MicrophoneUnit(arg0) {
  ...
  if (num > 0) {
    ...
    const items1 = [closure_1_10(tmp2(8826).WarningIcon, obj4), ];
    ...
    items1[1] = closure_1_10(tmp2(4718).Text, obj6);
