...
// Dependencies: [19, 17, 1372, 21, 4640, 576, 11637, 11650, 504, 11444, 7836, 1115, 4636, 5540, 5695, 15243, 11439, 15235, 11918, 15230, 5210, 8456, 5074, 5675, 15273, 8423, 2]
...
// Module 15272 (QuestBottomSheetProgressCard)
...
import QuestProgressIndicatorDefault from "QuestProgressIndicator" /* 15243 */;
import QuestDockBlurredContentBackgroundDefault from "QuestDockBlurredContentBackground" /* 15273 */;
...
export const QuestBottomSheetProgressCardPlayStreamTask = function QuestBottomSheetProgressCardPlayStreamTask(quest) {
  ...
  c8 = undefined;
  const tmp = closure_9();
  questTaskDetails = questTaskDetails(11637).useQuestTaskDetails(quest);
  let obj = questTaskDetails(11637);
  let isQuestProgressing = questTaskDetails(11637).useIsQuestProgressing(quest);
  ...
  noop = tmp9;
  let obj2 = questTaskDetails(11637);
  const result = questTaskDetails(11650).supportedTaskPlatforms(quest);
  c4 = result;
  const tmp2Result = questTaskDetails(11650);
  questFormattedDate = questTaskDetails(11637).useQuestFormattedDate(quest.config.rewardsConfig.rewardsExpireAt);
  gameTitle = quest.config.messages.gameTitle;
  const tmp2Result5 = questTaskDetails(11637);
  ...
  defaultRewardName = questTaskDetails(11444).getDefaultRewardName(quest.config, stateFromStores);
  const tmp2Result7 = questTaskDetails(11444);
  const isSponsoredPlayQuestResult = questTaskDetails(7836).isSponsoredPlayQuest(quest);
  ...
  const memo = noop.useMemo(() => {
    if (dependencyMap) {
      if (!closure_3) {
        ...
        const obj = {
          rewardHook() {
                return defaultRewardName(questTaskDetails(4636).Text, { variant: "text-sm/semibold", color: "text-strong", children });
              },
    ...
    if (isQuestProgressing) {
      ...
      const obj2 = {
        ...
        minutesHook(children) {
            return children(questTaskDetails(4636).Text, { variant: "text-sm/semibold", color: "text-strong", children });
          }
      };
  ...
  if (isQuestProgressing) {
    ...
    obj7.children = tmp18(tmp2(4636).Text, obj8);
    isQuestProgressing = tmp18(tmp17, obj7);
  }
  items3[1] = isQuestProgressing;
  obj3.children = items3;
  return c8(questTaskDetails(5695).Card, obj3);
};
export const QuestBottomSheetProgressCardWatchTask = function QuestBottomSheetProgressCardWatchTask(quest) {
  ...
  const obj = quest(11637);
  let isHeroVideoSupportedResult = null != memo;
  const obj2 = quest(15235);
  const obj3 = { questId: quest.id, sourceQuestContent: quest.sourceQuestContent };
  if (isHeroVideoSupportedResult) {
    isHeroVideoSupportedResult = tmp2(11918).isHeroVideoSupported(memo.mimetype);
    const tmp2Result = tmp2(11918);
  }
  const userStatus = quest.userStatus;
  let completedAt;
  if (userStatus != null) {
  ...
  const watchTaskPressHandler = quest(15235).useWatchTaskPressHandler({ questId: quest.id, sourceQuestContent: quest.sourceQuestContent });
  ...
  if (isHeroVideoSupportedResult) {
    ...
    obj7.source = obj8;
    const items4 = [tmp14(tmp2(8456).VideoComponent, obj7), ];
  ...
  tmp2Result2 = quest(11637);
  items7[1] = closure_7(quest(8423).PlayIcon, { color: nativeDefault.colors.WHITE });
  ...
  obj4.children = closure_8(quest(5695).Card, obj5);
  return closure_7(quest(5210).PressableOpacity, obj4);
};
