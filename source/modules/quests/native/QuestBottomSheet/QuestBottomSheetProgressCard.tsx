...
// Dependencies: [19, 17, 1371, 21, 4606, 576, 11577, 11590, 504, 11384, 7794, 1114, 4602, 5503, 5658, 15189, 11379, 15181, 11860, 15176, 5174, 8393, 5038, 5638, 15219, 8360, 2]
...
export const QuestBottomSheetProgressCardPlayStreamTask = function QuestBottomSheetProgressCardPlayStreamTask(quest) {
  ...
  c8 = undefined;
  const tmp = closure_9();
  questTaskDetails = questTaskDetails(11577).useQuestTaskDetails(quest);
  let obj = questTaskDetails(11577);
  let isQuestProgressing = questTaskDetails(11577).useIsQuestProgressing(quest);
  ...
  noop = tmp9;
  let obj2 = questTaskDetails(11577);
  const result = questTaskDetails(11590).supportedTaskPlatforms(quest);
  c4 = result;
  const tmp2Result = questTaskDetails(11590);
  questFormattedDate = questTaskDetails(11577).useQuestFormattedDate(quest.config.rewardsConfig.rewardsExpireAt);
  gameTitle = quest.config.messages.gameTitle;
  const tmp2Result5 = questTaskDetails(11577);
  ...
  defaultRewardName = questTaskDetails(11384).getDefaultRewardName(quest.config, stateFromStores);
  const tmp2Result7 = questTaskDetails(11384);
  const isSponsoredPlayQuestResult = questTaskDetails(7794).isSponsoredPlayQuest(quest);
  ...
  const memo = noop.useMemo(() => {
    if (dependencyMap) {
      if (!closure_3) {
        ...
        const obj = {
          rewardHook() {
                return defaultRewardName(questTaskDetails(4602).Text, { variant: "text-sm/semibold", color: "text-strong", children });
              },
    ...
    if (isQuestProgressing) {
      ...
      const obj2 = {
        ...
        minutesHook(children) {
            return children(questTaskDetails(4602).Text, { variant: "text-sm/semibold", color: "text-strong", children });
          }
      };
  ...
  const tmp2Result8 = questTaskDetails(7794);
  if (!tmp7) {
  ...
  if (tmp18Result) {
    ...
    tmp18Result = tmp18(tmp2(4602).Text, obj6);
  }
  ...
  if (isQuestProgressing) {
    ...
    obj7.children = tmp18(tmp2(4602).Text, obj8);
    isQuestProgressing = tmp18(tmp17, obj7);
  }
  items3[1] = isQuestProgressing;
  obj3.children = items3;
  return c8(questTaskDetails(5658).Card, obj3);
};
export const QuestBottomSheetProgressCardWatchTask = function QuestBottomSheetProgressCardWatchTask(quest) {
  ...
  if (isHeroVideoSupportedResult) {
    isHeroVideoSupportedResult = tmp2(11860).isHeroVideoSupported(memo.mimetype);
    const tmp2Result = tmp2(11860);
  }
  const userStatus = quest.userStatus;
  let completedAt;
  if (userStatus != null) {
  ...
  if (isHeroVideoSupportedResult) {
    ...
    obj7.source = obj8;
    const items4 = [tmp14(tmp2(8393).VideoComponent, obj7), ];
  ...
  tmp2Result2 = quest(11577);
  items7[1] = closure_7(quest(8360).PlayIcon, { color: nativeDefault.colors.WHITE });
  ...
  obj4.children = closure_8(quest(5658).Card, obj5);
  return closure_7(quest(5174).PressableOpacity, obj4);
};
