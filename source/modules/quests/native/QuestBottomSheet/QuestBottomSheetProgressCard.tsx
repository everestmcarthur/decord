...
// Dependencies: [19, 17, 1371, 21, 4607, 576, 11579, 11592, 504, 11386, 7795, 1114, 4603, 5504, 5659, 15189, 11381, 15181, 11862, 15176, 5175, 8395, 5039, 5639, 15219, 8362, 2]
...
// Module 15218 (QuestBottomSheetProgressCard)
...
import QuestProgressIndicatorDefault from "QuestProgressIndicator" /* 15189 */;
...
export const QuestBottomSheetProgressCardPlayStreamTask = function QuestBottomSheetProgressCardPlayStreamTask(quest) {
  ...
  c8 = undefined;
  const tmp = closure_9();
  questTaskDetails = questTaskDetails(11579).useQuestTaskDetails(quest);
  let obj = questTaskDetails(11579);
  let isQuestProgressing = questTaskDetails(11579).useIsQuestProgressing(quest);
  ...
  noop = tmp9;
  let obj2 = questTaskDetails(11579);
  const result = questTaskDetails(11592).supportedTaskPlatforms(quest);
  c4 = result;
  const tmp2Result = questTaskDetails(11592);
  questFormattedDate = questTaskDetails(11579).useQuestFormattedDate(quest.config.rewardsConfig.rewardsExpireAt);
  gameTitle = quest.config.messages.gameTitle;
  const tmp2Result5 = questTaskDetails(11579);
  ...
  defaultRewardName = questTaskDetails(11386).getDefaultRewardName(quest.config, stateFromStores);
  const tmp2Result7 = questTaskDetails(11386);
  const isSponsoredPlayQuestResult = questTaskDetails(7795).isSponsoredPlayQuest(quest);
  ...
  const memo = noop.useMemo(() => {
    if (dependencyMap) {
      if (!closure_3) {
        ...
        const obj = {
          rewardHook() {
                return defaultRewardName(questTaskDetails(4603).Text, { variant: "text-sm/semibold", color: "text-strong", children });
              },
    ...
    if (isQuestProgressing) {
      ...
      const obj2 = {
        ...
        minutesHook(children) {
            return children(questTaskDetails(4603).Text, { variant: "text-sm/semibold", color: "text-strong", children });
          }
      };
  ...
  if (isQuestProgressing) {
    ...
    obj7.children = tmp18(tmp2(4603).Text, obj8);
    isQuestProgressing = tmp18(tmp17, obj7);
  }
  items3[1] = isQuestProgressing;
  obj3.children = items3;
  return c8(questTaskDetails(5659).Card, obj3);
};
export const QuestBottomSheetProgressCardWatchTask = function QuestBottomSheetProgressCardWatchTask(quest) {
  ...
  const obj = quest(11579);
  let isHeroVideoSupportedResult = null != memo;
  const obj2 = quest(15181);
  const obj3 = { questId: quest.id, sourceQuestContent: quest.sourceQuestContent };
  if (isHeroVideoSupportedResult) {
    isHeroVideoSupportedResult = tmp2(11862).isHeroVideoSupported(memo.mimetype);
    const tmp2Result = tmp2(11862);
  }
  const userStatus = quest.userStatus;
  let completedAt;
  if (userStatus != null) {
  ...
  const watchTaskPressHandler = quest(15181).useWatchTaskPressHandler({ questId: quest.id, sourceQuestContent: quest.sourceQuestContent });
  ...
  if (isHeroVideoSupportedResult) {
    ...
    obj7.source = obj8;
    const items4 = [tmp14(tmp2(8395).VideoComponent, obj7), ];
  ...
  tmp2Result2 = quest(11579);
  items7[1] = closure_7(quest(8362).PlayIcon, { color: nativeDefault.colors.WHITE });
  ...
  obj4.children = closure_8(quest(5659).Card, obj5);
  return closure_7(quest(5175).PressableOpacity, obj4);
};
