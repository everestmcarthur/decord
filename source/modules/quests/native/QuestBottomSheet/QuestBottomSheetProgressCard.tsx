...
// Dependencies: [19, 17, 1371, 21, 4605, 576, 11555, 11568, 504, 11362, 7773, 1114, 4601, 5502, 5657, 15210, 11357, 15202, 11838, 15197, 5173, 8372, 5037, 5637, 15240, 8339, 2]
...
export const QuestBottomSheetProgressCardPlayStreamTask = function QuestBottomSheetProgressCardPlayStreamTask(quest) {
  ...
  c8 = undefined;
  const tmp = closure_9();
  questTaskDetails = questTaskDetails(11555).useQuestTaskDetails(quest);
  let obj = questTaskDetails(11555);
  let isQuestProgressing = questTaskDetails(11555).useIsQuestProgressing(quest);
  ...
  noop = tmp9;
  let obj2 = questTaskDetails(11555);
  const result = questTaskDetails(11568).supportedTaskPlatforms(quest);
  c4 = result;
  const tmp2Result = questTaskDetails(11568);
  questFormattedDate = questTaskDetails(11555).useQuestFormattedDate(quest.config.rewardsConfig.rewardsExpireAt);
  gameTitle = quest.config.messages.gameTitle;
  const tmp2Result5 = questTaskDetails(11555);
  ...
  defaultRewardName = questTaskDetails(11362).getDefaultRewardName(quest.config, stateFromStores);
  const tmp2Result7 = questTaskDetails(11362);
  const isSponsoredPlayQuestResult = questTaskDetails(7773).isSponsoredPlayQuest(quest);
  ...
  const memo = noop.useMemo(() => {
    if (dependencyMap) {
      if (!closure_3) {
        ...
        const obj = {
          rewardHook() {
                return defaultRewardName(questTaskDetails(4601).Text, { variant: "text-sm/semibold", color: "text-strong", children });
              },
    ...
    if (isQuestProgressing) {
      ...
      const obj2 = {
        ...
        minutesHook(children) {
            return children(questTaskDetails(4601).Text, { variant: "text-sm/semibold", color: "text-strong", children });
          }
      };
  ...
  if (isQuestProgressing) {
    ...
    obj7.children = tmp18(tmp2(4601).Text, obj8);
    isQuestProgressing = tmp18(tmp17, obj7);
  }
  items3[1] = isQuestProgressing;
  obj3.children = items3;
  return c8(questTaskDetails(5657).Card, obj3);
};
export const QuestBottomSheetProgressCardWatchTask = function QuestBottomSheetProgressCardWatchTask(quest) {
  ...
  const obj = quest(11555);
  let isHeroVideoSupportedResult = null != memo;
  const obj2 = quest(15202);
  const obj3 = { questId: quest.id, sourceQuestContent: quest.sourceQuestContent };
  if (isHeroVideoSupportedResult) {
    isHeroVideoSupportedResult = tmp2(11838).isHeroVideoSupported(memo.mimetype);
    const tmp2Result = tmp2(11838);
  }
  const userStatus = quest.userStatus;
  let completedAt;
  if (userStatus != null) {
  ...
  const watchTaskPressHandler = quest(15202).useWatchTaskPressHandler({ questId: quest.id, sourceQuestContent: quest.sourceQuestContent });
  ...
  if (isHeroVideoSupportedResult) {
    ...
    obj7.source = obj8;
    const items4 = [tmp14(tmp2(8372).VideoComponent, obj7), ];
  ...
  tmp2Result2 = quest(11555);
  items7[1] = closure_7(quest(8339).PlayIcon, { color: nativeDefault.colors.WHITE });
  ...
  obj4.children = closure_8(quest(5657).Card, obj5);
  return closure_7(quest(5173).PressableOpacity, obj4);
};
