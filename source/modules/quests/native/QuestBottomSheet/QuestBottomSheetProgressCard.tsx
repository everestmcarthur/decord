...
// Dependencies: [19, 17, 1371, 21, 4574, 576, 11515, 11528, 504, 11322, 7736, 1114, 4570, 5466, 5621, 15161, 11317, 15153, 11798, 15148, 5137, 8335, 5001, 5601, 15191, 8302, 2]
...
// Module 15190 (QuestBottomSheetProgressCard)
...
import QuestDockBlurredContentBackgroundDefault from "QuestDockBlurredContentBackground" /* 15191 */;
...
export const QuestBottomSheetProgressCardPlayStreamTask = function QuestBottomSheetProgressCardPlayStreamTask(quest) {
  ...
  c8 = undefined;
  const tmp = closure_9();
  questTaskDetails = questTaskDetails(11515).useQuestTaskDetails(quest);
  let obj = questTaskDetails(11515);
  let isQuestProgressing = questTaskDetails(11515).useIsQuestProgressing(quest);
  ...
  noop = tmp9;
  let obj2 = questTaskDetails(11515);
  const result = questTaskDetails(11528).supportedTaskPlatforms(quest);
  c4 = result;
  const tmp2Result = questTaskDetails(11528);
  questFormattedDate = questTaskDetails(11515).useQuestFormattedDate(quest.config.rewardsConfig.rewardsExpireAt);
  gameTitle = quest.config.messages.gameTitle;
  const tmp2Result5 = questTaskDetails(11515);
  ...
  defaultRewardName = questTaskDetails(11322).getDefaultRewardName(quest.config, stateFromStores);
  const tmp2Result7 = questTaskDetails(11322);
  const isSponsoredPlayQuestResult = questTaskDetails(7736).isSponsoredPlayQuest(quest);
  ...
  const memo = noop.useMemo(() => {
    if (dependencyMap) {
      if (!closure_3) {
        ...
        const obj = {
          rewardHook() {
                return defaultRewardName(questTaskDetails(4570).Text, { variant: "text-sm/semibold", color: "text-strong", children });
              },
    ...
    if (isQuestProgressing) {
      ...
      const obj2 = {
        ...
        minutesHook(children) {
            return children(questTaskDetails(4570).Text, { variant: "text-sm/semibold", color: "text-strong", children });
          }
      };
  ...
  if (isQuestProgressing) {
    ...
    obj8.children = intl.format(tmp2(1114).t.lIFg6I, obj9);
    obj7.children = tmp18(tmp2(4570).Text, obj8);
    isQuestProgressing = tmp18(tmp17, obj7);
  }
  items3[1] = isQuestProgressing;
  obj3.children = items3;
  return c8(questTaskDetails(5621).Card, obj3);
};
export const QuestBottomSheetProgressCardWatchTask = function QuestBottomSheetProgressCardWatchTask(quest) {
  ...
  const obj = quest(11515);
  let isHeroVideoSupportedResult = null != memo;
  const obj2 = quest(15153);
  const obj3 = { questId: quest.id, sourceQuestContent: quest.sourceQuestContent };
  if (isHeroVideoSupportedResult) {
    isHeroVideoSupportedResult = tmp2(11798).isHeroVideoSupported(memo.mimetype);
    const tmp2Result = tmp2(11798);
  }
  const userStatus = quest.userStatus;
  let completedAt;
  if (userStatus != null) {
  ...
  const watchTaskPressHandler = quest(15153).useWatchTaskPressHandler({ questId: quest.id, sourceQuestContent: quest.sourceQuestContent });
  ...
  if (isHeroVideoSupportedResult) {
    ...
    obj7.source = obj8;
    const items4 = [tmp14(tmp2(8335).VideoComponent, obj7), ];
  ...
  tmp2Result2 = quest(11515);
  items7[1] = closure_7(quest(8302).PlayIcon, { color: nativeDefault.colors.WHITE });
  ...
  obj4.children = closure_8(quest(5621).Card, obj5);
  return closure_7(quest(5137).PressableOpacity, obj4);
};
