...
// Dependencies: [32, 19, 17, 4631, 1372, 7808, 7258, 21, 576, 4639, 5532, 11621, 11428, 11433, 11181, 504, 4341, 7659, 11901, 15214, 5059, 11651, 11634, 15182, 11909, 15209, 15211, 8205, 1115, 5713, 11908, 11624, 15249, 11330, 11331, 11335, 5536, 7833, 1612, 1478, 4376, 4640, 5064, 2]
// Exports: default

// Module 15213 (QuestBottomSheetFooter)
...
import ContentImpressionTrackerHooks from "ContentImpressionTrackerHooks" /* 11624 */;
...
import AnalyticsHooks from "AnalyticsHooks" /* 11908 */;
import QuestBottomSheetHooks from "QuestBottomSheetHooks" /* 15214 */;
import RefreshIcon from "RefreshIcon" /* 15249 */;
  ...
  let obj3 = {
    ...
    onPress(arg0) {
      if (obj.shouldMigrateToAdAnalyticsInterface(AdAnalyticsInterfaceExperiment.AdAnalyticsInterfaceExperimentStep.STEP_2_CLICKED_INTERNAL, "quest_bottom_sheet_footer")) {
        const obj2 = { type: tmp(11335).AdUserActionType.CLICK_INTERNAL, adCreativeType: tmp(5536).AdCreativeType.QUEST, adCreativeId: questId, questContentCTA: tmp(7833).QuestContentCTA.DEFIBRILLATOR, surfaceId: tmp(5532).QuestContent.QUEST_BOTTOM_SHEET, sourceQuestContent, impressionId };
        tmp(11331).captureAdUserAction(obj2);
        const tmpResult = tmp(11331);
      } else {
        const obj3 = { questId, questContent: tmp(5532).QuestContent.QUEST_BOTTOM_SHEET, questContentCTA: tmp(7833).QuestContentCTA.DEFIBRILLATOR, sourceQuestContent };
        closure_3(obj3);
      }
...
function ClaimButton(arg0) {
  ...
  let obj3 = {
    ...
    onPress() {
      if (obj.shouldMigrateToAdAnalyticsInterface(AdAnalyticsInterfaceExperiment.AdAnalyticsInterfaceExperimentStep.STEP_2_CLICKED_INTERNAL, "quest_bottom_sheet_footer")) {
        const obj2 = { type: tmp(11335).AdUserActionType.CLICK_INTERNAL, adCreativeType: tmp(5536).AdCreativeType.QUEST, adCreativeId: questId, questContentCTA: tmp(7833).QuestContentCTA.CLAIM_REWARD, surfaceId: tmp(5532).QuestContent.QUEST_BOTTOM_SHEET, sourceQuestContent, impressionId };
        tmp(11331).captureAdUserAction(obj2);
        const tmpResult = tmp(11331);
      } else {
        const obj3 = { questId, questContent: tmp(5532).QuestContent.QUEST_BOTTOM_SHEET, questContentCTA: tmp(7833).QuestContentCTA.CLAIM_REWARD, sourceQuestContent };
        closure_3(obj3);
      }
      importDefault();
    },
...
export default function QuestBottomSheetFooter(quest) {
  ...
  const mobileActivityPressHandler = quest(15214).useMobileActivityPressHandler({ questId: quest.id, sourceQuestContent, launchMobileActivity });
  ...
  if (step !== quest(15211).QuestBottomSheetStep.TASK_SELECT) {
    ...
    if (tmp2(15211).QuestBottomSheetStep.CONSOLE_CONNECT === step) {
      ...
      if (tmp2(15211).QuestBottomSheetStep.TASK_STATUS === step) {
        if (tmp11) {
          ...
          const obj15 = { grow: true, size: "lg", onPress: mobileActivityPressHandler, text: primaryCtaCopy, icon: tmp2(11901).getPrimaryCtaIcon(quest) };
          let tmp23 = null;
          if (isQuestAccessSuspended) {
            tmp23 = obj10;
          }
          const merged2 = Object.assign(tmp23);
          tmp40Result4 = tmp40(tmp2(5059).Button, obj15);
          const tmp2Result2 = tmp2(11901);
        } else {
          if (_slicedToArray(obj3.useTaskPlatformScreen(quest, questTaskDetails), 1)[0] === tmp2(5532).TaskPlatformScreen.CONSOLE) {
    ...
    if (tmp40Result5) {
      ...
      obj18.icon = tmp40(tmp2(5713).ArrowLargeLeftIcon, {});
      obj18.onPress = onBack;
      tmp40Result5 = tmp40(tmp2(8205).IconButton, obj18);
    }
