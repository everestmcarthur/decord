...
// Function ID: 13298
// Name: QuestEmbed
// Dependencies: [17, 1183, 7754, 5494, 1074, 7787, 1115, 1114, 7775, 11846, 5497, 4456, 11357, 1608, 13298, 7750, 7769, 7773, 7997, 2]
// Exports: createQuestsEmbed

// Module 13297 (QuestEmbed)
import _mod17 from "module_17" /* 17 */;
import Constants from "Constants" /* 1074 */;
import Constants2 from "Constants" /* 7787 */;
...
export const createQuestsEmbed = function createQuestsEmbed(questId) {
  ...
  if (obj.isMetaQuest()) {
    ...
    obj2.thumbnailUrl = Image.resolveAssetSource(tmp(13298)).uri;
    ...
    const result = tmp2(7750).findQuestOrReplacement(questId, QuestStore.quests, QuestStore.excludedQuests);
    ...
    if (null == result) {
      if (null == value) {
        const result1 = tmp2(7769).trackQuestEmbedFallbackViewed(questId, constants2.UNKNOWN_QUEST);
        const intl12 = tmp2(1114).intl;
        const tmp2Result18 = tmp2(7769);
        ...
        obj8.thumbnailUrl = Image.resolveAssetSource(tmp(13298)).uri;
      ...
      const result2 = tmp2(7769).trackQuestEmbedFallbackViewed(questId, constants2.EXCLUDED_QUEST);
      const intl7 = tmp2(1114).intl;
      const tmp2Result20 = tmp2(7769);
      ...
      obj10.thumbnailUrl = Image.resolveAssetSource(tmp(13298)).uri;
      ...
      if (tmp2Result22.isShareableQuest(result.config)) {
        ...
        const tmp2Result23 = tmp2(4456);
        const intl3 = tmp2(1114).intl;
        const obj12 = { questName: result.config.messages.questName };
        const gamePublisher = result.config.messages.gamePublisher;
        const _Date2 = Date;
        const tmp29 = tmp2(4456).isThemeDark(ThemeStore.getState().theme) ? ThemeTypes.DARK : ThemeTypes.LIGHT;
        ...
        if (result.config.expiresAt < date1.toISOString()) {
          ...
          const questTaskDetails = tmp2(7775).getQuestTaskDetails(result);
          const tmp2Result24 = tmp2(7775);
          const thirdPartyTaskDetails = tmp2(7775).getThirdPartyTaskDetails(result);
          const tmp2Result25 = tmp2(7775);
          const obj15 = { quest: result, taskDetails: questTaskDetails, location: constants.EMBED_MOBILE, sourceQuestContent: tmp2(5497).QuestContent.QUEST_EMBED_MOBILE, thirdPartyTaskDetails, withoutMarkdown: true, currentUser };
          formatToPlainStringResult1 = tmp2(11846).getQuestsInstructionsToWinReward(obj15);
          const tmp2Result26 = tmp2(11846);
        }
        ...
        obj16.thumbnailUrl = tmp2(11357).getQuestAsset(result, tmp2(11357).QuestAssetType.GAME_TILE, tmp29).url;
        obj16.embedCanBeTapped = true;
        obj16.canBeAccepted = true;
        obj16.type = InviteTypes.GUILD;
        const tmp2Result27 = tmp2(11357);
        const obj17 = {};
        ...
        const result3 = tmp2(7769).trackQuestEmbedFallbackViewed(questId, constants2.NOT_SHAREABLE_QUEST);
        const intl = tmp2(1114).intl;
        const tmp2Result29 = tmp2(7769);
        ...
        obj20.thumbnailUrl = Image.resolveAssetSource(tmp(13298)).uri;
      ...
      tmp2Result22 = tmp2(7773);
    }
    const tmp2Result16 = tmp2(7750);
  }
};
