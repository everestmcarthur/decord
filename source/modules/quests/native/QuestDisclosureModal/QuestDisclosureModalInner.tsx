...
export default function QuestDisclosureModalInner(isTargetedDisclosure) {
  ...
  if (setting) {
    obj.icon = tmp5(tmp2(9415).ServerIcon, { size: "xs" });
    ...
    obj.icon = tmp5(tmp2(9015).GlobeEarthIcon, { size: "xs" });
    ...
    items1[1] = obj2;
    const obj3 = { icon: tmp5(tmp2(9352).GameControllerIcon, { size: "xs" }), text: null };
  ...
  if (isTargetedDisclosure) {
    ...
    isTargetedDisclosure = tmp6(tmp2(5688).Card, obj7);
  }
