...
// Function ID: 15204
// Name: QuestHooks
// Dependencies: [5, 19, 4330, 4868, 7816, 7817, 5532, 11433, 9665, 1085, 15204, 504, 5535, 15228, 7815, 7813, 5539, 10239, 4497, 15214, 7057, 11637, 11638, 11439, 7838, 7276, 9660, 7836, 9683, 7277, 9661, 11673, 2]
...
import useQuestForPlacement from "useQuestForPlacement" /* 15228 */;
...
function useIsMobileQuestDockRenderedBase(mobileQuestDock) {
  ...
  const tmp4 = deliveredAdCreativeId(7057)();
  ...
  if (isDismissedResult) {
    isDismissedResult = tmp(7813).isDismissed(deliveredQuest.userStatus, tmp(5535).QuestContent.QUEST_BAR_MOBILE);
  ...
  } else if (tmp(5539).AdCreativeType.QUEST === type) {
    if (stateFromStores) {
      if (!tmp10) {
...
const CAPTCHA_MODAL_KEY = fn(11433).CAPTCHA_MODAL_KEY;
...
export const useQuestDockHeroAsset = function useQuestDockHeroAsset(quest) {
  ...
  return noop.useMemo(() => {
    ...
    if (null != quest.config.assets.questBarHeroVideo) {
      videoAsset = tmp(11439).resolveAsset(tmp3.id, tmp3.config.assets.questBarHeroVideo);
      const tmpResult = tmp(11439);
    }
    if (questAsset.isAnimated) {
      let staticUrl = str.replace(tmp(11439).EXTENSION_RE, ".png");
    } else {
      staticUrl = str;
    }
          ...
          if (0 === v2) {
            if (arg0 === 1) {
              ...
              if (canLaunchFrameResult) {
                ...
                v2 = 1;
                v3 = 1;
                const obj8 = { value: v2(9661).launchFrame(obj5), done: false };
                return obj8;
              } else {
                let id;
                if (stateFromStores != null) {
                ...
                if (null != id) {
                  ...
                  v2 = 2;
                  v3 = 1;
                  const obj10 = { value: v3(11673).launchActivityInBotDM(obj9), done: false };
                  return obj10;
                }
              }
            }
