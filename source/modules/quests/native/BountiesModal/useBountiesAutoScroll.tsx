...
// Dependencies: [32, 19, 17, 5495, 11615, 15067, 4343, 4607, 4610, 4571, 2]
...
export const useBountiesAutoScroll = function useBountiesAutoScroll(listRef) {
  ...
  const config = BountiesVerticalScrollExperiment.useConfig({ location: slotHeight.VIDEO_MODAL_MOBILE });
  ...
  callback5 = obj3.useCallback(() => {
    ...
    if (!current) {
      ...
      closure_20.current = setTimeout(() => {
        closure_1_20.current = null;
        current = ref.current;
        if (!current) {
          current = ref2.current;
        }
        if (!current) {
          const result = sharedValue.set(listRef(enabled[7]).withTiming(0.83, listRef(enabled[8]).timingSlow));
          closure_1_12(true);
  ...
  callback12 = obj3.useCallback(() => {
    ...
    if (current) {
      current = !BrowserManager.getIsInAppBrowserOpen();
    }
    if (current) {
      ref5.current = false;
      callback10();
  ...
  const callback13 = obj3.useCallback((arg0) => {
    ...
    if (!current2) {
      ...
      _undefined(false);
      _undefined2(0);
      const result = sharedValue.set(timing.withTiming(1, timingPresets.timingStandard));
      if ("countdown" === mode) {
  ...
  const tmp18 = mode(activeIndex.useState(activeIndex), 2);
  return {
    visible: tmp6,
    peekActive: tmp7[0],
    progress: tmp9,
