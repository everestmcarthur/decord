...
// Dependencies: [19, 17, 4566, 5458, 21, 4574, 576, 4575, 4578, 11553, 15070, 1114, 4310, 5001, 15073, 15075, 4570, 10212, 15037, 504, 15038, 10001, 2]
// Exports: default

// Module 15072 (BountiesScrollEndCard)
...
import BountiesModalTransitionsRefactorExperiment from "BountiesModalTransitionsRefactorExperiment" /* 15037 */;
import useVisibilityTransition from "useVisibilityTransition" /* 15038 */;
...
function BountiesScrollEndCardContent(isScrollingInBoundsSharedValue) {
  ...
  const isBountiesVerticalScrollExperimentEnabled = isActive(11553).useIsBountiesVerticalScrollExperimentEnabled("BountiesScrollEndCard");
  let obj = isActive(11553);
  const bountyCtaInfo = isActive(15070).getBountyCtaInfo(bounty);
  const intl = isActive(1114).intl;
  const stringResult = intl.string(isActive(1114).t["9UtZAY"]);
  let obj2 = isActive(15070);
  const fn = function y() {
  ...
  const obj3 = isActive(4310);
  fn.__closure = { isScrollingInBoundsSharedValue, withTiming: isActive(4575).withTiming, isActive, timingStandard: isActive(4578).timingStandard };
  ...
  obj5.style = items;
  const items1 = [closure_9(closure_6, { style: tmp.backdropTint, pointerEvents: "none" }), closure_9(isScrollingInBoundsSharedValue(5001), { colors: ["rgba(0, 0, 0, 0.48)", "rgba(0, 0, 0, 0.8)"], style: tmp.backdropGradient, pointerEvents: "none" }), ];
  if (isBountiesVerticalScrollExperimentEnabled) {
    ...
    const items3 = [tmp10(tmp9(15073), obj9), ];
    ...
    obj10.children = tmp10(tmp9(15075), obj11);
    items3[1] = tmp10(tmp11, obj10);
    obj8.children = items3;
    let tmp10Result = tmp8(tmp9(4310).View, obj8);
  } else {
    let tmp13;
    if (isActive) {
      tmp13 = onReplay;
    ...
    items6[1] = tmp10(tmp2(10212).RetryIcon, obj17);
    obj13.children = items6;
    obj12.children = tmp8(tmp9(4310).View, obj13);
    tmp10Result = tmp10(closure_4, obj12);
  }
  items1[2] = tmp10Result;
  obj5.children = items1;
  return closure_10(isScrollingInBoundsSharedValue(4310).View, obj5);
}
...
let obj2 = { withTiming: fn(4575).withTiming, timingStandard: fn(4578).timingStandard };
fn2.__closure = { withTiming: fn(4575).withTiming, timingStandard: fn(4578).timingStandard };
