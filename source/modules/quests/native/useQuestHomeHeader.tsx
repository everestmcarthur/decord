...
// Dependencies: [19, 17, 5530, 1074, 1076, 21, 4638, 576, 15110, 4634, 1115, 8990, 4605, 11254, 1897, 1241, 7292, 7651, 11243, 15114, 8215, 15115, 1484, 13089, 2]
// Exports: default

// Module 15113 (useQuestHomeHeader)
...
function QuestHomeHeaderRight(isVirtualCurrencyEnabled) {
  ...
  const tmp3 = balance;
  balance = balance(8990).useFetchVirtualCurrencyBalance().balance;
  [][0] = balance;
  let obj2 = { style: tmp2.headerRightContainer, children: null };
  if (isVirtualCurrencyEnabled) {
    let obj3 = { balance, onPress: tmp5 };
    isVirtualCurrencyEnabled = closure_8(tmp3(11243).BalanceWidgetPillButton, obj3);
  }
...
function FiltersButton(setSelectedSortMethod) {
  ...
  const callback = selectedSortMethod.useCallback(() => {
    ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(15114, dependencyMap.paths), "QuestHomeSortingFilteringBottomSheet", { onSortMethodChange: setSelectedSortMethod, onFiltersChange: setSelectedFilters, initialSortMethod: selectedSortMethod, initialFilters: selectedFilters });
  }, items);
...
const QuestHomeSortMethods = fn(5530).QuestHomeSortMethods;
