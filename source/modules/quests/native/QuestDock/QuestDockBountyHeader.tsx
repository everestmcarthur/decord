...
// Dependencies: [19, 17, 5532, 15207, 21, 576, 4640, 15214, 15208, 4376, 5061, 15204, 15304, 5675, 7187, 4636, 2]

// Module 15315 (QuestDockBountyHeader)
import nativeDefault from "native" /* 576 */;
import spring from "spring" /* 5061 */;
import ReanimatedNativeViewDefault from "ReanimatedNativeView" /* 7187 */;
import QuestDockBackgroundBlurHeaderDefault from "QuestDockBackgroundBlurHeader" /* 15304 */;
import noop from "module_19" /* 19 */;

require = fn;
const View = fn(17).View;
const QuestDockMode = fn(5532).QuestDockMode;
const QUEST_DOCK_MODE_CHANGE_PHYSICS_CLAMPED = fn(15207).QUEST_DOCK_MODE_CHANGE_PHYSICS_CLAMPED;
const jsxProd = fn(21);
...
export default noop.memo(function QuestDockBountyHeader() {
  const questDockBounty = activeQuestDockMode(15214).useQuestDockBounty();
  const tmp4 = closure_9();
  let str = questDockBounty.productName;
  if (str == null) {
    str = "";
  }
  activeQuestDockMode = noop.useContext(tmp(15208).QuestDockGestureContext).activeQuestDockMode;
  let obj = activeQuestDockMode(15214);
  const fn = function n() {
    let num = 1;
  ...
  const obj2 = { withSpring: activeQuestDockMode(5061).withSpring, activeQuestDockMode, QuestDockMode, QUEST_DOCK_MODE_CHANGE_PHYSICS_CLAMPED };
  const bountyCreative = activeQuestDockMode(15214).useBountyCreative(questDockBounty);
  const tmpResult3 = activeQuestDockMode(15214);
  const tmpResult4 = activeQuestDockMode(15204);
  const obj3 = { onSubmenuPress: activeQuestDockMode(15204).useActionSheetPressHandler(bountyCreative), children: null };
  ...
  const actionSheetPressHandler = activeQuestDockMode(15204).useActionSheetPressHandler(bountyCreative);
  if (tmp8Result) {
    ...
    obj5.source = obj6;
    tmp8Result = tmp8(tmp9(5675), obj5);
  }
  ...
  obj8.children = closure_7(activeQuestDockMode(4636).Text, { variant: "text-md/medium", color: "text-strong", lineClamp: 1, accessible: false, children: str });
