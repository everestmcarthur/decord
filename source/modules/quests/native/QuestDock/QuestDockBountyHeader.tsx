...
// Dependencies: [19, 17, 5614, 15301, 21, 576, 4722, 15308, 15302, 4458, 5143, 15298, 15398, 5757, 7269, 4718, 2]

// Module 15409 (QuestDockBountyHeader)
...
const QUEST_DOCK_MODE_CHANGE_PHYSICS_CLAMPED = fn(15301).QUEST_DOCK_MODE_CHANGE_PHYSICS_CLAMPED;
const jsxProd = fn(21);
...
export default noop.memo(function QuestDockBountyHeader() {
  const questDockBounty = activeQuestDockMode(15308).useQuestDockBounty();
  const tmp4 = closure_9();
  let str = questDockBounty.productName;
  if (str == null) {
    str = "";
  }
  activeQuestDockMode = noop.useContext(tmp(15302).QuestDockGestureContext).activeQuestDockMode;
  let obj = activeQuestDockMode(15308);
  const fn = function n() {
    let num = 1;
  ...
  const tmpResult = activeQuestDockMode(4458);
  ...
  const obj2 = { withSpring: activeQuestDockMode(5143).withSpring, activeQuestDockMode, QuestDockMode, QUEST_DOCK_MODE_CHANGE_PHYSICS_CLAMPED };
  const bountyCreative = activeQuestDockMode(15308).useBountyCreative(questDockBounty);
  const tmpResult3 = activeQuestDockMode(15308);
  const tmpResult4 = activeQuestDockMode(15298);
  const obj3 = { onSubmenuPress: activeQuestDockMode(15298).useActionSheetPressHandler(bountyCreative), hideBlurWhenCollapsed: true, children: null };
  ...
  const actionSheetPressHandler = activeQuestDockMode(15298).useActionSheetPressHandler(bountyCreative);
  if (tmp8Result) {
    ...
    obj5.source = obj6;
    tmp8Result = tmp8(tmp9(5757), obj5);
  }
  ...
  obj8.children = closure_7(activeQuestDockMode(4718).Text, { variant: "text-md/medium", color: "text-strong", lineClamp: 1, accessible: false, children: str });
