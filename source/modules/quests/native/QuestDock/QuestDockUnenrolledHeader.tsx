...
// Dependencies: [19, 17, 1085, 21, 4640, 15214, 15204, 15225, 5535, 7842, 4574, 4491, 15203, 15304, 4636, 1115, 5675, 15308, 15309, 15262, 2]

// Module 15303 (QuestDockUnenrolledHeader)
...
import QuestGameLogotypeDefault from "QuestGameLogotype" /* 15262 */;
...
export default noop.memo(function QuestDockUnenrolledHeader() {
  const questDockQuest = questCreative(15214).useQuestDockQuest();
  let obj = questCreative(15214);
  questCreative = questCreative(15214).useQuestCreative(questDockQuest);
  let obj2 = questCreative(15214);
  const items = [questCreative];
  const obj3 = questCreative(15204);
  ...
  const actionSheetPressHandler = questCreative(15204).useActionSheetPressHandler(questCreative);
  ...
  const questGameLogotypeAssetUrl = questCreative(15203).useQuestGameLogotypeAssetUrl(questDockQuest);
  const questBarHeroBlurhash = questDockQuest.config.assets.questBarHeroBlurhash;
  const tmpResult = questCreative(15203);
  ...
  obj5.collapsedContent = closure_6(questCreative(4636).Text, obj6);
  ...
  if (LIGHT === tmp10.DARK) {
    let tmp7Result4 = tmp7(15308);
  } else {
    tmp7Result4 = tmp7(15309);
  }
