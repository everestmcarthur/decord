...
// Dependencies: [32, 5, 19, 17, 4631, 1372, 1896, 5529, 21, 4842, 11913, 1897, 4639, 576, 8965, 5716, 5765, 1115, 5672, 8938, 1365, 11914, 11915, 11916, 504, 8978, 1094, 11428, 11917, 11901, 5532, 11918, 7230, 4635, 5059, 2]
// Exports: default, openQuestOrbsRewardModal

// Module 11913 (QuestOrbsRewardModal)
...
import _modDef11916 from "module_11916" /* 11916 */;
...
function CancelButton() {
  ...
  return closure_13(require("module_5716").HeaderBackButton, obj);
}
function StaticOrb() {
...
function AnimatedOrbContainer(isAppActive) {
  ...
  const items1 = [tmp13, , ];
  const obj3 = { uri: _modDef11915, style: null, onLoad: null, animate: null };
  ...
  if (tmp15Result) {
    const obj5 = { uri: _modDef11916, style: null, onLoad: null, animate: null };
...
export default function QuestOrbsRewardModal(quest) {
  ...
  let obj = num(504);
  num = num(8978).useFetchVirtualCurrencyBalance().balance;
  let obj2 = num(8978);
  ...
  if (num2 == null) {
    num2 = tmp2(11428).getQuestOrbRewardQuantityForUser(quest.config, stateFromStores2);
    const tmp2Result = tmp2(11428);
  }
  const effect = obj3.useEffect(() => {
    num(11917).applyOrientationLock("PORTRAIT");
    return () => {
  ...
  let string = obj3.useCallback(() => {
    _undefined(4842).popWithKey(QuestOrbsRewardModal);
    const obj = _undefined(4842);
    const obj2 = num(11901);
    obj2.openQuestHome({ filter: constants.VIRTUAL_CURRENCY, fromContent: num(5532).QuestContent.REWARD_MODAL });
  }, []);
  const obj6 = { style: tmp.root, children: null };
  const obj7 = { style: absoluteFill.absoluteFill, accessible: false, accessibilityElementsHidden: true, importantForAccessibility: "no-hide-descendants", children: closure_13(num(11918).OrbsRewardBackground, { style: tmp.background, onReady: callback }) };
  ...
  if (!tmp6) {
    ...
    let items6 = [tmp13(tmp2(4635).Heading, obj14), ];
    ...
    if (num >= 4100) {
      ...
      const obj17 = {
        ...
        profileDecoHook() {
              ...
              return closure_1_13(num(4635).Text, obj, "profileDeco");
            }
      };
    ...
    tmp11(tmp2(7230).SafeAreaPaddingView, rect);
  }
};
