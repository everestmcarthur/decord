...
// Dependencies: [32, 5, 19, 17, 4630, 1372, 1896, 5530, 21, 4841, 11921, 1897, 4638, 576, 8976, 5717, 5766, 1115, 5673, 8949, 1365, 11922, 11923, 11924, 504, 8989, 1094, 11436, 11925, 11909, 5533, 11926, 7233, 4634, 5060, 2]
// Exports: default, openQuestOrbsRewardModal

// Module 11921 (QuestOrbsRewardModal)
...
function CancelButton() {
  ...
  return closure_13(require("module_5717").HeaderBackButton, obj);
}
function StaticOrb() {
...
function AnimatedOrbContainer(isAppActive) {
  ...
  const items1 = [tmp13, , ];
  const obj3 = { uri: _modDef11923, style: null, onLoad: null, animate: null };
  ...
  if (tmp15Result) {
    const obj5 = { uri: _modDef11924, style: null, onLoad: null, animate: null };
...
export default function QuestOrbsRewardModal(quest) {
  ...
  let obj = num(504);
  num = num(8989).useFetchVirtualCurrencyBalance().balance;
  let obj2 = num(8989);
  ...
  if (num2 == null) {
    num2 = tmp2(11436).getQuestOrbRewardQuantityForUser(quest.config, stateFromStores2);
    const tmp2Result = tmp2(11436);
  }
  const effect = obj3.useEffect(() => {
    num(11925).applyOrientationLock("PORTRAIT");
    return () => {
  ...
  let string = obj3.useCallback(() => {
    _undefined(4841).popWithKey(QuestOrbsRewardModal);
    const obj = _undefined(4841);
    const obj2 = num(11909);
    obj2.openQuestHome({ filter: constants.VIRTUAL_CURRENCY, fromContent: num(5533).QuestContent.REWARD_MODAL });
  }, []);
  const obj6 = { style: tmp.root, children: null };
  const obj7 = { style: absoluteFill.absoluteFill, accessible: false, accessibilityElementsHidden: true, importantForAccessibility: "no-hide-descendants", children: closure_13(num(11926).OrbsRewardBackground, { style: tmp.background, onReady: callback }) };
  ...
  if (!tmp6) {
    ...
    let items6 = [tmp13(tmp2(4634).Heading, obj14), ];
    ...
    if (num >= 4100) {
      ...
      const obj17 = {
        ...
        profileDecoHook() {
              ...
              return closure_1_13(num(4634).Text, obj, "profileDeco");
            }
      };
    ...
    tmp11(tmp2(7233).SafeAreaPaddingView, rect);
  }
};
