...
// Dependencies: [19, 17, 1074, 21, 4574, 576, 4541, 7396, 9999, 4268, 2024, 4989, 1114, 1611, 7168, 4284, 4570, 11325, 3366, 7164, 15194, 2]
// Exports: default

// Module 15197 (QuestOrbMultiplierPerkInfoActionSheet)
import nativeDefault from "native" /* 576 */;
import useSafeAreaInsetsDefault from "useSafeAreaInsets" /* 1611 */;
import HelpdeskUtilsDefault from "HelpdeskUtils" /* 2024 */;
import _modDef3366 from "module_3366" /* 3366 */;
...
function Footer(eligibleToReceivePremiumRewards) {
  ...
  if (eligibleToReceivePremiumRewards.eligibleToReceivePremiumRewards) {
    ...
    obj2.text = intl3.string(tmp10(1114).t.hvVgAZ);
    ...
    obj3.text = intl4.string(tmp10(1114).t.cpT0Cq);
    obj3.onPress = callback2;
    items[1] = tmp9(tmp10(4989).Button, obj3);
    obj.children = items;
    let tmp11 = obj;
  } else {
    const obj4 = { size: "lg", variant: "primary", text: null, onPress: null, loading: null };
    const intl = tmp10(1114).intl;
    obj4.text = intl.string(tmp10(1114).t.pj0XBN);
    ...
    obj5.text = intl2.string(tmp10(1114).t.PcTCB7);
    obj5.onPress = callback;
    items1[1] = tmp9(tmp10(4989).Button, obj5);
    obj.children = items1;
    tmp11 = obj;
  }
...
function SheetContent(arg0) {
  ...
  const obj5 = { style: null, variant: "display-md", color: "mobile-text-heading-primary", accessibilityRole: "header", children: title };
  const items3 = [, ];
  ({ text: arr4[0], title: arr4[1] } = tmp);
  obj5.style = items3;
  items2[1] = React6(Text_Text.Text, obj5);
...
export default function QuestOrbMultiplierPerkInfoActionSheet(multiplier) {
  ...
  const result = multiplier(11325).shouldReceiveQuestOrbMultiplier(orbMultiplierEligibility);
  ...
  const memo1 = noop.useMemo(() => {
    if (orbMultiplierEligibility === QuestOrbMultiplierUtils.QuestOrbMultiplierEligibilityType.CREPE) {
      ...
      let formatResult = intl2.format(_modDef3366.UkrcSH, obj2);
    } else {
      const intl = tmp(1114).intl;
  ...
  let obj3 = { visible: orbMultiplierEligibility === multiplier(11325).QuestOrbMultiplierEligibilityType.NITRO || orbMultiplierEligibility === multiplier(11325).QuestOrbMultiplierEligibilityType.UPSELL, children: null };
  const tmp4 = orbMultiplierEligibility === multiplier(11325).QuestOrbMultiplierEligibilityType.NITRO || orbMultiplierEligibility === multiplier(11325).QuestOrbMultiplierEligibilityType.UPSELL;
  obj3.children = closure_8(SheetContent, { title: memo, body: memo1, eligibleToReceivePremiumRewards: result });
  obj2.children = closure_8(orbMultiplierEligibility(15194), obj3);
  return closure_8(multiplier(7164).BottomSheet, obj2);
};
