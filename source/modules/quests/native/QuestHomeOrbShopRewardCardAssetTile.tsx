...
// Dependencies: [32, 19, 17, 1076, 21, 8904, 576, 4638, 8951, 38, 1890, 8952, 8953, 8300, 8938, 5673, 8984, 1077, 8985, 8940, 8963, 8965, 8905, 7663, 4487, 4339, 2]

// Module 15194 (QuestHomeOrbShopRewardCardAssetTile)
...
import useDefaultVariantIndex from "useDefaultVariantIndex" /* 8905 */;
import AvatarDecorationSampleV2 from "AvatarDecorationSampleV2" /* 8951 */;
import _modDef8952 from "module_8952" /* 8952 */;
import CutoutableAvatarDecorationDefault from "CutoutableAvatarDecoration" /* 8953 */;
...
function CompactAvatarDecorationPreview(arg0) {
  ...
  const items = [React6(hasOwnProperty, { style: tmp.avatar, resizeMode: "contain", source: _modDef8952, accessible: false }), ];
  const obj2 = { style: tmp.avatar, resizeMode: "contain", source: _modDef8952, accessible: false };
...
function ProductPreviewInner(cardHeight) {
  ...
  let tmp = closure_16();
  const shopProductItems = cardWidth(8300).useShopProductItems(product);
  ({ firstProfileEffect, firstAvatarDecoration, firstNameplate } = shopProductItems);
  const tmp5 = cardWidth < cardWidth(8904).COLLECTIBLES_SHOP_CARD_WIDTH || cardHeight < cardWidth(8904).COLLECTIBLES_SHOP_CARD_HEIGHT;
  dependencyMap = tmp5;
  ...
  if (product.type === cardWidth(1890).CollectiblesItemType.BUNDLE) {
    ...
    return closure_8(cardHeight(8938), obj2);
  } else if (product.skuId === EXTERNAL_PRODUCT_SKU_IDS.ORB_PROFILE_BADGE) {
    const obj3 = { source: null, style: null };
    const obj4 = { uri: cardHeight(8984) };
    obj3.source = obj4;
    obj3.style = tmp.externalProductImage;
    return closure_8(cardHeight(5673), obj3);
  } else {
    const ALL = tmp2(1077).FractionalPremiumSKUsSets.ALL;
    if (ALL.has(product.skuId)) {
      size = { skuId: product.skuId, width: tmp2(8985).FRACTIONAL_NITRO_COIN_SIZE.CARD, height: tmp2(8985).FRACTIONAL_NITRO_COIN_SIZE.CARD };
      return closure_8(tmp2(8985).FractionalNitroCoinIllustration, size);
    } else {
      ...
      if (tmp2(1890).CollectiblesItemType.AVATAR_DECORATION === type) {
        if (flag) {
          ...
          tmp24Result = tmp24(cardHeight(8951), obj6);
        }
        return tmp24Result;
        ...
        obj7.children = closure_8(cardHeight(8940), obj8);
        ...
        if (flag) {
          ...
          if (prop == null) {
            prop = tmp2(8904).COLLECTIBLES_SHOP_CARD_WIDTH - PX_32;
          }
          ...
          obj9.children = tmp14(cardHeight(8963), obj10);
        ...
        return closure_8(cardHeight(8965), obj11);
      } else {
        return null;
      }
    }
...
let c10 = 100;
const diff = fn(8904).COLLECTIBLES_SHOP_CARD_HEIGHT - 2 * nativeDefault.space.PX_16;
