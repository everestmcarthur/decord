...
// Dependencies: [32, 19, 17, 1076, 21, 8855, 576, 4606, 8902, 38, 1889, 8903, 8904, 8256, 8889, 5638, 8935, 1077, 8936, 8891, 8914, 8916, 8856, 7626, 4456, 4308, 2]
...
function CompactAvatarDecorationPreview(arg0) {
  ...
  const items = [React6(hasOwnProperty, { style: tmp.avatar, resizeMode: "contain", source: _modDef8903, accessible: false }), ];
  const obj2 = { style: tmp.avatar, resizeMode: "contain", source: _modDef8903, accessible: false };
...
function ProductPreviewInner(cardHeight) {
  ...
  let tmp = closure_16();
  const shopProductItems = cardWidth(8256).useShopProductItems(product);
  ({ firstProfileEffect, firstAvatarDecoration, firstNameplate } = shopProductItems);
  const tmp5 = cardWidth < cardWidth(8855).COLLECTIBLES_SHOP_CARD_WIDTH || cardHeight < cardWidth(8855).COLLECTIBLES_SHOP_CARD_HEIGHT;
  dependencyMap = tmp5;
  ...
  if (product.type === cardWidth(1889).CollectiblesItemType.BUNDLE) {
    ...
    return closure_8(cardHeight(8889), obj2);
  } else if (product.skuId === EXTERNAL_PRODUCT_SKU_IDS.ORB_PROFILE_BADGE) {
    const obj3 = { source: null, style: null };
    const obj4 = { uri: cardHeight(8935) };
    obj3.source = obj4;
    obj3.style = tmp.externalProductImage;
    return closure_8(cardHeight(5638), obj3);
  } else {
    const ALL = tmp2(1077).FractionalPremiumSKUsSets.ALL;
    if (ALL.has(product.skuId)) {
      size = { skuId: product.skuId, width: tmp2(8936).FRACTIONAL_NITRO_COIN_SIZE.CARD, height: tmp2(8936).FRACTIONAL_NITRO_COIN_SIZE.CARD };
      return closure_8(tmp2(8936).FractionalNitroCoinIllustration, size);
    } else {
      ...
      if (tmp2(1889).CollectiblesItemType.AVATAR_DECORATION === type) {
        if (flag) {
          ...
          tmp24Result = tmp24(cardHeight(8902), obj6);
        }
        return tmp24Result;
        ...
        obj7.children = closure_8(cardHeight(8891), obj8);
        ...
        if (flag) {
          ...
          if (prop == null) {
            prop = tmp2(8855).COLLECTIBLES_SHOP_CARD_WIDTH - PX_32;
          }
          ...
          obj9.children = tmp14(cardHeight(8914), obj10);
        ...
        return closure_8(cardHeight(8916), obj11);
      } else {
        return null;
      }
    }
