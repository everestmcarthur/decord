...
// Dependencies: [32, 19, 17, 1076, 21, 8920, 576, 4640, 8967, 38, 1890, 8968, 8969, 8319, 8954, 5675, 9000, 1077, 9001, 8956, 8979, 8981, 8921, 7667, 4489, 4341, 2]

// Module 15199 (QuestHomeOrbShopRewardCardAssetTile)
...
import useDefaultVariantIndex from "useDefaultVariantIndex" /* 8921 */;
import AvatarDecorationSampleV2 from "AvatarDecorationSampleV2" /* 8967 */;
import _modDef8968 from "module_8968" /* 8968 */;
import CutoutableAvatarDecorationDefault from "CutoutableAvatarDecoration" /* 8969 */;
...
function CompactAvatarDecorationPreview(arg0) {
  ...
  const items = [React6(hasOwnProperty, { style: tmp.avatar, resizeMode: "contain", source: _modDef8968, accessible: false }), ];
  const obj2 = { style: tmp.avatar, resizeMode: "contain", source: _modDef8968, accessible: false };
...
function ProductPreviewInner(cardHeight) {
  ...
  let tmp = closure_16();
  const shopProductItems = cardWidth(8319).useShopProductItems(product);
  ({ firstProfileEffect, firstAvatarDecoration, firstNameplate } = shopProductItems);
  const tmp5 = cardWidth < cardWidth(8920).COLLECTIBLES_SHOP_CARD_WIDTH || cardHeight < cardWidth(8920).COLLECTIBLES_SHOP_CARD_HEIGHT;
  dependencyMap = tmp5;
  ...
  if (product.type === cardWidth(1890).CollectiblesItemType.BUNDLE) {
    ...
    return closure_8(cardHeight(8954), obj2);
  } else if (product.skuId === EXTERNAL_PRODUCT_SKU_IDS.ORB_PROFILE_BADGE) {
    const obj3 = { source: null, style: null };
    const obj4 = { uri: cardHeight(9000) };
    obj3.source = obj4;
    obj3.style = tmp.externalProductImage;
    return closure_8(cardHeight(5675), obj3);
  } else {
    const ALL = tmp2(1077).FractionalPremiumSKUsSets.ALL;
    if (ALL.has(product.skuId)) {
      size = { skuId: product.skuId, width: tmp2(9001).FRACTIONAL_NITRO_COIN_SIZE.CARD, height: tmp2(9001).FRACTIONAL_NITRO_COIN_SIZE.CARD };
      return closure_8(tmp2(9001).FractionalNitroCoinIllustration, size);
    } else {
      ...
      if (tmp2(1890).CollectiblesItemType.AVATAR_DECORATION === type) {
        if (flag) {
          ...
          tmp24Result = tmp24(cardHeight(8967), obj6);
        }
        return tmp24Result;
      } else if (tmp2(1890).CollectiblesItemType.PROFILE_EFFECT === type) {
        const obj7 = { style: tmp.profileEffectContainer, children: null };
        const obj8 = { item: first, hideBackground: true };
        obj7.children = closure_8(cardHeight(8956), obj8);
        return closure_8(closure_6, obj7);
      } else if (tmp2(1890).CollectiblesItemType.PROFILE_FRAME === type) {
        if (flag) {
          if (null != memo) {
            const items2 = [, ];
          ...
          if (prop == null) {
            prop = tmp2(8920).COLLECTIBLES_SHOP_CARD_WIDTH - PX_32;
          }
          ...
          obj9.children = tmp14(cardHeight(8979), obj10);
          return tmp14(tmp15, obj9);
        }
        profileFrameContainer = tmp.profileFrameContainer;
      } else if (tmp2(1890).CollectiblesItemType.NAMEPLATE === type) {
        const obj11 = { item: first };
        return closure_8(cardHeight(8981), obj11);
      } else {
        return null;
      }
    }
...
let c10 = 100;
const diff = fn(8920).COLLECTIBLES_SHOP_CARD_HEIGHT - 2 * nativeDefault.space.PX_16;
...
export default noop.memo(function QuestHomeOrbShopRewardCardAssetTile(arg0) {
  ...
  useToken;
  if (!hideCardDetails) {
    if (productType !== tmp2(1890).CollectiblesItemType.PROFILE_EFFECT) {
      if (productType !== tmp2(1890).CollectiblesItemType.PROFILE_FRAME) {
        let str = "75%";
      }
