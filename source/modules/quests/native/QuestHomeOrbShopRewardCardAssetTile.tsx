...
// Dependencies: [32, 19, 17, 1076, 21, 8857, 576, 4607, 8904, 38, 1889, 8905, 8906, 8257, 8891, 5639, 8937, 1077, 8938, 8893, 8916, 8918, 8858, 7627, 4456, 4308, 2]

// Module 15145 (QuestHomeOrbShopRewardCardAssetTile)
...
import AvatarDecorationSampleV2 from "AvatarDecorationSampleV2" /* 8904 */;
import _modDef8905 from "module_8905" /* 8905 */;
import CutoutableAvatarDecorationDefault from "CutoutableAvatarDecoration" /* 8906 */;
...
function CompactAvatarDecorationPreview(arg0) {
  ...
  const items = [React6(hasOwnProperty, { style: tmp.avatar, resizeMode: "contain", source: _modDef8905, accessible: false }), ];
  const obj2 = { style: tmp.avatar, resizeMode: "contain", source: _modDef8905, accessible: false };
...
function ProductPreviewInner(cardHeight) {
  ...
  const tmp5 = cardWidth < cardWidth(8857).COLLECTIBLES_SHOP_CARD_WIDTH || cardHeight < cardWidth(8857).COLLECTIBLES_SHOP_CARD_HEIGHT;
  dependencyMap = tmp5;
  ...
  if (product.type === cardWidth(1889).CollectiblesItemType.BUNDLE) {
    ...
    return closure_8(cardHeight(8891), obj2);
  } else if (product.skuId === EXTERNAL_PRODUCT_SKU_IDS.ORB_PROFILE_BADGE) {
    const obj3 = { source: null, style: null };
    const obj4 = { uri: cardHeight(8937) };
    obj3.source = obj4;
    obj3.style = tmp.externalProductImage;
    return closure_8(cardHeight(5639), obj3);
  } else {
    const ALL = tmp2(1077).FractionalPremiumSKUsSets.ALL;
    if (ALL.has(product.skuId)) {
      size = { skuId: product.skuId, width: tmp2(8938).FRACTIONAL_NITRO_COIN_SIZE.CARD, height: tmp2(8938).FRACTIONAL_NITRO_COIN_SIZE.CARD };
      return closure_8(tmp2(8938).FractionalNitroCoinIllustration, size);
    } else {
      ...
      if (tmp2(1889).CollectiblesItemType.AVATAR_DECORATION === type) {
        if (flag) {
          ...
          tmp24Result = tmp24(cardHeight(8904), obj6);
        }
        return tmp24Result;
        ...
        obj7.children = closure_8(cardHeight(8893), obj8);
        ...
        if (flag) {
          ...
          if (prop == null) {
            prop = tmp2(8857).COLLECTIBLES_SHOP_CARD_WIDTH - PX_32;
          }
          ...
          obj9.children = tmp14(cardHeight(8916), obj10);
        ...
        return closure_8(cardHeight(8918), obj11);
      } else {
        return null;
      }
    }
