...
// Function ID: 7727
// Name: Task
// Dependencies: [7727, 5466, 7728, 7729, 2]
...
export const questTaskConfigFromServer = function questTaskConfigFromServer(type) {
  ...
  if (QuestTaskConfigTypes.QuestTaskConfigTypes.FIRST_PARTY === type) {
    const obj2 = { type: tmp(7727).QuestTaskConfigTypes.FIRST_PARTY, tasks: _firstPartyTasksFromServer(type.tasks), joinOperator: type.join_operator };
    return obj2;
  } else if (tmp(7727).QuestTaskConfigTypes.THIRD_PARTY === type) {
    const obj = { type: tmp(7727).QuestTaskConfigTypes.THIRD_PARTY, tasks: _thirdPartyTasksFromServer(type.tasks), enrollmentUrl: null, developerApplicationId: null, joinOperator: null };
