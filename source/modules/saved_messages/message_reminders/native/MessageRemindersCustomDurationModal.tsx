...
// Dependencies: [32, 19, 17, 21, 4606, 576, 1611, 1874, 4573, 9808, 1896, 1114, 5682, 7936, 1115, 5675, 5174, 4602, 2]
...
export default noop.memo((onClose) => {
  ...
  function handleOpenDatePicker(date) {
    ...
    obj2.openLazy(asyncRequireImpl(9808, dependencyMap.paths), "DatePicker", obj3);
  }
  const tmp = onPress();
  ...
  obj5 = onClose(1115);
  obj3.headerLeft = onClose(5675).getHeaderCloseButton(onClose);
  ...
  const items1 = [error(onClose(5682).Header, obj3), ];
  ...
  const items2 = [error(onClose(4602).Text, obj8), ];
  ...
  obj9.children = error(onClose(4602).Text, { variant: "text-md/medium", children: formatResult });
  items2[1] = error(onClose(5174).PressableOpacity, obj9);
  ...
  const items4 = [error(onClose(4602).Text, obj10), , ];
  ...
  obj11.children = error(onClose(4602).Text, { variant: "text-md/medium", children: formatResult1 });
  items4[1] = error(onClose(5174).PressableOpacity, obj11);
  ...
  if (tmp10Result) {
    ...
    tmp10Result = tmp10(tmp11(4602).Text, obj12);
  }
