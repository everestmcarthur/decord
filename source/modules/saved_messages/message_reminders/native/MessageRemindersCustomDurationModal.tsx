...
// Dependencies: [32, 19, 17, 21, 4605, 576, 1611, 1874, 4572, 9787, 1896, 1114, 5681, 7914, 1115, 5674, 5173, 4601, 2]

// Module 11784 (MessageRemindersCustomDurationModal)
...
export default noop.memo((onClose) => {
  ...
  function handleOpenDatePicker(date) {
    ...
    obj2.openLazy(asyncRequireImpl(9787, dependencyMap.paths), "DatePicker", obj3);
  }
  const tmp = onPress();
  ...
  obj5 = onClose(1115);
  obj3.headerLeft = onClose(5674).getHeaderCloseButton(onClose);
  ...
  const items1 = [error(onClose(5681).Header, obj3), ];
  ...
  const items2 = [error(onClose(4601).Text, obj8), ];
  ...
  obj9.children = error(onClose(4601).Text, { variant: "text-md/medium", children: formatResult });
  items2[1] = error(onClose(5173).PressableOpacity, obj9);
  ...
  const items4 = [error(onClose(4601).Text, obj10), , ];
  ...
  obj11.children = error(onClose(4601).Text, { variant: "text-md/medium", children: formatResult1 });
  items4[1] = error(onClose(5173).PressableOpacity, obj11);
  ...
  if (tmp10Result) {
    ...
    tmp10Result = tmp10(tmp11(4601).Text, obj12);
  }
