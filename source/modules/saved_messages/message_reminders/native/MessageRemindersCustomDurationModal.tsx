...
// Dependencies: [32, 19, 17, 21, 4607, 576, 1611, 1874, 4574, 9810, 1896, 1114, 5683, 7937, 1115, 5676, 5175, 4603, 2]

// Module 11808 (MessageRemindersCustomDurationModal)
...
export default noop.memo((onClose) => {
  ...
  function handleOpenDatePicker(date) {
    ...
    obj2.openLazy(asyncRequireImpl(9810, dependencyMap.paths), "DatePicker", obj3);
  }
  const tmp = onPress();
  ...
  obj5 = onClose(1115);
  obj3.headerLeft = onClose(5676).getHeaderCloseButton(onClose);
  ...
  const items1 = [error(onClose(5683).Header, obj3), ];
  ...
  const items2 = [error(onClose(4603).Text, obj8), ];
  ...
  obj9.children = error(onClose(4603).Text, { variant: "text-md/medium", children: formatResult });
  items2[1] = error(onClose(5175).PressableOpacity, obj9);
  ...
  const items4 = [error(onClose(4603).Text, obj10), , ];
  ...
  obj11.children = error(onClose(4603).Text, { variant: "text-md/medium", children: formatResult1 });
  items4[1] = error(onClose(5175).PressableOpacity, obj11);
  ...
  if (tmp10Result) {
    ...
    tmp10Result = tmp10(tmp11(4603).Text, obj12);
  }
