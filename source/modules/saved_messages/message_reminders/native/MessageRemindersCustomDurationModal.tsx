...
// Dependencies: [32, 19, 17, 21, 4639, 576, 1612, 1875, 4606, 9849, 1897, 1115, 5716, 7969, 1364, 5709, 5207, 4635, 2]

// Module 11847 (MessageRemindersCustomDurationModal)
...
export default noop.memo((onClose) => {
  ...
  function handleOpenDatePicker(date) {
    ...
    obj2.openLazy(asyncRequireImpl(9849, dependencyMap.paths), "DatePicker", obj3);
  }
  const tmp = onPress();
  ...
  obj5 = onClose(1364);
  obj3.headerLeft = onClose(5709).getHeaderCloseButton(onClose);
  ...
  const items1 = [error(onClose(5716).Header, obj3), ];
  ...
  const items2 = [error(onClose(4635).Text, obj8), ];
  ...
  obj9.children = error(onClose(4635).Text, { variant: "text-md/medium", children: formatResult });
  items2[1] = error(onClose(5207).PressableOpacity, obj9);
  ...
  const items4 = [error(onClose(4635).Text, obj10), , ];
  ...
  obj11.children = error(onClose(4635).Text, { variant: "text-md/medium", children: formatResult1 });
  items4[1] = error(onClose(5207).PressableOpacity, obj11);
  ...
  if (tmp10Result) {
    ...
    tmp10Result = tmp10(tmp11(4635).Text, obj12);
  }
