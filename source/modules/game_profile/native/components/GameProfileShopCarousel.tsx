...
// Dependencies: [19, 17, 7615, 21, 4606, 576, 8855, 8962, 8963, 563, 8777, 7614, 7255, 4602, 1114, 5026, 7282, 8964, 8811, 2]
// Exports: default

// Module 8854 (GameProfileShopCarousel)
...
function GameProfileShopCarouselContent(trackAction) {
  ...
  const tmp = closure_8();
  dependencyMap = tmp;
  const gameProfileShopCollection = closeModal(8962).useGameProfileShopCollection(collectionId);
  const obj = closeModal(8962);
  const fetchCollectiblesProducts = closeModal(8963).useFetchCollectiblesProducts(gameProfileShopCollection);
  let obj2 = closeModal(8963);
  ...
  if (0 !== stateFromStoresArray.length) {
    ...
    const items2 = [closure_6(tmp2(4602).Text, obj6), ];
    ...
    obj7.icon = closure_6(tmp2(7282).ChevronSmallRightIcon, { size: "sm" });
    obj7.onPress = tmp6;
    items2[1] = closure_6(tmp2(5026).Button, obj7);
    ...
    const obj8 = {
      ...
      snapToInterval: tmp2(8855).COLLECTIBLES_SHOP_CARD_WIDTH + tmp2(8855).COLLECTIBLES_SHOP_CARD_GAP
    };
    items3[1] = closure_6(tmp2(8964).FlashList, obj8, collectionId);
