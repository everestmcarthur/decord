...
// Dependencies: [19, 17, 21, 4639, 576, 8893, 8862, 8880, 9000, 9001, 8812, 7648, 7289, 1115, 9006, 8848, 2]
// Exports: default

// Module 8892 (GameProfileShopCarousel)
...
function GameProfileShopCarouselContent(trackAction) {
  ...
  const tmp = closure_6();
  dependencyMap = tmp;
  const gameProfileShopCollectionProducts = closeModal(9001).useGameProfileShopCollectionProducts(collectionId);
  ...
  if (gameProfileShopCollectionProducts.isLoading) {
    ...
    if (0 !== products.length) {
      ...
      const obj5 = {
        horizontal: true,
        renderScrollComponent: trackAction(8848),
        data: products,
        renderItem(item) {
              item = item.item;
        ...
        snapToInterval: tmp2(8893).COLLECTIBLES_SHOP_CARD_WIDTH + tmp2(8893).COLLECTIBLES_SHOP_CARD_GAP
      };
      obj3.children = jsx(tmp2(9006).FlashList, {
        horizontal: true,
        renderScrollComponent: trackAction(8848),
        data: products,
        renderItem(item) {
      ...
      tmp6 = jsx(trackAction(8862), { style: null, headerStyle: null, title: null, onPressViewAll: null, children: null });
      const tmp11 = trackAction(8862);
    }
  }
  return tmp6;
}
...
obj.skeletonCards = { paddingHorizontal: fn(8893).COLLECTIBLES_SHOP_CARD_GAP };
const obj6 = { paddingHorizontal: fn(8893).COLLECTIBLES_SHOP_CARD_GAP };
obj.horizontalSpacing = { width: fn(8893).COLLECTIBLES_SHOP_CARD_GAP };
