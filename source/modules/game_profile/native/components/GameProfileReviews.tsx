...
// Dependencies: [19, 17, 21, 4638, 576, 4358, 8818, 4332, 8861, 8862, 8821, 1115, 8829, 4634, 1935, 8863, 8869, 8825, 8826, 2]
// Exports: default

// Module 8860 (GameProfileReviews)
import nativeDefault from "native" /* 576 */;
import GameProfileAnalyticUtils from "GameProfileAnalyticUtils" /* 8821 */;
import useSteamWebsiteUrl from "useSteamWebsiteUrl" /* 8825 */;
...
function SteamReviewRow(url) {
  ...
  const tmp = closure_9();
  const tmp5Result = trackAction(8818)(trackAction(4332).openURL);
  dependencyMap = tmp5Result;
  const tmp4 = trackAction;
  const tmp5 = trackAction(8818);
  const result = url(8861).calculateSteamReviewScoreDescription(rating, ratingCount, isRecentRating);
  const obj = url(8861);
  const items = [tmp5Result, url, trackAction];
  const steamReviewScoreDescriptionColor = url(8862).getSteamReviewScoreDescriptionColor(result);
  const obj3 = {
  ...
  const items2 = [closure_7(url(8829).SteamNeutralIcon, { size: "sm", color: tmp4(576).colors.ICON_STRONG }), closure_7(url(4634).Text, { variant: "heading-sm/medium", color: "mobile-text-heading-primary", children: title })];
  ...
  let linkText;
  if (noop.useContext(url(4358).AccessibilityPreferencesContext).alwaysShowLinkDecorations) {
    linkText = tmp.linkText;
  }
  obj7.style = linkText;
  const obj5 = { size: "sm", color: tmp4(576).colors.ICON_STRONG };
  obj7.children = url(8862).getSteamReviewScoreDescriptionIntl(result);
  const items4 = [closure_7(url(4634).Text, obj7), ];
  let tmp12Result = null != ratingCount && result !== tmp2(1935).SteamReviewScoreDescription.NO_USER_REVIEWS;
  if (tmp12Result) {
    ...
    tmp12Result = tmp12(tmp2(4634).Text, obj8);
...
function OpenCriticReview(url) {
  ...
  if (opencritic == null) {
    opencritic = { topCriticRating: "Array", topCriticRatingCount: "accessible", tier: "diversity" };
  }
  ...
  const tmp4Result = trackAction(8818)(trackAction(4332).openURL);
  ...
  if (null != tier) {
    str = url(8863).getOpenCriticTierText(tier);
    const obj2 = url(8863);
  }
  if (null != tier) {
    let openCriticCircleRatingColor = url(8863).getOpenCriticCircleRatingColor(tier);
    const obj4 = url(8863);
  } else {
  ...
  const items1 = [closure_7(url(4634).Text, obj3), ];
  ...
  if (null != tier) {
    ...
    const obj8 = { uri: tmp11(8863).getOpenCriticTierImage(tier) };
    obj7.source = obj8;
  ...
  if (null != tier) {
    ...
    if (topCriticRating > 0) {
      ...
      if (num > 0) {
        ...
        const items4 = [tmp12(trackAction(8869), obj11), ];
        ...
        obj12.children = tmp12(tmp11(4634).Text, obj13);
  ...
  if (topCriticRating <= 0) {
    ...
    if (null == tier) {
      const obj14 = { variant: "text-xs/medium", color: tmp11(8862).getSteamReviewScoreDescriptionColor(tmp11(1935).SteamReviewScoreDescription.NO_USER_REVIEWS), children: null };
      const intl4 = tmp11(1115).intl;
      obj14.children = intl4.string(tmp11(1115).t["0xYzpO"]);
      tmp12Result2 = tmp12(tmp11(4634).Text, obj14);
      const tmp11Result2 = tmp11(8862);
    }
  } else {
    tmp12Result2 = null;
  }
...
export default function GameProfileReviews(arg0) {
  ...
  if (null == game) {
    ...
    const tmp6 = game.steamReleaseStatus !== tmp2(8826).SteamReleaseStatus.RETIRED_ABANDONED && null != steamWebsiteUrl;
    const reviews = game.reviews;
    let steam;
    if (reviews != null) {
      steam = reviews.steam;
    }
    const calculateSteamReviewScoreDescription = tmp2(8861).calculateSteamReviewScoreDescription;
    if (steam != null) {
    ...
    let tmp10 = tmp6;
    if (tmp6) {
      tmp10 = tmp9 !== tmp2(1935).SteamReviewScoreDescription.NO_USER_REVIEWS;
    }
    const tmp2Result = tmp2(8861);
    const result = tmp2(8862).canShowLocalizedSteamReview(steam);
    if (result) {
      let localizedRating;
      if (!tmp12) {
    ...
    const items = [React5(tmp2(4634).Text, obj3), ];
