...
// Dependencies: [19, 17, 21, 4639, 576, 8809, 4334, 8851, 8852, 8812, 1115, 8820, 4635, 1935, 8853, 8859, 8816, 8817, 2]
// Exports: default

// Module 8850 (GameProfileReviews)
import nativeDefault from "native" /* 576 */;
import GameProfileAnalyticUtils from "GameProfileAnalyticUtils" /* 8812 */;
import useSteamWebsiteUrl from "useSteamWebsiteUrl" /* 8816 */;
...
function SteamReviewRow(url) {
  ...
  const tmp = closure_9();
  const tmp4Result = trackAction(8809)(trackAction(4334).openURL);
  dependencyMap = tmp4Result;
  const tmp2 = trackAction;
  const tmp4 = trackAction(8809);
  const result = url(8851).calculateSteamReviewScoreDescription(rating, ratingCount, isRecentRating);
  const obj = url(8851);
  const items = [tmp4Result, url, trackAction];
  const steamReviewScoreDescriptionColor = url(8852).getSteamReviewScoreDescriptionColor(result);
  const obj3 = {
  ...
  const items2 = [closure_7(url(8820).SteamNeutralIcon, { size: "sm", color: tmp2(576).colors.ICON_STRONG }), closure_7(url(4635).Text, { variant: "heading-sm/medium", color: "mobile-text-heading-primary", children: title })];
  ...
  obj7.children = url(8852).getSteamReviewScoreDescriptionIntl(result);
  const items4 = [closure_7(url(4635).Text, obj7), ];
  let tmp12Result = null != ratingCount && result !== tmp6(1935).SteamReviewScoreDescription.NO_USER_REVIEWS;
  if (tmp12Result) {
    const obj8 = { variant: "text-sm/medium", color: "text-subtle", children: null };
    const intl2 = tmp6(1115).intl;
    const obj9 = { rating_count: ratingCount.toLocaleString() };
    obj8.children = intl2.format(tmp6(1115).t.sgIoin, obj9).toString();
    tmp12Result = tmp12(tmp6(4635).Text, obj8);
    const str = intl2.format(tmp6(1115).t.sgIoin, obj9);
  }
...
function OpenCriticReview(url) {
  ...
  if (opencritic == null) {
    opencritic = { topCriticRating: "isArray", topCriticRatingCount: "accessible", tier: "height" };
  }
  ...
  if (null != tier) {
    str = url(8853).getOpenCriticTierText(tier);
    const obj2 = url(8853);
  }
  if (null != tier) {
    let openCriticCircleRatingColor = url(8853).getOpenCriticCircleRatingColor(tier);
    const obj4 = url(8853);
  } else {
  ...
  const items1 = [closure_7(url(4635).Text, obj3), ];
  ...
  if (null != tier) {
    ...
    const obj8 = { uri: tmp11(8853).getOpenCriticTierImage(tier) };
    obj7.source = obj8;
  ...
  if (null != tier) {
    ...
    if (topCriticRating > 0) {
      ...
      if (num > 0) {
        ...
        const items4 = [tmp12(trackAction(8859), obj11), ];
        ...
        obj12.children = tmp12(tmp11(4635).Text, obj13);
  ...
  if (topCriticRating <= 0) {
    ...
    if (null == tier) {
      const obj14 = { variant: "text-xs/medium", color: tmp11(8852).getSteamReviewScoreDescriptionColor(tmp11(1935).SteamReviewScoreDescription.NO_USER_REVIEWS), children: null };
      const intl4 = tmp11(1115).intl;
      obj14.children = intl4.string(tmp11(1115).t["0xYzpO"]);
      tmp12Result2 = tmp12(tmp11(4635).Text, obj14);
      const tmp11Result2 = tmp11(8852);
    }
  } else {
    tmp12Result2 = null;
  }
...
export default function GameProfileReviews(arg0) {
  ...
  if (null == game) {
    ...
    const tmp6 = game.steamReleaseStatus !== tmp2(8817).SteamReleaseStatus.RETIRED_ABANDONED && null != steamWebsiteUrl;
    const reviews = game.reviews;
    let steam;
    if (reviews != null) {
      steam = reviews.steam;
    }
    const calculateSteamReviewScoreDescription = tmp2(8851).calculateSteamReviewScoreDescription;
    if (steam != null) {
    ...
    const tmp2Result = tmp2(8851);
    const result = tmp2(8852).canShowLocalizedSteamReview(steam);
    if (result) {
      let localizedRating;
      if (!tmp12) {
    ...
    const items = [React5(tmp2(4635).Text, obj3), ];
