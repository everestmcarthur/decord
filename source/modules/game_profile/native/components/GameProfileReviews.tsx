...
// Dependencies: [19, 17, 21, 4574, 576, 8716, 4268, 8756, 8757, 8719, 1114, 8727, 4570, 1934, 8758, 8764, 8723, 8724, 2]
// Exports: default

// Module 8755 (GameProfileReviews)
import nativeDefault from "native" /* 576 */;
import GameProfileAnalyticUtils from "GameProfileAnalyticUtils" /* 8719 */;
import useSteamWebsiteUrl from "useSteamWebsiteUrl" /* 8723 */;
...
function SteamReviewRow(url) {
  ...
  const result = url(8756).calculateSteamReviewScoreDescription(rating, ratingCount, isRecentRating);
  const obj = url(8756);
  const items = [tmp4Result, url, trackAction];
  const steamReviewScoreDescriptionColor = url(8757).getSteamReviewScoreDescriptionColor(result);
  const obj3 = {
  ...
  const items2 = [closure_7(url(8727).SteamNeutralIcon, { size: "sm", color: tmp2(576).colors.ICON_STRONG }), closure_7(url(4570).Text, { variant: "heading-sm/medium", color: "mobile-text-heading-primary", children: title })];
  ...
  obj7.children = url(8757).getSteamReviewScoreDescriptionIntl(result);
  const items4 = [closure_7(url(4570).Text, obj7), ];
  ...
  if (tmp12Result) {
    ...
    tmp12Result = tmp12(tmp6(4570).Text, obj8);
...
function OpenCriticReview(url) {
  ...
  if (null != tier) {
    str = url(8758).getOpenCriticTierText(tier);
    const obj2 = url(8758);
  }
  if (null != tier) {
    let openCriticCircleRatingColor = url(8758).getOpenCriticCircleRatingColor(tier);
    const obj4 = url(8758);
  } else {
  ...
  const items1 = [closure_7(url(4570).Text, obj3), ];
  ...
  if (null != tier) {
    ...
    const obj8 = { uri: tmp11(8758).getOpenCriticTierImage(tier) };
    obj7.source = obj8;
  ...
  if (null != tier) {
    ...
    if (topCriticRating > 0) {
      ...
      if (num > 0) {
        ...
        const items4 = [tmp12(trackAction(8764), obj11), ];
        ...
        obj12.children = tmp12(tmp11(4570).Text, obj13);
  ...
  if (topCriticRating <= 0) {
    ...
    if (null == tier) {
      const obj14 = { variant: "text-xs/medium", color: tmp11(8757).getSteamReviewScoreDescriptionColor(tmp11(1934).SteamReviewScoreDescription.NO_USER_REVIEWS), children: null };
      const intl4 = tmp11(1114).intl;
      obj14.children = intl4.string(tmp11(1114).t["0xYzpO"]);
      tmp12Result2 = tmp12(tmp11(4570).Text, obj14);
      const tmp11Result2 = tmp11(8757);
    }
  } else {
    tmp12Result2 = null;
  }
...
export default function GameProfileReviews(arg0) {
  ...
  if (null == game) {
    ...
    const tmp6 = game.steamReleaseStatus !== tmp2(8724).SteamReleaseStatus.RETIRED_ABANDONED && null != steamWebsiteUrl;
    const reviews = game.reviews;
    let steam;
    if (reviews != null) {
      steam = reviews.steam;
    }
    const calculateSteamReviewScoreDescription = tmp2(8756).calculateSteamReviewScoreDescription;
    if (steam != null) {
    ...
    const tmp2Result = tmp2(8756);
    const result = tmp2(8757).canShowLocalizedSteamReview(steam);
    if (result) {
      let localizedRating;
      if (!tmp12) {
    ...
    const items = [React5(tmp2(4570).Text, obj3), ];
