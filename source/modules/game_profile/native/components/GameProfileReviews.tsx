...
// Dependencies: [19, 17, 21, 4606, 576, 8774, 4301, 8814, 8815, 8777, 1114, 8785, 4602, 1934, 8816, 8822, 8781, 8782, 2]
// Exports: default

// Module 8813 (GameProfileReviews)
import nativeDefault from "native" /* 576 */;
import GameProfileAnalyticUtils from "GameProfileAnalyticUtils" /* 8777 */;
import useSteamWebsiteUrl from "useSteamWebsiteUrl" /* 8781 */;
...
function SteamReviewRow(url) {
  ...
  const tmp = closure_9();
  const tmp4Result = trackAction(8774)(trackAction(4301).openURL);
  dependencyMap = tmp4Result;
  const tmp2 = trackAction;
  const tmp4 = trackAction(8774);
  const result = url(8814).calculateSteamReviewScoreDescription(rating, ratingCount, isRecentRating);
  const obj = url(8814);
  const items = [tmp4Result, url, trackAction];
  const steamReviewScoreDescriptionColor = url(8815).getSteamReviewScoreDescriptionColor(result);
  const obj3 = {
  ...
  const items2 = [closure_7(url(8785).SteamNeutralIcon, { size: "sm", color: tmp2(576).colors.ICON_STRONG }), closure_7(url(4602).Text, { variant: "heading-sm/medium", color: "mobile-text-heading-primary", children: title })];
  ...
  obj7.children = url(8815).getSteamReviewScoreDescriptionIntl(result);
  const items4 = [closure_7(url(4602).Text, obj7), ];
  ...
  if (tmp12Result) {
    ...
    tmp12Result = tmp12(tmp6(4602).Text, obj8);
...
function OpenCriticReview(url) {
  ...
  if (opencritic == null) {
    opencritic = { topCriticRating: "Array", topCriticRatingCount: "call", tier: "__packager_asset" };
  }
  ...
  if (null != tier) {
    str = url(8816).getOpenCriticTierText(tier);
    const obj2 = url(8816);
  }
  if (null != tier) {
    let openCriticCircleRatingColor = url(8816).getOpenCriticCircleRatingColor(tier);
    const obj4 = url(8816);
  } else {
  ...
  const items1 = [closure_7(url(4602).Text, obj3), ];
  ...
  if (null != tier) {
    ...
    const obj8 = { uri: tmp11(8816).getOpenCriticTierImage(tier) };
    obj7.source = obj8;
  ...
  if (null != tier) {
    ...
    if (topCriticRating > 0) {
      ...
      if (num > 0) {
        ...
        const items4 = [tmp12(trackAction(8822), obj11), ];
        ...
        obj12.children = tmp12(tmp11(4602).Text, obj13);
  ...
  if (topCriticRating <= 0) {
    ...
    if (null == tier) {
      const obj14 = { variant: "text-xs/medium", color: tmp11(8815).getSteamReviewScoreDescriptionColor(tmp11(1934).SteamReviewScoreDescription.NO_USER_REVIEWS), children: null };
      const intl4 = tmp11(1114).intl;
      obj14.children = intl4.string(tmp11(1114).t["0xYzpO"]);
      tmp12Result2 = tmp12(tmp11(4602).Text, obj14);
      const tmp11Result2 = tmp11(8815);
    }
  } else {
    tmp12Result2 = null;
  }
...
export default function GameProfileReviews(arg0) {
  ...
  if (null == game) {
    ...
    const tmp6 = game.steamReleaseStatus !== tmp2(8782).SteamReleaseStatus.RETIRED_ABANDONED && null != steamWebsiteUrl;
    const reviews = game.reviews;
    let steam;
    if (reviews != null) {
      steam = reviews.steam;
    }
    const calculateSteamReviewScoreDescription = tmp2(8814).calculateSteamReviewScoreDescription;
    if (steam != null) {
    ...
    const tmp2Result = tmp2(8814);
    const result = tmp2(8815).canShowLocalizedSteamReview(steam);
    if (result) {
      let localizedRating;
      if (!tmp12) {
    ...
    const items = [React5(tmp2(4602).Text, obj3), ];
