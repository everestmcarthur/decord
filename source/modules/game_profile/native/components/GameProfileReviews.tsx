...
// Function ID: 8728
// Name: SteamReviewRow
// Dependencies: [19, 17, 21, 4560, 576, 8688, 4255, 8728, 8729, 8691, 1114, 8699, 4556, 1934, 8730, 8736, 8695, 8696, 2]
// Exports: default

// Module 8727 (SteamReviewRow)
import ThemesDefault from "Themes" /* 576 */;
import buildSteamStoreUrl from "buildSteamStoreUrl" /* 8695 */;
...
function SteamReviewRow(url) {
  ...
  obj = {
    onPress: React.useCallback(() => {
      trackAction(url(8691).GameProfileTrackActionActions.SteamReviews);
      dependencyMap(url);
    }, items),
  ...
  obj[2] = intl.string(url(1114).t.YNC5Di);
  ...
  const items2 = [callback(url(8699).SteamNeutralIcon, obj1), callback(url(4556).Text, { variant: "heading-sm/medium", color: "mobile-text-heading-primary", children: title })];
  obj[1] = items2;
  ...
  obj3[2] = url(8729).getSteamReviewScoreDescriptionIntl(result);
  const items4 = [callback(url(4556).Text, obj3), ];
  ...
  if (tmp12Result) {
    ...
    obj4[2] = intl2.format(tmp6(1114).t.sgIoin, obj5).toString();
    tmp12Result = tmp12(tmp6(4556).Text, obj4);
...
function OpenCriticReview(url) {
  ...
  const callback = React.useCallback(() => {
    trackAction(url(8691).GameProfileTrackActionActions.OpenCriticReviews);
    dependencyMap(url);
  }, items);
  if (null != tier) {
    obj1 = url(8730);
  ...
  const intl = url(1114).intl;
  obj[2] = intl.string(url(1114).t.aLNBAw);
  obj[3] = tmp.reviewRow;
  obj = { variant: "heading-sm/medium", color: "mobile-text-heading-primary", children: null };
  const intl2 = url(1114).intl;
  obj[2] = intl2.string(url(1114).t["UxvER+"]);
  const items1 = [callback(url(4556).Text, obj), ];
  ...
  if (null != tier) {
    ...
    if (topCriticRating > 0) {
      ...
      if (num > 0) {
        ...
        obj8[1] = tmp12(tmp11(4556).Text, obj9);
  ...
  if (topCriticRating <= 0) {
    ...
    if (null == tier) {
      ...
      obj10[1] = tmp11Result.getSteamReviewScoreDescriptionColor(tmp11(1934).SteamReviewScoreDescription.NO_USER_REVIEWS);
      const intl4 = tmp11(1114).intl;
      obj10[2] = intl4.string(tmp11(1114).t["0xYzpO"]);
      tmp12Result = tmp12(tmp11(4556).Text, obj10);
    }
  } else {
    tmp12Result = null;
  }
...
export default function GameProfileReviews(arg0) {
  ...
  if (null == game) {
    ...
    const tmp6 = game.steamReleaseStatus !== tmp2(8696).SteamReleaseStatus.RETIRED_ABANDONED && null != steamWebsiteUrl;
    const reviews = game.reviews;
    let steam;
    if (reviews != null) {
      steam = reviews.steam;
    }
    let tmp2Result = tmp2(8728);
    ...
    obj[3] = intl.string(tmp2(1114).t.GaAQXP);
    const items = [callback(tmp2(4556).Text, obj), ];
    ...
    if (tmp10) {
      ...
      if (null != steamWebsiteUrl) {
        ...
        obj2[3] = intl2.string(tmp2(1114).t.MQGNsN);
