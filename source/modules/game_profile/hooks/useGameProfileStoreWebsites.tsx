...
let items = [fn(8836).ThirdPartyGameApplicationWebsiteCategory.EPICGAMES, fn(8836).ThirdPartyGameApplicationWebsiteCategory.STEAM, fn(8836).ThirdPartyGameApplicationWebsiteCategory.ROBLOX, fn(8836).ThirdPartyGameApplicationWebsiteCategory.BATTLENET, fn(8836).ThirdPartyGameApplicationWebsiteCategory.RIOT, fn(8836).ThirdPartyGameApplicationWebsiteCategory.MINECRAFT];
const size = fn(2);
  ...
  return websites.useMemo(() => {
    if (null != websites) {
      if (null != id1) {
        ...
        if (!someResult) {
          someResult = found.some((category) => category.category === steamWebsiteUrl(8836).ThirdPartyGameApplicationWebsiteCategory.STEAM);
        }
        if (!someResult) {
        ...
        const sorted = found.sort((category, category2) => {
          let num = -1;
          if (category.category !== steamWebsiteUrl(8836).ThirdPartyGameApplicationWebsiteCategory.STEAM) {
            let num2 = 0;
            if (category2.category === steamWebsiteUrl(8836).ThirdPartyGameApplicationWebsiteCategory.STEAM) {
              num2 = 1;
            }
            num = num2;
          }
