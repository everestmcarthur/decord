...
// Dependencies: [8819, 1114, 8820, 8821, 8822, 8823, 2]
// Exports: getOpenCriticCircleRatingColor, getOpenCriticTierImage, getOpenCriticTierText

// Module 8818 (useGameProfileOpenCritic)
import OpenCriticTier from "OpenCriticTier" /* 8819 */;
...
export const getOpenCriticTierText = function getOpenCriticTierText(tier) {
  ...
  } else if (tmp(8819).OpenCriticTier.STRONG === tier) {
    const intl3 = tmp(1114).intl;
    return intl3.string(tmp(1114).t.MLxnSg);
  } else if (tmp(8819).OpenCriticTier.FAIR === tier) {
    const intl2 = tmp(1114).intl;
    return intl2.string(tmp(1114).t["3f19KA"]);
  } else if (tmp(8819).OpenCriticTier.WEAK === tier) {
...
export const getOpenCriticCircleRatingColor = function getOpenCriticCircleRatingColor(tier) {
  ...
  if (OpenCriticTier.OpenCriticTier.MIGHTY !== tier) {
    ...
    if (tmp(8819).OpenCriticTier.STRONG !== tier) {
      foregroundColor = "#4aa1ce";
      if (tmp(8819).OpenCriticTier.FAIR !== tier) {
        foregroundColor = "";
        if (tmp(8819).OpenCriticTier.WEAK === tier) {
