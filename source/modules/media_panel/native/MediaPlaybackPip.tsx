...
// Dependencies: [32, 19, 17, 1957, 4795, 4222, 1371, 1074, 17122, 21, 4574, 576, 4275, 504, 4727, 8294, 4570, 17260, 5001, 7470, 7261, 4310, 4575, 1114, 8934, 5642, 4527, 1242, 14564, 4199, 17257, 8304, 8302, 15088, 2]
// Exports: default

// Module 17259 (MediaPlaybackPip)
...
function MediaInfo(message) {
  ...
  const token = message(4275).useToken(first(576).colors.BACKGROUND_SURFACE_HIGH);
  let obj = message(4275);
  ...
  if (null != message) {
    if (null != contentMessage) {
      if (null != activeMediaPlayerSource) {
        if (isVoiceMessage) {
          ...
          if (contentMessage.attachments.length > 0) {
            ...
            if (null != activeMediaPlayerSource.attachmentIndex) {
              str2 = tmp4(8294)(contentMessage.attachments[activeMediaPlayerSource.attachmentIndex]);
            }
          }
        }
        const obj3 = {
        ...
        const tmp14 = closure_15(tmp2(4570).Text, obj3);
        const obj4 = {
        ...
        if (memo) {
          ...
          const items3 = [tmp13(tmp2(17260).Marquee, obj7), ];
        ...
        if (tmp13Result) {
          ...
          tmp13Result = tmp13(tmp2(4570).Text, obj9);
        }
...
function PiPControls(message) {
  ...
  let obj = message(4310);
  fn.__closure = { withTiming: message(4575).withTiming, visible };
  ...
  obj3.style = items1;
  const items2 = [closure_15(message(8934).BackgroundBlurFill, { blurAmount: 0.05 }), , ];
  const obj4 = { disabled: !visible, style: null, onPress: callback, accessible: true, accessibilityRole: "button", accessibilityLabel: stringResult, children: closure_15(message(5642).ArrowLargeLeftIcon, { size: "sm" }) };
  const items3 = [, ];
  ({ pipButton: arr4[0], backButton: arr4[1] } = tmp);
  obj4.style = items3;
  items2[1] = closure_15(closure_6, obj4);
  const obj5 = { disabled: !visible, style: null, onPress: message.handleClosePip, accessible: true, accessibilityRole: "button", accessibilityLabel: string2Result, children: closure_15(message(4527).XLargeIcon, { size: "sm" }) };
  const items4 = [, ];
  ({ pipButton: arr5[0], dismissButton: arr5[1] } = tmp);
  obj5.style = items4;
  items2[2] = closure_15(closure_6, obj5);
  obj3.children = items2;
  return closure_16(visible(4310).View, obj3);
}
...
const SquarePIPReferenceDimensions = fn(17122).SquarePIPReferenceDimensions;
const jsxProd = fn(21);
...
export default function MediaPlaybackPip() {
  ...
  const memo = obj.useMemo(() => {
    if (progress) {
      let PlayIcon = tmp2(8304).PauseIcon;
    } else {
      PlayIcon = tmp2(8302).PlayIcon;
    }
