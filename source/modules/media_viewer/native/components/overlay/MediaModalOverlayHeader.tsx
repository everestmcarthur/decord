...
// Dependencies: [19, 17, 8450, 21, 4606, 13055, 8420, 11755, 4464, 4343, 4607, 8454, 8455, 1114, 5731, 1178, 576, 4602, 13059, 8757, 1115, 13060, 8763, 2]
// Exports: MediaModalOverlayHeader

// Module 13058 (MediaModalOverlayHeader)
...
import useShouldHideMediaOptionsDefault from "useShouldHideMediaOptions" /* 11755 */;
import noop from "module_19" /* 19 */;

require = fn;
get_ActivityIndicator = fn(17);
({ StyleSheet: c3, View: closure_4 } = get_ActivityIndicator);
const SHARE_PREPARING_MODAL_KEY = fn(8450).SHARE_PREPARING_MODAL_KEY;
const jsxProd = fn(21);
...
export const MediaModalOverlayHeader = function MediaModalOverlayHeader(arg0) {
  ...
  const tmp = closure_8();
  const headerLayoutAnimation = isModalOpen(13055).useHeaderLayoutAnimation(animationDriver);
  let obj = isModalOpen(13055);
  const mediaShareActions = isModalOpen(8420).useMediaShareActions({ source, disableDownload, shareable });
  const tmp7 = useShouldHideMediaOptionsDefault(channelId);
  const obj2 = isModalOpen(8420);
  ...
  fn.__closure = { withTiming: isModalOpen(4607).withTiming, isPreparing: isModalOpen };
  ...
  const obj5 = { withTiming: isModalOpen(4607).withTiming, isPreparing: isModalOpen };
  let intl = tmp2(1114).intl;
  obj11.accessibilityLabel = intl.string(isModalOpen(1114).t.cpT0Cq);
  obj11.icon = closure_6(isModalOpen(5731).XSmallIcon, { size: "md", color: "interactive-text-active" });
  ...
  if (tmp10Result3) {
    ...
    obj15.children = tmp10(tmp2(4602).Text, obj16);
  ...
  if (!tmp7) {
    ...
    const obj18 = { source };
    const items4 = [tmp10(tmp6(13059), obj18), ];
    ...
    if (!disableMediaOverlayButton) {
      const obj19 = {
        ...
        children(ref) {
              ...
              if (obj2.isAndroid()) {
                let tmp2Result = tmp2(tmp5(13060).MoreVerticalIcon, { size: "md", color: "interactive-text-active" });
              } else {
                tmp2Result = tmp2(tmp5(8763).MoreHorizontalIcon, { size: "md", color: "interactive-text-active" });
              }
              obj.icon = tmp2Result;
              obj.ref = ref.ref;
      ...
      tmp10Result4 = tmp10(tmp2(8757).ContextMenu, obj19);
    }
  ...
  obj8.children = closure_7(isModalOpen(8454).MediaModalOverlayHeaderWrapper, obj9);
