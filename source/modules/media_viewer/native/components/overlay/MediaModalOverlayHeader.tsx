...
// Dependencies: [19, 17, 8452, 21, 4607, 13057, 8422, 11757, 4464, 4343, 4608, 8456, 8457, 1114, 5732, 1178, 576, 4603, 13061, 8759, 1115, 13062, 8765, 2]
// Exports: MediaModalOverlayHeader

// Module 13060 (MediaModalOverlayHeader)
import ReanimatedRexportDefault from "ReanimatedRexport" /* 4343 */;
import timing from "timing" /* 4608 */;
import MediaViewerOverlayButtonDefault from "MediaViewerOverlayButton" /* 8457 */;
import useShouldHideMediaOptionsDefault from "useShouldHideMediaOptions" /* 11757 */;
import noop from "module_19" /* 19 */;

require = fn;
get_ActivityIndicator = fn(17);
({ StyleSheet: c3, View: closure_4 } = get_ActivityIndicator);
const SHARE_PREPARING_MODAL_KEY = fn(8452).SHARE_PREPARING_MODAL_KEY;
const jsxProd = fn(21);
...
export const MediaModalOverlayHeader = function MediaModalOverlayHeader(arg0) {
  ...
  const tmp = closure_8();
  const headerLayoutAnimation = isModalOpen(13057).useHeaderLayoutAnimation(animationDriver);
  let obj = isModalOpen(13057);
  const mediaShareActions = isModalOpen(8422).useMediaShareActions({ source, disableDownload, shareable });
  const tmp7 = useShouldHideMediaOptionsDefault(channelId);
  const obj2 = isModalOpen(8422);
  isModalOpen = isModalOpen(4464).useIsModalOpen(SHARE_PREPARING_MODAL_KEY);
  const obj3 = isModalOpen(4464);
  const fn = function w() {
    let num = 1;
    if (isModalOpen) {
      num = 0;
  ...
  const obj4 = isModalOpen(4343);
  fn.__closure = { withTiming: isModalOpen(4608).withTiming, isPreparing: isModalOpen };
  ...
  const obj5 = { withTiming: isModalOpen(4608).withTiming, isPreparing: isModalOpen };
  let intl = tmp2(1114).intl;
  obj11.accessibilityLabel = intl.string(isModalOpen(1114).t.cpT0Cq);
  obj11.icon = closure_6(isModalOpen(5732).XSmallIcon, { size: "md", color: "interactive-text-active" });
  ...
  if (tmp10Result3) {
    ...
    obj15.children = tmp10(tmp2(4603).Text, obj16);
  ...
  if (!tmp7) {
    ...
    const obj18 = { source };
    const items4 = [tmp10(tmp6(13061), obj18), ];
    ...
    if (!disableMediaOverlayButton) {
      const obj19 = {
        ...
        children(ref) {
              ...
              if (obj2.isAndroid()) {
                let tmp2Result = tmp2(tmp5(13062).MoreVerticalIcon, { size: "md", color: "interactive-text-active" });
              } else {
                tmp2Result = tmp2(tmp5(8765).MoreHorizontalIcon, { size: "md", color: "interactive-text-active" });
              }
              obj.icon = tmp2Result;
              obj.ref = ref.ref;
      ...
      tmp10Result4 = tmp10(tmp2(8759).ContextMenu, obj19);
    }
  ...
  obj8.children = closure_7(isModalOpen(8456).MediaModalOverlayHeaderWrapper, obj9);
