...
// Dependencies: [19, 17, 1958, 1074, 4788, 1385, 7402, 7407, 1477, 8396, 7439, 8004, 4975, 8062, 4863, 1895, 8239, 8255, 1370, 8397, 1094, 4335, 8391, 4983, 1115, 8398, 2]
...
import transformMessageComponents from "transformMessageComponents" /* 8239 */;
import RowGeneratorTypes from "RowGeneratorTypes" /* 8255 */;
import useStateFromSharedValueDefault from "useStateFromSharedValue" /* 8397 */;
import NativePortalView from "NativePortalView" /* 8398 */;
        ...
        if (found.height > 0) {
          ...
          if (null != found.proxy_url) {
            ...
            if (isVideoFileResult) {
              ...
              if (!hasFlagResult) {
                hasFlagResult = tmp43(7439).isChannelSpoilerGated(ChannelStore.getChannel(message2.channel_id));
                const tmp43Result5 = tmp43(7439);
              }
              ...
              if (!hasFlagResult1) {
                hasFlagResult1 = tmp43(7439).isChannelSpoilerGated(ChannelStore.getChannel(message2.channel_id));
                const tmp43Result7 = tmp43(7439);
              }
              ...
              const formatted = tmp(8396)(found).toLowerCase();
              const str3 = tmp(8396)(found);
              const endsWithResult = formatted.endsWith(".webp");
              const formatted1 = tmp(8396)(found).toLowerCase();
              const endsWithResult1 = formatted1.endsWith(".avif");
              const str5 = tmp(8396)(found);
...
function toMediaSourceFromUnfurledMedia(id, guildId, media, description, spoiler) {
  ...
  if (unfurledMediaItemType === RowGeneratorTypes.MediaGalleryItemType.VISUAL_PLACEHOLDER) {
    ...
    const tmp4 = unfurledMediaItemType === tmp(8255).MediaGalleryItemType.VIDEO;
    let str;
    if (tmp4) {
      str = "png";
    }
    ...
    const enabledHarmTypesForMessage = tmp(7402).getEnabledHarmTypesForMessage(id);
    const tmpResult = tmp(7402);
    const obj2 = { type: tmp(7407).ObscuredMediaTypes.GenericMedia, media };
    ...
    const result = tmp(7402).isMediaObscuredForHarmTypes(obj2, enabledHarmTypesForMessage);
    if (width2 == null) {
      width2 = 0;
    }
    size.width = width2;
...
export const extractMediaFromMessageComponents = function extractMediaFromMessageComponents(getContentMessage, contentMessage, getContentMessage) {
  if (0 === contentMessage.components.length) {
    ...
    while (iter !== undefined) {
      ...
      } else if (tmp6(1895).ComponentType.THUMBNAIL === type) {
...
export const downloadMediaAsset = function downloadMediaAsset(mediaUrl, VIDEO) {
  ...
  return MediaManager.downloadMediaAsset(mediaUrl, VIDEO).then(() => {
    if (ConstantsIOS.MediaType.IMAGE === closure_0) {
      tmp2(4335).presentImageSaved();
      const tmp2Result = tmp2(4335);
    } else if (tmp2(1094).MediaType.GIF === tmp) {
      tmp2(4335).presentGifSaved();
      const tmp2Result3 = tmp2(4335);
    } else if (tmp2(1094).MediaType.VIDEO === tmp) {
      tmp2(4335).presentVideoSaved();
      const tmp2Result4 = tmp2(4335);
    }
    const MediaViewerAnalytics = tmp2(8391).MediaViewerAnalytics;
...
export const isAnimatedImageSource = function isAnimatedImageSource(source) {
  ...
  if (!result) {
    ...
    if (result1) {
      result1 = tmp(4788).urlMatchesFileExtension(source.sourceURI, React5);
  ...
  if (!result) {
    ...
    if (result2) {
      result2 = tmp(4788).urlMatchesFileExtension(source.sourceURI, re8);
