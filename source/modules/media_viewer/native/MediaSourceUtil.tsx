...
// Dependencies: [19, 17, 1958, 1074, 4789, 1385, 7399, 7404, 1477, 8387, 7436, 7995, 4976, 8053, 4864, 1895, 8230, 8246, 1370, 8388, 1094, 4337, 8382, 4984, 1115, 8389, 2]
        ...
        if (found.height > 0) {
          ...
          if (null != found.proxy_url) {
            ...
            if (isVideoFileResult) {
              ...
              if (!hasFlagResult) {
                hasFlagResult = tmp43(7436).isChannelSpoilerGated(ChannelStore.getChannel(message2.channel_id));
                const tmp43Result5 = tmp43(7436);
              }
              ...
              if (!hasFlagResult1) {
                hasFlagResult1 = tmp43(7436).isChannelSpoilerGated(ChannelStore.getChannel(message2.channel_id));
                const tmp43Result7 = tmp43(7436);
              }
              ...
              const formatted = tmp(8387)(found).toLowerCase();
              const str3 = tmp(8387)(found);
              const endsWithResult = formatted.endsWith(".webp");
              const formatted1 = tmp(8387)(found).toLowerCase();
              const endsWithResult1 = formatted1.endsWith(".avif");
              const str5 = tmp(8387)(found);
...
function toMediaSourceFromUnfurledMedia(id, guildId, media, description, spoiler) {
  ...
  if (unfurledMediaItemType === RowGeneratorTypes.MediaGalleryItemType.VISUAL_PLACEHOLDER) {
    ...
    const tmp4 = unfurledMediaItemType === tmp(8246).MediaGalleryItemType.VIDEO;
    let str;
    if (tmp4) {
      str = "png";
    }
    ...
    const enabledHarmTypesForMessage = tmp(7399).getEnabledHarmTypesForMessage(id);
    const tmpResult = tmp(7399);
    const obj2 = { type: tmp(7404).ObscuredMediaTypes.GenericMedia, media };
    ...
    const result = tmp(7399).isMediaObscuredForHarmTypes(obj2, enabledHarmTypesForMessage);
    if (width2 == null) {
      width2 = 0;
    }
    size.width = width2;
...
export const downloadMediaAsset = function downloadMediaAsset(mediaUrl, VIDEO) {
  ...
  return MediaManager.downloadMediaAsset(mediaUrl, VIDEO).then(() => {
    ...
    const MediaViewerAnalytics = tmp2(8382).MediaViewerAnalytics;
...
export const isAnimatedImageSource = function isAnimatedImageSource(source) {
  ...
  if (!result) {
    ...
    if (result1) {
      result1 = tmp(4789).urlMatchesFileExtension(source.sourceURI, React5);
  ...
  if (!result) {
    ...
    if (result2) {
      result2 = tmp(4789).urlMatchesFileExtension(source.sourceURI, re8);
