...
// Dependencies: [5, 32, 19, 17, 1980, 8522, 21, 4639, 576, 504, 4792, 5500, 1895, 9871, 4337, 4606, 7257, 7256, 1115, 5059, 9896, 1966, 2]
// Exports: default

// Module 16977 (AddModeratorsActionSheet)
...
export default function AddModeratorsActionSheet(channel) {
    ...
    const tmp = asyncGeneratorStep(async () => {
      ...
      const mapped = found.map((row) => {
        ...
        if (row.rowType === constants.ROLE) {
          ...
          let moderatorOverwrite = channel(5500).createModeratorOverwrite(row.id, channel(1895).PermissionOverwriteType.ROLE, closure_0);
          const obj2 = channel(5500);
        } else {
          closure_0 = closure_0 + 1;
          moderatorOverwrite = channel(5500).createModeratorOverwrite(row.id, channel(1895).PermissionOverwriteType.MEMBER, closure_0);
  ...
  if (null == stateFromStores) {
    ...
    if (channel.canSkip) {
      ...
      obj4.header = tmp12(tmp4(7256).BottomSheetTitleHeader, obj2);
      ...
      obj5.children = tmp12(pendingAdditions(9896), obj6);
      obj4.children = tmp12(View, obj5);
      return tmp12(tmp4(7257).BottomSheet, obj4);
    }
