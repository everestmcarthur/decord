...
// Function ID: 10146
// Name: StageGridRow
// Dependencies: [19, 17, 21, 4606, 10137, 5476, 5177, 10146, 10136, 2]

// Module 10145 (StageGridRow)
...
export default noop.memo((row) => {
  ...
  if (1 === num) {
    THIRD = tmp2(10137).StageTileSize.FULL;
  } else if (2 === num) {
    THIRD = tmp2(10137).StageTileSize.HALF;
  } else {
    THIRD = tmp2(10137).StageTileSize.THIRD;
  }
  ...
  return tmp4(tmp5, {
    ...
    children: participants.map((type) => {
      ...
      if (flag) {
        ...
        let tmp5Result = tmp5(tmp6(10146), obj2, "stage-media-participant-" + type.id);
        const tmp6Result = tmp6(10146);
      } else {
        const obj = { channel, participant: type, size: THIRD };
        const _HermesInternal = HermesInternal;
        tmp5Result = tmp5(tmp6(10136), obj, "stage-user-participant-" + type.id);
        const tmp6Result2 = tmp6(10136);
      }
      return tmp5Result;
    })
  });
