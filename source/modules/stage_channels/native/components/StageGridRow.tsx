...
// Function ID: 10148
// Name: StageGridRow
// Dependencies: [19, 17, 21, 4607, 10139, 5477, 5178, 10148, 10138, 2]

// Module 10147 (StageGridRow)
...
export default noop.memo((row) => {
  ...
  if (1 === num) {
    THIRD = tmp2(10139).StageTileSize.FULL;
  } else if (2 === num) {
    THIRD = tmp2(10139).StageTileSize.HALF;
  } else {
    THIRD = tmp2(10139).StageTileSize.THIRD;
  }
  ...
  return tmp4(tmp5, {
    ...
    children: participants.map((type) => {
      ...
      if (flag) {
        ...
        let tmp5Result = tmp5(tmp6(10148), obj2, "stage-media-participant-" + type.id);
        const tmp6Result = tmp6(10148);
      } else {
        const obj = { channel, participant: type, size: THIRD };
        const _HermesInternal = HermesInternal;
        tmp5Result = tmp5(tmp6(10138), obj, "stage-user-participant-" + type.id);
        const tmp6Result2 = tmp6(10138);
      }
      return tmp5Result;
    })
  });
