...
// Function ID: 10126
// Name: StageGridRow
// Dependencies: [19, 17, 21, 4605, 10117, 5475, 5176, 10126, 10116, 2]

// Module 10125 (StageGridRow)
...
export default noop.memo((row) => {
  ...
  if (1 === num) {
    THIRD = tmp2(10117).StageTileSize.FULL;
  } else if (2 === num) {
    THIRD = tmp2(10117).StageTileSize.HALF;
  } else {
    THIRD = tmp2(10117).StageTileSize.THIRD;
  }
  ...
  return tmp4(tmp5, {
    ...
    children: participants.map((type) => {
      ...
      if (flag) {
        ...
        let tmp5Result = tmp5(tmp6(10126), obj2, "stage-media-participant-" + type.id);
        const tmp6Result = tmp6(10126);
      } else {
        const obj = { channel, participant: type, size: THIRD };
        const _HermesInternal = HermesInternal;
        tmp5Result = tmp5(tmp6(10116), obj, "stage-user-participant-" + type.id);
        const tmp6Result2 = tmp6(10116);
      }
      return tmp5Result;
    })
  });
