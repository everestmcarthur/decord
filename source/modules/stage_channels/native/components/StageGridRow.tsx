...
// Function ID: 10206
// Name: StageGridRow
// Dependencies: [19, 17, 21, 4640, 10197, 5513, 5213, 10206, 10196, 2]

// Module 10205 (StageGridRow)
import useIsScreenLandscape from "useIsScreenLandscape" /* 5213 */;
...
export default noop.memo((row) => {
  ...
  if (1 === num) {
    THIRD = tmp2(10197).StageTileSize.FULL;
  } else if (2 === num) {
    THIRD = tmp2(10197).StageTileSize.HALF;
  } else {
    THIRD = tmp2(10197).StageTileSize.THIRD;
  }
  ...
  return tmp4(tmp5, {
    ...
    children: participants.map((type) => {
      ...
      if (flag) {
        ...
        let tmp5Result = tmp5(tmp6(10206), obj2, "stage-media-participant-" + type.id);
        const tmp6Result = tmp6(10206);
      } else {
        const obj = { channel, participant: type, size: THIRD };
        const _HermesInternal = HermesInternal;
        tmp5Result = tmp5(tmp6(10196), obj, "stage-user-participant-" + type.id);
        const tmp6Result2 = tmp6(10196);
      }
      return tmp5Result;
    })
  });
