...
// Dependencies: [32, 19, 17, 4714, 4737, 10144, 5584, 1074, 21, 4722, 576, 9735, 1115, 10145, 8630, 504, 10146, 10152, 10153, 10154, 10156, 10158, 8853, 9311, 10159, 8626, 9320, 7358, 5601, 5595, 4689, 10160, 1897, 10172, 10173, 1612, 7393, 5862, 4718, 5144, 8643, 8645, 10175, 10176, 5592, 10177, 8854, 10178, 10180, 10182, 9837, 5240, 10183, 4458, 5143, 9919, 8831, 8640, 8625, 5292, 1177, 10184, 2]
...
// Module 10143 (StageActionBarButtons)
...
import StageChannelActionCreatorExtras from "StageChannelActionCreatorExtras" /* 8626 */;
...
import useStageChannelConnectAction from "useStageChannelConnectAction" /* 9919 */;
import _modDef10145 from "module_10145" /* 10145 */;
import StageMusicActionCreators from "StageMusicActionCreators" /* 10158 */;
import shouldShowEndStageModalDefault from "shouldShowEndStageModal" /* 10159 */;
...
import StageMusicStore from "StageMusicStore" /* 10144 */;
...
let closure_10 = fn(5584).REQUEST_TO_SPEAK_SHEET_KEY;
...
export const MusicMuteButton = function MusicMuteButton(arg0) {
  ...
  if (!obj2.useShowStageMusicMuteButton(channel.id)) {
    ...
    const obj3 = { accessibilityLabel: stringResult, source: importDefault(stateFromStores ? 10152 : 10153), IconComponent: null, imageStyle: null, onPress: null, isSmallSize: null };
    if (stateFromStores) {
      tmpResult = tmp(10154);
      let MusicIcon = tmpResult.MusicSlashIcon;
    } else {
      MusicIcon = tmp(10156).MusicIcon;
    }
...
export const DisconnectStageButton = function DisconnectStageButton(channel) {
  ...
  obj.source = _modDef8853;
  obj.IconComponent = channel(9311).DoorExitIcon;
  obj.onPress = function onPress() {
    if (shouldShowEndStageModalDefault(channel)) {
      tmp3(8626).openEndStageModal(tmp2);
      const tmp3Result = tmp3(8626);
    } else {
      tmp3(9320).handleDisconnect(tmp2);
      const tmp3Result2 = tmp3(9320);
    }
  };
  obj.isSmallSize = channel.isSmallSize;
...
export const RequestToSpeakListButton = function RequestToSpeakListButton(channel) {
  ...
  function handleOpenAudienceList() {
    ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(10160, dependencyMap.paths), closure_10, { channelId: channel.id, analyticsLocations });
  }
  analyticsLocations = analyticsLocations(7358)().analyticsLocations;
  const stageParticipantsCount = channel(5601).useStageParticipantsCount(channel.id, channel(5595).StageChannelParticipantNamedIndex.REQUESTED_TO_SPEAK_ONLY);
  if (stageParticipantsCount > 0) {
    ...
    obj2.source = tmp(10172);
    const obj4 = { tintColor: tmp(576).unsafe_rawColors.WHITE };
    obj2.imageStyle = obj4;
    obj2.IconComponent = tmp3(10173).HandRequestSpeakListIcon;
    ...
    obj5.source = tmp(10172);
    const obj6 = { tintColor: tmp(576).unsafe_rawColors.WHITE };
    obj5.imageStyle = obj6;
    obj5.IconComponent = tmp3(10173).HandRequestSpeakListIcon;
...
export const ChatButton = function ChatButton(channel) {
  ...
  const isVoiceChannelLocked = channel(10182).useIsVoiceChannelLocked(channel);
  const obj2 = channel(10182);
  const voiceChatNavigationContext = channel(9837).useVoiceChatNavigationContext();
  let openChat;
  ...
  if (mentionCount <= 0) {
    if (unreadCount <= 0) {
      ...
      obj4.IconComponent = tmp(5240).ChatIcon;
      obj4.source = isVoiceChannelLocked(10183);
      obj4.onPress = onPress;
      obj4.appearsDisabled = isVoiceChannelLocked;
      obj4.isSmallSize = isSmallSize;
      let tmp7Result = closure_12(openChat(9735).ActionButton, obj4);
    }
    return tmp7Result;
  }
  if (mentionCount > 0) {
  ...
  obj6.IconComponent = channel(5240).ChatIcon;
  obj6.source = isVoiceChannelLocked(10183);
  obj6.onPress = onPress;
  obj6.appearsDisabled = isVoiceChannelLocked;
  obj6.isSmallSize = isSmallSize;
  tmp7Result = tmp7(openChat(9735).NotifiedActionButton, obj6);
};
...
export const JoinStagePrompt = function JoinStagePrompt(channel) {
  ...
  return closure_12(channel(8831).FormCTA, obj);
};
export const ContinueToStagePrompt = function ContinueToStagePrompt(onContinue) {
  ...
  const obj5 = { children: closure_1_12(native.Icon, { style: tmp.continueIcon, source: _modDef10184, size: native.Icon.Sizes.SMALL, disableColor: true }) };
