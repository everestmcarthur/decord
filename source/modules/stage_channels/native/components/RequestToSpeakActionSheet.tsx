...
// Dependencies: [32, 19, 17, 502, 1957, 5466, 21, 4607, 576, 10009, 7274, 1114, 504, 4754, 10010, 10012, 5657, 5474, 8499, 8501, 4574, 8486, 1178, 10014, 4308, 7236, 7256, 5483, 5477, 7224, 6699, 5025, 5739, 4603, 10015, 2]
// Exports: default

// Module 10008 (RequestToSpeakActionSheet)
import nativeDefault from "native" /* 576 */;
import util from "util" /* 1114 */;
import ActionSheetActionCreatorsDefault from "ActionSheetActionCreators" /* 4574 */;
import useAudienceRequestToSpeakStateDefault from "useAudienceRequestToSpeakState" /* 4754 */;
import useStageSpeakingForCurrentUser from "useStageSpeakingForCurrentUser" /* 5474 */;
...
import useRequestToSpeakPermission from "useRequestToSpeakPermission" /* 10009 */;
import _slicedToArray from "module_32" /* 32 */;
import noop from "module_19" /* 19 */;
import AuthenticationStore from "AuthenticationStore" /* 502 */;
import ChannelStore from "ChannelStore" /* 1957 */;

const _modDef10014 = tmp4(10014);
require = fn;
...
function ManageSelfSpeakerRow(channel) {
  ...
  const tmp8 = useAudienceRequestToSpeakStateDefault(stateFromStores, id) === channel(4754).RequestToSpeakStates.ON_STAGE;
  importDefault = tmp8;
  ...
  if (tmp8) {
    let MicrophoneArrowRightIcon = tmp(10010).GroupArrowDownIcon;
  } else {
    MicrophoneArrowRightIcon = tmp(10012).MicrophoneArrowRightIcon;
  }
  let obj2 = {
    onPress() {
      if (!closure_1) {
  ...
  obj2.trailing = closure_9(channel(1178).Icon, { source: _modDef10014 });
  return closure_9(channel(5657).TableRow, obj2);
}
const View = fn(17).View;
let closure_8 = fn(5466).REQUEST_TO_SPEAK_SHEET_KEY;
const jsxProd = fn(21);
...
export default function RequestToSpeakActionSheet(channelId) {
  ...
  const tmp6 = first(7236);
  items[HermesBuiltin.arraySpread(channelId.analyticsLocations, 0)] = first(7256).REQUEST_TO_SPEAK;
  ...
  const stageParticipantsCount = channelId(5483).useStageParticipantsCount(channelId, channelId(5477).StageChannelParticipantNamedIndex.ALL_REQUESTED_TO_SPEAK);
  ...
  if (null != stateFromStores) {
    ...
    const items3 = [closure_10(tmp(5739).TableRowGroup, obj9), ];
    ...
    obj12.children = closure_9(tmp(4603).Text, obj14);
    items3[1] = closure_9(View, obj12);
    obj8.children = items3;
    const items4 = [closure_10(tmp(5025).Stack, obj8), ];
    const obj16 = { channel: stateFromStores, height: null };
    const _Math = Math;
    obj16.height = Math.max(first1 - first - 8, 0);
    items4[1] = closure_9(tmp3(10015), obj16);
    obj7.children = items4;
    obj6.children = closure_10(tmp(5025).Stack, obj7);
    obj5.children = closure_9(tmp(6699).BottomSheetScrollView, obj6);
    obj4.children = closure_9(tmp(7224).BottomSheet, obj5);
    tmp14 = closure_9(tmp(7236).AnalyticsLocationProvider, obj4);
    const tmp3Result = tmp3(10015);
  }
  return tmp14;
};
