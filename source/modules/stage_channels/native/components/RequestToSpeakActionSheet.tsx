...
// Dependencies: [32, 19, 17, 502, 1958, 5584, 21, 4722, 576, 10161, 7396, 1115, 504, 4869, 10162, 10164, 5775, 5592, 8643, 8645, 4689, 8630, 1177, 10166, 4423, 7358, 7378, 5601, 5595, 7346, 6820, 5142, 5857, 4718, 10167, 2]
// Exports: default

// Module 10160 (RequestToSpeakActionSheet)
...
import useRequestToSpeakPermission from "useRequestToSpeakPermission" /* 10161 */;
import _slicedToArray from "module_32" /* 32 */;
import noop from "module_19" /* 19 */;
import AuthenticationStore from "AuthenticationStore" /* 502 */;
import ChannelStore from "ChannelStore" /* 1958 */;

const _modDef10166 = tmp4(10166);
require = fn;
...
function ManageSelfSpeakerRow(channel) {
  ...
  const tmp8 = useAudienceRequestToSpeakStateDefault(stateFromStores, id) === channel(4869).RequestToSpeakStates.ON_STAGE;
  importDefault = tmp8;
  ...
  if (tmp8) {
    let MicrophoneArrowRightIcon = tmp(10162).GroupArrowDownIcon;
  } else {
    MicrophoneArrowRightIcon = tmp(10164).MicrophoneArrowRightIcon;
  }
  let obj2 = {
    onPress() {
      if (!closure_1) {
  ...
  obj2.trailing = closure_9(channel(1177).Icon, { source: _modDef10166 });
  return closure_9(channel(5775).TableRow, obj2);
}
const View = fn(17).View;
let closure_8 = fn(5584).REQUEST_TO_SPEAK_SHEET_KEY;
const jsxProd = fn(21);
...
export default function RequestToSpeakActionSheet(channelId) {
  ...
  let first1;
  noop = undefined;
  const token = channelId(4423).useToken(first(576).modules.mobile.TABLE_ROW_PADDING);
  const obj = channelId(4423);
  const tmp3 = first;
  const items = [];
  const tmp5 = closure_11();
  const tmp6 = first(7358);
  items[HermesBuiltin.arraySpread(channelId.analyticsLocations, 0)] = first(7378).REQUEST_TO_SPEAK;
  ...
  const stageParticipantsCount = channelId(5601).useStageParticipantsCount(channelId, channelId(5595).StageChannelParticipantNamedIndex.ALL_REQUESTED_TO_SPEAK);
  ...
  if (null != stateFromStores) {
    ...
    const items3 = [closure_10(tmp(5857).TableRowGroup, obj9), ];
    ...
    obj12.children = closure_9(tmp(4718).Text, obj14);
    items3[1] = closure_9(View, obj12);
    obj8.children = items3;
    const items4 = [closure_10(tmp(5142).Stack, obj8), ];
    const obj16 = { channel: stateFromStores, height: null };
    const _Math = Math;
    obj16.height = Math.max(first1 - first - 8, 0);
    items4[1] = closure_9(tmp3(10167), obj16);
    obj7.children = items4;
    obj6.children = closure_10(tmp(5142).Stack, obj7);
    obj5.children = closure_9(tmp(6820).BottomSheetScrollView, obj6);
    obj4.children = closure_9(tmp(7346).BottomSheet, obj5);
    tmp14 = closure_9(tmp(7358).AnalyticsLocationProvider, obj4);
    const tmp3Result = tmp3(10167);
  }
  return tmp14;
};
