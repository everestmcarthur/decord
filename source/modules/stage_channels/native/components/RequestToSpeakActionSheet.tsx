...
// Dependencies: [32, 19, 17, 502, 1957, 5464, 21, 4605, 576, 9986, 7252, 1114, 504, 4752, 9987, 9989, 5655, 5472, 8476, 8478, 4572, 8463, 1178, 9991, 4306, 7214, 7234, 5481, 5475, 7202, 6677, 5023, 5737, 4601, 9992, 2]
// Exports: default

// Module 9985 (RequestToSpeakActionSheet)
import nativeDefault from "native" /* 576 */;
import util from "util" /* 1114 */;
import ActionSheetActionCreatorsDefault from "ActionSheetActionCreators" /* 4572 */;
import useAudienceRequestToSpeakStateDefault from "useAudienceRequestToSpeakState" /* 4752 */;
...
import useRequestToSpeakPermission from "useRequestToSpeakPermission" /* 9986 */;
import _slicedToArray from "module_32" /* 32 */;
import noop from "module_19" /* 19 */;
import AuthenticationStore from "AuthenticationStore" /* 502 */;
import ChannelStore from "ChannelStore" /* 1957 */;

const _modDef9991 = tmp4(9991);
require = fn;
...
function ManageSelfSpeakerRow(channel) {
  ...
  const tmp8 = useAudienceRequestToSpeakStateDefault(stateFromStores, id) === channel(4752).RequestToSpeakStates.ON_STAGE;
  importDefault = tmp8;
  ...
  if (tmp8) {
    let MicrophoneArrowRightIcon = tmp(9987).GroupArrowDownIcon;
  } else {
    MicrophoneArrowRightIcon = tmp(9989).MicrophoneArrowRightIcon;
  }
  let obj2 = {
    onPress() {
      if (!closure_1) {
  ...
  obj2.trailing = closure_9(channel(1178).Icon, { source: _modDef9991 });
  return closure_9(channel(5655).TableRow, obj2);
}
const View = fn(17).View;
let closure_8 = fn(5464).REQUEST_TO_SPEAK_SHEET_KEY;
const jsxProd = fn(21);
...
export default function RequestToSpeakActionSheet(channelId) {
  ...
  let first1;
  noop = undefined;
  const token = channelId(4306).useToken(first(576).modules.mobile.TABLE_ROW_PADDING);
  const obj = channelId(4306);
  const tmp3 = first;
  const items = [];
  const tmp5 = closure_11();
  const tmp6 = first(7214);
  items[HermesBuiltin.arraySpread(channelId.analyticsLocations, 0)] = first(7234).REQUEST_TO_SPEAK;
  ...
  const stageParticipantsCount = channelId(5481).useStageParticipantsCount(channelId, channelId(5475).StageChannelParticipantNamedIndex.ALL_REQUESTED_TO_SPEAK);
  ...
  if (null != stateFromStores) {
    ...
    const items3 = [closure_10(tmp(5737).TableRowGroup, obj9), ];
    ...
    obj12.children = closure_9(tmp(4601).Text, obj14);
    items3[1] = closure_9(View, obj12);
    obj8.children = items3;
    const items4 = [closure_10(tmp(5023).Stack, obj8), ];
    const obj16 = { channel: stateFromStores, height: null };
    const _Math = Math;
    obj16.height = Math.max(first1 - first - 8, 0);
    items4[1] = closure_9(tmp3(9992), obj16);
    obj7.children = items4;
    obj6.children = closure_10(tmp(5023).Stack, obj7);
    obj5.children = closure_9(tmp(6677).BottomSheetScrollView, obj6);
    obj4.children = closure_9(tmp(7202).BottomSheet, obj5);
    tmp14 = closure_9(tmp(7214).AnalyticsLocationProvider, obj4);
    const tmp3Result = tmp3(9992);
  }
  return tmp14;
};
