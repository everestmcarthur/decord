...
// Function ID: 10149
// Name: MediaTile
// Dependencies: [19, 17, 4623, 4628, 21, 4607, 576, 10138, 1477, 5178, 504, 10149, 1178, 2]

// Module 10148 (MediaTile)
...
export default noop.memo((channel) => {
  ...
  const tmp = closure_7();
  const speakerTileStyles = channel(10138).useSpeakerTileStyles();
  const obj = channel(10138);
  const tmp5 = participant;
  const isScreenLandscape = channel(5178).useIsScreenLandscape();
  const obj2 = channel(5178);
  ...
  if (null != stateFromStores) {
    ...
    if (stateFromStores.type !== ParticipantTypes.ACTIVITY) {
      ...
      const items2 = [tmp.container, tmp2(10138).getSizeStyle(size, speakerTileStyles), ];
      const tmp2Result = tmp2(10138);
      items2[2] = tmp2(10138).getTileWidthStyle(size, participant(1477)().width, isScreenLandscape);
      obj4.style = items2;
      ...
      const tmp2Result2 = tmp2(10138);
      obj5.avatarSize = tmp2(1178).AvatarSizes.XLARGE;
      obj5.channel = channel;
      obj5.contentStyle = tmp.media;
      obj4.children = jsx(tmp5(10149), { hasBottomSafeArea: false, hasLeftSafeArea: false, hasRightSafeArea: false, hasTopSafeArea: false, participant: stateFromStores, avatarSize: null, channel: null, shrinkStreamEmptyState: false, contentStyle: null });
      tmp8 = <View style={null}>{null}</View>;
      const tmp5Result = tmp5(10149);
    }
  }
  return tmp8;
});
