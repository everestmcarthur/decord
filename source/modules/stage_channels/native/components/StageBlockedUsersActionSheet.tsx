...
// Dependencies: [32, 19, 17, 4255, 5472, 5465, 21, 4606, 576, 504, 1114, 1178, 8710, 4602, 4573, 8711, 7196, 5026, 7223, 7145, 2]
// Exports: default

// Module 8709 (StageBlockedUsersActionSheet)
...
function RestrictedUser(guildId) {
  ...
  if (speaker) {
    ...
    obj6.style = items3;
    const obj7 = { style: tmp.icon, source: user(8710), color: user(576).unsafe_rawColors.WHITE };
...
let closure_8 = fn(5465).STAGE_BLOCKED_USERS_SHEET_KEY;
const jsxProd = fn(21);
...
export default function StageBlockedUsersActionSheet(channel) {
  ...
  const stageBlockedUsers = channel(8711).useStageBlockedUsers(channel.id);
  const obj = channel(8711);
  const stageIgnoredUsers = channel(8711).useStageIgnoredUsers(channel.id);
  ...
  const items = [closure_9(channel(5026).Button, obj4), ];
  ...
  obj5.onPress = function handleDismiss() {
    onAccept(4573).hideActionSheet(closure_1_8);
  };
  items[1] = closure_9(channel(5026).Button, obj5);
  obj3.children = items;
  items1 = [];
  const obj2 = channel(8711);
  HermesBuiltin.arraySpread(stageIgnoredUsers, HermesBuiltin.arraySpread(stageBlockedUsers, 0));
  const obj6 = { scrollable: true, header: closure_9(StageBlockedUsersActionSheetHeader, { blockedUserCount: stageBlockedUsers.length, ignoredUserCount: stageIgnoredUsers.length }), footer: closure_10(channel(7196).SafeAreaPaddingView, obj3), children: null };
  ...
  const tmp4 = closure_10(channel(7196).SafeAreaPaddingView, obj3);
  ...
  const items3 = [closure_9(onAccept(7145), obj7), closure_9(View, { style: { height: tmp2[0] } })];
  obj6.children = items3;
  return closure_10(channel(7223).BottomSheet, obj6);
};
