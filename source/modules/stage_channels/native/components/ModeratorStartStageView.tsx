...
// Dependencies: [32, 19, 1979, 21, 504, 9732, 9740, 9741, 9743, 9744, 9735, 9745, 1114, 9767, 9968, 2]
// Exports: default

// Module 9731 (ModeratorStartStageView)
import useCurrentUserStageRolesDefault from "useCurrentUserStageRoles" /* 9740 */;
import useCanCreateAnEventDefault from "useCanCreateAnEvent" /* 9743 */;
import _slicedToArray from "module_32" /* 32 */;
import noop from "module_19" /* 19 */;
import GuildStore from "GuildStore" /* 1979 */;

const StageViewWithPromptsDefault = tmp5(9745);
const require = fn;
const jsxProd = fn(21);
...
export default function ModeratorStartStageView(channel) {
  ...
  const first = _slicedToArray(guild_id(9732).useGuildChannelScheduledEvents(channel.id), 1)[0];
  const obj2 = guild_id(9732);
  const obj3 = guild_id(9741);
  const canManageGuildEventResult = guild_id(9741).useManageResourcePermissions(channel).canManageGuildEvent(first);
  const tmp7 = useCanCreateAnEventDefault(guild_id);
  const isLive = guild_id(9744).useStageChannelStartEvent(channel.id).isLive;
  const obj4 = guild_id(9744);
  const nextRecurrenceIdInEvent = guild_id(9735).getNextRecurrenceIdInEvent(first);
  ...
  if (null != stateFromStores) {
    ...
    if (canManageGuildEventResult) {
      ...
      if (null != first) {
        ...
        tmp10Result = tmp10(tmp(9767).StartEventPrompt, obj7);
      }
    }
    ...
    if (useCurrentUserStageRolesDefault(channel.id, true).moderator) {
      ...
      tmp10Result4 = tmp10(tmp(9968).StartStagePrompt, obj8);
    }
    items2[1] = tmp10Result4;
    let tmp10Result5 = null;
    if (tmp7) {
      const obj9 = { channel, isLive, guild: stateFromStores };
      tmp10Result5 = tmp10(tmp(9767).ScheduleEventPrompt, obj9);
    }
    const obj10 = { children: null };
    items2[2] = tmp10Result5;
    const obj11 = { onContinue: channel.onSkip };
    items2[3] = closure_5(tmp(9968).ContinueToStagePrompt, obj11);
