...
// Function ID: 9858
// Name: AddMembersBody
// Dependencies: [109, 32, 19, 17, 2021, 2015, 1371, 8489, 1085, 21, 4607, 576, 4250, 7055, 504, 4594, 9831, 1178, 1114, 9844, 4318, 6699, 9848, 5571, 4603, 9853, 2]
// Exports: default

// Module 9857 (AddMembersBody)
...
const ChannelPermissionsConstants = fn(8489);
...
export default function AddMembersBody(pendingAdditions) {
  ...
  if (first) {
    ...
    if (tmp19) {
      items2 = tmp10(9831).getNoRolesRow();
      const tmp10Result3 = tmp10(9831);
    }
  }
  let obj5 = guild(504);
  ...
  if (inActionSheet) {
    let BottomSheetScrollView = tmp6(6699).BottomSheetScrollView;
  } else {
    BottomSheetScrollView = c8;
  }
  if (inActionSheet) {
    let BottomSheetSectionList = tmp6(6699).BottomSheetSectionList;
  } else {
  ...
  obj9.children = closure_16(pendingAdditions(9848), obj10);
  ...
  if (null != inputDesc) {
    ...
    obj11.children = tmp27(tmp6(4603).Text, obj12);
  ...
  if ("" !== str) {
    if (0 === items2.length) {
      if (0 === membersRows.length) {
        ...
        const obj16 = { Illustration: tmp6(9853).NoResultsAlt, style: null, bodyStyle: null, body: null };
  ...
  const tmp4Result = pendingAdditions(9848);
  obj19.contentContainerStyle = { paddingHorizontal: pendingAdditions(576).space.PX_16, paddingBottom: pendingAdditions(576).space.PX_16 + pendingAdditions(7055)(obj).insets.bottom };
