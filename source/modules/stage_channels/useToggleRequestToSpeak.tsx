...
// Dependencies: [32, 19, 502, 504, 4786, 5507, 8532, 8534, 8519, 2]
// Exports: default
  ...
  const items2 = [
    ...
    () => {
      if (obj.shouldAgeVerifyToSpeakForCurrentUser(id.id)) {
        const obj2 = { entryPoint: tmp(8534).AgeVerificationModalEntryPoint.STAGE_CHANNEL_RAISE_HAND };
        const result = AgeVerificationActionCreatorsDefault.showAgeVerificationGetStartedModal(obj2);
      } else {
        if (closure_1 === tmp(4786).RequestToSpeakStates.REQUESTED_TO_SPEAK_AND_AWAITING_USER_ACK) {
          const result1 = tmp(8519).audienceAckRequestToSpeak(tmp3, true);
          const tmpResult = tmp(8519);
        } else {
          tmp(8519).toggleRequestToSpeak(tmp3, !first);
          const tmpResult2 = tmp(8519);
        }
        closure_4(!first);
      }
    }
