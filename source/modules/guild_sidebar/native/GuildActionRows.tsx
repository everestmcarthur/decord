...
// Dependencies: [19, 17, 7646, 4655, 10267, 7211, 4822, 21, 4640, 576, 7449, 4460, 1944, 563, 7642, 4843, 11699, 1897, 12531, 12443, 1115, 13958, 2]
// Exports: GuildRolesAndChannelsRow

// Module 16368 (GuildActionRows)
...
let closure_7 = fn(7211).CHANNELS_AND_ROLES_MODAL_KEY;
const ReadStateTypes = fn(4822).ReadStateTypes;
const jsx = fn(21).jsx;
const createStyles = fn(4640);
let obj2 = { container: { marginVertical: fn(10267).CHANNEL_MARGIN_VERTICAL, marginHorizontal: 8, borderRadius: nativeDefault.radii.md }, channelInfoContainer: { paddingStart: 4 } };
...
export const GuildRolesAndChannelsRow = function GuildRolesAndChannelsRow(guild) {
  ...
  const callback = noop.useCallback(() => {
    ModalActionCreatorsDefault.pushLazy(asyncRequireImpl(11699, dependencyMap.paths), { guildId: id }, closure_7);
  }, items2);
  let SELECTED = guild(12531).ChannelModes.DEFAULT;
  if (selected) {
    SELECTED = tmp5(12531).ChannelModes.SELECTED;
  }
  let tmp10 = !result;
  if (result) {
  ...
  let tmp11 = null;
  if (tmp10) {
    const obj4 = { style: tmp.channelInfoContainer, children: jsx(tmp5(12443).NewBadge, {}) };
    tmp11 = <View style={tmp.channelInfoContainer}>{jsx(tmp5(12443).NewBadge, {})}</View>;
  }
  ...
  obj5.name = jsx(guild(12531).BaseChannelName, { name: string2Result, mode: SELECTED });
  const tmp2Result = tmp2(12531);
  obj5.icon = jsx(guild(12531).BaseChannelIcon, { mode: SELECTED, IconComponent: guild(13958).ChannelListMagnifyingGlassIcon });
