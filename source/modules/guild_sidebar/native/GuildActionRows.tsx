...
// Dependencies: [19, 17, 7584, 4620, 10188, 7149, 4787, 21, 4605, 576, 7387, 4425, 1943, 563, 7580, 4808, 11620, 1896, 12449, 12361, 1114, 13918, 2]
// Exports: GuildRolesAndChannelsRow

// Module 16290 (GuildActionRows)
...
let closure_7 = fn(7149).CHANNELS_AND_ROLES_MODAL_KEY;
const ReadStateTypes = fn(4787).ReadStateTypes;
const jsx = fn(21).jsx;
const createStyles = fn(4605);
let obj2 = { container: { marginVertical: fn(10188).CHANNEL_MARGIN_VERTICAL, marginHorizontal: 8, borderRadius: nativeDefault.radii.md }, channelInfoContainer: { paddingStart: 4 } };
...
export const GuildRolesAndChannelsRow = function GuildRolesAndChannelsRow(guild) {
  ...
  const callback = noop.useCallback(() => {
    ModalActionCreatorsDefault.pushLazy(asyncRequireImpl(11620, dependencyMap.paths), { guildId: id }, closure_7);
  }, items2);
  let SELECTED = guild(12449).ChannelModes.DEFAULT;
  if (selected) {
    SELECTED = tmp5(12449).ChannelModes.SELECTED;
  }
  let tmp10 = !result;
  if (result) {
  ...
  let tmp11 = null;
  if (tmp10) {
    const obj4 = { style: tmp.channelInfoContainer, children: jsx(tmp5(12361).NewBadge, {}) };
    tmp11 = <View style={tmp.channelInfoContainer}>{jsx(tmp5(12361).NewBadge, {})}</View>;
  }
  ...
  obj5.name = jsx(guild(12449).BaseChannelName, { name: string2Result, mode: SELECTED });
  const tmp2Result = tmp2(12449);
  obj5.icon = jsx(guild(12449).BaseChannelIcon, { mode: SELECTED, IconComponent: guild(13918).ChannelListMagnifyingGlassIcon });
