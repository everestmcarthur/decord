...
export const getSectionFooterConfig = function getSectionFooterConfig(guildChannels, optInChannelsEnabled, section) {
  ...
  let tmp6 = !tmp3;
  if (!tmp3) {
    const tmp7 = section === tmp(7638).SECTION_INDEX_GUILD_ACTIONS;
    if (tmp7) {
      tmp6 = tmp7;
    } else {
      if (tmpResult.isFavoritesGuildId(guildChannels.id)) {
        ...
        tmp8 = section === tmp(7638).SECTION_INDEX_FAVORITES;
        if (!tmp8) {
          let tmp10 = optInChannelsEnabled;
          if (tmp10) {
            let tmp11 = section !== tmp(7638).SECTION_INDEX_UNCATEGORIZED_CHANNELS;
            if (tmp11) {
