...
// Dependencies: [19, 17, 21, 4605, 576, 1115, 4423, 672, 1611, 4306, 5037, 5173, 4601, 2]

// Module 10712 (MediaKeyboardBottomSheetActions)
...
export default noop.memo(function MediaKeyboardBottomSheetActions(onHeightChange) {
  ...
  noop = undefined;
  let variant;
  const gradientValue = onHeightChange(4423).useGradientValue(onHeightChange(4423).GradientPercentage.END);
  let hexResult = null;
  ...
  let obj = onHeightChange(4423);
  const tmp6 = closure_8(useSafeAreaInsetsDefault().bottom, onHeightChange(4306).useToken(nativeDefault.colors.MOBILE_FLOATINGBAR_BACKGROUND_HIGHER), hexResult);
  importDefault = tmp6;
  const tmpResult = onHeightChange(4306);
  dependencyMap = onHeightChange(4306).useToken(nativeDefault.modules.mobile.MEDIA_KEYBOARD_BUTTON_ICON_COLOR_ACTIVE);
  const tmpResult4 = onHeightChange(4306);
  noop = onHeightChange(4306).useToken(nativeDefault.modules.mobile.MEDIA_KEYBOARD_BUTTON_TEXT_COLOR_ACTIVE);
  const tmpResult5 = onHeightChange(4306);
  variant = onHeightChange(4306).useToken(nativeDefault.modules.mobile.MEDIA_KEYBOARD_BUTTON_TEXT_VARIANT);
  ...
  const tmpResult6 = onHeightChange(4306);
