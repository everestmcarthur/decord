...
// Dependencies: [19, 17, 4255, 1074, 21, 4607, 5576, 576, 12689, 504, 1242, 1114, 7963, 4946, 1178, 2]
...
export default noop.memo(function MessageRequestPreview(channel) {
  ...
  const tmp = closure_9();
  const messageRequestPreview = channel(12689).useMessageRequestPreview(channel);
  const message = messageRequestPreview.message;
  ({ loaded, error } = messageRequestPreview);
  let obj = channel(12689);
  ...
  if (error) {
    ...
    if (loaded) {
      ...
      if (null != content) {
        if ("" !== message.content) {
          const content1 = message(7963)(message, { noStyleAndInteraction: true, allowGameMentions: true }).content;
          const _Array = Array;
