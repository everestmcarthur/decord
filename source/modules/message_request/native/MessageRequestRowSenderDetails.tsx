...
// Dependencies: [19, 17, 4253, 21, 4605, 1178, 576, 504, 4449, 16974, 1399, 4601, 1114, 16975, 16976, 2]
// Exports: default

// Module 16973 (MessageRequestRowSenderDetails)
...
export default function MessageRequestRowSenderDetails(isRestricted) {
  ...
  const messageRequestRelativeTimestampText = otherUser(16974).useMessageRequestRelativeTimestampText(channel);
  ...
  const items4 = [closure_6(otherUser(4601).Text, obj8), closure_5(otherUser(4601).Text, { style: tmp.timestampSeparator, variant: "text-xs/medium", color: "text-muted", children: "\u00B7" }), closure_5(otherUser(4601).Text, { variant: "text-xs/semibold", color: "text-muted", children: messageRequestRelativeTimestampText })];
  obj7.children = items4;
  const items5 = [closure_6(View, obj7), , ];
  let tmp11Result = !flag;
  if (!flag) {
    const obj12 = { style: tmp.messagePreview, channel };
    tmp11Result = tmp11(tmp7(16975), obj12);
  }
  ...
  if (flag) {
    ...
    flag = tmp11(tmp7(16976), obj13);
    const tmp7Result = tmp7(16976);
  }
