...
// Dependencies: [19, 17, 4222, 21, 4574, 1178, 576, 504, 4418, 16917, 1399, 4570, 1114, 16918, 16919, 2]
// Exports: default

// Module 16916 (MessageRequestRowSenderDetails)
...
export default function MessageRequestRowSenderDetails(isRestricted) {
  ...
  const messageRequestRelativeTimestampText = otherUser(16917).useMessageRequestRelativeTimestampText(channel);
  ...
  const items4 = [closure_6(otherUser(4570).Text, obj8), closure_5(otherUser(4570).Text, { style: tmp.timestampSeparator, variant: "text-xs/medium", color: "text-muted", children: "\u00B7" }), closure_5(otherUser(4570).Text, { variant: "text-xs/semibold", color: "text-muted", children: messageRequestRelativeTimestampText })];
  obj7.children = items4;
  const items5 = [closure_6(View, obj7), , ];
  let tmp11Result = !flag;
  if (!flag) {
    const obj12 = { style: tmp.messagePreview, channel };
    tmp11Result = tmp11(tmp7(16918), obj12);
  }
  ...
  if (flag) {
    ...
    flag = tmp11(tmp7(16919), obj13);
    const tmp7Result = tmp7(16919);
  }
