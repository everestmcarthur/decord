...
// Dependencies: [19, 17, 21, 4560, 576, 4296, 4561, 4763, 12289, 4556, 1114, 2]
// Exports: default

// Module 17374 (NewMemberActionsCompleted)
...
export default function NewMemberActionsCompleted(arg0) {
  ...
  const effect1 = React.useEffect(() => {
    const timerId = setTimeout(() => callback2(4763).popWithKey(callback(12289).NEW_MEMBER_ACTION_COMPLETE_MODAL_KEY), 2500);
  }, []);
  ...
  obj1[3] = intl.format(sharedValue(1114).t.pGj5u2, { count: numActions });
  const items1 = [callback(sharedValue(4556).Text, obj1), ];
  const items2 = [tmp.progressForeground, animatedStyle];
  items1[1] = callback(View, { style: tmp.progressBackground, children: callback(_modDef4296.View, { style: items2 }) });
  obj[1] = items1;
