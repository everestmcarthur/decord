...
// Dependencies: [19, 17, 1958, 2013, 4943, 4361, 1074, 21, 4722, 576, 504, 8102, 12251, 16829, 7652, 12527, 1397, 5292, 4718, 4712, 16822, 1101, 1115, 16830, 5144, 2]
// Exports: default

// Module 16828 (GuildHomeResources)
...
function ResourceChannelRow(channelId) {
  ...
  const forumPostMediaProperties = channelId(8102).useForumPostMediaProperties(firstResult, false);
  const obj5 = channelId(8102);
  let length;
  const firstMediaIsEmbed = channelId(8102).useFirstMediaIsEmbed(firstResult, false);
  ...
  const obj6 = channelId(8102);
  let flag = channelId(12251).useSharedMediaProps({ channel: stateFromStores, media: first }).shouldObscure;
  const tmp11 = stateFromStores(16829)(firstResult);
  ...
  if (null != stateFromStores) {
    if (stateFromStores1) {
      ...
      const items4 = [closure_12(tmp2(4718).Text, obj9), , ];
      ...
      if (tmp19Result) {
        ...
        obj10.children = tmp10(4712).parse(tmp11, true, { guildId: null, channelId: null });
        tmp19Result = tmp19(tmp2(4718).Text, obj10);
        const obj11 = { guildId: null, channelId: null };
        const tmp10Result3 = tmp10(4712);
      }
      ...
      if (!(null == description || 0 === description.length)) {
        ...
        obj12.children = tmp10(4712).parse(description, true, { guildId: null, channelId: null });
        tmp19Result4 = tmp19(tmp2(4718).Text, obj12);
        const obj13 = { guildId: null, channelId: null };
        const tmp10Result4 = tmp10(4712);
      }
        ...
        if (null != firstResult) {
          ...
          if (!blocked) {
            ...
            if (null != first) {
              ...
              obj17.embedLeftBorderColor = tmp2(8102).getEmbedColor(firstResult, flag);
              let id;
              if (firstResult != null) {
                id = firstResult.id;
              }
              ...
              tmp19Result6 = tmp19(tmp2(12251).ForumPostMediaThumbnail, obj17);
              const tmp2Result2 = tmp2(8102);
            }
          }
        }
      }
      ...
      return closure_13(tmp2(5292).PressableOpacity, obj7);
    }
  }
  return null;
}
...
export default function GuildHomeResources(guildId) {
  ...
  if (0 === arr.length) {
    ...
    const items = [closure_12(guildId(4718).Text, obj3), , ];
    const obj4 = { style: tmp.emptyStateImage, source: _modDef16830 };
    ...
    items[2] = closure_12(guildId(5144).Button, obj5);
    obj2.children = items;
