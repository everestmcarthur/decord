...
// Dependencies: [19, 17, 1958, 2013, 4859, 4276, 1074, 21, 4638, 576, 504, 8013, 12151, 16702, 7566, 12427, 1397, 5208, 4634, 4628, 16695, 1101, 1115, 16703, 5060, 2]
// Exports: default

// Module 16701 (GuildHomeResources)
...
function ResourceChannelRow(channelId) {
  ...
  const forumPostMediaProperties = channelId(8013).useForumPostMediaProperties(firstResult, false);
  const obj5 = channelId(8013);
  let length;
  const firstMediaIsEmbed = channelId(8013).useFirstMediaIsEmbed(firstResult, false);
  ...
  const obj6 = channelId(8013);
  let flag = channelId(12151).useSharedMediaProps({ channel: stateFromStores, media: first }).shouldObscure;
  const tmp11 = stateFromStores(16702)(firstResult);
  ...
  if (null != stateFromStores) {
    if (stateFromStores1) {
      ...
      const items4 = [closure_12(tmp2(4634).Text, obj9), , ];
      ...
      if (tmp19Result) {
        ...
        obj10.children = tmp10(4628).parse(tmp11, true, { guildId: null, channelId: null });
        tmp19Result = tmp19(tmp2(4634).Text, obj10);
        const obj11 = { guildId: null, channelId: null };
        const tmp10Result3 = tmp10(4628);
      }
      ...
      if (!(null == description || 0 === description.length)) {
        ...
        obj12.children = tmp10(4628).parse(description, true, { guildId: null, channelId: null });
        tmp19Result4 = tmp19(tmp2(4634).Text, obj12);
        const obj13 = { guildId: null, channelId: null };
        const tmp10Result4 = tmp10(4628);
      }
        ...
        if (null != firstResult) {
          ...
          if (!blocked) {
            ...
            if (null != first) {
              ...
              obj17.embedLeftBorderColor = tmp2(8013).getEmbedColor(firstResult, flag);
              let id;
              if (firstResult != null) {
                id = firstResult.id;
              }
              ...
              tmp19Result6 = tmp19(tmp2(12151).ForumPostMediaThumbnail, obj17);
              const tmp2Result2 = tmp2(8013);
            }
          }
        }
      }
      ...
      return closure_13(tmp2(5208).PressableOpacity, obj7);
    }
  }
  return null;
}
...
export default function GuildHomeResources(guildId) {
  ...
  if (0 === arr.length) {
    ...
    const items = [closure_12(guildId(4634).Text, obj3), , ];
    const obj4 = { style: tmp.emptyStateImage, source: _modDef16703 };
    ...
    items[2] = closure_12(guildId(5060).Button, obj5);
    obj2.children = items;
