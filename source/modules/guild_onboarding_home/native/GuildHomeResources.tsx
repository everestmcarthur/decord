...
// Dependencies: [19, 17, 1958, 2013, 4861, 4278, 1074, 21, 4640, 576, 504, 8019, 12160, 16732, 7570, 12436, 1397, 5210, 4636, 4630, 16725, 1101, 1115, 16733, 5062, 2]
// Exports: default

// Module 16731 (GuildHomeResources)
...
function ResourceChannelRow(channelId) {
  ...
  const forumPostMediaProperties = channelId(8019).useForumPostMediaProperties(firstResult, false);
  const obj5 = channelId(8019);
  let length;
  const firstMediaIsEmbed = channelId(8019).useFirstMediaIsEmbed(firstResult, false);
  ...
  const obj6 = channelId(8019);
  let flag = channelId(12160).useSharedMediaProps({ channel: stateFromStores, media: first }).shouldObscure;
  const tmp11 = stateFromStores(16732)(firstResult);
  ...
  if (null != stateFromStores) {
    if (stateFromStores1) {
      ...
      const items4 = [closure_12(tmp2(4636).Text, obj9), , ];
      ...
      if (tmp19Result) {
        ...
        obj10.children = tmp10(4630).parse(tmp11, true, { guildId: null, channelId: null });
        tmp19Result = tmp19(tmp2(4636).Text, obj10);
        const obj11 = { guildId: null, channelId: null };
        const tmp10Result3 = tmp10(4630);
      }
      ...
      if (!(null == description || 0 === description.length)) {
        ...
        obj12.children = tmp10(4630).parse(description, true, { guildId: null, channelId: null });
        tmp19Result4 = tmp19(tmp2(4636).Text, obj12);
        const obj13 = { guildId: null, channelId: null };
        const tmp10Result4 = tmp10(4630);
      }
        ...
        if (null != firstResult) {
          ...
          if (!blocked) {
            ...
            if (null != first) {
              ...
              obj17.embedLeftBorderColor = tmp2(8019).getEmbedColor(firstResult, flag);
              let id;
              if (firstResult != null) {
                id = firstResult.id;
              }
              ...
              tmp19Result6 = tmp19(tmp2(12160).ForumPostMediaThumbnail, obj17);
              const tmp2Result2 = tmp2(8019);
            }
          }
        }
      }
      ...
      return closure_13(tmp2(5210).PressableOpacity, obj7);
    }
  }
  return null;
}
...
export default function GuildHomeResources(guildId) {
  ...
  if (0 === arr.length) {
    ...
    const items = [closure_12(guildId(4636).Text, obj3), , ];
    const obj4 = { style: tmp.emptyStateImage, source: _modDef16733 };
    ...
    items[2] = closure_12(guildId(5062).Button, obj5);
    obj2.children = items;
