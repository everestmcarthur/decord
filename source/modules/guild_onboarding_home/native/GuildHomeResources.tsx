...
// Dependencies: [19, 17, 1957, 2012, 4827, 4245, 1074, 21, 4606, 576, 504, 7971, 12102, 16643, 7529, 12377, 1396, 5174, 4602, 4596, 16636, 1100, 1114, 16644, 5026, 2]
// Exports: default

// Module 16642 (GuildHomeResources)
...
function ResourceChannelRow(channelId) {
  ...
  const forumPostMediaProperties = channelId(7971).useForumPostMediaProperties(firstResult, false);
  const obj5 = channelId(7971);
  let length;
  const firstMediaIsEmbed = channelId(7971).useFirstMediaIsEmbed(firstResult, false);
  ...
  const obj6 = channelId(7971);
  let flag = channelId(12102).useSharedMediaProps({ channel: stateFromStores, media: first }).shouldObscure;
  const tmp11 = stateFromStores(16643)(firstResult);
  ...
  if (null != stateFromStores) {
    if (stateFromStores1) {
      ...
      const resourceChannelIconURL = tmp10(1396).getResourceChannelIconURL(obj4);
      ...
      const items4 = [closure_12(tmp2(4602).Text, obj9), , ];
      ...
      if (tmp19Result) {
        ...
        obj10.children = tmp10(4596).parse(tmp11, true, { guildId: null, channelId: null });
        tmp19Result = tmp19(tmp2(4602).Text, obj10);
        const obj11 = { guildId: null, channelId: null };
        const tmp10Result3 = tmp10(4596);
      }
      ...
      if (!(null == description || 0 === description.length)) {
        ...
        obj12.children = tmp10(4596).parse(description, true, { guildId: null, channelId: null });
        tmp19Result4 = tmp19(tmp2(4602).Text, obj12);
        const obj13 = { guildId: null, channelId: null };
        const tmp10Result4 = tmp10(4596);
      }
        ...
        if (null != firstResult) {
          ...
          if (!blocked) {
            ...
            if (null != first) {
              ...
              obj17.embedLeftBorderColor = tmp2(7971).getEmbedColor(firstResult, flag);
              let id;
              if (firstResult != null) {
                id = firstResult.id;
              }
              ...
              tmp19Result6 = tmp19(tmp2(12102).ForumPostMediaThumbnail, obj17);
              const tmp2Result2 = tmp2(7971);
            }
          }
        }
      }
      ...
      return closure_13(tmp2(5174).PressableOpacity, obj7);
    }
  }
  return null;
}
...
export default function GuildHomeResources(guildId) {
  ...
  if (0 === arr.length) {
    ...
    const items = [closure_12(guildId(4602).Text, obj3), , ];
    const obj4 = { style: tmp.emptyStateImage, source: _modDef16644 };
    ...
    obj5.text = intl2.string(guildId(1114).t["3iCBUn"]);
    items[2] = closure_12(guildId(5026).Button, obj5);
    obj2.children = items;
