...
// Dependencies: [2015, 1958, 1980, 4826, 1074, 1965, 7330, 563, 1983, 7213, 4828, 2]
// Exports: canSeeOnboardingHome, useCanSeeOnboardingHome

// Module 7329 (OnboardingHomeUtils)
    ...
    if (closure_0 !== React6) {
      if (!obj3.isFavoritesGuildId(tmp2)) {
        ...
        if (!hasItem) {
          ...
          result = tmp23(7213).isGuildOnboardingSettingsAvailable(tmp2);
          if (!result) {
    ...
    if (id !== React6) {
      if (!obj3.isFavoritesGuildId(id)) {
        if (tmp2) {
          ...
          let result = tmp14(7213).isGuildOnboardingSettingsAvailable(id);
          if (result) {
