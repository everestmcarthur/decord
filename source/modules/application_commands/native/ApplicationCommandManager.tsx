...
// Dependencies: [32, 4981, 7900, 1074, 5087, 1608, 12143, 12142, 7898, 7635, 9570, 7637, 9566, 1895, 9461, 4508, 1610, 12306, 12, 12179, 12145, 4627, 12308, 12144, 4608, 4820, 10409, 2]

// Module 12558 (ApplicationCommandManager)
...
import ApplicationCommandActionCreators from "ApplicationCommandActionCreators" /* 7898 */;
...
import autocompleter_AutocompleteUtils from "autocompleter/AutocompleteUtils" /* 10409 */;
import ChatInputCommandOptionParser from "ChatInputCommandOptionParser" /* 12142 */;
import ChatInputParser from "ChatInputParser" /* 12143 */;
import ApplicationCommandOptionValueParser from "ApplicationCommandOptionValueParser" /* 12144 */;
import application_commands_ApplicationCommandValidationUtils from "application_commands/ApplicationCommandValidationUtils" /* 12308 */;
  ...
  constructor(arg0) {
    ...
    obj.setPastedCommand = function setPastedCommand(arg0, channel) {
      ...
      if (null != command) {
        let tmp8 = null;
        if (null != application) {
          const obj5 = { type: tmp2(7637).ApplicationCommandSectionType.APPLICATION, id: null, icon: null, name: null, application: null };
        ...
        const initialValuesFromInteractionOptions = tmp2(9566).getInitialValuesFromInteractionOptions(command, interactionOptions);
        ...
        const obj6 = { channelId: channel.id, command, section: tmp8, location: tmp2(7637).ApplicationCommandTriggerLocations.PASTE, commandText: null };
        ...
        obj.setCommand(obj6);
        const tmp2Result = tmp2(9566);
      } else {
        ({ id, name } = parsed);
        obj.setPartialCommand(id, name, tmp2(7637).ApplicationCommandTriggerLocations.PASTE);
      }
    };
    ...
    obj.updateStyles = function updateStyles(styles) {
      ...
      obj.chatInputNodes = chatInputNodes.map((style) => {
        if (null == style.style) {
          ...
          if (obj(12143).ChatInputNodeType.COMMAND_OPTION !== type) {
            if (tmp30(12143).ChatInputNodeType.COMMAND_OPTION_WITH_VALUE !== type) {
              if (tmp30(12143).ChatInputNodeType.GAME_HIGHLIGHT === type) {
                const obj2 = {};
                const merged = Object.assign(style);
                const styles4 = closure_1_0.styles;
                obj2.style = styles4.gameMention();
                return obj2;
              } else if (tmp30(12143).ChatInputNodeType.GAME_MENTION_INPUT === type) {
                const obj3 = {};
                const merged1 = Object.assign(style);
                const styles3 = closure_1_0.styles;
                obj3.style = styles3.commandOption();
                return obj3;
              } else if (tmp30(12143).ChatInputNodeType.ROLE_HIGHLIGHT === type) {
                ...
                if (tmp30(12143).ChatInputNodeType.EMOJI_HIGHLIGHT !== type) {
                  if (tmp30(12143).ChatInputNodeType.USER_HIGHLIGHT !== type) {
                    if (tmp30(12143).ChatInputNodeType.CHANNEL_HIGHLIGHT !== type) {
                      if (tmp30(12143).ChatInputNodeType.SILENT_HIGHLIGHT !== type) {
                        return style;
                      }
                    }
                  }
    ...
    obj.addCommandOptionParserRules = function addCommandOptionParserRules() {
      ...
      obj = {
        ...
        matchFunction(c22, activeCommand) {
          return obj(12142).getMatchedOptions(c22, activeCommand);
        },
        style() {
      ...
      parser2.addRule({
        ...
        matchFunction(arg0, arg1) {
          return obj(12142).getMatchedOptionsWithValue(arg0, arg1);
        },
        style() {
          const styles = obj.styles;
          return styles.commandOption();
        ...
        editDisabled(data) {
          ...
          return type === obj(1895).ApplicationCommandOptionType.ATTACHMENT;
        }
      });
      ...
      parser7.addRule({
        ...
        matchFunction(arg0) {
          return obj(12142).getSilentHighlightNodes(arg0);
        },
        style() {
          const styles = obj.styles;
          return styles.autocomplete();
      ...
      parser9.addRule({
        ...
        matchFunction(arr) {
          return obj(12142).getGameMentionInputNodes(arr);
        },
        style() {
          const styles = obj.styles;
          return styles.commandOption();
      ...
      parser11.addRule({
        ...
        matchFunction(arr) {
          return obj(12142).getTimestampMentionInputNodes(arr);
        },
        style() {
          const styles = obj.styles;
          return styles.commandOption();
        ...
        if (!obj.props.commandsDisabled) {
          if (text.startsWith(closure_2_11)) {
            ...
            if (null == tmp.contextCommands) {
              ...
              const commandContext = tmp3(12179).getCommandContext(obj3);
              ...
              if (preferredCommandType === constants.PARTIAL_COMMAND) {
                ...
                if (null != found) {
                  ...
                  let cachedApplicationSection = ApplicationCommandQueryApiAll.getCachedApplicationSection(obj4, tmp3(1895).ApplicationCommandType.CHAT, found.applicationId);
                  let tmp20 = null;
                ...
                const draftCommand = tmp3(12145).resolveDraftCommand(channel, text, DraftStore.getDraftCommand(channel.id, DraftType.ChannelMessage));
                  ...
                  if (0 < parts.length) {
                    function _loop() {
                      ...
                      if (found.length > 0) {
                        ...
                        const cachedApplicationSection = ApplicationCommandQueryApiAll.getCachedApplicationSection(obj, tmp3(1895).ApplicationCommandType.CHAT, first.applicationId);
                        if (null == cachedApplicationSection) {
                          let obj5 = { v: null };
                        } else {
                          obj5 = { v: null };
...
prototype["mergePropsAndUpdate"] = function mergePropsAndUpdate(editId) {
  ...
  if (text === editId.text) {
    ...
    if (tmp84) {
      ...
      if (!focused2) {
        const keyboardType = obj10(4508).getKeyboardType();
        focused2 = keyboardType !== obj10(1610).KeyboardTypes.SYSTEM;
        const obj6 = obj10(4508);
      }
      ...
      self.chatInputNodes = chatInputNodes.map((type) => {
        if (type.type === ChatInputParser.ChatInputNodeType.COMMAND_OPTION) {
          if (null != type.data) {
            ...
            if (type.type === tmp(12143).ChatInputNodeType.COMMAND_OPTION_WITH_VALUE) {
              let name;
            ...
            if (option.name !== name1) {
              if (undefined !== tmp15) {
                ...
                if (success) {
                  success = option.type === tmp(1895).ApplicationCommandOptionType.ATTACHMENT;
                }
                if (success) {
    ...
    if (name1 !== name3) {
      ...
      if (type === tmp113(1895).ApplicationCommandOptionType.ATTACHMENT) {
    ...
    if (tmp140) {
      tmp140 = activeOption.type !== tmp113(1895).ApplicationCommandOptionType.ATTACHMENT;
    }
    if (tmp140) {
      const keyboardType1 = tmp113(4508).getKeyboardType();
      tmp140 = keyboardType1 !== tmp113(1610).KeyboardTypes.SYSTEM;
      const tmp113Result2 = tmp113(4508);
    }
    if (tmp140) {
  ...
  if (editId.commandsDisabled) {
    ...
    const item1 = chatInputNodes1.forEach((type) => {
      if (type.type === ChatInputParser.ChatInputNodeType.COMMAND_OPTION) {
        ...
        if (type === tmp(12143).ChatInputParseResultDataType.COMMAND_OPTION) {
          const optionsToNodes = self.optionsToNodes;
          const result = optionsToNodes.set(type.data.option.name, type);
        }
      }
      let tmp5 = type.type === tmp(12143).ChatInputNodeType.COMMAND_OPTION_WITH_VALUE;
      if (tmp5) {
        ...
        tmp5 = type1 === tmp(12143).ChatInputParseResultDataType.COMMAND_OPTION;
      }
      if (tmp5) {
    ...
    if (tmp5) {
      ...
      if (preferredCommandType11 === constants3.FULL_COMMAND) {
        ...
        while (iter !== undefined) {
          ...
          if (!tmp60) {
            delete tmp[tmp2];
            if (tmp56.type === obj10(1895).ApplicationCommandOptionType.ATTACHMENT) {
