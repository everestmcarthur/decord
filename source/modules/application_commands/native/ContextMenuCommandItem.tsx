...
// Dependencies: [19, 17, 21, 4639, 576, 5690, 12, 1115, 12365, 5672, 1895, 4584, 2]
// Exports: ContextMenuCommandAppItem, ContextMenuCommandEmptyItem, ContextMenuCommandLoadingItem, default

// Module 17022 (ContextMenuCommandItem)
...
export default function ContextMenuCommandItem(item) {
  ...
  const tmp = closure_6();
  const tmp5 = item;
  const applicationCommandsIconSource = item(12365).getApplicationCommandsIconSource(section);
  ...
  if (tmp8Result) {
    ...
    tmp8Result = tmp8(section(5672), obj3);
  }
  obj2.icon = tmp8Result;
  obj2.trailing = jsx(tmp5(4584).SendMessageIcon, {});
  obj2.start = start;
  obj2.end = end;
  return jsx(item(5690).TableRow, { accessibilityLabel: memo, onPress, label: item.displayName, icon: null, trailing: null, start: null, end: null });
};
