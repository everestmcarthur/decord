...
// Dependencies: [19, 17, 21, 4722, 576, 5775, 12, 1115, 12473, 5757, 1895, 4666, 2]
// Exports: ContextMenuCommandAppItem, ContextMenuCommandEmptyItem, ContextMenuCommandLoadingItem, default

// Module 17250 (ContextMenuCommandItem)
...
export default function ContextMenuCommandItem(item) {
  ...
  const tmp = closure_6();
  const tmp5 = item;
  const applicationCommandsIconSource = item(12473).getApplicationCommandsIconSource(section);
  ...
  if (tmp8Result) {
    ...
    tmp8Result = tmp8(section(5757), obj3);
  }
  obj2.icon = tmp8Result;
  obj2.trailing = jsx(tmp5(4666).SendMessageIcon, {});
  obj2.start = start;
  obj2.end = end;
  return jsx(item(5775).TableRow, { accessibilityLabel: memo, onPress, label: item.displayName, icon: null, trailing: null, start: null, end: null });
};
