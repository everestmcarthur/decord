...
// Dependencies: [19, 17, 21, 4638, 576, 5691, 12, 1115, 12373, 5673, 1895, 4582, 2]
// Exports: ContextMenuCommandAppItem, ContextMenuCommandEmptyItem, ContextMenuCommandLoadingItem, default

// Module 17079 (ContextMenuCommandItem)
...
export default function ContextMenuCommandItem(item) {
  ...
  const tmp = closure_6();
  const tmp5 = item;
  const applicationCommandsIconSource = item(12373).getApplicationCommandsIconSource(section);
  ...
  if (tmp8Result) {
    ...
    tmp8Result = tmp8(section(5673), obj3);
  }
  obj2.icon = tmp8Result;
  obj2.trailing = jsx(tmp5(4582).SendMessageIcon, {});
  obj2.start = start;
  obj2.end = end;
  return jsx(item(5691).TableRow, { accessibilityLabel: memo, onPress, label: item.displayName, icon: null, trailing: null, start: null, end: null });
};
