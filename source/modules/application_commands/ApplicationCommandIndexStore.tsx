...
// Dependencies: [32, 5, 19, 9383, 2025, 502, 1957, 2021, 1979, 1371, 9384, 5051, 1074, 3, 9385, 9386, 7595, 7596, 5572, 38, 504, 9387, 1242, 1894, 9390, 573, 9391, 9392, 1940, 9498, 7597, 5569, 2]
...
// Module 9382 (ApplicationCommandIndexStore)
...
import ApplicationCommandIndexActionCreators from "ApplicationCommandIndexActionCreators" /* 9386 */;
import CommandPermissionContext from "CommandPermissionContext" /* 9387 */;
import ApplicationCommandQueryTypes from "ApplicationCommandQueryTypes" /* 9390 */;
import ApplicationCommandBuiltIns from "ApplicationCommandBuiltIns" /* 9392 */;
import CommandPermissionUtils from "CommandPermissionUtils" /* 9498 */;
...
function handleFetchSuccess(arg0, flag) {
  ...
  if (null == id) {
    ...
    const applicationCommands = id(7595).buildApplicationCommands(application_commands.map((description_default) => {
      const obj = {};
...
function queryIndex(allowApplicationCommands) {
  ...
  if (builtIns === undefined) {
    builtIns = NONE(9390).BuiltInCommandFilter.ALLOW;
  }
  ...
  if (NONE === undefined) {
    NONE = NONE(9390).ScoreMethod.NONE;
  }
  ...
  if (builtIns !== NONE(9390).BuiltInCommandFilter.DENY) {
    const tmp12 = builtIns === NONE(9390).BuiltInCommandFilter.ONLY_TEXT;
    let builtInCommands = tmp9(9392).getBuiltInCommands(permissionContext.commandTypes, true, tmp12);
    const tmp9Result = tmp9(9392);
  } else {
    builtInCommands = [];
  }
  const items = [];
  ...
  if (builtInCommands.length > 0) {
    const tmp87 = queryIndexSection(NONE(9392).BUILT_IN_SECTIONS[constants.BUILT_IN], builtInCommands, true, true, obj);
    if (null != tmp87) {
      items.push(tmp87);
    }
  }
  ...
  if (NONE === NONE(9390).ScoreMethod.COMMAND_ONLY) {
...
function queryIndexSection(descriptor, builtInCommands, isGuildInstalled, arg3, arg4) {
  ...
  if (0 !== arr2.length) {
    ...
    let tmp36 = obj5;
    tmp39 = scoreMethod !== ApplicationCommandQueryTypes.ScoreMethod.NONE && scoreMethod !== tmp27(9390).ScoreMethod.APPLICATION_ONLY;
  } else {
    tmp36 = null;
  }
  return tmp36;
...
const ApplicationCommandConstants = fn(5051);
  ...
  if (null == UserStore.getCurrentUser()) {
    ...
    if (applicationId.allowFetch) {
      ...
      if (!tmp14) {
        ...
        if (tmp36) {
          const applicationCommandIndex = tmp8(9386).requestApplicationCommandIndex({ type: "user" });
          flag5 = true;
          const tmp8Result = tmp8(9386);
        }
        ...
        if (tmp42) {
          ...
          const applicationCommandIndex1 = tmp8(9386).requestApplicationCommandIndex(obj4);
          flag5 = true;
          const tmp8Result4 = tmp8(9386);
        }
        flag3 = flag5;
      } else {
        ...
        if (tmp29) {
          if (null == guild_id.guild_id) {
            ...
            const applicationCommandIndex2 = tmp8(9386).requestApplicationCommandIndex(obj6);
            flag6 = true;
            const tmp8Result5 = tmp8(9386);
          }
        }
        const obj7 = { type: "guild", guildId: null };
        guild_id = guild_id.guild_id;
        obj7.guildId = guild_id;
        const applicationCommandIndex3 = tmp8(9386).requestApplicationCommandIndex(obj7);
        flag6 = true;
        const tmp8Result6 = tmp8(9386);
      }
    }
