...
// Dependencies: [32, 5, 19, 9434, 2026, 502, 1958, 2022, 1980, 1372, 9435, 5084, 1074, 3, 9436, 9437, 7631, 7632, 5606, 38, 504, 9438, 1241, 1895, 9441, 573, 9442, 9443, 1941, 9548, 7633, 5603, 2]
...
// Module 9433 (ApplicationCommandIndexStore)
...
import ApplicationCommandIndexActionCreators from "ApplicationCommandIndexActionCreators" /* 9437 */;
import CommandPermissionContext from "CommandPermissionContext" /* 9438 */;
import ApplicationCommandQueryTypes from "ApplicationCommandQueryTypes" /* 9441 */;
import ApplicationCommandBuiltIns from "ApplicationCommandBuiltIns" /* 9443 */;
import CommandPermissionUtils from "CommandPermissionUtils" /* 9548 */;
...
function handleFetchSuccess(arg0, flag) {
  ...
  if (null == id) {
    ...
    const applicationCommands = id(7631).buildApplicationCommands(application_commands.map((description_default) => {
      const obj = {};
...
function queryIndex(allowApplicationCommands) {
  ...
  if (builtIns === undefined) {
    builtIns = NONE(9441).BuiltInCommandFilter.ALLOW;
  }
  ...
  if (NONE === undefined) {
    NONE = NONE(9441).ScoreMethod.NONE;
  }
  ...
  if (builtIns !== NONE(9441).BuiltInCommandFilter.DENY) {
    const tmp12 = builtIns === NONE(9441).BuiltInCommandFilter.ONLY_TEXT;
    let builtInCommands = tmp9(9443).getBuiltInCommands(permissionContext.commandTypes, true, tmp12);
    const tmp9Result = tmp9(9443);
  } else {
    builtInCommands = [];
  }
  const items = [];
  ...
  if (sortOptions.applications.useFrecency) {
    const FrecencyUserSettingsActionCreators = NONE(1941).FrecencyUserSettingsActionCreators;
  ...
  if (builtInCommands.length > 0) {
    const tmp87 = queryIndexSection(NONE(9443).BUILT_IN_SECTIONS[constants.BUILT_IN], builtInCommands, true, true, obj);
    if (null != tmp87) {
      items.push(tmp87);
    }
  }
  ...
  if (NONE === NONE(9441).ScoreMethod.COMMAND_ONLY) {
    ...
    if (tmp96) {
      const FrecencyUserSettingsActionCreators2 = tmp89(1941).FrecencyUserSettingsActionCreators;
...
function queryIndexSection(descriptor, builtInCommands, isGuildInstalled, arg3, arg4) {
  ...
  if (0 !== arr2.length) {
    ...
    let tmp36 = obj5;
    tmp39 = scoreMethod !== ApplicationCommandQueryTypes.ScoreMethod.NONE && scoreMethod !== tmp27(9441).ScoreMethod.APPLICATION_ONLY;
  } else {
    tmp36 = null;
  }
  return tmp36;
  ...
  if (null == UserStore.getCurrentUser()) {
    ...
    if (applicationId.allowFetch) {
      ...
      if (!tmp14) {
        ...
        if (tmp36) {
          const applicationCommandIndex = tmp8(9437).requestApplicationCommandIndex({ type: "user" });
          flag5 = true;
          const tmp8Result = tmp8(9437);
        }
        ...
        if (tmp42) {
          ...
          const applicationCommandIndex1 = tmp8(9437).requestApplicationCommandIndex(obj4);
          flag5 = true;
          const tmp8Result4 = tmp8(9437);
        }
        flag3 = flag5;
      } else {
        ...
        if (tmp29) {
          if (null == guild_id.guild_id) {
            ...
            const applicationCommandIndex2 = tmp8(9437).requestApplicationCommandIndex(obj6);
            flag6 = true;
            const tmp8Result5 = tmp8(9437);
          }
        }
        const obj7 = { type: "guild", guildId: null };
        guild_id = guild_id.guild_id;
        obj7.guildId = guild_id;
        const applicationCommandIndex3 = tmp8(9437).requestApplicationCommandIndex(obj7);
        flag6 = true;
        const tmp8Result6 = tmp8(9437);
      }
    }
