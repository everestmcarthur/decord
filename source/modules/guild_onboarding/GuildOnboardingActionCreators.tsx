...
// Dependencies: [5, 32, 2015, 1958, 2022, 1372, 7210, 1074, 4263, 4820, 1271, 573, 1231, 5638, 12, 7216, 1370, 7219, 1241, 4818, 7220, 11, 1385, 7223, 7230, 2]

// Module 7215 (GuildOnboardingActionCreators)
import DispatcherDefault from "Dispatcher" /* 573 */;
import SentryUtilsDefault from "SentryUtils" /* 1231 */;
import AnalyticsUtilsDefault from "AnalyticsUtils" /* 1241 */;
import ImpersonateActionCreators from "ImpersonateActionCreators" /* 5638 */;
...
obj.completeOnboarding = function completeOnboarding(guildId, prompts) {
  ...
  const selectedRoleIds = items1(7216).getSelectedRoleIds(selectedOptions);
  const obj2 = items1(7216);
  const selectedChannelIds = items1(7216).getSelectedChannelIds(selectedOptions);
  if (GuildOnboardingPromptsStore.getEnabled(guildId)) {
    let defaultChannelIds = obj.getDefaultChannelIds(guildId);
  } else {
    defaultChannelIds = [];
  }
  const obj3 = items1(7216);
  const tmp2Result = items1(7216);
  [arr3, arr4] = items1(7216).getChannelCoverageForOnboarding(guildId, prompts, defaultChannelIds);
  ...
  const tmp6 = _slicedToArray(items1(7216).getChannelCoverageForOnboarding(guildId, prompts, defaultChannelIds), 2);
  const tmp2Result11 = items1(7219);
  if (null == tmp) {
    items1 = [];
  } else {
  ...
  const providerConnectionState = items1(7216).getProviderConnectionState(connections);
  const tmp2Result12 = items1(7216);
  const applicationConnectionState = items1(7216).getApplicationConnectionState(connections);
  const tmp2Result13 = items1(7216);
  const obj4 = {};
  const obj8 = AnalyticsUtilsDefault;
  const merged = Object.assign(items1(4818).collectGuildAnalyticsMetadata(guildId));
  ...
  const tmp2Result14 = items1(4818);
  const tmp2Result15 = items1(7220);
  ...
  if (ImpersonateStore.isFullServerPreview(guildId)) {
    const result = tmp2(5638).updateImpersonatedChannels(guildId, items, []);
    const tmp2Result16 = tmp2(5638);
    const result1 = tmp2(5638).updateImpersonatedData(guildId, { optInEnabled: true });
    const tmp2Result17 = tmp2(5638);
    const _Array = Array;
    const result2 = tmp2(5638).updateImpersonatedRoles(guildId, Array.from(selectedRoleIds));
