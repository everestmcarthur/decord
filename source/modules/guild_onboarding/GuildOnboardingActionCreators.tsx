...
// Dependencies: [5, 32, 2015, 1958, 2022, 1372, 7207, 1074, 4265, 4821, 1271, 573, 1231, 5637, 12, 7213, 1370, 7216, 1241, 4819, 7217, 11, 1385, 7220, 7227, 2]

// Module 7212 (GuildOnboardingActionCreators)
import DispatcherDefault from "Dispatcher" /* 573 */;
import SentryUtilsDefault from "SentryUtils" /* 1231 */;
import AnalyticsUtilsDefault from "AnalyticsUtils" /* 1241 */;
import ImpersonateActionCreators from "ImpersonateActionCreators" /* 5637 */;
...
obj.completeOnboarding = function completeOnboarding(guildId, prompts) {
  ...
  const selectedRoleIds = items1(7213).getSelectedRoleIds(selectedOptions);
  const obj2 = items1(7213);
  const selectedChannelIds = items1(7213).getSelectedChannelIds(selectedOptions);
  if (GuildOnboardingPromptsStore.getEnabled(guildId)) {
    let defaultChannelIds = obj.getDefaultChannelIds(guildId);
  } else {
    defaultChannelIds = [];
  }
  const obj3 = items1(7213);
  const tmp2Result = items1(7213);
  [arr3, arr4] = items1(7213).getChannelCoverageForOnboarding(guildId, prompts, defaultChannelIds);
  ...
  const tmp6 = _slicedToArray(items1(7213).getChannelCoverageForOnboarding(guildId, prompts, defaultChannelIds), 2);
  const tmp2Result11 = items1(7216);
  if (null == tmp) {
    items1 = [];
  } else {
  ...
  const providerConnectionState = items1(7213).getProviderConnectionState(connections);
  const tmp2Result12 = items1(7213);
  const applicationConnectionState = items1(7213).getApplicationConnectionState(connections);
  const tmp2Result13 = items1(7213);
  const obj4 = {};
  const obj8 = AnalyticsUtilsDefault;
  const merged = Object.assign(items1(4819).collectGuildAnalyticsMetadata(guildId));
  ...
  const tmp2Result14 = items1(4819);
  const tmp2Result15 = items1(7217);
  ...
  if (ImpersonateStore.isFullServerPreview(guildId)) {
    const result = tmp2(5637).updateImpersonatedChannels(guildId, items, []);
    const tmp2Result16 = tmp2(5637);
    const result1 = tmp2(5637).updateImpersonatedData(guildId, { optInEnabled: true });
    const tmp2Result17 = tmp2(5637);
    const _Array = Array;
    const result2 = tmp2(5637).updateImpersonatedRoles(guildId, Array.from(selectedRoleIds));
