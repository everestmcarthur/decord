...
// Dependencies: [5, 32, 2014, 1957, 2021, 1371, 7114, 1074, 4200, 4756, 1272, 573, 1232, 5566, 12, 7120, 1369, 7123, 1242, 4754, 7124, 11, 1384, 7127, 7134, 2]

// Module 7119 (GuildOnboardingActionCreators)
...
obj.completeOnboarding = function completeOnboarding(guildId, prompts) {
  ...
  const selectedRoleIds = items1(7120).getSelectedRoleIds(selectedOptions);
  const obj2 = items1(7120);
  const selectedChannelIds = items1(7120).getSelectedChannelIds(selectedOptions);
  if (GuildOnboardingPromptsStore.getEnabled(guildId)) {
    let defaultChannelIds = obj.getDefaultChannelIds(guildId);
  } else {
    defaultChannelIds = [];
  }
  const obj3 = items1(7120);
  const tmp2Result = items1(7120);
  [arr3, arr4] = items1(7120).getChannelCoverageForOnboarding(guildId, prompts, defaultChannelIds);
  ...
  const tmp6 = _slicedToArray(items1(7120).getChannelCoverageForOnboarding(guildId, prompts, defaultChannelIds), 2);
  const tmp2Result11 = items1(7123);
  if (null == tmp) {
    items1 = [];
  } else {
  ...
  const providerConnectionState = items1(7120).getProviderConnectionState(connections);
  const tmp2Result12 = items1(7120);
  const applicationConnectionState = items1(7120).getApplicationConnectionState(connections);
  const tmp2Result13 = items1(7120);
  const obj4 = {};
  const obj8 = AnalyticsUtilsDefault;
  const merged = Object.assign(items1(4754).collectGuildAnalyticsMetadata(guildId));
  ...
  const tmp2Result14 = items1(4754);
  const tmp2Result15 = items1(7124);
  ...
  if (ImpersonateStore.isFullServerPreview(guildId)) {
    const result = tmp2(5566).updateImpersonatedChannels(guildId, items, []);
    const tmp2Result16 = tmp2(5566);
    const result1 = tmp2(5566).updateImpersonatedData(guildId, { optInEnabled: true });
    const tmp2Result17 = tmp2(5566);
    const _Array = Array;
    const result2 = tmp2(5566).updateImpersonatedRoles(guildId, Array.from(selectedRoleIds));
