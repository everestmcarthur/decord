...
// Dependencies: [19, 17, 5545, 1980, 4653, 4459, 7210, 7211, 1074, 1375, 4820, 21, 4638, 576, 5610, 4346, 4572, 504, 11693, 11694, 1177, 1115, 4634, 7216, 7235, 7270, 7292, 1612, 11695, 7209, 7220, 7215, 7240, 1397, 1370, 4339, 4374, 4639, 11696, 4605, 7245, 1897, 5208, 7268, 7236, 2]
// Exports: default

// Module 11692 (CustomizeCommunity)
...
import GuildOnboardingPromptsActionCreators from "GuildOnboardingPromptsActionCreators" /* 7209 */;
import GuildOnboardingUtils from "GuildOnboardingUtils" /* 7216 */;
import EmojiDefault from "Emoji" /* 7240 */;
import ConnectionCardDefault from "ConnectionCard" /* 7270 */;
...
function EmptyCustomizeCommunity(setTab) {
  ...
  const obj = setTab(4346);
  const tmp4 = importDefault;
  const isThemeDarkResult = setTab(4346).isThemeDark(useThemeDefault());
  ...
  const items1 = [closure_16(closure_5, { style: tmp.emptyContainerImage, source: tmp4(isThemeDarkResult ? 11693 : 11694) }), , ];
  ...
  items1[2] = closure_16(setTab(4634).Text, obj6);
...
function DropdownOption(option) {
  ...
  const items1 = [tmp10Result, closure_16(option(4634).Text, { variant: "text-md/semibold", children: option.title })];
  obj2.children = items1;
...
function DropdownPrompt(guildId) {
  ...
  const callback = stateFromStoresArray.useCallback(() => {
    ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(7245, dependencyMap.paths), "DropdownOptions", {
      guildId,
      promptId: _prompt.id,
      onSelect(arg0, arg1) {
        let flag = arg1;
...
const GuildOnboardingPromptsConstants = fn(7211);
...
export default function CustomizeCommunity(guildId) {
  ...
  const tmp7 = stateFromStores(11695)(guildId);
  ...
  const effect1 = noop.useEffect(() => null != guildId ? (() => {
    guildId(7220).ackGuildFeature(closure_1_0, constants.GUILD_ONBOARDING_QUESTION, GuildOnboardingPromptsStore.ackIdForGuild(closure_1_0));
    const obj = guildId(7220);
    const result = stateFromStores(7215).updateOnboardingResponses(closure_1_0);
  }) : undefined, items3);
  ...
  if (0 === onboardingPromptsRaw.length) {
    ...
    if (tmp24Result) {
      ...
      const items5 = [closure_16(tmp4(4634).Text, obj9), newOnboardingPrompts.map(callback), onboardingPromptsWithNewAnswers.map(callback), ];
