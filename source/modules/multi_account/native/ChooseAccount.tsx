...
// Function ID: 16103
// Name: ChooseAccount
// Dependencies: [5, 19, 17, 12565, 12566, 1074, 21, 4640, 576, 1484, 16103, 1241, 12569, 4985, 1115, 1177, 4607, 7308, 7084, 4636, 13979, 16104, 5210, 9951, 8748, 16105, 2]
// Exports: default

// Module 16102 (ChooseAccount)
...
export default function ChooseAccount() {
      ...
      try {
        ...
        if (0 === dependencyMap) {
          if (arg0 === 1) {
            ...
            obj5 = tmp2(4985).confirm(obj5);
            dependencyMap = 1;
            c4 = 1;
            const obj8 = tmp2(4985);
          }
        } else if (arg0 === 1) {
          c4 = 3;
          throw value;
          ...
          if (value) {
            tmp2(4607).hideActionSheet();
            const obj = tmp2(4607);
            tmp5(12569).removeAccount(closure_129_0.id);
            const obj2 = tmp5(12569);
          }
          c4 = 3;
          return { value: "HermesInternal", done: null };
        }
  ...
  obj3.backgroundImageSource = multiAccountUsers(13979);
  ...
  let items = [
    multiAccountUsers.map((user) => {
      let obj = {
        ...
        onPressUser() {
          if (user.tokenStatus === constants.INVALID) {
            ...
            closure_1_2(12569).switchAccount(tmp.id, undefined, constants2.CHOOSE_ACCOUNT);
            const obj = closure_1_2(12569);
          }
          return tmp4;
        },
        trailing: null
      ...
      let obj2 = {
        ...
        onPress() {
          multiAccountUsers(4607).hideActionSheet();
          let obj = multiAccountUsers(4607);
          ...
          items[1] = obj5;
          obj3.options = items;
          const result = user(7308).showSimpleActionSheet(obj3);
        },
        children: null
      };
      const tmp = multiAccountUsers(16104);
      obj2.children = closure_1_11(user(1177).Icon, { size: user(1177).Icon.Sizes.SMALL_20, source: multiAccountUsers(9951), disableColor: true });
      obj.trailing = closure_1_11(user(5210).PressableOpacity, obj2);
      return closure_1_11(tmp, obj, user.id);
    }),

  ];
  let obj6 = { leading: null, label: null, labelStyle: null, onPress: null };
  const tmp2 = multiAccountUsers(7084);
  obj6.leading = closure_11(require("Form").FormRow.Icon, { themedColor: multiAccountUsers(576).colors.TEXT_LINK, size: require("native").Icon.Sizes.SMALL_20, source: multiAccountUsers(16105) });
