...
// Function ID: 16081
// Name: ChooseAccount
// Dependencies: [5, 19, 17, 12556, 12557, 1074, 21, 4638, 576, 1484, 16081, 1241, 12560, 4983, 1115, 1177, 4605, 7304, 7080, 4634, 13971, 16082, 5208, 9940, 8728, 16083, 2]
// Exports: default

// Module 16080 (ChooseAccount)
...
export default function ChooseAccount() {
      ...
      try {
        ...
        if (0 === dependencyMap) {
          if (arg0 === 1) {
            ...
            obj5 = tmp2(4983).confirm(obj5);
            dependencyMap = 1;
            c4 = 1;
            const obj8 = tmp2(4983);
          }
        } else if (arg0 === 1) {
          c4 = 3;
          throw value;
          ...
          if (value) {
            tmp2(4605).hideActionSheet();
            const obj = tmp2(4605);
            tmp5(12560).removeAccount(closure_129_0.id);
            const obj2 = tmp5(12560);
          }
          c4 = 3;
          return { value: "HermesInternal", done: null };
        }
  ...
  obj3.backgroundImageSource = multiAccountUsers(13971);
  ...
  let items = [
    multiAccountUsers.map((user) => {
      let obj = {
        ...
        onPressUser() {
          if (user.tokenStatus === constants.INVALID) {
            ...
            closure_1_2(12560).switchAccount(tmp.id, undefined, constants2.CHOOSE_ACCOUNT);
            const obj = closure_1_2(12560);
          }
          return tmp4;
        },
        trailing: null
      ...
      let obj2 = {
        ...
        onPress() {
          multiAccountUsers(4605).hideActionSheet();
          let obj = multiAccountUsers(4605);
          ...
          items[1] = obj5;
          obj3.options = items;
          const result = user(7304).showSimpleActionSheet(obj3);
        },
        children: null
      };
      const tmp = multiAccountUsers(16082);
      obj2.children = closure_1_11(user(1177).Icon, { size: user(1177).Icon.Sizes.SMALL_20, source: multiAccountUsers(9940), disableColor: true });
      obj.trailing = closure_1_11(user(5208).PressableOpacity, obj2);
      return closure_1_11(tmp, obj, user.id);
    }),

  ];
  let obj6 = { leading: null, label: null, labelStyle: null, onPress: null };
  const tmp2 = multiAccountUsers(7080);
  obj6.leading = closure_11(require("Form").FormRow.Icon, { themedColor: multiAccountUsers(576).colors.TEXT_LINK, size: require("native").Icon.Sizes.SMALL_20, source: multiAccountUsers(16083) });
