...
// Function ID: 16061
// Name: ChooseAccount
// Dependencies: [5, 19, 17, 12548, 12549, 1074, 21, 4639, 576, 1484, 16061, 1241, 12552, 4984, 1115, 1177, 4606, 7301, 7077, 4635, 13958, 16062, 5207, 9932, 8719, 16063, 2]
// Exports: default

// Module 16060 (ChooseAccount)
...
export default function ChooseAccount() {
      ...
      try {
        ...
        if (0 === dependencyMap) {
          if (arg0 === 1) {
            ...
            obj5 = tmp2(4984).confirm(obj5);
            dependencyMap = 1;
            c4 = 1;
            const obj8 = tmp2(4984);
          }
        } else if (arg0 === 1) {
          c4 = 3;
          throw value;
          ...
          if (value) {
            tmp2(4606).hideActionSheet();
            const obj = tmp2(4606);
            tmp5(12552).removeAccount(closure_129_0.id);
            const obj2 = tmp5(12552);
          }
          c4 = 3;
          return { value: "HermesInternal", done: null };
        }
  ...
  obj3.backgroundImageSource = multiAccountUsers(13958);
  ...
  let items = [
    multiAccountUsers.map((user) => {
      let obj = {
        ...
        onPressUser() {
          if (user.tokenStatus === constants.INVALID) {
            ...
            closure_1_2(12552).switchAccount(tmp.id, undefined, constants2.CHOOSE_ACCOUNT);
            const obj = closure_1_2(12552);
          }
          return tmp4;
        },
        trailing: null
      ...
      let obj2 = {
        ...
        onPress() {
          multiAccountUsers(4606).hideActionSheet();
          let obj = multiAccountUsers(4606);
          ...
          items[1] = obj5;
          obj3.options = items;
          const result = user(7301).showSimpleActionSheet(obj3);
        },
        children: null
      };
      const tmp = multiAccountUsers(16062);
      obj2.children = closure_1_11(user(1177).Icon, { size: user(1177).Icon.Sizes.SMALL_20, source: multiAccountUsers(9932), disableColor: true });
      obj.trailing = closure_1_11(user(5207).PressableOpacity, obj2);
      return closure_1_11(tmp, obj, user.id);
    }),

  ];
  let obj6 = { leading: null, label: null, labelStyle: null, onPress: null };
  const tmp2 = multiAccountUsers(7077);
  obj6.leading = closure_11(require("Form").FormRow.Icon, { themedColor: multiAccountUsers(576).colors.TEXT_LINK, size: require("native").Icon.Sizes.SMALL_20, source: multiAccountUsers(16063) });
