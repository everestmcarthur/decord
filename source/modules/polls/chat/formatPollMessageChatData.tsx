...
// Dependencies: [4599, 5511, 1957, 2021, 4828, 1371, 11523, 1074, 1085, 4792, 4259, 4263, 1396, 8847, 4251, 4233, 11813, 1114, 7834, 11815, 1115, 11816, 11817, 2]
// Exports: default, isPollMessageDirectlyInteractive

// Module 11812 (formatPollMessageChatData)
...
import useFormattedExpirationLabel from "useFormattedExpirationLabel" /* 8847 */;
...
const getPollState = fn(11523).getPollState;
    ...
    if (null != currentUser) {
      ...
      if (null != tmp9) {
        ...
        const mapped = answers.map((answer_id) => {
          ...
          let tmp7 = isExpired;
          const match = _mod4792.match({ didSelfVote: tmp6, hasVoted, isExpired, isSelected: hasItem, isLeader: num >= closure_14 && 0 !== num, showResults });
          ...
          if (null == name) {
            ...
            const match1 = tmp8(4792).match(layout_type);
            const str2 = tmp8(4792);
            obj2.votes = match1.with(tmp8(11815).PollLayoutTypes.IMAGE_ONLY_ANSWERS, () => "(" + num.toLocaleString() + ")").otherwise(() => {
