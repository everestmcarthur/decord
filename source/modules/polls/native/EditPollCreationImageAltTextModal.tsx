...
// Dependencies: [32, 19, 17, 7901, 21, 4606, 576, 12319, 12321, 7196, 1114, 1178, 7065, 4602, 5629, 9907, 8684, 2]
// Exports: default

// Module 12322 (EditPollCreationImageAltTextModal)
import nativeDefault from "native" /* 576 */;
import EditPollCreationImageAltTextModalActionCreators from "EditPollCreationImageAltTextModalActionCreators" /* 12321 */;
...
const MAX_POLL_ANSWER_LENGTH = fn(7901).MAX_POLL_ANSWER_LENGTH;
const jsxProd = fn(21);
...
export default function EditPollCreationImageAltTextModal(imageSize) {
  ...
  const tmp4 = value(12319)(channelId, answer.localCreationAnswerId, answer.image, imageSize, imageSize);
  ...
  const obj4 = { onPress: onSave(12321).closeEditPollCreationImageAltTextModal, activeOpacity: 0.5, accessibilityRole: "button", accessibilityLabel: null, children: null };
  const intl = onSave(1114).intl;
  obj4.accessibilityLabel = intl.string(onSave(1114).t.cpT0Cq);
  obj4.children = closure_8(onSave(1178).Icon, { source: value(7065) });
  ...
  items1[1] = closure_8(onSave(4602).Text, obj6);
  ...
  obj7.children = closure_8(onSave(4602).Text, obj8);
  ...
  const tmp2Result = value(5629);
  const intl4 = onSave(1114).intl;
  obj13.children = intl4.string(onSave(1114).t["/2Gnoa"]);
  items4[1] = closure_8(value(9907), obj13);
  const obj14 = { showTopContainer: false, showBorder: false, multiline: false, value, onChange: tmp5[1], clearButtonVisibility: onSave(1178).ClearButtonVisibility.WITH_CONTENT, style: tmp.textInput, textContentType: "none", maxLength: MAX_POLL_ANSWER_LENGTH, autoFocus: true, autoCorrect: true, accessibilityLabel: null };
  const intl5 = onSave(1114).intl;
  obj14.accessibilityLabel = intl5.string(onSave(1114).t["/2Gnoa"]);
  items4[2] = closure_8(onSave(8684).FormInput, obj14);
  ...
  return closure_9(onSave(7196).SafeAreaPaddingView, obj2);
};
