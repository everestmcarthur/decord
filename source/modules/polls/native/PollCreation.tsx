...
// Function ID: 12229
// Name: PollCreation
// Dependencies: [19, 17, 7843, 21, 4574, 576, 1115, 4924, 1114, 6620, 1178, 12229, 4570, 12230, 4541, 12231, 1896, 5619, 4439, 8801, 1250, 6995, 7859, 12227, 4272, 10670, 4285, 12232, 12237, 5644, 7775, 4920, 12238, 7391, 5695, 12239, 12241, 7861, 12253, 8626, 12255, 5618, 2]
// Exports: default

// Module 12228 (PollCreation)
...
import PollCreationModalActionCreators from "PollCreationModalActionCreators" /* 12227 */;
import _modDef12229 from "module_12229" /* 12229 */;
import ScheduledMessagesUtils from "ScheduledMessagesUtils" /* 12241 */;
import PollAnswerInputDefault from "PollAnswerInput" /* 12255 */;
...
function AddAnswerButton(onPress) {
  ...
  const obj = { source: _modDef12229, size: native.Icon.Sizes.LARGE, color: tmp.addAnswerIcon.color };
  const intl = util.intl;
  obj2.accessibilityLabel = intl.string(util.t.B2Uvme);
  const items = [React7(native.Icon, { source: _modDef12229, size: native.Icon.Sizes.LARGE, color: tmp.addAnswerIcon.color }), ];
...
function PollDurationSelectionRow(selectedDuration) {
  ...
  const tmp = onChange(12230)()[selectedDuration];
  ...
  obj.onPress = function onPress() {
    ...
    ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(12231, dependencyMap.paths), React6, { selectedDuration, onChange });
  };
  obj.trailing = closure_9(selectedDuration(4570).Text, { variant: "text-md/normal", color: "text-muted", children: tmp });
  return closure_9(selectedDuration(5619).TableRow, obj);
}
...
export default function PollCreation(channel) {
  ...
  const tmp7 = c12();
  const insets = onCancel(6995)({ includeKeyboardHeight: true }).insets;
  const tmp3 = onCancel(8801);
  let canUseScheduledMessages = channel(7859).useCanUseScheduledMessages();
  dependencyMap = answers.useRef(false);
  const callback = answers.useCallback((arg0) => {
    closure_2.current = true;
    PollCreationModalActionCreators.closeCreatePollModal();
    if (null == arg0) {
      const obj3 = { key: "POLL_CREATED_SUCCESS", IconComponent: tmp(10670).PollsIcon, content: null };
  ...
  let obj2 = channel(7859);
  closure_18 = channel(12237).useTrackPollCreationEvents(answers, allowMultiSelect).trackPollCreationCancelled;
  let obj3 = channel(12237);
  channel(5644).useNavigatorBackPressHandler(() => {
    if (obj.isPollCreationEmpty(c4, answers)) {
      ...
      tmp(4920).openAlert("poll-creation-unsaved-changes", React7(UnsavedChangesAlertModal, obj2));
      flag = true;
      const tmpResult = tmp(4920);
    }
    return flag;
  });
  const obj6 = onCancel(12238)({
    onAddAnswer() {
      handleAddAnswer();
    }
  });
  ...
  obj9.onPress = function onPress() {
    if (obj.isPollCreationEmpty(c4, answers)) {
      closure_18();
      tmp(12227).closeCreatePollModal();
      const AccessibilityAnnouncer = tmp(4285).AccessibilityAnnouncer;
      const intl = tmp(1114).intl;
      AccessibilityAnnouncer.announce(intl.string(tmp(1114).t["+G3oRq"]));
      const tmpResult = tmp(12227);
    } else {
      const obj2 = { onConfirm: handleCancelClose };
      tmp(4920).openAlert("poll-creation-unsaved-changes", React7(UnsavedChangesAlertModal, obj2));
  ...
  obj9.source = onCancel(5695);
  obj9.style = tmp7.actionButton;
  const items3 = [c9(channel(7391).HeaderActionButton, obj9), , , ];
  ...
  items3[1] = c9(channel(4570).Text, obj10);
  if (canUseScheduledMessages) {
    ...
    obj11.icon = tmp14(tmp4(12239).CalendarPlusIcon, obj12);
    ...
    canUseScheduledMessages = tmp14(tmp4(7391).HeaderActionButton, obj11);
  }
  ...
  items3[3] = c9(channel(7391).HeaderActionButton, obj13);
  ...
  if (tmp14Result) {
    ...
    tmp14Result = tmp14(tmp(12253), obj15);
    let tmpResult = tmp(12253);
  }
  ...
  const items7 = [
    c9(channel(8626).FormLabel, obj18),
  ...
  items8[1] = c9(channel(5618).TableCheckboxRow, obj22);
