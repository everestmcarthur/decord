...
// Function ID: 12269
// Name: PollCreation
// Dependencies: [19, 17, 7880, 21, 4605, 576, 1115, 4955, 1114, 6656, 1178, 12269, 4601, 12270, 4572, 12271, 1896, 5655, 4470, 8838, 1250, 7033, 7896, 12267, 4303, 10708, 4316, 12272, 12277, 5680, 7812, 4951, 12278, 7429, 5731, 12279, 12281, 7898, 12293, 8663, 12295, 5654, 2]
// Exports: default

// Module 12268 (PollCreation)
...
import _modDef12269 from "module_12269" /* 12269 */;
import ScheduledMessagesUtils from "ScheduledMessagesUtils" /* 12281 */;
import PollAnswerInputDefault from "PollAnswerInput" /* 12295 */;
...
function AddAnswerButton(onPress) {
  ...
  const obj = { source: _modDef12269, size: native.Icon.Sizes.LARGE, color: tmp.addAnswerIcon.color };
  const intl = util.intl;
  obj2.accessibilityLabel = intl.string(util.t.B2Uvme);
  const items = [React7(native.Icon, { source: _modDef12269, size: native.Icon.Sizes.LARGE, color: tmp.addAnswerIcon.color }), ];
...
function PollDurationSelectionRow(selectedDuration) {
  ...
  const tmp = onChange(12270)()[selectedDuration];
  ...
  obj.onPress = function onPress() {
    ...
    ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(12271, dependencyMap.paths), React6, { selectedDuration, onChange });
  };
  obj.trailing = closure_9(selectedDuration(4601).Text, { variant: "text-md/normal", color: "text-muted", children: tmp });
  return closure_9(selectedDuration(5655).TableRow, obj);
}
...
let closure_14 = noop.forwardRef((error, ref) => {
  ...
  const obj = { ref, textAlignVertical: "center", label: null, accessibilityHint: null, placeholder: null, onChange: null, isClearable: true, status: null, errorMessage: null, maxLength: null, returnKeyType: "next", blurOnSubmit: false, onSubmitEditing: null, textContentType: "none", autoFocus: true, autoCorrect: true };
  const intl = util.intl;
  ...
  if (tmp) {
    ...
    formatToPlainStringResult = intl2.formatToPlainString(tmp3(1114).t.jnq5Ho, obj2);
  }
...
export default function PollCreation(channel) {
  ...
  const tmp7 = c12();
  const insets = onCancel(7033)({ includeKeyboardHeight: true }).insets;
  const tmp3 = onCancel(8838);
  let canUseScheduledMessages = channel(7896).useCanUseScheduledMessages();
  dependencyMap = answers.useRef(false);
  const callback = answers.useCallback((arg0) => {
    closure_2.current = true;
    PollCreationModalActionCreators.closeCreatePollModal();
    if (null == arg0) {
      const obj3 = { key: "POLL_CREATED_SUCCESS", IconComponent: tmp(10708).PollsIcon, content: null };
      const intl = tmp(1114).intl;
      obj3.content = intl.string(tmp(1114).t.OPsckI);
      ToastActionCreatorsDefault.open(obj3);
    }
  }, []);
  const callback1 = answers.useCallback((indexToRemove) => {
    const AccessibilityAnnouncer = channel(4316).AccessibilityAnnouncer;
    const intl = channel(1114).intl;
    AccessibilityAnnouncer.announce(intl.formatToPlainString(channel(1114).t.BByGU4, { number: indexToRemove.indexToRemove + 1 }));
  }, []);
  const tmp11 = onCancel(12272)(channel, callback, callback1);
  ...
  let obj2 = channel(7896);
  closure_18 = channel(12277).useTrackPollCreationEvents(answers, allowMultiSelect).trackPollCreationCancelled;
  let obj3 = channel(12277);
  channel(5680).useNavigatorBackPressHandler(() => {
    if (obj.isPollCreationEmpty(c4, answers)) {
      ...
      tmp(4951).openAlert("poll-creation-unsaved-changes", React7(UnsavedChangesAlertModal, obj2));
      flag = true;
      const tmpResult = tmp(4951);
    }
    return flag;
  });
  const obj6 = onCancel(12278)({
    onAddAnswer() {
      handleAddAnswer();
    }
  });
  ...
  obj9.accessibilityLabel = intl.string(channel(1114).t["ETE/oC"]);
  obj9.onPress = function onPress() {
    if (obj.isPollCreationEmpty(c4, answers)) {
      closure_18();
      tmp(12267).closeCreatePollModal();
      const AccessibilityAnnouncer = tmp(4316).AccessibilityAnnouncer;
      const intl = tmp(1114).intl;
      AccessibilityAnnouncer.announce(intl.string(tmp(1114).t["+G3oRq"]));
      const tmpResult = tmp(12267);
    } else {
      const obj2 = { onConfirm: handleCancelClose };
      tmp(4951).openAlert("poll-creation-unsaved-changes", React7(UnsavedChangesAlertModal, obj2));
      const tmpResult2 = tmp(4951);
    }
  };
  obj9.source = onCancel(5731);
  obj9.style = tmp7.actionButton;
  const items3 = [c9(channel(7429).HeaderActionButton, obj9), , , ];
  ...
  obj10.children = intl2.string(channel(1114).t.Flr51u);
  items3[1] = c9(channel(4601).Text, obj10);
  if (canUseScheduledMessages) {
    const obj11 = { accessibilityLabel: null, style: null, disabled: null, icon: null, onPress: null };
    const intl3 = tmp4(1114).intl;
    obj11.accessibilityLabel = intl3.string(tmp4(1114).t.rlf0tb);
    ...
    obj11.icon = tmp14(tmp4(12279).CalendarPlusIcon, obj12);
  ...
  if (tmp14Result) {
    ...
    tmp14Result = tmp14(tmp(12293), obj15);
    let tmpResult = tmp(12293);
  }
  ...
  obj18.text = intl5.string(channel(1114).t.oMBfeS);
  obj18.style = tmp7.label;
  const items7 = [
    c9(channel(8663).FormLabel, obj18),
