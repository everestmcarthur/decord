...
// Function ID: 12294
// Name: PollCreation
// Dependencies: [19, 17, 7902, 21, 4607, 576, 1115, 4957, 1114, 6678, 1178, 12294, 4603, 12295, 4574, 12296, 1896, 5657, 4472, 8861, 1250, 7055, 7918, 12292, 4305, 10729, 4318, 12297, 12302, 5682, 7834, 4953, 12303, 7451, 5733, 12304, 12306, 7920, 12318, 8686, 12320, 5656, 2]
// Exports: default

// Module 12293 (PollCreation)
...
import PollAnswerInputDefault from "PollAnswerInput" /* 12320 */;
...
function AddAnswerButton(onPress) {
  ...
  const obj = { source: _modDef12294, size: native.Icon.Sizes.LARGE, color: tmp.addAnswerIcon.color };
  const intl = util.intl;
  obj2.accessibilityLabel = intl.string(util.t.B2Uvme);
  const items = [React7(native.Icon, { source: _modDef12294, size: native.Icon.Sizes.LARGE, color: tmp.addAnswerIcon.color }), ];
...
function PollDurationSelectionRow(selectedDuration) {
  ...
  const tmp = onChange(12295)()[selectedDuration];
  ...
  obj.onPress = function onPress() {
    ...
    ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(12296, dependencyMap.paths), React6, { selectedDuration, onChange });
  };
  obj.trailing = closure_9(selectedDuration(4603).Text, { variant: "text-md/normal", color: "text-muted", children: tmp });
  return closure_9(selectedDuration(5657).TableRow, obj);
}
get_ActivityIndicator = fn(17);
({ TouchableOpacity: closure_4, View: hasOwnProperty, ScrollView: metroRequire } = get_ActivityIndicator);
const PollsConstants = fn(7902);
...
export default function PollCreation(channel) {
  ...
  const tmp7 = c12();
  const insets = onCancel(7055)({ includeKeyboardHeight: true }).insets;
  const tmp3 = onCancel(8861);
  let canUseScheduledMessages = channel(7918).useCanUseScheduledMessages();
  dependencyMap = answers.useRef(false);
  const callback = answers.useCallback((arg0) => {
    closure_2.current = true;
    PollCreationModalActionCreators.closeCreatePollModal();
    if (null == arg0) {
      const obj3 = { key: "POLL_CREATED_SUCCESS", IconComponent: tmp(10729).PollsIcon, content: null };
  ...
  const tmp11 = onCancel(12297)(channel, callback, callback1);
  ...
  let obj2 = channel(7918);
  closure_18 = channel(12302).useTrackPollCreationEvents(answers, allowMultiSelect).trackPollCreationCancelled;
  let obj3 = channel(12302);
  channel(5682).useNavigatorBackPressHandler(() => {
    if (obj.isPollCreationEmpty(c4, answers)) {
      ...
      tmp(4953).openAlert("poll-creation-unsaved-changes", React7(UnsavedChangesAlertModal, obj2));
      flag = true;
      const tmpResult = tmp(4953);
    }
    return flag;
  });
  const obj6 = onCancel(12303)({
    onAddAnswer() {
      handleAddAnswer();
    }
  });
  ...
  obj9.onPress = function onPress() {
    if (obj.isPollCreationEmpty(c4, answers)) {
      closure_18();
      tmp(12292).closeCreatePollModal();
      const AccessibilityAnnouncer = tmp(4318).AccessibilityAnnouncer;
      const intl = tmp(1114).intl;
      AccessibilityAnnouncer.announce(intl.string(tmp(1114).t["+G3oRq"]));
      const tmpResult = tmp(12292);
    } else {
      const obj2 = { onConfirm: handleCancelClose };
      tmp(4953).openAlert("poll-creation-unsaved-changes", React7(UnsavedChangesAlertModal, obj2));
      const tmpResult2 = tmp(4953);
    }
  };
  obj9.source = onCancel(5733);
  obj9.style = tmp7.actionButton;
  const items3 = [c9(channel(7451).HeaderActionButton, obj9), , , ];
  ...
  items3[1] = c9(channel(4603).Text, obj10);
  if (canUseScheduledMessages) {
    ...
    obj11.icon = tmp14(tmp4(12304).CalendarPlusIcon, obj12);
    ...
    canUseScheduledMessages = tmp14(tmp4(7451).HeaderActionButton, obj11);
  }
  ...
  items3[3] = c9(channel(7451).HeaderActionButton, obj13);
  ...
  if (tmp14Result) {
    ...
    tmp14Result = tmp14(tmp(12318), obj15);
    let tmpResult = tmp(12318);
  }
  ...
  const items7 = [
    c9(channel(8686).FormLabel, obj18),
  ...
  items8[1] = c9(channel(5656).TableCheckboxRow, obj22);
