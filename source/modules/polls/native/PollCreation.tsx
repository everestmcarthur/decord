...
// Function ID: 12341
// Name: PollCreation
// Dependencies: [19, 17, 7943, 21, 4638, 576, 1364, 4988, 1115, 6714, 1177, 12341, 4634, 12342, 4605, 12343, 1897, 5691, 4504, 8908, 1249, 7091, 7959, 12339, 4336, 10779, 4349, 12344, 12349, 5716, 7875, 4984, 12350, 7487, 5767, 12351, 12353, 7961, 12365, 8728, 12367, 5690, 2]
// Exports: default

// Module 12340 (PollCreation)
...
import _modDef12341 from "module_12341" /* 12341 */;
import ScheduledMessagesUtils from "ScheduledMessagesUtils" /* 12353 */;
import PollAnswerInputDefault from "PollAnswerInput" /* 12367 */;
...
function AddAnswerButton(onPress) {
  ...
  const obj = { source: _modDef12341, size: native.Icon.Sizes.LARGE, color: tmp.addAnswerIcon.color };
  const intl = util.intl;
  obj2.accessibilityLabel = intl.string(util.t.B2Uvme);
  const items = [React7(native.Icon, { source: _modDef12341, size: native.Icon.Sizes.LARGE, color: tmp.addAnswerIcon.color }), ];
...
function PollDurationSelectionRow(selectedDuration) {
  ...
  const tmp = onChange(12342)()[selectedDuration];
  ...
  obj.onPress = function onPress() {
    ...
    ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(12343, dependencyMap.paths), React6, { selectedDuration, onChange });
  };
  obj.trailing = closure_9(selectedDuration(4634).Text, { variant: "text-md/normal", color: "text-muted", children: tmp });
  return closure_9(selectedDuration(5691).TableRow, obj);
}
...
export default function PollCreation(channel) {
  ...
  const tmp7 = c12();
  const insets = onCancel(7091)({ includeKeyboardHeight: true }).insets;
  const tmp3 = onCancel(8908);
  let canUseScheduledMessages = channel(7959).useCanUseScheduledMessages();
  dependencyMap = answers.useRef(false);
  const callback = answers.useCallback((arg0) => {
    closure_2.current = true;
    PollCreationModalActionCreators.closeCreatePollModal();
    if (null == arg0) {
      const obj3 = { key: "POLL_CREATED_SUCCESS", IconComponent: tmp(10779).PollsIcon, content: null };
  ...
  let obj2 = channel(7959);
  closure_18 = channel(12349).useTrackPollCreationEvents(answers, allowMultiSelect).trackPollCreationCancelled;
  let obj3 = channel(12349);
  channel(5716).useNavigatorBackPressHandler(() => {
    if (obj.isPollCreationEmpty(c4, answers)) {
      closure_18();
      const AccessibilityAnnouncer = tmp(4349).AccessibilityAnnouncer;
      const intl = tmp(1115).intl;
      AccessibilityAnnouncer.announce(intl.string(tmp(1115).t["+G3oRq"]));
      let flag = false;
    } else {
      const obj2 = { onConfirm: handleCancelClose };
      tmp(4984).openAlert("poll-creation-unsaved-changes", React7(UnsavedChangesAlertModal, obj2));
      flag = true;
      const tmpResult = tmp(4984);
    }
    return flag;
  });
  const obj6 = onCancel(12350)({
    onAddAnswer() {
      handleAddAnswer();
    }
  });
  ...
  obj9.onPress = function onPress() {
    if (obj.isPollCreationEmpty(c4, answers)) {
      closure_18();
      tmp(12339).closeCreatePollModal();
      const AccessibilityAnnouncer = tmp(4349).AccessibilityAnnouncer;
      const intl = tmp(1115).intl;
      AccessibilityAnnouncer.announce(intl.string(tmp(1115).t["+G3oRq"]));
      ...
      const obj2 = { onConfirm: handleCancelClose };
      tmp(4984).openAlert("poll-creation-unsaved-changes", React7(UnsavedChangesAlertModal, obj2));
      const tmpResult2 = tmp(4984);
    }
  };
  obj9.source = onCancel(5767);
  obj9.style = tmp7.actionButton;
  const items3 = [c9(channel(7487).HeaderActionButton, obj9), , , ];
  ...
  items3[1] = c9(channel(4634).Text, obj10);
  if (canUseScheduledMessages) {
    ...
    obj11.icon = tmp14(tmp4(12351).CalendarPlusIcon, obj12);
    ...
    canUseScheduledMessages = tmp14(tmp4(7487).HeaderActionButton, obj11);
  }
  ...
  items3[3] = c9(channel(7487).HeaderActionButton, obj13);
  ...
  if (tmp14Result) {
    ...
    tmp14Result = tmp14(tmp(12365), obj15);
  ...
  const items7 = [
    c9(channel(8728).FormLabel, obj18),
  ...
  items8[1] = c9(channel(5690).TableCheckboxRow, obj22);
