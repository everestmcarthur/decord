...
// Function ID: 12350
// Name: PollCreation
// Dependencies: [19, 17, 7949, 21, 4640, 576, 1364, 4990, 1115, 6717, 1177, 12350, 4636, 12351, 4607, 12352, 1897, 5693, 4506, 8924, 1249, 7095, 7965, 12348, 4338, 10787, 4351, 12353, 12358, 5718, 7881, 4986, 12359, 7491, 5769, 12360, 12362, 7967, 12374, 8748, 12376, 5692, 2]
// Exports: default

// Module 12349 (PollCreation)
...
import ScheduledMessageTypes from "ScheduledMessageTypes" /* 7967 */;
...
import PollAnswerInputDefault from "PollAnswerInput" /* 12376 */;
...
function AddAnswerButton(onPress) {
  ...
  const obj = { source: _modDef12350, size: native.Icon.Sizes.LARGE, color: tmp.addAnswerIcon.color };
  const intl = util.intl;
  obj2.accessibilityLabel = intl.string(util.t.B2Uvme);
  const items = [React7(native.Icon, { source: _modDef12350, size: native.Icon.Sizes.LARGE, color: tmp.addAnswerIcon.color }), ];
...
function PollDurationSelectionRow(selectedDuration) {
  ...
  const tmp = onChange(12351)()[selectedDuration];
  ...
  obj.onPress = function onPress() {
    ...
    ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(12352, dependencyMap.paths), React6, { selectedDuration, onChange });
  };
  obj.trailing = closure_9(selectedDuration(4636).Text, { variant: "text-md/normal", color: "text-muted", children: tmp });
  return closure_9(selectedDuration(5693).TableRow, obj);
}
...
export default function PollCreation(channel) {
  ...
  const tmp7 = c12();
  const insets = onCancel(7095)({ includeKeyboardHeight: true }).insets;
  const tmp3 = onCancel(8924);
  let canUseScheduledMessages = channel(7965).useCanUseScheduledMessages();
  dependencyMap = answers.useRef(false);
  const callback = answers.useCallback((arg0) => {
    closure_2.current = true;
    PollCreationModalActionCreators.closeCreatePollModal();
    if (null == arg0) {
      const obj3 = { key: "POLL_CREATED_SUCCESS", IconComponent: tmp(10787).PollsIcon, content: null };
  ...
  const tmp11 = onCancel(12353)(channel, callback, callback1);
  ...
  let obj2 = channel(7965);
  closure_18 = channel(12358).useTrackPollCreationEvents(answers, allowMultiSelect).trackPollCreationCancelled;
  let obj3 = channel(12358);
  channel(5718).useNavigatorBackPressHandler(() => {
    if (obj.isPollCreationEmpty(c4, answers)) {
      ...
      tmp(4986).openAlert("poll-creation-unsaved-changes", React7(UnsavedChangesAlertModal, obj2));
      flag = true;
      const tmpResult = tmp(4986);
    }
    return flag;
  });
  const obj6 = onCancel(12359)({
    onAddAnswer() {
      handleAddAnswer();
    }
  });
  ...
  obj9.onPress = function onPress() {
    if (obj.isPollCreationEmpty(c4, answers)) {
      closure_18();
      tmp(12348).closeCreatePollModal();
      const AccessibilityAnnouncer = tmp(4351).AccessibilityAnnouncer;
      const intl = tmp(1115).intl;
      AccessibilityAnnouncer.announce(intl.string(tmp(1115).t["+G3oRq"]));
      const tmpResult = tmp(12348);
    } else {
      const obj2 = { onConfirm: handleCancelClose };
      tmp(4986).openAlert("poll-creation-unsaved-changes", React7(UnsavedChangesAlertModal, obj2));
  ...
  obj9.source = onCancel(5769);
  obj9.style = tmp7.actionButton;
  const items3 = [c9(channel(7491).HeaderActionButton, obj9), , , ];
  ...
  items3[1] = c9(channel(4636).Text, obj10);
  if (canUseScheduledMessages) {
    ...
    obj11.icon = tmp14(tmp4(12360).CalendarPlusIcon, obj12);
    ...
    canUseScheduledMessages = tmp14(tmp4(7491).HeaderActionButton, obj11);
  }
  ...
  items3[3] = c9(channel(7491).HeaderActionButton, obj13);
  ...
  if (tmp14Result) {
    ...
    tmp14Result = tmp14(tmp(12374), obj15);
    let tmpResult = tmp(12374);
  }
  ...
  const items7 = [
    c9(channel(8748).FormLabel, obj18),
  ...
  items8[1] = c9(channel(5692).TableCheckboxRow, obj22);
