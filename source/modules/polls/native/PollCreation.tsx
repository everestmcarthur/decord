...
// Function ID: 12333
// Name: PollCreation
// Dependencies: [19, 17, 7934, 21, 4639, 576, 1364, 4989, 1115, 6711, 1177, 12333, 4635, 12334, 4606, 12335, 1897, 5690, 4505, 8897, 1249, 7088, 7950, 12331, 4338, 10771, 4351, 12336, 12341, 5715, 7866, 4985, 12342, 7484, 5766, 12343, 12345, 7952, 12357, 8719, 12359, 5689, 2]
// Exports: default

// Module 12332 (PollCreation)
...
import PollCreationModalActionCreators from "PollCreationModalActionCreators" /* 12331 */;
import _modDef12333 from "module_12333" /* 12333 */;
import ScheduledMessagesUtils from "ScheduledMessagesUtils" /* 12345 */;
import PollAnswerInputDefault from "PollAnswerInput" /* 12359 */;
...
function AddAnswerButton(onPress) {
  ...
  const obj = { source: _modDef12333, size: native.Icon.Sizes.LARGE, color: tmp.addAnswerIcon.color };
  const intl = util.intl;
  obj2.accessibilityLabel = intl.string(util.t.B2Uvme);
  const items = [React7(native.Icon, { source: _modDef12333, size: native.Icon.Sizes.LARGE, color: tmp.addAnswerIcon.color }), ];
...
function PollDurationSelectionRow(selectedDuration) {
  ...
  const tmp = onChange(12334)()[selectedDuration];
  ...
  obj.onPress = function onPress() {
    ...
    ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(12335, dependencyMap.paths), React6, { selectedDuration, onChange });
  };
  obj.trailing = closure_9(selectedDuration(4635).Text, { variant: "text-md/normal", color: "text-muted", children: tmp });
  return closure_9(selectedDuration(5690).TableRow, obj);
}
...
export default function PollCreation(channel) {
  ...
  const tmp7 = c12();
  const insets = onCancel(7088)({ includeKeyboardHeight: true }).insets;
  const tmp3 = onCancel(8897);
  let canUseScheduledMessages = channel(7950).useCanUseScheduledMessages();
  dependencyMap = answers.useRef(false);
  const callback = answers.useCallback((arg0) => {
    closure_2.current = true;
    PollCreationModalActionCreators.closeCreatePollModal();
    if (null == arg0) {
      const obj3 = { key: "POLL_CREATED_SUCCESS", IconComponent: tmp(10771).PollsIcon, content: null };
  ...
  let obj2 = channel(7950);
  closure_18 = channel(12341).useTrackPollCreationEvents(answers, allowMultiSelect).trackPollCreationCancelled;
  let obj3 = channel(12341);
  channel(5715).useNavigatorBackPressHandler(() => {
    if (obj.isPollCreationEmpty(c4, answers)) {
      closure_18();
      const AccessibilityAnnouncer = tmp(4351).AccessibilityAnnouncer;
      const intl = tmp(1115).intl;
      AccessibilityAnnouncer.announce(intl.string(tmp(1115).t["+G3oRq"]));
      let flag = false;
    } else {
      const obj2 = { onConfirm: handleCancelClose };
      tmp(4985).openAlert("poll-creation-unsaved-changes", React7(UnsavedChangesAlertModal, obj2));
      flag = true;
      const tmpResult = tmp(4985);
    }
    return flag;
  });
  const obj6 = onCancel(12342)({
    onAddAnswer() {
      handleAddAnswer();
    }
  });
  ...
  obj9.onPress = function onPress() {
    if (obj.isPollCreationEmpty(c4, answers)) {
      closure_18();
      tmp(12331).closeCreatePollModal();
      const AccessibilityAnnouncer = tmp(4351).AccessibilityAnnouncer;
      const intl = tmp(1115).intl;
      AccessibilityAnnouncer.announce(intl.string(tmp(1115).t["+G3oRq"]));
  ...
  obj9.source = onCancel(5766);
  obj9.style = tmp7.actionButton;
  const items3 = [c9(channel(7484).HeaderActionButton, obj9), , , ];
  ...
  items3[1] = c9(channel(4635).Text, obj10);
  if (canUseScheduledMessages) {
    ...
    obj11.icon = tmp14(tmp4(12343).CalendarPlusIcon, obj12);
    ...
    canUseScheduledMessages = tmp14(tmp4(7484).HeaderActionButton, obj11);
  }
  ...
  items3[3] = c9(channel(7484).HeaderActionButton, obj13);
  ...
  if (tmp14Result) {
    ...
    tmp14Result = tmp14(tmp(12357), obj15);
    let tmpResult = tmp(12357);
  }
  ...
  const items7 = [
    c9(channel(8719).FormLabel, obj18),
  ...
  items8[1] = c9(channel(5689).TableCheckboxRow, obj22);
