...
// Dependencies: [19, 17, 21, 4640, 576, 5060, 6721, 4636, 11171, 1115, 11173, 13310, 7355, 4574, 13312, 5062, 4491, 8992, 2]
...
// Module 13311 (OrbCheckoutModalComponents)
...
import OrbCheckoutAmountTagDefault from "OrbCheckoutAmountTag" /* 11173 */;
import OrbCheckoutModalContext from "OrbCheckoutModalContext" /* 13310 */;
import useVirtualCurrencyBalance from "useVirtualCurrencyBalance" /* 13312 */;
...
export const OrbCheckoutLegalFinePrint = function OrbCheckoutLegalFinePrint() {
  const tmp = closure_8();
  skuId = skuId(13310).useOrbCheckoutModalContext().skuId;
  const items = [skuId];
  const memo = noop.useMemo(() => OrbCheckoutUtils.getOrbCheckoutDisclaimerMessage(skuId), items);
  return closure_6(skuId(4636).Text, { style: tmp.disclaimer, variant: "text-xxs/normal", color: "interactive-text-active", children: memo });
};
