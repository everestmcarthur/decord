...
// Dependencies: [19, 17, 4714, 4590, 1878, 6818, 1610, 5161, 4591, 4588, 4594, 11608, 2]

// Module 12879 (PortalKeyboardInlineComponent)
...
export default noop.memo(function InlinePortalKeyboard(messagesRef) {
  ...
  const tmp2 = id(4590)();
  dependencyMap = tmp2;
  let tmp4 = id(6818)({ includeCustomKeyboard: false });
  noop = tmp4;
  let tmp3 = id(1878)();
  const keyboardContextForType = messagesRef(4590).useKeyboardContextForType(messagesRef(1610).KeyboardTypes.SYSTEM);
  id(5161)(() => () => {
  ...
  let PortalKeyboardUIStore = messagesRef(4591).PortalKeyboardUIStore;
  let field = PortalKeyboardUIStore.useField("keyboard");
  const PortalKeyboardUIStore2 = messagesRef(4591).PortalKeyboardUIStore;
  ...
  const layoutEffect = noop.useLayoutEffect(() => {
    ...
    if (null != field) {
      if (null == tmp2.handlerId) {
        if (null != tmp2) {
          if (field1 === messagesRef(4594).PortalKeyboardState.REQUEST_OPEN) {
            ...
            if (null != chatRef) {
              ...
              if (null != tmp38) {
                const result = messagesRef(4591).handlePortalKeyboardOpen(id);
                tmp.current = true;
                const obj2 = messagesRef(4591);
                const DCDChatManager3 = keyboardContextForType.DCDChatManager;
                const result1 = DCDChatManager3.customKeyboardWillShow(tmp38, messagesRef(11608).getKeyboardActionSheetHeight().minimum, 0.25, 7);
                const obj3 = messagesRef(11608);
              }
            }
          }
        }
        if (dependencyMap === messagesRef(1610).KeyboardTypes.SYSTEM) {
          if (keyboardContextForType.keyboardWillOpen) {
            if (field1 !== messagesRef(4594).PortalKeyboardState.REQUEST_CLOSE) {
              const _setTimeout = setTimeout;
              messagesRef = setTimeout(messagesRef(4591).closePortalKeyboardRequest, 250);
              return () => clearTimeout(closure_0);
            }
          }
          messagesRef(4591).closePortalKeyboard();
