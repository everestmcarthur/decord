...
// Dependencies: [19, 17, 21, 576, 4489, 4350, 5049, 4341, 5048, 4376, 5058, 5062, 2]
// Exports: BackgroundBlurFill, BackgroundBlurFillAnimated, BackgroundBlurFillWithPress

// Module 8723 (BackgroundBlurFill)
import nativeDefault from "native" /* 576 */;
import ReanimatedRexport from "ReanimatedRexport" /* 4376 */;
import VisualEffectView from "VisualEffectView" /* 5049 */;
import spring from "spring" /* 5058 */;
import springPresets from "springPresets" /* 5062 */;
import noop from "module_19" /* 19 */;

const VisualEffectViewAnimatedDefault = tmp3(5048);
const VisualEffectViewDefault = tmp3(5049);
require = fn;
...
export const BlurTheme = fn(5049).BlurTheme;
export const BlurStyle = fn(5049).BlurStyle;
...
export const BackgroundBlurFillWithPress = function BackgroundBlurFillWithPress(style) {
  ...
  obj = pressed(4350);
  const normalizeBlurThemeResult = pressed(5049).normalizeBlurTheme(blurTheme);
  const tmp2Result = pressed(5049);
  const tmp6 = pressed(5049).isBlurThemeLight(blurTheme) ? closure_6 : closure_8;
  importDefault = tmp6;
  const tmp2Result8 = pressed(5049);
  const tmp7 = pressed(5049).isBlurThemeLight(blurTheme) ? closure_7 : closure_9;
  dependencyMap = tmp7;
  const tmp2Result9 = pressed(5049);
  ...
  const isBlurDisabledResult = pressed(5049).isBlurDisabled(merged);
  c5 = isBlurDisabledResult;
  const tmp2Result12 = pressed(5049);
  const fn = function p() {
  ...
  fn.__closure = { withSpring: pressed(5058).withSpring, interpolateColor: pressed(4376).interpolateColor, pressed, fallbackColor: token, fallbackColorPressed: token1, ON_PRESS_SPRING: pressed(5062).ON_PRESS_SPRING };
  fn.__workletHash = 10497618157620;
  fn.__initData = __initData;
  let animatedStyle = tmp2Result13.useAnimatedStyle(fn);
  let obj2 = { withSpring: pressed(5058).withSpring, interpolateColor: pressed(4376).interpolateColor, pressed, fallbackColor: token, fallbackColorPressed: token1, ON_PRESS_SPRING: pressed(5062).ON_PRESS_SPRING };
  class C {
    constructor() {
  ...
  C.__closure = { shouldUseFallback: isBlurDisabledResult, withSpring: pressed(5058).withSpring, interpolateColor: pressed(4376).interpolateColor, pressed, restingTint: tmp6, pressedTint: tmp7, ON_PRESS_SPRING: pressed(5062).ON_PRESS_SPRING };
  ...
  let obj3 = { shouldUseFallback: isBlurDisabledResult, withSpring: pressed(5058).withSpring, interpolateColor: pressed(4376).interpolateColor, pressed, restingTint: tmp6, pressedTint: tmp7, ON_PRESS_SPRING: pressed(5062).ON_PRESS_SPRING };
  const tmp12 = c5;
