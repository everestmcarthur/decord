...
// Dependencies: [19, 17, 21, 576, 4487, 4348, 5048, 4339, 5047, 4374, 5059, 5063, 2]
// Exports: BackgroundBlurFill, BackgroundBlurFillAnimated, BackgroundBlurFillWithPress

// Module 8732 (BackgroundBlurFill)
import nativeDefault from "native" /* 576 */;
import ReanimatedRexport from "ReanimatedRexport" /* 4374 */;
import VisualEffectView from "VisualEffectView" /* 5048 */;
import spring from "spring" /* 5059 */;
import springPresets from "springPresets" /* 5063 */;
import noop from "module_19" /* 19 */;

const VisualEffectViewAnimatedDefault = tmp3(5047);
const VisualEffectViewDefault = tmp3(5048);
require = fn;
...
export const BlurTheme = fn(5048).BlurTheme;
export const BlurStyle = fn(5048).BlurStyle;
export const BackgroundBlurFill = function BackgroundBlurFill(arg0) {
  ...
  obj = blurTheme(4348);
  obj2 = noop;
  const tmpResult = blurTheme(4339);
...
export const BackgroundBlurFillWithPress = function BackgroundBlurFillWithPress(style) {
  ...
  obj = pressed(4348);
  const normalizeBlurThemeResult = pressed(5048).normalizeBlurTheme(blurTheme);
  const tmp2Result = pressed(5048);
  const tmp6 = pressed(5048).isBlurThemeLight(blurTheme) ? closure_6 : closure_8;
  importDefault = tmp6;
  const tmp2Result8 = pressed(5048);
  const tmp7 = pressed(5048).isBlurThemeLight(blurTheme) ? closure_7 : closure_9;
  dependencyMap = tmp7;
  const tmp2Result9 = pressed(5048);
  token = pressed(4339).useToken(nativeDefault.colors.BACKGROUND_SCRIM, normalizeBlurThemeResult);
  const tmp2Result10 = pressed(4339);
  token1 = pressed(4339).useToken(nativeDefault.colors.BACKGROUND_SCRIM_LIGHTBOX, normalizeBlurThemeResult);
  const tmp2Result11 = pressed(4339);
  const isBlurDisabledResult = pressed(5048).isBlurDisabled(merged);
  c5 = isBlurDisabledResult;
  const tmp2Result12 = pressed(5048);
  const fn = function p() {
  ...
  const tmp2Result13 = pressed(4374);
  fn.__closure = { withSpring: pressed(5059).withSpring, interpolateColor: pressed(4374).interpolateColor, pressed, fallbackColor: token, fallbackColorPressed: token1, ON_PRESS_SPRING: pressed(5063).ON_PRESS_SPRING };
  fn.__workletHash = 10497618157620;
  fn.__initData = __initData;
  let animatedStyle = tmp2Result13.useAnimatedStyle(fn);
  let obj2 = { withSpring: pressed(5059).withSpring, interpolateColor: pressed(4374).interpolateColor, pressed, fallbackColor: token, fallbackColorPressed: token1, ON_PRESS_SPRING: pressed(5063).ON_PRESS_SPRING };
  class C {
    constructor() {
  ...
  const tmp2Result14 = pressed(4374);
  C.__closure = { shouldUseFallback: isBlurDisabledResult, withSpring: pressed(5059).withSpring, interpolateColor: pressed(4374).interpolateColor, pressed, restingTint: tmp6, pressedTint: tmp7, ON_PRESS_SPRING: pressed(5063).ON_PRESS_SPRING };
  ...
  let obj3 = { shouldUseFallback: isBlurDisabledResult, withSpring: pressed(5059).withSpring, interpolateColor: pressed(4374).interpolateColor, pressed, restingTint: tmp6, pressedTint: tmp7, ON_PRESS_SPRING: pressed(5063).ON_PRESS_SPRING };
  const tmp12 = c5;
