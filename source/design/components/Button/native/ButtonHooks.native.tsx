...
// Function ID: 5066
// Name: ButtonHooks
// Dependencies: [19, 4639, 4350, 576, 4491, 4341, 4376, 5058, 5062, 5064, 5066, 5061, 4635, 1364, 2]
...
export const useIconSizeStyles = (arg0) => {
  ...
  return fontScale.useMemo(() => {
    Icon;
    if ("sm" === closure_0) {
      let iconSize = tmp(5061).getIconSize(tmp(5064).SMALL_BUTTON_ICON_SIZE);
      const tmpResult = tmp(5061);
    } else {
      iconSize = tmp4;
      if ("lg" === tmp5) {
        iconSize = tmp(5061).getIconSize(tmp(5064).LARGE_BUTTON_ICON_SIZE);
    ...
    if (flag) {
      width = iconSize;
      if (fontScale > 1) {
        const tmpResult5 = tmp(5064);
        const tmp13 = tmp(4635).TextStyleSheet[tmpResult5.getButtonDefaultTextVariant(tmpResult5, tmp5)];
