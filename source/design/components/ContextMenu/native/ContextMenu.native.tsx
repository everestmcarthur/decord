...
// Function ID: 8758
// Name: ContextMenu
// Dependencies: [19, 21, 1115, 4343, 1477, 8758, 5033, 8759, 5022, 8760, 4458, 1114, 8761, 6726, 4574, 5013, 2]
// Exports: ContextMenu

// Module 8757 (ContextMenu)
...
export const ContextMenu = function ContextMenu(triggerOnLongPress) {
  ...
  const memo1 = obj3.useMemo(() => {
    ...
    if (flag) {
      const Gesture2 = tmp2(6726).Gesture;
      ...
      const obj2 = { updateContextMenuState: tmp2(8758).updateContextMenuState, state: tmp };
      fn4.__closure = obj2;
      ...
      const Gesture3 = tmp2(6726).Gesture;
      ...
      let result = Gesture3.LongPress().enabled(flag5).minDuration(tmp2(8759).CONTEXT_MENU_LONG_PRESS_DURATION_MS).shouldCancelWhenOutside(false);
      ...
      const obj3 = { runOnJS: tmp2(4343).runOnJS, triggerHapticFeedback: tmp2(4574).triggerHapticFeedback, CONTEXT_MENU_OPEN_HAPTIC: tmp2(8759).CONTEXT_MENU_OPEN_HAPTIC, measureButtonAndShowMenu: callback3 };
      fn5.__closure = obj3;
      fn5.__workletHash = 13919366908951;
      fn5.__initData = __initData3;
      const minDurationResult = Gesture3.LongPress().enabled(flag5).minDuration(tmp2(8759).CONTEXT_MENU_LONG_PRESS_DURATION_MS);
      const Gesture4 = tmp2(6726).Gesture;
      return Gesture4.Simultaneous(result.onStart(fn5), onEndResult);
    } else {
      const Gesture = tmp2(6726).Gesture;
      if (flag2) {
        ...
        const obj5 = { runOnJS: tmp2(4343).runOnJS, triggerHapticFeedback: tmp2(4574).triggerHapticFeedback, CONTEXT_MENU_OPEN_HAPTIC: tmp2(8759).CONTEXT_MENU_OPEN_HAPTIC, measureButtonAndShowMenu: callback3 };
        fn.__closure = obj5;
        ...
        const obj6 = { updateContextMenuState: tmp2(8758).updateContextMenuState, state: tmp };
        fn2.__closure = obj6;
  ...
  const callback5 = obj3.useCallback(() => {
    ...
    if (isAndroidResult) {
      isAndroidResult = tmp(5013).getIsScreenReaderEnabled();
      const tmpResult = tmp(5013);
    }
    if (isAndroidResult) {
