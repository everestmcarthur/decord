...
// Dependencies: [19, 17, 13124, 21, 3, 1396, 13125, 13126, 8907, 5639, 4343, 5026, 1256, 8544, 558, 2]

// Module 13123 (CutoutableAvatarImage)
...
import getChannelIcon from "getChannelIcon" /* 13125 */;
import getReactNativeSVGImageSourceDefault from "getReactNativeSVGImageSource" /* 13126 */;
import noop from "module_19" /* 19 */;

const ClipViewDefault = ClipView;

const inlineStyles = Circle(8544);
const inlineStylesDefault = tmp9(8544);
require = fn;
...
function AnimatedNativeCutoutAvatarImage(cutout) {
  ...
  const fn = function h() {
    ...
    if (cutout.shape === ClipView.CutoutShape.RoundedRect) {
      const size1 = { shape: tmp(8907).CutoutShape.RoundedRect, x: tmp(5026).withSpring(size.x, CHANNEL_SPRING_CONFIG), y: null, width: null, height: null, cornerRadius: null };
      const tmpResult = tmp(5026);
      size1.y = tmp(5026).withSpring(size.y, CHANNEL_SPRING_CONFIG);
      const tmpResult8 = tmp(5026);
      size1.width = tmp(5026).withSpring(size.width, CHANNEL_SPRING_CONFIG);
      const tmpResult9 = tmp(5026);
      size1.height = tmp(5026).withSpring(size.height, CHANNEL_SPRING_CONFIG);
      const tmpResult10 = tmp(5026);
      size1.cornerRadius = tmp(5026).withSpring(size.cornerRadius, CHANNEL_SPRING_CONFIG);
      let point = size1;
      const tmpResult11 = tmp(5026);
    } else {
      point = { shape: tmp(8907).CutoutShape.Circle, x: tmp(5026).withSpring(size.x, CHANNEL_SPRING_CONFIG), y: null, size: null };
      const tmpResult12 = tmp(5026);
      point.y = tmp(5026).withSpring(size.y, CHANNEL_SPRING_CONFIG);
      const tmpResult13 = tmp(5026);
      point.size = tmp(5026).withSpring(size.size, CHANNEL_SPRING_CONFIG);
      const tmpResult14 = tmp(5026);
    }
  ...
  obj2 = { cutout, CutoutShape: cutout(8907).CutoutShape, withSpring: cutout(5026).withSpring, CHANNEL_SPRING_CONFIG };
  fn.__closure = obj2;
  fn.__workletHash = 12529564164821;
  fn.__initData = __initData;
  const animatedProps = cutout(4343).useAnimatedProps(fn);
  let obj = cutout(4343);
  ...
  return closure_7(cutout(8907).ClipViewAnimated, obj3);
}
...
const CHANNEL_SPRING_CONFIG = fn(13124).CHANNEL_SPRING_CONFIG;
const jsxProd = fn(21);
