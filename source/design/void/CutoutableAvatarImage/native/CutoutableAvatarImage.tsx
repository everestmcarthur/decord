...
// Dependencies: [19, 17, 13176, 21, 3, 1397, 13177, 13178, 8954, 5673, 4374, 5059, 1255, 8586, 558, 2]

// Module 13175 (CutoutableAvatarImage)
...
import getChannelIcon from "getChannelIcon" /* 13177 */;
import getReactNativeSVGImageSourceDefault from "getReactNativeSVGImageSource" /* 13178 */;
import noop from "module_19" /* 19 */;

const ClipViewDefault = ClipView;

const inlineStyles = Circle(8586);
const inlineStylesDefault = tmp9(8586);
require = fn;
...
function AnimatedNativeCutoutAvatarImage(cutout) {
  ...
  const fn = function h() {
    ...
    if (cutout.shape === ClipView.CutoutShape.RoundedRect) {
      const size1 = { shape: tmp(8954).CutoutShape.RoundedRect, x: tmp(5059).withSpring(size.x, CHANNEL_SPRING_CONFIG), y: null, width: null, height: null, cornerRadius: null };
      const tmpResult = tmp(5059);
      size1.y = tmp(5059).withSpring(size.y, CHANNEL_SPRING_CONFIG);
      const tmpResult8 = tmp(5059);
      size1.width = tmp(5059).withSpring(size.width, CHANNEL_SPRING_CONFIG);
      const tmpResult9 = tmp(5059);
      size1.height = tmp(5059).withSpring(size.height, CHANNEL_SPRING_CONFIG);
      const tmpResult10 = tmp(5059);
      size1.cornerRadius = tmp(5059).withSpring(size.cornerRadius, CHANNEL_SPRING_CONFIG);
      let point = size1;
      const tmpResult11 = tmp(5059);
    } else {
      point = { shape: tmp(8954).CutoutShape.Circle, x: tmp(5059).withSpring(size.x, CHANNEL_SPRING_CONFIG), y: null, size: null };
      const tmpResult12 = tmp(5059);
      point.y = tmp(5059).withSpring(size.y, CHANNEL_SPRING_CONFIG);
      const tmpResult13 = tmp(5059);
      point.size = tmp(5059).withSpring(size.size, CHANNEL_SPRING_CONFIG);
      const tmpResult14 = tmp(5059);
    }
  ...
  obj2 = { cutout, CutoutShape: cutout(8954).CutoutShape, withSpring: cutout(5059).withSpring, CHANNEL_SPRING_CONFIG };
  fn.__closure = obj2;
  fn.__workletHash = 12529564164821;
  fn.__initData = __initData;
  const animatedProps = cutout(4374).useAnimatedProps(fn);
  let obj = cutout(4374);
  ...
  return closure_7(cutout(8954).ClipViewAnimated, obj3);
}
...
const CHANNEL_SPRING_CONFIG = fn(13176).CHANNEL_SPRING_CONFIG;
const jsxProd = fn(21);
