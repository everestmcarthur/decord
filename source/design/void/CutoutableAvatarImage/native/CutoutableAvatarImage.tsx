...
// Dependencies: [19, 17, 13184, 21, 3, 1397, 13185, 13186, 8970, 5675, 4376, 5061, 1255, 8606, 558, 2]

// Module 13183 (CutoutableAvatarImage)
...
import getChannelIcon from "getChannelIcon" /* 13185 */;
import getReactNativeSVGImageSourceDefault from "getReactNativeSVGImageSource" /* 13186 */;
import noop from "module_19" /* 19 */;

const ClipViewDefault = ClipView;

const inlineStyles = Circle(8606);
const inlineStylesDefault = tmp9(8606);
require = fn;
...
function AnimatedNativeCutoutAvatarImage(cutout) {
  ...
  const fn = function h() {
    ...
    if (cutout.shape === ClipView.CutoutShape.RoundedRect) {
      const size1 = { shape: tmp(8970).CutoutShape.RoundedRect, x: tmp(5061).withSpring(size.x, CHANNEL_SPRING_CONFIG), y: null, width: null, height: null, cornerRadius: null };
      const tmpResult = tmp(5061);
      size1.y = tmp(5061).withSpring(size.y, CHANNEL_SPRING_CONFIG);
      const tmpResult8 = tmp(5061);
      size1.width = tmp(5061).withSpring(size.width, CHANNEL_SPRING_CONFIG);
      const tmpResult9 = tmp(5061);
      size1.height = tmp(5061).withSpring(size.height, CHANNEL_SPRING_CONFIG);
      const tmpResult10 = tmp(5061);
      size1.cornerRadius = tmp(5061).withSpring(size.cornerRadius, CHANNEL_SPRING_CONFIG);
      let point = size1;
      const tmpResult11 = tmp(5061);
    } else {
      point = { shape: tmp(8970).CutoutShape.Circle, x: tmp(5061).withSpring(size.x, CHANNEL_SPRING_CONFIG), y: null, size: null };
      const tmpResult12 = tmp(5061);
      point.y = tmp(5061).withSpring(size.y, CHANNEL_SPRING_CONFIG);
      const tmpResult13 = tmp(5061);
      point.size = tmp(5061).withSpring(size.size, CHANNEL_SPRING_CONFIG);
      const tmpResult14 = tmp(5061);
    }
  ...
  obj2 = { cutout, CutoutShape: cutout(8970).CutoutShape, withSpring: cutout(5061).withSpring, CHANNEL_SPRING_CONFIG };
  fn.__closure = obj2;
  fn.__workletHash = 12529564164821;
  fn.__initData = __initData;
  const animatedProps = cutout(4376).useAnimatedProps(fn);
  let obj = cutout(4376);
  ...
  return closure_7(cutout(8970).ClipViewAnimated, obj3);
}
...
const CHANNEL_SPRING_CONFIG = fn(13184).CHANNEL_SPRING_CONFIG;
const jsxProd = fn(21);
