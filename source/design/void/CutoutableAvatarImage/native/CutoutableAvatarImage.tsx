...
// Dependencies: [19, 17, 13165, 21, 3, 1397, 13166, 13167, 8943, 5672, 4376, 5058, 1255, 8577, 558, 2]

// Module 13164 (CutoutableAvatarImage)
...
import getChannelIcon from "getChannelIcon" /* 13166 */;
import getReactNativeSVGImageSourceDefault from "getReactNativeSVGImageSource" /* 13167 */;
import noop from "module_19" /* 19 */;

const ClipViewDefault = ClipView;

const inlineStyles = Circle(8577);
const inlineStylesDefault = tmp9(8577);
require = fn;
...
function AnimatedNativeCutoutAvatarImage(cutout) {
  ...
  const fn = function h() {
    ...
    if (cutout.shape === ClipView.CutoutShape.RoundedRect) {
      const size1 = { shape: tmp(8943).CutoutShape.RoundedRect, x: tmp(5058).withSpring(size.x, CHANNEL_SPRING_CONFIG), y: null, width: null, height: null, cornerRadius: null };
      const tmpResult = tmp(5058);
      size1.y = tmp(5058).withSpring(size.y, CHANNEL_SPRING_CONFIG);
      const tmpResult8 = tmp(5058);
      size1.width = tmp(5058).withSpring(size.width, CHANNEL_SPRING_CONFIG);
      const tmpResult9 = tmp(5058);
      size1.height = tmp(5058).withSpring(size.height, CHANNEL_SPRING_CONFIG);
      const tmpResult10 = tmp(5058);
      size1.cornerRadius = tmp(5058).withSpring(size.cornerRadius, CHANNEL_SPRING_CONFIG);
      let point = size1;
      const tmpResult11 = tmp(5058);
    } else {
      point = { shape: tmp(8943).CutoutShape.Circle, x: tmp(5058).withSpring(size.x, CHANNEL_SPRING_CONFIG), y: null, size: null };
      const tmpResult12 = tmp(5058);
      point.y = tmp(5058).withSpring(size.y, CHANNEL_SPRING_CONFIG);
      const tmpResult13 = tmp(5058);
      point.size = tmp(5058).withSpring(size.size, CHANNEL_SPRING_CONFIG);
      const tmpResult14 = tmp(5058);
    }
  ...
  obj2 = { cutout, CutoutShape: cutout(8943).CutoutShape, withSpring: cutout(5058).withSpring, CHANNEL_SPRING_CONFIG };
  fn.__closure = obj2;
  ...
  return closure_7(cutout(8943).ClipViewAnimated, obj3);
}
...
const CHANNEL_SPRING_CONFIG = fn(13165).CHANNEL_SPRING_CONFIG;
const jsxProd = fn(21);
