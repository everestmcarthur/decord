...
// Function ID: 4257
// Name: MessageRecord
// Dependencies: [1386, 1074, 1384, 4257, 7376, 7836, 9337, 2]
...
prototype["addReaction"] = function addReaction(emoji) {
  ...
  const mapped = reactions.map((emoji, index) => {
    ...
    if (obj.emojiEquals(emoji.emoji, closure_0)) {
      ...
      if (NORMAL === tmp(7836).ReactionTypes.BURST) {
        if (flag) {
          if (emoji.me_burst) {
            return emoji;
          }
        ...
        burst_colors = colors;
        tmp28 = flag;
      } else if (tmp12 === tmp(7836).ReactionTypes.VOTE) {
...
prototype["removeReaction"] = function removeReaction(arg0) {
  ...
  const mapped = reactions.map((emoji, index) => {
    ...
    if (!obj.emojiEquals(emoji.emoji, closure_0)) {
      ...
      if (NORMAL === tmp2(7836).ReactionTypes.BURST) {
        if (flag) {
          if (!merged1.me_burst) {
      ...
      } else if (tmp4 !== tmp2(7836).ReactionTypes.VOTE) {
        if (flag) {
          if (!merged1.me) {
