...
// Dependencies: [41, 42, 93, 95, 98, 10507, 10442, 10444]

// Module 10512 (_isNativeReflectConstruct)
...
function createTimeComponents(createParsingComponents) {
  ...
  let parsed = parseInt(JPTimeExpressionParser(10507).toHankaku(arg1));
  if (isNaN(parsed)) {
    parsed = tmp(10507).jaStringToNumber(arg1);
  }
  if (parsed > 24) {
    return null;
  } else {
    if (arg2) {
      ...
      if ("\u534A" !== arg2) {
        ...
        const parsed1 = parseInt(tmp(10507).toHankaku(arg2));
        const _isNaN = isNaN;
        num = parsed1;
        if (isNaN(parsed1)) {
          num = tmp(10507).jaStringToNumber(arg2);
        }
      }
      if (num >= 60) {
        return null;
    ...
    if (arg3) {
      ...
      let parsed2 = parseInt(tmp(10507).toHankaku(arg3));
      const _isNaN2 = isNaN;
      if (isNaN(parsed2)) {
        parsed2 = tmp(10507).jaStringToNumber(arg3);
      }
      if (parsed2 >= 60) {
        return null;
      } else {
        ...
        if ("\u5348\u524D" !== arg4) {
          if ("a" !== 41.toLowerCase()) {
            ...
            if (!tmp8) {
              ...
              num6 = sum;
              num5 = tmp(10442).Meridiem.PM;
            }
          }
        }
        const AM = tmp(10442).Meridiem.AM;
        num5 = AM;
        num6 = parsed;
        if (12 === parsed) {
          num6 = 0;
    ...
    if (num5 >= 0) {
      ...
      parsingComponents.imply("meridiem", tmp(10442).Meridiem.AM);
    } else {
      parsingComponents.imply("meridiem", tmp(10442).Meridiem.PM);
    }
