...
// Dependencies: [41, 42, 93, 95, 98, 9146, 9147]
// Exports: Chi, Maj

// Module 9152 (_isNativeReflectConstruct)
...
let items = [
  {
    ...
    value: function update(B) {
      let tmp8;
      const self = this;
      HashMD(9147).exists(this);
      ({ buffer, blockLen, view } = this);
      const toBytesResult = HashMD(9146).toBytes(B);
      let num = 0;
  ...
  },
  {
    ...
    value: function digestInto(content) {
      const self = this;
      HashMD(9147).exists(this);
      HashMD(9147).output(content, this);
      this.finished = true;
      ...
      self.process(view, 0);
      view = HashMD(9146).createView(content);
