...
// Function ID: 9174
// Name: _asyncLoop
// Dependencies: [41, 42, 5, 9174, 9175]
...
export const concatBytes = function concatBytes() {
  ...
  if (0 < items.length) {
    do {
      let arr2 = items[num];
      let bytesResult = Hash(9174).bytes(arr2);
...
export const randomBytes = function randomBytes(result) {
  ...
  if (Hash(9175).crypto) {
    if (typeof tmp(9175).crypto.getRandomValues === "function") {
      const _crypto2 = tmp(9175).crypto;
      const _Uint8Array = Uint8Array;
  ...
  if (Hash(9175).crypto) {
    if (typeof tmp(9175).crypto.randomBytes === "function") {
      const _crypto = tmp(9175).crypto;
