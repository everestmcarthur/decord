...
// Function ID: 9296
// Name: _asyncLoop
// Dependencies: [41, 42, 5, 9296, 9297]
...
export const concatBytes = function concatBytes() {
  ...
  if (0 < items.length) {
    do {
      let arr2 = items[num];
      let bytesResult = Hash(9296).bytes(arr2);
...
export const randomBytes = function randomBytes(result) {
  ...
  if (Hash(9297).crypto) {
    if (typeof tmp(9297).crypto.getRandomValues === "function") {
      const _crypto2 = tmp(9297).crypto;
      const _Uint8Array = Uint8Array;
  ...
  if (Hash(9297).crypto) {
    if (typeof tmp(9297).crypto.randomBytes === "function") {
      const _crypto = tmp(9297).crypto;
