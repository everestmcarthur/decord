...
// Module 1070 (_isNativeReflectConstruct)
...
class FeedbackWidget {
  constructor(arg0) {
        ...
        if (!obj.props.isEmailRequired) {
          if (trimmed2) {
            if (obj.props.shouldValidateEmail) {
              if (obj.props.isEmailRequired) {
                let obj2 = FeedbackWidget(1061);
                if (!obj2.isValidEmail(trimmed1)) {
                  FeedbackWidget(1061).feedbackAlertDialog(props2.errorTitle, props2.emailError);
                  const obj4 = FeedbackWidget(1061);
                }
              }
            }
            ...
            obj[1] = trimmed;
            obj[2] = trimmed1;
            obj[3] = FeedbackWidget(682).lastEventId();
            try {
              if (!onFormSubmitted) {
              ...
              FeedbackWidget(682).captureFeedback(obj, tmp19);
              ...
              obj2[3] = tmp13;
              tmp4(obj2);
              const obj8 = FeedbackWidget(682);
              FeedbackWidget(1061).feedbackAlertDialog(props2.successMessageText, "");
              onFormSubmitted();
              obj._didSubmitForm = true;
              const obj11 = FeedbackWidget(1061);
            } catch (tmp26) {
              const _Error = Error;
              const _HermesInternal = HermesInternal;
              error = new Error("Feedback form submission failed: " + tmp26);
              tmp3(error);
              FeedbackWidget(1061).feedbackAlertDialog(tmp2.errorTitle, tmp2.genericError);
              const debug = FeedbackWidget(682).debug;
              const _HermesInternal2 = HermesInternal;
              debug.error("Feedback form submission failed: " + tmp26);
              const obj12 = FeedbackWidget(1061);
            }
            const obj7 = FeedbackWidget(682);
          }
        }
      }
      obj1 = FeedbackWidget(1061);
    ...
    tmp4Result._getUser = () => {
      const currentScope = lib(682).getCurrentScope();
    ...
    tmp4Result._showImageRetrievalDevelopmentNote = () => {
      if (obj.isExpoGo()) {
        lib(1061).feedbackAlertDialog("Development note", "The feedback widget cannot retrieve image data in Expo Go. Please build your app to test this functionality.");
        const tmpResult = lib(1061);
      }
    };
