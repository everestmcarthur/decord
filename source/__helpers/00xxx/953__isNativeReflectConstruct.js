...
// Module 953 (_isNativeReflectConstruct)
import registerSpanErrorInstrumentation from "registerSpanErrorInstrumentation" /* 682 */;
import addClsInstrumentationHandler from "addClsInstrumentationHandler" /* 898 */;
...
function addNetworkBreadcrumb(isEnabled, name) {
  ...
  if (isEnabledResult) {
    ...
    if (isSentryRequestUrlResult) {
      obj = _require(682);
      isSentryRequestUrlResult = obj.isSentryRequestUrl(name.name, _require(682).getClient());
      obj2 = _require(682);
    }
    if (!isSentryRequestUrlResult) {
      isEnabled.addUpdate(() => {
        ...
        try {
          ...
          if (0 === c5) {
            ...
            if (arg0 === 1) {
              ...
              tmp = callback;
              tmp = dependencyMap;
              const obj18 = callback(682);
              const currentScope = callback(682).getCurrentScope();
              transport = undefined;
              if (client != null) {
