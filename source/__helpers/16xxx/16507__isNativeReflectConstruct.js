...
// Dependencies: [41, 42, 93, 95, 98, 19, 17, 21, 16508]
// Exports: captureScreen, ensureModuleIsLoaded

// Module 16507 (_isNativeReflectConstruct)
import noopDefault from "noop" /* 19 */;
import _inheritsDefault from "_inherits" /* 98 */;
import get_ActivityIndicatorDefault from "get ActivityIndicator" /* 16508 */;
...
function captureRef(obj, options) {
  if (ViewShot(16508)) {
    ...
    return tmp(16508).captureRef(tmp15, options);
  } else {
    const _Error = Error;
...
function releaseCapture(str) {
  if (typeof str === "string") {
    ViewShot(16508).releaseCapture(str);
    const obj = ViewShot(16508);
  }
}
noopDefault;
...
export const captureScreen = function captureScreen(options) {
  if (ViewShot(16508)) {
    ...
    return tmp(16508).captureScreen(options);
  } else {
    const _Error = Error;
    error = new Error("react-native-view-shot: NativeModules.RNViewShot is undefined. Make sure the library is linked on the native side.");
    throw error;
