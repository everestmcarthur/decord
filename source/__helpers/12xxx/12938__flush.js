...
// Function ID: 12939
// Name: _flush
// Dependencies: [5, 12917, 12939, 12918, 12890, 12901, 12904, 12897, 12927, 12891, 12913]
  ...
  if (c1 === 2) {
    ...
    try {
      ...
      if (arg0 === 1) {
        ...
        const client = require("module_12917").getClient();
        if (client) {
          client.flush(tmp13);
        } else {
        ...
        c1 = 3;
        const obj3 = require("module_12917");
        tmp13 = closure_0;
      }
    } catch (tmp8) {
      c1 = tmp;
...
function endSession() {
  ...
  if (tmp3) {
    tmp(12913).closeSession(tmp3);
    const tmpResult = tmp(12913);
  }
  const isolationScope1 = _mod12917.getIsolationScope();
  const tmpResult4 = _mod12917;
  const currentScope1 = _mod12917.getCurrentScope();
  const tmpResult5 = _mod12917;
...
export const captureException = function captureException(arg0, arg1) {
  const currentScope = _mod12917.getCurrentScope();
  return currentScope.captureException(arg0, _mod12939.parseEventHintOrCaptureContext(arg1));
};
...
export const startSession = function startSession(arg0) {
  ...
  if (undefined === environment) {
    environment = tmp(12927).DEFAULT_ENVIRONMENT;
  }
  const userAgent = _mod12891.GLOBAL_OBJ.navigator || {}.userAgent;
  const tmp3 = _mod12891.GLOBAL_OBJ.navigator || {};
  const obj2 = { release, environment, user: null };
  const tmpResult = _mod12913;
  ...
  if (tmp10) {
    tmp(12913).updateSession(session1, { status: "exited" });
    const tmpResult2 = tmp(12913);
  }
  endSession();
...
export const withMonitor = function withMonitor(monitorSlug, arg1, arg2) {
  ...
  function finishCheckIn(status) {
    const obj = { monitorSlug, status, checkInId: captureCheckInResult, duration: _mod12904.timestampInSeconds() - closure_3 };
    const currentScope = _mod12917.getCurrentScope();
    const client = _mod12917.getClient();
    if (client) {
      if (client.captureCheckIn) {
  ...
  let currentScope = require("module_12917").getCurrentScope();
  let obj = { monitorSlug, status: "in_progress" };
  let obj2 = require("module_12917");
  let client = require("module_12917").getClient();
  if (client) {
    if (client.captureCheckIn) {
      let captureCheckInResult = client.captureCheckIn(obj, arg2, currentScope);
    } else if (tmp(12918).DEBUG_BUILD) {
      let logger2 = tmp(12890).logger;
      logger2.warn("Cannot capture check-in. Client does not support sending check-ins.");
    }
    asyncGeneratorStep = captureCheckInResult;
    closure_3 = tmp(12904).timestampInSeconds();
    const tmpResult = tmp(12904);
    return tmp(12917).withIsolationScope(() => {
      try {
    ...
    let logger = tmp(12890).logger;
    logger.warn("Cannot capture check-in. No client defined.");
  }
  let obj3 = require("module_12917");
  captureCheckInResult = require("module_12901").uuid4();
};
