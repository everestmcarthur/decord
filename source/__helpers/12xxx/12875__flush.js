...
// Function ID: 12876
// Name: _flush
// Dependencies: [5, 12854, 12876, 12855, 12827, 12838, 12841, 12834, 12864, 12828, 12850]
  ...
  if (c1 === 2) {
    ...
    try {
      ...
      if (arg0 === 1) {
        ...
        const client = require("module_12854").getClient();
        if (client) {
          client.flush(tmp13);
        } else {
        ...
        c1 = 3;
        const obj3 = require("module_12854");
        tmp13 = closure_0;
      }
    } catch (tmp8) {
      c1 = tmp;
...
function endSession() {
  ...
  if (tmp3) {
    tmp(12850).closeSession(tmp3);
    const tmpResult = tmp(12850);
  }
  const isolationScope1 = _mod12854.getIsolationScope();
  const tmpResult4 = _mod12854;
  const currentScope1 = _mod12854.getCurrentScope();
  const tmpResult5 = _mod12854;
...
export const captureException = function captureException(arg0, arg1) {
  const currentScope = _mod12854.getCurrentScope();
  return currentScope.captureException(arg0, _mod12876.parseEventHintOrCaptureContext(arg1));
};
...
export const startSession = function startSession(arg0) {
  ...
  if (undefined === environment) {
    environment = tmp(12864).DEFAULT_ENVIRONMENT;
  }
  ...
  if (tmp10) {
    tmp(12850).updateSession(session1, { status: "exited" });
    const tmpResult2 = tmp(12850);
  }
  endSession();
...
export const withMonitor = function withMonitor(monitorSlug, arg1, arg2) {
  ...
  function finishCheckIn(status) {
    const obj = { monitorSlug, status, checkInId: captureCheckInResult, duration: _mod12841.timestampInSeconds() - closure_3 };
    const currentScope = _mod12854.getCurrentScope();
    const client = _mod12854.getClient();
    if (client) {
      if (client.captureCheckIn) {
  ...
  let currentScope = require("module_12854").getCurrentScope();
  let obj = { monitorSlug, status: "in_progress" };
  let obj2 = require("module_12854");
  let client = require("module_12854").getClient();
  if (client) {
    if (client.captureCheckIn) {
    ...
    closure_3 = tmp(12841).timestampInSeconds();
    const tmpResult = tmp(12841);
    return tmp(12854).withIsolationScope(() => {
      try {
  ...
  let obj3 = require("module_12854");
  captureCheckInResult = require("module_12838").uuid4();
};
