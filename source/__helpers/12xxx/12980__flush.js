...
// Function ID: 12981
// Name: _flush
// Dependencies: [5, 12959, 12981, 12960, 12932, 12943, 12946, 12939, 12969, 12933, 12955]
  ...
  if (c1 === 2) {
    ...
    try {
      ...
      if (arg0 === 1) {
        ...
        c1 = 3;
        const obj3 = require("module_12959");
        tmp13 = closure_0;
      }
    } catch (tmp8) {
      c1 = tmp;
...
function endSession() {
  ...
  if (tmp3) {
    tmp(12955).closeSession(tmp3);
    const tmpResult = tmp(12955);
  }
  const isolationScope1 = _mod12959.getIsolationScope();
  const tmpResult4 = _mod12959;
  const currentScope1 = _mod12959.getCurrentScope();
  const tmpResult5 = _mod12959;
...
export const captureException = function captureException(arg0, arg1) {
  const currentScope = _mod12959.getCurrentScope();
  return currentScope.captureException(arg0, _mod12981.parseEventHintOrCaptureContext(arg1));
};
...
export const startSession = function startSession(arg0) {
  ...
  if (undefined === environment) {
    environment = tmp(12969).DEFAULT_ENVIRONMENT;
  }
  const userAgent = _mod12933.GLOBAL_OBJ.navigator || {}.userAgent;
  const tmp3 = _mod12933.GLOBAL_OBJ.navigator || {};
  const obj2 = { release, environment, user: null };
  const tmpResult = _mod12955;
  ...
  if (tmp10) {
    tmp(12955).updateSession(session1, { status: "exited" });
...
export const withMonitor = function withMonitor(monitorSlug, arg1, arg2) {
  ...
  function finishCheckIn(status) {
    const obj = { monitorSlug, status, checkInId: captureCheckInResult, duration: _mod12946.timestampInSeconds() - closure_3 };
    const currentScope = _mod12959.getCurrentScope();
    const client = _mod12959.getClient();
    if (client) {
      if (client.captureCheckIn) {
  ...
  let currentScope = require("module_12959").getCurrentScope();
  let obj = { monitorSlug, status: "in_progress" };
  let obj2 = require("module_12959");
  let client = require("module_12959").getClient();
  if (client) {
    if (client.captureCheckIn) {
      let captureCheckInResult = client.captureCheckIn(obj, arg2, currentScope);
    } else if (tmp(12960).DEBUG_BUILD) {
      let logger2 = tmp(12932).logger;
      logger2.warn("Cannot capture check-in. Client does not support sending check-ins.");
    }
    asyncGeneratorStep = captureCheckInResult;
    closure_3 = tmp(12946).timestampInSeconds();
    const tmpResult = tmp(12946);
    return tmp(12959).withIsolationScope(() => {
      try {
    ...
    let logger = tmp(12932).logger;
    logger.warn("Cannot capture check-in. No client defined.");
  }
  let obj3 = require("module_12959");
  captureCheckInResult = require("module_12943").uuid4();
};
