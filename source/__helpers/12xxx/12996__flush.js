...
// Function ID: 12997
// Name: _flush
// Dependencies: [5, 12975, 12997, 12976, 12948, 12959, 12962, 12955, 12985, 12949, 12971]
  ...
  if (c1 === 2) {
    ...
    try {
      ...
      if (arg0 === 1) {
        ...
        const client = require("module_12975").getClient();
        if (client) {
          client.flush(tmp13);
        } else {
        ...
        c1 = 3;
        const obj3 = require("module_12975");
        tmp13 = closure_0;
      }
    } catch (tmp8) {
      c1 = tmp;
...
function endSession() {
  ...
  if (tmp3) {
    tmp(12971).closeSession(tmp3);
    const tmpResult = tmp(12971);
  }
  const isolationScope1 = _mod12975.getIsolationScope();
  const tmpResult4 = _mod12975;
  const currentScope1 = _mod12975.getCurrentScope();
  const tmpResult5 = _mod12975;
...
export const captureException = function captureException(arg0, arg1) {
  const currentScope = _mod12975.getCurrentScope();
  return currentScope.captureException(arg0, _mod12997.parseEventHintOrCaptureContext(arg1));
};
...
export const startSession = function startSession(arg0) {
  ...
  if (undefined === environment) {
    environment = tmp(12985).DEFAULT_ENVIRONMENT;
  }
  const userAgent = _mod12949.GLOBAL_OBJ.navigator || {}.userAgent;
  const tmp3 = _mod12949.GLOBAL_OBJ.navigator || {};
  ...
  if (tmp10) {
    tmp(12971).updateSession(session1, { status: "exited" });
    const tmpResult2 = tmp(12971);
  }
  endSession();
...
export const withMonitor = function withMonitor(monitorSlug, arg1, arg2) {
  ...
  function finishCheckIn(status) {
    const obj = { monitorSlug, status, checkInId: captureCheckInResult, duration: _mod12962.timestampInSeconds() - closure_3 };
    const currentScope = _mod12975.getCurrentScope();
    const client = _mod12975.getClient();
    if (client) {
      if (client.captureCheckIn) {
  ...
  let currentScope = require("module_12975").getCurrentScope();
  let obj = { monitorSlug, status: "in_progress" };
  let obj2 = require("module_12975");
  let client = require("module_12975").getClient();
  if (client) {
    if (client.captureCheckIn) {
      let captureCheckInResult = client.captureCheckIn(obj, arg2, currentScope);
    } else if (tmp(12976).DEBUG_BUILD) {
      let logger2 = tmp(12948).logger;
      logger2.warn("Cannot capture check-in. Client does not support sending check-ins.");
    }
    asyncGeneratorStep = captureCheckInResult;
    closure_3 = tmp(12962).timestampInSeconds();
    const tmpResult = tmp(12962);
    return tmp(12975).withIsolationScope(() => {
      try {
    ...
    let logger = tmp(12948).logger;
    logger.warn("Cannot capture check-in. No client defined.");
  }
  let obj3 = require("module_12975");
  captureCheckInResult = require("module_12959").uuid4();
};
