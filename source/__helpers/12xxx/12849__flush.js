...
import applyClientOptions from "applyClientOptions" /* 12850 */;
...
export const withMonitor = function withMonitor(monitorSlug) {
  ...
  function finishCheckIn(status) {
    const obj = { monitorSlug, status, checkInId: closure_2, duration: monitorSlug(12815).timestampInSeconds() - closure_3 };
    const obj2 = monitorSlug(12815);
    const currentScope = monitorSlug(12828).getCurrentScope();
    const obj3 = monitorSlug(12828);
    const client = monitorSlug(12828).getClient();
    if (client) {
      if (client.captureCheckIn) {
    ...
    const obj4 = monitorSlug(12828);
    monitorSlug(12812).uuid4();
  }
  let currentScope = _require(12828).getCurrentScope();
  let obj = { monitorSlug, status: "in_progress" };
  let obj2 = _require(12828);
  let client = _require(12828).getClient();
  if (client) {
    if (client.captureCheckIn) {
  ...
  let obj3 = _require(12828);
  captureCheckInResult = _require(12812).uuid4();
};
