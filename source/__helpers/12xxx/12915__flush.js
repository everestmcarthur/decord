...
// Function ID: 12916
// Name: _flush
// Dependencies: [5, 12894, 12916, 12895, 12867, 12878, 12881, 12874, 12904, 12868, 12890]
  ...
  if (c1 === 2) {
    ...
    try {
      ...
      if (arg0 === 1) {
        ...
        c1 = 3;
        const obj3 = require("module_12894");
        tmp13 = closure_0;
      }
    } catch (tmp8) {
      c1 = tmp;
...
function endSession() {
  ...
  if (tmp3) {
    tmp(12890).closeSession(tmp3);
    const tmpResult = tmp(12890);
  }
  const isolationScope1 = _mod12894.getIsolationScope();
  const tmpResult4 = _mod12894;
  const currentScope1 = _mod12894.getCurrentScope();
  const tmpResult5 = _mod12894;
...
export const captureException = function captureException(arg0, arg1) {
  const currentScope = _mod12894.getCurrentScope();
  return currentScope.captureException(arg0, _mod12916.parseEventHintOrCaptureContext(arg1));
};
...
export const startSession = function startSession(arg0) {
  ...
  if (undefined === environment) {
    environment = tmp(12904).DEFAULT_ENVIRONMENT;
  }
  const userAgent = _mod12868.GLOBAL_OBJ.navigator || {}.userAgent;
  const tmp3 = _mod12868.GLOBAL_OBJ.navigator || {};
  const obj2 = { release, environment, user: null };
  const tmpResult = _mod12890;
  ...
  if (tmp10) {
    tmp(12890).updateSession(session1, { status: "exited" });
...
export const withMonitor = function withMonitor(monitorSlug, arg1, arg2) {
  ...
  function finishCheckIn(status) {
    const obj = { monitorSlug, status, checkInId: captureCheckInResult, duration: _mod12881.timestampInSeconds() - closure_3 };
    const currentScope = _mod12894.getCurrentScope();
    const client = _mod12894.getClient();
    if (client) {
      if (client.captureCheckIn) {
  ...
  let currentScope = require("module_12894").getCurrentScope();
  let obj = { monitorSlug, status: "in_progress" };
  let obj2 = require("module_12894");
  let client = require("module_12894").getClient();
  if (client) {
    if (client.captureCheckIn) {
      let captureCheckInResult = client.captureCheckIn(obj, arg2, currentScope);
    } else if (tmp(12895).DEBUG_BUILD) {
      let logger2 = tmp(12867).logger;
      logger2.warn("Cannot capture check-in. Client does not support sending check-ins.");
    }
    asyncGeneratorStep = captureCheckInResult;
    closure_3 = tmp(12881).timestampInSeconds();
    const tmpResult = tmp(12881);
    return tmp(12894).withIsolationScope(() => {
      try {
    ...
    let logger = tmp(12867).logger;
    logger.warn("Cannot capture check-in. No client defined.");
  }
  let obj3 = require("module_12894");
  captureCheckInResult = require("module_12878").uuid4();
};
