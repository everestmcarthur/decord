...
// Function ID: 12975
// Name: _flush
// Dependencies: [5, 12953, 12975, 12954, 12926, 12937, 12940, 12933, 12963, 12927, 12949]
...
function endSession() {
  ...
  if (tmp3) {
    tmp(12949).closeSession(tmp3);
    const tmpResult = tmp(12949);
  }
  const isolationScope1 = _mod12953.getIsolationScope();
  const tmpResult4 = _mod12953;
  const currentScope1 = _mod12953.getCurrentScope();
  const tmpResult5 = _mod12953;
...
export const captureException = function captureException(arg0, arg1) {
  const currentScope = _mod12953.getCurrentScope();
  return currentScope.captureException(arg0, _mod12975.parseEventHintOrCaptureContext(arg1));
};
...
export const startSession = function startSession(arg0) {
  ...
  if (undefined === environment) {
    environment = tmp(12963).DEFAULT_ENVIRONMENT;
  }
  ...
  if (tmp10) {
    tmp(12949).updateSession(session1, { status: "exited" });
    const tmpResult2 = tmp(12949);
  }
  endSession();
...
export const withMonitor = function withMonitor(monitorSlug, arg1, arg2) {
  ...
  if (client) {
    ...
    closure_3 = tmp(12940).timestampInSeconds();
    const tmpResult = tmp(12940);
    return tmp(12953).withIsolationScope(() => {
      try {
