...
// Function ID: 12976
// Name: _flush
// Dependencies: [5, 12954, 12976, 12955, 12927, 12938, 12941, 12934, 12964, 12928, 12950]
  ...
  if (c1 === 2) {
    ...
    try {
      ...
      if (arg0 === 1) {
        ...
        const client = require("module_12954").getClient();
        if (client) {
          client.flush(tmp13);
        } else {
        ...
        c1 = 3;
        const obj3 = require("module_12954");
        tmp13 = closure_0;
      }
    } catch (tmp8) {
      c1 = tmp;
...
function endSession() {
  ...
  if (tmp3) {
    tmp(12950).closeSession(tmp3);
    const tmpResult = tmp(12950);
  }
  const isolationScope1 = _mod12954.getIsolationScope();
  const tmpResult4 = _mod12954;
  const currentScope1 = _mod12954.getCurrentScope();
  const tmpResult5 = _mod12954;
...
export const captureException = function captureException(arg0, arg1) {
  const currentScope = _mod12954.getCurrentScope();
  return currentScope.captureException(arg0, _mod12976.parseEventHintOrCaptureContext(arg1));
};
...
export const startSession = function startSession(arg0) {
  ...
  if (undefined === environment) {
    environment = tmp(12964).DEFAULT_ENVIRONMENT;
  }
  const userAgent = _mod12928.GLOBAL_OBJ.navigator || {}.userAgent;
  const tmp3 = _mod12928.GLOBAL_OBJ.navigator || {};
  ...
  if (tmp10) {
    tmp(12950).updateSession(session1, { status: "exited" });
    const tmpResult2 = tmp(12950);
  }
  endSession();
...
export const withMonitor = function withMonitor(monitorSlug, arg1, arg2) {
  ...
  function finishCheckIn(status) {
    const obj = { monitorSlug, status, checkInId: captureCheckInResult, duration: _mod12941.timestampInSeconds() - closure_3 };
    const currentScope = _mod12954.getCurrentScope();
    const client = _mod12954.getClient();
    if (client) {
      if (client.captureCheckIn) {
  ...
  let currentScope = require("module_12954").getCurrentScope();
  let obj = { monitorSlug, status: "in_progress" };
  let obj2 = require("module_12954");
  let client = require("module_12954").getClient();
  if (client) {
    if (client.captureCheckIn) {
      let captureCheckInResult = client.captureCheckIn(obj, arg2, currentScope);
    } else if (tmp(12955).DEBUG_BUILD) {
      let logger2 = tmp(12927).logger;
      logger2.warn("Cannot capture check-in. Client does not support sending check-ins.");
    }
    asyncGeneratorStep = captureCheckInResult;
    closure_3 = tmp(12941).timestampInSeconds();
    const tmpResult = tmp(12941);
    return tmp(12954).withIsolationScope(() => {
      try {
    ...
    let logger = tmp(12927).logger;
    logger.warn("Cannot capture check-in. No client defined.");
  }
  let obj3 = require("module_12954");
  captureCheckInResult = require("module_12938").uuid4();
};
