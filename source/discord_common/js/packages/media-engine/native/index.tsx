...
// Function ID: 4631
// Name: destroy
// Dependencies: [32, 5, 4599, 4631, 4632, 4633, 4637, 4, 1910, 4629, 4638, 1341, 1116, 4640, 4700, 2]
...
import VideoDefault from "Video" /* 4633 */;
import CameraDefault from "Camera" /* 4637 */;
import ConnectionDefault from "Connection" /* 4640 */;
import Devices from "Devices" /* 4700 */;
import _slicedToArray from "module_32" /* 32 */;
import asyncGeneratorStep from "asyncGeneratorStep" /* 5 */;
import TypedEventEmitter from "TypedEventEmitter" /* 4632 */;

const pollConnectionStatsDefault = tmp10(4638);
require = fn;
    ...
    obj.handleRemoveListener = function handleRemoveListener(arg0) {
      if (arg0 === BaseConnectionEvent.MediaEngineEvent.VoiceActivity) {
        if (null != tmpResult.getVoiceEngine().setEmitVADLevel2) {
          ...
          voiceEngine.setEmitVADLevel2(obj.listenerCount(tmp(4629).MediaEngineEvent.VoiceActivity) > 0);
          const tmpResult3 = tmp(1910);
        } else {
          const voiceEngine1 = tmp(1910).getVoiceEngine();
          voiceEngine1.setEmitVADLevel(obj.listenerCount(tmp(4629).MediaEngineEvent.VoiceActivity) > 0, false, {});
...
prototype["connect"] = function connect(arg0, arg1, videoSupported) {
  ...
  obj3 = self(4640).create(arg0, arg1, videoSupported, flag);
  obj3.on(obj3(4629).BaseConnectionEvent.Destroy, (arg0) => {
  ...
  obj3.on(obj3(4629).BaseConnectionEvent.Connected, () => {
    obj3.setVideoBroadcast(self.shouldConnectionBroadcastVideo(obj3));
  });
  obj3.on(obj3(4629).BaseConnectionEvent.Silence, (arg0) => {
  ...
  self.emit(obj3(4629).MediaEngineEvent.Connection, obj3);
  return obj3;
};
...
prototype["setAudioInputDevice"] = function setAudioInputDevice(audioInputDeviceId) {
  ...
  if (obj.supportsFeature(constants6.SET_AUDIO_DEVICE_BY_ID)) {
    ...
    const audioInputDevices = tmp(4700).getAudioInputDevices();
...
prototype["setAudioOutputDevice"] = function setAudioOutputDevice(audioOutputDeviceId) {
  ...
  if (obj.supportsFeature(constants6.SET_AUDIO_DEVICE_BY_ID)) {
    ...
    const audioOutputDevices = tmp(4700).getAudioOutputDevices();
...
prototype["setLoopback"] = function setLoopback(arg0, arg1) {
  ...
  if (null != tmp2Result4.getVoiceEngine().setEmitVADLevel2) {
    const voiceEngine1 = tmp2(1910).getVoiceEngine();
    if (!tmp) {
      tmp = self.listenerCount(tmp2(4629).MediaEngineEvent.VoiceActivity) > 0;
    }
    ...
    let tmp6 = tmp;
    if (!tmp) {
      tmp6 = self.listenerCount(tmp2(4629).MediaEngineEvent.VoiceActivity) > 0;
    }
...
prototype["stopLocalAudioRecording"] = function stopLocalAudioRecording(arg0) {
  ...
  let tmp5 = this.listenerCount(tmp(4629).MediaEngineEvent.VoiceActivity) > 0;
  if (tmp5) {
