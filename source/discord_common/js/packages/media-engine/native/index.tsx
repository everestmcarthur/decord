...
// Function ID: 4696
// Name: destroy
// Dependencies: [32, 5, 4664, 4696, 4697, 4698, 4702, 4, 1911, 4694, 4703, 1340, 1365, 4705, 4765, 2]
...
import VideoDefault from "Video" /* 4698 */;
import CameraDefault from "Camera" /* 4702 */;
import ConnectionDefault from "Connection" /* 4705 */;
import Devices from "Devices" /* 4765 */;
import _slicedToArray from "module_32" /* 32 */;
import asyncGeneratorStep from "asyncGeneratorStep" /* 5 */;
import TypedEventEmitter from "TypedEventEmitter" /* 4697 */;

const pollConnectionStatsDefault = tmp10(4703);
require = fn;
    ...
    obj.handleRemoveListener = function handleRemoveListener(arg0) {
      if (arg0 === BaseConnectionEvent.MediaEngineEvent.VoiceActivity) {
        if (null != tmpResult.getVoiceEngine().setEmitVADLevel2) {
          ...
          voiceEngine.setEmitVADLevel2(obj.listenerCount(tmp(4694).MediaEngineEvent.VoiceActivity) > 0);
          const tmpResult3 = tmp(1911);
        } else {
          const voiceEngine1 = tmp(1911).getVoiceEngine();
          voiceEngine1.setEmitVADLevel(obj.listenerCount(tmp(4694).MediaEngineEvent.VoiceActivity) > 0, false, {});
      ...
      if (c3 === 2) {
        ...
        try {
          ...
          if (0 === dependencyMap) {
            ...
            closure_129_0.emit(v0(4694).MediaEngineEvent.VoiceQueueMetrics, closure_128_1);
...
prototype["connect"] = function connect(arg0, arg1, videoSupported) {
  ...
  obj3 = self(4705).create(arg0, arg1, videoSupported, flag);
  obj3.on(obj3(4694).BaseConnectionEvent.Destroy, (arg0) => {
  ...
  obj3.on(obj3(4694).BaseConnectionEvent.Connected, () => {
    obj3.setVideoBroadcast(self.shouldConnectionBroadcastVideo(obj3));
  });
  obj3.on(obj3(4694).BaseConnectionEvent.Silence, (arg0) => {
  ...
  self.emit(obj3(4694).MediaEngineEvent.Connection, obj3);
  return obj3;
};
...
prototype["setAudioInputDevice"] = function setAudioInputDevice(audioInputDeviceId) {
  ...
  if (obj.supportsFeature(constants6.SET_AUDIO_DEVICE_BY_ID)) {
    ...
    const audioInputDevices = tmp(4765).getAudioInputDevices();
...
prototype["setAudioOutputDevice"] = function setAudioOutputDevice(audioOutputDeviceId) {
  ...
  if (obj.supportsFeature(constants6.SET_AUDIO_DEVICE_BY_ID)) {
    ...
    const audioOutputDevices = tmp(4765).getAudioOutputDevices();
...
prototype["setLoopback"] = function setLoopback(arg0, arg1) {
  ...
  if (null != tmp2Result4.getVoiceEngine().setEmitVADLevel2) {
    const voiceEngine1 = tmp2(1911).getVoiceEngine();
    if (!tmp) {
      tmp = self.listenerCount(tmp2(4694).MediaEngineEvent.VoiceActivity) > 0;
    }
    ...
    let tmp6 = tmp;
    if (!tmp) {
      tmp6 = self.listenerCount(tmp2(4694).MediaEngineEvent.VoiceActivity) > 0;
    }
...
prototype["createReplayConnection"] = function createReplayConnection(arg0, arg1) {
  ...
  let tmp2 = null;
  if (null != replay) {
    replay.on(self(4694).BaseConnectionEvent.Destroy, (arg0) => {
      const connections = self.connections;
      connections.delete(arg0);
      if (self.connectionsEmpty()) {
        inject.setProcessPriority(constants.NORMAL);
    ...
    self.emit(self(4694).MediaEngineEvent.Connection, replay);
    tmp2 = replay;
...
prototype["stopLocalAudioRecording"] = function stopLocalAudioRecording(arg0) {
  ...
  let tmp5 = this.listenerCount(tmp(4694).MediaEngineEvent.VoiceActivity) > 0;
  if (tmp5) {
