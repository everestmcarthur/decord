...
// Function ID: 4695
// Name: destroy
// Dependencies: [32, 5, 4663, 4695, 4696, 4697, 4701, 4, 1911, 4693, 4702, 1340, 1365, 4704, 4764, 2]
...
import VideoDefault from "Video" /* 4697 */;
import CameraDefault from "Camera" /* 4701 */;
import ConnectionDefault from "Connection" /* 4704 */;
import Devices from "Devices" /* 4764 */;
import _slicedToArray from "module_32" /* 32 */;
import asyncGeneratorStep from "asyncGeneratorStep" /* 5 */;
import TypedEventEmitter from "TypedEventEmitter" /* 4696 */;

const pollConnectionStatsDefault = tmp10(4702);
require = fn;
    ...
    obj.handleRemoveListener = function handleRemoveListener(arg0) {
      if (arg0 === BaseConnectionEvent.MediaEngineEvent.VoiceActivity) {
        if (null != tmpResult.getVoiceEngine().setEmitVADLevel2) {
          ...
          voiceEngine.setEmitVADLevel2(obj.listenerCount(tmp(4693).MediaEngineEvent.VoiceActivity) > 0);
          const tmpResult3 = tmp(1911);
        } else {
          const voiceEngine1 = tmp(1911).getVoiceEngine();
          voiceEngine1.setEmitVADLevel(obj.listenerCount(tmp(4693).MediaEngineEvent.VoiceActivity) > 0, false, {});
...
prototype["setLoopback"] = function setLoopback(arg0, arg1) {
  ...
  if (null != tmp2Result4.getVoiceEngine().setEmitVADLevel2) {
    const voiceEngine1 = tmp2(1911).getVoiceEngine();
    if (!tmp) {
      tmp = self.listenerCount(tmp2(4693).MediaEngineEvent.VoiceActivity) > 0;
    }
    ...
    let tmp6 = tmp;
    if (!tmp) {
      tmp6 = self.listenerCount(tmp2(4693).MediaEngineEvent.VoiceActivity) > 0;
    }
...
prototype["stopLocalAudioRecording"] = function stopLocalAudioRecording(arg0) {
  ...
  let tmp5 = this.listenerCount(tmp(4693).MediaEngineEvent.VoiceActivity) > 0;
  if (tmp5) {
