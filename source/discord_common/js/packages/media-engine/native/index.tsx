...
// Function ID: 4697
// Name: destroy
// Dependencies: [32, 5, 4665, 4697, 4698, 4699, 4703, 4, 1911, 4695, 4704, 1340, 1365, 4706, 4766, 2]
...
import VideoDefault from "Video" /* 4699 */;
import CameraDefault from "Camera" /* 4703 */;
import ConnectionDefault from "Connection" /* 4706 */;
import Devices from "Devices" /* 4766 */;
import _slicedToArray from "module_32" /* 32 */;
import asyncGeneratorStep from "asyncGeneratorStep" /* 5 */;
import TypedEventEmitter from "TypedEventEmitter" /* 4698 */;

const pollConnectionStatsDefault = tmp10(4704);
require = fn;
    ...
    obj.handleRemoveListener = function handleRemoveListener(arg0) {
      if (arg0 === BaseConnectionEvent.MediaEngineEvent.VoiceActivity) {
        if (null != tmpResult.getVoiceEngine().setEmitVADLevel2) {
          ...
          voiceEngine.setEmitVADLevel2(obj.listenerCount(tmp(4695).MediaEngineEvent.VoiceActivity) > 0);
          const tmpResult3 = tmp(1911);
        } else {
          const voiceEngine1 = tmp(1911).getVoiceEngine();
          voiceEngine1.setEmitVADLevel(obj.listenerCount(tmp(4695).MediaEngineEvent.VoiceActivity) > 0, false, {});
...
prototype["setLoopback"] = function setLoopback(arg0, arg1) {
  ...
  if (null != tmp2Result4.getVoiceEngine().setEmitVADLevel2) {
    const voiceEngine1 = tmp2(1911).getVoiceEngine();
    if (!tmp) {
      tmp = self.listenerCount(tmp2(4695).MediaEngineEvent.VoiceActivity) > 0;
    }
    ...
    let tmp6 = tmp;
    if (!tmp) {
      tmp6 = self.listenerCount(tmp2(4695).MediaEngineEvent.VoiceActivity) > 0;
    }
...
prototype["stopLocalAudioRecording"] = function stopLocalAudioRecording(arg0) {
  ...
  let tmp5 = this.listenerCount(tmp(4695).MediaEngineEvent.VoiceActivity) > 0;
  if (tmp5) {
