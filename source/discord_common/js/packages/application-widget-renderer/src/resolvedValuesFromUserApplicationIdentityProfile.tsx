...
// Dependencies: [32, 9019, 9102, 2]
// Exports: default

// Module 9101 (discord_common/resolvedValuesFromUserApplicationIdentityProfile)
import resolvedValues from "resolvedValues" /* 9019 */;
import ProfileDataDynamicType from "ProfileDataDynamicType" /* 9102 */;
  ...
  if (null == dynamic) {
    ...
    while (iter2 !== undefined) {
      ...
      if (nextResult.type === ProfileDataDynamicType.ProfileDataDynamicType.STRING) {
        let obj2 = { type: tmp5(9019).ResolvedValueType.STRING, value: iter.value };
        obj[iter.name] = obj2;
      } else if (iter.type === tmp5(9102).ProfileDataDynamicType.NUMBER) {
        let obj3 = { type: tmp5(9019).ResolvedValueType.NUMBER, value: iter.value };
        obj[iter.name] = obj3;
      } else if (iter.type === tmp5(9102).ProfileDataDynamicType.MEDIA) {
        if (isVisualUnfurledMedia(iter.value)) {
          let obj4 = { type: tmp5(9019).ResolvedValueType.MEDIA, media: null };
