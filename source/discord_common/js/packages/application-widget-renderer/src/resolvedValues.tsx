...
// Function ID: 9084
// Name: resolvedValues
// Dependencies: [9084, 9085, 2]
// Exports: bindResolveFieldValue

// Module 9083 (resolvedValues)
import ApplicationWidgetFieldPresentationType from "ApplicationWidgetFieldPresentationType" /* 9084 */;
import ApplicationWidgetFieldValueType from "ApplicationWidgetFieldValueType" /* 9085 */;
...
function resolveFieldValue(image, items, applicationAssets) {
  ...
  if (null == image) {
    ...
    return tmp10;
  } else if (image.value_type === tmp19(9085).ApplicationWidgetFieldValueType.CUSTOM_STRING) {
    let tmp6 = null;
    if (image.presentation_type === tmp19(9084).ApplicationWidgetFieldPresentationType.TEXT) {
      tmp6 = null;
      if (items.includes(obj.STRING)) {
        const obj5 = { type: tmp7.STRING, value: image.value, presentationType: tmp19(9084).ApplicationWidgetFieldPresentationType.TEXT };
        tmp6 = obj5;
      }
      tmp7 = obj;
    }
    return tmp6;
  } else if (image.value_type === tmp19(9085).ApplicationWidgetFieldValueType.APPLICATION_ASSET) {
    if (items.includes(obj.MEDIA)) {
      ...
      if (null != found) {
        ...
        obj.media = size;
        obj.presentationType = tmp19(9084).ApplicationWidgetFieldPresentationType.IMAGE;
        tmp5 = obj;
      }
      return tmp5;
    } else {
