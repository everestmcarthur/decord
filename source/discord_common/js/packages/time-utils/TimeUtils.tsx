...
// Function ID: 4668
// Name: TimeUtils
// Dependencies: [5, 4668, 569, 4669, 2]
// Exports: convertMinutesToGivenTimeUnit, getTimeAndUnit, getTimeUnit

// Module 4667 (TimeUtils)
import u from "u" /* 569 */;
import findLastIndexDefault from "findLastIndex" /* 4669 */;
...
export const getTimeUnit = function getTimeUnit(arg0, arg1) {
  closure_0 = arg0;
  importDefault = arg1;
  const tmp2 = findLastIndexDefault(items, (unit) => f79269(unit.unit), items.findIndex((max) => {
    max = max.max;
  ...
  if (null != tmp2) {
    ...
    const found = arr.find((unit) => f79269(unit.unit));
    let unit = null;
    if (null != found) {
      unit = found.unit;
    }
...
export const getTimeAndUnit = function getTimeAndUnit(rounded, items) {
  ...
  if (null == rounded) {
    ...
    const tmp12 = f79269(4669)(items, (unit) => f79269(unit.unit), items.findIndex((max) => {
      max = max.max;
      let tmp = max.unit === obj.NONE;
      if (tmp) {
        tmp = closure_0 === max;
