...
// Function ID: 4590
// Name: sleep
// Dependencies: [5, 4590, 569, 4591, 2]
// Exports: convertMinutesToGivenTimeUnit, getTimeAndUnit, getTimeUnit

// Module 4589 (sleep)
import u from "u" /* 569 */;
import findLastIndexDefault from "findLastIndex" /* 4591 */;
import closure_3 from "asyncGeneratorStep" /* 5 */;
import tryCatch from "tryCatch" /* 4590 */;
...
export const getTimeUnit = function getTimeUnit(arg0, arg1) {
  closure_0 = arg0;
  importDefault = arg1;
  const tmp2 = findLastIndexDefault(items, (unit) => f78414(unit.unit), items.findIndex((max) => {
    max = max.max;
  ...
  if (null != tmp2) {
    ...
    const found = arr.find((unit) => f78414(unit.unit));
    let unit = null;
    if (null != found) {
      unit = found.unit;
    }
...
export const getTimeAndUnit = function getTimeAndUnit(rounded, items) {
  ...
  if (null == rounded) {
    ...
    closure_0 = rounded;
    const f78414 = (arg0) => closure_0.includes(arg0);
    const tmp12 = f78414(4591)(items, (unit) => f78414(unit.unit), items.findIndex((max) => {
      max = max.max;
      let tmp = max.unit === closure_1_10.NONE;
      if (tmp) {
        tmp = closure_0 === max;
