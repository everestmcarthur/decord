...
export const getTimeUnit = function getTimeUnit(arg0, arg1) {
  closure_0 = arg0;
  importDefault = arg1;
  const tmp2 = findLastIndexDefault(items, (unit) => f78410(unit.unit), items.findIndex((max) => {
    max = max.max;
  ...
  if (null != tmp2) {
    ...
    const found = arr.find((unit) => f78410(unit.unit));
    let unit = null;
    if (null != found) {
      unit = found.unit;
    }
...
export const getTimeAndUnit = function getTimeAndUnit(rounded, items) {
  ...
  if (null == rounded) {
    ...
    closure_0 = rounded;
    const f78410 = (arg0) => closure_0.includes(arg0);
    const tmp12 = f78410(4591)(items, (unit) => f78410(unit.unit), items.findIndex((max) => {
      max = max.max;
      let tmp = max.unit === closure_1_10.NONE;
      if (tmp) {
        tmp = closure_0 === max;
