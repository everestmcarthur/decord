...
// Dependencies: [5166, 5167, 1086, 5168, 5169, 5170, 5171, 5172, 5173, 2]
// Exports: contentClassificationToAgeRestriction

// Module 5165 (ContentClassificationToAgeRestriction)
import BigFlagUtilsAll from "BigFlagUtils" /* 1086 */;
import AgeRestrictionSource from "AgeRestrictionSource" /* 5166 */;
import AgeRestrictionStatus8 from "AgeRestrictionStatus" /* 5167 */;
import DiscordContentClassificationFlags from "DiscordContentClassificationFlags" /* 5168 */;
import ContentRatingESRBRating from "ContentRatingESRBRating" /* 5169 */;
import ContentRatingPEGIRating from "ContentRatingPEGIRating" /* 5170 */;
import ContentRatingGOPClassification from "ContentRatingGOPClassification" /* 5171 */;
import ContentRatingIGDBTheme from "ContentRatingIGDBTheme" /* 5172 */;
import AgeRestrictionUtilsAll from "AgeRestrictionUtils" /* 5173 */;
    ...
    if (data.type === obj.MINIMAL) {
      if (null != data.discord_classifications) {
        ...
        if (obj12.has(deserializeResult, DiscordContentClassificationFlags.DiscordContentClassificationFlags.EMERGENCY_ONLY_USE_IF_YOU_HAVE_TO_FORCE_MARK_AGE_RESTRICTED)) {
          ADULT = tmp28(5167).AgeRestrictionStatus.ADULT;
          obj.status = ADULT;
          let tmp12 = obj;
        } else {
          const tmp29Result = tmp29(1086);
          const AgeRestrictionStatus3 = tmp28(5167).AgeRestrictionStatus;
          obj.status = tmp29(1086).hasAny(deserializeResult, tmp28(5168).DiscordContentClassificationFlagMasks.RESTRICTED_TO_ADULT) ? AgeRestrictionStatus3.ADULT : AgeRestrictionStatus3.EVERYONE;
          tmp12 = obj;
          const hasAnyResult = tmp29(1086).hasAny(deserializeResult, tmp28(5168).DiscordContentClassificationFlagMasks.RESTRICTED_TO_ADULT);
        }
        items.push(tmp12);
      ...
      if (obj2.has(deserializeResult1, DiscordContentClassificationFlags.DiscordContentClassificationFlags.EMERGENCY_ONLY_USE_IF_YOU_HAVE_TO_FORCE_MARK_AGE_RESTRICTED)) {
        obj3.status = tmp4(5167).AgeRestrictionStatus.ADULT;
        let tmp9 = obj3;
      } else {
        const tmp6Result = tmp6(1086);
        const AgeRestrictionStatus2 = tmp4(5167).AgeRestrictionStatus;
        obj3.status = tmp6(1086).hasAny(deserializeResult1, tmp4(5168).DiscordContentClassificationFlagMasks.RESTRICTED_TO_ADULT) ? AgeRestrictionStatus2.ADULT : AgeRestrictionStatus2.EVERYONE;
        tmp9 = obj3;
        const hasAnyResult1 = tmp6(1086).hasAny(deserializeResult1, tmp4(5168).DiscordContentClassificationFlagMasks.RESTRICTED_TO_ADULT);
      }
      items.push(tmp9);
      tmp6 = importAll;
      ...
      if (obj10.has(deserializeResult2, DiscordContentClassificationFlags.DiscordContentClassificationFlags.EMERGENCY_ONLY_USE_IF_YOU_HAVE_TO_FORCE_MARK_AGE_RESTRICTED)) {
        obj4.status = tmp24(5167).AgeRestrictionStatus.ADULT;
        let tmp2 = obj4;
      } else {
        const tmp26Result = tmp26(1086);
        const AgeRestrictionStatus = tmp24(5167).AgeRestrictionStatus;
        obj4.status = tmp26(1086).hasAny(deserializeResult2, tmp24(5168).DiscordContentClassificationFlagMasks.RESTRICTED_TO_ADULT) ? AgeRestrictionStatus.ADULT : AgeRestrictionStatus.EVERYONE;
        tmp2 = obj4;
        const hasAnyResult2 = tmp26(1086).hasAny(deserializeResult2, tmp24(5168).DiscordContentClassificationFlagMasks.RESTRICTED_TO_ADULT);
      }
      items.push(tmp2);
        ...
        if (null == data.agency_ratings.pegi) {
          if (null == data.agency_ratings.gop) {
            if (null != data.agency_ratings.igdb) {
              ...
              const obj5 = { source: tmp21(5166).AgeRestrictionSource.AGENCY_CLASSIFICATION_IGDB, status: EVERYONE };
