...
// Dependencies: [9927, 7732, 1957, 5478, 2021, 4245, 4255, 1074, 7809, 9915, 5570, 1369, 504, 573, 2]

// Module 9926 (InviteSuggestionsStore)
...
const inviteSuggestionsStore = new InviteSuggestionsStore(DispatcherDefault, {
  LOAD_INVITE_SUGGESTIONS: function refreshInviteSuggestions(arg0) {
    ...
    const tmp4 = (function _computeRows(query) {
      ...
      const mostRecentDMedUser = set1(9915).getMostRecentDMedUser(omitUserIds, tmp);
      ...
      const obj2 = set1(9915);
      return set1(9915).generateRowsForQuery({ query, omitUserIds, suggestedUserIds: set, maxRowsWithoutQuery: 100, omitGuildId: tmp, suggestedChannelIds: set1, inviteTargetType });
    })(query);
    rows = tmp4.rows;
    if (closure_4) {
    ...
    (function requestMembership(rows) {
      ...
      if (null != tmp) {
        ...
        while (iter !== undefined) {
          let tmp5 = nextResult;
          let tmp6 = id;
          let tmp8 = nextResult.type !== id(9915).RowTypes.DM;
          if (tmp8) {
            tmp8 = tmp5.type !== tmp6(9915).RowTypes.FRIEND;
          }
          if (!tmp8) {
