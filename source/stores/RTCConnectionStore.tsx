...
// Dependencies: [4656, 502, 4663, 1074, 4664, 3, 4666, 4693, 573, 13918, 1953, 4668, 5496, 13919, 504, 13921, 1241, 1897, 2]

// Module 4662 (RTCConnectionStore)
import LoggerDefault from "Logger" /* 3 */;
import initializeDefault from "initialize" /* 504 */;
import DispatcherDefault from "Dispatcher" /* 573 */;
import TimeUtils from "TimeUtils" /* 4668 */;
import VoiceStateAnalyticsDefault from "VoiceStateAnalytics" /* 13918 */;
import useSpatialAudioControlState from "useSpatialAudioControlState" /* 13919 */;
import trackVideoToggle from "trackVideoToggle" /* 13921 */;
...
function createRTCConnection(guildId, channelId, createdTime) {
  if (null == sessionId) {
    ...
    _default = new _default(4666).default(obj);
    _default.on(_default(4693).RTCConnectionEvent.State, (state, arg1, arg2) => {
      closure_1 = arg1;
      dependencyMap = arg2;
      closure_1(573).wait(() => {
    ...
    _default.on(_default(4693).RTCConnectionEvent.Video, (guildId, channelId, userId, streamId, rtcServerId) => {
    ...
    _default.on(_default(4693).RTCConnectionEvent.Ping, (pings, quality) => {
      quality(573).wait(() => DispatcherDefault.dispatch({ type: "RTC_CONNECTION_PING", pings, quality }));
    });
    _default.on(_default(4693).RTCConnectionEvent.OutboundLossRate, (lossRate) => {
      closure_1(573).wait(() => DispatcherDefault.dispatch({ type: "RTC_CONNECTION_LOSS_RATE", lossRate }));
    });
    _default.on(_default(4693).RTCConnectionEvent.Speaking, (userId, speaking) => {
      if (speaking != null) {
        speaking.setSpeaking(userId, speaking);
      }
    });
    _default.on(_default(4693).RTCConnectionEvent.Flags, (userId, flags) => {
      DispatcherDefault.wait(() => {
    ...
    _default.on(_default(4693).RTCConnectionEvent.UsersMerged, (userIds, context) => {
      DispatcherDefault.dispatch({ type: "RTC_CONNECTION_USERS_MERGED", userIds, context });
    });
    _default.on(_default(4693).RTCConnectionEvent.ClientConnect, (userIds) => {
      DispatcherDefault.wait(() => {
    ...
    _default.on(_default(4693).RTCConnectionEvent.ClientDisconnect, (userId) => {
      DispatcherDefault.wait(() => {
    ...
    _default.on(_default(4693).RTCConnectionEvent.Platform, (userId, platform, channelId) => {
      dependencyMap = channelId;
      platform(573).wait(() => {
        DispatcherDefault.dispatch({ type: "RTC_CONNECTION_PLATFORM", platform, userId, channelId });
      });
    });
    _default.on(_default(4693).RTCConnectionEvent.SecureFramesUpdate, () => {
      DispatcherDefault.wait(() => {
        closure_1_1(dependencyMap[8]).dispatch({ type: "RTC_CONNECTION_SECURE_FRAMES_UPDATE" });
      });
    });
    _default.on(_default(4693).RTCConnectionEvent.RosterMapUpdate, (userIds) => {
      closure_1(573).wait(() => {
...
fn(1897)(1241, dependencyMap.paths).then((addExtraAnalyticsDecorator) => {
