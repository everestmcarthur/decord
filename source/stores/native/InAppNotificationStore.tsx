...
// Dependencies: [1957, 4655, 10211, 7222, 7644, 1220, 1958, 1980, 10212, 4654, 2012, 1952, 1372, 1074, 8124, 8122, 4231, 12, 10213, 10215, 7436, 7377, 10216, 9742, 4862, 10224, 10225, 10227, 10220, 10228, 10232, 4793, 1936, 1186, 7960, 10233, 4819, 11, 504, 573, 2]

// Module 10210 (InAppNotificationStore)
...
import GuildAntiRaidUtils from "GuildAntiRaidUtils" /* 8122 */;
import GuildAntiRaidTypes from "GuildAntiRaidTypes" /* 8124 */;
import ExternalPipDefault from "ExternalPip" /* 9742 */;
import RestrictedScheduleNotificationUtils from "RestrictedScheduleNotificationUtils" /* 10213 */;
import MessageUtils from "MessageUtils" /* 10215 */;
import NotificationTextUtils from "NotificationTextUtils" /* 10216 */;
...
const playInAppMessageSound = tmp(10233);
require = fn;
function handleAlertMessage() {
  if (obj.allowInAppNotifications()) {
    const result = tmp(10228).shouldShowRaidInAppNotification();
    ...
    if (result.show) {
      if (null != guildId) {
        ...
        if (null == guild) {
          ...
          channel = ChannelStore.getChannel(channel(10232)(guild));
          if (null == channel) {
            return false;
            ...
            if (merged.wasRecentlyDismissed(obj2)) {
              ...
              const obj4 = {
                type: ALERT,
                key: guildId,
                duration: tmp(10225).getNotificationDuration(ALERT),
                onDismiss() {
              ...
              const tmpResult3 = tmp(10225);
              obj4.inAppNotificationId = tmp(10225).generateInAppNotificationId();
...
let closure_6 = fn(7222).trackMessageNotificationTimestamps;
...
let obj = {};
obj[fn(8124).GuildIncidentAlertTypes.JOIN_RAID] = new Map();
let map = new Map();
obj[fn(8124).GuildIncidentAlertTypes.DM_RAID] = new Map();
merged[0] = obj;
merged[1] = { amount: 1, unitOfTime: "hour" };
let c21 = null;
...
let c24 = false;
let EMPTY_SCHEDULE_SNAPSHOT = fn(10213).EMPTY_SCHEDULE_SNAPSHOT;
...
const inAppNotificationStore = new InAppNotificationStore(DispatcherDefault, {
          ...
          if (tmpResult8.allowInAppNotifications()) {
            if (!tmp3Result.isEnabled()) {
              if (!ChannelRTCStore.getChatOpen(channel_id)) {
                const result = tmp(10216).shouldIncludeSelectedChannel();
                const tmpResult9 = tmp(10216);
                if (tmpResult10.shouldNotify(message, channel_id, result)) {
                  const channel = ChannelStore.getChannel(channel_id);
                  if (null == channel) {
                    return false;
                  } else {
                    const messageRecord = tmp(4862).createMessageRecord(message);
                    const tmpResult11 = tmp(4862);
                    if (tmpResult12.isMessageContentPreviewable(messageRecord)) {
                      ...
                      const notificationDuration = tmp(10225).getNotificationDuration(MESSAGE);
                      ...
                      const tmpResult13 = tmp(10225);
                      obj3.inAppNotificationId = tmp(10225).generateInAppNotificationId();
                    ...
                    tmpResult12 = tmp(10224);
                  }
                  obj7 = ChannelStore;
                } else {
                  return false;
                }
                tmpResult10 = tmp(10216);
              }
            }
            tmp3Result = ExternalPipDefault;
          }
          tmpResult8 = tmp(10216);
        }
        return false;
      }
      tmpResult = tmp(7436);
  ...
  MESSAGE_ACK: function handleMessageAck(channelId) {
    ...
    if (tmp) {
      ...
      messageId(4819).trackWithMetadata(constants.IN_APP_NOTIFICATION_DISMISSED, obj4);
      const obj3 = messageId(4819);
    }
      ...
      if (null != message.reactions) {
        if (null != emoji) {
          if (obj14.allowInAppNotifications()) {
            let tryParseChannelPathResult = tmp19(4793).tryParseChannelPath(tmp);
                  ...
                  if (null != channel) {
                    if (!obj4.isEnabled()) {
                      if (!ChannelRTCStore.getChatOpen(channelId)) {
                        const result1 = tmp19(10216).shouldIncludeSelectedChannel();
                        const tmp19Result9 = tmp19(10216);
                        const obj = { message, channel, reactor: user, includeSelectedChannel: result1 };
                        if (tmp19Result10.shouldNotifyForReaction(obj)) {
                          const messageRecord = tmp19(4862).createMessageRecord(message);
                          const tmp19Result11 = tmp19(4862);
                          if (tmp19Result12.isMessageContentPreviewable(messageRecord)) {
                            ...
                            const obj3 = {
                              type: REACTION,
                              key: channelId,
                              duration: tmp19(10225).getNotificationDuration(REACTION),
                              onDismiss() {
                                                        InAppNotificationActionCreatorsDefault.clearNotification();
                                                      },
                              channel,
                            ...
                            const tmp19Result13 = tmp19(10225);
                            obj3.inAppNotificationId = tmp19(10225).generateInAppNotificationId();
                            obj2.notification = obj3;
                            handleEnqueueNotification(obj2);
                          } else {
                            return false;
                          }
                          tmp19Result12 = tmp19(10224);
                        } else {
                          return false;
                        }
                        tmp19Result10 = tmp19(10216);
                      }
                    }
                    return false;
                  }
      ...
      if (null != savedMessage.message) {
        ...
        if (null != channel) {
          ...
          const obj3 = {
            ...
            duration: tmp(10225).getNotificationDuration(MESSAGE_REMINDER),
            onDismiss() {
                    InAppNotificationActionCreatorsDefault.clearNotification();
                  },
            channel,
              ...
              if (null == user) {
                ...
                if (null == guild) {
                  ...
                  const obj3 = {
                    ...
                    guild,
                    key: channel.id,
                    duration: tmp(10225).getNotificationDuration(FORUM_THREAD_CREATED),
                    onDismiss() {
                                    InAppNotificationActionCreatorsDefault.clearNotification();
                                  },
                    inAppNotificationId: null
                  };
                  const tmpResult5 = tmp(10225);
                  obj3.inAppNotificationId = tmp(10225).generateInAppNotificationId();
  ...
  CHANNEL_SELECT: function handleChannelSelect(channelId) {
    ...
    if (tmp) {
      const result = channelId(10225).extractMetadataFromNotification(_null);
      ...
      if (null != diffSchedulesResult) {
        ...
        if (EnableScreenDowntimeScheduleNotifications.getSetting()) {
          if (tmp2Result7.allowInAppNotifications()) {
            ...
            const notificationDuration = tmp2(10225).getNotificationDuration(RESTRICTED_SCHEDULE_UPDATED);
            const tmp2Result8 = tmp2(10225);
            obj3.key = tmp2(10213).restrictedScheduleNotificationKey(diffSchedulesResult.kind);
            obj3.duration = notificationDuration;
            obj3.onDismiss = function onDismiss() {
              InAppNotificationActionCreatorsDefault.clearNotification();
            };
            const tmp2Result9 = tmp2(10213);
            obj3.title = tmp2(10213).getRestrictedScheduleNotificationTitle(diffSchedulesResult.kind);
            const tmp2Result10 = tmp2(10213);
            obj3.subtitle = tmp2(10213).getRestrictedScheduleNotificationSubtitle(diffSchedulesResult.rule);
            const tmp2Result11 = tmp2(10213);
            obj3.inAppNotificationId = tmp2(10225).generateInAppNotificationId();
            obj2.notification = obj3;
            handleEnqueueNotification(obj2);
            const tmp2Result12 = tmp2(10225);
          }
          tmp2Result7 = tmp2(10216);
        }
      }
    } else {
      return false;
